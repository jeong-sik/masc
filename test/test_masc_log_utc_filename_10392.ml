...
let test_kst_midnight_lands_in_utc_25 () =
  check
    string
    "KST midnight is still UTC date 2026-04-25"
    "2026-04-25"
...
let test_kst_midnight_minus_1s_also_25 () =
  check
    string
    "1 second before KST midnight is also UTC 2026-04-25"
    "2026-04-25"
...
let test_kst_midnight_plus_1s_still_25 () =
  check
    string
    "1 second after KST midnight is still UTC 2026-04-25"
    "2026-04-25"
    (L.format_utc_date_of (kst_midnight_utc +. 1.0))
;;
...
let test_known_utc_morning () =
  check
    string
    "2026-04-25T04:00:00Z formats as 2026-04-25"
    "2026-04-25"
...
let test_utc_day_boundary () =
  ...
  check
    string
    "UTC midnight rolls the date over"
    "2026-04-26"
...
let () =
  run
    "masc_log_utc_filename_10392"
    [ ( "utc_filename"
      , [ test_case
            "KST midnight lands in UTC date -1"
            `Quick
            test_kst_midnight_lands_in_utc_25
        ; test_case "1s before KST midnight" `Quick test_kst_midnight_minus_1s_also_25
        ; test_case
            "1s after KST midnight (UTC 15:00:01)"
            `Quick
            test_kst_midnight_plus_1s_still_25
        ; test_case "known UTC morning timestamp" `Quick test_known_utc_morning
        ; test_case "UTC midnight rolls forward" `Quick test_utc_day_boundary
        ] )
    ]
;;
