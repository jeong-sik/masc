...
let test_malformed_manifest () =
  ...
  let run_dir =
    match
      Proof_artifact_reader.run_artifact_path store ~run_id ~relative_path:"manifest.json"
    with
...
let test_malformed_contract () =
  ...
  let contract_path =
    match
      Proof_artifact_reader.run_artifact_path store ~run_id ~relative_path:"contract.json"
    with
    | Ok path -> path
    | Error msg -> Alcotest.fail msg
  in
...
let test_contract_id_roundtrip () =
  ...
  | Ok bundle ->
    ...
    let recomputed_again =
      Masc_mcp_cdal_runtime.Risk_contract.contract_id bundle.contract
    in
...
let test_manifest_is_truth_source () =
  ...
  let manifest_proof =
    make_proof
      ...
      ~contract_id
      ~requested:Masc_mcp_cdal_runtime.Execution_mode.Execute
      ~effective:Masc_mcp_cdal_runtime.Execution_mode.Draft
      ()
  in
  let input_proof =
    make_proof
      ...
      ~effective:Masc_mcp_cdal_runtime.Execution_mode.Diagnose
      ()
  in
  Masc_mcp_cdal_runtime.Proof_store.init_run store ~run_id;
  ...
  | Ok bundle ->
    Alcotest.(check string)
      ...
      "execute"
      (Masc_mcp_cdal_runtime.Execution_mode.to_string
         bundle.proof.requested_execution_mode);
    Alcotest.(check string)
      "effective from manifest"
      "draft"
      (Masc_mcp_cdal_runtime.Execution_mode.to_string
         bundle.proof.effective_execution_mode)
  | Error e -> Alcotest.fail (CL.load_error_to_string e)
;;
