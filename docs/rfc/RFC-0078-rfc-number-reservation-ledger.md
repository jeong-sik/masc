...
Multi-phase RFC 패턴 (예: `RFC-0058-declarative-runtime-config.md` + `RFC-0058-phase-5-...md`) 은 *명시적 opt-in* 으로 허용 — PR body 에 `RFC-EXTEND: NNNN` 라인 (또는 본문 frontmatter `extends: "NNNN"`) 이 있으면 main 의 기존 NNNN 위에 phase 추가 가능. 옵트인 없이 같은 NNNN 점유 시 fail.
