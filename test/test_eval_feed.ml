...
let test_parse_valid_verdict () =
  ...
  | Ok verdict ->
    ...
    check (list string) "first evidence" [ "correct tool chosen" ] first.evidence;
...
let test_parse_wrong_schema_version () =
  ...
  | Error msg ->
    check bool "error mentions schema_version" true (String.length msg > 0);
    check
      bool
      "error mentions version 2"
      true
      (try
...
let test_parse_missing_schema_version () =
  let json =
    Yojson.Safe.from_string {|{"all_passed": true, "coverage": 0.5, "layer_results": []}|}
  in
  match Eval_feed.read_verdict_json json with
...
let test_parse_layer_missing_name () =
  ...
  | Error msg ->
    check
      bool
      "error mentions layer_name"
      true
      (try
         ignore (Str.search_forward (Str.regexp_string "layer_name") msg 0);
         true
       with
       | Not_found -> false)
;;
...
let test_read_latest_nonexistent_dir () =
  let result =
    Eval_feed.read_latest ~base_path:"/nonexistent/path" ~agent_name:"keeper-a" ~limit:10
  in
  check int "empty result" 0 (List.length result)
;;
...
let test_read_latest_with_files () =
  ...
  let eval_dir =
    Filename.concat (Filename.concat (Filename.concat base ".oas") "eval") "keeper-a"
  in
  mkdir_p eval_dir;
  ...
  let results = Eval_feed.read_latest ~base_path:base ~agent_name:"keeper-a" ~limit:10 in
...
let test_read_latest_with_limit () =
  ...
  let eval_dir =
    Filename.concat (Filename.concat (Filename.concat base ".oas") "eval") "keeper-b"
  in
  mkdir_p eval_dir;
  for i = 1 to 5 do
    write_file
  ...
  let results = Eval_feed.read_latest ~base_path:base ~agent_name:"keeper-b" ~limit:2 in
...
let test_snapshot_json_roundtrip () =
  ...
  | Ok verdict ->
    ...
    let agent = Safe_ops.json_string ~default:"" "agent_name" json_out in
    check string "agent_name roundtrip" "keeper-a" agent;
    let verdict_j = Yojson.Safe.Util.member "verdict" json_out in
    let sv = Safe_ops.json_int ~default:0 "schema_version" verdict_j in
  ...
  run
    ...
    [ ( "verdict_parsing"
      , [ test_case "valid verdict" `Quick test_parse_valid_verdict
        ; test_case "wrong schema_version" `Quick test_parse_wrong_schema_version
        ; test_case "missing schema_version" `Quick test_parse_missing_schema_version
        ; test_case "empty layer_results" `Quick test_parse_empty_layer_results
    ...
    ; ( "read_latest"
      , [ test_case "empty directory" `Quick test_read_latest_empty_dir
        ; test_case "nonexistent directory" `Quick test_read_latest_nonexistent_dir
