...
let broadcast_cached_surface ~event_type (json : Yojson.Safe.t) : unit =
  ...
  else Log.Dashboard.debug "%s: payload unchanged, skipping broadcast" event_type
;;
...
let invalidate_execution_cache () =
  (try invalidate_cached_surface _execution_cache with
   | Eio.Cancel.Cancelled _ as e -> raise e
   | exn ->
     Log.Dashboard.error
       "Failed to invalidate execution surface cache: %s"
       (Printexc.to_string exn));
  try Dashboard_cache.invalidate "execution:default:light" with
  | Eio.Cancel.Cancelled _ as e -> raise e
  | exn ->
    Log.Dashboard.error
      "Failed to invalidate dashboard execution cache: %s"
      (Printexc.to_string exn)
;;
...
let paused_of_lifecycle_event = function
  | "started" | "restarted" | "reconciled" | "resumed" | "running" | "self_preservation"
    -> Some false
