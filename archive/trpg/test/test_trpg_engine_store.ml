...
let test_append_and_read_events () =
  ...
  get_ok (Trpg.Engine_store.append_event ~base_dir ~event:(make_event ~seq:1 ~room_id));
  get_ok (Trpg.Engine_store.append_event ~base_dir ~event:(make_event ~seq:2 ~room_id));
  get_ok (Trpg.Engine_store.append_event ~base_dir ~event:(make_event ~seq:3 ~room_id));

  let all = get_ok (Trpg.Engine_store.read_events ~base_dir ~room_id) in
  Alcotest.(check int) "event count" 3 (List.length all);
  Alcotest.(check (list int)) "seq order" [ 1; 2; 3 ] (List.map seq_of_event all);

  let tail = get_ok (Trpg.Engine_store.read_events_after ~base_dir ~room_id ~after_seq:1) in
...
let test_snapshot_roundtrip () =
  ...
  in
  get_ok
    (Trpg.Engine_store.write_snapshot
       ~base_dir
       ~room_id
       ~last_seq:27
  ...
  let snap = get_ok (Trpg.Engine_store.read_snapshot ~base_dir ~room_id) in
  match snap with
  ...
  | Some s ->
      ...
      Alcotest.(check string)
        "phase"
        "round"
        (Trpg.Engine_types.string_of_phase s.state.phase);
...
let test_recovery_tail_events () =
  ...
  List.iter
    (fun seq -> get_ok (Trpg.Engine_store.append_event ~base_dir ~event:(make_event ~seq ~room_id)))
    [ 1; 2; 3; 4; 5 ];
  ...
  let snapshot_opt, tail = get_ok (Trpg.Engine_store.load_recovery ~base_dir ~room_id) in
