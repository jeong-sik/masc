...
let check t =
  if t.count >= t.limit
  then Hard_stop { count = t.count; limit = t.limit; cumulative_ms = t.cumulative_ms }
  else if t.count >= t.soft_limit
  then Soft_warning { remaining = t.limit - t.count; limit = t.limit }
  else Ok { remaining = t.soft_limit - t.count }
;;
...
let status_to_json = function
  ...
  | Soft_warning { remaining; limit } ->
    `Assoc
      ...
      ; "limit", `Int limit
      ; "suggestion", `String "Consider consolidating commands or summarizing progress"
      ]
  | Hard_stop { count; limit; cumulative_ms } ->
    `Assoc
      ...
      ; ( "suggestion"
        , `String "Turn execution budget reached. Report progress and wait for next turn."
        )
      ]
;;
