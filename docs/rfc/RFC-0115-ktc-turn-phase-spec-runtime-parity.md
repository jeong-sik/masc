...
4. **audit doc 가 3 invariant candidate 제공**: `RoutingRequiresToolPolicy`, `ExhaustedRequiresTerminalRuntime`, +1. RFC 가 spec PR 의 source-of-truth 정리.
...
7 invariant 의 quantification 범위 확장 + audit doc 의 3 candidate invariant 추가 (`RoutingRequiresToolPolicy`, `ExhaustedRequiresTerminalRuntime`, `RoutingRequiresLiveTurn`).
...
새 `turn_phase` variant (또는 `decision_stage`, `runtime_state`) 추가 시 의무:
...
1. **Q1**: `decision_stage` 와 `runtime_state` 도 같은 정책? 두 type 은 *현재* spec 과 1:1 정렬 — audit doc 가 ✅ aligned 표기. 그래도 codegen 적용 시 future drift 차단 가능. **잠정**: P3 의 codegen 가 3 type 모두 처리.
