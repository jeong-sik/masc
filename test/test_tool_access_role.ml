...
let testable_permission =
  testable (Fmt.of_to_string Types.show_permission) (fun a b -> a = b)
;;
...
let test_equivalence_admin () =
  ...
  List.iter
    (fun tool_name ->
       ...
       let new_result =
         Tool_access_policy.allows_name (Tool_access_role.policy_for_role Admin) tool_name
       in
       check bool (Printf.sprintf "Admin/%s" tool_name) old_result new_result)
    tools
;;
    ...
    (fun tool_name ->
       ...
       let new_result =
         Tool_access_policy.allows_name
           (Tool_access_role.policy_for_role Worker)
           tool_name
       in
       check bool (Printf.sprintf "Worker/%s" tool_name) old_result new_result)
    tools
;;
...
let test_empty_string_tool_allowed () =
  ...
  check
    bool
    "empty string tool allowed by All"
    true
...
let test_admin_only_count () =
  ...
  check
    bool
    "admin_only_tools has no duplicates"
    true
...
let test_worker_only_count () =
  ...
  check
    bool
    "worker_only_tools has no duplicates"
    true
...
let () =
  run
    ...
    ; "hierarchy", [ test_case "Worker ⊂ Admin" `Quick test_worker_subset_of_admin ]
    ; ( "role_boundaries"
      , [ test_case "Worker denies admin_only" `Quick test_worker_denies_admin_only_tools
        ; test_case "Admin allows admin_only" `Quick test_admin_allows_admin_only_tools
        ; test_case
            "Worker allows worker_only"
            `Quick
            test_worker_allows_worker_only_tools
        ; test_case
            "channel_gate requires worker"
            `Quick
            test_channel_gate_requires_worker
        ; test_case "portal tools require worker" `Quick test_portal_tools_require_worker
        ; test_case
            "permissions promoted to metadata ssot"
            `Quick
            test_permissions_promoted_to_metadata_ssot
        ] )
    ; ( "unregistered"
      , [ test_case
            "unregistered allowed (fail-open)"
            `Quick
            test_unregistered_tool_allowed_all_roles
        ; test_case "empty string tool" `Quick test_empty_string_tool_allowed
        ] )
    ; ( "integrity"
      , [ test_case "admin_only count" `Quick test_admin_only_count
        ; test_case "worker_only count" `Quick test_worker_only_count
        ; test_case "no overlap admin/worker" `Quick test_no_overlap_admin_worker
        ] )
    ]
;;
