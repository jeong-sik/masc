...
let compute_typed : type i o r s. (i, o, r, s) Typed.command -> command_descriptor =
  ...
  | Typed.Git_merge { squash; _ } ->
    Gh_pr_merge { pr_number = 0; squash }
    ...
    then (
      ...
      | Some repo ->
        (match method_ with
         ...
         | `PUT ->
           Gh_api_pr_merge { repo; pr_number = find_pr_number parts }
         | `DELETE ->
           Gh_api_pr_comment { repo; pr_number = find_pr_number parts; body = "" }
         | _ -> Generic)
      | None -> Generic)
    else Generic
  | Typed.Ls _
