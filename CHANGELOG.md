...
- Keeper: delete `<base-path>/.masc/keepers/*/turn-records` before installing. A turn record's window now carries the digest of the message at the front, and a record written by an earlier version has no such key, so not one line of it decodes — 18 keepers, 69,976 lines, 138 MB at the time of the change. `deploy.sh` refuses a keyless line in its pre-deploy check and stops before installing, so forgetting this halts the install rather than corrupting anything. What goes with the records: the dashboard's turn history, most of the raw traces (213 MB), and each keeper's first-turn seed — that first turn sends the whole history again. Rolling back to the earlier binary needs the same emptying; there is no compatibility reader and no strip script (#36955, #36949).
- Runtime: a model binding `[<provider>.<model>]` refuses a key it does not declare, and the load error names the path and lists every key the binding accepts. It used to drop the key without a word, so a typo such as `context-high-water-token` was configuration that did nothing. A nested table inside a binding and a non-table value under a provider table are refused the same way. The live config, the seed and all ten `runtime*.toml` in this repository carry no unknown key, no nested table and no stray scalar, so they load unchanged; a workspace that does carry one gets an error naming the line to delete. There is no ignore path for the old spelling (#36906).
- Librarian: absorbing memories writes their text to a new per-keeper file, `<keeper>.memory-absorbed.jsonl`, before removing them, and a failed write commits nothing — so an absorbed fact can no longer be dropped without its text surviving somewhere. The 2,409 facts dropped before this change (since #36855) are not recoverable. The Librarian's answer schema gains `absorbs` as a required field (#36937, #36936).

### Added

- Memory: `keeper_memory_search` takes `source=absorbed`, which reads the absorbed record, so text the Librarian folded into a new claim is findable after it leaves current memory. `source=all` includes it, ordered current memory, then absorbed, then conversation history. A search of the absorbed record emits no `Retrieved` event. Results carry `into`, `into_current`, `absorbed_at`, `store` and `absorbed_unreadable_lines` (#36948).
- TUI: Enter on a fact in the Memory screen opens that claim's full text across the terminal, where it was previously readable only in a narrow pane under the list and Enter did nothing. `j`/`k` and PgUp/PgDn move, `g`/`G` reach the ends, Esc returns to the list. `[?]` lists those keys beside the list's own, marked with the screen each belongs to, and the reading screen's status line reads the same table, so the help and the keys cannot drift apart (#36960, #36983).

### Changed

- Keeper: a chat operation whose last runtime candidate fails after saving tool results resumes on that same path, from its latest checkpoint, instead of ending the turn. A lane holding one candidate had nowhere to rotate to, so a failure that passes with time — a rate limit, a capacity refusal, a dropped transport — cost the operation the work it had already done. Resume reads the latest checkpoint, so tools already run are not run again, and each continuation needs a fresh set of tool results to reach the same place (#36971, #36926).
- Keeper: the max-tokens recovery asks whether the candidate's wire can be told to stop thinking before it sends a request saying so. Some rows declare a thinking control with no off state and a categorical effort row whose ladder omits the off value cannot spell the disable at all, so the recovery's request was refused before dispatch and the turn was spent — on a lane with one candidate, with nothing to rotate to. Where the disable cannot be expressed the rejected response is dropped from the checkpoint instead, which is the remedy that path already owed, and the turn continues. The manifest row says which of the two happened (#36985, #36972).
- Catalog: the OpenRouter provider declares its own accepted reasoning efforts, so a runtime on it can carry an effort at all — every effort-bearing request was refused as undeclared before. The router publishes one contract for everything it serves, accepting the ladder and mapping a level the chosen model lacks to the nearest one it has, which is why the set belongs to the provider rather than to a row per served model. The off value is deliberately not in it: the router taking `none` and the model stopping are different claims, and a model that cannot stop receives the neighbouring level and reasons on. A model whose off behaviour is documented or measured declares it on its own row (#36991).
- Keeper: the tokens a turn's first request spends on its `[system context]` are no longer counted as history. Of 536 evictions over 09-16 and 09-17, 75–76% evicted without knowing what they had evicted and 81% were followed by another request in the same keeper turn: the analyst evicted all 76 history atoms on a turn's first request and sent the next one at 28k tokens with `cache_read` 0. The ceiling is judged once per turn. The ledger's JSON gains a `turn_context` field (#36903, #36901).
- Keeper: a tool loaded mid-turn keeps its place in the next turn's array, so one case of the provider prompt cache breaking at a tool-loading turn boundary is gone. The cost is a median 143 bytes per request (p90 and maximum 375), a median 0.21% of the tool schema, whose own median is 73,139 bytes. The gain is established for Anthropic (confidence High) and Kimi (Medium); GLM and DeepSeek have to be read from `cached_tokens` after deployment. `keeper_tool_search` now lists every name, including the tools already placed (#36944, #36863).
- Provider: a generation the provider cut short is typed as what it is. An errored choice that carries no error object reads as an interrupted generation rather than an unclassified provider fault, and an incomplete stream routes as a transient network failure — the request was accepted and the bytes that would have said why it stopped never arrived, which is what a dropped transport looks like. The other wire failures are defects in what did arrive and stay terminal (#36975, #36926).
...
- Keeper: a refusal whose reason this codebase cannot type narrows the history range as a typed one does. From 02:20 KST on 09-18 five keepers — msx-retro-mania, won-chik, kidsnote-slack-context-collector, pr-updater, goo-yang-bong — sent 15 MB and 9.5 MB every twenty seconds, were refused, and completed no turn. Sending the whole history on a turn's first request is unchanged. Note what a narrowed range costs after the refusals stop: the next turn seeds from the narrowest range the unfinished turn reached, and an accepted range is the newer evidence, so a keeper that walked down under repeated refusals keeps working from there rather than widening again — the polisher fell from 1,885 to 236 atoms over four quota-refused turns while its history stayed at 7,537. Whether a refusal whose reason this codebase cannot type should move the range at all is still open in that RFC's question 13; it is taken for now because the only other lever on a refusal whose reason cannot be read is to send the same bytes again. One path widens a keeper back: restart the server so the ledger starts empty, and clear that keeper's `<keeper-dir>/<name>/turn-records/` so no seed is found. The next turn sends the whole history and the ceiling — or a refusal — cuts it again from there. The inspector's turn history for that keeper goes with the records, which is the same emptying this release already asks for once before installing (#36977, #36901).
