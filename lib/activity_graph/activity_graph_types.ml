...
let node_status_of_string s =
  ...
  | None ->
    Log.Misc.warn
      "activity_graph: unknown node_status wire %S -> Observed (drift; see #8777)"
      s;
    Observed
;;
...
let span_status_to_string = function
  ...
  | Span_released -> "released"
  | Span_cancelled -> "cancelled"
  | Span_left -> "left"
  | Span_retired -> "retired"
  | Span_finalized -> "finalized"
  | Span_stopped -> "stopped"
  | Span_ended -> "ended"
;;
...
let span_status_of_string s =
  ...
  | None ->
    Log.Misc.warn
      "activity_graph: unknown span_status wire %S -> Span_ended (drift; see #8605)"
      s;
    Span_ended
;;
...
let entity_of_yojson (json : Yojson.Safe.t) : entity_ref option =
  match Safe_ops.json_string_opt "kind" json, Safe_ops.json_string_opt "id" json with
...
let event_of_yojson (json : Yojson.Safe.t) : event option =
  match
    ( Safe_ops.json_int_opt "seq" json
  ...
  | Some seq, Some ts_ms, Some ts_iso, Some room_id, Some kind ->
    ...
    let subject =
      Option.bind (Safe_ops.json_member_opt "subject" json) entity_of_yojson
    in
    let payload =
      Safe_ops.json_member_opt "payload" json |> Option.value ~default:(`Assoc [])
    in
