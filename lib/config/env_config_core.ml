...
let strip_path_trailing_slashes value =
  ...
  let rec loop current =
    ...
    then loop (String.sub current 0 (len - 1))
    else current
  in
...
let expand_home_prefix value =
  if String.length value >= 2 && value.[0] = '~' && value.[1] = '/'
  then (
...
let normalize_masc_base_path_input path =
  ...
  if normalized = ""
  then ""
  else if String.equal (Filename.basename normalized) Common.masc_dirname
  then (
...
let existing_dir path = Sys.file_exists path && Sys.is_directory path
let existing_file path = Sys.file_exists path && not (Sys.is_directory path)
let home_dir_opt () = raw_value_opt "HOME" |> trim_opt
...
let warn_deprecated ~old_name ~new_name =
  if not (Hashtbl.mem deprecation_warned old_name)
  then (
    Hashtbl.replace deprecation_warned old_name true;
    Log.Misc.warn
      "env %s is deprecated; use %s instead. Support will be removed in a future release."
      old_name
      new_name)
;;
...
let masc_http_port_int () =
  Safe_ops.int_of_string_with_default
    ~default:Masc_network_defaults.masc_http_default_port
    (masc_http_port ())
;;

let masc_host_opt () = raw_value_opt host_env_key |> trim_opt
...
let assets_dir_opt () = raw_value_opt "MASC_ASSETS_DIR" |> trim_opt

let cluster_name_opt () = raw_value_opt "MASC_CLUSTER_NAME" |> trim_opt
...
and masc_http_base_url_result () =
  ...
  | None ->
    let host =
      ...
      | Some value -> Ok value
      | None ->
        Error "MASC_HTTP_BASE_URL is required (or set MASC_HOST with MASC_HTTP_PORT)"
    in
    Result.map (fun host -> Printf.sprintf "http://%s:%s" host (masc_http_port ())) host
;;
...
let base_path_opt () = base_path_raw_opt () |> Option.map normalize_masc_base_path_input
...
let base_path_prod_guard path =
  if not (running_under_test_executable ())
  then path
  else (
    let allow =
    ...
    then path
      ...
      | Some home ->
        ...
        if
          home_norm <> ""
          && String.length path >= String.length home_norm
          && String.sub path 0 (String.length home_norm) = home_norm
        then
          raise
            (Config_error
               (Printf.sprintf
                  "#9903 test isolation breach: Env_config_core.base_path() resolved to \
                   %S under HOME=%S in test executable %S. This indicates a \
                   MASC_BASE_PATH override failure — writing to the production ledger \
                   under HOME would corrupt real data. Fix the override path in the \
                   test, or set MASC_TEST_ALLOW_HOME_BASE_PATH=1 to bypass (not \
                   recommended)."
                  path
                  home_norm
...
let sb_path_result () =
  ...
  | Some path -> Ok path
  | None -> Error "Unable to resolve scripts/sb. Set MASC_BASE_PATH."
;;
...
let config_dir_opt () = raw_value_opt config_dir_env_key |> trim_opt

(** Personas directory override. *)
let personas_dir_opt () = raw_value_opt personas_dir_env_key |> trim_opt
...
let data_dir_opt () = raw_value_opt data_dir_env_key |> trim_opt

(** {1 Relay Calibration} *)

(** Whether relay token calibration is enabled. Default: true. *)
let relay_calibration_enabled () = get_bool ~default:true "MASC_RELAY_CALIBRATION_ENABLED"
...
let admin_token_opt () = raw_value_opt admin_token_env_key |> trim_opt

(** Strict tool auth mode. Default: true.
    true = unknown masc_* tools require worker-level permission. *)
let tool_auth_strict () = get_bool ~default:true tool_auth_strict_env_key
...
let git_fetch_timeout_sec () =
  Float.max 10.0 (get_float ~default:120.0 git_fetch_timeout_sec_env_key)
;;
...
let log_level_opt () = raw_value_opt log_level_env_key |> trim_opt

(** Whether telemetry tracking is enabled. Default: true. *)
let telemetry_enabled () = get_bool ~default:true telemetry_enabled_env_key

(** Whether to log parse warnings. Default: false. *)
let parse_warn_enabled () = get_bool ~default:false parse_warn_env_key
...
let governance_level () =
  get_string ~default:"production" governance_level_env_key |> String.lowercase_ascii
;;

(** {1 Build Identity} *)

(** Git commit hash override for build identity. *)
let build_git_commit_opt () = raw_value_opt "MASC_BUILD_GIT_COMMIT" |> trim_opt

(** {1 Auto Respond} *)

(** Raw MASC_AUTO_RESPOND value for mode parsing. *)
let auto_respond_opt () = raw_value_opt "MASC_AUTO_RESPOND" |> trim_opt

(** PubSub max messages per read. Default: 1000. *)
let pubsub_max_messages () = get_int ~default:1000 "MASC_PUBSUB_MAX_MESSAGES"
...
let keeper_will () = get_string ~default:"" "MASC_KEEPER_WILL"

(** Default keeper needs (operational requirements). Default: "". *)
let keeper_needs () = get_string ~default:"" "MASC_KEEPER_NEEDS"

(** Default keeper desires (drive statement). Default: "". *)
let keeper_desires () = get_string ~default:"" "MASC_KEEPER_DESIRES"
