...
let test_shared_token_surfaces_pair () =
  with_temp_base (fun base ->
    ...
    match groups with
    | [ (prefix, agents) ] ->
      Alcotest.(check int) "exactly 2 agents in group" 2 (List.length agents);
      Alcotest.(check (list string))
        "agents sorted"
        [ "keeper-sangsu-agent"; "nick0cave-sage-heron" ]
        agents;
      Alcotest.(check int) "prefix is 12 chars" 12 (String.length prefix);
      Alcotest.(check string) "prefix matches token start" "shared_token" prefix
    | _ ->
      Alcotest.failf "expected exactly 1 duplicate group, got %d" (List.length groups))
;;
...
let test_mixed_unique_and_shared () =
  with_temp_base (fun base ->
    ...
    match groups with
    | [ (_, agents) ] ->
      Alcotest.(check (list string)) "shared pair" [ "shared-a"; "shared-b" ] agents
...
let () =
  Random.self_init ();
  Alcotest.run
    "auth_token_uniqueness_audit_9786"
    [ ( "audit"
      , [ Alcotest.test_case "empty store" `Quick test_empty_store_returns_empty
        ; Alcotest.test_case
            "unique tokens → empty"
            `Quick
            test_unique_tokens_return_empty
        ; Alcotest.test_case
            "shared token surfaces pair"
            `Quick
            test_shared_token_surfaces_pair
        ; Alcotest.test_case "mixed unique + shared" `Quick test_mixed_unique_and_shared
        ] )
    ]
;;
