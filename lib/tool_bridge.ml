...
let maybe_externalize ?(mime = "text/plain") (msg : string) : string =
  if externalization_disabled ()
  then msg
  else (
    let threshold = externalize_threshold_bytes () in
    if String.length msg <= threshold
    then msg
    else (
...
let to_oas_tool_result ?(recoverable = false) (success, msg) : Oas.Types.tool_result =
  if success
  then Ok { Oas.Types.content = maybe_externalize msg }
...
let oas_descriptor_of_masc_tool name =
  let descriptor_of_permission permission =
    let mutation_class, concurrency_class =
      ...
      | Oas.Tool.ReadOnly -> Some "read_only", Some Oas.Tool.Parallel_read
      | Oas.Tool.Write -> Some "workspace_mutating", Some Oas.Tool.Sequential_workspace
      | Oas.Tool.Destructive -> Some "external_effect", Some Oas.Tool.Exclusive_external
    in
...
let oas_tool_of_masc ~name ~description ~input_schema handler : Oas.Tool.t =
