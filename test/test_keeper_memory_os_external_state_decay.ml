...
let fact ~category ?(first_seen = now -. 60.0) ?(valid_until = None)
...
let test_reobservation_preserves_explicit_validity () =
  ...
  let existing =
    fact ~category:Types.Blocker ~valid_until ()
  in
  let incoming = fact ~category:Types.Blocker () in
  let merged =
