...
let dedupe_keep_order values =
  values |> List.filter_map trim_nonempty |> Json_util.dedupe_keep_order
;;
...
let find_run_record run_id =
  Eio.Mutex.use_ro provider_runs_mutex (fun () -> Hashtbl.find_opt provider_runs run_id)
;;
...
let candidate_models_for_provider provider = catalog_models_for_provider provider
  ...
  let discovery =
    ...
    | ep :: _ ->
      ...
      Some
        ...
        ; ctx_size =
            (match ep.props with
             | Some p -> Some p.ctx_size
             | None -> None)
        ; total_slots =
            (match ep.props with
             | Some p -> Some p.total_slots
             | None -> None)
        ; busy_slots =
            (match ep.slots with
             | Some s -> Some s.busy
...
let provider_snapshots () : provider_snapshot list =
  let managed =
    ...
    |> List.filter (fun (a : Provider_adapter.adapter) ->
      a.runtime_kind <> Provider_adapter.Local && a.default_model_id <> None)
...
let provider_inventory_json () =
  ...
  let local_models =
    ...
    |> List.fold_left (fun acc snapshot -> acc + List.length snapshot.models) 0
  in
  let cloud_models =
    snapshots
  ...
  `Assoc
    ...
    ; "providers", `List (List.map provider_snapshot_to_json snapshots)
    ]
;;
...
let provider_label_for_model provider model =
  match Provider_adapter.resolve_direct_adapter provider with
  | Some adapter -> Ok (Provider_adapter.cascade_prefix_of_adapter adapter ^ ":" ^ model)
  | None ->
    Error
      (Printf.sprintf
...
let resolve_provider_run_request ~provider ~model_opt ~prompt =
  ...
  | None -> Error (Printf.sprintf "Unknown provider '%s'" provider)
  | Some snapshot ->
    if not snapshot.supports_single_agent_run
    then
      Error
        (Option.value
           snapshot.note
      ...
      | None -> Error (Printf.sprintf "Provider '%s' requires an explicit model" provider)
      | Some model ->
        if snapshot.models <> [] && not (List.mem model snapshot.models)
        then
          Error
            (Printf.sprintf
        ...
        then Error "prompt is required"
        else Ok (snapshot, model))
;;
...
let run_system_prompt provider =
  Printf.sprintf
    "You are a single MASC dashboard run using provider %s. Answer directly, keep tool \
     use disabled, and return only the final answer."
    provider
;;
...
let execute_single_agent_run ~sw ~net ~run_id ~provider ~model ~prompt =
  ...
  | Ok label ->
    ...
    (match Cascade_config.parse_model_string label with
     ...
     | Some _cfg ->
       if not (is_label_runnable label)
       then
         Error
           (Printf.sprintf
         ...
         match
           Masc_oas_bridge.run_with_caller
             ~caller:Env_config_oas_bridge.Dashboard_provider_runs
             (fun () ->
                Oas_worker.run_model_by_label
                  ...
                  ~max_tokens:
                    (Cascade_inference.resolve_max_tokens
                       ~cascade_name:"provider_benchmark"
                       ~fallback:(fun () -> 2048))
                  ~temperature:
                  ...
                  ())
         with
         | Ok result -> Ok (response_text_of_api_response result.response)
...
let run_status_json run_id =
  ...
  | None -> Error (Printf.sprintf "run_id '%s' not found" run_id)
;;
