...
- `runtime-config`, `diagnostics`, `transport-health`, `journey`, `cognition`은 routeable compatibility/diagnostic/deep-link surface로 남지만 primary sidebar에는 노출하지 않는다.
...
- Transport/Diagnostics 상세는 Monitor daily default가 아니라 diagnostics/admin 성격이다. Monitor에는 degraded badge나 diagnostic link만 노출한다.
- Runtime & Runtime default view는 agent core/runtime signal을 먼저 보여주고 `transport-health`, `diagnostics`를 hidden diagnostics link 묶음으로 노출한다.
...
- `GET /api/v1/dashboard/harness-health`
  - lab harness health
- `GET /api/v1/ide/file-activity`, `GET /api/v1/ide/presence`
  - durable Keeper writes by repository file, and live collaboration presence
