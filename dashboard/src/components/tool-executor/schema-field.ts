...
export function SchemaField({ name, schema, value, required, onChange }: SchemaFieldProps) {
  ...
  const hintId = schema.description ? `schema-hint-${name}` : undefined
  const hint = schema.description
    ? html`<span id=${hintId} class="text-3xs text-[var(--text-muted)] mt-0.5">${schema.description}</span>`
    : null
    ...
    return html`
      <div class="flex flex-col gap-1">
        ...
        <${Select} value=${(value as string) ?? ''} options=${schema.enum} placeholder="-- 선택 --"
          ariaLabel=${name} ariaDescribedBy=${hintId} onInput=${(v: string) => onChange(name, v)} />
      </div>
    `
  }
      ...
      return html`
        <div class="flex flex-col gap-1">
          ...
          <${TextArea} value=${(value as string) ?? (schema.default as string) ?? ''} placeholder=${name} rows=${3}
            ariaLabel=${name} ariaDescribedBy=${hintId} onInput=${(e: Event) => onChange(name, (e.target as HTMLTextAreaElement).value)} />
        </div>
      `
    }
    return html`
      <div class="flex flex-col gap-1">
        ...
        <${TextInput} value=${(value as string) ?? (schema.default as string) ?? ''} placeholder=${name}
          ariaLabel=${name} ariaDescribedBy=${hintId} onInput=${(e: Event) => onChange(name, (e.target as HTMLInputElement).value)} />
      </div>
    `
  }
    ...
    return html`
      <div class="flex flex-col gap-1">
        ...
        <${NumberInput} value=${(value as number) ?? (schema.default as number) ?? ''} placeholder=${name}
          ariaLabel=${name} ariaDescribedBy=${hintId} step=${schema.type === 'integer' ? 1 : 'any'} onInput=${(v: number | undefined) => onChange(name, v)} />
      </div>
    `
  }
  ...
  if (schema.type === 'boolean') {
    return html`
      <div class="flex items-center gap-2 py-1">
        ...
        ${schema.description ? html`<span id=${hintId} class="text-3xs text-[var(--text-muted)]">- ${schema.description}</span>` : null}
      </div>
    `
  }
    ...
    return html`
      <div class="flex flex-col gap-1">
        ...
        <${TextArea} value=${strValue} placeholder=${name} rows=${3}
          ariaLabel=${name} ariaDescribedBy=${hintId} onInput=${(e: Event) => {
