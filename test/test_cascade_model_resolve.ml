...
let test_gemini_cli_explicit_model_passthrough () =
  with_clean_env (fun () ->
    let resolved = R.resolve_auto_model_id "gemini_cli" "gemini-2.5-flash" in
    check string "explicit model untouched" "gemini-2.5-flash" resolved)
;;
...
let test_gemini_env_override () =
  ...
  check string "gemini respects env override" "gemini-2.5-flash" resolved_gemini;
  check string "gemini_cli respects same env override" "gemini-2.5-flash" resolved_cli
;;
...
let test_order_weighted_entries_rotation_scope_rotates_generically () =
  with_clean_env (fun () ->
    State.clear_all ();
    let entry model =
      { Masc_mcp.Cascade_config_loader.model; weight = 1; supports_tool_choice = None }
    in
    let first =
