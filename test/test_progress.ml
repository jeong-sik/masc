...
let mock_broadcast json = captured_json := json :: !captured_json
...
let test_notify_basic () =
  ...
  check
    ...
    "progress is 0.5"
    true
    (Float.equal 0.5 (params |> member "progress" |> to_float))
;;
...
let test_tracker_create () =
  setup ();
  let tracker =
    Masc_mcp.Progress.Tracker.create ~task_id:"tracker-test" ~total_steps:10 ()
  in
...
let test_tracker_complete () =
  ...
  check
    ...
    (Float.equal
       1.0
       (Yojson.Safe.Util.member "progress" params |> Yojson.Safe.Util.to_float))
;;
...
let test_handle_tool_start () =
  ...
  let args =
    `Assoc
      [ "action", `String "start"; "task_id", `String "tool-test"; "total_steps", `Int 5 ]
  in
...
let test_handle_tool_update () =
  ...
  check
    ...
    true
    (String.length msg > 0
     && String.sub msg 0 (min 16 (String.length msg)) = "Progress updated")
;;

let test_handle_tool_complete () =
  setup ();
  let args =
    `Assoc [ "action", `String "complete"; "task_id", `String "complete-test" ]
  in
...
let test_handle_tool_invalid_action () =
  setup ();
  let args = `Assoc [ "action", `String "invalid"; "task_id", `String "test" ] in
  let success, _ = Masc_mcp.Progress.handle_progress_tool args in
...
let test_handle_tool_missing_task_id () =
  setup ();
  let args = `Assoc [ "action", `String "update"; "progress", `Float 0.5 ] in
  let success, _ = Masc_mcp.Progress.handle_progress_tool args in
...
let test_start_and_stop_tracking () =
  ...
  check
    ...
    "tracker exists"
    true
    (Option.is_some (Masc_mcp.Progress.get_tracker "track-test"));
  Masc_mcp.Progress.stop_tracking "track-test";
  check
    bool
    "tracker removed"
    true
    (Option.is_none (Masc_mcp.Progress.get_tracker "track-test"))
;;
...
let test_validate_task_id_empty () =
  setup ();
  let args = `Assoc [ "action", `String "start"; "task_id", `String "" ] in
  let success, msg = Masc_mcp.Progress.handle_progress_tool args in
...
let test_validate_task_id_too_long () =
  ...
  let args = `Assoc [ "action", `String "start"; "task_id", `String long_id ] in
  let success, msg = Masc_mcp.Progress.handle_progress_tool args in
...
let test_validate_task_id_invalid_chars () =
  setup ();
  let args =
    `Assoc [ "action", `String "start"; "task_id", `String "task\x00id" (* null char *) ]
  in
  let success, msg = Masc_mcp.Progress.handle_progress_tool args in
...
let test_validate_progress_out_of_range () =
  ...
  let args =
    `Assoc
      [ "action", `String "update"; "task_id", `String "test"; "progress", `Float 1.5 ]
  in
  let success, msg = Masc_mcp.Progress.handle_progress_tool args in
...
let test_validate_progress_negative () =
  ...
  let args =
    `Assoc
      [ "action", `String "update"; "task_id", `String "test"; "progress", `Float (-0.5) ]
  in
  let success, msg = Masc_mcp.Progress.handle_progress_tool args in
...
let () =
  ...
  run
    ...
    ; "tracking", [ test_case "start and stop" `Quick test_start_and_stop_tracking ]
    ; ( "validation"
