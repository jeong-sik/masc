(** Per-keeper-name in-memory cursor state.

    Strategy variants are currently stateless, but runtime
    model expansion still uses a per-keeper round-robin cursor to
    ...
    Moved from [Keeper_state] to decouple from cascade module surface. *)
