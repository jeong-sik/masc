...
각각 정확히 한 번씩, 모두 일곱 번 바뀌었다(§4.3). 다섯은 같은 3분 안에 같은 도구
하나가 늘어난 것이다. 즉 **이 함대는 아무도 요청하지 않은 표면 이동을 하루에도 몇
번씩 이미 치르고 있다.** 이 설계가 더하는 것은 사람이 파일을 고칠 때마다 Keeper 당
한 번이다.
...
그리고 이 표가 §4.2 의 대가를 함께 값매긴다. 17:57–18:00Z 의 다섯 줄은 배포가 도구
하나를 더한 것이고, 그것만으로 다섯 Keeper 의 `tool_surface_sha256` 이 움직여 다섯
