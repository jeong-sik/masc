...
module Api_error = Agent_core.Api_error
...
let unknown_invalid_request message =
  Api_error.InvalidRequest
    { message; reason = Api_error.Unknown_invalid_request }
...
let api_cases : (string * CoreError.api_error * string) list =
  [ ( "RateLimited"
    , Api_error.RateLimited { retry_after = Some 30.0; message = "" }
    , "api_error_rate_limited" )
  ; "Overloaded", Api_error.Overloaded { message = "" }, "api_error_overloaded"
  ; ( "ServerError"
    , Api_error.ServerError { status = 502; message = "" }
    , "api_error_server:502" )
  ; "AuthError", Api_error.AuthError { message = "" }, "api_error_auth"
  ; ( "AuthorizationError"
    , Api_error.AuthorizationError { message = "permission refused" }
    , "api_error_authorization" )
  ; ( "PaymentRequired"
    , Api_error.PaymentRequired { message = "billing required" }
    , "api_error_payment_required" )
  ; ( "InvalidRequest"
    , unknown_invalid_request "bad"
    , "api_error_invalid_request" )
  ; "NotFound", Api_error.NotFound { message = "missing" }, "api_error_not_found"
  ; ( "ContextOverflow"
    , Api_error.ContextOverflow { message = "ctx"; limit = Some 8192 }
  ...
  ; ( "InputCapacityMeasurementUnavailable"
    , Api_error.InputCapacity
        ...
        ; constraint_ = serving_constraint
        ; reason =
            Api_error.Token_measurement_unavailable
              Llm_provider.Input_token_count.Anthropic_messages_count_tokens
        }
    , "api_error_input_capacity:measurement_unavailable" )
  ; ( "NetworkError"
    , Api_error.NetworkError { message = "ECONNRESET"; kind = Http.Connection_refused }
    , "api_error_network" )
  ; "Timeout", Api_error.Timeout { message = "60s"; phase = None }, "api_error_timeout"
...
let core_error_cases : (string * CoreError.t * string) list =
  ...
  ; ( "Api/Timeout"
    , CoreError.Api (Api_error.Timeout { message = "60s"; phase = None })
    , "api_error_timeout" )
  ; ( "Api/AuthorizationError"
    , CoreError.Api (Api_error.AuthorizationError { message = "permission refused" })
    , "api_error_authorization" )
...
let test_quota_prose_does_not_override_typed_rate_limit () =
  ...
  in
  let err = CoreError.Api (Api_error.RateLimited { retry_after = None; message }) in
  Alcotest.(check bool)
...
let test_payment_required_is_hard_quota () =
  let err = CoreError.Api (Api_error.PaymentRequired { message = "Insufficient Balance" }) in
  Alcotest.(check bool)
...
let test_input_capacity_is_not_runtime_recovery () =
  ...
  | KFR.Rotate_now _ ->
    Alcotest.fail "InputCapacity must not authorize a runtime transition"
;;
...
let test_overloaded_with_quota_prose_is_not_hard_quota () =
  ...
  in
  let err = CoreError.Api (Api_error.Overloaded { message }) in
  Alcotest.(check bool)
...
let test_soft_rate_limit_classifies_as_rate_limit () =
  ...
  List.iter
    (fun (label, err) ->
       ...
       | _ -> Alcotest.failf "%s expected typed rate-window route" label)
