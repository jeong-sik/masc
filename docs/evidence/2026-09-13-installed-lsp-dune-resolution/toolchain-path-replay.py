...
out=pathlib.Path('/tmp/masc-lsp-direct-same-fixture-toolchain-path-20260913');out.mkdir()
...
sel=selectors.DefaultSelector();sel.register(proc.stdout,selectors.EVENT_READ,'stdout');sel.register(proc.stderr,selectors.EVENT_READ,'stderr');wire=bytearray();events=[];stderr=[];observed=None
def send(msg):
...
try:
 ...
 result={'scope':'Direct owned ocamllsp process; same existing fixture URI/source and backend initialize envelope; not installed proxy replay','pid':proc.pid,'exit_code':proc.returncode,'source_sha256':before,'published_diagnostics':observed,'server_messages':[e['message'] for e in events if e['direction']=='server' and e['message'].get('id')!=1]}
