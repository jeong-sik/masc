...
let test_completion_claim_non_keeper_increments () =
  ...
  let reason = "not_keeper_agent_name" in
  let before = counter_for ~kind ~reason in
  Acct.record_completion_claim
    config
    ~keeper_name:"taskmaster"
    ~agent_name:"taskmaster"
    ~trace_id:"t-1"
    ~turn_number:0
    ~subject:"shipped a thing"
...
let test_valid_call_does_not_increment () =
  ...
  with_temp_config
  @@ fun config ->
  let kt = counter_for ~kind:"completion_claim" ~reason:"not_keeper_agent_name" in
  let ks = counter_for ~kind:"completion_claim" ~reason:"empty_subject" in
  ...
  check
    ...
    kt
    (counter_for ~kind:"completion_claim" ~reason:"not_keeper_agent_name");
  check
    (float 0.0001)
...
let test_kind_and_reason_isolation () =
  ...
  Acct.record_task_transition
    ...
    ~agent_name:"qa-king"
    ~task_id:"task-9002"
    ~transition:Types.Start
    ~details:`Null;
  check
    (float 0.0001)
    "completion_claim/not_keeper_agent_name unchanged when task_transition bumps"
    unrelated_before
    (counter_for ~kind:"completion_claim" ~reason:"not_keeper_agent_name")
;;
...
let () =
  run
    "accountability_emit_skip_10314"
    [ ( "task_transition"
      , [ test_case
            "non-keeper agent_name drop is counted"
            `Quick
    ...
    ; ( "completion_claim"
      , [ test_case
        ...
        ; test_case
            "empty subject drop is counted"
            `Quick
            test_completion_claim_empty_subject_increments
        ; test_case
            "valid call leaves counters unchanged"
            `Quick
            test_valid_call_does_not_increment
        ] )
    ; ( "label_isolation"
      , [ test_case "(kind, reason) pairs separate" `Quick test_kind_and_reason_isolation
        ] )
    ]
;;
