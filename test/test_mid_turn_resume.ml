...
let test_checkpoint_roundtrip () =
  ...
  let config = { (Masc_agent_core.Types.default_config ~model:"mock-model") with
    name = "mid-turn-test";
    system_prompt = Some "test system prompt";
  } in
  let agent = Masc_agent_core.Agent.create ~net ~config () in
  (* Simulate 3 completed turns by manually updating state *)
  let msgs = [
    Masc_agent_core.Types.user_msg "turn 1 user";
    { Masc_agent_core.Types.role = Masc_agent_core.Types.Assistant;
      content = [Masc_agent_core.Types.Text "turn 1 response"];
      name = None; tool_call_id = None; metadata = [] };
    Masc_agent_core.Types.user_msg "turn 2 user";
    { Masc_agent_core.Types.role = Masc_agent_core.Types.Assistant;
      content = [Masc_agent_core.Types.Text "turn 2 response"];
      name = None; tool_call_id = None; metadata = [] };
    Masc_agent_core.Types.user_msg "turn 3 user";
    { Masc_agent_core.Types.role = Masc_agent_core.Types.Assistant;
      content = [Masc_agent_core.Types.Text "turn 3 response"];
      name = None; tool_call_id = None; metadata = [] };
  ] in
  Masc_agent_core.Agent.set_state agent { (Masc_agent_core.Agent.state agent) with
    messages = msgs;
    turn_count = 3;
  };
  (* Extract checkpoint — this is what mid-turn resume does on failure *)
  let cp = Masc_agent_core.Agent.checkpoint agent in
  ...
  let resumed = Masc_agent_core.Agent.resume ~net ~checkpoint:cp () in
  let resumed_state = Masc_agent_core.Agent.state resumed in
...
let test_zero_turns_no_checkpoint () =
  let net = require_net () in
  let agent =
    Masc_agent_core.Agent.create ~net
      ~config:(Masc_agent_core.Types.default_config ~model:"mock-model")
      ()
  in
  let state = Masc_agent_core.Agent.state agent in
...
let test_multi_runtime_accumulation () =
  let net = require_net () in
  (* Provider A: 2 turns *)
  let agent_a = Masc_agent_core.Agent.create ~net
    ~config:{ (Masc_agent_core.Types.default_config ~model:"anthropic") with
      name = "runtime-a" }
    ()
  in
  Masc_agent_core.Agent.set_state agent_a { (Masc_agent_core.Agent.state agent_a) with
    messages = [
      Masc_agent_core.Types.user_msg "t1";
      { Masc_agent_core.Types.role = Masc_agent_core.Types.Assistant;
        content = [Masc_agent_core.Types.Text "r1"]; name = None; tool_call_id = None; metadata = [] };
      Masc_agent_core.Types.user_msg "t2";
      { Masc_agent_core.Types.role = Masc_agent_core.Types.Assistant;
        content = [Masc_agent_core.Types.Text "r2"]; name = None; tool_call_id = None; metadata = [] };
    ];
    turn_count = 2;
  };
  let cp_a = Masc_agent_core.Agent.checkpoint agent_a in
  Alcotest.(check int) "cp_a turns" 2 cp_a.turn_count;
  (* Provider B: resume from A, add 1 turn *)
  let agent_b = Masc_agent_core.Agent.resume ~net ~checkpoint:cp_a () in
  Masc_agent_core.Agent.set_state agent_b { (Masc_agent_core.Agent.state agent_b) with
    messages = (Masc_agent_core.Agent.state agent_b).messages @ [
      Masc_agent_core.Types.user_msg "t3";
      { Masc_agent_core.Types.role = Masc_agent_core.Types.Assistant;
        content = [Masc_agent_core.Types.Text "r3"]; name = None; tool_call_id = None; metadata = [] };
    ];
    turn_count = 3;
  };
  let cp_b = Masc_agent_core.Agent.checkpoint agent_b in
  ...
  let agent_c = Masc_agent_core.Agent.resume ~net ~checkpoint:cp_b () in
  let state_c = Masc_agent_core.Agent.state agent_c in
...
let test_resume_preserves_checkpoint_model () =
  let net = require_net () in
  let agent = Masc_agent_core.Agent.create ~net
    ~config:(Masc_agent_core.Types.default_config ~model:"anthropic-model")
    ()
  in
  Masc_agent_core.Agent.set_state agent { (Masc_agent_core.Agent.state agent) with
    messages = [Masc_agent_core.Types.user_msg "hello"];
    turn_count = 1;
  };
  let cp = Masc_agent_core.Agent.checkpoint agent in
  ...
  let resumed = Masc_agent_core.Agent.resume ~net ~checkpoint:cp () in
  let state = Masc_agent_core.Agent.state resumed in
  Alcotest.(check string) "resumed keeps checkpoint model" "anthropic-model"
    (Masc_agent_core.Types.model_to_string state.config.model)
