...
let test_resolve_connector_status_name_prefers_explicit_name () =
  check
    (option string)
    "name wins and normalizes"
    (Some "discord")
    (Routes.resolve_connector_status_name ~name:"  Discord  " ~channel:"telegram" ())
;;

let test_resolve_connector_status_name_normalizes_legacy_channel () =
  check
    (option string)
    "legacy channel lowercased"
    (Some "discord")
