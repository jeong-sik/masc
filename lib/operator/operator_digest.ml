...
let health_from_attention_items (items : attention_item list) =
  if List.exists (fun (item : attention_item) -> item.severity = Sev_bad) items
  then "bad"
  else if items <> []
  then "warn"
  else "ok"
    ...
    | (entry : Log.Ring.entry) :: rest ->
      ...
      then dedup seen acc rest
        ...
        | Some envelope ->
          ...
          in
          if List.mem fingerprint seen
          then dedup seen acc rest
          else (
            let item =
              { kind = envelope.cause_code
              ; severity = operator_severity_of_failure_envelope envelope.severity
              ...
              ; evidence =
                  `Assoc
                    ...
                    ; "failure_envelope", Failure_envelope.to_yojson envelope
                    ]
              }
            in
...
let build_room_attention_items config =
  ...
  let pending_items =
    ...
    then []
      ...
      [ { kind = "pending_confirm_waiting"
        ...
        ; summary =
            Printf.sprintf
              "%d pending confirmation(s) are waiting for operator input"
  ...
  List.sort
    compare_attention
    (recent_tool_host_failures ~now:(Time_compat.now ()) () @ pending_items)
;;

let room_recommendations _config = dedup_recommendations []
...
let digest_json
      ...
      ?include_workers:_include_workers
      (ctx : 'a context)
  ...
  then (
    ...
    Ok
      (`Assoc
          ...
          ; ( "pending_confirm_summary"
            , pending_confirm_summary_json_of_scope
                (pending_confirm_scope_of_entries ?actor []) )
          ...
          ; ( "active_recommendation_summary"
            , summary_of_recommendations ~actor:"dashboard" [] )
    ...
    | "root" ->
      ...
      in
      let recommended_actions = dedup_recommendations (room_recommendations config) in
      ...
      Ok
        (`Assoc
            ([ "trace_id", `String (trace_id "opsd")
             ...
             ; "operator_judge_runtime", operator_judge_runtime_json config
             ; ( "attention_items"
               , `List (List.map attention_item_to_yojson attention_items) )
