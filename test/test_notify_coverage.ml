...
let test_sanitize_token_empty () = check string "empty" "" (Notify.sanitize_token "")
...
let test_token_value_none () = check string "none" "" (Notify.token_value None)
...
let test_is_truthy_1 () = check bool "1" true (Notify.is_truthy "1")
let test_is_truthy_true () = check bool "true" true (Notify.is_truthy "true")
let test_is_truthy_yes () = check bool "yes" true (Notify.is_truthy "yes")
let test_is_truthy_on () = check bool "on" true (Notify.is_truthy "on")
let test_is_truthy_y () = check bool "y" true (Notify.is_truthy "y")
let test_is_truthy_TRUE () = check bool "TRUE" true (Notify.is_truthy "TRUE")
let test_is_truthy_Yes () = check bool "Yes" true (Notify.is_truthy "Yes")
let test_is_truthy_0 () = check bool "0" false (Notify.is_truthy "0")
let test_is_truthy_false () = check bool "false" false (Notify.is_truthy "false")
let test_is_truthy_no () = check bool "no" false (Notify.is_truthy "no")
let test_is_truthy_empty () = check bool "empty" false (Notify.is_truthy "")
...
let test_escape_shell_plain () = check string "plain" "test" (Notify.escape_shell "test")
...
let test_escape_shell_empty () = check string "empty" "" (Notify.escape_shell "")
...
let test_render_focus_template_target () =
  let payload : Notify.focus_payload =
    { target_agent = Some "claude"; from_agent = None; task_id = None }
  in
...
let test_render_focus_template_from () =
  let payload : Notify.focus_payload =
    { target_agent = None; from_agent = Some "gemini"; task_id = None }
  in
...
let test_render_focus_template_task () =
  let payload : Notify.focus_payload =
    { target_agent = None; from_agent = None; task_id = Some "task-001" }
  in
...
let test_render_focus_template_none () =
  let payload : Notify.focus_payload =
    { target_agent = None; from_agent = None; task_id = None }
  in
...
let test_render_focus_template_sanitizes () =
  let payload : Notify.focus_payload =
    { target_agent = Some "claude@test"; from_agent = None; task_id = None }
  in
...
let test_escape_applescript_double_quote () =
  check
    ...
    "double quote"
    "he said \\\"hello\\\""
    (Notify.escape_applescript "he said \"hello\"")
;;
...
let test_escape_applescript_backslash () =
  check
    ...
    "backslash"
    "path\\\\to\\\\file"
    (Notify.escape_applescript "path\\to\\file")
;;
...
let test_escape_applescript_mixed () =
  check
    ...
    "mixed"
    "say \\\"hi\\\" and \\\\"
    (Notify.escape_applescript "say \"hi\" and \\")
;;
...
let test_agent_emoji_claude () = check string "claude" "🟣" (Notify.agent_emoji "claude")
let test_agent_emoji_gemini () = check string "gemini" "🔵" (Notify.agent_emoji "gemini")
let test_agent_emoji_codex () = check string "codex" "🟢" (Notify.agent_emoji "codex")
let test_agent_emoji_llama () = check string "llama" "🦙" (Notify.agent_emoji "llama")

let test_agent_emoji_system () = check string "system" "⚙️" (Notify.agent_emoji "system")

let test_agent_emoji_unknown () =
  check string "unknown" "🤖" (Notify.agent_emoji "unknown-agent")
;;

let test_agent_emoji_empty () = check string "empty" "🤖" (Notify.agent_emoji "")
...
let test_event_mention () =
  let e : Notify.event =
    Mention { from_agent = "gemini"; target_agent = Some "claude"; message = "hello" }
  in
  match e with
...
let test_event_portal_message () =
  let e : Notify.event =
    PortalMessage { from_agent = "codex"; target_agent = None; message = "data" }
  in
  match e with
...
let test_event_custom () =
  let e : Notify.event =
    Custom { title = "Title"; subtitle = "Subtitle"; message = "Message" }
  in
  match e with
