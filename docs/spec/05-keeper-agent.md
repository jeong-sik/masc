...
| 범주 | 대표 파일 | 파일 수 |
|------|----------|--------|
| Types | `keeper_types.ml`, `_profile.ml`, `_support.ml` | 3 |
...
JSON/MCP 경계에는 `scope_kind`, `room_scope` 같은 coordination 값만 남는다. 다만 현재 room 모델은 single-room으로 평탄화되어 `room_scope`의 실효값은 항상 `current`다.
