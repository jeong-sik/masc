...
let tool_result_msg ~tool_use_id ~content : Types.message =
  { role = Types.Tool
  ; content = [ Types.ToolResult { tool_use_id; content; is_error = false; json = None } ]
  ; name = None
...
let sum_counts counts = List.fold_left (fun acc (_, n) -> acc + n) 0 counts
...
let test_thinking_and_redacted () =
  ...
  check
    int
    "thinking uses content; redacted uses literal length"
...
let test_role_key_mapping () =
  ...
  check string "assistant -> assistant" "assistant" (WT.role_key Types.Assistant);
...
let test_compute_sizes_bytes_breakdown () =
  let history =
    [ text_msg Types.User "A"; (* 1 byte *) text_msg Types.Assistant "BB" (* 2 bytes *) ]
  in
  let tools = [] in
  let sizes =
    WT.compute_sizes
  ...
  check int "approx_body_bytes = sp + tools + msgs" (3 + 0 + 7) sizes.approx_body_bytes;
...
let test_compute_sizes_invariant_across_shapes () =
  ...
  let samples : Types.message list list =
    ...
    ; List.init 20 (fun i ->
        if i mod 2 = 0
        then text_msg Types.User (string_of_int i)
  ...
  List.iter
    (fun history ->
       ...
       check
         int
         (Printf.sprintf "invariant holds for history of length %d" (List.length history))
         sizes.message_count
         (sum_counts sizes.role_counts))
    samples
;;
...
let test_role_counts_are_stably_sorted () =
  let history =
    [ text_msg Types.Assistant "a"; text_msg Types.Tool "t"; text_msg Types.System "s" ]
  in
  ...
  check
    (list string)
    "role_counts keys are sorted for stable JSON output"
    sorted_keys
    keys
;;
  ...
  run
    ...
    [ ( "bytes_of_message"
      , [ test_case "text-only content" `Quick test_text_only_message
        ...
        ; test_case "tool_result id + content bytes" `Quick test_tool_result_bytes
        ; test_case "thinking + redacted thinking" `Quick test_thinking_and_redacted
        ] )
    ; ( "role_key"
      , [ test_case "all roles map to stable strings" `Quick test_role_key_mapping ] )
    ; ( "role_counts_with_pending_user"
      , [ test_case
            "empty history still counts the pending user turn"
            `Quick
            test_role_counts_adds_pending_user_on_empty_history
        ; test_case
            "assoc list is stably sorted by key"
            `Quick
            test_role_counts_are_stably_sorted
        ] )
    ; ( "compute_sizes"
      , [ test_case
            "byte breakdown matches inputs"
            `Quick
            test_compute_sizes_bytes_breakdown
        ; test_case
            "role_counts sum matches message_count"
            `Quick
            test_role_counts_sum_matches_message_count
        ; test_case
            "invariant holds across mixed content shapes"
            `Quick
