...
audit ledger / runtime_audit / decisions / anchored thread 모두 `ts_ms` 를 갖고 있어 timeline 슬라이더 한 축으로 통합 재투영 가능. 1단계 PR-bound replay (PR open~close 기간만), 2단계 free-range scrub.

## 2. Non-Goals

- audit ledger 자체 변경 (consumer only)
- write/mutating action (read-only)
- diff replay (별도 RFC; v1 은 thread/decision/runtime 만)
...
export interface ReplayOverlayStore {
  ...
  readonly runtimeHitsInWindow: () => ReadonlyArray<RuntimeHit>      // RFC-0023
}
```

기존 `anchored-thread-rail-store.ts` 의 `timestamp_ms` 필드를 filter 기준으로 사용. runtime overlay (RFC-0023) 도 같은 window 적용.
...
window 변경 → 모든 overlay (anchored-thread-rail, runtime-overlay-layer) 가 동일 store subscribe → 자동 re-render.
...
- e2e: PR-bound 1개 사례 (#13174 같은 short-lived PR) 에서 thread/decision/runtime ordering 이 timeline 과 일치
...
2. **scrubber density 색상**: density histogram 의 color band — keeper hue 평균 vs activity intensity vs status mix. UX 측정.
