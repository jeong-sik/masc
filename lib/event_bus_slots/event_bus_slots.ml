(** Event_bus_slots — process-wide holders for the two
    ...
    publishes on its own bus rather than onto AGENT_CORE's shared Event_bus.
    AGENT_CORE's [event_bus.mli:103-107] says: "External publishers should use
    their own Event_bus.t instance for domain-specific events rather
    than publishing onto AGENT_CORE's bus.  Treating AGENT_CORE's Event_bus as a
    ...
    {b keeper slot} — reference to the shared AGENT_CORE Event_bus.  The
    ...
    Both slots hold the same type, [Agent_core.Event_bus.t] — we re-use
...
(* masc slot — process-wide Atomic policy.  Do not merge this storage
   ...
   stylistic. *)
let masc_bus_ref : Agent_core.Event_bus.t option Atomic.t = Atomic.make None
...
let keeper_bus_key : Agent_core.Event_bus.t option Domain.DLS.key =
  Domain.DLS.new_key (fun () -> None)

let keeper_process_bus : Agent_core.Event_bus.t option Atomic.t =
  Atomic.make None
