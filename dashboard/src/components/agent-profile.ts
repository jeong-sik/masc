      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <${Card} title="타임라인" class="ff-card rounded">
          ${!timeline || (timeline.events ?? []).length === 0
            ...
            : html`<div class="flex flex-col gap-0.5 max-h-75 overflow-y-auto custom-scrollbar" tabindex="0" role="list" aria-label="에이전트 타임라인 이벤트">${(timeline.events ?? []).map((evt: AgentTimelineEvent, idx: number) => {
                const detail = evt.detail as Record<string, string | undefined>
                const title = detail.title ?? detail.content ?? ''
                return html`
                  <div class="agent-timeline-event flex items-baseline gap-1.5 py-1 px-2 text-sm transition-[background] duration-100 rounded hover:bg-[var(--white-4)]" key=${idx} role="listitem">
                ...
                return html`
                  <div class="flex flex-col gap-1.5">
                    ...
                    ${isFiltering && visible.length === 0
                      ...
                      : html`<div class="max-h-[210px] overflow-y-auto custom-scrollbar flex flex-col gap-1.5" tabindex="0" aria-label="세션 로그">${visible.map((line: string, idx: number) =>
