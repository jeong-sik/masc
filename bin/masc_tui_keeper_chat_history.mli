...
type row =
  ...
  ; operation_id : string option
      (** The chat operation a direct turn ran as -- the same value as
          [turn_id] when the delivery key is an operation, and only then.
          [None] on autonomous turns, on rows carrying another delivery key,
          on trace and skill rows, and on Memory journal entries. The v2
          journal endpoint is keyed by this, so a reload can ask for exactly
          the turns that have a journal without reading [turn_id]'s shape. *)
  ; kind : kind
  ; text : string
