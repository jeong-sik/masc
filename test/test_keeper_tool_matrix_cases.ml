...
let all_keeper_tool_schemas () = all_keeper_tool_schemas_raw () |> dedupe_tool_schemas
...
let make_fixture sw ~proc_mgr ~fs ~net ~mono_clock clock ~base_path init_mode =
  ...
  let ctx =
    Masc_mcp.Keeper_exec_context.create
      ~system_prompt:"keeper tool matrix"
      ~max_tokens:4000
    |> fun ctx ->
    Masc_mcp.Keeper_exec_context.append
      ctx
  ...
  (match init_mode with
   | Init_joined ->
     ...
     ignore (Masc_mcp.Coord.join config ~agent_name:meta.name ~capabilities:[] ());
     ignore
       (Masc_mcp.Coord.join
          config
          ~agent_name:("keeper-" ^ meta.name)
          ~capabilities:[]
          ())
   | Fresh | Init_only -> ());
...
let ensure_keeper_claim fixture =
  ...
  ignore (Masc_mcp.Coord.claim_next fixture.config ~agent_name:fixture.meta.agent_name)
;;

let ensure_voice_session fixture =
  let mgr = Masc_mcp.Keeper_voice_local.get_session_manager () in
  ignore
    (Masc_mcp.Voice_session_manager.start_session
       mgr
       ~agent_id:fixture.meta.name
       ~voice:"tool-matrix"
       ())
;;
...
let prepare_keeper_name fixture name =
  ...
  then ignore (Generic.ensure_board_post fixture.generic);
  if List.mem name [ "keeper_library_search"; "keeper_library_read" ]
  then ignore (Generic.ensure_library_topic fixture.generic);
  if
    List.mem
      name
      [ "keeper_task_claim"
  ...
  then ignore (Generic.ensure_task fixture.generic);
  if
    List.mem
      name
      [ "keeper_task_force_release"; "keeper_task_force_done"; "keeper_task_done" ]
  then ensure_keeper_claim fixture;
...
let keeper_arguments fixture (schema : Types.tool_schema) =
  ...
  | "keeper_voice_session_end" -> `Assoc []
  | "keeper_memory_search" -> `Assoc [ "query", `String "memory needle"; "limit", `Int 2 ]
  | "keeper_board_post" ->
    `Assoc
  ...
  | "keeper_board_search" -> `Assoc [ "query", `String "tool-matrix"; "limit", `Int 5 ]
  | "keeper_fs_read" -> `Assoc [ "path", `String (ensure_sample_file fixture) ]
  | "keeper_fs_edit" ->
    `Assoc
  ...
  | "keeper_bash" -> `Assoc [ "cmd", `String "pwd"; "timeout_sec", `Float 5.0 ]
  | "keeper_bash_output" ->
  ...
  | "keeper_bash_kill" ->
    `Assoc [ "task_id", `String "bgt-matrix-fixture-0-0"; "grace_sec", `Float 0.5 ]
  | "keeper_voice_speak" -> `Assoc [ "message", `String "tool matrix hello" ]
  | "keeper_voice_listen" -> `Assoc [ "timeout_seconds", `Float 1.0 ]
  | "keeper_voice_session_start" -> `Assoc [ "session_name", `String "tool-matrix" ]
  | "keeper_library_search" -> `Assoc [ "query", `String "tool matrix" ]
  | "keeper_library_read" ->
  ...
  | "keeper_broadcast" -> `Assoc [ "message", `String "tool matrix broadcast" ]
  | "keeper_task_done" ->
    ...
    `Assoc
      ...
      ; ( "result"
        , `String
            "Validated the keeper tool matrix case as a follow-up smoke check, confirmed \
             the task fixture was claimed, and recorded the successful completion path." )
      ]
  ...
  | "keeper_pr_review_read" -> `Assoc [ "pr_number", `Int 1 ]
  ...
  | "keeper_stay_silent" -> `Assoc [ "reason", `String "tool matrix silence" ]
  | "keeper_task_create" ->
    `Assoc
  ...
  | "keeper_tool_search" -> `Assoc [ "query", `String "tool matrix" ]
  | other -> failwith ("missing keeper arguments contract for " ^ other)
;;

let keeper_expectation_for_name name =
  match name with
  | "keeper_voice_listen" | "keeper_voice_speak" | "keeper_voice_agent" ->
    Expect_success_or_guard voice_guard_fragments
  | "keeper_pr_review_read"
  | "keeper_pr_review_comment"
  | "keeper_pr_review_reply"
  | "keeper_preflight_check" -> Expect_success_or_guard github_guard_fragments
  | "keeper_task_done" ->
  ...
  | "keeper_bash_output" | "keeper_bash_kill" ->
    ...
    Expect_success_or_guard [ "no background task with id="; "already reaped" ]
  | _ -> Expect_success
;;

let extra_guard_fragments_for_name = function
  | "masc_auth_refresh" ->
    [ "agent_name must match the authenticated agent"; "no credential found" ]
  | "masc_auth_revoke" -> [ "no credential found" ]
  | "masc_team_memory_read" | "masc_team_memory_search" | "masc_team_memory_write" ->
    [ "keeper-only"; "not a keeper agent" ]
  | "masc_autoresearch_cycle"
  | "masc_autoresearch_inject"
  | "masc_autoresearch_status"
  | "masc_autoresearch_stop" -> [ "no autoresearch loop running" ]
...
let merge_expectation base extras =
  ...
  | Expect_success_or_guard fragments -> Expect_success_or_guard (fragments @ extras)
;;

let case_for_name name =
  if string_starts_with ~prefix:"masc_" name
  then (
          ...
          then
            ignore
              (Generic.ensure_playground_clone_for
                 ~agent:"keeper-tool-matrix"
                 fixture.generic);
          if name = "masc_worktree_remove"
          then (
            ...
            ignore
              (Generic.ensure_worktree_created_for
                 ~agent:"keeper-tool-matrix"
                 fixture.generic)))
    ; arguments = (fun fixture schema -> Generic.tool_arguments fixture.generic schema)
    ; expectation =
        merge_expectation generic_case.expectation (extra_guard_fragments_for_name name)
    })
  else if string_starts_with ~prefix:"keeper_" name
  then
  ...
  else failwith ("missing keeper tool contract for " ^ name)
;;

let fatal_fragments = Generic.fatal_fragments @ keeper_matrix_guard_fragments
  ...
  | Ok _ ->
    (match expectation with
     ...
     | Expect_guard fragments ->
       Error
         (Printf.sprintf
            "%s expected guard %s but succeeded"
            name
            (String.concat ", " fragments)))
  | Error { Agent_sdk.Types.message; _ } ->
    if contains_any message fatal_fragments
    then Error (Printf.sprintf "%s hit fatal keeper-tool failure: %s" name message)
    else (
      match expectation with
      | Expect_success ->
        Error (Printf.sprintf "%s expected success but got error: %s" name message)
      | Expect_guard fragments ->
        if contains_any message fragments
        then Ok ()
        else
          Error
            (Printf.sprintf
    ...
    Fun.protect
      ...
      (fun () ->
         ...
         try
           ...
           | None ->
             Error (Printf.sprintf "missing keeper Tool.t for %s" schema.Types.name)
           | Some tool ->
             ...
             evaluate_expectation ~name:schema.Types.name case.expectation outcome
         with
         | exn ->
           Error
             (Printf.sprintf
                "%s raised during keeper case: %s"
                schema.Types.name
