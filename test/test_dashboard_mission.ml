...
let write_pending_confirm config _session_id =
  ...
  Coord_utils.write_json
    config
    (Filename.concat operator_dir "pending_confirms.json")
    (`List
        [ `Assoc
...
let seed_room config session_id =
  ...
  ignore
    (Lib.Coord.join
       config
       ~agent_name:"llama-local-delta"
       ~capabilities:[ "worker"; "local64"; "observer" ]
       ());
  ignore
    (Lib.Coord.broadcast
       config
       ~from_agent:"team-session-local64-smoke"
       ~content:"@llama-local-alpha recover failed worker coverage");
  ignore
    (Lib.Coord.broadcast
       config
       ~from_agent:"llama-local-alpha"
...
let test_dashboard_mission_projection () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         bool
         "attention_queue is public-only (empty in clean fixture)"
         true
         (attention_queue = []);
       check bool "mission summary trims paused" true (summary |> member "paused" = `Null);
       check
         bool
         "mission summary trims active_agents"
         true
         (summary |> member "active_agents" = `Null);
       check
         bool
         "mission summary namespace_id removed"
         true
         (summary |> member "namespace_id" = `Null);
       check
         bool
         "mission summary namespace removed"
         true
         (summary |> member "namespace" = `Null);
       check
         bool
         "mission summary namespace_mode removed"
         true
         (summary |> member "namespace_mode" = `Null);
       check
         bool
         "sessions removed from mission payload"
         true
         (json |> member "sessions" = `Null);
       let alpha_input = alpha_brief |> member "recent_input_preview" |> to_string in
       check
         bool
         "recent input preserves exact alpha mention"
         true
         (contains alpha_input "@llama-local-alpha");
       check
         bool
         "recent input excludes unrelated beta mention"
         false
         (contains alpha_input "@llama-local-beta");
       check
         bool
         "agent brief omits old audit surface"
         true
         (alpha_brief |> member "allowed_tool_names" = `Null);
       check
         bool
         "agent brief omits social context fields"
         true
         (alpha_brief |> member "where" = `Null);
       check
         string
         "agent brief signal truth"
         "message"
         (alpha_brief |> member "evidence_source" |> to_string);
       check
         bool
         "internal signal includes pending confirm"
         true
         (internal_signals
          |> List.exists (fun row ->
       ...
       check
         bool
         "internal signals are room-scoped"
         true
         (internal_signals
          |> List.for_all (fun row -> row |> member "target_type" |> to_string = "root")))
;;
...
let test_dashboard_mission_http_full_contract () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         bool
         "operator targets present in mission http payload"
         true
         (json |> member "operator_targets" <> `Null);
       check
         bool
         "internal signals retained in mission http payload"
         true
         (json |> member "internal_signals" |> to_list <> []);
       check
         bool
         "command focus retained in mission http payload"
         true
...
let test_dashboard_mission_http_default_bootstraps_first_success () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         string
         "default mission cache becomes fresh"
         "fresh"
         (json |> member "projection_diagnostics" |> member "cache_state" |> to_string);
       check
         bool
         "default mission records first success"
         true
         (json |> member "projection_diagnostics" |> member "last_success_at" <> `Null);
       check
         bool
         "default mission leaves initializing placeholder"
         true
         (json |> member "summary" |> member "room_health" |> to_string <> "initializing");
       check
         bool
         "mission summary namespace_id removed"
         true
...
let test_dashboard_mission_http_cache_isolation () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let request = request ("/api/v1/dashboard/mission?agent_name=" ^ actor) in
       let json_a =
         Lib.Server_dashboard_http.dashboard_mission_http_json
           ~state:state_a
           ~sw
       ...
       check
         bool
         "first room namespace_id removed"
         true
         (json_a |> member "summary" |> member "namespace_id" = `Null);
       check
         bool
         "second room namespace_id removed"
         true
...
let test_dashboard_mission_keeper_tool_audit_prefers_heartbeat_task () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let briefs =
         Lib.Dashboard_mission_assembly.build_keeper_briefs
           ...
           [ `Assoc
               ...
               ; ( "allowed_tool_names"
                 , `List [ `String "masc_board_get"; `String "masc_board_vote" ] )
       ...
       check
         bool
         "heartbeat task allowed tools present"
         true
         (brief |> member "allowed_tool_names" |> to_list <> []);
       check
         string
         "heartbeat task source wins in keeper brief"
         "heartbeat_task"
         (brief |> member "tool_audit_source" |> to_string);
       check
         string
         "heartbeat task preserves fallback action source"
         "structured_model"
         (brief |> member "latest_action_source" |> to_string);
       check
         bool
         "no observed tools without evidence"
         true
...
let test_dashboard_mission_keeper_tool_audit_uses_decision_log () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         string
         "decision log source present in keeper brief"
         "keeper_decision_log"
         (brief |> member "tool_audit_source" |> to_string);
       check
         string
         "decision log action source present in keeper brief"
         "structured_model"
         (brief |> member "latest_action_source" |> to_string);
       check
         int
         "decision log zero tool count preserved"
         0
         (brief |> member "latest_tool_call_count" |> to_int);
       check
         bool
         "decision log still reports empty tool list"
         true
         (brief |> member "latest_tool_names" |> to_list = []))
;;
...
let test_dashboard_mission_keeper_brief_registry_lookup_scoped_to_base_path () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       ignore
         (Result.get_ok
            (Lib.Keeper_exec_tools.init_policy_config ~base_path:policy_base_path));
       let config_a = Coord_utils.default_config dir_a in
       let config_z = Coord_utils.default_config dir_z in
       ignore
         (Lib.Keeper_registry.register
            ~base_path:config_a.base_path
            keeper_name
            (make_meta ~also_allow:[ "keeper_board_vote" ] keeper_name));
       ignore
         (Lib.Keeper_registry.register
            ~base_path:config_z.base_path
            keeper_name
       ...
       check
         bool
         "current base path registry tools included"
         true
         (List.mem "keeper_board_vote" allowed_tool_names);
       check
         bool
         "other base path registry tools excluded"
         false
...
let () =
  Alcotest.run
    ...
    [ ( "read_model"
      , [ Alcotest.test_case
            "projection groups root-cause lanes"
            `Quick
            test_dashboard_mission_projection
        ; Alcotest.test_case
            "http mission keeps full contract"
            `Quick
            test_dashboard_mission_http_full_contract
        ; Alcotest.test_case
            "http mission default bootstraps first success"
            `Quick
            test_dashboard_mission_http_default_bootstraps_first_success
        ; Alcotest.test_case
            "keeper tool audit fallback"
            `Quick
            test_dashboard_mission_keeper_tool_audit_fallback
        ; Alcotest.test_case
            "http mission cache stays room-scoped"
            `Quick
            test_dashboard_mission_http_cache_isolation
        ; Alcotest.test_case
            "keeper brief prefers heartbeat task"
            `Quick
            test_dashboard_mission_keeper_tool_audit_prefers_heartbeat_task
        ; Alcotest.test_case
            "keeper brief uses decision log fallback"
            `Quick
            test_dashboard_mission_keeper_tool_audit_uses_decision_log
        ; Alcotest.test_case
            "keeper brief registry lookup scoped to base path"
            `Quick
