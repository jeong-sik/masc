...
let test_run_verdict_has_reasons () =
  let verdict =
    WO.Run
      { reasons = WO.Mention_pending, [ WO.Task_backlog { unclaimed = 1; failed = 0 } ] }
  in
  let strings = WO.verdict_reasons_to_strings verdict in
  check bool "Run verdict produces non-empty string list" true (List.length strings > 0)
;;

let test_skip_verdict_has_reasons () =
  let verdict = WO.Skip { reasons = WO.No_signal, [] } in
  let strings = WO.verdict_reasons_to_strings verdict in
  check bool "Skip verdict produces non-empty string list" true (List.length strings > 0)
;;
...
let test_recovery_floor_non_empty () =
  ...
  check bool "recovery floor has at least 1 shard" true (List.length shards >= 1)
;;

let test_recovery_floor_tools_non_empty () =
  let tools = KTP.failing_minimum_tool_names () in
  check bool "recovery floor tools non-empty" true (List.length tools >= 1)
;;

let test_base_shard_not_removable () =
  match TS.get_shard "base" with
  | Some shard -> check bool "base shard is not removable" false shard.removable
  | None -> fail "base shard not found"
;;
...
let test_masc_whitelist_allows_playground () =
  check
    ...
    "playground allowed"
    true
    (KTP.is_masc_write_allowed ".masc/playground/keeper1/file.ml")
;;

let test_masc_whitelist_allows_decision_audit () =
  check
    bool
    "decision_audit allowed"
    true
    (KTP.is_masc_write_allowed ".masc/decision_audit/keeper1/2026-04/10.jsonl")
;;
...
let test_masc_whitelist_blocks_reputation () =
  check
    ...
    "reputation blocked"
    false
    (KTP.is_masc_write_allowed ".masc/reputation/keeper1.json")
;;
...
let test_masc_whitelist_blocks_economy () =
  check
    ...
    "economy blocked"
    false
    (KTP.is_masc_write_allowed ".masc/economy/keeper1.jsonl")
;;

let test_masc_whitelist_blocks_autonomy_stats () =
  check
    bool
    "autonomy_stats blocked"
    false
    (KTP.is_masc_write_allowed ".masc/autonomy_stats.jsonl")
;;
...
let test_masc_whitelist_blocks_traversal () =
  check
    ...
    "traversal blocked"
    false
    (KTP.is_masc_write_allowed ".masc/playground/../reputation/evil.json")
;;
...
let test_cascade_mermaid_renders_saturated_reason () =
  ...
  check
    bool
    "saturated reason appears in note"
    true
    (substring_present ~haystack:out ~needle:"unhealthy: saturated")
;;
...
let test_cascade_mermaid_renders_other_reason () =
  ...
  check
    bool
    "other reason string passes through"
    true
    (substring_present ~haystack:out ~needle:"unhealthy: custom-signal")
;;
...
let test_cascade_mermaid_other_sanitizes_newline_colon () =
  ...
  check
    bool
    "sanitized output contains expected string"
    true
    (substring_present ~haystack:out ~needle:"unhealthy: line1 line2 bad");
  check
    bool
    "unsanitized colon not present in note"
    false
    (substring_present ~haystack:out ~needle:"line2:bad")
;;
...
let test_cascade_mermaid_healthy_no_reason_note () =
  ...
  check
    bool
    "healthy provider does not emit unhealthy note"
    false
    (substring_present ~haystack:out ~needle:"unhealthy")
;;
...
let test_observer_idle_when_no_turn () =
  ...
  | Some entry ->
    ...
    check
      string
      "idle keeper has Idle turn phase"
      "idle"
      (Obs.turn_phase_to_string snap.ktc_turn_phase)
;;
...
let test_observer_executing_during_turn () =
  ...
  | Some entry ->
    ...
    check
      string
      "in-turn keeper has Executing turn phase"
      "executing"
      (Obs.turn_phase_to_string snap.ktc_turn_phase)
;;
...
let test_observer_no_stale_after_turn_end () =
  ...
  | Some entry ->
    ...
    check
      string
      "post-turn keeper reverts to Idle (no stale Executing)"
      "idle"
      (Obs.turn_phase_to_string snap.ktc_turn_phase)
;;
...
let test_observer_finished_idempotent () =
  ...
  | Some entry ->
    ...
    check
      string
      "stays Idle through repeated mark_turn_finished"
      "idle"
      (Obs.turn_phase_to_string snap.ktc_turn_phase);
    check bool "no last_outcome when no turn ever started" true (snap.last_outcome = None)
;;
...
let test_observer_last_outcome_preserved_across_finish_idempotent () =
  ...
  check
    bool
    "last_outcome preserved across redundant mark_turn_finished"
    true
    (lo_first = lo_second)
;;
...
let test_observer_snapshot_json_omits_retired_recovery_contract () =
  ...
  | Some entry ->
    ...
    check
      bool
      "recovery object omitted"
      true
      (match YU.member "recovery" json with
       | `Null -> true
       | _ -> false);
    check
      bool
      "recovery_two_store_sync omitted"
      true
      (match YU.member "recovery_two_store_sync" invariants with
       | `Null -> true
       | _ -> false)
;;
...
let test_observer_kcb_clean () =
  ...
  | Some entry ->
    let snap = Obs.observe entry in
    check string "fresh keeper kcb_state = clean" "clean" (kcb_state_str snap.kcb_state)
;;
...
let test_observer_kcb_warning () =
  ...
  ignore (KCB.maybe_enrich_error ~keeper_name:name ~error_msg:"path_not_found: /x");
  match Reg.get ~base_path:test_obs_bp name with
  | None -> Alcotest.fail "entry missing"
  | Some entry ->
    let snap = Obs.observe entry in
    check
      string
      "failing keeper kcb_state = warning"
      "warning"
      (kcb_state_str snap.kcb_state)
;;
...
let test_observer_kcb_json_payload () =
  ...
  | Some entry ->
    ...
    check
      bool
      "circuit_breaker object present"
      true
      (match cb with
       | `Null -> false
       | _ -> true);
    ...
    in
    check string "circuit_breaker.state on fresh keeper = clean" "clean" state_str
;;
...
let () =
  run
    ...
    ; ( "typed_variant_coverage"
      , [ test_case
            "turn_reason_to_string all variants"
            `Quick
            test_turn_reason_to_string_coverage
        ; test_case
            "skip_reason_to_string all variants"
            `Quick
            test_skip_reason_to_string_coverage
        ] )
    ; ( "recovery_floor"
      , [ test_case "floor shard names non-empty" `Quick test_recovery_floor_non_empty
        ; test_case
            "floor tool names non-empty"
            `Quick
            test_recovery_floor_tools_non_empty
    ...
    ; ( "masc_write_protection"
      , [ test_case "allows playground" `Quick test_masc_whitelist_allows_playground
        ; test_case
            "allows decision_audit"
            `Quick
            test_masc_whitelist_allows_decision_audit
        ...
        ; test_case
            "blocks autonomy_stats"
            `Quick
            test_masc_whitelist_blocks_autonomy_stats
    ...
    ; ( "provider_health_reason"
      , [ test_case
            "saturated reason in note"
            `Quick
            test_cascade_mermaid_renders_saturated_reason
        ; test_case
            "other reason passes through"
            `Quick
            test_cascade_mermaid_renders_other_reason
        ; test_case
            "other reason sanitizes newline and colon"
            `Quick
            test_cascade_mermaid_other_sanitizes_newline_colon
        ; test_case
            "healthy has no unhealthy note"
            `Quick
            test_cascade_mermaid_healthy_no_reason_note
        ] )
    ; ( "composite_observer_turn_scope"
      , [ test_case "idle when no turn" `Quick test_observer_idle_when_no_turn
        ...
        ; test_case
            "no stale Executing after turn end"
            `Quick
            test_observer_no_stale_after_turn_end
        ; test_case
            "mark_turn_finished is idempotent"
            `Quick
    ...
    ; ( "composite_observer_phase_2"
      , [ test_case "is_live = true during turn" `Quick test_observer_is_live_during_turn
        ; test_case
            "is_live = false on idle keeper"
            `Quick
            test_observer_is_live_false_when_idle
        ; test_case
            "last_outcome populated after turn"
            `Quick
            test_observer_last_outcome_populated_after_turn
        ; test_case
            "last_outcome preserved across redundant finish"
            `Quick
            test_observer_last_outcome_preserved_across_finish_idempotent
        ; test_case
            "snapshot json omits retired recovery contract"
            `Quick
            test_observer_snapshot_json_omits_retired_recovery_contract
        ] )
    ; ( "composite_observer_kcb_axis"
      , [ test_case "fresh keeper kcb_state = Clean" `Quick test_observer_kcb_clean
        ; test_case "failing keeper kcb_state = Warning" `Quick test_observer_kcb_warning
        ; test_case
            "snapshot_to_json exposes circuit_breaker.state"
            `Quick
            test_observer_kcb_json_payload
        ] )
    ]
;;
