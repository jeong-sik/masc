...
type spawn_config =
  ...
  ; mcp_mode : mcp_flag (** How MCP tools are passed on the CLI. *)
  ; prompt_mode : prompt_flag (** How the prompt is delivered to the agent. *)
  }
...
let parse_claude_output raw =
  try
    ...
    let usage =
      Safe_ops.json_member_opt "usage" json |> Option.value ~default:(`Assoc [])
    in
  ...
  with
  | Yojson.Json_error _ -> parse_raw_output raw
;;
...
let parse_gemini_output raw =
  try
    let json = Yojson.Safe.from_string raw in
    let response_text =
      Option.value (Safe_ops.json_string_opt "response" json) ~default:raw
    in
    ...
    let sum_stat_field field entries =
      ...
      |> List.fold_left
           (fun acc (_name, item) ->
              let tokens =
                Safe_ops.json_member_opt "tokens" item
    ...
    let input_tokens =
      ...
      | Some _ as v -> v
      | None -> from_stats_first [ "input"; "prompt" ]
    in
    let output_tokens =
      match from_usage "candidatesTokenCount" with
      | Some _ as v -> v
      | None -> from_stats_first [ "candidates" ]
    in
    let cached_tokens =
      match from_usage "cachedContentTokenCount" with
      | Some _ as v -> v
      | None -> from_stats_first [ "cached" ]
    in
...
let build_mcp_args_from_config (config : spawn_config) tools =
  ...
  then []
    ...
    | Mcp_joined flag -> [ flag; String.concat "," tools ]
    | Mcp_spread flag -> [ "--allowed-mcp-server-names"; "masc"; flag ] @ tools
    | Mcp_none -> [])
;;
...
let spawn ~agent_name ~prompt ?timeout_seconds ?working_dir () =
  ...
  if Provider_adapter.is_bare_ollama_label normalized_agent
  then
    { success = false
    ...
    let config =
      ...
      | None ->
        ...
        ; command = agent_name
        ; timeout_seconds =
            Option.value timeout_seconds ~default:Env_config.Spawn.timeout_seconds
        ; working_dir
        ; mcp_tools = []
        ; parse_output = parse_raw_output
        ; stdin_prompt = true
    ...
    then
      { success = false
      ; output = Printf.sprintf "spawn command is empty for agent '%s'" agent_name
      ; exit_code = 2
      ; elapsed_ms = int_of_float ((Time_compat.now () -. start_time) *. 1000.0)
      ; input_tokens = None
      ; output_tokens = None
...
let format_token_info result =
  ...
  | Some inp, Some out, Some cost ->
    let cache_info =
      match result.cache_creation_tokens, result.cache_read_tokens with
...
let result_to_string result =
  ...
  then
    Printf.sprintf
      "✅ Agent completed in %dms%s\n\n%s"
      result.elapsed_ms
      token_info
      result.output
  else
    Printf.sprintf
      "❌ Agent failed (exit %d) in %dms%s\n\n%s"
      result.exit_code
      result.elapsed_ms
      token_info
      result.output
