...
export function DetailPane() {
  ...
  return html`
    <div class="rounded border border-accent/30 bg-bg-0/60 shadow-sm" role="region" aria-label="선택 항목 상세 정보">
      ...
      <details class="border-t border-card-border">
        ...
        <pre class="max-h-64 overflow-auto px-3 py-2 text-3xs font-mono text-text-strong bg-[var(--white-5)]/30 leading-[1.4]" tabindex="0" aria-label="Raw JSON 항목">${formatJson(selection.entry)}</pre>
      </details>
    </div>
  `
}
