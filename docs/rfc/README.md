...
| 0090 | Keeper bash tool: cmd string → typed Argv schema (lexer/validator 박멸) | Draft | (this PR) 2026-05-17 | 24h log audit (`~/me/.masc/logs/`, 2026-05-16~17) Top 1 ERROR pattern `Path syntax blocked` 253건 (90 raw + 60 caller mirror + 59 retry + 44 registry recording) 의 single emission point `lib/worker_dev_tools.ml:613` 이 *string-as-protocol 워크어라운드 위에 string-classifier 워크어라운드가 누적된 자기참조 구조*. `worker_dev_tools.ml` 1718-LOC godfile 의 shell metachar lexer/parser stack 박멸 + `keeper_bash` 입력 schema 를 `{cmd: string}` 에서 `Exec / Pipeline` typed variant 로 전환. 4-phase migration. RFC-0089 §2 scope-out (외부 protocol boundary) 의 자매 RFC. Related RFC-0042, RFC-0084, RFC-0089 |
