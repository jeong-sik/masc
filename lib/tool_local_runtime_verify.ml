...
let safe_discovery_endpoints () =
  try Some (Discovery_cache.get_cached_or_refresh ()) with
    ...
    let matches_pool (endpoint : Discovery_cache.endpoint_info) =
      ...
      | Some pool
        when String.equal pool Local_runtime_pool.default_pool_label
             || String.equal pool "default" -> true
      | Some pool ->
        String.equal pool endpoint.url
        || String.equal pool (Local_runtime_pool.runtime_id_of_base_url endpoint.url)
    in
...
let probe_chat_completion_compatible
  ...
  | _, None ->
    Log.warn
      ~ctx:"runtime_verify"
      "chat-completions probe skipped caller_surface=runtime_verify endpoint=%s \
  ...
  | Some env, Some model_id ->
    Log.info
      ~ctx:"runtime_verify"
      "chat-completions probe caller_surface=runtime_verify endpoint=%s model_id=%s \
       timeout_sec=%d"
      endpoint.url
      model_id
      timeout_sec;
    ...
    let outcome =
      ...
      | Some clock ->
        (try
           Ok (Eio.Time.with_timeout_exn clock (float_of_int timeout_sec) run_completion)
         with
    ...
    (match outcome with
     ...
     | Ok (Error http_error) -> Some false, Some (error_message_of_http_error http_error))
;;

let provider_health_reachable ~status ~body:_ = status = Some 200
...
let chat_contract_probe_body ~model_id =
  Yojson.Safe.to_string
    (`Assoc
        ...
        ; ( "messages"
          , `List [ `Assoc [ "role", `String "user"; "content", `String "ping" ] ] )
        ; "max_tokens", `Int 1
...
let classify_runtime_blocker
  ...
  =
  if (not provider_reachable) || not slot_reachable
  then Some "provider_unreachable", Some "llama runtime health or slots endpoint failed"
  else if not chat_completion_compatible
  then
  ...
  then
    ...
    , Some
        (Printf.sprintf
           "expected at least %d slots, got %d"
           (Option.value ~default:0 expected_slots)
           actual_slots_total) )
  else if
  ...
  then
    ...
    , Some
        (Printf.sprintf
           "expected ctx %s, got %s"
           (match expected_ctx with
            | Some value -> string_of_int value
            | None -> "<none>")
           (match actual_ctx with
            | Some value -> string_of_int value
            | None -> "<mixed-or-missing>")) )
  else if String.equal chat_contract_status "rejected"
  then
...
let runtime_verify_json_from_discovery
      ?runtime_pool
      ?expected_slots
      ?expected_ctx
      ?expected_model
  ...
  let ( runtime_rows
    ...
    =
    List.fold_left
      (fun (rows, provider_ok, slot_ok, slots_acc, active_acc, ctxs, models, chat_ok)
        (endpoint : Discovery_cache.endpoint_info) ->
         ...
         let row =
           `Assoc
             [ ( "runtime_id"
               , `String (Local_runtime_pool.runtime_id_of_base_url endpoint.url) )
             ...
             ; ( "provider_status_code"
               , int_opt_to_json (if provider_ok_row then Some 200 else None) )
             ; ( "provider_error"
               , string_opt_to_json
                   (if provider_ok_row
                    then None
             ...
             ; ( "slot_status_code"
               , int_opt_to_json (if slot_ok_row then Some 200 else None) )
             ; "slot_error", `Null
             ; ( "props_status_code"
               , int_opt_to_json
             ...
             ; "props_error", `Null
             ; ( "models_status_code"
               , int_opt_to_json (if endpoint.models <> [] then Some 200 else None) )
             ; "models_error", `Null
         ...
         , (match actual_ctx with
            | Some value -> value :: ctxs
            | None -> ctxs)
         , (match actual_model with
            | Some value -> value :: models
            | None -> models)
         , match chat_probe_ok with
           | Some false -> false
           | _ -> chat_ok ))
  ...
  in
  let actual_ctx =
    match List.sort_uniq compare actual_ctxs with
    | [ value ] -> Some value
    | _ -> None
  in
  let actual_model_id =
  ...
  let runtime_blocker, detail =
    classify_runtime_blocker
      ...
      ~slot_reachable
      ~chat_contract_status:
        (if chat_completion_compatible then "confirmed" else "rejected")
      ~expected_model
      ~actual_model_id
      ~expected_slots
      ~actual_slots_total
...
let runtime_verify_json_legacy
      ?runtime_pool
      ?expected_slots
      ?expected_ctx
      ?expected_model
  ...
  let ( runtime_rows
      ...
      , actual_ctxs
      , actual_models
      , chat_contract_statuses )
    =
    List.fold_left
      (fun (rows, provider_ok, slot_ok, slots_acc, active_acc, ctxs, models, chat_states)
        (runtime : Local_runtime_pool.runtime_snapshot) ->
         ...
         let models_url = base_url ^ Masc_network_defaults.openai_models_path in
         ...
         let probe_model =
           ...
           | None ->
             (match expected_model with
              | Some model_id when String.trim model_id <> "" ->
                Some (String.trim model_id)
              | _ -> runtime.model)
         in
         let chat_status, chat_body, chat_err =
           ...
           | Some model_id ->
             let url = base_url ^ Masc_network_defaults.openai_chat_completions_path in
         ...
         let row =
           `Assoc
             ...
             ; ( "chat_contract_reachable"
               , `Bool (chat_contract_reachable ~status:chat_status ~body:chat_body) )
  ...
  let overall_chat_contract_status =
    ...
    if not has_runtimes
    then "unknown"
    else if List.mem "rejected" states
    then "rejected"
    else if List.mem "unknown" states
    then "unknown"
    else "confirmed"
  in
  let runtime_blocker, detail =
    classify_runtime_blocker
      ...
      ~slot_reachable:(slot_reachable && has_runtimes)
      ~expected_model
      ~actual_model_id
      ~expected_slots
      ~actual_slots_total
  ...
  `Assoc
    ...
    ; ( "chat_contract_reachable"
      , `Bool (String.equal overall_chat_contract_status "confirmed") )
    ; "expected_model", string_opt_to_json expected_model
    ; "actual_model_id", string_opt_to_json actual_model_id
    ; "expected_slots", int_opt_to_json expected_slots
    ; "actual_slots", `Int actual_slots_total
