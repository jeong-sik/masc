...
(** Bound the narrative fields of a [social_state] before it leaves the
    speech model. Caps each option field ([need]) with an ellipsis
    marker when it exceeds the budget. The [blocker] field is
    special-cased via {!cap_blocker} so structured [masc_oas_error]
    payloads are preserved intact (#9933). Idempotent. *)
