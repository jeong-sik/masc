...
let handle_keeper_bash_output
  ...
  if raw_id = ""
  then error_json "task_id is required. Example: task_id='bgt-<timestamp>-<seq>-<pid>'."
  else (
         ...
         let semantic_fields =
           ...
           then []
             ...
             | Some st ->
               ...
               let sem =
                 Masc_exec.Exec_semantic.interpret_cmd ~cmd:"" ~status:st ~output:merged
               in
...
let handle_keeper_bash_kill
  ...
  let grace_sec =
    ...
    if raw < 0.0 then 0.0 else if raw > 30.0 then 30.0 else raw
  in
  if raw_id = ""
  then error_json "task_id is required. Example: task_id='bgt-<timestamp>-<seq>-<pid>'."
  else (
