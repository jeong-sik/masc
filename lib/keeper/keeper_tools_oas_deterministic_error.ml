...
let recovery_plan_json_opt json =
  match Json_util.assoc_member_opt "recovery_plan" json with
  | Some (`Assoc _ as plan) -> Some plan
  | _ ->
    (match detail_json_opt json with
     | Some detail ->
       (match Json_util.assoc_member_opt "recovery_plan" detail with
        | Some (`Assoc _ as plan) -> Some plan
        | _ -> None)
     | None -> None)
;;
...
let deterministic_recovery_plan_fields raw =
  try
    ...
    | Some plan ->
      let next_tool =
        match Json_util.assoc_string_opt "next_tool" plan with
