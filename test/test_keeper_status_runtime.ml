...
let test_runtime_surface_derives_route_exhausted_from_meta () =
  KR.clear ();
  ...
  let reason =
    ...
    "Internal error: [masc_oas_error] {\"kind\":\"route_exhausted\",\"cascade_name\":\"primary\",\"reason\":\"connection_refused\"}"
  in
  let meta =
    {
      base with
      runtime =
        {
          ...
          last_blocker =
            Some (Keeper_meta_contract.blocker_info_of_class ~detail:reason
                    (Keeper_meta_contract.Route_exhausted Keeper_meta_contract.Connection_refused));
        };
    }
  in
  let config =
...
let test_runtime_surface_names_no_tool_provider_details () =
  ...
  in
  check string "runtime blocker class"
    "route_exhausted_no_tool_capable"
    (runtime |> member "runtime_blocker_class" |> to_string);
...
let test_runtime_blocker_summary_is_not_reparsed_from_masc_error_payload () =
  ...
  in
  let no_tool_surface =
    KSB.runtime_blocker_surface_of_typed_class ~summary (Keeper_meta_contract.Route_exhausted (Keeper_meta_contract.No_tool_capable None))
  in
  ...
  check string "cascade class stays typed" "route_exhausted"
    cascade_surface.blocker_class;
  check string "no-tool class stays typed" "route_exhausted_no_tool_capable"
      ...
      ( "surface_status",
        [
          ...
          test_case "runtime surface derives cascade exhausted blocker" `Quick
            test_runtime_surface_derives_route_exhausted_from_meta;
