...
def category:
  ...
  elif (combined | test("is a MASC tool, not a shell command|tool_invoked_as_shell_command|`?gh`? is NOT available in the keeper sandbox|Use scoped gh via Execute|For pull-request work use scoped gh via Execute|Use the typed Execute GitHub path|For pull-request work use the typed Execute GitHub path"; "i")) then "wrong_tool_channel"
  ...
  elif (combined | test("Execute cannot bypass the PR creation approval|tool_execute cannot bypass the PR creation approval|gh_pr_create_requires_execute_gh"; "i")) then "pr_create_policy_bypass"
