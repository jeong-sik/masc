...
let enqueue_autonomous_waiter ~(keeper_name : string) : int =
  with_autonomous_wait_queue (fun () ->
    ...
    autonomous_wait_queue := !autonomous_wait_queue @ [ { ticket; keeper_name } ];
    ticket)
;;
...
let autonomous_waiter_position ~(ticket : int) : int option =
  with_autonomous_wait_queue (fun () ->
    let rec loop idx = function
      | [] -> None
      | waiter :: rest -> if waiter.ticket = ticket then Some idx else loop (idx + 1) rest
    in
...
let others_waiting_in_queue ~(keeper_name : string) : bool =
  with_autonomous_wait_queue (fun () ->
    List.exists (fun w -> w.keeper_name <> keeper_name) !autonomous_wait_queue)
;;
...
let with_keeper_turn_slot ~keeper_name ~channel f =
  ...
  let acquire_bounded ~label sem =
    ...
    | Some clock ->
      (try
       ...
       | Eio.Time.Timeout ->
         ...
         Log.Keeper.info
           "semaphore_wait: %s semaphore wait exceeded %.0fs (channel=%s), skipping turn"
           label
           semaphore_wait_timeout_sec
           channel_label;
    ...
    | None ->
      ...
      Log.Keeper.warn
        "semaphore_wait: no Eio clock available — %s acquire will be unbounded \
         (environment drift?)"
        label;
      Eio.Semaphore.acquire sem;
      Ok ()
       ...
       | Ok () ->
         (match acquire_bounded ~label:"turn" turn_semaphore with
          ...
          | Ok () ->
            ...
            let semaphore_wait_ms = int_of_float ((Time_compat.now () -. t0) *. 1000.0) in
...
let wakeup_all_keepers ?base_path () =
  ...
  | Some expected ->
    ...
    |> List.iter (fun (entry : Keeper_registry.registry_entry) ->
      ...
      then Keeper_registry.wakeup ~base_path:entry.base_path entry.name)
;;
...
let wakeup_relevant_keeper_for_board_signal
  ...
  let explicit =
    candidates
    |> List.filter (fun (_meta, wake_reason) -> wake_reason = Some "explicit_mention")
  in
...
let dispatch_keepalive_event ~(ctx : _ context) ~(keeper_name : string) event =
  if keepalive_entry_accepts_late_event ~ctx ~keeper_name
  then
    ignore
      (Keeper_registry.dispatch_event ~base_path:ctx.config.base_path keeper_name event)
;;
...
let write_heartbeat_snapshot
  ...
  let cascade_models = Cascade_runtime.models_of_cascade_name meta_current.cascade_name in
  ...
  let messages_for_continuity =
    match ctx_opt with
    | Some c -> Keeper_exec_context.messages_of_context c
    | None ->
      let history_path =
        Keeper_types.keeper_history_path
          ctx.config
          (Keeper_id.Trace_id.to_string meta_current.runtime.trace_id)
      in
      let internal_history_path =
        Keeper_types.keeper_internal_history_path
          ctx.config
          (Keeper_id.Trace_id.to_string meta_current.runtime.trace_id)
      in
      ...
      let messages =
        try
          ...
          |> List.filter_map (fun line ->
            try
              ...
              in
              if
                Keeper_types.is_prompt_history_source source
        ...
        | exn ->
          Log.Keeper.warn
            "write_heartbeat_snapshot: history.jsonl load error (%s): %s"
            meta_current.name
      ...
      then
        Log.Keeper.warn
          "write_heartbeat_snapshot: failed to parse %d message(s) from history logs for \
           keeper=%s trace_id=%s path=%s"
          !parse_errors
          meta_current.name
          (Keeper_id.Trace_id.to_string meta_current.runtime.trace_id)
          history_path;
  ...
  let similarity_measurable =
    Option.is_some latest_user_message && Option.is_some latest_assistant_message
  in
  let context_ratio_v =
    match ctx_opt with
  ...
  in
  let turn_fail_count =
    Keeper_registry.get_turn_failures ~base_path:ctx.config.base_path meta_current.name
  in
  ...
  if auto_rules.guardrail_stop && Keeper_decision_audit.decision_layer_level () >= 2
  then (
    try Thompson_sampling.record_guard_penalty ~agent_name:meta_current.name with
    | Eio.Cancel.Cancelled _ as e -> raise e
    | exn ->
      Log.Keeper.warn
        "guard→thompson penalty failed for %s: %s"
        meta_current.name
        (Printexc.to_string exn));
  let snapshot =
    `Assoc
  ...
  (try
   ...
   with
   | Eio.Cancel.Cancelled _ as e -> raise e
   | exn -> Log.Keeper.error "heartbeat SSE broadcast failed: %s" (Printexc.to_string exn));
  ...
  | exn ->
    Log.Keeper.warn
      "keeper:%s flush_tool_usage failed: %s"
      meta_current.name
      (Printexc.to_string exn)
;;
      ...
      then (
        ...
        ignore
          (Keeper_registry.dispatch_event
             ...
             Keeper_state_machine.Heartbeat_ok);
        Prometheus.inc_counter
          Prometheus.metric_keeper_heartbeat_successes
          ~labels:[ "keeper", meta_current.name ]
          ();
        maybe_recover_from_failing ~ctx ~meta:meta_current);
      ...
      then (
        ...
        log_not_scheduled
          "keepalive turn not scheduled for %s: should_run=%b channel=%s reasons=[%s] \
           idle=%ds since_last=%s idle_gate=%s cooldown=%s task_cooldown=%s"
      ...
      then (
        ...
        | Error (`Semaphore_wait_timeout wait_sec) ->
          ...
          Log.Keeper.warn
            ...
            wait_sec;
          meta_after_triage)
      else if (not has_message_signal) && obs.message_cursor_updates <> []
...
let run_smart_heartbeat_gate
  ...
  (match smart_hb_decision with
   ...
   | Heartbeat_smart.Emit -> last_heartbeat_cycle_ts := Time_compat.now ());
...
let publish_keeper_lifecycle
  ...
  =
  match get_bus () with
  | Some bus -> Oas_events.publish_keeper_lifecycle bus ~event ~keeper_name ~detail ()
  | None -> ()
;;
