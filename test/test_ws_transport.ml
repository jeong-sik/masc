...
let test_parse_sse_dashboard_event_known_type () =
  ...
  | Some parsed ->
    Alcotest.(check string) "event_type preserved" "execution_snapshot" parsed.event_type;
    Alcotest.(check (option string))
      "execution_snapshot maps to execution"
      (Some "execution")
      parsed.slice
...
let test_parse_sse_dashboard_event_composite_change_maps_to_composite () =
  ...
  | Some parsed ->
    Alcotest.(check string)
      ...
      parsed.event_type;
    Alcotest.(check (option string))
      "composite change maps to composite"
      (Some "composite")
      parsed.slice
...
let test_parse_sse_dashboard_event_unknown_type () =
  ...
  match Ws.parse_sse_dashboard_event event_str with
  | Some parsed ->
    Alcotest.(check (option string)) "no slice for unknown type" None parsed.slice
...
let test_parse_sse_dashboard_event_malformed () =
  ...
  Alcotest.(check bool) "malformed yields None" true (Option.is_none result)
;;
...
let test_parse_sse_dashboard_event_stable_on_repeat () =
  ...
  Alcotest.(check (option (pair string (option string)))) "repeat returns same shape" a b
;;

let test_parse_sse_dashboard_event_invalidated_on_new_ref () =
  let e1 = Yojson.Safe.to_string (`Assoc [ "type", `String "execution_snapshot" ]) in
  let e2 =
    Yojson.Safe.to_string (`Assoc [ "type", `String "transport_health_snapshot" ])
  in
  let et = function
  ...
  Alcotest.(check (option string)) "first parse" (Some "execution_snapshot") (et r1);
  Alcotest.(check (option string))
    "second parse distinct"
    (Some "transport_health_snapshot")
    (et r2)
;;
...
let test_parse_sse_dashboard_event_handles_sse_format () =
  ...
  in
  let sse_formatted = Printf.sprintf "id: 42\nevent: message\ndata: %s\n\n" body in
  match Ws.parse_sse_dashboard_event sse_formatted with
  | Some parsed ->
    Alcotest.(check string)
      "event_type extracted from SSE wrapper"
      "execution_snapshot"
      parsed.event_type;
    Alcotest.(check (option string))
      "slice resolved past the wrapper"
      (Some "execution")
      parsed.slice
...
let test_parse_sse_dashboard_event_finds_data_line () =
  ...
  | Some parsed ->
    Alcotest.(check string)
      "event_type extracted without positional match"
      "transport_health_snapshot"
      parsed.event_type;
    Alcotest.(check (option string))
      "slice resolved from data line"
      (Some "transport")
      parsed.slice
...
let test_parse_cache_counters () =
  ...
  let e = Yojson.Safe.to_string (`Assoc [ "type", `String "execution_snapshot" ]) in
  let (_ : _ option) = Ws.parse_sse_dashboard_event e in
  (* miss *)
  let (_ : _ option) = Ws.parse_sse_dashboard_event e in
  (* hit *)
  let (_ : _ option) = Ws.parse_sse_dashboard_event e in
  ...
  Alcotest.(check (float 0.001)) "two hits observed" 2.0 (hits1 -. hits0);
  Alcotest.(check (float 0.001)) "one miss observed" 1.0 (misses1 -. misses0);
  (* A fresh string with the same content forces a reparse (physical
     inequality) — proves the cache key is not structural equality. *)
  let e2 = Yojson.Safe.to_string (`Assoc [ "type", `String "execution_snapshot" ]) in
  let (_ : _ option) = Ws.parse_sse_dashboard_event e2 in
  (* miss *)
  let misses2 = read_counter misses_name in
  Alcotest.(check (float 0.001)) "fresh allocation forces miss" 1.0 (misses2 -. misses1)
;;
...
let test_bytes_of_shared_text_reuses_same_ref () =
  ...
  Alcotest.(check bool) "same string ref returns same Bytes" true (b1 == b2)
;;
...
let test_bytes_of_shared_text_content_matches () =
  ...
  Alcotest.(check int) "length matches" (String.length text) (Bytes.length bytes);
  Alcotest.(check string) "content round-trips" text (Bytes.to_string bytes)
;;
...
let test_bytes_of_shared_text_repairs_invalid_utf8_for_text_frames () =
  let text =
    "id: 42\n\
     event: message\n\
     data: {\"type\":\"transport_health_snapshot\",\"payload\":\"bad\xC3\"}\n\n"
  in
  let bytes = Ws.bytes_of_shared_text text in
  let wire = Bytes.to_string bytes in
  Alcotest.(check bool) "wire payload is valid UTF-8" true (String.is_valid_utf_8 wire);
  Alcotest.(check bool)
    "invalid byte is replaced"
    true
    (contains_substring wire "\xEF\xBF\xBD")
;;
...
let test_bytes_of_shared_text_invalidates_on_new_ref () =
  ...
  Alcotest.(check bool) "distinct refs get distinct bytes" true (not (ba == bb));
  Alcotest.(check string) "content still correct for A" a (Bytes.to_string ba);
  Alcotest.(check string) "content still correct for B" b (Bytes.to_string bb)
;;
...
let test_bytes_cache_counters () =
  ...
  let _ = Ws.bytes_of_shared_text text in
  (* hit *)
  Alcotest.(check (float 0.001)) "two hits observed" 2.0 (read_counter hits_name -. hits0);
  Alcotest.(check (float 0.001))
    "one miss observed"
    1.0
    (read_counter misses_name -. misses0);
  ...
  let _ = Ws.bytes_of_shared_text text' in
  Alcotest.(check (float 0.001))
    "fresh allocation forces another miss"
    2.0
    (read_counter misses_name -. misses0)
;;
...
let test_observe_ws_client_buffered_bytes_accumulates () =
  ...
  Alcotest.(check (float 0.001))
    "sum increased by 350"
    350.0
    (read_counter sum_name -. sum0);
  Alcotest.(check (float 0.001))
    "count increased by 2"
    2.0
    (read_counter count_name -. cnt0);
  Alcotest.(check (float 0.001))
    "ack counter increased by 2"
    2.0
    (read_counter ack_name -. ack0)
;;
...
let test_observe_ws_client_buffered_bytes_clamps_negative () =
  ...
  Alcotest.(check (float 0.001))
    "negative observation floors to 0"
    0.0
    (read_counter sum_name -. sum0)
;;
...
let test_backpressure_gate_unauthenticated_ignored () =
  ...
  with_env_var "MASC_WS_CLIENT_BUFFER_LIMIT_BYTES" "1" (fun () ->
    ...
    let expected =
      ...
      false
    in
    Alcotest.(check bool) "unauthenticated session cannot be backpressured" false expected)
;;
...
let test_backpressure_gate_default_is_one_mib () =
  ...
  Ws.__test_reset_env_caches ();
  let limit = Ws.client_buffer_limit_bytes () in
  Alcotest.(check int) "default limit is 1 MiB" 1048576 limit
;;
...
let test_backpressure_gate_throttle_counter_increments () =
  ...
  Metrics.inc_ws_throttled_delivery ();
  Alcotest.(check (float 0.001))
    "throttle counter advances per drop"
    2.0
...
let test_ws_external_subscriber_receives_broadcast () =
  Eio_main.run (fun _env ->
    ...
    Alcotest.(check bool)
      "event contains data:"
      true
      (String.length (List.hd !received) > 0);
...
let test_ws_multi_session_broadcast () =
  Eio_main.run (fun _env ->
    ...
    and r2 = ref []
    and r3 = ref [] in
    Sse.subscribe_external ~id:"ws-multi-1" ~callback:(fun ev -> r1 := ev :: !r1) ();
    Sse.subscribe_external ~id:"ws-multi-2" ~callback:(fun ev -> r2 := ev :: !r2) ();
    Sse.subscribe_external ~id:"ws-multi-3" ~callback:(fun ev -> r3 := ev :: !r3) ();
...
let test_ws_unsubscribe_stops_delivery () =
  Eio_main.run (fun _env ->
    ...
    Sse.subscribe_external ~id:sub_id ~callback:(fun ev -> received := ev :: !received) ();
...
let test_ws_dead_subscriber_auto_removed () =
  Eio_main.run (fun _env ->
    ...
    Alcotest.(check bool)
      "subscriber removed"
      true
      (ext_count = 0 || not (List.mem sub_id (List.init ext_count (fun _ -> "")))))
;;

let test_ws_external_subscriber_count () =
  Eio_main.run (fun _env ->
    let before = Sse.external_subscriber_count () in
    Sse.subscribe_external ~id:"ws-count-1" ~callback:(fun _ -> ()) ();
    Sse.subscribe_external ~id:"ws-count-2" ~callback:(fun _ -> ()) ();
...
let test_slice_index_starts_empty_for_unknown_slice () =
  Eio_main.run (fun _env ->
    ...
    Alcotest.(check bool)
      "fresh session id not present"
      true
      (not (List.mem "ws-slice-test-fresh" subs)))
;;
...
let test_slice_index_add_records_session () =
  Eio_main.run (fun _env ->
    ...
    let subs = Ws.slice_index_subscribers "execution" in
    Alcotest.(check bool) "session present after add" true (List.mem sid subs);
    Ws.__test_slice_index_remove_session sid)
;;
...
let test_slice_index_remove_specific_slice () =
  Eio_main.run (fun _env ->
    ...
    Alcotest.(check bool) "removed from execution" true (not (List.mem sid exec));
    Alcotest.(check bool) "still in keepers" true (List.mem sid keepers);
    Ws.__test_slice_index_remove_session sid)
;;
...
let test_slice_index_size_reflects_pairs () =
  Eio_main.run (fun _env ->
    ...
    Alcotest.(check int) "size grew by exactly the new pair count" 3 (after - baseline);
    Ws.__test_slice_index_remove_session sid_a;
    Ws.__test_slice_index_remove_session sid_b;
    let final = Ws.slice_index_size () in
    Alcotest.(check int) "size returns to baseline after sweep" baseline final)
;;
...
let test_slice_index_add_is_idempotent () =
  Eio_main.run (fun _env ->
    ...
    Alcotest.(check int) "session appears at most once after duplicate adds" 1 occurrences;
    Ws.__test_slice_index_remove_session sid)
;;
...
let test_slice_fanout_flag_default_is_on () =
  Eio_main.run (fun _env ->
    ...
    with_env_var "MASC_WS_SLICE_INDEX_ENABLED" "" (fun () ->
      ...
      Alcotest.(check bool) "default on" true (Ws.slice_index_enabled ())))
;;
...
let test_slice_fanout_flag_reads_env () =
  Eio_main.run (fun _env ->
    ...
    with_env_var "MASC_WS_SLICE_INDEX_ENABLED" "true" (fun () ->
      Alcotest.(check bool) "env=true → enabled" true (Ws.slice_index_enabled ()));
    Ws.__test_reset_env_caches ();
    with_env_var "MASC_WS_SLICE_INDEX_ENABLED" "false" (fun () ->
      Alcotest.(check bool) "env=false → disabled" false (Ws.slice_index_enabled ())))
;;
...
let () =
  Alcotest.run
    ...
    ; ( "ack_observability"
      , [ Alcotest.test_case
            "buffered_bytes sum and count track observations"
            `Quick
            test_observe_ws_client_buffered_bytes_accumulates
        ; Alcotest.test_case
            "negative buffered_bytes floor to zero"
            `Quick
    ...
    ; ( "backpressure_gate"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "throttle counter advances per skipped delivery"
            `Quick
    ...
    ; ( "slice_index"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "size tracks (slice × session) pair count"
            `Quick
    ...
    ; ( "slice_fanout_gate"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case "flag reads env var" `Quick test_slice_fanout_flag_reads_env
        ] )
    ]
;;
