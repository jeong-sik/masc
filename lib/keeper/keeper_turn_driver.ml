  ...
  attempt_runtime_candidates
    ...
    ~run_attempt:(fun ~idx ~runtime_id:attempt_runtime_id candidate ->
      ...
      let has_tools, surface_enabled = match runtime.Runtime.execution with
        ...
        | Runtime_execution.Claude_code _ -> tools <> [], runtime.model.tools_support
        (* MASC tools are suppressed on Muse: the turn runs the client's own
           tools only, so the attempt carries no tool surface. *)
        | Runtime_execution.Muse_cli _ -> false, false in
      ...
      (match runtime.Runtime.execution with
       ...
       | Codex_app_server _ | Claude_code _ | Antigravity_cli _ | Muse_cli _ ->
      ...
      | (Runtime_execution.Codex_app_server _
        ...
        | Runtime_execution.Muse_cli _) when Option.is_some recovery_view ->
                ...
                Error
                  (Agent_core.Error.Config
                     (Agent_core.Error.InvalidConfig
                        { field = "provider_config_transform"
                        ; detail =
                            "provider config transforms cannot target a muse-cli runtime"
                        }))
            ; settled_session = None
            ; effect_disposition =
                Keeper_provider_attempt_effect.No_effect_observed
