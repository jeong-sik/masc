...
`content_revision` 포함)를 싣는다. 매 턴 3,765바이트다. 회수는 0.0072회/턴.
...
스킬당 약 190바이트가 줄어 블록이 약 1.9KB 가 된다.
...
| 지표 | 방법 | 기준선 |
|---|---|---|
| `Available:` 바이트 | 라이브 카탈로그로 블록 재구성 | 3,765 B |
