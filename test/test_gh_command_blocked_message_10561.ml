...
let test_disallowed_command_surfaces_allowed_list () =
  ...
  | Error msg ->
    check
      ...
      true
      (contains msg "allowed=["
       && contains msg "pr"
       && contains msg "issue"
       && contains msg "repo");
    check
      bool
      "error must still mention the rejected command"
      true
      (contains msg "'auth'")
;;
...
let () =
  run
    "gh_command_blocked_message_10561"
    [ ( "error-message-shape"
      , [ test_case
            "disallowed command lists allowed alternatives"
            `Quick
            test_disallowed_command_surfaces_allowed_list
        ; test_case
            "allowed command (pr list) still passes"
            `Quick
