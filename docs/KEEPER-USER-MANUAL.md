...
OAS pin metadata is generated from `scripts/oas-agent-sdk-pin.sh`. Current dependency floor: `agent_sdk >= 0.200.7`, runtime pin: `main@19f06f0965b9d81374c18e18f43da571eb90fad0`, declared base version: `v0.200.7`. 최신성 검증이 필요할 때는 문서에 적힌 숫자보다 `dune-project`와 pin script를 우선 truth source로 본다.
...
| `OAS_CLI_TOOL_A_DISALLOWED_TOOLS=Execute,Write` | CLI-Tool-A | 쉼표 분리된 각 항목마다 `--disallowedTools` |
...
- **Allowed value sets**: `tool_preset ∈ {minimal, social, messaging, research, delivery, full}`, `sandbox_profile ∈ {local, docker}`, `network_mode ∈ {none, inherit}`, `git_identity_mode ∈ {keeper_alias, repo_cli_identity}`, `social_model ∈ {bdi_speech_v1, magentic_ledger_v1}`, `runtime_id`는 `keeper_runtime.toml`에 `<name>_models` 키로 존재해야 함.
