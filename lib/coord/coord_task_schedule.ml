...
let verification_blocks_claim latest_status_by_task (task : Types.task) =
  ...
  | Some (_, `Pending) | Some (_, `Assigned) | Some (_, `Rejected) -> true
...
let required_tools_allowed ?agent_tool_names required_tools =
  ...
  | [], _ -> true
  | _ :: _, None -> true
  | required, Some allowed -> List.for_all (string_list_contains allowed) required
;;
...
let agent_current_task_matches_backlog backlog ~agent_name task_id =
  match
    List.find_opt (fun (task : Types.task) -> String.equal task.id task_id) backlog.tasks
  with
  | Some task ->
    (match task.task_status with
     | Claimed { assignee; _ }
     | InProgress { assignee; _ }
     | AwaitingVerification { assignee; _ } -> String.equal assignee agent_name
    ...
    with_file_lock config agent_file (fun () ->
      ...
      | Ok agent ->
        (match agent.current_task with
         | Some task_id
           when not (agent_current_task_matches_backlog backlog ~agent_name task_id) ->
           let updated_status =
            ...
            Coord_task.observe_task_transition
              ...
              ~details:
                (Coord_task.task_transition_details
                   ~from_status:prev.task_status
                   ~to_status:Types.Todo
                   ~reason:"auto_release_before_claim_next"
                   ())
          | None -> ()
        in
        let released_task_id, working_tasks =
          match previous_claim with
          ...
          | Some prev ->
            ...
            let updated =
              List.map
                (fun (t : Types.task) ->
                   if String.equal t.id prev.id then { t with task_status = Todo } else t)
                backlog.tasks
            in
          ...
          List.sort
            (fun a b ->
               ...
               then priority_cmp
                 ...
                 compare
                   b.created_at
                   a.created_at (* Newer first to unblock stale queues *))
            working_tasks
        in
        (* Identify blocked Todo tasks for observability *)
        let all_todo =
          List.filter (fun (t : Types.task) -> t.task_status = Types.Todo) sorted
        in
        let blocked_todo =
          List.filter
            (fun (t : Types.task) ->
        ...
        then
          log_event
            ...
            (Printf.sprintf
               ...
               agent_name
               (List.length verification_blocked_todo)
               (now_iso ()));
        ...
        (match all_todo, eligible with
         ...
         | _ :: _, task :: _ ->
           ...
           let message =
             ...
             | Some rid ->
               Printf.sprintf
                 "⚠ %s auto-released %s, then claimed [P%d] %s: %s"
                 agent_name
                 rid
                 task.priority
                 task.id
