...
let rec rm_path path =
  ...
  then
    if Sys.is_directory path
    then (
      Array.iter (fun name -> rm_path (Filename.concat path name)) (Sys.readdir path);
      Unix.rmdir path)
    else Unix.unlink path
;;
...
let clear_checkout_but_keep_git_dir repo =
  Array.iter
    (fun name -> if name <> ".git" then rm_path (Filename.concat repo name))
    (Sys.readdir repo)
;;
...
let write_keeper_toml ~base_path ~name ~sandbox_profile =
  let path =
    Filename.concat base_path (Printf.sprintf ".masc/config/keepers/%s.toml" name)
  in
  write_file
    path
    (Printf.sprintf
...
let test_dispatch_worktree_create_spoofed_agent_blocked () =
  ...
  | Some (false, msg) ->
    check
      bool
      "error mentions agent_name mismatch"
      true
      (contains "agent_name mismatch" msg);
    check bool "error mentions the caller ctx agent" true (contains "test-agent" msg);
    check bool "error mentions the spoofed arg value" true (contains "other-agent" msg);
    check bool "error explains cross-agent is blocked" true (contains "Cross-agent" msg)
;;
...
let test_dispatch_worktree_create_matching_agent_passes_check () =
  ...
  | Some (_ok, msg) ->
    check
      bool
      "matching agent_name does not trip spoof gate"
      false
      (contains "agent_name mismatch" msg)
;;
...
let test_dispatch_worktree_create_empty_agent_falls_back () =
  ...
  let ctx = make_ctx () in
  let args =
    `Assoc [ "task_id", `String "task-empty-fallback"; "base_branch", `String "main" ]
  in
  match Tool_worktree.dispatch ctx ~name:"masc_worktree_create" ~args with
  | None -> fail "dispatch returned None for masc_worktree_create"
  | Some (_ok, msg) ->
    check
      bool
      "empty agent_name arg does not trip spoof gate"
      false
      (contains "agent_name mismatch" msg)
;;
...
let test_dispatch_worktree_create_whitespace_agent_trimmed () =
  ...
  | Some (_ok, msg) ->
    check
      bool
      "whitespace agent_name trimmed to fallback"
      false
      (contains "agent_name mismatch" msg)
;;
...
let test_dispatch_worktree_create_reports_missing_sandbox_clone () =
  ...
  match Tool_worktree.dispatch ctx ~name:"masc_worktree_create" ~args with
  | None -> fail "dispatch returned None for masc_worktree_create"
  | Some (true, msg) -> fail ("expected missing_sandbox_clone error, got success: " ^ msg)
  | Some (false, msg) ->
    if not (contains "missing_sandbox_clone:" msg)
    then fail (Printf.sprintf "expected missing_sandbox_clone in: %s" msg);
    if not (contains "keeper_shell op=git_clone" msg)
    then fail (Printf.sprintf "expected keeper_shell git_clone hint in: %s" msg)
;;
...
let test_dispatch_worktree_create_auto_provisions_workspace_repo () =
  ...
  run_ok ~cwd:base_path "git init -q -b main";
  ignore
    (setup_nested_repo_with_remote ~base_path ~repo_rel:"workspace/yousleepwhen/masc-mcp");
  let config = Masc_mcp.Coord.default_config base_path in
  ignore (Masc_mcp.Coord.init config ~agent_name:(Some "test-agent"));
  let ctx : Tool_worktree.context = { config; agent_name = "test-agent" } in
  ...
  | Some (true, msg) ->
    check bool "message mentions auto-provision" true (contains "auto-provisioned" msg);
...
let test_dispatch_worktree_create_auto_provisions_after_file_storm () =
  ...
  | Some (false, msg) ->
    fail
      (Printf.sprintf
         "expected auto-provision success after file storm, got error: %s"
         msg)
  | Some (true, msg) ->
    check bool "message mentions auto-provision" true (contains "auto-provisioned" msg)
;;
...
let test_dispatch_worktree_create_and_remove_use_docker_keeper_lane () =
  ...
  let docker_worktree =
    Filename.concat
      docker_clone
      (Filename.concat ".worktrees" (Playground_paths.worktree_dir_name "sangsu" task_id))
  in
  let legacy_worktree =
    Filename.concat
      base_path
      (Filename.concat
         ".masc/playground/sangsu/repos/masc-mcp/.worktrees"
  ...
  | Some (false, msg) ->
    fail
      (Printf.sprintf "expected docker keeper auto-provision success, got error: %s" msg)
  | Some (true, msg) ->
    check bool "message mentions auto-provision" true (contains "auto-provisioned" msg);
    check bool "docker sandbox clone created" true (Sys.file_exists docker_clone);
    check bool "docker worktree created" true (Sys.file_exists docker_worktree);
    check bool "legacy worktree untouched" false (Sys.file_exists legacy_worktree);
    ...
    (match Tool_worktree.dispatch ctx ~name:"masc_worktree_remove" ~args:remove_args with
     ...
     | Some (true, _remove_msg) ->
       check bool "docker worktree removed" false (Sys.file_exists docker_worktree))
;;
...
let test_dispatch_worktree_create_repairs_existing_sandbox_clone_checkout () =
  ...
  in
  write_keeper_toml ~base_path ~name:"sangsu" ~sandbox_profile:"docker";
  let sandbox_repos = Filename.concat base_path ".masc/playground/docker/sangsu/repos" in
  ...
  | Some (true, msg) ->
    check
      bool
      "message mentions checkout restore"
      true
      (contains "restored from HEAD" msg);
    check bool "sandbox clone checkout restored" true (Sys.file_exists restored_readme);
    check
      bool
      "docker worktree created after repair"
      true
...
let test_dispatch_worktree_create_cleans_failed_auto_clone () =
  ...
  init_process_eio env;
  run_ok ~cwd:base_path "git init -q -b main";
  let broken_source = Filename.concat base_path "workspace/yousleepwhen/masc-mcp" in
  ...
  | None -> fail "dispatch returned None for masc_worktree_create"
  | Some (true, msg) ->
    fail (Printf.sprintf "expected auto-provision clone failure, got success: %s" msg)
  | Some (false, msg) ->
    check_contains
      "message mentions auto-provision failure"
      "auto_provision_clone_failed"
      msg;
    check bool "partial sandbox clone was cleaned" false (Sys.file_exists sandbox_clone)
;;
...
let test_dispatch_worktree_create_rejects_invalid_sandbox_clone () =
  ...
  | None -> fail "dispatch returned None for masc_worktree_create"
  | Some (true, msg) ->
    fail (Printf.sprintf "expected invalid sandbox clone failure, got success: %s" msg)
  | Some (false, msg) ->
    check_contains "message mentions invalid sandbox clone" "sandbox_clone_invalid" msg;
    check bool "invalid sandbox clone was preserved" true (Sys.file_exists sandbox_clone)
;;
...
let test_worktree_path_rejects_traversal_name () =
  ...
  | Ok (worktree_path, _) -> fail ("expected invalid worktree path, got: " ^ worktree_path)
  | Error (Types.IoError msg) ->
    check_contains "message mentions invalid worktree path" "Invalid worktree path" msg
  | Error err -> fail ("expected IoError, got: " ^ Types.masc_error_to_string err)
;;
...
let test_worktree_create_rejects_existing_non_git_worktree_path () =
  ...
  let source_repo =
    setup_nested_repo_with_remote ~base_path ~repo_rel:"workspace/yousleepwhen/masc-mcp"
  in
  let sandbox_repos = Filename.concat base_path ".masc/playground/test-agent/repos" in
  ensure_dir sandbox_repos;
  let sandbox_clone = Filename.concat sandbox_repos "masc-mcp" in
  run_ok
    ~cwd:base_path
  ...
  | Some (true, msg) ->
    fail
      (Printf.sprintf
         ...
         msg)
  | Some (false, msg) ->
    check_contains "message mentions worktree path conflict" "worktree_path_conflict" msg;
    check
      bool
      "plain directory conflict was preserved"
      true
...
let test_worktree_create_concurrent_same_name_converges () =
  ...
  let source_repo =
    setup_nested_repo_with_remote ~base_path ~repo_rel:"workspace/yousleepwhen/masc-mcp"
  in
  let sandbox_repos = Filename.concat base_path ".masc/playground/race-agent/repos" in
  ensure_dir sandbox_repos;
  let sandbox_clone = Filename.concat sandbox_repos "masc-mcp" in
  run_ok
    ~cwd:base_path
  ...
  let create_once () =
    Masc_mcp.Coord.worktree_create_r
      ~link_task:false
      ~repo_name:"masc-mcp"
      config
  ...
  let unwrap label = function
    | Some (Ok msg) -> msg
    | Some (Error err) ->
      fail (Printf.sprintf "%s failed: %s" label (Types.masc_error_to_string err))
  ...
  in
  check bool "concurrent worktree exists" true (Sys.file_exists worktree_path);
  check int "one create wins" 1 (count_matching "Worktree created");
  check int "one create observes existing" 1 (count_matching "Worktree already exists")
;;
