...
let receipt_duration_ms receipt =
  match
    Types.parse_iso8601_opt receipt.started_at, Types.parse_iso8601_opt receipt.ended_at
  with
...
let operator_disposition (receipt : t) =
  ...
  let tool_contract_result = String.lowercase_ascii receipt.tool_contract_result in
  let error_kind = Option.map String.lowercase_ascii receipt.error_kind in
  if
  ...
  then
    "pass_next_model", "cascade_fallback"
  ...
  else if String.equal receipt.outcome "ok" && String.equal cascade_outcome "completed"
  then "pass", "healthy"
  else "unknown", "unmapped_cascade_state"
;;

let to_json (receipt : t) =
  let operator_disposition, operator_disposition_reason = operator_disposition receipt in
  let error_json =
...
let emit_stale_keeper_broadcast
  ...
  let payload =
    `Assoc
      ...
      ; "disposition_reason", `String (Printf.sprintf "stale_turn_%.0fs" stale_seconds)
