...
let ok_output content : Masc_agent_core.Types.tool_result =
  Ok { Masc_agent_core.Types.content; _meta = None }

let err_output message : Masc_agent_core.Types.tool_result =
  Error { Masc_agent_core.Types.message; recoverable = true; error_class = None }
...
let test_injector_returns_some_on_success () =
  ...
  | Some inj ->
    check bool "has context_updates" true
      (List.length inj.Masc_agent_core.Hooks.context_updates > 0);
  ...
  | Some inj ->
    let last_outcome =
      List.assoc MCI.key_last_tool_outcome
        inj.Masc_agent_core.Hooks.context_updates
    in
...
let test_injector_increments_counts () =
  ...
  | Some inj ->
    let updates = inj.Masc_agent_core.Hooks.context_updates in
...
let test_context_populated_after_injection () =
  let config = MCI.default_config () in
  let injector = MCI.make ~config () in
  let ctx = Masc_agent_core.Context.create_sync () in
  match injector ~tool_name:"bash" ~input:`Null ~output:(ok_output "done") with
  | Some inj ->
    List.iter (fun (k, v) -> Masc_agent_core.Context.set ctx k v)
      inj.Masc_agent_core.Hooks.context_updates;
    (match Masc_agent_core.Context.get ctx MCI.key_last_tool_name with
     | Some (`String "bash") -> ()
     | _ -> fail "last_tool_name not set");
    (match Masc_agent_core.Context.get ctx MCI.key_wall_time with
     | Some (`String s) ->
...
let test_context_updates_overwrite_bounded_keys () =
  ...
  for idx = 1 to 200 do
    ...
    | Some inj ->
      List.iter
        (fun (key, value) -> Masc_agent_core.Context.set ctx key value)
        inj.Masc_agent_core.Hooks.context_updates
    | None -> fail "expected Some injection"
  done;
  check
    ...
    expected_keys
    (Masc_agent_core.Context.keys ctx |> List.sort String.compare);
  (match Masc_agent_core.Context.get ctx MCI.key_tool_call_count with
   | Some (`Int 200) -> ()
   | other ->
     fail
       (Printf.sprintf
...
let test_render_temporal_summary_populated () =
  ...
  Masc_agent_core.Context.set ctx
