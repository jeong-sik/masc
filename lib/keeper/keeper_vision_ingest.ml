...
let evict_block ~mode ~keeper_name ~eager_budget (block : Masc_agent_core.Types.content_block) =
  match block with
...
let evict_message ~mode ~policy ~keeper_name (message : Masc_agent_core.Types.message) =
  if delegating policy
  then
    { message with
      Masc_agent_core.Types.content =
        evict_blocks ~mode ~policy ~keeper_name message.Masc_agent_core.Types.content
    }
  else message
;;
