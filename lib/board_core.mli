...
(** [with_lock store f] runs [f ()] under
    ...
    {!create_post} for instance does its [Economy.earn]
...
(** Resolves the board base path from {!Env_config}.  Used by
    ...
    [Economy.earn] integration in {!create_post}. *)
...
(** Creates a new post.  Validates [author] via
    ...
    JSONL append and the [Economy.earn] credit are intentionally
...
(** Validates [post_id] / [author] / optional [parent_id]
    ...
    [Economy.earn] credits outside the lock (same
