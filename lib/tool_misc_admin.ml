...
let tool_inventory_json _ctx ~include_hidden ~include_deprecated =
  ...
  let add_surface name s =
    let prev =
      match Hashtbl.find_opt surface_map name with
      | Some l -> l
      | None -> []
    in
  ...
  |> List.iter (fun (schema : Types.tool_schema) ->
    if Tool_catalog.is_public_mcp schema.name then add_surface schema.name "public_mcp");
  List.iter
    ...
    |> List.map (fun (schema : Types.tool_schema) ->
      ...
      `Assoc
        ([ "name", `String schema.name
         ...
         ; ( "prompt_hints"
           , `List (List.map (fun value -> `String value) help_entry.prompt_hints) )
         ; ( "surfaces"
           , `List
...
let enforcement_summary_json () =
  `List
    [ `Assoc
        ...
        ; ( "reason"
          , `String "Auth permission checks are enforced only when room auth is enabled."
          )
        ]
    ; `Assoc
        ...
        ; ( "reason"
          , `String
              "Hidden tools are removed from default discovery and may be direct-call \
               blocked." )
        ]
    ; `Assoc
        [ "surface", `String "keeper.eval_gate.allowed_tools"
        ; "status", `String "enforced"
        ; "reason", `String "Keeper uses Eval_gate allow/deny lists at tool-call time."
        ]
    ; `Assoc
        ...
        ; ( "reason"
          , `String
              "Command-plane assignment blocks operations targeting units with \
               kill-switch enabled." )
        ]
    ; `Assoc
        ...
        ; "status", `String "enforced"
        ; ( "reason"
          , `String "Command-plane assignment blocks operations targeting frozen units." )
        ]
    ; `Assoc
        ...
        ; ( "reason"
          , `String
              "Stored in CPv2 topology/policy JSON but not wired into runtime tool \
               dispatch on main." )
        ]
    ; `Assoc
        ...
        ; ( "reason"
          , `String
              "Stored in CPv2 topology/policy JSON but not wired into runtime model \
               selection on main." )
        ]
    ]
;;
...
let handle_feature_flags args : tool_result =
  ...
  let flags =
    if only_overridden
    then Feature_flag_registry.overridden_flags ()
  ...
  let flags =
    ...
    | None -> flags
    | Some cat ->
      List.filter (fun (f : Feature_flag_registry.flag) -> f.category = cat) flags
  in
  ...
  let json =
    `Assoc
      ...
      ; ( "deprecated_tool_names"
        , `List (List.map (fun (name, _) -> `String name) deprecated_tools) )
      ]
  in
...
let handle_tool_admin_snapshot ctx args =
  ...
  let payload =
    `Assoc
      ...
      ; "tool_inventory", tool_inventory_json ctx ~include_hidden ~include_deprecated
      ]
  in
...
let handle_tool_admin_update ctx args =
  let section = get_string args "section" "" |> String.trim |> String.lowercase_ascii in
  match section with
  | "auth" ->
    let current = Auth.load_auth_config ctx.config.base_path in
    if U.member "default_role" args <> `Null
    then false, "❌ default_role is no longer supported"
    else (
      let require_token =
