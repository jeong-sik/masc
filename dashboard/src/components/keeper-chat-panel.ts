    ...
    <div class="overflow-hidden rounded-[var(--radius-xl)] border border-[var(--card-border)] bg-[linear-gradient(180deg,rgba(11,18,34,0.95),rgba(6,11,22,0.92))] shadow-[0_24px_56px_rgba(0,0,0,0.24)]" role="region" aria-label="키퍼 직접 대화">
      <div class="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--slate-gray-12)] px-4 py-4">
        ...
        <div class="flex items-center gap-2">
          ...
          />
          <span class="inline-flex items-center rounded-sm border border-[var(--accent-20)] bg-[var(--accent-10)] px-2.5 py-1 text-2xs font-medium text-[var(--text-strong)]" role="status">
      ...
      <div class="px-4 py-4">
        ${chatAccess.message
          ? html`<div class="mb-4 rounded-card border border-[var(--warn-20)] bg-[var(--warn-10)] px-3 py-2.5 text-xs leading-loose text-[var(--warn-bright)]" role="alert">${chatAccess.message}</div>`
          : null}
        <${ChatTranscript}
