...
let principal_kind_to_yojson kind = `String (principal_kind_to_string kind)
...
let principal_kind_of_yojson = function
  ...
  | json -> Error ("principal_kind_of_yojson: " ^ Yojson.Safe.to_string json)
;;
     ...
     | Ok kind ->
       (match member "id" json with
        | `String id when String.trim id <> "" ->
          Ok { kind; id; display_name = member "display_name" json |> to_string_option }
        | `String _ -> Error "goal_principal_of_yojson: id must be non-empty"
        | other ->
          Error ("goal_principal_of_yojson: invalid id " ^ Yojson.Safe.to_string other)))
  | json -> Error ("goal_principal_of_yojson: " ^ Yojson.Safe.to_string json)
;;
...
let inherit_mode_to_yojson mode = `String (inherit_mode_to_string mode)
...
let inherit_mode_of_yojson = function
  ...
  | json -> Error ("inherit_mode_of_yojson: " ^ Yojson.Safe.to_string json)
;;
        ...
        | `List values ->
          ...
          (match collect [] values with
           ...
           | Ok principals ->
             ...
             in
             Result.map
               (fun required_verdicts -> { inherit_mode; principals; required_verdicts })
               required_verdicts)
        | other ->
          Error
            ("goal_verifier_policy_of_yojson: invalid principals "
             ^ Yojson.Safe.to_string other)))
  | json -> Error ("goal_verifier_policy_of_yojson: " ^ Yojson.Safe.to_string json)
;;
...
let vote_decision_to_yojson decision = `String (vote_decision_to_string decision)
...
let vote_decision_of_yojson = function
  ...
  | json -> Error ("vote_decision_of_yojson: " ^ Yojson.Safe.to_string json)
;;
...
let request_status_to_yojson status = `String (request_status_to_string status)
...
let request_status_of_yojson = function
  ...
  | json -> Error ("request_status_of_yojson: " ^ Yojson.Safe.to_string json)
;;
...
let policy_snapshot_of_yojson = function
  | `Assoc _ as json ->
    ...
    let parse_principal_list field =
      ...
      | other ->
        Error
          (Printf.sprintf
             "policy_snapshot_of_yojson: invalid %s %s"
             field
             (Yojson.Safe.to_string other))
    in
    (match
       parse_principal_list "principals", parse_principal_list "eligible_principals"
     with
  ...
  | json -> Error ("policy_snapshot_of_yojson: " ^ Yojson.Safe.to_string json)
;;
       ...
       (match member "submitted_at" json with
        | `String submitted_at ->
          let evidence_refs =
            ...
            | `Null -> Ok []
            | `List values ->
              (try Ok (List.map to_string values) with
               | Eio.Cancel.Cancelled _ as e -> raise e
               | _ -> Error "goal_verification_vote_of_yojson: invalid evidence_refs")
            | other ->
              Error
        ...
        | other ->
          Error
            ("goal_verification_vote_of_yojson: invalid submitted_at "
             ^ Yojson.Safe.to_string other))
     | Error msg, _ | _, Error msg -> Error msg)
  | json -> Error ("goal_verification_vote_of_yojson: " ^ Yojson.Safe.to_string json)
;;
...
let goal_verification_request_to_yojson (request : goal_verification_request) =
  `Assoc
    ...
    ; ( "resolved_at"
      , match request.resolved_at with
        | Some value -> `String value
        | None -> `Null )
    ]
;;
       ...
       (match member "id" json, member "goal_id" json, member "created_at" json with
        | `String id, `String goal_id, `String created_at ->
          ...
          Result.map
            ...
            votes
        | _ -> Error "goal_verification_request_of_yojson: invalid id/goal_id/created_at")
     | Error msg, _, _, _ | _, Error msg, _, _ | _, _, Error msg, _ | _, _, _, Error msg
       -> Error msg)
  | json -> Error ("goal_verification_request_of_yojson: " ^ Yojson.Safe.to_string json)
;;
...
let state_of_yojson = function
  ...
  | json -> Error ("goal_verification_state_of_yojson: " ^ Yojson.Safe.to_string json)
;;
...
let events_path config = Filename.concat (Coord.masc_dir config) "goal_events.jsonl"
let default_state () = { version = 1; updated_at = Types.now_iso (); requests = [] }
...
let principal_equal left right = String.equal (principal_key left) (principal_key right)
...
let effective_policy_for_nodes ~(goals : goal_policy_node list) ~(goal_id : string) =
...
let exclude_requester
  ...
  in
  if policy_snapshot.required_verdicts > List.length eligible_principals
  then Error "requester exclusion makes goal verification quorum impossible"
  else Ok { policy_snapshot with eligible_principals }
;;
...
let list_requests_for_goal config ~(goal_id : string) =
  ...
  |> fun (state : state) ->
  List.filter
    (fun (request : goal_verification_request) -> String.equal request.goal_id goal_id)
    state.requests
;;
...
let find_request config ~(request_id : string) =
  ...
  |> fun (state : state) ->
  List.find_opt
    (fun (request : goal_verification_request) -> String.equal request.id request_id)
    state.requests
;;

let count_votes ~(decision : vote_decision) request =
  List.length (List.filter (fun vote -> vote.decision = decision) request.votes)
;;
...
let evaluate_quorum request =
  ...
  if approve_count >= request.policy_snapshot.required_verdicts
  then Passed
  else if approve_count + remaining < request.policy_snapshot.required_verdicts
  then Failed
  else Pending
;;
...
let cancel_request config ~(request_id : string) =
  ...
  Coord.with_file_lock config lock_path (fun () ->
    let state = read_state config in
    match
      List.find_opt (fun request -> String.equal request.id request_id) state.requests
    with
    ...
    | Some request ->
      ...
      let updated_request =
        { request with status = Cancelled; resolved_at = Some resolved_at }
      in
      let requests =
        List.map
          (fun row -> if String.equal row.id request_id then updated_request else row)
          state.requests
      in
      write_state
        config
  ...
  Coord.with_file_lock config lock_path (fun () ->
    ...
    | Some request
      when List.exists
             ...
             request.votes -> Error "principal has already voted on this request"
    | Some request ->
      let submitted_at = Types.now_iso () in
      let votes =
        request.votes @ [ { principal; decision; note; evidence_refs; submitted_at } ]
      in
      ...
      let next_request, outcome =
        ...
        | Passed ->
          { next_request with status = Approved; resolved_at = Some submitted_at }, Passed
        | Failed ->
          { next_request with status = Rejected; resolved_at = Some submitted_at }, Failed
      in
      let requests =
        List.map
          (fun row -> if String.equal row.id request_id then next_request else row)
          state.requests
      in
      write_state
        config
