      ...
      ${items.map(item => {
        ...
        return html`
          <div class="tool-bar-row" key=${item.name}>
            ...
            <div class="h-3.5 rounded-xs bg-[var(--white-6)] overflow-hidden" role="progressbar" aria-valuenow=${Math.round(pct)} aria-valuemin=${0} aria-valuemax=${100} aria-label=${`${item.name} 호출 비율`}>
