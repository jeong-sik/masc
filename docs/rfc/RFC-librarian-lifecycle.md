...
1. **턴 끝 기록** `<keepers_dir>/<keeper>.turn-boundaries.jsonl`
   ...
   - `history_empty` 줄(`kind`, 시각, trace id)은 "이 trace 의 저장된 이력에 지금 atom 이 없다"를 말한다. 이름은 쓴 쪽이 본 것이지 쓴 쪽이 누구인지가 아니다. 읽는 쪽에는 그 차이가 필요 없기 때문이다. 쓰는 곳은 둘이다. `keeper_clear` 도구가 checkpoint 의 메시지를 비우고 저장한 직후에 쓰고(`keeper_history_clear.ml`, §8 의 1b), 빈 이력에서 시작하는 턴이 시작할 때 쓴다(§8 의 1c). System 메시지는 atom 이 아니므로 system prompt 를 남기든 말든 비운 이력은 atom 0개다. 그래서 이 줄에는 위치가 없다. 비운 뒤의 턴이 끝에 쓰는 `Fresh_history` 줄도 같은 말을 하지만, 그 턴이 끝을 못 내고 죽으면 그 줄은 없고, 비운 뒤 Keeper 가 한동안 돌지 않으면 그동안 읽는 쪽이 설명 없는 불일치로 서 있게 된다. 그래서 본 자리에서 바로 쓴다.
   ...
   - 저장은 됐는데 줄을 못 쓰면 비우기는 그대로 성공이다. 턴 끝 줄과 같은 원칙이다: Librarian 을 위한 기록이 본래 동작을 실패시키지 않는다. ERROR 로그와 metric 을 남기고 도구 결과의 `history_empty_line_error` 에 이유를 싣는다. 그 뒤의 턴이 끝나면 그 줄이 `Fresh_history` 로 같은 말을 하므로 읽는 쪽은 그때 풀린다. 그 턴이 끝을 못 내면 설명이 남지 않는다(§6). 파일 끝이 잘려 있으면 append 가 전부 거절되므로 다시 비워도 줄은 쓰이지 않는다. 그 파일은 4단계의 기동 복구가 들어오기 전에는 고쳐지지 않는다.
...
2. **진행 파일** `<keepers_dir>/<keeper>.librarian-progress.json`
   ...
   - 이력이 새로 시작하는 일은 줄이 말한다. 새로 만든 Keeper 의 첫 턴, checkpoint 를 못 읽었거나 버전이 바뀐 뒤의 턴(`keeper_context_core.ml` 의 `load_context_from_checkpoint` 가 `None` 을 돌려주는 모든 경우), purge 로 지워진 뒤의 턴은 빈 이력에서 시작하므로 그 줄이 `Fresh_history` 다. `keeper_clear` 는 `history_empty` 줄을 남긴다. 그래서 루프 말고는 누구도 이 파일을 고쳐 쓰지 않는다. 줄을 남기지 못하는 경우가 둘 알려져 있다(§6).
...
- 빈 이력에서 시작한 턴이 저장은 하고 끝은 못 낸 경우. 재시작을 말하는 `Fresh_history` 줄은 턴 끝에서만 쓰인다. 그 턴이 턴 도중에 저장한 뒤 모든 후보가 죽어 끝을 못 내면, 다음 턴은 atom 이 있는 이력에서 시작해 `Continued_history` 다. 그 trace 에는 재시작 줄이 끝내 없다. 새 Keeper 라면 첫 구간이 기준점 앞에 놓여 읽히지 않고, 읽은 위치가 있던 Keeper 라면 설명 없는 불일치로 선다. 버전 컷은 Keeper 전체에 한꺼번에 온다. §8 의 1c 가 닫는다. 빈 이력에서 시작하는 턴이 시작할 때 `history_empty` 줄을 쓴다. 턴 도중의 저장, 끝의 저장, 공식 클라이언트 host 가 죽은 턴의 거절 기록을 덧붙이는 저장(`keeper_official_client_host.ml`) 가운데 어느 것이 그 이력에 처음 atom 을 써넣든 줄은 이미 있다.
...
| 1c | 빈 이력에서 시작하는 턴이 시작할 때 `history_empty` 줄을 쓴다. 그 턴이 저장만 하고 끝을 못 내도 재시작이 기록에 남는다(§6). 1b 와 같은 줄 종류다. 뜻이 겹치는 줄 종류를 하나 더 두지 않는다 | 읽는 곳이 없다. 4단계 전에 있어야 한다 |
