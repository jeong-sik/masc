...
let () =
  test "dispatch_dashboard_compact" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "dispatch_dashboard_current_scope" (fun () ->
    let ctx = make_test_ctx () in
    Coord.ensure_room_bootstrap ctx.config;
...
let () =
  test "dispatch_team_memory_write_and_read" (fun () ->
    let ctx = make_test_ctx () in
    let agent_name =
      write_team_memory_keeper_meta
        ~config:ctx.config
        ~name:"room-alpha"
        ~shared_memory_scope:"room"
    in
    let keeper_ctx : Tool_misc.context = { config = ctx.config; agent_name } in
    let write_body =
      match
        Tool_misc.dispatch
          keeper_ctx
    ...
    let read_body =
      match
        Tool_misc.dispatch
          ...
          ~args:
            (`Assoc [ "room", `String "default"; "key", `String "handoff/summary.md" ])
      with
      | Some (true, body) -> body
      | Some (false, err) -> failwith err
      | None -> failwith "dispatch returned None"
...
let () =
  test "dispatch_dashboard_invalid_scope" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "dispatch_cleanup_zombies" (fun () ->
    let ctx = make_test_ctx () in
    let args = `Assoc [] in
...
let () =
  test "dispatch_web_search_requires_query" (fun () ->
    let ctx = make_test_ctx () in
    let args = `Assoc [] in
...
let () =
  test "dispatch_web_search_rejects_long_query" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "dispatch_web_search_rejects_secret_like_query" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "validate_web_search_allows_task_id_with_sk_substring" (fun () ->
    match
      Tool_misc_web_search.validate_query "task-352 destructive check tools hardcoded"
    with
...
let () =
  test "validate_web_search_rejects_sk_prefixed_secret_token" (fun () ->
    match
...
let () =
  test "parse_searxng_json_basic" (fun () ->
    let payload =
      {|{"results": [
...
let () =
  test "parse_searxng_json_empty_results" (fun () ->
    let payload = {|{"results": []}|} in
    assert (Tool_misc.parse_searxng_json payload = []))
;;

let () =
  test "parse_searxng_json_malformed" (fun () ->
    assert (Tool_misc.parse_searxng_json "not json" = []))
;;

let () =
  test "web_search_provider_plan_includes_searxng_when_configured" (fun () ->
...
let () =
  test "web_search_provider_plan_defaults_to_scraping_fallbacks" (fun () ->
...
let () =
  test "web_search_provider_plan_prefers_configured_official_provider" (fun () ->
    with_env "MASC_SEARXNG_URL" None (fun () ->
...
let () =
  test "web_search_simulate_for_test_falls_back_after_error" (fun () ->
    let success, result =
...
let () =
  test "web_search_simulate_for_test_reports_all_failures" (fun () ->
    let success, result =
      Tool_misc.web_search_simulate_for_test
        ~query:"ocaml eio"
        ~limit:3
...
let () =
  test "parse_official_provider_json_payloads" (fun () ->
    let brave =
...
let () =
  test "parse_official_provider_json_payloads_tolerate_malformed_json" (fun () ->
...
let () =
  test "redact_transport_error_detail" (fun () ->
    assert (
    ...
    assert (
      Tool_misc.redact_transport_error_detail "forbidden response" = "forbidden response"))
;;

let () =
  test "dispatch_webrtc_offer" (fun () ->
    let ctx = make_test_ctx () in
    let args =
      `Assoc
...
let () =
  test "dispatch_webrtc_answer" (fun () ->
    let ctx = make_test_ctx () in
    let offer_args =
      `Assoc
        [ "agent_name", `String "offer-agent"
    ...
    in
    let offer_id = Yojson.Safe.Util.(offer_result |> member "offer_id" |> to_string) in
    let answer_args =
      `Assoc
...
let () =
  test "dispatch_webrtc_offer_disabled" (fun () ->
...
let () =
  test "dispatch_webrtc_answer_disabled" (fun () ->
    with_env "MASC_WEBRTC_ENABLED" (Some "0") (fun () ->
      let ctx = make_test_ctx () in
      let args =
        `Assoc
...
let () =
  test "dispatch_tool_admin_snapshot" (fun () ->
    let ctx = make_test_ctx () in
    let args = `Assoc [] in
    ...
    | Some (success, result) ->
      ...
      assert (
        Yojson.Safe.Util.member "http_auth_strict" (Yojson.Safe.Util.member "auth" json)
        <> `Null);
      assert (
        Yojson.Safe.Util.member "bind_host" (Yojson.Safe.Util.member "auth" json) <> `Null);
      assert (
        Yojson.Safe.Util.member "bind_is_loopback" (Yojson.Safe.Util.member "auth" json)
        <> `Null);
...
let () =
  test "dispatch_tool_admin_update_rejects_mode" (fun () ->
    let ctx = make_test_ctx () in
    let args =
      `Assoc
        [ "section", `String "mode"
...
let () =
  test "dispatch_tool_admin_update_auth" (fun () ->
    let ctx = make_test_ctx () in
    let args =
      `Assoc
        [ "section", `String "auth"
...
let () =
  test "dispatch_tool_admin_update_auth_rejects_removed_default_role" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "dispatch_tool_admin_update_keeper_policy" (fun () ->
    Eio_main.run
    @@ fun env ->
    Fs_compat.set_fs (Eio.Stdenv.fs env);
    Eio.Switch.run
    ...
    in
    Fun.protect
      ~finally:(fun () -> Keeper_keepalive.stop_keepalive "admin-keeper")
      (fun () ->
         match
         ...
         | Some (true, _) ->
           ...
           let args =
             `Assoc [ "section", `String "keeper_policy"; "name", `String "admin-keeper" ]
           in
           (match Tool_misc.dispatch ctx ~name:"masc_tool_admin_update" ~args with
