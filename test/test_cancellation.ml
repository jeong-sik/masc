...
let test_tool_cancel () =
  let token = Cancellation.TokenStore.create () in
  let args = `Assoc [ "action", `String "cancel"; "token_id", `String token.id ] in
...
let test_tool_check () =
  let token = Cancellation.TokenStore.create () in
  let args = `Assoc [ "action", `String "check"; "token_id", `String token.id ] in
  let success, result = Cancellation.handle_cancellation_tool args in
...
let json_tests = [ "token_to_json", `Quick, test_token_to_json ]

let () =
  run
    "Cancellation"
    [ "token", token_tests; "store", store_tests; "tool", tool_tests; "json", json_tests ]
;;
