(** Keeper_deliberation — triage logic, MODEL-driven deliberation, and response
    parsing for the keeper deliberation engine.

    Types, serialization, and observation predicates are in
    [Keeper_deliberation_types] (included below).
...
include Keeper_deliberation_types
