...
- **자를 턴이 없는 구간에서는 atom 절반이 "한 턴" 노릇을 한다.** 턴 끝 줄의 위치가 `No_atom_history` 면 그 줄은 자르는 자리로 쓰이지 못한다. 2026-09-22 goo-yang-bong 의 턴 끝 줄 159 개 중 134 개가 그것이고, 위치가 붙은 25 개는 모두 atom 12,719 이상이다. 0~12,719 에는 좁힐 때 자를 자리가 하나도 없다.
  ...
  두 무리는 시각으로 갈린다. `No_atom_history` 는 09-20 13:58Z~09-21 00:09Z 에 몰려 있고 `atom_history` 는 09-21 00:24Z 부터다. 경계 뒤의 `No_atom_history` 는 0 개다. 공식 클라이언트 턴이 `No_atom_history` 를 남기는 것은 §4.8 이 적은 사실이지만, 같은 시기 Agent Core 레인 턴도 위치를 남기지 않았으므로 이 구간의 원인으로 단정하지 않는다. 09-21 00:24Z 에 무엇이 바뀌어 위치가 붙기 시작했는지는 이 RFC 가 밝히지 않았다. 원인이 무엇이든 설계에서 달라지는 것은 없다. 자를 자리가 없는 구간은 남아 있고 좁히려면 다른 축이 필요하다. 그 구간에서만 범위를 반으로 접고 바닥은 1 atom 이다. 자를 자리가 있는 구간의 바닥은 위 규칙 그대로 가장 오래된 한 턴이다. 두 바닥은 다르고 섞지 않는다.
