...
let check_execution_mode (b : Cdal_loader.loaded_bundle) : Cdal_types.check_result =
  ...
  let propagation_ok = Oas.Execution_mode.equal proof_requested contract_mode in
  (* No-upward-escalation: effective <= requested *)
  let escalation_ok =
    Oas.Execution_mode.can_serve ~requested:proof_requested ~effective:proof_effective
  in
  ...
  then { check_id; status = Satisfied; findings = []; completeness_gaps = [] }
    ...
    then
      ...
      := ({ Cdal_types.check_id
          ...
          ; expected =
              `String
                (Printf.sprintf "<= %s" (Oas.Execution_mode.to_string proof_requested))
          ; trace_ref = None
          }
          : Cdal_types.contract_finding)
         :: !findings;
...
let check_risk_class (b : Cdal_loader.loaded_bundle) : Cdal_types.check_result =
  ...
  if Oas.Risk_class.equal contract_risk proof_risk
  then { check_id; status = Satisfied; findings = []; completeness_gaps = [] }
  else
    { check_id
    ; status = Violated
