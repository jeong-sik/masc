...
let test_calc_backoff_capped () =
  let config =
    { Bounded.default_retry_config with jitter_factor = 0.0; max_delay_ms = 5000 }
  in
...
let test_not_retryable_validation () =
  check
    ...
    "validation"
    false
    (Bounded.is_retryable_error "Validation error: invalid input")
;;
...
let test_goal_type () =
  let g : Bounded.goal =
    { path = "$.result.status"; condition = Bounded.Eq (`String "success") }
  in
...
let () =
  run
    ...
    ; "goal", [ test_case "type" `Quick test_goal_type ]
    ]
;;
