...
let build_agent_briefs config sessions attention_queue room_json =
  ...
  let messages = Room.get_messages_raw config ~since_seq:0 ~limit:200 in
  let current_room =
  ...
  |> List.map (fun (agent : Types.agent) ->
         let related_session =
           List.find_opt
             (fun (session : session_context) -> List.mem agent.name session.member_names)
             sessions
         in
         let related_attention_count =
           ...
           |> List.fold_left
                (fun acc attention ->
                  if List.mem agent.name attention.related_agent_names then acc + 1
                  else
                ...
                0
         in
         let latest_out = latest_message_from agent.name messages in
         let latest_in = latest_message_to agent.name messages in
         let where =
           match related_session with
           | Some session ->
               compact_text
         ...
         let with_whom =
           ...
           | Some session ->
               ...
               |> List.filter (fun name -> not (String.equal name agent.name))
           | None -> []
         in
         let current_work =
           task_label agent.current_task
         in
         ...
         in
         ({
           status_rank = status_rank (Types.agent_status_to_string agent.status);
           related_attention_count;
           last_seen_ts =
           ...
           json =
             `Assoc
               [
                 ...
                 ("status", `String (Types.agent_status_to_string agent.status));
                 ("where", `String where);
