...
let inference_default_model =
  Runtime_params.register
    ...
    ~default:(fun () -> "auto")
    ~validate:(fun v ->
      if String.length v > 0 && String.length v <= 100
      then Ok ()
...
let dashboard_max_path_length =
  Runtime_params.register
    ...
    ~deserialize:deserialize_int
    ~meta:
      { description = "대시보드 경로 출력 최대 길이 (문자)"
      ; value_type = "int"
      ; min_value = Some (`Int 10)
      ; max_value = Some (`Int 200)
      }
    ()
;;
...
let dashboard_max_message_length =
  Runtime_params.register
    ...
    ~deserialize:deserialize_int
    ~meta:
      { description = "대시보드 메시지 출력 최대 길이 (문자)"
      ; value_type = "int"
      ; min_value = Some (`Int 10)
      ; max_value = Some (`Int 500)
      }
    ()
;;
...
let dashboard_max_pending_tasks =
  Runtime_params.register
    ...
    ~deserialize:deserialize_int
    ~meta:
      { description = "대시보드 pending task 표시 최대 개수"
      ; value_type = "int"
      ; min_value = Some (`Int 1)
      ; max_value = Some (`Int 50)
      }
    ()
;;
...
let dashboard_max_recent_messages =
  Runtime_params.register
    ...
    ~deserialize:deserialize_int
    ~meta:
      { description = "대시보드 recent message 표시 최대 개수"
      ; value_type = "int"
      ; min_value = Some (`Int 1)
      ; max_value = Some (`Int 50)
      }
...
let dashboard_min_border_length =
  Runtime_params.register
    ...
    ~deserialize:deserialize_int
    ~meta:
      { description = "대시보드 섹션 경계선 최소 길이"
      ; value_type = "int"
      ; min_value = Some (`Int 20)
      ; max_value = Some (`Int 200)
      }
    ()
;;
...
let dashboard_agent_stuck_threshold_sec =
  Runtime_params.register
    ...
    ~validate:
      (validate_float_range
         ~min:60.0
         ~max:(7.0 *. Masc_time_constants.day)
...
let keeper_max_hb_failures =
  Runtime_params.register
    ...
    ~serialize:(fun v -> `Int v)
    ~meta:
      { description = "Heartbeat 연속 실패 허용 횟수"
      ; value_type = "int"
      ; min_value = Some (`Int 2)
      ; max_value = Some (`Int 50)
      }
    ~deserialize:deserialize_int
    ()
...
let keeper_max_turn_failures =
  Runtime_params.register
    ...
    ~serialize:(fun v -> `Int v)
    ~meta:
      { description = "Turn 연속 실패 허용 횟수"
      ; value_type = "int"
      ; min_value = Some (`Int 3)
      ; max_value = Some (`Int 100)
      }
    ~deserialize:deserialize_int
    ()
;;
...
let keeper_supervisor_sweep_sec =
  Runtime_params.register
    ...
    ~serialize:(fun v -> `Float v)
    ~meta:
      { description = "Supervisor sweep 주기(초)"
      ; value_type = "float"
      ; min_value = Some (`Float 10.0)
      ; max_value = Some (`Float 120.0)
      }
    ~deserialize:deserialize_float
    ()
;;
...
let keeper_keepalive_interval_sec =
  Runtime_params.register
    ...
    ~meta:
      ...
      ; value_type = "int"
      ; min_value = Some (`Int 5)
      ; max_value = Some (`Int 300)
      }
    ~deserialize:deserialize_int
    ()
;;
...
let keeper_dead_ttl_sec =
  Runtime_params.register
    ...
    ~validate:
      (validate_float_range ~min:60.0 ~max:Masc_time_constants.day "keeper_dead_ttl_sec")
    ~serialize:(fun v -> `Float v)
    ~meta:
      { description = "Dead 상태 유지 시간(초)"
      ; value_type = "float"
      ; min_value = Some (`Float 60.0)
      ; max_value = Some (`Float Masc_time_constants.day)
      }
...
let keeper_handoff_threshold =
  Runtime_params.register
    ...
    ~serialize:(fun v -> `Float v)
    ~meta:
      { description = "Handoff context ratio 임계값"
      ; value_type = "float"
      ; min_value = Some (`Float 0.5)
      ; max_value = Some (`Float 0.99)
      }
    ~deserialize:deserialize_float
    ()
;;
...
let keeper_handoff_cooldown_sec =
  Runtime_params.register
    ...
    ~serialize:(fun v -> `Int v)
    ~meta:
      { description = "Handoff 쿨다운(초)"
      ; value_type = "int"
      ; min_value = Some (`Int 30)
      ; max_value = Some (`Int 3600)
      }
    ~deserialize:deserialize_int
    ()
;;
...
let keeper_handoff_pressure_threshold =
  Runtime_params.register
    ...
    ~default:(fun () -> 0.88)
    ~validate:
      (validate_float_range ~min:0.5 ~max:0.99 "keeper.handoff_pressure_threshold")
    ~serialize:(fun v -> `Float v)
    ~meta:
      { description = "Handoff pressure 알림 임계값"
      ; value_type = "float"
      ; min_value = Some (`Float 0.5)
      ; max_value = Some (`Float 0.99)
      }
...
let relay_tokens_per_assistant_msg =
  Runtime_params.register
    ~key:"relay.tokens_per_assistant_msg"
    ~default:(fun () -> 500)
...
let relay_tokens_per_tool_call =
  Runtime_params.register
    ~key:"relay.tokens_per_tool_call"
    ~default:(fun () -> 200)
...
let relay_tokens_per_tool_result =
  Runtime_params.register
    ~key:"relay.tokens_per_tool_result"
    ~default:(fun () -> 300)
...
let relay_cost_large_file_read =
  Runtime_params.register
    ~key:"relay.cost_large_file_read"
    ~default:(fun () -> 10_000)
...
let keeper_work_as_hb_enabled =
  Runtime_params.register
    ...
    ~meta:
      { description = "Work-as-heartbeat 활성화 여부"
      ; value_type = "bool"
      ; min_value = None
      ; max_value = None
      }
...
let keeper_work_as_hb_max_silence_sec =
  Runtime_params.register
    ...
    ~validate:
      (validate_float_range ~min:10.0 ~max:600.0 "keeper_work_as_hb_max_silence_sec")
    ~serialize:(fun v -> `Float v)
    ~meta:
      { description = "Work-as-heartbeat 최대 침묵 시간(초)"
      ; value_type = "float"
      ; min_value = Some (`Float 10.0)
      ; max_value = Some (`Float 600.0)
      }
    ~deserialize:deserialize_float
    ()
;;
...
let keeper_smart_hb_enabled =
  Runtime_params.register
    ...
    ~serialize:(fun v -> `Bool v)
    ~meta:
      { description = "Smart heartbeat 적응형 스케줄링 활성화"
      ; value_type = "bool"
      ; min_value = None
      ; max_value = None
      }
...
let keeper_stage_timing_ring_size =
  Runtime_params.register
    ...
    ~serialize:(fun v -> `Int v)
    ~meta:
      { description = "Stage timing ring buffer 크기 (fiber restart 시 적용)"
      ; value_type = "int"
      ; min_value = Some (`Int 10)
      ; max_value = Some (`Int 1000)
      }
    ~deserialize:deserialize_int
    ()
;;
...
let drift_factual_coverage_floor =
  Runtime_params.register
    ...
    ~deserialize:deserialize_float
    ~meta:
      { description = "drift_guard factual drift 판정 — 토큰 커버리지 하한"
      ; value_type = "float"
      ; min_value = Some (`Float 0.0)
      ; max_value = Some (`Float 1.0)
      }
    ()
;;
...
let drift_factual_size_ratio_floor =
  Runtime_params.register
    ...
    ~deserialize:deserialize_float
    ~meta:
      { description = "drift_guard factual drift 판정 — 크기 비율 하한"
      ; value_type = "float"
      ; min_value = Some (`Float 0.0)
      ; max_value = Some (`Float 1.0)
      }
...
let drift_structural_divergence_threshold =
  Runtime_params.register
    ...
    ~default:(fun () -> 0.18)
    ~validate:
      (validate_float_range ~min:0.0 ~max:1.0 "drift_structural_divergence_threshold")
    ~serialize:(fun v -> `Float v)
    ~deserialize:deserialize_float
    ~meta:
      { description = "drift_guard structural drift 판정 — cosine-jaccard 발산 임계치"
      ; value_type = "float"
      ; min_value = Some (`Float 0.0)
      ; max_value = Some (`Float 1.0)
      }
...
let surfaces =
  ...
  ; { id = "inference_config"
    ...
    ; risk = "high"
    ; param_keys = [ "inference.default_model"; "inference.timeout_seconds" ]
    }
  ; { id = "cost_policy"
  ...
  ; { id = "relay_heuristic"
    ...
    ; risk = "low"
    ; param_keys =
        [ "relay.tokens_per_user_msg"
        ; "relay.tokens_per_assistant_msg"
        ; "relay.tokens_per_tool_call"
        ; "relay.tokens_per_tool_result"
        ; "relay.cost_large_file_read"
        ; "relay.cost_per_file_edit"
        ; "relay.cost_long_running"
        ; "relay.cost_exploration"
        ; "relay.cost_simple"
        ]
    }
  ; { id = "drift_guard"
    ; description =
        "Handoff drift classification thresholds (initial estimates, not \
         corpus-calibrated)"
    ; risk = "low"
    ; param_keys =
  ...
  ; { id = "dashboard"
    ; description =
        "Dashboard rendering — truncation lengths, row limits, borders, status thresholds"
    ; risk = "low"
    ; param_keys =
