...
- A keeper turn that succeeded without collecting a provider response in this
  process is no longer recorded as `keeper cycle FAILED` with
  `Internal error: successful Agent.run returned without an AfterTurn ordinal`.
  A durable resume that replays an already-settled turn, a pre-first-token
  preemption (RFC-0441) and a first turn that stops at `InputRequired` before
  the provider call all end Ok with no `AfterTurn`; the keeper result now
  carries `final_agent_core_turn_ordinal : int option`, the decision record
  writes `null`, and no resolved cost row is written for a run that made no
  provider call of its own. Live logs showed 16 such false failures across 8
  keepers in 8 days (#38066).
