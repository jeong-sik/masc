...
let test_max_length_enforced () =
  ...
  Alcotest.(check bool) "within limit" true (String.length preview <= 220)
;;
...
let test_denied_tool_input_returns_none () =
  let result =
    Observability_redact.redact_tool_input
      ~tool_name:"tool_auth_create"
      (`String "secret data")
  in
...
let test_denied_tool_output_returns_none () =
  let result =
    Observability_redact.redact_tool_output ~tool_name:"keeper_encryption_key" "secret"
  in
  Alcotest.(check (option string)) "denied tool" None result
;;
...
let test_normal_tool_input_returns_some () =
  ...
  in
  Alcotest.(check bool) "normal tool returns Some" true (Option.is_some result)
;;

let test_normal_tool_output_returns_some () =
  let result =
    Observability_redact.redact_tool_output ~tool_name:"masc_status" "room is active"
  in
  Alcotest.(check bool) "normal tool returns Some" true (Option.is_some result)
;;
...
let test_blob_sentinel_preserves_structure () =
  ...
  | Tool_output.Inline _ -> Alcotest.fail "marker structure destroyed by redaction"
;;
...
let test_blob_sentinel_redacts_preview_body () =
  ...
  let redacted = Observability_redact.redact_preview marker in
  Alcotest.(check bool)
    "raw key scrubbed from preview"
    true
    (not (Observability_redact.contains_substring ~sub:"abc123xyz456" redacted));
  Alcotest.(check bool)
    "sha256 still present as structural field"
    true
    ...
    (match List.assoc_opt "content" fields with
     | Some (`String s) ->
       (match Tool_output.decode_from_oas s with
        ...
        | Tool_output.Inline _ -> Alcotest.fail "sentinel in JSON leaf was corrupted")
  ...
  Alcotest.run
    ...
    [ ( "redaction"
      , [ Alcotest.test_case "API key redacted" `Quick test_api_key_redacted
        ...
        ; Alcotest.test_case
            "blob sentinel preserves structure"
            `Quick
            test_blob_sentinel_preserves_structure
        ; Alcotest.test_case
            "blob sentinel redacts preview body"
            `Quick
            test_blob_sentinel_redacts_preview_body
        ; Alcotest.test_case
            "preview_json_strings preserves embedded sentinel"
            `Quick
            test_preview_json_strings_preserves_embedded_sentinel
        ] )
    ; ( "deny_list"
      , [ Alcotest.test_case
            "denied tool input -> None"
            `Quick
            test_denied_tool_input_returns_none
        ; Alcotest.test_case
            "denied tool output -> None"
            `Quick
            test_denied_tool_output_returns_none
        ; Alcotest.test_case
            "normal tool input -> Some"
            `Quick
            test_normal_tool_input_returns_some
        ; Alcotest.test_case
            "normal tool output -> Some"
            `Quick
            test_normal_tool_output_returns_some
        ] )
    ]
;;
