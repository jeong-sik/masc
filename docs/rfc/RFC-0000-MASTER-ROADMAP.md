...
- OCaml 하한은 실제로 **MASC** dune-project 기준 **≥5.5** (CI가 5.5에서 authoritative). "5.4 원칙"은 lower-bound 참조일 뿐. (현재 상태: OAS 쪽 하한은 `agent_sdk.opam` `>= "5.4"`다 — OAS도 5.5라는 기대는 아직 현재 상태가 아님. 2026-07-20 확인.)
...
| **KILL** | goal-loop OODA 기계(scheduler/status/broadcast/dashboard 표면·legacy-status decode — Goal 엔티티/`goal_phase` FSM/MCP tool/task linkage는 KEEP, §3.2·RFC-0352), generic governance hierarchy, `Context_reducer`, implicit sampling/timeout/turn/token defaults, string/score/count가 semantic·lifecycle·effect 결정을 대신하는 heuristic, stub/synthetic ToolResult, migration/backfill/dual-read, producer·consumer 없는 타입 |
...
### 0.3 Fact-check snapshot (2026-07-20, HEAD `833b38d302`)

§0.2 이후 drift를 fresh grep으로 재확인해 `현재 상태` 기술만 정정했다(masc 워크트리 `833b38d302`, oas pin `9fef71dedf`). 방향(direction) 텍스트는 바꾸지 않았고, 규범과 현재 상태가 충돌한 1건(§3.2 Goal)은 §11 **D10**으로 올렸고, 2026-07-21 오너 결정(RFC-0352 Path B)으로 해소되어 §3.2 카드가 재작성됐다. 정정 항목:

- **OAS pin** — 0.215.0 @ `a7ea83fbbf` → **0.217.1** @ `9fef71dedf` (`scripts/oas-agent-sdk-pin.sh:4,28`, `masc.opam.locked:14,220-221`; `bash scripts/check-oas-pin.sh` 통과, 단 upstream main은 `658a910911`까지 전진 — §2.6).
- **§3.1 Board** — "`Hashtbl.replace`는 comments/reactions/sub_boards 갱신용"은 stale: edit 경로 `update_post_with_outcome`(`board_core_persist.ml:662-748`, `:739`)이 body/content를 파괴적 교체. keyword classifier 부재·`Board_render` 미구현은 재확인(유지).
- **§3.5 Scheduler** — `Reminder_only` clamp는 #24332에서 제거(HEAD grep 0); SSOT kind는 `masc.keeper_wake`뿐(`schedule_supported_kinds.ml:18`)이고 `masc.board_post`는 reject(`test_schedule_consumer_dispatch.ml:263`); Goal 7 occurrence identity는 `[LANDED]`(`schedule_occurrence_id.mli`, `test_schedule_runner.ml:394`).
- **§3.6 Connector** — speaker identity durable persist(`keeper_chat_store.ml:175-179,414-419`), busy 시 typed ack 발송(`gate_keeper_backend.ml:140,162`), Slack도 `Connector_attention` wake(`server_slack_in_process_gateway.ml:543`) — 옛 "소멸/silent drop/Discord-only"는 전부 stale.
- **§3.8 Keeper** — "Pause 3중 위반"(restart budget→Dead, auto-compact retry→Paused, overflow-pause shadow) 각 grep 0; supervisor는 budget 없이 재시작(`keeper_supervisor.ml:99-102`).
- **§3.9 Memory** — 당시 memory-os GC와 consolidation이 모두 default ON이었다. 이 snapshot 이후 periodic consolidation은 hard-cut 삭제되었고 현재 계약은 §3.9의 2026-07-29 갱신을 따른다. `chronicle_librarian.ml`은 #25213에서 삭제. episode retention은 미구현 — RFC-0272(Draft) + issue #25065 추적.
- **§3.10 Runtime** — pinned OAS pricing은 fail-closed(`pricing.mli:1-5`, unknown model → `None`, inline test `pricing.ml:217`); failover 회귀는 17 case.
- **§3.13 Keeper-as-a-Tool** — `masc_keeper_delegate`/`_delegate_status`/`_cancel`/`_list` first-class surface 존재(`keeper_schema.ml:322-348`, `tool_catalog.ml`, `keeper_invocation_contract.ml/mli`).
- **§0 OCaml 하한** — ≥5.5는 MASC dune-project 기준(`dune-project:28`); OAS `agent_sdk.opam` 하한은 `>= "5.4"`.

각 인용 수치는 확인 시점 snapshot이며 정본이 아니다(§0 규칙). 명령·출력 원문은 이 PR 설명을 참조한다.
...
**MASC**는 다수의 **자율·영속 Keeper**를 조율하는 **멀티-repo coordination server**다. 서버 프로세스는 하나의 `base_path`(`.masc` 런타임/설정 루트)에 앵커되지만, 조율 대상은 `repositories.toml`에 등록된 **여러 독립 repo**다(`repo_store`: 각 repo는 `url` + 등록 `local_path`, `.masc` 밖 절대경로 가능). MASC가 각 Keeper에 제공하는 substrate는 **격리 sandbox**(`sandbox_profile = Local | Docker`)다 — tool 실행은 이 sandbox 안에서 일어난다.

*현재 구현*은 per-keeper **playground**를 `<base_path>/.masc/playground/<keeper>/`(scratch `mind/` + git clone `repos/<repo>/`)로 두고 sandbox에 mount한다. `repos/<repo>`는 registry의 **URL**을 `Repo_git.clone`으로 per-keeper 경로에 clone한 **writable checkout**이며(registry 등록 `local_path`와는 다른 경로 — `playground_repo_readiness.ml`의 `clone_sandbox_repo`/`ensure_ready`가 clone·자동복구), keeper는 이 clone의 파일을 **직접 편집(write)**한다(`parse_playground_repo_path`가 편집 파일을 canonical-URL 버킷으로 귀속 → cross-keeper "같은 논리 repo" 인지). `keeper_sandbox_control.ml:433`의 "playground_repos는 관측이지 access verdict 아님"은 **live sandbox projection의 `playground_repos` 필드 성격**(디스크에 보이는 목록일 뿐 접근 허가/거부 판정 아님)을 말하는 것이지 repo가 read-only라는 뜻이 **아니다**. **이 playground 레이아웃(`mind`/`repos` 분리, Docker의 `docker/` 경로 세그먼트)이 필수인지 bare sandbox + env(creds/endpoint/repo-URL)로 단순화할지는 미결 — §11 D8.** 본질 thesis는 "격리 sandbox + 자율 keeper 조율"이지 특정 디렉토리 레이아웃이 아니다.

*server-local ≠ work-repo-local* — 서버가 한 checkout에서 도는 것과 조율 대상이 한 repo에 갇히는 것은 별개이며, MASC는 전자이되 후자가 아니다. 각 Keeper는 자기 Keeper로 공유 MASC world에 상주하는 자율 에이전트다. Keeper는 `MASC_CONFIG_DIR/<name>/AGENT.md`의 전체 개인 프롬프트이며, `masc_keeper_up`가 이를 instructions로 사용한다. Keeper의 일은 repo 코딩일 수도, Board 토론·투표·Discord/Slack 대화·Voice·역할극일 수도 있다(repo 바인딩 없는 Keeper도 1급; keeper↔repo 매핑은 `policy_mode=advisory` 기본값이지 authorization 경계가 아님). *[설계 배경: RFC-0213/0324/0338 — 모두 Draft, 근거는 위 코드]*
...
**OAS**는 순수 단일 에이전트 라이브러리다. 소유하는 실제 primitive는 provider 라우팅(4-variant sum은 `Provider.provider` = `Local|Anthropic|OpenAICompat|Custom_registered`; `Provider.config`는 이를 담는 record `{provider; model_id; api_key_env}` + `Backend_*`), checkpoint(`Checkpoint.t` v8 순수 값 + `Checkpoint_store`), tool-boundary cooperative yield(`Agent.Advanced.run_blocks`, `Continue|Yield`), reasoning replay(`Reasoning_replay_contract`/`Reasoning_history_projection`), wire observer(`Wire_observer`), stop reason(`Llm_provider.Types.stop_reason` 12-variant + `Stop_reason_wire`), sub-agent 위임(`Handoff.make_handoff_tool`), 6-stage turn pipeline(`Pipeline`)이다. OAS는 masc/keeper/board/keeper/hitl/fusion 등 downstream coordinator의 도메인 어휘를 전혀 모른다(grep 실측 각 0건, §2.2).
...
- MASC/keeper/board/keeper/hitl 도메인 개념을 public contract에 encode.
...
- OAS `lib/`에서 coordinator 어휘 `masc|keeper|keeper|hitl|fusion` = **0건**. (`board`는 `event_bus.mli` doc 주석의 `dashboards` substring 1건 외 실 어휘 0 — 경계 자체는 유지.)
...
- **불변식:** post body는 **비파괴적 append**. (방향 계약. 원 감사가 지목한 "50개 하드코딩 영어 키워드 substantive-vs-noise 판정" classifier는 HEAD `lib/board/`에서 **부재** 유지(`rg 'substantive' lib/board/` 0건, 2026-07-20). 단 body 파괴적 덮어쓰기 자체는 **실재**: edit 경로 `update_post_with_outcome`(`board_core_persist.ml:662-748`)이 `:739`에서 `Hashtbl.replace store.posts key updated`로 body/content를 새 값으로 교체 — 이전 "Hashtbl.replace는 comments/reactions/sub_boards 갱신용" 기술은 stale. 즉 비파괴적 append 불변식은 edit 경로에서 현재 미충족이며, 방향 계약은 유지한다.)
...
- **현재 상태:** rich-text/multimedia meta(attachment 4종)는 **미구현**. 이전 기록은 `Board_render` projection 모듈과 Discord/Slack send 어댑터를 [LANDED]로 표기했으나, 실측 결과 저장 경로가 배선된 적이 없다 — RFC-0037이 유일한 writer로 지정한 `Board_attachment_meta.attach_to_post_meta`의 호출자가 테스트뿐이었고, 따라서 post `meta_json`에 `attachments` 슬롯이 기록된 적이 없다. renderer 역시 어떤 실행 진입점에서도 도달 불가였다. 해당 모듈 4종은 제거했다(git history에 보존). dashboard는 `normalizeBoardAttachments(meta?.attachments)`로 방어적으로 읽으므로 서버가 필드를 보내지 않으면 생략한다 — 클라이언트 렌더 경로(`components/board/post-attachments.ts`)는 유지. 재개하려면 writer 배선부터 시작한다. board→connector **push 경로도 원래 HEAD에 부재**(outbound는 keeper `keeper_surface_post` 매개). proactive attention candidate는 producer가 `record_and_wake`로 durable row를 먼저 기록한다. MASC의 lifecycle-sibling worker는 immutable input scheduling과 durable callback/settlement를 소유하고, OAS는 exact provider execution과 failover를 소유한다. Keeper owner lane은 완료된 결과 이벤트를 소비하도록 wake될 뿐 provider 실행을 수행하지 않는다. dashboard projection은 model judge를 실행하지 않는다. claim-evidence gate가 opt-in이라 free-text "task done" 완료주장이 검증 우회.
- **candidate work:** effect permission(Goal 9)·CI breadth(Goal 11)에 얹지 않는다. Board-owned 독립 slice로 substantive-vs-status LLM judge+비파괴 append, proactive candidate consumer 배선 또는 ledger+metric 삭제를 수행한다. `Board_render`는 LANDED(상기); nested comment-tree serializer+grapheme-aware limit과 board→connector push 배선(어느 채널에 무엇을 병낼지 정책 포함)은 별도 surface slice다.

<a name="32-goal"></a>
### 3.2 Goal — 엔티티 KEEP / goal-loop OODA 기계 RETIRE (RFC-0352 Path B)

- **판정(2026-07-21 오너 결정, RFC-0352):** Goal은 제품 스펙 "Goal/Task 약한 결합"의 1급 엔티티다. `goal_store.ml` SSOT, MCP tool 3종(`masc_goal_list/upsert/transition`), task↔goal linkage(RFC-0267), keeper meta `active_goal_ids`는 **KEEP**. 반대로 goal-loop OODA 기계 — server refresh loop, `/api/v1/dashboard/goal-loop/status` 표면, dashboard goal-loop 컴포넌트, `goal_store.ml`의 legacy-status 호환 decode — 는 **RETIRED**이며 절제한다. **`goal_phase.ml` FSM은 RETIRE 대상이 아니다**: `decide_transition` 소비자는 수동 tool 전이(`workspace_goals.ml:328`)와 엔티티 건강도 표시(`dashboard_goals_types_health.ml:51`) 2곳뿐인 자기완결 엔티티 FSM이고, goal-loop 표면은 이를 한 번도 참조하지 않는다(goal-loop의 observe/orient/... "phase"는 외부 JSON 문자열 필드로 동명이인). "stagnation counter"라는 독립 기계도 실존하지 않는다 — 실체는 /goals 트리의 파생 표시 필드 `stagnation_seconds`(`dashboard_goals_types_builder.ml:128`, now−last_activity)이며 KEEP이다(`stagnation_threshold` grep 0건).
- **이유:** MASC는 AutoGPT식 goal-decomposition runtime이 아니다(§1.3 Non-Goals 유지). Goal은 자동 분해·자율 스케줄링의 주체가 아니라 Keeper가 tool로 수동 관리하는 공유 계획 엔티티이고, 협업 실행 단위는 여전히 명시적 Task와 Keeper lane이다. "Goal을 살린다"가 goal-loop 자율 기계의 부활을 의미하지 않는다.
- **절제 범위(RFC-0352가 SSOT):** goal-loop OODA scheduler/status(server refresh loop `server_runtime_bootstrap.ml:1391-1394`, `/api/v1/dashboard/goal-loop/status` 라우트, broadcast 모듈, dashboard goal-loop TS 표면, WS 브릿지 1줄 `server_mcp_transport_ws.ml:423-424`), legacy-status 호환 decode(`goal_store.ml:136-165` 등). 스크립트 계층(OODA producer 포함)은 이미 삭제됨(#25477) — 잔존 OCaml/TS 표면은 아무도 쓰지 않는 파일을 읽는 dead-projection이다.
- **acceptance:** (a) Goal 엔티티 CRUD가 MCP tool 경로만으로 완결 (b) goal-loop scheduler/status 코드 0 (c) legacy-status decode 0 — typed 마이그레이션 vs 명시 보존은 RFC-0352 절제 슬라이스에서 확정 (d) RFC-0111/0067/0267은 유효 전제로 유지(Supersede 아님).
- **경위:** 옛 카드는 "Legacy Goal — RETIRED / HARD DELETE"로, §3.15의 `goal_store.ml` SSOT 선언과 자기모순이었다(§11 D10, 2026-07-21 dead-surface 감사가 적발). RFC-0352 1문항(Goal=1급 엔티티 vs Task 흡수 잔재)에 오너가 (a)로 답해 Path B로 해소됐다.
...
- **소유 타입:** typed keeper wake bus (G6 Async ping completion plane). Wake variant(`keeper_event_queue.mli` stimulus): `Fusion_completed`·`Hitl_resolved`·`Bg_completed`·`Connector_attention`·`Schedule_due`(문서의 `Schedule_signal`은 오기 — 실제는 `Schedule_due of scheduled_wake`; `Schedule_signal`은 `Schedule_signal_decode_error`로만 존재). `Keeper_waiting_inventory` (per-lane read model). typed schedule payload registry (지원 kind = `masc.keeper_wake`뿐, `schedule_supported_kinds.ml:18` — "SSOT kind = `masc.board_post`" 기술은 stale; `masc.board_post`는 dispatch 시 unsupported reject, `test_schedule_consumer_dispatch.ml:263`). typed Corrupt ledger.
- **경계 계약:** schedule payload/tool 선택은 typed registry가 accept 전 validate (G3). 미지원 side-effecting payload는 생성 시 실패+visible product-gap(`masc_schedule_payload_unsupported_total`), 숨은 terminal data 금지. (`keeper_wake`의 `Reminder_only` clamp는 **삭제됨** — #23716이 self-wake approval deadlock 종료로 도입, #24332 nonhierarchical Gate 전환에서 제거; HEAD grep 0, 2026-07-20.) at-most-once signal. per-tick crash isolation. tool-surface policy(G4): create/list/get/cancel=public+keeper-standard; approve/reject=operator-only; spawned-agent·local-worker=lane ownership contract 전까지 **EMPTY by policy**. cooperative yield(#20849): reservation은 lane을 globally block 안 함; owner-priority yield(`Yielded_to_chat_waiting`).
...
- **open goals:** Goal 6(durable Scheduler recurrence의 live operational proof와 unregistered target의 typed dispatch rejection), ~~Goal 7(occurrence identity + 동일 signal_id 재시도)~~ **[LANDED]** — occurrence identity는 `schedule_occurrence_id`(retry가 동일 identity 유지: `schedule_runner.ml:268`; 회귀 `test_schedule_runner.ml:394` "same occurrence identity reused", `:600`). 단 Goal 7 acceptance의 seen-key ledger corrupt loud tick fail 회귀는 별도 확인 필요(store에 typed `Corrupt_ledger` 존재, schedule runner/service 테스트에 corrupt case 미확인, 2026-07-20 grep). standing grant와 digest-equality 승인 우회는 삭제한다.
...
- **현재 상태:** **[LANDED, RFC-0317 PR-3]** Slack inbound gateway 배선됨 — `Slack_socket_client.run`이 `server_slack_in_process_gateway.ml:492`(`start` :424)에서 호출되고 프로덕션 bootstrap `server_runtime_bootstrap.ml:1194`("RFC-0317 PR-3: in-process Slack Socket Mode gateway")가 기동(dead scaffold 아님, 헤더에 Python sidecar 주장 없음). **[갱신 2026-07-20]** 옛 "busy-defer 전무(silent drop) / ambient wake Discord-only / speaker identity 소멸"은 전부 stale: ① busy 시 typed ack 발송 — `busy_ack_reply_text`·`busy_ack_reply_text_queued`(`gate_keeper_backend.ml:140,162`, "…is busy; your message is queued…"; LLM-judged busy-defer 응답은 여전히 미구현). ② connector-attention ambient wake는 Slack도 동일 — durable `Connector_attention` stimulus enqueue(`server_slack_in_process_gateway.ml:543`, `.mli:17`). named flag `MASC_CONNECTOR_AMBIENT_WAKE_ENABLED`는 **HEAD 부재** 유지(0건). ③ speaker identity `{speaker_id; speaker_name; speaker_authority}`는 durable persist(`keeper_chat_store.ml:175-179`, JSONL 직렬화 `:414-419`; Discord intake 전달 `server_discord_in_process_gateway.ml:505-510`).
- **open goals:** Goal 8(exact origin/channel/actor intake + lane-isolated requeue, Discord+Slack; ~~`Slack_socket_client.run` 배선~~ **[LANDED]**; ~~Slack ambient user-line wake~~ **[LANDED 2026-07-20]** — Slack도 `Connector_attention` enqueue; ~~RFC-0223 speaker-persistence~~ **[LANDED]** — presence round-trip만 잔여; busy-defer LLM judge turn; backend sidecar lifecycle+config-write(0600 하드닝)+JSON-schema endpoint).
...
- **현재 상태:** turn FSM ADT 존재하나 `Awaiting_provider→Streaming`·`Streaming⇄Awaiting_tool_result` emit_transition 미배선; `safe_emit_turn_end` catch-all이 `Cancelled_*`를 FSM에서 삼킴(#24448 orphan과 연결); 옛 "Pause 원칙 3중 위반(restart budget→Dead, auto-compact retry→Paused, overflow-pause shadow)"은 **전부 HEAD 부재**(각 grep 0, 2026-07-20) — supervisor는 budget 없이 crashed lane을 다음 restart pass에 queue(`keeper_supervisor.ml:99-102`, "restart counters remain observation only"); state 어휘 중첩(#40-42, `Stopped`=`Operator_pause` #24034-37).
...
- **현재 상태:** **[갱신 2026-07-30]** periodic full-store consolidation은 maintenance fiber, runtime route, env toggle, prompt/schema와 함께 삭제되었다. production caller가 없으면서 canonical authority를 자칭하던 `Keeper_memory_os_store`도 삭제되어 실제 JSONL authority만 남았다. Memory fact의 `claim_kind`, `valid_until`, Librarian/tool의 `valid_for_days`, `Ephemeral` category와 episode의 `open_items`/`constraints`/`preserved_tool_refs`는 producer·codec·API·dashboard에서 제거되며 current-only closed decoder가 재등장을 거부한다. expiry GC·dry-run/sanity facade·maintenance fiber·env toggle·TTL dashboard projection도 함께 제거한다. 과거 shape를 읽는 호환 필드나 자동 마이그레이션 경로는 두지 않으며, 배포는 명시적 fresh-state cutover를 요구한다.
- **candidate work:** compaction/failover Goal 3에 얹지 않는다. Memory-owned 독립 slice에서 exact existing `claim_id`를 대상으로 하는 typed `Create|Update|Supersede|Forget|Noop` receipt와 immutable revision commit을 설계한다. bounded Librarian의 semantic decision 품질은 Harness-First memory-quality eval(P-1, LLM-as-judge + hand-labeled calibration)로 검증한다. periodic full-store consolidation, `claim_kind`, count/score/clock 기반 survival gate를 재도입하지 않는다.
...
- **책임:** typed target + capability로 keeper/model/agent/tool을 조립 호출. K2K messages·@mention·keeper_msg_requests는 live이고, first-class "call a keeper as a tool" surface도 **존재**(`masc_keeper_delegate`/`masc_keeper_delegate_status`/`masc_keeper_delegate_cancel`/`masc_keeper_delegate_list` — `keeper_schema.ml:322,334,341,348`, `tool_catalog.ml` 등록, 계약 `keeper_invocation_contract.ml/mli`; 옛 "first-class surface 부재" 기술은 stale, 2026-07-20 grep).
...
- **현재 상태:** first-class tool surface는 **배선됨**(위 책임 항 참조 — delegate/status/cancel/list 4종 + `keeper_invocation_contract.ml/mli`, 2026-07-20 grep). K2K primitive도 live. 잔여는 Goal 5의 typed receipt composition과 preset record 공유.
...
- **책임:** per-keeper의 (a) repo config (b) identity/keeper (c) runtime/model (d) sandbox/exec (e) playground layout (f) durable lifecycle state에 대해 **축당 단일 권위(SSOT) + 나머지는 명시적 stamped projection(단일 read path)**. Rich Hickey "simple" + "Parse, don't validate" — dir-name≠id, snapshot≠authored 같은 illegal state를 unrepresentable 또는 fail-loud로.
- **파편화 인벤토리(실측):**
  ...
  | **instructions**(system-prompt) | AGENT.md(keeper base) → keeper.toml(overlay, wins); keeper.json은 current runtime snapshot | **YES** | 낮음 — authored prompt와 runtime snapshot의 경계가 명확함 |
  | **mention_targets** | keeper.toml | **YES** | 낮음 — Keeper prompt와 분리 |
  | **proactive_enabled** | keeper.toml + keeper.json(nested `proactive.enabled`) | **YES(authored/runtime)** | 낮음 — authored setting과 runtime snapshot 분리 |
  ...
  | rejected model-selection 키 | `removed_keeper_input_key_names`(models/allowed_models/active_model) + 별도 `runtime_assignment_result`(model/runtime_id→Error) + keeper `removed_fields` | **NO** — 3 리스트 | 중 — `allowed_providers`는 **어느 리스트에도 없음(grep 0)** → 미강제; keeper.toml model=Error vs keeper model=WARN(비대칭) |
  ...
  | **identity handles**(name/keeper_name/agent_name/keeper_name/actor-id) | `ids.ml`(typed `Keeper_id.Keeper_name`, event/trace만) + `keeper_identity.ml`(name↔agent_name 4-variant reverse-parse, layered fallback) + `tool_agent_timeline.ml:118`(3-form OR-매칭) + json `keeper_name` vs `name` 2키 | **NO** — 5+ 핸들 fallback-resolve, 동치 미강제 | **SEVERE** — 코드 주석 자체가 "typed Keeper_id 통합이 proper fix(RFC-scale)"라 자백. **credential alias 워크어라운드 #10440의 근본원인**(short-form lookup 8/14 실패→alias 땜빵, `server_runtime_startup_credentials.ml:127`) |
  ...
  | **active_goal_ids**(goal 바인딩) | keeper.toml(권위) → keeper.json emit되나 reconcile이 TOML 소유 시 `[]` 강제 persist(`keeper_runtime.ml:459`). Goal **엔티티**는 `goal_store.ml` SSOT (KEEP 확정: RFC-0352 Path B) | **YES(TOML)** — json은 TOML-미선언 시만 값 보유 | 낮음 — TOML 권위 |
...
- **경계 계약:** repo config·identity·runtime·sandbox·playground·lifecycle 각 축은 **정확히 하나의 파일/키가 권위**이고, 나머지 on-disk/in-mem 사본은 source `meta_version`으로 stamp된 projection이며 단일 read path만 노출한다. keeper 개별 파일(AGENT.md/keeper.toml/keeper.json)에 각자 역할이 있고 **같은 필드를 2곳 이상에서 정의하지 않는다**.
...
- **config vs runtime 경계 (적대 평가 확정 2026-07-15):** keeper.toml = **authored config SSOT**, keeper.json = **system-owned 런타임 상태 store**(매 턴 `write_meta_with_merge`, `meta_version` **CAS** bypass 없음 `keeper_meta_store.ml:377`, fsync+atomic-rename `keeper_fs.ml:110`). 이 둘은 mirror가 아니다 — TOML이 이미 config 권위이고 json은 (a) 런타임 상태 (b) dashboard/checkpoint가 blank를 안 보게 하는 **derived config snapshot**(keeper/instructions/multimodal/oas_env/active_goal_ids만, `keeper_meta_json.ml`)이다. `keeper_runtime.ml:407-465` reconcile이 json을 TOML로 되맞춘다. **따라서 "keeper.json 삭제 → 전면 toml-only"는 라이브를 깬다**(런타임 상태 per-turn CAS write를 comment-bearing TOML에 못 담음; Otoml이 주석 소실). **실제 잔여 정리(라이브-안전, 작음):** ①json config 키의 **legacy read-fallback 제거**(`keeper_meta_json_parse.ml`의 always_allow:178/mention_targets:125/oas_env:532 — write는 이미 TOML-only인데 read 입력만 남음) → json을 순수 런타임 상태로 ②derived snapshot(keeper/instructions/…)을 render-only projection으로 명시 stamp ③`mention_targets` keeper↔toml overlay를 union 또는 replace 명시. (이전 판본의 "dual-home 데이터 손실"은 **오독** — write 측 TOML 권위라 replace는 SSOT 정상 동작, 잃을 independent-authored json config 없음.)
- **candidate work:** **[REPO-SSOT, LIVE]** 유일하게 살아있는 repo-location 작업은 **역파싱 조인 제거**(`parse_playground_repo_path`+`find_url_by_id`, **프로덕션 caller 단 1곳** `keeper_tool_filesystem_runtime.ml:314`) → `git remote get-url`(`Repo_git.get_origin_url`, **이미 `repo_store.ml:439`에서 동일 용도로 사용**) + `worktree_root`로 귀속(RFC-0343 §3.1). **[해소됨]** `clone_sandbox_repo` full-record/`local_path` 개명은 **moot** — `playground_repo_readiness` 전체가 dead였고 삭제됨(**#24558**); clone role이 사라져 `local_path`는 single-role(등록 트리), 개명은 선택적 명명 정리로 강등. **[identity, SEVERE]** typed `Keeper_id` 단일 canonical + explicit projection으로 5+ 핸들 통합 → `identity_matches` OR-매칭·#10440 alias 워크어라운드 제거. **[config/runtime 정리, 라이브-안전]** json config 키 legacy read-fallback 제거(always_allow/mention_targets/oas_env) → json=런타임 상태 전용; derived snapshot을 render-only projection으로 stamp; `mention_targets` keeper↔toml overlay union화 또는 replace 명시. **(keeper.json 삭제는 non-goal — 런타임 상태 CAS store.)** **[나머지 축]** single read path(`read_meta` private화), single `Local`/`Docker` enum+1 parser, rejected-key 단일 closed set+`allowed_providers` 강제, docker layout을 `playground_paths` SSOT가 **구성+파싱** 둘 다. **[D8 정합]** 역파싱 귀속을 git-remote로 옮기면 playground 번들 전체가 bare per-keeper 작업디렉토리+주입 env로 붕괴 — §11 D8·D9와 한 수술.
...
- **acceptance:** hidden `temperature=0` fallback 없음. closed structural unit만 LLM이 판단하고 open ToolUse/progress는 exact 보존. fit은 exact pending source와 OAS가 hooks/projection 뒤 생성해 실제 dispatch할 동일 immutable request에 bind한다(MASC request 재조립 금지, source-free manual은 future-fit 주장 금지). complete next request가 provider-native count로 selected Runtime window 안에 들어올 때만 commit하며, oversized input은 durable LLM wave로 진행. 첫 provider 500→두 번째 성공, 전부 소진→typed "runtime lane exhausted", attempt당 manifest row 1개; before/after/fit/reinjection receipt durable.
...
- **touch:** gate inbound(connector context carry + speaker identity persist, RFC-0223 P1/P2 — speaker persist는 **[LANDED 2026-07-20]** `keeper_chat_store.ml:175-179`); `Slack_socket_client.run` 서버 bootstrap 배선(RFC-0317 PR-3) **[LANDED]**; busy-defer light response turn(LLM judge, RFC-0334 확장); backend sidecar lifecycle+config-write(0600 TOML)+JSON-schema endpoint; ~~Slack ambient wake producer~~ **[LANDED 2026-07-20]**(`server_slack_in_process_gateway.ml:543` `Connector_attention` enqueue — "현재 Discord-only"는 stale).
...
| Runtime assignment(runtime.toml) | keeper.toml/keeper | **독립** | provider/model 변경이 identity 파일 안 건드림 |
...
| Memory-owned semantic retention(Goal 3과 독립) | current-only closed fact/episode schema + Librarian typed decision + immutable revision receipt | **중** — extraction/forget 판단 또는 partial publication이 recall 품질에 영향 | whole revision reject; prior committed revision remains authoritative |
| Busy-defer connector reply(Goal 8) | lib/gate_keeper_backend.ml(inbound reply path) + lib/gate/slack_socket_client.ml + lib/gate/discord_gateway_client.ml | **중** — connector reply turn 신설; busy 시 typed ack는 이미 발송(`gate_keeper_backend.ml:140,162`, 2026-07-20 — 옛 "현상태=silent drop"은 stale) | busy-defer LLM-judged turn(현상태=fixed-text ack) |
...
| Re-enable sleep/consolidation fiber(#21237 merged, #21244 killed) | full-store periodic mutation과 category 기반 생존 판단이 current Memory 계약과 충돌 | 중(keeper_memory_os_* + sleep fiber) | **KILL — periodic survival authority 재도입 금지** |
| Action-stream self-reconciler (keeper_memory_os_self_reconcile) | deterministic event→semantic-claim 삭제표는 두 번째 의미 authority가 되며 UNBUILT | 중 | **KILL — source-domain typed state와 Librarian verdict 없이 구현 금지** |
| L0 recall routing (self-obs를 episodic/audit log에 default, recall store 아님) | transient run/queue state를 Memory claim으로 복제하는 것 자체가 source-domain SSOT와 충돌 | 중 | source-domain projection 유지, Memory 복제 0 |
...
| Fallback Resolution | "Falls back to meta.name when no resolved keeper" | typed 분리 필요 |
...
| **D9** | **repo location SSOT + keeper-config 파편 통합**(§3.15) — repo가 keeper에게 어디 있나를 단일 권위로. D8과 한 수술. **repo-location 절반은 RFC-0343 Draft로 착수**(역파싱→git-remote 귀속, feasibility=FEASIBLE 확정: `get_origin_url` 이미 `repo_store.ml:439`에서 동일 용도) | (a) **repo-SSOT 우선**: `clone_sandbox_repo`가 catalog record 전체 읽기(`"main"`/`[]` 하드코딩 제거), clone location=catalog id keyed 순수 함수, 역파싱 조인 제거→git-remote 귀속, 등록 `local_path`→`operator_checkout_path` 개명 (b) **전면 통합**: (a) + single read path(`read_meta` private)·mention_targets union·single `Local`/`Docker` enum·1 parser·rejected-key 단일 closed set(+`allowed_providers`)·docker layout을 `playground_paths` SSOT가 구성+파싱 (c) doc-only(DS-9)만 먼저, 코드는 defer | **높음(RFC-scale, credential-adjacent)** — repo location dual-authority가 379 `path_not_found`/24h 유발; `mention_targets` replace가 keeper 목록 소실(무-에러 버그). N-of-M/string-classifier 시그니처라 site-by-site 패치 금지, RFC로 흡수 | §3.15, `playground_repo_readiness.ml:344`, `keeper_types_profile_toml_parser.ml:277` |
| **D10** | **DECIDED (2026-07-21, 오너, RFC-0352 Path B)** — Goal은 1급 엔티티. (b)+(c) 조합으로 해소: Goal 엔티티/`goal_store.ml` SSOT/MCP tool/task linkage(RFC-0267)는 KEEP, goal-loop OODA 기계(scheduler/status/broadcast/dashboard 표면·legacy-status decode)만 절제 — `goal_phase.ml` 엔티티 FSM은 KEEP. §3.2 카드 재작성됨. (원 쟁점: "RETIRED / HARD DELETE" 카드 ↔ §3.15 goal_store SSOT 선언 자기모순, 2026-07-21 dead-surface 감사 적발) | 결정 완료 — 절제 슬라이스 인벤토리는 RFC-0352가 SSOT | 절제 슬라이스: goal-loop dashboard/서버 표면 + legacy decode (RFC-0267/0284 전제는 유지) | §3.2, RFC-0352, RFC-0267/0284/0294 |
