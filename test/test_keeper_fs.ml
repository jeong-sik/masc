...
let has_tmp_files dir =
  Sys.readdir dir |> Array.exists (fun name -> Filename.check_suffix name ".tmp")
;;
...
let test_generate_trace_id_format () =
  ...
  check
    ...
    "starts with trace-"
    true
    (String.length id > 6 && String.sub id 0 6 = "trace-");
...
let test_concurrent_ensure_dir () =
  ...
  Eio.Fiber.all
    (List.init 10 (fun _i () ->
       try ignore (KF.ensure_dir dir) with
...
let test_concurrent_save_atomic () =
  ...
  check
    ...
    "contains fiber-"
    true
    (String.length content > 0 && String.sub content 0 6 = "fiber-");
...
let () =
  ...
  run
    ...
    ; "identity", [ test_case "trace_id format" `Quick test_generate_trace_id_format ]
    ; ( "concurrent"
