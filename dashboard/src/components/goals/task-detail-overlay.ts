...
function ContractSection({ task }: { task: Task }) {
  ...
  return html`
    <div class="flex flex-col gap-3">
      ...
      ${isAwaitingVerification ? html`
        <div class="rounded border border-accent/30 bg-[var(--accent-5)] px-4 py-3">
          <div class="flex items-center justify-between gap-3 flex-wrap">
            <div class="text-xs font-medium text-accent">검증 키퍼 검증 중</div>
            <a
      ...
      <${GateSection} title="검사 → 구현" gate=${gate?.inspect_to_implement} />
      ...
      ${requiredEvidence.length > 0 ? html`
        <div class="rounded border border-[var(--white-10)] bg-[var(--white-3)] px-4 py-3">
          <div class="text-xs font-medium text-text-strong">필수 근거</div>
...
export function TaskDetailOverlay() {
  ...
  return html`
    <${DialogOverlay}
      ...
      panelClass="w-full max-w-[900px] max-h-[90vh] overflow-y-auto custom-scrollbar bg-[#0d1526] rounded border border-[var(--card-border)] shadow-[0_24px_64px_var(--black-50)]"
    >
      ${'' /* Sticky Header */}
