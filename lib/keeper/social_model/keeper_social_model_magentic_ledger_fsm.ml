...
let all_events =
  [ Progress_observed; Signals_pending; Goal_idle_timeout; All_quiet; Failure_observed ]
;;
...
let belief_summary_field belief_summary field_name =
  let parts =
    ...
    |> List.filter_map (fun raw_part ->
      ...
      let part =
        if has_prefix part "ledger:"
        then String.sub part 7 (String.length part - 7)
        else part
      in
      ...
      | Some idx ->
        ...
        let value =
          String.sub part (idx + 1) (String.length part - idx - 1) |> String.trim
        in
        Some (key, value))
  in
  List.find_map
...
let phase_of_current_intention = function
  | Some "record_progress_evidence" | Some "publish_progress_update" -> Some Advancing
...
let classify_event ~previous (input : input) =
  if input.has_progress_evidence
  then Progress_observed
  else if input.has_reactive_signal
  then Signals_pending
  else if
