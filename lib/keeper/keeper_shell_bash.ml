...
open Keeper_shell_bash_block_json
...
open Keeper_shell_bash_shape_scan
...
let handle_keeper_bash
  ...
  else if cmd = "" && Keeper_shell_bash_typed.has_typed_bash_input_key args
  then
    Keeper_shell_bash_typed.handle
  ...
  then error_json "cmd is required. Good: cmd='ls -la lib/'. Bad: cmd=''."
    ...
    | None when command_mentions_task_state_file cmd ->
      task_state_file_probe_block ~cmd ~cmd_for_log ()
    | None when command_looks_like_task_state_http_probe cmd ->
      task_state_http_probe_block ~cmd ~cmd_for_log ()
    | None -> begin
