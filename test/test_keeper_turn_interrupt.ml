...
let test_finalizer_clears_only_its_own_switch () =
  with_env @@ fun ~base ->
  ...
  ignore (Keeper_registry.For_testing.register ~base_path:base name (make_meta name));
  let current () =
    Atomic.get (Option.get (Keeper_registry.get ~base_path:base name)).current_turn_switch in
  ...
  check "old finalizer cannot clear successor"
    (match current () with Some switch -> switch == successor | None -> false);
  Keeper_registry.clear_turn_switch_if_current ~base_path:base name successor;
  check "current finalizer clears itself" (Option.is_none (current ()))
;;

let () =
  test_finalizer_clears_only_its_own_switch ();
