...
let expected =
    [ {|keeper_voice_speak|}, {|Speak a short utterance via the voice bridge. Blocks until local playback finishes: status='spoken' with played_seconds means the user has already heard it — do NOT repeat or rephrase the same content. Duplicate identical messages within 30s return status='dedup_skipped' without playing. TTS or playback failures are returned as errors (ok=false), not silent successes. Concurrent calls are serialized by a global lock.|}, {|{"properties":{"audio_device":{"description":"Optional target output device id/name for the dashboard/client","type":"string"},"message":{"description":"Text to speak","type":"string"},"priority":{"description":"Optional priority hint for the TTS endpoint","type":"integer"},"provider":{"description":"Optional voice provider override","type":"string"}},"required":["message"],"type":"object"}|}
    ; {|keeper_voice_listen|}, {|Record user speech via microphone and transcribe to text. Starts recording, waits for speech, stops on silence (2s), then returns transcribed text.|}, {|{"properties":{"language_code":{"description":"ISO language hint, e.g. ko, en","type":"string"},"timeout_seconds":{"description":"Max recording duration in seconds (default 15)","type":"number"}},"type":"object"}|}
    ; {|keeper_voice_agent|}, {|Get your own voice capability and configuration. Reports assigned voice, available voices, active voice session state, available conversation modes, and voice_loop guidance. Without MASC_VOICE_REALTIME_WS_URL, operator audio is transcribed to normal keeper text turns and keeper output uses keeper_voice_speak/dashboard audio clips. No network required.|}, {|{"properties":{},"type":"object"}|}
    ; {|keeper_voice_sessions|}, {|List active voice sessions from the voice bridge.|}, {|{"properties":{},"type":"object"}|}
    ; {|keeper_voice_session_start|}, {|Start a voice session for this keeper. Defaults to turn_based batch STT/TTS. conversation_mode=realtime_bridge is accepted only when MASC_VOICE_REALTIME_WS_URL points at a configured realtime audio bridge; otherwise the tool fails closed instead of pretending a duplex stream exists.|}, {|{"properties":{"conversation_mode":{"description":"Optional mode. realtime_bridge requires MASC_VOICE_REALTIME_WS_URL.","enum":["turn_based","realtime_bridge"],"type":"string"},"session_name":{"description":"Optional session name","type":"string"}},"type":"object"}|}
