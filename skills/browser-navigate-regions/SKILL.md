---
name: browser-navigate-regions
description: Navigate an observed automation tab to a known HTTP(S) URL and read its landing-page regions in one ordered composition. Use when no intermediate site decision is needed.
---

Invoke `keeper_compose_browser-navigate-regions` with the observed automation
...
region read. Reuse the returned region references for the next scoped scene
read. A URL acknowledgement does not prove application readiness: the site's
...
If navigation succeeded but the region read failed, retain the navigation node's
...
[[compositions]]
name = "browser-navigate-regions"
description = "Navigate one observed automation tab and return landing-page regions. If only the read fails, retry BrowserRead, not navigation. Site content still needs verification."
execution = "inline"
...
input = { kind = "object", fields = [
  ...
  { name = "tabId", value = { kind = "param", name = "tabId" } },
  { name = "mode", value = { kind = "literal", value = "regions" } },
