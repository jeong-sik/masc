...
let assoc_ci_opt key fields =
  ...
  List.find_map
    (fun (field_key, value) ->
       if String.equal (String.lowercase_ascii field_key) expected
       then Some value
       else None)
    fields
;;
...
let sorted_keys fields = fields |> List.map fst |> List.sort_uniq String.compare
...
let accept_header_ok raw =
  ...
  in
  List.mem "application/json" media_types && List.mem "text/event-stream" media_types
;;
...
let analyze_content ~config_path content =
  ...
  | Error parse_error ->
    empty
      ...
      ~file_present:true
      ~parse_error
      [ stage "codex_config_file" Stage_pass (Printf.sprintf "read %s" config_path)
  ...
  | Ok toml ->
    ...
    let bearer_token_env_var =
      Option.bind masc_fields (fun fields -> string_opt "bearer_token_env_var" fields)
    in
    let bearer_token_env_matches =
      Option.map (String.equal expected_token_env_var) bearer_token_env_var
    in
    let headers_fields =
      ...
      then None
        ...
        Some
          (match headers_fields with
           | Some fields -> Option.is_some (assoc_ci_opt "Authorization" fields)
           | None -> false)
    in
    let accept_header =
    ...
    let x_masc_agent =
      Option.bind headers_fields (fun fields -> string_ci_opt "X-MASC-Agent" fields)
    in
    let x_masc_agent_ok = Option.map (String.equal expected_x_masc_agent) x_masc_agent in
    let config_stage =
      stage "codex_config_file" Stage_pass (Printf.sprintf "read %s" config_path)
    in
    let parse_stage = stage "codex_config_parse" Stage_pass "TOML parsed" in
    let server_stage =
      if server_present
      then stage "codex_server_config" Stage_pass "[mcp_servers.masc] is present"
      else
        stage
          "codex_server_config"
          Stage_fail
    ...
    let auth_stage =
      ...
      | true, Some env_var, Some false when String.equal env_var expected_token_env_var ->
        stage
          "codex_auth_model"
          Stage_pass
          "uses bearer_token_env_var=MASC_MCP_TOKEN and no hardcoded Authorization header"
      | true, Some env_var, Some true when String.equal env_var expected_token_env_var ->
        stage
          "codex_auth_model"
          Stage_fail
      ...
      | true, None, _ ->
        stage "codex_auth_model" Stage_fail "missing bearer_token_env_var=MASC_MCP_TOKEN"
    in
    let headers_stage =
    ...
    let agent_stage =
      ...
      | false, _ ->
        stage
          ...
          "skipped because [mcp_servers.masc] is missing"
      | true, Some true ->
        stage "codex_agent_header" Stage_pass "X-MASC-Agent identifies codex-mcp-client"
      | true, Some false ->
        stage
          "codex_agent_header"
          Stage_warn
  ...
  then
    empty
      ...
      ; stage
          ...
          Stage_skip
          "skipped because Codex config file is missing"
      ; stage "codex_auth_model" Stage_skip "skipped because Codex config file is missing"
      ; stage
          "codex_http_headers"
          Stage_skip
          "skipped because Codex config file is missing"
    ...
    try analyze_content ~config_path (Fs_compat.load_file config_path) with
    ...
    | exn ->
      empty
        ...
        [ stage
            "codex_config_file"
            Stage_fail
            (Printf.sprintf "failed to read %s: %s" config_path (Printexc.to_string exn))
        ; stage
            "codex_config_parse"
            Stage_skip
...
let analyze_default () =
  ...
  | None ->
    empty
      [ stage "codex_config_file" Stage_fail "HOME and MASC_CODEX_CONFIG_PATH are unset"
      ; stage
          "codex_config_parse"
          Stage_skip
      ...
      ; stage
          ...
          Stage_skip
          "skipped because Codex config path is unknown"
      ; stage "codex_auth_model" Stage_skip "skipped because Codex config path is unknown"
      ; stage
          "codex_http_headers"
          Stage_skip
          "skipped because Codex config path is unknown"
...
let to_yojson report =
  `Assoc
    ...
    ; "server_names", `List (List.map (fun value -> `String value) report.server_names)
    ...
    ; option_bool_field "bearer_token_env_matches" report.bearer_token_env_matches
    ; option_bool_field "authorization_header_present" report.authorization_header_present
...
let warnings report =
  ...
  |> List.filter_map (fun stage ->
    ...
    | Stage_fail | Stage_warn ->
      Some (Printf.sprintf "Codex MCP pipeline %s: %s" stage.name stage.detail)
...
let next_actions report =
  let has_stage name =
    List.exists
      ...
      report.stages
  in
  [ (if has_stage "codex_config_file"
     then
       Some
         "Set MASC_CODEX_CONFIG_PATH or HOME so doctor auth can inspect the Codex MCP \
          config file."
     else None)
  ; (if has_stage "codex_server_config"
     then
       Some
         "Create a [mcp_servers.masc] entry in Codex config; MASC does not use `codex \
          mcp login` OAuth."
     else None)
  ; (if has_stage "codex_auth_model"
     then
       Some
         "Set [mcp_servers.masc].bearer_token_env_var=\"MASC_MCP_TOKEN\" and remove \
          hardcoded Authorization from http_headers."
     else None)
  ; (if has_stage "codex_http_headers"
     then
       Some
         "Set [mcp_servers.masc].http_headers.Accept to include application/json and \
          text/event-stream."
     else None)
  ; (if has_stage "codex_agent_header"
     then
       Some
         "Set [mcp_servers.masc].http_headers.X-MASC-Agent=\"codex-mcp-client\" for \
          config/runtime attribution."
     else None)
  ]
