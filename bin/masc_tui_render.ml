...
let render_task_detail (state : state) (task : Masc_domain.task) =
  ...
  (* Nine fixed chrome rows -- top, header, divider, title, status, created,
     divider, bottom, helper -- plus the status note lines, which vary by
     state. Clamped through the same helper the keeper log pane uses. *)
  let content_height = max 1 (rows - 9 - List.length note_lines) in
