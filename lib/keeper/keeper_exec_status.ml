...
let active_model_of_meta (m : keeper_meta) : string =
  if m.runtime.usage.last_model_used <> ""
  then m.runtime.usage.last_model_used
  else (
...
let next_model_hint_of_meta (m : keeper_meta) : string option =
  ...
  let pool = Keeper_model_labels.configured_model_labels_of_meta m in
...
let keeper_health_of_string s =
  ...
  | None ->
    Log.Keeper.warn
      "keeper_health_of_string: unknown wire string %S → KH_offline fallback (#8670)"
      s;
    KH_offline
;;
...
let parse_agent_status (config : Coord.config) ~(agent_name : string) : Yojson.Safe.t =
  ...
  in
  if not (Coord.path_exists config agent_file)
  then `Assoc [ "exists", `Bool false ]
  else (
    match Coord.read_json_opt config agent_file with
    | None -> `Assoc [ "exists", `Bool true; "error", `String "failed_to_read" ]
    | Some json ->
      (match Types.agent_of_yojson json with
       ...
       | Ok (agent : Types.agent) ->
         ...
         let joined_ts =
           Resilience.Time.parse_iso8601_opt agent.joined_at |> Option.value ~default:0.0
         in
         let last_seen_ts =
           Resilience.Time.parse_iso8601_opt agent.last_seen |> Option.value ~default:0.0
         in
         ...
         `Assoc
           ...
           ; ( "is_zombie"
             , `Bool (Coord.is_zombie_agent ~agent_name:agent.name agent.last_seen) )
           ]))
;;
...
let json_bool key json default = Safe_ops.json_bool ~default key json
...
let agent_runtime_has_live_work agent_status =
  match agent_status_text agent_status with
  | "active" | "busy" | "listening" -> agent_last_seen_ago_s agent_status <= 120.0
  | _ -> false
;;
...
let quiet_hours_active () =
  ...
  quiet_start < quiet_end && current_hour >= quiet_start && current_hour < quiet_end
;;
...
let keeper_reply_snapshot_of_history (history_items : Yojson.Safe.t list) =
  ...
  let update_last role ts content ((last_user, last_assistant) as acc) =
    ...
    if role = "user"
    then Some (ts, content), last_assistant
    else if role = "assistant"
    then last_user, Some (ts, content)
    else acc
  in
...
let error_keywords =
  let provider_keywords =
    List.map
      (fun (a : Provider_adapter.adapter) -> a.canonical_name)
      Provider_adapter.direct_adapters
    @ List.concat_map
        (fun (a : Provider_adapter.adapter) -> a.aliases)
        Provider_adapter.direct_adapters
  in
  [ "error"; "failed"; "timeout"; "graphql"; "model" ] @ provider_keywords
;;

let looks_error_like text = List.exists (string_contains_ci text) error_keywords
...
let classify_keeper_quiet_reason ~meta ~keepalive_running ~agent_status ~now_ts =
  ...
  in
  let error_hint =
    if live_signal_supersedes_persisted_error
    then None
    else keeper_error_hint ~agent_status ~meta
  in
  if not meta.proactive.enabled
  then Some "disabled"
  else if not keepalive_running
  then Some "not_running"
  else if
    (not agent_exists) || agent_status_text = "offline" || agent_status_text = "inactive"
  then Some "agent_missing"
  else if meta.runtime.usage.total_turns = 0 && meta.runtime.proactive_rt.count_total = 0
  then (
    let keeper_age_s =
  ...
  then Some "quiet_hours"
    ...
    | None ->
      ...
      then (
        ...
        | Some age when age < float_of_int meta.proactive.cooldown_sec -> Some "min_gap"
        | _ ->
...
let keeper_health_state
      ?(fiber_health = Fiber_unknown)
  ...
  | Fiber_alive | Fiber_unknown ->
    ...
    in
    let effective_activity_ago_s =
      if agent_runtime_has_live_work agent_status
      then min last_turn_ago_s last_seen_ago_s
      else last_turn_ago_s
    in
    if
      (not agent_exists)
      || agent_status_text = "offline"
      || agent_status_text = "inactive"
    then KH_offline (* H-4 fix: true zombies are stale regardless of keepalive state *)
    else if is_zombie
    then KH_stale
    else if keepalive_running
    then
      if last_seen_ago_s > 2.0 *. keepalive_interval_s
      then KH_stale
      else (
        ...
        | _ ->
          ...
          then KH_idle
            ...
            effective_activity_ago_s > float_of_int (max meta.proactive.idle_sec 900)
          then KH_idle
          else KH_healthy)
    else if last_seen_ago_s > stale_threshold_s
    then KH_stale
    else KH_offline
;;
...
let keeper_diagnostic_summary ~meta ~(health_state : keeper_health) ~quiet_reason =
  match health_state with
  | KH_zombie ->
    "Keeper fiber has terminated but registry entry persists. Supervisor will \
     auto-restart."
  | KH_dead ->
    "Keeper restart budget exhausted. Manual restart via masc_keeper_up required."
  | KH_offline | KH_stale | KH_degraded ->
    "Keeper is not in a healthy reply state. Probe or recover before relying on \
     automation."
  | KH_healthy | KH_idle ->
    (match quiet_reason with
     | Some "disabled" ->
       "Keeper proactive automation is disabled. Direct messages still work, but \
        scheduled social ticks will stay quiet."
     | Some "not_running" -> "Keeper keepalive is enabled, but its loop is not running."
     | Some "agent_missing" ->
       "Keeper keepalive is running, but the live agent record is missing or offline."
     | Some "quiet_hours" ->
       "Quiet hours are active. Direct messages still work, but scheduled social ticks \
        may look asleep."
     | Some "min_gap" ->
       if meta.runtime.proactive_rt.last_outcome = Proactive_silent
       then
         "Latest autonomous proactive cycle completed silently. The next deterministic \
          cycle will open after cooldown."
       else
         "Keeper is inside its proactive cooldown window. Direct messages work now; \
          autonomous check-ins will wait."
     | Some "never_started" ->
...
let keeper_continuity_state
  ...
  let healthy_like =
    ...
    | KH_offline | KH_stale | KH_degraded | KH_zombie | KH_dead -> false
  in
  ...
  then Continuity_not_running
  else if recently_started || not healthy_like
  then Continuity_recovering
...
let keeper_continuity_summary = function
  ...
  | Continuity_recovering -> "Keeper runtime is reconciling back into live presence."
  | Continuity_healthy -> "Keeper runtime is aligned with the durable keeper state."
;;
...
let keeper_surface_status ~(agent_status : Yojson.Safe.t) ~(diagnostic : Yojson.Safe.t) =
  let health_state =
    json_string_opt "health_state" diagnostic
    |> Option.value ~default:"offline"
    |> keeper_health_of_string
...
let keeper_diagnostic_json
  ...
  in
  let last_error = Json_util.string_opt_to_json (keeper_error_hint ~agent_status ~meta) in
  `Assoc
