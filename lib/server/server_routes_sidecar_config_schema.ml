...
let python_argv_for sidecar_dir =
  ...
  then [ "uv"; "run"; "--directory"; sidecar_dir; "python"; "-m"; "src.schema_dump" ]
    ...
    [ "sh"; "-c";
      "{ test -f .venv/.deps-ok || { mkdir -p .venv && flock .venv/.bootstrap.lock sh -c 'test -f .venv/.deps-ok || { python3 -m venv .venv && .venv/bin/pip install -r requirements.txt 2>&1 && touch .venv/.deps-ok; };'; }; } && .venv/bin/python -m src.schema_dump" ]
;;
