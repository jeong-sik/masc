...
let test_unstreamed_reply_is_forwarded_at_the_end () =
  match
    Adapter.project_stream
      [ turn_started; Serve.Text_delta "MASC_"; Serve.Turn_finished { text = "MASC_MUSE_OK" } ]
  with
  | [ MessageStart _
...
let test_error_projection () =
  ...
  with
  | Agent_core.Error.Provider
      (Llm_provider.Error.ProviderReportedError { error_type = Some "turn_failed"; _ }) -> ()
  | error -> failf "turn failure: %s" (Agent_core.Error.to_string error)
;;
...
(* The serve client spawns [cli_path serve] in the Keeper's base path; with
   ...
   one session per process: a start or a resume, one turn, one MASC tool call
   through the session's MCP server, and one built-in tool item. *)
...
opened = read()
session_result = {"session": {"sessionId": SESSION, "status": "idle", "turnCount": 0,
                              "modelId": "muse-fixture-1",
...
send({"jsonrpc": "2.0", "id": opened["id"], "result": session_result})
if mode == "resume":
    approval = read()
...
notify("turn/started", {"sessionId": SESSION, "turnId": turn_id, "commandId": turn_id,
                        "viewCursor": "v:2"})
notify("item/started", {"sessionId": SESSION, "viewCursor": "v:3", "item": {
    "itemId": "m-1", "kind": "agentMessage", "turnId": turn_id, "revision": 1,
    "status": "inProgress", "text": ""}})
notify("item/delta", {"sessionId": SESSION, "viewCursor": "v:4", "itemId": "m-1",
                      "field": "text", "delta": "MASC_"})
...
tool_item = {"itemId": "tc-1", "kind": "toolCall", "turnId": turn_id, "revision": 1,
             "status": "inProgress", "tool": "read_file", "callId": "call-native-1",
             "args": "{}"}
notify("item/started", {"sessionId": SESSION, "viewCursor": "v:5", "item": tool_item})
tool_item = dict(tool_item, revision=2, status="completed")
notify("item/completed", {"sessionId": SESSION, "viewCursor": "v:6", "item": tool_item})
notify("item/delta", {"sessionId": SESSION, "viewCursor": "v:7", "itemId": "m-1",
                      "field": "text", "delta": "MUSE_KEEPER_OK"})
notify("item/completed", {"sessionId": SESSION, "viewCursor": "v:8", "item": {
    "itemId": "m-1", "kind": "agentMessage", "turnId": turn_id, "revision": 2,
    "status": "completed", "text": "MASC_MUSE_KEEPER_OK"}})
notify("turn/completed", {"sessionId": SESSION, "turnId": turn_id, "terminal": "completed",
                          "viewCursor": "v:9", "usage": {"inputTokens": 100,
                          "outputTokens": 7, "cachedTokens": 50, "reasoningTokens": 3}})
for _ in sys.stdin:
    pass
|}
;;
...
let run_turn ~base_path ~tool =
  ...
  let outcome =
    Keeper_muse_runtime.run
      ...
      ~event_bus:None
      ~raw_trace:None
      ~on_event:(Some (fun event -> events := event :: !events))
      ~config
      ()
  in
  { outcome
  ...
  | Agent_core.Types.MessageStart { model = "muse-fixture-1"; _ } :: rest ->
    (match List.rev rest with
     | MessageStop :: MessageDelta { stop_reason = Some EndTurn; _ } :: _ ->
       ...
       check int (label ^ ": one built-in tool block") 1 (count Runtime_native_tools.stream_content_type);
       check int (label ^ ": no other block") 2 (List.length starts)
            ...
            Eio.Switch.run (fun sw ->
              ...
              Eio_context.with_test_env
                ...
                (fun () ->
                  ...
                  check (list (pair int string)) "one built-in action"
                    [ 1, "call-native-1:read_file" ]
