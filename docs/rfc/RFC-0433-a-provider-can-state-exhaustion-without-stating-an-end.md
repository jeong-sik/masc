...
"남는 것 중 제일 싼 것"을 고르려면 값이 필요한데 **이 저장소에 가격표가 없다.**
`runtime.toml` 에 단가 필드가 없고, `cost_usd` 는 `dashboard_agent_core_bridge`
에서 provider 가 보고한 **사후 관측치**로만 존재한다.

없는 데이터로 순서를 정하지 않는다. 하려면 두 가지 중 하나가 먼저다.

1. 모델 행에 단가를 선언하고 운영자가 관리한다 — 값이 바뀌면 설정이 낡는다.
2. 관측된 `cost_usd` 를 scope 별로 누적해 실측 단가를 만든다 — 데이터가 이미
   흐르고 있고 낡지 않는다. 다만 표본이 쌓여야 한다.

2 번이 이 저장소의 방식에 맞지만 별개 RFC 다. 이 RFC 는 **죽은 것을 뒤로
보내는 것**까지만 한다. 순서는 선언 순서 그대로다.
