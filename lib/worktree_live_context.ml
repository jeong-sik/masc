...
let git_capture_hook_for_tests : git_capture_hook option Atomic.t = Atomic.make None

let set_git_capture_hook_for_tests hook =
  Atomic.set git_capture_hook_for_tests (Some hook)
;;

let clear_git_capture_hook_for_tests () = Atomic.set git_capture_hook_for_tests None
...
let git_status_timeout_sec () =
  Env_config_core.get_float
    ~default:default_git_status_timeout_sec
...
let repo_root_for ~base_path =
  if not (Coord_git.has_git_marker base_path) then None else nearest_git_root base_path
;;
...
let status_cache : (string, status_cache_entry) Hashtbl.t = Hashtbl.create 4
  ...
  then None
    ...
    Fun.protect
      ...
      (fun () ->
         ...
         | Some entry when now -. entry.refreshed_at <= ttl -> Some entry.lines
         | _ -> None))
;;
...
let status_cache_store repo_root lines ~now ~ttl =
  ...
  then (
    ...
    Fun.protect
      ~finally:(fun () -> Stdlib.Mutex.unlock status_cache_mu)
      (fun () -> Hashtbl.replace status_cache repo_root { lines; refreshed_at = now }))
;;
...
let single_flight_registry : (string, Stdlib.Mutex.t) Hashtbl.t = Hashtbl.create 4
...
let state_file ~repo_root ~actor_key =
  ...
  Filename.concat (state_dir ~repo_root) (Printf.sprintf "%s.git-status-hash" safe_key)
;;

let read_file_if_exists path =
  try
    if Fs_compat.file_exists path
    then Some (String.trim (Fs_compat.load_file path))
    else None
  with
  | Sys_error _ -> None
;;
...
let hash_lines lines = Digest.string (String.concat "\n" lines) |> Digest.to_hex

let change_block_of_lines lines =
  let visible_lines =
    if List.length lines > 20 then List.filteri (fun idx _ -> idx < 20) lines else lines
  in
...
let capture_change_block ~base_path ~actor_key =
  ...
  | Some repo_root ->
    ...
    if lines = [] || current_hash = previous_hash
    then None
