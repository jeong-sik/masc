...
let promote_confidence_re = Re.Pcre.re {|confidence: [0-9.]+|} |> Re.compile
let promote_verified_by_re = Re.Pcre.re {|verified_by: \[\]|} |> Re.compile
...
let candidates_dir () = Filename.concat (library_root ()) "candidates"
      ...
      List.find_map
        (fun line ->
           if
             ...
             && String.sub line 0 (String.length pattern) = pattern
           then
             Some
               (String.trim
                  (String.sub
...
let list_documents ?(include_candidates = false) () =
  ...
  let candidate_docs = if include_candidates then read_dir (candidates_dir ()) else [] in
    ...
    List.filter_map
      (fun path ->
         try
           ...
           | None -> Some (sprintf "- %s (no frontmatter)" (Filename.basename path))
         with
         | Sys_error _ -> None)
      docs
  in
...
let handle_read _ctx args =
  let topic =
    Yojson.Safe.Util.(member "topic" args |> to_string_option) |> Option.value ~default:""
  in
  if topic = ""
  then false, "topic is required"
  else (
    (* Find matching file *)
    ...
    | path :: _ ->
      (try
       ...
       | exn -> false, sprintf "Read error: %s" (Printexc.to_string exn)))
;;
...
let handle_add ctx args =
  ...
  let source =
    U.member "source" args
    |> U.to_string_option
    |> Option.value ~default:"direct_experience"
  in
  let confidence =
    U.member "confidence" args |> U.to_float_option |> Option.value ~default:0.7
  in
  let tags =
    try U.member "tags" args |> U.to_list |> List.filter_map U.to_string_option with
    | Yojson.Safe.Util.Type_error (_, _) -> []
  in
  let content =
    U.member "content" args |> U.to_string_option |> Option.value ~default:""
  in
  if title = ""
  then false, "title is required"
  else if content = ""
  then false, "content is required"
  else (
...
|}
      ...
      (try
         ...
         let status =
           if confidence < 0.5 then "candidate (needs verification)" else "library"
         in
         true, sprintf "Document added to %s: %s" status filepath
       with
       | Eio.Cancel.Cancelled _ as e -> raise e
       | exn -> false, sprintf "Write error: %s" (Printexc.to_string exn)))
;;
...
let handle_promote ctx args =
  ...
  in
  let new_confidence =
    Yojson.Safe.Util.(member "confidence" args |> to_float_option)
    |> Option.value ~default:0.7
  in
  if topic = ""
  then false, "topic is required"
  else if new_confidence < 0.5
  then false, "confidence must be >= 0.5 to promote"
  else (
    let topic_lower = String.lowercase_ascii topic in
    let candidates =
      list_documents ~include_candidates:true ()
      |> List.filter (fun f ->
    ...
    | src_path :: _ ->
      (try
         ...
         let updated =
           Re.replace_string
             promote_confidence_re
             ~by:(sprintf "confidence: %.2f" new_confidence)
             content
         in
         let with_verifier =
           Re.replace_string
             promote_verified_by_re
             ~by:(sprintf "verified_by: [%s]" ctx.agent_name)
             updated
         in
         (* Move to library *)
         let dest_path = Filename.concat (library_root ()) (Filename.basename src_path) in
         Out_channel.with_open_text dest_path (fun oc ->
           Out_channel.output_string oc with_verifier);
         Sys.remove src_path;
         ( true
         , sprintf "Promoted to library: %s (confidence: %.2f)" dest_path new_confidence )
       with
       | Eio.Cancel.Cancelled _ as e -> raise e
       | exn -> false, sprintf "Promote error: %s" (Printexc.to_string exn)))
;;

(* Search documents *)
let handle_search _ctx args =
  let query =
    Yojson.Safe.Util.(member "query" args |> to_string_option) |> Option.value ~default:""
  in
  if query = ""
  then false, "query is required"
  else (
      ...
      List.filter_map
        (fun path ->
           try
             ...
             if string_contains ~sub:query_lower content_lower
             then (
               match parse_frontmatter content with
               | Some fm ->
                 Some
    ...
    in
    if matches = []
    then true, sprintf "No documents matching '%s'" query
    else
      ( true
      , sprintf
          "## Search Results (%d)\n\n%s"
...
let tool_definitions =
  [ ( "masc_library_list"
    , {|List all documents in the agent knowledge library. Returns title, confidence, source, and tags for each document.|}
    , [ ( "include_candidates"
        , "boolean"
        , false
        , "Include candidate documents awaiting verification" )
      ] )
  ; ( "masc_library_read"
    , {|Read a specific library document by topic name.|}
  ...
  ; ( "masc_library_promote"
    , {|Promote a candidate document to the main library after verification.|}
    , [ "topic", "string", true, "Topic name to promote"
      ; "confidence", "number", true, "New confidence score (must be >= 0.5)"
      ] )
  ; ( "masc_library_search"
    , {|Search library documents by content or tags.|}
...
let schemas : Types.tool_schema list =
  [ (* masc_library_list *)
    ...
    ; description =
        "List all documents in the agent knowledge library with title, confidence, \
         source, and tags. Use when browsing available knowledge or checking if a topic \
         is already documented. Pair with masc_library_read to fetch a specific document \
         or masc_library_search to query by content."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
            , `Assoc
                [ ( "include_candidates"
                  , `Assoc
                      [ "type", `String "boolean"
                      ; ( "description"
                        , `String "Include candidate documents awaiting verification" )
                      ] )
                ] )
          ]
    }
  ; (* masc_library_read *)
    ...
    ; description =
        "Read a specific library document by topic name or partial match. Use when you \
         need the full content of a known knowledge document. After masc_library_list or \
         masc_library_search to find the topic name."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
            , `Assoc
                [ ( "topic"
                  , `Assoc
                      [ "type", `String "string"
                      ; ( "description"
                        , `String "Topic name or partial match (e.g., 'eio-mutex')" )
                      ] )
                ] )
  ...
  ; (* masc_library_add *)
    ...
    ; description =
        "Add a new document to the agent knowledge library (confidence < 0.5 goes to \
         candidates/ for review). Use when recording a new finding, experiment result, \
         or pattern that other agents should know about. Follow up with \
         masc_library_promote to move candidates to the main library after verification."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
                ...
                ; ( "source"
                  , `Assoc
                      ...
                      ; (* Issue #8601: enum derived from Variant SSOT. *)
                        "enum", `List (List.map (fun s -> `String s) valid_source_strings)
                      ] )
                ; ( "confidence"
                  , `Assoc
                      [ "type", `String "number"
  ...
  ; (* masc_library_promote *)
    ...
    ; description =
        "Promote a candidate document to the main library after verification (new \
         confidence must be >= 0.5). Use when a candidate document has been reviewed and \
         confirmed as accurate. After masc_library_add placed the document in \
         candidates/."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
  ...
  ; (* masc_library_search *)
    ...
    ; description =
        "Search the agent knowledge library by content keywords or tags. Use when \
         looking for documents on a specific topic without knowing the exact title. Pair \
         with masc_library_read to fetch matching documents in full."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
