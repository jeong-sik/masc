...
let annotate_section
  ...
  let operational =
    has_operational_signal ~section ~room_health ~incident_count ~recommended_action_count
  in
  let signal_class, evidence_quality =
    if gap_count > 0 && not operational
    then "metadata_gap", "missing"
    else if gap_count > 0 && operational
    then "mixed", "partial"
    else if operational && evidence <> []
    then "operational_risk", "strong"
    else if operational
    then "operational_risk", "partial"
    else if evidence <> []
    then "operational_risk", "partial"
    else "operational_risk", "missing"
  in
  `Assoc
...
let status_is_active_agent value =
  List.mem (String.lowercase_ascii (String.trim value)) [ "active"; "busy" ]
;;

let evidence_add_if cond text items = if cond && text <> "" then text :: items else items

let build_communication_section
      ~sessions
      ~recent_messages
      ~metadata_gaps
      ~room_health
  ...
  let metadata_evidence =
    evidence_of_metadata_gaps ~section:Communication metadata_gaps
  in
  let positive_signal =
  ...
  let evidence =
    ...
    then take 2 (metadata_evidence @ positive_evidence @ inactivity_evidence)
    else take 2 (positive_evidence @ inactivity_evidence)
  in
  if positive_signal && metadata_evidence = []
  then
    ( "healthy"
    , "Communication activity is recorded across recent messages and session metrics."
    , evidence )
  else if positive_signal
  then
    ( "watch"
    , "Communication activity exists, but some communication metadata is still missing."
    , evidence )
  else if live_session_count = 0
  then
    ( "unclear"
    , "No live session is present, so communication health cannot be judged."
    , evidence )
  else if metadata_evidence <> []
  then
    ( "unclear"
    , "Communication metadata is incomplete and no positive activity signal is recorded."
    , evidence )
  else if known_mode_count = 0
  then "unclear", "Communication mode is not recorded for the live sessions.", evidence
  else if
    Dashboard_utils.is_health_at_risk (Dashboard_utils.health_level_of_string room_health)
    || incident_count > 0
  ...
  then
    ( "watch"
    , "Live sessions exist without recorded communication activity while the namespace \
       still has open operator attention."
    , evidence )
  else
    ( "watch"
    , "Live sessions exist, but no communication activity is recorded yet."
    , evidence )
;;
...
let build_alignment_section ~sessions ~agents ~metadata_gaps =
  ...
  let assigned_active_agent_count =
    List.fold_left
      (fun acc json ->
         if
           status_is_active_agent (string_field "status" json)
  ...
  let evidence =
    ...
    |> fun items -> items @ metadata_evidence |> take 2
  in
  if active_agent_count = 0
  then "unclear", "No active agents are present, so alignment cannot be judged.", evidence
  else if metadata_evidence <> []
  then
    ( "unclear"
    , "Goal or focus bindings are incomplete, so alignment cannot be confirmed."
    , evidence )
  else if bound_goal_count = 0
  then "unclear", "Active agents exist, but no bound session goal is recorded.", evidence
  else if assigned_active_agent_count = active_agent_count
  then
    "aligned", "Active agents have bound focus and session goals are recorded.", evidence
  else "watch", "Some active agents are present without a bound focus.", evidence
;;

let build_watch_section
      ~room_health
      ~incident_count
      ~recommended_action_count
  ...
  let risky_room = Dashboard_utils.is_health_at_risk room_health_level in
  let evidence =
    []
  ...
  then
    ...
    , Printf.sprintf
        ...
        recommended_action_count
    , evidence )
  else if incident_count > 0 || recommended_action_count > 0
  then
    ( "watch"
    , Printf.sprintf
        ...
        recommended_action_count
    , evidence )
  else "ok", "No immediate operator action is flagged by the namespace summary.", evidence
;;

let build_briefing_sections
