...
(* Shorthand for emit_test_runner: produces "T.V_int 0" etc. that consumers
   read with [module T = Tlc_test_gen.Ttrace_parser]. *)
let value_to_ocaml_short = function
  | V_int n    -> Printf.sprintf "T.V_int %d" n
  | V_string s -> Printf.sprintf "T.V_string %S" s
  | V_bool b   -> Printf.sprintf "T.V_bool %B" b
  | V_raw r    -> Printf.sprintf "T.V_raw %S" r

let emit_test_runner (st : state) =
  let buf = Buffer.create 1024 in
  Buffer.add_string buf
    (Printf.sprintf
       "(* Auto-generated by tlc_test_gen --runner from %s. *)\n"
       st.trace_file);
  Buffer.add_string buf
    "(* DO NOT EDIT — regenerate via dune. *)\n\n";
  Buffer.add_string buf "module T = Tlc_test_gen.Ttrace_parser\n\n";

  (* Trace literal. *)
  Buffer.add_string buf
    "let trace : (string * T.value) list list =\n";
  (match st.steps with
   | [] ->
       Buffer.add_string buf
         "  [] (* TLC trace lacked a parseable _TETrace module. *)\n"
   | steps ->
       Buffer.add_string buf "  [\n";
       List.iter
         (fun step ->
  ...
  Buffer.add_string buf
    (Printf.sprintf
       "      prerr_endline \"FAIL [%s]: empty trace\";\n      exit 1\n"
       st.spec_module);
  Buffer.add_string buf "  | Some last ->\n";
  Buffer.add_string buf "      let ok name expected =\n";
  Buffer.add_string buf "        match List.assoc_opt name last with\n";
  Buffer.add_string buf "        | Some v when v = expected -> ()\n";
  Buffer.add_string buf "        | Some _ ->\n";
  Buffer.add_string buf
    (Printf.sprintf
       "            Printf.eprintf \"FAIL [%s.%%s]: value mismatch\\n\" name;\n"
       st.spec_module);
  Buffer.add_string buf "            exit 1\n";
  Buffer.add_string buf "        | None ->\n";
  Buffer.add_string buf
    (Printf.sprintf
       "            Printf.eprintf \"FAIL [%s.%%s]: missing\\n\" name;\n"
       st.spec_module);
  Buffer.add_string buf "            exit 1\n";
  Buffer.add_string buf "      in\n";
  (match st.bindings with
   | [] ->
       Buffer.add_string buf "      ignore ok;\n";
       Buffer.add_string buf
         (Printf.sprintf
            "      prerr_endline \"FAIL [%s]: no terminal bindings\";\n      exit 1\n"
            st.spec_module)
   | bindings ->
       List.iter
         (fun (name, v) ->
         ...
         bindings;
       Buffer.add_string buf
         (Printf.sprintf "      print_endline \"PASS [%s]\"\n"
            st.spec_module));
  Buffer.contents buf
