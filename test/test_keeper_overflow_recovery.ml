...
let overflow_event ?(tokens = 205_000) ?(limit = Some 200_000) () =
  SM.Context_overflow_detected
    { source = `Prompt_rejected; token_count = tokens; limit_tokens = limit }
;;

let apply_ok phase conds ev =
  match SM.apply_event ~current_phase:phase ~conditions:conds ~event:ev ~now:0.0 with
  | Ok tr -> tr
  | Error err ->
    failf
...
let test_happy_path () =
  ...
  check bool "context_overflow cleared" false tr3.updated_conditions.context_overflow;
  check
    bool
    "compact_retry_exhausted cleared"
    false
...
let test_compact_failure_latches_paused () =
  ...
  check bool "context_overflow still set" true tr3.updated_conditions.context_overflow;
  ...
  let exhausted_conds = { tr3.updated_conditions with compact_retry_exhausted = true } in
  ...
  check_phase
    SM.Paused
    (SM.derive_phase exhausted_conds)
...
let test_operator_clear_returns_to_running () =
  ...
  let tr2 =
    apply_ok
      SM.Overflowed
      tr1.updated_conditions
      (SM.Operator_clear_requested { preserve_system = true; reason = "manual test" })
  in
  check_phase SM.Running tr2.new_phase "operator clear drops context → Running";
  check bool "context_overflow cleared" false tr2.updated_conditions.context_overflow;
  check
    bool
    "compact_retry_exhausted cleared"
    false
...
let test_two_consecutive_overflows () =
  ...
  check
    bool
    "retry latch still clear after successful cycle"
    false
    tr4.updated_conditions.compact_retry_exhausted;
  let tr5 = apply_ok SM.Overflowed tr4.updated_conditions SM.Auto_compact_triggered in
...
let test_noop_compaction_keeps_overflow () =
  ...
  let tr3 =
    apply_ok
      SM.Compacting
      tr2.updated_conditions
      (SM.Compaction_completed { before_tokens = 200_000; after_tokens = 200_000 })
  in
  check
    bool
    "noop compaction does NOT clear context_overflow"
    true
    tr3.updated_conditions.context_overflow;
  check
    bool
    "noop compaction does NOT reset compact_retry_exhausted"
    tr2.updated_conditions.compact_retry_exhausted
    tr3.updated_conditions.compact_retry_exhausted;
  check
    bool
    "noop still exits Compacting (compaction_active=false)"
    false
...
let test_negative_savings_keeps_overflow () =
  ...
  let tr3 =
    apply_ok
      SM.Compacting
      tr2.updated_conditions
      (SM.Compaction_completed { before_tokens = 200_000; after_tokens = 210_000 })
  in
  check
    bool
    "negative-savings compaction does NOT clear overflow"
    true
    tr3.updated_conditions.context_overflow
;;
...
let test_noop_then_real_savings_clears () =
  ...
  in
  check bool "after noop, overflow still set" true tr3.updated_conditions.context_overflow;
  let tr4 = apply_ok tr3.new_phase tr3.updated_conditions (overflow_event ()) in
  let tr5 = apply_ok tr4.new_phase tr4.updated_conditions SM.Auto_compact_triggered in
  let tr6 =
    apply_ok
      SM.Compacting
      tr5.updated_conditions
      (SM.Compaction_completed { before_tokens = 180_000; after_tokens = 60_000 })
  in
  check_phase SM.Running tr6.new_phase "real savings → Running";
  check
    bool
    "real savings clear context_overflow"
    false
    tr6.updated_conditions.context_overflow;
  check
    bool
    "real savings clear compact_retry_exhausted"
    false
    tr6.updated_conditions.compact_retry_exhausted
;;
...
let test_heartbeat_failure_preserved_through_overflow () =
  ...
  check_phase SM.Overflowed tr2.new_phase "Overflowed outranks heartbeat failure";
  check bool "heartbeat unhealthy latched" false tr2.updated_conditions.heartbeat_healthy;
  (* Auto-compact finishes → overflow cleared.  Heartbeat failure now
     surfaces as Failing.  This confirms no event is swallowed. *)
  let tr3 = apply_ok SM.Overflowed tr2.updated_conditions SM.Auto_compact_triggered in
  check_phase SM.Compacting tr3.new_phase "compact starts";
  let tr4 =
    apply_ok
      SM.Compacting
      tr3.updated_conditions
...
let test_noop_compaction_keeps_overflow () =
  ...
  check
    bool
    "noop compaction keeps context_overflow set"
    true
    tr3.updated_conditions.context_overflow;
  check bool "compaction_active cleared" false tr3.updated_conditions.compaction_active
;;

let () =
  run
    "keeper_overflow_recovery"
    [ ( "overflow-lifecycle"
      , [ test_case "happy path" `Quick test_happy_path
        ; test_case
            "compact failure latches Paused"
            `Quick
            test_compact_failure_latches_paused
        ; test_case
            "operator clear returns to Running"
            `Quick
            test_operator_clear_returns_to_running
        ; test_case "two consecutive overflows" `Quick test_two_consecutive_overflows
        ; test_case
            "noop compaction keeps overflow (#9988)"
            `Quick
            test_noop_compaction_keeps_overflow
        ; test_case
            "negative-savings keeps overflow (#9988)"
            `Quick
            test_negative_savings_keeps_overflow
        ; test_case
            "noop then real savings clears (#9988)"
            `Quick
            test_noop_then_real_savings_clears
        ; test_case
            "heartbeat failure preserved through overflow"
            `Quick
