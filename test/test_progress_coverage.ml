...
let test_progress_to_jsonrpc_basic () =
  let p : Progress.progress =
    { task_id = "task-001"; progress = 0.5; message = None; estimated_remaining = None }
  in
  ...
  | `Assoc fields ->
    (match List.assoc_opt "params" fields with
     | Some (`Assoc params) ->
       check
         bool
         "has estimatedRemaining"
         true
         (List.mem_assoc "estimatedRemaining" params)
     | _ -> fail "expected params Assoc")
  | _ -> fail "expected Assoc"
;;
...
let test_progress_to_jsonrpc_method () =
  let p : Progress.progress =
    { task_id = "task-004"; progress = 1.0; message = None; estimated_remaining = None }
  in
  let json = Progress.progress_to_jsonrpc p in
  match json with
  | `Assoc fields ->
...
let test_progress_record_minimal () =
  let p : Progress.progress =
    { task_id = "minimal"; progress = 0.0; message = None; estimated_remaining = None }
  in
