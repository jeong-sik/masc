...
#### P4s~P4v 와 국면 전환 (09-06 밤 ~ 09-07 새벽)

하룻밤에 다섯 조각이 들어갔다. 순서가 그대로 방법이었다 — 이름을 붙이고, 이름이 가리킨 곳을 고치고, 다시 이름을 붙였다.

| 조각 | 무엇 | 결과 |
|---|---|---|
| P4s #33718 | 히스토리 지문 계산을 pool 로 | `append trace → disabled.json` 사라짐 |
| P4t #33739 | 128 KB 이상 JSON 파싱을 pool 로 | `schedules.json` 12회 466 ms → 0 |
| P4u #33754 | 모든 systhread 호출에 이름 (라벨 필수 인자) | 익명 `systhread` 0개 |
| P4v #33763 | SSE 재생 필터가 이미 파싱한 payload 를 읽음 | `caml_lex_engine` 55.7% → **0.8%** |

P4v 는 앞의 것들과 종류가 다르다. 옮긴 게 아니라 **필요 없는 계산을 지웠다**. delivery 는 `frame` 과 `payload` 를 둘 다 들고 있고 `frame` 은 그 `payload` 를 찍은 문자열인데, 필터가 그 문자열을 다시 쪼개고 파싱해서 옆 필드의 값을 복원하고 있었다. 버퍼의 이벤트마다, 재생하는 세션마다.

##### 남은 지연의 정체가 바뀌었다

09-07 00:35 창에서 main 스레드는 86.4% 놀고 있는데(직전 창 1%) domain 0 의 ≥100 ms 실행은 15회다. 그 실행들 안을 보면 GC 다.

| | 00:04 창 | 00:35 창 |
|---|---|---|
| 할당률 | 328 MB/s | 284 MB/s |
| 힙 | 1,605 MB | 1,963 MB |
| main 스레드 idle | 1% | 86.4% |
| 가장 긴 실행 15개 중 GC | 159 ms / 1,638 ms (10%) | **680 ms / 2,728 ms (25%)** |

`fstat → fstat` 231.8 ms 안에 GC 122.5 ms, `fs-compat-realpath sangsu` 194.0 ms 안에 GC 147.6 ms. **"fiber 위 계산을 옮긴다"는 레버는 거의 다 썼다.** 다음 레버는 옮기기가 아니라 덜 할당하기다.

단, 창마다 서버가 하는 일의 양이 크게 다르다(할당률 134 → 328 MB/s). 이 결론은 조용한 창에서 한 번 더 확인해야 한다.

##### 측정에서 배운 것

- `sample` 은 스택 프레임마다 한 줄을 찍는다. 줄 수를 더하면 꼬리재귀가 아닌 호출이 부풀려진다. `List.combine` 이 336 샘플로 1위처럼 보였는데 한 샘플이 337 프레임 깊이였다. 잎만 세는 `scratchpad/rtev/sample_leaf.py` 를 쓴다.
- `→ Mutex.lock` 으로 끝나는 실행은 락을 기다린 시간이 아니다. 추적기가 재는 것은 끊기지 않은 실행이므로, 락에 닿기 전까지 계산한 시간이다.
- 한 창의 부재를 소멸로 읽으면 안 된다. `masc-tui.json` 이 한 창에서 사라졌다가 다음 창에 18회로 돌아왔다. TUI 폴링 유무가 창마다 달랐다.

##### 남은 것

- 이슈 #33719 — 히스토리 지문을 턴마다 처음부터 다시 계산한다. pool 로 옮겼을 뿐 제곱은 그대로다.
- `event-queue-v19.json` 쓰기(501 KB 직렬화)는 fiber 에 남아 있다. 읽기만 고쳤다.
- `fs-compat-load-file masc-tui.json` 12~18회 — 242 바이트 파일이라 파싱이 아니다. HTTP 요청이 인증을 마친 지점을 표시할 뿐, 그 뒤 계산의 정체는 아직 모른다.
