...
let finding_of_yojson (json : Yojson.Safe.t) : (finding, string) result =
  try
    ...
    Ok
      { id = str "id"
      ; loop_id =
          (match str_opt "loop_id" with
           | Some s -> s
           | None -> "")
      ; keeper_name =
          (match str_opt "keeper_name" with
           | Some s -> s
           | None -> "unknown")
      ; goal = str "goal"
      ...
      ; confidence =
          confidence_of_string
            (match str_opt "confidence" with
             | Some s -> s
             | None -> "medium")
      ; tags =
          (match member "tags" json with
           ...
           | _ -> [])
      ; related_findings =
          (match member "related_findings" json with
           | `List l ->
             List.filter_map
               (fun v ->
                  match v with
           ...
           | _ -> [])
      ; cycle_range =
          (match member "cycle_range" json with
...
let findings_file ~base_path = Filename.concat (findings_dir ~base_path) "findings.jsonl"
let ensure_findings_dir ~base_path = Fs_compat.mkdir_p (findings_dir ~base_path)
...
let generate_finding_id () = Random_id.prefixed ~prefix:"fn-" ~bytes:6
...
let record_finding ~base_path ~(finding : finding) : Yojson.Safe.t =
  ...
  let graphql_ok =
    match sync_to_graphql finding with
    | Ok _ -> true
    | Error _ -> false
  in
  `Assoc
    [ "ok", `Bool true; "id", `String finding.id; "graphql_synced", `Bool graphql_ok ]
;;
