...
- 잠재 회의: F-9.1 fix가 *모든* caller에서 발생할 수 있는 *misuse pattern*을 강제하는 것 — 일부 legitimate caller path가 *spec보다 약하게* dispatch 했을 수도 (성능, 단순성). enforce 후 *test fail 회피*를 위한 caller 변경 runtime 가능.
