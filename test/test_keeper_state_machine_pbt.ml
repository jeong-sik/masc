...
let gen_event_chaos : SM.event QCheck.Gen.t =
  ...
  oneof
    ...
    ; (let* ratio = float_range 0.0 1.0 in
       ...
       return
         (SM.Context_measured
            { context_ratio = ratio; message_count = 50; token_count = 5000; auto_rules }))
  ...
  let base =
    ...
    | SM.Running ->
      ...
      ; SM.Context_overflow_detected
          { source = `Prompt_rejected; token_count = 205000; limit_tokens = Some 200000 }
      ]
    | SM.Failing ->
      [ SM.Heartbeat_ok
      ; SM.Turn_succeeded
    ...
    | SM.Draining -> [ SM.Drain_complete; SM.Fiber_terminated { outcome = "crash" } ]
    | SM.Paused ->
      [ SM.Operator_resume
      ; SM.Operator_compact_requested
      ; SM.Operator_clear_requested { preserve_system = true; reason = "pbt" }
    ...
    | SM.Crashed ->
      [ SM.Supervisor_restart_attempt { attempt = 1 }; SM.Restart_budget_exhausted ]
    | SM.Restarting ->
      [ SM.Fiber_started
    ...
    | SM.Running | SM.Failing ->
      [ SM.Context_measured
          ...
          ; auto_rules =
              ...
              ; handoff = false
              ; guardrail_stop = true
              ; guardrail_reason = Some "high_ratio"
              ; goal_drift = 0.8
              }
          }
      ]
...
let gen_event_guided (phase : SM.phase) (c : SM.conditions) : SM.event QCheck.Gen.t =
  ...
  | candidates -> QCheck.Gen.oneof (List.map QCheck.Gen.return candidates)
;;
...
let apply_event_sim (state : sim_state) (event : SM.event) : sim_state =
  ...
  | Ok tr ->
    ...
    in
    { phase = tr.new_phase; conditions = tr.updated_conditions; restart_count }
  | Error _ -> state (* rejected event: no state change *)
;;
...
let check_invariants_sequence events =
  let rec loop prev_state = function
    ...
    | event :: rest ->
      ...
      in
      if violations <> [] then violations else loop new_state rest
  in
  loop init_state events
;;

let show_events events = String.concat "; " (List.map SM.event_to_string events)
    ...
    (fun seq_len ->
       let rec build_events state n acc =
         ...
         then List.rev acc
           ...
           let event =
             QCheck.Gen.generate1 (gen_event_guided state.phase state.conditions)
           in
...
let () =
  ...
  run
    "keeper_state_machine_pbt"
    [ "chaos", [ QCheck_alcotest.to_alcotest test_chaos_invariants ]
    ; "guided", [ QCheck_alcotest.to_alcotest test_guided_invariants ]
    ]
;;
