...
(** Create an isolated worktree for the experiment.
    Returns [Some path] only when git exits with status 0. *)
  ...
  | Some hypothesis ->
    ...
    (match create_worktree ~repo_path:config.repo.path ~experiment_id with
     ...
     | Some worktree_path ->
       ...
       in
       (* 5. Auto-PR for kept experiments — each step checks exit status
            before proceeding. A failed step aborts the chain. *)
       (if entry.metric.status = Research_metric.Keep then begin
         ...
         (try
           ...
           if run_ok ~label:"git add" ~timeout_sec:10.0
                [ "git"; "-C"; worktree_path; "add"; "-A" ]
              && run_ok ~label:"git commit" ~timeout_sec:10.0
                [ "git"; "-C"; worktree_path; "commit"; "-m"; msg ]
              && run_ok ~label:"git push" ~timeout_sec:30.0
           ...
           then begin
             let pr_body = Printf.sprintf
               ...
               entry.metric.loc_delta
             in
             if run_ok ~label:"gh pr create" ~timeout_sec:30.0
                  [ "gh"; "pr"; "create"; "--draft";
             ...
             then
               ...
               Log.Server.warn "research: PR creation failed for %s" experiment_id
           end
         with exn ->
