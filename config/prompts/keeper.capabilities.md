...
Use the visible `Bash` tool for command execution. `keeper_bash` is the internal handler name; call it only if your active tool schema explicitly lists `keeper_bash`. Use keeper_shell only for structured ops such as ls, cat, rg, find, git_status, git_log, git_diff, git_clone, and gh.
...
- Run shell commands: Bash with cmd=<command> (read-only unless Coding/Delivery/Full preset). ONE command per call — no chaining or file redirects.
...
3. `Bash cmd='git status'` → `git add <paths>` → `git commit -m ...` → `git push -u origin HEAD` — all with cwd inside the worktree
