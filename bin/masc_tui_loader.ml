...
let report path err = Printf.eprintf "[masc-tui] decode failed for %s: %s\n%!" path err

(** Load keepers from .masc/keepers/ *)
let load_keepers (base_path : string) : keeper list =
  let keepers_dir =
    Filename.concat (Filename.concat base_path Common.masc_dirname) "keepers"
  in
  ...
  then
    ...
    |> Array.to_list
    |> List.filter (fun f ->
      Filename.check_suffix f ".json" && not (String.contains f '.'))
      (* This won't work, use different filter *)
    |> (fun _ ->
...
let find_metrics_files (base_path : string) (keeper_name : string) : string list =
  let metrics_dir =
    Filename.concat
      (Filename.concat (Filename.concat base_path Common.masc_dirname) "keepers")
      (Filename.concat keeper_name "metrics")
  in
  if not (Sys.file_exists metrics_dir && Sys.is_directory metrics_dir)
  then []
  else (
...
let load_keeper_logs (base_path : string) (keeper_name : string) (max_entries : int)
  : log_entry list
  =
  ...
  let len = List.length all in
  if len <= max_entries then all else List.filteri (fun i _ -> i >= len - max_entries) all
;;
...
let load_from_masc_dir (state : state) (base_path : string) =
  ...
  state.tasks
  <- (if Sys.file_exists tasks_dir && Sys.is_directory tasks_dir
      then
  ...
  then state.keeper_cursor <- max 0 (List.length state.keepers - 1);
