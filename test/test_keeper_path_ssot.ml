...
let assert_ssot ~name ~sandbox =
  ...
  Alcotest.(check string)
    (Printf.sprintf
       ...
       name
       (Masc_mcp.Keeper_types_profile.sandbox_profile_to_string sandbox))
    constructed host_abs

let test_ssot_docker_keeper () =
  assert_ssot ~name:"sangsu" ~sandbox:Masc_mcp.Keeper_types_profile.Docker

let test_ssot_local_keeper () =
  assert_ssot ~name:"analyst" ~sandbox:Masc_mcp.Keeper_types_profile.Local

let test_ssot_docker_keeper_with_dashed_name () =
  assert_ssot ~name:"masc-improver" ~sandbox:Masc_mcp.Keeper_types_profile.Docker
...
let test_root_depends_on_keeper_name () =
  ...
  let config = make_config () in
  let m1 = make_meta ~name:"masc-improver" ~sandbox:Masc_mcp.Keeper_types_profile.Docker in
  let m2 = make_meta ~name:"analyst" ~sandbox:Masc_mcp.Keeper_types_profile.Docker in
...
let test_root_depends_on_sandbox_profile () =
  ...
  let config = make_config () in
  let m_docker =
    make_meta ~name:"sangsu" ~sandbox:Masc_mcp.Keeper_types_profile.Docker
  in
  let m_local =
    make_meta ~name:"sangsu" ~sandbox:Masc_mcp.Keeper_types_profile.Local
  in
...
let test_ssot_idempotent () =
  let config = make_config () in
  let meta = make_meta ~name:"scholar" ~sandbox:Masc_mcp.Keeper_types_profile.Docker in
...
let test_egress_path_docker () =
  assert_egress_path_ssot ~name:"sangsu" ~sandbox:Masc_mcp.Keeper_types_profile.Docker

let test_egress_path_local () =
  assert_egress_path_ssot ~name:"ramarama" ~sandbox:Masc_mcp.Keeper_types_profile.Local

let test_egress_path_dashed () =
  assert_egress_path_ssot ~name:"masc-improver" ~sandbox:Masc_mcp.Keeper_types_profile.Docker

let test_egress_path_distinct_per_keeper () =
  let config = make_config () in
  let m1 = make_meta ~name:"executor" ~sandbox:Masc_mcp.Keeper_types_profile.Docker in
  let m2 = make_meta ~name:"analyst" ~sandbox:Masc_mcp.Keeper_types_profile.Docker in
...
let test_egress_path_distinct_per_profile () =
  let config = make_config () in
  let m_docker = make_meta ~name:"executor" ~sandbox:Masc_mcp.Keeper_types_profile.Docker in
  let m_local = make_meta ~name:"executor" ~sandbox:Masc_mcp.Keeper_types_profile.Local in
  let p_docker =
