...
Date: 2026-04-04 (version references updated 2026-04-09)
OAS Version: 0.118.0 floor (CI/runtime pin managed by `scripts/oas-agent-sdk-pin.sh`)
...
`masc-mcp` keeps the runtime pin ratcheted against upstream `main`, while the dependency floor tracks the pinned SDK's declared `agent_sdk.opam` version. The authoritative source for the current pin is `scripts/oas-agent-sdk-pin.sh` and `dune-project`.
