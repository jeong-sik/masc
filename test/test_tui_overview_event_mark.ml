...
(* The level was recorded on every Overview event and drawn on none, so a
...
let test_only_an_error_carries_a_mark () =
  check (option string) "an error is marked" (Some "\xe2\x9c\x97")
    (Types.overview_event_mark (event "error"));
  List.iter
    (fun level ->
      check (option string) (level ^ " is not marked") None
        (Types.overview_event_mark (event level)))
