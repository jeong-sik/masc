...
let live_override_details (meta : keeper_meta) (defaults : keeper_profile_defaults)
  ...
  |> maybe_string_list_override
       ...
       meta.tool_denylist
  |> (fun acc ->
  if effective_cascade_name <> meta.cascade_name
  then
    override_field
      "model.cascade_name"
...
let blocker_class_of_sdk_error (err : Oas.Error.sdk_error) : blocker_class option =
  ...
  | Some (Oas_worker_named.No_tool_capable_provider _) -> Some No_tool_capable_provider
  | Some (Oas_worker_named.Accept_rejected _) -> None
  | Some (Oas_worker_named.Admission_queue_timeout _) -> Some Admission_queue_wait_timeout
  | Some (Oas_worker_named.Admission_queue_rejected _) -> None
  | Some (Oas_worker_named.Oas_timeout_budget _) -> Some Oas_timeout_budget
  | Some (Oas_worker_named.Turn_timeout _) -> Some Turn_timeout
...
let runtime_blocker_surface_of_legacy_string reason cls =
  ...
  | _ -> runtime_blocker_surface_of_typed_class ~summary:reason cls
;;
...
let runtime_blocker_surface_opt (config : Coord_utils.config) (meta : keeper_meta) =
  let derived =
    ...
    | Some cls ->
      Some (runtime_blocker_surface_of_typed_class ~summary:meta.runtime.last_blocker cls)
    | None ->
  ...
  let derived =
    ...
    | None ->
      (match blocker_class_of_string meta.runtime.last_blocker with
       | Some cls ->
         Some (runtime_blocker_surface_of_legacy_string meta.runtime.last_blocker cls)
