...
let report_of_yojson ?fallback_agent (json : Yojson.Safe.t) : (report, string) result =
  match json with
  | `Assoc _ ->
    (match required_member json "tool_name", required_member json "message" with
     | Ok tool_name, Ok message ->
       let client_name =
         ...
         | Some value -> value
         | None ->
           Option.value ~default:"tool-host" (Option.bind fallback_agent trim_to_option)
       in
       let explicit_agent =
       ...
       in
       let transport =
         Option.value ~default:"mcp_http" (stringish_member_opt json "transport")
       in
       Ok
         { agent_name
         ; client_name
...
let details_json (report : report) =
  let envelope =
    Failure_envelope.tool_host_failure
...
let record ?fs config (report : report) =
  ...
  Log.client_tool_host_error
    ~module_name:Failure_envelope.tool_host_log_module_name
    ~details
    (ring_message report);
  Audit_log.log_client_tool_host_failure
    config
    ...
    ?timeout_ms:report.timeout_ms
    ();
  if Option.is_some fs
  then
    Telemetry_eio.track_error
      ?fs
      config
      ~code:"client_tool_host_failure"
      ~message:report.message
      ~context:
        (Printf.sprintf
           "client=%s tool=%s transport=%s"
           report.client_name
           report.tool_name
           report.transport)
;;
...
let record_assignment ?fs config (snapshot : assignment_snapshot) =
  Telemetry_eio.track_tool_assigned
    ?fs
    config
