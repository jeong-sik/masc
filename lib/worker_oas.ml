...
let default_internal_tool_retry_policy = Oas.Tool_retry_policy.default_internal
...
let build_agent
      ...
      ?context_injector
      ?context
      ?(approval : Oas.Hooks.approval_callback = Approval_callbacks.reject_by_default)
      ()
  : (Oas.Agent.t, string) result
  =
  let config = agent_config_of_worker_meta meta ~system_prompt in
  let tool_names = List.map (fun (tool : Oas.Tool.t) -> tool.schema.name) tools in
  let guardrails =
  ...
  let builder =
    ...
    | Some n -> Oas.Builder.with_max_tokens n b
    | None -> b)
  ...
  let builder =
    ...
    | None -> builder
  in
  Oas.Builder.build_safe builder |> Result.map_error Oas.Error.to_string
;;
              ...
              (function
                | Oas.Hooks.BeforeTurnParams { current_params; _ } ->
                  (match Masc_context_injector.render_temporal_summary ctx with
                   ...
                   | Some summary ->
                     let ctx_str =
                       match current_params.Oas.Hooks.extra_system_context with
                       | None -> summary
                       | Some prev -> prev ^ "\n\n" ^ summary
                     in
  ...
  Fun.protect
    ...
    (fun () ->
       let result, proof =
         ...
         | None -> Oas.Agent.run ~sw agent prompt, None
       in
