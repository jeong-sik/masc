...
let board_updated_at (stimulus : Keeper_event_queue.stimulus) =
  ...
  | Keeper_event_queue.Board_attention { signal = board; _ } -> board.updated_at
...
type keeper_summary =
  ...
  ; read_error : Keeper_reaction_store.error option
  }
...
let keeper_summary_status summary =
  match summary.read_error with
  | Some _ -> "unknown"
  | None when summary.row_count = 0 -> "empty"
  | None
    when summary.pending_stimulus_count = 0
         && summary.orphan_reaction_stimulus_count = 0
         && summary.escalated_stimulus_count = 0
         && summary.external_input_requested_stimulus_count = 0 -> "ok"
  | None -> "degraded"
;;

let keeper_summary_to_json summary =
  ...
  `Assoc
    ...
    ; ( "operator_action_required"
      , `Bool
          (pending_count > 0
           ...
           || Option.is_some summary.read_error) )
    ; "counts_complete", `Bool (Option.is_none summary.read_error)
    ...
    ; ( "read_error"
      , match summary.read_error with
        | None -> `Null
        | Some error -> `String (Keeper_reaction_store.error_to_string error) )
    ]
;;
...
let summary_record_for_keeper ~base_path ~keeper_name ~pending_id_display_limit =
  ...
  with
  | Ok exact -> keeper_summary_of_exact ~keeper_name ~limit:pending_id_display_limit exact
...
let unavailable_fleet_summary_json () =
  `Assoc
    ...
    ; "orphan_reaction_stimulus_count", `Int 0
    ; "in_progress_stimulus_count", `Int 0
    ; "acked_stimulus_count", `Int 0
    ; "escalated_stimulus_count", `Int 0
    ; "external_input_requested_stimulus_count", `Int 0
    ; "pending_stimulus_count", `Int 0
    ; "reaction_store_discovered_keeper_count", `Int 0
...
let fleet_summary_json
  ...
  let summaries =
    List.map
      ...
      keeper_names
  in
  let sum field = List.fold_left (fun total summary -> total + field summary) 0 summaries in
  ...
  let read_error_count =
    List.fold_left
      (fun total summary -> total + if Option.is_some summary.read_error then 1 else 0)
      0
      summaries
  in
  ...
  `Assoc
    ...
    ; "stimulus_count", `Int (sum (fun summary -> summary.stimulus_count))
    ; "reaction_count", `Int (sum (fun summary -> summary.reaction_count))
    ; "turn_started_count", `Int (sum (fun summary -> summary.turn_started_count))
    ; "event_queue_ack_count", `Int (sum (fun summary -> summary.event_queue_ack_count))
    ; "event_queue_requeue_count", `Int (sum (fun summary -> summary.event_queue_requeue_count))
    ; "event_queue_escalation_count", `Int (sum (fun summary -> summary.event_queue_escalation_count))
    ; "event_queue_external_input_count", `Int (sum (fun summary -> summary.event_queue_external_input_count))
    ; "cursor_ack_count", `Int (sum (fun summary -> summary.cursor_ack_count))
    ; "cursor_swept_stimulus_count", `Int (sum (fun summary -> summary.cursor_swept_stimulus_count))
    ; "orphan_reaction_stimulus_count", `Int orphan_count
    ; "in_progress_stimulus_count", `Int (sum (fun summary -> summary.in_progress_stimulus_count))
    ; "acked_stimulus_count", `Int (sum (fun summary -> summary.acked_stimulus_count))
    ; "escalated_stimulus_count", `Int escalated_count
    ; "external_input_requested_stimulus_count", `Int external_input_count
    ; "pending_stimulus_count", `Int pending_count
    ; ( "reaction_store_discovered_keeper_count"
      , `Int (List.length reaction_discovery.keeper_names) )
