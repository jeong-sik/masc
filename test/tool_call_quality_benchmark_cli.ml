...
let summary_rows_json view (summary : Tool_call_quality_benchmark.benchmark_summary) =
  let rows =
    ...
    | Tool_call_quality_benchmark.By_provider_model -> summary.grouped_by_provider_model
    | Tool_call_quality_benchmark.By_keeper_profile -> summary.grouped_by_keeper_profile
  in
...
let () =
  ...
  let cases_path = ref (Tool_call_quality_benchmark.default_case_set_path ~repo_root) in
  let evidence_path =
  ...
  let specs =
    ...
    ; ( "--artifact-dir"
      ...
      , "Write summary artifacts to this directory" )
    ; ( "--emit-canary"
      , Arg.Set_string emit_canary_path
      , "Write keeper benchmark canary recommendation JSON to this path" )
    ; ( "--view"
      , Arg.Set_string view
      , "Summary view: provider-model-keeper, provider-model, keeper" )
    ; ( "--models"
  ...
  in
  let summary =
    Tool_call_quality_benchmark.summarize
      ~cases
      ~runs
      ~model_filters:!model_filters
      ~keeper_filters:!keeper_filters
      ()
  in
  let canary_manifest = Keeper_benchmark_canary.build_manifest summary in
  let view =
  ...
  let output =
    ...
    | "csv" -> Tool_call_quality_benchmark.summary_rows_to_csv ~view summary
    | other -> failwith ("unknown --format: " ^ other)
  in
  if String.trim !artifact_dir <> ""
  then (
    let ext =
      match String.lowercase_ascii (String.trim !format) with
      | "csv" -> "summary.csv"
