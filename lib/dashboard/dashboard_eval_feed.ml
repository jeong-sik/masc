...
let read_layer_result_json (json : Yojson.Safe.t) : (layer_result_json, string) result =
  let layer_name = Safe_ops.json_string ~default:"" "layer_name" json in
  if layer_name = ""
  then Error "layer_result: missing layer_name"
  else (
...
let read_verdict_json (json : Yojson.Safe.t) : (swiss_verdict_json, string) result =
  let schema_version = Safe_ops.json_int ~default:0 "schema_version" json in
  if schema_version <> 1
  then Error (Printf.sprintf "unsupported schema_version: %d (expected 1)" schema_version)
  else (
    ...
    | Ok layer_results -> Ok { schema_version; all_passed; coverage; layer_results }
    | Error _ as e -> e)
;;

(* ── Eval snapshot parsing ───────────────────────────────────────── *)

let read_snapshot_json ~agent_name (json : Yojson.Safe.t) : eval_snapshot option =
  let worker_run_id = Safe_ops.json_string ~default:"" "worker_run_id" json in
  if worker_run_id = ""
  then None
  else (
    ...
    | Some verdict_json ->
      (match read_verdict_json verdict_json with
       ...
       | Ok verdict ->
         ...
         let timestamp = Safe_ops.json_float ~default:0.0 "timestamp" json in
         let baseline_status = Safe_ops.json_string_opt "baseline_status" json in
         Some
           { agent_name; session_id; worker_run_id; timestamp; verdict; baseline_status }))
;;

(* ── File system reading ─────────────────────────────────────────── *)

let eval_base ~base_path = Filename.concat (Filename.concat base_path ".oas") "eval"
let eval_dir ~base_path ~agent_name = Filename.concat (eval_base ~base_path) agent_name
...
let verdict_to_json (v : swiss_verdict_json) : Yojson.Safe.t =
  `Assoc
    ...
    ; "layer_results", `List (List.map layer_result_to_json v.layer_results)
    ]
;;
