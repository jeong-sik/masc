...
let json_ok fields = Yojson.Safe.to_string (`Assoc (("status", `String "ok") :: fields))
...
let parse_int_opt value = int_of_string_opt (String.trim value)
...
let parse_pid_and_command line =
  ...
  then None, ""
    ...
    | Some idx ->
      ...
      let command =
        String.sub trimmed (idx + 1) (String.length trimmed - idx - 1) |> String.trim
      in
      pid, command)
;;
...
let find_flag_value tokens flag =
  let rec loop = function
    | [] | [ _ ] -> None
    | key :: value :: rest ->
      if String.equal key flag then Some value else loop (value :: rest)
  in
  loop tokens
;;
...
let server_port_of_url url =
  ...
  | None -> None
  | Some idx ->
    let port = String.sub trimmed (idx + 1) (String.length trimmed - idx - 1) in
    parse_int_opt port
;;
  ...
  | Unix.WEXITED 0 ->
    let processes =
      ...
      |> List.filter_map (fun line ->
        ...
        if command = "" || not (string_contains_substring command "llama-server")
        then None
        else (
          ...
          if
            not
              (List.exists
                 (fun token -> String.ends_with ~suffix:"llama-server" token)
                 tokens)
          then None
          else
            Some
              { pid
              ; command
              ; port = Option.bind (find_flag_value tokens "--port") parse_int_opt
              ; host = find_flag_value tokens "--host"
              ; alias = find_flag_value tokens "--alias"
              ; model_path = find_flag_value tokens "-m"
              ; ctx_size = Option.bind (find_flag_value tokens "-c") parse_int_opt
              ; batch_size =
                  Option.bind (find_flag_value tokens "--batch-size") parse_int_opt
              ; ubatch_size =
                  Option.bind (find_flag_value tokens "--ubatch-size") parse_int_opt
              ; slots_enabled = has_flag tokens "--slots"
              }))
    in
    Ok processes
  | Unix.WEXITED code -> Error (Printf.sprintf "ps failed with exit code %d" code)
  | Unix.WSIGNALED sig_num -> Error (Printf.sprintf "ps killed by signal %d" sig_num)
  | Unix.WSTOPPED sig_num -> Error (Printf.sprintf "ps stopped by signal %d" sig_num)
;;

let fetch_models_at base_url =
  let url = String.trim base_url ^ Masc_network_defaults.openai_models_path in
  ...
  | Unix.WEXITED 0 ->
    (try
       ...
       let models =
         ...
         | `List items ->
           items |> List.filter_map (fun item -> item |> member "id" |> to_string_option)
         | _ -> []
       in
       Ok (url, models)
     with
     | Yojson.Json_error msg -> Error ("invalid llama models response: " ^ msg))
  | Unix.WEXITED code ->
    Error (Printf.sprintf "llama models request failed with exit code %d" code)
