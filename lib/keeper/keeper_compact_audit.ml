...
let store_base_dir base_path = Filename.concat base_path "data/harness-compact"
let legacy_base_dir base_path = Filename.concat base_path "data/harness-pre-compact"
...
let prune_best_effort base_path ~retention_days =
  match Dated_jsonl.prune (get_store base_path) ~days:retention_days with
  | _n -> ()
  | exception e ->
    Printf.eprintf
...
let persist_complete ~base_path ~retention_days (r : complete_record)
  ...
  =
  let json =
    try Ok (complete_to_json r) with
    | e -> Error (Serialize_failure (Printexc.to_string e))
  in
  match json with
  | Error _ as e -> e
...
let read_events ~base_path ~since ~until ?keeper () : (row list, write_error) result =
  ...
  let read_dir base_dir =
    ...
    Dated_jsonl.read_range store ~since:(iso_of_unix since) ~until:(iso_of_unix until)
  in
  try
    ...
    let sort_ts = function
      | Start r -> r.ts_unix
      | Complete r -> r.ts_unix
    in
...
module Pending = struct
  ...
  let take ~keeper_name =
    Eio.Mutex.use_rw ~protect:true mu (fun () ->
      ...
      (match r with
       | Some _ -> Hashtbl.remove tbl keeper_name
       | None -> ());
      r)
  ;;
end
...
let handle_event ~base_path ~retention_days (evt : Oas.Event_bus.event) : unit =
  ...
  | Oas.Event_bus.ContextCompacted { agent_name; before_tokens; after_tokens; phase } ->
    let compaction_id =
...
let compaction_filter : Oas.Event_bus.filter =
  fun evt ->
  match evt.payload with
  | Oas.Event_bus.ContextCompactStarted _ | Oas.Event_bus.ContextCompacted _ -> true
  | _ -> false
;;
...
let spawn_subscriber
  ...
  let effective_retention = resolve_retention_days ~default:retention_days in
  let sub =
    Oas_bus_instrument.subscribe ~purpose:"compact_audit" ~filter:compaction_filter bus
  in
  Eio.Switch.on_release sw (fun () -> Oas_bus_instrument.unsubscribe bus sub);
  Eio.Fiber.fork ~sw (fun () ->
    let rec loop () =
      let batch = Oas_bus_instrument.drain sub in
      List.iter (handle_event ~base_path ~retention_days:effective_retention) batch;
