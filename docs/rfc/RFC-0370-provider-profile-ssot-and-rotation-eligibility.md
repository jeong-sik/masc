...
### 3.2 P2: 죽은 카탈로그 행 정정 + 총량 개념 비확산 (구현 대상)

**철회 (2026-08-11)**: 초안은 "official-client 행에 `turn-timeout-s`
선언"을 처방했다. 이는 오류다 — 총량 턴 예산은 #24386이 의도적으로
집행에서 제거한 개념이고, 행마다 선언하는 것은 그 개념을 카탈로그에
새겨 확산시키는 일이다 (워크어라운드 누적 메커니즘 그 자체). 총량
축의 올바른 경로는 §5의 순서 제약 그대로: idle 감지 무장 → 발화 증명
→ 하드코딩 300.0 제거. 그때까지 300.0은 문서화된 backstop으로만
남는다. 운영자가 이미 선언한 effort 상위 티어 5행은 운영 판단으로
존중하되, 신규 선언은 하지 않는다.

유지되는 P2 작업:

1. **서버 부재 모델 행 정정**: 라이브 카탈로그 11개 ollama_cloud 행 중
   3행의 `api-name`이 서버에 존재하지 않았다 (`deepseek-v4-flash` →
   `:0731`만 존재, `gemma4:31b-cloud` → `gemma4:31b`, `qwen3.5:cloud` →
   `qwen3.5:397b`). 이 행들은 선택되는 순간 무조건 실패한다. 서버
   `/v1/models` 목록과의 대조를 반복 가능한 절차로 유지한다.
2. `keep-alive`/`num-ctx`/`price-*`/`effort`/`agent`/`is-default`는
   소비자 실사용과 효과를 확인하기 전에는 선언하지 않는다 — 소비자가
   없으면 선언이 아니라 키 제거가 옳다.
