...
**모드별 개별 검증**: buggy spec을 하나로 합치면 두 액션 중 하나가 unreachable해지거나 안전해져도 나머지가 Init에서 한 걸음에 위반을 만들어 "invariant violated"가 그대로 나온다 — 기대-위반 검사가 통과하면서 실제로는 한 모드만 덮게 된다. 따라서 `SpecBuggyQuarantine`/`SpecBuggyOrphan`으로 나누고 cfg도 `-buggy.cfg`(A)·`-orphan.cfg`(B)로 분리해 각 모드를 단독 검증한다. clean cfg = "No error has been found", 두 buggy cfg 각각 = "Invariant NoDurableRowLostOnBump is violated". 셋 모두 기대대로 나와야 spec 유효.
