...
let test_constraints_of_json_partial () =
  ...
  check
    int
    "hard_max_iterations default"
    Bounded.default_constraints.hard_max_iterations
    c.hard_max_iterations
;;
...
let test_goal_of_json_eq () =
  ...
  | Bounded.Eq v -> check string "eq value" "\"done\"" (Yojson.Safe.to_string v)
...
let test_goal_of_json_gte () =
  ...
  match goal.condition with
  | Bounded.Gte v -> check (float 0.001) "gte value" 0.95 v
...
let test_check_goal_nested_path () =
  let goal =
    { Bounded.path = "$.result.data.done"; condition = Bounded.Eq (`Bool true) }
  in
...
let test_bounded_run_empty_agents () =
  ...
  let result =
    Bounded.bounded_run ~constraints ~goal ~agents:[] ~prompt:"test" ~spawn_fn
  in
  check bool "should fail with empty agents" true (result.status = `Error);
  check bool "reason mentions agents" true (String.sub result.reason 0 2 = "No")
;;
...
let test_bounded_run_constraint_exceeded () =
  let constraints =
    { Bounded.default_constraints with max_turns = Some 2; hard_max_iterations = 10 }
  in
  let goal = { Bounded.path = "$.done"; condition = Bounded.Eq (`Bool true) } in
...
let test_bounded_run_hard_limit () =
  let constraints =
    { Bounded.default_constraints with
      max_turns = None
    ; max_tokens = None
    ; max_cost_usd = None
  ...
  check
    bool
    "reason mentions hard"
    true
    (String.lowercase_ascii result.reason
     |> fun s -> String.length s > 0 && String.sub s 0 4 = "hard")
;;
...
let test_bounded_run_token_tracking () =
  ...
  let spawn_fn _ _ =
    incr call_count;
    if !call_count >= 2
    then mock_spawn_result ~output:{|{"done": true}|} ()
    else mock_spawn_result ~output:{|{"done": false}|} ()
  in
  let result =
    Bounded.bounded_run ~constraints ~goal ~agents:[ "test" ] ~prompt:"test" ~spawn_fn
  in
  check int "should take 2 turns" 2 result.stats.turns;
  check int "tokens_in tracked" 100 result.stats.tokens_in;
  (* 50 * 2 *)
...
let test_bounded_run_round_robin () =
  ...
  in
  let _ =
    Bounded.bounded_run ~constraints ~goal ~agents:[ "a"; "b" ] ~prompt:"test" ~spawn_fn
  in
...
let test_bounded_run_spawn_exception () =
  ...
  check
    bool
    "reason includes execution failure wording"
    true
    (try
       let _ =
         Str.search_forward
  ...
  check
    bool
    "reason includes agent"
    true
    (try
       let _ = Str.search_forward (Str.regexp_string "Agent 'test'") result.reason 0 in
       true
     with
     | Not_found -> false);
  check
    bool
    "reason mentions error"
    true
    (String.lowercase_ascii result.reason
     |> fun s ->
     try
       let _ = Str.search_forward (Str.regexp "error\\|fail") s 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_bounded_run_failure_reason_names_agent_and_turn () =
  ...
  in
  let result =
    Bounded.bounded_run ~constraints ~goal ~agents:[ "bogus" ] ~prompt:"test" ~spawn_fn
  in
  check bool "should be error" true (result.status = `Error);
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp_string "Agent 'bogus'") result.reason 0 in
       true
     with
     | Not_found -> false);
  check
    bool
    "reason includes turn number"
    true
    (try
       let _ =
         Str.search_forward (Str.regexp_string "before completing turn 1") result.reason 0
       in
       true
     with
     | Not_found -> false)
...
let test_is_retryable_error () =
  ...
  check
    ...
    true
    (Bounded.is_retryable_error "Rate limit exceeded 429");
  check
    bool
    "503 is retryable"
    true
    (Bounded.is_retryable_error "Service unavailable 503");
  check
    bool
    "connection refused is retryable"
    true
    (Bounded.is_retryable_error "ECONNREFUSED");
  (* Non-retryable errors *)
  check
    bool
    "generic error not retryable"
    false
    (Bounded.is_retryable_error "Invalid JSON");
  check
    bool
    "auth error not retryable"
    false
    (Bounded.is_retryable_error "Unauthorized 401")
;;
...
let test_retry_success_after_failures () =
  ...
  let retry_cfg =
    { Bounded.default_retry_config with max_retries = 3; base_delay_ms = 10 }
  in
  let constraints =
    { Bounded.default_constraints with max_turns = Some 5; retry = retry_cfg }
  in
  let goal = { Bounded.path = "$.done"; condition = Bounded.Eq (`Bool true) } in
  let call_count = ref 0 in
  let spawn_fn _ _ =
    incr call_count;
    if !call_count <= 2
    then mock_spawn_result ~success:false ~output:"Connection timeout" ()
    else mock_spawn_result ~success:true ~output:{|{"done": true}|} ()
  in
...
let test_retry_exhausted () =
  let retry_cfg =
    { Bounded.default_retry_config with max_retries = 2; base_delay_ms = 10 }
  in
  let constraints =
    { Bounded.default_constraints with max_turns = Some 5; retry = retry_cfg }
  in
  let goal = { Bounded.path = "$.done"; condition = Bounded.Eq (`Bool true) } in
  let spawn_fn _ _ = mock_spawn_result ~success:false ~output:"Connection timeout" () in
  let result =
    Bounded.bounded_run ~constraints ~goal ~agents:[ "test" ] ~prompt:"test" ~spawn_fn
  in
  ...
  check
    bool
    "reason mentions attempts"
    true
    (try
       let _ = Str.search_forward (Str.regexp "3 attempts") result.reason 0 in
       true
     with
     | Not_found -> false)
;;
...
let bounded_run_tests =
  ...
  ; ( "failure reason names agent and turn"
    , `Quick
...
let serialization_tests = [ "result to json", `Quick, test_result_to_json ]
