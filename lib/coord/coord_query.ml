...
let update_priority config ~task_id ~priority =
  ...
  with_file_lock config backlog_path (fun () ->
    try
      ...
      match task_opt with
      | None -> Printf.sprintf "❌ Task %s not found" task_id
      | Some task ->
        let old_priority = task.priority in
        let new_tasks =
          List.map
            (fun (t : task) -> if t.id = task_id then { t with priority } else t)
            backlog.tasks
        in
        let new_backlog =
          { tasks = new_tasks; last_updated = now_iso (); version = backlog.version + 1 }
        in
...
let get_tasks_safe config =
  if not (root_is_initialized config) then [] else (read_backlog config).tasks
;;

let safe_yield () = Safe_ops.protect ~default:() (fun () -> Eio.Fiber.yield ())
...
let list_leaf_json_names config dir =
  ...
  |> List.filter (fun name ->
    name <> "" && (not (String.contains name '/')) && Filename.check_suffix name ".json")
;;
...
let load_agents_from_dir config dir ~include_inactive =
  ...
  |> List.filter_map (fun name ->
    ...
    | Ok agent when include_inactive || agent.status <> Types.Inactive -> Some agent
    | Ok _ | Error _ -> None)
;;
    ...
    then acc
      ...
      let rec insert prefix = function
        ...
        | ((existing_seq, _) as existing) :: rest ->
          if seq >= existing_seq
          then List.rev_append prefix ((seq, name) :: existing :: rest)
          else insert (existing :: prefix) rest
      in
...
let collect_recent_messages config ~msgs_path ~since_seq ~limit ~warn_label =
  if not (Sys.file_exists msgs_path)
  then []
  else (
    let names =
    ...
    let rec loop remaining acc = function
      ...
      | name :: rest ->
        safe_yield ();
        if extract_seq_from_filename name <= since_seq
        then List.rev acc
        else (
      ...
      List.filter
        (fun (task : task) ->
           ...
           let is_cancelled =
             ...
             | Types.Done _ -> false
           in
           (include_done || not is_done) && (include_cancelled || not is_cancelled))
        backlog.tasks
  in
  if tasks = []
  then
    if backlog.tasks = []
    then
      "📋 No tasks. ACTION: STOP calling keeper_tasks_list — the backlog is empty. Move \
       on to other work or end your turn."
    else
      "📋 No active tasks (all done/cancelled). ACTION: STOP calling keeper_tasks_list — \
       do not re-check. Move on to other work or end your turn."
  else (
