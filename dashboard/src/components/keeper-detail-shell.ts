...
function KeeperToolPresetChip({ keeperName }: { keeperName: string }) {
  ...
  return html`
    <span class="inline-flex items-center py-0.5 px-2 rounded text-3xs font-semibold uppercase tracking-wide
      ...
      title=${`도구 프리셋: ${preset}${canPR ? ' (복제/PR 가능)' : ''}`}
    >${preset}</span>
  `
}
  ...
  return html`
    <div class="flex items-center gap-1.5">
      <select autoComplete="off"
        class="py-1 min-h-6 px-2 rounded text-2xs font-mono bg-[var(--white-5)] text-[var(--text-muted)] border border-[var(--white-8)] cursor-pointer"
  ...
  return html`
    <div class="flex min-w-0 items-start gap-4">
      ...
      <div class="flex flex-col gap-0.5">
        <div class="text-3xs font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">모니터링 / Agents / 키퍼 상세</div>
