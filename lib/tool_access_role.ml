...
let all_surface_tools () =
  Tool_permission_map.known_tool_names |> List.sort_uniq String.compare
;;
...
let tools_for_required_role required_role =
  ...
  |> List.filter (fun tool_name ->
    ...
    | None -> false
    | Some permission -> required_role_of_permission permission = required_role)
;;
...
let policy_for_role : Types.agent_role -> Tool_access_policy.t = function
  | Admin -> { Tool_access_policy.allow = All; deny = Empty }
  | Worker ->
    { allow = Diff { base = All; exclude = Names (admin_only_tools ()) }; deny = Empty }
;;
