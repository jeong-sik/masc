...
let copy_script src dst = write_executable dst (read_file src)
  ...
  with_temp_dir "run-local-script" (fun dir ->
    ...
    let code, stdout, stderr =
      run_shell
        ...
        ~unset_env:[ "MASC_BASE_PATH"; "MASC_CONFIG_DIR"; "MASC_PERSONAS_DIR" ]
        (Printf.sprintf "%s --target-dir %s --port 9955" (quote script) (quote target))
    in
    if code <> 0
    then failf "run-local failed (%d)\nstdout:\n%s\nstderr:\n%s" code stdout stderr;
    let target_abs = Unix.realpath target in
    let captured = read_file capture in
    check
      bool
      "bootstrapped cascade"
      true
      (Sys.file_exists (Filename.concat target_abs ".masc/config/cascade.json"));
    check
      bool
      "bootstrapped keepers excluded by default"
      false
      (Sys.file_exists (Filename.concat target_abs ".masc/config/keepers/sangsu.toml"));
    check
      bool
      "base path set"
      true
    ...
    check
      ...
      (contains_substring
         captured
         ("MASC_CONFIG_DIR=" ^ Filename.concat target_abs ".masc/config"));
    check
      bool
      "personas dir set"
      true
      (contains_substring
         captured
         ("MASC_PERSONAS_DIR=" ^ Filename.concat target_abs ".masc/config/personas"));
    check
      bool
  ...
  with_temp_dir "run-local-script" (fun dir ->
    ...
    let code, stdout, stderr =
      run_shell
        ...
        ~unset_env:[ "MASC_BASE_PATH"; "MASC_CONFIG_DIR"; "MASC_PERSONAS_DIR" ]
        (Printf.sprintf
           "%s --target-dir %s --port 9956 --bootstrap-only --bootstrap-keepers"
           (quote script)
           (quote target))
    in
    if code <> 0
    then
      failf
        "run-local bootstrap-keepers failed (%d)\nstdout:\n%s\nstderr:\n%s"
        code
        stdout
        stderr;
    check
      bool
      "bootstrapped keeper copied with flag"
      true
      (Sys.file_exists (Filename.concat target ".masc/config/keepers/sangsu.toml"));
    check
      bool
      "keepers included message"
      true
...
let test_print_port_is_stable_for_target_dir () =
  with_temp_dir "run-local-script" (fun dir ->
    ...
    let code1, stdout1, stderr1 =
      run_shell
        ~cwd:repo_root
        (Printf.sprintf "%s --print-port --target-dir %s" (quote script) (quote target))
    in
  ...
  with_temp_dir "run-local-script" (fun dir ->
    ...
    let code_no_flag, stdout_no_flag, stderr_no_flag =
      run_shell
        ~cwd:repo_root
        ~env:[ "FAKE_CAPTURE_FILE", capture ]
        (Printf.sprintf "%s --target-dir %s --port 9956" (quote script) (quote target))
    in
    if code_no_flag <> 0
    then
      failf
        "run-local without flag failed (%d)\nstdout:\n%s\nstderr:\n%s"
        code_no_flag
        stdout_no_flag
        stderr_no_flag;
    ...
    let code_flag, stdout_flag, stderr_flag =
      run_shell
        ...
        ~env:[ "FAKE_CAPTURE_FILE", capture ]
        (Printf.sprintf
           "%s --target-dir %s --port 9957 --build-dashboard"
           (quote script)
           (quote target))
    in
    if code_flag <> 0
...
let test_existing_target_config_is_not_overwritten () =
  with_temp_dir "run-local-script" (fun dir ->
    ...
    write_file (Filename.concat target_config "cascade.json") "{\"seed\":\"target\"}";
    let capture = Filename.concat dir "captured-env.txt" in
    let script = Filename.concat repo_root "scripts/run-local.sh" in
    let code, stdout, stderr =
      run_shell
        ~cwd:repo_root
        ~env:[ "FAKE_CAPTURE_FILE", capture ]
        (Printf.sprintf "%s --target-dir %s --port 9958" (quote script) (quote target))
    in
    if code <> 0
    then failf "run-local failed (%d)\nstdout:\n%s\nstderr:\n%s" code stdout stderr;
  ...
  with_temp_dir "run-local-script" (fun dir ->
    ...
    let code, stdout, stderr =
      run_shell
        ...
        (Printf.sprintf "%s --target-dir %s --port 9959" (quote script) (quote target))
    in
    if code <> 0
    then
      failf
        "run-local with explicit config env failed (%d)\nstdout:\n%s\nstderr:\n%s"
        code
        stdout
        stderr;
    let captured = read_file capture in
...
let test_config_dir_set_personas_dir_unset_defaults_to_config_personas () =
  with_temp_dir "run-local-script" (fun dir ->
    ...
    let code, stdout, stderr =
      run_shell
        ~cwd:repo_root
        ~env:[ "FAKE_CAPTURE_FILE", capture; "MASC_CONFIG_DIR", override_config ]
        ~unset_env:[ "MASC_PERSONAS_DIR" ]
        (Printf.sprintf "%s --target-dir %s --port 9960" (quote script) (quote target))
    in
    if code <> 0
    then
      failf
        "run-local with MASC_CONFIG_DIR only failed (%d)\nstdout:\n%s\nstderr:\n%s"
        code
        stdout
        stderr;
    let captured = read_file capture in
    check
      bool
      "config dir uses explicit value"
      true
      (contains_substring captured ("MASC_CONFIG_DIR=" ^ override_config));
    check
      bool
      "personas dir defaults to config/personas"
      true
      (contains_substring
         captured
  ...
  with_temp_dir "run-local-script" (fun dir ->
    ...
    let code, stdout, stderr =
      run_shell
        ...
        ~unset_env:[ "MASC_BASE_PATH"; "MASC_CONFIG_DIR"; "MASC_PERSONAS_DIR" ]
        (Printf.sprintf
           "%s --target-dir %s --port 9961 --bootstrap-only"
           (quote script)
           (quote target))
    in
    if code <> 0
    then
      failf
        "run-local bootstrap-only failed (%d)\nstdout:\n%s\nstderr:\n%s"
        code
        stdout
        stderr;
    check
...
let () =
  run
    ...
    [ ( "script"
      , [ test_case
            "bootstraps local config and sets http-only env"
            `Quick
            test_bootstraps_local_config_and_sets_http_only_env
        ; test_case
            "print-port is stable for target dir"
            `Quick
            test_print_port_is_stable_for_target_dir
        ; test_case
            "build-dashboard flag is opt-in"
            `Quick
            test_build_dashboard_flag_is_opt_in
        ; test_case
            "bootstrap-keepers flag is opt-in"
            `Quick
            test_bootstrap_keepers_flag_is_opt_in
        ; test_case
            "existing target config is not overwritten"
            `Quick
            test_existing_target_config_is_not_overwritten
        ; test_case
            "explicit config env is preserved without bootstrap"
            `Quick
            test_explicit_config_env_is_preserved_without_bootstrap
        ; test_case
            "config_dir set personas_dir unset defaults to config/personas"
            `Quick
            test_config_dir_set_personas_dir_unset_defaults_to_config_personas
        ; test_case
            "bootstrap-only materializes state without exec"
            `Quick
