...
let text_metric_type = function
  | Prometheus_store.Counter -> Text.Counter
  | Prometheus_store.Gauge -> Text.Gauge
  | Prometheus_store.Histogram -> Text.Histogram
;;

let type_to_string metric_type = Text.type_to_string (text_metric_type metric_type)
