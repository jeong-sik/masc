...
2. **본선 측정.** main 에서 344개 스위트 중 지금 빨간 게 몇 개인지 센다. 이 숫자를 모르는 채로 게이트를 켜면 무관한 PR 이 막힌다.
   ...
   **지금 이 숫자를 야간 레인으로는 못 얻는다.** 완주한 한 번이 멈춤으로 끝나서 개수까지 못 갔다.

   대신 `test.yml` 의 `suite` 입력으로 **스위트 단위로 표본을 뽑을 수 있다.** 왕복 2~3분이고, 스탠자 env 를 제대로 거는지는 #33566 이 고쳤다 — 그 전에는 346개 중 한 스위트의 env 만 알아서, 나머지로 잰 값은 야간과 비교할 수 없는 숫자였다.

   2026-09-06 표본 셋:

   | 스위트 | 결과 |
   |---|---|
   | `test_heartbeat_integration` | 61건 중 **6건 실패** |
