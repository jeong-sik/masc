...
let test_lifecycle_boot_complete () =
  ...
  check
    (of_pp S.Lifecycle.pp_phase)
    "serving"
    Serving
    (match r with
     | Applied p -> p
     | Ignored _ -> fail "ignored");
  check
    bool
    "not ignored"
    true
...
let test_lazy_task_fail () =
  let current = Pending [ "a"; "b" ] in
  let r =
    S.Lazy_task_queue.apply_event ~current (Task_fail { task = "a"; error = "x" })
  in
  check (of_pp S.Lazy_task_queue.pp) "pending[b]" (Pending [ "b" ]) r
;;
...
let test_invariant_i3_pending_blocks_stop () =
  let state = { S.initial with lifecycle = Stopped; lazy_tasks = Pending [ "x" ] } in
  check_err "I3 violated" (S.check_invariants state)
;;

let test_invariant_i4_degraded_not_ready () =
  let state = { S.initial with backend = Degraded; readiness = Ready } in
  check_err "I4 violated" (S.check_invariants state)
;;

let test_invariant_i5_booting_uninitialized () =
  let state = { S.initial with lifecycle = Booting; backend = Filesystem } in
...
let test_apply_lifecycle_event () =
  ...
  let state = check_ok "boot complete" (S.apply_lifecycle_event state Boot_complete) in
  check (of_pp S.Lifecycle.pp_phase) "serving" Serving state.lifecycle;
  let state = check_ok "resolve backend" (S.apply_backend_event state Resolve_fs) in
  check (of_pp S.Backend.pp_phase) "filesystem" Filesystem state.backend;
  let state = check_ok "set ready" (S.apply_readiness_event state Set_ready) in
  check (of_pp S.Readiness.pp_phase) "ready" Ready state.readiness
;;

let test_apply_backend_event_degrade () =
  let state =
    { S.initial with lifecycle = Serving; backend = Filesystem; readiness = Ready }
  in
  (* Degrading backend while ready should fail invariant I4 *)
  check_err "degrade while ready" (S.apply_backend_event state (S.Backend.Degrade "fail"))
;;

let test_apply_backend_event_degrade_after_not_ready () =
  let state =
    { S.initial with lifecycle = Serving; backend = Filesystem; readiness = NotReady }
  in
  let state =
    check_ok
      "degrade after not_ready"
...
let test_apply_lazy_event () =
  ...
  let state =
    check_ok "task finish" (S.apply_lazy_event state (S.Lazy_task_queue.Task_finish "a"))
  in
...
let test_derive_flat_phase () =
  let check_phase expected state =
    check
      (of_pp S.pp_flat_phase)
      (S.flat_phase_to_string expected)
      expected
      (S.derive_flat_phase state)
  in
  check_phase Blocking { S.initial with lifecycle = Booting };
  check_phase Degraded { S.initial with lifecycle = Serving; backend = Degraded };
  check_phase
    Lazy
    { S.initial with
      lifecycle = Serving
  ...
  check_phase
    Ready
    { S.initial with lifecycle = Serving; lazy_tasks = Complete; readiness = Ready };
  check_phase Blocking { S.initial with lifecycle = Stopped };
  check_phase Ready { S.initial with lifecycle = Draining; readiness = Ready };
  check_phase Blocking { S.initial with lifecycle = Draining; readiness = NotReady }
;;
...
let test_lifecycle_chain () =
  ...
  let state = check_ok "boot complete" (S.apply_lifecycle_event state Boot_complete) in
  check (of_pp S.Lifecycle.pp_phase) "serving" Serving state.lifecycle;
  let state = check_ok "resolve fs" (S.apply_backend_event state Resolve_fs) in
  let state = check_ok "set ready" (S.apply_readiness_event state Set_ready) in
  check (of_pp S.Readiness.pp_phase) "ready" Ready state.readiness;
  let state =
    check_ok
  ...
  check (of_pp S.Lazy_task_queue.pp) "complete" Complete state.lazy_tasks;
  let state =
    check_ok "start draining" (S.apply_lifecycle_event state S.Lifecycle.Start_draining)
  in
  check (of_pp S.Lifecycle.pp_phase) "draining" Draining state.lifecycle;
  let state =
    check_ok "set not ready" (S.apply_readiness_event state S.Readiness.Set_not_ready)
  in
  let state = check_ok "stop" (S.apply_lifecycle_event state S.Lifecycle.Stop) in
...
let test_backend_degrade_recover_cycle () =
  ...
  in
  let state =
    check_ok "degrade" (S.apply_backend_event state (S.Backend.Degrade "timeout"))
  in
  check (of_pp S.Backend.pp_phase) "degraded" Degraded state.backend;
  let state = check_ok "recover" (S.apply_backend_event state S.Backend.Recover) in
  check (of_pp S.Backend.pp_phase) "filesystem" Filesystem state.backend
;;
  ...
  run
    ...
    ; ( "invariants"
      , [ test_case "I1 ready_not_booting" `Quick test_invariant_i1_ready_not_booting
        ...
        ; test_case
            "I5 booting_uninitialized"
            `Quick
            test_invariant_i5_booting_uninitialized
    ...
    ; ( "apply_events"
      , [ test_case "lifecycle_chain" `Quick test_apply_lifecycle_event
        ...
        ; test_case
            "backend_degrade_ok"
            `Quick
            test_apply_backend_event_degrade_after_not_ready
        ; test_case "lazy_events" `Quick test_apply_lazy_event
        ] )
    ; "derive_flat_phase", [ test_case "mappings" `Quick test_derive_flat_phase ]
    ; "json", [ test_case "product_to_json" `Quick test_product_to_json ]
    ; ( "integration"
