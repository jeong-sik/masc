...
if grep -Fq '(generate_opam_files true)' dune-project; then
  ...
  if grep -Fq '# This file is generated by dune' masc.opam \
...
opam_version="${opam_version:-$(sed -n 's/^version: "\([^"]*\)"/\1/p' masc.opam | head -n1)}"

[[ -n "$package_version" ]] || fail "missing package version in dune-project"
[[ -n "$opam_version" ]] || fail "missing version in masc.opam"
...
[[ "$package_version" == "$opam_version" ]] || fail "dune-project ($package_version) != masc.opam ($opam_version)"
