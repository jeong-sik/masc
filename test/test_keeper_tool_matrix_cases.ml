...
module Tool = Masc_agent_core.Tool
...
let make_fixture
  ...
  let ctx =
    ...
    Masc.Keeper_context_runtime.append ctx
      (Masc_agent_core.Types.user_msg "tool matrix memory needle")
  in
...
let find_tool fixture name =
  let by_name tool_name =
    List.find_opt
      (fun (tool : Masc_agent_core.Tool.t) -> String.equal tool.schema.name tool_name)
      fixture.tools
  in
...
let evaluate_expectation ~name expectation = function
  ...
  | Error { Masc_agent_core.Types.message; _ } ->
