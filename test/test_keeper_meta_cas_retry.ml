...
let test_no_conflict_writes_first_attempt () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let disk =
         match Keeper_types.read_meta config "alpha" with
         | Ok (Some m) -> m
...
let test_retry_succeeds_after_concurrent_bump () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let caller_view =
         match Keeper_types.read_meta config "beta" with
         | Ok (Some m) -> m
       ...
       check
         bool
         "version moved past racing write"
         true
...
let () =
  run
    "Keeper_types CAS retry (#9764/#9733/#9769)"
    [ ( "write_meta_with_retry"
      , [ test_case
            "writes on first attempt when no conflict"
            `Quick
            test_no_conflict_writes_first_attempt
        ; test_case
            "lifts payload onto disk version after concurrent bump"
            `Quick
            test_retry_succeeds_after_concurrent_bump
        ] )
    ; ( "is_version_conflict_error"
      , [ test_case
            "classifies conflict vs I/O error"
            `Quick
