...
- Keeper: an official client's start seed begins where the Librarian read, like the Agent Core lane's does. Claude Code and Antigravity carry the saved working state in place of the atoms it summarises, and their `model input carried range` line reports `origin=librarian_snapshot`. The range still starts at the latest of the three positions — the Librarian's, the lane's own cut, and the range the last completed turn carried — so it never moves back. The request can still grow by the working state, which no cut may drop, so a composition that passes a lane's declared ceiling falls back to the atoms it would have replaced. A saved position that does not describe the messages the lane is about to send is not used, and neither is a seed measured on that same history; the lane's own cut stands alone.
