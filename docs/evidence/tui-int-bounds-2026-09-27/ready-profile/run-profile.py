...
def measured_resize(process,master_fd,output,**kwargs):
 ...
 if kwargs.get('rows')==16:
  ...
  # The process starts before sample attaches and reports active collection.
  # Hold detail inputs until that exact readiness banner has been observed.
  deadline=time.monotonic()+10
  ready=f'Sampling process {pid} for 5 seconds'.encode()
  while ready not in (out/'sample-command.log').read_bytes():
   if profiler.poll() is not None:raise RuntimeError('sampler exited before readiness')
   if time.monotonic()>deadline:raise RuntimeError('sampler readiness not observed')
   h.read_available(master_fd,output)
   select.select([master_fd],[],[],0.01)
  profile_info['sampling_ready_monotonic_ns']=time.monotonic_ns()
 return frame

def finish_profile_before_quit(fd,data):
 if fd==owned_fd and data in (b'j',b'k',b'\x1b[<65;5;5M',b'\x1b[<64;5;5M'):
  now_ns=time.monotonic_ns()
  profile_info.setdefault('first_detail_input_monotonic_ns',now_ns)
  profile_info['last_detail_input_monotonic_ns']=now_ns
  profile_info['detail_writes_observed']=profile_info.get('detail_writes_observed',0)+1
  if profiler.poll() is None:
   profile_info['detail_writes_while_sampler_alive']=profile_info.get('detail_writes_while_sampler_alive',0)+1
