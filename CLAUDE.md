...
자율 에이전트가 무한 컨텍스트로 24시간+ 연속 실행. 벤더 무관 MODEL 지원 (Ollama, Claude, Gemini, GLM Cloud).
...
| 모듈 | 역할 |
|------|------|
| `model_client.ml` | 벤더 무관 MODEL 호출 (Ollama/Claude/Gemini/GLM/OpenRouter), cascade fallback |
...
|------|--------|------|
| `--goal` | (필수) | 에이전트 목표 |
| `--models` | (필수) | MODEL cascade (provider:model, 쉼표 구분) |
