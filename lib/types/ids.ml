...
module Turn_id : sig
  ...
  let equal = String.equal
  let pp fmt t = Format.fprintf fmt "%s" t
  let generate ~thread_id ~seq = Printf.sprintf "%s-turn-%04d" thread_id seq
  let to_yojson t = `String t
...
module Keeper_id : sig
  ...
  module Trace_id = struct
    ...
    let of_string s = if is_valid s then Ok s else Error "Invalid trace_id"
    let to_string s = s
    let equal = String.equal
  end
  ...
  module Task_id = struct
    ...
    let of_string s = if is_valid s then Ok s else Error "Invalid task_id"
    let to_string s = s
    let equal = String.equal
  end
end
