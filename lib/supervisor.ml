...
let rec start_child ~sw ~clock cs =
  if cs.disabled
  then Log.Server.info "[Supervisor] skipping disabled child: %s" cs.spec.name
  else (
    cs.running <- true;
    Eio.Fiber.fork ~sw (fun () ->
      ...
      | exn ->
        ...
        (match cs.spec.strategy with
         ...
         | Permanent | Transient ->
           if restart_limit_exceeded cs
           then (
             cs.disabled <- true;
             Log.Server.error
               "[Supervisor] child %s DISABLED: %d restarts in %.0fs"
               name
               cs.spec.max_restarts
               cs.spec.restart_window_s)
           else (
...
let start ~sw ~clock t =
  if t.started
  then Log.Server.warn "[Supervisor] already started"
  else (
    t.started <- true;
...
let find_child t name = List.find_opt (fun cs -> cs.spec.name = name) t.children
