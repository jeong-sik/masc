    ...
    (match List.assoc_opt "path" fields with
     | Some (`String path) when String_util.is_valid_utf8 path ->
       ...
       (match Result.map (fun bytes -> bytes, Verification_media_inspection.detect ~path ~bytes) bytes with
        | Error detail when Option.is_some (Verification_media_inspection.detect ~path ~bytes:"") ->
        ...
        | Ok (bytes, Some kind) ->
             if List.mem_assoc "offset" fields || List.mem_assoc "limit" fields then
               Some (Tool_result.error ~failure_class:Tool_result.Workflow_rejection
                 ...
                 (Verification_media_inspection.whole_file_label kind ^ " are inspected whole; omit line offset and limit"))
             else Some (Verification_media_inspection.inspect kind ~base_path:t.config.base_path
               ~name ~path ~bytes ~start_time ~max_image_bytes:limit)
        | Ok (bytes, None) ->
