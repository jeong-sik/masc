...
let legacy_migrate_post_kind_metric = "masc_board_legacy_migrate_post_kind_total"
...
let valid_visibility_strings = List.map visibility_to_string all_visibilities
...
let legacy_migrate_post_kind ~meta_json ~author ~visibility ~expires_at ~hearth =
  ...
  let hearth =
    ...
    | None -> ""
  in
  if legacy_system_board_author author
  then System_post
  else if meta_source meta_json = Some "keeper_board_post"
  then Automation_post
  else if
    visibility = Internal
    && expires_at > 0.0
    && hearth <> ""
    && (String.starts_with ~prefix:"mdal" hearth || contains_substring hearth "harness")
  then Automation_post
  else if legacy_author_looks_automation author
  then (
    ...
    Prometheus.inc_counter legacy_migrate_post_kind_metric ~labels:[ "author", author ] ();
    Automation_post)
  else Human_post
;;
  ...
  | None ->
    ...
    (match p.post_kind, meta_source p.meta_json with
     ...
     | Human_post, Some source ->
       Printf.sprintf "Direct board post without automation override (source=%s)." source
     | Human_post, None -> "Direct board post without automation provenance."
     | Automation_post, Some "keeper_board_post" ->
       Printf.sprintf
         "Automation classification based on source=keeper_board_post, author=%s, and \
          the automation post_kind contract."
         author
     ...
     | Automation_post, None -> "Automation post preserved by board post_kind contract."
     | System_post, _ ->
...
let reclassify_report_to_yojson (report : reclassify_report) =
  `Assoc
    ...
    ; "changed_post_ids", `List (List.map (fun id -> `String id) report.changed_post_ids)
    ]
;;
