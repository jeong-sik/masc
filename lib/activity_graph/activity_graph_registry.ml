...
let with_registry_rw f =
  ...
  then (
    try Eio.Mutex.use_rw ~protect:true registry_mutex f with
...
let with_registry_ro f =
  if Eio_guard.is_ready ()
  then (
    try Eio.Mutex.use_ro registry_mutex f with
    | Eio.Cancel.Cancelled _ as exn -> raise exn)
  else f ()
;;
...
let register session_id ~push ~last_seq ?(kind_filters = []) () =
  with_registry_rw (fun () ->
    ...
    let client = { client_id; push; kind_filters; last_seq; created_at } in
...
let unregister session_id =
  with_registry_rw (fun () ->
    if Hashtbl.mem clients session_id
    then (
