...
let assert_prefixed_identity_does_not_claim_membership ~assignee ~active_name =
  with_test_env (fun config ->
    let _ = Workspace.add_task config ~title:"Identity Boundary" ~priority:1
        ~description:"" in
    let backlog = Workspace.read_backlog config in
    let tasks =
      List.map
        (fun (task : Masc_domain.task) ->
          if String.equal task.id "task-001" then
            { task with
              task_status =
...
let test_audit_orphan_requires_exact_registered_identity () =
  assert_prefixed_identity_does_not_claim_membership
    ~assignee:"alice" ~active_name:"alice-worker";
  assert_prefixed_identity_does_not_claim_membership
    ~assignee:"alice-worker" ~active_name:"alice"
