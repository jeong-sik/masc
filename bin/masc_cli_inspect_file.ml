open Masc

let run ~base_path ~path =
  Eio_main.run @@ fun env ->
  Fs_compat.set_fs env#fs;
  (* Captures may live outside the invoking cwd capability. The filesystem
     capability supplies that path without changing the process working dir. *)
  Process_eio.init ~cwd_default:Eio.Path.(env#fs / Sys.getcwd ())
    ~proc_mgr:env#process_mgr ~clock:env#clock;
  let absolute path =
    if path = "" || not (Filename.is_relative path) then path
    else Filename.concat (Sys.getcwd ()) path
  in
  let base_path = absolute (Env_config_core.normalize_masc_base_path_input base_path) in
  let path = absolute path in
  let start_time = Time_compat.now () in
  let name = "inspect-file" in
  let error failure_class message =
    Tool_result.error ~failure_class ~tool_name:name ~start_time message
  in
  let source = ref `Null in
  let result =
    try
      if String.trim base_path = "" || String.trim path = "" then
        error Tool_result.Workflow_rejection "Workspace and input file paths must be nonempty"
      else
        let maximum = Verification_pdf_inspection.max_source_bytes in
        match Fs_compat.load_owned_regular_file_prefix
          ~ownership_root:(Filename.dirname path) ~max_bytes:(maximum + 1) path with
        | Error detail ->
          let failure_class = match detail.Fs_compat.failure with
            | Ownership_boundary_rejected _ | Path_is_not_regular_file _ -> Tool_result.Policy_rejection
            | Filesystem_identity_changed _ | Owned_file_operation_failed _ -> Tool_result.Runtime_failure
          in
          error failure_class (Fs_compat.owned_regular_file_read_error_to_string detail)
        | Ok None -> error Tool_result.Workflow_rejection "Selected input file does not exist"
        | Ok (Some captured) when captured.truncated || String.length captured.content > maximum ->
          error Tool_result.Policy_rejection
            (Printf.sprintf "media_source_too_large: inspection accepts at most %d bytes whole" maximum)
        | Ok (Some captured) ->
          let bytes = captured.content in
          source := `Assoc ["path", `String path; "bytes", `Int (String.length bytes);
            "sha256", `String Digestif.SHA256.(digest_string bytes |> to_hex)];
          let max_image_bytes = Env_config_keeper.KeeperVision.max_image_bytes () in
          if Verification_media_inspection.is_presentation path then
            Verification_media_inspection.presentation_result
              ~base_path ~name ~path ~bytes ~start_time ~max_image_bytes
          else if Verification_media_inspection.is_mp4 path bytes then
            Verification_media_inspection.video_result ~base_path ~name ~path ~bytes ~start_time
          else if Verification_media_inspection.is_pdf path bytes then
            Verification_media_inspection.pdf_result
              ~base_path ~name ~path ~bytes ~start_time ~max_image_bytes
          else error Tool_result.Workflow_rejection
            "Supported whole-file inspection formats are PDF, PPTX and MP4"
    with
    | Sys_error detail -> error Tool_result.Runtime_failure detail
    | Unix.Unix_error (code, operation, _) ->
      error Tool_result.Runtime_failure (operation ^ ": " ^ Unix.error_message code)
  in
  let result, content = match result with
    | Tool_result.Completed output ->
      (match Runtime_official_client_tool.mcp_content
        ~content:(Tool_result.message result) ~content_blocks:output.content_blocks with
       | Ok content -> result, content
       | Error detail -> error Tool_result.Runtime_failure detail, [])
    | Tool_result.Failed _ -> result, []
    | Tool_result.Deferred _ -> error Tool_result.Runtime_failure "File inspection unexpectedly deferred", []
  in
  print_endline (Yojson.Safe.to_string (`Assoc [
    "schema", `String "masc.operator_file_inspection.v1";
    "base_path", `String base_path; "source", !source;
    "llm_verdict", `String "not_run";
    "result", Tool_result.to_json result; "content", `List content]));
  match result with Tool_result.Completed _ -> 0 | Tool_result.Failed _ | Tool_result.Deferred _ -> 1
