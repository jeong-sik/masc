...
let identity_snapshot_json (meta : keeper_meta) =
  `Assoc
    (identity_fields |> List.map (fun (field, getter) -> field, `String (getter meta)))
;;
...
let classify_identity_fields
  ...
  List.fold_left
    (fun (inherited, changed, dropped) (field, before) ->
       ...
       then field :: inherited, changed, dropped
       else if before_trimmed <> "" && after_trimmed = ""
       then inherited, changed, field :: dropped
       else inherited, field :: changed, dropped)
    ([], [], [])
    previous
...
let continuity_judgment ~(original : string) ~(received : string) =
  if String.trim original = "" || String.trim received = ""
  then
    { json =
        `Assoc
    ...
    let result = Drift_guard.verify_handoff ~original ~received () in
    let verdict, similarity =
      match result with
      | Drift_guard.Verified summary -> "verified", Some summary.similarity
      | Drift_guard.Drift_detected details -> "drift_detected", Some details.similarity
    in
    { json = Drift_guard.result_to_json result; verdict; similarity })
;;
...
let record_handoff_artifacts
  ...
  =
  let child_trace_id = Keeper_id.Trace_id.to_string child.runtime.trace_id in
  let manifest_path = keeper_generation_manifest_path config child_trace_id in
  let index_path = keeper_generation_index_path config child.name in
  let manifest =
    manifest_json ~parent ~child ~parent_trace_id ~trigger_reason ~context_ratio ~model
  in
  let index_entry =
    index_entry_json
      ~manifest_path
  ...
  with
  | Ok () ->
    (try append_jsonl_line index_path index_entry with
...
let load_json_file_opt path =
  ...
  then None
    ...
    try Some (Yojson.Safe.from_string (Fs_compat.load_file path)) with
    | Eio.Cancel.Cancelled _ as e -> raise e
    | _ -> None)
;;
...
let surface_json (config : Coord.config) (meta : keeper_meta) ~recent_limit =
  ...
  let recent = index_entries |> List.rev |> take (max 0 recent_limit) in
  `Assoc
