...
let write_entries config entries = List.iter (Audit_log.append_entry config) entries
...
let test_build_profile_insufficient_entries () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let entries =
         gen_entries
           ~end_ts:(Unix.gettimeofday ())
           ~spacing_sec:3600.
           ~count:2
           ~tool_names:[ "read" ]
           ~outcomes:[ Audit_log.Success ]
           ~token_counts:[ 100 ]
       in
...
let test_detect_deviations () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let deviations =
         Governance_anomaly.detect_deviations ~profile ~entries:recent ~threshold:1.0
       in
       check bool "detected at least one deviation" true (List.length deviations > 0);
       let has_activity =
         List.exists
           (fun d -> String.equal d.Governance_anomaly.dimension "activity_volume")
           deviations
       in
    ...
    (fun () ->
       ...
       let entries =
         gen_entries
           ...
           ~count:10
           ~tool_names:[ "read"; "write" ]
           ~outcomes:[ Audit_log.Success; Audit_log.Failure "err" ]
           ~token_counts:[ 100 ]
       in
       write_entries config entries;
       let profile =
       ...
       check
         (float 0.001)
         "activity_volume mean"
         profile.activity_volume.mean
         loaded.activity_volume.mean;
       check
         (float 0.001)
         "activity_volume stddev"
         profile.activity_volume.stddev
         loaded.activity_volume.stddev;
       check
         (float 0.001)
         "tool_diversity mean"
         profile.tool_diversity.mean
         loaded.tool_diversity.mean;
       check
         (float 0.001)
         "failure_rate mean"
         profile.failure_rate.mean
         loaded.failure_rate.mean;
...
let test_check_agent_pipeline () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       write_entries config entries;
       match
         Governance_anomaly.check_agent ~config ~agent_id ~window_days:1 ~threshold:1.5
       with
       ...
       | Some report ->
         ...
         check
           ...
           "overall risk low"
           "low"
           (Governance_pipeline_types.risk_level_to_string
              report.Governance_anomaly.overall_risk))
;;
...
let test_detect_deviations_empty_entries () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let deviations =
         Governance_anomaly.detect_deviations ~profile ~entries:[] ~threshold:1.0
       in
...
let test_detect_deviations_no_token_volume () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let entries =
         List.map (fun e -> { e with Audit_log.token_count = None }) entries
       in
       write_entries config entries;
       let profile =
         match Governance_anomaly.build_profile ~config ~agent_id ~window_days:1 with
       ...
       check
         bool
         "token_volume is None"
         true
         (profile.Governance_anomaly.token_volume = None);
       ...
       let has_token =
         List.exists
           (fun d -> String.equal d.Governance_anomaly.dimension "token_volume")
           deviations
       in
...
let () =
  run
    ...
    [ ( "baseline"
      , [ test_case
            "build_profile returns None when insufficient entries"
            `Quick
    ...
    ; ( "pipeline"
      , [ test_case "check_agent produces report" `Quick test_check_agent_pipeline
        ; test_case
            "check_agent returns None when insufficient entries"
            `Quick
            test_check_agent_insufficient_entries
        ] )
    ]
;;
