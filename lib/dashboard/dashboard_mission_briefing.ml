...
let with_cache_lock f = Eio.Mutex.use_rw ~protect:true cache.mutex f
...
let with_cached_flag cached json =
  match json with
  | `Assoc fields -> `Assoc (("cached", `Bool cached) :: List.remove_assoc "cached" fields)
  | other -> other
;;
...
let compute_briefing_json ~actor_name ~config ~sw ~clock ~proc_mgr () =
  ...
  in
  let sessions =
    Briefing_compactors.relevant_sessions_for_briefing ~current_namespace ~now_ts []
  in
  let keepers =
  ...
  let compact_sessions =
    take 3 (List.map Briefing_compactors.compact_session_json sessions)
  in
  let compact_keepers =
    take 3 (List.map Briefing_compactors.compact_keeper_json keepers)
  in
  let agents_json =
    Coord.get_agents_raw config |> List.map Briefing_compactors.compact_agent_json
  in
  ...
  Ok
    (`Assoc
        ...
        ; ( "basis"
          , `Assoc
              [ "project", member_assoc "summary" mission_json |> member_assoc "project"
...
let start_async_refresh ~actor_name ~config ~sw ~clock ~proc_mgr () =
  ...
  then
    Eio.Fiber.fork_daemon ~sw:refresh_sw (fun () ->
      (try
         match
           compute_briefing_json ~actor_name ~config ~sw:refresh_sw ~clock ~proc_mgr ()
         with
         | Ok result_json ->
           with_cache_lock (fun () ->
...
let json ?actor ?(force = false) ~config ~sw ~clock ~proc_mgr () =
  ...
  match cached_json with
  | Some cached_json ->
    if (not force) && is_fresh
    then
      annotate_delivery_state
        cached_json
        ~cached:true
        ~stale:false
        ~refreshing:refresh_in_flight
        ~last_error
    else (
      if not refresh_in_flight
      then start_async_refresh ~actor_name ~config ~sw ~clock ~proc_mgr ();
      annotate_delivery_state
        cached_json
        ~cached:true
        ~stale:true
