...
let int_of_env_default name ~default ~min_v ~max_v =
  ...
  | None -> default
  | Some raw ->
    let parsed = Option.value ~default (int_of_string_opt (String.trim raw)) in
...
let parse_status_from_message ~success ~message =
  ...
  then
    ...
    then "ask_agent_question", Some "ask_agent_question"
    else if
    ...
    then "ask_for_auth", Some "ask_for_auth"
    else "error", None
  else "ok", None
;;
...
let quality_from_result ~success ~message ~attempts =
  if success
  then `Assoc [ "passed", `Bool true; "issues", `List [] ]
  else (
    let issue =
      if contains_casefold message "timeout" || contains_casefold message "timed out"
      then quality_issue "error" "tool_timeout" message attempts
      else quality_issue "error" "tool_failure" message attempts
    in
    `Assoc [ "passed", `Bool false; "issues", `List [ issue ] ])
;;
...
let runtime_mcp_tool_surface_class allowed_tool_names =
  ...
  then "none"
  else if List.for_all Tool_catalog.is_public_mcp allowed_tool_names
  then "public_only"
  else "mixed"
;;
...
let runtime_mcp_keeper_log_context_of_entry
  ...
  =
  let trace_id = Keeper_id.Trace_id.to_string entry.meta.runtime.trace_id in
  let model =
    let last_model_used = String.trim entry.meta.runtime.usage.last_model_used in
    if last_model_used <> "" then last_model_used else String.trim entry.meta.cascade_name
  in
  let session_id =
  ...
  let allowed_tool_names =
    Keeper_tool_policy.keeper_allowed_tool_names ~phase:entry.phase entry.meta
  in
  ...
  ; sandbox_root = Some (Keeper_sandbox.host_root_abs_of_meta ~config entry.meta)
  ; allowed_paths = Some (Keeper_alerting_path.effective_allowed_paths ~meta:entry.meta)
  ; network_mode = Some (Keeper_types.network_mode_to_string entry.meta.network_mode)
  ; shared_memory_scope =
      Some (Keeper_types.shared_memory_scope_to_string entry.meta.shared_memory_scope)
  ; approval_mode = keeper_oas_context.gemini_approval_mode
  ; tool_surface_class = Some (runtime_mcp_tool_surface_class allowed_tool_names)
...
let record_runtime_mcp_trajectory_coverage_gap
  ...
  | gap_exn ->
    log_mcp_exn ~label:"runtime MCP trajectory coverage gap append failed" gap_exn
;;
...
let record_runtime_mcp_keeper_trajectory
  ...
  let safe_output = Observability_redact.redact_preview ~max_len:4000 message in
  let existing_entries =
    try Trajectory.read_entries ~masc_root ~keeper_name:ctx.keeper_name ~trace_id with
    | Eio.Cancel.Cancelled _ as e -> raise e
    | exn ->
...
let record_runtime_mcp_keeper_tool_trace
  ...
  : unit
  =
  let ctx = runtime_mcp_keeper_log_context_of_entry ?mcp_session_id entry ~arguments in
...
let read_only_retry_limit () = Env_config.Tools.readonly_retry_limit
...
let is_retryable_message message =
  ...
  if contains_casefold message "Tool timed out"
  then false
  else
...
let call_tool_with_readonly_retry ~clock ~run_tool ~is_read_only () =
  let max_attempts = read_only_retry_limit () in
  let rec loop attempt =
    let success, message = run_tool () in
    if
      success
      || attempt >= max_attempts
      || (not is_read_only)
      || not (is_retryable_message message)
    then success, message, attempt
    else (
...
let handle_call_tool_eio
  ...
  let name, arguments =
    ...
    | Managed_agent ->
      (match resolve_managed_agent_call ?mcp_session_id params with
       ...
       | Error msg ->
         raise (Invalid_argument ("managed agent tool translation failed: " ^ msg)))
  ...
  let execute_with_timeout () =
    ...
    let result =
      ...
      | exn ->
        ...
        if contains_casefold err "Invalid_argument(\"MASC not initialized"
        then false, Types.masc_error_to_string Types.NotInitialized
        else (
  ...
  let success, message, attempts =
    if is_read_only
    then
      call_tool_with_readonly_retry ~clock ~run_tool:execute_with_timeout ~is_read_only ()
    else (
  ...
  let error_detail =
    ...
    then None
      ...
      Some
        (Printf.sprintf
           "timeout=%d|duration_ms=%d|detail=%s"
  ...
  let telemetry_error_kind =
    if not success
    then if !timeout_hit then Some "timeout" else Some "tool_failure"
    else None
  in
  ...
  then (
    ...
    | Some fs ->
      (try
       ...
       with
       | Eio.Cancel.Cancelled _ as e -> raise e
       | exn -> log_mcp_exn ~label:"telemetry tracking failed" exn)
    | None -> ());
  (* Prometheus: record errors for failed tool calls *)
  if not success then Prometheus.record_error ~error_type:name ();
  ...
  Tool_registry.record_call_if_known
    ~source
    ?assignment_id:called_assignment_id_opt
    ~tool_name:name
    ~success
    ~duration_ms
    ();
  ...
  (try
   ...
   with
   | Eio.Cancel.Cancelled _ as e -> raise e
   | exn -> log_mcp_exn ~label:"activity graph emit failed" exn);
  let trace_id =
  ...
  (match keeper_entry with
   | Some entry ->
     (try
      ...
      with
      | Eio.Cancel.Cancelled _ as e -> raise e
      | exn -> log_mcp_exn ~label:"runtime MCP keeper tool trace failed" exn)
   | None -> ());
  ...
  in
  let content_items = [ `Assoc [ "type", `String "text"; "text", `String message ] ] in
  ...
  if success && List.mem name [ "masc_tool_admin_update" ]
  then broadcast_tools_list_changed ();
  (* Log result *)
  let preview =
