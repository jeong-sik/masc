...
let iso_of_unix_seconds ts = Types.iso8601_of_unix_seconds ts
let take limit values = values |> List.filteri (fun idx _ -> idx < limit)
...
let severity_of_transition_type event_type =
  if
    String_util.contains_substring_ci event_type "failed"
...
let tool_call_timeline_event json =
  ...
  | Some ts_unix, Some tool_name ->
    let success = json_bool_opt_member "success" json |> Option.value ~default:true in
    ...
    let summary =
      ...
      | Some ms ->
        Printf.sprintf
          ...
          ms
      | None ->
        Printf.sprintf "%s %s" tool_name (if success then "succeeded" else "failed")
    in
    Some
      (timeline_event_json
         ...
         ~summary
         ~severity:(severity_of_tool_call success)
         ())
  | _ -> None
;;
...
let approval_event_timeline_event json =
  ...
  | Some ts_unix, Some event ->
    let tool_name = json_string_opt_member "tool" json |> Option.value ~default:"tool" in
    ...
    let kind, title, summary, next_human_action =
      ...
      | "rule_created" ->
        ...
        , None )
      | other -> "approval_event", Printf.sprintf "Approval · %s" tool_name, other, None
    in
    Some
      (timeline_event_json
         ?keeper_turn_id:(keeper_turn_id_of_json json)
...
let decision_timeline_event json =
  ...
  | Some ts_unix ->
    let turn_verdict =
      json_string_opt_member "turn_verdict" json |> Option.value ~default:"unknown"
    in
    let reasons = json_string_list_member "turn_reasons" json in
    let summary =
      match reasons with
...
let transition_timeline_event json =
  ...
  | Some ts_unix ->
    let operator_signal =
      ...
      | _ -> None
    in
    let signal_string key = Option.bind operator_signal (assoc_string_opt key) in
    let signal_bool key =
      Option.map
        (fun fields -> assoc_bool_default key ~default:false fields)
        operator_signal
    in
    let prev_phase = json |> json_member "prev_phase" |> json_string_opt_value in
    let new_phase = json |> json_member "new_phase" |> json_string_opt_value in
    let selected_event = json |> json_member "selected_event" in
    let event_type =
    ...
    let summary =
      ...
      | Some signal when String.trim signal <> "" ->
        Printf.sprintf "%s -> %s via %s · %s" prev_phase new_phase event_type signal
      | _ -> Printf.sprintf "%s -> %s via %s" prev_phase new_phase event_type
    in
    let severity =
...
let receipt_timeline_event receipt =
  ...
  | None -> None
  | Some ended_at ->
    let ts_unix = Types.parse_iso8601 ~default_time:0.0 ended_at in
    if ts_unix <= 0.0
    then None
    else (
      let outcome =
        json_string_opt_member "outcome" receipt |> Option.value ~default:"unknown"
      in
      ...
      let severity =
        ...
        | Some _ -> "bad"
        | None ->
          if String.equal tool_contract_result "violated"
          then "bad"
          else if
...
let blocker_timeline_event
  ...
  | None, None -> None
  | Some blocker_class, Some summary when String.trim summary <> "" ->
    Some
      (timeline_event_json
         ?trace_id
         ?task_id
         ...
         ~severity:
           (match blocker_class with
            ...
            | _ -> "warn")
         ())
  | None, Some summary when String.trim summary <> "" ->
    Some
      (timeline_event_json
         ?trace_id
         ?task_id
...
let disposition_of_snapshot ~pending_approval_count ~runtime_blocker_fields =
  ...
  then "Pause", "waiting_approval"
  else if continue_gate
  then "Pause", "waiting_human_decision"
  else (
    match blocker_class, blocker_summary with
    | Some "cascade_exhausted", _ -> "Alert", "cascade_exhausted"
    | Some "completion_contract_violation", _ -> "Alert", "fsm_invariant"
    | _, Some summary when String_util.contains_substring_ci summary "sandbox" ->
      "Alert", "sandbox_violation"
    | Some _, _ -> "Alert", "critical_block"
    | None, _ -> "Pass", "healthy")
;;

let disposition_fields_json ~(config : Coord.config) ~(meta : keeper_meta) : Yojson.Safe.t
  =
...
let pending_approval_json ~(keeper_name : string) =
  ...
  | `List entries ->
    ...
    |> List.filter (fun json ->
      String.equal keeper_name (Safe_ops.json_string ~default:"" "keeper_name" json))
...
let sort_timeline_events events =
  List.sort
    (fun left right ->
       match
         Bool.compare
           (json_bool_opt_member "observation_only" left |> Option.value ~default:false)
           (json_bool_opt_member "observation_only" right |> Option.value ~default:false)
       with
       | 0 ->
         Float.compare
  ...
  | `List items ->
    (match
       List.find_opt
         (fun json ->
            not
              (json_bool_opt_member "observation_only" json |> Option.value ~default:false))
         items
     with
     | Some event -> event
     | None ->
...
let approval_state_json
  ...
  let approval_profile =
    Option.bind latest_receipt (fun receipt ->
      receipt |> json_member "approval" |> json_string_opt_member "profile")
  in
  let state =
    if pending_approval_count > 0
    then "pending"
  ...
  `Assoc
    ...
    ; ( "matched_by"
      , match latest_rule_match with
        | Some json ->
          json |> json_string_opt_member "matched_by" |> Json_util.string_opt_to_json
        | None -> `Null )
    ; ( "rule_id"
      , match latest_rule_match with
        | Some json ->
          json |> json_string_opt_member "rule_id" |> Json_util.string_opt_to_json
        | None -> `Null )
    ; ( "auto_approved"
      , match latest_approval_audit with
        | Some json ->
          json_bool_opt_member "auto_approved" json |> Json_util.bool_opt_to_json
        | None -> `Null )
    ]
;;
...
let execution_summary_json ~meta ~latest_receipt =
  let sandbox_kind =
    ...
    | Some receipt -> receipt |> json_member "sandbox" |> json_string_opt_member "kind"
    | None -> Some (Keeper_types.sandbox_profile_to_string meta.sandbox_profile)
  in
  let network_mode =
    match latest_receipt with
    | Some receipt ->
      receipt |> json_member "sandbox" |> json_string_opt_member "network_mode"
    | None -> Some (Keeper_types.network_mode_to_string meta.network_mode)
  in
  let sandbox_root =
    match latest_receipt with
    | Some receipt ->
      receipt |> json_member "sandbox" |> json_string_opt_member "sandbox_root"
    | None -> None
  in
  let tool_contract_result =
  ...
  `Assoc
    ...
    ; "runtime_proof_status", Json_util.string_opt_to_json tool_contract_result
    ...
    ; ( "provider_fallback_applied"
      , match cascade_fallback_applied with
        ...
        | None -> `Null )
    ; "provider_selected_model", Json_util.string_opt_to_json cascade_selected_model
    ; "cascade_outcome", Json_util.string_opt_to_json cascade_outcome
    ; ( "sandbox_summary"
...
let causal_timeline_json
  ...
  in
  let approval_events =
    Keeper_approval_queue.read_recent_audit ~base_path ~keeper_name:meta.name ~n:8 ()
    |> List.filter_map approval_event_timeline_event
  in
  let transition_events =
    match
      Keeper_transition_audit.recent_transitions_json ~keeper_name:meta.name ~limit:6
    with
  ...
  let blocker_events =
    ...
    [ blocker_timeline_event
        ~ts_unix:observed_at_unix
        ~observed_at_unix
        ~runtime_blocker_fields
        ?task_id
        ~goal_ids
...
let snapshot_json ~(config : Coord.config) ~(meta : keeper_meta) =
  ...
  in
  let attention_reason = assoc_string_opt "attention_reason" attention_fields in
  let next_human_action = assoc_string_opt "next_human_action" attention_fields in
  let approval_state =
    approval_state_json
      ~pending_approval_count
      ~latest_tool_call
      ...
      ~latest_receipt
  in
  let execution_summary = execution_summary_json ~meta ~latest_receipt in
  let causal_timeline =
    causal_timeline_json
      ...
      ~next_human_action
  in
  let latest_causal_event = latest_causal_from_timeline causal_timeline in
  `Assoc
    ...
    ; ( "goal_id"
      , Json_util.string_opt_to_json (Keeper_runtime_contract.primary_goal_id_opt meta) )
    ...
    ; ( "last_event_bus_correlation"
      , match registry_entry with
        | Some entry -> Json_util.string_opt_to_json entry.last_event_bus_correlation
        | None -> `Null )
    ]
;;
