...
let test_visibility_witness_in_enum () =
  let module C = Masc_board_handlers.Board_core_classify in
  let witness s =
...
let test_visibility_strings_complete () =
  let strs = Masc_board_handlers.Masc_board_handlers.Board_core_classify.valid_visibility_strings in
  List.iter (fun expected ->
    Alcotest.(check bool) (Printf.sprintf "%s present" expected) true
      (List.mem expected strs)
  ) ["public"; "unlisted"; "internal"; "direct"]

(* Issue #8449 PR A: Masc_board_handlers.Board_dispatch.sort_order schema enum SSOT.
...
let test_sort_order_witness_in_enum () =
  let module D = Masc_board_handlers.Board_dispatch in
  let witness s =
