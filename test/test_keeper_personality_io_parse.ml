(** Tests for [Keeper_personality_io.parse] and [.to_json] —
    ...
    surface. RFC-0282 reduced [raw_personality] to the single
    [instructions] field. *)
...
(* nick0cave live data — 357-byte instructions (Layer 1 fix confirmed
   only this keeper exceeds prompt_render_max_bytes; we fix it as a
   regression fixture so the harness preserves byte-for-byte even at
   oversized inputs). *)
...
let test_parse_empty_object_with_defaults () =
  let defaults = make ~instructions:"d_instructions" () in
  check p_eq "empty object → defaults" defaults
    (Keeper_personality_io.parse ~defaults (`Assoc []))

let test_parse_explicit_field_overrides_default () =
  let defaults = make ~instructions:"d_instructions" () in
  let json = `Assoc [ ("instructions", `String "explicit_inst") ] in
  let expected = make ~instructions:"explicit_inst" () in
  check p_eq "explicit field overrides default" expected
...
let test_round_trip_ascii () =
  let p = make ~instructions:"some inst" () in
...
let test_round_trip_with_whitespace_and_newlines () =
  let p = make ~instructions:"  trailing space \n\nand newlines\n" () in
...
let test_parse_handles_non_string_field_via_default () =
  ...
  let json = `Assoc [ ("instructions", `Int 42) ] in
  let defaults = make ~instructions:"d_instructions" () in
  let expected = make ~instructions:"d_instructions" () in
    ...
    [
      ( "parse defaults",
        [
          ...
          test_case "explicit field overrides default" `Quick
            test_parse_explicit_field_overrides_default;
        ] );
      ( "round-trip",
        [
