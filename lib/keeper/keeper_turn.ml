...
(* Keep live/runtime claims scoped to the admitted turn. Historical checkpoint
   messages remain available for conversational continuity, but they are not
   evidence of the current runtime or of a current side effect. This is
   intentionally prompt-only: the admission block proves configuration and
   identity, while typed tool results are still required to prove effects. *)
let current_turn_evidence_block
      ~invocation_kind
      ~keeper_name
      ~turn_id
      ~runtime_id
  ...
  String.concat "\n"
    ...
    ; "This block was generated by MASC after admitting the current turn."
    ; "For live/runtime claims, use only this block and tool results produced during this turn."
    ; "Historical conversation, historical tool outputs, memory, and recalled text are not current evidence."
    ; "If a requested live fact is absent from current-turn evidence, answer UNKNOWN; never infer or copy it from history."
    ; "This admission block proves configuration/admission state only; it does not prove that a command, write, or external side effect completed."
    ; Printf.sprintf "- invocation: %s" invocation_kind
    ; Printf.sprintf "- keeper: %s" keeper_name
    ; Printf.sprintf "- turn_ref: %s#%d" trace_id turn_id
    ; Printf.sprintf "- generation: %d" generation
    ; Printf.sprintf "- runtime_id: %s" runtime_id
    ; Printf.sprintf "- sandbox_profile: %s" sandbox_profile
    ; Printf.sprintf "- network_mode: %s" network_mode
    ; "- pre_dispatch_tool_evidence: none"
    ; "--- End current-turn evidence contract ---"
    ]
...
module For_testing = struct
  ...
  let current_turn_evidence_block = current_turn_evidence_block
    ...
    | Ok meta0 ->
      ...
      (match
       ...
       with
            ...
            (* The prompt is rebuilt for a fallback runtime attempt. Keep the
               admission block aligned with the attempt that is actually
               dispatched instead of leaking the initial runtime identity. *)
            let prompt_runtime_id = ref initial_execution.runtime_id in
            let base_dir =
            ...
            let build_turn_prompt ~base_system_prompt ~messages:_
              ...
              let dynamic_context =
                direct_turn_dynamic_context
                  ...
                  ~turn_instructions_text
                  ~current_turn_evidence_text:
                    (current_turn_evidence_block
                       ~invocation_kind:(invocation_turn_type surface)
                       ~keeper_name:meta.name
                       ~turn_id:keeper_turn_id
                       ~runtime_id:!prompt_runtime_id
                       ~trace_id:(Keeper_id.Trace_id.to_string meta.runtime.trace_id)
                       ~generation:meta.runtime.nonce
                       ~sandbox_profile:
