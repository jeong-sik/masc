...
let recovery_of_reason = function
  | Agent_sdk.Retry.Serving_constraint_rejected
      (Llm_provider.Serving_constraint.Input_rejected { accepted_through; _ }
      | Llm_provider.Serving_constraint.Boundary_unknown { accepted_through; _ }) ->
    Compact_to_accepted_through accepted_through
  | Agent_sdk.Retry.Serving_constraint_rejected
...
let should_try_next_runtime = function
  | Agent_sdk.Error.Api (Agent_sdk.Retry.InputCapacity _) -> true
  | Agent_sdk.Error.Api
...
let compaction_limit = function
  | Agent_sdk.Error.Api (Agent_sdk.Retry.InputCapacity { reason; _ }) ->
    (match recovery_of_reason reason with
     | Compact_to_accepted_through accepted_through -> Some accepted_through
     | Failover_only -> None)
  | Agent_sdk.Error.Api
      ( Agent_sdk.Retry.RateLimited _ | Agent_sdk.Retry.Overloaded _
      | Agent_sdk.Retry.ServerError _ | Agent_sdk.Retry.AuthError _
