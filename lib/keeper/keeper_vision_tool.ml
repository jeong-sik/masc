...
let truncated_of_stop_reason : Masc_agent_core.Types.stop_reason -> bool = function
  | Masc_agent_core.Types.MaxTokens -> true
  ...
  | Masc_agent_core.Types.Unknown _ -> false
...
let message_of_request (req : Va.request) : Masc_agent_core.Types.message =
  let query =
    Printf.sprintf
  ...
  Masc_agent_core.Types.make_message
    ...
    [ Masc_agent_core.Types.text_block query
...
let vision_text_of_response (response : Masc_agent_core.Types.api_response) =
  match
    (Masc_agent_core.Structured.response_json_extractor ()) response
  with
  | Ok json -> vision_text_of_json json
  | Error msg -> Error ("vision response is not valid structured JSON: " ^ msg)
;;

let outcome_of_response (response : Masc_agent_core.Types.api_response) =
