...
(** Authenticated-operator keeper waiting/deferred read model for dashboard
    tools. This projection includes connector route coordinates and persistence
    error paths/messages; callers must complete token-bound [CanReadState]
    authorization before serving it.
