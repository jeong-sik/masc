(** Event_bus_slots — process-wide holders for the two
    [Agent_core.Event_bus.t] instances masc keeps at server bootstrap.

    {b masc slot} — MASC-owned bus, separate from the shared AGENT_CORE
    Event_bus. MASC domain events such as [masc.keeper.lifecycle] are
    published here, not onto AGENT_CORE's
    bus.  Enforces the AGENT_CORE boundary documented in
    [event_bus.mli:103-107] — AGENT_CORE's Event_bus is not a general-purpose
    telemetry channel.  Process-scoped [Atomic.t] singleton.

    {b keeper slot} — shared AGENT_CORE Event_bus reference with domain-local
...
val set_masc : Agent_core.Event_bus.t -> unit
(** Install the MASC Event_bus instance.  Called once at server
    bootstrap. *)

val get_masc : unit -> Agent_core.Event_bus.t option
(** [get_masc ()] returns the installed MASC Event_bus, or [None]
    before bootstrap (e.g. in unit tests that skip server bringup). *)

(** {1 keeper slot — shared AGENT_CORE bus, [Domain.DLS] + process fallback} *)

val set_keeper : Agent_core.Event_bus.t -> unit
...
val get_keeper : unit -> Agent_core.Event_bus.t option
