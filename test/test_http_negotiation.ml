...
let test_accepts_sse_header () =
  ...
  check
    bool
    "event-stream with params"
    true
    (accepts_sse_header (Some "text/event-stream; charset=utf-8"));
  check
    bool
    "event-stream not first"
    true
    (accepts_sse_header (Some "application/json, text/event-stream"));
...
let test_accepts_streamable_mcp () =
  ...
  check
    ...
    "json + sse"
    true
    (accepts_streamable_mcp (Some "application/json, text/event-stream"));
  check
    bool
    "json + sse reversed"
    true
    (accepts_streamable_mcp (Some "text/event-stream, application/json"));
  check
    bool
    "sse q=0"
    false
    (accepts_streamable_mcp (Some "application/json, text/event-stream;q=0"));
  check
    bool
    "case-insensitive"
    true
...
let test_classify_mcp_accept () =
  ...
  let check_mode label expected actual =
    let same =
      ...
      | Streamable, Streamable | Legacy_accepted, Legacy_accepted | Rejected, Rejected ->
        true
      | _ -> false
    in
  ...
  check_mode
    "strict streamable"
    Streamable
    (classify_mcp_accept ~allow_legacy:false (Some "application/json, text/event-stream"));
  check_mode
    "legacy accepted"
    Legacy_accepted
...
let test_protocol_version_for_session_falls_back_to_negotiated_version () =
  ...
  Fun.protect
    ...
    (fun () ->
       check
         string
         "falls back to remembered session version"
         "2025-03-26"
...
let test_protocol_continuity_rejects_mismatch () =
  ...
  let headers = Httpun.Headers.of_list [ "mcp-protocol-version", "2025-03-26" ] in
  let request = Httpun.Request.create ~headers `POST "/mcp" in
  Session.remember_protocol_version session_id "2025-11-25";
  Fun.protect
    ~finally:(fun () -> Session.forget_mcp_session session_id)
...
let test_notification_body_relaxes_accept () =
  ...
  let request = Httpun.Request.create ~headers `POST "/mcp" in
  let body = {|{"jsonrpc":"2.0","method":"notifications/initialized","params":{}}|} in
  let mode = Transport.classify_mcp_accept_for_body request body in
  check
    bool
    "notification legacy accepted"
    true
    (match mode with
...
let test_request_json_only_accepted () =
  ...
  let headers = Httpun.Headers.of_list [ "accept", "application/json" ] in
  let request = Httpun.Request.create ~headers `POST "/mcp" in
  let body = {|{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}|} in
  let mode = Transport.classify_mcp_accept_for_body request body in
  check
    bool
    "json-only accept is rejected"
    true
    (match mode with
...
let test_initialize_json_only_accepted () =
  ...
  check
    bool
    "initialize with json-only is rejected"
    true
    (match mode with
     | Mcp_transport_protocol.Http_negotiation.Rejected -> true
     | _ -> false)
...
let test_initialize_never_uses_sse () =
  let module Transport = Masc_mcp.Server_mcp_transport_http in
  let headers =
    Httpun.Headers.of_list [ "accept", "application/json, text/event-stream" ]
  in
  let request = Httpun.Request.create ~headers `POST "/mcp" in
  let body =
    {|{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-11-25","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}|}
  ...
  check
    bool
    "initialize disables sse"
    false
    (Transport.should_use_sse_for_body
       request
       body
...
let test_sse_guard_registry_is_shared_with_cleanup_loop () =
  ...
  | Error (reason, retry_after_s) ->
    failf "expected first guard insert to succeed, got %s %.3f" reason retry_after_s
  | Ok () ->
    check int "cleanup keeps fresh guard entries" 0 (Cleanup_view.reap_stale_guards ());
    (match Transport.check_sse_connect_guard session_id with
     ...
     | Ok () -> fail "expected second guard check to observe shared cooldown state")
;;
...
let () =
  ...
  run
    ...
    ; ( "accepts_streamable_mcp"
      , [ test_case "requires json+sse" `Quick test_accepts_streamable_mcp ] )
    ; ( "classify_mcp_accept"
      , [ test_case "strict vs legacy fallback" `Quick test_classify_mcp_accept ] )
    ; ( "protocol_continuity"
      , [ test_case
            "missing header falls back to session"
            `Quick
            test_protocol_continuity_allows_missing_header
        ; test_case
            "remembered session version is reused"
            `Quick
            test_protocol_version_for_session_falls_back_to_negotiated_version
        ; test_case
            "mismatch still rejects"
            `Quick
            test_protocol_continuity_rejects_mismatch
        ] )
    ; ( "body_aware_accept"
      , [ test_case
            "notification relaxes accept"
            `Quick
            test_notification_body_relaxes_accept
        ; test_case "json-only accept is rejected" `Quick test_request_json_only_accepted
        ; test_case
            "initialize json-only is rejected"
            `Quick
            test_initialize_json_only_accepted
        ...
        ; test_case
            "sse guard registry is shared"
            `Quick
            test_sse_guard_registry_is_shared_with_cleanup_loop
        ; test_case
            "preserve guard keeps cooldown"
            `Quick
