...
let spawn_http_server ~status_code =
  ...
  | pid ->
    if not (wait_until ~timeout_sec:1.0 (fun () -> port_accepting port))
    then (
...
let spawn_forever_process ?argv0 ~ignore_sigterm () =
  ...
  | Some name ->
    let script =
      if ignore_sigterm
      then "trap '' TERM; while :; do sleep 1; done"
      else "while :; do sleep 1; done"
    in
    with_dev_null_fds (fun ~in_fd ~out_fd ~err_fd ->
      Unix.create_process "/bin/sh" [| name; "-c"; script |] in_fd out_fd err_fd)
  | None ->
    (match Unix.fork () with
     | 0 ->
       if ignore_sigterm then Sys.set_signal Sys.sigterm Sys.Signal_ignore;
       while true do
...
let lock_path dir = Filename.concat dir "masc.pid"
let base_path_lock_path dir = Server_startup_takeover.base_path_lock_path dir
...
let test_server_command_heuristic () =
  ...
  Alcotest.(check bool)
    ...
    false
    (Server_startup_takeover.looks_like_server_command "python3 -m http.server 8935")
;;
  ...
  with_temp_dir "startup-takeover-responsive" (fun dir ->
    with_http_server ~status_code:200 (fun ~pid ~port ->
      ...
      | Server_startup_takeover.Already_running { pid = running_pid } ->
        ...
        Alcotest.(check bool) "server process stays alive" true (process_alive pid)
...
let test_reclaims_stale_pid_file () =
  with_temp_dir "startup-takeover-stale" (fun dir ->
    with_forever_process ~ignore_sigterm:false (fun pid ->
      ...
      match Server_startup_takeover.acquire_pid_lock ~lock_path:path port with
      | Server_startup_takeover.Acquired ->
        Alcotest.(check int) "current pid written" (Unix.getpid ()) (pid_from_file path)
...
let test_escalates_sigkill_for_unresponsive_holder () =
  with_temp_dir "startup-takeover-unresponsive" (fun dir ->
    with_forever_process ~argv0:"main_eio.exe" ~ignore_sigterm:true (fun pid ->
      ...
      let port = find_free_port () in
      match
        Server_startup_takeover.acquire_pid_lock
          ~lock_path:path
...
let () =
  Alcotest.run
    ...
    [ ( "helpers"
      , [ Alcotest.test_case "status line parser is exact" `Quick test_status_line_parser
        ; Alcotest.test_case
            "server command heuristic rejects unrelated processes"
            `Quick
            test_server_command_heuristic
        ] )
    ; ( "takeover"
      , [ Alcotest.test_case
            "responsive holder blocks takeover"
            `Quick
        ...
        ; Alcotest.test_case
            "invalid pid file is overwritten"
            `Quick
            test_tolerates_invalid_pid_file
        ; Alcotest.test_case
            "unresponsive holder escalates to sigkill"
            `Quick
            test_escalates_sigkill_for_unresponsive_holder
        ; Alcotest.test_case
            "live base-path owner blocks takeover"
            `Quick
            test_base_path_lock_rejects_live_server_holder
        ; Alcotest.test_case
            "stale base-path owner is reclaimed"
            `Quick
