...
let print_summary () =
  ...
  Log.Env.info
    "Inference: timeout=%.0fs cache_enabled=%b ttl=%ds max_prompt_chars=%d max_temp=%.2f \
     l1_max=%d spawn_policy=%s"
  ...
  Log.Env.info
    "KeeperBootstrap: enabled=%b stale_turn=%.0fs max_scan=%d"
  ...
  Log.Env.info
    "KeeperAlert: enabled=%b min_score=%.2f retries=%d board=%b slack=%b github=%b"
  ...
  Log.Env.info
    "KeeperAlert(SlackDM): enabled=%b user_id_set=%b"
    Env_config_keeper.KeeperAlert.slack_dm_enabled
    (String.trim Env_config_keeper.KeeperAlert.slack_dm_user_id <> "");
  Log.Env.info
    "KeeperKeepalive: interval=%ds max_hb_failures=%d debounce=%.0fs sleep_chunk=%.1fs \
     jitter=%.2f"
  ...
  Log.Env.info
    "ContextCompact: drop_thr=%.2f prune_limit=%d"
...
let to_json () : Yojson.Safe.t = Env_config_snapshot.to_json ()
