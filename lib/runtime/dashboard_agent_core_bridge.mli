(** Per-call AGENT_CORE telemetry collector backing the [/agent_core/telemetry] dashboard.

    Layer 4 collector for {b I1 Telemetry pipeline} (#11924, Phase 1, Track T1)
    of the [Meta] MASC × AGENT_CORE root improvement plan (#11923).
    ...
    before storage so MASC-side dashboard state does not retain concrete AGENT_CORE
    ...
    AGENT_CORE workers call {!record} after each LLM turn (tool path or chat path),
    ...
    [agent_core_telemetry_sample] dashboard event.
...
(** Outcome status of a single AGENT_CORE call. Mirrors the variant intent of
...
type sample = {
  ...
  usage_reported : bool;
      (** [true] when AGENT_CORE supplied provider usage. Missing usage remains
...
(** [sample_of_response ~provider_id ~model_id ?total_duration_ms
    ?serialization_ms response] projects an AGENT_CORE [api_response] into the
    twelve-signal bridge sample.

    - [usage] fields become token, cost, and cache-hit signals.
    - [response.telemetry.ttfrc_ms] is preferred for TTFB when AGENT_CORE reports
    ...
    - native decode throughput is preferred when AGENT_CORE telemetry exposes it;
    ...
    Missing AGENT_CORE usage is represented with [usage_reported = false] and
...
val record_response :
  ...
  Agent_core.Types.api_response ->
  unit
(** Convert an AGENT_CORE response with {!sample_of_response}, then {!record} it. *)
...
(** Dashboard payload for
    ...
    durable [agent_core_event] replay surface. *)

val summary_json : ?provider:string -> ?limit:int -> unit -> Yojson.Safe.t
(** Dashboard payload for
    [GET /api/v1/dashboard/agent_core/telemetry/summary?provider=P&limit=N].
