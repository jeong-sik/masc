...
let build_code_change_prompt
  ...
  let history_lines =
    List.map
      (fun (r : cycle_record) ->
         Printf.sprintf
           "  Cycle %d: %s -> delta=%.4f (%s)"
           r.cycle
           r.hypothesis
           r.delta
           (Autoresearch_serde.decision_to_string r.decision))
      recent
  in
  ...
  String.concat
    ...
    ([ "You are an autonomous research assistant optimizing code."
     ...
     @ (if history_lines <> []
        then [ ""; "Recent experiment history:" ] @ history_lines
        else [])
     @ (if insight_lines <> []
        then [ ""; "Accumulated insights:" ] @ insight_lines
        else [])
     @ [ ""
       ; "<current_code>"
       ; file_content
...
let normalize_modified_code code =
  ...
  let rec drop_blank = function
    | [] -> []
    | l :: rest -> if String.trim l = "" then drop_blank rest else l :: rest
  in
...
let parse_required_string_field ~field json =
  ...
  | None -> Result.error (Printf.sprintf "Missing \"%s\" field in MODEL response" field)
  | Some (`String value) ->
    let trimmed = String.trim value in
    if trimmed = ""
    then Result.error (Printf.sprintf "Empty \"%s\" field in MODEL response" field)
...
let parse_model_code_response response =
  ...
  if trimmed_response = ""
  then Result.error "MODEL returned empty response"
  else (
    ...
    | `Assoc [ ("raw", `String raw) ] ->
      ...
      let preview =
        ...
        |> String.trim
        |> fun normalized ->
        String_util.utf8_safe ~max_bytes:83 ~suffix:"..." normalized
        |> String_util.to_string
      in
      Result.error
        (Printf.sprintf
           "MODEL response is not valid JSON after lenient recovery: %s"
           preview)
    | `Assoc _ as json ->
       ...
       | Ok hypothesis ->
         (match parse_required_string_field ~field:"modified_code" json with
          ...
          | Ok code ->
            ...
            if normalized_code = ""
            then Result.error "Empty \"modified_code\" field in MODEL response"
            else Result.ok (String.trim hypothesis, normalized_code)))
    | _ -> Result.error "MODEL response must be a JSON object")
;;
...
let has_background_capacity () =
  ...
  | Some sw, Some net ->
    (try
       let capacity =
         Cascade_config.local_capacity_for_selections ~sw ~net [ "autoresearch" ]
       in
       not
...
let generate_code_change
      ...
      ~file_content
  =
  if not (has_background_capacity ())
  then (
    ...
    match
      Masc_oas_bridge.run_with_caller
        ~caller:Env_config_oas_bridge.Autoresearch_codegen
        (fun () ->
           Oas_worker.run_named
             ~cascade_name:"autoresearch"
             ~goal:prompt
             ~max_turns:1
             ~temperature:
    ...
    with
    | Error e ->
      Result.error (Printf.sprintf "MODEL call failed: %s" (Oas.Error.to_string e))
    | Ok result ->
      parse_model_code_response (Oas_response.text_of_response result.Oas_worker.response))
;;
