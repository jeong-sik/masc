...
let test_format_section_with_content () =
  let s : Dashboard.section =
    { title = "Agents"; content = [ "agent1"; "agent2" ]; empty_msg = "(no agents)" }
  in
...
let test_resilience_default_warning_threshold () =
  check
    ...
    "warning threshold"
    true
    (abs_float (Resilience.default_warning_threshold -. 120.0) < 0.001)
;;
...
let test_time_is_stale_custom_threshold () =
  ...
  check
    bool
    "2020 stale with 1 year threshold"
    true
...
let test_zombie_is_zombie_recent () =
  ...
  check
    ...
    "recent not zombie"
    false
    (Resilience.Zombie.is_zombie ~threshold:3600.0 recent)
;;
  ...
  run
    ...
    ; ( "resilience.zombie"
      , [ test_case "is_zombie old" `Quick test_zombie_is_zombie_old
        ...
        ; test_case
            "is_zombie custom threshold"
            `Quick
            test_zombie_is_zombie_custom_threshold
        ] )
