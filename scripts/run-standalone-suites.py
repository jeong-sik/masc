...
class Resolver:
    ...
    def generate_library(self, library: Library) -> bool:
        """Whether this runner can produce [library]'s modules itself.

        The text is made once and kept in memory; [stage_generated] writes it
        into a suite's build directory. Nothing is written into the checkout:
        a --list must not leave a file behind, and a source tree that grows an
        untracked .ml because a test ran is a worse trade than the coverage.
        """
        if library.name != "embedded_config":
            return False
    ...
    def plan(self, suite: str, deps: list[str]) -> tuple[Plan | None, str | None]:
        ...
        def visit(name: str) -> bool:
            ...
            #
            # embedded_config is `ocaml-crunch -m plain config`: one command,
            # no network, a pure function of a directory this run already
            # reads, and 896 suites -- most of the tree -- link it. Producing
            # it here is the same act as compiling the modules beside it.
            #
            # masc_proto is protoc, which is a system package CI's own opam
            # run reports as unavailable, so there is nothing to run and the
