...
let failure_reason_to_string = function
  ...
  | Stale_turn_timeout sec -> Printf.sprintf "stale_turn_timeout(%.0fs)" sec
...
let set_turn_cascade_state ~base_path name cascade_state =
  ...
  update_entry ~base_path name (fun e ->
    update_current_turn e (fun obs ->
      changed := true;
      { obs with cascade_state; turn_phase = turn_phase_of_cascade_state cascade_state }));
  if !changed then broadcast_composite_changed ~name ~ts_unix:now
;;
...
let mark_turn_gate_rejected_by_name name =
  ...
  | Some entry ->
    ...
    update_entry ~base_path:entry.base_path name (fun e ->
      update_current_turn e (fun obs ->
        changed := true;
        { obs with decision_stage = Decision_gate_rejected; turn_phase = Turn_finalizing }));
    if !changed then broadcast_composite_changed ~name ~ts_unix:now
;;
...
let mark_turn_finished ~base_path name =
  ...
  update_entry ~base_path name (fun e ->
    ...
    in
    { e with current_turn_observation = None; last_completed_turn });
  Option.iter
...
let flush_tool_usage ~base_path name =
  ...
  | Some entry ->
    ...
    (try
     ...
     with
     | Eio.Cancel.Cancelled _ as e -> raise e
     | exn -> Log.Keeper.error "flush_tool_usage %s: %s" name (Printexc.to_string exn))
;;
  ...
  then ()
    ...
    | Some _entry ->
      (try
       ...
       with
       | Eio.Cancel.Cancelled _ as e -> raise e
       | exn -> Log.Keeper.warn "restore_tool_usage %s: %s" name (Printexc.to_string exn)))
;;
...
let compaction_stage_after_event entry event =
  ...
  | Keeper_state_machine.Operator_compact_requested -> Compaction_compacting
