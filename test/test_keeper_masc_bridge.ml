...
let init_keeper_tool_registry () = Masc_test_deps.init_keeper_tool_registry ()
  ...
  match
    Masc_mcp.Keeper_types.meta_of_json
      (`Assoc
          ...
          ; ( "sandbox_profile"
            , `String (Masc_mcp.Keeper_types.sandbox_profile_to_string sandbox_profile) )
...
let test_inject_stores_filtered_masc () =
  ...
  let meta =
    make_meta
      ~tool_access:
        (Masc_mcp.Keeper_types.Custom [ "masc_status"; "masc_broadcast"; "masc_messages" ])
      ()
  in
  let names = KET.keeper_masc_tool_names meta in
  Alcotest.(check int) "only keeper-compatible masc tools remain" 1 (List.length names);
  Alcotest.(check bool) "keeps masc_status" true (List.mem "masc_status" names);
  Alcotest.(check bool) "filters masc_broadcast" false (List.mem "masc_broadcast" names);
  Alcotest.(check bool) "filters masc_messages" false (List.mem "masc_messages" names);
...
let test_full_preset_exposes_masc () =
  ...
  Alcotest.(check bool)
    "no masc_governance_status"
    false
    (List.mem "masc_governance_status" names);
  Alcotest.(check bool)
    "has masc_autoresearch_cycle"
    true
    (List.mem "masc_autoresearch_cycle" names);
  Alcotest.(check bool)
    "filters unsupported inline tool"
    false
...
let test_messaging_preset_exposes_board () =
  ...
  Alcotest.(check bool) "has keeper_board_post" true (List.mem "keeper_board_post" names);
  (* Governance tools are no longer available *)
  Alcotest.(check bool)
    "no masc_governance_status"
    false
    (List.mem "masc_governance_status" names);
  Alcotest.(check bool) "has keeper_shell" true (List.mem "keeper_shell" names);
  (* keeper_github tool was removed in #7306 (use keeper_shell op=gh). *)
  Alcotest.(check bool)
    "no keeper_github (removed)"
    false
    (List.mem "keeper_github" names);
  Alcotest.(check bool) "has keeper_fs_read" true (List.mem "keeper_fs_read" names)
;;
...
let test_custom_opens_specific_tools_only () =
  ...
  let meta =
    make_meta
      ~tool_access:
        (Masc_mcp.Keeper_types.Custom [ "masc_status"; "masc_tasks"; "masc_join" ])
      ()
  in
  let names = KET.keeper_masc_tool_names meta in
  Alcotest.(check int) "only keeper-compatible tools allowed" 2 (List.length names);
  Alcotest.(check bool) "has masc_status" true (List.mem "masc_status" names);
  Alcotest.(check bool) "has masc_tasks" true (List.mem "masc_tasks" names);
  Alcotest.(check bool) "filters masc_join" false (List.mem "masc_join" names);
...
let test_deny_overrides_allow () =
  ...
  Alcotest.(check bool) "has masc_status" true (List.mem "masc_status" names);
  Alcotest.(check bool) "no masc_tasks (denied)" false (List.mem "masc_tasks" names)
;;

let test_custom_empty_blocks_all () =
  prime_keeper_bridge ();
  let meta = make_meta ~tool_access:(Masc_mcp.Keeper_types.Custom []) () in
  let names = KET.keeper_allowed_tool_names meta in
...
let test_preset_with_also_allow_opens_extra_tool () =
  ...
  let meta =
    make_meta
      ~tool_access:
        (Masc_mcp.Keeper_types.Preset
           { preset = Masc_mcp.Keeper_types.Minimal; also_allow = [ "masc_tasks" ] })
      ()
  in
  let names = KET.keeper_allowed_tool_names meta in
  Alcotest.(check bool) "minimal keeps base tool" true (List.mem "keeper_time_now" names);
  Alcotest.(check bool) "also_allow adds tasks" true (List.mem "masc_tasks" names);
  Alcotest.(check bool)
    "minimal omits board post"
    false
...
let test_custom_keeps_registered_inline_board_tool () =
  ...
  let meta =
    make_meta
      ~tool_access:(Masc_mcp.Keeper_types.Custom [ "keeper_board_post"; "masc_who" ])
      ()
  in
  let names = KET.keeper_masc_tool_names meta in
  ...
  Alcotest.(check bool)
    "raw masc_board_post filtered out"
    false
    (List.mem "masc_board_post" names);
  Alcotest.(check bool) "drops unsupported inline tool" false (List.mem "masc_who" names)
;;
...
let test_dashboard_tool_count_uses_schema_ssot () =
  ...
  with_masc_schema_ref [ schema ] (fun () ->
    let meta = make_meta ~tool_access:(Masc_mcp.Keeper_types.Custom [ bridge_name ]) () in
    let allowed = KET.keeper_allowed_tool_names meta in
    Alcotest.(check bool) "schema bridge is allowed" true (List.mem bridge_name allowed);
    let json =
      Masc_mcp.Server_dashboard_http_keeper_api.keeper_tools_response_json meta
    in
    let count = Yojson.Safe.Util.(json |> member "active_masc_tool_count" |> to_int) in
...
let test_tool_access_missing_migrates_legacy_standard_policy () =
  ...
  in
  Alcotest.(check bool)
    "keeps keeper internal tool"
    true
    (List.mem "keeper_time_now" names);
  Alcotest.(check bool)
    "keeps legacy standard masc tool"
    true
    (List.mem "masc_status" names);
  Alcotest.(check (list string))
    "legacy migration keeps expected masc set"
    expected_legacy_masc_names
    legacy_masc_names;
  Alcotest.(check bool)
    "does not silently expand to full"
    false
    (List.mem "masc_autoresearch_cycle" names)
;;
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Ok (Some meta) ->
         let names = KET.keeper_allowed_tool_names meta in
         Alcotest.(check bool)
           "full keeps keeper internal tool"
           true
           (List.mem "keeper_fs_edit" names);
         Alcotest.(check bool)
           "full keeps autoresearch tool"
           true
           (List.mem "masc_autoresearch_cycle" names);
         let scrubbed = read_json_file path in
         Alcotest.(check string)
           "legacy kind rewritten"
           "preset"
...
let test_tool_access_preset_empty_json_preserved () =
  ...
  in
  match meta.Masc_mcp.Keeper_types.tool_access with
  | Masc_mcp.Keeper_types.Preset { preset = Masc_mcp.Keeper_types.Coding; also_allow } ->
    ...
    match
      Masc_mcp.Keeper_types.meta_of_json
        (`Assoc
            ...
            ; "tool_access", `Assoc [ "kind", `String "custom"; "tools", `List [] ]
            ])
    with
    | Ok meta -> meta
    | Error e -> failwith e
  ...
  match
    Masc_mcp.Keeper_types.meta_of_json
      (`Assoc
          ...
          ; "tool_access", `Assoc [ "kind", `String "preset"; "preset", `String "bogus" ]
          ])
  with
...
let test_allowlist_gates_shard_tools () =
  ...
  let meta =
    make_meta
      ~tool_access:(Masc_mcp.Keeper_types.Custom [ "masc_status"; "masc_tasks" ])
      ()
  in
  let names = KET.keeper_allowed_tool_names meta in
  Alcotest.(check bool) "has masc_status" true (List.mem "masc_status" names);
  Alcotest.(check bool) "has masc_tasks" true (List.mem "masc_tasks" names);
  Alcotest.(check bool)
    "masc_autoresearch_cycle blocked by custom policy"
    false
    (List.mem "masc_autoresearch_cycle" names)
;;

let test_dispatch_unregistered () =
  let result = Masc_mcp.Tool_dispatch.mint_token ~name:"masc_nonexistent_xyz" in
  Alcotest.(check bool)
    "unregistered mint_token returns Error"
    true
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       run_with_fs (fun () ->
         ...
         let path = Yojson.Safe.Util.member "path" json |> Yojson.Safe.Util.to_string in
         let lines =
...
let test_schemas_match_names () =
  ...
  let meta =
    make_meta
      ~tool_access:
        (Masc_mcp.Keeper_types.Custom [ "masc_status"; "masc_join"; "masc_tasks" ])
      ()
  in
  let names = KET.keeper_masc_tool_names meta in
  let schemas = KET.keeper_masc_tool_schemas meta in
  Alcotest.(check int) "count matches" (List.length names) (List.length schemas);
  List.iter
    (fun (s : Types.tool_schema) ->
       Alcotest.(check bool) (s.name ^ " in names") true (List.mem s.name names))
    schemas
;;
  ...
  List.iter
    (fun denied_name ->
       Alcotest.(check bool)
         ...
         false
         (List.mem denied_name names))
    denied
;;

let test_is_keeper_denied () =
  (* Post-pruning: keeper_denied surface is [masc_reset; masc_spawn]. *)
  Alcotest.(check bool) "masc_reset is denied" true (KET.is_keeper_denied "masc_reset");
  Alcotest.(check bool) "masc_spawn is denied" true (KET.is_keeper_denied "masc_spawn");
  Alcotest.(check bool)
    "masc_status is not denied"
    false
    (KET.is_keeper_denied "masc_status");
  Alcotest.(check bool)
    "keeper_time_now is not denied"
    false
...
let test_denied_excluded_from_allowed_names () =
  ...
  List.iter
    ...
    denied;
  Alcotest.(check bool)
    "keeper_time_now still present"
    true
    (List.mem "keeper_time_now" names);
  Alcotest.(check bool) "masc_status still present" true (List.mem "masc_status" names)
;;
  ...
  Alcotest.run
    ...
    ; ( "dispatch"
      , [ Alcotest.test_case "unregistered returns None" `Quick test_dispatch_unregistered
        ; Alcotest.test_case
            "read preflight accepts sandbox-relative repo path"
            `Quick
    ...
    ; ( "keeper_denied"
      , [ Alcotest.test_case
            "denied tools excluded from injection"
            `Quick
            test_denied_tools_excluded_from_injection
        ; Alcotest.test_case "is_keeper_denied correctness" `Quick test_is_keeper_denied
        ; Alcotest.test_case
            "denied excluded from allowed_names"
            `Quick
