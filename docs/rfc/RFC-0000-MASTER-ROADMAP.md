...
- **파편화 인벤토리(실측):**
  ...
  | **oas_env**(per-keeper OAS env) | keeper.toml`[keeper.oas_env]`(`keeper_types_profile_oas_env.ml`) + keeper.json meta(`keeper_meta_json_parse.ml:531`) | **NO** — dual-home | **높음** — `keeper_runtime.ml:402` all-or-nothing(toml oas_env가 비어있지 않으면 json meta oas_env 전체 무시) → toml 1키가 json 전체 map 소실. toml-overlay는 per-key merge하는데 toml-vs-json은 안 함(병합 비일관) |
  | **active_goal_ids**(goal 바인딩) | keeper.toml + keeper.json meta(`keeper_meta_contract.mli:278`). Goal **엔티티**는 `goal_store.ml` SSOT(clean) | **NO**(바인딩만) — dual-home | 중 — 같은 all-or-nothing(`keeper_runtime.ml:354`) → toml default가 json 바인딩 wholesale 마스킹 |
  | **always_allow**(per-keeper tool auto-approve) | keeper.toml(`keeper_types_profile_defaults.ml:18`) + keeper.json meta(`keeper_meta_json_parse.ml:31`) | **NO** — dual-home | 낮음(단일 bool) — 같은 precedence-drift(`keeper_runtime.ml:398`). tool allowlist/denylist tier는 3곳 fail-closed reject(제거 확정), auto-approve bool만 생존 |
...
- **공통 근본결함 (하나의 버그가 N곳 복제):** `keeper_runtime.ml`의 toml-vs-json 병합기가 **all-or-nothing(replace-not-union)** — `mention_targets`(L~352)·`oas_env`(L402)·`active_goal_ids`(L354)·`always_allow`(L398)가 전부 `match defaults.X with [] -> meta.X | v -> v` 또는 `apply_default` 형태. keeper.toml에 해당 축 값이 하나라도 있으면 keeper.json에 persist된 값 전체가 조용히 소실. 이건 CLAUDE.md "N-of-M / 같은 버그 N번 fix" 시그니처 → **site별 패치 금지, 단일 병합 모듈로 union화(또는 replace를 명시+loud)** 하고 4 소비처가 그것만 호출.
- **candidate work:** **[REPO-SSOT, 우선]** `clone_sandbox_repo`가 catalog record 전체 읽기(하드코딩 `"main"`/`[]` 제거; 동일 하드코딩이 `repo_store.ml:439` `discover_repositories`에도 있음 — 함께 수정); clone location을 catalog **id** keyed 순수 함수로; 역파싱 조인(`parse_playground_repo_path`+`find_url_by_id`, **프로덕션 caller 단 1곳** `keeper_tool_filesystem_runtime.ml:314`) 제거→`git remote get-url`(`Repo_git.get_origin_url`, **이미 `repo_store.ml:439`에서 동일 용도로 사용**) + `worktree_root`로 귀속; 등록 `local_path`를 `operator_checkout_path`로 개명(2역할 2이름, REGISTERED 소비처 ~11곳). **[identity, SEVERE]** typed `Keeper_id` 단일 canonical + explicit projection으로 5+ 핸들 통합 → `identity_matches` OR-매칭·#10440 alias 워크어라운드 제거. **[병합기]** 위 공통 근본결함 union화. **[나머지 축]** single read path(`read_meta` private화), single `Local`/`Docker` enum+1 parser, rejected-key 단일 closed set+`allowed_providers` 강제, docker layout을 `playground_paths` SSOT가 **구성+파싱** 둘 다. **[D8 정합]** 역파싱 귀속을 git-remote로 옮기면 playground 번들 전체가 bare per-keeper 작업디렉토리+주입 env로 붕괴 — §11 D8·D9와 한 수술.
