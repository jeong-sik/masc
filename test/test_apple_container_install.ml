...
let platform_runner ?(publisher=A.expected_publisher) ?(gatekeeper=true) calls argv =
  ...
  | ["/usr/bin/sudo";"/usr/sbin/installer";"-pkg";_;"-target";"/"] -> Ok "installer: succeeded"
...
let test_install_revalidates () = with_package @@ fun path ->
  ...
  check bool "explicit installation invokes verified package" true (A.install ~run artifact = Ok ());
  let before = List.length !calls in
  Out_channel.with_open_bin path (fun channel -> output_string channel "changed");
  check bool "changed file blocks installer" true (A.install ~run artifact = Error A.Digest_mismatch);
