...
let entry_to_json
      ?(result_max_len = default_result_truncation)
      ?runtime_contract
      ?action_radius
      (e : tool_call_entry)
  : Yojson.Safe.t
     ...
     ; ( "result"
       , match e.result with
         ...
         | Some r ->
           ...
           then
             `String
               (String_util.utf8_safe ~max_bytes:(result_max_len + 3) ~suffix:"..." r
...
let thinking_entry_to_json
      ?(content_max_len = default_thinking_truncation)
      (e : thinking_entry)
  : Yojson.Safe.t
  =
  let content =
    ...
    then
      String_util.utf8_safe ~max_bytes:(content_max_len + 3) ~suffix:"..." e.content
      |> String_util.to_string
    else e.content
  in
  `Assoc
...
let trajectory_path (masc_root : string) (keeper_name : string) (trace_id : string)
  : string
  =
  Filename.concat
    (trajectories_dir masc_root keeper_name)
...
let set_task_id (acc : accumulator) (id : string) : unit = acc.task_id <- Some id

(** Clear task binding (e.g., after masc_transition action=done). *)
let clear_task_id (acc : accumulator) : unit = acc.task_id <- None

let increment_turn (acc : accumulator) : unit = acc.turn <- acc.turn + 1
...
let record_entry
  ...
  | exn ->
    Log.Keeper.error
      "Failed to persist entry for %s: %s"
      acc.trace_id
...
let finalize (acc : accumulator) (outcome : trajectory_outcome) : trajectory =
  ...
  (try
   ...
   | exn ->
     Log.Keeper.error
       "Failed to persist summary for %s: %s"
       acc.trace_id
       (Printexc.to_string exn));
  traj
;;
...
let detect_entropy ?(threshold = 3) ?args_json (acc : accumulator) (tool_name : string)
  : (string * int) option
  =
  let recent =
    acc.entries
    ...
    |> List.of_seq
  in
  let count = List.length recent + 1 in
  (* +1 for the upcoming call *)
  if count >= threshold then Some (tool_name, count) else None
;;
...
let aggregate_tool_stats (entries : tool_call_entry list) : tool_stat list =
  ...
  in
  List.iter
    (fun (e : tool_call_entry) ->
       let is_failure =
         Option.is_some e.error
         ||
         match e.gate_decision with
         | Reject _ -> true
         | Pass -> false
       in
  ...
  let stats =
    Hashtbl.fold
      (fun name (durations, succ, fail, cost, _max_ts, last_iso) acc ->
         ...
         let max_d =
           if Array.length sorted > 0 then sorted.(Array.length sorted - 1) else 0
         in
         { name
         ; call_count = count
         ; success_count = succ
  ...
  in
  List.sort
    (fun (a : tool_stat) (b : tool_stat) -> compare b.call_count a.call_count)
    stats
;;
...
let hourly_timeline (entries : tool_call_entry list) : hourly_bucket list =
  ...
  let buckets =
    Hashtbl.fold
      (fun hour (call_count, error_count) acc -> { hour; call_count; error_count } :: acc)
      tbl
      []
  in
...
let tool_call_entry_of_json (json : Yojson.Safe.t) : (tool_call_entry * bool) option =
  try
    ...
    | _ ->
      let gate_decision, parsed_gate = gate_decision_of_json (member "gate" json) in
      Some
...
let read_entries_since_result
  ...
  then { entries = []; gate_decode = { parsed_gate_count = 0; legacy_default_count = 0 } }
  else (
        ...
        | `String "thinking" ->
          Some
            (Thinking
               ...
               ; redacted =
                   (match json |> member "redacted" with
                    | `Bool b -> b
                    | _ -> false)
               })
        | _ ->
