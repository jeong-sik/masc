...
let handle_shell_ir ~call_tool ctx descriptor args =
  match descriptor.Keeper_tool_descriptor.runtime_handler with
  | Tool_execute ->
      let shell_tool_runtime =
        Keeper_shell_tool_command.create_runtime
          ~descriptor_for_internal
          ~call:call_tool
      in
      Some
  ...
  | Tool_keeper_code_query_dispatch ->
    Some
      (Keeper_tool_in_process_runtime.handle_keeper_code_query_with_outcome
         ...
         ~args)
  | Tool_keeper_webmcp_dispatch ->
    Some (Keeper_tool_in_process_runtime.handle_keeper_webmcp_with_outcome ~name ~args)
  | Tool_masc_schedule_dispatch ->
    Some
...
let rec handle ctx ~descriptor ~args =
  ...
  | Shell_ir ->
    handle_shell_ir
      ~call_tool:(fun ~descriptor ~args -> handle ctx ~descriptor ~args)
      ctx
      descriptor
      args
