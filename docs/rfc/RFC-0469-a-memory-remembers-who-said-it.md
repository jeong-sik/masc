...
7. **먼저 확인할 것.** 커밋 저널 항목은 trace 를 갖고(`keeper_librarian.ml:461-464` 의
   ...
   - 2026-09-24 확인: 커밋 저널(`<keeper>.memory-journal.jsonl`)에는 revision·시각·추가된 기억 id 와
     Keeper 단위 trace id 만 있다. 발화는 가리키지 않는다. exact-lane 은 회차마다 입력과 출력
     (`input-*.json`·`output-*.json`)을 남기므로, 기억을 낳은 회차는 찾을 수 있다.
   - 그 회차 입력에 사람 쪽 발화자가 한 명이면 발화자가 정해지고, 둘 이상이면 정할 수 없다.
     09:55~ 회차 입력 1044건 중 사람의 말이 든 79건에서 한 명 45건, 둘 이상 34건(43%)이었다.
   - 결론: 나중에 되살리는 길은 절반쯤만 된다. 이 RFC 를 없앨 만큼은 아니지만, 한 명인 회차에는
     새 칸 없이도 출처를 구할 수 있다는 점을 대안으로 남긴다.
