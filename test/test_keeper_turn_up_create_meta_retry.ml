...
let test_bootstrap_write_retries_and_preserves_heartbeat_fields () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       (match Keeper_turn_up_create.write_initial_meta config bootstrap_payload with
        | Ok () -> ()
        | Error err -> fail ("write_initial_meta failed: " ^ err));
       let final = read_meta_exn config "bootstrap-race" in
       check string "bootstrap-owned field wins" "bootstrap payload wins" final.goal;
       check
         (list string)
         "heartbeat rooms preserved"
         [ "live-room" ]
         final.joined_room_ids;
       check
         (list (pair string int))
         "heartbeat cursors preserved"
         [ "live-room", 42 ]
         final.last_seen_seq_by_room;
       check
         bool
         "version advanced past heartbeat write"
         true
...
let () =
  run
    "keeper_turn_up_create bootstrap meta CAS retry"
    [ ( "write_initial_meta"
      , [ test_case
