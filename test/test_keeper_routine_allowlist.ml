...
let test_transition_force_release_not_matched () =
  ...
  Alcotest.(check bool)
    "force_release does not match"
    false
    (RA.matches ~tool_name:"masc_transition" ~input ~risk_level:RL.Critical)
;;

let test_transition_force_done_not_matched () =
  let input = transition_input "force_done" in
  Alcotest.(check bool)
    "force_done does not match"
    false
    (RA.matches ~tool_name:"masc_transition" ~input ~risk_level:RL.Critical)
;;
...
let test_case_insensitive_action () =
  ...
  Alcotest.(check bool)
    "uppercase CLAIM matches (lowercased)"
    true
    (RA.matches ~tool_name:"masc_transition" ~input ~risk_level:RL.Medium)
;;
...
let test_board_post_high_does_not_match () =
  Alcotest.(check bool)
    "board_post High exceeds max_risk"
    false
    (RA.matches
       ~tool_name:"keeper_board_post"
...
let test_board_post_critical_does_not_match () =
  Alcotest.(check bool)
    "board_post Critical exceeds max_risk"
    false
    (RA.matches ~tool_name:"keeper_board_post" ~input:(`Assoc []) ~risk_level:RL.Critical)
;;
...
let test_keeper_task_done_matches () =
  Alcotest.(check bool)
    "keeper_task_done matches"
    true
    (RA.matches ~tool_name:"keeper_task_done" ~input:(`Assoc []) ~risk_level:RL.Medium)
;;

let test_keeper_task_submit_for_verification_matches () =
  Alcotest.(check bool)
    "keeper_task_submit_for_verification matches"
    true
    (RA.matches
...
let test_keeper_shell_arbitrary_action_does_not_match () =
  ...
  Alcotest.(check bool)
    "keeper_shell action=ls is not auto-approved"
    false
    (RA.matches
...
let test_keeper_shell_git_clone_matches () =
  ...
  Alcotest.(check bool)
    "keeper_shell op=git_clone is auto-approved"
    true
    (RA.matches
       ~tool_name:"keeper_shell"
...
let test_keeper_shell_force_op_does_not_match () =
  (* allowed_actions=[git_clone] — anything else (including dangerous
     ...
     approval. *)
  Alcotest.(check bool)
    "keeper_shell op=force_push is NOT auto-approved"
    false
    (RA.matches
       ~tool_name:"keeper_shell"
...
let test_keeper_shell_op_takes_precedence_over_action () =
  ...
  Alcotest.(check bool)
    ...
    (RA.matches
       ~tool_name:"keeper_shell"
       ~input:(`Assoc [ "action", `String "git_clone"; "op", `String "force_push" ])
       ~risk_level:RL.Medium)
;;
...
let test_keeper_fs_edit_does_not_match () =
  Alcotest.(check bool)
    "keeper_fs_edit never auto-approved"
    false
    (RA.matches ~tool_name:"keeper_fs_edit" ~input:(`Assoc []) ~risk_level:RL.Medium)
;;
...
let test_unknown_tool_does_not_match () =
  Alcotest.(check bool)
    ...
    false
    (RA.matches ~tool_name:"random_tool_xyz" ~input:(`Assoc []) ~risk_level:RL.Low)
;;
...
let test_rule_label_for_claim () =
  ...
  Alcotest.(check (option string))
    "claim has routine label"
    (Some "keeper_routine.masc_transition")
    label
;;
...
let test_rules_summary_is_list () =
  ...
  | `List entries ->
    Alcotest.(check bool)
      "summary has at least the 5 expected rules"
      true
  ...
  | `List entries ->
    let has_masc_transition =
      List.exists
        ...
        entries
    in
    Alcotest.(check bool) "summary includes masc_transition" true has_masc_transition
  | _ -> Alcotest.fail "rules_summary should return `List"
;;
...
let () =
  Alcotest.run
    ...
    [ ( "transition_routine_actions"
      , [ Alcotest.test_case "claim auto-approves" `Quick test_transition_claim_matches
        ; Alcotest.test_case "start auto-approves" `Quick test_transition_start_matches
        ; Alcotest.test_case
            "heartbeat auto-approves"
            `Quick
            test_transition_heartbeat_matches
        ; Alcotest.test_case "done auto-approves" `Quick test_transition_done_matches
        ; Alcotest.test_case
            "release auto-approves"
            `Quick
            test_transition_release_matches
        ; Alcotest.test_case "case-insensitive action" `Quick test_case_insensitive_action
        ] )
    ; ( "transition_gated_actions"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "force_release still gated"
            `Quick
            test_transition_force_release_not_matched
        ; Alcotest.test_case
            "force_done still gated"
            `Quick
            test_transition_force_done_not_matched
        ; Alcotest.test_case
            "unknown action gated"
            `Quick
            test_transition_unknown_action_not_matched
        ; Alcotest.test_case
            "missing action gated"
            `Quick
            test_transition_missing_action_not_matched
        ] )
    ; ( "board_post_risk_ceiling"
      , [ Alcotest.test_case "Low passes" `Quick test_board_post_low_matches
        ; Alcotest.test_case "Medium passes" `Quick test_board_post_medium_matches
        ; Alcotest.test_case "High blocked" `Quick test_board_post_high_does_not_match
        ; Alcotest.test_case
            "Critical blocked"
            `Quick
    ...
    ; ( "keeper_task_lifecycle"
      , [ Alcotest.test_case "keeper_task_claim" `Quick test_keeper_task_claim_matches
        ; Alcotest.test_case "keeper_task_done" `Quick test_keeper_task_done_matches
        ; Alcotest.test_case
    ...
    ; ( "non_routine_tools_never_match"
      , [ Alcotest.test_case
            "keeper_shell action=ls"
            `Quick
            test_keeper_shell_arbitrary_action_does_not_match
        ; Alcotest.test_case "keeper_fs_edit" `Quick test_keeper_fs_edit_does_not_match
        ; Alcotest.test_case "unknown tool" `Quick test_unknown_tool_does_not_match
        ] )
    ; ( "keeper_shell_git_clone_allowlist"
      , [ Alcotest.test_case
            "op=git_clone matches"
            `Quick
            test_keeper_shell_git_clone_matches
        ; Alcotest.test_case
            "op=force_push rejected"
            `Quick
            test_keeper_shell_force_op_does_not_match
        ; Alcotest.test_case
            "op takes precedence over action"
            `Quick
            test_keeper_shell_op_takes_precedence_over_action
        ; Alcotest.test_case
            "Critical risk overrides routine"
            `Quick
            test_keeper_shell_git_clone_above_max_risk_rejected
        ] )
    ; ( "rule_label"
      , [ Alcotest.test_case "claim has label" `Quick test_rule_label_for_claim
        ; Alcotest.test_case
            "cancel has no label"
            `Quick
