...
let test_pause_caller_wins_heartbeat_disk_wins () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let heartbeat_payload = { overflow_view with joined_room_ids = [ "r1"; "r2" ] } in
       ...
       in
       check bool "paused = true (caller wins on cycle field)" true final.paused;
       check
         (list string)
       ...
       check
         bool
         "meta_version moved past heartbeat write"
         true
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let resume_view =
         ...
         | _ -> fail "seed read failed"
       in
       let heartbeat_payload = { resume_view with joined_room_ids = [ "r1"; "r3" ] } in
       (match Keeper_types.write_meta config heartbeat_payload with
        | Ok () -> ()
       ...
       let final =
         ...
         | _ -> fail "final read failed"
       in
       check bool "paused = false (caller wins on cycle field)" false final.paused;
       check
         (list string)
...
let () =
  run
    "Keeper paused-field CAS retain (#9733)"
    [ ( "pause-resume-merge"
      , [ test_case
