...
let loop_summary_json (base_path : string) (state : Autoresearch_types.loop_state)
  : Yojson.Safe.t
  =
  let recent_cycles =
    state.history |> List.filteri (fun i _ -> i < 5) |> List.map cycle_record_json
  in
  let insights =
    state.insights |> List.filteri (fun i _ -> i < 10) |> List.map (fun s -> `String s)
  in
  let link = Autoresearch_storage.load_execution_link_by_loop ~base_path state.loop_id in
  `Assoc
    ...
    ; "program_note", Json_util.string_opt_to_json state.program_note
    ; "warnings", `List (List.map (fun v -> `String v) state.warnings)
    ; "insights", `List insights
    ; "recent_cycles", `List recent_cycles
    ; "error", Json_util.string_opt_to_json state.error_message
    ; ( "session_id"
      , Json_util.string_opt_to_json
          (Option.map (fun (l : Autoresearch_types.execution_link) -> l.session_id) link)
      )
    ; ( "operation_id"
      , Json_util.string_opt_to_json
          (Option.bind link (fun (l : Autoresearch_types.execution_link) ->
             l.operation_id)) )
    ; ( "task_id"
      , Json_util.string_opt_to_json
    ...
    ; ( "linked_at"
      , Json_util.float_opt_to_json
          (Option.map (fun (l : Autoresearch_types.execution_link) -> l.linked_at) link) )
    ; "queued_hypothesis", Json_util.string_opt_to_json state.queued_hypothesis
    ]
;;

let persisted_to_loop_summary_json
      (base_path : string)
      (p : Autoresearch_types.persisted_summary)
  : Yojson.Safe.t
  =
  let link = Autoresearch_storage.load_execution_link_by_loop ~base_path p.loop_id in
  `Assoc
    ...
    ; ( "target_reached"
      , `Bool
          (match p.target_score with
           | None -> false
           | Some target ->
             if p.lower_is_better then p.best_score <= target else p.best_score >= target)
      )
    ...
    ; "program_note", Json_util.string_opt_to_json p.program_note
    ; "warnings", `List (List.map (fun v -> `String v) p.warnings)
    ; "insights", `List []
    ; "recent_cycles", `List []
    ; "error", Json_util.string_opt_to_json p.error_message
    ; ( "session_id"
      , Json_util.string_opt_to_json
          (Option.map (fun (l : Autoresearch_types.execution_link) -> l.session_id) link)
      )
    ...
    ; ( "linked_at"
      , Json_util.float_opt_to_json
          (Option.map (fun (l : Autoresearch_types.execution_link) -> l.linked_at) link) )
    ; "queued_hypothesis", Json_util.string_opt_to_json p.queued_hypothesis
    ]
;;
...
let safe_active_entry_json ~(base_path : string) (state : Autoresearch_types.loop_state) =
  try
    ...
    let sk =
      ...
      ; status_rank =
          (match summary.status with
           | Running -> 0
           | Completed | Stopped | Error -> 1)
      ; neg_updated_at = -.Option.value ~default:0.0 summary.updated_at
      }
    in
...
let persisted_summaries_cache
  : (string, float * Autoresearch_types.persisted_summary) Hashtbl.t
  =
  Hashtbl.create 1024
;;
...
let load_state_cached ~base_path loop_id =
  ...
  try
    ...
    | Some (cached_mtime, summary) when cached_mtime = mtime -> Some summary
    | _ ->
...
let autoresearch_loops_json ~(base_path : string) ?(offset = 0) ?(limit = 100) ()
  : Yojson.Safe.t
  =
  ...
  let active_entries =
    Autoresearch.with_loops_ro (fun () ->
      Hashtbl.fold
        ...
        [])
  in
  let active_ids = List.map (fun (id, _, _) -> id) active_entries in
  ...
  let sliced = sorted |> List.filteri (fun i _ -> i >= offset && i < offset + limit) in
  `Assoc
...
let autoresearch_loops_csv ~(base_path : string) : string =
  ...
  in
  let headers =
    "loop_id,author,goal,metric_fn,model_model,target_file,status,current_cycle,max_cycles,baseline,best_score,best_cycle,total_keeps,total_discards,elapsed_s,updated_at,live\n"
  in
  let rows =
    List.map
      (fun (_, _, json) ->
...
let autoresearch_loop_detail_json
      ~(base_path : string)
      ~(loop_id : string)
      ~(history_limit : int)
  ...
  let detail =
    ...
    | Some state ->
      ...
      let history = Autoresearch.load_cycle_history ~base_path loop_id in
  ...
  | Ok (base_json, insights_list, full_history) ->
    ...
    let history_json =
      ...
      |> List.rev
      |> (fun l ->
      if history_limit > 0 then List.filteri (fun i _ -> i < history_limit) l else l)
      |> List.map cycle_record_json
    in
    let insights_json =
      insights_list |> List.filteri (fun i _ -> i < 10) |> List.map (fun s -> `String s)
    in
...
let validate_loop_id loop_id =
  ...
  let rec all_safe idx =
    ...
    then true
      ...
      | _ -> false)
  in
  if len > 0 && all_safe 0 then Ok () else Error "invalid loop_id"
;;

let git_branch_exists ~repo_root branch =
  match
    Autoresearch.run_capture_lines
      ~workdir:repo_root
...
let ensure_managed_worktree ~base_path ~source_workdir ~loop_id =
  ...
  | Ok repo_root ->
    let warnings = ref [] in
    if Autoresearch.git_is_dirty ~workdir:source_workdir
    then warnings := "source_workdir_dirty" :: !warnings;
    (match Autoresearch.git_current_branch ~workdir:source_workdir with
     ...
     | Some _ | None -> ());
    let workdir = Autoresearch.managed_worktree_dir ~base_path loop_id in
    if Sys.file_exists workdir
    then Ok (workdir, repo_root, List.rev !warnings)
    else (
      ...
      let argv =
        ...
        then [ "worktree"; "add"; workdir; branch ]
        else [ "worktree"; "add"; "-b"; branch; workdir; "HEAD" ]
      in
      match Autoresearch.run_capture_lines ~workdir:repo_root argv with
      | Unix.WEXITED 0, _ -> Ok (workdir, repo_root, List.rev !warnings)
      | _, lines ->
        Error
          (Printf.sprintf
             "failed to create managed worktree: %s"
             (String.concat "\n" lines)))
;;
...
let delete_session_link ~base_path loop_id =
  ...
  | Some link ->
    let session_path = Autoresearch.session_link_file ~base_path link.session_id in
...
let delete_loop_json
  ...
  | Ok () ->
    ...
    in
    if not can_delete
    then Error "Access denied: you can only delete loops that you started."
    else (
      let state_or_persisted =
        Autoresearch.with_loops_rw (fun () ->
          ...
          if !Autoresearch.latest_loop_id = Some loop_id
          then Autoresearch.latest_loop_id := None;
          match active with
          | Some state -> Ok state
          | None ->
            (match load_state_cached ~base_path loop_id with
      ...
      | Ok state ->
        ...
        (match Autoresearch.git_top_level ~workdir:state.source_workdir with
         | Ok repo_root ->
           let workdir = Autoresearch.managed_worktree_dir ~base_path loop_id in
           if Sys.file_exists workdir
           then
           ...
           then
             ignore
               (Autoresearch.run_capture_lines
                  ...
                  [ "branch"; "-D"; branch ])
         | Error msg ->
           Log.Autoresearch.warn "delete loop git cleanup skipped for %s: %s" loop_id msg);
        let bundle_dir = Autoresearch.results_dir ~base_path loop_id in
        if Sys.file_exists bundle_dir then rm_rf bundle_dir;
        Ok
          (`Assoc
              [ "ok", `Bool true; "action", `String "delete"; "loop_id", `String loop_id ]))
;;
...
let start_loop_json ~(base_path : string) ~(args : Yojson.Safe.t)
  ...
  | `Assoc fields when List.mem_assoc "error" fields ->
    ...
    in
    Error error_msg
  | json -> Ok (`Assoc [ "ok", `Bool true; "action", `String "start"; "loop", json ])
;;
