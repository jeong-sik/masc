...
let with_registry f =
  ...
  Reg.clear ();
  Fun.protect ~finally:(fun () -> Reg.clear ()) f
;;
...
let test_bump_increments_each_violated () =
  ...
  List.iter
    (fun (inv, b) ->
       ...
       check (float 0.0001) (Printf.sprintf "counter %s increments by 1" inv) (b +. 1.0) a)
    before
;;
...
let test_bump_selective_increments () =
  ...
  check
    (float 0.0001)
    "phase_turn_alignment +1"
    (pt_before +. 1.0)
    (read ~keeper ~invariant:"PhaseTurnAlignment");
  check
    (float 0.0001)
    "no_cascade_before_measurement unchanged"
    nc_before
    (read ~keeper ~invariant:"NoCascadeBeforeMeasurement");
  check
    (float 0.0001)
    "compaction_atomicity +1"
    (ca_before +. 1.0)
    (read ~keeper ~invariant:"CompactionAtomicity");
  check
    (float 0.0001)
    "event_priority_monotone unchanged"
    ep_before
    (read ~keeper ~invariant:"EventPriorityMonotone")
;;
...
let test_all_invariant_keys_mapped () =
  let strings = List.map Obs.invariant_key_to_string Obs.all_invariant_keys in
  check int "4 invariant keys" 4 (List.length strings);
  check
    (list string)
    "key names match alert-rule labels"
    [ "PhaseTurnAlignment"
    ; "NoCascadeBeforeMeasurement"
    ; "CompactionAtomicity"
    ; "EventPriorityMonotone"
...
let test_snapshot_reports_collapsed_from_for_stable_phase () =
  with_registry (fun () ->
    ...
    check
      (option string)
      "stable snapshot exposes raw paused phase"
      (Some "paused")
      (Json.member "collapsed_from" snapshot |> Json.to_string_option);
    check
      (option string)
      "collapsed snapshot stays on Stable composite phase"
      (Some "Stable")
...
let test_snapshot_omits_collapsed_from_for_active_phase () =
  with_registry (fun () ->
    ...
    check
      (option string)
      "active composite phase does not expose raw collapse"
      None
      (Json.member "collapsed_from" snapshot |> Json.to_string_option);
    check
      (option string)
      "running phase preserved"
      (Some "Running")
      (Json.member "phase" snapshot |> Json.to_string_option))
;;
...
let test_snapshot_reports_keeper_identity () =
  with_registry (fun () ->
    ...
    check
      (option string)
      "snapshot exposes registry keeper identity"
      (Some keeper_name)
...
let test_direct_turn_mutations_emit_composite_changed () =
  with_registry (fun () ->
    ...
    with_sse_capture "test-direct-turn-composite-tick" (fun events ->
      ...
      check bool "turn start emits composite tick" true (saw_composite_changed events);
      events := [];
      Reg.set_turn_cascade_state ~base_path keeper_name Reg.Cascade_trying;
      check
        bool
        "turn cascade update emits composite tick"
        true
        (saw_composite_changed events);
      events := [];
      Reg.mark_turn_finished ~base_path keeper_name;
      check bool "turn finish emits composite tick" true (saw_composite_changed events)))
;;
...
let () =
  run
    ...
    ; ( "snapshot projection"
      , [ test_case
            "stable phase exposes collapsed_from"
            `Quick
            test_snapshot_reports_collapsed_from_for_stable_phase
        ; test_case
            "active phase leaves collapsed_from null"
            `Quick
            test_snapshot_omits_collapsed_from_for_active_phase
        ; test_case
            "snapshot exposes keeper identity"
            `Quick
            test_snapshot_reports_keeper_identity
        ] )
    ; ( "composite change signals"
      , [ test_case
            "direct turn mutations emit composite tick"
            `Quick
            test_direct_turn_mutations_emit_composite_changed
        ] )
    ]
;;
