...
let run_curl_json ?token ?(max_time_sec = 5) ~port ~path ~session_id ~payload () =
  ...
  Fun.protect ~finally:(fun () -> close_out_noerr oc) (fun () -> output_string oc payload);
  let base_args =
    [ "curl"
    ; "-sS"
    ; "--http1.1"
...
let tools_list_payload ~id =
  jsonrpc_payload ~id ~method_name:"tools/list" ~params:(`Assoc [])
;;
...
let require_http_ok label result =
  ...
  | Some code ->
    fail
      (Printf.sprintf
         "%s returned HTTP %d (curl_exit=%d stderr=%s body=%s)"
         label
         code
         result.curl_exit
         result.stderr
...
let extract_nickname_from_join_result result =
  ...
  | _ ->
    ...
    let tick_prefix = "\xe2\x9c\x85 " in
    (* UTF-8 for check mark emoji *)
    (match
       List.find_map
         (fun line ->
            ...
            if
              ...
              > String.length tick_prefix + String.length already_suffix
              && String.sub trimmed 0 (String.length tick_prefix) = tick_prefix
            then (
              let rest =
                String.sub
                  trimmed
                  (String.length tick_prefix)
     ...
     | None ->
       fail (Printf.sprintf "failed to extract nickname from join result:\n%s" result))
;;
...
let with_server ?(host = "127.0.0.1") ?(enable_auth = true) f =
      ...
      let supervisor_token =
        match
          Masc_mcp.Auth.create_token
            config.base_path
            ~agent_name:supervisor_nickname
            ~role:Types.Admin
        with
        ...
        | Error err ->
          fail
            (Printf.sprintf
               "failed to create supervisor token: %s"
               (Types.masc_error_to_string err))
      in
      let create_worker_token agent_name =
        match
          Masc_mcp.Auth.create_token config.base_path ~agent_name ~role:Types.Worker
        with
        | Ok (token, _cred) -> token
        | Error err ->
          fail
            (Printf.sprintf
               "failed to create %s token: %s"
               agent_name
               (Types.masc_error_to_string err))
      in
      ( supervisor_token
  ...
  in
  let argv =
    [| exe; "--host"; host; "--port"; string_of_int port; "--base-path"; base_path |]
  in
  ...
  Fun.protect ~finally:cleanup (fun () ->
    f
      ...
      ~supervisor_nickname
      ~planner_nickname
      ~implementer_a_nickname
      ~implementer_b_nickname)
;;

let _test_operator_mcp_supervision_impl () = Alcotest.skip ()
...
let test_mcp_requires_auth_when_bound_non_loopback () =
  ...
  @@ fun ~port
       ~supervisor_token:_
       ~planner_token:_
       ~implementer_a_token:_
       ~implementer_b_token:_
       ~supervisor_nickname:_
       ~planner_nickname:_
       ~implementer_a_nickname:_
  ...
  check
    ...
    "strict auth message"
    true
    (contains_substr "requires room auth enabled with require_token=true" result.body)
;;
...
let test_agent_json_route_served_on_canonical_path () =
  ...
  let rec fetch_agent_card retries =
    ...
    | Some 200 ->
      let json =
        try Yojson.Safe.from_string result.body with
        | Yojson.Json_error err ->
          fail (Printf.sprintf "agent.json invalid JSON: %s\nbody=%s" err result.body)
      in
...
let test_operator_mcp_supervision () = Alcotest.skip ()
...
let () =
  run
    ...
    [ ( "operator"
      , [ test_case
            "remote operator supervises team session over MCP"
            `Slow
            test_operator_mcp_supervision
        ; test_case
            "full mcp requires auth on non-loopback bind"
            `Slow
            test_mcp_requires_auth_when_bound_non_loopback
        ; test_case
            "canonical agent discovery route"
            `Quick
