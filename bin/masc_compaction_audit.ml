...
let usage =
  {|Usage: masc_compaction_audit [OPTIONS]
  ...
  then cfg
    ...
    | "--until" when i + 1 < Array.length argv ->
      parse_args
        argv
        (i + 2)
        { cfg with until_ts = parse_date_arg "until" argv.(i + 1) +. 86399.0 }
    | "--keeper" when i + 1 < Array.length argv ->
      parse_args argv (i + 2) { cfg with keeper = Some argv.(i + 1) }
    | "--orphans-only" -> parse_args argv (i + 1) { cfg with orphans_only = true }
    | "--prune" -> parse_args argv (i + 1) { cfg with prune = true }
...
let print_orphan_start (s : KCA.start_record) =
  Printf.printf
    "[%s] %-18s %-10s %s  ORPHAN_START (compaction did not complete)\n"
    (format_ts s.ts_unix)
    s.keeper_name
    (KCA.trigger_to_string s.trigger)
    s.compaction_id
;;

let print_orphan_complete (c : KCA.complete_record) =
  Printf.printf
    "[%s] %-18s %-10s %s  ORPHAN_COMPLETE (start event missing)\n"
    (format_ts c.ts_unix)
    c.keeper_name
    "?"
    c.compaction_id
  ...
  | Ok rows ->
    ...
    let filtered =
      ...
      then
        List.filter
          (function
            | KCA.Orphan_start _ | KCA.Orphan_complete _ -> true
            | _ -> false)
          paired
      else paired
    in
    if filtered = []
    then print_endline "(no rows in range)"
    else
      List.iter
        (function
...
let run_prune cfg =
  ...
  Eio_main.run
  @@ fun _env ->
  let n = KCA.prune_older_than ~base_path:base ~retention_days:cfg.retention_days in
  Printf.printf
...
let () =
  ...
  if cfg.prune then run_prune cfg else run_list cfg
;;
