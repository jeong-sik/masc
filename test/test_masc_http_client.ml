...
let count_open_fds () =
  let fd_dir =
    if Sys.file_exists "/dev/fd" then
      "/dev/fd"
    else if Sys.file_exists "/proc/self/fd" then
      "/proc/self/fd"
    else
      Alcotest.fail "no fd directory available for regression test"
  in
...
let rec wait_for_fd_baseline ~clock ~baseline ~allowed_extra_fds ~attempts_left =
  ...
  if current <= baseline + allowed_extra_fds || attempts_left <= 0 then
    current
  else (
    Eio.Time.sleep clock 0.01;
    wait_for_fd_baseline ~clock ~baseline ~allowed_extra_fds
      ~attempts_left:(attempts_left - 1))

let wrap_with_close_counter close_count
...
let exercise_client ~sw ~net ~https ~request_count ~url =
  ...
  for _ = 1 to request_count do
    let resp, body =
      Cohttp_eio.Client.get client ~sw ~headers:closable_headers uri
    in
    Alcotest.(check int) "status" 200
      (Cohttp.Response.status resp |> Cohttp.Code.code_of_status);
    let body_text =
      Eio.Buf_read.(parse_exn take_all) body ~max_size:1024
    in
...
let test_https_wrapper_close_is_invoked_for_each_connection () =
  ...
  let wrap _uri raw =
    wrap_with_close_counter close_count
  ...
  Eio.Switch.run (fun client_sw ->
    exercise_client ~sw:client_sw ~net ~https:(Some wrap) ~request_count
      ~url:(Printf.sprintf "https://127.0.0.1:%d/wrapped" port));
  check int "wrapper close count matches request count" request_count
    (Atomic.get close_count)

let () =
  run "masc_http_client"
    [
      ( "closing",
        [
          test_case "release closes all tracked connections" `Quick
            test_release_closes_all_tracked_connections;
          test_case "https wrapper close is invoked for each connection" `Quick
