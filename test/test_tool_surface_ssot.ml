...
let check_set_equal label ~expected ~actual =
  ...
  Alcotest.(check bool) (label ^ " sets equal") true (SS.equal expected actual)
;;
...
let test_local_worker_public_tools_resolvable () =
  let public_names = set_of (Tool_catalog.tools_for_surface Tool_catalog.Local_worker) in
  ...
  then
    Alcotest.failf
      "Local_worker public tools missing schemas: {%s}"
      (String.concat ", " (SS.elements missing));
  Alcotest.(check bool) "Local_worker public tools resolvable" true (SS.is_empty missing)
;;
...
let test_session_min_and_local_worker_share_core () =
  ...
  then
    Alcotest.failf
      "Session_min and Local_worker missing shared core: {%s}"
      (String.concat ", " (SS.elements missing));
...
let public_keeper_denied_overlap_allowed =
  set_of
    [ "masc_persona_generate"; "masc_persona_save"; "masc_keeper_create_from_persona" ]
;;
...
let test_keeper_denied_public_mcp_overlap_is_explicit () =
  ...
  if not (SS.is_empty unexpected)
  then
    Alcotest.failf
      "Unexpected Keeper_denied/Public_mcp overlap: {%s}"
      (String.concat ", " (SS.elements unexpected));
  if not (SS.is_empty missing)
  then
    Alcotest.failf
      "Expected Keeper_denied/Public_mcp overlap missing: {%s}"
      (String.concat ", " (SS.elements missing));
  Alcotest.(check bool)
    "Keeper_denied/Public_mcp overlap explicit"
    true
...
let test_keeper_internal_disjoint_from_public_mcp () =
  ...
  if not (SS.is_empty overlap)
  then
    Alcotest.failf
      "Keeper_internal overlaps with Public_mcp: {%s}"
      (String.concat ", " (SS.elements overlap));
  Alcotest.(check bool)
    "Keeper_internal disjoint from Public_mcp"
    true
    (SS.is_empty overlap)
;;
...
let test_destructive_check_tools_are_privileged () =
  ...
  List.iter
    (fun name ->
       if String.starts_with ~prefix:"keeper_" name
       then
         Alcotest.(check bool)
           (name ^ " is privileged")
           true
...
let test_privileged_keeper_tools_are_internal () =
  ...
  List.iter
    (fun name ->
       if String.starts_with ~prefix:"keeper_" name
       then
         Alcotest.(check bool) (name ^ " in Keeper_internal") true (SS.mem name internal))
...
let test_replacement_targets_have_schemas () =
  ...
  let all_schema_names =
    List.map (fun (s : Types.tool_schema) -> s.name) Config.raw_all_tool_schemas
  in
  List.iter
    (fun name ->
       ...
       | Some public_name ->
         Alcotest.(check bool)
           ...
           true
           (List.mem public_name all_schema_names)
       | None -> ())
    internal
;;
...
let test_keeper_internal_tools_have_schemas () =
  ...
  let schema_names =
    ...
    |> SS.of_list
  in
  let internal_names = Tool_catalog_surfaces.keeper_internal_tools |> SS.of_list in
  ...
  then
    Alcotest.failf
      ...
      (String.concat ", " (SS.elements missing));
  Alcotest.(check bool) "all internal tools have schemas" true (SS.is_empty missing)
;;
...
let test_no_orphaned_tools () =
  ...
  let on_any_surface name =
    List.exists
      (fun surface -> Tool_catalog.is_on_surface surface name)
      Tool_catalog.all_surfaces
  in
  ...
  in
  if orphaned <> []
  then Alcotest.failf "Orphaned tools (no surface): {%s}" (String.concat ", " orphaned);
...
let test_public_mcp_count_cap () =
  ...
  if count > 80 then Alcotest.failf "Public_mcp has %d tools (cap: 80)" count;
...
let test_system_internal_not_visible () =
  ...
  let visible = List.filter (fun name -> Tool_catalog.is_visible name) system_tools in
  if visible <> []
  then
    Alcotest.failf
      "System_internal tools visible in tools/list: {%s}"
...
let test_system_internal_callable () =
  ...
  let system_tools = Tool_catalog.tools_for_surface Tool_catalog.System_internal in
  let uncallable =
    List.filter (fun name -> not (Tool_catalog.allow_direct_call name)) system_tools
  in
  if uncallable <> []
  then
    Alcotest.failf
      "System_internal tools not callable: {%s}"
...
let test_pruned_tools_registered_as_deprecated () =
  ...
  let deprecated_names = List.map fst Tool_catalog.deprecated_tool_entries in
  List.iter
    (fun name ->
       Alcotest.(check bool)
         (name ^ " is Deprecated")
       ...
       Alcotest.(check bool) (name ^ " hidden") false (Tool_catalog.is_visible name);
       Alcotest.(check bool)
         (name ^ " callable")
         true
...
let test_role_catalogs_drop_stale_entries_when_built () =
  ...
  Alcotest.(check bool)
    "worker role excludes portal_open"
    false
    (List.mem "masc_portal_open" worker_tools);
  Alcotest.(check bool)
    "coordinator role excludes portal_open"
    false
...
let test_local_worker_compat_passthrough_schemas_match_registry () =
  List.iter
    (fun (schema : Types.tool_schema) ->
       ...
       | None -> Alcotest.failf "missing raw schema for passthrough tool %s" schema.name
       | Some raw_schema ->
  ...
  Alcotest.run
    ...
    [ ( "parity"
      , [ Alcotest.test_case "Public_mcp parity" `Quick test_public_mcp_parity
        ...
        ; Alcotest.test_case
            "Local_worker public tools resolvable"
            `Quick
    ...
    ; ( "invariants"
      , [ Alcotest.test_case
            "Session_min ∩ Local_worker shared core"
            `Quick
            test_session_min_and_local_worker_share_core
        ; Alcotest.test_case
            "Keeper_denied/Public_mcp overlap is explicit"
            `Quick
            test_keeper_denied_public_mcp_overlap_is_explicit
        ; Alcotest.test_case
            "Keeper_internal ∩ Public_mcp = ∅"
            `Quick
            test_keeper_internal_disjoint_from_public_mcp
        ; Alcotest.test_case
            "Keeper_internal contains known tools"
            `Quick
            test_keeper_internal_contains_known_tools
        ; Alcotest.test_case
            "Keeper voice replacement contract"
            `Quick
            test_keeper_voice_replacement_contract
        ; Alcotest.test_case
            "is_on_surface consistent"
            `Quick
            test_is_on_surface_consistent
        ] )
    ; ( "cross_classification"
      , [ Alcotest.test_case
            "destructive check tools are privileged"
            `Quick
            test_destructive_check_tools_are_privileged
        ; Alcotest.test_case
            "privileged keeper tools are internal"
            `Quick
            test_privileged_keeper_tools_are_internal
        ; Alcotest.test_case
            "replacement targets have schemas"
            `Quick
            test_replacement_targets_have_schemas
        ; Alcotest.test_case
            "keeper internal tools have schemas"
            `Quick
            test_keeper_internal_tools_have_schemas
        ; Alcotest.test_case
            "workspace_mutating canonical used"
            `Quick
            test_workspace_mutating_canonical_used
        ; Alcotest.test_case
            "role catalogs use only available tools"
            `Quick
            test_role_catalogs_only_expose_available_tools
        ; Alcotest.test_case
            "built role catalogs drop stale entries"
            `Quick
            test_role_catalogs_drop_stale_entries_when_built
        ; Alcotest.test_case
            "local worker passthrough schemas use registry"
            `Quick
            test_local_worker_compat_passthrough_schemas_match_registry
        ] )
    ; ( "ssot_validation"
      , [ Alcotest.test_case "no orphaned tools" `Quick test_no_orphaned_tools
        ; Alcotest.test_case "Public_mcp count cap <= 80" `Quick test_public_mcp_count_cap
        ; Alcotest.test_case
            "System_internal not visible"
            `Quick
            test_system_internal_not_visible
        ; Alcotest.test_case
            "System_internal callable"
            `Quick
            test_system_internal_callable
        ; Alcotest.test_case
            "pruned tools registered as Deprecated"
            `Quick
