        ...
        (match
         ...
         | Error e ->
           Tool_result.error
             ~tool_name
             ~start_time
             (Printf.sprintf "%s" (board_error_to_string e))))
;;
...
let string_opt_arg args key = Option.bind (get_string_opt args key) trim_nonempty_string
...
(** Re-exported from {!Tool_board_schemas}. *)
  ...
  then Tool_result.error ~tool_name ~start_time "post_id is required"
    ...
    | Error e ->
      Tool_result.error
        ~tool_name
        ~start_time
        (Printf.sprintf "Delete failed: %s" (board_error_to_string e)))
;;
...
let handle_tool name args =
  ...
  | "masc_board_curation_read" ->
    handle_board_curation_read ~tool_name:name ~start_time args
  | "masc_board_curation_submit" ->
    handle_board_curation_submit ~tool_name:name ~start_time args
  | "masc_board_delete" ->
