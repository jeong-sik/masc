...
const runtimeBlockerLabels = {
  ...
  // The eleven the server had been sending with no label here since #29665
  // widened the wire type; `satisfies` names the gap, so this list cannot
  // fall behind again without the build saying so.
  agent_core_input_required: 'Agent Core 입력 대기',
  capacity_backpressure: '용량 한계로 밀림',
  gate_replay_repair_required: 'Gate 재생 복구 필요',
  incomplete_tool_transcript: '도구 기록 미완',
  internal_bridge_exception: '내부 브리지 예외',
  internal_contract_rejected: '내부 계약 거부',
  internal_unhandled_exception: '내부 미처리 예외',
  provider_attempt_effect_fenced: '공급자 시도 효과 차단',
  receipt_persistence_failed: '영수증 저장 실패',
  terminal_effect_failed: '종단 효과 실패',
