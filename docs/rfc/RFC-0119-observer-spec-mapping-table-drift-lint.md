---
rfc: "0119"
title: "Observer spec mapping table drift lint — guard-marker validator for OCaml↔TLA+ collapse projections"
status: Implemented
created: 2026-05-17
updated: 2026-05-22
author: vincent
...
# RFC-0119: Observer spec mapping table drift lint — guard-marker validator for OCaml↔TLA+ collapse projections
...
2. **Drift class 의 *6번째* 변형**: RFC-0114~0118 가 *typed* spec-runtime drift, **본 RFC 는 *doc-only* spec-runtime drift**. type level 에서 catch 불가 (PPX 가 못 봄), validator 가 *주석 + guard marker* parsing 필요.
...
**Layer A — Guard marker convention**

모든 observer spec 의 mapping table 가 *marker block* 으로 둘러쌈:
...
| P4 | Guard marker convention adoption — 모든 observer spec mapping table 가 `MAPPING-{TABLE,UNMODELED}-{BEGIN,END}` 블록 | TLA+ 주석 syntax 유효, 기존 reader 영향 0 |
| P5 | `scripts/lint-spec-mapping-drift.sh` — OCaml constructor 와 marker block 정합 검증 | 5+ observer spec 모두 lint PASS |
...
1. **Q1**: guard marker 가 TLA+ comment syntax 안 — 다른 reader (TLC, TLAPS) 무시? **잠정**: YES, comment 이므로 TLC 영향 0. P2 PR 가 검증.
...
- **Risk 1**: guard marker convention 이 *기존 spec reader* 의 인지 부담 — 새 syntax 학습. → P4 의 doc commit 가 `docs/spec/MAPPING-TABLE-CONVENTION.md` (또는 `specs/keeper-state-machine/README.md` 안) 가이드 명시.
- **Risk 2**: P5 lint 가 *false positive* — PPX 비활성 build 또는 OCaml parse 실패 시. → fail-soft: lint skip + warn, hard fail 은 명시적 spec PR 만.
- **Risk 3**: P3 multi-spec sweep 가 *기존 spec 본문* 변경 — TLC re-verify 부담. → P3 의 각 PR 가 TLC PASS 확인 (guard marker 가 comment 라 결과 동일해야).
...
Rollback: P6 CI 비활성. P5 lint 비활성. P4 marker block 그대로 남음 (해롭지 않음).
...
- [ ] P4: guard marker convention adoption — all observer spec.
