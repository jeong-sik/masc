...
parser.add_argument('--verified-binaries', type=Path, required=True)
...
verified = json.loads(args.verified_binaries.read_text())
assert verified['source_commit'] == source, 'binary proof candidate differs'
binaries = {}
for name in ('masc-macos-arm64', 'masc-tui-macos-arm64', 'masc-browser-host-macos-arm64'):
    ...
    assert digest == verified['sha256'][name], f'binary differs from verified candidate: {name}'
...
manifest = {'source_commit': source, 'server_sha256': hashlib.sha256(server.read_bytes()).hexdigest(),
            'binaries': binaries, 'binary_source_proof': verified,
            'runtime_provenance': json.loads((runtime / 'runtime-provenance.json').read_text()),
            'packages': {}, 'exports': [], 'origin': 'candidate binary skills-refresh --export-to'}
for name in ('browser-lanes', 'browser-live-click-content'):
