...
let run
  ...
  ()
  =
  (* RFC-0256: snapshot the per-keeper tool-emission accumulator synchronously
  ...
  (* (1) deterministic write, (2) librarian extraction, (3) compaction run on
     this keeper's memory lane (RFC-0256), detached from the turn lane. All
  ...
  (try
   ...
   | exn ->
     ...
     Log.Keeper.warn ~keeper_name:meta.name
       ...
       (Printexc.to_string exn))
  in
  (* RFC-0256: detach (1)-(3) onto the per-keeper memory lane. When the executor
