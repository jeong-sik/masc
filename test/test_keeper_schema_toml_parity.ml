...
let expected =
    ...
    ; {|masc_keeper_up|}, {|Create or update a durable keeper. Keepers auto-start on server boot and are reconciled back into live presence.|}, {|{"additionalProperties":false,"properties":{"allowed_paths":{"description":"Restrict file writes to these path prefixes. Empty list means playground-only (.masc/playground/<name>/).","items":{"type":"string"},"type":"array"},"autoboot_enabled":{"description":"If false, persist the keeper but skip auto-start on future server boots.","type":"boolean"},"autonomous_wake_prompt":{"description":"Optional: user message this keeper's autonomous turns are woken with, overriding the fleet autonomous.wake_prompt. Non-blank, at most 2048 bytes; it is appended to the durable checkpoint every autonomous turn. Use null to clear.","type":"string"},"instructions":{"description":"Complete Keeper instructions, written to the keeper TOML declaration (kept across compaction/handoff).","type":"string"},"max_context_override":{"description":"Optional: absolute context token limit override for this keeper. Use 0 to clear the override.","type":"integer"},"mention_targets":{"description":"Exact direct-mention tokens that can wake the keeper in workspace traffic (for example ['alpha']).","items":{"type":"string"},"type":"array"},"name":{"description":"Keeper handle (stable). Example: 'keeper-helper'","type":"string"},"proactive_enabled":{"description":"If true, scheduled keeper cycles may produce proactive responses.","type":"boolean"},"runtime_id":{"description":"Optional opaque runtime id. Writes the keeper assignment to runtime.toml.","type":"string"},"sandbox_profile":{"default":"local","description":"Sandbox isolation profile. New keepers default to local with playground-only writes when omitted; select docker explicitly when required.","enum":["local","docker"],"type":"string"}},"required":["name"],"type":"object"}|}
    ; {|masc_keeper_status|}, {|Get keeper status (keepalive/live/reconcile state plus current context and monitoring tails).|}, {|{"additionalProperties":false,"properties":{"fast":{"description":"Enable fast mode (skip heavy sections unless explicitly enabled).","type":"boolean"},"include_context":{"description":"Include checkpoint-derived context stats (default: !fast).","type":"boolean"},"include_history_tail":{"description":"Include recent history tail + fragment counters (default: !fast).","type":"boolean"},"include_metrics_overview":{"description":"Include metrics overview + skill route scan (default: !fast).","type":"boolean"},"name":{"description":"Non-blank Keeper handle. Optional; defaults to the caller only when omitted.","minLength":1,"pattern":"\\S","type":"string"},"tail_bytes":{"description":"How many bytes from the end of files to scan for tails (default: 60000; range: 1000..65536).","maximum":65536,"minimum":1000,"type":"integer"},"tail_messages":{"description":"How many recent history messages to include (default: 5; maximum: 65536).","maximum":65536,"minimum":0,"type":"integer"},"tail_order":{"description":"Ordering for metrics/history tails and recent memory notes. Default: oldest_first.","enum":["oldest_first","newest_first"],"type":"string"},"tail_turns":{"description":"How many recent turns to include from keeper metrics (default: 3; maximum: 6553).","maximum":6553,"minimum":0,"type":"integer"}},"required":[],"type":"object"}|}
    ; {|masc_keeper_delegate|}, {|Submit one typed Keeper chat operation and return its durable operation_id without waiting for the turn.|}, {|{"additionalProperties":false,"properties":{"prompt":{"type":"string"},"target":{"additionalProperties":false,"properties":{"kind":{"enum":["keeper"],"type":"string"},"name":{"type":"string"}},"required":["kind","name"],"type":"object"}},"required":["target","prompt"],"type":"object"}|}
    ; {|masc_keeper_delegate_status|}, {|Read one Keeper chat operation by exact target and operation_id.|}, {|{"additionalProperties":false,"properties":{"operation_id":{"type":"string"},"target":{"additionalProperties":false,"properties":{"kind":{"enum":["keeper"],"type":"string"},"name":{"type":"string"}},"required":["kind","name"],"type":"object"}},"required":["target","operation_id"],"type":"object"}|}
    ...
    ; {|masc_keeper_list|}, {|List known keepers from persisted keeper metadata. The response carries total (keepers known before limit), limit (the value applied) and truncated, so a short answer is distinguishable from a complete one.|}, {|{"additionalProperties":false,"properties":{"detailed":{"description":"Return keeper summaries (model/context/handoff/compaction) instead of names only.","type":"boolean"},"limit":{"description":"Max keepers to return (default: 50). Names are sorted and cut from the end, so a low limit hides whatever sorts last; check truncated in the response.","type":"integer"}},"type":"object"}|}
    ...
    ; {|masc_keeper_compact|}, {|Trigger explicit context compaction for a non-terminal keeper checkpoint.|}, {|{"additionalProperties":false,"properties":{"name":{"description":"Keeper handle","type":"string"}},"required":["name"],"type":"object"}|}
