...
let init () =
  if Otel_config.enabled && not (Atomic.get initialized)
  then (
...
let setup_exporter ~sw (env : Eio_unix.Stdenv.base) =
  ...
  let setup () =
    let config = Opentelemetry_client_cohttp_eio.Config.make ~url:endpoint () in
...
let shutdown ?(enabled = Otel_config.enabled) () =
  if enabled && Atomic.get exporter_active
  then (
