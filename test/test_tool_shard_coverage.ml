...
let test_base_tools_names () =
  let names = List.map (fun (t : Masc_mcp.Masc_model.tool_def) -> t.tool_name)
...
let test_revoke_voice_removes_all_tools () =
  ...
  let voice_before = List.filter (fun (t : Masc_mcp.Masc_model.tool_def) ->
  ...
  | Ok shards_after ->
    ...
    let voice_after = List.filter (fun (t : Masc_mcp.Masc_model.tool_def) ->
      let n = t.tool_name in
