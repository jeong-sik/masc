  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Eio.Switch.run (fun sw ->
         ...
         check
           bool
           "summary removed from execution payload"
           true
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Eio.Switch.run (fun sw ->
         ...
         let key_absent key j = List.assoc_opt key (to_assoc j) = None in
         ...
         check
           bool
           "namespace_mode carrier removed"
           true
...
let test_dashboard_execution_namespace_status () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         (option string)
         "room state current_room flattened"
         (Some "default")
       ...
       Eio.Switch.run (fun sw ->
         ...
         in
         let open Yojson.Safe.Util in
         let key_absent key json = List.assoc_opt key (to_assoc json) = None in
         let status = json |> member "status" in
         check bool "status namespace_id removed" true (key_absent "namespace_id" status);
         check bool "status namespace removed" true (key_absent "namespace" status);
         check
           bool
           "status current_namespace removed"
           true
           (key_absent "current_namespace" status);
         check
           bool
           "status namespace_mode removed"
           true
           (key_absent "namespace_mode" status);
         check bool "legacy room removed" true (key_absent "room" status);
         check
           bool
           "legacy room base path removed"
           true
         ...
         check
           string
           "batch cluster exposed"
           "default"
           (batch_status |> member "cluster" |> to_string);
         check
           bool
           "batch current_namespace removed"
           true
           (key_absent "current_namespace" batch_status);
         check
           bool
           "batch current_room removed"
           true
...
let test_dashboard_shell_namespace_status () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         string
         "shell cluster exposed"
         "default"
         (status |> member "cluster" |> to_string);
       check bool "shell namespace_id removed" true (key_absent "namespace_id" status);
       check bool "shell namespace removed" true (key_absent "namespace" status);
       check
         bool
         "shell current_namespace removed"
         true
         (key_absent "current_namespace" status);
       check bool "shell current_room removed" true (key_absent "current_room" status);
       check bool "shell namespace_mode removed" true (key_absent "namespace_mode" status);
       check
         string
         "shell coordination root surfaced"
         dir
         (status |> member "coordination_root" |> to_string);
       check
         string
         "shell workspace path surfaced"
         dir
         (status |> member "workspace_path" |> to_string);
       check
         bool
         "shell workspace differs false when same root"
         false
         (status |> member "workspace_differs" |> to_bool);
       check
         string
         "shell diagnostics surface"
         "shell"
...
let test_dashboard_shell_surfaces_workspace_when_different () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         string
         "shell coordination root remains base path"
         dir
         (status |> member "coordination_root" |> to_string);
       check
         string
         "shell workspace path uses input path"
         workspace
         (status |> member "workspace_path" |> to_string);
       check
         bool
         "shell workspace differs true when worktree input"
         true
         (status |> member "workspace_differs" |> to_bool);
       check
         string
         "diagnostics coordination root surfaced"
         dir
         (json
          |> member "projection_diagnostics"
          |> member "coordination_root"
          |> to_string);
       check
         string
         "diagnostics workspace path surfaced"
         workspace
         (json |> member "projection_diagnostics" |> member "workspace_path" |> to_string))
;;
...
let append_execution_receipt config ~keeper_name =
  ...
  let receipt : Lib.Keeper_execution_receipt.t =
    ...
    ; sandbox_kind = Lib.Keeper_execution_receipt.sandbox_kind_of_meta meta
       ...
       Eio.Switch.run (fun sw ->
         Fun.protect
           ...
           (fun () ->
              ...
              check
                int
                "shell active keeper count uses runtime"
                0
                (counts |> member "keepers" |> to_int);
              check
                bool
                "shell configured keeper inventory stays visible"
                true
...
let test_dashboard_shell_excludes_keeper_agents_from_general_count () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Eio.Switch.run (fun sw ->
         Fun.protect
           ~finally:(fun () -> Masc_mcp.Keeper_keepalive.stop_keepalive "sangsu")
           (fun () ->
              ...
              check
                int
                "keeper-backed room has no general agents"
                0
                (counts |> member "agents" |> to_int);
              check
                int
                "stopped keeper is not counted as active"
                0
                (counts |> member "keepers" |> to_int);
              check
                bool
                "configured keeper inventory remains visible"
                true
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Eio.Switch.run (fun sw ->
         ...
         let has_test_agent =
           List.exists
             (fun row -> row |> member "name" |> to_string = "test-agent-fox")
             (worker_briefs @ offline_worker_briefs)
         in
         check
           bool
           "freshly joined agent should not appear stale in execution worker briefs"
           false
           has_test_agent))
;;
       ...
       Eio.Switch.run (fun sw ->
         Fun.protect
           ...
           (fun () ->
              ...
              check
                bool
                "diagnostic surfaced on execution keeper row"
                true
                (row |> member "diagnostic" <> `Null);
              check
                bool
                "diagnostic health state surfaced"
                true
                (row |> member "diagnostic" |> member "health_state" <> `Null);
              check
                bool
                "diagnostic next action surfaced"
                true
       ...
       Eio.Switch.run (fun sw ->
         Fun.protect
           ...
           (fun () ->
              ...
              let compact_json =
                Lib.Dashboard_http_keeper.keepers_dashboard_json ~compact:true config
              in
              let trust_json =
              ...
              check
                string
                "compact keeper row exposes trust outcome"
                "ok"
                (compact_row |> member "trust" |> member "last_outcome" |> to_string);
              check
                string
                "compact keeper row exposes trust contract result"
                "satisfied"
                (compact_row
                 |> member "trust"
                 |> member "tool_contract_result"
                 |> to_string);
              check
                string
                "execution trust row preserves sandbox kind"
                "local"
                (trust_row
                 |> member "trust"
                 |> member "sandbox"
              ...
              check
                string
                "execution trust row preserves cascade outcome"
              ...
              check
                string
                "execution trust row exposes operator disposition"
                "fail_open_next_cascade"
                (trust_row |> member "trust" |> member "operator_disposition" |> to_string);
              check
                string
                "execution trust row exposes operator disposition reason"
                "degraded_retry"
                (trust_row
                 |> member "trust"
                 |> member "operator_disposition_reason"
                 |> to_string);
              check
                bool
                "execution trust row preserves degraded retry flag"
                true
                (trust_row
                 |> member "trust"
                 |> member "cascade"
                 |> member "degraded_retry_applied"
                 |> to_bool);
              check
                (option string)
                "execution trust row preserves degraded retry lane"
                (Some Lib.Keeper_config.local_recovery_cascade_name)
                (trust_row
                 |> member "trust"
                 |> member "cascade"
                 |> member "degraded_retry_cascade"
                 |> to_string_option);
              check
                (option string)
                "execution trust row preserves fallback reason"
                (Some "turn_timeout")
                (trust_row
                 |> member "trust"
                 |> member "cascade"
              ...
              check
                string
                "execution trust row preserves rotation target"
                Lib.Keeper_config.local_recovery_cascade_name
                (trust_row
                 |> member "trust"
                 |> member "cascade"
                 |> member "rotation_attempts"
                 |> to_list
                 |> List.hd
                 |> member "to_cascade"
                 |> to_string);
              check
                (list string)
                "execution trust row preserves unexpected tools"
                [ "WebSearch" ]
                (trust_row
                 |> member "trust"
                 |> member "unexpected_tools"
                 |> to_list
                 |> List.map to_string))))
;;
...
let test_execution_trust_surfaces_coverage_gap_health () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         string
         "execution trust coverage gap health"
         "coverage_gap"
         (trust_json |> member "health" |> to_string);
       check
         string
         "execution trust coverage gap stale reason"
         "execution_receipt_append_failed"
         (trust_json |> member "stale_reason" |> to_string);
       check
         int
         "execution trust coverage gap count"
         1
...
let test_patch_keeper_dependent_caches_tolerates_null_agent () =
  let execution_json =
    `Assoc
      ...
      ; ( "keepers"
        , `List
            [ `Assoc
                [ "name", `String "sangsu"; "status", `String "running"; "agent", `Null ]
            ] )
      ]
  in
  with_execution_cache execution_json (fun () ->
    ...
    let open Yojson.Safe.Util in
    let keepers =
      Lib.Server_dashboard_http._execution_cache.json |> member "keepers" |> to_list
    in
    let row = List.hd keepers in
    check
      string
      "patched status falls back to idle"
      "idle"
      (row |> member "status" |> to_string);
    check
      bool
      "keepalive running patched"
      true
       ...
       Eio.Switch.run (fun sw ->
         Fun.protect
           ...
           (fun () ->
              ...
              check
                string
                "running keepers patch remains stable"
                "idle"
                (row |> member "status" |> to_string))))
;;
...
let test_patch_keeper_row_tolerates_null_agent_shape () =
  let row =
    `Assoc [ "name", `String "keeper-alpha"; "agent", `Null; "status", `String "unknown" ]
  in
  let patched =
  ...
  check
    string
    "null agent falls back to idle"
    "idle"
    (patched |> member "status" |> to_string);
  check string "phase still patched" "running" (patched |> member "phase" |> to_string)
;;
