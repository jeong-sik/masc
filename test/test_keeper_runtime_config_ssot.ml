...
let with_config_dir f =
  with_temp_dir "keeper-config-ssot"
  @@ fun config_dir ->
...
let test_personality_resync () =
  with_temp_dir "keeper-config-ssot-room"
  @@ fun room_dir ->
  with_config_dir
  @@ fun config_dir ->
  Fs_compat.clear_fs ();
...
|};
  ...
  match Keeper_runtime.ensure_keeper_meta config keeper_name with
  | Error e -> fail ("ensure_keeper_meta failed: " ^ e)
  | Ok updated -> check bool "policy_voice_enabled" false updated.policy_voice_enabled
;;
...
|};
  ...
  | Ok updated ->
    ...
    check (option string) "tool_preset_source" (Some "toml") updated.tool_preset_source
;;
...
|};
  ...
  match Keeper_runtime.ensure_keeper_meta config keeper_name with
  | Error e -> fail ("ensure_keeper_meta failed: " ^ e)
  | Ok updated -> check (list string) "allowed_paths cleared" [] updated.allowed_paths
;;
...
|};
  ...
  | Error e -> fail ("ensure_keeper_meta failed: " ^ e)
  | Ok updated ->
    check string "toml goal overrides persona" "대화에 바로 쓸 수 있는 연구 브리프를 만든다." updated.goal;
    check string "persona needs inherited" "원문 자료, 논문, 긴 문맥" updated.needs;
    check
      string
      "persona instructions inherited"
      "먼저 읽고 구조를 잡은 뒤 요약한다."
      updated.instructions;
    check
      (list string)
      "persona mention_targets inherited"
      [ "scholar"; "학자" ]
...
|};
  ...
  | Ok updated ->
    check
      (option (float 0.0001))
      "invalid TOML clears stale timeout"
      None
...
let test_persona_invalid_per_provider_timeout_clears_stale_runtime () =
  ...
  | Ok updated ->
    check
      (option (float 0.0001))
      "invalid persona clears stale timeout"
      None
      updated.Keeper_types.per_provider_timeout_s
;;
...
let test_meta_of_json_invalid_per_provider_timeout_is_ignored () =
  ...
  | Ok meta ->
    check
      (option (float 0.0001))
      "invalid persisted meta timeout ignored"
      None
      meta.Keeper_types.per_provider_timeout_s
;;
...
|};
  ...
  | Ok updated ->
    check
      ...
      (Some true)
      updated.Keeper_types.work_discovery_enabled;
    check
      (option int)
      "work_discovery_interval_sec"
      (Some 120)
      updated.work_discovery_interval_sec;
    check
      (option string)
      "work_discovery_guidance"
      (Some "TOML guidance")
      updated.work_discovery_guidance
;;
...
|};
  ...
  | Ok updated ->
    ...
    check
      string
      "cascade_name reset to keeper default"
...
|};
  ...
  | Ok updated ->
    ...
    check
      string
      "social_model resynced from TOML"
      "magentic_ledger_v1"
...
|};
  ...
  match Keeper_runtime.ensure_keeper_meta config keeper_name with
  | Ok updated ->
    failf "expected unknown cascade_name to be rejected, got %s" updated.cascade_name
  | Error detail ->
    check
      bool
      "points at profile.cascade_name"
      true
      (contains_substring detail "profile.cascade_name");
    check
      bool
      "mentions unknown cascade_name"
      true
...
let test_room_presence_syncs_capabilities () =
  ...
  in
  seed_persisted_meta config initial_meta;
  ignore (Coord.join config ~agent_name ~capabilities:[ "keeper"; "preset:minimal" ] ());
...
|}
  in
  match
    Keeper_toml_loader.update_field_in_content
      ~table:"keeper"
      ~key:"cascade_name"
      ~value:"local_only"
      input
  ...
  | Ok result ->
    check bool "contains new value" true (String.length result > 0);
...
let () =
  run
    "Keeper_runtime config SSOT resync"
    [ ( "personality"
      , [ test_case
    ...
    ; ( "toml_writer"
      , [ test_case "replaces existing field in TOML" `Quick test_toml_update_existing
        ; test_case "inserts new field into TOML table" `Quick test_toml_update_insert
        ; test_case "returns Error when table is missing" `Quick test_toml_update_no_table
        ] )
    ]
;;
