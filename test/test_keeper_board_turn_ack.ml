...
let install_keeper config name =
  ...
  let registry_entry = Keeper_registry.For_testing.register ~base_path:config.base_path name meta in
  (meta, registry_entry)
...
|}) in
  ...
  let cycle meta = Keeper_heartbeat_loop.run_keepalive_unified_turn
    ~wake:Keeper_world_observation.Attention_wake ~ctx ~registry_entry ~meta_after_triage:meta
