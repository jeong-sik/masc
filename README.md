...
> unsafe action. The installation contract targets 0.35.4; check
...
| **Dashboard** | The same state in a browser | `/dashboard/` on the same server; the 0.35.4 installer includes a binary-matched bundle |
...
Download the installer attached to [GitHub Releases](https://github.com/jeong-sik/masc/releases/tag/v0.35.4).
It verifies and installs the assets for the selected release.

> Installation target: v0.35.4 (check tag availability on GitHub Releases).
...
and runs a one-time wizard (`--no-wizard` skips it). The 0.35.4 wizard
...
Release **0.35.4** includes Intel macOS, `masc-browser-host`, and the matched
...
| `masc setup --base-path <dir>` | Prepares Docker, starts the existing `imp`, and opens the TUI (0.35.4) |
...
The server serves a TypeScript/Preact SPA at `/dashboard/`. The 0.35.4 release
