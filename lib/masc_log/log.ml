...
let infer_legacy_level message =
  ...
  if has_prefix ~prefix:"[FATAL]" upper || has_prefix ~prefix:"FATAL:" upper
  then Error, "FATAL"
  else if has_prefix ~prefix:"[ERROR]" upper || has_prefix ~prefix:"ERROR:" upper
  then Error, "ERROR"
  else if
    has_prefix ~prefix:"[WARN]" upper
    || has_prefix ~prefix:"[WARNING]" upper
    || has_prefix ~prefix:"WARN:" upper
    || has_prefix ~prefix:"WARNING:" upper
  then Warn, "WARN"
  else if has_prefix ~prefix:"[INFO]" upper || has_prefix ~prefix:"INFO:" upper
  then Info, "INFO"
  else if has_prefix ~prefix:"[DEBUG]" upper || has_prefix ~prefix:"DEBUG:" upper
  then Debug, "DEBUG"
  else Info, "INFO"
;;

(** Check if level should be logged *)
let should_log level = level_to_int level >= Atomic.get current_level

(** Set log level *)
let set_level level = Atomic.set current_level (level_to_int level)
...
module Ring = struct
  ...
  let ensure_dir dir =
    if not (Sys.file_exists dir) then protect ~default:() (fun () -> Sys.mkdir dir 0o755)
  ;;
  ...
  let entry_of_json json =
    ...
    | `Assoc _ ->
      ...
      (match
         ...
         , member "ts" json
         , member "level" json
         , member "raw_level" json
         , member "normalized_level" json
         , member "source" json
         , member "legacy_classified" json
         , member "module" json
         , member "message" json )
       with
       | ( `Int seq
  ...
  let init_file_sink dir =
    ...
    then
      Printf.eprintf
        "[%s] [INFO] [Log] Restored %d log entries from disk\n%!"
        (timestamp ())
        loaded
  ;;
  ...
  let cleanup_old_files ?(keep_days = 7) dir =
    ...
    then (
      ...
      let cutoff =
        format_utc_date_of (Time_compat.now () -. (float_of_int keep_days *. 86400.0))
      in
      Array.iter
        (fun fname ->
           if
             has_prefix ~prefix:"system_log_" fname
             && Filename.check_suffix fname ".jsonl"
           then (
             (* Extract date from system_log_YYYY-MM-DD.jsonl *)
             let date_part = String.sub fname 11 10 in
             if date_part < cutoff
             then protect ~default:() (fun () -> Sys.remove (Filename.concat dir fname))))
        files)
  ;;
  ...
  let push
    ...
    in
    buf.(idx) <- entry;
    if !file_channel <> None then write_to_sink (entry_to_json entry)
  ;;
  ...
  let recent
    ...
    then []
      ...
      while !i >= start && !count < limit do
        ...
        let level_ok = level_to_int (level_of_string e.normalized_level) >= min_level in
        let module_ok =
          module_filter = ""
          || String.lowercase_ascii e.module_name = String.lowercase_ascii module_filter
        in
        if level_ok && module_ok
        then (
          entries := e :: !entries;
          incr count);
        decr i
...
let emit level ?(module_name = "") ?(details = `Null) message =
  ...
  then (
    ...
    let prefix =
      if module_name = ""
      then Printf.sprintf "[%s] [%s]" (timestamp ()) level_str
      else Printf.sprintf "[%s] [%s] [%s]" (timestamp ()) level_str module_name
    in
    Printf.eprintf "%s %s\n%!" prefix message;
    Ring.push
      ~raw_level:level_str
...
struct
  let module_level : int option =
    let env_key = Printf.sprintf "MASC_LOG_%s_LEVEL" (String.uppercase_ascii M.name) in
  ...
  let log_module level fmt =
    Printf.ksprintf
      (fun msg ->
         if should_log_module level
         then (
           let level_str = level_to_string level in
           let prefix = Printf.sprintf "[%s] [%s] [%s]" (timestamp ()) level_str M.name in
           Printf.eprintf "%s %s\n%!" prefix msg;
           Ring.push
             ~raw_level:level_str
             ~normalized_level:level_str
...
module Coord = Make (struct
    let name = "Coord"
  end)

module Mcp = Make (struct
    let name = "MCP"
  end)

module Auth = Make (struct
    let name = "Auth"
  end)

module Retry = Make (struct
    let name = "Retry"
  end)

module Backend = Make (struct
    let name = "Backend"
  end)
...
module Spawn = Make (struct
    let name = "Spawn"
  end)

module Pulse = Make (struct
    let name = "Pulse"
  end)

module ModelClient = Make (struct
    let name = "ModelClient"
  end)

module Orchestrator = Make (struct
    let name = "Orchestrator"
  end)

module BoardLog = Make (struct
    let name = "Board"
  end)

module Metrics = Make (struct
    let name = "Metrics"
  end)

module Dashboard = Make (struct
    let name = "Dashboard"
  end)

module Trpg = Make (struct
    let name = "Trpg"
  end)

module Feed = Make (struct
    let name = "Feed"
  end)

module Telemetry = Make (struct
    let name = "Telemetry"
  end)

module Noosphere = Make (struct
    let name = "Noosphere"
  end)

module CmdPlane = Make (struct
    let name = "CmdPlane"
  end)

module Governance = Make (struct
    let name = "Governance"
  end)

module Social = Make (struct
    let name = "Social"
  end)

module Transport = Make (struct
    let name = "Transport"
  end)

module Gc = Make (struct
    let name = "GC"
  end)

module Reputation = Make (struct
    let name = "Reputation"
  end)

module Keeper = Make (struct
    let name = "Keeper"
  end)
...
module Mention = Make (struct
    let name = "Mention"
  end)

module Misc = Make (struct
    let name = "Misc"
  end)

module Autoresearch = Make (struct
    let name = "Autoresearch"
  end)
...
module Institution = Make (struct
    let name = "Institution"
  end)

module Pages = Make (struct
    let name = "Pages"
  end)

module Thompson = Make (struct
    let name = "Thompson"
  end)
...
module Task = Make (struct
    let name = "Task"
  end)

module Http = Make (struct
    let name = "Http"
  end)

module Langfuse = Make (struct
    let name = "Langfuse"
  end)
...
module Dispatch = Make (struct
    let name = "Dispatch"
  end)

module BoardPg = Make (struct
    let name = "BoardPg"
  end)

module MemoryPg = Make (struct
    let name = "MemoryPg"
  end)

module MemoryJsonl = Make (struct
    let name = "MemoryJsonl"
  end)

module AutoResponder = Make (struct
    let name = "AutoResponder"
  end)
...
module Level2 = Make (struct
    let name = "Level2"
  end)

module RoomTask = Make (struct
    let name = "RoomTask"
  end)
...
module Protocol = Make (struct
    let name = "Protocol"
  end)

module AlwaysOn = Make (struct
    let name = "AlwaysOn"
  end)

module KeeperExec = Make (struct
    let name = "KeeperExec"
  end)

module LocalWorker = Make (struct
    let name = "LocalWorker"
  end)

module Worker = Make (struct
    let name = "Worker"
  end)

module Sse = Make (struct
    let name = "SSE"
  end)

module Verifier = Make (struct
    let name = "Verifier"
  end)

module Planner = Make (struct
    let name = "Planner"
  end)

module Compact = Make (struct
    let name = "Compact"
  end)

module Harness = Make (struct
    let name = "Harness"
  end)

module Discovery = Make (struct
    let name = "Discovery"
  end)
