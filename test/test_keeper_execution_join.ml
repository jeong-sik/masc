...
let mk_event ?caused_by payload : Masc_agent_core.Event_bus.event =
  { meta =
...
let test_tool_approval_completed_preserves_exact_occurrence () =
  let json =
    Bridge.native_event_to_json
      (mk_event
         (Masc_agent_core.Event_bus.ToolApprovalCompleted
            { invocation = invocation ~turn:4 ~planned_index:2 "tu-approved"
            ; agent_name = "keeper-approval"
            ; tool_name = "Execute"
            ; approval = Masc_agent_core.Hooks.Approved
            }))
    |> Option.get
  in
  check (option string)
  ...
  check_terminal_projection
    ...
    (Masc_agent_core.Error.Agent
       (Masc_agent_core.Error.TerminalToolEffectFailed
          ...
          ; effect_disposition = Masc_agent_core.Error.proven_post_terminal_effect
  ...
  check_terminal_projection
    ...
    (Masc_agent_core.Error.Agent
       (Masc_agent_core.Error.TerminalToolDurabilityFailed
          ...
          ; effect_disposition = Masc_agent_core.Error.unknown_terminal_effect
...
let test_agent_failed_matches_typed_sse_event () =
  ...
  let actual =
    Bridge.native_event_to_json
      (mk_event
         ~caused_by
         (Masc_agent_core.Event_bus.AgentFailed
...
let test_authorization_errors_have_typed_projection () =
  ...
  check_projection
    ...
    (Masc_agent_core.Error.Api
       (Masc_agent_core.Retry.AuthorizationError { message = "permission refused" }));
  check_projection
...
let test_request_body_too_large_projection_preserves_bounds () =
  let projection =
    Error_json.agent_failed_error_projection
      (Masc_agent_core.Error.Api
         (Masc_agent_core.Retry.InvalidRequest
            { message = "request body too large"
            ; reason =
                Masc_agent_core.Retry.Request_body_too_large
    ...
    Error_json.agent_failed_error_projection
      (Masc_agent_core.Error.Api
         (Masc_agent_core.Retry.InvalidRequest
            ...
            ; reason =
                Masc_agent_core.Retry.Request_body_refused_by_provider { status = 413 }
            }))
  in
  check
    (option string)
    ...
    Error_json.agent_failed_error_projection
      (Masc_agent_core.Error.Api
         (Masc_agent_core.Retry.InputCapacity
            ...
            ; constraint_
            ; reason =
                Masc_agent_core.Retry.Serving_constraint_rejected
...
let test_timeout_projection_without_phase_reports_null () =
  let projection =
    Error_json.agent_failed_error_projection
      (Masc_agent_core.Error.Api
         (Masc_agent_core.Retry.Timeout { message = "unattributed timeout"; phase = None }))
  in
  check
    (option string)
    "typed variant"
