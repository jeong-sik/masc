...
let test_agent_status_of_string () =
  ...
  check
    ...
    true
    (Types.agent_status_of_string "unknown" = Types.Active)
;;
...
let test_agent_status_of_string_opt () =
  check
    ...
    "active Some"
    true
    (Types.agent_status_of_string_opt "active" = Some Types.Active);
...
let test_task_status_claimed () =
  let status =
    Types.Claimed { assignee = "claude"; claimed_at = "2024-01-01T00:00:00Z" }
  in
...
let test_task_status_in_progress () =
  let status =
    Types.InProgress { assignee = "gemini"; started_at = "2024-01-01T01:00:00Z" }
  in
...
let test_task_status_done_no_notes () =
  let status =
    Types.Done { assignee = "codex"; completed_at = "2024-01-01T02:00:00Z"; notes = None }
  in
  let json = Types.task_status_to_yojson status in
  let result = Types.task_status_of_yojson json in
  check bool "roundtrip ok" true (is_ok result)
...
let test_limit_for_category () =
  ...
  check
    ...
    (Types.limit_for_category cfg Types.BroadcastLimit);
  check
    int
    "task_ops"
    cfg.task_ops_per_minute
    (Types.limit_for_category cfg Types.TaskOpsLimit)
;;
...
let test_category_for_tool () =
  check
    ...
    "broadcast"
    true
    (Types.category_for_tool "masc_broadcast" = Types.BroadcastLimit);
...
let test_masc_error_to_string () =
  ...
  check
    ...
    true
    (String.length
       (Types.masc_error_to_string
          (Types.TaskAlreadyClaimed { task_id = "t1"; by = "agent" }))
     > 0);
  check
    bool
    "RateLimitExceeded"
...
let test_task_with_worktree () =
  let task =
    Types.
      ...
      ; task_status =
          InProgress { assignee = "claude"; started_at = "2024-01-01T01:00:00Z" }
      ; goal_id = None
      ; priority = 1
      ; files = []
      ; created_at = "2024-01-01T00:00:00Z"
    ...
    Types.
      { tasks =
          ...
          ; { id = "t2"
            ...
            ; task_status =
                Done
                  { assignee = "a"; completed_at = "2024-01-02T00:00:00Z"; notes = None }
            ; goal_id = None
            ; priority = 2
            ; files = []
...
let test_tool_result_no_data () =
  let result = Types.{ success = false; message = "Operation failed"; data = None } in
  let json = Types.tool_result_to_yojson result in
  let open Yojson.Safe.Util in
...
let test_room_eio_event_type_to_string () =
  check
    ...
    "AgentJoin"
    "agent_join"
    (Coord_eio.event_type_to_string Coord_eio.AgentJoin);
  check
    string
    "AgentLeave"
    "agent_leave"
    (Coord_eio.event_type_to_string Coord_eio.AgentLeave);
  check
    string
    "Broadcast"
    "broadcast"
    (Coord_eio.event_type_to_string Coord_eio.Broadcast);
  check
    string
    "LockAcquire"
    "lock_acquire"
    (Coord_eio.event_type_to_string Coord_eio.LockAcquire);
  check
    string
    "LockRelease"
    "lock_release"
    (Coord_eio.event_type_to_string Coord_eio.LockRelease)
;;
...
let error_tests = [ "masc_error to_string", `Quick, test_masc_error_to_string ]
let worktree_tests = [ "roundtrip", `Quick, test_worktree_info_roundtrip ]
...
let backlog_tests = [ "roundtrip", `Quick, test_backlog_roundtrip ]
...
let room_eio_time_tests = [ "now_iso", `Quick, test_room_eio_now_iso ]
