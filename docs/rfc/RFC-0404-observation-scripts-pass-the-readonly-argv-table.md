...
브래킷 글로브 `[` 도 일치 파일 존재 시 argv 모양을 바꾸므로 같이 금지한다.
탭을 직접 split에 포함하는 대신 금지하는 이유가 여기에 있다 — 실행 환경이
비표준 IFS를 상속하면 셸의 분리자 집합을 우리가 알 수 없어, split 확장은
어떤 형태로도 닫히지 않는다. 금지만이 상속 IFS 아래에서도 성립한다. split 결과를 기존 `classify_argv` 에 그대로
