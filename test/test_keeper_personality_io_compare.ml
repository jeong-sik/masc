...
let test_all_fields_drift () =
  let a = make ~will:"a1" ~needs:"n1" ~desires:"d1" ~instructions:"i1" () in
  let b = make ~will:"a2" ~needs:"n2" ~desires:"d2" ~instructions:"i2" () in
  assert_drift
    ~label:"all four fields"
    ~expected_fields:[ "will"; "needs"; "desires"; "instructions" ]
    a
    b
;;
  ...
  run
    ...
    [ ( "Equal: trim swallows whitespace"
      , [ test_case "identical inputs" `Quick test_identical_inputs_equal
        ...
        ; test_case "only-whitespace vs empty" `Quick test_only_whitespace_vs_empty_equal
        ] )
    ; ( "Drift: real content differences"
      , [ test_case "single field" `Quick test_single_field_drift
        ; test_case
            "two fields in canonical order"
            `Quick
    ...
    ; ( "Diff details"
      , [ test_case
            "diff_offset is index of first differing byte"
            `Quick
            test_diff_offset_matches_first_byte
        ; test_case
            "prefix relation: offset at shorter length"
            `Quick
            test_prefix_relation_offset_at_shorter_length
        ] )
    ; ( "Layer 1 regression"
      , [ test_case
            "nick0cave 357B + trailing newline → Equal"
            `Quick
