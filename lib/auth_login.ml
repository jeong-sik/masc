...
let normalize_base_path path = Env_config_core.normalize_masc_base_path_input path
...
let ensure_required_bearer_auth ~base_path ~agent_name =
  ...
  if cfg.enabled && cfg.require_token
  then Ok Auth_already_required
  else if not cfg.enabled
  then (
  ...
  | Ok auth_change ->
    (match Auth.create_token base_path ~agent_name ~role with
     ...
     | Ok (bearer_token, cred) ->
       let raw_token_file = persist_raw_token ~base_path ~agent_name bearer_token in
       let agent_param = url_encode agent_name in
       let token_param = url_encode bearer_token in
       let dashboard_url =
         Printf.sprintf
           "http://%s:%d/dashboard?agent=%s&token=%s"
           host
           port
           agent_param
           token_param
  ...
  `Assoc
    ...
    ; ( "codex_mcp"
      , `Assoc
          ...
          ; ( "login_note"
            , `String "`codex mcp login` is OAuth-only; masc-mcp uses bearer token auth."
            )
          ] )
    ]
;;
...
let render_shell report =
  String.concat
    "\n"
    [ Printf.sprintf
        "export MASC_OPERATOR_AGENT=%s"
        (single_quote_shell report.agent_name)
    ; Printf.sprintf
        "export MASC_OPERATOR_TOKEN=%s"
...
let render_text report =
  String.concat
    ...
    ; Printf.sprintf "auth_change: %s" (auth_change_to_string report.auth_change)
