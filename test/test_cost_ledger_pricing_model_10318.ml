...
let test_normal_model_returned_verbatim () =
  ...
  let pm = H.pricing_model_for_ledger ~model:"claude-sonnet-4-6" ~telemetry:(Some tel) in
...
let test_auto_alias_resolved_via_telemetry () =
  ...
  check string "auto + telemetry → canonical" "openai:gpt-5.3-codex" pm
;;

(* Compound "provider:auto" aliases also resolve to canonical. *)
let test_provider_auto_alias_resolved () =
  let tel = make_telemetry ~canonical_model_id:"openai:gpt-5.3-codex" () in
  let pm = H.pricing_model_for_ledger ~model:"openai:auto" ~telemetry:(Some tel) in
  check string "provider:auto + telemetry → canonical" "openai:gpt-5.3-codex" pm
;;
...
let test_empty_model_resolved_via_telemetry () =
  ...
  check string "empty model + telemetry → canonical" "anthropic:claude-opus-4-7" pm
;;
...
let test_unknown_sentinel_resolved_via_telemetry () =
  ...
  let pm = H.pricing_model_for_ledger ~model:sentinel ~telemetry:(Some tel) in
  check string "unknown_sentinel + telemetry → canonical" "openai:gpt-5-mini" pm
;;
...
let test_source_raw_when_model_unchanged () =
  let src =
    H.model_resolution_source_for_ledger
...
let test_source_alias_when_auto_resolved () =
  let src =
    H.model_resolution_source_for_ledger
      ~model:"auto"
      ~pricing_model:"openai:gpt-5.3-codex"
  in
  check string "auto resolved → telemetry_canonical_alias" "telemetry_canonical_alias" src
;;
...
let test_source_empty_when_empty_resolved () =
  let src =
    H.model_resolution_source_for_ledger
      ~model:""
      ~pricing_model:"anthropic:claude-opus-4-7"
  in
  check
    string
    "empty resolved → telemetry_canonical_empty"
    "telemetry_canonical_empty"
    src
;;
...
let test_source_unknown_when_sentinel_resolved () =
  ...
  check
    string
    "sentinel resolved → telemetry_canonical_unknown"
...
let test_catalog_status_hit_free () =
  ...
  let status = H.pricing_catalog_status ~pricing_model:"qwen3.6:35b-a3b-mlx-bf16-64k" in
  check string "known free (ollama) model → hit_free" "hit_free" status
;;

let test_catalog_status_miss () =
  let status =
    H.pricing_catalog_status ~pricing_model:"catalog-probe-unknown-model-10318"
  in
...
let test_status_unpriced_model () =
  check_status
    ...
    ~provider:"openai"
    ~pricing_model:"unpriced-probe-10318"
    ~usage_missing:false
    ~usage_trusted:true
    ~input_tokens:500
    ~output_tokens:200
...
let test_status_strings_non_empty () =
  ...
  List.iter
    (fun v ->
       ...
       check
         bool
         (Printf.sprintf "%s: label non-empty" label)
         true
         (String.length label > 0);
       let reason = H.cost_status_reason v in
       check
         bool
         (Printf.sprintf "%s: reason non-empty" label)
         true
...
let () =
  run
    "cost_ledger_pricing_model_10318"
    [ ( "pricing_model_for_ledger"
      , [ test_case "normal model verbatim" `Quick test_normal_model_returned_verbatim
        ; test_case
            "auto resolved via telemetry"
            `Quick
            test_auto_alias_resolved_via_telemetry
        ; test_case "provider:auto resolved" `Quick test_provider_auto_alias_resolved
        ; test_case
            "auto without telemetry → raw"
            `Quick
            test_auto_without_telemetry_returns_raw
        ; test_case
            "empty model via telemetry"
            `Quick
            test_empty_model_resolved_via_telemetry
        ; test_case
            "empty model no telemetry"
            `Quick
            test_empty_model_no_telemetry_returns_empty
        ; test_case
            "unknown sentinel via telemetry"
            `Quick
            test_unknown_sentinel_resolved_via_telemetry
        ] )
    ; ( "model_resolution_source_for_ledger"
      , [ test_case "raw when unchanged" `Quick test_source_raw_when_model_unchanged
        ...
        ; test_case
            "unknown_sentinel resolved"
            `Quick
            test_source_unknown_when_sentinel_resolved
        ] )
    ; ( "pricing_catalog_status"
    ...
    ; ( "cost_status_for_event"
      , [ test_case "usage_missing wins" `Quick test_status_usage_missing_wins
        ...
        ; test_case
            "priced when positive cost"
            `Quick
            test_status_priced_when_positive_cost
