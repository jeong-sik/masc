...
let stream_post_sse_start
  ...
  if not (send_raw info (sse_prime_event ()))
  then Log.Server.debug "SSE prime send failed for session %s" info.session_id;
  info
;;
...
let spawn_post_sse_keepalive ~sw ~clock info =
  Eio.Fiber.fork ~sw (fun () ->
    let rec loop () =
      if not !(info.stop)
      then (
        (try Eio.Time.sleep clock post_sse_keepalive_interval_s with
         | Eio.Cancel.Cancelled _ as e -> raise e
         | exn ->
           Printf.eprintf
             "[server_mcp_transport_http] keepalive sleep failed: %s\n%!"
             (Printexc.to_string exn));
        if info.closed
        then close_sse_conn info
        else if not !(info.stop)
        then
          if not (send_raw info ": keepalive\n\n")
          then Log.Server.debug "SSE keepalive send failed for session %s" info.session_id;
        loop ())
    in
    try loop () with
    | Eio.Cancel.Cancelled _ as e -> raise e
...
let stream_post_sse_json info (json : Yojson.Safe.t) =
  ...
  then Log.Server.debug "SSE json send failed for session %s" info.session_id
;;
...
let inject_agent_name_into_body
    ...
    | Some "tools/call" ->
      ...
      let new_args =
        ...
        | `Assoc fields ->
          ...
          let should_inject =
            rewrite_existing
            || (Option.is_none existing_agent && Option.is_none existing_legacy_agent)
          in
          if should_inject
      ...
      then body_str
        ...
        let new_params =
          ...
          | `Assoc fields ->
            `Assoc
              (List.map
                 (fun (k, v) -> if k = "arguments" then k, new_args else k, v)
                 fields)
          | _ -> params
        in
        let new_json =
          ...
          | `Assoc fields ->
            `Assoc
              (List.map
                 (fun (k, v) -> if k = "params" then k, new_params else k, v)
                 fields)
          | _ -> json
        in
...
let handle_post_mcp ~deps ?(profile = Full) request reqd =
  ...
  then respond_not_ready ~deps request reqd
  else (
    ...
    let auth_result =
      match profile with
      | Full | Managed_agent -> deps.verify_mcp_auth ~base_path request
      | Operator_remote -> deps.verify_operator_mcp_auth ~base_path request
    in
    ...
    ignore
      (let* () =
       ...
       let* () =
         ...
         | Ok () -> Ok ()
         | Error msg ->
           respond_mcp_auth_error ~deps request reqd ~session_id ~protocol_version msg;
           Error ()
       in
       Ok
         (Http.Request.read_body_async reqd (fun body_str ->
            ignore
              (let* () =
                 match validate_session_requirement ~session_was_provided body_str with
                 | Ok () -> Ok ()
                 | Error msg ->
                   let body =
                     Printf.sprintf
                       ...
                       (`Assoc
                           ...
                           ; ( "error"
                             , `Assoc
                                 ...
                                 ; ( "message"
                                   , `String
                                       "Invalid Accept header: must include \
                                        application/json and text/event-stream. Set \
                                        MASC_ALLOW_LEGACY_ACCEPT=1 for temporary \
                                        compatibility." )
                                 ] )
                           ])
                   in
                 ...
                 (Eio.Fiber.fork ~sw (fun () ->
                    let response_protocol_version =
                      ...
                      | Some v ->
                        ...
                        v
                      | None -> get_protocol_version_for_session ~session_id request
                    in
                    let wants_streaming_post =
                      should_stream_post_tools_call request body_str accept_mode
                    in
                    let response_id = body_jsonrpc_id body_str in
                    let inline_sse : sse_conn_info option ref = ref None in
                    try
                      if wants_streaming_post
                      then (
                        let info =
                          stream_post_sse_start
                            ~deps
                            ~origin
                            ~session_id
                      ...
                      let internal_keeper_runtime =
                        Server_auth.is_verified_internal_keeper_request ~base_path request
                      in
                      let response_json =
                      ...
                      then (
                        ...
                        | Some info ->
                          ...
                          then stream_post_sse_json info response_json;
                      ...
                      then (
                        ...
                        | `Null ->
                          let headers =
                            Httpun.Headers.of_list
                              ((("content-length", "0") :: accept_warn_headers)
                               @ mcp_headers session_id protocol_version)
                          in
...
let handle_get_mcp
  ...
  then respond_not_ready ~deps request reqd
    ...
    let auth_result =
      ...
      | Full | Managed_agent ->
        (match sse_kind with
         | Sse.Observer -> deps.verify_mcp_observer_stream_auth ~base_path request
         | Sse.Coordinator -> deps.verify_mcp_auth ~base_path request)
      | Operator_remote -> deps.verify_operator_mcp_auth ~base_path request
    in
    let legacy_headers =
             ...
             | Ok () ->
               ...
               (match legacy_messages_endpoint with
                ...
                | Some f ->
                  ...
                  then
                    Log.Server.debug
                      "SSE endpoint send failed for session %s"
                      info.session_id);
                  ...
                  Eio.Fiber.fork ~sw (fun () ->
                    let rec drain () =
                      ...
                      (try
                       ...
                       | Eio.Cancel.Cancelled _ as e -> raise e
                       | exn ->
                         Log.Server.error "drain write error: %s" (Printexc.to_string exn);
                    ...
                    | Eio.Cancel.Cancelled _ as e -> raise e
                    | exn ->
                      Log.Server.error "drain loop error: %s" (Printexc.to_string exn));
                  Eio.Fiber.fork ~sw (fun () ->
                    ...
                    let rec loop () =
                      ...
                      then (
                        ...
                        (try
                           if info.closed
                           then stop_sse_session info.session_id
                           else if not !(info.stop)
                           then
                         ...
                         | exn ->
                           if is_cancelled exn then raise exn;
                           Log.Server.error "ping send error: %s" (Printexc.to_string exn);
                           stop_sse_session info.session_id);
                        loop ())
                    in
                    try loop () with
                    ...
                    | exn ->
                      if is_cancelled exn
                      then ()
                      else Log.Server.error "ping loop error: %s" (Printexc.to_string exn))
                | Error msg ->
                  Log.Server.debug
                    "SSE runtime unavailable for session %s: %s"
                    session_id
                    msg);
               let client_count = Sse.client_count () in
               if client_count > Sse.max_clients / 2
               then
                 Log.Server.info
                   "SSE connected: %s (active: %d/%d)"
                   session_id
                   client_count
                   Sse.max_clients))))
;;
...
let handle_get_operator_mcp ~deps request reqd =
  ...
  | Error msg ->
    respond_mcp_auth_error ~deps request reqd ~session_id ~protocol_version msg
  | Ok () -> handle_get_mcp ~deps ~profile:Operator_remote request reqd
;;
       ...
       | Ok () ->
         Http.Request.read_body_async reqd (fun body_str ->
           ...
           | Ok runtime ->
             ...
             Eio.Fiber.fork ~sw (fun () ->
               let body_with_agent =
                 body_with_canonical_http_actor ~base_path ~auth_token request body_str
               in
               let internal_keeper_runtime =
                 Server_auth.is_verified_internal_keeper_request ~base_path request
               in
    ...
    | Ok () ->
      (match get_session_id_any request with
       ...
       | None ->
         ...
         let headers =
           Httpun.Headers.of_list [ "content-length", string_of_int (String.length body) ]
         in
         let response = Httpun.Response.create ~headers `Bad_request in
         Httpun.Reqd.respond_with_string reqd response body))
;;
