...
let extract_user_message (messages : Yojson.Safe.t) : string option =
  ...
  let user_msgs =
    List.filter
      (fun m -> String.equal (Safe_ops.json_string ~default:"" "role" m) "user")
      msgs
  in
  match List.rev user_msgs with
  | last :: _ -> Safe_ops.json_string_opt "content" last
  | [] -> None
;;
...
let route_keeper ~config ~sw ~clock ~keeper_name ~message : (string, string) result =
  let ctx : _ Keeper_types.context =
    { config; agent_name = "openai-compat"; sw; clock; proc_mgr = None; net = None }
  in
  let args = `Assoc [ "name", `String keeper_name; "message", `String message ] in
...
let route_cascade ~message ~system_prompt ~max_tokens ~temperature
  ...
  match
    Masc_oas_bridge.run_with_caller
      ~caller:Env_config_oas_bridge.Server_openai_compat
      (fun () ->
         Oas_worker.run_named
           ...
           ())
  with
  | Ok result -> Ok (Oas_response.text_of_response result.response)
...
let handle_chat_completions ~config ~sw ~clock (body : string) : Httpun.Status.t * string =
  try
    let json = Yojson.Safe.from_string body in
    ...
    in
    let max_tokens =
      Safe_ops.json_int ~default:Oas_worker_cascade.default_max_tokens "max_tokens" json
    in
    let temperature =
      Safe_ops.json_float
        ~default:Oas_worker_cascade.default_temperature
        "temperature"
        json
    in
    if model = ""
    then
      ...
      | Some user_message ->
        ...
        let is_keeper_prefix =
          String.length model > 7 && String.starts_with ~prefix:"keeper:" model
        in
        if is_keeper_prefix
        then (
          ...
          | Ok reply -> `OK, completion_response ~model ~content:reply
          | Error e ->
          ...
          let system_prompt =
            try
              ...
              let sys_msgs =
                List.filter_map
                  (fun m ->
                     if String.equal (Safe_ops.json_string ~default:"" "role" m) "system"
                     then Safe_ops.json_string_opt "content" m
                     else None)
                  msgs
              in
              String.concat "\n" sys_msgs
          ...
          in
          match
            route_cascade ~message:user_message ~system_prompt ~max_tokens ~temperature
          with
          | Ok reply -> `OK, completion_response ~model ~content:reply
          | Error e ->
            ( `Internal_server_error
