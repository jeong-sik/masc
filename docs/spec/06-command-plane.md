...
Command Plane V2(CPv2)는 삭제된 subsystem의 historical reference다. 이 문서는 migration context와 retired read-model vocabulary만 설명하며, 새 caller onboarding은 repo workspace collaboration, keeper runtime, dashboard/operator read visibility를 기준으로 한다.
...
**Workload Profile별 Progress Labels**:
...
4. `stage` 유효성 검증 (workload별 허용 label)
5. `search_strategy` 유효성 검증
6. `intent_id` 존재 + workload_profile 일치 검증
7. contract/checkpoint가 명시한 hard requirement 검증
8. chain record 파싱
...
`Workspace_hooks.cp_cleanup_fn` ref callback으로 Workspace_gc에서 호출되어 순환 의존을 회피한다.
...
|------|-------------|--------|
| `workspace` | workspace-core | Workspace state |
...
Alert, pending confirm, runtime blocker, hot proof 등 비정상 상태를 signal로 발행한다. Focus는 bad signal > unhealthy session > workspace 순서로 결정된다.
...
| Cleanup callback indirection | Workspace_gc -> Workspace_hooks ref callback으로 순환 의존 회피. 직접 호출 불가 | Architectural |
...
| Workspace state `search_strategy_default` | `best_first_v1` | Workspace별 기본 search strategy |
| Workspace state `speculation_enabled` | false | 추측적 실행 활성화 |
| Workspace state `speculation_budget` | 2 | 추측적 실행 예산 |
