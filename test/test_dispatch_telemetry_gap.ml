...
(** RFC-0083 §1.2 Telemetry 4-Tuple Emission Gap
    ...
    4-Tuple definition (RFC-0083 §2.1):
...
let test_span_emission_gap () =
  (check int)
    "Tracing.with_span calls per Tool_dispatch.dispatch (RFC-0083 §1.2; PR-9 target = 1)"
    0
    pinned_span_emission_per_dispatch

let test_audit_emission_gap () =
  (check int)
    "Audit_log.record calls per dispatch (RFC-0083 §1.2; PR-9 target = 1)"
    0
...
let test_metric_emission_gap_on_none () =
  (check int)
    "Prometheus.inc_counter calls when handler returns None \
     (RFC-0083 §1.1 / tool_dispatch.ml:127-129; PR-10 target = 1)"
    0
    pinned_metric_emission_per_dispatch_when_handler_none

let test_trace_id_propagation_gap () =
  (check int)
    "Trace_id propagation count per dispatch (RFC-0083 §1.2; PR-9 target = 1)"
    0
...
let test_4_tuple_total_gap () =
  ...
  (check int)
    "4-tuple emission sum per dispatch \
     (RFC-0083 §2.1 invariant; PR-14 target = 4 across all entries)"
    0
    total

let () =
  Alcotest.run
    "RFC-0083 dispatch telemetry gap"
    [ ( "telemetry-gap"
