...
let record_tool_skipped ~keeper_name ~tool_name ~reason_code =
  let event = { ts = Unix.gettimeofday (); tool_name; reason_code; keeper_name } in
  try
...
let reset_for_testing () = ring := []

let snapshot_ring () =
  Safe_ops.protect ~default:[] (fun () -> Eio.Mutex.use_ro ring_mu (fun () -> !ring))
;;
...
let tool_rejection_counts ?(now_ts = Unix.gettimeofday ()) ~window_minutes ()
  ...
  |> List.sort (fun (t1, r1, c1) (t2, r2, c2) ->
    ...
    then by_count
      ...
      if by_tool <> 0 then by_tool else String.compare r1 r2))
;;
...
let approval_queue_summary () : approval_summary =
  ...
  if depth = 0
  then { depth = 0; p50_wait_sec = None; p95_wait_sec = None; oldest_pending_sec = None }
  else (
...
let tool_rejections_json ?(top_n = 20) ~window_minutes ?(now_ts = Unix.gettimeofday ()) ()
  : Yojson.Safe.t list
  =
  tool_rejection_counts ~now_ts ~window_minutes ()
  |> (fun ls ->
  if List.length ls <= top_n then ls else List.filteri (fun i _ -> i < top_n) ls)
  |> List.map (fun (tool, reason, count) ->
    `Assoc [ "tool", `String tool; "reason", `String reason; "count", `Int count ])
;;
...
let governance_tool_events_json ?(now_ts = Unix.gettimeofday ()) ~window_minutes ()
  : Yojson.Safe.t
  =
