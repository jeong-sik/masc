(** Pure-function unit tests for [Workspace_eio].
...
(* ── workspace_state JSON round-trip ──────────────────────────────────────── *)

let test_default_workspace_state_round_trip () =
  let state = CE.default_workspace_state () in
  let json = CE.workspace_state_to_json state in
  match CE.workspace_state_of_json json with
...
let test_paused_workspace_state_round_trip () =
  let base = CE.default_workspace_state () in
  let state =
    { base with
      paused = true;
...
let test_workspace_state_of_json_missing_event_seq () =
...
let test_workspace_state_of_json_malformed () =
  let json = `String "not an object" in
  match CE.workspace_state_of_json json with
  | Ok _ -> Alcotest.fail "malformed json should not parse to workspace_state"
  | Error _ -> ()

(* ── runner ──────────────────────────────────────────────────────────── *)

let () =
  Alcotest.run "Workspace_eio_pure"
    [
      ( "key_formatters",
        [
