...
let test_any_mentioned_ambient_message () =
  check
    bool
    "ambient message not a direct mention"
    false
...
let test_keeper_policy_observation_direct_mention () =
  ...
  let obs =
    Keeper_memory.keeper_policy_observation_of_room_message
      ~meta
      ~room_id:"default"
      (room_message "@director, what do you think?")
  in
...
let test_keeper_policy_observation_non_mention () =
  ...
  let obs =
    Keeper_memory.keeper_policy_observation_of_room_message
      ~meta
      ~room_id:"default"
      (room_message "ambient room chatter")
  in
  check
    bool
    "keeper observation no longer hardcodes direct mention"
    false
    obs.direct_mention
;;
...
let test_user_visible_reply_strips_state_block () =
  ...
  check
    string
    "state block hidden from user text"
    "좋아요. 이어서 진행하겠습니다."
...
let test_user_visible_reply_strips_skill_and_state_markers () =
  ...
  check
    string
    "skill route lines hidden from user text"
    "본문입니다."
    (Keeper_execution.user_visible_reply_text raw)
;;
...
let test_user_visible_reply_falls_back_to_snapshot_progress () =
  ...
  check
    string
    "state-only reply falls back to progress"
    "다음 장면 전환 준비 완료"
    (Keeper_execution.user_visible_reply_text raw)
;;
...
let base_measurement : Meas.measurement_snapshot =
  ...
  ; context =
      { context_ratio = 0.3; message_count = 20; token_count = 200; max_tokens = 10000 }
  ; similarity =
      { repetition_risk = 0.1
      ; goal_alignment = 0.8
...
let test_prioritized_action_none () =
  ...
  check string "no rule fired" "none" (Recall.prioritized_action_to_string action)
;;
...
let test_prioritized_action_guardrail_stop () =
  ...
  | Recall.Act_guardrail_stop reason ->
    check bool "reason contains gates" true (String.length reason > 0)
  | _ -> fail "expected Act_guardrail_stop"
;;

let test_prioritized_action_reflect_over_plan () =
  let eval = { base_eval with reflect = true; plan = true; compact = true } in
  let action = Recall.prioritized_action eval in
  check
    string
    "reflect wins over plan"
    "reflect"
    (Recall.prioritized_action_to_string action)
;;

let test_prioritized_action_plan_over_compact () =
  let eval = { base_eval with plan = true; compact = true; handoff = true } in
  let action = Recall.prioritized_action eval in
  check
    string
    "plan wins over compact"
    "plan"
    (Recall.prioritized_action_to_string action)
;;

let test_prioritized_action_compact_over_handoff () =
  let eval = { base_eval with compact = true; handoff = true } in
  let action = Recall.prioritized_action eval in
  check
    string
    "compact wins over handoff"
    "compact"
    (Recall.prioritized_action_to_string action)
;;
...
let test_prioritized_action_handoff_alone () =
  ...
  let action = Recall.prioritized_action eval in
  check string "handoff alone" "handoff" (Recall.prioritized_action_to_string action)
;;

let test_prioritized_action_guardrail_default_reason () =
  let eval = { base_eval with guardrail_stop = true; guardrail_reason = None } in
  let action = Recall.prioritized_action eval in
  match action with
  | Recall.Act_guardrail_stop reason ->
...
let test_prioritized_action_to_string_all_variants () =
  ...
  check
    string
    "guardrail"
    "guardrail_stop(safety)"
    (prioritized_action_to_string (Act_guardrail_stop "safety"));
...
let test_tmpdir () =
  let dir =
    Filename.concat
      ...
      (Printf.sprintf "masc-test-%d" (Unix.getpid ()))
  in
  ...
  then (
    Array.iter
      (fun f ->
         ...
         then cleanup_tmpdir_recursive path
         else (
...
let test_recall_candidates_with_history_dedup () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let result =
         Keeper_memory_recall.recall_candidates_with_history
...
let make_test_room_config dir = Coord.default_config dir
...
let test_memory_write_then_recall_meta_fallback () =
  ...
  Fun.protect
    ...
    (fun () ->
       let config = make_test_room_config dir in
       let meta =
         keeper_meta ~name:"fallback-keeper" ~mention_targets:[ "fallback-keeper" ] ()
       in
...
let test_memory_write_persists_horizon_and_source () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let next_entry =
         List.find_opt
           (fun json -> Yojson.Safe.Util.(json |> member "kind" |> to_string = "next"))
           entries
       in
...
let test_memory_candidates_capture_next_summary () =
  ...
  check
    bool
    "next summary becomes a next candidate"
    true
    (List.exists
       (fun (kind, text, _) ->
          kind = "next" && String.equal text "verify harness output and update docs")
       candidates)
;;
...
let test_priority_for_kind_long_term () =
  check
    int
    "long_term gets durable priority"
    95
...
let test_read_continuity_summary_prefers_progress_log () =
  let dir = test_tmpdir () in
  Fun.protect
    ~finally:(fun () -> cleanup_tmpdir_recursive dir)
    (fun () ->
       let config = make_test_room_config dir in
       let meta =
         keeper_meta
           ~name:"progress-pref-keeper"
           ~mention_targets:[ "progress-pref-keeper" ]
           ()
       in
       let progress_path =
         Keeper_types.keeper_progress_path config "progress-pref-keeper"
       in
       ...
       let continuity = Keeper_world_observation.read_continuity_summary ~config ~meta in
       check
         bool
         "reads goal from progress log"
         true
         (String.contains continuity 'r'
          && Astring.String.is_infix ~affix:"recover from progress file" continuity);
       check
         bool
         "reads next plan from progress log"
         true
...
let test_keeper_context_status_reports_recovery_source_and_tiers () =
  ...
  Fun.protect
    ...
    (fun () ->
       let config = make_test_room_config dir in
       let meta =
         keeper_meta ~name:"status-keeper" ~mention_targets:[ "status-keeper" ] ()
       in
       ...
       let json =
         Masc_mcp.Keeper_exec_memory.keeper_context_status_json ~config ~meta ~ctx_work
         |> Yojson.Safe.from_string
       in
       check
         string
         "recovery source exposed"
         "progress_log"
         Yojson.Safe.Util.(json |> member "recovery_source" |> to_string);
       check
         string
         "sandbox root is tool-ready"
         "."
         Yojson.Safe.Util.(json |> member "sandbox_root" |> to_string);
       check
         string
         "sandbox repos is tool-ready"
         "repos"
         Yojson.Safe.Util.(json |> member "sandbox_repos" |> to_string);
       check
         string
         "sandbox backend is local by default"
         "local"
         Yojson.Safe.Util.(json |> member "sandbox_backend" |> to_string);
       check
         int
         "long-term count exposed"
         1
         Yojson.Safe.Util.(
           json |> member "memory_tier_summary" |> member "long_term" |> to_int))
;;
...
let test_progress_snapshot_cache_tracks_generation () =
  ...
  in
  let text =
    Masc_mcp.Keeper_memory_policy.progress_markdown_of_snapshot ~generation:7 snapshot
  in
  match Masc_mcp.Keeper_memory_policy.progress_snapshot_cache_of_text text with
  | None -> fail "expected progress cache to parse"
  | Some cache ->
    check (option int) "generation parsed from progress header" (Some 7) cache.generation;
    check
      (option string)
      "snapshot goal preserved"
...
let test_memory_search_cross_generation () =
  let dir = test_tmpdir () in
  Fun.protect
    ~finally:(fun () -> cleanup_tmpdir_r dir)
    (fun () ->
       let config = make_test_room_config dir in
       let meta =
         keeper_meta ~name:"cross-gen-keeper" ~mention_targets:[ "cross-gen-keeper" ] ()
       in
       ...
       let history_path =
         Keeper_types.keeper_history_path
           config
           (Masc_mcp.Keeper_id.Trace_id.to_string trace_id)
       in
       write_lines
         history_path
       ...
       let ctx_work =
         KEC.append
           ctx_work
           (Agent_sdk.Types.text_message Agent_sdk.Types.User "hello keeper")
       in
       ...
       let matches =
         Yojson.Safe.Util.(json |> member "matches" |> to_list |> List.map to_string)
       in
       check
         bool
         "match contains deploy canary"
         true
...
let test_memory_search_dedup () =
  ...
  Fun.protect
    ...
    (fun () ->
       let config = make_test_room_config dir in
       let meta =
         keeper_meta ~name:"dedup-keeper" ~mention_targets:[ "dedup-keeper" ] ()
       in
       let trace_id = meta.runtime.trace_id in
       let history_path =
         Keeper_types.keeper_history_path
       ...
       let ctx_work =
         KEC.append
           ctx_work
           (Agent_sdk.Types.text_message Agent_sdk.Types.User "shared needle message")
       in
       let result =
         KET.execute_keeper_tool_call
           ~config
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let ctx_work =
         KEC.append
           ctx_work
           (Agent_sdk.Types.text_message Agent_sdk.Types.User "status report")
       in
       ...
       check
         bool
         "match references schema migration"
         true
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       write_memory_bank
         ...
         [ memory_note
             ~kind:"decision"
             ~text:"Switched to Postgres for persistence"
             ~priority:86
             ~generation:1
             ~turn:3
             ~ts_unix:1000.0
         ...
         ; memory_note
             ~kind:"progress"
             ~text:"Implemented basic keyword matching"
             ~priority:66
             ~generation:2
             ~turn:5
             ~ts_unix:1100.0
       ...
       let first_match =
         Yojson.Safe.Util.(json |> member "matches" |> to_list |> List.hd)
       in
...
let test_memory_search_bank_kind_filter () =
  ...
  Fun.protect
    ...
    (fun () ->
       let config = make_test_room_config dir in
       let meta =
         keeper_meta ~name:"filter-keeper" ~mention_targets:[ "filter-keeper" ] ()
       in
       write_memory_bank
         config
         "filter-keeper"
       ...
       check
         ...
         "text contains Redis"
         true
         (Re.execp (Re.str "Redis" |> Re.compile) text))
;;
...
let test_memory_search_bank_scored_order () =
  ...
  Fun.protect
    ...
    (fun () ->
       let config = make_test_room_config dir in
       let meta =
         keeper_meta ~name:"score-keeper" ~mention_targets:[ "score-keeper" ] ()
       in
       write_memory_bank
         config
         "score-keeper"
...
let test_memory_search_bank_prefers_long_term_over_stale_short_term () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let meta =
         { base_meta with runtime = { base_meta.runtime with generation = 4 } }
       in
       write_memory_bank
         config
         "horizon-keeper"
       ...
       check
         string
         "long_term ranked first"
         "long_term"
         Yojson.Safe.Util.(first |> member "kind" |> to_string);
       check
         string
         "long_term horizon surfaced"
         "long_term"
...
let test_memory_search_bank_empty () =
  ...
  Fun.protect
    ...
    (fun () ->
       let config = make_test_room_config dir in
       let meta =
         keeper_meta ~name:"empty-keeper" ~mention_targets:[ "empty-keeper" ] ()
       in
       let ctx_work = KEC.create ~system_prompt:"test" ~max_tokens:4096 in
       let result =
         KET.execute_keeper_tool_call
...
let test_memory_search_bank_no_match () =
  ...
  Fun.protect
    ...
    (fun () ->
       let config = make_test_room_config dir in
       let meta =
         keeper_meta ~name:"nomatch-keeper" ~mention_targets:[ "nomatch-keeper" ] ()
       in
       write_memory_bank
         config
         "nomatch-keeper"
    ...
    (fun () ->
       ...
       let history_path =
         Keeper_types.keeper_history_path
           ...
           (Masc_mcp.Keeper_id.Trace_id.to_string trace_id)
       in
       write_lines history_path [ {|{"role":"user","content":"alpha from history"}|} ];
       let ctx_work = KEC.create ~system_prompt:"test" ~max_tokens:4096 in
       let result =
         KET.execute_keeper_tool_call
           ~config
           ...
           ~input:
             (`Assoc
                 [ "query", `String "alpha"; "source", `String "all"; "limit", `Int 10 ])
           ()
       in
       let json = Yojson.Safe.from_string result in
       let source = Yojson.Safe.Util.(json |> member "source" |> to_string) in
...
let test_quality_rejects_consensus_spam () =
  with_env "MASC_KEEPER_MEMORY_CONSENSUS_PATTERN" ""
  @@ fun () ->
  check
    bool
    "1234567ep rejected"
...
let test_quality_consensus_pattern_follows_env_changes () =
  with_env "MASC_KEEPER_MEMORY_CONSENSUS_PATTERN" "CUSTOMBLOCK"
  @@ fun () ->
  check
    bool
    "custom pattern rejected"
    false
    (Keeper_memory_bank.is_meaningful_memory_text "CUSTOMBLOCK");
  with_env "MASC_KEEPER_MEMORY_CONSENSUS_PATTERN" "OTHERBLOCK"
  @@ fun () ->
  check
    bool
    "new custom pattern rejected"
    false
    (Keeper_memory_bank.is_meaningful_memory_text "OTHERBLOCK");
  check
    bool
    "old custom pattern no longer rejected"
    true
    (Keeper_memory_bank.is_meaningful_memory_text "CUSTOMBLOCK")
;;
...
let test_quality_rejects_long_text () =
  ...
  check
    ...
    "5000 char rejected"
    false
    (Keeper_memory_bank.is_meaningful_memory_text long)
;;
...
let test_quality_accepts_meaningful () =
  check
    ...
    "meaningful accepted"
    true
    (Keeper_memory_bank.is_meaningful_memory_text
...
let test_schema_version_mismatch () =
  ...
  check
    ...
    "mismatch rejected"
    true
    (Keeper_memory_bank.parse_memory_bank_row line = None)
;;
...
let test_dedup_exact () =
  let items = [ "goal", "Fix bug", 10; "goal", "Fix bug", 20; "next", "Deploy", 30 ] in
...
let test_dedup_semantic () =
  let items = [ "goal", "Fix the login bug", 10; "next", "Fix the login bug", 20 ] in
  let result = Keeper_memory_bank.dedup_memory_candidates items in
...
let test_dedup_semantic_preserves_distinct () =
  let items =
    [ "goal", "Fix the login bug", 10; "goal", "Refactor the database schema", 20 ]
  in
  let result = Keeper_memory_bank.dedup_memory_candidates items in
  ...
  run
    ...
    [ ( "mention"
      , [ test_case "any_mentioned exact target" `Quick test_any_mentioned_exact_target
        ; test_case
            "any_mentioned ambient message"
            `Quick
            test_any_mentioned_ambient_message
        ; test_case
            "policy observation direct mention"
            `Quick
            test_keeper_policy_observation_direct_mention
        ; test_case
            "policy observation ambient message"
            `Quick
            test_keeper_policy_observation_non_mention
        ; test_case
            "user visible reply strips state block"
            `Quick
            test_user_visible_reply_strips_state_block
        ; test_case
            "user visible reply strips skill and state markers"
            `Quick
            test_user_visible_reply_strips_skill_and_state_markers
        ; test_case
            "user visible reply falls back to snapshot progress"
            `Quick
            test_user_visible_reply_falls_back_to_snapshot_progress
        ] )
    ; ( "history_recall"
      , [ test_case
            "load_history_user_messages from jsonl"
            `Quick
            test_load_history_user_messages
        ; test_case
            "load_history_user_messages ignores internal prompt entries"
            `Quick
            test_load_history_user_messages_ignores_internal_prompt_entries
        ; test_case
            "recall_candidates_with_history deduplicates"
            `Quick
            test_recall_candidates_with_history_dedup
        ; test_case
            "recall_candidates_with_history appends history"
            `Quick
    ...
    ; ( "prioritized_action"
      , [ test_case "none when no rules fire" `Quick test_prioritized_action_none
        ; test_case
            "guardrail_stop highest priority"
            `Quick
            test_prioritized_action_guardrail_stop
        ...
        ; test_case
            "compact over handoff"
            `Quick
            test_prioritized_action_compact_over_handoff
        ; test_case "handoff alone" `Quick test_prioritized_action_handoff_alone
        ; test_case
            "guardrail default reason"
            `Quick
            test_prioritized_action_guardrail_default_reason
        ; test_case
            "to_string all variants"
            `Quick
            test_prioritized_action_to_string_all_variants
        ; test_case
            "measurement evaluator respects cooldown"
            `Quick
            test_auto_rule_eval_of_measurement_respects_cooldown
        ; test_case
            "measurement evaluator plan requires both low"
            `Quick
            test_auto_rule_eval_of_measurement_plan_requires_both_low
        ; test_case
            "public evaluator plan requires both low"
            `Quick
            test_evaluate_keeper_auto_rules_plan_requires_both_low
        ; test_case
            "goal_alignment_score returns sentinel on missing messages (#10012)"
            `Quick
            test_goal_alignment_score_missing_messages_returns_sentinel
        ] )
    ; ( "e2e_memory_pipeline"
      , [ test_case
            "write with [STATE] then recall"
            `Quick
            test_memory_write_then_recall_with_state_block
        ; test_case
            "write via meta fallback then recall"
            `Quick
            test_memory_write_then_recall_meta_fallback
        ; test_case
            "memory note persists horizon + source"
            `Quick
            test_memory_write_persists_horizon_and_source
        ; test_case
            "next summary is preserved as memory"
            `Quick
            test_memory_candidates_capture_next_summary
        ; test_case
            "long_term priority matches durable tier"
            `Quick
            test_priority_for_kind_long_term
        ] )
    ; ( "cross_generation_search"
      , [ test_case
            "finds messages from history.jsonl"
            `Quick
            test_memory_search_cross_generation
        ; test_case
            "finds messages from checkpoint only"
            `Quick
            test_memory_search_checkpoint_only
        ; test_case "deduplicates checkpoint and history" `Quick test_memory_search_dedup
        ; test_case
            "finds messages from previous generation via trace_history"
            `Quick
            test_memory_search_prev_generation
        ] )
    ; ( "recovery_context"
      , [ test_case
            "continuity prefers progress log"
            `Quick
            test_read_continuity_summary_prefers_progress_log
        ; test_case
            "context status reports recovery source + tiers"
            `Quick
            test_keeper_context_status_reports_recovery_source_and_tiers
        ; test_case
            "progress cache tracks generation"
            `Quick
            test_progress_snapshot_cache_tracks_generation
        ; test_case
            "stale progress drops short-term prompt memory"
            `Quick
            test_prompt_memory_sections_drop_stale_short_term
        ] )
    ; ( "memory_bank_search"
      , [ test_case
            "basic keyword search in memory bank"
            `Quick
        ...
        ; test_case
            "long_term outranks stale short-term"
            `Quick
            test_memory_search_bank_prefers_long_term_over_stale_short_term
        ; test_case "empty bank returns no_match" `Quick test_memory_search_bank_empty
        ; test_case
            "no matching query returns no_match"
            `Quick
            test_memory_search_bank_no_match
        ; test_case
            "source=history uses legacy search"
            `Quick
            test_memory_search_source_history
        ; test_case
            "source=all merges bank and history"
            `Quick
    ...
    ; ( "memory_quality_filter"
      , [ test_case "placeholder rejects empty" `Quick test_quality_rejects_empty
        ...
        ; test_case
            "placeholder rejects korean 없음"
            `Quick
            test_quality_rejects_korean_none
        ; test_case
            "placeholder rejects korean 없습니다"
            `Quick
            test_quality_rejects_korean_none_formal
        ; test_case
            "placeholder rejects korean 모르겠음"
            `Quick
            test_quality_rejects_korean_dont_know
        ...
        ; test_case
            "consensus pattern follows env changes"
            `Quick
            test_quality_consensus_pattern_follows_env_changes
        ; test_case
            "invalid consensus pattern falls back"
            `Quick
            test_quality_invalid_consensus_pattern_falls_back
        ; test_case
            "max text length gate rejects long text"
            `Quick
            test_quality_rejects_long_text
    ...
    ; ( "memory_dedup"
      , [ test_case "exact dedup removes duplicates" `Quick test_dedup_exact
        ...
        ; test_case
            "semantic dedup preserves distinct"
            `Quick
            test_dedup_semantic_preserves_distinct
        ] )
    ; ( "memory_cap_telemetry"
      , [ test_case
            "cap enforcement reports dropped_by_kind"
            `Quick
            test_cap_dropped_by_kind
        ; test_case
            "total cap reports dropped_by_total_cap"
            `Quick
            test_cap_dropped_by_total
        ] )
    ]
;;
