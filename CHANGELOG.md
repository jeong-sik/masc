...
- Memory search is now counted: the new metric
  `masc_keeper_memory_search_total{source, outcome}` counts searches that
  matched, found nothing, or could not read the memory store, and each
  `memory_search` line in the decision log records how many memories were
  searched. Search results themselves do not change (#38802).
...
- A failed exact-lane run in the TUI detail now shows its failure code after the
  `RUN failed` status instead of a second FAILURE label. Server WARN lines for
  Librarian, Board attention and HITL slot failures now name each cause and
  runtime ID once instead of repeating the same failure words (#38786).
...
- After a turn that started with no history, the resume point a Keeper keeps now
  records "no front" as an absent value instead of an empty string. Resume points
  written by v0.40.0 (#39166) still load; new ones store `null` there (#39185).
...
- `approve-guard` judges each GitHub Actions workflow by its newest run and
  each check name by its newest check-run on the SHA, so an older cancelled,
  failed or Draft-time skipped run no longer blocks APPROVE forever (#38625,
  #38851).
...
- An upgrade keeps your existing `runtime.toml`, so its Codex models still have no prompt-size limit. Add `max-prompt-bytes = 10485760` to each `[models.codex-*]` section to get the vendor limit; without it, a Codex request above 10 MiB is refused by the Codex app-server (#38740).
- A Gate approval queued before this release with a `script` input is refused
  ...
  (#38763).
- Execute's one-line shell field is now `command`. `script` is refused as an unknown field, with no alias. Any client, skill, prompt or saved instruction that sends `script` must switch to `command` (#38763).
...
- The keeper config endpoint reports `prompt.system_prompt` as `available`
  (effective and assembled text) or `unavailable` (typed reason, path,
  detail), replacing `effective_system_prompt` and `assembled_system_prompt`;
  the dashboard and TUI draw the reason instead of an empty prompt (#38427).
...
- A checkpoint written by a later `checkpoint_version` is now its own
  ...
  still read it. This holds only when the writer bumped `checkpoint_version`;
  a codec change shipped without a bump still reads as a parse error and is
  deleted by the clear (#38680) (#38476).
...
- The TUI terminal sanitizers draw bidi controls and zero-width characters
  ...
  call. Emoji are unaffected: the zero width joiner that holds one together
  (🤷‍♂️, 🧑🏽‍💻, 👨‍👩‍👧) stays, as UAX #29 GB11 draws it, and only a joiner that
  joins no pictographs is escaped (#38485).
...
- `keeper_memory_search` follows an absorbed fact to the claim that is current
  ...
  as `event_unreadable_lines` (#38552, #38543).
...
- TUI Overview: Esc now clears the selected task as well as the highlight, so Ctrl-] no longer jumps to a task that is not on screen. `t` selects the first held task, and Enter explains in the footer why nothing opened. The GOALS header says "active work unread" before the first task read instead of "0 of 0 tasks" (#38602).
...
- `scripts/harness/tool_calls/first_call_validity.py` measures how often a
  model's first call to one tool follows the tool's schema at every depth
  plus the handler's own rules, per `runtime.toml` lane, against
...
- The dashboard verification queue (`GET /api/v1/verification/requests?view=awaiting`) says which verdict each waiting row needs: rows carry `intent` (`"complete"` or `"cancel"`, `null` in the history view). A cancel request can only be settled by an operator, and the row did not say it was one (#37970).
- A HITL decision that is delivered again, on boot replay or when the same request is resubmitted, no longer writes another `resolved` row to the approval audit ledger or announces the decision again. Only the first commit resolves; a redelivery wakes the Keeper and logs `hitl resolution redelivered`. The ledger now counts decisions, not deliveries (#37981).
...
- The Librarian's strict chat-store load stops with the file path and line number when it meets a `surface` row it cannot parse. A message from Slack or Discord could otherwise be attributed to a direct channel (#37934).
- Deployment preflight (`deployment_preflight_helper validate-stores`) also reads each keeper's Librarian range-commit receipt (`<keepers-dir>/<keeper>.librarian-range-commit.json`). An unreadable receipt used to pass preflight and then block every memory write for that keeper once deployed (#37938).
...
- The schedule runner no longer writes a `dispatch=deferred` line for every held
  ...
  although nothing was dispatched; `Dispatch_deferred` is gone and the tick
  result carries the held candidates as `held`. The server logs a hold once,
  when it starts, and `/health` lists the current held set under the schedule
  runner status instead of summing a per-tick held count into `totals` (#37912).
...
- A runtime.toml reload keeps the backpressure an unchanged official-client row carries. `same_candidate_binding` answered false for two `Official_client_binding` values, so every reload gave the claude_code, codex and agy rows a fresh observation cell even when provider, model and binding were equal, and a demoted official-client head moved back to the front of its lane on any lane save. The official-client dispatch is built from the provider and model alone, which the reload already compares, so the pair is now the same binding (#38069).
- Task Review says which verdict each request waits on. Nothing read the queue's `intent` field, so the column was blank on every row and the detail pane told every reader the row was "not joined (history view)" — while the live queue's eight rows all carried it, seven of them `cancel`. The field is now read as the domain's own closed pair; a row whose `intent` is `null` says the record does not state one rather than being folded into either verdict, and a word outside the pair reaches the screen as that word. The detail pane draws the case a stop makes under `WHY IT SHOULD STOP`, and says so when the record kept no copy of it (#38015).
- A sub-board `access` value the Board cannot read is refused instead of becoming `Open`. Creating a board with `"access": "Members_only"` or `"private"` over HTTP or through `masc_board_sub_board_create` made an Open board with no error, and the same value on an update left the access alone while reporting success. Create and update on both surfaces now answer a 400 / `Validation_error` naming `open, members_only, owner_only`; an absent `access` still means Open on create and unchanged on update. The two sub-board GET routes also take the public-read gate, so strict HTTP auth covers them like the other Board reads (#38076).
- A Codex head out of usage is recorded as out of quota. The app-server names the failure in `codexErrorInfo`, and the client read only `contextWindowExceeded` from it, so a weekly `usageLimitExceeded` reached the keeper as a generic provider failure: the lane rotated every cycle, nothing reached the quota window, and the status said "provider reported failure". The field is now read into a closed type, with unknown values kept whole, and usage or session budget becomes `HardQuota` as Claude Code's `Quota_blocked` does; rate limit, server, network and auth failures take their existing provider errors (#38077).
- An operator's Task verdict now names the submission the operator read. `POST /api/v1/verification/verdict` took only `task_id` and re-read the Task's current `verification_id`, so a Keeper that resubmitted — or superseded its submission with a cancellation — while the operator read the evidence got its new submission approved on evidence nobody saw. The request now requires `verification_id` (unknown or duplicate fields are refused), the commit compares it under the backlog lock, and a stale one is answered 409 with the new `Task_error.VerificationSuperseded`. The TUI sends the row's `request_id`; the dashboard sends the request row its card draws and refuses to send while that row is not loaded (#38073).
- A keeper whose history no longer matches any completed-turn boundary starts its request at an unknown boundary instead of atom 0. The turn start folded "this trace has no completed turn yet" and "completed-turn lines exist but none matches this history" into one error and answered 0 for both, so after a history rewrite the whole history was sent, the provider refused it, the failed turn wrote no boundary, and the next turn did the same. The second case is now `Unmatched_history`, answered `Turn_boundary_unknown`, which opens on the newest atom (#38070).
- A 429 carrying `Retry-After: 0` keeps its runtime candidate demoted until the candidate answers. The candidate's rate-limit record took a zero-second hint as a stated wait, so the next read cleared it and the head that had just refused was called first again, while the quota window, path rest and same-path resume already read zero as no hint. The record now reads the hint through `Keeper_runtime_failure_route.usable_retry_after`, the one rule, which also refuses infinity as the parse boundary does (#38065).
