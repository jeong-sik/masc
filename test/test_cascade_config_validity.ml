...
let cascade_path () =
  ...
  | _ ->
    failwith
      "MASC_CASCADE_JSON_PATH not set; dune stanza must inject it before running the test"
;;
...
let test_profile_parses_non_empty profile () =
  ...
  check bool (Printf.sprintf "%s has entries" profile) true (strings <> []);
...
let test_unknown_provider_is_dropped () =
  ...
  "regression_models": [
    ...
    "__nonexistent_provider_sentinel__:fake-model"
       ...
       let strings = load_profile_strings ~path:tmp ~profile:"regression" in
