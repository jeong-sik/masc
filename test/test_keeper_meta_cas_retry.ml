...
let test_no_conflict_writes_first_attempt () =
  ...
  Fun.protect ~finally:(fun () -> cleanup_dir base_dir) (fun () ->
    ...
    in
    let m1 = { disk with instructions = "updated instructions" } in
    match
      Keeper_meta_store.write_meta_with_merge
    ...
    | Ok () ->
      ...
      in
      check string "instructions updated" "updated instructions"
        after.instructions
...
let test_retry_succeeds_after_concurrent_bump () =
  ...
  Fun.protect ~finally:(fun () -> cleanup_dir base_dir) (fun () ->
    ...
    let racing = { caller_view with instructions = "racing instructions" } in
    ...
    (* Now the cycle attempts to write its own payload. CAS would fail
       ...
       version and succeed. *)
    let cycle_payload = { caller_view with instructions = "cycle instructions" } in
    ...
    in
    check string "cycle payload wins (last writer)" "cycle instructions"
      final.instructions;
