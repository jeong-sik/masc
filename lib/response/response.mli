...
(** Success with attached non-fatal [warnings] carried on
    {!error_detail}s. *)
val ok_with_warnings : ?message:string -> warnings:error_detail list -> Yojson.Safe.t -> t
...
val errors : ?data:Yojson.Safe.t -> message:string -> error_detail list -> t
...
val make_info : code:string -> message:string -> unit -> error_detail
