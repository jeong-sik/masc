...
### 3.1 핵심 분기 (runtime_runner.ml:438 의 `run` 함수 ~510줄대)
...
```ocaml
let liveness_mode = Runtime_attempt_liveness_config.current_mode () in
let liveness_observer_opt =
  match liveness_mode with
  | Off -> None
  | Observe | Enforce -> Some (Runtime_attempt_liveness_observer.create ...)
```

즉 **liveness_mode 가 master switch** — runtime 단위도 provider 단위도
...
| H2 | keeper_runtime.toml `streaming = true` provider/model 플래그가 *provider config → SDK request body* 변환 단계에서 dead-read (`Runtime_config.openai_compatible_custom_model` 흐름) | `rg '"stream"' lib/runtime_config*` + `rg 'streaming' lib/oas_compat/` 호출 경로 trace |
...
- keeper_runtime.toml 의 `streaming = true` 가 *capability 선언* 인지 *request body 명시* 인지는 `Runtime_config` 파서가 어떻게 해석하는지에 달림. `rg "\"stream\"" lib/runtime_config*` 결과가 0 hit 이면 H2 강력 확정.
...
스코프:
1. `lib/runtime_decl/` 또는 `lib/runtime/runtime_config.ml` 에 *임시
...
2. local llama-server 를 `--verbose` 또는 access-logging wrapper 로
   ...
   (5 분 트래픽 sample 충분).
3. `lib/runtime/runtime_runtime_candidate.ml` 의 `provider_label`
...
- **H1 confirmed**: SDK provider binding adapter 가 `Custom_openai_compat`
  ...
  필요 시 `Runtime_config` 에서 per-model override (`streaming = false`
  허용) 제공.
- **H2 confirmed**: `Runtime_config.openai_compatible_custom_model`
...
4. keeper_runtime.toml 의 `streaming = true` 플래그가 *capability 선언* 인지
   *호출 명시* 인지 — 현재 의미가 *capability 선언* 으로만 활용된다면
   별도 `request_stream = true` 같은 명시적 필드 추가가 더 정직할 수
   있다. RFC-0058 (declarative runtime config) 의 schema 와 cross-ref.
