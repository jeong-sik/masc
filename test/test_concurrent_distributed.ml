...
let parse_args () =
  let specs = [
    ("--dir", Arg.Set_string shared_dir, "Shared directory for MASC workspace");
...
let worker () =
  ...
  let config = Workspace_eio.create_config ~fs !shared_dir in
  ...
  (match Workspace_eio.register_agent config ~name:agent_name ~capabilities:["concurrent"] () with
  ...
  for i = 1 to !num_messages do
    ...
    match Workspace_eio.broadcast config ~from_agent:agent_name ~content with
    | Ok msg ->
        seqs := msg.seq :: !seqs;
  ...
  (* Remove agent *)
  let _ = Workspace_eio.remove_agent config ~name:agent_name in
  ()
...
let verify () =
  ...
  let rec scan seq max_empty =
    if max_empty <= 0 then ()
    else match Workspace_eio.get_message config ~seq with
    | Ok msg ->
