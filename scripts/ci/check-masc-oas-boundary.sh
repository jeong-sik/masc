...
#
# CONTRACT:
#   - Upstream OAS must remain workspaceSession-agnostic. When an OAS
...
# 1. Upstream OAS SDK should not learn MASC workspaceSession vocabulary.
...
PYEOF
  ...
  if [[ -n "$masc_in_surface" ]]; then
    ...
    echo "  OAS must not learn MASC workspaceSession vocabulary."
