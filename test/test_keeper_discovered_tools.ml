...
let dt = Masc.Keeper_discovered_tools.create

let test_add_and_active () =
  let t = dt ~decay_turns:5 in
  Masc.Keeper_discovered_tools.add t ~turn:0
    ~names:[ "tool_execute"; "tool_search_files" ];
  let active = Masc.Keeper_discovered_tools.active_names t ~turn:0 in
...
let test_decay_removes_old () =
  ...
  Masc.Keeper_discovered_tools.add t ~turn:0 ~names:[ "tool_a" ];
  Masc.Keeper_discovered_tools.add t ~turn:2 ~names:[ "tool_b" ];
  (* At turn 4: tool_a last active at 0, decay_turns=3, 4-0=4 > 3 → expired *)
  let expired = Masc.Keeper_discovered_tools.decay t ~turn:4 in
  Alcotest.(check int) "one expired" 1 (List.length expired);
  Alcotest.(check bool) "tool_a expired" true (List.mem "tool_a" expired);
  let active = Masc.Keeper_discovered_tools.active_names t ~turn:4 in
...
let test_mark_used_resets_decay () =
  ...
  Masc.Keeper_discovered_tools.mark_used t ~turn:3 ~name:"tool_a";
  let active = Masc.Keeper_discovered_tools.active_names t ~turn:4 in
  ...
  let expired = Masc.Keeper_discovered_tools.decay t ~turn:7 in
...
let test_readd_resets_decay () =
  ...
  Masc.Keeper_discovered_tools.add t ~turn:3 ~names:[ "tool_a" ];
  let active = Masc.Keeper_discovered_tools.active_names t ~turn:4 in
  Alcotest.(check int) "still active after re-add" 1 (List.length active)

let test_mark_used_unknown_is_noop () =
  let t = dt ~decay_turns:3 in
  Masc.Keeper_discovered_tools.mark_used t ~turn:0 ~name:"nonexistent";
  Alcotest.(check int) "count is 0" 0
    (Masc.Keeper_discovered_tools.count t)
...
let test_clear () =
  ...
  Alcotest.(check int) "count is 3" 3
    (Masc.Keeper_discovered_tools.count t);
  Masc.Keeper_discovered_tools.clear t;
  Alcotest.(check int) "count is 0 after clear" 0
    (Masc.Keeper_discovered_tools.count t)

let test_to_json () =
  let t = dt ~decay_turns:5 in
  Masc.Keeper_discovered_tools.add t ~turn:1 ~names:[ "tool_x" ];
  let json = Masc.Keeper_discovered_tools.to_json t in
  let count =
...
let test_decay_turns_minimum () =
  ...
  let active = Masc.Keeper_discovered_tools.active_names t ~turn:1 in
  Alcotest.(check int) "active at boundary" 1 (List.length active);
  (* At turn 2: 2 - 0 = 2 > 1 → expired *)
  let expired = Masc.Keeper_discovered_tools.decay t ~turn:2 in
