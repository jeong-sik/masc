...
let test_spawn_config_creation () =
  let cfg : Spawn.spawn_config =
    ...
    ; parse_output = Spawn.parse_raw_output
    ; stdin_prompt = true
    ; mcp_mode = Spawn.Mcp_none
    ; prompt_mode = Spawn.Prompt_stdin
    }
  in
...
let test_masc_mcp_tools_has_broadcast () =
  check
    ...
    "has broadcast"
    true
    (List.mem "mcp__masc__masc_broadcast" Spawn.masc_mcp_tools)
;;

let test_masc_mcp_tools_omits_claim () =
  check bool "omits claim" false (List.mem "mcp__masc__masc_claim" Spawn.masc_mcp_tools)
;;

let test_masc_mcp_tools_omits_done () =
  check bool "omits done" false (List.mem "mcp__masc__masc_done" Spawn.masc_mcp_tools)
;;
...
let test_masc_mcp_tools_has_heartbeat () =
  check
    ...
    "has heartbeat"
    true
    (List.mem "mcp__masc__masc_heartbeat" Spawn.masc_mcp_tools)
;;
...
let test_masc_mcp_tools_has_worktree () =
  check
    ...
    "has worktree"
    true
    (List.mem "mcp__masc__masc_worktree_create" Spawn.masc_mcp_tools)
;;
...
let test_lifecycle_suffix_has_protocol () =
  check
    ...
    (try
       let _ =
         Str.search_forward
           (Str.regexp_string "MASC LIFECYCLE")
           Spawn.masc_lifecycle_suffix
           0
       in
       true
     with
...
let test_lifecycle_suffix_has_join () =
  check
    ...
    (try
       let _ =
         Str.search_forward (Str.regexp_string "masc_join") Spawn.masc_lifecycle_suffix 0
       in
       true
     with
     | Not_found -> false)
...
let test_lifecycle_suffix_has_heartbeat () =
  check
    ...
    (try
       let _ =
         Str.search_forward (Str.regexp_string "heartbeat") Spawn.masc_lifecycle_suffix 0
       in
       true
     with
     | Not_found -> false)
...
let test_lifecycle_suffix_has_handover_create () =
  ...
  check
    ...
    (try
       let _ =
         Str.search_forward
           (Str.regexp_string "handover_create")
           Spawn.masc_lifecycle_suffix
           0
       in
       true
...
let test_lifecycle_suffix_omits_relay_checkpoint () =
  check
    bool
    "omits relay_checkpoint"
    false
    (try
       let _ =
         Str.search_forward
           (Str.regexp_string "relay_checkpoint")
           Spawn.masc_lifecycle_suffix
           0
       in
       true
...
let test_spawn_bare_ollama_rejected () =
  if Masc_mcp.Provider_adapter.is_bare_ollama_label "ollama"
  then (
    ...
    check
      ...
      (try
         let _ =
           Str.search_forward (Str.regexp_string "llama:<model>") result.Spawn.output 0
         in
         true
       with
       | Not_found -> false))
...
let test_build_mcp_args_gemini () =
  ...
  check
    ...
    [ "--allowed-mcp-server-names"; "masc"; "--allowed-tools"; "tool1" ]
    flags
;;
...
let test_build_mcp_args_gemini_allowed_tools () =
  ...
  check
    (list string)
    "gemini multi tools"
    [ "--allowed-mcp-server-names"; "masc"; "--allowed-tools"; "tool1"; "tool2" ]
    flags
;;
...
let test_parse_claude_output_valid () =
  let json_str =
    {|{"usage":{"input_tokens":100,"output_tokens":50},"total_cost_usd":0.01,"result":"done"}|}
  in
...
let test_parse_claude_output_with_cache () =
  let json_str =
    {|{"usage":{"input_tokens":100,"output_tokens":50,"cache_creation_input_tokens":20,"cache_read_input_tokens":30},"total_cost_usd":0.02,"result":"ok"}|}
  in
  let p = Spawn.parse_claude_output json_str in
...
let test_parse_gemini_output_response_text () =
  let json =
    {|{"response":"hello from gemini","session_id":"sess-1","stats":{"models":{}}}|}
  in
...
let test_parse_gemini_output_with_cache () =
  let json =
    {|{"usageMetadata": {"promptTokenCount": 100, "candidatesTokenCount": 50, "cachedContentTokenCount": 80}}|}
  in
  let p = Spawn.parse_gemini_output json in
...
let test_parse_gemini_output_with_cost () =
  let json =
    {|{"usageMetadata": {"promptTokenCount": 200, "candidatesTokenCount": 80}, "total_cost_usd": 0.0042}|}
  in
  let p = Spawn.parse_gemini_output json in
...
let test_float_opt_to_json_some () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "3.14") str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_result_to_json_success () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "\"success\":true") str 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "test output") str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_result_to_json_failure () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "\"success\":false") str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_result_to_json_with_cache () =
  ...
  check
    bool
    "has cache_creation_tokens"
    true
    (try
       let _ = Str.search_forward (Str.regexp "cache_creation_tokens") str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_format_token_info_with_tokens () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Tokens") info 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Cost") info 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_format_token_info_with_cache () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "cache") info 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_result_to_string_has_output () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "unique_output_xyz") str 0 in
       true
     with
     | Not_found -> false)
;;
  ...
  run
    ...
    ; ( "lifecycle_suffix"
      , [ test_case "not empty" `Quick test_lifecycle_suffix_not_empty
        ...
        ; test_case
            "omits relay_checkpoint"
            `Quick
            test_lifecycle_suffix_omits_relay_checkpoint
        ] )
    ; ( "get_config"
    ...
    ; ( "parse_gemini_output"
      , [ test_case
            ...
            `Quick
            test_parse_gemini_output_response_text
    ...
    ; "spawn_sync", [ test_case "alias" `Quick test_spawn_sync_is_spawn ]
    ]
;;
