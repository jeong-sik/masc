...
let test_list_handovers_empty () =
  with_eio_env
  @@ fun ~fs config ->
  let before = persistence_counter Safe_ops.persistence_read_drop_reason_list_dir_error in
...
let test_list_handovers_multiple () =
  ...
  let h2 =
    Handover_eio.create_handover
      ...
      ~task_id:"t2"
      ~session_id:"s2"
      ~reason:(Handover_eio.Timeout 60)
  in
...
let test_list_handovers_skips_bad_entries_with_metric () =
  ...
  check
    ...
    1.0
    (persistence_counter Safe_ops.persistence_read_drop_reason_entry_load_error -. before)
;;
...
let () =
  run
    ...
    ; "eio_pending", [ test_case "get pending" `Quick test_get_pending_handovers ]
    ; ( "eio_claim"
      , [ test_case "success" `Quick test_claim_handover_success
        ; test_case "nonexistent" `Quick test_claim_handover_nonexistent
        ] )
    ; "format", [ test_case "as markdown" `Quick test_format_as_markdown ]
    ; "successor_prompt", []
    ]
;;
