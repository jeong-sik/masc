...
let test_cap_long_string () =
  ...
  | Some cg ->
    ...
    Alcotest.(check bool) "goal length near cap" true (String.length cg <= 100 + 3);
    Alcotest.(check bool)
      "ellipsis appended"
      true
      (Astring.String.is_suffix ~affix:"…" cg)
...
let test_cap_long_list () =
  ...
  Alcotest.(check int) "decisions capped to 5" 5 (List.length c.decisions);
  Alcotest.(check string) "first item preserved" "item-0" (List.hd c.decisions)
;;
...
let test_cap_long_item_in_list () =
  ...
  | [ item ] ->
    Alcotest.(check bool) "item length near cap" true (String.length item <= 50 + 3);
    Alcotest.(check bool)
      "ellipsis appended"
      true
      (Astring.String.is_suffix ~affix:"…" item)
...
let test_idempotence () =
  let items = List.init 20 (fun i -> Printf.sprintf "item-%d" i) in
  let s = make_snapshot ~goal:(Some (long_string 500 'q')) ~decisions:items () in
  let c1 = KMP.cap_snapshot s in
  let c2 = KMP.cap_snapshot c1 in
  Alcotest.(check (option string)) "goal idempotent" c1.goal c2.goal;
  Alcotest.(check (list string)) "decisions idempotent" c1.decisions c2.decisions
;;
...
let () =
  Alcotest.run
    ...
    [ ( "cap_snapshot"
      , [ Alcotest.test_case
            "short snapshot unchanged"
            `Quick
            test_short_snapshot_unchanged
        ; Alcotest.test_case
            "long string capped with ellipsis"
            `Quick
            test_cap_long_string
        ; Alcotest.test_case "long list truncated" `Quick test_cap_long_list
        ; Alcotest.test_case "long item in list capped" `Quick test_cap_long_item_in_list
        ; Alcotest.test_case "None fields stay None" `Quick test_none_fields_stay_none
        ; Alcotest.test_case "cap is idempotent" `Quick test_idempotence
        ] )
    ]
;;
