...
let normalize_target_type value = String.trim value |> String.lowercase_ascii

let judgment_surface_for_target_type target_type =
  if Operator_digest_types.is_root_alias target_type
  then Some "command.namespace"
  else None
;;
...
let active_guidance_fields
  ...
  let fallback_recommendation_json =
    `List
      (List.map
         (Operator_digest_types.recommended_action_to_yojson ~actor)
         fallback_recommendations)
  in
