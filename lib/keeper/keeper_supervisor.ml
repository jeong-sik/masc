(** Keeper_supervisor — keeper keepalive fiber supervision.
    ...
    loop and only manages keeper liveness/restart policy.
...
let reconcile_registered_keepers (ctx : _ context) =
  let base_path = ctx.config.base_path in
  Keeper_types.list_registered_keepers ctx.config
         ...
         | Ok (Some meta)
           when not meta.paused
             ...
             if Keeper_registry.is_running ~base_path meta.name then begin
               publish_lifecycle "reconciled" meta.name "late keeper registration";
               Log.Keeper.info "%s: reconciled late keeper" meta.name
             end
         | _ -> ())
let sweep_and_recover (ctx : _ context) =
  ...
  let base_path = ctx.config.base_path in
  reconcile_registered_keepers ctx;
