...
def parse_bundle(doc: dict[str, object], where: str) -> Bundle:
    ...
    for index, raw in enumerate(_require_list(doc, "missions", where)):
        ...
        results[mission_id] = MissionResult(
            id=mission_id, status=status, failed_assertions=tuple(failed)
        )
    return Bundle(
...
def parse_residuals(doc: dict[str, object], where: str) -> Residuals:
    ...
    for index, raw in enumerate(_require_list(doc, "entries", where)):
        ...
        issue = raw.get("issue")
        if issue is not None and (not isinstance(issue, str) or "#" not in issue):
            raise ScoreboardError(f"{label}: issue must be null or 'owner/repo#N'")
        entries.append(
            Residual(
...
def parse_issue_states(doc: dict[str, object], where: str) -> Mapping[str, str]:
    ...
    for issue, state in raw.items():
        if state not in {ISSUE_STATE_OPEN, ISSUE_STATE_CLOSED}:
...
def _band_report(
    missions: tuple[Mission, ...], bundles: tuple[Bundle, ...]
) -> dict[str, object]:
    per_mission: dict[str, int] = {}
    for mission in missions:
        per_mission[mission.id] = sum(
            b.missions[mission.id].status == "passed" for b in bundles
        )
    return {
        "missions": [m.id for m in missions],
        "k": RUNS_PER_ROUND,
        "passes_of_k": per_mission,
        "k_of_3_passed": sum(1 for v in per_mission.values() if v == RUNS_PER_ROUND),
...
def build_scoreboard(
    *,
    ...
    verification = _band_report(band_missions(catalog, VERIFICATION_BAND_PHASES), bundles)
    pilot = _band_report(band_missions(catalog, PILOT_BAND_PHASES), bundles)
    everything = _band_report(catalog, bundles)
    return {
