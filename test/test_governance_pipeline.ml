...
let transition_done_input =
  `Assoc
    [ "action", `String "done"
    ; "notes", `String "Completed task implementation and verified correctness"
    ]
;;
...
let make_tmpdir () =
  let tmpdir =
    Filename.concat
      ...
      (Printf.sprintf "masc-gov-test-%d-%d" (Unix.getpid ()) (Random.bits ()))
  in
...
let test_risk_low_transition_done () =
  ...
  in
  Alcotest.(check string) "transition done is low" "low" (Gp.risk_level_to_string risk)
;;

let test_risk_high_goal_upsert () =
  let risk = Gp.assess_risk ~tool_name:"masc_goal_upsert" ~input:no_args in
  Alcotest.(check string) "goal_upsert is high" "high" (Gp.risk_level_to_string risk)
;;

let test_risk_high_goal_transition () =
  let risk = Gp.assess_risk ~tool_name:"masc_goal_transition" ~input:no_args in
  Alcotest.(check string) "goal_transition is high" "high" (Gp.risk_level_to_string risk)
;;

let test_risk_low_keeper_msg () =
  let risk = Gp.assess_risk ~tool_name:"masc_keeper_msg" ~input:no_args in
  Alcotest.(check string) "keeper_msg is low" "low" (Gp.risk_level_to_string risk)
;;
...
let test_risk_payload_destructive_rm_rf () =
  ...
  Alcotest.(check string)
    "rm -rf payload remains critical"
    "critical"
    (Gp.risk_level_to_string risk)
;;

let test_risk_payload_evasion_only_command_substitution () =
  let input = `Assoc [ "cmd", `String "echo \"ts: $(date -u +%FT%TZ)\" && pwd" ] in
  let risk = Gp.assess_risk ~tool_name:"keeper_bash" ~input in
  Alcotest.(check string)
...
let test_risk_payload_evasion_only_hex_escape () =
  ...
  let risk = Gp.assess_risk ~tool_name:"keeper_bash" ~input in
  Alcotest.(check string)
    "hex escape evasion alone is medium"
    "medium"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_payload_destructive_inside_substitution () =
  ...
  let risk = Gp.assess_risk ~tool_name:"keeper_bash" ~input in
  Alcotest.(check string)
    "destructive inside $(...) stays critical"
    "critical"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_low_unknown () =
  ...
  Alcotest.(check string) "unknown is low" "low" (Gp.risk_level_to_string risk)
;;
...
let test_risk_precedence_critical_over_high () =
  ...
  Alcotest.(check string)
    "force_create is critical (force wins)"
    "critical"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_metadata_destructive_override () =
  ...
  Alcotest.(check string)
    "operation_stop metadata marks destructive"
    "critical"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_metadata_admin_cleanup_override () =
  ...
  Alcotest.(check string)
    "admin_cleanup metadata marks destructive"
    "critical"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_metadata_pg_query_override () =
  ...
  Alcotest.(check string)
    "pg_query (removed) defaults to low"
    "low"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_payload_empty_overwrite () =
  let risk =
    Gp.assess_risk
      ...
      ~input:
        (`Assoc
            [ "path", `String ".worktrees/agent-task/demo.txt"; "content", `String "" ])
  in
  Alcotest.(check string)
    "empty overwrite payload is critical"
    "critical"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_payload_destructive_content () =
  ...
  in
  Alcotest.(check string)
    "destructive payload content is critical"
    "critical"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_payload_destructive_nested_string () =
  let risk =
    Gp.assess_risk
      ...
      ~input:
        (`Assoc [ "payload", `Assoc [ "note", `String "DROP TABLE production_users" ] ])
  in
  Alcotest.(check string)
    "nested destructive payload is critical"
    "critical"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_payload_safe_write_remains_high () =
  ...
  in
  Alcotest.(check string)
    "safe write payload remains high"
    "high"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_payload_safe_nested_string_remains_low () =
  let risk =
    Gp.assess_risk
      ~tool_name:"masc_status"
      ~input:
        (`Assoc [ "payload", `Assoc [ "note", `String "temporary scratch directory" ] ])
  in
  Alcotest.(check string)
    "safe nested payload stays low"
    "low"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_contract_risk_from_delivery_contract () =
  ...
  in
  Alcotest.(check string)
    "delivery contract drives critical risk"
    "critical"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_invalid_delivery_contract_falls_back_to_heuristic () =
  let risk =
    Gp.assess_risk
      ~tool_name:"masc_create_room"
      ~input:
        (`Assoc
            [ ( "delivery_contract"
              , `Assoc
                  [ "summary", `String "missing contract id"; "repair_budget", `Int 0 ] )
            ; "tool_names", `List [ `Int 1 ]
            ])
  in
  Alcotest.(check string)
    "invalid contract falls back to heuristic"
    "high"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_contract_risk_non_object_input_falls_back_to_heuristic () =
  ...
  in
  Alcotest.(check string)
    "non-object input keeps heuristic risk"
    "high"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_contract_risk_does_not_downgrade_heuristic () =
  ...
  in
  Alcotest.(check string)
    "contract risk cannot downgrade high heuristic"
    "high"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_payload_beats_contract_risk () =
  ...
  in
  Alcotest.(check string)
    "payload risk beats contract risk"
    "critical"
    (Gp.risk_level_to_string risk)
;;
...
let test_risk_payload_beats_low_override () =
  let risk =
    Gp.assess_risk
      ~tool_name:"masc_a2a_query_skill"
      ~input:(`Assoc [ "note", `String "rm -rf /tmp/demo" ])
  in
  Alcotest.(check string)
    "payload risk beats low override"
    "critical"
    (Gp.risk_level_to_string risk)
;;
...
let test_development_allows_all () =
  let d =
    Gp.decide ~governance_level:"development" ~tool_name:"masc_delete_room" ~input:`Null
  in
  (match d.action with
   | `Allow -> ()
...
let test_development_allows_low () =
  let d =
    Gp.decide ~governance_level:"development" ~tool_name:"masc_status" ~input:`Null
  in
  match d.action with
  | `Allow -> ()
...
let test_production_allows_low () =
  let d =
    Gp.decide ~governance_level:"production" ~tool_name:"masc_status" ~input:`Null
  in
  match d.action with
  | `Allow -> ()
...
let test_production_allows_medium () =
  let d = Gp.decide ~governance_level:"production" ~tool_name:"masc_join" ~input:`Null in
  match d.action with
  | `Allow -> ()
...
let test_production_allows_high () =
  let d =
    Gp.decide ~governance_level:"production" ~tool_name:"masc_create_room" ~input:`Null
  in
  match d.action with
  | `Allow -> ()
...
let test_production_confirms_critical () =
  let d =
    Gp.decide ~governance_level:"production" ~tool_name:"masc_delete_room" ~input:`Null
  in
  match d.action with
...
let test_enterprise_allows_low () =
  let d =
    Gp.decide ~governance_level:"enterprise" ~tool_name:"masc_status" ~input:`Null
  in
  match d.action with
  | `Allow -> ()
...
let test_enterprise_allows_medium () =
  let d = Gp.decide ~governance_level:"enterprise" ~tool_name:"masc_join" ~input:`Null in
  match d.action with
  | `Allow -> ()
...
let test_enterprise_confirms_high () =
  let d =
    Gp.decide ~governance_level:"enterprise" ~tool_name:"masc_create_room" ~input:`Null
  in
  match d.action with
  | `Require_confirm _ -> ()
...
let test_enterprise_confirms_critical () =
  let d =
    Gp.decide ~governance_level:"enterprise" ~tool_name:"masc_delete_room" ~input:`Null
  in
  match d.action with
  | `Require_confirm _ -> ()
...
let test_paranoid_allows_low () =
  let d = Gp.decide ~governance_level:"paranoid" ~tool_name:"masc_status" ~input:`Null in
  match d.action with
  | `Allow -> ()
...
let test_paranoid_confirms_medium () =
  let d = Gp.decide ~governance_level:"paranoid" ~tool_name:"masc_join" ~input:`Null in
  match d.action with
  | `Require_confirm _ -> ()
...
let test_paranoid_confirms_high () =
  let d =
    Gp.decide ~governance_level:"paranoid" ~tool_name:"masc_create_room" ~input:`Null
  in
  match d.action with
  | `Require_confirm _ -> ()
...
let test_paranoid_confirms_critical () =
  let d =
    Gp.decide ~governance_level:"paranoid" ~tool_name:"masc_delete_room" ~input:`Null
  in
  match d.action with
  | `Require_confirm _ -> ()
...
let test_decision_has_trace_id () =
  ...
  in
  Alcotest.(check bool)
    "trace_id starts with gov_"
    true
    (String.length d.trace_id > 4 && String.sub d.trace_id 0 4 = "gov_")
;;
...
let test_trace_ids_unique () =
  ...
  let d2 =
    Gp.decide ~governance_level:"development" ~tool_name:"masc_status" ~input:`Null
  in
  Alcotest.(check bool) "trace_ids differ" true (d1.trace_id <> d2.trace_id)
;;
...
let setup () = Tool_dispatch.clear_hooks ()
...
let test_hook_development_allows () =
  ...
  setup ();
  Tool_dispatch.register ~tool_name:"__gov_test_delete" ~handler:(fun ~name:_ ~args:_ ->
    Some (true, "ok"));
...
let test_hook_production_blocks_critical () =
  ...
  Fs_compat.set_fs (Eio.Stdenv.fs env);
  setup ();
  Tool_dispatch.register ~tool_name:"__gov_test_delete2" ~handler:(fun ~name:_ ~args:_ ->
...
let test_unknown_governance_level_fail_closed_on_critical () =
  ...
  let d =
    Gp.decide ~governance_level:"nonexistent" ~tool_name:"masc_delete_room" ~input:`Null
  in
  match d.action with
  | `Require_confirm _ -> ()
...
let test_trifecta_empty () =
  let count, _, _, _ = Gp.assess_trifecta ~active_tool_names:[] in
  Alcotest.(check int) "0 classes" 0 count
;;

let test_trifecta_bash_spans_all () =
  (* keeper_bash alone has all 3 classes *)
  let count, ext, sens, state = Gp.assess_trifecta ~active_tool_names:[ "keeper_bash" ] in
...
let test_escalation_state_mod_in_trifecta () =
  ...
  let escalated =
    Gp.combinatorial_risk_escalation
      ...
      ~input:`Null
  in
  Alcotest.(check string) "escalated to high" "high" (Gp.risk_level_to_string escalated)
;;
...
let test_escalation_keeps_higher_risk () =
  ...
  let escalated =
    Gp.combinatorial_risk_escalation
      ...
      ~input:`Null
  in
  Alcotest.(check string) "stays critical" "critical" (Gp.risk_level_to_string escalated)
;;
...
let test_escalation_no_trifecta_no_change () =
  ...
  in
  Alcotest.(check string)
    "stays low without trifecta"
    "low"
    (Gp.risk_level_to_string unchanged)
;;
...
let test_escalation_non_state_mod_unchanged () =
  ...
  let unchanged =
    Gp.combinatorial_risk_escalation
      ...
      ~input:`Null
  in
  Alcotest.(check string) "read-only stays low" "low" (Gp.risk_level_to_string unchanged)
;;
...
let test_escalation_read_only_keeper_shell_gh_unchanged () =
  ...
  in
  Alcotest.(check string)
    "read-only keeper_shell op=gh stays low"
    "low"
    (Gp.risk_level_to_string unchanged)
;;
  ...
  Alcotest.run
    ...
    [ ( "risk_assessment"
      , [ Alcotest.test_case "critical: delete" `Quick test_risk_critical_delete
        ...
        ; Alcotest.test_case
            "medium: transition start"
            `Quick
            test_risk_medium_transition_start
        ; Alcotest.test_case
            "payload: rm -rf is critical"
            `Quick
            test_risk_payload_destructive_rm_rf
        ; Alcotest.test_case
            "payload: $(date) is medium not critical"
            `Quick
            test_risk_payload_evasion_only_command_substitution
        ; Alcotest.test_case
            "payload: hex escape alone is medium"
            `Quick
            test_risk_payload_evasion_only_hex_escape
        ; Alcotest.test_case
            "payload: destructive inside $(...) is critical"
            `Quick
            test_risk_payload_destructive_inside_substitution
        ; Alcotest.test_case "high: goal upsert" `Quick test_risk_high_goal_upsert
        ; Alcotest.test_case "high: goal transition" `Quick test_risk_high_goal_transition
    ...
    ; ( "lethal_trifecta"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "unclassified tools ignored"
            `Quick
            test_trifecta_unclassified_tools_ignored
        ; Alcotest.test_case
            "escalation: state_mod in trifecta"
            `Quick
            test_escalation_state_mod_in_trifecta
        ; Alcotest.test_case
            "escalation: keeps higher risk"
            `Quick
            test_escalation_keeps_higher_risk
        ; Alcotest.test_case
            "escalation: no trifecta no change"
            `Quick
            test_escalation_no_trifecta_no_change
        ; Alcotest.test_case
            "escalation: non-state_mod unchanged"
            `Quick
            test_escalation_non_state_mod_unchanged
        ; Alcotest.test_case
            "escalation: read-only keeper_shell op=gh unchanged"
            `Quick
            test_escalation_read_only_keeper_shell_gh_unchanged
        ; Alcotest.test_case
            "capabilities: known tool"
            `Quick
            test_tool_capabilities_known
        ; Alcotest.test_case
            "capabilities: unknown tool"
            `Quick
            test_tool_capabilities_unknown
        ] )
    ; ( "governance_levels"
      , [ Alcotest.test_case "development allows all" `Quick test_development_allows_all
        ; Alcotest.test_case "development allows low" `Quick test_development_allows_low
        ; Alcotest.test_case "production allows low" `Quick test_production_allows_low
        ; Alcotest.test_case
            "production allows medium"
            `Quick
            test_production_allows_medium
        ; Alcotest.test_case "production allows high" `Quick test_production_allows_high
        ; Alcotest.test_case
            "production confirms critical"
            `Quick
            test_production_confirms_critical
        ; Alcotest.test_case "enterprise allows low" `Quick test_enterprise_allows_low
        ; Alcotest.test_case
            "enterprise allows medium"
            `Quick
            test_enterprise_allows_medium
        ; Alcotest.test_case
            "enterprise confirms high"
            `Quick
            test_enterprise_confirms_high
        ; Alcotest.test_case
            "enterprise confirms critical"
            `Quick
            test_enterprise_confirms_critical
        ; Alcotest.test_case "paranoid allows low" `Quick test_paranoid_allows_low
        ; Alcotest.test_case
            "paranoid confirms medium"
            `Quick
            test_paranoid_confirms_medium
        ; Alcotest.test_case "paranoid confirms high" `Quick test_paranoid_confirms_high
        ; Alcotest.test_case
            "paranoid confirms critical"
            `Quick
            test_paranoid_confirms_critical
        ; Alcotest.test_case
            "unknown level fail-closed on critical (#7641)"
            `Quick
    ...
    ; ( "pre_hook_integration"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "blocked response structure: claim_next"
            `Quick
