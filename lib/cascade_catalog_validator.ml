...
let is_provider_unavailable_error msg = contains_substring ~needle:"unavailable" msg
...
let priority_tier_issue ~profile configured_specs raw_tiers =
  ...
  then
    Some
      ...
      ; message =
          Printf.sprintf
            "Cascade preset %s uses priority_tier but has no configured models to \
             validate."
            profile
      }
  else (
    ...
    then
      Some
        ...
        ; message =
            Printf.sprintf
              "Cascade preset %s uses priority_tier, but every tier collapses after \
               model-id normalization; runtime will fall back to failover."
              profile
        }
    else if List.length normalized < List.length raw_tiers
    then
      Some
        { profile = Some profile
        ...
        ; message =
            Printf.sprintf
              "Cascade preset %s uses priority_tier, but %d/%d tier(s) collapse after \
         ...
         (match strategy_cfg.tiers with
          | None ->
            Some
              ...
              ; message =
                  Printf.sprintf
                    "Cascade preset %s uses priority_tier without a valid non-empty \
                     <name>_tiers configuration."
                    profile
              }
          | Some raw_tiers -> priority_tier_issue ~profile model_specs raw_tiers)
       | Ok _ -> None)
  in
  [ invalid_model_issue; strategy_issue ] |> List.filter_map (fun issue -> issue)
;;
...
let diagnose_catalog ~config_path =
  ...
  | Error msg ->
    [ { profile = None
      ; severity = Catalog_error
      ; message =
          Printf.sprintf "Cascade catalog %s could not be loaded: %s" config_path msg
      }
    ]
  | Ok json ->
...
let error_messages_by_profile ~config_path =
  ...
  |> List.fold_left
       ...
       StringMap.empty
  |> StringMap.bindings
  |> List.map (fun (profile, messages) -> profile, dedupe_keep_order messages)
;;
