...
let render_inline_skip_reason ~tool_name ~reason_code ~reason_text : string =
  let replacement_hint =
    ...
    | Some replacement -> Printf.sprintf " replacement=%s" (escape_field replacement)
    | None -> ""
  in
  Printf.sprintf
...
let broadcast_tool_skipped ~keeper_name ~tool_name ~reason_code =
  Dashboard_governance_metrics.record_tool_skipped ~keeper_name ~tool_name ~reason_code;
  try
    Sse.broadcast
      (`Assoc
...
let notify_gate_decision on_gate_decision (event : gate_decision_event) =
  try on_gate_decision event with
  | Eio.Cancel.Cancelled _ as e -> raise e
  | exn ->
    Log.Keeper.warn
  ...
  | Some bus ->
    ...
    (try
       Oas_bus_instrument.publish
         bus
         (Oas.Event_bus.mk_event (Oas.Event_bus.Custom ("masc.keeper_gate", payload)))
     with
     | Eio.Cancel.Cancelled _ as e -> raise e
     | exn ->
       Log.Keeper.warn
...
let hooks_of_pre_tool_use (fn : Oas.Hooks.hook) : Oas.Hooks.hooks =
...
let compose_all (hs : Oas.Hooks.hooks list) : Oas.Hooks.hooks =
  List.fold_left (fun acc h -> Oas.Hooks.compose ~outer:acc ~inner:h) Oas.Hooks.empty hs
;;
...
let timing_guard ~(tool_start_time : float ref) : Oas.Hooks.hooks =
...
let custom_guard
  ...
  hooks_of_pre_tool_use (fun event ->
    match event with
    | Oas.Hooks.PreToolUse { tool_name; input; accumulated_cost_usd; turn; _ } ->
      ...
      (match guard ~tool_name ~input with
       | Some reason ->
         ...
         Log.Keeper.info "keeper:%s pre_tool_use guard blocked %s" keeper_name tool_name;
         broadcast_tool_skipped ~keeper_name ~tool_name ~reason_code:"pre_tool_use_guard";
         report_gate_decision
           on_gate_decision
           ~stage:"pre_tool_use_guard"
           ~decision:"override"
    ...
    | Oas.Hooks.PreToolUse { tool_name; input; accumulated_cost_usd; turn; _ } ->
      ...
      then (
        ...
        Log.Keeper.warn
          ...
          tool_name
          new_count;
        broadcast_tool_skipped ~keeper_name ~tool_name ~reason_code:"streak_gate";
        report_gate_decision
          on_gate_decision
          ~stage:"streak_gate"
          ~decision:"override"
          ...
          ~stage_latency_ms:latency_ms;
        Oas.Hooks.Override
          (render_inline_skip_reason ~tool_name ~reason_code:"streak_gate" ~reason_text))
...
let deny_guard
  ...
  hooks_of_pre_tool_use (fun event ->
    ...
    | Oas.Hooks.PreToolUse { tool_name; input; accumulated_cost_usd; turn; _ } ->
      let t0 = Time_compat.now () in
      let keeper_name = !meta_ref.Keeper_types.name in
      if List.mem tool_name denied
      then (
        let reason_text = "tool is on the keeper deny list" in
        let latency_ms = (Time_compat.now () -. t0) *. 1000.0 in
        Log.Keeper.warn "keeper:%s deny list: blocked %s" keeper_name tool_name;
        ...
        report_gate_decision
          ...
          ~stage_latency_ms:latency_ms;
        Oas.Hooks.Override
          (render_inline_skip_reason ~tool_name ~reason_code:"keeper_deny" ~reason_text))
      else Oas.Hooks.Continue
    | _ -> Oas.Hooks.Continue)
;;
      ...
      (match max_cost_usd with
       | Some limit when accumulated_cost_usd >= limit ->
         ...
         report_gate_decision
           ...
           ~stage_latency_ms:latency_ms;
         Oas.Hooks.Override
           (render_inline_skip_reason ~tool_name ~reason_code:"cost_gate" ~reason_text)
       | _ -> Oas.Hooks.Continue)
    | _ -> Oas.Hooks.Continue)
;;
...
let destructive_guard
  ...
  hooks_of_pre_tool_use (fun event ->
    ...
    | Oas.Hooks.PreToolUse { tool_name; input; accumulated_cost_usd; turn; _ } ->
      ...
      else if not (Tool_dispatch.is_destructive tool_name)
      then Oas.Hooks.Continue
      else (
        let t0 = Time_compat.now () in
        let keeper_name = !meta_ref.Keeper_types.name in
        ...
        | Some (pattern, desc) ->
          ...
          Log.Keeper.warn
            ...
            pattern
            desc;
          broadcast_tool_skipped ~keeper_name ~tool_name ~reason_code:"destructive_guard";
          report_gate_decision
            on_gate_decision
...
let governance_approval_guard ~(meta_ref : Keeper_types.keeper_meta ref) ~on_gate_decision
  : Oas.Hooks.hooks
  =
  hooks_of_pre_tool_use (fun event ->
    match event with
...
let build_chain
      ...
      ~on_gate_decision
      ~(pre_tool_use_guard : tool_name:string -> input:Yojson.Safe.t -> string option)
  : Oas.Hooks.hooks
  =
  compose_all
