...
module Agent_turn = struct
  ...
  let all_phases =
    [ Idle; Prompting; Awaiting; Parsing; Dispatching; Collecting; Finalizing ]
  ;;
...
module Tool_validation = struct
  ...
  let phase_to_string = function
    | Unchecked -> "unchecked"
    | Det_correcting -> "det_correcting"
    | Det_valid -> "det_valid"
    | Det_invalid -> "det_invalid"
    | Nondet_retrying -> "nondet_retrying"
    | Valid -> "valid"
    | Rejected -> "rejected"
  ;;
  ...
  let event_to_string = function
    ...
    | Nondet_attempt n -> Printf.sprintf "nondet_attempt(%d)" n
    | Nondet_fixed -> "nondet_fixed"
    | Nondet_exhausted -> "nondet_exhausted"
  ...
  let apply_event ?(max_nondet_retries = default_max_nondet_retries) ~current event =
    ...
    | Nondet_retrying, Nondet_attempt n ->
      if n < max_nondet_retries then Applied Nondet_retrying else Applied Rejected
    | phase, event -> Ignored { phase; event }
  ;;
...
let check_invariants (state : product) : (unit, string) result =
  ...
  (match state.keeper with
   | Keeper.Stopped | Keeper.Dead ->
     if state.turn <> Agent_turn.Idle
     then
       add
         (Printf.sprintf
            "keeper=%s but turn=%s (expected Idle)"
  ...
  (match state.keeper with
   | Keeper.Draining ->
     if state.turn = Agent_turn.Prompting
     then add "keeper=Draining but turn=Prompting (no new LLM calls during drain)"
   | Keeper.Offline
   | Keeper.Running
   | Keeper.Failing
   | Keeper.Overflowed
  ...
  (match state.validation with
   | Tool_validation.Nondet_retrying ->
     if state.turn <> Agent_turn.Dispatching
     then
       add
         (Printf.sprintf
            "validation=Nondet_retrying but turn=%s (expected Dispatching)"
            (Agent_turn.phase_to_string state.turn))
   | Tool_validation.Unchecked
   | Tool_validation.Det_correcting
   | Tool_validation.Det_valid
   | Tool_validation.Det_invalid
   | Tool_validation.Valid
   | Tool_validation.Rejected -> ());
   ...
   | Keeper.Compacting ->
     (match state.turn with
      | Agent_turn.Prompting | Agent_turn.Awaiting ->
        add
          (Printf.sprintf
             "keeper=Compacting but turn=%s (no new LLM calls during compaction)"
             (Agent_turn.phase_to_string state.turn))
      | Agent_turn.Idle
      | Agent_turn.Parsing
      | Agent_turn.Dispatching
      | Agent_turn.Collecting
      | Agent_turn.Finalizing -> ())
   | Keeper.Offline
   | Keeper.Running
   | Keeper.Failing
   | Keeper.Overflowed
...
let apply_validation_event ?max_nondet_retries state event =
  ...
  then
    Error
      (Printf.sprintf
         "validation event %s rejected: turn=%s (expected Dispatching)"
         (Tool_validation.event_to_string event)
         (Agent_turn.phase_to_string state.turn))
  else (
    let new_validation =
      Tool_validation.apply_event_lossy
        ?max_nondet_retries
