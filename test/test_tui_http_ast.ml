...
let test_keeper_chat_uses_current_async_contract () =
  ...
  check int "message renderer emits no direct cursor move" 0
    (Ast_grep.count_calls_in_value_binding ~module_path:render_path
       ~binding_name:"render_keeper_message" ~callee:"Ansi.move_to");
  check bool "message renderer delegates every branch to a frame" true
    (Ast_grep.count_calls_in_value_binding ~module_path:render_path
       ~binding_name:"render_keeper_message" ~callee:"finish_frame"
     >= 3);
...
let test_render_loop_uses_monotonic_dirty_schedule () =
  ...
  check int "TUI installs and clears the console write observer" 2
    (Ast_grep.count_calls_in_value_binding ~module_path:main_path
       ~binding_name:"main" ~callee:"Console_sink.set_write_observer");
  check int "terminal cleanup clears the console write observer" 1
    (Ast_grep.count_calls_in_value_binding ~module_path:main_path
       ~binding_name:"cleanup" ~callee:"Console_sink.set_write_observer");
  check int "external writes schedule a forced repaint" 1
    (Ast_grep.count_calls_in_value_binding ~module_path:main_path
       ~binding_name:"main"
       ~callee:"terminal_write_outside_frame_pending");
