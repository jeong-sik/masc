...
let make_tool name description : Agent_sdk.Tool.t =
  Agent_sdk.Tool.create ~name ~description ~parameters:[] (fun _input ->
    Ok { content = "ok" })
;;

(** A mock rerank_fn that returns the first [k] candidates in order. *)
let mock_rerank_passthrough ~context:_ ~candidates = List.map fst candidates

(** A mock rerank_fn that reverses candidate order (verifies reranking effect). *)
let mock_rerank_reverse ~context:_ ~candidates = List.rev_map fst candidates
...
let mock_rerank_failing ~context:_ ~candidates:_ = failwith "LLM unavailable"
...
let test_topk_llm_basic_selection () =
  ...
  let selected =
    Agent_sdk.Tool_selector.select_names
      ~strategy
      ~context:"post a message on the board"
      ~tools:test_tools
  in
  ...
  Alcotest.(check bool)
    ...
    true
    (List.mem "keeper_context_status" selected);
  Alcotest.(check bool) "at most k+always results" true (List.length selected <= 6)
;;

let test_topk_llm_rerank_effect () =
  let passthrough_strategy =
    Agent_sdk.Tool_selector.TopK_llm
      { k = 3
      ; bm25_prefilter_n = 10
      ; always_include = []
  ...
  Alcotest.(check bool)
    "reranking changes results"
    true
    (pass_names <> rev_names || List.length test_tools <= 3)
;;
...
let test_topk_llm_fallback_on_failure () =
  ...
  Alcotest.(check bool)
    "always_include survives failure"
    true
    (List.mem "keeper_stay_silent" selected)
;;
...
let test_topk_llm_confidence_gate () =
  ...
  let strategy =
    Agent_sdk.Tool_selector.TopK_llm
      ...
      ; (* Impossibly high — will always skip LLM *)
        rerank_fn = counting_rerank
      }
  in
  let _selected =
    Agent_sdk.Tool_selector.select_names ~strategy ~context:"anything" ~tools:test_tools
  in
  Alcotest.(check int) "LLM not called when confidence below threshold" 0 !call_count
;;
...
let test_topk_llm_empty_tools () =
  ...
  in
  let selected =
    Agent_sdk.Tool_selector.select_names ~strategy ~context:"anything" ~tools:[]
  in
...
let test_topk_llm_always_include_survives () =
    ...
    Agent_sdk.Tool_selector.TopK_llm
      ...
      ; rerank_fn =
          (fun ~context:_ ~candidates ->
            ...
            List.filter_map
              (fun (name, _) ->
                 if String.length name > 6 && String.sub name 0 6 = "keeper"
                 then Some name
                 else None)
              candidates
  ...
  in
  Alcotest.(check bool)
    "always_include[0] present"
    true
    (List.mem "keeper_stay_silent" selected);
  Alcotest.(check bool)
    "always_include[1] present"
    true
    (List.mem "keeper_context_status" selected)
;;
...
let test_selection_boundary_preserves_deterministic_floor () =
  ...
  in
  Alcotest.(check bool)
    "input carries duplicate across boundary"
    true
    (List.mem "keeper_board_post" deterministic_prefilter
     && List.mem "keeper_board_post" llm_selected);
  let board_post_count =
    List.fold_left
      (fun acc name -> if name = "keeper_board_post" then acc + 1 else acc)
      0
      merged
  in
  Alcotest.(check int) "merged list contains 4 unique tools" 4 (List.length merged);
  Alcotest.(check int) "duplicate removed from merged result" 1 board_post_count;
...
let test_selection_boundary_appends_llm_only_extras () =
  ...
  Alcotest.(check bool)
    "deterministic tool stays ahead of llm extra"
    true
...
let test_deterministic_prefilter_surfaces_code_tools () =
  ...
  in
  Alcotest.(check bool)
    "code search appears without llm rerank"
    true
    (List.mem "masc_code_search" selected)
;;

let test_deterministic_prefilter_surfaces_code_read_for_explicit_read_intent () =
  let selected =
    deterministic_prefilter_for ~query_text:"read source file contents" ~selection_limit:5
  in
  Alcotest.(check bool)
    "code read appears for explicit read intent"
    true
    (List.mem "masc_code_read" selected)
;;

let test_deterministic_prefilter_surfaces_code_read_for_code_path_hint () =
  let selected =
    deterministic_prefilter_for ~query_text:"open lib/tool_code.ml" ~selection_limit:5
  in
  Alcotest.(check bool)
    "code read appears for code path hint"
    true
    (List.mem "masc_code_read" selected)
;;
...
let test_deterministic_prefilter_surfaces_code_symbols_for_explicit_symbol_intent () =
  let selected =
    deterministic_prefilter_for ~query_text:"show function symbols" ~selection_limit:5
  in
  Alcotest.(check bool)
    "code symbols appears for explicit symbol intent"
    true
    (List.mem "masc_code_symbols" selected);
  Alcotest.(check bool)
    "code read stays out of symbol-only intent"
    false
    (List.mem "masc_code_read" selected)
;;
...
let test_deterministic_prefilter_hides_code_navigation_without_code_intent () =
  ...
  in
  Alcotest.(check bool)
    "code search stays hidden for non-code query"
    false
    (List.mem "masc_code_search" selected);
  Alcotest.(check bool)
    "code read stays hidden for non-code query"
    false
    (List.mem "masc_code_read" selected);
  Alcotest.(check bool)
    "code symbols stays hidden for non-code query"
    false
    (List.mem "masc_code_symbols" selected)
;;
...
let test_deterministic_prefilter_hides_code_read_for_generic_file_read () =
  let selected =
    deterministic_prefilter_for ~query_text:"read file contents" ~selection_limit:5
  in
  Alcotest.(check bool)
    "code read stays hidden for generic file read"
    false
    (List.mem "masc_code_read" selected)
;;
...
let test_keeper_config_defaults () =
  ...
  Alcotest.(check string) "default cascade name" "tool_rerank" cascade
;;
...
let () =
  Alcotest.run
    "keeper_topk_llm"
    [ ( "topk_llm_selection"
      , [ Alcotest.test_case "basic selection" `Quick test_topk_llm_basic_selection
        ; Alcotest.test_case "rerank effect" `Quick test_topk_llm_rerank_effect
        ; Alcotest.test_case
            "fallback on rerank failure"
            `Quick
            test_topk_llm_fallback_on_failure
        ; Alcotest.test_case
            "confidence gate skips LLM"
            `Quick
            test_topk_llm_confidence_gate
        ; Alcotest.test_case "empty tools" `Quick test_topk_llm_empty_tools
        ; Alcotest.test_case "keyword reranker" `Quick test_topk_llm_keyword_rerank
        ; Alcotest.test_case
            "always_include survives"
            `Quick
