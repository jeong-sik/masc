...
let mermaid_lines phase = Keeper_state_machine_mermaid.phase_to_mermaid ~current:phase |> String.split_on_char '\n'
...
let test_mermaid_all_phases_appear () =
  List.iter
    (fun phase ->
       let diagram = Keeper_state_machine_mermaid.phase_to_mermaid ~current:phase in
       let id = Keeper_state_machine_mermaid.phase_to_mermaid_id phase in
       check
         bool
...
let test_mermaid_buffer_class () =
  let buffer_phases =
    [ SM.Failing; SM.Draining; SM.Restarting ]
  in
  List.iter
    (fun phase ->
       let lines = mermaid_lines phase in
