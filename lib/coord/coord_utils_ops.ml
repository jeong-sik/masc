...
let room_id_allowed_re =
  Re.(
    compile
      (whole_string
         (rep1 (alt [ rg 'A' 'Z'; rg 'a' 'z'; rg '0' '9'; char '.'; char '_'; char '-' ]))))
;;

let validate_room_id room_id =
  let room_id = String.trim room_id in
  if room_id = ""
  then Error "Coord id cannot be empty"
  else if String.length room_id > 128
  then Error "Coord id too long (max 128 chars)"
  else if room_id = "." || room_id = ".."
  then Error "Coord id cannot be '.' or '..'"
  else if contains_substring room_id "/" || contains_substring room_id "\\"
  then Error "Coord id cannot contain path separators"
  else if contains_substring room_id ".."
  then Error "Coord id cannot contain traversal segments"
  else if not (Re.execp room_id_allowed_re room_id)
  then Error "Coord id may only contain letters, digits, dot, underscore, and hyphen"
  else Ok room_id
;;
...
let validate_file_path path =
  ...
  if String.length path > 500
  then Error "File path too long (max 500 chars)"
  else if contains_substring path "<" || contains_substring path ">"
  then Error "Invalid characters in path (security)"
...
let safe_filename name =
  ...
  String.iter
    (fun c ->
       ...
       in
       if valid
       then Buffer.add_char buf c_lower
       else Buffer.add_string buf (Printf.sprintf "_%02x" (Char.code c)))
    name;
  Buffer.contents buf
;;
...
let validate_file_path_r path : (string, masc_error) result =
  (* Delegate to Validation module, convert error type *)
  if String.length path > 500
  then Error (InvalidFilePath "too long (max 500 chars)")
  else if contains_substring path "<" || contains_substring path ">"
  then Error (InvalidFilePath "invalid characters (security)")
  else (
    match Validation.Safe_path.validate_relative path with
    | Ok _ -> Ok path
...
let ensure_initialized config =
  ...
  then invalid_arg "MASC not initialized. Use masc_init first."
;;

let ensure_initialized_r config : (unit, masc_error) result =
  if is_initialized config then Ok () else Error NotInitialized
;;
...
let read_json_local_result path = Safe_ops.read_json_file_safe path
       ...
       (match backend_get config ~key with
        | Ok (Some content) ->
          ...
          then `Assoc []
            ...
            match Safe_ops.parse_json_safe ~context:"read_json_root" trimmed with
            | Ok json -> json
            | Error msg ->
        ...
        | Error e ->
          Log.Misc.warn
            "[read_json_root] backend_get failed for %s: %s"
            key
       ...
       (match backend_get config ~key with
        | Ok (Some content) ->
          ...
          then `Assoc []
            ...
            match Safe_ops.parse_json_safe ~context:"read_json" trimmed with
            | Ok json -> json
            | Error msg ->
        ...
        | Error e ->
          Log.Misc.warn
            "[read_json] backend_get failed for %s: %s"
            key
            (Backend_types.show_error e);
          `Assoc []))
  | None -> read_json_local path
...
let read_json_result config path =
  let parse_backend_json ~context content =
    let trimmed = String.trim content in
    if trimmed = "" then Ok (`Assoc []) else Safe_ops.parse_json_safe ~context trimmed
  in
  match key_of_path config path with
  | Some key ->
    (match config.backend with
     ...
     | Memory _ | FileSystem _ ->
       (match backend_get config ~key with
        | Ok (Some content) -> parse_backend_json ~context:"read_json_result" content
        | Ok None -> Ok (`Assoc [])
        | Error e ->
          Error
            (Printf.sprintf
...
let read_text config path =
  ...
  | Some key ->
    (match backend_get config ~key with
     ...
     | Error e ->
       Log.Misc.warn
         "[read_text] backend_get failed for %s: %s"
         key
         (Backend_types.show_error e);
       "")
  | None -> if Fs_compat.file_exists path then Fs_compat.load_file path else ""
;;
...
let write_json config path json =
  ...
  | None ->
    (match write_json_local path json with
     | Ok () -> ()
     | Error msg -> Log.Misc.warn "write_json: local write failed for %s: %s" path msg)
;;
...
let write_text config path content =
  ...
  | None ->
    (match write_text_local path content with
     | Ok () -> ()
     | Error msg -> Log.Misc.warn "write_text: local write failed for %s: %s" path msg)
;;
...
let read_json_opt config path =
  ...
  | Some key ->
    (match backend_get config ~key with
     ...
     | Error e ->
       Log.Misc.warn
         "[read_json_opt] backend_get failed for %s: %s"
         key
         (Backend_types.show_error e);
       None)
  | None -> if Sys.file_exists path then Some (read_json_local path) else None
;;
...
let read_agent_with_repair config path =
  ...
  | Ok agent as ok ->
    if agent_json_needs_repair json
    then (
      Log.Coord.warn
...
let with_distributed_lock ?clock config _path key f =
  ...
  then
    Common.protect
      ...
      ~finally:(fun () ->
        ...
        | Error e ->
          let msg =
            match e with
            | Backend_types.ConnectionFailed s
            | NotFound s
            | IOError s
            | BackendNotSupported s
            | InvalidKey s
            | AlreadyExists s -> s
          in
    ...
    (Atomic.get Coord_hooks.distributed_lock_acquire_failed_fn) ~key ~attempts:50;
    invalid_arg
      (Printf.sprintf
...
let with_file_lock_impl ?clock config path f =
  ...
  | Some key ->
    (match config.backend with
     ...
     | FileSystem _ -> with_distributed_lock ?clock config path key f)
;;

let with_file_lock_eio ~clock config path f = with_file_lock_impl ~clock config path f
...
let with_file_lock_r_impl ?clock config path f : ('a, masc_error) result =
  ...
  | Some key ->
    (match config.backend with
     ...
     | FileSystem _ -> with_distributed_lock_r ?clock config path key f)
;;
