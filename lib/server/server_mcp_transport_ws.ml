...
let slice_index : (string, (string, unit) Hashtbl.t) Hashtbl.t = Hashtbl.create 8
...
let slice_index_remove_session_locked session_id =
  Hashtbl.iter (fun _slice set -> Hashtbl.remove set session_id) slice_index;
...
let slice_index_size () =
  with_sessions_rw (fun () ->
    Hashtbl.fold (fun _slice set acc -> acc + Hashtbl.length set) slice_index 0)
;;
...
let send_frame_bytes session bytes ~len =
  if session.closed || Httpun_ws.Wsd.is_closed session.wsd
  then (
    session.closed <- true;
    false)
  else (
    try
      Httpun_ws.Wsd.send_bytes session.wsd ~kind:`Text bytes ~off:0 ~len;
    ...
    | exn ->
      Log.Transport.warn
        "WS text send failed for session=%s: %s"
        session.id
...
let websocket_text_payload text = Inference_utils.sanitize_text_utf8 text
...
let bytes_cache : (string * Bytes.t) Atomic.t = Atomic.make ("", Bytes.empty)
...
let dashboard_slice_for_sse_type = function
  | "project_snapshot" | "namespace_truth_snapshot" | "room_truth_snapshot" ->
    Some "namespace"
  | "execution_snapshot" -> Some "execution"
  | "operator_snapshot" | "operator_digest" -> Some "operator"
  | "transport_health_snapshot" -> Some "transport"
  | "keeper_composite_changed" -> Some "composite"
  | _ -> None
;;

let dashboard_session_result session =
  let slices =
    Hashtbl.fold (fun slice () acc -> `String slice :: acc) session.dashboard_slices []
    |> List.sort compare
  in
  `Assoc
...
let set_dashboard_snapshot_provider provider = dashboard_snapshot_provider := provider
...
let verify_dashboard_token ~base_path token =
  ...
  then Ok None
    ...
    | None -> Error "dashboard/hello requires a bearer token"
    | Some raw_token ->
      (match Auth.find_credential_by_token base_path ~token:raw_token with
       ...
       | Ok cred ->
         (match
            Auth.check_permission
              base_path
              ~agent_name:cred.Types.agent_name
...
let dashboard_subscribe ~session_id ?route ~slices () =
  ...
  | None -> Error "WebSocket session not found"
  | Some session ->
    if not session.dashboard_authenticated
    then Error "dashboard/subscribe requires dashboard/hello first"
    else (
      let invalid = List.filter (fun slice -> not (valid_dashboard_slice slice)) slices in
      match invalid with
      | bad :: _ -> Error (Printf.sprintf "unsupported dashboard slice: %s" bad)
      | [] ->
        with_sessions_rw (fun () ->
...
let dashboard_unsubscribe ~session_id ?slices () =
  ...
  | Some session ->
    if not session.dashboard_authenticated
    then Error "dashboard/unsubscribe requires dashboard/hello first"
    else (
      with_sessions_rw (fun () ->
        match slices with
        | None ->
...
let dashboard_ack ~session_id ~seq ?buffered_amount () =
  ...
  | Some session ->
    if not session.dashboard_authenticated
    then Error "dashboard/ack requires dashboard/hello first"
    else (
      if seq > session.dashboard_last_ack_seq then session.dashboard_last_ack_seq <- seq;
      ...
      Ok
        (`Assoc
            ...
            ; "server_last_buffered_amount", `Int session.dashboard_last_buffered_amount
            ]))
;;
...
let parse_cache : (string * parsed_sse_event option) Atomic.t = Atomic.make ("", None)
...
let extract_sse_data_payload_line line =
  if String.starts_with ~prefix:sse_data_prefix line
  then (
...
let extract_sse_data_line sse_event =
  match
    String.split_on_char '\n' sse_event |> List.filter_map extract_sse_data_payload_line
  with
  | [] ->
...
let client_buffer_limit_bytes () =
  ...
  if now -. last_at < env_cache_ttl_s
  then cached
  else (
    let value =
      Env_config_core.get_int ~default:1048576 "MASC_WS_CLIENT_BUFFER_LIMIT_BYTES"
    in
...
let slice_index_enabled () =
  ...
  then cached
    ...
    let value = Env_config_core.get_bool ~default:true "MASC_WS_SLICE_INDEX_ENABLED" in
...
let send_dashboard_or_raw_sse session sse_event =
  if session_is_backpressured session
  then (
    ...
    true)
  else if session.dashboard_authenticated
  then (
    ...
    | None ->
      ...
      let is_slice_mismatch =
        ...
        | _ -> false
      in
      if is_slice_mismatch && slice_index_enabled ()
      then (
  ...
  else send_text_shared_checked ~context:"sse-forward" session sse_event
;;
...
let read_payload_string payload ~len ~on_complete =
  ...
  let complete () =
    ...
    then (
      ...
      let text =
        if !offset = len
        then Bytes.unsafe_to_string buffer
    ...
    then complete ()
      ...
      Httpun_ws.Payload.schedule_read
        ...
        ~on_read:(fun bs ~off ~len:chunk_len ->
          ...
          Bigstringaf.blit_to_bytes bs ~src_off:off buffer ~dst_off:!offset ~len:copy_len;
...
let handle_inbound_text session ~on_message ~is_fin text =
  ...
  | None, true -> if String.length text > 0 then on_message session.id text
  | None, false ->
  ...
  | Some buffer, _ ->
    ...
    then (
      ...
      if String.length message > 0 then on_message session.id message)
;;
...
let cleanup_session session_id =
  let removed =
    with_sessions_rw (fun () ->
      ...
      | Some session ->
        session.closed <- true;
        (try Httpun_ws.Wsd.close session.wsd with
         | Eio.Cancel.Cancelled _ as e -> raise e
         | exn ->
           Log.Server.warn
        ...
        true)
  in
  Transport_metrics.set_ws_sessions (with_sessions_rw (fun () -> Hashtbl.length sessions));
  Sse.unsubscribe_external session_id;
  if removed then Log.Server.info "WebSocket session %s closed" session_id
;;

(** Number of active WebSocket sessions. *)
let session_count () = with_sessions_rw (fun () -> Hashtbl.length sessions)
...
let upgrade_connection ?(on_message = fun _session_id _text -> ()) (reqd : Httpun.Reqd.t)
  : (unit, string) result
  =
      ...
      Httpun_ws.Server_connection.create_websocket (fun wsd ->
        ...
        Sse.subscribe_external
          ...
          ~callback:(fun sse_event ->
            if (not session.closed) && not (send_dashboard_or_raw_sse session sse_event)
        ...
        { Httpun_ws.Websocket_connection.frame =
            (fun ~opcode ~is_fin ~len payload ->
              ...
              | `Connection_close ->
                ...
                Httpun_ws.Payload.close payload
              | `Pong | `Other _ -> Httpun_ws.Payload.close payload)
        ; eof = (fun ?error:_ () -> cleanup_session session_id)
        })
    in
...
let send_to_session_result session_id text =
  let session_opt = with_sessions_rw (fun () -> Hashtbl.find_opt sessions session_id) in
...
let broadcast_ws json_str =
  let snapshot =
    with_sessions_rw (fun () -> Hashtbl.fold (fun _ s acc -> s :: acc) sessions [])
  in
  let failed = ref [] in
  List.iter
    (fun session ->
       if not (send_text_checked ~context:"broadcast" session json_str)
       then failed := session.id :: !failed)
    snapshot;
...
let close_all () =
  let ids =
    with_sessions_rw (fun () -> Hashtbl.fold (fun k _ acc -> k :: acc) sessions [])
  in
