    ...
    (match List.assoc_opt "source_text" fields with
     ...
     | None ->
       (match List.assoc_opt "raw_json" fields with
        ...
        | _ -> Error "expected JSON body with string field source_text"))
...
let add_routes router =
  ...
  |> Http.Router.get "/api/v1/cascade/health" (fun request reqd ->
    with_public_read
      (fun state req reqd ->
         ...
         let json = Dashboard_cascade.health_json ~base_path () in
         Http.Response.json ~compress:true ~request:req (Yojson.Safe.to_string json) reqd)
      request
      reqd)
  |> Http.Router.get "/api/v1/cascade/client_capacity" (fun request reqd ->
    with_public_read
      ...
      request
      reqd)
  |> Http.Router.get "/api/v1/cascade/client_capacity/history" (fun request reqd ->
    with_public_read
      (fun _state req reqd ->
         let limit = clamp_history_limit (int_query_param req "limit" ~default:100) in
         let kind = parse_history_kind (query_param req "kind") in
         let since_ts = parse_history_since req in
         let json =
           Dashboard_cascade.client_capacity_history_json ~limit ?kind ?since_ts ()
         in
         Http.Response.json ~compress:true ~request:req (Yojson.Safe.to_string json) reqd)
      request
      reqd)
  |> Http.Router.get "/api/v1/cascade/strategy_trace" (fun request reqd ->
    with_public_read
      (fun _state req reqd ->
         ...
         let json = Dashboard_cascade.strategy_trace_json ~limit ?cascade () in
         Http.Response.json ~compress:true ~request:req (Yojson.Safe.to_string json) reqd)
      request
      reqd)
