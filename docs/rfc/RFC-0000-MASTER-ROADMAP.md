...
- **소유 타입:** named topology arm (`simple/refine/conditional/judge_of_judges/staged_judge_of_judges`, exhaustive dispatch, catch-all 금지). `panel_group list` (RFC-0277, 각 group이 system_prompt/web_tools/timeout_s 소유). `judge_pools` (RFC-0298, capability-filter 또는 explicit list). `Fusion_completed` wake variant (run_id/ok/resolved_answer/board_post_id). typed comment-preserving settings editor (RFC-0306).
- **경계 계약:** root switch fork + immediate delivery contract. escalation은 LLM typed decision(`Insufficient`) 읽기(heuristic 아님). gate fail-closed(disabled/unknown-preset/nested → Deny). model-ID 하드코딩 금지(`Config_dir_resolver` SSOT). registry append-only JSONL; startup active state는 provenance를 보존한 recovery inventory로 노출하고 임의 상태변환하지 않는다. JoJ: 기존 single judge가 meta-judge 재사용, `judges:judge_spec list`(default []), N first-round judge는 model/lens로 **차이** 필수. `<2 judges` → runtime Error(simple-fallback 아님). heterogeneous: flat `panel=[...]`는 length-1 group, grouped 문법과 동시 사용은 명시 거부. strict reject: `Empty_panels`/`Conflicting_panel_grammar`/`Duplicate_panel_model`/`Unexpected_field`. 비용·토큰·호출 수는 관측만 하며 실행 cap 없음. `ok=false`는 explicit failure path. Board post는 completion-only.
...
- **현재 상태:** topology dispatch/validation·heterogeneous panel desugar는 구현되어 exhaustive match로 컴파일 강제됨(가장 많은 arm이 배선된 satellite). 단 실행 관점 성숙도는 아직 낮다: **[LANDED]** staged JoJ 만족 preset은 이제 존재 — `council`(`config/runtime.toml:1149`, judge 6개 = staged_judge_group_size 3 × 2 group)이 옛 "0 preset dead option"을 해소(runtime.toml:1132-1135 주석이 명시). `Fusion_completed` wake와 현재 프로세스의 typed `continuation_channel` route는 배선됨(`fusion_tool.ml`→`fusion_wake_route.ml`→`fusion_sink.ml`의 `enqueue_durable_result`, RFC-0266). 시작 receipt JSONL append가 실패하면 worker를 띄우지 않는 fail-closed admission이다. completion append 실패도 실제 run 결과와 분리된 typed `Persistence_failed` receipt로 tool/waiting inventory/dashboard에 노출한다. restart replay는 미완료 `Running`을 삭제하지 않고 typed `Recovery_required Worker_process_restarted` inventory로 보존한다. 단 실제 재실행에는 아직 durable canonical request/channel이 없다. dashboard HTTP fusion path의 topology `'simple'` 리터럴과 unrouted direct 호출도 잔여다. **live proof는 failure-only(panels_unavailable)** — 성공 quorum row는 아직 관측되지 않음.
- **open goals:** Goal 5(Model/Agent/Keeper/Tool[]/Heterogeneous[] typed receipt 조립; ~~≥6-judge staged preset 추가~~ **[LANDED: council]**; durable fusion request/receipt/continuation channel + restart join/re-execution; dashboard direct path typed topology/channel), RFC-0266 async wake **[Phase 1 wake + same-process continuation + recovery inventory LANDED]** + exact restart execution, RFC-0306 typed editor.
