...
(** JSON-RPC payloads are the only durable events safe for MCP workspace_session
...
let jsonrpc_message_for_workspace_session = function
  | `Assoc fields ->
...
let event_string_jsonrpc_message_for_workspace_session event =
  match event_data_payload event with
  | None -> false
  | Some data ->
    (try jsonrpc_message_for_workspace_session (Yojson.Safe.from_string data) with
