...
| `Checkpoint.t` | 상태 스냅샷의 원자적 저장/복원 | `perpetual_loop.ml` / `perpetual_oas.ml`에서 직접 OAS Checkpoint 생성 |
| `Event_bus.t` | 에이전트 간 이벤트 발행/구독 | `oas_events.ml`을 통해 broadcast, heartbeat, board 이벤트 전달 |

OAS는 `dune-project`에 `agent_sdk >= 0.88.0`으로 명시된 의존성이며, CI/runtime은 `scripts/oas-agent-sdk-pin.sh`에 기록된 upstream `main` 커밋(`cda9f343891a8b408b6566fbe340f0247340ddc5`)으로 고정한다. 현재 필요한 `LLM_ENDPOINTS` round-robin 기능이 최신 태그 `v0.88.0`보다 한 커밋 앞에 있으므로, 버전 floor는 `0.88.0`으로 유지하되 pin은 `main` SHA를 따른다. `jeong-sik/oas` 저장소는 GitHub release 최신값이 stale일 수 있으므로, 최신성 검증은 release API 대신 upstream head 조회(`git ls-remote ... refs/heads/main`)를 기준으로 수행한다.
...
| 어댑터 | 역할 |
|--------|------|
| ~~`oas_checkpoint_bridge.ml`~~ | 삭제됨 — checkpoint 변환 로직이 `perpetual_loop.ml` / `perpetual_oas.ml`에 인라인 |
