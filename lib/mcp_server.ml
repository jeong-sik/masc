...
let svg_icon_data_uri ~bg ~fg ~label =
  let text = if String.length label <= 2 then label else String.sub label 0 2 in
  let svg =
    Printf.sprintf
      "<svg xmlns='http://www.w3.org/2000/svg' width='64' height='64' viewBox='0 0 64 \
       64'><rect width='64' height='64' rx='14' fill='%s'/><text x='32' y='38' \
...
let icons_for_mime mime_type =
  ...
  | "text/markdown" | "text/plain; charset=utf-8" | "text/plain" -> [ text_icon ]
  | _ -> [ doc_icon ]
;;
...
let server_info =
  `Assoc
    ...
    ; ( "description"
      , `String
          "Multi-agent MCP server exposing MASC room coordination, tools, prompts, and \
           resources." )
...
let resources : mcp_resource list =
  ...
  ; (* Agent Being Protocol - Institution Memory *)
    make_resource
      ...
      ~description:
        "Institutional knowledge: mission, values, procedural memory, succession policy"
      ~mime_type:"text/markdown"
      ()
  ; make_resource
...
let parse_masc_resource_uri uri_str =
  ...
  | Some "masc" ->
    ...
    in
    let path_segments =
      Uri.path uri |> String.split_on_char '/' |> List.filter (fun s -> s <> "")
    in
...
let task_fsm_transitions : (string * string list * string * string option) list =
  ...
  ; (* Action names match [Types.task_action_to_string] (SSOT):
    ...
    , Some "MASC_VERIFICATION_FSM_ENABLED + verifier-FSM only" )
  ; ( "approve"
    , [ "awaiting_verification" ]
    , "done"
    , Some "MASC_VERIFICATION_FSM_ENABLED + verifier != assignee" )
  ; ( "reject"
    , [ "awaiting_verification" ]
    , "in_progress"
    , Some "MASC_VERIFICATION_FSM_ENABLED + verifier != assignee" )
...
let schema_json =
  ...
  `Assoc
    [ ( "task_statuses"
      , `List (List.map (fun s -> `String s) Types.valid_task_status_strings) )
    ...
    ; ( "cas"
      , `Assoc
          [ "field", `String "backlog.version"; "parameter", `String "expected_version" ]
      )
    ]
;;
...
let schema_markdown =
  String.concat
    ...
    ; "- submit_for_verification: claimed/in_progress(by you) -> awaiting_verification \
       (MASC_VERIFICATION_FSM_ENABLED)"
    ; "- approve: awaiting_verification -> done (verifier != assignee, \
       MASC_VERIFICATION_FSM_ENABLED)"
    ; "- reject: awaiting_verification -> in_progress (verifier != assignee, \
       MASC_VERIFICATION_FSM_ENABLED)"
    ; ""
...
type server_state =
  ...
  ; on_sse_broadcast : (Yojson.Safe.t -> unit) option Atomic.t
    (* SSE push callback, Atomic for cross-fiber visibility *)
...
let create_state ~base_path =
  ...
  Tool_board.set_agent_lookup (fun name ->
    try Coord.is_agent_joined state.room_config ~agent_name:name with
...
let set_sse_callback state callback = Atomic.set state.on_sse_broadcast (Some callback)
