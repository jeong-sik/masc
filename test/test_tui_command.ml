...
let test_pane_commands_parse_by_word () =
  check (list string) "help, keeper, interrupt and thinking"
    ...
    (List.map
       (fun text -> describe (Command.parse text))
       [ "/help"; "/keeper mercury"; "/keeper   "; "/interrupt"; "/thinking" ])
...
let test_keeper_names_resolve_by_unique_prefix () =
  let names = [ "mercury"; "merc"; "atlas"; "vega" ] in
  ...
  check (list string)
    ...
    [ "found:merc" (* exact, though it prefixes mercury *)
    ; "found:atlas"
    ; "found:vega" (* unique prefix *)
    ; "ambiguous:mercury,merc" (* mer- prefixes two *)
    ; "unknown"
    ]
    (List.map
       (fun typed -> describe_match (Command.resolve_keeper_name ~names typed))
       [ "merc"; "atlas"; "ve"; "mer"; "zzz" ])
