...
let score_messages (msgs : Llm_types.message list) : (int * float) list =
  let n = List.length msgs in
  List.mapi (fun i (m : Llm_types.message) ->
    let recency = if n <= 1 then 1.0
      else let t = float_of_int i /. float_of_int (n - 1) in
           t *. t
    in
    let role_w = match m.role with
      | Llm_types.System -> 1.0
      | Llm_types.Tool -> 0.7
      | Llm_types.User -> 0.6
      | Llm_types.Assistant -> 0.4
    in
    let msg_text = Llm_types.text_of_message m in
