  ...
  | `Assoc entry_fields ->
    (match List.assoc_opt "receipt" entry_fields with
     | Some (`Assoc receipt_fields) ->
       (match
          ( List.assoc_opt "settlement" receipt_fields
          , List.assoc_opt "lease_id" receipt_fields
          , List.assoc_opt "transition_id" receipt_fields
          , List.assoc_opt "event_id" receipt_fields )
        with
        | ( Some (`Assoc settlement_fields)
          , Some (`String lease_id)
          , Some (`String transition_id)
          ...
          (match List.assoc_opt "kind" settlement_fields with
           | Some (`String kind)
             ...
             let legacy_transition_id =
               lease_id ^ ":" ^ legacy_source_terminal_settlement_kind
             in
             let canonical_transition_id = lease_id ^ ":ack_source_terminal" in
             if
               String.equal transition_id legacy_transition_id
               && String.equal event_id (event_id_of_transition legacy_transition_id)
             then
               ...
               let receipt_fields =
                 receipt_fields
                 |> replace "settlement" (`Assoc settlement_fields)
                 |> replace "transition_id" (`String canonical_transition_id)
                 |> replace "event_id" (`String (event_id_of_transition canonical_transition_id))
               in
