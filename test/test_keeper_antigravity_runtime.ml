                ...
                (fun () ->
                  ...
                  (match run () with
                   | Error (Agent_core.Error.Internal message) ->
                     (* The provider-attempt effect fence wraps the failure:
                        the antigravity adapter cannot prove whether an effect
                        was attempted ([observation_unavailable]), so same-turn
                        retry is refused and the envelope keeps the provider
                        diagnostic. *)
                     check bool
