...
OAS Pin Snapshot: dependency floor `0.124.0`, runtime pin `main@v0.124.1`, declared base version `v0.124.1`
...
`masc-mcp` keeps the runtime pin ratcheted against upstream `main`, while the dependency floor tracks the pinned SDK declaration in `agent_sdk.opam`. Generated snapshot: runtime pin `main@v0.124.1`, declared base version `v0.124.1`, dependency floor `0.124.0`.
