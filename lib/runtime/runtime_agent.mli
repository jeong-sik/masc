...
val worker_lifecycle_classification_of_result :
  (run_result, Masc_agent_core.Error.sdk_error) result -> worker_lifecycle_classification
...
module For_testing : sig
  ...
  val required_modalities_of_content_blocks :
    Masc_agent_core.Types.content_block list -> string list
  ...
  val required_modalities_of_messages :
    Masc_agent_core.Types.message list -> string list
  ...
  val validate_content_blocks_for_run_against_capabilities :
    ...
    initial_messages:Masc_agent_core.Types.message list ->
    goal_blocks:Masc_agent_core.Types.content_block list ->
    (unit, Masc_agent_core.Error.sdk_error) result
...
(** Builds an [Masc_agent_core.Agent.t] from a {!config} ready for a
...
(** Runs an OAS agent against [goal].  When
    ...
    as [Masc_agent_core.Error.sdk_error]. *)
...
(** Variant of {!run} that projects the supplied MASC schemas into exact inline
    [Masc_agent_core.Tool.t] values through [dispatch]. *)
...
(** [set_oas_tool_of_masc_hook f] registers a function to project MASC tool schemas
    into Masc_agent_core.Tool.t. Used to decouple the [Tool_bridge] module. *)
