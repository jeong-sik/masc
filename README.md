...
[![agent_sdk](https://img.shields.io/badge/agent__sdk-%3E%3D%200.211.0-blue.svg)](https://github.com/jeong-sik/oas)
...
| **Sandbox (Docker)** | 🟡 | Docker-profile shell execution is fail-closed and containerized; an explicitly selected `local` profile remains host execution | keeper TOML `sandbox_profile` + live `security_boundary` |
...
- **Sandbox** — Actually invokes `docker run --rm` with cap-drop / no-new-privileges / read-only rootfs and limits concurrent execution slots. Network is controlled by the keeper's `network_mode` (`host` explicitly shares the host namespace; `none` leaves loopback only). Docker is the fail-safe default and a missing image/preflight is an explicit failure; it never falls back to host execution. *Limit*: an explicitly selected `local` profile is still a host process. It gets a BasePath-owned managed HOME/XDG tree and default-deny environment, but no filesystem namespace isolation.
...
| 8 | **Sandbox** | Docker image/preflight failures are fail-closed; keep `sandbox_profile=local` as an explicit, visibly non-isolated operator choice only. | ✅ stabilization |
