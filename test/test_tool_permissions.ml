...
let test_admin_tools_classified () =
  Alcotest.(check bool) "execute_spawn is admin"
    true (Tool_permissions.requires_admin "masc_gardener_execute_spawn");
  Alcotest.(check bool) "execute_retire is admin"
    true (Tool_permissions.requires_admin "masc_gardener_execute_retire");
  Alcotest.(check bool) "reset_circuit is admin"
    true (Tool_permissions.requires_admin "masc_gardener_reset_circuit");
  Alcotest.(check bool) "status is not admin"
    false (Tool_permissions.requires_admin "masc_status");
  Alcotest.(check bool) "gardener_health is not admin"
    false (Tool_permissions.requires_admin "masc_gardener_health")
...
let test_check_denies_admin_without_cap () =
  ...
  Alcotest.(check bool) "admin tool denied"
    ...
    (Result.is_error (Tool_permissions.check
       ~agent_name:"normal_agent" ~tool_name:"masc_gardener_execute_spawn"))
...
let test_check_allows_admin_with_cap () =
  ...
  Alcotest.(check bool) "admin tool allowed with cap"
    true
    (Result.is_ok (Tool_permissions.check
       ~agent_name:"admin_agent" ~tool_name:"masc_gardener_execute_spawn"))
...
let test_hook_blocks_admin_no_identity () =
  ...
  Tool_dispatch.register
    ~tool_name:"masc_gardener_execute_spawn"
  ...
  match Tool_dispatch.dispatch_structured
          ~name:"masc_gardener_execute_spawn" ~args:`Null with
  | Some r ->
...
let test_hook_blocks_admin_no_cap () =
  setup ();
  Tool_dispatch.register
    ~tool_name:"masc_gardener_execute_retire"
    ~handler:(fun ~name:_ ~args:_ -> Some (true, "should not reach"));
  Tool_permissions.set_capability_checker (fun _agent _cap -> false);
  Tool_permissions.install ~get_agent_name:(fun () -> Some "normal");
  match Tool_dispatch.dispatch_structured
          ~name:"masc_gardener_execute_retire" ~args:`Null with
  | Some r ->
    Alcotest.(check bool) "blocked" false r.success
  | None -> Alcotest.fail "expected Some from short-circuit"
