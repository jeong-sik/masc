...
- **persisted default base path**: `masc setup`/`masc init` 성공 시, 그리고 `masc start --base-path X`가 성공적으로 부팅할 때마다, 해소된 절대경로를 사용자 전역 위치에 기록한다. 후보 위치: `$XDG_CONFIG_HOME/masc/default-base-path`(기본 `~/.config/masc/default-base-path`). `~/.masc` 안은 base path 자체와 혼동되므로 금지.
