...
let test_session_to_swarm_config_health_contract () =
  ...
  (* Collaboration module removed from OAS (oas#692).
     collaboration_context is now None — MASC no longer projects to OAS types. *)
  Alcotest.(check bool) "collaboration_context is None"
    true (Option.is_none swarm_cfg.collaboration_context);
