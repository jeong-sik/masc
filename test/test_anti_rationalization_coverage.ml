...
let assert_reject (r : Anti_rationalization.review_result) =
  ...
  | Anti_rationalization.Approve -> failwith "expected Reject but got Approve"
;;
...
let assert_reject_contains (r : Anti_rationalization.review_result) substring =
  ...
  | Anti_rationalization.Reject reason ->
    ...
    if
      not
        (let len_r = String.length lower_reason in
         ...
         then false
           ...
           let rec scan i =
             if i > len_r - len_s
             then false
             else if String.sub lower_reason i len_s = lower_sub
             then true
             else scan (i + 1)
           in
           scan 0))
    then
      failwith
        (Printf.sprintf "Reject reason '%s' does not contain '%s'" reason substring)
  | Anti_rationalization.Approve ->
...
let make_request
      ...
      notes
  =
  { Anti_rationalization.task_title = title
...
let () =
  test "empty_notes_rejected" (fun () ->
    assert_reject (Anti_rationalization.review (make_request "")))
;;

let () =
  test "whitespace_only_rejected" (fun () ->
    assert_reject (Anti_rationalization.review (make_request "   ")))
;;

let () =
  test "too_short_rejected" (fun () ->
    assert_reject_contains (Anti_rationalization.review (make_request "done")) "too short")
;;

let () =
  test "minimum_length_boundary" (fun () ->
...
let () =
  test "pre_existing_pattern" (fun () ->
    assert_reject_contains
      (Anti_rationalization.review
         (make_request "This was a pre-existing issue that I found"))
      "pre-existing")
;;

let () =
  test "out_of_scope_pattern" (fun () ->
    assert_reject_contains
      (Anti_rationalization.review
         (make_request "After investigation, this is out of scope for the current sprint"))
      "out of scope")
;;

let () =
  test "will_do_later_pattern" (fun () ->
    assert_reject_contains
      (Anti_rationalization.review
         (make_request "Partially completed. Will do later the remaining items"))
      "will do later")
;;

let () =
  test "will_fix_later_pattern" (fun () ->
    assert_reject_contains
      (Anti_rationalization.review
         (make_request "Found the issue but will fix later when we have time"))
      "will fix later")
;;

let () =
  test "follow_up_pattern" (fun () ->
    assert_reject_contains
      (Anti_rationalization.review
         (make_request "Created a follow-up ticket for the remaining work"))
      "follow-up")
;;

let () =
  test "works_on_my_end_pattern" (fun () ->
    assert_reject_contains
      (Anti_rationalization.review
         (make_request "Tested locally and works on my end without issues"))
      "works on my end")
;;

let () =
  test "not_reproducible_pattern" (fun () ->
    assert_reject_contains
      (Anti_rationalization.review
         (make_request "Investigated but the bug is not reproducible in staging"))
      "not reproducible")
;;

let () =
  test "beyond_scope_pattern" (fun () ->
    assert_reject_contains
      (Anti_rationalization.review
         (make_request "This change is beyond the scope of the current task"))
      "beyond the scope")
;;

let () =
  test "case_insensitive_pattern" (fun () ->
    assert_reject_contains
      (Anti_rationalization.review
         (make_request "This was a PRE-EXISTING issue in the codebase"))
      "pre-existing")
;;
...
let () =
  test "normal_notes_approve_when_llm_unavailable" (fun () ->
    (* In test env, LLM is unavailable. Gate 1+2 pass, Gate 3 defaults to Approve *)
    assert_approve
      (Anti_rationalization.review
         (make_request
            "Fixed the login bug by updating the OAuth callback URL in the auth service. \
             Added unit test to verify the fix.")))
;;

let () =
  test "substantive_notes_no_pattern_match" (fun () ->
    assert_approve
      (Anti_rationalization.review
         (make_request
            "Implemented the search feature with pagination support. 15 new tests added, \
             all passing.")))
;;
...
let () =
  test "review_result_evaluator_cascade_default" (fun () ->
    let r = Anti_rationalization.review (make_request "") in
    assert (r.evaluator_cascade = "cross_verifier"))
;;

let () =
  test "review_result_custom_evaluator_cascade" (fun () ->
    let r =
      Anti_rationalization.review ~evaluator_cascade:"cross_verifier" (make_request "")
    in
    assert (r.evaluator_cascade = "cross_verifier"))
;;
...
let with_fsm_disabled f =
  ...
  let restore () =
    match prev with
    | None ->
      (try Unix.putenv "MASC_VERIFICATION_FSM_ENABLED" "" with
       | _ -> ())
...
let () =
  test "contract_all_met" (fun () ->
    let r =
      Anti_rationalization.review
        ~completion_contract:[ "test"; "fix" ]
        (make_request "Applied fix to the login flow and added test coverage.")
    in
...
let () =
  test "contract_unmet_rejects_legacy" (fun () ->
    with_fsm_disabled (fun () ->
      let r =
        Anti_rationalization.review
          ~completion_contract:[ "test coverage"; "migration" ]
          (make_request "Applied fix to the login flow.")
      in
...
let () =
  test "contract_unmet_lists_items_legacy" (fun () ->
    with_fsm_disabled (fun () ->
      let r =
        Anti_rationalization.review
          ~completion_contract:[ "test"; "migration"; "rollback" ]
          (make_request "Applied fix and added test to verify.")
      in
      match r.verdict with
      | Anti_rationalization.Reject reason ->
        ...
        let has sub =
          ...
          then false
            ...
            let rec s i =
              if i > rlen - slen
              then false
              else if String.sub reason i slen = sub
              then true
              else s (i + 1)
            in
            s 0)
        ...
        assert (has "rollback")
      | Anti_rationalization.Approve -> failwith "expected Reject for unmet contract"))
;;

(* New: verify that FSM-enabled path bypasses the substring gate. *)
let () =
  test "contract_substring_bypassed_when_fsm_enabled" (fun () ->
...
let () =
  test "contract_empty_no_effect" (fun () ->
    let r =
      Anti_rationalization.review
        ~completion_contract:[]
        (make_request "Applied fix to the login flow and added test coverage.")
...
let () =
  test "contract_none_no_effect" (fun () ->
    let r =
      Anti_rationalization.review
        (make_request "Applied fix to the login flow and added test coverage.")
    in
...
let () =
  test "check_contract_direct" (fun () ->
    let unmet =
      Anti_rationalization.check_contract
        ~notes:"Fixed auth bug, added unit test, ran migration"
        ~contract:[ "test"; "migration"; "deployment" ]
    in
...
let () =
  test "parse_verdict_approve" (fun () ->
...
let () =
  test "parse_verdict_approve_with_trailing" (fun () ->
...
let () =
  test "parse_verdict_reject_with_reason" (fun () ->
...
let () =
  test "parse_verdict_reject_bare" (fun () ->
...
let () =
  test "parse_verdict_reject_colon_only" (fun () ->
    match Anti_rationalization.parse_verdict "REJECT:" with
    | Ok (Anti_rationalization.Reject _) -> ()
    | Ok _ -> failwith "expected Reject"
    | Error e -> failwith (Printf.sprintf "unexpected error: %s" e))
...
let () =
  test "parse_verdict_unrecognized_returns_error" (fun () ->
...
let () =
  test "parse_verdict_approved_word_returns_error" (fun () ->
...
let () =
  test "parse_verdict_rejected_word_returns_error" (fun () ->
...
let () =
  test "parse_verdict_empty_returns_error" (fun () ->
...
let () =
  test "find_excuse_pattern_none_for_clean_notes" (fun () ->
    match
      Anti_rationalization.find_excuse_pattern "Fixed the auth bug and added tests"
    with
    | None -> ()
...
let () =
  test "find_excuse_pattern_some_for_excuse" (fun () ->
