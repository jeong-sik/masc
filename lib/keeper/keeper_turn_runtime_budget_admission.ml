(** Retry admission denial reasons + attempt_kind enum for keeper
    runtime budget reasoner.

    RFC-0158 Phase A: the [retry_admission_denial] type and its JSON codec
    now live in [Keeper_meta_contract] (runtime layer) to avoid a
    dependency cycle — [Keeper_meta_contract] references this type in the
    ...
    transparent alias so [Keeper_turn_runtime_budget] and its callers are
    unchanged. *)
