...
let test_integration () = group "Integration" (fun () ->
  ...
  let provider_config = Masc_mcp.Llm_provider_dispatch.to_oas_provider Llm_types.claude_opus in
  assert_true "oas_adapter:claude_mapped" (Option.is_some provider_config);
  let provider_config_custom = Masc_mcp.Llm_provider_dispatch.to_oas_provider
  ...
  (match Masc_mcp.Llm_provider_dispatch.to_oas_message test_msg with
   | None -> assert_true "oas_adapter:msg_roundtrip" false
   | Some oas_m ->
     let back_m = Masc_mcp.Llm_provider_dispatch.of_oas_message oas_m in
  ...
  let oas_u = Masc_mcp.Llm_provider_dispatch.to_oas_usage test_usage in
  let back_u = Masc_mcp.Llm_provider_dispatch.of_oas_usage oas_u in
