...
let provider_content_messages
  ...
  | Some (projected_prefix, projection_suffix)
    ...
    let rec remove_prompt_context seen rev_retained = function
      ...
      | (message : Masc_agent_core.Types.message) :: rest ->
        (match
         Masc_agent_core.Types.Extra_system_context_provenance.classify
             message.metadata
         with
         | Masc_agent_core.Types.Extra_system_context_provenance.Absent ->
         ...
         | Masc_agent_core.Types.Extra_system_context_provenance.Duplicate -> None)
    in
    Option.map
...
let metric_of_block
    ~role:(_ : Masc_agent_core.Types.role)
    (block : Masc_agent_core.Types.content_block) : prompt_segment_metrics =
  let bytes =
    ...
    | None -> (
        ...
        | None -> (
          ...
          | Masc_agent_core.Types.ReasoningDetails { reasoning_content; details } ->
              ...
              |> String.length
          | Masc_agent_core.Types.RedactedThinking text ->
              String.length (Inference_utils.sanitize_text_utf8 text)
          | Masc_agent_core.Types.Image { data; _ }
          | Masc_agent_core.Types.Document { data; _ }
          | Masc_agent_core.Types.Audio { data; _ } -> String.length data
...
let input_component_of_block
    ...
    (block : Masc_agent_core.Types.content_block) : Turn_record.input_component_id =
  ...
  then Turn_record.Message_tool_use
    ...
    | None ->
      (match block with
  | Masc_agent_core.Types.ToolResult _ -> Turn_record.Message_tool_result
  | Masc_agent_core.Types.ToolUse _ ->
      invalid_arg
        "keeper_agent_prompt_metrics: OAS canonical tool-call projection unavailable"
  | Masc_agent_core.Types.Text _ -> (
      match role with
      | Masc_agent_core.Types.User -> Turn_record.Message_user
      | Masc_agent_core.Types.System -> Turn_record.Message_system
      | Masc_agent_core.Types.Assistant -> Turn_record.Message_assistant_text
      | Masc_agent_core.Types.Tool -> Turn_record.Message_tool_result)
  | Masc_agent_core.Types.Thinking _ | Masc_agent_core.Types.ReasoningDetails _ ->
      Turn_record.Message_thinking
  | Masc_agent_core.Types.RedactedThinking _ -> Turn_record.Message_redacted_thinking
  | Masc_agent_core.Types.Image _ -> Turn_record.Message_image
  | Masc_agent_core.Types.Document _ -> Turn_record.Message_document
  | Masc_agent_core.Types.Audio _ -> Turn_record.Message_audio)
  ...
  let tool_schema_bytes =
    List.fold_left
      (fun total tool ->
        total
        + String.length
            (Yojson.Safe.to_string (Masc_agent_core.Tool.schema_to_json tool)))
      0 tools
  in
