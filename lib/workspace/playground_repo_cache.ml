...
let git_meta repo_path args =
  Masc_exec.Exec_gate.run_argv_with_status ~actor:`Workspace_git ~raw_source:("git -C " ^ repo_path ^ " " ^ String.concat " " args) ~summary:"git metadata query" ~timeout_sec:git_meta_timeout_sec
