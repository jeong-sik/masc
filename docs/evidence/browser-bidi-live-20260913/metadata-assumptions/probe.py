...
DIST=pathlib.Path('/tmp/browser-navigation-native-fd441');REPO=pathlib.Path('/private/tmp/masc-browser-navigation-lifecycle');REV='fd441e234f58f34e2889d8fde0fc20d3ea2be4d1'
...
def dispatch(verb,args):
 with lock:
  if verb=='tabs.list':
   ...
   return [dict(id=i,index=i-1,windowId=1,active=i==1,**{k:v for k,v in state(c).items() if k in ['url','title']}) for i,c in mapping.items() if c in existing]
...
try:
 ...
 report['mapping']=mapping;report['before']={i:state(c) for i,c in mapping.items()}
 ...
 while True:
  ...
  time.sleep(.2)
 report['health_build']=health['build'];token=json.loads((ROOT/'login-private.json').read_text())['bearer_token'];client=str(uuid.uuid4());lane_token=(ROOT/'.masc/browser-lane/token').read_text().strip();headers={'x-lane':'live','x-lane-token':lane_token,'x-browser-client-id':client,'x-browser-name':'firefox','x-browser-version':'155.0.1','x-browser-engine-version':'155.0.1'};report['client_id']=client
 def poll():
 ...
 text=pathlib.Path('/tmp/capture-real-browser-gestures.py').read_text();text=text.replace("send(b'a', b'Gesture Lab')","send(b'l', b'Gesture Lab')")
...
'''+text[finish:]
 ...
 if ws:
  ...
  except Exception as e:report['session_cleanup_error']=repr(e)
  ws.close()
...
print(json.dumps({'out':str(OUT),'result':report['result'],'error':report.get('error')}))
