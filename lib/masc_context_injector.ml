...
let make ~(config : config) () : Masc_agent_core.Hooks.context_injector =
...
let get_string ctx key =
  match Masc_agent_core.Context.get ctx key with
...
let render_temporal_summary ?now (ctx : Masc_agent_core.Context.t) : string option =
  ...
  match Masc_agent_core.Context.get ctx key_wall_time with
  | None -> None
  | Some _ ->
