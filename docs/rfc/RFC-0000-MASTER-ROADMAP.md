...
<a name="32-goal"></a>
### 3.2 Goal — 엔티티 KEEP / legacy FSM 기계 RETIRE (RFC-0352 Path B)

- **판정(2026-07-21 오너 결정, RFC-0352):** Goal은 제품 스펙 "Goal/Task 약한 결합"의 1급 엔티티다. `goal_store.ml` SSOT, MCP tool 3종(`masc_goal_list/upsert/transition`), task↔goal linkage(RFC-0267), keeper meta `active_goal_ids`는 **KEEP**. 반대로 legacy goal-FSM 기계 — stagnation counter, goal-loop OODA scheduler/status 표면, `goal_store.ml`의 legacy-status 호환 decode — 는 **RETIRED**이며 절제한다.
...
- **절제 범위(RFC-0352가 SSOT):** goal-loop OODA scheduler/status(server refresh loop, `/api/v1/dashboard/goal-loop/*`, dashboard goal-loop 표면), stagnation counter, legacy-status 호환 decode. 스크립트 계층 10+8은 이미 삭제됨(#25477).
