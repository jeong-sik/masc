...
let invalidate_path config_path =
  let keep_snapshot = function
    | Some (snapshot : snapshot) when String.equal snapshot.source_path config_path ->
      None
    | other -> other
  in
  let keep_rejection = function
    | Some (rejection : rejection) when String.equal rejection.source_path config_path ->
      None
    | other -> other
  in
  with_cache_lock (fun () ->
...
let install_snapshot_for_tests ~source_path ~profile_names =
  ...
  in
  let snapshot = { source_path; mtime; validated_at = Unix.gettimeofday (); profiles } in
  with_cache_lock (fun () ->
  ...
  `Assoc
    ...
    ; ( "status"
      , match probe.status with
        ...
        | Probe_error message -> `String "error" )
    ; ( "error"
      , match probe.status with
        | Probe_ok -> `Null
...
let snapshot_to_yojson (snapshot : snapshot) =
  `Assoc
    ...
    ; "profiles", `List (List.map profile_snapshot_to_yojson snapshot.profiles)
    ]
;;
...
let rejection_to_yojson (rejection : rejection) =
  `Assoc
    ...
    ; "profiles", `List (List.map profile_rejection_to_yojson rejection.profiles)
    ]
;;
...
let expand_weighted_entries (entries : Cascade_config_loader.weighted_entry list)
...
let eio_caps ?sw ?net ?clock () =
  ...
  | _ -> Error "catalog validation requires Eio switch/net/clock capabilities"
;;
...
let validate_strategy ~config_path ~name =
  let cfg = Cascade_config_loader.resolve_strategy_config ~config_path ~name in
  let strategy_errors =
    match cfg.kind with
    | None -> []
    | Some raw_kind ->
      (match Cascade_strategy.parse_kind raw_kind with
       | Error msg -> [ Printf.sprintf "unknown strategy %S: %s" raw_kind msg ]
       | Ok Cascade_strategy.Priority_tier ->
         (match cfg.tiers with
          | None -> [ "priority_tier requires a non-empty <name>_tiers configuration" ]
          | Some raw_tiers ->
            (match
               Cascade_config.normalize_priority_tiers ~config_path ~name raw_tiers
             with
             | Ok _ -> []
             | Error msg ->
               [ Printf.sprintf "priority_tier normalization failed: %s" msg ]))
       | Ok _ -> [])
  in
  if strategy_errors <> []
  then Error strategy_errors
  else
    Ok
...
let active_source_state ~config_path = Cascade_toml_materializer.source_state ~config_path

let validate_profile_static ~config_path name : (profile_build, profile_rejection) result =
  let weighted_entries = Cascade_config_loader.load_profile_weighted ~config_path ~name in
  if weighted_entries = []
  then
    Error
      { name; errors = [ "profile has no non-empty configured candidates" ]; probes = [] }
  else (
    ...
    | Ok (strategy, ollama_max_concurrent, cli_max_concurrent) ->
      ...
      let candidates, candidate_errors =
        List.fold_left
          (fun (ok_acc, err_acc) (entry : Cascade_config_loader.weighted_entry) ->
             ...
             | Error (Cascade_config.Drop_unregistered_scheme { model; scheme }) ->
               ( ok_acc
               , Printf.sprintf
                   ...
                   scheme
                 :: err_acc )
             | Error (Cascade_config.Drop_unavailable_scheme { model; scheme }) ->
               ( ok_acc
               , Printf.sprintf
                   "candidate %S uses unavailable provider scheme %S (missing credential \
                    or disabled runtime lane)"
                   model
                   scheme
                 :: err_acc )
             | Error (Cascade_config.Drop_invalid_syntax model) ->
               ( ok_acc
               , Printf.sprintf "candidate %S has invalid provider:model syntax" model
                 :: err_acc ))
          ([], [])
          expanded_entries
      in
      ...
      if candidate_errors <> []
      then Error { name; errors = candidate_errors; probes = [] }
      else
        Ok
          { name
          ; weighted_entries
  ...
  let config_path =
    ...
    | None ->
      (match config_path_opt () with
       ...
       | None -> "")
  in
  if String.equal config_path ""
  then
    Keeper_cascade_profile.known_cascades @ [ "governance_judge"; "operator_judge" ]
    |> List.sort_uniq String.compare
    ...
    | Error msg ->
      Error
        (rejection_of_path
           ...
           ~attempted_mtime
           ~checked_at
           ~errors:[ Printf.sprintf "active cascade source could not be loaded: %s" msg ]
           ~profiles:[])
    | Ok json ->
      ...
      let top_errors =
        let base =
          if profiles = []
          then [ "active cascade catalog has no <name>_models profiles" ]
          else []
        in
        if List.mem Keeper_config.default_cascade_name profiles
        then base
        else
          base
          @ [ Printf.sprintf
      ...
      then
               ...
               ; probes =
                   List.map
                     (fun (candidate : candidate_runtime) ->
                        candidate_probe_skipped
                          candidate
                          "runtime provider health is advisory; bootstrap skips live \
                           probe")
                     profile.candidates
               })
            built_profiles
        ...
        then Ok { snapshot; rejected_update = None }
        else (
          let rejection =
            rejection_of_path
              ~config_path:source_path
...
let inspect_active ?sw ?net ?clock () =
  ...
  | None ->
    ...
    (match with_cache_lock (fun () -> !cache.active_snapshot) with
     | Some snapshot ->
       with_cache_lock (fun () ->
         cache := { active_snapshot = Some snapshot; rejected_update = Some rejection });
    ...
    let cached_result =
      with_cache_lock (fun () ->
        ...
        | Some snapshot, Some rejection, Some mtime
          ...
          Some (Ok (Validated_with_rejections { snapshot; rejected_update = rejection }))
     ...
     | None ->
       (match validate_path_result ~config_path with
        ...
        | Ok { snapshot; rejected_update = Some rejection } ->
          with_cache_lock (fun () ->
            cache := { active_snapshot = Some snapshot; rejected_update = Some rejection });
          Ok (Validated_with_rejections { snapshot; rejected_update = rejection })
        | Error rejection ->
          (match with_cache_lock (fun () -> !cache.active_snapshot) with
           | Some snapshot ->
             with_cache_lock (fun () ->
               cache
               := { active_snapshot = Some snapshot; rejected_update = Some rejection });
             Ok (Serving_last_known_good { snapshot; rejected_update = rejection })
           | None ->
             with_cache_lock (fun () ->
               cache := { active_snapshot = None; rejected_update = Some rejection });
             Error rejection)))
;;
...
let require_snapshot ?sw ?net ?clock () =
  ...
  | Error rejection ->
    let detail =
      if rejection.errors = []
      then "active catalog validation failed"
      else String.concat "; " rejection.errors
    in
    Error detail
;;

let normalize_declared_name raw = Keeper_cascade_profile.normalize_declared_name raw
  ...
  | Ok snapshot ->
    (match profile_lookup snapshot.profiles normalized with
     ...
     | None ->
       ...
       Error
         (Printf.sprintf "unknown cascade_name %S (active profiles: %s)" normalized known))
;;
...
let resolve_named_providers
  ...
  | Ok (_snapshot, normalized, profile) ->
    ...
    in
    if providers = []
    then Error (Printf.sprintf "cascade %s resolved to no callable providers" normalized)
    else (
      ...
      let leaked = List.filter (fun m -> not (List.mem m declared)) returned in
      if leaked <> []
      then
        Log.warn
          ~ctx:"CascadeCatalog"
          "resolve_named_providers(%s): %d providers NOT in parsed declared profile \
           (parsed_declared=[%s] returned=[%s] leaked=[%s])"
          normalized
          (List.length leaked)
...
let invalid_profile_errors ?sw ?net ?clock () =
  ...
  | Error rejected_update -> of_rejection rejected_update
;;
