...
(** Import python-pptx and open its bundled empty presentation with the exact
    managed interpreter; convert an owned one-slide presentation to PDF with Impress and
    probe the Poppler tools in the current host environment. Never installs dependencies or reads a submitted document. *)
