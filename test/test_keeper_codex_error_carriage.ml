...
(* A Gate continuation's thread that overflowed is full: a Resume's input is
   the same at every capacity and no fresh start may carry the continuation,
   so the Gate ends whether or not a tool effect came first. *)
let test_gate_resume_overflow_is_session_full () =
  ...
  Alcotest.(check bool)
    ...
    true;
  Alcotest.(check bool)
    "Gate resume without activity is session-full too"
    (recovery ~thread_mode:resume ~gate_continuation:true (overflow false)
     = Masc.Keeper_official_client_session_store.(Vendor_session_full No_activity_observed))
    true;
  Alcotest.(check bool)
    "an ordinary resume without activity keeps the fresh-start shrink retry"
    (recovery ~thread_mode:resume ~gate_continuation:false (overflow false)
  ...
  Alcotest.run
    ...
    [ ( "carriage"
      , [ Alcotest.test_case "transport uncertainty preserves prior effects" `Quick
        ...
        ; Alcotest.test_case
            "Gate resume overflow is session-full"
            `Quick
