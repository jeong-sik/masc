...
3. 원문마다 문장으로 자른다(줄바꿈 → 문장 끝 `. ! ? 다.` `;` ` — `, 백틱 제거, 20자 미만 조각은 다음 조각에 붙임). 문장 경계는 #37079 의 채점기와 같고(`**` 는 남긴다 — 코드 기억의 연산자일 수 있다), 표집 없이 모든 문장을 검사한다. 요청은 64개 질문·96,000바이트씩 나눈다. 그 안에 못 들어가는 문장이나 claim 은 판정할 수 없고, 그 원문은 현재 기억에 남긴다(요청이 크기로 거절되는 것은 예측 가능한 실패라 "열림"으로 흡수하지 않는다).
