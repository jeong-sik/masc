...
(** [runtime_toml] names no shared region: every table it carries is keyed
    by this connection's own provider hash. Shared regions, such as
    [runtime.exact_output_lanes], are written by the save's lane setup, not
    by a fragment. *)
