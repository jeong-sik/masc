...
let render_promoted_payload
  ...
  `Assoc
    [ "ok", `Bool false
    ; "promoted", `Bool true
    ; "background_task_id", `String (Bg_task.task_id_to_string p.task_id)
    ; "cmd", `String cmd
    ; "cwd", `String cwd
    ...
    ; ( "hint"
      , `String
          (Printf.sprintf
             "Command exceeded MASC_BLOCKING_BUDGET_MS=%d. Still running in background; \
              poll with keeper_bash_output or stop with keeper_bash_kill."
             budget_ms) )
    ]
;;
...
let test_promoted_json_shape () =
  ...
  in
  match out with
  | Exec_run.Completed _ -> fail "budget 150ms + sleep 5 should promote"
...
let () =
  run
    "exec_run_json"
    [ "shape", [ test_case "promoted payload pinning" `Slow test_promoted_json_shape ] ]
;;
