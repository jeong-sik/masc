...
function LongText({ text, truncateAt = 200 }: { text: string; truncateAt?: number | null }) {
  ...
  const truncated =
    truncateAt !== null && truncateAt >= 0 && text.length > truncateAt
      ...
      : text
  return html`<div class="text-xs text-text-body whitespace-pre-wrap max-h-35 overflow-y-auto custom-scrollbar border border-card-border bg-card/40 backdrop-blur-sm p-3 rounded mt-1.5 leading-relaxed shadow-inner hover:bg-card/60 transition-colors" tabindex="0" aria-label="설정 내용">${truncated}</div>`
}
  ...
  return html`
    <div class="flex flex-col gap-1.5">
      ...
      ${c.sources.cascade_catalog_source_path ? html`
        <div class="text-3xs font-semibold uppercase tracking-wider text-[var(--text-muted)] mt-2 mb-0.5">캐스케이드 카탈로그 소스</div>
      ...
      ${rd ? html`
        ...
        <div class="py-2 px-3 rounded bg-[var(--white-3)]">
          ...
          <textarea autoComplete="off" aria-label="allowed_paths" class="w-full text-xs font-mono bg-[var(--white-6)] border border-[var(--card-border)] rounded px-2 py-1.5 text-[var(--text-body)] resize-y"
            rows=${3}
      ...
      ${c.hooks ? (() => {
        ...
        return html`
          ...
          ${isFiltering && visibleEntries.length === 0 && allEntries.length > 0
            ? html`<div class="py-4 text-center text-2xs text-[var(--text-dim)]" role="status">필터 결과 없음 (${allEntries.length} slots)</div>`
