...
let reset_for_tests () = Eio.Mutex.use_rw ~protect:true mu (fun () -> Hashtbl.clear state)
...
let gate_reason_to_string = function
  | Attempts_exhausted { attempts; max_attempts; _ } ->
    Printf.sprintf "attempts_exhausted attempts=%d max_attempts=%d" attempts max_attempts
...
let classify_and_update_start ~now ~(keeper : string) ~(turn_id : int) : start_outcome =
  match Hashtbl.find_opt state keeper with
  | None ->
    Hashtbl.replace state keeper { turn_id; attempts = 1; first_started_at = now };
    Fresh
  ...
  | Some prev when prev.turn_id = turn_id ->
    ...
    Reattempt
      { previous_attempts = prev.attempts; first_started_at = prev.first_started_at }
  ...
  let outcome =
    Eio.Mutex.use_rw ~protect:true mu (fun () ->
      ...
      | Some prev when prev.turn_id = turn_id ->
        ...
        else Started (classify_and_update_start ~now:now_value ~keeper ~turn_id)
      | _ -> Started (classify_and_update_start ~now:now_value ~keeper ~turn_id))
  in
  (match outcome with
...
let seconds_since_first_attempt ~(keeper : string) : float option =
  current_state ~keeper |> Option.map (fun s -> now_unix () -. s.first_started_at)
;;
