...
let personality_text_equal a b =
  let one_field s : Keeper_personality_io.coerced_personality =
    Keeper_personality_io.coerce { will = s; needs = ""; desires = ""; instructions = "" }
  in
...
let personality_diff_summary fields =
  List.filter_map
    (fun (name, current, target) -> personality_field_diff_entry name current target)
    fields
;;
...
let personality_field_diff_summary ~field ~current ~target =
  if personality_text_equal current target
  then None
  else (
    let preview s =
      let trimmed = String.trim s in
      if String.length trimmed <= 32 then trimmed else String.sub trimmed 0 32 ^ "..."
    in
    Some
      (Printf.sprintf
...
let apply_default_opt opt current =
  match opt with
  | Some _ -> opt
  | None -> current
;;
...
let invalid_profile_defaults_error ~keeper_name detail =
  if contains_substring detail "cascade_name"
  then
    Printf.sprintf
      "invalid profile.cascade_name for keeper %s: unknown cascade_name: %s"
      keeper_name
      detail
  else Printf.sprintf "invalid keeper profile for keeper %s: %s" keeper_name detail
;;
...
let effective_declarative_cascade_name
  ...
  | None, None -> Keeper_cascade_profile.normalize_declared_name meta.cascade_name
;;
...
let resynced_tool_access
  ...
  let target_preset =
    ...
    | None -> current_preset
  in
  let target_also_allow = apply_default defaults.tool_also_allow current_also_allow in
  match target_preset with
  | Some preset -> Preset { preset; also_allow = target_also_allow }
...
let ensure_keeper_meta config name =
    ...
    (match defaults_result with
     ...
     | Ok defaults ->
       ...
       let target_proactive =
         apply_default defaults.proactive_enabled Keeper_config.default_proactive_enabled
       in
       let target_idle_sec =
         apply_default
           defaults.proactive_idle_sec
           Keeper_config.default_proactive_idle_sec
       in
       let target_cooldown_sec =
         apply_default
           defaults.proactive_cooldown_sec
       ...
       in
       let target_cascade_name = effective_declarative_cascade_name defaults meta in
       (match
          Cascade_catalog_runtime.resolve_declared_name ~raw_name:target_cascade_name ()
        with
        | Error detail ->
          let field =
            match defaults.cascade_name, defaults.manifest_path with
        ...
        | Ok resolved_target_cascade_name ->
          ...
          let target_instructions =
            apply_default defaults.instructions meta.instructions
          in
          (* --- Policy --- *)
          let target_policy_voice_enabled =
            apply_default defaults.policy_voice_enabled meta.policy_voice_enabled
          in
          let target_autoboot_enabled =
            apply_default defaults.autoboot_enabled meta.autoboot_enabled
          in
       ...
       so removing a key from TOML actually reverts the runtime value. *)
          let target_sandbox_profile =
            apply_default
              ...
              Keeper_types_profile.default_sandbox_profile
          in
          ...
          let target_shared_memory_scope =
            apply_default
              defaults.shared_memory_scope
              Keeper_types_profile.default_shared_memory_scope
          in
          let target_allowed_paths = apply_default defaults.allowed_paths [] in
          (* --- Work Discovery --- *)
          let target_wd_enabled =
            apply_default_opt defaults.work_discovery_enabled meta.work_discovery_enabled
          in
          let target_wd_sources =
            apply_default_opt defaults.work_discovery_sources meta.work_discovery_sources
          in
          let target_wd_interval =
            apply_default_opt
              defaults.work_discovery_interval_sec
              meta.work_discovery_interval_sec
          in
          let target_wd_guidance =
            apply_default_opt
              defaults.work_discovery_guidance
              meta.work_discovery_guidance
          in
          (* --- Telemetry Feedback --- *)
          let target_tf_enabled =
            apply_default_opt
              defaults.telemetry_feedback_enabled
              meta.telemetry_feedback_enabled
          in
          let target_tf_window =
            apply_default_opt
              defaults.telemetry_feedback_window_hours
       ...
       of the opaque [personality] category. *)
          ...
          let policy_changed =
            ...
            || meta.always_approve <> target_always_approve
          in
          let discovery_changed =
            ...
            || meta.work_discovery_guidance <> target_wd_guidance
          in
          let telemetry_changed =
            meta.telemetry_feedback_enabled <> target_tf_enabled
            || meta.telemetry_feedback_window_hours <> target_tf_window
          in
          let timeout_policy_changed =
            meta.per_provider_timeout_s <> target_per_provider_timeout
          in
         ...
         persona overlay) is visible without code-reading. *)
            ...
            then (
              let diffs =
                List.filter_map
                  Fun.id
                  [ personality_field_diff_summary
                      ~field:"goal"
                      ~current:meta.goal
                      ~target:target_goal
                  ; personality_field_diff_summary
                      ~field:"short_goal"
                      ~current:meta.short_goal
                      ~target:target_short_goal
                  ; personality_field_diff_summary
                      ~field:"mid_goal"
                      ~current:meta.mid_goal
                      ~target:target_mid_goal
                  ; personality_field_diff_summary
                      ~field:"long_goal"
                      ~current:meta.long_goal
                      ~target:target_long_goal
                  ; personality_field_diff_summary
                      ~field:"will"
                      ~current:meta.will
                      ~target:target_will
                  ...
                  ; personality_field_diff_summary
                      ~field:"desires"
                      ~current:meta.desires
                      ~target:target_desires
                  ; personality_field_diff_summary
                      ~field:"instructions"
                      ~current:meta.instructions
           ...
           hide drift from the dashboard [canonical] column. *)
                cascade_name =
                  (if cascade_changed
                   then resolved_target_cascade_name
  ...
  | Ok None ->
    Log.Keeper.warn
      "ensure_keeper_meta: no persistent meta for %s — run keeper_up to initialize"
      name;
...
let load_or_materialize_boot_meta (ctx : _ context) name
  ...
  | Error original_error ->
    (match Config_dir_resolver.keeper_toml_path_opt name with
     ...
     | Some toml_path ->
       Log.Keeper.info "bootstrapping declarative keeper %s from %s" name toml_path;
       let ok, body =
         Keeper_turn.handle_keeper_up ctx (`Assoc [ "name", `String name ])
       in
       if not ok
       then
         Error
...
let bootstrap_existing_keepers ctx : keeper_bootstrap_stats =
  ...
  then { enabled = false; scanned = 0; started = 0; stale = 0; recovering = 0 }
  else (
    let now_ts = Time_compat.now () in
    let proactive_warmup_sec = keeper_bootstrap_proactive_warmup_sec () in
    let stale_turn_sec = max 0.0 Env_config.KeeperBootstrap.stale_turn_seconds in
    let max_scan = max 0 Env_config.KeeperBootstrap.max_scan in
    let max_keepers = Keeper_runtime_resolved.bootstrap_max_active_keepers () in
    let remaining_slots =
      ref
        (if max_keepers > 0
         then max 0 (max_keepers - Keeper_registry.count_running ())
         else max_int)
    in
      ...
      List.fold_left
        (fun (enabled_acc, scanned_acc, started_acc, stale_acc, recovering_acc) name ->
           ...
           | Ok { meta = m; materialized } ->
             if m.paused
             then enabled_acc, scanned_acc + 1, started_acc, stale_acc, recovering_acc
             else (
               let stale_now =
                 (not materialized)
                 && stale_turn_sec > 0.0
               ...
               let started_here =
                 ...
                 then (
                   let started_now =
                     Keeper_registry.is_running ~base_path:ctx.config.base_path m.name
                   in
                   if started_now && max_keepers > 0
                   then remaining_slots := max 0 (!remaining_slots - 1);
                   started_now)
                 else if already_running
                 then false
                 else if max_keepers > 0 && !remaining_slots <= 0
                 then false
                 else (
                   ...
                   in
                   if started_now && max_keepers > 0
                   then remaining_slots := !remaining_slots - 1;
                   started_now)
               in
               ( true
               , scanned_acc + 1
...
let supervisor_sweeps : (string, Pulse.t) Hashtbl.t = Hashtbl.create 4
...
let start_supervisor_sweep ctx =
  ...
  if supervisor_sweep_running base_path
  then ()
  else (
    let consumer : (module Pulse.Consumer) =
      (module struct
        ...
        let on_beat _beat =
          (try Keeper_supervisor.sweep_and_recover ctx with
           | Eio.Cancel.Cancelled _ as e -> raise e
           | exn ->
             Log.Keeper.error "supervisor sweep failed: %s" (Printexc.to_string exn));
             ...
             |> List.iter (fun (entry : Keeper_registry.registry_entry) ->
               ...
               | Keeper_state_machine.Running ->
                 (match ensure_keeper_meta ctx.config entry.name with
                  ...
                  | Error e ->
                    Log.Keeper.warn "TOML reconcile failed for %s: %s" entry.name e)
               | _ -> ())
           with
           | Eio.Cancel.Cancelled _ as e -> raise e
           | exn ->
             Log.Keeper.error "TOML reconcile sweep failed: %s" (Printexc.to_string exn));
    ...
    let p =
      Pulse.create
        ~clock:ctx.clock
        ~rhythm:
          { Pulse.base_s = sweep_sec; min_s = sweep_sec; max_s = sweep_sec; quiet = 0, 0 }
        ~lifecycle:Always_on
        ~consumers:[ consumer ]
    in
    with_sweeps_rw (fun () -> Hashtbl.replace supervisor_sweeps base_path p);
...
let existing_keepalive_bootstrap_done : (string, unit) Hashtbl.t = Hashtbl.create 4
let has_boot_entries config = bootable_keeper_names config <> []
...
let start_existing_keepalives ctx =
  ...
  let should_run =
    if Hashtbl.mem existing_keepalive_bootstrap_done base_path
    then false
    else (
  ...
  then ()
    ...
    try
      ...
      then
        Log.Keeper.debug
          "bootstrap_existing_keepers enabled=%b scanned=%d started=%d stale=%d \
           recovering=%d"
          stats.enabled
          stats.scanned
          stats.started
...
let stop_keepalive ?base_path name = Keeper_keepalive.stop_keepalive ?base_path name
