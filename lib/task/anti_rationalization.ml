...
let run_llm_reviewer_fn
  : (base_path:string ->
     ...
     unit -> (verdict option, Masc_agent_core.Error.sdk_error) result) Atomic.t
  = Atomic.make (fun ~base_path:_ ?sw:_ ~evaluator_runtime:_ ~prompt:_ ~report_tool_schema:_ ~lookup:_ ~on_tool_result:_ () ->
      Error (Masc_agent_core.Error.Internal "Workspace_hooks: run_llm_reviewer_fn not connected"))
    ...
    (match
     ...
     | Ok prompt ->
       ...
       (match reviewer_result with
        ...
        | Error error ->
          let detail = Masc_agent_core.Error.to_string error in
          (Atomic.get outcome_observer_fn)
            ~outcome:"unavailable"
            ~runtime:evaluator_runtime;
          task_warn
