...
4. **proto 7-RPC 계약 변경 위험**: `proto/masc_workspace collaboration.proto:14-39`의 7 RPC는 안정 계약. CRDT projection을 새 RPC로 추가하면 grpc-direct(Experimental) 부하 증가 + 모든 client 재빌드.
...
```
{
  "topic": "yjs:awareness:workspace-1",
  "client_id": "<u32>",
...
| 5 | Q-P2-1 TODO Claim dual-protocol | `lib/keeper/keeper_todo_claim.*` + `proto/masc_workspace collaboration.proto`(stream Subscribe Event variant 추가, 새 RPC 회피) | TODOClaimProtocol.tla 100 case property test |
...
3. **client 쓰기 거부의 사용자 경험**: keeper workspace collaboration에서 사람이 dashboard에서 "claim"하는 인터랙션은? L0 RPC로 직접 호출 (REST/gRPC) — Y.Map write 아니라.
...
proto/masc_workspace collaboration.proto                  변경: Subscribe stream Event variant 추가 (RPC 시그니처 불변)
...
masc 저장소에 이미 `docs/rfc/RFC-0069-awareness-channel-split.md` ("RFC-0069 / 구 RFC PR-1.7 — Awareness Channel Split (Spec)", 2026-04-29) 가 존재한다. 본 RFC 0017가 *그 위에 얹는 형태*로 통합되어야 한다.
...
| Layer 정의 | L1 = `lib/workspace/workspace_broadcast.ml:58-59` pubsub key (`broadcast:<project>:default`) / L2 = `lib/sse.ml` SSE event_type (`masc:broadcast`, `keeper_heartbeat` 등) | "Authority" = OCaml authoritative state (L0) / "Projection" = Y.Doc read-only mirror (L1) / "Ephemeral" = Y.Doc Awareness (L2) |
...
- 단일 broadcast 채널(`broadcast:<project>:default`)에 yjs:projection:keepers, yjs:awareness:workspace-1, keeper_heartbeat가 모두 흐르면 *throttle/QoS 분리 불가*. 12+ keeper × 30 Hz coalescing이 다른 메시지 fan-out과 경쟁.
...
또는 별도 WebSocket endpoint(`/yjs/awareness/<workspace>`)를 신설할 수도. Q-1(§8)에서 결정.
