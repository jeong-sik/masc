...
| **HTTP write_string call sites** | 정상 — 모두 size-bounded (`content-length` known). | 변경 불필요. |
...
`rg -nc 'write_string' lib/server/` 기준 (모든 응답이 한 번의 `write_string` 으로 끝남):

| 파일 | 사이트 수 | 패턴 |
|---|---|---|
| `lib/server/server_h2_gateway_helpers.ml` | 4 (`h2_respond_json/text/html/...`) | `H2.Body.Writer.write_string writer body; H2.Body.Writer.close writer` — single-shot |
| `lib/server/server_h2_gateway_routes_extra.ml` | 3 | 동일 패턴 |
| `lib/server/server_activity_http.ml` | 1 (SSE event push) | `Httpun.Body.Writer.write_string info.writer data` |
...
본 audit 범위는 *masc-mcp 서버 응답* 측. *Anthropic API 호출 응답* (oas/agent_sdk 의 streaming) 은 별도. 메모리 `oas` audit (~/me/workspace/yousleepwhen/oas/lib/streaming.ml:130-148) 가 ToolUse JSON 을 block 완성 후 일괄 파싱하는 이슈 보고됨 — 그건 oas repo 의 별도 PR 대상.
