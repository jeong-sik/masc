...
let () =
  ...
  let result =
    Masc_exec.Exec_dispatch.(
      resolve_arg (Concat [ Lit "prefix-"; Var "MASC_TEST_P7_VAR"; Lit "-suffix" ]))
  in
...
let () =
  ...
  let ir =
    Simple
      { bin; args = [ Lit "hello"; Lit "world" ]; env = []; cwd = None; redirects = [] }
  in
...
let () = Printf.printf "p7_exec_dispatch: all tests passed.\n"
