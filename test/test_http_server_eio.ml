...
let test_frontend_transport_routes_present () =
  let routes =
    Masc.Server_routes_http_routes_frontend.add_routes
      ~port:8935
      ~host:"127.0.0.1"
      (Router.create ())
  in
...
let test_frontend_canonical_loopback_location_localhost () =
  ...
  let location =
    Masc.Server_routes_http_routes_frontend.canonical_loopback_location
      ~default_port:8935
      request
  in
...
let test_frontend_canonical_root_dashboard_location_localhost () =
  ...
  let location =
    Masc.Server_routes_http_routes_frontend.canonical_root_dashboard_location
      ~default_port:8935
      request
  in
  Alcotest.(check (option string))
...
let test_parse_host_port_rejects_scheme_in_host_header () =
  let parsed =
    Masc.Server_routes_http_common.parse_host_port
