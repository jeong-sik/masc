...
(* A turn of either kind leaves its record on disk and wakes the durable
   consumer; it hands over nothing else, and a disabled toggle wakes
   nothing. *)
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       run_post_turn
         ...
         ~config
         ~meta:(make_meta core_name)
         ~turn:1;
       let official_name = "official-client-owner" in
       run_post_turn
         ~checkpoint_owner:Runtime_execution.Official_client
         ~config
         ~meta:(make_meta official_name)
         ~turn:1;
       Alcotest.(check (list (pair string string)))
         "each owner emits one durable wake"
         [ config.base_path, core_name; config.base_path, official_name ]
         (List.rev !wakes);
       Alcotest.(check (option int))
         "no memory-lane unit is submitted by a turn"
         None
         (Lane.For_testing.pending ~base_path:config.base_path ~keeper_name:official_name);
       ...
       run_post_turn
         ...
         ~meta:(make_meta official_name)
         ~turn:2;
       Alcotest.(check int) "a disabled toggle wakes nothing" 2 (List.length !wakes))
;;
