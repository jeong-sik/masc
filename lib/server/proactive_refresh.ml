...
let log_refresh_failure ~config ~consecutive_failures ~current_interval ~dt exn =
  ...
  then current_interval := min config.max_backoff_s (!current_interval *. 2.0);
  Log.Dashboard.warn
...
let start ~sw ~clock ~config:raw_config ~compute ~on_result =
  ...
  Eio.Fiber.fork ~sw (fun () ->
    ...
    try
      match Eio.Time.with_timeout clock config.timeout_s (fun () -> Ok (compute ())) with
      | Ok v ->
        on_result v;
        Log.Dashboard.info
          "%s warm cache done (%.1fs)"
          config.label
          (Time_compat.now () -. t0)
      | Error `Timeout ->
        notify_error config (Failure "timeout");
        Log.Dashboard.warn
          "%s warm cache skipped (%.1fs timeout)"
          config.label
          (Time_compat.now () -. t0)
    with
    | exn ->
      if should_reraise_cancel exn
      then raise exn
      else (
        notify_error config exn;
        Log.Dashboard.warn
          "%s warm cache failed (%.1fs): %s"
          config.label
          (Time_compat.now () -. t0)
          (Printexc.to_string exn)));
  Eio.Fiber.fork ~sw (fun () ->
    ...
    let rec loop () =
      ...
      let health_ok =
        match config.health_check with
        | None -> true
      ...
      then (
        ...
        (try
           ...
           | Ok v ->
             ...
             then
               Log.Dashboard.info
                 "%s refresh recovered after %d failures"
                 config.label
                 !consecutive_failures;
            ...
            to reduce overlap probability when compute is slow *)
             if dt > config.interval_s *. 0.5
             then (
               current_interval := min config.max_backoff_s (config.interval_s *. 2.0);
               Log.Dashboard.info
             ...
             if dt >= 1.0
             then Log.Dashboard.info "%s refreshed (%.1fs)" config.label dt
             else Log.Dashboard.debug "%s refreshed (%.1fs)" config.label dt
           | Error `Timeout ->
             let dt = Time_compat.now () -. t0 in
