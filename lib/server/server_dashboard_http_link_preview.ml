...
let cache_store ~ttl key preview =
  Eio.Mutex.use_rw ~protect:true preview_cache_mu (fun () ->
    Hashtbl.replace preview_cache key { preview; expires_at = Time_compat.now () +. ttl })
;;
...
let html_entity_replacements =
  ...
  |> List.map (fun (needle, replacement) -> Re.compile (Re.str needle), replacement)
;;

let decode_html_entities value =
  List.fold_left
    (fun acc (re, replacement) -> Re.replace_string re ~all:true ~by:replacement acc)
    value
...
let whitespace_collapse_re = Re.Pcre.re "[ \t\r\n]+" |> Re.compile

let collapse_whitespace value =
  value |> Re.replace_string whitespace_collapse_re ~all:true ~by:" " |> String.trim
;;
...
let validate_resolved_host ~net (uri : Uri.t) =
  try
    ...
    | Some host when is_loopback_or_unspecified_host host -> Error "host is not allowed"
    | Some host ->
      let service =
        match Uri.port uri with
        | Some port -> Int.to_string port
        | None -> if Uri.scheme uri = Some "https" then "443" else "80"
      in
...
let normalize_request_url raw =
  ...
  if trimmed = ""
  then Error "url is empty"
  else (
    let uri = Uri.of_string trimmed in
    if not (is_http_scheme (Uri.scheme uri))
    then Error "only http/https URLs are allowed"
    else if Uri.userinfo uri <> None
    then Error "userinfo in URL is not allowed"
    else if Uri.host uri = None
    then Error "missing host"
    else Ok (Uri.with_fragment uri None |> Uri.to_string))
;;
...
let head_fragment_re =
  Re.Pcre.re ~flags:[ `CASELESS; `DOTALL ] "<head[^>]*>(.*?)</head>" |> Re.compile
;;
...
let title_tag head = first_match head "<title[^>]*>(.*?)</title>"
...
let list_header_ci headers name =
  ...
  |> List.find_map (fun (key, value) ->
    if String.equal (String.lowercase_ascii key) target then Some value else None)
;;
  ...
  | Ok response when is_redirect_status response.status && remaining_redirects > 0 ->
    (match list_header_ci response.headers "location" with
     | Some location when String.trim location <> "" ->
       ...
       fetch_response_following_redirects
         ~net
         ~url:next_url
...
let error_json ~url ~reason ~cache_state () =
  `Assoc
    [ "url", `String url; "error", `String reason; "cache_state", `String cache_state ]
;;
...
let fetch_preview ~clock ~net url =
  ...
  | Ok normalized_url ->
    (match validate_resolved_host ~net (Uri.of_string normalized_url) with
     | Error reason -> Error reason
     | Ok () ->
       if infer_image_url normalized_url
       then
         Ok
           (build_preview_json
              ~url:normalized_url
         ...
         let run () =
           ...
           | Ok response ->
             ...
             then Error (Printf.sprintf "HTTP %d" response.status)
             else (
                 ...
                 Option.value
                   ...
                   (Option.map
                      (fun value ->
                         ...
                         | head :: _ ->
                           String.starts_with ~prefix:"image/" (lower_trim head)
                         | [] -> false)
                      content_type)
               then
                 Ok
                 ...
                 let body =
                   if String.length response.body > max_html_chars
                   then String.sub response.body 0 max_html_chars
                   else response.body
                 in
                 let extracted = extract_html_preview_fields ~url:normalized_url body in
                 let canonical_url =
                 ...
                 Ok
                   (build_preview_json
                      ...
                      ())))
         in
         try Eio.Time.with_timeout_exn clock preview_timeout_sec run with
         | Eio.Cancel.Cancelled _ as e -> raise e
...
let urls_of_request args =
  ...
  | `List items ->
    ...
    in
    if urls = []
    then Error "urls must contain at least one non-empty string"
      ...
      (fun url ->
         ...
         | Some cached ->
           (match error_reason_of_json cached with
            | Some reason -> error_fields := (url, `String reason) :: !error_fields
            | None ->
              preview_fields := (url, with_cache_state cached "hit") :: !preview_fields)
         | None ->
