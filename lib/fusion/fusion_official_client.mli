...
type failure =
  ...
  | Attributed_setup_failure of Fusion_types.panel_failure
      (** A setup cause already attributed by this adapter's
          [provider_error ~runtime_id] boundary. *)
...
(** The adapter's own failure text, naming [runtime_id] once. A raw
    [Setup_failure] gains the ID here; an [Attributed_setup_failure] already
    has it. For log and status lines that should keep what {!panel_failure}
    folds away, such as a timeout's seconds. *)

val panel_failure : runtime_id:string -> failure -> Fusion_types.panel_failure
(** Project a client failure onto the panel vocabulary. A raw setup provider
    error gains [runtime_id]; an attributed setup failure passes through. This
    adapter emits the attributed variant for its own setup failures. Each
