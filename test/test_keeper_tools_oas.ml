...
let read_repo_dir meta =
  Filename.concat "repos" meta.Masc_mcp.Keeper_meta_contract.name
;;
...
let make_registered_tools ~config ~meta ~ctx_snapshot () =
  let keeper_name = meta.Masc_mcp.Keeper_meta_contract.name in
...
let make_research_meta ?tool_access () : Masc_mcp.Keeper_meta_contract.keeper_meta =
  let tool_access =
    match tool_access with
    | Some access -> access
    | None -> Masc_mcp.Keeper_meta_contract.Preset { preset = Masc_mcp.Keeper_meta_contract.Research; also_allow = [] }
  in
  match
    Masc_test_deps.meta_of_json_fixture
      (`Assoc
...
let make_learned_meta () : Masc_mcp.Keeper_meta_contract.keeper_meta =
  match
    Masc_test_deps.meta_of_json_fixture
      (`Assoc
          ...
          ; ( "tool_access"
            , Masc_mcp.Keeper_meta_contract.tool_access_to_json
                (Masc_mcp.Keeper_meta_contract.Preset { preset = Masc_mcp.Keeper_meta_contract.Full; also_allow = [] }) )
          ])
  with
  | Ok meta -> meta
