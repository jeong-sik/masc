...
- **GitHub**: inspect PRs/issues with native PR tools such as `keeper_pr_status`, `keeper_pr_review_read`, and `keeper_pr_list` when available. Create or update PRs through `Bash` with `executable="gh"` and typed `argv` after pushing from a prepared worktree.
