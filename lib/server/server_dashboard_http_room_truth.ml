(** Room-truth read model and SSE snapshot broadcasting. *)

open Server_dashboard_http_core

module Execution_surfaces = Server_dashboard_http_execution_surfaces
module Room_truth_support = Server_dashboard_http_room_truth_support
    ...
    with_dashboard_timeout ~clock (fun () ->
        ...
        let fiber_with_timeout ?(timeout_s = base_timeout_s) label f fallback =
          try
            ...
            | Ok v -> v
            | Error `Timeout ->
                Log.Dashboard.warn "room-truth fiber %s timed out (%.0fs)" label
                  timeout_s;
                fallback
          with
          | Eio.Cancel.Cancelled _ as e -> raise e
          | exn ->
              Log.Dashboard.warn "room-truth fiber %s failed: %s" label
        ...
        in
        Room_truth_support.compose_room_truth_snapshot ~config
...
let () =
  Execution_surfaces._broadcast_room_truth_ref := broadcast_room_truth_snapshot
