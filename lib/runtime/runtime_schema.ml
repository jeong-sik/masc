...
type model_spec =
  ...
  ; turn_timeout_s : float option
    (** [turn-timeout-s] — per-model liveness window for one official-client
        ...
        is a running tool step, and Codex while an item it started (a command,
        a file change, an MCP call, a sleep) is still open, because the CLI
        writes nothing until that step or item ends; [wall-clock-ceiling-s]
        below is the only bound there.
