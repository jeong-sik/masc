...
let test_loops_json_skips_invalid_persisted_state () =
  ...
  in
  (* load_state rejects partial state.json missing required fields
     (current_cycle, baseline, best_score, etc.) — returns None. *)
  check int "total rejects partial persisted state" 0
    Yojson.Safe.Util.(json |> member "total" |> to_int);
  check int "zero loop entries" 0
