...
let test_harness_footer_links_to_overview_task () =
  check str "Harness names its task link"
    "j/k:move  v:next Planning tab  PgUp/PgDn:page  Right / Enter:verdict  Left / Esc:back  y:agree  n:overrule  Y:copy task  r:refresh  Tab:next  q:quit"
...
let test_verification_footer_carries_the_verdict_keys () =
  ...
  check str "verification names detail, approve, and reject"
    "j/k:move  v:next Planning tab  Right / Enter:details  Left / Esc:back  a:approve  x:reject  r:refresh  Tab:next  q:quit"
...
let test_planning_footer_carries_filter_and_sort () =
  check str "planning names filter and sort"
    "j/k:move  v:next Planning tab  f:filter  s:sort  Right / Enter:detail  Left / Esc:back  c:complete  x:drop  o:reopen  Y:copy link  r:refresh  Tab:next  q:quit"
