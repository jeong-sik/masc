...
let permission_bits path = (Unix.stat path).Unix.st_perm land 0o777
...
let test_create_credential () =
  ...
  | Error _ -> fail "create_token should succeed"
;;
...
let test_verify_token () =
  ...
  | Error _, _ -> fail "create_token should succeed"
;;
...
let test_verify_wrong_token () =
  ...
  cleanup_test_room dir;
  match create_result, verify_result with
  | Ok _, Ok _ -> fail "verify_token should fail with wrong token"
...
let test_verify_token_reports_token_owner_on_agent_mismatch () =
  ...
  | Ok _, Error e ->
    fail
      (Printf.sprintf "wrong error type for mismatch: %s" (Types.masc_error_to_string e))
  | Error _, _ -> fail "create_token should succeed"
;;
...
let test_verify_token_allows_generated_alias_for_token_owner_prefix () =
  ...
  | Ok _, Error e ->
    fail
      (Printf.sprintf
         "generated alias should reuse owner token: %s"
         (Types.masc_error_to_string e))
  | Error _, _ -> fail "create_token should succeed"
;;
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         ...
         "uuid target removed"
         true
         (Option.is_none (Auth.load_credential dir (Types.Credential_id.to_string id)));
       ...
       check
         int
         "0 credentials after uuid-backed delete"
         0
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Error e ->
         fail
           (Printf.sprintf
              "stable keeper token creation failed: %s"
              (Types.masc_error_to_string e))
       | Ok (stable_raw_token, _) ->
         (match Auth.ensure_keeper_credential dir ~agent_name:"keeper-sangsu-agent" with
          | Error e ->
            fail
              (Printf.sprintf
                 "keeper credential bootstrap failed: %s"
                 (Types.masc_error_to_string e))
          | Ok _ ->
            check
              bool
              "exact alias credential exists"
              true
              (Option.is_some (Auth.load_credential dir "keeper-sangsu-agent"));
            (match
               Auth.verify_token
                 dir
             ...
             with
             | Ok cred ->
               check string "stable keeper owner accepted" "sangsu" cred.agent_name
             | Error e ->
               fail
                 (Printf.sprintf
...
let test_load_credential_missing_keeper_alias_stays_quiet () =
  ...
  check
    (option string)
    "missing keeper alias returns none"
    None
    (Option.map (fun cred -> cred.Types.agent_name) resolved);
  check string "missing keeper alias emits no parse noise" "" (String.trim stderr_output)
;;
...
let test_verify_token_dashboard_legacy_alias_fallback () =
  ...
  let result =
    ...
    | Ok (raw_token, _) -> Auth.verify_token dir ~agent_name:"dashboard" ~token:raw_token
    | Error e -> Error e
  in
  cleanup_test_room dir;
  match result with
  | Ok cred -> check string "legacy dashboard token owner" "dashboard-dev" cred.agent_name
  | Error e ->
    fail
      (Printf.sprintf
...
let test_save_raw_token_credential_uses_provided_token () =
  ...
  let verify_result =
    ...
    | Ok _ -> Auth.verify_token dir ~agent_name:"bootstrap-admin" ~token:raw_token
    | Error e -> Error e
  in
  cleanup_test_room dir;
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Ok (raw_token, cred) ->
         ...
         check
           bool
           "keeper bearer is not the shared internal token"
           false
         ...
         check
           bool
           "persisted raw token file created"
           true
         ...
         check
           bool
           "keeper credential persisted by exact name"
           true
           (Option.is_some (Auth.load_credential dir "keeper-masc-improver-agent"));
         check
           bool
           "no normalized keeper credential persisted"
           true
         ...
         (match
          ...
          | Ok verified ->
            check
              string
              "keeper bearer verifies exact agent"
              "keeper-masc-improver-agent"
              verified.agent_name
          | Error e ->
            fail
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Ok (first_raw_token, first_cred), Ok (reused_raw_token, reused_cred) ->
         ...
         check
           int
           "persisted raw token file mode 0600"
           0o600
           (permission_bits raw_token_path);
         check
           string
           "first credential uses exact keeper name"
           "keeper-masc-improver-agent"
           first_cred.agent_name;
         check
           string
           "reused credential keeps exact keeper name"
           "keeper-masc-improver-agent"
           reused_cred.agent_name;
         check string "persisted keeper raw token reused" first_raw_token reused_raw_token
       | Error e, _ | _, Error e ->
         fail
           (Printf.sprintf
...
let test_admin_permissions () =
  check bool "admin can read" true (Types.has_permission Types.Admin Types.CanReadState);
  check bool "admin can init" true (Types.has_permission Types.Admin Types.CanInit);
  check bool "admin can reset" true (Types.has_permission Types.Admin Types.CanReset);
...
let test_auth_disabled_allows_all () =
  ...
  let result =
    Auth.check_permission dir ~agent_name:"anyone" ~token:None ~permission:Types.CanInit
  in
  cleanup_test_room dir;
  match result with
  | Ok () -> ()
...
let test_authorize_unknown_non_masc_tool_strict_denied () =
  ...
  in
  let before =
    strict_unknown_tool_denial_count ~agent_name:"worker_agent" ~tool_class:"external"
  in
  let result =
    match create_result with
    | Ok (raw_token, _) ->
          ...
          (fun acc tool_name ->
             ...
             | Ok () ->
               Auth.authorize_tool
                 ...
                 ~tool_name)
          (Ok ())
          keeper_strict_auth_regression_tools)
    | Error e -> Error e
  in
  cleanup_test_room dir;
  match result with
...
let test_declared_tool_permission_from_tool_spec () =
  ...
  check
    bool
    "tool permission comes from Tool_spec metadata"
    true
  ...
  run
    ...
    ; ( "config"
      , [ test_case "default config" `Quick test_default_auth_config
        ...
        ; test_case
            "save/load config in Eio runtime"
            `Quick
            test_save_load_auth_config_in_eio_runtime
        ; test_case "auth config saved private" `Quick test_auth_config_saved_private
        ] )
    ; ( "credentials"
      , [ test_case "create credential" `Quick test_create_credential
        ; test_case "credential saved private" `Quick test_credential_saved_private
        ; test_case "verify token" `Quick test_verify_token
        ; test_case "verify wrong token" `Quick test_verify_wrong_token
        ; test_case
            "verify token reports token owner on agent mismatch"
            `Quick
            test_verify_token_reports_token_owner_on_agent_mismatch
        ; test_case
            "verify token allows generated alias for owner prefix"
            `Quick
    ...
    ; ( "authorization"
      , [ test_case "auth disabled allows all" `Quick test_auth_disabled_allows_all
        ...
        ; test_case
            ...
            `Quick
            test_auth_enabled_with_valid_token
        ; test_case
            "permission denied for worker admin action"
            `Quick
            test_permission_denied_for_worker_admin_action
        ; test_case
            "strict unknown masc tool allows worker"
            `Quick
            test_authorize_unknown_masc_tool_strict_worker_allowed
        ; test_case
            "strict unknown non-masc tool denied"
            `Quick
            test_authorize_unknown_non_masc_tool_strict_denied
        ; test_case
            "strict unknown canonical tool allows worker"
            `Quick
            test_authorize_unknown_canonical_tool_strict_worker_allowed
        ; test_case
            "strict known keeper tool allows worker"
            `Quick
            test_authorize_known_keeper_tool_strict_worker_allowed
        ; test_case
            "strict v2 known keeper tool allows worker"
            `Quick
            test_authorize_tool_v2_known_keeper_tool_strict_worker_allowed
        ; test_case
            "strict v2 fake keeper prefix denied"
            `Quick
            test_authorize_tool_v2_unknown_keeper_prefix_strict_denied
        ; test_case
            "declared tool permission from Tool_spec"
            `Quick
            test_declared_tool_permission_from_tool_spec
        ] )
    ; ( "enable_disable"
