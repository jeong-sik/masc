...
let is_scheduled_autonomous_channel = Keeper_world_observation.is_autonomous_channel

let is_scheduled_autonomous_cycle_of_observation
      (observation : Keeper_world_observation.world_observation)
  : bool
  =
  String.equal (decision_channel_of_observation observation) "scheduled_autonomous"
;;

let scheduled_autonomous_outcome_of_result ~(has_text : bool) ~(has_tool_calls : bool)
...
let record_turn_latency_bucket ~(keeper : string) ~(latency_ms : int) : unit =
  ...
  then
    Log.Keeper.warn
      "[long-turn] keeper=%s latency_ms=%d (>= %d ms threshold) bucket=%s — investigate \
       cascade exhaustion / oas_timeout_budget (#9933, #9943)"
      keeper
      latency_ms
      threshold
      bucket
...
let estimate_trusted_usage_cost_usd ~usage_trusted ~model usage =
  if usage_trusted then Keeper_hooks_oas.estimate_usage_cost_usd ~model usage else 0.0
;;
...
let record_usage_trust ~keeper_name ~(trust : usage_trust) =
  ...
  | Usage_untrusted reasons ->
    List.iter
      (fun reason ->
         Prometheus.inc_counter
           usage_anomaly_reason_metric
           ~labels:[ "keeper", keeper_name; "reason", reason ]
           ())
      reasons;
    Log.Keeper.warn
      "#9959 usage_anomaly keeper=%s reasons=[%s] — upstream fix tracked in \
       jeong-sik/oas#1181; cost accounting is suppressed to 0.0 for this turn by \
       [usage_trust_is_trusted] gate."
      keeper_name
...
let telemetry_reported_of_result (result : Keeper_agent_run.run_result) : bool =
...
let coverage_reason_of_result (result : Keeper_agent_run.run_result) : string option =
  let telemetry_reported = telemetry_reported_of_result result in
  if result.usage_reported && telemetry_reported
  then None
  else (
    let provider =
      Keeper_agent_run.surface_model_used result |> Keeper_hooks_oas.provider_of_model
    in
    match result.usage_reported, telemetry_reported with
    | false, false ->
      if
        String.equal result.tool_surface.tool_requirement "none"
...
let coverage_stage_of_result (result : Keeper_agent_run.run_result) : string option =
...
let validated_evidence_preview (v : Oas.Raw_trace.run_validation) : string =
  if v.has_file_write
  then "(validated evidence: file_write)"
  else (
    match v.tool_names with
    | [] -> "(validated evidence)"
    | names -> Printf.sprintf "(validated evidence: %s)" (String.concat ", " names))
;;
...
let accountability_evidence_refs
  ...
  let tool_refs =
    ...
    |> List.filter_map (fun tool_name ->
      ...
      if trimmed = "" || String.equal trimmed stay_silent
      then None
  ...
  let validation_refs =
    ...
    | Some validation ->
      ...
      in
      if validation.has_file_write then "validation:file_write" :: base else base
    | None -> []
  in
...
let scheduled_autonomous_outcome_for_result (result : Keeper_agent_run.run_result)
  : scheduled_autonomous_cycle_outcome
  =
...
let turn_mode_of_result (result : Keeper_agent_run.run_result) : turn_mode =
  ...
  if has_visible_tool_signal result
  then Tool_use
  else if text = ""
  then Noop
  else if String.starts_with ~prefix:"SKIP:" text
  then Skip_text
  else Text_response
;;
...
let observed_triggers_of_observation
  ...
  if observation.active_goals <> [] && observation.idle_seconds > 0
  then add "idle_timeout_candidate";
    ...
    Digest.to_hex
      (Digest.string
         (Printf.sprintf
            ...
            suffix_seed))
  in
  Printf.sprintf "dec-%Ld-%s" (Int64.of_float (ts *. 1000.0)) (String.sub digest 0 8)
;;

let tool_call_detail_to_json (detail : Keeper_agent_run.tool_call_detail) : Yojson.Safe.t =
  `Assoc
...
let append_decision_record
  ...
  in
  let turn_id = Option.value ~default:meta.runtime.usage.total_turns turn_id_opt in
  let task_id =
  ...
  let goal_id =
    ...
    | [] -> None
  in
  let runtime_contract = Keeper_runtime_contract.runtime_contract_json ~config meta in
  let pending_approval_count =
    Keeper_approval_queue.pending_count_for_keeper ~keeper_name:meta.name
  in
  let claim_executed = List.exists Keeper_tool_disclosure.is_claim_tool_name tools_used in
  let social_fields =
    ...
    | Some state ->
      ...
      ; ( "delivery_surface"
        , `String (Social.delivery_surface_to_string state.delivery_surface) )
      ]
  in
  let turn_mode =
  ...
  let json =
    `Assoc
      ([ "id", `String (decision_id ~meta ~ts:now_ts ~suffix_seed)
       ...
       ; "degraded_retry_cascade", Json_util.string_opt_to_json degraded_retry_cascade
       ...
       ; ( "observation"
         , `Assoc
             ...
             ; ( "pending_scope_messages"
               , `Int (List.length observation.pending_scope_messages) )
             ...
             ; ( "claim_blocked_task_count"
               , `Int
                   (max
                      0
                      (observation.unclaimed_task_count - observation.claimable_task_count))
               )
             ...
             ; ( "worktree_change_detected"
               , `Bool (Option.is_some observation.worktree_change_summary) )
             ] )
       ...
       ; ( "deliberation_execution"
         , match deliberation_execution with
           | Some execution -> Keeper_deliberation.execution_result_to_json execution
           | None -> `Null )
       ; ( "response_preview"
         ...
         , match result with
           | Some { proof = Some p; _ } ->
             `Assoc
               ...
               ; "result_status", Oas.Cdal_proof.result_status_to_yojson p.result_status
       ...
       ; ( "telemetry"
         , match result with
           | Some r ->
             ...
             let resolved_model_id = Keeper_agent_run.surface_resolved_model_id r in
             ...
             let cascade_fields =
               ...
               | Some co ->
                 ...
                 ; ( "primary_model"
                   , match co.primary_model with
                     | Some m -> `String m
                     | None -> `Null )
                 ; ( "selected_model"
                   , match co.selected_model with
                     | Some m -> `String m
                     | None -> `Null )
                 ; "fallback_applied", `Bool co.fallback_applied
                 ; ( "fallback_hops"
                   , match co.fallback_hops with
                     | Some n -> `Int n
                     | None -> `Int 0 )
                 ; ( "candidate_models"
                   , `List (List.map (fun s -> `String s) co.candidate_models) )
                 ]
               | None -> []
             in
               ...
               | Some t ->
                 let timings_fields =
                   ...
                   | Some ti ->
                     ...
                     [ ( "prompt_ms"
                       , match ti.prompt_ms with
                         | Some v -> `Float v
                         | None -> `Null )
                     ; ( "predicted_ms"
                       , match ti.predicted_ms with
                         | Some v -> `Float v
                         | None -> `Null )
                 ...
                 in
                 [ ( "system_fingerprint"
                   , match t.system_fingerprint with
                     | Some s -> `String s
                     | None -> `Null )
                 ; ( "reasoning_tokens"
             ...
             let usage_fields =
               ...
               then
                 ...
                 ; ( "cost_usd"
                   , match r.usage.cost_usd with
                     | Some c -> `Float c
                     | None -> `Null )
                 ; ( "tokens_per_second"
                   , if usage_trust_is_trusted usage_trust && latency_ms > 0
                     then
                       `Float
           ...
           | None ->
             ...
             let error_category =
               ...
               | Some e when String.length e > 0 ->
                 ...
                 in
                 let contains needle = string_contains_substring ~needle e_lower in
                 (* starts_with checks first (more specific), then contains *)
                 if starts_with "invalid request"
                 then "invalid_request"
                 else if starts_with "network error"
                 then "network_error"
                 else if starts_with "internal error"
                 then "internal_error"
                 else if starts_with "input to"
                 then
                   "input_budget_exceeded"
                   (* contains checks second (broader, order matters) *)
                 else if contains "turn outcome ambiguous"
                 then "ambiguous_side_effect"
                 else if contains "connection_failure" || contains "connection refused"
                 then "network_error"
                 else if contains "timeout" || contains "timed out"
                 then "timeout"
                 else if contains "context length" || contains "token budget"
                 then "input_budget_exceeded"
                 else "other"
               | _ -> "unknown"
             in
             `Assoc
       ...
       @ social_fields)
  in
  try append_jsonl_line (keeper_decision_log_path config meta.name) json with
  | Eio.Cancel.Cancelled _ as e -> raise e
  | exn ->
    Log.Keeper.warn
      "append decision record failed for %s: %s"
      meta.name
...
let update_metrics_from_result
      (meta : keeper_meta)
      ~(latency_ms : int)
      ~(observation : Keeper_world_observation.world_observation)
  ...
  let trusted_input_tokens = if usage_trusted then result.usage.input_tokens else 0 in
  let trusted_output_tokens = if usage_trusted then result.usage.output_tokens else 0 in
  ...
  let turn_cost =
    estimate_trusted_usage_cost_usd ~usage_trusted ~model:used_model_id result.usage
  in
  ...
  let visible_tool_signal_present = has_substantive_tools || has_validated_evidence in
  ...
  ; runtime =
      { rt with
        usage =
          ...
          ; total_output_tokens = rt.usage.total_output_tokens + trusted_output_tokens
          ; total_tokens = rt.usage.total_tokens + trusted_total_tokens
      ...
      ; (* Deterministic scheduled autonomous cycle accounting is separated from
        ...
        proactive_rt =
          ...
          ; last_ts =
              (if update_proactive_rt && is_scheduled_autonomous_cycle
               then now_ts
          ...
          ; last_reason =
              (if (not update_proactive_rt) || not is_scheduled_autonomous_cycle
               ...
               else if has_substantive_tools
               then
                 Printf.sprintf "unified:tools=[%s]" (String.concat "," result.tools_used)
               else if has_validated_evidence
               then (
                 match validated_evidence with
                 | Some v ->
                   Printf.sprintf
                     "unified:validated_evidence(ok=%b,file_write=%b,evidence=%d)"
                     v.ok
                     v.has_file_write
                     (List.length v.evidence)
                 | None -> "unified:validated_evidence(unreachable)")
               else if not has_text
               then
                 "unified:"
                 ^ scheduled_autonomous_cycle_outcome_to_string Proactive_silent
               else if has_text
               then "unified:text_response"
               else rt.proactive_rt.last_reason)
          ; last_preview =
              (if (not update_proactive_rt) || not is_scheduled_autonomous_cycle
               then rt.proactive_rt.last_preview
               else if has_text
               then short_preview result.response_text
               else if has_substantive_tools
               then Printf.sprintf "(tools: %s)" (String.concat ", " result.tools_used)
               else (
                 match validated_evidence with
          ...
          ; (* Work discovery timestamp only advances when the keeper
           ...
           preventing silent cycles from consuming the scan interval. *)
            last_work_discovery_ts =
              (if observation.work_discovery_due && has_substantive_tools
               then now_ts
               else rt.proactive_rt.last_work_discovery_ts)
          ; work_discovery_count =
              (rt.proactive_rt.work_discovery_count
               + if observation.work_discovery_due && has_substantive_tools then 1 else 0
              )
      ...
      ; last_active_desire = Option.value ~default:"" social_state.active_desire
      ; last_current_intention = Option.value ~default:"" social_state.current_intention
...
let append_metrics_snapshot
  ...
  let scheduled_autonomous_outcome =
    if is_scheduled_autonomous_channel channel
    then Some (scheduled_autonomous_outcome_for_result result)
    else None
  in
    ...
    then
      `Assoc
        ([ "input_tokens", `Int result.usage.input_tokens
         ...
         ; "total_tokens", `Int (Keeper_exec_context.total_tokens result.usage)
         ]
         @ usage_trust_json_fields usage_trust)
    else
      `Assoc
  ...
  in
  let cost_json =
    if result.usage_reported && usage_trust_is_trusted usage_trust
    then `Float turn_cost
    else `Null
  in
  ...
  let snapshot =
    `Assoc
      ...
      ; ( "usage_anomaly_reasons"
        , `List
            (List.map (fun reason -> `String reason) (usage_trust_reasons usage_trust)) )
      ; "latency_ms", `Int latency_ms
      ; "cost_usd", cost_json
      ...
      ; ( "scheduled_autonomous_outcome"
        , match scheduled_autonomous_outcome with
          | Some outcome -> `String (scheduled_autonomous_cycle_outcome_to_string outcome)
          | None -> `Null )
      ; ( "proactive_outcome"
        , match scheduled_autonomous_outcome with
          | Some outcome -> `String (scheduled_autonomous_cycle_outcome_to_string outcome)
      ...
      ; ( "trace_ref"
        , match result.trace_ref with
          | Some trace_ref -> Oas.Raw_trace.run_ref_to_yojson trace_ref
          | None -> `Null )
      ; ( "run_validation"
        , match result.run_validation with
          | Some validation -> Oas.Raw_trace.run_validation_to_yojson validation
          | None -> `Null )
      ; ( "cdal_proof"
        , match result.proof with
          | Some p ->
            `Assoc
              [ "run_id", `String p.Oas.Cdal_proof.run_id
              ; "effective_mode", Oas.Execution_mode.to_yojson p.effective_execution_mode
              ; "result_status", Oas.Cdal_proof.result_status_to_yojson p.result_status
              ; "violation_count", `Int (cdal_violation_ref_count p)
              ; "raw_evidence_ref_count", `Int (cdal_raw_evidence_ref_count p)
              ; "tool_trace_count", `Int (List.length p.tool_trace_refs)
      ...
      ; ( "inference_telemetry"
        , match result.inference_telemetry with
          | Some t -> Oas.Types.inference_telemetry_to_yojson t
          | None -> `Null )
      ]
  in
...
let broadcast_lifecycle_events
  ...
  then (
    ...
    | Eio.Cancel.Cancelled _ as e -> raise e
    | exn ->
      Log.Keeper.error "compaction SSE broadcast failed: %s" (Printexc.to_string exn));
  match handoff_json with
    ...
    (try
       Sse.broadcast
         (`Assoc
             ...
             ; "from_model", `Null
             ; ("to_model", if String.trim to_model = "" then `Null else `String to_model)
             ; "ts_unix", `Float now_ts
             ])
     with
     | Eio.Cancel.Cancelled _ as e -> raise e
     | exn -> Log.Keeper.error "handoff SSE broadcast failed: %s" (Printexc.to_string exn))
  | _ -> ()
;;

let update_metrics_from_failure
      (meta : keeper_meta)
      ~(latency_ms : int)
      ~(observation : Keeper_world_observation.world_observation)
      ~(reason : string)
  ...
  =
  ignore is_transient;
  (* Param retained for caller compatibility; no longer
    ...
    | Some err ->
      (match Oas_worker_named.classify_masc_internal_error err with
       ...
       | Some
         ...
         Printf.sprintf
           "OAS budget timeout after %.1fs (%s, estimated input %d tokens, keeper hard \
            cap %.1fs)"
           budget_sec
           source
    ...
    | Some err ->
      (match Oas_worker_named.classify_masc_internal_error err with
       | Some
           ...
           | Oas_worker_named.Resumable_cli_session _ ) -> true
       | Some _ | None -> false)
    | None -> false
  in
  let preview =
    let trimmed = String.trim public_reason in
    if trimmed = "" then "keeper cycle failed" else short_preview trimmed
  in
  { meta with
    updated_at = now_iso ()
  ; runtime =
      ...
      ; proactive_rt =
          ...
          ; consecutive_noop_count =
              (if failure_counts_for_proactive_backoff
               then meta.runtime.proactive_rt.consecutive_noop_count + 1
      ...
      ; last_blocker =
          (match social_state with
           | Some (state : Social.social_state) -> Option.value ~default:"" state.blocker
           | None -> short_preview public_reason)
      ; last_blocker_class =
          (match sdk_error with
           | Some err -> Keeper_status_bridge.blocker_class_of_sdk_error err
           | None -> None)
      ; last_need =
          (match social_state with
           | Some (state : Social.social_state) -> Option.value ~default:"" state.need
