...
env={'PATH':os.environ['PATH'],'LANG':'en_US.UTF-8','MASC_BASE_PATH':str(base),'MASC_CONFIG_DIR':str(cfg),'MASC_CONFIG_BOOTSTRAP':'skip','MASC_KEEPER_AUTONOMOUS_ENABLED':'false','MASC_ORCHESTRATOR_ENABLED':'false','MASC_GRPC_ENABLED':'0','MASC_WS_ENABLED':'0','MASC_LOG_LEVEL':'debug'}
headers={'Accept':'application/json, text/event-stream'};seq=0;observations=[];setup=[];responses=[];diagnostic_windows=[]
...
def rpc(method,params):
 global seq
 seq+=1;status,obj,rh,observation=request('POST','/mcp',{'jsonrpc':'2.0','id':seq,'method':method,'params':params})
 observation.update({'rpc_method':method,'rpc_id':seq,'status':status})
 ...
 return obj['result'],observation
def tool(name,args):
 r,observation=rpc('tools/call',{'name':name,'arguments':args});assert not r.get('isError',False) and r['structuredContent']['ok'] is True,r
 setup.append({'tool':name,'arguments_sha256':hashlib.sha256(json.dumps(args,sort_keys=True).encode()).hexdigest(),'result':r,'http_observation':observation})
 return r
...
try:
 identity={'source':manifest['commit'],'binary_sha256':manifest['sha256']['main_eio.exe'],'base':str(base),'port':port,'pid':process.pid,'artifact_id':meta['id'],'artifact_run':meta['workflow_run']['id'],'runner_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'environment_keys':sorted(env),'model_endpoint':f'http://127.0.0.1:{stub.server_port}/v1','requested_encoding':a.encoding,'text_kind':a.text_kind,'seed_tasks':a.tasks,'cycles':a.cycles,'command':[sys.executable,*sys.argv],'python':sys.version,'runtime_config_sha256':hashlib.sha256((cfg/'runtime.toml').read_bytes()).hexdigest(),'runtime_fixture_sha256':hashlib.sha256(fixture.encode()).hexdigest()}
 ...
 listed,_=rpc('tools/list',{});names={t['name'] for t in listed['tools']};assert {'masc_add_task','masc_batch_add_tasks'}<=names
 for offset in range(0,a.tasks,20):
  tool('masc_batch_add_tasks',{'tasks':[{'title':f'Synthetic backlog {n:04d}','description':'Synthetic response fixture. '+(({'ascii':'verify ASCII payload ','multilingual':'검증 ASCII payload '}[a.text_kind])+str(n)+' ')*50,'priority':3} for n in range(offset,min(offset+20,a.tasks))]})
 ...
 for cycle in range(1,a.cycles+1):
  ...
  for phase in ('cold','warm'):
   ...
   with (out/'server.log').open('rb') as captured:
    captured.seek(log_start);window=captured.read(log_end-log_start).decode('utf-8',errors='strict')
   diagnostic_windows.append({'cycle':cycle,'phase':phase,'start_offset':log_start,'end_offset':log_end,'lines':window.splitlines()})
   responses.append({'cycle':cycle,'phase':phase,'body':obj,'headers':rh})
 ...
 (out/'diagnostic-log-windows.json').write_text(json.dumps(diagnostic_windows,indent=2)+'\n')
