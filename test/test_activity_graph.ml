  ...
  with_config (fun config ->
    ...
    ignore
      (Activity_graph.emit
         ...
         ());
    let json = Activity_graph.graph_json config ~limit:20 ~timeline_limit:10 () in
...
let test_graph_json_tracks_runtime_activity_kinds () =
  ...
  with_config (fun config ->
    ...
    let has_node id status =
      List.exists
        (fun node ->
           member "id" node = `String id && member "status" node = `String status)
        nodes
    in
    let has_edge kind =
      List.exists (fun edge -> member "kind" edge = `String kind) edges
    in
    check
      bool
      "operation node marked running"
      true
      (has_node "operation:sess-001" "running");
    check
      bool
      "task node marked in progress"
      true
      (has_node "task:task-777" "in_progress");
    check bool "team turn edge captured" true (has_edge "participates_in");
    check bool "board post edge captured" true (has_edge "posts");
    check bool "board vote edge captured" true (has_edge "votes_on"))
;;
...
let test_graph_json_reports_kind_counts_and_heatmap_totals () =
  ...
  with_config (fun config ->
    ...
    check int "task.started count" 1 (kind_counts |> member "task.started" |> to_int);
    check
      int
      "heatmap total matches filtered events"
      3
      (heatmap |> member "total" |> to_int);
    check int "heatmap rows" 7 (List.length matrix);
    check
      bool
      "heatmap rows expose 24 hours"
      true
  ...
  with_config (fun config ->
    ...
    ignore
      (Activity_graph.emit
         ...
         ~payload:(`Assoc [ "task_id", `String "task-new" ])
         ());
    let json = Activity_graph.agent_spans_json config ~since_ms:cutoff_ms ~limit:20 () in
    let open Yojson.Safe.Util in
    let spans = json |> member "spans" |> to_list in
    check int "only recent span remains" 1 (List.length spans);
    check
      string
      "recent span label kept"
      "task-new"
...
let test_span_status_of_string_handles_ended_round_trip () =
  check
    string
    "ended round-trips explicitly"
    "ended"
    (Activity_graph.span_status_of_string "ended" |> Activity_graph.span_status_to_string)
;;

let test_span_status_of_string_keeps_legacy_unknown_fallback () =
  check
    string
    "unknown falls back to ended"
    "ended"
  ...
  run
    ...
    [ ( "core"
      , [ test_case "emit and list events" `Quick test_emit_and_list_events
        ; test_case
            "filtered client receives matching events"
            `Quick
            test_filtered_client_receives_matching_events
        ; test_case
            "graph summary builds nodes and edges"
            `Quick
            test_graph_json_summarizes_relationships
        ; test_case
            "graph summary tracks runtime activity kinds"
            `Quick
            test_graph_json_tracks_runtime_activity_kinds
        ; test_case
            "graph summary exposes kind counts and full heatmap totals"
            `Quick
            test_graph_json_reports_kind_counts_and_heatmap_totals
        ; test_case
            "agent spans honor since filter"
            `Quick
            test_agent_spans_json_honors_since_ms
        ; test_case
            "parse_since_ms supports minutes"
            `Quick
            test_parse_since_ms_supports_minutes
        ; test_case
            "span_status parses ended explicitly"
            `Quick
            test_span_status_of_string_handles_ended_round_trip
        ; test_case
            "span_status keeps unknown fallback"
            `Quick
            test_span_status_of_string_keeps_legacy_unknown_fallback
        ; test_case
            "span_status_opt None for unknown"
            `Quick
