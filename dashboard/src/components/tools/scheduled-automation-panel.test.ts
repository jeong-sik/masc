...
describe('selectWakeSignals', () => {
  ...
  it('orders upcoming wakes soonest-first and reads id, time, and kind verbatim', () => {
  ...
  it('falls back to due_at when next_due_at is absent', () => {
    const signals = selectWakeSignals(
      automation([request({ schedule_id: 's', due_at: 1500 })]),
    )
  ...
  it('drops rows with no concrete wake time', () => {
    const signals = selectWakeSignals(
      automation([request({ schedule_id: 's-nodue' })]),
    )
  ...
  it('excludes terminal statuses', () => {
    const signals = selectWakeSignals(
      automation([
        request({ schedule_id: 's-term', next_due_at: 100, status: 'succeeded' }),
        request({ schedule_id: 's-exp', next_due_at: 200, status: 'expired' }),
        request({
          schedule_id: 's-cancelled',
          next_due_at: 300,
          status: 'cancelled',
        }),
        request({ schedule_id: 's-live', next_due_at: 400 }),
      ]),
    )
  ...
  it('excludes running rows because they already woke', () => {
    const signals = selectWakeSignals(
      automation([
        ...
        request({ schedule_id: 's-live', next_due_at: 200 }),
      ]),
    )
    expect(signals.map(s => s.id)).toEqual(['s-live'])
  })

  it('keeps due rows because they are upcoming wakes', () => {
    const signals = selectWakeSignals(
      automation([
        request({
          schedule_id: 's-due',
    ...
    expect(signals.map(s => s.id)).toEqual(['s-due'])
    expect(signals[0]?.status).toBe('due')
  })
...
describe('ScheduledAutomationPanel wake-signal feed', () => {
  ...
  it('renders an explicit empty state when there are no upcoming wakes', () => {
    render(
      html`<${ScheduledAutomationPanel}
        automation=${automation([
          request({ schedule_id: 's-term', next_due_at: 1000, status: 'succeeded' }),
        ])}
      />`,
      container,
    )
...
describe('ScheduledAutomationPanel', () => {
  ...
  it('renders scheduled requests as read-only cards with a wake signal feed', () => {
    ...
    expect(container.textContent).not.toContain('human required')
    ...
    expect(container.textContent).not.toContain('실행 준비')
  ...
  it('renders explicit live supported non-terminal evidence absence', () => {
    ...
    auto.live_supported_non_terminal_evidence = {
      ...
      criteria: 'payload_support=supported && status not in {succeeded,failed,cancelled,expired}',
  ...
  it('renders matched live supported non-terminal evidence and opens matched rows', async () => {
    ...
    auto.live_supported_non_terminal_evidence = {
      ...
      criteria: 'payload_support=supported && status not in {succeeded,failed,cancelled,expired}',
      reason: 'live schedule_store contains supported non-terminal rows',
      request_count: 1,
  ...
  it('renders a mismatch when matched live evidence contradicts response rows', () => {
    ...
    expect(mismatch?.textContent).toContain('sched-terminal-row:status_terminal')
  ...
  it('uses exact due and terminal status matching', async () => {
  ...
  it('does not render schedule mutation controls', () => {
    render(html`<${ScheduledAutomationPanel} automation=${sampleAutomation()} />`, container)
...
describe('filterMatches', () => {
  it('matches each ScheduleFilterKey against the right request shape', () => {
    ...
    expect(filterMatches('due', request({ schedule_id: 'd', status: 'due' }))).toBe(true)
    expect(filterMatches('running', request({ schedule_id: 'r', status: 'running' }))).toBe(true)
...
describe('ScheduleAside', () => {
  ...
  it('renders read-only pulse + triage buckets and opens details on click', () => {
    ...
    const requests: DashboardScheduledAutomationRequest[] = [
      request({ schedule_id: 'due-first', status: 'due', payload_summary: 'run first' }),
    ...
    render(
      html`<${ScheduleAside}
        requests=${requests}
        sum=${{ scheduled: 0, dueRunning: 2, total: 5 }}
        onOpen=${onOpen}
      />`,
      container,
    )
    ...
    const dueButton = aside?.querySelector('[data-schedule-aside-open="due-first"]') as HTMLElement
    dueButton.click()
    expect(onOpen).toHaveBeenCalledWith('due-first')
  })
  ...
  it('does not classify payload-blocked rows as actionable aside work', () => {
    ...
    const requests: DashboardScheduledAutomationRequest[] = [
      ...
      request({
        schedule_id: 'due-unknown',
        status: 'due',
        payload_support: 'unknown',
        payload_kind: 'keeper.future',
        payload_summary: 'unknown due work',
      }),
    ]
    ...
    render(
      html`<${ScheduleAside}
        requests=${requests}
        sum=${{ scheduled: 0, dueRunning: 3, total: 3 }}
        onOpen=${onOpen}
      />`,
      container,
    )
