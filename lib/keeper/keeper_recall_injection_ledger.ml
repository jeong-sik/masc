(* RFC-0264 P2. See the .mli for the contract. Mirrors the cost-ledger
   ...
   Schema v4 is fact-only and has no compatibility reader. See the .mli. *)
...
let record_wire_fields =
  SS.of_list
    ...
    ; field_trace_id
    ; field_turn
    ; field_added_fact_keys
    ; field_removed_fact_keys
    ; field_content_hash
    ; field_n_facts_in_store
    ; field_ts
    ; field_failure_reason
    ]
;;

let first_unexpected_field fields =
  let rec loop seen = function
    | [] -> None
    | (field, _) :: rest ->
      if SS.mem field seen || not (SS.mem field record_wire_fields)
      then Some field
      else loop (SS.add field seen) rest
  in
  loop SS.empty fields
;;
  ...
  | Ok 4 ->
    (match
     ...
     | ( Ok reset
       ...
       , Ok content_hash ) ->
       Ok { reset; added_fact_keys; removed_fact_keys; content_hash }
     | Error err, _, _, _
     | _, Error err, _, _
     | _, _, Error err, _
...
let record_of_json_result = function
  | `Assoc fields ->
    (match first_unexpected_field fields with
     ...
     | Ok keeper_id, Ok trace_id, Ok turn ->
       (match
        ...
        | _, _, Error err, _
        | _, _, _, Error err -> Error err
        | Ok ts, Ok failure_reason, Ok n_facts_in_store, Ok delta ->
          Ok
            { keeper_id
            ; trace_id
            ; turn
...
let content_hash_of ~fact_keys =
  let canon keys = keys |> SS.of_list |> SS.elements |> String.concat "\x1f" in
  Digest.to_hex (Digest.string (canon fact_keys))
;;
...
let materialize records =
  ...
  let state : (string, SS.t) Hashtbl.t = Hashtbl.create 16 in
  List.map
    (fun record ->
       let previous =
         Option.value
       ...
       let
         ...
         = record.delta
       in
       let base = if reset then SS.empty else previous in
       let facts =
         SS.diff
           (SS.union base (SS.of_list added_fact_keys))
           (SS.of_list removed_fact_keys)
       in
       Hashtbl.replace state record.keeper_id facts;
       { record; fact_keys = SS.elements facts })
    records
;;
...
module Delta_state = struct
  ...
  let table : (string * string, SS.t) Hashtbl.t = Hashtbl.create 64
  ...
  let commit ~masc_root ~keeper_id ~facts =
    Stdlib.Mutex.protect mu (fun () ->
      Hashtbl.replace table (masc_root, keeper_id) facts)
  ;;
...
let append
  ...
  let added_fact_keys = SS.elements (SS.diff curr_facts previous) in
  let removed_fact_keys = SS.elements (SS.diff previous curr_facts) in
  let content_hash = content_hash_of ~fact_keys:injected_fact_keys in
  let entry =
    to_json_delta
      ?failure_reason
      ~reset
  ...
  try
    ...
    Delta_state.commit ~masc_root ~keeper_id ~facts:curr_facts
  with
