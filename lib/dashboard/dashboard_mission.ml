...
let _build_session_context session_json _cards =
  ...
  then None
    ...
    in
    let status =
      Dashboard_utils.session_lifecycle_of_string (session_status_string session_json)
    in
    let is_terminal =
      match status with
    ...
    let blocker_summary =
      ...
      then
        ...
        | Some attention -> trim_to_option (string_field "summary" attention)
        | None ->
          if
...
let rec evidence_preview_strings json =
  ...
  | `Assoc fields ->
    fields
    |> List.map snd
    |> List.concat_map evidence_preview_strings
    |> dedup_strings
    |> take 4
  | _ -> []
;;
  ...
  let by_actor =
    ...
    | Some actor_name ->
      ...
      |> List.filter_map (fun (session : session_context) ->
        if List.mem actor_name session.member_names then Some session.session_id else None)
  in
...
let build_attention_queue incidents actions sessions =
  let public_incidents =
    incidents |> List.filter (fun incident -> not (is_internal_attention incident))
  in
  public_incidents
  |> List.filter_map (fun incident ->
    ...
    then None
      ...
      Some
        ...
        ; last_seen_ts = parse_iso_opt last_seen_at |> Option.value ~default:0.0
        ; related_session_ids
        ; related_agent_names
        ; json =
            `Assoc
              ...
              ; ( "related_session_ids"
                , `List (List.map (fun value -> `String value) related_session_ids) )
              ; ( "related_agent_names"
                , `List (List.map (fun value -> `String value) related_agent_names) )
              ; ( "evidence_preview"
                , `List
                    (List.map
                       (fun value -> `String value)
                       (evidence_preview_strings (member_assoc "evidence" incident))) )
...
let _build_briefs_from_sessions sessions attention_queue actions =
  ...
  |> List.map (fun (session : session_context) ->
    ...
    let top_attention_json =
      ...
      | attention :: _ ->
        Some
          (member_assoc "severity" attention.json |> ignore;
           attention.json)
    ...
    , `Assoc
        ...
        ; ( "member_names"
          , `List (List.map (fun value -> `String value) session.member_names) )
        ...
        ; ( "top_recommendation"
          , option_to_json (fun value -> value) top_recommendation_json )
        ] ))
  |> List.sort
       (fun (left_sev, left_count, left_ts, _) (right_sev, right_count, right_ts, _) ->
...
let build_projection ?actor ~config ~sw ~clock ~proc_mgr () =
  ...
  in
  let agent_briefs =
    Dashboard_mission_assembly.build_agent_briefs
      config
      sessions
      attention_queue
      namespace_json
  ...
  in
  let keeper_briefs =
    Dashboard_mission_assembly.build_keeper_briefs config keeper_items
  in
  let internal_signals =
    Dashboard_mission_assembly.build_internal_signals incidents recommended_actions
  in
...
let json ?actor ~config ~sw ~clock ~proc_mgr () =
  ...
  let summary_json =
    `Assoc
      [ ( "room_health"
        , `String (string_field ~default:"ok" "health" projection.digest_json) )
      ; ( "cluster"
        , json_string_option (Some (string_field "cluster" projection.namespace_json)) )
      ; ( "project"
        , json_string_option (Some (string_field "project" projection.namespace_json)) )
      ]
  in
  ...
  `Assoc
    ...
    ; ( "attention_queue"
      , `List
          (List.map
             (fun (item : attention_context) -> item.json)
             projection.attention_queue) )
...
let session_json ?actor ~session_id ~config ~sw ~clock ~proc_mgr () =
  ...
  let operations_json =
    ...
    | None -> []
    | Some session ->
      Dashboard_mission_assembly.operation_badges_for_session session operation_contexts
  in
  let keepers_json =
    match session_context with
    | None -> []
    | Some session ->
      Dashboard_mission_assembly.keeper_refs_for_session
  ...
  let participants_json =
    ...
    | None -> []
    | Some session ->
      Dashboard_mission_assembly.participant_preview_json
        session.session_id
        session.member_names
        projection.agent_briefs
  in
