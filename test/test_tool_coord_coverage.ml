  ...
  test
    ...
    (fun () ->
       ...
       | Some (success, result) ->
         ...
         assert (str_contains result "owned=task-001 | current=task-001");
         assert (
           str_contains result "📝 Planning: deliverable_conflict=yes | task=task-001");
         assert (
           str_contains
             result
...
let () =
  test "dispatch_status_flags_todo_with_completed_deliverable_as_conflict" (fun () ->
    ...
    | Some (success, result) ->
      assert success;
      assert (
        str_contains result "⚠️ task-001 P2 [todo_conflict] Conflicted todo (unclaimed)");
      assert (str_contains result "📋 task-002 P2 [todo] Fresh todo (unclaimed)");
      assert (
        str_contains result "1 todo task(s) have completed-looking planning deliverables")
    | None -> failwith "dispatch returned None")
;;
...
let () =
  test "next_hint_cancelled is empty (terminal)" (fun () ->
    let h =
      next_hint
        (Types.Cancelled { cancelled_by = "a"; cancelled_at = "t"; reason = None })
    in
    assert (h = ""))
;;
