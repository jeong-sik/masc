...
let record_accountability ~config ~(updated_meta : Keeper_meta_contract.keeper_meta) ~social_state ~result claim =
...
let persist_success_meta ~config ~(original_meta : Keeper_meta_contract.keeper_meta) ~(updated_meta : Keeper_meta_contract.keeper_meta) =
  let updated_meta =
...
let reset_turn_failures_for_stop_reason ~config ~(updated_meta : Keeper_meta_contract.keeper_meta) result =
  match result.Keeper_agent_run.stop_reason with
