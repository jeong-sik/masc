...
- Keeper: an official client's start seed begins where the Librarian read, like the Agent Core lane's does. Claude Code and Antigravity carry the saved working state in place of the atoms it summarises, and their `model input carried range` line reports `origin=librarian_snapshot`. The range still starts at the latest of the three positions — the Librarian's, the lane's own cut, and the range the last completed turn carried — so it never moves back, and it always carries the newest atom, so a history the Librarian read to the end arrives with the turn it has to answer beside the summary. The request can still grow by the working state, which no cut may drop: on Antigravity, whose declared window would refuse such a request outright, the range is composed once more without the Librarian front. A saved position that does not describe the messages the lane is about to send is not used; the seed is, because it carries its own digest of the atom it names and is checked against the history separately.
