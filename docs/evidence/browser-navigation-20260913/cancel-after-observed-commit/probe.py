...
(async()=>{
 for(const name of ['closed-tab','read-deadline']){
  ...
  try{
   ...
   const sourceFrameBeforeFollow=await browser.webNavigation.getFrame({tabId:tab.id,frameId:0});
   ...
   const rejected=await read;const nativeEventsAtReadSettlement=[...nativeEvents];if(rejected.answer.ok)throw new Error('pending read unexpectedly succeeded');
   ...
   if(name==='read-deadline'){
    ...
    let commitListener;
    const lateCommit=new Promise(resolve=>{
     commitListener=d=>{if(d.tabId===tab.id&&d.frameId===0&&d.url===fixtureBase+'/pending'&&d.documentId!==sourceFrame.documentId){browser.webNavigation.onCommitted.removeListener(commitListener);resolve(d);}};
     browser.webNavigation.onCommitted.addListener(commitListener);
    });
    let committed;
    try{await fetch(fixtureBase+'/release');committed=await lateCommit;}
    finally{browser.webNavigation.onCommitted.removeListener(commitListener);}
    await emit({kind:'late-commit',committed,old_read_reply_count:rejected.replies.length,nativeEvents:[...nativeEvents],scope:'external wait after cancellation for verification only'});
    if(rejected.replies.length!==1)throw new Error('expired reply revived before fresh read');
    ...
    if(finalFrame.documentId!==committed.documentId)throw new Error('fresh native document differs');
    if(!fresh.answer.data.nodes.some(n=>n.tag==='h1'&&n.text==='Observed /pending'))throw new Error('fresh heading absent');
    if(rejected.replies.length!==1)throw new Error('expired reply revived');
    await emit({kind:'fresh-read',answer:fresh.answer,old_read_reply_count:rejected.replies.length,finalFrame});await browser.tabs.remove(tab.id);
   }
   await emit({kind:'case',name,sourceFrameBeforeFollow,follow:follow.answer,rejected:rejected.answer,nativeEventsAtReadSettlement,nativeEvents});
