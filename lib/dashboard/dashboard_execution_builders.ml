...
let worker_state_of_agent
  ...
  let last_signal_at =
    if last_signal_ts <= 0.0
    then None
    else if last_message_ts >= last_seen_ts
    then (
      match message_opt with
      | Some (_, message) -> trim_to_option message.timestamp
      | None -> trim_to_option agent.last_seen)
    else trim_to_option agent.last_seen
  in
  let signal_age_s =
    if last_signal_ts > 0.0 then max 0.0 (now_ts -. last_signal_ts) else infinity
  in
  let signal_truth =
    if last_signal_ts <= 0.0
    then "absent"
    else if signal_age_s <= signal_live_sec
    then "live"
    else "stale"
  in
  let evidence_source =
    if last_message_ts > 0.0 && last_message_ts >= last_seen_ts
    then "message"
    else if last_seen_ts > 0.0
    then "presence"
    else "none"
  in
  ...
  let state, tone, note =
    ...
    | Types.Busy | Types.Active | Types.Listening ->
      if signal_age_s > signal_stale_sec
      then
        ( "quiet"
        , Tone_bad
      ...
      then
        if signal_age_s > signal_quiet_sec
        then "quiet", Tone_warn, "Execution looks quiet for too long"
        else "working", Tone_ok, "Task and live signal aligned"
      else if signal_age_s > signal_quiet_sec
      then "quiet", Tone_warn, "Quiet but still reachable"
      else "watching", Tone_ok, "Standing by for the next task"
  in
  let focus =
    ...
    | None ->
      ...
      then
        Printf.sprintf
          "%d claimed tasks waiting for explicit current_task"
          active_task_count
      else Option.value ~default:"Idle / waiting for assignment" recent_output_preview
  in
  ...
  ; json =
      `Assoc
        ...
        ; ( "last_signal_age_sec"
          , if Float.is_finite signal_age_s
            then `Int (int_of_float signal_age_s)
            else `Null )
...
let continuity_row_of_keeper ~(now_ts : float) ?related_session_id keeper
  ...
  let context_ratio =
    ...
    | _ -> None
  in
  let last_action_at = trim_to_option (string_field "last_autonomous_action_at" keeper) in
  ...
  let last_action_age_s =
    if last_action_ts > 0.0 then max 0.0 (now_ts -. last_action_ts) else infinity
  in
  let effective_activity_age_s =
    if last_action_ts > 0.0
    then last_action_age_s
    else if last_signal_ts > 0.0
    then max 0.0 (now_ts -. last_signal_ts)
    else infinity
  in
  ...
  let lifecycle =
    if is_keeper_offline status
    then Lc_offline
    else if Option.value ~default:0.0 context_ratio >= ctx_handoff_imminent
    then Lc_handoff_imminent
    else if Option.value ~default:0.0 context_ratio >= ctx_preparing
    then Lc_preparing
    else if Option.value ~default:0.0 context_ratio >= ctx_compacting
    then Lc_compacting
    else if last_action_ts > 0.0
    then Lc_active
    else if last_signal_ts > 0.0
    then Lc_idle
    else Lc_idle
  ...
  let state, tone, note =
    if is_keeper_offline status
    then Exec_critical, Tone_bad, "keeper 오프라인"
    else (
      match lifecycle with
      | Lc_handoff_imminent -> Exec_critical, Tone_bad, "핸드오프 임박"
      | Lc_preparing | Lc_compacting -> Exec_warning, Tone_warn, "연속성 압력이 높습니다"
      | Lc_offline | Lc_active | Lc_idle ->
        if autonomous_turn_count = 0 && turn_count > 0
        then Exec_warning, Tone_warn, Printf.sprintf "자율 턴 없음 (턴 %d회 수행)" turn_count
        else if effective_activity_age_s >= keeper_action_stale_sec
        then
          ( Exec_warning
          , Tone_warn
          , Printf.sprintf "마지막 활동 %.0f시간 전" (effective_activity_age_s /. 3600.0) )
        else Exec_healthy, Tone_ok, "정상 동작 중")
  in
  let continuity =
    Printf.sprintf
      "Gen %d · Turns %d · Auto turns %d · Tool actions %d · Goals %d"
      generation
      turn_count
  ...
  let recent_output_preview =
    ...
    |> option_or_else (fun () ->
      trim_to_option (string_field "last_proactive_preview" keeper))
  in
      ...
      `Assoc
        ([ "name", `String name
         ...
         @ [ ( "latest_tool_call_count"
             , option_to_json (fun value -> `Int value) audit.latest_tool_call_count )
             ...
             , `String
                 (match trim_to_option (string_field "continuity_summary" keeper) with
                  ...
                  | None ->
                    if autonomous_turn_count = 0 && turn_count = 0
                    then "아직 활동 기록이 없습니다"
                    else if autonomous_turn_count = 0
                    then Printf.sprintf "대기 중 (턴 %d회, 자율 턴 0회)" turn_count
                    else (
                      let noop_note =
                        if noop_turn_count > 0
                        then Printf.sprintf " · noop %d회" noop_turn_count
                        else ""
                      in
                      Printf.sprintf
                        "자율 턴 %d회, 도구 행동 %d회, 턴 %d회, 세대 %d%s"
                        autonomous_turn_count
                        autonomous_action_count
                        turn_count
                        generation
           ...
           ; "korean_name", `String korean_name
           ; ( "skill_reason"
             , json_string_option (trim_to_option (string_field "goal" keeper)) )
           ])
  }
;;
...
let build_worker_support_briefs
      ~(now_ts : float)
      ~(tasks : Types.task list)
  ...
  |> List.map (fun (agent : Types.agent) ->
    let related = related_session_for_member session_contexts agent.name in
...
let build_continuity_briefs ~(now_ts : float) keepers session_contexts
  ...
  |> List.filter_map (fun keeper ->
    ...
    then None
      ...
      let related_session = related_session_for_member session_contexts name in
      let related_session_id =
        ...
        | None ->
          (match trim_to_option (string_field "agent_name" keeper) with
           ...
           | None -> None)
      in
      let row = continuity_row_of_keeper ~now_ts ?related_session_id keeper in
      Some row))
