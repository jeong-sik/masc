...
let test_valid_sha256 () =
  Alcotest.(check bool)
    "exact 64 hex"
    true
    (A.is_valid_sha256 "abc1234567890123456789012345678901234567890123456789012345678901");
  Alcotest.(check bool)
    "uppercase hex"
    true
    (A.is_valid_sha256 "ABC1234567890123456789012345678901234567890123456789012345678901");
  Alcotest.(check bool)
    "63 chars"
    false
    (A.is_valid_sha256 "abc123456789012345678901234567890123456789012345678901234567890");
  Alcotest.(check bool)
    "65 chars"
    false
    (A.is_valid_sha256
  ...
  Alcotest.(check bool)
    "non-hex char"
    false
    (A.is_valid_sha256 "ghi1234567890123456789012345678901234567890123456789012345678901");
...
let test_unavailable_when_no_base_path () =
  ...
  let json, status = A.blob_response ~sha256:(String.make 64 '0') in
...
let test_not_found () =
  with_temp_base_path (fun _dir ->
    let json, status = A.blob_response ~sha256:(String.make 64 'a') in
  ...
  with_temp_base_path (fun dir ->
    ...
    | O.Stored { sha256; _ } ->
      ...
      (match Yojson.Safe.Util.member "bytes" json with
       | `Int n -> Alcotest.(check int) "byte count" (String.length payload) n
...
let () =
  Alcotest.run
    ...
    ; ( "blob_response"
      , [ Alcotest.test_case
            "unavailable when MASC_BASE_PATH unset"
            `Quick
            test_unavailable_when_no_base_path
        ; Alcotest.test_case "not found" `Quick test_not_found
        ; Alcotest.test_case "hit returns envelope" `Quick test_hit_returns_envelope
        ] )
    ]
;;
