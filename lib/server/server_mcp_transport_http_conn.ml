...
let sse_connect_guard_by_session : sse_connect_guard_state SMap.t Atomic.t =
...
let env_float_or ~name ~default =
  ...
  | None -> default
  | Some raw -> Option.value ~default (float_of_string_opt raw)
;;

let env_int_or ~name ~default =
  match Sys.getenv_opt name with
  | None -> default
  | Some raw -> Option.value ~default (int_of_string_opt raw)
;;
...
let sse_connect_window_s = env_float_or ~name:"MASC_SSE_CONNECT_WINDOW_S" ~default:60.0
...
let guard_deadline state =
  let session_deadline =
    if sse_reconnect_min_interval_s <= 0.0 || state.last_connect_at < 0.0
    then neg_infinity
    else state.last_connect_at +. sse_reconnect_min_interval_s
  in
  let window_deadline =
    if sse_connect_window_s <= 0.0
    then neg_infinity
    else (
...
let guard_expired ~now ~session_id state =
  (not (SMap.mem session_id (Atomic.get sse_conn_by_session)))
  && now >= guard_deadline state
;;
...
let close_sse_conn info =
  ...
  then (
    ...
    (try Httpun.Body.Writer.close info.writer with
     ...
     | exn -> Log.Misc.debug "close_sse_conn: %s" (Printexc.to_string exn));
...
let stop_sse_session_impl ~clear_guard session_id =
  ...
  if clear_guard
  then atomic_update sse_connect_guard_by_session (fun map -> SMap.remove session_id map);
  match !info_opt with
  | None -> ()
  | Some info -> close_sse_conn info
;;

let stop_sse_session session_id = stop_sse_session_impl ~clear_guard:true session_id
...
let active_session_count () = SMap.cardinal (Atomic.get sse_conn_by_session)
...
let reap_stale_guards () =
  ...
  let stale =
    SMap.fold
      (fun sid state acc ->
         if guard_expired ~now ~session_id:sid state then sid :: acc else acc)
...
let close_all_sse_connections () =
  let infos = SMap.fold (fun _k v acc -> v :: acc) (Atomic.get sse_conn_by_session) [] in
  ...
  Log.Server.info "MASC MCP: Closed %d SSE connections" (List.length infos)
;;
  ...
  atomic_update sse_connect_guard_by_session (fun map ->
    let state =
      ...
      | Some v ->
        ...
        if guard_expired ~now ~session_id v'
        then { last_connect_at = -1.0; connect_times = [] }
        else v'
      | None -> { last_connect_at = -1.0; connect_times = [] }
    in
    let recent = state.connect_times in
    let session_wait_s =
      if sse_reconnect_min_interval_s <= 0.0
      then 0.0
      else sse_reconnect_min_interval_s -. (now -. state.last_connect_at)
    in
    if session_wait_s > 0.0
    then (
      result := Error ("session_cooldown", session_wait_s);
      map)
    else (
      let window_wait_s =
        if sse_connect_window_s <= 0.0 || sse_connect_max_in_window <= 0
        then 0.0
        else if List.length recent >= sse_connect_max_in_window
        then (
