...
**`Masc_time_constants` 을 별도 leaf library `masc_mcp.time_constants` 로 분리.**
...
| 파일 | 동작 |
|------|------|
| `lib/time_constants/dune` (NEW) | `(library (name masc_time_constants) (public_name masc_mcp.time_constants) (wrapped false))` |
...
| `lib/config/dune` (EDIT) | `(modules ...)` 에서 `masc_time_constants` 제거 + `(libraries ...)` 에 `masc_mcp.time_constants` 추가 |
| `lib/masc_log/dune` (EDIT) | `(libraries ...)` 에 `masc_mcp.time_constants` 추가 |
| `lib/core/dune` (EDIT) | `(libraries ...)` 에 `masc_mcp.time_constants` 추가 |
| `lib/briefing_compactors/dune` (EDIT) | `(libraries ...)` 에 `masc_mcp.time_constants` 추가 |
...
2. `lib/config/dune` 의 `modules` 에서 `masc_time_constants` 제거 + `libraries` 에 `masc_mcp.time_constants` 추가
...
- Library 이름: `masc_mcp.time_constants` (descriptive, 길음) vs `masc_mcp.time` (짧음, 향후 확장 base) vs `masc_mcp.consts` (가장 generic).  본 RFC 는 `time_constants` 권장 — *time-specific scope* 명시가 library 책임 명확.
