...
**Keeper**
자율적으로 동작하는 장기 실행 에이전트. TOML 프로필에서 persona, instructions와 typed policy를 로드하고, Workspace에 참여하여 Heartbeat를 유지하며, typed event와 tool surface를 통해 Board/Task/Connector 작업을 수행한다. 각 Keeper는 독립 lane과 OAS checkpoint를 가진다. `-> lib/keeper/keeper_types.ml`, `-> lib/keeper/`
...
**Keeper Meta**
Keeper의 전체 설정과 런타임 상태를 담는 레코드 타입. identity, model 설정, policy, initiative, compaction, handoff, voice, token 사용량 등을 포함한다. `-> lib/keeper/keeper_types.ml`
...
MASC 자체 `lib/context_budget_manager.mli` 모듈은 purge됐다. 현재 provider context budget/compression은 OAS Context_reducer가 소유한다.

**Compression Phase** (retired)
세션 레벨 4단계 압축(`None_phase`/`Compact_tools`/`Drop_low`/`Summarize`)과 MASC의 importance scorer/reducer bridge는 제거됐다. OAS reducer가 provider context-window 처리를 소유한다.

**Context Compact OAS**
OAS가 Agent 내부 context-window compaction과 overflow retry를 직접 소유한다. MASC checkpoint persistence는 원문 message/tool result를 보존하며, 선택적 post-turn 요약은 configured LLM의 유효한 plan만 적용한다.
...
이전 Team Session 에이전트의 실행 권한: `Observe_only` (관찰만), `Limited_code_change` (제한적 코드 수정), `Autonomous` (자율 실행). 현재 Keeper는 descriptor가 제공하는 도구 표면을 사용하고, 외부 효과의 실행 여부는 비계층 Gate에서 판정한다.
...
**Gate**
외부 효과 요청을 제품 의미와 분리하여 판정하는 비계층 경계. Gate는 도구 이름, 명령 의미, 제품 종류, 위험 등급을 알지 않으며 typed input, path jail, sandbox confinement처럼 실행기가 증명할 수 있는 불변식만 확인한다. 판정 순서는 exact Always Allowed, configured LLM Auto Judge, non-blocking HITL이다. 판정 저장소를 사용할 수 없으면 효과를 실행하지 않고 `Unavailable`을 명시적으로 반환하되 Keeper의 lane은 계속 동작한다. `-> lib/keeper/keeper_gate.mli`

**Always Allowed**
operator가 승인한 정확한 효과 identity와 canonical input fingerprint에만 일치하는 지속 규칙. 부분 문자열, 도구군, 위험 등급, 숫자 임계값으로 범위를 확대하지 않는다.

**Auto Judge**
주입된 세계관과 정책을 가진 configured LLM이 전체 효과 요청을 직접 판정하는 Gate 모드. 로컬 문자열 분류기나 risk taxonomy가 판정을 대행하지 않는다.

**HITL**
판정이 필요한 외부 효과를 durable queue에 저장하고 현재 Keeper를 막지 않는 사람 검증 경계. 결정되면 해당 Keeper의 lane만 깨우며, grant는 정확한 요청에 한 번만 소비된다.
...
**Pre-hook**
도구 핸들러 실행 전 호출되는 가로채기 함수. `Pass`는 진행, `Proceed args`는 정규화/강제 변환 후 진행, `Reject result`는 핸들러를 건너뛰고 단축 반환한다. typed input validation 같은 구조적 검증이 pre-hook으로 설치될 수 있다. 외부 효과의 주관적 허용 여부는 Keeper Gate가 별도로 판정한다. `-> lib/tool/tool_dispatch.mli`
