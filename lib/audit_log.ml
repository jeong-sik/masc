...
let outcome_to_json = function
  ...
  | Failure reason -> `Assoc [ "status", `String "failure"; "reason", `String reason ]
;;
...
let entry_to_json (e : audit_entry) : Yojson.Safe.t =
  ...
  in
  let with_cost =
    match e.cost_estimate with
    | Some c -> base @ [ "cost_estimate", `Float c ]
    | None -> base
  in
  let with_tokens =
    match e.token_count with
    ...
    let redacted =
      ...
      | `Assoc fields ->
        let safe_fields =
          List.filter_map
            (fun (k, v) ->
               if k = "details" then Some (k, `String "<redacted>") else Some (k, v))
            fields
        in
        `Assoc safe_fields
      | _ -> `String "<non-object>"
  ...
  List.iter
    (fun json ->
       ...
       | Error reason ->
         ...
         then Log.Misc.error "audit_log: corrupt entry (#%d): %s" !err_count reason)
    jsons;
  if !err_count > 0
  then
    Log.Misc.error
      "audit_log: %d/%d entries failed to parse (possible corruption)%s"
      !err_count
      (List.length jsons)
      (if !err_count > max_logged_errors
      ...
      let jsons =
        ...
        |> List.filter_map (fun line ->
          ...
          | exception Yojson.Json_error msg ->
            ...
            then
              Log.Misc.warn
                "audit_log: invalid JSON line (#%d): %s | line: %s"
                !invalid_count
                msg
                (preview ~max_len:100 line);
            None)
      in
      if !invalid_count > 0
      then
        Log.Misc.warn
          "audit_log: %d invalid JSON line(s) in legacy audit log%s"
          !invalid_count
          (if !invalid_count > max_logged_errors
...
let log_join config ~agent_id ?cost_estimate ?token_count () =
  log_action config ~agent_id ~action:Join ?cost_estimate ?token_count ~outcome:Success ()
;;
...
let log_cancel_task config ~agent_id ~task_id ~reason ?cost_estimate ?token_count () =
  log_action
    ...
    ~agent_id
    ~action:CancelTask
    ~details:(`Assoc [ "task_id", `String task_id; "reason", `String reason ])
    ?cost_estimate
    ?token_count
    ~outcome:Success
    ()
...
let log_system_internal_tool_call
      config
      ~agent_id
      ~tool_name
      ~success
  ...
  log_action
    config
    ~agent_id
    ~action:(Custom "system_internal_tool_call")
    ~details
    ?cost_estimate
    ?token_count
    ?trace_id
...
let log_client_tool_host_failure
      config
      ~agent_id
      ~client_name
      ~tool_name
  ...
  log_action
    config
    ~agent_id
    ~action:(Custom "client_tool_host_failure")
    ~details
    ?trace_id
...
let prune_old (config : config) ~days =
  ...
  if deleted > 0 then Log.Misc.info "Pruned %d old audit day-files" deleted
;;
