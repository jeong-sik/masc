let usage () =
  prerr_endline
    "usage: dune exec ./bin/cascade_materialize.exe -- [path/to/cascade.json]";
  exit 2
...
let () =
  ...
  match
    Masc_mcp.Cascade_toml_materializer.ensure_materialized_json ~config_path
  with
  | Error msg ->
      prerr_endline msg;
      exit 1
  | Ok { source; wrote_json } ->
      Printf.printf
        ...
        source.source_path source.json_path wrote_json
