...
let save_interests_to_neo4j ~agent_name ~persona_name interests =
  if interests = [] then Ok "no interests to save"
    ...
    let cypher = Printf.sprintf
      "MERGE (a:LodgeAgent {name: '%s', persona: '%s'}) \
       WITH a \
       UNWIND [%s] AS topic \
...
let get_agent_interests ~agent_name =
  let cypher = Printf.sprintf
    "MATCH (a:LodgeAgent {name: '%s'})-[r:INTERESTED_IN]->(t:Topic) \
...
let react_to_content ~net ?persona content =
  ...
  let agent_name = Printf.sprintf "lodge-%s" persona_name in
  ...
  match ollama_generate ~net ~temperature:0.8 ~num_predict:250 ~system
      ...
      | Review ->
        ...
        | Ok reaction ->
          let author_name = match persona with
            | Some p -> Printf.sprintf "lodge-%s" (string_of_persona p)
            | None -> "lodge-agent"
          in
...
(** {1 Full cycle: heartbeat + classify + react with Eio concurrency} *)
...
let full_cycle ~net (args : Yojson.Safe.t) =
  ...
  if not ok then (false, msg)
    ...
    if post_id = "" then (true, Printf.sprintf "%s\n(no post_id found for reaction)" msg)
      ...
      (* Step 2: React to own post (simulating another agent) *)
      let (ok2, msg2) = react ~net (`Assoc [("post_id", `String post_id)]) in
      let status = if ok2 then "✅" else "⚠️" in
      (true, Printf.sprintf "%s\n%s Reaction: %s" msg status msg2)
...
let get_all_agent_interests () =
  let cypher =
    "MATCH (a:LodgeAgent)-[r:INTERESTED_IN]->(t:Topic) \
...
let evolve ~net:_ (args : Yojson.Safe.t) =
  ...
  let all_agents = List.map (fun p -> Printf.sprintf "lodge-%s" (string_of_persona p)) all_personas in
  let agents_to_check = if persona_str = "" || persona_str = "all" then all_agents
    else [Printf.sprintf "lodge-%s" persona_str]
  in
  (* Single query for all agents *)
  let agent_data = get_all_agent_interests () in
  let debug_info = Printf.sprintf "[DEBUG] agent_data count: %d, agents: %s"
    (List.length agent_data)
    (String.concat ", " (List.map fst agent_data))
  in
  let results = List.map (fun agent_name ->
    match List.find_opt (fun (a, _) -> a = agent_name) agent_data with
    | None -> Printf.sprintf "🌱 %s: 아직 관심사 없음 (첫 방문을 기다리는 중)" agent_name
    | Some (_, topics) ->
      if topics = [] then
        Printf.sprintf "🌱 %s: 아직 관심사 없음 (첫 방문을 기다리는 중)" agent_name
      else
        Printf.sprintf "🌳 %s:\n   관심사: %s" agent_name (String.concat ", " topics)
  ) agents_to_check in
  (true, debug_info ^ "\n\n" ^ String.concat "\n\n" results)
