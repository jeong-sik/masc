...
(** {1 Tool-affinity aggregate}

    Per-keeper counts materialized at append time so
    [Keeper_tool_affinity.pre_populate_from_history] can pre-populate tools
    without rescanning the full trajectory on every turn (the "server
    freeze" P0). Counts are bucketed by UTC day to preserve the affinity
    read's rolling lookback window; only the fields
    {!Keeper_tool_affinity.compute_affinity} scores on are tracked. *)

type tool_day_bucket = {
  bucket_day : int;
  bucket_call_count : int;
  bucket_success_count : int;
  bucket_last_ts : float;
  bucket_last_iso : string;
}
...
(** Distinguishes an absent snapshot (expected first-time / cold state)
    from a corrupt or unreadable one, so callers rebuild rather than
    silently treating either as an empty aggregate. *)
type aggregate_load_error =
  | Aggregate_missing
  | Aggregate_unreadable of string
  | Aggregate_corrupt of string

val aggregate_snapshot_path : string -> string -> string
(** [aggregate_snapshot_path masc_root keeper_name] is the snapshot file
    path. Ends in [.json] (not [.jsonl]) so the trace scanners skip it. *)

val read_aggregate_snapshot :
...
(** Increment an already-seeded snapshot with [entries] (called on flush
    with the just-persisted batch). Never raises. No-op when the snapshot
    is missing/corrupt — seeding and healing are done by
    {!rebuild_tool_affinity_aggregate} via the affinity read path, so a
    partial is never written over a full or absent snapshot. *)

val build_tool_affinity_aggregate :
  keeper_name:string -> now:float -> tool_call_entry list ->
  tool_affinity_aggregate
(** Pure builder: fold [entries] into a fresh aggregate and prune buckets
    older than the retention horizon (relative to [now]). *)

val windowed_affinity_tool_stats :
  tool_affinity_aggregate -> since:float -> tool_stat list
(** Project the aggregate to per-tool {!tool_stat}s over [ts >= since].
    Equivalent to [aggregate_tool_stats (read_entries_since ~since)] for
    the call_count / success_count / last_used_at fields; latency and cost
    are 0 (not tracked). The window boundary is day-aligned. *)
