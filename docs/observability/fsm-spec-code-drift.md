...
**Sub-axis label drift (one state)**: TLA+ `KeeperTurnFSM.tla` uses the short string `"awaiting_tool"` for the `Awaiting_tool_result` state in `TurnStateSet`/`ActiveStateSet`/`Next` (`specs/keeper-turn-fsm/KeeperTurnFSM.tla:32, 69, 81, 170, 175`); OCaml `turn_state_label` emits `"awaiting_tool_result"` (`lib/keeper/keeper_turn_fsm.ml:58`). The TLA+ comment at line 32 documents the mapping explicitly. Keeping the short form in the spec preserves the verifier's existing model checks; renaming would force a re-verify of the clean / buggy spec pair (#11190 + #11198 + #11199 + #11225). Operators reading `bin/masc-trace` / Grafana / Prometheus see only the long form because every emit and label flows through `turn_state_label`. Tooling that bridges the two surfaces (e.g. a future trace ↔ TLA+ counterexample replayer) must apply this single-state rename inline.
