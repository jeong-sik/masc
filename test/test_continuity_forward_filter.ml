...
let test_strips_backward () =
  ...
  Alcotest.(check bool)
    "Done removed"
    false
    (String.length filtered >= 5 && Astring.String.is_infix ~affix:"Done:" filtered);
  Alcotest.(check bool)
    "Decisions removed"
    false
    (Astring.String.is_infix ~affix:"Decisions:" filtered);
  Alcotest.(check bool)
    "Progress removed (would be if present)"
    false
    (Astring.String.is_infix ~affix:"Progress:" filtered)
;;
...
let test_keeps_forward () =
  let filtered =
    Masc_mcp.Keeper_memory_policy.filter_forward_looking_summary full_summary
  in
  Alcotest.(check bool) "Goal kept" true (Astring.String.is_infix ~affix:"Goal:" filtered);
  Alcotest.(check bool)
    "Next plan kept"
    true
    (Astring.String.is_infix ~affix:"Next plan:" filtered);
  Alcotest.(check bool) "Next kept" true (Astring.String.is_infix ~affix:"Next:" filtered);
  Alcotest.(check bool)
    "OpenQuestions kept"
    true
    (Astring.String.is_infix ~affix:"OpenQuestions:" filtered);
  Alcotest.(check bool)
    "Constraints kept"
    true
    (Astring.String.is_infix ~affix:"Constraints:" filtered)
;;
...
let test_preserves_line_order () =
  ...
  | Some gi, Some npi -> Alcotest.(check bool) "Goal precedes Next plan" true (gi < npi)
  | _ -> Alcotest.fail "Expected Goal and Next plan lines to be present"
;;

let test_drops_inert_idle_directives () =
  let filtered =
    Masc_mcp.Keeper_memory_policy.filter_forward_looking_summary inert_idle_summary
  in
  Alcotest.(check bool)
    "idle next plan removed"
    false
    (Astring.String.is_infix ~affix:"stay silent" filtered);
  Alcotest.(check bool)
    "idle next removed"
    false
    (Astring.String.is_infix ~affix:"대기 유지" filtered);
  Alcotest.(check bool)
    "goal still kept"
    true
    (Astring.String.is_infix ~affix:"Goal:" filtered);
  Alcotest.(check bool)
    "constraints still kept"
    true
    (Astring.String.is_infix ~affix:"Constraints:" filtered)
;;
...
let test_drops_stale_tool_surface_claims () =
  ...
  in
  Alcotest.(check bool)
    "stale masc-only claim removed"
    false
    (Astring.String.is_infix ~affix:"masc_* only" filtered);
  Alcotest.(check bool)
    "stale missing keeper tools removed"
    false
    (Astring.String.is_infix ~affix:"no keeper_* tools" filtered);
  Alcotest.(check bool)
    "goal still kept"
    true
    (Astring.String.is_infix ~affix:"Goal:" filtered);
  Alcotest.(check bool)
    "live-policy action still kept"
    true
    (Astring.String.is_infix ~affix:"keeper_task_claim" filtered)
;;

let () =
  Alcotest.run
    "continuity forward filter"
    [ ( "filter_forward_looking_summary"
      , [ Alcotest.test_case "strips backward-looking fields" `Quick test_strips_backward
        ; Alcotest.test_case "keeps forward-looking fields" `Quick test_keeps_forward
        ; Alcotest.test_case "empty / all-backward → empty" `Quick test_empty_input
        ; Alcotest.test_case
            "preserves relative line order"
            `Quick
            test_preserves_line_order
        ; Alcotest.test_case
            "drops inert idle directives"
            `Quick
            test_drops_inert_idle_directives
        ; Alcotest.test_case
            "drops stale tool surface claims"
            `Quick
