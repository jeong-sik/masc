...
let make_schema name : Oas.Types.tool_schema = { name; description = ""; parameters = [] }
...
let test_denylist_maps_to_denylist () =
  let gate =
    make_gate ~allowlist_enabled:false ~denied:[ "keeper_bash"; "keeper_fs_edit" ] ()
  in
  let g = Verifier_oas.eval_gate_to_oas_guardrails gate in
  Alcotest.(check bool)
    "denied only -> DenyList"
    true
    (match g.tool_filter with
...
let test_neither_maps_to_allowall () =
  let gate = make_gate ~allowlist_enabled:false ~allowed:[] ~denied:[] () in
  let g = Verifier_oas.eval_gate_to_oas_guardrails gate in
  Alcotest.(check bool)
    "neither -> AllowAll"
    true
...
let test_empty_allowlist_maps_to_empty_allowlist () =
  let gate = make_gate ~allowlist_enabled:true ~allowed:[] () in
  let g = Verifier_oas.eval_gate_to_oas_guardrails gate in
  Alcotest.(check bool)
...
let test_max_tool_calls_preserved () =
  ...
  let g = Verifier_oas.eval_gate_to_oas_guardrails gate in
  Alcotest.(check (option int)) "max_tool_calls = 5" (Some 5) g.max_tool_calls_per_turn
;;
...
let test_pass_to_continue () =
  ...
  Alcotest.(check bool) "Pass -> Continue" true (decision = Oas.Hooks.Continue)
;;

let test_warn_to_continue () =
  let decision = Verifier_oas.verdict_to_hook_decision (Warn "minor issue") in
  Alcotest.(check bool) "Warn -> Continue" true (decision = Oas.Hooks.Continue)
;;

let test_fail_to_skip () =
  let decision = Verifier_oas.verdict_to_hook_decision (Fail "critical error") in
  Alcotest.(check bool) "Fail -> Skip" true (decision = Oas.Hooks.Skip)
;;
...
let test_read_tools_are_readonly () =
  ...
  List.iter
    (fun name ->
       ...
       Alcotest.(check bool)
         (Printf.sprintf "%s is read-only" name)
         true
...
let test_write_tools_are_not_readonly () =
  ...
  List.iter
    (fun name ->
       let schema = make_schema name in
       Alcotest.(check bool)
         (Printf.sprintf "%s is NOT read-only" name)
         false
         (Verifier_oas.read_only_predicate schema))
    write_tools
;;
...
let test_parse_verdict_pass () =
  Alcotest.(check bool) "PASS" true (Verifier_oas.parse_verdict "PASS" = Ok Pass)
;;
...
let test_parse_verdict_warn () =
  ...
  | Ok (Warn reason) ->
    Alcotest.(check bool) "reason preserved" true (String.length reason > 0)
...
let test_parse_verdict_case_insensitive () =
  Alcotest.(check bool) "pass lowercase" true (Verifier_oas.parse_verdict "pass" = Ok Pass)
;;

let test_parse_verdict_unknown_returns_error () =
  match Verifier_oas.parse_verdict "something unexpected" with
  | Error msg -> Alcotest.(check bool) "error mentions format" true (String.length msg > 0)
...
let test_parse_verdict_empty_returns_error () =
  ...
  | Error msg -> Alcotest.(check string) "empty output error" "empty verifier output" msg
...
let test_parse_verdict_rejects_passing_prefix () =
  ...
  | Error msg -> Alcotest.(check bool) "PASSING rejected" true (String.length msg > 0)
...
let test_parse_verdict_rejects_warning_prefix () =
  ...
  | Error msg -> Alcotest.(check bool) "WARNING rejected" true (String.length msg > 0)
...
let test_verify_skips_readonly () =
  ...
  in
  Alcotest.(check bool) "read-only skips to Pass" true (Verifier_oas.verify req = Ok Pass)
;;
...
let test_hook_continues_on_verify_error () =
  ...
  Alcotest.(check bool) "verifier errors degrade open" true (decision = Oas.Hooks.Continue)
;;
...
let test_hook_readonly_skips_verifier () =
  ...
  Alcotest.(check bool) "readonly still continues" true (decision = Oas.Hooks.Continue)
;;
  ...
  Alcotest.run
    ...
    [ ( "eval_gate_to_oas_guardrails"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case "denylist -> DenyList" `Quick test_denylist_maps_to_denylist
        ; Alcotest.test_case "neither -> AllowAll" `Quick test_neither_maps_to_allowall
        ; Alcotest.test_case
            "empty allowlist -> AllowList []"
            `Quick
    ...
    ; ( "parse_verdict"
      , [ Alcotest.test_case "PASS" `Quick test_parse_verdict_pass
        ...
        ; Alcotest.test_case "case insensitive" `Quick test_parse_verdict_case_insensitive
        ; Alcotest.test_case
            "unknown -> Error"
            `Quick
        ...
        ; Alcotest.test_case
            "PASSING prefix rejected"
            `Quick
            test_parse_verdict_rejects_passing_prefix
        ; Alcotest.test_case
            "WARNING prefix rejected"
            `Quick
    ...
    ; ( "verify_skip"
      , [ Alcotest.test_case "read-only skips" `Quick test_verify_skips_readonly
        ; Alcotest.test_case
            "hook verifier errors degrade open"
            `Quick
            test_hook_continues_on_verify_error
        ; Alcotest.test_case
            "hook skips verifier for readonly tools"
            `Quick
