...
function sparklineSvg(values: number[], color: string, w = 80, h = 20): string {
  ...
  return `<svg width="${w}" height="${h}" role="img" aria-label="sparkline" class="inline-block align-middle" viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg"><polyline fill="none" stroke="${color}" stroke-width="1.5" stroke-linejoin="round" points="${points}"/></svg>`
}
      ...
      <${Card} title="공급자 런타임">
        <div class="grid grid-cols-2 gap-3 mb-4">
          ...
          <${StatCell}
            ...
            detail=${`클라우드 ${providers?.summary?.cloud_models ?? 0} · CLI ${providers?.summary?.cli_models ?? 0}`}
          />
        </div>
