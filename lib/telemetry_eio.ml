...
let empty_tool_usage_stats =
  { count = 0; success_count = 0; failure_count = 0; last_used_at = None }
;;
  ...
  then []
    ...
    |> List.filter_map (fun line ->
      ...
      | Yojson.Json_error msg ->
        report_telemetry_drop ~reason:"json_syntax_error" ~path:file ~detail:msg;
        None))
;;

let event_to_json event =
  let record = { timestamp = Time_compat.now (); event } in
...
let read_all_events ?fs:_ config : event_record list =
  ...
  if jsons <> []
  then parse_event_records jsons
  else read_all_events_from_path (telemetry_file config)
;;
...
let summarize_tool_usage ?fs config : tool_usage_summary =
  ...
  let telemetry_available =
    Sys.file_exists telemetry_path || Sys.file_exists (Dated_jsonl.base_dir store)
  in
  let stats_by_tool = Hashtbl.create 32 in
  let total_calls = ref 0 in
  let records = read_all_events ?fs config in
  List.iter
    (fun (record : event_record) ->
       match record.event with
       | Tool_called { tool_name; success; _ } ->
         incr total_calls;
         update_tool_usage stats_by_tool ~tool_name ~success ~timestamp:record.timestamp
       | Agent_joined _
       | Agent_left _
       | Task_started _
       | Task_completed _
       ...
       | Tool_assigned _ -> ())
    records;
  { telemetry_path; telemetry_available; total_calls = !total_calls; stats_by_tool }
;;
    ...
    (fun (record : event_record) ->
       ...
       then (
         ...
         | Agent_joined { agent_id; _ } ->
           if not (Hashtbl.mem by_agent agent_id)
           then
             Hashtbl.replace
               by_agent
               agent_id
...
let calculate_handoff_rate events =
  ...
  let task_events =
    List_util.count_if
      ...
      events
  in
  if task_events = 0 then 0.0 else float_of_int handoffs /. float_of_int task_events
;;
...
let track_tool_called
  ...
  let stderr_excerpt = if success then None else nonempty_opt stderr_excerpt in
  track
    ?fs
    config
    (Tool_called
  ...
  then (
    ...
    | Some trimmed_kind ->
      let message =
        ...
        | Some m -> m
        | _ -> Printf.sprintf "tool %s failed (%s)" tool_name trimmed_kind
      in
      let context_parts =
        List.filter_map
          (fun (k, v) ->
             match v with
             | Some s when String.trim s <> "" -> Some (Printf.sprintf "%s=%s" k s)
             | _ -> None)
          [ "tool", Some tool_name
          ; "agent", agent_id
          ; "source", source
      ...
      track ?fs config (Error_occurred { code = trimmed_kind; message; context }))
;;

let track_tool_assigned
      ?fs
      config
      ~agent_id
      ~profile
  ...
  track
    ?fs
    config
    (Tool_assigned { agent_id; profile; preset; tool_count; assignment_id })
;;
...
let rotate ~fs:_ config ~max_age_days : unit =
  ...
  then
    Log.Metrics.info
      "telemetry_eio: pruned %d old day-files (max_age=%d days)"
      pruned
      max_age_days
;;
