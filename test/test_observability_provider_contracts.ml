...
let test_alias_roundtrip () =
  ...
  List.iter
    (fun (input, expected) ->
       ...
       | Some canonical ->
         check string (Printf.sprintf "alias %s -> %s" input expected) expected canonical
       | None -> fail (Printf.sprintf "alias %s resolved to None" input))
    cases
;;
...
let test_whitespace_trimmed () =
  ...
  check string "canonical" "claude-api" (Option.get a).Adapter.canonical_name
;;
...
let test_adapter_well_formed () =
  List.iter
    (fun (a : Adapter.adapter) ->
       check
         bool
         ("canonical non-empty: " ^ a.canonical_name)
         true
         (String.length a.canonical_name > 0);
       check bool ("has aliases: " ^ a.canonical_name) true (List.length a.aliases > 0))
...
let test_runtime_kind_strings () =
  ...
  check string "cli_agent" "cli_agent" (Adapter.string_of_runtime_kind Adapter.Cli_agent);
  check
    string
    "direct_api"
    "direct_api"
...
let test_resolve_canonical_wraps_adapter () =
  ...
  List.iter
    (fun label ->
       ...
       let via_adapter =
         Option.map
           (fun (a : Adapter.adapter) -> a.canonical_name)
...
let test_dashboard_provider_snapshots_include_cli_and_api () =
  Eio_main.run (fun _env ->
    ...
    check string "cli runtime kind" "cli_agent" (Option.get claude_cli).runtime_kind;
    check string "api runtime kind" "direct_api" (Option.get claude_api).runtime_kind;
    check string "glm api runtime kind" "direct_api" (Option.get glm_api).runtime_kind;
    check
      string
      "glm coding runtime kind"
      "direct_api"
      (Option.get glm_coding_plan).runtime_kind;
    check
      bool
      "gemini cli expands concrete models"
      true
      ((Option.get gemini_cli).models <> []);
    check
      bool
      "gemini cli does not expose bare auto"
      false
...
let test_default_registry_populated () =
  ...
  let ctx = Masc_mcp.Oas_model_resolve.max_context_of_label "claude:claude-sonnet-4-6" in
  check bool "registry resolves known provider" true (ctx > 0)
;;

let test_provider_name_of_label () =
  let name =
    Masc_mcp.Oas_model_resolve.provider_name_of_label "claude:claude-sonnet-4-6"
  in
  check (option string) "provider name" (Some "claude") name;
  let no_colon = Masc_mcp.Oas_model_resolve.provider_name_of_label "just-a-model" in
...
let test_max_context_of_label () =
  ...
  let fallback = Masc_mcp.Oas_model_resolve.max_context_of_label "nonexistent:model" in
...
let test_resolve_max_cascade_context () =
  ...
  check
    int
    "empty labels fallback 128000"
    128_000
    (Masc_mcp.Oas_model_resolve.resolve_max_cascade_context []);
  (* Unknown provider → fallback *)
  check
    int
    "unknown provider fallback 128000"
    128_000
    (Masc_mcp.Oas_model_resolve.resolve_max_cascade_context [ "nonexistent:model" ]);
  (* Malformed label (no colon) → fallback *)
  check
    int
    "malformed label fallback 128000"
    128_000
  ...
  let ctx =
    Masc_mcp.Oas_model_resolve.resolve_max_cascade_context [ "claude:claude-sonnet-4-6" ]
  in
...
let test_labels_require_local_discovery () =
  check
    bool
    "llama labels refresh local discovery"
    true
    (Masc_mcp.Oas_model_resolve.labels_require_local_discovery
       [ "llama:auto"; "glm:auto" ]);
  check
    bool
    "mixed non-local labels skip refresh"
    false
    (Masc_mcp.Oas_model_resolve.labels_require_local_discovery
       [ "glm:auto"; "claude:auto" ]);
  check
    bool
    "malformed labels skip refresh"
    false
    (Masc_mcp.Oas_model_resolve.labels_require_local_discovery [ "default"; "glm:auto" ])
;;

let test_cascade_model_resolve_alias_provenance () =
  let resolved = Model_resolve.resolve_glm_model ~getenv:(fun _ -> None) "flash" in
  check string "glm flash alias" "glm-4.7-flashx" resolved.resolved_model_id;
  check
    resolution_provenance
    "alias provenance"
    (Model_resolve.Alias "flash")
    resolved.provenance
;;
...
let test_cascade_model_resolve_hardcoded_default_provenance () =
  ...
  check
    ...
    Model_resolve.Hardcoded_default
    resolved.provenance
;;
...
let test_cascade_model_resolve_env_default_provenance () =
  let getenv = function
    ...
    | _ -> None
  in
  let resolved = Model_resolve.resolve_auto_model ~getenv "gemini" "auto" in
  check string "gemini env default" "gemini-2.5-flash" resolved.resolved_model_id;
  check
    resolution_provenance
    "env provenance"
...
let test_cascade_model_resolve_unresolved_auto_provenance () =
  ...
  in
  check string "openrouter unresolved auto stays auto" "auto" resolved.resolved_model_id;
  check
    resolution_provenance
    "unresolved provenance"
    Model_resolve.Unresolved_auto
    resolved.provenance
;;
...
let test_heartbeat_snapshot_has_required_fields () =
  ...
  let keys =
    match snapshot with
    | `Assoc kvs -> List.map (fun (k, _) -> k) kvs
    | _ -> []
  in
  List.iter
    (fun required -> check bool ("has field: " ^ required) true (List.mem required keys))
...
let test_prometheus_text_format () =
  ...
  check bool "prometheus output non-empty" true (String.length metrics >= 0)
;;
...
let test_event_serialization_roundtrip () =
  ...
  List.iter
    (fun event ->
       ...
       check
         bool
         ("json roundtrip: " ^ T.show_event event)
         true
...
let test_bearer_token_redacted () =
  ...
  check
    ...
    true
    (not
       (String.contains redacted 'k'
        && String.sub
             redacted
             (max 0 (String.length redacted - 10))
             (min 10 (String.length redacted))
           = "key-12345"))
;;

let test_nested_credentials_redacted () =
  let input = {|{"api_key": "sk-live-abc123", "config": {"token": "tok_xyz"}}|} in
  let redacted = Masc_mcp.Observability_redact.redact_preview input in
  check
    bool
    "api_key redacted"
    true
    (not (String.contains redacted 'a' && String.length redacted < String.length input))
;;
  ...
  run
    ...
    [ ( "provider_adapter"
      , [ test_case "alias roundtrip" `Quick test_alias_roundtrip
        ...
        ; test_case
            "dashboard snapshots include cli and api"
            `Quick
            test_dashboard_provider_snapshots_include_cli_and_api
        ] )
    ; ( "oas_model_resolve"
      , [ test_case
            "resolve canonical wraps adapter"
            `Quick
            test_resolve_canonical_wraps_adapter
        ; test_case "default registry populated" `Quick test_default_registry_populated
        ...
        ; test_case "effective discovered ctx floor" `Quick test_effective_discovered_ctx
        ; test_case
            "local discovery label detection"
            `Quick
            test_labels_require_local_discovery
        ; test_case
            "cascade alias provenance"
            `Quick
            test_cascade_model_resolve_alias_provenance
        ; test_case
            "cascade hardcoded default provenance"
            `Quick
            test_cascade_model_resolve_hardcoded_default_provenance
        ; test_case
            "cascade env default provenance"
            `Quick
            test_cascade_model_resolve_env_default_provenance
        ; test_case
            "cascade discovery provenance"
            `Quick
            test_cascade_model_resolve_discovery_provenance
        ; test_case
            "cascade unresolved auto provenance"
            `Quick
            test_cascade_model_resolve_unresolved_auto_provenance
        ; test_case "resolve max cascade context" `Quick test_resolve_max_cascade_context
        ] )
    ; ( "dashboard_schema"
      , [ test_case
            "heartbeat snapshot required fields"
            `Quick
    ...
    ; ( "redaction_extended"
      , [ test_case "bearer token redacted" `Quick test_bearer_token_redacted
        ; test_case "nested credentials redacted" `Quick test_nested_credentials_redacted
