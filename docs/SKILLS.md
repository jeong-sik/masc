...
- `masc-composition-tool`, `disable-model-invocation`은 모두 거부한다.
...
- Agent Skills 표준의 실험적 `allowed-tools`는 입력 호환을 위해 파싱한 뒤 버린다.
  MASC runtime 객체나 공개 projection에는 남기지 않으며, 기존 MASC 승인 정책과
  병합하지 않는다.
