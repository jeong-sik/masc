...
type agent_setup =
  ...
  ; receipt_stop_reason_ref : Keeper_runner.stop_reason option ref
            ...
            (fun event ->
              ...
              | Agent_sdk.Hooks.BeforeTurnParams
                ...
                (* RFC-0045: signal an SDK-turn boundary so the in-turn FSM
                  fields ([turn_phase], [route_phase], [decision_stage])
                  are reset before the hook writes [Route_selecting] /
                ...
                in
                let cascade_seed =
                  Keeper_inference.for_cascade ~name:cascade_name_string
                in
                let current_budget =
                ...
                (let now = Time_compat.now () in
                 ...
                 Keeper_registry.set_turn_route_phase
                   ~base_path:config.base_path
                   meta.name
                   (Keeper_registry.Packed Keeper_registry.Route_selecting
                    : Keeper_registry.packed_route_phase);
                 (* Spec atomic group: SelectToolPolicy(idle->selecting)
                   ...
                   Route_trying marking lived inside
                   ...
                   the prior cascade state is [Route_trying]; the
                 ...
                 Keeper_registry.set_turn_route_phase
                   ~base_path:config.base_path
                   meta.name
                   (Keeper_registry.Packed Keeper_registry.Route_trying
                    : Keeper_registry.packed_route_phase);
                 let disclosure_json =
                   `Assoc
                     [ "ts_unix", `Float now
