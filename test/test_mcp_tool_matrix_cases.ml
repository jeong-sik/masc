...
let strict_guard_cases = [ "masc_reset", [ "confirm" ] ]
...
let endpoint_unavailable_guard_fragments = [ "not available on this MCP endpoint" ]
...
let contains_substring haystack needle =
  ...
  let rec loop idx =
    if nlen = 0
    then true
    else if idx + nlen > hlen
    then false
    else if String.sub haystack idx nlen = needle
    then true
    else loop (idx + 1)
  in
  loop 0
...
let parse_json_from_text text =
  ...
  | Yojson.Json_error _ ->
    (try
       ...
       Some (Yojson.Safe.from_string (String.sub text idx (String.length text - idx)))
     with
     | Not_found | Yojson.Json_error _ -> None)
;;
...
let extract_prefixed_token text prefixes =
  ...
  let scan_prefix prefix start =
    ...
    let rec loop idx =
      if idx + plen > len
      then None
      else if String.sub text idx plen = prefix
      then (
...
let rec mkdir_p path =
  if path = "" || path = Filename.dir_sep || Sys.file_exists path
  then ()
  else (
...
let setup_git_repo base_path =
  ...
  run_cmd_exn
    [ "git"; "-C"; base_path; "config"; "user.email"; "tool-matrix@example.test" ];
...
let execute_tool_ok fixture ~name ~arguments =
  ...
  | true, body -> body
  | false, body -> failwith (Printf.sprintf "setup tool failed for %s: %s" name body)
;;
...
let ensure_initialized fixture =
  ...
  ignore
    (Masc_mcp.Coord.init fixture.state.room_config ~agent_name:(Some fixture.agent_name))
;;
...
let ensure_plan_initialized fixture =
  ...
  ignore
    (execute_tool_ok
       fixture
       ~name:"masc_plan_init"
       ~arguments:(`Assoc [ "task_id", `String task_id ]));
  ignore
    (execute_tool_ok
       fixture
...
let ensure_verification_request fixture =
  ...
  | Some req_id -> req_id
  | None ->
    let base_path = Masc_mcp.Coord.masc_dir fixture.state.room_config in
    let req =
      match
        Masc_mcp.Verification.create_request
          ~base_path
          ~task_id:(ensure_task fixture)
          ~output:(`String "tool matrix output")
          ~criteria:[]
          ~worker:"tool-matrix-worker"
          ~verifier:fixture.agent_name
          ()
      with
      | Ok request -> request
      | Error err -> failwith ("failed to seed verification request: " ^ err)
    in
...
let ensure_playground_clone_for ?agent fixture =
  ...
  if not (Sys.file_exists clone_target)
  then (
    ...
    run_cmd_exn [ "git"; "-C"; clone_target; "config"; "user.name"; "Tool Matrix" ]);
  clone_target
;;
...
let ensure_lock fixture =
  ignore
    (execute_tool_ok
       ...
       ~name:"masc_lock"
       ~arguments:
         (`Assoc [ "agent_name", `String fixture.agent_name; "file", `String "README.md" ]))
;;

let prepare_for_name fixture name =
  if List.mem name [ "masc_claim_next"; "masc_transition"; "masc_plan_set_task" ]
  then ignore (ensure_task fixture);
  if
    List.mem
      name
      [ "masc_plan_get"
  ...
  then ensure_plan_initialized fixture;
  if
    List.mem
      name
      [ "masc_board_get"
  ...
  then ignore (ensure_board_post fixture);
  (* masc_verify_* tools pruned from registry; no preparation needed. *)
  if name = "masc_webrtc_answer" then ignore (ensure_webrtc_offer fixture);
  if List.mem name [ "masc_worktree_create"; "masc_worktree_list" ]
  then ignore (ensure_playground_clone fixture);
  if name = "masc_worktree_remove" then ignore (ensure_worktree_created fixture);
  if
    List.mem
      name
      [ "masc_code_edit"
  ...
  then ignore (ensure_code_file fixture);
  if name = "masc_library_add"
  then (
    mkdir_p (Filename.concat fixture.base_path "me/docs/library");
    mkdir_p (Filename.concat fixture.base_path "me/docs/library/candidates"));
  if
...
let field_value fixture ~tool_name field_name schema =
  ...
  | "working_dir" when tool_name = "masc_keeper_repair" -> `String fixture.worktree_dir
  ...
  | "goal" when tool_name = "masc_bounded_run" ->
    `Assoc [ "path", `String "$.done"; "condition", `Assoc [ "eq", `Bool true ] ]
  ...
  | "name" when List.mem tool_name [ "masc_keeper_up"; "masc_keeper_repair" ] ->
    `String "bad keeper!"
  | "task_spec" when tool_name = "masc_keeper_repair" ->
    `String "Write only OCaml code for inc : int -> int."
  | "source_text" when tool_name = "masc_keeper_repair" -> `String "let inc n = n + 1\n"
  | "name" when tool_name = "masc_keeper_msg" -> `String "bad keeper!"
  | "name" -> `String "tool-matrix"
  | "verification_id" -> `String (ensure_verification_request fixture)
  | "verifier" -> `String fixture.agent_name
  | "verdict" -> `String "pass"
  | "score" -> `Float 0.9
  | "timeout_sec" when tool_name = "masc_keeper_msg" -> `Float 1.0
  ...
  | "capability_profile"
  | "active_goal_ids"
  | "depends_on_operation_ids" -> `List [ `String "tool-matrix-item" ]
  | "success_metric" | "current_focus" | "policy" | "budget" | "chain_input" | "meta" ->
    `Assoc []
  ...
  | "param" -> `String "volume"
  | "value" -> `Float 0.5
  | "replace_all" | "create_dirs" | "dry_run" | "force" | "clear" -> `Bool true
  | "time" | "step" -> `Int 1
  | "float" -> `Float 0.5
  | _ ->
...
let tool_arguments fixture (schema : Types.tool_schema) =
  ...
  let fields =
    ...
    let optional =
      ...
      | "masc_keeper_repair" -> [ "source_text"; "max_attempts"; "working_dir" ]
      | "masc_keeper_msg" -> [ "timeout_sec" ]
      | "masc_board_post" ->
...
let guard_fragments_for_name name =
  if String.equal name "masc_web_search"
  then web_search_guard_fragments
  else if
    List.exists
  ...
  then provider_guard_fragments @ state_guard_fragments
  else if
    List.exists
      (fun prefix -> string_starts_with ~prefix name)
      [ "masc_code_"; "masc_worktree_" ]
  then git_guard_fragments @ state_guard_fragments
  else state_guard_fragments @ git_guard_fragments
;;
...
let case_for_name name =
  ...
  let expectation =
    ...
    | None ->
      if List.mem name strict_success_names
      then Expect_success
      else if List.mem name endpoint_unavailable_guard_names
      then Expect_guard endpoint_unavailable_guard_fragments
      else if List.mem name all_known_tool_names
      then Expect_success_or_guard (guard_fragments_for_name name)
      else failwith ("missing contract for " ^ name)
  in
  { init_mode
  ; prepare
  ; arguments = (fun fixture schema -> tool_arguments fixture schema)
  ; expectation
  }
;;
...
let evaluate_expectation ~name expectation response =
  ...
  if contains_any text fatal_fragments
  then Error (Printf.sprintf "%s hit fatal tool-host failure: %s" name text)
  else (
    match expectation with
    | Expect_success ->
      if is_error
      then Error (Printf.sprintf "%s expected success but got error: %s" name text)
      else Ok ()
    | Expect_guard fragments ->
      if is_error && contains_any text fragments
      then Ok ()
      else if is_error
      then
        Error
          (Printf.sprintf
             "%s expected guard %s but got: %s"
             name
             (String.concat ", " fragments)
             text)
      else Error (Printf.sprintf "%s expected guard but succeeded" name)
    | Expect_success_or_guard fragments ->
      if (not is_error) || contains_any text fragments
      then Ok ()
      else
        Error
          (Printf.sprintf
  ...
  let request =
    Yojson.Safe.to_string
      (`Assoc
          ...
          ; "params", `Assoc [ "name", `String schema.Types.name; "arguments", arguments ]
          ])
  in
...
let run_case sw ~proc_mgr ~fs ~net ~mono_clock clock (schema : Types.tool_schema) =
    ...
    Fun.protect
      ...
      (fun () ->
         ...
         try
           ...
           if String.equal schema.Types.name "masc_heartbeat_start"
           then
             Heartbeat.list ()
         ...
         | exn ->
           Error
             (Printf.sprintf
                "%s raised during contract execution: %s"
                schema.Types.name
