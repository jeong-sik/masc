...
운영자 서명을 다시 요구해 기각을 값으로 돌려준다(§4.4). 거절은 재시도도
자동 확정도 아니다(§5).
...
### 4.3 증거 자세는 판정자의 질문 안에서 보인다

제출 시점에 고정된 스냅샷을 그 자리에서 계산한 증거 자세 — 열 수 있고 잘리지
않은 artifact의 수 — 는 완료 질문의 몸 안에 렌더된다. 자세가 note-only면 질문
그 자체가 "사용 가능한 artifact 없는 승인은 기각 사유" 절을 실어 나른다.
입구에서 제출을 막는 게이트가 아니다: 판정자의 질문이 자라는 것이고, 막는
것은 판정이다(RFC-0337의 교훈). 취소 질문은 이 절을 갖지 않는다 — 중단은
artifact 없는 것이 정상이기 때문이다(#33052). 계산은 판정자 확인 조항 3·4를
형(type)으로 옮긴 것일 뿐이라, 산수와 문장이 어긋날 길이 없다.
...
§6.3 = §4.1 게이트 계약의 증명(순수 함수 단언). §6.4 = §4.4 깔때기 계약의
...
| §4.4 깔때기 이중 잠금 | `test_workspace_task_lifecycle.ml` `test_system_approval_of_cancel_claim_is_refused` · `test_operator_approval_still_cancels_a_cancel_claim` | 시스템 서명 × 취소 청구 = `Verdict_cancel_requires_operator`, 운영자 서명 = `Cancelled` (슈트 녹색) |
