...
let validation_error_to_string = function
  ...
  | Content_too_long len -> Printf.sprintf "content too long: %d chars" len
  ...
  | Duplicate_message key -> Printf.sprintf "duplicate message (idempotency_key=%s)" key
;;

let validate ~max_content_length ~dedup_check (msg : inbound_message) =
  let name = String.trim msg.keeper_name in
  if name = ""
  then Error Empty_keeper_name
  else if String.trim msg.channel_user_id = ""
  then Error Empty_channel_user_id
  else if String.trim msg.idempotency_key = ""
  then Error Empty_idempotency_key
  else (
    let content = String.trim msg.content in
    if content = ""
    then Error Empty_content
    else (
      let len = String.length content in
      if len > max_content_length
      then Error (Content_too_long len)
      else if dedup_check msg.idempotency_key
      then Error (Duplicate_message msg.idempotency_key)
      else Ok ()))
;;
...
let inbound_of_json json =
  ...
  try
    let str key = json |> member key |> to_string_option |> Option.value ~default:"" in
    let channel =
    ...
    let keeper_name =
      ...
      if destination <> "" then destination else str "keeper_name"
    in
    let metadata =
...
let error_json msg = `Assoc [ "ok", `Bool false; "error", `String msg ]
