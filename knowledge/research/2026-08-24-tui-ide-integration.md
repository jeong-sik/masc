...
| 2 | ~~**`--remote` 로 vim 열기**~~ **완료** (#30248) — 1 의 목록에서 파일:줄 을 눌러 nvim 점프 | 1 이 있으면 문자열 파싱이 필요 없다. 서버 변경 0 | 작음 |
| 3 | **Kitty 키보드 프로토콜** 완료 (#30275) / truecolor 미착수 | 이후 전부가 여기 막힌다. 독립적으로도 이득 (조합키가 열린다) | 중간 |
| 4 | ~~**프레임을 조각(span) 모델로**~~ **완료** (#30320) — 모델 교체가 아니라 겹치는 색 합성으로 | diff 를 그리려면 필요. 1 없이 하면 목적 없는 리팩터가 된다 | 큼 |
| 5 | ~~**diff 렌더러**~~ **완료** (#30320) — `Enter` 로그 / `d` 트리, 화면 둘 — tool_calls 화면 / `git diff` 화면 둘로 나눠서 (§5.2) | 여기까지 와야 스크린샷 같은 그림이 나온다 | 중간 |
| 6 | **vim → MASC 커서 되먹임** — **전제가 틀렸다** (#30332) | 서버는 받을 준비가 됐지만 커서 평면에 생산자도 소비자도 없다. 누구에게 보일지부터 | 정해야 함 |
...
## 8. 4·5단계를 하며 재서 알게 된 것 (2026-08-25)

### 8.1 `git diff` 라우트가 키퍼 클론을 못 보고 있었다 (#30322 → #30324)

트리 화면을 붙이다 잡혔다. `GET /api/v1/git/diff?keeper=<name>` 이 **수정된 파일에 "변경 없음" 을 200 으로** 답하고 있었다.

라우트가 git 을 질의받은 루트에서 돌리는데, 플레이그라운드는 클론을 `repos/<id>/` 아래 둔다. 그 루트에 저장소가 없으면 **git 은 멈추지 않고 위로 올라가** MASC 자기 체크아웃을 감싸는 저장소에서 답한다. 거기 그 경로가 없으니 빈 출력이 나오고, 라우트는 그걸 "변경 없음" 으로 옮긴다.

지금 도는 키퍼 10명 중 4명이 그 레이아웃이었다. 클론이 루트에 바로 놓인 경우는 더 나쁘다 — 그 클론이 깨끗하면 답이 **우연히** 맞아서, 부르는 쪽이 맞은 답과 틀린 답을 구분할 수 없다.

**이 엔드포인트를 읽는 화면이 없어서 아무도 확인하지 않았다.** 3.1a 의 region 수집이 끊긴 것과 같은 모양이다: 소비자가 없으면 생산자가 틀려도 조용하다.

### 8.2 커서 평면은 양쪽이 다 비어 있다 (#30332)

6단계는 "서버는 이미 받을 준비가 됨" 을 전제로 작음이라고 적었다. 맞지만 절반이다.

- `GET /api/v1/ide/cursors` → `{"cursors":[],"count":0}`
- `Ide_bridge.list_cursors` 를 부르는 곳은 그 GET 하나뿐
- 키퍼 컨텍스트에서 커서를 읽는 코드 없음

쓰기 하나, 읽기 하나, **생산자 0, 소비자 0.** nvim `autocmd` 만 붙이면 아무에게도 안 보이는 쓰기가 하나 생긴다. 커서가 운영자에게 보여야 하는지 키퍼에게 보여야 하는지부터 정해야 하고, 후자는 `instructions/projects.md` 의 "가시성은 authority 가 아닌 projection" 선을 건드린다.

### 8.3 4단계는 렌더링 모델 교체가 아니었다

원래 "17,510줄 TUI 의 렌더링 모델을 바꾸는 일, 큼" 이라고 적었다. 실제로 필요한 건 **겹치는 색을 합성하는 것** 하나였다.

프레임은 `string list` 그대로 두고, 색이 겹치는 렌더러만 `Masc_tui_span` 을 지난다. 색 없는 줄은 원래 문자열과 바이트까지 같게 나온다. 큰 리팩터가 아니라 새 모듈 하나였다.

### 8.4 이름으로 짐작한 함수가 두 번 틀렸다

- `Masc_tui_message_layout.fit_width` 는 자르기가 아니다. **폭을 채우고 `~` 를 붙인다.**
- 빈 문자열에는 줄이 없다. `String.split_on_char '\n' ""` 이 `[""]` 를 줘서, 빈 파일에 쓴 변경이 *빈 줄 하나를 지웠다* 고 보고했다.

둘 다 테스트에서 잡혔다. 둘 다 읽지 않고 이름만 보고 쓴 결과다.
