...
module TokenStore = struct
  ...
  let init () : unit = if !lock = None then lock := Some (Eio.Mutex.create ())
  ...
  let cleanup_internal ~(max_age : float) : int =
    ...
    let old_tokens =
      StringMap.fold
        (fun id t acc -> if now -. t.created_at > max_age then id :: acc else acc)
        !tokens
        []
    in
  ...
  let maybe_auto_cleanup () =
    ...
    then (
      ...
      if removed > 0 then Log.Cancel.info "Auto-cleanup removed %d expired tokens" removed)
  ;;
  ...
  let create_with_id (id : string) : unit =
    with_lock (fun () ->
      if not (StringMap.mem id !tokens)
      then (
        let token =
          { id
          ; cancelled = Atomic.make false
        ...
        then (
          ...
          List.iter
            (fun cb ->
               ...
               | exn -> Log.Cancel.error "Callback failed: %s" (Printexc.to_string exn))
            t.callbacks)
      | None -> ())
  ;;
end
...
let is_cancelled (token : token) : bool = Atomic.get token.cancelled
...
let cancel ?(reason : string option) (token : token) : unit =
  ...
  token.reason <- reason;
  if Atomic.compare_and_set token.cancelled false true
  then
...
let handle_cancellation_tool (arguments : Yojson.Safe.t) : bool * string =
  ...
  | Some "cancel" ->
    (match get_string "token_id" with
     | Some id ->
       ...
       if cancel_by_id ?reason id
       then true, Printf.sprintf "Token '%s' cancelled" id
       else false, Printf.sprintf "Token '%s' not found" id
  ...
  | Some "cleanup" ->
    let removed =
      TokenStore.cleanup ~max_age:Env_config.Cancellation.token_max_age_seconds
    in
