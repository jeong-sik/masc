...
let test_no_crash () =
  ...
  Printf.printf
    "✅ %d iterations in %.3fs (%.0f ops/sec)\n"
    iterations
    elapsed
...
let test_consistency () =
  ...
  for _ = 1 to 1000 do
    ...
    then (
      Printf.printf
        "❌ Inconsistent: input=%S mode=%s extract=%s\n"
        input
        (Mention.mode_to_string mode)
        (match extract with
         | Some s -> s
         | None -> "None");
      incr failures)
  ...
  if !failures = 0
  then Printf.printf "✅ 1000 consistency checks passed\n"
  else Printf.printf "❌ %d consistency failures\n" !failures
;;
...
let test_long_inputs () =
  ...
  List.iter
    (fun len ->
       ...
       let elapsed = Unix.gettimeofday () -. start in
       if elapsed > 0.1
       then Printf.printf "⚠️  Length %d took %.3fs (slow!)\n" len elapsed
       else Printf.printf "✅ Length %d: %.4fs\n" len elapsed)
    lengths
;;
...
let test_rapid_fire () =
  ...
  let elapsed = Unix.gettimeofday () -. start in
  Printf.printf
    "✅ Rapid fire: %d parses in %.3fs (%.0f ops/sec)\n"
    count
    elapsed
