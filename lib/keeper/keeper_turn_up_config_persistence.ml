...
let full_fields
  ...
  let fields =
    ...
    ; "input_policy", Keeper_toml_loader.Toml_string (Keeper_input_policy.to_string meta.input_policy)
    ]
  in
...
let explicit_edits
  ...
  |> append_optional "input_policy" set_string (Option.map Keeper_input_policy.to_string parsed.input_policy_opt)
