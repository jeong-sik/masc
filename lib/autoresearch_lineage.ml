...
let cycle_failure_participants = [ lesson_reviewer_actor_name; cycle_runner_actor_name ]
...
let finding_tags ~target_file ~extra =
  [ Some domain_tag; normalize_tag target_file ] @ List.map normalize_tag extra
  |> List.filter_map (fun tag -> tag)
  |> List.fold_left (fun acc tag -> if List.mem tag acc then acc else tag :: acc) []
  |> List.rev
;;
