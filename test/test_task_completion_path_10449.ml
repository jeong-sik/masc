...
let with_completion = { empty_contract with completion_contract = [ "scan files" ] }
let with_evidence = { empty_contract with required_evidence = [ "board post" ] }

let test_contract_state () =
  check string "no contract record" "no_contract" (CT.classify_contract_state None);
  check
    string
    "empty lists collapse to empty_contract"
    "empty_contract"
    (CT.classify_contract_state (Some empty_contract));
  check
    string
    "completion_contract non-empty triggers with_contract"
    "with_contract"
    (CT.classify_contract_state (Some with_completion));
  check
    string
    "required_evidence non-empty alone triggers with_contract"
    "with_contract"
...
let test_path_via_verification () =
  ...
  check
    string
    "approve→done is via_verification regardless of drift"
    "via_verification"
    (CT.classify_completion_path ~action:T.Approve_verification ~drift:None ~force:false);
  check
    string
    "approve→done with force still via_verification"
    "via_verification"
    (CT.classify_completion_path ~action:T.Approve_verification ~drift:None ~force:true)
;;
...
let test_path_forced () =
  ...
  check
    string
    "force=true with no drift labels forced_done"
    "forced_done"
    (CT.classify_completion_path ~action:T.Done_action ~drift:None ~force:true);
  check
    string
    "force=true with claimed_to_done_skip drift still forced_done"
    "forced_done"
...
let test_path_drift () =
  check
    string
    "claimed_to_done_skip without force"
...
let test_path_normal () =
  check
    string
    "in_progress→done is the canonical path"
    "in_progress_to_done"
    (CT.classify_completion_path ~action:T.Done_action ~drift:None ~force:false)
;;

let () =
  run
    "task_completion_path_10449"
    [ ( "contract_state"
      , [ test_case
            "every contract surface maps to one of three labels"
            `Quick
            test_contract_state
        ] )
    ; ( "path"
