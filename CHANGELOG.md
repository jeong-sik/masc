...
- An approval whose first delivery failed still gets its decision on the ledger. The `Resolved` row, the chat record and the SSE `resolved` event went out only on the first commit, so when the wake enqueue failed after the journal, the operator's second press and every boot replay found no pending entry, delivered the wake and wrote no row: the trust timeline kept showing the approval as waiting for an operator decision. The row is now written when the audit ledger holds none for the approval, read newest-first up to its `Resolved` or `Pending` row, so a redelivery still records nothing twice (#38043).
