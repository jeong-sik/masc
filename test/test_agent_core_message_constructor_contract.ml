(** AGENT_CORE message smart-constructor contract (masc axis-1b — under-wired AGENT_CORE
    consumption).

    Seven MASC sites delegated hand-rolled [Agent_core.Types.message] record
    literals to the AGENT_CORE smart constructors instead of re-spelling the
    ...
    future AGENT_CORE change to a default (metadata, a new message field, role mapping)
...
let () =
  run "agent_core_message_constructor_contract"
    [ ( "contract"
