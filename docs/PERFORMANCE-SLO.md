...
- raw local runtime 비용은 `masc_local_runtime_bench`로 MCP read-path와 분리해서 읽음
...
- `quick-bench.sh`는 `mcp_session_init`, 주요 MCP read/write path, `masc_local_runtime_status`, `masc_local_runtime_bench`를 한 번에 보여준다.
...
- `local64`는 target runtime profile 이름이지 achieved fact가 아니다. 실제 용량은 `masc_local_runtime_status`의 `configured_capacity`, `healthy_runtime_count`로 확인한다.
