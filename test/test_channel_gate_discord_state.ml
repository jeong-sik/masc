...
let test_status_json_reports_missing_live_status () =
  ...
  with_discord_paths dir (fun () ->
    ...
    check string "channel" "discord" (json |> U.member "channel" |> U.to_string);
    check bool "available false" false (json |> U.member "available" |> U.to_bool);
    check bool "connected false" false (json |> U.member "connected" |> U.to_bool);
    check bool "stale true" true (json |> U.member "stale" |> U.to_bool);
    check
      string
      "generic missing-status error"
      "connector status file not found"
      (json |> U.member "error" |> U.to_string);
    check
      int
      "no configured bindings"
      0
  ...
  with_discord_paths dir (fun () ->
    ignore
      (Discord_state.bind
         ...
         ~keeper_name:"luna"
         ~actor_name:"dashboard");
    match Discord_state.unbind ~channel_id:"1234567890" ~actor_name:"dashboard" with
    | Error err -> fail err
    | Ok json ->
      check
        int
...
let test_connectors_json_advertises_gate_connector_descriptor () =
  ...
  with_discord_paths dir (fun () ->
    ...
    check
      bool
      "bindings capability exposed"
      true
      (connector
       |> U.member "capabilities"
       |> U.to_list
    ...
    check
      string
      "observed channel surfaced"
      "discord"
      (connector |> U.member "observed_channel" |> U.member "channel" |> U.to_string))
;;
...
let test_name_map_round_trip () =
  ...
  with_discord_paths dir (fun () ->
    ...
    check
      string
      "guild name round-trips"
      "sangsu-lab"
      (List.assoc "123" loaded.guild_names);
    check
      string
      "channel name round-trips"
      "#general"
      (List.assoc "456" loaded.channel_names);
    check
      string
      "channel_to_guild round-trips"
      "123"
      (List.assoc "456" loaded.channel_to_guild);
    check string "updated_at round-trips" "2026-04-15T00:00:00Z" loaded.updated_at)
;;
...
let test_read_name_map_missing_returns_empty () =
  ...
  with_discord_paths dir (fun () ->
    ...
    check int "no channel_to_guild entries" 0 (List.length loaded.channel_to_guild);
...
let test_bind_populates_guild_id_when_names_available () =
  ...
  with_discord_paths dir (fun () ->
    ...
    match
      Discord_state.bind ~channel_id:"chan-1" ~keeper_name:"luna" ~actor_name:"dashboard"
    with
    | Error err -> fail err
    | Ok json ->
      let audit = json |> U.member "recent_audit" |> U.to_list in
...
let test_bind_accepts_missing_names_with_empty_guild_id () =
  ...
  with_discord_paths dir (fun () ->
    match
      Discord_state.bind ~channel_id:"unknown" ~keeper_name:"luna" ~actor_name:"dashboard"
    with
    | Error err -> fail err
    | Ok json ->
      let audit = json |> U.member "recent_audit" |> U.to_list in
      check
        string
        "audit guild_id empty when names absent"
        ""
        (List.hd audit |> U.member "guild_id" |> U.to_string))
;;
  ...
  run
    ...
    [ ( "status"
      , [ test_case
        ...
        ; test_case
            "bind persists binding and audit"
            `Quick
            test_bind_persists_binding_and_audit
        ; test_case "unbind removes binding" `Quick test_unbind_removes_existing_binding
        ; test_case
            "connectors json advertises connector descriptor"
            `Quick
    ...
    ; ( "name_map"
      , [ test_case "round trip" `Quick test_name_map_round_trip
        ...
        ; test_case "resolve hits and misses" `Quick test_resolve_guild_id_hits_and_misses
        ; test_case
            "bind populates guild_id when names available"
            `Quick
            test_bind_populates_guild_id_when_names_available
        ; test_case
            "bind accepts missing names with empty guild_id"
            `Quick
