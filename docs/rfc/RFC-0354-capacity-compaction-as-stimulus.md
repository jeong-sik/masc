...
> **증거 소재 (2026-07-27 추가)**: 위 수치는 **디스크에서 더 이상 재현되지 않는다.** taskmaster 의 회전된 매니페스트 세대가 23:03~23:11 사이에 사라지면서 census 가 2,506 → 1,661 로 떨어졌다. `runtime-manifests` 는 기본값에서 라이브 1세대 + 회전 1세대만 보존하고(`env_config_keeper.ml:73,76` — `max_rotated_files` 기본 1, 파일당 10MB) 아카이브가 없으며, 이 채널을 노출하는 MCP resource 도 없어 다른 사본이 존재하지 않는다 (#25791).
>
> 따라서 §1 의 표는 **APC 원장** `<base-path>/.masc/runs/apc/selftest.jsonl` 의 2026-07-26T22:26:06Z ~ 23:03:47Z 줄(7회 연속 동일값)로만 검증 가능하다. §6 의 재현 스니펫은 그 시점 이후로는 더 작은 수를 낸다.
