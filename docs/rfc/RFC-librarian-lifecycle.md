...
- **Librarian 이 서도 Keeper 는 턴을 계속 돈다**(구현 전. #37793 이 물은 (가)/(나) 중 (가)를 택한다. 경위는 #37793 issuecomment-5788668419 에 있다). 아래 1~6 은 구현할 규칙이고, 지금 코드는 아직 이렇게 돌지 않는다. 요청이 읽은 위치부터 시작하면, Librarian 이 서 있는 동안 요청이 턴마다 커진다. 그 끝은 provider 의 거절이고, 그 뒤로는 매 턴 같은 거절이다. Keeper 가 턴을 돌지 못하니 I7 이 사실상 깨진다. "읽은 위치부터 보낸다"와 "건너뛰지 않는다"는 묶여 있지 않다. I3 은 Librarian 이 읽는 위치를 말하고, Keeper 요청이 어디서 시작하는지는 말하지 않는다. 그래서 이렇게 한다.
  ...
  6. 경계부터 보낸 요청도 크기로 거절되면, 이번 턴의 도구 결과를 마커로 바꿔 같은 후보에 한 번 더 보낸다(창 RFC §10.4, #38109). 바꿀 것이 없거나 그것도 거절되면 그 턴은 그 거절로 끝난다. 그때 넘치는 것은 이력이 아니라 매 턴 붙는 부분이다(창 RFC §13.9 첫 항목).
