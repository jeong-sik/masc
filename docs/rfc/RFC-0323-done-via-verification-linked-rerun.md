...
**Phase A predicate decision (resolved in G-1 implementation)**: `contract.strict = true`, NOT contract presence. Task creation auto-fills an advisory contract for every task (`ensure_task_contract_for_verification` always populates `completion_contract`), so the RFC-0308-era presence predicate (`contract_requires_verification`) is vacuously true fleet-wide — wiring it would flip Phase B semantics unannounced, before G-4 (teaching) and the G-5 readiness gates. `strict` is the explicit persisted opt-in and already carries a strict-only semantic (`strict_release_requires_handoff`). The guard is gated on `verification_enabled` (a disabled verification FSM must not deadlock completion) and is authority-blind (matching the approve arm). Phase B replaces the predicate with a default-on one behind the G-5 gates.
