...
OAS pin metadata is generated from `scripts/oas-agent-sdk-pin.sh`. Current dependency floor: `agent_sdk >= 0.192.0`, runtime pin: `main@fec011df7cac71521a261af362d172c3e5bade49`, declared base version: `v0.192.0`. 최신성 검증이 필요할 때는 문서에 적힌 숫자보다 `dune-project`와 pin script를 우선 truth source로 본다.
...
- **Unknown keys**: canonical/removed 둘 다 아닌 key는 health에 blocking schema state로 노출된다 (`keeper_config_schema_status=blocked`, `keeper_config_schema_terminal_reason=config_unknown_keys`). 과거에 `legacy_scope`/`scope_kind` 같은 dead config가 축적된 적이 있으므로 이 상태를 발견하면 TOML을 정리한다.
