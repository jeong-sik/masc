...
(* ================================================================ *)
(* OAS Eval_baseline bridge                                         *)
(* ================================================================ *)

(** Convert MASC eval_result to OAS run_metrics for baseline comparison.
    Bridges the MASC eval_harness to OAS Eval_baseline module.

    @since OAS v0.68.0 *)
let to_oas_run_metrics (r : eval_result) : Agent_sdk.Eval.run_metrics =
  let metrics : Agent_sdk.Eval.metric list =
    [ { name = "pass_at_k";
        value = Float_val r.pass_at_k; unit_ = None; tags = [] };
      { name = "mean_score";
        value = Float_val r.mean_score; unit_ = None; tags = [] };
      { name = "consistency";
        value = Float_val r.consistency; unit_ = None; tags = [] };
      { name = "total_cost_usd";
        value = Float_val r.total_cost_usd; unit_ = Some "USD"; tags = [] };
      { name = "num_runs";
        value = Int_val (List.length r.runs); unit_ = None; tags = [] };
    ]
  in
  let verdicts : Agent_sdk.Harness.verdict list =
    List.map (fun (run : eval_run) ->
      { Agent_sdk.Harness.passed = run.passed;
        score = Some run.weighted_score;
        evidence = [Printf.sprintf "scenario=%s run=%d" r.scenario.id run.run_index];
        detail = run.error;
      }
    ) r.runs
  in
  { run_id = Printf.sprintf "%s-%d" r.scenario.id (List.length r.runs);
    agent_name = r.scenario.name;
...
(** Save a MASC eval suite result as an OAS baseline.
    Converts first eval_result to run_metrics and delegates to
    [Agent_sdk.Eval_baseline.save]. *)
let save_oas_baseline ~(path : string) ~(description : string)
    (suite : eval_suite_result) : (unit, string) result =
  match suite.results with
  | [] -> Error "no eval results to save as baseline"
  | first :: _ ->
    let rm = to_oas_run_metrics first in
    let bl = Agent_sdk.Eval_baseline.create ~description rm in
    Agent_sdk.Eval_baseline.save ~path bl

(** Compare a MASC eval result against an OAS baseline.
    Returns the OAS comparison result with regression detection. *)
let compare_oas_baseline ~(baseline_path : string) (r : eval_result)
    : (Agent_sdk.Eval_baseline.comparison, string) result =
  match Agent_sdk.Eval_baseline.load ~path:baseline_path with
  | Error e -> Error e
  | Ok bl ->
    let current = to_oas_run_metrics r in
    Ok (Agent_sdk.Eval_baseline.compare ~baseline:bl ~current ())

(** Generate an OAS-format eval report from a MASC eval result.
    Optionally compares against a baseline. *)
let generate_oas_report ?baseline_path (r : eval_result)
    : Agent_sdk.Eval_report.t =
  let baseline = match baseline_path with
    | None -> None
    | Some path ->
      match Agent_sdk.Eval_baseline.load ~path with
      | Ok bl -> Some bl
      | Error _ -> None
  in
  let rm = to_oas_run_metrics r in
  Agent_sdk.Eval_report.generate ?baseline [rm]
