(** Fusion — out-of-band panel+judge gate 오케스트레이터. 한 요청에 대해
...
(** 요청을 심의한다.
    ...
    + [Allow]면 preset의 패널 모델로 [Fusion_panel.run], judge 모델로 [Fusion_judge.run].
      패널/심판 system prompt는 preset(=config)에서 필수이고, 타임아웃은 preset
      생략 시 [Fusion_policy.default_timeout_s](120s)로 falls back한다.
