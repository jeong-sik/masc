...
(** Resolve max context tokens from OAS Provider_registry / Capabilities (SSOT).
    Resolution order:
    1. Capabilities.for_model_id — per-model override (e.g. "provider-model-id" -> 1M)
    2. Provider_registry.find — provider-level default (e.g. "provider" -> 200K)
...
let resolve_max_context model =
  ...
  (* Layer 1: per-model capabilities (e.g. "provider-model-id" -> 1M) *)
  let from_caps =
  ...
  | Some n -> n
  | None ->
    (* Layer 2: exact provider name lookup (e.g. "provider" -> 200K) *)
