...
let cache_response (req : Llm.completion_request) ~content ~model_used =
  let key = Llm.cache_key_of_request req in
  let payload =
    `Assoc
      [
        ("v", `String "1");
...
let make_request model_id =
  let model : Llm.model_spec =
    {
      provider = Llm.Custom "cached";
      model_id;
      max_context = 4096;
...
let test_cascade_uses_next_cached_response_when_validator_rejects () =
  with_temp_cwd (fun () ->
      ...
      match
        Llm.cascade
          ~accept:(fun (resp : Llm.completion_response) ->
            not (String.equal (Llm.text_of_response resp) "reject me"))
          [ first; second ]
      with
      | Ok resp ->
          check string "content" "accept me" (Llm.text_of_response resp);
...
let test_model_spec_of_string_rejects_bare_ollama_provider () =
  match Llm.model_spec_of_string "ollama:glm-4.7-flash" with
  | Ok _ -> fail "expected ollama: prefix to be rejected"
  | Error _ -> ()

let test_model_spec_of_string_parses_llama_provider () =
  match Llm.model_spec_of_string "llama:glm-4.7-flash" with
  | Ok spec ->
      check bool "provider" true (spec.provider = Llm.Llama);
...
let test_model_spec_of_string_resolves_default_label () =
  with_env "MASC_DEFAULT_PROVIDER" "glm" (fun () ->
      with_env "MASC_DEFAULT_MODEL" "glm-4.7" (fun () ->
          match Llm.model_spec_of_string "default" with
          | Ok spec ->
              check bool "provider" true (spec.provider = Llm.Glm_cloud);
...
let test_model_spec_of_string_resolves_default_override () =
  with_env "MASC_DEFAULT_PROVIDER" "gemini" (fun () ->
      with_env "MASC_DEFAULT_MODEL" "gemini-2.5-pro" (fun () ->
          match Llm.model_spec_of_string "default:gemini-2.5-flash" with
          | Ok spec ->
              check bool "provider" true (spec.provider = Llm.Gemini);
...
let test_run_prompt_cascade_uses_same_request_shape () =
  with_temp_cwd (fun () ->
      ...
      match
        Llm.run_prompt_cascade ~temperature:0.0 ~timeout_sec:30
          ~accept:(fun (resp : Llm.completion_response) ->
            not (String.equal (Llm.text_of_response resp) "reject me"))
...
let test_llama_cache_key_preserves_requested_budget_below_global_cap () =
  let llama_model =
    { Llm.llama_default with model_id = "qwen3.5-35b-a3b-ud-q8-xl" }
  in
  let req_short =
  ...
  check bool "different cache key"
    true
    (Llm.cache_key_of_request req_short
     <> Llm.cache_key_of_request req_long)
...
let test_llama_cache_key_caps_requests_above_global_limit () =
  ...
  check string "same capped cache key"
    (Llm.cache_key_of_request req_near_cap)
    (Llm.cache_key_of_request req_far_above_cap)
