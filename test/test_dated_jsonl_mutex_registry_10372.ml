...
let test_same_base_dir_shares_mutex () =
  ...
  check
    bool
    "same base_dir → physically identical mutex"
    true
...
let test_different_base_dir_distinct_mutex () =
  ...
  check
    bool
    "distinct base_dir → distinct mutex"
    false
    (D.For_testing.mutex a == D.For_testing.mutex b)
;;
...
let test_explicit_mutex_overrides_registry () =
  ...
  check bool "explicit mutex used verbatim" true (D.For_testing.mutex store == injected);
  check
    bool
    "explicit mutex differs from registry entry"
    false
...
let test_trailing_slash_normalizes () =
  ...
  check
    bool
    "trailing slash normalises to same mutex"
    true
    (D.For_testing.mutex a == D.For_testing.mutex b)
;;
...
let big_payload tag =
  ...
  `Assoc [ "tag", `String tag; "body", `String body; "ts", `Float (Unix.gettimeofday ()) ]
;;
...
let test_concurrent_two_instances_no_corruption () =
  ...
  check
    bool
    "registry shares mutex across instances"
    true
...
let () =
  run
    "dated_jsonl_mutex_registry_10372"
    [ ( "registry-identity"
      , [ test_case "same base_dir shares mutex" `Quick test_same_base_dir_shares_mutex
        ; test_case
            "different base_dir distinct mutex"
            `Quick
            test_different_base_dir_distinct_mutex
        ; test_case
            "explicit ?mutex bypasses registry"
            `Quick
            test_explicit_mutex_overrides_registry
        ; test_case "trailing slash normalises" `Quick test_trailing_slash_normalizes
        ] )
    ; ( "concurrent-append"
      , [ test_case
            "two instances at same dir do not corrupt"
            `Quick
