      ...
      ( "surfaces",
        `List
          [
            ...
            surface ~id:"activity_graph" ~label:"Activity Graph"
              ~purpose:"실시간 실행 이벤트 관계와 actor 분포를 그래프와 집계로 보여준다."
              ~problem_solved:"활동 흐름을 텍스트 로그만으로 해석해야 하는 비용을 줄인다."
              ~when_active:"활동 탭에서 activity graph가 열릴 때."
              ~agent_role:"에이전트가 실행 관계 구조와 최근 상호작용을 함께 설명할 수 있다."
              ~ecosystem_function:"실행 이벤트 상호작용을 구조화된 관계로 드러낸다."
              [
                panel ~id:"activity_graph.graph" ~title:"활동 그래프"
                  ~purpose:"actor와 연결 관계를 시각적 그래프로 보여준다."
                  ~problem_solved:"누가 누구와 주로 상호작용하는지 한눈에 보기 어렵던 문제를 줄인다."
                  ~when_active:"activity graph panel."
                  ~agent_role:"에이전트가 실행 관계 구조를 시각적 증거로 설명한다."
                  ~ecosystem_function:"상호작용 구조의 가시화."
                  [];
                panel ~id:"activity_graph.leaderboard" ~title:"활동 주체 순위"
                  ~purpose:"최근 활동량이 높은 actor를 순위로 보여준다."
                  ~problem_solved:"주요 활동 주체를 빠르게 찾지 못하는 문제를 줄인다."
                  ~when_active:"activity leaderboard."
                  ~agent_role:"에이전트가 현재 중심 actor를 짚는다."
                  ~ecosystem_function:"활동 집중도를 드러낸다."
                  [];
                panel ~id:"activity_graph.kinds" ~title:"노드 종류 분포"
                  ~purpose:"그래프에 등장하는 node kind 분포를 요약한다."
                  ~problem_solved:"어떤 종류의 상호작용이 많은지 읽기 어려운 문제를 줄인다."
                  ~when_active:"activity kinds summary."
                  ~agent_role:"에이전트가 graph composition을 설명한다."
                  ~ecosystem_function:"관계 그래프의 구성비 노출."
                  [];
                panel ~id:"activity_graph.timeline" ~title:"최근 실행 이벤트"
                  ~purpose:"activity graph 기준 최근 실행 이벤트를 시간 순으로 보여준다."
                  ~problem_solved:"그래프 구조와 시간 흐름을 따로 읽어야 하는 문제를 줄인다."
                  ~when_active:"activity timeline."
                  ~agent_role:"에이전트가 관계와 시간을 함께 설명한다."
                  ~ecosystem_function:"activity causality 보존."
                  [];
              ];
