...
let dedup_strings items =
  List.sort_uniq String.compare (List.filter_map trim_to_option items)
;;
...
type archived_agent_meta = { last_event_at : string option }
  ...
  List.fold_left
    (fun best (message : Types.message) ->
       if
         not
           (String.equal
              (String.lowercase_ascii (String.trim message.from_agent))
              lowered)
       then best
       else (
         match best with
         | None -> Some message
         | Some current -> if message.seq >= current.seq then Some message else best))
    None
    messages
;;
...
let latest_message_to agent_name messages =
  ...
  List.fold_left
    (fun best (message : Types.message) ->
       ...
       let from_self =
         String.equal (String.lowercase_ascii (String.trim message.from_agent)) lowered
       in
       let mentioned =
...
let is_session_concluded (status : Dashboard_utils.session_lifecycle) =
  ...
  | SL_active | SL_running | SL_paused | SL_failed | SL_stopped | SL_expired | SL_unknown
    -> false
;;

let status_of_archived_session (session : session_context option) =
  match session with
  | Some session -> if is_session_concluded session.status then "inactive" else "offline"
  | None -> "unknown"
;;
...
let archived_agent_meta_map config agent_names =
  ...
  |> List.iter (fun line ->
    try
      ...
      if agent_name <> "" && Hashtbl.mem wanted agent_name
      then (
        ...
        in
        let row = if row.last_event_at = None then { last_event_at } else row in
...
let keeper_alias_by_agent_name (keepers : Yojson.Safe.t list) =
  ...
  List.iter
    (fun keeper ->
       ...
       | Some keeper_name, Some agent_name -> Hashtbl.replace table agent_name keeper_name
       | Some _, None | None, _ -> ())
    keepers;
  table
;;

let build_agent_briefs
      config
      sessions
      attention_queue
      _room_json
  ...
  let messages =
    ...
    then Coord.get_messages_raw config ~since_seq:0 ~limit:200
    else []
  in
  ...
  let room_agent_by_name =
    room_agents |> List.map (fun (agent : Types.agent) -> agent.name, agent)
  in
  let agent_names =
    dedup_strings
      (List.map (fun (agent : Types.agent) -> agent.name) room_agents
       @ List.concat_map
           (fun (session : session_context) -> session.member_names)
           sessions)
  in
  ...
  |> List.map (fun agent_name ->
    ...
    let related_attention_count =
      ...
      |> List.fold_left
           (fun acc attention ->
              if List.mem agent_name attention.related_agent_names
              then acc + 1
              else (
                match related_session with
                | Some session
                  when List.mem session.session_id attention.related_session_ids ->
                  acc + 1
                | _ -> acc))
           0
    in
    ...
    let current_work = task_label (Option.bind agent (fun value -> value.current_task)) in
    ...
    let recent_output_preview =
      latest_out
      |> Option.map (fun (message : Types.message) -> compact_text message.content)
    in
    ...
    let last_activity_age_sec =
      if last_seen_ts > 0.0
      then Some (max 0 (int_of_float (now_ts -. last_seen_ts)))
      else None
    in
    let signal_truth =
    ...
    in
    let evidence_source =
      if Option.is_some latest_out || Option.is_some latest_in
      then "message"
      else if Option.is_some agent
      then "presence"
      else if Option.is_some related_session
      then "session"
      else "none"
    in
    ({ status_rank = status_rank status
     ...
     ; json =
         `Assoc
           ...
           ; ( "related_session_id"
             , json_string_option (Option.map (fun s -> s.session_id) related_session) )
           ; "last_activity_at", json_string_option last_activity_at
           ; ( "last_activity_age_sec"
             , option_to_json (fun value -> `Int value) last_activity_age_sec )
  ...
  |> List.sort (fun (left : agent_context) (right : agent_context) ->
    let by_attention =
      Int.compare right.related_attention_count left.related_attention_count
    in
    if by_attention <> 0
    then by_attention
    else (
