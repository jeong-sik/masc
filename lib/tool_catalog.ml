...
let with_semantic_flags
  ...
  ; destructive =
      (match destructive with
       | Some value -> Some value
       | None -> meta.destructive)
  ; idempotent =
      (match idempotent with
       | Some value -> Some value
       | None -> meta.idempotent)
  ; effect_domain =
...
let destructive_tool = with_semantic_flags ~destructive:true default_metadata
...
let explicit_metadata : (string * metadata) list =
  [ ( "masc_operator_judgment_write"
    , hidden_active
        "Internal operator-judge write path hidden from the default tool list; use for \
         operator judgment experiments and keeper automation." )
  ...
  ; ( "masc_leave"
    , { actor_bound_masc_coordination_tool with
        required_permission = Some Types.CanLeave
      } )
  ...
  ; "masc_messages", { readonly_tool with required_permission = Some Types.CanReadState }
  ; "masc_who", { readonly_tool with required_permission = Some Types.CanReadState }
  ; ( "channel_gate"
    , { masc_coordination_tool with required_permission = Some Types.CanBroadcast } )
  ; ( "masc_portal_open"
  ...
  ; ( "masc_room_status"
    , hidden_active
        ~canonical_name:"masc_status"
        ~replacement:"masc_status"
        "Managed-agent compatibility alias. Prefer masc_status for canonical namespace \
         state reads." )
  ; ( "masc_list_tasks"
    , hidden_active
        ~canonical_name:"masc_tasks"
        ~replacement:"masc_tasks"
        "Managed-agent compatibility alias. Prefer masc_tasks for canonical backlog \
         reads." )
  ; ( "masc_claim_task"
    , hidden_active
  ...
  ; ( "masc_set_current_task"
    , hidden_active
        ~canonical_name:"masc_plan_set_task"
        ~replacement:"masc_plan_set_task"
        "Managed-agent compatibility alias that binds current_task. Prefer \
         masc_plan_set_task." )
  ; ( "masc_complete_task"
    , hidden_active
        ~canonical_name:"masc_transition"
  ...
  ; ( "masc_admin_cleanup"
    , with_semantic_flags
        ~destructive:true
        (hidden_active
           "Administrative cleanup mutates persisted namespace state and should be \
            treated as destructive.") )
  ; ( "masc_admin_reset"
    , with_semantic_flags
        ~destructive:true
        (hidden_active
           "Administrative reset clears namespace state and should be treated as \
            destructive.") )
  ; ( "masc_gc_force"
    , with_semantic_flags
        ~destructive:true
        (hidden_active
           "Forced garbage collection removes persisted artifacts and should be treated \
            as destructive.") )
  ; ( "masc_room_delete"
    , with_semantic_flags
        ~destructive:true
        (hidden_active
           "Namespace deletion removes persisted state and should be treated as \
            destructive.") )
  ; ( "masc_force_leave"
    , with_semantic_flags
        ~destructive:true
        (hidden_active
           "Forced membership removal mutates namespace state and should be treated as \
            destructive.") )
  ; ( "masc_operator_action"
    , with_semantic_flags
        ~destructive:true
        (hidden_active
           "Operator actions can execute privileged side effects and should be treated \
            as destructive.") )
  ; ( "masc_set_param"
    , { (with_semantic_flags
           ~destructive:true
           (hidden_active
              "Internal HTTP runtime-parameter mutation route; hidden from the public \
               tool surface."))
        with
  ...
  ; ( "masc_execute"
    , with_semantic_flags
        ~destructive:true
        (hidden_active
           "Direct execution can apply privileged side effects and should be treated as \
            destructive.") )
  ...
  ; ( "masc_keeper_clear"
    , with_semantic_flags
        ~destructive:true
        { masc_coordination_tool with required_permission = Some Types.CanBroadcast } )
  ; "masc_operation_stop", destructive_tool
  ; "masc_operation_pause", { default_metadata with destructive = Some false }
...
let register_metadata name (meta : metadata) = Hashtbl.replace metadata_table name meta
let registered_metadata name = Hashtbl.find_opt metadata_table name
...
let keeper_internal_metadata name =
  ...
  let meta =
    hidden_active
      ...
      ~allow_direct_call_when_hidden:false
      ~implementation_status
      "Keeper-internal tool. Use the keeper runtime or the public MASC equivalent when \
       available."
  in
  { meta with
...
let inferred_effect_domain_of_typed_tool_name = function
  | TN.Keeper TK.Bash | TN.Keeper TK.Bash_kill | TN.Keeper TK.Shell ->
  ...
  | TN.Keeper TK.Voice_sessions -> Some Read_only
  | TN.Keeper TK.Fs_edit | TN.Keeper TK.Write -> Some Playground_write
  ...
  | TN.Keeper TK.Voice_speak -> Some Masc_coordination
  ...
  | TN.Masc TM.Spawn
  | TN.Masc TM.Start -> Some Main_worktree_write
  ...
  | TN.Masc TM.Webrtc_offer -> Some Read_only
  ...
  | TN.Masc TM.Worktree_remove -> Some Playground_write
  | TN.Masc TM.Add_task
  ...
  | TN.Masc TM.Update_priority -> Some Masc_coordination
  | TN.Masc_keeper TMK.List | TN.Masc_keeper TMK.Persona_audit | TN.Masc_keeper TMK.Status
    -> Some Read_only
  ...
  | TN.Masc_keeper TMK.Up -> Some Masc_coordination
;;
...
let tool_group_of_typed_tool_name = function
  | TN.Keeper
      ...
      | TK.Board_search
      | TK.Board_stats
      | TK.Board_vote ) -> Some Board
  | TN.Keeper (TK.Memory_search | TK.Library_read | TK.Library_search) -> Some Knowledge
  | TN.Keeper
      ( TK.Task_claim
      | TK.Task_create
      | TK.Task_done
  ...
  | TN.Keeper
      ...
      | TK.Voice_sessions
      | TK.Voice_speak ) -> Some Voice
  | TN.Keeper (TK.Bash | TK.Fs_edit | TK.Fs_read | TK.Shell | TK.Write) -> Some Filesystem
  | TN.Keeper
      ( TK.Bash_kill
      | TK.Bash_output
      | TK.Broadcast
  ...
  | TN.Masc
      ...
      | TM.Autoresearch_stop ) -> Some Masc_autoresearch
  ...
  | TN.Masc
      ...
      | TM.Webrtc_offer
      | TM.Who
      | TM.Workflow_guide ) -> Some Masc_core
;;
...
let metadata name =
  let base =
    ...
    | None ->
      ...
      else if List.mem name keeper_internal_set
      then keeper_internal_metadata name
      else if Tool_catalog_surfaces.is_on_surface System_internal name
      then
  ...
  let with_surface_visibility =
    if Tool_catalog_surfaces.is_on_surface System_internal name
    then
      ...
      ; reason =
          (match base.reason with
           | Some _ -> base.reason
           | None -> Some "System-internal tool; callable but not listed in tools/list.")
      }
    else base
  in
...
let metadata_to_fields name =
  ...
  let with_effect_domain =
    match meta.effect_domain with
    | Some effect_domain ->
      ("effectDomain", `String (effect_domain_to_string effect_domain)) :: with_reason
    | None -> with_reason
  in
  let with_tool_group =
...
let public_contract_fields name =
  ...
  let with_effect_domain =
    match meta.effect_domain with
    | Some effect_domain ->
      ("effectDomain", `String (effect_domain_to_string effect_domain)) :: base
    | None -> base
  in
  let with_actor_binding =
    match meta.requires_actor_binding with
