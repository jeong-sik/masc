...
1. 터미널 모드를 감독(§4.2, 포그라운드 로그와 ^C 유지)으로 할지, 항상 분리(`masc start` 는 서버를
   분리해 띄우고 바로 끝남, 로그는 파일)로 할지.
2. `masc-stdio` 가 `setsid()` 로 클라이언트의 프로세스 그룹을 떠나도 되는지. 클라이언트가 그룹 단위로
   MCP 서버를 끄면, 이 서버는 stdin EOF 로만 끝난다.
3. 감독이 먼저 사라졌을 때 서버의 로그 쓰기(EPIPE)를 어떻게 할지. 로그 파일로 옮겨 쓸지, 버릴지.
