...
let test_decode_agent_missing_status_fails () =
  let json =
    `Assoc [ "name", `String "alice"; "last_seen", `String "2026-03-31T12:00:00Z" ]
  in
  Alcotest.(check bool)
...
let keeper_json
      ?(models = `List [ `String "glm-5.1" ])
      ?(last_turn_ts = `String "1700000000")
...
let test_decode_keeper_missing_legacy_fields_defaults_to_none () =
  match
    Tui_decode.decode_keeper
      ~filename:"keeper-main.json"
      (keeper_json ~active_model:None ~initiative_enabled:None ())
  with
  | Ok keeper ->
    Alcotest.(check string) "filename fallback" "keeper-main" keeper.k_name;
    Alcotest.(check (option string)) "missing active_model" None keeper.k_active_model;
    Alcotest.(check (option bool))
      "missing initiative_enabled"
      None
...
let test_decode_keeper_numeric_last_turn_ts_truncates () =
  ...
  with
  | Ok keeper ->
    Alcotest.(check string) "float timestamp truncates" "1700000000" keeper.k_last_turn_ts
  | Error err -> Alcotest.fail err
;;
...
let test_decode_keeper_rejects_non_string_model_items () =
  Alcotest.(check bool)
    "non-string model entries rejected"
    true
    (Result.is_error
       (Tui_decode.decode_keeper
          ~filename:"keeper-main.json"
...
let test_parse_log_entry_missing_required_field_fails () =
  ...
  in
  Alcotest.(check bool)
    "missing context_tokens rejected"
    true
...
let () =
  Alcotest.run
    ...
    ; ( "parse_keeper_chat_response"
      , [ Alcotest.test_case "sse delta" `Quick test_parse_keeper_chat_response_sse_delta
        ; Alcotest.test_case
            "json error"
            `Quick
