...
let task_is_linked_to_keeper_goals goal_ids (task : Types.task) =
  List.exists (fun goal_id -> Convergence.task_matches_goal ~goal_id task) goal_ids
;;
...
let goal_progress_json ?config (meta : keeper_meta) =
  ...
  | Some config ->
    ...
    let blocked_task_count =
      List.fold_left
        (fun acc (task : Types.task) -> if task_is_blocked task then acc + 1 else acc)
        0
        tasks
    in
    let convergence =
...
let runtime_contract_json_from_fields
      ~keeper_name
      ?agent_name
      ?trace_id
      ?session_id
  ...
  `Assoc
    ...
    ; "missing_required_tools", string_list_json (nonempty_list missing_required_tools)
  ...
  let rec loop acc = function
    | `Assoc fields ->
      List.fold_left
        (fun acc (key, value) ->
           match value with
           | `String path when path_like_key key && String.trim path <> "" -> path :: acc
    ...
    | _ -> acc
  in
  loop [] json |> List.sort_uniq String.compare
;;
...
let action_radius_json ~tool_name ~input ~success ~duration_ms ?error ?sandbox_target ()
  : Yojson.Safe.t
  =
  let action_key =
  ...
  let target_path =
    first_string_field
      [ "target_path"; "path"; "file_path"; "repo_path"; "worktree_path"; "cwd" ]
      input
  in
  `Assoc
