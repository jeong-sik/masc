...
let agent_metrics_dir config agent_id = Filename.concat (metrics_dir config) agent_id
...
let current_month_file config agent_id =
  ...
  let filename =
    Printf.sprintf "%04d-%02d.jsonl" (tm.Unix.tm_year + 1900) (tm.Unix.tm_mon + 1)
  in
...
let record config (metric : task_metric) : unit =
  ...
  if Atomic.get queue_active
  then Eio.Stream.add write_queue { file; line }
  else
...
let flush_pending () =
  ...
  let rec drain () =
    ...
    | Some entry ->
      let prev =
        Option.value ~default:(Buffer.create 256) (Hashtbl.find_opt batch entry.file)
      in
  ...
  Hashtbl.iter
    (fun file buf ->
       try Fs_compat.append_file file (Buffer.contents buf) with
       | Eio.Cancel.Cancelled _ as e -> raise e
       | exn ->
         Log.Metrics.error
           "flush_pending: append failed for %s: %s"
           file
...
let read_metrics_file file : task_metric list =
  if not (Fs_compat.file_exists file)
  then []
  else (
...
let month_key ~year ~month = (year * 12) + (month - 1)
...
let parse_month_key_from_filename filename =
  if not (Filename.check_suffix filename ".jsonl")
  then None
  else (
...
let calculate_agent_metrics config ~agent_id ~days : agent_metrics option =
  ...
  then None
    ...
    let handoffs =
      List.filter
        (fun m -> Option.is_some m.handoff_from || Option.is_some m.handoff_to)
        metrics
    in
    ...
    let handoff_rate =
      if handoffs <> []
      then
        float_of_int (List.length successful_handoffs)
        /. float_of_int (List.length handoffs)
      else 1.0
    in
    (* No handoffs = perfect handoff rate *)
    ...
    Some
      ...
      ; task_completion_rate =
          (if total > 0
           then float_of_int (List.length successful) /. float_of_int total
           else 0.0)
      ; error_rate =
          (if total > 0
           then float_of_int (List.length failed) /. float_of_int total
           else 0.0)
