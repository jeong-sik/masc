...
let test_classify_other () =
  check bool "other" true (CB.classify_error "random error" = CB.Other)
;;
...
let test_class_change_resets () =
  ...
  ignore
    (CB.maybe_enrich_error ~keeper_name:"t4" ~error_msg:"path_not_in_allowed_paths: /x");
  let r =
    CB.maybe_enrich_error ~keeper_name:"t4" ~error_msg:"path_not_in_allowed_paths: /y"
  in
...
let test_classify_snapshot_json_happy () =
  ...
  | Ok assoc ->
    ...
    check string "alpha=clean" "clean" (display_state_str (List.assoc "alpha" assoc));
    check string "beta=warning" "warning" (display_state_str (List.assoc "beta" assoc));
    check string "gamma=cooling" "cooling" (display_state_str (List.assoc "gamma" assoc))
;;
...
let test_classify_snapshot_skips_malformed () =
  let json : Yojson.Safe.t =
    `List
      ...
      ; `Assoc [ "keeper", `String "bad-no-count"; "total_tripped", `Int 0 ]
...
let test_classify_snapshot_not_a_list () =
  ...
  | Error msg -> check bool "error mentions expected shape" true (contains msg "array")
;;
...
let test_classify_snapshot_round_trip () =
  ...
  ignore (CB.maybe_enrich_error ~keeper_name:"rt1" ~error_msg:"path_not_found: /x");
  ...
  | Ok assoc ->
    ...
    check string "rt1=warning" "warning" (display_state_str (List.assoc "rt1" assoc))
;;
...
let test_display_state_of_matches_snapshot () =
  ...
  ignore (CB.maybe_enrich_error ~keeper_name:name ~error_msg:"path_not_found: /a");
  ...
  | Ok assoc ->
    check string "json-path agrees" "warning" (display_state_str (List.assoc name assoc))
;;
...
let test_recent_failures_bounded_and_newest_first () =
  ...
  match r with
  | first :: _ ->
    check bool "newest fingerprint is err-4" true (contains first.CB.fingerprint "err-4")
...
let test_snapshot_json_exposes_recent_failures () =
  ...
  CB.record_success ~keeper_name:name;
  ignore (CB.maybe_enrich_error ~keeper_name:name ~error_msg:"uniq-signature-ABC");
  let json = CB.snapshot_json () in
  match json with
  | `List entries ->
    let found =
      List.exists
        (fun e ->
           ...
           | `Assoc fields ->
             (match
                List.assoc_opt "keeper" fields, List.assoc_opt "recent_failures" fields
              with
  ...
  run
    ...
    ; ( "signatures"
      , [ test_case
            ...
            `Quick
            test_fingerprint_collapses_whitespace
        ; test_case
            "fingerprint truncates with ellipsis"
            `Quick
            test_fingerprint_truncates
        ; test_case
            "fingerprint does not fake truncation after space collapse"
            `Quick
            test_fingerprint_does_not_fake_truncation_after_space_collapse
        ; test_case
            "recent_failures empty for unknown"
            `Quick
            test_recent_failures_empty_for_unknown
        ; test_case
            "recent_failures bounded, newest first"
            `Quick
            test_recent_failures_bounded_and_newest_first
        ; test_case
            "snapshot_json exposes recent_failures"
            `Quick
            test_snapshot_json_exposes_recent_failures
        ; test_case
            "recent_failures survive trip"
            `Quick
            test_recent_failures_survive_trip
        ] )
    ; ( "threshold"
    ...
    ; "diagnostics", [ test_case "snapshot" `Quick test_snapshot ]
    ; ( "display_state"
      , [ test_case "clean" `Quick test_display_state_clean
        ...
        ; test_case
            "classify_snapshot_json happy path"
            `Quick
            test_classify_snapshot_json_happy
        ; test_case
            "classify_snapshot_json skips malformed"
            `Quick
            test_classify_snapshot_skips_malformed
        ; test_case
            "classify_snapshot_json rejects non-list"
            `Quick
            test_classify_snapshot_not_a_list
        ; test_case
            "round-trip via snapshot_json"
            `Quick
            test_classify_snapshot_round_trip
        ] )
    ; ( "display_state_of"
      , [ test_case "unknown keeper = clean" `Quick test_display_state_of_unknown_is_clean
        ; test_case
            "direct lookup matches json walk"
            `Quick
            test_display_state_of_matches_snapshot
        ; test_case
            "success clears back to clean"
            `Quick
            test_display_state_of_clears_after_success
        ] )
    ]
;;
