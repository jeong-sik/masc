...
let with_lock f = Eio.Mutex.use_rw ~protect:true mutex f
...
let record_turn ~keeper_name ~speech_act =
  with_lock (fun () ->
    ...
    if speech_act = "stay_silent"
    then (
      s.streak <- s.streak + 1;
      update_streak_gauge keeper_name s.streak;
      let t = threshold () in
      if s.streak >= t && not s.detected_latched
      then (
        ...
        Prometheus.inc_counter
          ...
          ();
        Log.Keeper.warn
          "#9926 stay_silent loop detected keeper=%s streak=%d threshold=%d — keeper is \
           returning stay_silent repeatedly. Check preset mismatch (#9926 proposal 1) or \
           scheduler/backlog drift. Counter will not re-fire until the streak resets via \
           any non-stay_silent speech act."
          keeper_name
          s.streak
          t))
    else (
