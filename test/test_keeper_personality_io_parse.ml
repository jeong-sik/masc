...
let test_parse_partial_object_picks_defaults_only_for_missing () =
  ...
  check p_eq "partial object" expected (Keeper_personality_io.parse ~defaults json)
;;
...
let test_round_trip_ascii () =
  let p =
    make ~will:"a will" ~needs:"a need" ~desires:"a desire" ~instructions:"some inst" ()
  in
...
let test_round_trip_korean_oversized () =
  let p =
    make ~will:nick0cave_will ~needs:"" ~desires:"another desire" ~instructions:"" ()
  in
...
let () =
  run
    "keeper_personality_io_parse"
    [ ( "parse defaults"
      , [ test_case
            "empty object → empty record"
            `Quick
            test_parse_empty_object_defaults_to_empty
        ; test_case "empty object → defaults" `Quick test_parse_empty_object_with_defaults
        ; test_case
            "partial object"
            `Quick
    ...
    ; ( "round-trip"
      , [ test_case "empty" `Quick test_round_trip_empty
        ...
        ; test_case
            "korean oversized (nick0cave fixture)"
            `Quick
