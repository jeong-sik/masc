...
let keeper_writable_prefixes =
  ...
  ; (* coding workspace *)
    ".masc/decision_audit/"
  ; (* self audit logs — forensics, not trust input *)
...
let init_policy_config ~base_path =
  ...
  | Ok cfg ->
    ...
    Log.Keeper.info
      "tool policy config loaded: %d presets, %d groups"
...
let is_keeper_denied (name : string) : bool = Hashtbl.mem keeper_denied_set name
...
let keeper_supported_masc_schemas (schemas : Types.tool_schema list) =
  let supported_in_keeper name =
    if Tool_dispatch.is_registered name
    then true
    else if not (Tool_dispatch.is_tag_registry_initialized ())
    then true
    else (
      ...
      | Some Tool_dispatch.Mod_control -> false
      | Some Tool_dispatch.Mod_keeper -> List.mem name keeper_supported_keeper_masc_tools
      | Some _ -> true
      | None -> false)
  in
...
let select_existing_masc_tool_names names =
  ...
  names |> List.filter (fun name -> List.mem name injected) |> dedupe_tool_names
;;

let keeper_maintenance_only_tools = [ "masc_heartbeat" ]
let is_keeper_maintenance_only_tool name = List.mem name keeper_maintenance_only_tools
...
let keeper_base_candidate_tool_names () =
  ...
  in
  dedupe_tool_names
    (config_tools @ keeper_internal_candidate_tool_names @ injected_masc_tool_names ())
...
let resolve_policy_group ~(fallback : string list) (group_name : string) : string list =
  ...
  | Some cfg ->
    (match Keeper_tool_policy_config.resolve_group cfg group_name with
     ...
     | None ->
       Log.Keeper.warn
         "tool_policy group %S not found, using fallback (%d tools)"
         group_name
...
let explicit_optional_candidate_tool_names (meta : keeper_meta) =
  ...
  requested |> List.filter (fun name -> List.mem name optional) |> dedupe_tool_names
;;
...
let preset_allowlist preset =
  ...
  | None ->
    Log.Keeper.error
      "tool policy config not loaded; preset '%s' returns empty. Call init_policy_config \
       at startup."
      name;
    []
  | Some cfg ->
    ...
    (match Keeper_tool_policy_config.resolve_preset cfg name ~masc_filter () with
     ...
     | None ->
       Log.Keeper.warn
         "preset '%s' not defined in config/tool_policy.toml, returning empty"
         name;
       [])
;;
...
let tool_policy_of_meta (meta : keeper_meta) =
  ...
  in
  { Tool_access_policy.allow; deny = Tool_access_policy.Names meta.tool_denylist }
;;
...
let tool_access_lookup_of_meta (meta : keeper_meta) =
  ...
  let allow_names =
    Tool_access_policy.resolve ~candidates:candidate_names (tool_policy_of_meta meta)
...
let filter_by_universe ~(lookup : tool_access_lookup) (name : string) : bool =
  StringSet.mem name lookup.candidate_set && not (StringSet.mem name lookup.deny_set)
;;
...
let can_execute ~(lookup : tool_access_lookup) (name : string) : bool =
  if Keeper_tool_registry.is_core_always_tool name
  then
    (* Core tools bypass candidate_set — only deny_set blocks them *)
    not (StringSet.mem name lookup.deny_set)
  else if not (StringSet.mem name lookup.candidate_set)
  then false
  else filter_by_access ~lookup name
;;
...
let keeper_masc_tool_names (meta : keeper_meta) : string list =
  ...
  |> List.filter_map (fun (schema : Types.tool_schema) ->
    if filter_by_access ~lookup schema.name then Some schema.name else None)
;;
...
let keeper_masc_tool_schemas (meta : keeper_meta) : Types.tool_schema list =
  ...
  |> List.filter (fun (schema : Types.tool_schema) ->
    filter_by_access ~lookup schema.name)
;;
...
let keeper_default_model_tools (_meta : keeper_meta) : Types.tool_schema list =
  keeper_model_tools @ keeper_voice_tool_schemas @ [ keeper_tool_search_schema ]
;;
...
let keeper_allowed_tool_names
  ...
  then []
    ...
    phase = Keeper_state_machine.Failing
    && Keeper_decision_audit.decision_layer_level () >= 2
  then failing_minimum_tool_names ()
  else (
    let lookup = tool_access_lookup_of_meta meta in
...
let keeper_universe_tool_names (meta : keeper_meta) : string list =
  let lookup = tool_access_lookup_of_meta meta in
  let from_candidates =
    lookup.candidate_names |> List.filter (fun name -> filter_by_universe ~lookup name)
  in
  let from_core =
...
let keeper_preset_universe_tool_names (meta : keeper_meta) : string list =
  ...
  let preset_tools =
    match meta.tool_access with
    | Preset { preset; also_allow } -> preset_allowlist preset @ also_allow
    | Custom allowlist -> allowlist
  in
  let from_preset =
    preset_tools
...
let all_keeper_schemas
      ~(masc_schemas_fn : keeper_meta -> Types.tool_schema list)
      (meta : keeper_meta)
...
let filter_schemas_by_names (names : string list) (schemas : Types.tool_schema list)
  : Types.tool_schema list
  =
...
let keeper_allowed_model_tools ?(write_done = false) (meta : keeper_meta)
  ...
  then []
    ...
    if count > Keeper_config.tool_policy_count_warn_threshold
    then
      Log.Keeper.warn
        "tool policy allows %d schemas (~%dKB). Progressive disclosure limits actual LLM \
         context to ~20-40, but universe build cost scales with policy size. Consider a \
         narrower preset or custom allowlist."
        count
        (count * 470 / 1024);
    result)
;;
...
let enum_hints_of_schema (schema : Yojson.Safe.t) : string =
  ...
  let properties =
    match U.member "properties" schema with
    | `Assoc props -> props
    | _ -> []
  in
  let enums =
  ...
  | `List reqs ->
    let names =
      List.filter_map
        ...
        reqs
    in
    if names = [] then "" else "required: " ^ String.concat ", " names
  | _ -> ""
;;
