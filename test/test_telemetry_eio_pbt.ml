...
let gen_event : Telemetry_eio.event QCheck.Gen.t =
  ...
  | 4 ->
    ...
    let* reason = string_small in
    return (Telemetry_eio.Handoff_triggered { from_agent; to_agent; reason })
  | 5 ->
...
let arb_record = QCheck.make ~print:Telemetry_eio.show_event_record gen_record
    ...
    (match List.assoc_opt "event" top with
     | Some (`List [ `String tag; `Assoc fields ]) ->
       ...
       `Assoc
         (List.map
            (fun (k, v) ->
               if k = "event" then k, `List [ `String tag; `Assoc fields' ] else k, v)
            top)
     | _ -> json)
  | _ -> json
;;
...
let prop_roundtrip =
  QCheck.Test.make ~count:200 ~name:"Telemetry_eio JSON round-trip" arb_record (fun r ->
