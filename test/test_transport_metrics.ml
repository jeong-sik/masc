...
let test_init () =
  ...
  check
    bool
    "sse sessions metric registered"
    true
    (try
       let _ = Str.search_forward (Str.regexp_string "masc_sse_sessions_total") text 0 in
       true
     with
     | Not_found -> false);
  check
    bool
    "grpc active streams metric registered"
    true
    (try
       let _ =
         Str.search_forward (Str.regexp_string "masc_grpc_active_streams_total") text 0
       in
       true
     with
     | Not_found -> false);
  check
    bool
    "agent heartbeat age metric registered"
    true
    (try
       let _ =
         Str.search_forward (Str.regexp_string "masc_agent_heartbeat_age_seconds") text 0
       in
       true
     with
     | Not_found -> false)
...
let test_sse_sessions () =
  ...
  let obs =
    Prometheus.metric_value_or_zero
      "masc_sse_sessions_total"
      ~labels:[ "kind", "observer" ]
      ()
  in
  let coord =
    Prometheus.metric_value_or_zero
      "masc_sse_sessions_total"
      ~labels:[ "kind", "coordinator" ]
      ()
  in
...
let test_broadcast_duration () =
  ...
  let count =
    Prometheus.metric_value_or_zero "masc_sse_broadcast_duration_seconds_count" ()
  in
...
let test_broadcast_events_counter () =
  let before = Prometheus.metric_value_or_zero "masc_sse_broadcast_events_total" () in
  TM.observe_broadcast_duration 0.01;
  let after = Prometheus.metric_value_or_zero "masc_sse_broadcast_events_total" () in
...
let test_grpc_active_streams () =
  ...
  let v = Prometheus.metric_value_or_zero "masc_grpc_active_streams_total" () in
...
let test_grpc_heartbeat_latency () =
  ...
  let sum = Prometheus.metric_value_or_zero "masc_grpc_heartbeat_latency_seconds" () in
  check bool "heartbeat latency sum > 0" true (sum > 0.0)
;;

let test_grpc_subscribers () =
  TM.set_grpc_subscribers 7;
  let v = Prometheus.metric_value_or_zero "masc_grpc_subscribers_total" () in
  check (float 0.01) "grpc subscribers" 7.0 v
;;

let test_grpc_events_delivered () =
  let before = Prometheus.metric_value_or_zero "masc_grpc_events_delivered_total" () in
  TM.inc_grpc_events_delivered ~delta:5 ();
  let after = Prometheus.metric_value_or_zero "masc_grpc_events_delivered_total" () in
  check (float 0.01) "grpc events delta" 5.0 (after -. before)
;;

let test_grpc_events_dropped () =
  let before = Prometheus.metric_value_or_zero "masc_grpc_events_dropped_total" () in
  TM.inc_grpc_events_dropped ();
  TM.inc_grpc_events_dropped ();
  TM.inc_grpc_events_dropped ();
  let after = Prometheus.metric_value_or_zero "masc_grpc_events_dropped_total" () in
  check (float 0.01) "three drop observations advance counter by 3" 3.0 (after -. before)
;;
...
let test_ws_sessions () =
  ...
  let v = Prometheus.metric_value_or_zero "masc_ws_sessions_total" () in
  check (float 0.01) "ws sessions" 4.0 v
;;

let test_ws_enabled_blank_env_matches_runtime () =
  with_env "MASC_WS_ENABLED" (Some "") (fun () ->
    check bool "transport metrics treats blank as enabled" true (TM.ws_enabled ());
    check
      bool
      "runtime server treats blank as enabled"
      true
...
let test_ws_enabled_normalized_env_matches_runtime () =
  with_env "MASC_WS_ENABLED" (Some " FALSE ") (fun () ->
    check bool "transport metrics normalizes false env" false (TM.ws_enabled ());
    check
      bool
      "runtime server normalizes false env"
      false
      (Masc_mcp.Server_ws_standalone.is_enabled ()))
;;
...
let test_agent_stale_counter () =
  let before = Prometheus.metric_value_or_zero "masc_agent_stale_total" () in
  TM.inc_agent_stale ();
  TM.inc_agent_stale ();
  let after = Prometheus.metric_value_or_zero "masc_agent_stale_total" () in
...
let test_transport_health_json () =
  ...
  ignore
    (Masc_mcp.Sse.register
       ~kind:Masc_mcp.Sse.Observer
       "observer-session"
       ~last_event_id:0);
  ignore
    (Masc_mcp.Sse.register
       ~kind:Masc_mcp.Sse.Coordinator
       "coordinator-session"
       ~last_event_id:0);
  ...
  Prometheus.inc_counter
    ...
    ~labels:[ "event", "compaction_started" ]
    ~delta:2.0
    ();
  ...
  check int "observer sessions" 1 (sse_json |> U.member "sessions_observer" |> U.to_int);
  check
    int
  ...
  check
    bool
    "queue depth reflects queued event"
    true
    (sse_json |> U.member "queue_max_depth" |> U.to_int > 0);
  check
    bool
    "hot sessions are reported"
    true
    (sse_json |> U.member "hot_sessions" |> U.to_list |> List.length > 0);
  check int "relay queue depth" 4 (sse_json |> U.member "relay_queue_depth" |> U.to_int);
  check int "relay retries total" 3 (sse_json |> U.member "relay_retry_total" |> U.to_int);
  check int "relay drops total" 4 (sse_json |> U.member "relay_drop_total" |> U.to_int);
  check
    bool
    "streamable http configured field exists"
    true
  ...
  check
    bool
    "streamable http protocol_capable field exists"
    true
  ...
  check
    bool
    "streamable http auth_policy_present field exists"
    true
    (match streamable_json |> U.member "auth_policy_present" with
     | `Bool _ -> true
     | _ -> false);
  check int "grpc active streams" 1 (grpc_json |> U.member "active_streams" |> U.to_int);
  check int "grpc subscribers" 2 (grpc_json |> U.member "subscribers" |> U.to_int);
  check
    bool
    "grpc events_dropped field present"
    true
  ...
  check
    bool
    "grpc listening field exists"
    true
    (match grpc_json |> U.member "listening" with
     | `Bool _ -> true
     | _ -> false);
  check
    bool
    "grpc reachable field exists"
    true
    (match grpc_json |> U.member "reachable" with
     | `Bool _ -> true
     | _ -> false);
  check
    bool
    "grpc listen_status field exists"
    true
    (match grpc_json |> U.member "listen_status" with
     | `String _ -> true
     | _ -> false);
  check
    bool
    "websocket listening field exists"
    true
    (match ws_json |> U.member "listening" with
     | `Bool _ -> true
     | _ -> false);
  check
    bool
    "websocket reachable field exists"
    true
    (match ws_json |> U.member "reachable" with
     | `Bool _ -> true
     | _ -> false);
  check
    bool
    "ws listen_status field exists"
    true
    (match ws_json |> U.member "listen_status" with
     | `String _ -> true
     | _ -> false);
  check
    bool
    "websocket section exists"
    true
    (match ws_json with
     | `Assoc _ -> true
     | _ -> false);
  check
    bool
    "webrtc section exists"
    true
    (match webrtc_json with
     | `Assoc _ -> true
     | _ -> false);
  check
    bool
    "webrtc configured field exists"
    true
    (match webrtc_json |> U.member "configured" with
     | `Bool _ -> true
     | _ -> false);
  check
    bool
    "webrtc signaling_available field exists"
    true
    (match webrtc_json |> U.member "signaling_available" with
     | `Bool _ -> true
     | _ -> false);
  check
    bool
    "webrtc signaling_mode field exists"
    true
    (match webrtc_json |> U.member "signaling_mode" with
     | `String _ -> true
     | _ -> false);
  check string "room id" "default" (cluster_json |> U.member "room_id" |> U.to_string);
  check
    bool
    "summary primary path exists"
    true
    (String.length (summary_json |> U.member "primary_path" |> U.to_string) > 0);
  check
    string
    "summary queue pressure reflects relay drops"
    "high"
    (summary_json |> U.member "queue_pressure" |> U.to_string);
  check
    int
    "agent lifecycle dispatch rejections surfaced"
    2
    (agent_health_json |> U.member "lifecycle_dispatch_rejections_total" |> U.to_int);
  ...
  check
    bool
    "websocket delivery sub-object present"
    true
    (match delivery_json with
     | `Assoc _ -> true
     | _ -> false);
  List.iter
    (fun (field, label) ->
       check
         bool
         (Printf.sprintf "%s field present (int)" label)
         true
         (match delivery_json |> U.member field with
          | `Int _ -> true
          | _ -> false))
  ...
  check
    bool
    "client_buffered_bytes_sum field present (float)"
    true
...
let test_grpc_listen_status_lifecycle () =
  check string "grpc status after init" "not_started" (Atomic.get TM.grpc_listen_status);
  TM.set_grpc_listen_status "listening";
  TM.set_grpc_runtime_listening true;
  check
    string
    "grpc status after listening"
    "listening"
  ...
  TM.set_grpc_runtime_listening false;
  check string "grpc status after stopped" "stopped" (Atomic.get TM.grpc_listen_status);
  check bool "grpc listening returns false" false (TM.grpc_listening ())
;;

let test_ws_listen_status_lifecycle () =
  check string "ws status after init" "not_started" (Atomic.get TM.ws_listen_status);
  TM.set_ws_listen_status "listening";
  TM.set_ws_runtime_listening true;
  check string "ws status after listening" "listening" (Atomic.get TM.ws_listen_status);
  check bool "ws listening returns true" true (TM.ws_listening ());
  TM.set_ws_listen_status "stopped";
  TM.set_ws_runtime_listening false;
  check string "ws status after stopped" "stopped" (Atomic.get TM.ws_listen_status);
...
let test_listen_status_bind_failed () =
  ...
  check
    string
    "grpc status is bind_failed"
    "bind_failed"
    (Atomic.get TM.grpc_listen_status);
  check string "ws status is bind_failed" "bind_failed" (Atomic.get TM.ws_listen_status)
;;

let test_listen_status_disabled () =
  TM.set_grpc_listen_status "disabled";
  TM.set_ws_listen_status "disabled";
  check string "grpc status disabled" "disabled" (Atomic.get TM.grpc_listen_status);
  check string "ws status disabled" "disabled" (Atomic.get TM.ws_listen_status)
;;
...
let () =
  run
    "Transport_metrics"
    [ "init", [ test_case "registers all metric families" `Quick test_init ]
    ; ( "sse"
      , [ test_case "set_sse_sessions by kind" `Quick test_sse_sessions
        ; test_case
            "observe_broadcast_duration accumulates"
            `Quick
            test_broadcast_duration
        ; test_case
            "broadcast events counter increments"
            `Quick
            test_broadcast_events_counter
        ] )
    ; ( "grpc"
    ...
    ; ( "websocket"
      , [ test_case "set_ws_sessions" `Quick test_ws_sessions
        ; test_case
            "blank env stays enabled"
            `Quick
            test_ws_enabled_blank_env_matches_runtime
        ; test_case
            "normalized env matches runtime"
            `Quick
            test_ws_enabled_normalized_env_matches_runtime
        ; test_case "runtime listening cache" `Quick test_ws_runtime_listening_cache
        ] )
    ; ( "agent_health"
    ...
    ; ( "listen_status"
      , [ test_case
            "grpc listen_status lifecycle"
            `Quick
            test_grpc_listen_status_lifecycle
        ; test_case "ws listen_status lifecycle" `Quick test_ws_listen_status_lifecycle
        ; test_case "listen_status bind_failed" `Quick test_listen_status_bind_failed
        ; test_case "listen_status disabled" `Quick test_listen_status_disabled
        ] )
    ]
;;
