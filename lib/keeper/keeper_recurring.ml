...
type action = Broadcast of string
...
let list ~keeper_name =
  with_tasks_ro (fun () ->
    Hashtbl.fold
      (fun _id task acc -> if task.keeper_name = keeper_name then task :: acc else acc)
      tasks
      [])
;;

let list_all () =
  with_tasks_ro (fun () -> Hashtbl.fold (fun _id task acc -> task :: acc) tasks [])
;;
  ...
  List.iter
    (fun task ->
       ...
       | Error _msg ->
         ...
         if task.max_failures > 0 && task.failure_count >= task.max_failures
...
let clear () = with_tasks_rw (fun () -> Hashtbl.clear tasks)
