...
let bare_reason s =
  ...
  strip "too_complex:"
  |> fun s ->
  if Base.String.is_prefix s ~prefix:"parse_aborted:" then "__parse_aborted__" else s
;;
...
let snapshot () =
  ...
  ; gate_diff_legacy_allow_shadow_deny = Atomic.get gate_diff_legacy_allow_shadow_deny
  ; gate_diff_legacy_deny_shadow_allow = Atomic.get gate_diff_legacy_deny_shadow_allow
  ; gate_diff_shadow_cannot_parse = Atomic.get gate_diff_shadow_cannot_parse
...
let snapshot_to_json (s : snapshot) : Yojson.Safe.t =
  `Assoc
    ...
    ; "gate_diff_legacy_allow_shadow_deny", `Int s.gate_diff_legacy_allow_shadow_deny
    ; "gate_diff_legacy_deny_shadow_allow", `Int s.gate_diff_legacy_deny_shadow_allow
    ; "gate_diff_shadow_cannot_parse", `Int s.gate_diff_shadow_cannot_parse
...
let safe_ratio ~num ~den = if den <= 0 then 0.0 else float_of_int num /. float_of_int den

let disagree_ratio (s : snapshot) : float =
  safe_ratio
    ~num:(s.gate_diff_legacy_allow_shadow_deny + s.gate_diff_legacy_deny_shadow_allow)
    ~den:s.gate_diff_total
;;

let shadow_parse_coverage (s : snapshot) : float =
  if s.gate_diff_total <= 0
  then 0.0
  else 1.0 -. safe_ratio ~num:s.gate_diff_shadow_cannot_parse ~den:s.gate_diff_total
;;

let auto_bg_promotion_rate (s : snapshot) : float =
  safe_ratio ~num:s.auto_bg_would_have_promoted ~den:s.auto_bg_observed
;;
