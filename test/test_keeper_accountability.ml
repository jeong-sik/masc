...
let append_accountability_event base_dir ~created_at json =
  ...
  let dir =
    Filename.concat base_dir ".masc/accountability"
    |> fun root -> Filename.concat root month
  in
      ...
      (fun () ->
         ...
         (try
            while true do
              ...
              if String.trim line <> ""
              then (
                try rows := Yojson.Safe.from_string line :: !rows with
                | Yojson.Json_error _ -> ())
            done
          with
          | End_of_file -> ());
...
let test_same_turn_evidence_marks_claim_supported () =
  with_room (fun config ->
    ...
    check string "routing hint" "normal_routing" (string_member "routing_hint" summary);
    check int "recent supported claims" 1 (int_member "recent_supported_claims" summary);
    check
      (float 0.0001)
      "evidence coverage"
      1.0
    ...
    let supporting_refs =
      Yojson.Safe.Util.(
        first |> member "supporting_evidence_refs" |> to_list |> List.map to_string)
    in
    check bool "contains turn reference" true (List.mem "turn:trace-1:7" supporting_refs))
;;

let test_stale_completion_claim_sets_high_risk () =
  with_room (fun config ->
    let created_at = iso_of_unix (Unix.gettimeofday () -. (25.0 *. 3600.0)) in
    append_accountability_event
      config.base_path
      ~created_at
      (`Assoc
...
let test_agent_reputation_penalizes_unsupported_claims () =
  with_room ~agent_name:"keeper-rep-agent" (fun config ->
    ignore
      (Coord.add_task config ~title:"Reputation task" ~priority:1 ~description:"desc");
    ignore (Coord.claim_task config ~agent_name:"keeper-rep-agent" ~task_id:"task-001");
    (match
     ...
     | Ok _ -> ()
     | Error err ->
       Alcotest.failf "done transition failed: %s" (Types.masc_error_to_string err));
    let created_at = iso_of_unix (Unix.gettimeofday () -. (25.0 *. 3600.0)) in
    append_accountability_event
      config.base_path
      ~created_at
    ...
    let unpenalized =
      Agent_reputation.compute_overall_score
        ...
        ~board_comments:0
    in
    let rep = Agent_reputation.compute_reputation config ~agent_name:"keeper-rep-agent" in
    check int "tasks completed" 1 rep.tasks_completed;
    check int "tasks claimed" 1 rep.tasks_claimed;
    check string "accountability risk band" "high" rep.accountability_risk_band;
    check
      (float 0.0001)
      "unsupported completion rate"
      1.0
    ...
    check
      (float 0.0001)
      "accountability score clamps to zero"
      0.0
      rep.accountability_score;
    check bool "overall reputation penalized" true (rep.overall_score < unpenalized))
;;
...
let test_generated_alias_inherits_accountability_penalty () =
  with_room ~agent_name:"adversary-eager-viper" (fun config ->
    ...
    let summary =
      Keeper_accountability.accountability_summary_json
        config
        ~keeper_name:"adversary"
        ~agent_name:"adversary-eager-viper"
    in
    check
      string
      "alias summary inherits canonical keeper risk"
      "high"
      (string_member "risk_band" summary);
    check
      string
      "alias summary source"
      "canonical_keeper_fallback"
      (string_member "source" summary);
    check
      string
      "alias summary source label"
      "Inherited from canonical identity: adversary"
      (string_member "source_label" summary);
    check
      (float 0.0001)
      "alias summary inherits unsupported rate"
      1.0
      (float_member "unsupported_completion_rate" summary);
    let rep =
      Agent_reputation.compute_reputation config ~agent_name:"adversary-eager-viper"
    in
    check string "generated alias risk band" "high" rep.accountability_risk_band;
    check
      (float 0.0001)
      "generated alias unsupported rate"
      1.0
      rep.accountability_unsupported_completion_rate;
    check
      (float 0.0001)
      "generated alias accountability score"
      0.0
      rep.accountability_score;
    check
      string
      "generated alias keeper provenance"
      "adversary"
      rep.accountability_keeper_name;
    check
      string
      "generated alias reputation source"
      "canonical_keeper_fallback"
      rep.accountability_source;
    check
      string
      "generated alias reputation source label"
      "Inherited from canonical identity: adversary"
...
let test_generated_alias_inherits_legacy_keeper_history () =
  with_room ~agent_name:"adversary-eager-viper" (fun config ->
    ...
    check
      string
      "legacy keeper summary source"
      "canonical_keeper_fallback"
      (string_member "source" summary);
    check
      string
      "legacy keeper source label"
      "Inherited from canonical identity: adversary"
      (string_member "source_label" summary);
    check
      (float 0.0001)
      "legacy keeper unsupported rate"
      1.0
      (float_member "unsupported_completion_rate" summary))
;;
...
let test_direct_agent_history_exposes_direct_source () =
  with_room ~agent_name:"keeper-direct-agent" (fun config ->
    ...
    let summary =
      Keeper_accountability.accountability_summary_json
        config
        ~keeper_name:"keeper-direct"
        ~agent_name:"keeper-direct-agent"
    in
    check string "direct summary source" "direct_agent" (string_member "source" summary);
    check
      string
      "direct summary source label"
      "Direct runtime alias history"
      (string_member "source_label" summary);
    let rep =
      Agent_reputation.compute_reputation config ~agent_name:"keeper-direct-agent"
    in
    check
      string
      "direct reputation keeper provenance"
      "direct"
      rep.accountability_keeper_name;
    check string "direct reputation source" "direct_agent" rep.accountability_source;
    check
      string
      "direct reputation source label"
      "Direct runtime alias history"
      rep.accountability_source_label)
;;
...
let test_task_transition_normalizes_keeper_name_in_history () =
  with_room ~agent_name:"keeper-sangsu-agent" (fun config ->
    ...
    check
      string
      "history keeper_name is canonical"
      "sangsu"
...
let test_task_transition_resolution_rows_include_identity () =
  with_room ~agent_name:"keeper-sangsu-agent" (fun config ->
    ...
    | Some row ->
      ...
      check string "resolution keeper_name" "sangsu" (string_member "keeper_name" row);
      check string "resolution task_id" "task-resolution" (string_member "task_id" row);
      check string "resolution kind" "task_commitment" (string_member "kind" row);
      check string "resolution subject" "task-resolution" (string_member "subject" row))
;;

let test_recent_decision_without_claim_exposes_coverage_gap () =
  with_room ~agent_name:"keeper-sangsu-agent" (fun config ->
    let now = Unix.gettimeofday () in
    append_decision_log_event
      config
      "sangsu"
      (`Assoc
    ...
    check
      string
      "summary source still no accountability history"
      "none"
      (string_member "source" summary);
    check
      string
    ...
    check
      string
      "routing hint"
      "accountability_coverage_gap_review"
      (string_member "routing_hint" summary);
    check
      string
      "source label mentions decision activity"
      "No accountability history; recent decision activity exists"
      (string_member "source_label" summary);
    check int "decision signal count" 1 (int_member "decision_signal_count" summary);
    check int "claim count" 0 (int_member "accountability_claim_count" summary))
;;
...
let test_unmapped_alias_exposes_no_history_source () =
  with_room ~agent_name:"orphan-eager-viper" (fun config ->
    ...
    in
    check string "unmapped alias source" "none" (string_member "source" summary);
    check
      string
      "unmapped alias source label"
      "No accountability history"
      (string_member "source_label" summary);
    check
      string
      "unmapped alias coverage health"
      "no_recent_activity"
      (string_member "coverage_health" summary);
    check bool "unmapped alias coverage gap" false (bool_member "coverage_gap" summary);
    check
      string
      "unmapped alias risk remains low"
      "low"
      (string_member "risk_band" summary);
    let rep =
      Agent_reputation.compute_reputation config ~agent_name:"orphan-eager-viper"
    in
    check
      string
      "unmapped alias keeper provenance"
      "orphan"
      rep.accountability_keeper_name;
    check string "unmapped alias reputation source" "none" rep.accountability_source;
    check
      string
      "unmapped alias reputation source label"
      "No accountability history"
      rep.accountability_source_label)
;;
...
let test_claim_tool_exposes_routing_warning_for_high_risk_keeper () =
  with_room (fun config ->
    ...
    check
      string
      "warning present"
      "⚠ Accountability risk is high for this keeper. Prefer manual review or lower-risk \
       routing when equivalent."
...
let test_preflight_exposes_routing_hint_for_high_risk_keeper () =
  with_room (fun config ->
    ...
    check
      string
      "routing hint exposed"
      "manual_review_recommended"
      (string_member "routing_hint" result);
    let repo_readiness = Yojson.Safe.Util.(result |> member "repo_readiness") in
    check
      string
      "repo readiness state exposed"
      "missing_clone"
      Yojson.Safe.Util.(repo_readiness |> member "state" |> to_string);
    check
      bool
      "repo readiness blocks code start without clone"
      false
...
let test_synthetic_claims_do_not_dilute_unsupported_rate () =
  ...
  with_room (fun config ->
    ...
    check
      (float 0.0001)
      "unsupported rate should be 1.0 not diluted"
      1.0
      (float_member "unsupported_completion_rate" summary);
    check string "risk band should be high" "high" (string_member "risk_band" summary))
;;
  ...
  with_room (fun config ->
    ...
    let read_count =
      Fun.protect
        ...
        (fun () ->
           let lookup = Keeper_accountability.accountability_summary_lookup config in
           let summary_a = lookup ~keeper_name:"keeper-a" ~agent_name:"keeper-a-agent" in
           let summary_b = lookup ~keeper_name:"keeper-b" ~agent_name:"keeper-b-agent" in
           check string "agent a risk" "high" (string_member "risk_band" summary_a);
           check string "agent b risk" "high" (string_member "risk_band" summary_b);
...
let test_attr_supported () =
  match KA.attribution_from_status KA.Supported ~evidence:sample_evidence () with
  | Some attr ->
...
let test_attr_unsupported_default_reason () =
  match KA.attribution_from_status KA.Unsupported ~evidence:sample_evidence () with
  | Some { outcome = A.Policy_failed { reason }; _ } ->
    check
      bool
      "default reason mentions evidence"
      true
...
let test_attr_expired () =
  match KA.attribution_from_status KA.Expired ~evidence:sample_evidence () with
  | Some { outcome = A.Policy_failed { reason }; _ } ->
    check
      bool
...
let () =
  ...
  Alcotest.run
    ...
    [ ( "accountability"
      , [ test_case
            "same turn evidence marks claim supported"
            `Quick
            test_same_turn_evidence_marks_claim_supported
        ; test_case
            "stale completion claim sets high risk"
            `Quick
            test_stale_completion_claim_sets_high_risk
        ; test_case
            "agent reputation penalizes unsupported claims"
            `Quick
            test_agent_reputation_penalizes_unsupported_claims
        ; test_case
            "generated alias inherits accountability penalty"
            `Quick
            test_generated_alias_inherits_accountability_penalty
        ; test_case
            "generated alias inherits legacy keeper history"
            `Quick
            test_generated_alias_inherits_legacy_keeper_history
        ; test_case
            "direct agent history exposes direct source"
            `Quick
            test_direct_agent_history_exposes_direct_source
        ; test_case
            "task transition canonicalizes keeper name in history"
            `Quick
            test_task_transition_normalizes_keeper_name_in_history
        ; test_case
            "task transition resolution rows include identity"
            `Quick
            test_task_transition_resolution_rows_include_identity
        ; test_case
            "recent decision without claim exposes coverage gap"
            `Quick
            test_recent_decision_without_claim_exposes_coverage_gap
        ; test_case
            "unmapped alias exposes no-history source"
            `Quick
            test_unmapped_alias_exposes_no_history_source
        ; test_case
            "claim tool exposes routing warning for high risk keeper"
            `Quick
            test_claim_tool_exposes_routing_warning_for_high_risk_keeper
        ; test_case
            "preflight exposes routing hint for high risk keeper"
            `Quick
            test_preflight_exposes_routing_hint_for_high_risk_keeper
        ; test_case
            "synthetic claims do not dilute unsupported rate"
            `Quick
            test_synthetic_claims_do_not_dilute_unsupported_rate
        ; test_case
            "summary lookup reads window once"
            `Quick
            test_summary_lookup_reads_window_once_for_multiple_agents
        ; test_case
            "summary json rereads window per agent"
            `Quick
    ...
    ; ( "attribution"
      , [ test_case "Pending → None" `Quick test_attr_pending_none
        ...
        ; test_case
            "Unsupported → Policy_failed (resolution reason)"
            `Quick
            test_attr_unsupported_with_reason
        ; test_case
            "Unsupported → Policy_failed (default reason)"
            `Quick
            test_attr_unsupported_default_reason
        ; test_case "Expired → Policy_failed" `Quick test_attr_expired
        ; test_case
            "Partial score clamped [0.5, 1.0]"
            `Quick
            test_attr_partial_score_clamped
        ; test_case
            "gate=accountability origin=Det invariant"
            `Quick
