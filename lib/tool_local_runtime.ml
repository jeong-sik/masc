...
let runtime_verify_json ?runtime_pool ?expected_slots ?expected_ctx ?expected_model () =
  ...
  let configured_capacity =
    ...
    |> List.fold_left
         ...
         0
  in
  let configured_max_concurrent_llm = Llm_cascade.max_concurrent_llm in
  let available_llm_permits = Llm_cascade.llm_semaphore_available () in
...
let runtime_status_json ?(include_models = true) () =
  ...
  `Assoc
    [
      ...
      ("configured_max_concurrent_llm", `Int Llm_cascade.max_concurrent_llm);
      ("available_llm_permits", `Int (Llm_cascade.llm_semaphore_available ()));
      ("llm_permits_in_use", `Int (Llm_cascade.llm_permits_in_use  ()));
