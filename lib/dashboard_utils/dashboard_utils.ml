...
let parse_iso_opt = function
  | Some raw when String.trim raw <> "" ->
    (try Some (Types.parse_iso8601 raw) with
     | Failure _ -> None)
  | _ -> None
;;
...
let string_contains ~needle haystack = Base.String.is_substring haystack ~substring:needle
...
let compact_text ?(max_len = 160) raw =
  ...
  then ""
    ...
    String_util.utf8_safe ~max_bytes:(max_len - 1 + 3) ~suffix:"\xe2\x80\xa6" normalized
    |> String_util.to_string
;;

let string_list_json values = `List (List.map (fun v -> `String v) values)
...
let is_keeper_offline status = List.mem status [ "offline"; "inactive"; "error" ]
