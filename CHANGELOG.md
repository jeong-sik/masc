...
- An approval whose first delivery failed still gets its decision on the ledger. The `Resolved` row, the chat record and the SSE `resolved` event went out only on the first commit, so when the wake enqueue failed after the journal, the operator's second press and every boot replay found no pending entry, delivered the wake and wrote no row; a grant the keeper consumed in between was skipped entirely. The trust timeline kept showing the approval as waiting for an operator decision. Each journaled delivery now carries `resolution_recorded`, set once the `Resolved` row is appended, and every completion writes the row while it is `false` (#38043).
...
- The gate approval store moves to version 11 (every delivery carries `resolution_recorded`), and a version 10 store no longer loads: the gate is unavailable with `gate_pending.version 10 is unsupported (current 11); reset runtime state before restarting MASC`. Before restarting on this build, move `<base-path>/.masc/gate/pending.json`, `pending.log.jsonl` and `replay-results.json` aside. What is lost is the journaled deliveries: unconsumed grants (the keeper asks again) and the tombstones of consumed ones; remembered rules live in `always-allowed.json` and stay (#38043).
