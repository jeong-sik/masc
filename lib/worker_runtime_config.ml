...
type team_session_spawn = {
  ...
  docker_scopes : Worker_contract_types.execution_scope list;
...
let execution_scope_of_string_opt value =
  ...
  | "observe_only" -> Some Worker_contract_types.Observe_only
  | "limited_code_change" -> Some Worker_contract_types.Limited_code_change
  | "autonomous" -> Some Worker_contract_types.Autonomous
  | _ -> None
