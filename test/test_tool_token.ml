...
let test_mint_unknown_tool () =
  match
    Tool_token.mint_with ~validate:(Hashtbl.mem full_tbl) ~name:"masc_nonexistent_xyz"
  with
  | Error _ -> ()
...
let test_mint_token_registered () =
  ...
  Tool_dispatch.register ~tool_name:tool ~handler:(fun ~name:_ ~args:_ ->
    Some (true, "ok"));
...
let test_dispatch_with_token () =
  ...
  Tool_dispatch.register ~tool_name:tool ~handler:(fun ~name ~args:_ ->
    Some (true, "dispatched:" ^ name));
  Tool_dispatch.register_name_tag ~tool_name:tool ~tag:Mod_misc;
  match Tool_dispatch.mint_token ~name:tool with
  | Error e -> fail e
  | Ok token ->
    (match Tool_dispatch.dispatch ~token ~args:`Null with
     | Some (true, msg) -> check string "dispatch result" ("dispatched:" ^ tool) msg
...
let () =
  run
    ...
    ; "token_properties", [ test_case "fields readable" `Quick test_token_name_readable ]
