...
let is_binary_file path =
  List.exists
    (fun ext ->
       ...
       && String.equal
            (String.sub path (String.length path - String.length ext) (String.length ext))
            ext)
    binary_extensions
;;
...
let normalize_agent_relative_path
      ~(config : Coord.config)
      ~(agent_name : string)
      (raw_path : string)
  : string
  ...
  let trimmed =
    ...
    then (
      ...
      in
      if String.starts_with ~prefix:(container_prefix ^ "/") trimmed
      then (
        let suffix =
          String.sub
            trimmed
        ...
        in
        Filename.concat (Filename.concat config.Coord.base_path own_bundle_rel) suffix)
      else trimmed)
    else trimmed
  in
  let trimmed =
  ...
  let is_playground_lane =
    ...
    && List.exists
         ...
         [ "mind"; "repos" ]
  in
  if is_playground_lane
  then Filename.concat (Filename.concat config.Coord.base_path own_bundle_rel) trimmed
  else trimmed
;;
...
let validate_path config path =
  try
    if String.contains path '\x00'
    then Error (IoError "Path contains null byte")
    else (
      ...
      | Some git_root ->
        let absolute_path =
          ...
          then Filename.concat config.Coord.base_path path
          else path
        in
        ...
        let within root candidate =
          String.starts_with ~prefix:(root ^ "/") candidate || String.equal candidate root
        in
        let allowed =
    ...
    (match
       ...
       | Unix.Unix_error _ ->
         ...
         Error
           (IoError
              (Printf.sprintf
                 "target path %S is not accessible (dangling symlink or racy deletion)"
                 path))
     with
     | Error e -> Error e
     | Ok canonical ->
       let playground_tree_rel = Playground_paths.all_playgrounds_prefix in
       let playground_tree_abs_raw = Filename.concat base_path playground_tree_rel in
       ...
       in
       if not target_mentions_playground_tree
       then
         ...
         match
           ...
           | Unix.Unix_error _ ->
             Error
               (IoError
                  (Printf.sprintf
                     "keeper playground tree %S does not exist; cannot validate \
                      cross-keeper containment. Clone via keeper_shell op=git_clone to \
                      provision your playground first. See #6527/#6637."
                     playground_tree_rel))
         with
         | Error e -> Error e
         | Ok playground_tree_canonical ->
           let is_under_any_playground =
             String.starts_with ~prefix:(playground_tree_canonical ^ "/") canonical
             || String.equal canonical playground_tree_canonical
           in
           if not is_under_any_playground
           then
             ...
             match
               ...
               | Unix.Unix_error _ ->
                 Error
                   (IoError
                      (Printf.sprintf
                         "keeper playground bundle %S does not exist yet; cannot \
                          validate containment. Provision via git_clone or \
                          masc_worktree_create first. See #6527/#6637."
                         own_rel_trailing))
             with
             | Error e -> Error e
             | Ok own_canonical ->
               if
                 String.equal canonical own_canonical
               ...
               then Ok canonical
                 ...
                 Error
                   (IoError
                      (Printf.sprintf
                         "cross-keeper playground read blocked: agent=%S tried to read \
                          path %S which is under another keeper's playground. Only %s is \
                          readable for this caller; reads outside your own playground \
                          must target the shared codebase (lib/, test/, config/, etc.). \
                          See #6527/#6637."
                         agent_name
                         path
                         own_rel_trailing)))))
...
let handle_code_search ctx args =
  ...
  if query = ""
  then false, "❌ Query required: 'query' parameter"
  else if String.trim path = ""
  then false, "❌ Path required: 'path' parameter"
  else (
    (* Validate path first *)
    ...
    | Ok search_path ->
      ...
      (match Process_eio.run_argv_with_status ~timeout_sec:30.0 cmd with
       | Unix.WEXITED 0, output ->
           ...
           List.filter_map
             (fun (json : Yojson.Safe.t) ->
                ...
                | `String "match" ->
                  ...
                  let path_str =
                    U.(data |> member "path" |> member "text" |> to_string)
                  in
                  (* lines is {"text": "..."} not a string *)
                  let line_content =
                    U.(data |> member "lines" |> member "text" |> to_string)
                  in
         ...
         in
         let response =
           `Assoc [ "count", `Int (List.length matches); "results", `List matches ]
         in
         true, Yojson.Safe.to_string response
       | Unix.WEXITED 1, _ ->
         (* No matches *)
         let response = `Assoc [ "count", `Int 0; "results", `List [] ] in
         true, Yojson.Safe.to_string response
       | Unix.WEXITED 2, output ->
         ...
         let hint =
           ...
           then
             Printf.sprintf
               "Search target not found. The 'path' argument resolved to %S — check that \
                it exists and is reachable from the keeper's allowed roots. \
                (Empty/missing 'path' defaults to '.')"
               search_path
           else if is_regex
           then
             "Regex error: check your pattern syntax, or set is_regex=false for literal \
              search."
           else
             "Literal search failed. If your query contains regex chars \
              (*+?[](){}^$|.\\), try simplifying the query or set is_regex=true with a \
              valid regex."
         in
         ( false
         , Printf.sprintf
             ...
             (if trimmed_out = ""
              then "(empty)"
                ...
                String_util.utf8_safe ~max_bytes:303 ~suffix:"..." trimmed_out
       ...
       | status, output ->
         ...
         , Printf.sprintf
             ...
             code
             (if output = ""
              then "(no output — check rg is in PATH and query is valid)"
              else output) )))
;;
...
let handle_code_symbols ctx args =
  ...
  then false, "❌ Path required: 'path' parameter"
    ...
    | Error e -> false, Types.masc_error_to_string e
    | Ok validated_path ->
      if not (Sys.file_exists validated_path)
      then false, Printf.sprintf "❌ File not found: %s" path
      else if is_binary_file validated_path
      then false, "❌ Binary file detected"
      else (
        ...
        then
          ( false
          , Printf.sprintf "❌ File too large: %d bytes (max: %d)" file_size max_file_size
          )
        else (
          try
            ...
            let rec extract_symbols acc line_num = function
              ...
              | line :: rest ->
                ...
                let try_keyword_extract keyword line =
                  if String.starts_with ~prefix:(keyword ^ " ") line
                  then (
                    let rest =
                      String.trim
                        (String.sub
                           ...
                           (String.length line - String.length keyword - 1))
                    in
                    (* Extract name: first identifier (alphanum + _) *)
                    let name_end = ref 0 in
                    while
                      !name_end < String.length rest
                      &&
                    ...
                    if !name_end > 0
                    then Some (String.sub rest 0 !name_end, keyword)
                    else None)
                  else None
                in
                let ocaml_keywords =
                ...
                let symbols =
                  match
                    List.find_map (fun kw -> try_keyword_extract kw trimmed) all_keywords
                  with
                  | Some (name, kind) ->
                    [ Some
                        (`Assoc
...
let schemas : Types.tool_schema list =
  [ (* masc_code_search *)
    ...
    ; description =
        "Search code using ripgrep. Literal match by default (is_regex=false). Use \
         simple words/phrases as query. Example: query='handle_request', \
         file_pattern='*.ml'. Set is_regex=true only for patterns like 'foo.*bar' or \
         'class \\w+'."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
            ...
            , `Assoc
                [ ( "query"
                  , `Assoc
                      ...
                      ; ( "description"
                        , `String
                            "Search pattern (literal by default, regex if is_regex=true)"
                        )
                      ] )
                ; ( "is_regex"
                  , `Assoc
                      ...
                      ; ( "description"
                        , `String
                            "Treat query as regex instead of literal string (default: \
                             false)" )
                      ; "default", `Bool false
                      ] )
                ; ( "path"
                  , `Assoc
                      ...
                      ; ( "description"
                        , `String
                            "Search path to scope the query (for example 'lib/', \
                             'test/', or 'workspace/yousleepwhen/masc-mcp/lib/')" )
                      ] )
                ; ( "file_pattern"
                  , `Assoc
                      [ "type", `String "string"
                      ; ( "description"
                        , `String "Glob pattern to filter files (e.g., '*.ml', '*.py')" )
                      ; "default", `String ""
                      ] )
                ; ( "case_insensitive"
                  , `Assoc
  ...
  ; (* masc_code_symbols *)
    ...
    ; description =
        "Extract symbols (functions, types, classes) from a file as a token-efficient \
         outline (~70% savings vs full read). Use when you need to understand a file's \
         structure without reading all content. Pair with masc_code_read to then read \
  ...
  ; (* masc_code_read *)
    { name = "masc_code_read"
    ; description =
        "Read a file with offset/limit pagination for token-efficient access to specific \
         sections. Use when you know the line range you need, especially for large \
         files. After masc_code_symbols identifies the relevant line numbers."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
            , `Assoc
                ...
                ; ( "offset"
                  , `Assoc
                      [ "type", `String "number"
                      ; ( "description"
                        , `String "Starting line number (0-indexed, default: 0)" )
                      ; "default", `Int 0
                      ] )
                ; ( "limit"
                  , `Assoc
