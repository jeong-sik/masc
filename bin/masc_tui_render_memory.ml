...
let memory_context_lines (k : memory_keeper_health) =
  ...
  (* RFC librarian-lifecycle §4.9: how far behind, when that was counted,
     and what the journal last said. A count the loop could not take prints
...
let render_memory_body ~cols ~budget (state : state)
  ...
  (match state.memory_health with
   ...
   | Some snapshot ->
       push (Printf.sprintf "  Ordinary: %d observed / %d derived · %d support invalidations · Librarian: %d turns unread · %d failures since server start"
         snapshot.mhs_total_observed_facts snapshot.mhs_total_derived_facts
         snapshot.mhs_total_support_invalidations
         snapshot.mhs_total_librarian_unread_turns snapshot.mhs_total_librarian_failures));
  push info_bar;
  let search_bar =
    if query <> "" then
