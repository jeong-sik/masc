...
시점에 고정된 본문을 읽으세요.
URL, 호스트 경로, commit, board 참조, 명령을 실행했다는 주장, 서술 노트는
...
기준으로 쓴 경로 앞에는 체크아웃 경로를 붙여야 합니다. 아래는 지금 루트에
...
스냅샷은 제출 시점의 상태이고, 조회 결과는 지금의 상태입니다. 스냅샷에
있던 파일을 지금 트리에서 찾지 못하면 경로·리비전·시점 차이를 확인하고
지금 상태를 따로 적으세요. 지금 없다는 이유만으로 제출 당시 기록을 거짓으로
보지 마세요.

코드를 읽어도 그 코드가 실행됐는지는 알 수 없습니다. 제출자가 빌드나
테스트가 통과했다고 하면 읽을 수 있는 실행 로그나 영수증이 있어야 합니다.
...
(이 producer에게는 playground 트리가 없습니다. {{root}}는 존재하지 않습니다. Keeper가 아닌 producer의 디렉터리는 따로 만들어지지 않으므로 여기서 artifact를 읽을 수 없습니다. 스냅샷에 기록된 typed 항목과, `masc_web_fetch`로 직접 연 URL 내용으로만 판정하세요.)
