...
- 다른 기억이 이미 같은 내용을 담는다는 이유로 없앨 때는 `dropped`에 적지 말고,
  그 기억의 문장을 글자 그대로 `new_claims`에 다시 쓰고 없앨 기억의 짧은 ID를 그
  claim의 `absorbs`에 적습니다. 그러면 묶기와 같은 확인을 거칩니다. `dropped`는
  이 확인을 거치지 않으므로, 빠진 문장은 그대로 사라집니다. 묶기가 실패해 남은
  재료를 다음에 "중복"으로 지우는 것도 여기에 해당합니다.
