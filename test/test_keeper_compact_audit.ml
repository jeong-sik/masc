...
let mk_start ?(id = "id-1") ?(ts = 1_000.0) ?(keeper = "k") ?(trig = KCA.Proactive) ()
  : KCA.start_record
  =
  { compaction_id = id
  ; ts_unix = ts
...
let test_pair_matching_pair () =
  ...
  | other -> fail (Printf.sprintf "expected 1 Paired, got %d rows" (List.length other))
;;

let test_pair_orphan_start () =
  let s = mk_start ~id:"c2" () in
  match KCA.pair_events [ KCA.Start s ] with
  | [ KCA.Orphan_start s' ] -> check string "orphan id" "c2" s'.compaction_id
...
let test_pair_orphan_complete () =
  ...
  | [ KCA.Orphan_complete c' ] -> check string "orphan complete id" "c3" c'.compaction_id
...
let test_pair_interleaved () =
  ...
  | other -> fail (Printf.sprintf "expected 2 Paired rows, got %d" (List.length other))
;;
...
let () =
  run
    ...
    [ "trigger", [ test_case "round-trip parse/to_string" `Quick test_trigger_roundtrip ]
    ; ( "pair_events"
