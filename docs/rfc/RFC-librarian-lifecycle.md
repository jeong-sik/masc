...
flowchart TD
  ...
  Q["받은 일이 바뀜"] --> S
  BOOT["서버 기동"] --> L1
  S --> L1["Librarian 루프가 디스크에서 읽는다<br/>진행 파일, 턴 끝 기록<br/>Keeper 마다 루프 하나, 서버 소유"]
  L1 --> C1{"읽은 위치보다 end_atom 이<br/>큰 줄이 있나"}
...
| 2 | 안 읽은 턴: 읽은 위치와 같은 trace 의 줄 가운데 `end_atom` 이 읽은 위치보다 큰 줄. 줄은 파일 위치가 아니라 `end_atom` 으로 줄 세운다 | 같음(trace), 정수 순서 | 안 읽은 턴 찾기. checkpoint 저장은 잠금 아래에서 직렬화되지만 줄을 쓰는 일은 그 잠금이 풀린 뒤라서 파일 순서는 턴 순서가 아닐 수 있다. 함께 끝난 두 턴은 `turn_ref` 도 같을 수 있다. 그래서 읽은 위치는 줄이 아니라 값(trace, `end_atom`, digest)이다 |
| 3 | 범위의 시작: 읽은 위치의 `end_atom`. 읽은 위치가 checkpoint 와 맞지 않으면 3c 가 정한다. 읽은 위치가 아직 없는 trace 는 그 trace 에 `Fresh_history` 줄이 있으면 0 이고, 없으면 `end_atom` 이 가장 작은 줄을 읽지 않고 위치의 기준점으로만 쓴다(기록 전부터 있던 이력의 첫 줄) | variant, 같음, 정수 순서 | 범위 계산 |
| 3c | 읽은 위치가 checkpoint 와 맞지 않을 때(그 자리 atom 의 digest 가 다르거나 그 atom 이 없다): 위치를 마지막으로 확인한 뒤에 파일에 더해진 줄 가운데, 같은 trace 의 `Fresh_history` 줄이나 `history_cleared` 줄이 있는가 | 같음, variant, 줄 번호 순서 | 있으면 이력이 새로 시작한 것이므로 0 부터 읽는다. 없으면 설명이 없는 불일치이므로 typed 오류로 선다(§4.10). 위치가 맞으면 그런 줄이 있어도 위치를 그대로 쓴다(§4.6 의 `keeper_clear`) |
...
- **I1 순서·빠짐없음** — 턴 끝 기록에 있는 턴은 하나도 빠짐없이 이력의 순서(`end_atom` 순서, §4.4 의 2)대로 읽힌다. Librarian 이 멈췄다 돌아오면 그 사이의 구간을 전부 읽는다.
...
- **I4 밀림이 보인다** — 읽은 위치 뒤에 끝난 턴의 수(§4.4 의 2 로 센 줄 수)가 typed 값으로 TUI 와 대시보드에 뜬다. Gate 가 아니다.
...
1. **턴 끝 기록** `<keepers_dir>/<keeper>.turn-boundaries.jsonl`
   ...
   - checkpoint 저장 뒤에 쓰고(`keeper_agent_run_finalize_response.ml` 의 checkpoint 저장 바로 다음), 쓴 뒤 Librarian 을 깨운다. 쓰기가 실패해도, 쓰다가 예외가 나도 턴은 실패하지 않는다(취소만 예외다). checkpoint 가 이미 저장됐고 턴의 영수증은 아직 없는 자리이기 때문이다. ERROR 로그와 metric 을 남긴다. 일시적인 실패로 빠진 줄의 내용은 잃지 않는다. 다음 줄의 범위가 앞 줄의 끝에서 시작하므로 두 턴을 같이 덮는다.
   ...
   - checkpoint 저장이 복구 사본을 쓴 드문 경우(payload 인코딩 거절 뒤 문제 json 을 뺀 사본), 저장이 돌려주는 값은 디스크의 값과 다르다(#37018). 마지막 atom 을 여는 메시지에 그 json 이 있었으면 digest 가 디스크와 어긋나고, 읽는 쪽은 §4.4 의 5 에서 선다. #37018 이 닫히면 사라지는 빈틈이다.
   ...
   - `keeper_clear` 도구는 checkpoint 의 메시지를 비우고 저장한다(`keeper_history_clear.ml`). 그 뒤의 턴은 checkpoint 를 불러와 시작하므로 `Continued_history` 인데 atom 번호는 0 근처에서 다시 시작한다. 이 사실은 비운 쪽만 안다. 그래서 `keeper_clear` 가 `history_cleared` 줄(`kind`, 시각, trace id)을 남긴다(§8 의 1b). System 메시지는 atom 이 아니므로 system prompt 를 남기든 말든 비운 이력은 atom 0개다. 그래서 이 줄에는 위치가 없다.
   - 줄은 비운 checkpoint 가 저장된 **뒤에만** 쓴다. 저장이 stale no-op 이거나 실패하면 줄을 쓰지 않고 도구는 오류를 돌려준다(전에는 둘 다 "N개 비움"으로 보고했다). 줄을 먼저 쓰면 안 되는 이유: 표식은 보이는데 이력은 아직 그대로인 순간에 읽는 쪽이 위치를 확인하면, 그 표식을 이미 지나간 것으로 친다. 그 뒤에 이력이 비워지면 불일치를 설명할 줄이 남지 않는다.
   - 읽는 쪽도 순서를 지킨다. **턴 끝 기록을 먼저 읽고 checkpoint 를 그다음에 읽는다.** 그래야 "읽은 줄에 표식이 있었는데 위치가 맞았다"가 "그 비우기는 되돌려졌거나 이미 지나갔다"를 뜻한다. 거꾸로 읽으면 비우기 전의 checkpoint 와 비운 뒤의 표식을 같이 보고 표식을 버리게 된다.
   - 이 줄은 읽은 위치를 옮기지 않는다. 읽는 쪽은 자기 위치가 checkpoint 와 맞지 않을 때만 이 줄을 찾는다(§4.4 의 3c). 턴이 도는 중에 비우면 그 턴이 끝나면서 자기 이력을 다시 저장해 비우기가 되돌려진다(`keeper_clear` 의 기존 결함, #37021). 그때는 위치가 그대로 맞으므로 줄은 아무 일도 하지 않는다.
   - 저장은 됐는데 줄을 못 쓰면(파일 끝이 잘려 있는 경우 등) 도구가 그 사실을 오류로 돌려준다. 다시 실행하면 줄이 쓰인다. 이미 빈 이력을 같은 `turn_count` 로 다시 저장하는 것은 통과하기 때문이다.
   - 비우기 전의 안 읽은 턴은 원문이 지워졌으므로 읽을 수 없다. 읽는 쪽은 0 부터 다시 시작할 때, 읽지 못한 채 지워진 양(그 trace 의 줄 가운데 가장 큰 `end_atom` 에서 읽은 위치를 뺀 값)을 저널에 남긴다.
   ...
   - 읽은 위치보다 앞에서 끝난 줄은 지워도 뜻이 같다. 지우는 일은 2단계에서 파일 읽는 비용을 재고 넣는다. 넣는다면 조건은 "읽은 위치보다 앞"뿐이고 크기나 줄 수 문턱은 두지 않는다. 줄을 지우면 줄 번호가 밀리므로, 진행 파일이 가진 "확인할 때 본 줄 수"(아래)를 같은 잠금 아래에서 같이 고쳐야 한다.
2. **진행 파일** `<keepers_dir>/<keeper>.librarian-progress.json`
   - 읽은 위치: trace id, `end_atom`, 그 자리 atom 의 digest. 줄을 가리키지 않고 값을 갖는다(§4.4 의 2).
   - 위치를 checkpoint 와 맞춰 확인했을 때 턴 끝 기록에 있던 줄 수도 같이 적는다. §4.4 의 3c 가 "확인한 뒤에 더해진 줄"을 가리는 데 쓴다. 파일에 더해진 순서는 턴 순서는 아니지만 줄이 보이기 시작한 순서이기는 하다. 3c 가 묻는 것은 뒤쪽이다.
...
| 1b | `keeper_clear` 가 비운 checkpoint 를 저장한 뒤 `history_cleared` 줄을 남긴다(§4.6). 저장이 stale no-op 이거나 실패한 비우기는 성공이 아니라 오류로 보고한다 | 줄을 읽는 곳이 없다. 도구의 보고가 사실과 맞게 바뀐다. 4단계 전에 있어야 한다 |
| 2 | 진행 파일 저장소와 순수 함수 둘: (턴 끝 기록, 진행 파일)에서 다음에 읽을 턴을 고르기, (checkpoint, 범위)에서 메시지를 자르기. I1·I9 와 §4.4 의 2~5(3c 포함)를 여기서 테스트한다. 두 파일을 배포 preflight 에 등록한다 | 부르는 곳이 없다 |
| 3 | 하네스(§9) | 저장소 밖 실행 |
...
**결정론으로 증명하는 것.** I1·I2·I3·I5·I9 와 §4.4 의 비교는 LLM 없이 테스트한다. 가짜 회차(항상 성공, 항상 실패, n번째에 실패, 두 턴 이상이면 실패)를 루프에 꽂아 본다: 턴 끝 기록 N 줄이 순서대로 빠짐없이 읽히는가, 실패 뒤에 위치가 그대로인가, 신호 없이 다시 돌지 않는가, 여러 턴을 읽다 실패한 다음 회차가 가장 오래된 한 턴만 읽는가, 도중에 취소하고 다시 띄우면 같은 위치부터 읽는가, `Fresh_history` 줄과 기준점 줄에서 범위의 시작이 맞는가, 위치가 맞지 않을 때 확인한 뒤에 더해진 `history_cleared` 줄이 있으면 0 부터 읽고 없으면 서는가, 위치가 맞으면 그 줄이 있어도 위치를 그대로 쓰는가, 이미 지나간 `history_cleared` 줄이 나중의 불일치를 설명하지 못하는가.
