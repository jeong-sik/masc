...
(* Hardcoded copy of ReceiptOutcomeSet from the spec, minus
   "receipt_unset" (initial-state-only, not a terminal). Kept in sync
   by hand for now — a future cycle may parse the .tla file directly
   so this list is no longer a second source of truth. *)
let spec_terminal_outcome_set =
  [
    "receipt_skipped";
    "receipt_done";
    "receipt_failed";
