...
let is_boundary_exempt ~tool_name ~input =
  Keeper_tool_registry.is_main_worktree_boundary_exempt_with_input ~tool_name ~input
;;

let mk_cmd cmd = `Assoc [ "op", `String "gh"; "cmd", `String cmd ]

(* Legacy helpers for backward compat with older tests (not exercised;
   keeper_shell op=gh only accepts cmd, not args). *)
let mk_args args = `Assoc [ "op", `String "gh"; "cmd", `String (String.concat " " args) ]
let mk_cmd_and_args cmd _args = `Assoc [ "op", `String "gh"; "cmd", `String cmd ]
let mk_action action = `Assoc [ "action", `String action ]
...
let make_meta ?current_task_id ?(sandbox_profile = Keeper_types.Local) () =
  let base_fields =
    ...
    ; "sandbox_profile", `String (Keeper_types.sandbox_profile_to_string sandbox_profile)
    ]
  in
  let fields =
...
let add_task_with_worktree ~config ~repo_dir ~repo_name =
  ...
  | task :: rest ->
    let updated_task =
      { task with
        worktree =
          Some { Types.branch = "main"; path = repo_dir; git_root = repo_dir; repo_name }
      }
    in
    Coord.write_backlog
      config
...
let test_read_only_case_insensitive () =
  Alcotest.(check bool)
    "PR LIST uppercase"
    true
    (is_ro ~tool_name:"keeper_shell" ~input:(mk_cmd "PR LIST"));
  Alcotest.(check bool)
    "Pr View mixed"
    true
    (is_ro ~tool_name:"keeper_shell" ~input:(mk_cmd "Pr View 123"))
;;

let test_prefixed_gh_command_normalization () =
  Alcotest.(check (option int))
    "prefixed gh parser still finds PR number"
    (Some 123)
    (match Keeper_gh_shared.extract_gh_target_number "gh pr view 123" with
     | Some (Keeper_gh_shared.PR, n) -> Some n
     | _ -> None);
  Alcotest.(check (option string))
    "prefixed gh parser still finds mutation"
    (Some "pr")
    (match Keeper_gh_shared.gh_mutates_entity "gh pr merge 123" with
     ...
     | None -> None);
  Alcotest.(check bool)
    "prefixed gh command stays read-only"
    true
    (is_ro ~tool_name:"keeper_shell" ~input:(mk_cmd "gh pr list --state open"));
  Alcotest.(check bool)
    "prefixed gh merge stays mutating"
    true
...
let test_parse_simple_gh_command_preserves_quoted_args () =
  match
    Keeper_gh_shared.parse_simple_gh_command "pr comment 123 --body 'looks good to me'"
  with
  | Ok cmd ->
...
let test_repo_flag_injection_prefixes_args () =
  ...
  | Ok cmd ->
    let updated =
      Keeper_gh_shared.gh_simple_command_with_repo_flag ~repo_slug:"new/repo" cmd
    in
    Alcotest.(check (list string))
      "repo flag injected at front"
...
let test_cmd_takes_precedence_over_args () =
  ...
  Alcotest.(check bool)
    "cmd=mutating overrides read-only args"
    false
    (is_ro
       ~tool_name:"keeper_shell"
       ~input:(mk_cmd_and_args "pr merge 123" [ "pr"; "list" ]));
  Alcotest.(check bool)
    "cmd=read-only overrides mutating args"
    true
    (is_ro
       ~tool_name:"keeper_shell"
...
let test_api_graphql_is_mutating () =
  let graphql_cmds =
    [ "api graphql -f query=repository"; "api graphql -f query=mergePullRequest" ]
  in
  List.iter
    (fun cmd ->
       Alcotest.(check bool)
...
let test_empty_input_not_read_only () =
  ...
  Alcotest.(check bool)
    "empty cmd"
    false
    (is_ro ~tool_name:"keeper_shell" ~input:(mk_cmd ""));
  Alcotest.(check bool)
    "whitespace cmd"
    false
    (is_ro ~tool_name:"keeper_shell" ~input:(mk_cmd "   "));
  Alcotest.(check bool)
    "empty args"
    false
    (is_ro ~tool_name:"keeper_shell" ~input:(mk_args []));
  ...
  Alcotest.(check bool)
    "no op (falls back to default read-only)"
    true
    (is_ro ~tool_name:"keeper_shell" ~input:(`Assoc []))
;;
...
let test_api_via_args () =
  Alcotest.(check bool)
    "api GET via args"
    true
    (is_ro ~tool_name:"keeper_shell" ~input:(mk_args [ "api"; "repos/owner/repo/pulls" ]));
  Alcotest.(check bool)
    "api POST via args"
    false
    (is_ro
...
let test_resolve_task_repo_context_uses_current_task_worktree () =
  with_repo_context_test_env
  @@ fun ~base:_ ~config ~repo_dir ->
  run_argv
    [ "git"
    ; "-C"
    ; repo_dir
  ...
  let task_id = add_task_with_worktree ~config ~repo_dir ~repo_name:"project" in
...
let test_keeper_shell_gh_without_current_task_uses_sandbox_context () =
  ...
  Alcotest.(check bool)
    "gh fails through docker route without image"
    false
  ...
  Alcotest.(check string) "cwd preserved" repo_dir (json |> member "cwd" |> to_string);
  Alcotest.(check bool)
    "repo omitted when sandbox fallback has none"
    true
    (json |> member "repo" = `Null);
  Alcotest.(check string)
    "docker image error surfaced"
...
let test_tool_call_observer_via_oas_event_bus () =
  ...
  Agent_sdk.Event_bus.publish
    bus
    (Agent_sdk.Event_bus.mk_event
       (Agent_sdk.Event_bus.ToolCalled
          { agent_name = "keeper-a"; tool_name = "keeper_shell"; input }));
  Agent_sdk.Event_bus.publish
    bus
    (Agent_sdk.Event_bus.mk_event
  ...
  (match called with
   | Some
       { payload =
           Agent_sdk.Event_bus.ToolCalled
             { agent_name; tool_name; input = observed_input }
       ; _
       } ->
  ...
  | Some
      { payload = Agent_sdk.Event_bus.ToolCompleted { agent_name; tool_name; output }; _ }
    ->
    Alcotest.(check string) "agent name" "keeper-a" agent_name;
    Alcotest.(check string) "tool name" "keeper_shell" tool_name;
...
let test_side_effect_tracking_via_event_bus () =
  ...
  in
  let mutating = ref [] in
  let pending : (string, Yojson.Safe.t Queue.t) Hashtbl.t = Hashtbl.create 4 in
      ...
      (fun (evt : Agent_sdk.Event_bus.event) ->
         ...
         | Agent_sdk.Event_bus.ToolCompleted { tool_name; output = Ok _; _ } ->
           let input =
             ...
             | None -> `Null
           in
           if Keeper_exec_tools.has_mutating_side_effect_with_input ~tool_name ~input
  ...
  let mut_input = `Assoc [ "op", `String "gh"; "cmd", `String "pr merge 123" ] in
  Agent_sdk.Event_bus.publish
    bus
    (Agent_sdk.Event_bus.mk_event
       (Agent_sdk.Event_bus.ToolCalled
          { agent_name = "keeper-a"; tool_name = "keeper_shell"; input = mut_input }));
  Agent_sdk.Event_bus.publish
    bus
    (Agent_sdk.Event_bus.mk_event
       (Agent_sdk.Event_bus.ToolCompleted
  ...
  let ro_input = `Assoc [ "op", `String "gh"; "cmd", `String "pr list" ] in
  Agent_sdk.Event_bus.publish
    bus
    (Agent_sdk.Event_bus.mk_event
       (Agent_sdk.Event_bus.ToolCalled
          { agent_name = "keeper-a"; tool_name = "keeper_shell"; input = ro_input }));
  Agent_sdk.Event_bus.publish
    bus
    (Agent_sdk.Event_bus.mk_event
       (Agent_sdk.Event_bus.ToolCompleted
...
let test_task_claim_is_mutating_but_boundary_exempt () =
  Alcotest.(check bool)
    "task claim is not read-only"
    false
    (is_ro ~tool_name:"keeper_task_claim" ~input:(`Assoc []));
  Alcotest.(check bool)
    "task claim bypasses boundary"
    true
...
let test_keeper_board_delete_and_cleanup_bypass_boundary () =
  let check name =
    ...
    Alcotest.(check bool)
      ...
      (is_boundary_exempt ~tool_name:name ~input:(`Assoc []))
  in
  List.iter check [ "keeper_board_delete"; "keeper_board_cleanup" ]
;;
...
let () =
  Alcotest.run
    "Keeper github read-only boundary"
    [ ( "read_only_cmd"
      , [ Alcotest.test_case "read-only cmd prefixes" `Quick test_read_only_cmd_prefixes
        ; Alcotest.test_case "case insensitive" `Quick test_read_only_case_insensitive
        ; Alcotest.test_case
            "prefixed gh command normalization"
            `Quick
            test_prefixed_gh_command_normalization
        ; Alcotest.test_case
            "simple gh parser preserves quoted args"
            `Quick
            test_parse_simple_gh_command_preserves_quoted_args
        ; Alcotest.test_case
            "simple gh parser rejects pipeline"
            `Quick
            test_parse_simple_gh_command_rejects_pipeline
        ; Alcotest.test_case
            "repo flag injection prefixes args"
            `Quick
            test_repo_flag_injection_prefixes_args
        ] )
    ; ( "read_only_args"
      , [ Alcotest.test_case "read-only via args field" `Quick test_read_only_via_args
        ; Alcotest.test_case
            "cmd takes precedence over args"
            `Quick
            test_cmd_takes_precedence_over_args
        ] )
    ; ( "mutating_cmds"
      , [ Alcotest.test_case
            "mutating cmds not read-only"
            `Quick
            test_mutating_cmds_not_read_only
        ] )
    ; ( "gh_api"
      , [ Alcotest.test_case
            "api GET default is read-only"
            `Quick
            test_api_get_default_is_read_only
        ; Alcotest.test_case
            "api with -X method is mutating"
            `Quick
            test_api_with_method_flag_is_mutating
        ; Alcotest.test_case
            "api with -f/-F is mutating"
            `Quick
            test_api_with_field_flag_is_mutating
        ; Alcotest.test_case "api graphql is mutating" `Quick test_api_graphql_is_mutating
        ] )
    ; ( "edge_cases"
      , [ Alcotest.test_case
            "empty input not read-only"
            `Quick
            test_empty_input_not_read_only
        ; Alcotest.test_case "non-gh tool" `Quick test_non_gh_tool
        ; Alcotest.test_case "api via args" `Quick test_api_via_args
        ; Alcotest.test_case
            "input-aware mutation detection"
            `Quick
            test_input_aware_mutation_detection
        ; Alcotest.test_case
            "dangerous gh classifier"
            `Quick
            test_dangerous_gh_command_classifier
        ; Alcotest.test_case
            "tool observer via OAS event bus"
            `Quick
            test_tool_call_observer_via_oas_event_bus
        ; Alcotest.test_case
            "side-effect tracking via OAS event bus"
            `Quick
            test_side_effect_tracking_via_event_bus
        ; Alcotest.test_case
            "task claim mutating but boundary exempt"
            `Quick
            test_task_claim_is_mutating_but_boundary_exempt
        ; Alcotest.test_case
            "masc_code_git write actions bypass boundary"
            `Quick
            test_masc_code_git_write_actions_bypass_boundary
        ; Alcotest.test_case
            "keeper_bash still opens boundary"
            `Quick
            test_keeper_bash_still_opens_boundary
        ; Alcotest.test_case
            "masc_* coordination aliases bypass boundary"
            `Quick
            test_masc_coordination_aliases_bypass_boundary
        ; Alcotest.test_case
            "keeper_board_delete and cleanup bypass boundary"
            `Quick
            test_keeper_board_delete_and_cleanup_bypass_boundary
        ] )
    ; ( "repo_context"
      , [ Alcotest.test_case
            "current task worktree resolves repo context"
            `Quick
            test_resolve_task_repo_context_uses_current_task_worktree
        ; Alcotest.test_case
            "missing worktree is structured error"
            `Quick
            test_resolve_task_repo_context_reports_missing_worktree
        ; Alcotest.test_case
            "non-github origin is structured error"
            `Quick
            test_resolve_task_repo_context_rejects_non_github_origin
        ; Alcotest.test_case
            "keeper_shell gh without current task uses sandbox context"
            `Quick
            test_keeper_shell_gh_without_current_task_uses_sandbox_context
        ] )
    ]
;;
