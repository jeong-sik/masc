# RFC-0286 — exec/keeper 스택: redirect & turn-termination 경계 소유권
...
**조사 항목 B1 (카운터 의미 불명확 — 결함 단정 철회)**: 로그에 `turn=14854 total_turns=1986`이 동시 등장한다. 카운터 정의:
- `total_turns` = `meta.runtime.usage.total_turns` (`keeper_hooks_oas.ml:437`) — runtime/usage 스코프 추정
- run summary 카운터 = `[turn; max_turns; ...]` (`agent_sdk_log_bridge.ml:166`) — keeper/run 스코프 추정

두 카운터가 **다른 스코프**(keeper 논리 턴 vs runtime usage)를 셀 수 있으므로, 역전 현상이 반드시 결함은 아니다. 단 (a) 의미 관계가 문서화되지 않았고, (b) 사용자 로그의 `x_turns=14855`는 상기 스키마 어디에도 대응되지 않아 **출처가 미확인**이다. §5 open question으로 이동. (초안에서 "SSOT 부재 결함"으로 단정했으나, 코드 재검증 결과 과장으로 철회.)

**설계 선택 B2 (watchdog 제거 — 결함 철회)**: `keeper_supervisor_launch.ml:161-181`에 따라 max-turn watchdog(`In_turn_hung`)은 **제거**되고 keepalive loop가 race 없이 직접 실행된다. 근거는 RFC-0250 "MASC must not impose wall-clock on turn" 원칙(메모리 `reference-masc-max-turn-watchdog-lifetime-cap-not-per-turn`과 일치). 따라서 keeper가 14855턴까지 도는 것은 **결함이 아니라 의도적 설계**의 결과이며, 종료 판정은 keepalive loop + ContractOk 등 다른 경로에 있다(별도 과제). (초안에서 "opt-in env로 존재, 부분 충돌"로 썼으나, 코드 주석으로 정정.)
...
- turn-termination: 카운터/keepalive-loop/ContractOk/state가 각자 종료·진행도를 판정. SSOT 또는 의존 관계 명시 없음 (→ B1 조사, B3 미확정).
...
- **INV-2 (turn 카운터 의미 문서화)**: `turn`/`total_turns`/`max_turns` 각 카운터가 **무엇을 세는지**(스코프)가 문서화되어야 하며, `x_turns` 같은 스키마 외 카운터는 출처가 명시되어야 한다. (B1이 약화됨에 따라 "단일 SSOT 강제"가 아닌 "의미 정합/문서화"로 조정.)
...
- **B1 상세**: `turn`/`total_turns`/`max_turns` 각 카운터의 정확한 스코프 정의, 그리고 로그 `x_turns` 출처. 역전 현상(turn > total_turns)이 실제 모순인지 다른 스코프의 정상 결과인지.
- **B2**: 해소됨 — max-turn watchdog은 RFC-0250에 의해 제거, keepalive loop로 대체. 종료 판정은 keepalive + ContractOk 경로로 별도 매핑 필요(→ B3).
