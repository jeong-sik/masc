...
let with_eio f =
  ...
  Fun.protect ~finally:(fun () -> Time_compat.clear_clock ()) (fun () -> f env)
;;
...
let test_list_masc_tools_exposes_board_and_keeper_schemas () =
  ...
  Eio.Switch.run (fun sw ->
    match
      Lib.Worker_runtime.list_masc_tools
        ~sw
        ~auth_token:None
        ~session_id:"worker-test"
        ~names:(Some [ "masc_board_post"; "masc_board_list" ])
        ()
    with
    | Ok schemas ->
...
let test_worker_runtime_config_prefers_env_override () =
  with_temp_dir "worker-runtime-config"
  @@ fun root ->
  ...
  @@ fun () ->
  with_env "MASC_WORKER_RUNTIME_BACKEND" None
  @@ fun () ->
  Lib.Config_dir_resolver.reset ();
  Lib.Worker_runtime_config.reset ();
  check
    string
    "file config enables docker backend"
    "docker"
    (Lib.Worker_execution_backend.to_string (Lib.Worker_runtime_config.backend ()));
  with_env "MASC_WORKER_RUNTIME_BACKEND" (Some "local_playground")
  @@ fun () ->
  Lib.Config_dir_resolver.reset ();
  Lib.Worker_runtime_config.reset ();
  check
    string
    "env override forces local backend"
    "local_playground"
    (Lib.Worker_execution_backend.to_string (Lib.Worker_runtime_config.backend ()))
;;
...
let test_worker_runtime_helper_protocol_roundtrip () =
  ...
  in
  let encoded =
    Lib.Worker_runtime_helper_protocol.success_json run_result |> Yojson.Safe.to_string
  in
  ...
  | Ok (Ok decoded) ->
    ...
    check (option int) "input_tokens" run_result.input_tokens decoded.input_tokens;
    check (list string) "tool_names" run_result.tool_names decoded.tool_names
  | Ok (Error payload) ->
    failf "expected success envelope, got helper error: %s" payload.message
  | Error err -> failf "expected helper envelope decode to succeed: %s" err
;;

let test_worker_runtime_invalid_config_fails_closed () =
  with_temp_dir "worker-runtime-invalid"
  @@ fun root ->
  let config_dir = make_config_root root in
  write_file
    (Filename.concat config_dir "worker-runtime.json")
  ...
  check
    string
    "malformed config resolves to fail-closed docker backend"
    "docker"
    (Lib.Worker_execution_backend.to_string (Lib.Worker_runtime_config.backend ()));
  check
    string
    "malformed config clears docker image"
    ""
...
let test_rewrite_custom_model_label_for_container () =
  ...
  check
    string
    "loopback custom label rewritten to host alias"
    "custom:qwen-test@http://host.docker.internal:19001"
    rewritten
;;
...
let test_run_process_with_timeout_returns_124_on_timeout () =
  ...
  let result =
    Lib.Worker_runtime_docker.run_process_with_timeout
      ~clock_opt:(Some (Eio.Stdenv.clock env))
      ~timeout_sec:1
      ~prog:"/bin/sh"
      ~argv:[ "/bin/sh"; "-c"; "trap '' TERM; kill -STOP $$" ]
...
let test_run_worker_oas_rejects_invalid_explicit_model_label () =
  with_temp_dir "worker-runtime-local"
  @@ fun root ->
  Eio_main.run
  @@ fun env ->
  Fs_compat.set_fs (Eio.Stdenv.fs env);
    ...
    (fun () ->
       ...
       match
         Lib.Worker_runtime.run_worker_oas
           ...
           ()
       with
       | Ok _ -> fail "expected invalid explicit model label to fail before execution"
       | Error err ->
         check
           bool
         ...
         check
           bool
           "does not use removed team-session stub"
           false
