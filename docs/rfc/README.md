...
| 0408 | 설치 마법사 — 이미 된 것은 감지하고 묻지 않는다 (모델 소스 × 실행 샌드박스) | Implemented | - |
...
| 0436 | 증거는 텍스트만이 아니라 파일이다 — binary artifact 를 해시로 못박고 judge 가 이미지로 보게 하는 채널 | Draft | - |
...
| RFC-keeper-self-authored-skills | 키퍼가 직접 만드는 Skill | Draft | - |
...
| RFC-last-path-resumes-after-progress | 마지막 경로가 잠깐 실패해도, 도구를 실행한 채팅 작업은 그 경로가 쉰 뒤 이어서 돈다 | Draft | - |
