...
describe('VerifyQueue', () => {
  ...
  // With no contract clause there is nothing to confirm, so approve stays
  // locked, the card is not pinned and the bar does not count it ready.
  it('keeps a submission gated when no clause is confirmed', () => {
    tasks.value = [makeTask({ contract: { completion_contract: [] } })]
    const { container } = render(html`<${VerifyQueue} />`)

    const approve = screen.getByText('✓ 승인 · 통과') as HTMLButtonElement
    expect(approve.disabled).toBe(true)
    expect(container.querySelector('.vq-card.pinned')).toBeFalsy()
    expect(container.querySelector('.vq-bar-stats')?.textContent).toContain('0 승인 준비')
  })
