(** Fusion 설정 작성기 — runtime.toml 의 [\[fusion\]] 구역을 typed 값으로 고친다.
    ...
    구역 안에서도 원래 줄이 기본이다.
    - 값이 그대로인 키는 줄을 그대로 둔다. 자리, 줄바꿈, 배열 안 주석, 줄 끝 주석,
      CRLF 줄 끝이 모두 남는다. 같은 preset 을 그대로 다시 쓰면 파일이 바이트
      그대로다.
    - 값이 바뀐 키는 그 자리에서 다시 쓴다. 위아래 주석은 남고, 원래 값 줄 안의
      주석(배열 안, 줄 끝)은 사라진다. 새로 쓰는 줄은 LF 로 끝난다.
    - preset 에서 빠진 키는 주석과 함께 지운다.
    - 파일에 없던 키는 그 표의 마지막 키 뒤에 붙인다. 기본값과 같은 선택 키
      ([label = ""], [web_tools = false], [min_answered = 1])는 파일에 없으면 새로
      쓰지 않는다.
    - 작성기가 모르는 키는 그대로 둔다. 항목이 있는 preset 본문의 그룹 키
      ([panel_system_prompt] 등)도 로더가 읽지 않으므로 그대로 둔다.

    주석은 빈 줄 없이 닿아 있는 키의 것이다. 키 아래 빈 줄 없이 이어진 주석은 그 키를
    따라가고, 그 밖의 주석은 아래 키나 항목 머리를 따라간다.

    항목(panels, judges)은 같은 라벨과 같은 경로를 가진 원래 항목의 줄을 이어받는다.
    그런 항목이 없으면 같은 라벨(비어 있지 않고 양쪽에서 하나뿐)인 항목을, 그것도
    없으면 아직 짝이 없는 다음 원래 항목을 이어받는다. 원래 항목이 남지 않으면 새로
    쓴다. 항목 순서는 preset 의 순서를 따르되, panels 와 judges 가 파일에서 섞여 있던
    자리는 그대로 쓴다.
...
type error =
  ...
  | Unaddressable_preset of string
      (** preset 이 자기 표 머리 없이 (점 키나 인라인 표로) 적혔거나, 그 preset 의 키나
          하위 표가 구역 밖에 있거나, 본문에 [panels]/[judges] 가 인라인 배열로
          있거나, 항목 옆에 평평한 [panel] 이 있거나, 항목의 라벨·경로가 읽히지
          않거나, 구역 안에 panels/judges 가 아닌 하위 표가 있다. 줄 편집으로는
          안전하게 고칠 수 없다. *)
  | Unaddressable_settings
      (** [fusion] 이 [\[fusion\]] 표 머리 없이 (루트의 점 키, 인라인 표, 스칼라로)
          적혀 있다. *)
...
val set_settings : string -> settings -> (string, error) result
