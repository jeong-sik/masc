...
let () = test "shutdown phases execute in order" (fun () ->
  ...
  Shutdown.initiate state
    ~reason:"test"
    ~notify_fn:(fun _reason -> notify_called := true)
    ~drain_check:(fun () -> true)  (* immediate drain *)
...
let () = test "shutdown ignores duplicate initiate" (fun () ->
  ...
  Shutdown.initiate state ~reason:"first"
    ~notify_fn:(fun _ -> ()) ~drain_check:(fun () -> true) ~exit_fn:(fun () -> ());
  Shutdown.initiate state ~reason:"second"
    ~notify_fn:(fun _ -> ()) ~drain_check:(fun () -> true) ~exit_fn:(fun () -> ());
...
let () = test "supervisor starts permanent children" (fun () ->
  Eio_main.run @@ fun env ->
  let clock = Eio.Stdenv.clock env in
  let started = ref false in
  let spec = Supervisor.child ~name:"worker"
    ~start:(fun () -> started := true)
    ~strategy:Permanent () in
...
let () = test "supervisor temporary child not restarted" (fun () ->
  Eio_main.run @@ fun env ->
  let clock = Eio.Stdenv.clock env in
  let call_count = ref 0 in
  let spec = Supervisor.child ~name:"temp"
    ~start:(fun () -> incr call_count; failwith "boom")
    ~strategy:Temporary () in
  let sup = Supervisor.create [spec] in
  Eio.Switch.run @@ fun sw ->
  Supervisor.start ~sw ~clock sup;
...
let () = test "supervisor status JSON serialization" (fun () ->
...
let () = test "supervisor permanent child restarts on failure" (fun () ->
  Eio_main.run @@ fun env ->
  let clock = Eio.Stdenv.clock env in
  let call_count = ref 0 in
  let spec = Supervisor.child ~name:"crasher"
    ~start:(fun () ->
      incr call_count;
      if !call_count <= 2 then failwith "crash"
      (* 3rd call succeeds *))
    ~strategy:Permanent
    ~max_restarts:5
    ~restart_window_s:60.0 () in
  let sup = Supervisor.create [spec] in
  Eio.Switch.run @@ fun sw ->
  Supervisor.start ~sw ~clock sup;
