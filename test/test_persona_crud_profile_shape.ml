...
let test_delete_removes_persona () =
  ...
  let error_key fields = List.mem_assoc "error" fields in
  (match Keeper_tool_persona_crud.handle_persona_delete_json (`Assoc [ ("persona_name", `String name) ]) with
   | `Assoc fields -> Alcotest.(check bool) "delete succeeds (no error field)" false (error_key fields)
   | _ -> Alcotest.fail "delete should return a JSON object");
  Alcotest.(check bool)
  ...
  (match Keeper_tool_persona_crud.handle_persona_delete_json (`Assoc [ ("persona_name", `String name) ]) with
   | `Assoc fields -> Alcotest.(check bool) "second delete reports error" true (error_key fields)
   | _ -> Alcotest.fail "delete should return a JSON object")
