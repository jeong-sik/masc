...
let task_linkage_source_opt (task : Types.task) goal_id =
  ...
  | Some _ -> None
  | None -> if task_is_linked_to_goal task goal_id then Some "title_tag" else None
;;
...
let task_is_done (task : Types.task) : bool = Types.task_status_is_done task.task_status
...
let dedupe_sort values = values |> List.sort_uniq String.compare
...
let receipt_cascade_fallback_applied json =
  json
  |> member "cascade"
  |> member "fallback_applied"
  |> to_bool_option
  |> Option.value ~default:false
;;

let receipt_outcome json = json |> member "outcome" |> to_string_option
let receipt_started_at json = json |> member "started_at" |> to_string_option
let receipt_ended_at json = json |> member "ended_at" |> to_string_option
let trust_disposition json = json |> member "disposition" |> to_string_option

let trust_disposition_reason json =
  json |> member "disposition_reason" |> to_string_option
;;

let trust_attention_reason json = json |> member "attention_reason" |> to_string_option

let trust_needs_attention json =
  json |> member "needs_attention" |> to_bool_option |> Option.value ~default:false
;;

let trust_turn_id json = json |> member "turn_id" |> to_int_option
...
let trust_sandbox_risk json =
  String.equal
    (json |> member "disposition_reason" |> to_string_option |> Option.value ~default:"")
    "sandbox_violation"
;;
...
let iso_max left right = if String.compare left right >= 0 then left else right

let latest_iso ?fallback values =
  match values with
  | [] -> fallback
  | first :: rest -> Some (List.fold_left iso_max first rest)
;;
...
let compute_convergence (goal : Goal_store.goal) linked_tasks children =
  ...
  let task_ratio =
    if task_count = 0
    then goal_done_weight
    else float_of_int done_count /. float_of_int task_count
  in
  let child_ratios = List.map (fun (child : tree_node) -> child.convergence) children in
  let child_avg =
  ...
  in
  if task_count > 0 && children <> []
  then (task_ratio +. child_avg) /. 2.0
  else if children <> []
  then child_avg
  else task_ratio
;;
...
let approval_matches_goal goal_id approval_json =
  let goal_ids =
    approval_json |> member "goal_ids" |> to_list |> List.filter_map to_string_option
  in
...
let keeper_name_of_assignee metas assignee =
  ...
  | None -> if keeper_name_matches_meta metas assignee then Some assignee else None
;;
...
let goal_health_reason
  ...
  | Some "blocked" ->
    (match goal_phase with
     ...
     | Goal_phase.Executing
     | Goal_phase.Awaiting_verification
     | Goal_phase.Awaiting_approval -> "Goal is blocked.")
  | Some _ | None ->
    if blocked_by_receipt
    then "Recent keeper execution ended with an error."
    else if child_blocked
    then "A linked sub-goal is blocked."
    else if pending_approvals > 0
    then Printf.sprintf "%d approval request(s) are still pending." pending_approvals
    else if sandbox_risk
    then "Linked keeper is constrained by the current sandbox or scope."
    else if cascade_risk
    then "Latest keeper run fell back within the configured cascade."
    else if fsm_risk
    then "Linked task is waiting on FSM verification or remediation."
    else if Option.is_some linkage_warning_reason
    then (
      ...
      | Some "unstaffed" -> "Linked tasks exist, but no keeper is assigned or linked."
      | Some reason -> reason
      | None -> "Goal linkage needs attention.")
    else if stalled
    then Printf.sprintf "No linked activity for %s." (human_duration stagnation_seconds)
    else if child_at_risk
    then "A linked sub-goal is at risk."
    else "Linked tasks and keepers are progressing."
;;
...
let tree_health ~goal_phase ~blocked_by_receipt ~child_blocked ~at_risk =
  ...
  | Some health -> health
  | None ->
    if blocked_by_receipt || child_blocked
    then "blocked"
    else if at_risk
    then "at_risk"
    else "on_track"
...
let rec build_tree context goals goal =
  ...
  in
  let direct_linkage_source = linked_tasks |> List.map snd |> link_source_of_values in
  let direct_pending_approvals =
    context.pending_approvals |> List.filter (approval_matches_goal goal.Goal_store.id)
  in
  let direct_task_keeper_names =
    linked_tasks
    |> List.filter_map (fun ((task, _) : Types.task * string) ->
      match task_assignee task with
      | Some assignee -> keeper_name_of_assignee context.keeper_metas assignee
      | None -> None)
  in
  ...
  in
  let child_blocked =
    List.exists (fun (child : tree_node) -> String.equal child.health "blocked") children
  in
  let child_at_risk =
    List.exists
      (fun (child : tree_node) ->
         String.equal child.health "at_risk" || String.equal child.health "blocked")
      children
  in
  ...
  in
  let blocked_by_receipt = List.exists receipt_has_error direct_receipts in
  ...
  let open_linked_task_count =
    linked_tasks
    |> List.filter (fun ((task, _) : Types.task * string) -> not (task_is_terminal task))
    |> List.length
  in
  ...
  let linkage_warning_reason =
    ...
    | Goal_phase.Awaiting_verification
    | Goal_phase.Awaiting_approval ->
      if linked_tasks = [] && children = [] && direct_linked_keeper_names = []
      then Some "no_linked_tasks"
      else if
        linked_tasks <> [] && open_linked_task_count = 0 && done_linked_task_count = 0
      then Some "no_open_work"
      else if open_linked_task_count > 0 && direct_linked_keeper_names = []
      then Some "unstaffed"
      else None
    | Goal_phase.Completed | Goal_phase.Blocked | Goal_phase.Paused | Goal_phase.Dropped
      -> None
  in
  let stalled =
  ...
  let direct_badges =
    tree_badges
      ~pending_approvals:(List.length direct_pending_approvals)
      ~sandbox_risk:direct_sandbox_risk
      ~cascade_risk:direct_cascade_risk
      ~fsm_risk:direct_fsm_risk
      ~stalled
  in
  let direct_badges =
    match linkage_warning_reason with
  ...
  let badges =
    dedupe_sort
      (direct_badges @ List.concat_map (fun (child : tree_node) -> child.badges) children)
  in
  ...
  let direct_infra_risk_count =
    List.length
      (List.filter
         (fun json ->
            receipt_has_error json
            || receipt_has_sandbox_risk json
  ...
  let linked_keeper_names =
    dedupe_sort
      (direct_linked_keeper_names
       @ List.concat_map (fun (child : tree_node) -> child.linked_keeper_names) children)
  in
  let linkage_source =
  ...
  let health =
    tree_health
      ~goal_phase:goal.Goal_store.phase
      ~blocked_by_receipt
      ~child_blocked
      ~at_risk
  in
  ...
  let blocking_source, blocking_reason =
    match goal.Goal_store.phase with
    | Goal_phase.Blocked | Goal_phase.Dropped -> "goal_phase", status_reason
    | Goal_phase.Completed
    | Goal_phase.Paused
    | Goal_phase.Executing
    | Goal_phase.Awaiting_verification
    | Goal_phase.Awaiting_approval ->
      if child_blocked
      then "child_goal", "A linked sub-goal is blocked."
      else if pending_approval_count > 0
      then "approval", status_reason
      else if Option.is_some direct_runtime_blocking_reason
      then
        ( "keeper_runtime"
        , Option.value direct_runtime_blocking_reason ~default:status_reason )
      else if direct_fsm_risk
      then "task_fsm", status_reason
      else if Option.is_some linkage_warning_reason
      then "goal_linkage", status_reason
      else if stalled
      then "stalled", status_reason
      else "none", status_reason
  in
  let latest_keeper_ref, latest_turn_ref =
    ...
    | [] -> None, None
  in
  let stalled_since = if stalled then Some last_activity_at else None in
  ...
  let context =
    ...
    ; latest_runtime_trusts =
        ...
        |> List.map (fun (meta : Keeper_types.keeper_meta) ->
          meta.name, Keeper_runtime_trust_snapshot.snapshot_json ~config ~meta)
    }
  in
  goals |> List.filter is_root |> List.map (build_tree context goals)
;;
...
let build_goal_verification_projection ~(config : Coord.config) goals =
  let requests =
    Goal_verification.read_state config
    |> fun (state : Goal_verification.state) -> state.requests
  in
  ...
  let goal_events =
    ...
    if Coord.path_exists config path then Fs_compat.load_jsonl path else []
  in
  ...
  List.iter
    (fun (goal : Goal_store.goal) ->
       match
         Goal_verification.effective_policy_for_nodes ~goals:policy_nodes ~goal_id:goal.id
       with
  ...
  List.iter
    (fun (request : Goal_verification.goal_verification_request) ->
       if request.status = Goal_verification.Open
       then Hashtbl.replace request_table request.goal_id request)
    requests;
  List.iter
    (fun json ->
    ...
    goal_events;
  ( (fun goal_id ->
      Option.value (Hashtbl.find_opt effective_policy_table goal_id) ~default:None)
  , (fun goal_id -> Hashtbl.find_opt request_table goal_id)
  , fun goal_id -> Option.value (Hashtbl.find_opt events_table goal_id) ~default:[] )
;;

let rec tree_node_to_json
          ?(effective_policy_for_goal = fun _ -> None)
  ...
  `Assoc
    ...
    ; ( "linked_keeper_names"
      , `List (List.map (fun keeper_name -> `String keeper_name) node.linked_keeper_names)
      )
...
let rec flatten_tree acc = function
  | [] -> List.rev acc
  | node :: rest -> flatten_tree (node :: acc) (node.children @ rest)
;;
...
let goal_event_timeline_json event =
  let event_type =
    event |> member "event_type" |> to_string_option |> Option.value ~default:"goal_event"
  in
  ...
  let title, summary, severity =
    ...
    | "goal_phase" ->
      let phase =
        payload |> member "phase" |> to_string_option |> Option.value ~default:"unknown"
      in
      let actor = payload |> member "actor" |> member "id" |> to_string_option in
      ( "Goal Phase"
      , (match actor with
    ...
    | "goal_verification_opened" ->
      ...
      let required =
        payload
        |> member "request"
        |> member "policy_snapshot"
        |> member "required_verdicts"
        |> to_int_option
      in
    ...
    | "goal_vote" ->
      ...
      let decision =
        vote |> member "decision" |> to_string_option |> Option.value ~default:"unknown"
      in
      let principal =
        vote
        |> member "principal"
    ...
    | "goal_verification_resolved" ->
      let status =
        payload |> member "status" |> to_string_option |> Option.value ~default:"unknown"
      in
...
let build_goal_timeline node linked_keepers approvals goal_events =
  let task_events =
    ...
    |> List.map (fun ((task, linkage_source) : Types.task * string) ->
      let status = task_status_label task in
      timeline_event_json
        ~ts:(task_updated_at task)
        ~kind:"task"
        ~lane:("task:" ^ task.id)
        ~title:task.title
        ~summary:(Printf.sprintf "%s · linkage=%s" status linkage_source)
        ~severity:
          (match status with
           | "cancelled" -> "bad"
           | "awaiting_verification" | "claimed" | "in_progress" -> "warn"
           | _ -> "ok"))
  in
  let approval_events =
    approvals
    |> List.filter_map (fun approval ->
      ...
      | Some requested_at ->
        let approval_id =
          approval |> member "id" |> to_string_option |> Option.value ~default:"approval"
        in
        let tool_name =
          approval
          |> member "tool_name"
  ...
  let keeper_events =
    ...
    |> List.filter_map (fun (detail : goal_detail_keeper) ->
      ...
      | Some event ->
        ...
        in
        let severity =
          event |> member "severity" |> to_string_option |> Option.value ~default:"warn"
        in
        let ts =
          event
          |> member "ts"
        ...
        (match detail.latest_receipt with
         ...
         | Some receipt ->
           (match receipt_ended_at receipt with
            | None -> None
            | Some ended_at ->
              let outcome = receipt_outcome receipt |> Option.value ~default:"unknown" in
              let severity =
                if receipt_has_error receipt
                then "bad"
                else if
                  receipt_has_sandbox_risk receipt || receipt_has_cascade_risk receipt
                then "warn"
                else "ok"
              in
              Some
...
let goal_detail_json ~(config : Coord.config) ~goal_id : (Yojson.Safe.t, string) result =
  ...
  match
    List.find_opt (fun (node : tree_node) -> String.equal node.goal.id goal_id) all_nodes
  with
  ...
  | Some node ->
            ...
            ; latest_receipt =
                List.assoc_opt
                  ...
                  (Keeper_execution_receipt.latest_json_by_keeper
                     config
                     node.linked_keeper_names)
            ; runtime_trust = Keeper_runtime_trust_snapshot.snapshot_json ~config ~meta
            }
        | Ok None | Error _ | Ok (Some _) -> None)
    in
    let approvals =
      match Keeper_approval_queue.list_pending_dashboard_json () with
      | `List items -> items |> List.filter (approval_matches_goal goal_id)
      | _ -> []
    in
    let latest_receipts =
      keeper_details
    ...
    Ok
      (`Assoc
          ...
          ; ( "timeline"
            , `List (build_goal_timeline node keeper_details approvals goal_events) )
          ])
;;
...
let dashboard_goals_tree_json ~(config : Coord.config) : Yojson.Safe.t =
  ...
  let overall_convergence =
    ...
    | roots ->
      let sum =
        List.fold_left (fun acc (node : tree_node) -> acc +. node.convergence) 0.0 roots
      in
  ...
  `Assoc
    ...
    ; ( "summary"
      , `Assoc
          ...
          ; "overall_convergence_pct", `Int (int_of_float (overall_convergence *. 100.0))
          ] )
    ]
;;
