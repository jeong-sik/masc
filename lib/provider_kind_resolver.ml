...
let resolve_builtin_provider provider_name model_id =
  ...
  | "kimi" ->
    Some
      (Registered
         { provider_name; model_id; kind = Llm_provider.Provider_config.OpenAI_compat })
  | _ -> None
;;
...
let resolve (spec : string) : resolution =
  ...
  | Some ("custom", rest) ->
    ...
    if model_id = ""
    then Unknown (Printf.sprintf "malformed custom spec %S: empty model id" spec)
    else Custom_url { model_id; base_url }
    ...
    (match resolve_builtin_provider provider_name model_id with
     ...
     | None ->
       ...
       (match Llm_provider.Provider_registry.find registry provider_name with
        | Some entry -> Registered { provider_name; model_id; kind = entry.defaults.kind }
        | None ->
          Unknown
            (Printf.sprintf
...
let kind_of_spec (spec : string) : Llm_provider.Provider_config.provider_kind option =
