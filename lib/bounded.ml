...
let default_retry_config =
  { max_retries = 3; base_delay_ms = 1000; max_delay_ms = 30000; jitter_factor = 0.2 }
;;
...
let check_constraints_with_buffer state =
  let avg_tokens_per_turn =
    if state.turns > 0
    then (state.tokens_in + state.tokens_out) / state.turns
    else state.constraints.token_buffer
  in
  let predicted_total = state.tokens_in + state.tokens_out + avg_tokens_per_turn in
  match state.constraints.max_tokens with
  | Some max when predicted_total > max ->
    Some
      (Printf.sprintf
         "Approaching token limit: %d + ~%d > %d"
...
let update_state state result =
  ...
  state.tokens_in <- state.tokens_in + Option.value result.input_tokens ~default:0;
  state.tokens_out <- state.tokens_out + Option.value result.output_tokens ~default:0;
  state.cost_usd <- state.cost_usd +. Option.value result.cost_usd ~default:0.0
;;
...
let bounded_run ~constraints ~goal ~agents ~prompt ~spawn_fn =
  ...
  | fallback_agent :: _ ->
    ...
    let rec loop () =
      ...
      if state.turns >= constraints.hard_max_iterations
      then
        { status = `Constraint_exceeded
        ; reason =
            Printf.sprintf
              "Hard iteration limit reached (%d)"
        ...
        | None ->
          ...
          let rec try_spawn attempt =
            let result =
              try Ok (spawn_fn agent prompt) with
            ...
            | Ok spawn_result ->
              ...
              if attempt < constraints.retry.max_retries && is_retryable_error err_msg
              then (
            ...
            | Error msg ->
              ...
              if attempt < constraints.retry.max_retries && is_retryable_error msg
              then (
                let delay_ms = calc_backoff_delay constraints.retry attempt in
                sleep_s (float_of_int delay_ms /. 1000.0);
                state.total_retries <- state.total_retries + 1;
          ...
          (match try_spawn 0 with
           ...
           | Ok (spawn_result, retries_used) ->
             ...
             let output_json =
               try Yojson.Safe.from_string spawn_result.output with
               | Yojson.Json_error _ -> `Assoc [ "raw", `String spawn_result.output ]
             in
...
let result_to_json result =
  ...
  `Assoc
    ...
    ; ( "final_output"
      , match result.final_output with
        | Some s -> `String s
        | None -> `Null )
    ; ( "stats"
      , `Assoc
...
let retry_config_of_json json =
  ...
  then default_retry_config
    ...
    ; jitter_factor =
        Safe_ops.json_float
          ~default:default_retry_config.jitter_factor
          "jitter_factor"
          retry
    }
;;
...
let constraints_of_json json =
  ...
  then default_constraints
    ...
    ; hard_max_iterations =
        Safe_ops.json_int
          ~default:default_constraints.hard_max_iterations
          "hard_max_iterations"
          json
...
let goal_of_json json =
  ...
  let condition =
    if member "eq" cond <> `Null
    then Eq (member "eq" cond)
    else if member "neq" cond <> `Null
    then Neq (member "neq" cond)
    else if member "lt" cond <> `Null
    then Lt (member "lt" cond |> to_float)
    else if member "lte" cond <> `Null
    then Lte (member "lte" cond |> to_float)
    else if member "gt" cond <> `Null
    then Gt (member "gt" cond |> to_float)
    else if member "gte" cond <> `Null
    then Gte (member "gte" cond |> to_float)
    else if member "between" cond <> `Null
    then (
      match member "between" cond |> to_list with
      | low :: high :: _ -> Between (low |> to_float, high |> to_float)
      | _ ->
        invalid_arg
          "Bounded.rule_of_yojson: 'between' array must have at least 2 elements")
    else if member "in" cond <> `Null
    then In (member "in" cond |> to_list)
    else Eq (`Bool true)
    (* Default: look for truthy value *)
  in
  { path; condition }
;;
