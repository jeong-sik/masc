...
let test_emits_both_counters () =
  ...
  let before_per_agent = counter_for_per_agent ~variant ~agent_name:agent ~force:false in
  Masc_mcp.Coord.record_fsm_drift_with_agent ~variant ~force:false ~agent_name:agent;
...
let test_per_agent_isolation () =
  ...
  let before_a = counter_for_per_agent ~variant ~agent_name:agent_a ~force:false in
  Masc_mcp.Coord.record_fsm_drift_with_agent ~variant ~force:false ~agent_name:agent_b;
  Alcotest.(check (float 0.0001))
...
let test_force_label_isolation () =
  ...
  let before_true = counter_for_per_agent ~variant ~agent_name:agent ~force:true in
  Masc_mcp.Coord.record_fsm_drift_with_agent ~variant ~force:false ~agent_name:agent;
  Alcotest.(check (float 0.0001))
...
let () =
  Alcotest.run
    "fsm_drift_per_agent_counter_9795"
    [ ( "metric_name"
      , [ Alcotest.test_case "canonical name stable" `Quick test_metric_name_stable ] )
    ; ( "emit"
      , [ Alcotest.test_case "emits to both counters" `Quick test_emits_both_counters ] )
    ; ( "isolation"
      , [ Alcotest.test_case "per-agent label isolation" `Quick test_per_agent_isolation
        ; Alcotest.test_case "force label isolation" `Quick test_force_label_isolation
        ] )
    ]
;;
