...
let test_validate_ok () =
  match
    Gate_protocol.validate ~max_content_length:4000 ~dedup_check:no_dedup (make_msg ())
  with
  | Ok () -> ()
...
let test_inbound_of_json_destination_id_wins_over_keeper_name () =
  ...
  in
  match Gate_protocol.inbound_of_json json with
  | Ok msg -> check string "destination_id takes precedence" "new-name" msg.keeper_name
  | Error e -> fail e
;;
...
let test_inbound_of_json_keeper_name_still_works () =
  ...
  in
  match Gate_protocol.inbound_of_json json with
  | Ok msg -> check string "keeper_name alone still routes" "legacy-only" msg.keeper_name
  | Error e -> fail e
;;

let test_outbound_emits_destination_id () =
  let out : Gate_protocol.outbound_message =
    { keeper_name = "luna"; content = "hi"; structured = None; turn_stats = None }
  in
  let json = Gate_protocol.outbound_to_json out in
  let open Yojson.Safe.Util in
  check
    string
    "destination_id present"
    "luna"
    (json |> member "destination_id" |> to_string);
...
let test_validation_error_strings () =
  ...
  check
    string
    "duplicate"
    "duplicate message (idempotency_key=k)"
    ...
    [ ( "validate"
      , [ test_case "accepts valid" `Quick test_validate_ok
        ...
        ; test_case
            ...
            `Quick
            test_validate_empty_idempotency_key
        ; test_case "rejects duplicate" `Quick test_validate_duplicate
        ; test_case
            "keeper checked before content"
            `Quick
            test_validate_keeper_name_checked_before_content
        ] )
    ; ( "json"
      , [ test_case "inbound basic" `Quick test_inbound_of_json_basic
        ...
        ; test_case
            ...
            `Quick
            test_inbound_of_json_accepts_destination_id
        ; test_case
            "destination_id wins over keeper_name"
            `Quick
            test_inbound_of_json_destination_id_wins_over_keeper_name
        ; test_case
            "inbound keeper_name still works"
            `Quick
            test_inbound_of_json_keeper_name_still_works
        ; test_case
            "outbound emits destination_id"
            `Quick
            test_outbound_emits_destination_id
