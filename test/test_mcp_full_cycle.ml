...
let test_identity_extraction () =
  let params =
    Fixtures.make_room_join_params ~room:"test-room" ~agent_name:"integration-agent"
  in
  let args =
    Yojson.Safe.Util.(
      params
...
let test_session_key_stability () =
  ...
  | Some updated -> check string "session_key unchanged" original_key updated.session_key
