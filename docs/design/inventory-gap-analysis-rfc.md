...
find .masc/perpetual-keepers -name "memory.jsonl" | wc -l
# 결과: 0
ls .masc/perpetual-keepers/ | wc -l
# 결과: 46
```
...
`.masc/` 디렉토리 구조 변경 시 (예: perpetual→keeper 리네이밍 #3627) 자동화된 마이그레이션 경로 없음. 수동 rename만 존재.
...
Wave 2 완료 조건: `find .masc/perpetual-keepers -name "memory.jsonl" | wc -l` > 0 + config 도구 등록 확인
...
find .masc/perpetual-keepers -name "memory.jsonl" -o -name "memory_bank.jsonl" | wc -l
