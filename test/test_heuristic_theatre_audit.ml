...
let counter_value metric ~labels = Prom.metric_value_or_zero metric ~labels ()
...
let test_thompson_counter_accepts_label_tuple () =
  (* Pin the {agent, trigger} label shape so Grafana rules
     ...
     we care about. *)
  let mentioned_labels = [ "agent", "unused-test-9919"; "trigger", "mentioned" ] in
  let content_alert_labels =
    [ "agent", "unused-test-9919"; "trigger", "content_alert" ]
  in
  Alcotest.(check (float 0.0001))
    "mentioned baseline is 0"
    0.0
    (counter_value TS.priority_trigger_selected_metric ~labels:mentioned_labels);
  Alcotest.(check (float 0.0001))
    "content_alert baseline is 0"
    0.0
    (counter_value TS.priority_trigger_selected_metric ~labels:content_alert_labels)
;;

let () =
  Alcotest.run
    "heuristic_theatre_audit_9919"
    [ ( "board_core_classify"
      , [ Alcotest.test_case "metric name stable" `Quick test_board_metric_name_stable
        ; Alcotest.test_case
            "author label accepted"
            `Quick
            test_board_counter_accepts_author_label
        ] )
    ; ( "thompson_sampling"
      , [ Alcotest.test_case "metric name stable" `Quick test_thompson_metric_name_stable
        ; Alcotest.test_case
            "label tuple pinned"
            `Quick
