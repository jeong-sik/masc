...
- Deploy: `deployment_preflight_helper validate-stores` reads the Librarian's four files with the production decoders — the turn-boundary log, the read position, the official-turn position and the range receipt — and a row the runtime could not read stops the deploy like any other store's. Each row's report says what the runtime does with such a row: a refused boundary line stops the keeper's Librarian pass, a refused position file is never read as "never read", a refused receipt refuses every Memory write for that keeper.
- Librarian: each keeper's Librarian runs in a loop the server owns (RFC librarian-lifecycle §4.3). The loops start with the server, one per keeper whose metadata is on disk, stopped keepers included, and each runs once at once; from then on a loop runs when a turn records its end line or the keeper's pending inputs change, drains every unread turn while each pass advances, and parks. A keeper that is stopped has its backlog read without coming back; a keeper that is created gets its loop when its metadata is written; a keeper's launch no longer submits a catch-up of its own. A dashboard purge retires the keeper's loop before it removes the keeper's files and releases it after, so no pass writes a read position for a keeper that is gone. The old queue driver (`keeper_librarian_queue_refresh`) with its remembered turn-end closure is gone.
...
- Librarian: an offline checkpoint purge moves the Librarian's read position with the checkpoint instead of leaving it against the old numbering (RFC librarian-lifecycle §10-2, #37361). The dashboard action and `masc-checkpoint-purge --apply` refuse the rewrite while the position is short of the history's end — an unread atom has no place in the rewritten history — and otherwise install the checkpoint and then write the position at the rewritten end, its `boundary_lines_seen` untouched; the preview names the refusal in its warnings. The dashboard retires the keeper's Librarian loop around the two writes and wakes it after; the CLI takes the workspace writer lease and refuses while a server holds it. The old check compared only the checkpoint's final atom, so a rewrite that changed an earlier one was installed and every later read stopped on the mismatch. The `LibrarianRead-purge-trim*` models in `specs/bug-models/` have their code counterparts in `test_keeper_checkpoint_purge.ml`, one test per rule the models measure.
- Librarian: the registry row and the journal line a pass writes after its Memory commit are written under a cancel shield. A cancellation that landed between them — the server going down mid-pass — left the row saying one thing and the journal another; now both are down before the cancellation is taken, and the cancellation branch finds the row completed and writes no second one.
