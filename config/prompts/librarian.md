...
다만 진행 중인 일의 현재 위치는 기억이 아니라 아래 `working_state`가 이어 줍니다.
기억에는 그 일에서 얻은 결정·이유·제약·교훈을 남깁니다.
...
- 현재 기억 중 진행 상태만 담은 것은 삭제 후보입니다. 작성 경로가 `authored`여도
  같습니다. 그 안에 결정·이유·교훈이 섞여 있으면 그 부분만 새 claim으로 남기고
  원래 기억은 교정 규칙대로 `supersedes`로 바꿉니다. 같은 대상을 시점만 달리
  적은 기억이 여러 개면 가장 최근 것만 뜻이 있고, 나머지는 낡은 상태입니다.
- 반복되는 개념이나 주제에 대한 정보면, 그 주제를 중심으로 묶어서 하나로
  ...
  묶을 거리가 없으면 묶지 않습니다.
- 묶은 claim은 재료가 말하던 것을 하나도 빠뜨리면 안 됩니다. 묶기는 재료를 문장
  단위로 쪼개 "이 claim이 그 문장을 말하는가"를 문장마다 확인한 뒤에야 적용되고,
  한 문장이라도 빠지면 그 재료는 현재 기억에 그대로 남습니다. 그러면 묶기는 없던
  일이 되고 claim만 한 건 늘어납니다. 조건·수치·예외·고유명은 줄이지 말고 그대로
  옮기세요. 짧게 만드는 것이 목적이 아닙니다.
- 그래서 조항이 많은 기억끼리 묶으면 대개 실패합니다. 한 claim이 실제로 다 말할
  수 있는 만큼만 재료로 삼고, 나머지는 다음에 묶으세요. 재료를 늘리는 것보다
...
- 한 claim은 한 주제만 답습니다. 주제가 둘이면 claim을 둘로 나누세요. 조건·예외·
...
- 깨어남, 턴 진행, 도구 호출 자체, 체크포인트, 현재 큐 상태, 순간적인 욕구,
  ...
  남깁니다.
- 진행 상태는 claim이 아닙니다. 지금 어디까지 왔는지, 마지막 체크포인트, 다음
  조작, PR·이슈 번호와 머지·리뷰·CI 상태는 다음 턴에 필요해도 `working_state`에
  적거나 원래 있는 곳(GitHub, task board, 게임 저장 파일)에 맡깁니다. 기억에
  넣으면 시점마다 한 벌씩 쌓이고, 쌓인 기억은 매 턴 전부 다시 전달됩니다.
