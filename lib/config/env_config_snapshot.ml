...
let server_entries =
  [ entry
      ~default:Masc_network_defaults.masc_http_default_port_s
      Env_config_core.http_port_env_key
      "HTTP server port"
  ; entry
      ~default:Env_config_core.default_host
      Env_config_core.host_env_key
      "Server bind host"
  ; entry
      ~default:"(derived)"
...
let runtime_entries =
  ...
  ; entry
      ~default:"true"
      Env_config_core.telemetry_enabled_env_key
...
let transport_entries =
  ...
  ; entry
      ~default:"48"
      "MASC_GRPC_STREAM_MAX_BUFFER"
      "Per-subscriber outbound buffer drop threshold.  When the stream has this many \
       unsent events queued, new events are dropped and masc_grpc_events_dropped_total \
       advances.  Stream capacity is 64, default leaves headroom."
  ...
  ; entry
      ...
      "When true (default), slice-scoped events skip the raw-SSE-forward to \
       authenticated WS sessions whose route does not subscribe to the event's slice. \
       Catch-all events (no slice mapping) still reach every session. \
       masc_ws_slice_fanout_skipped_total advances per skip. RFC #10119 Phase 2. Set to \
       false for emergency rollback only."
...
let keeper_entries =
  ...
  ; entry
      ~default:"true"
      "MASC_STRUCTURED_STATE"
      "Enable structured JSON state in checkpoints (default true; set to \"false\" to \
       opt out)"
  ; entry ~default:"(none)" "MASC_TLA_TRACE" "Enable TLA+ trace emission"
  ]
;;
...
let dashboard_entries =
  ...
  ; entry
      ~default:"(none)"
      "MASC_DASHBOARD_CACHE_MAX_ENTRIES"
...
let cache_entries =
  [ entry ~default:"1000" "MASC_CACHE_MAX_ENTRIES" "Maximum total number of cache entries"
  ; entry
      ~default:"102400"
...
let circuit_breaker_entries =
  [ entry
      ~default:"(none)"
      "MASC_CIRCUIT_COOLDOWN"
      "Cooldown before half-open retry (seconds)"
  ; entry
      ~default:"(none)"
      "MASC_CIRCUIT_FAILURE_WINDOW_SEC"
      "Failure counting window (seconds)"
  ; entry
      ~default:"(none)"
      "MASC_CIRCUIT_THRESHOLD"
      "Failure threshold before circuit opens"
  ]
;;

let cli_entries = [ entry ~default:"auto" "MASC_CLI_AGENT" "CLI default agent name" ]
...
let compaction_entries =
  ...
  ; entry ~default:"0.35" "MASC_COMPACT_W_ROLE" "Weight for role in compaction scoring"
  ; entry ~default:"0.15" "MASC_COMPACT_W_TOOL" "Weight for tool in compaction scoring"
  ; entry
      ~default:"0.95"
...
let keeper_sandbox_entries =
  [ entry
      ...
      "Strict Docker keeper mode: rootless/userns required, network=none, brokered \
       git/gh only"
  ; entry
      ~default:
...
let economy_entries =
  [ entry ~default:"false" "MASC_ECONOMY_ENABLED" "Agent economy feature flag"
  ; entry ~default:"5.0" "MASC_ECONOMY_FRUGAL_THRESHOLD" "Frugal behavior threshold"
  ; entry ~default:"0.0" "MASC_ECONOMY_HUSTLE_THRESHOLD" "Hustle behavior threshold"
  ; entry ~default:"5.0" "MASC_ECONOMY_INITIAL_BALANCE" "Initial agent balance"
  ; entry
      ~default:"true"
      "MASC_ECONOMY_REPUTATION_MULTIPLIER"
      "Enable reputation multiplier"
  ; entry ~default:"1.0" "MASC_ECONOMY_REWARD_BOARD_POST" "Reward for a board post"
  ; entry
      ~default:"0.5"
      "MASC_ECONOMY_REWARD_MENTION_RESPONSE"
      "Reward for responding to a mention"
  ; entry ~default:"10.0" "MASC_ECONOMY_REWARD_TASK_DONE" "Reward for completing a task"
  ; entry ~default:"0.5" "MASC_ECONOMY_REWARD_UPVOTE" "Reward for receiving an upvote"
  ]
;;
...
let file_lock_entries =
  [ entry
      ~default:"(none)"
      "MASC_FILE_LOCK_MAX_ENTRIES"
      "Maximum concurrent lock entries (clamped 64-4096)"
  ; entry
      ~default:"(none)"
      "MASC_FILE_LOCK_STALE_SEC"
...
let internal_timer_entries =
  ...
  ; entry
      ~default:"900.0"
      "MASC_LABEL_STUCK_THRESHOLD_SEC"
  ...
  ; entry ~default:"300.0" "MASC_SSE_BUFFER_TTL_SEC" "SSE buffer TTL (seconds, 5 min)"
  ; entry
      ~default:"300.0"
...
let keeper_board_entries =
  [ entry
      ~default:"(none)"
      "MASC_KEEPER_BOARD_LIST_CACHE_TTL_S"
...
let keeper_cascade_entries =
  [ entry
      ~default:"(none)"
      "MASC_KEEPER_CASCADE_PROVIDER_ALLOWLIST"
...
let keeper_keepalive_entries =
  ...
  ; entry
      ~default:"30.0"
      "MASC_KEEPER_AUTONOMOUS_SLOT_WAIT_TIMEOUT_SEC"
...
let keeper_metrics_entries =
  ...
  ; entry ~default:"1" "MASC_KEEPER_METRICS_MAX_ROTATED" "Number of rotated files to keep"
  ]
;;
...
let keeper_tool_entries =
  [ entry
      ~default:"(none)"
      "MASC_KEEPER_LLM_RERANK_CASCADE"
...
let local_runtime_entries =
  ...
  ; entry
      ...
      "Upper bound for local runtime requests (primary; falls back to \
       MASC_LLAMA_MAX_TOKENS)"
  ; entry ~default:"(none)" "MASC_MCP_URL" "MASC MCP endpoint URL"
  ]
;;
...
let model_routing_entries =
  ...
  ; entry ~default:"(none)" "MASC_DEFAULT_MODEL" "Default model id; None when unset"
  ; entry
      ~default:"(none)"
      "MASC_DEFAULT_PROVIDER"
      "Default provider name; None when unset"
  ; entry ~default:"task" "MASC_GOAL_DISPATCH_RUNTIME" "Goal dispatch runtime type"
  ; entry
      ~default:"(none)"
      "MASC_GOAL_MODELS"
...
let orchestrator_entries =
  [ entry ~default:"orchestrator" "MASC_ORCHESTRATOR_AGENT" "Orchestrator agent name"
  ; entry
      ~default:"(none)"
...
let path_entries =
  [ entry
      ~default:"false"
      "MASC_ALLOW_REPO_CONFIG_FALLBACK"
      "Allow repo config fallback resolution"
  ; entry ~default:"(none)" "MASC_ASSETS_DIR" "Assets directory override; None when unset"
  ; entry
      ~default:"(none)"
  ...
  ; entry
      ~default:"(none)"
      Env_config_core.data_dir_env_key
      "Data directory override; None=<base_path>/data"
  ; entry
      ~default:"(none)"
      Env_config_core.personas_dir_env_key
...
let relay_entries =
  [ entry ~default:"auto" "MASC_RELAY_TARGET_AGENT" "Relay target agent name" ]
;;
...
let telemetry_entries =
  ...
  ; entry
      ~default:"(none)"
      Env_config_core.log_level_env_key
      "Log level string (debug|info|warn|error)"
  ; entry
      ~default:"false"
      Env_config_core.parse_warn_env_key
      "Whether to log parse warnings"
  ; entry ~default:"(none)" "MASC_OTEL_ENABLED" "Enable OpenTelemetry span collection"
  ]
;;
...
let timeout_entries =
  [ entry
      ~default:"300"
      "MASC_A2A_DELEGATION_TIMEOUT_SEC"
      "A2A task delegation timeout (seconds)"
  ; entry ~default:"100" "MASC_EVENT_BUFFER_SIZE" "A2A event buffer size per subscription"
  ; entry
      ~default:"30.0"
...
let tool_entries =
  ...
  ; entry ~default:"512" "MASC_LIST_PAGE_SIZE" "Tool list page size (clamped 10-1024)"
  ; entry
      ~default:"(none)"
      "MASC_MAX_WRITE_SIZE_BYTES"
      "Maximum write size in bytes (1MB)"
  ; entry
      ~default:"(none)"
      "MASC_PLACEHOLDER_TOOLS_ENABLED"
  ...
  ; entry ~default:"2" "MASC_TOOL_READONLY_RETRY_LIMIT" "Read-only tool retry limit"
  ]
;;

let web_search_entries =
  [ entry ~default:"(none)" "MASC_SEARXNG_URL" "SearXNG instance URL; None when unset"
  ; entry
      ~default:"30.0"
...
let all_categories () =
  [ category
      "server"
      (server_entries @ path_entries @ docker_playground_entries @ test_entries)
  ...
  ; category
      "inference"
      (inference_entries @ model_routing_entries @ oas_sse_entries @ local_runtime_entries)
  ; category
      "keeper"
      (keeper_entries
       @ keeper_alert_entries
       @ keeper_bootstrap_entries
       @ keeper_keepalive_entries
       @ keeper_metrics_entries
       @ keeper_board_entries
       @ docker_playground_entries
       @ keeper_sandbox_entries)
  ; category
      "keeper_execution"
      (keeper_execution_entries
       @ compaction_entries
       @ decision_entries
       @ keeper_tool_entries
       @ keeper_cascade_entries
       @ keeper_proactive_entries
       @ keeper_grpc_entries)
...
let valid_config_category_strings = all_categories () |> List.map fst
