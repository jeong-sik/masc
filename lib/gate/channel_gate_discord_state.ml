...
let status_path () =
  Names.configured_read_path
    "MASC_DISCORD_STATUS_PATH"
    ~default:default_status_path
    ~legacy:legacy_status_path
;;

let status_write_path () =
  Names.configured_write_path "MASC_DISCORD_STATUS_PATH" ~default:default_status_path
;;

let binding_store_path () =
  Names.configured_write_path
    "MASC_DISCORD_BINDING_STORE_PATH"
...
let binding_store_read_path () =
  Names.configured_read_path
    "MASC_DISCORD_BINDING_STORE_PATH"
    ~default:default_binding_store_path
    ~legacy:legacy_binding_store_path
;;

let binding_audit_path () =
  Names.configured_write_path
    "MASC_DISCORD_BINDING_AUDIT_PATH"
...
let binding_audit_read_path () =
  Names.configured_read_path
    "MASC_DISCORD_BINDING_AUDIT_PATH"
    ~default:default_binding_audit_path
    ~legacy:legacy_binding_audit_path
;;
...
let read_recent_audit ~limit =
  ...
  if limit <= 0 || not (Sys.file_exists path)
  then []
  else (
...
let bool_option_member json key = json |> U.member key |> U.to_bool_option
...
let status_json ?(audit_limit = 10) () =
  ...
  `Assoc
    ...
    ; "last_ready_at", `String (status_field "last_ready_at" string_member "")
    ; "bot_user_name", `String (status_field "bot_user_name" string_member "")
    ...
    ; "binding_source", `String (status_field "binding_source" string_member "")
    ; "runtime_bindings_count", `Int (status_field "runtime_bindings_count" int_member 0)
...
let connector_json ?gate_status_json ?(audit_limit = 10) () =
  ...
  let runtime_summary =
    `Assoc
      ...
      ; "gate_health_checked_at", `String (string_member status "gate_health_checked_at")
  ...
  let binding_summary =
    `Assoc
      ...
      ; ( "configured_bindings_count"
        , `Int (status |> U.member "configured_bindings" |> U.to_list |> List.length) )
      ]
  in
  `Assoc
...
let bind ~channel_id ~keeper_name ~actor_name =
  ...
  if channel_id = ""
  then Error "channel_id is required"
  else if keeper_name = ""
  then Error "keeper_name is required"
  else (
    let original_bindings = read_bindings () in
    let previous_keeper =
      original_bindings
      |> List.find_map (fun (binding : binding) ->
        if String.equal binding.channel_id channel_id
        then Some binding.keeper_name
        else None)
    ...
    let updated_bindings =
      ...
      :: List.filter
           (fun (binding : binding) -> not (String.equal binding.channel_id channel_id))
           original_bindings
      |> List.sort (fun (a : binding) (b : binding) ->
        String.compare a.channel_id b.channel_id)
    in
