  ...
  then Keep_effective_cascade normalized_effective
    ...
    let ollama_urls =
      ...
      |> List.filter_map (fun (cfg : Llm_provider.Provider_config.t) ->
        if Cascade_ollama_probe.is_ollama_url cfg.base_url
...
let fail_open_local_only_when_unavailable
  ...
  | Probe_local_only_urls { effective_cascade; fallback_cascade; ollama_base_urls } ->
    ...
    (match probe_ollama_base_url with
     ...
     | Some probe ->
       if List.exists probe ollama_base_urls then effective_cascade else fallback_cascade)
;;
...
let resolve_ollama_only_base_url ?resolve_label (labels : string list) : string option =
  let resolve_label =
    match resolve_label with
    | Some f -> f
    | None -> fun label -> Cascade_config.parse_model_string label
    ...
    (match resolve_label first with
     | Some cfg when is_ollama_cfg cfg ->
       ...
       let same_ollama_host label =
         ...
         | Some other when is_ollama_cfg other -> String.equal other.base_url base_url
         | _ -> false
       in
       if List.for_all same_ollama_host rest then Some base_url else None
     | _ -> None)
;;
...
let is_ollama_saturated ?capacity_lookup (base_url : string) : bool =
  let capacity_lookup =
...
let fail_open_rotation_cascades_from_catalog
  ...
  then None
    ...
    in
    let is_keeper_assignable name = List.exists (String.equal name) keeper_assignable in
    match
      catalog_names
      |> List.filter (fun name -> is_reserved_recovery name || is_keeper_assignable name)
      |> dedupe_keep_order
    with
    | [] -> None
...
let next_fail_open_cascade_for_turn
  ...
  =
  let fallback_hint = Keeper_cascade_profile.fallback_cascade_for effective_cascade in
...
let oas_timeout_budget_resolution_to_yojson (budget : oas_timeout_budget_resolution)
  : Yojson.Safe.t
  =
  `Assoc
...
let bounded_oas_timeout_for_turn_budget
      ~(estimated_input_tokens : int)
      ~(remaining_turn_budget_s : float)
  : float option
  =
  ...
  | Oas.Error.Api (Timeout { message }), Some timeout_budget
    ...
    Oas_worker_named.sdk_error_of_masc_internal_error
      (Oas_worker_named.Oas_timeout_budget
         ...
         ; keeper_turn_timeout_sec = timeout_budget.keeper_turn_timeout_sec
...
let empty_turn_event_bus_summary = { correlation_id = None; overflow_imminent = None }

let merge_turn_event_bus_summary
      (left : turn_event_bus_summary)
      (right : turn_event_bus_summary)
...
let recover_context_overflow_retry
  ...
  | Some recovery ->
    Log.Keeper.warn
      "%s: context overflow retry — compacted checkpoint (%d->%d tokens, max_context=%d, \
       generation=%d)"
      meta.name
      recovery.compaction.before_tokens
  ...
  | None ->
    Log.Keeper.warn
      ...
      meta.name
      (short_preview (Oas.Error.to_string error));
    None
;;

let summarize_turn_event_bus (events : Oas.Event_bus.event list) : turn_event_bus_summary =
  List.fold_left
    (fun acc (evt : Oas.Event_bus.event) ->
       ...
       | Oas.Event_bus.ContextOverflowImminent { estimated_tokens; limit_tokens; _ } ->
         { correlation_id; overflow_imminent = Some { estimated_tokens; limit_tokens } }
  ...
  | None ->
    (match err with
     | Oas.Error.Agent (TokenBudgetExceeded { kind = "Input"; used; limit }) ->
       Keeper_state_machine.Context_overflow_detected
         { source = `Oas_signal; token_count = used; limit_tokens = Some limit }
...
let pause_keeper_for_overflow
  ...
  : keeper_meta
  =
  let paused_meta = { meta with paused = true; updated_at = now_iso () } in
  ...
  (match
   ...
   | Error err when is_version_conflict_error err ->
     Log.Keeper.warn
       ...
       err
   | Error err ->
     Log.Keeper.error "%s: overflow pause write_meta failed: %s" meta.name err);
...
let sync_keeper_paused_state
  ...
  =
  let synced_meta = { meta with paused; updated_at = now_iso () } in
  ...
  | Error err ->
    report_keeper_cycle_side_effect_issue
      ...
      ~keeper_name:meta.name
      ~side_effect:
        (Printf.sprintf "%s sync write_meta" (if paused then "pause" else "resume"))
      ~severity:`Error
      err;
  ...
  | Ok () ->
    ...
    dispatch_keeper_phase_event_checked
      ...
      ~keeper_name:meta.name
      ~side_effect:
        (Printf.sprintf "%s sync phase update" (if paused then "pause" else "resume"))
      (if paused
    ...
    ~on_resolution:(fun decision ->
      ...
      | Oas.Hooks.Reject reason ->
        (match sync_keeper_paused_state ~config ~meta:latest_meta ~paused:true with
         ...
         | Error err ->
           Log.Keeper.error
             "%s: partial-commit continue gate rejected but keeper pause sync failed: %s \
              (reason=%s)"
             meta.name
             err
             reason))
...
let cascade_budget_logged : (string * int * int, unit) Hashtbl.t = Hashtbl.create 16

let resolved_max_context_for_turn ~(meta : keeper_meta) (model_labels : string list) : int
  =
  let resolution =
    Keeper_exec_context.resolve_max_context_resolution
      ...
      model_labels
  in
  if resolution.primary_budget < resolution.cascade_budget
  then (
    let key = meta.name, resolution.primary_budget, resolution.cascade_budget in
    if not (Hashtbl.mem cascade_budget_logged key)
    then (
      Hashtbl.add cascade_budget_logged key ();
      Log.Keeper.info
        "%s: mixed cascade context budget primary=%d cascade_max=%d; using primary for \
         initial turn budget"
        meta.name
        resolution.primary_budget
        resolution.cascade_budget));
  (match resolution.requested_override with
   | Some requested ->
     Log.Keeper.debug
       "%s: using max_context_override=%d context_budget=%d primary_budget=%d \
        effective_budget=%d"
       meta.name
       requested
...
let run_keeper_cycle
  ...
  | phase_opt ->
    ...
    let effective_cascade_name =
      ...
      in
      let routing =
        Keeper_cascade_routing.select_cascade ~base_cascade:meta.cascade_name ~phase
      in
    ...
    let saturation_skip_meta =
      let meta_for_check = { meta with cascade_name = effective_cascade_name } in
      ...
      | None -> None
      | Some base_url ->
        if not (is_ollama_saturated base_url)
        then None
        else (
        ...
        | Ok initial_execution ->
               ...
               | Some bus ->
                 Some
                   (Oas_bus_instrument.subscribe
                      ~purpose:"keeper_turn"
                      ~filter:(Oas.Event_bus.filter_agent meta.name)
                      bus)
               | None -> None
             in
         ...
         ToolCompleted pops the oldest input for that tool_name. *)
             ...
             let pop_pending_input tool_name =
               ...
               | _ -> None
             in
             let process_tool_events_for_side_effects (events : Oas.Event_bus.event list)
               : unit
               =
               List.iter
                 (fun (evt : Oas.Event_bus.event) ->
                    ...
                    | Oas.Event_bus.ToolCompleted { tool_name; output = Ok _; _ } ->
                    ...
                    | Oas.Event_bus.ToolCompleted { tool_name; output = Error _; _ } ->
             ...
             let start_background_turn_event_bus_drain ~clock =
               ...
               | Some _, Some sw ->
                 Eio.Fiber.fork ~sw (fun () ->
                   let rec loop () =
                     if Atomic.get event_bus_drain_active
                     then (
                       (try ignore (drain_turn_event_bus ()) with
                        | Eio.Cancel.Cancelled _ as e -> raise e
                        | exn ->
                          Log.Keeper.warn
           ...
           instead of propagating them. *)
                           ...
                           Error
                             (Oas.Error.Api
                                (Timeout
                                   { message =
                                       Printf.sprintf
                                         "Turn wall-clock budget exhausted before retry \
                                          (remaining=%.1fs)"
                                         (remaining_turn_budget_s ())
                                   }))
                         | Some timeout_budget ->
                     ...
                     build a fresh prompt that may avoid the parse issue. *)
                         ...
                         then (
                           ...
                           then
                             ...
                             Log.Keeper.error
                               "%s: error after committed mutating tool call(s) [%s] — \
                                turn outcome is ambiguous and requires reconcile (error: \
                                %s)"
                               meta.name
                               (String.concat ", " committed_tools)
                               err_preview;
                           ...
                           | Degraded_retry_allowed degraded_retry ->
                             (match
                              ...
                              | Ok next_execution ->
                                ...
                                Log.Keeper.warn
                                  "%s: recoverable cascade failure in %s; rotation retry \
                                   on cascade=%s reason=%s max_context=%d \
                           ...
                           | Degraded_retry_budget_exhausted degraded_retry ->
                             Log.Keeper.warn
                               "%s: recoverable cascade failure in %s suggested degraded \
                                retry to %s (reason=%s), but remaining turn budget %.1fs \
                                is below the OAS retry guard/minimum; ending this cycle: \
                                %s"
                               meta.name
                               execution.cascade_name
                               degraded_retry.next_cascade
             ...
             connection blocks the keeper fiber forever. *)
                     (try
                      ...
                      | Eio.Time.Timeout ->
                        ...
                        if
                          committed_tools <> []
                          && Keeper_tool_registry.all_tools_reconcile_safe committed_tools
                        then (
                 ...
                 always transient by nature. *)
                          Log.Keeper.warn
                            "%s: turn wall-clock timeout after committed reconcile-safe \
                             tool(s) [%s] — auto-recovering (timeout: %s)"
                            meta.name
                            (String.concat ", " committed_tools)
                            msg;
                          Keeper_registry.set_turn_phase
                        ...
                        then (
                          let timeout_err = Oas.Error.Api (Timeout { message = msg }) in
                          let reclassified, failure_reason =
                            match
                              EC.classify_post_commit_failure
                                ~tool_names:committed_tools
                          ...
                          post_commit_failure_reason := Some failure_reason;
                          Log.Keeper.error
                            "%s: turn wall-clock timeout after committed mutating tool \
                             call(s) [%s] — treating as integrity failure; evidence \
                             recorded for next-turn observation"
                            meta.name
                            (String.concat ", " committed_tools);
                          Keeper_registry.set_turn_phase
                            ~base_path:config.base_path
             ...
             (match run_result with
              | Error err ->
                ...
                Prometheus.inc_counter
                  Prometheus.metric_keeper_turns
                ...
                then (
                  let failure_reason =
                    Option.value
                      ~default:
                        (Keeper_registry.Ambiguous_partial_commit
                           { kind = Keeper_registry.Post_commit_failure; detail = e_str })
                      !post_commit_failure_reason
                  in
                  Keeper_registry.set_failure_reason
                    ~base_path:config.base_path
                  ...
                  let committed_tools = committed_mutating_tools_snapshot () in
                  Log.Keeper.info
                    "%s: reconcile-required failure latched as %s after committed tools \
                     [%s]"
                    meta.name
                    (Keeper_registry.failure_reason_to_string failure_reason)
                    (String.concat ", " committed_tools));
             ...
             the SSOT — no new state surface, dashboard already renders this. *)
                ...
                then (
                  ...
                  | Ok _ ->
                    ...
                    Log.Keeper.warn
                      "%s: auto-paused after %d cascade_exhausted failures \
                       (pause_threshold=%d, crash_threshold=%d); operator must resume \
                       after cascade fix"
                      meta.name
                      count
                  ...
                  | Error sync_err ->
                    Log.Keeper.error
                      "%s: cascade auto-pause sync failed: %s (falling through to crash \
                       path)"
                      meta.name
                      sync_err);
                if count >= threshold && not cascade_auto_paused
                then (
                  Log.Keeper.error
              ...
              | Ok result ->
                ...
                let turn_cost =
                  Keeper_unified_metrics.estimate_trusted_usage_cost_usd
                    ~usage_trusted:
                      (Keeper_unified_metrics.usage_trust_is_trusted usage_trust_for_cost)
                    ~model:used_model_id
                    result.usage
                in
                let lifecycle =
             ...
             MASC_STAY_SILENT_LOOP_THRESHOLD (default 10). *)
               ...
               dead series. *)
                  ...
                  then
                    Prometheus.inc_counter
                      Prometheus.metric_keeper_cache_creation_tokens
                      ~labels:[ "keeper_name", updated_meta.name; "model", model_used ]
                  ...
                  then
                    Prometheus.inc_counter
                      Prometheus.metric_keeper_cache_read_tokens
                      ~labels:[ "keeper_name", updated_meta.name; "model", model_used ]
