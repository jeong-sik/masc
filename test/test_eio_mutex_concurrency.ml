...
let test_circuit_breaker_concurrent () =
  ...
  check
    ...
    "state is valid"
    true
    (List.mem status.state_name [ "closed"; "open"; "half_open" ])
;;
...
let test_prometheus_concurrent () =
  Eio.Fiber.all
    (List.init 10 (fun i ->
       fun () ->
       for _ = 1 to 100 do
         Prom.inc_counter "test_concurrent_metric" ~labels:[ "fiber", string_of_int i ] ()
       done));
  ...
  check
    ...
    true
    (try
       ignore (Str.search_forward (Str.regexp_string "test_concurrent_metric") text 0);
       true
     with
     | Not_found -> false)
;;
...
let test_prometheus_gauge_concurrent () =
  ...
  check
    ...
    true
    (try
       ignore (Str.search_forward (Str.regexp_string "test_concurrent_gauge") text 0);
       true
     with
     | Not_found -> false)
;;
...
let test_session_concurrent_create_find () =
  ...
  Array.iter
    (fun id -> check bool "session findable" true (Option.is_some (SH.Session.find id)))
    created_ids;
  (* Cleanup *)
...
let () =
  ...
  run
    "Eio.Mutex Migration Concurrency"
    [ ( "Rate Limit"
