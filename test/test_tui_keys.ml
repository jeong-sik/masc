...
let test_lanes_footer_opens_standalone_runs () =
  check str "Lanes names its run drill-down, config source, and way back"
    ...
    "j/k:move  e:lane config  A:add-ons  p:runtime  PgUp/PgDn:page  Home/End:top/bottom  Right / Enter:runs  a:append slot  Esc:overview  /:find  n / N:next / previous match  r:refresh  Tab:next  q:quit"
