...
let extract_tla_set ~marker content =
  ...
  let is_marker_decl i =
    ...
    then false
    else if String.sub content i marker_len <> marker
    then false
    else (
      let rec skip_ws j =
        if j < String.length content && (content.[j] = ' ' || content.[j] = '\t')
        then skip_ws (j + 1)
        else j
      in
  ...
  let rec find_marker start =
    ...
    then None
      ...
      | None -> None
      | Some i -> if is_marker_decl i then Some i else find_marker (i + 1))
  in
  ...
  | Some marker_idx ->
    ...
    in
    String.sub content brace_idx (close_idx - brace_idx + 1) |> extract_quoted_strings
;;
...
let rec find_repo_root dir =
  ...
  then dir
    ...
    then Alcotest.fail "could not find repo root for KeeperCompositeLifecycle.tla"
...
let keeper_turn_cycle_tla () =
  Filename.concat (project_root ()) "specs/keeper-state-machine/KeeperTurnCycle.tla"
;;
...
let test_observer_prompting_at_turn_start () =
  ...
  | Some entry ->
    ...
    check
      string
      "fresh turn starts in Prompting"
      "prompting"
      (Obs.turn_phase_to_string snap.ktc_turn_phase);
    check
      string
      "fresh turn starts undecided"
      "undecided"
      (Obs.decision_stage_to_string snap.kdp_decision);
    check
      string
      "fresh turn starts with idle cascade"
      "idle"
      (Obs.cascade_state_to_string snap.kcl_cascade_state)
;;
...
let test_observer_gate_rejected_finalizes_turn () =
  ...
  | Some entry ->
    ...
    check
      string
      "gate rejection moves turn to finalizing"
      "finalizing"
      (Obs.turn_phase_to_string snap.ktc_turn_phase);
    check
      string
      "gate rejection records decision stage"
      "gate_rejected"
      (Obs.decision_stage_to_string snap.kdp_decision);
    check
      string
      "gate rejection preserves in-flight trying edge"
      "trying"
      (Obs.cascade_state_to_string snap.kcl_cascade_state)
;;
  ...
  | Some entry ->
    ...
    (match snap.last_outcome with
     ...
     | Some lo ->
       ...
       check
         string
         "last_outcome decision persisted"
         "tool_policy_selected"
         (Obs.decision_stage_to_string lo.decision_stage);
       check
         string
         "last_outcome cascade persisted"
         "done"
         (Obs.cascade_state_to_string lo.cascade_state);
       check
         (option string)
         "last_outcome selected_model persisted"
         (Some "glm-4.5")
         lo.selected_model)
;;
...
let test_observer_event_priority_detects_competing_measurement () =
  ...
  | Some entry ->
    ...
    check
      bool
      "second measurement for same live turn breaks monotonicity"
      false
      snap.invariants.event_priority_monotone
;;
...
let test_turn_retry_after_compaction_resets_cascade_attempt () =
  ...
  | Some entry ->
    ...
    check
      string
      "retry returns to prompting"
      "prompting"
      (Obs.turn_phase_to_string snap.ktc_turn_phase);
    check
      string
      "retry preserves guard_ok posture"
      "guard_ok"
      (Obs.decision_stage_to_string snap.kdp_decision);
    check
      string
      "retry clears prior cascade attempt"
      "idle"
      (Obs.cascade_state_to_string snap.kcl_cascade_state);
    (match entry.current_turn_observation with
     | None -> Alcotest.fail "live turn missing after retry reset"
     | Some obs ->
       check (option string) "retry clears selected model" None obs.selected_model)
;;
  ...
  run
    ...
    [ ( "composite_observer_turn_scope"
      , [ test_case "idle when no turn" `Quick test_observer_idle_when_no_turn
        ...
        ; test_case
            "gate rejection finalizes the turn"
            `Quick
    ...
    ; ( "composite_observer_phase_2"
      , [ test_case "is_live = true during turn" `Quick test_observer_is_live_during_turn
        ...
        ; test_case
            "snapshot json includes terminal fields"
            `Quick
            test_observer_json_includes_terminal_fields
        ; test_case
            "event priority detects competing measurement"
            `Quick
            test_observer_event_priority_detects_competing_measurement
        ; test_case
            "compaction retry resets prior cascade attempt"
            `Quick
            test_turn_retry_after_compaction_resets_cascade_attempt
        ; test_case
            "variant sets match KeeperCompositeLifecycle.tla"
            `Quick
            test_composite_observer_variants_match_tla_sets
        ; test_case
            "named sets match KeeperCompositeLifecycle.tla"
            `Quick
