...
let add_strict_task config =
  ...
  in
  let _msg =
    Coord.add_task ~contract config ~title ~priority:3 ~description:"needs verification"
  in
  let backlog = Coord.read_backlog config in
  match
    List.find_opt (fun (t : Types.task) -> not (List.mem t.id existing_ids)) backlog.tasks
  with
  | Some t -> t.id
...
let claim_and_start config agent_name task_id =
  let _ = Coord.transition_task_r config ~agent_name ~task_id ~action:Types.Claim () in
  let _ = Coord.transition_task_r config ~agent_name ~task_id ~action:Types.Start () in
  ()
;;
...
let expect_claim_next_claimed result ~task_id ~released_task_id =
  ...
  | Coord.Claim_next_claimed
    ...
    Alcotest.(check (option string)) "released task" released_task_id actual_released
  ...
  | Coord.Claim_next_error message ->
    Alcotest.failf "expected claim_next to claim a task, got error: %s" message
;;
...
let expect_claim_next_no_eligible result =
  ...
  | Coord.Claim_next_claimed { task_id; _ } ->
    Alcotest.failf "expected claim_next to report no_eligible, got %s" task_id
  | Coord.Claim_next_error message ->
    Alcotest.failf "expected claim_next to report no_eligible, got error: %s" message
;;
...
let expect_claim_next_no_unclaimed result =
  ...
  | Coord.Claim_next_claimed { task_id; _ } ->
    Alcotest.failf "expected claim_next to report no_unclaimed, got %s" task_id
  | Coord.Claim_next_error message ->
    Alcotest.failf "expected claim_next to report no_unclaimed, got error: %s" message
;;
...
let test_submit_prepare_failure_keeps_task_in_progress () =
  with_temp_config ~fsm_enabled:true (fun config ->
    ...
    let result =
      Coord.transition_task_r
        ...
        ~prepare_verification_request:
          (fun
            ~task:_ ~assignee:_ ~verification_id:_ ~evidence_refs:_ ->
          prepare_called := true;
    ...
    | Error e ->
      ...
      Alcotest.(check bool)
        "error mentions verification request"
        true
        (Astring.String.is_infix ~affix:"verification request creation failed" msg);
      let reqs =
...
let test_submit_populates_criteria_from_completion_contract () =
  with_temp_config ~fsm_enabled:true (fun config ->
    ...
    submit_protocol_or_fail
      ...
      ~verification_id:"vrf-wiring"
      ~evidence_refs;
    let reqs = Verification.list_requests config.Coord.base_path in
    let req =
      List.find (fun (r : Verification.verification_request) -> r.task_id = task_id) reqs
    in
    let custom_texts =
      List.filter_map
        (function
    ...
    Alcotest.(check (list string))
      "criteria from completion_contract"
      [ "tests pass" ]
      custom_texts;
    let persisted_refs =
    ...
    in
    Alcotest.(check (list string))
      "evidence_refs from verify_gate_evidence"
      [ "output.json" ]
      persisted_refs)
;;
...
let test_submit_marks_conflict_triage_when_deliverable_claims_completion () =
  with_temp_config ~fsm_enabled:true (fun config ->
    ...
    in
    Alcotest.(check string) "request_kind" "conflict_triage" (string_field "request_kind");
    Alcotest.(check string)
      "request_summary"
...
let test_approve_by_other_agent_moves_to_done () =
  with_temp_config ~fsm_enabled:true (fun config ->
    ...
    | Ok _ -> Alcotest.(check string) "status" "done" (status_string config task_id))
;;
...
let test_reject_by_other_agent_moves_to_in_progress () =
  with_temp_config ~fsm_enabled:true (fun config ->
    ...
    | Ok _ ->
      Alcotest.(check string) "status" "in_progress" (status_string config task_id))
;;
...
let test_approve_prepare_failure_keeps_task_awaiting () =
  with_temp_config ~fsm_enabled:true (fun config ->
    ...
    let req =
      create_pending_request config ~task_id ~worker:"worker" ~request_id:verification_id
    in
    let prepare_called = ref false in
    let result =
      Coord.transition_task_r
        ...
        ~prepare_verification_verdict:
          (fun
            ~task:_ ~verifier:_ ~verification_id:_ ~decision:_ ->
          prepare_called := true;
    ...
    | Error e ->
      ...
      let msg = Types.show_masc_error e in
      Alcotest.(check bool)
        "error mentions verdict persistence"
        true
        (Astring.String.is_infix ~affix:"verification verdict persistence failed" msg);
...
let test_claim_next_skips_pending_verification_tasks () =
  with_temp_config ~fsm_enabled:true (fun config ->
    ...
    Alcotest.(check string)
      "pending task remains awaiting_verification"
      "awaiting_verification"
      (status_string config task_1);
    Coord.claim_next_r config ~agent_name:"other" () |> expect_claim_next_no_unclaimed)
;;
  ...
  with_temp_config ~fsm_enabled:true (fun config ->
    ...
    (match
       Coord.transition_task_r
         ...
         ~task_id:task_1
         ~action:Types.Reject_verification
         ~reason:"CI checks failed at plan commit and PR head"
         ()
     with
     | Ok _ -> ()
     | Error e -> Alcotest.fail ("reject failed: " ^ Types.show_masc_error e));
    (match
       Verification.submit_verdict
         ...
         ~req_id:req.id
         ~verifier:"verifier"
         ~verdict:(Verification.Fail "CI checks failed at plan commit and PR head")
     with
     | Ok _ -> ()
     | Error e -> Alcotest.fail ("submit_verdict failed: " ^ e));
    Coord.claim_next_r config ~agent_name:"worker" ()
    |> expect_claim_next_claimed ~task_id:task_2 ~released_task_id:(Some task_1);
    Coord.claim_next_r config ~agent_name:"other" () |> expect_claim_next_no_eligible)
;;
...
let test_claim_next_blocks_pending_requests_stored_only_under_masc_root () =
  with_temp_config ~fsm_enabled:true (fun config ->
    ...
    let active_path =
      Filename.concat
        (Filename.concat (Coord_utils.masc_dir config) "verifications")
        (req.id ^ ".json")
    in
    Alcotest.(check bool) "request stored under .masc" true (Sys.file_exists active_path);
    Alcotest.(check bool)
      "legacy root verifications absent"
      false
      (Sys.file_exists (Filename.concat config.Coord.base_path "verifications"));
    Coord.claim_next_r config ~agent_name:"other" () |> expect_claim_next_no_eligible)
;;
  ...
  with_temp_config ~fsm_enabled:true (fun config ->
    ...
    | Ok updated ->
      Alcotest.(check bool)
        ...
        true
        (match updated.status with
         | Completed (Fail r) -> Astring.String.is_infix ~affix:"missing evidence" r
         | _ -> false))
;;
...
let () =
  Alcotest.run
    ...
    [ ( "transitions_enabled"
      , [ Alcotest.test_case
            "submit moves to awaiting_verification"
            `Quick
            test_submit_for_verification_moves_to_awaiting
        ; Alcotest.test_case
            "submit from claimed moves to awaiting_verification"
            `Quick
            test_submit_for_verification_from_claimed_moves_to_awaiting
        ; Alcotest.test_case
            "submit prepare failure keeps task in_progress"
            `Quick
            test_submit_prepare_failure_keeps_task_in_progress
        ; Alcotest.test_case
            "submit splits criteria/evidence by contract field"
            `Quick
            test_submit_populates_criteria_from_completion_contract
        ; Alcotest.test_case
            "submit marks conflict triage from completed deliverable"
            `Quick
            test_submit_marks_conflict_triage_when_deliverable_claims_completion
        ; Alcotest.test_case
            "cross-agent approve moves to done"
            `Quick
            test_approve_by_other_agent_moves_to_done
        ; Alcotest.test_case
            "cross-agent reject moves to in_progress"
            `Quick
            test_reject_by_other_agent_moves_to_in_progress
        ; Alcotest.test_case
            "approve prepare failure keeps task awaiting"
            `Quick
            test_approve_prepare_failure_keeps_task_awaiting
        ; Alcotest.test_case
            "reject prepare failure keeps task awaiting"
            `Quick
            test_reject_prepare_failure_keeps_task_awaiting
        ; Alcotest.test_case
            "claim_next skips pending verification tasks"
            `Quick
            test_claim_next_skips_pending_verification_tasks
        ; Alcotest.test_case
            "claim_next skips rejected verification tasks"
            `Quick
            test_claim_next_skips_rejected_verification_tasks
        ; Alcotest.test_case
            ".masc pending verification blocks claim_next"
            `Quick
            test_claim_next_blocks_pending_requests_stored_only_under_masc_root
        ; Alcotest.test_case "self-approval blocked" `Quick test_self_approval_blocked
        ; Alcotest.test_case "self-rejection blocked" `Quick test_self_rejection_blocked
        ] )
    ; ( "fsm_disabled"
      , [ Alcotest.test_case
            "submit fails when FSM disabled"
            `Quick
            test_fsm_disabled_submit_fails
        ] )
    ; ( "verification_state_sync"
      , [ Alcotest.test_case
            "submit_verdict Pass updates state"
            `Quick
            test_submit_verdict_pass
        ; Alcotest.test_case
            "submit_verdict Fail preserves reason"
            `Quick
