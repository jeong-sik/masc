...
let validate_via_oas ~tool_name ~(schema : Yojson.Safe.t) ~(args : Yojson.Safe.t)
  ...
  then Pass
    ...
    | Agent_sdk.Tool_input_validation.Invalid errors ->
      let msg = Agent_sdk.Tool_input_validation.format_errors ~tool_name errors in
      Reject
...
let test_required_present () =
  ...
  | Reject r ->
    Alcotest.fail
      (Printf.sprintf "Expected Pass, got Reject: %s" (Yojson.Safe.to_string r.data))
;;

let test_required_missing () =
  let schema =
    make_schema ~required:[ "name"; "room" ] [ "name", "string"; "room", "string" ]
  in
  let args = `Assoc [ "name", `String "alice" ] in
  match validate_via_oas ~tool_name:"test" ~schema ~args with
  | Reject r ->
...
let test_coerce_string_to_integer () =
  ...
  | Reject r ->
    Alcotest.fail
      (Printf.sprintf "Expected Proceed, got Reject: %s" (Yojson.Safe.to_string r.data))
;;
...
let test_coerce_int_to_number () =
  ...
  | Reject r ->
    Alcotest.fail
      (Printf.sprintf
         "Expected Pass/Proceed, got Reject: %s"
...
let test_coerce_intlit_to_integer () =
  ...
  | Reject r ->
    Alcotest.fail
      (Printf.sprintf
         "Expected Proceed/Pass, got Reject: %s"
         (Yojson.Safe.to_string r.data))
;;
...
let test_registered_hook_coerces_args () =
  ...
  Alcotest.(check bool) "coercion changed args" false (Yojson.Safe.equal forwarded args);
...
let test_registered_hook_bypasses_unknown_tool () =
  let args = `Assoc [ "count", `String "42" ] in
  let blocked, forwarded =
    run_registered_hook ~tool_name:"__tool_input_validation_registered_unknown" ~args ()
  in
  Alcotest.(check bool) "not blocked" true (Option.is_none blocked);
  Alcotest.(check bool) "args unchanged" true (Yojson.Safe.equal forwarded args)
;;
...
let masc_transition_schema = find_schema_exn "masc_transition" Tool_task_schemas.schemas
...
let test_registered_hook_transition_compat_to_and_note () =
  ...
  in
  Alcotest.(check bool) "not blocked" true (Option.is_none blocked);
  Alcotest.(check string) "to -> action claim" "claim" (assoc_string "action" forwarded);
  Alcotest.(check string)
    "note -> notes"
    "PR #8308 Draft"
    (assoc_string "notes" forwarded);
  Alcotest.(check bool) "to removed" true (Yojson.Safe.Util.member "to" forwarded = `Null);
  Alcotest.(check bool)
    "note removed"
    true
...
let test_registered_hook_transition_compat_status_action () =
  ...
  in
  Alcotest.(check bool) "not blocked" true (Option.is_none blocked);
  Alcotest.(check string) "claimed -> claim" "claim" (assoc_string "action" forwarded)
;;
...
let test_registered_hook_transition_strips_internal_agent_marker () =
  ...
  Alcotest.(check bool) "not blocked" true (Option.is_none blocked);
  Alcotest.(check bool)
    "_agent_name removed before schema validation"
    true
...
let test_augment_suggests_closest_arg_key () =
  ...
  Alcotest.(check bool)
    "appends Did you mean section"
    true
    (string_contains augmented "Did you mean:");
  Alcotest.(check bool)
    "names actual key sent"
    true
...
let () =
  Alcotest.run
    "Tool_input_validation (OAS delegation)"
    [ ( "required"
    ...
    ; ( "coercion"
      , [ Alcotest.test_case "string -> integer" `Quick test_coerce_string_to_integer
        ...
        ; Alcotest.test_case
            "Intlit -> Int (normalize)"
            `Quick
            test_coerce_intlit_to_integer
        ; Alcotest.test_case
            "non-coercible string -> reject"
            `Quick
            test_invalid_string_to_integer
        ] )
    ; ( "correct_types"
