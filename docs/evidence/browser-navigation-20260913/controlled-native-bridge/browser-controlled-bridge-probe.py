...
parser.add_argument('--bridge-artifact',type=pathlib.Path,required=True)
parser.add_argument('--bridge-checkout',type=pathlib.Path,required=True)
...
# This is a controlled component comparison, not a single-source candidate.
# Preserve the baseline export provenance; name the actual replacement bridge
# separately and verify both against immutable committed/artifact bytes.
bridge_artifact=args.bridge_artifact.resolve()
assert (bridge_artifact/'SOURCE_COMMIT').read_text().strip()==args.bridge_commit
bridge_host=bridge_artifact/'masc-browser-host-macos-arm64'
bridge_digest=hashlib.sha256(bridge_host.read_bytes()).hexdigest()
bridge_manifest=dict((line.split()[1],line.split()[0]) for line in (bridge_artifact/'SHA256SUMS').read_text().splitlines())
assert bridge_manifest[bridge_host.name]==bridge_digest
bridge_files={}
for name in ['manifest.json','background.js']:
 relative='connectors/browser/extension/'+name
 content=subprocess.check_output(['git','-C',str(args.bridge_checkout),'show',args.bridge_commit+':'+relative])
 assert (args.bridge_checkout/relative).read_bytes()==content
 bridge_files[relative]=hashlib.sha256(content).hexdigest()
bridge_proof={'source_commit':args.bridge_commit,'native_host_sha256':bridge_digest,'files':bridge_files,
 'artifact_source_commit':(bridge_artifact/'SOURCE_COMMIT').read_text().strip(),
 'scope':'Replacement native host and extension only. Baseline server, TUI, fixture, prompt source and binary-exported Skills are unchanged.'}
(OUT/'bridge-source-proof.json').write_text(json.dumps(bridge_proof,indent=2))
...
report['component_sources']={'server_and_tui':args.server_commit,'native_host_and_extension':args.bridge_commit}
report['actual_binary_sha256']={'masc-macos-arm64':bundle_manifest['binaries']['masc-macos-arm64'],
 'masc-tui-macos-arm64':bundle_manifest['binaries']['masc-tui-macos-arm64'],
 'masc-browser-host-macos-arm64':bridge_digest}
report['comparison_scope']='Baseline server/TUI/Skills retained; only the live browser host and extension are replaced. This is not the complete new-source integration candidate.'
...
try:
 ...
 for name in ['manifest.json','background.js']:
  original=subprocess.check_output(['git','-C',str(args.bridge_checkout),'show',args.bridge_commit+':connectors/browser/extension/'+name]);content=original
 ...
 report['live_extension']={'host_name':host_name,'native_host_sha256':hashlib.sha256(host_source.read_bytes()).hexdigest(),
  ...
  'source_commit':args.bridge_commit,
