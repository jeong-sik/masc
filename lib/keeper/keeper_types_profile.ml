...
let network_mode_of_string raw =
  ...
  | "host" ->
    Log.Keeper.warn
      "network_mode=\"host\" is a deprecated alias for \"inherit\"; update TOML to use \
       \"inherit\"";
...
let normalize_social_model_opt = function
  ...
  | Some raw ->
    (match Keeper_social_model_types.model_id_of_string raw with
     | Some model_id -> Some (Keeper_social_model_types.model_id_to_string model_id)
     | None -> None)
;;

let valid_social_model_strings = Keeper_social_model_types.valid_model_id_strings
...
let default_voice_agent_id_for name = if default_voice_enabled_for name then name else ""
...
let room_seq_map_of_json (json : Yojson.Safe.t) : (string * int) list =
  ...
  | `Assoc fields ->
    ...
    |> List.filter_map (fun (room_id, value) ->
      if not (validate_name room_id)
      then None
      else (
        match value with
        | `Int seq -> Some (room_id, seq)
        | `Intlit raw -> Some (room_id, Safe_ops.int_of_string_with_default ~default:0 raw)
        | _ -> None))
  | _ -> []
;;
...
let normalize_per_provider_timeout_opt ~(source : string) (value : float option)
  : float option
  =
  match value with
...
let per_provider_timeout_of_declared_float_opt
  ...
  =
  if not declared
  then Per_provider_timeout_unset, None
  else (
    match value with
    | None ->
      Log.Keeper.warn "%s per_provider_timeout has invalid type; ignoring" source;
...
let oas_env_key_is_allowed suffix =
  ...
  List.exists
    (fun p ->
       String.length suffix > String.length p && String.sub suffix 0 (String.length p) = p)
    allowed_prefixes
;;

let extract_oas_env_from_doc (doc : Keeper_toml_loader.toml_doc) : (string * string) list =
  let prefix_len = String.length oas_env_key_prefix in
  List.filter_map
    (fun (k, v) ->
       if String.length k > prefix_len && String.sub k 0 prefix_len = oas_env_key_prefix
       then (
         let suffix = String.sub k prefix_len (String.length k - prefix_len) in
         if oas_env_key_is_allowed suffix
         then Option.map (fun sv -> suffix, sv) (string_of_toml_value_for_env v)
         else None)
       else None)
    doc
;;
...
let profile_defaults_of_toml (doc : Keeper_toml_loader.toml_doc)
  ...
  let per_provider_timeout_state, per_provider_timeout =
    per_provider_timeout_of_toml ~source:"keeper TOML" doc (k "per_provider_timeout")
  in
  let removed_present =
  ...
  let result =
    ...
    | [] -> Ok ()
    | fields ->
      Error (Printf.sprintf "removed keeper TOML keys: %s" (String.concat ", " fields))
  in
  let result =
      ...
      | Some raw ->
        (match network_mode_of_string raw with
         ...
         | None ->
           Error
             (Printf.sprintf
                "invalid network_mode '%s' (allowed: none, inherit; deprecated alias: \
                 host)"
                raw))
      | None -> Ok ())
  in
  ...
  let result =
    Result.bind result (fun () ->
      ...
      | Some raw ->
        let normalized =
          Keeper_cascade_profile.normalize_declared_name raw |> String.lowercase_ascii
        in
        if List.mem normalized reserved_cascade_names
        then Ok ()
        else (
          ...
          match Keeper_cascade_profile.catalog_names_with_toml_fallback () with
          | Ok (catalog, source) ->
            let all_valid =
              List.sort_uniq String.compare (reserved_cascade_names @ catalog)
            in
            let suffix =
              match source with
              | Keeper_cascade_profile.Live_catalog -> ""
              | Keeper_cascade_profile.Toml_section_fallback { catalog_error } ->
                Printf.sprintf
                  " [degraded toml-section fallback; live catalog unavailable: %s]"
                  catalog_error
            in
            if not (List.mem normalized all_valid)
            then
              Error
                (Printf.sprintf
            ...
            then (
              ...
              let assignable_hint =
                if assignable = [] then "(none)" else String.concat ", " assignable
              in
              Error
                (Printf.sprintf
                   "cascade_name '%s' is system-only (keeper_assignable=false); keepers \
                    must reference an assignable cascade. Assignable: %s"
                   raw
                   assignable_hint))
            else Ok ()
  ...
  Result.map
    (fun () ->
       ...
       ; goal = str "goal"
       ; short_goal = str "short_goal" |> normalize_goal_horizon_opt
       ; mid_goal = str "mid_goal" |> normalize_goal_horizon_opt
       ; long_goal = str "long_goal" |> normalize_goal_horizon_opt
       ; will = str "will"
       ; needs = str "needs"
       ; desires = str "desires"
       ...
       ; sandbox_profile = Option.bind (str "sandbox_profile") sandbox_profile_of_string
       ; network_mode = Option.bind (str "network_mode") network_mode_of_string
       ; shared_memory_scope =
           Option.bind (str "shared_memory_scope") shared_memory_scope_of_string
       ; github_identity = str "github_identity"
       ; git_identity_mode = normalize_git_identity_mode_opt (str "git_identity_mode")
       ; tool_preset =
           (match str "tool_preset" with
            | None -> None
       ...
       ; active_goal_ids =
           (if has "active_goal_ids"
            then Some (normalize_name_list (strs "active_goal_ids"))
            else None)
...
let detect_unknown_keeper_toml_keys (doc : Keeper_toml_loader.toml_doc) =
  let known = canonical_keeper_toml_key_names |> List.map (fun k -> "keeper." ^ k) in
  ...
  doc
  |> List.map fst
  |> List.filter (fun key -> (not (List.mem key known)) && not (starts_with_oas_env key))
  |> dedupe_keep_order
;;
...
let load_keeper_toml (path : string) : (string * keeper_profile_defaults, string) result =
       ...
       (match profile_defaults_of_toml doc with
        ...
        | Ok defaults ->
          ...
          let name =
            ...
            | Some n when n <> "" -> n
            | _ -> Filename.basename path |> Filename.remove_extension
          in
          if not (validate_name name)
          then Error (Printf.sprintf "%s: invalid keeper name '%s'" path name)
          else (
...
let log_toml_skip_once ~file ~error =
  ...
  if Hashtbl.mem logged_toml_skip key
  then false
  else (
...
let discover_keepers_toml (dir : string) : (string * keeper_profile_defaults) list =
     ...
     | Some json ->
       ...
       (match keeper_json with
        | `Assoc _ ->
          ...
          ; proactive_cooldown_sec =
              Safe_ops.json_int_opt "proactive_cooldown_sec" keeper_json
          ; room_signal_prompt_enabled =
          ...
          ; max_turns_per_call_scheduled_autonomous =
              Safe_ops.json_int_opt "max_turns_per_call_scheduled_autonomous" keeper_json
          ; social_model =
              (match
...
let should_log_profile_conflict_once ~agent_name ~field ~base_value ~overlay_value =
  let key = String.concat "\000" [ agent_name; field; base_value; overlay_value ] in
  Eio_guard.with_mutex profile_conflict_warn_mu (fun () ->
    if Hashtbl.mem profile_conflict_warned key
    then false
    else (
...
let merge_keeper_profile_defaults
  ...
  let prefer overlay_value base_value =
    match overlay_value with
    | Some _ -> overlay_value
    | None -> base_value
  in
  let warn_on_conflict ~field ~base_value ~overlay_value =
    ...
    | Some b, Some o when b <> o ->
      let should_log =
        should_log_profile_conflict_once ~agent_name ~field ~base_value:b ~overlay_value:o
      in
      Prometheus.inc_counter
        Prometheus.metric_keeper_profile_config_conflicts
        ~labels:
          [ "field", field
  ...
  warn_on_conflict
    ~field:"tool_preset"
    ~base_value:base.tool_preset
    ~overlay_value:overlay.tool_preset;
  let per_provider_timeout_state, per_provider_timeout =
    match overlay.per_provider_timeout_state with
    | Per_provider_timeout_unset ->
      base.per_provider_timeout_state, base.per_provider_timeout
    | Per_provider_timeout_invalid -> Per_provider_timeout_invalid, None
    | Per_provider_timeout_set -> Per_provider_timeout_set, overlay.per_provider_timeout
  in
  ...
  ; policy_voice_enabled = prefer overlay.policy_voice_enabled base.policy_voice_enabled
  ; autoboot_enabled = prefer overlay.autoboot_enabled base.autoboot_enabled
  ; mention_targets = merge_string_list ~base:base.mention_targets overlay.mention_targets
  ...
  ; shared_memory_scope = prefer overlay.shared_memory_scope base.shared_memory_scope
  ...
  ; telemetry_feedback_window_hours =
      prefer overlay.telemetry_feedback_window_hours base.telemetry_feedback_window_hours
  ; per_provider_timeout_state
  ; per_provider_timeout
  ...
  ; max_turns_per_call_scheduled_autonomous =
      prefer
        overlay.max_turns_per_call_scheduled_autonomous
...
let effective_oas_env pairs =
  let gemini_mcp_disabled =
    ...
    | None -> false
  in
  if gemini_mcp_disabled && not (oas_env_has_non_empty "OAS_GEMINI_APPROVAL_MODE" pairs)
...
let resolved_persona_name ~keeper_name (defaults : keeper_profile_defaults) : string =
  match defaults.persona_name with
  | Some name when String.trim name <> "" -> name
  | _ -> keeper_name
;;

let load_keeper_profile_defaults_result name : (keeper_profile_defaults, string) result =
  ...
  | Some toml_path ->
    (match load_keeper_toml toml_path with
     | Ok (_name, defaults) ->
       (match defaults.persona_name with
        | Some persona_name ->
          let persona_defaults = load_keeper_profile_defaults_from_persona persona_name in
          Ok
        ...
        | None -> Ok defaults)
     | Error e -> Error e)
  | None -> Ok (load_keeper_profile_defaults_from_persona name)
;;
  ...
  let contains needle =
    ...
    then false
      ...
      let rec loop i =
        if i + nl > hl
        then false
        else if String.sub err_lc i nl = needle
        then true
        else loop (i + 1)
      in
      loop 0)
  in
  if contains "invalid network_mode"
  then "invalid_network_mode"
  else if contains "invalid sandbox_profile"
  then "invalid_sandbox_profile"
  else if contains "invalid shared_memory_scope"
  then "invalid_shared_memory_scope"
  else if contains "invalid"
  then "invalid_enum"
  else if contains "unknown" || contains "unexpected field"
  then "unknown_field"
  else if contains "parse" || contains "syntax"
  then "parse_error"
  else "other"
;;
...
let clamp_max_turns_override : int option -> int option = function
  | Some n
    when n >= Keeper_runtime_resolved.max_turns_per_call_min
         && n <= Keeper_runtime_resolved.max_turns_per_call_max -> Some n
  | _ -> None
;;
...
let effective_max_turns_per_call_scheduled_autonomous (profile : keeper_profile_defaults)
  : int
  =
  ...
  | None -> Keeper_runtime_resolved.autonomous_max_turns_per_call ()
;;
...
let keeper_default_source_snapshot name : keeper_default_source_snapshot =
  ...
  | Some toml_path ->
    (match load_keeper_toml toml_path with
     | Ok (_name, defaults) -> { source_kind = Some "toml"; defaults }
     | Error e ->
       Log.Keeper.warn "toml config for %s failed (%s), falling back to persona" name e;
...
let load_persona_summary_from_path name profile_path : persona_summary option =
  match
    Safe_ops.read_json_file_logged ~label:"load_persona_summary_from_path" profile_path
  with
  | None -> None
  | Some json ->
    let display_name =
...
let keeper_meta_path config name = Filename.concat (keeper_dir config) (name ^ ".json")
...
let keeper_agent_name name = Printf.sprintf "keeper-%s-agent" (strip_keeper_prefix name)
