  ...
  | Some ev ->
    ...
    Alcotest.(check bool)
      ...
      (String.length ev > 0
       ...
       |> List.iter (fun line ->
         if String.length line >= 5 && String.sub line 0 5 = "data:" then has_data := true);
       !has_data);
    ...
    ; ( "broadcast_to_targeting"
      , [ Alcotest.test_case "observers only" `Quick test_broadcast_to_observers_only
        ; Alcotest.test_case
            "coordinators only"
            `Quick
            test_broadcast_to_coordinators_only
        ; Alcotest.test_case "all targets" `Quick test_broadcast_to_all
        ; Alcotest.test_case
            "broadcast = broadcast_to All"
            `Quick
            test_broadcast_equals_broadcast_to_all
        ; Alcotest.test_case
            "default kind is Coordinator"
            `Quick
            test_register_defaults_to_coordinator
        ] )
    ]
;;
