...
`masc start` 는 시작할 때 `/dev/tty` 를 열어 본다.

- 열리지 않으면 제어 터미널이 없다. 지금처럼 그대로 서버로 돈다.
- 열리면 `masc start` 는 감독 프로세스가 된다.
  1. 서버를 `Process_eio_detached` 와 같은 방식(`fork` + `setsid`)으로 띄운다. 서버는 새 세션의
     리더가 되고 제어 터미널이 없다.
  2. 서버의 stdin 은 `/dev/null` 이다. stdout·stderr 는 파이프로 받아 감독의 stdout·stderr 에
     옮겨 적는다. 서버 쪽에는 터미널 디스크립터가 하나도 남지 않는다(I1).
  3. 감독이 SIGINT·SIGTERM·SIGHUP·SIGQUIT 를 받으면 서버 그룹에 SIGTERM 을 보낸다. 서버는 지금처럼
     정리 종료한다(I3).
  4. 서버가 끝나면 감독은 서버의 종료 코드로 끝난다.
  5. 감독이 서버보다 먼저 사라지면 파이프가 끊긴다. 서버는 콘솔(stdout·stderr) 쓰기를 멈추고, 스스로 쓰는
     로그 파일(`<base_path>/.masc/logs/system_log_<날짜>.jsonl`)만 계속 쓴다. 서버는 계속 돈다 (§6.3).
     - 끊긴 파이프에 쓰면 커널이 SIGPIPE 를 보낸다. macOS 의 `eio_posix` 는 시작할 때 SIGPIPE 를 무시해서
       쓰기가 EPIPE 로 끝난다. 리눅스의 `eio_linux` 는 무시하지 않아서 서버가 죽는다.
     - 그래서 서버는 백엔드와 상관없이 SIGPIPE 를 무시하고, 콘솔 쓰기에서 EPIPE 를 받으면 그 뒤로 콘솔에 쓰지
       않는다. 자식에게는 이 무시를 물려주지 않는다 (§4.4).
...
시작할 때 세션 리더가 아니면 `setsid()` 로 새 세션을 만든다. MCP 클라이언트가 끝나면 stdin 이
EOF 가 되어 끝난다.
...
(`POSIX_SPAWN_SETSIGMASK`). 그래서 macOS 에서는 `eio_posix` 가 무시한 SIGPIPE 를 자식이 그대로 물려받는다.
파이프 앞쪽 명령이 SIGPIPE 로 조용히 끝나지 않고 쓰기 오류를 받는다. 서버가 쓰는 다른 spawn 경로도 같은 규칙을
따른다. 신호를 무시하는 쪽은 서버 자신뿐이고, 자식은 기본 동작을 받는다 (I4).
...
2. 서버가 띄운 자식이 `/dev/tty` 를 열면 ENXIO 를 받는다. 암호가 걸린 키로 `ssh-keygen -y` 를 돌리면
   프롬프트를 쏟지 않고 곧바로 실패한다.
3. 포그라운드로 띄우고 ^C 를 보낸다. 서버가 정리 종료하고 자식이 모두 끝난다.
4. pty 를 닫는다(hangup). 3 과 같은 결과가 나온다.
5. 막는 장치를 빼면 1 의 조건에서 감독·서버·자식이 멈추는지 음성 대조로 확인한다.
6. 감독을 SIGKILL 로 죽인 뒤 서버가 로그를 쓰게 한다. 서버가 살아 있고 `system_log` 에 계속 쓰는지 본다.
   리눅스와 macOS 둘 다에서 본다 (백엔드가 SIGPIPE 를 다르게 다룬다).
7. 서버가 띄운 자식의 SIGPIPE 가 기본값인지 본다.
...
1. 터미널 모드는 감독이다 (§4.2). 포그라운드 로그와 ^C 가 지금처럼 남는다.
2. `masc-stdio` 는 시작할 때 `setsid()` 로 클라이언트의 프로세스 그룹을 떠난다 (§4.3). 클라이언트가 그룹
   단위로 MCP 서버를 끄면, 이 서버는 stdin EOF 로 끝난다. MCP stdio 는 stdin 이 닫히면 끝나는 약속이다.
3. 감독이 먼저 사라지면 서버는 콘솔 쓰기를 멈추고 로그 파일만 쓴다 (§4.2 의 5).
