  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Mcp_eio.execute_tool_eio
         ~sw
         ~clock
         ~mcp_session_id:"approval-get-test"
         state
...
let test_risk_classification_low () =
  let tools =
    [ "masc_status", GP.Low; "keeper_context_status", GP.Low; "masc_board_list", GP.Low ]
  in
  List.iter
    (fun (tool_name, expected) ->
       let actual = GP.assess_risk ~tool_name ~input:(`Assoc []) in
...
let test_keeper_shell_gh_read_only_stays_low () =
  ...
  in
  check
    "keeper_shell op=gh pr view → low"
...
let test_keeper_shell_gh_mutation_escalates_high () =
  ...
  in
  check
    "keeper_shell op=gh pr comment → high"
...
let test_enterprise_blocks_high () =
  ...
  | Some GP.High -> ()
  | Some other ->
    Alcotest.fail (Printf.sprintf "expected High, got %s" (GP.risk_level_to_string other))
...
let test_approval_queue_submit_and_resolve () =
  ...
  | Some (Agent_sdk.Hooks.Reject r) -> Alcotest.fail ("expected Approve, got Reject: " ^ r)
  | Some (Agent_sdk.Hooks.Edit _) -> Alcotest.fail "expected Approve, got Edit"
...
let test_approval_queue_reject () =
  ...
  | Some (Agent_sdk.Hooks.Reject reason) ->
    Alcotest.(check bool) "reason contains text" true (String.length reason > 0)
...
let test_background_pending_callback_and_keeper_lookup () =
  ...
  in
  Alcotest.(check bool)
    "keeper has pending approval"
    true
    (AQ.has_pending_for_keeper ~keeper_name:"gate-keeper");
  Alcotest.(check int) "background entry added" (initial_count + 1) (AQ.pending_count ());
  (match AQ.resolve ~id ~decision:Agent_sdk.Hooks.Approve with
   | Ok () -> ()
   | Error err -> Alcotest.fail ("resolve failed: " ^ AQ.resolve_error_to_string err));
  Alcotest.(check bool)
    "keeper pending cleared"
    false
    (AQ.has_pending_for_keeper ~keeper_name:"gate-keeper");
  Alcotest.(check int) "background entry removed" initial_count (AQ.pending_count ());
...
let test_background_pending_reuses_existing_entry () =
  ...
  Alcotest.(check int) "only one entry created" (initial_count + 1) after_first;
  Alcotest.(check int)
    "second submit does not grow queue"
    after_first
    (AQ.pending_count ());
  ...
  Alcotest.(check bool)
    ...
    (match !first_callback with
     | Some Agent_sdk.Hooks.Approve -> true
     | _ -> false);
  Alcotest.(check bool)
    "second callback not attached to duplicate submit"
    true
...
let test_background_pending_distinct_inputs_do_not_reuse_entry () =
  ...
  Alcotest.(check int) "two entries created" (initial_count + 2) (AQ.pending_count ());
  ...
  Alcotest.(check int) "cleanup restores count" initial_count (AQ.pending_count ())
;;
...
let test_approval_queue_get_pending_detail () =
  ...
  Alcotest.(check int) "detail turn id" 7 (detail |> member "turn_id" |> to_int);
  Alcotest.(check string)
    "detail task id"
    "task-runtime-trust"
  ...
  Alcotest.(check bool)
    ...
    (match !callback_result with
     | Some Agent_sdk.Hooks.Approve -> true
     | _ -> false);
  Alcotest.(check int) "entry removed" initial_count (AQ.pending_count ())
;;
...
let test_approval_get_dispatch_success () =
  ...
  Fun.protect
    ...
    (fun () ->
       let ok, payload = execute_approval_get (`Assoc [ "id", `String id ]) in
       Alcotest.(check bool) "dispatch approval_get success" true ok;
       let open Yojson.Safe.Util in
       let json = Yojson.Safe.from_string payload in
       Alcotest.(check string) "dispatch detail id" id (json |> member "id" |> to_string);
       Alcotest.(check string)
         "dispatch includes full input"
         (Yojson.Safe.to_string input)
         (json |> member "input" |> Yojson.Safe.to_string));
  Alcotest.(check int)
    "dispatch cleanup removes pending"
    initial_count
    (AQ.pending_count ());
  Alcotest.(check bool)
    "cleanup rejects callback"
    true
    (match !callback_result with
     | Some (Agent_sdk.Hooks.Reject _) -> true
     | _ -> false)
;;
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Ok { remembered_rule = Some _ } ->
         let open Yojson.Safe.Util in
         let summary = AQ.policy_summary_json ~base_path ~keeper_name:"remember-keeper" in
         Alcotest.(check int)
           "allow rules persisted"
           1
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Ok { remembered_rule = None } ->
         ...
         let summary = AQ.policy_summary_json ~base_path ~keeper_name:"remember-keeper" in
         Alcotest.(check int)
           "no persisted rules for high allow"
           0
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let rule_id =
         ...
         | Error err ->
           Alcotest.fail
             ("dashboard resolve failed: " ^ SDH.approval_resolve_http_error_to_string err)
       in
       Alcotest.(check int)
         "room rule persisted"
         1
       ...
       (match
        ...
        with
        | Ok _ -> ()
        | Error message -> Alcotest.fail ("dashboard rule delete failed: " ^ message));
       Alcotest.(check int)
         "room rule deleted"
         0
         (List.length (AQ.list_rules ~base_path:room_base ()));
       Alcotest.(check int)
         "env fallback still empty"
         0
         (List.length (AQ.list_rules ~base_path:env_base ())))
;;
...
let test_approval_get_dispatch_not_found () =
  let ok, msg = execute_approval_get (`Assoc [ "id", `String "appr_missing" ]) in
...
let test_approval_get_rejects_worker_role () =
  match
    Masc_mcp.Auth.authorize_tool_for_role
      ~agent_name:"worker"
      ~role:Types.Worker
  ...
  | Error err ->
    Alcotest.fail
      (Printf.sprintf "expected forbidden, got %s" (Types.masc_error_to_string err))
       ...
       Eio.Fiber.fork ~sw (fun () ->
         ...
         let decision =
           cb
             ...
             ~input:
               (`Assoc
                   [ "path", `String "lib/example.ml"; "content", `String "let x = 1\n" ])
         in
         result := Some decision);
...
let test_callback_paranoid_medium_risk_uses_remembered_policy () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Agent_sdk.Hooks.Approve ->
         Alcotest.(check int)
           "remembered policy bypasses queue"
           pending_before
           (AQ.pending_count ())
       | Agent_sdk.Hooks.Reject reason ->
         Alcotest.fail ("expected remembered approve, got reject: " ^ reason)
       | Agent_sdk.Hooks.Edit _ -> Alcotest.fail "expected remembered approve, got edit")
;;
...
let test_runtime_trust_classifies_always_approve_flag () =
  ...
  Fun.protect ~finally:AQ.For_testing.reset_audit_store (fun () ->
    ...
    in
    AQ.audit_approval_event
      ~base_path:config.base_path
      ~event_type:"auto_approved_always"
      ~id:"auto-always-flag-test"
      ~keeper_name
      ~tool_name:"masc_create_task"
      ~risk_level:AQ.Medium
      ~auto_approved:true
      ();
    let snapshot = Masc_mcp.Keeper_runtime_trust_snapshot.snapshot_json ~config ~meta in
    let open Yojson.Safe.Util in
    let approval = snapshot |> member "approval" in
    Alcotest.(check string)
      "runtime trust always flag state"
      "always_flag"
      (approval |> member "state" |> to_string);
    Alcotest.(check string)
      "runtime trust latest event kind"
...
let test_callback_always_approve_respects_forbidden () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       yield_until (fun () -> AQ.pending_count () = initial_pending + 1);
       Alcotest.(check int)
         "destructive tool still requires approval"
         (initial_pending + 1)
         (AQ.pending_count ());
       let pending_json = AQ.list_pending_json () in
       let id =
...
let test_read_recent_audit_filters_after_wide_scan () =
  ...
  | [ json ] ->
    Alcotest.(check string)
      "target approval survives unrelated tail"
      "target-audit"
      Yojson.Safe.Util.(json |> member "id" |> to_string)
  | items ->
    Alcotest.fail (Printf.sprintf "expected one target audit, got %d" (List.length items))
;;
  ...
  Alcotest.run
    ...
    [ ( "risk_classification"
      , [ Alcotest.test_case "critical tools" `Quick test_risk_classification_critical
        ...
        ; Alcotest.test_case
            "keeper_shell op=gh read-only stays low"
            `Quick
            test_keeper_shell_gh_read_only_stays_low
        ; Alcotest.test_case
            "keeper_shell op=gh mutation escalates high"
            `Quick
    ...
    ; ( "threshold_decisions"
      , [ Alcotest.test_case "development allows all" `Quick test_development_allows_all
        ...
        ; Alcotest.test_case
            ...
            `Quick
            test_production_blocks_critical
    ...
    ; ( "approval_queue"
      , [ Alcotest.test_case
            "submit and approve"
            `Quick
            test_approval_queue_submit_and_resolve
        ...
        ; Alcotest.test_case
            "resolve nonexistent"
            `Quick
            test_approval_resolve_nonexistent
        ; Alcotest.test_case
            "cancel cleans up"
            `Quick
            test_approval_queue_cancel_cleans_up
        ; Alcotest.test_case
            "background pending callback"
            `Quick
            test_background_pending_callback_and_keeper_lookup
        ; Alcotest.test_case
            "background pending reuses existing entry"
            `Quick
            test_background_pending_reuses_existing_entry
        ; Alcotest.test_case
            "background pending distinct inputs do not reuse entry"
            `Quick
            test_background_pending_distinct_inputs_do_not_reuse_entry
        ; Alcotest.test_case
            "get pending detail includes full input"
            `Quick
            test_approval_queue_get_pending_detail
        ; Alcotest.test_case
            "dispatch approval_get success"
            `Quick
            test_approval_get_dispatch_success
        ; Alcotest.test_case
            "dispatch approval_get missing id"
            `Quick
            test_approval_get_dispatch_missing_id
        ; Alcotest.test_case
            "dispatch approval_get not found"
            `Quick
            test_approval_get_dispatch_not_found
        ; Alcotest.test_case
            "approval_get rejects worker role"
            `Quick
            test_approval_get_rejects_worker_role
        ; Alcotest.test_case
            "resolve_with_policy remembers medium allow"
            `Quick
            test_resolve_with_policy_remembers_medium_allow
        ; Alcotest.test_case
            "resolve_with_policy skips high allow memory"
            `Quick
            test_resolve_with_policy_does_not_remember_high_allow
        ; Alcotest.test_case
            "dashboard approve-always rules use room base_path"
            `Quick
            test_dashboard_resolve_and_delete_rules_use_room_base_path
        ; Alcotest.test_case
            "read_recent_audit scans before keeper filter"
            `Quick
    ...
    ; ( "callback_integration"
      , [ Alcotest.test_case
            ...
            `Quick
            test_callback_approves_low_risk
        ; Alcotest.test_case
            "production keeper write requires approval"
            `Quick
            test_callback_production_keeper_write_requires_approval
        ; Alcotest.test_case
            "production keeper_shell op=gh read-only auto-approved"
            `Quick
            test_callback_production_keeper_shell_gh_read_only_auto_approved
        ; Alcotest.test_case
            "paranoid medium risk uses remembered policy"
            `Quick
            test_callback_paranoid_medium_risk_uses_remembered_policy
        ; Alcotest.test_case
            "always_approve bypasses threshold"
            `Quick
            test_callback_always_approve_bypasses_threshold
        ; Alcotest.test_case
            "runtime trust classifies always_approve flag"
            `Quick
            test_runtime_trust_classifies_always_approve_flag
        ; Alcotest.test_case
            "always_approve respects forbidden"
            `Quick
