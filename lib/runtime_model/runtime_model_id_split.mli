(** Canonical split for a ["provider:model_id"] runtime model spec.
    ...
    {!Runtime} boundary (AGENT_CORE-facing). It is a zero-dependency leaf so that both
    ...
    This split lives strictly inside the AGENT_CORE/runtime boundary. masc-core
    consumers (auth, keeper dispatch) must not call it: a runtime id is
    opaque to the masc core, and only AGENT_CORE / the runtime adapter parses an id
