...
let seq_of_event (e : Trpg.Engine_event.t) = e.seq
...
let test_read_events_skips_malformed_rows () =
  ...
  get_ok (Trpg.Engine_store_sqlite.append_event ~base_dir ~event:(make_event ~seq:1 ~room_id));
  ...
  ignore (Sqlite3.db_close db);
  get_ok (Trpg.Engine_store_sqlite.append_event ~base_dir ~event:(make_event ~seq:3 ~room_id));
  let all = get_ok (Trpg.Engine_store_sqlite.read_events ~base_dir ~room_id) in
  Alcotest.(check (list int)) "skip malformed row" [ 1; 3 ] (List.map seq_of_event all);
  let tail = get_ok (Trpg.Engine_store_sqlite.read_events_after ~base_dir ~room_id ~after_seq:2) in
...
let test_append_and_read_events () =
  ...
  get_ok (Trpg.Engine_store_sqlite.append_event ~base_dir ~event:(make_event ~seq:1 ~room_id));
  get_ok (Trpg.Engine_store_sqlite.append_event ~base_dir ~event:(make_event ~seq:2 ~room_id));
  get_ok (Trpg.Engine_store_sqlite.append_event ~base_dir ~event:(make_event ~seq:3 ~room_id));
  let all = get_ok (Trpg.Engine_store_sqlite.read_events ~base_dir ~room_id) in
  Alcotest.(check (list int)) "seq order" [ 1; 2; 3 ] (List.map seq_of_event all);
  let tail = get_ok (Trpg.Engine_store_sqlite.read_events_after ~base_dir ~room_id ~after_seq:1) in
...
let test_snapshot_roundtrip () =
  ...
  let state =
    Trpg.Engine_types.initial_room_state
      ...
      ~turn_order:[ "a"; "b" ]
    |> fun s -> { s with phase = Trpg.Engine_types.Round; current_turn_index = Some 1; round = 3 }
  in
  get_ok
    (Trpg.Engine_store_sqlite.write_snapshot
       ~base_dir
       ~room_id
       ~last_seq:5
       ~ts:"2026-02-15T10:30:00Z"
       ~state);
  let snap = get_ok (Trpg.Engine_store_sqlite.read_snapshot ~base_dir ~room_id) in
  match snap with
  | None -> Alcotest.fail "snapshot should exist"
  | Some s ->
      Alcotest.(check int) "last_seq" 5 s.last_seq;
      Alcotest.(check string) "phase" "round" (Trpg.Engine_types.string_of_phase s.state.phase)
...
let test_recovery_tail_events () =
  ...
  List.iter
    (fun seq ->
      get_ok (Trpg.Engine_store_sqlite.append_event ~base_dir ~event:(make_event ~seq ~room_id)))
    [ 1; 2; 3; 4; 5 ];
  let state =
    Trpg.Engine_types.initial_room_state
      ~room_id
  ...
  get_ok
    (Trpg.Engine_store_sqlite.write_snapshot
       ...
       ~state);
  let snapshot_opt, tail = get_ok (Trpg.Engine_store_sqlite.load_recovery ~base_dir ~room_id) in
