...
1. `Agent_sdk.Tool.descriptor` 의 `keeper_bash` JSON schema 를
   `{cmd: string}` → `{executable: string, argv: string[], cwd?: string, env?: {[k]: v}, pipeline?: stage[]}` 변형 으로 갱신.
2. `keeper_runtime.toml`, `cascade.toml` 의 tool descriptor 영향 영역 동기 갱신
   (없다면 명시).
3. dogfooding: PR-3 자체의 CI 가 새 schema 로 keeper turn 을 돌려 통과해야
   머지 가능. CI green = 해당 keeper persona LLM 이 새 schema 를 인식하고
   argv array 로 응답하는지의 *현장 검증*.
4. failure mode: PR-3 머지 후 keeper LLM 이 여전히 string cmd 로 응답하면
   tool descriptor 검증이 *boundary parse* 에서 실패하고 `Error (Boundary_invalid {expected = "argv array"; got = "string"})` 를 반환한다. 이 boundary parse failure 는 PR-1 의 `validate_bash_input` 동일 경로를 그대로 사용하므로, *string-as-protocol 가 응답에 잔존해도 lexer 가 부활하지 않는다*.
...
1. **keeper LLM 적응 transition**. PR-3 머지 직후 keeper persona LLM 이 새
   ...
   머지를 *staged rollout* (한 keeper 부터, 24h soak 후 fleet 확대) 으로 진행.
...
- [ ] PR-2 caller 전수 변환 + lexer 삭제
- [ ] PR-3 tool descriptor JSON schema + dogfooding
- [ ] PR-4 godfile 분해
