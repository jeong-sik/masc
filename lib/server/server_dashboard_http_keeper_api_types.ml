...
let keeper_chat_pending_cancel_route req_path : keeper_chat_pending_cancel_route option =
...
let classify_keeper_post_route req_path =
  ...
  with
  | Some route, _ -> Keeper_post_chat_pending_cancel route
  | None, Some route -> Keeper_post_board_attention_quarantine_recovery route
