...
(** [tts_report ~body] synthesizes the body's ["message"] on every configured
    ...
    that works says nothing about the endpoints behind it, so a dead fallback
    looks exactly like a healthy one until the endpoint in front of it goes
    away. [Error] carries a reason for the caller to return as a bad
    request. *)
