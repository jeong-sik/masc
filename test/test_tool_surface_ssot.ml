...
let test_keeper_voice_replacement_contract () =
  (* Voice tools physically deleted in #5050. Verify no replacement mapping. *)
  List.iter (fun name ->
    Alcotest.(check (option string))
      (name ^ " removed")
      None
      (Tool_catalog_surfaces.keeper_internal_replacement name))
...
let test_pruned_tools_registered_as_deprecated () =
  (* Tools pruned in #4999/#5011 were physically deleted in #5050.
     No deprecated entries remain — verify the list is empty. *)
  let deprecated_names =
    List.map fst Tool_catalog.deprecated_tool_entries
  in
  Alcotest.(check int) "no deprecated entries" 0 (List.length deprecated_names)
