...
let start_managed_container
  ...
  then Error "keeper sandbox start requires sandbox_profile=docker"
  else (
    ...
    in
    let probe_timeout = Env_config_exec_timeout.timeout_sec ~caller:Sandbox () in
       ...
       | Some container ->
         Ok
           (`Assoc
               ...
               ; "container", Keeper_sandbox_runtime.live_container_to_yojson container
               ])
       | None ->
         ...
         then Error "keeper sandbox docker image is not configured"
           ...
           | Ok seccomp_args ->
             ...
             let container_name = managed_container_name ~meta ~network_label in
             let argv =
             ...
             then (
               ...
               let message =
                 Printf.sprintf
                   "docker_managed_container_start_failed: %s"
...
let why_no_container (meta : keeper_meta) ~preflight containers =
  if meta.sandbox_profile = Local
  then Some "sandbox_profile=local"
  else if containers <> []
  then None
  else (
    ...
    | _ ->
      (match meta.network_mode with
       | Network_inherit ->
         Some
           "network_mode=inherit uses one-shot Docker containers unless a managed \
            sandbox is explicitly started"
       | Network_none ->
         Some
           "no active turn or managed sandbox container; Docker containers start on \
            sandboxed tool calls or via masc_keeper_sandbox_start"))
;;

let recommendation (meta : keeper_meta) ~preflight containers =
  if meta.sandbox_profile = Local
  then Some "No Docker container is expected for sandbox_profile=local."
  else if containers <> []
  then None
  else (
    match preflight_ok preflight with
    | Some false ->
      Some "Fix Docker preflight first, then rerun masc_keeper_sandbox_status."
    | _ ->
      Some
        (Printf.sprintf
           "Run masc_keeper_sandbox_start with name=%S to prewarm a visible managed \
            container."
           meta.name))
;;
    ...
    ; ( "warnings"
      , if agent_name_matches
        then `List []
          ...
          `List
            [ `String
                "keeper agent_name does not match the canonical keeper name; repair or \
                 recreate this keeper trace before relying on scheduling evidence"
            ] )
    ]
;;
...
let live_status_json
  ...
  =
  let preflight =
    if include_preflight && meta.sandbox_profile = Docker
    then preflight_status_json ~timeout_sec
    else None
  in
  ...
  let recommendation =
    match container_error with
    | Some _ -> Some "Check Docker daemon availability and retry sandbox status."
  ...
  `Assoc
    ...
    ; ("preflight", if verbose then Json_util.option_to_yojson Fun.id preflight else `Null)
