...
`))}function Tu({hp:t,max:e}){const n=gu(t,e),s=_u(t,e);return o`
  ...
  `}function Ou({state:t,nowMs:e}){var p;const n=vt.value||((p=t.session)==null?void 0:p.room)||"",s=t.join_gate,a=is.value,i=bt(a)?a:null,r=(t.party??[]).filter(m=>m.role!=="dm"),l=$t.value.trim(),d=r.some(m=>m.id===l),u=d?l:l?"__manual__":"",v=async()=>{const m=$t.value.trim(),g=Qe.value.trim();if(!n||!m){b("Room/Actor가 필요합니다.","warning");return}Y.value="checking";try{const w=await Jr(n,m,g||void 0);is.value=w,Y.value="ok",b("참가 가능 여부를 갱신했습니다.","success")}catch(w){Y.value="error";const N=w instanceof Error?w.message:"참가 가능 여부 확인에 실패했습니다.";b(N,"error")}},c=async()=>{var h,k;const m=$t.value.trim(),g=Qe.value.trim(),w=as.value.trim();if(!n||!m||!g){b("Room/Actor/Keeper가 필요합니다.","warning");return}if(!un(e))return;const N=((h=t.current_round)==null?void 0:h.phase)??((k=t.session)==null?void 0:k.status)??"unknown";if(Js("Mid-Join 승인 요청",n,N)){Y.value="requesting";try{const O=await Vr({room_id:n,actor_id:m,keeper_name:g,role:ss.value,...w?{name:w}:{}});is.value=O;const H=bt(O)?Me(O,"granted",!1):!1,D=bt(O)?q(O,"reason_code",""):"";H?b("Mid-Join이 승인되었습니다.","success"):b(`Mid-Join이 거절되었습니다${D?`: ${D}`:""}`,"warning"),Y.value=H?"ok":"error",mt()}catch(O){Y.value="error";const H=O instanceof Error?O.message:"Mid-Join 요청에 실패했습니다.";b(H,"error")}finally{Rn()}}};return o`
    <div class="trpg-control-box">
