(** JSON builder for [GET /api/v1/runtime/resolved] — the single resolved
    ...
    max-context and which source produced it, every configured lane (including
    the sticky failover preference from {!Runtime_lane_preference}, as
    [preferred_candidate]/[preferred_at_ts]), and the
