...
| `packages/agent_core/lib/llm_provider/backend_openai.ml` | "48KB monolith, 10+ providers" | 1,550 lines, dispatches Provider-D/Provider-A/Provider-F/Ollama/Provider-K (5 providers, not 10+) | ⚠️ partially confirmed |
| `packages/agent_core/lib/llm_provider/backend_provider-a.ml` | "doesn't exist yet" implied | **210 lines, already separate file** | ❌ **STALE** — split already partially done |
| `packages/agent_core/lib/llm_provider/backend_provider-f.ml` | implied non-existent | **363 lines** | ❌ **STALE** |
| `packages/agent_core/lib/llm_provider/backend_ollama.ml` | implied non-existent | **641 lines** | ❌ **STALE** |
| `packages/agent_core/lib/llm_provider/backend_glm.ml` | not mentioned | **452 lines** | ❌ Roadmap omitted an existing backend |
...
| #12990 | refactor(runtime): externalize scoring magic numbers | **MERGED** | 2026-05-05 |
...
| `RFC-0024` | Ollama runtime integration | Phase 1-1 (Ollama gate) |
...
Current count over `lib/keeper/` + `lib/runtime/`:
...
| 3 | **`packages/agent_core/lib/llm_provider/backend_openai.ml` couples 5 providers in dispatch** | 1,550 lines; routes to Provider-A/Provider-F/Ollama/Provider-K despite separate `backend_*.ml` files (210/363/641/452 lines) existing. No `backend.mli` signature unifies them. No `backend_router.ml`. |
| 4 | **Mutex/Atomic ratio 54:31** in keeper+runtime hot paths | counted above. CAS-friendly cases (read-heavy, short critical sections) remain. |
...
- **Phase 3-3 Fair Scheduler (Token Bucket + DRR)** — current evidence (memory `feedback_semaphore_tier_is_architectural_anti_pattern.md`, 2026-05-05) recommends *multi-runtime fanout / deadline scheduling / per-provider token bucket*, not weighted-DRR. Different shape.
