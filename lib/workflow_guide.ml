  ...
  if success then
    { next_steps =
        [ s "masc_status" "Verify namespace state before proceeding";
          s "masc_start" "Use masc_start for full onboarding" ];
      preconditions = [];
      common_mistakes =
        [ "Treating masc_set_project/masc_set_room like full onboarding — use masc_start" ] }
  else
    { next_steps =
...
let after_join ~success =
  (* Compat: masc_join is removed, redirect to masc_start *)
  after_set_room ~success
...
let after_transition_start ~success =
  if success then
    { next_steps =
      ...
      preconditions = [ "project_set"; "task_claimed"; "current_task_set" ];
      common_mistakes = [] }
  else
    { next_steps =
  ...
  if success then
    { next_steps =
        [ s "masc_worktree_create" "Create worktree for isolated work";
          s "masc_heartbeat" "Signal liveness before starting long work" ];
      preconditions = [ "project_set"; "task_claimed" ];
      common_mistakes = [] }
  else
    { next_steps =
...
let after_heartbeat ~success =
  if success then
    ...
    { next_steps =
        [ s "masc_start" "Re-set project scope if session expired" ];
      preconditions = [];
      common_mistakes = [] }
  ...
  if success then
    { next_steps =
        [ s "masc_status" "Check for remaining tasks";
          ...
          s "masc_status" "Check for more tasks or signal you are done" ];
      preconditions = [ "project_set" ];
      common_mistakes = [] }
  else
    { next_steps =
...
let after_operation_start ~success =
  if success then
    { next_steps =
      ...
      preconditions = [ "project_set"; "unit_defined" ];
      common_mistakes =
...
let after_dispatch_tick ~success =
  if success then
    { next_steps =
      ...
      preconditions = [ "project_set"; "operation_active" ];
      common_mistakes = [] }
  else
    { next_steps =
...
let after_team_session_start ~success =
  if success then
    ...
    { next_steps =
        [ s "masc_start" "Ensure project scope is set";
...
let after_team_session_step ~success =
  if success then
    { next_steps =
      ...
      preconditions = [ "project_set"; "session_active" ];
      common_mistakes = [] }
  else
    { next_steps =
...
let next_steps ~tool_name ~success =
  ...
  (* Golden Path 1: Project/Task Hygiene *)
  | "masc_start" -> after_start ~success
  | "masc_set_room" | "masc_set_project" -> after_set_room ~success
  | "masc_join" -> after_join ~success  (* compat: join -> set_room *)
...
let workflow_context ~tool_name =
  let before = match tool_name with
    | "masc_status" -> [ "masc_start" ]
    | "masc_claim_next" ->
        [ "masc_start"; "masc_status" ]
...
let current_state_guidance ~room_set ~joined:_ ~task_claimed
    ~current_task_set ~worktree_active ~session_active =
  if not room_set then
    { next_steps =
        [ s "masc_start" "Set the project root — required before all other operations";
    ...
    { next_steps =
      ...
      preconditions = [ "project_set"; "task_claimed"; "current_task_set"; "worktree_active"; "session_active" ];
      common_mistakes = [] }
  else
    { next_steps =
        [ s "masc_heartbeat" "Signal liveness while working";
          ...
          s "masc_transition" "Mark task complete when finished (action=done)" ];
      preconditions = [ "project_set"; "task_claimed"; "current_task_set"; "worktree_active" ];
      common_mistakes = [] }
