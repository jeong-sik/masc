...
(** Hook fired from every [run_argv*] timeout branch.  Default no-op so
    ...
    fleet-wide). *)
val process_timeout_observer_fn : (program:string -> timeout_sec:float -> unit) Atomic.t
...
type detached_handle =
  { pid : int (** Child process PID (also the process-group leader). *)
  ; pgid : int (** Process group ID; always equal to [pid] for tree-kill. *)
  ...
  ; started_at : float (** [Unix.gettimeofday ()] at spawn time. *)
  }
...
val tree_kill : pgid:int -> signal:int -> grace_sec:float -> unit
