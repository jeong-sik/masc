...
let target_type_enums = [ `String "root"; `String "namespace"; `String "keeper" ]
...
let snapshot_schema ~remote =
  ...
  ; description =
      (if remote
       then
         "Read the unified operator control-plane state. Use this when you need current \
          namespace, keeper, message, and pending-confirm data before taking action."
       else
         "Read unified operator state for the default namespace, keepers, recent \
          messages, and pending confirmations. Use this before issuing control-plane \
          actions.")
  ; input_schema =
      `Assoc
        [ "type", `String "object"
...
let judgment_surface_enums = [ `String "command.namespace"; `String "intervene" ]
...
let digest_schema ~remote =
  ...
  ; description =
      (if remote
       then
         "Read an intervention-oriented operator digest. Use this when you need \
          namespace health, attention items, command-plane search or microarch signals, \
          worker summaries, and recommended next actions before deciding how to \
          intervene."
       else
         "Read a high-signal operator digest with intervention recommendations for the \
          default namespace. Use this when raw snapshot data is too low-level for fast \
          supervision and you want translated command-plane search or microarch signals.")
  ; input_schema =
      `Assoc
        [ "type", `String "object"
        ; ( "properties"
          , schema_properties
              ...
              ; ( "target_type"
                , `Assoc
                    [ "type", `String "string"; "enum", `List digest_target_type_enums ] )
...
let surface_audit_schema ~remote =
  ...
  ; description =
      (if remote
       then
         "Read dashboard surface readiness, exposure policy, and evidence references. \
          Use this before pointing operators to an experimental surface."
       else
         "Read dashboard surface readiness, exposure policy, and evidence references. \
          Use this to decide whether a surface belongs in main navigation, Lab, or \
          should stay hidden.")
  ; input_schema =
      `Assoc
        [ "type", `String "object"
        ; ( "properties"
          , schema_properties [ "surface_id", `Assoc [ "type", `String "string" ] ] )
        ]
  }
;;
...
let action_schema ~remote =
  let enum_values =
    if remote
    then strict_action_enums
    else strict_action_enums @ legacy_action_alias_enums
  in
  { name = "masc_operator_action"
  ; description =
      (if remote
       then
         "Preview or run a structured operator action. Use this when you need to \
          broadcast, pause a namespace, or message a keeper through the remote operator \
          surface. Use social_sweep for immediate public-square social processing."
       else
         "Run a structured operator action against the namespace or a keeper. Use this \
          when you need guided control with preview-confirm safety for disruptive \
          actions. Use social_sweep for immediate public-square social processing.")
  ; input_schema =
      `Assoc
        [ "type", `String "object"
        ; ( "properties"
          , schema_properties
              [ "actor", `Assoc [ "type", `String "string" ]
              ; ( "action_type"
                , `Assoc [ "type", `String "string"; "enum", `List enum_values ] )
              ; ( "target_type"
                , `Assoc [ "type", `String "string"; "enum", `List target_type_enums ] )
              ; "target_id", `Assoc [ "type", `String "string" ]
...
let confirm_schema =
  ...
  ; description =
      "Confirm and execute a previously previewed operator action. Use this only after \
       masc_operator_action returns confirm_required=true."
  ; input_schema =
      `Assoc
        [ "type", `String "object"
        ; ( "properties"
...
let judgment_write_schema =
  ...
  ; description =
      "Internal operator-judge write path. Use this to store a durable operator judgment \
       for namespace supervision. Hidden from the default catalog and intended for \
       keeper/automation experiments."
  ; input_schema =
      `Assoc
        [ "type", `String "object"
        ; ( "properties"
          , schema_properties
              [ ( "surface"
                , `Assoc
                    [ "type", `String "string"; "enum", `List judgment_surface_enums ] )
              ; ( "target_type"
                , `Assoc
                    [ "type", `String "string"; "enum", `List digest_target_type_enums ] )
              ; "target_id", `Assoc [ "type", `String "string" ]
        ...
        ; ( "required"
          , `List [ `String "surface"; `String "target_type"; `String "summary" ] )
        ]
  }
;;
...
let dispatch (ctx : 'a context) ~name ~args : tool_result option =
  ...
  | "masc_operator_judgment_write" ->
    Some (json_string_of_result (Operator_control.judgment_write_json control_ctx args))
  | _ -> None
;;
...
let tool_required_permission = function
  ...
  | "masc_operator_action" | "masc_operator_confirm" | "masc_operator_judgment_write" ->
