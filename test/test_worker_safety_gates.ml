...
let () =
  Masc_mcp.Tool_dispatch.init_destructive_set
    [ "keeper_bash"
    ; "keeper_fs_edit"
    ; "shell_exec"
    ; "masc_code_shell"
...
let default_gate
  ...
  : EG.gate_config
  =
  { EG.default_config with denied_tools; max_cost_usd; destructive_check_enabled }
;;
...
let test_no_gate_allows_all () =
  ...
  let result =
    fire_pre_tool_use ~hooks ~tool_name:"anything" ~input:`Null ~accumulated_cost_usd:0.0
  in
  check bool "continues" true (result = Agent_sdk.Hooks.Continue);
  check bool "tool tracked" true (List.mem "anything" !tool_names_ref)
;;
...
let test_deny_list_blocks () =
  ...
  let tool_names_ref, hooks = WO.make_tool_tracking_hooks ~gate_config () in
  let result =
    fire_pre_tool_use ~hooks ~tool_name:"evil_tool" ~input:`Null ~accumulated_cost_usd:0.0
  in
  (match result with
   | Agent_sdk.Hooks.Override msg ->
     check bool "contains reason" true (Astring.String.is_infix ~affix:"worker_deny" msg)
   | _ -> fail "expected Override for denied tool");
  (* Tool name is still tracked even when blocked *)
  check bool "tool tracked" true (List.mem "evil_tool" !tool_names_ref)
;;
...
let test_deny_list_allows_non_denied () =
  ...
  let result =
    fire_pre_tool_use ~hooks ~tool_name:"good_tool" ~input:`Null ~accumulated_cost_usd:0.0
  in
  check bool "continues" true (result = Agent_sdk.Hooks.Continue)
;;
...
let test_cost_gate_blocks_over_budget () =
  ...
  let _ref, hooks = WO.make_tool_tracking_hooks ~gate_config () in
  let result =
    fire_pre_tool_use ~hooks ~tool_name:"any_tool" ~input:`Null ~accumulated_cost_usd:0.55
  in
  match result with
  | Agent_sdk.Hooks.Override msg ->
    check bool "contains cost_gate" true (Astring.String.is_infix ~affix:"cost_gate" msg)
...
let test_cost_gate_blocks_at_exact_limit () =
  ...
  let _ref, hooks = WO.make_tool_tracking_hooks ~gate_config () in
  let result =
    fire_pre_tool_use ~hooks ~tool_name:"any_tool" ~input:`Null ~accumulated_cost_usd:0.50
  in
  match result with
  | Agent_sdk.Hooks.Override msg ->
    check bool "contains cost_gate" true (Astring.String.is_infix ~affix:"cost_gate" msg)
...
let test_cost_gate_allows_under_budget () =
  ...
  let _ref, hooks = WO.make_tool_tracking_hooks ~gate_config () in
  let result =
    fire_pre_tool_use ~hooks ~tool_name:"any_tool" ~input:`Null ~accumulated_cost_usd:0.30
  in
  check bool "continues" true (result = Agent_sdk.Hooks.Continue)
;;
...
let test_destructive_blocks_rm_rf () =
  ...
  let result =
    fire_pre_tool_use ~hooks ~tool_name:"shell_exec" ~input ~accumulated_cost_usd:0.0
  in
  match result with
  | Agent_sdk.Hooks.Override msg ->
    check
      bool
      "contains destructive_guard"
      true
...
let test_destructive_skips_non_shell_tools () =
  ...
  let input = `Assoc [ "command", `String "rm -rf /" ] in
  let result =
    fire_pre_tool_use ~hooks ~tool_name:"masc_status" ~input ~accumulated_cost_usd:0.0
  in
  check bool "continues (non-shell tool)" true (result = Agent_sdk.Hooks.Continue)
;;
...
let test_destructive_disabled () =
  ...
  check
    bool
    "continues (destructive check disabled)"
    true
...
let test_deny_takes_priority_over_cost () =
  let gate_config = default_gate ~denied_tools:[ "shell_exec" ] ~max_cost_usd:0.50 () in
  let _ref, hooks = WO.make_tool_tracking_hooks ~gate_config () in
  let result =
    fire_pre_tool_use
      ~hooks
  ...
  | Agent_sdk.Hooks.Override msg ->
    ...
    check bool "deny list first" true (Astring.String.is_infix ~affix:"worker_deny" msg)
...
let () =
  run
    "Worker Safety Gates"
    [ "no_gate", [ test_case "allows all tools" `Quick test_no_gate_allows_all ]
    ; ( "deny_list"
      , [ test_case "blocks denied tool" `Quick test_deny_list_blocks
        ; test_case "allows non-denied tool" `Quick test_deny_list_allows_non_denied
        ] )
    ; ( "cost_gate"
      , [ test_case "blocks over budget" `Quick test_cost_gate_blocks_over_budget
        ; test_case "blocks at exact limit" `Quick test_cost_gate_blocks_at_exact_limit
        ; test_case "allows under budget" `Quick test_cost_gate_allows_under_budget
        ] )
    ; ( "destructive"
      , [ test_case "blocks rm -rf" `Quick test_destructive_blocks_rm_rf
        ; test_case "allows safe command" `Quick test_destructive_allows_safe_command
        ; test_case "skips non-shell tools" `Quick test_destructive_skips_non_shell_tools
        ; test_case "disabled flag" `Quick test_destructive_disabled
        ] )
    ; ( "priority"
      , [ test_case "deny > cost > destructive" `Quick test_deny_takes_priority_over_cost
        ] )
    ]
;;
