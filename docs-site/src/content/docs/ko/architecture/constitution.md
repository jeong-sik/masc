...
flowchart TD
    ...
    ROOT --> P4["4. 외부 호출 전 영속화<br/>(persist_before_deliberate)"]
    ROOT --> P5["5. 실패 증거 보존<br/>(failure_leaves_evidence)"]
```
...
4. **외부 호출 전 영속화 (`persist_before_deliberate`)**: 외부 LLM API 호출 전 의도와 입력 대상을 디스크에 먼저 기록합니다.
5. **실패 증거 보존 (`failure_leaves_evidence`)**: 도구 실행이나 통신 실패 시 타임스탬프와 실패 내역을 증거 레코드에 기록합니다.
