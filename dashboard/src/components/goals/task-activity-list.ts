...
function ActivityEntry({ event }: { event: UnifiedTraceEvent }) {
  ...
  if (!hasDetail) {
    return html`
      <div class="flex items-center gap-3 py-1.5 px-3 rounded hover:bg-[var(--white-3)] transition-colors" role="listitem">
...
export function TaskActivityList({
  ...
  return html`
    <div class="flex flex-col gap-2" role="region" aria-label="활동 목록">
      ...
      <div class="flex flex-col gap-0.5 max-h-100 overflow-y-auto custom-scrollbar" tabindex="0" role="list" aria-label="활동 이력">
        ${filtered.map((evt, i) => {
