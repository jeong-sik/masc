...
let repo_root () =
  let rec loop dir =
    ...
    then dir
      ...
      if String.equal parent dir then fail "repo root not found" else loop parent)
  in
  loop (Sys.getcwd ())
;;
...
let test_live_spotcheck_serializes_route_values_as_routes () =
  ...
  match
    List.find_opt
      (fun surface -> Yojson.Safe.Util.(surface |> member "id" |> to_string = "overview"))
      surfaces
  with
