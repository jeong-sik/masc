...
## Env_config_keeper (42 knobs; typed classification 23/36)
...
| `MASC_KEEPER_BODY_TIMEOUT_SEC` | string_literal | n/a | n/a | 538 | Total HTTP body-consumption deadline for non-streaming OAS completion calls. In agent_sdk this wraps [Complete.comple... |
...
| `MASC_KEEPER_COMPACTION_SNAPSHOT_DEFAULT_LIMIT` | typed:int | Runtime | operator | 278 | Default item limit for [GET /keepers/:name/compaction-snapshots]. Default: 25. @category Runtime @ops_class operator |
| `MASC_KEEPER_COMPACTION_SNAPSHOT_MANIFEST_SCAN_LIMIT_MULTIPLIER` | typed:int | Runtime | operator | 304 | Multiplier from requested item limit to manifest files scanned. Default: 4. @category Runtime @ops_class operator |
| `MASC_KEEPER_COMPACTION_SNAPSHOT_MANIFEST_SCAN_MIN_FILES` | typed:int | Runtime | operator | 294 | Minimum manifest files scanned before applying [limit * multiplier]. Default: 8. @category Runtime @ops_class operator |
| `MASC_KEEPER_COMPACTION_SNAPSHOT_MANIFEST_TAIL_MAX_LINES` | typed:int | Runtime | operator | 314 | Tail line count read from each selected manifest file. Default: 200. @category Runtime @ops_class operator |
| `MASC_KEEPER_COMPACTION_SNAPSHOT_MAX_LIMIT` | typed:int | Runtime | operator | 284 | Maximum accepted item limit for the compaction snapshot endpoint. Default: 100. @category Runtime @ops_class operator |
...
| `MASC_KEEPER_DURABLE_QUEUE_STALE_SEC` | typed:float | Telemetry | operator | 463 | Durable event-queue backlog age threshold for fleet health degradation. The durable queue remains fully reported rega... |
| `MASC_KEEPER_GENERATED_MEDIA_DIR_MAX_BYTES` | typed:int | Policies | operator | 401 | Maximum total bytes retained in [<masc_dir>/media] after opportunistic cleanup. Default is 500 MiB. Range: [1, 5 GiB]... |
| `MASC_KEEPER_GENERATED_MEDIA_MAX_BYTES` | typed:int | Policies | operator | 390 | Maximum raw generated-media bytes accepted by the durable store and serve route. Default is 10 MiB. Range: [1, 50 MiB... |
| `MASC_KEEPER_GENERATED_MEDIA_RETENTION_SEC` | typed:float | Policies | operator | 412 | Maximum generated-media file age retained by opportunistic cleanup. Default is 24 hours. Range: [1 second, 30 days]. ... |
...
| `MASC_KEEPER_SLEEP_CHUNK_SEC` | typed:float | Thresholds | operator | 487 | Interruptible sleep chunk size in seconds: the upper bound on how long a keeper's heartbeat sleep takes to notice a w... |
...
| `MASC_KEEPER_STREAM_IDLE_TIMEOUT_SEC` | string_literal | n/a | n/a | 498 |  |
| `MASC_KEEPER_VISION_CANDIDATE_BACKOFF_BASE_SEC` | typed:float | Timeouts | operator | 351 | Base delay before trying the next vision runtime after a failed provider attempt. A small default avoids tight failov... |
| `MASC_KEEPER_VISION_CANDIDATE_BACKOFF_MAX_SEC` | typed:float | Timeouts | operator | 361 | Upper bound for the per-candidate vision failover delay. Range: [base, 30] seconds, so a typo cannot exceed the tool'... |
| `MASC_KEEPER_VISION_MAX_IMAGE_BYTES` | typed:int | Policies | operator | 341 | Maximum raw image bytes accepted by the one-shot vision tool before provider-message construction. Default is 5 MiB t... |
...
| `MASC_KEEPER_WORK_AS_HEARTBEAT` | feature_flag | n/a | n/a | 439 | Master switch. When true, successful Workspace.heartbeat after a unified turn counts as presence proof, allowing the ... |
...
| `MASC_ASSETS_DIR` | string_literal | n/a | n/a | 422 |  |
| `MASC_BASE_PATH_RESOLUTION_SOURCE` | string_literal | n/a | n/a | 426 |  |
| `MASC_BASE_PATH_STRICT` | string_literal | n/a | n/a | 428 |  |
...
| `MASC_DEFAULT_MODEL` | string_literal | n/a | n/a | 388 |  |
| `MASC_DEFAULT_PROVIDER` | string_literal | n/a | n/a | 390 |  |
...
| `MASC_OTEL_ENABLED` | string_literal | n/a | n/a | 474 |  |
| `MASC_PLACEHOLDER_TOOLS_ENABLED` | string_literal | n/a | n/a | 500 |  |
| `MASC_SEARXNG_URL` | string_literal | n/a | n/a | 508 |  |
| `MASC_SHUTDOWN_CLEANUP_TIMEOUT` | string_literal | n/a | n/a | 448 |  |
| `MASC_SHUTDOWN_DRAIN_TIMEOUT` | string_literal | n/a | n/a | 450 |  |
| `MASC_SHUTDOWN_FORCE_TIMEOUT` | string_literal | n/a | n/a | 452 |  |
| `MASC_SHUTDOWN_NOTIFY_DELAY` | string_literal | n/a | n/a | 454 |  |
| `MASC_SSE_STREAM_CAPACITY` | string_literal | n/a | n/a | 460 |  |
| `MASC_TELEMETRY_MAX_BYTES` | string_literal | n/a | n/a | 48 |  |
| `MASC_TELEMETRY_RETENTION_DAYS` | string_literal | n/a | n/a | 45 |  |
| `MASC_TEST_ALLOW_BASE_PATH_OVERRIDE` | string_literal | n/a | n/a | 490 |  |
| `MASC_TEST_ALLOW_CONFIG_PATH_OVERRIDE` | string_literal | n/a | n/a | 492 |  |
