...
(* The keeper.world frame and event-row prose this suite asserts moved out
   of the .ml sources into config/prompts/keeper.world.*.md group files,
   rendered through the prompt registry at observation/assembly time. This
   executable never pinned a markdown dir, so prompt resolution depended on
   whatever the host/dune context happened to expose — green on developer
   machines, bare-data fallbacks inside the CI dune sandbox. Pin resolution
   to the repo's own prompt files — the same idiom test_tool_task_coverage
   uses; that executable passes inside the CI sandbox, so the mechanism is
   CI-proven. *)
