...
let test_procedure_basic () =
  ...
  Alcotest.(check string)
    "pattern preserved"
    "When deploy fails, rollback and notify"
    op.pattern;
  Alcotest.(check string)
    "action = pattern"
    "When deploy fails, rollback and notify"
...
let test_procedure_metadata () =
  ...
  (match List.assoc_opt "agent_name" op.metadata with
   | Some (`String name) -> Alcotest.(check string) "agent_name" "test-agent" name
...
let test_memory_scratchpad_lifecycle () =
  ...
  let memory = Memory_oas_bridge.create_memory ~agent_name:"test-scratch" () in
  (* Store to scratchpad *)
  ignore
    (Oas.Memory.store memory ~tier:Oas.Memory.Scratchpad "current_tool" (`String "bash"));
...
let test_memory_working_survives_clear () =
  let dir = setup_tmp_dir () in
  let memory = Memory_oas_bridge.create_memory ~agent_name:"test-working" () in
  ignore
    (Oas.Memory.store
       memory
       ~tier:Oas.Memory.Working
       "session_goal"
       (`String "deploy v2"));
  ignore
    (Oas.Memory.store memory ~tier:Oas.Memory.Scratchpad "temp" (`String "ephemeral"));
  Oas.Memory.clear_scratchpad memory;
  ...
  (match Oas.Memory.recall memory ~tier:Oas.Memory.Working "session_goal" with
   | Some (`String g) -> Alcotest.(check string) "goal preserved" "deploy v2" g
...
let test_memory_promote_scratchpad_to_working () =
  let dir = setup_tmp_dir () in
  let memory = Memory_oas_bridge.create_memory ~agent_name:"test-promote" () in
  ignore
    (Oas.Memory.store
       memory
       ~tier:Oas.Memory.Scratchpad
       "important_finding"
       (`String "bug in auth"));
  ...
  (match Oas.Memory.recall memory ~tier:Oas.Memory.Working "important_finding" with
   | Some (`String v) -> Alcotest.(check string) "promoted value" "bug in auth" v
...
let test_episodic_store_recall () =
  ...
  Alcotest.(check string) "action matches" "Deployed v2 to staging" r.action;
  cleanup_tmp_dir dir
;;
...
let test_episodic_salience_ordering () =
  ...
  Alcotest.(check string) "highest salience first" "high" (List.hd recalled).id;
  cleanup_tmp_dir dir
;;
...
let test_flush_procedures_dedupes_legacy_records () =
  ...
  Alcotest.(check int) "legacy duplicates rewritten away" 1 (List.length persisted);
  ...
  Alcotest.(check string) "latest evidence preserved" "latest" (List.hd final.evidence);
  cleanup_tmp_dir dir
;;
...
let test_load_procedures_text_refreshes_after_external_append () =
  ...
  (match text_before with
   | Some t ->
     Alcotest.(check bool) "first procedure in text" true (contains_substring t "restart")
  ...
  (match text_after with
   | Some t ->
     Alcotest.(check bool)
       "cache invalidated — both procedures in text"
       true
...
let test_flush_procedures_updates_cache_for_immediate_reload () =
  ...
  (match text with
   | Some t ->
     Alcotest.(check bool)
       "flushed procedure visible in text"
       true
...
let test_jsonl_backend_persist () =
  let sid =
    Printf.sprintf "test-persist-%d" (int_of_float (Unix.gettimeofday () *. 1000.0))
  in
  let backend =
...
let test_jsonl_backend_retrieve () =
  let sid =
    Printf.sprintf "test-retrieve-%d" (int_of_float (Unix.gettimeofday () *. 1000.0))
  in
  let backend =
    Memory_oas_bridge.make_backend ~agent_name:"test-jsonl" ~session_id:sid ()
  in
...
let test_jsonl_backend_query () =
  let sid =
    Printf.sprintf "test-query-%d" (int_of_float (Unix.gettimeofday () *. 1000.0))
  in
  let backend =
    Memory_oas_bridge.make_backend ~agent_name:"test-jsonl" ~session_id:sid ()
  in
...
let test_jsonl_backend_uses_explicit_base_dir () =
  ...
  let sid =
    Printf.sprintf "test-scope-%d" (int_of_float (Unix.gettimeofday () *. 1000.0))
  in
  let backend =
    Memory_oas_bridge.make_backend ~base_dir ~agent_name:"test-jsonl" ~session_id:sid ()
  in
  ...
  in
  Alcotest.(check bool)
    "writes under explicit base_dir"
    true
...
let test_failed_turn_episode_flushes_failure_outcome () =
  ...
  Fun.protect
    ...
    (fun () ->
       let memory = Memory_oas_bridge.create_memory ~agent_name:"test-failed-turn" () in
       Memory_oas_bridge.store_failed_turn_episode
         ...
         ();
       let flushed =
         Memory_oas_bridge.flush_episodes ~memory ~agent_name:"test-failed-turn"
       in
       ...
       | Some episode ->
         ...
         Alcotest.(check string) "turn preserved" "7" (List.assoc "turn" episode.context);
         Alcotest.(check string)
  ...
  Alcotest.run
    ...
    ; ( "scratchpad_working"
      , [ Alcotest.test_case
            ...
            `Quick
            test_memory_scratchpad_lifecycle
        ; Alcotest.test_case
            "working survives clear"
            `Quick
            test_memory_working_survives_clear
        ; Alcotest.test_case
            "promote to working"
            `Quick
            test_memory_promote_scratchpad_to_working
        ] )
    ; ( "episodic"
    ...
    ; ( "procedural"
      , [ Alcotest.test_case "store and recall" `Quick test_procedural_store_recall
        ...
        ; Alcotest.test_case
            "flush dedupes legacy records"
            `Quick
            test_flush_procedures_dedupes_legacy_records
        ; Alcotest.test_case
            "load_procedures_text refreshes after append"
            `Quick
            test_load_procedures_text_refreshes_after_external_append
        ; Alcotest.test_case
            "flush updates cache for immediate reload"
            `Quick
            test_flush_procedures_updates_cache_for_immediate_reload
        ] )
    ; "stats", [ Alcotest.test_case "all tiers" `Quick test_stats_all_tiers ]
    ; ( "jsonl_backend"
      , [ Alcotest.test_case "persist returns Ok" `Quick test_jsonl_backend_persist
        ...
        ; Alcotest.test_case
            "uses explicit base_dir"
            `Quick
            test_jsonl_backend_uses_explicit_base_dir
        ; Alcotest.test_case
            "flush_episodes appends only new"
            `Quick
            test_flush_episodes_appends_only_new_records
        ; Alcotest.test_case
            "failed keeper turn flushes failure outcome"
            `Quick
