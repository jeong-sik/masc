...
let field_error kind field message = Printf.sprintf "%s.%s: %s" kind field message
...
let required_string_field kind json field =
  ...
  | `String value ->
    ...
    if trimmed = ""
    then Stdlib.Error (field_error kind field "empty string")
    else Ok trimmed
  | value ->
    Stdlib.Error
      (field_error
...
let optional_string_field json field =
  ...
  | value ->
    Stdlib.Error
      (Printf.sprintf
         "%s: expected string or null, got %s"
         field
...
let optional_int_field json field =
  ...
  | value ->
    Stdlib.Error
      (Printf.sprintf "%s: expected int or null, got %s" field (yojson_type_name value))
;;
...
let optional_float_field json field =
  ...
  | value ->
    Stdlib.Error
      (Printf.sprintf "%s: expected float or null, got %s" field (yojson_type_name value))
;;
  ...
  let* warnings =
    ...
    | `List values ->
      let rec collect acc = function
        ...
        | `String value :: rest ->
          let trimmed = String.trim value in
          if trimmed = "" then collect acc rest else collect (trimmed :: acc) rest
...
let execution_link_of_yojson_result (json : Yojson.Safe.t)
  : (execution_link, string) result
  =
