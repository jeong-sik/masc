...
let should_route_read ~(meta : keeper_meta) : bool = is_hardened meta.sandbox_profile
...
let container_root ~(meta : keeper_meta) = Keeper_sandbox.container_root meta.name
...
let container_path_of_host ~config ~(meta : keeper_meta) ~host_path
  ...
  let host_norm =
    Keeper_alerting_path.normalize_path_for_check host_path |> strip_trailing_slashes
  in
  let croot = container_root ~meta in
  if host_norm = host_root
  then Ok croot
  else if String.starts_with ~prefix:(host_root ^ "/") host_norm
  then (
    let suffix =
      String.sub
        host_norm
...
let run_command_in_container_with_status
  ...
  if Option.is_none turn_sandbox_runtime && String.trim image = ""
  then Error "keeper sandbox docker image is not configured"
  else if command_argv = []
  then Error "run_command_in_container_with_status: command_argv is empty"
  else (
      ...
      (match Keeper_sandbox_runtime.ensure_keeper_sandbox_runtime ~timeout_sec with
       ...
       | Ok seccomp_args ->
         ...
         (match st with
          ...
          | Unix.WSIGNALED n ->
            Error (Printf.sprintf "docker_%s_signaled: signal=%d" head_program n)
          | Unix.WSTOPPED n ->
            Error (Printf.sprintf "docker_%s_stopped: signal=%d" head_program n))))
;;
...
let run_command_in_container
  ...
  =
  match
    run_command_in_container_with_status
      ?turn_sandbox_runtime
      ~ok_exit_codes
      ~config
      ~meta
