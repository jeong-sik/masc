...
def run(*, repo, executable, base, api_port, token, out, observe, navigate):
    ...
    result = {'scope': 'real native TUI keystrokes/mouse reports; unchanged HTTP forwarding; real automation Firefox',
    ...
    try:
        ...
        geometry = image_input(mouse(geometry, (.24, .25), (.76, .25)), 'dragged')
        ...
        assert [row['input']['action'] for row in actions] == ['click_at','drag','scroll_at','scroll_at']
