...
let test_closed_case_and_whitespace () =
  check mode "CLOSED uppercase -> Closed" AR.Closed (AR.fail_mode_of_string "CLOSED");
  check mode "  Closed  trimmed -> Closed" AR.Closed (AR.fail_mode_of_string "  Closed  ");
  check mode "Reject mixed case -> Closed" AR.Closed (AR.fail_mode_of_string "Reject")
;;
...
let test_round_trip () =
  check
    ...
    "Open round-trip"
    AR.Open
    (AR.fail_mode_of_string (AR.fail_mode_to_string AR.Open));
  check
    mode
    "Closed round-trip"
    AR.Closed
    (AR.fail_mode_of_string (AR.fail_mode_to_string AR.Closed))
;;

let () =
  run
    "anti_rationalization fail_mode (#9794)"
    [ ( "default"
      , [ test_case
    ...
    ; ( "closed_synonyms"
      , [ test_case
            "closed/reject/fail_closed/deny all map to Closed"
            `Quick
