...
let write_raw_jsonl_rows dir rows =
  ...
  let content =
    List.init rows (fun i ->
      Printf.sprintf "{\"timestamp\": %.1f, \"event\": \"e%d\"}\n" (float_of_int i) i)
    |> String.concat ""
  in
...
let test_source_roundtrip () =
  List.iter
    (fun source ->
       ...
       Alcotest.(check bool) (Printf.sprintf "%s roundtrips" s) true (result = Some source))
...
let test_summary_empty () =
  ...
  | `Assoc fields ->
    let total =
      match List.assoc_opt "total_entries" fields with
      | Some (`Int n) -> n
      | _ -> -1
    in
...
let test_agent_tool_called_scope_promoted_for_filters () =
  ...
  let result =
    Telemetry_unified.read_unified_result
      ...
      ()
  in
  Alcotest.(check int) "scoped agent tool event visible" 1 result.total_matching_entries;
...
let test_shadow_agent_tool_called_deduped_from_unified_view () =
  ...
  let result =
    Telemetry_unified.read_unified_result
      ...
      ()
  in
  Alcotest.(check int) "shadow agent event removed" 1 (List.length result.entries);
  Alcotest.(check int)
    "total reflects visible unified entries"
    1
    result.total_matching_entries;
  Alcotest.(check string)
    "full tool call row preserved"
    "tool_call_io"
  ...
  in
  Alcotest.(check int)
    "source-filtered agent event remains available"
    1
...
let test_keeper_metrics_per_keeper () =
  ...
  write_jsonl
    cheolsu_dir
    [ `Assoc
        [ "ts_unix", `Float 3000.0; "name", `String "cheolsu"; "channel", `String "turn" ]
    ];
  write_jsonl
    sangsu_dir
    [ `Assoc
        [ "ts_unix", `Float 4000.0; "name", `String "sangsu"; "channel", `String "turn" ]
    ];
  (* All keepers *)
  let all =
    Telemetry_unified.read_unified
...
let test_keeper_metrics_fast_path_preserves_noisy_keeper_top_n () =
  ...
  write_jsonl
    old_dir
    (List.init 50 (fun i ->
       `Assoc [ "ts_unix", `Float (Float.of_int i); "name", `String "old"; "i", `Int i ]));
  let result =
    Telemetry_unified.read_unified_result
      ~base_path:dir
      ~masc_root:(masc_root dir)
...
let test_keeper_metrics_fast_path_sets_truncated_with_sentinel () =
  ...
  let write_keeper name ts =
    ...
    write_jsonl
      metrics_dir
      [ `Assoc [ "ts_unix", `Float ts; "name", `String name; "channel", `String "turn" ] ]
  in
...
let test_time_window_reports_total_before_limit () =
  ...
  let result =
    Telemetry_unified.read_unified_result
      ...
      ~masc_root:(masc_root dir)
      ~sources:[ Telemetry_unified.Agent_event ]
      ~since_ts:(now -. 3_600.0)
      ~until_ts:now
      ~n:2
      ()
  in
  Alcotest.(check int)
    "total matching preserved before limit"
    3
    result.total_matching_entries;
  ...
  in
  Alcotest.(check (list string))
    "newest matching entries returned"
    [ "within_3"; "within_2" ]
    events
;;
...
let test_time_window_reads_matching_day_files () =
  ...
  in
  Alcotest.(check (list string))
    "range spans multiple day files"
    [ "today"; "yesterday" ]
    events
;;
...
let test_summary_includes_trajectory_and_execution_receipt_sources () =
  ...
  Fs_compat.append_file
    ...
    (Yojson.Safe.to_string
       (`Assoc
           ...
           ; "error", `Null
           ; "cost_usd", `Float 0.0
           ; "runtime_contract", `Assoc [ "keeper_name", `String "alice" ]
           ; "action_radius", `Assoc [ "tool_name", `String "keeper_bash" ]
           ])
     ^ "\n");
  ...
  Alcotest.(check int)
    "execution receipt keeper count"
    1
    (json_int_field "keeper_count" receipt_summary);
  Alcotest.(check string)
    "execution receipt health"
    "ok"
    (json_string_field "health" receipt_summary);
  Alcotest.(check string)
    "execution receipt dashboard surface"
...
let test_read_unified_reads_trajectory_and_execution_receipts () =
  ...
  let entries =
    Telemetry_unified.read_unified
      ...
      ~masc_root:root
      ~sources:
        [ Telemetry_unified.Trajectory_tool_call; Telemetry_unified.Execution_receipt ]
      ~keeper_name:"alice"
      ~n:10
      ()
  in
  if List.length entries <> 2
  then
    Alcotest.failf
      "expected two entries, got sources: %s"
      (entries |> List.map (json_string_field "source") |> String.concat ",");
  Alcotest.(check string)
    "newest source"
...
let test_scope_filter_matches_runtime_contract_fields () =
  ...
  in
  Alcotest.(check int)
    "runtime contract scoped row visible"
    1
    result.total_matching_entries;
  ...
  | [ entry ] ->
    ...
    Alcotest.(check string) "tool" "keeper_bash" (json_string_field "tool_name" entry)
...
let test_goal_event_source_and_summary () =
  ...
  Alcotest.(check int) "goal event count" 2 (json_int_field "entry_count" goal_summary);
  Alcotest.(check string)
    "goal event health"
    "ok"
    (json_string_field "health" goal_summary);
  Alcotest.(check string)
    "goal dashboard surface"
...
let test_summary_surfaces_coverage_gaps () =
  ...
  in
  let before =
    Prometheus.metric_value_or_zero Prometheus.metric_telemetry_coverage_gap ~labels ()
  in
  ...
  Alcotest.(check (float 0.001))
    "coverage gap Prometheus counter increments"
    (before +. 1.0)
    (Prometheus.metric_value_or_zero Prometheus.metric_telemetry_coverage_gap ~labels ());
  let json = Telemetry_unified.summary_json ~base_path:dir ~masc_root:root () in
  match json with
  | `Assoc fields ->
    ...
    let source_summary =
      ...
      | Some (`List sources) ->
        List.find
          (fun source_json ->
             String.equal "agent_event" (json_string_field "source" source_json))
          sources
      | _ -> Alcotest.fail "expected sources"
    in
    Alcotest.(check string)
...
let test_cluster_aware_read () =
  ...
  in
  Alcotest.(check int)
    "default masc_root misses cluster data"
    0
...
let test_cluster_keeper_metrics () =
  ...
  write_jsonl
    keeper_dir
    [ `Assoc
        [ "ts_unix", `Float 6000.0; "name", `String "alpha"; "channel", `String "turn" ]
    ];
  let entries =
    Telemetry_unified.read_unified
      ~base_path:dir
  ...
  Alcotest.run
    ...
    ; ( "read"
      , [ Alcotest.test_case "empty base" `Quick test_empty_returns_empty
        ...
        ; Alcotest.test_case
            "oas events + scope filter"
            `Quick
            test_oas_event_source_and_scope_filter
        ; Alcotest.test_case
            "agent tool_called scope promotion"
            `Quick
            test_agent_tool_called_scope_promoted_for_filters
        ; Alcotest.test_case
            "dedupe shadow agent tool_called"
            `Quick
        ...
        ; Alcotest.test_case
            "keeper metrics fast path keeps noisy top n"
            `Quick
            test_keeper_metrics_fast_path_preserves_noisy_keeper_top_n
        ; Alcotest.test_case
            "keeper metrics fast path sentinel"
            `Quick
    ...
    ; ( "summary"
      , [ Alcotest.test_case "empty" `Quick test_summary_empty
        ...
        ; Alcotest.test_case
            "tool_metric surface points to raw metrics"
            `Quick
            test_summary_tool_metric_surface_points_to_raw_metrics
        ; Alcotest.test_case
            "includes trajectory and execution receipt sources"
            `Quick
        ...
        ; Alcotest.test_case
            "counts all rows beyond recent cap"
            `Quick
