...
let prompt_defs =
  [ { name = "tool_help"
    ...
    ; arguments =
        ...
        ; { name = "focus"
          ; description = "Optional question or emphasis"
          ; required = false
          }
        ]
    }
  ; { name = "execution_session_proof"
    ...
    ; description =
        "Summarize auditable collaboration evidence for an execution session \
         (deprecated: team session layer removed)."
    ...
    ; arguments =
        ...
        ; { name = "operation_id"
          ; description = "Optional managed operation identifier"
          ; required = false
          }
        ]
    }
  ; { name = "command_truth"
    ; title = "Command Truth"
    ; description =
        "Summarize managed command-plane evidence for an operation or run (deprecated: \
         command plane layer removed)."
    ; icons = [ Mcp_server.themed_icon ~label:"CT" ~bg:"#9A3412" ~fg:"#FFF7ED" ]
    ; arguments =
        [ { name = "operation_id"
          ; description = "Managed operation or trace identifier"
          ; required = false
          }
        ; { name = "run_id"
          ; description = "Optional swarm-live run identifier"
          ; required = false
          }
        ]
    }
...
let list_json () = `Assoc [ "prompts", `List (List.map prompt_json prompt_defs) ]
...
let rec json_contains_run_tokens tokens = function
  ...
  | `Assoc fields ->
    List.exists (fun (_, value) -> json_contains_run_tokens tokens value) fields
...
let filter_traces_json_by_run_id run_id = function
  | `Assoc fields as json ->
    (match List.assoc_opt "events" fields with
     | Some (`List events) ->
       let filtered =
         events |> List.filter (json_contains_run_tokens (run_tokens run_id))
       in
    ...
    Ok
      (String.concat
         ...
         ([ "Explain this MCP tool using only the grounded fields below."
          ...
          @ [ ""; "Details:"; entry.details_markdown ]
          @
          if entry.doc_refs = []
          then []
