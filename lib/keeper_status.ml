    ...
    | Ok (Some m) ->
      ...
      (match model_specs_of_strings models with
       ...
       | Ok specs ->
         ...
         let models_resolved = `List (List.map (fun (s : Llm.model_spec) ->
           `Assoc [
             ("provider", `String (Llm.string_of_provider s.provider));
        ...
        let all_internal_tools =
          keeper_llm_tools |> List.map (fun tool -> tool.Llm.tool_name)
        in
