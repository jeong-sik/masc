...
let compact_session_json session_json =
  ...
  let last_event =
    ...
    | latest :: _ ->
      ...
      `Assoc
        ...
        ; ( "task_title"
          , string_json ~default:"not_recorded" (member_assoc "task_title" detail) )
        ; ( "result"
          , string_json
              ~default:"not_recorded"
              ~max_len:160
              (member_assoc "result" detail) )
        ; ( "reason"
          , string_json
              ~default:"not_recorded"
              ~max_len:160
              (member_assoc "reason" detail) )
        ]
    | [] ->
      `Assoc
  ...
  `Assoc
    [ ( "session_id"
      , string_json ~default:"unknown-session" (member_assoc "session_id" session_json) )
    ...
    ; ( "communication_summary"
      , `String
          (Printf.sprintf
             ...
             communication_mode_text
             broadcast_count_value
             portal_count_value) )
...
let compact_keeper_json keeper_json =
  ...
  `Assoc
    ...
    ; ( "current_task"
      , string_json ~default:"unassigned" ~max_len:160 (member_assoc "current_task" agent)
      )
    ; ( "last_reply_status"
      , string_json ~default:"not_recorded" (member_assoc "last_reply_status" diagnostic)
      )
    ; ( "last_reply_preview"
      , string_json
          ~default:"not_recorded"
          ~max_len:160
          (member_assoc "last_reply_preview" diagnostic) )
    ; "active_goal_ids", string_list_json (member_assoc "active_goal_ids" keeper_json)
    ; ( "skill_primary"
      , string_json
          ~default:"unknown"
          ~max_len:120
          (member_assoc "skill_primary" keeper_json) )
    ]
;;
...
let compact_agent_json (agent : Types.agent) =
  ...
  `Assoc
    ...
    ; ( "assignment_status"
      , `String (if current_focus = "unassigned" then "unassigned" else "assigned") )
    ; "current_focus", `String current_focus
    ; "goal_hint", `String current_focus
    ; "joined_at", `String agent.joined_at
    ; "last_seen", `String agent.last_seen
    ; ( "capabilities"
      , `List (List.map (fun item -> `String item) (take 2 agent.capabilities)) )
    ]
;;
