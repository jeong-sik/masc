...
masc skills-refresh browser-lanes --base-path BASE --apply --expected-revision REVIEWED_SHA256
```

The expected revision comes from the first command and includes all resources.
If the installed package has changed since inspection, replacement is rejected.
...
the new package preserved on subsequent automatic updates.
