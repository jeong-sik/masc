...
**Capsule**
에이전트 컨텍스트의 압축된 표현. 현재 목표, 진행 상황, 완료/대기 단계, 핵심 결정과 근거, 수정된 파일 목록 등을 포함한다. Handoff 시 후임 에이전트에게 전달된다. (구 명칭: DNA) `-> lib/tool_relay.mli`, `-> lib/tool_handover.mli`
...
## Historical Mitosis Terms

**Mitosis**
이전 런타임이 사용하던 세포 분열 비유의 handoff 용어. 현재 active runtime에서는 keeper checkpoint rollover와 relay/handover surface가 이를 대체한다.

**Cell State**
이전 mitosis 상태 머신: `Stem` -> `Active` -> `Prepared` -> `Dividing` -> `Apoptotic`. 현재 active runtime의 공식 상태는 keeper surface status와 generation/trace continuity다.

**Mitosis Phase**
이전 2-phase handoff 용어. 현재는 relay checkpoint와 keeper rollover 메트릭으로 continuity를 추적한다.

**Apoptosis**
성공적인 mitosis 후 종료를 뜻하던 historical term. 현재 active runtime에서는 별도 apoptosis 상태를 사용하지 않는다.

**Cell**
이전 mitosis 런타임의 에이전트 단위. 현재 active runtime은 keeper meta, trace_id, generation, checkpoint를 기준으로 continuity를 관리한다.
...
| mitosis | Handoff | "세포 분열" 생물학 비유. active runtime에서는 제거됨 |
