...
let dispatch name args = Tool_board.handle_tool name args
let make_args pairs = `Assoc pairs

let parse_create_response_json body =
  match String.index_opt body '\n' with
  | Some idx ->
    Yojson.Safe.from_string (String.sub body (idx + 1) (String.length body - idx - 1))
...
let test_sort_order_of_string () =
  ...
  Alcotest.(check bool)
    "garbage rejected"
    true
    (match Tool_board.parse_sort_order "xyz" with
     | Error _ -> true
     | Ok _ -> false)
;;
...
let test_is_agent () =
  ...
  Alcotest.(check bool) "no hook = not agent" false (Tool_board.is_agent "dreamer");
  ...
  Fun.protect ~finally:Tool_board.set_agent_lookup_none (fun () ->
    Alcotest.(check bool) "registered agent" true (Tool_board.is_agent "dreamer");
    Alcotest.(check bool) "unregistered agent" false (Tool_board.is_agent "unknown");
    Alcotest.(check bool) "empty = not agent" false (Tool_board.is_agent ""))
;;
...
let test_board_actor_identity_canonicalizes_keeper_alias () =
  ...
  Alcotest.(check string) "raw" "keeper-analyst-agent" (json_member_string json "raw");
  Alcotest.(check string)
    "source"
    "keeper_alias_contract"
    (json_member_string json "source");
  Alcotest.(check string)
    "runtime agent"
    "keeper-analyst-agent"
...
let test_inline_board_post_author_accepts_matching_alias () =
  let args =
    make_args
      [ "content", `String "ctx-owned post"; "author", `String "keeper-analyst-agent" ]
  in
  let normalized =
    Tool_inline_dispatch_extra.ensure_board_post_author
...
let test_get_int () =
  ...
  Alcotest.(check int) "get existing" 42 (Tool_args.get_int args "n" 0);
  Alcotest.(check int) "get missing" 0 (Tool_args.get_int args "missing" 0)
;;
...
let test_get_bool () =
  ...
  Alcotest.(check bool) "get existing" true (Tool_args.get_bool args "flag" false);
  Alcotest.(check bool) "get missing" false (Tool_args.get_bool args "missing" false)
;;
...
let test_post_create_success () =
  ...
  let ok, body =
    dispatch
      "masc_board_post"
      (make_args [ "content", `String "Hello board"; "author", `String "tester" ])
  in
...
let test_post_create_structured_payload () =
  ...
  Alcotest.(check bool)
    "state_block absent after strip"
    true
...
let test_post_create_judgment_roundtrip () =
  ...
  cleanup ();
  let summary =
    "LLM judged this as a direct post because it is a user-authored explanation rather \
     than automation output."
  in
  let ok, body =
    dispatch
      "masc_board_post"
      (make_args
         ...
         ; "judgment", `Assoc [ "summary", `String summary; "confidence", `Float 0.77 ]
         ])
  in
  Alcotest.(check bool) "create ok" true ok;
  let json = parse_create_response_json body in
  Alcotest.(check string)
    "classification reason from judgment"
    summary
    Yojson.Safe.Util.(json |> member "classification_reason" |> to_string);
  Alcotest.(check string)
    "judgment summary kept in meta"
    summary
    Yojson.Safe.Util.(
      json |> member "meta" |> member "judgment" |> member "summary" |> to_string)
;;
...
let test_keeper_board_post_preserves_meta_reason () =
    ...
    Keeper_exec_board.handle_keeper_board_tool
      ...
      ~args:
        (make_args
           ...
           ; ( "meta"
             , `Assoc
                 [ "classification_reason", `String reason; "trace", `String "probe-1" ] )
           ])
  in
  let json = parse_create_response_json body in
  Alcotest.(check string)
    "classification reason kept"
    reason
    Yojson.Safe.Util.(json |> member "classification_reason" |> to_string);
  Alcotest.(check string)
    "keeper source injected"
    "keeper_board_post"
    Yojson.Safe.Util.(json |> member "meta" |> member "source" |> to_string);
  Alcotest.(check string)
    "existing meta preserved"
  ...
  Alcotest.(check string)
    "author forced from keeper meta"
    "judge-keeper"
...
let test_keeper_board_dispatch_uses_typed_tool_names () =
  ...
  in
  Alcotest.(check bool)
    "typed comment vote reaches board handler"
    true
    (contains_substring comment_vote "comment_id required");
  Alcotest.(check bool)
    "typed comment vote is not unknown"
    false
...
let test_post_create_empty_content () =
  ...
  let ok, body =
    dispatch
      "masc_board_post"
      (make_args [ "content", `String ""; "author", `String "tester" ])
  in
  ...
  if not ok
  then Alcotest.(check bool) "error mentions reason" true (String.length body > 0)
;;
...
let test_post_create_missing_author_rejected () =
  ...
  cleanup ();
  let ok, body =
    dispatch "masc_board_post" (make_args [ "content", `String "Hello board" ])
  in
  Alcotest.(check bool) "missing author rejected" false ok;
  Alcotest.(check bool) "error mentions author" true (contains_substring body "author")
;;
...
let test_post_create_anonymous_author_rejected () =
  ...
  let ok, body =
    dispatch
      "masc_board_post"
      (make_args [ "content", `String "Hello board"; "author", `String "anonymous" ])
  in
...
let test_post_list_empty () =
  ...
  Alcotest.(check bool) "no posts msg" true (String.length body > 0)
;;
...
let test_post_list_with_posts () =
  ...
  let ok1, _ =
    dispatch
      "masc_board_post"
      (make_args [ "content", `String "Post 1"; "author", `String "a" ])
  in
  Alcotest.(check bool) "create 1" true ok1;
  let ok2, _ =
    dispatch
      "masc_board_post"
      (make_args [ "content", `String "Post 2"; "author", `String "b" ])
  in
  Alcotest.(check bool) "create 2" true ok2;
  let ok, body = dispatch "masc_board_list" (make_args [ "limit", `Int 10 ]) in
  Alcotest.(check bool) "list ok" true ok;
  Alcotest.(check bool) "body has content" true (String.length body > 20)
;;
...
let test_post_list_limit_clamping () =
  ...
  let ok, body = dispatch "masc_board_list" (make_args [ "limit", `Int 1 ]) in
  Alcotest.(check bool) "list ok" true ok;
...
let test_post_list_sort_orders () =
  ...
  List.iter
    (fun s ->
       let ok, body = dispatch "masc_board_list" (make_args [ "sort_by", `String s ]) in
       Alcotest.(check bool) (Printf.sprintf "sort %s ok" s) true ok;
       Alcotest.(check bool)
         (Printf.sprintf "sort %s has content" s)
         true
...
let test_post_list_invalid_sort_rejected () =
  ...
  ignore
    (dispatch
       ...
       (make_args [ "content", `String "sort test"; "author", `String "a" ]));
  let ok, body =
    dispatch "masc_board_list" (make_args [ "sort", `String "invalid_xyz" ])
  in
  Alcotest.(check bool) "invalid sort rejected" false ok;
  Alcotest.(check bool)
    "error mentions valid sorts"
    true
    (contains_substring
       body
...
let test_post_list_filter_combinations () =
  ...
  ignore
    (Board_dispatch.create_post
       ...
       ());
  let ok1, body1 =
    dispatch "masc_board_list" (make_args [ "exclude_system", `Bool true ])
  in
  let ok2, body2 =
    dispatch "masc_board_list" (make_args [ "exclude_automation", `Bool true ])
  in
  let ok3, body3 =
    dispatch
      "masc_board_list"
      (make_args [ "exclude_system", `Bool true; "exclude_automation", `Bool true ])
  in
...
let test_dispatch_delete_success () =
  ...
  let _ok, body =
    dispatch
      "masc_board_post"
      (make_args [ "content", `String "to be deleted"; "author", `String "tester" ])
  in
  let post_id =
    match String.index_opt body '\n' with
    | Some idx ->
      ...
      (try
       ...
       with
       | _ -> Alcotest.fail ("Failed to parse post JSON from: " ^ json_str))
    | None -> Alcotest.fail ("No newline in create response: " ^ body)
  in
  let ok_del, msg_del =
    dispatch "masc_board_delete" (make_args [ "post_id", `String post_id ])
  in
  Alcotest.(check bool) "delete ok" true ok_del;
  Alcotest.(check bool) "delete msg contains id" true (contains_substring msg_del post_id)
;;
...
let test_dispatch_delete_not_found () =
  ...
  cleanup ();
  let ok, body =
    dispatch "masc_board_delete" (make_args [ "post_id", `String "nonexistent-id" ])
  in
...
let test_dispatch_delete_empty_id () =
  ...
  Fs_compat.set_fs (Eio.Stdenv.fs env);
  cleanup ();
  let ok, body = dispatch "masc_board_delete" (make_args [ "post_id", `String "" ]) in
...
let test_post_get_success () =
  ...
  let ok, body =
    dispatch
      "masc_board_post"
      (make_args [ "content", `String "Get me"; "author", `String "tester" ])
  in
  Alcotest.(check bool) "create ok" true ok;
  ...
  let ok2, body2 = dispatch "masc_board_get" (make_args [ "post_id", `String post_id ]) in
...
let test_post_get_not_found () =
  ...
  cleanup ();
  let ok, body =
    dispatch "masc_board_get" (make_args [ "post_id", `String "nonexistent-id" ])
  in
...
let test_vote_not_found () =
  ...
  let ok, body =
    dispatch
      "masc_board_vote"
      (make_args
         [ "post_id", `String "missing"; "voter", `String "v"; "direction", `String "up" ])
  in
...
let test_comment_add_missing_post () =
  ...
  let ok, body =
    dispatch
      "masc_board_comment"
      (make_args
         [ "post_id", `String "missing"; "content", `String "hi"; "author", `String "a" ])
  in
...
let test_comment_add_missing_author_rejected () =
  ...
  let ok, body =
    dispatch
      "masc_board_comment"
      (make_args [ "post_id", `String "missing"; "content", `String "hi" ])
  in
  Alcotest.(check bool) "missing author rejected" false ok;
  Alcotest.(check bool) "error mentions author" true (contains_substring body "author")
;;
...
let test_comment_vote_missing () =
  ...
  let ok, body =
    dispatch
      ...
      (make_args
         [ "comment_id", `String ""; "voter", `String "v"; "direction", `String "up" ])
  in
...
let test_search_empty_query () =
  ...
  Fs_compat.set_fs (Eio.Stdenv.fs env);
  cleanup ();
  let ok, body = dispatch "masc_board_search" (make_args [ "query", `String "" ]) in
...
let test_search_no_results () =
  ...
  cleanup ();
  let ok, body =
    dispatch "masc_board_search" (make_args [ "query", `String "nonexistent_xyz_123" ])
  in
...
let test_profile_empty_agent () =
  ...
  Fs_compat.set_fs (Eio.Stdenv.fs env);
  cleanup ();
  let ok, body = dispatch "masc_board_profile" (make_args [ "agent", `String "" ]) in
...
let test_profile_with_posts () =
  ...
  let ok, body =
    dispatch "masc_board_profile" (make_args [ "agent", `String "profiler" ])
  in
  ...
  Alcotest.(check bool)
    ...
    true
    (try
       ignore (Str.search_forward (Str.regexp_string "profiler") body 0);
       true
     with
     | Not_found -> false)
;;
...
let test_dispatch_unknown_tool () =
  ...
  Alcotest.(check bool)
    ...
    true
    (try
       ignore (Str.search_forward (Str.regexp_string "Unknown") body 0);
       true
     with
     | Not_found -> false)
;;
...
let test_tools_all_have_descriptions () =
  ...
  List.iter
    (fun (t : Types.tool_schema) ->
       Alcotest.(check bool)
         (Printf.sprintf "%s has description" t.name)
         true
  ...
  Alcotest.run
    ...
    [ ( "helpers"
      , [ Alcotest.test_case "visibility_of_string" `Quick test_visibility_of_string
        ...
        ; Alcotest.test_case
            "board actor identity canonicalizes keeper alias"
            `Quick
            test_board_actor_identity_canonicalizes_keeper_alias
        ; Alcotest.test_case
            "board actor identity keeps non-keeper agent"
            `Quick
            test_board_actor_identity_keeps_non_keeper_agent
        ; Alcotest.test_case
            "inline board post author rewrites caller claim"
            `Quick
            test_inline_board_post_author_rewrites_caller_claim
        ; Alcotest.test_case
            "inline board post author accepts matching alias"
            `Quick
            test_inline_board_post_author_accepts_matching_alias
        ] )
    ; ( "json_helpers"
    ...
    ; ( "post_crud"
      , [ Alcotest.test_case "create success" `Quick test_post_create_success
        ...
        ; Alcotest.test_case
            "accept automation reject system"
            `Quick
    ...
    ; "voting", [ Alcotest.test_case "vote not found" `Quick test_vote_not_found ]
    ; ( "comments"
      , [ Alcotest.test_case "comment missing post" `Quick test_comment_add_missing_post
        ; Alcotest.test_case
            "comment missing author rejected"
            `Quick
            test_comment_add_missing_author_rejected
        ; Alcotest.test_case
            "comment anonymous author rejected"
            `Quick
    ...
    ; ( "schemas"
      , [ Alcotest.test_case "tools count" `Quick test_tools_count
        ...
        ; Alcotest.test_case
            ...
            `Quick
            test_tools_all_have_descriptions
        ] )
    ; ( "post_kind_registry"
      , [ Alcotest.test_case "no hook: defaults to direct" `Quick (fun () ->
        ...
        ; Alcotest.test_case "with hook: non-agent stays direct" `Quick (fun () ->
            ...
            Alcotest.(check bool)
              ...
              (contains_substring msg {|"post_kind": "direct"|}))
        ; Alcotest.test_case
            "legacy human override normalizes to direct"
            `Quick
            (fun () ->
               Eio_main.run
               @@ fun env ->
               ...
               Alcotest.(check bool) "post created" true ok;
               Alcotest.(check bool)
                 "legacy override normalized to direct"
                 true
                 (contains_substring msg {|"post_kind": "direct"|}))
        ] )
    ; ( "board_list_cache"
      , [ Alcotest.test_case "cache hit returns same result" `Quick (fun () ->
        ...
        ; Alcotest.test_case "mutation invalidates cache" `Quick (fun () ->
            ...
            Alcotest.(check bool)
              "cache invalidated — different result"
              true
              (body1 <> body2))
        ; Alcotest.test_case
            "different args produce independent entries"
            `Quick
            (fun () ->
               Eio_main.run
               @@ fun env ->
               ...
               in
               let args_recent =
                 make_args [ "limit", `Int 10; "sort_by", `String "recent" ]
               in
               ...
               Alcotest.(check bool)
                 "recent result non-empty"
                 true
                 (String.length body_recent > 0);
               Alcotest.(check bool)
                 "hot result non-empty"
                 true
        ...
        ; Alcotest.test_case "comment invalidates cache" `Quick (fun () ->
            ...
            let _ok2, body2 = dispatch "masc_board_list" args in
            Alcotest.(check bool) "cache invalidated after comment" true (body1 <> body2))
        ; Alcotest.test_case "vote invalidates cache" `Quick (fun () ->
            ...
            in
            let _ok2, body2 = dispatch "masc_board_list" args in
            Alcotest.(check bool) "cache invalidated after vote" true (body1 <> body2))
        ] )
    ]
;;
