  ...
  let parts =
    List.filter_map
      ...
      ; (if observation.unclaimed_task_count > 0
         then Some (Printf.sprintf "unclaimed_tasks=%d" observation.unclaimed_task_count)
         else None)
      ; (if observation.claimable_task_count > 0
         then Some (Printf.sprintf "claimable_tasks=%d" observation.claimable_task_count)
         else None)
      ; (if observation.failed_task_count > 0
         then Some (Printf.sprintf "failed_tasks=%d" observation.failed_task_count)
         else None)
      ; (if observation.active_goals <> []
         then
           Some (Printf.sprintf "active_goals=%d" (List.length observation.active_goals))
         else None)
      ; (if observation.idle_seconds > 0
         then Some (Printf.sprintf "idle=%ds" observation.idle_seconds)
         else None)
      ; (if Option.is_some observation.worktree_change_summary
         then Some "worktree_delta"
         else None)
      ]
  in
  match parts with
...
let make_state
  ...
  in
  { social_model =
      Option.value ~default:(Types.normalize_social_model meta.social_model) social_model
  ; belief_summary =
      Option.value ~default:(belief_summary_of_observation observation) belief_summary
  ; active_desire = carry active_desire (fun state -> state.active_desire)
  ; current_intention = carry current_intention (fun state -> state.current_intention)
  ; blocker
  ; need = carry need (fun state -> state.need)
  }
;;

let protocol_violation_state
      ~(meta : keeper_meta)
      ~(observation : Keeper_world_observation.world_observation)
      ~(reason : string)
  =
...
let tools_include_keeper tool tools = List.exists (keeper_tool_name_matches tool) tools

let inferred_tool_surface tools =
  if tools = [ keeper_tool_name Tool_name.Keeper.Stay_silent ]
  then
    Some
      ( { speech_act = Types.Stay_silent; delivery_surface = Types.Silent }
      , Types.Tool_only_stay_silent )
  else if tools_include_keeper Tool_name.Keeper.Board_comment tools
  then
    Some
      ( { speech_act = Types.Comment_board; delivery_surface = Types.Board_comment }
      , Types.Tool_only_comment_board )
  else if tools_include_keeper Tool_name.Keeper.Board_post tools
  then
    Some
      ( { speech_act = Types.Post_board; delivery_surface = Types.Board_post }
      , Types.Tool_only_post_board )
  else if tools_include_keeper Tool_name.Keeper.Broadcast tools
  then
    Some
      ( { speech_act = Types.Broadcast; delivery_surface = Types.Broadcast_surface }
      , Types.Tool_only_broadcast )
  else if List.exists Keeper_tool_disclosure.is_claim_tool_name tools
  then
    Some
      ( { speech_act = Types.Claim_task; delivery_surface = Types.Task_claim_surface }
    ...
    (match
     ...
     | Some speech_act, Some delivery_surface ->
       ...
       ( make_state
           ...
           ~belief_summary
           ?active_desire:(Protocol.nonempty_header_opt headers "ACTIVE_DESIRE")
           ?current_intention:(Protocol.nonempty_header_opt headers "CURRENT_INTENTION")
     ...
     | _ ->
       let state, output =
         protocol_violation_state ~meta ~observation ~reason:"invalid social headers"
       in
       state, output, Types.Protocol_violation_invalid_social_headers)
  | _ ->
    let state, output =
      protocol_violation_state ~meta ~observation ~reason:"missing social headers"
    in
...
let request_help_post_body ~(meta : keeper_meta) ~(state : Types.social_state) blocker =
  String.concat
    "\n"
...
let deliver_request_help_post ~(meta : keeper_meta) ~(state : Types.social_state) =
    ...
    let meta_json =
      Some
        (`Assoc
            ...
            ; "speech_act", `String (Types.speech_act_to_string state.speech_act)
...
let transition (previous_state : state option) (input : input) =
  ...
  then tool_only_state ~meta ~observation ~previous_state ~result
  ...
  then (
    ...
    then (
      ...
      let fallback_reason =
        ...
        | _ -> Types.Inferred_visible_reply
      in
      state, output, fallback_reason)
    else state, output, transition_reason)
  else if input.has_text_reply
  then (
    let state, output = inferred_text_reply_state ~meta ~observation ?previous_state () in
    state, output, Types.Inferred_visible_reply)
  else (
    let state, output =
      protocol_violation_state
        ~meta
        ~observation
...
let apply_output_to_result
      ~(meta : keeper_meta)
      ~(result : Keeper_agent_run.run_result)
      ~(visible_response_body : string)
  ...
  | Defer, Silent -> { result with response_text = "" }, state
...
let apply_to_result
      ...
      (result : Keeper_agent_run.run_result)
  =
  let headers, response_body = Protocol.parse_header_block result.response_text in
...
let derive_failure_state
  ...
  in
  let state =
    if is_auto_recoverable && observation.claimable_task_count > 0
    then
      make_state
        ~meta
        ~observation
        ...
        ()
    else make_state ~meta ~observation ?previous_state ?blocker ()
  in
  let output = { speech_act = Types.Defer; delivery_surface = Types.Silent } in
