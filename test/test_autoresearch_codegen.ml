...
let test_prompt_requires_strict_json () =
  ...
  check bool "includes hypothesis key" true (contains prompt "\"hypothesis\"");
  check bool "includes modified_code key" true (contains prompt "\"modified_code\"")
;;
...
let test_parse_model_code_response_accepts_strict_json () =
  ...
  | Error err -> failf "expected strict JSON response to parse, got %s" err
;;
...
let test_parse_model_code_response_rejects_legacy_xml () =
  ...
  | Error err ->
    check
      bool
      "mentions lenient recovery failure"
      true
...
let test_parse_model_code_response_accepts_fenced_json () =
  ...
  | Error err -> failf "expected fenced JSON response to parse, got %s" err
;;
...
let test_parse_model_code_response_rejects_missing_fields () =
  ...
  | Error err ->
    check bool "mentions missing modified_code" true (contains err "\"modified_code\"")
;;
...
let () =
  run
    ...
    [ ( "prompt"
      , [ test_case
            "requires strict JSON object output"
            `Quick
            test_prompt_requires_strict_json
        ] )
    ; ( "parser"
      , [ test_case
            "accepts strict JSON object"
            `Quick
            test_parse_model_code_response_accepts_strict_json
        ; test_case
            "rejects legacy XML tags"
            `Quick
