(** Message Schema: Structured message validation for MASC
    ...
    - [@@deriving yojson] for automatic serialization
*)
...
(** Deserialize JSON to structured_message *)
let of_json json =
...
(** Validate a raw message string.

    Attempts to parse as structured JSON first.
    Falls back to Freeform based on validation_mode. *)
  ...
  | json ->
      (match of_json json with
       ...
       | Error reason ->
           ...
           | Warn ->
               (* Accept as freeform but the caller should log *)
               Ok (Freeform raw_message)
...
(** Extract the message type string for logging/routing *)
...
(** JSON Schema definition for MCP tool documentation *)
let json_schema =
  `Assoc [
    ("type", `String "object");
    ("oneOf", `List [
...
(** Roundtrip test: structured_message -> JSON -> structured_message *)
let roundtrip msg =
