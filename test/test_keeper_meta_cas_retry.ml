(** #9764/#9733/#9769: write_meta CAS retry semantics.
    ...
    Verifies that [Masc_mcp.Keeper_meta_store.write_meta_with_merge] with
...
let test_no_conflict_writes_first_attempt () =
  ...
  Fun.protect ~finally:(fun () -> cleanup_dir base_dir) (fun () ->
    ...
    let disk = match Masc_mcp.Keeper_meta_store.read_meta config "alpha" with
      | Ok (Some m) -> m
    ...
    with
    | Ok () ->
      let after = match Masc_mcp.Keeper_meta_store.read_meta config "alpha" with
        | Ok (Some m) -> m
...
let test_retry_succeeds_after_concurrent_bump () =
  ...
  Fun.protect ~finally:(fun () -> cleanup_dir base_dir) (fun () ->
    ...
    let caller_view = match Masc_mcp.Keeper_meta_store.read_meta config "beta" with
      | Ok (Some m) -> m
    ...
    (match Masc_mcp.Keeper_meta_store.write_meta config racing with
     | Ok () -> ()
    ...
    let after_retry_metric =
      Prometheus.metric_value_or_zero
        ...
        ()
    in
    let final = match Masc_mcp.Keeper_meta_store.read_meta config "beta" with
      | Ok (Some m) -> m
...
let test_is_version_conflict_error_classifies () =
  ...
  check bool "classifies version conflict" true
    (Masc_mcp.Keeper_meta_store.is_version_conflict_error conflict_msg);
  check bool "rejects unrelated error" false
    (Masc_mcp.Keeper_meta_store.is_version_conflict_error other_msg)
