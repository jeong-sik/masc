...
let of_process_status = function
  | Unix.WEXITED 0 ->
    { raw = Unix.WEXITED 0; code = 0; category = Success; label = "success"; hint = "" }
  | Unix.WEXITED 1 ->
    { raw = Unix.WEXITED 1
    ; code = 1
  ...
  | Unix.WEXITED n when n >= 128 ->
    ...
    let hnt =
      match cat with
      | Oom_killed -> "Process was killed (likely OOM). Reduce data size or add memory."
      | Segfault ->
        "Segmentation fault. This is a bug in the tool — try a simpler invocation."
      | Signal s ->
        Printf.sprintf
          "Process received %s. May have been killed externally."
          (signal_name s)
      | _ -> ""
    in
    { raw = Unix.WEXITED n; code = n; category = cat; label = lab; hint = hnt }
  | Unix.WEXITED n ->
    { raw = Unix.WEXITED n
    ; code = n
    ; category = Unknown n
  ...
  | Unix.WSTOPPED signum ->
    ...
    ; hint =
        Printf.sprintf "Process was stopped by %s (job control)." (signal_name signum)
    }
;;
...
let to_assoc t =
  let base = [ "exit_code", `Int t.code; "label", `String t.label ] in
  let with_hint = if t.hint = "" then base else ("hint", `String t.hint) :: base in
  with_hint
;;
