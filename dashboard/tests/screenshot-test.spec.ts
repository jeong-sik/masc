...
test('capture screenshots', async ({ page }) => {
  ...
  // Selector based on the class list or label we saw: <button ...> icon + label
  // Let's locate by text "예약" or selector `button:has-text("예약")`
  ...
  if (await scheduleBtnMock.count() > 0) {
    ...
    // If not found in sidebar, let's try direct hash or url
