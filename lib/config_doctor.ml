...
let local_base_config_root ~base_path =
  Filename.concat (Coord_utils.masc_dir_from_base_path ~base_path) "config"
;;

let runtime_data_root ~base_path = Coord_utils.masc_dir_from_base_path ~base_path

let repo_config_seed_path (inputs : inputs) =
  [ Config_dir_resolver.path_from_executable ~cwd:inputs.cwd inputs.executable_name
...
let diagnose_cascade_catalog ~active_config_root =
  ...
  in
  if not (Env_config_core.existing_file config_path)
  then []
  else (
    ...
    then []
      ...
      let error_count =
        issues |> List.filter (fun issue -> issue.severity = Catalog_error) |> List.length
      in
      ...
      ; message =
          Printf.sprintf
            "Cascade catalog check scanned %d preset(s): %d error, %d warn."
...
let cascade_catalog_next_actions ~config_path issues =
  ...
  then []
    ...
    let has_errors = List.exists (fun issue -> issue.severity = Catalog_error) issues in
    let primary_action =
      if has_errors
      then
        Printf.sprintf
          "Fix or disable the broken cascade preset entries in %s before assigning \
           keepers to them."
          config_path
      else
        Printf.sprintf
          "Clean up the degraded cascade preset metadata in %s so doctor and runtime see \
           the same routing intent."
          config_path
    in
    [ primary_action; "Rerun `masc-mcp doctor config` after editing cascade.json." ])
;;
...
let persona_tool_preset_conflict_to_warning conflict =
  Printf.sprintf
    "Keeper %s TOML tool_preset %S overrides persona %s tool_preset %S (toml=%s \
     persona=%s)."
...
let persona_tool_preset_conflict_to_action conflict =
  Printf.sprintf
    "Remove tool_preset from %s unless the override is intentional; TOML wins until \
     fixed."
    conflict.toml_path
;;
...
let discover_persona_tool_preset_conflicts ~active_config_root ~active_personas_root =
  let keepers_dir = Filename.concat active_config_root "keepers" in
  if not (Env_config_core.existing_dir keepers_dir)
  then []
  else
        ...
        (match Keeper_toml_loader.parse_toml content with
         ...
         | Ok doc ->
           let keeper_name =
             ...
             |> Option.value ~default:(Filename.chop_suffix filename ".toml")
           in
           let persona_name =
             Keeper_toml_loader.toml_string_opt doc "keeper.persona_name"
             |> non_empty_trimmed_opt
             |> Option.value ~default:keeper_name
           in
           let toml_tool_preset =
             Keeper_toml_loader.toml_string_opt doc "keeper.tool_preset"
            ...
            | Some toml_tool_preset ->
              let persona_path =
                Filename.concat
                  ...
                  "profile.json"
              in
              if not (Env_config_core.existing_file persona_path)
              then None
              else (
                match
                  Safe_ops.read_json_file_logged
                    ~label:"config_doctor_persona"
                    persona_path
                with
                | None -> None
                | Some json ->
                  ...
                  (match persona_tool_preset with
                   | Some persona_tool_preset when persona_tool_preset <> toml_tool_preset
                     ->
                     Some
                       { keeper_name
                       ; persona_name
  ...
  let resolution_source =
    ...
    | None ->
      ...
      let default_source =
        ...
        then "explicit_env"
        else if String.equal normalized_default normalized_base_path
        then (
          match Env_config_core.home_dir_opt () with
          | Some home ->
            let normalized_home = Env_config_core.normalize_masc_base_path_input home in
...
let analyze_with (inputs : inputs) =
  ...
  let shadowed =
    match explicit_config_dir with
    | Some path -> path <> local_base_config_root && local_base_config_initialized
    | None -> false
  in
  let init_state =
    if explicit_config_invalid || explicit_personas_invalid
    then Invalid_env
    else if not active_config_initialized
    then Missing_init
    else if shadowed
    then Shadowed
    else Initialized
  ...
  let path_diag =
    Server_base_path_diagnostics.detect
      ...
      ()
  in
  let cascade_catalog_issues = diagnose_cascade_catalog ~active_config_root in
  let persona_tool_preset_conflicts =
    discover_persona_tool_preset_conflicts ~active_config_root ~active_personas_root
  in
  let cascade_config_path =
    Filename.concat active_config_root Config_dir_resolver.cascade_json_filename
  in
  let warnings =
    [ (if explicit_config_invalid
       then
         Some
           (Printf.sprintf
    ...
    ; (if not active_config_initialized
       then
         Some
           (Printf.sprintf "Active config root is not initialized: %s" active_config_root)
       else None)
    ; (if shadowed
       then
         Some
    ...
    ; (if not (Env_config_core.existing_dir active_personas_root)
       then
         Some (Printf.sprintf "Active personas root is missing: %s" active_personas_root)
       else None)
    ; (match repo_config_seed_path with
       | Some path when path <> active_config_root ->
         Some
           (Printf.sprintf
              "Repo config seed exists at %s; it is bootstrap-only, not the active \
               config root."
              path)
       | _ -> None)
    ; (if inputs.repo_config_fallback_enabled
       then
         Some
           "MASC_ALLOW_REPO_CONFIG_FALLBACK=true is enabled; low-level resolver fallback \
            remains available."
       else None)
    ; path_diag.warning
    ]
    ...
    |> fun base_warnings ->
    base_warnings
    @ List.map persona_tool_preset_conflict_to_warning persona_tool_preset_conflicts
    @ List.map (fun issue -> issue.message) cascade_catalog_issues
    |> dedupe_keep_order
  in
  let cascade_actions =
    cascade_catalog_next_actions ~config_path:cascade_config_path cascade_catalog_issues
  in
  let next_actions =
    ...
    | Invalid_env ->
      ...
      ; Some
          (Printf.sprintf
             "For normal startup, keep the active config under %s or point \
              MASC_CONFIG_DIR at a valid config root."
             local_base_config_root)
      ]
    ...
    | Missing_init ->
      ...
      ; (match explicit_config_dir with
         | Some _ ->
           Some
             "If the explicit config root is not intentional, unset MASC_CONFIG_DIR and \
              use the base-path local config instead."
         | None ->
           Some
             "Supported launchers bootstrap <base-path>/.masc/config automatically; \
              direct binary use should create the same tree.")
      ; (match repo_config_seed_path with
         | Some path ->
           Some
             (Printf.sprintf
                "Do not edit %s expecting live changes; copy or bootstrap it into the \
                 active config root."
                path)
         | None -> None)
      ]
    ...
    | Initialized ->
      ...
      ; (match repo_config_seed_path with
         | Some path when path <> active_config_root ->
           Printf.sprintf "Treat %s as a bootstrap seed only." path
         | _ -> "No further action needed.")
      ]
      |> fun base_actions ->
      base_actions
      @ List.map persona_tool_preset_conflict_to_action persona_tool_preset_conflicts
      @ cascade_actions
      |> dedupe_keep_order
  in
  let has_catalog_errors =
    List.exists (fun issue -> issue.severity = Catalog_error) cascade_catalog_issues
  in
  let status =
    match init_state with
    ...
    | Shadowed -> Warn
    | Initialized ->
      if has_catalog_errors
      then Error
      else if persona_tool_preset_conflicts <> []
      then Warn
      else if warnings = []
      then Ok
      else Warn
...
let analyze ~base_path_input ~default_base_path () =
  current_inputs ~base_path_input ~default_base_path () |> analyze_with
;;
...
let live_catalog_summary = function
  | Stdlib.Ok (Cascade_catalog_runtime.Validated snapshot) ->
    ( Live_catalog_valid
    , None
    , Cascade_catalog_runtime.state_to_yojson (Cascade_catalog_runtime.Validated snapshot)
    )
  | Stdlib.Ok (Cascade_catalog_runtime.Validated_with_rejections _ as state) ->
    ( Live_catalog_partial
    , Some
        "Live cascade catalog validation kept the usable profile subset and rejected \
         some presets."
    , Cascade_catalog_runtime.state_to_yojson state )
  | Stdlib.Ok (Cascade_catalog_runtime.Serving_last_known_good _ as state) ->
    ( Live_catalog_serving_last_known_good
    , Some
        "Live cascade catalog validation rejected the current file; runtime is serving \
         last-known-good."
    , Cascade_catalog_runtime.state_to_yojson state )
  | Stdlib.Error rejection ->
    ( Live_catalog_invalid
    , Some "Live cascade catalog validation failed; no validated snapshot is available."
    , `Assoc
        [ "status", `String "invalid"
        ; "serving_last_known_good", `Bool false
        ; "snapshot", `Null
        ; "rejected_update", Cascade_catalog_runtime.rejection_to_yojson rejection
        ] )
;;

let analyze_live ~sw ~net ~clock ~fs ~proc_mgr ~base_path_input ~default_base_path () =
  let report = analyze ~base_path_input ~default_base_path () in
  Process_eio.init ~cwd_default:Eio.Path.(fs / report.base_path) ~proc_mgr ~clock;
  ...
  let live_outcome, live_warning, catalog_validation =
    match
      Cascade_catalog_runtime.inspect_active ~sw ~net ~clock () |> live_catalog_summary
    with
  ...
  let next_actions =
    let catalog_actions =
      ...
      then
        [ "Fix the active cascade catalog candidates/strategy and rerun `masc-mcp doctor \
           config`."
        ]
      else []
    in
    report.next_actions @ catalog_actions @ sandbox_actions |> dedupe_keep_order
  in
  let status =
    match report.init_state, report.status, live_outcome, sandbox_preflight_ok with
    | (Invalid_env | Missing_init), _, _, _ -> Error
    | _, _, (Live_catalog_serving_last_known_good | Live_catalog_invalid), _ -> Error
...
let to_yojson (report : t) =
  `Assoc
    ...
    ; ( "persona_tool_preset_conflicts"
      , `List
          (List.map
             persona_tool_preset_conflict_to_yojson
...
let render_text (report : t) =
  ...
  add_line (Printf.sprintf "init_state: %s" (init_state_to_string report.init_state));
  add_line (Printf.sprintf "base_path: %s" report.base_path);
  add_line (Printf.sprintf "runtime_data_root: %s" report.runtime_data_root);
  add_line
    (Printf.sprintf
       "active_config_root: %s (%s)"
       report.active_config_root
       report.config_root_source);
  add_line (Printf.sprintf "active_personas_root: %s" report.active_personas_root);
  add_line
    (Printf.sprintf
       "local_base_config_root: %s (initialized=%s)"
       report.local_base_config_root
       (if report.local_base_config_initialized then "yes" else "no"));
  add_line
    (Printf.sprintf
       "explicit_config_dir: %s"
       (Option.value ~default:"(unset)" report.explicit_config_dir));
  add_line
    (Printf.sprintf
       "explicit_personas_dir: %s"
       (Option.value ~default:"(unset)" report.explicit_personas_dir));
  add_line
    (Printf.sprintf
       "repo_config_seed_path: %s"
       (Option.value ~default:"(not found)" report.repo_config_seed_path));
  add_line
    (Printf.sprintf
       "repo_fallback_enabled: %s"
       (if report.repo_fallback_enabled then "yes" else "no"));
  add_line
    (Printf.sprintf
       "keeper_runtime.toml: %s"
       (if report.keeper_runtime_toml_present then "present" else "missing"));
  if report.persona_tool_preset_conflicts <> []
  then (
    add_line "";
    ...
    List.iter
      (fun conflict ->
         add_line
           (Printf.sprintf
              "- keeper=%s persona=%s toml=%s persona_profile=%s"
              conflict.keeper_name
              conflict.persona_name
              conflict.toml_path
              conflict.persona_path))
