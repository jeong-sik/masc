...
OAS는 `dune-project`에 `agent_sdk >= 0.93.0`으로 명시된 의존성이며, CI/runtime pin은 `scripts/oas-agent-sdk-pin.sh`의 `v0.93.0` / `67d7e0c43c5ed4382f91c3d457d7a70d104a359c`를 기준으로 관리한다. 최신성 검증이 필요할 때는 문서에 적힌 숫자보다 `dune-project`와 pin script를 우선 truth source로 본다.
