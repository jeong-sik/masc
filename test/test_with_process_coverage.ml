...
let test_happy_path () =
  let lines, status =
    WP.with_process_args_in "/bin/echo" [| "/bin/echo"; "hello" |] WP.drain_lines
  in
...
let test_sys_error_closes_pipe () =
  ...
  (try
     let _ : unit * Unix.process_status =
       WP.with_process_args_in "/bin/echo" [| "/bin/echo"; "fail" |] (fun _ic ->
         raise (Sys_error "synthetic"))
     in
     ()
   with
   | Sys_error msg when msg = "synthetic" -> raised := true);
...
let test_cancelled_closes_pipe_and_reraises () =
  ...
  (try
     let _ : unit * Unix.process_status =
       WP.with_process_args_in "/bin/echo" [| "/bin/echo"; "c" |] (fun _ic ->
         raise (Eio.Cancel.Cancelled (Failure "synthetic")))
     in
     ()
   with
...
let test_stress_no_leak () =
  ...
  for _ = 1 to 100 do
    try
      let _ : unit * Unix.process_status =
        WP.with_process_args_in "/bin/echo" [| "/bin/echo"; "x" |] (fun _ic ->
          raise (Sys_error "drop"))
      in
      ()
    with
    | Sys_error _ -> ()
  ...
  check
    bool
    (Printf.sprintf "100-iter fd delta bounded before=%d after=%d" before after)
    true
    (after <= before + 2)
;;
...
let test_drain_to_buffer () =
  let buf, _ =
    WP.with_process_args_in "/bin/echo" [| "/bin/echo"; "ab" |] WP.drain_to_buffer
  in
  ...
  run
    ...
    [ ( "happy_path"
      , [ test_case "echo returns stdout" `Quick test_happy_path
        ; test_case "drain_to_buffer fills buffer" `Quick test_drain_to_buffer
        ] )
    ; ( "error_path"
      , [ test_case "Sys_error in callback closes pipe" `Quick test_sys_error_closes_pipe
        ; test_case
            "Cancelled in callback closes and re-raises"
            `Quick
            test_cancelled_closes_pipe_and_reraises
        ] )
    ; ( "stress"
      , [ test_case
            "100 iterations with exceptions — fd bounded"
            `Quick
            test_stress_no_leak
        ] )
    ]
