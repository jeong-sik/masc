...
**Retain (Non-Goals)**: Upstream LLM provider classification (`apply_provider_filter`, `inference_model_bucket`) is a *different domain* — it classifies the LLM API endpoint (Provider-A, Provider-D, Provider-F, etc.) that MASC *calls*, not the MCP client that *calls MASC*. These belong to the runtime/provider layer and stay in this PR's Non-Goals.

**Wire-format adapters (Non-Goals)**: `runtime_transport_codex_omission_dedup.ml` and similar `*_adapter.ml` files encode protocol-level quirks of specific provider wire formats. RFC-0058 §3 lists these as non-goals; this RFC preserves that carve-out.
...
| `lib/runtime/runtime_config_provider_filter.ml` comment | 1 | Cleared (comment rewritten) |
| `lib/runtime/runtime_config.mli`, `runtime_config_loader.ml`, `runtime_config_parser.ml`, `runtime_observation.{ml,mli}` JSON/docstring examples | ~6 | Cleared |
| `lib/runtime/runtime_event_bridge_inference.ml` `inference_model_bucket` | substring classifier | Body collapsed to `"upstream"` (label cardinality = 1) |
| `lib/runtime/runtime_config_builder.ml` `binding.command = "agent-code"` dispatch | 1 | `provider_requires_argv_prompt_preflight` always returns `false` (legacy agent-code preflight path effectively disabled) |
...
| `lib/relay.ml`, `lib/coord/coord_query.ml`, `lib/coord/nickname.ml`, `lib/dashboard_runtime.mli`, `lib/provider_kind_resolver.mli`, `lib/metrics_store_eio.mli` docstring/comment | ~10 | Comments rewritten to placeholder syntax |
| `runtime_transport_codex_omission_dedup.ml` (wire adapter) | (module) | Kept; full removal would cut the cli-tool-a transport path entirely and is out of single-PR scope. Tracked in §9 |
| `lib/runtime/runtime_config_provider_filter.ml:38` `Some ("llama", ...)` Llama discovery dispatch | 1 | Kept; removing it disables Llama endpoint discovery as a feature. Tracked in §9 |
...
- `lib/runtime/runtime_event_bridge_inference.ml`: substring classifier collapsed to single `"upstream"` bucket.
- `lib/runtime/runtime_config_builder.ml`: `provider_requires_argv_prompt_preflight` always `false` (legacy agent-code preflight disabled; helper family retained for downstream wiring of runtime-decl `argv_prompt_preflight` capability flag).
...
- `runtime_transport_codex_omission_dedup.ml` (entire module + its facade in `runtime_transport.ml`): tracks per-agent cli-tool-a tool-omission fingerprints for WARN dedup. Its removal also removes the cli-tool-a transport path (`record_cli-tool-a_omission_for_agent` is called from the transport's bound-actor handling). Full removal needs a separate PR that decides whether cli-tool-a stays as a supported transport at all.
- `lib/runtime/runtime_config_provider_filter.ml:38` `| Some ("llama", model_id) -> ...`: Llama endpoint discovery (model-aware endpoint resolution + round-robin fallback). Removal disables Llama endpoint discovery as a feature.
...
- `rg '"agent-llm-a(_code)?"|"provider-f(_cli)?"|"agent-code(_cli)?"|"provider-c(_cli)?"|"provider-a"|"provider-k(_coding|-coding)?"|MASC_AGENT-LLM-A|MASC_PROVIDER-F|MASC_AGENT-CODE' lib/ bin/` returns only RFC-0166 closeout comments (`runtime_event_bridge_inference.ml:25`, `voice_runtime_overlay.ml:150`, `runtime_config_builder.ml:81`) — self-documenting historical citations.
- `dune build lib/ bin/` clean.
- The `runtime_transport_codex_omission_dedup` and Llama endpoint discovery features are still wired (see §9).
...
- **cli_prompt_preflight**: always `None` until the runtime-decl `argv_prompt_preflight` capability flag is wired into `Runtime_runner.config`. Legacy agent-code preflight is effectively disabled.
