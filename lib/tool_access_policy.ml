...
let dedupe_keep_order names =
  let rec loop seen acc = function
    ...
    | name :: rest -> loop (StringSet.add name seen) (name :: acc) rest
  in
...
let of_allowlist ?(deny = []) allow = { allow = Names allow; deny = Names deny }
...
let with_deny_names policy names = with_deny_selector policy (Names names)

let rec selector_matches_name selector name =
  match selector with
  | Empty -> false
  | All -> true
  | Names names -> List.mem name (normalize_names names)
  | Surface surface -> Tool_catalog.is_on_surface surface name
  | Union selectors -> List.exists (fun item -> selector_matches_name item name) selectors
  | Inter selectors ->
    List.for_all (fun item -> selector_matches_name item name) selectors
  | Diff { base; exclude } ->
    selector_matches_name base name && not (selector_matches_name exclude name)
;;

let allows_name policy name =
  selector_matches_name policy.allow name && not (selector_matches_name policy.deny name)
;;
...
let rec resolve_selector ?candidates selector =
  ...
  | Union selectors ->
    selectors |> List.concat_map (resolve_selector ?candidates) |> normalize_names
  | Inter selectors ->
    (match selectors with
     | [] ->
       normalize_names
...
let resolve ?candidates policy =
  ...
  allowed |> List.filter (fun name -> not (List.mem name denied)) |> normalize_names
;;
