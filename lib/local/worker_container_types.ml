...
let next_jsonrpc_id () = Atomic.fetch_and_add jsonrpc_id 1
...
let masc_http_base_url () = Env_config.masc_http_base_url ()
...
let post_json_via_eio
  ...
  | Some net ->
    (try
       ...
       | Ok (status, raw_body) ->
         if Cohttp.Code.is_success status
         then Ok raw_body
...
let call_jsonrpc
  ...
  let curl_fallback () =
    ...
    try
      ...
      | Unix.WEXITED code -> Error (sprintf "curl exited with code %d: %s" code raw_body)
      | Unix.WSIGNALED code -> Error (sprintf "curl signaled: %d" code)
      | Unix.WSTOPPED code -> Error (sprintf "curl stopped: %d" code)
    with
    | Eio.Cancel.Cancelled _ as e -> raise e
    | exn -> Error (Printexc.to_string exn)
  in
  ...
  let rec decode attempts_left =
    ...
    | Ok raw_body ->
      ...
      if String.trim normalized = "" && attempts_left > 0
      then decode (attempts_left - 1)
      else (
        try
  ...
  for idx = 0 to len - 1 do
    ...
    then (
      ...
      | '}' ->
        decr brace_depth;
        Buffer.add_char buf ch
      | c when c = delimiter && !paren_depth = 0 && !bracket_depth = 0 && !brace_depth = 0
        -> flush ()
    ...
    let rec build acc = function
      ...
      | part :: rest ->
        (match find_top_level_char '=' part with
         | None -> Error (sprintf "invalid tool arg assignment: %s" part)
         | Some idx ->
...
let parse_text_tool_calls (content : string) : Oas.Types.content_block list =
  ...
  let rec parse_args idx depth in_string escaped =
    ...
    then None
      ...
      then
        if escaped
        then parse_args (idx + 1) depth true false
        else (
          match ch with
        ...
        | ')' ->
          if depth = 1 then Some idx else parse_args (idx + 1) (depth - 1) false false
        | _ -> parse_args (idx + 1) depth false false))
  in
  let rec collect idx call_id acc =
    if idx >= len - prefix_len
    then List.rev acc
    else if String.sub content idx prefix_len <> prefix
    then collect (idx + 1) call_id acc
    else (
      ...
      if !name_end >= len || content.[!name_end] <> '('
      then collect !name_end call_id acc
      else (
        match parse_args (!name_end + 1) 1 false false with
        | None -> collect (!name_end + 1) call_id acc
        | Some close_idx ->
          let tool_name =
            String.sub content name_start (!name_end - name_start) |> strip_mcp_prefix
          in
          let args_raw = String.sub content (!name_end + 1) (close_idx - !name_end - 1) in
           ...
           | Ok args_json ->
             collect
               ...
               (Oas.Types.ToolUse
                ...
                :: acc)
           | Error msg ->
             Log.LocalWorker.warn "ignored text tool call (%s): %s" tool_name msg;
...
let estimate_cost_usd ~(model_id : string) (usage : Oas.Types.api_usage) : float option =
...
let join_worker ~sw ~(auth_token : string option) ~session_id ~worker_name =
  let args =
    `Assoc
      ...
      ; ( "capabilities"
        , `List [ `String "llama"; `String "mcp-worker"; `String "local-tool-loop" ] )
      ]
  in
...
let default_system_prompt ~worker_name ~model_id ?role ?selection_note () =
  let role_line =
    match role with
