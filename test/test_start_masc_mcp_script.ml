...
case "$cmd" in
  build)
    ...
    if [ "$count" = "0" ]; then
      ...
      echo "       make inconsistent assumptions over implementation Masc_agent_core__Streaming" >&2
      exit 1
    fi
      ...
      with_temp_dir "start-masc-stale-dune-fake-bin" (fun fake_bin ->
          ...
          check bool "original stale artifact error preserved" true
            (contains_substring stderr
               "make inconsistent assumptions over implementation Masc_agent_core__Streaming");
