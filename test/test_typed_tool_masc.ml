...
let parse json =
  ...
  |> Result.map_error
       (Agent_sdk.Tool_input_validation.format_errors ~tool_name:"masc_broadcast_typed")
;;

let test_parse_valid () =
  let json = `Assoc [ "message", `String "hello world" ] in
  match parse json with
  | Ok message -> Alcotest.(check string) "message" "hello world" message
...
let test_parse_coerces_int_message () =
  ...
  match parse json with
  | Ok message -> Alcotest.(check string) "message coerced" "42" message
...
let test_encode_mention () =
  let output : Tool_broadcast_typed.broadcast_output =
    { delivered = true; room_message = "hi"; mention = Some "alice" }
  in
...
let test_encode_no_mention () =
  let output : Tool_broadcast_typed.broadcast_output =
    { delivered = true; room_message = "hi"; mention = None }
  in
  let json = Tool_broadcast_typed.encode_broadcast output in
  let open Yojson.Safe.Util in
...
let test_e2e_parse_error () =
  let oas_tool = Typed_tool_masc.to_oas Tool_broadcast_typed.tool in
  match
    Agent_sdk.Typed_tool.execute oas_tool (`Assoc [ "message", `List [ `Int 1 ] ])
  with
  | Ok _ -> Alcotest.fail "expected error"
