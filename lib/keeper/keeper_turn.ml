...
let resolved_model_id_for_result
      ~(meta : keeper_meta)
...
let turn_cost_for_result ~(meta : keeper_meta) (result : Keeper_agent_run.run_result)
  : float
  =
  ...
  let usage_trust =
    Keeper_unified_metrics.classify_usage_trust
      ...
      ~context_max:0
  in
  if Keeper_unified_metrics.usage_trust_is_trusted usage_trust
  then
...
let update_direct_turn_meta
  ...
  let usage_trust =
    Keeper_unified_metrics.classify_usage_trust
      ...
      ~context_max:0
  in
  let usage_trusted = Keeper_unified_metrics.usage_trust_is_trusted usage_trust in
  ...
  ; runtime =
      { meta.runtime with
        usage =
          ...
          ; total_tokens = meta.runtime.usage.total_tokens + trusted_total_tokens
...
let handle_keeper_msg ?on_text_delta ctx args : tool_result =
  ...
  then false, "❌ invalid keeper name"
  else if message = ""
  then false, "❌ message is required"
  else (
    ...
    | Ok () ->
      (match reject_removed_keeper_input_keys ~tool_name:"masc_keeper_msg" args with
       ...
       | Ok () ->
         (match
            reject_removed_keeper_msg_input_keys ~tool_name:"masc_keeper_msg" args
          with
          | Error e -> false, "❌ " ^ e
          | Ok () ->
            (match ensure_keeper_exists ~ctx ~name with
             ...
             | Ok meta0 ->
               ...
               in
               let turn_tracker =
                 Progress.start_tracking ~task_id:turn_task_id ~total_steps:5 ()
               in
               Progress.Tracker.step
                 turn_tracker
                 ~message:"Preparing keeper turn configuration"
                 ();
               let meta = meta0 in
         ...
         slots, starving the synchronous run_turn call (Issue #2610). *)
                  ...
                  let effective_models =
                    if direct_reply
                    then Cascade_runtime.models_of_cascade_name turn_cascade_name
                   ...
                   | Ok () ->
                     ...
                     let max_cascade_context =
                       ...
                       (match resolution.requested_override with
                        | Some requested ->
                          Log.Keeper.debug
                            "%s: using max_context_override=%d context_budget=%d \
                             primary_budget=%d effective_budget=%d (manual turn)"
                            meta.name
                            requested
                     ...
                     let build_turn_prompt ~base_system_prompt ~messages
                       ...
                       let continuity_snapshot =
                         latest_state_snapshot_from_messages messages
                       in
                       let progress_cache =
                       ...
                       let continuity_text =
                         ...
                         let recovery_fallback =
                           ...
                           then []
                             ...
                             if
                               summary = ""
                               || summary = "No continuity snapshot available."
                             then []
                             else [ summary ])
                         in
                         let blocks =
                           (if recovery_sections = []
                            then recovery_fallback
                            else recovery_sections)
                           @
                       ...
                       let skill_route_text =
                         if effective_no_skill_route
                         then ""
                         else
                           skill_route_context_text ~fallback_route:fallback_skill_route
                       in
                       (* 3. Worktree changes *)
                       let worktree_text =
                       ...
                       let turn_instructions_text =
                         ...
                         | None -> ""
                         | Some ti -> "--- Turn-specific instructions ---\n" ^ ti
                       in
                       let soft_parts =
                         List.filter
                           (fun s -> String.trim s <> "")
                       ...
                       let prompt =
                         let policy_guards =
                           [ ( effective_no_skill_route
                             , "Output guard: NEVER output lines starting with SKILL: or \
                                SKILL_REASON:." )
                           ; ( effective_no_state_block
                             , "Output guard: NEVER output [STATE] or [/STATE] blocks in \
                                this turn." )
                           ]
                         in
                         let policy_lines =
                         ...
                         let tool_use_lines =
                           [ "Tool-use guidance:"
                           ; "- If the user asks you to speak, use voice, make sound, or \
                              output TTS, prefer keeper_voice_session_start and \
                              keeper_voice_speak."
                           ; "- Do not simulate spoken audio with plain text roleplay \
                              when a voice tool can handle the request."
                           ; "- If voice execution fails, say that voice output is \
                              unavailable and continue in text."
                           ]
                         in
                     ...
                     (match run_result with
                      | Error err ->
                        ...
                        (try
                           let _ =
                             Trajectory.finalize trajectory_acc (Trajectory.Failed e_str)
                           in
                           ()
                         with
                         ...
                         | exn ->
                           log_keeper_exn
                             ~label:"trajectory finalize (agent_run error)"
                             exn);
                      ...
                      | Ok result ->
                        ...
                        (try
                           let _ =
                             Trajectory.finalize trajectory_acc Trajectory.Completed
                           in
                           ()
                         with
                         | Eio.Cancel.Cancelled _ as e -> raise e
                         | exn ->
                           log_keeper_exn ~label:"trajectory finalize (agent_run ok)" exn);
                        let lifecycle =
                          Keeper_exec_context.apply_post_turn_lifecycle
                            ~base_dir
                 ...
                 owned fields are taken from disk. *)
                        (match
                         ...
                         | Error msg ->
                           ...
                           then
                             Log.Keeper.warn
                               "write_meta lost CAS race after retries (keeper_msg \
                                turn): %s"
                               msg
                           else
                             Log.Keeper.error
                      ...
                      ground truth. *)
                        ...
                        Progress.Tracker.complete
                          ...
                          ~message:
                            (Printf.sprintf
                               "Turn completed: %d tool calls"
                               result.tool_calls_made)
                          ();
                        let reply_json =
                          let surface_model_used =
