...
let websocket_discovery_json request =
  ...
  let ctx =
    Transport_read_model.make_http_context
      ~include_configured:true
      ~allow_legacy_accept:Server_routes_http_common.allow_legacy_accept
      ~host
...
let health_path_diagnostics () =
  ...
  | None ->
    ...
    let effective_masc_root =
      Common.masc_dir_from_base_path ~base_path:effective_base_path
    in
    Server_base_path_diagnostics.detect
      ?input_base_path:(Env_config_core.base_path_raw_opt ())
      ?env_masc_base_path:(Env_config_core.base_path_raw_opt ())
...
let make_health_json ?(listener = "http/1.1") request =
  ...
  let uptime_str =
    if uptime_secs < 60
    then Printf.sprintf "%ds" uptime_secs
    else if uptime_secs < 3600
    then Printf.sprintf "%dm %ds" (uptime_secs / 60) (uptime_secs mod 60)
  ...
  `Assoc
    ...
    ; ( "protocol"
      , `Assoc
          ...
          ; "supported", `List (List.map (fun v -> `String v) mcp_protocol_versions)
          ] )
    ...
    ; ( "feature_flags"
      , let features = Dashboard_feature_health.get_all_features () in
...
let health_handler request reqd =
  Http.Response.json (Yojson.Safe.to_string (make_health_json request)) reqd
;;
...
let liveness_handler _request reqd =
  ...
  let body = Yojson.Safe.to_string (`Assoc [ "live", `Bool true; "startup", startup ]) in
...
let board_post_detail_json ~response_format ~post_id =
  ...
  | Error _ -> `Not_found, {|{"error":"Post not found"}|}
  | Ok post ->
    ...
    let json =
      if String.equal (String.lowercase_ascii (String.trim response_format)) "flat"
      then (
        match post_json with
        | `Assoc fields -> `Assoc (fields @ [ "comments", comments_json ])
        | _ -> `Assoc [ "post", post_json; "comments", comments_json ])
      else `Assoc [ "post", post_json; "comments", comments_json ]
    in
...
let options_handler request reqd =
  ...
  let headers =
    Httpun.Headers.of_list (("content-length", "0") :: cors_preflight_headers origin)
  in
