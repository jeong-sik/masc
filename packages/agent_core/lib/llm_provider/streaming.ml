    ...
    | Ok evt_type ->
      (match evt_type with
       ...
       | "error" ->
         ...
         Some (SSEError { message; error_type; http_status = None; raw = data_str })
...
let openai_sse_parse_result_to_events state = function
  ...
  | Openai_empty -> [], None
  | Openai_provider_error { message; error_type; http_status; raw } ->
    [ SSEError { message; error_type; http_status; raw } ], None
