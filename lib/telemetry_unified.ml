...
let fixed_store_dir ~masc_root ~base_path = function
  ...
  | Keeper_metric | Trajectory_tool_call | Execution_receipt | Goal_event -> None
;;
...
let discover_keeper_metric_dirs masc_root : (string * string) list =
  ...
  then []
    ...
    let entries =
      Safe_ops.protect ~default:[] (fun () -> Array.to_list (Sys.readdir keepers_dir))
    in
    List.filter_map
      (fun name ->
         let metrics_dir = Filename.concat keepers_dir (name ^ "/metrics") in
         if Sys.file_exists metrics_dir then Some (name, metrics_dir) else None)
      entries)
;;
...
let discover_trajectory_keeper_dirs masc_root : (string * string) list =
  ...
  if not (is_directory trajectories_root)
  then []
  else
...
let discover_execution_receipt_dirs masc_root : (string * string) list =
  let keepers_dir = Filename.concat masc_root "keepers" in
  if not (is_directory keepers_dir)
  then []
  else
    Safe_ops.protect ~default:[] (fun () ->
      ...
      |> List.filter_map (fun name ->
        let dir =
          Filename.concat (Filename.concat keepers_dir name) "execution-receipts"
        in
        if is_directory dir then Some (name, dir) else None))
;;
...
let trajectory_tool_call_json = function
  | `Assoc fields ->
    (match List.assoc_opt "type" fields with
     ...
     | _ -> List.mem_assoc "tool_name" fields && List.mem_assoc "ts" fields)
  | _ -> false
;;
...
let latest_store_ts dir label : float option =
  ...
  then None
    ...
    | exception exn ->
      Log.Telemetry.warn
        "latest_store_ts: %s store open failed: %s"
        label
...
let take_first n entries = if n <= 0 then [] else List.filteri (fun i _ -> i < n) entries
...
let source_health_fields
  ...
  | Some gap ->
    ...
    ; ( "stale_reason"
      , `String (Safe_ops.json_string ~default:"coverage_gap" "stale_reason" gap) )
    ]
  | None ->
...
let latest_coverage_gap_for_source gaps source =
  ...
  |> List.find_opt (fun gap ->
    String.equal (Safe_ops.json_string ~default:"" "source" gap) source_name)
;;
...
let canonical_actor_name value =
  value |> String.trim |> drop_prefix "keeper-" |> drop_suffix "-agent"
;;
...
let tool_call_signature ?success ~actor ~tool ~ts () =
  ...
  if actor = "" || tool = "" || ts <= 0.0 then None else Some { actor; tool; success; ts }
;;
  ...
  | Some "tool_call_io" ->
    (match string_field "keeper" json, string_field "tool" json with
     | Some actor, Some tool ->
       tool_call_signature
         ?success:(bool_field "success" json)
         ~actor
         ~tool
         ~ts:(extract_ts json)
         ()
...
let agent_tool_called_signature json =
  ...
  | Some detail ->
    (match string_field "agent_id" detail, string_field "tool_name" detail with
     | Some actor, Some tool ->
       tool_call_signature
         ?success:(bool_field "success" detail)
         ~actor
         ~tool
         ~ts:(extract_ts json)
         ()
...
let suppress_shadow_agent_tool_events entries =
  let tool_call_io = List.filter_map tool_call_io_signature entries in
  if tool_call_io = []
  then entries
  else
    List.filter
...
let promote_detail_field_if_absent name detail_fields fields =
  ...
  then fields
    ...
    | Some (`String value) when String.trim value <> "" -> (name, `String value) :: fields
    | Some (`Bool _ as value)
    | Some (`Int _ as value)
    | Some (`Float _ as value)
    | Some (`Intlit _ as value) -> (name, value) :: fields
    | Some _ | None -> fields)
;;
...
let tag_entry source (json : Yojson.Safe.t) : Yojson.Safe.t =
  ...
  | other -> `Assoc [ "source", `String (source_to_string source); "data", other ]
;;
...
let matches_scope ?session_id ?operation_id ?worker_run_id (json : Yojson.Safe.t) : bool =
  let matches ~top_fields ~runtime_contract_fields = function
    | None -> true
    | Some expected ->
      (match json with
       | `Assoc fields ->
         List.exists (fun field -> matches_field fields field expected) top_fields
         ||
           (match List.assoc_opt "runtime_contract" fields with
           | Some (`Assoc runtime_fields) ->
             List.exists
...
let read_fixed_source dir source ~n ?since_ts ?until_ts () : Yojson.Safe.t list =
  ...
  then []
    ...
    | store ->
      ...
      in
      let entries = List.filter (within_requested_window ?since_ts ?until_ts) entries in
      List.map (tag_entry source) entries
    | exception (Eio.Cancel.Cancelled _ as e) -> raise e
    | exception exn ->
      Log.Telemetry.warn
        "read_fixed_source: %s store open failed: %s"
...
let read_keeper_metrics ~masc_root ?keeper_name ?since_ts ?until_ts ~n ()
  ...
  in
  List.concat_map
    (fun (_name, dir) -> read_fixed_source dir Keeper_metric ~n ?since_ts ?until_ts ())
    dirs
;;
      ...
      |> List.filter_map (fun line ->
        ...
        then None
          ...
          try
            ...
            if
              trajectory_tool_call_json json
      ...
      (fun (_name, dir) ->
         Safe_ops.protect ~default:[] (fun () ->
           ...
           |> List.concat_map (fun name ->
             read_trajectory_file (Filename.concat dir name) ?since_ts ?until_ts ())))
      dirs
  in
...
let goal_events_path ~masc_root = Filename.concat masc_root "goal_events.jsonl"
...
let read_unified_result
  ...
  let limited = n > 0 in
  let has_filter =
    Option.is_some keeper_name
    || Option.is_some session_id
    || Option.is_some operation_id
    || Option.is_some worker_run_id
    || Option.is_some since_ts
    || Option.is_some until_ts
  in
  let per_source =
    if not limited then 0 else if has_filter then max n (n * 2) else n + 1
  in
  let all_entries =
    List.concat_map
      (fun source ->
         match source with
         | Keeper_metric ->
           if limited && Option.is_none keeper_name && not has_filter
           then read_keeper_metrics_fast_top ~masc_root ~n ()
           else
             read_keeper_metrics
               ~masc_root
               ?keeper_name
         ...
         | Execution_receipt ->
           read_execution_receipts
             ...
             ~n:per_source
             ()
         | Goal_event -> read_goal_events ~masc_root ?since_ts ?until_ts ~n:per_source ()
         | _ ->
           (match fixed_store_dir ~masc_root ~base_path source with
            | Some dir ->
  ...
  in
  let filtered =
    if List.mem Agent_event sources && List.mem Tool_call_io sources
    then suppress_shadow_agent_tool_events filtered
    else filtered
  in
  ...
  let entries =
    if (not limited) || total_matching_entries <= n then sorted else take_first n sorted
  in
...
let count_fixed_source_entries ~masc_root ~base_path source : int =
  ...
  | Some dir ->
    ...
    then 0
      ...
      | exception exn ->
        Log.Telemetry.warn
          "count_source_entries: %s store open failed: %s"
          (source_to_string source)
          (Printexc.to_string exn);
        0)
;;
...
let goal_event_summary_stats ~masc_root =
  ...
  then 0, None
    ...
    let entries = Safe_ops.protect ~default:[] (fun () -> Fs_compat.load_jsonl path) in
...
let summary_json ~base_path ~masc_root () : Yojson.Safe.t =
  ...
  let keeper_total =
    List.fold_left
      (fun acc (name, dir) ->
         ...
         | exception exn ->
           Log.Telemetry.warn
             "summary_json: keeper %s store open failed: %s"
             name
             (Printexc.to_string exn);
           0)
      0
  ...
  let source_json_and_count source =
    ...
    let metadata_fields =
      ...
      ; "durable_store", `String (source_durable_store ~masc_root ~base_path source)
    ...
    let keeper_dir_fields dirs =
      [ ( "keepers"
        , `List
            (List.map
               (fun (name, dir) -> `Assoc [ "name", `String name; "path", `String dir ])
               dirs) )
    ...
    | Trajectory_tool_call ->
      ...
      let count, latest_ts =
        if exists then trajectory_tool_call_summary_stats ~masc_root else 0, None
      in
      ( `Assoc
          ([ "source", `String (source_to_string source)
    ...
    | Execution_receipt ->
      ...
      let count, latest_ts =
        if exists then execution_receipt_summary_stats ~masc_root else 0, None
      in
      ( `Assoc
          ([ "source", `String (source_to_string source)
    ...
    | Goal_event ->
      ...
      let count, latest_ts =
        if exists then goal_event_summary_stats ~masc_root else 0, None
      in
      ( `Assoc
          ([ "source", `String (source_to_string source)
           ; "path", `String path
    ...
    | _ ->
      ...
      let count =
        if exists then count_fixed_source_entries ~masc_root ~base_path source else 0
      in
      let latest_ts =
