...
이 문서는 **0.35.4 설치 계약**입니다. 태그와 자산 제공 여부는
[GitHub Releases](https://github.com/jeong-sik/masc/releases)에서 확인하세요.
아래 다운로드 명령은 `v0.35.4`과 같은 버전의 설치기를 선택합니다.
...
**0.35.4 바이너리**는 `activation_mode = "manual"`인 `imp` 하나와 `browser-lanes` skill을 설치합니다.
...
이 경로는 `masc setup`이 포함된 **0.35.4 설치 계약**입니다. 다운로드 전에
...
검증된 커밋에 `v0.35.4` 태그를 push하면 네 빌드와 자산 검증을 거쳐 GitHub Release와
