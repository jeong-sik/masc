...
let test_catalog_routed_names_return_none () =
  check
    (option (testable (fun fmt _ -> Format.fprintf fmt "<profile>") ( = )))
...
let test_unknown_falls_back_to_default () =
  ...
  check
    string
    "canonicalize forces fallback to big_three"
    "big_three"
...
let test_catalog_names_follow_live_config () =
  with_temp_config
    ...
    (fun path ->
       ...
       check
         (list string)
         "catalog_names follows cascade schema keys"
         [ "custom_live"
         ; "default"
         ; "governance_judge"
         ; "keeper_unified"
...
let test_resolve_live_with_catalog_requires_active_membership () =
  with_temp_config
    ...
    (fun path ->
       ...
       check
         string
         "active custom profile survives live resolution"
         "custom_live"
         (Profile.resolve_live_with_catalog ~catalog "custom_live");
       check
         string
         "keeper_unified catalog profile survives live resolution"
         "keeper_unified"
         (Profile.resolve_live_with_catalog ~catalog "keeper_unified");
       check
         string
         "tool_use_strict catalog profile survives live resolution"
         "tool_use_strict"
         (Profile.resolve_live_with_catalog ~catalog "tool_use_strict");
       check
         string
         "legacy alias resolves through active default"
         "big_three"
         (Profile.resolve_live_with_catalog ~catalog "oas-keeper_unified");
       check
         string
         "inactive built-in profile falls back to default"
         "big_three"
...
let test_catalog_read_failures_do_not_fallback_to_hardcoded_names () =
  ...
  check
    (list string)
    "catalog_names stays empty on read failure"
    []
    (Profile.catalog_names ~config_path:missing ());
  check
    (list string)
    "keeper catalog stays empty on read failure"
    []
    (Profile.keeper_catalog_names ~config_path:missing ());
  check
    (list string)
    "system catalog stays empty on read failure"
    []
  ...
  run
    ...
    [ ( "ssot"
      , [ test_case "active names round-trip" `Quick test_round_trip
        ; test_case
            "known_cascades covers active"
            `Quick
            test_known_cascades_covers_variants
        ; test_case
            "legacy aliases collapse"
            `Quick
            test_legacy_aliases_collapse_to_big_three
        ; test_case
            "catalog-routed names return None"
            `Quick
        ...
        ; test_case
            "catalog_names follow live config"
            `Quick
            test_catalog_names_follow_live_config
        ; test_case
            "resolve_live_with_catalog requires active membership"
            `Quick
            test_resolve_live_with_catalog_requires_active_membership
        ; test_case
            "catalog read failures stay empty"
            `Quick
