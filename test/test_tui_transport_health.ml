...
let test_a_listening_transport_reports_its_sessions_and_port () =
  ...
  (* #30240 closed both of these vocabularies. Comparing through the owner's
     [*_to_string] keeps the assertion readable while letting the compiler
     reject a word the type no longer publishes. *)
  check string "primary path" "websocket"
    (Masc.Transport_metrics.primary_path_kind_to_string health.Decode.th_primary_path);
  check string "queue pressure" "steady"
    (Masc.Transport_metrics.queue_pressure_kind_to_string health.Decode.th_queue_pressure);
...
let test_a_dropped_event_count_survives_the_decode () =
  ...
  check string "a loaded queue is reported as such" "high"
