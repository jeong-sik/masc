...
module Http_client = struct
    ...
    let h =
      String.lowercase_ascii
        (if String.length haystack > max_scan
         then String.sub haystack 0 max_scan
         else haystack)
    in
  ...
  let classify (err : Llm_provider.Http_client.http_error) : cascade_failure_class =
    if Llm_provider.Http_client.is_local_resource_exhaustion err
    then Local_resource_exhaustion
    else (
      match err with
      | Llm_provider.Http_client.HttpError { code; body }
        when List.mem code [ 400; 422 ]
             && Llm_provider.Retry.is_context_overflow_message body -> Context_overflow
      | Llm_provider.Http_client.HttpError { code; body }
        when List.mem code [ 400; 422 ] && is_provider_parse_error body ->
        Provider_parse_error
      | Llm_provider.Http_client.HttpError { code; _ } ->
        if List.mem code Llm_provider.Constants.Http.cascadable_codes
        then Transient_http code
        else Terminal_http code
      | Llm_provider.Http_client.AcceptRejected { reason } ->
        if accept_rejected_is_cascadable reason
        then Accept_rejected_capability_mismatch
        else Accept_rejected_terminal
      | Llm_provider.Http_client.CliTransportRequired _ -> Cli_transport_required
  ...
  let error_message (err : Llm_provider.Http_client.http_error) : string =
    ...
    | Llm_provider.Http_client.ProviderTerminal
      ...
      Printf.sprintf
        "provider terminal: max turns exceeded (%d/%d): %s"
        turns
        limit
        message
    | Llm_provider.Http_client.ProviderTerminal
