---
rfc: "0142"
title: "runtime_error_classify Decomposition + Typed JSON-Extraction Variant"
status: Active
created: 2026-05-20
updated: 2026-05-23
author: vincent
...
- **`runtime_error_classify.ml` migration deferred**: Phase 1 spec
...
`runtime_error_classify.ml` has been reduced from **939 LoC** at
...
`runtime_config_builder.ml` instead of a new sibling.

| Phase 2 PR | Target | Status |
|------------|--------|--------|
| PR-1 | `lib/runtime/runtime_internal_error.ml(.mli)` | **landed** (file present on `origin/main`) |
| PR-2 | `lib/runtime/runtime_error_from_sdk.ml(.mli)` | **landed** (PR #17474, merged 2026-05-22; appears in §frontmatter `implementation_prs`) |
| PR-3 | `lib/runtime/runtime_codex_preflight.ml(.mli)` | **cancelled** — the CLI-Tool-B prompt preflight content lives in `lib/runtime/runtime_config_builder.ml`, whose module-header comment reads *"Runtime config construction and CLI-Tool-B prompt preflight"*.  The functions reachable via `grep -n 'preflight' lib/runtime/runtime_config_builder.ml` cover the original PR-3 scope without creating a third sibling module. |
...
(`runtime_internal_error.ml`, `runtime_error_from_sdk.ml`, the
residual 111-line `runtime_error_classify.ml`).  Not started yet.
...
| PR-B-Openai_compat | Same for Chat Completions v1-compatible adapter (Provider-C, Provider-K, llamacpp-Chat Completions v1-API). | Adapter test covers 6 variants. |
...
RFC-0142's umbrella scope is "runtime/provider error classification
substring-and-catch-all elimination" (cf. RFC-0089 string-classifier
umbrella).  The originally-listed `runtime_error_classify.ml` was one
file under that umbrella; `keeper_health_probe.ml` is another.  Phase
4 closes the keeper-reaction-layer twin.

---

# RFC-0142 — runtime_error_classify Decomposition

## 1. Summary

`lib/runtime/runtime_error_classify.ml` is 873 LoC with 33 line-leading `| _ -> None | [] | 0.0 | …` catch-all arms. Most of these catch-alls are inside ad-hoc JSON-field extraction code. The file is both a godfile (RFC-0085 decomposition track) and a hotbed of sparse exhaustive matches (RFC-0088 §3.4 / §3.5 surface).
...
```
lib/runtime/runtime_error_classify.ml — 873 LoC
...
**Scope amendment (2026-05-20)**: the helper lives at `lib/json/json_field.ml(.mli)` — not `lib/runtime/` — because `lib/telemetry_unified.ml` carries the same JSON-extraction catch-all shape at 21 sites (`:53`, `:284-300`, `:448-525`, `:598-641`, `:899` measured 2026-05-20). Promoting the helper to `lib/json/` lets both call families share it. Both modules' migrations move under Phase 1.

The original `lib/runtime/`-scoped sketch below is preserved for archival purposes; the actual module path is `lib/json/`.
...
(** Typed extraction from Yojson.Safe.t with explicit shape-mismatch
    ...
    runtime_error_classify and similar JSON-shaped boundaries. *)
...
(** [to_option] + emit a [Log.Runtime.warn] on [Wrong_shape] so
    schema drift surfaces in operator logs without aborting the
    response classification path. *)
```

Phase 1 replaces ~20 catch-alls in `runtime_error_classify.ml` **plus the 21 catch-alls in `telemetry_unified.ml`** (audit 2026-05-20). The two helpers (`to_option`, `log_wrong_shape`) keep migration mechanical while preserving the option-call-site contract.

The telemetry_unified sites are technically *boundary-correct* today: a non-`String value gets `None` because the Yojson variant doesn't match, and the caller already accepts `None` semantics. They are migrated alongside runtime_error_classify so that *schema drift in upstream payloads becomes operator-visible* via `log_wrong_shape` rather than silently disappearing. No data-loss bug is being fixed in telemetry_unified — the migration is purely diagnostic.
...
- `runtime_internal_error.ml(.mli)` — variant + JSON codec (Concern A)
- `runtime_error_from_sdk.ml(.mli)` — SDK error mapping (Concern B)
- `runtime_codex_preflight.ml(.mli)` — CLI prompt preflight (Concern C)

Each ≤ 350 LoC. `runtime_error_classify.ml` becomes a thin re-exporting facade for backwards compatibility, then deleted once callers migrate.
...
- Phase 1: behavioral no-op on the happy path. `to_option` reproduces today's `Some|None` contract; `log_wrong_shape` adds a `Log.Runtime.warn` line when the response schema drifts. No on-wire change.
...
| Phase 2 | Existing `test/test_runtime_error_classify*.ml` suites must continue to pass against the new facade. Add a "no-direct-import" lint check ensuring downstream callers go through the facade until Phase 2 closeout. |
...
- MEMORY `project_runtime_misroute_2026_05_17` — sibling silent-drop incident at config-parse boundary, root cause closed by RFC-0058 strict parsing (PR #16739 in flight at audit time).
