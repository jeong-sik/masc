...
describe('buildKeeperPromptAssemblyReport', () => {
  ...
  it('keeps source-row details out of the default recipe view', () => {
    ...
    expect(container.textContent).toContain('Turn prompt recipe')
    ...
    expect(intro?.textContent).toContain('ordered model request')
    expect(intro?.textContent).toContain('Build details')
