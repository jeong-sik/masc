...
let test_state_to_json_has_fields () =
  let state : Tempo.tempo_state =
    { current_interval_s = 300.0; last_adjusted = 1704067200.0; reason = "test reason" }
  in
...
let test_state_to_json_interval_value () =
  let state : Tempo.tempo_state =
    { current_interval_s = 60.0; last_adjusted = 0.0; reason = "fast" }
  in
  let json = Tempo.state_to_json state in
  match json with
  | `Assoc fields ->
...
let test_state_to_json_reason_value () =
  let state : Tempo.tempo_state =
    { current_interval_s = 300.0; last_adjusted = 0.0; reason = "normal tempo" }
  in
  let json = Tempo.state_to_json state in
  match json with
  | `Assoc fields ->
...
let test_state_of_json_missing_field () =
  let json = `Assoc [ "current_interval_s", `Float 300.0 ] in
  match Tempo.state_of_json json with
  | None -> ()
...
let test_is_pending_task_claimed () =
  let task =
    make_task
      ~id:"t2"
      ~status:(Types.Claimed { assignee = "claude"; claimed_at = "2024-01-01T00:00:00Z" })
  in
...
let test_is_pending_task_in_progress () =
  let task =
    make_task
      ~id:"t3"
      ~status:
        (Types.InProgress { assignee = "claude"; started_at = "2024-01-01T00:00:00Z" })
  in
...
let test_is_pending_task_done () =
  let task =
    make_task
      ...
      ~status:
        (Types.Done
           { assignee = "claude"; completed_at = "2024-01-01T00:00:00Z"; notes = None })
  in
...
let test_calculate_adaptive_tempo_urgent () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "fast") reason 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_calculate_adaptive_tempo_normal () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "normal") reason 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_calculate_adaptive_tempo_low_priority () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "slow") reason 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_tempo_state_type () =
  let state : Tempo.tempo_state =
    { current_interval_s = 120.0; last_adjusted = 1704067200.0; reason = "custom reason" }
  in
...
let test_tempo_state_zero_values () =
  let state : Tempo.tempo_state =
    { current_interval_s = 0.0; last_adjusted = 0.0; reason = "" }
  in
...
let test_format_state_fast_tempo () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Tempo") formatted 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "60s") formatted 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_format_state_normal_tempo () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "5\\.0m") formatted 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_format_state_slow_tempo () =
  let state : Tempo.tempo_state =
    { current_interval_s = 600.0; last_adjusted = Unix.gettimeofday (); reason = "idle" }
  in
  let formatted = Tempo.format_state state in
  check
    bool
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "10\\.0m") formatted 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_format_state_shows_reason () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "urgent tasks present") formatted 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_format_state_old_adjustment () =
  let state : Tempo.tempo_state =
    { current_interval_s = 300.0
    ; last_adjusted = Unix.gettimeofday () -. 3600.0
    ; (* 1 hour ago *)
      reason = "test"
    }
  in
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "h ago") formatted 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_format_state_minutes_ago () =
  let state : Tempo.tempo_state =
    { current_interval_s = 300.0
    ; last_adjusted = Unix.gettimeofday () -. 120.0
    ; (* 2 minutes ago *)
      reason = "test"
    }
  in
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "m ago") formatted 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_format_state_just_now () =
  let state : Tempo.tempo_state =
    { current_interval_s = 300.0
    ; last_adjusted = Unix.gettimeofday () -. 5.0
    ; (* 5 seconds ago *)
      reason = "test"
    }
  in
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "just now") formatted 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_format_state_emoji () =
  let state : Tempo.tempo_state =
    { current_interval_s = 60.0; last_adjusted = Unix.gettimeofday (); reason = "test" }
  in
  let formatted = Tempo.format_state state in
  check
    bool
    "contains timer emoji"
    true
...
let test_default_config_max () =
  check
    ...
    "max > min"
    true
    (Tempo.default_config.max_interval_s > Tempo.default_config.min_interval_s)
;;
