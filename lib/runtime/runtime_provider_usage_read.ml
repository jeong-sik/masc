...
(* An HTTP runtime whose provider declares [usage-read] is read with the key
   ...
   official-client protocol; a Codex app-server, the Antigravity CLI, and a
   Muse serve host answer without a turn. Claude Code states its windows
   only during one. *)
...
let read_scope ~codex ~antigravity ~muse ~fetch { scope; how } =
  match how with
  ...
  | Muse exec ->
    (* No prompt is ever sent on this connection, so the exception text the
       shared helper logs cannot quote one. *)
    read_client_logged ~scope (fun () -> muse ~scope exec)
  | Http http -> read_http_logged ~fetch ~scope http
;;

let read_scopes ~codex ~antigravity ~muse ~fetch readables =
  List.iter (read_scope ~codex ~antigravity ~muse ~fetch) readables
;;
...
let read_all ~mgr ~net ~clock ~cwd =
  ...
  read_scopes ~codex ~antigravity:read_antigravity ~muse ~fetch (readable_scopes ())
;;
...
let repeat_of readables scope =
  List.find_map
    (fun (readable : readable) ->
       ...
       | Http _ | Codex _ | Antigravity _ | Muse _ -> None)
    readables
;;
  ...
  let repeating =
    List.filter_map
      (fun (readable : readable) ->
         ...
         | Codex _ | Antigravity _ | Muse _ -> None)
      (catalogue ())
  in
  Eio.Fiber.List.iter
