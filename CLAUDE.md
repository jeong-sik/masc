...
MASC_PUBLIC_TOOLS_EXTRA=masc_goal_upsert,masc_pause ./start-masc-mcp.sh --http --port "$PORT"

# 전체 표면 복원 (디버깅)
MASC_FULL_SURFACE=1 ./start-masc-mcp.sh --http --port "$PORT"
...
- git worktree checkout default port: `9100-9999` 범위에서 checkout path 기준 자동 파생
