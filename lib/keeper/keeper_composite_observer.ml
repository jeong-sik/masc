...
let collapsed_from_phase (phase : Keeper_state_machine.phase) (derived : ksm_phase)
...
let live_turn_phase (entry : Keeper_registry.registry_entry) =
  ...
  | None ->
    (match derive_ksm_phase entry.phase with
     ...
     | Ksm_running | Ksm_failing | Ksm_overflowed | Ksm_stable -> Turn_idle)
;;
...
let check_phase_turn_alignment (phase : ksm_phase) (turn_phase : turn_phase) : bool =
...
let check_compaction_atomicity (phase : ksm_phase) (compaction_stage : compaction_stage)
  : bool
  =
...
let check_event_priority_monotone (entry : Keeper_registry.registry_entry) : bool =
  match entry.current_turn_observation with
  | None -> true
  | Some obs ->
    obs.measurement_bind_count <= 1
    && not
         (Option.is_some obs.measurement && Option.is_some entry.pending_turn_measurement)
;;
...
let compute_invariants
  ...
  ; no_cascade_before_measurement =
      check_no_cascade_before_measurement ~cascade_state ~measurement_captured
  ...
  let bump key satisfied =
    ...
    then
      Prometheus.inc_counter
        Prometheus.metric_keeper_invariant_violations
        ~labels:[ "keeper", keeper_name; "invariant", invariant_key_to_string key ]
        ()
  in
...
let observe ?correlation_id ?run_id ?now (entry : Keeper_registry.registry_entry)
  : snapshot
  =
  let ts =
    match now with
  ...
  let kcb_state =
    Keeper_failure_circuit_breaker.display_state_of ~keeper_name:entry.name
  in
  ...
  ; consecutive_noop_count = entry.meta.runtime.proactive_rt.consecutive_noop_count
  ; idle_seconds =
      (let last = entry.meta.runtime.proactive_rt.last_ts in
       if last <= 0.0 then 0 else int_of_float (max 0.0 (Time_compat.now () -. last)))
...
let all_snapshots ~(base_path : string) () : snapshot list =
  Keeper_registry.all ~base_path () |> List.map (fun entry -> observe entry)
;;
...
let snapshot_to_json (s : snapshot) : Yojson.Safe.t =
  `Assoc
    ...
    ; "decision", `Assoc [ "stage", `String (decision_stage_to_string s.kdp_decision) ]
    ; "cascade", `Assoc [ "state", `String (cascade_state_to_string s.kcl_cascade_state) ]
    ; ( "compaction"
      , `Assoc [ "stage", `String (compaction_stage_to_string s.kmc_compaction) ] )
    ; ( "circuit_breaker"
      , `Assoc
          [ ( "state"
            , `String (Keeper_failure_circuit_breaker.display_state_to_string s.kcb_state)
            )
          ] )
    ; ( "measurement"
      , match s.shared_measurement with
        | Some m -> `Assoc [ "captured", `Bool true; "auto_rules", measurement_to_json m ]
        | None -> `Assoc [ "captured", `Bool false ] )
    ; "invariants", invariants_to_json s.invariants
    ; "is_live", `Bool s.is_live
    ; ( "last_outcome"
      , match s.last_outcome with
        | Some lo ->
          `Assoc
            [ "turn_id", `Int lo.turn_id
            ; "ended_at", `Float lo.ended_at
            ; "decision_stage", `String (decision_stage_to_string lo.decision_stage)
            ; "cascade_state", `String (cascade_state_to_string lo.cascade_state)
            ; ( "selected_model"
