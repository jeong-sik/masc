...
let expected =
    ...
    ; {|masc_runtime_ollama_probe|}, {|Admin-only native Ollama timing probe with repeated /api/generate calls. It may load a model and change warm/cache state, so it is not read-only or idempotent. Returns loaded models from /api/ps, per-run load/prompt-eval/generation timings, tok/sec estimates, and a timing-based repeated-prefix reuse inference. This does not expose direct KV occupancy or hit-rate.|}, {|{"properties":{"generate_when_unloaded":{"type":"boolean"},"keep_alive":{"type":"string"},"max_tokens":{"description":"Token ceiling for each timed generate call. Values outside 1..128 are clamped to that range rather than refused.","maximum":128,"minimum":1,"type":"integer"},"model":{"type":"string"},"probe_runs":{"description":"How many generate calls to time. Values outside 1..4 are clamped to that range rather than refused.","maximum":4,"minimum":1,"type":"integer"},"prompt":{"type":"string"},"run_generate":{"type":"boolean"},"server_url":{"type":"string"},"think":{"description":"Boolean shorthand for think_mode. false disables reasoning-mode thinking; true enables it.","type":"boolean"},"think_mode":{"description":"Think mode choice. auto lets model decide; disabled skips reasoning; enabled forces it.","enum":["auto","disabled","enabled"],"type":"string"},"think_policy":{"description":"Adaptive thinking policy for Ollama reasoning models. auto defaults to response-oriented non-thinking probes; enabled measures thinking path explicitly.","type":"string"},"timeout_sec":{"description":"Explicit probe timeout in seconds. Every positive value is passed through unchanged.","minimum":1,"type":"integer"}},"type":"object"}|}
    ]
;;
