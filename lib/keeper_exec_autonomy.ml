...
let execute_approved_plan
  ...
  let primary = match specs with p :: _ -> p | [] -> Llm.default_local_model_spec () in
...
Do NOT use destructive tools (bash rm, edit, delete).|}
  ...
  let execute_tool_calls
      (tcs : Llm.tool_call list) : (Llm.tool_call * string) list =
    List.map
      (fun (tc : Llm.tool_call) ->
         let execute () =
  ...
  in
  let requests = List.map (fun (spec : Llm.model_spec) ->
    { initial_request with Llm.model = spec }
  ) specs in
  ...
  | Ok resp0 ->
      let rec exec_loop ~round ~acc_cost ~acc_tools ~last_resp =
        if last_resp.Llm.tool_calls = [] || round > max_rounds then
          let content =
          ...
          let followup_requests = List.map (fun (spec : Llm.model_spec) ->
            { Llm.model = spec;
              messages = [
                Agent_sdk.Types.system_msg system_prompt;
...
let run_autonomous_goal_turn ~(config : Room.config) ~(meta : keeper_meta)
    ~(specs : Llm.model_spec list) : keeper_meta option =
