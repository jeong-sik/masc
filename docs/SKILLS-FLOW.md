...
`disable-model-invocation`은 명시적으로 거부한다. 표준의 실험적 `allowed-tools`는
입력 호환을 위해 파싱한 뒤 runtime 권한으로 해석하지 않고 버린다. composition 선언을
남겨 놓고 도구만 숨기는 별도 상태는 없다. 문서용 예시는 더 긴 CommonMark 외부 fence로
감싼다.
