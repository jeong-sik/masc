...
- `SB_PG_URL` 또는 `SUPABASE_DB_URL`가 Supabase transaction pooler(`*.pooler.supabase.com:6543`)를 가리키면, PostgresNative는 prepared statement 충돌을 피하기 위해 session pooler port `5432`로 정규화합니다.
