...
export function filterCheckpointHistory(
  ...
  return rows.filter((row) => {
    ...
    if (row.source_kind && row.source_kind.toLowerCase().includes(needle))
      return true;
    if (row.latest_preview && row.latest_preview.toLowerCase().includes(needle))
      return true;
    return false;
  });
}
...
function CheckpointSummaryCard({
  ...
  if (!summary) {
    if (status === "unavailable") {
      return html`
        ...
        >
          ...
          ${
            error?.detail
              ? html`<div class="mt-2 break-words font-mono text-2xs">
                  ${error.detail}
                </div>`
              : null
          }
    ...
    return html`
      <div
        class="rounded-[var(--r-1)] border border-[var(--color-border-default)] bg-[var(--color-bg-surface)] px-3 py-3 text-xs text-[var(--color-fg-muted)] v2-monitoring-card"
      >
    ...
    <div class="rounded-[var(--r-1)] border border-[var(--color-border-default)] bg-[var(--color-bg-surface)] px-3 py-3 v2-monitoring-card">
      ...
      ${
        summary.latest_preview
          ? html`<div
              class="mt-2 text-xs leading-relaxed text-[var(--color-fg-primary)]"
            >
              ${summary.latest_preview}
            </div>`
          : null
          ...
          >
            ...
            ${
              failure.error_detail
                ? html`<div
                    class="mt-1 break-words font-mono text-[var(--rose-light)]"
                  >
                    ${failure.error_detail}
                  </div>`
                : null
...
export function KeeperCheckpointPanel({
  ...
  const [inventory, setInventory] = useState<KeeperCheckpointInventory | null>(
    null,
  );
  ...
  const loadInventory = () => {
    void (async () => {
      ...
      try {
        ...
        setSelectedIds((prev) =>
          prev.filter((id) =>
            next.history.some((item) => item.snapshot_id === id),
          ),
        );
      } catch (err) {
        setError(
          err instanceof Error
            ? err.message
            : "checkpoint inventory load failed",
        );
      } finally {
        setLoading(false);
      }
  ...
  const deleteSelected = () => {
    void (async () => {
      ...
      setDeleting(true);
      try {
        const result = await deleteKeeperHistorySnapshots(
          keeperName,
          selectedIds,
        );
        ...
        const missingSuffix =
          result.missing_snapshot_ids.length > 0
            ...
            : "";
        showToast(
          `${result.deleted_snapshot_ids.length}개 snapshot 삭제${missingSuffix}`,
          "success",
        );
      } catch (err) {
        showToast(
          err instanceof Error ? err.message : "snapshot 삭제 실패",
          "error",
        );
      } finally {
        setDeleting(false);
  ...
  if (loading) {
    return html`
      <div
        class="rounded-[var(--r-1)] border border-[var(--color-border-default)] bg-[var(--color-bg-surface)] px-3 py-3 text-xs text-[var(--color-fg-muted)] v2-monitoring-panel"
        role="status"
      >
  ...
  return html`
    <div class="flex flex-col gap-3 v2-monitoring-panel">
      <div
        class="flex items-center justify-between gap-3 v2-monitoring-toolbar"
      >
        <div class="text-2xs text-[var(--color-fg-muted)]">
          current OAS checkpoint와 OAS snapshot history만 노출합니다.
        </div>
        <div class="flex items-center gap-2">
          <${ActionButton} variant="ghost" size="md" onClick=${loadInventory}
            >새로고침<//
          >
          <button
            type="button"
      ...
      <div
        class="rounded-[var(--r-1)] border border-[var(--color-border-default)] bg-[var(--color-bg-surface)] v2-monitoring-panel"
      >
        <div
          class="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--color-border-default)] px-3 py-2 v2-monitoring-toolbar"
        >
          <div
            class="text-2xs font-semibold uppercase tracking-[var(--track-caps)] text-[var(--color-fg-muted)]"
          >
            ...
            ${
              ...
              historyQuery.trim() !== ""
                ? html`<span
                    class="ml-2 text-3xs font-normal normal-case tracking-normal text-[var(--color-fg-disabled)]"
                    >${filterCheckpointHistory(inventory.history, historyQuery).length}/${inventory.history.length}</span
                  >`
                : null
            }
          </div>
        ...
        ${
          !inventory || inventory.history.length === 0
            ? html`<div
                class="px-3 py-3 text-xs text-[var(--color-fg-muted)] v2-monitoring-row"
              >
                저장된 OAS history snapshot이 아직 없습니다.
              </div>`
            : (() => {
                const visibleHistory = filterCheckpointHistory(
                  inventory.history,
                  historyQuery,
                );
                ...
                if (isFiltering && visibleHistory.length === 0) {
                  return html`<div
                    class="px-3 py-4 text-center text-2xs text-[var(--color-fg-disabled)] v2-monitoring-row"
                  >
                    필터 결과 없음 (${inventory.history.length} items)
                  </div>`;
                }
                return html`
...
function isLineageDelta(value: unknown): value is LineageDelta {
  ...
  return (
    isStringArray(value.inherited_fields) &&
    isStringArray(value.changed_fields) &&
    isStringArray(value.dropped_fields)
  );
}

function isGenerationLineageManifest(
  value: unknown,
): value is GenerationLineageManifest {
  if (!isRecord(value)) return false;
  return (
    typeof value.generation === "number" &&
    typeof value.trace_id === "string" &&
    (value.parent_generation == null ||
      typeof value.parent_generation === "number") &&
    (value.parent_trace_id == null ||
      typeof value.parent_trace_id === "string") &&
    (value.created_at == null || typeof value.created_at === "string") &&
    (value.trigger_reason == null ||
      typeof value.trigger_reason === "string") &&
...
function isGenerationLineageEntry(
  value: unknown,
): value is GenerationLineageEntry {
  if (!isRecord(value)) return false;
  return (
    typeof value.generation === "number" &&
    typeof value.trace_id === "string" &&
    ...
    (value.trigger_reason == null ||
      typeof value.trigger_reason === "string") &&
    (value.context_ratio == null || typeof value.context_ratio === "number") &&
    (value.identity_changed_fields == null ||
      isStringArray(value.identity_changed_fields)) &&
    (value.identity_dropped_fields == null ||
      isStringArray(value.identity_dropped_fields))
  );
}
...
export function lineageTransitionLabel(
  parentGeneration: number | null | undefined,
  generation: number,
): string {
...
export function GenerationLineagePanel({ keeperName }: { keeperName: string }) {
  ...
  const currentGeneration =
    typeof lineage.current_generation === "number"
      ? lineage.current_generation
      : null;
  const currentTraceId =
    typeof lineage.current_trace_id === "string"
      ? lineage.current_trace_id
      : null;
  const generationId =
    typeof lineage.generation_id === "string" ? lineage.generation_id : null;
  const traceHistoryCount =
    typeof lineage.trace_history_count === "number"
      ? lineage.trace_history_count
      : 0;
  const manifestPath =
    typeof lineage.manifest_path === "string" ? lineage.manifest_path : null;
  const indexPath =
    typeof lineage.index_path === "string" ? lineage.index_path : null;
  const manifest = isGenerationLineageManifest(lineage.manifest)
    ? lineage.manifest
    : null;
  const recent = (Array.isArray(lineage.recent) ? lineage.recent : []).filter(
    isGenerationLineageEntry,
  );
      ...
      <${PanelCard} title="생성 계보">
              ...
              <div class="flex flex-wrap items-center gap-2 mb-1">
                ...
                ${
                  latestEntry.created_at
                    ? html`<span
                        class="text-3xs text-[var(--color-fg-disabled)]"
                        >recorded
                        <${TimeAgo} timestamp=${latestEntry.created_at}
                      /></span>`
                    : null
                }
              </div>
        ...
        ${
          manifest
            ...
            <div class="rounded-[var(--r-1)] border border-[var(--color-border-default)] bg-[var(--color-bg-surface)] p-3 mb-3 v2-monitoring-panel">
              <div class="flex flex-wrap items-center gap-2 mb-2">
                ...
                ${
                  manifest.created_at
                    ? html`<span
                        class="text-3xs text-[var(--color-fg-disabled)]"
                        >created <${TimeAgo} timestamp=${manifest.created_at}
                      /></span>`
                    : null
                }
              </div>
                ...
                <div class="rounded-[var(--r-1)] border border-[var(--color-border-default)] bg-[var(--color-bg-surface)] px-3 py-2 v2-monitoring-card">
                  ...
                  ${
                    manifest.parent_trace_id
                      ? html`<div
                          class="font-mono text-[var(--color-fg-disabled)] truncate"
                          title=${manifest.parent_trace_id}
                        >
                          ${compactTraceId(manifest.parent_trace_id)}
                        </div>`
                      : null
                  }
                </div>
              ...
              ${
                ...
                delta.dropped_fields.length === 0
                  ? html`<div
                      class="mt-2 text-2xs text-[var(--color-fg-disabled)]"
                    >
                      identity-only inheritance stayed intact across the
                      rollover.
                    </div>`
                  : null
              }
          ...
          `
            : html`
                <div
                  class="rounded-[var(--r-1)] border border-[var(--color-border-default)] bg-[var(--color-bg-surface)] p-3 mb-3 text-2xs text-[var(--color-fg-muted)] v2-monitoring-panel"
                >
                  아직 handoff lineage manifest가 없습니다. generation 0에서는
                  현재 trace만 유지되고, 첫 successful handoff 이후부터
                  manifest/index가 생깁니다.
                </div>
              `
        }
              ...
              ? html`
                  <div class="flex flex-col gap-2 v2-monitoring-row">
                    ...
                    <div class=${`px-3 py-2 rounded-[var(--r-1)] border ${isLatest ? "border-[var(--accent-22)] bg-[var(--accent-8)]" : "border-[var(--color-border-default)] bg-[var(--color-bg-surface)]"} v2-monitoring-row`}>
                      ...
                      ${
                        (entry.identity_changed_fields?.length ?? 0) > 0 ||
                        (entry.identity_dropped_fields?.length ?? 0) > 0
                          ? html`
                              <div
                                class="mt-1 text-3xs text-[var(--color-fg-disabled)]"
                              >
                ...
                `
              : html`<div class="text-2xs text-[var(--color-fg-muted)]">
                  기록된 핸드오프 항목이 아직 없습니다.
                </div>`
          }
        </div>
        ...
        ${
          manifestPath || indexPath
            ? html`
                <div
                  class="mt-3 flex flex-col gap-1 text-3xs text-[var(--color-fg-disabled)]"
                >
