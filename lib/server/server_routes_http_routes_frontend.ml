...
let respond_redirect ~location reqd =
  let response =
    Httpun.Response.create
      ~headers:(Httpun.Headers.of_list [ "location", location; "content-length", "0" ])
      `Found
  in
...
let canonical_loopback_location ~default_port request =
  ...
  if String.equal canonical_host host
  then None
  else Some (Printf.sprintf "http://%s:%d%s" canonical_host port request.target)
;;
...
let canonical_root_dashboard_location ~default_port request =
  ...
  if String.equal canonical_host host
  then None
  else Some (Printf.sprintf "http://%s:%d/dashboard" canonical_host port)
;;
...
let redirect_to_dashboard reqd = respond_redirect ~location:"/dashboard" reqd
...
let is_dashboard_observer_stream request =
  ...
  | Some raw ->
    ...
    String.equal normalized "observer" || String.equal normalized "dashboard"
  | None -> false
;;

let websocket_discovery_handler request reqd =
  let body = websocket_discovery_json request |> Yojson.Safe.to_string in
    ...
    (fun _state _req reqd ->
       ...
       then
         ...
         Http.Request.read_body_async reqd (fun body_str ->
           ...
           | Ok body -> Http.Response.json body reqd
           | Error msg ->
             Http.Response.json
               ~status:`Bad_request
...
let add_routes ~port ~host router =
  ...
  |> Http.Router.get "/dashboard/credits" (fun request reqd ->
    with_canonical_loopback_host
      ~port
      (fun request reqd ->
         with_public_read
           (fun _state _req reqd -> Http.Response.html (Credits_dashboard.html ()) reqd)
           request
           reqd)
      request
      reqd)
  ...
  |> Http.Router.prefix_get "/dashboard/b/assets/" (fun request reqd ->
    with_public_read
      (fun _state req reqd ->
         let req_path = Http.Request.path req in
         let prefix_len = String.length "/dashboard/b/assets/" in
         let filename =
           String.sub req_path prefix_len (String.length req_path - prefix_len)
         in
         if Web_dashboard.is_safe_asset_relative_path filename
         then serve_bonsai_static filename req reqd
         else Http.Response.not_found reqd)
      request
      reqd)
  (* Bonsai API — must precede the /dashboard/b/ SPA catchall. *)
  |> Http.Router.get "/dashboard/b/api/keepers/summary" (fun request reqd ->
    with_public_read
      (fun _state req reqd -> bonsai_api_keepers_summary req reqd)
      request
      reqd)
  |> Http.Router.get "/dashboard/b" (fun request reqd ->
    with_canonical_loopback_host
      ...
      (fun request reqd ->
         with_public_read
           (fun _state req reqd -> serve_bonsai_index req reqd)
           request
           reqd)
      request
      reqd)
  |> Http.Router.get "/dashboard/b/" (fun request reqd ->
    with_canonical_loopback_host
      ...
      request
      reqd)
  |> Http.Router.prefix_get "/dashboard/b/" (fun request reqd ->
    with_canonical_loopback_host
      ~port
      (fun request reqd ->
         with_public_read
  ...
  |> Http.Router.get "/favicon.ico" (fun request reqd ->
    with_public_read (fun _state req reqd -> serve_favicon req reqd) request reqd)
  |> Http.Router.get "/favicon.svg" (fun request reqd ->
    with_public_read (fun _state req reqd -> serve_favicon req reqd) request reqd)
  (* Dashboard SPA: static assets — prefix match for /dashboard/assets/* *)
  |> Http.Router.prefix_get "/dashboard/assets/" (fun request reqd ->
    ...
    if Web_dashboard.is_safe_asset_relative_path filename
    then serve_dashboard_static ("assets/" ^ filename) request reqd
    else Http.Response.not_found reqd)
  ...
  |> Http.Router.get "/dashboard" (fun request reqd ->
    with_canonical_loopback_host
      ...
      (fun request reqd ->
         with_public_read
           (fun _state req reqd -> serve_dashboard_index req reqd)
           request
           reqd)
      request
      reqd)
  |> Http.Router.get "/dashboard/" (fun request reqd ->
    with_canonical_loopback_host
      ...
      request
      reqd)
  |> Http.Router.prefix_get "/dashboard/" (fun request reqd ->
    with_canonical_loopback_host
      ~port
      (fun request reqd ->
         with_public_read
           (fun _state req reqd ->
              let req_path = Http.Request.path req in
              if is_dashboard_spa_deep_link req_path
              then serve_dashboard_index req reqd
              else Http.Response.not_found reqd)
           request
           reqd)
      request
  ...
  |> Http.Router.get "/api/v1/credits" (fun request reqd ->
    with_public_read
      (fun _state _req reqd -> Http.Response.json (Credits_dashboard.json_api ()) reqd)
      request
      reqd)
  ...
  |> Http.Router.get "/graphiql/graphiql.min.js" (serve_graphiql_asset "graphiql.min.js")
  |> Http.Router.get
       "/graphiql/react.production.min.js"
       (serve_graphiql_asset "react.production.min.js")
  |> Http.Router.get
       "/graphiql/react-dom.production.min.js"
       (serve_graphiql_asset "react-dom.production.min.js")
  |> Http.Router.get "/mcp" (fun request reqd ->
    if is_dashboard_observer_stream request
    then
      with_public_read
        (fun _state req reqd -> handle_get_mcp ~sse_kind:Sse.Observer req reqd)
        request
        reqd
    else with_read_auth (fun _state req reqd -> handle_get_mcp req reqd) request reqd)
  ...
  |> Http.Router.get "/sse" (fun request reqd ->
    with_public_read
      (fun _state req reqd ->
         handle_get_mcp
           ~sse_kind:Sse.Observer
