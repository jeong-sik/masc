...
let session_link_file ~base_path session_id =
  Filename.concat
    (Common.masc_dir_from_base_path ~base_path)
    (Filename.concat "team-sessions" (Filename.concat session_id "autoresearch.json"))
;;
...
let ensure_dir path =
  let rec mkdir_p dir =
    ...
    then (
      ...
      | Sys_error msg -> Log.Misc.warn "ensure_dir: mkdir %s failed: %s" dir msg)
  in
  mkdir_p path
;;
...
let decode_json_file_result ~path ~kind decode =
  ...
  | None -> None
  | Some (Error msg) ->
    Some (Error (Printf.sprintf "%s load failed for %s: %s" kind path msg))
  | Some (Ok json) ->
    Some
      (match decode json with
       | Ok value -> Ok value
       | Error msg -> Error (Printf.sprintf "%s decode failed for %s: %s" kind path msg))
;;

let load_execution_link_by_loop_result ~base_path loop_id =
  let path = loop_link_file ~base_path loop_id in
  decode_json_file_result
    ~path
    ~kind:"execution link"
...
let load_state_result ~base_path loop_id =
  ...
  | None -> None
  | Some (Error msg) ->
    Some (Error (Printf.sprintf "autoresearch state load failed for %s: %s" path msg))
  | Some (Ok json) ->
    Some
      (match json with
       | `Assoc fields
         ...
         Error
           (Printf.sprintf
              "unsupported legacy autoresearch state schema for %s: found llm_model; \
               expected model_model"
              path)
       | _ ->
         (match Autoresearch_serde.state_of_yojson_result json with
          ...
          | Error msg ->
            Error (Printf.sprintf "autoresearch state decode failed for %s: %s" path msg)))
;;
    ...
    let lines =
      ...
      | exn ->
        Log.Autoresearch.warn
          "cycle history load failed for %s: %s"
          path
...
let scan_persisted_loop_ids ~base_path =
  ...
  then []
    ...
    | exn ->
      Log.Misc.warn
        "get_loop_ids_with_state: readdir %s failed: %s"
        dir
        (Printexc.to_string exn);
      [])
;;
