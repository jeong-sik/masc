...
let get_registry () : (Agent_identity.Registry.registry, string) result =
  ...
  | None ->
    ...
    (match !global_registry with
     ...
     | None ->
       Error
         "agent registry initialization failed: global_registry is None after creation")
;;
...
let maybe_evict_session_caches_locked () =
  ...
  if
    SMap.cardinal id_map > max_session_cache_entries
...
let get_or_create_identity ?mcp_session_id params =
  ...
  let touch_and_return identity =
    let room_id =
      Yojson.Safe.Util.(
        try Some (params |> member "room" |> to_string) with
        | Yojson.Safe.Util.Type_error _ -> None)
    in
    Agent_identity.Registry.touch reg identity.Agent_identity.session_key ?room_id ();
    ...
    (match mcp_session_id with
     | Some sid ->
       ...
       atomic_update session_identity_map (fun map ->
         SMap.add sid registered.session_key map)
     | None -> ());
    Log.Session.info
      "[AgentRegistry] New identity: %s (session=%s, mcp=%s)"
      registered.agent_name
      (String.sub registered.session_key 0 (min 8 (String.length registered.session_key)))
...
let get_resolved_name sid = SMap.find_opt sid (Atomic.get resolved_names)
...
let unregister session_key =
  ...
  | Error e ->
    Log.Identity.warn
      "unregister(%s): registry unavailable: %s"
  ...
  | Ok reg ->
    ...
    let to_remove =
      SMap.fold
        (fun sid sk acc -> if sk = session_key then sid :: acc else acc)
        current_map
        []
    in
    atomic_update session_identity_map (fun map ->
