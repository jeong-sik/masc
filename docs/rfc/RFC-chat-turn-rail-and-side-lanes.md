...
2026-09-22 에 "외부 메시지는 오른쪽에 왼쪽 정렬, 내 대화와 채팅 대상을
분리" 를 골랐다.

- 대상은 `Inbound` 행이다 — 다른 keeper, 다른 사람, connector 로 들어온
  남의 말. journal 은 이 keeper 자신의 기억이라 왼쪽 측선에 남는다.
- pane 안쪽 폭이 96칸 이상(100칸 터미널)일 때만 오른쪽으로 옮긴다.
  들여쓰기는 안쪽 폭의 3분의 1 이다. 그보다 좁으면 지금처럼 한 칼럼이다 —
  오른쪽 칸의 본문이 한 줄에 몇 단어도 못 담으면 분리가 읽기를 방해한다.
- 합류 표시 `───┤` 는 대화 줄에 그대로 둔다. `┤` 는 "이 선에 닿았다" 는
  뜻이라 블록 쪽으로 옮기면 거짓말이 된다. 그 뒤를 비우고 시계·라벨·본문이
  오른쪽 칸에서 시작한다.
- `metadata:full` 제목 줄도 같은 칸에서 시작하고, 규칙선은 오른쪽 끝의
  시계까지 간다.
- 폭 계산은 레이아웃 한 곳(`rows_of_entry`)이 한다. 측정과 그리기가 같은
  행을 쓰므로 스크롤 위치가 칼럼 전환으로 어긋나지 않는다.

```
  ▶ YOU · tui-01..a1 ───────────────────────────────────────── 17:20:01
    순찰 돌아줘
  ● tui-01..a1 ─────────────────────────────────────────────── 17:20:04
  │ ✓ Execute gh pr list …
    열린 PR 6건 확인했어요.
                             ◀ pangyo-preachers · board ───── 17:21:10
                               #37740 담당 등기했어요. 브랜치 갱신은
                               제가 할게요.
```
...
| 8 | 남이 보낸 줄을 오른쪽 칸으로 (§4.6) | 넓은 pane 에서 `Inbound` 블록이 3분의 1 지점에서 시작 | 96칸 미만은 한 칼럼, 측정과 그리기가 같은 행 | |
