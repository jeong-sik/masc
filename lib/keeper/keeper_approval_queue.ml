  ...
  with_rules_lock (fun () ->
    ...
    | None ->
      (match save_rules_unlocked ?base_path (candidate :: rules) with
       | Ok () -> ()
       | Error msg -> Log.Keeper.warn "upsert_rule: save failed: %s" msg);
      candidate, true)
;;
  ...
  with_rules_lock (fun () ->
    ...
    | Some rule ->
      ...
      (match save_rules_unlocked ?base_path updated_rules with
       | Ok () -> ()
       | Error msg -> Log.Keeper.warn "find_matching_rule: save failed: %s" msg);
  ...
  List.iter
    (fun (id, entry) ->
       ...
       audit_approval_event
         ...
         ();
       (match entry.resolver with
        | Some resolver -> Eio.Promise.resolve resolver (Oas.Hooks.Reject reason)
        | None -> ());
       match entry.on_resolution with
       | Some f ->
         (try f (Oas.Hooks.Reject reason) with
          | Eio.Cancel.Cancelled _ as e -> raise e
          | exn ->
            Log.Keeper.warn
