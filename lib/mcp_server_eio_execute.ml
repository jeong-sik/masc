...
let log_mcp_exn = Mcp_server_eio_execute_helpers.log_mcp_exn
let wait_for_message_eio = Mcp_server_eio_execute_helpers.wait_for_message_eio
let agent_runtime_root = Mcp_server_eio_execute_helpers.agent_runtime_root
let resolve_join_state = Mcp_server_eio_execute_helpers.resolve_join_state
let silent_auth_token_error_kind = Mcp_server_eio_execute_helpers.silent_auth_token_error_kind

let should_read_legacy_persisted_agent_name =
  Mcp_server_eio_execute_helpers.should_read_legacy_persisted_agent_name
;;

let caller_agent_name_from_arguments =
  Mcp_server_eio_execute_helpers.caller_agent_name_from_arguments
;;

let direct_call_block_message = Mcp_server_eio_execute_helpers.direct_call_block_message

let cleanup_internal_keeper_runtime_resource =
  Mcp_server_eio_execute_helpers.cleanup_internal_keeper_runtime_resource
;;

let run_with_cleanup_preserving_primary =
  Mcp_server_eio_execute_helpers.run_with_cleanup_preserving_primary
;;
...
let execute_tool_eio
  ...
  let read_mcp_session_agent () =
    Mcp_server_eio_execute_helpers.read_mcp_session_agent ~mcp_session_id ()
  in
  let write_mcp_session_agent agent_name =
    Mcp_server_eio_execute_helpers.write_mcp_session_agent
      ~mcp_session_id
      ~agent_name
  in
  ...
  let persisted_agent_name () =
    Mcp_server_eio_execute_helpers.persisted_agent_name
      ~mcp_session_id
