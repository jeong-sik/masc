...
(* The slot history and the run total sit one line apart in the lane detail,
   and the history covers only the runs that named a slot -- on the live Board
   Attention lane, 357 of 489. The shortfall is arithmetic over three counts
   this binding already reads, so it is asked of the one function that does
   it rather than spelled again beside the line. *)
let test_the_lane_slot_history_says_what_it_does_not_cover () =
  Alcotest.(check int) "the detail asks how many runs named no slot" 1
    (Ast_grep.count_calls_in_value_binding ~module_path:render
       ~binding_name:"standalone_lane_detail_lines"
