...
and last_outcome = {
  ...
  runtime_outcome : [`Done | `Exhausted] option;
...
- `derive_runtime_state`: `current_turn_observation`이 None이면 `Idle`. (Phase 2에서는 `Executing`까지만 도달, Phase 3에서 세분화)
