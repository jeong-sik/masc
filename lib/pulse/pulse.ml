...
type recovery_config =
  { max_consecutive_failures : int
    (** Disable consumer after this many consecutive failures *)
  }

let default_recovery_config = { max_consecutive_failures = 3 }
...
let default_rhythm = { base_s = 60.0; min_s = 30.0; max_s = 300.0; quiet = 1, 6 }
...
let effective_interval clock rhythm =
  let base = if is_quiet_hour clock rhythm then rhythm.base_s *. 3.0 else rhythm.base_s in
...
let dispatch_consumers_with_recovery t beat =
  ...
  List.iter
    (fun (module C : Consumer) ->
       if List.mem C.name t.disabled_consumers
       then () (* skip disabled consumers *)
       else if C.should_act beat
       then (
         ...
         | Error msg ->
           let prev =
             match Hashtbl.find_opt t.consumer_failures C.name with
             | Some n -> n
             | None -> 0
           in
           let count = prev + 1 in
           ...
           Log.Pulse.warn
             "consumer %s error on beat #%d (%d/%d): %s"
             C.name
             beat.seq
             count
             t.recovery.max_consecutive_failures
             msg;
           if count >= t.recovery.max_consecutive_failures
           then to_disable := C.name :: !to_disable))
    t.consumers;
  ...
  List.iter
    (fun name ->
       ...
       Log.Pulse.error
         "consumer %s DISABLED after %d consecutive failures"
         name
         t.recovery.max_consecutive_failures)
    !to_disable
;;
...
let tick t trigger =
  t.seq <- t.seq + 1;
  let beat = { seq = t.seq; ts = now t.clock; trigger } in
  t.last_beat_v <- Some beat;
  (match trigger with
   | Nudge _ -> t.total_nudges <- t.total_nudges + 1
   | Rhythm | Demand -> ());
  ...
  (match t.lifecycle with
   | Always_on -> ()
   | Bounded pred ->
     if pred beat && not (is_shutdown t)
     then (
...
let run ~sw t =
  ...
  | Bounded _ ->
    Eio.Fiber.fork ~sw (fun () ->
      (* Safe: finally is mutable field write — no I/O, no exception risk *)
      Fun.protect (fun () -> loop t) ~finally:(fun () -> t.alive <- false))
;;

let nudge t ~reason =
  if t.alive && Eio.Stream.is_empty t.nudge_stream
  then
...
let shutdown t = if not (is_shutdown t) then Eio.Promise.resolve t.shutdown_r ()

let set_rhythm t rhythm =
  t.rhythm <- rhythm;
  Log.Pulse.info
    "rhythm updated: base=%.1fs min=%.1fs max=%.1fs"
    rhythm.base_s
    rhythm.min_s
    rhythm.max_s
;;
...
let stats t =
  ...
  let avg = if t.seq > 0 then uptime /. Float.of_int t.seq else 0.0 in
  { total_beats = t.seq
...
let add_consumer t consumer = t.consumers <- t.consumers @ [ consumer ]
...
let reenable_consumer t name =
  if List.mem name t.disabled_consumers
  then (
...
module For_testing = struct
  let is_quiet_hour_at ~hour ~quiet_range =
    ...
    if qs <= qe then hour >= qs && hour < qe else hour >= qs || hour < qe
  ;;

  let effective_interval_at ~hour rhythm =
    let quiet = is_quiet_hour_at ~hour ~quiet_range:rhythm.quiet in
    let base = if quiet then rhythm.base_s *. 3.0 else rhythm.base_s in
    Float.max rhythm.min_s (Float.min rhythm.max_s base)
  ;;
end
