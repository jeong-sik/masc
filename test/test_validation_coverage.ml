...
let test_agent_id_valid_colon_namespace () =
  ...
  | Ok t ->
    check
      string
      "colon namespace"
      "keeper:keeper-test-98295-0"
      (Validation.Agent_id.to_string t)
  | Error e -> fail e
;;
...
let test_task_id_quoted_inner_invalid () =
  ...
  | Error msg ->
    check
      ...
      true
      (try
         ignore (Re.exec (Re.Pcre.re "quotes" |> Re.compile) msg);
         true
       with
       | Not_found -> false)
;;
...
let () =
  run
    ...
    ; ( "quote_stripping_9787"
      , [ test_case
            "task single-quoted recoverable"
            `Quick
            test_task_id_single_quoted_recoverable
        ; test_case
            "task double-quoted recoverable"
            `Quick
            test_task_id_double_quoted_recoverable
        ; test_case "task quoted inner invalid" `Quick test_task_id_quoted_inner_invalid
        ; test_case
            "task mismatched quotes"
            `Quick
            test_task_id_mismatched_quotes_not_stripped
        ; test_case "task unquoted still works" `Quick test_task_id_unquoted_still_works
        ; test_case
            "agent single-quoted recoverable"
            `Quick
            test_agent_id_single_quoted_recoverable
