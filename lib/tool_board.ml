...
let cached_board_list ~key compute =
  ...
  | Some cached_key, Some value
    when String.equal cached_key key && now < _board_list_cache.expires_at -> value
  | _ ->
...
let format_timestamp_relative ts =
  ...
  then Printf.sprintf "%dm ago" (int_of_float (diff /. 60.0))
  else if diff < Masc_time_constants.day
  then Printf.sprintf "%dh ago" (int_of_float (diff /. 3600.0))
...
let format_ttl_remaining expires_at =
  ...
  then "permanent"
    ...
    then Printf.sprintf "%dm left" (int_of_float (remaining /. 60.0))
    else if remaining < Masc_time_constants.day
    then Printf.sprintf "%dh left" (int_of_float (remaining /. 3600.0))
...
let resolve_board_post_kind ~author (raw_kind : string option)
  ...
  | None ->
    ...
    if author_lc = "" || author_lc = "anonymous"
    then
...
let detect_truncated_markdown_with_reason (text : string) : truncation_signal option =
  ...
  then Some Odd_double_asterisk
    ...
    then (
      ...
      if bracket_pos >= 0 && text.[bracket_pos] = ']'
      then (
        ...
        let is_image =
          ...
          if !scan >= 0 && text.[!scan] = '[' && !scan > 0 && text.[!scan - 1] = '!'
          then saw_image := true;
          !saw_image
        in
...
let handle_post_create args =
  ...
  match title with
  | Some t when String.trim t = "" -> false, "Title must not be empty or whitespace-only"
  | _ ->
    ...
    else if author = None || author = Some "" || author = Some "anonymous"
    then false, "❌ author is required"
    ...
    then
      ...
      let visibility =
        match visibility_of_string visibility_str with
        | Some v -> v
        | None -> Board.Internal
      in
...
let handle_post_list args =
  ...
  let random = get_bool args "random" false in
  if random
  then handle_post_list_uncached args
  else (
...
let handle_comment_add args =
  ...
  let ttl_hours = get_int args "ttl_hours" Board.Limits.default_ttl_hours in
  if String.trim post_id = ""
  then false, "post_id is required"
  else if String.trim content = ""
  then false, "Content must not be empty"
  else if author = None || author = Some "" || author = Some "anonymous"
  then false, "author is required"
  else if String.length content > Board.Limits.max_content_length
  then
...
let handle_vote args =
  ...
  | Error (Board.Already_voted _) ->
    ...
    (match Board_dispatch.get_post ~post_id with
     ...
     | Error _ -> true, "Already voted (idempotent). Score unchanged.")
  | Error e -> false, Printf.sprintf "❌ %s" (board_error_to_string e)
;;
...
let handle_search args =
  ...
  let compact = get_bool args "compact" true in
  if query = ""
  then false, "❌ query required"
  else (
    let results = Board_dispatch.search ~query ~limit in
    if results = []
    then true, Printf.sprintf "🔍 '%s' 검색 결과 없음" query
    else (
      let fmt = if compact then format_post_compact else format_post in
...
let handle_comment_vote args =
  ...
  let direction = if direction_str = "down" then Board.Down else Board.Up in
  if comment_id = ""
  then false, "❌ comment_id required"
  else (
    ...
    | Ok score ->
      ...
      , Printf.sprintf
          "%s 코멘트 투표 완료! 점수: %+d"
          (if direction_str = "down" then "👎" else "👍")
          score )
    | Error (Board.Already_voted _) ->
      ( true
      , Printf.sprintf
...
let handle_profile args =
  ...
  then false, "❌ agent required"
    ...
    let agent_posts =
      List.filter
        (fun (p : Board.post) -> norm (Board.Agent_id.to_string p.author) = agent_norm)
        all_posts
    in
    let post_votes =
      List.fold_left
    ...
    let agent_comments =
      List.filter
        (fun (c : Board.comment) -> norm (Board.Agent_id.to_string c.author) = agent_norm)
        all_comments
    in
    let comment_votes =
      List.fold_left
           ...
           constructor would silently miss this site. *)
              ...
              ; ( "since"
                , `Assoc
                    ...
                    ; ( "description"
                      , `String
                          "Unix timestamp. Posts with activity after this time show a 🔔 \
                           indicator" )
                    ] )
              ; ( "compact"
                , `Assoc
...
let handle_board_cleanup args =
  ...
  in
  let all_posts = Board_dispatch.list_posts ~sort_by:Recent ~limit:500 () in
  let candidates =
    List.filter
      (fun (p : Board.post) ->
...
let register () =
  ...
  let tool_required_permission = function
    ...
    | "masc_board_search"
    | "masc_board_profile"
    | "masc_board_hearths" -> Some Types.CanReadState
    | "masc_board_post"
    | "masc_board_comment"
    | "masc_board_vote"
    | "masc_board_comment_vote" -> Some Types.CanBroadcast
    | "masc_board_delete" | "masc_board_cleanup" -> Some Types.CanAdmin
    | _ -> None
  in
