...
let test_log_entry_type () =
  let e : Run_eio.log_entry =
    { timestamp = "2024-01-01T12:00:00Z"; note = "Started task execution" }
  in
...
let test_log_entry_empty_note () =
  let e : Run_eio.log_entry = { timestamp = "2024-01-01T12:30:00Z"; note = "" } in
...
let test_run_record_json_none_agent () =
  ...
  let json = Run_eio.run_record_to_json original in
  match Run_eio.run_record_of_json json with
  | Some decoded -> check bool "agent_name None" true (decoded.agent_name = None)
  | None -> fail "json decode failed"
;;

let test_log_entry_json_roundtrip () =
  let original : Run_eio.log_entry =
    { timestamp = "2024-01-01T15:00:00Z"; note = "Completed step 3" }
  in
...
let test_update_plan () =
  ...
  match
    Run_eio.update_plan
      ...
      ~content:"# New Plan\n- Step 1\n- Step 2"
  with
  | Ok run -> check bool "plan updated" true (String.length run.plan > 0)
...
let test_set_deliverable () =
  ...
  match
    Run_eio.set_deliverable
      config
      ~task_id:"task-deliv-001"
      ~content:"# Deliverable\nCompleted successfully."
  with
  | Ok run -> check bool "deliverable set" true (String.length run.deliverable > 0)
...
let test_log_entry_json_long_note () =
  ...
  let entry : Run_eio.log_entry =
    { timestamp = "2024-01-01T00:00:00Z"; note = long_note }
  in
  let json = Run_eio.log_entry_to_json entry in
  match Run_eio.log_entry_of_json json with
  | Some decoded -> check int "long note preserved" 10000 (String.length decoded.note)
...
let () =
  run
    ...
    ; ( "json_edge_cases"
      , [ test_case
            "run_record missing field"
            `Quick
            test_run_record_of_json_missing_field
