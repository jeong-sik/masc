...
- **Librarian 이 서도 Keeper 요청은 창을 넘지 않는다**(2026-09-23 운영자 결정, #37793). 창이 읽은 위치에 기대면, Librarian 이 서 있는 동안 Keeper 의 요청이 턴마다 커진다. 그 끝은 provider 의 거절이고, 그 뒤로는 매 턴 같은 거절이다. Keeper 가 턴을 돌지 못하니 I7 이 사실상 깨진다. "읽은 위치부터 보낸다"와 "건너뛰지 않는다"는 묶여 있지 않다. I3 은 Librarian 이 읽는 위치를 말하고, Keeper 요청이 어디서 시작하는지는 말하지 않는다. 그래서 이렇게 한다.
  1. 판정은 추정이 아니라 일어난 일이다. Librarian 지점부터 실은 요청을 provider 가 크기 때문에 typed 로 거절하면(`Context_overflow`, 요청 본문 거절) 그 턴은 마지막으로 끝난 턴의 경계부터 같은 후보에 한 번 더 보낸다. 지점이 없는 Keeper 의 씨앗이 거절될 때(`Turn_start_after_seed_refusal`, #38046)와 같은 모양이다. 크기를 미리 재는 문턱도, 시간도, 실패 횟수도 쓰지 않는다.
  2. 받아들여진 요청의 앞머리는 ledger 에 남는다. 다음 턴은 Librarian 지점이 그 앞머리보다 앞에 있으면 ledger 앞머리부터 싣는다. 같은 거절을 매 턴 다시 올려 보내지 않는다.
  3. 그 사이, 곧 Librarian 지점부터 ledger 앞머리 직전까지가 gap 이다. Keeper 요청에도 기억에도 아직 없는 구간이다. gap 은 저장하지 않는다. 진행 파일의 읽은 위치와 ledger 앞머리라는 두 durable 사실에서 매번 계산한다. 이 경로는 Librarian 의 읽은 위치를 옮기지 않고, 쓰는 것은 ledger 앞머리 하나뿐이다. 그래서 "시작점은 옮겼는데 gap 기록은 못 남긴" 상태가 없다. 빈 구간을 지나며 진행을 커밋해 거절을 영구 손실로 바꾸는 모양(#37941)과 다른 점이 이것이다.
  4. 요청은 gap 의 범위(시작 atom, 끝 atom, 바이트)를 typed 블록으로 싣는다. 모델이 그 구간을 없던 일로 읽지 않게 하려는 것이다. 화면에는 밀림 수(I4)와 같은 자리에 `Librarian_stalled` 가 뜬다. 경보이지 Gate 가 아니다.
  5. Librarian 은 gap 도 빠짐없이 읽는다(I1·I3 그대로). 읽은 위치가 ledger 앞머리에 닿으면 gap 과 경보가 함께 사라지고, 다음 턴은 다시 읽은 위치부터 싣는다.
  6. 경계부터 보낸 요청도 크기로 거절되면 그 턴은 그 거절로 끝난다. 그때 넘치는 것은 이력이 아니라 매 턴 붙는 부분이다(창 RFC §13.9 첫 항목).
  - 이것은 마지막 방어선이다. Librarian 이 서는 원인은 따로 고친다. 원인이 빠진 `agent_core_execution_failed` 를 크기 문제로 읽어 폭을 접는 것이 그 하나다(#37899). `Librarian_stalled` 가 자주 뜨면 원인 수정이 모자란 것이다.
  - 공식 클라이언트 레인은 자기 바이트 창으로 목록을 자르므로(`Keeper_official_client_host.window_carried_range`) 이 경로가 닿지 않는다. 그 레인에서 선 Librarian 은 I4 의 밀림 수로만 보인다.
