...
function ConvergenceBar({ pct, size = 'md' }: { pct: number; size?: 'sm' | 'md' }) {
  ...
  return html`
    <div class="flex items-center gap-2">
      <div class="flex-1 ${h} rounded-sm bg-white/10 overflow-hidden" role="progressbar" aria-valuenow=${clamped} aria-valuemin=${0} aria-valuemax=${100} aria-label="목표 진행률">
          ...
          <div class="flex flex-wrap items-center gap-2">
            ${data && data.tree.length > 0 ? html`
              <input
                ...
                class="min-w-45 max-w-65 rounded border border-white/10 bg-white/5 px-2 py-1 text-xs text-text-body placeholder:text-text-dim focus:outline-none focus:border-accent focus-visible:ring-2 focus-visible:ring-[var(--accent)]/50"
              />
