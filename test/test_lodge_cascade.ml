...
let test_load_models_from_config () =
  with_temp_json
    ...
    (fun path ->
      ...
      match specs with
      | [ first; second ] ->
          check bool "first is llama" true (first.provider = Masc_mcp.Llm_types.Llama);
          check string "first model" "qwen-local" first.model_id;
          check bool "second is llama" true (second.provider = Masc_mcp.Llm_types.Llama);
        ...
        (fun path ->
          ...
          | [ only ] ->
              check bool "llama provider" true
                (only.provider = Masc_mcp.Llm_types.Llama);
...
let test_missing_config_uses_defaults () =
  ...
  | first :: _ ->
      check bool "default starts with llama" true
        (first.provider = Masc_mcp.Llm_types.Llama);
