...
let update_keeper_sandbox_mode config keeper_name ~sandbox_profile ~network_mode =
  ...
  match Keeper_types.write_meta config { meta with sandbox_profile; network_mode } with
  | Ok () -> ()
...
let check_keeper_identity_repaired config keeper_name previous_trace_id =
  ...
  Alcotest.(check string)
    "agent name restored to canonical"
  ...
  Alcotest.(check bool)
    "previous trace retained in history"
    true
...
let fake_docker_managed_sandbox_script =
  "#!/bin/sh\n\
   ...
   IFS=$tab read -r cid name image status running created_at keeper kind network \
   owner_pid started_at ttl_sec < \"$state_file\"\n\
   }\n\
   if [ -n \"$log_file\" ]; then\n\
   printf '%s\\n' \"$1 $*\" >> \"$log_file\"\n\
   fi\n\
   ...
   *)\n\
   ...
   printf \
   'managed-1\\t%s\\t%s\\trunning\\ttrue\\t2026-04-24T00:00:00Z\\t%s\\t%s\\t%s\\t%s\\t%s\\t%s\\n' \
   \\\n\
   \"$name\" \"$image\" \"$keeper\" \"$kind\" \"$network\" \"$owner_pid\" \
   \"$started_at\" \"$ttl_sec\" > \"$state_file\"\n\
   printf 'managed-1\\n'\n\
   exit 0\n\
   ;;\n\
   inspect)\n\
   ...
   *'.Config.Image'*)\n\
   ...
   \"$cid\" \"$name\" \"$image\" \"$status\" \"$running\" \"$created_at\" \"$keeper\" \
   \"$kind\" \"$network\" \"$owner_pid\" \"$started_at\" \"$ttl_sec\"\n\
   ;;\n\
   *)\n\
    ...
    (fun () ->
       ...
       let ok, body =
         dispatch_keeper_exn
           ...
           ~args:
             (`Assoc [ "name", `String keeper_name; "include_preflight", `Bool false ])
       in
       Alcotest.(check bool) "sandbox status ok" true ok;
       let sandbox_json = parse_json_exn body |> Yojson.Safe.Util.member "sandbox" in
       ...
       Alcotest.(check (option string))
         "local why_no_container"
         (Some "sandbox_profile=local")
         (sandbox_json |> member "why_no_container" |> to_string_option);
       Alcotest.(check bool)
         "identity matches canonical agent"
         true
         (sandbox_json |> member "identity" |> member "agent_name_matches" |> to_bool);
       let ok, status_body =
         dispatch_keeper_exn
           keeper_ctx
       ...
       Alcotest.(check string)
         "status surfaces sandbox_live local"
         "local"
         Yojson.Safe.Util.(
           status_json |> member "sandbox_live" |> member "effective_mode" |> to_string))
;;
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Alcotest.(check int)
         ...
         (sandbox_json |> member "container_count" |> to_int);
       Alcotest.(check (option string))
         "no why_no_container while running"
         None
         (sandbox_json |> member "why_no_container" |> to_string_option);
       let container = sandbox_json |> member "containers" |> to_list |> List.hd in
       Alcotest.(check (option string))
         "managed kind surfaced"
         (Some "managed")
         (container |> member "container_kind" |> to_string_option);
       Alcotest.(check (option string))
         "container network surfaced"
         (Some "none")
       ...
       Alcotest.(check int)
         "no containers after stop"
         0
         (final_sandbox |> member "container_count" |> to_int);
       Alcotest.(check bool)
         "why_no_container restored"
         true
         (Option.is_some (final_sandbox |> member "why_no_container" |> to_string_option)))
;;
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Some row ->
         ...
         Alcotest.(check bool)
           "autonomy_tick hidden from available actions"
           true
         ...
         in
         Alcotest.(check bool)
           "keeper identity login requires confirm"
           true
           Yojson.Safe.Util.(
         ...
         let keeper_identity_status =
           ...
           | Some row -> row
           | None ->
             Alcotest.fail "expected keeper_github_identity_status in available_actions"
         in
         Alcotest.(check bool)
           "keeper identity status confirm false"
           false
           Yojson.Safe.Util.(
         ...
         Alcotest.(check bool)
           "team_stop is NOT in available actions"
           true
           (Option.is_none (find_action "team_stop"));
         Alcotest.(check bool)
           "team_turn is NOT in available actions"
           true
...
let test_keeper_status_exposes_summary_and_recoverable () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Alcotest.(check bool)
         "diagnostic removed from status"
         true
         Yojson.Safe.Util.(status_json |> member "diagnostic" = `Null);
       Alcotest.(check string)
         "auto team session removed"
         "removed"
         Yojson.Safe.Util.(
...
let test_keeper_up_ignores_non_public_social_model_arg () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Alcotest.(check string)
         "keeps canonical keeper name"
         keeper_name
         Yojson.Safe.Util.(json |> member "name" |> to_string);
       Alcotest.(check string)
         "ignores arg social_model"
         "bdi_speech_v1"
...
let test_keeper_status_accepts_agent_name_alias () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let status_json = parse_json_exn body in
       Alcotest.(check string)
         "status resolves canonical keeper name"
         keeper_name
         Yojson.Safe.Util.(status_json |> member "name" |> to_string))
;;
...
let test_keeper_status_accepts_legacy_separator_agent_alias () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let status_json = parse_json_exn body in
       Alcotest.(check string)
         "legacy alias resolves canonical keeper name"
         keeper_name
         Yojson.Safe.Util.(status_json |> member "name" |> to_string))
;;
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let previous_trace_id =
         drift_keeper_identity config keeper_name ~agent_name:"keeper_issue_king_agent"
       in
       let ok, body =
         dispatch_keeper_exn keeper_ctx ~name:"masc_keeper_up" ~args:up_args
       in
...
let test_keeper_status_reseeds_separator_identity_drift () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Alcotest.(check bool) "keeper up ok" true ok;
       let previous_trace_id =
         drift_keeper_identity config keeper_name ~agent_name:"keeper-issue-king-agent"
       in
       let ok, body =
         dispatch_keeper_exn
           keeper_ctx
       ...
       Alcotest.(check string)
         "status keeps canonical keeper name"
         keeper_name
         Yojson.Safe.Util.(status_json |> member "name" |> to_string);
...
let test_keeper_sandbox_status_reseeds_separator_identity_drift () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let open Yojson.Safe.Util in
       Alcotest.(check bool)
         "sandbox identity matches canonical agent"
         true
         (sandbox_json |> member "identity" |> member "agent_name_matches" |> to_bool);
       Alcotest.(check string)
         "sandbox expected canonical agent"
         (Keeper_types.keeper_agent_name keeper_name)
...
let test_keeper_repair_reseeds_identity_drift () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let working_dir = Keeper_sandbox.host_root_abs_of_meta ~config meta_before in
       let previous_trace_id =
         drift_keeper_identity
           config
           keeper_name
       ...
       Alcotest.(check string)
         "repair restores canonical agent name"
         (Keeper_types.keeper_agent_name keeper_name)
         meta_after.agent_name;
       Alcotest.(check bool)
         "repair rotates trace id"
         true
         (not (String.equal current_trace_id previous_trace_id)))
;;
...
let test_keeper_status_exposes_model_observability () =
  ...
  Fun.protect
    ...
    (fun () ->
                   ...
                   ; ( "attempts"
                     , `List
                         [ `Assoc
                             ...
                             ; "model_label", `String "llama:qwen3.5-35b-a3b-ud-q8-xl"
                             ; "latency_ms", `Null
                         ...
                         ; `Assoc
                             ...
                             ; "model_label", `String "llama:qwen3.5-3b-a3b-ud-q8-xl"
                             ; "latency_ms", `Int 187
                             ; "error", `Null
                             ]
                         ] )
       ...
       Alcotest.(check bool)
         "recent turn observation true"
         true
         (observability |> member "recent_turn_observation" |> to_bool);
       Alcotest.(check (list string))
         "configured labels surfaced"
         [ "llama:auto"; "glm:auto" ]
         (observability |> member "configured_labels" |> to_list |> List.map to_string);
       Alcotest.(check (list string))
         "resolved candidates surfaced"
         [ "llama:qwen3.5-35b-a3b-ud-q8-xl"; "llama:qwen3.5-3b-a3b-ud-q8-xl" ]
         (observability |> member "resolved_candidates" |> to_list |> List.map to_string);
       Alcotest.(check (option string))
         ("selected model surfaced\n" ^ status_dump)
         (Some "llama:qwen3.5-3b-a3b-ud-q8-xl")
         (observability |> member "selected_model" |> to_string_option);
       Alcotest.(check string)
         "attempt summary surfaced"
         "2 attempt(s); fallback after 1 hop(s); selected candidate 2/2."
         (observability |> member "attempt_summary" |> member "summary" |> to_string);
       Alcotest.(check string)
         "runtime scope local"
         "local"
         (observability
          |> member "runtime_contract"
          |> member "provider_scope"
          |> to_string);
       Alcotest.(check bool)
         "runtime contract unverified"
         false
         (observability |> member "runtime_contract" |> member "verified" |> to_bool);
       Alcotest.(check bool)
         "chat compatibility intentionally null"
         true
         (observability
          |> member "runtime_contract"
          |> member "chat_completion_compatible"
          = `Null);
       Alcotest.(check string)
         "runtime trust backend local"
         "local"
         (runtime_trust |> member "runtime_contract" |> member "backend" |> to_string);
       Alcotest.(check int)
         "runtime trust pending approvals empty"
         0
         (runtime_trust |> member "pending_approval_count" |> to_int);
       Alcotest.(check string)
         "runtime trust disposition pass"
         "Pass"
         (runtime_trust |> member "disposition" |> to_string);
       Alcotest.(check bool)
         "runtime trust attention false"
         false
         (runtime_trust |> member "needs_attention" |> to_bool);
       Alcotest.(check string)
         "runtime trust approval idle"
         "idle"
         (runtime_trust |> member "approval" |> member "state" |> to_string);
       let sandbox_summary =
         runtime_trust |> member "execution" |> member "sandbox_summary" |> to_string
       in
       Alcotest.(check bool)
         "runtime trust execution sandbox summary mentions local"
         true
         (contains_substring sandbox_summary "local");
       ...
       Alcotest.(check bool)
         "runtime trust causal timeline non-empty"
         true
...
let test_keeper_status_ignores_stale_cascade_observation () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let current_labels = Keeper_model_labels.configured_model_labels_of_meta meta in
       ...
       Alcotest.(check (list string))
         ...
         (sorted_strings current_labels)
         (observability
          |> member "resolved_candidates"
          |> to_list
          |> List.map to_string
          |> sorted_strings);
       Alcotest.(check bool)
         "stale selected model not surfaced"
         true
         (observability
       ...
       Alcotest.(check string)
         "attempt summary resets to current config"
...
let test_keeper_down_accepts_agent_name_alias () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Alcotest.(check string)
         "down resolves canonical keeper name"
         keeper_name
...
let test_operator_keeper_probe_accepts_agent_name_alias () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       Alcotest.(check string)
         "probe delegates to keeper status"
         "masc_keeper_status"
       ...
       in
       Alcotest.(check string)
         "probe status resolves canonical keeper name"
         keeper_name
         Yojson.Safe.Util.(
           delegated_result |> member "status" |> member "name" |> to_string);
       Alcotest.(check bool)
         "probe includes diagnostic"
         true
...
let test_operator_keeper_recover_accepts_agent_name_alias () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       Alcotest.(check string)
         "recover delegates to keeper recover"
         "masc_keeper_recover"
         Yojson.Safe.Util.(action_json |> member "tool_name" |> to_string);
       let delegated_result =
         Yojson.Safe.Util.(action_json |> member "result" |> member "result")
       in
       Alcotest.(check bool)
         "recover path marked recoverable before action"
         true
         Yojson.Safe.Util.(
           delegated_result |> member "before" |> member "recoverable" |> to_bool);
       Alcotest.(check string)
         "recover down resolves canonical keeper name"
         keeper_name
         Yojson.Safe.Util.(
           delegated_result |> member "down" |> member "name" |> to_string);
       Alcotest.(check string)
         "recover up resolves canonical keeper name"
         keeper_name
       ...
       Alcotest.(check bool)
         "recover reports after diagnostic"
         true
         Yojson.Safe.Util.(delegated_result |> member "after" <> `Null);
       Alcotest.(check bool)
         "recover after keepalive running"
         true
         Yojson.Safe.Util.(
           delegated_result |> member "after" |> member "keepalive_running" |> to_bool))
;;
...
let test_keeper_list_scoped_to_current_base_path () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       Alcotest.(check (list string))
         "list only includes current base path keeper"
         [ keeper_name_a ]
         keeper_names;
       let listed_items = list_json |> member "items" |> to_list in
       Alcotest.(check int)
         "list item count scoped to current base path"
         1
         (List.length listed_items);
       Alcotest.(check string)
         "listed item name stays local"
         keeper_name_a
...
let test_keeper_status_does_not_cross_base_path () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Some (false, err) ->
         Alcotest.(check bool)
           "status reports keeper missing outside current base path"
           true
           (contains_substring err ("keeper not found: " ^ keeper_name))
       | Some (true, body) ->
...
let test_keeper_down_only_pauses_current_base_path () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let down_json = parse_json_exn body in
       Alcotest.(check string)
         "down returns scoped keeper name"
         keeper_name
         Yojson.Safe.Util.(down_json |> member "name" |> to_string);
       let meta_a =
       ...
       Alcotest.(check bool)
         "base path B keeper remains running"
         true
...
let test_keeper_status_schema_makes_name_optional () =
  let schema =
    List.find
      (fun (spec : Types.tool_schema) -> String.equal spec.name "masc_keeper_status")
      Tool_keeper.schemas
  in
...
|};
       ...
       Alcotest.(check bool)
         "unknown active goal names surfaced"
         true
       ...
       let open Yojson.Safe.Util in
       Alcotest.(check bool)
         "trigger_mode removed from config surface"
         true
         (json |> member "coordination" |> member "trigger_mode" = `Null);
       Alcotest.(check (list string))
         "active_goal_ids top-level"
         [ linked_goal.id ]
         (json |> member "active_goal_ids" |> to_list |> List.map to_string);
       Alcotest.(check (list string))
         "active_goal_ids in coordination"
         [ linked_goal.id ]
         (json
          |> member "coordination"
          |> member "active_goal_ids"
          |> to_list
          |> List.map to_string);
       Alcotest.(check string)
         "active goal title resolved"
         linked_goal.title
         (json
          |> member "coordination"
          |> member "active_goals"
          ...
          |> to_string);
       Alcotest.(check string)
         "runtime trust disposition surfaced"
         "Pass"
         (json |> member "runtime_trust" |> member "disposition" |> to_string);
       Alcotest.(check bool)
         "runtime paused from live meta"
         true
         (json |> member "runtime" |> member "paused" |> to_bool);
       Alcotest.(check bool)
         "proactive enabled from live meta"
         false
       ...
       Alcotest.(check string)
         "selected cascade name"
         Masc_mcp.Keeper_config.default_cascade_name
         (json |> member "execution" |> member "selected_cascade_name" |> to_string);
       Alcotest.(check string)
         "selected cascade canonical"
         Masc_mcp.Keeper_config.default_cascade_name
         (json |> member "execution" |> member "selected_cascade_canonical" |> to_string);
       let expected_default_models =
         Masc_mcp.Cascade_runtime.models_of_cascade_name
           Masc_mcp.Keeper_config.default_cascade_name
       in
       Alcotest.(check (list string))
         "selected cascade models use default profile"
         expected_default_models
         (json |> member "execution" |> member "models" |> to_list |> List.map to_string);
       Alcotest.(check bool)
         "per-provider timeout is null by default"
         true
         (json |> member "execution" |> member "per_provider_timeout_sec" = `Null);
       Alcotest.(check string)
         "per-provider timeout mode uses heuristic"
         "turn_budget_heuristic"
         (json |> member "execution" |> member "per_provider_timeout_mode" |> to_string);
       Alcotest.(check string)
         "cascade catalog source kind"
         "json"
         (json |> member "sources" |> member "cascade_catalog_source_kind" |> to_string);
       Alcotest.(check string)
         "cascade catalog source path"
         (Filename.concat config_dir "cascade.json")
         (json |> member "sources" |> member "cascade_catalog_source_path" |> to_string);
       Alcotest.(check string)
         "cascade runtime json path"
         (Filename.concat config_dir "cascade.json")
         (json |> member "sources" |> member "cascade_runtime_json_path" |> to_string);
       Alcotest.(check bool)
         "cascade runtime json editable"
         true
         (json |> member "sources" |> member "cascade_runtime_json_editable" |> to_bool);
       Alcotest.(check string)
         "active config root"
         config_dir
         (json |> member "sources" |> member "active_config_root" |> to_string);
       Alcotest.(check string)
         "active config root source"
         "env"
       ...
       let override_fields =
         json
         |> member "sources"
         |> member "override_fields"
         |> to_list
         |> List.map to_string
       in
       Alcotest.(check bool)
         "override field proactive"
         true
       ...
       let proactive_source =
         List.find_opt
           (fun item ->
              String.equal (item |> member "field" |> to_string) "proactive.enabled")
       ...
       Alcotest.(check bool)
         "override field live value"
         false
         (proactive_source |> member "live_value" |> to_bool);
       Alcotest.(check bool)
         "initiative surface removed"
         true
         (json |> member "initiative" = `Null);
       Alcotest.(check int)
         "total input tokens surfaced"
         1200
         (json |> member "metrics" |> member "total_input_tokens" |> to_int);
       Alcotest.(check int)
         "last latency surfaced"
         4000
         (json |> member "metrics" |> member "last_latency_ms" |> to_int);
       Alcotest.(check (option (float 0.001)))
         "last total tokens per sec surfaced"
         (Some 50.0)
         (json
          |> member "metrics"
          |> member "last_total_tokens_per_sec"
          |> to_float_option);
       Alcotest.(check (option (float 0.001)))
         "last output tokens per sec surfaced"
         (Some 20.0)
         (json
          |> member "metrics"
          |> member "last_output_tokens_per_sec"
          |> to_float_option);
       ...
       let prompt_source =
         json
         |> member "prompt"
         |> member "system_prompt_blocks"
         |> member "world"
         |> member "source"
         |> to_string
       in
       Alcotest.(check bool)
         "prompt block source surfaced"
         true
       ...
       in
       Alcotest.(check bool)
         "effective system prompt includes goal"
         true
         (contains_substring effective_system_prompt ("Goal: " ^ mutated.goal));
       Alcotest.(check bool)
         "effective system prompt includes world block"
         true
       ...
       Alcotest.(check string)
         "stale cascade raw name preserved"
         "vendor_mix_balanced"
         (stale_json |> member "execution" |> member "selected_cascade_name" |> to_string);
       Alcotest.(check string)
         "stale cascade falls back to live default"
         Masc_mcp.Keeper_config.default_cascade_name
         (stale_json
          |> member "execution"
          |> member "selected_cascade_canonical"
          |> to_string);
       Alcotest.(check (list string))
         "stale cascade models use live default"
         expected_default_models
         (stale_json
          |> member "execution"
          |> member "models"
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let assert_config_root label expected_rel expected_abs =
         ...
         Alcotest.(check (list string))
           (label ^ " effective allowed paths")
           [ expected_rel ]
           (json |> member "effective_allowed_paths" |> to_list |> List.map to_string)
       in
       let local_rel =
         Masc_mcp.Keeper_sandbox.host_root_rel_of_profile Keeper_types.Local keeper_name
       in
       ...
       Keeper_status_detail.invalidate_status_cache_for keeper_name;
       let docker_rel =
         Masc_mcp.Keeper_sandbox.host_root_rel_of_profile Keeper_types.Docker keeper_name
       in
       ...
       let docker_container_root = Masc_mcp.Keeper_sandbox.container_root keeper_name in
       Alcotest.(check string)
         "status private workspace root"
       ...
       Alcotest.(check string)
         "status sandbox host root preserved"
         docker_abs
...
let test_snapshot_keeper_tool_audit_fallback () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let tool_audit_source = keeper |> member "tool_audit_source" |> to_string_option in
       Alcotest.(check bool)
...
let test_keeper_msg_auto_execution_session_bridge () =
  ...
  if
    Sys.getenv_opt "MASC_RUN_LIVE_KEEPER_TEAM_SESSION_TEST" <> Some "1"
  ...
  then Alcotest.skip ()
    ...
    in
    if not local_runtime_available then Alcotest.skip () else ensure_fs env;
    if Masc_mcp.Local_runtime_pool.healthy_runtime_count () <= 0
    then Alcotest.skip ()
    else
      Eio.Switch.run
      @@ fun sw ->
        ...
        (fun () ->
             ...
             dispatch_keeper_exn
               ...
               ~args:
                 (`Assoc
                     ...
                     ; ( "goal"
                       , `String
                           "Start projected team sessions from explicit keeper messages" )
                     ; "proactive_enabled", `Bool false
                     ; "autoboot_enabled", `Bool false
                     ])
           in
           ...
           let ok, body =
             dispatch_keeper_exn
               ...
               ~args:
                 (`Assoc [ "name", `String keeper_name; "message", `String first_message ])
           in
           if not ok
           then (
             ...
             let body_has needle =
               ...
               let rec loop i =
                 if i + n_len > s_len
                 then false
                 else if String.sub body_lc i n_len = needle
                 then true
                 else loop (i + 1)
               in
               n_len = 0 || loop 0
             ...
             let status_json = parse_json_exn status_body in
             Alcotest.(check string)
               "status exposes auto team session removal"
               "removed"
               Yojson.Safe.Util.(
                 status_json
                 |> member "auto_execution_session"
                 |> member "status"
                 |> to_string);
             Alcotest.(check bool)
               "status exposes auto team session disabled"
               false
               Yojson.Safe.Util.(
                 status_json |> member "auto_execution_session_enabled" |> to_bool);
             Alcotest.(check bool)
               "status omits team session state"
               true
               Yojson.Safe.Util.(status_json |> member "execution_session_state" = `Null);
             Alcotest.(check bool)
               "status omits team session bridge"
               true
