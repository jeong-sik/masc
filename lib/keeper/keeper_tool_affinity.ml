...
let clamped_env_int ?(getenv = Sys.getenv_opt) ~name ~min_val ~max_val ~default () =
  ...
  | Some s when String.trim s <> "" ->
    max min_val (min max_val (Safe_ops.int_of_string_with_default ~default s))
  | Some _ | None -> default
;;
...
let unix_of_iso8601 (s : string) : float option =
  try
    Scanf.sscanf s "%d-%d-%dT%d:%d:%d" (fun year mon day hour minute sec ->
      let tm : Unix.tm =
        { tm_sec = sec
        ; tm_min = minute
        ; tm_hour = hour
  ...
  then []
    ...
    |> List.filter_map (fun (s : Trajectory.tool_stat) ->
      ...
      then None
        ...
        let score = float_of_int s.call_count *. success_rate *. recency_weight in
        Some { tool_name = s.name; score; call_count = s.call_count; success_rate }))
...
let pre_populate_from_history
  ...
  then []
    ...
    let entries = Trajectory.read_entries_since ~masc_root ~keeper_name ~since in
    match entries with
    | [] -> []
    | _ ->
      ...
      let filtered_stats =
        ...
        |> List.filter (fun (s : Trajectory.tool_stat) ->
          Hashtbl.mem allowed_set s.name && not (Hashtbl.mem core_set s.name))
      in
      let affinity = compute_affinity ~tool_stats:filtered_stats ~now ~max_k in
      let names = List.map (fun a -> a.tool_name) affinity in
      if names <> [] then Keeper_discovered_tools.add discovered ~turn:0 ~names;
      affinity)
;;
