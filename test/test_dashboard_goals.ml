...
let string_list_field json field = json |> member field |> to_list |> List.map to_string
...
let rewrite_goal_updated_at config ~goal_id ~updated_at =
  ...
  let goals =
    ...
    |> List.map (fun (goal : Goal_store.goal) ->
      if String.equal goal.id goal_id
      then { goal with created_at = updated_at; updated_at }
      else goal)
  in
...
let test_goal_tree_surfaces_resolved_verification_evidence () =
  ...
  let goal, _kind =
    match
      Goal_store.upsert_goal
        config
        ~title:"Resolved verification evidence"
        ~verifier_policy
        ()
    with
    | Ok payload -> payload
  ...
  check
    bool
    "open request cleared in dashboard tree"
    true
    (summary |> member "open_request" = `Null);
  let latest_request = summary |> member "latest_request" in
  check
    string
    "latest request id retained in dashboard tree"
    request_id
    (latest_request |> member "id" |> to_string);
  check
    string
    "latest request approved in dashboard tree"
    "approved"
  ...
  check
    string
    "vote note surfaced in dashboard tree"
    "checked receipt and tests"
    (vote |> member "note" |> to_string);
  check
    (list string)
    "vote evidence surfaced in dashboard tree"
  ...
  | Ok detail_json ->
    ...
    check
      string
      "latest request retained in goal detail"
      request_id
...
let test_empty_executing_goal_is_at_risk () =
  ...
  check string "empty goal health" "at_risk" (node |> member "health" |> to_string);
  check
    string
    "empty goal blocker"
    "goal_linkage"
    (node |> member "blocking_source" |> to_string);
  check int "linkage warning count" 1 (node |> member "linkage_warning_count" |> to_int);
  check
    int
    "active summary counts status"
    1
    (json |> member "summary" |> member "active_goals" |> to_int);
  check
    int
    "on_track summary separate"
    0
...
let test_open_task_without_keeper_is_at_risk () =
  ...
  ignore
    (Coord_task.add_task
       ~goal_id:goal.id
       config
       ~title:"Unstaffed task"
       ~priority:3
       ~description:"needs a keeper");
  let node = Dashboard_goals.dashboard_goals_tree_json ~config |> root_node in
  check string "unstaffed health" "at_risk" (node |> member "health" |> to_string);
  check
    string
    "unstaffed blocker"
    "goal_linkage"
...
let test_cancelled_only_goal_is_at_risk () =
  ...
  let task_id =
    ...
    | [ task ] -> task.id
    | tasks -> fail (Printf.sprintf "expected one task, got %d" (List.length tasks))
  in
  (match
     Coord.cancel_task_r config ~agent_name:"planner" ~task_id ~reason:"test cancellation"
   with
   | Ok _ -> ()
   | Error err -> fail (Masc_domain.masc_error_to_string err));
  let node = Dashboard_goals.dashboard_goals_tree_json ~config |> root_node in
  check string "cancelled-only health" "at_risk" (node |> member "health" |> to_string);
  check
    string
    "cancelled-only blocker"
    "goal_linkage"
    (node |> member "blocking_source" |> to_string);
  check int "linkage warning count" 1 (node |> member "linkage_warning_count" |> to_int)
;;
...
let test_title_marker_links_legacy_task () =
  ...
  ignore
    (Coord_task.add_task
       ...
       ~priority:3
       ~description:"legacy title marker");
  let node = Dashboard_goals.dashboard_goals_tree_json ~config |> root_node in
  let task =
...
let test_goal_attainment_projects_percent_target () =
  ...
  let attainment =
    Dashboard_goals.dashboard_goals_tree_json ~config |> root_node |> member "attainment"
  in
  check string "attainment state" "attained" (attainment |> member "state" |> to_string);
  check
    string
    "attainment basis"
  ...
  check int "target-relative pct" 100 (attainment |> member "attainment_pct" |> to_int);
  check
    (float 0.001)
    "observed percent"
    75.0
  ...
  check int "done task count" 3 (attainment |> member "task_done_count" |> to_int)
;;
...
let test_goal_attainment_exports_prometheus_metric () =
  ...
  let goal, _kind =
    match
      Goal_store.upsert_goal
        config
        ~title:"Prometheus attainment goal"
        ~metric:"completion_pct"
        ~target_value:"75%"
        ()
    with
  ...
  check
    (option (float 0.001))
    "goal attainment pct metric"
    (Some 100.0)
    (Prometheus.get_metric_value Prometheus.metric_goal_attainment_pct ~labels ());
  check
    (option (float 0.001))
    "goal attainment measured metric"
    (Some 1.0)
    (Prometheus.get_metric_value Prometheus.metric_goal_attainment_measured ~labels ())
;;
...
let test_goal_attainment_projects_camel_case_percent_metric () =
  ...
  let goal, _kind =
    match
      Goal_store.upsert_goal
        config
        ~title:"Camel percent metric goal"
        ~metric:"successRate"
        ~target_value:"75"
        ()
    with
  ...
  check
    ...
    "metric_target_percent"
    (attainment |> member "basis" |> to_string);
  check string "camel percent state" "attained" (attainment |> member "state" |> to_string);
  check
    (float 0.001)
    "camel percent target"
    75.0
    (attainment |> member "target_numeric" |> to_float)
;;
...
let test_goal_attainment_does_not_fake_unparseable_target () =
  ...
  let attainment =
    Dashboard_goals.dashboard_goals_tree_json ~config |> root_node |> member "attainment"
  in
  check string "unmeasured state" "unmeasured" (attainment |> member "state" |> to_string);
  check string "unmeasured basis" "unmeasured" (attainment |> member "basis" |> to_string);
  check
    string
    "unparseable target"
    "unparseable"
    (attainment |> member "target_parse_status" |> to_string);
  check bool "attainment pct omitted" true (attainment |> member "attainment_pct" = `Null);
  check int "evidence task retained" 1 (attainment |> member "task_done_count" |> to_int)
;;
...
let test_goal_attainment_rejects_non_finite_target () =
  ...
  let goal, _kind =
    match
      Goal_store.upsert_goal
        config
        ~title:"Non-finite target goal"
        ~metric:"completion_pct"
        ~target_value:huge_decimal
        ()
    with
    | Ok payload -> payload
    | Error msg -> fail msg
  ...
  check
    string
    "non-finite target stays unmeasured"
    "unmeasured"
    (attainment |> member "state" |> to_string);
  check
    string
    "non-finite target is unparseable"
    "unparseable"
    (attainment |> member "target_parse_status" |> to_string);
  check
    bool
    "non-finite target numeric omitted"
    true
    (attainment |> member "target_numeric" = `Null);
  check
    bool
    "non-finite target pct omitted"
    true
...
let test_goal_attainment_parses_grouped_count_target () =
  ...
  let goal, _kind =
    match
      Goal_store.upsert_goal
        config
        ~title:"Grouped count target goal"
        ~metric:"task_count"
        ~target_value:"1,000 tasks"
        ()
    with
    | Ok payload -> payload
    | Error msg -> fail msg
  ...
  check
    string
    "grouped target parse status"
    "parseable"
    (attainment |> member "target_parse_status" |> to_string);
  check
    (float 0.001)
    "grouped target numeric"
    1000.0
    (attainment |> member "target_numeric" |> to_float);
  check int "grouped target pct" 0 (attainment |> member "attainment_pct" |> to_int);
...
let test_goal_attainment_parses_range_target_start () =
  ...
  let goal, _kind =
    match
      Goal_store.upsert_goal
        config
        ~title:"Range percent target goal"
        ~metric:"completion_pct"
        ~target_value:"75-80%"
        ()
    with
  ...
  check
    string
    "range target parse status"
    "parseable"
    (attainment |> member "target_parse_status" |> to_string);
  check
    (float 0.001)
    "range target numeric"
    75.0
    (attainment |> member "target_numeric" |> to_float);
  check int "range target pct" 100 (attainment |> member "attainment_pct" |> to_int)
;;
...
let test_goal_attainment_rejects_substring_pr_metric () =
  ...
  let goal, _kind =
    match
      Goal_store.upsert_goal
        config
        ~title:"Approval latency target goal"
  ...
  check
    string
    "substring pr metric remains unmeasured"
    "unmeasured"
    (attainment |> member "state" |> to_string);
  check
    string
    "substring pr metric unsupported"
    "unsupported_metric"
    (attainment |> member "target_parse_status" |> to_string);
  check
    (float 0.001)
    "unsupported target retained"
    5.0
    (attainment |> member "target_numeric" |> to_float);
  check
    bool
    "unsupported metric has no pct"
    true
    (attainment |> member "attainment_pct" = `Null)
;;
...
let test_goal_attainment_camel_case_percent_metric_is_percent () =
  ...
  let goal, _kind =
    match
      Goal_store.upsert_goal
        config
        ~title:"Camel-case percent metric"
        ~metric:"successRate"
        ~target_value:"80"
        ()
    with
  ...
  check
    string
    "camelCase metric inferred as percent target"
    "metric_target_percent"
    (attainment |> member "basis" |> to_string);
  check
    string
    "camelCase metric unit is percent"
    "percent"
...
let test_goal_attainment_rejects_non_finite_target_nan_token () =
  ...
  check
    string
    "non-finite target parse status"
    "unparseable"
    (attainment |> member "target_parse_status" |> to_string);
  check
    bool
    "non-finite target has no numeric value"
    true
    (attainment |> member "target_numeric" = `Null);
  check
    bool
    "non-finite target has no pct"
    true
    (attainment |> member "attainment_pct" = `Null)
;;
...
let test_goal_attainment_acronym_pascal_case_percent_metric_is_percent () =
  ...
  let goal, _kind =
    match
      Goal_store.upsert_goal
        config
        ~title:"Acronym-prefixed percent metric"
        ~metric:"APIRatio"
        ~target_value:"80"
        ()
    with
  ...
  check
    string
    "acronym-prefix metric inferred as percent target"
    "metric_target_percent"
    (attainment |> member "basis" |> to_string);
  check
    string
    "acronym-prefix metric unit is percent"
    "percent"
    (attainment |> member "unit" |> to_string)
;;
...
let test_blocked_phase_projects_blocked_health () =
  ...
  check
    string
    "legacy status remains paused"
    "paused"
    (node |> member "status" |> to_string);
  check string "phase remains blocked" "blocked" (node |> member "phase" |> to_string);
  check
    string
    "health follows blocked phase"
    "blocked"
    (node |> member "health" |> to_string);
  let fsm = node |> member "goal_fsm" in
  check
    string
    "goal fsm source is explicit phase"
    "goal.phase"
    (fsm |> member "source" |> to_string);
  check
    string
    "goal fsm state follows phase"
    "blocked"
    (fsm |> member "state" |> to_string);
  check
    string
    "goal fsm kind is blocked"
    "blocked"
    (fsm |> member "state_kind" |> to_string);
  check
    (list string)
    "blocked goal next actions"
...
let test_goal_fsm_withholds_stalled_when_activity_is_metadata_only () =
  ...
  let goal, _kind =
    ...
    | Error msg -> fail msg
  in
  rewrite_goal_updated_at config ~goal_id:goal.id ~updated_at:"2026-04-01T00:00:00Z";
  ...
  check
    string
    "phase remains the lifecycle source"
    "executing"
    (node |> member "phase" |> to_string);
  check
    string
    "metadata-only activity is not a runtime blocker"
    "none"
    (node |> member "blocking_source" |> to_string);
  check
    string
    "metadata-only activity keeps health on track"
    "on_track"
    (node |> member "health" |> to_string);
  check
    bool
    "stalled badge withheld"
    false
    (List.mem "stalled" (string_list_field node "badges"));
  check
    bool
    "unobserved badge explains weak freshness"
    true
    (List.mem "activity_unobserved" (string_list_field node "badges"));
  check
    string
    "activity observation is metadata only"
    "goal_metadata"
    (node |> member "activity_observation" |> to_string);
  check
    string
    "stagnation status is unobserved"
    "unobserved"
  ...
  check
    string
    "goal fsm state remains executing"
    "executing"
    (fsm |> member "state" |> to_string);
  check
    string
    "goal fsm carries observation confidence"
    "goal_metadata"
    (fsm |> member "activity_observation" |> to_string);
  check
    string
    "goal fsm carries unobserved freshness"
    "unobserved"
    (fsm |> member "stagnation_status" |> to_string);
  check
    bool
    "operator can still pause executing goal"
    true
...
let test_goal_detail_surfaces_keeper_runtime_trust_and_blockers () =
  ...
  check
    string
    "blocking source defaults to none"
    "none"
    (node |> member "blocking_source" |> to_string);
  check
    string
    "latest keeper ref surfaced"
    meta.name
    (node |> member "latest_keeper_ref" |> to_string);
  check int "latest turn ref surfaced" 7 (node |> member "latest_turn_ref" |> to_int);
  match Dashboard_goals.goal_detail_json ~config ~goal_id:goal.id with
  | Error msg -> fail msg
  | Ok json ->
    let linked_keeper =
...
let test_goal_detail_uses_receipt_disposition_for_required_tool_failure () =
  ...
  | Ok json ->
    ...
    check
      string
      "receipt-derived disposition pauses"
      "Pause"
      (runtime_trust |> member "disposition" |> to_string);
    check
      string
      "receipt-derived reason surfaced"
      "tool_required_unsatisfied"
      (runtime_trust |> member "disposition_reason" |> to_string);
    check
      string
      "operator disposition preserved"
      "pause_human"
      (runtime_trust |> member "operator_disposition" |> to_string);
    check
      string
      "operator reason preserved"
      "tool_required_unsatisfied"
      (runtime_trust |> member "operator_disposition_reason" |> to_string);
    check
      bool
      "receipt-derived failure needs attention"
      true
...
let test_goal_tree_tolerates_null_decision_telemetry () =
  ...
  | Ok json ->
    ...
    check
      (option string)
      "null telemetry selected_model stays absent"
      None
      (linked_keeper
...
let test_goal_detail_does_not_promote_synthetic_blocker_over_receipt () =
  ...
  | Ok json ->
    ...
    check
      string
      "durable receipt remains latest causal event"
      "execution_receipt"
      (runtime_trust |> member "latest_causal_event" |> member "kind" |> to_string);
    ...
    let blocker =
      ...
      |> List.find_opt (fun event ->
        String.equal "runtime_blocker" (event |> member "kind" |> to_string))
    in
    (match blocker with
     ...
     | Some event ->
       check
         bool
         "runtime blocker marked observation-only"
         true
...
let test_goal_detail_promotes_newer_runtime_blocker_over_stale_receipt () =
  ...
  | Ok json ->
    ...
    in
    let runtime_trust = linked_keeper |> member "runtime_trust" in
    let terminal_reason = runtime_trust |> member "latest_terminal_reason" in
    check
      string
      "newer blocker drives disposition"
      "Alert"
      (runtime_trust |> member "disposition" |> to_string);
    check
      string
      "newer blocker drives operator disposition"
      "alert_exhausted"
      (runtime_trust |> member "operator_disposition" |> to_string);
    check
      string
      "timeout blocker keeps attention reason"
      "timeout_budget_exhausted"
      (runtime_trust |> member "attention_reason" |> to_string);
    check
      string
      "latest terminal reason comes from blocker"
      "runtime_blocker"
      (terminal_reason |> member "source" |> to_string);
    check
      string
      "latest terminal reason is timeout budget"
      "oas_timeout_budget"
      (terminal_reason |> member "code" |> to_string);
    check
      string
      "latest causal event follows runtime blocker"
      "runtime_blocker"
      (runtime_trust |> member "latest_causal_event" |> member "kind" |> to_string)
;;
...
let test_goal_detail_keeps_decision_terminal_reason_over_newer_blocker () =
  ...
  let goal, _kind =
    ...
    | Error msg -> fail msg
  in
  let meta = make_keeper_meta ~name:"decision-over-blocker-keeper" ~goal_id:goal.id in
  (match Keeper_types.write_meta ~force:true config meta with
   | Ok () -> ()
   | Error err -> fail ("write_meta failed: " ^ err));
  append_keeper_receipt config meta;
  ...
  | Ok json ->
    ...
    in
    let terminal_reason =
      linked_keeper |> member "runtime_trust" |> member "latest_terminal_reason"
    in
    check
      string
      "decision terminal reason keeps precedence"
...
let test_goal_detail_derives_attention_from_receipt_disposition () =
  ...
  (match Keeper_types.write_meta ~force:true config meta with
   | Ok () -> ()
   | Error err -> fail ("write_meta failed: " ^ err));
  append_keeper_receipt ~tool_contract_result:"needs_execution_progress" config meta;
  match Dashboard_goals.goal_detail_json ~config ~goal_id:goal.id with
  | Error msg -> fail msg
  | Ok json ->
    let linked_keeper =
    ...
    check
      string
      "receipt disposition pauses"
      "Pause"
      (runtime_trust |> member "disposition" |> to_string);
    check
      string
      "receipt disposition fills attention reason"
      "tool_required_unsatisfied"
      (runtime_trust |> member "attention_reason" |> to_string);
    check
      string
      "receipt terminal reason follows operator disposition"
      "required_tool_use_unsatisfied"
      (terminal_reason |> member "code" |> to_string);
    check
      string
      "receipt terminal reason source"
      "execution_receipt"
      (terminal_reason |> member "source" |> to_string);
    check
      string
      "next action follows terminal reason"
...
let () =
  run
    ...
    [ ( "tree"
      , [ test_case
            "resolved goal verification evidence is surfaced"
            `Quick
            test_goal_tree_surfaces_resolved_verification_evidence
        ; test_case
            "empty executing goal is at risk"
            `Quick
            test_empty_executing_goal_is_at_risk
        ; test_case
            "open task without keeper is at risk"
            `Quick
            test_open_task_without_keeper_is_at_risk
        ; test_case
            "cancelled-only goal is at risk"
            `Quick
            test_cancelled_only_goal_is_at_risk
        ; test_case
            "title marker links legacy task"
            `Quick
            test_title_marker_links_legacy_task
        ; test_case
            "goal attainment projects percent targets"
            `Quick
            test_goal_attainment_projects_percent_target
        ; test_case
            "goal attainment exports prometheus metric"
            `Quick
            test_goal_attainment_exports_prometheus_metric
        ; test_case
            "goal attainment projects camel-case percent metrics"
            `Quick
            test_goal_attainment_projects_camel_case_percent_metric
        ; test_case
            "goal attainment does not fake unparseable targets"
            `Quick
            test_goal_attainment_does_not_fake_unparseable_target
        ; test_case
            "goal attainment rejects non-finite targets"
            `Quick
            test_goal_attainment_rejects_non_finite_target
        ; test_case
            "goal attainment parses grouped count targets"
            `Quick
            test_goal_attainment_parses_grouped_count_target
        ; test_case
            "goal attainment parses range target start"
            `Quick
            test_goal_attainment_parses_range_target_start
        ; test_case
            "goal attainment rejects substring pr metrics"
            `Quick
            test_goal_attainment_rejects_substring_pr_metric
        ; test_case
            "goal attainment handles camelCase percent metric"
            `Quick
            test_goal_attainment_camel_case_percent_metric_is_percent
        ; test_case
            "goal attainment handles acronym-prefix percent metric"
            `Quick
            test_goal_attainment_acronym_pascal_case_percent_metric_is_percent
        ; test_case
            "goal attainment rejects non-finite target (nan token)"
            `Quick
            test_goal_attainment_rejects_non_finite_target_nan_token
        ; test_case
            "blocked phase maps to blocked health"
            `Quick
        ...
        ; test_case
            "goal detail surfaces keeper runtime trust and blockers"
            `Quick
            test_goal_detail_surfaces_keeper_runtime_trust_and_blockers
        ; test_case
            "goal detail pauses on required tool receipt failure"
            `Quick
            test_goal_detail_uses_receipt_disposition_for_required_tool_failure
        ; test_case
            "goal tree tolerates null decision telemetry"
            `Quick
