...
let calibration = { samples = []; correction_factor = 1.0 }
  ...
  then (
    let log_event =
      Mutex.protect calibration_lock (fun () ->
        ...
        if List.length calibration.samples > 10
        then calibration.samples <- List.filteri (fun i _ -> i < 10) calibration.samples;
        let ratios =
          List.filter_map
            (fun (e, a) ->
    ...
    | Some (new_factor, prev_factor, n) ->
      if new_factor > 1.5 || new_factor < 0.5
      then
        Log.warn
          ~ctx:"relay"
          ...
          prev_factor
          n
      else if abs_float (new_factor -. prev_factor) > 0.1
      then
        Log.debug
          ~ctx:"relay"
...
let tokens_per_user_msg () =
  Runtime_params.get Governance_registry.relay_tokens_per_user_msg
;;

let tokens_per_assistant_msg () =
  Runtime_params.get Governance_registry.relay_tokens_per_assistant_msg
;;

let tokens_per_tool_call () =
  Runtime_params.get Governance_registry.relay_tokens_per_tool_call
;;

let tokens_per_tool_result () =
  Runtime_params.get Governance_registry.relay_tokens_per_tool_result
;;
...
let estimate_context ~messages ~tool_calls ~model =
  let message_tokens =
    messages * (tokens_per_user_msg () + tokens_per_assistant_msg ())
  in
...
let estimate_task_cost = function
  ...
  | Multi_file_edit n ->
    n * Runtime_params.get Governance_registry.relay_cost_per_file_edit
...
let should_relay_proactive ~config ~metrics ~task_hint =
  ...
  let predicted_ratio =
    float_of_int predicted_tokens /. float_of_int metrics.max_tokens
  in
...
let should_relay ~config ~metrics = metrics.usage_ratio >= config.threshold
...
let should_relay_smart ~config ~metrics ~task_hint =
  ...
  if proactive && not reactive
  then `Proactive (* Relay NOW before task consumes more *)
  else if reactive
  then `Reactive (* Already at limit *)
  else `No_relay (* Safe to continue *)
;;
    ...
    | progress ->
      let prog_str =
        String.concat
          "\n"
          (List.map
             (fun (gid, pct) -> Printf.sprintf "- %s: %.0f%%" gid (pct *. 100.0))
             progress)
      in
      ("## Goal Progress\n" ^ prog_str) :: sections
  in
...
let build_handoff_prompt ~payload ~generation =
  let header =
    Printf.sprintf
       ...
       **IMPORTANT**: Continue the work seamlessly. The user should not notice the \
       transition.\n\n"
      generation
  in
  let context =
...
let payload_to_json payload =
  `Assoc
    ...
    ; ( "goal_progress"
      , `List
          (List.map
             (fun (gid, pct) -> `List [ `String gid; `Float pct ])
