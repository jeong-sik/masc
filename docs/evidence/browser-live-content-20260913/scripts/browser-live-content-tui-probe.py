...
report={'scope':'isolated three-channel website; real live Firefox extension and native host; experimental source Skill loaded by existing native binary; persistent native TUI','steps':[],'cleanup':[]}
...
try:
 ...
 composition=args.checkout.resolve()/'skills/browser-live-click-content'
 ...
 installed_hashes={str(p.relative_to(installed)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted(installed.rglob('*')) if p.is_file()}
 assert installed_hashes=={str(f.relative_to(composition)):hashlib.sha256(f.read_bytes()).hexdigest() for f in composition.rglob('*') if f.is_file()}, 'installed experimental composition differs from reviewed source'
 report['composition_source_commit']=subprocess.check_output(['git','-C',str(args.checkout),'rev-parse','HEAD'],text=True).strip()
 report['composition_distribution']='experimental source file, not exported by this server binary'
 shutil.copytree(composition,OUT/'experimental-skill')
 ...
 spec=importlib.util.spec_from_file_location('clipboard_capture','/tmp/capture-live-content-browser.py')
