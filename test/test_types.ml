...
let test_task_status_done () =
  let status =
    Done
      { assignee = "gemini"; completed_at = "2024-01-01T00:00:00Z"; notes = Some "test" }
  in
  let json = task_status_to_yojson status in
  match task_status_of_yojson json with
...
let test_agent_status_witness_in_enum () =
  let witness s =
    ...
    if not (List.mem actual valid_agent_status_strings)
    then
...
let test_status_strings_match_variant_witness () =
  let witness s =
    ...
    if not (List.mem actual valid_task_status_strings)
    then Alcotest.failf "task_status_to_string %S not in valid_task_status_strings" actual
  in
  witness Todo;
...
let test_awaiting_verification_in_enum () =
  Alcotest.(check bool)
    "awaiting_verification present"
    true
    (List.mem "awaiting_verification" valid_task_status_strings)
;;
...
let test_backlog_parse_null_metadata_fields () =
  let json = `Assoc [ "tasks", `List []; "last_updated", `Null; "version", `Null ] in
  match backlog_of_yojson json with
  | Ok b ->
    Alcotest.(check string) "null last_updated -> empty" "" b.last_updated;
    Alcotest.(check int) "null version -> 1" 1 b.version
  | Error msg -> Alcotest.fail ("expected Ok with null metadata, got Error: " ^ msg)
;;
...
let test_backlog_parse_live_shape_with_null_optional_nested_fields () =
  ...
  match backlog_of_yojson json with
  | Error msg ->
    Alcotest.fail ("expected Ok for live-shaped backlog payload, got Error: " ^ msg)
  | Ok backlog ->
    Alcotest.(check int) "one live-shaped task parsed" 1 (List.length backlog.tasks);
    let task = List.hd backlog.tasks in
    Alcotest.(check string)
      "last_updated defaulted to empty for live payload"
      ""
      backlog.last_updated;
    Alcotest.(check int) "version defaulted to 1 for live payload" 1 backlog.version;
    (match task.handoff_context with
     | None -> Alcotest.fail "expected handoff_context"
     | Some handoff ->
       Alcotest.(check (option string)) "reason null -> None" None handoff.reason;
       Alcotest.(check (option string)) "next_step null -> None" None handoff.next_step;
       Alcotest.(check (option string))
         "failure_mode null -> None"
         None
    ...
    (match task.contract with
     ...
     | Some contract ->
       ...
       Alcotest.(check (option string))
         "autoresearch_loop_id null -> None"
         None
...
let () =
  Alcotest.run
    ...
    ; "message", [ Alcotest.test_case "roundtrip" `Quick test_message_roundtrip ]
    ; ( "timestamp"
      , [ Alcotest.test_case "parse utc epoch" `Quick test_parse_iso8601_epoch_utc ] )
    ; ( "backlog_lenient_parse"
      , [ Alcotest.test_case
            "missing last_updated/version -> defaults"
            `Quick
            test_backlog_parse_missing_metadata_fields
        ; Alcotest.test_case
            "null last_updated/version -> defaults"
            `Quick
            test_backlog_parse_null_metadata_fields
        ; Alcotest.test_case
            "live shape with nested null optionals"
            `Quick
    ...
    ; ( "task_action_lenient"
      , [ Alcotest.test_case "alias claimed -> claim" `Quick test_action_alias_claimed
        ...
        ; Alcotest.test_case
            "alias in_progress -> start"
            `Quick
            test_action_alias_in_progress
        ; Alcotest.test_case "alias completed -> done" `Quick test_action_alias_completed
        ; Alcotest.test_case
            "alias cancelled -> cancel"
            `Quick
            test_action_alias_cancelled
        ; Alcotest.test_case
            "canonical claim still works"
            `Quick
            test_action_canonical_still_works
        ; Alcotest.test_case "case insensitive" `Quick test_action_case_insensitive
        ; Alcotest.test_case
            "garbage still rejected"
            `Quick
            test_action_unknown_still_rejected
        ; Alcotest.test_case
            "strict parser ssot preserved"
            `Quick
            test_strict_parser_unchanged
        ] )
    ; ( "agent_status_ssot"
      , [ Alcotest.test_case
            "witness covers all variants"
            `Quick
            test_agent_status_witness_in_enum
        ; Alcotest.test_case
            "all 4 strings present"
            `Quick
            test_agent_status_strings_complete
        ] )
    ; ( "variant_ssot"
      , [ Alcotest.test_case
            "status strings match witness"
            `Quick
            test_status_strings_match_variant_witness
        ; Alcotest.test_case
            "awaiting_verification in enum"
            `Quick
            test_awaiting_verification_in_enum
        ; Alcotest.test_case
            "actions enum has verification actions"
            `Quick
            test_actions_enum_has_verification_actions
        ] )
    ; ( "agent_role_ssot"
      , [ Alcotest.test_case "witness covers all variants" `Quick (fun () ->
            ...
            let witness s =
              ...
              if not (List.mem actual valid_agent_role_strings)
              then
                Alcotest.failf
                  "agent_role_to_string %S not in valid_agent_role_strings"
                  actual
            in
            witness Worker;
            witness Admin;
    ...
    ; ( "tool_preset_ssot"
      , [ (* Issue #8430: witness covers all 8 variants — adding a 9th
         ...
         tool_preset_to_string. *)
          Alcotest.test_case "witness covers all variants" `Quick (fun () ->
            ...
            let witness s =
              ...
              if not (List.mem actual valid_tool_preset_strings)
              then
                Alcotest.failf
                  "tool_preset_to_string %S not in valid_tool_preset_strings"
                  actual
            in
            witness Minimal;
            witness Social;
        ...
        ; Alcotest.test_case "tool_access schema mirror stays in sync" `Quick (fun () ->
            ...
            in
            Alcotest.(check (list string))
              "tool_access enum == variant SSOT"
              Masc_mcp.Keeper_types.valid_tool_preset_strings
              enum)
        ; Alcotest.test_case
            "room signal defaults follow keeper tool access"
            `Quick
            (fun () ->
               let open Masc_mcp.Keeper_types in
               let check_access label expected access =
                 Alcotest.(check bool)
                   label
                   expected
                   (tool_access_default_room_signal_prompt_enabled ~default:false access)
               in
               check_access
                 "minimal stays quiet by default"
                 false
                 (Preset { preset = Minimal; also_allow = [] });
               check_access
                 "minimal board opt-in listens"
                 true
                 (Preset { preset = Minimal; also_allow = [ "keeper_board_post" ] });
               check_access
                 "minimal fake keeper board prefix stays quiet"
                 false
                 (Preset { preset = Minimal; also_allow = [ "keeper_board_fake" ] });
               check_access
                 "delivery listens to board by default"
                 true
                 (Preset { preset = Delivery; also_allow = [] });
               check_access
                 "messaging listens to board by default"
                 true
                 (Preset { preset = Messaging; also_allow = [] });
               check_access
                 "custom without board stays quiet"
                 false
                 (Custom [ "keeper_time_now" ]);
               check_access
                 "custom masc board allowlist listens"
                 true
                 (Custom [ "masc_board_post" ]);
               check_access
                 "custom fake masc board prefix stays quiet"
                 false
                 (Custom [ "masc_board_fake" ]);
               check_access
                 "custom board allowlist listens"
                 true
      ...
      , [ (* Issue #8471: tool_operator view enum was missing 'sessions'
         ...
         drift by deriving the asserted list from the Variant SSOT. *)
          Alcotest.test_case "witness covers all 5 variants" `Quick (fun () ->
            ...
            let witness v =
              ...
              if not (List.mem actual valid_snapshot_view_strings)
              then
                Alcotest.failf
                  "snapshot_view_to_string %S not in valid_snapshot_view_strings"
                  actual
            in
            witness Summary;
            witness Sessions;
        ...
        ; Alcotest.test_case "of_string_opt rejects garbage" `Quick (fun () ->
            ...
            Alcotest.(check bool)
              "sessions accepted"
              true
              (Masc_mcp.Operator_control_snapshot.snapshot_view_of_string_opt "sessions"
               <> None))
        ] )
    ; ( "keeper_profile_enum_ssot"
      , [ (* Issue #8467: witness exhaustiveness for [Keeper_types_profile]
         ...
         in the matching [*_to_string] function. *)
          Alcotest.test_case
            "sandbox_profile witness covers both variants"
            `Quick
            (fun () ->
               let open Masc_mcp.Keeper_types_profile in
               let witness s =
                 let actual = sandbox_profile_to_string s in
                 if not (List.mem actual valid_sandbox_profile_strings)
                 then
                   Alcotest.failf
                     "sandbox_profile_to_string %S not in valid_sandbox_profile_strings"
                     actual
               in
               witness Local;
               witness Docker;
        ...
        ; Alcotest.test_case "cmd_targets_git_or_gh dispatch predicate" `Quick (fun () ->
            ...
            Alcotest.(check bool)
              "git substring is not git command"
              false
              (p "git-foo bar");
            Alcotest.(check bool) "github CLI other binary" false (p "github-cli pr list"))
        ; Alcotest.test_case "network_mode witness covers both variants" `Quick (fun () ->
            let open Masc_mcp.Keeper_types_profile in
            let witness s =
              let actual = network_mode_to_string s in
              if not (List.mem actual valid_network_mode_strings)
              then
                Alcotest.failf
                  "network_mode_to_string %S not in valid_network_mode_strings"
                  actual
            in
            witness Network_none;
            witness Network_inherit;
            Alcotest.(check int) "count" 2 (List.length valid_network_mode_strings))
        ; Alcotest.test_case
            "shared_memory_scope witness covers both variants"
            `Quick
            (fun () ->
               let open Masc_mcp.Keeper_types_profile in
               let witness s =
                 let actual = shared_memory_scope_to_string s in
                 if not (List.mem actual valid_shared_memory_scope_strings)
                 then
                   Alcotest.failf
                     "shared_memory_scope_to_string %S not in \
                      valid_shared_memory_scope_strings"
                     actual
               in
    ...
    ; ( "admin_section_ssot"
      , [ (* Issue #8546: schema advertised [auth; unit_policy] while the
         ...
         dispatcher error message derive from it. *)
          Alcotest.test_case
            "SSOT contains exactly the implemented sections"
            `Quick
            (fun () ->
               Alcotest.(check (list string))
                 "auth only"
        ...
        ; Alcotest.test_case "schema mirror stays in sync" `Quick (fun () ->
            Alcotest.(check (list string))
              "tool_schemas mirror == SSOT"
              Masc_mcp.Tool_misc_admin.valid_admin_section_strings
              Tool_schemas_misc.admin_section_enum_strings)
        ] )
    ; ( "config_category_ssot"
      , [ Alcotest.test_case
            "producer helper matches all_categories order"
            `Quick
            (fun () ->
               Alcotest.(check (list string))
                 "all_categories -> names"
        ...
        ; Alcotest.test_case
            "missing runtime categories are now published"
            `Quick
            (fun () ->
      ...
      , [ (* Issue #8474: schema transition matrix had drifted from
         ...
         published [task_fsm_transitions] list. *)
          Alcotest.test_case "every valid action appears in transitions" `Quick (fun () ->
            ...
            List.iter
              (fun action ->
                 if not (List.mem action published_actions)
                 then
                   Alcotest.failf
                     "action %S valid per Coord_task.valid_next_actions_for_status but \
                      missing from Mcp_server.task_fsm_transitions"
                     action)
              valid_actions)
        ; Alcotest.test_case "verifier-FSM transitions present" `Quick (fun () ->
            let actions =
              Masc_mcp.Mcp_server.task_fsm_transitions |> List.map (fun (a, _, _, _) -> a)
            in
            List.iter
              (fun expected ->
                 Alcotest.(check bool)
      ...
      , [ (* Issue #8480: introduces [pr_review_event] Variant where 4 sites
         ...
         -> Tool_shard). *)
          Alcotest.test_case "witness covers all 3 variants" `Quick (fun () ->
            ...
            let witness e =
              ...
              if not (List.mem actual K.valid_pr_review_event_strings)
              then
                Alcotest.failf
                  "pr_review_event_to_string %S not in valid_pr_review_event_strings"
                  actual
            in
            witness K.Comment;
            witness K.Approve;
            witness K.Request_changes;
            Alcotest.(check int) "count" 3 (List.length K.valid_pr_review_event_strings))
        ; Alcotest.test_case
            "of_string_opt accepts canonical and case-insensitive"
            `Quick
            (fun () ->
               let module K = Masc_mcp.Keeper_tool_pr_review in
               Alcotest.(check bool)
                 "COMMENT"
                 true
                 (K.pr_review_event_of_string_opt "COMMENT" <> None);
               Alcotest.(check bool)
                 "approve (lower)"
                 true
                 (K.pr_review_event_of_string_opt "approve" <> None);
               Alcotest.(check bool)
                 "  request_changes  "
                 true
        ...
        ; Alcotest.test_case "gh flag mapping" `Quick (fun () ->
            ...
            Alcotest.(check string)
              "comment"
              "--comment"
              (K.pr_review_event_to_gh_flag K.Comment);
            Alcotest.(check string)
              "approve"
              "--approve"
              (K.pr_review_event_to_gh_flag K.Approve);
            Alcotest.(check string)
              "request"
              "--request-changes"
         ...
         #8480). *)
          Alcotest.test_case "witness covers all 3 variants" `Quick (fun () ->
            ...
            let witness s =
              ...
              if not (List.mem actual M.valid_memory_search_source_strings)
              then
                Alcotest.failf
                  "memory_search_source_to_string %S not in \
                   valid_memory_search_source_strings"
                  actual
            in
            witness M.Memory;
            witness M.History;
            ...
            Alcotest.(check int)
              "count"
              3
              (List.length M.valid_memory_search_source_strings))
        ; Alcotest.test_case "of_string_opt sound partial" `Quick (fun () ->
            let module M = Masc_mcp.Keeper_exec_memory in
            Alcotest.(check bool)
              "memory"
              true
              (M.memory_search_source_of_string_opt "memory" <> None);
            Alcotest.(check bool)
              "HISTORY (case)"
              true
              (M.memory_search_source_of_string_opt "HISTORY" <> None);
            Alcotest.(check bool)
              "  all  (trim)"
              true
    ...
    ; ( "fs_write_mode_ssot"
      , [ (* Issue #8490: introduces [fs_write_mode] Variant where 5 sites
         ...
         (cycle-avoidance pattern from #8467/#8480/#8484). *)
          Alcotest.test_case "witness covers both variants" `Quick (fun () ->
            ...
            let witness m =
              ...
              if not (List.mem actual F.valid_fs_write_mode_strings)
              then
                Alcotest.failf
                  "fs_write_mode_to_string %S not in valid_fs_write_mode_strings"
                  actual
            in
            witness F.Overwrite;
            witness F.Append;
        ...
        ; Alcotest.test_case
            "of_string_opt sound partial + empty back-compat"
            `Quick
            (fun () ->
               let module F = Masc_mcp.Keeper_exec_fs in
               Alcotest.(check bool)
                 "overwrite"
                 true
                 (F.fs_write_mode_of_string_opt "overwrite" <> None);
               Alcotest.(check bool)
                 "APPEND (case)"
                 true
                 (F.fs_write_mode_of_string_opt "APPEND" <> None);
               Alcotest.(check bool)
                 "  empty -> Overwrite (back-compat)"
                 true
                 (F.fs_write_mode_of_string_opt "" = Some F.Overwrite);
               Alcotest.(check bool)
                 "whitespace-only -> Overwrite"
                 true
                 (F.fs_write_mode_of_string_opt "   " = Some F.Overwrite);
               Alcotest.(check bool)
                 "garbage rejected"
      ...
      , [ (* Issue #8506: Variant + to_string already existed (board_types/
         ...
         in sync with the SSOT and the witness covers both Variants. *)
          Alcotest.test_case "witness covers both variants" `Quick (fun () ->
            ...
            let witness d =
              ...
              if not (List.mem actual B.valid_vote_direction_strings)
              then
                Alcotest.failf
                  "vote_direction_to_string %S not in valid_vote_direction_strings"
                  actual
            in
            witness Masc_mcp.Board_votes.Up;
            witness Masc_mcp.Board_votes.Down;
        ...
        ; Alcotest.test_case "of_string_opt sound partial + back-compat" `Quick (fun () ->
            ...
            Alcotest.(check bool)
              "DOWN (case)"
              true
              (B.vote_direction_of_string_opt "DOWN" <> None);
            Alcotest.(check bool)
              "  empty -> Up back-compat"
              true
    ...
    ; ( "agent_tool_variants_ssot"
      , [ (* Issue #8501: introduces [agent_card_action] and
         ...
         mirror+sync pattern. *)
          Alcotest.test_case
            "agent_card_action witness covers both variants"
            `Quick
            (fun () ->
               let module T = Masc_mcp.Tool_agent in
               let witness a =
                 let actual = T.agent_card_action_to_string a in
                 if not (List.mem actual T.valid_agent_card_action_strings)
                 then
                   Alcotest.failf
                     "agent_card_action_to_string %S not in \
                      valid_agent_card_action_strings"
                     actual
               in
               witness T.Get;
               witness T.Refresh;
               Alcotest.(check int)
                 "count"
                 2
                 (List.length T.valid_agent_card_action_strings))
        ; Alcotest.test_case
            "collaboration_format witness covers both variants"
            `Quick
            (fun () ->
               let module T = Masc_mcp.Tool_agent in
               let witness f =
                 let actual = T.collaboration_format_to_string f in
                 if not (List.mem actual T.valid_collaboration_format_strings)
                 then
                   Alcotest.failf
                     "collaboration_format_to_string %S not in \
                      valid_collaboration_format_strings"
                     actual
               in
               witness T.Text;
               witness T.Json;
      ...
      , [ (* Issue #8522: introduces [Tool_code_write.git_action] Variant
         ...
         comparisons). Witness covers all 11 constructors. *)
          Alcotest.test_case "witness covers all 11 variants" `Quick (fun () ->
            ...
            let witness a =
              ...
              if not (List.mem actual T.valid_git_action_strings)
              then
                Alcotest.failf
                  "git_action_to_string %S not in valid_git_action_strings"
                  actual
            in
            witness T.Add;
            witness T.Commit;
        ...
        ; Alcotest.test_case "of_string_opt sound partial" `Quick (fun () ->
            ...
            Alcotest.(check bool)
              "commit"
              true
              (T.git_action_of_string_opt "commit" <> None);
            Alcotest.(check bool)
              "PUSH (case)"
              true
              (T.git_action_of_string_opt "PUSH" <> None);
            Alcotest.(check bool)
              "  clone  (trim)"
              true
        ...
        ; Alcotest.test_case "allowed_git_actions == SSOT" `Quick (fun () ->
            Alcotest.(check (list string))
              "allowlist == valid_git_action_strings"
    ...
    ; ( "mcp_session_action_ssot"
      , [ (* Issue #8520: introduces [Mcp_session.action] Variant where 2
         ...
         stays in lock-step with the SSOT. *)
          Alcotest.test_case "witness covers all 5 variants" `Quick (fun () ->
            ...
            let witness a =
              ...
              if not (List.mem actual M.valid_action_strings)
              then Alcotest.failf "action_to_string %S not in valid_action_strings" actual
            in
            witness M.Get;
            witness M.Create;
            witness M.List;
        ...
        ; Alcotest.test_case "of_string_opt sound partial" `Quick (fun () ->
            ...
            Alcotest.(check bool) "empty rejected" true (M.action_of_string_opt "" = None))
        ; Alcotest.test_case "schema mirror stays in sync" `Quick (fun () ->
            Alcotest.(check (list string))
              "tool_schemas mirror == SSOT"
         ...
         as #8430 / #8471 / #8474 / #8493 / #8513. *)
          Alcotest.test_case "witness covers all 16 variants" `Quick (fun () ->
            ...
            let witness o =
              ...
              if not (List.mem actual S.valid_shell_op_strings)
              then
                Alcotest.failf
                  "shell_op_to_string %S not in valid_shell_op_strings"
                  actual
            in
            witness S.Pwd;
            witness S.Ls;
      ...
      , [ (* Issue #8569: keeper_keepalive used to hand-build the
         ...
         tests pin the SSOT label and catch any future re-truncation. *)
          Alcotest.test_case "channel_to_string emits full snake_case" `Quick (fun () ->
            ...
            Alcotest.(check string)
              "Scheduled_autonomous label"
              "scheduled_autonomous"
              (channel_to_string Scheduled_autonomous);
            Alcotest.(check string)
              "Reactive label"
              "reactive"
              (channel_to_string Reactive))
        ; Alcotest.test_case
            "truncated 'autonomous' label is NOT the SSOT (regression #8569)"
            `Quick
            (fun () ->
               let open Masc_mcp.Keeper_world_observation in
               let label = channel_to_string Scheduled_autonomous in
               Alcotest.(check bool)
                 "label is not the truncated form"
                 false
         ...
         Same shape as #8467 (sandbox/network/shared_memory). *)
          Alcotest.test_case "witness covers both variants" `Quick (fun () ->
            ...
            let witness o =
              ...
              if not (List.mem actual S.valid_tail_order_strings)
              then
                Alcotest.failf
                  "tail_order_to_string %S not in valid_tail_order_strings"
                  actual
            in
            witness S.Oldest_first;
            witness S.Newest_first;
            Alcotest.(check int) "count" 2 (List.length S.valid_tail_order_strings))
        ; Alcotest.test_case "schema mirror stays in sync" `Quick (fun () ->
            Alcotest.(check (list string))
              "tail_order mirror == SSOT"
      ...
      , [ (* Issue #8592: witness exhaustiveness for [Dashboard.scope] +
         ...
         constructor would silently drop from the JSON Schema. *)
          Alcotest.test_case "witness covers both variants" `Quick (fun () ->
            ...
            let witness s =
              ...
              if not (List.mem actual D.valid_scope_strings)
              then Alcotest.failf "scope_to_string %S not in valid_scope_strings" actual
            in
            witness D.All;
            witness D.Current;
        ...
        ; Alcotest.test_case "scope_of_string_opt rejects unknown input" `Quick (fun () ->
            Alcotest.(check (option string))
              ...
              None
              (Option.map
                 Masc_mcp.Dashboard.scope_to_string
    ...
    ; ( "cascade_strategy_kind_ssot"
      , [ (* Issue #8603: parse_kind's Error message used to hard-code the
         ...
         is derived (not hard-coded). *)
          Alcotest.test_case
            "all_kinds round-trips through kind_to_string"
            `Quick
            (fun () ->
        ...
        ; Alcotest.test_case
            "parse_kind error mentions every valid kind"
            `Quick
            (fun () ->
               let module C = Masc_mcp.Cascade_strategy in
                 ...
                 List.iter
                   (fun k ->
                      ...
                      let rec contains i =
                        if i + len > mlen
                        then false
                        else if String.sub msg i len = needle
                        then true
                        else contains (i + 1)
                      in
                      Alcotest.(check bool)
                        (Printf.sprintf "error message lists %S" k)
                        true
                        (contains 0))
    ...
    ; ( "library_source_ssot"
      , [ (* Issue #8601: 3-way drift — module docstring claimed 3 sources,
         ...
         fails this test. *)
          Alcotest.test_case "witness covers all 4 constructors" `Quick (fun () ->
            ...
            let witness s =
              ...
              if not (List.mem actual L.valid_source_strings)
              then Alcotest.failf "source_to_string %S not in valid_source_strings" actual
            in
        ...
        ; Alcotest.test_case
            "source_of_string_opt round-trips SSOT strings"
            `Quick
            (fun () ->
               let module L = Masc_mcp.Tool_library in
               List.iter
        ...
        ; Alcotest.test_case "source_of_string_opt rejects unknown" `Quick (fun () ->
            Alcotest.(check (option string))
              ...
              None
              (Option.map
                 Masc_mcp.Tool_library.source_to_string
    ...
    ; ( "assertion_kind_ssot"
      , [ (* Issue #8636: 3-way drift on masc_check assertion vocabulary —
         ...
         [assertion_kind_to_string] AND test failure here. *)
          Alcotest.test_case "witness covers all 5 constructors" `Quick (fun () ->
            ...
            let witness k =
              ...
              if not (List.mem actual C.valid_assertion_strings)
              then
                Alcotest.failf
                  "assertion_kind_to_string %S not in valid_assertion_strings"
                  actual
            in
            witness C.Room_set;
            witness C.Joined;
        ...
        ; Alcotest.test_case
            "valid_assertion_strings pinned to wire format"
            `Quick
            (fun () ->
               Alcotest.(check (list string))
                 "wire-format strings"
        ...
        ; Alcotest.test_case
            "lenient parser accepts namespace_ready alias"
            `Quick
            (fun () ->
               let module C = Masc_mcp.Tool_coord in
        ...
        ; Alcotest.test_case "lenient parser rejects unknown" `Quick (fun () ->
            Alcotest.(check bool)
              "unknown -> None"
              true
              (Masc_mcp.Tool_coord.assertion_kind_of_string_lenient "fabricated_assertion"
               = None))
        ] )
      ...
      , [ (* Issue #8581: the [compact_retry_exhausted] field was read by
         ...
         tests pin the new event surface and the latch transition. *)
          Alcotest.test_case "event_to_string emits stable wire string" `Quick (fun () ->
            ...
            Alcotest.(check string)
              "Compact_retry_exhausted wire form"
        ...
        ; Alcotest.test_case "update_conditions latches the flag" `Quick (fun () ->
            ...
            Alcotest.(check bool) "init: flag false" false c0.compact_retry_exhausted;
            let c1 = update_conditions c0 Compact_retry_exhausted in
            Alcotest.(check bool) "after event: flag true" true c1.compact_retry_exhausted)
        ; Alcotest.test_case
            "Operator_compact_requested clears the latch"
            `Quick
            (fun () ->
               let open Masc_mcp.Keeper_state_machine in
               let c = update_conditions default_conditions Compact_retry_exhausted in
               Alcotest.(check bool) "latched" true c.compact_retry_exhausted;
               let c' = update_conditions c Operator_compact_requested in
               Alcotest.(check bool)
                 "cleared by operator compact"
                 false
    ...
    ; ( "lifecycle_events_ssot"
      , [ (* Issue #8575: Oas_events.publish_keeper_lifecycle docstring
         ...
         phases that fire a wire event. *)
          Alcotest.test_case
            "custom-event witness covers all constructors"
            `Quick
            (fun () ->
               let module L = Masc_mcp.Keeper_lifecycle_events in
               let witness e =
                 let s = L.to_string e in
                 if not (List.mem s L.valid_custom_event_strings)
                 then Alcotest.failf "to_string %S not in valid_custom_event_strings" s
               in
               witness L.Started;
               ...
               Alcotest.(check int)
                 "all_custom_events count"
                 6
                 (List.length L.all_custom_events))
        ; Alcotest.test_case
            "phase-derived strings match Keeper_state_machine SSOT"
            `Quick
            (fun () ->
               let open Masc_mcp.Keeper_state_machine in
               let expected =
               ...
               in
               Alcotest.(check (list string))
                 "phase-derived event names match SSOT"
                 expected
                 Masc_mcp.Keeper_lifecycle_events.phase_derived_event_strings)
        ; Alcotest.test_case
            "all_event_names covers cleanup events (regression #8575)"
            `Quick
            (fun () ->
        ...
        ; Alcotest.test_case "all_event_names totals 10 distinct names" `Quick (fun () ->
            ...
            Alcotest.(check int) "no duplicates" (List.length names) (List.length dedup))
        ] )
    ; ( "lifecycle_event_cache_patcher_coverage"
      , [ (* Issue #8396: dashboard cache patchers used to recognise only
         ...
         the SSOT without updating the patcher fails this test. *)
          Alcotest.test_case
            "every SSOT event has at least one patcher hit"
            `Quick
            (fun () ->
               ...
               List.iter
                 ...
                 L.all_event_names)
        ; Alcotest.test_case
            "every SSOT event hits keepalive_running patcher"
            `Quick
            (fun () ->
               let module S = Masc_mcp.Server_dashboard_http_execution_surfaces in
               let module L = Masc_mcp.Keeper_lifecycle_events in
         ...
         (witness exhaustive over [task_status]) or the assertion. *)
          Alcotest.test_case "Todo is the only claim pool candidate" `Quick (fun () ->
            ...
            Alcotest.(check bool)
              ...
              (S.task_is_claim_pool_candidate
                 (dummy_task
                    (Types.Done { assignee = "a"; completed_at = "t"; notes = None })));
            Alcotest.(check bool)
              "Cancelled -> NOT claim pool"
              false
              (S.task_is_claim_pool_candidate
                 (dummy_task
                    (Types.Cancelled
                       { cancelled_by = "a"; cancelled_at = "t"; reason = None }))))
        ] )
    ; ( "publish_phase_lifecycle_ssot"
      , [ (* Issue #8572: phase-bearing publish_lifecycle calls used to pass
        ...
        ; Alcotest.test_case
            "all_phases round-trip through to_string/of_string"
            `Quick
            (fun () ->
               let open Masc_mcp.Keeper_state_machine in
               List.iter
                 (fun p ->
                    ...
                    | Some _ ->
                      Alcotest.failf "phase_of_string %S parsed to a different phase" s
                    | None ->
                      Alcotest.failf
                        "phase_of_string %S returned None — round-trip broken"
                        s)
                 all_phases)
        ] )
    ; ( "health_paths_ssot"
    ...
    ; ( "verdict_ssot"
      , [ (* Issue #8436: payload-bearing variants need a witness function
         ...
         witness inside the impl. *)
          Alcotest.test_case "verifier_core covers Pass/Warn/Fail" `Quick (fun () ->
            ...
            let witness v =
              ...
              if not (List.mem n valid_verdict_strings)
              then
                Alcotest.failf
                  "verdict_constructor_name %S not in valid_verdict_strings"
                  n
            in
            witness Pass;
            witness (Warn "x");
        ...
        ; Alcotest.test_case
            "anti_rationalization covers Approve/Reject"
            `Quick
            (fun () ->
