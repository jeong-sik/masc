...
- **실측 (2026-09-01 ~ 09-17, 총 2,164건)**:
  ...
  - `what-arrived`: 342건. 이 수는 `<base-path>/.masc/tool_calls` 의 09-01 ~ 09-17 줄 가운데 모델이 부른 `keeper_compose_what-arrived` 줄을 센 것이다(2026-09-20 확인).
    이 컴포지션은 저장소 `skills/` 에 정의가 없다. 지운 기록도 없다. 그래서 노드 목록으로는 셀 수 없고, 위 카탈로그 7개에도 들어가지 않는다.
- **한계와 문제점**:
  1. **작성의 병목**: 카탈로그는 7개뿐이며, 전부 사람이 손으로 100~200줄의 TOML을 작성했다.
     2026-09-20 에 저장소 `skills/*/SKILL.md` 의 ```` ```toml composition ```` 블록을 세었다.
     `tool-events` 의 `Assigned` 목록도 09-16 부터 같은 일곱 이름만 보여주고, 이 RFC 를 처음 쓴 09-17 의 저장소도 같은 7개였다.
     같은 로그를 08-18 까지 거슬러 보면 이름 16개가 나오지만, 지금 없는 9개는 노드 목록이 없어 셀 수 없다.
     넷은 #36484 에서 파일이 지워졌고, 다섯은 저장소 `skills/` 에 정의가 없던 런타임 카탈로그 항목이다.
