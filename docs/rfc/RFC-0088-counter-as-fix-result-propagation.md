...
| 7 | `lib/auth.ml:441` | `[identity_drift:alias_fallback]` WARN-only (no counter) | Diagnostic for in-flight credential migration. WARN is the contract, not a `_silent_` claim. | Out of scope (intentional diagnostic) |
