...
let resolve_join_state ~room_initialized ~join_required ~agent_name ~check_join =
  if room_initialized && join_required && agent_name <> "unknown"
  then (
    try check_join () with
...
let is_ephemeral_agent_name name = Base.String.is_prefix name ~prefix:"agent-"
...
let execute_tool_eio
  ...
  let arg_get_string key default = Safe_ops.json_string ~default key arguments in
  ...
  let with_system_internal_audit ~agent_name ((success, message) as result) =
    ...
    then (
      let error_msg = if success then None else Some (preview message) in
      let details =
        `Assoc
  ...
  | None ->
    ...
    (match auth_result with
     | Error err ->
       with_system_internal_audit ~agent_name (false, Types.masc_error_to_string err)
     | Ok () ->
     ...
     - Auto-join when allowed by auth (and safe for token-based auth). *)
       ...
       (match init_error with
        | Some msg ->
          with_system_internal_audit ~agent_name (false, Printf.sprintf "❌ %s" msg)
        | None ->
