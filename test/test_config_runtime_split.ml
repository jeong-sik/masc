...
|} in
  ...
  (* Test 2: Runtime JSON with TOML-owned config fields parses, then scrubs
     config-only fields from serialized runtime output. *)
   ...
   | Ok meta ->
     (match Keeper_meta_json.meta_to_json meta with
      | `Assoc fields ->
        ...
        Printf.printf "test2: PASS — config fields scrubbed from output\n"
      | _ -> Printf.printf "FAIL test2: not Assoc\n"; exit 1)
   | Error msg ->
     Printf.printf "FAIL test2: parse: %s\n" msg; exit 1);
