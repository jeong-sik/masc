...
(** Unit tests for [Workspace_bootstrap].

    Audit P2 follow-up (2026-04-29 §3.1.2) — closes the
    workspace_bootstrap entry of the "테스트 완전 부재 모듈 10건"
    group.

    The module has two exposed functions:

    - [default_workspace_state config] — pure (modulo [now_iso ()] for
      [started_at]) constructor for [Masc_domain.workspace_state].
    - [ensure_workspace_bootstrap config] — idempotent FS bootstrap
      that creates [.masc/<dirs>] and seed [workspace_state.json] /
      [backlog.json] under both root and scoped layouts.

    Properties pinned:

    1. {b default_workspace_state constants} — every field except
       [started_at] and [project] is a fixed default; [project]
       derives from [Filename.basename config.base_path].
    2. {b ensure_workspace_bootstrap idempotency} — running it twice
...
let make_tmp_dir () =
  let unique =
    Printf.sprintf "masc_workspace_bootstrap_test_%d_%.0f"
...
(* ── (1) default_workspace_state ─────────────────────────────── *)

let test_default_workspace_state_constants () =
  with_tmp (fun dir ->
      let config = Workspace.default_config dir in
      let st = B.default_workspace_state config in
...
let test_default_workspace_state_project_from_base_path () =
...
let test_default_workspace_state_started_at_iso_format () =
  with_tmp (fun dir ->
      let config = Workspace.default_config dir in
      let st = B.default_workspace_state config in
...
(* ── (2) ensure_workspace_bootstrap creates dirs ──────────────── *)

let test_ensure_workspace_bootstrap_creates_scoped_dirs () =
  with_tmp (fun dir ->
      let config = Workspace.default_config dir in
...
let test_ensure_workspace_bootstrap_idempotent () =
...
let test_ensure_workspace_bootstrap_does_not_clobber_existing_state () =
...
let () =
  test_default_workspace_state_constants ();
  test_default_workspace_state_project_from_base_path ();
  test_default_workspace_state_started_at_iso_format ();
  test_ensure_workspace_bootstrap_creates_scoped_dirs ();
  test_ensure_workspace_bootstrap_creates_root_dirs ();
  test_ensure_workspace_bootstrap_seeds_state_json ();
  test_ensure_workspace_bootstrap_idempotent ();
  test_ensure_workspace_bootstrap_does_not_clobber_existing_state ();
  print_endline "test_workspace_bootstrap: all assertions passed"
