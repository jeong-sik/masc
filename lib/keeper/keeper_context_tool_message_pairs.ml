  ...
  | Agent_sdk.Types.Assistant ->
    List.filter_map
      (fun block ->
        ...
        (* [call_id] is a field on both provider_tool_call and
           provider_tool_result (0.208.5); annotate so it resolves to the
           tool-call record, not the later-defined tool-result record. *)
        |> Option.map (fun (call : Canonical_tool.provider_tool_call) ->
