...
| `MASC_SHELL_GATE_AUTHORITY` | off | RFC-0131 per-caller Shell IR authority set. Comma-separated explicit caller tokens only: `worker`/`worker_dev_tools`, `code_write`/`tool_code_write`, `keeper_bash`/`keeper_shell_bash`. Broad truthy values (`1`, `true`, `on`) are ignored so authority cannot accidentally flip for all callers. Predicate-only until authority decision arms consume it. |
