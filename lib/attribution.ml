...
let origin_of_string = function
  ...
  | other ->
    Error
      (Printf.sprintf "attribution.origin: expected \"det\" | \"nondet\", got %S" other)
;;
...
let require_string fields key =
  ...
  | other ->
    Error
      (Printf.sprintf
         "attribution.%s: expected string, got %s"
         key
...
let require_float fields key =
  ...
  | other ->
    Error
      (Printf.sprintf
         "attribution.%s: expected number, got %s"
         key
         (Yojson.Safe.to_string other))
;;
  ...
  | `Assoc fields ->
    ...
    (match kind with
     ...
     | other ->
       Error
         (Printf.sprintf
            "attribution.outcome.kind: unknown %S (expected passed | policy_failed | \
             transition_blocked | partial_pass)"
            other))
  | json ->
    Error
      (Printf.sprintf
         "attribution.outcome: expected JSON object, got %s"
...
let of_yojson = function
  ...
  | json ->
    Error
      (Printf.sprintf
         "attribution: expected JSON object, got %s"
         (Yojson.Safe.to_string json))
;;
...
let show (t : t) : string =
  Printf.sprintf
    "Attribution{origin=%s; gate=%s; outcome=%s}"
