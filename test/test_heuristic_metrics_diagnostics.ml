...
let assert_eq_int ~msg expected got =
  if expected <> got then fail (Printf.sprintf "%s: expected=%d got=%d" msg expected got)
;;
...
let test_ignores_malformed_records () =
  let records : Yojson.Safe.t list =
    ...
    ; `Null
    ; `Assoc
        [ "site", `String "missing_threshold"
        ; "raw_value", `Float 0.5
        ; "triggered", `Bool true
        ]
    ; `Assoc
        [ "site", `String "bad_types"
        ; "raw_value", `String "oops"
        ; "threshold", `Float 0.3
        ; "triggered", `Bool true
        ]
    ]
  ...
  assert_eq_int ~msg:"only the well-formed record is counted" 1 r.total_records;
  match r.sites with
  | [ s ] -> assert_true "good site name parsed" (s.site = "ok")
  | other -> fail (Printf.sprintf "expected one site, got %d" (List.length other))
;;
...
let test_degenerate_site_flagged () =
  ...
  assert_list_equal
    ~msg:"post_tool_use_failure flagged degenerate"
    [ "post_tool_use_failure" ]
    r.degenerate_sites;
  assert_list_equal
    ~msg:"one-sided (100% triggered=true)"
    [ "post_tool_use_failure" ]
    r.one_sided_sites
;;
...
let test_below_min_records_not_flagged () =
  ...
  let r = D.analyze records in
  assert_false
    "below-threshold site NOT flagged degenerate"
    (List.mem "tiny" r.degenerate_sites);
  assert_false
    "below-threshold site NOT flagged one-sided"
...
let test_variance_prevents_flag () =
  ...
  in
  let r = D.analyze records in
  assert_false "varying site NOT degenerate" (List.mem "healthy" r.degenerate_sites);
  assert_false "varying site NOT one-sided" (List.mem "healthy" r.one_sided_sites);
  match r.sites with
  | [ s ] ->
    assert_true "multiple unique tuples" (s.unique_tuples > 1);
    assert_true
      "triggered counts split"
      (s.triggered_true_count > 0 && s.triggered_false_count > 0)
  | _ -> fail "expected one site"
;;
...
let test_mixed_workload () =
  let degenerate =
    repeat 25 (fun i ->
      record ~ts:(float_of_int i) ~site:"theatre" ~raw:1.0 ~thr:0.0 ~trig:true ())
  in
  let healthy =
    List.init 25 (fun i ->
      record
        ~ts:(100.0 +. float_of_int i)
        ~site:"real"
        ~raw:(float_of_int i /. 100.0)
        ~thr:0.5
        ~trig:(i mod 3 = 0)
        ())
  in
  let cold = repeat 3 (fun _ -> record ~site:"cold" ~raw:0.1 ~thr:0.9 ~trig:false ()) in
  let r = D.analyze (degenerate @ healthy @ cold) in
  assert_eq_int ~msg:"total records" 53 r.total_records;
  assert_list_equal
    ~msg:"only theatre flagged degenerate"
    [ "theatre" ]
    r.degenerate_sites;
  assert_list_equal ~msg:"only theatre one-sided" [ "theatre" ] r.one_sided_sites
;;
...
let test_all_triggered_false_flagged () =
  ...
  let r = D.analyze records in
  assert_false
    "not degenerate (variance present)"
    (List.mem "unreachable" r.degenerate_sites);
  assert_list_equal
    ~msg:"one-sided (all triggered=false)"
    [ "unreachable" ]
    r.one_sided_sites
;;
...
let test_latest_timestamp_is_max () =
  ...
  | [ s ] ->
    (match s.latest_timestamp with
     | Some 5.0 -> ()
     | Some f -> fail (Printf.sprintf "latest_timestamp should be 5.0, got %.3f" f)
...
let contains_substring s sub =
  ...
  then false
    ...
    let last = ls - lsub in
    let rec loop i =
      if i > last then false else if String.sub s i lsub = sub then true else loop (i + 1)
    in
    loop 0)
;;

let test_pretty_summary_contains_header () =
  let records = repeat 25 (fun _ -> record ~site:"x" ~raw:1.0 ~thr:0.0 ~trig:true ()) in
  let r = D.analyze records in
  let s = D.pretty_summary r in
  assert_true
    "header line present"
    (contains_substring s "heuristic_metrics diagnostics:");
  assert_true "degenerate count surfaces" (contains_substring s "degenerate=1");
  assert_true "site line present" (contains_substring s "site=x")
;;
