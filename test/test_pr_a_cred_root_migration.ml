...
(** RFC-0084 host-config-cleanup-A — credential root migration.
    ...
    PR-A delegates [Credential_host_config_provider.cred_root] to
    ...
    value without touching [credential_host_config_provider.ml]. *)
...
let test_no_tmp_keeper_creds_literal_in_provider () =
  let content = read_file "lib/keeper/credential_host_config_provider.ml" in
  let occurrences =
    count_substring ~haystack:content ~needle:{|"/tmp/keeper-creds"|}
  in
  (check int)
    "literal `\"/tmp/keeper-creds\"` must be 0 in \
     lib/keeper/credential_host_config_provider.ml after PR-A"
...
let test_cred_root_byte_identical_to_typed_surface () =
  (* Today's behaviour is preserved: [Credential_host_config_provider.cred_root]
     and [Host_config.host ().cred_root] must hold
     the same string at module-init time. *)
  let typed = (Host_config.host ()).cred_root in
  let provider = Masc_mcp.Credential_host_config_provider.cred_root in
  (check string)
    "Credential_host_config_provider.cred_root must equal \
