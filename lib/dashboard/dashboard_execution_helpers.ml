...
let latest_iso_timestamp values =
  ...
  in
  values |> List.fold_left pick_latest None |> Option.map fst
;;

let string_list_json values = `List (List.map (fun value -> `String value) values)
let string_list_of_field key json = member_assoc key json |> string_list_of_json
...
let cap_string_list ?(limit = execution_tool_preview_limit) values = take limit values
    ...
    Some
      (String.concat
         ...
         (List.filter_map
            ...
            [ (if extra = []
               then None
               else Some (Printf.sprintf "%d route(s)" (List.length extra)))
            ; source
            ]))
;;
...
let extract_persona_name (agent_name : string) : string =
  let s = agent_name in
  let s =
    if String.length s > 7 && String.starts_with ~prefix:"keeper-" s
    then String.sub s 7 (String.length s - 7)
    else s
  in
  let s =
    if String.length s > 6 && String.sub s (String.length s - 6) 6 = "-agent"
    then String.sub s 0 (String.length s - 6)
    else s
  in
  s
;;
...
let load_persona_profile (persona_name : string) : agent_profile option =
  ...
  then None
    ...
    | Ok json ->
      ...
      let name_val =
        Safe_ops.json_string_opt "name" json |> Option.value ~default:persona_name
      in
      let keeper_json =
        Safe_ops.protect ~default:None (fun () -> Some (json |> member "keeper"))
      in
      let model =
      ...
      Some
        { emoji = "🤖"
        ; (* generic default — enriched from Neo4j later *)
...
let populate_neo4j_identity_cache_locked () =
  ...
  | Ok output ->
    (try
       ...
       let open Yojson.Safe.Util in
       let edges =
         json |> member "data" |> member "agents" |> member "edges" |> to_list
       in
       List.iter
         (fun edge ->
            ...
            then (
              let emoji =
                Safe_ops.json_string_opt "emoji" node |> Option.value ~default:"🤖"
              in
              let korean_name =
                Safe_ops.json_string_opt "koreanName" node |> Option.value ~default:name
              in
...
let merge_profiles ~(base : agent_profile) ~(overlay : agent_profile) : agent_profile =
  { emoji =
      (if overlay.emoji <> "🎭" && overlay.emoji <> "🤖" then overlay.emoji else base.emoji)
  ; korean_name =
      (if overlay.korean_name <> "" then overlay.korean_name else base.korean_name)
  ; model =
      (match overlay.model with
       | Some _ -> overlay.model
       | None -> base.model)
  ...
  ; activity_level =
      (match overlay.activity_level with
       | Some _ -> overlay.activity_level
       | None -> base.activity_level)
  ; primary_value =
      (match overlay.primary_value with
       | Some _ -> overlay.primary_value
       | None -> base.primary_value)
  }
;;
