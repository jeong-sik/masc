...
val read_run : Raw_trace.run_ref -> (Raw_trace.record list, Error.t) result

(** {1 Summarize} *)

(** Produce a summary of a single run (event counts, tool/hook names,
    timestamps, final text and stop reason). *)
val summarize_run : Raw_trace.run_ref -> (Raw_trace.run_summary, Error.t) result
...
val validate_run : Raw_trace.run_ref -> (Raw_trace.run_validation, Error.t) result
