...
let read_stdin_all () = In_channel.input_all In_channel.stdin
...
let has_spec_stdin_flag () = Array.exists (String.equal "--spec-stdin") Sys.argv

let main_result () =
  if not (has_spec_stdin_flag ())
  then
        ...
        (fun () ->
           Fun.protect
             ...
             (fun () ->
                ...
                with
                | Ok run_result ->
                  Lib.Worker_runtime_helper_protocol.success_json run_result, 0
                | Error msg ->
                  ( Lib.Worker_runtime_helper_protocol.error_json
  ...
  | exn ->
    emit_json
      (Lib.Worker_runtime_helper_protocol.error_json
         ...
         ; kind = Lib.Worker_runtime_helper_protocol.Internal
         });
    exit 1
;;
