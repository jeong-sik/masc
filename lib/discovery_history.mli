...
val record_probe : base_path:string -> Llm_provider.Discovery.endpoint_status list -> unit
...
val read_recent : base_path:string -> count:int -> Yojson.Safe.t list

(** Read entries whose day-file falls within [[since, until]] (YYYY-MM-DD). *)
val read_range : base_path:string -> since:string -> until:string -> Yojson.Safe.t list

(** Delete discovery day-files older than [days] days. *)
val prune : base_path:string -> days:int -> unit
