...
let test_integration () = group "Integration" (fun () ->
  ...
  let provider_config = Llm_provider_dispatch.to_oas_provider Llm_types.claude_opus in
  assert_true "oas_adapter:claude_mapped" (Option.is_some provider_config);
  let provider_config_custom = Llm_provider_dispatch.to_oas_provider
  ...
  (match Llm_provider_dispatch.to_oas_message test_msg with
   | None -> assert_true "oas_adapter:msg_roundtrip" false
   | Some oas_m ->
     let back_m = Llm_provider_dispatch.of_oas_message oas_m in
  ...
  let oas_u = Llm_provider_dispatch.to_oas_usage test_usage in
  let back_u = Llm_provider_dispatch.of_oas_usage oas_u in
