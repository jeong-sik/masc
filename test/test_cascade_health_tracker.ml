...
let test_record_success_keeps_rate_1 () =
  ...
  check
    (float 0.001)
    "success rate 1.0 after 1 success"
    1.0
    (H.success_rate t ~provider_key:"p")
;;
...
let test_single_failure_no_cooldown () =
  ...
  check
    bool
    "single failure does not trip cooldown"
    false
    (H.is_in_cooldown t ~provider_key:"p")
;;
...
let test_cooldown_after_threshold () =
  ...
  check
    bool
    "cooldown trips after 3 consecutive failures"
    true
    (H.is_in_cooldown t ~provider_key:"p")
;;
...
let test_effective_weight_cooldown_zero () =
  ...
  check
    ...
    0
    (H.effective_weight t ~provider_key:"p" ~config_weight:100)
;;
...
let test_effective_weight_unknown_full () =
  ...
  check
    int
    "unknown provider → full config_weight"
    100
    (H.effective_weight t ~provider_key:"unseen" ~config_weight:100)
;;
...
let test_rejected_counts_as_failure_for_cooldown () =
  ...
  check
    bool
    "3 consecutive rejections trip cooldown"
    true
    (H.is_in_cooldown t ~provider_key:"p")
;;
...
let test_rejected_counts_against_success_rate () =
  ...
  check
    (float 0.001)
    "success_rate = 0.0 after only rejections"
    0.0
    (H.success_rate t ~provider_key:"p")
;;
...
let test_rejected_separately_counted_in_provider_info () =
  ...
  | Some info ->
    ...
    check int "rejected_in_window = 2 (failures excluded)" 2 info.rejected_in_window
;;
...
let test_success_after_rejected_clears_streak () =
  ...
  H.record_rejected t ~provider_key:"p" ();
  H.record_rejected t ~provider_key:"p" ();
  check bool "success resets rejected streak" false (H.is_in_cooldown t ~provider_key:"p")
;;
...
let test_evict_idle_drops_no_event_providers () =
  ...
  check int "evict_idle returns 0 when everyone has fresh events" 0 (H.evict_idle t);
...
let test_evict_idle_returns_zero_when_all_active () =
  ...
  check int "no eviction when all providers have recent events" 0 (H.evict_idle t)
;;
...
let test_hard_quota_triggers_immediate_cooldown () =
  ...
  check
    bool
    "single hard_quota event trips cooldown immediately"
    true
    (H.is_in_cooldown t ~provider_key:"p")
;;
    ...
    (match info.cooldown_expires_at with
     ...
     | Some expires ->
       ...
       check
         bool
         (Printf.sprintf
            "hard_quota cooldown (%.0fs) >> regular cooldown (60s)"
            remaining)
         true
         (remaining > 300.0))
;;
...
let test_hard_quota_effective_weight_zero () =
  ...
  check
    int
    "effective_weight = 0 during hard_quota cooldown"
    0
    (H.effective_weight t ~provider_key:"p" ~config_weight:100)
;;
...
let test_hard_quota_success_clears_cooldown () =
  ...
  check
    bool
    "success after hard_quota clears cooldown"
    false
    (H.is_in_cooldown t ~provider_key:"p")
;;
...
let test_hard_quota_preserves_longer_existing_cooldown () =
  ...
  check
    bool
    "second hard_quota does not shorten cooldown"
    true
...
let test_fingerprint_same_classification_accumulates () =
  ...
  H.record_failure
    ...
    ~provider_key:"p"
    ~error_kind:"timeout"
    ~error_reason:"deadline exceeded"
    ();
  H.record_failure
    t
    ~provider_key:"p"
  ...
  check int "exactly one fingerprint bucket" 1 (List.length info.top_fingerprints);
  match info.top_fingerprints with
  | [ (_, count) ] -> check int "fingerprint count = 3" 3 count
  | _ -> failwith "unreachable"
;;

let test_fingerprint_distinct_reasons_split () =
  let t = H.create () in
  H.record_failure t ~provider_key:"p" ~error_kind:"failure" ~error_reason:"reason A" ();
  H.record_failure t ~provider_key:"p" ~error_kind:"failure" ~error_reason:"reason B" ();
  let info = info_or_fail t ~provider_key:"p" in
  check int "two distinct fingerprints" 2 (List.length info.top_fingerprints)
;;
...
let test_fingerprint_top_3_cap () =
  ...
  for i = 1 to 5 do
    ...
    H.record_failure t ~provider_key:"p" ~error_kind:"failure" ~error_reason:r ()
  done;
  let info = info_or_fail t ~provider_key:"p" in
  check int "top_fingerprints capped at 3" 3 (List.length info.top_fingerprints)
;;
...
let test_fingerprint_unclassified_when_kind_missing () =
  ...
  let info = info_or_fail t ~provider_key:"p" in
  match info.top_fingerprints with
  | [ (fp, _) ] -> check string "fingerprint defaults to unclassified" "unclassified" fp
...
let test_last_failure_at_set_on_failure () =
  ...
  check
    bool
    "last_failure_at populated after failure"
    true
    (info_after.last_failure_at <> None)
;;
...
let test_last_failure_at_none_for_pure_success () =
  ...
  check
    bool
    "last_failure_at stays None after success-only events"
    true
    (info.last_failure_at = None)
;;

let test_fingerprint_top_sorted_descending () =
  let t = H.create () in
  (* low: 1, medium: 2, high: 3 *)
  H.record_failure t ~provider_key:"p" ~error_kind:"failure" ~error_reason:"low" ();
  H.record_failure t ~provider_key:"p" ~error_kind:"failure" ~error_reason:"medium" ();
  H.record_failure t ~provider_key:"p" ~error_kind:"failure" ~error_reason:"medium" ();
  H.record_failure t ~provider_key:"p" ~error_kind:"failure" ~error_reason:"high" ();
  H.record_failure t ~provider_key:"p" ~error_kind:"failure" ~error_reason:"high" ();
  H.record_failure t ~provider_key:"p" ~error_kind:"failure" ~error_reason:"high" ();
  let info = info_or_fail t ~provider_key:"p" in
  match info.top_fingerprints with
...
let test_terminal_failure_triggers_immediate_cooldown () =
  ...
  check
    bool
    "single terminal_failure event trips cooldown immediately"
    true
    (H.is_in_cooldown t ~provider_key:"kimi-for-coding")
;;
    ...
    (match info.cooldown_expires_at with
     ...
     | Some expires ->
       ...
       check
         bool
         (Printf.sprintf
            "terminal_failure cooldown (%.0fs) >> regular cooldown (60s)"
            remaining)
         true
         (remaining > 300.0))
;;
...
let test_terminal_failure_effective_weight_zero () =
  ...
  check
    int
    "effective_weight = 0 during terminal_failure cooldown"
    0
    (H.effective_weight t ~provider_key:"kimi-for-coding" ~config_weight:100)
;;
...
let test_terminal_failure_success_clears_cooldown () =
  ...
  check
    bool
    "success after terminal_failure clears cooldown"
    false
    (H.is_in_cooldown t ~provider_key:"kimi-for-coding")
;;
...
let test_terminal_failure_preserves_longer_existing_cooldown () =
  ...
  check
    bool
    "second terminal_failure does not shorten cooldown"
    true
    (expires_after_second >= expires_after_first)
;;
...
let test_terminal_failure_records_fingerprint () =
  ...
  | [ (fp, count) ] ->
    check
      bool
      "terminal failure fingerprint keeps kind"
      true
      (String.starts_with ~prefix:"resumable_cli_session" fp);
...
let () =
  run
    ...
    [ ( "record"
      , [ test_case
            "record_success keeps rate at 1.0"
            `Quick
            test_record_success_keeps_rate_1
        ; test_case
            "single failure does not cooldown"
            `Quick
            test_single_failure_no_cooldown
        ; test_case "cooldown after threshold" `Quick test_cooldown_after_threshold
        ; test_case "success resets streak" `Quick test_success_resets_streak
        ; test_case
            "effective_weight zero in cooldown"
            `Quick
            test_effective_weight_cooldown_zero
        ; test_case
            "unknown provider full weight"
            `Quick
            test_effective_weight_unknown_full
        ; test_case
            "provider_info reflects events"
            `Quick
            test_provider_info_reflects_events
        ] )
    ; ( "rejected"
      , [ test_case
            "rejected counts toward cooldown"
            `Quick
            test_rejected_counts_as_failure_for_cooldown
        ; test_case
            "rejected lowers success_rate"
            `Quick
            test_rejected_counts_against_success_rate
        ; test_case
            "rejected_in_window split from failures"
            `Quick
            test_rejected_separately_counted_in_provider_info
        ; test_case
            "success clears rejected streak"
            `Quick
            test_success_after_rejected_clears_streak
        ] )
    ; ( "evict_idle"
      , [ test_case
            "zero eviction on active tracker"
            `Quick
            test_evict_idle_drops_no_event_providers
        ; test_case
            "all-active → no eviction"
            `Quick
            test_evict_idle_returns_zero_when_all_active
        ] )
    ; ( "hard_quota"
      , [ test_case
            "single event trips immediate cooldown"
            `Quick
            test_hard_quota_triggers_immediate_cooldown
        ; test_case
            "cooldown duration is long (≫ 60s)"
            `Quick
    ...
    ; ( "fingerprint"
      , [ test_case
            "same classification accumulates"
            `Quick
            test_fingerprint_same_classification_accumulates
        ; test_case
            "distinct reasons → distinct fingerprints"
            `Quick
            test_fingerprint_distinct_reasons_split
        ; test_case "top_fingerprints capped at 3" `Quick test_fingerprint_top_3_cap
        ; test_case
            "missing kind defaults to unclassified"
            `Quick
            test_fingerprint_unclassified_when_kind_missing
        ; test_case
            "last_failure_at populated on failure"
            `Quick
            test_last_failure_at_set_on_failure
        ; test_case
            "last_failure_at stays None on success-only"
            `Quick
            test_last_failure_at_none_for_pure_success
        ; test_case
            "top_fingerprints sorted descending"
            `Quick
    ...
    ; ( "terminal_failure"
      , [ test_case
        ...
        ; test_case
            "terminal_failure records fingerprint"
            `Quick
