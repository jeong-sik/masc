...
| `LLAMA_SERVER_URL` | string | `Agent_sdk.Defaults.local_llm_url` | 로컬 Chat Completions v1-compatible runtime URL |
...
| `MASC_TIMEOUT_OPENAI_COMPAT_SEC` | int | 60 | Chat Completions v1 호환 API 타임아웃 |
...
| `OPENAI_DEFAULT_MODEL` | string | OAS runtime binding/catalog default | 설정 시 `chat-completions-v1` provider `auto` 기본 모델로 사용. |
...
protocol = "chat_completions_v1_cli"
command = "agent-code"
...
| `llama` | `Local_runtime` | `LLAMA_DEFAULT_MODEL` (legacy local Chat Completions v1-compatible runtime) |
...
| `chat-completions-v1` | `Chat Completions v1` | `OPENAI_DEFAULT_MODEL` |
