...
```
Evidence_change of { repository : string; pull_request : int }
```
...
거기서 네트워크를 부르면 저장소가 느리게 답하는 동안 그 워크스페이스의 모든 claim·release·판정이
뒤에 선다. 참조가 셋이면 셋을 곱한다. 같은 자리에 meta 수리 쓰기를 넣었다가 뺀 것과 같은 계열이다
(#36552 → #36560): 공유 락 안에서 느리거나 쓰는 일을 하지 않는다.

**그리고 생성자는 읽는 쪽과 같이 들어와야 한다.** 조회를 뺀 형태를 한 번 만들어 봤는데, 그 타입을
`Evidence_note` 와 다르게 읽는 코드가 하나도 없었다 — 판정자의 증거 개수, 도구 경계, 운영자가 보는 줄
셋 다 동작이 같고, `note:PR #30715 에 병합됨` 은 오늘 이미 된다. 차이는 렌더링되는 글자뿐인데 그
대가로 `masc_workspace` 가 `pull_request` 라는 forge 용어를 알고 exhaustive match 여덟 자리가 늘어난다.
#33218 이 "읽는 쪽이 없다" 며 `cancel_reason` 을 지운 것과 같은 기준을 여기에도 댄다.

그래서 이 단계는 **조회하는 자리를 정하는 것**이 설계의 전부다. 조회는 판정자가 증거를 읽을 때 한다 —
백로그 락 밖이고, 판정자는 이미 저장소 접근을 들고 있다. 그 자리에서 생성자와 typed 미열람
항목(`artifact_unreadable` 과 같은 모양)이 같이 들어온다.
...
| PR-4 (미착수, #36546 닫음) | §3.5 변경 증거 — 판정자가 증거를 읽는 자리에서 조회하고, 생성자를 같이 넣는다 | fixture: 병합된 PR 1건 → 판정자가 읽는 자리에서 스냅샷이 붙는다. 조회 실패 → typed 미열람. 제출 경계에서는 네트워크 0 |
