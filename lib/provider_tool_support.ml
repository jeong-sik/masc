...
(** Resolve OAS-level capabilities for a provider config, then merge
    ...
    a runtime MCP lane, so transport resolution sees the operator's
    [supports-runtime-mcp-tools] even when the OAS model-level lookup returns
...
let oas_capabilities_of_config (provider_cfg : Llm_provider.Provider_config.t) =
  ...
  then
    ...
    (* Non-CLI providers: merge declarative runtime.toml capabilities so
       transport resolution sees the operator's declared
       [supports-runtime-mcp-tools] even when the OAS model-level lookup
       returns a narrower capability set. *)
