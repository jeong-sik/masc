...
let with_clean_sandbox_env f =
  ...
  List.iter
    (fun n ->
       if List.mem n string_default_set
       then (
...
let test_defaults_pinned () =
  ...
  check int "Hardening.nofile_limit default" 245_760 (S.Hardening.nofile_limit ());
  ...
  check string "Hardening.seccomp_profile default" "" (S.Hardening.seccomp_profile ());
  check bool "Hardening.require_rootless default" false (S.Hardening.require_rootless ());
  check bool "Hardening.require_userns default" false (S.Hardening.require_userns ());
  (* Cleanup *)
  check bool "Cleanup.enabled default" true (S.Cleanup.enabled ());
  check approx "Cleanup.stale_after_sec default" 21_600.0 (S.Cleanup.stale_after_sec ());
  check approx "Cleanup.interval_sec default" 300.0 (S.Cleanup.interval_sec ());
  check int "Cleanup.managed_sleep_sec default" 3600 (S.Cleanup.managed_sleep_sec ());
  (* Runtime *)
  check
    string
    "Runtime.docker_image default"
    "masc-keeper-sandbox:local"
    (S.Runtime.docker_image ());
  check bool "Runtime.git_dispatch default" true (S.Runtime.git_dispatch ());
  check
    bool
    "Runtime.docker_playground_enabled default"
    false
  ...
  check
    approx
    "Auth_paths.gh_token_probe_timeout_sec default"
    2.0
    (S.Auth_paths.gh_token_probe_timeout_sec ());
  (* Preflight *)
  check bool "Preflight.enabled default" true (S.Preflight.enabled ());
  check approx "Preflight.min_timeout_sec default" 5.0 (S.Preflight.min_timeout_sec ());
  check approx "Preflight.max_timeout_sec default" 20.0 (S.Preflight.max_timeout_sec ());
  check
    int
    "Preflight.required_commands count"
    19
    (List.length (S.Preflight.required_commands ()));
  check
    (option string)
    "Preflight.required_commands has 'gh'"
    (Some "gh")
...
let test_env_overrides_default () =
  ...
  check
    string
    "memory falls back to default after clean restore"
    "2g"
    (S.Hardening.memory ())
;;
...
let test_per_bucket_env_var_shape () =
  ...
  check
    string
    "Token_probe env var name"
    "MASC_KEEPER_SHELL_TIMEOUT_TOKEN_PROBE_SEC"
    (S.Shell_timeout.per_bucket_env_var ~bucket:S.Shell_timeout.Token_probe);
  check
    string
    "Unknown bucket env var lowercases"
    "MASC_KEEPER_SHELL_TIMEOUT_FUTURE_X_SEC"
    (S.Shell_timeout.per_bucket_env_var ~bucket:(S.Shell_timeout.Unknown "future-x"))
;;
...
let test_unknown_bucket_uses_global_env () =
  ...
  with_env "MASC_KEEPER_SHELL_TIMEOUT_DEFAULT_SEC" (Some "99.0") (fun () ->
    check
      approx
      "Unknown bucket uses global env"
      99.0
      (S.Shell_timeout.timeout_sec ~bucket:(S.Shell_timeout.Unknown "future") ());
    check
      approx
      "Known bucket ignores global env"
      30.0
      (S.Shell_timeout.timeout_sec ~bucket:S.Shell_timeout.Io ()))
;;
...
let test_hard_mode_forces_require_flags () =
  ...
  with_env "MASC_KEEPER_SANDBOX_HARD_MODE" (Some "true") (fun () ->
    check
      bool
      "hard_mode forces require_rootless true"
      true
      (S.Hardening.require_rootless ());
    check bool "hard_mode forces require_userns true" true (S.Hardening.require_userns ());
    check bool "hard_mode forces git_dispatch false" false (S.Runtime.git_dispatch ()))
;;
...
let test_hard_mode_clears_credential_paths () =
  ...
  @@ fun () ->
  with_env "MASC_KEEPER_SANDBOX_HARD_MODE" (Some "true") (fun () ->
    check string "hard_mode clears gh_creds" "" (S.Auth_paths.gh_creds ());
    check string "hard_mode clears gitconfig" "" (S.Auth_paths.gitconfig ());
    check string "hard_mode clears ssh_dir" "" (S.Auth_paths.ssh_dir ()))
;;
...
let test_relax_fs_propagates_to_derived () =
  ...
  with_env "MASC_KEEPER_SANDBOX_RELAX_FS" (Some "true") (fun () ->
    check bool "relax_fs raw env wins" true (S.Hardening.relax_fs ());
    check
      (list string)
      "read_only_rootfs_args is empty when relax_fs"
      []
    ...
    let contains_noexec =
      ...
      let rec check_at i =
        if i + len > String.length mount
        then false
        else if String.sub mount i len = "noexec"
        then true
        else check_at (i + 1)
      in
      check_at 0
    in
    check bool "tmpfs_mount drops noexec when relax_fs" true (not contains_noexec))
;;
...
let test_json_shape_has_top_level_keys () =
  ...
  in
  let raw_keys =
    [ "hardening"; "cleanup"; "runtime"; "auth_paths"; "preflight"; "shell_timeout" ]
  in
  List.iter
    (fun k ->
       check bool (Printf.sprintf "raw.%s exists" k) true (List.mem_assoc k raw_assoc))
    raw_keys;
  ...
  let entry_keys =
    match hard_mode_entry with
    | `Assoc xs -> List.map fst xs
    | _ -> []
  in
  check
    (slist string compare)
    "raw entry has value/source/env_var keys"
...
let () =
  run
    "env_config_sandbox"
    [ "defaults", [ test_case "all defaults pinned" `Quick test_defaults_pinned ]
    ; ( "env-override"
      , [ test_case "env override wins" `Quick test_env_overrides_default
        ...
        ; test_case
            "invalid float -> global_default"
            `Quick
            test_invalid_float_falls_to_global_default
        ] )
    ; ( "shell-timeout-bucket"
      , [ test_case "env var name shape" `Quick test_per_bucket_env_var_shape
        ; test_case "per-bucket env override" `Quick test_per_bucket_env_override
        ; test_case
            "Unknown bucket -> global env"
            `Quick
            test_unknown_bucket_uses_global_env
        ; test_case "Gh_min floor ignores env" `Quick test_gh_min_floor_ignores_env
        ] )
    ; ( "hard-mode"
      , [ test_case
            "hard_mode forces require flags"
            `Quick
            test_hard_mode_forces_require_flags
        ; test_case
            "hard_mode clears credentials"
            `Quick
            test_hard_mode_clears_credential_paths
        ; test_case
            "relax_fs propagates to derived"
            `Quick
            test_relax_fs_propagates_to_derived
        ] )
    ; ( "json-shape"
      , [ test_case
            "top-level keys + entry shape"
            `Quick
