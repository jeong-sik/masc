...
let find_tool name tools = List.find (fun (t : Tool.t) -> t.schema.name = name) tools
...
let test_file_read_existing () =
  ...
  Out_channel.with_open_text path (fun oc -> Out_channel.output_string oc "hello world");
...
let test_file_read_nonexistent () =
  ...
  let tool = find_tool "file_read" tools in
  let result =
    Tool.execute tool (`Assoc [ "path", `String "/tmp/nonexistent_dev_tools_xyz.txt" ])
  in
  match result with
  | Error _ -> () (* expected *)
...
let test_file_read_blocked_path () =
  ...
  let tools = Worker_dev_tools.make_tools ~proc_mgr ~clock () in
  let tool = find_tool "file_read" tools in
  let result = Tool.execute tool (`Assoc [ "path", `String "/etc/passwd" ]) in
  match result with
...
let test_file_read_path_traversal () =
  ...
  let result = Tool.execute tool (`Assoc [ "path", `String "/tmp/../../etc/passwd" ]) in
  match result with
...
let test_file_read_rejects_prefix_sibling () =
  ...
  let result =
    Tool.execute
      tool
      (`Assoc [ "path", `String (Filename.concat home "me-sibling/secret.txt") ])
  in
  match result with
  | Error _ -> ()
...
let test_file_read_truncation () =
  ...
  Out_channel.with_open_text path (fun oc -> Out_channel.output_string oc large_content);
  let result = Tool.execute tool (`Assoc [ "path", `String path ]) in
  (match result with
   | Ok { Agent_sdk.Types.content } ->
     Alcotest.(check bool) "truncated to ~100KB" true (String.length content <= 100_100);
     Alcotest.(check bool)
...
let test_file_write_new () =
  ...
  let result =
    Tool.execute tool (`Assoc [ "path", `String path; "content", `String "test content" ])
  in
  (match result with
   | Ok { Agent_sdk.Types.content = msg } ->
     Alcotest.(check bool) "mentions bytes" true (String.length msg > 0);
...
let test_file_write_blocked_path () =
  ...
  let result =
    Tool.execute
      tool
      (`Assoc [ "path", `String "/etc/shadow_test"; "content", `String "bad" ])
  in
  match result with
  | Error _ -> () (* expected *)
...
let test_file_write_rejects_prefix_sibling () =
  ...
  let result =
    Tool.execute
      tool
      (`Assoc
          [ "path", `String (Filename.concat home "me-sibling/out.txt")
          ; "content", `String "bad"
          ])
  in
  match result with
    ...
    (fun () ->
       let result =
         Tool.execute
           tool
           (`Assoc
               [ "path", `String (Filename.concat path "passwd_copy")
               ; "content", `String "bad"
               ])
       in
       match result with
...
let test_shell_exec_echo () =
  ...
  let result = Tool.execute tool (`Assoc [ "command", `String "echo hello" ]) in
  match result with
...
let test_shell_exec_blocked_command () =
  ...
  let tools = Worker_dev_tools.make_tools ~proc_mgr ~clock () in
  let tool = find_tool "shell_exec" tools in
  let result = Tool.execute tool (`Assoc [ "command", `String "rm -rf /" ]) in
  match result with
  | Error { Agent_sdk.Types.message = msg; _ } ->
    Alcotest.(check bool) "mentions blocked" true (String.length msg > 0)
    ...
    (fun () ->
       ...
       let on_exec ~tool_name ~success ~duration_ms =
         Telemetry_eio.track_tool_called
           ...
           ~success
           ~duration_ms
           ~agent_id:"llama-local-worker"
           ()
       in
       let tools = Worker_dev_tools.make_tools ~proc_mgr ~clock ~on_exec () in
       ...
       (match
          Tool.execute
            write_tool
            (`Assoc [ "path", `String tmp_path; "content", `String "bridge" ])
        with
        | Ok _ -> ()
        | Error { Agent_sdk.Types.message = e; _ } ->
       ...
       (match
          Tool.execute shell_tool (`Assoc [ "command", `String "echo telemetry-ok" ])
        with
        | Ok _ -> ()
        | Error { Agent_sdk.Types.message = e; _ } ->
...
let test_shell_exec_rejects_shell_metacharacters () =
  ...
  let tools = Worker_dev_tools.make_tools ~proc_mgr ~clock () in
  let tool = find_tool "shell_exec" tools in
  let result = Tool.execute tool (`Assoc [ "command", `String "echo hello; pwd" ]) in
  match result with
  | Error { Agent_sdk.Types.message = msg; _ } ->
    ...
    Alcotest.(check bool) "mentions blocking guidance" true (has_blocked || has_chaining)
...
let test_shell_exec_nonexistent_cmd () =
  ...
  let tool = find_tool "shell_exec" tools in
  let result =
    Tool.execute tool (`Assoc [ "command", `String "nonexistent_cmd_xyz_123" ])
  in
  match result with
...
let test_shell_exec_missing_param () =
  ...
  match result with
  | Error { Agent_sdk.Types.message = msg; _ } ->
    Alcotest.(check bool) "error about missing command" true (String.length msg > 0)
...
let test_readonly_shell_exec_blocks_git () =
  ...
  let tools = Worker_dev_tools.make_readonly_tools ~proc_mgr ~clock () in
  let tool = find_tool "shell_exec" tools in
  let result = Tool.execute tool (`Assoc [ "command", `String "git status" ]) in
  match result with
  | Error _ -> ()
...
let test_workdir_enforcement () =
  ...
  let clock = Eio.Stdenv.clock env in
  let tools =
    Worker_dev_tools.make_tools ~proc_mgr ~clock ~workdir:"/tmp/test_workdir" ()
  in
  let tool = find_tool "file_write" tools in
  (* Writing inside workdir should succeed *)
  let ok_path = "/tmp/test_workdir/test.txt" in
  let result_ok =
    Tool.execute tool (`Assoc [ "path", `String ok_path; "content", `String "ok" ])
  in
  (match result_ok with
   | Ok _ -> ()
   | Error { Agent_sdk.Types.message = e; _ } ->
     Alcotest.fail (Printf.sprintf "workdir write failed: %s" e));
  (* Writing outside workdir (but inside ~/me) should be blocked *)
  let home = Sys.getenv "HOME" in
  let bad_path = Filename.concat home "me/should_not_write.txt" in
  let result_bad =
    Tool.execute tool (`Assoc [ "path", `String bad_path; "content", `String "bad" ])
  in
  ...
  try Unix.rmdir "/tmp/test_workdir" with
  | _ -> ()
;;
  ...
  Alcotest.run
    ...
    ; ( "file_read"
      , [ Alcotest.test_case "read existing file" `Quick test_file_read_existing
        ...
        ; Alcotest.test_case
            "prefix sibling blocked"
            `Quick
            test_file_read_rejects_prefix_sibling
        ; Alcotest.test_case
            "tmp symlink escape blocked"
            `Quick
    ...
    ; ( "file_write"
      , [ Alcotest.test_case "write new file" `Quick test_file_write_new
        ...
        ; Alcotest.test_case
            "write prefix sibling blocked"
            `Quick
            test_file_write_rejects_prefix_sibling
        ; Alcotest.test_case
            "write tmp symlink escape blocked"
            `Quick
    ...
    ; ( "validate_command_coding"
      , [ Alcotest.test_case "allows pipe" `Quick (fun () ->
            ...
            | Error e ->
              Alcotest.fail
                ("should allow pipe: " ^ Worker_dev_tools.block_reason_to_string e))
        ; Alcotest.test_case "allows redirect" `Quick (fun () ->
            match Worker_dev_tools.validate_command_coding "dune build 2>&1" with
            | Ok () -> ()
            | Error e ->
              Alcotest.fail
                ("should allow redirect: " ^ Worker_dev_tools.block_reason_to_string e))
        ...
        ; Alcotest.test_case "allows 2>&1 redirect" `Quick (fun () ->
            ...
            | Error e ->
              Alcotest.fail
                ("should allow 2>&1: " ^ Worker_dev_tools.block_reason_to_string e))
        ; Alcotest.test_case "single-command contract rejects pipe" `Quick (fun () ->
            ...
            | Error reason ->
              Alcotest.fail
                ("wrong rejection: " ^ Worker_dev_tools.block_reason_to_string reason)
        ...
        ; Alcotest.test_case "single-command contract keeps redirect" `Quick (fun () ->
            ...
            | Error e ->
              Alcotest.fail
                ("should allow direct build with fd redirect: "
                 ^ Worker_dev_tools.block_reason_to_string e))
        ; Alcotest.test_case
            "single-command contract enforces custom allowlist"
            `Quick
            (fun () ->
               match
                 Worker_dev_tools.validate_command_coding_with_allowlist
      ...
      , [ Alcotest.test_case "blocks force push" `Quick (fun () ->
        ...
        ; Alcotest.test_case "blocks force with lease" `Quick (fun () ->
            Alcotest.(check bool)
              ...
              true
              (Worker_dev_tools.is_destructive_bash_operation
                 "git push --force-with-lease origin feature/fix-1"))
    ...
    ; ( "validate_gh_command"
      , [ Alcotest.test_case
            "accepts allowed subcommand with no repo flag"
            `Quick
            (fun () ->
        ...
        ; Alcotest.test_case
            "rejects repo archive (parity with legacy guard)"
            `Quick
            (fun () ->
               match
                 Worker_dev_tools.validate_gh_command "repo archive jeong-sik/foo"
               with
        ...
        ; Alcotest.test_case "rejects repo outside allowed_orgs" `Quick (fun () ->
            ...
            | Error msg ->
              Alcotest.(check bool)
                "mentions not in allowed_orgs"
                true
                (contains_substring msg "not in allowed_orgs");
              Alcotest.(check bool)
                "mentions offending owner"
                true
        ...
        ; Alcotest.test_case "rejects -R short flag outside allowlist" `Quick (fun () ->
            ...
            | Error msg ->
              Alcotest.(check bool)
                ...
                (contains_substring msg "not in allowed_orgs"))
        ; Alcotest.test_case
            "allows no --repo flag with orgs configured"
            `Quick
            (fun () ->
               match
                 Worker_dev_tools.validate_gh_command
      ...
      , [ Alcotest.test_case "extracts from --repo flag" `Quick (fun () ->
        ...
        ; Alcotest.test_case "extracts from -R short flag" `Quick (fun () ->
            Alcotest.(check (option string))
              "owner"
              (Some "jeong-sik")
              (Worker_dev_tools.extract_gh_repo_owner "issue list -R jeong-sik/masc-mcp"))
      ...
      , [ (* R0 — read-only *)
        ...
        ; Alcotest.test_case "R2: graphql mutation deletePullRequest" `Quick (fun () ->
            Alcotest.(check bool)
              ...
              (Worker_dev_tools.classify_gh_reversibility
                 "api graphql -f \
                  query=mutation{deletePullRequest(input:{pullRequestId:abc}){clientMutationId}}"
               = Worker_dev_tools.R2_Irreversible))
        ; Alcotest.test_case "R2: graphql mutation transferRepository" `Quick (fun () ->
            Alcotest.(check bool)
              ...
              (Worker_dev_tools.classify_gh_reversibility
                 "api graphql -f \
                  query=mutation{transferRepository(input:{}){clientMutationId}}"
               = Worker_dev_tools.R2_Irreversible))
        ] )
    ; ( "validate_command_paths_redirect"
      , [ (* Field evidence (2026-04-17/18): 62 keeper_bash calls were rejected
         ...
         concrete replacement. *)
          Alcotest.test_case
            "glob path suggests masc_code_search file_pattern"
            `Quick
            (fun () ->
               match
                 Worker_dev_tools.validate_command_paths ~workdir:"/tmp" "ls repos/*.ml"
               with
               | Error msg ->
                 Alcotest.(check bool)
                   "names glob char"
                 ...
                 Alcotest.(check bool)
                   "names masc_code_search"
                   true
                   (contains_substring msg "masc_code_search")
               | Ok () -> Alcotest.fail "glob with path must be blocked")
        ; Alcotest.test_case "brace path suggests per-target / rg" `Quick (fun () ->
            match
              Worker_dev_tools.validate_command_paths ~workdir:"/tmp" "cat lib/{a,b}.ml"
            with
            | Error msg ->
              Alcotest.(check bool)
                "names brace"
        ...
        ; Alcotest.test_case
            "backslash path names masc_code_search is_regex"
            `Quick
            (fun () ->
               match
        ...
        ; Alcotest.test_case
            "plain path with no rewrite syntax is allowed"
            `Quick
            (fun () ->
               match
                 Worker_dev_tools.validate_command_paths ~workdir:"/tmp" "cat lib/foo.ml"
               with
               | Ok () -> ()
               | Error msg -> Alcotest.fail ("plain path unexpectedly rejected: " ^ msg))
        ] )
    ; ( "command_blocked_hint_redirects"
      , [ (* Field evidence (2026-04-17/18): keeper_bash rejected `gh`, `docker`,
        ...
        ; Alcotest.test_case "docker → escalation hint" `Quick (fun () ->
            ...
            in
            Alcotest.(check bool)
              "mentions escalation via masc_board_post"
              true
        ...
        ; Alcotest.test_case "ssh → network-primitive hint" `Quick (fun () ->
            ...
            in
            Alcotest.(check bool)
              "mentions masc_web_search"
              true
              (contains_substring msg "masc_web_search"))
        ; Alcotest.test_case
            "unknown command still gets keeper_tools_list pointer"
            `Quick
            (fun () ->
               let msg =
                 Worker_dev_tools.block_reason_to_string
        ...
        ; Alcotest.test_case "preserves existing source-code heuristic" `Quick (fun () ->
            ...
            in
            Alcotest.(check bool)
              "still suggests masc_code_edit for A.B names"
              true
    ...
    ; ( "structured_tool_hint_for_r2"
      , [ Alcotest.test_case "repo delete → board-post hint" `Quick (fun () ->
        ...
        ; Alcotest.test_case "credential op → operator-only hint" `Quick (fun () ->
            ...
            | Some msg ->
              Alcotest.(check bool)
                "mentions operator-only"
                true
                (contains_substring msg "operator-only")
            | None -> Alcotest.fail "expected Some hint")
        ; Alcotest.test_case "api R2 → generic hint" `Quick (fun () ->
            match
              Worker_dev_tools.structured_tool_hint_for_r2
                "api --method DELETE repos/x/y/releases/1"
            with
            | Some msg ->
              Alcotest.(check bool)
                "mentions gh api"
    ...
    ; ( "attribution"
      , [ Alcotest.test_case "Ok () → Passed with cmd in evidence" `Quick (fun () ->
            let attr = Worker_dev_tools.attribution_of_validation ~cmd:"ls -la" (Ok ()) in
            Alcotest.(check string) "gate" "worker_dev_tools" attr.gate;
            Alcotest.(check bool) "origin=Det" true (attr.origin = Attribution.Det);
            Alcotest.(check bool)
              "outcome=Passed"
              true
        ...
        ; Alcotest.test_case "Empty_command → Policy_failed" `Quick (fun () ->
            ...
            | Attribution.Policy_failed { reason } ->
              Alcotest.(check bool)
                "reason mentions empty"
                true
                (contains_substring reason "empty")
            | _ -> Alcotest.fail "expected Policy_failed")
        ; Alcotest.test_case
            "Command_not_allowed carries command_name in evidence"
            `Quick
            (fun () ->
               let attr =
                 Worker_dev_tools.attribution_of_validation
        ...
        ; Alcotest.test_case
            "all 7 block_reason variants → Policy_failed"
            `Quick
            (fun () ->
               let variants =
               ...
               List.iter
                 (fun br ->
                    let attr =
                      Worker_dev_tools.attribution_of_validation ~cmd:"test" (Error br)
                    in
                    Alcotest.(check bool)
                      "always Policy_failed"
                      true
                      (match attr.outcome with
