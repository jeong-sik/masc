...
- **현재 상태:** rich-text/multimedia meta(attachment 4종)는 저장+렌더됨 **[LANDED]** — `Board_render` 단일 projection 모듈(`lib/board/board_render.ml`, pure) + Discord/Slack send 어댑터(`lib/server/board_render_discord.ml` / `lib/server/board_render_slack.ml` — 발송 페이로드 변환만, 기존 wire type 소비)가 경계 계약대로 한 소스에서 나온다. dashboard는 상세(`post-detail.ts`)·목록 카드(`board-surface.ts`)에서 `meta.attachments` 4종을 렌더한다(`components/board/post-attachments.ts`; decode 실패 항목은 명시적 오류 카드). 단, board→connector **push 경로는 원래 HEAD에 부재**(outbound는 keeper `keeper_surface_post` 매개)이므로 어댑터의 발송 배선은 별도 슬라이스. proactive attention candidate는 producer가 `record_and_wake`로 durable row를 먼저 기록한다. MASC의 lifecycle-sibling worker는 immutable input scheduling과 durable callback/settlement를 소유하고, OAS는 exact provider execution과 failover를 소유한다. Keeper owner lane은 완료된 결과 이벤트를 소비하도록 wake될 뿐 provider 실행을 수행하지 않는다. dashboard projection은 model judge를 실행하지 않는다. claim-evidence gate가 opt-in이라 free-text "task done" 완료주장이 검증 우회.
