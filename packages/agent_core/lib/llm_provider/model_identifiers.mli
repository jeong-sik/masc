(* Distinguishing opaque identifiers for the three string meanings that
   ...
   [Id_prefix.starts_with], and [Id_prefix.matches_model_id], which fold ASCII
   case. [to_string] is the boundary escape hatch (TUI rows, JSON/TOML wire,
   logs) and returns the original bytes. *)
...
module Model_id : sig
  ...
  (** Model ids must not be empty or have leading or trailing whitespace.
...
module Id_prefix : sig
  ...
  val matches_model_id : prefix:t -> Model_id.t -> bool
  (** Prefix matching between a catalog row and a requested model id.
      ASCII case is folded on both sides. These two functions are the only
      sanctioned prefix comparisons on [t]: escaping through {!to_string}
      to run [String.starts_with] is a review flag. *)
