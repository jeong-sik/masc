...
let test_task_status_to_string_done () =
  let status =
    Types.Done { assignee = "claude"; completed_at = "2024-01-01"; notes = None }
  in
...
let test_task_status_to_string_cancelled () =
  let status =
    Types.Cancelled { cancelled_by = "user"; cancelled_at = "2024-01-01"; reason = None }
  in
...
let test_task_status_to_yojson_in_progress () =
  let status =
    Types.InProgress { assignee = "gemini"; started_at = "2024-01-01T12:00:00Z" }
  in
  let json = Types.task_status_to_yojson status in
  match json with
  | `Assoc fields -> check bool "has started_at" true (List.mem_assoc "started_at" fields)
  | _ -> fail "expected Assoc"
;;
...
let test_task_status_to_yojson_done_with_notes () =
  let status =
    Types.Done
      { assignee = "codex"; completed_at = "2024-01-01"; notes = Some "All tests pass" }
  in
  let json = Types.task_status_to_yojson status in
  match json with
  | `Assoc fields ->
...
let test_task_status_to_yojson_done_no_notes () =
  let status =
    Types.Done { assignee = "codex"; completed_at = "2024-01-01"; notes = None }
  in
  let json = Types.task_status_to_yojson status in
  match json with
  | `Assoc fields ->
...
let test_show_task_status_claimed () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "claude") s 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_backlog_to_yojson_empty () =
  let b : Types.backlog =
    { tasks = []; last_updated = "2024-01-15T12:00:00Z"; version = 1 }
  in
...
let test_backlog_to_yojson_with_tasks () =
  ...
  in
  let b : Types.backlog =
    { tasks = [ task ]; last_updated = "2024-01-15T12:00:00Z"; version = 2 }
  in
  let json = Types.backlog_to_yojson b in
  match json with
  | `Assoc fields ->
...
let test_a2a_task_status_to_string_completed () =
  check
    ...
    "completed"
    "completed"
    (Types.a2a_task_status_to_string Types.A2ACompleted)
;;
...
let test_masc_error_task_not_found () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "task-1") s 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_masc_error_task_already_claimed () =
  let s =
    Types.masc_error_to_string (Types.TaskAlreadyClaimed { task_id = "t1"; by = "agent" })
  in
  check
    bool
    "contains owner guidance"
    true
    (try
       let _ = Str.search_forward (Str.regexp "currently owned by agent") s 0 in
       true
     with
     | Not_found -> false)
;;
  ...
  let s =
    Types.masc_error_to_string
      (Types.RateLimitExceeded
         { limit = 100; current = 101; wait_seconds = 5; category = Types.GeneralLimit })
  in
...
let test_agent_credential_of_yojson_role_string_admin () =
  ...
  in
  match Types.agent_credential_of_yojson json with
  | Ok cred -> check bool "role:\"admin\" parses to Admin" true (cred.role = Types.Admin)
  | Error e -> fail ("expected Ok, got: " ^ e)
;;
...
let test_agent_credential_of_yojson_role_string_wins_over_admin_bool () =
  ...
  in
  match Types.agent_credential_of_yojson json with
  | Ok cred -> check bool "role string wins" true (cred.role = Types.Admin)
  | Error e -> fail ("expected Ok, got: " ^ e)
;;
...
let test_has_permission_worker () =
  ...
  check
    bool
    "worker can broadcast"
    true
    (Types.has_permission Types.Worker Types.CanBroadcast);
  check
    bool
    "worker cannot reset"
    false
    (Types.has_permission Types.Worker Types.CanReset)
;;
...
let test_effective_limit () =
  ...
  check
    ...
    "worker general"
    60
    (Types.effective_limit config ~role:Types.Worker ~category:Types.GeneralLimit);
  check
    int
    "admin general"
    120
    (Types.effective_limit config ~role:Types.Admin ~category:Types.GeneralLimit)
;;
...
let test_category_for_tool_task_ops () =
  check
    ...
    "masc_add_task"
    true
    (Types.category_for_tool "masc_add_task" = Types.TaskOpsLimit);
  check
    bool
    "masc_transition"
    true
    (Types.category_for_tool "masc_transition" = Types.TaskOpsLimit)
;;
...
let test_category_for_tool_general () =
  check
    ...
    "masc_status"
    true
    (Types.category_for_tool "masc_status" = Types.GeneralLimit);
...
let test_masc_error_task_not_claimed () =
  ...
  check
    bool
    "contains claim guidance"
    true
    (try
       let _ = Str.search_forward (Str.regexp "Claim/start it first") s 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_masc_error_portal_already_open () =
  let s =
    Types.masc_error_to_string (Types.PortalAlreadyOpen { agent = "a1"; target = "a2" })
  in
  check bool "nonempty" true (String.length s > 0)
;;
...
let test_masc_error_forbidden () =
  let s =
    Types.masc_error_to_string (Types.Forbidden { agent = "a1"; action = "reset" })
  in
  check bool "nonempty" true (String.length s > 0)
;;
...
let test_rate_limit_config_of_yojson_defaults () =
  let json = `Assoc [] in
  match Types.rate_limit_config_of_yojson json with
  | Ok c -> check int "default per_minute" 10 c.per_minute
...
let test_tool_result_to_yojson_success () =
  let r : Types.tool_result =
    { success = true; message = "Operation completed"; data = None }
  in
...
let test_tool_result_to_yojson_with_data () =
  ...
  let json = Types.tool_result_to_yojson r in
  match json with
  | `Assoc fields -> check bool "has data" true (List.mem_assoc "data" fields)
  | _ -> fail "expected Assoc"
;;
...
let test_task_to_yojson_with_worktree () =
  ...
  let t : Types.task =
    ...
    ; description = ""
    ; task_status =
        Types.InProgress { assignee = "claude"; started_at = "2024-01-15T12:00:00Z" }
    ; goal_id = None
    ; priority = 1
    ; files = []
    ; created_at = "2024-01-15T12:00:00Z"
  ...
  let json = Types.task_to_yojson t in
  match json with
  | `Assoc fields -> check bool "has worktree" true (List.mem_assoc "worktree" fields)
  | _ -> fail "expected Assoc"
;;
...
let test_portal_of_yojson_closed () =
  ...
  in
  match Types.portal_of_yojson json with
  | Ok p -> check string "status" "closed" (Types.portal_state_to_string p.portal_status)
  | Error e -> fail ("expected Ok: " ^ e)
;;
...
let () =
  run
    ...
    ; ( "task_status_to_yojson"
      , [ test_case "todo" `Quick test_task_status_to_yojson_todo
        ...
        ; test_case
            ...
            `Quick
            test_task_status_to_yojson_cancelled_with_reason
        ] )
    ...
    ; "string_of_tempo_mode", [ test_case "alias" `Quick test_string_of_tempo_mode ]
    ...
    ; ( "agent_credential"
      , [ test_case "to_yojson" `Quick test_agent_credential_to_yojson
        ; test_case
            "to_yojson with expiry"
            `Quick
            test_agent_credential_to_yojson_with_expiry
    ...
    ; ( "category_for_tool"
      , [ test_case "broadcast" `Quick test_category_for_tool_broadcast
        ...
        ; test_case
            "plan_slot_writes (#8873)"
            `Quick
            test_category_for_tool_plan_slot_writes
