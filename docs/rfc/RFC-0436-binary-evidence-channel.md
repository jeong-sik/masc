...
- judge 전달 상한도 같은 값 하나다(단계 2 구현이 확정한 설계). 스토어가
  캡처하지 않을 바이트는 judge 에게도 전달하지 않는다(#27397 의 "하나의 값"
  규칙). 전달 상한을 넘는 본문은 잘라 보내지 않고 참조+해시로 강등한다.
...
- **base64 를 스냅숏 content 에 그대로 실는 것** — 스냅숏 JSON 이 바이트의
  4/3 팽창을 떠안고, judge 가 base64 를 텍스트로 읽는 일이 생긴다. 본문은
  evidence 저장소, 명세는 스냅숏이라는 분리를 지킨다.
