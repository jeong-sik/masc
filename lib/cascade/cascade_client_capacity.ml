  ...
  with_lock registry_mu (fun () ->
    ...
    | Some existing ->
      ...
      then
        ...
        Hashtbl.replace registry url { max_concurrent; active = existing.active })
;;

let registered_urls () =
  with_lock registry_mu (fun () -> Hashtbl.fold (fun url _ acc -> url :: acc) registry [])
;;
...
let unregister_all () = with_lock registry_mu (fun () -> Hashtbl.clear registry)
let lookup url = with_lock registry_mu (fun () -> Hashtbl.find_opt registry url)
  ...
  | Some e ->
    ...
    let rec attempt () =
      ...
      then (
        ...
        None)
      else if Atomic.compare_and_set e.active current (current + 1)
      then (
        Cascade_client_capacity_history.record
          { ts = Unix.gettimeofday ()
          ; key = url
        ...
        let release () =
          if not (Atomic.exchange released true)
          then (
...
let ollama_default_max () = max 1 (int_of_env ~default:1 "MASC_OLLAMA_MAX_CONCURRENT")
let cli_default_max () = max 1 (int_of_env ~default:1 "MASC_CLI_MAX_CONCURRENT")
...
let capacity_entry_error_message = function
  | Missing_separator raw -> Printf.sprintf "missing '=' in %S" raw
  | Empty_url raw -> Printf.sprintf "empty url in %S" raw
  | Invalid_capacity raw -> Printf.sprintf "invalid capacity %S" raw
  | Non_positive_capacity value -> Printf.sprintf "capacity must be >= 1 (got %d)" value
;;
...
let parse_capacity_entry raw_item =
  ...
  then Ok None
    ...
    | Some idx ->
      ...
      let raw_capacity =
        String.trim (String.sub item (idx + 1) (String.length item - idx - 1))
      in
      if url = ""
      then Error (Empty_url item)
      else (
...
let parse_capacity_env s =
  ...
  |> List.filter_map (fun item ->
    ...
    | Error err ->
      Log.Misc.warn
        "Ignoring invalid MASC_CLIENT_CAPACITY entry: %s"
...
let auto_register_for_candidates ~base_urls =
  ...
  List.iter
    (fun url ->
       if looks_like_ollama url && not (is_registered url)
       then register ~url ~max_concurrent)
    base_urls
;;
...
let auto_register_cli_for_candidates ~capacity_keys =
  ...
  List.iter
    (fun key ->
       if looks_like_cli_sentinel key && not (is_registered key)
       then register ~url:key ~max_concurrent)
    capacity_keys
;;
