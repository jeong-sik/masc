...
let gemini_cli_auto_models () =
  Provider_adapter.auto_models_for_cascade_prefix "gemini_cli" |> Option.value ~default:[]
;;

let codex_cli_auto_models () =
  Provider_adapter.auto_models_for_cascade_prefix "codex_cli" |> Option.value ~default:[]
;;
...
let kimi_cli_auto_models () =
  Provider_adapter.auto_models_for_cascade_prefix "kimi_cli" |> Option.value ~default:[]
;;
...
let resolve_glm_model ?getenv model_id =
  ...
  let resolved_model_id =
    Llm_provider.Zai_catalog.resolve_glm_alias
      ...
      model_id
  in
  if String.equal model_id "auto"
  then { default_model with resolved_model_id }
  else if String.equal resolved_model_id model_id
...
let resolve_glm_model_id model_id = (resolve_glm_model model_id).resolved_model_id
...
let resolve_auto_model
  ...
  | Some _ ->
    if String.equal model_id "auto"
    then default_resolution ?getenv provider_name ~requested_model_id:model_id
    else explicit_resolution model_id model_id
  | None ->
    if String.equal model_id "auto"
    then unresolved_auto model_id
    else explicit_resolution model_id model_id
;;
