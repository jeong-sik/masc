...
| Team Session | `masc_team_session_start`, `masc_team_session_status`, `masc_team_session_turn`, `masc_team_session_events`, `masc_team_session_prove` | 1시간 협업 오케스트레이션 + 체크포인트 + 증명 가능한 팀 리포트 |
...
기본 `tools/list`는 canonical tool만 우선 노출하며, `masc_claim`, `experiment_*`, `masc_trpg_*`, low-usage social/vote utility 같은 convenience surface는 숨겨집니다. 직접 호출 호환성은 유지됩니다.
`tools/list`는 `include_hidden`, `include_deprecated`, `include_usage`를 지원합니다. `include_usage=true`이면 room telemetry(`.masc/telemetry.jsonl`) 기준 usage count/success/failure/lastUsedAt 메타가 같이 반환됩니다.
