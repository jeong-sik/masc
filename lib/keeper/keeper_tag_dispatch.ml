...
let dispatch
  ...
  try
    ...
    | Mod_a2a -> Tool_a2a.dispatch { Tool_a2a.config; agent_name } ~name ~args
    ...
    | Mod_operator ->
      ...
      Some
        ( false
        , Printf.sprintf
            "tool '%s' belongs to the removed operator surface; keeper runtime stays on \
             OAS Agent.run"
            name )
    | Mod_autoresearch ->
  ...
  | exn ->
    ...
    Some (false, Printf.sprintf "keeper dispatch error for %s: %s" name exn_type)
;;
