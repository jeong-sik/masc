...
export function ConnectionStatus() {
  ...
  return html`
    ...
    >
      <span class="inline-block size-[8px] rounded-sm ${isConnected ? 'bg-[var(--ok)] shadow-[0_0_7px_rgba(74,222,128,0.75)]' : 'bg-[var(--bad)]'}" aria-hidden="true"></span>
...
export function ErrorCounterBadge() {
  ...
  return html`
    <div class="relative" role="status">
      ...
      ${open ? html`<div id="error-panel"><${ErrorPanel} onClose=${() => { errorPanelOpen.value = false }} /></div>` : null}
    </div>
  `
}
  ...
  return html`
    <div class="relative">
      ...
      ${buildIdentityOpen.value
        ? html`
            <div id="build-identity-panel" class="absolute top-[calc(100%+8px)] right-0 min-w-70 rounded border border-solid border-[var(--card-border)] bg-[var(--bg-panel)] px-3 py-2.5 shadow-[0_10px_24px_rgba(0,0,0,0.22)] grid gap-1.5">
...
export function SideRail({ collapsed, onToggle }: { collapsed?: boolean; onToggle?: () => void }) {
  ...
  return html`
    <nav class="flex flex-col h-full" aria-label="대시보드 탐색">
  ...
  return html`
    <div class="mb-3 flex flex-col gap-1.5">
      ...
      <div class="flex items-center gap-2">
        <h1 class="text-[22px] font-bold tracking-tight text-[var(--text-strong)]">
          ${title}
        </h1>
        ${shareUrl !== ''
          ? html`<${CopyIdButton}
