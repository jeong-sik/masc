...
let of_broadcast ~agent_name ~message ~message_id : event list =
  ...
  ; make_event
      ...
      ~delta:(Some message)
      Text_message_content
  ; make_event ~thread_id ~run_id:(Some agent_name) ~message_id:mid Text_message_end
  ]
;;
...
let of_room_state (state : Yojson.Safe.t) : event =
  make_event ~thread_id:default_thread_id ~snapshot:(Some state) State_snapshot
;;
...
let of_task_update (task_json : Yojson.Safe.t) : event =
  ...
  match status with
  | "claimed" -> of_task_claimed ~agent_name:agent ~task_id
  | "done" -> of_task_done ~agent_name:agent ~task_id
  | _ ->
    of_custom
      ~name:"TASK_UPDATE"
      (`Assoc
          [ "taskId", `String task_id; "status", `String status; "agent", `String agent ])
;;
