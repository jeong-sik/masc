(** Otel_metric_store adapter for neutral Board metric hooks.
    ...
    typed label dimensions on {!Masc_board_handlers.Board_metrics_hooks.observer}. The emitted
...
let board_persist_surface_to_label :
  Masc_board_handlers.Board_metrics_hooks.board_persist_surface -> string = function
  | Board_post_meta_json -> "board_post_meta_json"

(* outcome label for masc_board_dispatch_flusher_start_outcomes_total *)
let flusher_outcome_to_label : Masc_board_handlers.Board_metrics_hooks.flusher_outcome -> string =
  function
...
let install () =
  Masc_board_handlers.Board_metrics_hooks.set_observer
    {
