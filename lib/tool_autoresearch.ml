...
let persisted_summary_json (summary : Autoresearch.persisted_summary) =
  `Assoc
    ...
    ; ( "target_reached"
      , `Bool
          (match summary.target_score with
           | None -> false
           | Some target ->
             if summary.lower_is_better
             then summary.best_score <= target
    ...
    ; "queued_hypothesis", Json_util.string_opt_to_json summary.queued_hypothesis
    ...
    ; "recent_cycles", `List []
    ; "program_note", Json_util.string_opt_to_json summary.program_note
...
let prepare_start_params (ctx : context) args =
  ...
  | Ok model_model ->
    if goal = ""
    then Error "goal is required"
    else if metric_fn = ""
    then Error "metric_fn is required"
    else if target_file = ""
    then Error "target_file is required"
    else (
...
let prepare_managed_target_file ~source_workdir ~managed_workdir target_file =
  ...
  | Error managed_error ->
    (match Autoresearch.validate_target_file ~workdir:source_workdir target_file with
     | Error _ -> Error (Printf.sprintf "Invalid target_file: %s" managed_error)
     | Ok source_abs ->
       (match
          Autoresearch.resolve_target_file_path ~workdir:managed_workdir target_file
        with
        | Error path_error -> Error (Printf.sprintf "Invalid target_file: %s" path_error)
        | Ok managed_abs ->
          (try
             Fs_compat.mkdir_p (Filename.dirname managed_abs);
             Fs_compat.save_file managed_abs (Autoresearch.read_file source_abs);
...
let setup_running_loop (ctx : context) (params : start_params) =
  let state =
    Autoresearch.create_state
      ~goal:params.goal
      ~metric_fn:params.metric_fn
      ?author:ctx.agent_name
      ~model_model:params.model_model
      ~target_file:params.target_file
      ?target_score:params.target_score
      ~cycle_timeout_s:params.cycle_timeout_s
      ~max_cycles:params.max_cycles
      ?patience:params.patience
      ?build_verify_fn:params.build_verify_fn
      ~lower_is_better:params.lower_is_better
      ~workdir:params.source_workdir
      ()
  in
  match
    Autoresearch.prepare_managed_worktree
      ~base_path:ctx.base_path
      ~source_workdir:params.source_workdir
      ~loop_id:state.loop_id
  with
  | Error message -> Error message
  | Ok (managed_workdir, source_workdir, warnings) ->
    (match
       prepare_managed_target_file ~source_workdir ~managed_workdir params.target_file
     with
     | Error message -> Error message
     | Ok target_file_warnings ->
       ...
       let baseline_result =
         ...
         | None ->
           (match
              Autoresearch.measure_metric
                ~workdir:managed_workdir
                ~timeout_s:params.cycle_timeout_s
                params.metric_fn
            with
            | Ok (baseline, _ms) -> Ok baseline
            | Error e -> Error (Printf.sprintf "Failed to measure baseline: %s" e))
       in
...
let status_json (ctx : context) ~loop_id json_fields =
  ...
  let base_fields =
    ...
    | `Assoc fields ->
      strip_keys
        [ "session_id"; "operation_id"; "task_id"; "program_note"; "queued_hypothesis" ]
        fields
    | _ -> [ "error", `String "invalid status payload" ]
  in
  let link = Autoresearch.load_execution_link_by_loop ~base_path:ctx.base_path loop_id in
  ...
  let link_fields =
    ...
    | None -> [ "session_id", `Null; "operation_id", `Null; "task_id", `Null ]
  in
  `Assoc
    (base_fields
     @ link_fields
...
let build_swarm_goal ~goal ~target_file ~program_note =
  ...
  | Some note ->
    Printf.sprintf
      ...
      target_file
      note
  | None -> Printf.sprintf "Autoresearch swarm goal: %s\nTarget file: %s" goal target_file
;;
...
let check_concurrency_limit (ctx : context) =
  ...
  | Some author ->
    ...
    then
      Error
        (Printf.sprintf
           "Concurrency limit reached: author '%s' already has %d active loops. Please \
            stop or wait for them to finish."
           author
           active_count)
    else Ok ()
;;
...
let handle_stop (ctx : context) args =
  ...
  | Some id ->
    ...
    then
      `Assoc
        [ "error", `String "Access denied: you can only stop loops that you started." ]
    else (
...
let handle_inject (ctx : context) args =
  ...
  if hypothesis = ""
  then `Assoc [ "error", `String "hypothesis is required" ]
  else (
    match resolve_loop_id args with
    | None -> `Assoc [ "error", `String "No autoresearch loop running" ]
    | Some id ->
      Autoresearch.with_loops_rw (fun () ->
        ...
        | Some state ->
          ...
          then `Assoc [ "error", `String "Loop is not running" ]
          else (
...
let parse_limit_arg args =
  ...
  | Ok limit ->
    ...
    then Error "limit must be an integer between 1 and 100"
    else Ok limit
;;
  ...
  let rec loop acc invalid = function
    | [] ->
      ...
      then Ok (List.rev acc)
        ...
        Error
          (Printf.sprintf
             "%s are required and must be non-empty strings"
             (String.concat ", " keys))
    | key :: rest ->
      (match arg_member key args with
       | Some (`String value) ->
         ...
         if trimmed = ""
         then loop acc (key :: invalid) rest
    ...
    (match parse_optional_int_arg "cycle_end" args with
     ...
     | Ok cycle_end ->
       (match cycle_start, cycle_end with
        ...
        | Some a, Some b when a > b -> Error "cycle_start must be <= cycle_end"
...
let handle_record_finding (ctx : context) args =
  ...
  in
  match
    parse_required_trimmed_strings [ "goal"; "hypothesis"; "evidence"; "conclusion" ] args
  with
  | Error msg -> `Assoc [ "error", `String msg ]
  | Ok required ->
    (match parse_confidence_arg args, parse_tags_arg args, parse_cycle_range_arg args with
...
let handle_search_findings (ctx : context) args =
  ...
  if query = ""
  then `Assoc [ "error", `String "query is required" ]
  else (
    match parse_limit_arg args with
    | Error msg -> `Assoc [ "error", `String msg ]
    | Ok limit ->
      let findings =
        Autoresearch_knowledge.search_findings ~base_path:ctx.base_path ~query ~limit ()
      in
      `Assoc
        [ "ok", `Bool true
...
let dispatch (ctx : context) ~name ~args : tool_result option =
  ...
  | "masc_autoresearch_cycle" ->
    Some (wrap_result (Tool_autoresearch_cycle.handle_cycle ctx args))
...
let tool_required_permission = function
  | "masc_autoresearch_status" -> Some Types.CanReadState
  | "masc_autoresearch_search_findings" -> Some Types.CanReadState
  | "masc_autoresearch_record_finding"
  | "masc_autoresearch_start"
  | "masc_autoresearch_cycle"
  | "masc_autoresearch_inject"
  | "masc_autoresearch_stop" -> Some Types.CanAdmin
  | _ -> None
;;
