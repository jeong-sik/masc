...
let test_target_score_reaches_completion_higher_is_better () =
  with_temp_dir "masc_autoresearch_target_high"
  @@ fun root ->
  with_base_path root
  @@ fun () ->
  with_eio
  ...
  write_file
    (Filename.concat repo "metric.py")
    "import pathlib, sys\n\
  ...
  check bool "target reached in result" true (result |> member "target_reached" |> to_bool);
  let final_state =
  ...
  check
    string
    "loop marked completed"
    "completed"
    (Lib.Autoresearch.status_to_string final_state.status);
  check bool "target reached in state" true (Lib.Autoresearch.target_reached final_state)
;;

let test_target_score_reaches_completion_lower_is_better () =
  with_temp_dir "masc_autoresearch_target_low"
  @@ fun root ->
  with_base_path root
  @@ fun () ->
  with_eio
...
let test_omitted_target_score_preserves_running_status () =
  with_temp_dir "masc_autoresearch_target_none"
  @@ fun root ->
  with_base_path root
  @@ fun () ->
  with_eio
  ...
  check
    string
    "cycle status remains running"
    "running"
    (result |> member "status" |> to_string);
  check
    bool
    "target not reported reached"
    false
...
let () =
  run
    "autoresearch_target_score"
    [ ( "target_score"
      , [ test_case
            "target score completes higher-is-better loop"
            `Quick
            test_target_score_reaches_completion_higher_is_better
        ; test_case
            "target score completes lower-is-better loop"
            `Quick
            test_target_score_reaches_completion_lower_is_better
        ; test_case
            "omitted target score preserves running status"
            `Quick
