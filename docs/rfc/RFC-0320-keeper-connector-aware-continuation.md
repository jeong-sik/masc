...
2. 승인 큐에 제출 — entry는 `turn_id` / `task_id`만 캡처. chat connector / thread는 캡처 안 함.
...
| approval entry에 커넥터 없음 | `keeper_approval_queue` (`create_entry`) | `turn_id`/`task_id`(+sandbox/runtime/model)만. chat connector/thread 필드 부재. |
...
|---|---|---|---|---|---|---|
| **G1** provenance capture | entry/stimulus가 turn/task만 | originating channel 미캡처 | Connector가 발행한 opaque `Channel_ref.t` 또는 `Unrouted`를 승인 제출·mention 접수 시점에 캡처 | `keeper_approval_queue`(create_entry), `keeper_chat_queue`, `connector_attention` | 제출 시 channel ref 왕복 테스트; 없으면 Unrouted | 제품별 좌표 해석은 Connector 소유 |
...
(* 승인 entry: turn/task만, chat connector 필드 없음 *)
...
(* create_entry … ~turn_id ~task_id ~channel  (제출 시점 캡처, resolve까지 관통) *)
```
