...
(** [actual_input_tokens] is provider-reported and only known after a response.
    ...
    and [input_messages] are the messages that existed immediately before the
    terminal provider response. [attributed_bytes] sums only these exact
    content values; provider serialization metadata is not estimated. *)
