...
**Date**: 2026-05-12
**Spec**: `specs/keeper-state-machine/KeeperRuntimeAttemptFSM.tla` (263 LOC, RFC-0065 Phase 5.1 B1)
**OCaml**: `lib/runtime/runtime_fsm.ml:7-21` (typed surface), `lib/keeper/keeper_turn_driver.ml::try_runtime` (recursive walker), `lib/keeper/keeper_turn_slot.ml::with_keeper_turn_slot` (slot guard)
**Risk**: MID — spec models 3 sub-classes of `Call_err` (runtimeable / terminal / hard_quota) that the OCaml side collapses into ONE `Call_err` constructor + runtime classifier.  The classification gap is invisible to the R-B-1.c drift validator.
...
| ProviderOutcomes (CONSTANT) | 6: `call_ok`, `call_err_runtimeable`, `call_err_terminal`, `call_err_hard_quota`, `accept_rejected`, `slot_full` | cfg-supplied |
| Actions (Next) | 6: `StartRuntime`, `SendRequest`, `ResolveSuccess`, `ResolveTryNext`, `ResolveExhaustedNormal`, `ResolveHardQuota` | line 108-182 |
...
```ocaml
(* lib/runtime/runtime_fsm.ml:7-13 *)
...
(* lib/runtime/runtime_fsm.ml:15-20 *)
type decision =
...
Phase machine is *implicit* — `attempt_phase` lives only in `try_runtime` recursion structure, not as a typed variable.  The spec is *more explicit* than OCaml on the FSM topology.
...
- `call_err_runtimeable` → drives `ResolveTryNext` (when not last tier)
...
```ocaml
(* lib/runtime/runtime_fsm.ml:47-52 *)
| Call_err err ->
  let should_runtime = Runtime_health_filter.should_runtime_to_next err in
  if should_runtime then Try_next { last_err = Some err }
...
**Consequence**: Bug `BugHardQuotaBypass` (spec line 191) maps to "OCaml accidentally removes the hard_quota override branch in keeper_turn_driver.ml".  But the change site is *outside* `runtime_fsm.ml` — a code review reading only `runtime_fsm.ml` cannot detect the bug.  Spec captures the cross-file coupling that OCaml doesn't.
...
| ID | Direction | Risk |
|---|---|---|
| R-D-1.a | OCaml — split `Call_err` into typed sub-variants: `Call_err_runtimeable of http_error`, `Call_err_terminal of http_error`, `Call_err_hard_quota of http_error`.  Push classification (`should_runtime_to_next`, `sdk_error_is_hard_quota`) from runtime predicates to constructor selection.  Spec's 3-way alphabet becomes OCaml's 3-way variant. | MID — touches `runtime_fsm.ml` + `keeper_turn_driver.ml` + every `Call_err _` pattern match in callers. |
...
- `Runtime_health_filter.should_runtime_to_next` and `sdk_error_is_hard_quota` are well-tested predicates with corpus coverage on real provider responses.  Reclassification regressions would surface in those test suites before reaching the FSM-level invariants.
- The 3-BugAction model in KCAF's `-buggy.cfg` *does* catch the canonical regressions (hard-quota bypass, mid-runtime slot release, off-by-one tier walker).  Coverage of those bugs is verified at TLC level on every spec-touching PR.
...
| KCT C-3 (#14783) | `"none"` literal for terminal | `base_runtime` return | OCaml more permissive |
| KCT S2/S3 (#14787) | string-literal runtime names | catalog-resolved names | OCaml more configurable |
...
- `lib/runtime/runtime_fsm.ml:7-21` (typed surface).
- `lib/runtime/runtime_fsm.ml:25-52` (`decide` function — runtime classification site).
