...
let setup_task_with_worktree
  ...
  ignore
    (Coord.add_task
       config
       ~title:"gh binding test"
       ~priority:1
       ~description:"validate keeper gh context");
  let task =
    match Coord.get_tasks_safe config with
    | [ task ] -> task
    | tasks -> Alcotest.failf "expected exactly one task, got %d" (List.length tasks)
  in
       ...
       | Some (`List tasks) ->
         let update_task json =
           ...
           | `Assoc task_fields ->
             ...
             then
               `Assoc
                 (("worktree", worktree_json) :: List.remove_assoc "worktree" task_fields)
             else json
           | _ -> json
         in
         `Assoc
           (("tasks", `List (List.map update_task tasks))
            :: List.remove_assoc "tasks" fields)
...
let test_legacy_keeper_unaffected () =
  ...
  in
  Alcotest.(check bool)
    "legacy bypasses symmetric containment"
    false
...
let test_docker_keeper_blocks_ls_outside () =
  ...
  let raw =
    Keeper_exec_shell.handle_keeper_shell
      ...
      ~config
      ~meta
      ~args:(`Assoc [ "op", `String "ls"; "path", `String outside_dir ])
  in
  Alcotest.(check bool)
    "ls outside playground blocked"
    true
    (blocked_by_symmetric_sandbox raw)
;;
...
let test_docker_keeper_blocks_cat_outside () =
  ...
  in
  Alcotest.(check bool)
    "cat outside playground blocked"
    true
    (blocked_by_symmetric_sandbox raw)
;;
...
let test_docker_keeper_blocks_rg_outside () =
  ...
  ensure_dir outside_dir;
  ignore
    (Fs_compat.save_file_atomic (Filename.concat outside_dir "leak.txt") "secret-token");
  let raw =
    Keeper_exec_shell.handle_keeper_shell
      ~turn_sandbox_runtime:None
      ~exec_cache:None
...
let test_docker_keeper_blocks_find_outside () =
  ...
  in
  Alcotest.(check bool)
    "find outside playground blocked"
    true
    (blocked_by_symmetric_sandbox raw)
;;
...
let test_docker_keeper_allows_inside_playground () =
  ...
  (* Goal: containment did not block. Whether `cat` succeeds depends on
     ...
     is silent. *)
  Alcotest.(check bool)
    "playground-internal cat not blocked"
    false
    (blocked_by_symmetric_sandbox raw)
;;
...
let test_docker_git_creds_contained () =
  ...
  in
  Alcotest.(check bool)
    "docker git-creds also contained"
    true
    (blocked_by_symmetric_sandbox raw)
;;
...
let test_gh_binds_repo_from_active_task_worktree () =
  ...
  let meta, _task_id =
    setup_task_with_worktree ~base ~config ~meta ~git_root ~worktree_rel
  in
  ...
  write_executable
    ...
    "#!/bin/sh\n\
     ...
     echo fake-gh-ok\n";
  with_env "GH_ARGS_FILE" gh_args_file
  @@ fun () ->
  with_env "GH_PWD_FILE" gh_pwd_file
  @@ fun () ->
  with_env "PATH" (bin_dir ^ ":" ^ existing_path ())
  @@ fun () ->
  let raw =
    Keeper_exec_shell.handle_keeper_shell
      ~turn_sandbox_runtime:None
      ...
      ~config
      ~meta
      ~args:(`Assoc [ "op", `String "gh"; "cmd", `String "pr list --state open" ])
  in
...
let () =
  Alcotest.run
    "Keeper_shell_containment"
    [ ( "containment"
      , [ Alcotest.test_case
            "legacy keeper unaffected"
            `Quick
            test_legacy_keeper_unaffected
        ; Alcotest.test_case
            "docker keeper blocks ls outside"
            `Quick
            test_docker_keeper_blocks_ls_outside
        ; Alcotest.test_case
            "docker keeper blocks cat outside"
            `Quick
            test_docker_keeper_blocks_cat_outside
        ; Alcotest.test_case
            "docker keeper blocks rg outside"
            `Quick
            test_docker_keeper_blocks_rg_outside
        ; Alcotest.test_case
            "docker keeper blocks find outside"
            `Quick
            test_docker_keeper_blocks_find_outside
        ; Alcotest.test_case
            "docker keeper allows inside playground"
            `Quick
            test_docker_keeper_allows_inside_playground
        ; Alcotest.test_case
            "docker git-creds also contained"
            `Quick
            test_docker_git_creds_contained
        ; Alcotest.test_case
            "gh binds repo from active task worktree"
            `Quick
            test_gh_binds_repo_from_active_task_worktree
        ; Alcotest.test_case
            "gh missing worktree returns typed error"
            `Quick
