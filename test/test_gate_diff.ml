...
let test_simple_ls_agrees () = check string "ls agree" "agree" (diff_tag "ls")
  ...
  List.iter
    (fun (pat, desc) ->
       ...
       | W.Legacy_deny_shadow_allow ->
         fail
           (Printf.sprintf "flip blocker: %S (%s) is legacy_deny_shadow_allow" pat desc)
       | W.Legacy_allow_shadow_deny ->
         fail
           (Printf.sprintf "inverted gap: %S (%s) is legacy_allow_shadow_deny" pat desc))
    patterns
;;
