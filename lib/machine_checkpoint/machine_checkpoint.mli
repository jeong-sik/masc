...
(** {!read} without the machine bytes: the same header check, checksum and
    decompression, but the decompressed body's tail is never sliced into a
    second string. For a caller that wants to say what a checkpoint is --
    not restore it -- so it never holds a copy of the guest's memory just to
    read who saved it. [modified] is the file's own mtime, the slot's
