...
let test_unread_checkpoint_leaves_both_stores_untouched () =
  with_keeper ~paused:false ~install_owner:true
  ...
  in
  Fs_compat.save_file checkpoint_path "{not-json";
  check_refused (clear ());
  check string
    "unread checkpoint bytes stay untouched"
    "{not-json"
    (Fs_compat.load_file checkpoint_path);
  check bool
    "unread checkpoint keeps the official-client session"
    true
    (Option.is_some (official_session ()))
;;
