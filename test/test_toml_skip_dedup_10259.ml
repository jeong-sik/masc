...
let test_repeat_call_suppressed () =
  K.reset_logged_toml_skip_for_test ();
  let _ =
    K.log_toml_skip_once ~file:"janitor.toml" ~error:"invalid cascade_name 'ollama_only'"
  in
  let second =
    K.log_toml_skip_once ~file:"janitor.toml" ~error:"invalid cascade_name 'ollama_only'"
  in
...
let test_distinct_files_both_emit () =
  K.reset_logged_toml_skip_for_test ();
  let a = K.log_toml_skip_once ~file:"janitor.toml" ~error:"E" in
  let b = K.log_toml_skip_once ~file:"taskmaster.toml" ~error:"E" in
...
let test_distinct_errors_both_emit () =
  ...
  in
  let b =
    K.log_toml_skip_once ~file:"janitor.toml" ~error:"invalid cascade_name 'big_three'"
  in
