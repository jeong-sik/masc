...
let counter_value name = Prometheus.metric_value_or_zero name ()

(* ── Provider prefix cache counter tests ──────────────── *)

let test_prefix_cache_creation_counter () =
  let before = counter_value "masc_provider_prefix_cache_creation_tokens_total" in
  Prometheus.inc_counter
    "masc_provider_prefix_cache_creation_tokens_total"
    ~delta:1500.0
    ();
  let after = counter_value "masc_provider_prefix_cache_creation_tokens_total" in
  let diff = after -. before in
  check (float 0.1) "creation counter incremented by 1500" 1500.0 diff
;;

let test_prefix_cache_read_counter () =
  let before = counter_value "masc_provider_prefix_cache_read_tokens_total" in
  Prometheus.inc_counter "masc_provider_prefix_cache_read_tokens_total" ~delta:3200.0 ();
  let after = counter_value "masc_provider_prefix_cache_read_tokens_total" in
  let diff = after -. before in
  check (float 0.1) "read counter incremented by 3200" 3200.0 diff
;;
...
let test_prefix_cache_zero_no_increment () =
  ...
  let before_creation =
    counter_value "masc_provider_prefix_cache_creation_tokens_total"
  in
  let before_read = counter_value "masc_provider_prefix_cache_read_tokens_total" in
  ...
  then
    Prometheus.inc_counter
      ...
      ();
  let after_creation = counter_value "masc_provider_prefix_cache_creation_tokens_total" in
  let after_read = counter_value "masc_provider_prefix_cache_read_tokens_total" in
...
let test_response_cache_increment () =
  ...
  check (float 0.1) "response cache hit counter incremented" 1.0 (after -. before)
;;
...
let test_two_layer_independence () =
  ...
  let prefix_before = counter_value "masc_provider_prefix_cache_read_tokens_total" in
  Prometheus.inc_counter "masc_inference_cache_hits_total" ();
  let prefix_after = counter_value "masc_provider_prefix_cache_read_tokens_total" in
  check (float 0.1) "prefix cache unaffected by response cache" prefix_before prefix_after;
  (* Incrementing provider prefix cache should not affect response cache *)
  let response_before = counter_value "masc_inference_cache_hits_total" in
  Prometheus.inc_counter "masc_provider_prefix_cache_read_tokens_total" ~delta:100.0 ();
  let response_after = counter_value "masc_inference_cache_hits_total" in
  check
    (float 0.1)
    "response cache unaffected by prefix cache"
    response_before
    response_after
;;
...
let test_cache_metrics_in_export () =
  ...
  check
    bool
    "export has prefix creation metric"
    true
    (has "masc_provider_prefix_cache_creation_tokens_total");
  check
    bool
    "export has prefix read metric"
    true
...
let () =
  run
    "cache_metrics_wiring"
    [ ( "provider_prefix_cache"
      , [ test_case
            "creation counter increments"
            `Quick
            test_prefix_cache_creation_counter
        ; test_case "read counter increments" `Quick test_prefix_cache_read_counter
        ; test_case "zero tokens no increment" `Quick test_prefix_cache_zero_no_increment
        ] )
    ; ( "response_cache"
      , [ test_case "counters registered" `Quick test_response_cache_counters_registered
        ; test_case "hit counter increments" `Quick test_response_cache_increment
        ] )
    ; ( "two_layer_distinction"
      , [ test_case "layers are independent" `Quick test_two_layer_independence ] )
    ; ( "prometheus_export"
      , [ test_case "cache metrics in export text" `Quick test_cache_metrics_in_export ] )
    ]
;;
