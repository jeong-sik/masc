...
(* Invariant for this predicate: the exemption gate is
   ...
   - transient network: initially exempt from Keeper crash accounting. Rotation
     does not apply ([recoverable_runtime_failure_reason] returns [None] for
     ...
     urgency-lane tail, so it cannot monopolize other independent queued
     sources. The source is retained with a new incarnation and may be retried
     after independent work. The exemption is bounded per keeper
     ([Keeper_unified_turn_failure.transient_transport_exemption_budget]), so a
     persistent fleet-wide transport outage becomes durable failing evidence
     instead of retrying forever with the crash counter pinned at zero.
