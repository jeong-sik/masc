...
let activity_log_file () =
  ...
  | Some root ->
    Filename.concat
      ...
      (Filename.concat "logs" "auto-responder.log")
  | None -> "/tmp/auto-responder.log"
;;

let debug_log msg =
  let line = Printf.sprintf "[%f] %s\n" (Time_compat.now ()) msg in
  try Fs_compat.append_file "/tmp/auto_debug.log" line with
...
let activity_log ~mode ~from_agent ~mention ~status ~detail =
  ...
  let line =
    Printf.sprintf
      ...
      detail
  in
  try Fs_compat.append_file log_file line with
  | Sys_error _ | Unix.Unix_error _ -> ()
;;
...
let with_throttle_lock f =
  ...
  Fun.protect ~finally:(fun () -> Stdlib.Mutex.unlock response_times_mu) f
;;
...
let should_throttle ~agent_type =
  ...
  with_throttle_lock (fun () ->
    ...
    in
    if List.length recent >= chain_limit
    then (
      debug_log
        (Printf.sprintf
           "THROTTLE: %s has %d responses in last %.0fs"
           agent_type
           (List.length recent)
           chain_window_sec);
...
let build_response_prompt ~from_agent ~content ~mention =
  Printf.sprintf
    {|You received a mention in the MASC room from %s.
...
let cli_argv_of_agent_type (agent_type : string) : string list =
  ...
  | Some config ->
    String.split_on_char ' ' config.command |> List.filter (fun s -> s <> "")
  | None -> [ agent_type ]
;;

let run_cli_agent ~agent_type ~prompt =
  if Provider_adapter.is_bare_ollama_label agent_type
  then debug_log (Provider_adapter.bare_ollama_migration_message ())
  else (
    let base = cli_argv_of_agent_type agent_type in
    let argv = base in
    let raw_source = String.concat " " (List.map Filename.quote argv) in
    debug_log
      (Printf.sprintf "SPAWN argv=%s" (String.concat " " (List.map Filename.quote argv)));
    let status, output =
...
let cascade_name_for_agent_type agent_type = Printf.sprintf "auto_responder_%s" agent_type
...
let model_response_is_valid (resp : Oas_response.api_response) =
  ...
  | Oas.Types.EndTurn
  | Oas.Types.MaxTokens
  | Oas.Types.StopSequence
  | Oas.Types.StopToolUse
  ...
  try
    match
      Masc_oas_bridge.run_with_caller
        ~caller:Env_config_oas_bridge.Auto_responder
        (fun () ->
           Oas_worker.run_named
             ~cascade_name
             ~goal:prompt
             ~max_turns:1
             ~accept:model_response_is_valid
             ~max_tokens:500
    ...
    | Ok result ->
      ...
      debug_log (Printf.sprintf "MODEL_USED %s for agent_type=%s" resp.model agent_type);
...
let masc_call ~sw:_ ~tool_name ~(args : Yojson.Safe.t) : (string, string) result =
  ...
  | Some net ->
    ...
    in
    (match Masc_http_client.post_sync ~net ~url:(Uri.to_string uri) ~headers ~body () with
     | Error e -> Error e
     | Ok (code, body_str) ->
       if not (Cohttp.Code.is_success code)
       then Error (Printf.sprintf "MASC HTTP %d" code)
       else (
         ...
         | exn ->
           Log.Misc.warn
             "auto_responder: MCP response parse failed: %s"
...
let call_model_and_broadcast ~sw ~agent_type ~prompt ~mention =
  ...
  debug_log
    (Printf.sprintf
       "MODEL_RESPONSE: %s"
       (String_util.utf8_safe ~max_bytes:103 ~suffix:"..." response
        |> String_util.to_string));
  if response = "" || response = "no response"
  then Log.AutoResponder.info "MODEL returned empty response"
  else (
    let join_args =
      `Assoc
    ...
    | Ok join_resp ->
      debug_log
        (Printf.sprintf
           "MASC_JOIN: %s"
           (String_util.utf8_safe ~max_bytes:203 ~suffix:"..." join_resp
            |> String_util.to_string));
      (match extract_nickname join_resp with
       ...
       | Some nickname ->
         ...
         let broadcast_args =
           `Assoc [ "agent_name", `String nickname; "message", `String msg ]
         in
         (try
            ignore
              (masc_call
         ...
         let short_resp =
           String_util.utf8_safe ~max_bytes:53 ~suffix:"..." response
           |> String_util.to_string
         in
...
let maybe_respond ~sw ~base_path:_ ~from_agent ~content ~mention =
  ...
  debug_log
    (Printf.sprintf
       "CALLED: from=%s mention=%s mode=%s enabled=%b"
       from_agent
       (match mention with
        | Some m -> m
        | None -> "NONE")
  ...
  | Some m ->
    ...
    debug_log
      (Printf.sprintf
         "CHECK: from_base=%s mention_base=%s spawnable=%b"
         from_base
         mention_base
         (is_spawnable m));
    if from_base = mention_base
    then (
      debug_log "EXIT: Self-mention";
      Log.AutoResponder.info "Skip self-mention @%s from %s" m from_agent;
      None)
    else if not (is_spawnable m)
    then (
      ...
      activity_log
        ...
        ~mention:m
        ~status:"SKIP"
        ~detail:"Not spawnable agent type";
      Log.AutoResponder.info "@%s not spawnable" m;
      None)
    else if should_throttle ~agent_type:mention_base
    then (
      ...
      Eio.Fiber.fork ~sw (fun () ->
        try
          ...
          | Model ->
            ...
            call_model_and_broadcast
              ~sw
              ~agent_type:mention_base
              ~prompt:content
              ~mention:from_agent
          | Spawn ->
        ...
        with
        | Eio.Cancel.Cancelled _ as e -> raise e
        | exn -> debug_log (Printf.sprintf "ERROR: %s" (Printexc.to_string exn)));
      Some task_id)
;;
