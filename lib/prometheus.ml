...
let observe_histogram name ?(labels = []) value =
  let key = name ^ List.fold_left (fun acc (k, v) -> acc ^ k ^ v) "" labels in
  let count_key =
    name ^ "_count" ^ List.fold_left (fun acc (k, v) -> acc ^ k ^ v) "" labels
  in
  with_lock (fun () ->
    (match Hashtbl.find_opt metrics key with
...
let init () =
  ...
  add
    metric_keeper_context_max_observed
  ...
  add
    ...
    Counter;
  add
    metric_keeper_turn_livelock_blocks
  ...
  add
    metric_keeper_supervisor_sweep_starts
    "Total times keeper supervisor sweep Pulse was started (labels: base_path)"
    Counter;
  add
    metric_keeper_supervisor_last_sweep_unixtime
  ...
  add metric_keeper_compactions "Total keeper compactions performed" Counter;
  add
    metric_keeper_compaction_ratio_change
    "Context ratio change after compaction (pre - post)"
    Gauge;
  add
    metric_keeper_compaction_saved_tokens
  ...
  add metric_keeper_heartbeat_successes "Total keeper heartbeat successes" Counter;
  add metric_keeper_heartbeat_failures "Total keeper heartbeat failures" Counter;
  register_histogram
    ~name:metric_keeper_tool_call_duration
    ~help:
  ...
  (* Orphan metrics — used via inc_counter/set_gauge but previously
     ...
     first observation. *)
  add
    metric_keeper_write_meta_failures
    "Total keeper meta-file write failures, labeled by keeper and phase"
    Counter;
  add
    metric_keeper_lifecycle_dispatch_rejections
  ...
  add
    ...
    Counter;
  add
    metric_keeper_unexpected_tool_partial_tolerance
  ...
  add
    ...
    Counter;
  add
    metric_keeper_near_exhaustion_total
    ...
    Counter;
  add
    metric_keeper_tool_alias_canonicalizations
    ...
    Counter;
  add
    metric_keeper_profile_config_conflicts
  ...
  add
    ...
    "Estimated OAS Event_bus per-subscriber stream depth, labeled by subscriber_purpose. \
     ...
     subscriptions created via Oas_bus_instrument. OAS uses bounded Eio.Stream (default \
     256); values approaching this cap indicate impending publish blocking."
    Gauge;
  add
    metric_oas_bus_publish_block_seconds
    "Cumulative seconds spent inside Oas.Event_bus.publish when routed through \
     Oas_bus_instrument.publish. A sustained ramp indicates a subscriber drain loop has \
     fallen behind and publishers are blocking on Eio.Stream.add."
    Counter;
  add
    metric_oas_bus_publish
    "Total Oas.Event_bus.publish calls routed through Oas_bus_instrument.publish."
    Counter;
  add
  ...
  add
    ...
    "Total times Server_auth.dashboard_actor_for_request resolved no agent from the \
     ...
     outcome (none | error). Counter exposes the path that masks identity drift in the \
     HTTP transport."
    Counter;
  add
    metric_coord_join_normalize_outcome
    "Total Coord.join calls preflighted by Keeper_identity.normalize_all_names (RFC \
     P3-a). Labels: outcome (ok | empty_input | persona_not_found | credential_missing | \
     name_ambiguous | ephemeral_suffix_rejected). Logging-only mode: non-ok outcomes \
     still proceed with the original agent_name; counter classifies bootstrap-window \
     silent-fallback patterns (paired with masc_silent_auth_token_resolve_error_total) \
     before the mode is promoted to hard-error in a follow-up PR."
    Counter;
  ...
  Hashtbl.iter
    (fun name ms ->
       List.iter
         ...
         ms)
    by_name;
  let label_key labels = List.fold_left (fun acc (k, v) -> acc ^ k ^ v) "" labels in
  Hashtbl.iter
    (fun name ms ->
