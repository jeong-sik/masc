...
앞의 둘은 수명이 맞지만 도달이 불확실하다. board 는 keeper 가 도구를 호출해야 읽고, chat queue 는 대화에 남아 매 턴 재전송된다.
...
새 노트를 쓰면 이전 노트를 덮는다. 큐가 아니다 — 큐면 다시 누적이고, 그건 chat queue 가 이미 하는 일이다.
...
| 큐(여러 노트 누적) | chat queue 가 이미 그 일을 한다. 누적은 영구화의 다른 이름 |
