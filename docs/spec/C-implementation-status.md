...
| ~~Team Session lossy projection~~ | 07 | ~50 | **→ IMPL** (`collaboration_context` 제거 + Prompt_composer 기반 context 유지. OAS #698에서 Collaboration.t→opaque JSON 전환 완료) |
...
| Session Bridge Fidelity | Prompt_composer + `worker_specs` projection (`collaboration_context` intentionally omitted) | IMPL | OAS #698 opaque JSON, fidelity gap remains |
...
- ~~Team Session lossy projection~~ → Prompt_composer + `worker_specs` projection (IMPL)
