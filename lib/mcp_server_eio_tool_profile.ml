...
let operator_remote_instructions =
  "MASC remote operator profile exposes only four control-plane tools: \
   masc_operator_snapshot, masc_operator_digest, masc_operator_action, and \
   masc_operator_confirm. Read raw state with masc_operator_snapshot first when needed, \
   and prefer masc_operator_digest for intervention-oriented supervision. Use \
   masc_operator_action for guided actions only. When confirm_required=true, you must \
   call masc_operator_confirm with the returned confirm_token before the action \
   executes. Do not assume access to any other MASC tool from this endpoint."
;;

let managed_agent_instructions =
  "MASC managed-agent profile exposes the internal agent control surface. Prefer \
   canonical task-control tools such as masc_status, masc_tasks, masc_claim_next, \
   masc_transition, and masc_plan_set_task. Managed aliases that remain listed on this \
   endpoint are compatibility helpers, not the recommended control plane. Do not assume \
   that the public /mcp surface and the managed-agent surface have the same inventory."
;;
...
let managed_agent_passthrough_tool_names =
  ...
  |> List.filter (fun name ->
    not
      (List.mem
         name
         [ "masc_status"; "masc_tasks"; "masc_transition"; "masc_a2a_delegate" ]))
;;
...
let default_instructions =
  "MASC (Multi-Agent Streaming Coordination) enables AI agent collaboration. PROJECT: \
   Agents sharing the same base path (.masc/ folder) coordinate together. CLUSTER: Set \
   MASC_CLUSTER_NAME for multi-machine coordination (otherwise tool surfaces use the \
   configured cluster/default label). READ: use resources/list + resources/read \
   (status/tasks/agents/events/schema) for snapshots. WRITE: prefer masc_transition \
   (claim/start/done/cancel/release) with expected_version for CAS. WORKFLOW: \
   masc_status → masc_transition(claim) → masc_worktree_create (isolation) → work → \
   masc_transition(done). Use masc_heartbeat periodically; use @agent mentions in \
   masc_broadcast. Prefer worktrees for parallel work. Use masc_tool_help to inspect \
   tool contracts and prefer the smallest useful surface."
;;

let tool_schemas_for_profile
  ...
  let schemas =
    ...
    | Full ->
      ...
      let keeper_internal_schemas =
        if not include_keeper_internal
        then []
        else
      ...
      let full_profile_tools =
        List.filter
          (fun (schema : Types.tool_schema) ->
             ...
             &&
             if is_keeper_internal
             then include_keeper_internal
    ...
    | Managed_agent ->
      ...
      in
      dedupe_tool_schemas_by_name (Sdk_tool_contract.sdk_tool_schemas @ passthrough)
    | Operator_remote -> Tool_operator.remote_schemas
  in
  schemas
;;

let tool_allowed_in_profile ?(internal_keeper_runtime = false) state profile tool_name =
  match profile with
  | Full ->
    if Tool_catalog.is_on_surface Tool_catalog.Keeper_internal tool_name
    then internal_keeper_runtime
    else (
...
let tool_annotations_for_profile _profile tool_name =
  ...
  let open_world_hint =
    if destructive then Some true else if read_only then Some false else None
  in
  let fields =
...
let tool_title_of_name name =
  ...
  | None ->
    let trimmed =
      if String.length name > 5 && String.starts_with ~prefix:"masc_" name
...
let tool_icons_for_name name =
  let icon =
    if Tool_dispatch.is_read_only name
    then Mcp_server.themed_icon ~label:"RD" ~bg:"#0F766E" ~fg:"#F0FDFA"
    else Mcp_server.themed_icon ~label:"WR" ~bg:"#9A3412" ~fg:"#FFF7ED"
  in
  [ icon ]
;;
...
let tool_json_for_profile ?usage_summary profile (schema : Types.tool_schema) =
  let base =
    ...
    ; "icons", `List (List.map Mcp_server.icon_to_json (tool_icons_for_name schema.name))
...
let cursor_param payload =
  ...
  | `String value ->
    ...
    if trimmed = ""
    then Error "Invalid params: cursor must not be empty"
...
let paginate_json_items ?(page_size = 128) ~field_name items cursor =
  ...
  | Ok offset ->
    ...
    let fields =
      ...
      then [ "nextCursor", `String (string_of_int next_offset) ]
      else []
    in
    Ok (`Assoc fields)
;;
...
let requested_tool_list_params params =
  ...
  then
    Error
      (Printf.sprintf
         "Invalid params: unsupported field(s): %s"
      ...
      | `List items ->
        ...
        |> List.fold_left
             (fun acc item ->
                ...
                | Ok _, _ -> Error "Invalid params: names must be an array of strings")
             (Ok [])
...
let encode_cursor ~kind offset = Base64.encode_string (Printf.sprintf "%s:%d" kind offset)
...
let decode_cursor ~kind cursor =
  ...
  | Ok decoded ->
    ...
    if String.length decoded >= prefix_len && String.sub decoded 0 prefix_len = prefix
    then
      int_of_string_opt
