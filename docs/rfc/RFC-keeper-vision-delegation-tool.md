...
- **Phase 0a (done)**: capability-catalog correctness — declare image input in the OAS-backed catalog and configure `media_failover` (`jeong-sik/me` PR #1223, MERGED). Resolves the live incident; reroute lands on a capable cloud model. **Phases 1–3 are not required for the incident.**
