...
(** {!verify} the way the [runtime-verify] command runs it: installs the
    ...
    {!Time_compat.set_clock}) and verifies with the env's resources, working
    in [private_dir]. The command and the suite that stands in for it both
    call this, so the suite runs the command's shape rather than a copy of
    it. *)
