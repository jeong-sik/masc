...
let tier1_check ~(model : Masc_model.model_spec) ~actor_name ~actor_persona
    ~response_text : tier1_result =
  let prompt = build_tier1_prompt ~actor_name ~actor_persona ~response_text in
  let req : Masc_model.completion_request = {
    model;
  ...
  | Ok resp -> parse_tier1 (Masc_model.text_of_response resp)
  | Error e ->
...
let tier2_evaluate ~(model : Masc_model.model_spec) ~actor_name ~actor_persona
  ...
  match Llm_orchestration.complete req with
  | Ok resp -> parse_tier2 (Masc_model.text_of_response resp)
  | Error e ->
