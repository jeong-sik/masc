...
let test_executor_deploy_not_registered () =
  (* Deploy pattern was removed — no fake stub pretending to work.
     A deploy topic should have no matching action. *)
  match Executor.find_action "deploy v2.0" with
  | None -> ()
  | Some _ -> fail "deploy pattern should not be registered"
...
let executor_tests = [
  ...
  "deploy pattern not registered", `Quick, test_executor_deploy_not_registered;
]
