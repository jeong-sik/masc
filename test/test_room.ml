...
let backlog_recovery_path config = Coord.backlog_path config ^ ".last-good"
...
let test_join_creates_agent () =
  ...
  let has_test_agent =
    List.exists
      (fun name -> String.length name >= 10 && String.sub name 0 10 = "test_agent")
      state.active_agents
  in
...
let test_add_and_claim_task () =
  ...
  (* Add task *)
  let add_result =
    Coord.add_task config ~title:"Test Task" ~priority:1 ~description:"Test"
  in
...
let test_add_task_uses_archive_max_id () =
  ...
  let archive_json =
    `Assoc
      ...
      ; "tasks", `List [ `Assoc [ "id", `String "task-005" ] ]
      ]
  in
...
let test_worktree_project_root_for_nested_subdir () =
  ...
  Alcotest.(check string)
    "nested path resolves to repo root"
    repo_root
...
let test_worktree_project_root_for_nested_gitfile_worktree_subdir () =
  ...
  let nested_config = { config with base_path = nested } in
  Alcotest.(check string)
    "nested worktree path resolves to shared repo root"
    repo_root
    (Coord.project_root nested_config);
  let _ = Coord.reset config in
  rm_rf tmp_dir
...
let test_worktree_project_root_for_masc_dir_base () =
  ...
  Unix.mkdir (Filename.concat repo_root ".git") 0o755;
  let masc_config =
    { config with base_path = Filename.concat repo_root Common.masc_dirname }
  in
  Alcotest.(check string)
    ".masc base path resolves to repo root"
    repo_root
...
let test_complete_without_claim () =
  with_test_env (fun config ->
    ...
    let result =
      transition_done config ~agent_name:"claude" ~task_id:"task-001" ~notes:""
    in
...
let test_complete_by_wrong_agent () =
  with_test_env (fun config ->
    ...
    let result =
      transition_done config ~agent_name:"gemini" ~task_id:"task-001" ~notes:""
    in
    Alcotest.(check bool) "wrong agent blocked" true (contains_error result);
    Alcotest.(check bool)
      "wrong agent points at current assignee"
      true
...
let test_complete_nonexistent_task () =
  with_test_env (fun config ->
    let result =
      transition_done config ~agent_name:"claude" ~task_id:"task-999" ~notes:""
    in
...
let test_double_complete () =
  with_test_env (fun config ->
    ...
    let _ = Coord.claim_task config ~agent_name:"claude" ~task_id:"task-001" in
    let _ =
      transition_done config ~agent_name:"claude" ~task_id:"task-001" ~notes:"first"
    in
    (* Done is idempotent at the Coord FSM layer. *)
    let result =
      transition_done config ~agent_name:"claude" ~task_id:"task-001" ~notes:"second"
    in
    Alcotest.(check bool) "double complete is no-op" true (contains_check result);
    Alcotest.(check bool)
      "double complete mentions no-op"
      true
...
let test_portal_send_without_open () =
  with_test_env (fun config ->
    ...
    Alcotest.(check bool)
      "send without open returns error"
      true
      (match result with
       | Error _ -> true
       | Ok _ -> false))
...
let test_task_state_after_claim () =
  with_test_env (fun config ->
    ...
    Alcotest.(check bool)
      "has claude"
      true
      (str_contains tasks "claude" || str_contains tasks "Claimed"))
;;
...
let test_multiple_tasks_independent () =
  with_test_env (fun config ->
    ...
    let result =
      transition_done config ~agent_name:"claude" ~task_id:"task-002" ~notes:""
    in
...
let test_room_bootstrap_preserves_backend_state () =
  with_memory_test_env (fun config ->
    ...
    let _ = Coord.update_state config (fun state -> { state with message_seq = 41 }) in
    let backlog = { Types.tasks = []; last_updated = Types.now_iso (); version = 7 } in
    Coord_utils.write_json
      config
      (Coord.backlog_path config)
...
let test_room_bootstrap_ignores_invalid_room_id_in_flat_mode () =
  with_memory_test_env (fun config ->
    Coord.ensure_room_bootstrap config;
    Alcotest.(check bool) "root state initialized" true (Coord.is_initialized config))
;;

let test_read_backlog_r_recovers_from_last_good_snapshot () =
  with_test_env (fun config ->
    let expected = { Types.tasks = []; last_updated = Types.now_iso (); version = 7 } in
...
let test_read_backlog_r_reports_parse_error_when_recovery_is_also_invalid () =
  with_test_env (fun config ->
    ...
    | Error msg ->
      Alcotest.(check bool)
        "mentions primary backlog failure"
        true
        (str_contains msg "read_backlog" || str_contains msg "JSON parse error");
      Alcotest.(check bool) "mentions recovery failure" true (str_contains msg "recovery"))
;;
...
let test_cleanup_zombies_detects_regular () =
  with_test_env (fun config ->
    ...
    let result = Coord.cleanup_zombies config in
    Alcotest.(check bool)
      "regular zombie detected"
      true
      (str_contains result "stale-regular-agent"))
;;
...
let test_cleanup_zombies_detects_keeper () =
  with_test_env (fun config ->
    ...
    let result = Coord.cleanup_zombies config in
    Alcotest.(check bool)
      "keeper zombie detected after keeper threshold"
      true
      (str_contains result "keeper-longplay-agent"))
;;
...
let test_cleanup_zombies_spares_recent_keeper () =
  with_test_env (fun config ->
    ...
    let result = Coord.cleanup_zombies config in
    Alcotest.(check bool)
      "recent keeper spared"
      true
      (not (str_contains result "keeper-active-agent")))
;;
...
let test_cleanup_zombies_removes_broken_agent_file () =
  with_test_env (fun config ->
    ...
    Alcotest.(check bool) "broken file exists before GC" true (Sys.file_exists path);
    let _result = Coord.cleanup_zombies config in
    Alcotest.(check bool) "broken file removed by GC" false (Sys.file_exists path))
;;
...
let test_cleanup_zombies_preserves_non_json_files () =
  with_test_env (fun config ->
    ...
    output_string oc "";
    close_out oc;
    Alcotest.(check bool) "non-json file exists before GC" true (Sys.file_exists path);
    let _result = Coord.cleanup_zombies config in
    Alcotest.(check bool) "non-json file preserved by GC" true (Sys.file_exists path))
;;
...
let test_register_capabilities () =
  with_test_env (fun config ->
    ...
    let result =
      Coord.register_capabilities
        config
        ~agent_name:"gemini"
        ~capabilities:[ "python"; "web-search"; "code-review" ]
    in
...
let test_find_by_capability () =
  with_test_env (fun config ->
    let _ =
      Coord.join config ~agent_name:"gemini" ~capabilities:[ "python"; "search" ] ()
    in
...
let test_register_capabilities_nonexistent_agent () =
  with_test_env (fun config ->
    ...
    let result =
      Coord.register_capabilities config ~agent_name:"ghost" ~capabilities:[ "magic" ]
    in
...
let test_message_sequence () =
  with_test_env (fun config ->
    ...
    Alcotest.(check bool)
      "has messages"
      true
      (str_contains msgs "First" || str_contains msgs "Third"))
;;
...
let test_portal_send_multiple () =
  with_test_env (fun config ->
    ...
    let check_send label r =
      match r with
      | Ok msg -> Alcotest.(check bool) label true (String.length msg > 0)
      | Error _ -> Alcotest.fail label
    in
    check_send "send 1" (Coord.portal_send_r config ~agent_name:"claude" ~message:"First");
    check_send
      "send 2"
      (Coord.portal_send_r config ~agent_name:"claude" ~message:"Second");
...
let test_xss_in_message () =
  with_test_env (fun config ->
    ...
    let has_raw_script =
      str_contains result "<script>" || str_contains result "</script>"
    in
...
let test_force_done_bypasses_assignee () =
  with_test_env (fun config ->
    ...
    Alcotest.(check bool)
      "task is done"
      true
      (str_contains tasks "Done" || str_contains tasks "done"))
;;
    ...
    Fun.protect
      ...
      (fun () ->
         ...
         let _ =
           Coord.add_task
             config
             ~title:"Verification Orphan Candidate"
             ~priority:1
             ~description:""
         in
         let _ = Coord.join config ~agent_name:test_agent_a ~capabilities:[] () in
         ...
         with
         | Error err ->
           Alcotest.failf "submit for verification failed: %s" (Types.show_masc_error err)
         | Ok _ ->
           let orphans_before = Coord.audit_orphan_tasks config in
           Alcotest.(check int)
             "no verification orphans while active"
             0
             (List.length orphans_before);
           let _ = Coord.leave config ~agent_name:test_agent_a in
           let orphans_after = Coord.audit_orphan_tasks config in
           Alcotest.(check int)
             "one verification orphan detected"
             1
             (List.length orphans_after);
           let task, assignee = List.hd orphans_after in
           Alcotest.(check string) "verification orphan assignee" test_agent_a assignee;
           Alcotest.(check string) "verification orphan task id" "task-001" task.id))
;;
...
let test_cleanup_zombies_releases_tasks () =
  with_test_env (fun config ->
    ...
    let agents_path =
      Filename.concat (Filename.concat config.base_path Common.masc_dirname) "agents"
    in
...
let test_rejoin_preserves_identity () =
  with_test_env (fun config ->
    ...
    let nick1 =
      List.find
        (fun name -> String.length name > 6 && String.sub name 0 6 = "claude")
        state1.active_agents
    in
    (* 2. Leave *)
...
let test_rejoin_restores_active_status () =
  with_test_env (fun config ->
    ...
    let is_active =
      List.exists
        (fun name -> String.length name > 6 && String.sub name 0 6 = "gemini")
        state.active_agents
    in
...
let test_multiple_rejoin_cycles () =
  with_test_env (fun config ->
    ...
    let nick1 =
      List.find
        (fun name -> String.length name > 5 && String.sub name 0 5 = "codex")
        state1.active_agents
    in
...
let read_event_log config =
  ...
  in
  let day_file = Filename.concat month_dir (Printf.sprintf "%02d.jsonl" tm.tm_mday) in
  if Sys.file_exists day_file
  then (
...
let test_zombie_file_delete_failure_keeps_state () =
  with_test_env (fun config ->
    ...
    let agents_path = Filename.concat (Coord.masc_dir config) "agents" in
    let path =
      Filename.concat agents_path (Coord.safe_filename "unremovable-agent" ^ ".json")
    in
    ...
    let state = Coord.read_state config in
    let still_present =
      List.exists (fun name -> str_contains name "unremovable") state.active_agents
    in
...
let test_zombie_cleanup_transitions_to_inactive () =
  with_test_env (fun config ->
    ...
    let agents_path = Filename.concat (Coord.masc_dir config) "agents" in
    let path =
      Filename.concat agents_path (Coord.safe_filename "transition-test-agent" ^ ".json")
    in
    Unix.chmod path 0o444;
    Unix.chmod agents_path 0o555;
    let _result = Coord.cleanup_zombies config in
...
let test_keeper_detection_by_agent_type () =
  ...
  let is_keeper_by_type =
    Resilience.Zombie.is_keeper ~name:"regular-bot" ~agent_type:"keeper"
  in
  Alcotest.(check bool) "agent_type=keeper detected" true is_keeper_by_type;
  (* A keeper-named agent should still be detected *)
  let is_keeper_by_name =
    Resilience.Zombie.is_keeper ~name:"keeper-test-agent" ~agent_type:"test"
  in
...
let test_heartbeat_concurrent_start_stop () =
  ...
  @@ fun _env ->
  (* Reset heartbeats *)
  List.iter (fun (hb : Heartbeat.t) -> ignore (Heartbeat.stop hb.id)) (Heartbeat.list ());
...
let test_bug006_transition_with_unsuffixed_name () =
  with_test_env (fun config ->
    ...
    let _ =
      Coord.join config ~agent_name:"keeper-coder-agent" ~capabilities:[ "code" ] ()
    in
    ...
    (match
       Coord.claim_task_r config ~agent_name:"keeper-coder-agent" ~task_id:"task-001" ()
     with
     | Ok _ -> ()
    ...
    (match
     ...
     | Error e ->
       Alcotest.failf
         "start with unsuffixed name failed (BUG-006): %s"
    ...
    | Error e ->
      Alcotest.failf
        "complete with unsuffixed name failed (BUG-006): %s"
        (Types.show_masc_error e))
;;
...
let test_bug006_cancel_with_unsuffixed_name () =
  with_test_env (fun config ->
    ...
    in
    let _ =
      Coord.add_task config ~title:"BUG-006 Cancel Task" ~priority:1 ~description:""
    in
    (match
       Coord.claim_task_r config ~agent_name:"keeper-coder-agent" ~task_id:"task-001" ()
     with
    ...
    | Error e ->
      Alcotest.failf
        "cancel with unsuffixed name failed (BUG-006): %s"
        (Types.show_masc_error e))
;;
...
let test_empty_backlog_stop_signal () =
  with_test_env (fun config ->
    ...
    Alcotest.(check bool)
      ...
      true
      (str_contains result "STOP calling keeper_tasks_list"))
;;
...
let test_no_unclaimed_tasks_stop_signal () =
  with_test_env (fun config ->
    ...
    Alcotest.(check bool)
      "contains ACTION stop signal"
      true
      (str_contains result "ACTION: Stop task-checking"))
;;
...
let () =
  ...
  Alcotest.run
    "Coord"
    [ (* === Happy Path Tests === *)
      "init", [ Alcotest.test_case "creates folder" `Quick test_init_creates_folder ]
    ; ( "join"
      , [ Alcotest.test_case "creates agent" `Quick test_join_creates_agent
        ; Alcotest.test_case "double join" `Quick test_double_join
        ] )
    ; "leave", [ Alcotest.test_case "removes agent" `Quick test_leave_removes_agent ]
    ; ( "rejoin"
      , [ Alcotest.test_case "preserves identity" `Quick test_rejoin_preserves_identity
        ; Alcotest.test_case
            "restores active status"
            `Quick
            test_rejoin_restores_active_status
        ; Alcotest.test_case "stable across 3 cycles" `Quick test_multiple_rejoin_cycles
        ] )
    ; "tasks", [ Alcotest.test_case "add and claim" `Quick test_add_and_claim_task ]
    ; "messages", [ Alcotest.test_case "broadcast" `Quick test_broadcast_message ]
    ; ( "worktree"
    ...
    ; (* === Robustness: Boundary Values === *)
      ...
      , [ Alcotest.test_case "empty task title" `Quick test_empty_task_title
        ...
        ; Alcotest.test_case
            "special agent name"
            `Quick
            test_agent_name_with_special_chars
    ...
    ; (* === Robustness: State Consistency === *)
      ...
      , [ Alcotest.test_case "task state after claim" `Quick test_task_state_after_claim
        ; Alcotest.test_case
            ...
            `Quick
            test_multiple_tasks_independent
        ] )
    ; (* === Archive Tests === *)
      ( "archive"
      , [ Alcotest.test_case
            "task id uses archive max"
            `Quick
            test_add_task_uses_archive_max_id
        ] )
    ; (* === Robustness: Concurrency Simulation === *)
      ...
      , [ Alcotest.test_case "rapid claim sequence" `Quick test_rapid_claim_sequence
        ; Alcotest.test_case
            ...
            `Quick
            test_multiple_agents_multiple_tasks
        ] )
    ; (* === Robustness: Recovery === *)
      ( "recovery"
      , [ Alcotest.test_case "reinit existing room" `Quick test_reinit_existing_room
        ; Alcotest.test_case
            "operations preserve state"
            `Quick
            test_operations_preserve_state
        ] )
    ; (* === Event Log Tests === *)
      ( "events"
    ...
    ; (* === Heartbeat & Zombie Detection Tests === *)
      ...
      , [ Alcotest.test_case "updates last_seen" `Quick test_heartbeat_updates_lastseen
        ; Alcotest.test_case
            "default join keeps joined status"
            `Quick
            test_is_agent_joined_after_default_join
        ...
        ; Alcotest.test_case
            "backend bootstrap preserves room state"
            `Quick
            test_room_bootstrap_preserves_backend_state
        ; Alcotest.test_case
            "bootstrap ignores invalid room id in flat mode"
            `Quick
            test_room_bootstrap_ignores_invalid_room_id_in_flat_mode
        ; Alcotest.test_case
            "read_backlog_r recovers from last good snapshot"
            `Quick
            test_read_backlog_r_recovers_from_last_good_snapshot
        ; Alcotest.test_case
            "read_backlog_r reports parse error when recovery also invalid"
            `Quick
            test_read_backlog_r_reports_parse_error_when_recovery_is_also_invalid
        ; Alcotest.test_case
            "release stale claims skips invalid backlog"
            `Quick
            test_release_stale_claims_skips_invalid_backlog
        ; Alcotest.test_case "cleanup zombies empty" `Quick test_cleanup_zombies_empty
        ; Alcotest.test_case
            "cleanup detects regular zombie"
            `Quick
            test_cleanup_zombies_detects_regular
        ; Alcotest.test_case
            "cleanup detects keeper zombie"
            `Quick
            test_cleanup_zombies_detects_keeper
        ; Alcotest.test_case
            "cleanup spares recent keeper"
            `Quick
            test_cleanup_zombies_spares_recent_keeper
        ; Alcotest.test_case
            "cleanup removes broken agent file"
            `Quick
            test_cleanup_zombies_removes_broken_agent_file
        ; Alcotest.test_case
            "cleanup preserves non-json files"
            `Quick
            test_cleanup_zombies_preserves_non_json_files
        ] )
    ; (* === Agent Discovery / Capability Tests === *)
      ( "capabilities"
      , [ Alcotest.test_case "register capabilities" `Quick test_register_capabilities
        ...
        ; Alcotest.test_case
            "register nonexistent agent"
            `Quick
            test_register_capabilities_nonexistent_agent
        ] )
    ; (* === Input Validation Tests === *)
      ( "validation"
    ...
    ; (* === Priority Tests === *)
      "priority", [ Alcotest.test_case "negative priority" `Quick test_negative_priority ]
    ; (* === Security Tests (v2.1) === *)
      ( "security"
    ...
    ; (* === Board Admin Tests === *)
      ( "board_admin"
      , [ Alcotest.test_case
            "force release bypasses assignee"
            `Quick
            test_force_release_bypasses_assignee
        ; Alcotest.test_case
            "force done bypasses assignee"
            `Quick
            test_force_done_bypasses_assignee
        ...
        ; Alcotest.test_case
            "cleanup zombies cascade"
            `Quick
            test_cleanup_zombies_releases_tasks
        ] )
    ; (* === Lifecycle Bug Fix Tests (#1655) === *)
      ( "lifecycle_bugs"
      , [ Alcotest.test_case "BUG-1: rejoin event log" `Quick test_rejoin_event_log
        ; Alcotest.test_case
            "BUG-2: file delete failure keeps state"
            `Quick
            test_zombie_file_delete_failure_keeps_state
        ; Alcotest.test_case
            "BUG-4: zombie transitions to inactive"
            `Quick
            test_zombie_cleanup_transitions_to_inactive
        ; Alcotest.test_case
            "BUG-5: keeper detection by agent_type"
            `Quick
            test_keeper_detection_by_agent_type
        ; Alcotest.test_case
            "BUG-6: heartbeat concurrent start/stop"
            `Quick
            test_heartbeat_concurrent_start_stop
        ] )
    ; (* === BUG-006: Task identity mismatch (unsuffixed keeper name) === *)
      ( "task_identity"
      , [ Alcotest.test_case
            "BUG-006: transition/complete with unsuffixed name"
            `Quick
            test_bug006_transition_with_unsuffixed_name
        ; Alcotest.test_case
            "BUG-006: cancel with unsuffixed name"
            `Quick
            test_bug006_cancel_with_unsuffixed_name
        ] )
    ; (* === Idle loop stop signal tests === *)
      ( "idle_stop_signals"
      , [ Alcotest.test_case
            "empty backlog has stop signal"
            `Quick
            test_empty_backlog_stop_signal
        ; Alcotest.test_case
            "no active tasks has stop signal"
            `Quick
            test_no_active_tasks_stop_signal
        ; Alcotest.test_case
            "no unclaimed tasks has stop signal"
            `Quick
            test_no_unclaimed_tasks_stop_signal
        ] )
    ]
;;
