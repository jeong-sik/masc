...
let with_identity_rng f = Eio.Mutex.use_ro identity_rng_mutex (fun () -> f identity_rng)
...
let channel_to_yojson = function
  ...
  | External s -> `List [ `String "External"; `String (normalize_channel_label s) ]
;;

let channel_of_yojson = function
  | `String "Api" | `String "api" -> Ok Api
  | `String "Internal" | `String "internal" -> Ok Internal
  | `String "Telegram" | `String "telegram" -> Ok (External "telegram")
  | `String "Discord" | `String "discord" -> Ok (External "discord")
  | `String "Slack" | `String "slack" -> Ok (External "slack")
  | `String "Signal" | `String "signal" -> Ok (External "signal")
  | `String "Webchat" | `String "webchat" -> Ok (External "webchat")
  | `String s -> Ok (channel_of_string s)
  | `List [ `String "External"; `String s ] -> Ok (External (normalize_channel_label s))
  | `List [ `String "Unknown"; `String s ] -> Ok (External (normalize_channel_label s))
...
let from_mcp_params params =
  ...
  let session_key_prefix session_key =
    ...
    if prefix_len = 0 then "unknown" else String.sub session_key 0 prefix_len
  in
  ...
  in
  let session_key =
    match get_opt "_session_key" with
    | Some k ->
...
module Registry = struct
  ...
  let with_lock reg f = Eio_guard.with_mutex reg.lock f
  ...
  let register reg identity =
    with_lock reg (fun () ->
      ...
      reg.by_agent_name
      := StringMap.add identity.agent_name identity.session_key !(reg.by_agent_name);
      identity)
  ;;

  (** Find by session key *)
  let find_by_session reg session_key =
    with_lock reg (fun () -> StringMap.find_opt session_key !(reg.identities))
  ;;
  ...
  let list_active reg ~within_seconds =
    with_lock reg (fun () ->
      ...
      !(reg.identities)
      |> StringMap.bindings
      |> List.filter_map (fun (_, id) -> if id.last_seen > cutoff then Some id else None))
  ;;

  (** Get identity count *)
  let count reg = with_lock reg (fun () -> StringMap.cardinal !(reg.identities))
end

(** {1 Utilities} *)

(** Check if identity has a specific capability *)
let has_capability identity cap = List.mem cap identity.capabilities
...
let same_agent a b = a.session_key = b.session_key || a.agent_name = b.agent_name
...
let archetype_of_string s =
  ...
  | None ->
    Log.Misc.warn
      "archetype_of_string: unknown wire string %S → Generalist fallback (#8691)"
      s;
    Generalist
;;
