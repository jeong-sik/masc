...
let score_for ?(weights = default_fitness_weights) ~min_avg ~max_collabs metrics =
  ...
  let reliability =
    if has_data then 1.0 -. metrics.Metrics_store_eio.error_rate else 0.0
  in
  let handoff =
    if has_data then metrics.Metrics_store_eio.handoff_success_rate else 0.0
  in
  let speed =
    if has_data && metrics.Metrics_store_eio.avg_completion_time_s > 0.0 && min_avg > 0.0
    then min 1.0 (min_avg /. metrics.Metrics_store_eio.avg_completion_time_s)
    else 0.0
  in
  let collab_count = List.length metrics.Metrics_store_eio.unique_collaborators in
  let collaboration =
    if max_collabs = 0 then 0.0 else float_of_int collab_count /. float_of_int max_collabs
  in
  let score =
...
let handle_agent_fitness ctx args =
  ...
  let agents =
    ...
    | None ->
      ...
      let room_agents =
        try
          Coord.get_agents_raw ctx.config |> List.map (fun (a : Types.agent) -> a.name)
        with
        | Eio.Cancel.Cancelled _ as e -> raise e
        | exn ->
          Log.Misc.warn
            "room agents fallback (metrics_store still used): %s"
  ...
  in
  if agents = []
  then true, Yojson.Safe.to_string (`Assoc [ "count", `Int 0; "agents", `List [] ])
  else (
    ...
    let agents_json =
      List.map
        (fun (agent_id, metrics) ->
           let score, completion, reliability, speed, handoff, collaboration =
             score_for ~min_avg ~max_collabs:max_col metrics
           in
           `Assoc
    ...
    in
    let json =
      `Assoc [ "count", `Int (List.length agents_json); "agents", `List agents_json ]
    in
    true, Yojson.Safe.to_string json)
;;
...
let handle_collaboration_graph ctx args =
  ...
  | Text ->
    let lines =
      ...
      |> List.map (fun s ->
        Printf.sprintf
          "%s → %s (%.2f, success:%d, failure:%d)"
          s.Hebbian_eio.from_agent
          s.Hebbian_eio.to_agent
          s.Hebbian_eio.weight
          s.Hebbian_eio.success_count
          s.Hebbian_eio.failure_count)
    in
    if lines = []
    then true, "No collaboration data yet."
    else true, String.concat "\n" lines
;;
...
let tool_required_permission = function
  ...
  | "masc_get_metrics" -> Some Types.CanReadState
  | "masc_register_capabilities" | "masc_agent_update" -> Some Types.CanBroadcast
  | _ -> None
;;
