...
let run_record_of_json (json : Yojson.Safe.t) : run_record option =
  match
    ...
    , Safe_ops.json_string_opt "updated_at" json )
  with
...
let log_entry_to_json (e : log_entry) : Yojson.Safe.t =
  `Assoc [ "timestamp", `String e.timestamp; "note", `String e.note ]
;;

let log_entry_of_json (json : Yojson.Safe.t) : log_entry option =
  match
    Safe_ops.json_string_opt "timestamp" json, Safe_ops.json_string_opt "note" json
  with
  | Some timestamp, Some note -> Some { timestamp; note }
  | _ ->
    Prometheus.inc_counter Prometheus.metric_error_events ~labels:[ "type", "parsing" ] ();
...
let runs_dir (config : config) = Filename.concat (masc_dir config) "runs"
let run_dir (config : config) task_id = Filename.concat (runs_dir config) task_id
let run_json_path config task_id = Filename.concat (run_dir config task_id) "run.json"
let plan_path config task_id = Filename.concat (run_dir config task_id) "plan.md"

let deliverable_path config task_id =
  Filename.concat (run_dir config task_id) "deliverable.md"
;;

let log_path config task_id = Filename.concat (run_dir config task_id) "log.jsonl"
...
let read_text_file path = if Sys.file_exists path then Fs_compat.load_file path else ""
...
let read_run config task_id : (run_record, string) result =
  ...
  then Error (Printf.sprintf "Run not found for task %s" task_id)
  else (
...
let init config ~task_id ~agent_name : (run_record, string) result =
  try
    ...
    if not (path_exists config plan_file) then write_text_file plan_file "# Run Plan\n\n";
    let deliverable_file = deliverable_path config task_id in
    if not (path_exists config deliverable_file) then write_text_file deliverable_file "";
    let log_file = log_path config task_id in
    if not (path_exists config log_file)
    then (
...
let read_logs config ~task_id ?limit () : log_entry list =
  ...
  then []
    ...
    let entries = Fs_compat.load_jsonl file |> List.filter_map log_entry_of_json in
    match limit with
    | None -> entries
    | Some n ->
...
let list config : Yojson.Safe.t =
  ...
  then `Assoc [ "count", `Int 0; "runs", `List [] ]
  else (
    ...
    let runs =
      List.filter_map
        (fun task_id ->
           ...
           then run_record_of_json (read_json config path)
           else None)
        entries
    in
    `Assoc
