...
let resolve_keeper_name_for_action (ctx : 'a context) ~(name : string) =
  ...
  | Error _ ->
    ...
    if requested_name = ""
    then Error "target_id is required"
    else (
      let configured = Keeper_types.configured_keeper_names ctx.config in
      if List.mem requested_name configured
      then Ok requested_name
      else (
...
let keeper_github_identity_target (ctx : 'a context) ~(name : string) =
  ...
  | None ->
    Error (Printf.sprintf "keeper %s has no github_identity configured" resolved_name)
...
let run_gh_auth_status ~gh_config_dir =
  ...
  with
  | Unix.Unix_error (Unix.ENOENT, _, _) -> Error "gh executable not found in PATH"
...
let keeper_recovery_outcome after_diagnostic =
  ...
  | Some ("healthy" | "idle") when not (keeper_diagnostic_recoverable after_diagnostic) ->
    true, None
  | Some state ->
    false, Some (Printf.sprintf "keeper remained %s after recovery attempt" state)
...
let execute_room_action (ctx : 'a context) (request : action_request) =
  ...
  | "namespace_pause" ->
    let* () = validate_target_type "root" request in
    let reason = get_string request.payload "reason" "Paused by operator control plane" in
    Coord.pause ctx.config ~by:request.actor ~reason;
    room_action_result request (`Assoc [ "paused", `Bool true; "reason", `String reason ])
  | "namespace_resume" ->
    let* () = validate_target_type "root" request in
    let status =
  ...
  | "social_sweep" ->
    room_action_result
      ...
      (`Assoc
          ...
          ; ( "reason"
            , `String
                "Social runtime removed. Keepers discover board events via proactive \
                 turns." )
          ])
  | "task_inject" ->
    let* () = validate_target_type "root" request in
  ...
  | "keeper_probe" ->
    ...
    Ok
      (`Assoc
          ...
          ; "result", `Assoc [ "status", status_json; "diagnostic", diagnostic ]
          ])
  | "keeper_recover" ->
    let* () = validate_target_type "keeper" request in
    let* name = require_target_id request in
    let* resolved_name, _meta = resolve_keeper_meta_for_name ctx ~name in
    ...
    then
        ...
        (`Assoc
            ...
            ; ( "result"
              , `Assoc
                  ...
                  ; ( "skipped_reason"
                    , `String "keeper is already healthy enough; recovery not required" )
                  ; "before", before_diagnostic
                  ] )
            ])
    else
      let* down_result =
        dispatch_keeper_json
          ctx
      ...
      let recovered, skipped_reason = keeper_recovery_outcome after_diagnostic in
      Ok
        (`Assoc
            [ "tool_name", `String "masc_keeper_recover"
            ; ( "result"
          ...
          ; ( "result"
            , `Assoc
                ...
                ; ( "legacy_gh_config_dir"
                  , match legacy_gh_config_dir with
                    | Some value -> `String value
                    | None -> `Null )
...
let execute_action (ctx : 'a context) (request : action_request)
  ...
  | "keeper_message"
  | "keeper_github_identity_login_prepare"
  | "keeper_github_identity_status" -> execute_keeper_action ctx request
...
let validate_request request =
  ...
  then Error "action_type is required"
  else if List.mem request.action_type known_action_types
  then Ok ()
  else Error (Printf.sprintf "unsupported action_type: %s" request.action_type)
;;

let action_json ?actor_hint (ctx : _ context) args : (Yojson.Safe.t, string) result =
  ...
  if confirm_required request.action_type
  then (
...
let confirm_json ?actor_hint (ctx : _ context) args : (Yojson.Safe.t, string) result =
