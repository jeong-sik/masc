...
let select_memory_candidates (rows : (string * string * int) list)
  ...
  let rec go acc dropped_total rest =
    ...
    | (kind, text, pr) :: rest' ->
      ...
      if cap <= 0 || used >= cap
      then (
        let cur = Option.value ~default:0 (Hashtbl.find_opt dropped_by_kind kind) in
...
let dedup_memory_candidates (items : (string * string * int) list)
  ...
  then exact
    ...
    let rec go kept = function
      ...
      | (kind, text, pr) :: rest ->
        let is_dup =
          List.exists
            (fun (_, kept_text, _) -> jaccard_similarity text kept_text >= threshold)
            kept
        in
        if is_dup then go kept rest else go ((kind, text, pr) :: kept) rest
    in
    go [] exact)
;;
...
let compile_consensus_re pattern =
  if pattern = ""
  then consensus_default_re
  else (
    try Re.Pcre.re pattern |> Re.compile with
    | exn ->
      Log.Keeper.warn
...
let consensus_re () =
  ...
  Eio.Mutex.use_rw ~protect:true consensus_re_mu (fun () ->
    ...
    | Some (cached_pattern, re) when String.equal cached_pattern pattern -> re
    | _ ->
...
let has_inflated_consensus_marker (s : string) : bool = Re.execp (consensus_re ()) s
...
let memory_candidates_from_snapshot (snapshot : keeper_state_snapshot)
  : candidate_selection_result
  =
  let add_opt kind value acc =
    ...
    | Some text ->
      ...
      then acc
      else (kind, text, tuned_priority_for_candidate ~kind ~text) :: acc
  in
  let add_list kind values acc =
    List.fold_left
      (fun acc item ->
         let item = String.trim item in
         if item = "" || not (is_meaningful_memory_text item)
         then acc
         else (kind, item, tuned_priority_for_candidate ~kind ~text:item) :: acc)
      acc
      values
  in
  let raw =
...
let parse_memory_bank_row (line : string) : keeper_memory_row_raw option =
  try
    ...
    if schema_version <> 0 && schema_version <> keeper_memory_schema_version
    then None
    else (
      ...
      if kind = "" || text = "" || not (is_meaningful_memory_text text)
      then None
      else Some { json = j; kind; horizon; source; generation; text; priority; ts_unix })
  with
  ...
  List.iter
    (fun (row : keeper_memory_row_raw) ->
       ...
       then (
         ...
         if norm <> "" && tid <> ""
         then (
           let existing = Option.value ~default:[] (Hashtbl.find_opt text_traces norm) in
           if not (List.mem tid existing)
           then Hashtbl.replace text_traces norm (tid :: existing))))
    rows;
  Hashtbl.iter
    (fun norm_text tids ->
       if List.length tids >= 3
       then (
         ...
         let best =
           List.fold_left
             (fun acc (row : keeper_memory_row_raw) ->
                if
                  normalize_memory_text_key row.text = norm_text
                  && row.priority
                     >
                     match acc with
                     | Some r -> r.priority
...
let memory_compaction_target_notes () : int =
  ...
  let raw =
    Safe_ops.get_env_int_logged "MASC_KEEPER_MEMORY_MAX_NOTES" ~default:default_target
  in
  max 40 (min 4000 raw)
;;
...
let memory_row_key (row : keeper_memory_row_raw) : string =
  String.lowercase_ascii (String.trim row.kind) ^ ":" ^ normalize_memory_text_key row.text
;;

let compaction_priority ~(current_generation : int) (row : keeper_memory_row_raw) : int =
  let horizon_bonus =
...
let write_memory_bank_rows (path : string) (rows : keeper_memory_row_raw list)
  : (unit, string) result
  =
  try
    let content =
...
let compact_memory_bank_if_needed (config : Coord.config) (meta : keeper_meta)
  ...
  then { no_memory_bank_compaction with target_notes; reason = Some "missing_file" }
  else (
    let size_bytes =
    ...
    then
      { no_memory_bank_compaction with target_notes; reason = Some "under_trigger_bytes" }
    else (
      match Safe_ops.read_file_safe path with
      | Error _ ->
        { no_memory_bank_compaction with target_notes; reason = Some "read_failed" }
      | Ok content ->
        let lines =
          content
        ...
        if before_notes <= target_notes && invalid = 0
        then
          ...
          let consolidated_parsed, consolidated_count = consolidate_memory_notes parsed in
          ...
          then
            ...
            let add_row ~ignore_kind_cap (row : keeper_memory_row_raw) =
              if !selected_count >= target_notes
              then ()
              else (
                let key = memory_row_key row in
                if key = "" || Hashtbl.mem selected_keys key
                then ()
                else (
                  let used =
                  ...
                  then (
                    ...
                    incr selected_count)
                  else Hashtbl.replace kind_dropped_keys key row.kind))
            in
            ...
            if !selected_count < target_notes
            then List.iter (fun row -> add_row ~ignore_kind_cap:true row) by_recency;
            let selected =
              !selected_rev
              |> List.rev
...
let append_memory_notes_from_reply
    ...
    | None ->
      (match parse_state_snapshot_from_reply reply with
       ...
       | None ->
         ...
         ( { Keeper_memory_policy.goal = (if meta.goal <> "" then Some meta.goal else None)
           ; progress = None
           ; done_summary = None
           ; next_summary = None
           ; next_items = []
  ...
  if selection.dropped_by_total_cap > 0 || selection.dropped_by_kind <> []
  then
    Eio.traceln
      "[keeper_memory] %s: memory_candidates dropped total_cap=%d kind=%s"
      meta.name
  ...
  then 0, []
    ...
    List.iter
      (fun (kind, text, priority) ->
         ...
         if not (Hashtbl.mem seen_kinds kind)
         then (
...
let summarize_memory_bank_lines (lines : string list) ~(recent_limit : int)
  ...
  let last_ts_unix =
    parsed
    |> List.fold_left (fun acc (row : keeper_memory_line) -> max acc row.ts_unix) 0.0
  in
  ...
  let kind_counts =
    ...
    |> List.sort (fun (ka, va) (kb, vb) ->
      ...
      then c
        ...
        let pa = Option.value ~default:min_int (Hashtbl.find_opt kind_priority_tbl ka) in
        let pb = Option.value ~default:min_int (Hashtbl.find_opt kind_priority_tbl kb) in
  ...
  let top_kind =
    ...
    | [] -> None
  in
  let recent_notes = parsed |> List.rev |> take (max 0 recent_limit) in
  { total_notes; last_ts_unix; top_kind; kind_counts; recent_notes }
;;
...
let memory_summary_to_json (summary : keeper_memory_summary) : Yojson.Safe.t =
  `Assoc
    ...
    ; ( "kind_counts"
      , `List
          (List.map
             (fun (kind, count) -> `Assoc [ "kind", `String kind; "count", `Int count ])
             summary.kind_counts) )
    ; ( "recent_notes"
      , `List
          (List.map
