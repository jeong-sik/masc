...
let resolved_keeper_args_to_json
  ...
  let shards_field =
    ...
    | None -> []
  in
  `Assoc (base @ allowed_paths_field @ autoboot_field @ tool_policy_field @ shards_field)
;;
...
let validate_resolved_keeper_create_json (json : Yojson.Safe.t) : string list =
  ...
  if mention_targets = [] then errors := "mention_targets is required" :: !errors;
  List.rev !errors
;;
...
let toml_float value =
  ...
  then Ok (Printf.sprintf "%.17g" value)
...
let append_int_field acc key value = Printf.sprintf "%s = %d" key value :: acc
...
let append_tool_access_fields acc json =
  ...
  | Some (`Assoc _ as tool_access) ->
    (match Safe_ops.json_string_opt "kind" tool_access with
     ...
     | Some "custom" ->
       Error
         "tool_access.kind=custom cannot be persisted to keeper TOML yet; use a \
          tool_preset/tool_also_allow policy for persona-backed durable keepers"
     | Some other ->
...
let render_keeper_toml_from_resolved_args (json : Yojson.Safe.t) : (string, string) result
  =
  match required_resolved_string "name" json with
  | Error _ as err -> err
  | Ok name ->
    if not (validate_name name)
    then Error "resolved_args.name is not a valid keeper name"
    else (
      match required_resolved_string "persona_name" json with
      | Error _ as err -> err
      | Ok persona_name ->
        if not (validate_name persona_name)
        then Error "resolved_args.persona_name is not a valid persona name"
        else (
          let fields = [] in
          ...
          let fields = append_optional_bool_field fields "policy_voice_enabled" json in
          let fields = append_optional_bool_field fields "autoboot_enabled" json in
          let fields = append_present_string_list_field fields "mention_targets" json in
          let fields = append_optional_bool_field fields "proactive_enabled" json in
          let fields = append_optional_int_field fields "proactive_idle_sec" json in
          let fields = append_optional_int_field fields "proactive_cooldown_sec" json in
          let fields = append_present_string_list_field fields "shards" json in
          let fields = append_present_string_list_field fields "allowed_paths" json in
          let fields = append_optional_string_field fields "sandbox_profile" json in
          let fields = append_optional_string_field fields "network_mode" json in
          let fields = append_optional_string_field fields "shared_memory_scope" json in
          let fields = append_present_string_list_field fields "active_goal_ids" json in
          match append_tool_access_fields fields json with
          | Error _ as err -> err
          | Ok fields ->
            let fields = append_present_string_list_field fields "tool_denylist" json in
            let timeout_fields =
              ...
              | Some value ->
                (match toml_float value with
                 | Error _ as err -> err
                 | Ok rendered ->
                   Ok (Printf.sprintf "per_provider_timeout = %s" rendered :: fields))
            in
            Result.map
              (fun fields ->
                 String.concat
                   "\n"
                   ([ "# Generated by masc_keeper_create_from_persona."; "[keeper]" ]
                    @ List.rev fields)
                 ^ "\n")
              timeout_fields))
;;
...
let persist_keeper_toml_from_resolved_args (json : Yojson.Safe.t)
  ...
  | Ok name ->
    ...
    then Error "resolved_args.name is not a valid keeper name"
      ...
      let path = Filename.concat (Config_dir_resolver.keepers_dir ()) (name ^ ".toml") in
        ...
        (match render_keeper_toml_from_resolved_args json with
         ...
         | Ok content ->
           ...
           (match Fs_compat.save_file_atomic path content with
            | Error msg -> Error msg
            | Ok () -> Ok (`Assoc [ "path", `String path; "created", `Bool true ]))))
;;
       ...
       | Ok () ->
         (match load_persona_summary persona_name with
          ...
          | Some persona ->
            ...
            let name = get_string_opt args "name" |> Option.value ~default:persona_name in
            let goal =
            ...
            let mention_targets =
              ...
              let raw =
                if explicit <> []
                then explicit
                else if defaults.mention_targets <> []
                then defaults.mention_targets
                else [ persona_name ]
              in
              raw
