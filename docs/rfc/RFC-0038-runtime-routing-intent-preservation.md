...
Runtime routing layer 가 사용자가 runtime.toml 에 명시한 model intent 와 무관한 모델을 silently 호출한다. 5월 1-6일 audit 에서 2,620 turns 중 254 turns (9.69%) 가 `configured_labels` 와 `candidate_models` 가 disjoint 한 silent substitution 을 겪었다. ollama 는 22번 의도된 후보로 등장했지만 실제로 1번만 실행됐다 (4.5% intent-realization rate).
...
| # | Invariant | Layer |
|---|-----------|-------|
| G1 | Cross-runtime fallback 은 default-off. fallback chain 은 runtime.toml 의 `fallback_runtime` 만 표현 path. | L0 |
...
**Migration guide**: runtime.toml 의 모든 user-facing runtime 가 `fallback_runtime` 명시. 예:
```toml
[primary]
...
(** Resolution order (highest precedence first):
    1. runtime.toml `[capability_overrides]` table per model-id
...
**runtime.toml override (Phase 1, 즉시)**:
```toml
...
| **PR-B** | L0 migration | `.masc/config/runtime.toml` 의 모든 runtime 에 명시적 `fallback_runtime` chain | PR-A | RFC-0027 §3.4 PR #4 (`__safe_lane`) |
| **PR-C** | B2 hot fix | `provider-k-coding:*` ↔ retired planning namespace mismatch 정규화 | none (parallel) | RFC-0027 §3 |
| **PR-D** | L1 override table | `runtime_config.ml` capability_overrides schema, runtime.toml 등록 | PR-A 권장 | RFC-0024, RFC-0027 |
...
- runtime.toml schema 추가 필드는 모두 optional.
