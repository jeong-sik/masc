...
- Task Review says which verdict each request waits on. The column read an `intent` field the verification queue has never sent, so it was blank on every row and the detail pane told every reader the row was "not joined (history view)" — while all seven awaiting requests were cancellations shown as completion claims short of evidence. The reading is now the closed pair the queue does send, and the detail pane draws the case a stop makes under `WHY IT SHOULD STOP` (#38015).
