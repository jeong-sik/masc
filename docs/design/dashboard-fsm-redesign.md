...
- 시나리오 C — Invariants violation: 테스트 전용 hook으로 cascade state를 `trying`으로 고정하고 measurement는 captured=false로 유지 → `/fsm` 페이지의 `no_cascade_before_measurement` invariant가 빨간색으로 표시되는지 확인. (이전에는 `recovery_two_store_sync` 시나리오였으나 #7334에서 manual_reconcile 제거 후 삭제됨)
