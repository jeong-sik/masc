...
let script_path () = Filename.concat (source_root ()) "scripts/pr-open.sh"
...
let run_shell_ok ?(env = []) ~cwd cmd =
  ...
  if code <> 0
  then failf "command failed (%d): %s\nstdout:\n%s\nstderr:\n%s" code cmd stdout stderr;
  stdout, stderr
;;
...
let init_repo_with_remote dir =
  ...
  ignore
    (run_shell_ok ~cwd:dir (Printf.sprintf "git init --bare -q %s" (quote remote_dir)));
  ignore
    (run_shell_ok ~cwd:dir (Printf.sprintf "git remote add origin %s" (quote remote_dir)));
...
let test_source_avoids_mapfile_only_bash4_features () =
  ...
  check bool "script no longer uses mapfile" false (contains_substring content "mapfile ");
  check
    bool
    "script no longer uses readarray"
    false
    (contains_substring content "readarray ");
  check
    bool
    "script has bash-compatible changed file loader"
    true
...
let test_script_runs_under_system_bash_without_watch () =
  with_temp_dir "pr-open-script" (fun dir ->
    ...
    let path =
      Printf.sprintf
        ...
        (match Sys.getenv_opt "PATH" with
         ...
         | None -> "")
    in
    let env = [ "PATH", path; "FAKE_GH_LOG", gh_log; "FAKE_GH_LABELS", gh_labels ] in
    let cmd =
      Printf.sprintf
        "/bin/bash %s --repo %s --title %s --body-file %s --no-watch"
        (quote (script_path ()))
        (quote "example/test")
    ...
    check
      bool
      "does not mention mapfile failure"
      false
    ...
    check
      bool
      "skips watched checks with --no-watch"
      false
      (contains_substring log "pr checks");
    let labels = read_file gh_labels in
    check
      bool
      "adds enhancement label for code changes"
      true
      (contains_substring labels "\"enhancement\"");
    check
      bool
      "does not add docs label for code-only change"
      false
...
let test_script_prints_final_status_after_watch () =
  with_temp_dir "pr-open-script-watch-status" (fun dir ->
    ...
    let cmd =
      Printf.sprintf
        "/bin/bash %s --repo %s --title %s --body-file %s"
        (quote (script_path ()))
        (quote "example/test")
    ...
    check bool "prints final status heading" true (contains_substring stdout "PR status:");
    check bool "prints final draft state" true (contains_substring stdout "draft=true");
    check
      bool
      "prints final merge state"
      true
...
let test_script_rejects_body_missing_required_sections () =
  with_temp_dir "pr-open-script-missing-sections" (fun dir ->
    ...
    check
      bool
      "mentions hygiene failure"
      true
      (contains_substring stderr "body file is missing required PR hygiene sections:");
    check
      bool
      "mentions product impact heading"
      true
      (contains_substring stderr "## Product impact");
    check
      bool
      "mentions linked issue heading"
      true
      (contains_substring stderr "## Linked issue");
    check bool "gh never invoked before validation" false (Sys.file_exists gh_log))
;;
...
let test_script_rejects_staged_changes_before_push () =
  with_temp_dir "pr-open-script-staged-changes" (fun dir ->
    ...
    check
      bool
      "mentions staged changes"
      true
      (contains_substring stderr "staged changes detected");
    check bool "mentions staged path" true (contains_substring stderr "lib/staged.ml");
    check bool "gh never invoked before staged validation" false (Sys.file_exists gh_log))
;;
