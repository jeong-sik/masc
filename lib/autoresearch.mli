...
type decision = Autoresearch_types.decision =
  | Keep
  | Discard
...
type status = Autoresearch_types.status =
  | Running
  | Completed
  | Stopped
  | Error
...
val load_execution_link_by_loop : base_path:string -> string -> execution_link option
val load_execution_link_by_session : base_path:string -> string -> execution_link option

val load_execution_link_by_loop_result
  :  base_path:string
  -> string
  -> (execution_link, string) result option
...
val load_state_result
  :  base_path:string
  -> string
  -> (persisted_summary, string) result option
...
val git_commit : workdir:string -> message:string -> (string option, string) result
