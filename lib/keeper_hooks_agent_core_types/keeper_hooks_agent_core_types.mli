(** Keeper_hooks_agent_core_types — pure type definitions and helpers extracted
    from Keeper_hooks_agent_core (2762 LoC godfile).

    Holds the cost_status verdict ADT + its pure label/reason converters.
    State-touching keeper_hooks_agent_core operations remain in Keeper_hooks_agent_core.
    Re-included by Keeper_hooks_agent_core so existing callers continue to use
    [Keeper_hooks_agent_core.cost_status] etc. unchanged. *)

(** Otel_metric_store + JSON label-key string constants used across
    keeper_hooks_agent_core.ml call sites. *)
...
(** JSON field-key string constants used across keeper_hooks_agent_core.ml. *)
...
(** Internal: cost-status wire labels exposed for keeper_hooks_agent_core's
...
val cost_label_agent_core_cost_unreported : string

val redact_inference_telemetry_json : Yojson.Safe.t -> Yojson.Safe.t
(** Redact provider/model identity fields from AGENT_CORE inference telemetry while
    preserving non-identifying runtime counters and timings. *)

val inference_telemetry_to_runtime_json :
  Agent_core.Types.inference_telemetry -> Yojson.Safe.t
...
val summarize_thinking_blocks :
  Agent_core.Types.content_block list -> thinking_log_summary
...
val usage_has_tokens : Agent_core.Types.api_usage -> bool
...
(** Neutral runtime lane used for keeper-facing tool-call telemetry.
    Concrete provider/model identity is AGENT_CORE-owned. *)

val stop_reason_to_label : Agent_core.Types.stop_reason -> string
(** Canonical telemetry/metric label for an AGENT_CORE stop reason.  Delegates to
    AGENT_CORE so [keeper_hooks_agent_core] finish-reason fields and response metrics share
...
val telemetry_has_canonical_model_id :
  Agent_core.Types.inference_telemetry option -> bool
(** Internal: true when telemetry carries a non-empty canonical_model_id. *)

val canonical_model_id_of_telemetry :
  Agent_core.Types.inference_telemetry option -> string option
...
val agent_core_reported_cost : Agent_core.Types.api_usage -> float
