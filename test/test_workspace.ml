...
let test_join_creates_agent () =
  ...
  let result = Workspace.join config ~agent_name:"test_agent" ~capabilities:["ocaml"] () in
...
let test_lifecycle_messages_are_typed () =
  with_test_env (fun config ->
    let join_result = Workspace.join config ~agent_name:"provider_f" ~capabilities:[] () in
    ...
    ignore (Workspace.join config ~agent_name:"provider_f" ~capabilities:[] ());
...
let test_leave_removes_agent () =
  with_test_env (fun config ->
    ...
    (* Leave *)
    let result = Workspace.leave config ~agent_name:"provider_f" in
...
let test_double_join () =
  with_test_env (fun config ->
    ...
    let result = Workspace.join config ~agent_name:"provider_f" ~capabilities:["updated"] () in
...
let test_agent_name_with_special_chars () =
  with_test_env (fun config ->
    ...
    let result = Workspace.join config ~agent_name:"model-a-sonnet-sonnet" ~capabilities:[] () in
...
let test_event_log_on_join () =
  ...
  let config = workspace_config tmp_dir in
  let _ = Workspace.init config ~agent_name:None in
  let _ = Workspace.join config ~agent_name:"test_agent" ~capabilities:["ocaml"] () in
...
let test_release_stale_claims_clears_agent_current_task () =
  with_test_env (fun config ->
    let _ = Workspace.join config ~agent_name:stale_nick ~capabilities:[] () in
...
let test_release_stale_claims_preserves_other_agent_task () =
  with_test_env (fun config ->
    let _ = Workspace.join config ~agent_name:other_nick ~capabilities:[] () in
    let _ = Workspace.add_task config ~title:"Stale work" ~priority:1 ~description:"" in
...
let test_get_agents_status () =
  with_test_env (fun config ->
    let _ = Workspace.join config ~agent_name:"provider_f" ~capabilities:["python"] () in
    let _ = Workspace.join config ~agent_name:"agent_code" ~capabilities:["rust"] () in
...
let make_stale_agent ?(agent_type = "test") config ~name ~age_seconds =
  let _ = Workspace.join config ~agent_name:name ~capabilities:[] () in
...
let test_find_by_capability () =
  with_test_env (fun config ->
    let _ = Workspace.join config ~agent_name:"provider_f" ~capabilities:["python"; "search"] () in
    let _ = Workspace.join config ~agent_name:"agent_code" ~capabilities:["python"; "rust"] () in
...
let test_korean_agent_name () =
  with_test_env (fun config ->
    ...
    let result = Workspace.join config ~agent_name:"클로드" ~capabilities:["한글"] () in
...
let test_many_agents () =
  with_test_env (fun config ->
    (* Join many agents *)
    for i = 1 to 10 do
      let _ = Workspace.join config ~agent_name:(Printf.sprintf "agent%d" i) ~capabilities:["test"] () in
      ()
    done;
...
let test_xss_in_message () =
  with_test_env (fun config ->
    ignore (Workspace.join config ~agent_name:"tester" ~capabilities:[] ());
...
let test_xss_in_agent_name () =
  with_test_env (fun config ->
    ...
    let result = Workspace.join config ~agent_name:xss_name ~capabilities:[] () in
...
let test_force_release_bypasses_assignee () =
  with_test_env (fun config ->
    ...
    let _ = Workspace.join config ~agent_name:test_agent_a ~capabilities:[] () in
...
let test_audit_orphan_tasks () =
  with_test_env (fun config ->
    ...
    let _ = Workspace.leave config ~agent_name:test_agent_a in
...
let test_cleanup_zombies_releases_tasks () =
  with_test_env (fun config ->
    ...
    let _ = Workspace.join config ~agent_name:test_agent_z ~capabilities:[] () in
...
let test_rejoin_preserves_identity () =
  with_test_env (fun config ->
    ...
    let join1 = Workspace.join config ~agent_name:"agent_llm_a" ~capabilities:["code"] () in
    ...
    (* 2. Leave *)
    let leave_result = Workspace.leave config ~agent_name:"agent_llm_a" in
    ...
    let join2 = Workspace.join config ~agent_name:"agent_llm_a" ~capabilities:["code"; "review"] () in
...
let test_rejoin_restores_active_status () =
  with_test_env (fun config ->
    let _ = Workspace.join config ~agent_name:"provider_f" ~capabilities:["search"] () in
    let _ = Workspace.leave config ~agent_name:"provider_f" in

    (* Re-join *)
    let result = Workspace.join config ~agent_name:"provider_f" ~capabilities:["search"] () in
...
let test_multiple_rejoin_cycles () =
  with_test_env (fun config ->
    let _ = Workspace.join config ~agent_name:"agent_code" ~capabilities:["impl"] () in
    let state1 = Workspace.read_state config in
    let nick1 = List.find (fun name ->
    ...
    for _ = 1 to 3 do
      let _ = Workspace.leave config ~agent_name:"agent_code" in
      let _ = Workspace.join config ~agent_name:"agent_code" ~capabilities:["impl"] () in
      ()
    done;
...
let test_rejoin_event_log () =
  with_test_env (fun config ->
    ...
    let _ = Workspace.join config ~agent_name:"logcheck" ~capabilities:[] () in
    let _ = Workspace.leave config ~agent_name:"logcheck" in
...
let test_bug006_transition_with_unsuffixed_name () =
  with_test_env (fun config ->
    ...
    let _ = Workspace.join config ~agent_name:"keeper-coder-agent" ~capabilities:["code"] () in
