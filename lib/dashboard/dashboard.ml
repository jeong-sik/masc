...
let split_tasks (tasks : Masc_domain.task list) =
  let active =
    List.filter (fun task ->
      ...
      | Masc_domain.Todo | Masc_domain.Done _ | Masc_domain.Cancelled _ | Masc_domain.OperatorBlocked _ -> false
    ) tasks
  in
  ...
  let content =
    (List.map (fun (task : Masc_domain.task) ->
      let assignee =
        ...
        | Masc_domain.Todo | Masc_domain.Done _ | Masc_domain.Cancelled _ | Masc_domain.OperatorBlocked _ -> "?"
      in
