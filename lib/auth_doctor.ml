...
let watched_agent_of_credential ~auth_dir agent_name credential_opt =
  let raw_token_file_present = file_exists (raw_token_file_path ~auth_dir agent_name) in
...
let admin_token_env_state ~base_path =
  ...
  | Some raw_token ->
    (match Auth.find_credential_by_token base_path ~token:raw_token with
     | Ok cred ->
       if has_permission cred.role CanAdmin then Env_admin cred else Env_non_admin cred
...
let admin_token_env_fields = function
  ...
  | Env_non_admin cred ->
    "non_admin", Some cred.agent_name, Some (agent_role_to_string cred.role)
  | Env_admin cred -> "admin", Some cred.agent_name, Some (agent_role_to_string cred.role)
;;
  ...
  |> List.filter_map (fun role ->
    let count =
      List.fold_left
        ...
        credentials
    in
    if count = 0 then None else Some (agent_role_to_string role, count))
;;
...
let admin_bearer_sources
      ...
      ~admin_token_env_state
      (credentials : agent_credential list)
  =
  let env_sources =
    match admin_token_env_state with
    | Env_admin cred ->
      [ Printf.sprintf "env:%s->%s" Env_config_core.admin_token_env_key cred.agent_name ]
    | Env_unset | Env_invalid_or_expired | Env_non_admin _ -> []
  in
  let dashboard_sources =
    if dashboard_dev_token_available then [ "dashboard_dev_endpoint" ] else []
  in
  let token_file_sources =
    credentials
    |> List.filter (fun (cred : agent_credential) -> has_permission cred.role CanAdmin)
    |> List.filter_map (live_admin_token_file_source ~base_path ~auth_dir)
  in
  env_sources @ dashboard_sources @ token_file_sources |> dedupe_keep_order
;;

let codex_mcp_report ~base_path =
  let config = Codex_mcp_config_doctor.analyze_default () in
  match Sys.getenv_opt codex_mcp_token_env_var |> Env_config_core.trim_opt with
  | None ->
    { server_name = "masc"
  ...
  | Some raw_token ->
    (match Auth.find_credential_by_token base_path ~token:raw_token with
     | Ok cred ->
       ...
       ; token_can_read_state = Some (has_permission cred.role CanReadState)
       ; login_supported = false
       ; login_note = codex_mcp_login_note
       ; config
       }
...
let analyze ~base_path_input ~default_base_path () =
  ...
  let base_path =
    let candidate =
      if String.trim normalized_base_path = ""
      then default_base_path
      else normalized_base_path
    in
  ...
  let dashboard_dev_token_available = bind_is_loopback && not http_auth_strict in
  ...
  let codex_mcp_client = Auth.load_credential base_path "codex-mcp-client" in
  let warnings =
    [ (if not auth_cfg.enabled
       then
         Some
           "Room auth is disabled; token-bound admin HTTP mutation routes will reject \
            until auth is enabled."
       else None)
    ; (if auth_cfg.enabled && not auth_cfg.require_token
       then
         Some
           "Room auth does not require bearer tokens; token-bound admin HTTP mutation \
            routes will reject until require_token=true."
       else None)
    ; (match admin_token_env_state with
       | Env_invalid_or_expired ->
         Some
    ...
    ; (if auth_cfg.enabled && auth_cfg.require_token && not token_bound_admin_http_ready
       then
         Some
           "No usable admin bearer source was detected for token-bound admin HTTP \
            mutation routes."
       else None)
    ; (match codex with
    ...
    ; (match codex_mcp_client with
       | Some cred when not (has_permission cred.role CanAdmin) ->
         Some
           (Printf.sprintf
              "codex-mcp-client is role=%s, so dashboard save flows using that bearer \
               will fail on admin-only routes such as POST /api/v1/cascade/config/raw."
              (agent_role_to_string cred.role))
       | _ -> None)
    ; (if not dashboard_dev_token_available
       then
         Some
           "Dashboard dev-token bootstrap is unavailable because the bind host is \
            non-loopback or HTTP strict auth is enabled."
       else None)
    ; (if auth_cfg.enabled && auth_cfg.require_token
       then (
         ...
         | "unset" ->
           Some
             "Codex MCP bearer env var MASC_MCP_TOKEN is unset; Codex should use \
              bearer_token_env_var, not `codex mcp login`."
         | "invalid_or_expired" ->
           Some
             "Codex MCP bearer env var MASC_MCP_TOKEN is set, but it does not resolve to \
              a live credential in this base path."
         | "live" -> None
         | _ -> None)
       else None)
    ]
  ...
  let next_actions =
    [ (if not auth_cfg.enabled
       then
         Some
           (Printf.sprintf
              "Enable room auth and set require_token=true in %s before relying on admin \
    ...
    ; (if auth_cfg.enabled && not auth_cfg.require_token
       then
         Some
           (Printf.sprintf
              "Set require_token=true in %s so token-bound admin HTTP routes can \
               authorize bearer tokens."
              (Auth.auth_config_file base_path))
       else None)
    ; (if
         auth_cfg.enabled
         && auth_cfg.require_token
         &&
         match codex_mcp_client with
       ...
       then
         Some
           "Use dashboard-dev or another admin bearer for POST \
            /api/v1/cascade/config/raw; the codex-mcp-client worker bearer cannot \
            satisfy CanAdmin."
       else None)
    ; (if auth_cfg.enabled && auth_cfg.require_token && dashboard_dev_token_available
       then
         Some
           "On loopback dev setups, fetch an admin bearer from GET \
            /api/v1/dashboard/dev-token before using admin-only dashboard actions."
       else None)
    ; (if auth_cfg.enabled && auth_cfg.require_token && not dashboard_dev_token_available
       then
         Some
           "Follow docs/LOCAL-DASHBOARD-AUTH-RUNBOOK.md to bootstrap an admin bearer \
            under the live auth root."
       else None)
    ; (if
         auth_cfg.enabled
         && auth_cfg.require_token
         && not (String.equal codex_mcp.token_status "live")
       then
         Some
           "For Codex MCP, run `masc-mcp login --agent codex-mcp-client --role worker \
            --shell` and export MASC_MCP_TOKEN; do not run `codex mcp login masc`."
       else None)
    ; Some
        "For Codex MCP pipeline drift, inspect the codex_mcp.config.stages section from \
         `masc-mcp doctor auth --json`."
...
let to_yojson (report : t) =
  `Assoc
    ...
    ; ( "role_counts"
      , `Assoc (List.map (fun (role, count) -> role, `Int count) report.role_counts) )
    ; "watched_agents", `List (List.map watched_agent_to_yojson report.watched_agents)
...
let render_text (report : t) =
  ...
  add_line (Printf.sprintf "initial_admin: %s" (option_value report.initial_admin));
  add_line (Printf.sprintf "bind_host: %s" report.bind_host);
  add_line (Printf.sprintf "bind_is_loopback: %s" (yes_no report.bind_is_loopback));
  add_line (Printf.sprintf "http_auth_strict: %s" (yes_no report.http_auth_strict));
  add_line
    (Printf.sprintf
       "dashboard_dev_token: available=%s file_present=%s"
  ...
  add_line
    (Printf.sprintf
       "token_bound_admin_http_ready: %s"
       (yes_no report.token_bound_admin_http_ready));
  add_line
    (Printf.sprintf
       "admin_bearer_sources: %s"
       (match report.admin_bearer_sources with
        | [] -> "(none)"
        | values -> String.concat ", " values));
  add_line (Printf.sprintf "credential_count: %d" report.credential_count);
  add_line
    (Printf.sprintf
       "role_counts: %s"
       (match report.role_counts with
        ...
        | values ->
          values
          |> List.map (fun (role, count) -> Printf.sprintf "%s=%d" role count)
          |> String.concat ", "));
  add_line "";
  ...
  add_line "";
  add_line "codex_mcp:";
  add_line (Printf.sprintf "- server_name: %s" report.codex_mcp.server_name);
  add_line (Printf.sprintf "- auth_model: %s" report.codex_mcp.auth_model);
  add_line (Printf.sprintf "- token_env_var: %s" report.codex_mcp.token_env_var);
  add_line
    (Printf.sprintf
       "- token_env_configured: %s"
       (yes_no report.codex_mcp.token_env_configured));
  add_line (Printf.sprintf "- token_status: %s" report.codex_mcp.token_status);
  add_line
    (Printf.sprintf "- token_agent: %s" (option_value report.codex_mcp.token_agent));
  add_line (Printf.sprintf "- token_role: %s" (option_value report.codex_mcp.token_role));
  add_line
    (Printf.sprintf
       "- token_can_read_state: %s"
       (match report.codex_mcp.token_can_read_state with
        | Some value -> yes_no value
        | None -> "(n/a)"));
  add_line
    (Printf.sprintf "- login_supported: %s" (yes_no report.codex_mcp.login_supported));
  add_line (Printf.sprintf "- login_note: %s" report.codex_mcp.login_note);
  add_line "- config:";
  add_line
    (Printf.sprintf "  path: %s" (option_value report.codex_mcp.config.config_path));
  add_line
    (Printf.sprintf "  file_present: %s" (yes_no report.codex_mcp.config.file_present));
  add_line
    (Printf.sprintf
       "  server_names: %s"
  ...
  then (
    add_line "";
    add_line "warnings:";
    List.iter (fun warning -> add_line (Printf.sprintf "- %s" warning)) report.warnings);
  if report.next_actions <> []
  then (
    add_line "";
    add_line "next_actions:";
    List.iter (fun action -> add_line (Printf.sprintf "- %s" action)) report.next_actions);
