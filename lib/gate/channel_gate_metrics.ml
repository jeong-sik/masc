...
let remember_room acc room_id =
  ...
  if not (Hashtbl.mem acc.rooms room_id)
  then (
    if
      Hashtbl.length acc.rooms >= max_tracked_rooms && not (Queue.is_empty acc.room_order)
    then (
...
let get_or_create_binding acc room_id =
  ...
  | None ->
    if
      Hashtbl.length acc.bindings >= max_tracked_rooms
...
let append_event ~channel ~room_id ~keeper ~duration_ms outcome ~timestamp =
  ...
  incr next_event_seq;
  if Queue.length recent_events >= max_recent_events
  then ignore (Queue.take recent_events);
  Queue.add
    { seq = !next_event_seq
    ; timestamp
    ; channel
...
let record_attempt ~channel ~room_id ~keeper ~duration_ms outcome =
  Eio_guard.with_mutex mu (fun () ->
    ...
    let channel_key =
      if trimmed_channel = "" then "unknown" else String.lowercase_ascii trimmed_channel
    in
    ...
    then (
      ...
      if duration_ms >= slow_threshold_ms () then acc.slow_count <- acc.slow_count + 1);
    (match binding with
    ...
    (match outcome with
     ...
     | Validation_error message ->
       ...
       (match binding with
        | Some binding ->
          update_binding_error_fields binding ~now ~kind:"validation" ~message
        | None -> ())
     | Keeper_error message ->
       acc.keeper_error_count <- acc.keeper_error_count + 1;
       update_error_fields acc ~now ~kind:"keeper" ~message;
       (match binding with
        | Some binding -> update_binding_error_fields binding ~now ~kind:"keeper" ~message
        | None -> ())
     | Dispatch_unavailable ->
     ...
     | Internal_error message ->
       ...
       (match binding with
        | Some binding ->
          update_binding_error_fields binding ~now ~kind:"internal" ~message
        | None -> ()));
    append_event
      ~channel:channel_key
      ~room_id:trimmed_room
      ~keeper:trimmed_keeper
      ~duration_ms
      outcome
      ~timestamp:now)
;;
...
let record_internal_error_exn ~channel ~room_id ~keeper ~duration_ms _exn =
  record_attempt ~channel ~room_id ~keeper ~duration_ms (Internal_error "internal error")
;;
...
let snapshot () : channel_stats list =
  ...
  |> List.sort (fun (a : channel_stats) (b : channel_stats) ->
    ...
    if by_messages <> 0 then by_messages else String.compare a.channel b.channel)
;;
...
let _binding_snapshot () =
  ...
  |> List.sort (fun a b ->
    ...
    then by_activity
      ...
      if by_channel <> 0 then by_channel else String.compare a.room_id b.room_id))
;;
...
let events ?channel ?keeper ?room_id ~limit () =
  Eio_guard.with_mutex_ro mu (fun () -> events_locked ?channel ?keeper ?room_id ~limit ())
;;
...
let percent numerator denominator =
  ...
  then 0
    ...
    int_of_float
      (Float.round (float_of_int numerator *. 100.0 /. float_of_int denominator))
;;
...
let success_rate_pct_counts ~success_count ~message_count ~duplicate_count =
  percent success_count (effective_attempt_count_counts ~message_count ~duplicate_count)
;;

let success_rate_pct (stats : channel_stats) =
  success_rate_pct_counts
    ~success_count:stats.success_count
    ~message_count:stats.message_count
    ~duplicate_count:stats.duplicate_count
;;

let slow_rate_pct (stats : channel_stats) = percent stats.slow_count stats.timed_count

let health_of_counts
      ~message_count
      ~error_count
      ~success_count
      ~duplicate_count
  ...
  then "failing"
    ...
    let err_rate =
      percent error_count (effective_attempt_count_counts ~message_count ~duplicate_count)
    in
    let slow_rate = percent slow_count timed_count in
    if err_rate >= 50
    then "failing"
    else if err_rate >= 10 || slow_rate >= 25
    then "degraded"
    else "healthy")
;;
...
let snapshot_json () =
  let channels, bindings, recent_events = Eio_guard.with_mutex_ro mu snapshot_locked in
  let total =
    List.fold_left
  ...
  in
  let total_errors =
    List.fold_left (fun sum (stats : channel_stats) -> sum + stats.error_count) 0 channels
  in
  ...
  let channel_json (stats : channel_stats) =
    let avg_dur =
      if stats.timed_count > 0 then stats.total_duration_ms / stats.timed_count else 0
    in
    `Assoc
  ...
  `Assoc
    ...
    ; "success_rate_pct", `Int (percent total_success (max 0 (total - total_duplicates)))
