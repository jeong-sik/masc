...
let test_caps_concurrency () =
  ...
  let body () =
    Runtime_binding_capacity.with_slot ~key:"cap-test" ~max_concurrent:(Some 2)
      (fun () ->
...
let test_none_is_ungated () =
  ...
  Eio.Fiber.all
    (List.init 4 (fun _ ->
         fun () ->
           Runtime_binding_capacity.with_slot ~key:"ungated" ~max_concurrent:None
             (fun () -> Atomic.incr ran)));
...
let test_release_on_exception () =
  ...
  let key = "exc-key" in
  (try
     Runtime_binding_capacity.with_slot ~key ~max_concurrent:(Some 1) (fun () ->
         failwith "boom")
   with Failure _ -> ());
...
let test_distinct_keys () =
  Eio_main.run @@ fun _env ->
  Runtime_binding_capacity.with_slot ~key:"A" ~max_concurrent:(Some 2) (fun () -> ());
  Runtime_binding_capacity.with_slot ~key:"B" ~max_concurrent:(Some 3) (fun () -> ());
  let cap_of k =
    List.find_map
    ...
    [
      ( "gate",
        [
          ...
          test_case "None runs ungated" `Quick
            test_none_is_ungated;
