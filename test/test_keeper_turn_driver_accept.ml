...
let response ?(content = []) ?(stop_reason = Masc_agent_core.Types.EndTurn) () =
  {
    Masc_agent_core.Types.id = "resp-test";
    model = "model-test";
    stop_reason;
    content;
    usage = None;
...
let checkpoint_with_messages messages : Masc_agent_core.Checkpoint.t =
  {
...
let test_input_required_bypasses_response_accept () =
  ...
  let reject (_ : Masc_agent_core.Types.api_response) =
    incr accept_calls;
    false
  in
...
let test_accept_rejected_threads_stop_reason () =
  ...
  Alcotest.(check bool) "MaxTokens threaded" true
    (threaded Masc_agent_core.Types.MaxTokens = Some Masc_agent_core.Types.MaxTokens);
  Alcotest.(check bool) "EndTurn threaded" true
    (threaded Masc_agent_core.Types.EndTurn = Some Masc_agent_core.Types.EndTurn)
...
let test_accept_rejected_stop_reason_survives_codec () =
  ...
  let err =
    accept_rejected_sdk_error
      ~stop_reason:(Some Masc_agent_core.Types.MaxTokens)
...
let test_finalization_does_not_surface_hidden_reasoning () =
  ...
  let response =
    run_result
      ~content:
        [ Masc_agent_core.Types.Thinking { signature = None; content = hidden }
  ...
  (match finalize [ "masc_schedule_get" ] with
   | Error err ->
     Alcotest.failf "tool fallback should succeed: %s" (Masc_agent_core.Error.to_string err)
   | Ok text ->
     Alcotest.(check string)
...
let test_prepare_degraded_retry_rejects_empty_runtime () =
  ...
  let err = Masc_agent_core.Error.Internal "empty direct response" in
...
let test_prepare_degraded_retry_reports_setup_failure () =
  ...
  in
  let err = Masc_agent_core.Error.Internal "empty direct response" in
  let setup_err = Masc_agent_core.Error.Internal "retry setup failed" in
  let retry : Masc.Keeper_error_classify.degraded_retry =
    {
  ...
  | Masc.Keeper_turn_runtime_budget.Degraded_retry_setup_failed
    ...
    Alcotest.(check string)
      "failure propagated"
      (Masc_agent_core.Error.to_string setup_err)
      (Masc_agent_core.Error.to_string fail_open_err);
    Alcotest.(check (list (pair string string)))
    ...
    (match List.rev !published with
     ...
     | rows ->
       Alcotest.failf "expected one prepared cascade event, got %d"
         (List.length rows));
    let setup_err = Masc_agent_core.Error.Internal "plan setup failed" in
    let step, published, selected, rotated =
...
let test_direct_retry_loop_publishes_non_retry_terminal_cascade () =
  let terminal_err = Masc_agent_core.Error.Internal "not retryable" in
  let initial_execution : Masc.Keeper_turn_runtime_budget.runtime_execution =
  ...
  (match result with
   ...
   | Error err ->
     Alcotest.(check string)
       ...
       (Masc_agent_core.Error.to_string terminal_err)
       (Masc_agent_core.Error.to_string err));
      ...
      (run_result
         ~content:
           [
             ...
             Masc_agent_core.Types.Text "final answer";
           ]
         ())
  in
  match result with
...
let check_accept_matches_oas_shape label content =
  ...
  let expected =
    response
    |> Masc_agent_core.Response_shape.summarize
...
let test_accept_contract_delegates_to_oas_response_shape () =
  ...
  check_accept_matches_oas_shape "blank text" [ Masc_agent_core.Types.Text " \n\t " ];
  check_accept_matches_oas_shape "text" [ Masc_agent_core.Types.Text "visible answer" ];
  check_accept_matches_oas_shape
    "tool use"
    [
      Masc_agent_core.Types.ToolUse
  ...
  let result =
    Masc.Keeper_turn_driver.For_testing.apply_accept
      ...
      (run_result
         ...
         ~stop_reason:Masc_agent_core.Types.StopSequence
         ())
  in
...
let test_empty_non_end_turn_response_is_rejected () =
  let result =
    Masc.Keeper_turn_driver.For_testing.apply_accept
      ...
      ~accept:Keeper_tool_response.response_has_text_or_tool_progress
      (run_result ~stop_reason:Masc_agent_core.Types.StopSequence ())
  in
  let err, reason_kind, reason = expect_accept_rejected result in
  Alcotest.(check bool)
    "reason kind is no usable progress"
...
let test_blank_text_non_end_turn_response_is_rejected () =
  let result =
    Masc.Keeper_turn_driver.For_testing.apply_accept
      ...
      ~accept:Keeper_tool_response.response_has_text_or_tool_progress
      (run_result
         ~content:[ Masc_agent_core.Types.Text " \n\t " ]
...
let test_custom_accept_reject_preserves_predicate_reason () =
  let result =
    Masc.Keeper_turn_driver.For_testing.apply_accept
      ...
      ~accept:(fun _ -> false)
      (run_result ~content:[ Masc_agent_core.Types.Text "visible answer" ] ())
  in
...
let test_dns_failure_exhaustion_classifies_as_runtime_exhausted () =
  ...
  | None ->
    Alcotest.failf "expected typed keeper error, got %s"
      (Masc_agent_core.Error.to_string mapped)
