...
let test_validate_rejects_readme () =
  let inputs =
    Adversarial_eval.[ Changed_file { path = "README.md"; content = "# Project" } ]
  in
  match Adversarial_eval.validate_inputs inputs with
...
let test_validate_rejects_design_doc () =
  let inputs =
    Adversarial_eval.
      [ Interface_contract
          { path = "docs/design/contract-driven-agent-loop-rfc.md"; content = "..." }
      ]
  in
  match Adversarial_eval.validate_inputs inputs with
...
let test_validate_rejects_room_history_path () =
  let inputs =
    Adversarial_eval.
      [ Changed_file { path = "memory/room-task-history.jsonl"; content = "{}\n" } ]
  in
  match Adversarial_eval.validate_inputs inputs with
...
let test_large_diff_warning () =
  ...
  in
  let ctx =
    Adversarial_eval.create_context ~session_id:"test" ~inputs:[ Diff big_diff ]
  in
  ...
  Alcotest.(check bool)
    ...
    true
    (List.exists
       (fun (f : Adversarial_eval.advisory_finding) -> String.equal f.category "scope")
       result.findings)
;;
...
let test_small_diff_no_warning () =
  ...
  let ctx =
    Adversarial_eval.create_context ~session_id:"test" ~inputs:[ Diff small_diff ]
  in
  let result = Adversarial_eval.evaluate ctx in
  Alcotest.(check bool)
    "no scope warning"
...
let test_unsafe_pattern_detection () =
  let ctx =
    Adversarial_eval.create_context
      ~session_id:"test"
      ~inputs:
        [ Changed_file
            { path = "lib/hack.ml"
  ...
  let safety_findings =
    List.filter
      (fun (f : Adversarial_eval.advisory_finding) -> String.equal f.category "safety")
      result.findings
  in
...
let test_missing_test_warning () =
  let ctx =
    Adversarial_eval.create_context
      ~session_id:"test"
      ~inputs:[ Changed_file { path = "lib/new_module.ml"; content = "let f () = ()" } ]
  in
  let result = Adversarial_eval.evaluate ctx in
  Alcotest.(check bool)
    "has testing warning"
    true
    (List.exists
       (fun (f : Adversarial_eval.advisory_finding) -> String.equal f.category "testing")
       result.findings)
;;
...
let test_always_advisory () =
  let ctx = Adversarial_eval.create_context ~session_id:"test" ~inputs:[] in
  let result = Adversarial_eval.evaluate ctx in
...
let test_result_to_yojson () =
  let ctx =
    Adversarial_eval.create_context ~session_id:"test" ~inputs:[ Diff "+line 1" ]
  in
  let result = Adversarial_eval.evaluate ctx in
  ...
  Alcotest.(check int) "input_count" 1 (json |> member "input_count" |> to_int)
;;
...
let () =
  Alcotest.run
    ...
    ; "advisory", [ Alcotest.test_case "always advisory" `Quick test_always_advisory ]
    ; ( "serialization"
