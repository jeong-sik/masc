...
Clients must know to open a separate `GET /sse` for streaming events. MCP 2025-03-26 clients (and the new Provider-A / Chat Completions v1 client SDKs) instead expect `POST /mcp` to be either synchronous *or* chunked depending on the server's processing time — one URL, one connection.
...
- [[RFC-0095]] — Chat Completions v1-compatible provider streaming wire-up (provider edge, in main)
