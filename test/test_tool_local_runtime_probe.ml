...
let test_ollama_generate_parser_computes_tok_per_second () =
  ...
  in
  let run_json =
    Masc_mcp.Tool_local_runtime.ollama_probe_run_of_generate_json
      ~run_index:1
      ~http_status:(Some 200)
      ~wall_clock_ms:9120
      json
  in
  let prompt_tps = Option.value ~default:(-1.0) run_json.prompt_tokens_per_second in
  let generation_tps =
  ...
  check_float_close "generation tok/sec computed" 57.2924649539597 generation_tps;
  check bool "thinking detected" true run_json.thinking_present;
  check (option string) "response preview kept" (Some "READY") run_json.response_preview
;;
...
let test_request_body_omits_keep_alive_by_default () =
  ...
  check bool "thinking disabled by default" false (json |> member "think" |> to_bool)
;;
...
let test_request_body_can_enable_thinking () =
  ...
  in
  let open Yojson.Safe.Util in
  check bool "thinking can be requested" true (json |> member "think" |> to_bool)
;;
...
let test_think_mode_parses_adaptive_policy () =
  let parse raw =
    ...
    |> Option.map Masc_mcp.Tool_local_runtime_probe.ollama_probe_think_mode_to_string
  in
  check (option string) "auto parsed" (Some "auto") (parse " auto ");
  check (option string) "disabled alias parsed" (Some "disabled") (parse "off");
  check (option string) "enabled alias parsed" (Some "enabled") (parse "YES");
...
let test_runtime_probe_reports_effective_think_mode () =
  ...
  check string "auto mode reported" "auto" (auto |> member "think_mode" |> to_string);
  check bool "auto effective think false" false (auto |> member "think" |> to_bool);
  ...
  check bool "enabled effective think true" true (enabled |> member "think" |> to_bool)
;;
...
let test_runtime_probe_status_only_skip_is_telemetry () =
  ...
  check
    string
    "skip reason reported"
    "status_only"
    (json |> member "generate_skip_reason" |> to_string);
  check bool "status-only flag reported" false (json |> member "run_generate" |> to_bool);
...
let test_endpoint_urls_use_normalized_base () =
  check
    string
    "ps endpoint normalized"
    "http://127.0.0.1:11434/api/ps"
    (Masc_mcp.Tool_local_runtime_probe.ollama_ps_url "http://127.0.0.1:11434/");
  check
    string
    "generate endpoint normalized"
    "http://127.0.0.1:11434/api/generate"
    (Masc_mcp.Tool_local_runtime_probe.ollama_generate_url "http://127.0.0.1:11434///")
;;
...
let test_ollama_ps_non_200_is_reported_as_error () =
  check
    string
    "ps non-200 surfaced"
    "ollama ps returned http 503"
    (Masc_mcp.Tool_local_runtime_probe.ollama_http_error "ps" (Some 503))
;;

let test_kv_cache_assessment_detects_repeat_improvement () =
  let runs =
    [ `Assoc [ "run_index", `Int 1; "prompt_eval_duration_ms", `Float 500.0 ]
    ; `Assoc [ "run_index", `Int 2; "prompt_eval_duration_ms", `Float 260.0 ]
    ; `Assoc [ "run_index", `Int 3; "prompt_eval_duration_ms", `Float 280.0 ]
    ]
  in
  let assessment = Masc_mcp.Tool_local_runtime.kv_cache_assessment_json runs in
  let open Yojson.Safe.Util in
  check string "likely reuse" "likely_reused" (assessment |> member "signal" |> to_string);
  check
    (option int)
    "best repeat run"
    (Some 2)
...
let test_generate_probe_is_skipped_after_failed_preflight () =
  check
    bool
    "ps preflight error skips generate"
    false
  ...
  check
    bool
    "ps preflight error dominates successful status"
    false
    (Masc_mcp.Tool_local_runtime_probe.should_attempt_generate_probe
       ~before_status:(Some 200)
       ~before_error:(Some "invalid ps payload")
       ~run_generate:true
       ~generate_when_unloaded:true
       ~effective_model_loaded_before:true);
  check
    bool
    "successful ps allows generate"
    true
    (Masc_mcp.Tool_local_runtime_probe.should_attempt_generate_probe
       ~before_status:(Some 200)
       ~before_error:None
  ...
  check
    bool
    "successful ps skips cold model when disabled"
    false
    (Masc_mcp.Tool_local_runtime_probe.should_attempt_generate_probe
       ~before_status:(Some 200)
       ~before_error:None
  ...
  check
    bool
    "resident model can still be probed when cold load disabled"
    true
    (Masc_mcp.Tool_local_runtime_probe.should_attempt_generate_probe
       ~before_status:(Some 200)
       ~before_error:None
  ...
  check
    bool
    "status-only probe skips generate even when resident"
    false
    (Masc_mcp.Tool_local_runtime_probe.should_attempt_generate_probe
       ~before_status:(Some 200)
       ~before_error:None
  ...
  check
    bool
    "default path with no status allows generate when enabled"
    true
    (Masc_mcp.Tool_local_runtime_probe.should_attempt_generate_probe
       ~before_status:None
       ~before_error:None
  ...
  check
    bool
    "default path blocks when disabled and not resident"
    false
    (Masc_mcp.Tool_local_runtime_probe.should_attempt_generate_probe
       ~before_status:None
       ~before_error:None
...
let test_generate_probe_decision_reports_typed_reasons () =
  let decision
        ...
        ()
    =
    Masc_mcp.Tool_local_runtime_probe.decide_generate_probe
      ~effective_model
      ~before_status
      ~before_error
      ~run_generate
  ...
  in
  check string "no model reason" "no_effective_model" (decision ~effective_model:None ());
  check string "status-only reason" "status_only" (decision ~run_generate:false ());
  check
    string
  ...
  check
    string
    "cold model skip reason"
    "model_unloaded"
    (decision ~before_status:200 ~generate_when_unloaded:false ());
  check
    string
    "unknown preflight skip reason"
    "policy_skip"
    (decision ~generate_when_unloaded:false ());
  check
    string
    "resident model runs even with cold-load disabled"
    "run_generate"
    (decision
       ~before_status:200
       ~generate_when_unloaded:false
