...
let test_strengthen () =
  with_temp_masc_dir (fun config ->
    ...
    assert (s.Hebbian_eio.weight > 0.5)
    (* Started at 0.5, strengthened *));
...
let test_consolidate () =
  with_temp_masc_dir (fun config ->
    ...
    let synapses, _ = Hebbian_eio.get_graph_data config in
    assert (List.length synapses >= 0)
    (* May or may not prune depending on params *));
...
let test_multiple_agents () =
  with_temp_masc_dir (fun config ->
    ...
    print_endline
      (Printf.sprintf
         "  Network: %d synapses, %d agents"
         (List.length synapses)
         (List.length agents)));
...
let test_custom_params () =
  with_temp_masc_dir (fun config ->
    ...
    let params =
      { Hebbian_eio.strengthen_rate = 0.2
      ; (* Stronger learning *)
        weaken_rate = 0.1
      ; decay_rate = 0.05
      ; min_weight = 0.1
...
let test_lock_stats () =
  with_temp_masc_dir (fun config ->
    ...
    (* At least 3 lock acquisitions *)
    print_endline
      (Printf.sprintf
         "  Lock stats: %d acquisitions, %.2fms avg, %.2fms max"
         acquisitions
         avg_wait
         max_wait));
...
let test_append_history_caps_and_preserves_order () =
  ...
  let h =
    let rec fill i h =
      if i > Hebbian_eio.history_cap + 5
      then h
      else
  ...
  Alcotest.run
    ...
    [ ( "synapse"
      , [ Alcotest.test_case "strengthen" `Quick test_strengthen
        ...
        ; Alcotest.test_case "weaken nonexistent" `Quick test_weaken_nonexistent
        ] )
    ; ( "queries"
      , [ Alcotest.test_case "get preferred partner" `Quick test_get_preferred_partner
        ; Alcotest.test_case "no preferred partner" `Quick test_no_preferred_partner
        ; Alcotest.test_case "multiple agents network" `Quick test_multiple_agents
        ] )
    ; ( "maintenance"
      , [ Alcotest.test_case "consolidate" `Quick test_consolidate
        ; Alcotest.test_case "custom learning params" `Quick test_custom_params
        ; Alcotest.test_case "lock stats" `Quick test_lock_stats
        ] )
    ; ( "consolidation_fiber"
      , [ Alcotest.test_case
            "fiber runs consolidate periodically"
            `Quick
            test_consolidation_fiber_runs
        ] )
    ; ( "weight_history"
      , [ Alcotest.test_case
            "appends on strengthen"
            `Quick
        ...
        ; Alcotest.test_case "JSON roundtrip" `Quick test_weight_history_json_roundtrip
        ; Alcotest.test_case
            "caps and preserves order"
            `Quick
