...
let test_current_room_defaults_to_default () =
  with_config (fun config ->
    check (option string) "default room" (Some "default") (Coord.read_current_room config))
;;
...
let test_current_room_write_and_resolve_scope () =
  with_config (fun config ->
    ...
    check
      (option string)
      "compat pointer stays default"
      (Some "default")
      (Coord.read_current_room focused);
    check
      string
      "resolved scope stays default"
      "default"
...
let test_current_room_writes_stay_canonical () =
  with_config (fun config ->
    ...
    check int "default namespace task count" 1 (List.length (Coord.get_tasks_raw config));
    (* All rooms are flattened to default — get_tasks_safe is the single path *)
    check
      int
      "same tasks regardless of former room"
      1
  ...
  let rec collect_jsonl dir =
    ...
    then []
      ...
      |> List.concat_map (fun name ->
        ...
        if Sys.is_directory path
        then collect_jsonl path
        else if Filename.check_suffix name ".jsonl"
        then [ path ]
        else [])
  in
    ...
    Fun.protect
      ...
      (fun () ->
         Atomic.set
           ...
           (fun _config ~agent_id:_ ~event ~details:_ ->
              captured_event_kind
              := Some (Coord_hooks.agent_lifecycle_event_to_string event));
         let result =
           Coord.join config ~agent_name:"claude" ~capabilities:[ "debug" ] ()
         in
         check bool "join succeeds" true (String.length result > 0);
         check (option string) "hook sees join event" (Some "join") !captured_event_kind;
         let event_log =
