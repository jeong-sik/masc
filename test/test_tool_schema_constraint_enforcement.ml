(** Pre-dispatch enforcement of declared JSON-Schema range/length bounds.
    ...
    Agent-Core's input validation reads only type/enum/const/required on
    root properties, never a bound. A caller learned about an out-of-range
    value only if the handler happened to re-check it — which is how
    [keeper_artifact_read] answered [max_bytes=565244] with a message that
    named [sha256] first (2026-08-05 20:06:36).
