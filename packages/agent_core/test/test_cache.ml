...
let test_parse_usage_with_cache_tokens () =
  ...
  let resp = Masc_agent_core.Llm_provider.Backend_anthropic.parse_response json in
...
let test_openai_usage_with_cached_tokens () =
  ...
  let resp =
    match
      Masc_agent_core.Llm_provider.Backend_openai_parse.parse_openai_response_result json_str
    with
    | Ok r -> r
...
let test_streaming_message_delta_with_cache () =
  ...
  in
  match
    Masc_agent_core.Llm_provider.Streaming.parse_sse_event (Some "message_delta") data_str
  with
