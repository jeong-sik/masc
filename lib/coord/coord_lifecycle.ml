...
let agent_parse_error_snapshot ~agent_name ~agent_file =
  ...
  `Assoc
    ...
    ; ("raw_head", if raw_head = "" then `Null else `String raw_head)
    ]
;;
...
let join
  ...
  let agent_type =
    match agent_type_override with
    | Some t -> t
    | None ->
      (* Check if agent_name looks like a nickname (has dashes) *)
      if Nickname.is_generated_nickname agent_name
      then Option.value (Nickname.extract_agent_type agent_name) ~default:agent_name
      else agent_name (* Legacy: agent_name is the type *)
  in
  ...
  let nickname =
    if Nickname.is_generated_nickname agent_name
    then agent_name (* Already a nickname, use as-is *)
    else (
  ...
  then (
    (match read_agent_with_repair config agent_file_dedup with
     | Ok existing_agent ->
       ...
       let new_session_id =
         ...
         then generate_session_id ()
           ...
           match existing_agent.meta with
           | Some m -> m.session_id
       ...
       then (
         ...
         log_event
           ...
           (Printf.sprintf
              ...
              new_session_id
              (now_iso ()));
         (Atomic.get Coord_hooks.observe_agent_lifecycle_fn)
           config
           ~agent_id:nickname
     ...
     | Error e ->
       let snapshot =
         agent_parse_error_snapshot ~agent_name:nickname ~agent_file:agent_file_dedup
       in
       Log.Coord.warn
...
let leave config ~agent_name =
  ...
  then (
    ...
    (match read_agent_with_repair config agent_file with
     ...
     | Error e ->
       let snapshot = agent_parse_error_snapshot ~agent_name:actual_name ~agent_file in
       Log.Coord.warn
    ...
    (Atomic.get Coord_hooks.observe_agent_lifecycle_fn)
      config
      ~agent_id:actual_name
      ~event:Coord_hooks.Lifecycle_leave
      ~details:`Null;
    (* Record co-presence relationships via hook (async, non-blocking) *)
    (try
       (Atomic.get Coord_hooks.relation_on_leave_fn)
         ~leaving_agent:actual_name
         ~active_agents:peers_before_leave
     with
     | Eio.Cancel.Cancelled _ as e -> raise e
     | exn ->
       Log.Coord.error
         "relation-materializer leave hook error: %s"
         (Printexc.to_string exn));
    Printf.sprintf "✅ %s left the namespace" actual_name)
  else Printf.sprintf "⚠ %s was not in the namespace" actual_name
;;
