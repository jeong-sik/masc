...
let test_static_safe_rejects_network_command () =
  ...
  | Error reason ->
    Alcotest.failf
      "expected curl to fail static safe verification, got %s"
      (Exec_policy.block_reason_tag reason)
  | Ok _ -> Alcotest.fail "expected curl to be rejected"
;;

let test_static_safe_rejects_shell_interpreter () =
  match verify_static_safe "bash -c 'echo x > file'" with
  | Error (Exec_policy.Command_not_allowed "bash") -> ()
  | Error reason ->
    Alcotest.failf
      "expected bash to fail static safe verification, got %s"
      (Exec_policy.block_reason_tag reason)
...
let () =
  run "ppx_safe_shell" [
    ...
    "static_safe", [
      ...
      test_case "rejects network command" `Quick
        test_static_safe_rejects_network_command;
      test_case "rejects shell interpreter" `Quick
        test_static_safe_rejects_shell_interpreter;
    ];
  ]
