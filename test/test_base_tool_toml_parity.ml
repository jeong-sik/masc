...
let expected =
    ...
    ; {|keeper_assemble_plan|}, {|Ask the configured Assembler exact-output lane to build one typed Tool DAG from an objective and exact ordinary-Tool references frozen for this Keeper turn. The result is a durable content-addressed proposal_id. This call never executes the proposal or any referenced Tool; pass the proposal_id to the separate proposal execution surface when execution is intended.|}, {|{"additionalProperties":false,"properties":{"execution":{"enum":["inline","async"],"type":"string"},"objective":{"minLength":1,"pattern":"^([^ \t\n\f\r]|[ \t\n\f\r])*[^ \t\n\f\r]([^ \t\n\f\r]|[ \t\n\f\r])*$","type":"string"},"ordinary_tool_references":{"items":{"additionalProperties":false,"properties":{"capability_id":{"minLength":1,"pattern":"^([^ \t\n\f\r]|[ \t\n\f\r])*[^ \t\n\f\r]([^ \t\n\f\r]|[ \t\n\f\r])*$","type":"string"},"descriptor_id":{"minLength":1,"pattern":"^([^ \t\n\f\r]|[ \t\n\f\r])*[^ \t\n\f\r]([^ \t\n\f\r]|[ \t\n\f\r])*$","type":"string"}},"required":["descriptor_id","capability_id"],"type":"object"},"minItems":1,"type":"array","uniqueItems":true}},"required":["objective","execution","ordinary_tool_references"],"type":"object"}|}
    ]
;;
