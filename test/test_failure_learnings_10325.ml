  ...
  List.iter
    (fun s ->
       check
         ...
         false
         (String.length s > 30
          && String.equal (String.sub s 0 30) "persist failed keeper turns so"))
    learnings
;;
...
let test_kind_and_message_emitted_as_tags () =
  let learnings =
    extract_failure_learnings
      ...
      ~error_message:"keeper exceeded budget after 12 turns"
  in
  let has_kind = List.exists (fun s -> s = "error_kind:oas_timeout_budget") learnings in
  let has_message_tag =
    List.exists
      (fun s -> String.length s > 14 && String.equal (String.sub s 0 14) "error_message:")
      learnings
  in
...
let test_no_learning_sentinel_when_both_blank () =
  let learnings = extract_failure_learnings ~error_kind:"" ~error_message:"" in
  check (list string) "explicit absence sentinel" [ "[NO_LEARNING]" ] learnings
;;

let test_only_kind_when_message_blank () =
  let learnings =
    extract_failure_learnings ~error_kind:"resumable_cli_session" ~error_message:""
  in
  check
    (list string)
    "only kind tag emitted"
...
let () =
  run
    "failure_learnings_10325"
    [ ( "learnings_emit"
      , [ test_case "no generic boilerplate string" `Quick test_no_generic_boilerplate
        ; test_case
            "kind and message become tag entries"
            `Quick
            test_kind_and_message_emitted_as_tags
        ; test_case
            "[NO_LEARNING] sentinel when both blank"
            `Quick
            test_no_learning_sentinel_when_both_blank
        ; test_case
            "only error_kind when message blank"
            `Quick
