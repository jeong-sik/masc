...
let test_grpc_subscribe_receives_sse_broadcast () =
  Eio_main.run (fun _env ->
    ...
    Alcotest.(check bool) "empty before broadcast" true (Grpc_eio.Stream.is_empty stream);
    (* Fire an SSE broadcast *)
    Masc_mcp.Sse.broadcast (`Assoc [ "type", `String "task_update" ]);
    (* The gRPC stream should now have an event *)
    Alcotest.(check bool)
      "not empty after broadcast"
      false
      (Grpc_eio.Stream.is_empty stream);
    ...
    Alcotest.(check bool)
      ...
      true
      (try
         let _ = Str.search_forward (Str.regexp_string "data:") event.payload_json 0 in
         true
       with
       | Not_found -> false);
    (* Cleanup *)
...
let test_grpc_subscribe_multiple_broadcasts () =
  Eio_main.run (fun _env ->
    ...
    Alcotest.(check bool)
      "seq ordering"
      true
      (Int64.compare e1.seq e2.seq < 0 && Int64.compare e2.seq e3.seq < 0);
    Masc_mcp.Sse.unsubscribe_external sub_id;
    Grpc_eio.Stream.close stream)
;;
...
let test_ws_external_subscriber_receives_broadcast () =
  Eio_main.run (fun _env ->
    ...
    Alcotest.(check bool)
      "contains ws_test data"
      true
      (try
         let _ = Str.search_forward (Str.regexp_string "ws_test") event 0 in
         true
       with
       | Not_found -> false);
    Masc_mcp.Sse.unsubscribe_external sub_id)
...
let test_ws_parse_handles_real_broadcast_wire_format () =
  Eio_main.run (fun _env ->
    ...
    | Some sse_event ->
      (match Masc_mcp.Server_mcp_transport_ws.parse_sse_dashboard_event sse_event with
       | None ->
         Alcotest.fail
           "parse returned None on real broadcast wire format (regression of #10194)"
       | Some parsed ->
...
let test_webrtc_full_signaling_flow () =
  Eio_main.run (fun _env ->
    ...
    Alcotest.(check bool) "offer pending" true (Wrtc.pending_offer_count () > 0);
    ...
    let answer_body =
      Printf.sprintf {|{"offer_id":"%s","agent_name":"agent-b"}|} offer_id
    in
    ...
    Alcotest.(check bool) "offer consumed" true (Option.is_none (Wrtc.get_offer offer_id));
    (* Peer should be active *)
    Alcotest.(check bool) "peer active" true (Wrtc.active_peer_count () > 0);
    (* Mark connected and cleanup *)
    Wrtc.mark_connected peer_id;
    Wrtc.remove_peer peer_id;
    Alcotest.(check int) "no active peers after cleanup" 0 (Wrtc.active_peer_count ()))
;;
...
let test_grpc_stream_closed_triggers_cleanup () =
  Eio_main.run (fun _env ->
    ...
    Masc_mcp.Sse.subscribe_external
      ...
      ~callback:(fun sse_event ->
        if not (Grpc_eio.Stream.is_closed stream)
        then (
          let seq = Int64.of_int (Atomic.fetch_and_add seq_counter 1) in
          let event =
            T.Event.
...
let test_all_protocol_variants_roundtrip () =
  ...
  List.iter
    (fun p ->
       ...
       | Some p' -> Alcotest.(check bool) (Printf.sprintf "%s roundtrip" s) true (p = p')
       | None -> Alcotest.fail (Printf.sprintf "roundtrip failed for %s" s))
    all
;;
...
let test_agent_transport_all_variants () =
  ...
  List.iter
    (fun t ->
       ...
       Alcotest.(check bool) (Printf.sprintf "%s non-empty" s) true (String.length s > 0))
    all
;;
...
let () =
  Alcotest.run
    ...
    [ ( "grpc_subscribe_sse"
      , [ Alcotest.test_case
            "broadcast reaches gRPC stream"
            `Quick
            test_grpc_subscribe_receives_sse_broadcast
        ; Alcotest.test_case
            "multiple broadcasts in order"
            `Quick
    ...
    ; ( "ws_sse"
      , [ Alcotest.test_case
            "broadcast reaches WS subscriber"
            `Quick
            test_ws_external_subscriber_receives_broadcast
        ; Alcotest.test_case
            "parse handles real broadcast wire format"
            `Quick
            test_ws_parse_handles_real_broadcast_wire_format
        ] )
    ; ( "webrtc_signaling"
      , if Sys.getenv_opt "MASC_TEST_WEBRTC" = Some "1"
        then
          [ Alcotest.test_case
              "full offer/answer/cleanup flow"
              `Quick
    ...
    ; ( "auto_cleanup"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "closed gRPC stream triggers cleanup"
            `Quick
            test_grpc_stream_closed_triggers_cleanup
        ] )
    ; ( "transport_enum"
      , [ Alcotest.test_case
            "all protocol variants roundtrip"
            `Quick
            test_all_protocol_variants_roundtrip
        ; Alcotest.test_case
            "all agent transport variants"
            `Quick
