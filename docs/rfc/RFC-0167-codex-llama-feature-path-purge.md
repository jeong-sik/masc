# RFC-0167 Codex CLI omission-dedup + Llama endpoint-discovery feature-path purge
...
| Scope | `lib/cascade/cascade_transport_codex_omission_dedup.{ml,mli}`, `lib/cascade/cascade_transport.{ml,mli}` facade + caller, `lib/cascade/cascade_config_provider_filter.ml`, `lib/cascade/cascade_metrics.{ml,mli}`, `test/test_codex_cli_omission_dedup_10097.ml` |
| Repos | masc-mcp |
...
1. `cascade_transport_codex_omission_dedup` (#10097): a 5-function module that fingerprints per-keeper codex_cli MCP-tool omissions and emits WARN-dedup + per-tool Prometheus counter.
...
### Codex omission-dedup
- Delete `lib/cascade/cascade_transport_codex_omission_dedup.{ml,mli}` wholesale.
- Delete the 5 facade re-exports in `cascade_transport.{ml,mli}` (`codex_cli_omission_fingerprint`, `codex_cli_omission_fingerprint_seen`, `record_codex_cli_omission`, `record_codex_cli_omission_for_agent`, `reset_codex_cli_omission_dedup_for_tests`).
- Delete `test/test_codex_cli_omission_dedup_10097.ml` (paired test, exclusive caller of the facade).
- Reword the one internal caller in `cascade_transport.ml` to client-agnostic names: `codex_can_auth_keeper_bound_actor_tools` → `provider_can_auth_keeper_bound_actor_tools`, `codex_keeper_bound_actor_tools` → `omitted_keeper_bound_actor_tools`. The `record_codex_cli_omission_for_agent` call at line 239 is removed; the structural omission detection that returns `Error (invalid_runtime_config ...)` remains intact.
...
| codex_cli runtime adapter omits keeper-bound MCP tools (per-keeper bridging required, no per-keeper bearer) | Logged WARN-once + ticked per-tool counter + (if `required`) returned `Error (invalid_runtime_config ...)` | Same `Error` path retained; WARN + counter removed. |
| Test `test_codex_cli_omission_dedup_10097` | Validated WARN-once + counter semantics | Removed; the validated behavior no longer exists. |

The Llama discovery removal is a *feature removal*. The codex omission-dedup removal is an *observability removal* — the structural error detection (the part that affects runtime behavior) is unchanged.

## 4. Domain boundary closure

After this RFC, `lib/` and `bin/` source contain zero MCP-client or upstream-provider name literals except for RFC-0166 / RFC-0167 closeout comments that historically cite the swept names (self-documenting). The remaining `claude` / `gemini` / `codex` / `llama` hits in the repository live in:

- `docs/` (history, audit reports, runbooks)
- `dashboard/src/` (frontend)
- `test/` fixtures other than the deleted codex omission dedup test
...
- `rg 'codex_cli_omission|on_llama_model_not_discovered|Some \("llama"' lib/ bin/` returns only RFC-0167 closeout comments.
- `rg -i 'claude|gemini|codex|kimi|anthropic|glm|llama' lib/ bin/` returns only:
...
- The structural codex_cli WARN/counter is gone; rely on the `tool_support` `Error` path for required-tool omissions on codex_cli transports.
