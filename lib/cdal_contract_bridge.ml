...
let infer_keeper_risk_class ~(scope_kind : string) ~(execution_scope : string) :
    Oas.Risk_class.t =
  match String.lowercase_ascii execution_scope with
  | "observe_only" | "read_only" | "readonly" | "observe" ->
      Oas.Risk_class.Low
  | "limited_code_change" | "workspace" | "local" | "autonomous"
  | "execute" | "full" | "unrestricted" ->
...
let infer_keeper_execution_mode ~(scope_kind : string)
    ~(execution_scope : string) : Oas.Execution_mode.t =
  match String.lowercase_ascii execution_scope with
  | "observe_only" | "read_only" | "readonly" | "observe" ->
      Oas.Execution_mode.Diagnose
  | "limited_code_change" | "workspace" | "local" -> Oas.Execution_mode.Draft
  | "autonomous" | "execute" | "full" | "unrestricted" ->
      Oas.Execution_mode.Execute
  | _ -> (
      match String.lowercase_ascii scope_kind with
...
let of_keeper
  ...
  let risk_class = infer_keeper_risk_class ~scope_kind ~execution_scope in
  {
    Oas.Risk_contract.runtime_constraints =
      {
        requested_execution_mode =
          infer_keeper_execution_mode ~scope_kind ~execution_scope;
        risk_class;
        allowed_mutations =
