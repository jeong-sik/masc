...
let convert_sqlite_snapshot (s : Engine_store_sqlite.snapshot) : snapshot =
  { last_seq = s.last_seq; ts = s.ts; state = s.state }

let convert_jsonl_snapshot (s : Engine_store.snapshot) : snapshot =
  { last_seq = s.last_seq; ts = s.ts; state = s.state }
...
let load_world_contracts_from_dir ~base_dir : Yojson.Safe.t =
  ...
  if not (Sys.file_exists path) then `Null
    ...
    match Util.read_json_eio path with
    | exception _ -> `Null
    | json -> json

let make_sqlite ~base_dir : t =
  let _ = Engine_store_sqlite.init ~base_dir in
  {
    append_event = (fun ~event -> Engine_store_sqlite.append_event ~base_dir ~event);
    read_events = (fun ~room_id -> Engine_store_sqlite.read_events ~base_dir ~room_id);
    read_events_after =
      (fun ~room_id ~after_seq ->
        Engine_store_sqlite.read_events_after ~base_dir ~room_id ~after_seq);
    write_snapshot =
      (fun ~room_id ~last_seq ~ts ~state ->
        Engine_store_sqlite.write_snapshot ~base_dir ~room_id ~last_seq ~ts ~state);
    read_snapshot =
      (fun ~room_id ->
        Engine_store_sqlite.read_snapshot ~base_dir ~room_id
        |> Result.map (Option.map convert_sqlite_snapshot));
    load_recovery =
      (fun ~room_id ->
        Engine_store_sqlite.load_recovery ~base_dir ~room_id
        |> Result.map (fun (snap_opt, events) ->
               (Option.map convert_sqlite_snapshot snap_opt, events)));
    load_catalog = (fun () -> Preset_store.load_catalog ~base_dir);
...
let make_jsonl ~base_dir : t =
  {
    append_event = (fun ~event -> Engine_store.append_event ~base_dir ~event);
    read_events = (fun ~room_id -> Engine_store.read_events ~base_dir ~room_id);
    read_events_after =
      (fun ~room_id ~after_seq ->
        Engine_store.read_events_after ~base_dir ~room_id ~after_seq);
    write_snapshot =
      (fun ~room_id ~last_seq ~ts ~state ->
        Engine_store.write_snapshot ~base_dir ~room_id ~last_seq ~ts ~state);
    read_snapshot =
      (fun ~room_id ->
        Engine_store.read_snapshot ~base_dir ~room_id
        |> Result.map (Option.map convert_jsonl_snapshot));
    load_recovery =
      (fun ~room_id ->
        Engine_store.load_recovery ~base_dir ~room_id
        |> Result.map (fun (snap_opt, events) ->
