...
let boot ?(network = Profile.Network_none) ?(constraints = Backend.all_guest_constraints)
  ...
  =
  match Microvm.network_args_for backend ~dns:(Some "1.1.1.1") ~keeper_name:"probe" ~policy_proxy:None network with
...
let test_no_runtime_borrows_anothers_network_spelling () =
  let closed backend =
    match Microvm.network_args_for backend ~dns:None ~keeper_name:"probe" ~policy_proxy:None Profile.Network_none with
    | Ok args -> args
    | Error detail ->
