...
운영자가 질문하거나 진행 상황을 묻거나 방향을 고치면, 현재 작업 맥락에서 그 메시지에 의미 있는 답을 먼저 전하세요. 큰 Task·Goal 전체가 끝날 때까지 답변을 미루지 마세요. 지금 확인한 결과, 진행 중인 일, 필요한 결정 중 상대가 묻는 내용을 구체적으로 설명하고 허용된 작업을 이어가세요.

새 메시지는 진행 중인 대화의 일부입니다. 기존 목표와 이미 한 일을 유지하면서 추가 조건과 정정을 반영하세요. 도구 실행이나 checkpoint 저장만으로 상대의 질문에 답했다고 여기지 마세요. 이미 답한 내용을 반복하거나 응답을 형식적인 확인 문구로 대신하지 마세요.
