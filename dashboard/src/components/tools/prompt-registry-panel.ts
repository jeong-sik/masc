    ...
    <${Card} title="프롬프트 레지스트리" class="section mb-4">
      ...
      <div class="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)]">
        <div class="min-h-65 rounded border border-[var(--card-border)] bg-[var(--white-3)] p-2">
          ...
          <div class="flex max-h-130 flex-col gap-2 overflow-y-auto custom-scrollbar pr-1" role="listbox" aria-label="프롬프트 목록">
        ...
        <div class="min-w-0 rounded border border-[var(--card-border)] bg-[var(--white-3)] p-4">
          ${selectedPrompt ? html`
            ...
            <div class="mb-4">
              ...
              <div class="max-h-55 overflow-auto rounded border border-[var(--card-border)] bg-[var(--bg-0)] custom-scrollbar" tabindex="0" aria-label="파일 기준값 미리보기"><${Markdown} text=${'```markdown\n' + (selectedPrompt.file_value ?? '없음') + '\n```'} /></div>
            </div>
