(** SSE (Server-Sent Events) module for MCP Streamable HTTP Transport
    ...
    workspace traffic.  [Workspace_session] sessions receive heartbeats
...
type session_kind =
  ...
  | Workspace_session [@tla.symbol "workspace_session"] (** MCP agent connections *)
...
type broadcast_target =
  ...
  | Workspace_sessions (** Only [Workspace_session] sessions *)
...
let sync_transport_snapshot () =
  ...
  let workspace_session = ref 0 in
  ...
  SMap.iter (fun session_id client ->
    ...
    (match client.kind with
     ...
     | Workspace_session -> incr workspace_session
  ...
  Transport_metrics.set_sse_sessions ~kind:"workspace_session" !workspace_session;
...
let event_matches_session_kind kind event =
  match kind with
  | Observer -> true
  | Workspace_session -> Sse_jsonrpc_filter.event_string_jsonrpc_message_for_workspace_session event
  | Presence -> false
...
let register ?(kind = Workspace_session) ?on_disconnect session_id ~last_event_id =
...
let client_matches_target target ~jsonrpc_payload (client : client) =
  ...
  | All -> (
      match client.kind with
      | Observer -> true
      | Workspace_session -> jsonrpc_payload
      | Presence -> false)
  | Observers -> client.kind = Observer
  | Workspace_sessions -> client.kind = Workspace_session && jsonrpc_payload
...
let broadcast_impl ?(buffer = true) ?(notify_external = true)
  ...
  let jsonrpc_payload =
    Sse_jsonrpc_filter.jsonrpc_message_for_workspace_session json
  in
...
(** Broadcast event to sessions matching [target].
    ...
    - [Workspace_sessions]: MCP agent sessions only *)
...
let send_to session_id json =
  if not (Sse_jsonrpc_filter.jsonrpc_message_for_workspace_session json) then
    Log.Server.warn
