...
let int_option_of_yojson = function
  ...
  | `Intlit value ->
    (match int_of_string_opt value with
     | Some v -> Ok (Some v)
     | None -> Error "invalid int option in worker helper payload")
...
let api_response_to_yojson = option_to_yojson Llm_provider.Cache.response_to_json
...
let proof_to_yojson = option_to_yojson Oas.Cdal_proof.to_json
...
let run_result_of_yojson (json : Yojson.Safe.t)
  ...
  try
    ...
    let* output_tokens = int_option_of_yojson (json |> member "output_tokens") in
    ...
    let* raw_trace_run =
      ...
      | value ->
        (match Oas.Raw_trace.run_ref_of_yojson value with
         ...
         | Error msg -> Error ("invalid raw_trace_run in worker helper payload: " ^ msg))
    in
         ...
         let kind =
           ...
           | Some (`String value) ->
             (match error_kind_of_string value with
              ...
              | None ->
                Log.Misc.warn
                  "worker_runtime_helper: unknown error_kind %S → Internal fallback \
                   (#8705)"
                  value;
                Internal)
           | _ -> Internal
