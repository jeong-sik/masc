...
let rec cleanup_dir path =
  ...
  then
    ...
    then (
      Sys.readdir path |> Array.iter (fun name -> cleanup_dir (Filename.concat path name));
      Unix.rmdir path)
...
let test_oas_worker_failed_lifecycle_includes_error () =
  ...
  Masc_event_bus.set bus;
  let sub = Event_bus.subscribe bus in
  Oas_worker_exec.publish_lifecycle
    bus
    ~name:"worker-a"
    ~event:"failed"
  ...
  | Event_bus.Custom ("masc.oas_worker.failed", payload) ->
    ...
    Alcotest.(check string) "agent" "worker-a" (payload |> member "agent" |> to_string);
    ...
    Alcotest.(check string) "status" "failed" (payload |> member "status" |> to_string)
...
let test_event_bus_task_transition () =
  ...
  Masc_event_bus.set bus;
  let sub = Event_bus.subscribe bus in
  Oas_events.publish_task_transition
    bus
    ~agent_name:"worker"
    ~task_id:"task-1"
    ~transition:Types_core.Done_action;
  let events = Event_bus.drain sub in
  Alcotest.(check int) "one event" 1 (List.length events);
  match (List.hd events : Event_bus.event).payload with
...
let test_event_bus_keeper_lifecycle_includes_phase () =
  ...
  Oas_events.publish_keeper_lifecycle
    bus
    ~event:
      (Masc_mcp.Keeper_lifecycle_events.Custom_event
...
let test_keeper_snapshot_envelope_agent_name () =
  ...
  | Some (`Assoc fields) ->
    ...
    Alcotest.(check string) "envelope agent_name" "sojin" (field_string "agent_name")
    ...
    (fun () ->
       ...
       try
         Eio.Switch.run (fun sw ->
           Oas_event_bridge.start_with_interval
           ...
           (match List.hd events with
            | `Assoc fields ->
              ...
              Alcotest.(check string) "run id" "run-bridge" (field_string "run_id")
    ...
    (fun () ->
       ...
       try
         Eio.Switch.run (fun sw ->
           ...
           Alcotest.(check bool) "observer got oas lifecycle" true (observer_event <> None);
           Alcotest.(check bool)
             "coordinator got oas lifecycle"
             true
    ...
    (fun () ->
       ...
       try
         Eio.Switch.run (fun sw ->
           ...
           Alcotest.(check bool)
             "observer eventually sees recovered event"
             true
    ...
    (fun () ->
       ...
       try
         Eio.Switch.run (fun sw ->
           ...
           Alcotest.(check bool) "drop marker broadcast" true (observer_event <> None);
           match observer_event with
           | None -> Alcotest.fail "expected relay drop marker"
           | Some raw_event ->
             let json = sse_data_json raw_event in
             let event_type = Yojson.Safe.Util.(member "type" json |> to_string) in
             let failed_stage =
               Yojson.Safe.Util.(member "failed_stage" json |> to_string)
             in
             Alcotest.(check string) "marker type" "oas:relay_dropped" event_type;
...
let test_oas_event_bridge_broadcast_retry_does_not_duplicate_append () =
  ...
  let pending_after_failure =
    ...
    | Oas_event_bridge.For_testing.Retryable_failure
        (pending, Oas_event_bridge.For_testing.Broadcast, _) -> pending
  ...
  Alcotest.(check bool)
    "pending remembers durable append"
    true
...
let test_message_roundtrip () =
  let masc_msg : Agent_sdk.Types.message = Agent_sdk.Types.assistant_msg "test content" in
  let oas_msg =
  ...
  | Some msg ->
    ...
    Alcotest.(check string)
      "role preserved"
      "assistant"
      (match roundtrip.role with
       | Agent_sdk.Types.Assistant -> "assistant"
       | _ -> "other");
    Alcotest.(check string)
      "content preserved"
      "test content"
      (Agent_sdk.Types.text_of_message roundtrip)
;;

let test_system_role_dropped () =
  let masc_msg : Agent_sdk.Types.message = Agent_sdk.Types.system_msg "system prompt" in
  let oas_msg =
    (fun (m : Agent_sdk.Types.message) ->
       match m.role with
       | Agent_sdk.Types.System -> None
...
let test_oas_context_sync () =
  ...
  let ctx = Keeper_exec_context.append ctx (Agent_sdk.Types.user_msg "hello") in
...
let test_compact_syncs_oas_context () =
  ...
  let messages =
    ...
    Context_compact_oas.compact
      ...
      ~strategies:[ Context_compact_oas.MergeContiguous ]
      ()
  in
  let ctx =
  ...
  in
  match ratio with
  | Some (`Float r) -> Alcotest.(check bool) "ratio is non-negative" true (r >= 0.0)
...
let test_agent_completed_includes_usage () =
  ...
  | Some (`Assoc fields) ->
    ...
    (match List.assoc_opt "cost_usd" payload_fields with
     | Some (`Float f) -> Alcotest.(check (float 0.001)) "cost_usd" 0.003 f
...
let test_oas_log_bridge_turn_completed_summary () =
  ...
  | entry :: _ ->
    ...
    (match entry.details with
     | `Assoc fields ->
       Alcotest.(check bool) "details preserve turn" true (List.mem_assoc "turn" fields)
  ...
  Alcotest.run
    ...
    [ ( "oas_events"
      , [ Alcotest.test_case "broadcast event" `Quick test_event_bus_broadcast
        ...
        ; Alcotest.test_case
            "oas worker failed lifecycle includes error"
            `Quick
            test_oas_worker_failed_lifecycle_includes_error
        ; Alcotest.test_case "task transition event" `Quick test_event_bus_task_transition
        ; Alcotest.test_case
            "keeper lifecycle includes phase"
            `Quick
            test_event_bus_keeper_lifecycle_includes_phase
        ; Alcotest.test_case
            "keeper snapshot envelope carries agent_name (#7827)"
            `Quick
            test_keeper_snapshot_envelope_agent_name
        ; Alcotest.test_case
            "keeper lifecycle envelope carries agent_name (#7827)"
            `Quick
            test_keeper_lifecycle_envelope_agent_name
        ; Alcotest.test_case
            "sse bridge persists native events"
            `Quick
            test_oas_event_bridge_persists_native_events
        ; Alcotest.test_case
            "sse bridge sends lifecycle to observers"
            `Quick
            test_oas_event_bridge_broadcasts_lifecycle_to_observers
        ; Alcotest.test_case
            "sse bridge retries append failure then recovers"
            `Quick
            test_oas_event_bridge_retries_append_failure_then_recovers
        ; Alcotest.test_case
            "sse bridge emits drop marker on exhausted append failure"
            `Quick
            test_oas_event_bridge_drop_marker_on_exhausted_append_failure
        ; Alcotest.test_case
            "sse bridge retry avoids duplicate append after broadcast failure"
            `Quick
            test_oas_event_bridge_broadcast_retry_does_not_duplicate_append
        ; Alcotest.test_case
            "agent_completed includes usage"
            `Quick
            test_agent_completed_includes_usage
        ; Alcotest.test_case
            "agent_completed no usage on error"
            `Quick
            test_agent_completed_no_usage_on_error
        ; Alcotest.test_case
            "oas log bridge adds turn completed summary"
            `Quick
            test_oas_log_bridge_turn_completed_summary
        ; Alcotest.test_case
            "sse bridge logs turn completed with agent name"
            `Quick
            test_oas_event_bridge_logs_turn_completed_with_agent_name
        ; Alcotest.test_case
            "sse bridge logs tool completed with agent name"
            `Quick
    ...
    ; ( "message_conversion"
      , [ Alcotest.test_case "message roundtrip" `Quick test_message_roundtrip
        ; Alcotest.test_case "system role dropped" `Quick test_system_role_dropped
    ...
    ; ( "context_oas_sync"
      , [ Alcotest.test_case "sync_oas_context" `Quick test_oas_context_sync
        ; Alcotest.test_case "compact syncs oas" `Quick test_compact_syncs_oas_context
        ] )
    ]
;;
