...
let sdk_bindings : sdk_tool_binding list =
  [ { sdk_name = "masc_list_tasks"
    ...
    ; description =
        "List all tasks in the MASC room with status, assignee, and priority. Use after \
         joining a room to find available work or check what others are doing."
  ...
  ; { sdk_name = "masc_add_task"
    ...
    ; description =
        "Create a single new task in the MASC room backlog. Use when you identify work \
         that any agent can pick up. Returns a task-XXX ID for tracking."
    ; input_schema =
        object_schema
          ~required:[ "title"; "description" ]
          [ assoc_field "title" (string_prop "Task title")
    ...
    ; arg_bindings =
        [ "title", Input_field "title"; "description", Input_field "description" ]
    ; discovery_hidden = false
    }
  ...
  ; { sdk_name = "masc_claim_task"
    ...
    ; description =
        "Claim a specific task by task_id, locking it to your agent. Use when you want a \
         particular task rather than the next available one."
    ; input_schema =
        object_schema
          ~required:[ "task_id" ]
  ...
  ; { sdk_name = "masc_claim_next"
    ...
    ; description =
        "Claim the next available task automatically by priority order. Use when you are \
         ready to work and any pending task is acceptable."
    ; input_schema = object_schema []
  ...
  ; { sdk_name = "masc_set_current_task"
    ...
    ; description =
        "Bind the claimed task as current_task when your claim path did not do it \
         automatically."
    ; input_schema =
        object_schema
          ~required:[ "task_id" ]
          [ assoc_field
              "task_id"
              (string_prop "The claimed task ID to bind as the current planning task")
          ]
  ...
  ; { sdk_name = "masc_complete_task"
    ; canonical_operation = "masc_transition"
    ; description =
        "Mark a task as done after finishing the work and verification. Use when \
         implementation is complete to release the task from your assignment."
    ; input_schema =
        object_schema
          ~required:[ "task_id" ]
          [ assoc_field "task_id" (string_prop "The task ID to mark as completed") ]
    ; arg_bindings =
  ...
  ; { sdk_name = "masc_broadcast"
    ...
    ; description =
        "Broadcast a message to all agents currently in the room. Use when sharing \
         status updates, coordination signals, or requesting help from any available \
         agent."
    ; input_schema =
        object_schema
          ~required:[ "message" ]
          [ assoc_field "message" (string_prop "The message to broadcast") ]
    ; arg_bindings = [ "agent_name", Agent_name; "message", Input_field "message" ]
    ; discovery_hidden = false
    }
...
let find_property properties key = List.assoc_opt key properties
...
let required_names schema =
  match
    Option.bind (assoc_members schema) (fun pairs -> find_property pairs "required")
  with
  | Some (`List items) ->
    List.filter_map
      (function
...
let property_map schema =
  match
    Option.bind (assoc_members schema) (fun pairs -> find_property pairs "properties")
  with
          ...
          let rec validate_props = function
            ...
            | (name, property_schema) :: rest ->
              (match List.assoc_opt name pairs with
               ...
               | Some property_value ->
                 (match
                    validate_json_value ~label:name property_schema property_value
                  with
  ...
  | "array" ->
    (match value with
     | `List items ->
       ...
       if min_items > 0 && List.length items < min_items
       then Error (Printf.sprintf "%s must be a non-empty JSON array" label)
       else (
         match
           Option.bind (assoc_members schema) (fun pairs -> find_property pairs "items")
         with
         | None -> Ok ()
         | Some item_schema ->
...
let validate_input_json schema json = validate_json_value ~label:"input" schema json
       ...
       let rec build acc = function
         ...
         | (target_name, source) :: rest ->
           (match source with
            ...
            | Agent_name -> build ((target_name, `String agent_name) :: acc) rest
