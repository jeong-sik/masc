...
let workspace_memory_context_http_json ~base_path =
  let keepers_dir = Config_dir_resolver.keepers_dir_for_base_path ~base_path in
  let encode_snapshot encode = function
    | Ok None -> `Assoc [ "status", `String "missing" ]
    | Ok (Some snapshot) ->
      `Assoc [ "status", `String "available"; "snapshot", encode snapshot ]
    | Error detail ->
      `Assoc [ "status", `String "unavailable"; "detail", `String detail ]
  in
  (* The optional store readers deliberately use a forgiving existence check.
     This reporting boundary must not label inaccessible paths as absent. Once
     stat observed a regular file, a later None is an interrupted observation
     (removal or access change), not evidence that no snapshot was stored. *)
  let read_snapshot path read =
    try
      ...
      | { Unix.st_kind = Unix.S_REG; _ } ->
        (try
           match read () with
           | Ok None -> Error (path ^ ": snapshot became unavailable during read")
           | result -> result
         with
         | Unix.Unix_error (error, operation, failed_path) ->
           Error (Printf.sprintf "%s: %s: %s" operation failed_path (Unix.error_message error)))
      | _ -> Error (path ^ ": snapshot is not a regular file")
    with
  ...
  let discovery =
    try
      (match Unix.stat keepers_dir with
       | { Unix.st_kind = Unix.S_DIR; _ } -> Ok (health_keeper_ids ~keepers_dir)
    ...
    | Sys_error detail -> Error detail
  in
  let entries keeper_ids =
    List.map (fun keeper_id ->
      `Assoc
        [ "keeper_id", `String keeper_id
        ; "ordinary", encode_snapshot Keeper_memory_os_current.to_json
            (read_snapshot
               (Keeper_memory_os_current.path_for_keepers_dir ~keepers_dir ~keeper_id)
               (fun () -> Keeper_memory_os_current.read_for_keepers_dir ~keepers_dir ~keeper_id))
        ; "source_bound", encode_snapshot Keeper_memory_source_current.to_json
            (read_snapshot
               (Keeper_memory_source_current.path_for_keepers_dir ~keepers_dir ~keeper_id)
  ...
  in
  `Assoc
    ([ "schema", `String "workspace.memory.context.v1"
     ; "generated_at", `Float (Time_compat.now ())
     ; "source_validation", `String "stored_bindings_not_revalidated"
     ; "consistency", `String "individual_store_snapshots"
     ] @ match discovery with
       | Ok keeper_ids ->
         [ "discovery", `Assoc [ "status", `String "available" ]
         ; "keepers", `List (entries keeper_ids) ]
       | Error detail ->
         [ "discovery", `Assoc [ "status", `String "unavailable"; "detail", `String detail ]
         ; "keepers", `List [] ])
;;
