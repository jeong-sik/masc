...
| Agent execution recovery | Draft leaf for server-owned runtime and local workers | `runtime_oas_execution.ml` owns one application runtime and exact slot/context association. `Runtime_agent.run*` requires an explicit stable `session_id`, mutation-boundary checkpoint sink, and consumer-settlement sink before Agent construction; missing ownership fails as a typed config error and no non-durable compatibility call exists. `Worker_oas` persists OAS mutation-boundary checkpoints. OAS `Internal` failures retain the slot fail-closed because v0.216.4 does not yet expose typed terminal disposition. The slot/checkpoint retirement crash window and projection retention remain blockers, so this leaf is not merge-ready. |
