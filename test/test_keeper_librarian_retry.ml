...
let input () : Librarian.input =
  ...
  ; messages =
      [ Masc_agent_core.Types.make_message
          ...
          [ Masc_agent_core.Types.Text "new conversation" ]
      ]
  }
;;
...
let user_text_of_messages messages =
  ...
  |> List.filter_map (fun (m : Masc_agent_core.Types.message) ->
    if m.role = Masc_agent_core.Types.User
    then
      Some
        (m.content
         |> List.filter_map (function
           | Masc_agent_core.Types.Text s -> Some s
           | Masc_agent_core.Types.ToolResult _ | Masc_agent_core.Types.ToolUse _
           | Masc_agent_core.Types.Thinking _ | Masc_agent_core.Types.ReasoningDetails _
           | Masc_agent_core.Types.RedactedThinking _ | Masc_agent_core.Types.Image _
           | Masc_agent_core.Types.Document _ | Masc_agent_core.Types.Audio _ -> None)
         |> String.concat "\n")
    else None)
  |> String.concat "\n"
;;
