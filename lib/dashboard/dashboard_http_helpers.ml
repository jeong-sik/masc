...
let int_of_env_default name ~default ~min_v ~max_v =
  let v =
    match Sys.getenv_opt name with
    | None -> default
    | Some s -> Option.value ~default (int_of_string_opt (String.trim s))
  in
...
let float_of_env_default name ~default ~min_v ~max_v =
  let v =
    match Sys.getenv_opt name with
    | None -> default
    | Some s -> Option.value ~default (float_of_string_opt (String.trim s))
  in
  max min_v (min max_v v)
;;
...
let operator_snapshot_session_window_seconds () =
  float_of_env_default
    "MASC_OPERATOR_SNAPSHOT_SESSION_WINDOW_SECONDS"
    ~default:Masc_time_constants.day
    ~min_v:300.0
    ~max_v:(Masc_time_constants.days_to_seconds 7)
;;

let operator_snapshot_session_limit () =
  int_of_env_default
    "MASC_OPERATOR_SNAPSHOT_SESSION_LIMIT"
    ~default:20
    ~min_v:5
    ~max_v:200
;;
...
let operator_snapshot_recent_completed_limit () =
  int_of_env_default
    "MASC_OPERATOR_SNAPSHOT_RECENT_COMPLETED_LIMIT"
    ~default:5
    ~min_v:1
    ~max_v:50
;;
...
let operator_snapshot_status_event_limit () =
  int_of_env_default
    "MASC_OPERATOR_SNAPSHOT_STATUS_EVENT_LIMIT"
    ~default:200
    ~min_v:20
    ~max_v:2000
;;
...
let parse_tool_call_detail (detail_opt : string option) : string * bool * int option =
  ...
  | Some raw ->
    ...
    List.iter
      (fun token ->
         ...
         | Some ("timeout", v) -> timeout := bool_of_tag_value v
         | Some ("duration_ms", v) ->
           (match int_of_string_opt v with
            | Some n -> duration_ms := Some (max 0 n)
            | None -> Log.Dashboard.warn "invalid duration_ms value: %s" v)
         | Some (_, _) | None -> ())
      tags;
...
let percentile_int (values : int list) ~(pct : float) : int option =
  ...
  | sorted ->
    ...
    let idx =
      int_of_float (ceil (pct *. float_of_int n) -. 1.0) |> max 0 |> min (n - 1)
    in
...
let safe_age_seconds_opt ~(now_ts : float) ~(event_ts : float) : int option =
  ...
  if Float.is_nan delta || Float.is_infinite delta
  then None
  else (
...
let json_int_field key json ~default =
  match safe_member key json with
  | `Int value -> value
  | `Intlit raw -> Option.value ~default (int_of_string_opt raw)
  | _ -> default
;;
...
let count_where items predicate =
  List.fold_left (fun acc item -> if predicate item then acc + 1 else acc) 0 items
;;
