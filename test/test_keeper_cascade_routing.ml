...
let select phase = Routing.select_cascade ~base_cascade:base ~phase
...
let test_failing_with_local_only_base () =
  let r =
    Routing.select_cascade
      ~base_cascade:Masc_mcp.Keeper_config.local_only_cascade_name
      ~phase:SM.Failing
  in
  check
    string
    "Failing overrides even local_only base"
    Masc_mcp.Keeper_config.local_recovery_cascade_name
    r.effective_cascade
;;
...
let test_running_with_custom_base () =
  ...
  check string "Running preserves custom base" "coding_first" r.effective_cascade
;;

let test_tool_required_turn_preserves_routed_cascade () =
  let r =
    Routing.route_effective_cascade_for_tool_requirement
      ~effective_cascade:"big_three"
      ~tool_requirement:"required"
  in
  check
    string
    "required tool turns preserve routed cascade"
    "big_three"
    r.effective_cascade;
  check
    bool
    "required tool turns do not rewrite to strict cascade"
    false
    (String.equal Masc_mcp.Keeper_config.tool_use_strict_cascade_name r.effective_cascade)
;;
...
let test_tool_required_turn_preserves_local_recovery () =
  ...
  check
    string
    "required tool turns preserve local recovery"
    Masc_mcp.Keeper_config.local_recovery_cascade_name
    r.effective_cascade
;;
...
let () =
  run
    ...
    ; ( "edge_cases"
      , [ test_case
            "Failing overrides local_only base"
            `Quick
            test_failing_with_local_only_base
        ; test_case "Running preserves custom base" `Quick test_running_with_custom_base
        ; test_case
            "Required tool turn preserves routed cascade"
            `Quick
            test_tool_required_turn_preserves_routed_cascade
        ; test_case
            "Required tool turn preserves local recovery"
            `Quick
