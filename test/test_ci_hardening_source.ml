  ...
  then None
    ...
    Fun.protect
      ...
      (fun () ->
         let content = In_channel.input_all ic in
         let re = Str.regexp_string pattern in
         try Some (Str.search_forward re content 0) with
         | Not_found -> None))
;;
...
let test_ci_sync_and_asset_contracts () =
  ...
  check
    bool
    "ci gate uses agent draft policy script"
    true
    (file_contains_pattern
       ".github/workflows/ci.yml"
  ...
  check
    bool
    "ci gate refreshes live draft state"
    true
    (file_contains_pattern ".github/workflows/ci.yml" "PR_LIVE_IS_DRAFT");
  check
    bool
    "ci gate refreshes live labels"
    true
  ...
  check
    bool
    "ci gate refreshes live PR state"
    true
    (file_contains_pattern ".github/workflows/ci.yml" "PR_LIVE_STATE");
  check
    bool
    "pr hygiene no longer checks dashboard assets (gitignored)"
    true
    (not
       (file_contains_pattern
          "scripts/check-pr-hygiene.sh"
          "dashboard source or Vite config changed but assets/dashboard was not updated"))
;;
...
let test_agent_draft_policy_script () =
  ...
  check
    int
    "live bypass label overrides stale event labels"
    0
    (run_agent_draft_policy
  ...
  check
    bool
    "ready agent PR without bypass fails"
    true
    (run_agent_draft_policy (("PR_IS_DRAFT", "false") :: base) <> 0);
  check
    int
    "ready agent PR with bypass label passes"
    0
    (run_agent_draft_policy
       (("PR_IS_DRAFT", "false")
  ...
  check
    int
    "ready agent PR with live state MERGED is skipped"
    0
    (run_agent_draft_policy
       (("PR_IS_DRAFT", "false") :: ("PR_LIVE_STATE", "MERGED") :: base));
  check
    int
    "ready agent PR with live state CLOSED is skipped"
    0
    (run_agent_draft_policy
       (("PR_IS_DRAFT", "false") :: ("PR_LIVE_STATE", "CLOSED") :: base));
  check
    int
    "live state lower-case merged is also skipped"
    0
    (run_agent_draft_policy
       (("PR_IS_DRAFT", "false") :: ("PR_LIVE_STATE", "merged") :: base));
  check
    bool
    "live state OPEN does NOT bypass policy"
    true
    (run_agent_draft_policy (("PR_IS_DRAFT", "false") :: ("PR_LIVE_STATE", "OPEN") :: base)
     <> 0)
;;
...
let test_health_and_ci_runner_diagnostics () =
  check
    bool
    "health snapshot records baseline source"
    true
    (file_contains_pattern "scripts/health_snapshot.sh" "\"baseline\": {");
  check
    bool
    "health snapshot records regressions array"
    true
    (file_contains_pattern
  ...
  check
    bool
    "ci runner tails dune log on failure"
    true
    (file_contains_pattern "scripts/ci-run-tests.sh" "tail -n 120 ${dune_log}");
  check
    bool
    "ci runner retries dune rpc lock failures in isolated build dir"
    true
    (file_contains_pattern
       "scripts/ci-run-tests.sh"
  ...
  check
    bool
    "ci runner detects native archive loss"
    true
    (file_contains_pattern "scripts/ci-run-tests.sh" "Unbound module Llm_provider");
  check
    bool
    "ci runner detects disk full failures"
    true
    (file_contains_pattern "scripts/ci-run-tests.sh" "No space left on device");
  check
    bool
    "ci runner prints disk hygiene repair guidance"
    true
    (file_contains_pattern "scripts/ci-run-tests.sh" "bash scripts/disk-hygiene.sh --fix");
  check
    bool
    "ci runner avoids recursive tmpdir du in diagnostics"
    true
    (file_not_contains_pattern "scripts/ci-run-tests.sh" "du -sh \"${TMPDIR:-/tmp}\"");
  check
    bool
    "ci runner avoids process substitution in clean retry"
    true
    (file_not_contains_pattern "scripts/ci-run-tests.sh" "> >(tee");
  check
    bool
    "ci runner tracks active build dir for diagnostics"
    true
    (file_contains_pattern "scripts/ci-run-tests.sh" "ACTIVE_TEST_BUILD_DIR");
  check
    bool
    "quick suite excludes operator control from monolithic dune test"
    true
    (file_contains_pattern
       ".github/workflows/ci.yml"
  ...
  check
    bool
    "ci workflow runs operator control in dedicated step"
    true
    (file_contains_pattern
       ".github/workflows/ci.yml"
  ...
  check
    bool
    "operator control test is env-gated in dune"
    true
    (file_contains_pattern
       "test/dune"
...
let test_release_truth_contracts () =
  ...
  check
    bool
    "release evidence changes stay in build scope"
    true
    (file_contains_pattern ".github/workflows/ci.yml" "scripts/release-evidence\\.sh$");
  check
    bool
    "health job reruns doc truth and OAS pin checks"
    true
    (file_contains_pattern
       ".github/workflows/ci.yml"
  ...
  check
    bool
    "doc truth script delegates version truth"
    true
    (file_contains_pattern "scripts/check-doc-truth.sh" "scripts/check-version-truth.sh");
  check
    bool
    "release workflow checks version truth"
    true
    (file_contains_pattern
  ...
  check
    bool
    "main build uploads release evidence"
    true
    (file_contains_pattern
       ".github/workflows/ci.yml"
  ...
  check
    bool
    "release workflow generates evidence bundle"
    true
    (file_contains_pattern
       ".github/workflows/release.yml"
  ...
  check
    bool
    "release workflow ships evidence with artifacts"
    true
    (file_contains_pattern ".github/workflows/release.yml" "path: dist/*");
  check
    bool
    "make install deps skips with-doc"
    true
    (file_contains_pattern "mk/build.mk" "opam install . --deps-only --with-test -y");
  check
    bool
    "make release evidence target exists"
    true
...
let test_oas_pin_source_contracts () =
  check
    bool
    "external opam pins retry transient git fetch failures"
    true
    (file_contains_pattern "scripts/opam-pin-external-deps.sh" "OPAM_PIN_RETRIES");
  check
    bool
    "external opam pins use retry wrapper"
    true
    (file_contains_pattern
  ...
  check
    bool
    "oas pin check parses local file pins from opam output"
    true
    (file_contains_pattern "scripts/check-oas-pin.sh" "extract_opam_pin_source()");
  check
    bool
    "oas pin check accepts rsync-style file pins"
    true
    (file_contains_pattern "scripts/check-oas-pin.sh" "git+*|file://*)");
  check
    bool
    "oas pin check normalizes local pin path helper"
    true
    (file_contains_pattern "scripts/check-oas-pin.sh" "local_pin_path_from_source()");
  check
    bool
    "oas pin check accepts both git file pins and plain file pins"
    true
    (file_contains_pattern "scripts/check-oas-pin.sh" "git+file://*|file://*)");
  check
    bool
    "oas pin check validates findlib artifact directories"
    true
    (file_contains_pattern "scripts/check-oas-pin.sh" "ocamlfind query");
  check
    bool
    "oas pin check validates agent sdk native archive"
    true
    (file_contains_pattern "scripts/check-oas-pin.sh" "agent_sdk.cmxa");
  check
    bool
    "oas pin check validates llm provider native archive"
    true
    (file_contains_pattern "scripts/check-oas-pin.sh" "llm_provider.cmxa");
  check
    bool
    "oas pin check gives rebuild guidance"
    true
    (file_contains_pattern
...
let test_doc_truth_guard_contracts () =
  check
    bool
    "doc truth script protects spec index front door wording"
    true
    (file_contains_pattern
       "scripts/check-doc-truth.sh"
       "Historical compatibility lane과 internal orchestration reference는 migration \
        context로만 남긴다.");
  check
    bool
    "doc truth script protects command plane downgrade"
    true
    (file_contains_pattern
       "scripts/check-doc-truth.sh"
  ...
  check
    bool
    "doc truth script protects system overview front door wording"
    true
    (file_contains_pattern
       "scripts/check-doc-truth.sh"
  ...
  check
    bool
    "doc truth script forbids dead server_command_plane_http row"
    true
    (file_contains_pattern
       "scripts/check-doc-truth.sh"
       "require_not_contains docs/spec/09-server-transport.md '| \
        `server_command_plane_http.ml` |'");
  check
    bool
    "doc truth script forbids old dashboard command-plane type wording"
    true
    (file_contains_pattern
       "scripts/check-doc-truth.sh"
...
let test_contract_harness_and_execution_session_authz_contracts () =
  check
    bool
    "contract harness exposes extract_text helper"
    true
    (file_contains_pattern "scripts/harness/lib/test_framework.sh" "extract_text()");
  check
    bool
    "golden path harness uses extract_text helper"
    true
    (file_contains_pattern
...
let test_route_auth_contracts () =
  ...
  check
    bool
    "http keeper chat stream uses keeper tool auth"
    true
    (file_contains_pattern
  ...
  check
    bool
    "dashboard runtime probe force refresh uses tool auth"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_dashboard.ml"
  ...
  check
    bool
    "http keeper chat stream forces direct reply mode"
    true
    (file_contains_pattern
  ...
  check
    bool
    "channel gate message route uses tool auth"
    true
    (file_contains_pattern
  ...
  check
    bool
    "channel gate message route is registered"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_channel_gate.ml"
  ...
  check
    bool
    "channel gate health route stays public read"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_channel_gate.ml"
       "with_public_read");
  check
    bool
    "channel gate events route stays public read"
    true
    (file_contains_pattern
  ...
  check
    bool
    "channel gate connectors route stays public read"
    true
    (file_contains_pattern
       "lib/server/server_auth.ml"
  ...
  check
    bool
    "generic connector status route stays public read"
    true
    (file_contains_pattern
       "lib/server/server_auth.ml"
  ...
  check
    bool
    "channel gate health route is registered"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_channel_gate.ml"
  ...
  check
    bool
    "generic connector status route is registered"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_channel_gate.ml"
  ...
  check
    bool
    "channel gate connectors route is registered"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_channel_gate.ml"
  ...
  check
    bool
    "generic connector bind route is registered"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_channel_gate.ml"
  ...
  check
    bool
    "generic connector unbind route is registered"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_channel_gate.ml"
...
let test_http_write_auth_contracts () =
  check
    bool
    "server auth scopes query token fallback to observer SSE helper"
    true
    (file_contains_pattern
       "lib/server/server_auth.ml"
  ...
  check
    bool
    "observer SSE helper still gates query token on sse_kind"
    true
    (file_contains_pattern
       "lib/server/server_auth.ml"
  ...
  check
    bool
    "observer SSE helper still reads token query param"
    true
    (file_contains_pattern
       "lib/server/server_auth.ml"
  ...
  check
    bool
    "observer SSE auth error documents scoped query token contract"
    true
    (file_contains_pattern
       "lib/server/server_auth.ml"
  ...
  check
    bool
    "server auth keeps general MCP auth header-only"
    true
    (file_contains_pattern
       "lib/server/server_auth.ml"
  ...
  check
    bool
    "observer SSE query token fallback stays scoped"
    true
    (file_contains_pattern
       "lib/server/server_auth.ml"
  ...
  check
    bool
    "observer SSE fallback reads token query param explicitly"
    true
    (file_contains_pattern "lib/server/server_auth.ml" {|query_param request "token"|});
  check
    bool
    "observer SSE auth has dedicated verifier"
    true
    (file_contains_pattern
       "lib/server/server_auth.ml"
  ...
  check
    bool
    "server auth defines token-bound permission helper"
    true
    (file_contains_pattern
       "lib/server/server_auth.ml"
  ...
  check
    bool
    "server auth exposes token-bound route helper"
    true
    (file_contains_pattern "lib/server/server_auth.ml" "and with_token_permission_auth");
  check
    bool
    "server auth defines same-origin browser guard"
    true
    (file_contains_pattern
       "lib/server/server_auth.ml"
  ...
  check
    bool
    "tool auth enforces same-origin when no bearer token"
    true
    (file_contains_pattern
       "lib/server/server_auth.ml"
  ...
  check
    bool
    "broadcast route requires token-bound broadcast permission"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_dashboard.ml"
  ...
  check
    bool
    "keeper config update requires admin permission"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_dashboard.ml"
  ...
  check
    bool
    "board vote route requires board vote tool auth"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_activity.ml"
       {|with_tool_auth ~tool_name:"masc_board_vote"|});
  check
    bool
    "board vote route overwrites voter from auth identity"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_activity.ml"
  ...
  check
    bool
    "board vote route writes normalized voter"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_activity.ml"
  ...
  check
    bool
    "board comment route overwrites author from auth identity"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_activity.ml"
  ...
  check
    bool
    "board comment route writes normalized author"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_activity.ml"
  ...
  check
    bool
    "tool-host-failures route requires tool auth"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_dashboard.ml"
  ...
  check
    bool
    "provider runs post requires admin permission"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_provider_runs.ml"
       {|with_token_permission_auth ~permission:Types.CanAdmin|});
  check
    bool
    "dashboard delete actions require token-bound admin permission"
    true
    (file_contains_pattern
  ...
  check
    bool
    "server auth defines public-read cors origin helper"
    true
    (file_contains_pattern "lib/server/server_auth.ml" "let public_read_cors_origin_opt");
  check
    bool
    "server auth exposes public-read json responder"
    true
    (file_contains_pattern "lib/server/server_auth.ml" "let respond_public_read_json");
  check
    bool
    "channel gate public reads use constrained cors responder"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_channel_gate.ml"
  ...
  check
    bool
    "artifacts endpoint uses constrained cors responder"
    true
    (file_contains_pattern
  ...
  check
    bool
    "provider runs route threads state net into dashboard single-run"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_provider_runs.ml"
  ...
  check
    bool
    "loopback cross-port auth uses explicit dev origin allowlist"
    true
    (file_contains_pattern
       "lib/server/server_auth.ml"
  ...
  check
    bool
    "loopback cross-port auth no longer trusts any loopback origin"
    true
    (not
       (file_contains_pattern
          "lib/server/server_auth.ml"
...
let test_tool_admin_snapshot_auth_contracts () =
  check
    bool
    "tool admin snapshot metadata requires admin permission"
    true
    (file_contains_pattern
       "lib/tool_misc.ml"
  ...
  check
    bool
    "tool admin snapshot legacy permission map requires admin"
    true
    (file_contains_pattern
       "lib/tool_permission_map.ml"
...
let test_keeper_direct_reply_contracts () =
  check
    bool
    "dashboard keeper direct messages request direct reply"
    true
    (file_contains_pattern "dashboard/src/api/keeper.ts" "direct_reply: true");
  check
    bool
    "operator keeper_message forwards direct reply flag"
    true
    (file_contains_pattern
  ...
  check
    bool
    "channel gate keeper bridge uses streaming reply path"
    true
    (file_contains_pattern "lib/gate_keeper_backend.ml" "Tool_keeper.dispatch_stream");
  check
    bool
    "keeper turn parses direct reply flag"
    true
    (file_contains_pattern "lib/keeper/keeper_turn.ml" "get_bool args \"direct_reply\"");
  ...
  check
    bool
    "keeper manual turns resolve declared cascade through runtime catalog"
    true
    (file_contains_pattern
  ...
  check
    bool
    "keeper turn suppresses skill route headers for direct reply"
    true
    (file_contains_pattern
       "lib/keeper/keeper_turn.ml"
  ...
  check
    bool
    "keeper turn applies direct reply persona prompt"
    true
    (file_contains_pattern
       "lib/keeper/keeper_turn.ml"
...
let test_dashboard_warm_hydration_contracts () =
  check
    bool
    "execution default route hydrates cache on first success"
    true
    (file_contains_pattern
  ...
  check
    bool
    "mission default route serves cached surface immediately"
    true
    (file_contains_pattern
  ...
  check
    bool
    "namespace truth advertises initializing while execution warms"
    true
    (file_contains_pattern
  ...
  check
    bool
    "execution render timeout is a named constant"
    true
    (file_contains_pattern "lib/dashboard/dashboard_execution.ml" "let render_timeout_s");
  check
    bool
    "execution proactive refresh timeout is extended"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_execution_surfaces.ml"
  ...
  check
    bool
    "mission proactive refresh timeout is extended"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_core.ml"
...
let test_http_read_surface_contracts () =
  check
    bool
    "room status route now requires read auth"
    true
    (file_contains_pattern
  ...
  check
    bool
    "room tasks route now requires read auth"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_room.ml"
  ...
  check
    bool
    "room agents route now requires read auth"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_room.ml"
  ...
  check
    bool
    "room messages route now requires read auth"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_room.ml"
  ...
  check
    bool
    "room route delegates status reads to protocol boundary"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_room.ml"
  ...
  check
    bool
    "room route delegates task reads to protocol boundary"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_room.ml"
  ...
  check
    bool
    "room route delegates agent reads to protocol boundary"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_room.ml"
  ...
  check
    bool
    "room route delegates message reads to protocol boundary"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_room.ml"
  ...
  check
    bool
    "room route does not read Coord directly"
    true
    (file_not_contains_pattern "lib/server/server_routes_http_routes_room.ml" "Coord.");
  check
    bool
    "room route does not read Tempo directly"
    true
    (file_not_contains_pattern "lib/server/server_routes_http_routes_room.ml" "Tempo.");
  check
    bool
    "room protocol owns the Coord read boundary"
    true
    (file_contains_pattern "lib/room_protocol.ml" "Coord.get_tasks_raw config");
  check
    bool
    "provider run status route now requires read auth"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_provider_runs.ml"
...
let test_input_validation_contracts () =
  ...
  check
    bool
    "broadcast validates empty message"
    true
    (file_contains_pattern
       "lib/tool_inline_dispatch_comm.ml"
       {|"Broadcast message cannot be empty"|});
  check
    bool
    "broadcast trims whitespace before check"
    true
    (file_contains_pattern "lib/tool_inline_dispatch_comm.ml" {|String.trim message|});
  (* Bug #1609: cache must have automatic eviction *)
  check
    bool
    "cache has maybe_evict_expired function"
    true
    (file_contains_pattern "lib/cache_eio.ml" "let maybe_evict_expired config");
  check
    bool
    "cache get triggers batch eviction"
    true
    (file_contains_pattern "lib/cache_eio.ml" "maybe_evict_expired config")
;;
...
let test_room_current_validation_contracts () =
  ...
  check
    bool
    "h2 gateway serves project-snapshot endpoint"
    true
    (file_contains_pattern
  ...
  check
    bool
    "h2 gateway serves namespace-truth endpoint alias"
    true
    (file_contains_pattern
       "lib/server/server_h2_gateway.ml"
  ...
  check
    bool
    "h2 gateway keeps room-truth alias endpoint during rollout"
    true
    (file_contains_pattern
       "lib/server/server_h2_gateway.ml"
  ...
  check
    bool
    "h2 gateway serves namespace current endpoint"
    true
    (file_contains_pattern
       "lib/server/server_h2_gateway.ml"
  ...
  check
    bool
    "h2 gateway maps invalid namespace writes to 400"
    true
    (file_contains_pattern "lib/server/server_h2_gateway.ml" {|Invalid_argument msg|});
  check
    bool
    "h2 gateway keeps room current alias endpoint during rollout"
    true
    (file_contains_pattern "lib/server/server_h2_gateway.ml" {|"/api/v1/room/current"|})
;;

let test_root_redirect_contracts () =
  check
    bool
    "http root redirects to dashboard"
    true
    (file_contains_pattern
  ...
  check
    bool
    "http root keeps dashboard fallback redirect"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_frontend.ml"
  ...
  check
    bool
    "http redirect sets dashboard location"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_frontend.ml"
  ...
  check
    bool
    "h2 root responds with server identity"
    true
    (file_contains_pattern
       "lib/server/server_h2_gateway.ml"
...
let test_dashboard_component_split_contracts () =
  check
    bool
    "harness health sections export verdict tone"
    true
    (file_contains_pattern
  ...
  check
    bool
    "harness health tests cover verdict tone"
    true
    (file_contains_pattern
       "dashboard/src/components/harness-health-sections.test.ts"
       "describe('verdictTone'");
  check
    bool
    "retired proof helpers module stays deleted"
    true
    (file_not_contains_pattern
       "dashboard/src/components/proof-helpers.ts"
       "export function");
  check
    bool
    "coord backend setup no longer references transaction companion after PG removal"
    true
    (file_not_contains_pattern
...
let test_mission_briefing_memory_guard_contracts () =
  check
    bool
    "mission briefing snapshot disables keeper payload"
    true
    (file_contains_pattern
  ...
  check
    bool
    "mission briefing snapshot no longer references command plane"
    true
    (file_not_contains_pattern
       "lib/dashboard/dashboard_mission_briefing.ml"
  ...
  check
    bool
    "mission briefing snapshot stays off command plane"
    true
    (file_contains_pattern
       "lib/dashboard/dashboard_mission_briefing.ml"
  ...
  check
    bool
    "mission briefing snapshot stays lightweight"
    true
    (file_contains_pattern
       "lib/dashboard/dashboard_mission_briefing.ml"
  ...
  check
    bool
    "mission briefing reuses mission keeper briefs"
    true
    (file_contains_pattern
       "lib/dashboard/dashboard_mission_briefing.ml"
  ...
  check
    bool
    "mission briefing card no longer forces eager operator snapshot"
    true
    (file_not_contains_pattern
       "dashboard/src/components/mission-briefing-card.ts"
...
let test_activity_surface_contracts () =
  check
    bool
    "observatory absorbs activity-derived panels"
    true
    (file_contains_pattern
       "dashboard/src/components/observatory/observatory.ts"
       "ObservatoryActivityPanels");
  check
    bool
    "dashboard fetches canonical activity graph route"
    true
    (file_contains_pattern "dashboard/src/api/actions.ts" "/api/v1/activity/graph");
  check
    bool
    "server exposes canonical activity events route"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_activity.ml"
  ...
  check
    bool
    "server exposes canonical activity graph route"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_activity.ml"
  ...
  check
    bool
    "activity routes thread sw/clock instead of reading Eio_context directly"
    true
    (not
       (file_contains_pattern
          "lib/server/server_routes_http_routes_activity.ml"
  ...
  check
    bool
    "server drops legacy social graph alias"
    true
    (not
       (file_contains_pattern
          "lib/server/server_routes_http_routes_activity.ml"
  ...
  check
    bool
    "coord top-level module emits activity events"
    true
    (file_contains_pattern "lib/coord.ml" "Activity_graph.emit config");
  check
    bool
    "coord task lifecycle emits activity events via hook"
    true
    (file_contains_pattern
  ...
  check
    bool
    "coord broadcast emits activity events via hook"
    true
    (file_contains_pattern
  ...
  check
    bool
    "board success paths emit activity events"
    true
    (file_contains_pattern
...
let test_local_review_script_contracts () =
  check
    bool
    "local review script exists"
    true
    (file_contains_pattern "scripts/review/local-review.sh" "#!/usr/bin/env bash");
  check
    bool
    "local review script caches under .masc review-cache"
    true
    (file_contains_pattern
       "scripts/review/local-review.sh"
       ".masc/review-cache/local-review");
  check
    bool
    "local review script resolves shared git common dir cache root"
    true
    (file_contains_pattern "scripts/review/local-review.sh" "--git-common-dir");
  check
    bool
    "local review script keeps pending registry"
    true
    (file_contains_pattern "scripts/review/local-review.sh" ".pending.json");
  check
    bool
    "local review script chunks large diffs"
    true
    (file_contains_pattern
       "scripts/review/local-review.sh"
  ...
  check
    bool
    "local review script bounds reviewer request time"
    true
    (file_contains_pattern "scripts/review/local-review.sh" "--max-time");
  check
    bool
    "local review script exposes cache key print"
    true
    (file_contains_pattern "scripts/review/local-review.sh" "--print-cache-key")
;;

let test_keeper_oas_cleanup_contracts () =
  check
    bool
    "keeper config no longer exposes stale unified turn flag"
    true
    (not (file_contains_pattern "lib/keeper/keeper_config.ml" "MASC_KEEPER_UNIFIED_TURN"));
  check
    bool
    "keeper turn comment no longer mentions context manager"
    true
    (not (file_contains_pattern "lib/keeper/keeper_turn.ml" "Context_manager"));
  check
    bool
    "tool compact comment now references OAS-backed pipeline"
    true
    (file_contains_pattern "lib/tool_compact.ml" "OAS-backed compaction pipeline")
;;

let test_dashboard_executor_pool_contracts () =
  check
    bool
    "dashboard runtime support defines executor pool helper"
    true
    (file_contains_pattern
  ...
  check
    bool
    "dashboard runtime support submits compute to executor pool"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_runtime_support.ml"
  ...
  check
    bool
    "mission refresh loop uses dashboard compute helper"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_core.ml"
  ...
  check
    bool
    "mission actor path uses dashboard compute helper"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_core.ml"
  ...
  check
    bool
    "execution refresh loop uses dashboard compute helper"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_execution_surfaces.ml"
  ...
  check
    bool
    "server state captures mono_clock for threaded readonly compute"
    true
    (file_contains_pattern
       "lib/mcp_server.ml"
  ...
  check
    bool
    "dashboard core threads state runtime caps into readonly compute"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_core.ml"
  ...
  check
    bool
    "dashboard core no longer reads global eio net directly"
    true
    (file_not_contains_pattern
       "lib/server/server_dashboard_http_core.ml"
       "Eio_context.get_net ()");
  check
    bool
    "dashboard core no longer reads global mono_clock directly"
    true
    (file_not_contains_pattern
       "lib/server/server_dashboard_http_core.ml"
  ...
  check
    bool
    "execution parameterized path uses dashboard compute helper"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_execution_surfaces.ml"
       "run_dashboard_compute ~mode:Offloaded_readonly ?net ?mono_clock ~sw");
  check
    bool
    "server bootstrap wires executor pool into dashboard"
    true
    (file_contains_pattern
...
let test_transport_route_contracts () =
  ...
  check
    bool
    "frontend exposes ws discovery route"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_frontend.ml"
  ...
  check
    bool
    "mcp http agent injection preserves explicit legacy agent_name"
    true
    (file_contains_pattern
       "lib/server/server_mcp_transport_http.ml"
  ...
  check
    bool
    "common http deps prefer runtime captured in server_state"
    true
    (file_contains_pattern
  ...
  check
    bool
    "frontend exposes webrtc offer route"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_frontend.ml"
  ...
  check
    bool
    "frontend exposes webrtc answer route"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_frontend.ml"
  ...
  check
    bool
    "frontend webrtc routes require tool auth"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_frontend.ml"
  ...
  check
    bool
    "h2 gateway exposes webrtc offer route"
    true
    (file_contains_pattern "lib/server/server_h2_gateway.ml" {|`POST, "/webrtc/offer"|});
  check
    bool
    "h2 gateway exposes webrtc answer route"
    true
    (file_contains_pattern "lib/server/server_h2_gateway.ml" {|`POST, "/webrtc/answer"|});
  check
    bool
    "transport delete path verifies full mcp auth"
    true
    transport_delete_path_verifies_full_mcp_auth;
  check
    bool
    "h2 delete path verifies full mcp auth"
    true
    h2_delete_path_verifies_full_mcp_auth;
  check
    bool
    "h2 gateway webrtc routes enforce tool auth"
    true
    (file_contains_pattern "lib/server/server_h2_gateway.ml" {|authorize_tool_request|}
     && file_contains_pattern
          "lib/server/server_h2_gateway.ml"
  ...
  check
    bool
    "h2 gateway respects webrtc disabled state"
    true
    (file_contains_pattern
       "lib/server/server_h2_gateway.ml"
...
let test_transport_health_contracts () =
  check
    bool
    "standalone ws updates transport metrics on connect"
    true
    (file_contains_pattern
  ...
  check
    bool
    "transport metrics ws env parse matches runtime server"
    true
    (file_contains_pattern
  ...
  check
    bool
    "standalone ws reuses transport metrics env parser"
    true
    (file_contains_pattern
       "lib/server/server_ws_standalone.ml"
  ...
  check
    bool
    "transport health avoids room message scans"
    true
    (not
       (file_contains_pattern
...
let test_worktree_list_contracts () =
  ...
  check
    bool
    "worker oas no longer reads global net directly"
    true
    (file_not_contains_pattern "lib/worker_oas.ml" "Eio_context.get_net_opt ()");
  (* research dispatch assertions removed — lib/research/ subsystem deleted (#4715) *)
  check
    bool
    "worktree create/remove still require join"
    true
    (file_contains_pattern
       "lib/tool_worktree.ml"
  ...
  check
    bool
    "worktree list excluded from join-required list"
    true
    (file_not_contains_pattern "lib/tool_worktree.ml" {|_tool_spec_requires_join = [|}
     || file_not_contains_pattern
          "lib/tool_worktree.ml"
...
let test_oas_worker_capability_threading_contracts () =
  check
    bool
    "oas worker model-by-label accepts threaded sw capability"
    true
    (file_contains_pattern "lib/oas_worker.mli" "?sw:Eio.Switch.t ->");
  check
    bool
    "oas worker model-by-label accepts threaded net capability"
    true
    (file_contains_pattern
       "lib/oas_worker.mli"
...
let test_oas_capacity_restore_contracts () =
  check
    bool
    "operator judge backoff uses OAS local capacity"
    true
    (file_contains_pattern
  ...
  check
    bool
    "operator judge selection is explicit"
    true
    (file_contains_pattern
       "lib/dashboard/dashboard_operator_judge.ml"
       {|[ "operator_judge" ]|});
  check
    bool
    "governance judge backoff uses OAS local capacity"
    true
    (file_contains_pattern
  ...
  check
    bool
    "governance judge selection is explicit"
    true
    (file_contains_pattern
       "lib/dashboard/dashboard_governance_judge.ml"
       {|[ "governance_judge" ]|});
  check
    bool
    "autoresearch background gating restores OAS capacity query"
    true
    (file_contains_pattern
  ...
  check
    bool
    "autoresearch uses Eio context fallback for capacity probing"
    true
    (file_contains_pattern
       "lib/autoresearch_codegen.ml"
...
let test_dashboard_timeout_guard_contracts () =
  check
    bool
    "http transport health route uses cached dashboard helper"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_dashboard.ml"
  ...
  check
    bool
    "dashboard shell helper accepts threaded clock capability"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_core.ml"
  ...
  check
    bool
    "http dashboard shell route threads state clock"
    true
    (file_contains_pattern
       "lib/server/server_routes_http_routes_dashboard.ml"
  ...
  check
    bool
    "h2 dashboard shell route threads state clock"
    true
    (file_contains_pattern
       "lib/server/server_h2_gateway.ml"
       "dashboard_shell_http_json ?clock:state.Mcp_server.clock");
  check
    bool
    "h2 transport health route uses cached dashboard helper"
    true
    (file_contains_pattern
       "lib/server/server_h2_gateway.ml"
  ...
  check
    bool
    "server dashboard transport health helper uses cached surface"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_execution_surfaces.ml"
  ...
  check
    bool
    "shell timeout detector recognizes computation_timeout"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_core.ml"
  ...
  check
    bool
    "shell meta cognition seeds stale fallback before refresh"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_core.ml"
  ...
  check
    bool
    "namespace-truth pending confirm seeds stale fallback"
    true
    (file_contains_pattern
  ...
  check
    bool
    "mission refresh dedupes inflight fetches"
    true
    (file_contains_pattern
       "dashboard/src/mission-actions.ts"
       "let inflightMissionSnapshotRefresh: Promise<void> | null = null");
  check
    bool
    "transport health panel dedupes inflight fetches"
    true
    (file_contains_pattern
...
let test_namespace_truth_adaptive_timeout_contracts () =
  check
    bool
    "shell fiber uses adaptive timeout"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_namespace_truth.ml"
       "shell_timeout_s");
  check
    bool
    "namespace-truth warm timeout is a named constant"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_namespace_truth.ml"
       "let warm_timeout_s");
  check
    bool
    "namespace-truth cold timeout is a named constant"
    true
    (file_contains_pattern
       "lib/server/server_dashboard_http_namespace_truth.ml"
       "let cold_timeout_s");
...
let test_http_client_fd_safety_contracts () =
  check
    bool
    "masc http client forbids direct Cohttp client construction in docs"
    true
    (file_contains_pattern
  ...
  check
    bool
    "voice bridge builds clients through masc http client"
    true
    (file_contains_pattern
       "lib/voice/voice_bridge_core.ml"
       "Masc_http_client.make_closing_client");
  check
    bool
    "otel exporter builds clients through masc http client"
    true
    (file_contains_pattern
       "lib/opentelemetry_client_cohttp_eio.ml"
       "Masc_http_client.make_closing_client");
  ()
;;
...
let test_runtime_precondition_contracts () =
  ...
  check
    bool
    "graphql routes expose result-based server state lookup"
    true
    (file_contains_pattern
...
let test_masc_dirname_ssot_contracts () =
  ...
  in
  let file_uses_masc_dir_helper file =
    List.exists (fun p -> file_contains_pattern file p) masc_dir_helper_patterns
  in
  List.iter
    (fun file ->
       check
  ...
  run
    ...
    [ ( "source_guard"
      , [ test_case "sync and asset contracts" `Quick test_ci_sync_and_asset_contracts
        ; test_case "agent draft policy script" `Quick test_agent_draft_policy_script
        ; test_case
            "contract harness and team session authz contracts"
            `Quick
            test_contract_harness_and_execution_session_authz_contracts
        ; test_case
            "health and ci diagnostics"
            `Quick
            test_health_and_ci_runner_diagnostics
        ...
        ; test_case
            "tool admin snapshot auth contracts"
            `Quick
            test_tool_admin_snapshot_auth_contracts
        ; test_case
            "keeper direct reply contracts"
            `Quick
            test_keeper_direct_reply_contracts
        ; test_case
            "dashboard warm hydration contracts"
            `Quick
            test_dashboard_warm_hydration_contracts
        ; test_case "http read surface contracts" `Quick test_http_read_surface_contracts
        ; test_case
            "operator surface route contracts"
            `Quick
            test_operator_surface_route_contracts
        ; test_case "input validation contracts" `Quick test_input_validation_contracts
        ; test_case
            "room current validation contracts"
            `Quick
            test_room_current_validation_contracts
        ; test_case "root redirect contracts" `Quick test_root_redirect_contracts
        ; test_case
            "dashboard component split contracts"
            `Quick
            test_dashboard_component_split_contracts
        ; test_case
            "mission briefing memory guard contracts"
            `Quick
            test_mission_briefing_memory_guard_contracts
        ; test_case "activity surface contracts" `Quick test_activity_surface_contracts
        ; test_case
            "local review script contracts"
            `Quick
            test_local_review_script_contracts
        ; test_case
            "keeper oas cleanup contracts"
            `Quick
            test_keeper_oas_cleanup_contracts
        ; test_case
            "dashboard executor pool contracts"
            `Quick
            test_dashboard_executor_pool_contracts
        ; test_case "transport route contracts" `Quick test_transport_route_contracts
        ; test_case "transport health contracts" `Quick test_transport_health_contracts
        ; test_case "worktree list contracts" `Quick test_worktree_list_contracts
        ; test_case
            "oas worker capability threading contracts"
            `Quick
            test_oas_worker_capability_threading_contracts
        ; test_case
            "oas capacity restore contracts"
            `Quick
            test_oas_capacity_restore_contracts
        ; test_case
            "team session spawn tool contracts"
            `Quick
            test_execution_session_spawn_tool_contracts
        ; test_case
            "dashboard timeout guard contracts"
            `Quick
            test_dashboard_timeout_guard_contracts
        ; test_case "mermaid xss contracts" `Quick test_mermaid_xss_contracts
        ; test_case
            "http client fd safety contracts"
            `Quick
            test_http_client_fd_safety_contracts
        ; test_case
            "namespace-truth adaptive timeout contracts"
            `Quick
            test_namespace_truth_adaptive_timeout_contracts
        ; test_case
            "runtime precondition contracts"
            `Quick
            test_runtime_precondition_contracts
        ; test_case "router contract alignment" `Quick test_router_contract_alignment
        ; test_case
            "masc_dirname SSOT contracts (#9571 batch 1)"
            `Quick
