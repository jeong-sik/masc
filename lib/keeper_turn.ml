       ...
       | Ok specs ->
         (match ensure_api_keys specs with
          ...
          | Ok () ->
            ...
            in
	            let user_msg = Llm_types.user_msg message in
            ...
            let requests =
	              List.map (fun (model : Llm_types.model_spec) ->
	                let msgs =
	                  (Llm_types.system_msg turn_system_prompt) :: ctx_work.messages
	                in
	                ({
                  Llm_types.model;
                  messages = msgs;
            ...
            | Ok resp0 ->
                  ...
                  let followup_requests =
                    List.map (fun (model : Llm_types.model_spec) ->
                      ({
                        Llm_types.model;
                        messages = [
                          Llm_types.system_msg (keeper_tool_loop_system_prompt
                    ...
                    List.map (fun (model : Llm_types.model_spec) ->
	                      ({
	                        ...
	                        messages = [
	                          ...
	                          Llm_types.user_msg correction_prompt;
	                        ];
                        temperature = 0.2;
                  ...
                  let forced_requests =
                    List.map (fun (model : Llm_types.model_spec) ->
	                      ({
	                        ...
	                        messages = [
	                          Llm_types.system_msg turn_system_prompt;
	                          Llm_types.user_msg forced_prompt;
	                        ];
                        temperature = 0.0;
                        max_tokens = correction_max_tokens;
                        tools = [];
              ...
              let response_alignment = jaccard_similarity message safe_reply in
		              ...
		              let assistant_msg = Llm_types.assistant_msg safe_reply in
