...
let test_max_path_length () = check bool "positive" true (Dashboard.max_path_length () > 0)
...
let test_section_type () =
  let s : Dashboard.section =
    { title = "Test Section"; content = [ "line1"; "line2" ]; empty_msg = "No items" }
  in
...
let test_section_empty () =
  let s : Dashboard.section =
    { title = "Empty"; content = []; empty_msg = "Nothing here" }
  in
...
let test_format_section_with_content () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp_string "Agents") result 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp_string "claude") result 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_format_section_empty_shows_msg () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp_string "no tasks") result 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_format_section_has_border () =
  let s : Dashboard.section = { title = "Test"; content = [ "line" ]; empty_msg = "" } in
  let result = Dashboard.format_section s in
  check
    bool
    "has equals"
    true
    (try
       let _ = Str.search_forward (Str.regexp_string "==") result 0 in
       true
     with
     | Not_found -> false)
;;
  ...
  let json =
    `Assoc
      [ ( "worktrees"
        , `List
            [ `Assoc
                [ "path", `String "/path/to/wt"
                ; "branch", `String "refs/heads/feature-x"
                ]
            ] )
      ]
  in
    ...
    ; ( "parse_worktrees"
      , [ test_case "empty list" `Quick test_parse_worktrees_empty_list
        ...
        ; test_case
            ...
            `Quick
            test_parse_worktrees_strips_refs_heads_prefix
        ...
        ; test_case
            ...
            `Quick
            test_parse_worktrees_unexpected_structure
        ] )
    ]
;;
