...
let test_fiber_health_unknown () =
  ...
  check bool "unknown for unregistered" true (health = KT.Fiber_unknown)
;;
...
let test_backoff_monotonic_until_cap () =
  ...
  let cap = Sup.backoff_delay 20 in
  (* at attempt 20, always at cap *)
  let rec check_mono i prev =
    if i > 20
    then ()
    else (
      let curr = Sup.backoff_delay i in
      check bool (Printf.sprintf "attempt %d >= prev" i) true (curr >= prev);
      check bool (Printf.sprintf "attempt %d <= cap" i) true (curr <= cap);
      check_mono (i + 1) curr)
  in
  check_mono 0 0.0
;;
...
let test_self_preservation_subset () =
  ...
  in
  let result =
    Sup.apply_self_preservation ~keepers_dir:"/tmp/test-keepers" ~total_keepers:10 entries
  in
...
let test_self_preservation_empty_input () =
  let result =
    Sup.apply_self_preservation ~keepers_dir:"/tmp/test-keepers" ~total_keepers:5 []
  in
...
let test_fiber_health_respects_max_restarts_override () =
  ...
  check bool "zombie at 3/5 restarts (restartable)" true (health_before = KT.Fiber_zombie);
  (* Override max_restarts to 2 — now restart_count 3 >= 2 = dead *)
  (match
     Masc_mcp.Runtime_params.set
  ...
  check bool "dead at 3/2 restarts (overridden)" true (health_after = KT.Fiber_dead);
  (* Restore default *)
...
let test_sweep_restores_reconcile_gate_for_paused_keeper () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         bool
         "paused keeper has pending approval"
         true
         (AQ.has_pending_for_keeper ~keeper_name:meta.name);
       check int "approval count incremented" (pending_before + 1) (AQ.pending_count ());
       let approval_id =
         ...
         | `List entries ->
           ...
           |> List.find_map (function
             | `Assoc fields ->
               ...
               if
                 Yojson.Safe.Util.(row |> member "keeper_name" |> to_string_option)
                 = Some meta.name
       ...
       check
         bool
         "keeper registered after approval"
         true
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Reg.restore_supervisor_state
         ...
         ~crash_log:[];
       let baseline =
         Masc_mcp.Prometheus.metric_total Masc_mcp.Prometheus.metric_keeper_dead_total
       in
       let ctx : _ KT.context =
         { config
         ; agent_name = "supervisor"
       ...
       check
         (float 0.001)
         "metric_keeper_dead_total incremented by 1"
         (baseline +. 1.0)
         after;
       ...
       check
         bool
         "keeper phase advanced to Dead"
         true
         (phase = Masc_mcp.Keeper_state_machine.Dead))
;;
  ...
  run
    ...
    ; ( "fiber_health"
      , [ test_case "unknown for unregistered" `Quick test_fiber_health_unknown
        ...
        ; test_case
            "should cleanup dead when ttl exceeded"
            `Quick
            test_should_cleanup_dead_true
        ; test_case
            "should not cleanup dead when recent"
            `Quick
            test_should_cleanup_dead_false_when_recent
        ] )
    ; ( "backoff_properties"
    ...
    ; ( "runtime_override"
      , [ test_case
            "fiber_health_of respects max_restarts override"
            `Quick
            test_fiber_health_respects_max_restarts_override
        ] )
    ; ( "reconcile_gate_recovery"
      , [ test_case
            "sweep restores reconcile gate for paused keeper"
            `Quick
            test_sweep_restores_reconcile_gate_for_paused_keeper
        ] )
    ; ( "dead_state_alert"
      , [ test_case
            "max_restarts exhaustion emits Dead alert"
            `Quick
