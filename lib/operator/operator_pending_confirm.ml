  ...
  let remaining =
    List.filter
      (fun (entry : pending_confirm) ->
         not (String.equal entry.target_type target_type && entry.target_id = target_id))
      all
  in
...
let pending_confirm_scope_of_entries ?actor entries =
  let actor_filter = normalize_pending_confirm_actor_filter actor in
  let all_entries =
    entries
  ...
  let visible_entries =
    ...
    | Some value ->
      List.filter
        (fun (entry : pending_confirm) -> String.equal value entry.actor)
        all_entries
  in
  let hidden_entries =
    match actor_filter with
    ...
    | Some value ->
      List.filter
        (fun (entry : pending_confirm) -> not (String.equal value entry.actor))
        all_entries
  in
...
let available_actions : available_action list =
  ...
  ; make_available_action
      ...
      ~target_type:"keeper"
      ~description:
        "Prepare a keeper-bound GitHub CLI login command under operator confirmation."
  ; make_available_action
...
let available_actions_json = `List (List.map available_action_to_yojson available_actions)
