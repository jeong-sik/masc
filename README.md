...
| Autoresearch Swarm | `masc_autoresearch_swarm_start` | raw autoresearch loop를 Team Session + CPv2 surface로 감싸서 operator-visible 연구 세션으로 시작 |
| Gardener | `masc_gardener_health`, `masc_gardener_propose_spawn` | 에이전트 생태계 자율 관리 (homeostatic balance) |
...
기본 `tools/list`는 canonical tool만 우선 노출하며, `masc_claim`, `experiment_*`, `masc_trpg_*`, low-usage social/vote utility 같은 convenience surface는 숨겨집니다. 직접 호출 호환성은 유지됩니다. dotted canonical tool(`decision.*`, `experiment.*`, `trpg.*`)이 public surface이고, underscore 형태는 주로 compatibility alias입니다.
`tools/list`는 MCP 표준 shape만 지원하며 `cursor` pagination을 사용합니다. hidden/deprecated inventory, auth/mode gates, keeper wrapper coverage 같은 MASC 전용 진단은 `masc_tool_admin_snapshot`, `masc_tool_help`, `masc_keeper_tool_catalog`에서 읽습니다.
