...
let create ?(config = config_from_env ()) () =
  { phase = Running; started_at = 0.0; reason = ""; entered = Atomic.make false; config }
;;
...
let register ~name ?(priority = 50) action = hooks := { name; priority; action } :: !hooks
let sorted_hooks () = List.sort (fun a b -> compare a.priority b.priority) !hooks
...
let phase_drain state ~clock ~drain_check =
  ...
  Log.Server.info
    "[Shutdown] Phase 2/4: DRAIN (timeout=%.1fs)"
    state.config.drain_timeout_s;
  let deadline = Eio.Time.now clock +. state.config.drain_timeout_s in
  let rec wait () =
    if drain_check ()
    then Log.Server.info "[Shutdown] drain complete (all work finished)"
    else if Eio.Time.now clock >= deadline
    then Log.Server.warn "[Shutdown] drain timeout reached, proceeding"
    else (
...
let phase_cleanup state ~clock =
  ...
  Log.Server.info
    "[Shutdown] Phase 3/4: CLEANUP (%d hooks, timeout=%.1fs)"
  ...
  | Eio.Time.Timeout ->
    Log.Server.warn
      "[Shutdown] cleanup timeout (%.1fs) exceeded, proceeding"
      state.config.cleanup_timeout_s
;;
...
let initiate state ~clock ~reason ~notify_fn ~drain_check ~exit_fn =
  ...
  if not (Atomic.compare_and_set state.entered false true)
  then
    Log.Server.warn
      "[Shutdown] already in progress (phase=%s), ignoring"
    ...
    ignore
      (Thread.create
         (fun () ->
            ...
            if not (Atomic.get done_flag)
            then (
              Log.Server.error
                "Shutdown exceeded %.0fs force timeout, forcing exit"
...
let elapsed state =
  if state.started_at > 0.0 then Unix.gettimeofday () -. state.started_at else 0.0
;;
