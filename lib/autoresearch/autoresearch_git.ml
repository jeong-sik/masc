...
let run_capture_lines ~workdir ?(timeout_sec = 30.0) argv =
  ...
  let lines =
    if String.length raw_output = 0
    then []
    else String.split_on_char '\n' raw_output |> List.filter (fun s -> s <> "")
  in
  status, lines
;;
...
let git_commit ~workdir ~message : (string option, string) Stdlib.result =
  if not (is_in_git_repo workdir)
  then Result.error "not inside a git repository"
  else (
    ...
    | Unix.WEXITED 0 ->
      let check_status, _check_output =
        run_git_with_status ~timeout_sec:30.0 ~workdir [ "diff"; "--cached"; "--quiet" ]
      in
          ...
          | Unix.WEXITED 0 ->
            ...
            (match hash_status with
             | Unix.WEXITED 0 ->
               ...
               if trimmed = ""
               then Result.error "git commit succeeded but no hash returned"
               else Result.ok (Some trimmed)
             | _ -> Result.error "git commit succeeded but rev-parse failed")
          | _ -> Result.error (Printf.sprintf "git commit failed: %s" raw_output))
       | Unix.WEXITED code ->
         Result.error (Printf.sprintf "git diff --cached --quiet exited %d" code)
       | _ -> Result.error "git diff --cached --quiet terminated abnormally")
    | _ -> Result.error (Printf.sprintf "git add failed: %s" add_output))
;;
  ...
  then ()
    ...
    | exn ->
      Log.Autoresearch.warn
        "git restore HEAD failed in %s: %s"
        workdir
...
let git_reset_last ~workdir =
  ...
  then ()
    ...
    try
      let status, _output =
        run_git_with_status ~timeout_sec:30.0 ~workdir [ "reset"; "--soft"; "HEAD~1" ]
      in
      match status with
      | Unix.WEXITED 0 -> ()
    ...
    | exn ->
      Log.Autoresearch.warn
        "git reset HEAD~1 failed in %s: %s"
        workdir
        (Printexc.to_string exn))
;;
...
let git_commit_cycle ~workdir ~cycle ~hypothesis ~baseline =
  ...
  in
  let message =
    Printf.sprintf "[autoresearch] cycle %d: %s (baseline=%.4f)" cycle safe_hyp baseline
  in
...
let git_tag_best ~workdir ~cycle ~score =
  ...
  then ()
    ...
    try
      let _status, _output =
        run_git_with_status ~timeout_sec:10.0 ~workdir [ "tag"; "-f"; tag ]
      in
      ()
    with
    | Eio.Cancel.Cancelled _ as e -> raise e
    | exn ->
      Log.Autoresearch.warn "git tag failed in %s: %s" workdir (Printexc.to_string exn))
;;

(** Get the git top-level directory for a workdir. *)
let git_top_level ~workdir =
  if not (is_in_git_repo workdir)
  then Result.error "workdir is not inside a git repository"
  else (
    match run_capture_lines ~workdir [ "rev-parse"; "--show-toplevel" ] with
    | Unix.WEXITED 0, top :: _ ->
      let trimmed = String.trim top in
      if trimmed = "" then Result.error "git top-level was empty" else Result.ok trimmed
...
let managed_branch_name loop_id = "autoresearch/" ^ loop_id
...
let prepare_managed_worktree ~base_path ~source_workdir ~loop_id =
  ...
  | Ok repo_root ->
    ...
    then Result.error (Printf.sprintf "managed worktree already exists: %s" workdir)
    else (
      ...
      with
      | Unix.WEXITED 0, _ -> Result.ok (workdir, repo_root, List.rev warnings)
      | _, lines ->
        Result.error
          (Printf.sprintf
