...
```text
GET /mcp                     (유지, MCP transport observer/workspace client stream)
...
```ocaml
(* lib/sse.ml 발췌 — #12129 *)
type session_kind = Observer | Workspace client | Presence
type broadcast_target = All | Observers | Workspace clients | Presence_only
