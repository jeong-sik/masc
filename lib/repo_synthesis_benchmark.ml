...
let bench_root ~base_path =
  Filename.concat (Common.masc_dir_from_base_path ~base_path) "repo-synthesis-benchmarks"
;;
...
let validate_run_id run_id =
  ...
  if run_id = ""
  then Error "benchmark run id cannot be empty"
  else if String.length run_id > 128
  then Error "benchmark run id too long (max 128 chars)"
  else if String.contains run_id '/' || String.contains run_id '\\'
  then Error "benchmark run id cannot contain path separators"
  else if contains_substring run_id ".."
  then Error "benchmark run id cannot contain traversal segments"
  else if
    not
      (String.for_all
         (function
         ...
         run_id)
  then
    Error "benchmark run id may only contain letters, digits, dot, underscore, and hyphen"
  else Ok run_id
;;
...
let normalize_ci_string value = value |> String.trim |> String.lowercase_ascii
...
let path_matches ~gold ~cited =
  ...
  String.equal gold cited || String.ends_with ~suffix:("/" ^ gold) cited
;;
...
let question_to_yojson (q : question) =
  `Assoc
    ...
    ; ( "difficulty"
      , Option.fold ~none:`Null ~some:(fun value -> `String value) q.difficulty )
...
let question_of_yojson (json : Yojson.Safe.t) =
  ...
  | Some question_id ->
    Some
      { question_id
      ; title =
          json |> member "title" |> to_string_option |> Option.value ~default:question_id
      ; question =
          json |> member "question" |> to_string_option |> Option.value ~default:""
    ...
    |> List.filter_map (fun item ->
      ...
      | Some question_id ->
        Some
          ...
          ; evidence_precision =
              item
              |> member "evidence_precision"
              |> to_float_option
              |> Option.value ~default:0.0
          ; claim_coverage =
              item
              |> member "claim_coverage"
              |> to_float_option
              |> Option.value ~default:0.0
          ; unsupported_claim_penalty =
              item
              |> member "unsupported_claim_penalty"
              |> to_float_option
              |> Option.value ~default:0.0
          ; latency_ms =
              item |> member "latency_ms" |> to_int_option |> Option.value ~default:0
          ; matched_claims =
              item
              |> member "matched_claims"
              |> to_list
              |> List.filter_map to_string_option
          ; missing_claims =
              item
              |> member "missing_claims"
              |> to_list
              |> List.filter_map to_string_option
          ; matched_paths =
              item
              |> member "matched_paths"
              |> to_list
              |> List.filter_map to_string_option
          ; unsupported_claims =
              item
              |> member "unsupported_claims"
              |> to_list
              |> List.filter_map to_string_option
          })
  in
...
let run_record_to_yojson (run : run_record) =
  `Assoc
    ...
    ; ( "created_by"
      , Option.fold ~none:`Null ~some:(fun value -> `String value) run.created_by )
    ; "goal", `String run.goal
    ; "question", `String run.question
    ; ( "question_id"
      , Option.fold ~none:`Null ~some:(fun value -> `String value) run.question_id )
    ; "repo_root", `String run.repo_root
    ; "artifact_scope", `List (List.map (fun value -> `String value) run.artifact_scope)
    ; ( "program_note"
      , Option.fold ~none:`Null ~some:(fun value -> `String value) run.program_note )
    ; ( "baseline_label"
      , Option.fold ~none:`Null ~some:(fun value -> `String value) run.baseline_label )
    ...
    ; ( "operation_id"
      , Option.fold ~none:`Null ~some:(fun value -> `String value) run.operation_id )
    ; "trace_id", Option.fold ~none:`Null ~some:(fun value -> `String value) run.trace_id
    ; ( "session_id"
      , Option.fold ~none:`Null ~some:(fun value -> `String value) run.session_id )
    ; ( "report_json_path"
      , Option.fold ~none:`Null ~some:(fun value -> `String value) run.report_json_path )
    ; ( "report_md_path"
      , Option.fold ~none:`Null ~some:(fun value -> `String value) run.report_md_path )
    ; ( "proof_json_path"
      , Option.fold ~none:`Null ~some:(fun value -> `String value) run.proof_json_path )
    ; ( "proof_md_path"
      , Option.fold ~none:`Null ~some:(fun value -> `String value) run.proof_md_path )
    ; ( "dataset_ref"
      , Option.fold ~none:`Null ~some:(fun value -> `String value) run.dataset_ref )
    ; "case_refs", `List (List.map (fun value -> `String value) run.case_refs)
    ; ( "planned_worker_roles"
      , `List (List.map (fun value -> `String value) run.planned_worker_roles) )
    ; ( "recommended_next_tools"
      , `List (List.map (fun value -> `String value) run.recommended_next_tools) )
...
let run_record_of_yojson (json : Yojson.Safe.t) =
  ...
  | Some benchmark_run_id ->
    Some
      ...
      ; repo_root =
          json |> member "repo_root" |> to_string_option |> Option.value ~default:""
      ; artifact_scope = string_list_field "artifact_scope"
      ...
      ; max_workers =
          json |> member "max_workers" |> to_int_option |> Option.value ~default:0
      ; time_budget_sec =
          json |> member "time_budget_sec" |> to_int_option |> Option.value ~default:0
      ; workload_profile =
          json
          |> member "workload_profile"
          |> to_string_option
      ...
      ; status =
          json |> member "status" |> to_string_option |> Option.value ~default:"started"
      }
;;
...
let save_run ~base_path (run : run_record) =
  ...
  | Ok run_id ->
    write_json_file (run_json_path_unchecked ~base_path run_id) (run_record_to_yojson run)
  | Error msg -> invalid_arg msg
;;
...
let save_score ~base_path ~run_id (score : score_summary) =
  ...
  | Ok run_id ->
    write_json_file
      (score_json_path_unchecked ~base_path run_id)
...
let scan_run_ids ~base_path =
  ...
  if not (Sys.file_exists root && Sys.is_directory root)
  then []
  else
    Sys.readdir root
    |> Array.to_list
...
let list_runs ~base_path =
  ...
  |> List.filter_map (fun run_id ->
    Option.map (fun run -> run, load_score ~base_path run_id) (load_run ~base_path run_id))
...
let score_answer ~(question : question) (answer : answer) =
  ...
  let unsupported_claims =
    ...
    |> List.filter (fun claim ->
      not (string_list_member_ci claim question.required_claims))
  in
  let matched_paths =
    answer.cited_paths
  ...
  let evidence_precision =
    ...
    | [] -> 0.0
    | cited ->
      float_of_int (List.length matched_paths) /. float_of_int (List.length cited)
  in
  let claim_coverage =
    match question.required_claims with
    | [] -> 1.0
    | required ->
      float_of_int (List.length matched_claims) /. float_of_int (List.length required)
  in
  let unsupported_claim_penalty =
    match answer.claims with
    | [] -> 0.0
    | claims ->
      float_of_int (List.length unsupported_claims) /. float_of_int (List.length claims)
  in
...
let run_summary_json ~base_path (run : run_record) score_opt =
  `Assoc
    ...
    ; ( "score_json_path"
      , if Option.is_some score_opt
        then `String (score_json_path ~base_path run.benchmark_run_id)
        else `Null )
    ]
;;
