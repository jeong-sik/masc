...
let make_fixture_dir () =
  ...
  write_file (Filename.concat boundary "RuntimeStrategy.tla") "MODULE RuntimeStrategy\n";
  write_file (Filename.concat boundary "RuntimeStrategy.cfg") "SPECIFICATION Spec\n";
  write_file (Filename.concat boundary "RuntimeStrategy-buggy.cfg") "SPECIFICATION SpecBuggy\n";
...
let test_list_specs_basic () =
  ...
  Fun.protect ~finally:(fun () -> cleanup root) (fun () ->
    with_specs_dir root (fun () ->
      let entries = Dashboard_tla_specs.list_specs () in
      check int "three specs" 3 (List.length entries);
      let names = List.map (fun (e : Dashboard_tla_specs.spec_entry) -> e.name) entries in
...
let test_cfg_presence_flags () =
  ...
  Fun.protect ~finally:(fun () -> cleanup root) (fun () ->
    with_specs_dir root (fun () ->
      let entries = Dashboard_tla_specs.list_specs () in
      let runtime =
        List.find (fun (e : Dashboard_tla_specs.spec_entry) -> e.name = "RuntimeStrategy") entries
      in
      check bool "clean cfg present" true runtime.has_clean_cfg;
      check bool "buggy cfg present" true runtime.has_buggy_cfg;
      let only_tla =
        List.find (fun (e : Dashboard_tla_specs.spec_entry) -> e.name = "ZOnlyTla") entries
      in
...
let test_category_mapping () =
  ...
  Fun.protect ~finally:(fun () -> cleanup root) (fun () ->
    with_specs_dir root (fun () ->
      let entries = Dashboard_tla_specs.list_specs () in
      let overflow =
        List.find (fun (e : Dashboard_tla_specs.spec_entry) -> e.name = "Overflow") entries
      in
...
let test_tlc_results_from_logs () =
  ...
  Fun.protect
    ...
    (fun () ->
      write_file
        (Filename.concat logs_root "tlc-RuntimeStrategy.log")
      ...
      write_file
        (Filename.concat logs_root "tlc-RuntimeStrategy-buggy.log")
      ...
      with_specs_dir specs_root (fun () ->
        with_tlc_results_dir logs_root (fun () ->
          let entries = Dashboard_tla_specs.list_tlc_results () in
          check int "cfg-backed results" 3 (List.length entries);
          let runtime =
            List.find
              (fun (e : Dashboard_tla_specs.tlc_result_entry) ->
                 e.cfg_name = "RuntimeStrategy.cfg")
              entries
          in
          check string "clean spec name" "RuntimeStrategy" runtime.spec_name;
          check bool "clean log path present" true (Option.is_some runtime.log_path);
          check (option int) "states generated" (Some 334) runtime.states_explored;
          check (option int) "distinct states" (Some 334) runtime.distinct_states;
          check (option int) "diameter" (Some 14) runtime.diameter;
          check bool "clean passed" true
            (match runtime.status with
             | Dashboard_tla_specs.Tlc_passed -> true
             | _ -> false);
          let buggy =
            List.find
              (fun (e : Dashboard_tla_specs.tlc_result_entry) ->
          ...
          check bool "buggy violated" true
            (match buggy.status with
             | Dashboard_tla_specs.Tlc_violated -> true
             | _ -> false);
          ...
          check bool "missing log is not_run" true
            (match overflow.status with
             | Dashboard_tla_specs.Tlc_not_run -> true
             | _ -> false))))
