...
(** {1 Metric Name Constants}
    ...
    [include] re-exports them so callers continue to write
    [Prometheus.metric_X]. *)

include Prometheus_metric_names
include Prometheus_metric_names_b
include Prometheus_metric_names_c

(* These three are kept in [Prometheus] (rather than the included modules)
   because [Prometheus.mli] never published them — moving them to the
   shared module would silently widen the public API. They are referenced
   only inside [init] / [update_fd_gauges] below. *)
...
(* Backend filesystem mutex observers must remain in [Prometheus] because
   they capture [observe_histogram] (defined above) and depend on
   [Backend.FileSystem]. The metric-name constants they reference live in
