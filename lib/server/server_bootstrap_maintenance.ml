  ...
  fork_logged_fiber
    ...
    let rec loop () =
      ...
      (try
         ...
         (* Agent registry: remove caches whose registered identity is gone. *)
         let ar_reaped =
           Client_registry_eio.cleanup_unregistered_session_caches ()
         in
         if ar_reaped > 0
         then
           Log.Server.info
             "Removed %d unregistered agent session cache entries"
             ar_reaped;
