...
let test_transport_from_env_default () =
  ...
  Alcotest.(check string)
    "default is local"
    "local"
    (Masc_mcp.Masc_grpc_transport.to_string t)
;;
...
let test_transport_to_string () =
  Alcotest.(check string)
    "http"
    "http"
    (Masc_mcp.Masc_grpc_transport.to_string Masc_mcp.Masc_grpc_transport.Http);
  Alcotest.(check string)
    "grpc"
    "grpc"
    (Masc_mcp.Masc_grpc_transport.to_string Masc_mcp.Masc_grpc_transport.Grpc);
  Alcotest.(check string)
    "local"
    "local"
    (Masc_mcp.Masc_grpc_transport.to_string Masc_mcp.Masc_grpc_transport.Local)
;;
  ...
  Alcotest.run
    ...
    [ ( "client-types"
      , [ Alcotest.test_case "join response roundtrip" `Quick test_join_response_roundtrip
        ; Alcotest.test_case
            "leave response roundtrip"
            `Quick
            test_leave_response_roundtrip
        ; Alcotest.test_case
            "heartbeat ping roundtrip"
            `Quick
            test_heartbeat_ping_roundtrip
        ; Alcotest.test_case "heartbeat ack roundtrip" `Quick test_heartbeat_ack_roundtrip
        ; Alcotest.test_case
            "tool call response roundtrip"
            `Quick
            test_tool_call_response_roundtrip
        ; Alcotest.test_case
            "tool call request roundtrip"
            `Quick
            test_tool_call_request_roundtrip
        ; Alcotest.test_case
            "broadcast request roundtrip"
            `Quick
            test_broadcast_request_roundtrip
        ; Alcotest.test_case
            "broadcast response roundtrip"
            `Quick
            test_broadcast_response_roundtrip
        ; Alcotest.test_case "event roundtrip" `Quick test_event_roundtrip
        ; Alcotest.test_case
            "status response roundtrip"
            `Quick
            test_status_response_roundtrip
        ; Alcotest.test_case "subscribe request serde" `Quick test_subscribe_request_serde
        ] )
    ; ( "directives"
      , [ Alcotest.test_case
            "P3 directive types roundtrip"
            `Quick
            test_heartbeat_ack_directive_types
        ; Alcotest.test_case
            "empty directives roundtrip"
            `Quick
            test_heartbeat_ack_empty_directives
        ] )
    ; ( "transport"
