(** Pin the typed classification of the MASC-internal "Eio context
    unavailable" error ([Runtime_agent_core_runner.is_eio_context_error]).
    ...
    substring-scanning [Agent_core.Error.to_string]:
...
module R = Runtime_agent_core_runner
...
let test_tagged_error_is_eio_context () =
  assert (
    R.is_eio_context_error
      (R.eio_context_error_to_core_error
...
let test_wording_independence () =
  assert (
    R.is_eio_context_error
      (R.eio_context_error_to_core_error "totally different diagnostic wording"))
...
let test_other_config_field_excluded () =
  assert (
    not
      (R.is_eio_context_error
         (R.runtime_catalog_error_to_core_error "Eio switch not available")))

(* Non-Config error families are never eio-context, even when their message
   contains the phrase. *)
let test_non_config_errors_excluded () =
  assert (
    not (R.is_eio_context_error (Agent_core.Error.Internal "Eio switch not available")))
...
let () =
  ...
  print_endline "test_runtime_agent_core_eio_context_classify: OK"
