...
- 날짜(ISO8601): 2026-08-31T06:56:11+09:00
- 작성자: Codex
...
- 출처: `benchmarks/context_recovery/results/20260831-tool-metrics-flush-r1/r100-5000-call-replacement/summary.json`, `benchmarks/context_recovery/results/20260831-tool-metrics-flush-r1/r101-r103-adversarial/summary.json`, `docs/research/2026-08-31-tool-metrics-flush-cadence-linux-r1.md`, issue #32011
- 확인일시: 2026-08-31T06:56:11+09:00
- 신뢰도: High
- 제한조건: Linux/arm64 Docker Desktop, local named volume와 macOS bind mount, 68초 idle A/B, admitted sequential 및 256-way 5,000-call load
...
- 5차: r101에서 같은 image의 `0.5s`와 `300s`를 68초 유휴 A/B로 비교했다.
- 6차: HTTP 200만 센 r102를 폐기하고, r103에서 body-level success와 live/disk/replacement를 함께 확인했다.
- 재현 결과: r100과 r103은 5,000회를 모두 background batch로 기록했고 replacement API도 5,000이었다. r101의 짧은 유휴 CPU 차이는 0.07초였다. 관련 runtime은 exit 0/OOM false였다.

## 불확실성

- 미확인 항목: 0.5초 안의 4,096 초과 handled-call burst, 강제로 지연한/network filesystem, 장시간 idle wake cost, 전체 테스트, GitHub CI, Kubernetes/PVC.
