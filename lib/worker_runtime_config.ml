...
let default =
  { worker_spawn =
      ...
      ; docker = { image = "masc-worker-runtime:dev"; host_mcp_base_url = None }
      }
  }
;;
...
let malformed_fail_closed =
  { worker_spawn =
      ...
      ; docker = { image = ""; host_mcp_base_url = None }
      }
  }
;;
...
let env_image_opt () = Sys.getenv_opt "MASC_WORKER_RUNTIME_DOCKER_IMAGE" |> trim_opt
...
let load_file_config path =
  ...
  then (
    ...
    | exn ->
      Log.CmdPlane.warn
        "worker-runtime config malformed at %s: %s"
        path
...
let config_path resolution =
  Filename.concat resolution.Config_dir_resolver.config_root.path "worker-runtime.json"
;;
...
let backend () = (resolve ()).worker_spawn.backend
let docker_image () = (resolve ()).worker_spawn.docker.image
let host_mcp_base_url_opt () = (resolve ()).worker_spawn.docker.host_mcp_base_url
