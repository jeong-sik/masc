...
확인한 후 반영합니다. 검증 실패 시 기존 파일은 보존됩니다. 인증은 선택한 shell,
CLI 계정 또는 비공개 인증 저장소를 사용합니다. 제공되는 설치·로그인 작업은 선택했을
때만 실행하며 모델 가중치를 자동으로 받지 않습니다. **Configure later**로 미룰 수
있고 imp는 자동으로 시작하지 않습니다.
