...
let schemas : tool_schema list =
  [ { name = "masc_plan_init"
    ; description =
        "Initialize a planning context for a task, creating task_plan.md, notes.md, and \
         ...
         that needs planning artifacts. After masc_claim_next or masc_add_task; follow \
         up with masc_plan_update to write the plan."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
    ...
    ; input_schema =
        `Assoc
          ...
          ; ( "properties"
            , `Assoc
                [ ( "task_id"
                  , `Assoc [ "type", `String "string"; "description", `String "Task ID" ]
                  )
                ; ( "content"
                  , `Assoc
                      [ "type", `String "string"
        ...
        `Assoc
          ...
          ; ( "properties"
            , `Assoc
                ...
                ; ( "note"
                  , `Assoc
                      [ "type", `String "string"; "description", `String "Note content" ]
                  )
                ] )
  ...
  ; { name = "masc_deliver"
    ; description =
        "Attach final output/result to a task for handoff or review. Use for: code \
         diffs, PR URLs, analysis reports, generated files. Deliverables persist with \
         task and are visible to other agents. Call before \
         masc_transition(action='done'). Example: masc_deliver({task_id: 'task-001', \
         content: 'PR: github.com/org/repo/pull/123'})"
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
