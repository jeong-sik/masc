...
let dedicated_pr_tool_required (cmd : string) : (string * string * string) option =
  let parts = cmd |> normalize_gh_command |> gh_words |> skip_gh_leading_global_options in
  match parts with
  | "pr" :: "create" :: _ ->
    Some
      ...
      , "keeper_pr_create"
      , "Use keeper_pr_create with draft=true so governance approval and PR lifecycle \
         markers are enforced." )
  | "pr" :: "review" :: _ ->
    Some
      ( "gh_pr_review_requires_keeper_pr_review_comment"
      , "keeper_pr_review_comment"
      , "Use keeper_pr_review_comment for PR review COMMENT/APPROVE/REQUEST_CHANGES \
         mutations." )
  | _ -> None
;;
       ...
       (match task.worktree with
        ...
        | Some wt ->
          ...
          (match repo_slug_of_task_worktree ~git_root ~worktree_cwd with
           ...
           | None ->
             (match origin_url_of_task_worktree ~git_root ~worktree_cwd with
              | Some _ -> Error (Current_task_origin_not_github { task_id; git_root })
