---
title: Runtime Fallback Chain Capability-Tier Routing
rfc: 0055
status: Withdrawn
created: 2026-05-09
withdrawn_date: 2026-05-21
superseded_by: RFC-0058
withdrawn_reason: "Self-declared superseded by RFC-0058 in original body (2026-05-11): RFC-0058 §1 Supersedes ledger explicitly absorbs the capability-tier routing concern via the declarative runtime config schema. Archived for history."
---

# RFC-0055: Runtime Fallback Chain Capability-Tier Routing

**Status**: Superseded by RFC-0058 (declarative runtime config — RFC-0058 §1 / Supersedes ledger explicitly absorbs the capability-tier routing concern)  
**Author**: Agent (Agent-LLM-A Sonnet 4.6)  
**Date**: 2026-05-09 · Superseded 2026-05-11  
**Related**: RFC-0041 (hierarchical runtime config), RFC-0058 (declarative runtime config), PR #14358, PR #14360, 2026-05-04 ollama_bench incident

---

## 1. Problem Statement

The current `runtime.json` defines fallback chains that terminate in `local_recovery`, which is a capability dead-end for any turn requiring runtime MCP tools.

### 1.1 Dead-End Chain

`runtime.json` (live SSOT) contains three independent routes into the same sink:

| Line | Runtime | Fallback Target | `keeper_assignable` |
|------|---------|-----------------|---------------------|
...
The runtime system has no terminal fallback that satisfies the capability requirements of the original request.
...
1. **Structural guarantee**: A runtime chain must terminate in a model set that satisfies the capability requirements of the original request, or in an explicit terminal `Sink` that signals unrecoverable failure.
2. **Compile-time enforcement**: It must be impossible to configure an `Assignable` runtime whose fallback points to a `Sink` or to a runtime with mismatched capabilities.
...
### 4.1 Phantom-Typed Runtime Definition

Introduce a GADT that separates runtimes that can be assigned to keepers from terminal runtimes that cannot:
...
(* phantom-typed runtime identifier *)
type 'a runtime_id = private string
...
type _ runtime_def =
  | Assignable : {
      id : assignable runtime_id;
      models : model_entry list;
      fallback : assignable runtime_id;
      (* capability contract: what this runtime guarantees *)
      min_capability : Capability_set.t;
    } -> assignable runtime_def
  | Sink : {
      id : sink runtime_id;
      models : model_entry list;
...
```ocaml
let fallback_monotonic (src : assignable runtime_def) (dst : assignable runtime_def) : bool =
  Capability_set.is_subset src.min_capability dst.min_capability
```

A runtime whose `min_capability` requires `runtime_mcp_tools` cannot fall back to a runtime whose `min_capability` lacks it. This check runs at config load time (not per-request) and fails fast with a typed error.
...
```ocaml
let local_recovery : sink runtime_def =
  Sink {
    id = ("local_recovery" :> sink runtime_id);
    models = [
...
No assignable runtime may point to a `sink runtime_id`. The only way to reach a `Sink` is via an explicit terminal dispatch decision, not via fallback.

### 4.4 New Assignable Terminal Runtime

Introduce a new assignable runtime that serves as the **true** fallback for runtime-MCP-capable requests. It contains only models that satisfy `runtime_mcp_caps_ok`:

```ocaml
let retired_tool_profile : assignable runtime_def =
  Assignable {
    id = ("retired_tool_profile" :> assignable runtime_id);
    models = [
    ...
    fallback = ("retired_fast_profile" :> assignable runtime_id);
    min_capability = Capability_set.of_list [ Runtime_mcp_tools; Inline_tools; Tool_choice ];
  }
```

`retired_fast_profile` fallback is preserved but its own fallback is changed to a new assignable terminal runtime `retired_cloud_profile` (see Migration).

### 4.5 JSON Config Validation

At config load time (`runtime_catalog_runtime.ml` or equivalent):

1. Parse all runtime definitions into the GADT.
2. Run `fallback_monotonic` on every `Assignable` edge.
3. If any edge violates monotonicity, fail with `Runtime_config_invalid` containing the violating pair.
...
Where `retired_cloud_profile` is a new assignable runtime containing only cloud providers that are known to satisfy `runtime_mcp_caps_ok`:
...
```json
{
  ...
  "retired_cloud_profile_fallback_runtime": null
}
```

`null` fallback means terminal: no further fallback, explicit failure.

### 5.2 Per-Runtime Changes

| Runtime | Current Fallback | New Fallback | Rationale |
|---------|-----------------|--------------|-----------|
...
| `retired_fast_profile` | `retired_tool_profile` | `retired_cloud_profile` | Break the cycle; point to terminal cloud-only runtime |
| `local_recovery` | N/A | N/A (Sink) | Remove from assignable set; only explicit non-tool dispatch may use it |

### 5.3 Backward Compatibility

- Existing `runtime.json` files without the new `retired_cloud_profile` section will fail validation at load time with a typed error, not silently default.
- The `admission` table (lines 178--362) does not reference `local_recovery` directly; no admission changes required.
- `ollama_bench` and `ollama_only` keep their direct runtime override path; they are unaffected because they bypass the assignable_set gate.
...
1. **Monotonicity**: For random capability sets `A` and `B`, generate random runtime definitions. Assert that every fallback edge satisfies `is_subset`.
...
1. **Config load rejection**: Provide a config where `primary.fallback = "local_recovery"`. Assert load fails with `Runtime_config_invalid`.
...
- Verify in logs that fallback hops emit structured events with `from_runtime`, `to_runtime`, and `capability_check` fields.

---

## 7. Open Questions

1. **RFC-0041 mesh**: RFC-0041 introduced hierarchical runtime config + TOML groups. How does the GADT validation interact with group-level overrides? Does a group override that changes `fallback` re-trigger monotonicity checking?
2. **Terminal fallback necessity**: Is `retired_cloud_profile` (a terminal assignable runtime) actually required, or should `retired_fast_profile` itself become terminal? The trade-off is between "one more retry layer" and "simpler topology".
...
- `<base-path>/.masc/config/runtime.json` (live SSOT, lines 30, 67--73, 92, 127, 169)
...
- RFC-0041: Hierarchical runtime config + TOML groups (PR #14358, merged 2026-05-08)
- `~/me/instructions/software-development.md` (Workaround Rejection Bar, TLA+ Bug Model)
- 2026-05-04 incident note: `runtime.json:169` (`ollama_bench` 600 s ceiling, 14 keepers, 5,963 skip-turn events)
