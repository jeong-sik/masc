...
let test_legacy_traceln_records_metadata () =
  ...
  let message = Printf.sprintf "[WARN] legacy warning %f" (Unix.gettimeofday ()) in
...
let test_recent_since_seq_returns_only_new_entries () =
  ...
  let info_message = Printf.sprintf "delta info %f" (Unix.gettimeofday ()) in
  let warn_message = Printf.sprintf "delta warn %f" (Unix.gettimeofday ()) in
  ...
  let rendered =
    Masc_mcp.Oas_log_bridge.render_record_message
      (oas_record
         ...
         [ Agent_sdk.Log.S ("tool", "keeper_github"); Agent_sdk.Log.I ("fixes", 2) ])
  in
  Alcotest.(check string)
    "placeholder message rendered"
...
let test_oas_bridge_normalizes_generic_correction_messages () =
  ...
  in
  Alcotest.(check string)
    "generic correction message normalized"
    "tool keeper_github: correction_pipeline fixed 2 field(s)"
    rendered
;;
  ...
  let level =
    Masc_mcp.Oas_log_bridge.effective_level
      (oas_record
         ...
         [ Agent_sdk.Log.S ("tool", "keeper_fs_read"); Agent_sdk.Log.S ("error", "boom") ])
  in
  Alcotest.(check string)
    "context injector failure promoted"
    "ERROR"
    (Log.level_to_string level)
;;
  ...
  let level =
    Masc_mcp.Oas_log_bridge.effective_level
      (oas_record
         ...
         [ Agent_sdk.Log.S ("tool", "keeper_bash"); Agent_sdk.Log.S ("agent", "demo") ])
  in
  Alcotest.(check string)
    "approval callback gap promoted"
    "ERROR"
    (Log.level_to_string level)
;;
...
let test_file_sink_reopens_when_log_path_is_deleted () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let first_message = Printf.sprintf "file sink first %f" (Unix.gettimeofday ()) in
       let second_message = Printf.sprintf "file sink second %f" (Unix.gettimeofday ()) in
       Log.Ring.init_file_sink dir;
       Log.emit Log.Info ~module_name:"TestLogFileSink" first_message;
       Alcotest.(check bool) "initial log file created" true (Sys.file_exists log_path);
       Sys.remove log_path;
       Alcotest.(check bool) "log file removed" false (Sys.file_exists log_path);
       Log.emit Log.Info ~module_name:"TestLogFileSink" second_message;
       Alcotest.(check bool) "log file recreated" true (Sys.file_exists log_path);
       let messages = read_log_messages log_path in
       Alcotest.(check bool)
         "recreated log contains latest entry"
         true
...
let () =
  Alcotest.run
    ...
    [ ( "ring"
      , [ Alcotest.test_case
            "legacy traceln records metadata"
            `Quick
            test_legacy_traceln_records_metadata
        ; Alcotest.test_case
            "recent since_seq returns only new entries"
            `Quick
            test_recent_since_seq_returns_only_new_entries
        ; Alcotest.test_case
            "oas bridge interpolates placeholder messages"
            `Quick
            test_oas_bridge_interpolates_placeholder_messages
        ; Alcotest.test_case
            "oas bridge normalizes generic correction messages"
            `Quick
            test_oas_bridge_normalizes_generic_correction_messages
        ; Alcotest.test_case
            "oas bridge promotes MCP server failures"
            `Quick
            test_oas_bridge_promotes_mcp_server_failures_to_error
        ; Alcotest.test_case
            "oas bridge promotes context injector failures"
            `Quick
            test_oas_bridge_promotes_context_injector_failures_to_error
        ; Alcotest.test_case
            "oas bridge promotes missing approval callback"
            `Quick
            test_oas_bridge_promotes_missing_approval_callback_to_error
        ; Alcotest.test_case
            "file sink reopens when current log path is deleted"
            `Quick
            test_file_sink_reopens_when_log_path_is_deleted
        ] )
    ]
;;
