...
let test_stateless_basic () =
  ...
  List.iter
    (fun (content, expected) ->
       ...
       if not (mention_mode_equal result expected)
       then
         Alcotest.fail
           (Printf.sprintf
              ...
              (Mention.mode_to_string expected)))
    cases
;;
...
let test_no_mention () =
  let cases = [ "Hello world"; "No mention here"; "" ] in
  List.iter
    (fun content ->
       let result = Mention.parse content in
       if result <> Mention.None
       then
         Alcotest.fail
           (Printf.sprintf
...
let test_broadcast_priority () =
  ...
  let result = Mention.parse content in
  if not (mention_mode_equal result (Mention.Broadcast "ollama"))
  then
    Alcotest.fail
      (Printf.sprintf
...
let test_stateful_priority () =
  ...
  let result = Mention.parse content in
  if not (mention_mode_equal result (Mention.Stateful "ollama-gentle-gecko"))
  then
    Alcotest.fail
      (Printf.sprintf
...
let test_multiple_mentions () =
  ...
  let result = Mention.parse content in
  if not (mention_mode_equal result (Mention.Stateless "ollama"))
  then
    Alcotest.fail
      (Printf.sprintf
...
let test_resolve_stateless () =
  ...
  let targets =
    Mention.resolve_targets (Mention.Stateless "ollama") ~available_agents:available
  in
  match targets with
...
let test_resolve_stateful () =
  let available = [ "ollama-gentle-gecko"; "ollama-bold-shark"; "gemini-swift-tiger" ] in
  let targets =
    Mention.resolve_targets
      (Mention.Stateful "ollama-gentle-gecko")
...
let test_resolve_broadcast () =
  let available = [ "ollama-gentle-gecko"; "ollama-bold-shark"; "gemini-swift-tiger" ] in
  let targets =
    Mention.resolve_targets (Mention.Broadcast "ollama") ~available_agents:available
  in
  if
    List.length targets = 2 && List.for_all (fun n -> String.sub n 0 6 = "ollama") targets
  then ()
  else Alcotest.fail "Should resolve to all ollama agents"
;;
...
let () =
  ...
  run
    ...
    ; "stateful", [ test_case "nickname" `Quick test_stateful_nickname ]
    ; "broadcast", [ test_case "basic" `Quick test_broadcast_basic ]
    ; "none", [ test_case "no_mention" `Quick test_no_mention ]
    ; ( "priority"
