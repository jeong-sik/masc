...
let build_keeper_index () =
  ...
  let tool_index_config = { Agent_sdk.Tool_index.default_config with top_k = 20 } in
  let tool_entries =
    List.map
...
let assert_retrieves ~label index query expected_tool =
  ...
  let rank =
    match List.find_index (fun n -> String.equal n expected_tool) names with
    | Some i -> Some (i + 1)
    | None -> None
  in
  (match rank with
   | Some r ->
     Alcotest.(check bool)
       (Printf.sprintf "%s: %s in top-20 (rank %d)" label expected_tool r)
       true
       true
   | None ->
     let top5 = List.filteri (fun i _ -> i < 5) names in
     Alcotest.failf
       "%s: %s NOT in top-20 for query '%s'. Top 5: [%s]"
       label
       expected_tool
       query
       (String.concat ", " top5));
...
let test_file_read_en () =
  ...
  ignore
    (assert_retrieves
       ~label:"file_read_en"
       idx
       "read the contents of lib/types.ml"
       "keeper_fs_read")
;;
...
let test_file_read_kr () =
  let idx = build_keeper_index () in
  ignore (assert_retrieves ~label:"file_read_kr" idx "파일 읽기" "keeper_fs_read")
;;
...
let test_file_write_en () =
  ...
  ignore
    (assert_retrieves
       ~label:"file_write_en"
       idx
       "write a new config file with updated settings"
       "keeper_fs_edit")
;;

let test_file_write_kr () =
  let idx = build_keeper_index () in
  ignore (assert_retrieves ~label:"file_write_kr" idx "파일 쓰기 편집" "keeper_fs_edit")
;;
...
let test_file_search_en () =
  ...
  ignore
    (assert_retrieves
       ~label:"file_search_en"
       idx
       "search for all occurrences of keeper_fs_edit in the codebase"
       "keeper_shell")
;;

let test_file_search_kr () =
  let idx = build_keeper_index () in
  ignore (assert_retrieves ~label:"file_search_kr" idx "명령어 검색 탐색" "keeper_shell")
;;
...
let test_memory_search_en () =
  ...
  ignore
    (assert_retrieves
       ~label:"memory_en"
       idx
       "what did the user say earlier about the deployment"
       "keeper_memory_search")
;;

let test_memory_search_kr () =
  let idx = build_keeper_index () in
  ignore (assert_retrieves ~label:"memory_kr" idx "기억 검색 대화" "keeper_memory_search")
;;
...
let test_library_search_en () =
  ...
  ignore
    (assert_retrieves
       ~label:"library_en"
       idx
       "search the knowledge library for relevant docs"
...
let test_library_search_kr () =
  let idx = build_keeper_index () in
  ignore
    (assert_retrieves ~label:"library_kr" idx "라이브러리 지식 문서 검색" "keeper_library_search")
;;
...
let test_board_post_en () =
  ...
  ignore
    (assert_retrieves
       ~label:"board_post_en"
       idx
       "post my findings to the board"
       "keeper_board_post")
;;

let test_board_read_kr () =
  let idx = build_keeper_index () in
  ignore (assert_retrieves ~label:"board_read_kr" idx "게시판 글 읽기 조회" "keeper_board_get")
;;
...
let test_build_en () =
  ...
  ignore
    (assert_retrieves
       ~label:"build_en"
       idx
       "run dune build to check if the code compiles"
       "keeper_bash")
;;

let test_build_kr () =
  let idx = build_keeper_index () in
  ignore (assert_retrieves ~label:"build_kr" idx "명령어 실행 빌드 테스트" "keeper_bash")
;;
...
let test_task_claim_en () =
  ...
  ignore
    (assert_retrieves
       ~label:"task_claim_en"
       idx
       "claim the next available task from the backlog"
       "keeper_task_claim")
;;

let test_task_list_kr () =
  let idx = build_keeper_index () in
  ignore (assert_retrieves ~label:"task_list_kr" idx "태스크 목록 할일" "keeper_tasks_list")
;;
...
let test_voice_speak_en () =
  ...
  ignore
    (assert_retrieves
       ~label:"voice_speak_en"
       idx
       "say hello to the user out loud"
       "keeper_voice_speak")
;;

let test_voice_speak_kr () =
  let idx = build_keeper_index () in
  ignore (assert_retrieves ~label:"voice_speak_kr" idx "음성 말하기 보이스" "keeper_voice_speak")
;;
...
let test_github_pr_en () =
  ...
  ignore
    (assert_retrieves
       ~label:"github_pr_en"
       idx
       "check the status of open pull requests"
       "keeper_shell")
;;

let test_github_issue_kr () =
  let idx = build_keeper_index () in
  ignore (assert_retrieves ~label:"github_issue_kr" idx "깃허브 이슈 풀리퀘스트" "keeper_shell")
;;
...
let test_masc_code_search_kr () =
  let idx = build_keeper_index () in
  ignore (assert_retrieves ~label:"masc_code_kr" idx "코드 검색 소스코드" "masc_code_search")
;;
...
let test_masc_worktree_kr () =
  let idx = build_keeper_index () in
  ignore (assert_retrieves ~label:"worktree_kr" idx "워크트리 생성 브랜치" "masc_worktree_create")
;;
...
let test_prefer_fs_edit_over_bash () =
  ...
  | Some fe, Some ba ->
    Alcotest.(check bool)
      (Printf.sprintf
         "fs_edit (rank %d) should rank higher than bash (rank %d)"
         (fe + 1)
         (ba + 1))
      true
      (fe < ba)
  ...
  | None, _ -> Alcotest.fail "keeper_fs_edit not in top-20 for file creation query"
;;
...
let test_index_size () =
  ...
  Alcotest.(check bool)
    (Printf.sprintf "index has %d tools (>= 25)" size)
    true
    (size >= 25)
;;
...
let test_search_alias_entries_target_keeper_universe () =
  ...
  Alcotest.(check (list string))
    "alias entries resolve to keeper universe tools"
    []
    missing
;;
...
let test_tool_search_self_en () =
  ...
  ignore
    (assert_retrieves
       ~label:"tool_search_self"
       idx
       "discover tools by describing what I need"
       "keeper_tool_search")
;;
...
let () =
  ...
  Alcotest.run
    ...
    ; ( "discrimination"
      , [ Alcotest.test_case
            "fs_edit ranks higher than bash for file creation"
            `Quick
            test_prefer_fs_edit_over_bash
        ] )
    ; ( "tool_search"
      , [ Alcotest.test_case
            "tool_search retrieves keeper_tool_search (en)"
            `Quick
            test_tool_search_self_en
        ; Alcotest.test_case
            "worktree via full-universe search (en)"
            `Quick
    ...
    ; ( "stats"
      , [ Alcotest.test_case "index size >= 25 tools" `Quick test_index_size
        ; Alcotest.test_case
            "search aliases target real tools"
            `Quick
