...
**Typed getter classification**: 24/209 tagged (`operator`: 24, `algorithm`: 0, `unclassified`: 185).
...
| `MASC_CONTEXT_RATIO_HARD_CAP` | typed:float | unclassified | unclassified | 537 | {1 Context Ratio Hard Cap} Absolute ceiling for compaction ratio_gate and handoff threshold after multiplier adjustme... |
| `MASC_DASHBOARD_HEALTH_CTX_CRITICAL` | typed:float | unclassified | unclassified | 572 | {1 Dashboard Health Thresholds} Thresholds used by the dashboard keeper health scorer and harness health panels.  Dis... |
| `MASC_DASHBOARD_HEALTH_CTX_WARN` | typed:float | unclassified | unclassified | 573 | {1 Dashboard Health Thresholds} Thresholds used by the dashboard keeper health scorer and harness health panels.  Dis... |
| `MASC_DASHBOARD_HEALTH_PENALTY_CRITICAL` | typed:float | unclassified | unclassified | 574 | {1 Dashboard Health Thresholds} Thresholds used by the dashboard keeper health scorer and harness health panels.  Dis... |
| `MASC_DASHBOARD_HEALTH_PENALTY_WARN` | typed:float | unclassified | unclassified | 575 | {1 Dashboard Health Thresholds} Thresholds used by the dashboard keeper health scorer and harness health panels.  Dis... |
| `MASC_DASHBOARD_RUNTIME_WARNING_CTX_RATIO` | typed:float | unclassified | unclassified | 578 |  |
...
| `MASC_KEEPER_ATTEMPT_WATCHDOG_SAFETY_CAP_SEC` | typed:float | unclassified | unclassified | 399 | Deprecated compatibility knob for the removed whole-run attempt watchdog. The keeper runtime must not apply this as a... |
| `MASC_KEEPER_BODY_TIMEOUT_SEC` | string_literal | n/a | n/a | 451 | Total HTTP body-consumption deadline for non-streaming OAS completion calls. In agent_sdk this wraps [Complete.comple... |
...
| `MASC_KEEPER_CLI_SUBPROCESS_IDLE_SEC` | typed:float | Timeouts | operator | 471 | Stdout-idle timeout for CLI subprocess transports (Anthropic CLI today; other CLI providers need an OAS upstream chan... |
...
| `MASC_KEEPER_EXECUTION_IDLE_TIMEOUT_SEC` | typed:float | Timeouts | operator | 432 | OAS Agent.run inactivity deadline. This is progress-based rather than cumulative wall-clock: OAS resets the timer whe... |
| `MASC_KEEPER_GRPC_MAX_RECONNECT` | typed:int | unclassified | unclassified | 491 | Maximum gRPC reconnect attempts before stopping the heartbeat fiber. Default: 5. Range: [1, 20]. |
| `MASC_KEEPER_GRPC_RECONNECT_BACKOFF_SEC` | typed:float | unclassified | unclassified | 499 | Backoff delay between gRPC reconnect attempts in seconds. Default: 5.0. Range: [1.0, 60.0]. |
...
| `MASC_KEEPER_HEARTBEAT_JITTER_FACTOR` | typed:float | unclassified | unclassified | 292 | Jitter factor applied to heartbeat interval (fraction of base). Default: 0.2 (20%). Range: [0.0, 0.5]. |
| `MASC_KEEPER_IDLE_SKIP_THRESHOLD` | typed:int | unclassified | unclassified | 481 | Consecutive idle tool repetitions before on_idle hook issues Skip. Below this: graduated Nudge messages. Optional too... |
| `MASC_KEEPER_MAX_CONSECUTIVE_HB_FAILURES` | typed:int | unclassified | unclassified | 266 | Maximum consecutive heartbeat failures before raising Keeper_fiber_crash (structured crash via dispatch_event). Defau... |
| `MASC_KEEPER_MAX_CONSECUTIVE_TOOL_FAILURES` | typed:int | unclassified | unclassified | 526 | Maximum consecutive failures for the same (tool_name, args_hash) before blocking further attempts. Prevents infinite ... |
| `MASC_KEEPER_MAX_CONSECUTIVE_TURN_FAILURES` | typed:int | unclassified | unclassified | 273 | Maximum consecutive unified turn failures before marking keeper as crashed. Covers LLM timeout, rate limit, and other... |
| `MASC_KEEPER_MAX_IDLE_TURNS_AUTONOMOUS` | typed:int | unclassified | unclassified | 307 | Max idle turns for scheduled autonomous keeper turns. Keepers have workspace to explore multi-step tool sequences. 10... |
| `MASC_KEEPER_MAX_IDLE_TURNS_REACTIVE` | typed:int | unclassified | unclassified | 314 | Max idle turns for reactive (board/mention triggered) keeper turns. Reactive turns have an explicit trigger — more ... |
...
| `MASC_KEEPER_OAS_TIMEOUT_SEC` | string_literal | n/a | n/a | 360 | Per-call OAS timeout override in seconds. Legacy/env override value is clamped to the active keepalive retry/admissio... |
| `MASC_KEEPER_PROACTIVE_MAX_ATTEMPTS` | typed:int | unclassified | unclassified | 509 | Maximum proactive generation attempts before falling back. Default: 3. Range: [1, 10]. |
...
| `MASC_KEEPER_SLEEP_CHUNK_SEC` | typed:float | unclassified | unclassified | 284 | Interruptible sleep chunk size in seconds. Smaller = faster wakeup response but more CPU polling. Default: 2.0. Range... |
...
| `MASC_KEEPER_STAGE_TIMING_RING_SIZE` | typed:int | unclassified | unclassified | 515 | Stage timing ring buffer size for Phase 0 profiling. Default: 100. Range: [10, 1000]. |
| `MASC_KEEPER_STREAM_IDLE_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 408 | Idle-gap timeout for streaming OAS provider responses. This bounds time between streamed lines, not total turn durati... |
| `MASC_KEEPER_TURN_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 346 | Retry/admission budget in seconds for a single unified turn (including all retries and runtime fallbacks). This value... |
| `MASC_KEEPER_VISIBILITY_GATE` | feature_flag | n/a | n/a | 250 | Consumer-driven idle backoff: when true, keepers with no dashboard/SSE observer and no pending signal delay proactive... |
...
| `MASC_PAYLOAD_TELEMETRY` | typed:bool | unclassified | unclassified | 596 | Master switch for wake-payload measurement. Default off so the hot path is untouched until a baseline sweep is explic... |
| `MASC_RUNTIME_SATURATION_SIGNAL_ENABLED` | typed:bool | unclassified | unclassified | 610 | {1 Runtime Saturation Signal (RFC-0153 Phase A.2)} Feature flag for typed [Runtime_saturation_signal.t] emission from... |
...
## Env_config_runtime (115 knobs; typed classification 7/91)

| Env var | Kind | Category | Ops class | Line | Doc |
|---|---|---|---|---|---|
| `MASC_AGENT_RATE_BURST` | typed:int | unclassified | unclassified | 632 | Per-agent burst capacity. Default: 50. |
| `MASC_AGENT_RATE_LIMIT` | typed:float | unclassified | unclassified | 629 | Per-agent requests per second. Default: 20. |
...
| `MASC_BOARD_BACKEND` | string_literal | n/a | n/a | 519 | Board backend type as a typed selector (e.g. "jsonl", "pg"). |
| `MASC_BOARD_FLUSH_INTERVAL_SEC` | typed:float | unclassified | unclassified | 515 | Flush interval for board persistence (seconds). Default: 30. |
| `MASC_BRIEFING_CACHE_TTL_SEC` | typed:float | unclassified | unclassified | 863 | Dashboard mission briefing cache TTL (seconds). Default: 300 (5 min). |
...
| `MASC_CANCELLATION_CLEANUP_SEC` | typed:float | unclassified | unclassified | 875 | Cancellation token cleanup interval (seconds). Default: 300 (5 min). |
...
| `MASC_DASHBOARD_CTX_COMPACTING` | typed:float | unclassified | unclassified | 710 |  |
| `MASC_DASHBOARD_CTX_HANDOFF_IMMINENT` | typed:float | unclassified | unclassified | 706 | Keeper context-ratio lifecycle thresholds. Higher ratio = closer to context limit = more urgency. |
| `MASC_DASHBOARD_CTX_PREPARING` | typed:float | unclassified | unclassified | 708 |  |
| `MASC_DASHBOARD_EXECUTION_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 749 | Execution surface compute timeout (light + parameterized). Wraps two [Dashboard_cache.get_or_compute_with_timeout] si... |
| `MASC_DASHBOARD_EXECUTION_TRUST_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 763 | Execution-trust surface compute timeout. Wraps [Dashboard_cache.get_or_compute_with_timeout] at [server_dashboard_htt... |
| `MASC_DASHBOARD_KEEPER_ACTION_STALE_SEC` | typed:float | unclassified | unclassified | 701 | Keeper action-age threshold (seconds). Default: 3600 (1 hour). |
| `MASC_DASHBOARD_MISSION_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 773 | Mission card compute timeout. Wraps three [Dashboard_cache.get_or_compute_with_timeout] sites at [server_dashboard_ht... |
| `MASC_DASHBOARD_RENDER_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 806 | Maximum wall-clock for a single dashboard render ([Dashboard_execution.json_render]). Wraps the entire render pipelin... |
| `MASC_DASHBOARD_SHELL_LIGHT_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 793 | Shell render compute timeout (light path). Default 8s. Must remain strictly less than [shell_timeout_sec] so the spli... |
| `MASC_DASHBOARD_SHELL_PREWARM_OUTER_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 738 |  |
| `MASC_DASHBOARD_SHELL_PREWARM_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 733 | Dashboard shell-cache pre-warm timeouts. The pre-warm fires once on server bootstrap. It is wrapped in two nested tim... |
| `MASC_DASHBOARD_SHELL_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 782 | Shell render compute timeout (full path). Used by [Dashboard_cache.get_or_compute_with_timeout] for the full shell re... |
| `MASC_DASHBOARD_SIGNAL_LIVE_SEC` | typed:float | unclassified | unclassified | 697 | Duration (seconds) for a signal to count as "live". Default: 300 (5 min). |
| `MASC_DASHBOARD_SIGNAL_QUIET_SEC` | typed:float | unclassified | unclassified | 693 | Duration (seconds) for borderline "quiet" warning. Default: 600 (10 min). |
| `MASC_DASHBOARD_SIGNAL_STALE_SEC` | typed:float | unclassified | unclassified | 689 | Duration (seconds) after which a signal is considered stale. Default: 1200 (20 min). |
...
| `MASC_FULL_HEALTH_CRITICAL_FAILURE_THRESHOLD` | typed:int | unclassified | unclassified | 836 | Number of consecutive [/health?full=1] cache-refresh failures that must accumulate before [masc_full_health_refresh_c... |
| `MASC_FULL_HEALTH_REFRESH_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 824 | Full-health snapshot proactive refresh timeout (seconds). Caps a single [/health?full=1] cache refresh attempt in [Se... |
| `MASC_FULL_SURFACE` | feature_flag | n/a | n/a | 552 | Full tool surface override. Default: false. Re-readable within the process; callers should still document the effecti... |
...
| `MASC_JANITOR_INTERVAL_SEC` | typed:float | unclassified | unclassified | 891 | Bootstrap janitor tick interval (seconds). Drives the SSE/session/ rate-limit/webrtc reaper loop in [server_bootstrap... |
| `MASC_KEEPER_BOOTSTRAP_WINDOW_SEC` | typed:float | unclassified | unclassified | 867 | Keeper world observation bootstrap window (seconds). Default: 300 (5 min). |
| `MASC_KEEPER_MAX_TURN_WATCHDOG_TIMEOUT_SEC` | typed:float | Timeouts | operator | 426 | Default-on at [1800.0] (30 min). Set the env var to [0] to disable. @category Timeouts @ops_class operator |
| `MASC_KEEPER_MID_TURN_PROGRESS_TIMEOUT_SEC` | typed:float | Timeouts | operator | 480 | In-turn progress-silence threshold in seconds; produces [Mid_turn_no_progress] when a running turn records no progres... |
| `MASC_KEEPER_STALE_RUN_SEC` | typed:float | Timeouts | operator | 452 | Default-on at [1800.0] (30 min). Set the env var to [0] to disable. @category Timeouts @ops_class operator |
...
| `MASC_LABEL_QUIET_THRESHOLD_SEC` | typed:float | unclassified | unclassified | 855 | Dashboard label "quiet" threshold (seconds). Default: 300 (5 min). |
| `MASC_LABEL_STUCK_THRESHOLD_SEC` | typed:float | unclassified | unclassified | 859 | Dashboard label "stuck" threshold (seconds). Default: 900 (15 min). |
| `MASC_LIST_PAGE_SIZE` | typed:int | unclassified | unclassified | 557 | Tool list page size, clamped to [10, 1024]. Default: 512. Re-readable within the process; not a guarantee of shell-le... |
...
| `MASC_LOCAL_RUNTIME_COOLDOWN_SEC` | string_literal | n/a | n/a | 644 | Local runtime cooldown (seconds). |
| `MASC_LOCAL_RUNTIME_DEBUG` | feature_flag | n/a | n/a | 640 | Enable local runtime debug logging. Default: false. |
| `MASC_LOCAL_WORKER_HEARTBEAT_SEC` | typed:int | unclassified | unclassified | 650 | Local worker heartbeat interval (seconds). Default: 60. |
| `MASC_LOCAL_WORKER_MAX_TOKENS` | typed:int | unclassified | unclassified | 647 | Local worker max tokens per request. Default: 1024. |
...
| `MASC_METRICS_FLUSH_SEC` | typed:float | unclassified | unclassified | 847 | Tool metrics flush interval (seconds). Default: 300 (5 min). |
| `MASC_OAS_SSE_DRAIN_INTERVAL_SEC` | typed:float | unclassified | unclassified | 658 | SSE drain interval (seconds). Default: 2.0. |
...
| `MASC_PROC_MIN_CONFIDENCE` | typed:float | unclassified | unclassified | 532 | Minimum confidence for crystallization, clamped to [0, 1]. Default: 0.7. |
| `MASC_PROC_MIN_EVIDENCE` | typed:int | unclassified | unclassified | 528 | Minimum evidence count for crystallization. Default: 3. |
| `MASC_PROVIDER_RUN_TTL_SEC` | typed:float | unclassified | unclassified | 879 | Provider run finished TTL (seconds). Default: 3600 (1 hour). |
| `MASC_PUBLIC_TOOLS_EXTRA` | string_literal | n/a | n/a | 591 | Extra public tools (comma-separated names). |
| `MASC_PULSE_MAX_CONSUMER_FAILURES` | typed:int | unclassified | unclassified | 539 | Max consecutive consumer failures before recovery. Default: 3. |
| `MASC_RATE_BURST` | typed:int | unclassified | unclassified | 626 | Burst capacity. Default: 150. |
| `MASC_RATE_LIMIT` | typed:float | unclassified | unclassified | 623 | Requests per second. Default: 100. |
| `MASC_RATE_LIMIT_BUCKET_TTL_SEC` | typed:int | unclassified | unclassified | 905 | Rate-limit bucket staleness TTL (seconds). Buckets with no traffic for this long are reaped by the janitor loop. Defa... |
| `MASC_RELAY_TARGET_AGENT` | typed:string | unclassified | unclassified | 124 | {1 Relay Configuration} |
| `MASC_REPO_SYNC_INTERVAL_SEC` | typed:float | unclassified | unclassified | 897 | Repository auto-sync interval (seconds). The repo_sync fiber in [server_bootstrap_loops] wakes at this cadence to fet... |
| `MASC_SESSION_LIVE_TURN_WINDOW_SEC` | typed:float | unclassified | unclassified | 851 | Team session live turn window (seconds). Default: 300 (5 min). |
...
| `MASC_SHELL_IR_APPROVAL_GATE_ENABLED` | feature_flag | n/a | n/a | 998 | Enable the Shell IR approval policy gate. Default: true (RFC-0254 — the autonomous policy is a strict safety improv... |
| `MASC_SHELL_IR_PATH_JAIL_ENABLED` | feature_flag | n/a | n/a | 1012 | Apply the workspace path jail [Exec_policy.validate_shell_ir_paths] to keeper Execute commands. Default: true. Re-rea... |
| `MASC_SIDECAR_CONTROL_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 931 | Subprocess timeout (seconds) for sidecar control commands — [stop], [tail], and similar quick housekeeping operatio... |
| `MASC_SIDECAR_RECONCILE_BACKOFF_SEC` | typed:float | unclassified | unclassified | 919 | Backoff window (seconds) between repeated same-generation [running + unavailable] start dispatches. Default: 30 (matc... |
| `MASC_SIDECAR_SCHEMA_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 947 | Subprocess timeout (seconds) for sidecar Python schema generation. Wraps [Process_eio.run_argv_with_status] at [serve... |
| `MASC_SLOT_YIELD_ENABLED` | feature_flag | n/a | n/a | 491 | Release LLM slot during tool execution so other agents can use it. Default: false. Set MASC_SLOT_YIELD_ENABLED=true t... |
| `MASC_SMART_HB_BASE_INTERVAL_SEC` | typed:float | unclassified | unclassified | 667 | Base heartbeat interval (seconds), clamped [5, 300]. Default: 30. |
| `MASC_SMART_HB_IDLE_MULTIPLIER` | typed:float | unclassified | unclassified | 672 | Idle multiplier for interval, clamped [1, 10]. Default: 3. |
| `MASC_SMART_HB_IDLE_THRESHOLD_SEC` | typed:float | unclassified | unclassified | 677 | Idle threshold (seconds) before multiplier kicks in, clamped [60, 3600]. Default: 300. |
...
| `MASC_SSE_BUFFER_TTL_SEC` | typed:float | unclassified | unclassified | 871 | SSE buffer TTL (seconds). Default: 300 (5 min). |
| `MASC_STALLED_SESSION_THRESHOLD_SEC` | typed:float | unclassified | unclassified | 883 | Operator digest stalled session threshold (seconds). Default: 300 (5 min). |
...
| `MASC_TOOL_DESCRIPTION_BUDGET` | string_literal | n/a | n/a | 579 | Tool description budget (max chars). None = unlimited. |
| `MASC_TOOL_READONLY_RETRY_LIMIT` | typed:int | unclassified | unclassified | 587 | Read-only tool retry limit. Default: 2. |
| `MASC_TOOL_TIMEOUT_BOARD_SEC` | typed:int | Timeouts | operator | 574 | Board write outer MCP tool-call timeout, clamped to [5, 300] seconds. Default: 90 seconds (#10569). @category Timeout... |
| `MASC_TOOL_TIMEOUT_DEFAULT_SEC` | typed:int | Timeouts | operator | 566 | Default outer MCP tool-call timeout, clamped to [5, 300] seconds. Board writes have a separate timeout below so opera... |
...
| `MASC_WEB_SEARCH_CACHE_TTL_SEC` | typed:float | unclassified | unclassified | 607 |  |
| `MASC_WEB_SEARCH_FALLBACKS` | string_literal | n/a | n/a | 600 |  |
| `MASC_WEB_SEARCH_PROVIDER` | string_literal | n/a | n/a | 594 |  |
| `MASC_WEB_SEARCH_PROVIDER_ORDER` | string_literal | n/a | n/a | 597 |  |
| `MASC_WEB_SEARCH_RATE_LIMIT_MAX_CALLS` | typed:int | unclassified | unclassified | 615 |  |
| `MASC_WEB_SEARCH_RATE_LIMIT_WINDOW_SEC` | typed:float | unclassified | unclassified | 611 |  |
| `MASC_WEB_SEARCH_TIMEOUT_SEC` | typed:int | unclassified | unclassified | 603 |  |
| `MASC_WORKSPACE_GIT_LOCAL_OP_TIMEOUT_SEC` | typed:float | Timeouts | operator | 982 | Budget (seconds) for local-only git operations under [Masc_exec.Exec_gate.run_argv*] in {!Workspace_git}: [rev-parse]... |
...
| `MASC_ASSETS_DIR` | string_literal | n/a | n/a | 653 |  |
| `MASC_BASE_PATH_RESOLUTION_SOURCE` | string_literal | n/a | n/a | 657 |  |
| `MASC_BASE_PATH_STRICT` | string_literal | n/a | n/a | 659 |  |
...
| `MASC_EVENT_BUFFER_SIZE` | string_literal | n/a | n/a | 775 |  |
| `MASC_GOAL_DISPATCH_RUNTIME` | string_literal | n/a | n/a | 609 |  |
...
| `MASC_KEEPER_LLM_RERANK_RUNTIME` | string_literal | n/a | n/a | 563 |  |
| `MASC_KEEPER_OAS_MAX_TURNS_PER_CALL` | string_literal | n/a | n/a | 511 |  |
...
| `MASC_KEEPER_TOOL_AFFINITY_K` | string_literal | n/a | n/a | 567 |  |
| `MASC_KEEPER_TOOL_AFFINITY_LOOKBACK_DAYS` | string_literal | n/a | n/a | 569 |  |
| `MASC_KEEPER_TOOL_COST_MAX_USD` | string_literal | n/a | n/a | 177 |  |
| `MASC_KEEPER_TOOL_DECAY_TURNS` | string_literal | n/a | n/a | 571 |  |
| `MASC_KEEPER_TURN_LIVELOCK_MAX_ATTEMPTS` | string_literal | n/a | n/a | 519 |  |
| `MASC_KEEPER_TURN_LIVELOCK_STUCK_AFTER_SEC` | string_literal | n/a | n/a | 521 |  |
...
| `MASC_OTEL_ENABLED` | string_literal | n/a | n/a | 751 |  |
| `MASC_PLACEHOLDER_TOOLS_ENABLED` | string_literal | n/a | n/a | 787 |  |
| `MASC_ROUTING_RUNTIME` | string_literal | n/a | n/a | 613 |  |
| `MASC_RUNTIME_ATTEMPT_LIVENESS` | string_literal | n/a | n/a | 484 |  |
| `MASC_SEARXNG_URL` | string_literal | n/a | n/a | 799 |  |
...
| `MASC_SSE_KEEPALIVE_SEC` | string_literal | n/a | n/a | 777 |  |
| `MASC_SSE_STREAM_CAPACITY` | string_literal | n/a | n/a | 731 |  |
...
| `MASC_TEST_ALLOW_BASE_PATH_OVERRIDE` | string_literal | n/a | n/a | 767 |  |
| `MASC_TEST_ALLOW_CONFIG_PATH_OVERRIDE` | string_literal | n/a | n/a | 769 |  |
| `MASC_TLA_TRACE` | string_literal | n/a | n/a | 165 |  |
| `MASC_WORKER_RUNTIME_BACKEND` | string_literal | n/a | n/a | 831 |  |
| `MASC_WORKER_RUNTIME_DOCKER_IMAGE` | string_literal | n/a | n/a | 833 |  |
| `MASC_WORKER_RUNTIME_HOST_MCP_BASE_URL` | string_literal | n/a | n/a | 835 |  |
