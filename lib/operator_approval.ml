...
let risk_of_action action_type : Oas.Approval.risk_level =
  if List.mem action_type high_risk_actions
  then High
  else if List.mem action_type allowed_actions
  then Low
  else Medium
;;

let is_allowed action_type = List.mem action_type allowed_actions
let confirm_required action_type = List.mem action_type high_risk_actions
...
let pipeline : Oas.Approval.t =
  Oas.Approval.create
    ...
    ; { Oas.Approval.name = "high_risk_gate"
      ; evaluate =
          (fun ctx ->
            if List.mem ctx.tool_name high_risk_actions
            then Decided (Oas.Hooks.Reject "requires operator confirmation")
            else Pass)
      ; timeout_s = None
      }
    ]
