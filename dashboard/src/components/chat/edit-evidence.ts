...
export function ChatEditEvidence({ output }: { output: ToolCallEntry | null }) {
  ...
  if (!input || typeof input.old_string !== 'string'
      || typeof input.new_string !== 'string') return null
  ...
  return html`
    <section class="m-2 rounded-md border border-border p-2" aria-label="편집 변경 기록" data-chat-edit-evidence>
      ...
      <p class="my-1 text-xs text-[var(--color-fg-muted)]">성공한 편집 호출의 입력 기록입니다. 비밀값 마스킹·길이 제한·앞뒤 공백 제거로 일부 내용이 생략될 수 있어 실제 파일 diff와 다를 수 있습니다.</p>
