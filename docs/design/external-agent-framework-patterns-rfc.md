...
- Agent-LLM-A Agent SDK, OpenAI Agents SDK, Google ADK, CrewAI, LangGraph를
...
| OpenAI Agents SDK workflow primitives | handoff, guardrails, traces, evals are workflow-level building blocks | verification/proof/handoff는 있으나 envelope family가 분산됨 | adopt selectively |
