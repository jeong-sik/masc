(** OAS LLM Provider adapter — bridges MASC LLM types to OAS Provider layer.
    ...
    Routes LLM calls through OAS {!Agent_sdk.Provider.config} and
    {!Agent_sdk.Provider.cascade} types instead of the raw
    {!Llm_provider_bridge} direct HTTP path.
