...
export function CopyIdButton({ value, label, ariaLabel, size = 12 }: CopyIdButtonProps) {
  ...
  return html`
    <button
      type="button"
      class=${`inline-flex shrink-0 cursor-pointer items-center justify-center rounded-[var(--r-1)] min-w-6 min-h-6 p-1.5 text-[var(--color-fg-disabled)] opacity-60 transition-all hover:bg-[var(--color-bg-hover)] hover:text-[var(--color-fg-primary)] hover:opacity-100 focus-visible:opacity-100 ${ringFocusClasses({ tone: 'accent-subtle', width: 1 })}`}
