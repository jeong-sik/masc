...
let privileged_bins = [ "sudo"; "su"; "chmod"; "chown"; "rm"; "dd"; "mkfs" ]

let classify name =
  if List.mem name safe_bins
  then Some `Safe
  else if List.mem name audited_bins
  then Some `Audited
  else if List.mem name privileged_bins
  then Some `Privileged
  else None
;;
