...
let followup_prompt ~original_prompt ~tool_outputs ~already_used =
  let tool_lines =
    tool_outputs
    |> List.mapi (fun idx ((tc : Llm.tool_call), output) ->
...
let parse_text_tool_calls (content : string) : Llm.tool_call list =
  let prefix = "mcp__masc__" in
        ...
        | Some close_idx ->
            ...
            | Ok args_json ->
                collect (close_idx + 1) (call_id + 1)
                  ({
                     Llm.call_id = sprintf "text-fallback-%d" call_id;
                     call_name = tool_name;
...
let make_usage ?(input_tokens = 0) ?(output_tokens = 0) () : Llm.token_usage =
  { Agent_sdk.Types.input_tokens;
    output_tokens;
    cache_creation_input_tokens = 0;
    cache_read_input_tokens = 0 }

let merge_usage (a : Llm.token_usage) (b : Llm.token_usage) : Llm.token_usage =
...
let estimate_cost_usd (model : Llm.model_spec)
    (usage : Llm.token_usage) : float option =
  let input_cost =
