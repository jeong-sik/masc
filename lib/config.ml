...
(* Project every descriptor-owned internal schema into the substrate so help,
   introspection, validation, and dispatch all read the same input contract.
...
let raw_all_tool_schemas : Masc_domain.tool_schema list =
  ...
  |> fun schemas -> schemas @ descriptor_owned_internal_tool_schemas
