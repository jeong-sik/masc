  ...
  | Ok () ->
    Printf.printf
      ...
      (Yojson.Safe.to_string
         (`Assoc
             [ "name", `String name; "ok", `Bool true; "base_path", `String base_path ]))
  | Error message ->
    Printf.printf
      "%s%s\n"
      result_prefix
...
let () =
  ...
  | None ->
    emit_result ~base_path:"" tool_name (Error ("unknown keeper tool: " ^ tool_name));
    flush stdout;
    Unix._exit 2
  | Some schema ->
    Eio_main.run
    ...
    let base_path, result =
      Eio.Switch.run
      @@ fun sw -> Cases.run_case sw ~proc_mgr ~fs ~net ~mono_clock clock schema
    in
