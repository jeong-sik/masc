...
let test_round_trip_all_kinds () =
  List.iter
    (fun kind ->
      let s = Masc.Tool_capability.to_string kind in
      match Masc.Tool_capability.of_string s with
      | Some kind' when kind = kind' -> ()
      | Some _ -> failf "round-trip kind mismatch for %s" s
      | None -> failf "of_string %S returned None" s)
    Masc.Tool_capability.all_kinds
;;
...
let test_all_kinds_cardinality () =
  (check int)
    ...
    4
    (List.length Masc.Tool_capability.all_kinds)
;;
...
let test_of_string_unknown_returns_none () =
  (check (option string))
    ...
    None
    (Option.map
       Masc.Tool_capability.to_string
       (Masc.Tool_capability.of_string "unknown_capability"))
;;
...
let test_has_unknown_tool_returns_false () =
  (check bool)
    ...
    false
    (Masc.Tool_capability.has Read_only "__nonexistent_tool__")
;;
...
let test_has_catalog_metadata () =
  ...
  List.iter
    (fun kind ->
      (check bool)
        ("catalog metadata grants " ^ Masc.Tool_capability.to_string kind)
        true
        (Masc.Tool_capability.has kind name))
    Masc.Tool_capability.all_kinds
;;
...
let test_has_catalog_inferred_capabilities () =
  (check bool)
    ...
    true
    (Masc.Tool_capability.has Read_only "tool_read_file");
  (check bool)
    "static inline metadata grants Mcp_context_required"
    true
    (Masc.Tool_capability.has Mcp_context_required "masc_agents");
  (check bool)
    "static destructive metadata grants Destructive"
    true
    (Masc.Tool_capability.has Destructive "shell_exec")
;;

let test_granted_unknown_tool_is_empty () =
  let s = Masc.Tool_capability.granted "__nonexistent_tool__" in
  (check bool)
    "granted on an unregistered tool returns the empty set"
    true
    (Masc.Tool_capability.Set.is_empty s)
;;

let test_check_empty_required_is_ok () =
  let result =
    Masc.Tool_capability.check
      ~required:Masc.Tool_capability.Set.empty
      ~granted:Masc.Tool_capability.Set.empty
  in
  match result with
  | Ok () -> ()
  | Error _ ->
...
let test_check_missing_capability_returns_error () =
  let required =
    Masc.Tool_capability.Set.singleton Masc.Tool_capability.Read_only
  in
  let granted = Masc.Tool_capability.Set.empty in
  match Masc.Tool_capability.check ~required ~granted with
  | Ok () -> failf "check with missing capability should return Error"
  | Error missing ->
    (check int)
      "Error.missing.cardinality"
      1
      (Masc.Tool_capability.Set.cardinal missing)
;;
...
let test_check_granted_superset_is_ok () =
  ...
  in
  let granted =
    Masc.Tool_capability.Set.add
      Masc.Tool_capability.Idempotent
      required
  in
  match Masc.Tool_capability.check ~required ~granted with
  | Ok () -> ()
