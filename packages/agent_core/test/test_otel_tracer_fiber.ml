...
let test_parallel_spans_have_correct_parents () =
  Eio_main.run (fun _env ->
    let inst = Masc_agent_core.Otel_tracer.create_instance_eio () in
    let module T = (val Masc_agent_core.Otel_tracer.tracer_of_instance inst) in
    let parent_attrs =
      { Masc_agent_core.Tracing.kind = Agent_run
      ; name = "parent"
      ; agent_name = "test"
      ; turn = 1
      ; extra = []
    ...
    T.with_span parent_attrs (fun () ->
      let parent_span = Option.get (Masc_agent_core.Otel_tracer.inst_current_span inst) in
      let parent_id = parent_span.Masc_agent_core.Otel_tracer.span_id in
      let child_results =
        Eio.Fiber.List.map
          (fun i ->
             let child_attrs =
               { Masc_agent_core.Tracing.kind = Tool_exec
             ...
             T.with_span child_attrs (fun () ->
               let child_span =
                 Option.get (Masc_agent_core.Otel_tracer.inst_current_span inst)
               in
               child_span.Masc_agent_core.Otel_tracer.parent_span_id))
          [ 0; 1; 2 ]
      in
      List.iteri
        (fun i ppid ->
...
let test_parallel_spans_do_not_cross_contaminate () =
  Eio_main.run (fun _env ->
    ...
    T.with_span root_attrs (fun () ->
        ...
        Eio.Fiber.List.map
          (fun i ->
             ...
             T.with_span sibling_attrs (fun () ->
               let span = Option.get (Masc_agent_core.Otel_tracer.inst_current_span inst) in
               span.Masc_agent_core.Otel_tracer.name, span.Masc_agent_core.Otel_tracer.parent_span_id))
          [ 0; 1; 2 ]
      in
      ...
      let active = Option.get (Masc_agent_core.Otel_tracer.inst_current_span inst) in
      Alcotest.(check string "active span is root after siblings" "agent_run/root")
        active.Masc_agent_core.Otel_tracer.name))
;;

let test_global_with_span_is_fiber_safe () =
  Eio_main.run (fun _env ->
    (* Ensure the global tracer is initialized inside the Eio runtime. *)
    let (_ : Masc_agent_core.Otel_tracer.span) =
      Masc_agent_core.Otel_tracer.start_span
        { Masc_agent_core.Tracing.kind = Agent_run
        ; name = "warmup"
        ; agent_name = "test"
        ; turn = 1
    ...
    in
    Masc_agent_core.Otel_tracer.reset ();
    let root_attrs =
      { Masc_agent_core.Tracing.kind = Agent_run
      ; name = "global_root"
      ; agent_name = "test"
    ...
    in
    Masc_agent_core.Otel_tracer.with_span root_attrs (fun () ->
      let parent_span = Option.get (Masc_agent_core.Otel_tracer.current_span ()) in
      let parent_id = parent_span.Masc_agent_core.Otel_tracer.span_id in
      let child_ids =
        Eio.Fiber.List.map
          (fun i ->
             ...
             in
             Masc_agent_core.Otel_tracer.with_span child_attrs (fun () ->
               let child = Option.get (Masc_agent_core.Otel_tracer.current_span ()) in
               child.Masc_agent_core.Otel_tracer.parent_span_id))
          [ 0; 1; 2 ]
      in
      List.iteri
        (fun i ppid ->
