...
OAS pin metadata is generated from `scripts/oas-agent-sdk-pin.sh`. Current dependency floor: `agent_sdk >= 0.155.0`, runtime pin: `main@6c79cf3f33c550cd928308b7b2168446c7c0a630`, declared base version: `v0.155.0`. 최신성 검증이 필요할 때는 문서에 적힌 숫자보다 `dune-project`와 pin script를 우선 truth source로 본다.
...
- 기본 sandbox image는 digest-pinned `ubuntu:24.04@sha256:...`를 사용한다.
- `MASC_KEEPER_SANDBOX_SECCOMP_PROFILE`, `MASC_KEEPER_SANDBOX_REQUIRE_ROOTLESS`, `MASC_KEEPER_SANDBOX_REQUIRE_USERNS`로 hardening을 더 강하게 만들 수 있다.
- `shared_memory_scope=room`은 파일시스템 공유 mount가 아니라 flattened `default` namespace typed lane만 연다.
...
- `effective_sandbox_image`는 현재 sandbox에 실제로 적용되는 digest-pinned image를 보여준다.
- `sandbox_last_error`가 있으면 Docker sandbox 실행 실패나 정책 위반 흔적을 먼저 확인한다.
- `shared_memory_scope=room`이면 공용 파일시스템이 아니라 flattened `default` namespace의 `masc_team_memory_*` 호출을 기준으로 협업해야 한다.
