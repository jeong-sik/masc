...
Default registration: a single hook that drains tracked subprocess pids (closes #32). Subprocess registration is added to `tool_execute`, `keeper_shell`, and `keeper_fs_*` execution paths — every external `Eio.Process.spawn` records pid+keeper_id, the cleanup hook sends SIGTERM and then `waitpid`.
...
|-------|-------|---------|---------------|
| A | Cleanup hook plumbing + subprocess pid tracking (closes #32 in single-container mode) | `lib/keeper/keeper_supervisor.ml`, `lib/keeper/keeper_subprocess_registry.ml` (new), call-site instrumentation in `tool_execute`/`keeper_shell` | 2 PRs (foundation + instrumentation) |
