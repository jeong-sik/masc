...
let agent_names = List.init num_agents (fun i -> Printf.sprintf "agent-%02d" i)
...
let test_session_lock_contention () =
  ...
  (* Register all agents *)
  List.iter (fun name -> ignore (Session.register registry ~agent_name:name)) agent_names;
  ...
  let worker agent_name =
    for _ = 1 to iterations_per_agent do
      ...
      ignore
        (Session.check_rate_limit_ex
           registry
           ~agent_name
           ~category:Types.GeneralLimit
           ~role:Types.Worker)
    done
  in
...
let test_cancellation_token_race () =
  ...
  Eio.Fiber.all (List.init num_agents (fun i -> fun () -> worker i));
  Printf.printf
    "Created: %d, Cancelled: %d, Checked: %d\n%!"
    (Atomic.get created)
...
let test_zombie_detection_concurrent () =
  ...
  let checker () =
    for _ = 1 to 100 do
      if Resilience.Zombie.is_zombie stale_time then Atomic.incr zombie_checks;
      if not (Resilience.Zombie.is_zombie fresh_time) then Atomic.incr fresh_checks
    done
  in
    ...
    [ ( "session_lock"
      , [ test_case "lock contention (20 agents)" `Slow test_session_lock_contention
        ; test_case
            ...
            `Slow
            test_session_concurrent_register_unregister
        ] )
    ; ( "cancellation"
