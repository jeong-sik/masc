...
let avg_int_option values =
  values |> List.filter_map (fun value -> value) |> List.map float_of_int |> avg_float
;;

let avg_float_option values = values |> List.filter_map (fun value -> value) |> avg_float
...
let percentile95_int_option values =
  ...
  | _ ->
    ...
    let idx =
      int_of_float (Float.ceil (0.95 *. float_of_int n)) - 1 |> max 0 |> min (n - 1)
    in
...
let modal_ratio values =
  ...
  | _ ->
    ...
    List.iter
      ...
      values;
    let best = Hashtbl.fold (fun _ count acc -> max acc count) counts 0 in
...
let repeated_metrics_for_view view runs =
  ...
  Hashtbl.fold
    (fun (provider, model, keeper_profile, _case_id)
      (grouped_runs : evidence_run list)
      acc ->
       if List.length grouped_runs < 2
       then acc
       else (
         ...
         let stability =
           ...
           |> avg_float
         in
         ( (provider, model, keeper_profile)
         , (stability, tool_sequence_ratio, prompt_ratio, pass_ratio) )
         :: acc))
    grouped
    []
;;
  ...
  Hashtbl.fold
    (fun key values acc ->
       let stability_values, tool_values, prompt_values, pass_values =
         List.fold_left
           (fun (st_acc, tl_acc, pr_acc, pa_acc) (st, tl, pr, pa) ->
              ( st :: st_acc
              , (match tl with
                 | Some value -> value :: tl_acc
                 | None -> tl_acc)
              , (match pr with
                 | Some value -> value :: pr_acc
                 | None -> pr_acc)
              , match pa with
                | Some value -> value :: pa_acc
                | None -> pa_acc ))
           ([], [], [], [])
           values
       in
       ...
       , ( avg_float stability_values
         , (match tool_values with
            | [] -> None
            | _ -> Some (avg_float tool_values))
         , (match prompt_values with
            | [] -> None
            | _ -> Some (avg_float prompt_values))
         , (match pass_values with
            | [] -> None
            | _ -> Some (avg_float pass_values))
         , List.length values ) )
       :: acc)
    grouped
    []
...
let build_summary_rows view runs scores =
  let repeated_metrics =
    repeated_metrics_for_view view runs
  ...
  Hashtbl.fold
    (fun ((provider, model, keeper_profile) as key)
      (grouped_scores : case_score list)
      acc ->
       let grouped_runs =
         runs
         |> List.filter (fun run ->
       ...
       let cases_passed =
         Hashtbl.fold
           (fun _ scores_for_case count ->
              if List.for_all (fun score -> score.passed) scores_for_case
              then count + 1
              else count)
           grouped_scores_by_case
           0
       in
       let row =
         { provider
         ...
         ; task_pass_rate =
             grouped_scores
             |> List.map (fun (score : case_score) -> score.task_pass)
             |> avg_float
         ; correct_tool_rate =
             grouped_scores
         ...
         ; p95_latency_ms =
             grouped_scores
             |> List.map (fun (score : case_score) -> score.latency_ms)
             |> percentile95_int_option
         ; avg_input_tokens =
             grouped_scores
             |> List.map (fun (score : case_score) -> score.input_tokens)
             |> avg_int_option
         ; avg_output_tokens =
             grouped_scores
             |> List.map (fun (score : case_score) -> score.output_tokens)
             |> avg_int_option
         ; avg_cost_usd =
             grouped_scores
             |> List.map (fun (score : case_score) -> score.cost_usd)
             |> avg_float_option
         ; composite_score =
             grouped_scores
             |> List.map (fun (score : case_score) -> score.composite_score)
             |> avg_float
         ; unsupported_runs
...
let summarize ~cases ~runs ?model_filters ?keeper_filters () =
  let filtered_runs =
    runs |> List.filter (fun run -> keep_run ?model_filters ?keeper_filters run)
  in
  let score_results =
    filtered_runs
