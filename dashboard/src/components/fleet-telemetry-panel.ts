      ...
      ${events.slice(0, 6).map(event => html`
        <div class="rounded border px-3 py-2 ${attentionSeverityClass(event.severity)}">
          ...
          ${event.recommended_action
            ? html`<div class="mt-1 text-3xs text-[var(--text-dim)]">다음: ${event.recommended_action}</div>`
            : null}
        </div>
      `)}
    </div>
...
function TelemetrySourcesPanel({ sources }: { sources: TelemetrySourceSummary[] }) {
  if (sources.length === 0) {
    return html`<div class="text-2xs text-[var(--text-dim)]" role="status">텔레메트리 스토어 요약을 사용할 수 없습니다.</div>`
  }
