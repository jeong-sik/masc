...
(* A gateway that is connected under a CONNECTED badge, or disconnected under
   a DISCONNECTED one, says what the badge said. Every other pair adds a
   reading: a resuming gateway under CONNECTED / UNAVAILABLE, a disconnected
   one under UNAVAILABLE. *)
let badge_says_gateway (connection : Reading.connector_connection) :
  ...
  | Reading.Connector_gateway_connected -> (
      ...
      | Reading.Connector_connected -> true
      | Connector_connected_unavailable | Connector_disconnected
