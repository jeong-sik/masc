...
let live_keeper_cascade_name (raw : string) = Keeper_cascade_profile.resolve_live raw
...
let compute_health_score
  ...
  then 0
    ...
    in
    let crash_penalty = Float.min 30.0 (float_of_int recent_crash_count *. 10.0) in
    let context_penalty =
      if context_ratio > health_ctx_critical
      then health_penalty_critical
      else if context_ratio > health_ctx_warn
      then health_penalty_warn
      else 0.0
    in
...
let compute_outcomes_rollup
  ...
  List.iter
    ...
    completed_turns;
  let transitions = Keeper_transition_audit.recent_transitions ~keeper_name ~limit:50 in
  List.iter
  ...
  let cdal_bucket gate_name =
    match
      List.find_opt
        (fun (s : Dashboard_attribution.gate_summary) -> String.equal s.gate gate_name)
...
let execution_receipt_dir config keeper_name =
  Filename.concat
    (Filename.concat (Filename.concat (Coord.masc_root_dir config) "keepers") keeper_name)
    "execution-receipts"
;;
    ...
    | None ->
      ...
      then "empty", "no_entries"
        ...
        | Some ts ->
          ...
          if latest_age_s > execution_trust_freshness_slo_s
...
let execution_receipt_coverage_gaps config =
  Telemetry_coverage_gap.read_recent ~masc_root:(Coord.masc_root_dir config) ~n:50
  |> List.filter (fun gap ->
    String.equal execution_trust_source (Safe_ops.json_string ~default:"" "source" gap))
;;

let keeper_names (config : Coord.config) = Keeper_types.keeper_names config
let keeper_count (config : Coord.config) : int = List.length (keeper_names config)
...
let keepers_dashboard_json ?(compact = false) (config : Coord.config) : Yojson.Safe.t =
  ...
  let shared_sp_events =
    try Keeper_crash_persistence.recent_sp_events ~keepers_dir ~max_entries:20 with
    | Eio.Cancel.Cancelled _ as exn -> raise exn
    | exn ->
      Log.Dashboard.warn
  ...
  let accountability_summary =
    if compact || Keeper_decision_audit.decision_layer_level () < 3
    then
      fun ~keeper_name ~agent_name ->
        Keeper_exec_status_metrics.accountability_summary_json
          ...
          ~keeper_name
          ~agent_name
    else Keeper_exec_status_metrics.accountability_summary_lookup config
  in
       ...
       (fun idx name ->
          ...
          <- (match Keeper_types.read_meta config name with
              ...
              | Ok (Some (m : Keeper_types.keeper_meta)) ->
                ...
                in
                let keeper_age_s =
                  if created_ts <= 0.0 then 0.0 else now_ts -. created_ts
                in
                let last_turn_ago_s =
                  if m.runtime.usage.last_turn_ts <= 0.0
                  then 0.0
                  else now_ts -. m.runtime.usage.last_turn_ts
                in
                let last_handoff_ago_s =
                  if m.runtime.last_handoff_ts <= 0.0
                  then 0.0
                  else now_ts -. m.runtime.last_handoff_ts
                in
                let last_compaction_ago_s =
                  if m.runtime.compaction_rt.last_ts <= 0.0
                  then 0.0
                  else now_ts -. m.runtime.compaction_rt.last_ts
                in
                let last_proactive_ago_s =
                  if m.runtime.proactive_rt.last_ts <= 0.0
                  then 0.0
                  else now_ts -. m.runtime.proactive_rt.last_ts
                in
                let last_visible_proactive_ago_s =
                  if m.runtime.proactive_rt.last_visible_ts <= 0.0
                  then 0.0
             ...
             to avoid showing misleading staleness when agent is actually active *)
                let last_activity_ts =
                  List.fold_left
                    max
                    0.0
                    [ m.runtime.usage.last_turn_ts
                    ; m.runtime.proactive_rt.last_ts
                    ; m.runtime.last_handoff_ts
                    ; m.runtime.compaction_rt.last_ts
                    ; created_ts
                    ]
                in
                ...
                let last_compaction_saved_tokens =
                  max
                    0
                    (m.runtime.compaction_rt.last_before_tokens
                     - m.runtime.compaction_rt.last_after_tokens)
                in
             ...
             Previous value of 12000 caused 60K+ lines across 5 keepers. *)
                ...
                in
                let ( metrics_series_items
                    , metrics_window_summary
                    , last_handoff_event
                    , last_compaction_event )
                  =
                  compute_metrics_window
                    ~parsed_metrics
                    ~generation:m.runtime.generation
                    ~compact
                    ~series_points
                    ~metrics_window_max_bytes
                    ~primary_model_norm
                    ~primary_model
                in
             ...
             Full memory bank is only needed for individual keeper detail view. *)
                let memory_bank_json, memory_recent_note =
                  if compact
                  then `Assoc [ "total_files", `Int 0; "skipped", `Bool true ], None
                  else (
                    let summary =
                    ...
                    in
                    let note =
                      match summary.Keeper_memory.recent_notes with
                ...
                let history_path =
                  Filename.concat
                    (Filename.concat
                       (Keeper_types.session_base_dir config)
                  ...
                  |> List.fold_left
                       (fun acc item ->
                          let role =
                            ...
                            |> String.trim
                          in
                          if String.equal role role_name
                          then (
                            let preview =
                              Safe_ops.json_string ~default:"" "preview" item
                              |> String.trim
                            in
                ...
                let conditions_json =
                  match registry_entry with
                  | Some entry -> Keeper_state_machine.conditions_to_json entry.conditions
                  | None -> `Null
                in
                ...
                let effective_sandbox_image =
                  if
                    m.sandbox_profile = Keeper_types.Docker
                ...
                let runtime_blocker_fields = runtime_blocker_fields_json config m in
                let attention_fields = attention_fields_json config m in
                    ...
                    ( `Assoc
                        ...
                        ; ( "last_failure_reason"
                          , match entry.last_failure_reason with
                            | Some r ->
                              `String (Keeper_registry.failure_reason_to_string r)
                            | None -> `Null )
                        ; ( "dead_since"
                ...
                let context =
                  ...
                  | Some metrics ->
                    `Assoc
                      ...
                      ; ( "context_ratio"
                        , `Float (Safe_ops.json_float "context_ratio" metrics) )
                      ; ( "context_tokens"
                        , `Int (Safe_ops.json_int "context_tokens" metrics) )
                  ...
                  | None ->
                    let effective_cascade_name =
                      live_keeper_cascade_name m.cascade_name
                    in
                  ...
                  let trust_observatory =
                    ...
                    then `Null
                      ...
                      let reputation =
                        ...
                        | exn ->
                          Log.Keeper.warn
                            "trust_observatory reputation failed for %s: %s"
                            m.name
                            (Printexc.to_string exn);
                          `Null
                      in
                  ...
                  `Assoc
                    ([ "name", `String m.name
                     ...
                     @ [ "supervisor_diagnostics", supervisor_diagnostics
                       ...
                       ; ( "emoji"
                         , `String
                             (let e, _ = get_agent_identity m.name in
                              e) )
                       ; ( "koreanName"
                         , `String
                             (let _, k = get_agent_identity m.name in
                              k) )
                       ; ( "trace_id"
                         , `String (Keeper_id.Trace_id.to_string m.runtime.trace_id) )
                       ...
                       ; ( "active_goal_ids"
                         , `List
                             (List.map (fun goal_id -> `String goal_id) m.active_goal_ids)
                         )
                       ...
                       ; ( "short_goal"
                         , if include_goals then `String m.short_goal else `Null )
                       ; ("mid_goal", if include_goals then `String m.mid_goal else `Null)
                       ; ( "long_goal"
                         , if include_goals then `String m.long_goal else `Null )
                       ; ( "goal_horizons"
                         , if include_goals
                           then
                             `Assoc
                       ...
                       ; ( "active_goals_tree"
                         , if (not compact) && include_goals && m.active_goal_ids <> []
                           then (
                             let all_goals = Goal_store.list_goals config () in
                             let linked =
                               List.filter
                                 (fun (g : Goal_store.goal) ->
                                    List.mem g.id m.active_goal_ids)
                                 all_goals
                             in
                             ...
                             `Assoc
                               ...
                               ; ( "nodes"
                                 , `List
                                     (List.map Dashboard_goals.tree_node_to_json forest) )
                               ])
                           else `Null )
                       ; ( "will"
                         , if String.trim m.will = "" then `Null else `String m.will )
                       ; ( "needs"
                         , if String.trim m.needs = "" then `Null else `String m.needs )
                       ; ( "desires"
                         , if String.trim m.desires = "" then `Null else `String m.desires
                         )
                       ; ( "self_model"
                         , `Assoc
                             [ ( "will"
                             ...
                             ; ( "needs"
                               , if String.trim m.needs = ""
                                 then `Null
                                 else `String m.needs )
                             ; ( "desires"
                               , if String.trim m.desires = ""
                                 then `Null
                                 else `String m.desires )
                             ] )
                       ...
                       ; ( "sandbox_profile"
                         , `String
                             (Keeper_types.sandbox_profile_to_string m.sandbox_profile) )
                       ...
                       ; ( "last_visible_proactive_ago_s"
                         , `Float last_visible_proactive_ago_s )
                       ...
                       ; ( "proactive_visible_count_total"
                         , `Int m.runtime.proactive_rt.visible_count_total )
                       ; "autonomous_turn_count", `Int m.runtime.autonomous_turn_count
                       ; ( "autonomous_text_turn_count"
                         , `Int m.runtime.autonomous_text_turn_count )
                       ; ( "autonomous_tool_turn_count"
                         , `Int m.runtime.autonomous_tool_turn_count )
                       ; ( "board_reactive_turn_count"
                         , `Int m.runtime.board_reactive_turn_count )
                       ; ( "mention_reactive_turn_count"
                         , `Int m.runtime.mention_reactive_turn_count )
                       ...
                       ; ( "last_visible_proactive_ts"
                         , `Float m.runtime.proactive_rt.last_visible_ts )
                     ...
                     @ [ ( "skill_primary"
                         ...
                         , match memory_bank_json with
                           | `Assoc fields ->
                             (match List.assoc_opt "total_notes" fields with
                              | Some n -> n
                              | None ->
                                (match List.assoc_opt "total_files" fields with
                                 | Some n -> n
                                 | None -> `Int 0))
                           | _ -> `Int 0 )
                       ; ( "memory_top_kind"
                       ...
                       ; ( "recent_output_preview"
                         , match recent_preview_for_role "assistant" with
                           ...
                           | None -> `Null )
                       ; ( "recent_tool_names"
                         , `List (List.map (fun item -> `String item) recent_tool_names) )
                       ...
                       ; ( "conversation_fragment_filtered_count"
                         , `Int conversation_fragment_filtered_count )
                       ; ( "conversation_fragment_filter_enabled"
                         , `Bool history_fragment_filter_enabled )
                       ; "k2k_count", `Int k2k_count
                       ; "k2k_mentions", k2k_mentions
                       ; ( "last_handoff_event"
                         , match last_handoff_event with
                           | Some j -> j
                           | None -> `Null )
                       ; ( "last_compaction_event"
                         , match last_compaction_event with
                           | Some j -> j
                           | None -> `Null )
                       ; "trust", trust_json
                       ; "context", context
                           ...
                           | s :: _ ->
                             `Assoc
                               ...
                               ; ( "passed_count"
                                 , `Int
                                     (List_util.count_if
                                        (fun (lr : Dashboard_eval_feed.layer_result_json) ->
                                           lr.passed)
                               ...
                               ; ( "baseline_status"
                                 , Json_util.string_opt_to_json s.baseline_status )
                               ]
                           | [] -> `Null )
                       ]
                     @ detail_fields)
       ...
       | Some (`List rows) ->
         ...
         |> List.map (fun row ->
           `Assoc
             ...
             ; "pipeline_stage", Yojson.Safe.Util.member "pipeline_stage" row
...
let keeper_config_json (config : Coord.config) (name : string)
  ...
  | Ok None ->
    `Not_found, `Assoc [ "error", `String (Printf.sprintf "keeper %S not found" name) ]
  | Ok (Some (m : Keeper_types.keeper_meta)) ->
    ...
    let last_model_used_label =
      if String.trim m.runtime.usage.last_model_used = ""
      then None
      ...
      `List
        (List.map
           (fun (id, title, horizon) ->
              `Assoc
                [ "id", `String id; "title", `String title; "horizon", `String horizon ])
           active_goals)
    in
    let resolved_active_goal_ids = List.map (fun (id, _, _) -> id) active_goals in
    let missing_active_goal_ids =
      m.active_goal_ids
      |> List.filter (fun goal_id -> not (List.mem goal_id resolved_active_goal_ids))
    in
    let coordination =
          ...
          (fields
           @ [ "active_goal_ids", active_goal_ids_json
             ...
             ; ( "missing_active_goal_ids"
               , `List (List.map (fun goal_id -> `String goal_id) missing_active_goal_ids)
               )
             ])
      | other -> other
    in
    let runtime_trust = Keeper_runtime_trust_snapshot.snapshot_json ~config ~meta:m in
    let effective_system_prompt =
      Keeper_prompt.build_keeper_system_prompt
        ~goal:m.goal
        ~short_goal:m.short_goal
        ~mid_goal:m.mid_goal
        ~long_goal:m.long_goal
        ~will:m.will
        ~needs:m.needs
        ~desires:m.desires
    ...
    let execution =
      `Assoc
        ...
        ; "per_provider_timeout_sec", Json_util.float_opt_to_json m.per_provider_timeout_s
    ...
    let metrics =
      `Assoc
        ...
        ; ( "last_total_tokens_per_sec"
          , tokens_per_sec_json
              ~tokens:m.runtime.usage.last_total_tokens
              ~latency_ms:m.runtime.usage.last_latency_ms )
        ; ( "last_output_tokens_per_sec"
          , tokens_per_sec_json
              ~tokens:m.runtime.usage.last_output_tokens
              ~latency_ms:m.runtime.usage.last_latency_ms )
        ; "compaction_count", `Int m.runtime.compaction_rt.count
        ]
    in
    let current_phase = Keeper_registry.get_phase ~base_path:config.base_path m.name in
    let pipeline_stage =
      ...
      let tool_policy_mode : [ `Preset of string | `Custom ] option =
        ...
        | None ->
          (match Keeper_types.tool_access_preset m.tool_access with
           | Some preset -> Some (`Preset (Keeper_types.tool_preset_to_string preset))
           | None -> None)
      in
      let turn_outcome : [ `Ok | `Failed ] option =
        match Keeper_registry.get ~base_path:config.base_path m.name with
        | Some entry when entry.turn_consecutive_failures > 0 -> Some `Failed
        | Some _ -> Some `Ok
        | None -> None
      in
    ...
    let tools_access =
      ...
      let masc_tool_count = List.length (Keeper_exec_tools.keeper_masc_tool_names m) in
      ...
      `Assoc
        ...
        ; ( "tool_custom_allowlist"
          , `List
              (List.map (fun s -> `String s) (Option.value ~default:[] custom_allowlist))
          )
        ...
        ; "active_keeper_tool_count", `Int (List.length allowed - masc_tool_count)
    ...
    let sandbox_preflight =
      ...
      | _ -> None
    in
    let private_workspace_root = Keeper_sandbox.host_root_abs_of_meta ~config m in
    let sandbox_environment =
      ...
      `Assoc
        ...
        ; "project_root", `String (Keeper_alerting_path.project_root_of_config config)
        ; "docker_playground_enabled", `Bool Env_config_keeper.DockerPlayground.enabled
        ; ( "docker_container_name"
          , string_or_null Env_config_keeper.DockerPlayground.container_name )
        ; ( "container_playground_root"
          , string_or_null Env_config_keeper.DockerPlayground.container_playground_root )
        ; "hard_mode", `Bool (Env_config_keeper.KeeperSandbox.hard_mode ())
        ; ( "git_egress"
          , `String
        ...
        ; "docker_image", string_or_null (Env_config_keeper.KeeperSandbox.docker_image ())
        ; "pids_limit", `Int (Env_config_keeper.KeeperSandbox.pids_limit ())
        ; "memory", string_or_null (Env_config_keeper.KeeperSandbox.memory ())
        ; "tmpfs_size", string_or_null (Env_config_keeper.KeeperSandbox.tmpfs_size ())
        ; "relax_fs", `Bool (Env_config_keeper.KeeperSandbox.relax_fs ())
        ; ( "seccomp_profile"
          , string_or_null (Env_config_keeper.KeeperSandbox.seccomp_profile ()) )
        ; "require_rootless", `Bool (Env_config_keeper.KeeperSandbox.require_rootless ())
        ; "require_userns", `Bool (Env_config_keeper.KeeperSandbox.require_userns ())
        ; "preflight", Json_util.option_to_yojson Fun.id sandbox_preflight
        ]
    in
    ( `OK
    , `Assoc
        ...
        ; "sandbox_preflight", Json_util.option_to_yojson Fun.id sandbox_preflight
        ; "effective_sandbox_image", Json_util.string_opt_to_json effective_sandbox_image
