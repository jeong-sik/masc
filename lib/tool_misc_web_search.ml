...
let ddg_snippet_re =
  Re.Pcre.re {|<a class="result__snippet"[^>]*>([^<]*(?:<b>[^<]*</b>[^<]*)*)</a>|}
  |> Re.compile
;;
...
let html_entity_replacements =
  ...
  |> List.map (fun (entity, replacement) -> Re.str entity |> Re.compile, replacement)
;;
...
let normalize_spaces text = text |> Re.replace_string whitespace_re ~by:" " |> String.trim
let strip_html_tags text = Re.replace_string html_tag_re ~by:"" text

let strip_cdata text =
  text |> Re.replace_string cdata_start_re ~by:"" |> Re.replace_string cdata_end_re ~by:""
;;
...
let decode_html_entities text =
  ...
  let decode_numeric entity =
    ...
    match maybe_n with
    | Some n ->
      Uchar.of_int n |> Buffer.add_utf_8_uchar buf;
      Some ""
    | None -> None
  in
  let rec loop index =
    ...
    then (
      ...
      | Some semi ->
        ...
        if String.length entity >= 4 && String.starts_with ~prefix:"&#" entity
        then (
...
let parse_bing_rss_items payload =
  ...
  |> List.filter_map (fun groups ->
    ...
    with
    | Some title, Some url, Some snippet when title <> "" && valid_search_result_url url
      -> Some (title, url, snippet)
    | Some title, Some url, None when title <> "" && valid_search_result_url url ->
      Some (title, url, "")
    | _ -> None)
;;
...
let parse_ddg_html payload =
  ...
  |> List.filter (fun (title, url, _snippet) ->
    title <> "" && valid_search_result_url url)
;;
...
let parse_json_search_results ~results_path ~title_field ~snippet_field payload =
  ...
  Safe_ops.protect ~default:[] (fun () ->
    ...
    let items = Safe_ops.protect ~default:[] (fun () -> results_path root |> to_list) in
    items
...
let provider_order () =
  ...
  | Some raw ->
    ...
    if configured = [] then default_provider_order () else configured
  | None ->
    let primary =
...
let provider_plan () = provider_order () |> List.map provider_to_string
...
let contains_secret_token_prefix ~prefix ~min_suffix_len query =
  ...
  let rec count_suffix_chars idx count =
    if idx + count >= query_len
    then count
    else if is_secret_token_char lowered.[idx + count]
    then count_suffix_chars idx (count + 1)
    else count
  in
  let rec loop idx =
    if idx + prefix_len > query_len
    then false
    else if String.sub lowered idx prefix_len = prefix
    then (
...
let query_contains_secret_like_content query =
  let lowered = String.lowercase_ascii query in
  let markers =
    [ "-----begin "; "authorization:"; "bearer "; "x-api-key:"; "api_key="; "token=" ]
  in
  let secret_prefixes = [ "ghp_", 8; "github_pat_", 8; "sk-", 10 ] in
...
let validate_query query =
  ...
  if normalized = ""
  then Error "query is required"
  else if String.length normalized > max_web_search_query_length
  then
    Error
      (Printf.sprintf "query must be at most %d characters" max_web_search_query_length)
  else if query_contains_secret_like_content normalized
  then Error "query looks like it may contain secrets; refine it before using web search"
  else Ok normalized
;;
...
let strip_trailing_slashes s =
  let rec find_last_non_slash i =
    if i < 0 then -1 else if s.[i] = '/' then find_last_non_slash (i - 1) else i
  in
...
let fetch_searxng ~timeout_sec ~query =
  ...
  | Error msg -> Error msg
  | Ok base ->
    let search_url = base ^ "/search?q=" ^ Uri.pct_encode query ^ "&format=json" in
    (match Tool_local_runtime_http.http_get_text_with_status ~timeout_sec search_url with
     | Error detail ->
...
let fetch_ddg_html ~timeout_sec ~query =
  let search_url = "https://html.duckduckgo.com/html/?q=" ^ Uri.pct_encode query in
  match Tool_local_runtime_http.http_get_text_with_status ~timeout_sec search_url with
  | Error detail -> Error (endpoint_error ~fallback:"search endpoint unavailable" detail)
  | Ok (Some 200, payload) -> Ok (search_url, payload)
  | Ok (Some status, _) ->
    Error (Printf.sprintf "search endpoint returned HTTP %d" status)
  | Ok (None, _) -> Error "search endpoint returned no HTTP status"
...
let fetch_bing_rss ~timeout_sec ~query =
  let search_url = "https://www.bing.com/search?format=rss&q=" ^ Uri.pct_encode query in
  match Tool_local_runtime_http.http_get_text_with_status ~timeout_sec search_url with
  | Error detail -> Error (endpoint_error ~fallback:"search endpoint unavailable" detail)
  | Ok (Some 200, payload) -> Ok (search_url, payload)
  | Ok (Some status, _) ->
  ...
  | Some api_key ->
    ...
    (match
       Tool_local_runtime_http.http_get_text_with_status_with_headers
         ...
         search_url
     with
     | Error detail -> Error (endpoint_error ~fallback:"provider request failed" detail)
     | Ok (Some 200, payload) ->
       Safe_ops.protect ~default:(Error "provider returned invalid JSON") (fun () ->
         let hits =
...
let fetch_exa ~timeout_sec ~query ~limit =
  ...
  | Some api_key ->
    ...
    let body_json =
      `Assoc
        [ "query", `String query; "numResults", `Int limit; "type", `String "keyword" ]
      |> Yojson.Safe.to_string
    in
    (match
       Tool_local_runtime_http.http_post_json_text_with_status_with_headers
...
let fetch_bing_api ~timeout_sec ~query ~limit =
  let api_key =
    ...
    | Some key -> Some key
    | None ->
      Sys.getenv_opt "AZURE_BING_SEARCH_API_KEY" |> Fun.flip Option.bind trim_nonempty
  in
  match api_key with
...
let cache_key ~query ~limit =
  String.concat "|" [ query; string_of_int limit; String.concat "," (provider_plan ()) ]
;;
...
let enforce_rate_limit now =
  ...
  Eio.Mutex.use_rw ~protect:true rate_limit_mutex (fun () ->
    while Queue.length request_times > 0 && now -. Queue.peek request_times > window do
      let (_ : float) = Queue.pop request_times in
      ()
    done;
    if Queue.length request_times >= max_calls
    then Error "web search rate limit exceeded; retry shortly"
    else (
...
let search_impl ~query ~limit =
  let rec loop errors = function
    | [] ->
      Error
        (if errors = []
         then "no web search providers configured"
         else "all web search providers failed: " ^ String.concat "; " (List.rev errors))
    | provider :: rest ->
...
let simulate_for_test ~query ~limit outcomes =
  let normalize source tuples = tuples |> take_results limit |> normalize_hits ~source in
  let rec loop errors = function
    | [] ->
      ( false
      , json_error
          (if errors = []
           then "all web search providers failed"
    ...
    | (provider_name, outcome) :: rest ->
      (match outcome with
       ...
       | `Hits _ | `Empty -> loop ((provider_name ^ ": no results") :: errors) rest
       | `Error message -> loop ((provider_name ^ ": " ^ message) :: errors) rest)
  in
  loop [] outcomes
;;
