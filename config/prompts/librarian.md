...
- claim을 쓰기 전에 **이 문장이 언제 거짓이 되는지** 스스로 답하세요.
  다음 달에, 다음 턴에, 이 작업이 끝나면 거짓이 된다면 장기 기억이 아닙니다.
  진행 상황, 지금 어디까지 왔는지, 방금 저장한 슬롯, 다음에 할 일은
  `working_contexts`에 속합니다. 여기 적으면 세상이 움직일 때마다 같은 문장을
  다시 써야 하고, 기억의 신원은 문장 자체이므로 그때마다 원래 기억은 사라집니다.
- 이 Keeper의 책임 밖에서 벌어진 일은 그 일을 하는 Keeper의 기억입니다.
  보드나 대화에서 읽었다는 것만으로 남기지 마세요.
- 오래 유용할 결정의 이유, 제약, 명시된 안정적 선호, 외부 사실, 검증된 방법,
  재사용할 교훈을 남깁니다. 지속성과 근거가 불확실하면 추가하지 않습니다.
- 코드·git 이력·PR 상태·task board를 복제하지 마세요. 변경 목록 대신 이후
  판단에 필요한 결정과 이유를 남깁니다.
