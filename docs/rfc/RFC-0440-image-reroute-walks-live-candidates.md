---
rfc: "0440"
title: 이미지 턴의 reroute 는 살아 있는 후보를 걷는다 — lane 안에서, 402·429 후보는 이번 걸음에서 뒤로
status: Draft
created: 2026-09-09
...
이미지가 실린 턴은 지금 "lane 안에서 이미지를 받는 첫 후보" 하나를 고르고, 그 후보가 죽어 있어도 다음 턴에 또 고른다. 후보는 lane 안의 이미지 후보로 두고, 402·429·quota 로 끝난 후보는 같은 걸음에서 뒤로 보내며, 살아 있는 후보가 없으면 이미지를 떨구지 않고 위임(`keeper_analyze_image` 경로)으로 내려간다.
...
- 후보 집합은 lane 이다. "키퍼 K 의 이미지 후보" 는 K 의 lane 에서 이미지를 받는 후보를 선언 순서대로 늘어놓은 목록이다. lane 밖 런타임은 턴을 받지 않는다. `media_failover` 는 이미지를 글로 읽어 주는 read fleet(`analyze_image`, fallback projector)이고, 턴의 후보가 아니다.
