...
type cascade_failure_class = Oas_compat.Http_client.cascade_failure_class =
...
let filter_healthy_internal ~sw ~net (providers : Llm_provider.Provider_config.t list) =
  ...
  let providers =
    ...
    if with_keys = []
    then providers (* keep all rather than empty *)
    else (
      let dropped = initial_count - List.length with_keys in
      if dropped > 0
      then
        Llm_provider.Diag.debug
          "cascade_health_filter"
          "dropped %d provider(s) missing API keys"
          dropped;
      with_keys)
  in
  let local_providers = List.filter is_local_provider providers in
  let cloud_providers = List.filter (fun cfg -> not (is_local_provider cfg)) providers in
  if local_providers = []
  then providers, []
  else (
    let endpoints =
    ...
    then providers, statuses
      ...
      then providers, statuses
        ...
        Llm_provider.Diag.info
          ...
          (List.length local_providers)
          (List.length cloud_providers);
        cloud_providers, [])))
;;

let filter_healthy ~sw ~net providers = fst (filter_healthy_internal ~sw ~net providers)
