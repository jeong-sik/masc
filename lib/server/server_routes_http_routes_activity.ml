...
let activity_events_http_json ~sw ~clock ~state request =
  Server_activity_http.events_http_json
    ~deps:(activity_http_deps ~sw ~clock)
    ~state
    request
;;
...
let json_upsert_string_field name value = function
  ...
  | _non_object ->
    Error
      (Printf.sprintf
         "json_upsert_string_field: expected JSON object, got non-object for field %S"
         name)
;;
         ...
         | _ ->
           `Assoc
             (("source", `String source)
              :: List.filter (fun (k, _) -> k <> "source") meta_fields))
      | _ -> `Assoc [ "source", `String source ]
    in
    let fields = ("meta", meta_json) :: List.filter (fun (k, _) -> k <> "meta") fields in
    Ok (`Assoc fields)
  | _non_object -> Error "json_ensure_meta_source: expected JSON object"
;;
...
let json_ensure_meta_string_field name value = function
  | `Assoc fields ->
    ...
    then Ok (`Assoc fields)
      ...
      let meta_json =
        ...
        | Some (`Assoc meta_fields as existing_meta) ->
          (match List.assoc_opt name meta_fields with
           ...
           | _ ->
             `Assoc
               ((name, `String value) :: List.filter (fun (k, _) -> k <> name) meta_fields))
      ...
      in
      Ok (`Assoc fields))
  | _non_object -> Error "json_ensure_meta_string_field: expected JSON object"
;;
    ...
    (Yojson.Safe.to_string
       (`Assoc
           ...
           ; ( "error"
             , `String
                 (Printf.sprintf
                    "runtime param %s is %s risk and no longer supports direct dashboard \
                     mutation"
                    param_key
                    risk) )
           ]))
...
let add_routes ~sw ~clock router =
  ...
  |> Http.Router.get "/api/v1/governance/params" (fun request reqd ->
    with_public_read
      (fun _state _req reqd ->
           ...
           | Some (m : Runtime_params.param_meta) ->
             `Assoc
               ([ "description", `String m.description
                ...
                @ (match m.min_value with
                   | Some v -> [ "min_value", v ]
                   | None -> [])
                @
                match m.max_value with
                | Some v -> [ "max_value", v ]
                | None -> [])
         in
         let items =
         ...
         let json = `Assoc [ "parameters", `List items; "surfaces", surfaces ] in
  ...
  |> Http.Router.get "/api/v1/governance/params/audit" (fun request reqd ->
    with_public_read
      (fun state req reqd ->
         ...
         let limit =
           int_query_param req "limit" ~default:50 |> clamp ~min_v:1 ~max_v:200
         in
         let entries = Runtime_params.recent_audit ~base_path limit in
         let json =
           `Assoc [ "entries", `List entries; "count", `Int (List.length entries) ]
         in
         Http.Response.json (Yojson.Safe.to_string json) reqd)
      request
      reqd)
  |> Http.Router.get "/api/v1/board" (fun request reqd ->
    with_public_read
      (fun _state req reqd ->
         ...
         in
         let offset =
           int_query_param req "offset" ~default:0 |> clamp ~min_v:0 ~max_v:5000
         in
         let base_fetch =
      ...
      (fun _state req reqd ->
         ...
         | None ->
           Http.Response.json
             ...
             reqd
         | Some post_id ->
           let format = query_param req "format" |> Option.value ~default:"nested" in
  ...
  |> Http.Router.post "/api/v1/tools/masc_board_vote" (fun request reqd ->
    with_tool_auth
      ...
      (fun _state _req reqd ->
         let agent_name =
           (let hdr k =
              Option.bind (Httpun.Headers.get request.Httpun.Request.headers k) (fun s ->
                if s = "" then None else Some s)
            in
            match hdr "x-gate-agent" with
            | Some _ as v -> v
            | None -> hdr "x-masc-agent")
         ...
         Http.Request.read_body_async reqd (fun body_str ->
           try
             let ( let* ) r f =
               ...
               | Error msg ->
                 respond_json_with_cors
                   ...
                   reqd
                   (Yojson.Safe.to_string
                      (`Assoc [ "ok", `Bool false; "message", `String msg ]))
             in
             let* args =
             ...
             respond_json_with_cors
               ...
               request
               reqd
               (Yojson.Safe.to_string (`Assoc [ "ok", `Bool ok; "message", `String msg ]))
           with
           ...
           | exn ->
             respond_json_with_cors
               ...
               (Yojson.Safe.to_string
                  (`Assoc
                      [ "ok", `Bool false; "message", `String (Printexc.to_string exn) ]))))
      request
      reqd)
      ...
      (fun _state _req reqd ->
         ...
         Http.Request.read_body_async reqd (fun body_str ->
           try
             ...
             let* args =
               if String.equal author (String.trim agent_name)
               then Ok args
    ...
    with_public_read
      (fun state _req reqd ->
         ...
         | Some agent_name ->
           ...
           let mentions =
             Mention_inbox.read_mentions
               state.Mcp_server.room_config
               ~target_agent:agent_name
               ~limit
           in
           let unread =
             Mention_inbox.unread_count
               state.Mcp_server.room_config
               ~target_agent:agent_name
           in
           let json =
             `Assoc
               ...
               ; ( "mentions"
                 , `List (List.map Mention_inbox.mention_record_to_json mentions) )
               ]
           in
           Http.Response.json (Yojson.Safe.to_string json) reqd)
      request
  ...
  |> Http.Router.prefix_get "/api/v1/reputation/" (fun request reqd ->
    with_public_read
      (fun state _req reqd ->
         ...
         | Some agent_name ->
           let rep =
             Agent_reputation.compute_reputation state.Mcp_server.room_config ~agent_name
           in
           Http.Response.json
  ...
  |> Http.Router.post "/api/v1/prompts" (fun request reqd ->
    with_tool_auth
      ~tool_name:"masc_prompt_override"
      (fun state _req reqd ->
         Http.Request.read_body_async reqd (fun body_str ->
           try
             let args = Yojson.Safe.from_string body_str in
             let key =
               Yojson.Safe.Util.(member "key" args |> to_string_option)
               |> Option.value ~default:""
             in
             let action =
               Yojson.Safe.Util.(member "action" args |> to_string_option)
             ...
             then
               respond_json_with_cors
                 ...
                 reqd
                 (Yojson.Safe.to_string
                    (`Assoc [ "ok", `Bool false; "error", `String "key is required" ]))
             else (
               let result =
                 match action with
                 | "clear" ->
                   ...
                   (try
                      Prompt_registry.persist_overrides
                        state.Mcp_server.room_config.base_path
                    with
                    | Eio.Cancel.Cancelled _ as e -> raise e
                    | exn ->
                      Log.Pages.warn
                        "prompt override persist (clear) failed: %s"
                        (Printexc.to_string exn));
                   Ok "override cleared"
                 | "set" | _ ->
                   let value =
                     Yojson.Safe.Util.(member "value" args |> to_string_option)
                     |> Option.value ~default:""
                   in
                   (match Prompt_registry.set_override key value with
                    | Ok () ->
                      (try
                       ...
                       | exn ->
                         Log.Pages.warn
                           "prompt override persist (set) failed: %s"
                           (Printexc.to_string exn));
                      Ok "override set"
                    | Error msg -> Error msg)
               in
      ...
      (fun state _req reqd ->
         Http.Request.read_body_async reqd (fun body_str ->
           try
             ...
             in
             let param_key =
               Yojson.Safe.Util.(member "param_key" args |> to_string_option)
               |> Option.value ~default:""
               |> String.trim
             in
             let value_json =
             ...
             then
               ...
               | None ->
                 respond_json_with_cors
                   ...
                   reqd
                   (Yojson.Safe.to_string
                      (`Assoc [ "ok", `Bool false; "error", `String "value is required" ]))
               | Some value ->
                 (match governance_param_risk param_key with
                  | Some "high" ->
                    respond_high_risk_param_blocked request reqd ~param_key ~risk:"high"
                  | _ ->
                    let old_value =
                      match
                        Runtime_params.registry ()
                        |> List.find_opt (fun (k, _, _, _, _) -> k = param_key)
                      with
