...
let test_register_agent () =
  ...
  match Workspace_eio.register_agent config ~name:"agent_llm_a" () with
  | Ok agent ->
...
let test_get_agent () =
  with_eio_env @@ fun config ->
  (* Register first *)
  let _ = Workspace_eio.register_agent config ~name:"provider_f" ~capabilities:["code"; "review"] () in

  (* Get agent *)
  match Workspace_eio.get_agent config ~name:"provider_f" with
  | Ok agent ->
...
let test_remove_agent () =
  with_eio_env @@ fun config ->
  (* Register *)
  let _ = Workspace_eio.register_agent config ~name:"agent_code" () in

  (* Remove *)
  (match Workspace_eio.remove_agent config ~name:"agent_code" with
   | Ok () -> ()
   | Error e -> Alcotest.failf "remove_agent failed: %s" e);

  (* Verify removed *)
  match Workspace_eio.get_agent config ~name:"agent_code" with
...
let test_acquire_lock () =
  with_eio_env @@ fun config ->
  match Workspace_eio.acquire_lock config ~resource:"file.txt" ~owner:"agent_llm_a" with
  | Ok (Some lock) ->
...
let test_lock_conflict () =
  ...
  let _ = Workspace_eio.acquire_lock config ~resource:"shared.txt" ~owner:"agent_llm_a" in

  (* Second agent tries to acquire same lock *)
  match Workspace_eio.acquire_lock config ~resource:"shared.txt" ~owner:"provider_f" with
...
let test_release_lock () =
  with_eio_env @@ fun config ->
  (* Acquire *)
  let _ = Workspace_eio.acquire_lock config ~resource:"temp.txt" ~owner:"agent_llm_a" in

  (* Release *)
  (match Workspace_eio.release_lock config ~resource:"temp.txt" ~owner:"agent_llm_a" with
   | Ok () -> ()
   | Error e -> Alcotest.failf "release_lock failed: %s" e);

  (* Now another agent can acquire *)
  match Workspace_eio.acquire_lock config ~resource:"temp.txt" ~owner:"provider_f" with
...
let test_broadcast_message () =
  with_eio_env @@ fun config ->
  match Workspace_eio.broadcast config ~from_agent:"agent_llm_a" ~content:"Hello world!" with
  | Ok msg ->
...
let test_mention_extraction () =
  with_eio_env @@ fun config ->
  match Workspace_eio.broadcast config ~from_agent:"agent_llm_a" ~content:"@provider_f please review" with
  | Ok msg ->
...
let test_get_message () =
  with_eio_env @@ fun config ->
  (* Broadcast first *)
  let msg_result = Workspace_eio.broadcast config ~from_agent:"agent_llm_a" ~content:"Test message" in
...
let test_workspace_state () =
  ...
  let _ = Workspace_eio.register_agent config ~name:"agent_llm_a" () in
  let _ = Workspace_eio.register_agent config ~name:"provider_f" () in
