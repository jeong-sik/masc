...
let is_local_only_cascade name =
  ...
  let rec loop i =
    if i > slen - plen
    then false
    else if String.sub lc i plen = pattern
    then true
    else loop (i + 1)
  in
  loop 0
  ...
  let all_labels =
    ...
    | Error _ ->
      (match Provider_adapter.preferred_execution_model_labels () with
       ...
       | labels -> labels)
  in
  if is_local_only_cascade cascade_name
  then (
...
let refresh_local_discovery_if_possible ?sw ?net (labels : string list) : bool =
  if not (labels_require_local_discovery labels)
  then false
  else (
    let sw =
      match sw with
    ...
    | Some sw, Some net ->
      ...
      (try
         ignore (Llm_provider.Provider_registry.refresh_llama_endpoints ~sw ~net ());
         let after = Llm_provider.Provider_registry.discovered_max_context () in
         (match before, after with
          | Some prev, Some next when prev <> next ->
            Log.info
              ~ctx:"CascadeRuntime"
              "refreshed local runtime context: %d -> %d"
              prev
              next
          | None, Some next ->
            Log.info
              ~ctx:"CascadeRuntime"
              "refreshed local runtime context: unset -> %d"
              next
          | Some _, Some _ -> ()
...
let clamp_context_for_pure_local_labels ~(labels : string list) ~(max_context : int) : int
  =
...
let default_local_model_label_and_id () : string * string =
  ...
  | Ok label ->
    (match try_label label with
     ...
     | None ->
       (match
          List.find_map try_label (Provider_adapter.preferred_execution_model_labels ())
        with
        | Some pair -> pair
        | None -> fallback))
  | Error _ ->
...
let ensure_api_keys_for_labels (labels : string list) : (unit, string) result =
  ...
  then Ok ()
    ...
    then Ok ()
      ...
      Error
        (Printf.sprintf
           "No valid/available model specs for labels: %s (missing: %s)"
           (String.concat ", " labels)
           (String.concat ", " missing))))
;;

let apply_required_tool_choice_filter
      ?runtime_mcp_policy
...
let models_of_cascade_name_result (cascade_name : string) : (string list, string) result =
...
let models_of_cascade_name (cascade_name : string) : string list =
  ...
  | Ok labels -> labels
  | Error detail ->
    let normalized = Keeper_cascade_profile.normalize_declared_name cascade_name in
    Log.warn
      ~ctx:"CascadeRuntime"
...
let resolve_providers_from_model_strings
  ...
  let filtered =
    Cascade_config.apply_provider_filter
      ...
      specs
    |> apply_required_tool_choice_filter
         ?runtime_mcp_policy
         ~require_tool_choice_support
         ~require_tool_support
         ~label:"direct_model_strings"
  ...
  then filtered
    ...
    Log.Misc.warn
      "direct model strings: no callable models from %d entries"
      (List.length model_strings);
    [])
;;

let resolve_named_providers_result
      ?provider_filter
      ?(require_tool_choice_support = false)
      ?(require_tool_support = false)
      ?runtime_mcp_policy
...
let resolve_named_providers
  ...
  =
  match
    resolve_named_providers_result
      ?provider_filter
      ?runtime_mcp_policy
      ~require_tool_choice_support
      ~require_tool_support
