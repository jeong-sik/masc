...
| RuntimeKeeperRecovery | (no explicit ref — header lists 4 domains) | no | **cross-domain** (per #12149) |
| RuntimeResolver | (no explicit ref) | likely | cross-domain |
| **RuntimeStrategy** | `lib/runtime/runtime_strategy.{ml,mli}` (per spec text) | yes — `kind` (8 variants) | **mappable** (per #12149, adoption in #12153) |
...
|---|---|---|
| **mappable** (confirmed) | **2** | RuntimeStrategy, AuditLog (multiple ADTs) |
| **mappable candidate** (needs ADT inspection) | **2** | AuditLogAppendOrder, SandboxDispatch |
| **cross-domain** (confirmed) | **3** | RuntimeKeeperRecovery, KeeperContractViolated, KeeperRecoveryOrchestration |
| cross-domain (heuristic — no module ref) | 11 | most Keeper* specs, RuntimeResolver, ToolCallContract |
...
This refines the spot-check's 1/3 estimate downward — boundary domain is mostly cross-domain interaction specs, not single-ADT mirrors. The spot-check's RuntimeStrategy hit was a positive selection bias (it was the most mappable example in the sample).

## 4. Implication for Cycle 14 §8 ratchet floors

- `ppx_deriving_tla_modules` floor: stay at 4 (current). Adding RuntimeStrategy adoption (PR #12153 sibling — proposed) lifts to 5. Adding AuditLog adoption could lift to 6+. But stop after these — the remaining 14 specs have no ADT to derive.
...
- `lib_subdirs_with_ppx` (descriptive, currently 3): adding runtime adoption → 4. Adding audit_log adoption → 5. Topping out around 5-6 unless `[@@fsm_guard]` extends to non-keeper subsystems.

## 5. ADT-confirmed candidates queue

Order by `[@@deriving tla]` adoption priority:

1. **RuntimeStrategy.kind** — 8 variants, perfect fit, spec exists, PR queued (Cycle 18 sibling)
...
PR #12149 (3-spec sample) reported ~⅓ mappable. Full sweep reports ~⅙–⅙ mappable (2 confirmed + 2 candidate of 18). The spot-check's RuntimeStrategy choice was selection bias — the spec was the most mappable example to start with.
...
- PR #12151 — ratchet
- `lib/audit_log.mli` — second adoption candidate after RuntimeStrategy
