...
let events_http_json ~deps ~state request =
  ...
  let after_seq = deps.int_query_param request "after_seq" ~default:0 |> max 0 in
  let limit =
    deps.int_query_param request "limit" ~default:200 |> clamp ~min_v:1 ~max_v:1000
  in
  Activity_graph.json_response state.Mcp_server.room_config ~kinds ~after_seq ~limit ()
;;
...
let graph_http_json ~deps ~state request =
  let kinds = kind_filters deps request in
  let limit =
    deps.int_query_param request "limit" ~default:500 |> clamp ~min_v:50 ~max_v:2000
  in
  let timeline_limit =
...
let swimlane_http_json ~deps ~state request =
  let limit =
    deps.int_query_param request "limit" ~default:500 |> clamp ~min_v:1 ~max_v:2000
  in
  let since_ms =
    match deps.query_param request "since" with
    | Some raw ->
    ...
    | None -> None
  in
  Activity_graph.agent_spans_json state.Mcp_server.room_config ~limit ?since_ms ()
;;
...
let send_raw info data =
  Eio.Mutex.use_rw ~protect:true info.mutex (fun () ->
    if !(info.stop) || info.closed || Httpun.Body.Writer.is_closed info.writer
    then false
    else (
      try
...
let close_stream info =
  Eio.Mutex.use_rw ~protect:true info.mutex (fun () ->
    ...
    then (
      info.closed <- true;
      info.stop := true;
      if not (Httpun.Body.Writer.is_closed info.writer)
      then Httpun.Body.Writer.close info.writer))
;;

let handle_stream ~deps ~state request reqd =
  let origin = deps.get_origin request in
  let session_id = Mcp_session.get_or_generate (deps.get_session_id_any request) in
  let kinds = kind_filters deps request in
  let replay_limit =
    deps.int_query_param request "limit" ~default:500 |> clamp ~min_v:1 ~max_v:1000
  in
  let after_seq =
    max (last_event_id request) (deps.int_query_param request "after_seq" ~default:0)
  in
  ...
  let push event =
    match !info_ref with
    | Some info ->
      if not (send_raw info event)
      then Activity_graph.unregister_if_current info.session_id info.client_id
    | None -> ()
  in
  let client_id =
    Activity_graph.register session_id ~push ~last_seq:after_seq ~kind_filters:kinds ()
  in
  let info =
    { session_id
    ; client_id
  ...
  in
  List.iter
    (fun value -> ignore (send_raw info (Activity_graph.format_sse_event value)))
    replay;
   ...
   | Some sw, Some clock ->
     Eio.Fiber.fork ~sw (fun () ->
       ...
       let rec loop () =
         if not !(info.stop)
         then (
           (try Eio.Time.sleep clock keepalive_interval_s with
            | Eio.Cancel.Cancelled _ as e -> raise e
            | exn -> if is_cancelled exn then raise exn);
           if not !(info.stop) then ignore (send_raw info ": keepalive\n\n");
           loop ())
       in
       try loop () with
       | Eio.Cancel.Cancelled _ as e -> raise e
