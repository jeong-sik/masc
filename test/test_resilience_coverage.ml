...
let test_is_stale_custom_threshold () =
  let old_ts = "2020-01-01T00:00:00Z" in
  check bool "stale with threshold" true (Resilience.Time.is_stale ~threshold:1.0 old_ts)
;;
...
let test_keeper_zombie_threshold_matches_env_config () =
  ...
  check
    bool
    "keeper threshold uses env config"
    expected
...
let () =
  run
    ...
    [ ( "constants"
      , [ test_case
            "zombie threshold positive"
            `Quick
            test_default_zombie_threshold_positive
        ; test_case
            "zombie threshold matches env config"
            `Quick
            test_default_zombie_threshold_matches_env_config
        ; test_case
            "zombie threshold reasonable"
            `Quick
            test_default_zombie_threshold_reasonable
        ; test_case
            "warning threshold positive"
            `Quick
            test_default_warning_threshold_positive
    ...
    ; ( "zombie"
      , [ test_case "old agent" `Quick test_zombie_old_agent
        ...
        ; test_case
            "keeper threshold matches env config"
            `Quick
            test_keeper_zombie_threshold_matches_env_config
        ] )
    ; ( "global_stats"
