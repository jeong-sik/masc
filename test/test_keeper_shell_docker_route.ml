...
let with_tool_policy_config f =
  let config_dir = Filename.concat (Masc_test_deps.find_project_root ()) "config" in
  Tool_code_write.reset_policy_config_cache ();
  with_env "MASC_CONFIG_DIR" config_dir
  @@ fun () -> Fun.protect ~finally:Tool_code_write.reset_policy_config_cache f
;;
...
let with_keeper_identity_toml ~config ~keeper_name ~github_identity ~git_identity_mode f =
  ...
  let gh_dir =
    Filename.concat
      (Filename.concat (Filename.concat masc_dir "github-identities") github_identity)
      "gh"
  in
...
let parse_field raw field = Yojson.Safe.from_string raw |> Json.member field
let parse_string_field raw field = parse_field raw field |> Json.to_string_option
...
let response_mentions raw field needle =
  ...
  | Some s ->
    ...
    let rec loop i =
      if i + len > n
      then false
      else if String.sub s i len = needle
      then true
      else loop (i + 1)
    in
    loop 0
...
let parse_bool_field raw field = parse_field raw field |> Json.to_bool_option
...
let assert_docker_route_fires ~config ~meta ~playground =
  ...
  let cases =
    ...
    ; ( "rg"
      , `Assoc
          [ "op", `String "rg"; "pattern", `String "alpha"; "path", `String playground ] )
    ; ( "find"
      , `Assoc
          [ "op", `String "find"; "pattern", `String "*.txt"; "path", `String playground ]
      )
    ; "head", `Assoc [ "op", `String "head"; "lines", `Int 1; "path", `String host_path ]
    ; "tail", `Assoc [ "op", `String "tail"; "lines", `Int 1; "path", `String host_path ]
    ...
    ; "git_status", `Assoc [ "op", `String "git_status"; "cwd", `String playground ]
    ; ( "git_log"
      , `Assoc [ "op", `String "git_log"; "cwd", `String playground; "count", `Int 1 ] )
    ]
  in
  List.iter
    (fun (op, args) ->
...
let test_readonly_ops_route_through_docker () =
  ...
  @@ fun ~config ~meta ~playground -> assert_docker_route_fires ~config ~meta ~playground
;;
...
let test_rg_no_match_remains_successful_in_docker_route () =
  ...
  @@ fun () ->
  with_fake_docker fake_docker_rg_no_match_script
  @@ fun () ->
  setup ~sandbox:Keeper_types.Docker
  @@ fun ~config ~meta ~playground ->
  let host_path = Filename.concat playground "mind/demo.txt" in
  ...
  Alcotest.(check (option bool))
    "rg no-match stays ok"
    (Some true)
    (parse_bool_field raw "ok");
  Alcotest.(check int) "rg keeps exit=1 status" 1 (parse_status_exit_code raw);
  Alcotest.(check int)
    "rg no-match returns empty matches"
    0
...
let test_git_clone_routes_through_docker () =
  ...
  in
  Alcotest.(check (option bool))
    "git_clone fails through docker route"
    (Some false)
    (parse_bool_field raw "ok");
  ...
  Alcotest.(check bool)
    "output includes docker image error"
    true
    (response_mentions raw "output" "docker image");
  Alcotest.(check bool)
    "clone stays inside keeper repos"
    true
    (response_mentions raw "path" (Filename.concat playground "repos/masc-mcp"))
;;
...
let test_hard_mode_git_clone_uses_brokered_route () =
  ...
  in
  Alcotest.(check (option bool))
    "git_clone fails before host git without identity"
    (Some false)
    (parse_bool_field raw "ok");
  Alcotest.(check (option string))
    "via=brokered"
    ...
    (parse_string_field raw "via");
  Alcotest.(check bool)
    "output mentions missing github_identity"
    true
    (response_mentions raw "output" "github_identity");
  Alcotest.(check bool)
    "clone path stays inside keeper repos"
    true
    (response_mentions raw "path" (Filename.concat playground "repos/masc-mcp"))
;;
...
let test_bash_legacy_skips_docker () =
  ...
  Fun.protect ~finally:(fun () -> cleanup_dir outside_cwd)
  @@ fun () ->
  let raw =
    Keeper_exec_shell.handle_keeper_bash
      ~turn_sandbox_runtime:None
...
let run_git_creds_docker_shell ~config ~meta ~playground ~log_path =
  ...
  @@ fun () ->
  with_env "MASC_KEEPER_SANDBOX_CLEANUP_ENABLED" "false"
  @@ fun () ->
  (match Sys.getenv_opt "MASC_TEST_FAKE_DOCKER_PATH" with
   | Some expected ->
     Alcotest.(check string)
       "fake docker command selected"
       expected
  ...
  with
  | Error msg -> Alcotest.failf "expected fake docker git-creds run, got %s" msg
  | Ok result ->
    let observed =
    ...
    then
      Alcotest.failf
        "expected fake docker exit 0, got %s %d; log=%S; output=%S"
        (fst observed)
        (snd observed)
        (read_file log_path)
...
let test_git_creds_skips_missing_ssh_auth_sock () =
  with_fake_docker fake_docker_echo_script
  @@ fun () ->
  setup ~sandbox:Keeper_types.Docker
  @@ fun ~config ~meta ~playground ->
  Masc_mcp.Config_dir_resolver.reset ();
  let log_path = Filename.concat config.Coord.base_path "docker.log" in
  let missing_sock = Filename.concat config.Coord.base_path "missing-agent.sock" in
  with_env "SSH_AUTH_SOCK" missing_sock
  @@ fun () ->
  let line = run_git_creds_docker_shell ~config ~meta ~playground ~log_path in
  Alcotest.(check bool)
    "missing ssh-agent socket is not mounted"
    false
    (contains_substring line missing_sock);
  Alcotest.(check bool)
    "missing ssh-agent env is not forwarded"
    false
...
let test_git_creds_inherit_network_omits_invalid_network_flag () =
  ...
  let line = run_git_creds_docker_shell ~config ~meta ~playground ~log_path in
  Alcotest.(check bool)
    "network inherit never uses invalid flag value"
    false
...
let test_git_creds_respects_keeper_alias_identity_mode () =
  ...
  let line = run_git_creds_docker_shell ~config ~meta ~playground ~log_path in
  Alcotest.(check bool)
    "keeper_alias keeps keeper author"
    true
    (contains_substring line "GIT_AUTHOR_NAME=minjae (MASC Keeper)");
  Alcotest.(check bool)
    "keeper_alias does not force GitHub identity author"
    false
...
let test_git_creds_uses_github_identity_mode () =
  ...
  let line = run_git_creds_docker_shell ~config ~meta ~playground ~log_path in
  Alcotest.(check bool)
    "github_identity mode uses GitHub author"
    true
    (contains_substring line "GIT_AUTHOR_NAME=anyang-keepers");
  Alcotest.(check bool)
    "github_identity mode uses noreply email"
    true
    (contains_substring line "GIT_AUTHOR_EMAIL=anyang-keepers@users.noreply.github.com")
;;
...
let test_git_clone_repairs_existing_docker_clone_checkout () =
  ...
  Alcotest.(check bool)
    "checkout file removed before repair"
    false
  ...
  in
  Alcotest.(check (option bool))
    "git_clone succeeds after checkout repair"
    (Some true)
    (parse_bool_field raw "ok");
  Alcotest.(check (option string))
    "via=docker"
    (Some "docker")
    (parse_string_field raw "via");
  Alcotest.(check bool) "checkout restored" true (Sys.file_exists restored_readme);
  Alcotest.(check bool)
    "repair note surfaced"
    true
...
let test_bash_fake_docker_executes () =
  ...
  Alcotest.(check (option string))
    ...
    (parse_string_field raw "via");
  Alcotest.(check bool)
    "bash output includes fake docker stdout"
    true
...
let () =
  Alcotest.run
    "Keeper_shell_docker_route"
    [ ( "docker_route_fires"
      , [ Alcotest.test_case
            "docker keeper shell ops route through docker"
            `Quick
            test_readonly_ops_route_through_docker
        ; Alcotest.test_case
            "docker keeper bash routes through docker"
            `Quick
            test_bash_routes_through_docker
        ; Alcotest.test_case
            "docker keeper bash git cmd routes through git-creds docker"
            `Quick
            test_bash_git_creds_routes_through_docker
        ; Alcotest.test_case
            "hard mode blocks raw gh keeper_bash"
            `Quick
            test_hard_mode_blocks_raw_gh_bash
        ; Alcotest.test_case
            "docker keeper bash executes through fake docker"
            `Quick
            test_bash_fake_docker_executes
        ] )
    ; ( "docker_route_skipped"
      , [ Alcotest.test_case
            "legacy keeper skips docker route"
            `Quick
            test_cat_legacy_keeper_skips_docker
        ; Alcotest.test_case
            "legacy keeper bash skips docker route"
            `Quick
            test_bash_legacy_skips_docker
        ] )
    ; ( "docker_route_contract"
      , [ Alcotest.test_case
            "rg no-match remains successful"
            `Quick
            test_rg_no_match_remains_successful_in_docker_route
        ; Alcotest.test_case
            "git_clone routes through docker"
            `Quick
            test_git_clone_routes_through_docker
        ; Alcotest.test_case
            "git-creds skips missing SSH_AUTH_SOCK"
            `Quick
            test_git_creds_skips_missing_ssh_auth_sock
        ; Alcotest.test_case
            "git-creds inherit network omits invalid docker flag"
            `Quick
            test_git_creds_inherit_network_omits_invalid_network_flag
        ; Alcotest.test_case
            "git-creds respects keeper_alias git identity mode"
            `Quick
            test_git_creds_respects_keeper_alias_identity_mode
        ; Alcotest.test_case
            "git-creds uses GitHub author only in github_identity mode"
            `Quick
            test_git_creds_uses_github_identity_mode
        ; Alcotest.test_case
            "hard mode git_clone uses brokered route"
            `Quick
            test_hard_mode_git_clone_uses_brokered_route
        ; Alcotest.test_case
            "git_clone repairs existing docker clone checkout"
            `Quick
            test_git_clone_repairs_existing_docker_clone_checkout
        ] )
    ]
;;
