...
export function ContextChart({ keeper }: { keeper: Keeper }) {
  ...
  return html`
    <div class="flex items-center gap-3 mb-5 p-3 rounded border border-[var(--card-border)] bg-[var(--white-3)]">
      <svg role="img" aria-label="컨텍스트 사용률 차트" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" class="rounded" style="background:var(--bg-deepest);">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div class="md:col-span-2 p-3 rounded border border-[var(--card-border)] bg-[var(--white-3)]">
          <div class="flex items-center gap-4 mb-1.5">
            ...
            <span class="flex items-center gap-1 text-3xs text-[var(--text-muted)]">
              <span class="inline-block w-2.5 h-0.5 rounded bg-[var(--ok)]" aria-hidden="true"></span> output
              <span class="font-mono text-[var(--good)]">${formatTokens(lastOutput)}</span>
            </span>
          </div>
          <svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="입출력 토큰 추이" class="rounded w-full" style="background:var(--bg-deepest);">
    ...
    <div class="mb-5">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
        <div class="md:col-span-2 p-3 rounded border border-[var(--card-border)] bg-[var(--white-3)]">
          ...
          <svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="프롬프트 토큰 추이" class="rounded w-full" style="background:var(--bg-deepest);">
    ...
    <div class="mb-5">
      ...
      <div class="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
        <div class="md:col-span-2 p-3 rounded border border-[var(--card-border)] bg-[var(--white-3)]">
          ...
          <svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="컨텍스트 구성 스택 히스토리" class="rounded w-full" style="background:var(--bg-deepest);">
    ...
    <div class="mb-5">
      ...
      <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <div class="p-3 rounded border border-[var(--card-border)] bg-[var(--white-3)]">
          ...
          <svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="Wall tok/s 추이" class="rounded w-full" style="background:var(--bg-deepest);">
        ...
        <div class="p-3 rounded border border-[var(--card-border)] bg-[var(--white-3)]">
          ...
          <svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="HW tok/s 추이" class="rounded w-full" style="background:var(--bg-deepest);">
        ...
        <div class="p-3 rounded border border-[var(--card-border)] bg-[var(--white-3)]">
          ...
          <svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="API 지연 시간 추이" class="rounded w-full" style="background:var(--bg-deepest);">
  ...
  return html`
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-5">
      ...
      <div class="p-3 rounded border border-[var(--card-border)] bg-[var(--white-3)]">
        ...
        <svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="지연 시간 및 폴백 추이" class="rounded w-full" style="background:var(--bg-deepest);">
      ...
      <div class="p-3 rounded border border-[var(--card-border)] bg-[var(--white-3)]">
        ...
        <svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="비용 추이" class="rounded w-full" style="background:var(--bg-deepest);">
...
export function RawDataDebug({ keeper }: { keeper: Keeper }) {
  ...
  return html`
    <div class="max-h-[460px] overflow-y-auto custom-scrollbar" role="region" aria-label="키퍼 디버그 데이터">
      <${TextInput}
        placeholder="필드 검색..."
        ariaLabel="키퍼 필드 검색"
      ...
      <div class="flex flex-col">
        ${filtered.map((f, i) => html`
          <div class="grid grid-cols-[100px_80px_1fr] gap-2 py-2 px-2 text-xs rounded ${i % 2 === 0 ? 'bg-[var(--white-2)]' : ''}">
            <span class="font-semibold text-[var(--text-body)] truncate" title=${f.title}>${f.title}</span>
            <span class="font-mono text-[var(--cyan)] text-2xs truncate" title=${f.key}>${f.key}</span>
            <span class="text-right text-[var(--text-body)] truncate" title=${f.value}>${f.value}</span>
          </div>
        `)}
        ${extras.map((f, i) => html`
          <div class="grid grid-cols-[100px_1fr] gap-2 py-2 px-2 text-xs rounded ${(filtered.length + i) % 2 === 0 ? 'bg-[var(--white-2)]' : ''}">
            <span class="font-semibold text-[var(--text-body)] truncate" title=${f.title}>${f.title}</span>
            <span class="text-right text-[var(--text-body)] truncate ${f.mono ? 'font-mono' : ''}" title=${f.value}>${f.value}</span>
          </div>
        `)}
      </div>
    </div>
