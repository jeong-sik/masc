...
let input_capacity constraint_ reason =
  Masc_agent_core.Error.Api
    (InputCapacity
       ...
       ; constraint_
       ; reason = Masc_agent_core.Retry.Serving_constraint_rejected reason
       })
...
let test_is_context_overflow_only_for_overflow_errors () =
  check
    ...
    (EC.is_context_overflow
       (Masc_agent_core.Error.Api (ContextOverflow { message = "exceeded"; limit = Some 32768 })));
  check
    bool
    "ContextOverflow without limit"
    true
    (EC.is_context_overflow
       (Masc_agent_core.Error.Api (ContextOverflow { message = "exceeded"; limit = None })));
  check
    bool
...
let request_body_too_large ~actual_bytes ~limit_bytes =
  Masc_agent_core.Error.Api
    (InvalidRequest
       ...
       ; reason = Masc_agent_core.Retry.Request_body_too_large { actual_bytes; limit_bytes }
       })
;;
...
let request_body_refused_by_provider ~status =
  Masc_agent_core.Error.Api
    (InvalidRequest
       ...
       ; reason = Masc_agent_core.Retry.Request_body_refused_by_provider { status }
       })
;;
