...
let check_has_tool steps tool_name =
  let found = List.exists (fun (s : WG.step) -> s.tool = tool_name) steps in
  check bool (Printf.sprintf "next_steps contains %s" tool_name) true found
;;

let check_not_empty msg steps = check bool msg true (List.length steps > 0)
...
let check_has_precondition g expected =
  check
    ...
    true
    (List.mem expected g.WG.preconditions)
;;
...
let check_lacks_precondition g unexpected =
  check
    ...
    false
    (List.mem unexpected g.WG.preconditions)
;;
...
let test_operation_start_removed () =
  ...
  check
    (list string)
    "operation_start removed returns empty"
    []
...
let test_dispatch_tick_removed () =
  ...
  check
    (list string)
    "dispatch_tick removed returns empty"
    []
    (List.map (fun (s : WG.step) -> s.tool) g.next_steps)
;;
...
let test_unknown_tool () =
  ...
  check
    (list string)
    "unknown tool returns empty next_steps"
    []
    (List.map (fun (s : WG.step) -> s.tool) g.next_steps)
;;
...
let test_workflow_context_join () =
  ...
  | Some (before, after, _mistakes) ->
    check bool "join before includes start" true (List.mem "masc_start" before);
...
let test_state_not_room_set () =
  let g =
    WG.current_state_guidance
      ~room_set:false
      ~joined:false
      ~task_claimed:false
      ~current_task_set:false
...
let test_set_current_task_alias_matches_canonical () =
  ...
  check
    (list string)
    "alias guidance matches canonical guidance"
    canonical_steps
    alias_steps
;;
...
let test_transition_claim_call_guidance () =
  ...
  check
    bool
    "claim guidance omits plan_set_task"
    false
...
let test_transition_done_call_guidance () =
  ...
  check
    bool
    "done guidance omits plan_set_task"
    false
    (List.exists (fun (s : WG.step) -> s.tool = "masc_plan_set_task") g.next_steps)
;;
...
let test_transition_release_call_guidance () =
  ...
  check
    bool
    "release guidance omits plan_set_task"
    false
    (List.exists (fun (s : WG.step) -> s.tool = "masc_plan_set_task") g.next_steps)
;;
...
let check_tool_exists_in_schemas name =
  if not (List.mem name all_schema_names)
  then
    Alcotest.fail
      (Printf.sprintf
         "Workflow_guide references tool '%s' not in Config.all_tool_schemas"
         name)
;;
...
let test_next_steps_reference_real_tools () =
  ...
  List.iter
    (fun tool_name ->
       ...
       List.iter
         (fun (s : WG.step) -> check_tool_exists_in_schemas s.tool)
         g_ok.next_steps;
  ...
  run
    ...
    [ ( "golden_path_1"
      , [ test_case "start success" `Quick test_start_success
        ...
        ; test_case
            "set_current_task alias matches canonical"
            `Quick
    ...
    ; ( "edge_cases"
      , [ test_case "unknown tool" `Quick test_unknown_tool
        ...
        ; test_case
            "transition claim call guidance"
            `Quick
            test_transition_claim_call_guidance
        ; test_case
            "add_task uses project_ready"
            `Quick
            test_add_task_success_uses_project_ready
        ; test_case
            "broadcast uses project_ready"
            `Quick
            test_broadcast_success_uses_project_ready
        ; test_case
            "transition done call guidance"
            `Quick
            test_transition_done_call_guidance
        ; test_case
            "transition release call guidance"
            `Quick
            test_transition_release_call_guidance
        ] )
    ; ( "structural"
      , [ test_case
            "next_steps reference real tools"
            `Quick
            test_next_steps_reference_real_tools
        ] )
    ; ( "json"
