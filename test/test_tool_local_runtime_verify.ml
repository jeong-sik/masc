...
let test_provider_health_reachable_accepts_json_health () =
  check
    bool
    "json health body counts as reachable"
    true
    (Masc_mcp.Tool_local_runtime.provider_health_reachable
       ~status:(Some 200)
       ~body:(Some {|{"status":"ok"}|}));
  check
    bool
    "plain text health body counts as reachable"
    true
    (Masc_mcp.Tool_local_runtime.provider_health_reachable
       ~status:(Some 200)
       ~body:(Some "ok"));
...
let test_classify_runtime_blocker_prefers_slot_count_when_health_ok () =
  let blocker, detail =
    Masc_mcp.Tool_local_runtime.classify_runtime_blocker
      ~provider_reachable:true
      ~slot_reachable:true
      ~chat_contract_status:"confirmed"
      ~expected_model:(Some "qwen3.5-35b-a3b-ud-q8-xl")
      ~actual_model_id:(Some "qwen3.5-35b-a3b-ud-q8-xl")
      ~expected_slots:(Some 12)
      ~actual_slots_total:4
      ~expected_ctx:(Some 262144)
      ~actual_ctx:(Some 262144)
      ~chat_completion_compatible:true
  in
  check (option string) "slot blocker" (Some "slot_count_insufficient") blocker;
  check
    bool
    "detail mentions expected slots"
    true
    (match detail with
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       := [ make_endpoint
              ...
              ~model_id:"qwen3.5-35b-a3b-ud-q4-xl"
              ~ctx_size:262144
              ~total_slots:4
              ~busy:0
          ; make_endpoint
       ...
       check string "source" "oas_discovery" (result |> member "source" |> to_string);
       check
         bool
         "provider reachable"
         true
         (result |> member "provider_reachable" |> to_bool);
       check bool "slot reachable" true (result |> member "slot_reachable" |> to_bool);
       check int "actual slots" 12 (result |> member "actual_slots" |> to_int);
       check int "active slots now" 2 (result |> member "active_slots_now" |> to_int);
       check bool "passes expected contract" true (result |> member "pass" |> to_bool))
;;
...
let test_classify_runtime_blocker_flags_chat_contract_mismatch () =
  let blocker, detail =
    Masc_mcp.Tool_local_runtime.classify_runtime_blocker
      ~provider_reachable:true
      ~slot_reachable:true
      ~chat_contract_status:"rejected"
      ~expected_model:(Some "Qwen3.5-9B-Q4_K_M.gguf")
      ~actual_model_id:(Some "Qwen3.5-9B-Q4_K_M.gguf")
      ~expected_slots:(Some 4)
      ~actual_slots_total:4
      ~expected_ctx:(Some 131072)
      ~actual_ctx:(Some 131072)
      ~chat_completion_compatible:true
  in
  check (option string) "chat blocker" (Some "chat_contract_incompatible") blocker;
  check
    bool
    "detail mentions chat contract"
    true
    (match detail with
