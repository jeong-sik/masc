...
let handle_finalizer_error ~module_name ~label ~during_exception ~backtrace ex =
  ...
  Log.Misc.error
    "%s %s failed in finalizer%s: %s"
    module_name
    label
    suffix
    (Printexc.to_string ex);
  if (not during_exception) && strict_finalizers ()
  then Printexc.raise_with_backtrace ex backtrace
;;
...
let masc_dir_from_base_path ~base_path = Filename.concat base_path masc_dirname
