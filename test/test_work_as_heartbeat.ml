...
let test_wah_enabled_default () =
  ...
  check bool "work-as-heartbeat enabled by default" true Cfg.WorkAsHeartbeat.enabled
;;
...
let test_wah_max_silence_floor_logic () =
  ...
  check bool "max_silence >= keepalive interval" true (v >= interval)
;;
...
let test_keepalive_max_failures_default () =
  check int "default max failures 5" 5 Cfg.KeeperKeepalive.max_consecutive_failures
;;
...
let test_keepalive_board_debounce_default () =
  check (float 0.1) "default debounce 60s" 60.0 Cfg.KeeperKeepalive.board_debounce_sec
;;

let test_keepalive_sleep_chunk_default () =
  check (float 0.01) "default sleep chunk 2.0s" 2.0 Cfg.KeeperKeepalive.sleep_chunk_sec
;;

let test_keepalive_jitter_default () =
  check (float 0.001) "default jitter 0.2" 0.2 Cfg.KeeperKeepalive.jitter_factor
;;
...
let test_oas_timeout_32k_uses_wall_clock () =
  ...
  check (float 1.0) "32K input → wall-clock cap (turn_timeout_sec)" turn_cap v
;;

let test_oas_timeout_128k_uses_wall_clock () =
  let v = adaptive ~estimated_input_tokens:128_000 in
  check (float 1.0) "128K input → wall-clock cap (turn_timeout_sec)" turn_cap v
;;

let test_oas_timeout_262k_uses_wall_clock () =
  let v = adaptive ~estimated_input_tokens:262_144 in
  check (float 1.0) "262K input → wall-clock cap (turn_timeout_sec)" turn_cap v
;;
...
let test_oas_timeout_scheduled_autonomous_uses_wall_clock () =
  ...
  in
  check (float 1.0) "scheduled_autonomous → wall-clock cap (turn_timeout_sec)" turn_cap v
;;

let test_oas_timeout_zero_uses_wall_clock () =
  let v = adaptive ~estimated_input_tokens:0 in
  check (float 1.0) "0 context → wall-clock cap (turn_timeout_sec)" turn_cap v
;;
...
let test_oas_timeout_cap () =
  ...
  check (float 0.1) "1M estimated input tokens → capped at turn_timeout_sec" turn_cap v
;;

let test_turn_timeout_default () =
  check
    (float 0.1)
    "default turn timeout 3600s"
    3600.0
    Cfg.KeeperKeepalive.turn_timeout_sec
;;

let test_max_turns_default () =
  check int "default max_turns_per_call 30" 30 Cfg.KeeperKeepalive.oas_max_turns_per_call
;;
...
let test_scheduled_autonomous_max_turns_default () =
  ...
  check
    int
    "default scheduled autonomous max_turns_per_call 10"
    10
...
let test_scheduled_autonomous_max_turns_range () =
  ...
  check
    bool
    "scheduled autonomous max_turns <= global"
    true
...
let test_semaphore_wait_timeout_default () =
  ...
  check
    (float 0.1)
    "default semaphore wait timeout 60s"
    60.0
...
let test_semaphore_wait_timeout_exception_shape () =
  ...
  let carried =
    try raise (KK.Semaphore_wait_timeout 42.5) with
    | KK.Semaphore_wait_timeout v -> v
  in
...
let test_autonomous_queue_fifo_prevents_reentry_cutting () =
  ...
  check
    (list string)
    "older waiters stay ahead of reentry"
    [ "cheolsu" ]
    (KK.autonomous_waiter_snapshot_for_test ());
  KK.drop_autonomous_waiter_for_test cheolsu_2;
  check (list string) "queue drained" [] (KK.autonomous_waiter_snapshot_for_test ())
;;

(* ── KeeperGrpc config defaults ────────────────────────── *)

let test_grpc_max_reconnect_default () =
  check int "default grpc max reconnect 5" 5 Cfg.KeeperGrpc.max_reconnect_attempts
;;
...
let test_grpc_backoff_default () =
  check (float 0.1) "default grpc backoff 5.0s" 5.0 Cfg.KeeperGrpc.reconnect_backoff_sec
;;
...
let test_proactive_max_attempts_default () =
  check int "default proactive max attempts 3" 3 Cfg.KeeperProactive.max_attempts
;;
...
let test_timing_ring_size_default () =
  check int "default timing ring size 100" 100 Cfg.KeeperProactive.stage_timing_ring_size
;;
...
let test_freshness_never_heartbeated () =
  ...
  let max_silence = 120.0 in
  let fresh = now -. last_hb < max_silence in
  check bool "never heartbeated (0.0) → stale → forces presence sync" false fresh
;;
...
let () =
  run
    ...
    ; ( "oas_timeout_adaptive"
      , [ test_case
            "32K context uses wall-clock cap (#10008 fm2)"
            `Quick
            test_oas_timeout_32k_uses_wall_clock
        ; test_case
            "128K context uses wall-clock cap (#10008 fm2)"
            `Quick
            test_oas_timeout_128k_uses_wall_clock
        ; test_case
            "262K context uses wall-clock cap (#10008 fm2)"
            `Quick
            test_oas_timeout_262k_uses_wall_clock
        ; test_case
            "scheduled autonomous uses wall-clock cap (#10008 fm2)"
            `Quick
            test_oas_timeout_scheduled_autonomous_uses_wall_clock
        ; test_case
            "0 context uses wall-clock cap (#10008 fm2)"
            `Quick
            test_oas_timeout_zero_uses_wall_clock
        ; test_case
            "budget is token-count independent (#10008 fm2)"
            `Quick
    ...
    ; ( "semaphore_wait_timeout"
      , [ test_case "default 60s" `Quick test_semaphore_wait_timeout_default
        ...
        ; test_case
            "exception carries wait sec"
            `Quick
            test_semaphore_wait_timeout_exception_shape
        ; test_case
            "autonomous queue FIFO prevents reentry cutting"
            `Quick
    ...
    ; ( "freshness_logic"
      , [ test_case "within window → fresh" `Quick test_freshness_fresh
        ...
        ; test_case
            "exact boundary → stale (strict <)"
            `Quick
            test_freshness_exact_boundary
    ...
    ; ( "config_invariants"
      , [ test_case
            ...
            `Quick
            test_config_invariant_silence_ge_interval
        ; test_case
            "sweep/backoff coherence"
            `Quick
            test_config_invariant_sweep_independent
        ; test_case
            "debounce >= interval"
            `Quick
            test_config_invariant_debounce_ge_interval
        ] )
    ; ( "percentile"
