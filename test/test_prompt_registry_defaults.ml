...
let test_fragment_group_slots () =
  (* [check] and its testables come from Alcotest, which this file opens
     inside its entry point rather than at the top. A case defined outside
     that scope has to open it for itself. *)
