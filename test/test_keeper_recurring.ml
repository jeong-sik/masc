...
let base_path = Filename.concat (Filename.get_temp_dir_name ()) "keeper_recurring_test_base"
...
let test_add_and_list () =
  Rec.clear ();
  let _t =
    Rec.add ~base_path ~keeper_name:"k1" ~label:"status" ~interval_sec:60 (Rec.Broadcast "hello")
  in
  let tasks = Rec.list ~base_path ~keeper_name:"k1" in
...
let test_list_filters_by_keeper () =
  Rec.clear ();
  let _t1 = Rec.add ~base_path ~keeper_name:"k1" ~label:"a" ~interval_sec:60 (Rec.Broadcast "a") in
  let _t2 = Rec.add ~base_path ~keeper_name:"k2" ~label:"b" ~interval_sec:60 (Rec.Broadcast "b") in
  check int "k1 tasks" 1 (List.length (Rec.list ~base_path ~keeper_name:"k1"));
  check int "k2 tasks" 1 (List.length (Rec.list ~base_path ~keeper_name:"k2"));
  check int "all tasks" 2 (List.length (Rec.list_all ~base_path))
;;

let test_list_filters_by_base_path () =
  Rec.clear ();
  let other_base_path =
    Filename.concat (Filename.get_temp_dir_name ()) "keeper_recurring_other_base"
  in
  let task =
    Rec.add
      ~base_path
  ...
  ignore
    (Rec.add
       ...
       ~interval_sec:60
       (Rec.Broadcast "b"));
  let scoped = Rec.list ~base_path ~keeper_name:"k1" in
  check int "base A only" 1 (List.length scoped);
  check string "base A label" "base-a" (List.hd scoped).label;
  check int "base B only" 1 (List.length (Rec.list ~base_path:other_base_path ~keeper_name:"k1"));
  check int "list_all scoped" 1 (List.length (Rec.list_all ~base_path));
  check bool "wrong base cannot remove task" false (Rec.remove ~base_path:other_base_path ~id:task.id);
  check bool "owning base removes task" true (Rec.remove ~base_path ~id:task.id)
;;

let test_remove () =
  Rec.clear ();
  let t = Rec.add ~base_path ~keeper_name:"k1" ~label:"x" ~interval_sec:60 (Rec.Broadcast "x") in
  check bool "remove existing" true (Rec.remove ~base_path ~id:t.id);
  check bool "remove again" false (Rec.remove ~base_path ~id:t.id);
  check int "empty" 0 (List.length (Rec.list ~base_path ~keeper_name:"k1"))
;;

let test_dispatch_due () =
  Rec.clear ();
  let _t =
    Rec.add ~base_path ~keeper_name:"k1" ~label:"tick" ~interval_sec:60 (Rec.Broadcast "tick msg")
  in
  (* Not due yet at t=30 *)
  let dispatched_early =
    Rec.dispatch_due ~base_path ~keeper_name:"k1" ~now_ts:30.0 ~dispatch:(fun _task _action -> Ok ())
  in
  check int "too early" 0 dispatched_early;
  (* Due at t=100 (100 - 0 = 100 >= 60) *)
  let dispatched = ref 0 in
  let count =
    Rec.dispatch_due ~base_path ~keeper_name:"k1" ~now_ts:100.0 ~dispatch:(fun _task action ->
      (match action with
  ...
  let count2 =
    Rec.dispatch_due ~base_path ~keeper_name:"k1" ~now_ts:110.0 ~dispatch:(fun _task _action ->
      Ok ())
  in
  check int "not due yet" 0 count2;
  (* Due again after interval *)
  let count3 =
    Rec.dispatch_due ~base_path ~keeper_name:"k1" ~now_ts:200.0 ~dispatch:(fun _task _action ->
      Ok ())
  in
...
let test_failure_auto_disable () =
  ...
  (* Fail twice *)
  let _ =
    Rec.dispatch_due ~base_path ~keeper_name:"k1" ~now_ts:100.0 ~dispatch:(fun _task _action ->
      Error "boom")
  in
  let tasks = Rec.list ~base_path ~keeper_name:"k1" in
  ...
  let _ =
    Rec.dispatch_due ~base_path ~keeper_name:"k1" ~now_ts:200.0 ~dispatch:(fun _task _action ->
      Error "boom again")
  in
  let tasks2 = Rec.list ~base_path ~keeper_name:"k1" in
  ...
  (* No more dispatches *)
  let count =
    Rec.dispatch_due ~base_path ~keeper_name:"k1" ~now_ts:300.0 ~dispatch:(fun _task _action ->
      Ok ())
  in
...
let test_reenable_after_cooldown () =
  ...
  let _ =
    Rec.dispatch_due ~base_path ~keeper_name:"k1" ~now_ts:10.0 ~dispatch:(fun _t _a -> Error "boom")
  in
  let task = List.hd (Rec.list ~base_path ~keeper_name:"k1") in
  ...
  let n0 = Rec.reenable_due_tasks ~base_path ~keeper_name:"k1" ~now_ts:15.0 in
  check int "within cooldown 0" 0 n0;
  let task1 = List.hd (Rec.list ~base_path ~keeper_name:"k1") in
  check bool "still disabled" false task1.enabled;
  (* After cooldown; now=25 >= 0 + 20 -> re-enable, failure_count reset *)
  let n1 = Rec.reenable_due_tasks ~base_path ~keeper_name:"k1" ~now_ts:25.0 in
  check int "after cooldown 1" 1 n1;
  let task2 = List.hd (Rec.list ~base_path ~keeper_name:"k1") in
  ...
  let n2 = Rec.reenable_due_tasks ~base_path ~keeper_name:"k1" ~now_ts:25.0 in
...
let test_reenable_filter_by_keeper () =
  ...
  (* Disable both *)
  let _ =
    Rec.dispatch_due ~base_path ~keeper_name:"k1" ~now_ts:10.0 ~dispatch:(fun _ _ -> Error "x")
  in
  let _ =
    Rec.dispatch_due ~base_path ~keeper_name:"k2" ~now_ts:10.0 ~dispatch:(fun _ _ -> Error "x")
  in
  (* Re-enable only k1 *)
  let n = Rec.reenable_due_tasks ~base_path ~keeper_name:"k1" ~now_ts:30.0 in
  check int "k1 re-enabled 1" 1 n;
  let k1 = List.hd (Rec.list ~base_path ~keeper_name:"k1") in
  let k2 = List.hd (Rec.list ~base_path ~keeper_name:"k2") in
...
let test_failure_does_not_update_last_run_ts () =
  ...
  in
  let _ =
    Rec.dispatch_due ~base_path ~keeper_name:"k1" ~now_ts:10.0 ~dispatch:(fun _ _ ->
      Error "transient")
  in
  let task = List.hd (Rec.list ~base_path ~keeper_name:"k1") in
...
let test_reenabled_multiple_tasks_preserve_partial_failure () =
  ...
  let _ = Rec.dispatch_due ~base_path ~keeper_name:"k1" ~now_ts:10.0 ~dispatch:fail_all in
  let _ = Rec.dispatch_due ~base_path ~keeper_name:"k1" ~now_ts:11.0 ~dispatch:fail_all in
  let disabled = Rec.list ~base_path ~keeper_name:"k1" in
  ...
  List.iter
    ...
    disabled;
  let n = Rec.reenable_due_tasks ~base_path ~keeper_name:"k1" ~now_ts:25.0 in
  check int "two re-enabled" 2 n;
  let successes =
    Rec.dispatch_due ~base_path ~keeper_name:"k1" ~now_ts:25.0 ~dispatch:(fun task _action ->
  ...
  run
    ...
    [ ( "crud"
      , [ test_case "add and list" `Quick test_add_and_list
        ...
        ; test_case "filter by base_path" `Quick test_list_filters_by_base_path
