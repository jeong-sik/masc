...
| 3 | exact-output 레인 슬롯이 가리키는 provider 가 `exact-body-timeout-s` 를 선언하지 않음 → 레인·슬롯·provider 이름을 댄다 | 있다(#38779). `validate_exact_slot_body_deadlines`(`runtime.ml`)가 `Exact_slot_body_deadline_absent` 로 거절한다. 규칙 1 과 같은 자리에서 걸려서, 저장은 쓰기 전에 막히고 부팅은 `Config_invalid` 로 설정 대기 상태가 된다. `slots` 가 가리키는 HTTP 런타임만 보고, `cli_slots` 와 `AGENT_CORE_MODEL_CATALOG` 교체 카탈로그는 보지 않는다 |
