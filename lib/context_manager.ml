...
let message_to_json (m : Agent_sdk.Types.message) : Yojson.Safe.t =
  let m = Masc_model.sanitize_message_utf8 m in
  let base = [
...
let message_of_json (json : Yojson.Safe.t) : Agent_sdk.Types.message =
  ...
  let text = json |> member "content" |> to_string |> Masc_model.sanitize_text_utf8 in
  match role with
  | Agent_sdk.Types.Tool ->
...
let serialize_context (ctx : working_context) : string =
  let json = `Assoc [
    ("system_prompt", `String (Masc_model.sanitize_text_utf8 ctx.system_prompt));
...
let deserialize_context (s : string) ~max_tokens : working_context =
  let json = Yojson.Safe.from_string (Masc_model.sanitize_text_utf8 s) in
  let open Yojson.Safe.Util in
  let system_prompt =
    json |> member "system_prompt" |> to_string |> Masc_model.sanitize_text_utf8
  in
...
let create_checkpoint ctx ~generation =
  {
    ...
    serialized = serialize_context ctx |> Masc_model.sanitize_text_utf8;
  }
...
let persist_message session msg =
  let msg = Masc_model.sanitize_message_utf8 msg in
  ...
  in
  let line =
    (Yojson.Safe.to_string payload |> Masc_model.sanitize_text_utf8) ^ "\n"
  in
  Fs_compat.append_file path line

let save_checkpoint session ckpt =
  let ckpt =
    { ckpt with serialized = Masc_model.sanitize_text_utf8 ckpt.serialized }
  in
  ...
  let content = Yojson.Safe.to_string json |> Masc_model.sanitize_text_utf8 in
  ...
  if not (Sys.file_exists dir) then None
    ...
    | latest :: _ ->
      ...
      Some {
        ...
        serialized =
          json |> member "serialized" |> to_string |> Masc_model.sanitize_text_utf8;
      }
