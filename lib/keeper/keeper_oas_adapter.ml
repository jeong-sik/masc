...
let run_with_custom_dispatch
    ~(meta : keeper_meta)
    ?model_spec_override:_model_override
    ~(system_prompt : string)
    ~(goal : string)
    ~(max_turns : int)
    ~(temperature : float)
  ...
  ignore (meta, _model_override);
  Oas_worker.run_named_with_masc_tools
    ~cascade_name:"keeper_turn" ~goal ~system_prompt
    ~masc_tools ~dispatch ~max_turns ~temperature ~max_tokens
    ?guardrails ()
