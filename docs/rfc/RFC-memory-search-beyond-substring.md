...
- 단계 2 를 2a(선택형)로 갈지 2b(질의 확장형)로 갈지. 제안: 2a.
- 단계 2 판정 lane 을 absorb gate 와 같은 TypeSafe 경로로 쓸지, 로컬 모델로 쓸지. 데이터 반출 면에서는 로컬이
  낫다(absorb gate RFC §8 의 반출 리스크와 같은 문제).
- 임베딩 거부(RFC-0247 §3)를 유지하는지. 이 문서는 유지를 전제로 한다.
