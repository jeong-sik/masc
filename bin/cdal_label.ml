     ...
     | Ok label ->
       let now =
         ...
         Printf.sprintf
           ...
           tm.tm_sec
       in
       let lv : L.labeled_verdict = { verdict; label; labeler; note; labeled_at = now } in
       let out_json = L.labeled_verdict_to_json lv in
       (match output_path with
        | Some path ->
          write_json_file path out_json;
          Printf.printf "labeled verdict written to %s\n" path
        | None -> print_endline (Yojson.Safe.pretty_to_string out_json)))
;;
...
let apply_term =
  ...
  Cmd.v
    info
    Term.(
      const apply_cmd $ verdict_path_arg $ label_arg $ labeler_arg $ note_arg $ output_arg)
;;
