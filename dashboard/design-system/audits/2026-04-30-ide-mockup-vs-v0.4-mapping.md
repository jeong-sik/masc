...
**Status**: SSOT for IDE Plane production migration; framework choices refreshed
for the Preact-only dashboard in PR #24015
**Trigger**: agent-runtime-a.example/design `MASC Cockpit.html` `code-mode.js` mockup 이 production 진입 후보로 부상. 사용자 지시 "supervisor 에이전트가 우리 mockup의 raw hex를 그대로 베껴쓸까" — 즉 mockup을 그대로 픽셀 카피하지 말고 v0.4 SSOT로 번역할 매핑이 선행되어야 한다.
...
| KPI strip (NCV/QAK/MIP/SGS/RMA/TSK/JTR) | `KpiStripView` + `KpiStrip` (Preact) | high — 동일 | 그대로 재사용. IDE plane 진입 시에도 KPI strip 유지 (cockpit 공통 chrome) |
...
Dashboard production components use Preact. RFC 0017's Solid experiment was
reverted by PR #24015 after its performance hypothesis failed for the product
shape; component cardinality no longer selects a second UI runtime.

| component | N (예상) | framework | 근거 |
|-----------|---------|-----------|------|
| EXPLORER tree (file-tree-store) | 500-5000 노드 | **Preact** | Single dashboard runtime; virtualize large collections when measured evidence requires it. |
| Editor + blame strip | 100-2000 라인 | **Preact** | Single dashboard runtime; line data stays in typed stores/signals. |
| LAYERS toggle | 6 entries | **Preact** | Native dashboard component. |
| CONVERSATION rail | 5-30 threads | **Preact** | Native dashboard component. |
| ACTIVITY THIS RUN | 100-1000 events | **Preact** | Existing virtual-list and signal primitives own scaling. |
| INTERJECT input | 1 input + 4 actions | **Preact** | Native dashboard component. |
| Cockpit mode tabs (topbar) | 4-6 tabs | **Preact** | Native dashboard component. |
| View tabs (SOURCE/SPLIT DIFF/UNIFIED/BLAME) | 4 tabs | **Preact** | Native dashboard component. |
...
- [ ] Component가 Preact 외 UI runtime 또는 framework adapter를 추가하지 않음.
...
- agent-runtime-a.example/design `MASC Cockpit.html` `code-mode.js` mockup (사용자 image 첨부, 2026-04-30)
