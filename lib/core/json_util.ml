...
let get_string : Yojson.Safe.t -> string -> string option =
  fun json key ->
...
let get_int : Yojson.Safe.t -> string -> int option =
  fun json key ->
  match Yojson.Safe.Util.member key json with
  | `Int n -> Some n
...
let get_float : Yojson.Safe.t -> string -> float option =
  fun json key ->
  match Yojson.Safe.Util.member key json with
  | `Float f -> Some f
...
let get_bool : Yojson.Safe.t -> string -> bool option =
  fun json key ->
  match Yojson.Safe.Util.member key json with
  | `Bool b -> Some b
  | _ -> None
...
let get_string_list : Yojson.Safe.t -> string -> string list =
  fun json key ->
  match Yojson.Safe.Util.member key json with
  | `List xs ->
    List.filter_map
...
let get_object : Yojson.Safe.t -> string -> Yojson.Safe.t option =
  fun json key ->
  match Yojson.Safe.Util.member key json with
...
let get_array : Yojson.Safe.t -> string -> Yojson.Safe.t option =
  fun json key ->
  match Yojson.Safe.Util.member key json with
...
let json_assoc_list kv = `Assoc (List.map (fun (k, v) -> k, `String v) kv)

let parse_json_or_string s =
  try Yojson.Safe.from_string s with
  | Yojson.Json_error _ -> `String s
;;
...
let dedupe_keep_order xs =
  let rec loop seen acc = function
    | [] -> List.rev acc
    | x :: rest ->
      if List.mem x seen then loop seen acc rest else loop (x :: seen) (x :: acc) rest
  in
  loop [] [] xs
;;
