...
- A board-attention judgment is no longer settled as "candidate absent" when the candidate's ledger row is unreadable. The ledger read skips rows it cannot decode, so a Relevant judgment whose candidate row had bad JSON or an unsupported `schema_version` was closed without being delivered, and a refused row newer than an older readable one could re-deliver from the stale row. A refused row that could be the candidate now keeps that partition `Completed` with its judgment and nothing is delivered; the keeper's other completed partitions still settle in the same pass, and the kept one settles once the row reads again (#38041).
