...
let max_risk left right = if risk_rank left >= risk_rank right then left else right
...
let make_seed
  ...
  { capability_id =
      Option.value ~default:(canonical_capability_id backend_tool_name) capability_id
  ; risk_class
  ; audiences
...
let public_projection_seeds_from (public_tool_source_schemas : Types.tool_schema list)
  ...
  let make_public_seed schema =
    ...
    let risk_class =
      if List.mem name privileged_public_tool_names
      then Privileged
      else if List.mem name spawned_agent_public_tool_names
      then Audited
      else Safe
    in
    let audiences =
      unique_preserve_order
        ((External_mcp_client
          ::
          (if List.mem name spawned_agent_public_tool_names
           then [ Spawned_managed_agent ]
           else []))
         @
         if List.mem name local_worker_public_tool_names
         then [ Local_worker_agent ]
         else [])
    in
    let supports_audit_evidence = List.mem name spawned_agent_public_tool_names in
    let base =
      [ make_seed
          ~risk_class
          ~audiences
...
let keeper_projection_seeds : capability_seed list =
  ...
  |> List.concat_map (fun (tool : Types.tool_schema) ->
    ...
    let primary_surface = if privileged then Keeper_privileged else Keeper_standard in
    let primary_seed =
      make_seed
        ~risk_class:(if privileged then Privileged else Audited)
        ~audiences:
          (if privileged then [ Keeper_agent; Privileged_executor ] else [ Keeper_agent ])
        ~supports_audit_evidence:true
        ~supports_direct_user_discovery:false
    ...
    List.fold_left
      (fun (tbl, ordered_ids) (seed : capability_seed) ->
         ...
         | None ->
           ...
           in
           StringMap.add seed.capability_id def tbl, seed.capability_id :: ordered_ids
         | Some existing ->
           let def =
             { capability_id = existing.capability_id
             ; risk_class = max_risk existing.risk_class seed.risk_class
             ; audiences = unique_preserve_order (existing.audiences @ seed.audiences)
             ; supports_audit_evidence =
...
let surface_tool_names_from (public_tool_source_schemas : Types.tool_schema list) surface
  : string list
  =
...
let public_tool_schemas_from (public_tool_source_schemas : Types.tool_schema list)
  : Types.tool_schema list
  =
  dedupe_schemas public_tool_source_schemas |> Tool_help_registry.canonicalize_schemas
;;
...
let keeper_privileged_tool_names : string list = privileged_keeper_tool_names
...
let keeper_wrapped_internal_tools : string list = keeper_all_tool_names

let surface_snapshot_json (public_tool_source_schemas : Types.tool_schema list) =
...
let capability_to_json (capability : capability_def) =
  `Assoc
    ...
    ; "supports_direct_user_discovery", `Bool capability.supports_direct_user_discovery
    ; ( "projections"
      , `List
          (List.map
