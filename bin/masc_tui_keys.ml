    ...
    List.map
      (fun (title, surface) ->
         ...
         let tab_entries =
           ...
           | _ -> []
         in
         (* The shared tail -- r, Tab and q exactly as [listing_meta] spells
            them -- is said once, under Global. Repeated under every surface
            it took three of the dozen rows an 80x24 sheet shows of the
            reader's own section. A surface that names one of those keys its
            own way ([r] reload on Config) keeps that row: it says something
            Global does not. Footers keep the tail, since a footer is all a
            surface shows. *)
         let own =
           List.filter
             ...
             (for_surface surface)
         in
         (surface, (title, entries own @ tab_entries)))
      help_surfaces
  in
  let here, rest =
    match current with
