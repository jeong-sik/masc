...
- A keeper request whose Librarian snapshot fits but ends before the Librarian's read position now carries the snapshot's working state and starts the range at the read position, instead of resending every atom after the snapshot's end. A snapshot the Librarian is rewriting from atom 0 is this shape. The request origin is `librarian_snapshot_then_progress`.
