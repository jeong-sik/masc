...
> two ways. (a) L1 Workspace pubsub split was **dropped** — heartbeats never
...
| 레이어 | 식별자 | 위치 | 소비자 |
|--------|--------|------|--------|
| L1: Workspace pubsub 채널 | `broadcast:<project>:default` (string key) | `lib/workspace/workspace_broadcast.ml:58-59` | `Backend_types.Pubsub_mem` 구독자 (현재 in-process), 잔존 메시지는 `messages_dir/*.json` |
...
### 1.1 L1 Workspace 단일 채널

`lib/workspace/workspace_broadcast.ml:58-89` — 단일 채널 정의 + publish:
...
이 함수의 caller (검색 결과 `rg -n "broadcast " lib/workspace/`, 본 RFC 작성 1일 후 `workspace_task` 모듈이 `workspace_task.ml` + `workspace_task_create.ml`로 분리 + `workspace_lifecycle` 추가 호출 발생):
...
| `activity_*` (prefix) | activity graph emit 시 | `Workspace_hooks.activity_emit_fn` | activity 그래프 |
...
PR-0.2 (perf baseline) 머지 후 `legacy metrics backend.metric_sse_broadcast_events`로 갱신.
...
#### L1 Workspace 레이어 — **변경 없음**

원안은 `presence:<project>:default` pubsub 채널을 신설했으나,
구현 단계에서 L1 트래픽 audit 결과 heartbeat-class 이벤트는 L1 workspace pubsub로
publish되지 않음을 (재)확인했다 (sec 1.1 footnote 참조 — `workspace_gc.ml:17`
heartbeat 함수는 `agent_file.last_seen`만 갱신하지 channel publish 없음).
따라서 L1 split은 source traffic이 0이어서 무의미. `workspace_broadcast.ml`
...
```ocaml
(* lib/sse.ml 발췌 — #12129 *)
type session_kind = Observer | Agent stream | Presence
type broadcast_target = All | Observers | Agent streams | Presence_only
...
| task claim 알림 | (`workspace_broadcast.broadcast` → `masc:broadcast`) | broadcast | broadcast (유지) | |
...
| 컴포넌트 | 변경 | 위험 (실측) |
|---------|------|------|
| `workspace_broadcast.ml` | **변경 없음** (L1 split 불필요로 판명, 본 RFC 헤더 postmortem 참조) | — |
...
| `lib/transport_metrics.ml` | `sse_sessions{kind=presence}` legacy metrics backend label 추가 | 낮음 — metric label 확장 |
...
- **단일 채널 완전 복원**: 위 RFC 자체를 revert하고 `workspace_broadcast.broadcast_channel`을
