...
let prefixed_tool_names names = names |> List.map (fun name -> "mcp__masc__" ^ name)
...
let lookup_schemas_by_name_exn ~label all_schemas values =
  ...
  List.iter
    (fun (schema : Types.tool_schema) ->
       if not (Hashtbl.mem by_name schema.name)
       then Hashtbl.add by_name schema.name schema)
    all_schemas;
  let missing =
    requested |> List.filter (fun tool_name -> not (Hashtbl.mem by_name tool_name))
  in
  if missing <> []
  then
    invalid_arg
      (Printf.sprintf "%s: unknown tool schema(s): %s" label (String.concat ", " missing));
...
let local_worker_code_schemas : Types.tool_schema list = Tool_schemas_code.schemas
let local_worker_worktree_schemas : Types.tool_schema list = Tool_schemas_worktree.schemas
let local_worker_run_schemas : Types.tool_schema list = Tool_schemas_run.schemas
...
let select_public_local_worker_schemas () =
  ...
  |> List.filter (fun (schema : Types.tool_schema) -> List.mem schema.name wanted)
;;

let resolve_named_schemas all_schemas values : (Types.tool_schema list, string) result =
  let requested =
    values
    |> List.map String.trim
    |> List.filter (fun value -> value <> "")
  ...
  let missing =
    ...
    |> List.filter (fun tool_name ->
      not
        (List.exists
           (fun (schema : Types.tool_schema) -> String.equal schema.name tool_name)
           schemas))
  in
  if missing <> []
  then Error (Printf.sprintf "unknown tool schema(s): %s" (String.concat ", " missing))
  else Ok schemas
;;

let local_worker_tool_schemas ?names () : (Types.tool_schema list, string) result =
  let all_schemas =
    dedupe_schemas
...
let admin_tool_names : string list = Tool_catalog.tools_for_surface Tool_catalog.Admin
...
let coordination_tool_names : string list = Tool_catalog_surfaces.coordination_role_tools

(** Role-catalog candidates for worker agents.
    SSOT: Tool_catalog_surfaces.execution_role_tools. *)
let execution_tool_names : string list = Tool_catalog_surfaces.execution_role_tools

let filter_catalog_to_available ~available names =
  let available = SS.of_list available in
  names |> List.filter (fun name -> SS.mem name available) |> unique_preserve_order
;;
...
let build_tool_catalog ~(role : string) () : string list =
  ...
  let filtered =
    match role with
    | "worker" -> filter_catalog_to_available ~available:all_names execution_tool_names
    ...
    | _ ->
      ...
      List.filter (fun name -> not (List.mem name admin_tool_names)) all_names
  in
...
let local_worker_resolvable_tool_names () : string list =
  ...
  | Ok schemas -> List.map (fun (s : Types.tool_schema) -> s.name) schemas
  | Error msg ->
    Eio.traceln "[AgentToolSurfaces] local_worker_tool_schemas failed: %s" msg;
    []
;;
