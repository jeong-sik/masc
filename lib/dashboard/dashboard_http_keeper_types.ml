...
let terminal_reason_code_of_decision_json json =
  ...
  | None ->
    (match Option.value ~default:`Null (Json_util.assoc_member_opt "terminal_reason" json) with
...
let latest_receipt_ts_of_keeper_rows rows =
  ...
  |> List.fold_left
       (fun acc row ->
         match
           let trust = Option.value ~default:`Null (Json_util.assoc_member_opt "trust" row) in
           Option.value ~default:`Null (Json_util.assoc_member_opt "last_receipt_at" trust)
         with
         | `String iso -> (
