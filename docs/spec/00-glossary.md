...
**Cascade**
LLM 모델 호출의 폴백 순서를 정의하는 설정. live authoring source는 `<base-path>/.masc/config/cascade.toml` 또는 명시적 `MASC_CONFIG_DIR/cascade.toml`이고, 체크인된 `config/cascade.toml`은 default/example fallback source다. 실제 순서는 route/cascade 설정을 따른다. `-> docs/cascade/README.md`, `-> lib/cascade/cascade_runtime.mli`
...
> 이전 SPEC 에 있던 별도 `Error.t` 인프라 계층(Room/Federation/Mcp/Severity/recoverability helper)은 #10659 에서 0-caller dead code 로 삭제되었다. Room/Federation 도메인 에러는 더 이상 별도 variant 가 아니며, Portal 은 `masc_error` 의 `PortalNotOpen`/`PortalAlreadyOpen`/`PortalClosed` 로 흡수되었다. MCP 프로토콜 에러는 `Mcp_*` 모듈이 자체 응답으로 처리한다.
