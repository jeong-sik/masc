...
let case_score_to_yojson (score : case_score) =
  `Assoc
    ...
    ; ( "latency_ms"
      , Option.fold ~none:`Null ~some:(fun value -> `Int value) score.latency_ms )
    ; ( "input_tokens"
      , Option.fold ~none:`Null ~some:(fun value -> `Int value) score.input_tokens )
    ; ( "output_tokens"
      , Option.fold ~none:`Null ~some:(fun value -> `Int value) score.output_tokens )
    ; "cost_usd", Option.fold ~none:`Null ~some:(fun value -> `Float value) score.cost_usd
    ; ( "prompt_fingerprint"
      , Option.fold
          ~none:`Null
          ~some:(fun value -> `String value)
          score.prompt_fingerprint )
...
let json_check_to_yojson (check : json_check) =
  `Assoc
    ...
    ; ( "contains"
      , Option.fold ~none:`Null ~some:(fun value -> `String value) check.contains )
...
let summary_row_to_yojson (row : summary_row) =
  `Assoc
    ...
    ; ( "stability_score"
      , Option.fold ~none:`Null ~some:(fun value -> `Float value) row.stability_score )
