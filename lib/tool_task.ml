...
let validate_task_id task_id =
  if task_id = "" then Error (Types.TaskNotFound "") else Ok task_id
;;
...
let keeper_agent_tool_names (ctx : context) =
  let resolved =
    ...
    | exn ->
      Log.Task.warn
        "resolve_agent_name failed for keeper tool surface %s: %s"
        ctx.agent_name
        (Printexc.to_string exn);
      ctx.agent_name
  in
...
let review_completion_notes
  ...
  | Some task ->
    ...
    let on_verdict result =
      Eval_calibration.record_verdict ~task_id ~req:ar_req ~result ();
      try
        Sse.broadcast
...
let persisted_completion_contract ~(task_opt : Types.task option) =
  ...
  | Some ({ contract = Some contract; _ } : Types.task)
    when contract.completion_contract <> [] -> Some contract.completion_contract
  | _ -> None
;;
...
let completion_notes_example =
  "Example of accepted notes: 'Added Event_kind.Board variant to \
   lib/coord/event_kind.{ml,mli}, migrated 8 call-sites in coord_task.ml and \
   activity_graph.ml, test_event_kind round-trip green, CI green on PR #NNNN.'"
;;
...
let parse_task_contract args =
  ...
  | `Assoc _ as json ->
    (match Types.task_contract_of_yojson json with
     ...
     | Error error -> Error (Printf.sprintf "Invalid contract payload: %s" error))
  | _ -> Error "contract must be an object when provided"
;;

let is_internal_marker key = String.length key > 0 && key.[0] = '_'
...
let parse_handoff_context ~(agent_name : string) args =
  ...
  | `Assoc _ as json ->
    (match Types.task_handoff_context_of_yojson json with
     | Error error -> Error (Printf.sprintf "Invalid handoff_context payload: %s" error)
     | Ok handoff_context ->
       let summary = String.trim handoff_context.summary in
       let summary =
         if summary = ""
         then Option.value ~default:"" (synthesize_summary_from_siblings args)
         else summary
       in
       if summary = ""
       then
         Error
           "handoff_context.summary is required (non-empty string). Example: \
            {\"summary\": \"tests green, PR #123 pending review\", \"next_step\": \"wait \
            for CI\", \"evidence_refs\": [\"PR#123\"]}. Alternatively pass a non-empty \
            top-level 'notes' or 'reason' and it will be synthesized into summary \
            automatically."
       else
         Ok
           (Some
  ...
  | Some task ->
    (match task.task_status with
     | Types.Claimed { assignee; _ } | Types.InProgress { assignee; _ } ->
       ...
       then None
       else Some (Types.TaskAlreadyClaimed { task_id; by = assignee })
     | Types.Todo -> Some (Types.TaskNotClaimed task_id)
     | Types.Done { assignee; _ } ->
       Some
         (Types.TaskInvalidState
            (Printf.sprintf
               "task %s is already done by %s; inspect task history instead of calling \
                masc_transition(action=done) again"
               task_id
               assignee))
     | Types.Cancelled { cancelled_by; _ } ->
       Some
         (Types.TaskInvalidState
            (Printf.sprintf
               "task %s was cancelled by %s; reopen or create a new task instead of \
                calling masc_transition(action=done)"
               task_id
               cancelled_by))
     | Types.AwaitingVerification { assignee; _ } ->
       Some
         (Types.TaskInvalidState
            (Printf.sprintf
               "task %s is awaiting verification by %s; approve or reject before marking \
                done"
               task_id
               assignee)))
;;

let persisted_contract_rejection
      ~(ctx : context)
      ~(task_opt : Types.task option)
      ~(notes : string)
  =
  ...
  | None -> None
  | Some task ->
    if not (Env_config_runtime.Cdal.gate_enabled ())
    then (
      Log.Task.info
      ...
      | Some contract ->
        ...
        let gate_label =
          if contract.strict
          then Cdal_verdict_gate.strict_gate_label
        ...
        Log.Task.info
          ...
          contract.strict
          gate_label;
        let rejection = Cdal_verdict_gate.gate_check ~gate_label ~task_id:task.id () in
        if contract.strict
        then rejection
        else (
...
let handle_add_task ctx args =
  ...
  then
    ...
    if trimmed_title = ""
    then false, "Task title cannot be empty or whitespace-only"
    else if priority < 1 || priority > 5
    then false, Printf.sprintf "Priority must be between 1 and 5, got %d" priority
    else if
      Option.is_some goal_id
      && not
           (Goal_store.list_goals ctx.config ()
            |> List.exists (fun (goal : Goal_store.goal) ->
              String.equal goal.id (Option.value ~default:"" goal_id)))
    then false, Printf.sprintf "Unknown goal_id '%s'" (Option.value ~default:"" goal_id)
    else (
...
let handle_batch_add_tasks ctx args =
  let tasks_json =
    match args |> member "tasks" with
    | `List l -> l
    | _ -> []
  in
  if tasks_json = []
  then false, "tasks array is empty or missing"
  else (
    let validated =
      List.mapi
        (fun idx t ->
           ...
           let priority =
             t |> member "priority" |> to_int_option |> Option.value ~default:3
           in
           let description =
             t |> member "description" |> to_string_option |> Option.value ~default:""
           in
           let goal_id =
           ...
           in
           if title = ""
           then Error (Printf.sprintf "item[%d]: title cannot be empty" idx)
           else if priority < 1 || priority > 5
           then
             ...
             | Ok contract ->
               let has_removed_field name =
                 ...
                 | _ -> true
               in
               if has_removed_field "required_preset"
               then
                 Error
                   (Printf.sprintf "item[%d]: required_preset is no longer supported" idx)
               else if has_removed_field "required_role"
               then
                 Error
                   (Printf.sprintf "item[%d]: required_role is no longer supported" idx)
               else if has_removed_field "required_verifier_role"
               then
                 Error
                   (Printf.sprintf
                      "item[%d]: required_verifier_role is no longer supported"
                      idx)
               else Ok (title, priority, description, contract, goal_id)
    ...
    in
    if errors <> []
    then false, Printf.sprintf "Validation failed:\n%s" (String.concat "\n" errors)
    else (
      let tasks =
        List.filter_map
          (function
      ...
      in
      ( true
      , Coord.batch_add_tasks_with_contracts ~created_by:ctx.agent_name ctx.config tasks )))
;;
...
let handle_claim ?agent_tool_names ctx args =
  ...
  then result_to_response (Error (Types.AgentNotJoined ctx.agent_name))
  else if args |> member "agent_role" <> `Null
  then false, "agent_role is no longer supported"
  else (
...
let handle_claim_next ?agent_tool_names ctx _args =
  if
    not
      (try Coord.is_agent_joined ctx.config ~agent_name:ctx.agent_name with
       | Sys_error _ | Not_found -> false)
  then false, Printf.sprintf "Agent '%s' is not a member of this room" ctx.agent_name
  else (
    let agent_tool_names =
      match agent_tool_names with
      | Some _ -> agent_tool_names
    ...
    let message =
      ...
      | Coord.Claim_next_no_eligible { excluded_count; _ } ->
        Printf.sprintf
          "📋 No eligible tasks available (blocked/excluded: %d)"
          excluded_count
  ...
  | Ok task_id ->
    ...
    (match handoff_context with
     ...
     | Ok handoff_context ->
       ...
       then false, "Strict task release requires handoff_context.summary"
       else (
         let result =
...
and handle_cancel_task ctx args =
  ...
  | Ok task_id ->
    ...
    in
    let result =
      Coord.cancel_task_r ctx.config ~agent_name:ctx.agent_name ~task_id ~reason
    in
    ...
    (match result with
     | Ok _ ->
       ...
       let metric : Metrics_store_eio.task_metric =
         { id =
             Printf.sprintf
               "metric-%s-%d"
               task_id
               (int_of_float (Time_compat.now () *. 1000.))
       ...
       in
       (try
          let _ = Metrics_store_eio.record ctx.config metric in
          ()
        with
        | Eio.Cancel.Cancelled _ as e -> raise e
        | exn ->
          Log.Task.error
            "Metrics_store_eio.record(cancel) failed: %s"
            (Printexc.to_string exn));
...
and handle_transition ?agent_tool_names ctx args =
  ...
  let is_internal_marker k = String.length k > 0 && k.[0] = '_' in
    ...
    | `Assoc kvs ->
      ...
      let kvs =
        ...
        then
          ...
          List.filter
            (fun (k, _) -> (not (String.equal k "note")) || not (has "notes"))
            kvs
      in
      let kvs =
        ...
        then
          ...
          List.filter
            (fun (k, _) -> (not (String.equal k "to")) || not (has "action"))
            kvs
      in
      `Assoc kvs
    | other -> other
  ...
  let unknown =
    ...
    | `Assoc kvs ->
      List.filter
        (fun (k, _) ->
           (not (is_internal_marker k)) && not (List.mem k transition_known_args))
        kvs
    | _ -> []
  in
  if unknown <> []
  then (
    ...
    , Printf.sprintf
        "Unknown argument(s): %s. Valid: %s"
        names
        (String.concat ", " transition_known_args) ))
  else (
    let task_id = get_string args "task_id" "" in
    match validate_task_id task_id with
    | Error e -> result_to_response (Error e)
    | Ok task_id ->
        ...
        | Ok action ->
          ...
          let force =
            ...
            then (
              ...
              | _ ->
                Log.Task.warn
                  "[anti-rationalization] force=true rejected: agent=%s lacks admin \
                   privilege"
                  ctx.agent_name;
                false)
            else false
          ...
          (match handoff_context with
           ...
           | Ok handoff_context ->
             if
               ...
               && strict_release_requires_handoff task_opt
               && Option.is_none handoff_context
             then false, "Strict task release requires handoff_context.summary"
             else (
               let completion_state_error =
                 if action = Types.Done_action && not force
                 then completion_state_error ~task_id ~agent_name:ctx.agent_name ~task_opt
                 else None
               in
               match completion_state_error with
               | Some err ->
                 Log.Task.error
                   "task transition failed: %s"
               ...
               | None ->
                 ...
                 let persisted_gate_rejection =
                   if action = Types.Done_action && not force
                   then
                     if not completion_owned_by_caller
                     then None
                     else if task_has_persisted_contract task_opt
                     then persisted_contract_rejection ~ctx ~task_opt ~notes
                     else None
                   else None
                 in
                  ...
                  | None ->
                    let review_gate_rejection =
                      ...
                      then
                        if not completion_owned_by_caller
                        then None
                        else if can_review_completion ~task_opt ~agent_name:ctx.agent_name
                        then
     ...
     measurement by the verifier. See issue #7598. *)
                           ...
                           | Some task ->
                             (match task.contract with
                              | Some contract
                                ...
                                Log.Task.info
                                  "[verifier-gate] redirecting \
                                   Done→Submit_for_verification task=%s agent=%s \
                                   contract_items=%d"
                                  task_id
                                  ctx.agent_name
                       ...
                       let started_at_actual, collaborators_from_task =
                         match task_opt with
                         | Some t ->
                           (match t.task_status with
                       ...
                       let prepare_verification_verdict =
                         ...
                         | Types.Cancel
                         | Types.Release
                         | Types.Submit_for_verification -> None
                       in
                       let rec try_transition attempt =
                         ...
                         let r =
                           Coord.transition_task_r
                             ...
                             ()
                         in
                         if is_version_mismatch r && attempt < max_cas_retries
                         then (
                           Log.Task.info
                             "CAS version mismatch on %s (attempt %d/%d), retrying in \
                              %.0fms"
                             task_id
                             (attempt + 1)
                             max_cas_retries
                             (cas_retry_delay_s *. 1000.0);
     ...
     Issue #7543. *)
                       ...
                       (match result with
                        | Ok _ ->
                          ...
                          (match action with
                           ...
                           | Types.Approve_verification ->
                             let verification_id =
                               Option.value ~default:"" verification_id_before
                             in
                             Verification_protocol.notify_approve_verification
                               ~task_id
                               ~verifier:ctx.agent_name
                               ~verification_id
                               ~notes;
             ...
             [gate_check] performs internally. *)
                             if Env_config_runtime.Cdal.gate_enabled ()
                             then ignore (Cdal_verdict_gate.gate_check ~task_id ())
                       ...
                       (match result, action with
                        | Ok _, Types.Done_action ->
                          ...
                          (try
                           ...
                           | exn ->
                             Log.Task.error
                               "Metrics_store_eio.record(transition-done) failed: %s"
                               (Printexc.to_string exn));
                        ...
                        | Ok _, Types.Cancel ->
                          let metric : Metrics_store_eio.task_metric =
                            ...
                            ; success = false
                            ; error_message =
                                Some (if reason = "" then "Cancelled" else reason)
                            ; collaborators = collaborators_from_task
                            ; handoff_from = None
                            ; handoff_to = None
                            }
                          ...
                          (try
                           ...
                           | exn ->
                             Log.Task.error
                               "Metrics_store_eio.record(transition-cancel) failed: %s"
                               (Printexc.to_string exn));
                          Thompson_sampling.record_vote
                            ~agent_name:ctx.agent_name
                            ~direction:`Down;
...
let task_history_events_json (config : Coord.config) ~task_id ~limit =
  ...
  let parsed, _malformed = Fs_compat.parse_jsonl_lines ~source:"task_events" lines in
...
let tool_required_permission = function
  | "masc_tasks" | "masc_task_history" -> Some Types.CanReadState
  | "masc_add_task" | "masc_batch_add_tasks" -> Some Types.CanAddTask
  | "masc_claim_next" -> Some Types.CanClaimTask
  | "masc_transition" | "masc_update_priority" -> Some Types.CanCompleteTask
  | _ -> None
;;
