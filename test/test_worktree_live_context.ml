...
let init_repo dir =
  ...
  run_ok
    ~cwd:dir
    "git add .gitignore sample.ml other.ml && git -c core.hooksPath=/dev/null commit -q \
     -m base"
;;
...
let test_capture_only_on_change () =
  with_temp_dir "worktree-live-context" (fun dir ->
    ...
    check
      (option string)
      "clean repo produces no block"
      None
      (Wlc.capture_change_block ~base_path:dir ~actor_key:"keeper-a");
    check
      (option string)
      "clean repo stays quiet after state write"
      None
      (Wlc.capture_change_block ~base_path:dir ~actor_key:"keeper-a");
    ...
    check
      bool
      "block mentions changed file"
      true
      (try
         ignore (Str.search_forward (Str.regexp_string "sample.ml") block 0);
         true
       with
       | Not_found -> false);
    check
      (option string)
      "same change not repeated"
      None
      (Wlc.capture_change_block ~base_path:dir ~actor_key:"keeper-a"))
;;
...
let test_capture_distinguishes_new_changes () =
  with_temp_dir "worktree-live-context" (fun dir ->
    ...
    Wlc.clear_status_cache_for_tests ();
    let second = Wlc.capture_change_block ~base_path:dir ~actor_key:"keeper-a" in
    check bool "new change produces new block" true (Option.is_some second);
    let block = Option.value ~default:"" second in
    check
      bool
      "block mentions second file"
      true
      (try
         ignore (Str.search_forward (Str.regexp_string "other.ml") block 0);
         true
       with
       | Not_found -> false))
;;
...
let test_capture_change_block_in_eio_runtime () =
  with_temp_dir "worktree-live-context" (fun dir ->
    ...
    with_eio_runtime (fun () ->
      ...
      check
        (option string)
        "same change not repeated in eio"
        None
        (Wlc.capture_change_block ~base_path:dir ~actor_key:"keeper-a")))
;;
...
let test_git_status_timeout_defaults_to_30_seconds () =
  with_env "MASC_WORKTREE_GIT_STATUS_TIMEOUT_SEC" "" (fun () ->
    check (float 0.01) "default git status timeout" 30.0 (Wlc.git_status_timeout_sec ()))
;;

let test_git_status_timeout_honors_env_override () =
  with_env "MASC_WORKTREE_GIT_STATUS_TIMEOUT_SEC" "12.5" (fun () ->
    check (float 0.01) "env override" 12.5 (Wlc.git_status_timeout_sec ()))
;;
  ...
  run
    ...
    [ ( "capture_change_block"
      , [ test_case "only on change" `Quick test_capture_only_on_change
        ; test_case
            "distinguishes new changes"
            `Quick
            test_capture_distinguishes_new_changes
        ; test_case "works in Eio runtime" `Quick test_capture_change_block_in_eio_runtime
        ; test_case
            "status cache uses no optional locks"
            `Quick
            test_current_status_lines_uses_short_cache_and_no_optional_locks
        ; test_case
            "status cache keeps clean status"
            `Quick
            test_current_status_lines_caches_clean_status
        ; test_case
            "git status timeout defaults to 30s"
            `Quick
            test_git_status_timeout_defaults_to_30_seconds
        ; test_case
            "git status timeout honors env override"
            `Quick
