...
let mk_cand ?(url = "http://test/" ^ "x") ?(w = 1) name = { name; url = url ^ name; w }
...
let stub_capacity table url =
  try Some (List.assoc url table) with
  | Not_found -> None
;;
...
let mk_ctx
  ...
  : S.signal_ctx
  =
  { health; capacity; now; rand_int = rand; keeper_name; cascade_name }
;;

let mk_t ?(cycle = S.default_cycle_policy) ?(tiers = []) ?(sticky_ttl_ms = 0) kind : S.t =
...
let test_failover_preserves_order () =
  ...
  check (list string) "input order preserved" [ "a"; "b"; "c" ] (names ordered)
;;
...
let test_failover_filters_cooldown () =
  ...
  check
    (list string)
    "cooldown candidate removed, remaining order preserved"
    [ "b"; "c" ]
    (names ordered)
;;
...
let test_capacity_aware_filters_busy () =
  ...
  check
    (list string)
    "busy 'a' filtered, 'b' and 'c' (unknown) kept"
    [ "b"; "c" ]
    (names ordered)
;;
...
let test_capacity_aware_unknown_passes () =
  ...
  check
    (list string)
    "unknown capacity → all kept (fail-open)"
    [ "a"; "b" ]
    (names ordered)
;;
...
let test_weighted_random_deterministic_with_rand0 () =
  ...
  let cands = [ mk_cand ~w:30 "a"; mk_cand ~w:50 "b"; mk_cand ~w:20 "c" ] in
  let ctx = mk_ctx ~rand:(fun _ -> 0) () in
  let strat = mk_t S.Weighted_random in
  let ordered = S.order_candidates strat ~adapter ~ctx ~cycle:0 cands in
  check (list string) "rand=0 picks left-to-right" [ "a"; "b"; "c" ] (names ordered)
;;
...
let test_weighted_random_all_cooldown_yields_empty () =
  ...
  let strat = mk_t S.Weighted_random in
  let ordered = S.order_candidates strat ~adapter ~ctx ~cycle:0 cands in
  check (list string) "all-cooldown → empty" [] (names ordered)
;;
...
let test_cb_cycling_excludes_cooldown_and_busy () =
  ...
  let cands = [ mk_cand "a"; mk_cand "b"; mk_cand "c" ] in
  let table =
    [ (List.nth cands 1).url, mk_capacity_info ~total:1 ~active:1 (* "c" unknown → kept *)
    ]
  in
  let ctx = mk_ctx ~health:h ~capacity:(stub_capacity table) () in
  let strat =
    mk_t
      S.Circuit_breaker_cycling
      ~cycle:{ max_cycles = 3; backoff_base_ms = 100; backoff_cap_ms = 1000 }
  in
  let ordered = S.order_candidates strat ~adapter ~ctx ~cycle:0 cands in
  check
    (list string)
    "cooldown 'a' + busy 'b' filtered, 'c' (unknown) kept"
    [ "c" ]
    (names ordered)
;;
...
let test_cb_cycling_starvation_guard () =
  ...
  check
    (list string)
    "all-busy cooled list → fall through non-empty"
    [ "a"; "b" ]
    (names ordered)
;;
...
let test_backoff_zero_at_cycle_zero () =
  check
    int
    "cycle 0 → 0ms (no sleep before first attempt)"
    0
    (S.backoff_ms S.default_cycle_policy ~cycle:0)
;;
...
let test_backoff_exponential_capped () =
  ...
  check int "cycle 4 → cap (would be 800)" 500 (S.backoff_ms p ~cycle:4);
  check int "cycle 30 → cap (would overflow)" 500 (S.backoff_ms p ~cycle:30)
;;
...
let test_parse_kind_known () =
  let check_ok s expected =
    ...
    | Ok k -> check string ("parse " ^ s) (S.kind_to_string expected) (S.kind_to_string k)
...
let test_parse_kind_unknown () =
  ...
  | Error msg ->
    check
      bool
      "error mentions the rejected name"
      true
      (String.length msg > 0
       &&
       ...
       let rec loop i =
         if i + nlen > hlen
         then false
         else if String.sub msg i nlen = needle
         then true
         else loop (i + 1)
       in
       loop 0)
...
let test_client_capacity_acquire_release () =
  ...
  | Some release ->
    ...
    release ();
    (match C.capacity "http://x:11434" with
     | Some info -> check int "active = 0 after release" 0 info.process_active
...
let test_client_capacity_release_idempotent () =
  ...
  | Some release ->
    release ();
    release ();
    (* second release must be a no-op, not underflow *)
    (match C.capacity "http://y:11434" with
     | Some info -> check int "active = 0 not -1" 0 info.process_active
     | None -> fail "capacity disappeared")
;;
...
let test_client_capacity_unregistered_url () =
  ...
  check
    (option int)
    "capacity = None for unregistered URL"
    None
    (Option.map (fun (i : T.capacity_info) -> i.total) (C.capacity "http://nope:9999"));
  check
    bool
    "try_acquire = None for unregistered URL"
    true
    (C.try_acquire "http://nope:9999" = None)
;;

let test_client_capacity_clamp_max () =
  C.unregister_all ();
  C.register ~url:"http://z:11434" ~max_concurrent:0;
  (* clamped up to 1 *)
  match C.capacity "http://z:11434" with
  | None -> fail "register did not register"
  | Some info -> check int "max_concurrent <=0 clamped to 1" 1 info.total
;;

let test_ollama_auto_register () =
  C.unregister_all ();
  C.auto_register_for_candidates
    ~base_urls:
      [ "http://127.0.0.1:11434"
      ; "http://glm.example.com/api"
      ; (* not ollama: 11434 not in URL *)
        "http://other:11434/api" (* ollama-like *)
      ];
  let urls = C.registered_urls () in
  check bool "127.0.0.1:11434 registered" true (List.mem "http://127.0.0.1:11434" urls);
  check bool "other:11434 registered" true (List.mem "http://other:11434/api" urls);
  check
    bool
    "glm.example.com NOT registered"
    false
    (List.mem "http://glm.example.com/api" urls)
;;
...
let test_ollama_register_with_override () =
  ...
  | Some info -> check int "override max=4" 4 info.total
;;
...
let test_cli_auto_register_filters_sentinels () =
  ...
  let urls = C.registered_urls () in
  check bool "cli:claude_code registered" true (List.mem "cli:claude_code" urls);
  check bool "cli:gemini_cli registered" true (List.mem "cli:gemini_cli" urls);
  check
    bool
    "http URL NOT registered as CLI"
    false
    (List.mem "http://127.0.0.1:8085" urls);
  check bool "empty key NOT registered" false (List.mem "" urls)
;;

let test_cli_register_with_override () =
  C.unregister_all ();
  C.auto_register_cli_with_override ~capacity_keys:[ "cli:codex_cli" ] ~max_concurrent:3;
  match C.capacity "cli:codex_cli" with
  | None -> fail "expected CLI registration"
  | Some info -> check int "CLI override max=3" 3 info.total
;;

let test_cli_acquire_blocks_at_cap () =
  C.unregister_all ();
  C.auto_register_cli_with_override ~capacity_keys:[ "cli:claude_code" ] ~max_concurrent:1;
  match C.try_acquire "cli:claude_code" with
  | None -> fail "first acquire should succeed"
  | Some release ->
    check
      bool
      "second acquire returns None at cap"
      true
      (C.try_acquire "cli:claude_code" = None);
    release ();
    check
      bool
      "after release: capacity available again"
      true
      (C.try_acquire "cli:claude_code" <> None)
;;
...
let test_cli_idempotent_registration () =
  C.unregister_all ();
  C.auto_register_cli_with_override ~capacity_keys:[ "cli:gemini_cli" ] ~max_concurrent:5;
  C.auto_register_cli_for_candidates ~capacity_keys:[ "cli:gemini_cli" ];
  (* should be no-op *)
  match C.capacity "cli:gemini_cli" with
  | None -> fail "expected registration"
  | Some info -> check int "first override preserved (idempotent)" 5 info.total
;;
...
let test_priority_tier_picks_first_tier () =
  let cands = [ mk_cand "a"; mk_cand "b"; mk_cand "c" ] in
  let strat = mk_t S.Priority_tier ~tiers:[ [ "a" ]; [ "b"; "c" ] ] in
  let ctx = mk_ctx () in
  let ordered = S.order_candidates strat ~adapter ~ctx ~cycle:0 cands in
...
let test_priority_tier_capacity_filter () =
  let cands = [ mk_cand "a"; mk_cand "b" ] in
  let table = [ (List.nth cands 0).url, mk_capacity_info ~total:1 ~active:1 ] in
  let strat = mk_t S.Priority_tier ~tiers:[ [ "a"; "b" ] ] in
  let ctx = mk_ctx ~capacity:(stub_capacity table) () in
  let ordered = S.order_candidates strat ~adapter ~ctx ~cycle:0 cands in
...
let test_priority_tier_starvation_guard () =
  ...
  check
    (list string)
    "all-busy tier → fall through with tier list"
    [ "a"; "b" ]
    (names ordered)
;;
...
let test_sticky_expires_after_ttl () =
  ...
  check (list string) "expired → fall back to full list" [ "a"; "b" ] (names ordered)
;;
...
let test_sticky_pinned_provider_missing_falls_back () =
  ...
  let ordered = S.order_candidates strat ~adapter ~ctx ~cycle:0 cands in
  check (list string) "missing pin → fall back to full list" [ "a"; "b" ] (names ordered)
;;
...
let test_round_robin_singleton_no_op () =
  ...
  check int "cursor stays at 0" 0 (Cascade_state.peek_round_robin ~cascade:"singleton")
;;
...
let test_round_robin_per_cascade_cursor () =
  ...
  check (list string) "cas-y has its own cursor (still 0)" [ "a"; "b" ] (names r_y)
;;
...
let test_history_record_snapshot_roundtrip () =
  CH.clear ();
  CH.record { ts = 1000.0; key = "cli:claude_code"; kind = Acquired; active_after = 1 };
  CH.record { ts = 1001.0; key = "cli:claude_code"; kind = Released; active_after = 0 };
  CH.record
    { ts = 1002.0
  ...
  | [ e0; e1; e2 ] ->
    ...
    check bool "newest kind = Rejected_full" true (e0.kind = CH.Rejected_full);
    check bool "middle kind = Released" true (e1.kind = CH.Released);
    check bool "oldest kind = Acquired" true (e2.kind = CH.Acquired)
...
let test_history_ring_buffer_drops_oldest () =
  ...
  for i = 0 to cap + 4 do
    CH.record { ts = float_of_int i; key = "cli:x"; kind = Acquired; active_after = i }
  done;
  ...
  check (float 0.0) "oldest retained ts = 5 (earlier 5 dropped)" 5.0 oldest.ts
;;

let test_history_snapshot_kind_filter () =
  CH.clear ();
  CH.record { ts = 1.0; key = "cli:claude_code"; kind = Acquired; active_after = 1 };
  CH.record
    { ts = 2.0; key = "http://127.0.0.1:11434"; kind = Acquired; active_after = 1 };
  CH.record
    { ts = 3.0; key = "http://other.example/api"; kind = Rejected_full; active_after = 0 };
  CH.record { ts = 4.0; key = "cli:gemini_cli"; kind = Released; active_after = 0 };
  ...
  List.iter
    (fun e ->
       check string "cli filter matches classify_key" "cli" (CH.classify_key e.CH.key))
    cli_events;
...
let test_history_try_acquire_records_events () =
  ...
  | Some release ->
    ...
    check bool "second acquire hits cap" true (C.try_acquire "cli:claude_code" = None);
    release ();
    let events = CH.snapshot () in
    ...
    (match events with
     | [ r; f; a ] ->
       ...
       check bool "middle = Rejected_full" true (f.kind = CH.Rejected_full);
       ...
       check string "all keys = cli:claude_code" "cli:claude_code" a.key
      ...
      (fun line ->
         ...
         let has s =
           ...
           let rec f i =
             if i + nlen > llen
             then false
             else if String.sub line i nlen = s
             then true
             else f (i + 1)
           in
           f 0
         in
         has target_kind && has target_key)
      lines
  in
  match matching with
  | [] -> None
...
let test_history_prometheus_counter_increments () =
  CH.clear ();
  let before =
    counter_value_from_text (Masc_mcp.Prometheus.to_prometheus_text ()) "acquired" "cli"
    |> Option.value ~default:0.0
  in
  CH.record { ts = 1.0; key = "cli:claude_code"; kind = Acquired; active_after = 1 };
  CH.record { ts = 2.0; key = "cli:gemini_cli"; kind = Acquired; active_after = 1 };
  CH.record
    { ts = 3.0; key = "http://127.0.0.1:11434"; kind = Rejected_full; active_after = 1 };
  let text = Masc_mcp.Prometheus.to_prometheus_text () in
  let cli_acquired =
    counter_value_from_text text "acquired" "cli" |> Option.value ~default:0.0
  in
  let ollama_rejected =
    counter_value_from_text text "rejected_full" "ollama" |> Option.value ~default:0.0
  in
  check bool "cli/acquired counter advanced by >= 2" true (cli_acquired >= before +. 2.0);
  check bool "ollama/rejected_full counter advanced by >= 1" true (ollama_rejected >= 1.0)
;;
...
let test_trace_record_snapshot_roundtrip () =
  ...
  ST.record
    (mk_trace_event
       ...
       ~candidates_out:0
       ~backoff_ms:500
       ~kind:ST.Filtered_empty
       ());
  ST.record (mk_trace_event ~ts:1002.0 ~cycle:2 ~candidates_out:0 ~kind:ST.Exhausted ());
...
let test_trace_cascade_filter () =
  ...
  List.iter (fun e -> check string "cascade filter" "big_three" e.ST.cascade_name) unified;
...
let test_trace_kind_labels () =
  ...
  check string "filtered_empty" "filtered_empty" (ST.kind_to_string ST.Filtered_empty);
...
let test_trace_prometheus_counter_increments () =
  ...
  in
  ST.record
    (mk_trace_event ~cascade_name:"big_three" ~strategy:"failover" ~kind:ST.Ordered ());
  ST.record
    (mk_trace_event ~cascade_name:"big_three" ~strategy:"failover" ~kind:ST.Ordered ());
  ST.record
    (mk_trace_event
  ...
  let filtered =
    ...
    |> Option.value ~default:0.0
  in
  check bool "big_three/failover/ordered advanced by >= 2" true (ordered >= before +. 2.0);
  check bool "nick0cave/circuit_breaker_cycling/filtered_empty >= 1" true (filtered >= 1.0)
;;
  ...
  run
    ...
    ; ( "cycle_policy"
      , [ test_case
        ...
        ; test_case "backoff zero at cycle 0" `Quick test_backoff_zero_at_cycle_zero
        ; test_case "backoff exponential, capped" `Quick test_backoff_exponential_capped
        ] )
    ; ( "parse_kind"
      , [ test_case "known kinds parse" `Quick test_parse_kind_known
        ; test_case "unknown kind returns Error with name" `Quick test_parse_kind_unknown
        ] )
    ; ( "client_capacity"
      , [ test_case "register + query" `Quick test_client_capacity_register_query
        ...
        ; test_case "release is idempotent" `Quick test_client_capacity_release_idempotent
        ; test_case
            "unregistered URL returns None"
            `Quick
            test_client_capacity_unregistered_url
        ; test_case
            "max_concurrent <= 0 clamped to 1"
            `Quick
            test_client_capacity_clamp_max
        ; test_case "auto-register matches :11434 hosts" `Quick test_ollama_auto_register
        ; test_case
            "auto_register override sets max"
            `Quick
            test_ollama_register_with_override
        ; test_case
            "cli sentinel auto-register filters non-CLI"
            `Quick
            test_cli_auto_register_filters_sentinels
        ; test_case
            "cli auto_register override sets max"
            `Quick
            test_cli_register_with_override
        ; test_case
            "cli acquire blocks at cap, releases freely"
            `Quick
            test_cli_acquire_blocks_at_cap
        ; test_case
            "cli registration is idempotent"
            `Quick
            test_cli_idempotent_registration
        ; test_case
            "snapshot returns all registered entries"
            `Quick
            test_snapshot_returns_all_entries
        ; test_case
            "snapshot reflects active acquires"
            `Quick
            test_snapshot_reflects_active_acquires
        ] )
    ; ( "priority_tier"
      , [ test_case "cycle 0 picks first tier" `Quick test_priority_tier_picks_first_tier
        ; test_case
            "cycle advances with tier index"
            `Quick
            test_priority_tier_advances_with_cycle
        ; test_case
            "cycle overflow clamps to last tier"
            `Quick
            test_priority_tier_clamps_overflow
        ; test_case
            "tier respects capacity filter"
            `Quick
            test_priority_tier_capacity_filter
        ; test_case
            "all-busy tier falls through (starvation guard)"
            `Quick
            test_priority_tier_starvation_guard
        ] )
    ; ( "sticky"
      , [ test_case
            "record_choice → pinned on next call"
            `Quick
            test_sticky_records_and_pins
        ; test_case "expires after ttl" `Quick test_sticky_expires_after_ttl
        ; test_case
            "missing pinned provider falls back"
            `Quick
            test_sticky_pinned_provider_missing_falls_back
        ; test_case "per-keeper isolation" `Quick test_sticky_per_keeper_isolation
        ] )
    ; ( "round_robin"
      , [ test_case "rotates each call" `Quick test_round_robin_rotates_each_call
        ; test_case "singleton list is no-op" `Quick test_round_robin_singleton_no_op
        ; test_case
            "per-cascade cursor isolation"
            `Quick
            test_round_robin_per_cascade_cursor
        ] )
    ; ( "cascade_state"
      , [ test_case
            "sticky ttl_ms=0 does not record"
            `Quick
            test_cascade_state_sticky_zero_ttl_no_record
        ; test_case
            "round_robin bound<=0 returns 0"
            `Quick
            test_cascade_state_round_robin_negative_bound
        ] )
    ; ( "client_capacity_history"
      , [ test_case
            "record + snapshot roundtrip newest-first"
            `Quick
        ...
        ; test_case "snapshot kind filter" `Quick test_history_snapshot_kind_filter
        ; test_case
            "try_acquire records events on registered URL"
            `Quick
            test_history_try_acquire_records_events
        ; test_case
            "record bumps Prometheus counter with label"
            `Quick
            test_history_prometheus_counter_increments
        ] )
    ; ( "strategy_trace"
      , [ test_case
            "record + snapshot newest-first"
            `Quick
            test_trace_record_snapshot_roundtrip
        ; test_case "cascade filter scopes events" `Quick test_trace_cascade_filter
        ; test_case "ring buffer drops oldest" `Quick test_trace_ring_drops_oldest
        ; test_case "limit clamp" `Quick test_trace_limit_clamp
        ; test_case "kind_to_string serialisation" `Quick test_trace_kind_labels
        ; test_case
            "record bumps Prometheus counter with labels"
            `Quick
