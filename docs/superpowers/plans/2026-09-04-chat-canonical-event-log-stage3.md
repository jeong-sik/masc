...
- **Completeness, not `Reply_details`, decides whether a log holds its turn.** `turn_log_holds_the_turn` = committed and transcript phase `Stream_ended | Stream_failed _`. A log whose stream never told it how the turn ended (the no-Eio-clock path posts without a live view; a cut stream with nothing replayed) holds part of the turn at most, so the loaded rows keep drawing it and the strict decode's reply row is appended as before. The same predicate gates the suppression rule, the reply row, and which settled blocks the renderer draws — one function, three callers.
- **The suppression rule keeps `Message_error` too**, and is stated by what the log draws rather than by what it spares: `log_draws_role` is true for `Message_keeper | Message_autonomous | Message_tool | Message_skill _ | Message_thinking`; every other role stays. A failed turn's failure is a kept error row (the strict decode's `Error` branch); the log's own failure notice lives in `status_rows`, which only the live block draws.
- **`Transcript.drawn`** is the one projection step: it flattens superseded blocks (tagged with their attempt) and reconciles the recorded reply by outcome — a `Visible_reply` equal to the current attempt's text (trimmed) leaves the stretches; a differing one replaces the current attempt's text stretches with one `Drawn_reply` where the last of them was (appended when nothing streamed); a blank `Visible_reply` or a control outcome appends one `Drawn_status`. `Transcript.reply` became a record carrying `turn_ref`, which the status sentence needs. `turn_status_text` is the shared sentence; `chat_status_text` in `masc_tui.ml` delegates to it.
...
- **Rails**: a settled block's last drawn row closes the turn (`Turn_closes`, or `Turn_alone` when it is also the first and no committed row of the request exists); a live block stays open. Edges are computed after the hidden-reasoning filter.
- **Known cost, left for 3b**: while any block is drawn, `layout_entries` is a fresh list each frame, so `clamped_scrolled_rows` re-counts rows — the same cost the pane already paid while a turn streamed, now also paid on an idle pane that holds settled turns. The memo chain rework is 3b's.
- **Scroll pin**: `rows_since_pin` is 0 only while a live block exists; settled blocks are static and are counted like committed rows after the anchor.
