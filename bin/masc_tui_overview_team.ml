...
type group = Needs_you | Working | Idle | Paused | Stopped
...
(* Where a Keeper is drawn: on a row of its own, or by name on the paused or
   the stopped line. *)
type placement = On_row of group * detail | Paused_name | Stopped_name
...
let project ~keepers ~tasks ~attention =
  let rows, paused, stopped =
    List.fold_left
      (fun (rows, paused, stopped) (keeper : Types.overview_keeper) ->
        ...
        | On_row (group, detail) ->
            ({ keeper; group; detail } :: rows, paused, stopped)
        | Paused_name -> (rows, entry :: paused, stopped)
        | Stopped_name -> (rows, paused, entry :: stopped))
      ([], [], []) keepers
  in
  let rows =
    List.stable_sort
  ...
  in
  { rows; paused = by_name paused; stopped = by_name stopped; other_holders }

let line_if_any = function [] -> 0 | _ :: _ -> 1

let drawn_rows t =
  List.length t.rows + line_if_any t.paused + line_if_any t.stopped
  + line_if_any t.other_holders
