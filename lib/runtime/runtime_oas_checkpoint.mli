...
val build_checkpoint :
  ...
  Masc_agent_core.Agent.t ->
  Masc_agent_core.Checkpoint.t
(** Build an [Masc_agent_core.Checkpoint.t] for [agent].

    When [checkpoint_sidecar] is omitted, delegates to
    [Masc_agent_core.Agent.checkpoint ~session_id agent] (the SDK's own
    checkpoint capture).

    When [checkpoint_sidecar] is supplied, builds the checkpoint
    via [Masc_agent_core.Agent_checkpoint.build_checkpoint] threading the
...
(** Synthesise an [Masc_agent_core.Types.api_response] for the early-stop
