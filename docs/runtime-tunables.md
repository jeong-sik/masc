...
## Env_config_runtime_services (25 knobs; typed classification 2/19)
...
| `MASC_DASHBOARD_FIXTURE` | string_literal | n/a | n/a | 134 | Dashboard fixture name override. |
| `MASC_DASHBOARD_FIXTURES_ENABLED` | feature_flag | n/a | n/a | 130 | Whether dashboard fixtures are enabled. Default: false. Re-readable within the process; this does not imply shell-lev... |
| `MASC_DEFAULT_RUNTIME` | string_literal | n/a | n/a | 143 | Default runtime label (e.g. "glm:pro,openai:gpt-4.1"). |
...
| `MASC_NEO4J_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 86 | Neo4j / zombie-cleanup interval (seconds). Controls the zero-zombie Pulse rhythm in the orchestrator. Clamped to >= 1... |
| `MASC_OPERATOR_CACHE_BACKGROUND_REVALIDATE` | feature_flag | n/a | n/a | 121 | Enable background revalidation when serving stale snapshots. Default: true. Disabling makes stale entries behave like... |
| `MASC_OPERATOR_CACHE_STALE_GRACE_FACTOR` | typed:float | Timeouts | operator | 115 | Stale-while-revalidate grace factor. After the TTL expires, the previous snapshot is still served for [ttl * factor] ... |
| `MASC_OPERATOR_CACHE_TTL` | typed:float | unclassified | unclassified | 107 | Operator snapshot cache TTL (seconds). Default: 30. |
| `MASC_OPERATOR_JUDGE_ENABLED` | feature_flag | n/a | n/a | 93 | Whether operator judge background loop is enabled. Default: true. |
| `MASC_OPERATOR_JUDGE_INTERVAL_SEC` | typed:int | unclassified | unclassified | 96 | Operator judge interval, clamped to >= 15s. Default: 60. |
| `MASC_OPERATOR_JUDGE_SESSION_TTL_SEC` | typed:int | unclassified | unclassified | 104 | Session TTL for operator judge cleanup, clamped to >= 30s. Default: 300. |
| `MASC_OPERATOR_JUDGE_WORKSPACE_TTL_SEC` | typed:int | Timeouts | operator | 101 | Workspace TTL for operator judge cleanup, clamped to >= 15s. Default: 60. @category Timeouts @ops_class operator |
