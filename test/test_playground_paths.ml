...
let test_sanitize_allows_safe_chars () =
  ...
  check
    string
    "already sanitized stays the same"
    "Abc-123_x.y"
    (PP.sanitize_keeper_name "Abc-123_x.y")
;;

let test_sanitize_replaces_unsafe_chars () =
  check string "slash becomes underscore" "a_b" (PP.sanitize_keeper_name "a/b");
  check string "empty string becomes single underscore" "_" (PP.sanitize_keeper_name "");
  check string "single dot is replaced with underscore" "_" (PP.sanitize_keeper_name ".");
  check
    string
    "dot-dot is replaced with double underscore (no traversal)"
    "__"
    (PP.sanitize_keeper_name "..");
  check
    string
    "path traversal with slash is neutralized"
    ".._.._etc_passwd"
    (PP.sanitize_keeper_name "../../etc/passwd");
  check
    string
    "whitespace and punctuation replaced"
    "hi_there___"
    (PP.sanitize_keeper_name "hi there!?*")
;;

let test_all_playgrounds_prefix_stable () =
  check string "canonical prefix" ".masc/playground" PP.all_playgrounds_prefix
;;

let test_bundle_root_format () =
  check
    string
    "bundle root has trailing slash"
    ".masc/playground/cheolsu/"
    (PP.bundle_root "cheolsu");
  check string "bundle root sanitizes name" ".masc/playground/a_b/" (PP.bundle_root "a/b")
;;

let test_mind_and_repos_paths () =
  check string "mind path" ".masc/playground/sangsu/mind/" (PP.mind_path "sangsu");
  check string "repos path" ".masc/playground/sangsu/repos/" (PP.repos_path "sangsu")
;;

let test_bundle_paths_order () =
  check
    (list string)
    "bundle order: root, mind, repos"
    [ ".masc/playground/k1/"; ".masc/playground/k1/mind/"; ".masc/playground/k1/repos/" ]
    (PP.bundle_paths "k1")
;;
...
let test_no_path_escape () =
  ...
  check
    bool
    "sanitized bundle stays under prefix"
    true
  ...
  check
    bool
    "no '/../' path segment remains in bundle"
    false
  ...
  check
    string
    "dot-dot as whole name is neutralized"
    ".masc/playground/__/"
    (PP.bundle_root "..");
  check
    string
    "single dot as whole name is neutralized"
    ".masc/playground/_/"
    (PP.bundle_root ".");
  check string "empty name is neutralized" ".masc/playground/_/" (PP.bundle_root "")
;;
...
let test_strip_canonical_to_short () =
  ...
  check string "sangsu canonical" "sangsu" (PP.sanitize_keeper_name "keeper-sangsu-agent")
;;
...
let test_strip_edge_cases () =
  check
    string
    "keeper-agent not stripped (inner would be empty)"
    "keeper-agent"
    (PP.sanitize_keeper_name "keeper-agent");
  check string "single-char inner name" "x" (PP.sanitize_keeper_name "keeper-x-agent");
  check
    string
    "idempotent"
    (PP.sanitize_keeper_name "masc-improver")
    (PP.sanitize_keeper_name (PP.sanitize_keeper_name "keeper-masc-improver-agent"));
  check
    string
    "different keepers stay different"
    "sangsu"
    (PP.sanitize_keeper_name "keeper-sangsu-agent");
  check
    bool
    "sangsu != cheolsu after normalize"
    true
    (PP.sanitize_keeper_name "keeper-sangsu-agent"
     <> PP.sanitize_keeper_name "keeper-cheolsu-agent")
;;
...
let test_strip_no_traversal () =
  ...
  check bool "no slash in result" true (not (String.contains result '/'));
  check bool "no backslash in result" true (not (String.contains result '\\'))
;;
...
let () =
  run
    ...
    ; ( "prefix"
      , [ test_case
            "all_playgrounds_prefix stable"
            `Quick
            test_all_playgrounds_prefix_stable
        ] )
    ; ( "paths"
    ...
    ; "security", [ test_case "no path escape" `Quick test_no_path_escape ]
    ; ( "canonical_normalize"
      , [ test_case "canonical stripped to short" `Quick test_strip_canonical_to_short
        ; test_case
            "both forms produce identical paths"
            `Quick
            test_canonical_short_path_identity
