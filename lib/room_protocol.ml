...
let task_status_matches status_filter (task : Types.task) =
  ...
  | None -> true
  | Some status -> String.equal status (Types.string_of_task_status task.task_status)
;;

let tasks ?status_filter ?(include_done = false) ?(include_cancelled = false) config =
  ...
  |> List.filter (fun (task : Types.task) ->
    ...
    | None ->
      ...
      if Types.task_status_is_done s
      then include_done
      else if Types.task_status_is_terminal s
      then include_cancelled
      else true)
;;

let task_assignee (task : Types.task) = Types.task_assignee_of_status task.task_status

let agents ?status_filter config =
  let agents =
    try Coord.get_agents_raw config with
    | Invalid_argument _ -> []
  in
  match status_filter with
  | None -> agents
...
let messages ?agent_filter ~since_seq ~limit config =
  ...
  | Some agent ->
    List.filter
      (fun (message : Types.message) -> String.equal agent message.from_agent)
      messages
;;
