...
let with_lock t f = Eio.Mutex.use_rw ~protect:true t.mutex (fun () -> f ())
...
let get_or_create_breaker t ~agent_id =
  ...
  | None ->
    let b =
      { agent_id; state = Closed; failures = []; last_check = Time_compat.now () }
    in
...
let prune_old_failures t breaker =
  ...
  { breaker with
    failures = List.filter (fun f -> f.timestamp > threshold) breaker.failures
  }
;;
...
let record_failure t ~agent_id ~reason =
  with_lock t (fun () ->
    ...
    let breaker =
      { breaker with failures = { timestamp = now; reason } :: breaker.failures }
    in
    let breaker = prune_old_failures t breaker in
    let failure_count = List.length breaker.failures in
    let breaker =
      if failure_count >= t.failure_threshold
      then (
        Log.Session.warn
          "[CircuitBreaker] OPENED for %s: %d failures"
          agent_id
          failure_count;
        { breaker with
          state =
...
let record_success t ~agent_id =
  with_lock t (fun () ->
    ...
    | Closed -> put_breaker t (prune_old_failures t breaker)
    | Open _ -> ())
;;
...
let force_open t ~agent_id ~reason ~duration_sec =
  with_lock t (fun () ->
    ...
    Log.Session.warn
      "[CircuitBreaker] FORCE-OPENED for %s: %s (%.0fs)"
      agent_id
      reason
      duration_sec)
;;
...
let get_status t ~agent_id =
  with_lock t (fun () ->
    ...
    | Some breaker ->
      let breaker = prune_old_failures t breaker in
      put_breaker t breaker;
      let state_name, open_until, open_reason =
        match breaker.state with
...
let record_failure_global ~agent_id ~reason =
  record_failure (Eio.Lazy.force global) ~agent_id ~reason
;;
