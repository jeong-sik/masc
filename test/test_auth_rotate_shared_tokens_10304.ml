...
let seed_shared_credential base_path ~agent_name ~role ~raw_token =
  match Auth.save_raw_token_credential base_path ~agent_name ~role ~raw_token with
  | Ok cred -> cred
  | Error e -> failf "seed credential failed: %s" (Types.masc_error_to_string e)
;;

let outcome_names_of (o : Auth.rotation_outcome) = List.map fst o.rotated_agents
let outcome_results_of (o : Auth.rotation_outcome) = List.map snd o.rotated_agents
...
let raw_token_value base_path agent_name =
  ...
  if Sys.file_exists path
  then In_channel.with_open_bin path (fun ic -> String.trim (In_channel.input_all ic))
  else failf "raw token file missing: %s" path
;;
...
let test_no_shared_returns_empty () =
  ...
  check int "no shared groups, no rotation" 0 (List.length outcomes);
  (* Audit confirms nothing to fix. *)
  check int "audit also reports clean" 0 (List.length (Auth.audit_token_uniqueness base))
;;
...
let test_shared_group_rotates_to_unique () =
  ...
  check int "audit sees 1 group before rotation" 1 (List.length groups_before);
  (* Rotate. *)
  let outcomes = Auth.rotate_shared_tokens base in
  check int "rotation reports 1 outcome" 1 (List.length outcomes);
  ...
  let tokens =
    [ "keeper-a"; "keeper-b"; "keeper-c" ] |> List.map (credential_token base)
  in
  check
    int
    "3 distinct token hashes after rotation"
    3
  ...
  |> List.iter (fun agent_name ->
    ...
    | Ok cred ->
      check string (agent_name ^ " raw token verifies") agent_name cred.agent_name
    | Error e ->
      failf
        "%s raw token should verify after rotation: %s"
        agent_name
        (Types.masc_error_to_string e));
  (* Audit is empty after rotation. *)
  check
    int
    "audit is clean after rotation"
    0
    (List.length (Auth.audit_token_uniqueness base))
;;
...
let test_rotation_preserves_role () =
  ...
  ignore
    (seed_shared_credential
       base
       ~agent_name:"admin-keeper"
       ~role:Types.Admin
       ~raw_token:shared);
  ignore
    (seed_shared_credential
       base
       ~agent_name:"worker-keeper"
       ~role:Types.Worker
       ~raw_token:shared);
  let _ = Auth.rotate_shared_tokens base in
  check
    string
    "admin keeper kept Admin role"
    "admin"
    (Types.agent_role_to_string (credential_role base "admin-keeper"));
  check
    string
    "worker keeper kept Worker role"
    "worker"
...
let test_two_shared_groups_sorted_by_prefix () =
  ...
  ignore
    (seed_shared_credential
       ...
       ~raw_token:token_beta);
  let outcomes = Auth.rotate_shared_tokens base in
  check int "two outcome groups" 2 (List.length outcomes);
  let prefixes =
  ...
  check (list string) "outcomes sorted by token_hash_prefix" sorted_prefixes prefixes
;;
...
let test_idempotent_after_rotation () =
  ...
  ignore
    (seed_shared_credential base ~agent_name:"x" ~role:Types.Worker ~raw_token:shared);
  ignore
    (seed_shared_credential base ~agent_name:"y" ~role:Types.Worker ~raw_token:shared);
  let _ = Auth.rotate_shared_tokens base in
  let second_outcomes = Auth.rotate_shared_tokens base in
  check int "second rotation finds nothing to rotate" 0 (List.length second_outcomes)
;;
...
let test_scoped_rotation_only_rotates_selected_agents () =
  ...
  ignore
    (seed_shared_credential
       ...
       ~raw_token:shared);
  ignore
    (seed_shared_credential base ~agent_name:"admin" ~role:Types.Admin ~raw_token:shared);
  let outcomes =
    Auth.rotate_shared_tokens_for_agents base ~agent_names:[ "keeper-a"; "keeper-b" ]
  in
  check int "one scoped outcome" 1 (List.length outcomes);
  let outcome = List.hd outcomes in
  check
    (list string)
    "only selected keepers rotated"
    [ "keeper-a"; "keeper-b" ]
    (outcome_names_of outcome);
  check
    string
    "admin kept old shared token hash"
    (Auth.sha256_hash shared)
    (credential_token base "admin");
  let keeper_tokens = [ "keeper-a"; "keeper-b" ] |> List.map (credential_token base) in
  check
    int
    "selected keepers now unique"
    2
    (List.sort_uniq String.compare keeper_tokens |> List.length);
  check
    bool
    "keeper-a no longer shares with admin"
    false
    (String.equal (credential_token base "keeper-a") (credential_token base "admin"));
  check
    bool
    "keeper-b no longer shares with admin"
    false
    (String.equal (credential_token base "keeper-b") (credential_token base "admin"))
;;

let () =
  run
    "auth_rotate_shared_tokens_10304"
    [ ( "clean-store"
      , [ test_case "no shared -> empty rotation" `Quick test_no_shared_returns_empty ] )
    ; ( "rotation"
      , [ test_case
            "shared group rotates to unique"
            `Quick
            test_shared_group_rotates_to_unique
        ; test_case "role preserved across rotation" `Quick test_rotation_preserves_role
        ] )
    ; ( "ordering"
      , [ test_case
            "two groups sorted by prefix"
            `Quick
            test_two_shared_groups_sorted_by_prefix
        ] )
    ; ( "idempotency"
      , [ test_case "second call is a no-op" `Quick test_idempotent_after_rotation ] )
    ; ( "scoped"
      , [ test_case
            "selected agents rotate without admin blast radius"
            `Quick
