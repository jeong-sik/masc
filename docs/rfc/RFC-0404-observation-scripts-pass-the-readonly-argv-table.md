...
동치다. 전개·치환·리다이렉트·파이프를 만들 수 있는 문자가 하나도 없다.
탭은 인용부가 아니지만 셸의 필드 분할(word splitting)이 토큰 경계로 쓰므로
금지 집합에 넣는다 — 리뷰가 찾은 반례: `"sed -e<TAB>-i s/a/b/ f"` 는 우리
split에서 `["-e<TAB>-i"]` 한 토큰(in-place 아님, 관측)으로 분류되지만 셸은
`["-e"; "-i"]` 로 나눠 in-place 쓰기를 활성화한다. 토큰 단위로 검사하는 플래그
가드(sed -i, rg --pre, sort -o, uniq 피연산자) 전부가 이 경로로 우회된다.
브래킷 글로브 `[` 도 일치 파일 존재 시 argv 모양을 바꾸므로 같이 금지한다. split 결과를 기존 `classify_argv` 에 그대로
