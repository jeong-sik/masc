...
let parse_present_tool_name_list_opt args key =
  ...
  | Some (`List items) ->
    let rec collect acc index = function
      ...
      | _ :: _ -> Error (Printf.sprintf "%s[%d] must be a string" key index)
    in
    collect [] 0 items
...
let parse_enum_string_opt args key of_string ~allowed_values =
  ...
  | Some (`String raw) ->
    (match of_string raw with
     ...
     | None ->
       Error (Printf.sprintf "invalid %s '%s' (allowed: %s)" key raw allowed_values))
  | Some `Null -> Error (Printf.sprintf "%s must not be null" key)
  | Some _ -> Error (Printf.sprintf "%s must be a string" key)
;;

let resolve_tool_name_list ~preferred ~fallback =
  first_some preferred fallback |> Option.value ~default:[] |> normalize_tool_name_list
;;
...
let reject_legacy_tool_access_kind access_json =
  ...
  | Some (`String ("restricted" | "unrestricted")) ->
    Error
      "tool_access.kind must be \"preset\" or \"custom\"; legacy kinds \"restricted\" \
       and \"unrestricted\" are not supported for this endpoint"
  | _ -> Ok ()
;;
...
let parse_tool_access_input (args : Yojson.Safe.t)
  ...
  then
    Error
      "tool_access cannot be combined with tool_preset, tool_also_allow, or \
       tool_custom_allowlist"
  else if tool_custom_allowlist_present && (tool_preset_present || tool_also_allow_present)
  then
    Error "tool_custom_allowlist cannot be combined with tool_preset or tool_also_allow"
  else (
    let tool_access_opt =
    ...
    | Ok tool_access_opt ->
      ...
      in
      let tool_also_allow_opt = parse_present_tool_name_list_opt args "tool_also_allow" in
...
let parse (ctx : _ context) (args : Yojson.Safe.t) : (parsed_args, tool_result) result =
  ...
  then Error (false, "invalid keeper name (allowed: [A-Za-z0-9._-])")
  else (
    ...
    | Ok () ->
      (match reject_removed_keeper_input_keys ~tool_name:"masc_keeper_up" args with
       ...
       | Ok () ->
         ...
         let active_goal_ids_opt_res =
           parse_present_string_list_opt args "active_goal_ids"
         in
         let sandbox_profile_opt_res =
           parse_enum_string_opt
             args
             "sandbox_profile"
             sandbox_profile_of_string
         ...
         in
         (match
            ( compaction_profile_opt_res
            , tool_access_input_res
            , allowed_paths_opt_res
            , active_goal_ids_opt_res
            , sandbox_profile_opt_res
            , network_mode_opt_res
          ...
          | ( Ok compaction_profile_opt
            ...
            let max_context_override_opt =
              ...
              | None -> None
              | Some v when v >= min_keeper_context && v <= max_keeper_context -> Some v
            ...
            let proactive_idle_sec_opt =
              Safe_ops.json_int_opt "proactive_idle_sec" args
            in
            let proactive_cooldown_sec_opt =
              Safe_ops.json_int_opt "proactive_cooldown_sec" args
            in
            let compaction_ratio_gate_opt =
              Safe_ops.json_float_opt "compaction_ratio_gate" args
            in
            let compaction_message_gate_opt =
              Safe_ops.json_int_opt "compaction_message_gate" args
            in
            let compaction_token_gate_opt =
              Safe_ops.json_int_opt "compaction_token_gate" args
            in
            let continuity_compaction_cooldown_sec_opt =
              Safe_ops.json_int_opt "continuity_compaction_cooldown_sec" args
            in
            let tool_denylist_opt_res =
              parse_present_tool_name_list_opt args "tool_denylist"
            in
            ...
            in
            let soul_content =
              if not (Fs_compat.file_exists soul_path)
              then (
...
let resolve_mention_targets ~mention_targets_in ~fallback_targets ~name =
  let raw =
    if mention_targets_in <> []
    then mention_targets_in
    else if fallback_targets <> []
    then fallback_targets
    else [ name ]
  in
...
let resolve_sandbox_profile ~preferred ~fallback =
  first_some preferred fallback |> Option.value ~default:default_sandbox_profile
;;
...
let resolve_shared_memory_scope ~preferred ~fallback =
  first_some preferred fallback |> Option.value ~default:default_shared_memory_scope
;;
...
let sandbox_allowed_path_within_private_root
  ...
  if trimmed = ""
  then false
  else if sandbox_allowed_path_has_forbidden_segments trimmed
  then false
  else (
    let private_root = private_workspace_root_abs ~config ~sandbox_profile keeper_name in
    let candidate =
      (if Filename.is_relative trimmed
       then Filename.concat (Keeper_alerting_path.project_root_of_config config) trimmed
       else trimmed)
      |> Keeper_alerting_path.normalize_path_for_check
      |> Keeper_alerting_path.strip_trailing_slashes
    in
    candidate = private_root || String.starts_with ~prefix:(private_root ^ "/") candidate)
;;
...
let validate_sandbox_settings
  ...
  =
  if allowed_paths = [ "*" ]
  then Error "allowed_paths=[\"*\"] is not supported; enumerate explicit paths instead"
  else if Env_config_keeper.KeeperSandbox.hard_mode () && sandbox_profile <> Docker
  then Error "MASC_KEEPER_SANDBOX_HARD_MODE requires sandbox_profile=docker"
  else if Env_config_keeper.KeeperSandbox.hard_mode () && network_mode <> Network_none
  then
    Error
      "MASC_KEEPER_SANDBOX_HARD_MODE requires network_mode=none; git/gh egress is \
       brokered by structured tools"
  else if Env_config_keeper.KeeperSandbox.hard_mode () && github_identity = None
  then Error "MASC_KEEPER_SANDBOX_HARD_MODE requires github_identity in keeper profile"
  else if
    Env_config_keeper.KeeperSandbox.hard_mode ()
    && Env_config_keeper.KeeperSandbox.relax_fs ()
  then Error "MASC_KEEPER_SANDBOX_HARD_MODE requires MASC_KEEPER_SANDBOX_RELAX_FS=false"
  else (
    match sandbox_profile with
    | Local ->
      (match network_mode with
       | Network_inherit -> Ok ()
       | Network_none -> Error "network_mode=none requires sandbox_profile=docker")
    | Docker ->
