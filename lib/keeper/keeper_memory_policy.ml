...
let observation_has_question (message : string) = String.contains message '?'

let keeper_policy_observation_of_room_message
      ~(meta : keeper_meta)
      ~(room_id : string)
      (msg : Types.message)
  : keeper_policy_observation
  =
...
let strip_prefix_ci ~(prefix : string) (s : string) : string option =
  ...
  then None
    ...
    if head = String.lowercase_ascii prefix
    then Some (String.sub s plen (String.length s - plen) |> String.trim)
    else None)
;;
      ...
      (fun acc line ->
         ...
         | None ->
           (match strip_prefix_ci ~prefix:"DONE:" line with
            | Some v ->
              { acc with
                done_summary = trim_nonempty v
              ; progress =
                  (match acc.progress with
              ...
              (match strip_prefix_ci ~prefix:"Progress:" line with
               ...
               | None ->
                 (match strip_prefix_ci ~prefix:"NEXT:" line with
                  | Some v ->
                    { acc with
                      next_summary = trim_nonempty v
                    ; next_items =
                        (match acc.next_items with
                         | [] -> split_state_items v
                         | existing -> existing)
                    }
                  | None ->
                       ...
                       (match strip_prefix_ci ~prefix:"Decisions:" line with
                        ...
                        | None ->
                          (match strip_prefix_ci ~prefix:"OpenQuestions:" line with
                           | Some v -> { acc with open_questions = split_state_items v }
                           | None ->
                             (match strip_prefix_ci ~prefix:"Constraints:" line with
                              | Some v -> { acc with constraints = split_state_items v }
                              | None -> acc))))))))
      empty_keeper_state_snapshot
      lines
  in
  if
    snapshot.goal = None
...
let forward_looking_snapshot (snapshot : keeper_state_snapshot) : keeper_state_snapshot =
...
let cap_list ~max_items ~max_item_chars items =
  ...
  |> List.map (fun item ->
    String_util.utf8_safe ~max_bytes:(max_item_chars + 3) ~suffix:"…" item
    |> String_util.to_string)
;;
...
let cap_snapshot
  ...
  ; next_items = cap_list ~max_items:max_list_items ~max_item_chars snapshot.next_items
  ; decisions = cap_list ~max_items:max_list_items ~max_item_chars snapshot.decisions
  ; open_questions =
      cap_list ~max_items:max_list_items ~max_item_chars snapshot.open_questions
  ; constraints = cap_list ~max_items:max_list_items ~max_item_chars snapshot.constraints
  }
;;
...
let progress_markdown_of_snapshot
  ...
  let header =
    ...
    ; (match updated_at with
       ...
       | _ -> "")
    ; "This file is a filesystem-first recovery cache. Re-verify live world state before \
       acting."
    ]
    |> List.filter (fun line -> String.trim line <> "")
  in
...
let short_term_prompt_text_of_snapshot (snapshot : keeper_state_snapshot) : string =
  let parts =
    [ (match snapshot.next_summary with
       | Some text when String.trim text <> "" -> Some ("Next plan: " ^ String.trim text)
       | _ -> None)
    ; (match snapshot.next_items with
       | [] -> None
       | items -> Some ("Next steps: " ^ String.concat "; " (List.map String.trim items)))
    ; (match snapshot.open_questions with
       | [] -> None
       | items ->
...
let mid_term_prompt_text_of_snapshot (snapshot : keeper_state_snapshot) : string =
  let parts =
    [ (match snapshot.goal with
       | Some text when String.trim text <> "" -> Some ("Goal: " ^ String.trim text)
       | _ -> None)
    ; (match snapshot.decisions with
       | [] -> None
       | items -> Some ("Decisions: " ^ String.concat "; " (List.map String.trim items)))
    ; (match snapshot.constraints with
       | [] -> None
       | items -> Some ("Constraints: " ^ String.concat "; " (List.map String.trim items)))
    ]
    |> List.filter_map Fun.id
  in
  match parts with
...
let progress_snapshot_cache_of_text (text : string) : progress_snapshot_cache option =
  ...
  | None -> None
  | Some snapshot -> Some { generation = progress_generation_of_text text; snapshot }
;;
...
let prompt_memory_sections_of_snapshot
  ...
  let allow_short_term =
    ...
    | None -> true
  in
  [ (if allow_short_term then short_term_prompt_text_of_snapshot snapshot else "")
...
let read_progress_snapshot ~(config : Coord.config) ~(name : string)
  ...
  match
    ...
    then None
      ...
      try progress_snapshot_cache_of_text (Fs_compat.load_file path) with
...
let continuity_fallback_summary_text
  ...
  if trimmed = ""
  then "No continuity snapshot available."
  else (
    let freshness_line =
      if last_continuity_update_ts > 0.0
      then (
      ...
      else "Freshness: unknown (last continuity update timestamp unavailable)."
    in
    String.concat
      "\n"
      ...
      ; "Treat the following as prior context only and re-verify blockers, constraints, \
         and repo state against the live world state before acting."
      ; trimmed
      ])
;;
...
let structured_working_context_of_snapshot (snapshot : keeper_state_snapshot)
  : Yojson.Safe.t
  =
  `Assoc [ "version", `Int 1; "state_snapshot", keeper_state_snapshot_to_json snapshot ]
;;

let replay_metadata_of_snapshot (snapshot : keeper_state_snapshot) : Yojson.Safe.t =
  `Assoc
...
let snapshot_of_replay_metadata (json : Yojson.Safe.t) : keeper_state_snapshot option =
  try
    let open Yojson.Safe.Util in
...
let with_snapshot_metadata (msg : Oas.Types.message) (snapshot : keeper_state_snapshot)
  : Oas.Types.message
  =
  let metadata =
...
let snapshot_of_message_metadata (msg : Oas.Types.message) : keeper_state_snapshot option =
  match List.assoc_opt replay_metadata_key msg.metadata with
  | Some json -> snapshot_of_replay_metadata json
  | None -> None
;;

let snapshot_of_message (msg : Oas.Types.message) : keeper_state_snapshot option =
  match snapshot_of_message_metadata msg with
  | Some _ as snapshot -> snapshot
  | None -> parse_state_snapshot_from_reply (Oas.Types.text_of_message msg)
;;
...
let snapshot_of_structured_working_context (json : Yojson.Safe.t)
  : keeper_state_snapshot option
  =
...
let signal_bonus ~(text : string) : int =
  ...
  in
  let keyword_bonus = if contains_any_ci text high_priority_words then 8 else 0 in
  let uncertainty_penalty = if contains_any_ci text uncertainty_words then -8 else 0 in
  keyword_bonus + uncertainty_penalty
;;

let tuned_priority_for_candidate ~(kind : string) ~(text : string) : int =
...
let synthesize_state_from_run_result
  ...
  let next_items =
    ...
    then [ "Continue previous work"; "Review results from last generation" ]
    else []
  in
  let response_hint =
...
let render_state_block (snapshot : keeper_state_snapshot) : string =
  ...
  (match snapshot.decisions with
   | [] -> ()
   | items ->
     Buffer.add_string buf (Printf.sprintf "Decisions: %s\n" (String.concat "; " items)));
  (match snapshot.open_questions with
   | [] -> ()
   | items ->
     Buffer.add_string
  ...
  (match snapshot.constraints with
   | [] -> ()
   | items ->
     Buffer.add_string buf (Printf.sprintf "Constraints: %s\n" (String.concat "; " items)));
  Buffer.add_string buf "[/STATE]";
  Buffer.contents buf
;;
