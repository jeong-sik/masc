...
| 3 | exact-output 레인 슬롯이 가리키는 provider 가 `exact-body-timeout-s` 를 선언하지 않음 → 레인·슬롯·provider 이름을 댄다 | 있다(#38779). 저장은 쓰기 전에 `Exact_slot_body_deadlines_absent` 로 거절하고, 걸린 레인·슬롯·provider 를 전부 댄다. 부팅은 멈추지 않는다. 규칙 2 처럼 degraded 로 뜬다. 그 슬롯은 레인에서 빠지고, 부팅 WARN 과 시작 보고(`runtime_startup_degradation`의 `exact_slot_body_deadline_gaps`)가 이름을 댄다. 슬롯이 다 빠진 필수 레인은 원래 규칙(`validate_required_lanes` → Exact 출력 권한 없음, 대화 런타임은 유지)을 따른다. `slots` 가 가리키는 HTTP 런타임만 보고, `cli_slots` 와 `AGENT_CORE_MODEL_CATALOG` 교체 카탈로그는 보지 않는다 |
