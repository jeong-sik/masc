...
상자 안에서 실행됐다면 이 필드는 없습니다. 그래서 이 필드가 있으면 요청의 어느
...
이 필드가 없으면 상자 실행이 아예 없었거나, 어떤 명령은 상자 안에서 실행된
것입니다. 필드가 있든 없든 위의 기준대로 요청 그 자체를 판정합니다. 상자를 만들지 못했다는 사실 자체는 거부 사유도
