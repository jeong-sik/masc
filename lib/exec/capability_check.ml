...
let head_cap (bin : Bin.t) (args : Shell_ir.arg list) : Capability.t =
  if String.equal (Bin.to_string bin) "git"
  then (
  ...
  else Capability.Exec_bin (bin, args)
;;
...
let redirect_cap = function
  ...
  | Redirect_scope.File
      { target; mode = (Redirect_scope.Write | Redirect_scope.Append) as m; _ } ->
    [ Capability.Write_path (target, m) ]
  | Redirect_scope.Fd_to_fd _ -> []
;;

let of_simple (s : Shell_ir.simple) : Capability.t list =
  let env_caps = List.map (fun (k, v) -> Capability.Env_set (k, v)) s.env in
