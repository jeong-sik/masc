...
let test_dashboard_truth_and_asset_helpers () =
  check bool "vite uses stable entry filenames" true
    (file_contains_pattern "dashboard/vite.config.ts" "entryFileNames: 'assets/[name].js'");
  check bool "vite uses stable chunk filenames" true
    (file_contains_pattern "dashboard/vite.config.ts" "chunkFileNames: 'assets/[name].js'");
  check bool "truth helper exists" true
    (file_contains_pattern "dashboard/src/components/common/truth-copy.ts" "export function provenanceLabel");
  check bool "room truth strip uses truth helper" true
    (file_contains_pattern "dashboard/src/components/common/room-truth-strip.ts" "provenanceLabel(");
  check bool "command swarm uses truth helper" true
    (file_contains_pattern "dashboard/src/components/command/swarm.ts" "sourceOfTruthLabel(")
  ...
  run "ci_hardening_source"
    [
      ("source_guard", [
           ...
           test_case "dashboard truth and stable asset helpers" `Quick test_dashboard_truth_and_asset_helpers;
         ]);
    ]
