...
| `RetryScheduled` action | the `Degraded_retry_allowed _` branch of `keeper_turn_runtime_budget.ml`'s degraded-retry classifier | ✓ exists — citation tightened to name the file/classifier |
| `ProductivePhaseExhausted` action | the `Degraded_retry_slot_phase_exhausted _` branch of the same classifier | ✓ exists — same |
| `release_at_phase` | `runtime_rotation_attempt.slot_release_at_phase : slot_release_phase option` in `keeper_execution_receipt.ml` | ✓ exists, correct field name |
...
- Spec keeps `retry_scheduled` ↔ `Retry_scheduled` and `productive_phase_exhausted` ↔ `Productive_phase_exhausted` (the two slot-release outcomes the leak invariant cares about), **drops** `retry_setup_failed` / `retry_budget_exhausted` (other terminal rotation outcomes — they also release the slot, but aren't on the leak path this spec pins), and **adds** two spec-internal markers: `none` (↔ the `option` `None` / no rotation) and `finish` (a clean productive-phase finish releases the slot — the runtime records *no* `slot_release_phase` for that case, since `slot_release_at_phase` only exists on `runtime_rotation_attempt`, which is only constructed for degraded retries).

So the spec is a *leak-relevant projection* of the runtime alphabet, plus the no-rotation / clean-finish endpoints needed to make the model a closed FSM. That's a legitimate modeling choice — but the original mapping table cited `runtime_rotation_attempt.slot_release_at_phase` as the counterpart of `release_at_phase` with no hint that the alphabets differ. A future maintainer adding `Retry_setup_failed` handling, or seeing the spec's `finish` and looking for a `Finish` constructor, would be misled.
