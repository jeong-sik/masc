...
let http_get_text_with_status_with_headers ?(timeout_sec = 10) ?(headers = []) url =
  ...
  | Unix.WEXITED code -> Error (Printf.sprintf "curl exit code %d for %s" code url)
  | Unix.WSIGNALED sig_num -> Error (Printf.sprintf "curl signal %d for %s" sig_num url)
  | Unix.WSTOPPED sig_num -> Error (Printf.sprintf "curl stopped %d for %s" sig_num url)
;;
...
let http_get_json_with_status ?(timeout_sec = 10) url =
  ...
  | Error _ as err -> err
  | Ok (http_status, payload) ->
    (try Ok (http_status, Yojson.Safe.from_string payload) with
     | Yojson.Json_error msg -> Error (Printf.sprintf "invalid json from %s: %s" url msg))
;;

let http_post_json_text_with_status_with_headers
      ~timeout_sec
      ?(headers = [])
      ~url
      ~body_json
