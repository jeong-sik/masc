...
- **Librarian 이 서도 Keeper 는 턴을 계속 돈다**(2026-09-23 운영자 결정, #37793). 요청이 읽은 위치부터 시작하면, Librarian 이 서 있는 동안 요청이 턴마다 커진다. 그 끝은 provider 의 거절이고, 그 뒤로는 매 턴 같은 거절이다. Keeper 가 턴을 돌지 못하니 I7 이 사실상 깨진다. "읽은 위치부터 보낸다"와 "건너뛰지 않는다"는 묶여 있지 않다. I3 은 Librarian 이 읽는 위치를 말하고, Keeper 요청이 어디서 시작하는지는 말하지 않는다. 그래서 이렇게 한다.
  1. 판정은 추정이 아니라 일어난 일이다. 시작점이 이번 턴의 경계보다 앞인 요청을 provider 가 크기 때문에 거절하면, 그 턴은 마지막으로 끝난 턴의 경계부터 같은 후보에 한 번 더 보낸다. 시작점이 Librarian 지점이든 아래 2의 받아들여진 시작점이든 같다. 크기 거절은 typed 생성자 둘이다: `ContextOverflow` 와 `InvalidRequest { Request_body_refused_by_provider }`. 사유를 모르는 거절(`Unknown_invalid_request`)은 크기 거절이 아니므로 그 턴을 그 거절로 끝낸다. 크기를 미리 재는 문턱도, 시간도, 실패 횟수도 쓰지 않는다. 지점이 없는 Keeper 의 씨앗이 거절될 때(`Turn_start_after_seed_refusal`, #38046)와 같은 모양이다.
