...
let provider_kind_uses_anthropic_caching
  ...
  | OpenAI_compat | Ollama | Gemini | Gemini_cli | Kimi | Kimi_cli | Glm | Codex_cli ->
    false
;;

let model_label_provider_kind label = Provider_kind_resolver.kind_of_spec label

let model_uses_anthropic_caching_with_provider_kind
      ~provider_kind
      ~(model_used : string)
...
let model_uses_anthropic_caching ~(model_used : string) ~(resolved_model_id : string)
  : bool
  =
  model_uses_anthropic_caching_with_provider_kind
    ~provider_kind:None
    ~model_used
    ~resolved_model_id
;;
...
let classify_with_provider_kind
      ~provider_kind
      ~(usage_reported : bool)
  ...
  then Usage_missing
    ...
    if model_missing model_used && model_missing resolved_model_id
    then add "missing_model_id";
    if usage.input_tokens < 0 then add "negative_input_tokens";
    if usage.output_tokens < 0 then add "negative_output_tokens";
    if usage.cache_creation_input_tokens < 0 then add "negative_cache_creation_tokens";
    if usage.cache_read_input_tokens < 0 then add "negative_cache_read_tokens";
    if usage.input_tokens = 0 && usage.output_tokens = 0
    then add "zero_token_usage_reported";
    if usage.input_tokens > absurd_token_threshold then add "input_tokens_gt_1m";
    if usage.output_tokens > absurd_token_threshold then add "output_tokens_gt_1m";
    if context_max > 0 && usage.input_tokens > context_max * 2
    then add "input_tokens_gt_2x_context_max";
