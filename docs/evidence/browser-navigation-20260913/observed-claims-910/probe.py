...
parser.add_argument('--instruction-checkout',type=pathlib.Path)
parser.add_argument('--instruction-commit')
args=parser.parse_args()
assert bool(args.instruction_checkout)==bool(args.instruction_commit), 'instruction checkout and commit must be supplied together'
...
if args.instruction_commit:
 assert args.instruction_commit=='9109071810955b29163077ba3837e586d0118739', 'unexpected instruction candidate'
 prefix='skills/browser-lanes/'
 paths=subprocess.check_output(['git','-C',str(args.instruction_checkout),'ls-tree','-r','--name-only',args.instruction_commit,'--',prefix],text=True).splitlines()
 source_package={}
 for path in paths:
  content=subprocess.check_output(['git','-C',str(args.instruction_checkout),'show',args.instruction_commit+':'+path])
  assert (args.instruction_checkout/path).read_bytes()==content, 'instruction checkout bytes differ: '+path
  source_package[path[len(prefix):]]=content
 baseline=bundle_manifest['packages']['browser-lanes']
 assert set(source_package)==set(baseline), 'instruction package inventory differs'
 modified=[name for name,content in source_package.items() if hashlib.sha256(content).hexdigest()!=baseline[name]]
 assert modified==['SKILL.md'], 'override changes more than the reviewed entry'
 instruction_override=source_package['SKILL.md']
 instruction_proof={'source_commit':args.instruction_commit,'baseline_export_source_commit':bundle_manifest['source_commit'],
  'modified_paths':modified,'old_sha256':baseline['SKILL.md'],'new_sha256':hashlib.sha256(instruction_override).hexdigest(),
  'package_sha256':{name:hashlib.sha256(content).hexdigest() for name,content in source_package.items()},
  'scope':'Filesystem instruction override only; baseline bundle.json remains the original binary export. All other package files and binaries are unchanged.'}
 (OUT/'instruction-source-proof.json').write_text(json.dumps(instruction_proof,indent=2))
...
report={'scope':'isolated three-channel website; verified live Firefox extension, native host, server and TUI; separately recorded browser instruction provenance; typed TUI default action and composition','steps':[],'cleanup':[]}
...
try:
 ...
 if instruction_override is not None:
  (installed/'SKILL.md').write_bytes(instruction_override)
  actual={str(p.relative_to(installed)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted(installed.rglob('*')) if p.is_file()}
  assert actual==instruction_proof['package_sha256']
  shutil.copytree(installed,OUT/'installed-browser-lanes')
  report['instruction_distribution']='Baseline binary-exported full package with explicitly recorded filesystem SKILL.md override'
  report['instruction_source_commit']=args.instruction_commit
  report['instruction_source_proof']=instruction_proof
 else:
  report['instruction_distribution']='Unmodified full package exported by this server binary'
 ...
 report['instruction_sha256']=hashlib.sha256((ROOT/'.masc/skills/browser-lanes/SKILL.md').read_bytes()).hexdigest()
