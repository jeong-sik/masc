...
let collect_metadata_gaps ~sessions ~keepers ~agents =
  ...
  let session_gaps =
    ...
    |> List.concat_map (fun json ->
      let session_id =
        string_field "session_id" json
        |> fun value -> if value = "" then None else Some value
      in
      let items = ref [] in
      if string_field "goal" json = "unassigned"
      then
        items
        := metadata_gap_json
             ...
             ~severity:"watch"
           :: !items;
      if string_field "communication_mode" json = "unknown"
      then
        items
        := metadata_gap_json
             ~kind:"session_communication_mode_missing"
  ...
  let keeper_gaps =
    ...
    |> List.filter_map (fun json ->
      ...
      then
        Some
          (metadata_gap_json
             ~kind:"keeper_last_reply_missing"
  ...
  let agent_gaps =
    agents
    |> List.filter_map (fun json ->
      if string_field "assignment_status" json = "unassigned" && agent_needs_focus json
      then
        Some
          (metadata_gap_json
             ~kind:"agent_focus_missing"
...
let gap_kinds_for_section = function
  | Communication -> [ "session_communication_mode_missing"; "keeper_last_reply_missing" ]
  | Alignment -> [ "session_goal_missing"; "agent_focus_missing" ]
  | Watch -> []
;;
