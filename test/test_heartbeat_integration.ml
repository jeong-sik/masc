...
let test_keeper_shutdown_delivers_dead_tombstone_completion_after_receipt () =
  ...
  let completion_contract =
    Masc.Masc_event_bus_subscription.for_subscriber
      Masc.Masc_event_bus_subscription.Keeper_lifecycle_listener
  in
  ...
  Fun.protect
    ~finally:(fun () ->
      Masc.Agent_sdk_metrics_bridge.unsubscribe
        completion_bus
    ...
    (fun () ->
      ...
      check int
        ...
        0
        (List.length
           (Masc.Agent_sdk_metrics_bridge.drain completion_subscription));
      check
        bool
