...
- Keeper: a board comment notification row that reports `new_replies_since_own=N` also carries `new_replies_comment_offset`, the `comment_offset` at which `masc_board_post_get` starts on the oldest of those N replies, with `oldest_new_reply_id` and `newest_new_reply_id`, so a Keeper can tell whether anything came after the reply it last read and read from there. The row stays the same size however long the thread grows (#36485, #36504).
