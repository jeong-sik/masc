...
- hitl 프롬프트 쪽 별건. `judge.md` 의 `effect` 슬롯 는 "JSON 으로만 답하라" 를 마지막 줄에 추상적으로 두고, `librarian.md` 는 첫 줄에 구체적으로 둔다. 같은 조건에서 지시를 앞으로 옮기니 qwen 4/5→5/5, minimax 3/5→4/5 였다. 표본이 작아 단정하지 않았고, 통로를 바꾸면 이 문제 자체가 사라진다.
