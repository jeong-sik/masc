...
| `Jsonl_atomic` | ~~Eio writer with per-canonical-path `Eio.Mutex`, scoped `fs`, long-lived sink, explicit close.~~ Removed 2026-07-18: production fan-in was 0 (no caller required a long-lived Eio writer), so the conditional-keep judgment resolved to deletion. `Fs_compat.append_jsonl` remains the single append SSOT. | — | — | — |
...
| Keeper single-file logs | `.masc/keepers/<name>.memory.jsonl`, `.decisions.jsonl`, `.feedback.jsonl`, `.generation_index.jsonl`, `.tla-trace.jsonl` | `Keeper_types_support.append_jsonl_line` | keeper memory/status/recall/detail readers | Per-keeper modules | Preserve rotation and UTF-8 repair for these paths. |
| Board posts/comments/reactions/sub-boards | `.masc/board_posts.jsonl`, `board_comments.jsonl`, `board_reactions.jsonl`, `board_sub_boards.jsonl` | create-only append for new rows; snapshot rewrite via `save_file_atomic` for mutation flushes | `Board_core` load/list, `Activity_feed` projections | `Board_core` serializers | Exact current writer shapes only: required fields and exact types; unknown or duplicate fields are rejected. These are snapshot-like JSONL files after mutation. |
| Board votes | `.masc/board_votes.jsonl` | append on cast; snapshot rewrite on flush | board vote state and analytics | `Board_votes` | Exact current row: canonical `target`, matching derived `voter`, exact `up`/`down`, positive finite float `ts`. Snapshot rewrites preserve the cast timestamp. |
