...
let metrics_summary_to_json (s : metrics_summary) : Yojson.Safe.t =
  ...
  let memory_compaction_drop_avg =
    if s.memory_compaction_events = 0
    then 0.0
    else
      float_of_int s.memory_compaction_dropped_notes
  ...
  `Assoc
    ...
    ; ( "last_handoff"
      , match s.last_handoff with
        | Some j -> j
        | None -> `Null )
    ; ( "last_compaction"
      , match s.last_compaction with
        | Some j -> j
        | None -> `Null )
    ]
;;
  ...
  List.fold_left
    (fun acc line ->
       try
         ...
         let is_scheduled_autonomous = is_scheduled_autonomous_channel channel in
         ...
         let drift_applied_now = Safe_ops.json_bool ~default:false "applied" drift in
         ...
         ; guardrail_stop_count =
             (acc.guardrail_stop_count
              + if is_interaction && guardrail_stop_now then 1 else 0)
         ; drift_applied_count =
             (acc.drift_applied_count
              + if is_interaction && drift_applied_now then 1 else 0)
         ; handoff_count =
         ...
         ; memory_compaction_before_notes =
             (acc.memory_compaction_before_notes
              +
              if is_interaction && memory_compaction_performed
              then memory_compaction_before_now
              else 0)
         ; memory_compaction_dropped_notes =
             (acc.memory_compaction_dropped_notes
              +
              if is_interaction && memory_compaction_performed
              then memory_compaction_dropped_now
              else 0)
         ; memory_compaction_invalid_dropped =
             (acc.memory_compaction_invalid_dropped
              +
              if is_interaction && memory_compaction_performed
              then memory_compaction_invalid_now
              else 0)
         ; memory_checks =
         ...
         ; memory_correction_applied =
             (acc.memory_correction_applied
              +
              if is_interaction && memory_performed && memory_correction_applied
              then 1
              else 0)
         ; memory_correction_success =
             (acc.memory_correction_success
              +
              if is_interaction && memory_performed && memory_correction_success
              then 1
              else 0)
         ; memory_score_sum =
         ...
         ; memory_weather_passed =
             (acc.memory_weather_passed
              +
              if is_interaction && memory_performed && memory_is_weather && memory_passed
              then 1
              else 0)
         ; repetition_risk_sum =
         ...
         ; response_alignment_sum =
             (acc.response_alignment_sum
              +.
              if is_interaction
              then Option.value ~default:0.0 response_alignment_opt
              else 0.0)
       ...
       with
       | Yojson.Json_error _ | Yojson.Safe.Util.Type_error _ -> acc)
...
let action_source_opt_member json =
  ...
  | None ->
    (match Yojson.Safe.Util.member "deliberation_execution" json with
     | `Assoc _ as nested -> Safe_ops.json_string_opt "action_source" nested
     | _ -> None)
;;
...
let latest_tool_audit_snapshot_from_decisions config keeper_name =
  ...
  then None
    ...
    let parse_snapshot line =
      ...
      | Yojson.Json_error _ -> None
    in
    latest_snapshot_of_lines lines ~parse_snapshot ~has_legacy_shape:(fun json ->
    ...
    |> Option.map (fun snapshot ->
      { snapshot with
        tool_audit_source =
          Some (Option.value ~default:"keeper_decision_log" snapshot.tool_audit_source)
      }))
;;
...
let latest_tool_audit_snapshot_from_metrics config keeper_name =
  ...
  let parse_snapshot line =
    try
      let json = Yojson.Safe.from_string line in
      let tools =
        json_string_list_member "tools_used" json |> List.sort_uniq String.compare
      in
      let raw_tool_call_count = json_int_opt_member "tool_call_count" json in
      let tool_call_count =
        match raw_tool_call_count with
  ...
  |> Option.map (fun snapshot ->
    { snapshot with
      tool_audit_source =
        Some (Option.value ~default:"keeper_metrics" snapshot.tool_audit_source)
    })
;;
...
let accountability_summary_json config ~keeper_name ~agent_name =
  Keeper_accountability.accountability_summary_json config ~keeper_name ~agent_name
;;
