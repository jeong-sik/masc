...
(** Issue #8449 PR C: HTTP query-param sort_by parser. Delegates to
    [Masc_board_handlers.Board_dispatch.sort_order_of_string_opt] instead of duplicating
...
let board_sort_order_of_request request =
  ...
  | None -> Masc_board_handlers.Board_dispatch.Hot
  | Some sort ->
    (match Masc_board_handlers.Board_dispatch.sort_order_of_string_opt sort with
     | Some s -> s
     | None -> Masc_board_handlers.Board_dispatch.Hot)

(** Issue #8449 PR C: thin alias over the Variant SSOT helper. *)
let board_sort_label = Masc_board_handlers.Board_dispatch.sort_order_to_string
...
let board_current_vote_for_post ~voter ~post_id =
  ...
  | None -> None
  | Some voter -> (
      match Masc_board_handlers.Board_dispatch.current_vote_for_post ~voter ~post_id with
      | Ok vote -> Some vote
      | Error _ -> Some None)
...
let board_current_vote_for_comment ~voter ~comment_id =
  ...
  | None -> None
  | Some voter -> (
      match Masc_board_handlers.Board_dispatch.current_vote_for_comment ~voter ~comment_id with
      | Ok vote -> Some vote
      | Error _ -> Some None)
...
let board_reactions_for_post ~voter ~post_id =
  match
    Masc_board_handlers.Board_dispatch.list_reactions ~target_type:Board.Reaction_post
...
let board_reactions_for_comment ~voter ~comment_id =
  match
    Masc_board_handlers.Board_dispatch.list_reactions ~target_type:Board.Reaction_comment
...
let board_reactions_batch ~targets ~voter =
  match targets with
  | [] -> []
  | _ -> Masc_board_handlers.Board_dispatch.list_reactions_batch ~targets ?user_id:voter ()
...
let board_moderation_fields ~include_moderation ~target_kind ~target_id =
  if not include_moderation then []
    ...
    let summary = Masc_board_handlers.Board_moderation.target_summary ~target_kind ~target_id in
    [
      ("report_count", `Int summary.Masc_board_handlers.Board_moderation.report_count);
      ("moderation_status", `String summary.Masc_board_handlers.Board_moderation.moderation_status);
    ]
      ...
      `Assoc
        (fields
         ...
         @ board_moderation_fields ~include_moderation
             ~target_kind:Masc_board_handlers.Board_moderation.Target_comment
...
let board_post_dashboard_json ?(include_moderation = false)
  ...
  let base_fields =
    match Masc_board_handlers.Board_dispatch.post_to_yojson_with_karma p ~author_karma with
    | `Assoc fields -> fields
    | _ -> []
  in
  let fields =
  ...
  `Assoc
    ( fields
      ...
      @ board_moderation_fields ~include_moderation
          ~target_kind:Masc_board_handlers.Board_moderation.Target_post
          ~target_id:post_id
