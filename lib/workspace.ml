...
(* #9795: wire the FSM drift hook to a Prometheus counter emit.
   [Workspace_task.transition] calls the hook whenever
   [Workspace_task_lifecycle.decide] returns a [drift]; this side
   puts the signal on
...
let () = Atomic.set Workspace_hooks.fsm_drift_observer_fn record_fsm_drift_with_agent
...
let () = Atomic.set Workspace_hooks.task_auto_release_observed_fn record_task_auto_release

(* Workspace broadcast latency histogram and file lock retry/duration metrics.
...
let () = Atomic.set Workspace_hooks.workspace_broadcast_observed_fn record_workspace_broadcast

(* RFC-0040: route Mention_dedup decision counts to Prometheus.
   Default no-op in [Workspace_hooks] is replaced at startup so the
   workspace layer keeps zero static dep on Prometheus. *)
...
let () = Atomic.set Workspace_hooks.mention_dedup_decision_fn record_mention_dedup_decision
...
(* #9645: distributed lock acquire exhaustion observability.
   ...
   [Workspace_utils_ops.with_distributed_lock]/[..._r] now signal
   exhaustion via [Workspace_hooks.distributed_lock_acquire_failed_fn].
...
let () =
  Atomic.set
    Workspace_hooks.distributed_lock_acquire_failed_fn
...
(* Activity graph emit — wraps Activity_graph for workspace sub-modules *)
let () =
  Atomic.set
...
let () = Atomic.set Workspace_hooks.relation_on_leave_fn Relation_materializer.on_agent_session_ended
...
let () =
  Atomic.set
    Workspace_hooks.observe_agent_lifecycle_fn
...
let () =
  Atomic.set
    Workspace_hooks.observe_task_transition_fn
...
(* Subscription auto-subscribe on session binding — wraps Subscriptions for workspace_eio *)
let () =
...
let () = Atomic.set Workspace_hooks.tool_assigned_fn Tool_assignment_telemetry.emit_assigned
