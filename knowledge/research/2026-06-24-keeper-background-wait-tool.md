...
핵심 구분: **"sustained fiber" ≠ "sustained in-memory context".** LLM 메시지 히스토리는 fiber 메모리에 상주하지 않고 매 turn 디스크 OAS checkpoint에서 재로드된다(`keeper_agent_run.ml:51, :262-263`). 연속성은 `read_continuity_summary`가 checkpoint의 `[STATE]` 블록 → 없으면 meta fallback(`continuity_summary`/`progress_snapshot`)을 골라 재주입한다(`keeper_world_observation_continuity.ml:39-103`).
