...
report={'scope':'isolated three-channel website; binary-exported MASC callable composition joins navigation with visible content; persistent native TUI','steps':[],'cleanup':[]}
...
try:
 ...
 composition=args.bundled_skills.resolve()/'browser-navigate-content'
 installed=ROOT/'.masc/skills/browser-navigate-content'
 if installed.exists() or installed.is_symlink():installed.rename(OUT/'previous-browser-navigate-content')
 shutil.copytree(composition,installed)
 installed_hashes={str(p.relative_to(installed)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted(installed.rglob('*')) if p.is_file()}
 assert installed_hashes==bundle_manifest['packages']['browser-navigate-content'], 'installed composition differs from binary export'
