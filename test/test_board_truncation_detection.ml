...
let signal_eq a b =
  Tool_board.truncation_signal_to_string a = Tool_board.truncation_signal_to_string b
;;

let signal_pp ppf s = Format.fprintf ppf "%s" (Tool_board.truncation_signal_to_string s)
...
let test_complete_text_no_signal () =
  check
    ...
    "complete sentence"
    None
    (detect "Hello, this is a complete sentence.")
;;
...
let test_balanced_inline_tick_no_signal () =
  check
    (option signal_t)
    "balanced inline `let x` ticks"
    None
    (detect "Use `let` and `in` together.")
;;

let test_double_asterisk_in_inline_code_no_signal () =
  check
    (option signal_t)
    "glob in balanced inline code"
    None
    (detect "Run `ls **/*.ml` before submitting.")
;;
...
let test_underscore_in_identifier_not_truncation () =
  ...
  check
    ...
    None
    (detect "See snake_case_var and another_one.")
;;
...
let test_paren_in_prose_not_unfinished_link () =
  ...
  check
    ...
    None
    (detect "(this is a parenthetical aside)")
;;
...
let test_odd_double_asterisk_outside_inline_code_triggers () =
  check
    (option signal_t)
    "unclosed bold after inline glob"
    (Some Tool_board.Odd_double_asterisk)
...
let () =
  run
    "board truncation detection (#9777)"
    [ ( "negative"
      , [ test_case "complete text" `Quick test_complete_text_no_signal
        ...
        ; test_case
            "double asterisk in inline code"
            `Quick
            test_double_asterisk_in_inline_code_no_signal
        ; test_case
            "underscore identifier"
            `Quick
            test_underscore_in_identifier_not_truncation
    ...
    ; ( "positive"
      , [ test_case "odd fence" `Quick test_odd_fence_triggers
        ...
        ; test_case
            "odd double asterisk outside inline code"
            `Quick
            test_odd_double_asterisk_outside_inline_code_triggers
