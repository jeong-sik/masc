...
- **Librarian 이 서도 Keeper 는 턴을 계속 돈다**(2026-09-23 운영자 결정, 기록: #37793 issuecomment-5788668419). 요청이 읽은 위치부터 시작하면, Librarian 이 서 있는 동안 요청이 턴마다 커진다. 그 끝은 provider 의 거절이고, 그 뒤로는 매 턴 같은 거절이다. Keeper 가 턴을 돌지 못하니 I7 이 사실상 깨진다. "읽은 위치부터 보낸다"와 "건너뛰지 않는다"는 묶여 있지 않다. I3 은 Librarian 이 읽는 위치를 말하고, Keeper 요청이 어디서 시작하는지는 말하지 않는다. 그래서 이렇게 한다.
  ...
  6. 경계부터 보낸 요청도 크기로 거절되면 그 턴은 그 거절로 끝난다. 그때 넘치는 것은 이력이 아니라 매 턴 붙는 부분이다(창 RFC §13.9 첫 항목).
  ...
  - 구현 상태: 아직 없다. #38046(병합됨)과 #38109 위에 쌓는다. 순서는 #38109 의 도구 결과 마커 재전송이 먼저이고, 그래도 크기로 거절되면 1 의 경계 재전송이다. `Librarian_stalled` 를 그리는 곳은 이 구현 PR 이 Memory 화면 밀림 표시(#37840)와 같은 자리에 둔다.
