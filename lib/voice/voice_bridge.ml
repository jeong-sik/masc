            ...
            (fun (endpoint : Voice_config.endpoint) ->
              let outcome =
                ...
                then Skipped "disabled in the configuration"
                  ...
                  (* One flattener for both ways of getting a transcript: an
                     endpoint that answers JSON and one that prints text end in
                     the same report line, and a probe that read them
                     differently would compare two shapes. *)
                  let flatten text =
                    let spoken =
                      String.trim
                        (String.map
                  ...
                  | Voice_config.Macos_say -> Skipped "this endpoint speaks and does not listen"
                  | Voice_config.Whisper_cli ->
                    (match
