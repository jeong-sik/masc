(** Keeper_exec_status — health/diagnostic state classification,
    quiet-hours logic, and surface status helpers.
    Metrics summary aggregation is in Keeper_exec_status_metrics.
    Type conversion helpers and pipeline stage mapping are in
    [Keeper_exec_status_helpers]. *)

open Keeper_types

(** Type conversion helpers and pipeline stage mapping extracted to
    [Keeper_exec_status_helpers].
    Health classification, diagnostics, and surface status below. *)

include Keeper_exec_status_helpers
