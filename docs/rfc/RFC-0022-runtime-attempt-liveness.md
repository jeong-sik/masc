...
```
attempt killed by this RFC  ⇒  runtime FSM advances *before*
                               RFC-0012 watchdog observes idle.
```
The attempt-level kill must always be faster than the turn-level kill
or the turn dies before the runtime gets a chance to recover.
...
1. **MAX_S converges to ~3600s for 11/14 keepers.** Latency is bounded
   ...
   (default 3600s). The runtime exhausts the entire turn budget on a
   single bad attempt.
...
keeper_turn (timeout = 3600s)
  └─ runtime.run                    (no timeout, no liveness)
...
A grep across `lib/runtime/runtime_fsm.ml`, `lib/runtime/runtime_strategy.ml`,
`lib/runtime/runtime_runtime.ml` for the keywords `timeout|deadline|with_clock`
returns **zero matches**. Runtime has no notion of attempt liveness.
...
- t₀: keeper turn starts, runtime slot 1 (cli-tool-a:spark) is called.
...
- t₁ < t < t₀+3600s: provider holds the connection without yielding
  ...
  `now - started_at > 3600`.
- t₀+~1170s or t₀+~3600s: HTTP layer or turn-cap times out → runtime
...
typed runtime failure (not a generic timeout):
...
c.kind = Done  ⇒  no further chunks accepted
                  runtime FSM observes Done → success
```
...
### 4.6 Wiring into existing runtime FSM

`runtime_fsm.ml` already has variants for advancing on failure. Wire
...
| File | Change |
|---|---|
| `lib/runtime/runtime_attempt_liveness.ml` (new) | FSM, decision table, telemetry hooks |
| `lib/runtime/runtime_attempt_liveness.mli` (new) | export `liveness_budget`, `step`, `Outcome.t` |
| `lib/runtime/runtime_runtime.ml` | wrap each provider call in liveness gate; report typed `Failed_attempt` |
| `lib/runtime/runtime_fsm.ml` | accept `Failed_attempt` as advance trigger; preserve trust hook |
...
| `test/test_runtime_attempt_liveness.ml` (new) | property tests (§9) |
| `tla/RuntimeAttemptLiveness.tla` (new) | spec from §5 |
| `tla/RuntimeAttemptLiveness.cfg` (new, clean) | INVARIANT LivenessKillsFastEnough |
| `tla/RuntimeAttemptLiveness-buggy.cfg` (new, mutation) | NextBuggy |
| `docs/operations/runtime-liveness-tuning.md` (new) | how operators calibrate thresholds against `diag-keeper-cycle.sh` output |
...
- FSM module + tests in PR-1 (`lib/runtime/runtime_attempt_liveness.{ml,mli}`).
- Observer + tick fiber + `oas_worker_named.ml::try_provider` integration in PR-2 (`lib/runtime/runtime_attempt_liveness_observer.{ml,mli}`, `lib/runtime/runtime_attempt_liveness_config.{ml,mli}`).
- Behind `MASC_RUNTIME_ATTEMPT_LIVENESS=off|observe|enforce`. Current runtime default is `enforce` when the env var is unset; explicit values must be canonical, and empty/unknown/boolean-style aliases raise a config error.
- `observe`: liveness clock runs, kills emit `masc_runtime_attempt_liveness_kill_total{mode=observe}` and `masc_runtime_attempt_liveness_observed_total{outcome=...}` but no `Switch.fail`; runtime still advances on real wire errors.
...
- Flat global lowering of `MASC_KEEPER_TURN_TIMEOUT_SEC` to a single
  value below the per-runtime design floor. RFC-0012 §Out of scope
...
- **Permitted (per-runtime override, added 2026-05-06)**: a runtime
  ...
  `masc_runtime_attempt_liveness_kill_total{mode="observe",kind="wall_exceeded"}`
  ...
  mapped as follows: `turn_timeout` is the resolved runtime
  `turn_timeout_sec` (or `MASC_KEEPER_TURN_TIMEOUT_SEC` fallback);
  `oas_guard` is `Keeper_turn_runtime_budget.oas_timeout_guard_sec`;
  `admission_wait` is
...
1. **Thinking-only providers without protocol heartbeat.** If a
   ...
   heartbeat is standard on all currently-runtimed providers; if a
...
| Formal | TLC on `RuntimeAttemptLiveness.cfg` (clean) | `INVARIANT LivenessKillsFastEnough` pass |
| Formal (mutation) | TLC on `-buggy.cfg` | invariant violated |
| Empirical pre | `diag-keeper-cycle.sh` snapshot | recorded as baseline |
| Empirical post Phase A | `diag-keeper-cycle.sh` 24h | counter `runtime_attempt_liveness_kill_total{mode=observe}` non-zero on at least one provider; no behaviour change |
...
- `lib/runtime/runtime_fsm.ml`, `runtime_strategy.ml`, `runtime_runtime.ml` (current runtime modules — `timeout|deadline|with_clock` keyword count: 0)
- `lib/runtime/runtime_health_filter.ml` (104 LOC) — pre-attempt prune; complementary, not overlapping
- `lib/runtime/runtime_health_tracker.ml` (723 LOC) — RFC-0009 trust state, will receive new failure-class taxonomy
