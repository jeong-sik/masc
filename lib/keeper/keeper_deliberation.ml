...
let rec deliberation_action_to_json = function
  | Noop reason -> `Assoc [ "type", `String "noop"; "reason", `String reason ]
...
let triage_result_to_json = function
  | Skip reason -> `Assoc [ "decision", `String "skip"; "reason", `String reason ]
  | Triggered triggers ->
    `Assoc
      [ "decision", `String "triggered"
      ; "triggers", `List (List.map deliberation_trigger_to_json triggers)
      ]
;;
...
let triage (obs : world_observation) : triage_result =
  ...
  if obs.board_mention_count > 0 then add (BoardActivity "mentioned_in_post");
  if obs.idle_seconds > obs.idle_gate && obs.active_goal_count > 0 then add IdleTimeout;
  if obs.active_goal_count > 0 && obs.idle_seconds > obs.idle_gate * 2
  then add GoalDeadline;
  if obs.idle_seconds > obs.idle_gate * 5 && obs.active_goal_count > 0
  then add StrategicReview;
  ...
  if
    obs.board_new_post_count > 0
    && obs.idle_seconds >= obs.idle_gate / 2
    && obs.board_mention_count = 0
  then add (BoardActivity "new_posts");
  if obs.active_goal_count = 0 && obs.idle_seconds > obs.idle_gate * 4
  then add SelfDirectedExplore;
...
let deliberation_meta_of_json (json : Yojson.Safe.t) : deliberation_meta =
  { deliberation_count = Safe_ops.json_int ~default:0 "deliberation_count" json
  ; deliberation_cost_total_usd =
      Safe_ops.json_float ~default:0.0 "deliberation_cost_total_usd" json
  ; last_deliberation_ts = Safe_ops.json_float ~default:0.0 "last_deliberation_ts" json
  ; last_triage_triggers = Safe_ops.json_string ~default:"" "last_triage_triggers" json
  }
;;
...
let deterministic_baseline_action (obs : world_observation) : deliberation_action =
  if obs.direct_mention
  then ReplyInRoom { room_id = ""; content = "" }
  else Noop "no_trigger"
;;
...
let build_deliberation_prompt
      ...
      (obs : world_observation)
  : string
  =
  let multi_step_line =
    "\n\
     - multi_step: Execute multiple actions sequentially (max 5). Requires steps array \
     of action objects."
  in
...
{"action":"multi_step","params":{"steps":[{"action":"task_claim","params":{"task_id":"task-1","reason":"urgent"}},{"action":"broadcast","params":{"message":"Claimed task-1"}}]},"reasoning":"Claim and announce","confidence":0.7}|}
  ...
  match
    Prompt_registry.render_prompt_template
      Keeper_prompt_names.deliberation
...
let action_source_to_json source = `String (action_source_to_string source)
...
let rec policy_labels_of_action = function
  | MultiStep actions -> List.concat_map policy_labels_of_action actions
  | action -> [ deliberation_action_to_policy_label action ]
;;
...
let has_room_signal (obs : world_observation) = obs.direct_mention || obs.has_question
...
let rec legality_error (obs : world_observation) = function
  | Noop _ -> None
  | ReplyInRoom _ ->
    if has_room_signal obs
    then None
    else Some "reply_in_room requires direct mention or a question"
  | BoardPost _ ->
    if has_board_signal obs || obs.active_goal_count > 0 || is_self_directed obs
    then None
    else Some "board_post requires board activity, active goals, or self-directed context"
  | BoardComment _ ->
    if has_board_signal obs then None else Some "board_comment requires board activity"
  | BoardVote _ ->
    if has_board_signal obs then None else Some "board_vote requires board activity"
  | TaskClaim _ ->
    if obs.unclaimed_task_count > 0
    then None
    else Some "task_claim requires unclaimed tasks"
  | Broadcast _ ->
    if has_operational_signal obs
    then None
  ...
  | StartDiscussion _ ->
    ...
    then None
      ...
      Some
        "start_discussion requires board activity, active goals, or self-directed context"
  | ShareFinding _ ->
    if has_board_signal obs || obs.active_goal_count > 0 || is_self_directed obs
    then None
    else
      Some "share_finding requires board activity, active goals, or self-directed context"
  | MultiStep actions ->
    (match actions with
     ...
     | _ ->
       let rec loop index = function
         ...
         | MultiStep _ :: _ ->
           Some
             (Printf.sprintf "multi_step step %d cannot contain nested multi_step" index)
         | action :: rest ->
           (match legality_error obs action with
            | Some msg -> Some (Printf.sprintf "multi_step step %d illegal: %s" index msg)
...
let action_source_of_execution_result (result : execution_result) = result.action_source
...
let execute_structured_result (obs : world_observation) (result : structured_result)
  : execution_result
  =
...
let clamp_confidence raw_conf = max 0.0 (min 1.0 raw_conf)
...
let rec parse_action_from_json (json : Yojson.Safe.t)
  ...
  | "reply_in_room" ->
    ...
    if content = ""
    then Error "reply_in_room requires non-empty content"
  ...
  | "multi_step" ->
    ...
    (match steps_json with
     ...
     | items ->
       let max_steps = 5 in
       let truncated =
         if List.length items > max_steps
         then (
       ...
       in
       let results =
         List.map (fun step_json -> parse_action_from_json step_json) truncated
       in
       let errors =
         List.filter_map
           (function
       ...
       then
         Error
           (Printf.sprintf
              "multi_step contains invalid actions: %s"
         ...
         in
         if has_nested
         then Error "multi_step cannot contain nested multi_step actions"
...
let structured_result_of_json (json : Yojson.Safe.t) : (structured_result, string) result =
  match parse_action_from_json json with
  | Error msg -> Error msg
  | Ok action ->
    let reasoning = Safe_ops.json_string ~default:"" "reasoning" json in
    let confidence =
...
let structured_result_schema : structured_result Oas.Structured.schema =
  ...
  ; description =
      "Choose exactly one typed keeper deliberation action and return only the tool \
       input object."
  ; params =
      [ { Oas.Types.name = "action"
        ; description =
            "One of: noop, reply_in_room, task_claim, broadcast, board_post, \
             board_comment, board_vote, propose_spawn, start_discussion, share_finding, \
             multi_step."
...
let parse_deliberation_response (raw : string)
  ...
  | Ok json ->
    ...
    |> Result.map (fun result -> result.action, result.reasoning, result.confidence)
;;
