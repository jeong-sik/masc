...
let default_max_tokens = Llm_provider.Constants.Inference_profile.agent_default.max_tokens
...
let find_cascade_eviction_candidate counters =
  StringMap.fold
    (fun name (counter : cascade_counter) best ->
       match best with
       | None -> Some { name; calls = counter.calls; last_used_at = counter.last_used_at }
       | Some current ->
         if
           counter.calls < current.calls
           || (counter.calls = current.calls
               && counter.last_used_at < current.last_used_at)
         then Some { name; calls = counter.calls; last_used_at = counter.last_used_at }
         else best)
    counters
    None
;;
  ...
  | Some raw ->
    ...
    let rec loop idx = function
      ...
      | cfg :: rest ->
        ...
        if selected = candidate_label || selected = candidate_model
        then Some idx
...
let cascade_observation_of_candidates
      ~cascade_name
      ?strategy
      ~configured_labels
      ~(candidate_cfgs : Llm_provider.Provider_config.t list)
  ...
  let primary_model =
    match candidate_models with
    | first :: _ -> Some first
    | [] -> None
  in
  let selected_index = selected_index_of_model ~selected_model_raw ~candidate_cfgs in
  let selected_model =
...
let cascade_fallback_event_to_json (event : cascade_fallback_event) : Yojson.Safe.t =
  `Assoc
...
let update_first_attempt_if ~predicate ~update attempts_rev =
  let rec loop = function
    ...
    | attempt :: rest ->
      ...
      then Some (update attempt :: rest)
      else Option.map (fun rest' -> attempt :: rest') (loop rest)
  in
  loop attempts_rev
;;

let record_attempt_start
...
let record_fallback_event
  ...
  <- { from_model_id = from_model
     ; from_model_label = model_label_option_of_model_id ~candidate_cfgs from_model
...
let cascade_metrics_for_candidates
      ...
      ()
  =
  let capture = { next_attempt_index = 0; attempts_rev = []; fallback_events_rev = [] } in
  let metrics =
...
let cascade_observation_with_metrics
      ~cascade_name
      ?strategy
      ~configured_labels
      ~(candidate_cfgs : Llm_provider.Provider_config.t list)
...
let cascade_observation_to_json (obs : cascade_observation) : Yojson.Safe.t =
  `Assoc
    ...
    ; "attempts", `List (List.map cascade_attempt_to_json obs.attempts)
    ; ( "fallback_events"
      , `List (List.map cascade_fallback_event_to_json obs.fallback_events) )
...
let get_cascade_audit_store store_opt =
  ...
  | None ->
    ...
    (match Dated_jsonl.create ~base_dir:dir () with
     ...
     | exception exn ->
       Log.Misc.warn "cascade audit store creation failed: %s" (Printexc.to_string exn);
       None)
;;
...
let record_cascade_audit store_opt ~now ~cascade_name ~observation ~outcome =
  ...
  | Some store ->
    (try
     ...
     | exn ->
       Log.Misc.warn
         "cascade audit append failed cascade=%s error=%s"
         cascade_name
...
let distribution_json map =
  StringMap.fold
    (fun model count acc -> `Assoc [ "model", `String model; "count", `Int count ] :: acc)
    map
    []
...
let handle_record state ~now ~cascade_name ~observation ~outcome =
  ...
  let counter, counters, evicted =
    ...
    | None ->
      let counters_after_evict, evicted =
        if StringMap.cardinal counters >= cascade_max_keys
        then (
  ...
  let counter =
    ...
    | Some obs ->
      let counter =
        ...
        ; fallback_calls =
            (if obs.fallback_applied
             then counter.fallback_calls + 1
             else counter.fallback_calls)
      ...
      let counter =
        ...
        | Some model when String.trim model <> "" ->
          { counter with
            selected_models = increment_counter counter.selected_models model
          }
        | _ -> counter
      in
      List.fold_left
        (fun c attempt ->
           ...
           let c =
             { c with attempted_models = increment_counter c.attempted_models model }
           in
           match attempt.error with
           | Some _ ->
             { c with errored_models = increment_counter c.errored_models model }
           | None -> c)
        counter
        obs.attempts
    | None -> counter
  ...
  Option.iter
    (fun candidate ->
       Log.Misc.warn
         "cascade metrics evicted key=%s calls=%d last_used_at=%.3f to admit %s \
          (limit=%d)"
         candidate.name
         candidate.calls
  ...
  { counters = StringMap.add cascade_name counter counters
    ...
    StringMap.fold
      (fun name (c : cascade_counter) acc ->
         ...
         `Assoc
           ...
           ; "last_attempts", `List (List.map cascade_attempt_to_json c.last_attempts)
           ; ( "last_fallback_events"
             , `List (List.map cascade_fallback_event_to_json c.last_fallback_events) )
...
let run_actor () =
  let rec loop state =
    ...
    | Get_metrics_json p ->
      ...
      loop state
    | Reset_counters_for_test -> loop { counters = StringMap.empty; audit_store = None }
  in
  loop { counters = StringMap.empty; audit_store = None }
;;

let start_actor_if_needed ~sw = Eio.Fiber.fork ~sw run_actor
...
let reset_cascade_counters_for_test () = Eio.Stream.add stream Reset_counters_for_test
