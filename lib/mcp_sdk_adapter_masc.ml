...
let sdk_owned_methods = [ "ping" ]
let handles_method method_ = List.mem method_ sdk_owned_methods
...
let make_context ?mcp_session_id () : Handler.context =
  let send_notification ~method_ ~params =
    (match mcp_session_id with
     | Some session_id -> Sse.send_to session_id (jsonrpc_notification ?params method_)
     | None -> ());
    Ok ()
  in
  ...
  let send_progress ~token ~progress ~message ~total =
    let params =
      MP.Mcp_result.progress_to_yojson
        { MP.Mcp_result.progress_token = token; progress; total; message }
    in
...
let sdk_resource_template_of_local (template : Mcp_server.mcp_resource_template)
  : MP.Mcp_types.resource_template
  =
...
let sdk_prompt_of_local (prompt : Mcp_prompt_surface.prompt_def) =
  ...
  in
  MP.Mcp_types.make_prompt ~name:prompt.name ~description:prompt.description ~arguments ()
;;
    ...
    List.fold_left
      (fun acc resource ->
         Handler.add_resource
           ...
           (fun _ctx uri ->
              Mcp_server_eio_resource.handle_read_resource_eio
                state
                (`Int 0)
  ...
  |> List.fold_left
       (fun acc prompt ->
          Handler.add_prompt
            ...
            (fun _ctx _name arguments ->
               let args_json =
                 `Assoc (List.map (fun (key, value) -> key, `String value) arguments)
               in
               match
...
let dispatch_ping ~id = Some (Mcp_transport_protocol.make_response ~id (`Assoc []))
