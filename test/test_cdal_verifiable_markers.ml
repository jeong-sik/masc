...
let test_git_not_a_repo_exact () =
  check
    marker_list
    "single Git_not_a_repo marker"
    [ Cdal_judge.Git_not_a_repo ]
    (call `Git_not_a_repo)
;;

let test_ok_with_nothing_known_emits_empty () =
  check marker_list "unknown producer yields []" [] (call ~stdout:"hello world" `Ok)
;;
...
let test_dune_build_ok_heuristic () =
  ...
  | other ->
    fail
      ("expected Build_ok heuristic, got "
       ^ String.concat "," (List.map Cdal_judge.marker_to_string other))
;;
...
let test_pytest_banner_required () =
  ...
  let stdout = "some log output mentioning 12 passed tests in passing\n" in
  match call ~stdout `Ok with
  | [] -> ()
  | markers ->
    fail
      ("expected [] without pytest banner, got "
       ^ String.concat "," (List.map Cdal_judge.marker_to_string markers))
;;
...
let test_go_test_banner_required () =
  ...
  let stdout = "doc: the test passed successfully\nuser-visible result: PASS\n" in
  match call ~stdout `Ok with
  | [] -> ()
  | markers ->
    fail
      ("expected [] without go test preface, got "
       ^ String.concat "," (List.map Cdal_judge.marker_to_string markers))
;;
...
let test_jest_vitest_banner_required () =
  ...
  | markers ->
    fail
      ("expected [] without jest/vitest banner, got "
       ^ String.concat "," (List.map Cdal_judge.marker_to_string markers))
;;
...
let test_policy_denied_emits_nothing () =
  check marker_list "policy_denied yields []" [] (call (`Policy_denied "rm -rf /"))
;;

let test_tool_missing_emits_nothing () =
  check marker_list "tool_missing yields []" [] (call (`Tool_missing "cargo"))
;;
...
let () =
  run
    "cdal_verifiable_markers"
    [ ( "semantic_passthrough"
      , [ test_case "git_not_a_repo emits single marker" `Quick test_git_not_a_repo_exact
        ; test_case "timeout emits nothing" `Quick test_timeout_emits_nothing
        ; test_case "policy_denied emits nothing" `Quick test_policy_denied_emits_nothing
        ; test_case "tool_missing emits nothing" `Quick test_tool_missing_emits_nothing
        ] )
    ; ( "classifiers"
      , [ test_case "unknown ok → []" `Quick test_ok_with_nothing_known_emits_empty
        ...
        ; test_case "jest/vitest banner required" `Quick test_jest_vitest_banner_required
        ; test_case "git status clean" `Quick test_git_status_clean_exact
        ; test_case "git status dirty" `Quick test_git_status_dirty_exact
        ] )
    ; "wire", [ test_case "marker_to_string stable" `Quick test_marker_wire_format ]
    ]
;;
