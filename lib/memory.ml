...
let compaction_source_to_string = Keeper_memory_policy.compaction_source_to_string
...
let read ~config ~name ?max_bytes ?max_lines ?recent_limit () =
  read_summary ~config ~name ?max_bytes ?max_lines ?recent_limit ()
  |> Result.map (fun bank_summary -> make ~bank_summary ())
;;
    ...
    ; ( "error"
      , match compaction.error with
        ...
        | Some (Write_error message) ->
          `Assoc [ "write_error", `String message ]
