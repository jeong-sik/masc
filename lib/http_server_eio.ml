  ...
  let accepts_zstd (request : Httpun.Request.t) : bool =
    ...
    | Some accept_encoding ->
      ...
      |> List.exists (fun enc ->
        ...
        |> fun e -> e = "zstd" || String.sub e 0 (min 4 (String.length e)) = "zstd")
    | None -> false
  ;;
...
module Response = struct
  ...
  let json ?(status = `OK) ?(compress = true) ?(extra_headers = []) ?request body reqd =
    ...
    let response =
      Httpun.Response.create ~headers:(Httpun.Headers.of_list headers) status
    in
  ...
  let sunset_headers ~date ?successor () =
    let base = [ "Sunset", date; "Deprecation", "true" ] in
  ...
  let html_cached ?(status = `OK) ~etag ~request body reqd =
    ...
    | Some inm when String.equal inm etag_value ->
      let headers =
        Httpun.Headers.of_list [ "etag", etag_value; "cache-control", html_cache_control ]
      in
...
module Request = struct
  ...
  let respond_too_large reqd max_bytes =
    let body = Printf.sprintf "413 Request Entity Too Large (max %d bytes)" max_bytes in
    respond_error reqd `Payload_too_large body
  ;;

  let respond_internal_error reqd exn =
    let body = Printf.sprintf "500 Internal Server Error: %s" (Printexc.to_string exn) in
  ...
  let read_body_async_with_limit reqd ~on_body ~on_error =
    ...
    let stop () =
      ...
      then (
        ...
        | exn -> Log.Misc.error "[http] body close failed: %s" (Printexc.to_string exn))
    in
      ...
      let rec read_loop () =
        Httpun.Body.Reader.schedule_read
          ...
          ~on_eof:(fun () ->
            ...
            | Eio.Cancel.Cancelled _ as e -> raise e
            | exn -> on_error (`Internal exn))
          ~on_read:(fun buffer ~off ~len ->
            if !stopped
            then ()
            else (
              let next_bytes = !seen_bytes + len in
              if next_bytes > max_body_bytes
              then (
                stop ();
                on_error (`Too_large max_body_bytes))
              else (
  ...
  let read_body_async reqd callback =
    read_body_async_with_limit reqd ~on_body:callback ~on_error:(function
  ...
  let read_body_sync reqd =
    ...
    let resolve_once outcome =
      if Atomic.compare_and_set resolved false true
      then Eio.Promise.resolve resolve_result outcome
    in
    read_body_async_with_limit
      reqd
      ~on_body:(fun body_str -> resolve_once (Ok body_str))
      ~on_error:(function
        | `Too_large max_bytes ->
          respond_too_large reqd max_bytes;
          resolve_once
            (Error (Printf.sprintf "Request too large (max %d bytes)" max_bytes))
        | `Internal exn ->
  ...
  (** Get HTTP method *)
  let method_ (request : Httpun.Request.t) = request.meth

  (** Get header value *)
  let header (request : Httpun.Request.t) name = Httpun.Headers.get request.headers name
end
...
module Router = struct
  ...
  let empty : t = []
  let add ~path ~methods ~handler routes = { path; methods; handler } :: routes
  let get path handler routes = add ~path ~methods:[ `GET ] ~handler routes
  let post path handler routes = add ~path ~methods:[ `POST ] ~handler routes
  ...
  let resolve routes request =
    ...
    let path_matches =
      List.filter
        (fun route ->
           (not (String.length route.path > 7 && String.sub route.path 0 7 = "PREFIX:"))
    ...
    | None ->
      ...
      (match prefix_matches with
       ...
       | [] -> if path_matches = [] then `Not_found else `Method_not_allowed)
  ;;
...
let ready_handler _request reqd =
  ...
  let status =
    if !Server_startup_state.state.state_ready then `OK else `Service_unavailable
  in
...
let mcp_post_handler ?request_handler (request : Httpun.Request.t) (reqd : Httpun.Reqd.t) =
  ...
  | Ok body ->
    ...
    let response_mode, session_opt =
      Streamable_http.handle_post ?session_id ~body ?request_handler ()
    in
    (match response_mode with
     ...
     | Streamable_http.Sse_upgrade ->
       Response.text
         ...
         "SSE upgrade is not supported on this transport"
         reqd
...
let mcp_get_handler request reqd =
  ...
  | Ok session ->
    ...
    let json =
      Yojson.Safe.to_string
        (`Assoc
            [ "session_id", `String session.id; "transport", `String "streamable_http" ])
    in
    Response.json json reqd
  | Error msg -> Response.text ~status:`Bad_request msg reqd
;;
...
let default_routes =
  ...
  |> Router.get "/" (fun _req reqd -> Response.text "MASC MCP Server" reqd)
;;

let with_streamable_mcp_request_handler ~request_handler routes =
  let replace_post_mcp route =
    if String.equal route.Router.path "/mcp" && List.mem `POST route.Router.methods
    then { route with Router.handler = mcp_post_handler ~request_handler }
    else route
  in
  ...
  if has_post_mcp
  then rewritten
  else Router.post "/mcp" (mcp_post_handler ~request_handler) rewritten
;;
  ...
  then (
    ...
    | Some value ->
      (match Otel_trace_context.parse value with
       | Some ctx ->
         let scope =
           Opentelemetry.Scope.make ~trace_id:ctx.trace_id ~span_id:ctx.parent_id ()
         in
...
let run ~sw ~net ~clock config routes =
  ...
  in
  if String.equal effective_host config.host
  then
    Printf.printf
      "🚀 MASC MCP Server listening on http://%s:%d\n"
      effective_host
      config.port
  else
    Printf.printf
      "🚀 MASC MCP Server listening on http://%s:%d (configured=%s, parse failed → \
       loopback)\n"
      effective_host
      config.port
      config.host;
...
let start ?(config = default_config) ?(routes = default_routes) () =
  ...
  let initiate_shutdown signal_name =
    if not (Atomic.get shutdown_initiated)
    then (
  ...
  with
  | Shutdown -> Log.Http.info "MASC MCP: Shutdown complete."
  | Eio.Cancel.Cancelled _ -> Log.Http.info "MASC MCP: Shutdown complete."
;;
