...
implementation_prs: []
withdrawn_reason: "Tier-group concept removed by PR #19381."
---
...
`runtime_catalog_runtime_resolve.ml` 의 `normalize_declared_name` 와 `profile_lookup` 사이의 auto-normalize 경로가 raw short form (`"strict_tool_candidates"`) 을 `"runtime.strict_tool_candidates"` 로 silent prefix-prepend 한다. 이것은 RFC-0058 이 정의한 "routes → runtime" canonical path 를 우회하는 사이드 채널이다 — caller 가 short string 을 직접 던지면 route table 을 통째로 걸러뛴다.
...
1. **typed dedup**: `runtime_id` 을 closed-sum `Runtime_name.t` 로 lift 하여 canonical path 와 short form 을 한 타입 안에서 표현. auto-normalize 는 *parser* 에서 한 번, 이후 모든 downstream 은 canonical path 만 본다.
...
`lib/runtime/runtime_catalog_runtime_resolve.ml:264-273` (origin/main, 2026-05-23):

```ocaml
let normalized =
  match
    [ trimmed; "runtime." ^ trimmed; "tier." ^ trimmed ]
...
이 코드의 동작: `"strict_tool_candidates"` 를 입력받으면 `"strict_tool_candidates"`, `"runtime.strict_tool_candidates"`, `"tier.strict_tool_candidates"` 를 *순차 시도* 하여 첫 match 를 반환. 결과적으로 caller 가 `"strict_tool_candidates"` 라고 던지면 `"runtime.strict_tool_candidates"` 로 자동 확장되어 동작한다.
...
auto-normalize 와 string classifier 는 표면적으로 다른 파일의 다른 문제처럼 보이지만, **같은 root** 에서 온다: `runtime_id` 이나 `error_message` 가 *raw string* 으로 전달되며, 그 의미가 문자열 조작(string prefix/suffix/containment) 에 의존한다. typed sum 으로 lift 하면 둘 다 해결된다.
...
### 3.1 Typed runtime name — `Runtime_name.t`

```ocaml
(* lib/runtime/runtime_id.mli — NEW MODULE *)

(** Canonical runtime name with no silent auto-normalization.
...
(** Parse a raw runtime name.
    - Accepts: "runtime.X", "tier.X", "route.X" (canonical prefixes)
...
`runtime_catalog_runtime_resolve.ml` 의 `profile_lookup` 호출 전단계를 수정:
...
(* AFTER (Phase A) *)
match Runtime_name.of_string raw_name with
| Ok name ->
    let canonical = Runtime_name.to_string name in
    (match profile_lookup snapshot.profiles canonical with
     | Some profile -> Ok (snapshot, canonical, profile)
     | None -> Error (Printf.sprintf "unknown runtime_id %S" canonical))
| Error `Invalid_prefix ->
    Error (Printf.sprintf
             "runtime_id %S lacks required prefix (runtime|tier|route)"
             raw_name)
| Error `Empty ->
    Error "runtime_id is empty"
```

auto-prefix 리스트 `[trimmed; "runtime." ^ trimmed; ...]` 와 `List.find_opt` 제거. 단일 시도, 명시적 실패.
...
| Phase | 작업 | 측정 |
|---|---|---|
| **A** | `Runtime_name` 모듈 신설. `runtime_catalog_runtime_resolve.ml` 에 `Runtime_name.of_string` 적용 — short form caller 는 동시에 canonical path 로 교체. `dune build @all` 통과. | (a) 기존 short form caller 0개 확인 (`rg '"strict_tool_candidates"' lib/` 가 0 match); (b) unit test: `of_string "runtime.X"` -> Ok, `of_string "X"` -> Error |
...
| **C** | `Runtime_name.t` 를 다른 `runtime_id` consumer 로 확산 — `keeper_turn_driver.ml`, `runtime_attempt_fsm.ml`, `runtime_capability_profile.ml` 등. | 각 consumer 파일에서 `string` 타입의 `runtime_id` 인자가 `Runtime_name.t` 로 교첵. 컴파일 오류 = 미변경 사이트 식별 |
...
| 3 | "PR #N only fixed K of M sites" | **No** | 단일 boundary (`Runtime_name.of_string`) + 단일 consumer 교체 (`keeper_health_probe.ml`) |
| 4 | catch-all `_ ->` 추가 | **No** | `Runtime_name.of_string` 의 `result` 타입이 모든 오류를 명시. `_ ->` 없음 |
...
`Runtime_name.t` 를 `Printf.sprintf` 에 넣거나 `Yojson.Safe.t` 로 직렬화할 때 `to_string` 호출이 번거로울 수 있다.
...
1. `rg 'List.find_opt.*runtime\|List.find_opt.*tier\.' lib/runtime/` 0 match — auto-normalize 코드 완전 제거
2. `rg 'contains.*strict_tool_candidates' lib/keeper/` 0 match — 문자열 분류기 제거
3. `Runtime_name.of_string "strict_tool_candidates"` -> `Error `Invalid_prefix` — unit test 로 고정
4. `dune build @all` 및 `dune build @runtest` 통과
5. (선택) `Runtime_name.t` 를 사용하는 consumer 파일 수 / 전체 `runtime_id` consumer 파일 수 >= 0.5
...
| **typed 도입** | `Provider_capability.t`, `Runtime_candidate_skip_reason.t` | `Runtime_name.t`, `Keeper_error_classify` typed sum |
| **관계** | sister — 같은 "capability boundary 를 typed 로 lift" 방향 | sister — 같은 "capability boundary 를 typed 로 lift" 방향 |

두 RFC 는 독립적으로 merge 가능. 순서: RFC-0163 Phase A (route canonicalization) 먼저 -> RFC-0157 Phase A (pre-dispatch gate wiring) 가 `Runtime_name.t` 를 그대로 사용.
