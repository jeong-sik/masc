...
let post_keeper_run_next ~host ~port ~keeper_name ~request_id =
  ...
  | Ok (`Assoc fields) ->
    (match List.assoc_opt "request_id" fields, List.assoc_opt "prioritized" fields,
      List.assoc_opt "signalled" fields, List.assoc_opt "detail" fields with
     | Some (`String echoed), Some (`Bool true), Some (`Bool signalled), Some (`String detail) when echoed = request_id ->
       Ok (if signalled then detail else detail ^ "; the current turn was not stopped")
...
(** GET /api/v1/dashboard/keeper-costs -- per-Keeper cost sums over the last
    [window_minutes]. *)
let fetch_keeper_costs ~(host : string) ~(port : int) ~(window_minutes : int) :
    (Yojson.Safe.t, string) result =
  get_json ~host ~port
    ~path:
      (Printf.sprintf "/api/v1/dashboard/keeper-costs?window=%d" window_minutes)
