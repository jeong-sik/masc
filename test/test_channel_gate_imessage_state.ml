...
let test_connector_json_keeps_redacted_guid () =
  ...
  with_imessage_paths dir (fun () ->
    ...
    check string "connector id" "imessage" (json |> U.member "connector_id" |> U.to_string);
    check
      string
      "reply mode surfaced"
      "self-chat"
...
let () =
  run
    ...
    [ ( "status"
      , [ test_case
            "status json redacts self-chat guid"
            `Quick
            test_status_json_redacts_self_chat_guid
        ; test_case
            "connector json keeps redacted self-chat guid"
            `Quick
