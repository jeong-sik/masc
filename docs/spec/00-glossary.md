...
**Usage**
에이전트의 컨텍스트 윈도우 소비율(%). 관측 및 provider capacity 진단에
쓰이며 compaction, handoff, Pause, Stop을 허가하지 않는다.
...
MASC 자체 `lib/context_budget_manager.mli` 모듈은 purge됐다. OAS는 정확한
finite run과 typed provider capacity/overflow를 보고하고, MASC가 Keeper
owner lane의 compaction과 reinjection을 소유한다.

**Compression Phase** (retired)
세션 레벨 4단계 압축(`None_phase`/`Compact_tools`/`Drop_low`/`Summarize`)과
importance scorer/reducer bridge는 제거됐다. 이를 OAS reducer나 새로운
deterministic fallback으로 대체하지 않는다.

**Keeper Compaction**
OAS는 자동 reducer, truncation, compaction policy, overflow retry를
소유하지 않는다. 목표 계약에서 MASC는 이를 durable owner-lane
operation으로 수행하고 canonical message/tool values와 active anchors를
...
**OAS (OCaml Agent SDK)**
MASC가 finite Agent 실행에 사용하는 SDK 라이브러리. 프로젝트:
`workspace/yousleepwhen/oas` (agent_sdk). Agent.run, typed provider/runtime,
tools, reasoning, multimodal values, hooks, and execution receipts를 제공한다.
Keeper lifecycle, durable owner lane, Gate, compaction, Memory는 MASC 소유다.
