...
(** Return a copy of the given [Unix.environment]-shaped array with only
    allowed keys retained, followed by the entries that make a keeper
    subprocess non-interactive -- a command that would wait for a person fails
    instead, because nothing in this process can answer an editor or a
    credential prompt. Entries that do not contain ['='] are kept iff their key
    is allowed. *)
