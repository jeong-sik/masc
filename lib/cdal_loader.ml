...
let load_error_to_string = function
  ...
  | Schema_unsupported v ->
    Printf.sprintf "unsupported schema version: %d (expected %d)"
      v Oas.Cdal_proof.schema_version_current
...
let load ~(store : Oas.Proof_store.config)
    (proof : Oas.Cdal_proof.t)
    : (loaded_bundle, load_error) result =
  let open Result.Syntax in
  (* 1. Compute manifest path and read *)
  let manifest_path =
    Oas.Proof_store.manifest_path store ~run_id:proof.run_id in
  let* manifest_json =
  ...
  if manifest_proof.schema_version <> Oas.Cdal_proof.schema_version_current then
