...
echo "[3/10] masc_team_session_start (base)"
...
echo "[4/10] masc_team_session_status"
...
echo "[5/10] masc_team_session_start (target)"
...
echo "[5.5/10] verify base session still running before step"
r5b="$(call_tool 4106 "masc_team_session_status" "{\"session_id\":\"$s1\"}")"
require_ok "$r5b"

echo "[6/10] masc_team_session_step + events"
...
echo "[7/10] masc_team_session_list"
...
echo "[8/10] masc_team_session_stop (base) + report"
...
echo "[9/10] cleanup stop target"
_="$(call_tool 4112 "masc_team_session_stop" "{\"session_id\":\"$s2\",\"reason\":\"contract_done\",\"generate_report\":false}")"

echo "[10/10] leave"
_="$(call_tool 4113 "masc_leave" "{\"agent_name\":\"$AGENT_NAME\"}")"
