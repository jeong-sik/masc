...
let make_request_handler ~sw ~clock ~server_start_time:_ =
    ...
    let body =
      Yojson.Safe.to_string
        (`Assoc
            ...
            ; ( "message"
              , `String
                  "This compatibility surface was removed. Keepers and local clients \
                   should use the OAS-backed repo coordination front door." )
            ])
    in
  ...
  let h2_request_handler _client_addr h2_reqd =
    ...
    in
    let httpun_request =
      Httpun.Request.create ~headers:httpun_headers httpun_meth h2_req.target
    in
    let path = Http.Request.path httpun_request in
    let origin =
      match H2.Headers.get h2_headers "origin" with
      | Some o -> o
      | None -> "*"
    in
      ...
      | Ok body ->
        ...
        (match if_none_match with
         ...
         | _ ->
           let extra =
             [ "etag", etag_value
             ; "cache-control", dashboard_index_cache_control
    ...
    let dispatch_h2_route () =
      ...
      | `GET, p when String.equal p Server_health_paths.liveness ->
        let body =
          Yojson.Safe.to_string
            (`Assoc [ "live", `Bool true; "startup", Server_startup_state.to_yojson () ])
        in
        h2_respond_json h2_reqd body ~extra_headers:cors
          ...
          then
            ( Yojson.Safe.to_string
                (`Assoc
                    [ "ready", `Bool true
                    ; ( "phase"
                      , `String (Server_startup_state.phase_to_string current.phase) )
          ...
          with_server_state h2_reqd (fun state ->
            ...
            | Ok () ->
              h2_read_body h2_reqd (fun body_str ->
                ...
                | Ok body -> h2_respond_json h2_reqd body ~extra_headers:cors
                | Error msg ->
                  h2_respond_json
                    h2_reqd
      ...
      | `POST, "/mcp" | `POST, "/" | `POST, "/mcp/managed" ->
        ...
        let auth_result =
          match profile with
          | Server_mcp_transport_http.Full | Server_mcp_transport_http.Managed_agent ->
            ...
            | Ok () ->
              ...
              (match auth_result with
               ...
               | Ok _cred_opt ->
                 h2_read_body h2_reqd (fun body_str ->
                   match
                     Server_mcp_transport_http.validate_session_requirement
                     ...
                     (match accept_mode with
                      | Http_negotiation.Rejected ->
                        let body =
                          json_rpc_error
                            (-32600)
                            "Invalid Accept header: must include application/json and \
                             text/event-stream. Set MASC_ALLOW_LEGACY_ACCEPT=1 for \
                      ...
                      | accept_mode ->
                        ...
                        in
                        with_server_state h2_reqd (fun state ->
                          let profile = mcp_eio_profile_of_transport_profile profile in
                          let response_json =
                 ...
                 (match
                  ...
                  | Error msg ->
                    ...
                    h2_respond_json
                      ...
                      ~extra_headers:
                        (cors
                         @ mcp_headers session_id (get_protocol_version httpun_request))
                  | Ok () ->
      ...
      (* ─────────────────────────────────────────────────────────────────────
         Dashboard
         ───────────────────────────────────────────────────────────────────── *)
      | `GET, "/dashboard" | `GET, "/dashboard/" -> h2_respond_dashboard_index ()
      | `GET, "/dashboard/credits" ->
        h2_respond_html h2_reqd (Credits_dashboard.html ()) ~extra_headers:cors
      | `GET, p when is_dashboard_spa_deep_link p -> h2_respond_dashboard_index ()
      (* ─────────────────────────────────────────────────────────────────────
         GraphQL
         ───────────────────────────────────────────────────────────────────── *)
      | `GET, "/graphql" ->
        ...
        h2_respond_html
          h2_reqd
          (graphql_playground_html ~nonce)
          ~extra_headers:(csp_header :: cors)
      | `POST, "/graphql" ->
        h2_read_body h2_reqd (fun body_str ->
          with_server_state h2_reqd (fun state ->
            let response =
              Graphql_api.handle_request ~config:state.room_config body_str
            in
            let status =
      ...
      | `GET, "/api/v1/dashboard" ->
        let json =
          `Assoc
            ...
            ; ( "message"
              , `String
                  "Use /api/v1/dashboard/shell and surface-specific projection endpoints."
              )
            ]
        in
        h2_respond_json
          ...
          try
            ...
            | Ok patterns ->
              (match Anti_rationalization.save_excuse_patterns patterns with
               | Ok () -> h2_respond_json h2_reqd {|{"ok":true}|} ~extra_headers:cors
               | Error msg ->
                 let err_json =
                   Yojson.Safe.to_string
      ...
      | `GET, "/api/v1/dashboard/execution-trust" ->
        with_server_state h2_reqd (fun state ->
          let json =
            dashboard_execution_trust_http_json ~state ~sw ~clock httpun_request
          in
          h2_respond_json h2_reqd (Yojson.Safe.to_string json) ~extra_headers:cors)
      ...
      | `GET, "/api/v1/dashboard/governance" ->
        with_server_state h2_reqd (fun state ->
          let json =
            dashboard_governance_http_json
              httpun_request
              ~base_path:state.Mcp_server.room_config.base_path
          in
          h2_respond_json h2_reqd (Yojson.Safe.to_string json) ~extra_headers:cors)
      ...
      | `GET, "/api/v1/dashboard/mission/briefing" ->
        with_server_state h2_reqd (fun state ->
          let json =
            dashboard_mission_briefing_http_json ~state ~sw ~clock httpun_request
          in
          h2_respond_json h2_reqd (Yojson.Safe.to_string json) ~extra_headers:cors)
      ...
      | `GET, "/api/v1/autoresearch/loops" ->
        with_server_state h2_reqd (fun state ->
          ...
          let json = Dashboard_http_autoresearch.autoresearch_loops_json ~base_path () in
          h2_respond_json h2_reqd (Yojson.Safe.to_string json) ~extra_headers:cors)
      ...
      | _ ->
        h2_respond_text
          h2_reqd
          (Printf.sprintf "404 Not Found: %s" path)
          ~status:`Not_found
          ~extra_headers:cors
    in
