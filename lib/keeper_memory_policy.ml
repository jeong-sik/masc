...
let latest_state_snapshot_from_messages (messages : Llm_types.message list) :
    keeper_state_snapshot option =
  let rec loop (msgs : Llm_types.message list) =
    match msgs with
    | [] -> None
    | msg :: rest ->
      match parse_state_snapshot_from_reply (Llm_types.text_of_message msg) with
      | None -> loop rest
