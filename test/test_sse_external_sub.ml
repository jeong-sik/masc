...
let test_subscribe_replaces_same_id () =
  setup ();
  Eio_main.run (fun _env ->
    Masc_mcp.Sse.subscribe_external ~id:"dup-id" ~callback:(fun _ -> ()) ();
...
let test_broadcast_notifies_external () =
  ...
  Eio_main.run (fun _env ->
    ...
    Alcotest.(check bool) "event contains data" true (String.length event > 0);
    Alcotest.(check bool)
      "event has SSE format (contains 'data:')"
      true
      (try
         let _ = Str.search_forward (Str.regexp_string "data:") event 0 in
         true
       with
       | Not_found -> false);
...
let test_callback_error_does_not_crash_broadcast () =
  ...
  Eio_main.run (fun _env ->
    ...
    Alcotest.(check int)
      "healthy subscriber still got event"
      1
      (List.length !received_events);
...
let test_multiple_subscribers () =
  ...
  Eio_main.run (fun _env ->
    ...
    Masc_mcp.Sse.subscribe_external ~id:"multi-a" ~callback:(fun _ev -> incr counter_a) ();
    Masc_mcp.Sse.subscribe_external ~id:"multi-b" ~callback:(fun _ev -> incr counter_b) ();
...
let test_external_subscriber_count_linearized_under_domain_contention () =
  ...
  let count_before = Masc_mcp.Sse.external_subscriber_count_with_prefix prefix in
  Fun.protect
    ~finally:(fun () ->
    ...
    (fun () ->
       run_domains_together worker_count (fun index ->
         Masc_mcp.Sse.subscribe_external ~id:(sub_id index) ~callback:(fun _ -> ()) ());
       Alcotest.(check int)
         "count after concurrent subscribe"
         (count_before + worker_count)
         (Masc_mcp.Sse.external_subscriber_count_with_prefix prefix);
       run_domains_together worker_count (fun index ->
         Masc_mcp.Sse.unsubscribe_external (sub_id index));
       Alcotest.(check int)
         "count restored after concurrent unsubscribe"
         count_before
         (Masc_mcp.Sse.external_subscriber_count_with_prefix prefix))
;;
...
let () =
  Alcotest.run
    "SSE External Subscribers"
    [ ( "lifecycle"
      , [ Alcotest.test_case
            "subscribe and unsubscribe"
            `Quick
            test_subscribe_and_unsubscribe
        ; Alcotest.test_case "replace same id" `Quick test_subscribe_replaces_same_id
        ] )
    ; ( "broadcast"
      , [ Alcotest.test_case
            "broadcast notifies external"
            `Quick
            test_broadcast_notifies_external
        ; Alcotest.test_case
            "skips after unsubscribe"
            `Quick
            test_broadcast_skips_after_unsubscribe
        ; Alcotest.test_case
            "error does not crash broadcast"
            `Quick
            test_callback_error_does_not_crash_broadcast
        ; Alcotest.test_case "multiple subscribers" `Quick test_multiple_subscribers
        ] )
    ; ( "reaper"
      , [ Alcotest.test_case "reap dead subscribers" `Quick test_reap_dead_subscribers
        ; Alcotest.test_case
            "reap returns zero when all alive"
            `Quick
            test_reap_returns_zero_when_all_alive
        ] )
    ; ( "concurrency"
      , [ Alcotest.test_case
            "external subscriber count linearized"
            `Quick
