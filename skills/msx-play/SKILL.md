...
- `masc_msx_press` holds all `keys` together for `hold_frames`, then releases them for the rest of `frames`. Type a multi-digit command with separate ordered calls, followed by Return. Use short holds for a single press and observe at decision boundaries.
...
- `masc_msx_change_disk` changes the floppy without rebooting and preserves modified media in checkpoints. Follow the game's disk prompt and confirm it with the key the game requests.
