...
let mismatch_total ~expected ~actual =
  Prom.metric_value_or_zero
    ...
    ~labels:[ "expected_agent", expected; "actual_agent", actual ]
    ()
;;
...
let with_temp_base_path f =
  let dir =
    Filename.concat
      ...
      (Printf.sprintf "masc-test-auth-bearer-9786-%06x" (Random.bits ()))
  in
...
let save_cred base_path ~agent_name ~raw_token =
  match
    Auth.save_raw_token_credential base_path ~agent_name ~role:Types.Worker ~raw_token
  with
  | Ok _ -> ()
  | Error e ->
    Alcotest.failf
      "failed to seed credential %s: %s"
      agent_name
...
let test_cross_agent_mismatch_advances_counter () =
  ...
  let result = Auth.verify_token dir ~agent_name:"agent-a-9786" ~token:"token-b-raw" in
  (match result with
   | Error (Types.Unauthorized _) -> ()
   | Error e ->
     Alcotest.failf "expected Unauthorized, got: %s" (Types.masc_error_to_string e)
...
let test_valid_token_no_mismatch () =
  ...
  in
  let result =
    Auth.verify_token dir ~agent_name:"agent-valid-9786" ~token:"token-valid"
  in
  Alcotest.(check bool) "verify Ok" true (Result.is_ok result);
  Alcotest.(check (float 0.0001))
...
let test_wrong_token_no_other_owner_does_not_count () =
  ...
  in
  let result =
    Auth.verify_token dir ~agent_name:"agent-c-9786" ~token:"completely-unseen-token-abc"
  in
...
let () =
  Alcotest.run
    "auth_bearer_mismatch_9786"
    [ ( "mismatch-counter"
      , [ Alcotest.test_case
            "cross-agent mismatch advances"
            `Quick
            test_cross_agent_mismatch_advances_counter
        ; Alcotest.test_case
            "valid token: no counter movement"
            `Quick
            test_valid_token_no_mismatch
        ; Alcotest.test_case
            "wrong token but no other owner: no counter"
            `Quick
            test_wrong_token_no_other_owner_does_not_count
        ; Alcotest.test_case
            "distinct (expected, actual) pairs"
            `Quick
