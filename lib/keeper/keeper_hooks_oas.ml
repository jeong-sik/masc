...
let keeper_denied_tools = Tool_catalog.tools_for_surface Tool_catalog.Keeper_denied
...
let provider_kind_of_telemetry (telemetry : Oas.Types.inference_telemetry option) =
...
let render_pre_tool_gate_output (event : Keeper_guards.gate_decision_event) =
  if String.equal event.decision "approval_required"
  then
    Printf.sprintf
...
let pre_tool_gate_error (event : Keeper_guards.gate_decision_event) =
  Printf.sprintf "%s:%s: %s" event.decision event.reason_code event.reason_text
;;
...
let record_pre_tool_gate_attempt
  ...
  | Some acc ->
    ...
    let runtime_contract =
      Keeper_tool_call_log.runtime_contract_json_for_call ~keeper_name ~model ()
    in
    let action_radius =
        ...
        Telemetry_coverage_gap.record
          ...
          ~durable_store:
            (Trajectory.trajectory_path
               acc.Trajectory.masc_root
               acc.Trajectory.keeper_name
               trace_id)
...
let empty_response_model_metric = "masc_after_turn_response_model_empty_total"
let alias_response_model_metric = "masc_after_turn_response_model_alias_total"
...
let canonical_model_id_of_telemetry
  ...
  =
  match telemetry with
  | Some { canonical_model_id = Some id; _ } when String.trim id <> "" -> String.trim id
  | _ -> model
;;
...
let known_provider_model_id_of_label model =
  ...
  | Some idx ->
    ...
    if String.equal provider "unknown"
    then None
    else
      Some (String.sub trimmed (idx + 1) (String.length trimmed - idx - 1) |> String.trim)
;;
...
let is_auto_model_label model =
  String.equal (String.lowercase_ascii (model_id_leaf model)) "auto"
;;

let canonical_model_id_opt (telemetry : Oas.Types.inference_telemetry option) =
  match telemetry with
  | Some { canonical_model_id = Some id; _ } ->
    let trimmed = String.trim id in
    if trimmed = "" || is_auto_model_label trimmed then None else Some trimmed
  | Some _ | None -> None
;;
...
let resolve_after_turn_model ~keeper_name ~(response : Oas.Types.api_response) =
  ...
  then (
    ...
    let resolved, source =
      if String.trim canonical <> ""
      then canonical, "telemetry_resolved"
      else unknown_model_sentinel, "unknown_sentinel"
    in
    Prometheus.inc_counter
      empty_response_model_metric
...
let context_max_of_telemetry (telemetry : Oas.Types.inference_telemetry option) =
  match telemetry with
...
let classify_usage_trust ?usage ~model ~telemetry () =
  ...
  in
  let provider_kind = provider_kind_of_telemetry telemetry in
  Keeper_usage_trust.classify_with_provider_kind
    ~provider_kind
    ~usage_reported
    ~usage
    ~model_used:model
...
let record_usage_anomaly_metrics ~keeper_name ~model usage_trust =
  if not (Keeper_usage_trust.is_trusted usage_trust)
  then (
    let reasons =
    ...
    List.iter
      (fun reason ->
         Prometheus.inc_counter
           ...
           ~labels:[ "keeper_name", keeper_name; "model", model; "reason", reason ]
           ())
      reasons)
;;
...
let estimate_usage_cost_usd ~(model : string) (usage : Oas.Types.api_usage) : float =
  ...
  | Some pricing ->
    Llm_provider.Pricing.estimate_cost
      ...
      ()
  | None ->
    Prometheus.inc_counter "masc_pricing_catalog_miss_total" ~labels:[ "model", model ] ();
    Log.Keeper.warn
      "pricing_catalog_miss model=%s input_tokens=%d output_tokens=%d — no pricing entry \
       in Llm_provider.Pricing catalog; cost recorded as 0.0 (not a true zero). Add the \
       entry upstream in agent_sdk/llm_provider/pricing.ml, then bump the OAS pin."
      model
      usage.input_tokens
      usage.output_tokens;
    0.0
...
let cost_status_reason = function
  | Cost_reported_or_estimated -> "provider_reported_or_pricing_catalog_estimate"
...
let model_resolution_source_for_ledger ~model ~pricing_model =
  let trimmed = String.trim model in
  if String.equal trimmed pricing_model
  then "raw"
  else if String.equal trimmed ""
  then "telemetry_canonical_empty"
  else if is_auto_model_label trimmed
  then "telemetry_canonical_alias"
  else if String.equal trimmed unknown_model_sentinel
  then "telemetry_canonical_unknown"
  else "raw"
;;

let pricing_catalog_status ~pricing_model =
  match Llm_provider.Pricing.pricing_for_model_opt pricing_model with
  | Some pricing when pricing.input_per_million = 0.0 && pricing.output_per_million = 0.0
    -> "hit_free"
  | Some _ -> "hit_paid"
  | None -> "miss"
;;
...
let cost_status_for_event
  ...
  =
  if usage_missing
  then Cost_usage_missing
  else if not usage_trusted
  then Cost_usage_untrusted
  else if input_tokens <= 0 && output_tokens <= 0
  then Cost_no_tokens
  else if cost_usd > 0.0
  then Cost_reported_or_estimated
  else if structurally_unmetered_provider provider
  then Cost_known_free
  else if String.equal provider "unknown"
  then Cost_provider_unknown
  else (
    match Llm_provider.Pricing.pricing_for_model_opt pricing_model with
    | Some pricing
      when pricing.input_per_million = 0.0 && pricing.output_per_million = 0.0 ->
      Cost_known_free
    | Some _ -> Cost_reported_or_estimated
    | None -> Cost_unpriced_model)
;;

let cost_usd_for_usage ?provider_kind ~(model : string) (usage : Oas.Types.api_usage)
  : float
  =
  ...
  | Some cost ->
    if usage_has_tokens usage && not (structurally_unmetered_provider provider)
    then estimate_usage_cost_usd ~model usage
    else cost
  | None ->
    if usage_has_tokens usage && not (structurally_unmetered_provider provider)
    then estimate_usage_cost_usd ~model usage
...
let record_llm_tok_s_metrics
  ...
  let prompt_tok_s_opt, decode_tok_s_opt =
    match telemetry with
    | Some { timings = Some t; _ } -> t.prompt_per_second, t.predicted_per_second
    | _ -> None, None
  in
  ...
  let labels =
    [ "model", model; "provider", provider; "provider_kind", provider_kind_label ]
  in
  (match prompt_tok_s_opt with
   | Some v when v > 0.0 ->
     Prometheus.observe_histogram Prometheus.metric_llm_prompt_tok_per_sec ~labels v
   | _ -> ());
  match decode_tok_s_opt with
  | Some v when v > 0.0 ->
    Prometheus.observe_histogram Prometheus.metric_llm_decode_tok_per_sec ~labels v
  | _ -> ()
;;
...
let wall_tokens_per_second
  ...
  =
  match telemetry with
  | Some t when (not usage_missing) && output_tokens > 0 && t.request_latency_ms > 0 ->
    Some (Float.of_int output_tokens /. (Float.of_int t.request_latency_ms /. 1000.0))
  | _ -> None
;;
...
let () =
  Prometheus.register_counter
    ...
    ~help:
      "Total cost.jsonl emits where cost_usd ended up as 0.0 due to a known \
       classification path (vs an actually-zero call).  Labels: source ∈ {missing_usage, \
       untrusted_usage, unmetered_provider, pricing_catalog_miss, zero_token_call}.  A \
       high [pricing_catalog_miss] rate is the hint to add upstream OAS pricing entries; \
       a high [untrusted_usage] rate points at the trust classifier; a high \
       [missing_usage] rate points at the provider adapter not surfacing usage.  See \
       #10318."
    ()
;;

let classify_cost_usd_source ~usage_missing ~usage_trusted ~provider ~model ~cost_usd =
  if usage_missing
  then "missing_usage"
  else if not usage_trusted
  then "untrusted_usage"
  else if structurally_unmetered_provider provider
  then "unmetered_provider"
  else if cost_usd > 0.0
  then "computed"
  else (
    match Llm_provider.Pricing.pricing_for_model_opt model with
...
let record_cost_emit_source source =
  if not (String.equal source "computed")
  then Prometheus.inc_counter cost_emit_source_metric ~labels:[ "source", source ] ()
;;
...
let emit_cost_event
  ...
  let raw_usage_fields =
    if usage_missing || usage_trusted
    then []
    else
  ...
  let wall_tok_s_fields =
    float_field
      "tokens_per_second"
      (if usage_trusted
       then wall_tokens_per_second ~usage_missing ~output_tokens ~telemetry
       else None)
  in
  let cost_usd_source =
    classify_cost_usd_source
      ~usage_missing
      ~usage_trusted
      ~provider
      ~model:pricing_model
  ...
  let entry =
    `Assoc
      ([ "agent", `String agent_name
       ...
       ; "cost_pricing_catalog", `String (pricing_catalog_status ~pricing_model)
       ; ( "model_resolution_source"
         , `String (model_resolution_source_for_ledger ~model ~pricing_model) )
  ...
  try Fs_compat.append_file path line with
  | Eio.Cancel.Cancelled _ as e -> raise e
  | exn ->
    Log.Keeper.error
      "emit_cost_event: failed to write %s: %s"
      path
      (Printexc.to_string exn)
;;
...
let suggest_alternatives
  ...
  let repeated_set = List.fold_left (fun acc t -> SS.add t acc) SS.empty repeated_tools in
  allowed_tools
  |> List.filter (fun t ->
...
let on_idle_decision_with_threshold
      ~skip_at
  ...
  in
  let alternatives =
    suggest_alternatives ~allowed_tools ~repeated_tools:tool_names ~max_suggestions:5
  in
  let alt_str =
  ...
  in
  if consecutive_idle_turns >= skip_at
  then Oas.Hooks.Skip
  else if consecutive_idle_turns = skip_at - 1
  then
    Oas.Hooks.Nudge
      (Printf.sprintf
         "FINAL WARNING: you repeated %s %d times. Next idle = turn ends. Use one of \
          these instead: %s — or call keeper_stay_silent to do nothing."
         tools_str
         consecutive_idle_turns
         alt_str)
  else
    Oas.Hooks.Nudge
      (Printf.sprintf
         "You are repeating %s without progress. Available alternatives: %s."
         tools_str
         alt_str)
;;
...
let on_idle_decision ~consecutive_idle_turns ~allowed_tools ~tool_names
  ...
  on_idle_decision_with_threshold
    ~skip_at
    ~consecutive_idle_turns
    ~allowed_tools
    ~tool_names
...
let recent_tool_streak_count
  ...
  let rec loop count = function
    ...
    | entry :: rest ->
      (match
         Safe_ops.json_string_opt "tool" entry, Safe_ops.json_float_opt "ts" entry
       with
...
let make_hooks
      ...
      ?(on_tool_executed :
        ...
        fun ~tool_name:_ ~input:_ ~output_text:_ ~success:_ ~duration_ms:_ ~provider:_ ->
          ())
      ?(trajectory_acc : Trajectory.accumulator option)
      ?(discover_work_nudge : unit -> string option = fun () -> None)
      ()
  : Oas.Hooks.hooks
  =
  ...
  let record_gate_decision event =
    record_pre_tool_gate_attempt ~meta_ref ~tool_call_count_ref ?trajectory_acc event
  in
  ...
  let non_gate_hooks =
          ...
          (fun event ->
            ...
            | Oas.Hooks.BeforeTurn _ ->
              (match discover_work_nudge () with
               ...
               | Some text when String.trim text = "" -> Oas.Hooks.Continue
              ...
              drift. See #9036 for the first observed producer fix. *)
                 Log.Keeper.warn
                   "keeper:%s before_turn: dropped invalid UTF-8 nudge (%d bytes)"
                   !meta_ref.name
                   (String.length text);
                 Oas.Hooks.Continue
               | Some text ->
                 Log.Keeper.info
                   "keeper:%s before_turn: injecting work_discovery nudge (%d chars)"
                   !meta_ref.name
                   (String.length text);
        ...
        Some
          (fun event ->
            ...
            | Oas.Hooks.AfterTurn { turn; response } ->
              ...
              if (not usage_missing) && not usage_trusted
              then
                Log.Keeper.warn
                  "keeper:%s after_turn usage telemetry untrusted model=%s \
                   resolved_model=%s reasons=%s input=%d output=%d context_max=%d"
                  meta.name
                  model
           ...
           and in downstream dashboards. *)
              ...
              Log.Keeper.info
                "keeper:%s turn=%d total_turns=%d model=%s tokens=%d wall_tok_s=%s \
                 prompt_tok_s=%s decode_tok_s=%s latency_ms=%d"
                meta.name
                turn
              ...
              (match trajectory_acc with
               | Some acc ->
                 emit_cost_event
                   ...
                   ~output_tokens:raw_output_tok
                   ~cost_usd:cost_usd_for_event
                   ~usage_missing
                   ~usage_trust
                   ?telemetry:response.telemetry
                   ()
    ...
    ; post_tool_use =
        Some
          (fun event ->
            match event with
            | Oas.Hooks.PostToolUse
                { tool_name; input; output; duration_ms = hook_duration_ms; _ } ->
              incr tool_call_count_ref;
              let output_text =
                match output with
              ...
              in
              Log.Keeper.info
                "keeper:%s tool_call tool=%s params=[%s] outcome=%s out_len=%d"
                !meta_ref.name
                tool_name
                input_keys
                outcome
           ...
           Tool calls within Agent.run are sequential, so no race. *)
              ...
              let summary =
                tool_execution_summary
                  ...
                  ~duration_ms
              in
              record_keeper_tool_duration_metric ~keeper_name:!meta_ref.name summary;
           ...
           when no truncation info was set (e.g. OAS-internal tool calls). *)
              ...
              let ( lane
                  ...
                  , approval_mode )
                =
                Keeper_tool_call_log.get_turn_context ~keeper_name:!meta_ref.name ()
              in
              (try
                 Keeper_tool_call_log.log_call
                   ~keeper_name:!meta_ref.name
              ...
              (match trajectory_acc with
               ...
               | Some acc ->
                 ...
                 let trace_id = acc.Trajectory.trace_id in
                 let safe_input = Observability_redact.redact_json_value input in
                 let safe_output =
                   Observability_redact.redact_preview ~max_len:4000 output_text
                 in
                 let runtime_contract =
              ...
              (try
               ...
               | exn ->
                 Log.Keeper.error
                   "keeper:%s on_tool_executed callback failed for %s: %s"
                   !meta_ref.name
                   tool_name
                   (Printexc.to_string exn));
              if is_keeper_board_write_tool_name tool_name
              then
                Log.Keeper.debug "keeper:%s social_event tool=%s" !meta_ref.name tool_name;
              Oas.Hooks.Continue
            | _ -> Oas.Hooks.Continue)
        ...
        Some
          (fun event ->
            ...
            | Oas.Hooks.OnIdle { consecutive_idle_turns; tool_names; _ } ->
              let allowed_tools =
                Keeper_tool_policy.keeper_allowed_tool_names !meta_ref
              in
              let decision =
                on_idle_decision ~consecutive_idle_turns ~tool_names ~allowed_tools
              in
              let tools_str =
                match tool_names with
                | [] -> "<none>"
              ...
              (match decision with
               | Oas.Hooks.Skip ->
                 Log.Keeper.warn
                   "keeper:%s idle_turns=%d repeated_tools=[%s] — requesting stop"
                   !meta_ref.name
                   consecutive_idle_turns
                   tools_str
               | Oas.Hooks.Nudge _ ->
                 Log.Keeper.info
                   "keeper:%s idle_turns=%d tools=[%s] — nudging LLM via Nudge"
                   !meta_ref.name
                   consecutive_idle_turns
                   tools_str
               | _ -> ());
...
let hook_introspection_json
  ...
  : Yojson.Safe.t
  =
  let denied_json = `List (List.map (fun s -> `String s) keeper_denied_tools) in
  ...
  `Assoc
      ...
      , `Assoc
          [ ( "before_turn_params"
            , `Assoc
                ...
                ; ( "features"
                  , `List
                      [ `String "dynamic_context"; `String "bm25_progressive_disclosure" ]
                  )
                ] )
          ; ( "pre_tool_use"
            , `Assoc
          ...
          ; ( "after_turn"
            , `Assoc
                ...
                ; ( "effects"
                  , `List
                      [ `String "sse_broadcast"; `String "cost_event"; `String "metrics" ]
                  )
                ] )
          ; ( "post_tool_use"
            , `Assoc
                ...
                ; "source", `String "keeper_hooks_oas"
                ; ( "features"
                  , `List [ `String "tool_callback"; `String "board_write_detection" ] )
                ] )
          ; ( "on_idle"
            , `Assoc [ "active", `Bool true; "source", `String "keeper_hooks_oas" ] )
          ; ( "on_error"
            , `Assoc [ "active", `Bool true; "source", `String "keeper_hooks_oas" ] )
          ; ( "on_tool_error"
            , `Assoc [ "active", `Bool true; "source", `String "keeper_hooks_oas" ] )
          ; ( "post_tool_use_failure"
            , `Assoc
                [ "active", `Bool true
                ; "source", `String "keeper_hooks_oas"
                ; "effects", `List [ `String "heuristic_metrics" ]
                ] )
          ] )
    ...
    ; ( "cost_budget"
      , match max_cost_usd with
        | Some v -> `Assoc [ "max_cost_usd", `Float v; "active", `Bool true ]
        | None -> `Assoc [ "active", `Bool false ] )
    ]
;;
