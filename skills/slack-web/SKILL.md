...
관측한 채널 링크를 선택한 뒤 Available 목록에 `browser-live-click-regions`가
있으면 그 composition Skill을 그때 불러온다. 하나의 호출로 링크 클릭과 새
페이지 영역 목록 읽기를 순서대로 실행한다. 그 결과에서 새 메시지 영역을
선택하는 판단은 다시 모델이 한다. 같은 revision을 이미 읽었다면 재로딩하지
않는다. 현재 도구나 Skill 목록에 없으면 지원을 가정하지 않는다.
