...
- A Daily or Cron keeper wake with nothing to deliver (`result_delivery = none`) now waits while the keeper still holds its earlier occurrence, as an Interval wake already did. The hold was keyed on the recurrence kind, so a `*/5 * * * *` wake on a busy keeper cancelled the pending occurrence and enqueued a new one every period — one cancellation and one wake record per period — while the schedule advanced on the wall clock. An unreadable payload or keeper queue still fires, so dispatch records the failure, and a queue read failure is logged (#38039).
