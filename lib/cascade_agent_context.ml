...
let wire_overlay_auth_header_authorization = "Authorization"

let wire_overlay_api_key_from_env name =
...
let wire_overlay_auth_headers_for_provider_cfg
      ~(api_key : string)
...
let wire_overlay_request_kind_of_provider_cfg (provider_cfg : Llm_provider.Provider_config.t) =
  match provider_cfg.kind with
...
let wire_overlay_agent_capabilities_of_llm_capabilities
...
let wire_overlay_register_capability_overlay_provider
      ~(name : string)
      ~(provider_cfg : Llm_provider.Provider_config.t)
  =
  let capabilities =
    Agent_sdk.Provider_runtime_binding.capabilities_for_provider_config provider_cfg
    |> wire_overlay_agent_capabilities_of_llm_capabilities
  in
  Agent_sdk.Provider.register_provider
    { name
    ; request_kind = wire_overlay_request_kind_of_provider_cfg provider_cfg
    ...
    ; resolve =
        (fun provider ->
           let api_key =
             ...
             | "" -> wire_overlay_api_key_from_env provider.Agent_sdk.Provider.api_key_env
             | value -> value
           in
           Ok
             ( provider_cfg.base_url
             , api_key
             , wire_overlay_auth_headers_for_provider_cfg ~api_key provider_cfg ))
    }

let wire_overlay_apply_capability_overlay
      ~(provider_cfg : Llm_provider.Provider_config.t)
  ...
  | Some _ ->
    ...
    (match Llm_provider.Provider_registry.find registry name with
     | None -> provider
     | Some _ ->
       wire_overlay_register_capability_overlay_provider ~name ~provider_cfg;
       { provider with provider = Agent_sdk.Provider.Custom_registered { name } })

let wire_overlay_apply
      ~(provider_cfg : Llm_provider.Provider_config.t)
      (provider : Agent_sdk.Provider.config)
  ...
  let provider =
    ...
    | ( Llm_provider.Provider_config.Provider_d_compat
      ...
      let auth_header =
        if String.equal api_key_trimmed "" then None
        else Some wire_overlay_auth_header_authorization
      in
      let static_token =
    ...
    | _ -> provider
  in
  wire_overlay_apply_capability_overlay ~provider_cfg provider
...
let default_config
  ...
  let provider =
    ...
    |> wire_overlay_apply ~provider_cfg
  in
  { name
  ; provider_cfg
  ; provider
