# RFC-0038: Runtime Routing Intent Preservation

> **Status**: Draft
> **Authors**: vincent (with Agent-LLM-A)
> **Created**: 2026-05-07
> **Related RFCs**: RFC-0001 (det/nondet boundary — silent substitution anti-pattern), RFC-0024 (ollama runtime integration), RFC-0026 (work-conserving keeper admission), RFC-0027 (capability-typed runtime catalog) — RFC-0027 §3.4 PR #3 + PR #4 의 specification 을 가속화/구체화한다.
> **Anchor commit**: `065e568be1` (PR #11210, 2026-04-27 — cross-runtime fallback 도입)

## 1. Summary

Runtime routing layer 가 사용자가 keeper_runtime.toml 에 명시한 model intent 와 무관한 모델을 silently 호출한다. 5월 1-6일 audit 에서 2,620 turns 중 254 turns (9.69%) 가 `configured_labels` 와 `candidate_models` 가 disjoint 한 silent substitution 을 겪었다. ollama 는 22번 의도된 후보로 등장했지만 실제로 1번만 실행됐다 (4.5% intent-realization rate).

본 RFC 는 이 문제를 단일 패치가 아니라 **3-layer compounding failure** 로 진단하고 layer 별 invariant 와 fix sequence 를 정의한다.

- **L0 — Default-deny cross-runtime fallback**: runtime fallback 은 toml 의 explicit `fallback_runtime` chain 으로만 표현. 코드의 cross-runtime resolver 는 default-disabled, env flag 로 legacy mode 만 유지.
...
- **L4 — Audit substitution visibility**: substitution 이 일어나면 runtime observation 에 `substituted_from_runtime`, `substitution_reason` 필드를 기록. RFC-0001 §"표현" axis 직접 충족.
...
데이터 소스: `<base-path>/.masc/runtime_audit/2026-05/0[1-6].jsonl`
...
`lib/oas_worker_named.ml:270-312` — 빈 후보일 때 cross-runtime fallback (silent swap path):
```ocaml
let candidate_cfgs =
  match candidate_cfgs with
  | [] ->
      (match resolve_tool_capable_provider_across_runtimes
              ~exclude_runtime:runtime_id ()
       with
       | Some (source_runtime, provider_cfg) ->
           Llm_metric_bridge.emit_fallback_triggered
             ~kind:"cross_runtime"
             ~detail:(Printf.sprintf "%s->%s" runtime_id source_runtime);
           [provider_cfg]
       | None -> [])
  | _ -> candidate_cfgs
in
...
지원 함수들:
- `lib/oas_worker_named_runtime.ml:106` — `keeper_internal_tools_require_materialized_runtime_surface`
- `lib/oas_worker_named_runtime.ml:379` — `filter_candidate_providers_for_tool_support`
- `lib/oas_worker_named_runtime.ml:473` — `resolve_tool_capable_provider_across_runtimes`
...
본 RFC 의 cross-runtime silent substitution 이 단일 turn level 에 그치지 않고 keeper-level 자율행동 정지로 amplify 되는 사례가 2026-04-28 에 관측되었다. 분석 출처: `~/Downloads/Kimi_Agent_Keeper FSM 고정/` (Provider-C Agent 5-stage analysis, 2026-04-28).

**사고 흐름** (Provider-C `executor_reanalysis.md` §3 인용):

1. `local_with_kimi_coding_with_glm` runtime 의 모든 provider 가 tool-use gate reject (`codex_keeper_bound_actor_required`)
2. **`ollama:qwen3.6:27b-coding-nvfp4` 로 cross-runtime fallback** ← 본 RFC L0 가 차단하려는 path
...
Axis 3: Runtime State (KeeperOASAdvanced) — provider hang (trigger)
```
...
- ✅ **차단 가능**: cross-runtime fallback default-off 시 step 2 가 발생하지 않고 명시적 에러 (`Runtime_local_only_filter_drained`) 로 즉시 실패. step 3-5 chain 미발동.
...
- **I-Intent**: 사용자가 keeper_runtime.toml 에 명시한 model 집합과 실제 실행 model 은 같거나, 명시적 fallback chain 의 결과여야 한다.
...
RFC-0027 §3.4 PR #3 가 "cross-runtime fallback resolver capability propagation" 으로 사후 조정을 시도했으나 미구현.
...
| # | Invariant | Layer |
|---|-----------|-------|
| G1 | Cross-runtime fallback 은 default-off. fallback chain 은 keeper_runtime.toml 의 `fallback_runtime` 만 표현 path. | L0 |
| G2 | Substitution 이 (legacy mode 또는 fallback_runtime chain 으로) 발생 시 audit observation 에 source runtime + reason 필드 포함. | L4 |
| G3 | Capability classification 은 model-id 별 probe 결과 또는 명시적 pattern table 을 우선. provider-kind 정적 분류는 fallback 만. | L1 |
| G4 | 5월 1-6일 audit 의 254 substitution 케이스 replay 시 audit observation 의 `substituted_from_runtime` 가 non-null 이거나 runtime 가 명시적 실패 (`Runtime_local_only_filter_drained`) 로 처리됨. | L0 + L4 |
...
### 4.1 L0 — Cross-runtime fallback default-off

**Code surface**: `lib/oas_worker_named.ml:270-312`, `lib/oas_worker_named_runtime.ml:473-550`.
...
- 변경:
  ...
  - `Explicit_chain_only` (default): keeper_runtime.toml 의 `fallback_runtime` chain 만 따라감 (기존 정상 코드 path 유지). cross-runtime resolver 는 호출 안 함.
...
- Phase 2 (1-2 weeks 운영 후): 기존 implicit cross-runtime 의존 runtime 들의 toml 에 명시적 `fallback_runtime` chain 추가 마이그레이션.
- Phase 3: `Legacy` mode deprecation warning → 차후 `Off` default.

**Migration guide**: keeper_runtime.toml 의 모든 user-facing runtime 가 `fallback_runtime` 명시. 예:
```toml
[primary]
fallback_runtime = "retired_fast_profile"  # 기존 implicit cross-runtime 대신
```
...
(** Resolution order (highest precedence first):
    1. keeper_runtime.toml `[capability_overrides]` table per model-id
...
**keeper_runtime.toml override (Phase 1, 즉시)**:
```toml
...
**Code surface**: `lib/oas_worker_runtime.ml:220-258` (`runtime_observation_of_candidates`).
...
type runtime_observation = {
  ...existing fields...
  substituted_from_runtime : string option;  (* NEW *)
  substitution_reason : string option;       (* NEW: e.g. "explicit_fallback_chain", "cross_runtime_legacy", "capability_drop" *)
...
**Dashboard panel**: runtime audit 패널에 substitution rate 그래프 추가 (별도 PR, 본 RFC scope-out 가능).

### 4.4 L2/L3 — Invariant 선언 only

**L2 (transport keeper-identity)** invariant: ollama 가 cross-runtime resolver 의 swap target 이 아니라 본인 runtime 의 진짜 후보로 동작하려면 keeper identity HTTP header 를 carry 해야 한다. 구현 path 후보:
...
**L3 (permission-aware filtering)** invariant: runtime candidate 는 keeper 의 effective credential set 과 호환되는 model 만 포함한다. 구현은 `Keeper_admission_glue` 와 통합. → RFC-0026 후속 RFC 로 분리.

## 5. PR Sequence

| PR | Layer | 변경 surface | Dependencies | RFC 인용 |
|----|-------|-------------|--------------|---------|
| **PR-A** | L0 + L4 schema | `oas_worker_named.ml:270-312`, `oas_worker_named_runtime.ml:473-550`, `oas_worker_runtime.ml:220-258`, `runtime_config.ml` | none | RFC-0001, RFC-0027 §3.4 PR #3 |
| **PR-B** | L0 migration | `.masc/config/keeper_runtime.toml` 의 모든 runtime 에 명시적 `fallback_runtime` chain | PR-A | RFC-0027 §3.4 PR #4 (`__safe_lane`) |
| **PR-C** | B2 hot fix | `provider-k-coding:*` ↔ retired planning namespace mismatch 정규화 | none (parallel) | RFC-0027 §3 |
| **PR-D** | L1 override table | `runtime_config.ml` capability_overrides schema, keeper_runtime.toml 등록 | PR-A 권장 | RFC-0024, RFC-0027 |
...
| Layer | Test |
|-------|------|
| L0 | Unit test: runtime routing decision trace — 빈 candidate × mode 별 분기 |
...
| Property | Invariant I1 (Intent): `configured_labels ⊆ candidate_models` 또는 `substituted_from_runtime ≠ None` |

## 8. Risks

| Risk | Mitigation |
|------|------------|
| L0 default 변경으로 BDI defer loop 재발 (PR #11210 의 원래 motivation) | Phase 1 default = `Legacy`. keeper_runtime.toml 마이그레이션 (Phase 2) 후 default 변경. metric `cross_runtime_fallback_legacy_total` 모니터링. |
| L1 pattern table 이 model 출시 속도를 못 따라감 | keeper_runtime.toml `[capability_overrides]` 가 일급 escape hatch. 운영 중 즉시 추가 가능. |
...
- `feedback_rfc_section_1_4_caller_context_unverified.md`: §2.3 의 모든 file:line citation 은 `rg -nC1` 으로 grep 검증 후 기재함 (oas_worker_named.ml:193-197, 237-247, 270-312; oas_worker_named_runtime.ml:106, 379, 473; provider_tool_support.ml:39-69, 148-242).
...
- Q2: L1 의 `keeper_runtime.toml [capability_overrides]` block 이 RFC-0026 admission schema 와 충돌 가능성?
...
- RFC-0024: ollama runtime integration — provider adapter, KV cache
- RFC-0026: work-conserving keeper admission — admission layer 와 분리
- RFC-0027: capability-typed runtime catalog — capability profile registry, §3.4 후속 PR 표
- Anchor commit: `065e568be1` (PR #11210, 2026-04-27)
- Audit data: `<base-path>/.masc/runtime_audit/2026-05/0[1-6].jsonl` (2,620 turns)
- 분석 노트: 2026-05-07 5-track parallel investigation (cross-runtime intent, tool-support filter, keeper-internal requirement, audit log triage, RFC + git history)
- 외부 분석 (Provider-C Agent, 2026-04-28): `~/Downloads/Kimi_Agent_Keeper FSM 고정/`
  ...
  - `executor_failure_chain_analysis.md` — 5-stage runtime failure timeline
