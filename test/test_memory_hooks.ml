...
let make_before_turn_params_event ?(extra_ctx = None) ~turn () =
  Agent_sdk.Hooks.BeforeTurnParams
    ...
    ; last_tool_results = []
    ; current_params =
        { Agent_sdk.Hooks.default_turn_params with extra_system_context = extra_ctx }
...
let test_load_episodes_text_type () =
  ...
  match result with
  | None -> ()
  | Some s -> check bool "episodes text is non-empty" true (String.length s > 0)
;;

let test_load_procedures_text_empty () =
  (* No procedures for unknown agent -> None *)
  let result =
    Memory_oas_bridge.load_procedures_text
      ~agent_name:"nonexistent_test_agent_xyz"
      ~limit:10
  in
...
let test_hook_preserves_existing_context () =
  ...
  let hooks = Memory_hooks.make ~agent_name:"test_preserve" ~config ~memory () in
     ...
     (match params.extra_system_context with
      | Some ctx ->
        check
          ...
          true
          (String.length ctx >= String.length existing_ctx);
        check
          bool
          "existing context is prefix"
          true
          (String.sub ctx 0 (String.length existing_ctx) = existing_ctx)
      | None -> fail "AdjustParams should not clear extra_system_context")
...
let test_after_turn_hook_returns_continue () =
  ...
  let hooks = Memory_hooks.make ~agent_name:"test_after" ~config ~memory () in
  let event =
  ...
  check
    bool
    "after_turn returns Continue"
    true
    (match decision with
     | Agent_sdk.Hooks.Continue -> true
     | _ -> false);
  try Sys.rmdir tmp_dir with
  | _ -> ()
;;
...
let () =
  run
    ...
    ; ( "hook_decisions"
      , [ test_case
            "returns Continue when no memory"
            `Quick
            test_hook_returns_continue_when_no_memory
        ; test_case
        ...
        ; test_case
            "after_turn returns Continue"
            `Quick
            test_after_turn_hook_returns_continue
        ] )
    ; ( "hook_structure"
      , [ test_case "hook slots populated correctly" `Quick test_hook_slots_populated ] )
    ; ( "flush"
      , [ test_case
            "flush_incremental idempotent"
            `Quick
            test_flush_incremental_idempotent
        ] )
    ; ( "feature_flag"
