...
let sandbox_roots name = [ KAP.sandbox_path_of_keeper name ]
...
let test_explicit_paths_append_to_sandbox_root () =
  ...
  check
    (list string)
    "read paths append explicit entries"
    expected
    (KAP.effective_allowed_paths ~meta);
  check
    (list string)
    "write paths append explicit entries"
    expected
    (KAP.effective_write_allowed_paths ~meta)
;;
...
let test_playground_path_sanitizes_name () =
  ...
  check string "special chars sanitized" ".masc/playground/my_keeper_.._.._etc/" path
;;
...
let test_validate_docker_allows_private_root_paths () =
  with_temp_config (fun config ->
    let allowed =
      [ Masc_mcp.Keeper_turn_up_args.private_workspace_root_rel
...
let test_validate_docker_rejects_paths_outside_private_root () =
  with_temp_config (fun config ->
    ...
    | Error err ->
      check bool "error mentions rejected path" true (String.contains err 'w'))
;;
...
let test_hard_mode_requires_docker_none_identity () =
  ...
  with_temp_config (fun config ->
    ...
    (match
       validate
         ~github_identity:(Some "anyang-keepers")
         ~sandbox_profile:KT.Docker
         ~network_mode:KT.Network_inherit
         ()
     with
     ...
     | Error err ->
       check
         ...
         "MASC_KEEPER_SANDBOX_HARD_MODE requires network_mode=none; git/gh egress is \
          brokered by structured tools"
         err);
    (match
       validate
         ~github_identity:None
