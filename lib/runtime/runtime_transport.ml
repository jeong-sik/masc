...
let resolve_tool_lane_for_oas_tools
  ...
  (* RFC-0167: previously routed [omitted_keeper_bound_actor_tools] through
     a client-named omission adapter and could reject "required" tool turns.
     Tool surfaces are not completion contracts now: omitted bound-actor tools
     are removed from the materialized lane and exposed only through
     diagnostics. *)
  (
    ...
    (* RFC-0167 (was #12676): When all tools were bound-actor and got
       stripped, runtime_tool_names is empty. The keeper may still use an MCP
       connection for discovery, so build a minimal connect-only policy with
       the server URL and auth but no allowed_tool_names. *)
