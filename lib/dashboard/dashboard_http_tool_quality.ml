...
let normalize_failure_text (text : string) : string =
  ...
  match
    List.find_opt
      (fun (prefix, _category) -> String.starts_with ~prefix lowered)
      prefix_rules
  with
...
let classify_process_status (json : Yojson.Safe.t) : string option =
  ...
  | `Assoc _ as status ->
    let kind =
      Safe_ops.json_string_opt "kind" status |> Option.value ~default:"unknown"
    in
    let op = Safe_ops.json_string_opt "op" json |> Option.value ~default:"tool" in
    (match kind with
     | "timeout" -> Some (Printf.sprintf "%s_timeout" op)
     | "signaled" ->
...
let classify_failure_output (output : string) : string =
  ...
  then "empty_output"
    ...
    let json_str =
      ...
      with
      | Some p ->
        String.sub output (String.length p) (String.length output - String.length p)
      | None -> output
    in
      ...
      (match Safe_ops.json_string_opt "error" j |> Option.map String.trim with
       ...
       | _ ->
         (match Safe_ops.json_string_opt "message" j |> Option.map String.trim with
          ...
          | _ -> classify_process_status j |> Option.value ~default:"unknown_error"))
...
let render_rate_table ~field table =
  Hashtbl.fold
    (fun name (c, s) acc ->
       ...
       let successes = !s in
       let pct =
         if calls > 0 then Float.of_int successes /. Float.of_int calls *. 100.0 else 0.0
       in
       ( calls
       , `Assoc [ field, `String name; "calls", `Int calls; "success_pct", `Float pct ] )
       :: acc)
    table
    []
...
let source_metadata_fields () =
  Dashboard_tool_source_freshness.keeper_tool_call_io_fields ~dashboard_surface ()
;;
...
let aggregate ?(n = 5000) ?window_hours () : Yojson.Safe.t =
  let records, sampling_mode, window_hours =
    ...
    | Some hours when hours > 0.0 ->
      Keeper_tool_call_log.read_window ~window_hours:hours (), "window_hours", Some hours
    | _ -> Keeper_tool_call_log.read_recent ~n (), "recent_n", None
  in
  if records = []
  then empty_summary ~window_hours ~n ~sampling_mode
  else (
    let total = ref 0 in
    ...
    let keeper_stats : (string, int ref * int ref) Hashtbl.t = Hashtbl.create 16 in
    (* model/lane/thinking/tool_choice -> (calls, successes) *)
    let model_stats : (string, int ref * int ref) Hashtbl.t = Hashtbl.create 16 in
    let lane_stats : (string, int ref * int ref) Hashtbl.t = Hashtbl.create 8 in
    let thinking_mode_stats : (string, int ref * int ref) Hashtbl.t = Hashtbl.create 4 in
    let tool_choice_stats : (string, int ref * int ref) Hashtbl.t = Hashtbl.create 8 in
    ...
    List.iter
      (fun record ->
         incr total;
         let tool =
           Safe_ops.json_string_opt "tool" record |> Option.value ~default:"unknown"
         in
         let keeper =
           Safe_ops.json_string_opt "keeper" record |> Option.value ~default:"unknown"
         in
         let ok =
           match record with
           | `Assoc fields ->
         ...
         let hc, hs =
           ...
           | None ->
             ...
             Hashtbl.replace hourly_trend hour_key v;
             v
         in
         incr hc;
         if ok then incr hs;
         ...
         let tc, ts, td, ttrunc, tochars =
           ...
           | None ->
             ...
             Hashtbl.replace tool_stats tool v;
             v
         in
         incr tc;
         if ok then incr ts;
         ...
         let kc, ks =
           ...
           | None ->
             let v = ref 0, ref 0 in
             Hashtbl.replace keeper_stats keeper v;
             v
         in
         incr kc;
         if ok then incr ks;
         ...
         update_rate_table
           tool_choice_stats
           (bucket_key record "tool_choice" ~default:"unknown")
           ok;
         (* failure category *)
         if not ok
         then (
           ...
           let r =
             match Hashtbl.find_opt failure_cats cat with
             | Some r -> r
             | None ->
               let r = ref 0 in
               Hashtbl.replace failure_cats cat r;
               r
           in
           incr r))
      records;
    ...
    let rate =
      if total_n = 0 then 0.0 else Float.of_int success_n /. Float.of_int total_n *. 100.0
    in
    (* Sort by call count descending *)
    let by_tool =
      Hashtbl.fold
        (fun name (c, s, d, ttrunc, tochars) acc ->
           let calls = !c in
           let successes = !s in
           ...
           in
           let avg_output =
             if calls > 0 then Float.of_int !tochars /. Float.of_int calls else 0.0
           in
           ( calls
           , `Assoc
               [ "name", `String name
      ...
      |> List.map snd
    in
    let by_keeper = render_rate_table ~field:"name" keeper_stats in
    let by_model = render_rate_table ~field:"name" model_stats in
    let by_lane = render_rate_table ~field:"name" lane_stats in
    let by_thinking_mode = render_rate_table ~field:"name" thinking_mode_stats in
    let by_tool_choice = render_rate_table ~field:"name" tool_choice_stats in
