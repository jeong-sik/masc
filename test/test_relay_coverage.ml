...
let test_estimate_context_usage_ratio () =
  ...
  check
    bool
    "usage_ratio between 0 and 1"
    true
    (m.usage_ratio > 0.0 && m.usage_ratio < 1.0)
;;
...
let test_should_relay_custom_threshold () =
  let config : Relay.relay_config = { Relay.default_config with threshold = 0.5 } in
  let metrics : Relay.context_metrics =
...
let test_proactive_large_file_relay () =
  ...
  check
    bool
    "large file triggers relay"
    true
    (Relay.should_relay_proactive
       ~config
       ~metrics
       ~task_hint:(Relay.Large_file_read "big.ml"))
;;
...
let test_smart_proactive () =
  ...
  let result =
    Relay.should_relay_smart ~config ~metrics ~task_hint:(Relay.Large_file_read "big.ml")
  in
...
let test_compress_context_summary_only () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Context Summary") result 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "This is the summary") result 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_compress_context_with_task () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Current Task") result 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "feature X") result 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_compress_context_with_todos () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "TODO List") result 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "- TODO 1") result 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_compress_context_with_pdca () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "PDCA State") result 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Design API") result 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_compress_context_with_files () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Relevant Files") result 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "`src/main.ml`") result 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_compress_context_full () =
  let result =
    Relay.compress_context
      ...
      ()
  in
  check bool "has all sections" true (String.length result > 100)
;;
...
let test_build_handoff_prompt_basic () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "RELAY HANDOFF") prompt 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_build_handoff_prompt_generation () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Generation 5") prompt 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_build_handoff_prompt_instructions () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Instructions") prompt 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "MASC") prompt 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_build_handoff_prompt_seamless () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "seamless") prompt 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_metrics_to_json_basic () =
  ...
  check
    bool
    "has estimated_tokens"
    true
    (try
       let _ = Str.search_forward (Str.regexp "estimated_tokens") str 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "max_tokens") str 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "usage_ratio") str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_metrics_to_json_values () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "12345") str 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "100000") str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_payload_to_json_basic () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Test summary") str 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Test task") str 0 in
       true
     with
     | Not_found -> false);
  check
    bool
    "has relay_generation"
    true
    (try
       let _ = Str.search_forward (Str.regexp "relay_generation") str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_payload_to_json_nulls () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "\"current_task\":null") str 0 in
       true
     with
     | Not_found -> false);
  check
    bool
    "has null for session_id"
    true
    (try
       let _ = Str.search_forward (Str.regexp "\"session_id\":null") str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_payload_to_json_todos_array () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "\"todos\":\\[") str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_payload_to_json_with_goals () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "active_goal_ids") str 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "goal_progress") str 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "goal_blockers") str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_compress_context_with_goal_progress () =
  ...
  check
    bool
    "has goal progress section"
    true
    (try
       let _ = Str.search_forward (Str.regexp "Goal Progress") result 0 in
       true
     with
     | Not_found -> false);
  check
    bool
    "has goal-1 percentage"
    true
    (try
       let _ = Str.search_forward (Str.regexp "goal-1: 75%") result 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_compress_context_with_goal_blockers () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Blockers") result 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Waiting for CI") result 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_calibration_initial_state () =
  ...
  check
    bool
    "has correction_factor"
    true
    (try
       let _ = Str.search_forward (Str.regexp "correction_factor") str 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "sample_count") str 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "enabled") str 0 in
       true
     with
     | Not_found -> false)
;;
...
let () =
  ...
  run
    ...
    ; "checkpoint_cap", [ test_case "capped at 500" `Quick test_checkpoint_cap ]
    ; ( "handoff_payload"
      , [ test_case "creation" `Quick test_handoff_payload_creation
        ...
        ; test_case
            ...
            `Quick
            test_handoff_payload_incremented_generation
        ; test_case "with goals" `Quick test_handoff_payload_with_goals
        ] )
    ; "context_metrics", [ test_case "creation" `Quick test_context_metrics_creation ]
    ; ( "goal_aware"
      , [ test_case "payload_to_json with goals" `Quick test_payload_to_json_with_goals
        ; test_case
            "compress with goal progress"
            `Quick
            test_compress_context_with_goal_progress
        ; test_case
            "compress with goal blockers"
            `Quick
            test_compress_context_with_goal_blockers
        ] )
    ; ( "calibration"
