...
let make_closing_client ~sw ~net ~https =
  ...
  let tracked_flows : [ `Close | `Flow | `R | `Shutdown | `W ] Eio.Resource.t list ref =
    ref []
  in
  ...
  let close_flow flow =
    ...
    | exn ->
      Printf.eprintf
        "[masc_http_client] close flow failed: %s\n%!"
  ...
  let connect ~sw:conn_sw uri =
    ...
    | Some "https" ->
      (match https with
       ...
       | None ->
         raise (Invalid_argument "masc_http_client: HTTPS requested but not enabled"))
    | _ -> register_flow (sock :> [ `Close | `Flow | `R | `Shutdown | `W ] Eio.Resource.t)
  in
...
let post_sync ?clock ?timeout_sec ~net ?(https = None) ~url ~headers ~body () =
  with_optional_timeout ?clock ?timeout_sec
  @@ fun () ->
  try
    Eio.Switch.run
    @@ fun sw ->
    ...
    let hdr = Cohttp.Header.of_list (("connection", "close") :: headers) in
...
let get_response_sync ?clock ?timeout_sec ~net ?(https = None) ~url ~headers () =
  ...
  try
    ...
    let code = Cohttp.Response.status resp |> Cohttp.Code.code_of_status in
    let response_headers = Cohttp.Response.headers resp |> Cohttp.Header.to_list in
    let body_str =
      Eio.Buf_read.(parse_exn take_all) resp_body ~max_size:(8 * 1024 * 1024)
    in
