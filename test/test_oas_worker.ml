...
let make_noop_tool () =
  Oas.Tool.create ~name:"noop" ~description:"No-op test tool" ~parameters:[] (fun _ ->
    Ok Oas.Types.{ content = "ok" })
;;

let make_named_noop_tool name =
  Oas.Tool.create ~name ~description:"No-op test tool" ~parameters:[] (fun _ ->
    Ok Oas.Types.{ content = "ok" })
;;
...
let check_policy_matches_default_internal label (policy : Oas.Tool_retry_policy.t option) =
...
let test_cascade_metrics_concurrent_recording () =
  ...
  Fun.protect
    ~finally:(fun () -> Masc_mcp.Oas_worker_cascade.reset_cascade_counters_for_test ())
    (fun () ->
       Eio.Fiber.all
         (List.init 8 (fun _ ->
            fun () ->
...
let test_sdk_error_to_cascade_outcome_cascades_resumable_cli_session () =
  ...
  in
  let detail =
    Oas_worker_exec.Kimi_cli_transport_local.resumable_session_detail_of_text raw_message
  in
  let sdk_error =
...
let test_enrich_sdk_error_for_moonshot_auth_includes_env_hint () =
  ...
  in
  let err =
    Oas.Error.Api (Llm_provider.Retry.AuthError { message = "Invalid Authentication" })
  in
  let rendered =
    Oas_worker_named.enrich_sdk_error ~cascade_name:"keeper_unified" ~provider_cfg err
    |> Oas.Error.to_string
  in
  Alcotest.(check bool)
    "env hint included"
...
let test_oas_worker_exec_build_applies_retry_policy () =
  ...
  in
  let config =
    { base_config with tool_retry_policy = Some Oas.Tool_retry_policy.default_internal }
  in
  Eio.Switch.run
  @@ fun sw ->
  match Oas_worker_exec.build ~sw ~net:(require_test_net ()) ~config with
...
let test_run_model_with_masc_tools_rejects_invalid_explicit_label () =
  ...
  | Error err -> Alcotest.failf "unexpected error shape: %s" (Oas.Error.to_string err)
;;
...
let test_cascade_provider_labels_keep_glm_and_glm_coding_distinct () =
  let glm =
    Masc_mcp.Oas_worker_cascade.provider_name_of_config (make_glm_provider_cfg ())
  in
  let glm_coding =
    Masc_mcp.Oas_worker_cascade.provider_name_of_config
...
let test_cascade_provider_labels_preserve_registered_openai_compat_family () =
  let provider_name =
    Masc_mcp.Oas_worker_cascade.provider_name_of_config (make_openrouter_provider_cfg ())
  in
  let model_label =
    Masc_mcp.Oas_worker_cascade.model_label_of_config (make_openrouter_provider_cfg ())
  in
...
let test_cascade_provider_labels_detect_kimi_from_endpoint_metadata () =
  let provider_name =
    Masc_mcp.Oas_worker_cascade.provider_name_of_config (make_kimi_provider_cfg ())
  in
  let model_label =
    Masc_mcp.Oas_worker_cascade.model_label_of_config (make_kimi_provider_cfg ())
  in
...
let test_resolve_tool_lane_for_codex_cli_keeper_bound_public_tools_omits_bound_tools () =
  ...
  | Ok (effective_tools, Some policy) ->
    ...
    Alcotest.(check (list string))
      ...
      policy.allowed_tool_names;
    Alcotest.(check (option string))
      "codex_cli strips keeper header"
      None
...
let test_filter_candidate_providers_for_tool_support_keeps_header_capable_cli_for_keeper_internal_tools
  ...
  let tools = [ make_named_noop_tool "keeper_bash" ] in
  let runtime_mcp_policy =
    Masc_mcp.Oas_worker_named.runtime_mcp_policy_for_tools ~keeper_name:"sangsu" tools
  in
  let filtered =
    Masc_mcp.Oas_worker_named.filter_candidate_providers_for_tool_support
      ~keeper_name:"sangsu"
...
let test_kimi_cli_runtime_mcp_jsons_include_request_policy () =
  ...
  in
  let merged = Oas_worker_exec.kimi_cli_runtime_mcp_jsons ~base:[] (Some policy) in
  Alcotest.(check int)
...
let test_kimi_cli_should_log_stderr_line_filters_resume_noise () =
  let should_log = Oas_worker_exec.Kimi_cli_transport_local.should_log_stderr_line in
    ...
    | Ok agent ->
      Fun.protect
        ...
        (fun () ->
           ...
           | Error
               (Oas.Error.Agent (Oas.Error.ToolRetryExhausted { attempts; limit; detail }))
             ->
...
let test_oas_worker_exec_run_exit_condition_result_returns_partial_success () =
  try
    ...
    in
    match Oas_worker_exec.run ~sw ~net:(require_test_net ()) ~config "say hello" with
    | Ok result ->
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let ctx =
         ...
         |> fun ctx ->
         Keeper_exec_context.append ctx (Agent_sdk.Types.user_msg (String.make 800 'x'))
...
let test_same_trace_multi_turn_accumulation () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let ctx_turn2 =
         Keeper_exec_context.append ctx_turn2 (Agent_sdk.Types.user_msg "turn 2 user")
         |> fun ctx ->
         Keeper_exec_context.append ctx (Agent_sdk.Types.assistant_msg "turn 2 reply")
       in
       let session2 = Keeper_exec_context.create_session ~session_id:trace_id ~base_dir in
       (match
          Keeper_exec_context.save_oas_checkpoint
            ~max_checkpoint_messages:120
            ~session:session2
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let ctx =
         ...
         |> fun c -> Keeper_exec_context.append c (Agent_sdk.Types.user_msg "msg3")
       in
       let meta = make_keeper_meta ~trace_id () in
       (match
          Keeper_exec_context.save_oas_checkpoint
  ...
  Alcotest.run
    ...
    ; ( "resume_config"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "oas_worker builds Kimi direct config"
            `Quick
