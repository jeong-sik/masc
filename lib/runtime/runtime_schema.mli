...
type provider =
  ...
  ; connect_timeout_s : float option
    (** Per-provider override for the OAS connect + initial-response-headers
      ...
      on the provider (not the model) because it is a transport property of
      the endpoint: a cloud OpenAI-compatible endpoint whose upstream behaves
      like a queued model (Ollama Cloud serving reasoning models whose TTFB
      exceeds the 60s OpenAI_compat default) requests a larger connect budget
      without changing wire-format kind. oas#2163, RFC-OAS-026 I2 — MASC
      declares the budget; OAS owns enforcement and phase=Http_operation
      attribution. *)
  }
[@@deriving show, eq]
