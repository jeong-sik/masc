...
Captured `chat/displayed.diff` and `chat/page-text.txt` intentionally retain their exact whitespace, including blank context lines. An unrestricted Git whitespace check flags those raw captures; the documentation and remaining changed files pass with only these two raw paths excluded. Their recorded hashes and Chat reconstruction proof must not be changed by whitespace normalization.
