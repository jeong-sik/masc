...
OAS Pin Snapshot: dependency floor `0.130.0`, runtime pin `main@v0.130.0`, declared base version `v0.130.0`
...
`masc-mcp` keeps the runtime pin ratcheted against upstream `main`, while the dependency floor tracks the pinned SDK declaration in `agent_sdk.opam`. Generated snapshot: runtime pin `main@v0.130.0`, declared base version `v0.130.0`, dependency floor `0.130.0`.
