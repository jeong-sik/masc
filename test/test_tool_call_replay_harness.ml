...
let load_fixture () =
  ...
  | Ok snapshots -> Alcotest.failf "expected one snapshot, got %d" (List.length snapshots)
...
let test_load_fixture_snapshot () =
  ...
  check (list string) "declared tools" [ "masc_add_task"; "masc_status" ] snapshot.tools;
  match snapshot.expected_tool_calls with
  | [ tool_call ] -> check string "expected tool" "masc_add_task" tool_call.name
...
let () =
  Alcotest.run
    ...
    [ ( "replay"
      , [ Alcotest.test_case "load fixture snapshot" `Quick test_load_fixture_snapshot
        ; Alcotest.test_case
            "validate fixture snapshot"
            `Quick
            test_validate_fixture_snapshot
        ; Alcotest.test_case
            "reject undeclared tool"
            `Quick
