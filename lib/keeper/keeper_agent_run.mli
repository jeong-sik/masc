...
val run_turn
  ...
  -> ?on_runtime_attempt_failed:
       (runtime_id:string -> error:Agent_core.Error.t -> unit)
       (* Called once per candidate that dispatched and errored, with that
          candidate's own id and the error it answered with. A failure
          returns no [run_result], so this is the only place the caller
          learns who answered and with what. *)
