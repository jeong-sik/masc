...
let test_existing_decision_note_is_validated_without_rewriting_history () =
  with_fixture (fun config task_id _ ->
    ...
    let independent_note = `Assoc (("notes", `String "operator annotation") :: fields) in
    Fs_compat.append_file dated.path (Yojson.Safe.to_string independent_note ^ "\n");
    let with_annotation = Fusion_decision.read ~config ~run_id:"run-advice"
      |> require "read independent historical note" in
    check bool "an independent note stays readable and unchanged" true
      (List.mem independent_note with_annotation);
    let blank_note = `Assoc (("notes", `String " ") :: fields) in
    Fs_compat.append_file dated.path (Yojson.Safe.to_string blank_note ^ "\n");
    match Fusion_decision.read ~config ~run_id:"run-advice" with
    | Error (Fusion_decision.Storage_failure _) -> ()
    | Error (Fusion_decision.Rejected _) | Ok _ ->
      fail "a blank stored note was accepted")
...
let () = run "Fusion decision attribution" ["behavior", [
  ...
  test_case "existing decision notes are validated without rewriting history" `Quick test_existing_decision_note_is_validated_without_rewriting_history;
