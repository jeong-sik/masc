...
let test_fresh_agent_healthy () =
  ...
  | Agent_health.Unhealthy r ->
    fail (Printf.sprintf "expected Healthy, got Unhealthy(%s)" r)
  | Agent_health.Unknown raw ->
    fail (Printf.sprintf "expected Healthy, got Unknown(%S)" raw)
;;
...
let test_unhealthy_has_reason () =
  ...
  | Agent_health.Unknown raw ->
    fail (Printf.sprintf "expected Unhealthy, got Unknown(%S)" raw)
;;
...
let test_status_to_string () =
  check
    ...
    "healthy"
    "healthy"
    (Agent_health.health_status_to_string Agent_health.Healthy);
  check
    string
    "recovering"
    "recovering"
    (Agent_health.health_status_to_string Agent_health.Recovering);
  check
    string
    "unhealthy"
    "unhealthy"
    (Agent_health.health_status_to_string (Agent_health.Unhealthy "x"));
  (* Issue #8607 *)
  check
    string
    "unknown"
    "unknown"
    (Agent_health.health_status_to_string (Agent_health.Unknown "throttled"))
;;
...
let test_summary_to_json () =
  ...
  check
    ...
    true
    (try
       ignore (Yojson.Safe.Util.member "agent_name" json);
       true
     with
     | _ -> false);
  check
    ...
    true
    (try
       ignore (Yojson.Safe.Util.member "status" json);
       true
     with
     | _ -> false)
;;
  ...
  run
    ...
    [ ( "check_health"
      , [ test_case "fresh agent is healthy" `Quick test_fresh_agent_healthy
        ...
        ; test_case
            "success resets partial failures"
            `Quick
