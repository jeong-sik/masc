...
type sweep_config =
  ...
  ; stagnant_days : int
    (** Drop Active goals with no update after this many days. Default 30. *)
  }

let default_config = { dropped_ttl_days = 7; stagnant_days = 30 }
...
let sweep_result_to_yojson r =
  `Assoc
    [ "purged", `Int r.purged; "stagnated", `Int r.stagnated; "orphans", `Int r.orphans ]
;;
  ...
  let result =
    ...
    |> List.filter_map (fun (g : Goal_store.goal) ->
      ...
      | Goal_phase.Dropped, Some d when d >= config.dropped_ttl_days ->
        incr purged;
        Log.Misc.info "[GoalJanitor] purge: %s (%s, dropped %d days ago)" g.id g.title d;
        None
      | Goal_phase.Executing, Some d when d >= config.stagnant_days ->
        incr stagnated;
        Log.Misc.info
          "[GoalJanitor] stagnate: %s (%s, no update for %d days)"
          g.id
          g.title
          d;
        Some
          ...
          ; last_review_note =
              Some (Printf.sprintf "auto-dropped: no update for %d days" d)
...
let prune_active_goal_ids ~(valid_goal_ids : string list) (active_ids : string list)
  : string list * int
  =
  let before = List.length active_ids in
  let pruned = List.filter (fun id -> List.mem id valid_goal_ids) active_ids in
  let removed = before - List.length pruned in
  if removed > 0
  then Log.Misc.info "[GoalJanitor] pruned %d orphaned active_goal_ids" removed;
  pruned, removed
;;
...
let run ?(config = default_config) (room_config : Coord.config) : sweep_result =
  ...
  if partial.purged > 0 || partial.stagnated > 0
  then
    Goal_store.write_state
      room_config
      { goals = goals'; version = st.version + 1; updated_at = Types.now_iso () };
  (* Prune active_goal_ids from all keeper metas *)
  let total_orphans = ref 0 in
  let keeper_dir = Filename.concat (Coord.masc_dir room_config) "keepers" in
  if Sys.file_exists keeper_dir && Sys.is_directory keeper_dir
  then
    Sys.readdir keeper_dir
    |> Array.iter (fun entry ->
      if Filename.check_suffix entry ".json"
      then (
        ...
        | Ok (Some meta) when meta.active_goal_ids <> [] ->
          ...
          then (
            ...
            | Error e ->
              Log.Misc.warn
                "[GoalJanitor] failed to persist orphan-pruned meta for keeper=%s \
                 removed=%d: %s"
                name
                removed
                e)
