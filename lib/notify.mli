(** Notify — macOS desktop notification for a keeper mention.
    ...
    The post is a projection on the keeper's tool calls that mention an
    agent -- masc_broadcast, masc_board_post and masc_board_comment -- and a
...
(** Post "@from_agent mentioned you" with [message] as the body. Returns when
    the notifier has exited or its bound has passed. A notifier still running
    at the bound is stopped with SIGTERM, then SIGKILL after
    {!Process_eio.child_exit_grace_seconds}, so the call returns within the
    bound plus two of those. Every failure is a log line and none reaches the
    caller, except Eio cancellation, which is re-raised. *)
