...
let legacy_verifications_dir base_path = Filename.concat base_path "verifications"
  ...
  with_temp_base_path (fun base_path ->
    let _req =
      create_pending_request
        ...
        ~worker:"keeper-alpha"
        ~criteria:
          [ V.Custom "Must reduce FD leak"; V.Custom "Must pass integration tests" ]
        ~evidence:[ "artifacts/lsof.before"; "artifacts/lsof.after" ]
    in
    ...
    (match member "total" j with
     | `Int n -> Alcotest.(check int) "total = 1" 1 n
    ...
    List.iter
      (fun key ->
         ...
         | _ ->
           Alcotest.fail
             (Printf.sprintf
                "%s should be string, got %s"
                key
                (Yojson.Safe.to_string (member key r))))
    ...
    List.iter
      (fun key ->
         match member key r with
         | `String _ | `Null -> ()
         | _ -> Alcotest.fail (Printf.sprintf "%s should be string or null" key))
      nullable_string_fields;
    (* List fields *)
    (match member "completion_contract" r with
     | `List items ->
       Alcotest.(check int) "completion_contract len" 2 (List.length items);
       List.iter
         (function
           | `String _ -> ()
    ...
    (match member "required_evidence" r with
     | `List items ->
       Alcotest.(check int) "required_evidence len" 2 (List.length items);
       List.iter
         (function
           | `String _ -> ()
    ...
    | `String s -> Alcotest.fail (Printf.sprintf "task_title mismatch: got %S" s)
  ...
  with_temp_base_path (fun base_path ->
    ...
    let _ =
      create_pending_request
        ...
        ~task_id:"task-A"
        ~worker:"alpha"
        ~criteria:[ V.Custom "A criterion 2" ]
        ~evidence:[ "ref-A2" ]
    in
    let _ =
      create_pending_request
    ...
    (match member "total" j_all with
     | `Int 3 -> ()
     | `Int n -> Alcotest.fail (Printf.sprintf "expected 3 total, got %d" n)
    ...
    (match member "total" j_a with
     | `Int 2 -> ()
     | `Int n -> Alcotest.fail (Printf.sprintf "expected 2 for task-A, got %d" n)
     | _ -> Alcotest.fail "total not int");
     ...
     | `List reqs ->
       List.iter
         (fun r ->
            ...
            | `String "task-A" -> ()
            | `String s -> Alcotest.fail (Printf.sprintf "expected task-A, got %s" s)
    ...
    (match member "total" j_none with
     | `Int 0 -> ()
     | `Int n -> Alcotest.fail (Printf.sprintf "expected 0, got %d" n)
     | _ -> Alcotest.fail "total not int");
...
let test_summary_bucket_counts () =
  with_temp_base_path (fun base_path ->
    ...
    let verdict_of req ~verdict =
      match V.submit_verdict ~base_path ~req_id:req.V.id ~verifier:"v" ~verdict with
      | Ok _ -> ()
...
let test_summary_recent_clamp () =
  with_temp_base_path (fun base_path ->
    ...
    in
    (match
       V.submit_verdict ~base_path ~req_id:r.V.id ~verifier:"v" ~verdict:(V.Fail "x")
     with
     | Ok _ -> ()
     | Error e -> Alcotest.fail e);
  ...
  Alcotest.run
    ...
    [ ( "requests_json"
      , [ Alcotest.test_case "shape" `Quick test_requests_json_shape
        ...
        ; Alcotest.test_case
            "ignores legacy root entries"
            `Quick
            test_requests_json_ignores_legacy_root_entries
        ; Alcotest.test_case
            "conflict triage fields"
            `Quick
    ...
    ; ( "summary_json"
      , [ Alcotest.test_case "empty base_path" `Quick test_summary_empty
        ; Alcotest.test_case
            "bucket counts + recent rejections"
            `Quick
            test_summary_bucket_counts
