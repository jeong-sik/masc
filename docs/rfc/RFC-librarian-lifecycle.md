...
| Goal 기준 | durable·continuity: 없음(`No_task`). queue: Keeper 의 지금 task(`Keeper_librarian_input_sources.goal_context_for_task`) | durable·continuity 회차는 턴 끝 줄에 task 정체성이 없어 싣지 않는다(§8 "멈춘 Keeper 의 역할과 task"). 지금 task 를 지나간 턴의 것으로 추측하지 않는다. queue 회차는 턴 범위 없이 지금 밀린 입력만 정리하므로 지금 task 가 곧 그 입력의 task 다 |
