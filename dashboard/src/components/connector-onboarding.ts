  ...
  return html`
    <div class="rounded border border-[var(--white-8)] p-4" style=${connectorAccentStyle(connectorId)}>
      <div class="mb-2 flex items-center justify-between gap-2">
        ...
        <button type="button"
          class=${`rounded cursor-pointer transition-all duration-200 font-medium border border-solid border-[var(--accent-30)] bg-[var(--accent-12)] text-[var(--text-strong)] hover:bg-[var(--accent-20)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-45)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg-1)] py-1 px-2 text-2xs ${starting ? 'opacity-50 pointer-events-none' : 'active:scale-[0.97]'}`}
          disabled=${starting}
      ...
      <div class="text-2xs text-[var(--text-dim)]">
        <strong>시작</strong>을 누르면 backend가 sidecar를 spawn합니다. 또는 명령을 복사해 새 터미널에서 직접 실행하세요.
      </div>
    ...
    <div role="region" aria-label="커넥터 온보딩">
      <div class="mb-3">
        ...
        <div class="mt-1 text-2xs text-[var(--text-dim)]">
          ...
          <strong>전체 시작</strong>으로 한 번에 spawn하세요. spawn 후 이 화면이 라이브 상태로 갱신됩니다.
        </div>
      </div>
