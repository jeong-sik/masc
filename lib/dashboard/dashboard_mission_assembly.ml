...
let keeper_tool_audit_json_fields config registry_lookup keeper agent_name =
  ...
  in
  let fallback_latest = string_list_of_json (member_assoc "latest_tool_names" keeper) in
  let fallback_count =
  ...
  in
  let fallback_at = trim_to_option (string_field "tool_audit_at" keeper) in
  let file_snapshot =
    let keeper_updated_at = trim_to_option (string_field "updated_at" keeper) in
    match
      Keeper_exec_status_metrics.latest_tool_audit_snapshot_from_files config ~keeper_name
    with
    | Some snapshot ->
      Some
        { snapshot with
          tool_audit_at =
            (match
               snapshot.tool_audit_source, snapshot.tool_audit_at, keeper_updated_at
             with
  ...
  let ( allowed_tool_names
    ...
    =
    match
      ( A2a_tools.latest_heartbeat_task agent_name
      , A2a_tools.latest_heartbeat_result agent_name )
    with
      ...
      (match file_snapshot with
       ...
       | None ->
         ...
         then (
           ...
           let total =
             List.fold_left (fun acc (_, e) -> acc + e.Keeper_tools_oas.count) 0 tracked
           in
           let latest_at =
             List.fold_left
               (fun acc (_, e) -> max acc e.Keeper_tools_oas.last_used_at)
               0.0
               tracked
           in
           let at_str =
             if latest_at > 0.0
             then Some (Dashboard_utils.iso_of_unix latest_at)
             else None
           in
...
let build_keeper_briefs (config : Coord.config) (keepers : Yojson.Safe.t list) =
  ...
  let registry_lookup name =
    List.find_opt
      (fun (e : Keeper_registry.registry_entry) -> String.equal e.name name)
      all_entries
  in
  keepers
  |> List.filter_map (fun keeper ->
    ...
    then None
      ...
      in
      let pressure_rank =
        if Dashboard_utils.is_keeper_offline status
        then 3
        else if Option.value ~default:0.0 context_ratio >= lane_pressure_ctx_ratio
        then 2
        else if status = "idle"
        then 1
        else 0
            ...
            `Assoc
              ([ "name", `String name
               ...
               ; ( "last_autonomous_action_at"
                 , member_assoc "last_autonomous_action_at" keeper )
               ]
...
let operation_badge_json (operation : operation_context) =
  let status_str =
    ...
    | None -> "unknown"
  in
  let detachment_status_str = operation.detachment_status in
  `Assoc
...
let operation_badges_for_session session operation_contexts =
  let linked_operation_ids =
    ...
    |> List.filter_map (fun (operation : operation_context) ->
      ...
      | Some session_id when String.equal session_id session.session_id ->
...
let build_sessions sessions attention_queue agent_briefs keeper_briefs =
  ...
  |> List.sort
       (fun (left_count, left_sev, left_ts, _) (right_count, right_sev, right_ts, _) ->
