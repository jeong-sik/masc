...
let ensure_masc_dir (config : Coord.config) =
  ...
  if not (Sys.file_exists dir) then Coord_utils.mkdir_p dir
;;

let load_governance (config : Coord.config) : governance_config =
  let path = governance_path config in
  if Coord_utils.path_exists config path
  then (
    let json = Coord_utils.read_json config path in
    let module U = Yojson.Safe.Util in
    let level =
      Json_util.get_string json "level" |> Option.value ~default:"development"
    in
  ...
  else governance_defaults "development"
;;

let save_governance (config : Coord.config) (g : governance_config) =
  ensure_masc_dir config;
  let json =
    match governance_config_to_yojson g with
    | `Assoc fields -> `Assoc (fields @ [ "updated_at", `String (Types.now_iso ()) ])
    | other -> other
  in
