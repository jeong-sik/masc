...
- Keeper: a failed `keeper_memory_write` or `keeper_memory_retract` returns to the model and does not end the turn, including a failure after the store committed. The failure result carries `effect_disposition` and a `what_committed` sentence chosen by the error kind: nothing was committed for a refused input or a `fact_not_found` retraction, which also lists why the id may be absent; a new revision without the claim for `commit_receipt_inconsistent`; and for `persistence_failed`, that the write may or may not have committed and memory should be searched first, with how a repeat behaves for an ordinary fact (the same title and content make the same fact) or a source-bound one (the same `source_path` replaces that path's claim) (#36510, #36521, #36532).
...
- Keeper: health is `healthy`, `idle` or `offline`: `offline` when the keepalive is not running, `idle` when it runs with no turn recorded, and `healthy` otherwise. The time since the last heartbeat line does not enter health, `next_action_path`, `recoverable`, the surface status (`active`, `idle` or `offline`), the TUI roster mark (`●` healthy, `·` idle, `×` offline, `○` paused, `-` unread) or `masc_keeper_list` rows, so a Keeper in a turn longer than six minutes is not reported `stale`. Keeper status JSON carries no `last_heartbeat`, `last_heartbeat_age_s`, `heartbeat_stale_after_s` or `heartbeat_observation_error`. The dashboard draws a Keeper's status from its registry phase, so a `Running` Keeper shows as running and `Crashed`, `Restarting` and `Failing` show those words; it has no heartbeat-lost badge or stale count, and takes activity times from the last activity, tool audit or turn. The TUI Overview has no inactive count, and a `masc-tui` from this release refuses a keeper list that reports another health value, such as `stale` from a server still running 0.35.17 (#36548, #36549, #36550).
- Keeper: an interrupt ends a turn that is waiting for its own Keeper owner to answer, such as a turn that asked the owner to wait until it is idle, so that turn releases its slot instead of leaving every later message to report that the previous turn has not finished. Claiming the next chat operation still completes when an interrupt arrives, so the claimed operation is not left `Running` until the next boot (#36542, #36551).
...
- Keeper: on a long history, measuring the model input before each provider request reuses the size of a message measured earlier in the attempt even when the request rebuilds the message records, and a checkpoint save during a turn encodes and validates only the messages added since the turn's previous save; the checkpoint bytes are unchanged (#36523, #36544).
- TUI: the Activity pane draws one row per Keeper turn instead of one `unsettled` row per provider call inside it, and finishing the turn settles that row (#36577).

### Documentation

- Skills: the builtin `browser-lanes` Skill tells a Keeper to read a site reference again in a new turn, and to use a site Skill listed separately in Available Skills together with the bundled reference, following that Skill where they differ. `browser-live-follow-read` says a login page or unrelated destination stays unverified and a read that still shows the original URL or document means navigation has not happened; `browser-navigate-read` names `keeper_compose_browser-live-follow-read` only for when the tool list has it; both browser compositions' frontmatter description equals their tool description, so `keeper_capability_search` shows the same text. `run-and-read` says `exit` is kind `exited` with a `code` or kind `signalled` with a `signal`, and `skill-authoring` says a composition checks literal and parameter inputs before any node runs (#36505, #36543).
- Skills: `docs/guides/builtin-skill-updates.md` lists what `masc init` and server start each do per package state, and describes the backup, staging, move note and lock rules; `docs/SKILLS.md` states that Agent-Core refuses an `enum` value outside the list and which members are load errors (#36507, #36514, #36517, #36527, #36531, #36539).
- Browser: `connectors/browser/host/README.md`, `docs/design/browser-lane.md` and the browser lanes guide describe `launch.json`, when a host moves to another server, and running `install-host.sh` again for a host installed without the declaration (#36516, #36537).
