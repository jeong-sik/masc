...
let test_violated_rejects () =
  ...
  | CVG.Allow -> Alcotest.fail "Violated should Reject"
;;
...
let test_violated_no_findings_still_rejects () =
  ...
  match CVG.check_verdict v with
  | CVG.Reject _ -> ()
  | CVG.Allow -> Alcotest.fail "Violated with no findings should still Reject"
;;
...
let test_inconclusive_blocking_gap_rejects () =
  ...
  | CVG.Allow -> Alcotest.fail "Inconclusive with blocking gap should Reject"
;;
...
let test_review_gap_mentions_verification_fsm () =
  ...
  | CVG.Reject msg ->
    Alcotest.(check bool)
      "mentions submit_for_verification"
      true
      (Astring.String.is_infix ~affix:"Submit for verification" msg)
  | CVG.Allow -> Alcotest.fail "Review gap should Reject"
;;
...
let test_inconclusive_mixed_gaps_rejects () =
  ...
  match CVG.check_verdict v with
  | CVG.Reject _ -> ()
  | CVG.Allow -> Alcotest.fail "Mixed gaps with at least one blocking should Reject"
;;
...
let write_verdict_jsonl ~base_dir ?task_id (verdict : CT.contract_verdict) =
  ...
  let date_dir = Printf.sprintf "%04d-%02d" (today.tm_year + 1900) (today.tm_mon + 1) in
...
let test_gate_no_verdict_rejects () =
  with_temp_dir (fun base_dir ->
    ...
    | None -> Alcotest.fail "No verdict should reject")
;;
...
let test_gate_violated_verdict_rejects () =
  with_temp_dir (fun base_dir ->
    ...
    | Some _ -> ()
    | None -> Alcotest.fail "Violated verdict should reject")
;;
...
let test_gate_different_task_id_not_found () =
  with_temp_dir (fun base_dir ->
    ...
    | Some _ -> ()
    | None -> Alcotest.fail "Different task_id should not match")
;;
...
let test_to_attribution_inconclusive_blocking_is_policy_failed () =
  ...
  Alcotest.(check bool)
    ...
    true
    (match attr.outcome with
     | A.Policy_failed _ -> true
     | _ -> false);
  Alcotest.(check (option int))
    "evidence.blocking_gaps_count"
    (Some 1)
  ...
  Alcotest.run
    ...
    [ ( "check_verdict"
      , [ Alcotest.test_case "satisfied allows" `Quick test_satisfied_allows
        ...
        ; Alcotest.test_case
            "violated no findings rejects"
            `Quick
            test_violated_no_findings_still_rejects
        ; Alcotest.test_case
            "inconclusive blocking gap rejects"
            `Quick
            test_inconclusive_blocking_gap_rejects
        ; Alcotest.test_case
            "review gap mentions verification fsm"
            `Quick
            test_review_gap_mentions_verification_fsm
        ; Alcotest.test_case
            "inconclusive annotation only allows"
            `Quick
            test_inconclusive_annotation_only_allows
        ; Alcotest.test_case
            "inconclusive no gaps allows"
            `Quick
            test_inconclusive_no_gaps_allows
        ; Alcotest.test_case
            "inconclusive mixed gaps rejects"
            `Quick
            test_inconclusive_mixed_gaps_rejects
        ] )
    ; ( "gate_check_integration"
      , [ Alcotest.test_case "no verdict rejects" `Quick test_gate_no_verdict_rejects
        ; Alcotest.test_case
            "satisfied verdict allows"
            `Quick
            test_gate_satisfied_verdict_allows
        ; Alcotest.test_case
            "violated verdict rejects"
            `Quick
            test_gate_violated_verdict_rejects
        ; Alcotest.test_case
            "different task_id not found"
            `Quick
            test_gate_different_task_id_not_found
    ...
    ; ( "attribution"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "violated → Policy_failed with reason"
            `Quick
            test_to_attribution_violated_is_policy_failed
        ; Alcotest.test_case
            "inconclusive+blocking → Policy_failed"
            `Quick
        ...
        ; Alcotest.test_case
            "missing verdict envelope"
            `Quick
            test_attribution_missing_verdict
        ; Alcotest.test_case
            "advisory gate label on to_attribution"
            `Quick
            test_to_attribution_honors_advisory_label
        ; Alcotest.test_case
            "advisory gate label on missing verdict"
            `Quick
            test_attribution_missing_verdict_honors_advisory_label
        ; Alcotest.test_case
            "strict vs advisory labels are distinct"
            `Quick
            test_gate_labels_are_distinct
        ; Alcotest.test_case "yojson roundtrip" `Quick test_attribution_roundtrips_to_json
        ] )
    ]
;;
