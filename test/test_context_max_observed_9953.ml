...
let () =
  let dir =
    Filename.concat
      ...
      (Printf.sprintf "masc-test-context-max-observed-9953-%06x" (Random.bits ()))
  in
...
let test_bucket_boundary_64k () =
  Alcotest.(check string) "64_000 → 64k" "64k" (UM.context_max_bucket 64_000);
  Alcotest.(check string) "64_001 → 128k" "128k" (UM.context_max_bucket 64_001)
;;

let test_bucket_boundary_256k () =
  Alcotest.(check string) "262_144 → 256k" "256k" (UM.context_max_bucket 262_144);
  Alcotest.(check string) "262_145 → 1m" "1m" (UM.context_max_bucket 262_145)
;;

let test_bucket_boundary_1m () =
  Alcotest.(check string) "1_048_576 → 1m" "1m" (UM.context_max_bucket 1_048_576);
  Alcotest.(check string) "1_048_577 → other" "other" (UM.context_max_bucket 1_048_577)
;;
...
let test_record_increments_correct_bucket () =
  ...
  let before_1m =
    counter_for ~keeper ~model_used:model ~resolved_model_id:resolved ~bucket:"1m"
  in
  let before_256k =
    counter_for ~keeper ~model_used:model ~resolved_model_id:resolved ~bucket:"256k"
  in
...
let test_drift_visible_as_two_bucket_rows () =
  ...
  let before_64k =
    counter_for ~keeper ~model_used:model ~resolved_model_id:resolved ~bucket:"64k"
  in
  let before_1m =
    counter_for ~keeper ~model_used:model ~resolved_model_id:resolved ~bucket:"1m"
  in
...
let test_keeper_label_isolation () =
  ...
  let before_b =
    counter_for
      ~keeper:"keeper-B-9953"
      ~model_used:model
      ~resolved_model_id:resolved
      ~bucket:"1m"
  in
  UM.record_context_max_observation
    ~keeper:"keeper-A-9953"
    ~model_used:model
    ~resolved_model_id:resolved
    ~context_max:1_000_000;
  Alcotest.(check (float 0.0001))
...
let () =
  Alcotest.run
    "context_max_observed_9953"
    [ ( "vocabulary"
      , [ Alcotest.test_case "bucket vocabulary table" `Quick test_bucket_vocabulary
        ; Alcotest.test_case "boundary 64k / 128k" `Quick test_bucket_boundary_64k
        ; Alcotest.test_case "boundary 256k / 1m" `Quick test_bucket_boundary_256k
        ; Alcotest.test_case "boundary 1m / other" `Quick test_bucket_boundary_1m
        ] )
    ; ( "counter_emission"
      , [ Alcotest.test_case
            "increments correct bucket"
            `Quick
            test_record_increments_correct_bucket
        ; Alcotest.test_case
            "drift visible as two bucket rows"
            `Quick
            test_drift_visible_as_two_bucket_rows
        ; Alcotest.test_case "keeper label isolation" `Quick test_keeper_label_isolation
        ] )
    ]
;;
