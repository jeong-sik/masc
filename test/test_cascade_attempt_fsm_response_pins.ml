...
|},
    ...
    ( "monthly_usage_limit",
      ...
      true );
    ( "resets_date",
      {|Error: quota will reset after 2026-05-10T00:00:00Z|},
      true );
...
let max_turns_fixtures =
  [
    ...
    ( "claude_code_terminal_reason_max_turns",
      ...
      true );
    ( "text_max_turns_exceeded",
      {|Error: max turns exceeded for this session.|},
      true );
    ( "text_reached_maximum",
      {|You have reached maximum number of turns allowed in this conversation.|},
      true );
    ( "negative_normal_completion",
      {|{"subtype":"success","cost_usd":0.03}|},
      false );
    ( "negative_auth_error",
...
let test_case_insensitive () =
  let upper =
    Cascade_attempt_fsm.message_looks_like_cli_wrapped_hard_quota
      "HARD_QUOTA: exceeded"
  in
  let mixed =
    Cascade_attempt_fsm.message_looks_like_cli_wrapped_hard_quota
...
let () =
  Alcotest.run "cascade_attempt_fsm_response_pins"
    [
      ( "hard_quota",
        [ Alcotest.test_case "pin fixtures" `Quick test_hard_quota_pins ] );
      ( "max_turns",
        [ Alcotest.test_case "pin fixtures" `Quick test_max_turns_pins ] );
      ( "case_insensitive",
        [ Alcotest.test_case "ci check" `Quick test_case_insensitive ] );
    ]
