...
let markdown_fixture key body =
  ...
  let meta_lines =
    ...
    @ (if template_variables = []
       then []
       else [ "template_variables: [" ^ String.concat ", " template_variables ^ "]" ])
    @ [ "---" ]
  in
  String.concat "\n" (meta_lines @ [ body ])
;;
...
let fixtures =
  ...
  ; ( "keeper.deliberation"
    , "Keeper {{keeper_name}} {{soul_profile}} {{goal}} {{triggers}} {{world_state}}" )
  ...
  run
    ...
    [ ( "registration"
      , [ test_case "all markdown-backed prompts are registered" `Quick (fun () ->
        ...
        ; test_case
            "validate_required_prompt_files detects missing file"
            `Quick
            (fun () ->
               with_registry
               @@ fun ~dir:_ ~prompts_dir ->
               Sys.remove (Filename.concat prompts_dir "dashboard.governance_judge.md");
    ...
    ; ( "rendering"
      , [ test_case "render_prompt_template uses markdown template" `Quick (fun () ->
            with_registry
            @@ fun ~dir:_ ~prompts_dir:_ ->
            match
            ...
            | Ok rendered ->
              check
                ...
                (String.length rendered > 0
                 &&
                 try
                   ignore (Str.search_forward (Str.regexp_string "TestKeeper") rendered 0);
                   true
                 with
                 | Not_found -> false)
            | Error msg -> fail msg)
    ...
    ; ( "override"
      , [ test_case "set_override replaces file content" `Quick (fun () ->
            ...
            (match Prompt_registry.set_override "keeper.constitution" override_text with
             | Ok () -> ()
             | Error msg -> fail msg);
            check
              string
        ...
        ; test_case "clear_override reverts to file" `Quick (fun () ->
            with_registry
            @@ fun ~dir:_ ~prompts_dir:_ ->
            (match Prompt_registry.set_override "keeper.world" "temporary override" with
             | Ok () -> ()
             | Error msg -> fail msg);
            Prompt_registry.clear_prompt_override "keeper.world";
            check
              string
              "back to file baseline"
              (fixture "keeper.world")
        ...
        ; test_case "set_override rejects unknown template variable" `Quick (fun () ->
            with_registry
            @@ fun ~dir:_ ~prompts_dir:_ ->
            match
    ...
    ; ( "integration"
      , [ test_case "keeper_constitution reads markdown-backed registry" `Quick (fun () ->
            with_registry
            @@ fun ~dir:_ ~prompts_dir:_ ->
            check
              string
              "keeper constitution function"
              (fixture "keeper.constitution")
              (Lib.Keeper_prompt.keeper_constitution ()))
        ] )
    ; ( "prompts_json"
      , [ test_case
            "prompts_json exposes effective file and override fields"
            `Quick
            (fun () ->
               with_registry
               @@ fun ~dir:_ ~prompts_dir:_ ->
               (match
                  Prompt_registry.set_override "keeper.capabilities" "runtime override"
                with
                | Ok () -> ()
                | Error msg -> fail msg);
               ...
               | `Assoc fields ->
                 check
                   int
                   "template_variables field exists"
                   0
