...
|------|--------|------|---------------|
| `runtime-alerts.yml` | runtime | alerts | `docs/observability/runtime-metrics.md` |
| `runtime-slo.yml` | runtime | recording rules + SLO alerts | `docs/observability/runtime-metrics.md` |
| `grafana-runtime-dashboard.json` | runtime | Grafana dashboard | `docs/observability/runtime-metrics.md` |
...
Tracks runtime-routing decisions inside `Keeper_turn_driver.cycle_loop`. Counters: `masc_runtime_strategy_decisions_total{runtime,strategy,kind}`, `masc_runtime_capacity_events_total{kind,key_type}`. State vocabulary: `ordered / filtered_empty / exhausted` (TLA+ `KeeperRuntimeLifecycle`).
...
The two domains overlap *only* at `runtime_routing` — when the keeper FSM enters runtime routing, the runtime subsystem takes over until a provider is selected (or exhausted). Both surfaces emit independently; an operator chasing a turn that died at runtime exhaustion sees:

- `masc_keeper_turn_fsm_transitions_total{to="failed:runtime_unavailable",keeper=...}` — keeper view
- `masc_runtime_strategy_decisions_total{kind="exhausted",runtime=...}` — runtime view
...
Prometheus picks up the alert / recording files via the standard rule_files glob; Grafana imports the JSON via File → Import. There's no apply script — deployments use whatever IaC they already run for the runtime rules.

## Companion code references

- `lib/prometheus.{ml,mli}` — counter + label declarations
- `lib/keeper/keeper_turn_fsm.{ml,mli}` — typed FSM ADT
- `lib/runtime/runtime_strategy_trace.ml` — runtime decision events
- `bin/masc_trace.ml` — per-turn timeline CLI (reads receipts + system_log + tool_calls)

## Adding a new artefact

1. Pick a domain (runtime / keeper-turn-fsm / new-domain). Don't mix domains in one file — each file maps to one companion doc.
