...
let test_workflow_guide_tools_exist () =
  let guide_tools =
    ...
    ; "masc_dispatch_tick" (* team session tools removed — team session cleanup *)
    ]
  in
  List.iter
    (fun tool_name ->
       ...
       List.iter
         (fun (s : WG.step) ->
            if not (List.mem s.tool all_schema_names)
            then
              Alcotest.fail
                (Printf.sprintf
                   "WG.next_steps(%s) references '%s' which is not in \
       ...
       List.iter
         (fun (s : WG.step) ->
            ...
            then
              Alcotest.fail
                (Printf.sprintf
                   "WG.next_steps(%s, fail) references '%s' which is not in \
                    Config.all_tool_schemas"
                   tool_name
                   s.tool))
         g_fail.next_steps)
...
let contains_token haystack needle =
  ...
  let n_len = String.length needle in
  let boundary_ok idx =
    let before_ok = idx = 0 || not (is_token_char haystack.[idx - 1]) in
    let after_idx = idx + n_len in
    let after_ok = after_idx >= h_len || not (is_token_char haystack.[after_idx]) in
    before_ok && after_ok
  in
  let rec loop i =
    if i + n_len > h_len
    then false
    else if String.sub haystack i n_len = needle && boundary_ok i
    then true
    else loop (i + 1)
  in
  n_len = 0 || loop 0
...
let extract_masc_tokens contents =
  ...
  let rec loop idx acc =
    if idx >= len
    then List.rev acc
    else if idx + 5 <= len && String.sub contents idx 5 = "masc_"
    then (
      let rec advance j =
...
let repo_root () =
  let has_docs_dir path = Sys.file_exists (Filename.concat path "docs") in
  match Sys.getenv_opt "DUNE_SOURCEROOT" with
  | Some root when has_docs_dir root -> root
  | _ ->
    let rec ascend path =
      if has_docs_dir path
      then path
      else (
...
let doc_path name = Filename.concat (Filename.concat (repo_root ()) "docs") name
let repo_path relative = Filename.concat (repo_root ()) relative
...
let test_docs_do_not_reintroduce_ghost_claim_surface () =
  ...
  |> List.iter (fun name ->
    ...
    if contains_substring contents "masc_task_list"
    then Alcotest.failf "doc %s reintroduces ghost tool masc_task_list" name;
    if contains_token contents "masc_claim" && not (List.mem name allowed_claim_docs)
    then
      Alcotest.failf
...
let test_benchmark_scripts_follow_session_contract () =
  let scripts = [ "benchmarks/quick-bench.sh"; "benchmarks/benchmark.sh" ] in
  List.iter
    (fun relative ->
       let contents = read_file (repo_path relative) in
       if
       ...
       then
         Alcotest.failf
           ...
           relative;
       if not (contains_token contents "MCP_SESSION_ID")
       then
         Alcotest.failf
...
let test_benchmark_scripts_only_reference_registered_tools () =
  ...
  let pruned_benchmark_exceptions =
    [ "masc_runtime_verify"; "masc_lock"; "masc_unlock" ]
  in
  let scripts = [ "benchmarks/quick-bench.sh"; "benchmarks/benchmark.sh" ] in
  List.iter
    (fun relative ->
       ...
       |> List.iter (fun tool_name ->
         if
           (not (List.mem tool_name all_schema_names))
...
let test_docs_do_not_reintroduce_removed_mode_surface () =
  let paths =
    [ repo_path "README.md"; doc_path "QUICK-START.md"; doc_path "MODE-SYSTEM.md" ]
  in
  List.iter
    (fun path ->
       ...
       then (
         ...
         List.iter
           (fun tool_name ->
              if contains_token contents tool_name
              then Alcotest.failf "doc %s reintroduces removed tool %s" path tool_name)
           [ "masc_switch_mode"
           ; "masc_get_config"
           ; "masc_tool_enable"
           ; "masc_tool_disable"
...
let test_admin_dispatched_keeper_tools_not_orphaned () =
  ...
  | Error msg -> Alcotest.failf "failed to load tool policy config: %s" msg
  | Ok () ->
    let validation = Tool_registration_check.validate () in
    List.iter
      (fun name ->
         if List.mem name validation.Tool_registration_check.orphan_toml
         then
           Alcotest.failf
    ...
    Fun.protect
      ...
      (fun () ->
         ...
         Alcotest.(check (list string))
           "no masc orphan_toml without injected schemas"
           []
           masc_orphans)
;;
...
let test_no_duplicate_schemas () =
  ...
  List.iter
    (fun name ->
       ...
       then duplicates := name :: !duplicates
  ...
  | dups ->
    Alcotest.fail
      (Printf.sprintf
         "Found %d duplicate tool schema(s):\n  %s"
         (List.length dups)
         (String.concat "\n  " dups))
;;
...
let test_board_delete_tag_registered () =
  ignore
    (Masc_mcp.Mcp_server_eio.create_state
       ~test_mode:true
       ~base_path:"/tmp/masc-pr3991-tag-reg"
       ());
  ...
  | Some _ -> Alcotest.fail "masc_board_delete should route through Mod_inline"
  | None -> Alcotest.fail "masc_board_delete missing from tag registry"
;;
...
let test_board_read_only_metadata_registered () =
  ignore
    (Masc_mcp.Mcp_server_eio.create_state
       ~test_mode:true
       ~base_path:"/tmp/masc-pr5973-board-meta"
       ());
  let meta = Masc_mcp.Tool_catalog.metadata "masc_board_list" in
  Alcotest.(check bool)
    "board list is read-only in dispatch"
    true
    (Masc_mcp.Tool_dispatch.is_read_only "masc_board_list");
  Alcotest.(check bool)
    "board list is idempotent in dispatch"
    true
    (Masc_mcp.Tool_dispatch.is_idempotent "masc_board_list");
  Alcotest.(check (option bool)) "board list metadata readonly" (Some true) meta.readonly;
  Alcotest.(check (option bool))
    "board list metadata idempotent"
    (Some true)
    meta.idempotent;
  Alcotest.(check bool)
    "board post stays mutable"
    false
    (Masc_mcp.Tool_dispatch.is_read_only "masc_board_post")
;;
...
let test_keeper_alias_ssot_consistency () =
  ...
  Alcotest.(check string)
    "keeper_tasks_list quirk"
    "masc_tasks"
    (Capability_registry.keeper_backend_tool_name "keeper_tasks_list");
  (* Privileged native tools must remain identity *)
  Alcotest.(check string)
    "keeper_bash identity"
    "keeper_bash"
    (Capability_registry.keeper_backend_tool_name "keeper_bash");
  Alcotest.(check string)
    "keeper_fs_edit identity"
    "keeper_fs_edit"
    (Capability_registry.keeper_backend_tool_name "keeper_fs_edit");
  (* Arbitrary unknown name must pass through *)
  Alcotest.(check string)
    "unknown identity"
    "foobar"
    (Capability_registry.keeper_backend_tool_name "foobar")
;;
...
let () =
  Alcotest.run
    "tool_registration_consistency"
    [ ( "linter"
      , [ Alcotest.test_case
            "workflow guide references valid tools"
            `Quick
            test_workflow_guide_tools_exist
        ; Alcotest.test_case
            "docs do not reintroduce ghost claim surface"
            `Quick
            test_docs_do_not_reintroduce_ghost_claim_surface
        ; Alcotest.test_case
            "front-door surfaces do not reintroduce claim alias"
            `Quick
            test_front_door_surfaces_do_not_reintroduce_claim_alias
        ; Alcotest.test_case
            "benchmark scripts follow session contract"
            `Quick
            test_benchmark_scripts_follow_session_contract
        ; Alcotest.test_case
            "benchmark scripts only reference registered tools"
            `Quick
            test_benchmark_scripts_only_reference_registered_tools
        ; Alcotest.test_case
            "docs do not reintroduce removed mode surface"
            `Quick
            test_docs_do_not_reintroduce_removed_mode_surface
        ; Alcotest.test_case "no duplicate tool schemas" `Quick test_no_duplicate_schemas
        ; Alcotest.test_case
            "board delete tag registered"
            `Quick
            test_board_delete_tag_registered
        ; Alcotest.test_case
            "board read-only metadata registered"
            `Quick
        ...
        ; Alcotest.test_case
            "keeper_backend_tool_name matches keeper_internal_replacement"
            `Quick
