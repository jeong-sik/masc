...
let resolve_active_goal_ids config p old_ids =
  let active_goal_ids =
    ...
    | Some ids -> ids
    | None -> Option.value ~default:old_ids p.profile_defaults.active_goal_ids
  in
  match p.active_goal_ids_opt with
  ...
  | Some _ ->
    ...
    in
    if missing = []
    then Ok active_goal_ids
    else Error (Printf.sprintf "unknown active_goal_ids: %s" (String.concat ", " missing))
;;
    ...
    (match resolve_active_goal_ids ctx.config p old.active_goal_ids with
     ...
     | Ok active_goal_ids ->
       ...
       let policy_voice_enabled =
         first_some
           ...
           (first_some
              (Some old.policy_voice_enabled)
              p.profile_defaults.policy_voice_enabled)
         |> Option.value ~default:false
       in
       let allowed_paths = Option.value ~default:old.allowed_paths p.allowed_paths_opt in
       let sandbox_profile =
       ...
       let network_mode =
         ...
         | None ->
           if
             Option.is_some p.sandbox_profile_opt
       ...
       let mention_targets =
         resolve_mention_targets
           ...
           ~fallback_targets:
             (if old.mention_targets <> []
              then old.mention_targets
       ...
       let tool_denylist =
         let profile_or_old =
           ...
           | Some _ as toml -> toml
           | None -> if old.tool_denylist <> [] then Some old.tool_denylist else None
         in
         resolve_tool_name_list ~preferred:p.tool_denylist_opt ~fallback:profile_or_old
       in
       let new_will =
         Option.value
           ~default:
             (if String.trim old.will <> ""
              then old.will
              else
                Option.value
     ...
     preserves the raw value; only prompt rendering applies the cap. *)
       let warn_personality_cap field old_value new_value =
         if not (String.equal old_value new_value)
         then (
           let len = String.length new_value in
           if len > Keeper_config.prompt_render_max_bytes
           then
             Log.Keeper.warn
               "update_keeper personality.%s for %s exceeds prompt cap (%d bytes > %d). \
                Stored as-is; truncated only at prompt rendering."
               field
               old.name
               len
       ...
       let updated =
         ...
         [canonical] column of [Dashboard_cascade.keeper_profile_json]. *)
             (match p.profile_defaults.cascade_name with
              | Some name -> name
              | None ->
                if String.trim old.cascade_name <> ""
                then old.cascade_name
         ...
         ; instructions =
             Option.value
               ~default:
                 (if String.trim old.instructions <> ""
                  then old.instructions
         ...
         ; autoboot_enabled
         ; active_goal_ids
         ; voice_enabled = Option.value ~default:old.voice_enabled p.voice_enabled_opt
         ; voice_channel =
             p.voice_channel_opt
             |> Option.map canonical_voice_channel
             |> Option.value ~default:old.voice_channel
         ; voice_agent_id = Option.value ~default:old.voice_agent_id p.voice_agent_id_opt
         ; mention_targets
         ...
         ; handoff_threshold =
             Option.value ~default:old.handoff_threshold p.handoff_threshold_opt
         ; handoff_cooldown_sec =
             Option.value ~default:old.handoff_cooldown_sec p.handoff_cooldown_sec_opt
         ; max_context_override =
             (match p.max_context_override_opt with
              | Some _ as v -> v
              | None -> old.max_context_override)
         ; updated_at = now_iso ()
         }
       in
       (match
        ...
        with
        | Error err ->
          Log.Keeper.warn "update_keeper failed sandbox validation for %s: %s" p.name err;
          false, err
        | Ok () ->
          (match
           ...
           | Error err ->
             Log.Keeper.warn
               "update_keeper failed sandbox preflight for %s: %s"
               p.name
               err;
             false, err
           | Ok () ->
