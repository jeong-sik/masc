...
let seeded () =
  let t = R.create () in
  R.register_running t ~run_id:"r-run" ~keeper:"k1" ~preset:"balanced" ~roster:Fusion_types.preset_roster ~topology:Fusion_types.Simple ~started_at:300.0;
  R.register_running t ~run_id:"r-done" ~keeper:"k1" ~preset:"deep" ~roster:Fusion_types.preset_roster ~topology:Fusion_types.Simple ~started_at:100.0;
  R.mark_completed t ~run_id:"r-done" ~outcome:R.Succeeded;
  R.register_running t ~run_id:"r-fail" ~keeper:"k1" ~preset:"deep" ~roster:Fusion_types.preset_roster ~topology:Fusion_types.Simple ~started_at:200.0;
  ...
  R.register_running
    ...
    ~run_id:"r-foreign"
    ~keeper:"k2"
    ~preset:"balanced" ~roster:Fusion_types.preset_roster ~topology:Fusion_types.Simple
    ~started_at:400.0;
  t
;;
