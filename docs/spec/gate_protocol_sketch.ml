...
let payload_content_to_json (p : payload_content) : Yojson.Safe.t =
  let text_field = match p.text with Some t -> [("text", `String t)] | None -> [] in
  let media_field = match p.media with [] -> [] | m -> [("media", `List (List.map media_attachment_to_json m))] in
  let audio_field = match p.audio_blob with Some a -> [("audio_blob", `String a)] | None -> [] in
