...
let test_max_concurrent_metric_tracks_capacity () =
  Eio_main.run (fun _env ->
    ...
    let initial =
      Masc_mcp.Prometheus.metric_value_or_zero "masc_inference_queue_max_concurrent" ()
    in
...
let test_snapshot_json_shape () =
  Eio_main.run (fun _env ->
    ...
    match json with
    | `Assoc fields ->
      check bool "has max_concurrent" true (List.mem_assoc "max_concurrent" fields);
      check bool "has queue_depth" true (List.mem_assoc "queue_depth" fields);
      check bool "has waiters" true (List.mem_assoc "waiters" fields)
...
let test_with_permit_releases_inflight_gauge () =
  Eio_main.run (fun _env ->
    let before =
      Masc_mcp.Prometheus.metric_value_or_zero "masc_inference_queue_inflight" ()
    in
    (match
       AQ.with_permit
         ~priority:Interactive
         ~keeper_name:"metric-test"
         ~cascade_name:"test"
         (fun () -> ())
     with
     | Ok () -> ()
...
let test_with_permit_increments_acquired_counter () =
  Eio_main.run (fun _env ->
    let before =
      Masc_mcp.Prometheus.metric_value_or_zero "masc_inference_queue_acquired_total" ()
    in
    (match
       AQ.with_permit
         ~priority:Interactive
         ~keeper_name:"counter-test"
         ~cascade_name:"test"
         (fun () -> ())
     with
     | Ok () -> ()
  ...
  run
    ...
    [ ( "passthrough"
      , [ test_case "with_permit runs" `Quick test_with_permit_runs
        ...
        ; test_case
            "wait timeout passthrough no leak"
            `Quick
    ...
    ; ( "metric_regression"
      , [ test_case
            "with_permit balances inflight gauge"
            `Quick
            test_with_permit_releases_inflight_gauge
        ; test_case
            "with_permit releases on exception"
            `Quick
            test_with_permit_releases_on_exception
        ; test_case
            "with_permit increments acquired counter"
            `Quick
            test_with_permit_increments_acquired_counter
        ; test_case
            "try_with_permit balances inflight gauge"
            `Quick
