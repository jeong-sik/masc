...
- An official-client session binding's context frontier now requires
  `held_context`. A binding written before this release has none and does not
  load, so that Keeper's official-client turns fail on the binding load. Stop the server, delete
  `<base-path>/.masc/keepers/*/official-client-runtime/session.json`, then
  start it again; each Keeper starts a fresh vendor session (#38986).
