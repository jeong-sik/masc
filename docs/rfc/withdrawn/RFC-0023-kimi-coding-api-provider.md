...
|---|---|---|---|
| `provider-c` (CLI) | local binary `model-c-coding` | `Cli_agent` | Active in runtime |
| `provider-c-api` | `api.provider-b.ai` | `Direct_api` | Registered, not in runtime |
...
3. Smoke: add to runtime, run keeper turn, verify routing
