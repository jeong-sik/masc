...
원문 checkpoint를 그대로 유지하면서 실패를 관측한다.
...
| 유지됨 | 바뀜 | 보장되지 않음 |
|--------|------|---------------|
| keeper 이름, persona, instructions, typed goal/task links | `trace_id`, session 디렉터리, checkpoint 저장 위치, `generation`, `last_handoff_ts` | 과거 전체 회상, 자유로운 cross-generation 검색, old/new generation 동시 실행 |
| 현재 작업 continuity를 위한 checkpoint/context | `trace_history`에 이전 trace가 추가 | 세대별로 독립된 child keeper 존재 |

**Generation 증가 메커니즘**:
1. post-turn lifecycle가 handoff 요청을 관측한다.
...
`network_mode`는 `masc_keeper_up` 인자가 아니다. 지속 설정은 Keeper TOML 또는
profile 기본값이 소유하며, `masc_keeper_up`에는 격리 경계인
`sandbox_profile`만 전달한다. Managed container를 직접 시작할 때는
`masc_keeper_sandbox_start`의 선택적 `network_mode` 인자로 그 시작에만 적용할
값을 지정할 수 있고, 생략하면 Keeper의 지속 설정을 사용한다.
...
- hard mode 실행 검사는 Docker `SecurityOptions`에서 `rootless`와 `userns`를 모두 요구한다. 조건이 충족되지 않으면 해당 sandbox 실행만 명시적으로 실패하며 Keeper 생성과 다른 활동은 계속된다.
...
- keeper Docker 컨테이너에는 `masc.mcp.component=keeper-sandbox`와 base path hash 라벨이 붙는다. 중지됐거나 owner process가 끝났거나 명시적 `ttl_sec`가 지난 컨테이너만 정리하며, TTL을 생략하거나 0으로 지정한 컨테이너는 나이만으로 제거하지 않는다. 조정값은 `MASC_KEEPER_SANDBOX_CLEANUP_ENABLED`, `MASC_KEEPER_SANDBOX_CLEANUP_INTERVAL_SEC`이다.
...
- `masc_keeper_status`의 `sandbox_live`에서 `sandbox_profile=docker`, `effective_mode=oneshot_or_managed_inherit`, `container_count=0`이면 "Docker keeper지만 현재 prewarmed container는 없고, sandboxed tool call 때 one-shot Docker를 쓴다"는 뜻이다.
...
| `timeout_sec` | 없음 (unbounded) | 선택적 명시 타임아웃. 유한하고 0보다 큰 초 단위 숫자만 허용한다. |
...
| 필드 | 타입 | `-`일 때 의미 | 채우는 방법 |
|------|------|--------------|------------|
| `instructions` | string | keeper TOML 또는 빈 값 | profile.json의 `keeper.instructions` |
...
Memory write는 시스템 소유 post-turn 경로가 수행한다. 보존과 회상은
Memory OS의 typed fact/episode 구조와 retention policy로 결정하며,
assistant reply의 라벨이나 키워드 순위로 상태를 만들지 않는다.
...
```json
{
  "name": "helper",
  "keeper": {
    "instructions": "사용자 질문에 근거를 붙여 응답한다."
  }
}
```

**표준 (instructions + traits 포함)**:
```json
{
  "name": "sangsu",
  "role": "커뮤니티 매니저",
  ...
  "keeper": {
    "instructions": "갈등 상황에서는 근거를 확인하고 중립적으로 조율한다."
  }
}
```
...
```json
{
  ...
  "trait": "분석적이고 꼼꼼한 탐구자",
  "keeper": {
    "instructions": "연구 원문을 확인하고 출처와 불확실성을 함께 기록한다."
  }
}
```
...
**복원 방법**:
- checkpoint에서 generation과 messages를 추출
