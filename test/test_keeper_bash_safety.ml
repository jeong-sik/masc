...
let error_msg = function
  | Error m -> Masc_mcp.Worker_dev_tools.block_reason_to_string m
  | Ok () -> ""
;;
...
let test_blocked_commands () =
  ...
  List.iter
    (fun cmd ->
       Alcotest.(check bool)
         (Printf.sprintf "blocked: %s" cmd)
         true
         (is_error (validate cmd)))
    blocked
;;
...
let test_shell_metachar_blocked () =
  ...
  List.iter
    (fun cmd ->
       ...
       Alcotest.(check bool)
         (Printf.sprintf "metachar blocked: %s" cmd)
         true
         (is_error result);
       let msg = error_msg result in
       Alcotest.(check bool) "error mentions chaining" true (String.length msg > 0))
    chained
;;
...
let test_playground_path_structure () =
  ...
  Alcotest.(check string)
    ...
    (playground_path_of "cheolsu");
  Alcotest.(check string)
    "masc-improver"
    ".masc/playground/masc-improver/"
    (playground_path_of "masc-improver")
;;
...
let test_playground_guard_inside () =
  ...
  Alcotest.(check bool)
    "exact playground dir"
    true
    (is_inside_playground ~playground_abs:pg pg);
  Alcotest.(check bool)
    "repos subdir"
    true
    (is_inside_playground ~playground_abs:pg (pg ^ "/repos/masc-mcp"));
  Alcotest.(check bool)
    "deep nested"
    true
    (is_inside_playground ~playground_abs:pg (pg ^ "/repos/masc-mcp/lib/keeper"))
;;
...
let test_playground_guard_outside () =
  ...
  Alcotest.(check bool)
    "project root"
    false
    (is_inside_playground ~playground_abs:pg "/project");
  Alcotest.(check bool)
    "project lib"
    false
    (is_inside_playground ~playground_abs:pg "/project/lib/keeper");
  Alcotest.(check bool)
    "other keeper playground"
    false
    (is_inside_playground ~playground_abs:pg "/project/.masc/playground/sangsu");
  Alcotest.(check bool)
    "playground parent"
    false
    (is_inside_playground ~playground_abs:pg "/project/.masc/playground");
  (* Prefix attack: cheolsu2 should not match cheolsu *)
  Alcotest.(check bool)
    "prefix attack (cheolsu2)"
    false
    (is_inside_playground ~playground_abs:pg (pg ^ "2/repos"))
;;

let test_playground_guard_trailing_slash () =
  let pg = "/project/.masc/playground/cheolsu/" in
  Alcotest.(check bool)
    "trailing slash exact match"
    true
    (is_inside_playground ~playground_abs:pg "/project/.masc/playground/cheolsu");
  Alcotest.(check bool)
    "trailing slash nested match"
    true
    (is_inside_playground
       ~playground_abs:pg
...
let test_playground_guard_symlink_escape () =
  ...
  Alcotest.(check bool)
    "symlinked cwd resolving outside is rejected"
    false
    (is_inside_playground ~playground_abs:playground symlinked_cwd)
;;

let test_cleanup_dir_does_not_follow_symlinks () =
  let root = temp_dir () in
  Fun.protect ~finally:(fun () -> cleanup_dir root)
  @@ fun () ->
...
let parse_error_field raw =
  Yojson.Safe.from_string raw |> Json.member "error" |> Json.to_string_option
;;
...
let test_docker_blocks_nested_docker_command () =
  ...
  | Some err ->
    Alcotest.(check bool)
      "mentions nested container block"
      true
...
let test_docker_missing_seccomp_profile_fails_closed () =
  ...
  with_boot_override
    ...
    (fun () ->
       ...
       | Some err ->
         Alcotest.(check bool)
           "mentions missing seccomp profile"
           true
           (String_util.contains_substring err "sandbox seccomp profile not found")
       | None -> Alcotest.fail ("expected error json, got: " ^ raw))
;;
...
let test_playground_guard_traversal () =
  ...
  Alcotest.(check bool)
    "traversal resolves outside (canonicalized)"
    false
    (is_inside_playground ~playground_abs:pg "/project/lib");
  (* After realpath, ".../cheolsu/repos/../../../.masc" becomes "/project/.masc" *)
  Alcotest.(check bool)
    "traversal to .masc (canonicalized)"
    false
    (is_inside_playground ~playground_abs:pg "/project/.masc");
  ...
  Alcotest.(check bool)
    "raw traversal WOULD match prefix (proves canonicalization needed)"
    true
    would_match_raw
;;
...
let parse_hint raw =
  Yojson.Safe.from_string raw |> Json.member "hint" |> Json.to_string_option
;;

let parse_category raw =
  Yojson.Safe.from_string raw |> Json.member "category" |> Json.to_string_option
;;
...
let test_keeper_shell_ls_recovers_doubled_playground_prefix () =
  ...
  let raw =
    Keeper_exec_shell.handle_keeper_shell
      ...
      ~config
      ~meta
      ~args:(`Assoc [ "op", `String "ls"; "path", `String doubled_path ])
  in
  let json = Yojson.Safe.from_string raw in
  Alcotest.(check bool) "ls succeeds" true (json |> Json.member "ok" |> Json.to_bool);
  Alcotest.(check string)
    "path normalized to repos root"
    repos
...
let test_readonly_chaining_hint_lists_subops () =
  ...
  | Some hint ->
    Alcotest.(check bool)
      "hint names the separate-call alternative"
      true
    ...
    List.iter
      (fun sub_op ->
         Alcotest.(check bool)
           (Printf.sprintf "hint lists %s sub-op" sub_op)
           true
...
let test_readonly_redirect_hint_points_at_fs_edit () =
  ...
  let raw =
    Keeper_exec_shell.handle_keeper_shell
      ...
      ~config
      ~meta
      ~args:(`Assoc [ "op", `String "bash"; "command", `String "echo hi > out.txt" ])
  in
  Alcotest.(check (option string))
    "category is redirect"
    (Some "redirect")
    (parse_category raw);
  match parse_hint raw with
  | None -> Alcotest.fail ("expected hint field, got: " ^ raw)
...
let test_git_write_classification () =
  ...
  Alcotest.(check bool)
    "checkout is branch switch"
    true
    (is_branch_switch "git checkout -b my-feature");
  Alcotest.(check bool)
    "switch is branch switch"
    true
    (is_branch_switch "git switch -c my-feature");
  ...
  Alcotest.(check bool)
    ...
    true
    (is_destructive "rm -rf /tmp/something");
  (* git push is NOT destructive (it's write, not destructive) *)
  Alcotest.(check bool)
    "push is not destructive"
    false
    (is_destructive "git push origin my-branch")
;;
...
let test_rewrite_turn_runtime_paths_to_host () =
  ...
  in
  Alcotest.(check string)
    "container paths rewritten to host root"
    (Printf.sprintf
       "worktree %s/repos/masc-mcp\npwd=%s/repos/masc-mcp\n"
       host_root
       host_root)
...
let test_bash_missing_cmd_field () =
  ...
  | Some err ->
    Alcotest.(check bool)
      "error mentions cmd is required"
      true
      (String_util.contains_substring err "cmd is required")
  | None -> Alcotest.fail ("expected error json for missing cmd field, got: " ^ raw)
;;
...
let test_shell_missing_op_field () =
  ...
  | Some err ->
    Alcotest.(check bool)
      "error mentions unsupported op"
      true
      (String_util.contains_substring err "unsupported_op")
  | None -> Alcotest.fail ("expected error json for missing op field, got: " ^ raw)
;;
...
let test_shell_unsupported_op () =
  ...
  let raw =
    Keeper_exec_shell.handle_keeper_shell
      ...
      ~meta
      ~args:
        (`Assoc [ "op", `String "definitely_not_a_real_op"; "path", `String "/some/path" ])
  in
  match parse_error_field raw with
  | Some err ->
    Alcotest.(check bool)
      ...
      true
      (String_util.contains_substring err "unsupported_op")
  | None -> Alcotest.fail ("expected error json for unsupported op, got: " ^ raw)
;;
  ...
  Alcotest.run
    ...
    ; ( "playground_guard"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "git write classification"
            `Quick
            test_git_write_classification
        ] )
    ; ( "edge"
      , [ Alcotest.test_case "empty command blocked" `Quick test_empty_command
        ; Alcotest.test_case
            "docker blocks nested docker command"
            `Quick
            test_docker_blocks_nested_docker_command
        ; Alcotest.test_case
            "docker blocks docker socket reference"
            `Quick
            test_docker_blocks_docker_socket_reference
        ; Alcotest.test_case
            "docker missing seccomp fails closed"
            `Quick
    ...
    ; ( "readonly_hints"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "chaining hint lists sub-ops (task-238)"
            `Quick
            test_readonly_chaining_hint_lists_subops
        ; Alcotest.test_case
            "redirect hint points at keeper_fs_edit"
            `Quick
    ...
    ; ( "git_worktree_op"
      , [ Alcotest.test_case
            ...
            `Quick
            test_git_worktree_action_normalization
        ; Alcotest.test_case
            "branch required for add"
            `Quick
            test_git_worktree_branch_required
        ] )
    ; ( "turn_runtime_paths"
      , [ Alcotest.test_case
