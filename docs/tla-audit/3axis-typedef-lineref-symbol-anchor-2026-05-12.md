# 3-axis type-def line-ref sweep — symbol-anchor `keeper_registry.ml:63-81` across KeeperRuntimeLifecycle / KeeperDecisionPipeline / KeeperTurnCycle (iter 82)

**Date**: 2026-05-12 · **Iteration**: 82 (`/loop` FSM/TLA+/OCaml drift hunt) · **Phase**: R (line-ref sweep, cluster #1)
**Specs**: `KeeperRuntimeLifecycle.tla` (303 LOC) · `KeeperDecisionPipeline.tla` (267 LOC) · `KeeperTurnCycle.tla` (418 LOC)
**OCaml**: `lib/keeper/keeper_registry.ml` — `type turn_phase` / `type decision_stage` / `type runtime_state` (the 3-axis turn FSM) + `module Turn_phase_transition` (the GADT)
...
| `keeper_registry.ml:76-81` — `type runtime_state = Runtime_idle \| ...` | `type runtime_state` at line **632** | **+556** | `keeper_registry.ml — type runtime_state` |
...
| `runtime_state` axis (`Runtime_idle` / ...) | `type runtime_state` (now line 632) | ✓ |
