...
let parse_json_safe ~context str : (Yojson.Safe.t, string) result =
  ...
  | Yojson.Json_error msg ->
    let preview =
      String_util.utf8_safe ~max_bytes:53 ~suffix:"..." str |> String_util.to_string
    in
...
let report_persistence_read_drop ~on_drop ~surface ~reason ~path ~detail =
  Log.Misc.warn "[%s] persistence read drop (%s) path=%s: %s" surface reason path detail;
  on_drop ()
;;
...
let list_dir_safe path : (string list, string) result =
  if not (Sys.file_exists path)
  then Error (Printf.sprintf "Directory not found: %s" path)
  else if not (Sys.is_directory path)
  then Error (Printf.sprintf "Not a directory: %s" path)
  else (
    try Ok (Sys.readdir path |> Array.to_list) with
    | Eio.Cancel.Cancelled _ as e -> raise e
    | e -> Error (Printf.sprintf "Failed to list %s: %s" path (Printexc.to_string e)))
;;
...
let remove_file_logged ?(context = "cleanup") path =
  ...
  | Eio.Cancel.Cancelled _ as e -> raise e
  | e -> Log.Misc.error "[%s] Failed to remove %s: %s" context path (Printexc.to_string e)
;;
...
let close_in_logged ic =
  ...
  | Eio.Cancel.Cancelled _ as e -> raise e
  | e -> Log.Misc.error "Failed to close input channel: %s" (Printexc.to_string e)
;;
...
let parse_float_string s =
  let trimmed = String.trim s in
  if trimmed = "" then None else float_of_string_opt trimmed
;;
...
let map_safe (f : 'a -> 'b) (l : 'a list) : 'b list = List.rev (List.rev_map f l)
