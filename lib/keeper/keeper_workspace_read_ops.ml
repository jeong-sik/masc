...
let try_handle
  ...
  let hostify_turn_runtime_output out =
    Keeper_tool_execute_runtime_paths.rewrite_turn_runtime_paths_to_host ~config ~meta out
  in
      ...
      (match read_target () with
       ...
       | Ok target ->
         ...
         if Keeper_sandbox_read_runner.should_route_read ~meta then
           ...
           let ir = Keeper_tool_execute_shell_ir.simple Masc_exec.Exec_program.Ls [ "-la"; target ] in
           run_host_shell_ir
             ~workdir:target
             ~cmd:"ls -la"
             ~path:target
      ...
      (match read_target () with
       ...
       | Ok target ->
         ...
         if Keeper_sandbox_read_runner.should_route_read ~meta then
           ...
           let ir = Keeper_tool_execute_shell_ir.simple Masc_exec.Exec_program.Cat [ target ] in
           run_host_shell_ir
             ~workdir:target
             ~cmd:"cat"
             ~path:target
      ...
      (match read_target () with
       ...
       | Ok target ->
         if Keeper_sandbox_read_runner.should_route_read ~meta then
           ...
           let ir = Keeper_tool_execute_shell_ir.simple Masc_exec.Exec_program.Wc [ "-l"; target ] in
           run_host_shell_ir
             ~workdir:target
             ~cmd:"wc"
             ~path:target
