...
| Diagnostics should remain routeable without dominating daily Monitor IA. | `runtime-config`, `doctor`, `transport-health`, `feature-health`, `journey`, and `cognition` remain configured with `hidden: true`; `runtime-panel` links diagnostics from Runtime & Runtime. | Done |
