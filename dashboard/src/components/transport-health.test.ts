...
describe('filterHotSessions', () => {
  const sessions: HotSession[] = [
    ...
    { session_id: 'bbbb9999-8888-7777-6666-555544443333', kind: 'workspace client', queue_depth: 12, last_event_id: 202, idle_seconds: 9 },
  ...
  it('matches substring of kind', () => {
    ...
    expect(result).toHaveLength(1)
    expect(result[0]!.kind).toBe('workspace client')
  })
  ...
  it('trims the query before matching', () => {
    const result = filterHotSessions(sessions, '  workspace client  ')
    expect(result).toHaveLength(1)
    expect(result[0]!.kind).toBe('workspace client')
  })
