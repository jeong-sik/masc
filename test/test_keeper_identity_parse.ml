...
let test_explicit_keeper_name_is_not_nickname_canonicalized () =
  ...
  | Ok meta -> check string "explicit keeper name" "personality-resync-test" meta.name
  | Error e -> fail ("expected Ok, got Error: " ^ e)
;;
...
let test_legacy_keeper_cascade_alias_preserved_raw () =
  ...
  match Keeper_types.meta_of_json json with
  | Ok meta ->
    check string "legacy alias preserved raw" "oas-keeper_unified" meta.cascade_name;
    check
      string
      "legacy alias canonicalizes to default"
...
let test_unknown_cascade_name_preserved_raw () =
  ...
  | Ok meta ->
    check
      string
      "raw user-declared cascade preserved"
      "playground_experiment_xyz"
      meta.cascade_name;
    (* Point-of-use canonicalize still maps unknown → default. *)
    check
      string
      "unknown canonicalizes to default"
      Keeper_config.default_cascade_name
      (Keeper_cascade_profile.canonicalize meta.cascade_name)
  | Error e -> fail ("expected Ok, got Error: " ^ e)
;;
  ...
  | Error msg ->
    check
      ...
      (String.length msg > 0
       &&
       try
         ignore (Str.search_forward (Str.regexp_string "trace_id") msg 0);
         true
       with
       | Not_found -> false)
...
let test_empty_trace_id () =
  ...
  | Error msg ->
    check
      bool
      "error mentions missing trace_id"
      true
      (String.length msg > 0
       &&
       try
         ignore (Str.search_forward (Str.regexp_string "missing trace_id") msg 0);
         true
       with
       | Not_found -> false)
...
let test_invalid_trace_id () =
  ...
  | Error msg ->
    check
      bool
      "error mentions invalid trace_id"
      true
      (String.length msg > 0
       &&
       try
         ignore (Str.search_forward (Str.regexp_string "invalid trace_id") msg 0);
         true
       with
       | Not_found -> false)
