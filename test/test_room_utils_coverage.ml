...
let with_envs bindings f =
  List.fold_right
    (fun (name, value) acc -> fun () -> with_env name value acc)
    bindings
    f
    ()
;;
...
let test_resolve_masc_base_path_ignores_inherited_env_in_test_by_default () =
  ...
  with_envs
    ...
    (fun () ->
       check
         string
         "requested git root wins in tests"
         repo_root
...
let test_resolve_masc_base_path_prefers_requested_path_in_test () =
  ...
  with_envs
    ...
    (fun () ->
       check
         string
         "requested path wins in tests"
         requested
         (Coord_utils.resolve_masc_base_path requested))
;;

let test_resolve_masc_base_path_keeps_matching_explicit_env () =
  let requested = Filename.concat (Filename.get_temp_dir_name ()) "room-utils-matching" in
  with_envs
    [ "MASC_BASE_PATH", Some requested; "MASC_TEST_ALLOW_BASE_PATH_OVERRIDE", None ]
    (fun () ->
       check
         string
         "matching explicit env preserved"
         requested
         (Coord_utils.resolve_masc_base_path requested))
;;
...
let test_resolve_masc_base_path_collapses_requested_masc_dir () =
  ...
  in
  with_envs
    [ "MASC_BASE_PATH", None; "MASC_TEST_ALLOW_BASE_PATH_OVERRIDE", None ]
    (fun () ->
       check
         string
         "requested .masc input collapses to parent"
         (Filename.dirname requested)
         (Coord_utils.resolve_masc_base_path requested))
;;

let test_resolve_masc_base_path_collapses_explicit_env_masc_dir () =
  let requested = Filename.concat (Filename.get_temp_dir_name ()) "room-utils-explicit" in
  let explicit = Filename.concat requested Common.masc_dirname in
  with_envs
    [ "MASC_BASE_PATH", Some explicit; "MASC_TEST_ALLOW_BASE_PATH_OVERRIDE", None ]
    (fun () ->
       check
         string
         "explicit .masc env collapses to parent"
         requested
         (Coord_utils.resolve_masc_base_path requested))
;;

let test_resolve_masc_base_path_ignores_base_path_override_without_opt_in () =
  let requested = Filename.concat (Filename.get_temp_dir_name ()) "room-utils-opt-in" in
  ...
  with_envs
    ...
    (fun () ->
       check
         string
         "base path override ignored without opt-in"
         requested
         (Coord_utils.resolve_masc_base_path requested))
;;
    ...
    (fun () ->
       with_envs
         ...
         (fun () ->
            check
              string
              "local .masc still ignores base path override in tests"
              requested
              (Coord_utils.resolve_masc_base_path requested)))
;;
    ...
    (fun () ->
       with_envs
         ...
         (fun () ->
            check
              string
              "opt-in preserves base path override"
              explicit
              (Coord_utils.resolve_masc_base_path requested)))
;;
    ...
    (fun () ->
       with_envs
         ...
         (fun () ->
            check
              string
              "requested sub-repo wins without base path override opt-in"
              sub_repo
              (Coord_utils.resolve_masc_base_path sub_repo)))
;;

let test_default_config_syncs_test_base_path_env () =
  let requested = Filename.concat (Filename.get_temp_dir_name ()) "room-utils-sync-env" in
  with_envs
    [ "MASC_BASE_PATH", None
    ...
    (fun () ->
       ...
       check
         (option string)
         "env synced to requested path"
         (Some requested)
         (Sys.getenv_opt "MASC_BASE_PATH"))
;;

let test_auto_synced_test_base_path_does_not_override_later_requests () =
  let first = Filename.concat (Filename.get_temp_dir_name ()) "room-utils-sync-first" in
  let second = Filename.concat (Filename.get_temp_dir_name ()) "room-utils-sync-second" in
  with_envs
    [ "MASC_BASE_PATH", None
    ; "MASC_TEST_SYNCED_BASE_PATH", None
    ; "MASC_TEST_ALLOW_BASE_PATH_OVERRIDE", None
    ...
    (fun () ->
       ...
       check
         string
         "later requested path wins over auto-synced env"
         second
...
let test_storage_type_default () =
  with_envs (pg_env_bindings ()) (fun () ->
    ...
    check string "defaults to filesystem when no pg env exists" "filesystem" storage_type)
;;
...
let test_sanitize_html_empty () = check string "empty" "" (Coord_utils.sanitize_html "")
...
let test_sanitize_message_html () =
  check
    ...
    "html stripped"
    "&lt;b&gt;bold&lt;/b&gt;"
    (Coord_utils.sanitize_message "<b>bold</b>")
;;
...
let test_storage_type_legacy_url_does_not_auto_select () =
  ...
  with_envs (pg_env_bindings ~supabase_db_url:url ()) (fun () ->
    check
      string
      "legacy url does not trigger postgres"
      "filesystem"
      (Coord_utils.storage_type_from_env ()))
;;

let test_storage_type_auto_is_deprecated () =
  with_envs (pg_env_bindings ~masc_storage_type:"auto" ()) (fun () ->
    check
      string
      "auto falls back to filesystem"
      "filesystem"
      (Coord_utils.storage_type_from_env ()))
;;
...
let test_storage_type_unknown_normalised_to_filesystem () =
  with_envs (pg_env_bindings ~masc_storage_type:"postgres" ()) (fun () ->
    check
      string
      "unknown postgres normalised to filesystem"
      "filesystem"
      (Coord_utils.storage_type_from_env ()))
;;

let test_storage_type_typo_normalised_to_filesystem () =
  with_envs (pg_env_bindings ~masc_storage_type:"memoryy" ()) (fun () ->
    check
      string
      "typo memoryy normalised to filesystem"
      "filesystem"
      (Coord_utils.storage_type_from_env ()))
;;
...
let test_safe_filename_empty () = check string "empty" "" (Coord_utils.safe_filename "")
...
let test_list_dir_prefers_backend_for_memory_keys () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         (list string)
         "memory backend ignores local-only stale files"
         [ "backend.json" ]
         listed)
;;
  ...
  run
    ...
    [ ( "parse_gitdir_to_main_root"
      , [ test_case "worktree" `Quick test_parse_gitdir_worktree
        ...
        ; test_case
            "ignores ancestor base path override without opt-in"
            `Quick
            test_resolve_masc_base_path_ignores_ancestor_base_path_override_without_opt_in
        ; test_case
            "default config syncs test base env"
            `Quick
            test_default_config_syncs_test_base_path_env
        ; test_case
            "auto-synced test base env does not override later requests"
            `Quick
    ...
    ; "storage_type_from_env", [ test_case "default" `Quick test_storage_type_default ]
    ; ( "storage_backend"
    ...
    ; ( "storage_backend_selection"
      , [ test_case
            "defaults to filesystem"
            `Quick
            test_storage_type_defaults_to_filesystem
        ; test_case
            "legacy url does not auto select"
            `Quick
            test_storage_type_legacy_url_does_not_auto_select
        ; test_case "auto is deprecated" `Quick test_storage_type_auto_is_deprecated
        ; test_case
            "unknown normalised to filesystem (#8737)"
            `Quick
            test_storage_type_unknown_normalised_to_filesystem
        ; test_case
            "typo normalised to filesystem (#8737)"
            `Quick
            test_storage_type_typo_normalised_to_filesystem
        ] )
    ; ( "safe_filename"
    ...
    ; ( "path_helpers"
      , [ test_case
            "masc_root_dir default cluster"
            `Quick
            test_masc_root_dir_default_cluster
        ; test_case "masc_root_dir empty cluster" `Quick test_masc_root_dir_empty_cluster
        ; test_case
            "masc_root_dir custom cluster"
            `Quick
            test_masc_root_dir_custom_cluster
        ; test_case
            "masc_root_dir nested with cluster"
            `Quick
            test_masc_root_dir_with_cluster_nested
        ; test_case
            "list_dir prefers backend for memory keys"
            `Quick
            test_list_dir_prefers_backend_for_memory_keys
        ; test_case
            "read_current_room always default"
            `Quick
