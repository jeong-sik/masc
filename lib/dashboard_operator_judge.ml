...
let prompt_for_facts facts_json =
  Printf.sprintf
    "You are the resident operator judge for a MASC control room.\n\
     ...
     Allowed action_type values: broadcast, room_pause, room_resume, lodge_tick, team_note, team_broadcast, team_task_inject, team_worker_spawn_batch, team_stop, keeper_message, keeper_probe, keeper_recover, swarm_run_continue, swarm_run_rerun, swarm_run_abandon.\n\
...
let compute_judgments ~facts_json =
  ...
  if specs = [] then Error "No operator_judge model is available."
    ...
    let timeout_sec =
      match Sys.getenv_opt "MASC_OPERATOR_JUDGE_TIMEOUT_SEC" with
      | Some raw -> (
