      ...
      (match Voice_config.parse_runtime_toml_text (read path) with
       ...
       | Ok (Some config) ->
         (match config.Voice_config.stt with
          ...
          | Some stt ->
            (* The existing endpoint's fallback is untouched: the new model went
               on the new endpoint, not over the section's default. *)
            Alcotest.(check (option string)) "the section's model is the one it had"
            ...
            (match landed with
             ...
             | Some endpoint ->
               ...
               Alcotest.(check (option string)) "with the model the wizard was given"
