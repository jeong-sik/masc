...
let candidate ~identity ~message (checkpoint : Agent_core.Checkpoint.t) =
...
let admit ~session_dir ~identity ~message (fallback : Agent_core.Checkpoint.t) =
  ...
  | Ok (current, reference) ->
    ...
    (match candidate ~identity ~message current with
  ...
  | Error Keeper_checkpoint_store.Ref_not_found ->
    (match candidate ~identity ~message fallback with
     | Error _ as error -> error
     | Ok checkpoint ->
