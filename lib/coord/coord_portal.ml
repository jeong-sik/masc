...
let gen_a2a_task_id () =
  ...
  Printf.sprintf
    "a2a-%04d%02d%02d%02d%02d%02d-%04x"
...
let with_two_file_locks config path_a path_b f =
  let p1, p2 =
    if String.compare path_a path_b <= 0 then path_a, path_b else path_b, path_a
  in
  with_file_lock config p1 (fun () -> with_file_lock config p2 f)
;;
...
let portal_open_r config ~agent_name ~target_agent ~initial_message : string masc_result =
  ...
  then Error NotInitialized
    ...
    match validate_agent_name_r agent_name, validate_agent_name_r target_agent with
    | Error e, _ -> Error e
    | _, Error e -> Error e
    | Ok _, Ok _ ->
      ...
      let portal_path =
        Filename.concat (portals_dir config) (safe_filename agent_name ^ ".json")
      in
      ...
      with_two_file_locks config portal_path target_portal_path (fun () ->
        ...
        | Some json ->
          (match portal_of_yojson json with
           | Ok p ->
             Error (PortalAlreadyOpen { agent = agent_name; target = p.portal_target })
        ...
        | None ->
          ...
          if not (Sys.file_exists target_portal_path)
          then (
            let reverse_portal =
          ...
          (match initial_message with
           | Some msg ->
             ...
             Ok
               (Printf.sprintf
                  "🌀 Portal opened: %s ↔ %s (initial task: %s)"
                  agent_name
                  target_agent
                  task_id)
           | None ->
...
let get_portal_target config ~agent_name =
  let portal_path =
    Filename.concat (portals_dir config) (safe_filename agent_name ^ ".json")
  in
  if Sys.file_exists portal_path
  then (
    match read_json_opt config portal_path with
    | Some json ->
      (match portal_of_yojson json with
...
let portal_close config ~agent_name =
  ...
  let rec close_locked target_portal_path =
        ...
        | Some json ->
          (match portal_of_yojson json with
           ...
           | Ok portal ->
             ...
             in
             if not (String.equal current_target_path target_portal_path)
             then `Retry current_target_path
             else (
               if Sys.file_exists portal_path then Sys.remove portal_path;
    ...
    | `Closed (target_agent, task_count) ->
      Printf.sprintf
        "🌀 Portal closed: %s ↔ %s (%d tasks sent)"
        agent_name
        target_agent
        task_count
...
let portal_status config ~agent_name =
  ...
  let portal_path =
    Filename.concat (portals_dir config) (safe_filename agent_name ^ ".json")
  in
  if not (Sys.file_exists portal_path)
  then
    `Assoc
    ...
    | Ok portal ->
      ...
      let pending_tasks =
        if Sys.file_exists (a2a_tasks_dir config)
        then
          Array.fold_left
            (fun acc f ->
               ...
               then (
                 ...
                 | Ok t when t.to_agent = agent_name && t.a2a_status = A2APending ->
                   acc + 1
                 | _ -> acc)
               else acc)
            0
            (Sys.readdir (a2a_tasks_dir config))
        else 0
      in
      `Assoc
        [ "status", `String "open"
