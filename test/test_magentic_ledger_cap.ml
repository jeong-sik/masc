...
let test_overlay_capped () =
  ...
  Alcotest.(check bool)
    "belief bounded"
    true
    (String.length capped.belief_summary <= belief_cap + 3);
  let check_opt label = function
    | Some v ->
      Alcotest.(check bool) (label ^ " bounded") true (String.length v <= opt_cap + 3)
...
let test_model_name_preserved () =
  ...
  Alcotest.(check string)
    "magentic social_model preserved"
    "magentic_ledger_v1"
    capped.social_model
;;
...
let test_speech_and_surface_preserved () =
  ...
  Alcotest.(check string)
    ...
    "request_help"
    (T.speech_act_to_string capped.speech_act)
;;
...
let test_short_magentic_state_unchanged () =
  ...
  Alcotest.(check string) "short belief unchanged" "quiet_room" capped.belief_summary;
...
let () =
  Alcotest.run
    ...
    [ ( "cap invariant on magentic emission"
      , [ Alcotest.test_case "all fields capped when oversized" `Quick test_overlay_capped
        ; Alcotest.test_case "model_name preserved" `Quick test_model_name_preserved
        ; Alcotest.test_case
            "speech/surface preserved"
            `Quick
            test_speech_and_surface_preserved
        ; Alcotest.test_case
            "short magentic state unchanged"
            `Quick
