...
let handle_ag_ui_events ~deps request reqd =
  ...
  | Ok () ->
    ...
    let headers =
      Httpun.Headers.of_list (sse_stream_headers ~deps session_id protocol_version origin)
    in
    ...
    let client_id, event_stream, evicted =
      Sse.register session_id ~last_event_id:(Option.value ~default:0 last_event_id)
    in
    (match evicted with
     | Some evicted_sid -> stop_sse_session evicted_sid
     | None -> ());
    let info =
      { session_id; client_id; writer; mutex; stop = ref false; closed = false }
    in
    info_ref := Some info;
    ...
    let prime =
      Ag_ui.(
        ...
        |> event_to_sse)
    in
    if not (send_raw info prime)
    then Log.Server.debug "ag-ui prime send failed for session %s" info.session_id;
    (match last_event_id with
     | Some last_id ->
       let missed = Sse.get_events_after last_id in
       List.iter
         (fun ev ->
            if not (send_raw info ev)
            then
       ...
       Eio.Fiber.fork ~sw (fun () ->
         let rec drain () =
           ...
           (try
            ...
            | Eio.Cancel.Cancelled _ as e -> raise e
            | exn ->
              Log.Server.error "ag-ui drain write error: %s" (Printexc.to_string exn);
              stop_sse_session_preserve_guard info.session_id);
           if not !(info.stop) then drain ()
         in
         try drain () with
         | Eio.Cancel.Cancelled _ as e -> raise e
         | exn -> Log.Server.error "ag-ui drain loop error: %s" (Printexc.to_string exn));
       (* Ping fiber *)
       Eio.Fiber.fork ~sw (fun () ->
         let rec loop () =
           if not !(info.stop)
           then (
             (try Eio.Time.sleep clock sse_ping_interval_s with
              ...
              | exn ->
                Log.Server.debug "SSE ping sleep interrupted: %s" (Printexc.to_string exn));
             (try
                if info.closed
                then stop_sse_session_preserve_guard info.session_id
                else if not !(info.stop)
                then
              ...
              | exn ->
                Log.Server.warn
                  "SSE ping send failed for session %s: %s"
                  info.session_id
         ...
         | exn ->
           Log.Server.error
             "SSE ping loop exited for session %s: %s"
             info.session_id
             (Printexc.to_string exn);
           stop_sse_session_preserve_guard info.session_id)
     | Error msg ->
       Log.Server.debug "ag-ui SSE runtime unavailable for session %s: %s" session_id msg)
;;
