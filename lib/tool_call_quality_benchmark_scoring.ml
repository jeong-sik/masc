...
let evaluate_json_check ~(target : Yojson.Safe.t option) (check : json_check) =
  ...
  let present_ok =
    ...
    | Some false ->
      (match actual with
       | Some value when value <> `Null -> false
       | _ -> true)
  in
  let equals_ok =
...
let required_tool_score (benchmark_case : benchmark_case) (run : evidence_run) =
  ...
  | Tool_forbidden -> if run.tool_calls = [] then 1.0 else 0.0
  | _ ->
    (match benchmark_case.required_tools with
     | [] -> 1.0
     | required_tools ->
       required_tools
       |> List.map (fun tool_name -> if tool_used tool_name run then 1.0 else 0.0)
       |> avg_float)
;;
  ...
  | Some policy ->
    ...
    let rec find_success_after failures_seen failures_before_success = function
      ...
      | call :: rest ->
        ...
        then
          if failures_seen
          then Some failures_before_success
          else find_success_after failures_seen failures_before_success rest
        else find_success_after true (failures_before_success + 1) rest
    in
    ...
    let failure_limit_ok =
      ...
      | None, _ -> false
    in
    if task_pass = 1.0 && failure_limit_ok && success_after_failure <> None
    then 1.0
    else 0.0
;;
    ...
    | _ ->
      let forbidden_count =
        ...
        |> List.filter (fun call ->
          List.mem call.tool_name benchmark_case.forbidden_tools)
        |> List.length
      in
      let over_limit = max 0 (call_count - benchmark_case.max_tool_calls) in
      min 1.0 (float_of_int (forbidden_count + over_limit) /. float_of_int call_count))
;;

let efficiency_score (benchmark_case : benchmark_case) (run : evidence_run) =
  let call_count = List.length run.tool_calls in
  match benchmark_case.category with
  | Tool_forbidden -> if call_count = 0 then 1.0 else 0.0
  | _ ->
    if benchmark_case.max_tool_calls <= 0
    then if call_count = 0 then 1.0 else 0.0
    else (
      let over_limit = max 0 (call_count - benchmark_case.max_tool_calls) in
      max
        0.0
...
let score_run ~cases (run : evidence_run) =
  ...
  then None
    ...
    let cases_by_id =
      cases |> List.to_seq |> Seq.map (fun case -> case.id, case) |> Hashtbl.of_seq
    in
    match Hashtbl.find_opt cases_by_id run.case_id with
    | None -> None
    | Some benchmark_case ->
      if not (List.mem run.keeper_profile benchmark_case.keeper_profiles)
      then None
      else (
