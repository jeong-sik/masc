...
let handle_goal_list (ctx : context) args =
  ...
  with
  | Error err, _, _ | _, Error err, _ | _, _, Error err -> validation_error_result [ err ]
...
let handle_goal_upsert (ctx : context) args =
  ...
  | _, _, _, Error err, _, _
  | _, _, _, _, Error err, _
  | _, _, _, _, _, Error err -> validation_error_result [ err ]
  | ( Ok horizon
    , Ok status
    , Ok phase
    , Ok priority
  ...
  | Ok goal_id, Ok (Some principal), Ok (Some decision), Ok evidence_refs ->
    ...
    (match Goal_store.get_goal ctx.config ~goal_id with
     ...
     | Some goal ->
       ...
       (match request_id with
        | None ->
          error_result_typed ~code:Conflict "goal has no active verification request"
        | Some request_id ->
          (match
          ...
          (match outcome with
           ...
           | Goal_store.ReviewDropped ->
             (match
              ...
              with
              | Error msg -> error_result_typed ~code:Not_found msg
              | Ok goal ->
                ok_result
