...
```ocaml
(* lib/runtime/runtime_event_bridge.ml:664-668 *)
...
- **SDK error → masc_internal_error 분류**: RFC-0142 의 영역. runtime_error_classify.ml 의 33 catch-all 은 본 RFC 가 다루지 않음.
...
| **RFC-0142** | runtime_error_classify decomp — 다른 모듈 (SDK error vs OS error), §6 비-목표 |
