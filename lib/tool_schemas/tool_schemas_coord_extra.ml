  ...
  `Assoc
    ...
    ; ( "properties"
      , `Assoc
          ...
          ; ( "principals"
            , `Assoc [ "type", `String "array"; "items", goal_principal_schema ] )
...
let schemas : tool_schema list =
  [ { name = "masc_goal_list"
    ; description =
        "List shared planning goals from the Goal Store, optionally filtered by horizon, \
         explicit phase, or legacy status. Use when a PM/planner agent needs current \
         long/mid/short goals before creating tasks or reviews. The dashboard Goal Tree \
         reads the same store. Linked tasks prefer structured task.goal_id; title tags \
         like [goal:<id>] remain a legacy fallback. The response includes each goal's \
         explicit lifecycle phase and verification policy."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
            , `Assoc
                [ ( "horizon"
                  , enum_schema ~description:"Optional horizon filter" goal_horizon_enum )
                ; ( "phase"
                  , enum_schema
                      ~description:"Optional explicit Goal FSM phase filter"
                      goal_phase_enum )
                ; ( "status"
                  , enum_schema
                      ~description:"Optional legacy status filter"
                      goal_status_enum )
                ] )
          ]
    }
  ; { name = "masc_goal_upsert"
    ; description =
        "Create or update a shared Goal Store entry used by Planning > Goal Tree. For \
         new goals, provide at least title; omitted horizon defaults to short. After \
         creation, link tasks into the tree with task.goal_id=<goal_id>; legacy \
         [goal:<id>] title markers remain supported for compatibility. Use this tool for \
         goal metadata, parent linkage, and verifier-policy configuration. Lifecycle \
         status/phase fields are intentionally omitted here; use masc_goal_transition / \
         masc_goal_verify for lifecycle moves."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
  ...
  ; { name = "masc_goal_review"
    ; description =
        "Compatibility wrapper for legacy goal review flows. Use masc_goal_transition / \
         masc_goal_verify for the full Goal FSM and quorum verification."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
  ...
  ; { name = "masc_goal_transition"
    ; description =
        "Apply an explicit Goal FSM transition such as request_complete, pause, unblock, \
         or approval resolution. The actor field records who initiated the transition."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
            , `Assoc
                ...
                ; ( "override_note"
                  , `Assoc
                      ...
                      ; ( "description"
                        , `String
                            "Operator rationale for forcing request_complete when linked \
  ...
  ; { name = "masc_goal_verify"
    ; description =
        "Submit one verifier vote on an open goal verification request. Supports mixed \
         operator and keeper principals, one vote per principal, and N-of-M quorum \
         resolution."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ...
          ; ( "required"
            , `List [ `String "goal_id"; `String "principal"; `String "decision" ] )
          ]
    }
  ]
;;
