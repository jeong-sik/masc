...
let with_config_dir f =
  with_temp_dir "keeper-runtime-denylist-config"
  @@ fun config_dir ->
...
let test_ensure_keeper_meta_resyncs_denylist_from_toml () =
  with_temp_dir "keeper-runtime-denylist-room"
  @@ fun room_dir ->
  with_config_dir
  @@ fun config_dir ->
  Fs_compat.clear_fs ();
    ...
    match
      Keeper_types.meta_of_json
        (`Assoc
            ...
            ; "tool_denylist", `List [ `String "old-stale-tool" ]
            ])
    with
    | Ok meta -> meta
...
let () =
  run
    "Keeper_runtime denylist resync"
    [ ( "ensure_keeper_meta"
      , [ test_case
