...
let set_tempo (config : Coord_utils.config) ~interval_s ~reason : tempo_state =
  let clamped =
    max default_config.min_interval_s (min default_config.max_interval_s interval_s)
  in
  let state =
    { current_interval_s = clamped; last_adjusted = Time_compat.now (); reason }
  in
...
(** Get current tempo *)
let get_tempo (config : Coord_utils.config) : tempo_state = load_state config
...
let calculate_adaptive_tempo (tasks : Types.task list) : float * string =
  if tasks = []
  then default_config.max_interval_s, "idle - no pending tasks"
  else (
    let urgent_count =
      List.filter (fun t -> t.Types.priority <= 2) tasks |> List.length
    in
...
let format_state (state : tempo_state) : string =
  let interval_str =
    if state.current_interval_s < 120.0
    then Printf.sprintf "%.0fs" state.current_interval_s
    else Printf.sprintf "%.1fm" (state.current_interval_s /. 60.0)
  in
  let age =
