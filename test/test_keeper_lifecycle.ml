...
let tool_use_message ?(name = "list_files") ~tool_use_id () =
  ...
  ; content = [ Agent_sdk.Types.ToolUse { id = tool_use_id; name; input = `Assoc [] } ]
  ; name = None
  ; tool_call_id = None
  ; metadata = []
  }
...
let tool_result_message ?(is_error = false) ~tool_use_id content =
  ...
  ; content =
      [ Agent_sdk.Types.ToolResult { tool_use_id; content; is_error; json = None } ]
  ; name = None
  ; tool_call_id = None
  ; metadata = []
  }
...
let has_tool_use_id ~tool_use_id msgs =
  List.exists
    (fun (msg : Agent_sdk.Types.message) ->
       List.exists
         (function
           | Agent_sdk.Types.ToolUse { id; _ } -> String.equal id tool_use_id
           | _ -> false)
         msg.content)
    msgs
;;
...
let append_turn ctx idx =
  ...
  KEC.append
    ctx
    (Agent_sdk.Types.user_msg (Printf.sprintf "turn %02d user %s" idx payload))
  |> fun next ->
  KEC.append
    next
    (Agent_sdk.Types.assistant_msg (Printf.sprintf "turn %02d assistant %s" idx payload))
;;
...
let build_dense_context ~turns ~max_tokens ~state_reply =
  ...
  KEC.append ctx (Agent_sdk.Types.assistant_msg state_reply) |> KEC.sync_oas_context
;;
...
let test_apply_post_turn_lifecycle_without_checkpoint_records_skip () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check string "skip decision" "skipped:no_checkpoint" lifecycle.compaction.decision;
       check
         string
         "runtime decision persisted"
         "skipped:no_checkpoint"
         lifecycle.updated_meta.runtime.compaction_rt.last_decision;
       check
         bool
         "last check ts recorded"
         true
         (lifecycle.updated_meta.runtime.compaction_rt.last_check_ts > 0.0);
       check
         int
         "turn generation unchanged"
...
let test_load_context_prefers_live_primary_max_tokens_over_checkpoint_limit () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         int
         "stored checkpoint max preserved in checkpoint helper"
         4096
       ...
       | Some loaded ->
         check
           int
           "restore uses live primary max tokens"
           32768
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let meta =
         ...
         ; runtime = { base.runtime with last_continuity_update_ts = now_ts -. 60.0 }
         }
       in
       let state_reply =
         "ready\n\n[STATE]\nGoal: preserve continuity\nProgress: compacted\n[/STATE]"
       in
       let original_ctx = build_dense_context ~turns:24 ~max_tokens:320 ~state_reply in
       ...
       check
         int
         "last before tokens recorded"
         lifecycle.compaction.before_tokens
         lifecycle.updated_meta.runtime.compaction_rt.last_before_tokens;
       check
         int
         "last after tokens recorded"
         lifecycle.compaction.after_tokens
         lifecycle.updated_meta.runtime.compaction_rt.last_after_tokens;
       check
         bool
         "continuity summary captured"
         true
         (contains_substring
       ...
       match
         load_context
           ~base_dir
           ~trace_id:
             (Masc_mcp.Keeper_id.Trace_id.to_string
       ...
       | Some loaded ->
         check
           bool
           "compacted checkpoint persisted"
           true
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let meta =
         ...
         ; runtime = { base.runtime with last_continuity_update_ts = 1.0 }
         }
       in
       let original_ctx =
         build_dense_context
...
let test_apply_post_turn_lifecycle_handoffs_after_compaction () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check (option string) "handoff no failure" None lifecycle.handoff_failure_reason;
       check bool "compaction applied before handoff" true lifecycle.compaction.applied;
       check bool "handoff emitted" true (Option.is_some lifecycle.handoff_json);
       check
         int
         "turn generation remains pre-handoff generation"
         0
         lifecycle.turn_generation;
       check int "meta generation advanced" 1 lifecycle.updated_meta.runtime.generation;
       check
         bool
         "trace rotated"
         true
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let checkpoint =
         ...
         |> fun ctx ->
         KEC.append ctx (Agent_sdk.Types.user_msg "short checkpoint")
       ...
       check
         bool
         "ratio stays below threshold"
         true
         (lifecycle.context_ratio < meta.handoff_threshold);
       check bool "handoff attempted" true lifecycle.handoff_attempted;
       check bool "handoff emitted" true (Option.is_some lifecycle.handoff_json);
       check int "generation advanced" 1 lifecycle.updated_meta.runtime.generation)
;;
...
let test_rollover_aborts_on_save_failure () =
  ...
  Fun.protect
    ~finally:(fun () ->
      ...
      (try Unix.chmod base_dir 0o755 with
       | _ -> ());
    ...
    (fun () ->
       ...
       check bool "handoff failure recorded" true (Option.is_some rollover.failure_reason);
       check
         bool
         "handoff NOT emitted on save failure"
         false
       ...
       check int "generation unchanged" 0 rollover.updated_meta.runtime.generation)
;;
...
let test_recover_latest_checkpoint_for_overflow_retry_compacts_oas_checkpoint () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       match
         KEC.recover_latest_checkpoint_for_overflow_retry
           ~base_dir
           ~meta
           ~model:"llama:auto"
           ~primary_model_max_tokens:256
       ...
       | Some recovery ->
         ...
         check bool "saved tokens positive" true (recovery.compaction.saved_tokens > 0);
         (match
            load_context
              ~base_dir
              ~trace_id:(Masc_mcp.Keeper_id.Trace_id.to_string meta.runtime.trace_id)
       ...
       | Some recovery ->
         ...
         (match
          ...
          | Some loaded ->
            check
              int
              "legacy retry max tokens clamped"
              192
              (KEC.max_tokens_of_context loaded)
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let original_ctx =
         ...
         in
         let ctx =
           loop 1 (KEC.create ~system_prompt:long_system_prompt ~max_tokens:4096)
         in
         KEC.append
           ctx
       ...
       | Some _ ->
         (match
          ...
          | Some loaded ->
            check
              int
              "history retry max tokens clamped"
              512
              (KEC.max_tokens_of_context loaded)
          | None -> fail "expected overflow retry checkpoint to be saved")
       | None ->
         fail
           "expected overflow retry recovery to keep message history within budget even \
            with a large checkpoint system prompt")
;;
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Some recovery ->
         check
           (option string)
           "overflow retry drops structured orphan tool result"
           None
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Ok saved ->
         check
           (option string)
           "rollover drops structured orphan tool result"
           None
...
let contaminated_user_message ?(prompt = "짧게 ping만 해봐") () : Agent_sdk.Types.message =
...
let oversized_checkpoint_text = String.make 20_000 'x'

let oversized_user_message ?(prompt = "긴 텍스트도 저장되면 안 돼") () : Agent_sdk.Types.message =
  { Agent_sdk.Types.role = Agent_sdk.Types.User
  ; content =
      [ Agent_sdk.Types.Text prompt; Agent_sdk.Types.Text oversized_checkpoint_text ]
  ; name = None
  ; tool_call_id = None
  ; metadata = []
  }
...
let test_persist_message_drops_world_state_and_separates_internal_history () =
  ...
  Fun.protect
    ~finally:(fun () -> cleanup_dir base_dir)
    (fun () ->
       let session = KEC.create_session ~session_id:"trace-history-split" ~base_dir in
       KEC.persist_message
         ~source:"direct_user"
         session
       ...
       let main_history =
         Fs_compat.load_file (Filename.concat session.session_dir "history.jsonl")
       in
       ...
       check
         bool
         "main history keeps direct conversation"
         true
         (contains_substring main_history "real conversation");
       check
         bool
         "main history excludes world state prompt"
         false
         (contains_substring main_history "Current World State");
       check
         bool
         "main history excludes internal assistant"
         false
         (contains_substring main_history "internal reply");
       check
         bool
         "internal history drops world state prompt"
         false
         (contains_substring internal_history "Current World State");
       check
         bool
         "internal history stores internal assistant"
         true
...
let test_migrate_session_history_logs_moves_internal_entries () =
  ...
  Fun.protect
    ~finally:(fun () -> cleanup_dir base_dir)
    (fun () ->
       let session = KEC.create_session ~session_id:"trace-history-migrate" ~base_dir in
       ...
       Fs_compat.save_file
         ...
         (String.concat
          ...
          ^ "\n");
       let stats = KCC.migrate_session_history_logs ~session_dir:session.session_dir in
       ...
       let internal_history = Fs_compat.load_file internal_history_path in
       check
         bool
         "main history keeps real conversation"
         true
         (contains_substring main_history "real conversation");
       check
         bool
         "main history excludes world state"
         false
         (contains_substring main_history "Current World State");
       check
         bool
       ...
       check
         bool
         "internal history drops world state"
         false
         (contains_substring internal_history "Current World State");
       check
         bool
         "internal history keeps moved internal assistant"
         true
         (contains_substring internal_history "internal reply"))
;;
...
let test_save_oas_checkpoint_strips_ephemeral_world_state () =
  ...
  Fun.protect
    ~finally:(fun () -> cleanup_dir base_dir)
    (fun () ->
       let session = KEC.create_session ~session_id:"trace-save-strip" ~base_dir in
       let ctx =
         KEC.create ~system_prompt:"keeper lifecycle" ~max_tokens:64_000
         |> fun ctx ->
         KEC.append ctx (contaminated_user_message ()) |> KEC.sync_oas_context
       in
       match
         KEC.save_oas_checkpoint
           ~max_checkpoint_messages:120
       ...
       | Error e -> Alcotest.fail (Printf.sprintf "save_oas_checkpoint failed: %s" e)
       | Ok checkpoint ->
         check int "saved message count" 1 (List.length checkpoint.messages);
         let text = Agent_sdk.Types.text_of_message (List.hd checkpoint.messages) in
         check bool "preserves actual prompt" true (contains_substring text "짧게 ping만 해봐");
         check
           bool
           "drops world state snapshot"
           false
...
let test_save_oas_checkpoint_caps_oversized_text () =
  ...
  Fun.protect
    ~finally:(fun () -> cleanup_dir base_dir)
    (fun () ->
       let session = KEC.create_session ~session_id:"trace-save-cap" ~base_dir in
       let ctx =
         KEC.create ~system_prompt:"keeper lifecycle" ~max_tokens:64_000
         |> fun ctx -> KEC.append ctx (oversized_user_message ()) |> KEC.sync_oas_context
       in
       match
         KEC.save_oas_checkpoint
           ~max_checkpoint_messages:120
       ...
       | Ok checkpoint ->
         let text = Agent_sdk.Types.text_of_message (List.hd checkpoint.messages) in
         check bool "keeps prompt prefix" true (contains_substring text "긴 텍스트도 저장되면 안 돼");
         check bool "adds truncation marker" true (contains_substring text "[capped]");
         check
           bool
           "text was compacted"
           true
...
let test_sanitize_checkpoint_message_caps_oversized_tool_result () =
  let oversized = String.make (KCC.default_max_checkpoint_tool_result_chars + 1024) 'x' in
  ...
  | Some sanitized, stats ->
    (match sanitized.content with
     | [ Agent_sdk.Types.ToolResult { content; _ } ] ->
       ...
       check
         bool
         "tool result stays within configured cap"
         true
  ...
  | Some sanitized, stats ->
    (match sanitized.content with
     | [ Agent_sdk.Types.ToolResult { content = first; _ }
       ...
       check
         bool
         "second tool result truncated"
         true
         (contains_substring second KCC.checkpoint_text_cap_marker);
       check string "aggregate overflow gets stubbed" "[tool result cleared]" third
     | _ -> fail "expected three ToolResult blocks after aggregate cap");
    check int "aggregate cap records truncated blocks" 2 stats.truncated_blocks;
...
let test_save_oas_checkpoint_stubs_old_tool_results () =
  ...
  Fun.protect
    ~finally:(fun () -> cleanup_dir base_dir)
    (fun () ->
       let session = KEC.create_session ~session_id:"trace-save-tool-stub" ~base_dir in
       ...
       | Ok checkpoint ->
         let old_saved =
           tool_result_content_for_id ~tool_use_id:old_tool_use_id checkpoint.messages
         in
         let recent_saved =
           tool_result_content_for_id ~tool_use_id:recent_tool_use_id checkpoint.messages
         in
         (match old_saved with
          ...
          | Some content ->
            check
              bool
              "old tool result is compacted"
              true
              (not (String.equal old_output content));
            check
              bool
              "old tool result stays structured"
              true
              (String.starts_with ~prefix:"[tool:" content);
            check
              bool
              "old tool result keeps tool name"
              true
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Ok checkpoint ->
         ...
         check
           bool
           "save cap enforced with pair preservation"
           true
           (List.length checkpoint.messages <= 2);
         check
           (option string)
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Some loaded ->
         check
           bool
           "load cap enforced with pair preservation"
           true
           (List.length (ctx_messages loaded) <= 2);
         check
           (option string)
           "loaded orphan tool result removed"
           None
...
let test_deserialize_context_repairs_orphan_tool_result () =
  ...
  in
  let ctx = KCC.deserialize_context (KEC.serialize_context ctx) ~max_tokens:4096 in
  check
    (option string)
    "deserialized orphan tool result downgraded"
    None
  ...
  | { Agent_sdk.Types.content = [ Agent_sdk.Types.Text text ]; _ } ->
    check
      string
      "deserialized orphan falls back to marker when payload metadata is absent"
      "[tool result call-orphan-json]"
      text
...
let test_deserialize_context_preserves_valid_tool_pair () =
  ...
  in
  let roundtrip = KCC.deserialize_context (KEC.serialize_context ctx) ~max_tokens:4096 in
  check
    (option string)
    "paired tool result stays structured"
    (Some "paired output")
...
let test_deserialize_context_repairs_dangling_tool_use () =
  ...
  check
    bool
    "dangling tool use removed on deserialize"
    false
  ...
  | Some { Agent_sdk.Types.content = [ Agent_sdk.Types.Text text ]; _ } ->
    check
      bool
      "dangling tool use downgraded to text on deserialize"
      true
      (contains_substring text "tool use keeper_board_comment");
    check
      bool
      "dangling tool use id retained on deserialize"
      true
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Some loaded ->
         check
           bool
           "dangling tool use removed on load"
           false
         ...
         (match List.nth_opt (ctx_messages loaded) 1 with
          | Some { Agent_sdk.Types.content = [ Agent_sdk.Types.Text text ]; _ } ->
            check
              bool
              "dangling tool use downgraded to text on load"
              true
              (contains_substring text "tool use keeper_board_comment");
            check
              bool
              "dangling tool use id retained on load"
              true
              (contains_substring text tool_id)
          | _ -> fail "expected dangling tool use to downgrade to text on load"))
;;
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let ctx =
         KEC.create ~system_prompt:"keeper lifecycle" ~max_tokens:64_000
         |> fun ctx ->
         KEC.append ctx (summarized_contaminated_message ()) |> KEC.sync_oas_context
       in
       match
         KEC.save_oas_checkpoint
           ~max_checkpoint_messages:120
       ...
       | Ok checkpoint ->
         ...
         check
           bool
           "drops embedded world state from summary"
           false
           (contains_substring text "Current World State");
         check
           bool
           "preserves assistant summary text"
           true
...
let test_patch_checkpoint_replaces_last_assistant () =
  ...
  let patched =
    KCC.patch_checkpoint_last_assistant
      cp
  ...
  check bool "contains patched text" true (contains_substring text "patched response");
  check
    bool
    "replay metadata attached"
    true
...
let test_patch_checkpoint_preserves_non_assistant () =
  ...
  let patched =
    KCC.patch_checkpoint_last_assistant
      cp
      ~session_id:"s"
      ~response_text:"answer 2 with STATE"
  in
  ...
  check
    bool
    "no metadata without state snapshot"
    true
...
let test_patch_checkpoint_strips_ephemeral_world_state () =
  ...
  check bool "keeps user prompt" true (contains_substring first_text "지금 상태 한 줄로만 말해.");
  check
    bool
    "removes world state from prior user message"
    false
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       (match
        ...
        with
        | Ok () -> ()
        | Error e -> Alcotest.fail (Printf.sprintf "seed save_oas failed: %s" e));
       ...
       check
         bool
         "loaded prompt preserved"
         true
         (contains_substring loaded_text "짧게 ping만 해봐");
       check
         bool
         "loaded world state removed"
         false
       ...
       | Ok migrated ->
         ...
         check
           bool
           "migrated checkpoint persists cleanup"
           false
    ...
    (fun () ->
       ...
       let loaded_text =
         String.concat
           ...
           (List.map Agent_sdk.Types.text_of_message (ctx_messages loaded))
       in
       check bool "loaded prompt preserved" true (contains_substring loaded_text "짧게 요약해");
       check
         bool
         "loaded checkpoint capped"
         true
       ...
       | Ok migrated ->
         ...
         check
           bool
           "migrated checkpoint persists cap"
           true
           (contains_substring migrated_text "[capped]");
         check
           bool
           "migrated text reduced"
           true
...
let test_load_context_migrates_summarized_world_state_checkpoint () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let session = KEC.create_session ~session_id:trace_id ~base_dir in
       let checkpoint =
         make_test_checkpoint ~session_id:trace_id [ summarized_contaminated_message () ]
       in
       (match
          Masc_mcp.Keeper_checkpoint_store.save_oas
            ~session_dir:session.session_dir
       ...
       check
         bool
         "loaded summary drops world state"
         false
         (contains_substring loaded_text "Current World State");
       check
         bool
         "loaded summary keeps assistant text"
         true
       ...
       | Ok migrated ->
         ...
         check
           bool
           "migrated summary strips world state"
           false
           (contains_substring migrated_text "Current World State");
         check
           bool
           "migrated summary keeps assistant text"
           true
...
let make_gate_only_meta ?(last_continuity_update_ts = 0.0) ?(cooldown_sec = 3600) () =
  ...
  ; runtime = { base.runtime with last_continuity_update_ts }
  }
;;
...
let test_compact_if_needed_ts_zero_bypasses_cooldown () =
  ...
  let now_ts = 1000.0 in
  (* well within the 3600s cooldown window *)
...
let test_compact_if_needed_emergency_bypass_ignores_cooldown () =
  ...
  let ctx =
    ...
    |> fun c ->
    KEC.append c (Agent_sdk.Types.user_msg long_text)
    |> fun c ->
    KEC.append c (Agent_sdk.Types.assistant_msg long_text) |> KEC.sync_oas_context
  in
...
let test_compact_if_needed_records_saved_tokens_metric () =
  ...
  let meta =
    make_gate_only_meta ~last_continuity_update_ts:(now_ts -. 60.0) ~cooldown_sec:0 ()
  in
  let long_text = String.make emergency_test_text_length 'x' in
  let ctx =
    KEC.create ~system_prompt:"sp" ~max_tokens:100
  ...
  in
  let compacted_ctx, trigger, decision = KEC.compact_if_needed ~meta ~now_ts ctx in
  let after_metric =
    Masc_mcp.Prometheus.get_metric_value
      Masc_mcp.Prometheus.metric_keeper_compaction_saved_tokens
      ~labels
...
let test_pre_dispatch_resume_checkpoint_uses_loaded_working_context () =
  ...
  let ctx =
    ...
    |> fun c ->
    KEC.append c (Agent_sdk.Types.user_msg "sanitized user") |> KEC.sync_oas_context
  in
  let checkpoint =
  ...
  check
    bool
    "resume checkpoint is present for loaded context"
    true
  ...
  check
    int
    "resume turn count comes from sanitized working context"
    7
    resume_checkpoint.turn_count;
  check
    int
    "resume messages come from sanitized working context"
  ...
  check
    bool
    "fresh context does not force Agent.resume"
    false
...
let test_pre_dispatch_resume_checkpoint_saves_compacted_context () =
  ...
  let base_meta =
    make_gate_only_meta ~last_continuity_update_ts:now_ts ~cooldown_sec:3600 ()
  in
  let meta =
    { base_meta with compaction = { base_meta.compaction with ratio_gate = 0.1 } }
  in
  let long_text = String.make emergency_test_text_length 'x' in
  let ctx =
  ...
  let result =
    KAR.prepare_resume_checkpoint_for_dispatch
      ...
      ~save_checkpoint:(fun compacted_ctx ->
        ...
        Ok { (KEC.checkpoint_of_context compacted_ctx) with turn_count = 42 })
      ctx
  in
  check bool "pre-dispatch compaction applied despite fresh cooldown" true result.applied;
  check bool "compaction reduced token pressure" true result.meaningful_reduction;
  ...
  check
    int
    "resume checkpoint is the saved compacted checkpoint"
    42
    resume_checkpoint.turn_count;
  check
    int
    "context carries the same saved checkpoint"
    42
    result.context.checkpoint.turn_count;
  check
    int
    "saved checkpoint received compacted messages"
    !saved_message_count
    (List.length resume_checkpoint.messages)
;;
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Some entry ->
         check
           string
           "compaction start reaches registry"
           "compacting"
...
let test_dispatch_post_turn_lifecycle_events_uses_room_base_path () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       KEC.dispatch_post_turn_lifecycle_events ~config ~keeper_name:meta.name lifecycle;
       match KR.get ~base_path:config.base_path meta.name with
       | Some entry ->
         check
           string
           "compaction completion reaches registry"
           "running"
           (KST.phase_to_string entry.phase)
  ...
  run
    ...
    [ ( "post_turn_lifecycle"
      , [ test_case
        ...
        ; test_case
            "overflow retry repairs orphan tool result"
            `Quick
            test_recover_latest_checkpoint_for_overflow_retry_repairs_orphan_tool_result
        ; test_case
            "rollover repairs orphan tool result"
            `Quick
