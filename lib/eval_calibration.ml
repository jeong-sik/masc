...
let verdict_of_string raw =
  if String.equal raw "approve"
  then Some Anti_rationalization.Approve
  else if String.equal raw "reject"
  then Some (Anti_rationalization.Reject "")
  else if String.starts_with ~prefix:"reject:" raw
  then (
...
let base_path () = Filename.concat (Env_config_core.base_path ()) "data/verdicts"
...
let set_store_for_testing ~base_dir = store_ref := Some (Dated_jsonl.create ~base_dir ())
...
let to_harness_verdict (r : verdict_record) : Oas.Harness.verdict =
  let passed =
...
let record_verdict
      ...
      ?(on_harness_verdict : (Oas.Harness.verdict -> unit) option)
      ()
  : unit
  =
...
let string_field json key =
  try Yojson.Safe.Util.(json |> member key |> to_string) with
...
let find_divergences ?(since = "") ?(until = "") () : divergence list =
  ...
  let records =
    if since = "" && until = ""
    then Dated_jsonl.read_recent store 1000
    else (
    ...
    (fun hash v_json acc ->
       ...
       | Some l_json ->
         (match
          ...
          with
          | Some ev, Some hv ->
            if label_verdict_of_verdict ev <> hv
            then
              { notes_hash = hash
...
let select_examples ~(max_examples : int) : calibration_example list =
  ...
  let limited =
    if List.length sorted <= max_examples
    then sorted
...
let calibration_stats ?(since = "") ?(until = "") () : Yojson.Safe.t =
  ...
  let records =
    if since = "" && until = ""
    then Dated_jsonl.read_recent store 5000
    else (
      let s = if since = "" then "2020-01-01" else since in
      let u = if until = "" then "2099-12-31" else until in
      Dated_jsonl.read_range store ~since:s ~until:u)
  ...
  let ( total_verdicts
      ...
      , verdicts_with_generator
      , cross_model_match )
    =
    List.fold_left
      (fun (tv, ac, rc, gc, vh, lh, fbr, vwg, cmm) json ->
         let rt = string_field json "record_type" |> record_type_of_string in
         let hash = string_field json "notes_hash" in
         match rt with
         | Some Verdict_record ->
           (match verdict_of_string (string_field json "verdict") with
            | Some v ->
              ...
              in
              let fbr' =
                if gate = fallback_tag && List.length fbr < max_fallback_reasons
                then (
            ...
            | None -> tv, ac, rc, gc, vh, lh, fbr, vwg, cmm)
         | Some Label_record ->
           (match label_verdict_of_string (string_field json "human_verdict") with
            | Some v -> tv, ac, rc, gc, vh, StringMap.add hash v lh, fbr, vwg, cmm
            | None -> tv, ac, rc, gc, vh, lh, fbr, vwg, cmm)
         | None -> tv, ac, rc, gc, vh, lh, fbr, vwg, cmm)
      (0, 0, 0, StringMap.empty, StringMap.empty, StringMap.empty, [], 0, 0)
      records
  in
  (* Count divergences *)
  let false_pos, false_neg, agree =
    StringMap.fold
      (fun hash ev (fp, fn, ag) ->
  ...
  let agreement_rate =
    if labeled_total = 0 then 0.0 else float_of_int agree /. float_of_int labeled_total
  in
