...
parse None [] args|}
  ...
  ; { name = "Cargo"
    ...
    ; bind_pattern = "Cargo { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Cargo { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Go"
    ; anon_pattern = "Go _"
    ; bind_pattern = "Go { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Go { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Gh"
    ; anon_pattern = "Gh _"
    ; bind_pattern = "Gh { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Gh { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None None args|}
  ...
  ; { name = "Docker"
    ...
    ; bind_pattern = "Docker { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Docker"
    ; to_simple_body =
        {|
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Docker { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Opam"
    ; anon_pattern = "Opam _"
    ; bind_pattern = "Opam { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Opam { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Npx"
    ; anon_pattern = "Npx _"
    ; bind_pattern = "Npx { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Npx { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None None args|}
  ...
  ; { name = "Ruff"
    ; anon_pattern = "Ruff _"
    ; bind_pattern = "Ruff { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Ruff { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Pyright"
    ...
    ; bind_pattern = "Pyright { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Pyright { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Tsc"
    ; anon_pattern = "Tsc _"
    ; bind_pattern = "Tsc { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Tsc { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Ocamlfind"
    ; anon_pattern = "Ocamlfind _"
    ; bind_pattern = "Ocamlfind { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
      let all_args = subcommand :: args in
      { Shell_ir.bin = Exec_program.of_known Exec_program.Ocamlfind
      ; args = List.map (fun s -> Shell_ir.Lit (s, Shell_ir.default_meta)) all_args
      ; env = []
      ; cwd = None
      ; redirects = []
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Ocamlfind { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Rustc"
    ...
    ; bind_pattern = "Rustc { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Rustc { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Gofmt"
    ...
    ; bind_pattern = "Gofmt { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Gofmt { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Gradle"
    ...
    ; bind_pattern = "Gradle { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Gradle { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Ninja"
    ...
    ; bind_pattern = "Ninja { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Ninja { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Osascript"
    ; anon_pattern = "Osascript _"
    ; bind_pattern = "Osascript { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
      let all_args = subcommand :: args in
      { Shell_ir.bin = Exec_program.of_known Exec_program.Osascript
      ; args = List.map (fun s -> Shell_ir.Lit (s, Shell_ir.default_meta)) all_args
      ; env = []
      ; cwd = None
      ; redirects = []
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Osascript { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Play"
    ; anon_pattern = "Play _"
    ; bind_pattern = "Play { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
      let all_args = subcommand :: args in
      { Shell_ir.bin = Exec_program.of_known Exec_program.Play
      ; args = List.map (fun s -> Shell_ir.Lit (s, Shell_ir.default_meta)) all_args
      ; env = []
      ; cwd = None
      ; redirects = []
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Play { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Rec"
    ; anon_pattern = "Rec _"
    ; bind_pattern = "Rec { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
      let all_args = subcommand :: args in
      { Shell_ir.bin = Exec_program.of_known Exec_program.Rec
      ; args = List.map (fun s -> Shell_ir.Lit (s, Shell_ir.default_meta)) all_args
      ; env = []
      ; cwd = None
      ; redirects = []
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Rec { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Ffplay"
    ; anon_pattern = "Ffplay _"
    ; bind_pattern = "Ffplay { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
      let all_args = subcommand :: args in
      { Shell_ir.bin = Exec_program.of_known Exec_program.Ffplay
      ; args = List.map (fun s -> Shell_ir.Lit (s, Shell_ir.default_meta)) all_args
      ; env = []
      ; cwd = None
      ; redirects = []
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Ffplay { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Mpg123"
    ; anon_pattern = "Mpg123 _"
    ; bind_pattern = "Mpg123 { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
      let all_args = subcommand :: args in
      { Shell_ir.bin = Exec_program.of_known Exec_program.Mpg123
      ; args = List.map (fun s -> Shell_ir.Lit (s, Shell_ir.default_meta)) all_args
      ; env = []
      ; cwd = None
      ; redirects = []
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Mpg123 { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
...
parse None [] args|}
  ...
  ; { name = "Open"
    ; anon_pattern = "Open _"
    ; bind_pattern = "Open { subcommand; args }"
    ; risk = "`Audited"
    ; sandbox = "`Host"
    ; to_simple_body =
        {|
      let all_args = subcommand :: args in
      { Shell_ir.bin = Exec_program.of_known Exec_program.Open
      ; args = List.map (fun s -> Shell_ir.Lit (s, Shell_ir.default_meta)) all_args
      ; env = []
      ; cwd = None
      ; redirects = []
...
let rec parse subcmd extra = function
  | [] ->
    (match subcmd with
     | Some s ->
       Some (Shell_ir_typed_types.W (Shell_ir_typed_types.Open { subcommand = s; args = List.rev extra }))
     | None -> None)
  | arg :: rest ->
    (match subcmd with
     | None -> parse (Some arg) extra rest
