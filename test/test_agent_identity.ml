...
let test_from_mcp_params_empty_session_key () =
  ...
  check
    bool
    "empty session_key is regenerated"
    true
    (String.length identity.session_key > 0);
  check
    bool
    "regenerated session agent_name starts with agent-"
    true
    (String.sub identity.agent_name 0 6 = "agent-")
;;
...
let test_from_mcp_params_blank_session_key () =
  ...
  check
    bool
    "blank session_key is regenerated"
    true
    (String.length identity.session_key > 0);
  check
    bool
    "regenerated blank session agent_name starts with agent-"
    true
    (String.sub identity.agent_name 0 6 = "agent-")
;;
...
let test_from_mcp_params_short_session_key () =
  ...
  let identity = Agent_identity.from_mcp_params params in
  check string "short session_key uses full key" "agent-abc" identity.agent_name
;;

let test_from_mcp_params_long_session_key_prefix () =
  let params = `Assoc [ "_session_key", `String "abcdefghijk" ] in
  let identity = Agent_identity.from_mcp_params params in
  check string "long session_key trimmed to 8 chars" "agent-abcdefgh" identity.agent_name
;;
...
let test_channel_roundtrip () =
  ...
  List.iter
    (fun ch ->
       ...
       check
         bool
         (Printf.sprintf "roundtrip for %s" str)
         true
...
let test_channel_normalization () =
  ...
  in
  check string "trim + lowercase external" "discord" (normalize "  DisCord  ");
  check string "blank becomes unknown" "unknown" (normalize "   ");
  check string "api normalized" "api" (normalize "  API  ");
  check string "internal normalized" "internal" (normalize "  INTERNAL  ");
  check
    string
    "external stringified as normalized label"
    "slack"
...
let test_channel_yojson_backward_compat () =
  ...
  check
    (result string string)
    "tagged external Discord decodes"
    (Ok "discord")
    (match
       Agent_identity.channel_of_yojson (`List [ `String "External"; `String "discord" ])
     with
     | Ok channel -> Ok (Agent_identity.string_of_channel channel)
     | Error err -> Error err);
  check
    string
    "known external serializes as tagged external"
    "[\"External\",\"discord\"]"
    (Yojson.Safe.to_string
       (Agent_identity.channel_to_yojson (Agent_identity.External "DisCord")));
  check
    string
    "unknown external serializes as tagged external"
    "[\"External\",\"custom-edge\"]"
    (Yojson.Safe.to_string
       (Agent_identity.channel_to_yojson (Agent_identity.External "  Custom-Edge  ")))
;;
...
let test_to_display_string () =
  ...
  check
    ...
    "contains channel"
    true
    (str_contains (String.lowercase_ascii display) "telegram")
;;
...
let test_to_display_string_empty_session_key () =
  ...
  check
    bool
    "empty session key display uses unknown prefix"
    true
...
module RegistryTests = struct
  ...
  let test_register_and_find () =
    ...
    let found_by_session =
      Agent_identity.Registry.find_by_session reg identity.session_key
    in
  ...
  run
    ...
    [ ( "basics"
      , [ test_case "generate_session_key" `Quick test_generate_session_key
        ...
        ; test_case
            "from_mcp_params_empty_session_key"
            `Quick
            test_from_mcp_params_empty_session_key
        ; test_case
            "from_mcp_params_blank_session_key"
            `Quick
            test_from_mcp_params_blank_session_key
        ; test_case
            "from_mcp_params_short_session_key"
            `Quick
            test_from_mcp_params_short_session_key
        ; test_case
            "from_mcp_params_long_session_key_prefix"
            `Quick
            test_from_mcp_params_long_session_key_prefix
        ...
        ; test_case
            "channel_yojson_backward_compat"
            `Quick
        ...
        ; test_case
            ...
            `Quick
            test_to_display_string_empty_session_key
        ] )
    ; ( "registry"
