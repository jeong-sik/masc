...
let build_perpetual_agent
  ...
  (* Build provider via Phase 3 adapter.
     Uses Llm.to_oas_provider which handles the Llm.model_spec
     -> Oas.Provider.config conversion (avoiding Llm_types nominality gap). *)
  let provider = match Llm.to_oas_provider primary_model with
    | Some cfg -> cfg
    | None ->
  ...
  let tools = List.map (fun (td : Llm.tool_def) ->
