...
- **Agent Skills 스펙** (agentskills.io/specification): frontmatter 전체 필드는 6개
  ...
  병합하지 않는다. YAML string은 decode 결과에 관측용 원문으로만 보존하고 prompt,
  공개 registry, Keeper effective surface에는 투영하지 않는다. null/list 등 string이
  아닌 값은 다른 복구 가능한 편차처럼 `runtime_compatible` 진단으로 남긴다. MASC 승인
  정책만 실행 권한의 권위다.
