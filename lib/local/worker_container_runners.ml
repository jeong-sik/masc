...
let default_shell_tool_names () = [ "file_read"; "file_write"; "shell_exec" ]

let build_execution_spec
      ~base_path
      ~worker_name
      ~model_label
      ~runtime_backend
...
let effective_model_of_resume ~existing_meta spec =
  ...
  | Some meta when String.trim meta.effective_model <> "" -> Ok meta.effective_model
  | _ ->
    resolve_oas_provider_of_label spec.Worker_execution_spec.model_label |> Result.map snd
;;
...
let dedupe_tools_by_name (tools : Oas.Tool.t list) =
  let rec loop seen acc = function
    ...
    | (tool : Oas.Tool.t) :: rest ->
      if List.mem tool.schema.name seen
      then loop seen acc rest
      else loop (tool.schema.name :: seen) (tool :: acc) rest
  in
  loop [] [] tools
;;
...
let create_raw_trace ~base_path ~worker_name =
  try
    ...
    match
      Oas.Raw_trace.create ~path:(worker_raw_trace_path ~base_path ~worker_name) ()
    with
    | Ok raw_trace -> Ok raw_trace
    | Error err -> Error (Oas.Error.to_string err)
  with
  | Sys_error msg ->
    Error (Printf.sprintf "failed to create worker raw trace for %s: %s" worker_name msg)
;;

let run_worker_oas ~sw ?net ~room_config (spec : Worker_execution_spec.t)
  ...
  let mcp_session_id = resolved_mcp_session_id ~base_path ~worker_name in
  ...
  let* effective_model =
    ...
    | None -> resolve_oas_provider_of_label spec.model_label |> Result.map snd
  in
  let meta =
    make_worker_meta
      ~base_path
  ...
  in
  let* auth_token = worker_auth_token ~base_path ~worker_name in
  let* masc_tools =
    build_oas_mcp_tools ~sw ~auth_token ~session_id:mcp_session_id ~worker_name
  in
  let* shell_tools =
  ...
  match checkpoint with
  | Some checkpoint ->
    Worker_oas.resume_worker_via_oas
      ~sw
      ~net
      ~base_path
      ~auth_token
      ...
      ()
  | None ->
    let* provider, model_id = resolve_oas_provider_of_label spec.model_label in
    let system_prompt =
...
let run_worker
  ...
  | Worker_execution_backend.Local_playground -> run_worker_oas ~sw ?net ~room_config spec
  | Worker_execution_backend.Docker ->
