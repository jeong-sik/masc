...
let handle_keeper_down ctx args : tool_result =
  ...
  then false, "❌ invalid keeper name"
    ...
    | Ok (Some (name, m)) ->
      ...
      if remove_meta
      then (
        Safe_ops.remove_file_logged
          ~context:"keeper_down"
          (keeper_meta_path ctx.config name);
        Keeper_registry.unregister ~base_path:ctx.config.base_path name)
      else (
        let retained = { m with updated_at = now_iso (); paused = true } in
        (match write_meta ctx.config retained with
         | Ok () -> ()
         | Error err -> Log.Keeper.error "keeper_down write_meta failed: %s" err);
      ...
      then (
        ...
        in
        if validate_name (Keeper_id.Trace_id.to_string m.runtime.trace_id)
        then (
          let dir =
            Filename.concat
              (session_base_dir ctx.config)
              (Keeper_id.Trace_id.to_string m.runtime.trace_id)
          in
          try rm_rf dir with
          | Eio.Cancel.Cancelled _ as e -> raise e
          | exn ->
            Log.Keeper.error "session dir cleanup failed: %s" (Printexc.to_string exn)));
      let json =
        `Assoc
          [ "name", `String name
          ; "stopped", `Bool true
