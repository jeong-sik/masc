...
let governance_response_model_empty_metric = "masc_governance_response_model_empty_total"

let () =
  Prometheus.register_counter
    ~name:governance_response_model_empty_metric
    ~help:
      "Count of governance compute_judgments cycles where [response.model] was empty.  \
       Labels: [source=telemetry_resolved | unknown_sentinel]."
    ()
;;
...
let with_lock (st : state) f = Eio.Mutex.use_rw ~protect:true st.mutex f
...
let cache_ttl_sec () = float_of_int (max (interval_sec () * 4) 600)
...
let mark_fresh_cache_served (st : state) =
  ...
  if st.runtime_status <> status_stale_visible
  then (
...
let mark_refresh_failure ~now_ts (st : state) ~message =
  ...
  st.runtime_status <- (if cache_fresh then status_stale_visible else status_offline);
  ...
  | `Assoc action_fields ->
    ...
    let normalized_action =
      `Assoc
        (List.map
           (fun (key, value) ->
              if String.equal key "resolved_tool"
              then
                ( "resolved_tool"
                , option_to_yojson (fun item -> `String item) canonical_tool )
              else key, value)
           action_fields)
    in
    (match judgment with
     | `Assoc fields ->
       `Assoc
         (List.map
            (fun (key, value) ->
               if String.equal key "recommended_action"
               then "recommended_action", normalized_action
               else key, value)
            fields)
     | other -> other)
  | _ -> judgment
    ...
    (fun json ->
       try
         ...
         then (
           ...
           | Some current when judgment_generated_at current >= judgment_generated_at json
             -> ()
           | _ -> Hashtbl.replace table key json)
       with
       | Yojson.Safe.Util.Type_error _ -> ()
       | exn ->
         Log.Governance.warn "load_latest_from_disk parse: %s" (Printexc.to_string exn))
    jsons;
  table
;;
...
let load_latest_from_disk base_path =
  ...
  if jsons <> []
  then load_judgments_into_table jsons
  else (
    (* Legacy fallback *)
    let path = judgments_path base_path in
    if not (Sys.file_exists path)
    then Hashtbl.create 32
    else (
...
let latest_judgments base_path =
  ...
  with_lock st (fun () ->
    if Hashtbl.length st.judgments = 0
    then (
      let should_reload =
...
let runtime_status_at ~now_ts base_path =
  ...
  with_lock st (fun () ->
    ...
    let status =
      if st.refreshing
      then status_refreshing
      else (
        match st.runtime_status with
        | value when value = status_online && cache_fresh -> status_online
        | value when value = status_stale_visible && cache_fresh -> status_stale_visible
    ...
    ; cached_judgments_visible =
        cache_fresh && (status = status_stale_visible || status = status_backoff)
...
let runtime_status base_path = runtime_status_at ~now_ts:(Unix.gettimeofday ()) base_path
...
let normalize_allowed_tool_name value = value |> String.trim |> String.lowercase_ascii

let allowed_tool tool =
  List.mem
    (normalize_allowed_tool_name tool)
  ...
  | Ok result ->
    ...
    (try
       ...
       let parsed =
         ...
         | `Assoc [ ("raw", `String raw) ] ->
           (match Judge_json_recovery.extract_balanced_object raw with
            | Some block ->
              (try Llm_provider.Lenient_json.parse block with
               | _ -> parsed)
            | None -> parsed)
         | _ -> parsed
       in
       ...
       | `Assoc [ ("raw", `String raw) ] ->
         ...
         let msg =
           Judge_diagnostics.format_lenient_fallback ~judge_label:"Governance" raw
         in
       ...
       | _ ->
         ...
         let judgments =
           items
           |> List.filter_map
                (parse_item_judgment ~generated_at ~expires_at ~model_used:resolved_model)
         in
...
let should_backoff ~sw ~net =
  try
    let capacity =
      Cascade_config.local_capacity_for_selections ~sw ~net [ "governance_judge" ]
    in
...
let refresh_once
  ...
  let served_from_cache =
    ...
    with_lock st (fun () ->
      if cached_result_still_fresh ~now_ts st
      then (
  ...
  then Log.Governance.debug "refresh_once: fresh cached result; skipping compute"
  else if should_backoff ~sw ~net
  then (
    let was_online =
      with_lock st (fun () ->
    ...
    in
    if was_online
    then Log.Governance.info "backoff: local slots saturated, skipping cycle"
    else Log.Governance.debug "backoff: local slots saturated (first cycle)")
  else (
    with_lock st (fun () ->
      st.refreshing <- true;
    ...
    | Ok (model_used, generated_at, expires_at, judgments) ->
      ...
      with_lock st (fun () ->
        ...
        List.iter
          ...
          judgments)
    | Error message ->
      Log.Governance.warn "refresh_once: compute_judgments failed: %s" message;
      with_lock st (fun () ->
  ...
  then
    Eio.Fiber.fork_daemon ~sw (fun () ->
      ...
      let rec loop () =
        ...
        in
        if n > 0
        then Log.Governance.debug "backoff: sleeping %.0fs (consecutive=%d)" sleep_s n;
