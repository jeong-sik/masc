...
let extract_and_append_with_provider
  ...
  in
  let generation = max inp.Keeper_librarian.generation next_generation in
  ...
  | Ok episode ->
    ...
    (match
     ...
     with
     | Ok () ->
       (* RFC-0251: the RFC-0247 co-occurrence edge write was removed with the
          dark spreading-activation organ — no consumer remained. *)
       Ok episode
