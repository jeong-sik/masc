...
#
# CONTRACT:
#   - Upstream OAS must remain workspace client-agnostic. When an OAS
...
# 1. Upstream OAS SDK should not learn MASC workspace client vocabulary.
...
PYEOF
  ...
  if [[ -n "$masc_in_surface" ]]; then
    ...
    echo "  OAS must not learn MASC workspace client vocabulary."
