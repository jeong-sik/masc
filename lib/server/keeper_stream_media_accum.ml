...
let on_event t (evt : Masc_agent_core.Types.sse_event) =
  match evt with
  | Masc_agent_core.Types.ContentBlockStart { index; content_type; tool_id; tool_name }
  ...
  | Masc_agent_core.Types.ContentBlockDelta
      { index; delta = Masc_agent_core.Types.MediaDelta { media_type; source_type; data } } ->
      (match stream_block_for_index t index with
       ...
       | Some (Active_media active) ->
           Log.Keeper.warn
             ...
             active.media_type
             media_type
             (Masc_agent_core.Types.media_source_kind_to_string active.source_type)
