...
let vote_direction_to_string = function
  | Up -> "up"
  | Down -> "down"
;;
...
let valid_vote_direction_strings = List.map vote_direction_to_string all_vote_directions
...
let vote_log_path () =
  ...
  Filename.concat (Common.masc_dir_from_base_path ~base_path:base) "board_votes.jsonl"
;;
...
let append_vote_log ~target ~voter ~direction ~ts =
  ...
  with
  | Sys_error msg -> Log.BoardLog.error "persist error (append_vote_log): %s" msg
;;
...
let rewrite_vote_log store =
  ...
  with
  | Sys_error msg -> Log.BoardLog.error "persist error (rewrite_vote_log): %s" msg
;;
             ...
             (match Hashtbl.find_opt store.vote_log vote_key with
              | Some (prev, _prev_ts) when prev = direction ->
                Error
                  (Already_voted
                     (Printf.sprintf
                        "%s already voted %s on %s"
                        voter
                        (vote_direction_to_string direction)
                        post_id))
              | Some (_opposite, _prev_ts) ->
                let flipped =
                  ...
                  | Up ->
                    { post with
                      votes_up = post.votes_up + 1
                    ; votes_down = max 0 (post.votes_down - 1)
                    ; updated_at = now
                    }
                  | Down ->
                    { post with
                      votes_down = post.votes_down + 1
              ...
              | None ->
                let updated =
                  ...
                  | Down ->
                    { post with votes_down = post.votes_down + 1; updated_at = now }
                in
                ...
                in
                Thompson_sampling.record_vote ~agent_name:author_name ~direction:vote_dir;
                let earn = if direction = Up then Some author_name else None in
                Ok
           ...
           (match Hashtbl.find_opt store.vote_log vote_key with
            | Some (prev, _prev_ts) when prev = direction ->
              Error
                (Already_voted
                   (Printf.sprintf
                      "%s already voted %s on comment %s"
                      voter
                      (vote_direction_to_string direction)
                      comment_id))
            | Some (_opposite, _prev_ts) ->
              let flipped =
                ...
                | Up ->
                  { cmt with
                    votes_up = cmt.votes_up + 1
                  ; votes_down = max 0 (cmt.votes_down - 1)
                  }
                | Down ->
                  { cmt with
                    votes_down = cmt.votes_down + 1
...
let recalculate_reply_counts store =
  ...
  Hashtbl.iter
    (fun key (p : post) -> Hashtbl.replace store.posts key { p with reply_count = 0 })
    store.posts;
...
let load_persisted_votes store =
  ...
  then (
        ...
        (fun json ->
           ...
           | Some target, Some dir_str ->
             ...
             let ts =
               ...
               | None -> 0.0
             in
             if is_fixture_voter_target target
             then (
      ...
      then (
        ...
        Log.BoardLog.warn
          "#9921 fixture contamination: %d vote rows match fixture patterns \
           (hot-voter-*, synthetic-voter-*, :test-voter-*) in %s. Live ledger was \
           written by a test fixture at some point. Loaded=%d quarantined=%d \
           (MASC_BOARD_VOTE_QUARANTINE=%b). Truncation is an operator decision; see \
           #9921."
          !fixture_detected
          path
          !loaded
    ...
    with
    | Eio.Cancel.Cancelled _ as e -> raise e
    | e -> Log.BoardLog.error "load votes failed: %s" (Printexc.to_string e))
;;
  ...
  | Ok pid ->
    with_lock store (fun () ->
      ...
      | Some _ ->
        ...
        List.iter
          (fun comment_key -> Hashtbl.remove store.comments comment_key)
          comment_ids;
        let vote_keys =
          Hashtbl.fold
            (fun key _ acc ->
...
let flush_dirty store =
  with_lock store (fun () ->
    if store.dirty_posts
    then (
      rewrite_posts store;
      store.dirty_posts <- false);
    if store.dirty_comments
    then (
      rewrite_comments store;
      store.dirty_comments <- false);
    rewrite_vote_log store;
...
let get_agent_karma store ~agent_name =
  ...
  let post_karma =
    List.fold_left
      (fun acc (p : post) ->
         if Agent_id.to_string p.author = agent_name then acc + p.votes_up else acc)
      0
      all_posts
  in
  let comment_karma =
    Hashtbl.fold
      (fun _ (c : comment) acc ->
         if Agent_id.to_string c.author = agent_name then acc + c.votes_up else acc)
      store.comments
      0
  in
