...
let saved_when_post_smaller () =
  let pre = Keeper_token_count.pre_estimate 1000 in
  let post = Keeper_token_count.post_recount 200 in
  check saved_t "post < pre yields Saved" (`Saved 800)
    (Keeper_token_count.saved ~pre ~post)

let saved_zero_when_equal () =
  let pre = Keeper_token_count.pre_estimate 500 in
  let post = Keeper_token_count.post_recount 500 in
  check saved_t "post = pre yields Saved 0" (`Saved 0)
    (Keeper_token_count.saved ~pre ~post)

let divergent_when_post_bigger () =
  let pre = Keeper_token_count.pre_estimate 300 in
  let post = Keeper_token_count.post_recount 500 in
  check saved_t "post > pre yields Divergent" (`Divergent 200)
    (Keeper_token_count.saved ~pre ~post)

let negative_input_clamps_to_zero () =
  let pre = Keeper_token_count.pre_estimate (-100) in
  let post = Keeper_token_count.post_recount (-50) in
  check int "pre clamped to 0" 0 (Keeper_token_count.to_int pre);
  check int "post clamped to 0" 0 (Keeper_token_count.to_int post);
  check saved_t "0 vs 0 yields Saved 0" (`Saved 0)
    (Keeper_token_count.saved ~pre ~post)

let to_int_preserves_value () =
  let pre = Keeper_token_count.pre_estimate 1234 in
  check int "to_int round-trips constructor input" 1234
    (Keeper_token_count.to_int pre)

let () =
  run "keeper_token_count"
    [ ( "saved variant"
