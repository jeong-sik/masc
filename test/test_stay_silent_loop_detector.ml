...
let set_threshold n = Unix.putenv "MASC_STAY_SILENT_LOOP_THRESHOLD" (string_of_int n)
let clear_threshold () = Unix.putenv "MASC_STAY_SILENT_LOOP_THRESHOLD" ""
...
let test_any_other_act_resets () =
  ...
  D.record_turn ~keeper_name:k ~speech_act:"stay_silent";
  Alcotest.(check int) "after new stay_silent" 1 (D.current_streak ~keeper_name:k)
;;
...
let test_threshold_crossing_fires_counter () =
  ...
  Alcotest.(check (float 0.0001)) "no fire at 2" before (detected_count k);
  D.record_turn ~keeper_name:k ~speech_act:"stay_silent";
  Alcotest.(check (float 0.0001)) "fires at 3" (before +. 1.0) (detected_count k);
  clear_threshold ()
;;
...
let test_latched_no_repeat_while_streak_grows () =
  ...
  Alcotest.(check (float 0.0001))
    "latched: exactly +1 across 10 stay_silent"
    (before +. 1.0)
    (detected_count k);
  clear_threshold ()
;;
...
let test_explicit_reset () =
  ...
  Alcotest.(check int) "pre explicit reset" 5 (D.current_streak ~keeper_name:k);
  D.reset ~keeper_name:k;
  Alcotest.(check int) "post explicit reset" 0 (D.current_streak ~keeper_name:k)
;;
...
let () =
  Alcotest.run
    ...
    [ ( "streak semantics"
      , [ Alcotest.test_case
            "increments on stay_silent"
            `Quick
            (with_eio test_streak_increments)
        ; Alcotest.test_case
            "any other act resets"
            `Quick
            (with_eio test_any_other_act_resets)
        ; Alcotest.test_case "explicit reset" `Quick (with_eio test_explicit_reset)
        ] )
    ; ( "threshold crossing"
      , [ Alcotest.test_case
            "fires counter at threshold"
            `Quick
            (with_eio test_threshold_crossing_fires_counter)
        ; Alcotest.test_case
            "latched: no repeat while streak grows"
            `Quick
            (with_eio test_latched_no_repeat_while_streak_grows)
        ; Alcotest.test_case
            "latch releases on reset, then re-fires"
            `Quick
            (with_eio test_latch_releases_on_reset_then_refires)
        ] )
    ; ( "per-keeper isolation"
      , [ Alcotest.test_case
            "A and B independent"
    ...
    ; ( "threshold env var"
      , [ Alcotest.test_case
            "default 10"
            `Quick
            (with_eio test_threshold_env_default_is_10)
        ; Alcotest.test_case "custom values" `Quick (with_eio test_threshold_env_custom)
        ; Alcotest.test_case
            "invalid falls to default"
            `Quick
            (with_eio test_threshold_env_invalid_falls_through_to_default)
        ] )
    ]
;;
