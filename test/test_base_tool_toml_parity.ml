...
let expected =
    [ {|keeper_time_now|}, {|Read the server clock once: now_iso (ISO8601) and now_unix (float). Every tool result already ends with a [Temporal] time=... line, so a second call in the same turn returns what that line shows. A later time is reached by ending the turn with a masc_schedule_create wake, not by calling this again.|}, {|{"additionalProperties":false,"properties":{},"type":"object"}|}
    ...
    ; {|keeper_memory_retract|}, {|Retract one exact ordinary-current Memory OS fact. Use the memory_id returned by memory write or search and state the concrete reason. The same atomic commit removes every derived fact that no longer has a complete support path and records both the direct reason and the support invalidations for inspection.|}, {|{"additionalProperties":false,"properties":{"memory_id":{"description":"Exact sha256 Memory OS identity to retract.","type":"string"},"reason":{"description":"Concrete non-empty reason this fact is no longer current.","type":"string"}},"required":["memory_id","reason"],"type":"object"}|}
    ; {|keeper_memory_write|}, {|Record a durable claim that later turns read back. Your context resets between turns, so a conclusion you leave only in this turn's reasoning is gone. Task sequencing and operating constraints belong to their typed domain stores, not memory prose. The runtime records explicit typed provenance and returns validation or persistence failures directly. When source_path is set, the claim is recalled only while the exact source bytes still match. To record a conclusion rather than an observation, provide both rule_id and premise_ids; the conclusion remains current only while one complete support set remains current.|}, {|{"additionalProperties":false,"properties":{"content":{"description":"Body. Required, must be non-empty. For decisions, lead with the decision then **Why** and **How to apply** lines.","type":"string"},"premise_ids":{"description":"Exact memory_id values supporting a derived conclusion. Must be paired with rule_id.","items":{"type":"string"},"minItems":1,"type":"array","uniqueItems":true},"rule_id":{"description":"Opaque rule identity for a derived conclusion. Must be paired with premise_ids.","type":"string"},"source_path":{"description":"Optional keeper-visible regular file that makes this claim source-bound.","type":"string"},"title":{"description":"Optional hook; may be empty.","type":"string"}},"required":["content"],"type":"object"}|}
