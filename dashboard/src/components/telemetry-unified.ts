      ...
      <div id=${contentId} class=${expanded.value ? 'px-3 pb-3 flex flex-col gap-2' : 'hidden'} role="region" aria-label=${`${meta.label} 상세`}>
        ${expanded.value ? html`
          ...
          <details class="rounded bg-[var(--black-20)] px-2 py-1.5">
            ...
            <pre class="mt-2 text-3xs font-mono text-[var(--text-muted)] overflow-x-auto max-h-70 overflow-y-auto custom-scrollbar whitespace-pre-wrap break-all leading-[1.4]" tabindex="0" aria-label="Raw JSON">
  ...
  return html`
    <div class="flex flex-col gap-4">
      ...
      <div class="rounded border border-[var(--card-border)] overflow-hidden">
        ...
        ` : null}
        <div class="max-h-150 overflow-y-auto custom-scrollbar" tabindex="0" role="list" aria-label="이벤트 목록">
          ${displayItems.length > 0
            ...
            : isFilteringEntries && allDisplayItems.length > 0
              ? html`<div class="px-4 py-6 text-sm text-[var(--text-muted)]" role="status">필터 결과 없음 (${allDisplayItems.length} items)</div>`
