...
let event_to_json = function
  ...
  | Completed { assignment_id; tool_name; success; duration_ms; error_kind; timestamp } ->
    `Assoc
...
let emit_assigned
  ...
  : assignment_id
  =
  let assignment_id = with_rng (fun rng -> Uuidm.(v4_gen rng () |> to_string)) in
  let config_hash =
...
let emit_called ~agent_id ~tool_name ?(arguments_hash = "") ~source ()
  : assignment_id option
  =
  let assignment_id_opt =
    Eio_guard.with_mutex_ro index_mu (fun () -> Hashtbl.find_opt agent_index agent_id)
  in
