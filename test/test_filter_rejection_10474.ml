...
let agent_code = make_provider ~kind:Cli_tool_d ~model_id:"gpt-5.4"
let provider_c = make_provider ~kind:Cli_tool_b ~model_id:"model-c-coding"
...
(* Cli_tool_d has tool_policy=no_tool_http_headers; so it cannot satisfy
...
(* Cli_tool_b advertises request-scoped runtime MCP HTTP-header support,
...
(* Cli_tool_d cannot carry arbitrary request-scoped headers, but the
