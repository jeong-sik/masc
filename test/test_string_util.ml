...
let test_safe_truncated_ascii () =
  ...
  | Truncated { prefix; suffix; dropped_bytes } ->
    ...
    check bool "total <= max" true (String.length prefix + String.length suffix <= 8)
;;
...
let test_safe_suffix_longer_than_budget () =
  ...
  | Untouched _ -> fail "should have truncated"
  | Truncated { prefix; _ } -> check string "empty prefix when suffix>=max" "" prefix
;;
...
let test_find_substring_basic () =
  check_int_option "first occurrence" (Some 0) (SU.find_substring "abcabc" "abc");
  check_int_option "respects pos" (Some 3) (SU.find_substring ~pos:1 "abcabc" "abc");
  check_int_option "absent" None (SU.find_substring "abcabc" "z")
;;

let test_find_substring_boundaries () =
  check_int_option
    "match at last valid index"
    (Some 2)
    (SU.find_substring ~pos:2 "aaaa" "aa");
  check_int_option "pos past last valid index" None (SU.find_substring ~pos:3 "aaaa" "aa");
  check_int_option "pos at end" None (SU.find_substring ~pos:4 "aaaa" "a")
;;

let test_find_substring_empty_needle () =
  check_int_option "empty returns pos" (Some 2) (SU.find_substring ~pos:2 "abc" "");
  check_int_option "empty can return end" (Some 3) (SU.find_substring ~pos:3 "abc" "")
;;

let test_find_substring_rejects_negative_pos () =
  check_raises
    "negative pos"
    (Invalid_argument "String_util.find_substring: negative position")
...
let test_contains_substring_ci_ascii () =
  ...
  check bool "absent" false (SU.contains_substring_ci "sandbox profile" "keeper")
;;

let test_contains_substring_ci_empty_and_literal () =
  check bool "empty needle stays false" false (SU.contains_substring_ci "abc" "");
  check
    bool
    "regex chars are literal"
    true
...
let test_starts_with_ci_basic () =
  check bool "exact case" true (SU.starts_with_ci ~prefix:"Bearer " "Bearer abc123");
  check
    bool
    "lowercase prefix vs mixed value"
    true
    (SU.starts_with_ci ~prefix:"bearer " "Bearer abc123");
  check
    bool
    "uppercase prefix vs lowercase value"
    true
    (SU.starts_with_ci ~prefix:"BEARER " "bearer abc123");
  check bool "mismatch" false (SU.starts_with_ci ~prefix:"Bearer " "Basic abc123")
;;
...
let test_starts_with_ci_boundaries () =
  ...
  check bool "empty prefix matches empty string" true (SU.starts_with_ci ~prefix:"" "");
  check bool "prefix longer than string" false (SU.starts_with_ci ~prefix:"hello" "hi");
  check bool "exact length equal" true (SU.starts_with_ci ~prefix:"abc" "ABC")
;;
  ...
  run
    ...
    ; ( "utf8_safe"
      , [ test_case "short untouched" `Quick test_safe_untouched_short
        ...
        ; test_case "Korean UTF-8 valid" `Quick test_safe_truncated_korean_is_valid_utf8
        ; test_case "UTF-8 suffix (ellipsis)" `Quick test_safe_utf8_suffix
        ; test_case "emoji 4-byte boundary" `Quick test_safe_emoji_4byte_boundary
        ; test_case "empty string" `Quick test_safe_empty_string
        ; test_case "zero budget" `Quick test_safe_zero_budget
        ; test_case "suffix >= budget" `Quick test_safe_suffix_longer_than_budget
        ; test_case "was_truncated + to_string" `Quick test_was_truncated_and_to_string
        ; test_case "invalid UTF-8 best-effort" `Quick test_safe_invalid_utf8_best_effort
        ] )
    ; ( "find_substring"
    ...
    ; "equals_ci", [ test_case "basic" `Quick test_equals_ci_basic ]
    ]
;;
