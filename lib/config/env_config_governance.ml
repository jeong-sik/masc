...
module Inference = struct
  ...
  let timeout_seconds = get_float ~default:30.0 "MASC_INFERENCE_TIMEOUT_SEC"

  (** Integer fallback for call sites that use second granularity only. *)
  let timeout_seconds_int = max 1 (int_of_float timeout_seconds)
  ...
  let cache_enabled = Feature_flag_registry.get_bool "MASC_INFERENCE_CACHE_ENABLED"

  (** Default TTL for inference response cache (seconds). *)
  let cache_ttl_seconds = get_int ~default:300 "MASC_INFERENCE_CACHE_TTL_SEC"
  ...
  let cache_max_temperature = get_float ~default:0.0 "MASC_INFERENCE_CACHE_MAX_TEMP"
  ...
  let cache_l1_max_entries = get_int ~default:512 "MASC_INFERENCE_CACHE_L1_MAX_ENTRIES"
...
module Autonomy = struct
  ...
  let quiet_start = get_int ~default:3 "MASC_AUTONOMY_QUIET_START"

  (** Quiet hours end (0-23). *)
  let quiet_end = get_int ~default:7 "MASC_AUTONOMY_QUIET_END"
end
...
module AgentSelection = struct
  let max_starvation_ticks = get_int ~default:12 "MASC_AUTONOMY_MAX_STARVATION_TICKS"

  let starvation_bonus_coefficient =
    get_float ~default:0.15 "MASC_AUTONOMY_STARVATION_BONUS_COEF"
  ;;

  let thompson_weight = get_float ~default:0.7 "MASC_AUTONOMY_THOMPSON_WEIGHT"
  let vote_decay_factor = get_float ~default:0.95 "MASC_AUTONOMY_VOTE_DECAY_FACTOR"
end
...
module Timeouts = struct
  ...
  let neo4j_timeout_sec = Float.max 1.0 (get_float ~default:60.0 "MASC_NEO4J_TIMEOUT_SEC")

  (** SSE keepalive interval (seconds).
      Frequency of `: keepalive` frames on command-plane SSE streams.
      Clamped to >= 1.0 to prevent tight-loop when misconfigured. *)
  let sse_keepalive_sec = Float.max 1.0 (get_float ~default:30.0 "MASC_SSE_KEEPALIVE_SEC")

  (** A2A event buffer size per subscription.
      Caps the in-memory event list to prevent unbounded growth. *)
  let event_buffer_size = get_int ~default:100 "MASC_EVENT_BUFFER_SIZE"
end
...
module Dashboard_config = struct
  ...
  let fixtures_enabled () =
    Feature_flag_registry.get_bool "MASC_DASHBOARD_FIXTURES_ENABLED"
  ;;
  ...
  let command_plane_snapshot_cache_ttl_s () =
    Float.max 5.0 (get_float ~default:30.0 "MASC_COMMAND_PLANE_SNAPSHOT_CACHE_TTL_S")
  ;;

  (** Dashboard fixture name override. *)
  let fixture_opt () = Sys.getenv_opt "MASC_DASHBOARD_FIXTURE" |> trim_opt
  ...
  let governance_judge_enabled =
    Feature_flag_registry.get_bool "MASC_DASHBOARD_GOVERNANCE_JUDGE_ENABLED"
  ;;
end
...
module Model_defaults = struct
  ...
  let default_cascade_opt () = Sys.getenv_opt "MASC_DEFAULT_CASCADE" |> trim_opt

  (** Default provider name. *)
  let default_provider_opt () = Sys.getenv_opt "MASC_DEFAULT_PROVIDER" |> trim_opt

  (** Default model id. *)
  let default_model_opt () = Sys.getenv_opt "MASC_DEFAULT_MODEL" |> trim_opt
  ...
  let goal_models_opt () = Sys.getenv_opt "MASC_GOAL_MODELS" |> trim_opt

  (** Goal dispatch runtime. Default: "task". *)
  let goal_dispatch_runtime () = get_string ~default:"task" "MASC_GOAL_DISPATCH_RUNTIME"
end
...
module AntiRationalization = struct
  ...
  let fail_mode =
    fail_mode_of_string (get_string ~default:"open" "MASC_ANTI_RATIONALIZATION_FAIL_MODE")
  ;;
