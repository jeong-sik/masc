(** Turn-scoped OAS event-bus state for [Keeper_unified_turn].
    ...
    ([Masc_agent_core_metrics_bridge]) pushes events from the agent-sdk event domain,
...
type event_bus_subscription =
  ...
  | Subscribed of
      ...
      ; event_bus_sub : Masc_agent_core_metrics_bridge.handle
      }
      ...
      Subscribed
        ...
        ; event_bus_sub =
            Masc_agent_core_metrics_bridge.subscribe
              ...
              ~purpose:"keeper_turn"
              ~filter:(Masc_agent_core.Event_bus.filter_agent keeper_name)
              event_bus
        }
    | None -> No_event_bus
  in
...
let record_fsm_tool_transitions ~keeper_name ~turn_id old_count events =
  ...
  List.iter
    (fun (evt : Masc_agent_core.Event_bus.event) ->
       ...
       | Masc_agent_core.Event_bus.ToolCalled _ ->
         let prev = !count in
         count := prev + 1;
         if prev = 0 then transitions := Enter_awaiting :: !transitions
       | Masc_agent_core.Event_bus.ToolCompleted _ ->
         let prev = !count in
         if prev > 0
         then (
           count := prev - 1;
...
let drain ?(site = "unspecified") t =
  let events =
    ...
    | Subscribed { event_bus_sub; _ } -> Masc_agent_core_metrics_bridge.drain event_bus_sub
    | No_event_bus -> []
  in
...
let unsubscribe t =
  ...
  | Subscribed { event_bus; event_bus_sub } ->
    Masc_agent_core_metrics_bridge.unsubscribe event_bus event_bus_sub
  | No_event_bus -> ())
;;
