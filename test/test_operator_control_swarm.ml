...
let expect_unsupported_action ~action_type result =
  ...
  | Ok body ->
    Alcotest.failf
      "expected error %S, got success: %s"
      expected
      (Yojson.Safe.to_string body)
  | Error err ->
    if String.equal err expected
    then ()
    else Alcotest.failf "expected error %S, got error %S" expected err
;;
    ...
    (fun () ->
       ...
       match
         Operator_control.confirm_json
           ctx
           (`Assoc
               [ "actor", `String "operator"; "confirm_token", `String "expired-token" ])
       with
