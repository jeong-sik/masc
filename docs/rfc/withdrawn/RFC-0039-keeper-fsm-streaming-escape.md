...
> **Created**: 2026-05-07
> **Related RFCs**: RFC-0001 (det/nondet boundary), RFC-0026 (work-conserving keeper admission), RFC-0027 (capability-typed runtime catalog), **RFC-0038 (runtime routing intent preservation)** — RFC-0038 §3.2 NG6/NG7/NG8 가 본 RFC scope 로 분리됨. 두 RFC 가 함께 진행되어야 2026-04-28 executor incident 가 자율 복구 가능.
...
RFC-0038 가 runtime routing layer (Axis 3) 의 silent substitution 을 다룬다. 본 RFC 는 keeper Turn FSM (Axis 1) 의 **streaming-state escape gap** 을 다룬다. 두 RFC 는 직교 layer 를 다루지만, Provider-C 분석이 보여준 4월 28일 사고 같은 케이스에서는 양쪽 fix 가 모두 필요하다.
...
| Time (KST) | Event |
|------------|-------|
| 17:21:02 | `local_with_kimi_coding_with_glm` runtime tool-use gate 에서 모든 provider reject (`codex_keeper_bound_actor_required`) |
| 17:21:39 | FSM `awaiting_provider → streaming` |
| 17:21:40 | OAS turn 35 시작, tool surface 133개 노출. ollama qwen3.6 으로 cross-runtime fallback (RFC-0038 L0 의 path) |
...
RFC-0038 L0 (cross-runtime fallback default-off) 가 step "ollama qwen3.6 으로 cross-runtime fallback" 자체를 차단해도, **이미 streaming 에 진입한 turn 의 hang** 은 별도 문제. 본 RFC 의 F1 (TLA spec gap) 와 F2 (timeout default 불일치) 가 같이 풀려야 자율 복구.
...
| G6 | Cross-axis trace correlation — runtime audit observation (RFC-0038 §4.3) 와 keeper turn FSM transition 이 같은 `turn_id` 로 묶임. RFC-0038 의 `substituted_from_runtime` audit 와 본 RFC 의 turn timeline 이 single trace 로 결합. | F1+F4 |

### 3.2 Non-Goals

| # | 분리 |
|---|------|
| NG1 | Runtime routing 자체 — RFC-0038 |
...
**Trade-off**: 60s 가 너무 짧으면 normal slow generation (e.g. 27B+ local model) 에 false positive. 별도 runtime-level override 필요 (e.g. ollama runtime 는 longer timeout, cloud runtime 는 짧은 timeout).
...
**Code surface**: `lib/oas_worker_runtime.ml:220-258` (RFC-0038 §4.3 의 runtime observation), `specs/keeper-turn-fsm/KeeperTurnFSM.tla` (transition 의 turn_id 변수).

**Design**: runtime observation 의 `turn_id` 와 keeper turn FSM 의 transition log 가 같은 ID 로 묶임. RFC-0038 의 `substituted_from_runtime` audit 와 본 RFC 의 streaming hang detection 이 single trace 로 결합 — 4월 28일 사고 같은 case 의 forensic 이 한 query 로 가능.

**Implementation**: 기존 `runtime_observation.turn_id` 필드 (이미 존재) 를 turn FSM transition log 에 propagate. 신규 schema 변경 없음, 단지 logging side 의 enrichment.
...
| G6 | E2E test: runtime observation 의 turn_id 가 keeper turn FSM transition log 와 join 가능 |
...
| Risk | Mitigation |
|------|------------|
| G3 의 60s default 가 normal local 27B+ generation 에 false positive | runtime-level override (capability profile 별 다른 timeout). benchmarking 후 default 조정. |
...
- `feedback_keeper_hallucinated_audit_runtime`: 본 RFC §2 의 정량 주장 (line 번호, default 값) 모두 직접 grep. 다른 분석의 인용을 재인용하지 않음.
...
- Q2: G3 의 `MASC_STREAM_HANG_TIMEOUT_S` default 값 — 60s 가 정말 합리적? local 35B+ MoE 의 first token latency 가 60s 이상인 케이스가 있는데. runtime-level override 만으로 충분한가, 아니면 model-id 별 default 가 필요한가?
...
- RFC-0038: runtime routing intent preservation (PR #13913) — 본 RFC 의 sister RFC, 함께 4월 28일 사고 root-cause coverage
