...
let record config (metric : task_metric) : unit =
  ...
  Fun.protect ~finally:(fun () -> try Unix.close fd with exn -> Printf.eprintf "[metrics] fd close failed: %s\n%!" (Printexc.to_string exn)) (fun () ->
    Unix.lockf fd Unix.F_LOCK 0;
    Fun.protect ~finally:(fun () -> try Unix.lockf fd Unix.F_ULOCK 0 with exn -> Printf.eprintf "[metrics] fd unlock failed: %s\n%!" (Printexc.to_string exn)) (fun () ->
