  ...
  let args =
    Array.of_list
      ([ "curl"
       ...
       @ extra_header_args
       @ [ "-o"; body_file; "-D"; header_file; "--data-binary"; "@" ^ data_file; url ])
  in
...
let parse_json_body label result =
  ...
  try Yojson.Safe.from_string normalized with
...
let rec call_status_until_ready ~port ~retries_left =
  let result =
    run_curl_post
      ...
      ~payload:(tool_payload ~id:201 ~name:"masc_status" ~arguments:(`Assoc []))
      ()
  in
  match result.status, retries_left with
  | Some 500, retries
    when retries > 0 && contains_substr "Server state not initialized" result.body ->
    Unix.sleepf 0.5;
    call_status_until_ready ~port ~retries_left:(retries - 1)
  | Some 503, retries
    when retries > 0 && contains_substr "Server is starting up, not ready yet" result.body
    ->
    Unix.sleepf 0.5;
    call_status_until_ready ~port ~retries_left:(retries - 1)
  | _ -> result
...
let test_post_tools_call_streams_sse_framing () =
  ...
  check bool "message event sent" true (contains_substr "event: message" result.body);
...
let test_post_tools_call_preserves_explicit_legacy_agent_name () =
  ...
  check
    bool
    "explicit legacy agent_name preserved"
    true
    (contains_substr "Type: gemini" text);
  check
    bool
    "header actor not injected over explicit legacy agent_name"
    false
...
let () =
  run
    ...
    [ ( "mcp"
      , [ test_case
            "post tools/call streams sse framing"
            `Slow
            test_post_tools_call_streams_sse_framing
        ; test_case
            "post tools/call preserves explicit legacy agent_name"
            `Slow
            test_post_tools_call_preserves_explicit_legacy_agent_name
        ] )
    ]
;;
