...
let pending_hypotheses : (string, string) Hashtbl.t = Hashtbl.create 4
...
let custom_generators : (string, code_generator) Hashtbl.t = Hashtbl.create 4

(** Set a custom code generator for a loop (used in tests). *)
let set_generator loop_id gen = Hashtbl.replace custom_generators loop_id gen
