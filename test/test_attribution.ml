...
let evidence_sample : Yojson.Safe.t =
  `Assoc [ "check_id", `String "tool_limit"; "observed", `Int 42; "threshold", `Int 30 ]
;;
...
let test_roundtrip_policy_failed () =
  ...
  | Ok t' ->
    (match t'.outcome with
     ...
     | other -> Alcotest.fail ("expected Policy_failed, got " ^ outcome_kind other))
...
let test_roundtrip_transition_blocked () =
  ...
  | Ok t' ->
    (match t'.outcome with
     ...
     | other -> Alcotest.fail ("expected Transition_blocked, got " ^ outcome_kind other))
  ...
  | Ok t' ->
    ...
    (match t'.outcome with
     | Partial_pass { score; rationale } ->
       ...
       Alcotest.(check string) "rationale" "output partially matches spec" rationale
     | other -> Alcotest.fail ("expected Partial_pass, got " ^ outcome_kind other))
...
let test_wire_format_tagged_union () =
  let cases =
    ...
    ; A.policy_failed ~origin:Det ~gate:"x" ~evidence:`Null ~reason:"r", "policy_failed"
    ; ( A.transition_blocked
          ~origin:Det
          ~gate:"x"
          ~evidence:`Null
          ...
          ~reason:"r"
      , "transition_blocked" )
    ; ( A.partial_pass ~origin:Det ~gate:"x" ~evidence:`Null ~score:0.5 ~rationale:"r"
      , "partial_pass" )
    ]
  in
  List.iter
...
let test_wire_format_toplevel_shape () =
  ...
  | `Assoc fields ->
    ...
    List.iter
      (fun key ->
         Alcotest.(check bool)
           (key ^ " absent (sum type, not product)")
           false
...
let test_parse_transition_blocked_missing_field () =
  (* Missing to_state. *)
  expect_error
    "transition_blocked missing to_state"
    (`Assoc
        [ "origin", `String "det"
        ; "gate", `String "x"
        ; "evidence", `Null
...
let test_parse_non_object () = expect_error "non-object" (`String "nope")
...
let test_show_elides_long_fields () =
  ...
  Alcotest.(check bool)
    "no reason text in show"
    false
    (Astring.String.is_infix ~affix:"also long rationale" s);
  Alcotest.(check bool) "has gate" true (Astring.String.is_infix ~affix:"verification" s);
  Alcotest.(check bool)
    "has outcome kind"
    true
  ...
  Alcotest.run
    ...
    [ ( "roundtrip"
      , [ Alcotest.test_case "passed" `Quick test_roundtrip_passed
        ; Alcotest.test_case "policy_failed" `Quick test_roundtrip_policy_failed
        ; Alcotest.test_case "transition_blocked" `Quick test_roundtrip_transition_blocked
        ; Alcotest.test_case
            "partial_pass nondet"
            `Quick
    ...
    ; ( "wire format"
      , [ Alcotest.test_case "tagged union kind" `Quick test_wire_format_tagged_union
        ; Alcotest.test_case
            "top-level shape (no leftover flat fields)"
            `Quick
            test_wire_format_toplevel_shape
        ] )
    ; ( "parse errors"
      , [ Alcotest.test_case "missing origin" `Quick test_parse_missing_origin
        ...
        ; Alcotest.test_case "unknown outcome kind" `Quick test_parse_unknown_outcome_kind
        ; Alcotest.test_case
            "policy_failed missing reason"
            `Quick
            test_parse_policy_failed_missing_reason
        ; Alcotest.test_case
            "transition_blocked missing field"
            `Quick
            test_parse_transition_blocked_missing_field
        ; Alcotest.test_case
            "partial_pass wrong type"
            `Quick
    ...
    ; ( "show"
      , [ Alcotest.test_case "elides long fields" `Quick test_show_elides_long_fields ] )
    ]
;;
