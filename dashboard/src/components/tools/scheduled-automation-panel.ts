 ...
 * and payload identity") ordered soonest-first. The scheduler is a turn-start channel
 ...
 * recurrence label). Nothing is fabricated — a row without a
      ...
      ${signals.map(
        signal => html`
          ...
          >
            ...
            <${StatusChip} tone=${signal.tone} uppercase=${false}>${enumLabel(signal.status)}<//>
...
function reactionEvidenceRows(
  ...
  const rows: Array<{ label: string; value: string | number | boolean | null | undefined }> = [
    ...
    { label: 'unattributed_syntax_error_count', value: evidence.unattributed_syntax_error_count },
    { label: 'unattributed_identity_quarantine_count', value: evidence.unattributed_identity_quarantine_count },
    { label: 'first_syntax_error_path', value: evidence.first_syntax_error?.path },
    { label: 'first_syntax_error_line', value: evidence.first_syntax_error?.line_number },
    { label: 'first_syntax_error_detail', value: evidence.first_syntax_error?.detail },
    { label: 'first_identity_quarantine_reason', value: evidence.first_identity_quarantine_reason },
    { label: 'first_matching_quarantine_reason', value: evidence.first_matching_quarantine_reason },
    ...
    matchedIds.length === 0
      ...
      : matchedIds.flatMap(scheduleId => {
          ...
          if (isTerminalStatus(row.status)) {
            return [{ scheduleId, reason: 'status_terminal' }]
          }
          return []
        })
...
type SchTabKey = 'scheduled' | 'due' | 'running' | 'done' | 'all'
 ...
 *  a read-only pulse (예약됨/due·실행) + failed/due/recent triage
    ...
    <aside class="ov-aside" aria-label="예약 운영 상태" data-testid="schedule-aside">
      ...
      <section class="wka-sec">
        ...
        <div class="wka-list">
          ...
          `)}
          ${needTotal === 0 ? html`<div class="wka-calm mono">실행 대기 없음</div>` : null}
        </div>
      </section>
        ...
        ? html`
            <div class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]">
              ...
              <aside class="grid content-start gap-2">
                <${ScheduleDetailPanel} request=${selectedRequest} />
                <div
                  data-schedule-durable-signal-contract="payload_support"
                  data-schedule-durable-signal-raw=${durableSignalContract.rawCount}
                  data-schedule-durable-signal-visible=${durableSignalContract.visibleCount}
