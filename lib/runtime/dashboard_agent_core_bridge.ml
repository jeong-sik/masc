...
let source_label = "agent_core_runtime_bridge"
let durable_replay_surface = "/api/v1/dashboard/telemetry?source=agent_core_event"
...
let broadcast_sample_entry entry =
  ...
  | exn ->
      Log.Dashboard.warn "agent_core telemetry SSE broadcast failed: %s"
...
let ttfb_from_telemetry (telemetry : Agent_core.Types.inference_telemetry) =
...
let duration_from_response ?total_duration_ms
  ...
  let duration_from_telemetry (telemetry : Agent_core.Types.inference_telemetry) =
    let decode_ms =
      match telemetry.timings with
...
let ttfb_from_response (response : Agent_core.Types.api_response) =
  match response.telemetry with
  | Some telemetry -> Option.value ~default:default_ttfb_ms (ttfb_from_telemetry telemetry)
  | _ -> default_ttfb_ms

let cache_hit_from_response ~(usage : Agent_core.Types.api_usage option)
    (response : Agent_core.Types.api_response) =
  let usage_cache_hit =
    Option.map
      (fun (usage : Agent_core.Types.api_usage) ->
...
let throughput_from_response ~(usage : Agent_core.Types.api_usage option)
    ~ttfb_ms
    ~total_duration_ms (response : Agent_core.Types.api_response) =
  match response.telemetry with
...
let sample_of_response ~provider_id:_ ~model_id:_ ?total_duration_ms
  ...
  let ttfb_ms = ttfb_from_response response in
  {
    ...
    input_tokens =
      Option.map
        (fun (usage : Agent_core.Types.api_usage) -> usage.input_tokens)
        usage;
    output_tokens =
      Option.map
        (fun (usage : Agent_core.Types.api_usage) -> usage.output_tokens)
        usage;
    throughput_tokens_per_s =
      throughput_from_response ~usage ~ttfb_ms ~total_duration_ms response;
    cost_usd =
      Option.bind usage (fun (usage : Agent_core.Types.api_usage) ->
          usage.cost_usd);
...
let recent_json ?provider ?limit () =
  ...
  `Assoc
    [
      ...
      ("dashboard_surface", `String "/api/v1/dashboard/agent_core/telemetry/recent");
...
let summary_json ?provider ?limit () =
  ...
  `Assoc
    [
      ("generated_at", `String (Masc_domain.now_iso ()));
      ("dashboard_surface", `String "/api/v1/dashboard/agent_core/telemetry/summary");
      ("source", `String source_label);
      ("retention", retention_json);
      ("provider", provider_json provider);
      ( "limit", Json_util.int_opt_to_json limit );
