...
- Setup: a fresh workspace declares a librarian lane. Setup renders the runtime it just verified with a real reply into `[runtime.exact_output_lanes.librarian_exact]`; nothing wrote the key before, so the Librarian ran degraded from the first boot — one WARN, every surface reading healthy, and nothing curated for the life of the install. The line is an operator's to read and point at a cheaper model, and a fleet that wants no Librarian empties it (#36885).
