...
let summary_json_of_runtime
  ...
  `Assoc
    ...
    ; "judge_online", `Bool runtime.judge_online
    ; ( "judge_last_seen_at"
      , timestamp_option_json runtime.generated_at runtime.generated_at_unix )
    ]
;;
...
let dashboard_json ~base_path ~limit ~offset:_ ~status_filter:_ =
  ...
  let approval_rules = Keeper_approval_queue.list_rules_dashboard_json ~base_path () in
  `Assoc
