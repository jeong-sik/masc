...
Use Bash for command execution. Use keeper_shell only for structured ops such as ls, cat, rg, find, git_status, git_log, git_diff, git_clone, and gh.
...
- Run shell commands: Bash with command=<command> (read-only unless Coding/Delivery/Full preset). ONE command per call — no chaining or file redirects.
- Write or create a file: Edit or Write (Coding/Delivery/Full). Writable scope: your sandbox only.
...
3. `Bash command='git status'` → `git add <paths>` → `git commit -m ...` → `git push -u origin HEAD` — all with cwd inside the worktree
