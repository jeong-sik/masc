...
let test_measure_metric_parses_tagged_output () =
  with_temp_dir "masc_metric_oas"
  @@ fun workdir ->
  with_eio
  @@ fun ~sw:_ ~clock:_ ->
  let metric_fn = "python3 -c 'print(\"<metric name=\\\"score\\\">1.25</metric>\")'" in
  ...
  | Error err -> failf "expected tagged metric to parse, got %s" err
;;

let test_measure_metric_rejects_shell_metacharacters () =
  let cases = [ "echo $(whoami)"; "echo foo\\ bar"; "echo test | cat"; "echo (\n)" ] in
  List.iter
    (fun metric_fn ->
...
let test_cycle_reinjects_diff_guard_lesson () =
  ...
  in
  Lib.Tool_autoresearch_registry.set_generator
    state.loop_id
    (fun
        ~goal:_
         ~baseline:_
  ...
  let first =
    Lib.Tool_autoresearch_cycle.handle_cycle
      ctx
  ...
  check
    string
    "external drift preserved"
    "drifted\n"
    (Fs_compat.load_file (Filename.concat repo "notes.txt"));
  let feedback_findings =
    Lib.Autoresearch_knowledge.search_findings
      ~base_path:root
      ~query:"Diff guard rejected patch"
      ~limit:5
      ()
  in
  ...
  check
    string
    "finding keeper uses lineage actor"
    Lib.Autoresearch_lineage.lesson_reviewer_actor_name
    feedback_finding.keeper_name;
  check
    (list string)
    "finding tags use lineage contract"
    [ Lib.Autoresearch_lineage.domain_tag; state.target_file; "diff-guard" ]
...
let test_build_verify_downgrade_rewrites_history () =
  with_temp_dir "masc_cycle_build_verify"
  @@ fun root ->
  with_base_path root
  @@ fun () ->
  with_eio
  ...
  let first_decision =
    ...
    | None -> failf "expected first cycle decision, got %s" (Yojson.Safe.to_string first)
  in
  check string "first cycle downgraded to discard" "discard" first_decision;
  ...
  check
    string
    "stored history head rewritten to discard"
    "discard"
  ...
  let second_decision =
    ...
    | Some decision -> decision
    | None ->
      failf "expected second cycle decision, got %s" (Yojson.Safe.to_string second)
  in
  check string "second cycle still runs" "discard" second_decision;
  ...
  check
    string
    "next cycle receives downgraded discard history"
    "discard"
...
let test_start_seeds_source_only_target_file_into_managed_worktree () =
  with_temp_dir "masc_autoresearch_seed"
  @@ fun root ->
  with_base_path root
  @@ fun () ->
  with_eio
  ...
  in
  match
    Lib.Tool_autoresearch.dispatch
      ctx
...
let test_resolve_target_file_path_reports_realpath_errors () =
  ...
  in
  match
    Lib.Autoresearch.resolve_target_file_path
      ~workdir:missing_root
      ".masc/keepers/admin-keeper/test_ar.py"
  with
  | Ok path -> failf "expected missing workdir to error, got %s" path
  | Error message ->
    check bool "realpath failure surfaced" true (contains message "realpath failed")
;;

let test_cycle_restores_ignored_target_after_empty_diff () =
  with_temp_dir "masc_autoresearch_restore"
  @@ fun root ->
  with_base_path root
  @@ fun () ->
  with_eio
...
let test_is_in_git_repo_false_for_tmpdir () =
  with_temp_dir "test_no_git"
  @@ fun dir ->
  check bool "temp dir without .git" false (Lib.Autoresearch.is_in_git_repo dir)
;;

let test_is_in_git_repo_true_for_git_dir () =
  with_temp_dir "test_with_git"
  @@ fun dir ->
  init_git_repo dir;
  check bool "dir with git init" true (Lib.Autoresearch.is_in_git_repo dir)
;;

let test_is_in_git_repo_true_for_subdirectory () =
  with_temp_dir "test_subdir"
  @@ fun dir ->
  init_git_repo dir;
  let subdir = Filename.concat dir "nested" in
  Unix.mkdir subdir 0o755;
  check bool "subdirectory of git repo" true (Lib.Autoresearch.is_in_git_repo subdir)
;;

let test_git_top_level_fast_fail_no_git () =
  with_temp_dir "test_fast_fail"
  @@ fun dir ->
  with_eio
  @@ fun ~sw:_ ~clock:_ ->
...
let test_git_head_short_none_for_no_git () =
  with_temp_dir "test_head_no_git"
  @@ fun dir ->
  with_eio
  @@ fun ~sw:_ ~clock:_ ->
  check
    (option string)
    "returns None without git"
    None
    (Lib.Autoresearch.git_head_short ~workdir:dir)
;;

let test_git_is_dirty_false_for_no_git () =
  with_temp_dir "test_dirty_no_git"
  @@ fun dir ->
  with_eio
  @@ fun ~sw:_ ~clock:_ ->
  check
    bool
    "returns false without git"
    false
...
let () =
  run
    "autoresearch_oas_primitives"
    [ ( "metric_contract"
      , [ test_case
            "tagged metric output parses"
            `Quick
