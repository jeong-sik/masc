...
let project_root_of_base_path base_path =
  if Filename.basename base_path = Common.masc_dirname
  then Filename.dirname base_path
  else base_path
;;
...
let warn_if_legacy_verifications_dir_present ~base_path ~active_dir =
  ...
  then (
    ...
    Fun.protect
      ...
      (fun () ->
         if not (Hashtbl.mem warned_legacy_dirs legacy_dir)
         then (
...
let list_request_headers base_path =
  ...
  let report_drop ~reason ~path ~detail =
    Safe_ops.report_persistence_read_drop ~on_drop:ignore ~surface ~reason ~path ~detail
  in
