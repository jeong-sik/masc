...
`specs/keeper-state-machine/KeeperRuntimeRouting.tla` (KCR) 와 `lib/keeper/keeper_runtime_selector.ml` 가 runtime fallback 종료를 *다른 metric* 으로 제한 — 양쪽 모두 *terminate* 하지만 *equivalent* 하지 않음. `docs/tla-audit/kcr-c1-fallback-cap-mechanism-gap-2026-05-12.md` 가 명시.
...
`FallbackCountBounded` invariant 는 production 에서 *우연히* 만족 — *real runtime profile 이 typically MaxFallbacks 보다 적은 group 을 가짐*. 둘 중 하나만 변경 (e.g. `MaxFallbacks = 10`, profile 6 groups) 시 gap 노출.
...
`Keeper_runtime_selector` 에 `fallback_count` 명시:
...
`scripts/lint-kcr-max-fallbacks.sh`: `specs/keeper-state-machine/KeeperRuntimeRouting.cfg` 의 `MaxFallbacks = N` 와 `lib/keeper/keeper_runtime_selector.ml` 의 default value 일치 확인. CI 가 drift 차단.
...
4. **Q4**: P3 default 가 `Int.max_int` 시 *기존 behavior 보존* 인데, 실제 production 에서 fallback_count 의 *현재 max* 측정? Telemetry counter `keeper_runtime_fallback_count_distribution`. **잠정**: P4 의 PBT 외에 production telemetry 추가 — 실제 cap 사이즈 측정 후 P3 default 조정.
...
- **runtime profile schema 변경**: 별도. 본 RFC 는 *기존 profile* 의 fallback 종료 시점만.
...
- [ ] P1: RFC body merge.
- [ ] P2: `Keeper_runtime_selector.try_group` 가 `~max_fallbacks ~fallback_count` typed parameter 도입. Backwards-compat.
