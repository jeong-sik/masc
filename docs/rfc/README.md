...
| 0098 | Typed JSON-RPC error envelope & production-code silent-failure lint | Draft | (this PR) 2026-05-17 | IMPROVE-01 of the masc-mcp + oas improvement series (silent-failure / streaming / TTFT / FD / stability). Renumbered from 0094/0097 after ledger race-loss against #15716 / #15728 / #15722 / #15725 and the merged RFC-0097 sandbox-container spec. `server_mcp_transport_http_respond.ml` 의 4 hand-rolled JSON-RPC error site (-32001/-32002/-32603×2, literal integer 산재) → `-32603` 단일 코드가 provider timeout / tool failure / serialization bug 흡수. 신규 `lib/server/mcp_error_code.ml` closed-sum SSOT + 단일 `respond_mcp_error` factory. `scripts/anti-fake-audit.sh` 를 `--production-scan` 모드로 확장 — lib/ 베이스라인: `ignore (Error _)` 0 site (hold), `\| Error _ -> ()` 13 site (grandfather), `try ... with _ -> ()` 15 site (grandfather). `Quiet { reason ; recovered }` variant 로 last-resort silent-skip 을 *명시 선언*해야 통과. PR-1 byte-equal wire. PR-3+ 신규 코드 (`-32003 Provider_timeout`, `-32004 Tool_dispatch_failure`, `-32005 Backpressure_shed`). [[RFC-0077]] write-side / [[RFC-0088]] umbrella / [[RFC-0089]] string-class 와 직교. Related RFC-0077, RFC-0088, RFC-0089, RFC-0090, RFC-0062, RFC-0042 |
| 0099 | Session lifecycle — typed events, explicit eviction, resume backpressure | Draft | (this PR) 2026-05-17 | IMPROVE-05 of the masc-mcp + oas improvement series. SSE/WS/gRPC/WebRTC 4 transport의 session lifecycle 통일. 3 결함: (1) SSE 200-client cap eviction이 close frame 없이 silent drop, (2) mailbox-full drop policy 미문서 미테스트, (3) keep-alive interval이 transport마다 산재. 해결: `lib/server/session_lifecycle_event.ml(i)` closed-sum 5-variant (Open/Upgrade/Resume/Evict/Close) typed event를 `Event_bus.Custom("session_lifecycle", ...)` 발행 + `evict_reason` 4-variant에 대응하는 explicit close frame (SSE `event: evicted`, WS close code 4001-4099, gRPC `Status=ABORTED` trailer, WebRTC datachannel close) + mailbox-full → 1s drain grace → backpressure evict + 4 env knob SSOT (`MASC_TRANSPORT_KEEPALIVE_SEC` 등). 6-PR migration (PR-2 inert event publish → PR-3 SSE close frames → PR-4 WS/gRPC/WebRTC fan-out → PR-5 keep-alive SSOT + CI lint → PR-6 Last-Event-ID resume). [[RFC-0098]] sibling — RFC-0098이 response edge, 0099가 transport edge. `docs/TIMEOUT-MATRIX.md`와 보완 관계 (timeout layer는 inner Tool/MCP/OAS chain, 본 RFC는 transport edge). Related RFC-0098 |
...
| 0101 | FD accountant — generic Eio.Pool extension to cover all spawn classes | Draft | (this PR) 2026-05-17 | IMPROVE-03 of the masc-mcp + oas improvement series. PR #15727 `Docker_spawn_throttle` 는 docker spawn 한 class만 cap — provider HTTP (per-call TCP+TLS), sandbox exec (popen pipes), log writer (file FDs)는 같은 `kern.maxfiles` ceiling 공유하나 회계 없음. 신규 `lib/server/fd_accountant.ml(i)` 4-kind generic `Eio.Pool` (Docker_spawn / Provider_http / Sandbox_exec / Log_writer) + per-kind env knob cap (defaults 8/16/32/64=120 합) + `/metrics` Prometheus 노출 (`masc_fd_open`, `masc_fd_in_flight{kind=...}`, `masc_fd_pressure_active`) + 시작 시 nofile rlimit 로그 + RFC-0099 Backpressure evict 결합. `Docker_spawn_throttle.with_slot`은 `Fd_accountant.with_slot ~kind:Docker_spawn`로 delegate (public API preserved, RFC-0098 PR-2 패턴 재사용). 6-PR migration (PR-2 module+delegation inert → PR-3 provider HTTP wrap → PR-4 sandbox+log wrap → PR-5 /metrics+dashboard → PR-6 RFC-0099 compose). Related #15727, #13642, RFC-0097, RFC-0098, RFC-0099, RFC-0100 |
...
| 0102 | Pre-turn cascade availability gate | Draft | (this PR) 2026-05-17 | Closes the gap between RFC-0009 (pre-attempt ordering) and RFC-0022 (in-attempt liveness) with a fourth *pre-turn* layer: `Phase_gating` consults a cached `Cascade_health_filter.cached_availability` snapshot before emitting `Phase_gating → Cascade_routing`. When no provider is healthy, transitions `Phase_gating → Done(Skipped)` with `terminal_reason_code=cascade_unavailable` and emits **no** `operator_broadcast_required`. Removes the N-keepers × M-turns/min WARN flood observed 2026-05-17 (memory: `project_cascade_tier_group_misroute_2026_05_17.md`) while preserving every broadcast that carries independent information. Insertion site: `lib/keeper/keeper_unified_turn.ml:256-277` executable-phase arm. RFC-0088 (Counter-as-Fix) self-check §6 — change is monotone, no new counter/string-classifier/N-of-M/cap. Related RFC-0009, RFC-0012, RFC-0022, RFC-0072, RFC-0088 |
