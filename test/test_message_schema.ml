...
let test_task_update_no_payload () =
  let msg = MS.TaskUpdate {
    task_id = "task-002"; status = "in_progress"; payload = None;
  } in
  match MS.roundtrip msg with
  | Ok result ->
...
let test_status_report_roundtrip () =
  let msg = MS.StatusReport {
    agent = "claude"; progress = 0.75; details = "Processing step 3 of 4";
  } in
  match MS.roundtrip msg with
  | Ok result ->
...
let test_response_roundtrip () =
  let msg = MS.Response {
    request_id = "req-123"; success = true; result = `String "LGTM";
  } in
  match MS.roundtrip msg with
  | Ok result ->
...
let test_is_targeted_at () =
  let msg = MS.Request {
    target = "codex"; action = "review"; params = `Null;
  } in
...
let test_of_json_unknown_type () =
  ...
  | Error msg ->
      Alcotest.(check bool) "mentions unknown type" true (String.length msg > 0)
