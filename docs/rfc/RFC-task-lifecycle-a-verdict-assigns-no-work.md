...
| 묻는 것 | 코드에서 | 써야 하는 쪽 |
|---|---|---|
| 이 Task 가 아직 필요한가 | `Done`·`Cancelled` 인가 아닌가 | 판정(끝났다고 닫는다), 끝나지 않은 상태가 이름을 들고 있으면 그 에이전트와 운영자(취소한다) |
...
이 RFC 는 사실마다 쓰는 쪽을 정한다. 먼저 `Rejected` 를 더해 반려와 `Todo` 를
가르고 전이 네 개를 바꾼다. 마지막 hard-cut 단계에서 현재 0건인 `Claimed` 와
`Start` 를 함께 지운다(D6). 따라서 구현 도중에는 상태가 일곱 개지만 최종 상태
어휘는 다시 여섯 개다.
...
| 3 | `Todo` 의 `Cancel` 은 누구나 즉시 된다 | 끝나지 않은 상태가 이름을 들고 있으면 그 이름의 에이전트와 운영자, `Todo` 면 운영자만 즉시 취소한다. 끝난 상태는 누구도 취소하지 않는다 |
| 4 | 제출에 `intent = Complete_task \| Cancel_task` 가 있다 | `intent` 를 지운다. 판정을 기다리는 것은 완료 제출뿐이다 |

첫 단계에서 더하는 상태는 `Rejected` 하나다(소유자 결정 D1). `Todo` 에 선택 필드를
더하지 않는 이유는 둘의 전이가 다르기 때문이다. `Todo` 는 `claim_next` 가 권하지만
`Rejected` 는 id나 운영자 목록으로만 맡는다. 생성자를 가르면 새 전이를 추가할 때
컴파일러가 둘을 따로 묻게 된다. 마지막 단계에서는 `Claimed` 하나를 지우므로 최종
상태 수는 늘지 않는다. 타이머·Gate·액션 이름은 더하지 않고, 지워지는 것이 더
많다(§3.1, §3.2, §3.9, D6).
...
- `Rejected` 는 열려 있고 아무도 안 맡은 자리다. `Todo` 와 다른 점은 누가 냈던 것인지를 Task 자신이
  ...
  적지 않기 위해서다.
- **이름이 셋이었다.** 같은 에이전트를 Task 상태와 backlog.json 은 `assignee`, 판정 쪽 코드는
  `producer`, 판정자에게 넘기는 레코드는 `worker` 라고 부른다. 셋째는 코드에 그대로 있다.
  `verification_protocol.ml:242`·`:271` 이 `("worker", assignee)` 로 쓴다. `assignee` 라는 값을 꺼내
  `worker` 라는 키로 적는 것이라, 이름 바꾸기가 이미 그 경계에서 일어나고 있다. D5 로 상태 쪽이
  `producer` 가 되면 셋이 둘로 준다. `assignee` 는 "지금 맡은 쪽" 한 뜻만 갖는다.
...
`cancel` 은 §3.5의 상태별 자격이 있는 쪽만 한다. 판정에서 나가는 화살표는 `Done` 과 `Rejected` 둘뿐이고, 어느
...
| `<task>` 규칙 추가 | — | "끝나지 않은 Task 는 그 상태가 이름을 들고 있으면 그 에이전트와 인증된 운영자만 취소한다. `Todo` 는 운영자만, `Done`·`Cancelled` 는 누구도 취소하지 않는다. 판정을 거치지 않는다" |
...
### 3.12 RFC-0417과의 병합 순서

소유자가 개정하기로 했고(2026-09-19), #37112가 RFC-0417 본문을 현재 규칙으로
고쳤다. #37112가 먼저 병합되어야 한다. 이 PR을 먼저 병합하면 main의 두 RFC가 서로
반대로 말하는 창이 생긴다.

개정의 논지는 하나다. 남의 일을 없애는 권한은 운영자 몫이고, 자기 일을 접는 데는
승인이 필요 없다. #37112는 그 규칙을 RFC-0417의 제목·요약·상태별 권한표·hard cut에
직접 반영하고, 과거 운영자 승인 대기 계약은 본문에서 삭제한다. 이 RFC는 Task 생애주기
전체와 모델 증명을 소유한다.
...
| 0417 | **#37112가 개정한다(§3.12).** 남의 일을 없애는 권한은 운영자 몫이고, 자기 일을 접는 데는 승인이 필요 없다. 이 PR보다 먼저 병합한다 |
...
| D2 | Task 를 취소할 자격 | **정해짐(2026-09-19): 끝나지 않은 상태가 이름을 들고 있으면 그 에이전트와 인증된 운영자. `Todo` 는 운영자만, 끝난 상태는 누구도 아니다.** `created_by` 문자열은 자격에서 뺀다. 판정 LLM 에이전트에게는 취소 권한을 주지 않는다. 운영자 쪽은 배치로 한다(§3.5, §3.11). RFC-0417 개정이 따라온다(§3.12) | — |
| D3 | 대기 중인 취소 요청 65건 | **정해짐(2026-09-19): 운영자 일괄 승인, 65건 모두 `Cancelled`** | — |
| D4 | §3.10 헌법 문구. `Done.assignee` 의 뜻 포함 | **정해짐(2026-09-19): 표대로.** D2가 정해지면서 취소 자격은 §3.5의 상태별 닫힌 표로 확정됐다 | — |
| D5 | 판정을 기다리는 줄의 `assignee` 를 저장 형식에서도 `producer` 로 바꾸는가 | **정해짐(2026-09-19): 바꾼다.** 한 에이전트를 세 이름으로 부르던 것이 둘이 된다(§1.7). 배포 전 조건인 "대기 줄 0" 은 정하는 시점에 이미 충족돼 있었다 | — |
| D6 | `Claimed` 를 없애고 claim 이 곧바로 `InProgress` 를 만들게 하는가. `Start` 도 같이 없어진다 | **정해짐(2026-09-19): 없앤다. 4단계로.** 허용 여부를 가르는 곳이 `Start` 하나이고, Keeper 는 이미 claim 직후 Start 를 보내며, 지금 `claimed` 줄이 0건이라 저장 형식 비용이 없다 | 둔다. 화면의 "claimed but not in-progress" 집계(`dashboard.ml:454-461`), `masc_status` 집계, 프롬프트 문구, `test_task_status_vocabulary` 를 고치는 일이 든다 |
