...
let execute_downscale ~plans ~max_dim ~media_type ~bytes =
  ...
  Fun.protect
    ~finally:(fun () ->
      ...
      (try Sys.remove in_file with _ -> ()) (* cancel-guard-ok: Sys.remove is not an Eio operation *);
      (try Sys.remove out_file with _ -> ()) (* cancel-guard-ok: Sys.remove is not an Eio operation *))
    (fun () ->
