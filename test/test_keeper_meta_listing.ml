...
let write_keeper_toml_exn ?autoboot_enabled config ~name =
  let keepers_dir = Filename.concat (Coord.masc_root_dir config) "config/keepers" in
  let autoboot_line =
...
let write_persona_profile_exn config ~name =
  let persona_dir =
    Filename.concat (Filename.concat (Coord.masc_root_dir config) "config/personas") name
  in
  ...
  write_json
    ...
    (`Assoc
        ...
        ; ( "keeper"
          , `Assoc
              [ "goal", `String "test persona keeper"; "tool_preset", `String "research" ]
          )
        ])
;;
...
let register_keeper_offline_exn config ~name =
  ...
  | Ok (Some meta) ->
    ignore (Keeper_registry.register_offline ~base_path:config.base_path name meta)
...
let test_keeper_listing_ignores_sidecar_json_files () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check (list string) "keeper_names filters sidecars" [ "dot.name"; "sangsu" ] names;
       let keepalive_names = Keeper_types.keepalive_keeper_names config in
       check
         (list string)
         "keepalive_keeper_names filters sidecars"
         [ "dot.name"; "sangsu" ]
         keepalive_names;
       ...
       check
         (list string)
         "status handler filters sidecars"
         [ "dot.name"; "sangsu" ]
         listed;
       check
         int
         "status handler count filters sidecars"
         2
...
let test_bootable_keeper_names_skip_autoboot_disabled_meta () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         bool
         "autoboot disabled sangsu excluded from bootable list"
         false
...
let test_declarative_autoboot_disabled_skips_boot_without_meta () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         bool
         "bootable list excludes declarative autoboot-disabled keeper"
         false
         (List.mem "sangsu" bootable_names);
       let keepalive_names = Keeper_types.keepalive_keeper_names config in
       check
         bool
         "keepalive list excludes declarative autoboot-disabled keeper"
         false
...
let test_autoboot_policy_resync_from_declarative_toml () =
  ...
  "big_three_models": ["test-only:model"]
       ...
       | Ok updated ->
         check
           bool
           "autoboot_enabled resynced from TOML"
           false
...
let test_keeper_up_uses_toml_autoboot_default () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Ok (Some meta) ->
         check bool "autoboot_enabled defaulted from TOML" false meta.autoboot_enabled
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       | Some keeper ->
         check
           string
           "social_model normalized"
           "bdi_speech_v1"
           Yojson.Safe.Util.(keeper |> member "social_model" |> to_string);
         check
           string
           "configured_social_model preserved"
           "experimental_v99"
         ...
         check
           string
           "social_model_fallback explicit"
           "bdi_speech_v1"
...
let test_keeper_persona_audit_reports_durable_live_persona_keeper () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       write_keeper_persona_toml_exn config ~name:"analyst" ~persona_name:"analyst";
       ...
       (match Keeper_types.read_meta config "analyst" with
        | Ok (Some meta) ->
          ignore (Keeper_registry.register ~base_path:config.base_path "analyst" meta)
       ...
       | Some item ->
         check bool "item ok" true Yojson.Safe.Util.(item |> member "ok" |> to_bool);
         check
           string
           "default source"
           "toml"
         ...
         check
           ...
           "keeper toml exists"
           true
           Yojson.Safe.Util.(item |> member "keeper_toml" |> member "exists" |> to_bool);
         check
           bool
           "persona profile exists"
           true
           Yojson.Safe.Util.(
         ...
         check
           ...
           "runtime meta exists"
           true
           Yojson.Safe.Util.(item |> member "runtime_meta" |> member "exists" |> to_bool);
         check
           bool
           "registry present"
           true
...
let test_keeper_persona_audit_flags_missing_persona_runtime () =
  ...
  Fun.protect
    ...
    (fun () ->
       let config = Coord.default_config base_dir in
       ignore (Coord.init config ~agent_name:(Some "operator"));
       write_keeper_persona_toml_exn config ~name:"ghost" ~persona_name:"missing-persona";
       let config_root = Filename.concat (Coord.masc_root_dir config) "config" in
       Unix.putenv "MASC_CONFIG_DIR" config_root;
       Config_dir_resolver.reset ();
       let ctx = keeper_ctx env sw config "operator" in
       ...
       check
         int
         "missing persona count"
         1
         Yojson.Safe.Util.(
           json |> member "summary" |> member "missing_persona_profile" |> to_int);
       check
         int
         "missing runtime count"
         1
         Yojson.Safe.Util.(
       ...
       | Some item ->
         let issues = Yojson.Safe.Util.(item |> member "issues") |> string_list_of_json in
         check
           bool
           "flags missing persona"
           true
           (List.mem "missing_persona_profile" issues);
         check bool "flags missing runtime" true (List.mem "missing_runtime_meta" issues);
         check
           bool
           "keeper toml exists"
           true
         ...
         check
           string
           "persona profile candidate path surfaced"
           (Filename.concat
              (Filename.concat (Filename.concat config_root "personas") "missing-persona")
              "profile.json")
           Yojson.Safe.Util.(
...
let test_keeper_persona_audit_flags_runtime_meta_parse_error () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         int
         "runtime meta parse error count"
         1
         Yojson.Safe.Util.(
           json |> member "summary" |> member "runtime_meta_error" |> to_int);
       check
         int
         "runtime meta file is not missing"
         0
         Yojson.Safe.Util.(
           json |> member "summary" |> member "missing_runtime_meta" |> to_int);
       ...
       | Some item ->
         let issues = Yojson.Safe.Util.(item |> member "issues") |> string_list_of_json in
         check bool "flags runtime meta error" true (List.mem "runtime_meta_error" issues);
         check bool "item not ok" false Yojson.Safe.Util.(item |> member "ok" |> to_bool);
         check
           bool
           "runtime meta file exists"
           true
           Yojson.Safe.Util.(item |> member "runtime_meta" |> member "exists" |> to_bool);
         check
           bool
           "runtime meta error surfaced"
           true
           (Yojson.Safe.Util.(
...
let () =
  run
    ...
    [ ( "listing"
      , [ test_case
            "keeper_names and keeper_list ignore sidecar json"
            `Quick
            test_keeper_listing_ignores_sidecar_json_files
        ; test_case
            "bootable list skips autoboot-disabled meta"
            `Quick
            test_bootable_keeper_names_skip_autoboot_disabled_meta
        ; test_case
            "declarative autoboot-disabled keeper skips boot without meta"
            `Quick
            test_declarative_autoboot_disabled_skips_boot_without_meta
        ; test_case
            "autoboot policy resyncs from declarative TOML"
            `Quick
            test_autoboot_policy_resync_from_declarative_toml
        ; test_case
            "keeper_up uses TOML autoboot default"
            `Quick
            test_keeper_up_uses_toml_autoboot_default
        ; test_case
            "tool keeper list normalizes unknown social model"
            `Quick
            test_keeper_list_normalizes_unknown_social_model
        ; test_case
            "tool keeper list preserves known social model"
            `Quick
            test_keeper_list_preserves_known_social_model
        ; test_case
            "tool keeper list exposes last social transition reason"
            `Quick
            test_keeper_list_exposes_last_social_transition_reason
        ; test_case
            "keeper persona audit reports durable live keeper"
            `Quick
            test_keeper_persona_audit_reports_durable_live_persona_keeper
        ; test_case
            "keeper persona audit flags missing persona runtime"
            `Quick
            test_keeper_persona_audit_flags_missing_persona_runtime
        ; test_case
            "keeper persona audit flags runtime meta parse error"
            `Quick
