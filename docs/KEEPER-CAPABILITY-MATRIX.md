...
Notes:
- The old governance petition/case subsystem is retired. Collaboration uses Board and Task, while external effects use the product-neutral Gate and its approval audit.
...
- operator/maintenance keeper controls: broader descriptors such as
  ...
  `masc_keeper_down`, and `masc_keeper_up`
...
| Workspace planning, run logs, notes, deliverables | `masc_plan_*`, `masc_run_*`, `masc_note_add`, `masc_deliver` | Mutating plans for ordinary status narration |
...
DirectMention, NewUnclaimedTask, FailedTask, KeeperFiberStartedOrStopped,
BoardActivity, IdleTimeout, MetricsAnomaly, StrategicReview, SelfDirectedExplore.
