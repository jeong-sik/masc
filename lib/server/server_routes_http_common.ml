...
let parse_host_port host_header default_host default_port =
  ...
  | Some host_value ->
    ...
    then default_host, default_port
    else (
      try
...
let accepts_streamable_mcp (request : Httpun.Request.t) =
  Http_negotiation.accepts_streamable_mcp (Httpun.Headers.get request.headers "accept")
;;

let request_force_json_response = Server_mcp_transport_http.request_force_json_response
...
let mcp_transport_http_deps () : Server_mcp_transport_http.deps =
  let mcp_eio_profile_of_transport_profile = function
    ...
    | Server_mcp_transport_http.Operator_remote -> Mcp_server_eio.Operator_remote
  in
  { get_origin
  ; cors_headers
...
let close_all_sse_connections = Server_mcp_transport_http.close_all_sse_connections

let handle_get_mcp
      ?legacy_messages_endpoint
      ?(profile = Server_mcp_transport_http.Full)
      ?sse_kind
      request
      reqd
  =
  Server_mcp_transport_http.handle_get_mcp
    ~deps:(mcp_transport_http_deps ())
    ?legacy_messages_endpoint
    ~profile
    ?sse_kind
...
let sse_simple_handler request reqd =
  Server_mcp_transport_http.sse_simple_handler
    ~deps:(mcp_transport_http_deps ())
    request
    reqd
;;
...
let handle_post_messages request reqd =
  Server_mcp_transport_http.handle_post_messages
    ~deps:(mcp_transport_http_deps ())
    request
    reqd
;;
...
let handle_post_mcp ?(profile = Server_mcp_transport_http.Full) request reqd =
  Server_mcp_transport_http.handle_post_mcp
    ~deps:(mcp_transport_http_deps ())
    ~profile
    request
    reqd
...
let handle_delete_mcp ?(profile = Server_mcp_transport_http.Full) request reqd =
  Server_mcp_transport_http.handle_delete_mcp
    ~deps:(mcp_transport_http_deps ())
    ~profile
    request
    reqd
...
let handle_ag_ui_events request reqd =
  Server_mcp_transport_http.handle_ag_ui_events
    ~deps:(mcp_transport_http_deps ())
    request
    reqd
;;
