...
let mcp_profile_by_session : Server_mcp_transport_http_types.tool_profile SMap.t Atomic.t =
...
let is_valid_protocol_version version = List.mem version mcp_protocol_versions
...
let reap_stale_sessions ~is_active_session =
  let stale =
    SMap.fold
      (fun sid _ acc -> if not (is_active_session sid) then sid :: acc else acc)
     ...
     | None ->
       Error
         (Printf.sprintf
            ...
            session_id
            (profile_label profile)))
  | Server_mcp_transport_http_types.Full | Server_mcp_transport_http_types.Managed_agent
...
let capitalize_ascii (s : string) =
  ...
  then s
    ...
    let rest =
      ...
      then String.sub s 1 (String.length s - 1) |> String.lowercase_ascii
      else ""
    in
    first ^ rest)
;;
...
let get_cookie_value (request : Httpun.Request.t) cookie_name =
  ...
  | Some raw ->
    ...
    |> List.find_map (fun part ->
      ...
      | key :: value_parts
        when String.lowercase_ascii (String.trim key) = String.lowercase_ascii cookie_name
        ->
  ...
  | Some host ->
    let proto =
      ...
      | None ->
        ...
        if String.starts_with ~prefix:"masc.crying.pict" host then "https" else "http"
    in
  ...
  | Some expected ->
    (match provided with
     ...
     | Some version ->
       ...
       if String.equal version expected
       then Ok ()
       else
         Error
           (Printf.sprintf
