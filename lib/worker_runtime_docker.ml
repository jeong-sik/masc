...
let host_is_linux () = Sys.os_type = "Unix" && Sys.file_exists "/proc/version"

let rewrite_loopback_url value =
  let uri = Uri.of_string value in
  if Masc_network_defaults.is_loopback_host_opt (Uri.host uri)
  then Uri.with_host uri (Some docker_host_alias) |> Uri.to_string
  else value
;;
...
let docker_llama_server_url () = rewrite_loopback_url Env_config.Local_runtime.server_url

let rewrite_model_label_for_container model_label =
  if String.starts_with ~prefix:"custom:" model_label
  then (
    ...
    | Some cfg ->
      ...
      if String.equal rewritten cfg.Llm_provider.Provider_config.base_url
      then model_label
      else
        Printf.sprintf "custom:%s@%s" cfg.Llm_provider.Provider_config.model_id rewritten
    | None -> model_label)
  else model_label
;;
...
let allowlisted_env_pairs () =
  ...
  in
  let inherited =
    inherited |> List.filter (fun (key, _) -> not (List.mem key cleared_keys))
  in
  let overrides =
    overrides
    |> List.filter_map (fun (key, value) ->
...
let container_name (spec : Worker_execution_spec.t) =
  let token =
    ...
    | Some worker_run_id when String.trim worker_run_id <> "" -> worker_run_id
    | _ -> spec.worker_name
  in
  let unique_suffix =
    Printf.sprintf
...
let artifact_dir (spec : Worker_execution_spec.t) =
  Worker_container.worker_container_dir
...
let stderr_artifact_path (spec : Worker_execution_spec.t) =
  let suffix =
    ...
    | _ -> "latest"
  in
  Filename.concat (artifact_dir spec) (Printf.sprintf "docker-stderr-%s.log" suffix)
;;

let persist_stderr_artifact (spec : Worker_execution_spec.t) stderr =
  if String.trim stderr <> ""
  then (
    Worker_container.ensure_worker_container_dirs
      ~base_path:spec.base_path
      ~worker_name:spec.worker_name;
...
let mount_args (spec : Worker_execution_spec.t) =
  ...
  let config_mount =
    ...
    if path_within ~root:spec.base_path config_root
    then []
        ...
        let rec check_auth = function
          ...
          | subject :: rest ->
            (match auth_requirements_of_model_label subject.model_label with
             | Error msg -> Error (Printf.sprintf "%s (%s)" msg subject.worker_name)
             | Ok keys ->
  ...
  let env_flags =
    ...
    |> List.filter_map (fun pair ->
      ...
      | Some idx ->
        ...
        if String.trim value = "" then None else Some [ "-e"; key ^ "=" ^ value ]
      | None -> None)
    |> List.flatten
  in
  let linux_host_flags =
    if host_is_linux () then [ "--add-host"; docker_host_alias ^ ":host-gateway" ] else []
  in
  [ "docker"
  ; "run"
  ; "--rm"
       ...
       | 0 ->
         (match Worker_runtime_helper_protocol.parse_stdout result.stdout with
          ...
          | Ok (Error payload) ->
            Error
              (Printf.sprintf
                 "Docker worker runtime error (%s): %s"
                 (Worker_runtime_helper_protocol.error_kind_to_string payload.kind)
                 payload.message)
          | Error msg ->
            Error
              (Printf.sprintf
                 ...
                 msg
                 (if String.trim result.stderr = ""
                  then ""
                  else "\n[stderr]\n" ^ tail_text result.stderr)))
       | 124 ->
         Error
           (Printf.sprintf
              "Docker worker timed out after %ds%s"
       ...
       | _ ->
         ...
         (match Worker_runtime_helper_protocol.parse_stdout result.stdout with
          ...
          | Error _ ->
            Error
              (Printf.sprintf
                 "docker run exited with code %d%s"
                 result.exit_code
                 stderr_tail)))
;;
