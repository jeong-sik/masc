...
let test_config_uses_live_catalog () =
  with_temp_config_root
    ...
    (fun cascade_path ->
       ...
       check
         bool
         "includes profiles declared by non-model schema keys"
         true
...
let test_raw_config_shape () =
  with_temp_config_root
    ...
    (fun cascade_path ->
       ...
       check
         bool
         "json raw is editable"
         true
         Yojson.Safe.Util.(j |> member "raw_json_editable" |> to_bool);
       check
         bool
         "source_text includes current file contents"
         true
         (contains_substring source_text "big_three_models");
       check
         bool
         "raw_json includes current file contents"
         true
...
let test_raw_config_defaults_when_file_missing () =
  with_temp_config_root "{}\n" (fun cascade_path ->
    ...
    check
      (option string)
      "config_path still known"
      (Some cascade_path)
      Yojson.Safe.Util.(j |> member "config_path" |> to_string_option);
    check
      bool
      "still editable in json mode"
      true
      Yojson.Safe.Util.(j |> member "raw_json_editable" |> to_bool);
    check
      string
...
|}
  ...
  with_temp_config_root_setup
    ...
    (fun dir ->
       ...
       check
         (option string)
         "runtime json path still surfaced"
         (Some json_path)
         Yojson.Safe.Util.(j |> member "config_path" |> to_string_option);
       check
         string
       ...
       check
         bool
         "generated json preview stays read-only"
         false
         Yojson.Safe.Util.(j |> member "raw_json_editable" |> to_bool);
       check
         bool
       ...
       check
         ...
         (contains_substring
            Yojson.Safe.Util.(j |> member "raw_json" |> to_string)
            "big_three_models");
       check bool "runtime json materialized on read" true (Sys.file_exists json_path))
;;
  ...
  with_temp_config_root
    ...
    (fun cascade_path ->
       ...
       | Ok saved_config ->
         ...
         check string "file persisted verbatim" next_raw (read_file cascade_path))
;;
  ...
  run
    ...
    ; ( "raw_config_json"
      , [ test_case "top-level shape" `Quick test_raw_config_shape
        ...
        ; test_case
            "toml source save persists + materializes json"
            `Quick
