...
let () =
  ...
  run
    ...
    [ ( "hidden_tool_leak"
      , [ test_case
            "no explicitly-hidden tools in spawned_agent_public_tool_names"
            `Quick
            (fun () ->
               ...
               let explicit_hidden =
                 List.filter_map
                   (fun (name, (meta : Tool_catalog.metadata)) ->
    ...
    ; ( "passthrough_dead_entries"
      , [ test_case "all passthrough names exist in visible schemas" `Quick (fun () ->
            let all_schemas =
            ...
            in
            let schema_names =
              List.map (fun (s : Types.tool_schema) -> s.name) all_schemas
            in
            let names = Agent_tool_surfaces.spawned_agent_public_tool_names in
            let dead = List.filter (fun name -> not (List.mem name schema_names)) names in
            ...
            then
              Alcotest.failf
                ...
                (String.concat ", " dead))
        ; test_case "agent passthrough omits operator and diagnostics" `Quick (fun () ->
            let names = Agent_tool_surfaces.spawned_agent_public_tool_names in
            let banned =
              [ "masc_tool_stats"
            ...
            List.iter
              (fun name ->
                 check
                   bool
                   (name ^ " omitted from public agent surface")
                   false
                   (List.mem name names))
              banned)
        ] )
    ; ( "annotation_overrides"
      , [ test_case "masc_run_get has readonly override" `Quick (fun () ->
        ...
        ; test_case "masc_operation_stop has destructive override" `Quick (fun () ->
            let meta = Tool_catalog.metadata "masc_operation_stop" in
            check (option bool) "destructive" (Some true) meta.destructive)
        ; test_case "masc_operation_pause is not destructive" `Quick (fun () ->
    ...
    ; ( "hidden_tools_contract"
      , [ test_case
            "keeper internal tools are hidden and not directly callable"
            `Quick
            (fun () ->
               List.iter
                 (fun name ->
                    let meta = Tool_catalog.metadata name in
                    check bool (name ^ " hidden") false (Tool_catalog.is_visible name);
                    check
                      bool
                      (name ^ " direct call blocked")
                      false
                      (Tool_catalog.allow_direct_call name);
                    check
                      bool
                      (name ^ " on keeper_internal surface")
                      true
        ...
        ; test_case "known hidden tools are not visible by default" `Quick (fun () ->
            let hidden_names = [ "masc_operator_judgment_write"; "masc_set_room" ] in
            List.iter
              (fun name ->
                 check
                   bool
              ...
              hidden_names)
        ; test_case "hidden tools are visible with include_hidden" `Quick (fun () ->
            let hidden_names = [ "masc_operator_judgment_write"; "masc_set_room" ] in
            List.iter
              (fun name ->
                 check
                   bool
                   (name ^ " should be visible with flag")
                   true
                   (Tool_catalog.is_visible ~include_hidden:true name))
              hidden_names)
        ; test_case
            "legacy mitosis tools are removed from the public registry"
            `Quick
            (fun () ->
               let removed_names =
                 [ "masc_mitosis_status"
               ...
               List.iter
                 ...
                 removed_names)
        ; test_case
            "removed named-room tools are absent from the schema registry"
            `Quick
            (fun () ->
               let all_names = Config.all_tool_names () in
               List.iter
    ...
    ; ( "description_budget_audit"
      , [ test_case "no single tool description exceeds 2000 chars" `Quick (fun () ->
            ...
            let violations =
              List.filter_map
                (fun (s : Types.tool_schema) ->
                   ...
                   if len > max_chars
                   then Some (Printf.sprintf "%s: %d chars" s.name len)
                   else None)
                schemas
            in
            if violations <> []
        ...
        ; test_case "public MCP surface total under 60K chars" `Quick (fun () ->
            ...
            let total =
              List.fold_left
                (fun acc (s : Types.tool_schema) -> acc + String.length s.description)
                0
                schemas
            in
            if total > max_total
    ...
    ; ( "public_mcp_surface"
      , [ test_case "all public_mcp_tools exist in schema registry" `Quick (fun () ->
            let all_names = Config.all_tool_names () in
            let missing =
              List.filter
        ...
        ; test_case "public surface count is between 30 and 80" `Quick (fun () ->
            let count = List.length Tool_catalog.public_mcp_tools in
            check bool "count is within expected range" true (count >= 30 && count <= 80))
        ; test_case "is_public_mcp returns true for listed tools" `Quick (fun () ->
            List.iter
              (fun name ->
                 check bool (name ^ " is public") true (Tool_catalog.is_public_mcp name))
              Tool_catalog.public_mcp_tools)
        ; test_case "internal tools are not public" `Quick (fun () ->
            let internal =
              [ "masc_code_search"
            ...
            List.iter
              (fun name ->
                 check bool (name ^ " not public") false (Tool_catalog.is_public_mcp name))
              internal)
        ; test_case "non-public tool remains in full registry" `Quick (fun () ->
            let all_names =
            ...
            check bool (internal ^ " in registry") true (List.mem internal all_names);
            check
              bool
              (internal ^ " not public")
              false
