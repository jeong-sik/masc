...
let test_domain_failure_kind_survives_failed_cli_slot () =
  ...
  let runner ~runtime_id:_ ~system_prompt:_ ~output_schema:_ ~prompt:_ =
    incr attempts;
    Error (Masc.Fusion_official_client.Setup_failure (Masc.Fusion_types.Provider_error "synthetic bridge failure"))
  in
  match execute ~net ~clock ~base_path ~runner with
      ...
      ~cli_failure:
        (Cli.Execution_failed
           ...
           ; cause = Masc.Fusion_official_client.Setup_failure
               (Masc.Fusion_types.Provider_error "synthetic bridge failure")
           })
      (Runtime.For_testing.classified_error_detail error)
;;
...
let test_cli_prompt_drift_is_not_reported_as_no_cli_declaration () =
  ...
  let runner ~runtime_id:_ ~system_prompt:_ ~output_schema:_ ~prompt:_ =
    incr calls;
    Error (Masc.Fusion_official_client.Setup_failure (Masc.Fusion_types.Provider_error "must not run"))
  in
  match
    Runtime.For_testing.execute_exact_output_classified
      ~cli_runner:runner
      ...
      ; test_case
        ...
        ; test_case "no CLI declaration preserves the API failure" `Quick
            (test_failure_reaches_journal ~cli_only:false ~cli_slot_ids:[]
              ~answer:(Error (Masc.Fusion_official_client.Setup_failure (Masc.Fusion_types.Provider_error "must not run"))) ~failure:None
        ...
        ; test_case "CLI admission refusal reaches journal and exact-run projection" `Quick
            (test_failure_reaches_journal ~cli_only:false
              ~cli_slot_ids:["missing-cli-runtime"] ~answer:(Error (Masc.Fusion_official_client.Setup_failure (Masc.Fusion_types.Provider_error "must not run")))
        ...
        ; test_case "CLI execution failure reaches journal and exact-run projection" `Quick
            (test_failure_reaches_journal ~cli_only:false
              ~cli_slot_ids:[Fixture.cli_primary_runtime] ~answer:(Error (Masc.Fusion_official_client.Setup_failure (Masc.Fusion_types.Provider_error "synthetic bridge failure")))
