...
let () =
  test "create returns not-alive engine" (fun () ->
    Eio_main.run
    @@ fun env ->
...
let () =
  test "run fires startup demand beat and can shutdown" (fun () ->
    Eio_main.run
    @@ fun env ->
    let clock = Eio.Stdenv.clock env in
    let beats = ref [] in
    ...
    if n < 2 then failwith (Printf.sprintf "expected >=2 beats, got %d" n);
...
let () =
  test "nudge triggers immediate beat" (fun () ->
    Eio_main.run
    @@ fun env ->
    let clock = Eio.Stdenv.clock env in
    let triggers = ref [] in
    ...
    (* let startup beat fire *)
    Pulse.nudge t ~reason:"test-nudge";
    Eio.Time.sleep clock 0.15;
    (* let nudge beat process *)
    Pulse.shutdown t;
    Eio.Time.sleep clock 0.15;
    let has_nudge =
      List.exists
        ...
        !triggers
    in
    if not has_nudge then failwith "expected a Nudge(test-nudge) trigger in beats")
;;

(* ── Test: bounded lifecycle stops engine ──────────────────── *)

let () =
  test "bounded lifecycle stops after predicate" (fun () ->
    Eio_main.run
    @@ fun env ->
    let clock = Eio.Stdenv.clock env in
    ...
    if s.total_beats < 3
    then failwith (Printf.sprintf "expected >=3 beats, got %d" s.total_beats))
;;

(* ── Test: consumer error doesn't crash pulse ─────────────── *)

let () =
  test "consumer error doesn't crash pulse" (fun () ->
    Eio_main.run
    @@ fun env ->
    let clock = Eio.Stdenv.clock env in
    ...
    then
      failwith
        (Printf.sprintf
           "good consumer should have been called >=2 times, got %d"
           !good_count))
;;
...
let () =
  test "should_act filters beats" (fun () ->
    Eio_main.run
    @@ fun env ->
    let clock = Eio.Stdenv.clock env in
    ...
    if !even_count = 0 then failwith "even-only consumer was never called")
;;

(* ── Test: add/remove consumer dynamically ─────────────────── *)

let () =
  test "add and remove consumer dynamically" (fun () ->
    Eio_main.run
    @@ fun env ->
    let clock = Eio.Stdenv.clock env in
    ...
    if count_at_remove < 1 then failwith "late-joiner was never called";
    (* Should not increase after removal *)
    if !late_count > count_at_remove + 1
    then
      failwith
        (Printf.sprintf
           "late-joiner called after removal: before=%d after=%d"
           count_at_remove
           !late_count))
;;
...
let () =
  test "default_rhythm has sane values" (fun () ->
...
let () =
  test "stats tracks beats and nudges" (fun () ->
    Eio_main.run
    @@ fun env ->
    let clock = Eio.Stdenv.clock env in
    let t =
    ...
    let s = Pulse.stats t in
    if s.total_nudges < 2
    then failwith (Printf.sprintf "expected >=2 nudges, got %d" s.total_nudges);
    if s.total_beats < 3
    then
      failwith
        (Printf.sprintf "expected >=3 beats (startup + 2 nudges), got %d" s.total_beats))
;;

(* ── Test: quiet hour — normal range (1..6) ──────────────────── *)

let () =
  test "quiet_hour_normal_range" (fun () ->
...
let () =
  test "quiet_hour_wrap_around" (fun () ->
    let q = Pulse.For_testing.is_quiet_hour_at in
    let range = 22, 6 in
...
let () =
  test "effective_interval_normal" (fun () ->
...
let () =
  test "effective_interval_clamping" (fun () ->
    let eff = Pulse.For_testing.effective_interval_at in
    (* Clamp to max: base=200, quiet→200*3=600 > max=300 → 300 *)
    let r_high = { Pulse.base_s = 200.0; min_s = 30.0; max_s = 300.0; quiet = 1, 6 } in
    let v = eff ~hour:3 r_high in
    assert (v = 300.0);
    (* Clamp to min: base=5, not quiet→5 < min=30 → 30 *)
    let r_low = { Pulse.base_s = 5.0; min_s = 30.0; max_s = 300.0; quiet = 1, 6 } in
...
let () =
  test "nudge_coalescing" (fun () ->
    ...
    if s.total_beats < 2
    then failwith (Printf.sprintf "expected >=2 beats, got %d" s.total_beats))
;;

(* ── Test: consumer recovery — disabled after consecutive failures ── *)

let () =
  test "consumer disabled after 3 consecutive failures" (fun () ->
    Eio_main.run
    @@ fun env ->
    let clock = Eio.Stdenv.clock env in
    let fail_consumer =
...
let () =
  test "reenable previously disabled consumer" (fun () ->
    Eio_main.run
    @@ fun env ->
    let clock = Eio.Stdenv.clock env in
    ...
    let consumer =
      (module struct
        ...
        let on_beat _b =
          incr fail_count;
          if !fail_count <= 3 then Error "flaky" else Ok ()
        ;;
      end : Pulse.Consumer)
    in
    let t =
...
let () =
  test "circuit_breaker wrap records success/failure" (fun () ->
    Eio_main.run
    @@ fun env ->
    Fs_compat.set_fs (Eio.Stdenv.fs env);
    let cb =
      Circuit_breaker.create ~failure_threshold:2 ~failure_window:60.0 ~cooldown:1.0 ()
    in
    (* Success path *)
    ...
    match r2 with
    | Error _ -> ()
    | Ok _ -> failwith "expected Error while open")
;;

(* ── Test: circuit_breaker wrap_result ────────────────────────── *)

let () =
  test "circuit_breaker wrap_result catches exceptions" (fun () ->
    Eio_main.run
    @@ fun env ->
    Fs_compat.set_fs (Eio.Stdenv.fs env);
    let cb =
      Circuit_breaker.create ~failure_threshold:3 ~failure_window:60.0 ~cooldown:1.0 ()
    in
    let r =
      Circuit_breaker.wrap_result cb ~agent_id:"exc-test" (fun () -> failwith "boom")
    in
    (match r with
     | Error _ -> ()
     | Ok _ -> failwith "expected Error from exception");
...
let () =
  test "set_rhythm changes interval" (fun () ->
    Eio_main.run
    @@ fun env ->
    let clock = Eio.Stdenv.clock env in
    let t =
    ...
    if gained < 3
    then failwith (Printf.sprintf "expected >=3 beats after set_rhythm, got %d" gained))
;;

(* ── Test: get_rhythm returns current value ──────────────── *)

let () =
  test "get_rhythm returns current rhythm" (fun () ->
    Eio_main.run
    @@ fun env ->
    let clock = Eio.Stdenv.clock env in
    let t =
