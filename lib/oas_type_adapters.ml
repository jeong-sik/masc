(** OAS type adapters — convert MASC model types to OAS (Agent SDK) types.
    ...
    Thin wrappers bridging {!Llm_types.model_spec} and {!Llm_types.message}
...
let to_oas_provider (spec : Llm_types.model_spec) : Agent_sdk.Provider.config option =
...
let to_oas_message (m : Llm_types.message) : Agent_sdk.Types.message option =
  match m.role with System -> None | _ -> Some m

let of_oas_message (m : Agent_sdk.Types.message) : Llm_types.message = m
let of_oas_usage (u : Agent_sdk.Types.api_usage) : Llm_types.token_usage = u
let to_oas_usage (u : Llm_types.token_usage) : Agent_sdk.Types.api_usage = u
