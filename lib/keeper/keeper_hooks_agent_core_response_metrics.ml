...
module Response_shape = Agent_core.Response_shape
...
(* #10083 originally patched empty [response.model] leaks by recovering a
   ...
   AGENT_CORE owns concrete provider/model identity, while keeper-facing telemetry
...
(* zero_usage moved to Keeper_hooks_agent_core_types (intra-library file split). *)
...
(* #10083: keep the missing/alias observability, but return the keeper-facing
   runtime lane instead of reconstructing AGENT_CORE-owned model identity. *)
...
(* stop_reason_metric_label unified into
   Keeper_hooks_agent_core_types.stop_reason_to_label (2026-06-24): it was a
   byte-for-output-identical 9-arm match that bypassed the
   stop_reason_label_* SSOT constants by inlining their literals.  [open
   Keeper_hooks_agent_core_types] above brings stop_reason_to_label into scope. *)
...
(* Cost-only usage (all token counters zero, only cost_usd set) is not
   ...
   [Keeper_hooks_agent_core.usage_missing_of_usage] — instead of producing the
...
(** Emit prompt/decode tokens-per-second histograms from an AGENT_CORE turn
    ...
    model id, and the finer [provider_kind] reported by AGENT_CORE.  Split from
    ...
    constructing a full [Agent_core.Hooks.AfterTurn] event. *)
