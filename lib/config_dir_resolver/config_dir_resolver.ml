...
(* Startup publishes the admitted canonical owner in MASC_BASE_PATH.
   Configuration IO follows that owner. *)
...
let prompts_dir_for_base_path ~base_path =
  (resolve_for_base_path ~base_path).prompts.path
...
let constitution_dir ~base_path =
  Filename.concat (masc_root ~base_path) "constitution"
