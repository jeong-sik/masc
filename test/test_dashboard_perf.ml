...
let ensure_dir path =
  let rec loop current =
    if current = "" || current = Filename.dirname current
    then ()
    else (
...
let write_json path json = write_file path (Yojson.Safe.pretty_to_string json)
let set_mtime path value = Unix.utimes path value value
...
let test_dashboard_perf_reads_root_benchmarks () =
  ...
  check
    string
    "result file"
    "benchmarks/results/results_20260331_140000.csv"
    (json |> member "source" |> member "result_file" |> to_string);
  check
    string
    "baseline file"
    "benchmarks/results/results_20260331_130000.csv"
  ...
  check
    ...
    (json
     ...
     |> member "note_tags"
     |> member "healthy_runtime_count"
     |> to_string);
  check
    string
    "top change benchmark"
    "oas_runtime_single"
    (json
     |> member "comparison"
     |> member "top_changes"
...
let test_dashboard_perf_reads_worktree_benchmarks () =
  ...
  with_env "MASC_BENCHMARK_RESULTS_DIR" worktree_results_dir
  @@ fun () ->
  let json = Lib.Server_dashboard_http.dashboard_perf_http_json config in
  let open Yojson.Safe.Util in
  check string "status" "ok" (json |> member "status" |> to_string);
  check
    string
    "worktree dir selected"
    ".worktrees/feat-perf/benchmarks/results"
  ...
  check
    ...
    (json
     |> member "highlights"
     |> member "runtime_single"
     |> member "note_tags"
     |> member "measured_ceiling"
     |> to_string)
;;
...
let test_dashboard_perf_prefers_latest_scoped_artifact () =
  ...
  check
    string
    "latest scoped file wins"
    "benchmarks/results/results_20260331_160000.csv"
    (json |> member "source" |> member "result_file" |> to_string)
;;
...
let test_dashboard_perf_empty_shape () =
  ...
  check int "benchmarks empty" 0 (json |> member "benchmarks" |> to_list |> List.length);
  check
    bool
    "comparison null"
    true
    (match json |> member "comparison" with
     | `Null -> true
     | _ -> false)
;;
...
let () =
  run
    ...
    [ ( "dashboard_perf"
      , [ test_case
            "returns stable empty shape without artifacts"
            `Quick
            test_dashboard_perf_empty_shape
        ; test_case
            "reads latest root benchmark artifacts"
            `Quick
            test_dashboard_perf_reads_root_benchmarks
        ; test_case
            "falls back to worktree benchmark artifacts"
            `Quick
            test_dashboard_perf_reads_worktree_benchmarks
        ; test_case
            "prefers latest artifact within scoped dirs"
            `Quick
