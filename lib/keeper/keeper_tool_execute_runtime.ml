          ...
          let dispatch_result =
            ...
            then (
              ...
              (* RFC-0254 §5.2/§5.5: the keeper lane is autonomous — no human or
                 ...
                 for non-privileged, non-catastrophic commands.  The dispatch
                 chokepoint still fail-closes privileged programs until a real
                 Shell IR approval resolver exists, and the trust-independent
                 catastrophic floor denies destructive git, write-escape, mkfs,
                 destructive DB commands, and power-control binaries. *)
              let approval_config =
