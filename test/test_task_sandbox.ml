...
let make_temp_dir () =
  ...
  let dir =
    Filename.concat
      ...
      (Printf.sprintf
         "masc-sandbox-test-%d-%d-%d"
         (Unix.getpid ())
         !temp_counter
         (Random.int 999999))
  in
  if Sys.file_exists dir
  then ignore (Sys.command (Printf.sprintf "rm -rf %s" (Filename.quote dir)));
...
let test_create_fails_without_git () =
  with_non_git_room (fun _dir config ->
    match Task_sandbox.create ~config ~task_id:"task-001" ~agent_name:"test-agent" () with
    | Ok _ -> fail "expected error for non-git directory"
    | Error e -> check bool "error mentions failure" true (String.length e > 0))
;;

let test_create_fails_with_empty_task_id () =
  with_non_git_room (fun _dir config ->
    match Task_sandbox.create ~config ~task_id:"" ~agent_name:"test-agent" () with
...
let test_with_sandbox_fails_without_git () =
  with_non_git_room (fun _dir config ->
    ...
    with
    | Ok _ -> fail "expected error for non-git directory"
    | Error e -> check bool "error message non-empty" true (String.length e > 0))
;;
...
let run_cmd cmd =
  ...
  if exit_code <> 0
  then failwith (Printf.sprintf "command failed (exit %d): %s" exit_code cmd)
;;

let seed_playground_clone ~base_path ~agent_name ~source_repo =
  let repos_dir =
    Filename.concat base_path (Printf.sprintf ".masc/playground/%s/repos" agent_name)
  in
  let clone_path = Filename.concat repos_dir (Filename.basename source_repo) in
  Fs_compat.mkdir_p repos_dir;
  if Sys.file_exists clone_path
  then ignore (Sys.command (Printf.sprintf "rm -rf %s" (Filename.quote clone_path)));
  run_cmd
    (Printf.sprintf
       "git clone %s %s"
       (Filename.quote source_repo)
       (Filename.quote clone_path));
  clone_path
;;
  ...
  Fun.protect
    ...
    (fun () ->
       Eio_main.run (fun env ->
         ...
         run_cmd
           (Printf.sprintf
              "git init --bare --initial-branch=main %s"
         ...
         let _msg = Coord.init config ~agent_name:None in
         let _join =
           Coord.join config ~agent_name:"lifecycle-agent" ~capabilities:[ "test" ] ()
         in
         let clone_path =
           seed_playground_clone
             ~base_path:dir
         ...
         | Ok sb ->
           ...
           run_cmd
             (Printf.sprintf "git -C %s add work.txt" (Filename.quote sb.worktree_path));
           ...
           check
             ...
             (List.exists
                (fun f ->
                   try
                     ignore (Str.search_forward (Str.regexp_string "work.txt") f 0);
                     true
                   with
                   | Not_found -> false)
                files_after_add);
  ...
  Fun.protect
    ...
    (fun () ->
       Eio_main.run (fun env ->
         ...
         let _msg = Coord.init config ~agent_name:None in
         let _join =
           Coord.join config ~agent_name:"with-agent" ~capabilities:[ "test" ] ()
         in
         let _clone_path =
           seed_playground_clone ~base_path:dir ~agent_name:"with-agent" ~source_repo:dir
         in
         match
           Task_sandbox.with_sandbox
             ~config
             ...
             (fun sb ->
                ...
                42)
         with
         | Ok (result, _files) -> check int "function result" 42 result
         | Error e -> fail (Printf.sprintf "with_sandbox failed: %s" e)))
;;
  ...
  Fun.protect
    ...
    (fun () ->
       Eio_main.run (fun env ->
         ...
         let _msg = Coord.init config ~agent_name:None in
         let _join =
           Coord.join config ~agent_name:"exc-agent" ~capabilities:[ "test" ] ()
         in
         let _clone_path =
           seed_playground_clone ~base_path:dir ~agent_name:"exc-agent" ~source_repo:dir
         in
           ...
           let _ =
             Task_sandbox.with_sandbox
               ...
               (fun sb ->
                  ...
                  failwith "intentional test failure")
           in
           fail "expected exception to propagate"
         with
         | Failure msg ->
           check string "exception message" "intentional test failure" msg;
           if !worktree_path_ref <> ""
           then
             check bool "worktree cleaned up" false (Sys.file_exists !worktree_path_ref)))
;;
...
let () =
  run
    "Task_sandbox"
    [ "types", [ test_case "sandbox_fields" `Quick test_sandbox_type_fields ]
    ; ( "error_paths"
      , [ test_case "fails_without_git" `Quick test_create_fails_without_git
        ...
        ; test_case
            "cleanup_nonexistent"
            `Quick
            test_cleanup_fails_for_nonexistent_sandbox
    ...
    ; ( "changed_files"
      , [ test_case
            ...
            `Quick
            test_changed_files_returns_empty_for_nonexistent_path
        ] )
    ; "symlink", [ test_case "masc_symlink" `Quick test_symlink_created_when_masc_exists ]
    ; ( "integration"
