...
| Created | 2026-09-18 |
| Target | `agent_core` — `lib/llm_provider/`, `lib/provider_runtime_binding.ml`, `models.toml`; masc — `lib/runtime/runtime_adapter.ml`, `lib/keeper/keeper_effective_tool_surface.ml`, `config/agent-core-models-overlay.toml` |
| Related | RFC-AC-018 §3·Phase 2, RFC-AC-023 §3.2·§3.3, RFC-AC-034 §2 규칙 4, RFC-AC-036, masc RFC-0342 D1·D2, masc RFC-0370 §3.2 |
| 측정 기준 | `origin/main` `7900ef4e18` (2026-09-18). 모든 수와 `파일:줄` 은 부록 A 의 명령으로 다시 낼 수 있다 |

## 0. 요약

- `capabilities.ml` 에 공급자 preset 이 12개 있다. 카탈로그 행은 적지 않은 필드를 이 preset 에서 받는다.
- 저장소 카탈로그 두 파일(`models.toml` 195행, `config/agent-core-models-overlay.toml` 19행)에서 이렇게 받는 값 중 `default_capabilities` 와 다른 값이 1,440개다.
...
`capabilities.ml` 은 preset 12개를 정의한다.
...
| `mimo_capabilities` | 492 | openai_compat_chat | 18 |
| `provider_l_capabilities` | 541 | openai_compat_chat_extended | 4 |
| `ollama_capabilities` | 550 | openai_compat_chat_extended | 5 |
| `ollama_cloud_capabilities` | 573 | ollama | 2 |
| `ollama_cloud_v1_capabilities` | 600 | ollama_cloud | 3 |
| `glm_capabilities` | 623 | default | 12 |
| `gemini_capabilities` | 669 | default | 17 |

`default_capabilities` 를 빼면 필드 대입이 108개다. 그중 84개는 `default_capabilities` 와 값이 다르다.
라벨은 `capabilities_for_provider_label` (`capabilities.ml:754`) 이 preset 으로 바꾼다. 카탈로그의 `base` 와 `[[providers]]` 의 `capabilities_base` 가 이 라벨을 쓴다.

### 1.2 행이 적지 않은 필드는 코드에서 온다

`models.toml` 195행 중 183행이 `base` 를 적는다. 나머지 12행은 공급자의 `capabilities_base` 를 쓴다. overlay 19행은 모두 `base` 를 적는다.
`Capabilities.apply_declarative_capability_overrides` (`capabilities.ml:979`) 는 행이 적은 필드만 덮어쓰고, 나머지는 preset 값을 그대로 둔다.

행이 적지 않은 (행, 필드) 쌍을 출처별로 세면 다음과 같다. `supported_models`, `serving_constraint` 는 뺐다.

| 출처 | `models.toml` | overlay |
|---|---|---|
| preset 값이 `default_capabilities` 와 다름 | 1,365 | 75 |
| preset 이 기본값과 같은 값을 일부러 다시 적음 | 447 | 48 |
| `default_capabilities` 에서만 옴 | 3,745 | 361 |

214행 모두 적어도 한 필드를 preset 에서 받는다.

### 1.3 코드 값과 카탈로그 값이 어긋난다

| preset | 코드 값 | 값을 직접 적은 행 | 코드 값을 받는 행 |
|---|---|---|---|
| `glm_capabilities` | ctx 200,000 (`:625`), out 40,960 (`:631`) | 32행. out 은 128,000×20, 96,000×3, 4,096×3, 32,768×3, 16,384×2, 131,072×1. 40,960 을 적은 행은 없다 | ctx·out 2행 (`zai-image` 의 `glm-image`, `cogview-4-250304`) |
| `anthropic_capabilities` | ctx 200,000 (`:365`), out 8,192 (`:367`) | 10행 모두 ctx 1,000,000. out 은 128,000×6, 384,000×2, 64,000×2 | 0행 |
| `gemini_capabilities` | ctx 1,000,000 (`:671`), out 65,000 (`:672`) | out 은 65,536×2, 64,000×2. 65,000 을 적은 행은 없다 | out 10행 (채팅 6, 이미지·음성 4), ctx 4행 |
| `kimi_capabilities` | out 32,768 (`:405`) | 0행 | 8행 (`models.toml` 7, overlay `kimi_code` 1) |

값마다 사정이 있다.

- GLM. `capabilities.ml:626-630` 주석은 "glm-5.1 이 `max_tokens <= 40960` 을 강제한다(2026-04-12)" 고 적는다. 같은 저장소의 `glm-5.1` 행은 out 128,000 이다. 두 기록 중 하나는 틀렸다.
...
코드 값이 실제 요청을 망친 기록이 있다. 2026-09-07 `keeper_analyze_image` 가 `glm-coding.glm-4.6v` 에서 HTTP 400 code 1210 을 받았다 (`models.toml:2976-2982`, `test/test_model_catalog_default.ml:428-435`, `test/test_output_token_receipt.ml:283-288`).
provider 이름이 붙은 행이 없어서 조회가 공급자 preset 으로 떨어졌다. 요청한 65,536 이 preset 값 40,960 으로 깎였다. 모델의 실제 범위는 `[1,32768]` 이었다.
고친 방법은 같은 행을 provider 이름만 바꿔 두 번 더 적는 것이었다 (`models.toml:2984`, `:3007`).

이때 출력 토큰 영수증은 값의 출처를 `Catalog_model` 이라고 적는다 (`backend_openai_request.ml:231`). `capabilities_for_config_model` 이 행이 없을 때도 `for_provider_label_base` (`capabilities.ml:1383`) 로 preset 을 `Some` 으로 돌려주기 때문이다. 출처 표시는 카탈로그인데 값은 코드에서 왔다.
...
| 출처 | 행 |
|---|---|
| `ollama_cloud` preset 의 `true` | 67 (`models.toml` 60, overlay 7) |
| GLM 행이 `default_capabilities` 에서 받은 `false` | 34 (28, 6) |
| `openai_chat` preset 의 `true` | 27 |
| `openai_chat_extended` preset 의 `true` | 17 |
| `gemini` 14, `anthropic` 10, `kimi` 8, `ollama` 8, `nvidia` 4 (모두 `true`) | 44 |
| 행이 직접 적음 | 25 (`true` 13, `false` 12) |

GLM 행 중 이 필드를 적은 행은 없다. 그래도 34행 모두 `false` 로 읽힌다.
...
- `uncontrolled_reasoning`. 키가 없다. `ollama_cloud_v1_capabilities` (`capabilities.ml:611`) 만 값을 정한다.
- `emits_usage_tokens`. `models.toml` 에도 capability manifest 에도 키가 없다. JSON provider catalog (`provider_catalog.ml:654`) 와 masc `runtime.toml` (`runtime_toml.ml:893`) 에만 있다. 그런데 `Provider_catalog.set_global` 과 `Capability_manifest.set_global` 을 부르는 곳은 이 저장소에서 인라인 테스트(`capabilities.ml:1934`)뿐이다. 그래서 카탈로그 행은 늘 기본값 `true` 를 받는다.
- `supported_models`. `models.toml` 이 읽는다 (`model_catalog.ml:483`). 일반 경로 `overrides_of_catalog_entry` (`capabilities.ml:1249-1286`) 는 이 값을 넘기지 않는다. exact-output 경로(`exact_output_catalog_binding.ml:575`)만 넘긴다.

코드에만 있는 규칙이 다섯 개다.

| # | 규칙 | 위치 | 성격 |
|---|---|---|---|
| R1 | thinking 이 켜지면 required·named `tool_choice` 를 거절한다 | `provider_config.ml:467-479` | `config.kind = Anthropic` 으로 판정한다. DeepSeek Anthropic 호환 행은 같은 사실을 카탈로그 필드로 따로 적었다 (`models.toml:57-58`) |
...
- `capabilities.ml:1383` `for_provider_label_base` — 공급자가 `[[providers]]` 에 있고 행이 없으면 공급자 preset 을 돌려준다. 1.3 의 1210 사고가 이 경로였다.
- `capabilities.ml:980-986` — `base` 가 없으면 `default_capabilities`.
...
- masc `runtime_adapter.ml:478-512` — 카탈로그에 없는 모델의 `runtime.toml` 선언을 `default_capabilities` 위에 얹는다.
...
- 일반 경로: `Capabilities.apply_catalog_entry` (`capabilities.ml:1298-1318`) → `apply_declarative_capability_overrides` (`:979-1177`)
- exact-output 경로: `Exact_output_catalog_binding.capabilities_of_catalog_binding` (`exact_output_catalog_binding.ml:416-577`)

코드를 읽고 찾은 차이다.

| 항목 | 일반 경로 | exact-output 경로 |
|---|---|---|
| preset 고르기 | provider 이름이 붙은 행이면 `capabilities_base_by_identity_kind` 를 먼저 본다 (`capabilities.ml:1300-1315`) | 행의 `base`, 없으면 공급자 `capabilities_base` (`:420`). wire 별 base 를 보지 않는다 |
| 틀린 effort 문자열 | 경고하고 목록 전체를 preset 값으로 둔다 (`capabilities.ml:869-894`, `:1065-1075`) | 틀린 항목만 조용히 빼고 나머지를 쓴다 (`:444`) |
| 모르는 enum 문자열 | `Diag.warn` 후 preset 값 | 경고 없이 preset 값 |
| `modality_priority` 별칭 | `preserve`, `visual-first` 등도 받는다 (`capabilities.ml:861-867`) | 두 철자만 받는다 (`:409-414`) |
| `supported_models` | 넘기지 않는다 | 넘긴다 (`:575`) |

첫 줄의 차이는 `ollama_cloud` 공급자 행 43개(`models.toml` 36, overlay 7)에 걸린다. 이 공급자는 `capabilities_base_by_identity_kind = { openai_compat = "ollama_cloud_v1" }` 를 적는다.
OpenAI 호환 wire 에서 일반 경로는 `ollama_cloud_v1` 을 얹는다. `thinking_control_format`, `uncontrolled_reasoning`, `accepted_reasoning_efforts` 세 필드가 달라진다. 43행 중 `accepted_reasoning_efforts` 를 적은 행은 없다.
...
overlay 를 합치는 방법도 둘이다.

- `Model_catalog.merge` (`model_catalog.ml:1110-1131`) — 같은 키의 행을 통째로 바꾼다. RFC-AC-036 이 정한 방식이다.
- `merge_exact_model_entry` (`exact_output_catalog_binding.ml:211-297`) — 필드마다 합친다. `:209-210` 주석이 "sparse 행 의미는 exact resolver 에만 속하고, 전역 카탈로그는 통째로 바꾼다" 고 적는다.

저장소 overlay 는 본 카탈로그 행 4개를 덮는다. 그중 `ollama_cloud` 의 `kimi-k2.6`, `deepseek-v4-pro` 는 본 행이 적은 `reasoning_streaming_format = "delta:reasoning"` 을 적지 않는다.
전역 경로에서는 이 값이 사라지고, overlay 행이 적은 `thinking_control_format = "none"` 에서 기본값을 끌어낸다. 코드를 읽으면 `No_streaming_reasoning` 이다 (`reasoning_dialect.ml:96-106`, `:130-141`). exact-output 경로는 `delta:reasoning` 을 그대로 둔다. 실행해서 확인하지는 않았다.
...
1. **공급자·모델 사실은 카탈로그 한 곳에만 적는다.** 카탈로그는 내장 `models.toml` 과 배포 overlay 다.
...
- **RFC-AC-036 변경 1** — overlay 가 행을 통째로 바꾸도록 정했다. 3.f 는 이 결정을 뒤집는다. 이유는 3.f 에 적는다.
...
### 3.a 지금 값을 카탈로그에 적어 둔다 (동작 변화 없음)

**무엇.** 행마다 지금 resolver 가 내는 값을 TOML 에 적는다. 새로 쓰는 값은 두 종류다.

- preset 값이 `default_capabilities` 와 다른 필드: `models.toml` 1,365개, overlay 75개.
- preset 이 기본값과 같은 값을 일부러 적은 필드: `models.toml` 447개, overlay 48개. 예를 들어 `glm_capabilities` 의 `supports_structured_output = false` 는 문서 근거와 함께 적은 선언이다 (`capabilities.ml:647-656`).

`default_capabilities` 에서만 오는 값(`models.toml` 3,745개, overlay 361개)은 적지 않는다. 아무도 선언하지 않은 값이기 때문이다. 3.c 에서 `unknown` 이 된다.
...
새로 적는 줄에는 `# from-preset` 주석을 붙인다. 근거 없이 옮긴 값이라는 표시다. 3.b·3.c 가 이 표시를 줄여 간다. 남은 개수를 세는 스크립트를 함께 둔다.

**wire 에 따라 답이 다른 행.** `ollama_cloud` 행 43개는 wire 마다 세 필드가 다르다(1.7). 한 행에 값 하나로 적을 수 없다. 적는 형식은 열린 질문 Q1 이다.
...
**바뀌는 파일.** `packages/agent_core/models.toml`, `config/agent-core-models-overlay.toml`, 새 실행 파일과 dune 규칙, 표시 개수를 세는 스크립트.

**wire 변화.** 없다. `base` 는 아직 남아 있고 적은 값은 preset 값과 같다.

**검증.** 모든 행 × 공급자의 모든 `identity_kinds` 에 대해 적기 전과 뒤의 capability 레코드가 같아야 한다. 두 resolver 모두 확인한다. 기존 요청 스냅샷 테스트가 그대로 통과해야 한다.

### 3.b 틀린 값을 근거와 함께 고친다

**무엇.** 3.a 가 드러낸 값 중 근거가 있는 값만 고친다. PR 은 공급자 하나 또는 사실 한 종류 단위로 나눈다. 값마다 공식 문서 링크나 실측 기록, 확인 날짜를 행 주석에 적는다. `models.toml:2976-2982` 의 형식을 따른다.
...
| Kimi `max_output_tokens = 32768` | 8 | 기본값이 아니라 상한인지 |
| GLM 이미지 행 `max_context_tokens`·`max_output_tokens` | 2 | 이미지 생성 모델에 이 필드가 뜻이 있는지 |
| GLM `supports_parallel_tool_calls` | 34 | 2026-09-18 실측은 `glm-5.3-flash` 1개 모델뿐이다. 잰 행만 `true` 로 적는다 |
| `ollama_cloud` `supports_parallel_tool_calls = true` | 67 | 모델마다 잰 기록 |
...
**검증.** 고친 행마다 근거 링크나 실측 기록이 있다. `# from-preset` 표시 개수가 줄어든다.
...
**바뀌는 파일 (측정).** `capabilities.ml(i)` 레코드와 `effective_disable_parallel_tool_use` (`:307-313`), `backend_openai_request.ml:487-495`, `backend_openai_responses.ml:540-545`, `backend_anthropic.ml:414-418`, `backend_ollama.ml:161-170`, `exact_output_catalog_binding.ml:231`·`:549`, `model_catalog.ml:420-421`, `capability_manifest.ml:475`, `provider_catalog.ml:635-636`·`:676-677`, masc `runtime_schema.ml(i)` (`:136`, `:160`), `runtime_toml.ml:881`, `runtime_adapter.ml:489`, `server_dashboard_http_runtime_info.ml:1738`·`:1853`·`:1997`.

**wire 변화.** 필드를 적지 않은 행 중 지금 `false` 로 읽히는 행이 요청에서 끄기 필드를 뺀다. 3.b 전 기준으로 GLM 34행이다. 필드를 직접 `false` 로 적은 OpenRouter 12행은 계속 보낸다. 나머지 행은 지금도 `true` 라 바뀌지 않는다.
...
| `emits_usage_tokens` | `true` → usage 가 없으면 drift 관찰 (`capabilities.ml:2245-2246`) | drift 관찰을 만들지 않는다 |
...
- `capabilities.ml:241-701` preset 12개, `:725-733` `capabilities_of_kind`, `:735-744` `provider_kind_alias_of_label`, `:754-769` `capabilities_for_provider_label`, `:896-935` 의 `base_label`, `capabilities.mli:197-219`·`:319-352` 공개 선언.
...
- `capability_vocab.ml:726-751` `base_label_values`.
- 카탈로그: `models.toml` 183행과 overlay 19행의 `base`, 공급자 26곳의 `capabilities_base`, `ollama_cloud` 의 `capabilities_base_by_identity_kind`.
...
| R4 | 행 필드로 옮긴다. `uncontrolled_reasoning` 과 합칠지는 Q4 | "요청에 제어가 없을 때 엔드포인트가 무엇을 하나" 는 엔드포인트 사실이다 |
...
**wire 변화.** 없다. 값은 3.a 에서 이미 행에 있다. 행이 없는 config 는 아직 3.e 전이라 "아무것도 선언하지 않음" 레코드를 받는다. 1.6 의 1·2·3번과 나머지가 이 시점에 같은 답을 내게 된다.

**검증.** `rg -n '_capabilities\b|capabilities_base|base_label'` 가 운영 코드에서 0건. 3.a 의 레코드 비교 테스트가 그대로 통과한다.

### 3.e 행이 없을 때의 답을 하나로 만든다

**무엇.** `Provider_config.capabilities_for_config_model` 을 `(capabilities, unknown_model) result` 로 바꾼다. 오류 이름은 RFC-AC-018 의 `Unknown_model` 을 따른다. 1.6 의 11곳과 라벨 기본값 7곳에서 기본값 갈래를 지우고 오류를 처리한다.

- 요청을 만드는 backend 는 HTTP 전에 typed 오류로 요청을 끝낸다.
- masc `keeper_effective_tool_surface.ml` 은 오류를 도구 표면 판정에 그대로 올린다.
- masc `runtime_adapter.ml:478-512` 는 `runtime.toml` 선언이 모든 필드를 채울 때만 capability 를 만든다 (RFC-0342 D2).
- `for_provider_label_base` 를 지운다. 공급자는 알지만 행이 없으면 `Unknown_model` 이다. RFC-0342 가 막은 bare 행 fallback 은 되살리지 않는다.

**wire 변화.** 카탈로그에 행이 없는 config 는 지금 preset 값으로 요청을 보낸다. 바뀐 뒤에는 요청 전에 거절된다. 출력 토큰 영수증의 `Catalog_model` 은 실제로 행에서 온 값에만 붙는다.

**영향 측정.** masc 기동 검사는 카탈로그에 없는 routed binding 을 이미 거절한다 (RFC-0342 §1). 그래서 masc 런타임 영향은 작을 것 같지만 재지 않았다. 이 PR 은 실제 배포 `runtime.toml` 과 overlay 로 행이 없는 binding 을 먼저 센다.
...
### 3.f overlay 합치기와 행 읽기를 하나로 만든다

**무엇.**

- `Model_catalog.merge` 가 필드마다 합친다. `merge_exact_model_entry` 로직을 `model_catalog.ml` 로 옮기고 `merge_exact_model_entries` (`exact_output_catalog_binding.ml:304-327`) 를 지운다.
- `capabilities_of_catalog_binding` (`exact_output_catalog_binding.ml:416-577`) 을 지우고 `Capabilities` 의 함수 하나만 남긴다. 1.7 표의 다섯 차이는 이 함수 하나의 동작으로 정한다. 틀린 문자열은 로드 실패로 둔다.

**RFC-AC-036 을 뒤집는 이유.** 통째 교체는 preset 이 빈칸을 채워 주던 때에 맞는 선택이었다. overlay 행이 적지 않은 필드는 preset 값이 됐기 때문이다.
3.d 뒤에는 빈칸을 채우는 곳이 없다. 통째 교체를 유지하면 overlay 행이 필드 37개를 모두 다시 적어야 한다. 그러지 않으면 빠진 필드가 `unknown` 이 된다. RFC-AC-036 이 없애려던 "카탈로그 복사본" 문제가 행 단위로 돌아온다.
필드마다 합치면 overlay 는 배포에서 다르게 잰 값만 적는다. exact-output 경로는 이미 이렇게 동작한다.

대가도 있다. 한 행의 값이 두 파일에서 온다. 행 하나를 이해하려면 두 파일을 같이 봐야 한다. 그래서 overlay 가 덮은 필드를 기동 로그에 행마다 한 줄로 적는다.

**wire 변화.** 저장소 overlay 기준으로 `ollama_cloud` 의 `kimi-k2.6`, `deepseek-v4-pro` 가 일반 경로에서도 `reasoning_streaming_format = "delta:reasoning"` 을 받는다(1.7). 43개 `ollama_cloud` 행의 wire 별 세 필드는 Q1 의 결정을 따른다.

**검증.** 같은 overlay 로 두 경로가 모든 행에 같은 레코드를 낸다. RFC-AC-036 의 `test_model_catalog_overlay.ml` 은 필드 합치기를 기대하도록 바뀐다.

### 3.g preset 값을 붙잡는 테스트를 카탈로그 데이터 검사로 바꾼다

**현재 (측정, 휴리스틱).** preset 필드나 라벨 조회 결과를 직접 확인하는 테스트 파일이 7개다.
preset 을 fixture 바탕이나 인자로만 쓰는 파일이 20개다. `capabilities.ml` 인라인 테스트는 58개다(preset 과 무관한 것도 섞여 있다).
이름을 거치지 않고 값을 붙잡는 테스트도 있다. `test/test_runtime_provider_auth_headers.ml:1343-1344` 는 공급자 기본 경로로 40,960 을 확인한다. 이런 테스트는 이름 검색으로 다 못 찾는다. 3.d 에서 테스트를 돌려야 드러난다.
테스트 안 TOML 문자열에서 `base`·`capabilities_base` 를 쓰는 파일은 17개, 61줄이다.
...
- 프로토콜 코드. 기본 요청 경로(`provider_config.ml:21-26`), 인증 헤더(`provider_config.ml:253-269`, `exact_output_plan.ml:293-294`, `http_client.ml:1312`, `provider_files.ml:80`), `/responses` 판정(`provider_config.ml:725`). 이 리터럴은 인라인 테스트 밖에서 4파일 13곳이다.
...
- **틀린 값이 선언처럼 보인다.** 3.a 가 근거 없는 값을 TOML 에 옮기면 읽는 사람은 누군가 적은 값으로 본다. `# from-preset` 표시와 개수 검사로 막는다. 표시가 남아 있는 동안은 1.8 의 문제가 그대로다.
- **배포 overlay 의 행이 조용히 사라진다.** 3.d 뒤에 `base` 는 모르는 키다. `model_catalog.ml:363-375` 가 행을 거절한다. 그런데 masc 기동은 overlay 를 관대한 로더로 읽는다 (`server_runtime_bootstrap.ml:102-125`, #35034). `base` 를 적은 overlay 행은 WARN 한 줄씩 남기고 모두 빠진다. 그 행을 쓰던 lane 은 행이 없는 상태가 된다.
  저장소 안에서 `base` 나 `capabilities_base` 를 적은 overlay 사본은 `config/` 밖에 6개다(`scripts/fixtures/` 2, `docs/evidence/` 4). 운영자 배포 경로의 overlay 는 재지 않았다. 3.d 는 같은 릴리스 노트에 overlay 변환을 적고, 기동 WARN 이 아니라 기동 실패로 드러낼지 함께 정한다.
...
- **변경이 잦은 파일이다.** 2026-09-10 이후 `capabilities.ml`, `model_catalog.ml`, `exact_output_catalog_binding.ml`, `reasoning_dialect.ml`, `provider_config.ml`, `models.toml`, overlay 를 건드린 커밋이 19개다. 단계마다 PR 을 작게 유지한다.

### 4.3 열린 질문

- **Q1. wire 마다 값이 다른 행을 어떻게 적나.** `ollama_cloud` 43행은 wire 에 따라 `thinking_control_format`, `uncontrolled_reasoning`, `accepted_reasoning_efforts` 가 다르다.
...
- **Q3. overlay 가 값을 unknown 으로 되돌리는 문법.** TOML 에는 null 이 없다. 배포에서 "모름" 을 다시 선언할 일이 드물면 문법을 만들지 않는다. 필요해지면 `unset = ["field"]` 같은 키를 검토한다.
- **Q4. R4 와 `uncontrolled_reasoning` 을 하나로 합치나.** 둘 다 "요청에 제어가 없을 때 엔드포인트가 reasoning 을 켜는가" 에 답한다.
- **Q5. JSON 두 경로를 같이 지우나.** `Capability_manifest` 와 `Provider_catalog` 는 capability 를 만드는 세 번째·네 번째 경로다. 이 저장소에서 운영 코드가 `set_global` 을 부르지 않는다. 둘 다 `base_label` 을 쓰므로 3.d 에서 함께 지우기를 권한다. 외부 소비자 여부는 모른다.
...
모든 명령은 worktree 루트에서 `origin/main` `7900ef4e18` 기준으로 돌렸다.

### A.1 preset·행·상속·어긋남·병렬·overlay·리터럴·테스트 (1.1–1.4, 1.7, 3.a, 3.g, 4.1)

`python3 rfc040_measure.py` 로 아래 스크립트를 돌렸다. 출력 요약:

```text
presets 12 | assignments in non-default presets 108 | of which differ from default 84 | record fields 38
[models.toml] rows 195 | with base 183 | provider-scoped 86 | providers 24
  pairs inheriting a preset value that differs from default_capabilities: 1365
  ...
  parallel inherited: ollama_cloud true 60, glm false 28, openai_chat true 26, openai_chat_extended true 17,
           gemini true 14, anthropic true 10, kimi true 7, ollama true 5, nvidia true 4
  parallel stated: openai_chat_extended false 12, openai_chat true 7, openai_chat_extended true 5
  stated anthropic ctx {1000000:10} out {128000:6, 384000:2, 64000:2}
  stated gemini ctx {1000000:8, 1048576:2} out {65536:2, 64000:2}
  stated glm ctx {200000:13, 128000:12, 1000000:1} out {128000:15, 96000:3, 4096:3, 32768:3, 16384:2}
  stated kimi ctx {256000:7}
  rows scoped to ollama_cloud 36 | stating thinking_control_format 11 | stating accepted_reasoning_efforts 0
[overlay] rows 19 | with base 19 | provider-scoped 19 | providers 2
  pairs inheriting a preset value that differs from default_capabilities: 75
  inherited limit max_output_tokens kimi 32_768 ×1
  parallel inherited: ollama_cloud true 7, glm false 6, ollama true 3, kimi true 1, openai_chat true 1
  parallel stated: openai_chat true 1
  stated glm ctx {200000:5, 1048576:1} out {128000:5, 131072:1}
  rows scoped to ollama_cloud 7 | stating thinking_control_format 6 | stating accepted_reasoning_efforts 0
overlay replaces (ollama_cloud, kimi-k2.6)        | omits: reasoning_streaming_format
overlay replaces (ollama_cloud, minimax-m3)       | omits: —
overlay replaces (ollama_cloud, deepseek-v4-pro)  | omits: reasoning_streaming_format
overlay replaces (ollama_cloud, deepseek-v4-flash)| omits: —
...
test files asserting a preset field or label lookup: 7
test files using a preset only as fixture base/argument: 20
unstated pairs by origin: models.toml differs 1365, explicit-default 447, only-default 3745;
                          overlay differs 75, explicit-default 48, only-default 361
```

행의 preset 은 "행의 `base`, 없으면 공급자 `capabilities_base`" 로 골랐다. exact-output 경로와 같은 규칙이다. `ollama_cloud` 행의 wire 별 base 는 반영하지 않았다(1.7).
테스트 분류는 정규식 휴리스틱이다. `test_provider_registry.ml:342-344` 는 registry 를 거쳐 preset 값을 확인하지만 fixture 쪽으로 분류됐다.
...
ovl = tomllib.load(open("config/agent-core-models-overlay.toml", "rb"))
prov = {p["id"].lower(): p for p in cat["providers"] + ovl.get("providers", [])}
...
for name, doc in (("models.toml", cat), ("overlay", ovl)):
    rows = doc["models"]
    print(f"\n[{name}] rows {len(rows)} | with base {sum('base' in r for r in rows)} | provider-scoped {sum('provider_name' in r for r in rows)} | providers {len(doc.get('providers', []))}")
...
k = lambda r: ((r.get("provider_name") or "").lower() or None, r["id_prefix"].lower())
base = {k(r): r for r in cat["models"]}
for r in ovl["models"]:
    if k(r) in base:
        print("overlay replaces", k(r), "| fields the embedded row states and the overlay row omits:", sorted(set(base[k(r)]) - set(r)))
...
for name, doc in (("models.toml", cat), ("overlay", ovl)):
    for r in doc["models"]:
        ...
        for f in D:
            ...
            if P[p][f] != D[f]: tot[(name, "differs from default")] += 1
            elif f in ch: tot[(name, "preset assigns the default value explicitly")] += 1
            else: tot[(name, "only default_capabilities")] += 1
...
인라인 테스트 줄(`reasoning_dialect.ml` 700번대 이후, `complete_common.ml` 140번대, `backend_ollama.ml:655`)을 빼면 1.6 표의 11곳이다. 라벨 기본값 자리는 `rg -n 'default_capabilities|capabilities_for_provider_label' --type ocaml -g '!**/test/**' .` 결과에서 인라인 테스트를 뺀 것이다.

### A.3 JSON 경로의 운영 호출 (1.5, Q5)

```sh
rg -n 'Capability_manifest\.set_global|Provider_catalog\.set_global' --type ocaml lib bin packages/agent_core/lib
```

결과는 `capabilities.ml:1934`(`[@@@coverage off]` 뒤 인라인 테스트)와 `provider_registry.mli:63`(문서 주석) 두 줄이다.
...
`models.toml:2979-2981`, `:3002-3004`, `test_model_catalog_default.ml:428-430`, `test_output_token_receipt.ml:285-286`, `test_runtime_provider_auth_headers.ml:1344`, `test_llm_provider_cov.ml:1225`, `capabilities.ml:626-631`, `keeper_vision_tool.ml:263`.

### A.5 overlay 사본·테스트 TOML·변경 빈도 (4.2, 3.g)

```sh
rg --files -g 'agent-core-models-overlay.toml' | rg -v '^config/' | xargs rg -l '^base = |^capabilities_base = ' | wc -l   # 6
rg -l 'capabilities_base|base = \\"|base=\\"' --type ocaml test packages/agent_core/test | wc -l                        # 17
rg -c 'capabilities_base|base = \\"|base=\\"' --type ocaml test packages/agent_core/test | awk -F: '{s+=$2} END {print s}'  # 61
git log origin/main --since=2026-09-10 --oneline -- packages/agent_core/lib/llm_provider/{capabilities,model_catalog,exact_output_catalog_binding,reasoning_dialect,provider_config}.ml packages/agent_core/models.toml config/agent-core-models-overlay.toml | wc -l  # 19
```
...
gh pr list --repo jeong-sik/masc --state open --json number,title,files --limit 200 \
  | python3 -c 'import json,sys; d=json.load(sys.stdin); print(len(d)); [print(p["number"]) for p in d if any(f["path"].startswith("packages/agent_core/lib/llm_provider/") or "models.toml" in f["path"] or "models-overlay" in f["path"] for f in p["files"])]'
gh pr view 36944 --repo jeong-sik/masc --json state,mergedAt,files
```

첫 명령은 `15` 만 출력했다(겹치는 PR 없음). 둘째 명령은 `MERGED`, `2026-09-17T17:06:37Z` 와 1.4 절에 적은 파일 8개를 돌려줬다.

### A.7 코드를 읽어 확인한 것 (실행하지 않음)

다음은 코드 읽기로만 확인했다. dune 으로 돌려 보지 않았다.

- 1.3 영수증의 `Catalog_model` 표시가 preset 값에도 붙는다는 것 (`capabilities.ml:1397-1422` → `backend_openai_request.ml:216-237`).
- 1.6 표의 kind 별 답.
- 1.7 두 resolver 의 다섯 차이와 overlay 두 행의 `No_streaming_reasoning`.
