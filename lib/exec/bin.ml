...
let name_of_known : known -> string = function
  | Ls -> "ls" | Cat -> "cat" | Pwd -> "pwd" | Echo -> "echo"
  | Head -> "head" | Tail -> "tail" | Grep -> "grep" | Rg -> "rg"
  | Find -> "find" | Which -> "which" | Test -> "test"
  | Basename -> "basename" | Dirname -> "dirname"
  | Stat -> "stat" | Du -> "du" | Df -> "df"
  | Sort -> "sort" | Uniq -> "uniq" | Wc -> "wc"
  | Cut -> "cut" | Tr -> "tr" | Date -> "date"
  | Env -> "env" | Printenv -> "printenv"
  | Hostname -> "hostname" | Whoami -> "whoami"
  | Uname -> "uname" | Ps -> "ps" | Tty -> "tty"
  | Git -> "git" | Docker -> "docker" | Curl -> "curl"
  | Wget -> "wget" | Ssh -> "ssh" | Scp -> "scp"
  | Tar -> "tar" | Rsync -> "rsync"
  | Make -> "make" | Cmake -> "cmake" | Npm -> "npm"
  | Yarn -> "yarn" | Pnpm -> "pnpm" | Pip -> "pip"
  | Opam -> "opam" | Cargo -> "cargo"
  | Gh -> "gh" | Glab -> "glab"
  | Terminal_notifier -> "terminal-notifier"
  | Osascript -> "osascript"
  | Play -> "play" | Rec -> "rec"
  | Ffplay -> "ffplay" | Mpg123 -> "mpg123" | Open -> "open"
  | Claude -> "claude" | Gemini -> "gemini" | Codex -> "codex"
  | Sudo -> "sudo" | Su -> "su" | Chmod -> "chmod"
  | Chown -> "chown" | Rm -> "rm" | Dd -> "dd" | Mkfs -> "mkfs"
...
let known_of_string : string -> known option = function
  | "ls" -> Some Ls | "cat" -> Some Cat | "pwd" -> Some Pwd
  | "echo" -> Some Echo | "head" -> Some Head | "tail" -> Some Tail
  | "grep" -> Some Grep | "rg" -> Some Rg | "find" -> Some Find
  | "which" -> Some Which | "test" -> Some Test
  | "basename" -> Some Basename | "dirname" -> Some Dirname
  | "stat" -> Some Stat | "du" -> Some Du | "df" -> Some Df
  | "sort" -> Some Sort | "uniq" -> Some Uniq | "wc" -> Some Wc
  | "cut" -> Some Cut | "tr" -> Some Tr | "date" -> Some Date
  | "env" -> Some Env | "printenv" -> Some Printenv
  | "hostname" -> Some Hostname | "whoami" -> Some Whoami
  | "uname" -> Some Uname | "ps" -> Some Ps | "tty" -> Some Tty
  | "git" -> Some Git | "docker" -> Some Docker | "curl" -> Some Curl
  | "wget" -> Some Wget | "ssh" -> Some Ssh | "scp" -> Some Scp
  | "tar" -> Some Tar | "rsync" -> Some Rsync
  | "make" -> Some Make | "cmake" -> Some Cmake | "npm" -> Some Npm
  | "yarn" -> Some Yarn | "pnpm" -> Some Pnpm | "pip" -> Some Pip
  | "opam" -> Some Opam | "cargo" -> Some Cargo
  | "gh" -> Some Gh | "glab" -> Some Glab
  | "terminal-notifier" -> Some Terminal_notifier
  | "osascript" -> Some Osascript
  | "play" -> Some Play | "rec" -> Some Rec
  | "ffplay" -> Some Ffplay | "mpg123" -> Some Mpg123
  | "open" -> Some Open
  | "claude" -> Some Claude | "gemini" -> Some Gemini
  | "codex" -> Some Codex
  | "sudo" -> Some Sudo | "su" -> Some Su | "chmod" -> Some Chmod
  | "chown" -> Some Chown | "rm" -> Some Rm | "dd" -> Some Dd
  | "mkfs" -> Some Mkfs
  | _ -> None
...
(** Construct a [t] directly from a [known] variant — no string parsing,
    no risk of [Error]. *)
let of_known k =
  { name = name_of_known k; risk = risk_of_known k; kind = kind_of_known k; known = Some k }
