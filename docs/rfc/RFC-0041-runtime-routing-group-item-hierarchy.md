...
# RFC-0041: Runtime Routing Architecture — Group/Item Hierarchy with Health-Aware Fallback
...
- `RFC-0022-runtime-attempt-liveness.md` — runtime attempt lifecycle and liveness probes
- `RFC-0027-capability-typed-runtime.md` — capability-based runtime selection
...
- PR #14168 — feat(keeper): remove runtime auto-pause, restart with fallback runtime

## Problem Statement

### Current State

현재 `keeper_registry.registry_entry`의 `runtime_id` 필드:

```ocaml
type registry_entry = {
  name : string;
  runtime_id : string;  (* 단순 문자열, 계층 없음 *)
  ...
}
```
...
PR #14168은 supervisor가 restart 시점에 `fallback_runtime_for`를 호출하여 runtime를 변경한다.

```ocaml
let meta_with_fallback =
  match Keeper_runtime_profile.fallback_runtime_for meta.runtime_id with
  | Some fallback -> { meta with runtime_id = fallback }
  | None -> meta
```

문제:
- **layering 위반**: supervisor는 keeper lifecycle을 관리해야 하며, runtime 선택은 routing layer의 책임이다.
- **불필요한 restart**: keeper process 자체는 정상인데 runtime(provider)가 문제인 경우에도 restart한다.
- **상태 손실**: restart는 keeper의 컨텍스트, 메모리, 세션 상태를 모두 초기화한다.

### Issue 2: Keeper Failure와 Runtime Failure 신호 혼재

현재 `restart_count`는 keeper crash와 runtime failure를 구분하지 않는다.

- runtime rate limit으로 3번 restart되면 keeper가 Dead 상태로 전이된다.
...
`health_cache`는 `Hashtbl.t`로 global mutex로 보호된다. 한 keeper의 runtime 측정이 다른 keeper의 routing 결정에 영향을 줄 수 있다.
...
| # | 원칙 | 의미 |
|---|------|------|
| 1 | runtime 때문에 Keeper 움직임을 끊기게 하면 안 됨 | runtime failure는 restart 대상이 아님. turn-level fallback으로 해결 |
| 2 | 한 Keeper의 runtime가 다른 Keeper를 막지 않음 | health probe는 per-keeper 격리. global shared state 금지 |
| 3 | 텔레메트리 정보가 분명하게 수집될 것 | 어떤 item/group 사용했는지, fallback 발생 여부가 observability에 잡혀야 함 |
| 4 | runtime는 group과 item을 소유함 | 계층 구조: runtime -> group -> item |
...
│  PR #14168에서 runtime switching 제거 후 단순화              │
│  파일: keeper_supervisor.ml                                  │
└─────────────────────────────────────────────────────────────┘
```

핵심 불변량: **runtime failure는 L1/L2에서 해결. L3는 keeper 자체 crash만 담당.**
...
type registry_entry = {
  name : string;
  runtime_ref : runtime_ref;  (* 기존 runtime_id 대체 *)
  ...
}
```

Backward compatibility: 기존 `runtime_id: string`은 `runtime_ref`로 migration. 단일 group에 단일 item으로 래핑.
...
```ocaml
let select_item_for_turn ~keeper_name ~runtime_profile ~health_cache ~last_used_item =
...
### legacy metrics backend Metrics

```ocaml
(* 새로 추가 *)
let metric_runtime_item_used_total =
  Counter.v_label ~label_name:"item_id" ~help:"..." "runtime_item_used_total"

let metric_runtime_fallback_triggered_total =
  Counter.v_label ~label_name:"reason" ~help:"..." "runtime_fallback_triggered_total"

let metric_runtime_group_exhausted_total =
  Counter.v_label ~label_name:"group_name" ~help:"..." "runtime_group_exhausted_total"

let metric_keeper_runtime_health_state =
  Gauge.v_label ~label_name:"state" ~help:"..." "keeper_runtime_health_state"
```
...
TurnStart(keeper) ==
  ...
  /\ LET group == runtime_profile[keeper].primary_group
...
```tla
(* 핵심: runtime 때문에 keeper가 멈추지 않음 *)
KeeperNeverBlockedByRuntime ==
  \A keeper \in Keepers :
    keeper_state[keeper] \in {"running", "turning", "restarting"}
    => keeper_state[keeper] /= "paused_due_to_runtime"
...
- `runtime_id: string` -> `runtime_ref: RuntimeRef.t`
...
- legacy metrics backend metric 추가
- Dashboard UI 업데이트
- TLA+ model checker 실행 (TLC)

### Phase 5: PR #14168 수정

- Supervisor에서 runtime switching 제거
...
3. **Health probe interval**: item-level probe는 기존 runtime-level probe보다 더 자주 실행 필요?
4. **Registry migration**: 기존 `runtime_id` 필드를 어떻게 migration? dual-read period?
...
|------|----------|-----------|
| 2026-05-08 | runtime_id: string 제거 | 단일 레벨로는 group/item 계층 표현 불가 |
| 2026-05-08 | per-keeper, per-item health cache | 요구사항 2: 한 keeper의 runtime가 다른 keeper 막지 않음 |
| 2026-05-08 | supervisor에서 runtime switching 제거 | 요구사항 1: layering 위반, 불필요한 restart |
