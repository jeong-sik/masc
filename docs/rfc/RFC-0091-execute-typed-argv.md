...
emission 코드는 당시 `lib/retired_worker_shell_facade` 안의 path-tokenizer diagnostic helper였다. 2026-05-21 purge에서는 해당 helper, path-token syntax bucket, 그리고 downstream classifier arm을 모두 제거했다.
...
증거로 `lib/retired_worker_shell_facade`은 현재 **1718줄 godfile**(`software-development.md` §"파일 크기 임계값 500줄+ 즉시 분할" 위반)이며, 그 본문의 대부분(line 102 ~ 750+)이
...
2. **`lib/retired_worker_shell_facade` 의 shell metachar lexer/validator 코드 박멸** —
...
3. **godfile 분해**: `retired_worker_shell_facade` 1718줄 → typed schema 모듈
...
- **interactive shell** 또는 **REPL**. typed argv는 single-shot 실행만 지원.
  ...
  로 분리되며 본 RFC 범위 밖.
- **legacy string schema 호환**. RFC 머지 PR이 `lib/retired_worker_shell_facade`
...
| 경로 | ERROR 건수 | 코드 위치 |
|---|---:|---|
| 직접 emission | 90 | `retired_worker_shell_facade` retired path-tokenizer diagnostic helper |
...
retired_worker_shell_facade 의 *현재 godfile* 구성 중 본 RFC가 삭제 대상으로 지정하는
...
2. `retired_worker_shell_facade` 의 `dev_allowed_commands` / `readonly_allowed_commands`
   를 신규 `lib/keeper/dev_exec_allowlist.ml(i)` 로 분리.
3. 한 caller (`retired_worker_shell_facade.run_tool_execute` 의 single entry) 를 typed
...
2. `retired_worker_shell_facade` 의 §2.2 enumerated 함수 17 개 + 그 dependent 변수
   (`forbidden_shell_chars`, `forbidden_shell_chars_coding`, `forbidden_shell_chars_coding_base`)
   삭제. 신규 LOC 0, 삭제 LOC 측정치를 PR body 에 기록.
3. test fixture (`test/test_retired_worker_shell_facade`, `test/test_keeper_tool_call_log.ml`,
...
### PR-4 — godfile 분해 (retired_worker_shell_facade split)

PR-2 머지 후 `retired_worker_shell_facade` 가 ~800 LOC 까지 줄어든 상태에서 남은 코드를
도메인별로 분리:
...
- `lib/retired_worker_shell_facade` — 단순 re-export (≤30 LOC) 또는 완전 삭제
...
- [ ] `wc -l lib/retired_worker_shell_facade` ≤ 300 또는 파일 자체 삭제.
...
2. **외부 MCP 서버**. masc 외부 클라이언트 (CLI-Tool-A, Agent-Code, Provider-F)
...
3. **dune build 영향**. `retired_worker_shell_facade` 가 godfile 이라 caller 가 많을
   가능성. `rg "Retired_worker_shell_facade\." lib/ | wc -l` 로 측정 후 PR-1 body 에 기록.
...
- **Confidence**: High (코드 단일 emission point `retired_worker_shell_facade:613` 그
