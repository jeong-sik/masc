...
supersedes: []
superseded_by: null
related: ["0239", "0242", "0257", "0243", "0259", "0285"]
implementation_prs: []
---
...
1. **near-dup 무한 append**: `keeper_memory_bank.ml` `append_memory_notes_from_reply`(:793)가
   매 턴 무조건 새 줄. 내용 dedup은 compaction(caller `keeper_agent_run_post_turn_memory.ml:183`
...
4. **continuity는 bank 비의존**: keeper working-state 복구는 `.memory.jsonl`이 아니라 snapshot
   cache(`keeper_turn.ml:533-553`, `read_progress_snapshot_cache` / checkpoint / meta_summary)에서
...
- **Stage 2 (read 재포인팅)**: dashboard/status의 `memory_bank` JSON 표면 4곳(`keeper_status.ml`,
  ...
  **가장 광범위한 리스크 표면**.
- **Stage 3 (write 중단 + retention 실작동)**: deterministic bank write(`append_from_reply` +
  tool-result long_term notes `post_turn_memory.ml:58`) 중단, tool-result notes는 Memory OS facts
