...
[![OAS](https://img.shields.io/badge/agent__sdk-%E2%89%A50.200.9-blue.svg)](https://github.com/jeong-sik/oas)
...
OCaml 5.4 + Eio 위에서 도는 MCP 서버. 같은 저장소를 동시에 만지는 코딩 에이전트들(Claude, Gemini, 로컬 모델)이 turn / lock / heartbeat / 작업 owner 를 공유하기 위한 상태 저장소입니다. 단일 머신 / 신뢰 네트워크 / 저자 한 사람이 운영하는 환경을 가정합니다.
...
┌──────────────────────────────────────────────────┐
│              Consumer / Client                    │
│       (Claude, Gemini, Local Agent)        │
└────────────────┬─────────────────────────────────┘
                 │  MCP (JSON-RPC over HTTP/SSE)
┌────────────────▼─────────────────────────────────┐
│            MASC  (workspace collaboration)               │
│                                                   │
│  Workspace/Board  Keeper   Runtime     Operator        │
...
| `lib/runtime/` + `lib/runtime_decl/` | 모델 선택 정책, TOML 스키마, validation (RFC-0058) |
| `lib/cdal/` + `lib/cdal_runtime/` | CDAL (workspace collaboration / decision audit layer) 평가 / 판정 |
| `lib/workspace/` | workspace, board, task, claim 같은 workspace collaboration primitive |
...
- 최신 GitHub Release 바이너리를 `~/.local/bin/masc` 로 다운로드
...
| Full runtime | `./start-masc.sh --http` | 모든 transport, keeper autoboot, dashboard |
| Stdio MCP | `./start-masc.sh --stdio` | stdio-only 클라이언트 |
...
### 1. 코딩 워크플로우 workspace collaboration
...
- `start-masc.sh --http` 는 `scripts/build-dashboard-if-needed.sh` 를 백그라운드로 띄웁니다. 차단 빌드가 필요하면 `MASC_DASHBOARD_BUILD_BLOCKING=1`.
...
- Discord: in-process gateway. `DISCORD_BOT_TOKEN` 환경변수만 설정하면 서버 부팅 시 자동으로 Discord Gateway WSS 에 연결되고 inbound 메시지를 keeper workspace 으로 라우팅합니다. 별도 sidecar 프로세스 없음 (`sidecars/discord-bot/` 은 RFC-0203 §Phase 3 에서 삭제됨). 트리거 정책은 `MASC_DISCORD_TRIGGER_POLICY` (`mention_only` 기본, `user_only:<id>`, `all`) 로 조정.
...
핵심 모듈: `lib/ide/ide_region_tracker.ml`, `ide_annotations.ml`, `ide_annotation_types.ml`, `ide_paths.ml`. backend 쪽 region write 진입점은 `lib/keeper/keeper_tool_filesystem_runtime.ml` 의 filesystem write 흐름이 `Ide_region_tracker.ingest_tool_call` 로 전달되는 지점.
...
|-----|------|------------------------------|
| RFC-0058 | Runtime decl SSOT — closed-variant dispatch 제거, capability 기반 분기, TOML schema 확장 | 50 |
...
| RFC-0072 | Keeper sub-FSM 전이를 GADT + typed resolver 로 (decision / runtime / turn_phase / compaction 4축) | 7 |
...
- role / bearer mismatch (예: `caller cannot CanAdmin`) 진단은 login JSON과 dashboard auth runbook부터.
