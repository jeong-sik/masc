...
let read_file_tail_lines path ~max_bytes ~max_lines : string list =
  if max_lines <= 0
  then []
  else if not (Fs_compat.file_exists path)
  then []
  else (
    try
      let full_content = Fs_compat.load_file path in
      let file_len = String.length full_content in
      let read_start = if max_bytes <= 0 then 0 else max 0 (file_len - max_bytes) in
...
let read_keeper_memory_summary
  ...
  =
  let lines =
    read_file_tail_lines (keeper_memory_bank_path config name) ~max_bytes ~max_lines
  in
...
let read_memory_horizon_counts
  ...
  List.iter
    (fun line ->
       ...
       | None -> ()
       | Some row ->
         let cur = Option.value ~default:0 (Hashtbl.find_opt counts row.horizon) in
...
let is_memory_recall_query (s : string) : bool =
  ...
  let ko_keywords =
    [ "기억"
    ; (* "memory/remember" — base morpheme *)
      "기억해"
    ; (* "do you remember" *)
      "기억안나"
    ; (* "can't remember" — no space variant *)
      "기억 안나"
    ; (* "can't remember" — spaced variant *)
      "기억나"
    ; (* "I remember" — no space variant *)
      "기억 나"
    ; (* "I remember" — spaced variant *)
      "전에 뭐"
    ; (* "what before" — asking about prior *)
      "이전에"
    ; (* "previously" *)
      "첫 질문"
    ; (* "first question" *)
...
let latest_message_content_by_role
  ...
  =
  match
    messages |> List.rev |> List.find_opt (fun (m : Oas.Types.message) -> m.role = role)
  with
  | None -> None
  | Some m -> trim_nonempty (String.trim (Oas.Types.text_of_message m))
;;

let previous_assistant_message_content (messages : Oas.Types.message list) : string option
  =
  let assistants =
    messages
    |> List.rev
    |> List.filter_map (fun (m : Oas.Types.message) ->
      if m.role = Oas.Types.Assistant
      then trim_nonempty (Oas.Types.text_of_message m)
      else None)
  in
...
let goal_horizon_candidates (meta : keeper_meta) : string list =
  ...
  |> List.filter_map (fun raw -> raw |> normalize_goal_horizon_text |> trim_nonempty)
  |> List.fold_left
       (fun acc goal ->
          let key = normalize_memory_text_key goal in
          if List.exists (fun existing -> normalize_memory_text_key existing = key) acc
          then acc
          else goal :: acc)
       []
  |> List.rev
...
let keeper_reflection_payload_of_auto_rules (e : keeper_auto_rule_eval) : Yojson.Safe.t =
  let actions_rev = [] in
  let actions_rev = if e.reflect then `String "reflect" :: actions_rev else actions_rev in
  let actions_rev = if e.plan then `String "plan" :: actions_rev else actions_rev in
  let actions_rev = if e.compact then `String "compact" :: actions_rev else actions_rev in
  let actions_rev = if e.handoff then `String "handoff" :: actions_rev else actions_rev in
  let actions_rev =
...
let context_measured_auto_rules_of_events (events : Keeper_state_machine.event list)
  ...
  let rec loop = function
    ...
    | _ :: rest -> loop rest
    | [] ->
      invalid_arg "keeper_auto_rule_eval_of_measurement: events missing Context_measured"
  in
  loop events
;;
...
let keeper_auto_rule_eval_of_measurement
  ...
  let effective_handoff_threshold = t.handoff_threshold *. t.model_handoff_multiplier in
  let reasons = [] in
  let reasons =
    if auto_rules.reflect
    then
      Printf.sprintf
    ...
    Keeper_measurement.capture
      ...
      ~thresholds:
        { compaction_ratio_gate =
            Float.min
              Env_config_keeper.context_ratio_hard_cap
        ...
        ; reflect_repetition_threshold = keeper_rule_reflect_repetition_threshold ()
        ; plan_goal_alignment_threshold = keeper_rule_plan_goal_alignment_threshold ()
        ; plan_response_alignment_threshold =
            keeper_rule_plan_response_alignment_threshold ()
        ; guardrail_repetition_threshold = keeper_rule_guardrail_repetition_threshold ()
...
let prioritized_action (eval : keeper_auto_rule_eval) : prioritized_action =
  if eval.guardrail_stop
  then Act_guardrail_stop (Option.value eval.guardrail_reason ~default:"guardrail_stop")
  else if eval.reflect
  then Act_reflect
  else if eval.plan
  then Act_plan
  else if eval.compact
  then Act_compact
  else if eval.handoff
  then Act_handoff
  else Act_none
;;
...
let learned_policy_auto_rules
  ...
  let goal_drift =
    1.0 -. max 0.0 (min 1.0 (max goal_alignment response_alignment)) |> max 0.0 |> min 1.0
  in
  let compact =
  ...
  in
  let adjusted_handoff_threshold =
    Float.min
      Env_config_keeper.context_ratio_hard_cap
      (meta.handoff_threshold *. handoff_mult)
  in
...
let recall_candidates_with_history
  ...
  let seen =
    List.fold_left (fun acc s -> SS.add (key_of s) acc) SS.empty from_checkpoint
  in
  let unique_history = List.filter (fun s -> not (SS.mem (key_of s) seen)) from_history in
...
let evaluate_memory_recall
  ...
  let has_weather_word (s : string) =
    let q = String.lowercase_ascii s in
    Re.execp (Re.str "날씨" |> Re.compile) s || Re.execp (Re.str "weather" |> Re.compile) q
  in
    ...
    let topic_bonus =
      ...
      | Some "first_question" ->
        let has_first =
          ...
          || Re.execp
               (Re.str "first" |> Re.compile)
               (String.lowercase_ascii assistant_reply)
        in
...
let work_kind_of_eval (e : memory_recall_eval) : string =
  if e.performed
  then
    if e.query_kind <> "" && e.query_kind <> "none" then e.query_kind else "memory_recall"
  else (
