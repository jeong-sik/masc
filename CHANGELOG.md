...
- Keeper: `<masc-root>/keepers/<keeper>/window-position.json` records where a keeper's next request starts — the Librarian's position in the saved history, and what the Librarian said the keeper was in the middle of when it absorbed up to there (RFC keeper-context-window-in-tokens §7 (라)). Nothing reads or writes it yet; the file, its field-exact codec and the rules that say whether it still describes the history a turn is about to send land first so the round that writes it and the turn that reads it cannot disagree about the shape. A file that exists and cannot be read is an error rather than "nothing absorbed", which would send the whole history without saying so. The keeper purge removes it with the Librarian's other files.
