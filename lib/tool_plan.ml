...
let handle_plan_init ctx args : tool_result =
  ...
  | Error e -> false, Printf.sprintf "❌ Failed to init planning: %s" e
;;
...
let handle_plan_update ctx args : tool_result =
  ...
  | Ok plan_ctx ->
    ...
    in
    true, Yojson.Safe.to_string response
  | Error e -> false, Printf.sprintf "❌ Failed to update plan: %s" e
;;
...
let handle_note_add ctx args : tool_result =
  ...
  | Ok plan_ctx ->
    ...
    in
    true, Yojson.Safe.to_string response
  | Error e -> false, Printf.sprintf "❌ Failed to add note: %s" e
;;
...
let handle_deliver ctx args : tool_result =
  ...
  | Ok task_id ->
    ...
    then false, "❌ content is required for masc_deliver"
    else (
      ...
      | Ok plan_ctx ->
        ...
        in
        true, Yojson.Safe.to_string response
      | Error e -> false, Printf.sprintf "❌ Failed to set deliverable: %s" e)
;;
  ...
  | Ok task_id ->
    ...
    (match result with
     | Ok plan_ctx ->
       ...
       in
       true, Yojson.Safe.to_string response
     | Error e -> false, Printf.sprintf "❌ Planning context not found: %s" e)
;;

let handle_plan_set_task ctx args : tool_result =
  let task_id = get_string args "task_id" "" in
  if task_id = ""
  then false, "❌ task_id is required"
  else (
    Planning_eio.set_current_task ctx.config ~task_id;
    let response = `Assoc [ "status", `String "set"; "current_task", `String task_id ] in
    true, Yojson.Safe.to_string response)
;;
...
let handle_plan_get_task ctx _args : tool_result =
  ...
  | Some task_id ->
    let response = `Assoc [ "current_task", `String task_id ] in
    true, Yojson.Safe.to_string response
  | None ->
    let response =
      `Assoc
...
let handle_plan_clear_task ctx _args : tool_result =
  ...
  let response =
    `Assoc [ "status", `String "cleared"; "message", `String "Current task cleared" ]
  in
  true, Yojson.Safe.to_string response
;;
...
let tool_required_permission = function
  | "masc_plan_get" | "masc_plan_get_task" -> Some Types.CanReadState
  | _ -> None
;;
