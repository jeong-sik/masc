...
let parse schema ~tool_name json =
  ...
  | Ok value -> Ok value
  | Error errs -> Error (Oas.Tool_input_validation.format_errors ~tool_name errs)
;;

let schema_to_tool_schema ~name ~description schema : Types.tool_schema =
  { Types.name; description; input_schema = Sg.to_json_schema schema }
;;
...
let schemas =
  [ schema_to_tool_schema
      ...
      ~description:
        "Read shared team memory by key from the flattened default namespace. Uses a \
         typed lane instead of exposing a shared writable shell directory."
      read_schema
  ; schema_to_tool_schema
      ~name:"masc_team_memory_write"
      ~description:
        "Write shared team memory by key in the flattened default namespace. Traversal, \
         symlink escape, and secret-like payloads are blocked."
      write_schema
  ; schema_to_tool_schema
      ~name:"masc_team_memory_search"
      ~description:
        "Search shared team memory in the flattened default namespace by filename or \
         content substring."
      search_schema
  ]
;;
...
let validate_team_memory_room room =
  ...
  if trimmed = ""
  then Error "room is required"
  else if String.equal (String.lowercase_ascii trimmed) default_namespace
  then Ok default_namespace
  else
    Error
      (Printf.sprintf
...
let resolve_keeper_access ~(config : Coord.config) ~(agent_name : string) =
  ...
  if trimmed = ""
  then Error "team memory tools require keeper agent context"
  else (
    ...
    | Some keeper_name ->
      (match Keeper_types.read_meta_resolved config keeper_name with
       ...
       | Ok None ->
         Error
           (Printf.sprintf "keeper context not found for team memory agent '%s'" trimmed)
       | Ok (Some (_resolved_name, meta)) -> Ok (keeper_name, meta)))
;;

let authorize_team_memory ~(config : Coord.config) ~(agent_name : string) ~room =
  ...
  | Ok (keeper_name, meta) ->
    (match meta.shared_memory_scope with
     | Keeper_types.Shared_memory_disabled ->
       Error
         (Printf.sprintf
            "team memory is disabled for keeper '%s'; set shared_memory_scope=room to \
             enable the flattened default namespace"
            keeper_name)
     | Keeper_types.Shared_memory_room -> validate_team_memory_room room)
;;
...
let safe_realpath path =
  ...
  | Unix.Unix_error (code, _, _) ->
    Error (Printf.sprintf "realpath failed for %s: %s" path (Unix.error_message code))
;;

let encoded_traversal_markers = [ "%2e"; "%2f"; "%5c" ]
...
let validate_team_memory_key key =
  ...
  if trimmed = ""
  then Error "key is required"
  else if String.contains trimmed '\x00'
  then Error "key must not contain NUL"
  else if String.contains trimmed '\\'
  then Error "key must not contain backslashes"
  else if String.contains trimmed '%' && contains_encoded_traversal trimmed
  then Error "key must not contain encoded traversal"
  else Validation.Safe_path.validate_relative trimmed
;;
    ...
    (match validate_team_memory_key key with
     ...
     | Ok rel_key ->
       ...
       let root_real =
         ...
         then safe_realpath root
         else safe_realpath (nearest_existing_path root)
       in
       (match root_real, safe_realpath (nearest_existing_path parent) with
        | Error err, _ | _, Error err -> Error err
        | Ok real_root, Ok real_parent ->
          if is_safe_subpath ~parent:real_root ~child:real_parent
          then Ok (room_id, rel_key, root, abs, real_root)
          else
            Error
              (Printf.sprintf "team memory key escapes room root via symlink: %s" rel_key)))
;;
...
let contains_secret_token_prefix ~prefix ~min_suffix_len text =
  ...
  let rec count_suffix idx count =
    if idx + count >= text_len
    then count
    else if is_secret_token_char lowered.[idx + count]
    then count_suffix idx (count + 1)
    else count
  in
  let rec loop idx =
    if idx + prefix_len > text_len
    then false
...
let read_json ~config (room, key) =
  ...
  | Ok (room_id, rel_key, _root, abs, real_root) ->
    ...
    then false, Printf.sprintf "team memory key not found: %s" rel_key
    else if Sys.is_directory abs
    then false, Printf.sprintf "team memory key is a directory: %s" rel_key
    else (
      match safe_realpath abs with
      | Error err -> false, err
      | Ok real_path ->
        if not (is_safe_subpath ~parent:real_root ~child:real_path)
        then
          ( false
          , Printf.sprintf "team memory key escapes room root via symlink: %s" rel_key )
        else (
...
let write_json ~config (room, key, content) =
  ...
  | Ok (room_id, rel_key, root, abs, _real_root) ->
    if content_looks_secret_like content
    then false, "team memory write blocked: content looks like it may contain secrets"
    else (
...
let search_json ~config (room, query) =
  ...
  | Ok room_id ->
    ...
    then false, "query is required"
      ...
      then
        ( true
        , Yojson.Safe.to_string (`Assoc [ "room", `String room_id; "matches", `List [] ])
        )
      else (
        ...
        let to_key path =
          ...
          else Filename.basename normalized
        in
        let matches =
          collect_files [] root
          |> List.rev
          |> List.filter_map (fun path ->
            ...
            if
              String_util.contains_substring lowered_key lowered_query
              || String_util.contains_substring lowered_content lowered_query
            then
              Some
                (`Assoc
                    [ "key", `String key; "preview", `String (preview_snippet content) ])
            else None)
          |> fun xs ->
          if List.length xs > 20 then List.filteri (fun idx _ -> idx < 20) xs else xs
        in
        ( true
        , Yojson.Safe.to_string
            (`Assoc
...
let tool_required_permission = function
  | "masc_team_memory_read" | "masc_team_memory_search" -> Some Types.CanReadState
  | "masc_team_memory_write" -> Some Types.CanBroadcast
  | _ -> None
;;
