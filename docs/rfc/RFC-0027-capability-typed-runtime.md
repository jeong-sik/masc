# RFC-0027: Capability-typed runtime catalog
...
Runtime catalog는 현재 capability-blind. `retired_coding_profile.fallback = primary` 같은 fallback chain이 keeper-bound runtime MCP HTTP header를 carry 못하는 provider (cli-tool-b / cli-tool-a) 다수를 포함해도 startup-time 검증이 없다. 결과: runtime에서 `no_tool_capable_provider` 사고가 발생할 때까지 silent.

본 RFC는 runtime catalog에 **declarative capability profile**을 도입하여 mismatch를 startup-time 정적 검증으로 변환한다.

## 2. Motivation

### 2.1 사고 사례 (2026-05-05)

3 keeper (`sangsu`, `masc-improver`, retired coding keeper)가 stuck. runtime `retired_coding_profile` (model = `provider-k-coding:auto`)의 fallback이 `primary`인데, primary의 `cli-tool-b:auto` (7 variants) + `cli-tool-a:model-d-spark`가 모두 `Runtime_mcp_http_headers_required` rejection. 통과한 cli-tool-d/provider-k-coding/provider-c가 있어도 turn semaphore 별도 사고로 막힘.

### 2.2 Root cause

- Runtime fallback chain은 capability를 인지하지 않음
- Provider별 capability는 `Provider_tool_support.capabilities` record에 5 fields로 이미 표현 가능하지만, runtime 측에서 "어떤 runtime는 어떤 capability를 요구한다"는 declarative 정보 없음
- Validator (`runtime_catalog_validator.ml`)는 spec/strategy/cycle은 검사하지만 capability mismatch는 검사 안 함
...
새 모듈 `lib/runtime/runtime_capability_profile.{ml,mli}`. 명명 profile 4개:
...
| 신규 | 용도 |
|---|---|
| `lib/runtime/runtime_capability_profile.mli` | Public API |
| `lib/runtime/runtime_capability_profile.ml` | Implementation |
| `test/test_runtime_capability_profile.ml` | Unit tests |
...
- `feedback_agent_prs_stay_draft`: agent-authored PRs stay Draft
