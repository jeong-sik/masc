  ...
  |> Http.Router.get "/api/v1/tasks" (fun request reqd ->
    with_read_auth
      (fun state req reqd ->
         ...
         let include_cancelled =
           bool_query_param req "include_cancelled" ~default:false
         in
         let limit =
         ...
         in
         let filtered =
           Room_protocol.tasks ?status_filter ~include_done ~include_cancelled config
         in
         let total = List.length filtered in
         let page =
           filtered |> List.filteri (fun idx _ -> idx >= offset && idx < offset + limit)
         in
         let tasks_json =
           List.map
             (fun (t : Types.task) ->
                let base_fields =
                  ...
                  ; ( "assignee"
                    , Json_util.string_opt_to_json (Room_protocol.task_assignee t) )
    ...
    with_read_auth
      (fun state req reqd ->
         ...
         let offset =
           int_query_param req "offset" ~default:0 |> clamp ~min_v:0 ~max_v:5000
         in
         let agents = Room_protocol.agents ?status_filter config in
         let total = List.length agents in
         let page =
           agents |> List.filteri (fun idx _ -> idx >= offset && idx < offset + limit)
         in
         let agents_json =
           List.map
             (fun (a : Types.agent) ->
