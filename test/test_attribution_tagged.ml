...
let test_det_policy_failed () =
  let t =
    AT.det_policy_failed ~gate:"worker_dev_tools" ~evidence:`Null ~reason:"rm not allowed"
  in
  let a = AT.to_attribution t in
  Alcotest.(check string) "origin" "det" (origin_str a.origin);
  match a.outcome with
  | A.Policy_failed { reason } -> Alcotest.(check string) "reason" "rm not allowed" reason
  | _ -> Alcotest.fail "expected Policy_failed"
;;

let test_det_transition_blocked () =
  let t =
    AT.det_transition_blocked
      ~gate:"keeper_fsm"
      ~evidence:`Null
      ~from_state:"Idle"
      ~to_state:"Running"
...
let test_nondet_partial_pass () =
  ...
  | A.Partial_pass { score; rationale } ->
    ...
    Alcotest.(check string)
      "rationale"
      "partial match — missing edge case coverage"
      rationale
  | _ -> Alcotest.fail "expected Partial_pass"
;;
...
let test_origin_of () =
  ...
  Alcotest.(check bool)
    "nondet"
    true
    (AT.origin_of (AT.nondet_passed ~gate:"x" ~evidence:(`Assoc []) ~rationale:"r")
     = A.NonDet)
;;
...
let test_nondet_wraps_non_object_evidence () =
  ...
  let t =
    AT.nondet_passed ~gate:"x" ~evidence:(`String "raw blob") ~rationale:"judged ok"
  in
  let a = AT.to_attribution t in
  match a.evidence with
  | `Assoc fields ->
    Alcotest.(check (option bool))
      "has original_evidence"
      (Some true)
...
let _example_det_site () = _consume_only_det (AT.det_passed ~gate:"x" ~evidence:`Null)
  ...
  Alcotest.run
    ...
    [ ( "det"
      , [ Alcotest.test_case "passed" `Quick test_det_passed
        ...
        ; Alcotest.test_case "transition_blocked" `Quick test_det_transition_blocked
        ; Alcotest.test_case
            "partial_pass (score-based rule)"
            `Quick
            test_det_partial_pass
        ] )
    ; ( "nondet"
      , [ Alcotest.test_case
            "passed embeds rationale in evidence"
            `Quick
            test_nondet_passed_embeds_rationale
        ; Alcotest.test_case
            "policy_failed keeps reason + rationale"
            `Quick
            test_nondet_policy_failed_keeps_both_reason_and_rationale
        ; Alcotest.test_case "partial_pass" `Quick test_nondet_partial_pass
        ; Alcotest.test_case
            "non-object evidence wrapped"
            `Quick
            test_nondet_wraps_non_object_evidence
        ] )
    ; ( "introspection"
      , [ Alcotest.test_case "origin_of returns runtime origin" `Quick test_origin_of ] )
    ]
;;
