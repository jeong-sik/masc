...
let test_write_read_search () =
  with_temp_dir "team-memory-room"
  @@ fun base_path ->
  Fs_compat.clear_fs ();
  Eio_main.run
  @@ fun env ->
  ...
  let read_json =
    json_result_exn
      (dispatch
         ...
         (`Assoc [ "room", `String "default"; "key", `String "notes/demo.md" ]))
  in
  let open Yojson.Safe.Util in
  check
    string
    "read content"
    "hello shared team memory"
  ...
  let search_json =
    json_result_exn
      (dispatch
         ...
         (`Assoc [ "room", `String "default"; "query", `String "shared" ]))
  in
  check
    int
    "search match count"
...
let test_traversal_blocked () =
  with_temp_dir "team-memory-traversal"
  @@ fun base_path ->
  Fs_compat.clear_fs ();
  Eio_main.run
  @@ fun env ->
  ...
  let ok, err =
    dispatch
      ...
      ~agent_name
      ~name:"masc_team_memory_read"
      (`Assoc [ "room", `String "default"; "key", `String "../outside.txt" ])
  in
  check bool "blocked" false ok;
  check bool "mentions traversal" true (String_util.contains_substring err "traversal")
;;

let test_secret_like_write_blocked () =
  with_temp_dir "team-memory-secret"
  @@ fun base_path ->
  Fs_compat.clear_fs ();
  Eio_main.run
  @@ fun env ->
  ...
  let ok, err =
    dispatch
      ...
      (`Assoc
          ...
          ; "content", `String "Authorization: Bearer sk-secret-token-1234567890"
          ])
  in
  check bool "blocked" false ok;
  check
...
let test_symlink_escape_blocked () =
  with_temp_dir "team-memory-symlink"
  @@ fun base_path ->
  Fs_compat.clear_fs ();
  Eio_main.run
  @@ fun env ->
  ...
  let ok, err =
    dispatch
      ...
      ~agent_name
      ~name:"masc_team_memory_read"
      (`Assoc [ "room", `String "default"; "key", `String "escape.txt" ])
  in
  check bool "blocked" false ok;
  check bool "mentions symlink" true (String_util.contains_substring err "symlink")
;;

let test_disabled_scope_blocked () =
  with_temp_dir "team-memory-disabled"
  @@ fun base_path ->
  Fs_compat.clear_fs ();
  Eio_main.run
  @@ fun env ->
...
let test_non_default_room_blocked () =
  ...
  let ok, err =
    dispatch
      ...
      ~agent_name
      ~name:"masc_team_memory_search"
      (`Assoc [ "room", `String "incident-alpha"; "query", `String "shared" ])
  in
  check bool "blocked" false ok;
  check
    bool
    "mentions default namespace"
    true
...
let () =
  run
    "team_memory"
    [ "crud", [ test_case "write read search" `Quick test_write_read_search ]
    ; ( "guards"
      , [ test_case "traversal blocked" `Quick test_traversal_blocked
        ; test_case "secret-like write blocked" `Quick test_secret_like_write_blocked
        ; test_case "symlink escape blocked" `Quick test_symlink_escape_blocked
        ; test_case "disabled scope blocked" `Quick test_disabled_scope_blocked
        ; test_case "non-keeper agent blocked" `Quick test_non_keeper_agent_blocked
        ; test_case "non-default room blocked" `Quick test_non_default_room_blocked
        ] )
    ]
;;
