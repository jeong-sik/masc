...
let test_state_to_string_crashed () =
  ...
  ignore (R.dispatch_event ~base_path:bp "k1" (KSM.Fiber_terminated { outcome = "test" }));
  match R.get ~base_path:bp "k1" with
  | None -> fail "expected k1"
  | Some e -> check string "state is crashed" "crashed" (KSM.phase_to_string e.phase)
;;
...
let test_state_to_string_dead () =
  ...
  match R.get ~base_path:bp "k1" with
  | None -> fail "expected k1"
  | Some e -> check string "state is dead" "dead" (KSM.phase_to_string e.phase)
;;
...
let test_running_count_crashed () =
  ...
  ignore (R.dispatch_event ~base_path:bp "a" (KSM.Fiber_terminated { outcome = "test" }));
  ...
  let s =
    R.failure_reason_to_string
      (R.Ambiguous_partial_commit
         { kind = R.Post_commit_timeout; detail = "turn outcome ambiguous" })
  in
  check
    string
    "ambiguous partial commit reason"
...
let test_failure_reason_ambiguous_partial_commit_kind_string () =
  check
    string
    "ambiguous partial commit kind string"
    "post_commit_timeout"
...
let test_failure_reason_stored () =
  ...
  (match R.get ~base_path:bp "k1" with
   ...
   | None -> fail "expected k1");
  (* Set a reason *)
  R.set_failure_reason ~base_path:bp "k1" (Some (R.Heartbeat_consecutive_failures 5));
  match R.get ~base_path:bp "k1" with
  | None -> fail "expected k1"
  | Some e ->
    check bool "has reason" true (Option.is_some e.last_failure_reason);
    (match e.last_failure_reason with
     | Some (R.Heartbeat_consecutive_failures n) -> check int "failure count" 5 n
...
let test_failure_reason_cleared_on_reregister () =
  R.clear ();
  let _e = R.register ~base_path:bp "k1" (make_meta "k1") in
  R.set_failure_reason ~base_path:bp "k1" (Some R.Fiber_unresolved);
  (* Re-register (simulates restart) clears the reason *)
  let _e2 = R.register ~base_path:bp "k1" (make_meta "k1") in
  match R.get ~base_path:bp "k1" with
  | None -> fail "expected k1"
  | Some e ->
    check bool "cleared on re-register" true (Option.is_none e.last_failure_reason)
;;
...
let test_cohort_key_heartbeat () =
  let key = Sup.cohort_key_of_reason (Some (R.Heartbeat_consecutive_failures 3)) in
...
let test_cohort_key_exception () =
  let key = Sup.cohort_key_of_reason (Some (R.Exception "Sys_error(disk full)")) in
  ...
  run
    ...
    ; ( "config"
      , [ test_case "ratio default" `Quick test_self_preservation_ratio_default
        ; test_case
            "min candidates default"
            `Quick
            test_self_preservation_min_candidates_default
    ...
    ; ( "state_transitions"
      , [ eio_test
            ...
            test_state_transition_running_crashed_running
