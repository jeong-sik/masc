  ...
  then []
    ...
    let trim_punct token =
      ...
      then token
        ...
        let rec left i =
          if i >= len then len else if is_punct token.[i] then left (i + 1) else i
        in
        let rec right i =
          if i < 0 then -1 else if is_punct token.[i] then right (i - 1) else i
        in
        let l = left 0 in
...
let cosine_similarity a b =
  let freq_add m token =
    let prev =
      match StringMap.find_opt token m with
      | Some n -> n
      | None -> 0
    in
  ...
  in
  let norm m =
    StringMap.fold (fun _token count acc -> acc +. float_of_int (count * count)) m 0.0
    |> sqrt
  in
  let na = norm fa in
  let nb = norm fb in
...
let classify_drift ~tokens_a ~tokens_b ~jacc ~cos =
  ...
  in
  if coverage < factual_coverage_floor () || size_ratio < factual_size_ratio_floor ()
  then Factual
  else if cos -. jacc > structural_divergence_threshold ()
  then Structural
  else Semantic
;;
...
let text_similarity original received =
  let summary, _, _ = summarize ~original ~received ~threshold:(default_threshold ()) in
  summary.similarity
;;
...
let verify_handoff ~original ~received ?threshold () =
  ...
  Heuristic_metrics.record
    { module_name = "drift_guard"
    ; site = "verify_handoff"
    ; raw_value = summary.similarity
    ; threshold
    ; triggered = summary.similarity < threshold
    ; provenance = Drift_guard "handoff"
    ; timestamp = Unix.gettimeofday ()
    };
  if summary.similarity >= threshold
  then Verified summary
  else (
    let drift_type =
      classify_drift ~tokens_a ~tokens_b ~jacc:summary.jaccard ~cos:summary.cosine
    in
    Drift_detected
...
let append_json_line path json = Fs_compat.append_jsonl path json

let verify_and_log config ~from_agent ~to_agent ~task_id ~original ~received ?threshold ()
  =
...
let get_drift_stats config ~days =
  ...
  then 0, 0, 0.0
    ...
    let cutoff = Time_compat.now () -. (float_of_int (max 0 days) *. 24.0 *. 3600.0) in
