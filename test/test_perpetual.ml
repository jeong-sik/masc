...
let test_llm_client () = group "LLM Client" (fun () ->
  ...
  (match Llm.model_spec_of_string "gemini:gemini-2.5-flash" with
   | Ok m ->
     assert_equal "parse_model:gemini_id" "gemini-2.5-flash" m.model_id;
     assert_true "parse_model:gemini_provider"
       (m.provider = Llm.Gemini)
   | Error _ -> assert_true "parse_model:gemini" false);

  (match Llm.model_spec_of_string "llama:qwen3.5-35b-a3b-ud-q8-xl:latest" with
   | Ok m ->
  ...
  (match Llm.model_spec_of_string "llama:qwen3.5-coder" with
   | Ok m ->
  ...
  (match Llm.model_spec_of_string "anthropic:sonnet" with
   | Ok m ->
  ...
  (match Llm.model_spec_of_string "google:flash" with
   | Ok m ->
  ...
  (match Llm.model_spec_of_string "claude-api:sonnet" with
   | Ok m ->
  ...
  (match Llm.model_spec_of_string "gemini-api:gemini-2.5-flash" with
   | Ok m ->
  ...
  let msg2 = Llm.tool_msg ~name:"grep" ~call_id:"c1" "results" in
  ...
  let tokens = Llm.estimate_tokens msgs in
  ...
  assert_true "builtin:llama_default_context"
    (Llm.llama_default.max_context = 128000);
  assert_true "builtin:claude_opus_cost"
    (Llm.claude_opus.cost_per_1k_input > 0.0);
  assert_true "builtin:gemini_pro_provider"
    (Llm.gemini_pro.provider = Llm.Gemini);
  assert_true "builtin:openai_default_provider"
    (Llm.openai_default.provider = Llm.OpenAI);
  assert_true "builtin:llama_default_provider"
    (Llm.llama_default.provider = Llm.Llama);
  ...
  let default_local = Llm.default_local_model_spec () in
  assert_true "builtin:default_local_provider"
    (default_local.provider = Llm.Llama);
...
let test_context_manager () = group "Context Manager" (fun () ->
  ...
  (* 7. PruneToolOutputs *)
  let long_tool_msg = { (Llm.tool_msg ~name:"t" ~call_id:"c" (String.make 1000 'x'))
...
let test_succession () = group "Succession" (fun () ->
  ...
  let msgs = [
    ...
    Llm.tool_msg ~name:"grep" ~call_id:"c1" "results";
    Agent_sdk.Types.assistant_msg "done";
  ] in
  let normalized = Succession_oas.normalize_for_model msgs Llm.llama_default in
...
let test_perpetual_loop () = group "Perpetual Loop" (fun () ->
  ...
  let config = Perpetual_loop.default_config
    ~goal:"test" ~models:[Llm.llama_default] () in
  ...
  let config3 = Perpetual_loop.default_config
    ...
    ~models:
      [
        Llm.llama_default;
        { Llm.llama_default with model_id = "qwen3.5-9b" };
      ]
    () in
...
let test_integration () = group "Integration" (fun () ->
  ...
  let tool_msg_rt = Llm.tool_msg ~name:"grep" ~call_id:"tc1" "search results" in
  ...
  let ctx_with_tools = Context_manager.append_many
    ...
    [Agent_sdk.Types.user_msg "run grep";
     Llm.tool_msg ~name:"grep" ~call_id:"c1" (String.make 800 'r');
  ...
  let tool_with_id = Llm.tool_msg ~name:"grep" ~call_id:"tc-42" "result" in
  ...
  (* 3e. Llm OAS type adapters *)
  let provider_config = Llm.to_oas_provider Llm.claude_opus in
  assert_true "oas_adapter:claude_mapped" (Option.is_some provider_config);
  let provider_config_custom = Llm.to_oas_provider
    { Llm.llama_default with provider = Llm.Custom "test" } in
  assert_true "oas_adapter:custom_mapped" (Option.is_some provider_config_custom);

  (* 3f. Llm message/usage roundtrip *)
  let test_msg = Agent_sdk.Types.user_msg "test" in
  (match Llm.to_oas_message test_msg with
   | None -> assert_true "oas_adapter:msg_roundtrip" false
   | Some oas_m ->
     let back_m = Llm.of_oas_message oas_m in
  ...
  let oas_u = Llm.to_oas_usage test_usage in
  let back_u = Llm.of_oas_usage oas_u in
...
let test_history_offload () = group "History Offload" (fun () ->
  ...
  let tool_msg = Llm.tool_msg ~name:"grep" ~call_id:"c1" "search results" in
