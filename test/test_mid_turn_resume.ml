...
let test_checkpoint_roundtrip () =
  ...
  Oas.Agent.set_state
    agent
    { (Oas.Agent.state agent) with messages = msgs; turn_count = 3 };
  ...
  Alcotest.(check (option string))
    ...
    resumed_state.config.system_prompt
    cp.system_prompt
;;
...
let test_multi_cascade_accumulation () =
  ...
  let agent_a =
    Oas.Agent.create
      ~net
      ~config:{ Oas.Types.default_config with name = "cascade-a"; model = "provider-a" }
      ()
  in
  Oas.Agent.set_state
    agent_a
    { (Oas.Agent.state agent_a) with
      messages =
        [ Oas.Types.user_msg "t1"
        ; { Oas.Types.role = Oas.Types.Assistant
          ; content = [ Oas.Types.Text "r1" ]
          ; name = None
          ; tool_call_id = None
          ; metadata = []
          }
  ...
  Oas.Agent.set_state
    agent_b
    { (Oas.Agent.state agent_b) with
      messages =
        (Oas.Agent.state agent_b).messages
        @ [ Oas.Types.user_msg "t3"
          ; { Oas.Types.role = Oas.Types.Assistant
            ; content = [ Oas.Types.Text "r3" ]
            ; name = None
            ; tool_call_id = None
            ; metadata = []
            }
...
let test_resume_preserves_checkpoint_model () =
  ...
  let agent =
    Oas.Agent.create
      ~net
      ~config:{ Oas.Types.default_config with model = "provider-a-model" }
      ()
  in
  Oas.Agent.set_state
    agent
    { (Oas.Agent.state agent) with
  ...
  Alcotest.(check string)
    "resumed keeps checkpoint model"
    "provider-a-model"
...
let () =
  ...
  Alcotest.run
    ...
    [ ( "checkpoint_threading"
      , [ Alcotest.test_case
            "roundtrip preserves turn state"
            `Quick
            test_checkpoint_roundtrip
        ; Alcotest.test_case
            "zero turns yields no checkpoint"
            `Quick
            test_zero_turns_no_checkpoint
        ; Alcotest.test_case
            "multi-cascade accumulation (A->B->C)"
            `Quick
            test_multi_cascade_accumulation
        ; Alcotest.test_case
            "resume preserves checkpoint model"
            `Quick
