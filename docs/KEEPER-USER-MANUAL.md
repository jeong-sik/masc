...
OAS는 `dune-project`에 `agent_sdk >= 0.90.0`으로 명시된 의존성이며, CI/runtime은 `scripts/oas-agent-sdk-pin.sh`에 기록된 upstream `main` 커밋(`088a7346537339cde8a01a4b973d1bb229fdc9ef`)으로 고정한다. 현재 runtime pin은 최신 태그 `v0.90.0`보다 앞선 `main` head를 따르며, 버전 floor는 최신 태그 기준인 `0.90.0`으로 유지한다. `jeong-sik/oas` 저장소는 GitHub release 메타데이터가 늦게 반영될 수 있으므로, 최신성 검증은 release API 대신 upstream head 조회(`git ls-remote ... refs/heads/main`)를 기준으로 수행한다.
