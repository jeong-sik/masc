...
OAS는 `dune-project`에 `agent_sdk >= 0.118.0`으로 명시된 의존성이며, CI/runtime pin은 `scripts/oas-agent-sdk-pin.sh`의 base tag와 commit SHA를 기준으로 관리한다. 최신성 검증이 필요할 때는 문서에 적힌 숫자보다 `dune-project`와 pin script를 우선 truth source로 본다.
