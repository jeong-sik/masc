...
let labels_for_turn meta =
  with_worktree_config_root
  @@ fun () -> Eio_main.run @@ fun _env -> KEC.effective_model_labels_for_turn meta
;;
...
let test_matching_last_model_is_preserved_when_still_in_cascade () =
  ...
  | first :: _ ->
    ...
    (match labels with
     ...
     | actual_first :: _ -> check string "matching model stays first" first actual_first)
;;

let test_explicit_models_override_cascade_resolution () =
  let explicit = [ "ollama:qwen3.5:35b-a3b-nvfp4"; "glm-coding:glm-5.1" ] in
  let labels = labels_for_turn (make_meta ~last_model_used:"" ~models:explicit ()) in
...
let () =
  run
    ...
    [ ( "effective_model_labels_for_turn"
      , [ test_case
            "drops stale glm pin outside current cascade"
            `Quick
            test_stale_last_model_is_not_reused_outside_current_cascade
        ; test_case
            "keeps llama pin when still allowed"
            `Quick
            test_matching_last_model_is_preserved_when_still_in_cascade
        ; test_case
            "prefers explicit models over cascade defaults"
            `Quick
