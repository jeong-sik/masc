...
- Keeper: delete `<base-path>/.masc/keepers/*/turn-records` before installing. A turn record's window now carries `front_atom_digest`, and the decoder requires that key on every record, so no record an earlier release wrote decodes. The deploy preflight decodes every row and stops before the install when one is refused, so a missed reset stops the deploy instead of the running server. The dashboard's turn history, the raw traces nothing references any more (the first retention sweep collects them) and each keeper's first-turn seed go with the records, so each keeper's first turn after the upgrade sends its whole history. Rolling back to an older binary needs the same reset, because the older decoder refuses the new key as unknown; there is no compatibility reader or strip script (#36955).
...
- Runtime: a keeper runs only on the runtimes its lane names. `[runtime].default` is no longer appended to every lane, and an assignment that names a runtime no lane declares walks that runtime alone; `[runtime].default` only names the runtime of a keeper with no assignment (#37064). An image turn picks among the lane's own candidates, and when none of them takes images the turn runs on the lane with the image described in text; `[runtime].media_failover` is only the list `analyze_image` and the image describer read (#37066). A direct message turn that ends with no progress is no longer run again on every tool-capable runtime in the catalog once its lane is walked (#37069). A keeper that should fail over to another runtime needs a `[runtime.lanes]` entry listing it. What only that rotation filled goes with it: `masc_keeper_runtime_rotation_total`, the `source="fallback"` series of `masc_keeper_runtime_selected_total`, the "Fallback Reasons" and "Runtime Rotations / min" Grafana panels, the receipt's `rotation_attempts`, and the `keeper.is_retry` span attribute (#37081, #37082).
- Config: delete `<base-path>/.masc/config/agent-core-models-overlay.toml`. Nothing loads it — neither the model catalog nor the exact-output resolver — and no boot error names it, because no reader is left to raise one; a copy left in the config root sits there looking live. Provider and model facts live in AGENT_CORE's embedded catalog, which now carries the rows deployments kept in their overlays, including `glm-coding` `glm-5.3` and `glm-5.3-flash`, `ollama_cloud` `glm-5.3-flash`, `deepseek-v4.1-flash` and `kimi-k3`, and the `kimi_coding` provider with `k3` and `kimi-for-coding`. A deployment names the rows it uses in `runtime.toml`; a model the embedded catalog does not carry is declared with a `[models.<id>.capabilities]` block there, and without one the capability gate refuses the binding at boot (#37016, #37009, #37013).
- Setup: delete the `setup_*` provider, model and binding entries the install wizard wrote from `runtime.toml` before running the wizard again. A connection's id now comes from the parsed answers rather than the JSON that carried them, so the same answers produce a different `setup_*` id; the wizard appends the new id and leaves the old entry in place, which then fails the capability gate at boot while the new one works (#37016).
- Exact output: a lane slot is a `runtime.toml` binding id (`<provider>.<model>`) resolved against the AGENT_CORE catalog, and its only deadline is the provider's `connect-timeout-s`. A slot whose provider declares no `connect-timeout-s` is refused at plan admission as `missing_deadline` before any request goes out, and a lane whose every slot is refused fails with that reason, so set `connect-timeout-s` on every provider an exact-output lane names. A slot that names no binding the catalog resolves is ignored with a WARN at boot, and a subscription CLI runtime is not an exact-output slot — name it in `cli_slots` (#36984, #37009, #37016).
- Exact output: `[runtime.exact_output_lanes.verifier_exact].slots` takes runtime ids only. A lane name there is a load error naming the slot's position; it used to load and then fail every judgement, because verification dispatches the slot id itself (#37075).
- Sandbox: `MASC_KEEPER_MICROVM_MEMORY` takes a size with a unit, `<n>m` or `<n>g` (`512m`, `8g`); a bare number, which the container runtimes read as bytes, is refused. `MASC_KEEPER_MICROVM_CPUS` takes a positive integer. A value that does not parse stops a microVM keeper's guest from starting, with the variable named, where it used to be handed to the runtime as written. Unset, a guest boots with `2g` and 4 CPUs (#36973).
- Board attention: set `MASC_TYPESAFEAI_ENABLED=false` if the server's environment carries `TYPESAFEAI_API_KEY` and Board posts should not leave the machine. A non-blank key alone turns on a first pass that sends the Board post and the keeper's context to TypeSafe AI's Jev (`https://api.typesafe.ai/v1/systemone`, overridable with `MASC_TYPESAFEAI_ENDPOINT`). Without the key nothing changes (#36970).

### Added

- Runtime: `POST /api/v1/runtime/config/routing` takes an `action`. `set`, the default, writes the candidates of an existing lane or runtime and still refuses an unknown name. `create` makes a new lane and refuses a name that is already a lane, a declared runtime id, or another route name (`default`, `media_failover`, `exact/...`). `remove` deletes a declared lane and refuses, naming each one, while a `[runtime.assignments]` keeper or `[runtime].default` still routes to it; a lane written as an inline table is refused. All three take the same lock, whole-file validation, atomic write and audit record (#37075).
...
- Keeper: `<keeper>.turn-boundaries.jsonl` records where each finished turn left the durable history (`turn_ended`, with the atom count and the digest of the last atom), and a `history_restarted` line after `masc_keeper_clear`, at the start of a turn on an empty history, and after the first accepted save of a turn that could not read its checkpoint. Nothing reads the file yet. It is append-only, never trimmed, and removed with a keeper purge. A line that cannot be written never fails the turn or the clear; it is logged at ERROR and counted in `masc_keeper_turn_boundary_failures_total` (#37020, #37024, #37027, #37030).
- Board attention: with `TYPESAFEAI_API_KEY` set, TypeSafe AI's Jev answers first. A `relevant` answer is kept without an LLM call, and `not_relevant`, a failed call or an answer that does not decode goes to the existing LLM slots. The flow's terminal log entry carries a `jev` object saying what Jev answered and, after `not_relevant`, what the LLM lane decided (#36970).
...
- Keeper: a turn whose checkpoint cannot be read ends with that error, before a prompt is built or a model called, instead of starting from an empty history that the turn's save would then write over the file. A missing checkpoint and an explicit version cut still start fresh (#37089).
...
- Antigravity: the lane hands over the whole history instead of cutting it to `max-prompt-bytes`, and reports the window as whole, because agy 1.2.6 passed a 2,078,915-byte prompt through intact (#37007). The answer is shown as the model writes it: the `text_delta` of each `step_update` is drawn instead of the whole response at the end (#37002).
...
- Keeper: a refusal whose reason MASC cannot classify narrows the carried range like a classified overflow, and the narrowed front lasts into the next turn (#36977). A refusal that was not about size moves the front the same way, and a front only moves toward the newest atom, so that keeper does not carry the atoms it passed again. To carry them again, stop the server, delete `<base-path>/.masc/keepers/<keeper>/turn-records`, and start it; that keeper's turn history on the dashboard goes with the records. The next candidate in the same turn starts from the front a refusal moved instead of the whole history (#36986), and the turn records Claude Code, Codex and Antigravity leave are read as fronts (#36997).
