...
let test_tool_get () =
  let session = Session.McpSessionStore.create () in
  let args = `Assoc [ "action", `String "get"; "session_id", `String session.id ] in
  let success, result = Session.handle_mcp_session_tool args in
...
let test_tool_get_not_found () =
  let args = `Assoc [ "action", `String "get"; "session_id", `String "nonexistent" ] in
...
let test_tool_remove () =
  let session = Session.McpSessionStore.create () in
  let args = `Assoc [ "action", `String "remove"; "session_id", `String session.id ] in
  let success, _ = Session.handle_mcp_session_tool args in
...
let json_tests = [ "session_to_json", `Quick, test_session_to_json ]
