...
let handle_run_init ctx args : tool_result =
  ...
  then false, "task_id is required"
    ...
    | Ok run -> true, Yojson.Safe.to_string (Run_eio.run_record_to_json run)
    | Error e -> false, Printf.sprintf "❌ Failed to init run: %s" e)
;;
...
let handle_run_plan ctx args : tool_result =
  ...
  then false, "task_id is required"
    ...
    | Ok run -> true, Yojson.Safe.to_string (Run_eio.run_record_to_json run)
    | Error e -> false, Printf.sprintf "❌ Failed to update run plan: %s" e)
;;
...
let handle_run_log ctx args : tool_result =
  ...
  then false, "task_id is required"
    ...
    | Ok entry -> true, Yojson.Safe.to_string (Run_eio.log_entry_to_json entry)
    | Error e -> false, Printf.sprintf "❌ Failed to append run log: %s" e)
;;
...
let handle_run_deliverable ctx args : tool_result =
  ...
  then false, "task_id is required"
    ...
    | Ok run -> true, Yojson.Safe.to_string (Run_eio.run_record_to_json run)
    | Error e -> false, Printf.sprintf "❌ Failed to set run deliverable: %s" e)
;;
...
let schemas : Types.tool_schema list =
  [ (* masc_run_init *)
    { name = "masc_run_init"
    ; description =
        "Create an execution memory directory (.masc/runs/{task_id}/) to track plan, \
         logs, and deliverables. Call when starting work on a claimed task to enable \
         structured progress tracking. After init, use masc_run_plan to set approach, \
         masc_run_log for notes, masc_run_deliverable to close."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
  ...
  ; (* masc_run_plan *)
    { name = "masc_run_plan"
    ; description =
        "Set or update the execution plan (markdown) for a task run; each update creates \
         a new revision. \\nCall after masc_run_init to document your approach before \
         starting implementation. \\nOther agents can view plans via masc_run_get for \
         coordination and handoff context."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
  ...
  ; (* masc_run_log *)
    { name = "masc_run_log"
    ; description =
        "Append a timestamped note (ISO8601) to a task's execution log for audit and \
         handoff continuity. \\nCall when reaching milestones, finding blockers, or \
         making key decisions during task execution. \\nPair with masc_run_plan for the \
         approach and masc_run_get to review the full log."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
  ...
  ; (* masc_run_deliverable *)
    ...
    ; description =
        "Record the final deliverable (markdown) and mark the task run as completed. \
         Call when task implementation is finished and verified to close out the \
         execution record. After recording, the run shows as completed in masc_run_list \
         and masc_run_get."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
  ...
  ; (* masc_run_get *)
    { name = "masc_run_get"
    ; description =
        "Retrieve full execution history (plan, timestamped logs, deliverable) for a \
         task as markdown. \\nUse when resuming work on a task, reviewing progress, or \
         preparing a handoff. \\nPair with masc_run_list to find task IDs, masc_run_log \
         to add entries."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
  ...
  ; (* masc_run_list *)
    { name = "masc_run_list"
    ; description =
        "List all task runs with their status (active/completed), plan presence, and log \
         count. \\nUse when starting a session to find abandoned work or review \
