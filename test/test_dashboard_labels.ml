...
let test_working_agent () =
  ...
  let result = Lib.Dashboard_labels.translate_agent_status ~now Types.Active recent_iso in
  Alcotest.(check string) "active+recent = working" "working" result
;;

let test_quiet_threshold_override () =
  with_dashboard_label_thresholds ~quiet:30.0 ~stuck:900.0
  @@ fun () ->
  let now, quiet_iso = make_timestamp_pair 60.0 in
  let result = Lib.Dashboard_labels.translate_agent_status ~now Types.Active quiet_iso in
  Alcotest.(check bool)
    "override surfaces quiet warning"
    true
    (try
...
let test_stuck_agent () =
  let now, old_iso = make_timestamp_pair 1200.0 in
  (* 20 minutes ago *)
  let result = Lib.Dashboard_labels.translate_agent_status ~now Types.Active old_iso in
  Alcotest.(check bool)
    "stuck agent contains STUCK"
    true
    (try
...
let test_stuck_threshold_override () =
  with_dashboard_label_thresholds ~quiet:300.0 ~stuck:60.0
  @@ fun () ->
  let now, stuck_iso = make_timestamp_pair 120.0 in
  let result = Lib.Dashboard_labels.translate_agent_status ~now Types.Busy stuck_iso in
  Alcotest.(check bool)
    "override surfaces stuck warning"
    true
    (try
       ignore (Str.search_forward (Str.regexp_string "STUCK") result 0);
       true
...
let test_parse_iso_timestamp_matches_canonical_utc () =
  ...
  match Lib.Dashboard_labels.parse_iso_timestamp ts, Types.parse_iso8601_opt ts with
  | Some actual, Some expected ->
    Alcotest.(check bool)
      "dashboard parser matches canonical UTC parser"
      true
...
let test_parse_iso_timestamp_fractional_utc_normalizes () =
  ...
  | Some actual, Some expected ->
    Alcotest.(check bool)
      "fractional UTC keeps sub-second precision"
      true
      (abs_float (actual -. (expected +. 0.123)) < 0.001)
  | _ -> Alcotest.fail "expected dashboard parser to preserve fractional UTC timestamp"
;;
...
let test_parse_iso_timestamp_offset_matches_utc () =
  ...
  | Some actual, Some expected ->
    Alcotest.(check bool)
      "numeric offset normalizes to UTC"
      true
      (abs_float (actual -. expected) < 0.001)
...
let test_parse_iso_timestamp_fractional_offset_matches_utc () =
  ...
  | Some actual, Some expected ->
    Alcotest.(check bool)
      "fractional offset keeps sub-second precision"
      true
      (abs_float (actual -. (expected +. 0.250)) < 0.001)
  | _ -> Alcotest.fail "expected dashboard parser to preserve fractional timezone offsets"
;;
...
let test_parse_iso_timestamp_local_without_timezone_is_supported () =
  ...
  | Some actual ->
    ...
    Alcotest.(check bool)
      "bare local timestamps stay parseable"
      true
      (abs_float (actual -. (expected +. 0.125)) < 0.001)
  | None -> Alcotest.fail "expected dashboard parser to accept bare local timestamps"
;;
...
let test_parse_iso_timestamp_partial_rejected () =
  Alcotest.(check (option (float 0.001)))
    "partial timestamps are rejected"
    None
...
let test_idle_agent () =
  ...
  let result =
    Lib.Dashboard_labels.translate_agent_status
      ~now
      Types.Listening
...
let test_offline_agent () =
  let now = Unix.gettimeofday () in
  let result =
    Lib.Dashboard_labels.translate_agent_status ~now Types.Inactive "2026-01-01T00:00:00Z"
  in
...
let test_classify_uses_stuck_threshold_override () =
  with_dashboard_label_thresholds ~stuck:60.0
  @@ fun () ->
  let now, stuck_iso = make_timestamp_pair 120.0 in
  let agent : Types.agent =
    { id = None
  ...
  let group = Lib.Dashboard_labels.classify_agent ~now agent in
  Alcotest.(check bool)
    "active can classify as Stuck via override"
    true
...
let test_attention_empty () =
  let now = Unix.gettimeofday () in
  let items = Lib.Dashboard_attention.collect ~now [] in
  ...
  Alcotest.run
    ...
    [ ( "Agent Status"
      , [ "working agent", `Quick, test_working_agent
        ...
        ; ( "utc parser matches canonical"
          , `Quick
          , test_parse_iso_timestamp_matches_canonical_utc )
        ; ( "fractional utc normalizes"
          , `Quick
          , test_parse_iso_timestamp_fractional_utc_normalizes )
        ; "numeric offset normalizes", `Quick, test_parse_iso_timestamp_offset_matches_utc
        ; ( "fractional offset normalizes"
          , `Quick
        ...
        ; "empty timestamp rejected", `Quick, test_parse_iso_timestamp_empty_rejected
        ; "garbage timestamp rejected", `Quick, test_parse_iso_timestamp_garbage_rejected
        ; "partial timestamp rejected", `Quick, test_parse_iso_timestamp_partial_rejected
