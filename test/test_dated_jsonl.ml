...
let make_json i = `Assoc [ "i", `Int i; "ts", `Float (Unix.gettimeofday ()) ]
...
let test_read_recent () =
  ...
  let values = List.map (fun j -> Yojson.Safe.Util.(j |> member "i" |> to_int)) result in
...
let test_count_entries () =
  ...
  Fs_compat.append_file (Filename.concat old_month "15.jsonl") "{\"i\":4}\n\n{\"i\":5}\n";
  check int "counts non-empty rows across dated files" 5 (Dated_jsonl.count_entries store)
;;
...
let test_prune () =
  ...
  let old_month = Filename.concat dir "2020-01" in
  Fs_compat.mkdir_p old_month;
  Fs_compat.append_file (Filename.concat old_month "15.jsonl") "{\"old\":true}\n";
...
let test_concurrent_append () =
  ...
  let store = Dated_jsonl.create ~base_dir:dir () in
  let n = 50 in
  Eio.Fiber.all (List.init n (fun i -> fun () -> Dated_jsonl.append store (make_json i)));
...
let () =
  run
    "Dated_jsonl"
    [ "append", [ test_case "creates dated file" `Quick test_append_creates_dated_file ]
    ; ( "read_recent"
      , [ test_case "returns newest N chronological" `Quick test_read_recent
        ...
        ; test_case
            "drops partial chunk prefix"
            `Quick
            test_load_tail_lines_drops_partial_chunk_prefix
        ; test_case
            "keeps first data row after blank partial prefix"
            `Quick
            test_load_tail_lines_keeps_first_data_after_blank_prefix
        ; test_case
            "keeps first row when full file spans chunks"
            `Quick
    ...
    ; "concurrent", [ test_case "concurrent append" `Quick test_concurrent_append ]
    ; "empty", [ test_case "empty store" `Quick test_empty_store ]
    ]
;;
