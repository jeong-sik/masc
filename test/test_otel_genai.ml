...
let test_tool_execution_attrs () =
  let attrs = Lib.Otel_genai.tool_execution_attrs ~tool_name:"tool_workspace_inspect" in
  check
    (option (of_pp Fmt.Dump.string))
    "tool operation"
    (Some "execute_tool")
...
let test_dispatch_hook_emits_tool_span_payload () =
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      Lib.Otel_dispatch_hook.with_test_span_emitter
        ...
        (fun () ->
          ...
          let result : Tool_result.result =
            Ok
              { Tool_result.tool_name = "tool_workspace_inspect"
              ; data = `String "ok"
              ; duration_ms = 123.4
              }
          in
      ...
      match !spans with
      | [ (name, attrs) ] ->
          check string "span name" "tool/tool_workspace_inspect" name;
          check
            (option (of_pp Fmt.Dump.string))
            "tool operation"
            (Some "execute_tool")
