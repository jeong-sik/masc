...
- **Phase 2b — purge** (**merged to main** #22052): deleted prompt protocol + types lib + facade + registry + impl + protocol + TOML validation; removed request-help post + reply-suppression arms; folded §3.3. The OCaml closed-sum deletion forced the compiler to enumerate every consumer. Also removed `SocialStateCap.tla` and the orphan `test_keeper_toml.ml`. Done.
- **Phase 2c — clean-up** (this PR): removed the residual `/bdi-snapshot` endpoint chain (the `keeper_bdi_snapshot_json` projection + its three private metric/token/tool-call helpers in `dashboard_http_keeper_snapshot`, the route branch in `server_dashboard_http_keeper_api`, the suffix const, and the facade `.mli` vals) and the frontend BDI dashboard subsystem (`inspector-keeper-bdi`, `bdi-snapshot-trace-bridge`, and the `bdi-snapshot` member of the `KeeperTraceSource` closed union — the TS compiler then enumerated every overlay/gutter/store consumer). Re-grounded the four retired-protocol comments off the dead `delivery_surface`/`speech_act` tokens (the §3.2 `turn_delivery` replacement is unchanged) and trimmed the dead `speech_act`/`belief_summary` decision-JSONL test fixtures. The blocker/need data the endpoint exposed survives on the independent `blocker_class` operator surface (§3.3) and `keeper_config_json`; the persona `KeeperBDIPanel` (will/needs/desires self-model) is a separate live feature, untouched. Done.
