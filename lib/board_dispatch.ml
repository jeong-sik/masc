...
type board_backend = Jsonl of Board.store
...
let start_flusher_actor ~sw store =
  Eio.Fiber.fork_daemon ~sw (fun () ->
    ...
    while true do
      ...
      | Board_types.Flush ->
        (try Board.flush_dirty store with
...
let ensure_flusher_actor store =
  ...
  | None -> ()
  | Some sw ->
    if Atomic.compare_and_set flusher_started false true
    then (
      try start_flusher_actor ~sw store with
      | exn ->
        Atomic.set flusher_started false;
        (match exn with
         | Invalid_argument msg when String.equal msg "Switch finished!" ->
           Log.BoardLog.warn "Skipping board flusher actor startup on finished switch"
         | _ -> raise exn))
;;

let keeper_board_signal_hook : (keeper_board_signal -> unit) option Atomic.t =
  Atomic.make None
;;

let set_keeper_board_signal_hook hook = Atomic.set keeper_board_signal_hook (Some hook)
...
let set_board_sse_hook hook = Atomic.set board_sse_hook (Some hook)
...
let init_jsonl () =
  if
    ...
    | Active _ -> true
    | Uninitialized -> false
  then Log.BoardLog.warn "already initialized, ignoring init_jsonl"
  else (
    let store = Board.global () in
    let backend = Active (Jsonl store) in
    if Atomic.compare_and_set backend_state Uninitialized backend
    then (
      ensure_flusher_actor store;
      Log.BoardLog.info "JSONL backend initialized")
    else Log.BoardLog.warn "already initialized concurrently, ignoring init_jsonl")
;;
...
let sort_posts_in_memory ~sort_by (posts : Board.post list) =
  ...
  | Recent ->
    List.sort
      (fun (a : Board.post) (b : Board.post) -> compare b.created_at a.created_at)
      posts
  | Updated ->
    List.sort
      (fun (a : Board.post) (b : Board.post) -> compare b.updated_at a.updated_at)
      posts
  | Trending ->
    ...
    List.sort
      (fun (a : Board.post) (b : Board.post) ->
         ...
         let score_a =
           float_of_int (a.votes_up - a.votes_down + (a.reply_count * 2)) /. (age_a ** 0.5)
         in
         let score_b =
           float_of_int (b.votes_up - b.votes_down + (b.reply_count * 2)) /. (age_b ** 0.5)
         in
...
let matching_post_ids_for_comment_author_filter ~needle (comments : Board.comment list) =
  ...
  List.iter
    (fun (comment : Board.comment) ->
       if agent_matches_author_filter ~needle comment.author
       then Hashtbl.replace matches (Board.Post_id.to_string comment.post_id) true)
    comments;
  matches
;;
...
let list_posts
  ...
  let apply_post_kind_filter posts =
    ...
    | Some kind -> List.filter (fun (p : Board.post) -> Board.classify_post_kind p = kind)
    | None -> Fun.id
  in
  match backend () with
  | Jsonl store ->
    ...
    in
    let fetch_limit = if needs_full_scan then Board.Limits.max_posts else max limit 200 in
    let posts =
      if needs_full_scan
      then Board.search_posts store ~predicate:(fun _ -> true) ~limit:fetch_limit
      else Board.list_posts store ~visibility_filter ?hearth ~limit:fetch_limit ()
    in
    let sorted =
      posts |> apply_visibility_and_hearth_filters |> sort_posts_in_memory ~sort_by
    in
      ...
      | Some needle ->
        ...
        List.filter
          (fun (post : Board.post) ->
             ...
             || Hashtbl.mem matching_comment_post_ids (Board.Post_id.to_string post.id))
          filtered
    in
    ...
    (match Board.add_comment store ~post_id ~author ~content ?parent_id ~ttl_hours () with
     | Ok comment as ok ->
       ...
       (match Board.get_post store ~post_id with
        ...
        | Error e ->
          Log.BoardLog.warn
            "board signal skipped: get_post failed for %s: %s"
            post_id
            (Board_types.show_board_error e));
       emit_board_sse_event (Comment_added { post_id; comment_id = cid; author = auth });
       ok
     | Error _ as err -> err)
;;
