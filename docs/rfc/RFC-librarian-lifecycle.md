...
1. **턴 끝 기록** `<keepers_dir>/<keeper>.turn-boundaries.jsonl`
   ...
   - 줄은 비운 checkpoint 가 저장된 **뒤에만** 쓴다. 저장이 stale no-op 이거나 실패하면 줄을 쓰지 않고 도구는 오류를 돌려준다(전에는 둘 다 "N개 비움"으로 보고했다). 줄을 먼저 쓰면 안 되는 이유: 표식은 보이는데 이력은 아직 그대로인 순간에 읽는 쪽이 회차를 끝내면, 옛 이력을 0 부터 다시 읽고 그 표식을 지나간 것으로 친다. 그 뒤에 이력이 비워지면 불일치를 설명할 줄이 남지 않는다.
   ...
   - 저장은 됐는데 줄을 못 쓰면 도구가 그 사실을 오류로 돌려준다. 일시적인 실패였다면 바로 다시 실행해서 줄을 쓴다. 이미 빈 이력을 같은 `turn_count` 로 다시 저장하는 것은 통과하기 때문이다. 파일 끝이 잘려 있으면 append 가 전부 거절되므로 다시 실행해도 같은 오류다. 그 파일은 4단계의 기동 복구가 들어오기 전에는 고쳐지지 않는다. 그 사이에 턴이 돌았으면 다시 실행은 새로 쌓인 이력도 비운다.
