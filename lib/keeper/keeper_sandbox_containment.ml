...
let check_target ~operation ~config ~meta ~target =
  ...
  then Ok ()
    ...
    if target_is_inside_playground ~playground ~target:target_norm
    then Ok ()
    else
      Error
        (Printf.sprintf
           "symmetric_sandbox_blocked: target %s is outside keeper playground %s. \
            Keepers with sandbox_profile=docker may only %s inside their playground. \
            Clone the source into your playground via keeper_shell op=git_clone, or \
            operate inside %s/repos/."
           target_norm
           playground
           operation
           playground))
