...
let table : (string, (Attribution.t * float) Queue.t) Hashtbl.t = Hashtbl.create 8
...
let recent ?gate ?(limit = 50) () =
  ...
  with_lock (fun () ->
    ...
    | None ->
      let all =
        Hashtbl.fold (fun _ q acc -> Queue.fold (fun acc x -> x :: acc) acc q) table []
      in
      (* Sort by timestamp descending. *)
      let sorted = List.sort (fun (_, t1) (_, t2) -> compare t2 t1) all in
      let rec take n = function
        | [] -> []
        | _ when n <= 0 -> []
        | x :: xs -> x :: take (n - 1) xs
