...
let counter_for ~keeper ~source =
  Prom.metric_value_or_zero metric_name ~labels:[ "keeper", keeper; "source", source ] ()
;;
...
let test_empty_raw_falls_back_to_telemetry () =
  ...
  let telemetry = make_telemetry ~canonical_model_id:"anthropic:claude-opus-4-7" () in
...
let test_empty_everywhere_uses_sentinel () =
  ...
  let resolved = Hooks.resolve_after_turn_model ~keeper_name:keeper ~response in
  Alcotest.(check string) "sentinel returned" Hooks.unknown_model_sentinel resolved;
  Alcotest.(check string)
...
let test_empty_canonical_id_falls_through_to_sentinel () =
  ...
  let response = make_response ~model:"" ~telemetry () in
  let resolved = Hooks.resolve_after_turn_model ~keeper_name:keeper ~response in
  Alcotest.(check string) "falls through to sentinel" "unknown_provider" resolved;
  Alcotest.(check (float 0.0001))
    "unknown_sentinel counter +1"
    (before_sentinel +. 1.0)
    (counter_for ~keeper ~source:"unknown_sentinel");
...
let test_auto_raw_falls_back_to_canonical_model_id () =
  ...
  let before = alias_counter_for ~keeper ~alias:"auto" ~source:"telemetry_canonical" in
  ...
  Alcotest.run
    ...
    [ ( "fallback_tiers"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "empty canonical_model_id falls through"
            `Quick
            test_empty_canonical_id_falls_through_to_sentinel
        ; Alcotest.test_case
            "raw auto -> telemetry canonical"
            `Quick
            test_auto_raw_falls_back_to_canonical_model_id
        ; Alcotest.test_case
            "provider-prefixed auto -> telemetry canonical"
            `Quick
