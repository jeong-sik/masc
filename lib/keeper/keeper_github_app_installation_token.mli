...
val mint :
  app_id:string -> installation_id:string -> pem:string -> now:int -> unit -> (string, string) result
(** [mint ~app_id ~installation_id ~pem ~now ()] ignores the cache and requests
    a fresh installation token. Signs the App JWT via
...
(** [get] returns a cached token whose 55-minute validity window (1h lifetime
    ...
    token otherwise. The cache is keyed by [app_id ^ ":" ^ installation_id] and
    guarded by a process-wide mutex so concurrent keeper projections serialize
    only on the same installation (different installations proceed in
    parallel). [pem] is consumed only on a mint. *)
