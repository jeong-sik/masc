...
let test_same_stage () =
  check
    bool
    "decompose → decompose (idempotent)"
    true
...
let test_validate_transition_error () =
  ...
  | Error msg -> check bool "error mentions backward" true (String.length msg > 0)
...
let test_task_without_stage () =
  ...
  let json = Types_core.task_to_yojson task in
  match Types_core.task_of_yojson json with
  | Ok decoded -> check bool "stage is None" true (Option.is_none decoded.stage)
  | Error e -> fail e
;;
