...
let assert_float_equal msg ~epsilon expected actual =
  if abs_float (expected -. actual) > epsilon
  then
    failwith
      (Printf.sprintf
...
let test_error_with_data () =
  ...
  let resp = Response.error ~data ~code:"WITH_DATA" ~message:"Error with data" () in
...
let test_severity_of_string () =
  ...
  assert_true
    "warning parses"
    (Response.severity_of_string "warning" = Ok Response.Warning);
  assert_true "info parses" (Response.severity_of_string "info" = Ok Response.Info);
  assert_true
    "unknown returns Error"
    (Result.is_error (Response.severity_of_string "unknown"));
  (* Test backwards-compatible default function *)
  assert_true
    "default fallback"
    (Response.severity_of_string_default "unknown" = Response.Info);
  assert_true
    "default with custom"
    (Response.severity_of_string_default ~default:Response.Fatal "bad" = Response.Fatal);
...
let test_ok_with_warnings () =
  ...
  let resp =
    Response.ok_with_warnings
      ~message:"OK but warned"
      ~warnings:[ warning ]
      (`String "data")
  in
  assert_true "success is true" resp.success;
...
let test_drift_detected () =
  let resp =
    Response.drift_detected
      ...
      ~drift_type:"factual"
      ~threshold:0.85
      ~details:"Content mismatch detected"
  in
  ...
  assert_float_equal
    ...
    ~epsilon:0.001
    0.72
    (json_get_float resp.data "similarity");
  assert_equal_string
    "drift_type value"
    "factual"
    (json_get_string resp.data "drift_type");
  assert_float_equal
    "threshold value"
    ~epsilon:0.001
    0.85
    (json_get_float resp.data "threshold");
  ...
  assert_true
    "factual hint mentions 'source agent'"
    (try Str.search_forward (Str.regexp_case_fold "source agent") hints_concat 0 >= 0 with
     | Not_found -> false);
...
let test_drift_detected_semantic () =
  ...
  let hints_concat = String.concat " " err.recovery_hints in
  assert_true
    "hint mentions 'threshold'"
    (try Str.search_forward (Str.regexp "threshold") hints_concat 0 >= 0 with
     | Not_found -> false);
  assert_true
    "hint contains threshold value 0.85"
    (try Str.search_forward (Str.regexp "0\\.85") hints_concat 0 >= 0 with
     | Not_found -> false);
...
let test_handoff_verified () =
  ...
  assert_true
    "message contains 95%"
    (try Str.search_forward (Str.regexp "95") resp.message 0 >= 0 with
     | Not_found -> false);
...
let test_task_claimed () =
  ...
  assert_true
    "message mentions task-001"
    (try Str.search_forward (Str.regexp "task-001") resp.message 0 >= 0 with
     | Not_found -> false);
  assert_true
    "message mentions claude"
    (try Str.search_forward (Str.regexp "claude") resp.message 0 >= 0 with
     | Not_found -> false);
...
let test_task_already_claimed () =
  ...
  assert_equal_string
    "claimed_by in data"
    "gemini"
    (json_get_string resp.data "claimed_by");
  let err = List.hd resp.errors in
  ...
  let hints_concat = String.concat " " err.recovery_hints in
  assert_true
    "hints mention 'gemini'"
    (try Str.search_forward (Str.regexp "gemini") hints_concat 0 >= 0 with
     | Not_found -> false);
...
let test_task_completed () =
  let resp =
    Response.task_completed ~task_id:"task-001" ~agent:"claude" ~notes:"All tests pass"
  in
  assert_true "success is true" resp.success;
...
let test_severity_of_string_rejects_invalid () =
  ...
  assert_true
    "uppercase FATAL fails"
    (Result.is_error (Response.severity_of_string "FATAL"));
