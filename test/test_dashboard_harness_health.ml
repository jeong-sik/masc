...
let record_handoff_metric
      ?(timestamp = Time_compat.now ())
      ?(keeper_name = "keeper-a")
      ?(trace_id = "trace-keeper-a")
      ?(generation = 0)
      ?next_generation
      ?prev_trace_id
      ?new_trace_id
...
let make_result ?(verdict = AR.Approve) ?(gate = AR.Structured_tool) ?fallback_reason ()
  : AR.review_result
  =
  { verdict
...
let test_runtime_since_only_window_empty_reason () =
  ...
  check
    string
    "pre empty reason (since only)"
    "window_empty"
    Yojson.Safe.Util.(pre_compact |> member "empty_reason" |> to_string);
  check
    string
    "handoff empty reason (since only)"
    "window_empty"
    Yojson.Safe.Util.(recent_handoffs |> member "empty_reason" |> to_string);
  check
    int
    "pre filtered empty (since only)"
    0
    Yojson.Safe.Util.(pre_compact |> member "recent_events" |> to_list |> List.length);
  check
    int
    "handoff filtered empty (since only)"
    0
    Yojson.Safe.Util.(recent_handoffs |> member "recent_events" |> to_list |> List.length)
;;
...
let test_runtime_until_only_window_empty_reason () =
  ...
  check
    string
    "pre empty reason (until only)"
    "window_empty"
    Yojson.Safe.Util.(pre_compact |> member "empty_reason" |> to_string);
  check
    string
    "handoff empty reason (until only)"
    "window_empty"
    Yojson.Safe.Util.(recent_handoffs |> member "empty_reason" |> to_string);
  check
    int
    "pre filtered empty (until only)"
    0
    Yojson.Safe.Util.(pre_compact |> member "recent_events" |> to_list |> List.length);
  check
    int
    "handoff filtered empty (until only)"
    0
    Yojson.Safe.Util.(recent_handoffs |> member "recent_events" |> to_list |> List.length)
;;
...
let test_overview_warns_when_evaluator_falls_back () =
  ...
  let result = make_result ~gate:AR.Fallback ~fallback_reason:"judge timeout" () in
  Cal.record_verdict ~task_id:"task-1" ~req ~result ();
  let json = Harness.json ~config ~since:(today_string ()) ~until:(today_string ()) () in
  let overview = require_assoc "overview" json in
  check
    string
    "evaluator status"
...
let test_agent_scoped_verdicts_filter_before_limit () =
  ...
  let global_latest = Harness.read_recent_verdicts ~limit:1 () in
  check
    string
    "global latest is other agent"
    "other-agent"
  ...
  check
    string
    "scoped latest verdict agent alias"
    "keeper-agent"
    (match scoped with
     | verdict :: _ -> verdict.agent_name
  ...
  check
    int
    "outcomes pass count uses scoped helper"
    1
    Yojson.Safe.Util.(
      outcomes |> member "validation" |> member "oas_verdicts" |> member "pass" |> to_int);
  check
    int
    "outcomes fail count parses reject variants"
    1
    Yojson.Safe.Util.(
      outcomes |> member "validation" |> member "oas_verdicts" |> member "fail" |> to_int);
  check
    int
    "outcomes unknown count stays zero"
    0
    Yojson.Safe.Util.(
      outcomes
      |> member "validation"
  ...
  check
    (list string)
    "failure reasons parse reject suffix"
    [ "schema_violation" ]
    Yojson.Safe.Util.(
      outcomes
      |> member "validation"
      |> member "oas_verdicts"
      |> member "top_failure_reasons"
      |> to_list
      |> filter_string);
  check
    (option (float 0.000_001))
    "last verdict timestamp preserved"
    (Some 101.0)
    Yojson.Safe.Util.(
      outcomes |> member "validation" |> member "last_verdict_at" |> to_float_option)
;;
...
let test_outcomes_rollup_counts_gate_rejected_from_completed_turns () =
  ...
  Reg.set_turn_decision_stage
    ...
    keeper_name
    Reg.Decision_tool_policy_selected;
  Reg.set_turn_cascade_state ~base_path:config.base_path keeper_name Reg.Cascade_done;
  Reg.mark_turn_finished ~base_path:config.base_path keeper_name;
  Reg.mark_turn_started ~base_path:config.base_path keeper_name;
  Reg.set_turn_decision_stage
    ~base_path:config.base_path
    keeper_name
    Reg.Decision_tool_policy_selected;
  Reg.set_turn_cascade_state ~base_path:config.base_path keeper_name Reg.Cascade_exhausted;
  Reg.mark_turn_finished ~base_path:config.base_path keeper_name;
  let outcomes =
    Masc_mcp.Dashboard_http_keeper.compute_outcomes_rollup
      ~keeper_name
  ...
  check
    int
    "observed turns comes from completed turn ring"
    3
    Yojson.Safe.Util.(outcomes |> member "observed_turns" |> to_int);
  check
    int
    "gate_rejected bucket populated"
    1
    Yojson.Safe.Util.(outcomes |> member "failures" |> member "gate_rejected" |> to_int);
  check
    int
    "substantive turns exclude gate_rejected"
    1
    Yojson.Safe.Util.(
      outcomes |> member "successes" |> member "substantive_turns" |> to_int);
  check
    int
    "turn_failed bucket keeps non-gate failures"
    1
...
let sample_role_counts = [ "system", 1; "user", 6; "assistant", 18; "tool", 17 ]

let test_wake_payload_round_trip () =
  with_wake_payload_store
  @@ fun () ->
  let before =
  ...
  check int "context_window round-trip" before.context_window after.context_window;
  check
    int
    "approx_body_bytes round-trip"
    before.approx_body_bytes
    after.approx_body_bytes;
  check
    int
    "system_prompt_bytes round-trip"
    before.system_prompt_bytes
    after.system_prompt_bytes;
  check int "tool_defs_bytes round-trip" before.tool_defs_bytes after.tool_defs_bytes;
  check int "messages_bytes round-trip" before.messages_bytes after.messages_bytes;
  check int "message_count round-trip" before.message_count after.message_count;
  check int "tool_count round-trip" before.tool_count after.tool_count;
  check
    bool
    "has_compact_happened round-trip"
    before.has_compact_happened
    after.has_compact_happened;
  let sorted = List.sort (fun (a, _) (b, _) -> compare a b) in
  check
...
let test_wake_payload_role_count_invariant () =
  ...
  let message_count = List.fold_left (fun acc (_, n) -> acc + n) 0 role_counts in
  let event =
    Harness.record_wake_payload
      ~keeper_name:"k"
      ~trace_id:"t"
  ...
  in
  check int "role_counts sum equals message_count" event.message_count total_from_counts
;;
...
let test_wake_payload_empty_history () =
  ...
  check (list (pair string int)) "empty history role_counts is empty" [] event.role_counts;
  let events = Harness.read_wake_payload_events () in
...
let test_wake_payload_filters_foreign_records () =
  ...
  check string "only wake_payload survives" "k" (List.hd events).keeper_name
;;
...
let () =
  run
    ...
    [ ( "runtime signals"
      , [ test_case
            "persisted runtime signals surface as healthy"
            `Quick
            test_runtime_signals_are_persisted
        ; test_case
            "filtered windows explain empty runtime rails"
            `Quick
            test_runtime_window_empty_reason
        ; test_case
            "since-only windows stay stable"
            `Quick
            test_runtime_since_only_window_empty_reason
        ; test_case
            "until-only windows stay stable"
            `Quick
            test_runtime_until_only_window_empty_reason
        ; test_case
            "stale runtime signals surface as stale"
            `Quick
            test_runtime_stale_status
        ; test_case
            "fallback-heavy evaluator shows warning overview"
            `Quick
            test_overview_warns_when_evaluator_falls_back
        ; test_case
            "agent-scoped verdicts filter before limit"
            `Quick
            test_agent_scoped_verdicts_filter_before_limit
        ; test_case
            "outcomes rollup counts gate_rejected from completed turns"
            `Quick
            test_outcomes_rollup_counts_gate_rejected_from_completed_turns
        ] )
    ; ( "wake payload telemetry"
      , [ test_case "round-trip preserves every field" `Quick test_wake_payload_round_trip
        ; test_case
            "role_counts sum equals message_count"
            `Quick
            test_wake_payload_role_count_invariant
        ; test_case
            "empty history persists with zero counts"
            `Quick
            test_wake_payload_empty_history
        ; test_case
            "non-wake_payload records ignored"
            `Quick
