...
let evaluate (s : measurement_snapshot) : event list =
  ...
  if s.failures.consecutive_hb_failures >= t.max_consecutive_hb_failures
  then
    add
      (Heartbeat_failed
  ...
  if s.failures.consecutive_turn_failures >= t.max_consecutive_turn_failures
  then
    add
      (Turn_failed
  ...
  let compact_msg =
    t.compaction_message_gate > 0 && s.context.message_count >= t.compaction_message_gate
  in
  let compact_tok =
    t.compaction_token_gate > 0 && s.context.token_count >= t.compaction_token_gate
  in
  let cooldown_ok =
    s.timing.since_last_compaction_sec >= float_of_int t.compaction_cooldown_sec
  in
  if (compact_ratio || compact_msg || compact_tok) && cooldown_ok
  then add Compaction_started;
  ...
  if
    ...
    && handoff_cooldown_ok
  then add Handoff_started;
  (* 6. Heartbeat health: if below threshold, report ok *)
  if
    s.failures.consecutive_hb_failures > 0
    && s.failures.consecutive_hb_failures < t.max_consecutive_hb_failures
  then
    add
      (Heartbeat_failed
         { consecutive = s.failures.consecutive_hb_failures
  ...
  add
    (Context_measured
       ...
       ; auto_rules =
           ...
           ; handoff =
               t.auto_handoff_enabled && s.context.context_ratio >= handoff_threshold
           ...
           ; goal_drift = 1.0 -. s.similarity.goal_alignment
           }
       });
  (* Sort by priority (highest first) and return *)
  List.sort (fun a b -> compare (event_priority a) (event_priority b)) (List.rev !events)
;;
