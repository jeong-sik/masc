...
let handle_read_resource_eio state id params =
  ...
  | Some (`Assoc _ as p) ->
    ...
    if uri_str = ""
    then make_error ~id (-32602) "Missing uri"
    else (
      ...
      let mime_type, text_opt =
        ...
        | "tool-help-index" ->
          ( "text/markdown"
          , Some (Tool_help_registry.index_markdown (public_tool_help_schemas ())) )
        | s when String.starts_with ~prefix:"tool-help/" s ->
          let tool_name =
            String.sub
              s
              (String.length "tool-help/")
        ...
        | "worktrees.json" ->
          let json = Coord.worktree_list config in
          "application/json", Some (Yojson.Safe.pretty_to_string json)
        | "schema" -> "text/markdown", Some Mcp_server.schema_markdown
        | "schema.json" ->
        ...
        | "institution" ->
          ...
          then (
            ...
            ( "text/markdown"
            , Some "No institution memory found. Create one with masc_init." )
        | "institution.json" ->
          let file =
            Filename.concat
              (Common.masc_dir_from_base_path ~base_path:config.base_path)
          ...
          then (
            let content = Fs_compat.load_file file in
            "application/json", Some content)
          else "application/json", Some "{\"error\": \"No institution memory found\"}"
        | s when Base.String.is_prefix s ~prefix:"library" ->
          ...
          then
                ...
                | "---" :: rest ->
                  ...
                  let rec scan = function
                    ...
                    | line :: tl ->
                      ...
                      if String.length line > tplen && String.sub line 0 tplen = tp
                      then (
                        let raw =
                          String.trim (String.sub line tplen (String.length line - tplen))
                        in
                        let inner =
                          if
                            String.length raw >= 2
            ...
            let strip_frontmatter content =
              if Base.String.is_prefix content ~prefix:"---"
              then (
                  ...
                  let rec find_end pos =
                    ...
                    | Some nl ->
                      ...
                      then (
                        ...
                        if rest_start < String.length content
                        then
                          String.trim
                            (String.sub
                               content
            ...
            let is_json, topic =
              ...
              then false, ""
                ...
                let rest =
                  if String.length s > 8 then String.sub s 8 (String.length s - 8) else ""
                in
                if Filename.check_suffix rest ".json"
                then true, Filename.chop_suffix rest ".json"
                else false, rest)
            in
            let library_files () =
              Sys.readdir library_dir
              |> Array.to_list
              |> List.filter (fun f -> Filename.check_suffix f ".md" && f <> "README.md")
              |> List.sort String.compare
            in
            if topic = "" && not is_json
            then (
              ...
              let entries =
                List.map
                  (fun f ->
                     ...
                     let title, source, _verified, _date, tags =
                       parse_frontmatter path name
                     in
                     let tag_str =
                       if tags = []
                       then ""
                     ...
                     in
                     let src_str =
                       if source = "" then "" else " ([source](" ^ source ^ "))"
                     in
                     Printf.sprintf
                       "- **%s** -- `masc://library/%s`%s%s"
                       title
                       name
                       src_str
                       tag_str)
            ...
            then (
              ...
              let docs =
                List.map
                  (fun f ->
                     let name = Filename.chop_suffix f ".md" in
                     let path = Filename.concat library_dir f in
                     let title, source, verified_by, date, tags =
                       parse_frontmatter path name
                     in
                     `Assoc
                       [ "topic", `String name
              ...
              in
              let json =
                `Assoc [ "documents", `List docs; "count", `Int (List.length docs) ]
              in
            ...
            then (
                ...
                , Some
                    (Yojson.Safe.to_string
                       (`Assoc
                           [ ( "error"
                             , `String
                                 (Printf.sprintf "Library document '%s' not found" topic)
                             )
                           ])) ))
            else (
              let path = Filename.concat library_dir (topic ^ ".md") in
              ...
              then (
                ...
                ( "text/markdown"
                , Some (Printf.sprintf "Library document '%s' not found." topic) )))
        | _ -> "text/plain", None
      in
      match text_opt with
      | None ->
  ...
  | Some _ -> make_error ~id (-32602) "Invalid params"
;;
