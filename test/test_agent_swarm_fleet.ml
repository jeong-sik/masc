...
let test_run_full_plan () =
  let provider = { Provider.provider = Local { base_url = "http://127.0.0.1:8085" };
           model_id = "qwen3.5-35b-a3b-ud-q8-xl"; api_key_env = "DUMMY_KEY" } in
  let plan =
...
let test_run_full_plan_rejects_zero_members () =
  Alcotest.check_raises "zero members rejected"
    ...
    (fun () ->
       ignore
         (Agent_swarm_fleet.build_run_full_plan ~provider:({ Provider.provider = Local { base_url = "http://127.0.0.1:8085" };
           model_id = "qwen3.5-35b-a3b-ud-q8-xl"; api_key_env = "DUMMY_KEY" })
