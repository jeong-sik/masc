...
(* Enter is an update to this conversation, not a wait for its completion. *)
let test_a_turn_in_flight_updates () =
  check bool "a turn in flight accepts an update" true
    (d ~inflight:(Some "req-1") ~waiting:None
     = Masc_tui_send_disposition.Updates "req-1")
...
let test_a_waiting_line_updates () =
  check bool "a waiting line accepts an update" true
    (d ~inflight:None ~waiting:(Some "wait-1")
     = Masc_tui_send_disposition.Updates "wait-1")
...
let test_inflight_wins_over_waiting () =
  check bool "inflight names the conversation" true
...
let () =
  Alcotest.run "tui_send_disposition"
    [ ( "disposition"
      , [ Alcotest.test_case "a turn in flight accepts an update" `Quick
            test_a_turn_in_flight_updates
        ; Alcotest.test_case "nothing standing sends" `Quick
            test_nothing_standing_sends
        ; Alcotest.test_case "a waiting line accepts an update" `Quick
            test_a_waiting_line_updates
