...
- Modify: `bin/masc_tui.ml:2074-2120` (`post_keeper_chat_watching`) — one `Keeper_chat_live.create ()` for the whole watch; on reconnect, re-POST `{ request with since_seq = Some (Log.last_seq log) }` (the log is reachable through the inflight entry); the acceptance frame of the new stream has no seq and is not deduplicated (already true by decision 5).
