...
| L2: SSE wire event_type | `masc:broadcast`, `keeper_heartbeat` 등 | `lib/sse.ml`, `lib/oas_event_bridge.ml:348-366` | dashboard `dashboard/src/sse-store.ts:121-143` |
...
이 함수의 caller (검색 결과 `rg -n "broadcast" lib/coord/`):

| 호출자 | 파일:라인 | 메시지 종류 |
|--------|----------|-------------|
| `coord_task.add_task_internal` | `coord_task.ml:608-613` | `📋 New quest: <title>` (task 추가 알림) |
| `coord_task.batch_add_tasks_internal` | `coord_task.ml:705` | `📋 New batch of N quests added` |
| `coord_task.claim_task` | `coord_task.ml:826-830` | `📋 Claimed <task_id>` (claim 알림) |
| `coord_task.claim_next` | `coord_task.ml:959-963` | `📋 Claimed <task_id>` (claim 알림) |
| `coord_task.<unknown>` | `coord_task.ml:1901` | 추가 task transition broadcast |
| 사용자 채팅/멘션 | `tool_inline_dispatch_comm.ml` 경유 | 일반 broadcast (멘션 포함) |
...
heartbeat 이벤트 publisher (`lib/keeper/keeper_keepalive.ml:1182-1189`):
...
|-------------------|------|---------------|--------|
| `keeper_heartbeat` | keeper × keepalive 주기 (~5s) | `keeper_keepalive.ml:1182` `Sse.broadcast` | dashboard heartbeat tick, status |
| `masc:broadcast` (구 `masc.broadcast`) | 사용자 broadcast/claim/task 알림 | `oas_event_bridge.ml` relay (`. → :`) | live feed, execution 패널 |
| `masc:keeper:lifecycle` | keeper 시작/재시작/사망 (수 분 ~ 시간 단위) | `oas_events.publish_keeper_lifecycle` | operator 알림 |
| `masc:keeper:snapshot` | heartbeat tick과 동일 주기 | `oas_events.publish_keeper_snapshot` (`keeper_keepalive.ml:1196`) | dashboard 카드 |
| `keeper_composite_changed` | composite FSM 전이 시 | `Sse.broadcast` | dashboard composite tick |
...
heartbeat-tick 등가물은 `keeper_heartbeat` + `masc:keeper:snapshot` 두 줄이며,
keeper 1개당 ~5s 주기로 둘 다 발생한다 (검증 필요 — 실제 주기는
`Env_config.Keeper.keepalive_interval_seconds`).
...
가정이 아님). 12 × 5s keepalive = **2.4 hb/s**. 두 이벤트 (heartbeat + snapshot)
모두 발생하면 **~4.8 evt/s**가 SSE awareness 잡음으로 흐른다.
...
|------------|------------------------|----------|--------|------|
| heartbeat tick | `keeper_heartbeat` | broadcast | **presence** | `keeper_keepalive.ml:1182` `Sse.broadcast` → `Sse.broadcast_to (Presence)` 또는 `presence`-tag |
| keeper snapshot | `masc:keeper:snapshot` | broadcast | **presence** | `oas_events.publish_keeper_snapshot` 경로의 SSE bridge dispatch tag만 변경 |
| keeper composite changed | `keeper_composite_changed` | broadcast | **presence** | composite tick은 heartbeat-등가 |
...
| `keeper_keepalive.ml:1182` | `Sse.broadcast` → `Sse.broadcast_to Presence` 또는 새 dispatch helper | 중간 — 호출부 1곳, **(검증 필요)** snapshot publish 경로의 SSE relay 태그도 같이 |
...
- gRPC gateway (`lib/grpc/masc_grpc_service.ml:499`): `Sse.broadcast` 호출을 gRPC Event로 변환.
...
1. gRPC gateway (`masc_grpc_service.ml:499`)가 분리된 두 stream을 어떻게 union 할 것인가?
