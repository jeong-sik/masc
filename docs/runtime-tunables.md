...
**Total**: 372 unique knobs across 10 modules.

**Typed getter classification**: 54/224 tagged (`operator`: 54, `algorithm`: 0, `unclassified`: 170).

## Env_config_core (28 knobs; typed classification 3/8)
...
| `MASC_BUILD_GIT_COMMIT` | string_literal | n/a | n/a | 600 | Git commit hash override for build identity. |
| `MASC_CLUSTER_NAME` | string_literal | n/a | n/a | 276 |  |
| `MASC_CODE_EXEMPT_KEEPERS` | string_literal | Security | operator | 581 | Environment variable for space-separated keeper names exempt from code-execution governance gates (per task-1807). @c... |
...
| `MASC_KEEPER_DEFAULT_SANDBOX_PROFILE` | typed:string | unclassified | unclassified | 611 | Default sandbox profile for keepers. Default: "local". Set to "docker" to default all keepers to containerized execut... |
...
| `MASC_PUBSUB_MAX_MESSAGES` | typed:int | unclassified | unclassified | 604 | PubSub max messages per read. Default: 1000. |
...
## Env_config_keeper (95 knobs; typed classification 31/82)

| Env var | Kind | Category | Ops class | Line | Doc |
|---|---|---|---|---|---|
| `MASC_ALERT_DEDUP_WINDOW_SEC` | typed:float | unclassified | unclassified | 599 | Alert dedup window, clamped to >= 5s. Default: 60. |
| `MASC_CONTEXT_RATIO_HARD_CAP` | typed:float | unclassified | unclassified | 984 | {1 Context Ratio Hard Cap} Absolute ceiling for compaction ratio_gate and handoff threshold after multiplier adjustme... |
| `MASC_DASHBOARD_HEALTH_CTX_CRITICAL` | typed:float | unclassified | unclassified | 1019 | {1 Dashboard Health Thresholds} Thresholds used by the dashboard keeper health scorer and harness health panels.  Dis... |
| `MASC_DASHBOARD_HEALTH_CTX_WARN` | typed:float | unclassified | unclassified | 1020 | {1 Dashboard Health Thresholds} Thresholds used by the dashboard keeper health scorer and harness health panels.  Dis... |
| `MASC_DASHBOARD_HEALTH_PENALTY_CRITICAL` | typed:float | unclassified | unclassified | 1021 | {1 Dashboard Health Thresholds} Thresholds used by the dashboard keeper health scorer and harness health panels.  Dis... |
| `MASC_DASHBOARD_HEALTH_PENALTY_WARN` | typed:float | unclassified | unclassified | 1022 | {1 Dashboard Health Thresholds} Thresholds used by the dashboard keeper health scorer and harness health panels.  Dis... |
| `MASC_DASHBOARD_RUNTIME_WARNING_CTX_RATIO` | typed:float | unclassified | unclassified | 1025 |  |
...
| `MASC_KEEPER_GRPC_MAX_RECONNECT` | typed:int | unclassified | unclassified | 938 | Maximum gRPC reconnect attempts before stopping the heartbeat fiber. Default: 5. Range: [1, 20]. |
| `MASC_KEEPER_GRPC_RECONNECT_BACKOFF_SEC` | typed:float | unclassified | unclassified | 946 | Backoff delay between gRPC reconnect attempts in seconds. Default: 5.0. Range: [1.0, 60.0]. |
...
| `MASC_KEEPER_IDLE_SKIP_THRESHOLD` | typed:int | unclassified | unclassified | 928 | Consecutive idle tool repetitions before on_idle hook issues Skip. Below this: graduated Nudge messages. Optional too... |
...
| `MASC_KEEPER_MAX_CONSECUTIVE_TOOL_FAILURES` | typed:int | unclassified | unclassified | 973 | Maximum consecutive failures for the same (tool_name, args_hash) before blocking further attempts. Prevents infinite ... |
...
| `MASC_KEEPER_PROACTIVE_MAX_ATTEMPTS` | typed:int | unclassified | unclassified | 956 | Maximum proactive generation attempts before falling back. Default: 3. Range: [1, 10]. |
...
| `MASC_KEEPER_STAGE_TIMING_RING_SIZE` | typed:int | unclassified | unclassified | 962 | Stage timing ring buffer size for Phase 0 profiling. Default: 100. Range: [10, 1000]. |
...
| `MASC_PAYLOAD_TELEMETRY` | typed:bool | unclassified | unclassified | 1043 | Master switch for wake-payload measurement. Default off so the hot path is untouched until a baseline sweep is explic... |
| `MASC_RUNTIME_SATURATION_SIGNAL_ENABLED` | typed:bool | unclassified | unclassified | 1057 | {1 Runtime Saturation Signal (RFC-0153 Phase A.2)} Feature flag for typed [Runtime_saturation_signal.t] emission from... |
...
| `MASC_DEFAULT_MODEL` | string_literal | n/a | n/a | 662 |  |
| `MASC_DEFAULT_PROVIDER` | string_literal | n/a | n/a | 664 |  |
...
| `MASC_EVENT_BUFFER_SIZE` | string_literal | n/a | n/a | 816 |  |
| `MASC_GOAL_DISPATCH_RUNTIME` | string_literal | n/a | n/a | 666 |  |
...
| `MASC_KEEPER_LLM_RERANK_RUNTIME` | string_literal | n/a | n/a | 560 |  |
...
| `MASC_KEEPER_TOOL_AFFINITY_K` | string_literal | n/a | n/a | 564 |  |
| `MASC_KEEPER_TOOL_AFFINITY_LOOKBACK_DAYS` | string_literal | n/a | n/a | 566 |  |
| `MASC_KEEPER_TOOL_COST_MAX_USD` | string_literal | n/a | n/a | 172 |  |
| `MASC_KEEPER_TOOL_DECAY_TURNS` | string_literal | n/a | n/a | 568 |  |
...
| `MASC_OTEL_ENABLED` | string_literal | n/a | n/a | 792 |  |
| `MASC_PLACEHOLDER_TOOLS_ENABLED` | string_literal | n/a | n/a | 828 |  |
| `MASC_ROUTING_RUNTIME` | string_literal | n/a | n/a | 668 |  |
| `MASC_RUNTIME_ATTEMPT_LIVENESS` | string_literal | n/a | n/a | 471 |  |
| `MASC_SEARXNG_URL` | string_literal | n/a | n/a | 838 |  |
| `MASC_SHUTDOWN_CLEANUP_TIMEOUT` | string_literal | n/a | n/a | 750 |  |
| `MASC_SHUTDOWN_DRAIN_TIMEOUT` | string_literal | n/a | n/a | 752 |  |
| `MASC_SHUTDOWN_FORCE_TIMEOUT` | string_literal | n/a | n/a | 754 |  |
| `MASC_SHUTDOWN_NOTIFY_DELAY` | string_literal | n/a | n/a | 756 |  |
| `MASC_SSE_KEEPALIVE_SEC` | string_literal | n/a | n/a | 818 |  |
| `MASC_SSE_STREAM_CAPACITY` | string_literal | n/a | n/a | 772 |  |
...
| `MASC_TEST_ALLOW_BASE_PATH_OVERRIDE` | string_literal | n/a | n/a | 808 |  |
| `MASC_TEST_ALLOW_CONFIG_PATH_OVERRIDE` | string_literal | n/a | n/a | 810 |  |
| `MASC_TLA_TRACE` | string_literal | n/a | n/a | 160 |  |
| `MASC_WORKER_RUNTIME_BACKEND` | string_literal | n/a | n/a | 870 |  |
| `MASC_WORKER_RUNTIME_DOCKER_IMAGE` | string_literal | n/a | n/a | 872 |  |
| `MASC_WORKER_RUNTIME_HOST_MCP_BASE_URL` | string_literal | n/a | n/a | 874 |  |
