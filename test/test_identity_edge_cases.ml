...
let test_empty_params () =
  ...
  check bool "starts with agent-" true (String.sub identity.agent_name 0 6 = "agent-")
;;
...
let test_null_values () =
  ...
  Fs_compat.set_fs (Eio.Stdenv.fs env);
  Agent_registry_eio.reset_for_testing ();
  let params = `Assoc [ "_agent_name", `Null; "room", `Null ] in
...
let test_unknown_channel () =
  ...
  Agent_registry_eio.reset_for_testing ();
  let params =
    `Assoc [ "_agent_name", `String "test"; "_channel", `String "unknown_platform_xyz" ]
  in
  let identity = Agent_registry_eio.get_or_create_identity params in
  match identity.channel with
  | Some (Agent_identity.External s) ->
    check string "opaque external" "unknown_platform_xyz" s
...
let test_empty_capabilities () =
  ...
  Fs_compat.set_fs (Eio.Stdenv.fs env);
  Agent_registry_eio.reset_for_testing ();
  let params = `Assoc [ "_agent_name", `String "test"; "_capabilities", `List [] ] in
  let identity = Agent_registry_eio.get_or_create_identity params in
...
let test_invalid_capabilities () =
  ...
  Agent_registry_eio.reset_for_testing ();
  let params =
    `Assoc [ "_agent_name", `String "test"; "_capabilities", `String "not_a_list" ]
  in
  let identity = Agent_registry_eio.get_or_create_identity params in
...
let () =
  run
    ...
    ; "channels", [ test_case "unknown" `Quick test_unknown_channel ]
    ; ( "capabilities"
      , [ test_case "empty" `Quick test_empty_capabilities
        ; test_case "invalid" `Quick test_invalid_capabilities
        ] )
    ; "stress", [ test_case "rapid_creation" `Quick test_rapid_creation ]
    ]
;;
