...
| 기존 prometheus | **제거됨** (PR #20189) — OTel-only 로 전환 | 코드 확인 |
...
| metric | 출처 | 수집 방법 | 비고 |
|--------|------|----------|------|
| WS parse cache hit ratio | `masc_ws_parse_cache_{hits,misses}_total` | OTel counter 합산 | 이미 export 됨 |
| WS bytes cache hit ratio | `masc_ws_bytes_cache_{hits,misses}_total` | OTel counter 합산 | 이미 export 됨 |
...
OCaml 의 `Gc.quick_stat` 은 정량 정보가 풍부하지만 OTel metric store 에
...
- 본 baseline 은 **단일 Railway 인스턴스 + 12+ keeper** 의 측정
  ...
  을 다시 14 일 쌓아야 한다.
- in-memory counter (OTel export 대상) 는 server restart 시 0 으로 초기화된다.
