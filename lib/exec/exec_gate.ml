...
let notify_overlay : Approval_config.agent_overlay =
  { safe_trust = Auto_safe; audited_trust = Auto_safe; privileged_trust = Enforced }
;;
...
let env_bindings_of_array env =
  ...
  |> List.map (fun binding ->
    ...
    | Some idx ->
      ...
      let value = String.sub binding (idx + 1) (String.length binding - idx - 1) in
      key, lit value
    | None -> binding, lit "")
;;
    ...
    (match Bin.of_string bin_raw with
     ...
     | Ok bin ->
       ...
       let env =
         ...
         | None -> []
       in
       let cwd = Option.map (fun dir -> Path_scope.classify ~raw:dir ~cwd:dir) cwd in
...
let verdict_for_argv ~actor ~raw_source ~summary ~argv ?env ?cwd () =
  ...
  | Error `Empty_argv -> Error (`Denied Verdict.Parse_failed)
  | Error `Parse_failed -> Error (`Denied Verdict.Parse_failed)
  | Ok simple ->
...
let with_verdict
  ...
  | Error gate_error ->
    ...
    in
    record_decision ~actor ~raw_source ~summary ~mode ~verdict ~argv ?env ?cwd ();
    (match mode with
...
let run_argv ~actor ~raw_source ~summary ?(timeout_sec = 60.0) ?env argv =
  with_verdict
    ...
    ~on_blocked:(fun reason -> blocked_output ~summary ~raw_source ~reason)
    ()
;;

let run_argv_with_status ~actor ~raw_source ~summary ?(timeout_sec = 60.0) ?env ?cwd argv =
  with_verdict
    ~actor
    ~raw_source
    ~summary
    ...
    ?env
    ?cwd
    ~on_allow:(fun () -> Process_eio.run_argv_with_status ~timeout_sec ?env ?cwd argv)
    ~on_blocked:(fun reason ->
      Unix.WEXITED 126, blocked_output ~summary ~raw_source ~reason)
    ()
;;
