...
let test_default_config_interval () =
  check bool "interval positive" true (Orchestrator.default_config.check_interval_s > 0.0)
;;

let test_default_config_interval_value () =
  check bool "interval is 300s" true (Orchestrator.default_config.check_interval_s = 300.0)
;;
...
let test_default_config_agent () =
  check
    string
    "orchestrator_agent"
    Env_config_runtime.Orchestrator.agent_name
...
let test_default_config_port () = check int "port" 8935 Orchestrator.default_config.port
...
let test_make_orchestrator_prompt_contains_mcp () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "mcp__masc") prompt 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_make_orchestrator_prompt_contains_tools () =
  ...
  check
    bool
    "mentions masc_status"
    true
    (try
       let _ = Str.search_forward (Str.regexp "masc_status") prompt 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_make_orchestrator_prompt_contains_claim () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "masc_transition") prompt 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_config_reasonable_priority () =
  let cfg = Orchestrator.default_config in
  check bool "priority 0-10" true (cfg.min_priority >= 0 && cfg.min_priority <= 10)
;;
    ...
    ; ( "make_orchestrator_prompt"
      , [ test_case "basic" `Quick test_make_orchestrator_prompt_basic
        ...
        ; test_case
            "mentions broadcast"
            `Quick
            test_make_orchestrator_prompt_mentions_broadcast
        ] )
    ; ( "config_bounds"
