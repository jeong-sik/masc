  ...
  String.concat ""
    [
      ...
      "Autonomous behavior:\n\
       ...
       - ACTION TOOLS: For productive turns, use these: keeper_task_claim (claim work), keeper_fs_read + keeper_fs_edit/keeper_write (read then modify files), keeper_bash (run commands, including git add/commit/push inside worktrees), keeper_shell op=gh (PR/issues via gh CLI; after git push, use it for `gh pr create --draft ...`; only after you already hold a claimed task/current_task_id), keeper_board_post (share findings), keeper_stay_silent (nothing to do). Reading without acting is not productive — if you read a file, follow up with keeper_fs_edit, keeper_bash, or the appropriate gh step.\n\
