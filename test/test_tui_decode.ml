...
let test_decode_keeper_tool_approvals () =
  ...
  | Error err -> Alcotest.fail err
  | Ok [ held ] ->
      Alcotest.(check string) "keeper" "haneul" held.Tui_decode.kta_keeper;
...
let test_decode_keeper_tool_approvals_rejects_a_thin_row () =
  let thin =
    `Assoc [ ("pending", `List [ `Assoc [ ("keeper", `String "haneul") ] ]) ]
  in
...
let test_decode_runtime_resolved () =
  ...
  | Ok (runtimes, assignments) ->
      ...
      (match assignments with
       | [ a ] ->
           Alcotest.(check string) "keeper" "haneul" a.Tui_decode.ra_keeper;
