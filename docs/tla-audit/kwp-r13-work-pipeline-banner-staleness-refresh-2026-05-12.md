...
| `find lib -name "*github*"` returns 0 hits in lib/keeper/ | **Retired as a probe** — dedicated PR helper files were removed later, and generic GitHub filenames are too broad for the modeled work-pipeline surface | replace the probe: the durable signal is `rg 'force_push_attempted\|workspace_init' lib/keeper/` → 0 |
...
| Actual keeper tool runtime surface: `agent_tool_board_runtime / _context / _fs / _masc / _memory` | Still accurate; dedicated PR helper wrappers are no longer part of the active surface | keep |
...
| dedicated PR helper wrappers exist & are they the modeled surface? | no; they were retired, and they were not the work-pipeline exec module |
