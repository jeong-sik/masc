...
let test_malformed_lines_skipped () =
  with_tmp_dir (fun base_path ->
    ...
    let day_file =
      Filename.concat month_dir (Printf.sprintf "%02d.jsonl" tm.Unix.tm_mday)
    in
...
let () =
  Alcotest.run
    ...
    [ ( "persistence"
      , [ eio_test "enqueue, flush, restore round-trip" test_enqueue_flush_restore
        ; eio_test "restore from empty dir" test_restore_empty_dir
        ; eio_test "malformed lines skipped" test_malformed_lines_skipped
        ; eio_test "reset clears cached store" test_reset_clears_cached_store
        ] )
    ]
;;
