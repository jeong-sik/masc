...
if grep -Fq '(generate_opam_files true)' dune-project; then
  ...
  if grep -Fq '# This file is generated by dune' masc_mcp.opam && [[ "$opam_version" != "$package_version" ]]; then
    # PR merge refs can inherit a stale checked-in generated opam file from the
    # base branch. Normalize the generated version field from dune-project
    # before asserting version truth.
    tmp_file="$(mktemp)"
    awk -v version="$package_version" '
      !done && $0 ~ /^version: "/ { print "version: \"" version "\""; done=1; next }
      { print }
    ' masc_mcp.opam > "$tmp_file"
    mv "$tmp_file" masc_mcp.opam
  fi
fi
