...
let test_resolve_commit_prefers_env () =
  ...
  let commit =
    Build_identity.resolve_commit ~env_value:(Some " abc12345 ") ~probe:(fun () ->
      probe_called := true;
      Some "deadbeef")
  in
...
let test_resolve_commit_uses_probe_when_env_missing () =
  let commit =
    Build_identity.resolve_commit ~env_value:None ~probe:(fun () -> Some "deadbeef")
  in
...
let () =
  Alcotest.run
    ...
    [ ( "identity"
      , [ Alcotest.test_case
            "resolve commit prefers env"
            `Quick
            test_resolve_commit_prefers_env
        ; Alcotest.test_case
            "resolve commit falls back to probe"
            `Quick
            test_resolve_commit_uses_probe_when_env_missing
        ; Alcotest.test_case
            "current started_at stable"
            `Quick
            test_current_started_at_is_stable
        ; Alcotest.test_case
            "pick_repo_candidates exe first when distinct"
            `Quick
            test_pick_repo_candidates_exe_first_when_distinct
        ; Alcotest.test_case
            "pick_repo_candidates dedups equal"
            `Quick
            test_pick_repo_candidates_dedups_equal
        ; Alcotest.test_case
            "pick_repo_candidates not sorted alphabetically"
            `Quick
