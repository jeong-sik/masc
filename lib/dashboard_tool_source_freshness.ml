...
let health_fields ~now ~exists ~entry_count ~latest_ts ~freshness_slo_s ?coverage_gap () =
...
let metadata_fields
  ...
  @ health_fields ~now ~exists ~entry_count ~latest_ts ~freshness_slo_s ?coverage_gap ()
;;
...
let keeper_tool_call_io_fields ~dashboard_surface () =
  metadata_fields
    ...
    ~durable_store:(Keeper_tool_call_log.store_dir () |> Option.value ~default:"")
