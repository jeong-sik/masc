...
let test_bare_json_error () =
  ...
  check string "bare JSON extracts error key" "Invalid task state" (classify output)
;;
...
let test_error_prefix_stripped () =
  ...
  check
    string
    "error: prefix stripped, key extracted"
    "\xe2\x9d\x8c Invalid task state: Cannot start"
    (classify output)
;;

let test_tool_error_prefix_stripped () =
  let output = {|tool_error: {"ok":false,"error":"command_blocked"}|} in
  check string "tool_error: prefix stripped" "command_blocked" (classify output)
;;
...
let test_readonly_block_category_preserved () =
  ...
  check
    string
    "readonly block keeps category"
    "command_blocked_readonly:destructive"
    (classify output)
;;

let test_empty_output () =
  check string "empty output classified" "empty_output" (classify "")
;;

let test_plain_text_is_parse_error () =
  check
    string
    "plain text remains parse_error"
    "parse_error"
...
let test_no_error_key_is_unknown () =
  ...
  check string "JSON without error key" "unknown_error" (classify output)
;;

let test_nested_json_in_error_prefix () =
  let output = {|error: {"ok":false,"error":"path blocked","detail":{"path":"/etc"}}|} in
  check string "nested JSON after prefix" "path blocked" (classify output)
;;
...
let test_path_error_is_normalized () =
  ...
  check
    string
    "path boundary error normalized"
...
let test_sandbox_path_error_is_normalized () =
  ...
  check
    string
    "sandbox path boundary error normalized"
...
let test_message_error_is_normalized () =
  ...
  in
  check string "message-only error normalized" "query_secret_like" (classify output)
;;
...
let test_signaled_status_is_classified () =
  ...
  in
  check string "signaled process classified" "bash_signaled_-11" (classify output)
;;
...
let test_timeout_error_is_preserved () =
  ...
  in
  check string "timeout error preserved" "command_timed_out" (classify output)
;;

let test_timeout_status_is_classified () =
  let output = {|{"ok":false,"op":"bash","status":{"kind":"timeout"},"output":""}|} in
  check string "timeout process classified" "bash_timeout" (classify output)
;;

let () =
  run
    "tool_quality_classify"
    [ ( "classify_failure_output"
      , [ test_case "bare JSON error" `Quick test_bare_json_error
        ...
        ; test_case
            "readonly block keeps category"
            `Quick
            test_readonly_block_category_preserved
        ...
        ; test_case
            "sandbox path error normalized"
            `Quick
            test_sandbox_path_error_is_normalized
        ; test_case
            "message-only error normalized"
            `Quick
            test_message_error_is_normalized
