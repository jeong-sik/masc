...
let criterion_to_yojson = function
  | Schema_match schema -> `Assoc [ "type", `String "schema_match"; "schema", schema ]
  | Contains s -> `Assoc [ "type", `String "contains"; "value", `String s ]
  | Not_contains s -> `Assoc [ "type", `String "not_contains"; "value", `String s ]
  | Custom s -> `Assoc [ "type", `String "custom"; "description", `String s ]
;;
...
let verdict_to_yojson = function
  ...
  | Partial (score, reason) ->
    `Assoc
      [ "verdict", `String "partial"; "score", `Float score; "reason", `String reason ]
;;
...
let request_status_to_yojson = function
  ...
  | Assigned agent -> `Assoc [ "status", `String "assigned"; "verifier", `String agent ]
  | Completed v ->
       ...
       let status =
         ...
         | Some json ->
           (match request_status_of_yojson json with
            ...
            | Error msg ->
              Eio.traceln
                "[Verification] unparseable status, falling back to Pending: %s"
                msg;
              Pending)
         | None -> Pending
       in
...
let generate_id () = Random_id.prefixed ~prefix:"vrf-" ~bytes:16
...
let contains_nonempty_literal ~needle haystack =
  ...
  then false
    ...
    let rec match_at i j =
      if j = nlen
      then true
      else if String.get haystack (i + j) <> String.get needle j
      then false
      else match_at i (j + 1)
    in
...
let evaluate_criterion output criterion =
  ...
  | Custom _ -> Partial (0.5, "custom criterion requires verifier judgment")
;;
...
let validate_cross_agent ~worker ~verifier =
  if String.equal worker verifier
  then Error "Cross-agent violation: worker cannot verify own output"
  else Ok ()
;;
...
let request_path base_path req_id = Coord_verification_store.request_path base_path req_id
...
let save_request base_path req =
  ...
  | exn -> Error (Printf.sprintf "save_request %s: %s" req.id (Printexc.to_string exn))
;;
...
let load_request base_path req_id =
  ...
  then (
    ...
    | exn ->
      Error
        (Printf.sprintf
           "Failed to load verification %s: %s"
           req_id
           (Printexc.to_string exn)))
  else Error (Printf.sprintf "Verification %s not found" req_id)
;;
  ...
  | Ok req ->
    (match validate_cross_agent ~worker:req.worker ~verifier with
     ...
     | Ok () ->
       ...
       let updated = { req with status = Completed verdict; verifier = Some verifier } in
       (match save_request base_path updated with
        | Ok _ -> Ok updated
        | Error e -> Error e))
;;
...
let auto_verify ~base_path ~req_id =
  ...
  | Ok req ->
    ...
    in
    if has_custom
    then Error "Cannot auto-verify: custom criteria require agent judgment"
    else (
...
let to_attribution ~origin ~evidence (v : verdict) : Attribution.t =
  match v with
  | Pass -> Attribution.passed ~origin ~gate:"verification" ~evidence
  | Fail reason ->
    Attribution.policy_failed ~origin ~gate:"verification" ~evidence ~reason
  | Partial (score, rationale) ->
    Attribution.partial_pass ~origin ~gate:"verification" ~evidence ~score ~rationale
;;
