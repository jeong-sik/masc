...
let cascade_rotation_attempt_to_json attempt =
  `Assoc
    [ "from_cascade", `String (Keeper_name.to_string attempt.from_cascade)
    ; "to_cascade", `String (Keeper_name.to_string attempt.to_cascade)
    ; ( "reason"
...
let to_json (receipt : t) =
  ...
  let runtime_contract =
    Keeper_runtime_contract.runtime_contract_json_from_fields
      ...
      ~cascade_profile:(Keeper_name.to_string receipt.cascade_name)
      ()
  in
  let action_radius =
  ...
  `Assoc
    ...
    ; ( "cascade"
      , `Assoc
          [ "name", `String (Keeper_name.to_string receipt.cascade_name)
          ; "selected_model", `Null
          ...
          ; ( "degraded_retry_cascade"
            , match receipt.degraded_retry_cascade with
              | Some value -> `String (Keeper_name.to_string value)
              | None -> `Null )
          ; ( "fallback_reason"
...
let operator_broadcast_payload (receipt : t) ~disposition ~reason =
  ...
  `Assoc
    ...
    ; "goal_ids", list_json receipt.goal_ids
    ; "response_text_present", `Bool receipt.response_text_present
    ; "cascade_name", `String (Keeper_name.to_string receipt.cascade_name)
