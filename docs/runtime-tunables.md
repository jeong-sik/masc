...
| `MASC_AUTO_RESPOND` | string_literal | n/a | n/a | 512 | Raw MASC_AUTO_RESPOND value for mode parsing. |
...
| `MASC_BUILD_GIT_COMMIT` | string_literal | n/a | n/a | 506 | Git commit hash override for build identity. |
...
| `MASC_GIT_FETCH_TIMEOUT_SEC` | string_literal | n/a | n/a | 469 | [git fetch origin] before worktree creation is network-bound and can stall behind a slow Docker bridge or a large rem... |
| `MASC_GOVERNANCE_LEVEL` | string_literal | n/a | n/a | 482 | SSOT for logging / observability env-var names (issue 8352). |
...
| `MASC_LOG_LEVEL` | string_literal | n/a | n/a | 478 | SSOT for logging / observability env-var names (issue 8352). |
| `MASC_LOG_ROUTINE_LEVEL` | string_literal | n/a | n/a | 479 | SSOT for logging / observability env-var names (issue 8352). |
...
| `MASC_PARSE_WARN` | string_literal | n/a | n/a | 481 | SSOT for logging / observability env-var names (issue 8352). |
...
| `MASC_TELEMETRY_ENABLED` | string_literal | n/a | n/a | 480 | SSOT for logging / observability env-var names (issue 8352). |
...
| `MASC_CASCADE_ATTEMPT_LIVENESS` | typed:string | unclassified | unclassified | 922 |  |
| `MASC_CASCADE_SATURATION_SIGNAL_ENABLED` | typed:bool | unclassified | unclassified | 834 | {1 Cascade Saturation Signal (RFC-0153 Phase A.2)} Feature flag for typed [Cascade_saturation_signal.t] emission alon... |
| `MASC_CASCADE_TIER_ADMISSION_ENABLED` | feature_flag | n/a | n/a | 846 | {1 Cascade Tier Admission (RFC-0153 Phase B.2)} Runtime kill switch for per-tier inflight admission in the main keepe... |
| `MASC_CONTEXT_RATIO_HARD_CAP` | typed:float | unclassified | unclassified | 683 | {1 Context Ratio Hard Cap} Absolute ceiling for compaction ratio_gate and handoff threshold after multiplier adjustme... |
| `MASC_DASHBOARD_HEALTH_CTX_CRITICAL` | typed:float | unclassified | unclassified | 794 | Legacy RFC-0006 Phase B-2 flag. Docker read routing now follows [sandbox_profile=docker] unconditionally. This getter... |
| `MASC_DASHBOARD_HEALTH_CTX_WARN` | typed:float | unclassified | unclassified | 795 | Legacy RFC-0006 Phase B-2 flag. Docker read routing now follows [sandbox_profile=docker] unconditionally. This getter... |
| `MASC_DASHBOARD_HEALTH_PENALTY_CRITICAL` | typed:float | unclassified | unclassified | 796 | Legacy RFC-0006 Phase B-2 flag. Docker read routing now follows [sandbox_profile=docker] unconditionally. This getter... |
| `MASC_DASHBOARD_HEALTH_PENALTY_WARN` | typed:float | unclassified | unclassified | 797 | Legacy RFC-0006 Phase B-2 flag. Docker read routing now follows [sandbox_profile=docker] unconditionally. This getter... |
| `MASC_DASHBOARD_RUNTIME_WARNING_CTX_RATIO` | typed:float | unclassified | unclassified | 800 |  |
...
| `MASC_KEEPER_BATCH_THRESHOLD` | typed:int | unclassified | unclassified | 628 | Number of distinct keepers terminating within [batch_window_sec] before emitting a fleet batch alert. Default: 5. |
| `MASC_KEEPER_BATCH_WINDOW_SEC` | typed:float | unclassified | unclassified | 623 | Fleet batch-termination detection window in seconds. Default: 60. |
...
| `MASC_KEEPER_BODY_TIMEOUT_SEC` | string_literal | n/a | n/a | 503 | Total HTTP body-consumption deadline for one OAS streaming call. Wraps the body callback in [Eio.Time.with_timeout_ex... |
...
| `MASC_KEEPER_CASCADE_PROVIDER_ALLOWLIST` | string_literal | n/a | n/a | 876 | Comma-separated provider kind allowlist for every keeper cascade call. Values are OAS [Provider_config.string_of_prov... |
| `MASC_KEEPER_CLI_SUBPROCESS_IDLE_SEC` | typed:float | Timeouts | operator | 523 | Stdout-idle timeout for CLI subprocess transports (Kimi CLI today; Claude Code / Gemini CLI / Codex CLI need an OAS u... |
...
| `MASC_KEEPER_DOCKER_CONTAINER` | typed:string | unclassified | unclassified | 735 | Docker container name for keeper playground execution. Env: [MASC_KEEPER_DOCKER_CONTAINER]. Default: "keeper-playgrou... |
| `MASC_KEEPER_DOCKER_PLAYGROUND_ROOT` | typed:string | unclassified | unclassified | 745 | Container-side root under which keeper playground bundles are mounted. Host [<base_path>/.masc/playground/<keeper>/�... |
| `MASC_KEEPER_DOCKER_READ` | typed:bool | unclassified | unclassified | 790 | Legacy RFC-0006 Phase B-2 flag. Docker read routing now follows [sandbox_profile=docker] unconditionally. This getter... |
| `MASC_KEEPER_ESCALATION_THRESHOLD` | typed:int | unclassified | unclassified | 618 | Number of stale terminations within [termination_window_sec] before escalating to [Stale_termination_storm]. Default: 5. |
| `MASC_KEEPER_GRPC_MAX_RECONNECT` | typed:int | unclassified | unclassified | 637 | Maximum gRPC reconnect attempts before stopping the heartbeat fiber. Default: 5. Range: [1, 20]. |
| `MASC_KEEPER_GRPC_RECONNECT_BACKOFF_SEC` | typed:float | unclassified | unclassified | 645 | Backoff delay between gRPC reconnect attempts in seconds. Default: 5.0. Range: [1.0, 60.0]. |
...
| `MASC_KEEPER_IDLE_SKIP_THRESHOLD` | typed:int | unclassified | unclassified | 533 | Consecutive idle tool repetitions before on_idle hook issues Skip. Below this: graduated Nudge messages. With tool_ch... |
...
| `MASC_KEEPER_MAX_CONSECUTIVE_TOOL_FAILURES` | typed:int | unclassified | unclassified | 672 | Maximum consecutive failures for the same (tool_name, args_hash) before blocking further attempts. Prevents infinite ... |
...
| `MASC_KEEPER_PROACTIVE_MAX_ATTEMPTS` | typed:int | unclassified | unclassified | 655 | Maximum proactive generation attempts before falling back. Default: 3. Range: [1, 10]. |
...
| `MASC_KEEPER_STAGE_TIMING_RING_SIZE` | typed:int | unclassified | unclassified | 661 | Stage timing ring buffer size for Phase 0 profiling. Default: 100. Range: [10, 1000]. |
| `MASC_KEEPER_STREAM_IDLE_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 484 | Idle-gap timeout for streaming OAS provider responses. This bounds time between streamed lines, not total turn durati... |
| `MASC_KEEPER_SYMMETRIC_SANDBOX` | typed:bool | unclassified | unclassified | 782 | Legacy RFC-0006 Phase B-1 flag. Read-side containment now follows [sandbox_profile=docker] unconditionally. This gett... |
| `MASC_KEEPER_TERMINATION_WINDOW_SEC` | typed:float | unclassified | unclassified | 613 | Sliding window for stale-termination escalation tracking. Default: 21600 (6 hours). |
...
| `MASC_KEEPER_WATCHDOG_GRACE_SEC` | typed:float | unclassified | unclassified | 607 | Grace period after fiber start before idle-stale detection activates. Prevents false positives on server restart when... |
| `MASC_KEEPER_WATCHDOG_NOOP_THRESHOLD` | typed:int | unclassified | unclassified | 599 | Consecutive noop turns before considering the keeper stuck in a failure loop. Must be >= 2. Default: 3. |
| `MASC_KEEPER_WATCHDOG_POLL_SEC` | typed:float | unclassified | unclassified | 595 | Watchdog poll interval in seconds. Must be >= 5. Default: 30. |
| `MASC_KEEPER_WATCHDOG_PROGRESS_SEC` | typed:float | Timeouts | operator | 580 | Seconds since the last in-turn progress signal before an active turn is considered mid-turn-stale. Default: 300 (5 mi... |
| `MASC_KEEPER_WATCHDOG_STALE_SEC` | typed:float | unclassified | unclassified | 555 | Seconds since last turn before a Running keeper is considered idle-stale. Must be >= 60. Default: 300 (5 minutes). In... |
...
| `MASC_PAYLOAD_TELEMETRY` | typed:bool | unclassified | unclassified | 818 | Master switch for wake-payload measurement. Default off so the hot path is untouched until a baseline sweep is explic... |
...
| `MASC_BRIEFING_CACHE_TTL_SEC` | typed:float | unclassified | unclassified | 837 | Dashboard mission briefing cache TTL (seconds). Default: 300 (5 min). |
...
| `MASC_CANCELLATION_CLEANUP_SEC` | typed:float | unclassified | unclassified | 849 | Cancellation token cleanup interval (seconds). Default: 300 (5 min). |
...
| `MASC_COORD_GIT_LOCAL_OP_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 962 | Budget (seconds) for local-only git operations under [Masc_exec.Exec_gate.run_argv*] in {!Coord_git} and {!Coord_work... |
...
| `MASC_FULL_HEALTH_CRITICAL_FAILURE_THRESHOLD` | typed:int | unclassified | unclassified | 810 | Number of consecutive [/health?full=1] cache-refresh failures that must accumulate before [masc_full_health_refresh_c... |
| `MASC_FULL_HEALTH_REFRESH_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 798 | Full-health snapshot proactive refresh timeout (seconds). Caps a single [/health?full=1] cache refresh attempt in [Se... |
...
| `MASC_JANITOR_INTERVAL_SEC` | typed:float | unclassified | unclassified | 865 | Bootstrap janitor tick interval (seconds). Drives the SSE/session/ rate-limit/webrtc reaper loop in [server_bootstrap... |
| `MASC_KEEPER_BOOTSTRAP_WINDOW_SEC` | typed:float | unclassified | unclassified | 841 | Keeper world observation bootstrap window (seconds). Default: 300 (5 min). |
...
| `MASC_LABEL_QUIET_THRESHOLD_SEC` | typed:float | unclassified | unclassified | 829 | Dashboard label "quiet" threshold (seconds). Default: 300 (5 min). |
| `MASC_LABEL_STUCK_THRESHOLD_SEC` | typed:float | unclassified | unclassified | 833 | Dashboard label "stuck" threshold (seconds). Default: 900 (15 min). |
...
| `MASC_METRICS_FLUSH_SEC` | typed:float | unclassified | unclassified | 821 | Tool metrics flush interval (seconds). Default: 300 (5 min). |
...
| `MASC_PROVIDER_RUN_TTL_SEC` | typed:float | unclassified | unclassified | 853 | Provider run finished TTL (seconds). Default: 3600 (1 hour). |
...
| `MASC_RATE_LIMIT_BUCKET_TTL_SEC` | typed:int | unclassified | unclassified | 879 | Rate-limit bucket staleness TTL (seconds). Buckets with no traffic for this long are reaped by the janitor loop. Defa... |
| `MASC_RELAY_TARGET_AGENT` | typed:string | unclassified | unclassified | 117 | {1 Relay Configuration} |
| `MASC_REPO_SYNC_INTERVAL_SEC` | typed:float | unclassified | unclassified | 871 | Repository auto-sync interval (seconds). The repo_sync fiber in [server_bootstrap_loops] wakes at this cadence to fet... |
| `MASC_SESSION_LIVE_TURN_WINDOW_SEC` | typed:float | unclassified | unclassified | 825 | Team session live turn window (seconds). Default: 300 (5 min). |
...
| `MASC_SIDECAR_CONTROL_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 905 | Subprocess timeout (seconds) for sidecar control commands — [stop], [tail], and similar quick housekeeping operatio... |
| `MASC_SIDECAR_RECONCILE_BACKOFF_SEC` | typed:float | unclassified | unclassified | 893 | Backoff window (seconds) between repeated same-generation [running + unavailable] start dispatches. Default: 30 (matc... |
| `MASC_SIDECAR_SCHEMA_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 921 | Subprocess timeout (seconds) for sidecar Python schema generation. Wraps [Process_eio.run_argv_with_status] at [serve... |
...
| `MASC_SSE_BUFFER_TTL_SEC` | typed:float | unclassified | unclassified | 845 | SSE buffer TTL (seconds). Default: 300 (5 min). |
| `MASC_STALLED_SESSION_THRESHOLD_SEC` | typed:float | unclassified | unclassified | 857 | Operator digest stalled session threshold (seconds). Default: 300 (5 min). |
