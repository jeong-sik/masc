...
let load_from_path_result ~keeper_name path =
  let queue, read_errors = load_from_path_with_errors ~keeper_name path in
  ...
  | first :: rest ->
    let message error =
      ...
      | None -> error.message
      | Some path -> Printf.sprintf "%s: %s" path error.message
    in
    Error (String.concat "; " (List.map message (first :: rest)))
