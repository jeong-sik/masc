...
(** Increment {!fsm_drift_metric} with the supplied labels.
    ...
    {!Workspace_hooks.fsm_drift_observer_fn}. *)
...
(** Emit BOTH the variant-only {!fsm_drift_metric} and the
    ...
    {!Workspace_hooks.fsm_drift_observer_fn} at module load so every
    drift detected by [Workspace_task.transition] surfaces with agent
...
(** Increment {!distributed_lock_acquire_failed_metric} with
    ...
    {!Workspace_hooks.distributed_lock_acquire_failed_fn} so
    [Workspace_utils_ops] can fire the metric without taking a
