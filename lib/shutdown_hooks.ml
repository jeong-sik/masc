...
let sse_cleanup_ref : (unit -> int * int) option Atomic.t = Atomic.make None
let ws_cleanup_ref : (unit -> int * int) option Atomic.t = Atomic.make None
...
let register_sse_cleanup (f : unit -> int * int) =
  Atomic.set sse_cleanup_ref (Some f)
;;

let register_ws_cleanup (f : unit -> int * int) =
  Atomic.set ws_cleanup_ref (Some f)
;;
...
let run_all () =
  ...
  let sse_count, remaining_sse =
    match Atomic.get sse_cleanup_ref with
  ...
  let ws_count, remaining_ws =
    match Atomic.get ws_cleanup_ref with
    | Some cleanup -> cleanup ()
    | None -> 0, 0
  in
  Log.Server.info
