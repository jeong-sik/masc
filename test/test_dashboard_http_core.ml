...
let with_test_env f =
  ...
  Fun.protect
    ...
    (fun () ->
       with_env "MASC_STORAGE_TYPE" "filesystem"
       @@ fun () ->
       with_env "MASC_POSTGRES_URL" ""
       @@ fun () ->
       with_env "DATABASE_URL" ""
       @@ fun () ->
       with_env "SUPABASE_DB_URL" ""
       @@ fun () ->
       with_env "SB_PG_URL" ""
       @@ fun () ->
       Eio_main.run
...
let test_run_dashboard_compute_without_pool_stays_in_current_domain () =
  ...
  in
  check bool "no pool keeps compute on caller domain" true (result_domain = caller_domain)
;;
...
let test_run_dashboard_compute_with_pool_uses_executor_domain () =
  ...
  check
    bool
    "non-PG backend offloads to executor pool domain"
    true
...
let test_dashboard_shell_http_json_includes_paths () =
  ...
  let config_resolution =
    List.assoc_opt "config_resolution" fields |> Option.value ~default:`Null
  in
  let runtime_resolution =
    List.assoc_opt "runtime_resolution" fields |> Option.value ~default:`Null
  in
  ...
  let expected_masc_root =
    Unix.realpath (Filename.concat config.base_path Common.masc_dirname)
  in
  check
    bool
    "paths present"
    ...
    (match paths with
     | `Assoc _ -> true
     | _ -> false);
  check bool "paths key present" true (List.mem_assoc "paths" fields);
  check
    bool
    "config_resolution key present"
    true
    (List.mem_assoc "config_resolution" fields);
  check
    bool
    "runtime_resolution key present"
    true
    (List.mem_assoc "runtime_resolution" fields);
  check
    string
    "effective_base_path matches config"
    (Unix.realpath config.base_path)
    effective_base_path;
  check string "effective_masc_root matches config" expected_masc_root effective_masc_root;
  check
    bool
    "paths include cwd"
    true
  ...
  check
    bool
    "paths include strict_mode_requested bool"
    true
  ...
  check
    bool
    "paths include startup_rejected bool"
    true
  ...
  check
    bool
    "shell config resolution is object or null"
    true
  ...
  check
    bool
    "shell config root path surfaced when available"
    true
    (match config_resolution with
     | `Null -> true
     | _ ->
  ...
  check
    bool
    "shell cascade authoring path surfaced when available"
    true
    (match config_resolution with
     | `Null -> true
     | _ ->
  ...
  check
    bool
    "shell runtime resolution is object or null"
    true
  ...
  check
    bool
    "shell runtime data root path surfaced when available"
    true
    (match runtime_resolution with
     | `Null -> true
     | _ ->
  ...
  check
    bool
    "shell runtime warnings surfaced as list when available"
    true
    (match runtime_resolution with
     | `Null -> true
     | _ ->
...
let test_dashboard_shell_http_json_prefers_preserved_base_path_input () =
  ...
  with_env "MASC_BASE_PATH_INPUT" raw_input
  @@ fun () ->
  with_env "MASC_BASE_PATH" config.base_path
  @@ fun () ->
  let json = Lib.Server_dashboard_http_core.dashboard_shell_http_json config in
  let open Yojson.Safe.Util in
  check
    string
    "runtime base_path preserves raw input"
    raw_input
    (json
     |> member "runtime_resolution"
     |> member "base_path"
     |> member "path"
     |> to_string)
;;
...
let test_dashboard_shell_http_json_prefers_last_good_while_prewarming () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         string
         "last-good project reused"
         "warm-room"
         (json |> member "status" |> member "project" |> to_string);
       check
         int
         "last-good counts reused"
         7
         (json |> member "counts" |> member "agents" |> to_int))
;;
...
let test_dashboard_shell_auth_json_canonicalizes_token_owner () =
  ...
  | Ok (raw_token, _) ->
    ...
    check
      string
      "requested actor surfaced"
      "dashboard"
      (auth |> member "requested_agent" |> to_string);
    check string "token owner surfaced" "codex" (auth |> member "token_agent" |> to_string);
    check
      string
      "effective actor canonicalized to token owner"
      "codex"
      (auth |> member "effective_agent" |> to_string);
    check
      bool
      "auth error cleared after canonicalization"
      true
      (match auth |> member "auth_error_code" with
       | `Null -> true
       | _ -> false);
    check
      bool
      "keeper message allowed for canonicalized worker"
      true
...
let test_execution_actor_for_request_canonicalizes_token_owner () =
  ...
  | Ok (raw_token, _) ->
    ...
    check
      (option string)
      "execution actor canonicalized to token owner"
      (Some "codex")
      actor
;;
  ...
  | Ok (raw_token, _) ->
    let verifier =
      Lib.Server_routes_http_routes_verification.verifier_of_request
        ~base_path:config.base_path
        (request_with_headers
           "/api/v1/verification/resolve"
           [ "authorization", "Bearer " ^ raw_token; "x-masc-agent", "dashboard" ])
    in
    check
      string
      "verification verifier canonicalized to token owner"
      "operator:codex"
      verifier
;;
