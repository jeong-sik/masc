...
Some sites cannot use the loader. `masc.config` itself owns the canonical
reads, and its dependencies cannot read it back without a dependency cycle.
That set is not written down here — it is read from `dune describe`, so it
follows the build rather than a hand-maintained list.
...
def libraries_above_floor(local, floor) -> set[str]:
    """Local libraries that can consume masc.config instead of the env."""
    return {
        name
        for name, is_local in local.items()
        if is_local and name != CONFIG_LIB and name not in floor
    }
...
def main() -> int:
    ...
    above = libraries_above_floor(local, floor)
    ...
    if above_count > ceiling:
        print()
        print(
            f"FAIL: {above_count} direct env read(s) above the config floor, "
            f"baseline {ceiling}."
        )
    ...
    if above_count < ceiling:
        print()
        print(
            f"IMPROVED: {above_count} vs baseline {ceiling} — lower the ceiling "
...
(library
 ...
 (source_dir _build/default/lib/config))
(library
 (name masc.core)
 (uid core_uid)
 (local true)
 (requires ())
 (source_dir _build/default/lib/core))
(library
 (name external.lib)
 (uid external_uid)
 (local false)
 (requires ())
 (source_dir /opam/lib/external))
(library
 (name masc.app)
 (uid app_uid)
 (local true)
 (requires (config_uid))
 (source_dir _build/default/lib/app))
"""
    uid_to_name, requires, source_dir, local = library_graph(fixture)
    floor = config_floor(uid_to_name, requires, local)
    assert floor == {"masc.core"}, floor
    assert libraries_above_floor(local, floor) == {"masc.app"}
    assert repo_dir(source_dir[CONFIG_LIB]) == REPO / "lib/config"
...
if __name__ == "__main__":
    if sys.argv[1:] == ["--self-test"]:
        sys.exit(self_test())
    if sys.argv[1:]:
        sys.exit("usage: check_env_reads_below_config.py [--self-test]")
    sys.exit(main())
