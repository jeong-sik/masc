...
# their normal state), so the smoke asserts the contract attributes and the
# ABSENCE of the badge state. Assistant rows still carry the badge.
NO_TURN_REF_SELECTOR='[data-chat-entry-id="smoke-legacy-user"][data-chat-stream-contract-status="history_without_turn_ref"][data-chat-stream-contract-delivery-receipt="no_delivery_receipt"]:not([data-chat-stream-contract-badge-state])'
