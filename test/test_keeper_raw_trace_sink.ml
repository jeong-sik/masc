...
let ok_or_fail label = function
  ...
  | Error err ->
      Alcotest.fail
        (Printf.sprintf "%s: %s" label (Masc_agent_core.Error.to_string err))
...
(* Drive one turn's records through the sink with the OAS run API — the
   same append sequence [Masc_agent_core.Agent.run] performs when the agent is
   built with the sink. *)
let materialize_turn ~(meta : Keeper_meta_contract.keeper_meta) ~turn sink =
  let active =
    ok_or_fail "start_run"
      (Masc_agent_core.Raw_trace.start_run sink ~agent_name:meta.name
         ~prompt:(Printf.sprintf "turn-%d" turn) ())
  in
  ok_or_fail "finish_run"
    (Masc_agent_core.Raw_trace.finish_run active
...
let test_sink_path_and_session_identity () =
  ...
  Alcotest.(check bool) "second turn gets a fresh file" true
    (not
       (String.equal
          (Masc_agent_core.Raw_trace.file_path sink)
          (Masc_agent_core.Raw_trace.file_path sink2)))
...
let test_per_turn_files_isolate_turns () =
  ...
  let run_once ~turn =
    let sink =
      sink_or_fail "keeper_raw_trace_sink"
        (Keeper_agent_run.For_testing.keeper_raw_trace_sink ~config ~meta)
    in
    let (_ref : Masc_agent_core.Raw_trace.run_ref) =
      materialize_turn ~meta ~turn sink
    in
    let path = Masc_agent_core.Raw_trace.file_path sink in
  ...
  Alcotest.(check bool) "turn files are distinct" true
    (not (String.equal path1 path2));
  let records_of path =
    ok_or_fail "read_all" (Masc_agent_core.Raw_trace.read_all ~path ())
  in
  let count_started records =
    List.length
      (List.filter
         (fun (r : Masc_agent_core.Raw_trace.record) ->
           match r.record_type with
           | Masc_agent_core.Raw_trace.Run_started -> true
           | _ -> false)
         records)
  in
...
let test_corrupt_history_does_not_block_sink () =
  ...
  Alcotest.(check bool) "fresh file, not the corrupt one" true
    (not
       (String.equal (Masc_agent_core.Raw_trace.file_path sink) corrupt_turn_file));
...
let test_post_commit_cleanup_prunes_orphan_and_preserves_reference () =
  ...
  let sink =
    sink_or_fail "keeper_raw_trace_sink"
      (Keeper_agent_run.For_testing.keeper_raw_trace_sink ~config ~meta)
  in
  let current_path = Masc_agent_core.Raw_trace.file_path sink in
  let (_ref : Masc_agent_core.Raw_trace.run_ref) =
...
let test_traced_turn_yields_result_level_fields () =
  ...
  let (_ref : Masc_agent_core.Raw_trace.run_ref) =
    materialize_turn ~meta ~turn:1 sink
  in
  let trace_ref = Masc_agent_core.Raw_trace.last_run sink in
  let run_validation =
    match trace_ref with
    | Some ref_ ->
        Some
          (ok_or_fail "validate_run"
             (Masc_agent_core.Raw_trace_query.validate_run ref_))
    | None -> None
  in
  ...
  | Some r ->
      Alcotest.(check string) "trace_ref points at the per-turn sink file"
        (Masc_agent_core.Raw_trace.file_path sink)
        r.Masc_agent_core.Raw_trace.path);
  ...
  | Some v ->
      Alcotest.(check bool) "minimal traced run validates ok" true
        v.Masc_agent_core.Raw_trace.ok
