...
(* The window the turn driver would declare for this runtime, with the same
   ...
   spawned client owns its window and no Agent Core cut applies. The compiled
   default gives way to the model ({!Keeper_context_window.for_runtime}). *)
       ...
       (match Runtime.max_context_of_runtime_id runtime_id with
        | Some max_context ->
          (match
           ...
           | Keeper_context_window.Declared_window_exceeds_max_context
               { window_tokens; max_context } ->
             Error
               (Contradiction
                  (Printf.sprintf
