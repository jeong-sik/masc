  ...
  run
    ...
    [ ( "create"
      , [ test_case "create with required args only" `Quick (fun () ->
            ...
            check
              bool
              "allow_direct_call default"
              false
              spec.allow_direct_call_when_hidden;
            check
              bool
            ...
            check bool "effect_domain default" true (Option.is_none spec.effect_domain);
            check
              bool
              "requires_actor_binding default"
              true
    ...
    ; ( "register"
      , [ test_case "register populates tag_registry" `Quick (fun () ->
            ...
            check bool "tag is Mod_misc" true (tag = Tool_dispatch.Mod_misc))
        ...
        ; test_case "register sets read_only" `Quick (fun () ->
            ...
            in
            Tool_spec.register spec;
            check bool "is_read_only" true (Tool_dispatch.is_read_only "__test_spec_ro"))
        ; test_case "register non-read_only stays out of set" `Quick (fun () ->
            ...
            in
            Tool_spec.register spec;
            check bool "not read_only" false (Tool_dispatch.is_read_only "__test_spec_rw"))
    ...
    ; ( "tool_catalog_groups"
      , [ test_case
            "typed tool groups are exposed without string-prefix routing"
            `Quick
            (fun () ->
               let check_group name expected =
                 check
                   ...
                   (Some expected)
                   (Option.map
                      Tool_catalog.tool_group_to_string
    ...
    ; ( "verify_handler_coverage"
      , [ test_case "Tag_dispatch binding not in verify missing" `Quick (fun () ->
        ...
        ; test_case "Direct binding registers handler" `Quick (fun () ->
            ...
            let missing = Tool_spec.verify_handler_coverage () in
            check bool "not in missing" false (List.mem name missing))
        ] )
    ]
;;
