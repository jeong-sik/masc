...
> Historical audit response. Reverified 2026-07-19: the former global Keeper
> turn-slot limiter and its environment controls were removed. The corrected
> §0/§1-1 record below is not an active tuning contract.
...
| Semaphore wait > 180s (100%) | 작성 시점 전제. 현재 head에는 해당 global Keeper turn-slot limiter가 없으며 provider/client capacity는 typed runtime boundary에서 관측됨 | **historical** |
| Warmup 60s→255s (선형 → 8분 45초) | 산출 근거 미명시. 제거된 global slot knob로 현재 fleet를 tuning할 수 없음 | **historical** |
| `autonomous_available=14` | 제거된 semaphore surface의 historical observation | **historical** |
...
| 클레임 | 실제 | 분류 |
|--------|------|------|
| `let autonomous_sem = Eio.Semaphore.create 14` `let turn_sem = Eio.Semaphore.create 1` | 현재 head에는 global Keeper turn-slot module/knob 없음 | **historical** |
| 14 keeper가 1 turn slot 경쟁 → 36배 더 심각 | 현재 구조에 적용할 수 없는 제거된 전제. Provider/client capacity는 typed OAS/runtime 결과로 관측 | **historical** |

**§1-1 결과**: 이 섹션의 2026-05-05 tuning 제안은 superseded되었다.
제거된 permit 수나 env knob를 복원하지 않는다.
