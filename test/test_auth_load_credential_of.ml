...
let test_exact_match_load_ok () =
  with_temp_base (fun base ->
    write_cred ~base ~agent_name:"keeper-sangsu-agent" ~token_hash:"abc123";
    match
      Masc_mcp.Auth.load_credential_of
        base
...
let test_mismatch_rejects_even_when_resolved_stem_exists () =
  ...
  with_temp_base (fun base ->
    ...
    write_cred ~base ~agent_name:"keeper-sangsu-agent" ~token_hash:"canonical-token";
    match
      Masc_mcp.Auth.load_credential_of
        base
        ~ctx_agent_name:"keeper-sangsu-agent"
    ...
    | Error (Masc_mcp.Auth.Credential_missing _) ->
      ...
      exit 1
    | Error
        (Masc_mcp.Auth.Credential_mismatch { ctx_agent_name; resolved_credential_stem })
      ->
      assert (ctx_agent_name = "keeper-sangsu-agent");
      assert (resolved_credential_stem = "sangsu");
      print_endline "PASS: ctx<>stem -> Credential_mismatch (no silent fallback)")
;;
