...
let dedup_check key =
  with_dedup_lock (fun () ->
    ...
    if not result
    then (
      if Hashtbl.length dedup_table >= dedup_max_entries
      then (
...
let dedup_table_size () = with_dedup_lock (fun () -> Hashtbl.length dedup_table)
...
let make_dedup_cleanup_consumer () : (module Pulse.Consumer) =
  (module struct
    ...
    let on_beat _beat =
      ...
      | exn -> Error (Printf.sprintf "dedup_cleanup failed: %s" (Printexc.to_string exn))
    ;;
  end)
;;
...
let validate (msg : inbound_message) =
  Gate_protocol.validate ~max_content_length:(max_content_length ()) ~dedup_check msg
;;
  ...
  | Error e ->
    Channel_gate_metrics.record_attempt
      ...
      (match e with
       ...
       | _ -> Channel_gate_metrics.Validation_error (validation_error_to_string e));
    Error (Validation e)
  | Ok () ->
