...
function getSurfaceContext(surface, keepers, selId) {
  ...
  const MAP = {
    ...
    cockpit: { label: '코크핏 · Command Map', route: '/cockpit', scene: '라우트 커버리지 맵을 함께 보는 중',
      fields: [{ k: 'routes', v: '30' }, { k: 'planes', v: '5' }, { k: 'blocked', v: '1', tone: 'warn' }] },
...
const DOCK_STARTERS = {
  ...
  '/cockpit': ['커버리지 갭 어디야?', 'blocked 라우트 보여줘'],
...
function buildReply(keeper, ctx) {
  ...
  if (ctx.route === '/cockpit') {
    return { body: `**Command Map**을 같이 보고 있어요. 30개 라우트 중 1건이 \`backend-blocked\` — goal snapshot diff가 백엔드 대기입니다.`,
      sug: ['blocked 라우트 상세', '커버리지 갭 목록', 'work 플레인부터 보기'] };
  }
  ...
  return (
    <aside ref={rootRef} className={`dock ${docked ? 'docked' : 'float'}`} style={floatStyle} data-screen-label="Copilot 도크">
      ...
      <div className="dock-coview">
        <div className="dock-coview-h"><span className="lbl">지금 함께 보는 화면</span><span className="route mono">{ctx.route}</span></div>
