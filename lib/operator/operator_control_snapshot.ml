...
let keeper_tool_audit_fields
  ...
  in
  match
    ( A2a_tools.latest_heartbeat_task meta.agent_name
    , A2a_tools.latest_heartbeat_result meta.agent_name )
  with
                       ...
                       then (
                         ...
                         let tool_audit_source =
                           match
                             U.to_option
                               U.to_string
                               (U.member "tool_audit_source" audit_json)
                           with
                           | v -> v
                           | exception U.Type_error _ -> None
                         in
                         ...
                         let pipeline_stage =
                           ...
                           | Some phase ->
                             Keeper_exec_status.pipeline_stage_of_phase phase
                           | None -> "offline"
                         in
                         let phase_str =
                           match registry_phase with
                         ...
                         Some
                           (`Assoc
                               ([ "runtime_class", `String "keeper"
                                ...
                                ; ( "last_model_used"
                                  , `String meta.runtime.usage.last_model_used )
                                ; ( "active_model"
                                ...
                                ; ( "last_autonomous_action_at"
                                  , if
                                      String.trim meta.runtime.last_autonomous_action_at
                                      = ""
                                    then `Null
                                ...
                                ; ( "tool_audit_source"
                                  , string_option_to_json tool_audit_source )
                                ; "tool_audit_at", string_option_to_json tool_audit_at
                                ; ( "last_speech_act"
                                  , string_option_to_json
                                      (let value =
                                         String.trim meta.runtime.last_speech_act
                                       in
                                ...
                                ; ( "last_social_transition_reason"
                                  , string_option_to_json
                                      (let value =
                                         String.trim
                                           meta.runtime.last_social_transition_reason
                                       in
                                       if value = "" then None else Some value) )
                                ; "proactive_enabled", `Bool meta.proactive.enabled
                                ; "proactive_idle_sec", `Int meta.proactive.idle_sec
                                ; ( "proactive_cooldown_sec"
                                  , `Int meta.proactive.cooldown_sec )
                                ; ( "turn_budget"
                                ...
                                ; ( "last_proactive_preview"
                                  , string_option_to_json
                                      (let value =
                                         String.trim
                                           meta.runtime.proactive_rt.last_preview
                                       in
                                       if value = "" then None else Some value) )
                                ; ( "last_blocker"
                                  , string_option_to_json
                                  ...
                                  , let t_act = Time_compat.now () in
                                    let result =
                                      ...
                                      then (
                                        ...
                                        let lines =
                                          let dated =
                                            Dated_jsonl.read_recent_lines store 5
                                          in
                                          if dated <> []
                                          then dated
                                          else (
