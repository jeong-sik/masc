...
p=argparse.ArgumentParser();p.add_argument('--tui',type=Path,required=True);p.add_argument('--port',type=int,required=True);p.add_argument('--commit',required=True);p.add_argument('--root',type=Path,required=True);p.add_argument('--evidence',type=Path,required=True);p.add_argument('--checkout',type=Path,required=True);a=p.parse_args()
root=a.root.resolve()
assert root==Path('/var/folders/bv/cjrbl01x52s6j80krdfb63400000gp/T/masc-browser-handoff-yq3d680h').resolve(), 'only the owned scratch runtime is allowed'
evidence=a.evidence.resolve();assert evidence.parent==root
report=json.loads((evidence/'report.json').read_text());keeper=report['keeper']
assert report['build']['binary_commit']==a.commit, 'producer and history reader use different source commits'
...
for row in rows:
 for ref in row.get('artifact_refs',[]):
  ...
  expected[(row['execution_id'],blob['sha256'])]=(row,json.loads(raw),blob)
assert expected, 'no retained observations for this producer'
count=len(expected)
roster=read('/api/v1/gate/keepers?detailed=true')
assert roster['truncated'] is False and keeper in [k['name'] for k in roster['keepers']]
roster_count=len(roster['keepers'])
sys.path.insert(0,str(a.checkout.resolve()/'test'));import test_tui_keyboard_input as h
...
try:
 ...
 for step in range(roster_count+1):
 ...
 for index in range(count):
  ...
  row,scene,blob=expected[(context['execution_id'],context['artifact']['_blob']['sha256'])]
  ...
  h.resize_and_wait(proc,master,output,rows=35,columns=131,needle=b'retained observations',controls=(h.FULL_REDRAW,),final_cursor=b'\x1b[?25l')
  h.resize_and_wait(proc,master,output,rows=35,columns=130,needle=b'retained observations',controls=(h.FULL_REDRAW,),final_cursor=b'\x1b[?25l')
  (out/f'observation-{index}.pty').write_bytes(output)
  if index<count-1:send(b']',f'Observation {index+2}/{count}'.encode())
 assert {(c['execution_id'],c['artifact']['_blob']['sha256']) for c in contexts}==set(expected)
finally:
