...
```
총 도구 스키마 바이트                     459,939,613
그중 keeper_tool_search 가 없는 요청      157,953,353   (34.3%)
```

(138개, 목록 없음) 요청 374건, (150개, 목록 없음) 요청 177건. Keeper 별로 자기 도구
바이트 중 이 레인이 차지하는 몫은 lane-smith 98.1%, code-reviewer 71.7%.

이 초안이 앞서 실었던 **28.2% (162,412,956 / 575,346,108) 는 철회한다.** 같은 창을 두
번 독립적으로 재서 34.3% 와 34.4% 를 얻었고, 6,182 라는 행 수는 세 번 모두 같았다.
회전 보존이 앞쪽 파일을 지웠다는 초안의 설명도 함께 철회한다 —— 회전은 행을 지우므로
행 수가 같을 수 없다. 두 값은 시점이 아니라 **집계 방법**이 다르다. 어느 방법이
575,346,108 을 냈는지는 확인되지 않았고, 재현되는 값만 남긴다.
