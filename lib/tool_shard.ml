...
let shell_tools : Types.tool_schema list =
  [ { name = "keeper_shell"
    ; description =
        "Run a safe project shell command. ops: pwd, ls, cat, rg, git_status, find, \
         head, tail, wc, tree, git_log, git_diff, bash, git_clone, gh. Read-only ops \
         default to the keeper sandbox. IMPORTANT: paths resolve automatically — use \
         ...
         pattern=\"*.ml\"). bash op: single command only, no chaining (&&, ||, |, ; are \
         blocked), no redirects (>, >>). git_clone: clone a repo into your sandbox \
         repos/ lane (url required). gh op: run a gh CLI subcommand with \
         cmd=\"<subcommand>\" (e.g. cmd=\"pr list --state open\"). Requires an active \
         claimed task/current_task_id because repo context is derived from the task \
         worktree. Always run `gh pr list` first before referencing a PR number to avoid \
         hallucinations. Dangerous commands (repo delete, auth logout, secret \
         set/delete) are blocked. If path not found, clone the repo first with \
         op=git_clone. Use rg for pattern search, find for path discovery, head/tail for \
         line ranges, git_log/git_diff for repo history, bash for curl/jq/env/which, gh \
         for GitHub PR/issue/CI."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
           ...
           omit git_worktree even though the handler accepted it. *)
                  ...
                  , `Assoc
                      ...
                      ; "description", `String "Command to run"
                      ] )
                ; ( "cmd"
                  , `Assoc
                      [ "type", `String "string"
                      ...
                      ; ( "description"
                        , `String
                            "Optional working directory for \
                             pwd/git_status/git_log/git_diff/git_worktree/bash. Must \
                             stay within the keeper sandbox or an explicit allowed path."
                        )
                      ] )
                ; ( "pattern"
                  , `Assoc
                ...
                ; ( "command"
                  , `Assoc
                      ...
                      ; ( "description"
                        , `String
                            "Single shell command for bash op. No chaining (&&/||/;/|) \
                             or redirects (>/>>) allowed. Read-only." )
                      ] )
                ; ( "timeout_sec"
                  , `Assoc
                      [ "type", `String "number"
                      ; ( "description"
                        , `String "Timeout seconds for bash op (default: 30, max: 180)" )
                      ] )
                ; ( "url"
                  , `Assoc
                      [ "type", `String "string"
...
let coding_keeper_bridge_tools : Types.tool_schema list =
  [ { name = "keeper_bash"
    ; description =
        "Execute ONE shell command. NO chaining (&&, ||, ;), NO pipes (|), NO redirects \
         (> >>). Violations are blocked. Good: cmd='dune build', cmd='ls -la lib/'. Bad: \
         cmd='cd x && dune build', cmd='rg foo | wc -l'. Runs in the keeper sandbox by \
         default; use cwd to target an explicit allowed directory. Paths resolve \
         automatically — never include host storage prefixes such as \
         '.masc/playground/your-name/' in cwd. Use 'repos/X' instead. For read-only ops \
         use keeper_shell, for file edits use keeper_fs_edit. Set run_in_background=true \
         for long-running tasks (returns background_task_id; poll with \
         keeper_bash_output, terminate with keeper_bash_kill)."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
            , `Assoc
                [ ( "cmd"
                  , `Assoc
                      ...
                      ; ( "description"
                        , `String
                            "Single command only. No chaining/pipe/redirect. Example: \
                             'dune build', 'rg pattern lib/'" )
                      ] )
                ; ( "cwd"
                  , `Assoc
                      [ "type", `String "string"
  ...
  [ { name = "keeper_preflight_check"
    ; description =
        "Validate prerequisites before starting autonomous work: gh auth, repo access, \
         keeper identity, preset level, repo readiness. Returns structured JSON with all \
         check results. Read-only, no side effects."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
...
let keeper_pr_review_tools : Types.tool_schema list =
  ...
  ; { name = "keeper_pr_review_comment"
    ; description =
        "Submit a PR review with optional inline comments. Events: COMMENT, APPROVE, \
         REQUEST_CHANGES. Requires delivery or coding preset. Pass the PR number as \
         `pr_number` (preferred) or `number` (legacy alias)."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
  ...
  ; { name = "keeper_pr_review_reply"
    ; description =
        "Reply to a specific PR review comment. Requires delivery or coding preset. Pass \
         the PR number as `pr_number` (preferred) or `number` (legacy alias)."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
            ...
            , `Assoc
             ...
             Hand-rolled enum used to drop awaiting_verification. *)
                        ...
                        , `List
                            (List.map
                               (fun s -> `String s)
                               Types.valid_task_status_strings) )
  ...
  ; { name = "keeper_task_claim"
    ; description =
        "Claim the next unclaimed todo task that matches your capabilities. Returns \
         ...
         If the keeper has active_goal_ids configured, only goal-linked tasks are \
         eligible."
    ; input_schema = `Assoc [ "type", `String "object"; "properties", `Assoc [] ]
    }
...
let autoresearch_keeper_tools : Types.tool_schema list = Tool_autoresearch_schemas.schemas
...
let revoke_shard (active_shards : string list) (shard_name : string)
  ...
  | Some shard ->
    ...
    else Ok (List.filter (fun n -> n <> shard_name) active_shards)
;;
...
let keeper_model_tools : Types.tool_schema list = tools_of_shards default_shard_names
