...
let eq_class t label expected actual =
  Alcotest.(check string) label (GEC.to_string expected) (GEC.to_string actual);
  ignore t
;;
...
let test_auth_failure_variants () =
  ...
  let c_login = GEC.classify ~exit_code:1 ~stderr:"Please run: gh auth login" in
  eq_class () "gh auth login prompt → Auth_failed" GEC.Auth_failed c_login;
  let c_403 = GEC.classify ~exit_code:1 ~stderr:"HTTP 403 Forbidden" in
  eq_class () "403 Forbidden → Auth_failed" GEC.Auth_failed c_403
;;

let test_network_variants () =
  let dns = GEC.classify ~exit_code:1 ~stderr:"Could not resolve host: api.github.com" in
  eq_class () "DNS fail → Network" GEC.Network dns;
  let tls = GEC.classify ~exit_code:1 ~stderr:"remote error: tls: handshake failure" in
  eq_class () "TLS handshake → Network" GEC.Network tls;
  let timeout =
    GEC.classify ~exit_code:1 ~stderr:"dial tcp 140.82.114.4:443: i/o timeout"
  in
...
let test_type_mismatch () =
  ...
  let c_flag = GEC.classify ~exit_code:1 ~stderr:"Error: unknown flag: --foo" in
  eq_class () "unknown flag → Type_mismatch" GEC.Type_mismatch c_flag;
  let c_cmd =
    GEC.classify ~exit_code:1 ~stderr:"Error: unknown command \"foobar\" for \"gh\""
  in
...
let test_make_carries_interpretation () =
  ...
  Alcotest.(check string) "class" (GEC.to_string GEC.Auth_failed) (GEC.to_string r.class_);
  match r.interpretation with
  | Some s ->
    Alcotest.(check bool)
      "interpretation mentions auth"
      true
      (String.length s > 0
       && String.length s >= 4
       &&
       ...
       let rec sub_at i =
         if i + 4 > String.length lower
         then false
         else if String.sub lower i 4 = "auth"
         then true
         else sub_at (i + 1)
       in
       sub_at 0)
...
let test_to_legacy_err_prefix () =
  ...
  | Error body ->
    ...
    let has_prefix = String.length body >= 14 && String.sub body 0 13 = "[Auth_failed]" in
...
let test_overrides_precedence () =
  ...
  GEC.install_overrides
    [ { GEC.exit_code = 1; stderr_contains = Some "QUOTA"; class_ = GEC.Policy_blocked } ];
...
let () =
  Alcotest.run
    ...
    ; ( "gh_result"
      , [ Alcotest.test_case
            "make carries interpretation"
            `Quick
            test_make_carries_interpretation
