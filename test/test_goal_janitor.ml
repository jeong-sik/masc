...
let make_goal ?(status = Goal_store.Active) ?(days_ago = 0) id title =
  ...
  ; priority = 3
  ; status
  ; phase = Goal_store.phase_of_goal_status status
...
let test_purge_old_dropped () =
  ...
  Goal_store.write_state
    config
    { version = 1; updated_at = Types.now_iso (); goals = [ g1; g2; g3 ] };
...
let test_stagnate_old_active () =
  ...
  Goal_store.write_state
    config
    { version = 1; updated_at = Types.now_iso (); goals = [ g1; g2 ] };
  let result = Goal_janitor.run config in
