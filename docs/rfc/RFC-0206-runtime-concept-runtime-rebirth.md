...
| `binding` | Layer 3 record (provider_id/model_id/is_default/max_concurrent/price_*/keep_alive/num_ctx) + `binding_key` | `runtime_binding` |
...
| `Runtime_metrics.on_{provider_cooldown,runtime_audit_failure,resolve_live_fallback,runtime_metrics_eviction}` | 9 | **신규** `Runtime_metrics.on_*` prometheus emitter (오염 runtime_metrics 미복원; 순수 카운터만) |
