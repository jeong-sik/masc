  ...
  | `Assoc fields ->
    ...
    (match json |> member "model" with
     | `String s when String.trim s <> "" ->
       ...
       let stc =
         parse_supports_tool_choice_field (json |> member "supports_tool_choice")
       in
    ...
    let required_capability_profile =
      ...
      | `String s ->
        ...
        then Cp_unset
          ...
          match Cascade_capability_profile.resolve_required_capabilities trimmed with
...
let detect_fallback_cycles (entries : catalog_entry list) : string list list =
  ...
  let rec walk visited current =
    ...
    | Some entry ->
      (match entry.fallback_cascade with
       ...
       | Some next -> walk (current :: visited) next)
  in
  List.filter_map (fun entry -> walk [] entry.name) entries
  |>
  ...
  List.fold_left
    (fun (seen, acc) cycle ->
       ...
       if List.mem key seen then seen, acc else key :: seen, cycle :: acc)
    ([], [])
  |> snd
  |> List.rev
;;
         ...
         (match StringMap.find_opt target_name by_name with
          ...
          | Some (target : catalog_entry) ->
            let assignable_violation =
              if entry.keeper_assignable && not target.keeper_assignable
              then
                Some
                  ( entry.name
                  , target_name
                  , "assignable cascade falls back to sink (keeper_assignable=false)" )
              else None
            in
            let cap_violation =
              match
                entry.required_capability_profile, target.required_capability_profile
              with
              | Some src_p, Some dst_p ->
                if not (Cascade_tier.is_subset_named_profile src_p dst_p)
                then (
                  ...
                  | Some _ ->
                    Some
                      ...
                      , target_name
                      , Printf.sprintf
                          "capability profile %s is not a subset of target profile %s"
                          src_p
                          dst_p ))
                else None
              | _ -> None
  ...
  | Ok (`Assoc fields) ->
      ...
      List.filter_map
        (fun (name, builder) ->
           ...
           | Cp_invalid raw ->
             Some
               (Printf.sprintf
                  "cascade %s: unknown required_capability_profile %S (known built-in: \
                   %s; declared: %s)"
                  name
                  raw
                  (Cascade_capability_profile.all_profiles |> String.concat ", ")
    ...
    then Error (String.concat "; " invalid_profile_errors)
      ...
      let entries =
        List.map
          (fun (name, builder) ->
             ...
             in
             { name
             ; keeper_assignable = Option.value builder.keeper_assignable ~default:true
      ...
      then
        Error
          (Printf.sprintf
             "cascade capability mismatch (RFC-0055): %s"
             (mismatches
...
let load_profile ~config_path ~name =
  load_profile_weighted ~config_path ~name |> List.map (fun e -> e.model)
;;
