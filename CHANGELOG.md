...
- A board-attention judgment is no longer settled as "candidate absent" when the candidate's ledger row is unreadable. The ledger read skips rows it cannot decode, so a Relevant judgment whose candidate row had bad JSON or an unsupported `schema_version` was closed without being delivered. A refused row that names the candidate, or whose `candidate_id` cannot be read, now fails the delivery and the partition keeps its judgment until the row is readable again (#38041).
