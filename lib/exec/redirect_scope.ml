...
let pp fmt = function
  ...
  | Literal { bytes } ->
      (* The bytes are the caller's content and can hold anything, so the
         rendering says how many there are and not what they say. *)
      Format.fprintf fmt "0<<(%d bytes)" (String.length bytes)
