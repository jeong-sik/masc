...
DEFAULT_ME_ROOT="$(cd "${ROOT_DIR}/../../.." && pwd)"
...
if [ "$MCP_URL" = "http://127.0.0.1:${PORT}/mcp" ]; then
  ...
  env -i \
    ...
    MASC_LOCAL_RUNTIME_COOLDOWN_SEC="${MASC_LOCAL_RUNTIME_COOLDOWN_SEC:-${MASC_LLAMA_RUNTIME_COOLDOWN_SEC:-}}" \
    MASC_LOCAL_RUNTIME_DEBUG="${MASC_LOCAL_RUNTIME_DEBUG:-${MASC_LLAMA_RUNTIME_DEBUG:-}}" \
    ...
    MASC_WORKSPACE_ROOT="${MASC_WORKSPACE_ROOT:-$ROOT_DIR}" \
    ME_ROOT="${ME_ROOT:-$DEFAULT_ME_ROOT}" \
