...
let make_meta ?(name = "test-keeper") ?(policy_voice_enabled = false) ()
...
let has_keeper_prefix name = String.length name >= 7 && String.sub name 0 7 = "keeper_"
...
let test_heuristic_only_keeper_prefixed () =
  ...
  Alcotest.(check (list string))
    "heuristic keeper only has keeper_* or curated masc_* tools"
    []
    unexpected
;;
...
let test_learned_only_keeper_prefixed () =
  ...
  in
  Alcotest.(check (list string))
    "learned keeper only has keeper_* or curated masc_* tools"
    []
    unexpected
;;
...
let test_research_extra_tools_are_research_only () =
  ...
  let unexpected =
    List.filter (fun n -> not (List.mem n (known_non_keeper_tool_names ()))) non_keeper
  in
  Alcotest.(check (list string)) "non-keeper tools come from known sources" [] unexpected
;;
...
let test_no_overlap_heuristic_vs_agent () =
  ...
  let overlap =
    List.filter
      (fun n ->
         List.mem n agent_names && not (List.mem n known_shared_agent_keeper_tool_names))
      keeper_names
  in
  Alcotest.(check (list string))
...
let test_heuristic_has_fewer_tools_than_learned () =
  ...
  let learned = make_meta ~policy_voice_enabled:true () in
  ...
  Alcotest.(check bool) "learned mode has >= heuristic tools" true (l_count >= h_count)
;;
...
let test_strip_keeper_prefix_double () =
  Alcotest.(check string)
    "strips outer keeper- leaving keeper-admin"
    "keeper-admin"
...
let test_keeper_agent_name_prefixed () =
  Alcotest.(check string)
    "no double prefix in agent_name"
    "keeper-admin-agent"
...
let test_keeper_name_from_agent_name_roundtrip () =
  Alcotest.(check (option string))
    "agent alias resolves to keeper name"
    (Some "sangsu")
...
let test_canonical_keeper_name_from_generated_nickname () =
  Alcotest.(check (option string))
    "generated nickname resolves to canonical keeper"
    (Some "claude")
...
let test_canonical_keeper_name_from_keeper_agent_alias_preserves_full_name () =
  Alcotest.(check (option string))
    ...
    (Keeper_types.canonical_keeper_name_from_agent_name "keeper-kimi-null-canary-agent")
;;

let test_canonical_keeper_name_from_legacy_keeper_name () =
  Alcotest.(check (option string))
    "legacy keeper-prefixed name normalizes"
    (Some "sangsu")
...
let () =
  ...
  Alcotest.run
    "Keeper_agent_isolation"
    [ ( "non_research_prefix"
      , [ Alcotest.test_case
            "heuristic only keeper_*"
            `Quick
            test_heuristic_only_keeper_prefixed
        ; Alcotest.test_case
            "learned only keeper_*"
            `Quick
            test_learned_only_keeper_prefixed
        ] )
    ; ( "research_boundary"
      , [ Alcotest.test_case
            "research extras are research-only"
            `Quick
    ...
    ; ( "agent_no_keeper_tools"
      , [ Alcotest.test_case
            "spawned agent surface"
            `Quick
            test_agent_surface_no_keeper_tools
        ] )
    ; ( "disjoint_namespaces"
      , [ Alcotest.test_case
            "shared tool whitelist stays real"
            `Quick
            test_known_shared_tools_exist_on_agent_surface
        ; Alcotest.test_case
            "heuristic vs agent"
            `Quick
            test_no_overlap_heuristic_vs_agent
        ; Alcotest.test_case "research vs agent" `Quick test_no_overlap_research_vs_agent
        ; Alcotest.test_case
            "shard vs agent"
            `Quick
            test_shard_tools_overlap_with_agent_documented
        ] )
    ; ( "admin_boundary"
      , [ Alcotest.test_case
            "research admin overlap documented"
            `Quick
            test_research_admin_overlap_documented
        ; Alcotest.test_case
            "non-research admin documented"
            `Quick
            test_non_research_admin_tools_documented
        ] )
    ; ( "policy_consistency"
      , [ Alcotest.test_case
            "learned >= heuristic"
            `Quick
            test_heuristic_has_fewer_tools_than_learned
        ] )
    ; ( "keeper_prefix_no_double"
      , [ Alcotest.test_case "strip no prefix" `Quick test_strip_keeper_prefix_no_prefix
        ...
        ; Alcotest.test_case
            "sender plain name"
            `Quick
            test_keeper_agent_sender_plain_name
        ; Alcotest.test_case
            "sender prefixed name"
            `Quick
            test_keeper_agent_sender_prefixed_name
