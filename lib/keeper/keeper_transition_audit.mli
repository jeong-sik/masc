...
val record_transition : keeper_name:string -> transition_record -> unit
...
val recent_transitions_json : keeper_name:string -> limit:int -> Yojson.Safe.t

(** Record a completed keeper turn in the per-keeper ring buffer (last 50). *)
val record_completed_turn : keeper_name:string -> completed_turn_record -> unit

(** Retrieve recent completed keeper turns for a keeper, newest first. *)
val recent_completed_turns : keeper_name:string -> limit:int -> completed_turn_record list
