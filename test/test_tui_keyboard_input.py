...
def keeper_lanes_ia_interaction(
    ...
    def interact(
        ...
        # screen says "MASC Keepers (0)", so `j` on an empty list moves nothing
        # -- correctly. A timeout dumps the screen from when the wait gave up,
        # rows by then drawn, which made this read as a dropped key. Holding the
        # briefing read fails a bare `j` every time and passes
        # select_keeper_row every time (#35754).
        select_keeper_row(process, master_fd, output, b"beta")
        if not wait_for_fixture_event(
            process, master_fd, output, gate.requested, timeout=10.0
        ):
