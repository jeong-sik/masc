...
- Keeper: `<base-path>/.masc/keepers/<keeper>/turn-boundaries.jsonl` records where each finished turn left the durable history (`turn_ended`, with the atom count and the digest of the last atom), and a `history_restarted` line after `masc_keeper_clear`, at the start of a turn on an empty history, and after the first accepted save of a turn that started fresh because its saved checkpoint version was superseded. `masc-librarian-replay` reads it, and nothing else does. It is append-only, never trimmed, and removed with a keeper purge. A line that cannot be written never fails the turn or the clear; it is logged at ERROR and counted in `masc_keeper_turn_boundary_failures_total` (#37020, #37024, #37027, #37030).
