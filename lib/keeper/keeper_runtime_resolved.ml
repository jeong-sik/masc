...
let admission_wait_timeout_sec_live () =
  Float.max
    5.0
    (Float.min 1200.0 (get_float ~default:180.0 "MASC_KEEPER_ADMISSION_WAIT_TIMEOUT_SEC"))
;;
...
let freeze_from_current () =
  let source_field name value =
    { value; source = Option.value ~default:Default (source_of_env_name name) }
  in
  ...
  let autonomous_max_turns_per_call =
    let source =
      match
        source_of_env_name "MASC_KEEPER_OAS_MAX_TURNS_PER_CALL_SCHEDULED_AUTONOMOUS"
      with
      | None | Some Default -> Derived
      | Some other -> other
    in
    { value = autonomous_max_turns_per_call_live (); source }
  in
  let reactive_max_idle_turns =
    source_field "MASC_KEEPER_MAX_IDLE_TURNS_REACTIVE" (reactive_max_idle_turns_live ())
  in
  ...
  let turn_timeout_sec =
    source_field "MASC_KEEPER_TURN_TIMEOUT_SEC" turn_timeout_sec_value
  in
  ...
  let oas_timeout_override_sec =
    ...
    ; source =
        Option.value ~default:Default (source_of_env_name "MASC_KEEPER_OAS_TIMEOUT_SEC")
    }
  in
...
let reset_for_tests () = Atomic.set frozen None
...
let to_yojson (runtime : t) =
  `Assoc
    [ ( "bootstrap_max_active_keepers"
      , field_to_yojson (fun value -> `Int value) runtime.bootstrap_max_active_keepers )
    ; ( "reactive_max_turns_per_call"
      , field_to_yojson (fun value -> `Int value) runtime.reactive_max_turns_per_call )
    ; ( "autonomous_max_turns_per_call"
      , field_to_yojson (fun value -> `Int value) runtime.autonomous_max_turns_per_call )
    ; ( "reactive_max_idle_turns"
      , field_to_yojson (fun value -> `Int value) runtime.reactive_max_idle_turns )
    ; ( "autonomous_max_idle_turns"
      , field_to_yojson (fun value -> `Int value) runtime.autonomous_max_idle_turns )
    ; ( "turn_timeout_sec"
      , field_to_yojson (fun value -> `Float value) runtime.turn_timeout_sec )
    ; ( "admission_wait_timeout_sec"
      , field_to_yojson (fun value -> `Float value) runtime.admission_wait_timeout_sec )
    ; ( "oas_timeout_override_sec"
      , field_to_yojson option_float_to_yojson runtime.oas_timeout_override_sec )
    ; ( "oas_timeout_per_1k"
      , field_to_yojson (fun value -> `Float value) runtime.oas_timeout_per_1k )
    ; ( "oas_timeout_per_turn"
      , field_to_yojson (fun value -> `Float value) runtime.oas_timeout_per_turn )
    ]
;;

let bootstrap_max_active_keepers () = (current ()).bootstrap_max_active_keepers.value
let reactive_max_turns_per_call () = (current ()).reactive_max_turns_per_call.value
let autonomous_max_turns_per_call () = (current ()).autonomous_max_turns_per_call.value
let reactive_max_idle_turns () = (current ()).reactive_max_idle_turns.value
let autonomous_max_idle_turns () = (current ()).autonomous_max_idle_turns.value
let turn_timeout_sec () = (current ()).turn_timeout_sec.value
let admission_wait_timeout_sec () = (current ()).admission_wait_timeout_sec.value
