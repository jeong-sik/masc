...
- `POST /api/v1/keepers/chat/stream` accepts an optional `since_seq : int` (>= 0) next to `request_id` — absent or `null` means the whole turn, `n` means events after seq `n`, a negative value is a 400 (#33173, 2026-09-05; before it the wire took `-1` for the whole turn); when the owner already holds the operation (`existing`) the server replays journaled events with `seq > since_seq` through the same projection, then attaches live. Seqs of events that project to nothing (connector-only blocks) leave gaps: `since_seq` is a journal position, not a frame count.
...
5. **Reconnect = `since_seq` = the log's resume position** (`After_seq` of the last seq held; a log that holds nothing sends no `since_seq`, which asks for the whole turn — #33173), same decoder state, seq dedup on `add`. Entries without a seq (ACCEPTED, synthesized RUN_ERROR) are never deduplicated.
...
**Superseded in part by #33173 (2026-09-05).** The `-1` sentinel left the wire: the position is `Masc.Keeper_chat_event_log.replay_position` (`Whole_turn | After_seq n`), `request_to_yojson` omits `since_seq` for `Whole_turn`, `fetch_keeper_chat_events` omits the query parameter the same way, `Log.resume_position` replaces `last_seq` as what the watcher and `journal_resume_position` carry, and `next_since_seq` may be `null` on an empty whole-journal page. One behaviour moved with it: re-POSTing an existing operation without a position now replays the whole turn (it used to be live-only). The TUI always sends a position once it holds a log, so only a fresh open of an existing operation sees the difference — the recovery re-POST of a prepared request is that case.
