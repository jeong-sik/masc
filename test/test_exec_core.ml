...
let get_string_field json key = json |> member key |> to_string
...
let test_rg_no_match_is_semantic_success () =
  ...
  check string "semantic_status" "no_match" (get_string_field json "semantic_status");
  check
    string
    "family"
    "search"
...
let test_find_missing_path_is_runtime_error () =
  ...
  in
  check bool "ok" false (json |> member "ok" |> to_bool);
  check string "semantic_status" "runtime_error" (get_string_field json "semantic_status")
;;
...
let test_blocked_json_adds_classification () =
  let json =
    Masc_mcp.Exec_core.blocked_result_json
      ...
      ()
  in
  check string "error" "write_operation_gated" (get_string_field json "error");
  check string "semantic_status" "blocked" (get_string_field json "semantic_status");
  check
    string
    "family"
    "git_write"
  ...
  check string "retryability" "operator_required" (get_string_field json "retryability")
;;
...
let test_unknown_write_is_not_git_write () =
  let classification = Masc_mcp.Exec_core.classify_command ~cmd:"mkdir tmp/generated" in
  check
    string
    "family"
    "unknown"
...
let test_large_output_persists_artifact () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check string "persisted contents" output (Fs_compat.load_file artifact_path))
;;
...
let test_semantic_fail_carries_exit_code () =
  with_semantic_flag true (fun () ->
    let json =
      run_json ~cmd:"ls /no/such/path" ~status:(Unix.WEXITED 2) ~output:"ls: …"
    in
    let sem = json |> member "semantic_exit" in
...
let test_semantic_git_not_a_repo () =
  with_semantic_flag true (fun () ->
    ...
    in
    let sem = json |> member "semantic_exit" in
    check string "kind=git_not_a_repo" "git_not_a_repo" (sem |> member "kind" |> to_string))
;;
...
let test_semantic_tool_missing_payload () =
  with_semantic_flag true (fun () ->
    let json =
      run_json
        ~cmd:"notarealtool --help"
        ~status:(Unix.WEXITED 127)
        ~output:"bash: notarealtool: command not found"
    in
    let sem = json |> member "semantic_exit" in
    check string "kind=tool_missing" "tool_missing" (sem |> member "kind" |> to_string);
    check string "tool=notarealtool" "notarealtool" (sem |> member "tool" |> to_string))
;;
...
let test_markers_git_not_a_repo () =
  with_semantic_flag true (fun () ->
    with_markers_flag true (fun () ->
      ...
      check string "confidence" "exact" (m |> member "confidence" |> to_string)))
;;
...
let test_markers_unknown_output_emits_absent () =
  ...
  with_semantic_flag true (fun () ->
    with_markers_flag true (fun () ->
      let json = run_json ~cmd:"ls" ~status:(Unix.WEXITED 0) ~output:"hello world" in
      match json |> member "verifiable_markers" with
      | `Null -> ()
...
let test_output_cap_absent_by_default () =
  with_env "MASC_BASH_OUTPUT_CAP" None (fun () ->
    let json = run_json ~cmd:"ls" ~status:(Unix.WEXITED 0) ~output:"hello" in
...
let test_output_cap_on_preserves_small_output () =
  with_env "MASC_BASH_OUTPUT_CAP" (Some "1") (fun () ->
    let json = run_json ~cmd:"ls" ~status:(Unix.WEXITED 0) ~output:"short" in
    check string "short output unchanged" "short" (json |> member "output" |> to_string);
  ...
  with_env "MASC_BASH_OUTPUT_CAP" (Some "1") (fun () ->
    with_env "MASC_BASH_CAP_HEAD" (Some "8") (fun () ->
      with_env "MASC_BASH_CAP_TAIL" (Some "8") (fun () ->
        ...
        let json = run_json ~cmd:"cat big" ~status:(Unix.WEXITED 0) ~output:long in
...
let test_blocked_with_diagnosis_has_all_fields () =
  ...
  check string "explanation" "explaining why" (d |> member "explanation" |> to_string);
  check string "rewrite" "use this instead" (d |> member "rewrite" |> to_string);
  check
    bool
    "tool_suggestion absent"
    true
...
let test_blocked_diagnosis_both_rewrite_and_tool () =
  ...
  in
  let d = json |> member "diagnosis" in
  check string "rewrite" "split into two calls" (d |> member "rewrite" |> to_string);
  check
    string
    "tool_suggestion"
    "keeper_shell"
...
let test_failed_git_status_has_no_structured_output () =
  ...
  check
    bool
    "no structured_output on failed git status"
    true
...
let test_failed_git_log_has_no_structured_output () =
  ...
  check
    bool
    "no structured_output on failed git log"
    true
    (match json |> member "structured_output" with
     | `Null -> true
     | _ -> false)
...
let test_unknown_cmd_no_structured () =
  ...
  check
    bool
    "no structured_output for unknown"
    true
    (match json |> member "structured_output" with
     | `Null -> true
     | _ -> false)
...
let test_history_suggest_empty () =
  ...
  @@ fun base_path ->
  let module H = Masc_exec.Bash_history in
  let results = H.suggest ~base_path ~keeper_name:"no-keeper" ~pattern:"x" ~limit:5 in
...
let test_history_compaction () =
  ...
  for i = 1 to 15 do
    H.append
      ~base_path
      ~keeper_name:"compact-test"
      { ts = float_of_int i
      ; cmd_hash = string_of_int i
...
let () =
  run
    ...
    [ ( "semantic_status"
      , [ test_case
            "rg no match is semantic success"
            `Quick
            test_rg_no_match_is_semantic_success
        ; test_case
            "find partial is recoverable but not success"
            `Quick
            test_find_partial_is_semantic_success
        ; test_case
            "find missing path is runtime error"
            `Quick
            test_find_missing_path_is_runtime_error
        ; test_case
            "quoted regex pipe keeps no-match semantics"
            `Quick
            test_regex_pipe_inside_quotes_keeps_no_match_semantics
        ; test_case
            "blocked json adds classification"
            `Quick
            test_blocked_json_adds_classification
        ; test_case
            "unknown write is not git_write"
            `Quick
            test_unknown_write_is_not_git_write
        ; test_case
            "large output persists artifact"
            `Quick
            test_large_output_persists_artifact
        ; test_case
            "inline only keeps artifact refs empty"
            `Quick
            test_inline_only_keeps_artifact_refs_empty
        ] )
    ; ( "semantic_exit"
      , [ test_case
            "explicit opt-out hides field"
            `Quick
            test_semantic_hidden_on_explicit_off
        ; test_case "ok kind on exit 0" `Quick test_semantic_ok_when_flag_on
        ; test_case
            "fail kind carries exit_code"
            `Quick
            test_semantic_fail_carries_exit_code
        ; test_case
            "git exit 128 maps to git_not_a_repo"
            `Quick
            test_semantic_git_not_a_repo
        ; test_case
            "exit 127 maps to tool_missing with tool payload"
            `Quick
            test_semantic_tool_missing_payload
        ; test_case "flag flip isolates state" `Quick test_semantic_flag_isolation
        ] )
    ; ( "verifiable_markers"
      , [ test_case
            "absent when markers flag off"
            `Quick
            test_markers_absent_when_flag_off
        ; test_case
            "absent when semantic flag off"
            `Quick
            test_markers_absent_when_semantic_off
        ; test_case "git_not_a_repo emits exact marker" `Quick test_markers_git_not_a_repo
        ; test_case
            "unknown output emits no marker field"
            `Quick
            test_markers_unknown_output_emits_absent
        ] )
    ; ( "output_cap"
      , [ test_case "absent by default" `Quick test_output_cap_absent_by_default
        ; test_case
            "small output unchanged, cap metadata emitted"
            `Quick
            test_output_cap_on_preserves_small_output
        ; test_case
            "large output truncated with bytes_dropped > 0"
            `Quick
            test_output_cap_truncates_large_output
        ] )
    ; ( "p8_diagnosis"
      , [ test_case
            "no diag => diagnosis absent"
            `Quick
            test_blocked_without_diagnosis_has_no_field
        ; test_case
            "diag with rewrite => all fields present"
            `Quick
            test_blocked_with_diagnosis_has_all_fields
        ; test_case "diag with tool_suggestion" `Quick test_blocked_with_tool_suggestion
        ; test_case
            "diag with both rewrite and tool"
            `Quick
            test_blocked_diagnosis_both_rewrite_and_tool
        ] )
    ; ( "p10_structured_output"
      , [ test_case
            "git status --porcelain produces structured fields"
            `Quick
            test_git_status_structured
        ; test_case
            "git status with global option produces structured fields"
            `Quick
            test_git_status_with_global_option_structured
        ; test_case
            "git log --oneline produces commits array"
            `Quick
            test_git_log_structured
        ; test_case
            "failed git status has no structured_output"
            `Quick
            test_failed_git_status_has_no_structured_output
        ; test_case
            "failed git log has no structured_output"
            `Quick
            test_failed_git_log_has_no_structured_output
        ; test_case "wc -l produces lines count" `Quick test_wc_structured
        ; test_case
            "git diff --stat produces summary counts"
            `Quick
            test_git_diff_stat_structured
        ; test_case
            "git diff --stat plural summary counts"
            `Quick
            test_git_diff_stat_structured_plural
        ; test_case
            "unknown cmd has no structured_output"
            `Quick
            test_unknown_cmd_no_structured
        ; test_case
            "dune runtest produces passed/failed counts"
            `Quick
            test_dune_test_structured
        ] )
    ; ( "p11_command_history"
      , [ test_case "append and read history entries" `Quick test_history_append_and_read
        ; test_case
            "suggest returns empty for nonexistent"
            `Quick
            test_history_suggest_empty
        ; test_case "cmd_hash produces 12-char hex" `Quick test_history_cmd_hash
        ; test_case
            "compaction preserves entries below threshold"
            `Quick
            test_history_compaction
        ] )
    ]
;;
