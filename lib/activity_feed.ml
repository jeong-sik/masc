...
let activity_item_to_json : activity_item -> Yojson.Safe.t = activity_item_to_yojson
...
let parse_created_at_or_fallback ~kind ~path raw =
  ...
  then fallback ()
    ...
    try
      Scanf.sscanf raw "%d-%d-%dT%d:%d:%d" (fun y m d h mi s ->
        let tm =
          { Unix.tm_sec = s
          ; tm_min = mi
          ; tm_hour = h
    ...
    | exn ->
      Log.Feed.warn
        "%s timestamp parse error for %s: %s"
        kind
        path
...
let task_activities (config : Coord.config) : activity_item list =
  ...
  if (not (Fs_compat.file_exists tasks_dir)) || not (Sys.is_directory tasks_dir)
  then []
  else (
    let files =
      try Sys.readdir tasks_dir |> Array.to_list with
      | Sys_error msg ->
    ...
    |> List.filter_map (fun fname ->
      ...
      | Ok json ->
        ...
        then None
          ...
          let created_at =
            parse_created_at_or_fallback ~kind:"task activity" ~path created_at_str
          in
...
let board_post_activities (config : Coord.config) : activity_item list =
  ...
  |> List.filter_map (fun json ->
    ...
    let created_at = json_created_at_or_fallback ~kind:"board post" ~path json in
    if id = ""
    then None
    else (
      let preview =
...
let board_comment_activities (config : Coord.config) : activity_item list =
  ...
  |> List.filter_map (fun json ->
    ...
    let author = Safe_ops.json_string ~default:"" "author" json in
    let content = Safe_ops.json_string ~default:"" "content" json in
    let created_at = json_created_at_or_fallback ~kind:"board comment" ~path json in
    if id = ""
    then None
    else (
      let preview =
        (* UTF-8-safe truncation (#7690). *)
        String_util.utf8_safe ~max_bytes:83 ~suffix:"..." content |> String_util.to_string
      in
      Some
        { id = "act-comment-" ^ id
        ; kind = "board_comment"
...
let mention_activities (config : Coord.config) : activity_item list =
  ...
  |> List.filter_map (fun json ->
    ...
    let created_at = json_created_at_or_fallback ~kind:"mention inbox" ~path json in
    if id = ""
    then None
    else
      Some
        ...
        ; summary =
            Printf.sprintf
              ...
              source_agent
              target_agent
              (String_util.utf8_safe ~max_bytes:63 ~suffix:"..." content_preview
               |> String_util.to_string)
        ; detail_json = json
        ; created_at
        })
...
let recent_activity (config : Coord.config) ?agent_name ~(limit : int) ()
  ...
  let filtered =
    ...
    | None -> all_items
    | Some name -> List.filter (fun item -> item.agent_name = name) all_items
  in
  let sorted = List.sort (fun a b -> compare b.created_at a.created_at) filtered in
