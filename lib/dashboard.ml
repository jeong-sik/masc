...
let max_message_length () =
  Runtime_params.get Governance_registry.dashboard_max_message_length
;;

(** Maximum pending tasks to show. *)
let max_pending_tasks () =
  Runtime_params.get Governance_registry.dashboard_max_pending_tasks
;;

(** Maximum recent messages to show. *)
let max_recent_messages () =
  Runtime_params.get Governance_registry.dashboard_max_recent_messages
;;

(** Minimum section border length. *)
let min_border_length () =
  Runtime_params.get Governance_registry.dashboard_min_border_length
;;
...
let format_section (s : section) : string =
  ...
  let content =
    if s.content = []
    then [ Printf.sprintf "  %s" s.empty_msg ]
    else List.map (fun line -> Printf.sprintf "  %s" line) s.content
  in
...
let truncate_message (msg : string) : string =
  ...
  String_util.utf8_safe ~max_bytes:limit ~suffix:"..." msg |> String_util.to_string
;;
...
let task_lines (tasks : Types.task list) =
  ...
  if pending_more > 0
  then content @ [ Printf.sprintf "   ... +%d more pending" pending_more ]
  else content
;;
...
let add_group label lines empty_msg =
  if lines = []
  then [ Printf.sprintf "%s: %s" label empty_msg ]
  else (label ^ ":") :: List.map (fun line -> "  " ^ line) lines
;;
...
let normalize_worktree_branch branch =
  ...
  if
    ...
    && String.sub branch 0 (String.length prefix) = prefix
  then
...
let worktrees_section (config : Coord_utils.config) : section =
  ...
  let content =
    List.map
      (fun (branch, path) -> Printf.sprintf "%s -> %s" branch (truncate_path path))
      worktrees
  in
      ...
      then (
        ...
        List.fold_left
          (fun acc name ->
             ...
             then acc + count_lock_files full
             else if Filename.check_suffix name ".flock"
             then acc
             else acc + 1)
          0
          entries)
  ...
  let content =
    List.map
      (fun snapshot ->
         ...
         Printf.sprintf
           "%s%s: %d agents | %d active | %d pending | %d locks"
           snapshot.room_id
...
let room_section now (snapshot : room_snapshot) : section =
  ...
  let content =
    [ Printf.sprintf
        "Summary: %d agents | %d active | %d pending | %d locks"
        (List.length snapshot.agents)
        (List.length active)
        (List.length pending)
        snapshot.locks
  ...
  { title =
      (if snapshot.is_current
       then Printf.sprintf "Namespace: %s (flattened current)" snapshot.room_id
       else Printf.sprintf "Namespace: %s" snapshot.room_id)
  ; content
  ; empty_msg = ""
  }
;;
...
let agent_workflow_section now (_config : Coord_utils.config) (agents : Types.agent list)
  : section
  =
  let content =
    agents
    |> List.filter (fun (a : Types.agent) ->
      match a.status with
      | Types.Active | Types.Busy -> true
      | Types.Listening | Types.Inactive -> false)
...
let agents_grouped_section now (agents : Types.agent list) : section =
  ...
  let offline =
    List.filter
      (fun a -> Dashboard_labels.classify_agent ~now a = Dashboard_labels.Offline)
      agents
  in
  let content =
    (if working <> [] then add_group "Working" (List.map format_agent working) "" else [])
    @ (if stuck <> [] then add_group "Stuck" (List.map format_agent stuck) "" else [])
    @ (if idle <> [] then add_group "Idle" (List.map format_agent idle) "" else [])
    @ if offline <> [] then add_group "Offline" (List.map format_agent offline) "" else []
  in
  { title = "Agents"; content; empty_msg = "(no agents)" }
;;
...
let format_elapsed_float now ts =
  ...
  then Printf.sprintf "%.0fs" elapsed
  else if elapsed < 3600.0
  then Printf.sprintf "%.0fm" (elapsed /. 60.0)
...
let keepers_section now : section =
  ...
  let sorted =
    List.sort
      (fun (a : Keeper_registry.registry_entry) b -> String.compare a.name b.name)
      entries
  in
  let format_entry (e : Keeper_registry.registry_entry) =
    ...
    let last_info =
      ...
      | Some err -> Printf.sprintf " | err=%s" (truncate_message err)
      | None -> ""
    in
    Printf.sprintf
      "%s: %s | seq=%d | since=%s%s"
...
let generate ?(scope = All) (config : Coord_utils.config) : string =
  ...
  let sections =
    ...
    ; messages_section (List.concat_map (fun s -> s.messages) snapshots)
    ]
  in
  let tempo = Tempo.get_tempo config in
  let worktrees = parse_worktrees (Coord.worktree_list config) in
  let total_locks = List.fold_left (fun acc s -> acc + s.locks) 0 snapshots in
  let footer =
    Printf.sprintf
      "-- Tempo: %.0fs | Locks: %d | Worktrees: %d"
      tempo.Tempo.current_interval_s
      total_locks
      (List.length worktrees)
  in
...
let generate_compact ?(scope = All) (config : Coord_utils.config) : string =
  ...
  let idle_count =
    List.length
      (List.filter
         ...
         all_agents)
  in
  let offline_count = List.length all_agents - working_count - stuck_count - idle_count in
  ...
  String.concat
    "\n"
    [ Printf.sprintf
        "MASC [%s namespace] %d agents / %d tasks"
        current_room
        (List.length all_agents)
        (List.length all_tasks)
    ; Printf.sprintf "ATTENTION: %s" attention_line
    ; Printf.sprintf
        "AGENTS: %d working / %d idle / %d stuck / %d offline | TASKS: %d active / %d \
         pending / %d blocked"
        working_count
        idle_count
        stuck_count
    ...
    ; Printf.sprintf "KEEPERS: %d running / %d dead / %d other" k_running k_dead k_other
    ]
;;
