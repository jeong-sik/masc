---
rfc: "0118"
title: "KCT NoTerminalRuntime S1 — typed Result at select_runtime boundary + Zombie mapping correction"
status: Implemented
created: 2026-05-17
updated: 2026-05-22
author: vincent
...
# RFC-0118: KCT NoTerminalRuntime S1 — typed Result at select_runtime boundary + Zombie mapping correction

## §1 Problem (caller-context)

`docs/tla-audit/kct-c3-terminal-runtime-contract-gap-2026-05-12.md` 가 `KeeperCoreTriad.tla` (KCT, KSM/KTC 와 별개 spec) §S1 `NoTerminalRuntime` 와 OCaml `keeper_runtime_routing.ml:32-34` 의 *dormant contract violation* 을 문서화. Spec-runtime drift family 의 5번째 변형.
...
Spec 는 6 KSM phase 를 `"Terminal"` abstract phase 로 collapse 하고 *그 phase 에서는 runtime 가 "none"* 라고 단언.

### OCaml violation

`lib/keeper/keeper_runtime_routing.ml:32-34`:

```ocaml
| Offline | Stopped | Dead | Zombie | Crashed | Restarting ->
    { effective_runtime = base_runtime;        (* NOT "none" *)
      reason = "non-turn phase (blocked upstream)" }
```

OCaml 가 6 phase 모두에서 `base_runtime` (real runtime name) 반환 — *spec 명세 "none" 위반*. Reason string 가 "blocked upstream" 으로 자백.

추가: `Paused` 는 별도 arm 에서 `Draining | Paused -> base_runtime` (line 29-31), *같은 값* 반환 *다른 reason* ("winding down: complete in-progress work"). `Zombie` 는 spec mapping comment 에 *omit* — 의도 불분명.

### Why dormant (production stays correct today)

Audit doc 가 명시:

> The `base_runtime` return for terminal phases is *unreachable in production*, making this a paper contract gap rather than a runtime bug.
...
근본 원인: **`select_runtime : phase -> runtime_result` signature 가 terminal-phase 경우 *Result.None* 또는 *Result.Error* 가 아닌 *real value*. 의미적 nullity 가 type 에 표현 안됨.**

## §2 Approach

3 layer:

**Layer A — Typed Result at `select_runtime` (R-C-3.a)**

```ocaml
module Runtime_selection : sig
  type t =
    | Active of { effective_runtime : string; reason : string }
    | None_terminal of { phase : string; reason : string }
    (* terminal phases: spec S1 honored at type *)
end

val select_runtime : phase -> ... -> Runtime_selection.t
```

호출자는 `match` 강제. terminal phase 시 `None_terminal _` 반환, *real runtime name 반환 불가* — type system 가 S1 enforce.
...
기존 `NoTerminalRuntime` 는 *function return* 에 관한 invariant. 이를 *call graph* 에 관한 invariant 로 보강:
...
`TerminalRuntimeNeverQueried` 가 OCaml 의 *실제 upstream gating* 을 spec 가 모델. dashboard caller 는 *read-only* 라서 violation 가능 — 그건 typed `None_terminal` 처리로 OK.
...
| P2 | `Runtime_selection.t` typed module + `select_runtime` signature change | dune build PASS, 2 caller 모두 match exhaustive |
...
| P4 | KCT spec §102 Zombie 추가 + `TerminalRuntimeNeverQueried` invariant 추가 | TLC PASS clean, KCT-buggy.cfg `BuggyTerminalRuntime` mutation 가 violation 검출 |
| P5 | Telemetry counter `keeper_runtime_selection_terminal_total{phase}` — **Counter-as-Validator** | 4주 monitoring count == 0 (production 에서 dormant 유지) |
...
1. **Q1**: `Runtime_selection.None_terminal` vs `option` 형 (`Runtime_selection.t option`)? typed sum 가 *phase 정보* 까지 carry — 호출자가 debugging 정보 활용 가능. **잠정**: typed sum (option 보다 정보 풍부).

2. **Q2**: Dashboard caller 가 *Read-only* — Spec `NoTerminalRuntime` 만으로 dashboard 가 *None* 표시 가능, *real runtime name* 표시 가능. **잠정**: dashboard 가 typed `None_terminal { phase; reason }` 표시 — operator 가 더 informative.
...
- **Runtime routing 자체 변경**: 본 RFC 는 *terminal 처리* 만, normal runtime selection 무영향.
...
- **Risk 2**: P4 spec PR 의 `TerminalRuntimeNeverQueried` 가 *현재 production* 보다 강한 invariant — 향후 dashboard 새 endpoint 가 violation 가능. → invariant 가 `c \notin {"TurnExecution"}` 만 — read context 허용.
...
- [ ] P1: RFC body merge.
- [ ] P2: `Runtime_selection.t` typed module + signature change.
- [ ] P3: 2 caller (`keeper_unified_turn:411`, `server_dashboard:1423`) typed match.
- [ ] P4: KCT spec §102 Zombie + `TerminalRuntimeNeverQueried` invariant.
