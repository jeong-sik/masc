    ...
    ~finally:(fun () ->
      let rec rmrf p =
        ...
        | { st_kind = Unix.S_DIR; _ } ->
          Sys.readdir p |> Array.iter (fun e -> rmrf (Filename.concat p e));
...
let test_docker_keeper_blocks_outside () =
  ...
  let outside = "/etc/passwd" in
  match Keeper_sandbox_containment.check_read_target ~config ~meta ~target:outside with
  | Ok () -> Alcotest.fail "expected containment to block /etc/passwd for minjae"
  | Error msg ->
    Alcotest.(check bool)
      "error mentions symmetric_sandbox_blocked"
      true
      (let needle = "symmetric_sandbox_blocked" in
       let len = String.length needle in
       String.length msg >= len && String.sub msg 0 len = needle)
;;
...
let test_docker_keeper_allows_inside_playground () =
  ...
  Alcotest.(check bool)
    "playground-internal path is allowed"
    true
    (Result.is_ok
       (Keeper_sandbox_containment.check_read_target ~config ~meta ~target:inside))
;;
...
let test_docker_git_creds_contained () =
  ...
  let outside = "/etc/passwd" in
  Alcotest.(check bool)
    "Docker is also subject to containment"
    true
    (Result.is_error
       (Keeper_sandbox_containment.check_read_target ~config ~meta ~target:outside))
;;
...
let test_path_just_outside_playground_blocked () =
  ...
  Alcotest.(check bool)
    "lookalike sibling path is blocked"
    true
    (Result.is_error
       (Keeper_sandbox_containment.check_read_target ~config ~meta ~target:sibling))
;;
...
let () =
  Alcotest.run
    ...
    [ ( "containment"
      , [ Alcotest.test_case
            "legacy keeper always allowed"
            `Quick
            test_legacy_keeper_always_allowed
        ; Alcotest.test_case
            "docker keeper blocks /etc/passwd"
            `Quick
        ...
        ; Alcotest.test_case
            "lookalike sibling path blocked"
            `Quick
