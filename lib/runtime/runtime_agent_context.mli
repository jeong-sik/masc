...
type config = {
  ...
  call_timeout_s : float option;
      (** Bound on a non-streaming completion as a whole, the wait for the
          ...
          exact-fit path the count-tokens round trip spends from the same
          window first. A caller that has one deadline for its whole call (a
          fusion panelist or judge) sets this together with
          [body_timeout_s]; the streaming path does not read it. *)
