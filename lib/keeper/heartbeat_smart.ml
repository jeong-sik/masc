...
let effective_interval ~config ~last_activity =
  ...
  if idle_duration > config.idle_threshold_s
  then config.base_interval_s *. config.idle_multiplier
  else config.base_interval_s
;;

let should_emit ~config ~agent_status ~last_activity ~last_heartbeat =
  let now = Time_compat.now () in
  (* Rule 1: Skip if busy and busy_skip is enabled *)
  if config.busy_skip && agent_status = Types.Busy
  then Skip_busy
  else (
