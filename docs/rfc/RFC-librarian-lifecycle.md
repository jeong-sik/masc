...
1. 스냅숏이 이력과 맞지 않게 되면 `[0, N)` 을 다시 읽는다. 이 크기를 묶는 물리 조건이 없다. 2026-09-22 goo-yang-bong 은 옛 규칙 purge 뒤 12,756 atom 을 읽으려 했고, 회차마다 `12,756 → 6,378 → 3,189` 를 걷고 끝났다. 10:38:41Z 이후 커밋이 없었다.
2. 턴이 끝난 자리를 남기지 않는 턴이 있다. 공식 클라이언트 턴은 `No_atom_history` 다(§4.8). checkpoint 저장이 실패한 Agent Core 턴은 줄이 아예 없다(#37102, #37851). 자를 자리가 없는 구간에서는 단위가 없고, 그래서 atom 을 반으로 접는 탐색이 생겼다. 탐색은 단위 없이 단위를 지어내는 일이다.

**왜 턴이 물리적 단위인가.** 한 턴의 atom 은 그 턴의 마지막 요청과 마지막 응답 안에 있었다. 창에 들어갔다는 것을 Keeper 자신이 증명한 크기다. Librarian 은 도구 호출과 결과를 한 줄로 읽으므로(`keeper_librarian.ml` 의 `text_of_content`) 그보다 작다. 그러므로 "이전 상태 + facts + 턴 하나 + 지시문" 이 후보 창에 들어가는 조건은 후보의 선언 창이 그 Keeper 런타임의 선언 창보다 작지 않은 것이다. 이 조건은 보내 보고 아는 것이 아니라 설정을 읽을 때 아는 것이다.

**다른 제품은 무엇을 단위로 잡나 (2026-09-22 확인).** Claude Code, Claude Developer Platform, Codex CLI, Letta/MemGPT, LangChain, Gemini CLI, Mem0 전부 "이미 창 안에 있던 조각"을 이전 요약과 접는다. 디스크의 원문을 0 부터 다시 읽어 요약을 재구축하는 곳은 없다. 넘치면 전부 유한한 손실을 택한다 — Codex 는 가장 오래된 항목을 하나씩 떼고, LangChain 은 4,000 토큰으로 자르고, Gemini 는 truncation 으로 내려간다. 제품들은 "창 = 이력" 이라 턴 경계를 찾을 필요가 없다. 이 RFC 는 이력을 창 밖의 durable 원문으로 두고 별도 프로세스가 흡수하는 더 어려운 구조를 골랐고(§1), 그 대가가 "턴이 끝난 자리를 그 순간에 기록한다" 이다. 출처: `code.claude.com/docs/en/how-claude-code-works`, `platform.claude.com/docs/en/build-with-claude/context-editing`, `github.com/openai/codex` `codex-rs/core/src/compact.rs`, arXiv 2310.08560 p.3, `docs.letta.com/v1-sdk/messages/compaction`, langchain `libs/langchain_v1/.../middleware/summarization.py`, gemini-cli `packages/core/src/context/chatCompressionService.ts`, arXiv 2504.19413.

**결정.**

1. **단위는 턴이고, 끝난 자리를 그 순간에 남긴다.** 누가 돌렸든 같다. 공식 클라이언트 턴도 원문과 atom 위치를 남긴다(#37207). checkpoint 저장이 실패한 턴도 줄을 남긴다(#37102). "턴이 어디서 끝났나" 는 그 순간에만 아는 사실이라 나중에 복원할 수 없다. 이미 남기지 못한 구간은 영구 빚이고, §4.3 의 atom 사다리는 그 빚에만 남는다. 두 이슈가 닫히면 사다리를 지운다.
2. **working state 는 턴마다 이어 쓰는 fold 이고, 자동으로 다시 만들지 않는다.** 상태 n+1 = f(상태 n, 턴 n). purge 가 atom 을 지키면 digest 만 다시 서명한다(#37751 이 맞는 스냅숏에 대해 이미 그렇게 한다). trace 나 이력 세대가 바뀌면 이전 상태를 "물려받은 상태" 로 표시해 새 이력 첫 회차의 입력으로 넣고, 위치는 새 이력의 0 에 둔다. 0 부터 다시 쓰기는 운영자가 명시적으로 시키는 동작으로만 남긴다. 스냅숏의 뜻(요약)과 무결성 증거(digest)는 다른 것이다. 증거가 깨졌다고 요약을 버리지 않는다.
3. **들어감은 설정을 읽을 때 검사한다.** Librarian 레인의 후보가 선언한 `max-context` 가 그 레인을 쓰는 Keeper 런타임의 선언 `max-context` 보다 작으면 설정 로드를 거절한다. 고른 숫자가 아니라 선언된 능력 둘의 비교라 §5 와 충돌하지 않는다. 오늘처럼 보내 보고 거절로 아는 대신, 뜨기 전에 안다.
4. **손실은 typed 로 기록되고 화면에 보인다.** 한 턴이 그래도 안 들어가면 메시지 단위로 내려가고, typed 크기 거절이 있을 때만 읽지 않은 조각을 기록하고 지나간다(#37850, §4.3 바닥 규칙). 모름은 내려가되 건너뛰지 않는다. 연속성 밀림은 I4 가 센다(#37793).
5. **커밋 뒤에 잰다.** 연속성 커밋 뒤 다음 요청의 carried range 는 커밋 전보다 작아야 한다. 아니면 ERROR 로그를 남긴다. Gemini CLI 가 압축 뒤 토큰이 안 줄면 되돌리는 것과 같은 자리이되, 되돌리지는 않는다. 문턱 숫자 없이 앞뒤 크기만 비교한다.

**보장.** 매 턴 상태가 앞으로 가거나, 못 간 조각이 기록된다. 기억이 자동으로 버려지는 길이 없다. 어떤 읽기도 "이미 들어갔던 크기" 를 넘지 않는다. 보장하지 못하는 것은 셋이다. 기록된 유한한 손실. 이미 남기지 못한 옛 구간(사다리가 소화한다). atom 폭이 바이트로 정확하지 않은 것(#37207 까지).

**하지 않는 것.** 요약으로 이력을 고쳐 쓰지 않는다(§5). 상태 텍스트에 토큰 상한을 두지 않는다 — 상태는 다시 쓰기라 작게 머문다(2026-09-22 실측 8 개 Keeper 1,487~2,567 자). 커지면 3 이 시끄럽게 거절한다.

**순서.**

1. §4.3 개정과 사다리 코드(#37846, 후속 PR). 지금 이력을 소화한다.
2. 바닥 규칙(#37850). 영원한 정지를 없앤다.
3. I4 연속성 밀림(#37793). 정지가 보이게 한다.
4. 설정 검사(3)와 커밋 뒤 검사(5).
5. 세대 전이의 물려받기(2).
6. #37207 과 #37102. 단위를 세운다.
7. 사다리 삭제.

**검증.** 하네스: 턴 하나의 Librarian 렌더링 크기가 그 턴의 마지막 요청 크기(`turn-records` 의 `request_body_bytes`)를 넘지 않는다. 라이브: 재시작 뒤 첫 회차가 한 턴만 읽고 커밋한다. 세대 전이 뒤 첫 요청에 물려받은 상태가 실린다. 설정 검사가 작은 후보를 가진 레인을 거절한다.
