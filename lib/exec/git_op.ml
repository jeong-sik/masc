     ...
     | sub :: rest ->
       (match sub with
        ...
        | "branch" ->
          ...
          then Ok (Destructive `Branch_delete)
        ...
        | "push" ->
          if has_flag rest "--force" || has_flag rest "-f"
          then Ok (Destructive `Push_force)
          else Ok (Mutating `Push)
        | "reset" when has_flag rest "--hard" -> Ok (Destructive `Reset_hard)
