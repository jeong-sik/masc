...
1. `lib/tool_shard_types_schemas_bash.ml` 의 `keeper_bash` descriptor 는 typed
   schema 하나만 emit 한다.
2. Public schema:
   - `input_schema.properties` 에서 `cmd` 필드 없음
   - `oneOf` 는 `executable | pipeline | stages` 3 branch
   - top-level `required` 없음; branch 선택은 `oneOf` 가 담당
   - description 은 typed argv / explicit pipeline 사용만 안내한다.
3. **Reader-side reject**:
   `lib/keeper/keeper_tool_bash_input.ml` 이 `cmd` 입력을
   `Result.Error "cmd string is not a typed keeper_bash input; provide
   executable/argv or pipeline stages"` 로 거부한다.
...
- [x] `keeper_bash` tool descriptor JSON schema 가 `{executable, argv, ...}` 구조.
- [ ] CI: representative bash invocation tests green.
...
1. **keeper LLM 적응 transition**. PR-3 머지 직후 keeper persona LLM 이 새
   ...
   bash call 이 boundary parse fail 하면 keeper 가 정지한다. mitigation: typed
