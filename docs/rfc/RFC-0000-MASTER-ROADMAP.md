...
- **현재 상태:** **[갱신 2026-07-29]** Librarian LLM이 bounded recent-message window에서 fact와 `valid_for_days`를 추출하고 stable `claim_id`로 upsert한다. GC는 그 producer-declared `valid_until`을 적용할 뿐, 별도의 중요도 점수나 전 저장소 재판정을 수행하지 않는다. 모든 keeper fact를 600s마다 한 요청에 넣어 keep/rewrite/forget을 다시 판단하던 periodic full-store consolidation은 maintenance fiber, runtime route, env toggle, prompt/schema와 함께 삭제되었다. 호환 필드나 마이그레이션 경로는 두지 않는다. 옛 dead scaffold `chronicle_librarian.ml`은 **삭제됨**(#25213, HEAD 파일·참조 0) — `auto_recall.ml`·`procedural_memory` 모듈도 **HEAD 부재**(더 강한 형태). `stale_factor`는 dead field가 아니라 **삭제된 field**(`keeper_memory_os_types.mli:117` RFC-0247 purge 목록; 살아있는 `stale_factor`는 `dashboard_cache.ml:161`의 무관한 cache TTL 계수). episode(events.jsonl) retention/GC는 미구현(HEAD) — RFC-0272(Draft) + issue #25065가 추적.
- **candidate work:** compaction/failover Goal 3에 얹지 않는다. Memory-owned 독립 slice에서 bounded Librarian의 extraction/expiry 품질을 Harness-First memory-quality eval(P-1, LLM-as-judge + hand-labeled calibration)로 검증한다. periodic full-store consolidation 또는 count/score/clock 기반 survival gate를 재도입하지 않는다. sleep cycle 재활성(P0a typed category + Ephemeral non-promotable arm), claim_kind clamp, action-stream self-reconciler도 같은 boundary에서만 승격한다.
