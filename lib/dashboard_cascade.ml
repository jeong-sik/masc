...
let raw_config_json () =
  ...
  (match Cascade_toml_materializer.ensure_materialized_json
     ~config_path:source.json_path with
   | Ok () -> ()
   | Error e ->
       Log.Keeper.warn "dashboard_cascade: materialization failed for %s: %s"
         source.json_path e);
  let source_text =
