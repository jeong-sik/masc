...
let handle_keeper_list ctx args : tool_result =
  ...
  | Ok _files ->
    ...
    then (
          ...
          (fun name ->
             ...
             | Ok (Some m) ->
               ...
               let continuity_reflection_hold_s =
                 ...
                 if cooldown <= 0.0
                 then 0.0
                 else if last_reflection_ts <= 0.0
                 then cooldown
                 else (
               ...
               Some
                 (`Assoc
                     ([ "name", `String m.name
                      ...
                      ; ( "keepalive_running"
                        , `Bool (runtime_keepalive_running ctx.config m) )
                      ...
                      ; ( "last_compaction_check_ts"
                        , `Float m.runtime.compaction_rt.last_check_ts )
                      ; ( "last_compaction_decision"
                        , if String.trim m.runtime.compaction_rt.last_decision = ""
                          then `Null
                      ...
                      @ [ ( "continuity_summary"
                        ...
                        ; ( "continuity_compaction_cooldown_sec"
                          , `Int m.compaction.cooldown_sec )
                        ; ( "continuity_reflection_hold_s"
                          , `Float continuity_reflection_hold_s )
                        ; ( "last_continuity_update_ts"
                          , `Float m.runtime.last_continuity_update_ts )
                        ...
                        ; ( "autonomous_action_count"
                          , `Int m.runtime.autonomous_action_count )
                        ...
                        ; ( "storage_paths"
                          , `Assoc
                              ...
                              ; ( "memory_bank"
                                , `String (keeper_memory_bank_path ctx.config m.name) )
                              ; ( "policy"
                                , `String (keeper_policy_log_path ctx.config m.name) )
                              ; ( "feedback"
                                , `String (keeper_feedback_log_path ctx.config m.name) )
                              ; ( "dataset_export"
      ...
      in
      let json =
        `Assoc [ "count", `Int (List.length keepers); "keepers", `List keepers ]
      in
      true, Yojson.Safe.to_string json)
;;
...
let handle_keeper_eval ctx args : tool_result =
  ...
  then false, "invalid keeper name"
    ...
    | Ok (Some (_resolved_name, m)) ->
      ...
      in
      if entries = []
      then true, Printf.sprintf "Keeper %s has no trajectory data to evaluate." m.name
      else (
        let total = List.length entries in
        ...
        in
        let unique_tools = List.sort_uniq String.compare tool_names in
        let tool_counts =
          List.map
            (fun tn ->
        ...
        let scenario_info =
          ...
          | Some sf ->
            (match Eval_harness.load_scenarios_from_file sf with
             ...
             | Ok scenarios ->
               `String
                 (Printf.sprintf "loaded %d scenarios from %s" (List.length scenarios) sf))
        in
        let json =
          `Assoc
            [ "keeper", `String m.name
