...
let test_the_credential_is_named_not_carried () =
  ...
  let endpoint = endpoint_of {|{"kind": "elevenlabs_direct"}|} in
  Alcotest.(check (option string)) "absence is no variable at all" None
    endpoint.Voice_config.api_key_env

let test_an_explicit_invalid_credential_name_is_refused () =
  List.iter
    (fun credential ->
      let body =
        Yojson.Safe.to_string
          (`Assoc [ "kind", `String "elevenlabs_direct"; "api_key_env", credential ])
      in
