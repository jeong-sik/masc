...
let with_config_dir config_dir f =
  ...
  @@ fun () ->
  with_env "MASC_CONFIG_DIR" (Some config_dir)
  @@ fun () ->
  reset ();
...
let test_repo_seed_excludes_claude_from_automatic_profiles () =
  ...
  check
    bool
    "governance_judge is system-only"
    false
    (rendered |> member "governance_judge_keeper_assignable" |> to_bool);
  check
    bool
    "operator_judge is system-only"
    false
...
let test_repo_toml_renders_to_committed_json () =
  ...
  check
    string
    "repo cascade json stays in sync with toml"
...
|}
  ...
  | Ok json_str ->
    ...
    check
      string
      "fallback_cascade key is rendered"
      "big_three"
...
|}
  ...
  | Ok json_str ->
    ...
    check
      bool
      "fallback_cascade key absent when not declared"
      true
...
let test_loader_catalog_exposes_fallback_cascade () =
  with_temp_dir "cascade-fallback-loader"
  @@ fun dir ->
  ...
  match Masc_mcp.Cascade_config_loader.load_catalog ~config_path:json_path with
  ...
  | Ok entries ->
    ...
    (match find_entry "ollama_only" with
     ...
     | Some entry ->
       check
         (option string)
         "ollama_only fallback_cascade hint"
         (Some "big_three")
         entry.fallback_cascade);
    (match find_entry "big_three" with
     | None -> fail "big_three entry missing"
     | Some entry ->
       check
         (option string)
         "big_three has no fallback_cascade"
         None
         entry.fallback_cascade)
;;

let test_keeper_profile_drops_unknown_fallback_target () =
  with_temp_dir "cascade-fallback-unknown"
  @@ fun dir ->
...
let test_keeper_profile_resolves_known_fallback_target () =
  with_temp_dir "cascade-fallback-known"
  @@ fun dir ->
  let config_dir = Filename.concat dir "config" in
  init_config_root config_dir;
  let json_path = Filename.concat config_dir "cascade.json" in
  ...
  check
    (option string)
    "known fallback target is exposed"
    (Some "big_three")
    (Masc_mcp.Keeper_cascade_profile.fallback_cascade_for "ollama_only")
;;
...
let test_runtime_rewrites_drifted_json_from_toml () =
  ...
  check
    string
    "drifted runtime json rewritten from toml"
    expected_json
    (read_file json_path)
;;

let test_invalid_toml_blocks_runtime_without_using_stale_json () =
  with_temp_dir "cascade-toml-invalid"
  @@ fun dir ->
  let config_dir = Filename.concat dir "config" in
  init_config_root config_dir;
  write_file
...
|};
  ...
  | Error rejection ->
    ...
    check
      bool
      "rejection mentions unknown field"
      true
    ...
    check
      ...
      (let n = String.length msg
       ...
       let rec loop i =
         if i + m > n
         then false
         else if String.sub msg i m = sub
         then true
         else loop (i + 1)
       in
       loop 0)
...
let () =
  run
    "cascade_toml_materialization"
    [ ( "repo_sync"
      , [ test_case
            "repo toml renders to committed json"
            `Quick
            test_repo_toml_renders_to_committed_json
        ; test_case
            "repo seed excludes claude from automatic profiles"
            `Quick
            test_repo_seed_excludes_claude_from_automatic_profiles
        ] )
    ; ( "validation"
      , [ test_case
            "unknown profile field is rejected"
            `Quick
            test_unknown_profile_field_is_rejected
        ; test_case
            "fallback_cascade field is parsed"
            `Quick
            test_fallback_cascade_field_is_parsed
        ; test_case
            "fallback_cascade absent is backward compatible"
            `Quick
            test_fallback_cascade_absent_is_backward_compatible
        ; test_case
            "loader catalog exposes fallback_cascade"
            `Quick
            test_loader_catalog_exposes_fallback_cascade
        ; test_case
            "keeper profile drops unknown fallback target"
            `Quick
            test_keeper_profile_drops_unknown_fallback_target
        ; test_case
            "keeper profile resolves known fallback target"
            `Quick
            test_keeper_profile_resolves_known_fallback_target
        ; test_case
            "weight=0 is accepted (#10571 disabled-entry idiom)"
            `Quick
            test_weight_zero_is_accepted
        ; test_case "negative weight is rejected" `Quick test_weight_negative_is_rejected
        ] )
    ; ( "runtime"
      , [ test_case
            "missing json materializes on load"
            `Quick
            test_runtime_materializes_missing_json_on_load
        ; test_case
            "drifted json rewrites from toml"
            `Quick
            test_runtime_rewrites_drifted_json_from_toml
        ; test_case
            "invalid toml blocks runtime without stale json fallback"
            `Quick
            test_invalid_toml_blocks_runtime_without_using_stale_json
        ] )
    ]
;;
