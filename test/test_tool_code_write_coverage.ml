...
let test_https_url () =
  (check (option string))
    "https with .git"
    (Some "jeong-sik")
    (Tool_code_write.extract_github_org "https://github.com/jeong-sik/masc-mcp.git")
;;
...
let test_https_url_no_git () =
  (check (option string))
    "https without .git"
    (Some "jeong-sik")
    (Tool_code_write.extract_github_org "https://github.com/jeong-sik/masc-mcp")
;;
...
let test_ssh_url () =
  (check (option string))
    "ssh URL"
    (Some "jeong-sik")
    (Tool_code_write.extract_github_org "git@github.com:jeong-sik/oas.git")
;;
...
let test_ssh_protocol_url () =
  (check (option string))
    "ssh protocol URL"
    (Some "jeong-sik")
    (Tool_code_write.extract_github_org "ssh://git@github.com/jeong-sik/oas.git")
;;
...
let test_non_github_url () =
  (check (option string))
    ...
    None
    (Tool_code_write.extract_github_org "https://gitlab.com/someone/repo.git")
;;

let test_bare_string () =
  (check (option string))
    "bare string returns None"
    None
...
let test_different_org () =
  (check (option string))
    "different org"
    (Some "kidsnote")
    (Tool_code_write.extract_github_org "https://github.com/kidsnote/backend.git")
;;
...
let test_no_repo_path () =
  (check (option string))
    "URL with no path after org"
    None
    (Tool_code_write.extract_github_org "https://github.com/jeong-sik")
;;
...
let test_domain_spoofing () =
  (check (option string))
    "github.com.evil.com rejected"
    None
    (Tool_code_write.extract_github_org "https://github.com.evil.com/jeong-sik/repo.git")
;;

let test_authority_spoofing () =
  (check (option string))
    "authority via @ rejected"
    None
    (Tool_code_write.extract_github_org "https://jeong-sik@evil.com/repo")
;;
...
let test_uppercase_normalized () =
  (check (option string))
    ...
    (Some "jeong-sik")
    (Tool_code_write.extract_github_org "https://github.com/JEONG-SIK/repo.git")
;;

let test_percent_encoded_org () =
  (check (option string))
    "percent-encoded org rejected"
    None
    (Tool_code_write.extract_github_org "https://github.com/jeong%2Dsik/repo.git")
;;

let test_org_with_dots () =
  (check (option string))
    "org with dots rejected"
    None
    (Tool_code_write.extract_github_org "https://github.com/jeong.sik/repo.git")
;;
...
let make_ctx () : Tool_code_write.context =
  ...
  { Tool_code_write.config = Coord.default_config base_path; agent_name = "test-agent" }
;;
...
let test_disallowed_org_mentions_workspace_path_hint () =
  ...
  match
    Tool_code_write.validate_clone_url
      ~base_path:bp
      "https://github.com/yousleepwhen/masc-mcp.git"
  with
  | Error reason ->
    check
      bool
      "error hints against workspace path inference"
      true
      (msg_contains
...
let test_non_github_rejected () =
  ...
  match
    Tool_code_write.validate_clone_url
      ~base_path:bp
      "https://gitlab.com/jeong-sik/repo.git"
  with
  | Error _ -> ()
...
let test_ssh_allowed () =
  let bp = project_base_path () in
  (check (result unit string))
    "ssh allowed org passes"
    (Ok ())
    (Tool_code_write.validate_clone_url ~base_path:bp "git@github.com:jeong-sik/oas.git")
;;
...
let test_missing_base_path_without_config_fails_closed () =
  ...
  Fun.protect ~finally:Tool_code_write.reset_policy_config_cache (fun () ->
    with_trimmed_env "MASC_CONFIG_DIR" None
    @@ fun () ->
    check
      (option string)
    ...
    match
      Tool_code_write.validate_clone_url
        ...
        "https://github.com/evil-corp/repo.git"
    with
    | Error _ -> ()
...
let test_explicit_config_dir_override_still_validates () =
  ...
  Tool_code_write.reset_policy_config_cache ();
  Fun.protect ~finally:Tool_code_write.reset_policy_config_cache (fun () ->
    with_trimmed_env "MASC_CONFIG_DIR" (Some config_dir)
    @@ fun () ->
    check
      (option string)
...
let test_code_git_push_main_blocked_before_cwd_validation () =
  ...
  let args =
    `Assoc
      [ "action", `String "push"; "args", `List [ `String "origin"; `String "main" ] ]
  in
  let ok, msg = dispatch_exn ctx ~name:"masc_code_git" ~args in
  check bool "push main rejected" false ok;
  check
    bool
    "dangerous op message"
    true
...
let test_code_git_checkout_dot_blocked_before_cwd_validation () =
  let ctx = make_ctx () in
  let args =
    `Assoc [ "action", `String "checkout"; "args", `List [ `String "--"; `String "." ] ]
  in
  let ok, msg = dispatch_exn ctx ~name:"masc_code_git" ~args in
...
let test_validate_code_shell_command_allows_pipe () =
  ...
  check
    (result unit string)
    "piped allowlisted commands accepted"
    (Ok ())
...
let test_validate_code_shell_command_rejects_pipe_to_disallowed () =
  match Tool_code_write.validate_code_shell_command "dune build | xargs rm -rf" with
  | Error _ -> ()
  | Ok () -> fail "expected pipe-to-disallowed-command to be rejected by allowlist"
;;

let test_validate_code_shell_command_allows_direct_build () =
  check
    (result unit string)
    "direct build allowed"
    (Ok ())
    (Tool_code_write.validate_code_shell_command "dune build 2>&1")
;;

let test_validate_code_shell_command_rejects_semicolon () =
  match Tool_code_write.validate_code_shell_command "dune build; tail -5" with
  | Error reason ->
    check
      bool
      "reason mentions shell injection"
      true
...
let fresh_base_path () =
  ...
  let run_git args =
    let cmd =
      String.concat
        " "
        (List.map Filename.quote ("git" :: args) @ [ ">"; "/dev/null"; "2>&1" ])
    in
  ...
  Out_channel.with_open_bin readme (fun oc -> output_string oc "# iter6 test\n");
...
let test_writable_path_allows_own_playground () =
  ...
  in
  (check bool)
    "agent-a writing into agent-a own playground is allowed"
    false
    (is_error result)
;;
...
let test_writable_path_maps_relative_repos_prefix () =
  ...
  in
  let result = Tool_code_write.validate_writable_path ~agent_name:"agent-a" config raw in
  match result with
  | Error e ->
    fail
      ("expected repos/ prefix to map into own playground, got: "
       ^ Types.masc_error_to_string e)
  | Ok resolved ->
    (check string) "repos/ path resolves under own playground repos" expected resolved
;;
...
let test_writable_path_blocks_cross_agent () =
  ...
  in
  (check bool)
    "agent-a writing into agent-b playground is rejected"
    true
    (is_error result);
  (check bool)
    "error mentions own playground prefix"
    true
    (contains "agent-a" (error_msg result));
  (check bool)
    "error flags cross-agent block"
    true
...
let test_clone_cwd_allows_own_repos () =
  ...
  let result = Tool_code_write.validate_clone_cwd ~agent_name:"agent-a" config cwd_a in
  ...
  (check bool)
    "agent-a cloning into own repos is not rejected by containment"
    false
    (contains "Cross-agent" (error_msg result))
;;
...
let test_clone_cwd_blocks_cross_agent_repos () =
  ...
  let result = Tool_code_write.validate_clone_cwd ~agent_name:"agent-a" config cwd_b in
  ...
  (check bool) "agent-a cloning into agent-b repos is rejected" true (is_error result)
;;
  ...
  Alcotest.run
    ...
    ; ( "validate_clone_url"
      , [ test_case "allowed org" `Quick test_allowed_org
        ...
        ; test_case
            "disallowed org hints against workspace path inference"
            `Quick
        ...
        ; test_case
            ...
            `Quick
            test_missing_base_path_without_config_fails_closed
        ; test_case
            "explicit config dir override still validates"
            `Quick
            test_explicit_config_dir_override_still_validates
        ; test_case "mixed-case org" `Quick test_mixed_case_org
        ] )
    ; ( "masc_code_git"
      , [ test_case "clone rejects flag args" `Quick test_code_git_clone_rejects_flag_args
        ; test_case
            "push main blocked before cwd validation"
            `Quick
            test_code_git_push_main_blocked_before_cwd_validation
        ; test_case
            "force push blocked before cwd validation"
            `Quick
            test_code_git_force_push_blocked_before_cwd_validation
        ; test_case
            "checkout dot blocked before cwd validation"
            `Quick
            test_code_git_checkout_dot_blocked_before_cwd_validation
        ] )
    ; ( "validate_code_shell_command"
      , [ test_case
            "allows pipe with allowlisted segments"
            `Quick
            test_validate_code_shell_command_allows_pipe
        ; test_case
            "rejects pipe to disallowed command"
            `Quick
    ...
    ; ( "per_agent_containment_6527_iter6"
      , [ test_case
            "writable_path allows own playground"
            `Quick
            test_writable_path_allows_own_playground
        ; test_case
            "writable_path maps relative repos prefix"
            `Quick
            test_writable_path_maps_relative_repos_prefix
        ; test_case
            "writable_path blocks cross-agent"
            `Quick
            test_writable_path_blocks_cross_agent
        ; test_case
            "clone_cwd does not reject own repos on containment axis"
            `Quick
            test_clone_cwd_allows_own_repos
        ; test_case
            "clone_cwd blocks cross-agent repos"
            `Quick
