(** Persona authoring tools.
    ...
    This module is the writer/schema side of persona-backed keeper profiles.
    The loader in [Keeper_types_profile] remains the read-time source of truth. *)
...
let json_string_list_normalized key json =
  Safe_ops.json_string_list key json |> normalize_name_list
;;
...
let choice_values effects = List.map (fun choice -> choice.value) effects
...
let risk_posture_choice_effects =
  ...
  ; choice_effect
      ~value:"high-autonomy"
      ~effect_text:
        "Biases the draft toward proactive follow-through while concrete fields still require validation."
      ~generated_fields:[ "keeper.instructions"; "keeper.proactive_enabled" ]
      ()
  ]
;;
  ...
  `List
    (List.map
       ...
       [ { name = "alignment"
         ...
         ; effect_text =
             "Generation prompt input only. The saved effect appears through role, trait, goal, and instructions."
         }
       ; { name = "risk_posture"
...
let social_model_choices_json =
  string_list_to_json Keeper_social_model.valid_model_id_strings
;;
...
let field_catalog_entries =
  ...
  ; field_catalog_entry
      ...
      ~required:true
      ~field_effect:
        "Required keeper purpose. masc_keeper_create_from_persona refuses materialization when this is empty."
      ()
  ; field_catalog_entry
...
let schema_json ?(include_examples = false) () =
  ...
  `Assoc
    ([ "personas_root", `String root
     ...
     ; ( "handle_rules"
       , `String
           "Non-empty [A-Za-z0-9._-]+. The handle is the directory name and the default keeper name."
       )
...
let normalize_keeper_json ~handle keeper_json =
  ...
  | `Assoc _ ->
    ...
    then
      Error
        (Printf.sprintf
           "keeper.%s %s removed. Assign keeper runtime in runtime.toml [[runtime.assignments]] keyed by keeper name."
...
let normalize_profile ~handle profile =
  ...
  else if json_has_operator_todo_placeholder profile
  then
    Error
...
let save_persona ?(overwrite = false) ?(dry_run = false) ~handle profile =
  Result.bind (normalize_profile ~handle profile) (fun normalized ->
    ...
    then
      Ok { handle; personas_root = root; profile_path; profile = normalized; warnings = [] }
    else
...
let handle_persona_save_no_ctx args =
  ...
  | Some profile ->
    (match save_persona ~overwrite ~dry_run ~handle profile with
     ...
     | Ok result ->
       tool_result_ok (Yojson.Safe.to_string (save_result_to_json ~dry_run result)))
;;
  ...
  Result.bind
    ...
    (fun alignment ->
      Result.map
        ...
        (optional_choice_arg ~field:"risk_posture" ~choices:risk_posture_choices args))
;;
