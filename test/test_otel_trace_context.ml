...
let test_parse_valid () =
  match Otel_trace_context.parse valid_traceparent with
  | Some ctx ->
...
let test_parse_not_sampled () =
  ...
  match Otel_trace_context.parse tp with
  | Some ctx ->
...
let test_parse_truncated () =
  match Otel_trace_context.parse "00-4bf92f35" with
  | None -> ()
  | Some _ -> fail "expected None for truncated input"

let test_parse_empty () =
  match Otel_trace_context.parse "" with
  | None -> ()
  | Some _ -> fail "expected None for empty input"

let test_parse_garbage () =
  match Otel_trace_context.parse "not-a-traceparent-at-all" with
  | None -> ()
...
let test_to_traceparent_roundtrip () =
  ...
  | Some ctx ->
    let regenerated =
      Otel_trace_context.to_traceparent
...
let test_propagate_keeps_trace_id () =
  match Otel_trace_context.parse valid_traceparent with
  | Some ctx ->
    let child = Otel_trace_context.propagate ctx in
...
let test_of_json_present () =
  ...
  match Otel_trace_context.of_json json with
  | Some ctx ->
...
let test_of_json_not_object () =
  match Otel_trace_context.of_json (`String "not an object") with
  | None -> ()
...
let test_inject_json_some () =
  ...
  let result =
    Otel_trace_context.inject_json fields (Some valid_traceparent)
  in
...
let test_inject_json_none () =
  let fields = [("type", `String "masc/broadcast")] in
  let result = Otel_trace_context.inject_json fields None in
...
let test_to_header () =
  match Otel_trace_context.parse valid_traceparent with
  | Some ctx ->
    let (name, value) = Otel_trace_context.to_header ctx in
    check string "header name" "traceparent" name;
    check string "header value" valid_traceparent value
  | None -> fail "parse failed"

let test_header_name () =
  check string "W3C spec lowercase" "traceparent" Otel_trace_context.header_name
