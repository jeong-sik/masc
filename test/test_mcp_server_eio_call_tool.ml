...
let first_issue quality = quality |> U.member "issues" |> U.to_list |> List.hd
...
let make_keeper_meta ?agent_name ?current_task_id ?(goal_ids = []) name =
  let agent_name =
    Option.value agent_name ~default:(Masc_mcp.Keeper_types.keeper_agent_name name)
  in
  let fields =
    [ "name", `String name
    ...
    | [] -> []
    | ids ->
      [ "active_goal_ids", `List (List.map (fun goal_id -> `String goal_id) goal_ids) ]
  in
...
let extract_json_from_text text =
  ...
  with
  | Not_found -> failf "expected JSON payload in text: %s" text
;;
...
let test_contains_casefold_keeps_semantics () =
  ...
  check bool "ascii casefold" true (contains "Auth REQUIRED by server" "auth required");
  check
    bool
    "middle substring"
    true
    (contains "prefix temporary network suffix" "TEMPORARY NETWORK");
  check bool "numeric literal" true (contains "HTTP 503 Service Unavailable" "503");
...
let test_transition_has_no_fixed_timeout () =
  check
    bool
    "masc_transition has no fixed timeout"
    true
...
let test_persona_generate_timeout_exceeds_oas_budget () =
  ...
  | Some timeout_sec ->
    check
      bool
      "persona generate timeout exceeds internal OAS budget"
      true
      (timeout_sec > 120.)
...
let test_regular_tool_uses_default_timeout () =
  ...
  with
  | Some timeout_sec ->
    check bool "default timeout remains enabled" true (timeout_sec >= 5.)
...
let test_runtime_mcp_keeper_log_context_uses_keeper_trace_and_current_turn () =
  ...
  Fun.protect
    ...
    (fun () ->
       ignore (Masc_mcp.Keeper_registry.register_offline ~base_path keeper_name meta);
       ...
       in
       check (option string) "trace_id" (Some ("trace-test-" ^ keeper_name)) ctx.trace_id;
       check
         (option string)
         "agent_name"
         (Some (Masc_mcp.Keeper_types.keeper_agent_name keeper_name))
         ctx.agent_name;
       check (option string) "session_id" (Some "session-explicit") ctx.session_id;
       ...
       check (option (list string)) "goal_ids" (Some [ "goal-1"; "goal-2" ]) ctx.goal_ids;
       ...
       check
         bool
         "shared memory scope present"
         true
         (Option.is_some ctx.shared_memory_scope);
       check
         bool
         "tool surface class present"
         true
         (Option.is_some ctx.tool_surface_class);
       check
         bool
         "visible tool count present"
         true
         (Option.is_some ctx.visible_tool_count);
       check
         (option (list string))
         "runtime mcp required tools empty"
         (Some [])
         ctx.required_tools;
       check
         (option (list string))
         "runtime mcp missing required tools empty"
         (Some [])
...
let test_record_runtime_mcp_keeper_tool_trace_logs_and_broadcasts () =
  ...
  let meta =
    make_keeper_meta ~current_task_id:"task-456" ~goal_ids:[ "goal-runtime" ] keeper_name
  in
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Masc_mcp.Mcp_server_eio_call_tool.record_runtime_mcp_keeper_tool_trace
         ...
         ~arguments:
           (`Assoc [ "cmd", `String "false"; "session_id", `String "session-explicit" ])
         ~message:"command exited 1"
         ~success:false
         ~duration_ms:87;
       let rows = Masc_mcp.Keeper_tool_call_log.read_recent ~keeper_name ~n:1 () in
       ...
       check string "output" "command exited 1" (row |> U.member "output" |> U.to_string);
       check string "lane" "runtime_mcp" (row |> U.member "lane" |> U.to_string);
       check
         string
         "trace id"
         ("trace-test-" ^ keeper_name)
       ...
       check int "keeper turn id" 1 (row |> U.member "keeper_turn_id" |> U.to_int);
       check string "task id" "task-456" (row |> U.member "task_id" |> U.to_string);
       check int "goal id count" 1 (row |> U.member "goal_ids" |> U.to_list |> List.length);
       ...
       check
         bool
         "runtime contract has generation"
         true
       ...
       check
         string
         "runtime contract sandbox profile"
         (row |> U.member "sandbox_profile" |> U.to_string)
         (runtime_contract |> U.member "sandbox_profile" |> U.to_string);
       check
         bool
         "runtime contract allowed paths present"
         true
         (runtime_contract |> U.member "allowed_paths" |> U.to_list |> List.length > 0);
       check
         bool
         "runtime contract visible tool count present"
         true
       ...
       check
         int
         "runtime contract required tools empty"
         0
         (runtime_contract |> U.member "required_tools" |> U.to_list |> List.length);
       check
         int
         "runtime contract missing required tools empty"
         0
         (runtime_contract
          |> U.member "missing_required_tools"
          |> U.to_list
          |> List.length);
       check
         string
         "runtime contract cascade profile"
         meta.cascade_name
         (runtime_contract |> U.member "cascade_profile" |> U.to_string);
       let masc_root = Filename.concat base_path Common.masc_dirname in
       ...
       check int "trajectory turn" 1 trajectory_entry.Masc_mcp.Trajectory.turn;
       check int "trajectory round" 1 trajectory_entry.Masc_mcp.Trajectory.round;
       let trajectory_json =
         let path = Masc_mcp.Trajectory.trajectory_path masc_root keeper_name trace_id in
       ...
       check
         string
         "trajectory runtime keeper"
         keeper_name
         (trajectory_json
          |> U.member "runtime_contract"
          |> U.member "keeper_name"
          |> U.to_string);
       check
         string
         "trajectory runtime agent"
         (Masc_mcp.Keeper_types.keeper_agent_name keeper_name)
         (trajectory_json
          |> U.member "runtime_contract"
          |> U.member "agent_name"
       ...
       check
         string
         "trajectory runtime cascade profile"
         meta.cascade_name
         (trajectory_json
          |> U.member "runtime_contract"
          |> U.member "cascade_profile"
          |> U.to_string);
       check
         string
         "trajectory action tool"
         "keeper_bash"
         (trajectory_json
          |> U.member "action_radius"
          |> U.member "tool_name"
          |> U.to_string);
       let sse_payload =
       ...
       check bool "sse success" false (sse_payload |> U.member "success" |> U.to_bool);
       check
         string
         "sse error text"
         "command exited 1"
  ...
  run
    ...
    [ ( "quality"
      , [ test_case "timeout is error" `Quick test_timeout_quality_is_error
        ; test_case
            "generic failure is error"
            `Quick
            test_generic_failure_quality_is_error
        ; test_case "success has no issues" `Quick test_success_quality_has_no_issues
        ; test_case
            "contains casefold keeps semantics"
            `Quick
            test_contains_casefold_keeps_semantics
        ; test_case
            "transition has no fixed timeout"
            `Quick
            test_transition_has_no_fixed_timeout
        ; test_case
            "persona generate timeout exceeds OAS budget"
            `Quick
            test_persona_generate_timeout_exceeds_oas_budget
        ; test_case
            "regular tool keeps default timeout"
            `Quick
            test_regular_tool_uses_default_timeout
        ; test_case
            "runtime MCP log context uses keeper trace/current turn"
            `Quick
            test_runtime_mcp_keeper_log_context_uses_keeper_trace_and_current_turn
        ; test_case
            "runtime MCP trace logs and broadcasts"
            `Quick
