    ...
    Fun.protect
      ...
      (fun () ->
         ...
         let rec loop idx =
           ...
           remaining >= plen && (String.sub content idx plen = pattern || loop (idx + 1))
         in
...
let test_activity_log_file_ends_with_log () =
  ...
  check
    ...
    "ends with .log"
    true
    (String.length path > 4 && String.sub path (String.length path - 4) 4 = ".log")
;;
...
let test_build_response_prompt_nonempty () =
  let prompt =
    Auto_responder.build_response_prompt
      ~from_agent:"alice"
      ~content:"hello"
      ~mention:"bob"
  in
...
let test_build_response_prompt_contains_from () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "alice") prompt 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_build_response_prompt_contains_content () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "unique_message_xyz") prompt 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_build_response_prompt_contains_mention () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "bob") prompt 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_build_response_prompt_contains_join () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "masc_join") prompt 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_build_response_prompt_contains_broadcast () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "masc_broadcast") prompt 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_build_response_prompt_contains_leave () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "masc_leave") prompt 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_extract_nickname_returns_option () =
  ...
  check (option string) "returns parsed nickname" (Some "claude-rare-beaver") nick
;;
...
let test_auto_responder_uses_shared_model_runtime () =
  check
    bool
    "uses Oas_worker.run_named"
    true
    (file_contains_pattern "lib/auto_responder.ml" {|Oas_worker.run_named ~cascade_name|});
  check
    bool
    "no direct run_prompt_cascade"
    false
    (file_contains_pattern "lib/auto_responder.ml" "Llm_orchestration.run_prompt_cascade");
  check
    bool
    "legacy Llm_direct dispatch removed"
    false
  ...
  run
    ...
    ; ( "build_response_prompt"
      , [ test_case "nonempty" `Quick test_build_response_prompt_nonempty
        ...
        ; test_case
            "contains broadcast"
            `Quick
            test_build_response_prompt_contains_broadcast
    ...
    ; ( "re-exports"
      , [ test_case "spawnable_agents nonempty" `Quick test_spawnable_agents_nonempty
        ; test_case
            "spawnable_agents contains claude"
            `Quick
            test_spawnable_agents_contains_claude
        ; test_case
            "agent_type_of_mention claude"
            `Quick
            test_agent_type_of_mention_claude
        ; test_case
            "agent_type_of_mention gemini"
            `Quick
            test_agent_type_of_mention_gemini
    ...
    ; "is_enabled", [ test_case "returns bool" `Quick test_is_enabled_type ]
    ; "get_mode", [ test_case "returns valid" `Quick test_get_mode_returns_valid ]
    ; ( "edge_cases"
    ...
    ; ( "source"
      , [ test_case
            ...
            `Quick
            test_auto_responder_uses_shared_model_runtime
        ] )
    ]
;;
