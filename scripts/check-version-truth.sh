...
if grep -Fq '(generate_opam_files true)' dune-project; then
  [[ -n "$opam_file" ]] || fail "missing checked-in opam file"
  opam_version="$(sed -n 's/^version: "\([^"]*\)"/\1/p' "$opam_file" | head -n1)"
  if grep -Fq '# This file is generated by dune' "$opam_file" \
...
[[ -n "$opam_file" ]] || fail "missing checked-in opam file"
opam_version="${opam_version:-$(sed -n 's/^version: "\([^"]*\)"/\1/p' "$opam_file" | head -n1)}"

[[ -n "$package_version" ]] || fail "missing package version in dune-project"
[[ -n "$opam_version" ]] || fail "missing version in $opam_file"
...
[[ "$package_version" == "$opam_version" ]] || fail "dune-project ($package_version) != $opam_file ($opam_version)"
