...
let with_clean_env f =
  let saved =
    try Some (Sys.getenv "MASC_BASE_PATH") with
    | Not_found -> None
  in
...
let counter_value labels =
  Prometheus.metric_value_or_zero Prometheus.metric_keeper_path_rejection ~labels ()
;;

let assert_no_roots_leak msg err =
  check
    bool
    (msg ^ ": no '(roots=' substring")
    false
  ...
  check
    bool
    (msg ^ ": no sandbox roots substring")
    false
    (Astring.String.is_infix ~affix:"sandbox roots:" err);
  check
    bool
    (msg ^ ": no resolved path hint")
    false
...
let assert_legacy_not_found_prefix msg err =
  check
    bool
    (msg ^ ": legacy prefix preserved")
    true
    (Astring.String.is_prefix ~affix:"path_not_found_under_allowed_roots:" err)
;;
    ...
    (fun () ->
       ...
       let result =
         Keeper_alerting_path.resolve_keeper_read_path
           ~config
           ~allowed_paths:[]
           ~raw_path:"nonexistent_repo_10349/"
       in
       check bool "rejection occurred" true (Result.is_error result);
       let err = Result.get_error result in
    ...
    (fun () ->
       ...
       let _ =
         Keeper_alerting_path.resolve_keeper_read_path
           ~config
           ~allowed_paths:[]
           ~raw_path:"definitely_missing_10349/"
       in
       check
         (float 0.0001)
...
let () =
  run
    "keeper_path_roots_leak_10349"
    [ ( "no-roots-leak"
      , [ test_case
            "path_outside_sandbox error is opaque"
            `Quick
            test_path_outside_sandbox_no_leak
        ; test_case "out_of_roots error is opaque" `Quick test_out_of_roots_no_leak
        ; test_case
            "not_found_relative error is opaque"
            `Quick
            test_not_found_relative_no_leak
        ] )
    ; ( "counter-labels"
      , [ test_case
            "kinds tick independent label series"
            `Quick
