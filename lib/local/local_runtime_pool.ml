...
let debug_log fmt =
  if debug_enabled ()
  then Printf.ksprintf (fun msg -> Log.LocalWorker.debug "%s" msg) fmt
  else Printf.ksprintf (fun _ -> ()) fmt
;;

let empty_pool =
  { runtimes = []; fingerprint = ""; parse_errors = []; measured_ceiling = None }
;;
...
let parse_int_opt raw = int_of_string_opt (String.trim raw)
...
let runtime_port base_url =
  try Uri.of_string base_url |> Uri.port with
  | Failure _ -> None
;;
...
let max_concurrency_of_discovery_status (status : Discovery_cache.endpoint_info) =
  ...
  | _ -> int_of_env_default "LLAMA_SERVER_PARALLEL_HINT" ~default:default_parallel_hint
;;
...
let safe_discovery_statuses () =
  try Discovery_cache.get_cached_or_refresh () with
...
let current_fingerprint () =
  String.concat
    ...
    ; Option.value ~default:"" (Env_config.Worker.local_runtime_cooldown_sec_opt ())
    ; string_of_int
        (int_of_env_default "LLAMA_SERVER_PARALLEL_HINT" ~default:default_parallel_hint)
    ]
;;
...
let load_runtimes_from_env () =
  let discovered = safe_discovery_statuses () |> List.map runtime_of_discovery_status in
...
let ensure_loaded () =
  ...
  then (
    ...
    if String.equal loaded_fingerprint fingerprint
    then (
      let refreshed = List.map refresh_runtime_metrics loaded in
      let reloaded =
        with_pool_lock (fun () ->
          let state = !pool in
          if not (String.equal fingerprint state.fingerprint)
          then (
            pool
      ...
      then
        debug_log
          "reload runtimes count=%d errors=%d"
          (List.length loaded)
          (List.length errors))
    else
      ...
      debug_log
        "env drift during reload (captured=%s, post-load=%s); skipping install"
        fingerprint
        loaded_fingerprint)
  else
    with_pool_lock (fun () ->
...
let allocated_slots () =
  snapshots ()
  |> List.fold_left (fun acc (runtime : runtime_snapshot) -> acc + runtime.active_slots) 0
;;
  ...
  | None ->
    (match runtime.model with
     ...
     | None ->
       Error
         (sprintf
            "no explicit model provided for local runtime %s; set spawn_model or \
             runtime.model"
            runtime.id))
;;
...
let select_runtime_from (runtimes : runtime list) ?preferred_pool ?model_name ()
  ...
  | runtimes ->
           ...
           (match generic_model_matches with
            ...
            | [] ->
              let scope =
                ...
                | None -> ""
              in
              Error (sprintf "no local runtime configured for model %s%s" requested scope)))
    in
    ...
    let sorted =
      List.sort (fun a b -> compare (runtime_sort_key a) (runtime_sort_key b)) candidates
    in
    (match sorted with
     ...
     | [] ->
       (match requested_model with
        | Some requested ->
          Error (sprintf "no local runtime configured for model %s" requested)
...
let acquire ?preferred_pool ~model_name () =
  ...
  with_pool_lock (fun () ->
    ...
    let new_runtimes : runtime list =
      List.map
        (fun (r : runtime) -> if String.equal r.id runtime.id then updated else r)
...
let release (lease : lease) ~success ?error ?latency_ms () =
    ...
    with_pool_lock (fun () ->
      ...
      | Some runtime ->
        ...
        let failure_streak, cooldown_until, last_error, total_success, total_failure =
          if success
          then 0, None, None, runtime.total_success + 1, runtime.total_failure
          else (
            let streak = runtime.failure_streak + 1 in
            let cooldown =
              if streak >= 3
              then Some (wall_now () +. cooldown_seconds ())
  ...
  | Some (runtime_id, before_failure_streak, updated) ->
    debug_log
      "release runtime=%s success=%b before_streak=%d after_streak=%d cooldown=%s \
       total_success=%d total_failure=%d error=%s"
      runtime_id
      success
      before_failure_streak
      updated.failure_streak
      (match updated.cooldown_until with
       | Some value -> Printf.sprintf "%.3f" value
       | None -> "null")
      updated.total_success
      updated.total_failure
