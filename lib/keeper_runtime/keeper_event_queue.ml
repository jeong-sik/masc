...
let is_board_signal = function
  ...
  | Hitl_resolved _
  | Goal_verification_failed _ -> false
...
let payload_to_yojson = function
  ...
  | Connector_attention ca ->
    `Assoc [ "kind", `String "connector_attention"; "event_id", `String ca.event_id ]
  | Hitl_resolved r ->
    `Assoc
...
let queue_of_yojson json =
  ...
  then Error (Printf.sprintf "unsupported keeper event queue schema: %s" schema_value)
    ...
    let* items =
      list_of_json
        ~context:"keeper event queue snapshot.items"
        stimulus_of_yojson
        items_json
    in
    Ok (of_list items))
