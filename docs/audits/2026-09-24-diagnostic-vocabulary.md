...
- Dashboard and TUI renderers that print a status next to a code with the same
  status word.
- Keeper Channels detail repeats a Connector read failure before the loader's
  cause; its transport, decode, and async error paths need one attribution
  boundary before the display wording changes.
