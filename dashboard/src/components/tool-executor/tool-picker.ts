    ...
    <div class="flex flex-col gap-2 h-full" role="region" aria-label="도구 선택">
      ...
      <div class="flex gap-1">
        ${TIER_OPTIONS.map(opt => html`
          <button type="button" class="text-2xs px-2 py-1 rounded transition-colors cursor-pointer
        ...
        `)}
        <span class="text-2xs text-[var(--text-muted)] ml-auto self-center">${tools.length}개</span>
      </div>
