...
let ensure_dir ~base_dir ~agent_name =
  let dir = Filename.concat (Filename.concat base_dir "memory") agent_name in
...
let encode_line ~key ~(value : Yojson.Safe.t option) : string =
  ...
  let value_json =
    ...
    | Some v ->
      ...
      if String.length s > max_value_size
      then (
        Log.Memory.warn
          "memory_jsonl: value for key=%s exceeds 1MB (%d bytes), truncating"
          key
          (String.length s);
  ...
  in
  let obj = `Assoc [ "key", `String key; "value", value_json; "ts", `Float ts ] in
...
let read_lines ~path : (string * Yojson.Safe.t option * float) list =
  ...
  then []
    ...
    (try
       ...
       then Log.Memory.warn "memory_jsonl: file %s is %d bytes (>50MB)" path size
     with
     | Unix.Unix_error _ -> ());
...
let make_backend ~base_dir ~agent_name ~session_id : Oas.Memory.long_term_backend =
  ...
  let persist ~key json =
    ...
    | exn ->
      let msg =
        Printf.sprintf "memory_jsonl persist(%s) failed: %s" key (Printexc.to_string exn)
      in
  ...
  let retrieve ~key =
    try
      ...
      let last_value =
        List.fold_left
          (fun acc (k, v, _ts) -> if k = key then Some v else acc)
          None
          entries
      in
    ...
    | Eio.Cancel.Cancelled _ as e -> raise e
    | exn ->
      Log.Memory.error "memory_jsonl retrieve(%s) failed: %s" key (Printexc.to_string exn);
      None
  in
  let remove ~key =
    try
    ...
    | exn ->
      let msg =
        Printf.sprintf "memory_jsonl remove(%s) failed: %s" key (Printexc.to_string exn)
      in
      Log.Memory.error "%s" msg;
      Error msg
  in
  let batch_persist pairs =
    ...
    | errs ->
      ...
      let msg =
        Printf.sprintf
          "memory_jsonl batch_persist: %d/%d failed: %s"
          (List.length errs)
          (List.length pairs)
          first_err
      in
  ...
  let query ~prefix ~limit =
    try
      ...
      List.iter
        (fun (key, value, ts) ->
           ...
           then Hashtbl.replace tbl key (value, ts))
        entries;
      (* Collect non-tombstone entries *)
      let results =
        Hashtbl.fold
          (fun key (value, ts) acc ->
             match value with
             | Some v -> (key, v, ts) :: acc
             | None -> acc (* tombstone *))
          tbl
      ...
      let sorted = List.sort (fun (_, _, t1) (_, _, t2) -> Float.compare t2 t1) results in
    ...
    | Eio.Cancel.Cancelled _ as e -> raise e
    | exn ->
      Log.Memory.error "memory_jsonl query(%s) failed: %s" prefix (Printexc.to_string exn);
      []
  in
