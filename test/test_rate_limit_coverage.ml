...
let test_default_rate () = check (float 0.001) "default rate" 60.0 Rate_limit.default_rate
let test_default_burst () = check int "default burst" 150 Rate_limit.default_burst
...
let test_headers_has_limit () =
  ...
  check bool "has X-RateLimit-Limit" true (List.mem_assoc "X-RateLimit-Limit" hdrs)
;;
...
let test_too_many_requests_body () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "Too Many Requests") body 0 in
       true
     with
     | Not_found -> false)
;;
