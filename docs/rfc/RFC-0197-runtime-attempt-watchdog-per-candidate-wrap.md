...
runtime=runtime.glm-coding-with-spark 
max_context=128000 context_budget=128000 
latency=579373ms 
error=Timeout: Turn wall-clock budget exhausted during runtime attempt 
       (budget=555.0s, watchdog=570.0s)
```

**Frequency**: 32 events / 2h (~16/h, ~384/day extrapolation)
**Single keeper**: 100% analyst
**Single runtime runtime**: `runtime.glm-coding-with-spark` (4 tier failover chain)
**Single budget value**: 99% `budget=555.0s, watchdog=570.0s` — *동일 runtime tier 만 fire*

`runtime.glm-coding-with-spark` 의 runtime config:
```toml
...
`Eio.Time.with_timeout_exn clock attempt_watchdog_s run` (`keeper_unified_turn_attempt_watchdog.ml:8`) 발화 시 `Agent_sdk.Error.Api (Timeout)` Error 반환 → runtime chain 자체가 cancel.

**Semantic mismatch**: 변수 / log / RFC-0192 invariant 모두 *per-attempt* 명시. 그러나 wrap target = `run_turn` (= 전체 runtime chain 4 tier failover 포함).
...
이 layer 의 *intent* = fallback. 발화 시 Error 반환 → runtime `run` loop 가 next candidate 시도.

**그러나 Enforce mode 에서 비활성**:
```ocaml
(* lib/runtime/runtime_attempt_liveness_config.ml:236 *)
...
The retired RFC-0022 mode defaulted to `Enforce`. In that historical build,
`outer_wall_for_provider = None` led to `keeper_turn_driver_try_provider.ml:516-517`:
```ocaml
...
### Layer 3 — Runtime attempt liveness observer (RFC-0022)

`Runtime_attempt_liveness` observer 가 streaming chunks 기반으로 attempt liveness 판정 → 위반 시 `Liveness_kill` raise via `Eio.Switch.fail`.
...
Layer 1 (570s, whole turn) > Layer 2 (disabled in Enforce) > Layer 3 (observer, not firing). **항상 Layer 1 이 fire** → runtime fallback path 0.
...
2. `lib/keeper/keeper_turn_driver.ctx` (이미 RFC-0192 PR-1 에서 deadline field 추가) — try_runtime ctx 로 deadline propagation 보장.
...
- `test_runtime_fallback_on_provider_timeout.ml`: provider 가 hang 시 next candidate 시도 검증
- `test_runtime_deadline_shared_across_candidates.ml`: 첫 candidate 가 deadline 다 쓰면 next candidate 즉시 exhausted

### Phase D — Fleet validation (7d window)

- Baseline: 32 events / 2h (단일 candidate terminate)
- Target: `runtime-fallback` log message 가 LLM 한테 도달, next candidate 시도 사례 fleet 측정
...
| Anti-pattern | Check |
|---|---|
| §1 Counter-as-fix | ✓ 새 counter 0, 기존 `runtime-fallback` log 활용 |
...
| 사용자 메모 `[[feedback-runtime-budget-no-hard-gates]]` | ✓ 정합 — wrap location 만 이동, budget 계산은 그대로 |
| 사용자 메모 `[[feedback-surgical-workaround-rejected-for-tool-surface]]` | ✓ tool surface 가 아닌 runtime scheduling 영역. RFC-driven |
...
- Provider hang 자체 진단 (`runtime.glm-coding-with-spark` 어느 tier 가 hang) — 별도 incident, Issue #18894 영역
...
- RFC-0022 (retired runtime attempt liveness mode)
