...
let check_kind ~msg expected payload =
  check string msg expected (payload_kind (KET.classify_tool_result_payload payload))
;;
...
let json_contains_tool name = function
  | `Assoc fields -> List.exists (fun (_, value) -> json_list_contains name value) fields
  | _ -> false
;;
...
let test_malformed_json_like_payload_detected () =
  ...
  | KET.Malformed_structured detail ->
    check bool "detail mentions JSON parse error" true (String.length detail > 0)
  | other ->
    fail (Printf.sprintf "expected malformed_structured, got %s" (payload_kind other))
;;
...
let test_keeper_tools_list_json_uses_typed_groups () =
  ...
  let member group name = json_list_contains name Yojson.Safe.Util.(member group json) in
  check bool "board canonical tool grouped" true (member "board" "keeper_board_post");
  check
    bool
    "fake board-looking tool excluded"
    false
    (json_contains_tool "keeper_board_fake" json);
  check bool "voice tool grouped" true (member "voice" "keeper_voice_speak");
  check
    bool
    "task tool grouped as coordination"
    true
    (member "coordination" "keeper_task_claim");
  check bool "shell tool grouped" true (member "shell" "keeper_shell");
  check bool "fs tool grouped" true (member "fs" "keeper_fs_read");
  check bool "memory tool grouped" true (member "memory" "keeper_memory_search")
;;

let test_execute_with_outcome_missing_file_is_failure () =
  with_exec_fixture "keeper_exec_tools_missing_file" (fun ~config ~meta ~ctx_work ->
    let result =
      KET.execute_keeper_tool_call_with_outcome
        ~config
        ~meta
    ...
    check
      string
      "missing file payload shape"
      "structured_error"
      (payload_kind result.payload_shape))
;;

let test_execute_with_outcome_bad_query_is_failure () =
  with_exec_fixture "keeper_exec_tools_bad_query" (fun ~config ~meta ~ctx_work ->
    let result =
      KET.execute_keeper_tool_call_with_outcome
        ~config
        ~meta
    ...
    ~handler:(fun ~name ~args:_ ->
      Some
        ...
        , Yojson.Safe.to_string
            (`Assoc
                [ "ok", `Bool true; "tool", `String name; "route", `String "registered" ])
        ))
;;
...
let test_registered_tool_dispatch_without_masc_prefix () =
  ...
  check
    bool
    "probe has no masc_ prefix"
    false
    (String.starts_with ~prefix:"masc_" registered_dispatch_probe_tool);
  with_exec_fixture "keeper_exec_registered_dispatch" (fun ~config ~meta ~ctx_work:_ ->
    match
    ...
    | Some raw ->
      ...
      check
        ...
        registered_dispatch_probe_tool
...
let test_exec_cache_miss_then_hit () =
  with_exec_fixture "keeper_exec_cache_hit" (fun ~config ~meta ~ctx_work ->
...
let test_exec_cache_none_no_caching () =
  with_exec_fixture "keeper_exec_cache_none" (fun ~config ~meta ~ctx_work ->
...
let test_exec_cache_stats_json () =
  ...
  check int "initial hit_count" 0 Yojson.Safe.Util.(member "hit_count" json |> to_int);
  check int "initial miss_count" 0 Yojson.Safe.Util.(member "miss_count" json |> to_int);
  check int "initial entry_count" 0 Yojson.Safe.Util.(member "entry_count" json |> to_int);
  (* Store an entry and check *)
  Masc_exec.Exec_cache.store
    cache
    ~cmd:"test_cmd"
    ~exit_code:0
    ~output:"test output"
