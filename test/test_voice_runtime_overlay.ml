...
let test_resolve_voice_aliases () =
  ...
  check string "chat completions v1 compat alias" "voice-chat-completions-v1-compat" openai_compat.canonical_name
;;
...
let test_voice_auth_env_resolution () =
  ...
  let openai_compat = Option.get (Voice.resolve_adapter "voice-chat-completions-v1-compat") in
  check
    (option string)
...
let test_stt_request_openai_compat () =
  let endpoint : Masc_mcp.Voice_config.endpoint =
    { id = "test-chat-completions-v1-stt"
    ; kind = Openai_compat
    ; base_url = Some "https://chat-completions-v1.example/v1"
    ; mcp_url = None
    ; health_url = None
  ...
  with
  | Ok req ->
    check string "url" "https://chat-completions-v1.example/v1/audio/transcriptions" req.url;
    check
      bool
  ...
  run
    ...
    ; ( "stt"
      , [ test_case
        ...
        ; test_case "stt request chat completions v1 compat" `Quick test_stt_request_openai_compat
