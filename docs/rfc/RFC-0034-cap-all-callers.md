...
#13981이 `Workspace_task_create.add_task`에 `?reject_if` optional 파라미터를 추가하고 `keeper_task_create` MCP tool 핸들러에 cap rejection을 wire했다. 이로써 keeper가 `keeper_task_create`로 만드는 task에는 cap이 적용된다.

그러나 [iter-2~5 코드 추적](../iter-2.md) 결과: `Workspace_task.add_task`의 **다른 4 caller가 여전히 `?reject_if`를 미전달**, 즉 동일 cap을 우회한다.
...
- `Workspace_task_create.add_task` 시그니처 변경 — 별도 RFC (`?reject_if` non-optional화).
...
**핵심**: 4 caller에 `~reject_if:(Workspace_task_capacity.rejection_for_add_task ?goal_id)` 명시적 추가.

해당 rejection helper는 #13981에서 keeper task tool runtime helper로 들어왔고, RFC-0034.v2에서 `Workspace_task_capacity`로 이동한다. 시그니처:
```ocaml
val rejection_for_add_task :
  ?goal_id:string -> Masc_domain.backlog -> string option
```

문제: helper가 keeper runtime에 남아 있으면 `lib/workspace/`에서 호출할 때 dependency 위반이 된다. 또한 `tool_task.ml`, `task_dispatch.ml`, `operator_control.ml`은 keeper layer 외부다.

**해결**: keeper task runtime helper를 `Workspace_task_capacity` 모듈로 *이전*하고 keeper runtime은 `Workspace_task_capacity`를 호출한다.

### 3.1 신규 모듈 `lib/workspace/workspace_task_capacity.{ml,mli}` (#13981 코드 이전)
...
- 호출 사이트(line 322-340의 `goal_task_capacity_error`, `task_create_capacity_error_json`, `task_create_goal_capacity_rejection`)를 `Workspace_task_capacity` 호출로 치환.
- 결과: keeper layer가 workspace layer 호출 (의존성 정상 방향).

### 3.3 4 caller에 cap 적용

각 caller에 `~reject_if:(Workspace_task_capacity.rejection_for_add_task ?goal_id)` 추가.
...
Workspace.add_task config ?goal_id
  ~reject_if:(Workspace_task_capacity.rejection_for_add_task ?goal_id)
...
Workspace.add_task ctx.config ?goal_id
  ~reject_if:(Workspace_task_capacity.rejection_for_add_task ?goal_id)
  ~title ~priority ~description
```
...
`tool_task.ml:485`의 `handle_add_task`는 이미 args에서 `goal_id` 추출 → `Workspace.add_task ?goal_id` 전달함. 그러나 *capacity check가 `goal_id`만 보고 cap 적용*하니, `goal_id=None`이면 자연 우회 — 정상.
...
- `keeper/agent_tool_task_runtime.ml` 1 caller은 `Workspace_task_capacity` 모듈 호출로 변경.
...
- **모듈 이전 의존성**: task runtime ↔ `Workspace_task_capacity` 의존성 변경. dune 파일 의존성 그래프 재확인 필요.

## 7. Implementation Plan

| 단계 | 산출물 | LOC 추정 |
|---|---|---|
| S1 | `lib/workspace/workspace_task_capacity.{ml,mli}` 신설 (#13981 코드 이전) | ~110 |
...
- TLA+ spec: `~/me/.tmp/loop-masc-fsm/spec/GoalCapEntrypoint.tla`
- 모듈 sketch: `~/me/.tmp/loop-masc-fsm/spec/Workspace_task_capacity.{ml,mli}`
