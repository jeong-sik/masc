...
let test_per_keeper_average () =
  ...
  (* 54000 / 9 = 6000 *)
  check (float 1e-3) "average per-keeper enrich ms" 6000.0 (DE.per_keeper_enrich_ms t)
;;
...
let test_format_includes_all_phases () =
  ...
  let must_contain affix =
    check bool ("contains: " ^ affix) true (Astring.String.is_infix ~affix s)
  in
...
let test_format_with_zero_keepers () =
  ...
  check
    bool
    "zero keepers reports per_keeper=0"
    true
...
let () =
  run
    "dashboard_render_timing_9766"
    [ ( "phase_timing"
      , [ test_case "per-keeper average is enrich/n" `Quick test_per_keeper_average
        ; test_case
            "zero keepers does not divide-by-zero"
            `Quick
            test_per_keeper_zero_keepers_safe
        ; test_case "format includes every phase" `Quick test_format_includes_all_phases
        ; test_case "format handles zero keepers" `Quick test_format_with_zero_keepers
        ] )
    ]
;;
