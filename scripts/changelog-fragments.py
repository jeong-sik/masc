...
def parse_fragment(path: pathlib.Path) -> dict:
    ...
    for lineno, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if line.startswith("#"):
            ...
            if section in sections:
                raise FragmentError(f"{path}:{lineno}: section {section!r} appears twice")
            sections[section] = []
...
def fragment_paths(directory: pathlib.Path) -> list:
    if not directory.is_dir():
        return []
    return sorted(
        (p for p in directory.iterdir() if p.is_file() and p.name != "README.md"),
        key=lambda p: (int(FRAGMENT_NAME.match(p.name).group(1))
                       if FRAGMENT_NAME.match(p.name) else -1, p.name),
    )


def load_all(directory: pathlib.Path) -> list:
...
def split_unreleased(lines: list):
...
def assemble(directory: pathlib.Path, changelog: pathlib.Path) -> int:
    ...
    sources = [{name: bullets} for name, bullets in existing]
...
def unreleased_bullets(text) -> int:
    if text is None:
        return 0
    try:
        _, body, _ = split_unreleased(text.splitlines())
    except FragmentError:
        # A release pull request can open its version section above
        # [Unreleased] or promote it away; either way no live section.
        return 0
...
def pr_guard(base: str, head: str) -> int:
    ...
    if head_count <= base_count:
        return 0
    base_version = VERSION_LINE.search(git_show(base, "dune-project") or "")
    head_version = VERSION_LINE.search(git_show(head, "dune-project") or "")
    if (base_version and head_version
            and base_version.group(1) != head_version.group(1)):
        return 0
