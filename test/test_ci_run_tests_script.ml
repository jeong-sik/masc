...
let script_path () = Filename.concat (source_root ()) "scripts/ci-run-tests.sh"
...
let test_rpc_retry_uses_isolated_build_dir () =
  with_temp_dir "ci-run-tests-retry" (fun dir ->
    ...
    let code, stdout, stderr =
      run_shell
        ...
        (Printf.sprintf
           "%s %s"
           (quote (script_path ()))
           (quote (Printf.sprintf "cd %s && dune test --root ." (quote repo_dir))))
    in
    if code <> 0
    then failf "ci-run-tests failed (%d)\nstdout:\n%s\nstderr:\n%s" code stdout stderr;
    let ci_log_contents = read_file ci_log in
    let observed_output = String.concat "\n" [ ci_log_contents; stdout; stderr ] in
    check
      bool
      "retry warning present"
      true
      (contains_substring
         observed_output
    ...
    check
      bool
      "isolated command exports build dir"
      true
      (contains_substring
         observed_output
    ...
    | [ first; second ] ->
      check
        string
        "first attempt uses default build dir and repo cwd"
        (Printf.sprintf "test||%s" repo_dir)
        first;
      check
        string
        "second attempt uses isolated build dir and repo cwd"
        (Printf.sprintf "test|.ci_build|%s" repo_dir)
        second
    | _ ->
      failf
        "expected exactly two dune invocations, got:\n%s"
        (String.concat "\n" log_lines))
;;
...
let test_agent_sdk_artifact_failure_after_flaky_retry_disables_cache () =
  with_temp_dir "ci-run-tests-interface" (fun dir ->
    ...
    check
      bool
      "agent sdk artifact warning present"
      true
      (contains_substring
         observed_output
         "detected Agent_sdk/OAS artifact or interface mismatch; running dune clean and \
          retrying once with DUNE_CACHE=disabled");
    check
      bool
      "retry command disables cache"
      true
      (contains_substring
         observed_output
         "retry_command: export DUNE_CACHE=disabled; export \
          DUNE_BUILD_DIR=.ci_build_flaky; unset DUNE_RPC;");
    let log_lines =
      read_file fake_log
      |> String.split_on_char '\n'
      |> List.filter (fun line -> String.trim line <> "")
    ...
    | [ first; second; third; fourth ] ->
      check
        string
        "first attempt uses default build dir"
        (Printf.sprintf "test|||%s" repo_dir)
        first;
      check
        string
        "flaky retry uses isolated build dir without cache override"
        (Printf.sprintf "test|.ci_build_flaky||%s" repo_dir)
        second;
      check
        string
        "clean uses isolated build dir"
        (Printf.sprintf "clean|.ci_build_flaky||%s" dir)
        third;
      check
        string
        "clean retry disables dune cache"
        (Printf.sprintf "test|.ci_build_flaky|disabled|%s" repo_dir)
        fourth
    | _ ->
      failf
        "expected exactly four dune invocations, got:\n%s"
        (String.concat "\n" log_lines))
;;
...
let test_disk_full_failure_skips_flaky_retry () =
  with_temp_dir "ci-run-tests-disk-full" (fun dir ->
    ...
    check
      bool
      "disk guidance present"
      true
      (contains_substring observed_output "detected disk exhaustion during dune build");
    check
      bool
      "disk hygiene repair present"
      true
      (contains_substring observed_output "bash scripts/disk-hygiene.sh --fix");
    check
      bool
      "flaky retry skipped"
      false
    ...
    | [ first ] ->
      check
        string
        "single attempt uses repo cwd"
        (Printf.sprintf "test||%s" repo_dir)
        first
    | _ ->
      failf
        "expected exactly one dune invocation, got:\n%s"
        (String.concat "\n" log_lines))
;;
  ...
  with_temp_dir "ci-run-tests-timeout" (fun dir ->
    ...
    let code, stdout, stderr =
      run_shell
        ~cwd:dir
        ~env
        (Printf.sprintf "%s %s" (quote (script_path ())) (quote "sh -c 'sleep 10'"))
    in
    ...
    check
      bool
      "active command pid recorded"
      true
      (contains_substring observed_output "active_cmd_pid=");
    check
      bool
      "active command pgid recorded"
      true
      (contains_substring observed_output "active_cmd_pgid=");
    check
      bool
      "process tree snapshot recorded"
      true
      (contains_substring observed_output "active command process tree snapshot:");
    check
      bool
      "sleeping process captured"
      true
      (contains_substring observed_output "sleep 10");
    check
      bool
      "timeout error present"
      true
      (contains_substring
         observed_output
...
let () =
  run
    ...
    [ ( "script"
      , [ test_case
            "rpc retry uses isolated build dir"
            `Quick
            test_rpc_retry_uses_isolated_build_dir
        ; test_case
            "agent sdk artifact failure after flaky retry disables cache"
            `Quick
            test_agent_sdk_artifact_failure_after_flaky_retry_disables_cache
        ; test_case
            "disk full failure skips flaky retry"
            `Quick
            test_disk_full_failure_skips_flaky_retry
        ; test_case
            "timeout diagnostics capture active process group"
            `Quick
