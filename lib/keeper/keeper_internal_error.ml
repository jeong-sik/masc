(** Keeper_internal_error — the [masc_internal_error] ADT, its JSON codec, the
    ...
    re-homed successor of the deleted [runtime_internal_error] /
    [runtime_error_from_sdk] / [runtime_error_classify] modules (RFC-0206
    ...
    [Runtime_name.t] type is deleted).  JSON keys and the per-kind label
    strings are preserved verbatim because the operator dashboard
    ([dashboard/src]) parses [kind] / [runtime_id] off the wire. *)

(* The originating runtime id is a plain string post-runtime.  Kept as a named
