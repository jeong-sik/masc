...
let expected =
    ...
    ; {|keeper_capability_search|}, {|Search the complete Tool and Skill capability inventory frozen for this Keeper turn. SQLite FTS5 ranks lexical candidates with BM25. Results retain exact typed capability identity and availability but never invoke a candidate.|}, {|{"additionalProperties":false,"properties":{"query":{"description":"FTS5 query over exact capability name, description, category, and invocation name. No substring, regex, stop-word, threshold, top-N, or intent-expansion heuristic is added.","type":"string"}},"required":["query"],"type":"object"}|}
    ; {|keeper_assemble_plan|}, {|Ask the configured Assembler exact-output lane to build one typed Tool DAG from an objective and exact ordinary-Tool references frozen for this Keeper turn. The result is a durable content-addressed proposal plus an exact execution_request. This call never executes the proposal or any referenced Tool; pass execution_request unchanged to the separate proposal execution surface when execution is intended.|}, {|{"additionalProperties":false,"properties":{"execution":{"enum":["inline","async"],"type":"string"},"objective":{"minLength":1,"pattern":"^([^ \t\n\f\r]|[ \t\n\f\r])*[^ \t\n\f\r]([^ \t\n\f\r]|[ \t\n\f\r])*$","type":"string"},"ordinary_tool_references":{"items":{"additionalProperties":false,"properties":{"capability_id":{"minLength":1,"pattern":"^([^ \t\n\f\r]|[ \t\n\f\r])*[^ \t\n\f\r]([^ \t\n\f\r]|[ \t\n\f\r])*$","type":"string"},"descriptor_id":{"minLength":1,"pattern":"^([^ \t\n\f\r]|[ \t\n\f\r])*[^ \t\n\f\r]([^ \t\n\f\r]|[ \t\n\f\r])*$","type":"string"}},"required":["descriptor_id","capability_id"],"type":"object"},"minItems":1,"type":"array","uniqueItems":true}},"required":["objective","execution","ordinary_tool_references"],"type":"object"}|}
    ]
;;
