...
let test_heartbeat_failure_cascade () =
  ...
  let tr =
    dispatch "hb-fail" (KSM.Heartbeat_failed { consecutive = 1; max_allowed = 5 })
  in
  ...
  let tr =
    dispatch "hb-fail" (KSM.Fiber_terminated { outcome = "heartbeat exceeded max" })
  in
...
let test_budget_exhaustion_to_dead () =
  ...
  dispatch_expect_rejected "budget-dead" (KSM.Supervisor_restart_attempt { attempt = 99 });
...
let test_compaction_crash_recovery () =
  ...
  let tr =
    dispatch "compact" (KSM.Compaction_failed { reason = "OOM during compaction" })
  in
  ...
  let tr = dispatch "compact" (KSM.Fiber_terminated { outcome = "cascading OOM" }) in
  check phase_t "fiber death → crashed" KSM.Crashed tr.new_phase;
...
let test_handoff_success () =
  ...
  let tr =
    dispatch
      "handoff"
      (KSM.Handoff_completed { new_trace_id = "trace-2"; generation = 2 })
  in
...
let test_full_chaos_sequence () =
  setup "chaos";
  let tr = dispatch "chaos" (KSM.Heartbeat_failed { consecutive = 1; max_allowed = 5 }) in
  ...
  let tr =
    dispatch "chaos" (KSM.Fiber_terminated { outcome = "handoff target unreachable" })
  in
...
let test_fleet_chaos () =
  ...
  List.iter (fun name -> ignore (R.register ~base_path:bp name (make_meta name))) keepers;
...
let test_turn_failure_cascade () =
  ...
  let tr = dispatch "turn-fail" (KSM.Turn_failed { consecutive = 3; max_allowed = 5 }) in
...
let () =
  run
    "Keeper lifecycle chaos"
    [ "heartbeat", [ eio_test "failure cascade → crash" test_heartbeat_failure_cascade ]
    ; "supervisor", [ eio_test "restart cycle" test_supervisor_restart_cycle ]
    ; ( "terminal"
