...
let compute config entries =
  let success_durations =
    List.filter_map
      (fun (e : BH.history_entry) -> if e.success then Some e.duration_ms else None)
      entries
  in
  let n = List.length success_durations in
  if n < config.min_samples
  then config.default_ms
  else (
...
let stats config entries =
  ...
  then
    Default
      { reason = Printf.sprintf "only %d successful runs (need %d)" n config.min_samples
