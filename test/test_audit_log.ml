...
let test_system_internal_details_deduplicate_canonical_keys () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check int "tool_name appears once" 1 (assoc_key_count "tool_name" entry.details);
       check
         string
         "keeps non-canonical fields"
         "unit_test"
...
let () =
  run
    ...
    [ ( "audit_log"
      , [ test_case
            "system_internal details deduplicate canonical keys"
            `Quick
