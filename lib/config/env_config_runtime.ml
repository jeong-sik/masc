...
module Zombie = struct
  ...
  let threshold_seconds = get_float ~default:300.0 "MASC_ZOMBIE_THRESHOLD_SEC"
...
module Lock = struct
  ...
  let timeout_seconds = get_float ~default:1800.0 "MASC_LOCK_TIMEOUT_SEC"

  (** Lock expiry warning threshold (seconds before expiry) *)
  let expiry_warning_seconds = get_float ~default:300.0 "MASC_LOCK_EXPIRY_WARNING_SEC"
end
...
module Session = struct
  ...
  let max_age_seconds = get_float ~default:3600.0 "MASC_SESSION_MAX_AGE_SEC"
...
module Tempo = struct
  ...
  let min_interval_seconds = get_float ~default:60.0 "MASC_TEMPO_MIN_INTERVAL_SEC"

  (** Maximum polling interval (seconds) - for idle tempo *)
  let max_interval_seconds = get_float ~default:600.0 "MASC_TEMPO_MAX_INTERVAL_SEC"
...
module Decision = struct
  ...
  let ttl_seconds = get_float ~default:3600.0 "MASC_DECISION_TTL_SEC"
end
...
module Cache = struct
  ...
  let max_entry_size = get_int ~default:102400 "MASC_CACHE_MAX_ENTRY_SIZE"

  (** Maximum total number of cache entries (default 1000) *)
  let max_entries = get_int ~default:1000 "MASC_CACHE_MAX_ENTRIES"
end
...
module Claim = struct
  ...
  let ttl_seconds = get_float ~default:3600.0 "MASC_CLAIM_TTL_SECONDS"
end
...
module Orchestrator = struct
  ...
  let check_interval_seconds = get_float ~default:300.0 "MASC_ORCHESTRATOR_INTERVAL"

  (** Orchestrator agent name *)
  let agent_name = get_string ~default:"orchestrator" "MASC_ORCHESTRATOR_AGENT"

  let min_priority = max 0 (min 10 (get_int ~default:2 "MASC_ORCHESTRATOR_MIN_PRIORITY"))
...
module Relay = struct
  let target_agent = get_string ~default:"auto" "MASC_RELAY_TARGET_AGENT"
end

(** {1 CLI Configuration} *)

module Cli = struct
  let default_agent = get_string ~default:"auto" "MASC_CLI_AGENT"
end
...
module Spawn = struct
  ...
  let timeout_seconds = int_of_float (get_float ~default:600.0 "MASC_SPAWN_TIMEOUT_SEC")
...
module Local_runtime = struct
  ...
  let default_model = get_string ~default:"explicit-model-required" "LLAMA_DEFAULT_MODEL"
  ...
  let max_tokens =
    ...
    if primary > 0 then primary else get_int ~default:32768 "MASC_LLAMA_MAX_TOKENS"
  ;;

  (** Default worker model override for the local runtime. *)
  let worker_model_opt () = Sys.getenv_opt "LLAMA_WORKER_MODEL" |> trim_opt
...
module Ollama = struct
  ...
  let default_model = get_string ~default:"" "OLLAMA_DEFAULT_MODEL"
end
...
module Voice = struct
  ...
  let default_host =
    get_string ~default:Masc_network_defaults.masc_http_default_host "VOICE_MCP_HOST"
  ;;

  (** Default Voice MCP server port *)
  let default_port = get_int ~default:8936 "VOICE_MCP_PORT"
end
...
module Timeout = struct
  ...
  let gcloud_auth_sec = get_float ~default:15.0 "MASC_TIMEOUT_GCLOUD_AUTH_SEC"
end
...
module Message = struct
  ...
  let max_count = get_int ~default:200 "MASC_MESSAGE_MAX_COUNT"
end
...
module Transport = struct
  ...
  let normalize_token raw = raw |> String.trim |> String.lowercase_ascii
  ...
  let grpc_target_opt () = Sys.getenv_opt "MASC_GRPC_TARGET" |> trim_opt
  ...
  let _http_auth_strict_registry = Feature_flag_registry.get_bool "MASC_HTTP_AUTH_STRICT"
...
module Cdal = struct
  ...
  let enabled () = Feature_flag_registry.get_bool "MASC_CDAL_ENABLED"

  (** Block task completion when CDAL verdict is Violated/Inconclusive. Default: false. *)
  let gate_enabled () = Feature_flag_registry.get_bool "MASC_CDAL_GATE_ENABLED"
  ...
  (** Max verdicts to scan when looking up the latest verdict by task_id.
      ...
      Issue #7546. *)
  let verdict_lookup_limit () = get_int ~default:500 "MASC_CDAL_VERDICT_LOOKUP_LIMIT"
end

module Verification = struct
  (** Enable AwaitingVerification state and cross-agent approval. Default: false. *)
  let fsm_enabled () = Feature_flag_registry.get_bool "MASC_VERIFICATION_FSM_ENABLED"
...
module Slot = struct
  ...
  let yield_enabled () = Feature_flag_registry.get_bool "MASC_SLOT_YIELD_ENABLED"
end
...
module Board = struct
  ...
  let flush_interval_sec = get_float ~default:30.0 "MASC_BOARD_FLUSH_INTERVAL_SEC"

  (** Board backend type as a typed selector (e.g. "jsonl", "pg"). *)
  let backend_opt () =
    Sys.getenv_opt "MASC_BOARD_BACKEND" |> trim_opt |> Option.map backend_of_string
  ;;
end
...
module Pulse_config = struct
  ...
  let max_consumer_failures =
    max 1 (get_int ~default:3 "MASC_PULSE_MAX_CONSUMER_FAILURES")
  ;;
end
...
module Tools = struct
  ...
  let public_tools_extra_opt () = Sys.getenv_opt "MASC_PUBLIC_TOOLS_EXTRA" |> trim_opt

  let web_search_provider_opt () = Sys.getenv_opt "MASC_WEB_SEARCH_PROVIDER" |> trim_opt

  let web_search_provider_order_opt () =
    Sys.getenv_opt "MASC_WEB_SEARCH_PROVIDER_ORDER" |> trim_opt
  ;;

  let web_search_fallbacks_opt () = Sys.getenv_opt "MASC_WEB_SEARCH_FALLBACKS" |> trim_opt
...
module Worker = struct
  ...
  let local_runtime_debug = Feature_flag_registry.get_bool "MASC_LOCAL_RUNTIME_DEBUG"
  ...
  let local_worker_max_tokens =
    max 1 (get_int ~default:1024 "MASC_LOCAL_WORKER_MAX_TOKENS")
  ;;

  (** Local worker heartbeat interval (seconds). Default: 60. *)
  let local_worker_heartbeat_sec =
    max 1 (get_int ~default:60 "MASC_LOCAL_WORKER_HEARTBEAT_SEC")
  ;;
end
...
module Dashboard = struct
  ...
  let signal_stale_sec = get_float ~default:1200.0 "MASC_DASHBOARD_SIGNAL_STALE_SEC"

  (** Duration (seconds) for borderline "quiet" warning. Default: 600 (10 min). *)
  let signal_quiet_sec = get_float ~default:600.0 "MASC_DASHBOARD_SIGNAL_QUIET_SEC"

  (** Duration (seconds) for a signal to count as "live". Default: 300 (5 min). *)
  let signal_live_sec = get_float ~default:300.0 "MASC_DASHBOARD_SIGNAL_LIVE_SEC"
  ...
  let ctx_handoff_imminent = get_float ~default:0.85 "MASC_DASHBOARD_CTX_HANDOFF_IMMINENT"

  let ctx_preparing = get_float ~default:0.70 "MASC_DASHBOARD_CTX_PREPARING"
  let ctx_compacting = get_float ~default:0.50 "MASC_DASHBOARD_CTX_COMPACTING"
end
...
module InternalTimers = struct
  ...
  let metrics_flush_sec = get_float ~default:300.0 "MASC_METRICS_FLUSH_SEC"
  ...
  let briefing_cache_ttl_sec = get_float ~default:300.0 "MASC_BRIEFING_CACHE_TTL_SEC"

  (** Keeper world observation bootstrap window (seconds). Default: 300 (5 min). *)
  let bootstrap_window_sec = get_float ~default:300.0 "MASC_KEEPER_BOOTSTRAP_WINDOW_SEC"

  (** SSE buffer TTL (seconds). Default: 300 (5 min). *)
  let sse_buffer_ttl_sec = get_float ~default:300.0 "MASC_SSE_BUFFER_TTL_SEC"

  (** Cancellation token cleanup interval (seconds). Default: 300 (5 min). *)
  let cancellation_cleanup_sec = get_float ~default:300.0 "MASC_CANCELLATION_CLEANUP_SEC"

  (** Provider run finished TTL (seconds). Default: 3600 (1 hour). *)
  let provider_run_ttl_sec = get_float ~default:3600.0 "MASC_PROVIDER_RUN_TTL_SEC"
  ...
  let janitor_interval_sec = get_float ~default:60.0 "MASC_JANITOR_INTERVAL_SEC"
  ...
  let rate_limit_bucket_ttl_sec = get_int ~default:300 "MASC_RATE_LIMIT_BUCKET_TTL_SEC"
end
...
module Sidecar = struct
  ...
  let reconcile_backoff_sec = get_float ~default:30.0 "MASC_SIDECAR_RECONCILE_BACKOFF_SEC"
end
