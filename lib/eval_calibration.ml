...
let to_harness_verdict (r : verdict_record) : Masc_agent_core.Harness.verdict =
  let passed =
  ...
  in
  { Masc_agent_core.Harness.passed; score; evidence; detail }
...
let record_verdict
    ...
    ?(on_harness_verdict : (Masc_agent_core.Harness.verdict -> unit) option)
    () : unit =
