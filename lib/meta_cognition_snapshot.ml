...
let load_jsonl_safe path =
  ...
  then []
    ...
    try Fs_compat.load_jsonl path with
    | Eio.Cancel.Cancelled _ as e -> raise e
    | exn ->
      Log.Pages.warn
        "load_jsonl_safe: failed to load %s: %s"
        path
...
let load_board_posts config =
  ...
  load_jsonl_safe path |> List.filter_map Board.post_of_yojson
;;

let load_board_comments config =
  let path = Filename.concat (Coord.masc_dir config) "board_comments.jsonl" in
  load_jsonl_safe path |> List.filter_map Board.comment_of_yojson
;;
  ...
  let sources =
    ...
    |> List.map (fun (post : Board.post) ->
      let target_author =
        ...
        | Some thread_id ->
          (match StringMap.find_opt thread_id post_by_id with
           | Some thread_post -> Some (Board.Agent_id.to_string thread_post.author)
           | None -> None)
        | None -> None
      in
      ...
      ; text =
          String.concat
            "\n"
            (List.filter (fun value -> String.trim value <> "") [ post.title; post.body ])
...
let comment_sources ?hearth_filter (post_by_id : Board.post StringMap.t) comments =
  comments
  |> List.filter_map (fun (comment : Board.comment) ->
    match StringMap.find_opt (Board.Post_id.to_string comment.post_id) post_by_id with
    | None -> None
    | Some parent_post ->
      let hearth_matches =
        match hearth_filter with
      ...
      then None
        ...
        Some
          ...
          ; target_author = Some (Board.Agent_id.to_string parent_post.author)
          })
;;
...
let belief_json ~limit (rule : belief_rule) sources =
  ...
  then None
    ...
    let last_support_at =
      support_sources
      |> List.fold_left (fun best (source : source) -> max best source.created_at) 0.0
    in
    ...
    in
    let status =
      if challenge_agents <> [] && last_challenge_at > last_support_at
      then "contested"
      else if List.length support_agents >= 3
      then "corroborated"
      else "emerging"
    in
    let confidence =
      ...
      in
      let challenge_penalty = float_of_int (List.length challenge_agents) /. 6.0 in
    ...
    Some
      (`Assoc
          ...
          ; ( "evidence_refs"
            , `List
                (support_sources
                 |> List.sort (fun (a : source) (b : source) ->
...
let tension_json ~limit governance_cases (rule : tension_rule) sources =
  ...
  then None
    ...
    in
    let severity =
      if recurrence_count >= 6 || List.length affected_agents >= 4
      then "high"
      else if recurrence_count >= 3 || List.length affected_agents >= 2
      then "medium"
      else "low"
    in
  ...
  then None
    ...
    Some
      (`Assoc
          ...
          ; "source_agents", `List (List.map (fun agent -> `String agent) source_agents)
          ; ( "evidence_refs"
            , `List
                (matching
                 |> List.sort (fun (a : source) (b : source) ->
...
let social_edges_json ~limit sources =
  let record_edge table (source : source) edge_type target_author =
    ...
    | Some edge ->
      StringMap.add
        ...
        ; evidence_refs = unique_non_empty (source.ref_id :: edge.evidence_refs)
  ...
  let table =
    List.fold_left
      (fun table (source : source) ->
         match
           ( source.target_author
           , Meta_cognition_rules.classify_interaction_text source.text )
         with
  ...
  |> List.sort (fun a b ->
    ...
    if by_weight <> 0 then by_weight else compare b.last_seen_at a.last_seen_at)
  |> take limit
  |> List.map (fun edge ->
    `Assoc
      ...
      ; ( "evidence_refs"
        , `List
            (edge.evidence_refs |> take limit |> List.map (fun ref_id -> `String ref_id))
        )
      ])
;;
...
let active_task_count tasks =
  List_util.count_if
    (fun (task : Types.task) -> not (Types.task_status_is_terminal task.task_status))
    tasks
;;
...
let snapshot_json ?hearth ~limit config =
  ...
  let comment_sources = comment_sources ?hearth_filter:hearth post_by_id comments in
  ...
  let all_beliefs =
    ...
    |> List.sort (fun a b ->
      let count_of json key =
        json |> Yojson.Safe.Util.member key |> Yojson.Safe.Util.to_int
      in
  ...
  let tensions =
    ...
    |> List.sort (fun a b ->
      let count_of json =
        json |> Yojson.Safe.Util.member "recurrence_count" |> Yojson.Safe.Util.to_int
      in
  ...
  let collective_desires =
    ...
    |> List.sort (fun a b ->
      let count_of json =
        json |> Yojson.Safe.Util.member "source_agent_count" |> Yojson.Safe.Util.to_int
      in
      compare (count_of b) (count_of a))
    |> take limit
  in
  ...
  `Assoc
    ...
    ; ( "notes"
      , `List
          [ `String
              "Votes are treated as secondary evidence. The current snapshot weights \
               posts, comments, and thread-linked corroboration more heavily."
          ; `String
...
let summary_json ?hearth config =
  ...
  `Assoc
    [ "stagnation_score", Yojson.Safe.Util.member "stagnation_score" snapshot
    ...
    ; ( "top_desire"
      , assoc_subset_or_null
          ...
          [ "id"; "desired_state"; "type"; "actionability"; "strength"; "evidence_refs" ]
      )
    ]
;;
