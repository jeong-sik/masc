...
`cascade_catalog_runtime_resolve.ml` 의 `normalize_declared_name` 와 `profile_lookup` 사이의 auto-normalize 경로가 raw short form (`"strict_tool_candidates"`) 을 `"cascade.strict_tool_candidates"` 로 silent prefix-prepend 한다. 이것은 RFC-0058 이 정의한 "routes → cascade" canonical path 를 우회하는 사이드 채널이다 — caller 가 short string 을 직접 던지면 route table 을 통째로 걸러뛴다.
...
let normalized =
  match
    [ trimmed; "cascade." ^ trimmed; "tier." ^ trimmed ]
...
이 코드의 동작: `"strict_tool_candidates"` 를 입력받으면 `"strict_tool_candidates"`, `"cascade.strict_tool_candidates"`, `"tier.strict_tool_candidates"` 를 *순차 시도* 하여 첫 match 를 반환. 결과적으로 caller 가 `"strict_tool_candidates"` 라고 던지면 `"cascade.strict_tool_candidates"` 로 자동 확장되어 동작한다.

문제:

- **SSOT 위반**: `cascade.toml` 의 `[[routes]]` 테이블이 "canonical path 는 `"cascade.strict_tool_candidates"`" 라고 정의하는데, 코드는 `"strict_tool_candidates"` 도 받아들인다. 같은 의미가 두 가지 wire format 으로 존재.
- **Silent drift**: `cascade.toml` 에서 `"cascade.strict_tool_candidates"` 를 `"tier.strict_tool_candidates"` 로 옮기면, auto-normalize 는 *여전히* `"strict_tool_candidates"` 를 받아들여 첫 match (`"cascade"` prefix) 를 반환. operator 가 route table 을 바꿨음에도 실제 동작이 바뀌지 않을 수 있다.
...
1. **Route table semantics 변경**: `cascade.toml` 의 `[[routes]]` 구조나 cascade 정의를 바꾸지 않는다. 본 RFC 는 *wire format* 과 *분류기* 만 변경.
...
(** Parse a raw cascade name.
    - Accepts: "cascade.X", "tier.X", "route.X" (canonical prefixes)
...
| Error `Invalid_prefix ->
    Error (Printf.sprintf
             "cascade_name %S lacks required prefix (cascade|tier|route)"
             raw_name)
| Error `Empty ->
    Error "cascade_name is empty"
```

auto-prefix 리스트 `[trimmed; "cascade." ^ trimmed; ...]` 와 `List.find_opt` 제거. 단일 시도, 명시적 실패.
...
| Phase | 작업 | 측정 |
|---|---|---|
| **A** | `Cascade_name` 모듈 신설. `cascade_catalog_runtime_resolve.ml` 에 `Cascade_name.of_string` 적용 — short form caller 는 동시에 canonical path 로 교체. `dune build @all` 통과. | (a) 기존 short form caller 0개 확인 (`rg '"strict_tool_candidates"' lib/` 가 0 match); (b) unit test: `of_string "cascade.X"` -> Ok, `of_string "X"` -> Error |
...
1. `rg 'List.find_opt.*cascade\|List.find_opt.*tier\.' lib/cascade/` 0 match — auto-normalize 코드 완전 제거
