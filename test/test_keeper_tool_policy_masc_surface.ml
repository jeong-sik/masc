  ...
  List.iter
    (fun board_name ->
      ...
      | Keeper_tool_name.Direct_masc ->
        check_member (raw_name ^ " direct Keeper projection") raw_name true names
      | Keeper_tool_name.Keeper_wrapper _
      | Keeper_tool_name.External_only ->
        check_member (raw_name ^ " excluded from Keeper projection") raw_name false names)
    ...
    [
      ( "board wrapper filter",
        [
          ...
          test_case
            "projects every Board capability by typed policy"
            `Quick
