...
> **Removed (board-karma-v2 S1):** `health_score`/`health_components` record 필드와 OAS input schema 항목(2곳)을 제거했다. LLM이 rubric 없이 자율 제출하던 값을 keeper가 `effective_scheduled_autonomous_cooldown`의 결정론적 곱셈 인자(0.5x/2.0x)로 사용한 것이 `Board_curation`의 projection-only 계약 위반이었다. Keeper 폴링 빈도는 이제 noop backoff와 idle decay로만 조정된다.
...
6. **댓글 삭제 runtime:** JSONL에서는 delete_post 시 comments/votes를 수동 정리한다.
