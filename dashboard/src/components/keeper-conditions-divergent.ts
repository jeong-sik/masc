...
export function KeeperConditionsDivergent({ keeper }: { keeper: Keeper }) {
  ...
  return html`
    <section class="rounded border border-[var(--warn-24)] bg-[rgba(251,191,36,0.05)] p-3 mb-3" role="alert" aria-label="조건-Phase 불일치">
      <header class="mb-2 flex items-baseline justify-between gap-2">
        <h3 class="text-2xs font-semibold tracking-1 uppercase text-[var(--warn)]">
          <span aria-hidden="true">⚠️</span> 조건-Phase 불일치
        </h3>
