...
let transition_done_r config ~agent_name ~task_id ~notes =
  Coord.transition_task_r config ~agent_name ~task_id ~action:Types.Done_action ~notes ()
;;
...
let test_claim_next_reconciles_stale_agent_current_task () =
  with_test_env (fun config ->
    ...
    let stale_agent =
      ...
      | Ok agent -> { agent with status = Types.Busy; current_task = Some "task-001" }
    ...
    | Coord.Claim_next_no_unclaimed ->
      ...
      let agent_after =
        List.find_opt
          (fun (agent : Types.agent) -> String.equal agent.name agent_name)
          agents
      in
...
let test_claim_next_auto_releases_previous () =
  with_test_env (fun config ->
    ...
    | Some t ->
      Alcotest.(check string)
        "task-001 released to todo"
        "todo"
        (Types.task_status_to_string t.task_status)
...
let test_release_hard_stop_blocks_direct_reclaim () =
  with_test_env (fun config ->
    ...
    | Error e ->
      Alcotest.fail ("expected TaskInvalidState, got " ^ Types.masc_error_to_string e)
...
let test_transition_done_idempotent () =
  with_test_env (fun config ->
    ...
    | Error e ->
      Alcotest.failf "Expected Ok (no-op), got error: %s" (Types.masc_error_to_string e))
;;
...
let test_transition_done_awards_task_reward_once () =
  with_task_economy_enabled (fun () ->
    with_test_env (fun config ->
      ...
      (match Coord.claim_task_r config ~agent_name:claude ~task_id:"task-001" () with
       | Ok _ -> ()
       | Error err -> Alcotest.failf "claim_task_r failed: %s" (Types.show_masc_error err));
      (match
         Coord.transition_task_r
           config
           ~agent_name:claude
       ...
       | Ok _ -> ()
       | Error err ->
         Alcotest.failf "transition_task_r start failed: %s" (Types.show_masc_error err));
      (match
         transition_done_r config ~agent_name:claude ~task_id:"task-001" ~notes:"done"
       with
       | Ok _ -> ()
       | Error err ->
         Alcotest.failf "transition_task_r done failed: %s" (Types.show_masc_error err));
      ...
      (match
       ...
       | Error err -> Alcotest.failf "repeat done failed: %s" (Types.show_masc_error err));
...
let test_update_agent_not_found () =
  with_test_env (fun config ->
    let result =
      Coord.update_agent_r config ~agent_name:"nonexistent" ~status:"active" ()
    in
    match result with
  ...
  Alcotest.run
    ...
    ; (* === Claim Next === *)
      ...
      , [ Alcotest.test_case "basic" `Quick test_claim_next_basic
        ...
        ; Alcotest.test_case
            "BUG-004: auto-releases previous"
            `Quick
            test_claim_next_auto_releases_previous
        ; Alcotest.test_case
            "BUG-004: released task claimable"
            `Quick
            test_claim_next_released_task_claimable_by_others
        ; Alcotest.test_case
            "BUG-004: released_task_id field"
            `Quick
            test_claim_next_r_released_task_field
        ; Alcotest.test_case
    ...
    ; (* === Observability === *)
      ...
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "claim_next auto-release fan-out"
            `Quick
