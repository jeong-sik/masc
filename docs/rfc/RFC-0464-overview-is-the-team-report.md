...
| 4 | 닫힌 quota 창과 재개 시각을 Team 첫 줄에 (`/runtime/resolved`). 처음에는 비용을 뺐다. keeper-costs 가 가격 모르는 턴을 $0 으로 더했기 때문이다(#38083). 지금은 가격이 없으면 합계를 `null` 로 두고 가격 있는·없는·못 읽은 턴을 따로 센다(#38105·#38289·#38373). 그래서 Keeper 줄마다 24시간 비용·토큰을, 제목에 합계를 싣는다(#38363). 모르는 비용은 `$0.00` 이 아니라 `≥`(최소값)나 토큰만으로 그린다. 기본은 꺼 두고 `/cost` 로 켠다. 비용을 켜고 끄는 명령은 이것 하나다(`/burn` 은 없앴다). keeper-costs 는 서버 캐시(60초)가 비면 모든 Keeper 의 하루치 기록을 다시 읽으므로, 꺼져 있으면 요청하지 않는다. 이 스위치 하나로 5번을 정한 것은 아니다 | types/masc_tui/render/keeper_spend |
...
- TUI Session Events 는 그대로 둔다. `RFC-tui-operator-ia` 의 결정과 같다.
  132×40 실측에서 3줄 모두 TUI 기동 로그였다. 끌 수 있게 할지는 5번(보류)에서 정한다.
