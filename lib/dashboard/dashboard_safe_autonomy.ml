...
let worse_level left right = if level_rank left >= level_rank right then left else right
...
let finding_json (finding : finding) =
  `Assoc
    ...
    ; "evidence_refs", `List (List.map evidence_ref_json finding.evidence_refs)
    ]
;;
...
let approval_stats_of_pending_json pending_json : (string, approval_stats) Hashtbl.t =
  ...
  let add_entry keeper_name entry =
    ...
    Hashtbl.replace
      table
      keeper_name
      { count = current.count + 1; oldest_wait_sec; entries = entry :: current.entries }
  in
...
let activity_stats_by_keeper
      (keepers : keeper_meta list)
      (items : Activity_feed.activity_item list)
  =
  ...
  List.iter
    (fun (item : Activity_feed.activity_item) ->
       ...
       | Some keeper_name ->
         ...
         in
         Hashtbl.replace stats keeper_name { count = current.count + 1; last_ts })
    items;
...
let candidate_keeper_profiles keeper_name =
  ...
  if trimmed = ""
  then []
  else if String.starts_with ~prefix:"bench-" trimmed
  then (
    let bare = String.sub trimmed 6 (String.length trimmed - 6) in
    Keeper_types.dedupe_keep_order [ trimmed; bare ])
  else Keeper_types.dedupe_keep_order [ trimmed; "bench-" ^ trimmed ]
;;

let recommendation_for_keeper
      (manifest : Keeper_benchmark_canary.manifest option)
      ~keeper_name
  =
  match manifest with
  | None -> None
...
let transport_health_status transport_json =
  ...
  if (grpc_enabled && not grpc_reachable) || (ws_enabled && not ws_reachable)
  then Fail, "configured transport is unreachable", Some "transport_unhealthy"
  else if String.equal queue_pressure "high"
  then Warn, "transport queue pressure is high", Some "transport_unhealthy"
  else if String.equal queue_pressure "watch"
  then Warn, "transport queue pressure is elevated", None
  else Pass, "transport paths are healthy", None
;;

let domain_definition id =
  match
    List.find_opt (fun (candidate, _, _) -> String.equal candidate id) domain_catalog
  with
...
let classify_tool_correctness
  ...
  | Some recommendation, _ ->
    ...
    let status = if stability >= 0.9 then Pass else Warn in
    let summary =
      Printf.sprintf
  ...
  | None, Some stats when stats.calls >= 3 && stats.success_pct < 80.0 ->
    ...
    , [ make_finding
          ...
          ~human_action_required:false
          ~suggested_next_action:
            "Run the tool-call benchmark corpus for this keeper profile before trusting \
             it for code work."
          ~evidence_refs
          ()
      ] )
  | None, Some stats ->
    let summary =
      Printf.sprintf
        "live tool telemetry shows %.1f%% success across %d calls, but no deterministic \
         benchmark proof"
        stats.success_pct
        stats.calls
    in
...
let classify_sandbox_truth
  ...
  let repo_clone_path = Safe_ops.json_string ~default:"" "clone_path" repo_readiness in
  let evidence_refs =
  ...
  then (
    let summary = Printf.sprintf "sandbox root %s is missing" sandbox.host_root_abs in
    ( make_domain ~id:sandbox_domain_id ~status:Fail ~score:0.0 ~summary ~evidence_refs
    , [ make_finding
    ...
    | "ready" ->
      ( make_domain
          ...
          ~summary:
            (Printf.sprintf
               "sandbox %s is present and repo clone is ready"
    ...
    | "auto_provisionable" ->
      ( make_domain
          ...
          ~score:80.0
          ~summary:
            "sandbox exists and the repo clone can be auto-provisioned on worktree \
             creation"
          ~evidence_refs
      , [] )
    | "missing_clone" ->
      ( make_domain
          ...
          ~status:Warn
          ~score:60.0
          ~summary:"sandbox exists but no repo clone is ready under the keeper playground"
          ~evidence_refs
      , [ make_finding
            ...
            ~human_action_required:false
            ~suggested_next_action:
              "Clone the target repo into the keeper playground before autonomous code \
               work."
            ~evidence_refs
            ()
        ] )
    | _ ->
      let summary = Printf.sprintf "sandbox repo readiness is %s" repo_state in
      ( make_domain ~id:sandbox_domain_id ~status:Fail ~score:25.0 ~summary ~evidence_refs
      , [ make_finding
            ~reason_code:"repo_not_ready"
            ~domain_id:sandbox_domain_id
            ~severity:Fail
            ...
            ~human_action_required:false
            ~suggested_next_action:
              "Repair the keeper sandbox clone state before allowing Git or file \
               mutations."
            ~evidence_refs
            ()
        ] ))
...
let classify_approval_truth ~(keeper_name : string) ~(approval : approval_stats) =
  ...
  | count, Some wait_s when wait_s >= 900.0 ->
    let summary =
      Printf.sprintf "%d approval(s) pending; oldest has waited %.0fs" count wait_s
    in
    ( make_domain ~id:approval_domain_id ~status:Fail ~score:25.0 ~summary ~evidence_refs
    , [ make_finding
          ...
          ~suggested_next_action:
            "Review or resolve the pending approvals before continuing autonomous \
             execution."
          ~evidence_refs
          ()
      ] )
  | count, wait_s_opt ->
    ...
    in
    ( make_domain ~id:approval_domain_id ~status:Warn ~score:60.0 ~summary ~evidence_refs
    , [ make_finding
...
let classify_cascade_fsm ~(config : Coord.config) ~(meta : keeper_meta) =
  ...
  let blocker_class =
    Safe_ops.json_string_opt "runtime_blocker_class" blocker_json |> normalize_string_opt
  in
  let blocker_summary =
  ...
  | Some "no_tool_capable_provider", Some summary ->
    ( make_domain ~id:cascade_domain_id ~status:Warn ~score:60.0 ~summary ~evidence_refs
    , [ make_finding
  ...
  | Some _cls, Some summary ->
    let next_action =
      if continue_gate
      then "Keep the blocker visible and watch the next turn before escalating."
      else "Pause or inspect the keeper before more autonomous turns are admitted."
    in
    ( make_domain ~id:cascade_domain_id ~status:Warn ~score:55.0 ~summary ~evidence_refs
    , [ make_finding
  ...
  | None, Some summary ->
    ...
    , [ make_finding
          ...
          ~human_action_required:true
          ~suggested_next_action:
            "Normalize the blocker path or pause this keeper until the failure is \
             classified."
          ~evidence_refs
          ()
      ] )
...
let classify_audit_trail
  ...
  in
  if total_turns > 0 && trace_history_count = 0 && activity.count = 0
  then
    ( make_domain
        ~id:audit_domain_id
        ~status:Fail
        ~score:20.0
        ~summary:
          "keeper has turns recorded but no visible trace history or activity trail"
        ~evidence_refs
    , [ make_finding
          ...
          ~keeper_name:meta.name
          ~summary:
            "turns exist without trace_history, handoff lineage, or recent activity \
             evidence"
          ~human_action_required:false
          ~suggested_next_action:
            "Inspect trace persistence and activity projection before trusting this \
             keeper's audit trail."
          ~evidence_refs
          ()
      ] )
  else if total_turns = 0 && trace_history_count = 0 && activity.count = 0
  then
    ( make_domain
        ~id:audit_domain_id
        ~status:Warn
...
let build_keeper_snapshot
  ...
  let recommendation = recommendation_for_keeper bench_manifest ~keeper_name:meta.name in
  ...
  let tool_domain, tool_findings =
    classify_tool_correctness
      ~manifest_path
      ~recommendation
      ~live_stats:live_tool_stats
      ~keeper_name:meta.name
  in
  ...
  in
  let cascade_domain, cascade_findings = classify_cascade_fsm ~config ~meta in
  let audit_domain, audit_findings = classify_audit_trail ~config ~meta ~activity in
  { meta
  ; sandbox
  ; repo_readiness
...
let keeper_snapshot_json ~(config : Coord.config) (snapshot : keeper_snapshot) =
  ...
  `Assoc
    ...
    ; ( "active_goal_ids"
      , `List (List.map (fun value -> `String value) meta.active_goal_ids) )
    ...
    ; ( "last_blocker_class"
      , string_opt_to_json
          (Option.map
             Keeper_types.blocker_class_to_string
    ...
    ; ( "live_tool_stats"
      , match snapshot.live_tool_stats with
        | None -> `Null
        | Some stats ->
          `Assoc [ "calls", `Int stats.calls; "success_pct", `Float stats.success_pct ] )
...
let timeline_entries_json
  ...
  List.iter
    (fun snapshot ->
       let blocker = non_empty_string_opt snapshot.meta.runtime.last_blocker in
       match blocker with
       | None -> ()
       | Some summary ->
...
let domain_summary_json
  ...
  in
  let statuses =
    List.map (fun (domain : keeper_domain) -> domain.status) keeper_domains
  in
  let statuses =
  ...
  let pass_count =
    List_util.count_if
      (fun (domain : keeper_domain) -> domain.status = Pass)
      keeper_domains
  in
  let warn_count =
    List_util.count_if
      (fun (domain : keeper_domain) -> domain.status = Warn)
      keeper_domains
  in
  let fail_count =
    List_util.count_if
      (fun (domain : keeper_domain) -> domain.status = Fail)
      keeper_domains
  in
  `Assoc
    [ "id", `String id
      ...
      `Assoc
        (fields
         |> List.filter_map (fun (key, value) ->
           if List.mem key [ "generated_at"; "generated_at_unix" ]
           then None
...
let audit_dir config = Filename.concat (Coord.masc_root_dir config) "audits/safe_autonomy"
...
let write_artifacts ~(config : Coord.config) payload =
  ...
  (match
     Fs_compat.save_file_atomic latest_path (Yojson.Safe.pretty_to_string payload)
   with
   | Ok () -> ()
   | Error message ->
...
let global_status_of_score ~has_fail score =
  if has_fail || score < 60.0 then Fail else if score < 85.0 then Warn else Pass
;;
...
let json ~(config : Coord.config) () =
  ...
  let transport_finding =
    ...
    | Some reason_code ->
      [ make_finding
          ...
          ~human_action_required:false
          ~suggested_next_action:
            "Inspect transport-health and runtime listener state before escalating \
             keeper failures."
          ~evidence_refs:
            [ base_evidence_ref
                "route"
  ...
  let domains =
    ...
    ; domain_summary_json
        ...
        ~extra_note:
          (Printf.sprintf
             "keeper blocker surfacing + transport health (%s)"
             transport_summary)
        ()
    ; domain_summary_json ~id:audit_domain_id ~keepers:keeper_snapshots ()
    ]
  in
  let per_keeper = List.map (keeper_snapshot_json ~config) keeper_snapshots in
  ...
  let human_action_required_count =
    List.length (List.filter (fun finding -> finding.human_action_required) findings)
  in
  let keeper_scores =
    keeper_snapshots
    |> List.map (fun snapshot ->
  ...
  let global_score =
    ...
    then 0.0
      ...
      List.fold_left ( +. ) 0.0 keeper_scores /. float_of_int (List.length keeper_scores)
  in
  let has_fail = List.exists (fun finding -> finding.severity = Fail) findings in
  let global_status = global_status_of_score ~has_fail global_score in
  let timeline =
    timeline_entries_json
      ~alias_to_keeper
      ~activity_items
      ~approval_by_keeper
      ~keepers:keeper_snapshots
  ...
  let payload =
    `Assoc
      ...
      ; ( "evidence_refs"
        , `List
            [ evidence_ref_json (base_evidence_ref "file" "bench_manifest" manifest_path)
            ; evidence_ref_json
                (base_evidence_ref
                   "route"
                   "tool_quality"
            ...
            ; evidence_ref_json
                (base_evidence_ref
                   "route"
                   "surface_readiness"
                   "/api/v1/dashboard/surface-readiness")
            ; evidence_ref_json (base_evidence_ref "route" "metrics" "/metrics")
            ] )
