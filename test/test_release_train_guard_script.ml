...
let test_cross_major_reset_ignores_legacy_2x_tags () =
  with_temp_dir "release-train-reset" (fun dir ->
    ...
    ignore
      (commit_on_branch
         ~dir
         ~branch:"version-reset"
         ~version:"0.2.0"
         ~message:"reset active line");
    let cmd =
      Printf.sprintf "/bin/bash %s --base main --head version-reset" (quote script)
    in
    let code, stdout, stderr = run_shell ~cwd:dir cmd in
    if code <> 0
    then failf "guard failed (%d)\nstdout:\n%s\nstderr:\n%s" code stdout stderr;
    check
      bool
      "uses base major tag lineage"
      true
      (contains_substring
         stdout
...
let test_cross_major_reset_rejects_older_head_series_version () =
  with_temp_dir "release-train-older-head" (fun dir ->
    ...
    let script = install_script_under_test dir in
    ignore
      (commit_on_branch ~dir ~branch:"bad-reset" ~version:"0.1.0" ~message:"bad reset");
    let cmd = Printf.sprintf "/bin/bash %s --base main --head bad-reset" (quote script) in
    let code, _stdout, stderr = run_shell ~cwd:dir cmd in
    check bool "command fails" true (code <> 0);
    check
      bool
      "mentions older head series tag"
      true
      (contains_substring stderr "older than latest tag v0.1.1 in major 0"))
;;
...
let test_pending_bootstrap_series_warns_without_blocking_same_version () =
  with_temp_dir "release-train-bootstrap" (fun dir ->
    ...
    let cmd =
      Printf.sprintf
        "/bin/bash %s --base version-reset --head version-reset"
        (quote script)
    in
    let code, stdout, stderr = run_shell ~cwd:dir cmd in
    if code <> 0
    ...
    check
      bool
      "warns for pending tag"
      true
      (contains_substring
         stdout
         "Release train guard OK (warn): base=0.2.0 head=0.2.0 latest_tag=0.1.1 (pending \
          release)"))
;;
...
let test_pending_bootstrap_series_blocks_next_train_until_tagged () =
  with_temp_dir "release-train-block-next" (fun dir ->
    ...
    let cmd =
      Printf.sprintf "/bin/bash %s --base version-reset --head next-train" (quote script)
    in
    let code, _stdout, stderr = run_shell ~cwd:dir cmd in
    check bool "command fails" true (code <> 0);
    check
      bool
      "requires pending tag first"
      true
      (contains_substring stderr "publish/tag v0.2.0 before widening the release train"))
;;

let () =
  run
    "release_train_guard_script"
    [ ( "script"
      , [ test_case
            "cross-major reset ignores legacy 2.x tags"
            `Quick
            test_cross_major_reset_ignores_legacy_2x_tags
        ; test_case
            "cross-major reset rejects older head series version"
            `Quick
            test_cross_major_reset_rejects_older_head_series_version
        ; test_case
            "pending bootstrap series warns on same version"
            `Quick
            test_pending_bootstrap_series_warns_without_blocking_same_version
        ; test_case
            "pending bootstrap series blocks next train"
            `Quick
