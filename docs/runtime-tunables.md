...
**Total**: 353 unique knobs across 9 modules.

**Typed getter classification**: 23/206 tagged (`operator`: 23, `algorithm`: 0, `unclassified`: 183).
...
| `MASC_BRIEFING_CACHE_TTL_SEC` | typed:float | unclassified | unclassified | 829 | Dashboard mission briefing cache TTL (seconds). Default: 300 (5 min). |
...
| `MASC_CANCELLATION_CLEANUP_SEC` | typed:float | unclassified | unclassified | 841 | Cancellation token cleanup interval (seconds). Default: 300 (5 min). |
...
| `MASC_DASHBOARD_EXECUTION_TRUST_REFRESH_INTERVAL_SEC` | typed:float | unclassified | unclassified | 729 | Execution-trust proactive refresh cadence. Default 60s preserves the original proactive refresh interval. Floor 5s pr... |
...
| `MASC_DASHBOARD_MISSION_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 739 | Mission card compute timeout. Wraps three [Dashboard_cache.get_or_compute_with_timeout] sites at [server_dashboard_ht... |
| `MASC_DASHBOARD_RENDER_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 772 | Maximum wall-clock for a single dashboard render ([Dashboard_execution.json_render]). Wraps the entire render pipelin... |
| `MASC_DASHBOARD_SHELL_LIGHT_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 759 | Shell render compute timeout (light path). Default 8s. Must remain strictly less than [shell_timeout_sec] so the spli... |
...
| `MASC_DASHBOARD_SHELL_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 748 | Shell render compute timeout (full path). Used by [Dashboard_cache.get_or_compute_with_timeout] for the full shell re... |
...
| `MASC_FULL_HEALTH_CRITICAL_FAILURE_THRESHOLD` | typed:int | unclassified | unclassified | 802 | Number of consecutive [/health?full=1] cache-refresh failures that must accumulate before [masc_full_health_refresh_c... |
| `MASC_FULL_HEALTH_REFRESH_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 790 | Full-health snapshot proactive refresh timeout (seconds). Caps a single [/health?full=1] cache refresh attempt in [Se... |
...
| `MASC_JANITOR_INTERVAL_SEC` | typed:float | unclassified | unclassified | 857 | Bootstrap janitor tick interval (seconds). Drives the SSE/session/ rate-limit/webrtc reaper loop in [server_bootstrap... |
| `MASC_KEEPER_BOOTSTRAP_WINDOW_SEC` | typed:float | unclassified | unclassified | 833 | Keeper world observation bootstrap window (seconds). Default: 300 (5 min). |
...
| `MASC_LABEL_QUIET_THRESHOLD_SEC` | typed:float | unclassified | unclassified | 821 | Dashboard label "quiet" threshold (seconds). Default: 300 (5 min). |
| `MASC_LABEL_STUCK_THRESHOLD_SEC` | typed:float | unclassified | unclassified | 825 | Dashboard label "stuck" threshold (seconds). Default: 900 (15 min). |
...
| `MASC_METRICS_FLUSH_SEC` | typed:float | unclassified | unclassified | 813 | Tool metrics flush interval (seconds). Default: 300 (5 min). |
...
| `MASC_PROVIDER_RUN_TTL_SEC` | typed:float | unclassified | unclassified | 845 | Provider run finished TTL (seconds). Default: 3600 (1 hour). |
...
| `MASC_RATE_LIMIT_BUCKET_TTL_SEC` | typed:int | unclassified | unclassified | 871 | Rate-limit bucket staleness TTL (seconds). Buckets with no traffic for this long are reaped by the janitor loop. Defa... |
| `MASC_RELAY_TARGET_AGENT` | typed:string | unclassified | unclassified | 124 | {1 Relay Configuration} |
| `MASC_REPO_SYNC_INTERVAL_SEC` | typed:float | unclassified | unclassified | 863 | Repository auto-sync interval (seconds). The repo_sync fiber in [server_bootstrap_loops] wakes at this cadence to fet... |
| `MASC_SESSION_LIVE_TURN_WINDOW_SEC` | typed:float | unclassified | unclassified | 817 | Team session live turn window (seconds). Default: 300 (5 min). |
...
| `MASC_SHELL_IR_APPROVAL_GATE_ENABLED` | feature_flag | n/a | n/a | 964 | Enable the Shell IR approval policy gate. Default: true (RFC-0254 — the autonomous policy is a strict safety improv... |
| `MASC_SHELL_IR_PATH_JAIL_ENABLED` | feature_flag | n/a | n/a | 978 | Apply the workspace path jail [Exec_policy.validate_shell_ir_paths] to keeper Execute commands. Default: true. Re-rea... |
| `MASC_SIDECAR_CONTROL_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 897 | Subprocess timeout (seconds) for sidecar control commands — [stop], [tail], and similar quick housekeeping operatio... |
| `MASC_SIDECAR_RECONCILE_BACKOFF_SEC` | typed:float | unclassified | unclassified | 885 | Backoff window (seconds) between repeated same-generation [running + unavailable] start dispatches. Default: 30 (matc... |
| `MASC_SIDECAR_SCHEMA_TIMEOUT_SEC` | typed:float | unclassified | unclassified | 913 | Subprocess timeout (seconds) for sidecar Python schema generation. Wraps [Process_eio.run_argv_with_status] at [serve... |
...
| `MASC_SSE_BUFFER_TTL_SEC` | typed:float | unclassified | unclassified | 837 | SSE buffer TTL (seconds). Default: 300 (5 min). |
| `MASC_STALLED_SESSION_THRESHOLD_SEC` | typed:float | unclassified | unclassified | 849 | Operator digest stalled session threshold (seconds). Default: 300 (5 min). |
...
| `MASC_WORKSPACE_GIT_LOCAL_OP_TIMEOUT_SEC` | typed:float | Timeouts | operator | 948 | Budget (seconds) for local-only git operations under [Masc_exec.Exec_gate.run_argv*] in {!Workspace_git}: [rev-parse]... |
...
## Env_config_snapshot (81 knobs; typed classification 0/0)
...
| `MASC_CHANNEL_GATE_DEDUP_TTL_SEC` | string_literal | n/a | n/a | 325 |  |
| `MASC_CHANNEL_GATE_MAX_CONTENT_LENGTH` | string_literal | n/a | n/a | 327 |  |
...
| `MASC_DASHBOARD_TRANSPORT_HEALTH_TIMEOUT_S` | string_literal | n/a | n/a | 289 |  |
| `MASC_DECISION_AUDIT_RING_CAPACITY` | string_literal | n/a | n/a | 343 |  |
| `MASC_DECISION_LAYER_LEVEL` | string_literal | n/a | n/a | 345 |  |
| `MASC_DISCORD_STATUS_STALE_SEC` | string_literal | n/a | n/a | 329 |  |
...
| `MASC_ECONOMY_ENABLED` | string_literal | n/a | n/a | 390 |  |
| `MASC_ECONOMY_FRUGAL_THRESHOLD` | string_literal | n/a | n/a | 392 |  |
| `MASC_ECONOMY_HUSTLE_THRESHOLD` | string_literal | n/a | n/a | 394 |  |
| `MASC_ECONOMY_INITIAL_BALANCE` | string_literal | n/a | n/a | 396 |  |
| `MASC_ECONOMY_REPUTATION_MULTIPLIER` | string_literal | n/a | n/a | 398 |  |
| `MASC_ECONOMY_REWARD_BOARD_POST` | string_literal | n/a | n/a | 400 |  |
| `MASC_ECONOMY_REWARD_MENTION_RESPONSE` | string_literal | n/a | n/a | 402 |  |
| `MASC_ECONOMY_REWARD_TASK_DONE` | string_literal | n/a | n/a | 404 |  |
| `MASC_ECONOMY_REWARD_UPVOTE` | string_literal | n/a | n/a | 406 |  |
...
| `MASC_IMESSAGE_STATUS_STALE_SEC` | string_literal | n/a | n/a | 331 |  |
...
| `MASC_RUNTIME_ATTEMPT_LIVENESS` | string_literal | n/a | n/a | 486 |  |
