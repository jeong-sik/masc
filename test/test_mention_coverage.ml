...
let test_agent_type_empty () = check string "empty" "" (Mention.agent_type_of_mention "")
...
let test_resolve_targets_none () =
  let targets =
    Mention.resolve_targets Mention.None ~available_agents:[ "claude"; "gemini" ]
  in
...
let test_resolve_targets_stateless_found () =
  let targets =
    Mention.resolve_targets
      (Mention.Stateless "claude")
      ~available_agents:[ "claude-gentle-gecko"; "gemini-swift-fox" ]
  in
...
let test_resolve_targets_stateful_not_found () =
  let targets =
    Mention.resolve_targets
      (Mention.Stateful "claude-unknown-animal")
      ~available_agents:[ "claude-gentle-gecko"; "gemini-swift-fox" ]
  in
...
let test_resolve_targets_broadcast () =
  let targets =
    Mention.resolve_targets
      (Mention.Broadcast "claude")
      ~available_agents:
        [ "claude-gentle-gecko"; "claude-brave-tiger"; "gemini-swift-fox" ]
  in
...
let test_resolve_targets_empty_agents () =
  let targets =
    Mention.resolve_targets (Mention.Stateless "claude") ~available_agents:[]
  in
...
let test_is_spawnable_unknown () =
  check
    bool
    "unknown not spawnable"
    false
    (Masc_mcp.Auto_responder.is_spawnable "unknown-agent")
;;
...
let test_is_spawnable_nickname () =
  ...
  check
    bool
    "claude nickname spawnable"
    true
    (Masc_mcp.Auto_responder.is_spawnable "claude-gentle-gecko")
;;
...
let () =
  run
    ...
    ; "spawnable_agents", [ test_case "list" `Quick test_spawnable_agents_list ]
    ]
;;
