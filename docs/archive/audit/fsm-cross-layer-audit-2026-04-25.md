...
- Updated tests to assert unknown phase/turn/decision/runtime/breaker values are preserved.
...
`Keeper_registry.dispatch_event_with_audit` emitted `keeper_composite_changed`, but direct live-turn helpers also mutate fields consumed by `Keeper_composite_observer`: `current_turn_observation`, turn phase, decision stage, runtime state, selected model, and `last_completed_turn`.
...
- Added an OCaml test proving `mark_turn_started`, `set_turn_runtime_state`, and `mark_turn_finished` each produce `keeper_composite_changed`.
...
```text
specs/boundary/RuntimeKeeperRecovery.tla
specs/boundary/RuntimeStrategy.tla
specs/boundary/RuntimeStrategyStateful.tla
...
- `KeeperCompositeLifecycle-buggy-runtime.cfg`
