...
let test_crash_heartbeat_failure () =
  ...
  | Some (`Crashed msg) -> check bool "msg contains heartbeat" true (String.length msg > 0)
...
let test_crash_generic_exception () =
  ...
  | Some e ->
    check string "state" "crashed" (KSM.phase_to_string e.phase);
    (match e.last_failure_reason with
     | Some (R.Exception s) -> check string "exception text" exn_str s
...
let test_dead_tombstone_full_lifecycle () =
  ...
  ignore
    (R.dispatch_event ~base_path:bp "mortal" (KSM.Fiber_terminated { outcome = "test" }));
  (* Simulate budget exhaustion *)
  let max_restarts = Cfg.KeeperSupervisor.max_restarts in
  R.restore_supervisor_state
    ~base_path:bp
    "mortal"
  ...
  (match R.get ~base_path:bp "mortal" with
   | Some e ->
     check string "still dead after Running attempt" "dead" (KSM.phase_to_string e.phase)
   | None -> fail "expected mortal");
  ...
  (match R.get ~base_path:bp "mortal" with
   | Some e ->
     check string "still dead after Crashed attempt" "dead" (KSM.phase_to_string e.phase)
   | None -> fail "expected mortal");
  (* Only unregister removes Dead entry *)
  R.unregister ~base_path:bp "mortal";
  check bool "gone after unregister" false (R.is_registered ~base_path:bp "mortal")
;;
...
let test_self_preservation_suppresses_dominant () =
  ...
  let entries =
    List.map
      (fun name ->
         ...
         let reason =
           if String.length name > 4 && String.sub name 3 2 = "hb"
           then Some (R.Heartbeat_consecutive_failures 5)
           else Some (R.Exception "timeout")
         in
  ...
  let result =
    Sup.apply_self_preservation
      ~keepers_dir:"/tmp/test-keepers"
      ~total_keepers:4
      (to_restart @ [ List.nth entries 3 ])
  in
...
let test_self_preservation_below_threshold () =
  ...
  ignore
    (R.dispatch_event ~base_path:bp "lone" (KSM.Fiber_terminated { outcome = "test" }));
  R.set_failure_reason ~base_path:bp "lone" (Some (R.Heartbeat_consecutive_failures 3));
  let entry =
    match R.get ~base_path:bp "lone" with
    | Some e -> e
    | None -> fail "missing lone"
  in
...
let test_self_preservation_min_candidates_not_met () =
  ...
  ignore
    (R.dispatch_event ~base_path:bp "solo" (KSM.Fiber_terminated { outcome = "test" }));
  R.set_failure_reason ~base_path:bp "solo" (Some (R.Heartbeat_consecutive_failures 3));
  let entry =
    match R.get ~base_path:bp "solo" with
    | Some e -> e
    | None -> fail "missing solo"
  in
  let to_restart = [ entry, "crash" ] in
...
let test_reconcile_predicate_sweep_owned () =
  ...
  (* Crashed = sweep-owned *)
  ignore (R.dispatch_event ~base_path:bp "r1" (KSM.Fiber_terminated { outcome = "test" }));
  (match R.get ~base_path:bp "r1" with
   | Some e -> check bool "crashed is sweep-owned" true (e.phase = KSM.Crashed)
   | None -> fail "expected r1");
  (* Dead = sweep-owned *)
  R.mark_dead ~base_path:bp "r1" ~at:(Unix.gettimeofday ());
  match R.get ~base_path:bp "r1" with
  | Some e -> check bool "dead is sweep-owned" true (e.phase = KSM.Dead)
  | None -> fail "expected r1"
;;
...
let test_reconcile_predicate_stopped_resolved () =
  ...
  | Some e ->
    ...
    check bool "done_p resolved" true (Option.is_some (Eio.Promise.peek e.done_p));
...
let test_reconcile_predicate_stopped_unresolved () =
  ...
  | Some e ->
    check string "stopped" "stopped" (KSM.phase_to_string e.phase);
    check bool "done_p NOT resolved" true (Option.is_none (Eio.Promise.peek e.done_p));
    let dominated =
      match e.phase with
      | KSM.Running | KSM.Paused | KSM.Crashed | KSM.Dead -> true
      | KSM.Failing
...
let test_restart_state_preservation () =
  ...
  | Some e ->
    ...
    check bool "failure_reason cleared" true (Option.is_none e.last_failure_reason);
    (* state should be Running after re-register *)
    check string "state running after restart" "running" (KSM.phase_to_string e.phase)
;;
...
let test_crash_turn_failures () =
  ...
  | Some e ->
    ...
    (match e.last_failure_reason with
     | Some (R.Turn_consecutive_failures n) -> check int "turn failure count" 10 n
...
let test_cohort_key_turn_failures () =
  let key = Sup.cohort_key_of_reason (Some (R.Turn_consecutive_failures 10)) in
...
let test_stop_keepalive_resolves_running_entry_immediately () =
  ...
  | Some entry ->
    ...
    (match Eio.Promise.peek reg.done_p with
     | Some `Stopped -> ()
     | Some (`Crashed reason) -> fail ("expected stopped promise, got crashed: " ^ reason)
...
let test_pipeline_stage_unregistered_is_offline () =
  ...
  check
    bool
    "unregistered → no phase"
    true
    (Option.is_none (R.get_phase ~base_path:bp "ghost"));
  ...
  ignore
    (R.dispatch_event ~base_path:bp "alive" (KSM.Fiber_terminated { outcome = "test" }));
  match R.get_phase ~base_path:bp "alive" with
  | Some phase ->
    let stage = ES.pipeline_stage_of_phase phase in
...
let test_pipeline_stage_sensitivity () =
  ...
  List.iter
    (fun phase ->
       ...
       check
         bool
         (Printf.sprintf "%s should NOT map to offline" (KSM.phase_to_string phase))
         true
         (stage <> "offline"))
    non_offline_phases;
  ...
  List.iter
    (fun phase ->
       ...
       check
         string
         (Printf.sprintf "%s should map to offline" (KSM.phase_to_string phase))
         "offline"
         stage)
    offline_phases
;;
...
let () =
  run
    ...
    ; "dead_tombstone", [ eio_test "full lifecycle" test_dead_tombstone_full_lifecycle ]
    ; ( "self_preservation"
    ...
    ; ( "reconcile_predicates"
      , [ eio_test "sweep-owned states" test_reconcile_predicate_sweep_owned
        ...
        ; eio_test
            "stopped unresolved = sweep"
            test_reconcile_predicate_stopped_unresolved
        ] )
    ; ( "restart_flow"
