...
let contains s sub =
  let rec aux i =
    if i > String.length s - String.length sub
    then false
    else if String.sub s i (String.length sub) = sub
    then true
    else aux (i + 1)
  in
  aux 0
...
let check_contains msg needle haystack = check bool msg true (contains haystack needle)
let temp_dir () = Filename.temp_file "test_" "" ^ "_dir"

let rec mkdir_p path =
  if path = "" || Sys.file_exists path
  then ()
  else (
...
let test_auto_provision_rejects_plain_dir_clone_conflict () =
  ...
  Fun.protect ~finally:(fun () -> cleanup_dir base_path)
  @@ fun () ->
  mkdir_p base_path;
  ...
  match
    Masc_mcp.Coord.auto_provision_sandbox_clone
      ~config
  ...
  | Error (Types.IoError msg) ->
    check_contains "message mentions sandbox_clone_conflict" "sandbox_clone_conflict" msg;
...
let () =
  run
    "Sandbox clone conflict"
    [ ( "auto_provision"
      , [ test_case
            "plain directory at sandbox clone path is rejected"
            `Quick
