...
2. **Checkpoint / history rehydration** — `keeper_context_core_message_json.message_of_json` reconstructs `Agent_sdk.Types.Image` from persisted JSON. The eviction therefore also runs at the checkpoint **write** boundary through typed `Keeper_vision_ingest.evict_message ~mode:Store_only`, so the **persisted JSON** never holds inline base64 and rehydration deserializes a `Text` placeholder it can no longer turn back into an `Image`. This write-path site is store-only (handle placeholder, **no** re-extraction — checkpoint writes have no turn-scoped vision budget and must not block the turn fiber on a provider call), which also makes it the **migration path** for images already persisted in existing keeper checkpoints: they are evicted to bare-handle placeholders on the next write, stopping the reroute.
