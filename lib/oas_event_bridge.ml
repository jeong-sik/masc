...
let emit_native_event_log (evt : Oas.Event_bus.event) (json : Yojson.Safe.t) =
  let log message = Log.emit Log.Info ~module_name:"oas:event" ~details:json message in
...
let native_event_to_json (evt : Oas.Event_bus.event) : Yojson.Safe.t option =
  ...
  | Oas.Event_bus.ElicitationCompleted _ -> None (* Internal; no SSE relay needed *)
  | Oas.Event_bus.ContextOverflowImminent
      { agent_name; estimated_tokens; limit_tokens; ratio } ->
...
let emit_relay_retry_log
  ...
  =
  Log.Misc.warn
    "oas_sse_bridge: retrying event_type=%s stage=%s attempt=%d/%d correlation_id=%s \
     run_id=%s error=%s"
...
let emit_relay_drop_log
  ...
  =
  Log.Server.error
    "oas_sse_bridge: dropping event_type=%s stage=%s attempts=%d correlation_id=%s \
     run_id=%s"
    (relay_event_type pending.json)
    stage_label
    attempts
...
let broadcast_drop_marker
  ...
  | exn ->
    Log.Misc.warn
      "oas_sse_bridge: drop marker broadcast failed: %s"
      (Printexc.to_string exn)
;;
...
let deliver_pending ?store_ref (pending : pending_relay) =
  ...
  in
  try
    deliver_pending_with ~append_json ~broadcast_json:(Sse.broadcast_to All) pending
  with
  | Eio.Cancel.Cancelled _ as e -> raise e
...
let enqueue_pending pending item =
  if List.length pending < relay_max_queue_depth
  then pending @ [ item ], None
  else (
    match pending with
