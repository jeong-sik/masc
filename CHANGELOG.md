...
- Schedule: `masc_schedule_create`, `masc_schedule_update` and `masc_schedule_cancel` record the authenticated caller as the actor instead of reading `requested_by_*`, `scheduled_by_*` and `cancelled_by_*` from the arguments, which are removed from the tool schemas; a Keeper is recorded as `automated_actor` and only the operator's HTTP routes record `human_operator`. A Keeper may update or cancel only the schedules `owner=self` lists for it (scheduled by it or waking it); any other is refused with `error_kind: "not_schedule_owner"`, and a caller with no name is refused with `caller_unidentified` (#38053).
