...
let test_delete_goal_bumps_version () =
  ...
  Goal_store.write_state config { version = 10; updated_at = iso_now (); goals = [ g ] };
...
let test_multiple_deletes_each_bump () =
  ...
  let goals =
    List.init 3 (fun i ->
      make_goal (Printf.sprintf "g-%d" i) (Printf.sprintf "goal %d" i))
  in
  Goal_store.write_state config { version = 5; updated_at = iso_now (); goals };
  let v0 = (Goal_store.read_state config).version in
  List.iter
    (fun i ->
       let _ = Goal_store.delete_goal config ~goal_id:(Printf.sprintf "g-%d" i) in
       ())
    [ 0; 1; 2 ];
  let v_final = (Goal_store.read_state config).version in
  check int "three deletes = +3 versions" (v0 + 3) v_final;
  check int "all goals removed" 0 (List.length (Goal_store.read_state config).goals)
;;
...
let test_delete_nonexistent_does_not_bump () =
  ...
  Goal_store.write_state config { version = 42; updated_at = iso_now (); goals = [ g ] };
  let v_before = (Goal_store.read_state config).version in
...
let test_updated_at_also_refreshed () =
  ...
  Goal_store.write_state config { version = 1; updated_at = stale_ts; goals = [ g ] };
...
let test_legacy_status_defaults_phase () =
  ...
  | [ goal ] ->
    check
      string
      "legacy active becomes executing"
      "executing"
...
let test_blocked_phase_projects_legacy_status () =
  ...
  let goal, _kind =
    match
      Goal_store.upsert_goal config ~title:"Blocked goal" ~phase:Goal_phase.Blocked ()
    with
    | Ok payload -> payload
    | Error msg -> fail msg
  in
  ...
  check
    string
    "blocked phase projects to paused status"
    "paused"
    (match goal.status with
     | Paused -> "paused"
     | _ -> "other")
...
let test_list_goals_filters_by_phase () =
  ...
  let goals = Goal_store.list_goals config ~phase:Goal_phase.Awaiting_approval () in
  check int "one goal in awaiting approval" 1 (List.length goals);
  match goals with
  | [ goal ] ->
    check
      string
      "filtered phase preserved"
      "awaiting_approval"
      (Goal_phase.to_string goal.phase)
...
let test_update_missing_goal_does_not_bump () =
  ...
  Goal_store.write_state config { version = 9; updated_at = iso_now (); goals = [ goal ] };
...
let () =
  run
    "Goal_store.delete_goal"
    [ ( "regression-7690"
      , [ test_case "version bumps +1" `Quick test_delete_goal_bumps_version
        ; test_case "three deletes = +3" `Quick test_multiple_deletes_each_bump
        ; test_case "missing goal: no bump" `Quick test_delete_nonexistent_does_not_bump
        ; test_case "updated_at also refreshed" `Quick test_updated_at_also_refreshed
        ; test_case
            "legacy status defaults phase"
            `Quick
            test_legacy_status_defaults_phase
        ; test_case
            "blocked phase projects legacy status"
            `Quick
            test_blocked_phase_projects_legacy_status
        ; test_case "list_goals filters by phase" `Quick test_list_goals_filters_by_phase
        ; test_case
            "missing update: no bump"
            `Quick
