...
let rec await_turn_terminal io ~tools ~tool_call_count ~tool_effect_attempted ~thread_id ~turn_id ~seen_final
  ...
  | Server_request { id; method_ = "mcpServer/elicitation/request"; params } ->
    ...
    await_turn_terminal io ~tools ~tool_call_count ~tool_effect_attempted ~thread_id ~turn_id
  ...
  | Notification { method_ = "item/started"; params } ->
    ...
    in
    await_turn_terminal
      io ~tools ~tool_call_count ~tool_effect_attempted ~thread_id ~turn_id ~seen_final ~seen_fallback
...
let with_spawned_client ~mgr ~clock ~cwd config run =
  ...
  Eio.Switch.run (fun sw ->
    ...
    (* Terminate inside the body: the child can be waiting for more stdin on
       typed early returns, and only here does it get SIGTERM and a bounded
       reap before the switch's release hook kills whatever is left.
       [Eio.Cancel.protect] inside the finalizer preserves cleanup across
       fiber cancellation. *)
    Fun.protect
