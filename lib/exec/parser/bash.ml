...
let to_shell_ir (stages : (string * string list) list) : Shell_ir.t Parsed.t =
  ...
  | Ok [] ->
    ...
    Parsed.Parse_error { pos = Lexing.dummy_pos; token = ""; expected = [ "command" ] }
;;
...
let classify_too_complex (source : string) : Parsed.reason_too_complex option =
  ...
  if has "$(("
  then Some `Arith_expansion
  else if has "<<<"
  then Some `Here_string
  else if has "<<"
  then Some `Heredoc
  else if has "&&" || has "||"
  then Some `Logic_op
  ...
  then Some `Glob_brace
  else None
;;

let map_error_or_classify (source : string) (lexbuf : Lexing.lexbuf) : Shell_ir.t Parsed.t
  =
