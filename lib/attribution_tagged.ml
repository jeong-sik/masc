...
let det_passed ~gate ~evidence : det t = Attribution.passed ~origin:Det ~gate ~evidence
...
let det_transition_blocked ~gate ~evidence ~from_state ~to_state ~reason : det t =
  Attribution.transition_blocked ~origin:Det ~gate ~evidence ~from_state ~to_state ~reason
;;
...
let fold_rationale_into_evidence (evidence : Yojson.Safe.t) ~rationale : Yojson.Safe.t =
  match evidence with
  | `Assoc fields -> `Assoc (fields @ [ "rationale", `String rationale ])
  | _other -> `Assoc [ "original_evidence", evidence; "rationale", `String rationale ]
;;
