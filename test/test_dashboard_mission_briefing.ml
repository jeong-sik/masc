...
let test_briefing_cold_call_returns_pending () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let json = Dashboard_mission_briefing.json ~config ~sw ~clock ~proc_mgr:None () in
       let open Yojson.Safe.Util in
       (* Cold call now computes synchronously instead of returning pending *)
       check string "cold call returns ok" "ok" (json |> member "status" |> to_string);
       check
         bool
         "refreshing false on cold call"
         false
...
let test_force_refresh_without_cache_returns_pending () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       ignore (Coord.init config ~agent_name:None);
       let json =
         Dashboard_mission_briefing.json ~force:true ~config ~sw ~clock ~proc_mgr:None ()
       in
       let open Yojson.Safe.Util in
       check string "status pending" "pending" (json |> member "status" |> to_string);
       check bool "refreshing true" true (json |> member "refreshing" |> to_bool))
;;
...
let test_force_refresh_with_cached_result_returns_stale_cached_payload () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let open Yojson.Safe.Util in
       check string "status ok" "ok" (json |> member "status" |> to_string);
       check bool "cached true" true (json |> member "cached" |> to_bool);
       check bool "stale true" true (json |> member "stale" |> to_bool);
       check bool "refreshing true" true (json |> member "refreshing" |> to_bool);
       check
         string
         "summary preserved"
  ...
  let stale_terminal =
    `Assoc
      ...
      ; ( "status"
        , `Assoc
            [ ( "session"
              , `Assoc [ "project", `String "default"; "status", `String "interrupted" ] )
            ; "summary", `Assoc []
            ] )
      ; "recent_events", `List []
      ]
  ...
  let active_session =
    `Assoc
      ...
      ; ( "status"
        , `Assoc
            [ ( "session"
              , `Assoc [ "project", `String "default"; "status", `String "running" ] )
            ; "summary", `Assoc []
            ] )
      ; ( "recent_events"
        , `List [ `Assoc [ "ts_iso", `String (iso_of_unix (now_ts -. 30.0)) ] ] )
      ]
  in
  let relevant =
    Briefing.relevant_sessions_for_briefing
...
let test_collect_metadata_gaps_separates_null_like_inputs () =
  ...
  in
  let gaps =
    Briefing.collect_metadata_gaps
      ~sessions
      ~keepers
...
let test_collect_metadata_gaps_ignores_inactive_agents () =
  ...
  let active_agent : Types.agent =
    { inactive_agent with name = "agent-live"; status = Types.Active }
  in
  let gaps =
    Briefing.collect_metadata_gaps
      ~sessions:[]
...
let test_build_briefing_sections_demotes_metadata_only_communication () =
  ...
  let watch_summary, sections =
    Briefing.build_briefing_sections
  ...
  check
    string
    "watch summary"
    "No immediate operator action is flagged by the namespace summary."
    watch_summary;
...
let test_build_briefing_sections_keeps_metadata_evidence_visible () =
  ...
  let _watch_summary, sections =
    Briefing.build_briefing_sections
      ...
      ~sessions:[ session ]
      ~agents:[]
      ~recent_messages:[ `Assoc [ "from", `String "agent-a" ] ]
      ~metadata_gaps:
        [ `Assoc
            [ "kind", `String "keeper_last_reply_missing"
            ; "summary", `String "Keeper last reply status is not recorded."
...
let () =
  run
    ...
    [ ( "deterministic"
      , [ test_case
            "cold call returns pending"
            `Quick
            test_briefing_cold_call_returns_pending
        ; test_case
            "force refresh without cache returns pending"
            `Quick
            test_force_refresh_without_cache_returns_pending
        ; test_case
            "force refresh with cache returns stale cached payload"
            `Quick
