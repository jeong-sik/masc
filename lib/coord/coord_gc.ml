...
let heartbeat config ~agent_name =
  ...
  else Printf.sprintf "⚠ Agent %s not found" agent_name
;;
...
let cleanup_zombies
  ...
  let scan_paths = if Sys.file_exists agents_path then [ agents_path ] else [] in
  if scan_paths = []
  then "📋 No agents directory"
  else (
    (* Phase 1: Detect zombie agents (no side effects) *)
    let zombie_entries = ref [] in
    (* (name, path) list *)
    List.iter
      (fun agents_path ->
         ...
         |> Array.iter (fun name ->
           ...
           then (
             ...
             | Error err ->
               ...
               let quarantine_path = Printf.sprintf "%s.broken-%d" path ts_ms in
               Log.Gc.warn
               ...
               (try
                  (try Sys.rename path quarantine_path with
                   | Sys_error _ ->
                ...
                | exn ->
                  Log.Gc.warn
                    "failed to quarantine broken agent %s: %s"
                    path
                    (Printexc.to_string exn)))))
      scan_paths;
    if !zombie_entries = []
    then "✅ No zombie agents found"
    else (
      ...
      List.iter
        (fun (name, path) ->
           (try
              ...
              | Error err ->
                Log.Gc.warn "gc status update parse error for %s: %s" name err
            with
            | Sys_error msg ->
              Log.Gc.warn "gc status update I/O error for %s: %s" name msg);
           ...
           (try (Atomic.get Coord_hooks.stop_keeper_fn) name with
            | Eio.Cancel.Cancelled _ as e -> raise e
            | exn ->
              Log.Gc.warn
                "gc stop_keeper_fn error for %s: %s"
                name
                (Printexc.to_string exn));
           ())
        !zombie_entries;
      ...
      List.iter
        (fun (task : task) ->
           ...
           | (Types.Claimed { assignee; _ } | Types.InProgress { assignee; _ })
             ...
             (match
                (Atomic.get Coord_hooks.force_release_task_fn)
                  config
                  ~agent_name:"keeper-gc"
                  ~task_id:task.id
                  ()
              with
              | Ok msg -> released_tasks := (task.id, msg) :: !released_tasks
              | Error e ->
                if not (List.mem assignee !release_failed_agents)
                then release_failed_agents := assignee :: !release_failed_agents;
                log_event
                  config
                  (Printf.sprintf
      ...
      List.iter
        (fun (name, path) ->
           if List.mem name !release_failed_agents
           then Log.Gc.warn "skipping file removal for %s: task release failed" name
           else (
        ...
        let _state =
          update_state config (fun s ->
            { s with
              active_agents =
                List.filter
                  (fun a -> not (List.mem a !successfully_cleaned))
                  s.active_agents
            })
        in
        log_event
          ...
          (Printf.sprintf
             ...
             (Yojson.Safe.to_string
                (`List (List.map (fun s -> `String s) !successfully_cleaned)))
      ...
      then
        Printf.sprintf
          ...
          cleaned
          total
          (String.concat ", " !successfully_cleaned)
          task_note
          skipped
      else
        Printf.sprintf
          "🧟 Cleaned up %d zombie agent(s): %s%s"
          cleaned
          (String.concat ", " !successfully_cleaned)
          task_note))
;;
...
let gc config ?(days = 7) () =
  ...
  then (
    ...
    let new_backlog =
      { tasks = kept_tasks; last_updated = now_iso (); version = backlog.version + 1 }
    in
    write_backlog config new_backlog;
    results
    := Printf.sprintf "📦 Archived %d stale task(s) (older than %d days)" !stale_count days
       :: !results)
  else results := Printf.sprintf "✅ No stale tasks (threshold: %d days)" days :: !results;
  ...
  let open_task_ids =
    List.filter_map
      (fun task ->
         if Types.task_status_is_terminal task.task_status then None else Some task.id)
      backlog.tasks
  in
  ...
  in
  if Sys.file_exists messages_path
  then
    Sys.readdir messages_path
    |> Array.iter (fun name ->
      Coord_query.safe_yield ();
      if Filename.check_suffix name ".json"
      then (
        ...
        let content =
          Yojson.Safe.Util.(member "content" json |> to_string_option)
          |> Option.value ~default:""
        in
        match ts with
        | Some ts when ts < cutoff_iso ->
          (* Preserve if message references an open task *)
          if mentions_open_task content
          then incr preserved_count
          else (
            Sys.remove path;
            incr old_msg_count)
        | None | Some _ -> ()));
  if !old_msg_count > 0 || !preserved_count > 0
  then (
    if !old_msg_count > 0
    then
      results
      := Printf.sprintf
           "🗑️ Deleted %d old message(s) (older than %d days)"
           !old_msg_count
           days
         :: !results;
    ...
    then
      results
      := Printf.sprintf
           "🔒 Preserved %d message(s) referencing open tasks"
           !preserved_count
         :: !results)
  else results := Printf.sprintf "✅ No old messages (threshold: %d days)" days :: !results;
  ...
  (match backend_cleanup_pubsub config ~days ~max_messages:10000 with
   | Ok count when count > 0 ->
     ...
     results
     := Printf.sprintf "🗃️ Cleaned %d pubsub message(s) from backend" count :: !results
   | Ok _ -> () (* No messages to clean *)
   | Error e ->
     results
     := Printf.sprintf "⚠️ Backend pubsub cleanup failed: %s" (Backend_types.show_error e)
        :: !results);
      ...
      (fun name ->
         ...
         then (
           ...
           then (
             ...
             if not (List.mem base active_keepers)
             then (
               (try Sys.remove (Filename.concat pk_dir name) with
                | Sys_error _ -> ());
      ...
      then (
        Array.iter
          (fun child ->
             ...
             if Sys.is_directory child_path
             then rmdir_recursive child_path
             else (
               try Sys.remove child_path with
               | Sys_error _ -> ()))
          (Sys.readdir path);
    ...
    List.iter
      (fun name ->
         ...
         then (
           ...
           if Sys.file_exists dir_path && Sys.is_directory dir_path
           then (
             let metrics_dir = Filename.concat dir_path "metrics" in
             if
               (not (List.mem name active_keepers))
               && Sys.file_exists metrics_dir
               && Sys.is_directory metrics_dir
             then (
               rmdir_recursive metrics_dir;
               (* Remove the keeper dir itself if now empty *)
               (try
                  if Array.length (Sys.readdir dir_path) = 0 then Unix.rmdir dir_path
                with
                | Sys_error _ | Unix.Unix_error _ -> ());
               incr keeper_orphan_count))))
      entries);
  if !keeper_orphan_count > 0
  then
    results
    := Printf.sprintf "🧹 Removed %d orphan keeper sidecar file(s)" !keeper_orphan_count
       :: !results
  else results := "✅ No orphan keeper files" :: !results;
  ...
  let ts_root =
    Filename.concat
      ...
      "team-sessions"
  in
  if Sys.file_exists ts_root && Sys.is_directory ts_root
  then (
    let archive_ts_dir =
      Filename.concat
        (Filename.concat
    ...
    |> Array.iter (fun session_id ->
      ...
      if Sys.is_directory sdir
      then (
        let sjson = Filename.concat sdir "session.json" in
        if Sys.file_exists sjson
        then (
          try
            ...
            then (
              ...
              (try Unix.rename sdir dest with
               | Unix.Unix_error _ ->
          ...
          | exn ->
            Log.Gc.warn
              "session archive %s failed: %s"
              session_id
              (Printexc.to_string exn)))));
  if !session_archive_count > 0
  then
    results
    := Printf.sprintf
         "📦 Archived %d completed/interrupted/cancelled team session(s)"
         !session_archive_count
       :: !results
  else results := "✅ No team sessions to archive" :: !results;
  (* 7. Hard-delete board artifacts (via hooks) *)
  let board_artifact_count = (Atomic.get Coord_hooks.cleanup_board_artifacts_fn) () in
  if board_artifact_count > 0
  then
    results
    := Printf.sprintf "🧽 Removed %d board artifact post(s)" board_artifact_count
       :: !results
  else results := "✅ No board artifacts" :: !results;
  let cp_json = "null" in
