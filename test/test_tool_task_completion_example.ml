...
let test_message_includes_example () =
  ...
  Alcotest.(check bool)
    "rejection msg includes example"
    true
    (contains TT.completion_notes_example msg);
  Alcotest.(check bool) "rejection msg keeps reason" true (contains "verdict text" msg)
;;

let test_allow_force_variant_includes_example () =
  let msg = TT.completion_rejection_message ~allow_force:true "verdict text" in
  Alcotest.(check bool)
    "force variant includes example"
    true
    (contains TT.completion_notes_example msg);
  Alcotest.(check bool) "force variant keeps force hint" true (contains "force=true" msg)
;;

let test_example_nonempty () =
  Alcotest.(check bool)
    "example is nontrivial"
    true
...
let () =
  Alcotest.run
    "tool_task_completion_example"
    [ ( "completion_rejection_message"
      , [ Alcotest.test_case
            "plain variant embeds example"
            `Quick
            test_message_includes_example
        ; Alcotest.test_case
            "force variant embeds example"
            `Quick
            test_allow_force_variant_includes_example
        ; Alcotest.test_case "example string is substantive" `Quick test_example_nonempty
        ] )
    ]
;;
