...
let test_digest_room_prefers_fresh_operator_judgment () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Alcotest.(check bool)
         "authoritative judgment available"
         true
         Yojson.Safe.Util.(digest |> member "authoritative_judgment_available" |> to_bool);
       Alcotest.(check string)
         "active guidance layer"
         "judgment"
         Yojson.Safe.Util.(digest |> member "active_guidance_layer" |> to_string);
       Alcotest.(check string)
         "active summary from judgment"
         "Pause the namespace before taking any destructive action."
         Yojson.Safe.Util.(
           digest |> member "active_summary" |> member "summary" |> to_string);
       Alcotest.(check string)
         "active recommendation source"
         "judgment"
...
let test_digest_room_ignores_stale_operator_judgment () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       Alcotest.(check string)
         "judgment owner fallback"
         "fallback_read_model"
         Yojson.Safe.Util.(digest |> member "judgment_owner" |> to_string);
       Alcotest.(check bool)
         "authoritative judgment unavailable"
         false
         Yojson.Safe.Util.(digest |> member "authoritative_judgment_available" |> to_bool);
       Alcotest.(check string)
         "active guidance layer fallback"
         "fallback"
         Yojson.Safe.Util.(digest |> member "active_guidance_layer" |> to_string);
       Alcotest.(check bool)
         "judgment missing"
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       record_operator_judgment
         ...
         ();
       let fields =
         Operator_digest_guidance.active_guidance_fields
           ~config
           ~actor:"operator"
           ~target_type:"keeper"
           ~target_id:None
...
let test_operator_judgment_write_and_latest_roundtrip () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let written =
         match
           Operator_control.judgment_write_json
             ctx
             (`Assoc
       ...
       let latest =
         match
           Operator_control.judgment_latest_json
             ctx
             (`Assoc
                 [ "surface", `String "command.namespace"
                 ; "target_type", `String "namespace"
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       (match
        ...
        | Error err -> Alcotest.(check bool) "non-empty error" true (String.length err > 0));
