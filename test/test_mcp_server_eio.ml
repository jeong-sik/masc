...
let make_keeper_meta ?agent_name ?tool_access name =
  let agent_name =
    Option.value agent_name ~default:(Keeper_types.keeper_agent_name name)
  in
...
let extract_json_from_text text =
  ...
  with
  | Not_found -> Alcotest.failf "expected JSON payload in text: %s" text
;;
...
let find_mcp_tool_log_exn ~phase ~tool_name ~request_id entries =
  ...
  | None ->
    Alcotest.failf
      "MCP tool log missing phase=%s tool=%s request_id=%s"
      phase
      tool_name
      request_id
;;
...
let tools_list_all ~clock ~sw ?profile state = collect_tools ~clock ~sw ?profile state []
...
let test_create_state () =
  ...
  Alcotest.(check string) "base_path preserved" base_path state.room_config.base_path;
...
let test_type_compatibility () =
  ...
  let state2 : Mcp.server_state = state in
  (* Type unification at compile time *)
  (* Verify the unified type preserves field access *)
  Alcotest.(check string)
    "base_path via unified type"
    base_path
    state2.room_config.base_path;
  cleanup_dir base_path
;;
...
let test_eio_context_delegation () =
  ...
  in
  Alcotest.(check bool)
    "net delegated to shared Eio_context"
    true
    (Option.is_some delegated_net);
  Alcotest.(check bool) "clock delegated to shared Eio_context" true clock_delegated
;;
...
let test_is_notification () =
  let with_id =
    `Assoc [ "jsonrpc", `String "2.0"; "id", `Int 1; "method", `String "test" ]
  in
  let without_id =
    `Assoc [ "jsonrpc", `String "2.0"; "method", `String "notifications/initialized" ]
  in
  (match Masc_mcp.Mcp_server.jsonrpc_request_of_yojson with_id with
   | Ok req ->
     Alcotest.(check bool) "with id" false (Masc_mcp.Mcp_server.is_notification req)
   | Error _ -> Alcotest.fail "parse error");
  match Masc_mcp.Mcp_server.jsonrpc_request_of_yojson without_id with
  | Ok req ->
    Alcotest.(check bool) "without id" true (Masc_mcp.Mcp_server.is_notification req)
  | Error _ -> Alcotest.fail "parse error"
;;
...
let test_protocol_version () =
  ...
  (match Mcp.validate_protocol_version "2025-06-18" with
   | Ok version -> Alcotest.(check string) "2025-06-18 is supported" "2025-06-18" version
     ...
     (match List.assoc_opt "result" fields with
      | Some (`Assoc result_fields) ->
        Alcotest.(check (option string))
          "echoes requested protocol version"
          (Some "2025-06-18")
...
let test_handle_request_tools_list () =
  ...
  Alcotest.(check bool)
    "next cursor matches totalCount/pageSize"
      ...
      (`Assoc
          ...
          ; ( "params"
            , `Assoc
                ...
                ; "capabilities", `Assoc []
                ; ( "clientInfo"
                  , `Assoc [ "name", `String "remote-operator"; "version", `String "1.0" ]
                  )
                ] )
          ])
  in
      ...
      | Some (`Assoc result_fields) ->
        (match List.assoc_opt "tools" result_fields with
         | Some (`List tools) ->
           ...
           Alcotest.(check bool)
             "snapshot readonly hint"
             true
             (snapshot_tool
              |> Yojson.Safe.Util.member "annotations"
              |> Yojson.Safe.Util.member "readOnlyHint"
              |> Yojson.Safe.Util.to_bool);
           Alcotest.(check bool)
             "digest readonly hint"
             true
             (digest_tool
              |> Yojson.Safe.Util.member "annotations"
              |> Yojson.Safe.Util.member "readOnlyHint"
      ...
      (`Assoc
          ...
          ; ( "params"
            , `Assoc
                ...
                ; "capabilities", `Assoc []
                ; ( "clientInfo"
                  , `Assoc [ "name", `String "managed-agent"; "version", `String "1.0" ] )
                ] )
          ])
  in
  let response =
   ...
   | `Assoc fields ->
     (match List.assoc_opt "result" fields with
      | Some (`Assoc result_fields) ->
        ...
        in
        Alcotest.(check bool)
          "mentions managed profile"
          true
          (contains_substring instructions "managed-agent profile");
        Alcotest.(check bool)
          "mentions canonical task control"
          true
      ...
      | Some (`Assoc result_fields) ->
        (match List.assoc_opt "tools" result_fields with
         | Some (`List tools) ->
           ...
           in
           Alcotest.(check bool)
             "has managed room status alias"
             true
             (List.mem "masc_room_status" names);
           Alcotest.(check bool)
             "has managed list tasks alias"
             true
             (List.mem "masc_list_tasks" names);
           Alcotest.(check bool)
             "hides managed claim alias"
             false
             (List.mem "masc_claim_task" names);
           Alcotest.(check bool)
             "omits raw masc_status"
             false
             (List.mem "masc_status" names);
           Alcotest.(check bool)
             "omits raw masc_transition"
             false
             (List.mem "masc_transition" names);
           Alcotest.(check bool)
             "omits managed voice agent"
             false
             (List.mem "masc_voice_agent" names);
           Alcotest.(check bool)
             "omits managed voice speak"
             false
             (List.mem "masc_voice_speak" names);
           Alcotest.(check bool)
             "omits managed voice ping pong"
             false
             (List.mem "masc_voice_ping_pong" names)
         | _ -> Alcotest.fail "tools not a list")
      | _ -> Alcotest.fail "result not an object")
...
let test_handle_request_tools_call_managed_profile_sdk_alias_claim () =
  ...
  Alcotest.(check bool)
    "claim response mentions task"
    true
    (contains_substring response_text "task-001");
  Alcotest.(check bool)
    "claim response mentions claimed"
    true
...
let test_handle_request_tools_call_transition_claim_guidance () =
  ...
  in
  let response = Mcp_eio.handle_request ~clock ~sw ~mcp_session_id:sid state request in
  let steps = workflow_next_step_names response in
  Alcotest.(check (option string))
    "claim binds current_task"
    (Some "task-001")
...
let test_handle_request_tools_call_transition_done_guidance () =
  ...
  let ok_claim, _ =
    Mcp_eio.execute_tool_eio
      ...
      ~arguments:(`Assoc [ "task_id", `String "task-001"; "action", `String "claim" ])
  in
...
let test_handle_request_tools_list_rejects_nonstandard_names_filter () =
  ...
  let request =
    Yojson.Safe.to_string
      (`Assoc
          ...
          ; "method", `String "tools/list"
          ; ( "params"
            , `Assoc [ "names", `List [ `String "masc_messages"; `String "masc_status" ] ]
            )
          ])
  in
  let response = Mcp_eio.handle_request ~clock ~sw state request in
  ...
  in
  Alcotest.(check (list string))
    "requested tools only"
    [ "masc_messages"; "masc_status" ]
    names;
  cleanup_dir base_path
;;
...
let test_handle_request_tools_list_include_deprecated_claim_alias_metadata () =
  ...
  Alcotest.(check string)
    "transition lifecycle remains active"
    "active"
    (tool_string_field transition_tool "lifecycle");
  (match transition_tool with
   | `Assoc fields ->
     Alcotest.(check bool)
       "transition omits canonical alias metadata"
       false
       (List.mem_assoc "canonicalName" fields);
     Alcotest.(check bool)
       "transition omits replacement alias metadata"
       false
...
let _test_handle_request_tools_list_hides_internal_tool_by_default () =
  ...
  let tools = tools_list_all ~clock ~sw state in
  Alcotest.(check bool)
    "internal tool hidden from public surface"
    false
    (List.exists
       (function
         | `Assoc fields ->
...
let test_handle_request_tools_list_include_usage_metadata () =
  ...
  Alcotest.(check bool)
    "usage telemetry availability exposed"
    true
  ...
  in
  Alcotest.(check bool)
    "per-tool usage count exposed"
    true
    (Yojson.Safe.Util.member "usageCount" first_tool <> `Null);
  Alcotest.(check bool)
    "per-tool last-used field present"
    true
    (List.mem_assoc
       "usageLastUsedAt"
       (match first_tool with
        | `Assoc fields -> fields
        | _ -> []));
  cleanup_dir base_path
;;
...
let _test_execute_tool_trpg_flow () =
  ...
  let ok_turn, _turn_msg =
    Mcp_eio.execute_tool_eio
      ...
      ~arguments:(`Assoc [ "room_id", `String "room-mcp-e2e"; "phase", `String "round" ])
  in
  ...
  let ok_stream_dice, stream_dice_msg =
    Mcp_eio.execute_tool_eio
      ...
      ~name:"masc_trpg_stream"
      ~arguments:
        (`Assoc [ "room_id", `String "room-mcp-e2e"; "event_type", `String "dice.rolled" ])
  in
  ...
  let passed =
    roll_json
    |> Yojson.Safe.Util.member "roll"
    |> Yojson.Safe.Util.member "passed"
    |> Yojson.Safe.Util.to_bool
  in
...
let _test_execute_tool_trpg_validation () =
  ...
  let ok_bad_event_type, msg_bad_event_type =
    Mcp_eio.execute_tool_eio
      ...
      ~arguments:
        (`Assoc
            [ "room_id", `String "room-mcp-e2e"; "event_type", `String "totally.invalid" ])
  in
...
let test_execute_tool_explicit_alias_reuses_joined_nickname () =
  ...
  let _added =
    Masc_mcp.Coord.add_task
      ...
      ~priority:2
      ~description:
        "Verify that an explicit alias can reuse the nickname established during \
         claim/start/done transitions."
  in
  ...
  Alcotest.(check bool)
    "claim message has claimed"
    true
    (contains_substring claim_msg "claimed");
  let ok_start, start_msg = transition "start" in
  Alcotest.(check bool) "start success with same explicit alias" true ok_start;
  Alcotest.(check bool)
    "start message has in_progress"
    true
    (contains_substring start_msg "in_progress");
  let ok_done, done_msg =
    transition
      ~extra:
        [ ( "notes"
          , `String
              "Completed the alias reuse regression by claiming, starting, and finishing \
               task-001 with the same explicit alias, confirming the joined nickname \
               stayed stable and the transition responses reported success." )
        ]
      "done"
  in
  ...
  Alcotest.(check bool)
    "done or awaiting_verification reached"
    true
...
let check_task_still_todo config task_id =
  ...
  | Some task ->
    Alcotest.failf
      "expected %s to remain todo, got %s"
      task_id
...
let check_auth_preflight_result ~tool_name ok msg =
  ...
  Alcotest.(check bool)
    (tool_name ^ " reports auth/credential blocker")
    true
...
let test_execute_tool_explicit_generated_alias_claim_next_not_rewritten_by_token () =
  ...
  in
  ignore
    (Masc_mcp.Coord.join state.room_config ~agent_name:"stable-admin" ~capabilities:[] ());
  ignore
    (Masc_mcp.Coord.add_task
       state.room_config
       ~title:"explicit-alias-claim-next"
       ~priority:2
       ~description:"");
  let ok, msg =
    Mcp_eio.execute_tool_eio
...
let test_execute_tool_explicit_generated_alias_transition_not_rewritten_by_token () =
  ...
  ignore
    (Masc_mcp.Coord.add_task
       state.room_config
       ~title:"explicit-alias-transition"
       ~priority:2
       ~description:"");
  let ok, msg =
    Mcp_eio.execute_tool_eio
...
let test_execute_tool_hyphenated_generated_alias_claim_next_reuses_base_token () =
  ...
  let raw_token =
    match
      Masc_mcp.Auth.create_token base_path ~agent_name:"qa-king" ~role:Types.Admin
    with
    | Ok (token, _cred) -> token
    | Error e -> Alcotest.fail (Types.masc_error_to_string e)
  in
  ignore
    (Masc_mcp.Coord.add_task
       state.room_config
       ~title:"hyphenated-generated-alias-claim-next"
       ~priority:2
       ~description:"");
  let ok, msg =
    Mcp_eio.execute_tool_eio
  ...
  Alcotest.(check bool)
    "claim_next reports claimed task"
    true
    (contains_substring msg "task-001");
  Alcotest.(check (option string))
    "current task set after claim_next"
    (Some "task-001")
    (Masc_mcp.Planning_eio.get_current_task state.room_config);
  Alcotest.(check bool)
    "explicit alias joined"
    true
    (Masc_mcp.Coord.is_agent_joined state.room_config ~agent_name:"qa-king-warm-heron");
  cleanup_dir base_path
;;
...
let test_execute_tool_claim_next_requires_auth_before_mutation () =
  ...
  ignore
    (Masc_mcp.Coord.join
       state.room_config
       ~agent_name:"uncredentialed-agent"
       ~capabilities:[]
       ());
  ignore
    (Masc_mcp.Coord.add_task
       state.room_config
       ~title:"claim-next-auth-preflight"
       ~priority:2
       ~description:"");
  let ok, msg =
    Mcp_eio.execute_tool_eio
  ...
  check_task_still_todo state.room_config "task-001";
  Alcotest.(check (option string))
    "no current task after rejected claim_next"
    None
    (Masc_mcp.Planning_eio.get_current_task state.room_config);
  cleanup_dir base_path
;;
...
let test_execute_tool_transition_requires_auth_before_mutation () =
  ...
  ignore
    (Masc_mcp.Coord.add_task
       state.room_config
       ~title:"transition-auth-preflight"
       ~priority:2
       ~description:"");
  let ok, msg =
    Mcp_eio.execute_tool_eio
  ...
  check_task_still_todo state.room_config "task-001";
  Alcotest.(check (option string))
    "no current task after rejected transition"
    None
    (Masc_mcp.Planning_eio.get_current_task state.room_config);
  cleanup_dir base_path
;;
...
let test_execute_tool_add_task_with_admin_token_without_join () =
  ...
  Alcotest.(check bool)
    "response mentions added task"
    true
    (contains_substring msg "Added task-001");
  let task =
    match Masc_mcp.Coord.get_tasks_raw state.room_config with
    | [ task ] -> task
    | tasks -> Alcotest.failf "expected exactly one task, found %d" (List.length tasks)
  in
  Alcotest.(check (option string))
    "created_by set from token owner"
    (Some "stable-admin")
    task.created_by;
  Alcotest.(check string)
    "task remains todo"
...
let test_execute_tool_http_auth_token_overrides_stale_argument_token () =
  ...
  Alcotest.(check bool)
    "does not report stale token mismatch"
    false
...
let test_execute_tool_legacy_argument_token_still_authorizes_without_http_auth () =
  ...
  let ok, msg =
    Mcp_eio.execute_tool_eio
      ...
      state
      ~name:"masc_status"
      ~arguments:(`Assoc [ "token", `String raw_token ])
  in
  Alcotest.(check bool) "status succeeds" true ok;
  Alcotest.(check bool) "status response returned" true (String.length msg > 0);
  cleanup_dir base_path
;;
...
let test_execute_tool_mcp_session_ignores_term_persistence () =
  ...
  with_env "TERM_SESSION_ID" term_sid (fun () ->
    ...
    try Unix.unlink term_file with
    | Unix.Unix_error _ -> ());
  cleanup_dir base_path
;;
      ...
      (`Assoc
          ...
          ; ( "params"
            , `Assoc
                ...
                ; ( "arguments"
                  , `Assoc
                      [ "content", `String "hello board"; "author", `String "tester" ] )
                ] )
          ])
  in
  let response = Mcp_eio.handle_request ~clock ~sw state request in
...
let test_handle_request_tools_call_logs_structured_mcp_details () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let response =
         Mcp_eio.handle_request ~clock ~sw ~mcp_session_id:"session-log" state request
       in
       ignore (result_fields_exn response);
       let entries =
         Masc_log.Ring.recent
           ~limit:20
           ~module_filter:"MCP"
           ~since_seq:baseline
           ~order:`Oldest_first
           ...
           (`Assoc
               ...
               ; ( "params"
                 , `Assoc
                     ...
                     ; "arguments", `Assoc [ "_agent_name", `String keeper_agent_name ]
                     ] )
               ])
       in
       let response = Mcp_eio.handle_request ~clock ~sw state request in
  ...
  let request =
    Yojson.Safe.to_string
      (`Assoc
          ...
          ; "id", `Int 118
          ; "method", `String "tools/call"
          ; "params", `Assoc [ "name", `String "keeper_time_now"; "arguments", `Assoc [] ]
          ])
  in
  let response = Mcp_eio.handle_request ~clock ~sw state request in
    ...
    (fun () ->
       ...
       let response =
         Mcp_eio.handle_request
           ...
           ~sw
           ~auth_token:token
           ~internal_keeper_runtime:true
           state
           request
       in
       let names = tool_names_from_list_response response in
       Alcotest.(check bool) "keeper_bash listed" true (List.mem "keeper_bash" names);
       Alcotest.(check bool)
         "system internal still hidden"
         false
    ...
    (fun () ->
       ...
       ignore
         (Keeper_registry.register
            ~base_path
            keeper_name
            (make_keeper_meta ~agent_name:keeper_agent_name ~tool_access keeper_name));
       let state = Mcp_eio.create_state ~test_mode:true ~base_path () in
       let token = Masc_mcp.Auth.ensure_internal_keeper_token base_path in
       let request =
         Yojson.Safe.to_string
       ...
       Alcotest.(check bool)
         "keeper_bash is not an MCP error"
         false
...
let test_handle_request_prompts_list_non_empty () =
  ...
  let prompts =
    ...
    | _ -> Alcotest.fail "response not an object"
  in
  Alcotest.(check bool) "prompt inventory is non-empty" true (List.length prompts >= 3);
  cleanup_dir base_path
;;
...
let test_handle_request_prompts_get_tool_help () =
  ...
  in
  Alcotest.(check bool)
    "description contains help intent"
    true
    (contains_substring description "tool");
  Alcotest.(check bool) "prompt has one or more messages" true (messages <> []);
  cleanup_dir base_path
;;
...
let test_handle_request_resources_list_includes_tool_help () =
  with_env "MASC_LIST_PAGE_SIZE" "10"
  @@ fun () ->
  Eio_main.run
  @@ fun env ->
  Fs_compat.set_fs (Eio.Stdenv.fs env);
  ...
  let tool_help_index_present =
    List.exists
      (function
        | `Assoc fields ->
          List.assoc_opt "uri" fields = Some (`String "masc://tool-help-index")
        | _ -> false)
      resources
  in
...
let test_handle_request_tools_list_paginates () =
  ...
  Alcotest.(check bool) "tools second page non-empty" true (List.length second_tools > 0);
  Alcotest.(check bool) "pages advance" true (not (String.equal first_name second_name));
  cleanup_dir base_path
;;
...
let test_handle_request_resources_read_matrix () =
  ...
  ensure_dir masc_dir;
  write_text_file
    (Filename.concat library_dir "alpha.md")
    {|---
title: Alpha Doc
...
|};
  write_text_file
    (Filename.concat masc_dir "institution.json")
    {|{
  "identity": {
    "id": "inst-1",
...
let test_handle_request_resources_subscribe_roundtrip () =
  ...
  let subscribe_response =
    Mcp_eio.handle_request
      ~clock
      ~sw
      ~mcp_session_id:"session-spec"
      state
      subscribe_request
  in
  (match subscribe_response with
   | `Assoc fields ->
     Alcotest.(check bool) "subscribe ok" true (List.mem_assoc "result" fields)
  ...
  (match unsubscribe_response with
   | `Assoc fields ->
     Alcotest.(check bool) "unsubscribe ok" true (List.mem_assoc "result" fields)
...
let eio_tests =
  ...
  ; ( "handle prompts/list rejects invalid cursor"
    , `Quick
  ...
  ; (* handle prompts/get command_truth filters run_id removed (CP purge) *)
    ( "handle resources/list includes tool-help"
    , `Quick
    , test_handle_request_resources_list_includes_tool_help )
  ; ( "handle resources/list rejects unknown field"
    , `Quick
    , test_handle_request_resources_list_rejects_unknown_field )
  ; ( "handle resources/list paginates"
    , `Quick
  ...
  ; ( "handle resources/templates/list rejects invalid cursor"
    , `Quick
    , test_handle_request_resources_templates_rejects_invalid_cursor )
  ; ( "handle resources/subscribe requires session"
    , `Quick
    , test_handle_request_resources_subscribe_requires_session )
  ; ( "handle resources/subscribe roundtrip"
    , `Quick
  ...
  ; ( "handle tools/list filters requested names"
    , `Quick
    , test_handle_request_tools_list_rejects_nonstandard_names_filter )
  ; ( "handle initialize managed profile"
    , `Quick
    , test_handle_request_initialize_managed_profile )
  ; ( "handle tools/list managed profile"
    , `Quick
    , test_handle_request_tools_list_managed_profile )
  ; ( "handle tools/list operator profile"
    , `Quick
    , test_handle_request_tools_list_operator_profile )
  ; ( "handle tools/list with placeholder flag"
    , `Quick
    , test_handle_request_tools_list_with_placeholder_flag )
  ; ( "handle tools/list include hidden metadata"
    , `Quick
    , test_handle_request_tools_list_include_hidden_metadata )
  ; ( "handle tools/list include deprecated claim alias metadata"
    , `Quick
  ...
  ; (* execution_session_turn hide test removed — team session cleanup *)
    ( "handle tools/list include usage metadata"
    , `Quick
  ...
  ; ( "explicit agent_name not overridden"
    , `Quick
    , test_execute_tool_explicit_agent_name_not_overridden )
  ; ( "explicit alias reuses joined nickname"
    , `Quick
    , test_execute_tool_explicit_alias_reuses_joined_nickname )
  ; ( "generated agent_name uses token identity"
    , `Quick
    , test_execute_tool_generated_agent_name_uses_token_identity )
  ; ( "explicit generated alias claim_next not rewritten by token"
    , `Quick
    , test_execute_tool_explicit_generated_alias_claim_next_not_rewritten_by_token )
  ; ( "explicit generated alias transition not rewritten by token"
    , `Quick
    , test_execute_tool_explicit_generated_alias_transition_not_rewritten_by_token )
  ; ( "hyphenated generated alias claim_next reuses base token"
    , `Quick
    , test_execute_tool_hyphenated_generated_alias_claim_next_reuses_base_token )
  ; ( "claim_next auth preflight blocks mutation"
    , `Quick
    , test_execute_tool_claim_next_requires_auth_before_mutation )
  ; ( "transition auth preflight blocks mutation"
    , `Quick
    , test_execute_tool_transition_requires_auth_before_mutation )
  ; ( "add_task admin token works without join"
    , `Quick
    , test_execute_tool_add_task_with_admin_token_without_join )
  ; ( "http auth token overrides stale argument token"
    , `Quick
    , test_execute_tool_http_auth_token_overrides_stale_argument_token )
  ; ( "legacy argument token still authorizes without http auth"
    , `Quick
    , test_execute_tool_legacy_argument_token_still_authorizes_without_http_auth )
  ; ( "mcp session ignores term persistence"
    , `Quick
    , test_execute_tool_mcp_session_ignores_term_persistence )
