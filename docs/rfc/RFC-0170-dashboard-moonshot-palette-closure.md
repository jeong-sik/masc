...
`SPEC.md §3.6.5 "Provider runtime (unchanged)"` and `preview/colors.html §"Provider Palette (W15)"` still document the removed tokens as live surfaces.
...
- Rewrite `SPEC.md §3.6.5` from `"Provider runtime (unchanged)"` to `"Provider runtime (removed by RFC-0168 / RFC-0170)"` with a one-paragraph rationale.
...
- Rewrite the `preview/index.html` link card description from `LLM provider tints — desaturated, muted. ... <code>--p-provider-a</code> etc.` to `Closed-roster vendor palette removed by RFC-0168 / RFC-0170. Runtime chips render with neutral styling.`
...
- `rg -i 'p-provider-b|p-provider-a|p-provider-c|p-chat-completions-v1|p-provider-e|p-provider-f|p-agent-code' dashboard/design-system/tokens*.css dashboard/design-system/ui_kits/cockpit/tokens.generated.css` returns 0 hits (token defs).
