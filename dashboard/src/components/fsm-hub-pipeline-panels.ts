...
export function OperationalMeaningPanel({
  ...
  return html`
    <div
      class=${`v2-monitoring-card rounded-[var(--r-1)] border p-4 transition-colors duration-[var(--t-slow)] ${panelCls}`}
...
export function HeroPhase({
  ...
  return html`
    <div class=${`v2-monitoring-card rounded-[var(--r-1)] border p-5 transition-[background-color,border-color,box-shadow] duration-[var(--t-xslow)] ${flash ? 'border-[var(--color-accent-fg)] bg-[var(--accent-6)] shadow-[0_0_16px_var(--accent-20)]' : 'border-[var(--color-border-default)] bg-[var(--color-bg-surface)]'}`}
...
export function PipelineStep({
  ...
  return html`
    <div class="v2-monitoring-row flex items-center gap-0 flex-1 min-w-0" role="listitem" aria-label=${`${label}: ${displayState(value)}${limited ? ' (관찰 제한)' : ''}${heldFor ? `, ${heldFor}` : ''}`}
    ...
    >
      <div class=${`v2-monitoring-card flex-1 rounded-[var(--r-1)] border px-3 py-2 transition-[background-color,border-color,opacity] duration-[var(--t-xslow)] ${borderCls} ${bgCls} ${limited && !isActive ? 'opacity-60' : ''}`}>
