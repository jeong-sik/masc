...
let help_doc_refs name =
  ...
  then [ "docs/COMMAND-PLANE-RUNBOOK.md"; "docs/BENCHMARK-RUNBOOK.md" ]
  else if String.starts_with ~prefix:"masc_keeper_" name
  then [ "docs/SUPERVISOR-MODE.md" ]
  else []
;;

let help_prompt_hints name =
  if String.equal name "masc_tool_help"
  then [ "Use prompt 'tool_help' when the caller needs a guided explanation." ]
  else if
    String.starts_with ~prefix:"masc_operation_" name
    || String.starts_with ~prefix:"masc_dispatch_" name
    || String.starts_with ~prefix:"masc_observe_" name
  then [ "Use prompt 'command_truth' for managed execution evidence." ]
  else []
;;

let default_when_to_use name =
  if String.equal name "masc_tool_help"
  then "Use when you need canonical guidance for a specific MASC tool."
  else "Use when you need this tool's canonical action."
;;
...
let manual_help_entry name =
  ...
  | "keeper_tool_search" ->
    Some
      ...
      ; when_to_use =
          "Use when your core tools are insufficient for the current task and you need \
           help identifying relevant tools by describing the capability you need."
      ; key_constraints = [ "Returns up to 10 matching results per query." ]
      ; details_markdown =
          "BM25 search over the full tool universe. Returns matching tool names, \
           descriptions, and usage guidance for discovery and selection."
      ; doc_refs = []
      ; prompt_hints = []
      }
  | "masc_tool_help" ->
    Some
      ...
      ; when_to_use =
          "Use when you need the concise description, lifecycle, and detailed guidance \
           for one tool."
      ; key_constraints = []
      ; details_markdown =
          "Returns the canonical short description, lifecycle/visibility metadata, \
           replacement info, and detailed help for a specific tool."
      ; doc_refs = [ "docs/COMMAND-PLANE-RUNBOOK.md" ]
      ; prompt_hints =
          [ "Pair with prompt 'tool_help' when you want a ready-to-use explanation." ]
      }
  | "masc_operation_start" ->
    Some
      ...
      ; when_to_use =
          "Use when you explicitly need the managed-operation compatibility lane for \
           benchmarking or topology experiments."
      ; key_constraints =
          [ "Requires assigned_unit_id and objective."
          ; "Managed operation state is later advanced through \
             checkpoint/finalize/policy tools."
          ]
      ; details_markdown =
          "Creates the managed-operation record on the experimental command-plane lane, \
           binds it to a unit, and seeds the trace/checkpoint path used by operator and \
           proof surfaces."
      ; doc_refs = [ "docs/COMMAND-PLANE-RUNBOOK.md"; "docs/BENCHMARK-RUNBOOK.md" ]
      ; prompt_hints =
          [ "Use prompt 'command_truth' to inspect resulting execution evidence." ]
      }
  | "masc_tool_admin_snapshot" ->
    Some
      { name
      ; short_description =
          "Return a unified admin snapshot covering tool inventory, auth, and \
           command-plane policy surfaces."
      ; when_to_use =
          "Use when you need one truthful view of what tools exist, what is visible, \
           what auth/RBAC applies, and which policy surfaces are enforced versus \
           advisory."
      ; key_constraints =
          [ "Tool catalog visibility metadata is read-only in this snapshot."
          ; "Command-plane tool/model allowlists now constrain unit routing and \
             assignment via tagged capabilities, but they do not yet hard-stop every \
             per-tool worker invocation."
          ]
      ; details_markdown =
          "Provides a namespace-scoped control snapshot: auth config and credentials, \
           command-plane policy topology, and the full tool inventory with metadata and \
           permission hints."
      ; doc_refs = [ "docs/COMMAND-PLANE-RUNBOOK.md"; "docs/SUPERVISOR-MODE.md" ]
      ; prompt_hints =
          [ "Use before changing auth or unit policy to confirm what is actually \
             enforced."
          ]
      }
  | "masc_tool_admin_update" ->
    Some
      ...
      ; when_to_use = "Use when you need to change auth settings."
      ; key_constraints =
          ...
          ; "Unit tool/model allowlist plumbing is tracked separately and not yet \
             exposed through this entrypoint."
          ]
      ; details_markdown =
          "Delegates to the existing truthful write paths: Config mode updates, Auth \
           config persistence, managed-operation unit policy updates, and keeper meta \
           policy updates. Command-plane unit policy now feeds routing/assignment gates; \
           deeper worker-runtime enforcement remains a separate step."
      ; doc_refs = [ "docs/COMMAND-PLANE-RUNBOOK.md"; "docs/SUPERVISOR-MODE.md" ]
      ; prompt_hints =
          [ "Run masc_tool_admin_snapshot first, then apply the smallest necessary \
             section update."
          ]
      }
  | _ -> None
...
let canonicalize_schemas schemas = List.map canonicalize_schema schemas
...
let entry_json (entry : help_entry) =
  ...
  `Assoc
    ([ "name", `String entry.name
     ...
     ; ( "key_constraints"
       , `List (List.map (fun value -> `String value) entry.key_constraints) )
...
let entry_markdown (entry : help_entry) =
  ...
  let replacement_lines =
    ...
    | None -> []
  in
  let when_lines = [ ""; "## When To Use"; ""; entry.when_to_use ] in
  let constraint_lines =
    if entry.key_constraints = []
    then []
    else
      [ ""; "## Key Constraints"; "" ]
      @ List.map (fun item -> "- " ^ item) entry.key_constraints
  in
  let detail_lines = [ ""; "## Details"; ""; entry.details_markdown ] in
  let doc_lines =
    if entry.doc_refs = []
    then []
...
let index_json (schemas : Types.tool_schema list) =
  `Assoc
    ...
    ; ( "tools"
      , `List (List.map (fun schema -> entry_json (entry_of_schema schema)) schemas) )
    ]
;;
...
let index_markdown (schemas : Types.tool_schema list) =
  ...
  String.concat
    "\n"
    ([ "# Tool Help Index"; ""; "Canonical help entries for MCP-exposed MASC tools."; "" ]
     @ rows)
;;
