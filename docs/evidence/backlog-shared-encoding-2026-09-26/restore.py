...
with tarfile.open(a.experiment/'receipts.tar.xz','r:xz') as archive:
 ...
 for member in archive.getmembers():
  ...
  if m['encoding']=='verbatim':assert hashlib.sha256(content).hexdigest()==m['original_sha256']
  dst=a.output/rel;dst.parent.mkdir(parents=True,exist_ok=True);dst.write_bytes(content)
print(f'Verified and restored {len(manifest)} files; gzip byte streams may vary by Python/zlib version, decompressed receipt bytes are exact.')
