...
let make_state ?(lower_is_better = false) ?(target_score = None) ?(baseline = 0.0) ()
  : AR.loop_state
  =
  { loop_id = "loop-test"
  ; author = None
...
let test_keep_above_target_is_pass () =
  ...
  | V.Pass -> ()
  | other -> Alcotest.fail (Printf.sprintf "expected Pass, got %s" (V.show_verdict other))
;;
...
let test_keep_below_target_is_partial () =
  ...
  | V.Partial (score, rationale) ->
    Alcotest.(check bool) "score in (0.5, 1.0)" true (score > 0.5 && score <= 1.0);
    Alcotest.(check bool)
      "rationale mentions hypothesis"
      true
      (Astring.String.is_infix ~affix:"try X" rationale)
  | other ->
    Alcotest.fail (Printf.sprintf "expected Partial, got %s" (V.show_verdict other))
;;
...
let test_lower_is_better_orientation () =
  ...
  let st = make_state ~lower_is_better:true ~baseline:1.0 ~target_score:(Some 0.5) () in
  let rec_ = make_record ~decision:AR.Keep ~score_before:1.0 ~score_after:0.7 () in
  match Br.verdict_of_cycle st rec_ with
  | V.Partial (score, _) ->
    Alcotest.(check bool) "score above baseline midpoint" true (score > 0.5)
...
let test_lower_is_better_target_met () =
  let st = make_state ~lower_is_better:true ~baseline:1.0 ~target_score:(Some 0.5) () in
  let rec_ = make_record ~decision:AR.Keep ~score_before:0.6 ~score_after:0.4 () in
  match Br.verdict_of_cycle st rec_ with
  | V.Pass -> ()
...
let test_attribution_keep_partial () =
  ...
  | A.Partial_pass { score; rationale } ->
    Alcotest.(check bool) "score normalized in (0, 1]" true (score > 0.0 && score <= 1.0);
    Alcotest.(check bool)
...
let test_attribution_discard_policy_failed () =
  ...
  let attr = Br.attribution_of_cycle st rec_ in
  Alcotest.(check bool) "origin=NonDet (even on discard)" true (attr.origin = A.NonDet);
  match attr.outcome with
...
let test_partial_has_rationale_on_outcome_not_evidence () =
  ...
  | A.Partial_pass { rationale; _ } ->
    Alcotest.(check bool) "rationale on outcome" true (String.length rationale > 0)
  | _ -> Alcotest.fail "expected Partial_pass"
;;
  ...
  Alcotest.run
    ...
    ; ( "attribution_of_cycle"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "Discard → Policy_failed"
            `Quick
            test_attribution_discard_policy_failed
        ; Alcotest.test_case
            "evidence has loop_id and cycle"
            `Quick
            test_attribution_evidence_has_loop_identity
        ; Alcotest.test_case
            "Partial_pass rationale on outcome"
            `Quick
