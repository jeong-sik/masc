...
let test_pane_commands_parse_by_word () =
  check (list string) "help, keeper, interrupt and thinking"
    ...
    (List.map
       (fun text -> describe (Command.parse text))
       [ "/help"; "/keeper haneul"; "/keeper   "; "/interrupt"; "/thinking" ])
...
let test_keeper_names_resolve_by_unique_prefix () =
  let names = [ "haneul"; "hane"; "bandleader"; "largo" ] in
  ...
  check (list string)
    ...
    [ "found:hane" (* exact, though it prefixes haneul *)
    ; "found:bandleader"
    ; "found:largo" (* unique prefix *)
    ; "ambiguous:haneul,hane" (* han- prefixes two *)
    ; "unknown"
    ]
    (List.map
       (fun typed -> describe_match (Command.resolve_keeper_name ~names typed))
       [ "hane"; "bandleader"; "la"; "han"; "zzz" ])
