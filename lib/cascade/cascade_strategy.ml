  ...
  | other ->
    Error
      (Printf.sprintf
         ...
         other
         (String.concat ", " valid_kind_strings))
;;
...
let failover =
  { kind = Failover; cycle = default_cycle_policy; tiers = []; sticky_ttl_ms = 0 }
;;
...
let weighted_shuffle adapter ctx cands =
  ...
  let rec pick acc remaining =
    ...
    | _ ->
      ...
      if total <= 0
      then List.rev_append acc (List.map fst remaining)
      else (
...
let sticky_order adapter ctx cands =
  ...
  | None -> cands
  | Some pinned ->
    (match List.find_opt (fun c -> adapter.health_key c = pinned) cands with
     | Some c -> [ c ]
     | None ->
...
let round_robin_order ctx cands =
  ...
  then cands
    ...
    let cursor = Cascade_state.rotate_round_robin ~cascade:ctx.cascade_name ~bound:n in
    let head, tail =
...
let order_candidates t ~adapter ~ctx ~cycle cands =
  match t.kind with
  | Failover -> filter_cooldown adapter ctx cands
  | Capacity_aware -> filter_capacity adapter ctx cands
  | Weighted_random -> weighted_shuffle adapter ctx cands
  | Circuit_breaker_cycling ->
  ...
  | Priority_tier -> priority_tier_order adapter ctx ~tiers:t.tiers ~cycle cands
  | Sticky -> sticky_order adapter ctx cands
  | Round_robin -> round_robin_order ctx cands
;;
