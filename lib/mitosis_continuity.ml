(** Mitosis_continuity — Pure text processing helpers for DNA quality
    validation and continuity regression checking.

    All functions are pure (no MCP/Eio dependency). Used by the mitosis
    handoff pipeline to validate DNA before handing off to a successor.
