...
채널 목록과 메시지 영역을 같은 전체 응답으로 반복 수집하지 않는다. 영역은
목록의 고정 순번이나 추정된 DOM id가 아니라 반환된 이름·역할·내용으로 고른다.
...
않는다. 채널 제목과 실제 메시지 내용이 요청한 채널인지 확인한 뒤 메시지 영역을
선택한다. 전환 중인 화면은 미확인으로 남기고 관측만 갱신하며 클릭을 반복하지 않는다.
목적지 URL 검사가 거절되면 같은 탭을 expectedUrl 없이 읽어 현재 URL과 내용을
확인한다. 원래 URL에 남아 있는 이동 대기와 다른 URL로 바뀐 리다이렉트를 구분한다.
로그인·다른 workspace 화면은 요청 채널 도착으로 받아들이지 않는다. 관측한
workspace·채널 제목·메시지가 요청 대상임을 확인한 뒤 현재 URL로 읽기를 고정한다.
리다이렉트된 화면에 이전 destinationUrl을 계속 요구하거나 링크를 다시 누르지 않는다.

스크린샷을 보고 메시지 목록을 더 읽을 때는 지원되는 `scroll_at`으로 목록 안의
점을 지정한다. `window` 스크롤이 움직이지 않는다고 같은 요청을 반복하지 않는다.
스크롤 뒤 갱신된 화면과 영역을 읽고, 가상 목록에서 사라진 노드는 새로 관측한다.
