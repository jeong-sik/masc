...
let test_belief_capped () =
  ...
  Alcotest.(check bool) "belief near cap" true (String.length c.belief_summary <= 103);
  Alcotest.(check bool) "ellipsis appended" true (String.length c.belief_summary > 100)
;;
...
let test_option_fields_capped () =
  ...
  let check_opt label = function
    | Some v -> Alcotest.(check bool) (label ^ " near cap") true (String.length v <= 53)
...
let test_idempotent () =
  let s = { base with belief_summary = long 1000; blocker = Some (long 1000) } in
...
let test_speech_and_surface_pass_through () =
  ...
  Alcotest.(check string)
    "speech_act preserved"
    "request_help"
    (T.speech_act_to_string c.speech_act);
  Alcotest.(check string)
    "delivery_surface preserved"
    "board_post"
    (T.delivery_surface_to_string c.delivery_surface)
;;
  ...
  Alcotest.run
    ...
    [ ( "cap_social_state"
      , [ Alcotest.test_case "short state unchanged" `Quick test_short_unchanged
        ...
        ; Alcotest.test_case "option fields capped" `Quick test_option_fields_capped
