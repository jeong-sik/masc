...
이 RFC 는 그 결함을 고치는 순서와, 그 위에 올릴 세 가지를 정한다. 언어 서버를 서버가 띄울 수 있는 모든 언어로 넓히되 언어와 명령의 대응을 닫힌 합타입과 운영자 설정으로 둔다. Mermaid 블록을 TUI 안에서 그린다. 첫 단계는 OCaml 로 짠 텍스트 렌더러이고, 픽셀 경로(Kitty graphics)는 렌더러가 설정된 경우에만 여는 다음 단계다. keeper 의 줄 메모는 파일 안 주석으로 쓴다 (§3.5).
...
### 3.0 S0 — 요청 stall 을 잰 뒤 고친다 (선행, 차단 결함)

계측: `Masc_tui_http.http_get` 에 요청마다 `Unix.gettimeofday` 와 `Eio.Time.now` 두 시계로 네 시각을 적는다. 요청을 만든 때, pool 에서 client 를 받은 때, 첫 바이트를 읽은 때, 끝난 때. TUI 로그 파일 (`masc-tui-<pid>.log`) 에 한 줄씩 남긴다. 두 시계의 차이가 벌어지면 fiber 가 순서를 못 받은 것이고, 벽시계는 흐르는데 pool 이후 시각이 없으면 client 를 못 받은 것이고, 첫 바이트가 없으면 소켓이 죽은 것이다.

재현: tmux 220×50 에서 Code 화면을 열고 `K` → 후보 선택. 서버 로그의 `LSP pool started` 시각과 TUI 로그의 네 시각을 나란히 적는다. 세 번 반복.

고침은 계측 결과가 정한다. 두 후보 모두 pool (`lib/masc_http_client/pool.ml`) 또는 TUI 메인 루프의 yield 위치에 있다. 이 PR 은 계측만 넣고, 고침은 결과와 함께 두 번째 PR 로 간다.
...
메모의 원본은 파일 자체다. `keeper_ide_annotate` 가 `masc(작성자) 종류: 본문` 한 줄을 지정한 줄 위에 그 파일의 주석 문법으로 넣고, TUI 의 `m` 이 같은 문법으로 읽어 목록과 여백 표시를 만든다. 메모를 쓰는 일이 곧 파일 변경이므로 diff 와 PR 에 실리고 다른 checkout 에도 따라간다. 쓰는 쪽과 읽는 쪽이 표 하나(`Lsp_process_manager.memo_markers_of_path`)를 같이 본다.

읽는 쪽은 렉서가 있는 파일에서 렉서가 주석이라고 표시한 행만 본다. 한 줄은 두 줄 위에서 열린 문자열을 모르고, 렉서는 안다. 렉서가 없는 언어(Lua, Markdown)에서만 파일의 주석 표지가 판정한다.

종류는 `Ide_memo.word_of_kind` 한 표에서 온다. 도구 스키마의 enum, 쓰는 쪽의 오류 문구, 읽는 쪽의 해석이 모두 그 표에서 나온다.

## 4. 검증

| 단계 | 통과 조건 |
|---|---|
| S0 | 세 번 재현에서 네 시각이 모두 로그에 있고, 어느 구간이 10초를 먹는지 한 문장으로 말할 수 있다 |
