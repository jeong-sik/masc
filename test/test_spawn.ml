...
let test_spawn_bare_ollama_rejected () =
  if Provider_adapter.is_bare_ollama_label "ollama"
  then (
...
let test_spawn_success_ignores_stderr_noise () =
  ...
  Alcotest.(check bool) "stdout preserved" true (contains result.Spawn.output "stdout-ok");
  Alcotest.(check bool)
    "stderr omitted on success"
    false
...
let tests =
  ...
  ; Alcotest.test_case
      "spawn failure surfaces stderr"
      `Quick
      test_spawn_failure_surfaces_stderr
  ; Alcotest.test_case
      "spawn failure falls back when silent"
      `Quick
      test_spawn_failure_falls_back_when_silent
  ; Alcotest.test_case
      "spawn success ignores stderr noise"
      `Quick
      test_spawn_success_ignores_stderr_noise
  ]
;;

let () = Alcotest.run "Spawn" [ "Config", tests ]
