...
let test_openapi_route_serves_document () =
  ...
  let rec fetch_openapi retries =
    ...
    | None ->
      fail (Printf.sprintf "openapi field missing after retries\nbody=%s" res.body)
  in
  let json = fetch_openapi 5 in
  let open Yojson.Safe.Util in
  check string "openapi version" "3.1.0" (json |> member "openapi" |> to_string);
  check bool "/mcp path present" true (json |> member "paths" |> member "/mcp" <> `Null);
  let operations =
    json
    |> member "paths"
    |> member "/mcp"
    |> member "post"
    |> member "x-mcp-operations"
    |> to_list
  in
  check
    bool
    "masc_status exported"
    true
    (List.exists
...
let () =
  run
    "openapi_api_e2e"
    [ ( "openapi"
      , [ test_case "route serves document" `Slow test_openapi_route_serves_document ] )
    ]
;;
