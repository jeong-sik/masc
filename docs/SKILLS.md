...
발행은 운영자가 한다. 관리자 편집기의 `/api/v1/skills/editor/create`·`/preview`·`/save`는
...
Skill reference의 package와 다르면 거절한다. 쓰기 감사 기록에는 artifact sha256이 남고, 읽지 못한 draft도 `draft_refused`로 남는다. TUI와 대시보드의 생성·저장 화면은
