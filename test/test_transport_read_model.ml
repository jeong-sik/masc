    ...
    `Assoc
      (fields
       |> List.filter_map (fun (key, value) ->
         if String.equal key "configured" then None else Some (key, strip_configured value))
      )
...
let test_websocket_discovery_http_shape_extends_tool_shape () =
  ...
  check
    bool
    "tool and http surfaces match after stripping configured"
    true
    (tool_json = strip_configured http_json)
;;

let test_transport_status_http_shape_extends_tool_shape () =
  let tool_json = TRM.transport_status_json (make_context ()) in
  let http_json = TRM.transport_status_json (make_context ~include_configured:true ()) in
  check
    bool
    "http grpc surface adds configured"
    true
    (match Yojson.Safe.Util.(http_json |> member "grpc" |> member "configured") with
     | `Bool _ -> true
     | _ -> false);
  check
    bool
    "http webrtc surface adds configured"
    true
    (match Yojson.Safe.Util.(http_json |> member "webrtc" |> member "configured") with
     | `Bool _ -> true
     | _ -> false);
  check
    bool
    "http surface exposes streamable configured"
    true
    (match Yojson.Safe.Util.(http_json |> member "http" |> member "configured") with
     | `Bool _ -> true
     | _ -> false);
  check
    bool
    "http surface exposes streamable protocol capability"
    true
    (match Yojson.Safe.Util.(http_json |> member "http" |> member "protocol_capable") with
     | `Bool _ -> true
     | _ -> false);
  check
    bool
    "http surface exposes auth policy dimension"
    true
    (match
  ...
  check
    bool
    "grpc surface exposes reachability"
    true
    (match Yojson.Safe.Util.(http_json |> member "grpc" |> member "reachable") with
     | `Bool _ -> true
     | _ -> false);
  check
    bool
    "websocket surface exposes reachability"
    true
    (match Yojson.Safe.Util.(http_json |> member "websocket" |> member "reachable") with
     | `Bool _ -> true
     | _ -> false);
  check
    bool
    "webrtc surface exposes signaling availability"
    true
    (match
  ...
  check
    bool
    "tool and http transport status match after stripping configured"
    true
    (tool_json = strip_configured http_json)
;;
...
let test_transport_status_reports_streamable_http_protocol () =
  ...
  check
    bool
    "enabled_protocols includes canonical json-rpc path"
    true
  ...
  with_env "MASC_HTTP_BASE_URL" None (fun () ->
    with_env "MASC_HOST" (Some "0.0.0.0") (fun () ->
      with_env "MASC_HTTP_PORT" (Some "8935") (fun () ->
        ...
        check string "default base_url" "http://127.0.0.1:8935" ctx.base_url)))
;;
...
let test_context_from_env_normalizes_loopback_alias_base_url () =
  with_env "MASC_HTTP_BASE_URL" (Some "http://localhost:8935/root/") (fun () ->
    ...
    check
      string
      "loopback alias base_url canonicalized"
      "http://127.0.0.1:8935/root"
      ctx.base_url)
;;
  ...
  run
    ...
    ; ( "env"
      , [ test_case
        ...
        ; test_case
            "trim explicit base url"
            `Quick
            test_context_from_env_trims_explicit_base_url
        ; test_case
            "normalize loopback alias base url"
            `Quick
        ...
        ; test_case
            "preserve noncanonical 127 alias"
            `Quick
