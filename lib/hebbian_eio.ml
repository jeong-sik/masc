...
let default_params () =
  ...
  ; weaken_rate = Level2_config.Hebbian.learning_rate ()
  ; (* Symmetric *)
...
let synapses_file (config : config) =
  Filename.concat
    (Common.masc_dir_from_base_path ~base_path:config.Coord_utils.base_path)
    "synapses/graph.json"
;;

(** Get lock file path *)
let synapses_lock_file (config : config) = synapses_file config ^ ".lock"
...
let get_lock_stats () =
  let avg_wait =
    if Atomic.get lock_acquisitions = 0
    then 0.0
    else Atomic.get lock_total_wait_ms /. float_of_int (Atomic.get lock_acquisitions)
  in
...
let with_graph_lock config f =
  ...
  let fd =
    run_blocking_op (fun () ->
      File_lock_eio.acquire_flock_retry
        ~lock_path:lock_file
        ~mode:[ Unix.O_WRONLY; Unix.O_CREAT ]
        ~perm:0o600
        ~caller:"hebbian_eio"
        ())
  in
  ...
  then
    Log.Misc.warn
      "Lock contention detected: %.1fms wait (threshold: %.0fms)"
      wait_ms
      warn_threshold;
  (* Run body in Eio context, release lock in systhread *)
  Common.protect
    ~module_name:"hebbian_eio"
...
let graph_of_json json : synapse_graph =
  ...
  try
    let synapses =
      json |> member "synapses" |> to_list |> List.filter_map synapse_of_json
    in
...
let load_graph config : synapse_graph =
  ...
  then { synapses = []; last_consolidation = 0.0 }
  else (
    try
...
let find_synapse graph ~from_agent ~to_agent : synapse option =
  List.find_opt
    (fun s -> s.from_agent = from_agent && s.to_agent = to_agent)
    graph.synapses
;;
...
let weaken config ?params ~from_agent ~to_agent () : unit =
  ...
  with_graph_lock config (fun () ->
    ...
    | Some synapse ->
      ...
      save_graph config new_graph;
      outcome := "weakened");
  Prometheus.inc_counter edge_update_outcome_metric ~labels:[ "outcome", !outcome ] ()
;;
...
let consolidate config ?params ~decay_after_days () : int =
  ...
  with_graph_lock config (fun () ->
    ...
    let decayed, pruned_count =
      List.fold_left
        (fun (acc, count) synapse ->
           if synapse.last_updated < cutoff
           then (
             (* Apply decay *)
             ...
             if new_weight < params.min_weight
             then
               (* Prune weak synapse *)
               acc, count + 1
             else (
               let updated =
                 ...
                 ; last_updated = now
                 ; weight_history =
                     append_history ~ts:now ~w:new_weight synapse.weight_history
                 }
               in
               updated :: acc, count))
           else synapse :: acc, count)
        ([], 0)
        graph.synapses
    in
...
let run_consolidation_once config ~decay_after_days =
  ...
  try
    ...
    Prometheus.inc_counter "masc_hebbian_consolidate_total" ~labels:[ "outcome", "ok" ] ();
    if pruned > 0
    then
      Prometheus.inc_counter
        ...
        ();
    Prometheus.observe_histogram "masc_hebbian_consolidate_duration_seconds" elapsed;
    if pruned > 0
    then
      Log.Coord.info
        ...
        pruned
        elapsed
    else Log.Coord.debug "hebbian: consolidated graph, pruned=0 duration=%.3fs" elapsed
  with
  | Eio.Cancel.Cancelled _ as e -> raise e
  | exn ->
    Prometheus.inc_counter
      ...
      ();
    Log.Coord.error "hebbian: consolidation iteration failed: %s" (Printexc.to_string exn)
;;

let start_consolidation_fiber ~sw ~clock config =
  let interval_s = Level2_config.Hebbian.consolidation_interval_s () in
  let decay_after_days = int_of_float (Level2_config.Hebbian.decay_after_days ()) in
  Log.Coord.info
