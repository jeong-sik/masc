...
let first_match_for_hash ~base ~token_hash =
  ...
  |> List.find (fun (cred : Types.agent_credential) -> String.equal cred.token token_hash)
...
let test_single_match_no_counter () =
  with_temp_base (fun base ->
    ...
    let _ = Masc_mcp.Auth.find_credential_by_token base ~token:raw_token in
...
let test_duplicate_hash_increments_counter () =
  with_temp_base (fun base ->
    ...
    let result = Masc_mcp.Auth.find_credential_by_token base ~token:raw_token in
    (match result with
     | Ok cred ->
       Alcotest.(check string)
         ...
         first_match
         cred.agent_name
     | Error _ -> Alcotest.fail "expected Ok on ambiguous lookup");
    Alcotest.(check (float 0.0001))
