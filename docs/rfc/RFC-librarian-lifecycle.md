...
1. **턴 끝 기록** `<keepers_dir>/<keeper>.turn-boundaries.jsonl`
   ...
   - checkpoint 를 못 읽은 채 시작한 턴은 시작할 때 이 줄을 쓰지 않는다. 읽기가 실패하면(버전 컷, 읽기 오류, 파싱 오류) 턴은 빈 이력으로 시작하지만, 저장된 이력을 본 것이 아니고 디스크의 이력은 그대로일 수 있다. 그때 줄을 쓰면 재시작보다 앞선 줄이 된다: 읽는 쪽이 그 줄을 보고 옛 이력을 0 부터 다시 읽으며 줄을 지나가고, 그 뒤에 그 턴의 저장이 받아들여져 이력을 갈아 끼운 채 턴이 죽으면 새 이력에는 지나가지 않은 재시작 줄이 없다. 그래서 읽기의 결과를 셋으로 가른다(`keeper_context_core.ml` 의 `load_context_from_checkpoint_classified`: 읽음, 없음, 못 읽음). 못 읽은 턴은 턴 도중 저장(`keeper_agent_run.ml` 의 `checkpoint_sink`)이 처음 받아들여진 바로 뒤에 줄을 쓴다. 그 저장이 이력을 갈아 끼우는 저장이다(`keeper_checkpoint_store.ml` 은 `turn_count` 가 작을 때만 stale 로 본다). 처음 받아들여진 저장이 끝의 저장이면 그 턴의 `Fresh_history` 끝 줄이 같은 일을 한다. 받아들여진 저장이 하나도 없으면 이력은 그대로이고 줄도 없다.
   ...
   - **확인 필요.** 공식 클라이언트 레인(Codex app server, Antigravity CLI, Claude Code)에서 `checkpoint_sink` 가 도는지는 아직 확인하지 않았다. 안 돈다면 (다)의 자리가 없어서 그 레인의 못 읽는 턴은 어떤 경로로도 재시작 줄을 쓰지 않는다. 위 문단대로 갈아 끼우는 일이 없으니 지금은 문제가 아니지만, 4단계의 루프가 "재시작 줄이 온다"를 가정하면 안 되는 이유가 된다. §10 의 3 과 같이 본다.
...
| 1d | checkpoint 읽기의 결과를 읽음·없음·못 읽음으로 갈라 `run_turn` 까지 가져온다. 못 읽은 채 시작한 턴은 시작할 때 줄을 쓰지 않고, 처음 받아들여진 턴 도중 저장 바로 뒤에 재시작 줄을 쓴다(§4.6). 줄 종류의 이름은 읽는 쪽의 말로 정한다: `history_restarted` | 읽는 곳이 없다. 4단계 전에 있어야 한다 |
