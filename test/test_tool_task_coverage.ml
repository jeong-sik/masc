...
let () =
  test "task_history_events_json_filters_by_task_id" (fun () ->
    let ctx = make_test_ctx () in
    ...
    let json =
      Tool_task.task_history_events_json ctx.config ~task_id:"task-001" ~limit:20
    in
    let events =
      match json with
      | `List rows -> rows
...
let () =
  test "task_history_events_json_returns_empty_for_missing_task" (fun () ->
    let ctx = make_test_ctx () in
    let json =
      Tool_task.task_history_events_json ctx.config ~task_id:"task-404" ~limit:20
    in
    match json with
    | `List [] -> ()
...
let () =
  test "masc_oas_bridge_runs_without_eio_env" (fun () ->
    match Masc_eio_env.get_opt () with
    | Some _ ->
      failwith
        "masc_oas_bridge_runs_without_eio_env requires Masc_eio_env.get_opt () = None \
         before calling run_safe"
    | None ->
...
let () =
  test "dispatch_transition_claim" (fun () ->
    let ctx = make_test_ctx () in
    (* First add a task *)
...
let () =
  test "handle_done_records_calibration_verdict" (fun () ->
    let ctx = make_test_ctx () in
    (* Setup: add task, claim it *)
    let _ =
      Tool_task.handle_add_task ctx (`Assoc [ "title", `String "Calibration test task" ])
    in
    ...
    let success, result =
      Tool_task.handle_done
        ctx
        (`Assoc [ "task_id", `String "task-001"; "notes", `String "x" ])
    in
    assert (not success);
...
let () =
  test "handle_done_records_approved_calibration_verdict" (fun () ->
    let ctx = make_test_ctx () in
    let _ =
      Tool_task.handle_add_task
        ctx
        (`Assoc [ "title", `String "Approved calibration task" ])
    in
    let _ = Tool_task.handle_claim ctx (`Assoc [ "task_id", `String "task-001" ]) in
    ...
    let success, result =
      Tool_task.handle_done
        ...
        (`Assoc
            ...
            ; ( "notes"
              , `String
                  "Implemented the calibration coverage path, verified the JSONL verdict \
                   store, and completed the task cleanly." )
            ])
    in
    if not success then failwith result;
    let store = Eval_calibration.get_store () in
...
let () =
  test
    "handle_transition_respects_completion_contract_and_records_custom_evaluator"
    (fun () ->
     ...
     test_verification_fsm.ml. *)
       with_env "MASC_VERIFICATION_FSM_ENABLED" (Some "false") (fun () ->
         ...
         let _ =
           Tool_task.handle_add_task
             ctx
             (`Assoc [ "title", `String "Contract calibration task" ])
         in
         let _ = Tool_task.handle_claim ctx (`Assoc [ "task_id", `String "task-001" ]) in
...
let () =
  test "handle_add_task_persists_contract" (fun () ->
    let ctx = make_test_ctx () in
    let success, result =
      Tool_task.handle_add_task
        ctx
...
let () =
  test "handle_add_task_injects_default_verification_contract" (fun () ->
    let ctx = make_test_ctx () in
    let success, result =
      Tool_task.handle_add_task
        ctx
    ...
    | [ task ] ->
      (match task.contract with
       | Some contract ->
         ...
         assert (
           str_contains (List.hd contract.completion_contract) "Default verification task")
       | None -> failwith "expected default verification contract")
    | _ -> failwith "expected exactly one task")
;;

let () =
  test "handle_batch_add_tasks_injects_default_verification_contracts" (fun () ->
    let ctx = make_test_ctx () in
    let success, result =
      Tool_task.handle_batch_add_tasks
        ctx
        (`Assoc
            [ ( "tasks"
              , `List
...
let () =
  test "handle_done_uses_persisted_contract_gate" (fun () ->
    ...
    let success_done, result_done =
      Tool_task.handle_done
        ctx
        (`Assoc [ "task_id", `String "task-001"; "notes", `String "deliverable-ready" ])
    in
    if success_done
    then failwith "expected gate to reject handle_done for strict task without verdict";
    if not (str_contains result_done "CDAL verdict")
    then
      failwith
...
(* Advisory contract (strict=false): CDAL gate must still record an attribution
   ...
   no audit trail. *)
let () =
  test "handle_done_advisory_contract_records_attribution" (fun () ->
    ...
    in
    if not success_done
    then failwith "advisory contract (strict=false) must not block handle_done";
    ...
    let advisory_recent =
      Dashboard_attribution.recent
        ~gate:Cdal_verdict_gate.advisory_gate_label
        ~limit:20
        ()
    in
    if advisory_recent = []
    then
      failwith
        "expected Dashboard_attribution to record a cdal_verdict_advisory entry for \
         strict=false contract (audit trail regression)";
    let strict_recent =
      Dashboard_attribution.recent ~gate:Cdal_verdict_gate.strict_gate_label ~limit:20 ()
    in
    if strict_recent <> []
    then failwith "advisory recording must not leak into the strict cdal_verdict bucket")
;;

let () =
  test "handle_transition_release_requires_handoff_for_strict_task" (fun () ->
    let ctx = make_test_ctx () in
    let _ =
      Tool_task.handle_add_task
        ctx
        ...
        (`Assoc
            ...
            ; ( "handoff_context"
              , `Assoc
                  ...
                  ; "evidence_refs", `List [ `String "task-001"; `String "session:test" ]
                  ] )
            ])
    in
...
let () =
  test "handle_transition_start_on_todo_points_at_claim_first" (fun () ->
    ...
    let ctx = make_test_ctx () in
    let _ =
      Tool_task.handle_add_task ctx (`Assoc [ "title", `String "Start-without-claim" ])
    in
    let success, result =
      Tool_task.handle_transition
        ctx
...
let () =
  test "handle_transition_release_by_nonowner_redirects_to_board_post" (fun () ->
    ...
    let _ =
      Tool_task.handle_add_task ctx_owner (`Assoc [ "title", `String "Owned-by-other" ])
    in
    let _ = Tool_task.handle_claim ctx_owner (`Assoc [ "task_id", `String "task-001" ]) in
    (* A separate context for a different agent against the SAME config,
     so the backlog/task state is shared. *)
    let ctx_other = { ctx_owner with Tool_task.agent_name = "other-agent" } in
    let success, result =
      Tool_task.handle_transition
        ctx_other
        (`Assoc [ "task_id", `String "task-001"; "action", `String "release" ])
...
let () =
  test "handle_transition_release_synthesizes_summary_from_notes" (fun () ->
    ...
    | [ task ] ->
      (match task.handoff_context with
       | Some handoff_context -> assert (handoff_context.summary = synthesized_note)
       | None -> failwith "expected persisted handoff_context")
    | _ -> failwith "expected exactly one task")
;;

let () =
  test "handle_transition_release_prefers_notes_then_reason_for_synthesis" (fun () ->
    ...
    | [ task ] ->
      (match task.handoff_context with
       | Some handoff_context -> assert (handoff_context.summary = notes_line)
       | None -> failwith "expected persisted handoff_context")
    | _ -> failwith "expected exactly one task")
;;

let () =
  test "handle_transition_release_empty_summary_error_includes_example" (fun () ->
    let ctx = make_test_ctx () in
    let _ =
      Tool_task.handle_add_task
        ctx
    ...
    let success_empty, result_empty =
      Tool_task.handle_transition
        ...
        (`Assoc
            ...
            ; ( "handoff_context"
              , `Assoc
                  [ "summary", `String "   "; "next_step", `String "re-check fixture" ] )
            ])
    in
...
let () =
  test "handle_transition_done_prefers_ownership_error_over_cdal_gate" (fun () ->
    let ctx = make_test_ctx () in
    let _ =
      Tool_task.handle_add_task
        ctx
...
let () =
  test "handle_transition_done_on_awaiting_verification_is_explicit" (fun () ->
...
let () =
  test "handle_claim_sets_planning_current_task" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "handle_claim_blocks_required_tools_without_server_surface" (fun () ->
    let ctx = make_test_ctx () in
    add_task_requiring_tools ctx ~title:"Needs bash" [ "keeper_bash" ];
    let success, result =
      Tool_task.handle_claim
        ~agent_tool_names:[ "masc_status" ]
        ctx
        (`Assoc
            [ "task_id", `String "task-001"
...
let () =
  test "handle_claim_allows_required_tools_with_server_surface" (fun () ->
    let ctx = make_test_ctx () in
    add_task_requiring_tools ctx ~title:"Needs bash" [ "keeper_bash" ];
    let success, result =
      Tool_task.handle_claim
...
let () =
  test "handle_add_task_rejects_removed_required_preset_argument" (fun () ->
    let agent_name = "test-agent" in
    let ctx = make_test_ctx_with_agent agent_name in
    let success, result =
      Tool_task.handle_add_task
        ctx
        (`Assoc [ "title", `String "Needs social"; "required_preset", `String "social" ])
    in
    assert (not success);
...
let () =
  test "add_task_schema_omits_removed_required_preset_argument" (fun () ->
    let schema =
      match
        List.find_opt
          (fun (schema : Types.tool_schema) -> String.equal schema.name "masc_add_task")
          Tool_task_schemas.schemas
      with
      | Some schema -> schema
      | None -> failwith "masc_add_task schema not found"
    in
    let properties = Yojson.Safe.Util.(schema.input_schema |> member "properties") in
    assert (Yojson.Safe.Util.member "required_preset" properties = `Null))
;;

let () =
  test "handle_claim_rejects_removed_agent_role_argument" (fun () ->
    let ctx = make_test_ctx () in
    let _ =
    ...
    let success, result =
      Tool_task.handle_claim
        ctx
        (`Assoc [ "task_id", `String "task-001"; "agent_role", `String "worker" ])
    in
    assert (not success);
...
let () =
  test "handle_claim_next_sets_planning_current_task" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "handle_claim_next_blocks_required_tools_without_server_surface" (fun () ->
    ...
    add_task_requiring_tools ctx ~title:"Needs bash" [ "keeper_bash" ];
    let success, result =
      Tool_task.handle_claim_next ~agent_tool_names:[ "masc_status" ] ctx (`Assoc [])
    in
    assert success;
...
let () =
  test "handle_claim_next_ignores_keeper_preset_for_open_claims" (fun () ->
      ...
      match
        Keeper_types.meta_of_json
          (`Assoc
              ...
              ; ( "tool_access"
                , `Assoc [ "kind", `String "preset"; "preset", `String "social" ] )
              ])
      with
      | Ok meta -> meta
    ...
    let _ =
      Tool_task.handle_add_task ctx (`Assoc [ "title", `String "Open claim task" ])
    in
...
let () =
  test "transition_claim_sets_planning_current_task" (fun () ->
    let ctx = make_test_ctx () in
    let _ =
      Tool_task.handle_add_task ctx (`Assoc [ "title", `String "Transition claim" ])
    in
    let success, _result =
      Tool_task.handle_transition
        ctx
...
let () =
  test "transition_claim_blocks_required_tools_even_with_force" (fun () ->
    let ctx = make_test_ctx () in
    add_task_requiring_tools ctx ~title:"Needs bash" [ "keeper_bash" ];
    let success, result =
      Tool_task.handle_transition
...
let () =
  test "dispatch_claim_task_uses_server_surface_not_payload_surface" (fun () ->
    let ctx = make_test_ctx () in
    add_task_requiring_tools ctx ~title:"Needs bash" [ "keeper_bash" ];
    match
      Tool_task.dispatch
...
let () =
  test "transition_release_clears_planning_current_task" (fun () ->
    let ctx = make_test_ctx () in
    let _ =
      Tool_task.handle_add_task ctx (`Assoc [ "title", `String "Transition release" ])
    in
...
let () =
  test
    "transition_done_redirects_to_verification_and_keeps_planning_current_task"
    (fun () ->
       let ctx = make_test_ctx () in
       let _ =
         Tool_task.handle_add_task ctx (`Assoc [ "title", `String "Transition done" ])
       in
       let success_claim, _result =
         Tool_task.handle_transition
           ctx
       ...
       let success_done, result =
         Tool_task.handle_transition
           ...
           (`Assoc
               ...
               ; ( "notes"
                 , `String
                     "Implemented the transport parity checks and verified the result." )
               ])
       in
       assert success_done;
...
let () =
  test "transition_accepts_underscore_prefixed_internal_markers" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "transition_still_rejects_plain_unknown_arguments" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "handle_done_owned_by_other_guidance" (fun () ->
    let ctx = make_test_ctx () in
    ...
    let success, result =
      Tool_task.handle_done
        ctx
        (`Assoc [ "task_id", `String "task-001"; "notes", `String "" ])
    in
    assert (not success);
    assert (str_contains result "currently owned by other-agent"))
;;
...
let () =
  test "handle_done_todo_guidance" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "handle_done_already_done_guidance" (fun () ->
    let ctx = make_test_ctx () in
    let _ = Tool_task.handle_add_task ctx (`Assoc [ "title", `String "Done test" ]) in
    let _ = Coord.claim_task ctx.config ~agent_name:"other-agent" ~task_id:"task-001" in
    let _ =
...
let () =
  test "handle_done_cancelled_guidance" (fun () ->
    let ctx = make_test_ctx () in
    let _ =
      Tool_task.handle_add_task ctx (`Assoc [ "title", `String "Cancelled test" ])
    in
    let _ =
      Coord.cancel_task_r
        ctx.config
...
let () =
  test "dispatch_transition_release" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "dispatch_update_priority" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "dispatch_task_history" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "handle_batch_add_tasks" (fun () ->
    let ctx = make_test_ctx () in
    let args =
      `Assoc
        [ ( "tasks"
...
let () =
  test "handle_batch_add_tasks_rejects_removed_role_fields" (fun () ->
    let ctx = make_test_ctx () in
    let args =
      `Assoc
        [ ( "tasks"
          , `List
              [ `Assoc [ "title", `String "Task 1"; "required_role", `String "writer" ] ]
          )
        ]
    in
...
let () =
  test "get_int_opt_present" (fun () ->
    let args = `Assoc [ "key", `Int 42 ] in
    assert (Tool_args.get_int_opt args "key" = Some 42))
;;

let () =
  test "get_int_opt_missing" (fun () ->
    let args = `Assoc [] in
...
let () =
  test "build_verdict_sse_payload: distinct cascades = cross_model true" (fun () ->
    ...
    in
    let json =
      Tool_task.build_verdict_sse_payload ~now:1234567890.0 ~task_id:"t1" ~req ~result
    in
    assert (payload_member "cross_model" json = `Bool true);
    assert (
      payload_member "generator_cascade" json
...
let () =
  test "build_verdict_sse_payload: same cascade = cross_model false" (fun () ->
    let req = make_review_request () in
    let result =
      make_review_result ~evaluator_cascade:"verifier" ~generator_cascade:"verifier" ()
    in
    let json =
      Tool_task.build_verdict_sse_payload ~now:1234567890.0 ~task_id:"t2" ~req ~result
    in
...
let () =
  test "build_verdict_sse_payload: no generator = cross_model false + null" (fun () ->
    let req = make_review_request () in
    let result = make_review_result ~evaluator_cascade:"verifier" () in
    let json =
      Tool_task.build_verdict_sse_payload ~now:1234567890.0 ~task_id:"t3" ~req ~result
    in
    assert (payload_member "cross_model" json = `Bool false);
...
let () =
  test "build_verdict_sse_payload: empty generator string = cross_model false" (fun () ->
    ...
    let req = make_review_request () in
    let result =
      make_review_result ~evaluator_cascade:"verifier" ~generator_cascade:"" ()
    in
    let json =
      Tool_task.build_verdict_sse_payload ~now:1234567890.0 ~task_id:"t4" ~req ~result
    in
    assert (payload_member "cross_model" json = `Bool false);
...
let () =
  test "build_verdict_sse_payload: empty evaluator string = cross_model false" (fun () ->
    let req = make_review_request () in
    let result =
      make_review_result
        ~evaluator_cascade:""
    ...
    in
    let json =
      Tool_task.build_verdict_sse_payload ~now:1234567890.0 ~task_id:"t5" ~req ~result
    in
    assert (payload_member "cross_model" json = `Bool false))
;;

let () =
  test "build_verdict_sse_payload: fallback_reason serialized" (fun () ->
    let req = make_review_request () in
    let result =
      make_review_result
    ...
    in
    let json =
      Tool_task.build_verdict_sse_payload ~now:1234567890.0 ~task_id:"t6" ~req ~result
    in
...
let () =
  test "claim_next_returns_no_unclaimed_when_all_tasks_terminal" (fun () ->
    let ctx = make_test_ctx () in
...
let () =
  test "claim_next_filters_out_cancelled_tasks" (fun () ->
    let ctx = make_test_ctx () in
    let _ =
      Tool_task.handle_add_task ctx (`Assoc [ "title", `String "Cancelled task" ])
    in
    let _ =
      Coord.cancel_task_r
        ctx.config
