...
let test_invalid_oas_timeout_env_does_not_create_override () =
  with_clean_boot_overrides @@ fun () ->
  with_env "MASC_KEEPER_OAS_TIMEOUT_SEC" (Some "not-a-float") @@ fun () ->
  Keeper_runtime_resolved.init ();
  let runtime = Keeper_runtime_resolved.current () in
  check (option (float 0.0001)) "invalid oas timeout disables override"
    None runtime.oas_timeout_override_sec.value;
  check (float 0.0001) "resolved call timeout falls back to turn timeout"
    runtime.turn_timeout_sec.value
    (Keeper_runtime_resolved.oas_call_timeout_sec ())
  ...
  run "runtime_toml_overrides"
    [ ( "resolve_overrides"
      , [ test_case "missing file returns 0 overrides" `Quick test_missing_file_returns_zero
        ...
        ; test_case "invalid oas timeout env disables override" `Quick test_invalid_oas_timeout_env_does_not_create_override
