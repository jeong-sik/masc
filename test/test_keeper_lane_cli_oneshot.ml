...
let test_walk_advances_and_keeps_every_failure_in_order () =
  with_runtime (fun () ->
    let runner ~runtime_id ~system_prompt:_ ~output_schema:_ ~prompt:_ =
      ...
      else Error (Masc.Fusion_official_client.Setup_failure (Masc.Fusion_types.Provider_error "spawn failed"))
    in
...
let test_walk_exhaustion_returns_every_failure () =
  with_runtime (fun () ->
    let runner ~runtime_id:_ ~system_prompt:_ ~output_schema:_ ~prompt:_ = Error (Masc.Fusion_official_client.Setup_failure (Masc.Fusion_types.Provider_error "quota")) in
    match
      ...
      (match first, second with
       | ( Cli_oneshot.Execution_failed { runtime_id = first_id; cause }
         ...
         check bool "runner cause is kept" true
           (cause = Masc.Fusion_official_client.Setup_failure (Masc.Fusion_types.Provider_error "quota"));
