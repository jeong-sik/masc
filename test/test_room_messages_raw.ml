...
let test_get_messages_raw_since_seq_stops_early () =
  with_test_env (fun config ->
    ...
    let contents = List.map (fun (msg : Types.message) -> msg.content) msgs in
    Alcotest.(check (list string)) "only newer than since_seq" [ "Message 3" ] contents)
;;
...
let test_get_messages_raw_large_history_keeps_newest_window () =
  with_test_env (fun config ->
    ...
    let contents = List.map (fun (msg : Types.message) -> msg.content) msgs in
    Alcotest.(check (list string))
      "large history keeps newest 3"
      [ "Message 20"; "Message 19"; "Message 18" ]
      contents)
;;

let () =
  Alcotest.run
    "Coord raw message regression"
    [ ( "messages_raw"
      , [ Alcotest.test_case
            "limit and newest-first ordering"
            `Quick
            test_get_messages_raw_limit_and_order
        ; Alcotest.test_case
            "since_seq filters older history"
            `Quick
            test_get_messages_raw_since_seq_stops_early
        ; Alcotest.test_case
            "large history keeps newest window"
            `Quick
