...
- 기존 *내부* vendor-식별 literal (legacy canonical-provider 상수군) 은 RFC-0058 추적의 일환이며 본 RFC 의 scope 가 아니다. 그러나 sub-lib *경계* 에는 단 한 개도 노출되지 않아야 한다.

### 2.2 검증

vendor-식별자 regex (RFC-0058 추적 자료의 vendor-token list 와 동기화) 로 `lib/provider_adapter_*/dune` 의 *normative 텍스트* 를 grep 했을 때 0 hit 여야 한다 (sub-lib 내부 .ml 의 string literal 은 별개; sub-lib *디렉토리 메타* 는 vendor-free).
...
- **`.mli` 원본 section 헤더 인용**: §3.1 의 cluster 분석 표가 현행 `lib/provider_adapter.mli` 의 OCamldoc `{1 ...}` 헤더를 *그대로* 인용 (예: "Llama Model Resolution", "Ollama" 는 현행 .mli line 431–439 의 section 헤더 원문). 이 헤더들은 RFC-0058 추적이 별개로 처리하며, 본 RFC 의 분할은 이 섹션들을 dispatch sub-lib 에 통합한 후 OCamldoc 헤더 자체를 abstract 한 이름 (예: "Local Runtime Model Resolution") 으로 재명명하는 별도 PR 의 trigger 가 된다.
...
`lib/provider_adapter.mli` 의 OCamldoc `{1 ...}` 헤더로 식별되는 sections (Section 열은 현행 `.mli` 의 *원문 헤더 인용* — §2.2 의 vendor-token 화이트리스트 적용 대상):

| `.mli` Section 헤더 (원문 인용) | LOC range | 책임 cluster |
|---------|-----------|-------------|
| Types | 9–173 | **types** — types 만 (공용) |
| Canonical Provider Names | 175–191 | **dispatch** — legacy canonical-provider 상수 |
...
Phase 0/1/2/3 모두 `.mli` 추가/이동만 허용. 기존 `lib/provider_adapter.mli` 의 *signature 자체* 는 byte-identical 로 새 위치로 이동 — 단순 textual move. signature 의 *분할* 은 발생하지만 (한 .mli 가 N .mli 로) 각 함수의 type signature 는 동일.
...
2. **Byte-identity (이동된 section 단위)**: 각 이동된 OCamldoc section 에 대해 `diff <(awk '/^{1 SectionName}/,/^{1 [^}]+}/' lib/provider_adapter.mli) <(awk ... lib/<new-sub-lib>/<new>.mli)` 가 empty output. 또는 `git log --follow --patch -- lib/<new-sub-lib>/<new>.mli` 가 첫 commit 이외의 modify 가 없음을 확인.
...
- **voice ↔ dispatch**: voice 가 legacy canonical name 상수 (dispatch) 를 사용 가능. 해결: canonical-provider 상수는 Phase 0 types 가 아닌 dispatch 에 둠 — voice 가 직접 string literal 을 사용하지 않고 dispatch helper 를 import. 또는 voice 가 canonical-provider 의존성 없으면 격리 가능. Phase 1 PR 의 dune build 가 검증.
...
- **Vendor literal 제거**: 본 RFC 는 *경계* 의 opacity 만 보장. sub-lib *내부* 의 legacy canonical-provider literal 은 RFC-0058 의 추적 대상이며 별개 작업.
