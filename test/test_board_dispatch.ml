...
let test_default_backend () =
  Alcotest.(check string) "default is jsonl" "jsonl" (Board_dispatch.backend_name ())
;;
...
let test_create_and_get_post () =
  ...
  | Ok post ->
    ...
    (match Board_dispatch.get_post ~post_id:pid with
     | Error e -> Alcotest.fail (Board.show_board_error e)
     | Ok fetched ->
       Alcotest.(check string) "content matches" "dispatch test post" fetched.content)
;;
...
let test_list_posts_with_sort () =
  ...
  let counts =
    List.map
      List.length
      [ posts_hot; posts_recent; posts_trending; posts_updated; posts_discussed ]
  in
...
let test_recent_sort_bypasses_hot_cutoff () =
  ...
  for i = 1 to 101 do
    let hot_post =
      create_post_exn
        ~author:(Printf.sprintf "hot-author-%03d" i)
        ~content:(Printf.sprintf "hot post %03d" i)
    in
    vote_up_exn
      ~post_id:(Board.Post_id.to_string hot_post.id)
      ~voter:(Printf.sprintf "hot-voter-%03d" i)
  done;
  let cold_post =
    create_post_exn
      ~author:"recent-cold-author"
  ...
  in
  let hot_posts = Board_dispatch.list_posts ~sort_by:Board_dispatch.Hot ~limit:1 () in
  let recent_post_id =
  ...
  in
  Alcotest.(check string)
    "recent returns latest post beyond hot top 100"
    cold_post_id
    recent_post_id;
  Alcotest.(check bool)
    "hot ranking still excludes cold post"
    false
...
let test_list_posts_with_filters () =
  ...
  ignore
    (Board_dispatch.create_post
       ~author:"filter-human"
       ~content:"human-filter-test"
       ~post_kind:Board.Human_post
       ());
  ignore
    (Board_dispatch.create_post
       ~author:"filter-harness-bot"
       ~content:"automation"
  ...
  ignore
    (Board_dispatch.create_post
       ~author:"filter-keeper"
       ~content:"keeper"
       ~post_kind:Board.Automation_post
       ~meta_json:keeper_meta
       ());
  let all_posts =
...
let test_list_posts_matches_comment_author () =
  ...
  (match
     Board_dispatch.add_comment
       ...
       ~author:"comment-match-agent"
       ~content:"I touched this thread"
       ()
   with
   | Ok _ -> ()
   | Error e -> Alcotest.fail (Board.show_board_error e));
  ...
  in
  Alcotest.(check bool)
    "matching comment author includes post"
    true
    (List.mem matching_post_id ids);
  Alcotest.(check bool)
    "non matching comment author excluded"
    false
...
let test_author_filter_treats_wildcards_literally () =
  ignore
    (Board_dispatch.create_post
       ~author:"wildcard-alpha"
       ~content:"literal wildcard filter"
       ~post_kind:Board.Human_post
       ());
  let filtered =
    Board_dispatch.list_posts
      ...
      ()
  in
  Alcotest.(check int) "percent does not match all authors" 0 (List.length filtered)
;;
...
let test_reclassify_posts_dry_run_and_apply () =
  ...
  (match Board_dispatch.get_post ~post_id with
   ...
   | Ok fetched ->
     Alcotest.(check string)
       "legacy row resolves as automation"
       "automation"
       (Board.post_kind_to_string fetched.post_kind));
...
let test_add_and_get_comments () =
  ...
  | Ok post ->
    let pid = Board.Post_id.to_string post.id in
    (match
       Board_dispatch.add_comment ~post_id:pid ~author:"responder" ~content:"nice post" ()
     with
     | Error e -> Alcotest.fail (Board.show_board_error e)
     | Ok _ -> ());
    (match Board_dispatch.get_comments ~post_id:pid with
     | Error e -> Alcotest.fail (Board.show_board_error e)
     | Ok comments -> Alcotest.(check bool) "has comment" true (List.length comments >= 1))
;;
...
let test_vote_post () =
  ...
  | Ok post ->
    ...
    (match Board_dispatch.vote ~voter:"judge" ~post_id:pid ~direction:Board.Up with
     | Error e -> Alcotest.fail (Board.show_board_error e)
     | Ok score -> Alcotest.(check int) "score after upvote" 1 score)
;;
...
let test_vote_flip () =
  ...
  | Ok post ->
    ...
    (match Board_dispatch.vote ~voter:"flipper" ~post_id:pid ~direction:Board.Down with
     | Error e -> Alcotest.fail (Board.show_board_error e)
     | Ok score -> Alcotest.(check int) "score after flip" (-1) score)
;;
  ...
  try
    ...
    Eio_context.with_test_env
      ...
      (fun () ->
         ...
         (match Board_dispatch.get_post ~post_id with
          | Error e -> Alcotest.fail (Board.show_board_error e)
          | Ok post -> Alcotest.(check int) "vote persisted after restart" 1 post.votes_up);
...
let test_search () =
  ignore
    (Board_dispatch.create_post
       ~author:"searcher"
       ~content:"unique_dispatch_search_term"
       ~post_kind:Board.Human_post
       ());
    ...
    (match Board_dispatch.set_thread_id ~post_id:pid ~thread_id:"thread-abc" with
     ...
     | Ok () ->
       (match Board_dispatch.get_post ~post_id:pid with
        | Error e -> Alcotest.fail (Board.show_board_error e)
        | Ok p ->
          Alcotest.(check (option string)) "thread_id set" (Some "thread-abc") p.thread_id))
;;

let test_flush () = Board_dispatch.flush ()
...
let test_jsonl_forced_default () =
  (try Unix.putenv "MASC_BOARD_BACKEND" "" with
   | _ -> ());
  Alcotest.(check bool) "empty env not forced" false (Board_dispatch.jsonl_forced ())
;;

let test_jsonl_forced_explicit () =
  Unix.putenv "MASC_BOARD_BACKEND" "jsonl";
  Alcotest.(check bool) "jsonl is forced" true (Board_dispatch.jsonl_forced ());
  Unix.putenv "MASC_BOARD_BACKEND" ""
;;

let test_jsonl_forced_pg () =
  Unix.putenv "MASC_BOARD_BACKEND" "pg";
  Alcotest.(check bool) "pg is not forced" false (Board_dispatch.jsonl_forced ());
  Unix.putenv "MASC_BOARD_BACKEND" ""
;;

let test_jsonl_forced_case_insensitive () =
  Unix.putenv "MASC_BOARD_BACKEND" "JSONL";
  Alcotest.(check bool) "JSONL uppercase is forced" true (Board_dispatch.jsonl_forced ());
  Unix.putenv "MASC_BOARD_BACKEND" ""
;;
  ...
  Alcotest.run
    ...
    ; ( "posts"
      , [ Alcotest.test_case "create and get" `Quick (with_eio test_create_and_get_post)
        ; Alcotest.test_case
            ...
            `Quick
            (with_eio test_structured_post_roundtrip)
    ...
    ; ( "votes"
      , [ Alcotest.test_case "upvote" `Quick (with_eio test_vote_post)
        ...
        ; Alcotest.test_case
            "vote persisted by flusher actor"
            `Quick
    ...
    ; ( "env_control"
      , [ Alcotest.test_case
            "default not forced"
            `Quick
            (with_eio test_jsonl_forced_default)
        ...
        ; Alcotest.test_case
            "case insensitive"
            `Quick
            (with_eio test_jsonl_forced_case_insensitive)
        ] )
    ]
;;
