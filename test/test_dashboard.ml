...
let test_format_section_empty () =
  let section =
    Lib.Dashboard.{ title = "Empty Section"; content = []; empty_msg = "(nothing here)" }
  in
  let output = Lib.Dashboard.format_section section in
...
let test_generate_full () =
  ...
  Alcotest.(check bool)
    ...
    true
    (contains output "Attention Required");
...
let test_keepers_section_with_entry () =
  ...
  let dir = test_dir () in
  Lib.Keeper_registry.clear ();
  ignore (Lib.Keeper_registry.register ~base_path:dir "alpha" (make_test_meta "alpha"));
  let now = Unix.gettimeofday () in
  let section = Lib.Dashboard.keepers_section now in
  Alcotest.(check string) "title" "Keepers" section.title;
...
let test_keepers_section_dead_phase () =
  ...
  let dir = test_dir () in
  Lib.Keeper_registry.clear ();
  ignore (Lib.Keeper_registry.register ~base_path:dir "delta" (make_test_meta "delta"));
...
let test_keepers_section_with_error_truncated () =
  ...
  let dir = test_dir () in
  Lib.Keeper_registry.clear ();
  ignore (Lib.Keeper_registry.register ~base_path:dir "echo" (make_test_meta "echo"));
  let long_err =
    "this is a long error message that should exceed the default display length of 35 \
     chars and therefore be truncated with an ellipsis"
  in
  ...
  Alcotest.(check bool) "long error body is not verbatim" false (contains line "ellipsis");
  Lib.Keeper_registry.clear ();
  cleanup_dir dir
;;
...
let keepers_tests =
  ...
  ; ( "keepers section with error truncated"
    , `Quick
    , test_keepers_section_with_error_truncated )
  ]
;;
