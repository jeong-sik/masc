...
let resolve_kimi_max_context resolved_model_id =
  ...
  let from_model =
    Option.bind (Capabilities.for_model_id resolved_model_id) (fun c ->
      c.Capabilities.max_context_tokens)
  in
  ...
  | None ->
    (match Capabilities.kimi_capabilities.max_context_tokens with
     ...
     | None ->
       ...
       (match
          Provider_registry.find (Provider_registry.default ()) kimi_provider_name
        with
        | Some entry -> entry.Provider_registry.max_context
        | None -> 0))
;;

let is_kimi_provider provider_name = String.equal provider_name kimi_provider_name
...
let kimi_is_available ~api_key_env_overrides =
  ...
  | "" -> false
  | env_name ->
    Option.is_some (nonempty_env env_name) || Option.is_some (nonempty_env "KIMI_API_KEY")
;;

let make_kimi_config
      ~temperature
      ~max_tokens
      ?system_prompt
      ?(api_key_env_overrides = [])
      ...
      model_id
  =
  let effective_api_key_env = resolve_kimi_api_key_env ~api_key_env_overrides in
  let api_key =
  ...
  let resolved_model_id = resolve_auto_model_id kimi_provider_name model_id in
  Llm_provider.Provider_config.make
    ~kind:OpenAI_compat
...
let make_registry_config
  ...
  let discover =
    ...
    | _ -> None
  in
  let model_resolution = resolve_auto_model ?discover provider_name model_id in
  ...
  let request_path =
    ...
    | OpenAI_compat ->
      normalize_openai_compat_request_path ~base_url ~request_path:defaults.request_path
...
let parse_model_string
  ...
  | Some (provider_name, model_id) when is_kimi_provider provider_name ->
    if kimi_is_available ~api_key_env_overrides
    then
      Some
        (make_kimi_config
           ~temperature
...
type weighted_entry_drop =
  | Drop_unregistered_scheme of
      { model : string
      ; scheme : string
      }
  | Drop_unavailable_scheme of
      { model : string
      ; scheme : string
      }
    ...
    then
      Ok
        (make_kimi_config
           ...
           ?num_ctx
           model_id)
    else Error (Drop_unavailable_scheme { model = raw; scheme = provider_name })
  | Some (provider_name, model_id) ->
    (match Llm_provider.Provider_registry.find default_registry provider_name with
     | None -> Error (Drop_unregistered_scheme { model = raw; scheme = provider_name })
  ...
  | Some scope, _ :: _ :: _ ->
    let cursor =
      Cascade_state.rotate_round_robin
        ~cascade:(Printf.sprintf "auto-expand:%s:%s" scope (String.lowercase_ascii spec))
...
let expand_auto_model_string ?rotation_scope (s : string) : string list =
  let trimmed = String.trim s in
  match split_provider_model trimmed with
  | Some (provider_name, model_id) when String_util.equals_ci model_id "auto" ->
...
let parse_weighted_entries
  ...
  let render_drops drops =
    String.concat
      ", "
      (List.map
         (fun (model, scheme) -> Printf.sprintf "%s (scheme=%s)" model scheme)
         drops)
  in
  if unregistered <> []
  then
    Log.Misc.error
      "%s: dropped %d entry/entries referencing unregistered provider scheme(s): [%s]. \
       Likely library/binary drift or cascade.json typo — rebuild or fix the config \
       entry."
      label
      (List.length unregistered)
      (render_drops unregistered);
  if invalid <> []
  then
    Log.Misc.error
  ...
  then
    Log.Misc.warn
      "%s: skipped %d unavailable entry/entries (missing credential or CLI binary): [%s]"
      label
      (List.length unavailable)
      (render_drops unavailable);
  if parsed = [] && entries <> []
  then
    Log.Misc.error
      "%s: all %d configured entries filtered out — cascade will produce no responses \
       until at least one entry resolves"
      label
      (List.length entries);
  List.rev parsed
;;
...
let parse_model_string_result
  ...
  | Some (provider_name, model_id) when is_kimi_provider provider_name ->
    if kimi_is_available ~api_key_env_overrides:[]
    then Ok (make_kimi_config ~temperature ~max_tokens ?system_prompt model_id)
    else
      Error
        (Printf.sprintf
           "provider %S unavailable (missing env var %S)"
           provider_name
...
let parse_model_strings
  ...
  List.filter_map
    (parse_model_string ~temperature ~max_tokens ?system_prompt ~api_key_env_overrides)
    expanded
;;
  ...
  | Some ("llama", model_id) ->
    ...
    (match Llm_provider.Discovery.context_for_model model_id with
     ...
     | None ->
       ...
       if url = "" then None else Llm_provider.Discovery.discovered_context_for_url url)
  | Some (_, _) ->
...
let filter_by_capabilities
  ...
  if filtered = [] then providers else filtered
;;
...
let weighted_shuffle
  ...
  | first :: rest ->
    let total_weight =
      List.fold_left
        (fun acc (e : Cascade_config_loader.weighted_entry) -> acc + e.weight)
        0
        entries
    in
    if total_weight <= 0
    then entries
      ...
      let sorted_remaining =
        List.sort
          (fun (i1, (a : Cascade_config_loader.weighted_entry))
...
let order_weighted_entries
  ...
  let has_weights =
    List.exists (fun (e : Cascade_config_loader.weighted_entry) -> e.weight <> 1) entries
  in
  if not has_weights
  then entries
  else (
    ...
    let health_adjusted =
      List.map
        (fun (e : Cascade_config_loader.weighted_entry) ->
           ...
           let provider_key =
             match String.split_on_char ':' e.model with
             | _ :: rest when rest <> [] -> String.concat ":" rest
             | _ -> e.model
           in
           let ew =
             Cascade_health_tracker.effective_weight
               health
               ~provider_key
...
let resolve_model_strings_traced_with ~rand_int ?config_path ~name ~defaults () =
  match config_path with
  | Some path ->
    let from_file_weighted =
      Cascade_config_loader.load_profile_weighted ~config_path:path ~name
    in
    if from_file_weighted <> []
    then (
      let ordered = order_weighted_entries ~rand_int from_file_weighted in
      let models =
        List.map (fun (e : Cascade_config_loader.weighted_entry) -> e.model) ordered
      in
      models, Named)
    else (
      let fallback_weighted =
        Cascade_config_loader.load_profile_weighted ~config_path:path ~name:"default"
      in
...
let display_model_string s =
  match split_provider_model s with
  | Some (provider_name, model_id) ->
    Printf.sprintf "%s:%s" (Provider_adapter.display_provider_name provider_name) model_id
  | None -> s
;;
...
let candidate_info_of_weighted (e : Cascade_config_loader.weighted_entry) =
  ...
  let health_rows =
    List.map
      (fun provider_key ->
         let success_rate = Cascade_health_tracker.success_rate health ~provider_key in
         let in_cooldown = Cascade_health_tracker.is_in_cooldown health ~provider_key in
  ...
  let provider_name, display_provider_name, runtime_kind =
    ...
    | Some (provider_name, _model_id) ->
      ...
      Some provider_name, Some display_name, runtime_kind_of_provider_name provider_name
    | None -> None, None, None
  in
...
let expand_model_strings_for_execution ?rotation_scope (items : string list) =
  items |> List.concat_map (expand_auto_model_string ?rotation_scope) |> dedupe_stable
;;
...
let apply_provider_filter ~provider_filter ~label providers =
  ...
  | Some filters ->
    ...
    then (
      Eio.traceln
        "[CascadeConfig] provider_filter matched no providers (%s); falling back to \
         unfiltered (filter=[%s] providers=[%s])"
        label
        (String.concat "," filters)
        (String.concat
           ","
           (List.map
              (fun (p : Llm_provider.Provider_config.t) ->
...
let local_capacity_for_selections ~sw ~net ?config_path selections =
  ...
  then empty_capacity
    ...
    let infos = List.filter_map (fun url -> Cascade_throttle.capacity url) local_urls in
    match infos with
    | [] -> empty_capacity
    | _ ->
      List.fold_left
...
let warn_unknown_strategy ~name ~raw ~msg ~fallback_kind =
  let key = name, raw in
  if not (Hashtbl.mem strategy_warned key)
  then (
    ...
    Printf.eprintf
      ...
      name
      msg
      (Cascade_strategy.kind_to_string fallback_kind))
;;

let invalid_priority_tier_warned : (string * string, unit) Hashtbl.t = Hashtbl.create 4

let warn_invalid_priority_tier ~name ~msg ~fallback_kind =
  let key = name, msg in
  if not (Hashtbl.mem invalid_priority_tier_warned key)
  then (
     ...
     | Ok entries ->
       (match
          List.find_opt
            ...
            entries
        with
        | Some entry when entry.keeper_assignable -> Cascade_strategy.Round_robin
...
let cycle_policy_from_loader (cfg : Cascade_config_loader.strategy_config) =
  ...
  in
  let backoff_base_ms =
    match cfg.backoff_base_ms with
    | Some n when n >= 1 -> n
    | _ -> d.backoff_base_ms
  in
  let backoff_cap_ms =
    match cfg.backoff_cap_ms with
...
let normalize_priority_tiers ~config_path ~name raw_tiers =
  ...
  then Error "priority_tier has no configured models to validate against"
  else (
    let normalized =
      raw_tiers
      |> List.filter_map (fun tier ->
        let tier_model_ids =
          ...
          |> List.filter (fun model_id -> List.mem model_id configured_model_ids)
        in
        if tier_model_ids = [] then None else Some tier_model_ids)
    in
    if normalized = []
    then Error "priority_tier tiers did not match any configured candidate model ids"
    else Ok normalized)
;;
...
let resolve_strategy ?config_path ~name () =
  ...
  | Some path ->
    ...
    let cfg = Cascade_config_loader.resolve_strategy_config ~config_path:path ~name in
    let parsed_kind = parse_kind_or_default ~name ~default_kind cfg.kind in
    ...
    let kind, tiers =
      ...
      | Cascade_strategy.Priority_tier ->
        let result =
          ...
          | Some raw_tiers -> normalize_priority_tiers ~config_path:path ~name raw_tiers
          | None -> Error "priority_tier requires a non-empty <name>_tiers configuration"
        in
        (match result with
         | Ok tiers -> parsed_kind, tiers
         | Error msg ->
           warn_invalid_priority_tier ~name ~msg ~fallback_kind:default_kind;
           default_kind, [])
      | _ -> parsed_kind, []
    in
    let sticky_ttl_ms =
