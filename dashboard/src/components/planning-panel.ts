  ...
  return html`
    <li class="rounded border border-card-border/60 bg-black/10 p-2">
      ...
      ${evidence.length > 0 ? html`
        <ul class="mt-2 grid gap-1" aria-label="증거 목록">
...
function CoordinationHealthPanel() {
  ...
  return html`
    <section class="rounded border border-card-border/70 bg-[rgba(8,13,22,0.74)] p-3" aria-label="조정 상태">
      ...
      ${topEvidence.length > 0 ? html`
        <div class="mt-3">
          ...
          <ul class="grid gap-1 md:grid-cols-2" aria-label="근거 목록">
