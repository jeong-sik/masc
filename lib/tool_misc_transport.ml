...
let encode_string_list values = `List (List.map (fun value -> `String value) values)

let pretty_json_string raw =
  try Yojson.Safe.from_string raw |> Yojson.Safe.pretty_to_string with
  | Yojson.Json_error _ -> raw
;;
...
let handle_webrtc_offer args : tool_result =
  if not (Server_webrtc_transport.is_enabled ())
  then error_result "webrtc transport disabled"
  else
    ...
    let fields =
      ...
      | Some fingerprint -> [ "dtls_fingerprint", `String fingerprint ]
      | None -> []
    in
    match
      Server_webrtc_transport.handle_offer_request (Yojson.Safe.to_string (`Assoc fields))
    with
