...
| --------------------------- | ------------------------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------- |
| `HebbianLearning`           | Trait drift under reinforcement                   | none — `lib/keeper/social_model/` removed by RFC-0276 | **Retire spec to docs/**. The social_model code is purged (RFC-0276 Phase 2b); no reactive state machine remains. Keep as `docs/research/` reference. |
...
| KSM-Ledger — Magentic ledger   | _(removed)_ | _(removed)_      | —                          | **Removed (RFC-0275).** Spec and FSM module were deleted with the BDI triple; `magentic_ledger_v1` was never assigned to a keeper. No longer a candidate axis.   |
