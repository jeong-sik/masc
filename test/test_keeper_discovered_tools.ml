...
let dt = Discovered.create

let test_add_and_active () =
  let t = dt ~decay_turns:5 in
  Discovered.add t ~turn:0
    ~names:[ "tool_execute"; "tool_search_files" ];
  let active = Discovered.active_names t ~turn:0 in
...
let test_mark_used_resets_decay () =
  ...
  Discovered.mark_used t ~turn:3 ~name:"tool_a";
  let active = Discovered.active_names t ~turn:4 in
  ...
  let expired = Discovered.decay t ~turn:7 in
...
let test_to_json () =
  let t = dt ~decay_turns:5 in
  Discovered.add t ~turn:1 ~names:[ "tool_x" ];
  let json = Discovered.to_json t in
  let count =
...
let test_decay_turns_minimum () =
  ...
  let active = Discovered.active_names t ~turn:1 in
  Alcotest.(check int) "active at boundary" 1 (List.length active);
  (* At turn 2: 2 - 0 = 2 > 1 → expired *)
  let expired = Discovered.decay t ~turn:2 in
