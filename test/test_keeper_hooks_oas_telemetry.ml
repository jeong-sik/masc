...
module Hooks_gate_attempt = Masc_mcp.Keeper_hooks_oas_gate_attempt
...
let test_trajectory_duration_ms_preserves_positive_sub_ms () =
  check int "positive sub-ms" 1 (Hooks_gate_attempt.trajectory_duration_ms 0.4);
  check int "rounded positive" 13 (Hooks_gate_attempt.trajectory_duration_ms 12.5)
;;

let test_trajectory_duration_ms_rejects_zero_and_non_finite () =
  check int "zero" 0 (Hooks_gate_attempt.trajectory_duration_ms 0.0);
  check int "negative" 0 (Hooks_gate_attempt.trajectory_duration_ms (-0.1));
  check int "nan" 0 (Hooks_gate_attempt.trajectory_duration_ms nan);
  check int "infinity" 0 (Hooks_gate_attempt.trajectory_duration_ms infinity)
;;
...
let test_pr_review_action_metric_extracts_approve () =
  ...
  in
  check string "attested keeper" "sangsu" (attestation |> member "keeper" |> to_string);
  let credential =
...
let test_pr_work_action_metric_extracts_bash_git_push_with_redirection () =
  let events =
    pr_work_events
      ...
      ()
  in
  check (list string) "git push with redirection" [ "GIT_PUSH" ] (work_actions events);
  match events with
  | [ event ] -> check (option string) "route via" (Some "docker") event.route_via
  | _ -> failf "expected one git push event"
;;
...
let test_pr_work_action_metric_extracts_bash_git_sequence_failure () =
  let events =
    pr_work_events
      ...
      ()
  in
  check (list string) "git action sequence" [ "GIT_ADD" ] (work_actions events);
  check
    bool
    "failure propagated to every action"
    true
    ...
    pr_work_events
      ...
      ~input:
        (`Assoc
            [ ( "cmd"
              , `String
                  "git commit -m \"revert git push\" && gh issue comment 1 --body \
                   \"please pr create later\"" )
            ])
      ~output_text:{|{"ok":true}|}
      ()
  in
  ...
  let or_events =
    pr_work_events
      ...
      ~input:
        (`Assoc
            [ ( "cmd"
              , `String "git commit -m reviewed || gh pr create --draft --title retry" )
            ])
      ~output_text:{|{"ok":true}|}
      ()
  in
...
let () =
  run
    ...
    ; ( "pr_review_action"
      , [ test_case
            "extracts approve event from structured output"
            `Quick
            test_pr_review_action_metric_extracts_approve
        ; test_case
            "marks structured gh failure as not successful"
            `Quick
            test_pr_review_action_metric_marks_structured_failure
        ; test_case
            "extracts reply action"
            `Quick
