...
let keeper_profile_json (entry : Keeper_registry.registry_entry) : Yojson.Safe.t =
  `Assoc (keeper_profile_fields ~keeper:entry.name ~cascade_name:entry.meta.cascade_name)
;;
...
let save_raw_config_json raw_json =
  ...
  | Cascade_toml_materializer.Json ->
    let parse_result =
      ...
      | exn -> Error (Printf.sprintf "failed to parse JSON: %s" (Printexc.to_string exn))
    in
...
let slo_json () =
  ...
  let ordered_ratio =
    if total = 0 then 1.0 else float_of_int ordered /. float_of_int total
  in
