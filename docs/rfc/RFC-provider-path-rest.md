...
- 다음에 보낼 경로가 모두 쉬면 가장 먼저 풀리는 시각까지 기다린다. 그동안 wake 는
  잠을 끊지 못한다(#34653 보장 유지).
...
|---|---|---|
| deferred suffix 있음 | quota 순서로 첫 경로가 쉬지 않음 | 기다리지 않는다. 남은 입력이 있으면 곧바로 그 suffix 로 턴을 잇는다 |
| deferred suffix 있음 | suffix 경로가 모두 쉼 | suffix 중 가장 먼저 풀리는 시각까지 기다린다 |
| suffix 없음, route 가 `Rate_limited`·`Hard_quota` | 실패한 경로가 쉼 | 그 경로가 풀리는 시각까지 기다린다 |
| suffix 없음, route 가 `Capacity_backpressure` | 실패한 경로가 쉼 | 그 시각까지 기다리되 wake 는 잠을 끊는다 (지금과 같음) |
| 그 밖 | — | 지금처럼 cadence |
...
풀리는 시각을 정하는 쪽과 순서를 정하는 쪽이 다를 수 있다. 시간을 말하지 않은 증거는
`demote_unavailable_candidates` 에서 성공이 올 때까지 뒤로 밀린다. 이 RFC 의 대기 판단은
위 표의 시각이 지나면 그 경로를 쉬지 않는 것으로 본다. 순서는 그대로 두고 기다림만 끝낸다.
...
2. **이어가는 턴은 쉬는 경로에서 시작하지 않는다.** `Continue_on_deferred_lane` 은 suffix 의
   첫 경로가 쉬지 않을 때만 나온다. suffix 에는 실패한 경로가 들어 있지 않다.
...
- 결정 함수 테스트: 쉬지 않는 suffix → 이어가기, 모두 쉬는 suffix → 가장 이른 시각,
  suffix 없는 rate limit → 실패 경로의 쉼, 힌트 5초 → 5초(cadence 무관), 그 밖 → cadence.
