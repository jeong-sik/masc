...
let test_kimi_api_anthropic_body_bearer_auth () =
  ...
  | Some e ->
    (match e.body_schema with
     ...
     | _ -> Alcotest.fail "kimi-api must use Anthropic_content_blocks body schema");
    (match e.auth with
     | E.Bearer { env_var } ->
       Alcotest.(check string)
         ...
         "KIMI_API_KEY_SB"
         env_var
     | _ -> Alcotest.fail "kimi-api must use Bearer auth (OpenAI-style on Anthropic body)")
;;
...
let test_ollama_uses_ndjson_stream () =
  ...
  | Some e ->
    (match e.stream_format with
     ...
     | _ ->
       Alcotest.fail
         "ollama must use Ndjson_ollama (done:true terminator carries eval_count)");
    (match e.body_schema with
     | E.Ollama_options -> ()
     | _ -> Alcotest.fail "ollama must use Ollama_options body schema");
    (match e.discovery with
     | E.Ps_endpoint { path } ->
       Alcotest.(check string) "ollama discovery uses /api/ps" "/api/ps" path
...
let test_gemini_cli_no_per_call_mcp () =
  ...
  | Some e ->
    Alcotest.(check bool)
      "gemini_cli supports_per_call_mcp_config is false (gemini-cli #4674 unimplemented)"
      false
      e.capabilities.supports_per_call_mcp_config
;;
...
let test_glm_no_v1_path () =
  ...
  | Some e ->
    ...
    (match e.body_schema with
     ...
     | _ ->
       Alcotest.fail
         "glm must use OpenAI_messages_with_thinking (thinking field is GLM-specific)")
;;
...
let test_local_http_endpoints_declare_discovery () =
  ...
  Alcotest.(check int)
    ...
    2
    (List.length local_http);
  List.iter
    (fun (e : E.t) ->
       match e.discovery with
       | E.No_discovery ->
         Alcotest.failf
           "local endpoint %s must declare a discovery method (Models or Ps endpoint)"
           e.label_prefix
       | _ -> ())
    local_http
;;
...
let test_find_by_label_prefix_round_trip () =
  ...
  | None -> ()
  | Some _ -> Alcotest.fail "find_by_label_prefix must return None for unknown prefix"
;;
...
let test_claude_api_anthropic_version_header () =
  ...
  | Some e ->
    (match e.auth with
     ...
     | _ -> Alcotest.fail "claude-api must use X_api_key with anthropic-version header")
;;
...
let () =
  Alcotest.run
    ...
    [ ( "structural"
      , [ Alcotest.test_case
            "1:1 alignment with Provider_adapter"
            `Quick
            test_count_aligns_with_provider_adapter
        ; Alcotest.test_case "label_prefix uniqueness" `Quick test_label_prefixes_unique
        ] )
    ; ( "§2.1 wire-level surprises"
      , [ Alcotest.test_case
            "kimi-api: Anthropic body + Bearer auth"
            `Quick
            test_kimi_api_anthropic_body_bearer_auth
        ; Alcotest.test_case
            "ollama: ndjson stream + Ps_endpoint discovery"
            `Quick
            test_ollama_uses_ndjson_stream
        ; Alcotest.test_case
            "gemini_cli: no per-call MCP config"
            `Quick
            test_gemini_cli_no_per_call_mcp
        ; Alcotest.test_case
            "glm: /chat/completions (no /v1) + thinking field"
            `Quick
            test_glm_no_v1_path
        ] )
    ; ( "auth and transport invariants"
      , [ Alcotest.test_case
            "all 4 CLI subprocess endpoints use Cli_cached_login"
            `Quick
            test_cli_transports_use_cached_login
        ; Alcotest.test_case
            "local HTTP endpoints (llama, ollama) declare discovery"
            `Quick
            test_local_http_endpoints_declare_discovery
        ; Alcotest.test_case
            "claude-api includes anthropic-version header"
            `Quick
            test_claude_api_anthropic_version_header
        ] )
    ; ( "lookup"
      , [ Alcotest.test_case
            "find_by_label_prefix round-trip + None for unknown"
            `Quick
            test_find_by_label_prefix_round_trip
        ] )
    ]
;;
