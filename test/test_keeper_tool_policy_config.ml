...
let test_load_honors_masc_config_dir_override () =
  with_temp_dir "tool-policy-config"
  @@ fun root ->
  ...
  | Ok cfg ->
    ...
    check
      bool
      "loads config from MASC_CONFIG_DIR override"
      true
      (List.mem "full" presets && List.mem "messaging" presets)
  | Error msg ->
    fail
...
let test_load_anchors_resolution_to_base_path_over_cwd_candidate () =
  with_temp_dir "tool-policy-build-root"
  @@ fun fake_build_root ->
  let source_root = Masc_test_deps.find_project_root () in
  ...
  | Ok cfg ->
    ...
    check
      bool
      "ignores cwd-only config candidate without tool_policy"
      true
      (List.mem "full" presets && List.mem "messaging" presets)
  | Error msg ->
    fail
...
let test_social_cannot_satisfy_delivery () =
  ...
  check
    bool
    "social cannot satisfy delivery"
    false
...
let test_unknown_preset_returns_false () =
  ...
  check
    bool
    "unknown agent preset cannot satisfy delivery"
    false
    (KTPC.preset_can_satisfy cfg ~agent_preset:"nonexistent" ~required_preset:"delivery");
  check
    bool
    "any preset cannot satisfy unknown required"
    false
...
let () =
  run
    ...
    [ ( "load"
      , [ test_case
            "falls back to resolved config dir"
            `Quick
            test_load_falls_back_to_resolved_config_dir
        ; test_case
            "honors MASC_CONFIG_DIR override"
            `Quick
            test_load_honors_masc_config_dir_override
        ; test_case
            "anchors resolution to base_path over cwd candidate"
            `Quick
    ...
    ; ( "preset_can_satisfy"
      , [ test_case "same preset satisfies" `Quick test_same_preset_satisfies
        ; test_case
            "social cannot satisfy delivery"
            `Quick
            test_social_cannot_satisfy_delivery
        ...
        ; test_case
            "minimal cannot satisfy social"
            `Quick
            test_minimal_cannot_satisfy_social
        ; test_case
            "unknown preset returns false"
            `Quick
            test_unknown_preset_returns_false
        ] )
    ]
;;
