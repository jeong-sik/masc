...
let setup ?(sandbox = Keeper_types.Local) f =
  ...
  let playground =
    Filename.concat base (Keeper_alerting_path.playground_path_of_keeper meta.name)
  in
...
let parse_int raw field = parse raw |> Json.member field |> Json.to_int_option
...
let test_patch_unique_match () =
  ...
  Alcotest.(check (option int)) "occurrences=1" (Some 1) (parse_int raw "occurrences");
  let after = Fs_compat.load_file path in
  Alcotest.(check string) "file content updated" "let x = 42\nlet y = 2\n" after
;;
...
let test_patch_multiple_matches_without_replace_all_errors () =
  ...
  Alcotest.(check bool) "ok=false" false (parse_ok raw);
  let after = Fs_compat.load_file path in
  Alcotest.(check string) "file unchanged on rejection" "x = 1\nx = 1\nx = 1\n" after
;;
...
let test_patch_replace_all () =
  ...
  in
  Alcotest.(check bool) "ok" true (parse_ok raw);
  Alcotest.(check (option int)) "occurrences=3" (Some 3) (parse_int raw "occurrences");
...
let test_patch_empty_old_string_errors () =
  ...
  in
  Alcotest.(check bool) "ok=false" false (parse_ok raw);
  Alcotest.(check bool) "error message present" true (Option.is_some (parse_error raw))
;;
...
let test_overwrite_unchanged_by_patch_addition () =
  ...
  in
  Alcotest.(check bool) "ok" true (parse_ok raw);
  Alcotest.(check string) "overwrite wrote bytes" "fresh" (Fs_compat.load_file path)
;;

let () =
  Alcotest.run
    "Keeper_fs_edit_patch"
    [ ( "patch-mode"
      , [ Alcotest.test_case "unique match replaces" `Quick test_patch_unique_match
        ; Alcotest.test_case "no match returns error" `Quick test_patch_no_match_errors
        ; Alcotest.test_case
            "multi match without replace_all rejected"
            `Quick
            test_patch_multiple_matches_without_replace_all_errors
        ; Alcotest.test_case
        ...
        ; Alcotest.test_case "missing file rejected" `Quick test_patch_missing_file_errors
        ; Alcotest.test_case
            "empty new_string deletes substring"
            `Quick
            test_patch_delete_via_empty_new_string
        ; Alcotest.test_case
            "overwrite mode regression"
            `Quick
