...
Phase 1 (#10365) gave the runtime an in-memory `trust_score` that auto-rotates away from rate-limited / persistently failing providers. Replay validation against 4044 live decisions confirmed the algorithm matches operator intent: dead runtimes (ollama_only at 1% success → trust 0.000 on 99.5% of decisions), healthy runtimes (primary at 41% success → trust 2.000 ceiling).

Two gaps remain:

1. **Restart resets reputation.** Trust lives only in memory. After every server restart the runtime re-learns each provider's persistence pattern from scratch — tens of additional failed turns per restart for the dominant fingerprints.
...
`Runtime_health_tracker.record` invokes the hook after every trust update (under the same mutex).
...
val low_trust_recommendations :
  Runtime_health_tracker.t -> recommendation list
...
`lib/runtime/runtime_config_loader.ml` mtime-based reloader must ignore reloads triggered ≤5s after a self-write. Implementation choices:
...
| Dashboard/persist disagreement | Persist module is the writer; dashboard is the reader. Use the same `Runtime_health_tracker.global` snapshot. No second source of truth. |
...
|---|---|---|
| 2a | Phase 1 has been live ≥ 24h | Recommendation card renders ≥ 1 entry corresponding to a known-bad runtime in replay data |
