...
type msg =
  ...
  | Check_rate_limit_req of
      string * rate_limit_category * agent_role * float * (bool * int) Eio.Promise.u
  ...
  | Push_message of
      ...
      ; timestamp : string
      ; reply : string list Eio.Promise.u
      }
...
let create ?(config = default_rate_limit) () =
  { config; mailbox = Eio.Stream.create max_int }
;;
...
let process_msg config state msg =
  ...
  | Register (agent_name, now, p) ->
    ...
    if existing
    then Log.Session.debug "Session refreshed: %s (total: %d)" agent_name total
    else Log.Session.info "Session registered: %s (total: %d)" agent_name total;
  ...
  | Unregister agent_name ->
    ...
    Log.Session.info
      "Session unregistered: %s (total: %d)"
      agent_name
      (AgentMap.cardinal sessions');
    { state with sessions = sessions' }
  | Update_activity (agent_name, is_listening_opt, now) ->
    (match AgentMap.find_opt agent_name state.sessions with
     | Some session ->
       let is_listen =
         match is_listening_opt with
         | Some v -> v
         | None -> session.is_listening
       in
         ...
         then (
           ...
           then (
             let rec try_add st ev =
               if Eio.Stream.length st >= max_notification_queue
               then (
                 ignore (Eio.Stream.take_nonblocking st);
                 try_add st ev)
               else Eio.Stream.add st ev
             in
             try_add session.message_queue notification;
             targets := name :: !targets)))
      state.sessions;
    if !targets <> [] then Log.Session.debug "Pushed to: %s" (String.concat ", " !targets);
  ...
  | Check_rate_limit_req (agent_name, category, role, now, p) ->
    ...
    in
    let tracker =
      if now -. tracker.last_burst_reset > 60.0
      then { tracker with burst_used = 0; last_burst_reset = now }
      else tracker
    in
    ...
    if current >= limit
    then
      if tracker.burst_used < config.burst_allowed
      then (
        ...
        { state with
          rate_trackers = AgentMap.add agent_name tracker' state.rate_trackers
        })
      else (
  ...
  | Restore_from_disk_req (agents_path, now, p) ->
    ...
    if Sys.file_exists agents_path && Sys.is_directory agents_path
    then (
      ...
      |> Array.iter (fun name ->
        ...
        then (
          ...
          := { !state' with sessions = AgentMap.add agent_name session !state'.sessions };
          incr restored));
      if !restored > 0 then Log.Session.info "Restored %d session(s) from disk" !restored);
...
let update_activity registry ~agent_name ?is_listening () =
  Eio.Stream.add
    registry.mailbox
    (Update_activity (agent_name, is_listening, Time_compat.now ()))
;;
...
let check_rate_limit_ex registry ~agent_name ~category ~role =
  ...
  Eio.Stream.add
    registry.mailbox
    (Check_rate_limit_req (agent_name, category, role, Time_compat.now (), r));
  Eio.Promise.await p
;;
...
let get_rate_limit_status registry ~agent_name ~role =
  ...
  Eio.Stream.add
    registry.mailbox
    (Get_rate_limit_status_req (agent_name, role, Time_compat.now (), r));
  Eio.Promise.await p
;;
...
let push_message registry ~from_agent ~content ~mention =
  ...
  Eio.Stream.add
    registry.mailbox
    (Push_message { from_agent; content; mention; timestamp = now_iso (); reply = r });
  Eio.Promise.await p
;;
...
let get_agent_statuses registry =
  ...
  AgentMap.fold
    (fun name session acc ->
       ...
       let status_icon =
         if session.is_listening then "🎧" else if idle_secs > 60 then "💤" else "🔨"
       in
       let status =
         `Assoc
           [ "name", `String name
...
let status_string registry =
  ...
  if statuses = []
  then "📡 No agents connected."
  else (
    let buf = Buffer.create 256 in
    Buffer.add_string
      buf
      (Printf.sprintf "📡 Connected agents (%d):\n" (List.length statuses));
    ...
    let inactive =
      get_inactive_agents registry ~threshold:Resilience.default_warning_threshold
    in
    if inactive <> []
    then (
      Buffer.add_string buf "\n\n⚠️ **INACTIVE AGENTS**: ";
...
let restore_from_disk registry ~agents_path =
  ...
  Eio.Stream.add
    registry.mailbox
    (Restore_from_disk_req (agents_path, Time_compat.now (), r));
  Eio.Promise.await p
;;
  ...
  let process_msg state msg =
    ...
    | Get (session_id, now, p) ->
      (match AgentMap.find_opt session_id state with
       ...
       | Some session ->
         let session' =
           { session with last_activity = now; request_count = session.request_count + 1 }
         in
    ...
    | Cleanup_stale (now, max_age_val, p) ->
      let state', stale_count =
        AgentMap.fold
          (fun id session (acc_state, acc_count) ->
             if now -. session.last_activity > max_age_val
             then AgentMap.remove id acc_state, acc_count + 1
             else acc_state, acc_count)
          state
          (state, 0)
      in
...
let start_mcp_session_cleanup_loop
      ~sw
      ~clock
      ?(interval = Env_config.Session.max_age_seconds /. 10.0)
      ()
  =
  ...
  Eio.Fiber.fork ~sw (fun () ->
    let rec loop () =
      ...
      (try
         ...
         if removed > 0 then Log.Session.info "Cleaned up %d stale MCP sessions" removed
       with
       | Eio.Cancel.Cancelled _ as e -> raise e
       | exn -> Log.Session.warn "mcp session cleanup failed: %s" (Printexc.to_string exn));
      loop ()
    in
    loop ())
;;
...
let add_mcp_session_header
      (headers : Cohttp.Header.t)
      (session : McpSessionStore.mcp_session)
  : Cohttp.Header.t
  =
...
let handle_mcp_session_tool (arguments : Yojson.Safe.t) : bool * string =
  ...
  | Some "remove" ->
    (match get_string "session_id" with
     | Some id ->
       if McpSessionStore.remove id
       then true, "Session removed"
