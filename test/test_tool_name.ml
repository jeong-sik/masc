...
let test_all_names_unique () =
  ...
  let rec check = function
    | a :: b :: rest ->
      if a = b
      then Alcotest.fail (Printf.sprintf "duplicate tool name: %s" a)
      else check (b :: rest)
    | _ -> ()
  in
  check sorted
...
let test_is_keeper () =
  List.iter
    (fun k -> Alcotest.(check bool) "is_keeper" true (Tool_name.is_keeper (Keeper k)))
    all_keeper;
  List.iter
    (fun m ->
       Alcotest.(check bool) "masc is not keeper" false (Tool_name.is_keeper (Masc m)))
    all_masc
;;
  ...
  let tool_names =
    List.concat_map
      (fun sn ->
         ...
         | Some shard -> List.map (fun (t : Types.tool_schema) -> t.name) shard.tools
         | None -> [])
      shard_names
  in
  let unparsed = List.filter (fun name -> Tool_name.of_string name = None) tool_names in
  if unparsed <> []
  then
    Alcotest.fail
      (Printf.sprintf
         "Tool_shard names not in Tool_name: [%s]"
         (String.concat "; " unparsed))
;;

let test_prefilter_synonyms_parse () =
  let unparsed =
    List.filter (fun name -> Tool_name.of_string name = None) Tool_prefilter.synonym_keys
  in
  if unparsed <> []
  then
    Alcotest.fail
      (Printf.sprintf
         "Tool_prefilter synonyms not in Tool_name: [%s]"
         (String.concat "; " unparsed))
;;
