...
let transition
      ?(prev_phase = KSM.Running)
      ?(new_phase = KSM.Paused)
      ?(selected_event = KSM.Operator_pause)
      ()
  : Audit.transition_record
  =
  { Audit.snapshot = None
...
let test_compact_retry_exhausted_signal_is_alerting () =
  let json =
    Audit.to_json
      (transition ~new_phase:KSM.Paused ~selected_event:KSM.Compact_retry_exhausted ())
  in
  let open Yojson.Safe.Util in
  let signal = json |> member "operator_signal" in
  check
  ...
  check string "class" "context_management" (signal |> member "class" |> to_string);
  ...
  check
    string
    "next action"
    "approve_handoff_or_reduce_context"
    (signal |> member "next_human_action" |> to_string)
;;
...
let () =
  run
    ...
    ; ( "transition_operator_signal"
      , [ test_case
            "operator pause requires decision"
            `Quick
            test_operator_pause_signal_requires_decision
        ; test_case
            "compact retry exhausted is alerting"
            `Quick
            test_compact_retry_exhausted_signal_is_alerting
        ; test_case
            "runtime trust timeline carries signal"
            `Quick
