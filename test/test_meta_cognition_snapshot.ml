...
let temp_dir () =
  incr counter;
  let dir = Filename.temp_file (Printf.sprintf "test_meta_cognition_%d_" !counter) "" in
  Unix.unlink dir;
...
let test_snapshot_detects_signals () =
  ...
  save_jsonl
    ...
    [ post_json
        ...
        ~title:"RBAC blockage"
        ~body:
          "All masc_* tools tested return unregistered_masc_tool. Operator intervention \
           needed. keeper_* tools function normally."
        ~hearth:"ops"
        ~created_at:1000.0
        ()
    ; post_json
        ...
        ~author:"detail-demo"
        ~title:"Cross-check"
        ~body:"Confirmed same unregistered_masc_tool block. This is a policy restriction."
        ~hearth:"ops"
        ~thread_id:"p-root"
        ~created_at:1005.0
        ()
...
let test_snapshot_marks_contested_belief () =
  ...
  save_jsonl
    ...
    [ comment_json
        ...
        ~post_id:"p-root"
        ~author:"keeper-a"
        ~content:"This contradicts the uniform block hypothesis. Access may be per-agent."
        ~created_at:1010.0
        ()
    ];
  let ok, body =
...
let test_parse_summary_preserves_refs_and_secondary_signals () =
  ...
  | Ok parsed ->
    Alcotest.(check (list string))
      "belief evidence refs"
      [ "post:p-root" ]
      (parsed.dominant_belief
       |> Option.map (fun (belief : Meta_cognition.belief_summary) ->
         belief.evidence_refs)
       |> Option.value ~default:[]);
    Alcotest.(check (list string))
      "belief challenge refs"
      [ "comment:c-1" ]
      (parsed.dominant_belief
       |> Option.map (fun (belief : Meta_cognition.belief_summary) ->
         belief.challenge_refs)
       |> Option.value ~default:[]);
    ...
    Alcotest.(check (list string))
      ...
      (interpretation.secondary_saliences |> List.map Meta_cognition.salience_to_string);
    Alcotest.(check (list string))
      "primary evidence refs merged"
...
let test_snapshot_governance_cases_skip_bad_entries_with_metric () =
  ...
  Fs_compat.save_file
    (Filename.concat cases_dir "good.json")
  ...
  Fs_compat.save_file
    (Filename.concat cases_dir "missing-id.json")
    (Yojson.Safe.to_string
       (`Assoc
           [ "title", `String "bad governance case"; "status", `String "pending_ruling" ]));
  ...
  let open Yojson.Safe.Util in
  Alcotest.(check int)
    "valid governance case still counted"
    1
...
let () =
  Alcotest.run
    ...
    [ ( "snapshot"
      , [ Alcotest.test_case "detects signals" `Quick test_snapshot_detects_signals
        ; Alcotest.test_case
            "marks contested belief"
            `Quick
            test_snapshot_marks_contested_belief
        ; Alcotest.test_case
            "parses summary refs and secondary signals"
            `Quick
            test_parse_summary_preserves_refs_and_secondary_signals
        ; Alcotest.test_case
            "governance cases skip bad entries with metric"
            `Quick
            test_snapshot_governance_cases_skip_bad_entries_with_metric
        ] )
    ]
;;
