...
open Tool_args
open Tool_misc_snapshots

module U = Yojson.Safe.Util

type result = bool * string

(* Re-export context type and externally used helpers *)
type context = Tool_misc_snapshots.context = {
  config: Room.config;
  agent_name: string;
}

let tool_inventory_json = Tool_misc_snapshots.tool_inventory_json
