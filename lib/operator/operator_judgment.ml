...
let operator_dir config = Filename.concat (Coord.masc_dir config) "operator"
let judgments_path config = Filename.concat (operator_dir config) "judgments.jsonl"
let generate_id () = "judg-" ^ String.sub (Auth.generate_token ()) 0 20
...
let to_yojson (value : record) =
  `Assoc
    ...
    ; "evidence_refs", `List (List.map (fun item -> `String item) value.evidence_refs)
...
let of_yojson json =
  try
    ...
    Ok
      ...
      ; status =
          json |> member "status" |> to_string_option |> Option.value ~default:"active"
      ...
      ; supersedes =
          (match json |> member "supersedes" with
           ...
           | _ -> [])
      ; fallback_used =
          json |> member "fallback_used" |> to_bool_option |> Option.value ~default:false
  ...
  with
  | Type_error (msg, _) | Failure msg -> Error msg
;;

let generated_at_unix value = value.generated_at_unix
let fresh_until_unix value = value.fresh_until_unix
let is_fresh ?(now = Unix.gettimeofday ()) value = fresh_until_unix value > now
...
let append config values =
  ...
  let path = judgments_path config in
  List.iter (fun value -> Fs_compat.append_jsonl path (to_yojson value)) values
;;
