(** Tests for Dashboard_gate_metrics — tool rejection ring + approval queue. *)

module GM = Dashboard_gate_metrics
...
let test_record_failure_is_metric_visible () =
  let labels =
    [ ("callback", "dashboard_gate_tool_skipped_record") ]
  in
  let before =
