...
let test_verify_handoff_factual_drift () =
  let original =
    "Completed tasks: 1. Setup database 2. Create API endpoints 3. Write tests 4. Deploy \
     to staging 5. Monitor logs"
  in
...
let test_verify_and_log_and_stats () =
  with_temp_masc_dir (fun config ->
    ...
    let total, drift_count, avg_similarity = Drift_guard.get_drift_stats config ~days:7 in
    check int "two records" 2 total;
    check int "one drift" 1 drift_count;
    check bool "avg in range" true (avg_similarity >= 0.0 && avg_similarity <= 1.0))
;;
...
let () =
  run
    "Drift_guard core"
    [ "similarity", [ test_case "identical text" `Quick test_text_similarity_identical ]
    ; ( "verify_handoff"
      , [ test_case "verified" `Quick test_verify_handoff_verified
        ; test_case "factual drift" `Quick test_verify_handoff_factual_drift
        ] )
    ; ( "logging"
      , [ test_case "verify_and_log + stats" `Quick test_verify_and_log_and_stats ] )
