...
- **현재 상태:** rich-text/multimedia meta는 저장되나 dashboard 미렌더; proactive attention candidate는 producer가 `record_and_notify`로 durable row를 먼저 기록하고, process-owned partition worker가 owner turn admission 밖에서 exact-context cohort를 판정한다. owner heartbeat는 completed 결과의 idempotent delivery/settlement만 수행한다. fixed candidate cap은 없고 typed response-contract failure만 결정론적 partition split을 허용하며 provider/runtime failure는 `Deferred`, singleton contract failure는 `Blocked`로 보존한다. claim-evidence gate가 opt-in이라 free-text "task done" 완료주장이 검증 우회.
