...
### 2.2 Keeper_msg_async (대안 패턴; timeout 설명은 폐기됨)

> 2026-07-13: 이 절의 request-level worker timeout 설명은 폐기됐다.
> Keeper 메시지는 전체 turn wall-clock deadline을 소유하지 않으며,
> provider progress, tool-local deadline, typed completion, operator cancellation이
> 각각의 경계를 소유한다.

submit→`request_id` 동기 반환, 서버 수명 워커 + 워커 내부 **자체 child `Eio.Switch.run` 격리**, FSM(`Queued→Running→{Done|Cancelled|Lost}`) + 디스크 영속(재시작 후 Lost 복구) + 개별 cancel, `masc_keeper_msg_result` poll.
