...
let essential_tools =
  ...
  ; make_schema "masc_broadcast" "Send a message visible to ALL agents via SSE push."
  ; make_schema "masc_join" "Join the MASC namespace to collaborate with other AI agents."
  ; make_schema "masc_leave" "Leave the MASC namespace and mark yourself as offline."
  ; make_schema
      "masc_status"
  ...
  ; make_schema "masc_tasks" "List tasks in backlog with their status and assignee."
  ; make_schema "masc_add_task" "Add a new task to the backlog for agents to claim."
  ; make_schema "masc_claim_next" "Claim the highest priority unclaimed task."
  ; make_schema "masc_plan_init" "Initialize a planning context for a task."
  ; make_schema
      "masc_plan_get"
      "Retrieve the full planning context for a task as markdown."
  ; make_schema "masc_plan_update" "Overwrite the current task plan with new content."
  ]
;;
...
let extended_tools =
  ...
  @ [ make_schema
    ...
    ; make_schema "masc_code_read" "Read the contents of a source file."
    ; make_schema "masc_autoresearch_start" "Start an automated research cycle."
    ; make_schema
        "masc_worktree_create"
        "Create a new git worktree for an isolated branch."
    ; make_schema "masc_worktree_list" "List all active git worktrees."
    ; make_schema "masc_agent_card" "Retrieve the agent card and profile information."
    ; make_schema "masc_auth_status" "Check authentication and token credential status."
    ; make_schema "masc_web_search" "Search the internet for information."
    ]
;;

let names_of results = List.map (fun (s : Types.tool_schema) -> s.name) results
let has_tool name results = List.mem name (names_of results)
...
let test_heartbeat_in_top3 () =
  let result =
    Tool_prefilter.filter ~tools:essential_tools ~query:"send heartbeat" ~k:3
  in
  check bool "masc_heartbeat in top-3" true (has_tool "masc_heartbeat" result)
;;

let test_broadcast_in_top3 () =
  let result =
    Tool_prefilter.filter
      ~tools:essential_tools
      ~query:"broadcast a message to all agents"
      ~k:3
  in
  check bool "masc_broadcast in top-3" true (has_tool "masc_broadcast" result)
;;

let test_join_in_top3 () =
  let result =
    Tool_prefilter.filter ~tools:essential_tools ~query:"join the namespace" ~k:3
  in
  check bool "masc_join in top-3" true (has_tool "masc_join" result)
;;

let test_add_task_in_top3 () =
  let result =
    Tool_prefilter.filter ~tools:essential_tools ~query:"create a new task" ~k:3
  in
  check bool "masc_add_task in top-3" true (has_tool "masc_add_task" result)
;;
...
let test_synonym_broadcast () =
  let result =
    Tool_prefilter.filter
      ~tools:essential_tools
      ~query:"let everyone know that CI is fixed"
      ~k:3
  in
  check bool "synonym: broadcast via 'everyone'" true (has_tool "masc_broadcast" result)
;;

let test_synonym_dashboard () =
  let result =
    Tool_prefilter.filter ~tools:essential_tools ~query:"show me activity overview" ~k:3
  in
  check
    bool
    "synonym: dashboard via 'activity overview'"
    true
...
let test_synonym_claim_next () =
  let result =
    Tool_prefilter.filter ~tools:essential_tools ~query:"give me work to do" ~k:3
  in
  check
    bool
    "synonym: claim_next via 'give me work'"
    true
...
let test_synonym_code_search () =
  ...
  check
    bool
    "synonym: masc_code_search via 'codebase search'"
    true
...
let test_synonym_code_read () =
  let result =
    Tool_prefilter.filter ~tools:extended_tools ~query:"read source file contents" ~k:3
  in
  check
    bool
    "synonym: masc_code_read via 'read source'"
    true
...
let test_synonym_autoresearch_start () =
  let result =
    Tool_prefilter.filter ~tools:extended_tools ~query:"begin auto research" ~k:3
  in
  check
    bool
    "synonym: masc_autoresearch_start via 'begin research'"
    true
...
let test_synonym_worktree_create () =
  let result =
    Tool_prefilter.filter ~tools:extended_tools ~query:"create a new worktree" ~k:3
  in
  check
    bool
    "synonym: masc_worktree_create via 'create worktree'"
    true
...
let test_synonym_web_search () =
  ...
  check
    bool
    "synonym: masc_web_search via 'search internet'"
    true
...
let test_empty_query () =
  let result = Tool_prefilter.filter ~tools:essential_tools ~query:"" ~k:3 in
...
let test_disjoint_query () =
  let result =
    Tool_prefilter.filter ~tools:essential_tools ~query:"xyzzy foobar bazzle" ~k:3
  in
...
let test_k_exceeds_tools () =
  ...
  check
    bool
    "returns at most available non-zero tools"
    true
...
let test_scores_descending () =
  let scored =
    Tool_prefilter.filter_with_scores
      ~tools:essential_tools
      ~query:"send heartbeat ping"
      ~k:5
  in
...
let test_scores_positive () =
  let scored =
    Tool_prefilter.filter_with_scores ~tools:essential_tools ~query:"send heartbeat" ~k:5
  in
...
let test_synonym_text_enriches_description () =
  ...
  check bool "enriched longer than base" true (String.length enriched > String.length base)
;;
  ...
  run
    ...
    ; ( "synonyms"
      , [ test_case "broadcast via synonym" `Quick test_synonym_broadcast
        ...
        ; test_case
            "autoresearch_start via synonym"
            `Quick
            test_synonym_autoresearch_start
