...
let test_keeper_surface_status_preserves_live_agent_states () =
  let status =
    ES.keeper_surface_status
      ~agent_status:(`Assoc [ "exists", `Bool true; "status", `String "busy" ])
...
let test_keeper_surface_status_maps_stale_to_inactive () =
  let status =
    ES.keeper_surface_status
      ~agent_status:(`Assoc [ "exists", `Bool true; "status", `String "active" ])
...
let test_keeper_surface_status_maps_degraded_to_inactive () =
  let status =
    ES.keeper_surface_status
      ~agent_status:(`Assoc [ "exists", `Bool true; "status", `String "listening" ])
...
let test_keeper_status_helpers_tolerate_null_status_json () =
  ...
  check
    bool
    "null agent status has no live signal"
    false
    (ES.agent_runtime_has_live_signal `Null);
  check
    string
    "null surface inputs fall back offline"
    "offline"
...
let make_agent_status
      ?(exists = true)
      ?(status = "active")
      ?(last_seen_ago_s = 10.0)
      ?(is_zombie = false)
      ()
  : Yojson.Safe.t
  =
...
let test_keeper_diagnostic_tolerates_null_agent_status () =
  ...
  check
    string
    "null agent status maps diagnostic offline"
    "offline"
    (diagnostic |> member "health_state" |> to_string);
  check
    string
    "null agent status maps quiet reason"
    "agent_missing"
...
let test_health_keepalive_running_overrides_stale_last_seen () =
  ...
  check
    bool
    "keepalive running + last_seen at boundary is NOT stale"
    true
    (health <> KT.KH_stale);
  check
    bool
    "keepalive running + last_seen at boundary is NOT offline"
    true
    (health <> KT.KH_offline)
;;
...
let test_health_keepalive_stuck_secondary_timeout () =
  ...
  in
  check
    keeper_health_testable
    "keepalive running but last_seen > 2x interval → stale (stuck fiber)"
    KT.KH_stale
    health
;;
...
let test_health_keepalive_below_secondary_timeout () =
  ...
  check
    bool
    "keepalive running + last_seen just below 2x interval is NOT stale"
    true
    (health <> KT.KH_stale)
;;
...
let test_health_keepalive_stuck_respects_interval () =
  ...
  check
    keeper_health_testable
    "keepalive running + last_seen > 2x configured interval → stale"
    KT.KH_stale
    health
;;
  ...
  let meta =
    ...
    { base with
      runtime =
        { base.runtime with
          usage =
            { base.runtime.usage with total_turns = 5; last_turn_ts = Time_compat.now () }
        }
    }
  in
...
let test_health_keepalive_running_recent_live_signal_avoids_idle () =
  ...
  let meta =
    ...
    { base with
      runtime =
        { base.runtime with
          usage =
            { base.runtime.usage with total_turns = 5; last_turn_ts = now_ts -. 3600.0 }
        }
    }
  in
  ...
  check
    keeper_health_testable
    "fresh live signal keeps keeper healthy despite stale last turn"
    KT.KH_healthy
    health
;;
...
let test_diagnostic_ignores_stale_error_when_live_signal_is_newer () =
  ...
  let meta =
    ...
    ; runtime =
        { base.runtime with
          usage =
            { base.runtime.usage with total_turns = 5; last_turn_ts = stale_error_ts }
        ; proactive_rt =
            { base.runtime.proactive_rt with
              count_total = 5
            ; last_ts = stale_error_ts
            ; last_outcome = KT.Proactive_error
            ; last_reason = "unified:error:Timeout: Execution cancelled after 300.0s"
  ...
  check
    string
    "fresh live signal suppresses stale degraded status"
    "healthy"
    (diagnostic |> member "health_state" |> to_string)
;;
...
let test_runtime_surface_derives_autonomous_slot_wait_timeout_from_meta () =
  ...
  let reason =
    "autonomous turn slot wait timeout after 30.0s (limit=30.0s, wait_ms=30000); skipped \
     cycle before OAS run"
  in
  let meta = { base with runtime = { base.runtime with last_blocker = reason } } in
  ...
  check
    bool
    "runtime blocker continue gate stays false"
    false
...
let test_runtime_surface_derives_cascade_exhausted_from_meta () =
  ...
  let reason =
    "Internal error: [masc_oas_error] \
     {\"kind\":\"cascade_exhausted\",\"cascade_name\":\"big_three\",\"detail\":\"all \
     providers failed: Connection refused\"}"
  in
  let meta = { base with runtime = { base.runtime with last_blocker = reason } } in
  let config = Coord.default_config "/tmp/test-keeper-exec-status-cascade-exhausted" in
  let runtime = KSB.runtime_surface_json config meta in
  let open Yojson.Safe.Util in
  check
    string
...
let test_runtime_surface_exposes_redacted_resumable_cli_session_blocker () =
  ...
  let meta =
    { base with
      runtime =
        { base.runtime with
          last_blocker = reason
        ; last_blocker_class = Some (KT.Cascade_exhausted (KT.Other_detail reason))
        }
    }
  in
  let config =
  ...
  check
    ...
    "cascade_exhausted"
    (runtime |> member "runtime_blocker_class" |> to_string);
  check string "runtime blocker summary stays redacted" reason summary;
  check
    bool
    "runtime blocker hides raw session command"
    false
...
let test_runtime_surface_derives_continue_gate_from_ambiguous_partial_commit () =
  ...
  let detail = "Mutating tools [keeper_fs_edit] committed before the turn timed out." in
  let config = Coord.default_config "/tmp/test-keeper-exec-status-continue-gate" in
  ignore (KR.register ~base_path:config.base_path meta.name meta);
  KR.set_failure_reason
    ~base_path:config.base_path
    meta.name
    (Some (KR.Ambiguous_partial_commit { kind = KR.Post_commit_timeout; detail }));
  let runtime = KSB.runtime_surface_json config meta in
  let open Yojson.Safe.Util in
  check
    string
...
let test_runtime_surface_derives_continue_gate_from_persisted_ambiguous_blocker () =
  ...
  let reason =
    "turn outcome ambiguous after committed mutating tool call(s): \
     [keeper_board_cleanup]; retry disabled to avoid duplicate mutation; \
     original_error=Completion contract [require_tool_use] violated"
  in
  let meta =
    { base with paused = true; runtime = { base.runtime with last_blocker = reason } }
  in
  let config =
...
let test_runtime_surface_suppresses_stale_proactive_timeout_reason () =
  ...
  let meta =
    { base with
      runtime =
        { base.runtime with
          usage = { base.runtime.usage with total_turns = 3; last_turn_ts = now_ts }
        ; proactive_rt =
            { base.runtime.proactive_rt with
              last_ts = now_ts -. 600.0
            ; last_outcome = KT.Proactive_error
            ; last_reason =
                "unified:error:Internal error: Turn wall-clock timeout after 3600s \
                 (MASC_KEEPER_TURN_TIMEOUT_SEC)"
            ; last_preview =
                "Internal error: Turn wall-clock timeout after 3600s \
                 (MASC_KEEPER_TURN_TIMEOUT_SEC)"
            }
        }
    }
  ...
  check
    (option string)
    "stale proactive timeout suppressed"
    None
    (runtime |> member "runtime_blocker_summary" |> json_string_opt);
  check
    (option string)
    "stale proactive blocker class suppressed"
    None
...
let test_runtime_surface_exposes_social_model_resolution_fields () =
  ...
  check
    string
    "configured social model preserved"
    "experimental_v99"
  ...
  check
    string
    "delivery surface view"
    "silent"
    (runtime |> member "delivery_surface_view" |> to_string);
  check
    string
    "delivery surface view source"
...
let test_runtime_surface_exposes_model_display_labels () =
  ...
  let meta =
    { base with
      runtime =
        { base.runtime with
          usage = { base.runtime.usage with last_model_used = "codex" }
        }
    }
  in
  let config = Coord.default_config "/tmp/test-keeper-exec-status-model-display-labels" in
  let runtime = KSB.runtime_surface_json config meta in
  let open Yojson.Safe.Util in
  let active_model_label = runtime |> member "active_model_label" |> to_string in
  check
    bool
...
let test_parser_roundtrip_all_constructors () =
  let all : KT.keeper_health list =
    [ KH_healthy; KH_idle; KH_offline; KH_stale; KH_degraded; KH_zombie; KH_dead ]
  in
  List.iter
    (fun h ->
...
let test_parser_rejects_unknown () =
  Alcotest.(check (option string))
    "typo → None"
    None
    (Option.map ES.keeper_health_to_string (ES.keeper_health_of_string_opt "healty"));
  Alcotest.(check (option string))
    "future variant → None"
    None
    (Option.map ES.keeper_health_to_string (ES.keeper_health_of_string_opt "compacting"))
;;
