...
let test_client_count_linearized_under_domain_contention () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         int
         "count after concurrent register"
         (count_before + worker_count)
         (client_count ());
       run_domains_together worker_count (fun index -> unregister (session_id index));
       check
         int
         "count restored after concurrent unregister"
         count_before
         (client_count ()))
;;
...
let () =
  run
    ...
    [ ( "unregister_if_current"
      , [ test_case "guards reconnect" `Quick test_unregister_if_current ] )
    ; ( "cleanup_stale"
      , [ test_case "uses idle time" `Quick test_cleanup_stale_respects_touch ] )
    ; ( "concurrency"
      , [ test_case
        ...
        ; test_case
            "client_count linearized under domain contention"
            `Quick
