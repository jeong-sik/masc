...
### Added
- MASC now has a draft durable OAS execution leaf on its existing runtime and local-worker surfaces. Opaque recovery locators, exact checkpoint/slot matching, and fail-closed unknown settlement are wired; `Runtime_agent.run*` rejects missing stable session/checkpoint ownership instead of synthesizing an identity or entering a non-durable compatibility call. Terminal retirement and projection retention remain explicit blockers until the newer typed OAS contracts are released and pinned.

### Fixed
- Direct Keeper turns now retain acquired OAS execution ownership across every post-run exception or cancellation, heartbeat settlement finds nested manual-compaction completions and retains them when lease settlement cannot commit, and the standalone local-worker executable initializes its own BasePath-scoped OAS execution runtime before dispatch. No path restores a non-durable fallback.
- OAS recovery no longer holds one global Keeper mutex across slot I/O, fsync, execution-store construction, or settled-scope cleanup. The process-wide registry now uses a short cross-domain-safe `Stdlib.Mutex` section only for exact recovery-key claim/release, while each prepared execution serializes only its own settlement with `Eio.Mutex`; direct Unix fsync/link/lstat/unlink calls run in Eio system threads, so unrelated Keeper fibers and lanes remain independent.
- OAS recovery records no longer use an arbitrary hardcoded byte budget as an admission gate. Private slot files are accepted or rejected by the exact closed MASC record decoder and opaque OAS locator decoder; malformed, unknown, or mismatched state still fails explicitly.
