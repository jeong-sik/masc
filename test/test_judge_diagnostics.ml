...
let test_long_input_truncated_with_marker () =
  ...
  check
    bool
    "ends with chars marker"
    true
    (try
       let re = Re.Pcre.re {|\+\d+ chars\]$|} |> Re.compile in
       ignore (Re.exec re out);
       true
     with
     | Not_found -> false)
;;
...
let test_format_lenient_fallback_truncates_huge_raw () =
  ...
  check
    ...
    (try
     ...
     with
     | Not_found -> false);
  check bool "preview is bounded" true (String.length out < 1500)
;;

let () =
  run
    "judge_diagnostics (#9774)"
    [ ( "truncate_with_marker"
      , [ test_case "short input passes through" `Quick test_short_input_passes_through
        ; test_case
            "long input truncated with marker"
            `Quick
            test_long_input_truncated_with_marker
        ; test_case "custom max_bytes respected" `Quick test_custom_max_bytes_respected
        ] )
    ; ( "format_lenient_fallback"
      , [ test_case
            "includes judge label, size, preview, class"
            `Quick
            test_format_lenient_fallback_includes_label
        ; test_case
            "preview is bounded for huge raw"
            `Quick
