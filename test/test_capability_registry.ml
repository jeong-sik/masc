...
let test_public_visible_surface_hides_deprecated_aliases () =
  ...
  in
  check bool "public contains masc_transition" true (List.mem "masc_transition" names)
;;
...
let test_public_visible_surface_hides_pruned_voice_tools () =
  let names =
    ...
    |> List.map (fun (schema : Types.tool_schema) -> schema.name)
  in
  check bool "public omits masc_voice_agent" false (List.mem "masc_voice_agent" names);
  check bool "public omits masc_voice_speak" false (List.mem "masc_voice_speak" names);
  check
    bool
    "public omits masc_voice_ping_pong"
    false
...
let test_board_post_capability_merges_public_and_keeper_projections () =
  let capability =
    ...
    |> List.find (fun (capability : Lib.Capability_registry.capability_def) ->
      String.equal capability.Lib.Capability_registry.capability_id "masc_board_post")
  in
...
let test_local_worker_projection_exposes_internal_and_auditable_tools () =
  match
    Lib.Capability_registry.local_worker_tool_schemas
      ~names:[ "masc_heartbeat"; "masc_code_search"; "masc_run_plan" ]
      ()
  with
  | Error err -> failf "expected local worker schemas: %s" err
  | Ok schemas ->
    let names = List.map (fun (schema : Types.tool_schema) -> schema.name) schemas in
...
let test_spawned_agent_surface_stays_curated () =
  ...
  check bool "contains masc_status" true (List.mem "mcp__masc__masc_status" names);
  check bool "omits a2a_delegate" false (List.mem "mcp__masc__masc_a2a_delegate" names);
  check bool "omits portal_send" false (List.mem "mcp__masc__masc_portal_send" names);
  check bool "omits voice agent" false (List.mem "mcp__masc__masc_voice_agent" names);
  check bool "omits voice speak" false (List.mem "mcp__masc__masc_voice_speak" names);
  check
    bool
    "omits voice ping pong"
    false
...
let test_privileged_keeper_surface_is_split () =
  check
    bool
    "keeper_bash privileged"
    true
    (List.mem "keeper_bash" Lib.Capability_registry.keeper_privileged_tool_names);
  check
    bool
    "keeper_fs_edit privileged"
    true
    (List.mem "keeper_fs_edit" Lib.Capability_registry.keeper_privileged_tool_names);
  check
    bool
    "keeper_fs_read standard"
    true
    (List.mem "keeper_fs_read" Lib.Capability_registry.keeper_safe_tool_names);
  check
    bool
    "keeper_board_post not privileged"
    false
    (List.mem "keeper_board_post" Lib.Capability_registry.keeper_privileged_tool_names)
;;
