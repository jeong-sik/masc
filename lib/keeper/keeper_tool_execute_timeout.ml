...
let keeper_tool_execute_shell_ir_native_min_timeout_sec =
...
let rec keeper_tool_execute_shell_ir_args_need_tool_dispatch_floor = function
  | `Assoc fields ->
    typed_stage_needs_tool_dispatch_floor fields
    ||
    (match List.assoc_opt "pipeline" fields with
     | Some (`List stages) ->
       List.exists keeper_tool_execute_shell_ir_args_need_tool_dispatch_floor stages
     | _other -> false)
  | _other -> false
;;

let keeper_tool_execute_shell_ir_min_timeout_sec_for_args args =
  if keeper_tool_execute_shell_ir_args_need_tool_dispatch_floor args
  then tool_dispatch_min_timeout_sec
  else keeper_tool_execute_shell_ir_native_min_timeout_sec
;;
