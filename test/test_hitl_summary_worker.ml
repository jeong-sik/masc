...
let test_cancellation_after_dispatch_is_terminal () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       (* Boot recovery leaves this row alone. A cancelled judgment reached
          no verdict and nothing re-runs one: [reserve_summary_attempt_retry]
          has no arm for a bound quarantined row. The row stays pending and
          an operator decides it. *)
       install_queue base_path;
       let recovery = Gate.resume_persisted_auto_judges ~base_path in
       check int "boot recovery does not request the cancelled row" 0 recovery.requested;
       check (list string) "and starts no worker for it" [] recovery.started_ids;
       check bool
         "nor reports it skipped"
         false
         (List.mem entry.id recovery.skipped_ids);
       check int "and records no failure" 0 (List.length recovery.failures))
;;
       ...
       let execute input_tag =
         ...
         let runner ~runtime_id ~system_prompt:_ ~output_schema:_ ~prompt:_ =
           ...
           if String.equal runtime_id cli_primary then (
             ...
             Error (Masc.Fusion_official_client.Setup_failure (Fusion_types.Provider_error "typed adapter quota already recorded")))
...
let test_cli_walk_exhaustion_quarantines_the_last_cli_identity () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let entry = pending_entry ~base_path () in
       let runner ~runtime_id:_ ~system_prompt:_ ~output_schema:_ ~prompt:_ =
         Error (Masc.Fusion_official_client.Setup_failure (Fusion_types.Provider_error "subscription window exhausted"))
       in
       Worker.For_testing.execute_prepared_flow_with_queue_ops
         ~queue_ops:(exact_queue_ops ())
         ~cli_runner:runner
