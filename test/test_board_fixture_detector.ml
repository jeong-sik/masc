...
let test_real_keeper_names_not_flagged () =
  ...
  List.iter
    (fun s ->
       Alcotest.(check (pair string bool)) s (s, false) (s, BV.is_fixture_voter_target s))
    cases
;;
...
let test_empty_target_not_flagged () =
  Alcotest.(check bool) "empty string safe" false (BV.is_fixture_voter_target "");
  Alcotest.(check bool) "partial keyword safe" false (BV.is_fixture_voter_target "hot")
;;

let () =
  Alcotest.run
    "board_fixture_detector"
    [ ( "fixture patterns"
      , [ Alcotest.test_case "hot-voter-*" `Quick test_hot_voter_flagged
        ; Alcotest.test_case "synthetic-voter-*" `Quick test_synthetic_voter_flagged
        ; Alcotest.test_case ":test-voter-*" `Quick test_test_voter_flagged
        ; Alcotest.test_case "empty / partial safe" `Quick test_empty_target_not_flagged
        ] )
    ; ( "false positives"
      , [ Alcotest.test_case
            "real keeper names NOT flagged"
            `Quick
            test_real_keeper_names_not_flagged
        ] )
    ; ( "quarantine env gate"
      , [ Alcotest.test_case "default off" `Quick test_quarantine_default_off
        ; Alcotest.test_case "truthy values → on" `Quick test_quarantine_on_values
        ] )
    ]
;;
