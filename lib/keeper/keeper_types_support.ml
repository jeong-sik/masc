...
let execution_receipt_store_cache : (string, Dated_jsonl.t) Hashtbl.t = Hashtbl.create 8
let execution_receipt_store_mu = Eio.Mutex.create ()

let keeper_execution_receipt_store config name : Dated_jsonl.t =
  let dir = Filename.concat (keeper_dir_ config) (name ^ "/execution-receipts") in
  let lookup () =
...
let keeper_alerts_path config = Filename.concat (keeper_dir_ config) "_alerts.jsonl"
