...
module Session = struct
  ...
  let find id = with_lock (fun () -> StringMap.find_opt id !sessions)
  let touch session = session.last_seen <- Time_compat.now ()
  let remove id = with_lock (fun () -> sessions := StringMap.remove id !sessions)

  let list_all () =
    with_lock (fun () -> !sessions |> StringMap.bindings |> List.map (fun (_, v) -> v))
  ;;
  ...
  let cleanup ~ttl_seconds =
    ...
    with_lock (fun () ->
      let to_remove =
        StringMap.fold
          (fun id session acc -> if session.last_seen < cutoff then id :: acc else acc)
          !sessions
          []
      in
...
let jsonrpc_is_valid_request = function
  | `Assoc fields -> List.mem_assoc "jsonrpc" fields && List.mem_assoc "method" fields
  | _ -> false
;;
...
let jsonrpc_dispatch_request (handler : request_handler) request =
  ...
  | exn ->
    jsonrpc_error_response
      ...
      ~code:(-32603)
      ~message:
        (Log.Server.error "streamable_http dispatch: %s" (Printexc.to_string exn);
         "Internal error")
;;
...
let handle_post ?session_id ~body ?request_handler () =
  ...
  in
  let request_handler =
    Option.value request_handler ~default:(fun request ->
      let id = jsonrpc_extract_id request in
      if jsonrpc_is_valid_request request
      then
        jsonrpc_error_response
          ~id
          ~code:(-32601)
          ~message:"Method not found: no request handler configured"
      else jsonrpc_error_response ~id ~code:(-32600) ~message:"Invalid Request")
  in
  match json_result with
  | Error _ -> Error_response (400, "Invalid JSON"), None
  | Ok json ->
    ...
    if jsonrpc_is_batch json
    then Error_response (400, "JSON-RPC batch requests are not supported"), session
    else if jsonrpc_is_valid_request json
    then (
    ...
    else Error_response (400, "Invalid JSON-RPC request"), session
;;
...
let is_streamable_request (request : Httpun.Request.t) =
  ...
  let is_json =
    ...
    | Some ct ->
      String.lowercase_ascii ct
      |> fun s -> String.sub s 0 (min 16 (String.length s)) = "application/json"
    | None -> false
  in
...
let with_session_header session headers = ("mcp-session-id", session.id) :: headers
