...
let with_room f =
  ...
  let dir =
    Filename.concat
      ...
      (Printf.sprintf "test_keeper_tag_dispatch_%d" (Random.int 1_000_000))
  in
...
let test_pause_status_allowed () =
  with_room (fun config ->
    ...
    | None -> fail "masc_pause_status returned None (tool not recognized)")
;;
...
let test_pause_blocked () =
  with_room (fun config ->
    ...
    | Some (false, msg) ->
      check bool "error mentions blocked" true (contains_substring msg "blocked")
    | Some (true, _) -> fail "masc_pause should be blocked in keeper context"
    | None -> fail "masc_pause returned None")
;;
...
let test_resume_blocked () =
  with_room (fun config ->
    ...
    | Some (false, msg) ->
      check bool "error mentions blocked" true (contains_substring msg "blocked")
    | Some (true, _) -> fail "masc_resume should be blocked in keeper context"
    | None -> fail "masc_resume returned None")
;;
