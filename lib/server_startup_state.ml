...
let to_product (current : t) : Server_state_product.product =
  ...
  in
  let readiness = if current.state_ready then Readiness.Ready else Readiness.NotReady in
  { lifecycle
  ; backend
  ; lazy_tasks
  ; readiness
...
let derive_phase (product : Server_state_product.product) : phase =
  ...
  | Some _
    when product.lifecycle = Lifecycle.Serving && product.readiness = Readiness.Ready ->
    Degraded
  | _ ->
...
let of_product
      (product : Server_state_product.product)
      (started_at : float)
      (path_diagnostics : Yojson.Safe.t option)
      (config_resolution : Yojson.Safe.t option)
  : t
...
let is_live () = true
let elapsed_since_start () = Unix.gettimeofday () -. !state.started_at
let default_watchdog_timeout_sec = 240.0
let watchdog_timeout_sec () = Env_config.Transport.startup_watchdog_sec ()
let pending_lazy_tasks () = !state.pending_lazy_tasks
let lazy_tasks_complete () = !state.pending_lazy_tasks = []
