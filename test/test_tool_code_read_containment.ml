...
let test name f = test_cases := (name, f) :: !test_cases
...
let make_fixture ~tag =
  ...
  let own_abs =
    try Unix.realpath (Filename.concat base_path own_rel) with
    | Unix.Unix_error _ -> Filename.concat base_path own_rel
  in
  let other_abs =
    try Unix.realpath (Filename.concat base_path other_rel) with
    | Unix.Unix_error _ -> Filename.concat base_path other_rel
  in
...
let () =
  test "shared_codebase_read_allowed" (fun () ->
    ...
    match Tool_code.validate_read_path ~agent_name:"agent-alpha" config shared_abs with
    | Ok resolved ->
      let shared_real =
        try Unix.realpath shared_abs with
        | Unix.Unix_error _ -> shared_abs
      in
      assert (resolved = shared_real)
    | Error e ->
      failwith
        (Printf.sprintf
           "expected Ok on shared codebase, got Error %s"
...
let () =
  test "own_playground_read_allowed" (fun () ->
    ...
    match Tool_code.validate_read_path ~agent_name:"agent-alpha" config own_file with
    | Ok resolved ->
      let own_file_real =
        try Unix.realpath own_file with
        | Unix.Unix_error _ -> own_file
      in
      assert (resolved = own_file_real)
    | Error e ->
      failwith
        (Printf.sprintf
           "expected Ok on own playground, got Error %s"
           (Types.masc_error_to_string e)))
;;
  ...
  test "relative_repos_prefix_maps_to_own_playground" (fun () ->
    ...
    match
      Tool_code.validate_read_path
        ...
        config
        "repos/masc-mcp/lib/demo.ml"
    with
    | Ok resolved ->
      let expected_real =
        try Unix.realpath expected with
        | Unix.Unix_error _ -> expected
      in
...
let () =
  test "cross_keeper_playground_read_blocked" (fun () ->
    ...
    match Tool_code.validate_read_path ~agent_name:"agent-alpha" config victim_file with
    | Ok resolved ->
      failwith (Printf.sprintf "expected Error on cross-keeper read, got Ok %S" resolved)
    | Error e ->
...
let () =
  test "outside_git_root_still_rejected_as_traversal" (fun () ->
    ...
    match Tool_code.validate_read_path ~agent_name:"agent-alpha" config "/etc/passwd" with
    | Ok resolved ->
      failwith
        (Printf.sprintf "expected Error on /etc/passwd traversal, got Ok %S" resolved)
    | Error e ->
      let msg = Types.masc_error_to_string e in
...
let () =
  test "own_playground_root_allowed" (fun () ->
    ...
    match Tool_code.validate_read_path ~agent_name:"agent-alpha" config own_abs with
    | Ok _ -> ()
    | Error e ->
      failwith
        (Printf.sprintf
...
let () =
  test "other_playground_root_blocked" (fun () ->
    ...
    match Tool_code.validate_read_path ~agent_name:"agent-alpha" config other_abs with
    | Ok resolved ->
      failwith
        (Printf.sprintf "expected Error on other playground root, got Ok %S" resolved)
    | Error e ->
      let msg = Types.masc_error_to_string e in
      assert (contains_substring msg "cross-keeper playground read blocked"))
;;
...
let () =
  test "symlink_from_own_to_other_keeper_blocked" (fun () ->
    ...
    match Tool_code.validate_read_path ~agent_name:"agent-alpha" config sneaky_link with
    | Ok resolved ->
      failwith
        (Printf.sprintf "expected Error on symlink to other keeper, got Ok %S" resolved)
    | Error e ->
      let msg = Types.masc_error_to_string e in
...
let () =
  test "missing_playground_fails_closed" (fun () ->
    ...
    let target = Filename.concat base_path ".masc/playground/agent-alpha/absent" in
    ...
    match Tool_code.validate_read_path ~agent_name:"agent-alpha" config target with
    | Ok resolved ->
    ...
    | Error e ->
      let msg = Types.masc_error_to_string e in
      assert (
        contains_substring msg "does not exist" || contains_substring msg "cross-keeper"))
;;

let () =
  Alcotest.run
    "Tool_code_read_containment"
    [ ( "containment"
      , List.rev !test_cases
