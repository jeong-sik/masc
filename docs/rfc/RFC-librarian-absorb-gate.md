...
관문을 껐거나 Keeper 를 제외했으면 Librarian 결정을 그대로 적용하고 `skipped`와 이유를
남긴다. 관문을 켰는데 물을 수 없으면(레인 꺼짐, 키 없음) 아무것도 흡수하지 않고 원문을 전부
현재 기억에 남긴다(2026-09-22 개정. 그 전에는 이 경우도 그대로 적용해서, 운영자가 판정을
켜 둔 채 키가 빠지면 판정 없이 원문이 스냅숏에서 빠졌다). 새 claim 은 두 경우 다 적용한다.
검사를 시작한 뒤 HTTP 오류·시간 초과·응답 모양 오류가 나면
...
| 검사 생략과 실패 시 원문 보존 | 선언 | `Typesafeai_config` — 키는 각 목적지가 `api_key_env` 로 이름 붙인 환경변수(유일한 비밀), 나머지는 runtime.toml 의 `[typesafeai]` 테이블(`enabled`, `destinations` — 서버마다 `endpoint`·`model`·`api_key_env`) |
