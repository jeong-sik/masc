(** Regression guard for #25272 — keeper tool schemas must convert cleanly
    through the AGENT_CORE boundary.
    ...
    [Agent_core.Mcp.json_schema_to_params] (agent-core boundary, fail-closed on schema
    ...
    reintroduces a schema the AGENT_CORE boundary cannot convert (multi-type union,
    unknown property type, malformed [type], ...). It runs the same MASC/AGENT_CORE
  ...
  List.iter
    (fun (schema : Masc_domain.tool_schema) ->
       ...
       | exception Invalid_argument detail ->
         Alcotest.failf
           "Keeper model tool schema %S is not convertible by the MASC/AGENT_CORE \
            boundary (Tool_bridge.params_of_json_schema): %s. Fix the reported \
            descriptor-owned or canonical schema before exposing it to AGENT_CORE \
            (#25272)."
           schema.name
           detail)
    keeper_schemas
...
let () =
  Alcotest.run
    "tool_schema_agent_core_boundary"
    [ ( "boundary"
      , [ Alcotest.test_case "keeper catalog non-empty" `Quick test_catalog_non_empty
        ; Alcotest.test_case
            "all keeper tool schemas convert through the AGENT_CORE boundary"
            `Quick
