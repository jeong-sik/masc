...
let test_mcp_session_persistence () =
  ...
  let id2 =
    Agent_registry_eio.get_or_create_identity ~mcp_session_id:mcp_sid (`Assoc [])
  in
...
let test_active_count () =
  ...
  let _ =
    Agent_registry_eio.get_or_create_identity (`Assoc [ "_agent_name", `String name ])
  in
...
let test_reset_clears_cached_session_mappings () =
  ...
  let id2 =
    Agent_registry_eio.get_or_create_identity
      ...
      (`Assoc [ "_agent_name", `String "cached-agent" ])
  in
  check bool "session mapping cleared by reset" true (id1.session_key <> id2.session_key)
;;
  ...
  Eio.Fiber.all
    (List.init 5 (fun i () ->
       let params =
         `Assoc
           [ ( "_agent_name"
             , `String (Printf.sprintf "concurrent-agent-%d-%d" i (Random.int 10000)) )
           ]
       in
       for _ = 1 to 10 do
...
let test_concurrent_same_mcp_session_id () =
  ...
  Fs_compat.set_fs (Eio.Stdenv.fs env);
  Agent_registry_eio.reset_for_testing ();
  let mcp_sid = Printf.sprintf "race-session-%d" (Random.int 1_000_000) in
  let collected : Agent_identity.t list ref = ref [] in
  let collect_mu = Eio.Mutex.create () in
  Eio.Fiber.all
    (List.init 8 (fun _i () ->
       let params = `Assoc [ "_agent_name", `String "racing-agent" ] in
       let id =
         Agent_registry_eio.get_or_create_identity ~mcp_session_id:mcp_sid params
       in
       Eio.Mutex.use_rw ~protect:true collect_mu (fun () -> collected := id :: !collected)));
  let keys =
    List.sort_uniq compare (List.map (fun id -> id.Agent_identity.session_key) !collected)
  in
  check int "all fibers converged to a single session_key" 1 (List.length keys);
  (* And the map-resolved identity is the same key. *)
  match Agent_registry_eio.get_or_create_identity ~mcp_session_id:mcp_sid (`Assoc []) with
  | id -> check (list string) "re-lookup returns the shared key" keys [ id.session_key ]
;;
  ...
  run
    ...
    ; ( "statistics"
      , [ test_case "active_count" `Quick test_active_count
        ...
        ; test_case
            "reset_clears_cached_session_mappings"
            `Quick
    ...
    ; ( "concurrency"
      , [ test_case "concurrent_access" `Quick test_concurrent_access
        ; test_case
            "concurrent_same_mcp_session_id"
            `Quick
