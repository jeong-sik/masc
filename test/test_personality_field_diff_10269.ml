...
let test_trim_equal_returns_none () =
  check
    opt_string
    "trailing newline drift collapses"
    None
  ...
  check
    opt_string
    "leading whitespace drift collapses"
    None
    (Kr.personality_field_diff_summary ~field:"goal" ~current:"  same  " ~target:"same");
  check
    opt_string
    "identical strings collapse"
    None
    (Kr.personality_field_diff_summary ~field:"will" ~current:"x" ~target:"x")
;;
...
let test_inner_diff_returns_summary () =
  ...
  | Some s ->
    ...
    check
      bool
      "summary exposes meta length"
      true
      (Astring.String.is_infix ~affix:"raw_meta_len=11" s);
    check
      bool
      "summary exposes target length"
      true
...
let test_length_drift_returns_summary () =
  match
    Kr.personality_field_diff_summary
      ~field:"goal"
      ~current:"short"
      ~target:"a much longer goal text"
  with
  | None -> fail "expected Some summary for length drift"
  | Some s ->
    check
      bool
      "asymmetric length is reported"
      true
...
let test_long_field_truncates_preview () =
  ...
  match
    Kr.personality_field_diff_summary ~field:"instructions" ~current:long_a ~target:long_b
  with
  | None -> fail "expected Some summary"
  | Some s ->
    check
      bool
      "ellipsis present for long preview"
      true
      (Astring.String.is_infix ~affix:"..." s);
    (* Sanity: full 200-char string must NOT appear. *)
    check
      bool
      "untruncated preview must not appear"
      false
...
let () =
  run
    "personality_field_diff_10269"
    [ ( "diff_summary"
      , [ test_case "trim-equal pairs return None" `Quick test_trim_equal_returns_none
        ; test_case
        ...
        ; test_case "long preview is truncated" `Quick test_long_field_truncates_preview
        ] )
    ]
;;
