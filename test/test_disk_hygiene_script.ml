...
let test_disk_hygiene_fix_path () =
  with_temp_dir "disk-hygiene-fix" (fun dir ->
    ...
    check bool "reports dune cache warning" true (contains_substring stdout1 "dune_cache");
    check
      bool
      "reports tlc artefacts warning"
      true
      (contains_substring stdout1 "tlc_artifacts");
    check bool "reports build dirs warning" true (contains_substring stdout1 "build_dirs");
    ...
    let code2, stdout2, stderr2 =
      run_shell
        ...
        ~env
        (Printf.sprintf
           "%s --fix --reset-dune-cache --clean-extra-build-dirs"
           (quote script))
    in
    if code2 <> 0
    then
...
let test_tla_check_cleans_generated_artifacts_by_default () =
  with_temp_dir "tla-check-cleanup" (fun dir ->
    ...
    in
    if code <> 0
    then failf "tla-check failed (%d)\nstdout:\n%s\nstderr:\n%s" code stdout stderr;
    check
      bool
      "cleanup message emitted"
      true
    ...
    let path =
      Printf.sprintf
        ...
        (match Sys.getenv_opt "PATH" with
         ...
         | None -> "")
    in
    let env = [ "PATH", path; "TLC_DIR", tlc_dir; "KEEP_TLC_ARTIFACTS", "1" ] in
    let script = Filename.concat repo_dir "scripts/tla-check.sh" in
    let code, stdout, stderr =
      run_shell ~cwd:repo_dir ~env (Printf.sprintf "%s" (quote script))
    in
...
let read_makefile_surface () =
  ...
  let fragments =
    if Sys.file_exists mk_dir && Sys.is_directory mk_dir
    then
      Sys.readdir mk_dir
      |> Array.to_list
...
let () =
  run
    ...
    [ ( "script"
      , [ test_case
            "disk hygiene fixes tlc cache and extra builds"
            `Quick
            test_disk_hygiene_fix_path
        ; test_case
            "tla-check cleans artefacts by default"
            `Quick
            test_tla_check_cleans_generated_artifacts_by_default
        ; test_case
            "tla-check keep flag preserves artefacts"
            `Quick
            test_tla_check_respects_keep_tlc_artifacts
        ; test_case
            "makefile exposes disk hygiene targets"
            `Quick
