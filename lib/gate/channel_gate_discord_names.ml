...
let resolve_path raw_path =
  ...
  then Filename.concat (Env_config_core.base_path ()) raw_path
  else raw_path
;;
...
let configured_read_path env_name ~default ~legacy =
  ...
  | None ->
    ...
    if Sys.file_exists preferred
    then preferred
    else if Sys.file_exists legacy
    then legacy
    else preferred
;;
...
let names_read_path () =
  configured_read_path
    "MASC_DISCORD_NAMES_PATH"
    ~default:default_names_path
    ~legacy:legacy_names_path
;;
...
let empty =
  { guild_names = []; channel_names = []; channel_to_guild = []; updated_at = "" }
;;
...
let to_json nm =
  let to_assoc items = `Assoc (List.map (fun (k, v) -> k, `String v) items) in
  `Assoc
...
let save nm =
  ...
  Fun.protect
    ...
    (fun () -> output_string oc (Yojson.Safe.pretty_to_string (to_json nm) ^ "\n"));
  Sys.rename tmp path
;;
