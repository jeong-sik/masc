...
let normalize_cascade_name raw =
  let normalized = Keeper_turn_profile.normalize_declared_name raw in
  let catalog =
    try Keeper_turn_profile.catalog_names () with
...
let handle_persona_generate ctx args =
  ...
  | Error error_json -> Keeper_types_profile.tool_result_error error_json
  | Ok concept ->
    ...
    then
      ...
      | Ok archetype_axes ->
           ...
           | Ok result ->
             let raw_text = Agent_sdk_response.text_of_response result.Keeper_runner.response in
             (try
                ...
                | Ok normalized ->
                  ...
                  in
                  Keeper_types_profile.tool_result_ok (Yojson.Safe.to_string json)
              with
