...
let test_resolver_default_matches_ssot () =
  ...
  | Some p ->
    ...
    let canonical_pattern = "~default:3600.0 \"MASC_KEEPER_TURN_TIMEOUT_SEC\"" in
    let contains s sub =
...
let test_resolver_upper_matches_ssot () =
  ...
  | Some p ->
    ...
    check
      bool
      "canonical 7200 upper bound in resolver"
      true
...
let () =
  run
    "keeper_turn_timeout_default_10456"
    [ ( "ssot-pin"
      , [ test_case
            "env_config_keeper SSOT default is 3600 (post-#9637)"
            `Quick
            test_ssot_default_3600
        ] )
    ; ( "resolver-drift-gate"
      , [ test_case
            "resolver default literal matches SSOT (no 1200 drift)"
            `Quick
            test_resolver_default_matches_ssot
        ; test_case
            "resolver upper bound literal matches SSOT (7200)"
            `Quick
