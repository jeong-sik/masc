...
| Persistent meta | `.masc/perpetual-keepers/<name>.json` | JSON | 런타임 상태. turn 카운트, context ratio 등 포함 |

별도 keepalive 설정 파일은 없다. keeper 선언과 런타임 상태는 `.masc/perpetual-keepers/<name>.json`에 모이고, keeper는 durable always-on으로 취급된다. runtime 중지 여부는 `paused` 또는 `keeper_down`으로 표현한다.
