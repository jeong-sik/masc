...
let with_tmp_log f =
  ...
  let dir =
    Filename.concat
      ...
      (Printf.sprintf
         "test-keeper-tool-call-log-%d-%d-%d"
         (Unix.getpid ())
         !counter
...
let with_tmp_corrupt_tool_call_store f =
  ...
  let dir =
    Filename.concat
      (Filename.get_temp_dir_name ())
      (Printf.sprintf
         "test-keeper-tool-call-log-corrupt-%d-%d-%d"
         (Unix.getpid ())
         !counter
         (int_of_float (Unix.gettimeofday () *. 1000.0)))
  in
...
let test_denied_tool_not_logged () =
  with_tmp_log (fun () ->
    ...
    Keeper_tool_call_log.log_call
      ~keeper_name:"k"
      ~tool_name:"mcp_auth_create"
      ~input:(`Assoc [ "token", `String "secret123" ])
      ~output_text:"done"
      ~success:true
      ~duration_ms:1.0
      ();
...
let test_sensitive_input_fields_redacted () =
  with_tmp_log (fun () ->
    ...
    Alcotest.(check bool)
      ...
      false
      (Observability_redact.contains_substring
         ~sub:"sk-proj-abcdefghijklmnop12345678"
         entry_str))
;;
...
let test_turn_context_fields_stored () =
  with_tmp_log (fun () ->
    ...
    Alcotest.(check int) "turn field" 7 (Safe_ops.json_int ~default:0 "turn" entry);
    Alcotest.(check int)
      "keeper_turn_id field"
      7
    ...
    let runtime_contract = Yojson.Safe.Util.member "runtime_contract" entry in
    Alcotest.(check (option string))
      "runtime_contract keeper"
      (Some "k")
      (Safe_ops.json_string_opt "keeper_name" runtime_contract);
    Alcotest.(check (option string))
      "runtime_contract agent"
      (Some "keeper-k-agent")
      (Safe_ops.json_string_opt "agent_name" runtime_contract);
    Alcotest.(check int)
      "runtime_contract generation"
      3
      (Safe_ops.json_int ~default:0 "generation" runtime_contract);
    Alcotest.(check (list string))
      "runtime_contract allowed_paths"
    ...
    Alcotest.(check (list string))
      "runtime_contract missing_required_tools"
      [ "keeper_fs_edit" ]
      Yojson.Safe.Util.(
        runtime_contract
        |> member "missing_required_tools"
        |> to_list
        |> List.map to_string);
...
let test_turn_context_fields_absent_without_context () =
  with_tmp_log (fun () ->
    ...
    Alcotest.(check (option string))
      ...
      (Safe_ops.json_string_opt "tool_choice" entry);
    Alcotest.(check bool)
      "thinking_enabled absent"
      true
      (match Yojson.Safe.Util.member "thinking_enabled" entry with
       | `Null -> true
       | _ -> false);
    Alcotest.(check bool)
      "thinking_budget absent"
      true
      (match Yojson.Safe.Util.member "thinking_budget" entry with
       | `Null -> true
       | _ -> false);
    Alcotest.(check bool)
      "prompt_fingerprint absent"
      true
...
let find_bucket name json =
  ...
  |> List.find (fun item -> Safe_ops.json_string_opt "name" item = Some name)
;;
...
let test_dashboard_aggregate_groups_runtime_fields () =
  with_tmp_log (fun () ->
    ...
    Keeper_tool_call_log.log_call
      ...
      ~tool_name:"masc_status"
      ~input:(`Assoc [])
      ~output_text:"error: {\"ok\":false,\"error\":\"boom\"}"
      ~success:false
      ~duration_ms:3.0
      ~model:"qwen3.5-27b-unified"
      ~lane:"retry"
      ~tool_choice:"auto"
    ...
    Alcotest.(check bool)
      "dashboard durable store present"
      true
...
let test_dashboard_hourly_trend_numeric_ts () =
  with_tmp_log_dir (fun dir ->
    let store =
      Dated_jsonl.create ~base_dir:(Filename.concat dir ".masc/tool_calls") ()
    in
    ...
    let bucket =
      List.find
        (fun item -> Safe_ops.json_string_opt "hour" item = Some expected_hour)
        hourly
    in
    Alcotest.(check int)
      "hour bucket calls"
...
let test_dashboard_aggregate_window_hours () =
  with_tmp_log_dir (fun dir ->
    ...
    Alcotest.(check (option string))
      ...
      (Some "window_hours")
      (Safe_ops.json_string_opt "sampling_mode" summary);
    Alcotest.(check int) "window total" 1 (Safe_ops.json_int ~default:0 "total" summary);
    Alcotest.(check (option int))
      "sample limit omitted"
      None
...
let test_output_invalid_utf8_sanitized () =
  with_tmp_log_dir (fun dir ->
    ...
    let today =
      ...
      Printf.sprintf "%04d-%02d/%02d.jsonl" (tm.tm_year + 1900) (tm.tm_mon + 1) tm.tm_mday
    in
    let file = Filename.concat dir (Filename.concat ".masc/tool_calls" today) in
    let contents =
    ...
    let rec scan i =
      ...
      then true
        ...
        if dlen > 0 && Uchar.utf_decode_is_valid dec then scan (i + dlen) else false)
    in
...
let test_output_inline_string_preserved () =
  with_tmp_log (fun () ->
    Keeper_tool_call_log.log_call
      ~keeper_name:"k"
      ~tool_name:"tool_inline"
      ~input:(`Assoc [])
      ~output_text:"small inline result"
      ~success:true
      ~duration_ms:1.0
      ();
    let results = Keeper_tool_call_log.read_recent ~n:1 () in
    ...
    | [ json ] ->
      ...
      Alcotest.(check string) "inline output stays a string" "small inline result" s
    | _ -> Alcotest.fail "expected exactly one entry")
;;
  ...
  Alcotest.run
    ...
    ; ( "redaction"
      , [ eio_test "denied tool not logged" test_denied_tool_not_logged
        ...
        ; eio_test
            "turn context fields absent without context"
            test_turn_context_fields_absent_without_context
        ; eio_test
            "dashboard aggregate groups runtime fields"
            test_dashboard_aggregate_groups_runtime_fields
        ; eio_test
            "dashboard hourly trend buckets numeric ts"
            test_dashboard_hourly_trend_numeric_ts
        ; eio_test
            "dashboard aggregate window hours"
            test_dashboard_aggregate_window_hours
        ; eio_test
            "append failure records coverage gap"
            test_append_failure_records_coverage_gap
        ; eio_test
            "dashboard aggregate surfaces coverage gap"
            test_dashboard_aggregate_surfaces_coverage_gap
        ] )
    ; ( "utf8_sanitize"
      , [ eio_test
            "invalid UTF-8 bytes scrubbed before persist"
            test_output_invalid_utf8_sanitized
        ; eio_test "valid UTF-8 preserved verbatim" test_output_valid_utf8_untouched
        ] )
    ; ( "blob_normalize"
      , [ eio_test
            "blob sentinel persists as structured _blob object"
            test_output_blob_sentinel_normalized
        ; eio_test
            "inline string output stays a JSON string"
