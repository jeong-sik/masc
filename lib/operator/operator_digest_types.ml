...
let operator_severity_of_failure_envelope (sev : Failure_envelope.severity)
  : operator_severity
  =
  match sev with
...
let stalled_session_threshold_sec =
  Env_config.InternalTimers.stalled_session_threshold_sec
;;
...
let summary_of_recommendations ~actor (items : recommended_action list) =
  ...
  `Assoc
    [ "count", `Int (List.length sorted)
    ; "top_action", option_to_json (recommended_action_to_yojson ~actor) top_item
    ; "provenance", `String "fallback"
    ; "authoritative", `Bool false
    ]
;;
...
let is_root_alias value =
  String.equal value "root" || String.equal value "namespace" || String.equal value "room"
;;
...
let normalize_digest_target_type value =
  ...
  | Some raw ->
    ...
    if is_root_alias normalized then Ok "root" else Error "target_type must be root"
  | None -> Ok "root"
;;
