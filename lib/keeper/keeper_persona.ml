...
let handle_persona_list _ctx args : tool_result =
  ...
  let payload =
    if detailed
    then `List (List.map persona_summary_to_json personas)
    else
      string_list_to_json
        (List.map
           (fun (persona : Keeper_types_profile.persona_summary) -> persona.persona_name)
           personas)
  in
  let json = `Assoc [ "count", `Int (List.length personas); "personas", payload ] in
    ...
    then
      ...
      then false, body
        ...
        then (
          ...
          | _ :: _ as shard_names -> Tool_shard.set_agent_shards name shard_names
          | [] -> ());
        let created_json =
