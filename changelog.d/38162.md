...
- The boot asset sync records the SHA-256 of every file it writes in
  ...
  a WARN boot line naming the path and what was done. A manifest under
  another schema reads as none: nothing is retired that boot. The
  capability and lane probes no longer write the runtime prompt directory;
  they read the binary's own prompts from a private temporary one (#38162).
