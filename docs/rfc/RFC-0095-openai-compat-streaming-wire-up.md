...
Configured telemetry backend (retired metrics backend exposition, masc dashboard port 8935):
...
| H2 | runtime.toml `streaming = true` provider/model 플래그가 *provider config → SDK request body* 변환 단계에서 dead-read (`Runtime_config.openai_compatible_custom_model` 흐름) | `rg '"stream"' lib/runtime_config*` + `rg 'streaming' lib/oas_compat/` 호출 경로 trace |
...
- runtime.toml 의 `streaming = true` 가 *capability 선언* 인지 *request body 명시* 인지는 `Runtime_config` 파서가 어떻게 해석하는지에 달림. `rg "\"stream\"" lib/runtime_config*` 결과가 0 hit 이면 H2 강력 확정.
...
- `feature/rfc-0095-provider-d-compat-streaming` 브랜치 + Draft PR + sangsu /
  ...
  canary observation.
- Rollback path: runtime.toml 에서 `streaming = false` per-model override
...
4. runtime.toml 의 `streaming = true` 플래그가 *capability 선언* 인지
