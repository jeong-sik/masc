...
(* RFC-0232: the scheduled-autonomous "what is this keeper doing" preview, by
   precedence. Callers already decide whether [response_text] is eligible for
   the preview; this helper only prevents empty text from replacing richer tool
   or evidence signals. Then substantive tool calls, then validated evidence,
   else the prior preview is kept (never overwritten with a synthetic filler).
   Pure so the precedence is unit-testable without a keeper_meta. *)
let select_proactive_preview
    ~(previous : string)
    ~(has_text : bool)
    ~is_visible_reply:(_ : bool)
