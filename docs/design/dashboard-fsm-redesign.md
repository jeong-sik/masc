...
<<<<<<< HEAD
- 시나리오 C — Invariants violation: historical/compatibility-only recovery contract. `manual_reconcile` two-store runtime was removed in `#7334`, so this path is no longer a live runtime scenario. Keep it only as a spec-compat test hook until a new recovery contract is defined.
