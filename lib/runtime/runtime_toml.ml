...
let parse_binding_fields (provider_id : string) (model_id : string) (tbl : Otoml.t)
  ...
  (* RFC-0153 §4.2.4: max-concurrent is an OPTIONAL per-binding cap.
     Missing or non-positive parses to [None] ("no per-binding cap"), a real
     [int option] — replacing the RFC-0058 [0]-as-required-marker, which was an
     Unknown->permissive-default (CLAUDE.md anti-pattern #2). *)
  let max_concurrent =
