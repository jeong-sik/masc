...
let expected =
    ...
    ; {|keeper_ide_annotate|}, {|Leave a memo above a line of a file, as a comment in the file's own syntax.

The memo reads masc(<your name>): <text>, or masc(<your name>) <kind>: <text> with kind decision, question or bookmark; the TUI lists these and marks their lines, and they travel with the file into diffs and reviews. This edits the file: the comment is inserted above line, indented like it, through the same write path as Edit. file_path, line and text are required. The file's extension picks the comment syntax; a file no language here owns, or one with no comment syntax (JSON), is refused.|}, {|{"additionalProperties":false,"properties":{"file_path":{"description":"Absolute or sandbox-relative path of the file, as Edit takes it. The file must exist.","type":"string"},"kind":{"description":"Memo kind; defaults to comment","enum":["comment","decision","question","bookmark"],"type":"string"},"line":{"description":"1-based line the memo goes above","minimum":1,"type":"integer"},"text":{"description":"One line of memo text","type":"string"}},"required":["file_path","line","text"],"type":"object"}|}
    ]
;;
