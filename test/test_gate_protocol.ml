...
let test_validate_ok () =
  match Gate_protocol.validate ~max_content_length:4000 (make_msg ()) with
  | Ok () -> ()
  | Error _ -> fail "expected valid message"

let test_validate_empty_content () =
  match Gate_protocol.validate ~max_content_length:4000 (make_msg ~content:"  " ()) with
...
let test_validate_content_too_long () =
  ...
  match Gate_protocol.validate ~max_content_length:100 (make_msg ~content:long ()) with
...
let test_validate_empty_user_id () =
  match Gate_protocol.validate ~max_content_length:4000 (make_msg ~channel_user_id:"" ()) with
  | Error Gate_protocol.Empty_channel_user_id -> ()
  | _ -> fail "expected Empty_channel_user_id"

let test_validate_empty_idempotency_key () =
  match Gate_protocol.validate ~max_content_length:4000 (make_msg ~idempotency_key:"" ()) with
...
let test_validate_keeper_name_checked_before_content () =
  match Gate_protocol.validate ~max_content_length:4000
...
let test_inbound_of_json_rejects_keeper_name_only () =
  ...
  | Ok msg ->
      ...
      (match
         Gate_protocol.validate ~max_content_length:4000 msg
       with
       | Error Gate_protocol.Empty_keeper_name -> ()
