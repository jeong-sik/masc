...
let mk_usage ?(cache_creation = 0) ?(cache_read = 0) ?(cost_usd = None) ~input ~output ()
  : Oas.Types.api_usage
  =
  { input_tokens = input
...
let anthropic_kind = provider_kind_of_spec "claude:claude-sonnet-4-6"
let openai_compat_kind = provider_kind_of_spec "openrouter:anthropic/claude-3.5"
...
let test_threshold_is_1024 () =
  Alcotest.(check int)
    "anthropic cache threshold = 1024"
    1024
    T.anthropic_cache_min_input_tokens
;;
...
let test_model_detector_recognizes_anthropic () =
  ...
  let no m =
    Alcotest.(check bool)
      (Printf.sprintf "%s -> not Anthropic" m)
      false
      (T.model_uses_anthropic_caching ~model_used:m ~resolved_model_id:m)
  in
  let no_kind kind m =
...
let test_anthropic_with_zero_cache_above_threshold_flags () =
  ...
  let t =
    classify_with_kind ~provider_kind:anthropic_kind ~model:"claude-sonnet-4-6" ~usage
  in
  Alcotest.(check bool)
    "anthropic_caching_likely_disabled flagged"
    true
    (has_reason t "anthropic_caching_likely_disabled");
  Alcotest.(check bool)
...
let test_threshold_boundary_1024_inclusive () =
  ...
  let t =
    classify_with_kind ~provider_kind:anthropic_kind ~model:"claude_code:auto" ~usage
  in
  Alcotest.(check bool)
...
let test_cache_read_positive_does_not_flag () =
  let usage = mk_usage ~input:5000 ~output:500 ~cache_creation:0 ~cache_read:4500 () in
  let t =
    classify_with_kind ~provider_kind:anthropic_kind ~model:"claude-sonnet-4-6" ~usage
  in
  Alcotest.(check bool)
...
let test_cache_creation_positive_does_not_flag () =
  let usage = mk_usage ~input:5000 ~output:500 ~cache_creation:4500 ~cache_read:0 () in
  let t =
    classify_with_kind ~provider_kind:anthropic_kind ~model:"claude-sonnet-4-6" ~usage
  in
  Alcotest.(check bool)
...
let test_multiple_anomalies_compose () =
  ...
  in
  Alcotest.(check bool)
    "input_tokens_gt_1m present"
    true
    (has_reason t "input_tokens_gt_1m");
  Alcotest.(check bool)
    "anthropic_caching_likely_disabled present"
    true
    (has_reason t "anthropic_caching_likely_disabled");
...
let () =
  Alcotest.run
    "keeper_usage_trust_anthropic_cache_9959"
    [ ( "threshold-constant"
    ...
    ; ( "fires-when-it-should"
      , [ Alcotest.test_case
            "anthropic + 0 cache above threshold"
            `Quick
    ...
    ; ( "false-positive-prevention"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "cache_creation > 0: silent"
            `Quick
            test_cache_creation_positive_does_not_flag
        ] )
    ; ( "composition"
      , [ Alcotest.test_case
            "stacks with input_tokens_gt_1m"
            `Quick
