...
let test_all_schemas_not_empty () =
  Alcotest.(check bool) "all_schemas is not empty" true (List.length schema_inventory > 0)
;;
...
let test_schema_has_required_fields () =
  List.iter
    (fun schema ->
       ...
       Alcotest.(check bool)
         (Printf.sprintf "%s has name" schema.name)
         true
         (String.length schema.name > 0);
       (* Description must not be empty *)
       Alcotest.(check bool)
         (Printf.sprintf "%s has description" schema.name)
         true
         (String.length schema.description > 0);
       (* input_schema must be an object *)
       match schema.input_schema with
       | `Assoc _ -> ()
       | _ ->
         Alcotest.fail (Printf.sprintf "%s input_schema is not an object" schema.name))
    schema_inventory
;;
...
let test_all_names_start_with_masc () =
  List.iter
    (fun schema ->
       Alcotest.(check bool)
         (Printf.sprintf "%s starts with masc_" schema.name)
         true
         (String.length schema.name >= 5 && String.sub schema.name 0 5 = "masc_"))
    schema_inventory
;;
...
let test_find_tool_existing () =
  let tools =
    [ "masc_join"; "masc_leave"; "masc_status"; "masc_broadcast"; "masc_transition" ]
  in
  List.iter
    (fun name ->
...
let test_input_schema_type_is_object () =
  List.iter
    (fun schema ->
       ...
       | Some t ->
         Alcotest.fail
           (Printf.sprintf "%s input_schema type is %s, expected object" schema.name t)
       | None ->
         Alcotest.fail (Printf.sprintf "%s input_schema missing type field" schema.name))
    schema_inventory
;;
...
let test_input_schema_has_properties () =
  List.iter
    (fun schema ->
       ...
       | Some _ -> ()
       | None ->
         Alcotest.fail (Printf.sprintf "%s input_schema missing properties" schema.name))
    schema_inventory
;;
    ...
    (fun schema ->
       ...
       | `Assoc fields ->
         (match List.assoc_opt "required" fields with
          ...
          | Some (`List _) -> ()
          | Some _ ->
            Alcotest.fail (Printf.sprintf "%s required field is not a list" schema.name))
       | _ ->
         Alcotest.fail (Printf.sprintf "%s input_schema is not an object" schema.name))
    schema_inventory
;;
...
let test_masc_transition_schema () =
  ...
  | Some schema ->
    (match get_json_assoc "properties" schema.input_schema with
     | Some props ->
       Alcotest.(check bool)
         "has completion_contract"
         true
         (List.mem_assoc "completion_contract" props);
       Alcotest.(check bool)
         "has evaluator_cascade"
         true
  ...
  | Some schema ->
    (match get_json_assoc "properties" schema.input_schema with
     | Some props ->
       ...
       Alcotest.(check bool) "has horizon" true (List.mem_assoc "horizon" props);
       Alcotest.(check bool)
         "omits status lifecycle field"
         false
         (List.mem_assoc "status" props);
       Alcotest.(check bool)
         "omits phase lifecycle field"
         false
       ...
       Alcotest.(check bool)
         "has require_completion_approval"
         true
  ...
  | Some schema ->
    (match get_json_assoc "properties" schema.input_schema with
     | Some props ->
       ...
       Alcotest.(check bool) "has outcome" true (List.mem_assoc "outcome" props);
       Alcotest.(check bool) "has new_horizon" true (List.mem_assoc "new_horizon" props)
     | None -> Alcotest.fail "masc_goal_review missing properties");
    (match get_json_list "required" schema.input_schema with
     | Some reqs ->
       Alcotest.(check bool) "goal_id required" true (List.mem (`String "goal_id") reqs);
       Alcotest.(check bool) "outcome required" true (List.mem (`String "outcome") reqs)
...
let test_masc_goal_transition_schema () =
  ...
  | Some schema ->
    (match get_json_assoc "properties" schema.input_schema with
     | Some props ->
       Alcotest.(check bool) "has goal_id" true (List.mem_assoc "goal_id" props);
       Alcotest.(check bool) "has action" true (List.mem_assoc "action" props);
       Alcotest.(check bool) "has actor" true (List.mem_assoc "actor" props);
       Alcotest.(check bool) "has note" true (List.mem_assoc "note" props);
       Alcotest.(check bool)
         "has override_note"
         true
    ...
    (match get_json_list "required" schema.input_schema with
     | Some reqs ->
       Alcotest.(check bool) "goal_id required" true (List.mem (`String "goal_id") reqs);
       Alcotest.(check bool) "action required" true (List.mem (`String "action") reqs);
       Alcotest.(check bool) "actor required" true (List.mem (`String "actor") reqs)
...
let test_masc_goal_verify_schema () =
  ...
  | Some schema ->
    (match get_json_assoc "properties" schema.input_schema with
     | Some props ->
       Alcotest.(check bool) "has goal_id" true (List.mem_assoc "goal_id" props);
       Alcotest.(check bool) "has request_id" true (List.mem_assoc "request_id" props);
       Alcotest.(check bool) "has principal" true (List.mem_assoc "principal" props);
    ...
    (match get_json_list "required" schema.input_schema with
     | Some reqs ->
       ...
       Alcotest.(check bool) "decision required" true (List.mem (`String "decision") reqs)
...
let test_remote_operator_action_schema_is_strict () =
  let schema =
    match
      List.find_opt
        (fun schema -> schema.name = "masc_operator_action")
...
let test_retired_front_door_tools_absent_from_schema_inventory () =
  ...
  List.iter
    (fun name ->
       ...
       | None -> ()
       | Some _ ->
         Alcotest.fail (Printf.sprintf "%s should be absent from schema inventory" name))
    retired_tools
;;
...
let test_masc_persona_authoring_schemas () =
  ...
  (match find_tool "masc_persona_generate" with
   ...
   | Some schema ->
     (match get_json_assoc "properties" schema.input_schema with
      | Some props ->
        Alcotest.(check bool) "generate has concept" true (List.mem_assoc "concept" props);
        Alcotest.(check bool)
          "generate has tool_preset"
          true
          (List.mem_assoc "tool_preset" props);
        Alcotest.(check bool)
          "generate has alignment axis"
          true
          (List.mem_assoc "alignment" props);
        Alcotest.(check bool)
          "generate has operating_style axis"
          true
          (List.mem_assoc "operating_style" props);
        Alcotest.(check bool)
          "generate has risk_posture axis"
          true
        ...
        Alcotest.(check (list string))
          "alignment enum follows contract"
          Contract.alignment_choices
          (enum_strings "alignment");
        Alcotest.(check (list string))
          "operating_style enum follows contract"
          Contract.operating_style_choices
          (enum_strings "operating_style");
        Alcotest.(check (list string))
          "risk_posture enum follows contract"
          Contract.risk_posture_choices
          (enum_strings "risk_posture");
        Alcotest.(check string)
          "language default follows contract"
          Contract.default_generation_language
          (default_string "language");
        Alcotest.(check string)
          "tool_preset default follows contract"
          Contract.default_tool_preset
          (default_string "tool_preset");
        Alcotest.(check string)
          "cascade default follows contract"
          Contract.default_generation_cascade_name
          (default_string "cascade_name");
        Alcotest.(check bool)
          "proactive default follows contract"
          Contract.default_proactive_enabled
          (default_bool "proactive_enabled")
  ...
  | Some schema ->
    (match get_json_assoc "properties" schema.input_schema with
     | Some props ->
       Alcotest.(check bool) "save has handle" true (List.mem_assoc "handle" props);
       Alcotest.(check bool) "save has profile" true (List.mem_assoc "profile" props);
       Alcotest.(check bool) "save has dry_run" true (List.mem_assoc "dry_run" props)
...
let test_masc_keeper_create_from_persona_schema () =
  ...
  | Some schema ->
    (match get_json_assoc "properties" schema.input_schema with
     | Some props ->
       Alcotest.(check bool) "has persona_name" true (List.mem_assoc "persona_name" props);
       Alcotest.(check bool)
         "omits social_model"
         false
...
let test_masc_keeper_up_schema () =
  ...
  | Some schema ->
    (match get_json_assoc "properties" schema.input_schema with
     | Some props ->
       Alcotest.(check bool) "has short_goal" true (List.mem_assoc "short_goal" props);
       Alcotest.(check bool) "has mid_goal" true (List.mem_assoc "mid_goal" props);
       Alcotest.(check bool) "has long_goal" true (List.mem_assoc "long_goal" props);
       Alcotest.(check bool)
         "has sandbox_profile"
         true
         (List.mem_assoc "sandbox_profile" props);
       Alcotest.(check bool) "has network_mode" true (List.mem_assoc "network_mode" props);
       Alcotest.(check bool)
         "has shared_memory_scope"
         true
       ...
       Alcotest.(check bool)
         ...
         (List.mem_assoc "social_model" props);
       Alcotest.(check bool)
         "has autoboot_enabled"
         true
         (List.mem_assoc "autoboot_enabled" props);
       Alcotest.(check bool) "omits models" false (List.mem_assoc "models" props);
       Alcotest.(check bool)
         "omits allowed_models"
         false
...
let test_masc_keeper_msg_schema () =
  ...
  | Some schema ->
    (match get_json_assoc "properties" schema.input_schema with
     | Some props ->
       Alcotest.(check bool) "omits new_goal" false (List.mem_assoc "new_goal" props);
       Alcotest.(check bool)
         "omits new_short_goal"
         false
...
let test_masc_keeper_repair_schema () =
  ...
  | Some schema ->
    (match get_json_assoc "properties" schema.input_schema with
     | Some props ->
       Alcotest.(check bool) "has task_spec" true (List.mem_assoc "task_spec" props);
       Alcotest.(check bool) "has source_text" true (List.mem_assoc "source_text" props);
       Alcotest.(check bool) "has target_mode" true (List.mem_assoc "target_mode" props);
       Alcotest.(check bool)
...
let test_masc_team_memory_read_schema () =
  ...
  | Some schema ->
    (match get_json_assoc "properties" schema.input_schema with
     | Some props ->
       Alcotest.(check bool) "has room" true (List.mem_assoc "room" props);
...
let test_masc_team_memory_search_schema () =
  ...
  | Some schema ->
    (match get_json_assoc "properties" schema.input_schema with
     | Some props ->
       Alcotest.(check bool) "has room" true (List.mem_assoc "room" props);
       Alcotest.(check bool) "has query" true (List.mem_assoc "query" props)
...
let test_masc_webrtc_answer_schema () =
  ...
  | Some schema ->
    (match get_json_assoc "properties" schema.input_schema with
     | Some props ->
       Alcotest.(check bool) "has offer_id" true (List.mem_assoc "offer_id" props);
       Alcotest.(check bool) "has agent_name" true (List.mem_assoc "agent_name" props)
...
let test_description_not_too_short () =
  List.iter
    (fun schema ->
       Alcotest.(check bool)
         (Printf.sprintf "%s description >= 20 chars" schema.name)
         true
...
let test_description_not_too_long () =
  List.iter
    (fun schema ->
       ...
       Alcotest.(check bool)
         (Printf.sprintf "%s description <= 1000 chars" schema.name)
         true
         (String.length schema.description <= 1000))
    schema_inventory
;;
  ...
  List.iter
    (fun schema ->
       ...
       | Some props ->
         ...
         Alcotest.(check int)
           (Printf.sprintf "%s no duplicate properties" schema.name)
           (List.length prop_names)
           (List.length unique_names)
       | None -> ())
    schema_inventory
;;
  ...
  Alcotest.run
    ...
    ; ( "core_tools"
      , [ Alcotest.test_case "masc_join" `Quick test_masc_join_schema
        ...
        ; Alcotest.test_case
            "masc_board_post supports judgment"
            `Quick
            test_masc_board_post_schema_supports_judgment
        ; Alcotest.test_case
            "remote_operator_action_strict"
            `Quick
            test_remote_operator_action_schema_is_strict
        ; Alcotest.test_case
            "retired front-door tools absent"
            `Quick
    ...
    ; ( "agent_tools"
      , [ Alcotest.test_case "agents" `Quick test_masc_agents_schema
        ; Alcotest.test_case
            ...
            `Quick
            test_masc_register_capabilities_schema
    ...
    ; (* auth_tools, a2a_tools (poll_events/heartbeat_result), handover_tools,
      ...
      "spawn_runtime_tools", [ Alcotest.test_case "spawn" `Quick test_masc_spawn_schema ]
    ; ( "keeper_runtime_tools"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "keeper-create-from-persona"
            `Quick
            test_masc_keeper_create_from_persona_schema
        ; Alcotest.test_case "keeper-up" `Quick test_masc_keeper_up_schema
        ; Alcotest.test_case "keeper-msg" `Quick test_masc_keeper_msg_schema
        ; Alcotest.test_case "keeper-repair" `Quick test_masc_keeper_repair_schema
        ] )
    ; ( "runtime_admin_tools"
      , [ Alcotest.test_case
            "tool-admin-snapshot"
        ...
        ; Alcotest.test_case "tool-admin-update" `Quick test_masc_tool_admin_update_schema
        ] )
    ; ( "team_memory_tools"
      , [ Alcotest.test_case "team-memory-read" `Quick test_masc_team_memory_read_schema
        ; Alcotest.test_case "team-memory-write" `Quick test_masc_team_memory_write_schema
        ; Alcotest.test_case
            "team-memory-search"
            `Quick
    ...
    ; (* runtime_verify_tools removed: masc_runtime_verify pruned *)
      ...
      , [ Alcotest.test_case
            "removed_from_public_schemas"
            `Quick
            test_legacy_swarm_tools_removed
        ] )
    ; ( "legacy_lifecycle_removed"
      , [ Alcotest.test_case
            "mitosis_removed_from_public_schemas"
            `Quick
