...
let of_json (json : Yojson.Safe.t) : procedure option =
  match
    ...
    , Safe_ops.json_string_opt "pattern" json )
  with
  | Some id, Some agent_name, Some pattern ->
    (match
       ( Safe_ops.json_int_opt "success_count" json
...
let load_procedures ~agent_name : procedure list =
  ...
  Fs_compat.load_jsonl path |> List.filter_map of_json
;;
...
let record_outcome ~agent_name ~pattern ~evidence_id ~success =
  ...
  | Some p ->
    ...
    in
    let patched = List.map (fun q -> if q.id = p.id then updated else q) procs in
...
let min_evidence () = Env_config.ProcMemory.min_evidence

(** Minimum confidence required for crystallization.
    Configurable via MASC_PROC_MIN_CONFIDENCE (default: 0.7). *)
let min_confidence () = Env_config.ProcMemory.min_confidence
  ...
  then ""
    ...
    let lines =
      List.map
        (fun p ->
           sprintf
             "- %s (confidence: %.0f%%, evidence: %d)"
             p.pattern
             (p.confidence *. 100.0)
             (List.length p.evidence))
        procs
    in
    "[PROCEDURES]\n" ^ String.concat "\n" lines ^ "\n[/PROCEDURES]")
;;
