...
let is_text_only (message : Masc_agent_core.Types.message) =
  (match message.role with
   | Masc_agent_core.Types.User | Masc_agent_core.Types.Assistant -> true
   | Masc_agent_core.Types.System | Masc_agent_core.Types.Tool -> false)
  && Option.is_none message.tool_call_id
  && message.content <> []
  && List.for_all
       (function
         | Masc_agent_core.Types.Text _ -> true
         | _ -> false)
       message.content
;;

let has_tool_use (message : Masc_agent_core.Types.message) =
  List.exists
    (function
      | Masc_agent_core.Types.ToolUse _ -> true
      | _ -> false)
    message.content
;;
...
let strip_reasoning_blocks (message : Masc_agent_core.Types.message) =
  let kept, stripped =
    List.fold_left
      (fun (kept, stripped) block ->
         match block with
         | Masc_agent_core.Types.Thinking { signature = None; _ }
         | Masc_agent_core.Types.ReasoningDetails _ -> kept, stripped + 1
         | _ -> block :: kept, stripped)
      ([], 0)
      message.content
  in
  { message with Masc_agent_core.Types.content = List.rev kept }, stripped
;;
...
(* R3: replace a SUCCESSFUL tool result's payload with the fixed marker while
   ...
   type. *)
let clear_tool_result_blocks (message : Masc_agent_core.Types.message) =
  let cleared_count = ref 0 in
  let content =
    List.map
      (fun block ->
         match block with
         | Masc_agent_core.Types.ToolResult { outcome = Masc_agent_core.Types.Tool_failed _; _ }
           -> block
      ...
      message.content
  in
  { message with Masc_agent_core.Types.content }, !cleared_count
;;
  ...
  then
    ...
    | Ok { closed_prefix; protected_suffix } ->
      ...
      let dedup_key_of message =
        ...
        then Some (Masc_agent_core.Types.show_message message)
        else None
      in
      let purge_item item =
        if protected item
          ...
          | Keeper_compaction_unit.Ordinary_message message ->
            if config.strip_thinking
               && (match message.role with
                   ...
                   | Masc_agent_core.Types.System
                   | Masc_agent_core.Types.Tool -> false)
            ...
            then (
              ...
              match stripped_message.Masc_agent_core.Types.content with
              | [] ->
...
let purge ~config (ckpt : Masc_agent_core.Checkpoint.t) =
  match purge_messages ~config ckpt.messages with
  | Error error -> Error error
  | Ok (messages, report) ->
    Ok ({ ckpt with Masc_agent_core.Checkpoint.messages }, report)
;;
