...
let rate_limit_config_of_yojson json =
  ...
  try
    let per_minute =
      json |> member "per_minute" |> to_int_option |> Option.value ~default:10
    in
    let burst_allowed =
      json |> member "burst_allowed" |> to_int_option |> Option.value ~default:5
    in
    let priority_agents =
    ...
    let worker_multiplier =
      json |> member "worker_multiplier" |> to_float_option |> Option.value ~default:1.0
    in
    let admin_multiplier =
      json |> member "admin_multiplier" |> to_float_option |> Option.value ~default:2.0
    in
    let broadcast_per_minute =
      json |> member "broadcast_per_minute" |> to_int_option |> Option.value ~default:15
    in
    let task_ops_per_minute =
      json |> member "task_ops_per_minute" |> to_int_option |> Option.value ~default:30
    in
    Ok
      { per_minute
      ; burst_allowed
...
let rec masc_error_to_string = function
  ...
  | AgentNotJoined name ->
    Printf.sprintf "❌ Agent not joined: %s. Use masc_join first." name
  | AgentAlreadyJoined name -> Printf.sprintf "⚠ %s is already in the room" name
  | TaskNotFound id ->
    Printf.sprintf "❌ Task not found: %s. Call masc_status to refresh your task list." id
  | TaskAlreadyClaimed { task_id; by } ->
    Printf.sprintf
      "❌ Task %s is currently owned by %s. Ask that agent to finish it, or claim a \
       different task."
      task_id
      by
  | TaskNotClaimed id ->
    Printf.sprintf "❌ Task %s is still todo. Claim/start it first, then mark it done." id
  | TaskInvalidState msg -> Printf.sprintf "❌ Invalid task state: %s" msg
  | PortalNotOpen agent ->
    Printf.sprintf "❌ No portal open for %s. Use masc_portal_open first." agent
  | PortalAlreadyOpen { agent; target } ->
    Printf.sprintf "⚠ Portal already open: %s ↔ %s" agent target
  | PortalClosed agent ->
    Printf.sprintf "❌ Portal is closed for %s. Use masc_portal_open to reopen." agent
  ...
  | TokenExpired agent ->
    Printf.sprintf "⏰ Token expired for %s. Use masc_auth_refresh." agent
  | InvalidToken reason -> Printf.sprintf "🔑 Invalid token: %s" reason
  | RateLimitExceeded { limit; current; wait_seconds; category } ->
    Printf.sprintf
      "⏳ Rate limit exceeded (%s): %d/%d requests. Wait %d seconds."
      (show_rate_limit_category category)
      current
      limit
      wait_seconds
...
and cache_error_to_string = function
  ...
  | CacheExpired { key; age_hours } ->
    Printf.sprintf "❌ Cache: Expired [key=%s, age=%.1fh]" key age_hours
...
let agent_credential_of_yojson json =
  ...
  try
    let id =
      json |> member "id" |> to_string_option |> Option.map Credential_id.of_string
    in
    let agent_id =
      json |> member "agent_id" |> to_string_option |> Option.map Agent_id.of_string
    in
...
let auth_config_of_yojson json =
  ...
  try
    ...
    let require_token =
      json |> member "require_token" |> to_bool_option |> Option.value ~default:false
    in
    let token_expiry_hours =
      json |> member "token_expiry_hours" |> to_int_option |> Option.value ~default:24
    in
...
let has_permission role permission = List.mem permission (permissions_for_role role)
