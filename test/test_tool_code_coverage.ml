...
let temp_dir () =
  incr test_counter;
  let dir = Filename.temp_file (Printf.sprintf "test_code_%d_" !test_counter) "" in
  Unix.unlink dir;
...
let test_code_search_with_query () =
  ...
  Alcotest.(check bool)
    "mentions path required"
    true
    (msg_contains ~needle:"path required" msg);
  cleanup_dir base_dir
;;
...
let test_code_search_with_file_pattern_finds_matches () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       write_text_file (Filename.concat base_dir "match.ml") "let mascot_needle = 1\n";
       write_text_file
         (Filename.concat base_dir "ignore.md")
...
let test_code_search_treats_dash_prefixed_query_as_literal () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       run_shell_ok ~cwd:base_dir "git config user.name test";
       write_text_file
         (Filename.concat base_dir "sample.txt")
...
let test_code_search_schema_requires_path () =
  let schema =
    List.find_opt
      (fun (schema : Types.tool_schema) -> String.equal schema.name "masc_code_search")
      Tool_code.schemas
  in
  match schema with
...
let test_code_read_path_traversal () =
  ...
  let has_security_msg =
    msg_contains ~needle:"traversal" msg || msg_contains ~needle:"git" msg
  in
...
let test_code_read_with_offset_limit () =
  let ctx, base_dir = make_ctx () in
  let args = `Assoc [ "path", `String "test.ml"; "offset", `Int 10; "limit", `Int 20 ] in
  ...
  Alcotest.run
    ...
    ; ( "code_search"
      , [ Alcotest.test_case "empty query" `Quick test_code_search_empty_query
        ...
        ; Alcotest.test_case
            "dash-prefixed query stays literal"
            `Quick
    ...
    ; ( "code_read"
      , [ Alcotest.test_case "no path" `Quick test_code_read_no_path
        ...
        ; Alcotest.test_case
            "dotdot inside prefix"
            `Quick
            test_code_read_dotdot_inside_prefix
        ; Alcotest.test_case
            "absolute sibling path"
            `Quick
            test_code_read_absolute_sibling
