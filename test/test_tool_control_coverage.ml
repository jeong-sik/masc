...
let () =
  test "dispatch_pause_status_paused" (fun () ->
    ...
    @@ fun ctx ->
    let _ =
      Tool_control.handle_pause ctx (`Assoc [ "reason", `String "For status test" ])
    in
...
let () =
  test "dispatch_resume" (fun () ->
    ...
    @@ fun ctx ->
    let _ =
      Tool_control.handle_pause ctx (`Assoc [ "reason", `String "For resume test" ])
    in
