...
let test_observation_reads_do_not_wait_for_durable_writer () =
  ...
  | child ->
    ...
    Unix.close ready_read;
    (* [waitpid] is interruptible under the Eio runtime (EINTR on signal
       delivery); an uncaught EINTR in the finally masks the test result as
       [Fun.Finally_raised]. Retry until the child is actually reaped. *)
