...
let test_merge_contiguous_still_merges_plain_user () =
  ...
  in
  let result =
    Compact.compact ~messages:msgs ~strategies:[ Compact.MergeContiguous ] ()
  in
  let user_msgs =
    List.filter
      (fun (m : Agent_sdk.Types.message) -> m.role = Agent_sdk.Types.User)
      result
  in
...
let test_compact_prune_tool_outputs () =
  ...
  in
  let result =
    Compact.compact ~messages:msgs ~strategies:[ Compact.PruneToolOutputs ] ()
  in
  let tool_text = Agent_sdk.Types.text_of_message (List.nth result 1) in
  check
    bool
    "tool output was pruned"
    true
...
let test_compact_empty_messages () =
  let result =
    Compact.compact
      ~messages:[]
      ~strategies:[ Compact.PruneToolOutputs; Compact.MergeContiguous ]
      ()
  in
...
let test_compact_single_message () =
  ...
  let result =
    Compact.compact
      ~messages:msgs
      ~strategies:[ Compact.MergeContiguous; Compact.DropLowImportance ]
      ()
  in
...
let test_compact_drop_low_importance () =
  ...
  let msgs =
    (msg Agent_sdk.Types.System "important system prompt"
     :: List.init 10 (fun i -> msg Agent_sdk.Types.Assistant (Printf.sprintf "ok %d" i)))
...
let test_compact_summarize_old () =
  ...
  let msgs =
    List.init 12 (fun i ->
      msg
        (if i mod 2 = 0 then Agent_sdk.Types.User else Agent_sdk.Types.Assistant)
        (Printf.sprintf "message %d with enough content to be meaningful" i))
  in
  let result = Compact.compact ~messages:msgs ~strategies:[ Compact.SummarizeOld ] () in
  check bool "message count reduced" true (List.length result < List.length msgs)
;;
...
let test_summarize_old_masks_tool_results_but_preserves_pairing () =
  ...
  | Some content ->
    check
      bool
      "structured stub marker present"
      true
      (String.starts_with ~prefix:"[tool:grep_search id:call-123" content);
    check bool "summary preserved in stub" true (String.contains content '3')
;;
...
let test_scoring_ssot () =
  ...
  List.iter
    (fun (idx, score) ->
       check
         bool
         (Printf.sprintf "score[%d] in [0,1]" idx)
         true
         (score >= 0.0 && score <= 1.0))
    scores
;;
...
let test_scoring_goal_sticky () =
  let msgs = [ msg Agent_sdk.Types.User "[GOAL] Monitor CI" ] in
  let scores = Scoring.score_messages msgs in
...
let test_scoring_legacy_goal () =
  let msgs = [ msg Agent_sdk.Types.User "[MASC_GOAL] old format goal" ] in
  let scores = Scoring.score_messages msgs in
  let score = List.assoc 0 scores in
...
let test_dynamic_high_pressure_multi_agent () =
  ...
  let resolved =
    Compact.resolve_strategies
      ~obs:(Some obs)
      [ Compact.Dynamic Compact.default_dynamic_selector ]
  in
...
let test_dynamic_local_at_64k_boundary () =
  ...
  let obs : Compact.observation_context =
    ...
    ; unclaimed_task_count = 0
    ; is_single_focused_task = false
    ; context_window = Compact.small_local_ctx_floor
    ; is_local_model = true
    }
  in
  let resolved =
  ...
  in
  let names = strategy_names resolved in
  check (list string) "local 64K gets default strategies" [ "DropLowImportance" ] names
;;
...
let test_dynamic_local_below_64k () =
  ...
  let obs : Compact.observation_context =
    ...
    ; unclaimed_task_count = 0
    ; is_single_focused_task = false
    ; context_window = Compact.small_local_ctx_floor - 1
    ; is_local_model = true
    }
  in
  let resolved =
  ...
  check
    (list string)
    "local below 64K gets small local strategies"
    [ "PruneToolOutputs"; "MergeContiguous" ]
    names
;;
...
let test_dynamic_large_context_cloud () =
  ...
  check
    (list string)
    "large context cloud strategies"
...
let test_dynamic_medium_cloud_default () =
  ...
  in
  let names = strategy_names resolved in
  check (list string) "medium cloud default strategies" [ "DropLowImportance" ] names
;;
...
let test_dynamic_no_observation () =
  ...
  in
  let names = strategy_names resolved in
  check (list string) "default (no observation)" [ "DropLowImportance" ] names
;;
...
let () =
  run
    ...
    [ ( "merge_contiguous"
      , [ test_case
            "still merges plain user"
            `Quick
            test_merge_contiguous_still_merges_plain_user
        ] )
    ; ( "strategy_mapping"
      , [ test_case "prune_tool_outputs" `Quick test_compact_prune_tool_outputs
        ...
        ; test_case
            "summarize_old masks tool results but preserves pairing"
            `Quick
    ...
    ; ( "scoring_ssot"
      , [ test_case "scores match messages" `Quick test_scoring_ssot
        ...
        ; test_case
            "legacy memory summary compat"
            `Quick
            test_scoring_legacy_memory_summary
        ; test_case "legacy goal compat" `Quick test_scoring_legacy_goal
        ] )
    ; ( "dynamic_strategy"
      , [ test_case
            "high pressure multi-agent"
            `Quick
            test_dynamic_high_pressure_multi_agent
