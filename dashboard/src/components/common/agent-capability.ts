...
const BASE_BADGE =
  'inline-flex items-center rounded-[var(--r-0)] border border-[var(--white-8)] bg-[var(--white-2)] px-2 py-0.5 text-3xs text-[var(--color-fg-primary)] transition-colors hover:bg-[var(--white-4)]'
...
export function AgentCapability({
  ...
  if (visible.length === 0 && extra === 0) {
    return html`
      <span
        class="inline-flex items-center rounded-[var(--r-0)] border border-dashed border-[var(--white-8)] bg-[var(--white-2)] px-2 py-0.5 text-3xs text-[var(--color-fg-muted)]"
