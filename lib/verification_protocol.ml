...
let submit_request_spec
  ...
  let request_kind, request_summary, next_action, board_type, board_title, board_content =
    ...
    | Ok plan_ctx when deliverable_claims_completion ~task_id:task.id plan_ctx.deliverable
      ->
      ( "conflict_triage"
      ...
      , Printf.sprintf
          "Conflict verification required for task %s (%s) by %s. Do not approve as \
           ordinary merged-PR verification; reconcile board / planning / mutation \
           surfaces first."
          task.id
          task.title
          assignee )
    | Ok _ | Error _ ->
      ...
      , Printf.sprintf
          "Verification requested for task %s (%s) by %s"
          task.id
          task.title
          assignee )
  in
...
let warn_contract_gap (task : Types.task) =
  ...
  | None ->
    Log.Task.warn
      "[verification-submit] task=%s has no contract — completion_contract and evidence \
       will be empty in the verification record"
      task.id
  | Some c when c.completion_contract = [] && c.verify_gate_evidence = [] ->
    Log.Task.warn
      "[verification-submit] task=%s has a contract but both completion_contract and \
       verify_gate_evidence are empty"
      task.id
  | Some _ -> ()
;;

let create_submit_request
      ~(config : Coord.config)
      ~(task : Types.task)
      ~assignee
      ~verification_id
...
let notify_submit_for_verification
      ~(config : Coord.config)
      ~(task : Types.task)
      ~assignee
      ~verification_id
...
let on_submit_for_verification
      ~(config : Coord.config)
      ~(task : Types.task)
      ~assignee
      ~verification_id
...
let record_approve_verification
  ...
  (* Update Verification.ml state machine: Pending -> Completed Pass.
     Issue #7544. *)
  if verification_id = ""
  then Error "verification_id is required for approval verdict persistence"
  else (
    match
...
let record_reject_verification
  ...
  (* Update Verification.ml state machine: Pending -> Completed (Fail reason).
     Issue #7544. *)
  if verification_id = ""
  then Error "verification_id is required for rejection verdict persistence"
  else (
    match
      Verification.submit_verdict
        ~base_path
  ...
  let () =
    match
      Board_dispatch.create_post
        ~author:verifier
        ~content:
          (Printf.sprintf "Rejected task %s (vrf:%s): %s" task_id verification_id reason)
        ~post_kind:Board.System_post
        ~meta_json
        ~visibility:Board.Internal
        ~hearth:"verification"
...
let check_timeouts ~(config : Coord.config) =
  if not (Env_config_runtime.Verification.fsm_enabled ())
  then ()
  else (
    try
      ...
      List.iter
        (fun (task : Types.task) ->
           ...
           | Types.AwaitingVerification
               { assignee; verification_id; deadline = Some dl; _ } ->
                ...
                let () =
                  match
                    Board_dispatch.create_post
                      ...
                      ~content:
                        (Printf.sprintf
                           "Verification timeout: task %s (%s) by %s — no verifier \
                            responded within deadline %s"
                           task.id
                           task.title
                           assignee
                           dl)
           ...
           | AwaitingVerification { deadline = None; _ }
           | Done _ | Cancelled _ -> ())
        backlog.tasks
    with
    | Eio.Cancel.Cancelled _ as e -> raise e
    | exn ->
      Log.Task.error "verification timeout check failed: %s" (Printexc.to_string exn))
;;
