...
let test_read_state_recovers_legacy_active_agent_entries () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         (list string)
         "legacy active_agents recovered"
       ...
       check
         (list string)
         "canonical active_agents rewritten"
         state.active_agents
         repaired_agents)
;;
...
let test_agent_of_yojson_bootstraps_null_last_seen_from_joined_at () =
  ...
  | Ok agent ->
    ...
    check (option string) "current_task preserved" (Some "task-208") agent.current_task;
    check
      string
      "last_seen bootstrapped from joined_at"
      "2026-04-15T03:00:00Z"
      agent.last_seen
  | Error msg -> fail ("null last_seen should bootstrap from joined_at: " ^ msg)
;;
  ...
  | Error msg ->
    let contains needle =
      ...
      let rec loop i =
        if i + n > h
        then false
        else if String.sub msg i n = needle
        then true
        else loop (i + 1)
      in
      loop 0
    in
    check bool "error mentions last_seen value" true (contains "last_seen=")
;;
...
let test_agent_of_yojson_missing_last_seen_falls_back_to_now () =
  ...
  let json =
    `Assoc
      ...
      ; "status", `String "active"
      ; "capabilities", `List []
      ; "current_task", `Null (* no joined_at, no last_seen *)
      ]
  in
  match Types.agent_of_yojson json with
  | Ok agent ->
    check string "agent parsed without last_seen or joined_at" "keeper-orphan" agent.name;
    check
      bool
      "last_seen populated with ISO timestamp"
      true
      (String.length agent.last_seen > 0
       && String.contains agent.last_seen 'T'
       && String.contains agent.last_seen 'Z')
  | Error msg -> fail ("missing last_seen+joined_at should fall back, not error: " ^ msg)
;;
...
let test_heartbeat_repairs_legacy_agent_last_seen () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       write_text_file
         (agent_path config "keeper-sangsu-agent")
       ...
       let repaired_json =
         ...
         | Ok raw -> raw |> Backend.Compression.decompress_auto |> Yojson.Safe.from_string
       in
       check
         bool
         "last_seen rewritten as string"
         true
...
let () =
  run
    "Coord_state_recovery"
    [ ( "room_state"
      , [ test_case "repairs empty object" `Quick test_read_state_repairs_empty_object
        ; test_case
            "recovers legacy active_agents entries"
            `Quick
            test_read_state_recovers_legacy_active_agent_entries
        ; test_case
            "filters invalid active_agents entries"
            `Quick
            test_read_state_filters_invalid_active_agent_entries
        ; test_case
            "agent parser accepts numeric last_seen"
            `Quick
            test_agent_of_yojson_accepts_numeric_last_seen
        ; test_case
            "agent parser bootstraps null last_seen from joined_at (#7947)"
            `Quick
            test_agent_of_yojson_bootstraps_null_last_seen_from_joined_at
        ; test_case
            "agent parser error annotates invalid last_seen (#7947)"
            `Quick
            test_agent_of_yojson_annotates_invalid_last_seen
        ; test_case
            "agent parser falls back when both last_seen and joined_at missing (#9751)"
            `Quick
            test_agent_of_yojson_missing_last_seen_falls_back_to_now
        ; test_case
            "heartbeat repairs legacy agent last_seen"
            `Quick
