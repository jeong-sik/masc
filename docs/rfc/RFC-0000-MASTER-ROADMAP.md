...
- **현재 상태:** **[갱신 2026-07-30]** periodic full-store consolidation은 maintenance fiber, runtime route, env toggle, prompt/schema와 함께 삭제되었다. production caller가 없으면서 canonical authority를 자칭하던 `Keeper_memory_os_store`도 삭제되어 실제 JSONL authority만 남았다. Memory fact의 `claim_kind`와 episode의 `open_items`/`constraints`/`preserved_tool_refs`는 producer·codec·API·dashboard에서 제거되며 current-only decoder가 재등장을 거부한다. `valid_for_days`/`valid_until`과 default-on expiry GC는 아직 context-invisible horizon authority로 남아 있어 별도 원자 hard cut이 필요하다. 호환 필드나 자동 마이그레이션 경로는 두지 않는다.
