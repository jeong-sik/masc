---
rfc: "0344"
title: "Durable store schema-version migration & boot preflight"
status: Draft
created: 2026-07-19
updated: 2026-07-20
author: vincent
...
| #25135 (**MERGED** `4190faacf1`) | gate approval order | v2→v3 | hard-cut + quarantine | 라이브 `<base-path>/.masc/gate/pending.json`(version=2, 367 durable HITL 레코드)을 migration 없이 `.v2.quarantine`으로 rename 후 빈 상태로 시작 → 367 레코드 orphan. 이 RFC 리뷰 기간(2026-07-20)에 머지된 **5번째 사례** |
...
(* §3.4 분기 근거. raw-file 절차를 SQLite에 적용하면 사이드카(-wal/-shm)를
   ...
   preflight가 런타임 추론 없이 분기할 수 있다. *)
type store_class =
  | Raw_file
  | Sqlite

type 'current descriptor = {
  ...
  store_class : store_class;                  (* §3.4 primitive 선택 *)
  current_version : 'current Store_version.witness;
    (* 이 바이너리가 쓰는 version. witness가 version 값을 타입 'current에
       결속해, 아래 migrate의 착지점을 컴파일러가 검사할 수 있게 한다. *)
  ...
  migrate : from_:migration_source -> Eio.Fs.dir_ty Eio.Path.t
            -> ('current Store_version.witness, Migration_error.t) result;
    (* from_ → current_version 변환. atomic. 대상 경로를 인자로 받아
       동적 store의 각 인스턴스에 적용된다. 성공 시 'current witness를
       돌려주므로 §3.3대로 중간 version 착지가 타입 에러가 된다. *)
}
```
...
base-path 배타 락(§3.2.1) 획득 후, **어떤 durable store도 읽히기 전에** 실행한다.

초안은 위치를 "keeper 시작 전"이라고 적었는데 **너무 늦다**. 실제 bootstrap은 keeper를 띄우기 한참 전에 이미 store를 복원한다 — `server_runtime_bootstrap.ml:629-635`가 `prepare_keeper_persistence`를 호출하고, 그 안에서 chat queue와 keeper-message 레코드가 `server_bootstrap_loops.ml:517,557`에서 복원된다. 그 시점의 복원은 구 version 파일을 만나 hard-reject하거나 빈 상태로 출발하므로, preflight가 그 뒤에 돌면 이미 유실된 뒤다.

따라서 배치 기준은 "keeper 시작 전"이 아니라 **"첫 durable read 전"**이다. 구체적으로 `prepare_keeper_persistence` 진입 이전이며, §4는 이 순서를 테스트로 고정한다(구 version 파일을 심고 bootstrap을 돌려, restore가 아니라 preflight가 먼저 그것을 보는지 assert).
...
**store class 디스패치도 preflight가 한다.** descriptor에 `store_class`를 두는 것만으로는 아무것도 강제되지 않는다 — 위처럼 preflight가 그 값으로 분기해야 `Sqlite` 선언이 실제로 SQLite-aware 경로를 선택한다.

**다만 backup만 분기해서는 부족하다.** 위 `run_migration`은 backup 함수만 고르고 두 arm 모두 같은 `d.migrate` 콜백을 부른다 — `Sqlite` descriptor의 `migrate`가 여전히 raw temp-file/rename 절차를 구현해 `-wal`/`-journal`/`-shm` 사이드카를 깨뜨려도 타입이 통과한다. class 선언이 backup 방식만 바꾸고 변환 방식은 자유롭게 두면, 그 선언은 절반짜리 약속이다.

따라서 **`migrate`의 타입을 class로 갈라야 한다** — descriptor가 `Raw_file`이면 raw primitive만 받는 변환 함수를, `Sqlite`면 SQLite 연결/backup API 위에서 동작하는 변환 함수를 요구한다(각 class가 서로 다른 변환 인터페이스를 갖는 형태). 그러면 SQLite store에 raw 절차를 넣는 것이 컴파일 에러가 되고, 위 `run_migration`의 분기는 backup 선택이 아니라 **이미 타입으로 분리된 두 경로의 디스패치**가 된다. 구체 인코딩(GADT / class별 모듈 / 분리된 descriptor 타입)은 구현 시점 결정이다.
...
preflight를 "새 프로세스의 boot 전"에만 두는 것으로는 부족하다 — 같은 base-path를 공유하는 **구 프로세스가 아직 쓰는 중**이면 preflight의 backup·rewrite가 라이브 write와 경합한다(§2 non-goal "old runtime이 쓰는 중 rewrite 금지"의 실제 강제 수단 부재). preflight 진입 시 `<base-path>/.masc/preflight.lock`에 대한 **배타 flock**을 획득하고, 실패하면(구 프로세스 잔존) migrate하지 않고 fatal로 종료한다. 락 보유 중에만 migration을 수행한다.

**락은 keeper 시작이 아니라 프로세스 수명 동안 유지한다.** 초안은 "keeper 시작까지"라고 적었는데, 그러면 첫 프로세스가 keeper 기동을 마치고 **서빙 중일 때** 두 번째 lock-aware 바이너리가 락을 얻어 store를 rewrite할 수 있다 — 이 락이 막으려던 바로 그 동시 write다. 락은 base-path를 쓰는 프로세스가 살아 있는 동안 계속 보유하고, 종료 시에만 놓는다(비정상 종료는 flock이 커널 레벨에서 회수하므로 stale lock이 남지 않는다).
...
exhaustive match만으로는 부족하다 — 새 arm이 **어느 version으로 착지하는지**는 강제되지 않아, `Gate_v4` 추가 후에도 `Gate_v2 → Gate_v3`(중간 version) migration이 exhaustive를 만족하며 통과할 수 있다. 따라서 `migrate`의 결과 타입에 target을 결속한다. §3.1의 `migrate : from_:migration_source -> path -> ('current Store_version.witness, Migration_error.t) result`에서 `'current`는 같은 descriptor의 `current_version : 'current Store_version.witness`와 동일한 파라미터다. migration이 성공을 주장하려면 착지 version의 witness를 반환해야 하고, 그 witness는 `'current`로만 타입이 맞으므로 current 아닌 version으로 착지하는 migration은 **타입 에러**가 된다(exhaustiveness가 아니라 target identity를 컴파일러가 강제).

`Store_version.witness`는 store별 closed variant `Store_version.t`에 대응하는 GADT 형태의 값 증거다 — 예컨대 `Gate_v3 : [`Gate_v3] Store_version.witness`. `t`(런타임 비교·파싱용)와 `witness`(타입 결속용)를 함께 두는 이유는, on-disk 값은 파싱 시점까지 어떤 version인지 알 수 없어 `t`로 받아야 하지만, `current_version`은 컴파일 타임에 하나로 확정되기 때문이다.
...
**SQLite store는 이 raw-file 절차를 쓰지 않는다.** chat queue(`chat-queue.sqlite3`)는 `-wal`/`-journal`/`-shm` 사이드카를 동반해, 라이브 DB 파일의 단순 copy·temp-rename은 DB를 손상시킨다. §3.1의 `store_class : store_class` 필드가 이 분기를 값으로 들고 있어, SQLite는 backup·변환을 SQLite backup API(`VACUUM INTO` 또는 online backup)로 수행하는 별도 primitive로 간다. 파일명·확장자 추론이 아니라 descriptor 등록 시점의 선언이므로, 새 store가 class를 빠뜨리면 등록이 컴파일되지 않는다. 기존 부분 선례(#25197 generation-bump, #25231 compat read)를 raw-file primitive로 수렴한다.

## 4. Acceptance

- **TLA+ bug model** (`specs/bug-models/DurableStoreSchemaMigration.tla`, TLC v1.8.0 검증): 실제 관측된 두 실패 모드를 모두 모델링한다 — `HardCutQuarantine`(bump 시 구 version 파일을 `.v2.quarantine`으로 rename하고 live 경로는 빈 상태로 시작, #25135 `4190faacf1`의 실제 전이) + `HardCutOrphan`(hard-cut reject: 파일은 디스크에 남고 store Unavailable, `live_rows=0`이나 boot ok → silent outage, #25078). `NoDurableRowLostOnBump` invariant(`boot_outcome=ok ⇒ live_rows=OldRows`; fatal boot은 fail-loud로 면제)는 두 모드 모두에서 위반된다(파일 잔존 여부와 무관하게 "ok인데 live row 없음"이 버그).

**모드별 개별 검증**: buggy spec을 하나로 합치면 두 액션 중 하나가 unreachable해지거나 안전해져도 나머지가 Init에서 한 걸음에 위반을 만들어 "invariant violated"가 그대로 나온다 — 기대-위반 검사가 통과하면서 실제로는 한 모드만 덮게 된다. 따라서 `SpecBuggyQuarantine`/`SpecBuggyOrphan`으로 나누고 cfg도 `-buggy.cfg`(A)·`-buggy-orphan.cfg`(B)로 분리해 각 모드를 단독 검증한다. 두 파일 모두 `-buggy` 세그먼트를 유지해야 한다 — `specs/Makefile`의 `BUGGY_CFGS`는 `*-buggy*.cfg` 글롭이고, 걸리지 않는 cfg는 "위반하면 안 되는" clean 모델로 분류된다. clean cfg = "No error has been found", 두 buggy cfg 각각 = "Invariant NoDurableRowLostOnBump is violated". 셋 모두 기대대로 나와야 spec 유효.

**적대 리뷰 정정(2026-07-20)**: mode A는 원래 §현황 위험 C(메모리 뱅크 drop-on-mismatch)로 인용했는데 잘못이었다. 메모리 뱅크는 mismatch 시 쓰기 전에 compaction을 거부하므로(`keeper_memory_bank.ml:567-582`) 파일을 rewrite하지도, on-disk version을 전진시키지도 않는다. 액션 자체는 #25135의 quarantine이라는 실재 전이라 삭제하지 않고 출처를 바로잡았다.
...
- **§3.5 등록 게이트가 vacuous하지 않음**: **음성 fixture**로 검증한다 — 의도적으로 미등록인 durable writer를 담은 테스트 트리에서 게이트가 FAIL해야 하고, 등록을 추가하면 PASS해야 한다. (초안은 "현재 트리에서 최소 1건 검출"을 요구했는데, 이는 바로 위 "미등록 0건" 항목과 **정면 모순**이었다 — 롤아웃이 완료되면 둘을 동시에 만족할 수 없어 acceptance가 미등록 store를 일부러 남겨두도록 강요한다. 스캐너가 살아 있음을 증명하는 일은 라이브 트리가 아니라 fixture가 해야 한다.)
...
1. **탐지**: durable 쓰기 API를 호출하면서 descriptor 레지스트리에 등록되지 않은 모듈을 스캔한다. grep 대상이 "store 이름"이 아니라 **"durable 쓰기 API"**라 새 store가 이름을 뭐라 짓든 걸린다.

   **단, API 집합이 한 계층이 아니다.** 초안은 `Fs_compat`의 durable append/rewrite 계열만 세면 된다고 했는데 틀렸다 — 실측하면 durable 쓰기는 최소 두 계층이다: `Fs_compat` 직접 호출(4개 파일)과 `Keeper_fs.save_json_durable_atomic` 같은 래퍼 경유(`keeper_msg_async.ml`, `keeper_chat_queue.ml`, `keeper_checkpoint_store.ml`, `fusion_delivery_obligation.ml`, `keeper_chat_direct_delivery.ml` 등 6개 파일). 한 계층만 세면 나머지가 통째로 새므로, 그 스캐너는 "이번엔 다 셌다"는 또 하나의 검증되지 않은 주장이 된다.

   따라서 탐지 대상은 **durable 쓰기 API의 폐포(transitive closure)**여야 한다: seed 집합(`Fs_compat` durable 계열)에서 시작해, 그것을 호출하면서 스스로도 durable 쓰기를 노출하는 래퍼(`Keeper_fs` 등)를 집합에 추가하고, 고정점에 도달할 때까지 반복한다. seed와 확장 규칙은 스캐너 소스에 명시하고, 새 래퍼 추가 시 집합이 자동으로 자라도록 한다.
...
| Gate approval order | `.masc/gate/pending.json` | `pending_store_version=3` (#25135 머지 후) | `keeper_approval_queue.ml:83`; v2 파일은 `quarantine_snapshot_unlocked`가 `.v2.quarantine`으로 rename 후 빈 상태 시작 |
...
### 위험 C — drop-on-mismatch (1개, 무경고 read 유실)

Keeper memory bank(`memory_bank.jsonl`, `keeper_memory_schema_version=2`): version 불일치 row를 조용히 `None`으로 drop (`keeper_memory_bank.ml:97-99`). bump 시 **읽기 경로에서 무경고로 사라진다**.

**정정(적대 리뷰 반영)**: 이 store는 *durable* 유실이 아니다. compaction이 mismatch를 감지하면 쓰기 전에 거부하고(`keeper_memory_bank.ml:567-582`, `"schema mismatch detected; refusing compaction"`), 유일한 rewrite 경로인 `rewrite_memory_bank_preserving_concurrent_appends`(`:474-489`, 호출 `:715`)는 else 분기라 mismatch 상태에서 도달 불가다. 즉 디스크 행은 남고 on-disk version도 전진하지 않는다. 그럼에도 우선순위는 유지한다 — **운영자에게 보이는 결과가 "데이터가 사라졌다"로 동일**하고(경고 없는 read 침묵), 되돌릴 방법이 migration뿐이기 때문이다. 다만 "durable 유실"이라는 표현은 과장이라 철회한다.
