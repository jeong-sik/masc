...
type recovery_action =
  ...
  | Terminal_no_usable_progress (* 더 시도 안 함, 기존 typed terminal 그대로 surface *)
```
...
3. recovery는 **최대 1회 re-shape attempt + 최대 1회 reroute**로 bounded. 그 다음은 기존 typed terminal `No_usable_progress` error를 그대로 surface한다.
...
- **PR-1 (recovery arm)**: `keeper_turn_driver_try_runtime.ml` accept-rejected 분기에 `recovery_action` typed variant + 단일 bounded recovery. RFC-0265 seam 재사용(reroute), RFC-0012 progress 신호 emit. 테스트: thinking_only 거부 → `Retry_no_thinking` → (성공/재거부) → reroute/terminal 분기 단위 테스트. `dune build --root .` 전체 green.
- **PR-2 (budget wiring)**: `max_thinking_budget` producer+consumer 동시 wiring(`worker_oas` `with_thinking_budget`). per-runtime config, incapable no-op. 라운드트립 테스트(설정값 → request에 반영). flash·qwen3.5:397b 같은 thinking-support runtime에서 ceiling 적용 확인.
