...
let opt_kind =
  testable
    (fun fmt -> function
       ...
       | Some (Agent_stress.Failure_streak n) -> Fmt.pf fmt "Some (Failure_streak %d)" n
       | Some Agent_stress.Fallback_approval -> Fmt.string fmt "Some Fallback_approval"
       | Some Agent_stress.Task_released -> Fmt.string fmt "Some Task_released"
       | Some (Agent_stress.Turn_failure _) -> Fmt.string fmt "Some Turn_failure")
    (fun a b ->
       match a, b with
       | None, None -> true
...
let test_turn_timeout_to_timeout () =
  ...
  check
    opt_kind
    "ambiguous_post_commit_timeout -> Timeout"
    (Some Agent_stress.Timeout)
    (classify "ambiguous_post_commit_timeout");
  check
    opt_kind
    "autonomous_slot_wait_timeout -> Timeout"
    (Some Agent_stress.Timeout)
    (classify "autonomous_slot_wait_timeout");
  check
    opt_kind
    "turn_timeout_after_queue_wait -> Timeout"
    (Some Agent_stress.Timeout)
...
let test_unmapped_kinds_return_none () =
  check opt_kind "cascade_exhausted -> None" None (classify "cascade_exhausted");
  check
    opt_kind
    "no_tool_capable_provider -> None"
    None
    (classify "no_tool_capable_provider");
  check opt_kind "empty string -> None" None (classify "");
  check opt_kind "whitespace -> None" None (classify "   ")
;;

let () =
  run
    "agent_stress_emit_10341"
    [ ( "classifier"
      , [ test_case
            "oas_timeout_budget maps to Timeout"
            `Quick
            test_oas_timeout_budget_to_timeout
        ; test_case "*_timeout suffix maps to Timeout" `Quick test_turn_timeout_to_timeout
        ; test_case
            "completion_contract_violation maps to Parse_degraded"
            `Quick
            test_completion_contract_violation_to_parse_degraded
        ; test_case "unmapped kinds return None" `Quick test_unmapped_kinds_return_none
        ] )
    ]
;;
