...
let source_name = "tool_call_io"
let source_producer = "keeper_hooks_oas|mcp_server_eio_call_tool"
...
let count_source_entries dir =
  if not (Sys.file_exists dir) then 0
    ...
    | exception exn ->
      Log.Dashboard.warn
        "tool_quality source entry count failed for %s: %s"
        dir
...
let health_fields ~now ~exists ~entry_count ~latest_ts ?coverage_gap () =
...
let coverage_gaps_for_store durable_store =
...
let source_metadata_fields () =
  ...
  let latest_ts =
    if exists then
      Option.bind (Keeper_tool_call_log.read_latest ()) latest_ts_of_record
    else
      None
  in
  let coverage_gaps = coverage_gaps_for_store durable_store in
  ...
  @ health_fields ~now ~exists ~entry_count ~latest_ts ?coverage_gap ()
