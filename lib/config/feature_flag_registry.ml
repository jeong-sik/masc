...
type flag =
  ...
  ; category : string
    (** Grouping: transport, keeper, dashboard, tool, inference, runtime *)
...
let all_flags : flag list =
  ...
  ; { env_name = "MASC_KEEPER_SMART_HEARTBEAT"
    ; description =
        "Skip heartbeat cycles when busy (task proves liveness) or extend interval when \
         idle"
    ; default = true
    ; category = "keeper"
    ; lifecycle = Active
  ...
  ; (* ── Runtime ──────────────────────────────────────────────── *)
    ...
    ; description =
        "Auto-orchestration background loop (superseded by zero-zombie cleanup)"
    ; default = false
    ; category = "runtime"
    ; lifecycle = Deprecated "superseded by zero-zombie cleanup since v2.130.0"
    ; since = "2.0.0"
    }
  ...
  ; { env_name = "MASC_CDAL_GATE_ENABLED"
    ; description =
        "CDAL verdict gate: block task completion when verdict is Violated/Inconclusive"
    ; default = true
    ; category = "runtime"
    ; lifecycle = Active
    ; since = "0.9.3"
  ...
  ; { env_name = "MASC_VERIFICATION_FSM_ENABLED"
    ; description =
        "Task verification FSM: AwaitingVerification state and cross-agent approval"
    ; default = true
    ; category = "runtime"
    ; lifecycle = Active
    ; since = "0.9.3"
...
let find_opt env_name = List.find_opt (fun f -> f.env_name = env_name) all_flags

(** Read runtime value using canonical default. *)
let runtime_value flag = get_bool ~default:flag.default flag.env_name

(** Source: "env", "boot_override", or "default". *)
let runtime_source flag = Config_boot_overrides.source flag.env_name
...
let to_json () =
  let categories =
    [ "transport"; "tool"; "keeper"; "dashboard"; "inference"; "runtime" ]
  in
  ...
  `Assoc
    ...
    ; ( "categories"
      , `Assoc
          (List.map
             (fun cat -> cat, `List (List.map flag_to_json (flags_in cat)))
             categories) )
    ]
;;

(** Flags where runtime value differs from canonical default. *)
let overridden_flags () = List.filter (fun f -> runtime_value f <> f.default) all_flags

(** Flags in deprecated lifecycle state. *)
let deprecated_flags () =
  List.filter
    (fun f ->
       match f.lifecycle with
       | Deprecated _ -> true
       | Active | Experimental -> false)
    all_flags
;;
