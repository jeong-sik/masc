...
let validation_error = testable Keeper_identity.pp_validation_error ( = )
let normalize input = Keeper_identity.normalize_all_names ~input_agent_name:input ()
...
let test_keeper_dash_agent_wrapper () =
  check_bundle
    ~label:"keeper-X-agent wrapper"
    ~input:"keeper-sangsu-agent"
    ~persona:"sangsu"
    ~keeper:"sangsu"
    ~agent:"keeper-sangsu-agent"
    ~credential:"sangsu"
;;

let test_keeper_underscore_agent_wrapper () =
  check_bundle
    ~label:"keeper_X_agent wrapper"
    ~input:"keeper_sangsu_agent"
    ~persona:"sangsu"
    ~keeper:"sangsu"
    ~agent:"keeper_sangsu_agent"
    ~credential:"sangsu"
;;

let test_ephemeral_suffix_only () =
  check_bundle
    ~label:"ephemeral suffix"
    ~input:"issue_king-pale-llama"
    ~persona:"issue_king"
    ~keeper:"issue_king"
    ~agent:"issue_king-pale-llama"
    ~credential:"issue_king"
;;

let test_wrapper_plus_ephemeral () =
  check_bundle
    ~label:"wrapper + suffix"
    ~input:"keeper-executor-warm-raven-agent"
    ~persona:"executor"
    ~keeper:"executor"
    ~agent:"keeper-executor-warm-raven-agent"
    ~credential:"executor"
;;
...
let test_empty_input () =
  ...
  | Error e ->
    check validation_error "empty input maps to Empty_input" Keeper_identity.Empty_input e
;;
...
let test_whitespace_only_input () =
  ...
  | Error e ->
    check
      validation_error
      "whitespace input maps to Empty_input"
      Keeper_identity.Empty_input
      e
;;
...
let test_round_trip_for_name name () =
  ...
  List.iter
    (fun shape ->
       let b = ok_or_fail ("round-trip shape " ^ shape) (normalize shape) in
       check
         string
         ("round-trip persona_name for " ^ shape)
         canonical_bundle.persona_name
         b.persona_name;
       check
         string
         ("round-trip keeper_name for " ^ shape)
         canonical_bundle.keeper_name
         b.keeper_name;
       check
         string
         ("round-trip credential_stem for " ^ shape)
         canonical_bundle.credential_stem
         b.credential_stem)
    (wrappers name)
;;
...
let with_tmp_base f =
  ...
  Fun.protect ~finally:(fun () -> rm_rf tmp_dir) (fun () -> f tmp_dir)
;;
...
let credential_path base stem =
  Filename.concat
    ...
    (Filename.concat
       ".masc"
       (Filename.concat "auth" (Filename.concat "agents" (stem ^ ".json"))))
;;
...
let test_check_credential_present () =
  with_tmp_base (fun base ->
    ...
    | Error e ->
      fail
        (Printf.sprintf
           "expected Ok with credential present, got %s"
           (Keeper_identity.show_validation_error e)))
;;
...
let test_check_credential_missing () =
  with_tmp_base (fun base ->
    ...
    | Error (Keeper_identity.Credential_missing { searched; resolved; input }) ->
    ...
    | Error other ->
      fail
        (Printf.sprintf
           "expected Credential_missing, got %s"
           (Keeper_identity.show_validation_error other)))
;;
...
let test_check_credential_wrapper_input () =
  ...
  with_tmp_base (fun base ->
    ...
    match
      Keeper_identity.normalize_all_names
        ~input_agent_name:"keeper-sangsu-agent"
        ~base_path:base
        ~check_credential:true
        ()
    with
    | Ok b ->
      check string "credential resolves to canonical stem" "sangsu" b.credential_stem;
...
let () =
  run
    "keeper_identity_normalize"
    [ ( "input_shape_matrix"
      , [ test_case "bare canonical" `Quick test_bare_canonical
        ; test_case "keeper-X-agent wrapper" `Quick test_keeper_dash_agent_wrapper
        ; test_case "keeper_X_agent wrapper" `Quick test_keeper_underscore_agent_wrapper
        ...
        ; test_case "invalid chars" `Quick test_invalid_chars_persona_not_found
        ] )
    ; ( "round_trip"
      , [ test_case "round-trip sangsu" `Quick (test_round_trip_for_name "sangsu")
        ; test_case "round-trip executor" `Quick (test_round_trip_for_name "executor")
        ; test_case "round-trip qa-king" `Quick (test_round_trip_for_name "qa-king")
        ; test_case
            "round-trip masc-improver"
            `Quick
