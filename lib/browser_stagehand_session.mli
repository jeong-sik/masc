(** One Stagehand runtime inside one Chromium, reached over one
    ...
    refuses new calls, so nothing takes effect after the caller was told the
    outcome is unknown. *)
...
and call_failure =
  ...
  | Connection_gone of string  (** The CDP connection ended. Refused before any effect. *)
...
(** Answers one [llm.generate] with its [params]. Runs on its own fiber. *)
...
(** Loads the extension at [extension_dir], attaches to its service worker,
    checks the runtime marker, and calls [stagehand.init] with
    [browser_cdp_url]. Returns the init result. *)
val attach :
