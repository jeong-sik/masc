...
let test_fact_row_line () =
  let fact : Decode.memory_fact =
    { mf_claim = "System uses Roger voice for fixture_worker"
    ; mf_category = "persona"
    ; mf_origin = "manual"
    ; mf_first_seen = 100.0
    ; mf_last_seen = 200.0
...
let test_fleet_fact_row_line () =
  let fact : Decode.memory_fact =
    { mf_claim = "System uses Roger voice for fixture_worker"
    ; mf_category = "persona"
    ; mf_origin = "fixture_worker · manual"
    ; mf_first_seen = 100.0
    ; mf_last_seen = 200.0
    ; mf_memory_id = "mem-1"
    ; mf_events = Decode.no_memory_fact_events
  ...
  check bool "fleet fact row has fixture_worker tag" true (contains "fixture_worker" stripped);
