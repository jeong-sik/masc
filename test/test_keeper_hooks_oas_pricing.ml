...
let test_known_model_returns_nonzero_cost () =
  ...
  let cost = Hooks.estimate_usage_cost_usd ~model:"claude-sonnet-4-6" usage in
  (* $3/M input + $15/M output = $3 + $1.5 = $4.5. Exact arithmetic;
     any pricing regression in the catalog will shift this. *)
  Alcotest.(check bool) "sonnet-4-6 priced > 0" true (cost > 0.0);
  Alcotest.(check (float 0.01)) "sonnet-4-6 exact" 4.5 cost
;;

let test_cache_tokens_use_provider_cache_rates () =
  let usage =
    mk_usage ~input:1_000_000 ~output:0 ~cache_creation:100_000 ~cache_read:200_000 ()
  in
  let cost = Hooks.estimate_usage_cost_usd ~model:"claude-sonnet-4-6" usage in
  (* regular input 700k * $3/M + cache write 100k * $3/M * 1.25
     + cache read 200k * $3/M * 0.1 = $2.535 *)
  Alcotest.(check (float 0.001)) "cache tokens use Anthropic cache multipliers" 2.535 cost
;;
...
let test_unknown_model_repeated_calls_accumulate () =
  ...
  let after = catalog_miss_for sentinel_model in
  Alcotest.(check (float 0.0001)) "3 calls → delta 3" 3.0 (after -. before)
;;
...
let test_ollama_model_known_free_no_catalog_miss () =
  ...
  Alcotest.(check (float 0.0001)) "known-free → NO catalog miss" 0.0 (after -. before)
;;
...
let test_pricing_for_model_opt_contract () =
  ...
  Alcotest.(check bool)
    ...
    true
    (Option.is_some (Pricing.pricing_for_model_opt "claude-sonnet-4-6"));
  Alcotest.(check bool)
    "totally-unknown not in catalog"
    true
    (Option.is_none (Pricing.pricing_for_model_opt "totally-unknown-pricing-probe"))
;;

let () =
  Alcotest.run
    "keeper_hooks_oas_pricing"
    [ ( "pricing_for_model_opt contract"
      , [ Alcotest.test_case
            "upstream opt-API semantics"
            `Quick
            test_pricing_for_model_opt_contract
        ] )
    ; ( "estimate_usage_cost_usd"
      , [ Alcotest.test_case
            "known model → non-zero cost"
            `Quick
            test_known_model_returns_nonzero_cost
        ; Alcotest.test_case
            "cache tokens use provider cache rates"
            `Quick
            test_cache_tokens_use_provider_cache_rates
        ; Alcotest.test_case
            "unknown model → catalog miss metric"
            `Quick
            test_unknown_model_emits_catalog_miss
        ; Alcotest.test_case
            "repeated miss accumulates"
            `Quick
            test_unknown_model_repeated_calls_accumulate
        ; Alcotest.test_case
            "known free (ollama) does NOT trip miss"
            `Quick
            test_ollama_model_known_free_no_catalog_miss
        ] )
    ]
;;
