...
let loop_memory_cache : (string, Oas.Memory.t) Hashtbl.t = Hashtbl.create 8

let make_loop_memory (ctx : Tool_autoresearch_context.t) (state : Autoresearch.loop_state)
  =
...
let build_goal_with_feedback
  ...
  let lesson_text =
    Memory_oas_bridge.render_lesson_prompt_context ~memory ~pattern ~limit:3
  in
  let finding_text =
    Autoresearch_knowledge.search_findings
  ...
  in
  [ Some state.goal; lesson_text; finding_text ]
...
let persist_failure_feedback
  ...
  Memory_oas_bridge.record_failure_lesson
    ...
    ?trace_summary
    ?metric_name:
      (Option.map (fun _ -> Autoresearch_metric.default_metric_name) metric_error)
    ?metric_error
  ...
  let finding : Autoresearch_knowledge.finding =
    ...
    ; conclusion = Option.value ~default:(clip summary 240) conclusion
    ; confidence = Autoresearch_knowledge.Medium
    ; tags = Autoresearch_lineage.finding_tags ~target_file:state.target_file ~extra:tags
    ; related_findings = []
    ; cycle_range = Some (state.current_cycle, state.current_cycle)
    ; timestamp = Unix.gettimeofday ()
    }
  in
  ignore (Autoresearch_knowledge.record_finding ~base_path:ctx.base_path ~finding)
;;

let check_patience_limit (state : Autoresearch.loop_state) =
  if state.consecutive_discards >= state.patience
  then (
    let state = { state with status = Autoresearch.Completed } in
    Autoresearch.add_insight
      state
      (Printf.sprintf
         "Early stopped: %d consecutive discards without improvement"
         state.patience))
  else state
;;
...
let forced_discard_record
  ...
  let state =
    Autoresearch.add_insight
      state
      (Printf.sprintf "Cycle %d: %s discarded (%s)" state.current_cycle hypothesis reason)
  in
...
let git_diff_patch ~workdir =
  ...
  | Unix.WEXITED code -> Error (Printf.sprintf "git diff exited with code %d" code)
...
let sync_target_file_to_index ~workdir ~target_file =
  ...
  | _ ->
    Log.Autoresearch.warn
      "failed to sync %s back into git index: %s"
      target_file
      (clip (String.trim output) 200)
;;

let restore_original_content ~workdir ~target_file ~original_content ~sync_index =
  match
    Autoresearch.apply_code_change ~workdir ~target_file ~new_content:original_content
  with
  | Ok _ when sync_index -> sync_target_file_to_index ~workdir ~target_file
  | Ok _ -> ()
  | Error err -> Log.Autoresearch.warn "failed to restore %s: %s" target_file err
;;
  ...
  | Some id ->
    let state_or_error =
      Autoresearch.with_loops_rw (fun () ->
        ...
        | Some state ->
          if state.status <> Autoresearch.Running
          then Error "Loop is not running"
          else if not (Autoresearch.should_continue state)
          then (
            let reason =
    ...
    (match state_or_error with
     | Error completed when String.starts_with ~prefix:"completed:" completed ->
       let reason = String.sub completed 10 (String.length completed - 10) in
       ...
       (match Autoresearch.validate_target_file ~workdir target_file with
        ...
        | Ok abs_path ->
          ...
          (match code_result with
           | Error e ->
             persist_failure_feedback
               ctx
               state
               ~hypothesis:"<generation_failed>"
               ~summary:(Printf.sprintf "Code generation failed: %s" e)
               ~action:
                 "Use prior lessons, keep the patch small, and regenerate only the \
                  target file."
               ~stderr:e
             ...
             (match before_result with
              | Error e ->
                persist_failure_feedback
                  ...
                  ~action:
                    ("Ensure the metric command exits 0 and emits either a final metric \
                      tag or "
                     ^ "a last-line float.")
                  ~metric_error:e
                  ~stderr:e
                   ...
                   (match git_diff_patch ~workdir with
                    | Error diff_err ->
                      ...
                      persist_failure_feedback
                        ...
                        ~hypothesis
                        ~summary:
                          (Printf.sprintf "Unable to inspect git diff: %s" diff_err)
                    ...
                    | Ok patch ->
                      ...
                      then (
                        ...
                        persist_failure_feedback
                          ...
                          ~hypothesis
                          ~summary:
                            (Printf.sprintf "Diff guard rejected patch: %s" summary)
                          ~action:
                            "Restrict edits to the declared target_file and avoid risky \
                        ...
                        | Error git_err ->
                          ...
                          persist_failure_feedback
                            ...
                            ~action:
                              "Keep git identity/hooks valid and retry after the target \
                               file is restored."
                            ~stderr:git_err
                          ...
                          `Assoc
                            [ ( "error"
                              , `String (Printf.sprintf "git commit failed: %s" git_err) )
                            ; "loop_id", `String id
                            ; "cycle", `Int state.current_cycle
                            ]
                        | Ok None ->
                          ...
                          build gate later downgrades Keep to Discard *)
                             ...
                             let build_gate_override =
                               ...
                               | Autoresearch.Keep ->
                                 (match state.build_verify_fn with
                                  ...
                                  | Some cmd ->
                                    (match
                                       Autoresearch_metric.split_metric_fn_argv cmd
                                     with
                                     | Error e ->
                                       Log.Autoresearch.warn
                                         "build_verify_fn validation failed: %s"
                                         e;
                                       true
                                     | Ok argv ->
                                       (match
                                          Autoresearch_metric.run_metric_argv
                                            ~workdir
                                            ~timeout_s
                                            argv
                                        with
                                          ...
                                          persist_failure_feedback
                                            ...
                                            ~summary:
                                              (Printf.sprintf
                                                 ...
                                                 reason)
                                            ~action:
                                              "Fix the build before the metric can \
                                               improve."
                                            ~stderr:reason
                             ...
                             let effective_decision, state =
                               ...
                               then (
                                 ...
                                 in
                                 let state =
                                   if state.best_cycle = state.current_cycle
                                   then
                                     { state with
                                       best_score = prev_best_score
                                     ; best_cycle = prev_best_cycle
                                     }
                                   else state
                                 in
                                 let state =
                                   Autoresearch.add_insight
                                     state
                                     (Printf.sprintf
                                        "Cycle %d: %s build verification failed, \
                                         downgraded to discard"
                                        state.current_cycle
                                        hypothesis)
                                 in
                                 Autoresearch.Discard, state)
                               else record.decision, state
                             in
                             let state =
                             ...
                             let effective_record =
                               if build_gate_override
                               then { record with decision = Autoresearch.Discard }
                               else record
                             in
                             let state =
                               if build_gate_override
                               then (
                                 ...
                                 | _ :: rest ->
                                   { state with history = effective_record :: rest }
                             ...
                             `Assoc
                               ...
                               ; ( "status"
                                 , `String (Autoresearch.status_to_string state.status) )
                               ; ( "target_reached"
                                 , `Bool (Autoresearch.target_reached state) )
                               ; "baseline", `Float state.baseline
