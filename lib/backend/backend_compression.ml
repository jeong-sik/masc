...
let encode_with_header ~(used_dict : bool) (orig_size : int) (compressed : string)
  : string
  =
  let header = Bytes.create 9 in
  if used_dict
  then Bytes.blit_string magic_dict 0 header 0 5 (* "ZSTDD" = 5 chars *)
  else (
...
let decode_header (data : string) : (int * string * bool) option =
  ...
  then None (* Check dictionary header FIRST (ZSTDD) *)
  else if Base.String.is_prefix data ~prefix:magic_dict
  then (
    (* Dictionary header: ZSTDD *)
    let orig_size =
      (Char.code data.[5] lsl 24)
      lor (Char.code data.[6] lsl 16)
      lor (Char.code data.[7] lsl 8)
      lor Char.code data.[8]
    in
  ...
  (* Then check standard headers *)
    ...
    then
      ...
      if String.length data >= 9 && data.[4] = '\x00'
      then (
        ...
        let orig_size =
          (Char.code data.[4] lsl 24)
          lor (Char.code data.[5] lsl 16)
          lor (Char.code data.[6] lsl 8)
          lor Char.code data.[7]
        in
...
let decompress ~(orig_size : int) ~(used_dict : bool) (compressed : string)
  : string option
  =
  match
