...
1. **GLM Cloud** (direct ZAI path; do not treat `llm-mcp` as the default GLM route)
2. **Gemini** (fallback)
3. Skip (둘 다 실패 시)
...
에이전트 생태계를 자율적으로 관리하는 시스템. Homeostatic balance 유지.

### 핵심 기능

| 기능 | 설명 |
|------|------|
| **Homeostatic Balance** | Target population (15) 기준 역U자 보상 — 과소/과다 모두 패널티 |
| **Gap Signal Processing** | 미충족 니즈 감지 → 적절한 에이전트 스폰 |
| **Retirement Management** | Idle 에이전트 퇴직 (최소 인원 보장) |
| **Safety Mechanisms** | Circuit breaker, 일일 예산, 쿨다운, Grace period |
...
# 인원 제한
MASC_GARDENER_MIN_AGENTS=5       # 최소 인원
MASC_GARDENER_TARGET_AGENTS=15   # 목표 인원 (homeostatic target)
MASC_GARDENER_MAX_AGENTS=30      # 최대 인원

# 예산 제한
MASC_GARDENER_MAX_DAILY_SPAWNS=3       # 일일 스폰 한도
MASC_GARDENER_MAX_DAILY_RETIREMENTS=2  # 일일 퇴직 한도

# 쿨다운 (초)
MASC_GARDENER_SPAWN_COOLDOWN_SEC=3600       # 스폰 간 대기 (1시간)
MASC_GARDENER_RETIREMENT_COOLDOWN_SEC=7200  # 퇴직 간 대기 (2시간)

# LLM 결정 사용 여부
MASC_GARDENER_USE_LLM=true

# 백그라운드 루프 주기 (초)
MASC_GARDENER_CHECK_INTERVAL_SEC=1800  # 30분
```
...
| 도구 | 설명 |
|------|------|
| `masc_gardener_health` | 생태계 건강 지표 조회 (인구, 활동, homeostatic score) |
| `masc_gardener_config` | 현재 설정 조회 + circuit breaker 상태 |
| `masc_gardener_propose_spawn` | 스폰 제안 평가 (승인/보류/거부) |
| `masc_gardener_execute_spawn` | 승인된 스폰 실행 (Neo4j 생성 + 공지) |
| `masc_gardener_retire_agent` | 퇴직 제안 평가 |
| `masc_gardener_execute_retire` | 승인된 퇴직 실행 (Grace period 시작) |
| `masc_gardener_reset_circuit` | Circuit breaker 수동 리셋 |
...
# 새 에이전트 스폰 제안
masc_gardener_propose_spawn(topic: "security", reason: "Need security review", urgency: "high")

# 승인되면 실행
masc_gardener_execute_spawn(topic: "security", urgency: "high")

# 에이전트 퇴직 제안
masc_gardener_retire_agent(agent_name: "idle-agent")
```

### Homeostatic Score 계산

```
score = 1.0 - |current - target| / (max - min)
```
...
1. **Circuit Breaker**: 연속 3회 실패 시 1시간 일시 정지
2. **Daily Budget**: 일일 스폰 3회, 퇴직 2회 제한
3. **Cooldown**: 스폰 간 1시간, 퇴직 간 2시간 대기
4. **Grace Period**: 퇴직 전 1-2시간 경고 기간
5. **Population Limits**: 최소 5명, 최대 30명 보장
