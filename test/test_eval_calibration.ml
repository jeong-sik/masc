...
let contains ~sub s =
  ...
  then false
    ...
    let rec scan i =
      if i > l - ls then false else if String.sub s i ls = sub then true else scan (i + 1)
    in
    scan 0)
;;
...
let make_result
  ...
  =
  { verdict
  ; evaluator_cascade = cascade
  ; generator_cascade = gen_cascade
  ; gate
  ; fallback_reason
  }
;;
...
let test_record_verdict_hash_matches () =
  ...
  let expected_hash =
    Cal.notes_hash ~task_title:req.task_title ~notes:req.completion_notes
  in
  let store = Cal.get_store () in
  let records = Dated_jsonl.read_recent store 10 in
  let first = List.hd records in
...
let test_record_human_label () =
  ...
  Cal.record_human_label
    ~notes_hash:"abc123"
    ~human_verdict:Cal.Reject_label
    ~labeler:"vincent"
...
let test_find_divergences_false_positive () =
  ...
  Cal.record_human_label
    ...
    ~human_verdict:Cal.Reject_label
    ~labeler:"vincent"
    ~reason:"did not address the task";
  let divs = Cal.find_divergences () in
  check int "1 divergence found" 1 (List.length divs);
  let d = List.hd divs in
  check string "evaluator approved" "approve" (Cal.verdict_to_string d.evaluator_verdict);
  check string "human rejected" "reject" (Cal.label_verdict_to_string d.human_verdict);
  Cal.reset_store_for_testing ()
;;
...
let test_find_divergences_false_negative () =
  ...
  Cal.record_human_label
    ~notes_hash:hash
    ~human_verdict:Cal.Approve_label
    ~labeler:"vincent"
    ~reason:"";
  let divs = Cal.find_divergences () in
  check int "1 divergence found" 1 (List.length divs);
  let d = List.hd divs in
  check string "human approved" "approve" (Cal.label_verdict_to_string d.human_verdict);
  Cal.reset_store_for_testing ()
;;
...
let test_format_few_shot_block_nonempty () =
  ...
  check bool "contains calibration header" true (contains ~sub:"calibration" block);
  check bool "contains task title" true (contains ~sub:"Fix auth" block)
;;
...
let test_calibration_stats () =
  ...
  let with_gen =
    Yojson.Safe.Util.(stats |> member "verdicts_with_generator_cascade" |> to_int)
  in
  let cross_match =
    Yojson.Safe.Util.(stats |> member "cross_model_match_count" |> to_int)
  in
  let cross_rate = Yojson.Safe.Util.(stats |> member "cross_model_rate" |> to_number) in
...
let test_calibration_stats_cross_model_mix () =
  ...
  let same_cascade = make_result ~cascade:"verifier" ~gen_cascade:"verifier" () in
  let cross_a =
    make_result
      ~cascade:"verifier"
      ~gen_cascade:Masc_mcp.Keeper_config.default_cascade_name
      ()
  in
  let cross_b = make_result ~cascade:"cross_verifier" ~gen_cascade:"local_only" () in
  let no_generator = make_result ~cascade:"verifier" () in
  let req = make_req () in
  Cal.record_verdict
    ~task_id:"cm1"
    ~req:(make_req ~title:"a" ~notes:"na" ())
    ~result:same_cascade
    ();
  Cal.record_verdict
    ~task_id:"cm2"
    ~req:(make_req ~title:"b" ~notes:"nb" ())
    ~result:cross_a
    ();
  Cal.record_verdict
    ~task_id:"cm3"
    ~req:(make_req ~title:"c" ~notes:"nc" ())
    ~result:cross_b
    ();
...
let test_to_harness_verdict_approve () =
  let record : Cal.verdict_record =
    { record_type = Cal.Verdict_record
    ; notes_hash = "abc"
    ; task_id = "t1"
    ...
    ; verdict = AR.Approve
    ; gate = AR.Structured_tool
    ; evaluator_cascade = "glm5"
    ; generator_cascade = Some "claude"
...
let test_to_harness_verdict_reject () =
  let record : Cal.verdict_record =
    ...
    ; agent_name = "coder"
    ; verdict = AR.Reject "too short"
    ; gate = AR.Length
    ; evaluator_cascade = "local"
  ...
  check
    bool
    "detail mentions gate"
    true
    (match hv.detail with
     | Some d -> contains ~sub:"length" d
     | None -> false)
;;
...
let test_on_harness_verdict_with_collector () =
  ...
  Cal.set_store_for_testing ~base_dir:dir;
  let collector =
    Agent_sdk.Eval.create_collector ~agent_name:"test-agent" ~run_id:"run-1"
  in
  let req = make_req () in
  let result = make_result () in
  Cal.record_verdict
...
let test_on_harness_verdict_exception_safe () =
  ...
  let store = Cal.get_store () in
  let records = Dated_jsonl.read_recent store 10 in
  check bool "record persisted despite callback failure" true (List.length records >= 1);
  Cal.reset_store_for_testing ()
;;
...
let () =
  run
    ...
    ; "human_label", [ test_case "writes label" `Quick test_record_human_label ]
    ; ( "divergences"
    ...
    ; ( "oas_integration"
      , [ test_case "callback invoked" `Quick test_on_harness_verdict_callback
        ...
        ; test_case
            "callback exception safe"
            `Quick
            test_on_harness_verdict_exception_safe
        ] )
    ]
;;
