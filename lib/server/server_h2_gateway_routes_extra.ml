...
let dispatch
  ...
  | `GET, "/api/v1/voice/config" ->
    ...
    in
    h2_respond_json h2_reqd (Yojson.Safe.to_string json) ~status ~extra_headers:cors;
    true
  | `GET, "/api/v1/board" ->
    ...
    in
    let limit =
      int_query_param httpun_request "limit" ~default:50 |> clamp ~min_v:1 ~max_v:200
    in
    let offset =
      int_query_param httpun_request "offset" ~default:0 |> clamp ~min_v:0 ~max_v:5000
    in
    let base_fetch =
  ...
  | `GET, "/api/v1/board/flairs" ->
    ...
    h2_respond_json h2_reqd (Yojson.Safe.to_string json) ~extra_headers:cors;
    true
  | `GET, p when String.starts_with ~prefix:"/api/v1/board/" p && String.length p > 14 ->
    ...
    let json =
      `Assoc
        [ ( "karma"
          , `List
              (List.map
                 (fun (agent, k) -> `Assoc [ "agent", `String agent; "karma", `Int k ])
                 sorted) )
        ]
    in
    h2_respond_json h2_reqd (Yojson.Safe.to_string json) ~extra_headers:cors;
  ...
  | `GET, "/static/js/middleware.js" ->
    (match read_file (playground_asset_path "static/js/middleware.js") with
     ...
     | Error _ -> h2_respond_text h2_reqd "404 Not Found" ~status:`Not_found);
    true
  | `GET, p when String.starts_with ~prefix:"/dashboard/assets/" p && String.length p > 18
    ->
    let filename = String.sub p 18 (String.length p - 18) in
    if not (Web_dashboard.is_safe_asset_relative_path filename)
    then (
      ...
      (match read_file file_path with
       | Ok body ->
         ...
         in
         let accepts_zstd = Http_server_eio.Compression.accepts_zstd httpun_request in
         let final_body, encoding_headers =
           if is_compressible && accepts_zstd
           then (
