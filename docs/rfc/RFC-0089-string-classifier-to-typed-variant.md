...
이는 CLAUDE.md §"워크어라운드 거부 기준 §2 String/Substring 분류기 보강" 정의에
...
| File:Line | 패턴 | 정당화 |
|---|---|---|
| `lib/keeper/keeper_gh_shared.ml:401, 414, 595, 602` | `"-" / "--repo=" / ";"` | CLI argv tokenizer — 외부 입력 string의 lexer |
...
- `String.starts_with ~prefix:"..."` lib/ 합계: **215 site**
...
도메인별로 다음이 동시 성립할 때 본 RFC가 Implemented로 이행:
...
- RFC-0042 (`keeper_turn_terminal.t.code` closed sum, Draft 2026-05-08) — 동일
  안티패턴의 한 도메인.
- CLAUDE.md §"워크어라운드 거부 기준 §2 String 분류기 보강".
