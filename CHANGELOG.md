...
- Keeper: `[turn] context_window_tokens` in `runtime.toml` (env `MASC_KEEPER_CONTEXT_WINDOW_TOKENS`, compiled default 85,000) sets how much history an Agent-Core-lane request carries. `max-request-body-bytes` no longer sizes that window, so raising the cap no longer sends more history; a workspace that raised it for that reason sets the token window instead. A runtime candidate whose `max-context` is below the declared window is refused before dispatch (#36709).
...
- Keeper: `GET /api/v1/keepers/:name/next-request` reports what the keeper's next request would carry, without firing a turn. It runs the turn driver's arithmetic forward: the declared window times the runtime's measured density, less the tool schemas, keeper instructions and pinned prompt blocks as the newest turn record measured them, then the cut over the durable checkpoint with the autonomous wake line as the newest atom. Nothing dispatches, advances a board cursor or consumes an operator note, and only the keeper's bound runtime is forecast. A runtime with no density yet reports the newest-atom floor the turn would send. The MASC Context screen draws the answer as a NEXT REQUEST band under HOW FAR BACK, in tokens; a refused window is drawn with the driver's reason, and a server that does not serve the endpoint is named in the band (#36757, #36760).
...
- Keeper: the cut that fills an Agent-Core-lane request still measures bytes, and the bridge to the token window is a density observed per runtime from the previous response — that response's `input_tokens` over the bytes MASC measured for the request. There is no bytes-per-token constant: a runtime with no observation sends the newest atom only until the first response reports usage. The request-body cap keeps two jobs, admitting the serialized request and demoting this turn's tool results when the smallest view that carries the turn cannot be sent at all. A turn logs its declared and effective window, an overrun, and a window that contradicts the cap, once per attempt (#36709).
- TUI: the MASC Context screen reads in tokens. The three bands are named for the question each answers: `WHAT WENT IN` for the pieces the request was built from, `THIS REQUEST, AS SENT` for the provider's count of it beside the estimate from the body MASC sent, and `HOW FAR BACK` for how much of the kept conversation it carried. The composition rows, the composition rows, the request tab's items, the proof tab's rows and the skill delivery row on the Tools screen are drawn as `≈N tok`, with the bytes they were read from beside the sentence that names the ratio. The ratio is this record's own body over its per-request count, else the median of the page's rows that carried both, else a fleet median; a conversation-cumulative or unknown-scope count never becomes the ratio, and the sentence says which of those it refused. Disk sizes, such as memory snapshots and checkpoints, stay in bytes (#36685, #36692, #36707, #36724, #36742).
...
- Keeper: the transmission window reads in the operator settings panel. `turn.context_window_tokens` was registered without an arm in the effective-value projector, so the row an operator changes to send more or less history reached the panel with a null value and an error string (#36784).
