...
let test_without_snapshot_seed_demotes_earlier_tool_bodies () =
  ...
  let front : Front.seed =
    {first_atom = 0; front_digest = Some front_digest; source = Front.Ledger}
  in
...
let test_an_old_front_does_not_drop_unread () =
  ...
  let front : Front.seed =
    {first_atom = 4; front_digest = Some front_digest; source = Front.Ledger}
  in
...
let test_without_snapshot_starts_at_the_turn_start () =
  ...
  let front : Front.seed =
    {first_atom = seed_atom; front_digest = Some front_digest; source = Front.Ledger}
  in
  let seeded =
  ...
  let stale : Front.seed =
    {first_atom = seed_atom; front_digest = Some other_digest; source = Front.Ledger}
  in
  let dropped =
...
let test_the_forecast_takes_the_drivers_start () =
  ...
  let held_seed : Front.seed =
    {first_atom = 0
    ; front_digest = Some (Option.get (digest_at 0))
    ; source = Front.Ledger} in
  let outlived_seed : Front.seed =
    {first_atom = 0
    ; front_digest = Some "not-the-opening-message"
    ; source = Front.Ledger} in
  ...
  let accepted : Front.seed =
    {first_atom = 2
    ; front_digest = Some (Option.get (digest_at 2))
    ; source = Front.Turn_record { turn = 3 }} in
  let cases =
...
let test_a_start_past_the_snapshot_keeps_the_working_state () =
  ...
  let accepted : Front.seed =
    {first_atom = 3
    ; front_digest = Some (Option.get (digest_at 3))
    ; source = Front.Turn_record { turn = 5 }} in
  let projected =
