  ...
  run
    ...
    [ ( "register_and_dispatch"
      , [ test_case "register single tool and dispatch" `Quick (fun () ->
        ...
        ; test_case "mint_token unknown tool returns Error" `Quick (fun () ->
            let result =
              Tool_dispatch.mint_token ~name:"__test_dispatch_nonexistent_xyz"
            in
            check bool "is Error" true (Result.is_error result))
        ; test_case "register_module bulk registers" `Quick (fun () ->
            let schemas =
              List.map make_schema [ "__test_bulk_a"; "__test_bulk_b"; "__test_bulk_c" ]
            in
            Tool_dispatch.register_module ~schemas ~handler:echo_handler;
            List.iter
              (fun s ->
                 Tool_dispatch.register_name_tag ~tool_name:s.Types.name ~tag:Mod_misc)
              schemas;
            List.iter
              (fun name ->
                 check bool (name ^ " registered") true (Tool_dispatch.is_registered name))
              [ "__test_bulk_a"; "__test_bulk_b"; "__test_bulk_c" ])
        ; test_case "register_module dispatches each name" `Quick (fun () ->
            let token =
              match Tool_dispatch.mint_token ~name:"__test_bulk_b" with
              | Ok t -> t
              | Error e -> Alcotest.fail e
            in
            let result = Tool_dispatch.dispatch ~token ~args:`Null in
    ...
    ; ( "replace_semantics"
      , [ test_case "re-register replaces handler" `Quick (fun () ->
            ...
            let token1 =
              ...
              | Error e -> Alcotest.fail e
            in
            let ok1, _ = Option.get (Tool_dispatch.dispatch ~token:token1 ~args:`Null) in
    ...
    ; ( "registry_queries"
      , [ test_case "is_registered reflects state" `Quick (fun () ->
            check bool "bulk_a exists" true (Tool_dispatch.is_registered "__test_bulk_a");
            check
              bool
              "unknown absent"
              false
        ...
        ; test_case "registered_count >= registered tools" `Quick (fun () ->
            ...
            check bool "count >= 5" true (Tool_dispatch.registered_count () >= 5))
        ] )
    ; ( "read_only_set"
      , [ test_case "init and query read_only" `Quick (fun () ->
        ...
        ; test_case "keeper read-only tools use shipped registry policy" `Quick (fun () ->
            ...
            check
              bool
              "keeper_tasks_list read_only"
              true
              (Tool_dispatch.is_read_only "keeper_tasks_list");
            check
              bool
              "keeper_memory_search read_only"
              true
    ...
    ; ( "requires_join_set"
      , [ test_case "known join-required tools" `Quick (fun () ->
            ...
            Tool_dispatch.init_requires_join_set [ "masc_broadcast"; "masc_transition" ];
            check
              bool
              "masc_broadcast"
              true
        ...
        ; test_case "non-join-required tool returns false" `Quick (fun () ->
            check bool "masc_status" false (Tool_dispatch.is_join_required "masc_status");
            check bool "masc_who" false (Tool_dispatch.is_join_required "masc_who"))
        ; test_case "worktree list uses shipped registry policy" `Quick (fun () ->
            ignore (Mcp_eio.get_clock_opt ());
            check
              bool
              "masc_claim_next join_required"
              true
              (Tool_dispatch.is_join_required "masc_claim_next");
            check
              bool
              "masc_worktree_list read_only"
              true
              (Tool_dispatch.is_read_only "masc_worktree_list");
            check
              bool
              "masc_worktree_list not join_required"
              false
              (Tool_dispatch.is_join_required "masc_worktree_list"))
        ] )
    ; ( "mcp_context_required_set"
      , [ test_case
            "inline tools can be marked as requiring mcp context"
            `Quick
            (fun () ->
    ...
    ; ( "handler_exception_safety"
      , [ test_case "throwing handler returns error tuple" `Quick (fun () ->
            ...
            let throwing_handler ~name:_ ~args:_ = failwith "boom" in
    ...
    ; ( "did_you_mean_9784"
      , [ test_case "find_similar_names returns close match" `Quick (fun () ->
            ...
            let suggestions =
              Tool_dispatch.find_similar_names ~query:"__sim_masc_claim_task" ()
            in
            check bool "non-empty suggestions" true (List.length suggestions >= 1);
            check
              bool
              "top suggestion is closest"
              true
        ...
        ; test_case "find_similar_names respects limit" `Quick (fun () ->
            ...
            let suggestions =
              Tool_dispatch.find_similar_names ~limit:2 ~query:"__limit_test_tool_1" ()
            in
            check bool "at most 2 returned" true (List.length suggestions <= 2))
        ; test_case "all_registered_names enumerates registry" `Quick (fun () ->
            ...
            check bool "contains registered name" true (List.mem "__enum_check_xyz" all))
        ] )
    ]
;;
