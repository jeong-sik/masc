  ...
  List.filter
    (function
      ...
      | Trajectory.Thinking entry ->
        let key =
          Printf.sprintf "%.6f\x1f%b\x1f%s" entry.ts entry.redacted entry.content
        in
...
let internal_history_json_to_trajectory_line (json : Yojson.Safe.t)
  ...
  if source <> "internal_assistant" || String.trim content = ""
  then None
  else (
    let ts =
...
let read_internal_history_lines ~(config : Coord.config) ~(trace_id : string)
  ...
  Fs_compat.load_jsonl path |> List.filter_map internal_history_json_to_trajectory_line
;;

let merge_keeper_trace_lines
      ~(config : Coord.config)
      ~(trace_id : string)
...
let keeper_tools_response_json (meta : Keeper_types.keeper_meta) =
  ...
  let tool_custom_allowlist =
    Keeper_types.tool_access_custom_allowlist meta.tool_access |> Option.value ~default:[]
  in
  `Assoc
    [ "ok", `Bool true
...
let handle_keeper_tools_post state req reqd =
  Http.Request.read_body_async reqd (fun body_str ->
    ...
    then Http.Response.json ~status:`Bad_request {|{"error":"keeper name required"}|} reqd
    else (
        ...
        (try
           ...
           let updated_meta =
             ...
             | "set_policy" ->
               ...
               in
               let deny = Safe_ops.json_string_list "deny" args |> dedupe_tool_names in
                       ...
                       | None ->
                         Error
                           (Printf.sprintf
                              ...
                              raw
                              (String.concat ", " Keeper_types.valid_tool_preset_strings))
                       | Some preset -> Ok (Keeper_types.Preset { preset; also_allow })))
                 | "custom" ->
                   if not allow_present
                   then Error "allow required when mode=custom"
                   else Ok (Keeper_types.Custom allow)
                 | "full" ->
                   Ok
             ...
             (match Keeper_types.write_meta ~force:true config meta' with
              | Ok () ->
                Http.Response.json
                  ~compress:true
                  ~request:req
                  (Yojson.Safe.to_string (keeper_tools_response_json meta'))
                  reqd
              | Error e ->
                Http.Response.json
                  ~status:`Internal_server_error
                  (Printf.sprintf {|{"error":"write failed: %s"}|} (String.escaped e))
                  reqd)
         with
         | Yojson.Json_error e ->
           Http.Response.json
             ~status:`Bad_request
             (Printf.sprintf {|{"error":"invalid json: %s"}|} (String.escaped e))
             reqd)))
;;
...
let classify_keeper_post_route req_path =
  ...
  then Keeper_post_unknown
  else (
    let plen = String.length keeper_api_prefix in
    let tlen = String.length req_path in
    let ends_with suffix =
      tlen > plen + String.length suffix && String.ends_with ~suffix req_path
    in
    if ends_with keeper_suffix_tools
    then Keeper_post_tools
    else if ends_with keeper_suffix_config
    then Keeper_post_config
    else if ends_with keeper_suffix_boot
    then Keeper_post_boot
    else if ends_with keeper_suffix_shutdown
    then Keeper_post_shutdown
    else if ends_with keeper_suffix_reset
    then Keeper_post_reset
    else if ends_with keeper_suffix_clear
    then Keeper_post_clear
    else if ends_with keeper_suffix_checkpoints
    then Keeper_post_checkpoints
    else if ends_with keeper_suffix_directive
    then Keeper_post_directive
...
let extract_keeper_name_for_suffix req_path suffix =
  ...
  let slen = String.length suffix in
  let raw =
    String.trim (String.sub req_path plen (String.length req_path - plen - slen))
  in
  let valid =
    String.length raw > 0
...
let truncate_text ~max_chars text =
  ...
  then String.sub text 0 (max 0 max_chars)
  else
    String_util.utf8_safe ~max_bytes:max_chars ~suffix:"…" text |> String_util.to_string
;;
...
let latest_preview_of_messages (messages : Oas.Types.message list) =
  ...
  |> List.find_map (fun (message : Oas.Types.message) ->
    if message.role = Oas.Types.System
    then None
    else
...
let continuity_summary_of_messages (messages : Oas.Types.message list) =
  ...
  | Some snapshot ->
    Keeper_memory_policy.keeper_state_snapshot_to_summary_text snapshot |> trim_to_opt
  | None -> None
;;
...
let keeper_checkpoint_inventory_json (config : Coord.config) (name : string)
  ...
  | Ok (Some (_, meta)) ->
    ...
    let history_json =
      ...
      |> List.filter_map (fun snapshot_id ->
        match Keeper_checkpoint_store.load_oas_history_file ~session_dir ~snapshot_id with
        | Ok checkpoint ->
          Some
            (oas_checkpoint_summary_json
...
let handle_keeper_checkpoints_post state req reqd body_str =
  ...
  then
    Http.Response.json
      ~status:`Bad_request
      {|{"ok":false,"error":"keeper name is required"}|}
      reqd
  else (
    let config = state.Mcp_server.room_config in
    try
      ...
      | "delete_history" ->
        ...
        then
          Http.Response.json
            ~status:`Bad_request
            {|{"ok":false,"error":"snapshot_ids is required"}|}
            reqd
        else (
          let trace_id_result =
            match Keeper_types.read_meta_resolved config name with
            ...
            | Ok None -> Error (Printf.sprintf "keeper %S not found" name)
            | Error msg -> Error msg
          in
          ...
          | Ok trace_id ->
            let session_dir = Keeper_types.keeper_session_dir config trace_id in
            let deleted, missing =
              Keeper_checkpoint_store.delete_oas_history_files ~session_dir ~snapshot_ids
            in
            let _status, inventory = keeper_checkpoint_inventory_json config name in
            Http.Response.json
              ~compress:true
              ~request:req
              (Yojson.Safe.to_string
                 (`Assoc
                     ...
                     ; ( "deleted_snapshot_ids"
                       , `List (List.map (fun id -> `String id) deleted) )
                     ; ( "missing_snapshot_ids"
                       , `List (List.map (fun id -> `String id) missing) )
                     ; "inventory", inventory
                     ]))
              reqd)
      | "" ->
      ...
      | other ->
        Http.Response.json
          ~status:`Bad_request
          (Printf.sprintf
             {|{"ok":false,"error":"unknown action: %s"}|}
    ...
    | Yojson.Json_error e ->
      Http.Response.json
        ~status:`Bad_request
        (Printf.sprintf {|{"ok":false,"error":"invalid json: %s"}|} (String.escaped e))
        reqd)
;;
...
let handle_keeper_config_post ~sw ~clock state agent_name req reqd body_str =
  ...
  if String.length name = 0
  then
    Http.Response.json ~status:`Bad_request {|{"error":"keeper name is required"}|} reqd
  else (
    let config = state.Mcp_server.room_config in
    match Keeper_types.read_meta config name with
    | Error msg ->
      ...
      (try
         ...
         | Some fields ->
           let body_name =
             ...
             | _ -> None
           in
           if Option.is_some body_name && body_name <> Some name
           then
             Http.Response.json
               ~status:`Bad_request
               (Printf.sprintf
             ...
             | Ok parsed ->
               ...
               then
                 ...
                 let _st, json = Dashboard_http_keeper.keeper_config_json config name in
                 Http.Response.json
                   ~compress:true
                   ~request:req
...
let handle_keeper_lifecycle_post
  ...
  let suffix_result =
    ...
    | unknown -> Error (Printf.sprintf "unknown keeper lifecycle action: %s" unknown)
  in
  ...
  | Ok suffix ->
    ...
    then
      ...
      let resolve_keeper_agent_name () =
        ...
        | None ->
          (match Keeper_types.read_meta config name with
           ...
           | Ok None -> None
           | Error err ->
             Log.Keeper.warn "resolve_keeper_agent_name %s: read_meta failed: %s" name err;
             None)
      in
      let persist_keeper_paused_state paused =
        ...
        | Ok (Some meta) ->
          let updated_meta = { meta with paused; updated_at = Keeper_types.now_iso () } in
      ...
      let resume_booted_keeper_if_needed () =
        ...
        | Ok (Some meta) when meta.paused ->
          ...
          (match resolve_keeper_agent_name () with
           ...
           | None ->
             Log.Keeper.warn "keeper boot: agent_name not found for paused keeper %s" name)
        | Ok (Some _) -> ()
      ...
      let args_result =
        ...
        | "clear" ->
          (match body_str with
           | None -> Error "request body is required for clear"
           | Some raw ->
             (try
              ...
              with
              | Yojson.Json_error err -> Error (Printf.sprintf "invalid json: %s" err)))
        ...
        (match Tool_keeper.dispatch keeper_ctx ~name:tool_name ~args with
         ...
         | Some (false, body) ->
           Http.Response.json
             ~status:`Bad_request
             ~request:req
             (Yojson.Safe.to_string (`Assoc [ "ok", `Bool false; "error", `String body ]))
             reqd
         | None ->
           Http.Response.json
             ~status:`Internal_server_error
  ...
  then
    ...
    let action =
      try
        ...
        | Some a ->
          Error (Printf.sprintf "invalid action %S: expected pause, resume, or wakeup" a)
    ...
    | Error msg ->
      Http.Response.json
        ~status:`Bad_request
        (Printf.sprintf {|{"ok":false,"error":%s}|} (Yojson.Safe.to_string (`String msg)))
        reqd
    | Ok action_str ->
      let config = state.Mcp_server.room_config in
      ...
      let meta_opt =
        ...
        | Ok None -> None
        | Error err ->
          Log.Keeper.warn "directive %s %s: read_meta failed: %s" action_str name err;
          None
      in
      ...
      let proceed () =
        ...
        (match action_str with
         ...
         | "wakeup" -> ()
         | _ -> Log.Server.warn "Unknown keeper directive: %s" action_str);
      ...
      (match read_result, needs_meta_for_state_transition with
       | Error err, true ->
         Log.Keeper.error "directive %s: read_meta failed for %s: %s" action_str name err;
         Prometheus.inc_counter
           Prometheus.metric_keeper_paused_state_persist_errors
           ~labels:[ "phase", "directive"; "reason", "read_meta_error" ]
           ();
         Http.Response.json
           ~status:`Internal_server_error
           ~request:req
       ...
       | Ok None, true ->
         ...
         Prometheus.inc_counter
           Prometheus.metric_keeper_paused_state_persist_errors
           ~labels:[ "phase", "directive"; "reason", "meta_missing" ]
           ();
         Http.Response.json
           ~status:`Not_found
           ~request:req
       ...
       | Error err, false ->
         ...
         Log.Keeper.warn
           ...
           err;
         proceed ()
       | Ok None, false | Ok (Some _), _ -> proceed ()))
;;
...
let handle_keeper_get_subroutes state req request reqd =
  ...
  let ends_with suffix =
    let slen = String.length suffix in
    tlen > plen + slen && String.sub req_path (tlen - slen) slen = suffix
  in
  let extract_name suffix =
    let slen = String.length suffix in
    String.trim (String.sub req_path plen (tlen - plen - slen))
  in
  if ends_with "/chat/history"
  then (
    let name = extract_name "/chat/history" in
    if name = ""
    then
      Server_auth.respond_json_with_cors
        ~status:`Bad_request
        request
        reqd
  ...
  then (
    ...
    then
      ...
      let config = state.Mcp_server.room_config in
      let st, json = Dashboard_http_keeper.keeper_config_json config name in
      let status : Httpun.Status.t =
        match st with
        | `OK -> `OK
        | `Not_found -> `Not_found
  ...
  then (
    ...
    then
      ...
      in
      let entries = Keeper_tool_call_log.read_recent ~keeper_name:name ~n:limit () in
      let config = state.Mcp_server.room_config in
      let masc_root = Coord.masc_root_dir config in
      let latest_ts =
        List.fold_left
      ...
      let coverage_gaps =
        Telemetry_coverage_gap.read_recent ~masc_root ~n:32
        |> List.filter (fun gap ->
          String.equal "tool_call_io" (Safe_ops.json_string ~default:"" "source" gap)
          &&
          match Safe_ops.json_string_opt "keeper_name" gap with
          | Some keeper_name -> String.equal keeper_name name
          | None -> true)
  ...
  then (
    ...
    then
      ...
      | Ok (Some m) ->
        ...
        let limit =
          Server_utils.int_query_param req "limit" ~default:trajectory_default_limit
          |> max 1
          |> min trajectory_max_limit
        in
        ...
        let trajectory_lines =
          Trajectory.read_all_lines ~masc_root ~keeper_name:m.name ~trace_id
        in
        let all_lines =
          if include_thinking
          then merge_keeper_trace_lines ~config ~trace_id trajectory_lines
        ...
        let json =
          `Assoc
            ...
            ; ( "entries"
              , `List
                  (List.map
                     (Trajectory.trajectory_line_to_json ~result_max_len ~content_max_len)
                     recent) )
            ]
        in
        Http.Response.json ~compress:true ~request:req (Yojson.Safe.to_string json) reqd))
  else if ends_with "/transitions"
  then (
  ...
  then (
    ...
    then
      ...
      in
      Http.Response.json ~compress:true ~request:req (Yojson.Safe.to_string json) reqd))
  else if ends_with "/state-diagram"
  then (
    ...
    then
      ...
      let meta = Keeper_types.read_meta state.Mcp_server.room_config name in
      let tool_count =
        match meta with
        | Ok (Some m) -> List.length (Keeper_exec_tools.keeper_allowed_tool_names m)
        | _ -> 0
      in
      ...
      let turn_outcome : [ `Ok | `Failed ] option =
        match
          Keeper_registry.get ~base_path:state.Mcp_server.room_config.base_path name
        with
        ...
        | Ok (Some m) ->
          let routing =
            Keeper_cascade_routing.select_cascade
              ~base_cascade:m.cascade_name
              ~phase:current
          in
          let models = Cascade_runtime.models_of_cascade_name routing.effective_cascade in
          let last_model = m.runtime.usage.last_model_used in
          let last_provider_result = if last_model <> "" then Some last_model else None in
      ...
      let cascade_models =
        match meta with
        | Ok (Some m) -> Cascade_runtime.models_of_cascade_name m.cascade_name
        | _ -> [ "(unknown)" ]
      in
      let last_provider =
        match meta with
      ...
      let memory_kind_usage : Yojson.Safe.t =
        ...
        let used_by_kind =
          ...
          | _ -> []
        in
        let lookup_used k = List.assoc_opt k used_by_kind |> Option.value ~default:0 in
        `List
          (List.map
             (fun (kind, cap) ->
                `Assoc
      ...
      in
      Http.Response.json ~compress:true ~request:req (Yojson.Safe.to_string json) reqd))
  else if req_path = prefix ^ "composite"
  then (
  ...
  then (
    ...
    then
      ...
      | None ->
        Http.Response.json
          ~status:`Not_found
          (Printf.sprintf {|{"error":"keeper %S not registered"}|} name)
          reqd
      | Some entry ->
        let json =
        ...
        in
        Http.Response.json ~compress:true ~request:req (Yojson.Safe.to_string json) reqd))
  else if req_path = prefix ^ "regime"
  then (
    ...
    let base_path = state.Mcp_server.room_config.base_path in
    let snapshots = Keeper_behavioral_regime_observer.all_snapshots ~base_path () in
    let json =
      `Assoc
        [ "generated_at", `Float (Unix.gettimeofday ())
        ; "count", `Int (List.length snapshots)
        ; ( "snapshots"
          , `List (List.map Keeper_behavioral_regime_observer.snapshot_to_json snapshots)
          )
        ]
    in
    Http.Response.json ~compress:true ~request:req (Yojson.Safe.to_string json) reqd)
  else if ends_with "/regime"
  then (
    ...
    then
      ...
      | None ->
        Http.Response.json
          ...
          reqd
      | Some entry ->
        let snapshot = Keeper_behavioral_regime_observer.observe entry in
        let json = Keeper_behavioral_regime_observer.snapshot_to_json snapshot in
        Http.Response.json ~compress:true ~request:req (Yojson.Safe.to_string json) reqd))
  else Http.Response.json ~status:`Not_found {|{"error":"not found"}|} reqd
;;
