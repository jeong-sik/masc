...
let test_find_matching_no_filter () =
  ...
  in
  let matches =
    Subscriptions.SubscriptionStore.find_matching
      ~resource:Subscriptions.Tasks
...
let test_find_matching_with_filter () =
  ...
  check
    ...
    true
    (List.exists
       (fun (s : Subscriptions.subscription) -> s.filter = Some "task-specific")
       matches)
;;
...
let test_find_matching_wildcard () =
  ...
  let matches =
    Subscriptions.SubscriptionStore.find_matching
      ~resource:Subscriptions.Messages
      ~resource_id:"any-message"
  in
  check
    bool
    "wildcard matches"
    true
    (List.exists (fun (s : Subscriptions.subscription) -> s.filter = Some "*") matches)
;;
...
let test_find_matching_wrong_resource () =
  ...
  let matches =
    Subscriptions.SubscriptionStore.find_matching
      ...
      ~resource_id:"vote-001"
  in
  check
    bool
    "no match for different resource"
    false
    (List.exists (fun (s : Subscriptions.subscription) -> s.id = sub.id) matches)
;;
...
let test_notify_change () =
  ...
  let count =
    Subscriptions.notify_change
      ...
      ~data:(`Assoc [ "title", `String "Test Task" ])
  in
...
let test_pop_notifications () =
  ...
  let _ =
    Subscriptions.notify_change
      ...
      ~data:(`Assoc [ "status", `String "active" ])
  in
...
let test_tool_unsubscribe () =
  ...
  in
  let args =
    `Assoc [ "action", `String "unsubscribe"; "subscription_id", `String sub.id ]
  in
...
let test_tool_list_by_subscriber () =
  ...
  in
  let args =
    `Assoc [ "action", `String "list"; "subscriber", `String "specific_list_agent" ]
  in
  let success, result = Subscriptions.handle_subscription_tool args in
...
let test_tool_poll () =
  let sub =
    Subscriptions.SubscriptionStore.subscribe
      ...
      ()
  in
  let args = `Assoc [ "action", `String "poll"; "subscription_id", `String sub.id ] in
  let success, result = Subscriptions.handle_subscription_tool args in
...
let test_notify_message () =
  ...
  Subscriptions.notify_message ~message_id:"msg-001" ~data:(`Assoc []);
  (* Verify notification was queued *)
  let pending = Subscriptions.SubscriptionStore.pop_notifications sub.id in
