...
let model_provider_label = function
  ...
  | Llm.Custom name -> "custom:" ^ name

let model_label (spec : Llm.model_spec) =
  sprintf "%s:%s" (model_provider_label spec.provider) spec.model_id

let validate_model_spec (spec : Llm.model_spec) :
    (Llm.model_spec, string) result =
  match spec.provider with
  | Llm.Claude
  | Llm.OpenAI
  | Llm.Gemini
  | Llm.Llama
  | Llm.Glm_cloud -> Ok spec
  | Llm.OpenRouter
  | Llm.Custom _ ->
      Error
        (sprintf
...
let resolve_model_spec ~(agent : string) ~(worker_model : string option) :
    (Llm.model_spec * string, string) result =
  let parse_worker_model raw =
    match Llm.model_spec_of_string raw with
    | Error _ as e -> e
    | Ok spec -> (
  ...
  | _ ->
      ...
      if String.contains normalized ':' then parse_worker_model normalized
        ...
        | "claude" -> Ok (Llm.claude_opus, model_label Llm.claude_opus)
        | "openai" | "codex-api" ->
            Ok (Llm.openai_default, model_label Llm.openai_default)
        | "gemini" -> Ok (Llm.gemini_pro, model_label Llm.gemini_pro)
        | "ollama" ->
            Error (Provider_adapter.bare_ollama_migration_message ())
        | "glm" ->
            Ok (Llm.glm_cloud, model_label Llm.glm_cloud)
        | "llama" ->
            Error
...
let run ~(sw : Eio.Switch.t) ~(config : Room.config) (state : Mdal.loop_state)
  ...
  in
  let model_spec =
    match Llm.model_spec_of_string model_label with
    | Ok spec -> spec
