...
- **Minimal setup**: MASC requires keeper_runtime.toml, keeper personas, and a running workspaceSession. The tradeoff is multi-tenant governance over single-user convenience.
...
| Fleet of agents with different policies | Manage N SDK clients externally | Single workspaceSession with per-keeper phase and runtime rules |
