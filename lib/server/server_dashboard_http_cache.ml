...
let upsert_assoc_field key value fields = (key, value) :: List.remove_assoc key fields
...
let cached_surface_json surface =
  ...
  let cache_state, stale_reason, stale_age_ms =
    ...
    | Some success_ts, Some error_ts when error_ts > success_ts ->
      "stale", surface.last_error, Some (int_of_float ((now_ts -. success_ts) *. 1000.0))
...
let cached_surface_has_success surface = Option.is_some surface.last_success_unix

let cached_surface_or_first_success_json
      surface
      ~cache_key
      ~ttl
      ~clock
      ...
      compute
  =
  if cached_surface_has_success surface
  then cached_surface_json surface
  else (
    ...
    let json =
      Dashboard_cache.get_or_compute_with_timeout
        ...
        compute_and_track
    in
    if cached_surface_has_success surface then cached_surface_json surface else json)
;;
