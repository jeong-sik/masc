(* Fusion — 심판 (구현).
   ...
   Prompt-injection 방어: 질문·패널 답변은 신뢰 불가(모델/사용자 생성)하므로
   [Fusion_judge_parse.escape_xml]로 XML 메타문자를 escape하고 <question>/
   <panel_answers> 태그로 감싼 뒤 judge에게 전달한다. 이스케이프 구현은
   lib/fusion_core/fusion_judge_parse.ml:escape_xml 에서 단위 테스트된다. *)
  ...
  let answers =
    ...
    |> List.map (fun (a : Fusion_types.panel_answer) ->
           Printf.sprintf "<panel model=\"%s\">%s</panel>"
             (Fusion_judge_parse.escape_xml a.model)
             (Fusion_judge_parse.escape_xml a.answer))
    |> String.concat "\n"
  in
  Printf.sprintf
...
%s|}
    (Fusion_judge_parse.escape_xml question)
    answers
