...
let counter_for ?(origin = Timeout_origin.Command) ~program ~timeout_sec () =
  Masc_mcp.Prometheus.metric_value_or_zero
    Masc_mcp.Workspace.process_timeout_metric
    ~labels:
      [ "program", program
...
let test_record_increments () =
  ...
  Masc_mcp.Workspace.record_process_timeout ~program ~timeout_sec ~origin;
...
let test_program_isolation () =
  ...
  Masc_mcp.Workspace.record_process_timeout ~program:prog_b ~timeout_sec ~origin;
  Alcotest.(check (float 0.0001))
    "program A unchanged"
    before_a
...
let test_timeout_sec_isolation () =
  ...
  Masc_mcp.Workspace.record_process_timeout ~program ~timeout_sec:15.0 ~origin;
  Alcotest.(check (float 0.0001))
    "15s budget +1"
    (before_15 +. 1.0)
...
let test_stage_isolation () =
  ...
  in
  Masc_mcp.Workspace.record_process_timeout
    ~program
    ~timeout_sec
