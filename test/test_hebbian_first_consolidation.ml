...
let test_immediate_run_advances_last_consolidation () =
  with_temp_masc_dir (fun config ->
    ...
    Hebbian_eio.strengthen config ~from_agent:"sangsu" ~to_agent:"issue-king" ();
    Alcotest.(check (float 1e-9))
      "boot graph has last_consolidation = 0"
      0.0
      (load_consolidation_ts config);
    ...
    Alcotest.(check bool) "last_consolidation advanced past boot 0.0" true (after_ts > 0.0);
    Alcotest.(check bool)
      "last_consolidation is recent (>= just before call)"
      true
      (after_ts >= before -. 0.001))
;;
...
let test_idempotent_under_no_decay_horizon () =
  ...
  with_temp_masc_dir (fun config ->
    ...
    Alcotest.(check int) "synapse count unchanged" 1 (List.length after_synapses);
    let s_after = List.hd after_synapses in
    Alcotest.(check int)
      "success_count preserved"
      s_before.Hebbian_eio.success_count
      s_after.Hebbian_eio.success_count;
    Alcotest.(check (float 1e-9))
      "weight preserved"
      s_before.Hebbian_eio.weight
      s_after.Hebbian_eio.weight)
;;

let () =
  Alcotest.run
    "hebbian_first_consolidation"
    [ ( "first_pass"
      , [ Alcotest.test_case
            "immediate run advances last_consolidation"
            `Quick
            test_immediate_run_advances_last_consolidation
        ; Alcotest.test_case
            "no-decay run is idempotent on rows"
            `Quick
