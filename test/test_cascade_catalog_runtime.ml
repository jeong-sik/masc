...
let json_string_field name json =
  ...
  | `String value -> value
  | other -> failf "expected %s to be a string, got %s" name (Yojson.Safe.to_string other)
;;

let json_int_field name json =
  match json_member name json with
  | `Int value -> value
  | other -> failf "expected %s to be an int, got %s" name (Yojson.Safe.to_string other)
;;

let json_list_field name json =
  match json_member name json with
  | `List values -> values
  | other -> failf "expected %s to be a list, got %s" name (Yojson.Safe.to_string other)
;;
...
let test_valid_catalog_skips_live_probes_at_bootstrap () =
  with_temp_dir "cascade-catalog-runtime"
  @@ fun dir ->
  ...
  in
  let strict_model = Printf.sprintf "custom:tool-use-strict@%s/v1" dummy_base_url in
  ignore
    (write_cascade_json
       config_dir
       (Printf.sprintf
  ...
  let snapshot =
    ...
    | Error rejection ->
      failf
        "unexpected validation rejection: %s"
  ...
  check
    bool
    "bootstrap probe status is skipped"
    true
    (contains_substring (Yojson.Safe.to_string snapshot_json) "\"status\":\"skipped\"");
  let blank_name =
    require_ok (Cascade_catalog_runtime.resolve_declared_name ~raw_name:"" ())
  in
  check
    string
    "blank name defaults to big_three"
    Keeper_config.default_cascade_name
    blank_name;
  let keeper_unified_name =
    require_ok
      (Cascade_catalog_runtime.resolve_declared_name ~raw_name:"keeper_unified" ())
  in
  check
    string
    "keeper_unified resolves to catalog profile"
    "keeper_unified"
    keeper_unified_name;
  check
    (list string)
    "keeper_unified models use exact catalog profile"
    [ keeper_unified_model ]
    (require_ok (Cascade_catalog_runtime.models_of_cascade_name "keeper_unified"));
  let strict_name =
    require_ok
      (Cascade_catalog_runtime.resolve_declared_name ~raw_name:"tool_use_strict" ())
  in
  check string "tool_use_strict resolves to catalog profile" "tool_use_strict" strict_name;
  check
    (list string)
    "tool_use_strict models use exact catalog profile"
    [ strict_model ]
    (require_ok (Cascade_catalog_runtime.models_of_cascade_name "tool_use_strict"));
  match Cascade_catalog_runtime.resolve_declared_name ~raw_name:"missing_profile" () with
  | Ok resolved -> failf "expected missing profile to be rejected, got %s" resolved
  | Error detail ->
    check
      bool
      "unknown cascade_name is surfaced"
      true
      (contains_substring detail "unknown cascade_name");
    check
      bool
      "active profile list is included"
      true
      (contains_substring detail "tool_rerank")
;;

let test_invalid_hot_reload_preserves_last_known_good () =
  with_temp_dir "cascade-catalog-lkg"
  @@ fun dir ->
  let config_dir = Filename.concat dir "config" in
  init_config_root config_dir;
  with_config_dir config_dir
  ...
  match Cascade_catalog_runtime.inspect_active () with
  | Ok (Cascade_catalog_runtime.Serving_last_known_good { snapshot; rejected_update }) ->
    let served_json = Cascade_catalog_runtime.snapshot_to_yojson snapshot in
    check
      string
      "last-known-good source path"
      config_path
      (json_string_field "source_path" served_json);
    let labels =
      require_ok (Cascade_catalog_runtime.models_of_cascade_name "big_three")
    in
    check (list string) "served snapshot keeps prior model" [ valid_model ] labels;
    let rejection_json =
      Cascade_catalog_runtime.rejection_to_yojson rejected_update |> Yojson.Safe.to_string
    in
    check
      bool
      "rejection mentions invalid candidate"
      true
  ...
  | Error rejection ->
    failf
      "expected last-known-good fallback, got hard error: %s"
      (Yojson.Safe.to_string (Cascade_catalog_runtime.rejection_to_yojson rejection))
;;

let test_legacy_runtime_wrapper_does_not_fallback_to_defaults () =
  with_temp_dir "cascade-runtime-wrapper"
  @@ fun dir ->
  let config_dir = Filename.concat dir "config" in
  init_config_root config_dir;
  with_config_dir config_dir
...
let test_legacy_runtime_wrapper_preserves_configured_label_order () =
  ...
  @@ fun () ->
  with_eio
  @@ fun ~sw ~net ~clock ~fs:_ ~proc_mgr:_ ->
  let port = find_free_port () in
  let base_url, _request_count =
    start_counting_mock ~sw ~net ~port ~response:(openai_text_response "pong")
  in
  ...
  check
    (list string)
    "legacy wrapper keeps configured order"
    expected
    (List.hd observed_orders);
  check
    int
    "legacy wrapper order is stable across calls"
    1
    (observed_orders
...
let test_partial_catalog_keeps_validated_subset_available () =
  with_temp_dir "dashboard-cascade-profiles"
  @@ fun dir ->
  let config_dir = Filename.concat dir "config" in
  init_config_root config_dir;
  with_config_dir config_dir
  ...
  let rejection_json =
    ...
    | Error rejection ->
      failf
        "expected partial validation, got hard error: %s"
        (Yojson.Safe.to_string (Cascade_catalog_runtime.rejection_to_yojson rejection))
  in
  check
    bool
    "rejected invalid candidate is surfaced"
    true
    (contains_substring rejection_json "__nonexistent_provider_sentinel__:fake");
  check
    (list string)
    "dashboard only advertises validated profiles"
  ...
  check
    (list string)
    "invalid profiles use runtime rejected subset"
    [ "broken_profile" ]
    (List.map fst invalid_profiles);
  check
    bool
    "invalid profile reason is actionable"
    true
    (match List.assoc_opt "broken_profile" invalid_profiles with
     | Some reasons ->
       List.exists
         (fun reason -> contains_substring reason "uses unregistered provider scheme")
         reasons
     | None -> false);
  let config_json =
    with_eio
    @@ fun ~sw:_ ~net:_ ~clock:_ ~fs:_ ~proc_mgr:_ ->
    Masc_mcp.Dashboard_cascade.config_json ()
  in
  let profile_names =
    json_list_field "profiles" config_json |> List.map (json_string_field "name")
  in
  check
    (list string)
    "dashboard config_json only renders validated profiles"
    [ Keeper_config.default_cascade_name; "tool_rerank" ]
    profile_names;
  check
    bool
    "dashboard config_json keeps rejected profile metadata"
    true
...
let test_partial_catalog_rejects_invalid_default_profile () =
  with_temp_dir "dashboard-cascade-default-gate"
  @@ fun dir ->
  let config_dir = Filename.concat dir "config" in
  init_config_root config_dir;
  with_config_dir config_dir
  ...
  | Error rejection ->
    ...
    check
      bool
      "default-profile gate is surfaced"
      true
      (List.exists
         (function
           | `String value ->
    ...
    check
      bool
      "rejected default profile invalid candidate is surfaced"
      true
      (contains_substring
...
let test_resolve_named_providers_tool_choice_filters_runtime_only_providers () =
  with_temp_dir "cascade-catalog-tool-choice"
  @@ fun dir ->
  let config_dir = Filename.concat dir "config" in
  init_config_root config_dir;
  with_config_dir config_dir
  ...
  check
    bool
    "every surviving provider supports inline tool choice"
    true
...
let test_resolve_named_providers_tool_support_keeps_runtime_mcp_providers () =
  with_temp_dir "cascade-catalog-tool-support"
  @@ fun dir ->
  let config_dir = Filename.concat dir "config" in
  init_config_root config_dir;
  with_config_dir config_dir
  ...
  check
    bool
    "keeps codex_cli via runtime MCP lane"
    true
    (List.mem "codex_cli" (provider_kinds providers));
  check
    bool
...
let test_resolve_named_providers_runtime_mcp_headers_drop_unsupported_providers () =
  with_temp_dir "cascade-catalog-runtime-mcp-headers"
  @@ fun dir ->
  let config_dir = Filename.concat dir "config" in
  init_config_root config_dir;
  with_config_dir config_dir
...
let test_resolve_named_providers_canonical_labels_are_not_leaks () =
  with_temp_dir "cascade-catalog-canonical-labels"
  @@ fun dir ->
  let config_dir = Filename.concat dir "config" in
  init_config_root config_dir;
  with_config_dir config_dir
  ...
  let stderr_output =
    capture_stderr (fun () ->
      ...
      check bool "keeps canonical custom provider" true (List.mem "openai_compat" kinds);
  ...
  check
    bool
    "canonicalized provider labels are not false leaks"
    false
...
let test_resolve_named_providers_three_cli_auto_not_leaks_10087 () =
  with_temp_dir "cascade-catalog-three-cli-auto-10087"
  @@ fun dir ->
  let config_dir = Filename.concat dir "config" in
  init_config_root config_dir;
  with_config_dir config_dir
  ...
  let big_three_stderr =
    capture_stderr (fun () ->
      let providers =
        require_ok
          (Cascade_catalog_runtime.resolve_named_providers ~cascade_name:"big_three" ())
      in
      let kinds = provider_kinds providers in
      check bool "big_three: claude_code present" true (List.mem "claude_code" kinds);
      check bool "big_three: codex_cli present" true (List.mem "codex_cli" kinds);
      check bool "big_three: gemini_cli present" true (List.mem "gemini_cli" kinds))
  in
  check
    bool
  ...
  let gov_stderr =
    capture_stderr (fun () ->
      ...
      | Ok providers ->
        ...
        check
          bool
          "governance_judge: codex_cli expanded"
          true
          (List.mem "codex_cli" kinds);
        check
          bool
          "governance_judge: gemini_cli expanded"
          true
...
let test_config_doctor_live_reports_catalog_validation () =
  with_temp_dir "cascade-doctor-live"
  @@ fun dir ->
  ...
  let report =
    Config_doctor.analyze_live
      ...
      ()
  in
  check string "live doctor status" "error" (Config_doctor.status_to_string report.status);
  match report.catalog_validation with
  | None -> fail "expected catalog_validation output from analyze_live"
  | Some json ->
    check string "catalog validation status" "invalid" (json_string_field "status" json);
    check
      bool
      "live warning present"
      true
      (List.exists
         (fun warning ->
...
let test_config_doctor_live_warns_on_partial_catalog_validation () =
  with_temp_dir "cascade-doctor-live-partial"
  @@ fun dir ->
  let base_path = Filename.concat dir "base" in
  let config_dir = Filename.concat base_path ".masc/config" in
  init_config_root config_dir;
  ...
  check
    string
    "live doctor partial status"
    "warn"
    (Config_doctor.status_to_string report.status);
  check
    bool
    "live partial warning present"
    true
    (List.exists
       (fun warning ->
          contains_substring
  ...
  | Some json ->
    check
      string
      "partial catalog validation status"
      "validated"
      (json_string_field "status" json);
    check
      bool
      "rejected profile is surfaced in live doctor json"
      true
      (contains_substring
         (Yojson.Safe.to_string json)
         "__nonexistent_provider_sentinel__:fake")
