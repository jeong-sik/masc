...
let encode_broadcast (output : broadcast_output) : Yojson.Safe.t =
  `Assoc
    ([ "delivered", `Bool output.delivered; "room_message", `String output.room_message ]
     @
...
let handle_broadcast (message : string) : (broadcast_output, string) result =
  let trimmed = String.trim message in
  if trimmed = ""
  then Error "Broadcast message cannot be empty"
  else (
...
let parse_broadcast (json : Yojson.Safe.t) =
  ...
  | Ok v -> Ok v
  | Error errs ->
    Error (Oas.Tool_input_validation.format_errors ~tool_name:"masc_broadcast_typed" errs)
;;

let tool =
  Typed_tool_masc.create
