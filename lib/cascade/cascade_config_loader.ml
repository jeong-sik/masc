...
let parse_weighted_item = function
  | `String s -> Some { model = String.trim s; weight = 1; supports_tool_choice = None }
  | `Assoc fields ->
...
let empty_catalog_builder =
  { has_schema_field = false; keeper_assignable = None; fallback_cascade = None }
;;
...
let resolve_inference_params ~config_path ~name =
  ...
  | Error msg ->
    Eio.traceln
      ...
      name
      config_path;
    { temperature = None; max_tokens = None; keep_alive = None; num_ctx = None }
  | Ok json ->
    let temp =
