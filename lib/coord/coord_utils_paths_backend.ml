...
let key_of_path config path =
  key_of_path_from_root config ~root:(masc_root_dir config) path
;;

let root_key_of_path config path =
  key_of_path_from_root config ~root:(masc_root_dir config) path
;;
...
let list_dir config path =
  ...
  | None ->
    ...
    then Sys.readdir path |> Array.to_list
    else []
  | Some _
    when backend_supports_local_dir config.backend
         && Sys.file_exists path
         && Sys.is_directory path -> Sys.readdir path |> Array.to_list
  | Some key_prefix ->
...
let root_is_initialized config =
  ...
  | FileSystem _ ->
    ...
    && (Sys.file_exists (root_state_path config)
        || Sys.file_exists (legacy_root_state_path config))
;;
