...
(* ── Pixel dimensions ─────────────────────────────────────────────────
    ...
    The attachment notes a chat row carries name a file without saying what
    it looks like, and a screenshot read as "12.3 KB" answers the wrong
    question -- the reader asks "how big is it" in pixels. The bytes already
    travel with the attachment, so the size is read out of the same header
    the media type came from. [None] is a normal answer, not a failure: an
    unparseable or unhandled layout shows nothing rather than a guess. *)
...
(* PNG: the signature is the one [sniff_image_media_type] matched, and the
   spec puts IHDR first, so width and height are at fixed offsets. A file
   whose first chunk is not IHDR is not a PNG this function wants to speak
   about. *)
...
let rec jpeg_walk bytes offset =
  ...
  if offset + 4 > length || Char.code bytes.[offset] <> 0xff then None
    ...
    if code = 0x01 || (code >= 0xd0 && code <= 0xd7) then jpeg_walk bytes (offset + 2)
      ...
      else Some (be16 bytes (offset + 5), be16 bytes (offset + 7))
