...
let detect_stuck_agents ~(now : float) (snapshots : Dashboard_labels.room_snapshot list)
  : attention_item list
  =
  let all_agents =
  ...
  |> List.filter_map (fun (agent : Types.agent) ->
    ...
    | Dashboard_labels.Stuck ->
      ...
      Some
        ...
        ; summary = Printf.sprintf "Agent stuck: %s (%s%s)" agent.name elapsed task_info
...
let detect_idle_with_pending
      ~(now : float)
      (snapshots : Dashboard_labels.room_snapshot list)
  : attention_item list
  =
  let all_agents =
    List.concat_map (fun (s : Dashboard_labels.room_snapshot) -> s.agents) snapshots
  ...
  let pending_count =
    List.length
      (List.filter (fun (t : Types.task) -> t.task_status = Types.Todo) all_tasks)
  in
  if idle_count > 0 && pending_count > 0
  then
    [ { severity = Info
      ; category = "idle_with_pending"
      ; summary =
          Printf.sprintf
            "%d idle agent%s with %d pending task%s"
            idle_count
...
let collect ~(now : float) (snapshots : Dashboard_labels.room_snapshot list)
  ...
  =
  let items =
    detect_stuck_agents ~now snapshots @ detect_idle_with_pending ~now snapshots
  in
  List.sort
...
let compact_summary (items : attention_item list) : string =
  let critical = List.length (List.filter (fun i -> i.severity = Critical) items) in
  let warning = List.length (List.filter (fun i -> i.severity = Warning) items) in
  ...
  then "No action needed"
    ...
    let parts = [] in
    let parts =
      if critical > 0 then parts @ [ Printf.sprintf "%d critical" critical ] else parts
    in
    let parts =
      if warning > 0 then parts @ [ Printf.sprintf "%d warning" warning ] else parts
    in
    Printf.sprintf
      "%d item%s (%s)"
      total
