...
let oas_error_payload_small =
  "[masc_oas_error] \
   {\"kind\":\"oas_timeout_budget\",\"budget_sec\":30.0,\"keeper_turn_timeout_sec\":120.0,\"estimated_input_tokens\":45000,\"source\":\"tool_list_build\"}"
;;
...
let test_huge_structured_payload_safety_capped () =
  ...
  Alcotest.(check bool) "ends with ellipsis" true (ends_with_ellipsis result)
;;
...
let test_wrapped_internal_error_preserved () =
  ...
  Alcotest.(check string)
    ...
    wrapped_oas_error_payload_small
    result;
  Alcotest.(check bool)
...
let test_plain_narrative_long_truncated () =
  ...
  Alcotest.(check bool)
    ...
    true
    (String.length result < String.length narrative_long);
  Alcotest.(check bool)
...
let test_idempotence () =
  let check label s =
    ...
    Alcotest.(check string) (Printf.sprintf "idempotent: %s" label) once twice
  in
...
let () =
  Alcotest.run
    ...
    ; ( "plain narrative"
      , [ Alcotest.test_case "short preserved" `Quick test_plain_narrative_short_preserved
        ; Alcotest.test_case
            "long truncated at narrative cap"
            `Quick
            test_plain_narrative_long_truncated
        ; Alcotest.test_case
            "wrapped 'Internal error:' preserved"
            `Quick
            test_wrapped_internal_error_preserved
        ] )
    ; "idempotence", [ Alcotest.test_case "four shapes" `Quick test_idempotence ]
    ; ( "cap_social_state integration"
      , [ Alcotest.test_case
            "blocker via cap_blocker"
            `Quick
            test_cap_social_state_routes_blocker_through_cap_blocker
        ; Alcotest.test_case
            "narrative fields unchanged policy"
            `Quick
