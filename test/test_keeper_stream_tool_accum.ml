...
let stop ~index = Masc_agent_core.Types.ContentBlockStop { index }
...
let media_delta ~index data =
  Masc_agent_core.Types.ContentBlockDelta
    { index
    ; delta =
        Masc_agent_core.Types.MediaDelta
          { media_type = "image/png"; source_type = Masc_agent_core.Types.Base64; data }
    }
...
let test_message_stop_finalizes_open_blocks () =
  ...
  A.on_event t (json_delta ~index:0 "{\"path\":\"a.ml\"}");
  A.on_event t Masc_agent_core.Types.MessageStop;
...
let test_invalid_tool_starts_are_ignored () =
  ...
  A.on_event t (Masc_agent_core.Types.ContentBlockStart
