...
let normalize_path ?base_dir path =
  let abs =
    ...
    then Filename.concat (Option.value ~default:(Sys.getcwd ()) base_dir) path
    else path
  in
  ...
  let alt =
    ...
    | "gh" ->
      " 'gh' is NOT available in the keeper sandbox. For pull-request work use \
       keeper_pr_list / keeper_pr_view / keeper_pr_comment / keeper_pr_review_read. For \
       issues use masc_board_list / masc_board_post / masc_board_comment. For commits or \
       branches just use 'git' directly — it is on the allowlist."
    | "docker"
    | "podman"
    | "kubectl"
    | "systemctl"
    ...
    | "ssh" | "scp" | "rsync" | "ftp" | "sftp" | "nc" ->
      Printf.sprintf
        " '%s' is a network primitive and is not permitted. Keeper network access goes \
         through masc_web_search or masc_autoresearch_* tools."
        name
...
let redact_url_credentials token =
  ...
  in
  token
  |> fun t -> redact_after_scheme t "https://" |> fun t -> redact_after_scheme t "http://"
;;
...
type tool_exec_observer = tool_name:string -> success:bool -> duration_ms:int -> unit
    ...
    (fun input ->
       ...
       | Ok command ->
         ...
         (match validation with
          | Error reason -> tool_error (block_reason_to_string reason)
          | Ok () ->
            let timeout =
            ...
            (try
               ...
               let result =
                 try
                   ...
                   in
                   match status with
                   | `Exited 0 -> Ok { Oas.Types.content = output }
                   | `Exited code ->
               ...
               Option.iter
                 (fun f ->
                    f ~tool_name:"shell_exec" ~success:(Result.is_ok result) ~duration_ms)
                 on_exec;
               result
             with
             ...
             | exn ->
               ...
               Option.iter
                 ...
                 on_exec;
               tool_error (Printf.sprintf "Command failed: %s" (Printexc.to_string exn)))))
;;
