...
**2026-09-07 정정.** 이 −55% 는 줄이 **맨 이름**만 싣는 경우의 값인데, §4.2 가
`identity` 를 유지하기로 했으므로 이름만 적힌 줄로는 호출을 만들 수 없다 —
`identity` 는 `source_id`·`package_id`·`name` 셋을 요구한다. 두 절이 어긋나 있었다.

정합한 지점은 **줄이 identity 를 싣고 revision 을 뺀다**이다. §4.2 가 요구하는 것과
같은 모양이라 그대로 복사해 넣을 수 있다. 실측(14개, 09-07): **4,991 → 3,605 B, −28%**.

나머지 27%는 `identity` 까지 optional 로 가야 나온다. 그건 §4.3 의 충돌 규칙이 실제로
물리는 지점이라 이 증분에 넣지 않는다 — 지금은 두 소스가 같은 이름을 가지면 `identity`
가 갈라주지만, 이름만 받으면 그 구분이 사라진다.
...
**다만 "§4 를 하면 작아지니 되돌리자" 는 성립하지 않는다.** §4.1·§4.2 적용 후 실측:

| | 도구 표면 |
|---|---:|
| 지연을 결정할 때 (스킬 10개, 09-03) | 3,695 B |
| 지금, §4 적용 전 (14개) | 5,593 B |
| 지금, §4 적용 후 (14개) | **4,207 B** |

카탈로그가 10 → 14 로 자라서, 28%를 덜어내고도 지연을 결정했던 값보다 크다. §4 는
증가를 되돌린 것이 아니라 **막은 것**이다.

그러므로 지연을 되돌리는 근거는 크기가 아니라 §8.1 의 것이어야 한다 — 스키마가 곧
카탈로그이므로 지연이 발견을 지연시킨다. 그 판단에는 매 요청 4,207 B 라는 값이 붙는다.
참고로 Anthropic 공식 모델은 스킬당 ~100토큰(≈400 B)을 상정하므로 14개면 ≈5,600 B 이고,
그 비용을 받아들이는 쪽을 택하고 있다.
