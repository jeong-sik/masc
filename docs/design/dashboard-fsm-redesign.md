...
| 페이지 | FSM 위젯 | 출처 |
|--------|----------|------|
| `dashboard/src/components/keeper-detail.ts:509` | parent phase diagram + Decision FSM + Runtime FSM | `keeper-state-diagram.ts` |
...
`lib/keeper/keeper_decision_audit.ml:256-315` `runtime_fsm_to_mermaid`는 state name, edge label, fallback 조건을 문자열 리터럴로 고정 출력한다. 결과적으로:
...
| `/keepers/:name` (기존) | parent phase diagram + Decision FSM + Runtime FSM + Memory tier panel + Compaction sub-FSM | 운영자 콘솔 심화 | 확장 |
...
| 파일 | 라인 | 변경 |
|------|------|------|
| `lib/keeper/keeper_decision_audit.ml` | 256-315 | `runtime_fsm_to_mermaid` state set을 `RuntimeLiveness.tla`의 `{idle, selecting, trying, done, exhausted}` subset으로 제한. 렌더 시점에 받은 실제 profile name, provider health list, last_provider_result로 label/색 bind |
| `lib/keeper/keeper_decision_audit.mli` | 55-73 | 시그니처 확장 (optional labelled args): `?provider_health:(string * [`Healthy|`Unhealthy|`SlotFull|`Unknown]) list`, `?slot_state:int * int`, `?effective_runtime_reason:string` |
| `lib/server/server_routes_http_routes_dashboard.ml` | 618-633 | 기존 `Keeper_turn_driver.default_model_strings` 호출 유지, `Keeper_runtime_routing.select_runtime` 결과와 `Keeper_status_metrics.last_model_used`를 함께 넘긴다 |
| `lib/dashboard/dashboard_http_keeper.ml` | 147-148 | 이미 phase-aware runtime 리졸브 로직 보유 — `effective_runtime_reason` 노출 경로만 추가 |

### 재사용할 기존 함수 (새 추상화 금지)

- `Keeper_runtime_routing.select_runtime` — phase→runtime 매핑 (`lib/keeper/keeper_runtime_routing.mli`)
- `Runtime_runtime.models_of_runtime_id` — runtime 이름 → provider list
...
`runtime_fsm_to_mermaid`가 생성하는 state id는 반드시 `RuntimeLiveness.tla`의 state 집합 subset. CI에 파일 파싱 단계를 추가해 임의 문자열 유입을 차단한다. Property-based test는 Phase 2의 TLC 통과 여부와 함께 실행한다.
...
|-------|--------|------|------------|
| **1a** | Runtime 하드코딩 제거 + 시그니처 확장 + `runtime_fsm_to_mermaid` state set 제한 | — | 1 PR |
...
- OCaml alcotest: 각 신규/수정된 mermaid builder에 대해 golden output per phase. phase 개수 × runtime profile 개수 조합.
...
- 시나리오 A — Failing 전이: `masc_keeper_reset`으로 Failing 강제 → runtime 그래프에 `local_recovery` 노출 확인, provider health bar 변경 확인.
- 시나리오 B — Compaction: context ratio로 compaction 트리거 → sub-FSM `compacting` 렌더 확인 → `done` 전이 후 parent phase `Running` 복귀 확인.
- 시나리오 C — Invariants violation: live composite contract drift. 예: `Compacting`인데 turn phase가 `compacting`이 아니거나, measurement 없이 runtime가 `trying`으로 진입한 경우를 fixture/API stub로 재현한다.
