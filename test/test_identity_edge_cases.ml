...
let test_empty_params () =
  ...
  let identity = Client_registry_eio.get_or_create_identity (`Assoc []) in
...
let test_unknown_channel () =
  ...
  | Some (Client_identity.External s) -> check string "opaque external" "unknown_platform_xyz" s
...
let test_null_payload () =
  ...
  Fs_compat.set_fs (Eio.Stdenv.fs env);
  Client_registry_eio.reset_for_testing ();
  let identity = Client_registry_eio.get_or_create_identity `Null in
  check bool "has agent_name" true (String.length identity.agent_name > 0);
  check bool "starts with agent-" true
    (String.length identity.agent_name >= 6
     && String.sub identity.agent_name 0 6 = "agent-")

(* tools/call argument admission moved behind Mcp_server_eio_call_request.decode;
   its acceptance/rejection matrix lives in test_mcp_server_eio_call_tool.ml. *)
...
let test_scalar_payload () =
  ...
  Fs_compat.set_fs (Eio.Stdenv.fs env);
  Client_registry_eio.reset_for_testing ();
  let identity_string = Client_registry_eio.get_or_create_identity (`String "garbage") in
  let identity_int = Client_registry_eio.get_or_create_identity (`Int 42) in
  let identity_list = Client_registry_eio.get_or_create_identity (`List []) in
