...
- `MASC_KEEPER_MEMORY_LLM_SUMMARY`: opt-in semantic summarizer hook. 기본값은 off이며, keeper turn runtime에서 현재 runtime의 direct completion provider를 사용한다. CLI subprocess provider만 있는 runtime, provider resolve 실패, timeout, 빈 응답은 deterministic summary로 fallback한다.

`runtime.toml`의 `[memory]` 섹션은 같은 값을 startup boot override로 주입한다.
...
- `model`: OAS `run_named(runtime_id="context_router")` 호출
