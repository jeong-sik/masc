...
명시한다.
문단이나 Markdown 본문 흐름 안의 inline link는 이 role의 적용 대상이 아니다.
inline link는 본문 행 높이를 강제로 늘리지 않고 underline과 주변 텍스트 간격으로
식별성을 유지한다. 작성자 chip, 공유 icon, drill-down link처럼 독립된 action은
inline exception이 아니며 이 role을 적용한다.
