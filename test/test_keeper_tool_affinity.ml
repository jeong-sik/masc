...
let test_filters_low_success () =
  let stats =
    [ stat ~successes:1 ~failures:9 "bad_tool"
    ; (* 10% success *)
  ...
  Alcotest.(check string) "good_tool selected" "good_tool" (List.hd result).tool_name
;;

let test_respects_max_k () =
  let stats =
    List.init 10 (fun i -> stat ~successes:(10 - i) (Printf.sprintf "tool_%d" i))
  in
...
let test_score_ordering () =
  let stats =
    [ stat ~successes:2 ~failures:0 "low_count"
    ; (* 2 calls, 100% *)
      stat ~successes:10 ~failures:0 "high_count"
    ; (* 10 calls, 100% *)
...
let test_recency_decay () =
  ...
  Alcotest.(check string)
    "recent_tool ranked first"
    "recent_tool"
    (List.hd result).tool_name;
  Alcotest.(check bool)
    "recent has higher score"
    true
    ((List.hd result).score > (List.nth result 1).score)
;;
...
let test_populates_discovered () =
  ...
  let stats =
    [ stat ~successes:5 "keeper_board_post"; stat ~successes:3 "keeper_board_comment" ]
  in
  ...
  Alcotest.(check bool) "board_post present" true (List.mem "keeper_board_post" active);
  Alcotest.(check bool)
    "board_comment present"
    true
...
let () =
  Alcotest.run
    ...
    ; ( "environment_configuration"
      , [ Alcotest.test_case
            "configured_max_k default"
            `Quick
            test_configured_max_k_default
        ; Alcotest.test_case
            "configured_max_k clamps lower"
            `Quick
            test_configured_max_k_clamps_lower
        ; Alcotest.test_case
            "configured_max_k clamps upper"
            `Quick
            test_configured_max_k_clamps_upper
        ; Alcotest.test_case
            "configured_max_k boundary values"
            `Quick
            test_configured_max_k_boundary_values
        ; Alcotest.test_case
            "configured_max_k invalid input"
            `Quick
            test_configured_max_k_invalid_input
        ; Alcotest.test_case
            "configured_lookback_days default"
            `Quick
            test_configured_lookback_days_default
        ; Alcotest.test_case
            "configured_lookback_days clamps lower"
            `Quick
            test_configured_lookback_days_clamps_lower
        ; Alcotest.test_case
            "configured_lookback_days clamps upper"
            `Quick
            test_configured_lookback_days_clamps_upper
        ; Alcotest.test_case
            "configured_lookback_days boundary values"
            `Quick
            test_configured_lookback_days_boundary_values
        ; Alcotest.test_case
            "configured_lookback_days invalid input"
            `Quick
            test_configured_lookback_days_invalid_input
        ] )
    ]
;;
