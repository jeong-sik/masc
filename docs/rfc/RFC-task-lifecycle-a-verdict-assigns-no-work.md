...
소유자가 개정하기로 했다(2026-09-19). 고치는 자리는 셋이다.

| 자리 | 지금 | 개정안 |
|---|---|---|
| §0 한 줄 요약 | "Cancel 요청은 일이 존재를 멈추는 허락이므로 그 판정 권한은 운영자의 클릭 하나에만 있다" | "남의 일을 없애는 허락은 운영자의 클릭 하나에만 있다. 자기가 맡은 일을 접는 것은 허락을 구하는 일이 아니라 그만두는 일이고, 맡은 쪽이 직접 한다" |
| §7 거절한 대안, 첫 줄 | "시스템 LLM이 취소도 심사하고 운영자에게 권고만: 권한의 주인이 뒤바뀐다" | 그대로 둔다. 이 RFC 도 판정 에이전트에게 취소 권한을 주지 않는다 |
| §7 거절한 대안, 둘째 줄 | "타이머로 자동 취소: 침묵이 판정이 된다. 기다림의 부채는 부채로 남고, 진짜 원인(운영자 주의)은 지불되지 않은 채 소멸시킨다" | 결론은 그대로. 다만 **그 부채가 실제로 지불되지 않았다**는 기록을 붙인다. 2026-09-19 에 취소 요청 65건이 쌓여 있었고 가장 오래된 것이 345시간이었다. 이 RFC 는 기다림을 없애는 쪽이 아니라 **기다릴 일 자체를 없애는** 쪽으로 간다 |

개정의 논지는 하나다. RFC-0417 은 "취소" 를 한 덩어리로 보고 전부 운영자에게 맡겼다. 그 안에 서로 다른
두 가지가 섞여 있었다. **남의 일을 없애는 것**과 **자기 일을 그만두는 것**이다. 앞의 것은 그대로 운영자
몫이고, 이 RFC 도 `Todo` 를 운영자만 취소하게 해서 오히려 지금보다 좁힌다. 뒤의 것은 애초에 허락을 구할
일이 아니었고, 그걸 줄로 세운 것이 345시간의 원인이다.

세탁 경로는 그대로 닫혀 있다. 판정 LLM 에이전트는 취소 권한을 받지 않고, 취소는 판정을 거치지 않는다.
