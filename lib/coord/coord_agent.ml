...
let get_agents_status config =
  ...
  if not (Sys.file_exists agents_path)
  then `Assoc [ "agents", `List []; "count", `Int 0 ]
  else (
    let agents = ref [] in
    ...
    |> Array.iter (fun name ->
      ...
      then (
        ...
        | Ok agent ->
          ...
          let status =
            if is_zombie then "zombie" else agent_status_to_string agent.status
          in
          agents
          := `Assoc
             ...
             :: !agents
        | Error msg -> Log.Misc.error "agent state read failed: %s" msg));
    `Assoc [ "agents", `List (List.rev !agents); "count", `Int (List.length !agents) ])
;;
...
let register_capabilities config ~agent_name ~capabilities =
  ...
  let agent_file =
    Filename.concat (agents_dir config) (safe_filename actual_name ^ ".json")
  in
  ...
  then
    with_file_lock config agent_file (fun () ->
      ...
      | Error _ -> Printf.sprintf "⚠ Invalid agent file for %s" actual_name)
  else Printf.sprintf "⚠ Agent %s not found. Join first!" agent_name
;;
...
let update_agent_r config ~agent_name ?status ?capabilities () : string Types.masc_result =
  ...
  then Error Types.NotInitialized
    ...
    match validate_agent_name_r agent_name with
    | Error e -> Error e
    | Ok _ ->
      let actual_name = resolve_agent_name config agent_name in
      ...
      if not (Sys.file_exists agent_file)
      then Error (Types.AgentNotFound actual_name)
      else (
        let locked =
                    ...
                    log_event
                      ...
                      (Printf.sprintf
                         ...
                         (Yojson.Safe.to_string
                            (`List (List.map (fun s -> `String s) updated_caps)))
                         (now_iso ()));
      ...
      then (
        ...
        | Ok agent
          when List.mem capability agent.capabilities
               && not (is_zombie_agent ~agent_name:agent.name agent.last_seen) ->
          matching
          := `Assoc
               [ "name", `String agent.name
