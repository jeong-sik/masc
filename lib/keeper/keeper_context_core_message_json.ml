...
let role_to_string role = Masc_agent_core.Types.role_to_string role
...
let role_of_string_opt role = Masc_agent_core.Types.role_of_string role

let content_blocks_to_json
    (blocks : Masc_agent_core.Types.content_block list) : Yojson.Safe.t =
  `List (List.map Masc_agent_core.Llm_provider.Api_common.content_block_to_json blocks)

let content_blocks_of_json
    (json : Yojson.Safe.t) : Masc_agent_core.Types.content_block list option =
  match Json_util.assoc_member_opt "content_blocks" json with
  | Some (`List blocks) ->
      let parsed = List.filter_map Masc_agent_core.Llm_provider.Api_common.content_block_of_json blocks in
...
let message_to_json (m : Masc_agent_core.Types.message) : Yojson.Safe.t =
  ...
  let tool_call_id =
    ...
    | None -> (
        ...
        | Masc_agent_core.Types.Assistant -> None)
  in
  let base =
    [
...
let message_of_json (json : Yojson.Safe.t) : Masc_agent_core.Types.message =
  let raw_role =
...
let text_of_history_jsonl_json (json : Yojson.Safe.t) : string =
  let text_of_blocks blocks =
    ...
    then ""
      ...
      Inference_utils.sanitize_text_utf8 (Masc_agent_core.Types.text_of_content blocks)
  in
  match content_blocks_of_json json with
