...
let has_pattern_with_word_boundary ~text ~pat =
  ...
  then false
    ...
    let rec loop i =
      if i > tlen - plen
      then false
      else if String.sub text i plen = pat
      then (
...
let should_skip ~action_description =
  ...
  List.exists (fun pat -> has_pattern_with_word_boundary ~text ~pat) read_only_patterns
;;
...
let extract_reason trimmed keyword_len default_reason =
  let reason =
    if String.length trimmed > keyword_len + 1
    then
      String.trim
        (String.sub trimmed (keyword_len + 1) (String.length trimmed - keyword_len - 1))
    else default_reason
  in
  if String.length reason > 0 && (reason.[0] = ':' || reason.[0] = '-')
  then String.trim (String.sub reason 1 (String.length reason - 1))
  else reason
;;
...
let parse_verdict (text : string) : (verdict, string) result =
  ...
  if len >= 4 && String.sub upper 0 4 = "PASS" && has_keyword_boundary upper 4
  then Ok Pass
  else if len >= 4 && String.sub upper 0 4 = "WARN" && has_keyword_boundary upper 4
  then Ok (Warn (extract_reason trimmed 4 "unspecified concern"))
  else if len >= 4 && String.sub upper 0 4 = "FAIL" && has_keyword_boundary upper 4
  then Ok (Fail (extract_reason trimmed 4 "action did not achieve goal"))
  else if len = 0
  then Error "empty verifier output"
  else
    Error
      (sprintf
         "unrecognized verdict format: %s"
         (let _ = len in
...
let report_verdict_schema : Types.tool_schema =
  { name = "report_verdict"
  ; description =
      "Report your verification verdict. You MUST call this tool with your assessment. \
        ...
        ; ( "properties"
          , `Assoc
             ...
             risks dropping a constructor on extension. *)
                    ...
                    ; ( "description"
                      , `String
                          "PASS if correct, WARN if acceptable with concerns, FAIL if \
                           wrong or harmful" )
                    ] )
              ; ( "reason"
                , `Assoc
                    ...
                    ; ( "description"
                      , `String
                          "Brief explanation for the verdict (required for WARN and FAIL)"
                      )
                    ] )
              ] )
...
let parse_verdict_from_json (args : Yojson.Safe.t) : (verdict, string) result =
  ...
  try
    ...
    | "FAIL" ->
      ...
      Ok (Fail r)
    | other -> Error (sprintf "unexpected verdict value: %s" other)
  with
  | Type_error (msg, _) -> Error (sprintf "verdict JSON type error: %s" msg)
  | exn -> Error (sprintf "verdict JSON parse error: %s" (Printexc.to_string exn))
;;
