...
let findings_path ~base_path =
  Filename.concat (Coord_utils.masc_dir_from_base_path ~base_path) "shared_findings.jsonl"
;;
        ...
        (fun line ->
           ...
           then None
             ...
             try
               ...
               in
               let finding =
                 json |> member "finding" |> to_string_option |> Option.value ~default:""
               in
               if finding <> ""
               then Some (Printf.sprintf "[%s] %s" worker finding)
               else None
             with
...
let truncate_list n lst = List.filteri (fun i _ -> i < n) lst
...
let to_prompt_section ctx =
  if not (has_visible_content ctx)
  then ""
  else (
    let buf = Buffer.create 512 in
    Buffer.add_string buf "--- Team Context ---\n";
    if String.trim ctx.team_goal <> ""
    then Buffer.add_string buf (Printf.sprintf "Goal: %s\n" ctx.team_goal);
    (match truncate_list max_decisions ctx.prior_decisions with
     | [] -> ()
     | decisions ->
       Buffer.add_string buf "\nPrior decisions:\n";
       List.iter (fun d -> Buffer.add_string buf (Printf.sprintf "- %s\n" d)) decisions);
     ...
     | tasks ->
       ...
       List.iter
         (fun t ->
            ...
            Buffer.add_string
              buf
              (Printf.sprintf "- [%s] %s: %s%s\n" t.status t.task_id t.title assignee_str))
         tasks);
