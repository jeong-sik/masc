...
**Date**: 2026-05-12
**Spec**: `specs/keeper-state-machine/KeeperRuntimeAttemptFSM.tla:79-84, 153-174, 240-241` (terminal phase set + ResolveExhaustedNormal/ResolveHardQuota + HardQuotaTerminalImmediate invariant)
**OCaml**: `lib/runtime/runtime_fsm.ml:20, 44-52` (single `Exhausted` constructor + decide branches), `lib/keeper/keeper_turn_driver.ml:644-672` (hard-quota fast-path)
...
\* Normal exhaustion: last tier + runtimeable error
...
```ocaml
(* lib/runtime/runtime_fsm.ml:20 — single typed surface *)
| Exhausted of { last_err : Llm_provider.Http_client.http_error option }
    [@tla.symbol "exhausted"]

(* lib/runtime/runtime_fsm.ml:44, 52 — decide returns Exhausted from
...
| Call_err err when not should_runtime ->
...
match sdk_error_to_runtime_outcome sdk_err with
| Some outcome ->
  let decision =
    if sdk_error_is_hard_quota sdk_err then
      let last_err = ... in
      Runtime_fsm.Exhausted { last_err }    (* fast-path: skip decide *)
    else
      Runtime_fsm.decide ~accept_on_exhaustion:false ~is_last outcome
  in
  ...
```
...
None of these are visible to the caller's `| Runtime_fsm.Exhausted _ ->`
...
|---|---|---|
| Spec abstract alphabet | `call_err_runtimeable` / `_terminal` / `_hard_quota` | `exhausted_normal` / `exhausted_hard_quota` |
| OCaml type | single `Call_err` | single `Exhausted` |
| Classifier location | `Runtime_health_filter.should_runtime_to_next` (input side) + `sdk_error_is_hard_quota` | `sdk_error_is_hard_quota` (already noted in D-1) + tier_index check |
...
| Production preservation | Runtime_health_filter corpus tests + Fun.protect finalizer | + cooldown side effect persists to next turn |
...
- `Runtime_metrics.on_exhausted` (runtime_fsm.ml:133) fires for both flavors but with the same `runtime_id` — the metric loses the distinction without the side effect.
- The bug model in `KeeperRuntimeAttemptFSM-buggy.cfg` already covers
...
| ID | Direction | Risk |
|---|---|---|
| **R-D-2.a** | OCaml — split `Runtime_fsm.Exhausted` into typed sub-variants `Exhausted_normal of {...}` and `Exhausted_hard_quota of {...}`.  Removes the runtime-classifier-recovery requirement at every caller.  Pairs cleanly with R-D-1.a (Call_err split) since both share `sdk_error_is_hard_quota`. | MID — type sig change in runtime_fsm.ml/.mli + keeper_turn_driver.ml caller arm + dashboard metric updates + tests.  ~10-15 call sites. |
...
1. Update `runtime_fsm.ml/.mli` `provider_outcome` to 6 typed
   constructors (call_ok, call_err_runtimeable, call_err_terminal,
   call_err_hard_quota, accept_rejected, slot_full) — matches spec's
   ProviderOutcomes alphabet.
2. Update `runtime_fsm.ml/.mli` `decision` to 5 typed constructors
   (accept, accept_on_exhaustion, try_next, exhausted_normal,
   exhausted_hard_quota) — matches spec's PhaseSet's terminal members.
3. Move `sdk_error_is_hard_quota` *into* `runtime_fsm.decide` as a
...
This is a sizeable refactor — ~150-300 LOC across runtime + keeper +
...
- `Runtime_metrics.on_exhausted` distinguishing the two flavors —
...
- `lib/runtime/runtime_fsm.ml:20, 44-52, 116-133` (decide + caller pattern).
