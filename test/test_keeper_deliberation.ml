...
let base_obs = D.empty_world_observation ~keeper_name:"test-keeper"
...
let test_triage_skip_on_empty_observation () =
  ...
  match result with
  | D.Skip _ -> ()
  | D.Triggered _ -> fail "expected Skip for empty observation, got Triggered"
;;
...
let test_triage_direct_mention () =
  ...
  | D.Triggered triggers ->
    check bool "contains DirectMention" true (List.mem D.DirectMention triggers)
;;
...
let test_triage_unclaimed_task () =
  ...
  | D.Triggered triggers ->
    check bool "contains NewUnclaimedTask" true (List.mem D.NewUnclaimedTask triggers)
;;
...
let test_triage_failed_task () =
  ...
  | D.Triggered triggers ->
    check bool "contains FailedTask" true (List.mem D.FailedTask triggers)
;;
...
let test_triage_agent_change () =
  ...
  | D.Triggered triggers ->
    check bool "contains AgentJoinedOrLeft" true (List.mem D.AgentJoinedOrLeft triggers)
;;
...
let test_triage_idle_with_goals () =
  ...
  | D.Triggered triggers ->
    check bool "contains IdleTimeout" true (List.mem D.IdleTimeout triggers)
;;
...
let test_triage_idle_without_goals_skips () =
  ...
  match D.triage obs with
  | D.Skip _ -> ()
  | D.Triggered _ -> fail "idle without goals should not trigger"
;;
...
let test_action_to_json_roundtrip () =
  ...
  let typ = Yojson.Safe.Util.member "type" json |> Yojson.Safe.Util.to_string in
...
let test_action_to_json_noop () =
  ...
  let json = D.deliberation_action_to_json action in
  let reason = Yojson.Safe.Util.member "reason" json |> Yojson.Safe.Util.to_string in
...
let test_deliberation_meta_json_roundtrip () =
  ...
  check
    string
    "triggers roundtrip"
    "direct_mention,new_unclaimed_task"
...
let test_keeper_meta_deliberation_fields_roundtrip () =
  ...
  | Ok meta -> check string "name preserved" "test-keeper" meta.name
;;
...
let test_keeper_meta_deliberation_fields_default () =
  ...
  match Keeper_types.meta_of_json json with
  | Error err -> fail ("meta parse failed: " ^ err)
  | Ok meta -> check string "name preserved" "test-keeper-2" meta.name
;;
...
let test_triage_result_skip_json () =
  ...
  let decision = Yojson.Safe.Util.member "decision" json |> Yojson.Safe.Util.to_string in
  check string "skip decision" "skip" decision
;;

let test_triage_result_triggered_json () =
  let json =
    D.triage_result_to_json (D.Triggered [ D.DirectMention; D.NewUnclaimedTask ])
  in
  let decision = Yojson.Safe.Util.member "decision" json |> Yojson.Safe.Util.to_string in
  let triggers = Yojson.Safe.Util.member "triggers" json |> Yojson.Safe.Util.to_list in
...
let test_world_observation_json () =
  ...
  let dm = Yojson.Safe.Util.member "direct_mention" json |> Yojson.Safe.Util.to_bool in
  let utc =
    Yojson.Safe.Util.member "unclaimed_task_count" json |> Yojson.Safe.Util.to_int
  in
...
let test_prompt_contains_keeper_name () =
  ...
  check
    ...
    (String.length prompt > 0
     &&
     try
       ignore (Str.search_forward (Str.regexp_string "alpha-keeper") prompt 0);
       true
     with
     | Not_found -> false)
;;
...
let test_prompt_contains_triggers () =
  ...
  check
    bool
    "contains failed_task"
    true
    (try
       ignore (Str.search_forward (Str.regexp_string "failed_task") prompt 0);
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       ignore (Str.search_forward (Str.regexp_string "idle_timeout") prompt 0);
       true
     with
     | Not_found -> false)
;;
...
let test_prompt_contains_action_list () =
  ...
  check
    bool
    "mentions noop action"
    true
    (try
       ignore (Str.search_forward (Str.regexp_string "noop") prompt 0);
       true
     with
     | Not_found -> false);
  check
    bool
    "mentions reply_in_room action"
    true
    (try
       ignore (Str.search_forward (Str.regexp_string "reply_in_room") prompt 0);
       true
     with
     | Not_found -> false);
  check
    bool
    "mentions task_claim action"
    true
    (try
       ignore (Str.search_forward (Str.regexp_string "task_claim") prompt 0);
       true
     with
     | Not_found -> false);
  check
    bool
    "mentions broadcast action"
    true
    (try
       ignore (Str.search_forward (Str.regexp_string "broadcast") prompt 0);
       true
     with
     | Not_found -> false)
;;
...
let test_parse_valid_noop_json () =
  ...
  | Ok (action, reasoning, confidence) ->
    (match action with
     | D.Noop reason -> check string "noop reason" "All quiet" reason
...
let test_parse_valid_broadcast_json () =
  ...
  | Ok (action, _reasoning, _confidence) ->
    (match action with
     | D.Broadcast { message } -> check string "message" "Status update" message
...
let test_legality_verdict_rejects_illegal_task_claim () =
  let obs = D.{ base_obs with unclaimed_task_count = 0 } in
  match
    D.legality_verdict obs (D.TaskClaim { task_id = "task-1"; reason = "urgent" })
  with
  | D.Illegal msg ->
    check bool "mentions unclaimed tasks" true (contains_substring msg "unclaimed tasks")
  | D.Legal -> fail "expected illegal task_claim without unclaimed tasks"
;;

let test_legality_verdict_rejects_nested_multistep () =
  let obs = D.{ base_obs with unclaimed_task_count = 1 } in
  let action =
    D.MultiStep
...
let test_execute_structured_result_keeps_legal_action () =
  ...
  | D.TaskClaim { task_id; _ } -> check string "selected task" "task-1" task_id
...
let test_execute_structured_result_falls_back_to_baseline () =
  ...
  match result.selected_action with
  | D.Noop reason -> check string "fallback noop" "no_trigger" reason
...
let test_parse_missing_action_field () =
  let raw = {|{"params":{"reason":"test"},"reasoning":"no action","confidence":0.5}|} in
  match D.parse_deliberation_response raw with
  | Error _ -> ()
...
let test_parse_unknown_action_type () =
  ...
  | Error msg ->
    check
      ...
      true
      (try
         ignore (Str.search_forward (Str.regexp_string "unknown action") msg 0);
         true
       with
       | Not_found -> false)
...
let test_parse_missing_confidence_defaults () =
  let raw = {|{"action":"noop","params":{"reason":"test"},"reasoning":"test"}|} in
  match D.parse_deliberation_response raw with
  | Error msg -> fail ("expected Ok, got Error: " ^ msg)
  | Ok (_action, _reasoning, confidence) ->
...
let test_budget_check_zero_cost () =
  check
    bool
    "zero cost under positive budget"
    true
...
let test_triage_goal_deadline () =
  ...
  let obs =
    { base_obs with active_goal_count = 1; idle_seconds = 700; idle_gate = 300 }
  in
  match D.triage obs with
  | D.Skip _ -> fail "expected GoalDeadline trigger"
  | D.Triggered triggers ->
    check bool "contains GoalDeadline" true (List.mem D.GoalDeadline triggers)
;;

let test_triage_goal_deadline_not_triggered_below_threshold () =
  (* idle_seconds (500) is NOT > idle_gate * 2 (600) *)
  let obs =
    { base_obs with active_goal_count = 2; idle_seconds = 500; idle_gate = 300 }
  in
  match D.triage obs with
  | D.Skip _ -> ()
  | D.Triggered triggers ->
    (* IdleTimeout may fire (500 > 300) but GoalDeadline should not *)
    check bool "GoalDeadline should not trigger" false (List.mem D.GoalDeadline triggers)
;;

let test_triage_goal_deadline_no_goals () =
  (* No active goals, so GoalDeadline should not fire even with high idle *)
  let obs =
    { base_obs with active_goal_count = 0; idle_seconds = 1000; idle_gate = 300 }
  in
  match D.triage obs with
  ...
  | D.Triggered triggers ->
    check
      bool
      "GoalDeadline should not trigger without goals"
      false
...
let test_triage_strategic_review () =
  ...
  let obs =
    { base_obs with active_goal_count = 1; idle_seconds = 1600; idle_gate = 300 }
  in
  match D.triage obs with
  | D.Skip _ -> fail "expected StrategicReview trigger"
  | D.Triggered triggers ->
    check bool "contains StrategicReview" true (List.mem D.StrategicReview triggers)
;;

let test_triage_strategic_review_not_triggered_below_threshold () =
  (* idle_seconds (1400) is NOT > idle_gate * 5 (1500) *)
  let obs =
    { base_obs with active_goal_count = 1; idle_seconds = 1400; idle_gate = 300 }
  in
  match D.triage obs with
  ...
  | D.Triggered triggers ->
    check
      bool
      "StrategicReview should not trigger"
      false
...
let test_parse_multi_step_truncated_to_5 () =
  let steps =
    List.init 7 (fun i ->
      Printf.sprintf {|{"action":"noop","params":{"reason":"step-%d"}}|} i)
  in
  ...
  | Ok (action, _reasoning, _confidence) ->
    (match action with
     | D.MultiStep actions -> check int "truncated to 5 steps" 5 (List.length actions)
     | _ -> fail "expected MultiStep action")
;;
...
let test_parse_multi_step_nested_rejected () =
  ...
  | Error msg ->
    check
      ...
      true
      (try
         ignore (Str.search_forward (Str.regexp_string "nested") msg 0);
         true
       with
       | Not_found -> false)
...
let test_prompt_always_includes_multi_step () =
  ...
  check
    bool
    "prompt contains multi_step"
    true
    (try
       ignore (Str.search_forward (Str.regexp_string "multi_step") prompt 0);
       true
     with
     | Not_found -> false)
;;
...
let test_removed_initiative_field_rejected () =
  let json_str =
    {|{"name":"test","initiative_enabled":true,"trace_id":"t1","goal":"g","cascade_name":"local","proactive_enabled":true,"proactive_idle_sec":300,"proactive_cooldown_sec":60}|}
  in
  let json = Yojson.Safe.from_string json_str in
  match Keeper_types.meta_of_json json with
  | Ok _ -> fail "initiative_enabled should be rejected"
  | Error e ->
    check bool "removed initiative field mentioned" true (String.contains e 'i')
;;

let test_removed_persona_profile_path_rejected () =
  let json_str =
    {|{"name":"test","persona_profile_path":"config/personas/test/profile.json","trace_id":"t2","goal":"g","cascade_name":"local","proactive_enabled":true,"proactive_idle_sec":300,"proactive_cooldown_sec":60}|}
  in
  let json = Yojson.Safe.from_string json_str in
  match Keeper_types.meta_of_json json with
  ...
  | Error e ->
    check
      bool
      "removed persona field mentioned"
      true
      (try
         ignore (Str.search_forward (Str.regexp_string "persona_profile_path") e 0);
         true
       with
       | Not_found -> false)
;;
...
let test_idle_gate_drives_idle_timeout () =
  let obs = { base_obs with active_goal_count = 1; idle_seconds = 120; idle_gate = 60 } in
  match D.triage obs with
  | D.Skip _ -> fail "should trigger IdleTimeout with custom idle_gate=60"
  | D.Triggered triggers ->
    check bool "has idle_timeout" true (List.exists (fun t -> t = D.IdleTimeout) triggers)
;;
...
let test_triage_self_directed_explore () =
  ...
  let obs =
    { base_obs with active_goal_count = 0; idle_seconds = 1300; idle_gate = 300 }
  in
  match D.triage obs with
...
let test_triage_self_directed_not_with_goals () =
  ...
  let obs =
    { base_obs with active_goal_count = 1; idle_seconds = 1300; idle_gate = 300 }
  in
  match D.triage obs with
  | D.Skip _ -> ()
  | D.Triggered triggers ->
    check
      bool
      "SelfDirectedExplore should not trigger with goals"
      false
      (List.mem D.SelfDirectedExplore triggers)
;;
...
let test_triage_self_directed_not_below_threshold () =
  ...
  | D.Triggered triggers ->
    check
      bool
      "SelfDirectedExplore should not trigger below threshold"
      false
      (List.mem D.SelfDirectedExplore triggers)
;;
...
let test_triage_board_new_posts_without_mention () =
  ...
  | D.Triggered triggers ->
    let has_new_posts =
      List.exists
        (function
          | D.BoardActivity "new_posts" -> true
          | _ -> false)
        triggers
    in
...
let test_triage_board_new_posts_suppressed_by_mention () =
  ...
  | D.Triggered triggers ->
    ...
    let has_mention =
      List.exists
        (function
          | D.BoardActivity "mentioned_in_post" -> true
          | _ -> false)
        triggers
    in
...
let test_legality_self_directed_allows_share_finding () =
  ...
  in
  match
    D.legality_verdict obs (D.ShareFinding { finding = "test"; source = "board_scan" })
  with
  | D.Legal -> ()
...
let test_legality_self_directed_allows_start_discussion () =
  ...
  match
    D.legality_verdict
      obs
      (D.StartDiscussion { topic = "test topic"; context = "exploring" })
  with
  | D.Legal -> ()
...
let test_legality_not_self_directed_rejects_board_post () =
  ...
  in
  match D.legality_verdict obs (D.BoardPost { content = "test"; hearth = None }) with
...
let () =
  run
    ...
    [ ( "triage"
      , [ test_case
            "skip on empty observation"
            `Quick
            test_triage_skip_on_empty_observation
        ; test_case "direct mention triggers" `Quick test_triage_direct_mention
        ; test_case "unclaimed task triggers" `Quick test_triage_unclaimed_task
        ; test_case "failed task triggers" `Quick test_triage_failed_task
        ; test_case "agent change triggers" `Quick test_triage_agent_change
        ; test_case "board mention triggers" `Quick test_triage_board_mention
        ; test_case "idle with goals triggers" `Quick test_triage_idle_with_goals
        ; test_case "idle without goals skips" `Quick test_triage_idle_without_goals_skips
        ; test_case "multiple triggers" `Quick test_triage_multiple_triggers
        ; test_case "goal deadline triggers" `Quick test_triage_goal_deadline
        ; test_case
            "goal deadline not triggered below threshold"
            `Quick
            test_triage_goal_deadline_not_triggered_below_threshold
        ; test_case "goal deadline no goals" `Quick test_triage_goal_deadline_no_goals
        ; test_case "strategic review triggers" `Quick test_triage_strategic_review
        ; test_case
            "strategic review not triggered below threshold"
            `Quick
            test_triage_strategic_review_not_triggered_below_threshold
        ] )
    ; ( "actions"
      , [ test_case "noop to policy label" `Quick test_action_to_policy_label_noop
        ; test_case "reply to policy label" `Quick test_action_to_policy_label_reply
        ; test_case
            "board_post to policy label"
            `Quick
            test_action_to_policy_label_board_post
        ; test_case
            "task_claim to policy label"
            `Quick
            test_action_to_policy_label_task_claim
        ; test_case "action to json roundtrip" `Quick test_action_to_json_roundtrip
        ; test_case "noop to json preserves reason" `Quick test_action_to_json_noop
        ; test_case "multistep to string" `Quick test_action_multistep_to_string
        ] )
    ; ( "baseline"
      , [ test_case
            "mention returns ReplyInRoom"
            `Quick
            test_baseline_mention_returns_reply
        ; test_case "no mention returns Noop" `Quick test_baseline_no_mention_returns_noop
        ; test_case
            "baseline execution emits baseline source"
            `Quick
            test_baseline_execution_result_emits_baseline_source
        ] )
    ; ( "deliberation_meta"
      , [ test_case "json roundtrip" `Quick test_deliberation_meta_json_roundtrip
        ; test_case "defaults from empty json" `Quick test_deliberation_meta_defaults
        ] )
    ; ( "keeper_meta"
      , [ test_case
            "deliberation fields roundtrip"
            `Quick
            test_keeper_meta_deliberation_fields_roundtrip
        ; test_case
            "deliberation fields default"
            `Quick
    ...
    ; ( "build_deliberation_prompt"
      , [ test_case "prompt contains keeper name" `Quick test_prompt_contains_keeper_name
        ; test_case "prompt contains trigger strings" `Quick test_prompt_contains_triggers
        ; test_case
            "prompt mentions tool input schema"
            `Quick
        ...
        ; test_case
            "prompt always includes multi_step"
            `Quick
            test_prompt_always_includes_multi_step
        ] )
    ; ( "structured_result_schema"
      , [ test_case "schema metadata" `Quick test_structured_result_schema_metadata
        ; test_case
            "schema parse valid json"
            `Quick
            test_structured_result_schema_parse_valid_json
        ] )
    ; ( "deterministic_execution"
      , [ test_case
            "rejects illegal task_claim"
            `Quick
            test_legality_verdict_rejects_illegal_task_claim
        ; test_case
            "rejects nested multi_step"
            `Quick
    ...
    ; ( "parse_deliberation_response"
      , [ test_case "parse valid noop" `Quick test_parse_valid_noop_json
        ; test_case "parse valid reply_in_room" `Quick test_parse_valid_reply_json
        ; test_case "parse valid task_claim" `Quick test_parse_valid_task_claim_json
        ; test_case "parse valid broadcast" `Quick test_parse_valid_broadcast_json
        ; test_case
            "parse JSON with code fences rejected"
            `Quick
            test_parse_json_with_code_fences_rejected
        ; test_case
            "parse JSON with surrounding text rejected"
            `Quick
            test_parse_json_with_surrounding_text_rejected
        ; test_case "parse malformed JSON returns Error" `Quick test_parse_malformed_json
        ; test_case "parse empty string returns Error" `Quick test_parse_empty_string
        ; test_case "parse missing action field" `Quick test_parse_missing_action_field
        ; test_case "parse unknown action type" `Quick test_parse_unknown_action_type
        ; test_case
            "reply with empty content fails"
            `Quick
            test_parse_reply_empty_content_fails
        ; test_case
            "task_claim with empty task_id fails"
            `Quick
            test_parse_task_claim_empty_task_id_fails
        ; test_case "confidence clamped to 1.0" `Quick test_parse_confidence_clamped
        ; test_case
        ...
        ; test_case "parse multi_step action" `Quick test_parse_multi_step_action
        ; test_case
            "multi_step empty steps fails"
            `Quick
            test_parse_multi_step_empty_steps_fails
        ; test_case
            "multi_step truncated to 5"
            `Quick
            test_parse_multi_step_truncated_to_5
        ; test_case
            "nested multi_step rejected"
            `Quick
            test_parse_multi_step_nested_rejected
        ; test_case
            "multi_step invalid substep fails"
            `Quick
            test_parse_multi_step_invalid_substep_fails
        ] )
    ; ( "deliberation_budget"
      , [ test_case "under budget returns true" `Quick test_budget_check_under_budget
        ; test_case "at budget returns false" `Quick test_budget_check_at_budget
        ; test_case "over budget returns false" `Quick test_budget_check_over_budget
        ; test_case "zero budget always false" `Quick test_budget_check_zero_budget
        ; test_case "zero cost under positive budget" `Quick test_budget_check_zero_cost
        ; test_case "default daily budget value" `Quick test_default_daily_budget
        ; test_case
            "daily budget from env default"
            `Quick
            test_daily_budget_from_env_default
        ; test_case
            "daily budget from env custom"
            `Quick
            test_daily_budget_from_env_custom
        ] )
    ; ( "keeper_field_cleanup"
      , [ test_case
            "initiative field rejected"
            `Quick
            test_removed_initiative_field_rejected
        ; test_case
            "persona profile path rejected"
            `Quick
            test_removed_persona_profile_path_rejected
        ; test_case
            "idle_gate drives idle timeout"
            `Quick
            test_idle_gate_drives_idle_timeout
        ] )
    ; ( "self_directed_triggers"
      , [ test_case "self-directed explore fires" `Quick test_triage_self_directed_explore
        ; test_case
            "self-directed not with goals"
            `Quick
            test_triage_self_directed_not_with_goals
        ; test_case
            "self-directed not below threshold"
            `Quick
            test_triage_self_directed_not_below_threshold
        ; test_case
            "board new posts without mention"
            `Quick
            test_triage_board_new_posts_without_mention
        ; test_case
            "board new posts suppressed by mention"
            `Quick
            test_triage_board_new_posts_suppressed_by_mention
        ; test_case "board new posts too soon" `Quick test_triage_board_new_posts_too_soon
        ] )
    ; ( "self_directed_legality"
      , [ test_case
            "self-directed allows board_post"
            `Quick
            test_legality_self_directed_allows_board_post
        ; test_case
            "self-directed allows share_finding"
            `Quick
            test_legality_self_directed_allows_share_finding
        ; test_case
            "self-directed allows start_discussion"
            `Quick
            test_legality_self_directed_allows_start_discussion
        ; test_case
            "not self-directed rejects board_post"
            `Quick
