...
let paused_meta_requires_reconcile_recovery (meta : keeper_meta) =
  ...
  | Some Ambiguous_post_commit_timeout | Some Ambiguous_post_commit_failure -> true
  | None ->
    (match Keeper_status_bridge.blocker_class_of_string meta.runtime.last_blocker with
     | Some Ambiguous_post_commit_timeout | Some Ambiguous_post_commit_failure -> true
     | _ -> false)
  | _ -> false
;;
...
let committed_tools_of_ambiguous_blocker (blocker : string) =
  ...
  if String.starts_with ~prefix:"[masc_oas_error]" trimmed
  then (
    let payload =
      String.sub
        trimmed
...
let launch_supervised_fiber
  ...
  Eio.Fiber.fork ~sw:ctx.sw (fun () ->
    ...
    let resolve_done value =
      if (not !resolved) && Option.is_none (Eio.Promise.peek reg.done_p)
      then (
        resolved := true;
         ...
         try
           ...
           then (
             let reason =
               match Keeper_registry.get ~base_path meta.name with
               | Some e ->
                 Option.map Keeper_registry.failure_reason_to_string e.last_failure_reason
        ...
        try
          ...
          then
            ...
            then (
              ...
              let reason =
                Keeper_registry.failure_reason_to_string Keeper_registry.Fiber_unresolved
              in
...
let supervise_keepalive ~proactive_warmup_sec (ctx : _ context) (meta : keeper_meta) =
  ...
  then ()
    ...
    (try
     ...
     with
     | Eio.Cancel.Cancelled _ as e -> raise e
     | exn -> Log.Keeper.error "supervisor room init failed: %s" (Printexc.to_string exn));
    let live_meta =
      try
...
let resume_keeper_after_reconcile_gate (ctx : _ context) (meta : keeper_meta) =
  ...
  let resumed_meta =
    { latest_meta with
      paused = false
    ; updated_at = now_iso ()
    ; runtime = { latest_meta.runtime with last_blocker = ""; last_blocker_class = None }
    }
  in
  ...
  let _approval_id =
    Keeper_approval_queue.submit_pending
      ...
      ~on_resolution:(fun decision ->
        match decision with
        | Oas.Hooks.Approve | Oas.Hooks.Edit _ ->
...
let reconcile_keepalive_keepers (ctx : _ context) =
  ...
  List.iter
    (fun name ->
       ...
       | Ok None -> ()
       | Error err -> Log.Keeper.debug "reconcile: read_meta failed for %s: %s" name err)
    names;
  Log.Keeper.info
...
let apply_self_preservation ~keepers_dir ~total_keepers to_restart =
  ...
  then (
    ...
    then (
      ...
      let dominant_set =
        List.map
          (fun ((e : Keeper_registry.registry_entry), _) -> e.name)
          dominant_entries
      in
      List.filter
...
let sweep_and_recover (ctx : _ context) =
  ...
  List.iter
    (fun ((entry : Keeper_registry.registry_entry), msg) ->
       ...
       (match Keeper_keepalive.get_bus () with
        | Some bus ->
          Oas_events.publish_keeper_dead
            bus
       ...
       Prometheus.inc_counter
         Prometheus.metric_keeper_dead_total
         ~labels:
       ...
       | Ok (Some meta) ->
         ...
         then (
           Log.Keeper.warn
             ...
             max_restarts;
           Prometheus.inc_counter
             Prometheus.metric_keeper_near_exhaustion_total
