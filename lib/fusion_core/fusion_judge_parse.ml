...
let parse_position : Yojson.Safe.t -> (string * string) option = function
  | `Assoc kvs ->
    (match opt_string kvs "model", opt_string kvs "stance" with
...
let parse_insight : Yojson.Safe.t -> Fusion_types.insight option = function
  | `Assoc kvs ->
    (match opt_string kvs "text", opt_string kvs "model" with
...
let parse_decision : Yojson.Safe.t -> (Fusion_types.judge_decision, string) result = function
  | `Assoc kvs ->
    (match find kvs "kind" with
     | Some (`String "answer") ->
       let* answer = req_string kvs "answer" in
       Ok (Fusion_types.Answer answer)
     | Some (`String "recommend") ->
       let* action = req_string kvs "action" in
...
let of_json (j : Yojson.Safe.t) : (Fusion_types.judge_synthesis, string) result =
  ...
  let* resolved_answer = req_string kvs "resolved_answer" in
  let* decision =
    match find kvs "decision" with
