...
let dispatch (ctx : context) ~(name : string) : tool_result option =
  ...
  let _arg_get_bool key default = Safe_ops.json_bool ~default key arguments in
  let _arg_get_string_list key = Safe_ops.json_string_list key arguments in
  let arg_get_string_opt key =
    match Safe_ops.json_string_opt key arguments with
    | Some "" -> None
    | other -> other
  in
  let _arg_get_string_required key = Tool_args.get_string_required arguments key in
  let _arg_get_int_opt _key = () in
  (* unused but kept for symmetry *)
  let _arg_get_float_opt key = Safe_ops.json_float_opt key arguments in
  match name with
  ...
  | "masc_approval_get" ->
    ...
    if id = ""
    then Some (false, "id is required")
    else (
  ...
  | "masc_approval_resolve" ->
    ...
    then Some (false, "id is required")
      ...
      | Error err -> Some (false, Keeper_approval_queue.resolve_error_to_string err))
     ...
     | None ->
       Some
         ...
         , Printf.sprintf
             ...
             (String.concat "|" Mcp_session.valid_action_strings)
             raw )
     | Some action ->
       ...
       let response =
         ...
         | Mcp_session.Create ->
           ...
           let record : Mcp_server_eio_governance.mcp_session_record =
             { id; agent_name; created_at = now; last_seen = now }
           in
         ...
         | Mcp_session.Get ->
           ...
           (match
              List.find_opt
                (fun (s : Mcp_server_eio_governance.mcp_session_record) ->
                   s.id = session_id)
                sessions
            with
            ...
            | Some s ->
              ...
              let others =
                List.filter
                  (fun (x : Mcp_server_eio_governance.mcp_session_record) ->
                     x.id <> session_id)
                  sessions
              in
              save (updated :: others);
         ...
         | Mcp_session.List ->
           Ok
             (`Assoc
                 ...
                 ; ( "sessions"
                   , `List
                       (List.map Mcp_server_eio_governance.mcp_session_to_json sessions) )
                 ])
         | Mcp_session.Cleanup ->
         ...
         | Mcp_session.Remove ->
           ...
           let remaining =
             List.filter
               ...
               sessions
           in
           if List.length remaining = List.length sessions
           then Error (Printf.sprintf "MCP session '%s' not found" session_id)
           else (
             save remaining;
             Ok (`Assoc [ "status", `String "removed"; "session_id", `String session_id ]))
       in
       (match response with
  ...
  | "masc_spawn" ->
    ...
    if prompt = ""
    then Some (false, "prompt is required")
    else (
      ...
      let runtime_model_valid =
        ...
        | "llama", Some raw ->
          ...
          (match Cascade_config.parse_model_string spec_name with
           | Some _ -> Ok ()
           | None -> Error "invalid model spec")
        | _ ->
      ...
      let working_dir =
        match arguments |> U.member "working_dir" with
  ...
  | "masc_discover_tools" ->
    ...
    if query = ""
    then Some (false, "query is required")
    else (
      let all_schemas =
        Config.visible_tool_schemas ~include_hidden:true ~include_deprecated:false ()
      in
      let words =
        String.split_on_char ' ' query |> List.filter (fun w -> String.length w > 0)
      in
      let matches =
        all_schemas
        |> List.filter (fun (schema : Types.tool_schema) ->
          ...
          words |> List.exists (fun w -> String_util.contains_substring haystack w))
      ...
      let results =
        List.map
          (fun (schema : Types.tool_schema) ->
             `Assoc
               [ "name", `String schema.name; "description", `String schema.description ])
          matches
      in
      Some
        ( true
        , Yojson.Safe.to_string
            (`Assoc
                ...
                ; ( "hint"
                  , `String
                      "These tools are callable via tools/call even if not in the \
                       default tools/list." )
                ]) ))
  (* ── Fallthrough to extra dispatch ──────────────────────────── *)
  | _ ->
    Tool_inline_dispatch_extra.dispatch
      ~config
      ~agent_name
      ~arguments
      ~state
