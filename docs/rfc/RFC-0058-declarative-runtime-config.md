---
title: Declarative Runtime Configuration (v2)
rfc: 0058
status: Active
created: 2026-05-09
implementation_prs: []
...
# RFC-0058: Declarative Runtime Configuration (v2)
...
| Created | 2026-05-10 |
| Updated | 2026-05-10 |
| Supersedes | RFC-0055 (runtime fallback tier routing), RFC-0058 Phase 1 (TOML-only loader hotpath, PR #14460) |
| Depends | agent_sdk `Provider_kind.t` elimination (breaking) |
| Breaking | Yes — current `keeper_runtime.toml` profile format incompatible |

---

## 1. Problem

Phase 1 (PR #14460) moved the runtime loader to TOML-only mode, but the schema
...
4. Routing by runtime composition, not by hardcoded profile name
...
Layer 5: [tier.*] + [runtime.*] + [routes.*] — Routing strategy
```
...
(* tier and tier_group types removed in PR #19381.
   Routing now uses direct runtime names (provider:model format). *)
...
`providers`, `models`, `system`, `tier`, `runtime`, `routes`, `profiles`
...
6. Route targets must resolve to valid runtimes
...
**Breaking change.** Current `keeper_runtime.toml` profile format is incompatible.
The migration path:
...
| `runtime_capability_profile.ml` closed variant | Config-driven `api_format` + `transport` |
...
A migration script converts the current `config/keeper_runtime.toml` to the new schema.
...
| `runtime_transport.ml` | 200+ | Provider-kind CLI dispatch → protocol adapter dispatch |
| `runtime_capability_profile.ml` | 105 | Closed variant → eliminated (config-driven) |
| `runtime_tier.ml` | 22 | Tier def → expanded with new types |
| `runtime_client_capacity.ml` | 100 | URL-based → binding-based capacity |
| `runtime_toml_materializer.ml` | 595 | Flat profile parser → 5-layer parser |

### Moderate changes

| File | Role |
|------|------|
| `runtime_config_loader.ml` | Profile loading → declarative config loading |
| `runtime_catalog_runtime.ml` | Profile snapshot → binding/tier snapshot |
| `runtime_config.ml` | Re-exports → updated types |
| `runtime_routes.ml` | Route mapping → runtime routing |
| `runtime_strategy.ml` | Strategy selection → config-driven strategy |
| `runtime_pool.ml` | Model pool → binding pool |

### New modules

| Module | Role |
|--------|------|
| `runtime_declarative_types.ml` | 5-layer type definitions |
| `runtime_declarative_parser.ml` | TOML → typed config parser |
| `runtime_declarative_validator.ml` | Cross-reference validation |
...
- Write RFC-0058 v2
- Define `runtime_declarative_types.ml` with internal types

### Phase 1: Schema + Parser
- `runtime_declarative_parser.ml` — TOML → typed config
- `runtime_declarative_validator.ml` — cross-reference checks
...
- Adapter layer: new types → existing `Runtime_catalog` interface
...
- Migration script: old `keeper_runtime.toml` → new schema
- Dashboard JSON removal (absorbed into TOML-only mode)
- `ensure_materialized_json` eliminated

### Phase 9: `runtime.json` elimination (split into 3 PRs)

The acceptance criterion "*`runtime.json` no longer generated or consumed*"
unwinds in three steps. The work splits not because there are many
*readers* (most readers go through `Runtime_config_loader.load_json`,
which is already in-memory — see Phase 9.1) but because there are two
remaining *writers* that materialise `config/runtime.json` on disk:
`lib/dashboard_runtime.ml` (two `ensure_materialized_json` call sites)
and the standalone `bin/runtime_materialize` CLI. Eliminating those
...
- **Phase 9.1 — Stop tracking the committed JSON** *(landed in #14578)*:
  drop the committed `config/runtime.json` and the byte-equality test
  ...
  the runtime do not depend on its presence on disk:
  - `Runtime_config_loader.load_json` (lib/runtime/runtime_config_loader.ml:96)
    branches on `source_info.kind`. Under the current TOML SSOT the
    branch is always `Toml`, which routes through
    `load_toml_in_memory` → `Runtime_toml_materializer.render_toml_to_json_string`
    and never opens the JSON sibling. The `Json` branch (which would
    `read_json_file`) is unreachable until someone removes the TOML.
  - The two writers that *do* materialise `config/runtime.json` on disk
    are `lib/dashboard_runtime.ml` (two `ensure_materialized_json` call
    sites, kept so the dashboard can serve the rendered file) and the
    `bin/runtime_materialize` CLI. Both keep working after Phase 9.1 —
  ...
  *will* keep appearing as an **untracked** file in `git status` until
  ...
  entirely. Phase 9.1 intentionally does not add `config/runtime.json`
...
- **Phase 9.2 — Migrate the remaining disk writers off JSON**: rework
  `lib/dashboard_runtime.ml` and `bin/runtime_materialize` to serve /
  emit the rendered JSON in memory via
  `Runtime_toml_materializer.render_toml_to_json_string`. After this
...
- **Phase 9.3 — Delete the JSON path entirely**: remove
  ...
  `Runtime_toml_materializer.source_kind` (now unreachable), retire the
  `read_json_file` branch in `Runtime_config_loader.load_json`, and
  audit-grep `lib/` + `bin/` for any residual `runtime.json`
  references.
...
- [ ] `Provider_kind.t` eliminated from runtime code paths
...
- [ ] Existing runtime routes expressible in new schema
- [ ] Tier/runtime routing works (within-tier failover + between-tier priority)
...
- [ ] `runtime.json` no longer generated or consumed

## 10. Risks

| Risk | Mitigation |
|------|------------|
| agent_sdk `Provider_kind.t` is used outside runtime | Adapter layer wraps runtime-internal types; agent_sdk boundary is explicit |
...
- `docs/runtime/README.md` — runtime design index
