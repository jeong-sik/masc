...
let test_json_emits_scorecard_and_artifacts () =
  ...
  with_env "MASC_KEEPER_BENCH_CANARY_ENABLED" (Some "true") (fun () ->
    with_env "MASC_KEEPER_BENCH_CANARY_PATH" (Some manifest_path) (fun () ->
      ...
      check
        bool
        "domain includes tool correctness"
        true
        (List.exists
           (fun item ->
...
let test_history_dedupes_identical_payloads () =
  ...
  with_env "MASC_KEEPER_BENCH_CANARY_ENABLED" (Some "true") (fun () ->
    with_env "MASC_KEEPER_BENCH_CANARY_PATH" (Some manifest_path) (fun () ->
      ...
      let latest_path =
        Yojson.Safe.Util.(
          first |> member "artifacts" |> member "latest_path" |> to_string)
      in
      let history_path =
        Yojson.Safe.Util.(
          second |> member "artifacts" |> member "history_path" |> to_string)
      in
      let latest_json =
      ...
      check
        bool
        "latest file is valid json"
        true
        (match latest_json with
         | `Assoc _ -> true
         | _ -> false)))
;;
...
let () =
  run
    ...
    [ ( "dashboard_safe_autonomy"
      , [ test_case
            "json emits scorecard and artifacts"
            `Quick
            test_json_emits_scorecard_and_artifacts
        ; test_case
            "artifact history dedupes identical payloads"
            `Quick
