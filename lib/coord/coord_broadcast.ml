...
let emit_message_activity
  ...
  let emit ?subject ~kind ~tags () =
    ...
    | exn ->
      Log.Misc.warn "message activity emit failed (%s): %s" kind (Printexc.to_string exn)
  in
  emit
...
let on_broadcast_mention : (string option -> unit) ref = ref (fun _mention -> ())
...
let broadcast ?trace_context config ~from_agent ~content =
  ...
  (match
     backend_publish
       config
       ~channel:(broadcast_channel config)
  ...
  emit_message_activity config ~from_agent:safe_agent ~content:safe_content ~mention ();
  (try !on_broadcast_mention mention with
   | Eio.Cancel.Cancelled _ as e -> raise e
   | exn ->
     Log.Misc.warn "on_broadcast_mention callback failed: %s" (Printexc.to_string exn));
