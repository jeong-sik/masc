...
let handle_keeper_fs_read
  ...
  | Error e when is_missing_read_path_error e ->
    ...
    let target = if Filename.is_relative path then Filename.concat root path else path in
             ...
             (try
                ...
                then
                  error_json
                    ...
                    "patch target file does not exist or is empty. Use mode=overwrite to \
                     create."
                else (
                  match apply_patch ~old_string ~new_string ~replace_all current with
                  | Error msg -> error_json ~fields:[ "path", `String target ] msg
          ...
          (match validate_write_target ~config ~meta ~target with
           ...
           | Ok () ->
             (try
                ...
                in
                match write_result with
                | Error msg -> raise_fs_edit_error ~fields:[ "path", `String target ] msg
                | Ok () ->
                  Log.Keeper.info
