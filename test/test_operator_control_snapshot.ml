...
let test_keeper_up_clears_dead_tombstone_resume_state () =
  ...
  (* A resumable keeper resolves its sandbox_profile from the keeper TOML;
     masc_keeper_up fails closed without one (keeper_meta_contract
     [effective_meta_of_profile_defaults], introduced by the sandbox_profile
     SSOT/fail-closed work in #24224/#24226). Seed the profile the way a real
     workspace persists it so this test exercises the operator-resume clearing
     path instead of the missing-profile rejection. *)
  let () =
    let keepers_dir =
    ...
    in
    mkdir_p keepers_dir;
    let oc = open_out (Filename.concat keepers_dir (keeper_name ^ ".toml")) in
