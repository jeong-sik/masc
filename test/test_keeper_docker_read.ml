...
let test_legacy_keeper_never_routes () =
  ...
  Alcotest.(check bool)
    "legacy keeper never routes through docker"
    false
...
let test_docker_keeper_routes () =
  ...
  Alcotest.(check bool)
    "docker keeper routes through docker"
    true
    (Keeper_docker_read.should_route_read ~meta)
;;
...
let setup_config name =
  ...
  let meta = make_meta ~name ~sandbox:Keeper_types.Docker in
  base, config, meta
;;
...
let with_fake_docker script f =
  ...
  let path =
    ...
    | _ -> dir
  in
  Fun.protect ~finally:(fun () -> cleanup_dir dir)
  @@ fun () ->
  with_env "MASC_TEST_FAKE_DOCKER_PATH" docker_path @@ fun () -> with_env "PATH" path f
;;

let test_container_path_root_maps () =
  let base, config, meta = setup_config "minjae" in
  Fun.protect ~finally:(fun () -> cleanup_dir base)
  @@ fun () ->
  let host_root = Keeper_sandbox.host_root_abs_of_meta ~config meta in
  let croot = Keeper_sandbox.container_root meta.name in
  match Keeper_docker_read.container_path_of_host ~config ~meta ~host_path:host_root with
  | Ok mapped ->
    Alcotest.(check string) "host playground root maps to container root" croot mapped
  | Error e -> Alcotest.fail e
;;
...
let test_container_path_nested_maps_with_suffix () =
  ...
  let croot = Keeper_sandbox.container_root meta.name in
  match Keeper_docker_read.container_path_of_host ~config ~meta ~host_path with
  | Ok mapped ->
...
let test_container_path_outside_playground_errors () =
  ...
  match Keeper_docker_read.container_path_of_host ~config ~meta ~host_path:outside with
  | Ok mapped ->
    Alcotest.failf "expected error for outside-playground path, got Ok %s" mapped
  | Error _ -> ()
;;
...
let test_run_command_empty_argv_errors () =
  with_env "MASC_KEEPER_SANDBOX_DOCKER_IMAGE" "alpine:test"
  @@ fun () ->
  let base, config, meta = setup_config "minjae" in
  Fun.protect ~finally:(fun () -> cleanup_dir base)
  @@ fun () ->
...
let test_sandbox_container_label_args_include_owner_scope () =
  ...
  let has_label_prefix prefix = List.exists (String.starts_with ~prefix) args in
  Alcotest.(check bool)
    "component label"
    true
    (has_label "masc.mcp.component=keeper-sandbox");
  Alcotest.(check bool)
    "base path hash label"
    true
    (has_label_prefix "masc.mcp.base_path_hash=");
  Alcotest.(check bool)
    "sanitized keeper label"
    true
    (has_label "masc.mcp.keeper=min_jae");
  Alcotest.(check bool) "kind label" true (has_label "masc.mcp.kind=turn");
  Alcotest.(check bool)
    "owner pid label"
    true
    (has_label ("masc.mcp.owner_pid=" ^ string_of_int (Unix.getpid ())));
  Alcotest.(check bool) "started_at label" true (has_label_prefix "masc.mcp.started_at=");
  Alcotest.(check bool) "network label" true (has_label "masc.mcp.network=none")
;;
...
let test_sandbox_container_label_args_include_managed_ttl () =
  ...
  in
  let has_label value = List.mem value args in
  Alcotest.(check bool) "managed kind label" true (has_label "masc.mcp.kind=managed");
  Alcotest.(check bool) "ttl label" true (has_label "masc.mcp.ttl_sec=90");
  Alcotest.(check bool)
    "inherit network label"
    true
...
let test_docker_network_args_follow_masc_policy () =
  ...
  Alcotest.(check (list string))
    "network none passes docker flag"
    [ "--network"; "none" ]
    args_none;
  ...
  in
  Alcotest.(check (list string)) "network inherit omits docker flag" [] args_inherit;
  Alcotest.(check string) "network inherit label" "inherit" label_inherit
;;

let test_docker_nofile_args_follow_config () =
  with_env "MASC_KEEPER_SANDBOX_NOFILE_LIMIT" "not-a-number"
  @@ fun () ->
  Alcotest.(check (list string))
    "default nofile limit"
    [ "--ulimit"; "nofile=245760:245760" ]
    (Keeper_sandbox_runtime.docker_nofile_args ());
  with_env "MASC_KEEPER_SANDBOX_NOFILE_LIMIT" "8192"
  @@ fun () ->
  Alcotest.(check (list string))
    "configured nofile limit"
    [ "--ulimit"; "nofile=8192:8192" ]
    (Keeper_sandbox_runtime.docker_nofile_args ());
  with_env "MASC_KEEPER_SANDBOX_NOFILE_LIMIT" "256"
  @@ fun () ->
  Alcotest.(check (list string))
    "nofile floor"
...
let test_cleanup_stale_containers_removes_only_stale_masc_scope () =
  with_fake_docker fake_docker_cleanup_script
  @@ fun () ->
  let base = temp_dir () in
  let log_path = Filename.concat base "docker.log" in
  Fun.protect ~finally:(fun () -> cleanup_dir base)
  @@ fun () ->
  with_env "KEEPER_DOCKER_LOG" log_path
  @@ fun () ->
  with_env "KEEPER_TEST_PID" (string_of_int (Unix.getpid ()))
  @@ fun () ->
  let result =
...
let test_docker_preflight_reports_ready_image () =
  ...
  | Some preflight ->
    ...
    Alcotest.(check (list string)) "no missing commands" [] preflight.missing_commands
;;

let test_docker_preflight_surfaces_missing_image_actions () =
  with_fake_docker fake_docker_preflight_missing_image_script
  @@ fun () ->
  with_env "MASC_KEEPER_SANDBOX_PREFLIGHT_ENABLED" "true"
  @@ fun () ->
  ...
  | Some preflight ->
    ...
    Alcotest.(check bool)
      "next actions mention build script"
      true
      (List.exists
         (fun action ->
            String.contains action 'b'
            && contains_substring action "scripts/build-keeper-sandbox-image.sh")
         preflight.next_actions);
    Alcotest.(check bool)
      "failure message mentions build script"
      true
      (contains_substring
...
let test_run_command_nonzero_exit_errors_by_default () =
  with_fake_docker fake_docker_exit_1_script
  @@ fun () ->
  with_env "MASC_KEEPER_SANDBOX_DOCKER_IMAGE" "alpine:test"
  @@ fun () ->
  let base, config, meta = setup_config "minjae" in
  ...
  match
    Keeper_docker_read.run_command_in_container_with_status
      ...
      ()
  with
  | Ok (_st, _out) -> Alcotest.fail "expected exit=1 docker command to error by default"
  | Error msg ->
    Alcotest.(check bool)
      "error preserves exit code"
      true
...
let test_run_command_allows_configured_nonzero_exit () =
  ...
  match
    Keeper_docker_read.run_command_in_container_with_status
      ...
      ()
  with
  | Error msg -> Alcotest.failf "expected exit=1 to be allowed for rg, got %s" msg
  | Ok (st, out) ->
    Alcotest.(check (pair string int))
      "preserves rg no-match status"
      ("exit", 1)
      (match st with
       | Unix.WEXITED code -> "exit", code
       | Unix.WSIGNALED code -> "signaled", code
       | Unix.WSTOPPED code -> "stopped", code);
    Alcotest.(check string) "preserves stdout on allowed exit" "no matches\n" out
;;

let test_run_command_preserves_bare_command_argv () =
  with_fake_docker fake_docker_echo_command_script
  @@ fun () ->
  with_env "MASC_KEEPER_SANDBOX_DOCKER_IMAGE" "alpine:test"
  @@ fun () ->
  let base, config, meta = setup_config "minjae" in
  ...
  match
    Keeper_docker_read.run_command_in_container_with_status
      ~config
      ~meta
      ~command_argv:[ "head"; "-n"; "1"; "/home/keeper/playground/minjae/mind/demo.txt" ]
      ~max_bytes:4096
      ~timeout_sec:5.0
      ()
  with
  | Error msg -> Alcotest.failf "expected bare command argv echo, got %s" msg
  | Ok (st, out) ->
    Alcotest.(check (pair string int))
      "echo script exits cleanly"
      ("exit", 0)
      (match st with
       | Unix.WEXITED code -> "exit", code
       | Unix.WSIGNALED code -> "signaled", code
...
let test_turn_runtime_reuses_single_container () =
  ...
  let count prefix =
    List.fold_left
      (fun acc line -> if String.starts_with ~prefix line then acc + 1 else acc)
      0
      lines
  in
...
let test_default_fs_hardening_helpers () =
  with_env "MASC_KEEPER_SANDBOX_RELAX_FS" "false"
  @@ fun () ->
  Alcotest.(check (list string))
    "default helper keeps read-only rootfs"
    [ "--read-only" ]
    (Env_config_keeper.KeeperSandbox.read_only_rootfs_args ());
  Alcotest.(check bool)
    "default helper keeps tmpfs noexec"
    true
    (contains_substring
...
let test_relaxed_fs_helpers () =
  with_env "MASC_KEEPER_SANDBOX_RELAX_FS" "true"
  @@ fun () ->
  Alcotest.(check (list string))
    "relaxed helper drops read-only rootfs"
    []
    (Env_config_keeper.KeeperSandbox.read_only_rootfs_args ());
  Alcotest.(check bool)
    "relaxed helper drops tmpfs noexec"
    false
    (contains_substring
       (Env_config_keeper.KeeperSandbox.tmpfs_mount ())
       "/tmp:rw,nosuid,nodev,noexec,size=");
  Alcotest.(check bool)
    "relaxed helper keeps writable tmpfs mount"
    true
    (contains_substring
       (Env_config_keeper.KeeperSandbox.tmpfs_mount ())
...
let test_hard_mode_forces_policy_helpers () =
  with_env "MASC_KEEPER_SANDBOX_HARD_MODE" "true"
  @@ fun () ->
  with_env "MASC_KEEPER_SANDBOX_REQUIRE_ROOTLESS" "false"
  @@ fun () ->
  with_env "MASC_KEEPER_SANDBOX_REQUIRE_USERNS" "false"
  @@ fun () ->
  with_env "MASC_KEEPER_SANDBOX_GIT_DISPATCH" "true"
  @@ fun () ->
  with_env "MASC_KEEPER_SANDBOX_GH_CREDS" "/tmp/host-gh"
  @@ fun () ->
  with_env "MASC_KEEPER_SANDBOX_GITCONFIG" "/tmp/host-gitconfig"
  @@ fun () ->
  with_env "MASC_KEEPER_SANDBOX_SSH_DIR" "/tmp/host-ssh"
  @@ fun () ->
  with_env "GH_TOKEN" "host-token"
  @@ fun () ->
  with_env "MASC_KEEPER_SANDBOX_GH_TOKEN" "explicit-token"
  @@ fun () ->
  Alcotest.(check bool)
    "hard mode forces rootless"
    true
    (Env_config_keeper.KeeperSandbox.require_rootless ());
  Alcotest.(check bool)
    "hard mode forces userns"
    true
    (Env_config_keeper.KeeperSandbox.require_userns ());
  Alcotest.(check bool)
    "hard mode disables docker git dispatch"
    false
    (Env_config_keeper.KeeperSandbox.with_git_dispatch_enabled ());
  Alcotest.(check string)
    "hard mode drops gh config mount"
    ""
    (Env_config_keeper.KeeperSandbox.gh_creds_host_path ());
  Alcotest.(check string)
    "hard mode drops gitconfig mount"
    ""
    (Env_config_keeper.KeeperSandbox.gitconfig_host_path ());
  Alcotest.(check string)
    "hard mode drops ssh mount"
    ""
    (Env_config_keeper.KeeperSandbox.ssh_dir_host_path ());
  Alcotest.(check string)
    "hard mode drops GH_TOKEN forwarding"
    ""
...
let test_gh_token_prefers_explicit_override () =
  ...
  Env_config_keeper.KeeperSandbox.For_testing.with_gh_token_probe
    (fun () ->
       ...
       Some "probe-token")
    (fun () ->
       with_env "MASC_KEEPER_SANDBOX_HARD_MODE" ""
       @@ fun () ->
       with_env "GH_TOKEN" "host-token"
       @@ fun () ->
       with_env "MASC_KEEPER_SANDBOX_GH_TOKEN" "explicit-token"
...
let test_gh_token_prefers_host_env_over_keychain_probe () =
  ...
  Env_config_keeper.KeeperSandbox.For_testing.with_gh_token_probe
    ...
    (fun () ->
       with_env "MASC_KEEPER_SANDBOX_HARD_MODE" ""
       @@ fun () ->
       with_env "MASC_KEEPER_SANDBOX_GH_TOKEN" ""
       @@ fun () ->
       with_env "GH_TOKEN" "host-token"
       @@ fun () ->
       Alcotest.(check string)
...
let test_gh_token_uses_cached_keychain_probe () =
  ...
  Env_config_keeper.KeeperSandbox.For_testing.with_gh_token_probe
    ...
    (fun () ->
       ...
       @@ fun () ->
       Alcotest.(check string)
         "keychain fallback token"
         "probe-token"
         (Env_config_keeper.KeeperSandbox.gh_token ());
       Alcotest.(check string)
         "cached keychain fallback"
         "probe-token"
         (Env_config_keeper.KeeperSandbox.gh_token ());
...
let test_gh_token_caches_empty_keychain_probe () =
  ...
  Env_config_keeper.KeeperSandbox.For_testing.with_gh_token_probe
    ...
    (fun () ->
       ...
       @@ fun () ->
       Alcotest.(check string)
         "empty keychain fallback"
         ""
         (Env_config_keeper.KeeperSandbox.gh_token ());
       Alcotest.(check string)
         "cached empty keychain fallback"
         ""
         (Env_config_keeper.KeeperSandbox.gh_token ());
       Alcotest.(check int) "probe called once" 1 !calls)
;;
...
let test_turn_runtime_relaxed_fs_omits_readonly_and_noexec () =
  ...
  | Some line ->
    Alcotest.(check bool)
      "relaxed runtime drops read-only rootfs"
      false
      (contains_substring line "--read-only");
    Alcotest.(check bool)
      "relaxed runtime drops tmpfs noexec"
      false
      (contains_substring line "/tmp:rw,nosuid,nodev,noexec,size=");
    Alcotest.(check bool)
      "relaxed runtime keeps tmpfs mount"
      true
...
let run_tests () =
  Alcotest.run
    "Keeper_docker_read"
    [ ( "should_route_read"
      , [ Alcotest.test_case "legacy never routes" `Quick test_legacy_keeper_never_routes
        ; Alcotest.test_case "docker keeper routes" `Quick test_docker_keeper_routes
        ; Alcotest.test_case
            "docker git-creds also routes"
            `Quick
    ...
    ; ( "container_path_of_host"
      , [ Alcotest.test_case
            "docker network args follow policy"
            `Quick
            test_docker_network_args_follow_masc_policy
        ; Alcotest.test_case
            "docker nofile args follow config"
            `Quick
            test_docker_nofile_args_follow_config
        ; Alcotest.test_case
            "managed label args include ttl"
            `Quick
            test_sandbox_container_label_args_include_managed_ttl
        ; Alcotest.test_case
            "sandbox label args include owner scope"
            `Quick
            test_sandbox_container_label_args_include_owner_scope
        ; Alcotest.test_case
            "playground root maps to container root"
            `Quick
            test_container_path_root_maps
        ; Alcotest.test_case
            "nested host path maps with suffix"
            `Quick
            test_container_path_nested_maps_with_suffix
        ; Alcotest.test_case
            "outside playground errors"
            `Quick
            test_container_path_outside_playground_errors
        ] )
    ; ( "read_file_in_container"
      , [ Alcotest.test_case
            "outside playground returns mapping error"
            `Quick
            test_read_outside_playground_returns_mapping_error
        ; Alcotest.test_case
            "empty image configuration errors"
            `Quick
            test_read_empty_image_config_errors
        ] )
    ; ( "run_command_in_container"
      , [ Alcotest.test_case
            "empty command_argv errors"
            `Quick
        ...
        ; Alcotest.test_case
            "nonzero exit errors by default"
            `Quick
            test_run_command_nonzero_exit_errors_by_default
        ; Alcotest.test_case
            "configured nonzero exit is allowed"
            `Quick
            test_run_command_allows_configured_nonzero_exit
        ; Alcotest.test_case
            "preserves bare command argv"
            `Quick
            test_run_command_preserves_bare_command_argv
        ; Alcotest.test_case
            "default fs hardening helpers"
            `Quick
            test_default_fs_hardening_helpers
        ; Alcotest.test_case "relaxed fs helpers" `Quick test_relaxed_fs_helpers
        ; Alcotest.test_case
            "hard mode forces policy helpers"
            `Quick
            test_hard_mode_forces_policy_helpers
        ; Alcotest.test_case
            "GH_TOKEN explicit override wins"
            `Quick
            test_gh_token_prefers_explicit_override
        ; Alcotest.test_case
            "GH_TOKEN host env wins before keychain"
            `Quick
            test_gh_token_prefers_host_env_over_keychain_probe
        ; Alcotest.test_case
            "GH_TOKEN keychain probe is cached"
            `Quick
            test_gh_token_uses_cached_keychain_probe
        ; Alcotest.test_case
            "GH_TOKEN empty keychain probe is cached"
            `Quick
            test_gh_token_caches_empty_keychain_probe
        ; Alcotest.test_case
            "GH_TOKEN hard mode skips keychain probe"
            `Quick
            test_gh_token_hard_mode_skips_keychain_probe
        ; Alcotest.test_case
            "hard mode rejects relaxed fs without docker"
            `Quick
            test_hard_mode_rejects_relaxed_fs_without_docker
        ; Alcotest.test_case
            "turn runtime reuses single container"
            `Quick
            test_turn_runtime_reuses_single_container
        ; Alcotest.test_case
            "turn runtime relaxed fs omits readonly and noexec"
            `Quick
            test_turn_runtime_relaxed_fs_omits_readonly_and_noexec
        ] )
    ; ( "docker_preflight"
      , [ Alcotest.test_case
            "ready image reports ok"
            `Quick
            test_docker_preflight_reports_ready_image
        ; Alcotest.test_case
            "missing image surfaces remediation"
            `Quick
            test_docker_preflight_surfaces_missing_image_actions
        ] )
    ; ( "docker_cleanup"
      , [ Alcotest.test_case
            "label args include owner scope"
            `Quick
            test_sandbox_container_label_args_include_owner_scope
        ; Alcotest.test_case
            "cleanup removes stale scoped containers"
            `Quick
