...
let test_docker_keeper_routes () =
  let meta =
    make_meta ~name:"minjae" ~sandbox:Masc_mcp.Keeper_types_profile.Docker
  in
...
let test_docker_git_creds_routes () =
  let meta =
    make_meta ~name:"poe" ~sandbox:Masc_mcp.Keeper_types_profile.Docker
  in
...
let setup_config name =
  ...
  let meta =
    make_meta ~name ~sandbox:Masc_mcp.Keeper_types_profile.Docker
  in
  base, config, meta
...
let test_docker_network_args_follow_masc_policy () =
  let args_none, label_none =
    Keeper_sandbox_runtime.docker_network_args Masc_mcp.Keeper_types_profile.Network_none
  in
  ...
  let args_inherit, label_inherit =
    Keeper_sandbox_runtime.docker_network_args Masc_mcp.Keeper_types_profile.Network_inherit
  in
...
let test_startup_preflight_skips_required_command_inventory () =
  ...
  match
    Keeper_sandbox_runtime.ensure_keeper_startup_preflight
      ~timeout_sec:5.0 ~sandbox_profile:Masc_mcp.Keeper_types_profile.Docker
  with
