...
let test_criterion_roundtrip () =
  ...
  List.iter
    (fun c ->
       ...
       | Ok result ->
         Alcotest.(check bool) "criterion roundtrip" true (V.equal_criterion c result)
...
let test_verdict_roundtrip () =
  ...
  List.iter
    (fun v ->
       ...
       | Ok result ->
         Alcotest.(check bool) "verdict roundtrip" true (V.equal_verdict v result)
       | Error e -> Alcotest.fail e)
    verdicts
;;
...
let test_evaluate_literal_and_empty_needles () =
  ...
  Alcotest.(check bool)
    "contains treats regex metacharacters literally"
    true
    (V.evaluate_criterion output (V.Contains ".*") = V.Pass);
  Alcotest.(check bool)
    "contains empty needle stays fail"
    true
    (match V.evaluate_criterion output (V.Contains "") with
     | V.Fail _ -> true
     | _ -> false);
  Alcotest.(check bool)
    "not_contains empty needle stays pass"
    true
...
let test_evaluate_all_pass () =
  ...
  Alcotest.(check bool) "all pass" true (V.evaluate_all output criteria = V.Pass)
;;
...
let test_evaluate_empty_criteria () =
  Alcotest.(check bool) "empty criteria = pass" true (V.evaluate_all `Null [] = V.Pass)
;;
...
let test_create_and_load () =
  with_temp_dir (fun base_path ->
    ...
    | Ok req ->
      Alcotest.(check bool)
        "persisted under .masc/verifications"
        true
        (Sys.file_exists
...
let test_list_requests () =
  with_temp_dir (fun base_path ->
    ...
    in
    let _ =
      V.create_request ~base_path ~task_id:"t2" ~output:`Null ~criteria:[] ~worker:"b" ()
    in
...
let test_list_requests_missing_dir_stays_quiet () =
  with_temp_dir (fun base_path ->
    ...
    Alcotest.(check (float 0.1))
      "missing dir does not increment metric"
      before
      (persistence_counter Safe_ops.persistence_read_drop_reason_list_dir_error))
;;
...
let test_list_requests_ignores_legacy_root_entries () =
  with_temp_dir (fun base_path ->
    ...
    Alcotest.(check (float 0.1))
      "legacy root does not increment metric"
      before
      (persistence_counter Safe_ops.persistence_read_drop_reason_entry_load_error))
;;
...
let test_submit_verdict () =
  with_temp_dir (fun base_path ->
    ...
    | Ok req ->
      (match
         V.submit_verdict ~base_path ~req_id:req.id ~verifier:"codex" ~verdict:V.Pass
       with
       | Error e -> Alcotest.fail e
       | Ok updated ->
         Alcotest.(check bool)
...
let test_pending_for_agent () =
  with_temp_dir (fun base_path ->
    ...
    Alcotest.(check int)
      "claude sees 0 (own work filtered)"
      0
      (List.length pending_claude))
;;
...
let test_verdict_fail_to_attribution () =
  let attr =
    V.to_attribution ~origin:Det ~evidence:`Null (V.Fail "output does not match schema")
  in
  match attr.outcome with
...
let test_verdict_partial_to_attribution () =
  let attr =
    V.to_attribution ~origin:NonDet ~evidence:`Null (V.Partial (0.75, "partial match"))
  in
  match attr.outcome with
  ...
  with_temp_dir (fun base_path ->
    match
      V.create_request
        ...
        ~task_id:"t1"
        ~output:`Null
        ~criteria:[ V.Contains "x" ]
        ~worker:"w"
        ()
    with
    | Error e -> Alcotest.fail ("create failed: " ^ e)
    | Ok req ->
      Alcotest.(check bool) "None for Pending" true (V.attribution_of_request req = None))
;;
  ...
  Alcotest.run
    ...
    ; ( "attribution"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "Fail → Attribution.Policy_failed"
            `Quick
            test_verdict_fail_to_attribution
        ; Alcotest.test_case
            "Partial → Attribution.Partial_pass"
            `Quick
            test_verdict_partial_to_attribution
        ; Alcotest.test_case
            "attribution_of_request None for Pending"
            `Quick
            test_attribution_of_request_none_for_pending
        ; Alcotest.test_case
            "attribution_of_request derives origin"
            `Quick
