...
let test_nested_object () =
  ...
  match Judge_json_recovery.extract_balanced_object raw with
  | Some block ->
    check string "depth-2 object closes correctly" "{\"a\": {\"b\": 1}, \"c\": 2}" block
...
let test_braces_inside_string_ignored () =
  ...
  | Some block ->
    check
      string
      "brace inside string is content, not scope"
      "{\"text\": \"hello {world}\"}"
      block
  | None -> fail "expected Some"
;;
  ...
  run
    ...
    [ ( "extract_balanced_object"
      , [ test_case "no brace → None" `Quick test_no_brace_returns_none
        ; test_case "balanced object after prose" `Quick test_balanced_object_after_prose
        ; test_case "nested object" `Quick test_nested_object
        ; test_case
            "braces inside string are ignored"
            `Quick
            test_braces_inside_string_ignored
        ; test_case "escaped quote inside string" `Quick test_escaped_quote_inside_string
