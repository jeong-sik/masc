...
let pr_number_of_args args =
  ...
  if from_pr <> 0 then from_pr else Safe_ops.json_int ~default:0 "number" args
;;
...
let pr_not_found_in_output (s : string) : bool =
  ...
  List.exists
    (fun n ->
       ...
       let len_s = String.length lower
       and len_n = String.length nl in
       if len_n > len_s
       then false
       else (
         let rec scan i =
           if i + len_n > len_s
           then false
           else if String.sub lower i len_n = nl
           then true
           else scan (i + 1)
         in
         scan 0))
...
let handle_keeper_pr_review_read
  ...
  if pr_number = 0
  then error_json "pr_number is required. Good: pr_number=123."
  else (
    let root = Keeper_alerting_path.project_root_of_config config in
    let repo_flag =
      if repo <> "" then Printf.sprintf " -R %s" (Filename.quote repo) else ""
    in
    (* Get PR metadata *)
    let meta_cmd =
      Printf.sprintf
        "cd %s && gh pr view %d%s --json \
         title,body,state,files,reviews,comments,additions,deletions 2>&1"
        (Filename.quote root)
        pr_number
        repo_flag
    in
    ...
    if
      (not meta_ok) && (pr_not_found_in_output out_meta || pr_not_found_in_output out_diff)
    then
      Yojson.Safe.to_string
        (`Assoc
            ...
            ; ( "hint"
              , `String
                  "PR may have been closed/deleted or the number is wrong. Use \
                   keeper_pr_list (or `gh pr list`) to see open PRs before retrying." )
            ])
    else
      Yojson.Safe.to_string
        (`Assoc
...
let handle_keeper_pr_review_comment
  ...
  let repo = Safe_ops.json_string ~default:"" "repo" args |> String.trim in
  if pr_number = 0
  then error_json "pr_number is required."
  else if body = ""
  then error_json "body is required."
  else (
    ...
    | None ->
      error_json
        (Printf.sprintf
           ...
           (String.concat ", " valid_pr_review_event_strings)
           event_raw)
    | Some event ->
      ...
      then
        Yojson.Safe.to_string
          (`Assoc
              ...
              ; ( "reason"
                , `String
                    "keeper_pr_review_comment requires delivery, coding, or full preset" )
              ])
      else (
        let root = Keeper_alerting_path.project_root_of_config config in
        let repo_flag =
        ...
        Log.Keeper.info
          ...
          pr_number
          (pr_review_event_to_string event)
          meta.name
          (st = Unix.WEXITED 0);
        Yojson.Safe.to_string
          (`Assoc
...
let handle_keeper_pr_review_reply
  ...
  then error_json "comment_id is required."
  else if body = ""
  then error_json "body is required."
  else (
    let preset_ok =
    ...
    then
      Yojson.Safe.to_string
        (`Assoc
            ...
            ; "error", `String "preset_insufficient"
            ; ( "reason"
              , `String "keeper_pr_review_reply requires delivery, coding, or full preset"
              )
            ])
    else (
      let root = Keeper_alerting_path.project_root_of_config config in
        ...
        then repo
          ...
          let st, out =
            Process_eio.run_argv_with_status
              ...
              ; "-lc"
              ; Printf.sprintf
                  "cd %s && gh repo view --json nameWithOwner -q .nameWithOwner 2>&1"
                  (Filename.quote root)
              ]
          in
      ...
      in
      if owner_repo = ""
      then error_json "Could not determine repository. Provide repo parameter."
      else (
        let cmd =
          Printf.sprintf
        ...
        in
        Log.Keeper.info
          "pr_review_reply: pr=%d comment=%d keeper=%s ok=%b"
          pr_number
          comment_id
          meta.name
          (st = Unix.WEXITED 0);
