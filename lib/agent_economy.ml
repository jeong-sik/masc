...
let enabled () = Env_config_core.get_bool ~default:false "MASC_ECONOMY_ENABLED"
...
let reward_upvote () = Env_config_core.get_float ~default:0.5 "MASC_ECONOMY_REWARD_UPVOTE"
...
let ledger_path base_path = Filename.concat (economy_dir base_path) "ledger.jsonl"
...
let append_transaction base_path (txn : transaction) : (unit, string) result =
  ...
  | e ->
    Error
      (Printf.sprintf "[agent_economy] ledger write failed: %s" (Printexc.to_string e))
;;
...
let load_balances_from_ledger base_path =
  if with_economy_rw (fun () -> Hashtbl.mem loaded_paths base_path)
  then ()
  else (
    let path = ledger_path base_path in
    ...
    | Ok content ->
      ...
      |> List.filter_map (fun line ->
        ...
        then None
          ...
          try transaction_of_json (Yojson.Safe.from_string trimmed) with
          | Yojson.Json_error msg ->
            Log.Misc.warn "agent_economy: skipping malformed ledger entry: %s" msg;
            None)))
;;
...
let earn ~base_path ~agent_name ~kind ~reason ?reputation_score ?(metadata = `Null) () =
  if not (enabled ())
  then Ok (get_balance ~base_path ~agent_name)
  else (
    let base_amount = base_reward_for_kind kind in
    if base_amount <= 0.0
    then Error "[agent_economy] earn called with non-earning kind"
    else (
      let multiplier =
        if reputation_multiplier_enabled ()
        then (
...
let economic_pressure ~base_path ~agent_name =
  ...
  then Normal
    ...
    if balance < hustle_threshold ()
    then Hustle
    else if balance < frugal_threshold ()
    then Frugal
    else Normal)
;;
