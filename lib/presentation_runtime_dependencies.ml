...
type t = {base_path:string; parser:check; renderer:check}
let parser_probe =
...
let observe ~base_path () =
  {base_path; parser=observe_command [parser_python ~base_path; "-I"; "-B"; "-c"; parser_probe; environment_dir ~base_path];
   renderer=observe_command ["soffice"; "--headless"; "--version"]}
...
let available checks = parser_available checks && renderer_available checks
