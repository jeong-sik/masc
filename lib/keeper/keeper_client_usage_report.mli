...
type t =
  ...
  ; vendor_total_tokens : int option
        (** The client's own total, when it reports one. A Codex context
            overflow resets the thread count to zero input and output with
            the window size here, which is not the same as zero usage. *)
  }
