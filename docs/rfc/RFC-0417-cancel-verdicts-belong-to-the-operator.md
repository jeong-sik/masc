...
허락이므로 그 판정 권한은 운영자의 클릭 하나에만 있다. **2026-09-19 개정: 이 문장이
말하는 것은 남의 일을 없애는 허락이다. 자기가 맡은 일을 접는 것은 허락을 구하는 일이
아니라 그만두는 일이고, 맡은 쪽이 직접 한다(§9).** 이 문서는 그 권한 배분을
...
- RFC `task-lifecycle-a-verdict-assigns-no-work` — 이 문서의 2026-09-19 개정을
  낸 설계. 취소 자격을 상태가 들고 있는 이름으로 옮기고 승인 줄을 없앤다.

관련: #33046, task-1306, task-1303(사례), RFC-0407, RFC-0361,
`task-lifecycle-a-verdict-assigns-no-work`.
...
- **2026-09-19 운영자 결정: 자기 일을 접는 데 승인을 받지 않는다.** 이 문서는
  ...
  취소할 자격은 **그 상태가 이름을 들고 있는 에이전트**와 인증된 운영자에게 있다.
  맡고 있으면 맡은 쪽, 판정을 기다리거나 반려됐으면 낸 쪽, `Todo` 면 아무도 아니다.
  취소 요청이라는 대기 상태는 없어진다.

  §7 의 첫 거절은 그대로 선다. 판정 LLM 에이전트는 취소 권한을 받지 않고, 취소는
  판정을 아예 거치지 않는다. 세탁 경로는 닫힌 채다.

  설계와 근거: RFC `task-lifecycle-a-verdict-assigns-no-work` §3.5, §3.12.
