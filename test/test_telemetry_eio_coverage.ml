...
let test_event_agent_joined () =
  let e =
    Telemetry_eio.Agent_joined
      { agent_id = "claude-001"; capabilities = [ "code"; "review" ] }
  in
  match e with
...
let test_event_agent_left () =
  let e =
    Telemetry_eio.Agent_left { agent_id = "claude-001"; reason = "session ended" }
  in
  match e with
  | Telemetry_eio.Agent_left r -> check string "reason" "session ended" r.reason
  | _ -> fail "expected Agent_left"
;;

let test_event_task_started () =
  let e = Telemetry_eio.Task_started { task_id = "task-001"; agent_id = "claude-001" } in
  match e with
  | Telemetry_eio.Task_started r -> check string "task_id" "task-001" r.task_id
  | _ -> fail "expected Task_started"
;;

let test_event_task_completed () =
  let e =
    Telemetry_eio.Task_completed
      { task_id = "task-001"; duration_ms = 5000; success = true }
  in
  match e with
...
let test_event_handoff_triggered () =
  let e =
    Telemetry_eio.Handoff_triggered
      { from_agent = "claude-001"; to_agent = "codex-001"; reason = "context limit" }
  in
  match e with
...
let test_event_error_occurred () =
  let e =
    Telemetry_eio.Error_occurred
      { code = "E001"; message = "Something failed"; context = "test" }
  in
  match e with
...
let test_event_tool_called () =
  ...
  | Telemetry_eio.Tool_called r ->
    ...
    check (option string) "error_message" (Some "timed out after 30s") r.error_message
...
let test_event_record_type () =
  let r : Telemetry_eio.event_record =
    ...
    ; event = Telemetry_eio.Agent_joined { agent_id = "test"; capabilities = [] }
    }
  in
    ...
    (match record.event with
     | Telemetry_eio.Tool_called r ->
       ...
       check
         (option string)
         (label ^ " agent_id")
         (Some "keeper-masc-improver-agent")
         r.agent_id;
       check (option string) (label ^ " operation_id") operation_id r.operation_id;
       check (option string) (label ^ " worker_run_id") worker_run_id r.worker_run_id
     | _ -> fail (label ^ ": expected Tool_called"))
  | records ->
    fail
      (Printf.sprintf
         "%s: expected one parsed record, got %d"
         label
         (List.length records))
;;
...
let test_parse_event_records_tool_called_null_options () =
  ...
  in
  check_one_tool_called_record "null options" json ~operation_id:None ~worker_run_id:None
;;
  ...
  | [ record ] ->
    (match record.event with
     | Telemetry_eio.Tool_assigned r ->
       check string (label ^ " agent_id") "keeper-masc-improver-agent" r.agent_id;
...
let test_event_json_roundtrip () =
  let original =
    Telemetry_eio.Task_completed
      { task_id = "task-roundtrip"; duration_ms = 1234; success = true }
  in
...
let test_parse_event_records_drop_increments_counter () =
  ...
  let malformed =
    `Assoc
      [ "timestamp", `Float 1.0
      ; "event", `List [ `String "Unknown_variant"; `Assoc [ "x", `Int 1 ] ]
      ]
  in
...
let test_event_to_json_agent_joined () =
  let e = Telemetry_eio.Agent_joined { agent_id = "test"; capabilities = [ "a"; "b" ] } in
...
let test_event_to_json_task_completed () =
  let e =
    Telemetry_eio.Task_completed { task_id = "t1"; duration_ms = 100; success = true }
  in
  let json = Telemetry_eio.event_to_json e in
  let json_str = Yojson.Safe.to_string json in
  check bool "nonempty" true (String.length json_str > 0);
...
let test_count_tasks_in_progress_started_completed () =
  let events : Telemetry_eio.event_record list =
    [ { timestamp = 1.0; event = Task_started { task_id = "t1"; agent_id = "a1" } }
    ; { timestamp = 2.0
      ; event = Task_completed { task_id = "t1"; duration_ms = 100; success = true }
      }
    ]
  in
...
let test_count_completed_tasks_multiple () =
  let events : Telemetry_eio.event_record list =
    ...
    ; { timestamp = 2.0
      ; event = Task_completed { task_id = "t2"; duration_ms = 200; success = false }
      }
...
let test_avg_duration_one () =
  let events : Telemetry_eio.event_record list =
    [ { timestamp = 1.0
      ; event = Task_completed { task_id = "t1"; duration_ms = 1000; success = true }
      }
    ]
  in
  let avg = Telemetry_eio.avg_duration events in
...
let test_avg_duration_multiple () =
  let events : Telemetry_eio.event_record list =
    ...
    ; { timestamp = 2.0
      ; event = Task_completed { task_id = "t2"; duration_ms = 2000; success = true }
      }
    ]
  in
  let avg = Telemetry_eio.avg_duration events in
...
let test_calculate_handoff_rate_some_handoffs () =
  let events : Telemetry_eio.event_record list =
    ...
    ; { timestamp = 2.0
      ; event = Handoff_triggered { from_agent = "a1"; to_agent = "a2"; reason = "x" }
      }
    ]
  in
  let rate = Telemetry_eio.calculate_handoff_rate events in
...
let test_calculate_error_rate_some_errors () =
  let events : Telemetry_eio.event_record list =
    ...
    ; { timestamp = 2.0
      ; event = Error_occurred { code = "E1"; message = "err"; context = "ctx" }
      }
    ]
  in
  let rate = Telemetry_eio.calculate_error_rate events in
...
let () =
  run
    ...
    ; ( "count_tasks_in_progress"
      , [ test_case "empty" `Quick test_count_tasks_in_progress_empty
        ...
        ; test_case
            "started completed"
            `Quick
            test_count_tasks_in_progress_started_completed
        ] )
    ...
    ; ( "store_reads"
      , [ test_case
            "summarize_tool_usage reads date-split store"
            `Quick
