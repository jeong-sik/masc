...
val board_sort_order_of_request :
  Httpun.Request.t -> Masc_board_handlers.Board_dispatch.sort_order
(** [board_sort_order_of_request request] reads [sort_by] query
    param and resolves it through
    {!Masc_board_handlers.Board_dispatch.sort_order_of_string_opt}.  Missing or
    invalid values fall back to {!Masc_board_handlers.Board_dispatch.Hot} — graceful
    UI degradation, not silent data corruption. *)

val board_sort_label : Masc_board_handlers.Board_dispatch.sort_order -> string
(** Thin alias over {!Masc_board_handlers.Board_dispatch.sort_order_to_string}. *)
