...
let import_signed_in ~source_home home =
  ...
  (* Match the official CLI's keyring-first account selection. A stale fallback
     file must not override an account changed through official sign-in. *)
  let keychain = Filename.concat source_home "Library/Keychains/login.keychain-db" in
  let keychain_contents =
    if not (Sys.file_exists "/usr/bin/security" && Sys.file_exists keychain) then None
    else
      match Process_eio.run_argv_with_status_split_or_refusal
        ["/usr/bin/security"; "find-generic-password"; "-s"; "gemini"; "-a"; "antigravity"; "-w"; keychain] with
      | Ok (Unix.WEXITED 0, contents, _) when String.trim contents <> "" -> Some (String.trim contents)
      | Ok _ | Error _ -> None
  in
  let* contents = match keychain_contents, file with
    | Some contents, _ | None, Some contents -> Ok contents
    | None, None -> Error Sign_in_required  in
