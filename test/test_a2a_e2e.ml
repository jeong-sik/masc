...
let test_broadcast_delivery () =
  ...
  | Ok msg ->
    ...
    (match Coord_eio.get_message config ~seq:msg.seq with
     ...
     | Error e -> failf "Message retrieval failed: %s" e)
;;
...
let test_message_persistence () =
  ...
  | Error e -> failf "Persistent message retrieval failed: %s" e
;;
...
let test_mention_extraction () =
  ...
  match
    Coord_eio.broadcast config ~from_agent:"claude" ~content:"@gemini please review this"
  with
  | Ok msg -> check (option string) "mention extracted" (Some "gemini") msg.mention
  | Error e -> failf "Broadcast with mention failed: %s" e
;;
...
let test_concurrent_broadcasts () =
  ...
  (* All agents register *)
  List.iter (fun a -> ignore (Coord_eio.register_agent config ~name:a ())) agents;
...
let test_auto_subscribe () =
  ...
  let has_messages_sub =
    List.exists
      (fun (s : Subscriptions.subscription) -> s.resource = Subscriptions.Messages)
      subs
  in
...
let () =
  run
    ...
    ; "stress", [ test_case "concurrent broadcasts" `Slow test_concurrent_broadcasts ]
    ]
;;
