...
- The dashboard's Fusion settings panel edits presets through the typed config API: panel and judge seats, their system prompts, timeout, output token limit, first-pass judges and grouped panels, with a preset added, renamed or deleted. Each save carries the revision it read, so a file changed in between is refused as `configuration_changed` instead of overwritten (#37819).
