...
let reactive_signal_count (observation : Keeper_world_observation.world_observation) =
...
let belief_summary_of_snapshot
...
let current_intention_of_phase
      ~(phase : Fsm.phase)
  ...
  then Some "record_progress_evidence"
  else if has_text_reply
  then Some "publish_progress_update"
  else (
    match phase with
...
let should_overlay_ledger = function
  ...
  | Types.Protocol_violation_no_tools_no_social_headers -> true
  ...
  | Types.Failure_run_error -> false
;;

let overlay_ledger_state
      ~(observation : Keeper_world_observation.world_observation)
      ...
      (base_state : Types.social_state)
  =
  let previous_snapshot = Option.bind previous_state Fsm.snapshot_of_social_state in
...
let adapt_tool_only_turn
      (state : Types.social_state)
      (transition_reason : Types.transition_reason)
  =
  match transition_reason with
  | Types.Tool_only_visible_reply ->
    ( { state with speech_act = Types.Stay_silent; delivery_surface = Types.Silent }
    , Types.Tool_only_progress_ledger
    , true )
  | Types.Tool_only_stay_silent
  | Types.Tool_only_comment_board
...
let apply_to_result
  ...
  in
  let state =
    if should_overlay_ledger base_reason
    then
      overlay_ledger_state ~observation ~result ~previous_state ~has_text_reply base_state
  ...
  in
  let routed_result =
    if suppress_visible_text then { base_result with response_text = "" } else base_result
  in
