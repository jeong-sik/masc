...
let script_path () = Filename.concat (source_root ()) "start-masc-mcp.sh"
let loopback_script_path () = Filename.concat (source_root ()) "scripts/start-loopback.sh"
...
let run_shell ?(env = []) ~cwd cmd =
  let env =
    if List.mem_assoc "MASC_ALLOW_PORT_REUSE" env
    then env
  ...
  in
  let full = Printf.sprintf "cd %s && %s" (quote cwd) shell_cmd in
...
let make_fake_stdio_eio_exe repo_root =
  let exe_path = Filename.concat repo_root "_build/default/bin/main_stdio_eio.exe" in
...
let test_explicit_env_overrides_repo_env_files () =
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    write_file (Filename.concat dir ".env.local") "MASC_STORAGE_TYPE=memory\n";
    ...
    let code, stdout, stderr =
      run_shell
        ...
        (Printf.sprintf "%s --http --port 9955 --base-path %s" (quote script) (quote dir))
    in
    if code <> 0
    then failf "start script failed (%d)\nstdout:\n%s\nstderr:\n%s" code stdout stderr;
    let captured = read_file capture in
    check
      bool
      "explicit env wins over env file"
      true
      (contains_substring captured "MASC_STORAGE_TYPE=filesystem");
    check
      bool
      "base path passed through"
      true
      (contains_substring captured ("MASC_BASE_PATH=" ^ canonical_path dir));
    check
      bool
...
let test_realtime_transports_default_to_base_path_config_and_preserve_override () =
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    let bootstrapped_config = Filename.concat (canonical_path dir) ".masc/config" in
    ...
    let code_default, stdout_default, stderr_default =
      run_shell
        ...
        (Printf.sprintf "%s --http --port 9956 --base-path %s" (quote script) (quote dir))
    in
    if code_default <> 0
    then
      failf
        "start script failed (%d)\nstdout:\n%s\nstderr:\n%s"
        code_default
        stdout_default
        stderr_default;
    ...
    check
      bool
      "config dir defaults to base path config"
      true
      (contains_substring captured_default ("MASC_CONFIG_DIR=" ^ bootstrapped_config));
    check
      bool
      "base path config bootstrapped"
      true
    ...
    let code_override, stdout_override, stderr_override =
      run_shell
        ...
        (Printf.sprintf "%s --http --port 9957 --base-path %s" (quote script) (quote dir))
    in
    if code_override <> 0
    then
      failf
  ...
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    let code, stdout, stderr =
      run_shell
        ...
        (Printf.sprintf "%s --http --port 9959 --base-path %s" (quote script) (quote dir))
    in
    if code <> 0
    then failf "start script failed (%d)\nstdout:\n%s\nstderr:\n%s" code stdout stderr;
    let captured = read_file capture in
    check
      bool
      "defaults to base path config"
      true
      (contains_substring captured ("MASC_CONFIG_DIR=" ^ bootstrapped_config));
    check
      bool
      "repo config copied to base path config"
      true
      (Sys.file_exists (Filename.concat bootstrapped_config "cascade.json")))
;;
...
let test_default_base_path_falls_back_to_home_when_unset () =
  with_temp_dir "start-masc-script-home-fallback" (fun dir ->
    ...
    check
      bool
      "default config root follows home fallback"
      true
      (contains_substring
         captured
  ...
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    let code, stdout, stderr =
      run_shell
        ~cwd:dir
        ~env:
          [ "FAKE_CAPTURE_FILE", capture; "HOME", home_dir; "MASC_BASE_PATH", stale_root ]
    ...
    check
      bool
      "absolute env base path preserved"
      true
...
let test_absolute_parent_project_base_path_is_preserved () =
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    let code, stdout, stderr =
      run_shell
        ~cwd:repo
        ~env:[ "FAKE_CAPTURE_FILE", capture; "HOME", home_dir; "MASC_BASE_PATH", parent ]
    ...
    check
      bool
      "absolute parent root inheritance preserved"
      true
      (contains_substring captured ("MASC_BASE_PATH=" ^ expected_root)))
;;
...
let test_cli_sidecar_root_is_exported () =
  with_temp_dir "start-masc-script-sidecar-root" (fun dir ->
    ...
    let code, stdout, stderr =
      run_shell
        ~cwd:dir
        ~env:[ "FAKE_CAPTURE_FILE", capture; "MASC_BASE_PATH", base_path ]
        (Printf.sprintf
           "%s --http --port 9970 --base-path %s --sidecar-root %s"
           (quote script)
           (quote base_path)
           (quote sidecar_root))
    in
    ...
    check
      bool
      "sidecar root exported to server env"
      true
      (contains_substring captured ("MASC_SIDECAR_ROOT=" ^ canonical_path sidecar_root)))
;;
...
let test_zshenv_absolute_base_path_is_preserved () =
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    mkdir_p (Filename.concat repo Common.masc_dirname);
    write_file
      (Filename.concat home_dir ".zshenv")
    ...
    let code, stdout, stderr =
      run_shell
        ~cwd:repo
        ~env:[ "FAKE_CAPTURE_FILE", capture; "HOME", home_dir ]
    ...
    check
      bool
      "zshenv absolute base path preserved"
      true
      (contains_substring captured ("MASC_BASE_PATH=" ^ expected_root)))
;;
...
let test_shared_root_env_base_path_is_preserved () =
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    check
      bool
      "shared-root env base path preserved"
      true
      (contains_substring captured ("MASC_BASE_PATH=" ^ expected_root)))
;;
...
let test_worktree_prefers_local_build_over_workspace_build () =
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    copy_script (script_path ()) script;
    let local_exe = Filename.concat worktree "_build/default/bin/main_eio.exe" in
    let workspace_exe =
    ...
    let code, stdout, stderr =
      run_shell
        ~cwd:worktree
        ~env:[ "FAKE_CAPTURE_FILE", capture; "MASC_BASE_PATH", repo_root ]
        (Printf.sprintf
           "%s --http --port 9958 --base-path %s"
           (quote script)
           (quote repo_root))
    in
    if code <> 0
...
let test_explicit_base_path_execs_from_base_path () =
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    check
      bool
      "exec cwd matches explicit base path"
      true
...
let test_explicit_base_path_ignores_repo_local_config_from_zshenv () =
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    check
      bool
      "explicit base path resets config root to base path"
      true
      (contains_substring
         captured
    ...
    check
      bool
      "stderr explains repo-local config ignore"
      true
      (contains_substring stderr "Ignoring repo-local MASC_CONFIG_DIR");
    check
      bool
      "stderr explains repo-local personas ignore"
      true
...
let test_zshenv_retired_pg_envs_are_scrubbed_before_exec () =
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    check
      bool
      "filesystem storage is reasserted after env file load"
      true
      (contains_substring captured "MASC_STORAGE_TYPE=filesystem");
    check
      bool
      "SUPABASE_DB_URL scrubbed before exec"
      true
      (contains_substring captured "SUPABASE_DB_URL=");
    check
      bool
      "SB_PG_URL scrubbed before exec"
      true
      (contains_substring captured "SB_PG_URL=");
    check
      bool
      "MASC_POSTGRES_URL remains scrubbed before exec"
      true
      (contains_substring captured "MASC_POSTGRES_URL=");
    check
      bool
      "DATABASE_URL remains scrubbed before exec"
      true
...
let test_explicit_base_path_ignores_repo_local_config_from_parent_env () =
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    check
      bool
      "parent env repo-local config ignored under external base path"
      true
      (contains_substring
         captured
         ("MASC_CONFIG_DIR=" ^ Filename.concat expected_parent ".masc/config"));
    check
      bool
      "parent env repo-local config ignore is logged"
      true
      (contains_substring stderr "Ignoring repo-local MASC_CONFIG_DIR"))
;;
  ...
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    let code, stdout, stderr =
      run_shell
        ...
        (Printf.sprintf "%s --http --port 9951 --base-path %s" (quote script) (quote dir))
    in
    if code <> 0
    then failf "start script failed (%d)\nstdout:\n%s\nstderr:\n%s" code stdout stderr;
    let captured = read_file capture in
    check
      bool
      "grpc port derived from explicit http port"
      true
      (contains_substring captured "MASC_GRPC_PORT=9952");
    check
      bool
      "ws port derived from explicit http port"
      true
...
let test_grpc_direct_banner_is_preserved_in_stderr () =
  with_temp_dir "start-masc-script" (fun dir ->
    ...
    check
      bool
      "grpc-direct tls banner preserved"
      true
      (contains_substring stderr "Running without TLS (plaintext h2c)");
    check
      bool
      "grpc-direct server banner preserved"
      true
      (contains_substring stderr "gRPC server on 127.0.0.1:9952");
    check bool "other stderr preserved" true (contains_substring stderr "stderr-keep"))
;;
...
|};
    ...
    let code, stdout, stderr =
      run_shell
        ...
        (Printf.sprintf "%s --stdio --base-path %s" (quote script) (quote dir))
    in
    if code <> 0
    then failf "start script failed (%d)\nstdout:\n%s\nstderr:\n%s" code stdout stderr;
    let captured = read_file capture in
    check
      bool
      "stdio executable selected without HTTP build"
      true
      (contains_substring captured "FAKE_EXE_MARKER=stdio");
    check
      bool
      "dashboard helper not invoked in stdio mode"
      false
      (Sys.file_exists dashboard_marker);
    check
      bool
      "stderr explains stdio dashboard skip"
      true
...
let test_loopback_disables_keeper_autoboot_by_default_and_preserves_override () =
  with_temp_dir "start-loopback-script" (fun dir ->
    ...
    write_file
      (Filename.concat repo_root ".env.local")
    ...
    let code_default, stdout_default, stderr_default =
      run_shell
        ...
        (Printf.sprintf
           "%s --port 9961 --base-path %s"
           (quote loopback_script)
           (quote repo_root))
    in
    if code_default <> 0
    then
      failf
        "loopback script failed (%d)\nstdout:\n%s\nstderr:\n%s"
        code_default
        stdout_default
        stderr_default;
    let captured_default = read_file capture_default in
    check
      bool
      "loopback disables keeper autoboot by default"
      true
      (contains_substring captured_default "MASC_KEEPER_BOOTSTRAP_ENABLED=false");
    let capture_override = Filename.concat dir "captured-loopback-override.txt" in
    let code_override, stdout_override, stderr_override =
      run_shell
        ~cwd:repo_root
        ~env:
    ...
    then
      failf
        "loopback script override failed (%d)\nstdout:\n%s\nstderr:\n%s"
        code_override
        stdout_override
        stderr_override;
    let captured_override = read_file capture_override in
    check
      bool
      "loopback preserves explicit keeper autoboot override"
      true
...
let () =
  run
    "start_masc_mcp_script"
    [ ( "script"
      , [ test_case
            "explicit env overrides repo env files"
            `Quick
        ...
        ; test_case
            "bootstraps base path config from repo when unset"
            `Quick
        ...
        ; test_case
            "CLI sidecar root is exported"
            `Quick
            test_cli_sidecar_root_is_exported
        ; test_case
            "explicit base path execs from base path"
            `Quick
        ...
        ; test_case
            "explicit http port derives sidecar ports"
            `Quick
            test_explicit_http_port_derives_sidecar_ports
        ; test_case
            "grpc-direct banner is preserved in stderr"
            `Quick
            test_grpc_direct_banner_is_preserved_in_stderr
        ; test_case
            "stdio skips dashboard build and HTTP preflight"
            `Quick
            test_stdio_skips_dashboard_build_and_http_preflight
        ; test_case
            "zshenv absolute base path is preserved"
            `Quick
            test_zshenv_absolute_base_path_is_preserved
        ; test_case
            "shared-root env base path is preserved"
            `Quick
            test_shared_root_env_base_path_is_preserved
        ; test_case
            "worktree prefers local build over workspace build"
            `Quick
