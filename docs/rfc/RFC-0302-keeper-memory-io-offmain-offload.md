...
### 4.1 1축 — 순수 blocking I/O를 오프로드 (primary)

> **§0 정정 반영**: 아래는 v1 초안이 정찰한 기존 선례(`auth_credential_base`)와 그 메커니즘(`Eio_guard.run_in_systhread`)이다. **실제 채택 메커니즘은 `Domain_pool_ref.submit_io_or_inline`**(§0) — 이유는 (a) #22824가 이미 이 메커니즘으로 read 경로를 오프로드해 선례가 갱신됐고, (b) `run_in_systhread` 안에서 다시 `run_in_systhread`를 중첩하면 handler 없는 스레드에서 `Effect.Unhandled` 위험이 있기 때문. 아래 코드 인용은 "왜 systhread 계열 오프로드가 안전한가"의 근거(대칭적 fallback 구조)로만 남기고, 실제 구현은 `Domain_pool_ref.submit_io_or_inline`을 직접 사용한다.

**기존 선례**(`lib/auth/auth_credential_base.ml`이 동일 문제를 이렇게 푼다; 참고로 `lib/auth_credential_base.ml`은 별도의 구식 사본이라 인용 시 디렉터리까지 명시) —
...
`Eio_guard.run_in_systhread`(`lib/core/eio_guard.ml:55`)는 Eio 활성 시 `Eio_unix.run_in_systhread`로 body를 시스템 스레드에서 실행(→ main 스케줄러 비점유, 다른 fiber 진행), 비활성(module-init/테스트) 시 직접 실행 — `Domain_pool_ref.submit_io_or_inline`도 풀 미설치 시 동일하게 inline fallback한다(§0). keeper_memory_os_io는 이 대칭 fallback 구조를 유지한 채 `Domain_pool_ref.submit_io_or_inline`으로 **순수-blocking file I/O를 감싼다**.
