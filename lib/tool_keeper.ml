...
let annotate_keeper_json ~runtime_class json =
  match json with
  | `Assoc fields -> `Assoc (("runtime_class", `String runtime_class) :: fields)
  | other -> other
;;
    ...
    (match maybe_reseed_keeper_identity ctx meta with
     | Ok (updated_meta, identity_reseed) ->
       let prepared_args =
         match args with
         | `Assoc fields ->
           `Assoc (("name", `String updated_meta.name) :: List.remove_assoc "name" fields)
         | other -> other
       in
...
let with_keeper_startup_gate f =
  if not Server_startup_state.(!state.state_ready)
  then (
...
let execute_keeper_up ctx args : tool_result =
  ...
  | Ok (prepared_args, identity_reseed) ->
    ...
    then ok, body
      ...
      ( true
      , Yojson.Safe.pretty_to_string (annotate_keeper_json ~runtime_class:"keeper" json) ))
;;
  ...
  let rec find_latest = function
    ...
    | line :: tl ->
      (try
         ...
         | _ -> find_latest tl
       with
       | Yojson.Json_error _ | Yojson.Safe.Util.Type_error _ -> find_latest tl)
  in
  ...
  | Ok (Some (meta : keeper_meta)) ->
    ...
    let diagnostic =
      ...
      |> Keeper_exec_status.augment_keeper_diagnostic_json
           ...
           ~now_ts
    in
    let status = Keeper_exec_status.keeper_surface_status ~agent_status ~diagnostic in
    Some
      (`Assoc
...
let invalidate_status_cache name = Keeper_status_detail.invalidate_status_cache_for name

let with_keeper_name args name =
  match args with
  | `Assoc fields -> `Assoc (("name", `String name) :: List.remove_assoc "name" fields)
  | other -> other
;;
      ...
      then (
        ...
        | Ok _ ->
          ...
          then false, body
            ...
            match
              Keeper_exec_persona.persist_keeper_toml_from_resolved_args resolved_args
            with
            | Error e -> false, "❌ keeper created but durable config write failed: " ^ e
            | Ok durable_config ->
              let created_json =
                try Yojson.Safe.from_string body with
                | Yojson.Json_error _ -> `String body
...
let resolve_keeper_name ctx args =
  ...
  | Ok None ->
    (match keeper_name_from_agent_name name with
     | Some stripped ->
       Error (Printf.sprintf "❌ keeper not found: %s (also tried %s)" name stripped)
...
let handle_keeper_msg ctx args : tool_result =
  ...
  | Ok name ->
    ...
    let request_id =
      Keeper_msg_async.submit ~sw:ctx.sw ~keeper_name:name ~f:(fun () ->
...
let handle_keeper_msg_result _ctx args : tool_result =
  ...
  if request_id = ""
  then false, {|{"error":"request_id is required"}|}
  else (
    match Keeper_msg_async.poll request_id with
    | None ->
      ( false
      , Printf.sprintf {|{"error":"request_id not found","request_id":"%s"}|} request_id )
    | Some entry -> true, Yojson.Safe.to_string (Keeper_msg_async.entry_to_json entry))
;;
...
let is_safe_subpath ~parent ~child =
  ...
  then true
    ...
    let parent_with_sep =
      if Filename.check_suffix parent Filename.dir_sep
      then parent
      else parent ^ Filename.dir_sep
    in
    let plen = String.length parent_with_sep in
    String.length child >= plen && String.sub child 0 plen = parent_with_sep)
;;
...
let validate_target_file ~working_dir ~target_file =
  ...
  | None -> Ok None
  | Some tf ->
    if not (Filename.is_relative tf)
    then Error "target_file must be a relative path"
    else (
      ...
      in
      if is_safe_subpath ~parent:working_dir ~child:resolved
      then Ok (Some tf)
      else Error "target_file must reside within working_dir")
;;

let resolve_playground_working_dir ~agent_name ~base_path ~working_dir_arg =
  let playground_rel = Keeper_alerting_path.playground_path_of_keeper agent_name in
  ...
  match
    ...
    | Unix.Unix_error _ ->
      Error
        (Printf.sprintf
           "keeper playground directory %S does not exist yet — cannot validate \
            working_dir containment. Run masc_worktree_create to provision your \
  ...
  | Ok playground_abs ->
    let effective_arg =
      if String.trim working_dir_arg = "" then playground_abs else working_dir_arg
    in
    let resolved =
      try Ok (Unix.realpath effective_arg) with
      | Unix.Unix_error _ -> Error "working_dir does not exist or is not accessible"
    in
    (match resolved with
     | Error msg -> Error msg
     | Ok working_dir ->
       if is_safe_subpath ~parent:playground_abs ~child:working_dir
       then Ok working_dir
       else
         Error
           (Printf.sprintf
              "working_dir must be inside your own keeper playground (%s). Cross-keeper \
               repair loops are blocked — use masc_worktree_create to provision a \
               workspace under your playground first. See #6527/#6641."
              playground_rel))
;;
  ...
  | Ok meta ->
    (match maybe_reseed_keeper_identity ctx meta with
     ...
     | Ok (meta, identity_reseed) ->
       ...
       if String.trim task_spec = ""
       then false, "task_spec is required"
       else (
         ...
         | Error msg -> false, msg
         | Ok working_dir ->
           (match validate_target_file ~working_dir ~target_file:target_file_opt with
            | Error msg -> false, msg
            | Ok validated_target_file ->
              ...
              ok, annotate_keeper_repair_json ?identity_reseed ~keeper_name:meta.name body)))
;;
...
let handle_keeper_list ctx args : tool_result =
  ...
  let cache_key = Printf.sprintf "%s:%d:%b" ctx.config.base_path limit detailed in
  let body =
    cached_text_by_key
      _keeper_list_cache
      ~key:cache_key
      ~ttl_s:(keeper_list_cache_ttl_s ())
      (fun () ->
         let entries =
           Keeper_registry.all ~base_path:ctx.config.base_path ()
           |> List.sort
                (fun
                    (a : Keeper_registry.registry_entry)
                     (b : Keeper_registry.registry_entry)
                   -> String.compare a.name b.name)
           |> take limit
         in
         let names =
           List.map (fun (e : Keeper_registry.registry_entry) -> e.name) entries
         in
         let rows =
           entries
         ...
         let json =
           ...
           else `Assoc [ "count", `Int (List.length rows); "keepers", `List rows ]
         in
...
let keeper_persona_audit_item ctx requested_name =
  ...
  let persona_name =
    ...
    | None ->
      (match default_source_kind, inferred_persona_profile_path with
       | Some "persona", _ when inferred_persona_name <> "" -> Some inferred_persona_name
  ...
  in
  let keepalive_running =
    runtime_meta |> Option.map (Keeper_status_bridge.runtime_keepalive_running ctx.config)
  in
  let keepalive_started_at =
    ...
    | None -> None
  in
  let runtime_status = runtime_meta |> Option.map (keeper_persona_audit_status ctx) in
  ...
  let issues =
    ...
    |> add (persona_expected && not persona_profile_exists) "missing_persona_profile"
    |> add (not live_meta_exists) "missing_runtime_meta"
    |> add (Option.is_some runtime_meta_error) "runtime_meta_error"
    |> add
         (Option.is_some runtime_meta && Option.is_none registry_entry)
         "registry_missing"
    |> add
  ...
  `Assoc
    ...
    ; ( "requested_name"
      , if String.equal requested_name resolved_name
        then `Null
...
let handle_keeper_persona_audit ctx args : tool_result =
  ...
  then
    ...
    let audited_items = names |> take limit |> List.map (keeper_persona_audit_item ctx) in
    let returned_items =
      if include_ok
      then audited_items
      else
    ...
    let roots =
      `Assoc
        ...
        ; ( "personas_dirs"
          , `List
              (List.map (fun path -> `String path) (Config_dir_resolver.personas_dirs ()))
          )
        ]
    in
    ( true
    ...
    (match prepare_passive_keeper_identity ctx args with
     ...
     | Ok (prepared_args, identity_reseed) ->
       (match resolve_keeper_meta ctx prepared_args with
        ...
        | Ok meta ->
          let json =
            `Assoc [ "keeper", `String meta.name; "sandbox", render_item meta ]
            |> attach_identity_reseed ?identity_reseed
          in
          true, Yojson.Safe.pretty_to_string json))
;;
...
let handle_keeper_sandbox_start ctx args : tool_result =
  ...
  | Ok meta ->
    ...
    let network_mode_raw =
      String.trim
        (get_string args "network_mode" (network_mode_to_string meta.network_mode))
    in
...
let handle_keeper_sandbox_stop ctx args : tool_result =
  ...
  let stale_cleanup =
    ...
    then
      Some
        (Keeper_sandbox_control.cleanup_stale
           ~config:ctx.config
  ...
  let stale_json =
    ...
    | Some cleanup ->
      `Assoc
        ...
        ; "errors", `List (List.map (fun err -> `String err) cleanup.errors)
        ]
  in
  ( true
  , Yojson.Safe.pretty_to_string
...
let should_bootstrap_existing_keepalives name args =
  match name with
  | "masc_keeper_msg" -> String.trim (get_string args "message" "") <> ""
  | _ -> false
;;
...
let maybe_bootstrap_existing_keepalives ctx ~name ~args =
  ...
  then (
    try start_existing_keepalives ctx with
...
let resolve_primary_max_context (meta : Keeper_types.keeper_meta option) : int =
  ...
  | None -> min_ctx
  | Some meta ->
    let resolution = Keeper_exec_context.resolve_max_context_resolution_of_meta meta in
    ...
    (match Keeper_registry.get ~base_path:ctx.config.base_path name with
     | None ->
       Prometheus.inc_counter
         Prometheus.metric_keeper_operator_compact
     ...
     | Some entry ->
       ...
       let allowed =
         ...
         | Offline | Stopped | Dead | Crashed | Restarting | HandingOff | Draining ->
           false
       in
       if not allowed
       then (
         ...
         error_result_typed
           ~code:Validation_error
           (Printf.sprintf
              "keeper %s is in phase %s; compaction requires Overflowed, Paused, or \
               force=true"
              name
              phase_before))
       else (
              ...
              , Yojson.Safe.to_string
                  (`Assoc
                      ...
                      ; ( "phase_after"
                        , `String
                            (match
                               Keeper_registry.get ~base_path:ctx.config.base_path name
                             with
                             | Some entry ->
                               Keeper_state_machine.phase_to_string entry.phase
                             | None -> "unknown") )
            ...
            | None ->
             ...
             signal. *)
              Keeper_exec_context.dispatch_keeper_phase_event
                ~config:ctx.config
                ~keeper_name:name
                (Keeper_state_machine.Compaction_failed { reason = "no_valid_checkpoint" });
              Prometheus.inc_counter
                Prometheus.metric_keeper_operator_compact
              ...
              ( false
              , Printf.sprintf
                  "keeper %s: checkpoint compaction unavailable (no valid checkpoint \
                   found)"
                  name ))))
;;
  ...
  | Ok name ->
    ...
    then
      ...
      | Some entry ->
          ...
          | Some wctx ->
            ...
            let cleared_messages =
              ...
              then
                ...
                List.filter
                  (fun (m : Oas.Types.message) -> m.role = Llm_provider.Types.System)
                  existing_messages
              else []
            in
            let cleared_ctx =
            ...
            (match meta_for_trace with
             | Some meta ->
               ...
               (match
                ...
                | Ok _ -> ()
                | Error err ->
                  Log.Keeper.warn "%s: failed to save cleared OAS checkpoint: %s" name err)
             | None -> ());
            msg_count - List.length cleared_messages
        in
        let legacy_shadow_removed_count =
          let files =
            Keeper_checkpoint_store.list_checkpoints ~session_dir:session.session_dir
          in
          List.fold_left
            (fun count filename ->
          ...
          | Some meta ->
            let updated_meta =
              { meta with
                continuity_summary = ""
              ; updated_at = Keeper_types.now_iso ()
              ; runtime = { meta.runtime with last_continuity_update_ts = 0.0 }
              }
            in
            (match Keeper_types.write_meta ~force:true ctx.config updated_meta with
             | Ok () -> true
        ...
        invalidate_status_cache name;
        Log.Keeper.warn
          "%s: context cleared by operator (reason=%s, preserve_system=%b, cleared=%d \
           msgs)"
          name
          reason
          preserve_system
          cleared_count;
        Prometheus.inc_counter
          Prometheus.metric_keeper_operator_clear
          ~labels:[ "keeper", name; "preserve_system", string_of_bool preserve_system ]
          ();
        ( true
        , Yojson.Safe.to_string
            (`Assoc
...
let dispatch_stream ~on_text_delta ctx ~name ~args : tool_result option =
  ...
  let ctx = resolve_ctx ctx ~name args in
  match name with
  | "masc_keeper_msg" -> Some (handle_keeper_msg_stream ~on_text_delta ctx args)
  | _ -> None
;;
...
let tool_required_permission = function
  ...
  | "masc_keeper_status"
  | "masc_keeper_persona_audit"
  | "masc_keeper_sandbox_status" -> Some Types.CanReadState
  ...
  | "masc_keeper_reset"
  | "masc_keeper_compact"
  | "masc_keeper_clear" -> Some Types.CanBroadcast
  | _ -> None
;;
