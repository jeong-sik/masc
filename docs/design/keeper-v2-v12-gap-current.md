...
- The Connectors page now has the prototype's primary grid and audit framing, but exact drawer binding editing parity is still incomplete: the current drawer delegates to the existing config form instead of the prototype's inline channel-direction editor.
