...
- `coding_task` stage list는 문서상 progress label set이며 gate authority가 아니다.
...
- accept / reject / hard gate는 `contract_verdict`만 사용한다. Progress labels do not gate.
...
- MASC supervision / explicit contract gate
...
- progress label이 OAS 내부 workflow policy나 review gate로 승격되는 변경
...
### PoC-2. MASC Risk-Aware Supervisor Digest + coding_task Progress Labels
...
그리고 `coding_task`의 advisory progress labels:

```text
decompose -> inspect -> implement -> verify -> review
```

를 dashboard/reporting metadata로 노출한다. Hard gating은 progress label이 아니라 explicit contract/checkpoint requirement에서만 나온다.
...
모든 label 전이를 gate라고 부르지 않는다. 1차 분류는 다음과 같다.

- `decompose -> inspect`: advisory checkpoint
  - task statement, contract, intended read scope artifact 존재
- `inspect -> implement`: advisory checkpoint
  - inspected file manifest, intended edit scope, protected-path check 완료
- `implement -> verify`: advisory checkpoint
  - diff artifact, changed file manifest, declared verification plan 존재
- `verify -> review`: advisory checkpoint
  - verification artifact와 declared verification command 결과는 review input이다
  - 누락된 verification은 review를 금지하지 않고, reviewer가 볼 수 있는 explicit gap으로 남긴다
  - allowlist 없이 non-zero exit를 무시하는 것은 금지하지만, 이는 review blocker가 아니라 contract/checkpoint verdict 입력이다
...
- MASC digest field 초안과 progress label projection을 추가한다.
...
- `coding_task` 외 workload에도 동일한 progress label projection을 적용할 것인가?
