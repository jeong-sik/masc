...
let test_roundtrip_user_msg () =
  ...
  | Some oas ->
    ...
    Alcotest.(check string)
      "content preserved"
      "hello world"
      (Agent_sdk.Types.text_of_message rt)
;;
...
let test_roundtrip_system_msg_dropped () =
  ...
  in
  Alcotest.(check bool)
    "system message dropped (belongs in system_prompt)"
    true
...
let test_roundtrip_tool_msg () =
  ...
  | Some oas ->
    ...
    Alcotest.(check bool) "content preserved" true (String.length text > 0)
;;
...
let compact_ctx (ctx : Keeper_exec_context.working_context) strategies =
  let messages =
    ...
    Context_compact_oas.compact ~messages:(ctx_messages ctx) ~strategies ()
  in
  Keeper_exec_context.sync_oas_context
    { ctx with
      checkpoint = { (Keeper_exec_context.checkpoint_of_context ctx) with messages }
    }
;;
...
let test_compact_prune_tool_outputs () =
  ...
  Alcotest.(check bool)
    "messages preserved"
    true
    (List.length (ctx_messages compacted) > 0);
  Alcotest.(check bool)
    "token count positive"
    true
    (Keeper_exec_context.token_count compacted > 0);
  (* Short tool output (< 1500 chars) should not be truncated *)
  Alcotest.(check int)
    "message count unchanged for short tool output"
...
let test_compact_merge_contiguous () =
  ...
  Alcotest.(check bool)
    "merged reduces count"
    true
    (List.length (ctx_messages compacted) <= List.length msgs)
;;
...
let test_compact_summarize_old () =
  ...
  let msgs =
    List.init 12 (fun i ->
      if i mod 2 = 0
      then Agent_sdk.Types.user_msg (Printf.sprintf "user message %d with content" i)
      else Agent_sdk.Types.assistant_msg (Printf.sprintf "assistant response %d" i))
  in
  let ctx = List.fold_left Keeper_exec_context.append ctx msgs in
  ...
  Alcotest.(check bool)
    "summarize_old reduces messages"
    true
    (List.length (ctx_messages compacted) < List.length msgs);
  Alcotest.(check bool)
    "token count positive"
...
let test_compact_small_list_unchanged () =
  let ctx = Keeper_exec_context.create ~system_prompt:"test" ~max_tokens:4000 in
  let msgs =
    [ Agent_sdk.Types.user_msg "hello"; Agent_sdk.Types.assistant_msg "world" ]
  in
  let ctx = List.fold_left Keeper_exec_context.append ctx msgs in
  let compacted = compact_ctx ctx [ Context_compact_oas.SummarizeOld ] in
  (* Small list (< first_n + last_n = 7) should be unchanged *)
  Alcotest.(check int)
    "small list unchanged"
    (List.length msgs)
    (List.length (ctx_messages compacted))
;;
...
let () =
  Alcotest.run
    "OAS Adapters"
    [ ( "message_roundtrip"
      , [ Alcotest.test_case "user msg roundtrip" `Quick test_roundtrip_user_msg
        ; Alcotest.test_case "assistant msg roundtrip" `Quick test_roundtrip_assistant_msg
        ; Alcotest.test_case "system msg dropped" `Quick test_roundtrip_system_msg_dropped
        ; Alcotest.test_case "tool msg roundtrip" `Quick test_roundtrip_tool_msg
        ] )
    ; ( "context_compact"
      , [ Alcotest.test_case "PruneToolOutputs" `Quick test_compact_prune_tool_outputs
        ; Alcotest.test_case "MergeContiguous" `Quick test_compact_merge_contiguous
        ; Alcotest.test_case
            "SummarizeOld (keep first+last)"
            `Quick
            test_compact_summarize_old
        ; Alcotest.test_case
            "SummarizeOld small list unchanged"
            `Quick
            test_compact_small_list_unchanged
        ] )
    ; ( "message_restore"
      , [ Alcotest.test_case
            "restore messages all roles"
            `Quick
