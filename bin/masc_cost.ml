    ...
    Fun.protect
      ...
      (fun () ->
         let rec read_lines acc =
           ...
           | line ->
             let acc' =
               match parse_cost_line line with
               | Some e -> e :: acc
               | None -> acc
             in
             read_lines acc'
...
let filter_by_agent agent entries =
  if agent = "" then entries else List.filter (fun e -> e.agent = agent) entries
;;
  ...
  List.iter
    (fun e ->
       let tokens, cost, _model =
         try Hashtbl.find tbl e.agent with
         | Not_found -> 0, 0.0, e.model
       in
       Hashtbl.replace
         tbl
         e.agent
    ...
    entries;
  Hashtbl.fold
    (fun agent (tokens, cost, model) acc -> (agent, tokens, cost, model) :: acc)
    tbl
    []
;;
...
let print_report ~agent ~task_id ~period =
  ...
  let filtered =
    entries |> filter_by_period period |> filter_by_agent agent |> filter_by_task task_id
  in
  ...
  then (
    ...
    exit 0);
  let total_tokens =
    List.fold_left (fun acc e -> acc + e.input_tokens + e.output_tokens) 0 filtered
  in
...
let print_json_report ~agent ~task_id ~period =
  ...
  let agents_json =
    String.concat
      ", "
      (List.map
         (fun (agent, tokens, cost, model) ->
            sprintf
              {|{"agent":"%s","tokens":%d,"cost_usd":%.4f,"model":"%s"}|}
              agent
              tokens
              cost
              model)
  ...
  in
  printf
    {|{"period":"%s","total_entries":%d,"total_cost_usd":%.4f,"by_agent":[%s]}|}
    period
    (List.length filtered)
    total_cost
    agents_json;
...
let () =
  ...
  | Report ->
    let task = !task_id in
    if !json_output
    then print_json_report ~agent:!agent ~task_id:task ~period:!period
    else print_report ~agent:!agent ~task_id:task ~period:!period
;;
