...
let store_a_blob store payload =
  ...
  | O.Stored { sha256; bytes; preview; mime } ->
    let marker = O.encode_for_oas (O.Stored { sha256; bytes; preview; mime }) in
    sha256, marker
...
let test_hydrates_recent_marker () =
  with_temp_dir (fun dir ->
    ...
    Alcotest.(check string) "hydrated" payload (extract_tool_content (List.hd result)))
;;
...
let test_only_last_k_hydrated () =
  with_temp_dir (fun dir ->
    ...
    | [ c1; c2; c3; c4 ] ->
      ...
      Alcotest.(check string) "third hydrated" "third payload bytes" c3;
      Alcotest.(check string) "fourth hydrated" "fourth payload bytes" c4
...
let test_non_tool_result_unchanged () =
  with_temp_dir (fun dir ->
    ...
    let r = H.hydrate_recent ~store ~keep_recent:3 in
    let result = invoke_reducer r [ msg ] in
    Alcotest.(check int) "still one block" 1 (List.length (List.hd result).content);
...
let test_keep_recent_from_env_default () =
  ...
  Alcotest.(check int) "default applied" H.default_keep_recent (H.keep_recent_from_env ());
  ...
  Alcotest.(check int)
    "garbage = default"
    H.default_keep_recent
    (H.keep_recent_from_env ());
  Unix.putenv "MASC_TOOL_HYDRATE_RECENT" ""
;;
...
let () =
  Alcotest.run
    ...
    [ ( "hydrate_recent"
      , [ Alcotest.test_case
            "single recent marker hydrated"
            `Quick
            test_hydrates_recent_marker
        ; Alcotest.test_case
            "keep_recent=0 = no hydration"
            `Quick
            test_keep_recent_zero_no_hydration
        ; Alcotest.test_case "only last K hydrated" `Quick test_only_last_k_hydrated
        ] )
    ; ( "passthrough"
      , [ Alcotest.test_case "inline unchanged" `Quick test_inline_unchanged
        ; Alcotest.test_case
            "non-tool-result unchanged"
            `Quick
            test_non_tool_result_unchanged
        ] )
    ; ( "resilience"
      , [ Alcotest.test_case "miss keeps marker" `Quick test_hydration_miss_keeps_marker ]
      )
    ; ( "config"
      , [ Alcotest.test_case
            "keep_recent_from_env"
            `Quick
