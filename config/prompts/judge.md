...
실행하려다 상자를 만들지 못한 것입니다. 상자는 소켓을 막는 격리 환경이고,
실행 방식에 따라 스크래치 밖의 파일 쓰기도 막습니다. 이 필드는 어느 방식이었는지
기록하지 않습니다. 상자를 만들지 못하면 호스트는 그 프로그램을 시작하지 않고,
상자 없이 대신 실행하지도 않습니다. 요청이 여러 명령으로 이어져 있어도, 이
필드는 모든 명령의 상자가 만들어지지 않았을 때만 붙습니다. 어느 한 명령이라도
상자가 만들어졌다면 이 필드는 없습니다. 그래서 이 필드가 있으면 요청의 어느
명령도 시작되지 않았고, 아무 효과도 남지 않았으며, 요청이 무엇을 하려 했는지도
알 수 없습니다.
...
- `setup_failed`: 상자가 적용되기 전에 실패했지만, 어느 규칙 때문인지 이름을
  대지 못했습니다. 작업 디렉터리로 이동하지 못한 경우나, 이름 없는 규칙이
  설치되지 않은 경우가 여기에 들어갑니다.
- `unattributed`: 실행기가 프로그램을 시작하기 전에 요청을 거절했고, 어느
  규칙 때문인지 밝히지 않았습니다.
어느 값이든 요청의 명령은 하나도 시작되지 않았습니다. 명령이 여럿이면 이
값은 처음 거절된 명령의 것입니다. 이 값은 호스트의 상자가 왜 준비되지
않았는지를 말할 뿐, 요청이 네트워크나 파일 쓰기를 쓴다는 뜻이
아닙니다.

`observation.status`는 실행기가 멈추며 남긴 종료 상태이고, 요청한 명령의
...
이 필드가 없는 경우는 세 가지입니다.
- 상자 실행을 아예 하지 않았습니다.
- 상자 실행을 했지만 실행기의 확인 응답이 없거나 빠져서, 상자가 만들어졌는지
  알 수 없습니다.
- 적어도 한 명령의 상자가 만들어졌습니다. 그 명령이 실제로 시작됐는지는 이
  필드로 알 수 없습니다.
필드가 있든 없든 위의 기준대로 요청 그 자체를 판정합니다. 상자를 만들지 못했다는 사실 자체는 거부 사유도
