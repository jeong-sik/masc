...
- `tool_task.ml:198-205` (`handle_release`): `task_id` + `expected_version`만 수용. `handoff_context` 파라미터 없음.
- `tool_task.ml:757+` (`masc_transition` schema): `action`, `task_id`, `agent_name`, `expected_version`만 정의.
- `room_task.ml:497-500` (`task.released` activity): payload에 `task_id`만 포함.
