...
let for_json ~(name : string) (json : Yojson.Safe.t) : t =
  let name = Keeper_turn_profile.canonicalize name in
  let read_float key =
  ...
  in
  let temperature =
    match read_float (Keeper_turn_profile.temperature_key name) with
    | Some _ as v -> v
    | None -> read_float "default_temperature"
  in
  let max_tokens =
    match read_int (Keeper_turn_profile.max_tokens_key name) with
    | Some _ as v -> v
