...
| 2c' | 그 줄 **뒤에** 같은 trace 의 재시작 줄(3d)이나 시작 상태를 실은 끝 줄(`Continued_history_from`)이 있는가 | bool | 있으면 서는 것을 그만둔다. 재시작은 시작점을 atom 0 으로 놓고, 그보다 앞선 시작점은 없다. 그 줄이 담았을 수 있는 자를 지점은 이미 번호가 다시 매겨진 이력의 것이다. 시작 상태를 실은 끝 줄은 그 턴이 비어 있지 않은 이력 위에서 시작했다고 말하므로, 못 읽는 줄이 그 이력을 재시작하지 않았다 — 또는 재시작했다면 그 뒤에 저장된 atom 이 atom 0 부터 닿는다. 서는 것만 그만두고 시작점은 3c 가 정한다. 재시작 줄이 같은 trace 여야 하는 이유는, 못 읽는 줄이 어느 trace 것인지가 바로 못 읽는 부분이기 때문이다 |
...
- **I7 Keeper 는 Librarian 을 기다리지 않는다** — Keeper 기동, 재기동, 턴 진행 어디에도 Librarian 완료를 기다리는 자리가 없다. 턴의 성패도 Librarian 이 따라왔는지에 달려 있지 않다(§4.10).
...
1. **턴 끝 기록** `<keepers_dir>/<keeper>/turn-boundaries.jsonl`
   ...
   - 시작할 때의 이력은 `Fresh_history`(이 턴이 시작한 이력에 atom 이 없었다), `Continued_history`, 또는 `Continued_history_from { start_atom; start_atom_digest }` 다. "checkpoint 를 불러왔는가"가 아니라 "atom 이 있었는가"로 정한다. Keeper 는 메시지 0개짜리 checkpoint 를 갖고 만들어지고(`keeper_turn_up_create.ml`), `keeper_clear` 가 비운 이력에도 checkpoint 는 있다. 불러왔는지로 정하면 새 Keeper 의 첫 턴과 비운 직후의 턴이 `Continued_history` 가 되어, 그 trace 에는 이력이 0 에서 시작했다고 말하는 줄이 끝내 없다. checkpoint 를 못 읽었거나 버전이 바뀌었거나 purge 된 경우도 atom 이 없는 시작이라 같은 값이다. 읽는 쪽은 `Fresh_history` 를 재시작 줄로 쓴다(§4.4 의 3d).
   - 이어지는 이력으로 시작한 턴은 자기 시작 상태를 함께 싣는다(`Continued_history_from`). `start_atom` 은 그 턴이 시작한 이력의 atom 수이고 `start_atom_digest` 는 atom `start_atom - 1` 을 여는 메시지의 digest 다. 이 갈래는 기존 `turn_ended` 줄에 필드를 더한 것이 아니라 `history_at_start` 합타입에 더한 새 갈래다(§5). 그래야 못 읽는 줄 하나가 그 Keeper 의 회차를 영구히 세우지 않는다(§4.10, #37061): 뒤따르는 줄이 자기 시작 상태를 실으면 못 읽는 줄이 재시작 줄이었는지 아닌지를 그 줄이 답할 수 있다. 시작 atom 의 여는 메시지가 없어 digest 를 만들 수 없으면 그 줄은 `Continued_history` 로 남고, 읽는 쪽은 그 줄을 시작 상태로 쓰지 않는다(보수적).
...
- 못 읽는 줄에 선 것은(§4.4 의 2c) 다시 읽어서 풀리지 않는다. 파일은 고쳐 쓰지 않으므로 그 줄은 그대로 있고, 다음 회차도 같은 자리에 선다. 이 정지를 끝내는 것은 그 줄 뒤에 붙는 재시작 줄(2c') 또는 시작 상태를 실은 끝 줄(`Continued_history_from`)이다. `keeper_clear`, 버전 컷, 빈 이력에서 시작하는 턴이 재시작 줄을 쓴다. 시작 상태를 실은 끝 줄은 이어지는 이력으로 시작한 턴이 쓴다(§4.6). 그 줄이 있으면 회차는 그 줄이 재시작 줄이었을 가능성을 받아들여 atom 0 부터 읽는다 — 최악이 같은 구간을 한 번 더 읽는 것이고 그 중복은 I2 가 받아들였다. 그래서 정지는 유한하다. 그때까지 그 Keeper 의 기억은 늘지 않으므로, 4단계의 루프는 이 정지를 세어야 한다. 밀린 턴 수만으로는 "읽을 것이 없어서 조용한 것"과 구분되지 않는다.

  이 정지는 이제 모델이 잡는다(09-19). 여기 적힌 것은 문장이었고 증거가 없었다 — `LibrarianRead` 의 cfg 아홉이 거는 불변식은 `TypeOK` 와 `NoAtomPassedUnread` 둘뿐인데, **영구 정지는 둘 다 만족시킨다.** 아무것도 건네지 않는 회차는 아무것도 건너뛰지 않기 때문이다. 그래서 생존 속성을 세웠다: 마지막 자르는 자리까지의 atom 은 언젠가 읽혀야 한다(`EveryReachableAtomEventuallyRead`). 저장하고 죽은 턴의 atom 은 자르는 자리가 없어 이 물음에서 빠진다. 그 예외를 빼고도 **여덟 걸음에 위반**한다: 턴 시작 줄이 안 읽히고 → 그 턴이 저장하고 죽고 → 다음 턴이 이어지는 이력으로 시작해 저장하고 정상으로 끝난다. 끝 줄이 `Continued_history` 라 재시작이 아니므로 첫 줄은 영영 안 풀리고, 자르는 자리가 있는데도 atom 둘이 영영 안 읽힌다. `specs/bug-models/LibrarianRead-stop-forever-buggy.cfg` 가 이 상태를 고정했다. **#37061(2026-09-23)이 정지를 닫았다**: 이어지는 이력으로 시작한 턴의 끝 줄이 자기 시작 상태를 싣고(§4.6), 회차는 그 줄로 못 읽는 줄을 풀어 atom 0 부터 읽는다. 그 cfg 는 `LibrarianRead-stop-forever-live.cfg` 로 승격됐고 이제 위반하지 않아야 한다.

  생존 속성은 한 가지를 더 excused 로 둔다. 로그의 **마지막 줄이 못 읽는 줄**이면 회차는 그 줄이 풀리기를 기다리는 것이고, 이것은 §4.10 의 보이는 정지이지 손실이 아니다. 다음 턴의 끝 줄이 그 줄을 풀면 회차는 돌아온다. 유한한 턴 예산을 가진 모델은 로그를 거기서 끝낼 수 있으므로 이 상태는 도달 가능하고, 이 속성이 잡으려는 정지가 아니다. 속성이 금지하는 것은 **앞에 푸는 줄이 있는데도 돌아오지 않는 회차**다 — #37061 의 정지가 그것이다. 그래서 `LibrarianRead-stop-forever-live.cfg` 는 고침을 되돌리면(끝 줄이 시작 상태를 안 실으면) 다시 위반한다.

  세는 것은 알람이지 고침이 아니다. 고침은 못 읽는 줄의 정체를 뒤따르는 줄이 답할 수 있게 만드는 것이고, 그 선택지는 이슈에 적었다. 세는 것(1안)은 §4.9 의 밀린 턴 수와 함께 남는다 — 정지가 유한해져도 그 Keeper 의 기억이 늘지 않은 구간은 보여야 한다.
- 공식 클라이언트 턴 쪽은 못 읽는 줄에서 계속 선다. 그 줄이 공식 턴의 끝 줄이었을 수 있고, 뒤에 오는 재시작 줄이나 시작 상태를 실은 끝 줄은 그 물음에 답하지 못한다. 지나가면 그 턴을 건너뛰게 된다(I3). 푸는 방법은 그 Keeper 를 purge 하는 것이다(`Keeper_librarian_range.Official_stop`).

  다만 이 정지는 공식 쪽 위치만 세운다. 두 위치는 따로 움직이므로, 공식 쪽이 서 있어도 atom 쪽은 자기 규칙대로 읽고 위치를 옮긴다. 전에는 공식 쪽 정지가 회차 전체를 실패로 끝냈다. 공식 턴을 한 번도 돌리지 않은 Keeper 는 공식 위치가 없어서 로그의 못 읽는 줄이 모두 그 위치 뒤에 있다. 그래서 atom 쪽이 #37061 로 푼 줄에서도 모든 회차가 섰다. 지금은 atom 쪽에 읽을 것이 없을 때 회차가 `Official_range_stopped` 로 끝난다. 한 번 깨어난 회차는 atom 을 다 읽은 뒤 이 정지로 끝나므로, 회차 종결 상태(`Stopped`)와 그 원인은 전처럼 화면에 남는다.

  남은 일: 공식 턴일 수 있는 줄에서만 서기, 그리고 못 읽는 줄이 공식 턴이었는지를 뒤따르는 줄로 답할 수 있게 하기. 둘 다 이 규칙을 바꾸는 설계라 따로 정한다.
...
- 선행 조건: 한 후보의 `Context_overflow` 가 다음 후보로 넘어가야 한다(D8). 넘어가므로 한도가 더 큰 다음 후보가 앞 후보에서 거절된 턴을 읽는다. 모든 후보가 거절하면 걸음 전체를 크기 증거로 보고 폭을 접는다.
- **Librarian 이 서도 Keeper 는 턴을 계속 돈다**(구현 전. #37793 이 물은 (가)/(나) 중 (가)를 택한다. 경위는 #37793 issuecomment-5788668419 에 있다). 아래 1~7 은 구현할 규칙이고, 지금 코드는 아직 이렇게 돌지 않는다. 요청이 읽은 위치부터 시작하면, Librarian 이 서 있는 동안 요청이 턴마다 커진다. 그 끝은 provider 의 거절이고, 그 뒤로는 매 턴 같은 거절이다. Keeper 가 턴을 돌지 못하니 I7 이 사실상 깨진다. "읽은 위치부터 보낸다"와 "건너뛰지 않는다"는 묶여 있지 않다. I3 은 Librarian 이 읽는 위치를 말하고, Keeper 요청이 어디서 시작하는지는 말하지 않는다. 그래서 이렇게 한다.
  1. 판정은 추정이 아니라 일어난 일이다. 시작점이 이번 턴의 경계보다 앞인 요청을 provider 가 크기 때문에 거절하면, 그 턴은 마지막으로 끝난 턴의 경계부터 같은 후보에 한 번 더 보낸다. 시작점이 Librarian 지점이든 아래 2의 받아들여진 시작점이든 같다. 크기 거절로 보는 것은 typed 생성자 셋이다: `ContextOverflow`, `InvalidRequest { Request_body_refused_by_provider }`, `Unknown_invalid_request`. 셋째는 사유를 모르는 거절인데, 그 사유를 문장으로 읽는 것은 문자열 분류라 하지 않고 크기 거절과 같게 다룬다(창 RFC §10.4). 실제로 크기 거절이 이 모양으로 온다. 2026-09-23 01:41Z msx-retro-mania 의 glm-5.3-flash 턴은 `Invalid request (unknown): Prompt exceeds max length` 로 끝났다. 없이 두면 그런 Keeper 는 매 턴 거절된다. 크기와 무관한 400 이면 경계부터 한 번 더 보내고, 그것도 거절되면 턴이 끝난다(6). 잃는 것은 그 턴에 경계 앞을 싣지 못한 것뿐이다. 받아들여진 시작점도 기록되지 않는다. 크기 문제였다면 빠진 구간은 Librarian 이 읽고, 따라잡으면 시작점이 돌아온다(5). 이 셋은 경계 재전송에만 쓰는 판정이다. 범위를 줄이는 다른 사다리(묶음 비우기, 반 자르기, 도구 결과 마커 강등)는 typed 크기 거절 둘(`ContextOverflow`, `Request_body_refused_by_provider`)에서만 돈다(창 RFC §8 의 13, `refusal_evicts`, #38286). 두 판정은 코드에서도 따로 둔다. 2026-09-23 운영자 결정이고, 경위는 #38286 댓글에 있다. 크기를 미리 재는 문턱도, 시간도, 실패 횟수도 쓰지 않는다. 지점이 없는 Keeper 의 씨앗이 거절될 때(`Turn_start_after_seed_refusal`, #38046)와 같은 모양이다.
  2. 받아들여진 요청의 첫 atom 은 턴 기록(`response_observed_model_input`)에 남는다. 이것을 "받아들여진 시작점"이라 부른다. 다음 턴은 Librarian 지점이 받아들여진 시작점보다 앞이면 받아들여진 시작점부터 싣는다. 그래서 같은 거절을 매 턴 다시 올려 보내지 않는다. 턴 기록은 파일이므로 서버를 다시 띄워도, 레인 걸음에서 다른 후보로 넘어가도 같은 값을 읽는다. 메모리에만 있는 ledger(`Keeper_model_input_ledger.Table`)는 이 판단에 쓰지 않는다.
  3. gap 은 Keeper 요청에도 기억에도 아직 없는 구간이다. 스냅숏 컷과 durable 읽은 위치 중 뒤의 것부터 받아들여진 시작점 직전까지다. 받아들여진 시작점이 그보다 앞이면 gap 은 없다. 요청의 시작점(Librarian 지점)은 이와 따로 정한다. 맞는 스냅숏이 있으면 스냅숏 컷이고, 없을 때만 진행 파일의 읽은 위치다. 순서는 driver 가 범위의 시작을 고르는 규칙과 같다. 그래서 거절이 없어도 시작점이 스냅숏 컷을 넘을 수 있다. 어떤 턴이 읽은 위치 R 부터 받아들여지고, 다음 턴에 컷이 R 보다 앞인 스냅숏이 맞는 경우다. 그때 빠지는 구간은 이미 기억에 있으니 gap 이 아니다. 두 값은 크게 다르다. 2026-09-21~23 에 Librarian 기점으로 나간 요청 2,647 건 중 2,548 건이 스냅숏 컷에서 시작했다. 09-23 msx-retro-mania 는 읽은 위치로 재면 2 턴, 스냅숏 컷으로 재면 50 턴 밀려 있었다. gap 은 따로 저장하지 않고, 그 시작과 턴 기록의 받아들여진 시작점에서 매번 계산한다. 이 경로는 Librarian 의 읽은 위치를 옮기지 않고, 새로 쓰는 것은 턴 기록 하나뿐이다. 그래서 "시작점은 옮겼는데 gap 기록은 못 남긴" 상태가 없다. 빈 구간을 지나며 진행을 커밋해 거절을 영구 손실로 바꾸는 모양(#37941)과 다른 점이 이것이다.
  4. 요청에는 gap 의 범위(시작 atom, 끝 atom, 바이트)를 typed 블록으로 싣는다. 모델이 그 구간을 없던 일로 읽지 않게 하려는 것이다. 블록은 Librarian 스냅숏 블록 바로 뒤에 둔다. Librarian 지점이나 받아들여진 시작점이 움직일 때만 바뀌므로, 그 앞부분의 prompt cache 는 그대로다. 스냅숏의 working state 는 그대로 싣는다. gap 앞까지를 정리한 것이기 때문이다. 화면에는 밀림 수(I4)와 같은 자리에 `Librarian_stalled` 가 뜬다. 경보이지 Gate 가 아니다.
  5. Librarian 은 gap 도 빠짐없이 읽는다(I1·I3 그대로). 읽은 위치가 받아들여진 시작점에 닿으면 gap 과 경보가 함께 사라지고, 다음 턴은 다시 읽은 위치부터 싣는다.
  6. 경계부터 보낸 요청도 typed 크기 거절(`ContextOverflow`, `Request_body_refused_by_provider`)로 거절되면, 이번 턴의 도구 결과를 마커로 바꿔 같은 후보에 한 번 더 보낸다(창 RFC §10.4, #38109). 사유를 모르는 거절이면 여기서 턴이 끝난다(1). 바꿀 것이 없거나 그것도 거절되면 그 턴은 그 거절로 끝난다. 그때 넘치는 것은 이력이 아니라 매 턴 붙는 부분이다(창 RFC §13.9 첫 항목). 이 장치가 닫는 것은 이력이 자라서 나는 거절 하나다. facts 는 Librarian 의 `absorbs`·`dropped` 로만 줄어드는데, Librarian 이 서 있으면 그것도 멈춘다. 그래서 facts 가 이미 부푼 Keeper 는 gap 이 넓어져도 계속 거절된다. `Librarian_stalled` 가 뜬 것만으로 해결됐다고 읽지 않는다.
  7. 시작점은 이번 턴의 경계보다 뒤로 가지 않는다. 경계 재전송도 이번 턴 입력은 늘 싣는다. 그래서 요약과 gap 블록만 있고 답할 입력이 없는 요청은 만들어지지 않는다. 매 턴 붙는 부분과 이번 턴 입력만으로 한도를 넘으면 gap 을 넓혀도 줄일 이력이 없고, 거절이 그대로 난다. 답할 입력 없는 요청보다 거절이 낫기 때문에 정한 결과다. 이 규칙을 깎아 시작점을 이번 턴 안으로 옮기지 않는다. 이 거절은 턴의 마지막 오류로 보이고, gap 이 닫히면 풀리는 `Librarian_stalled` 경보와 같은 표시로 묶지 않는다.
  - Tick 으로 따라가면 이렇다. Librarian 이 P 에 선다. T1 은 P 부터 보내 거절되고, 경계 B1 부터 다시 보내 받아들여진다. T2 부터는 B1 부터 싣고 요청이 다시 자란다. Tk 에서 B1 부터 보낸 요청이 거절되면 1 에 따라 경계 Bk 부터 다시 보낸다. Librarian 이 P' < Bk 까지 오면 시작점은 Bk 그대로이고, P'' ≥ Bk 가 되면 P'' 부터 싣는다. 어느 회차에도 같은 거절이 되풀이되는 고리가 없다.
  - 이것은 마지막 방어선이다. Librarian 이 서는 원인은 따로 고친다. 원인이 빠진 `agent_core_execution_failed` 를 크기 문제로 읽어 읽는 범위를 줄이는 것이 그 하나다(#37899). `Librarian_stalled` 가 자주 뜨면 원인 수정이 모자란 것이다.
  - 공식 클라이언트 레인은 선언된 바이트 창이 있는 동안 자기 컷(`Keeper_official_client_host.window_carried_range`)으로 같은 구간을 말없이 건너뛴다. 그 구간도 같은 방식으로 gap 으로 계산해 `Librarian_stalled` 를 띄운다. 바이트 창이 없어지면(창 RFC §13.2) 이 레인도 1 의 규칙을 따른다.
...
- **엄격하게 디코딩되는 기존 저장소에 필드 더하기.** §4.6 의 이유다. 대신 쓰는 형태는 새 갈래다: `turn_ended` 줄에 필드를 더하는 대신 `history_at_start` 합타입에 `Continued_history_from` 을 더한다(§4.6). 기존 줄의 필드 집합은 그대로라 옛 줄이 계속 읽히고, 새 갈래를 빠뜨린 소비자는 컴파일에서 멈춘다.
...
- 공식 클라이언트 턴을 고르는 규칙과 읽은 곳을 적는 값. §4.4 의 2 는 `end_atom` 이 있는 줄만 안 읽은 턴으로 고르므로 `No_atom_history` 줄은 뽑히지 않는다. §4.8 은 그 턴을 턴 끝 시각 사이의 history 줄로 읽는다고 하면서 atom 위치는 움직이지 않는다고 한다. 그러면 읽었다는 것을 적는 값이 없고, 공식 클라이언트 런타임만 쓰는 Keeper 는 자르는 자리가 하나도 없어 회차가 돌지 않는다. **#37521·#37527(2026-09-21)이 채웠다**: history 줄이 `turn_ref` 를 싣고, durable 소비자가 `No_atom_history` 턴 끝 줄을 그 `turn_ref` 의 history 조각으로 읽으며, 읽은 곳은 `librarian-official-progress.json` 의 `boundary_line`(턴 끝 기록의 줄 번호)에, 커밋의 영수증은 `official_range_id { after_boundary_line; turns }` 에 적는다. 조각이 하나도 없는 줄은 경고를 남기고 지나간다. 실패로 끝난 공식 클라이언트 턴도 끝 줄을 남긴다(2026-09-24). 그 턴은 `finalize` 에 닿지 않지만 입력은 턴 시작에 이미 조각이고 도구는 이미 움직였을 수 있다. `keeper_agent_run.ml` 이 마지막으로 보낸 후보의 소유자가 공식 클라이언트면 `record_errored_official_turn_boundary` 가 도구 관측과 `No_atom_history` 끝 줄을 쓴다. 그 전까지 이 턴은 끝 줄이 없어 밀린 턴 수에도 잡히지 않았다(I3). 한 턴 안에서 공식 클라이언트 시도가 실패하고 다른 런타임이 성공한 경우, 실패한 시도의 도구 관측은 아직 남지 않는다. 4단계 때는 옛 경로(`remember_turn`·`attempt_remembered`)를 그대로 두었고(§2.5 의 D6) — durable 경로와는 `checkpoint_owner` 로 갈려 같은 턴을 읽을 수 없었다 — 그 위에서 5단계가 지웠다(§8 의 5 행).
...
3. **공식 클라이언트 턴을 어떤 자로 읽을지.** 그 턴의 내용은 trace 디렉터리의 `history.jsonl` 과 `history.internal.jsonl` 에 있다(§4.8). 줄마다 `ts_unix` 가 있고 turn ref 는 없다(`keeper_context_core_history.ml` 의 `persist_message`). 선택지는 셋이다. (가) 읽은 곳을 시각으로 적는다. 벽시계에 기대므로 시계가 뒤로 가면 줄을 놓친다. (나) 읽은 곳을 턴 끝 기록의 줄 번호로 적는다. 줄 번호는 잠금 아래에서 매겨져 뒤로 가지 않는다. 다만 파일에 쌓인 순서는 턴 순서가 아니고(§4.4 의 2), 내용은 여전히 `ts_unix` 구간으로 고른다. (다) 읽은 곳을 내용이 있는 곳의 자(두 history 파일의 줄 수)로 적고, 턴 끝 줄이 그 턴이 끝났을 때의 줄 수를 싣는다. atom 레인과 같은 모양이 된다: 읽은 위치는 값이고, 줄은 자르는 자리이고, 내용은 언제나 저장소에서 읽는다. `No_atom_history` 가 줄 수를 가진 variant 로 바뀌므로 줄 형식이 달라진다. 1단계의 줄이 아직 배포되지 않은 지금이 가장 싸다. 권고는 (다)이다. 먼저 확인할 것은 두 history 파일이 덧붙이기만 하는가이다(회전, 비우기, purge). 4단계 앞에서 정하지 않는다(2026-09-21) — 4단계는 공식 클라이언트 direct closure 를 그대로 두므로 이 결정을 기다릴 이유가 없고, 이 결정은 생산자 쪽 변경(아래)이라 따로 간다.
   ...
   **#37061(2026-09-23)이 (나) 방향을 턴 경계판에 먼저 넣었다.** 이어지는 이력으로 시작한 턴의 끝 줄이 자기 시작 상태를 싣는다(`Continued_history_from`, §4.6). 이것은 위 혼합 history 소비자의 offset 문제를 닫지 않는다 — 그 소비자는 아직 production 에 없다(#37104, 후속 #37192). 닫는 것은 §4.4 의 2c 정지다: 못 읽는 줄 뒤에 붙는 줄이 자기 시작 상태를 실어 그 줄의 정체를 가른다. `turn boundary not recorded` 로그의 실제 발생 빈도는 아직 재지 않았다 — 라이브 서버 로그가 필요하고, 이 수정을 준비한 레인에서는 잴 수 없었다. (가)를 같이 볼지는 그 실측 뒤에 정한다.
