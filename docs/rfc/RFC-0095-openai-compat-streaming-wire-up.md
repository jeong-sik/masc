...
**`Custom_provider_d_compat` runtime binding 을 사용하는 provider
...
| ID | 가설 | 일차 검증 방법 |
|---|---|---|
| H1 | Agent SDK 가 `Custom_provider_d_compat` binding 에 대해 `run_stream` 진입 시 outbound HTTP body 의 `stream: true` 필드를 *송신하지 않음* | local llama-server `--verbose` 또는 mitmproxy 로 inbound body capture, `"stream"` 필드 검사 |
...
- provider-k 은 *built-in adapter* (별도 `Provider_runtime_binding.Http` variant), openai_compat 은 *generic adapter* (`Provider_runtime_binding.Custom_provider_d_compat`). 두 path 가 SDK 내부에서 분기될 가능성 (H1).
...
3. `lib/cascade/cascade_runtime_candidate.ml` 의 `provider_label`
   결정 trace — `Custom_provider_d_compat` candidate 가 어떤 label 로
   classify 되는지.
...
- **H1 confirmed**: SDK provider binding adapter 가 `Custom_provider_d_compat`
