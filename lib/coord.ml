...
let observe_task_transition_event
  ...
  (try
     ...
     then (
       ...
       | Types.Release | Types.Submit_for_verification | Types.Reject_verification -> ())
   with
   | Stdlib.Effect.Unhandled _ -> ());
  try
  ...
  Atomic.set Coord_hooks.hebbian_on_task_done_fn (fun config ~assignee ~active_agents ->
    List.iter
      (fun peer ->
         if peer <> assignee
         then (
           (try Hebbian_eio.strengthen config ~from_agent:assignee ~to_agent:peer () with
            | Eio.Cancel.Cancelled _ as e -> raise e
            | exn ->
    ...
    (fun config ~agent_name ~active_agents ->
       List.iter
         (fun peer ->
            if peer <> agent_name
            then (
              (try Hebbian_eio.weaken config ~from_agent:agent_name ~to_agent:peer () with
               | Eio.Cancel.Cancelled _ as e -> raise e
               | exn ->
