...
let test_known_default_pin () =
  List.iter
    (fun (caller, expected) ->
       ...
       | Some v ->
         check approx (Printf.sprintf "%s default" (E.caller_key caller)) expected v
       | None -> failf "expected default for %s, got None" (E.caller_key caller))
    cases
;;
...
let test_unknown_default_is_none () =
  check
    ...
    None
    (E.known_default_sec (E.Unknown "future_caller"))
;;

let test_known_callers_complete () =
  let n_known = List.length (E.known_callers ()) in
  check int "known caller count matches case table" (List.length cases) n_known
;;
...
let test_per_caller_env_overrides_default () =
  with_env "MASC_EXEC_TIMEOUT_SHELL_SEC" (Some "7.5") (fun () ->
    check approx "Shell uses env override" 7.5 (E.timeout_sec ~caller:E.Shell ()));
  (* And the default returns after override is cleared. *)
  check approx "Shell falls back to default" 60.0 (E.timeout_sec ~caller:E.Shell ())
;;

let test_empty_env_treated_as_unset () =
  with_env "MASC_EXEC_TIMEOUT_FS_SEC" (Some "") (fun () ->
    check approx "Empty env still hits default" 30.0 (E.timeout_sec ~caller:E.Fs ()))
;;
...
let test_invalid_env_falls_to_global_default () =
  with_env "MASC_EXEC_TIMEOUT_PREFLIGHT_SEC" (Some "not-a-number") (fun () ->
    ...
    check
      approx
      "Invalid float -> global_default_sec"
...
let test_global_env_only_for_unknown () =
  with_env "MASC_EXEC_TIMEOUT_DEFAULT_SEC" (Some "999.0") (fun () ->
    check approx "Known caller ignores global env" 60.0 (E.timeout_sec ~caller:E.Shell ());
    check
      approx
      "Unknown caller uses global env"
      999.0
      (E.timeout_sec ~caller:(E.Unknown "future") ()))
;;

let test_unknown_falls_to_global_default_when_unset () =
  check
    approx
    "Unknown without env -> global_default_sec"
    E.global_default_sec
...
let () =
  run
    "env_config_exec_timeout_10426"
    [ ( "defaults"
      , [ test_case "per-caller defaults pinned" `Quick test_known_default_pin
        ; test_case "Unknown default is None" `Quick test_unknown_default_is_none
        ; test_case "known_callers covers case table" `Quick test_known_callers_complete
        ] )
    ; ( "env-override"
      , [ test_case "per-caller env wins" `Quick test_per_caller_env_overrides_default
        ; test_case "empty env treated as unset" `Quick test_empty_env_treated_as_unset
        ; test_case
            "invalid env -> global_default_sec"
            `Quick
            test_invalid_env_falls_to_global_default
        ] )
    ; ( "global-env"
      , [ test_case
            "global env only affects Unknown"
            `Quick
    ...
    ; "env-var-name", [ test_case "shape and case" `Quick test_env_var_name_shape ]
    ]
;;
