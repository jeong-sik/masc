...
let workspace_preflight ~base_path =
  ...
  try
    (* The lstat only asks whether the root exists: ENOENT is a fresh
       workspace, anything else falls through to the realpath preflight. *)
    match (try Some (Unix.lstat requested_root) with
           | Unix.Unix_error (Unix.ENOENT, _, _) -> None) with
    | None -> Workspace_ready
    | Some _ ->
