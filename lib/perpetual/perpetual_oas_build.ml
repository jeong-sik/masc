...
let build_perpetual_agent
  ...
  let builder =
    ...
    |> Oas.Builder.with_event_targets [
         Oas.Event_forward.File_append {
           path = Filename.concat
             (Option.value ~default:".masc" (Sys.getenv_opt "ME_ROOT" |> Option.map (fun r -> Filename.concat r "logs")))
             (sprintf "perpetual-%s-events.jsonl" pstate.trace_id);
         }
       ]
