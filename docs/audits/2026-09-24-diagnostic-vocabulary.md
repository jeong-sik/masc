...
| TUI Connector read | Keeper Channels displayed `channel transports unavailable: connector load failed` on a first failure and `refresh failed: connector load failed` over retained rows. | #38875 attributes HTTP, decode, and async failures once before display; Channels keeps the cause and marks retained rows `STALE`. The unbind-all offer names its own unavailable action separately. |
