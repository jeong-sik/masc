  ...
  let rec collect blocks current = function
    ...
    | line :: rest ->
      ...
      then (
        let blocks = if current = [] then blocks else List.rev current :: blocks in
        collect blocks [] rest)
      else collect blocks (line :: current) rest
  in
  ...
  let last_http_block =
    List.fold_left
      (fun acc block ->
         match block with
         | status_line :: _
           when String.length status_line >= 5 && String.sub status_line 0 5 = "HTTP/" ->
           Some block
         | _ -> acc)
      None
      blocks
...
let run_curl ?(headers = []) ?max_time ~port ~path () =
  ...
  in
  let header_args =
    List.concat_map (fun (k, v) -> [ "-H"; Printf.sprintf "%s: %s" k v ]) headers
  in
  let args =
    [| "curl"; "-sS"; "--http1.1"; "-X"; "GET"; "-o"; body_file; "-D"; header_file |]
    |> Array.to_list
    |> fun base -> base @ max_time_args @ header_args @ [ url ] |> Array.of_list
  in
...
let wait_for_health ~port ~timeout_s =
  let has_ready_flag body =
    ...
    let rec loop idx =
      if idx + needle_len > body_len
      then false
      else if String.sub body idx needle_len = needle
      then true
      else loop (idx + 1)
    in
    loop 0
...
let test_ag_ui_rejects_reconnect_then_recovers () =
  ...
  let first =
    run_curl ~headers ~max_time:0.2 ~port ~path:"/ag-ui/events?room=default" ()
  in
  check_status "first /ag-ui/events connect accepted" 200 first;
  let second =
    run_curl ~headers ~max_time:0.5 ~port ~path:"/ag-ui/events?room=default" ()
  in
  check_status "immediate /ag-ui/events reconnect rejected" 429 second;
  Unix.sleepf 2.0;
  let third =
    run_curl ~headers ~max_time:1.5 ~port ~path:"/ag-ui/events?room=default" ()
  in
...
let () =
  ...
  run
    "sse_storm_e2e"
    [ ( "mcp"
      , [ test_case "follow-up reconnect accepted" `Slow test_mcp_reconnect_stays_accepted
        ] )
    ; ( "ag_ui"
      , [ test_case
