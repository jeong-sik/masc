...
let test_relevance_pass () =
  let result =
    Pv.verify ~content:"This is a meaningful post with enough substance to pass."
  in
...
let test_relevance_filler_long () =
  let result =
    Pv.verify
      ~content:"This is a hello world test with more text around it to pass length check."
  in
...
let test_quality_pass () =
  let result =
    Pv.verify ~content:"A well-formed post about software architecture patterns."
  in
...
let test_safety_url_spam () =
  let result =
    Pv.verify
      ~content:
        "https://spam.example.com/very/long/path/to/something \
         https://another.example.com/more/content"
  in
  check bool "URL-heavy content no longer affects safety" true (is_pass result.safety)
;;
...
let test_overall_pass () =
  let result =
    Pv.verify
      ~content:"A thoughtful discussion about Eio concurrency patterns in OCaml 5."
  in
...
let test_overall_warn_propagation () =
  let result =
    Pv.verify ~content:"A post with some aaaaa repeated chars but otherwise OK content."
  in
  ...
  let post_verifier_site =
    List.find
      (fun (site : Hm.coverage_site) ->
         site.module_name = "post_verifier" && site.site = "relevance")
      report.sites
  in
