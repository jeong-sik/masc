...
let qc_normalized_in_unit =
  QCheck.Test.make
    ~name:"normalized entropy in [0,1]"
    ~count:200
    (int_list_gen ~min_len:2 ~max_len:20 ~min_val:1 ~max_val:100)
    (fun counts ->
       let h = Masc_mcp.Keeper_tool_diversity.shannon_entropy counts in
...
let () =
  run
    "Masc_mcp.Keeper_tool_diversity"
    [ ( "unit"
    ...
    ; ( "qcheck"
      , List.map
          ...
          [ qc_entropy_non_negative; qc_entropy_bounded; qc_normalized_in_unit ] )
    ]
;;
