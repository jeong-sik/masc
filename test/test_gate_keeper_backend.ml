...
(* OAS 0.208.10 dropped the [content] field from [provider_reasoning_block]
   (reasoning payloads are no longer projected into the canonical tool block).
   These checks now assert reasoning structure (order/kind/signature/adjacency);
   reasoning text is asserted separately via the trajectory path. *)
...
let test_oas_tool_call_projection_preserves_adjacent_reasoning_groups () =
  ...
  | [ first; second; third ] ->
      ...
      (* OAS 0.208.10 (#2399) indexes tool calls by tool-only ordinal
         (0,1,2), not block position. Adjacent-reasoning order_index still
         tracks block position, so the reasoning checks below are unchanged. *)
...
let test_oas_interleaving_matches_masc_receipt_and_progress_facts () =
  ...
  | [ first; second ] ->
      ...
      (* OAS 0.208.10 (#2399): tool-only order index (0,1). *)
