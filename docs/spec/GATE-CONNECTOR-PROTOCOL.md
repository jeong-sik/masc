...
```json
{
  ...
  "content": {
    ...
    "webrtc_session_id": "uuid-v4", // only when type=webrtc_signal
    "webrtc_payload": {} // only when type=webrtc_signal (SDP or ICE)
  },
  "metadata": {}
}
```
...
- **Connector 연동:** Gate가 Inbound 요청을 받을 때 상태를 검사하고, Muted 상태면 `403 Forbidden` (정책적 차단, 재시도 불필요), Rate-limit 초과면 `429 Too Many Requests` + `Retry-After` 헤더를 반환합니다.

### 4.3. Connection Stability & Reliability (연결 안정성 및 에러 복구)
- **SSE Reconnect & Recovery:** Connector는 SSE 연결 시 `Last-Event-ID`를 명시하여 재연결을 시도합니다. Gate의 인메모리 이벤트 버퍼에서 해당 이벤트가 이미 삭제된 경우, 두 경로 중 하나로 처리합니다:
  - **경로 A (410 Gone):** `HTTP 410 Gone` + 표준 에러 바디 (`{"error":"event_evicted","last_available":"<event_id>"}`) 반환. Connector는 전체 상태를 재요청.
  - **경로 B (Snapshot):** `HTTP 200` + `event: snapshot` SSE 이벤트로 전체 상태를 내려줌. Snapshot 스키마는 `outbound_event`와 동일하되 `type: "state_snapshot"`.
...
### 4.5. MASC Internal Data Translation (내부 데이터 매핑 룰)
