...
report={'scope':'owned Gesture Lab; real native TUI inputs through unchanged HTTP forwarding to compiled MASC and automation Firefox; no Keeper model','steps':[],'cleanup':[]}
...
try:
 ...
 report['scope']='Actual native TUI screenshot input through recording HTTP forwarder to compiled automation Browser Lane and real Firefox; no Keeper/model in this gesture run'
 ...
 session=api('open','/api/v1/dashboard/browser-lane/session',{'action':'open','headless':True})
 assert session['reused'] is False;opened=True
 api('goto','/api/v1/dashboard/browser-lane/goto',{'url':url})
 page=api('initial-page','/api/v1/dashboard/browser-lane/read',{'lane':'automation'})
 tab=next(t for t in page['tabs'] if t['url']==url)['id'];report['tab_id']=tab
 spec=importlib.util.spec_from_file_location('gesture_capture','/tmp/capture-real-browser-gestures.py')
 capture=importlib.util.module_from_spec(spec);spec.loader.exec_module(capture)
 count=[0]
 def observe(label):
  count[0]+=1
  return api(label+'-'+str(count[0]),'/api/v1/dashboard/browser-lane/scene',{'lane':'automation','tabId':tab,'view':'content'})
 def navigate(old_scroll):
  destination=url+'?actor=external'
  api('external-navigation','/api/v1/dashboard/browser-lane/goto',{'url':destination})
  ...
  assert status==400 and body=={'ok':False,'error':'javascript error: Error: page_url_changed'}, ('unexpected stale-probe result',status,body)
  ...
  return destination
 (OUT/'gesture-capture.py').write_bytes(pathlib.Path('/tmp/capture-real-browser-gestures.py').read_bytes())
 report['gesture_capture_sha256']=hashlib.sha256(pathlib.Path('/tmp/capture-real-browser-gestures.py').read_bytes()).hexdigest()
 ...
 if opened:
  try:
   closed=api('close','/api/v1/dashboard/browser-lane/session',{'action':'close'})
   assert closed.get('closed') is True, 'session close did not acknowledge closure'
   report['cleanup'].append('owned automation Firefox session closed')
  except Exception as error:report['cleanup'].append('session close failed: '+str(error))
 if server:
 ...
 cleanup_ok=('owned automation Firefox session closed' in report['cleanup']
