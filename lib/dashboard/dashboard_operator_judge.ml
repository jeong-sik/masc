...
let with_lock st f = Eio.Mutex.use_rw ~protect:true st.mutex f
  ...
  | `Assoc _ ->
    ...
    (match action_type with
     | Some action_type when action_type <> "" && allowed_action_type action_type ->
       let severity =
         json |> member "severity" |> to_string_option |> Option.value ~default:"warn"
       in
       let reason =
         normalize_text
  ...
  | `Assoc _ ->
    ...
    then None
      ...
      let confidence =
        ...
        | _ -> 0.0
      in
      let fresh_until_unix = generated_at_unix +. float_of_int (room_ttl_sec ()) in
      Some
        (Operator_judgment.record
           config
           ~surface:"command.namespace"
           ...
           ~disagreement_with_truth:
             (json
              |> member "disagreement_with_truth"
              |> to_bool_option
  ...
  | Ok result ->
    ...
    (try
       ...
       | `Assoc [ ("raw", `String raw) ] ->
         ...
         let msg =
           Judge_diagnostics.format_lenient_fallback ~judge_label:"Operator" raw
         in
...
let should_backoff ~sw ~net =
  try
    let capacity =
      Cascade_config.local_capacity_for_selections ~sw ~net [ "operator_judge" ]
    in
    capacity.all_discovered
    && capacity.endpoints_found > 0
...
let refresh_once
  ...
  then (
    ...
    in
    if was_online
    then Log.Dashboard.info "operator: backoff: local slots saturated, skipping cycle"
    else (
  ...
  then
    Eio.Fiber.fork_daemon ~sw (fun () ->
      ...
      let rec loop () =
        ...
        if was_backoff
        then Atomic.incr consecutive_backoffs
        ...
        then
          Log.Dashboard.debug
            "operator: backoff: sleeping %.0fs (consecutive=%d)"
            sleep_s
            n;
