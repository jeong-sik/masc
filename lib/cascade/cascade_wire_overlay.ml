...
let auth_headers_for_provider_cfg
  ...
  then headers
    ...
    | Provider_a | Provider_k -> ("x-api-key", api_key) :: headers
    | Provider_f | Cli_tool_d | Cli_tool_c | Cli_tool_b | Cli_tool_a -> headers
    | Provider_h ->
      ("Authorization", "Bearer " ^ api_key) :: headers
    | Provider_d_compat | Ollama | Provider_c ->
      ("Authorization", "Bearer " ^ api_key) :: headers)
;;

let request_kind_of_provider_cfg (provider_cfg : Llm_provider.Provider_config.t) =
  match provider_cfg.kind with
  | Provider_a | Provider_k -> Agent_sdk.Provider.Provider_a_messages
  | Provider_d_compat
  | Ollama
  | Provider_f
  | Provider_h
