...
let test_derive_long_korean_title_is_valid_utf8 () =
  ...
  let prefix = String.sub title 0 (String.length title - String.length suffix) in
...
let test_derive_korean_with_json_roundtrip () =
  (* The actual failure mode: write a post, serialize to JSONL, then
     ...
     does at runtime. *)
  let body =
    "## Verdict: #7460 needs-evidence + #7464 needs-evidence \xe2\x80\x94 poe scan #5 \
     나머지 리뷰 결과입니다 길게 이어지는 텍스트 추가"
  in
  let title = BP.derive_post_title body in
...
let () =
  run
    "Board_core_payload.derive_post_title"
    [ ( "regression-7690"
      , [ test_case "short title unchanged" `Quick test_derive_short_title_returned_as_is
        ; test_case "empty body -> default" `Quick test_derive_empty_body_uses_default
        ; test_case
            "long ASCII truncated"
            `Quick
            test_derive_long_ascii_truncated_with_ellipsis
        ; test_case
            "long Korean is valid UTF-8"
            `Quick
            test_derive_long_korean_title_is_valid_utf8
        ; test_case "Korean JSON roundtrip" `Quick test_derive_korean_with_json_roundtrip
        ] )
    ]
;;
