...
let test_real_savings_marks_ok () =
  ...
  Alcotest.(check (float 0.0001))
    ...
    (before_ok +. 1.0)
    (counter_for ~keeper ~outcome:"ok");
  Alcotest.(check (float 0.0001))
    "noop outcome counter unchanged"
    before_noop
    (counter_for ~keeper ~outcome:"noop")
;;
...
let test_per_keeper_isolation () =
  ...
  EC.record_compaction_outcome ~keeper_name:a ~before_tokens:100_000 ~after_tokens:50_000;
  Alcotest.(check (float 0.0001))
    "A counter +1"
    (a_before +. 1.0)
...
let () =
  Alcotest.run
    "keeper_compaction_outcome_counter_9988b"
    [ ( "metric_name"
      , [ Alcotest.test_case "canonical name stable" `Quick test_metric_name_stable ] )
    ; ( "outcome_classification"
      , [ Alcotest.test_case "real savings marks ok" `Quick test_real_savings_marks_ok
        ; Alcotest.test_case
            "before==after marks noop"
            `Quick
            test_noop_before_equals_after_marks_noop
        ; Alcotest.test_case
            "after>before marks noop"
            `Quick
