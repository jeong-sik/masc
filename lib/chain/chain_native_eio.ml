      ...
      let result =
        if tool_defs = [] then
          Oas_worker.run_model_by_label ~model_label ~goal:prompt ~system_prompt
            ~max_turns:1 ~priority:Oas.Llm_provider.Request_priority.Interactive ~sw:runtime.sw ?net:runtime.mcp_state.Mcp_server.net ()
        else
          (Oas_worker.run_model_with_masc_tools ~model_label ~goal:prompt
              ...
              ~max_turns:1 ~priority:Oas.Llm_provider.Request_priority.Interactive ~sw:runtime.sw
