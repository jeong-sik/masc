...
let test_shard_base_tools_registered_for_help () =
  List.iter
    (fun (tool : Types.tool_schema) ->
       ...
       | Some entry -> check string (tool.name ^ " help name") tool.name entry.name
...
let test_all_tool_names_omit_removed_mode_tools () =
  ...
  List.iter
    ...
    [ "masc_switch_mode"; "masc_get_config"; "masc_tool_enable"; "masc_tool_disable" ]
;;
...
let test_is_tool_visible_pause () =
  ...
  check
    bool
    "pause visible with include_hidden"
    true
  ...
  run
    ...
    [ ( "schema_registry"
      , [ test_case "dedupe_schemas" `Quick test_dedupe_schemas
        ; test_case "raw schemas non-empty" `Quick test_raw_all_tool_schemas_non_empty
        ; test_case "all schemas non-empty" `Quick test_all_tool_schemas_non_empty
        ; test_case
            "all_tool_names contains pause"
            `Quick
            test_all_tool_names_contains_pause
        ; test_case
            "all_tool_names contains approval_get"
            `Quick
            test_all_tool_names_contains_approval_get
        ; test_case
            "shard base tools registered for help"
            `Quick
            test_shard_base_tools_registered_for_help
        ; test_case
            "approval get requires admin permission"
            `Quick
            test_approval_get_requires_admin_permission
        ; test_case
            "removed mode tools omitted"
            `Quick
            test_all_tool_names_omit_removed_mode_tools
        ; test_case
            "visible is subset of all"
            `Quick
