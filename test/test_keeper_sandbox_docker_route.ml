...
let assert_docker_route_fires ~config ~meta ~playground =
  ...
  List.iter
    (fun (op, args) ->
      let raw =
        Keeper_exec_shell.handle_tool_search_files ~turn_sandbox_factory:None ~exec_cache:None ~config ~meta ~args
      in
...
let test_bash_routes_through_docker () =
  ...
  let raw =
    Keeper_exec_shell.handle_tool_search_files_ir ~turn_sandbox_factory:(Some factory)
...
let test_bash_legacy_skips_docker () =
  ...
  let raw =
    Keeper_exec_shell.handle_tool_search_files_ir ~turn_sandbox_factory:None
      ~turn_sandbox_factory_git:None ~exec_cache:None ~config ~meta
