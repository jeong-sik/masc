...
describe('filterAttributionEvents', () => {
  const events: readonly AttributionEvent[] = [
    ...
    makeEvent({ gate: 'agent_core_completion', origin: 'nondet', outcome: { kind: 'passed' } }),
  ]
  ...
  it('matches by origin', () => {
    ...
    expect(result.map(r => r.attribution.gate)).toEqual(['verification', 'agent_core_completion'])
  })
