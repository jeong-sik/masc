...
let agent_type_of_mention mention =
  ...
  match parts with
  | [] -> mention
  | base :: _ -> if String.length base > 0 then base else mention
;;
...
let broadcast_re =
  Re.(
    compile
      (seq
         [ str "@@"; group (rep1 (alt [ rg 'a' 'z'; rg 'A' 'Z'; rg '0' '9'; char '_' ])) ]))
;;
...
let mention_re =
  Re.(
    compile
      (seq
         [ char '@'
         ; group (rep1 (alt [ rg 'a' 'z'; rg 'A' 'Z'; rg '0' '9'; char '_'; char '-' ]))
         ]))
;;
...
let resolve_targets mode ~available_agents =
  ...
  | Broadcast agent_type ->
    (* All agents of type *)
    available_agents |> List.filter (fun name -> agent_type_of_mention name = agent_type)
;;
         ...
         [ (* Start of string or non-mention character *)
           group
             (alt
                [ bos
                ; compl
                    [ rg 'A' 'Z'; rg 'a' 'z'; rg '0' '9'; char '@'; char '_'; char '-' ]
                ])
         ; no_case (seq [ char '@'; str target ])
         ; (* End of string or non-mention character *)
           group
             (alt
                [ eos; compl [ rg 'A' 'Z'; rg 'a' 'z'; rg '0' '9'; char '_'; char '-' ] ])
         ]))
;;
...
let is_mentioned target content =
  ...
  if target = "" then false else Re.execp (target_re target) content
;;
