...
let extract_variables template =
  ...
  let vars = Re.all regex template |> List.map (fun g -> Re.Group.get g 1) in
             ...
             try
               ...
               | Ok entry ->
                 ...
                 let versions =
                   ...
                   | Some vs ->
                     if List.mem entry.version vs then vs else entry.version :: vs
                   | None -> [ entry.version ]
                 in
                 Hashtbl.replace version_index entry.id versions
               | Error msg -> Log.Misc.error "Failed to parse %s: %s" file msg
             with
...
let set_markdown_dir dir = markdown_dir := Some dir
...
let prompt_markdown_path key =
  if not (is_valid_prompt_key key)
  then None
  else Option.map (fun dir -> Filename.concat dir (key ^ ".md")) !markdown_dir
;;
...
let register (entry : prompt_entry) : unit =
  with_mutex (fun () ->
    ...
    let entry =
      if entry.variables = []
      then { entry with variables = extract_variables entry.template }
      else entry
    in
    let key = make_key ~id:entry.id ~version:entry.version in
    Hashtbl.replace registry key entry;
...
let list_all () : prompt_entry list =
  with_mutex (fun () -> Hashtbl.fold (fun _ entry acc -> entry :: acc) registry [])
;;

(** List all prompt IDs (unique, without versions) *)
let list_ids () : string list =
  with_mutex (fun () -> Hashtbl.fold (fun id _ acc -> id :: acc) version_index [])
;;
...
let exists ~id ?version () : bool =
  with_mutex (fun () ->
    ...
    | None -> Hashtbl.mem version_index id)
;;
      ...
      then (
        ...
        (match Hashtbl.find_opt version_index id with
         | Some vs ->
           ...
           if new_vs = []
           then Hashtbl.remove version_index id
  ...
  with_mutex (fun () ->
    ...
    | Some entry ->
      ...
      let new_metrics =
        ...
        | None -> { usage_count = 1; avg_score = score; last_used = now }
        | Some m ->
          let new_count = m.usage_count + 1 in
          let new_avg =
            ((m.avg_score *. float_of_int m.usage_count) +. score)
            /. float_of_int new_count
          in
...
(* Static unresolved-var detector — same pattern, hoisted out of
   ...
   of per render call. *)
let unresolved_var_re = Re.Pcre.re {|\{\{[^}]+\}\}|} |> Re.compile
...
let render_template ~template ~vars () : (string, string) result =
  try
    ...
    if has_unresolved then Error "Unresolved variables in template" else Ok !result
  with
  | e -> Error (Printf.sprintf "Render error: %s" (Printexc.to_string e))
;;
...
let stats () : registry_stats =
  with_mutex (fun () ->
        ...
        (fun acc entry ->
           ...
           | Some m ->
             (match acc with
              ...
              | Some (_, count) when m.usage_count > count ->
                Some (entry.id, m.usage_count)
              | _ -> acc))
        None
        all_entries
      |> Option.map fst
    ...
    in
    let avg_usage =
      if total > 0 then float_of_int total_usage /. float_of_int total else 0.0
    in
    { total_prompts = total
  ...
  with_mutex (fun () ->
    ...
    | Some dir when Sys.file_exists dir && Sys.is_directory dir ->
      ...
      Array.iter
        (fun file ->
           if Filename.check_suffix file ".json"
           then Sys.remove (Filename.concat dir file))
        files
    | None | Some _ -> ())
;;

(** Count of registered prompts (all versions) *)
let count () : int = with_mutex (fun () -> Hashtbl.length registry)

(** Count of unique prompt IDs *)
let count_unique () : int = with_mutex (fun () -> Hashtbl.length version_index)

(** Export registry to JSON *)
let to_json () : Yojson.Safe.t =
  with_mutex (fun () ->
    let entries =
      Hashtbl.fold (fun _ entry acc -> prompt_entry_to_yojson entry :: acc) registry []
    in
    `List entries)
;;
...
let build_resolved_from_snapshot ~key ~override_value ~default_value ~file_value =
...
let load_prompts_from_directory dir =
  ...
  then (
    ...
    Array.iter
      (fun file ->
         if Filename.check_suffix file ".md"
         then (
           let key = Filename.remove_extension file in
           if is_valid_prompt_key key
           then (
             let path = Filename.concat dir file in
             try
               let content = In_channel.with_open_text path In_channel.input_all in
               ...
               | Some description ->
                 let category =
                   Option.value ~default:"general" (List.assoc_opt "category" meta_pairs)
                 in
...
let register_default ~key ~default ~description ?(category = "general") () =
  with_mutex (fun () ->
    Hashtbl.replace
      meta_tbl
      key
      { description; category; required_file = false; template_variables = [] };
    let entry =
      { id = key
      ; template = default
      ; version = "default"
    ...
    let versions =
      match Hashtbl.find_opt version_index key with
...
let render_prompt_template key vars =
  ...
  if String.trim template = ""
  then Error (Printf.sprintf "Prompt '%s' is missing" key)
  else render_template ~template ~vars ()
;;
...
let apply_override_validated key value =
  ...
  if not (is_valid_prompt_key key)
  then Error "Invalid prompt key"
  else if trimmed = ""
  then Error "Prompt cannot be empty"
  else if String.length trimmed > 10000
  then Error "Prompt too long (max 10000 chars)"
  else
    with_mutex (fun () ->
...
let clear_prompt_override key = with_mutex (fun () -> Hashtbl.remove override_tbl key)
...
let prompts_json () = `Assoc [ "prompts", `List (list_prompts ()) ]
...
let persist_overrides base_path =
  ...
  let json =
    with_mutex (fun () ->
      Hashtbl.fold (fun key value acc -> (key, `String value) :: acc) override_tbl [])
  in
          ...
          (fun (key, value) ->
             ...
             | `String s ->
               (match apply_override_validated key s with
                | Ok () -> ()
                | Error reason ->
                  Log.Misc.warn "prompt override restore: skipping %s: %s" key reason)
             | `Null | `Bool _ | `Int _ | `Intlit _ | `Float _ | `Assoc _ | `List _ -> ())
          pairs
      | `Null | `Bool _ | `Int _ | `Intlit _ | `Float _ | `String _ | `List _ -> ()
    with
    | Eio.Cancel.Cancelled _ as e -> raise e
    | exn -> Log.Misc.warn "prompt override restore failed: %s" (Printexc.to_string exn))
;;
