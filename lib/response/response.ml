...
let to_string (r : t) : string = Yojson.Safe.pretty_to_string (to_json r)

(** Create a success response *)
let ok ?(message = "OK") (data : Yojson.Safe.t) : t =
  { success = true; data; message; errors = []; timestamp = Time_compat.now () }
;;

(** Create a success response with warnings *)
let ok_with_warnings ?(message = "OK") ~warnings (data : Yojson.Safe.t) : t =
  { success = true; data; message; errors = warnings; timestamp = Time_compat.now () }
;;
...
let error ?(data = `Null) ~code ~message ?(hints = []) () : t =
  ...
  ; data
  ; message
  ; errors = [ { code; severity = Fatal; message; recovery_hints = hints } ]
...
let errors ?(data = `Null) ~message (error_list : error_detail list) : t =
  { success = false; data; message; errors = error_list; timestamp = Time_compat.now () }
;;
...
let not_found ~resource ~id : t =
  error
    ...
    ~hints:
      ...
      ; Printf.sprintf
          "Use list_%ss to see available items"
...
let already_exists ~resource ~id : t =
  error
    ...
    ~hints:
      [ Printf.sprintf
          "Use a different ID for the new %s"
          (String.lowercase_ascii resource)
      ; Printf.sprintf
          "Or update the existing %s instead"
          (String.lowercase_ascii resource)
      ]
    ()
;;
...
let permission_denied ~action ~resource : t =
  error
    ...
    ~hints:[ "Check if you have the required permissions" ]
    ()
;;
...
let drift_detected ~similarity ~drift_type ~threshold ~details : t =
  let hints =
    ...
    | _ -> [ "Review the handoff process"; "Check for network or encoding issues" ]
  in
  { success = false
  ; data =
      `Assoc
  ...
  ; message =
      Printf.sprintf "Context drift detected (%.0f%% similarity)" (similarity *. 100.0)
  ; errors =
      [ { code = "DRIFT_DETECTED"
        ; severity = Warning
        ; message = details
...
let handoff_verified ~similarity : t =
  ok
    ...
    (`Assoc [ "similarity", `Float similarity; "verified", `Bool true ])
;;
...
let task_already_claimed ~task_id ~claimed_by : t =
  error
    ...
    ~data:(`Assoc [ "task_id", `String task_id; "claimed_by", `String claimed_by ])
    ~hints:
