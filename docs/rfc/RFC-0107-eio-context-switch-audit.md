...
### 2.2 Runtime/keeper turn-scoped — *turn-lifetime intent*
...
총 **15 callsite**. 이들은 *turn 단위로 자르고 싶은* 작업이지만 *현재는 root_sw 에 매달림* (Phase C 문제 진앙). wrap 이 도입되고 위 §2.1 의 server 계열과 분리되면 이들은 자연스럽게 turn_sw 로 흡수됨.
...
**합계**: 25 callsite, 9:15:1 = server : keeper-turn : misc.
...
- **추가 비용**: 25 callsite 그대로 유지 (`get_switch_opt` 내부 구현만 변경). ~80 LoC + Eio 버전 확인.

### Option (4) — 명시적 `~sw` propagation

25 callsite 의 caller chain 을 따라 `~sw:Eio.Switch.t` 인자를 propagate. 각 함수 signature 변경.
...
| 디버깅 cost | 중 | 저 | 저 |
| Eio 권고 부합 | △ | ◯ | ◎ |
| 후속 cleanup | §2.1 callsite 9개 refactor | 25 callsite 유지 가능 | 25 callsite 전부 변경 |
...
- 25 callsite refactor (Phase C.1 또는 별도 RFC)

## 8. 기존 패턴 — `try_provider.ml:406`

`lib/keeper/keeper_turn_driver_try_provider.ml:406` 의 `Eio.Switch.run (fun attempt_sw -> ...)` runtime attempt 패턴은 *작동 중*. 그 이유:
...
이 사실이 Option (3) (fiber-local) 의 매력을 높임 — 25 callsite 안 건드리고 attempt_sw 처럼 *암묵적 propagation* 만 추가.
