...
const report = data => {const task=fetch(base+'/report',{method:'POST',body:JSON.stringify(data)});pendingReports.add(task);task.finally(()=>pendingReports.delete(task));return task;};
(async()=>{
 ...
 for(const [name,path] of [['document-change','/first'],['same-url-reload','/first'],['redirect','/redirect'],['hash','/final#hash'],['destination-error','/broken']]) {
  ...
  const onUpdated=(id,change,updated)=>{if(id===tab.id){events.push({kind:'tab-updated',change,url:updated.url,status:updated.status});}};
  ...
  const onCommitted=d=>{if(owned(d)){commit=d;events.push({kind:'committed',details:d});if(d.documentId!==sourceFrame.documentId)resolveReady(d);}};
  const onReady=d=>{if(owned(d)&&commit && commit.documentId===d.documentId && commit.documentId!==sourceFrame.documentId){events.push({kind:'dom-ready',details:d});}};
  const onError=d=>{if(owned(d)){events.push({kind:'navigation-error',details:d});report({kind:'navigation-error-observed',name,sourceFrame,details:d,error_policy:'observation_only'});if(name==='destination-error' && d.url===base+'/broken')destinationError=d;}};
  ...
  try {
   ...
   if(name==='destination-error'){
    if(event.url!==base+'/broken' || event.documentId===sourceFrame.documentId)throw new Error('new destination commit absent');
    const finalFrame=await browser.webNavigation.getFrame({tabId:tab.id,frameId:0});
    let errorPageRead;try{errorPageRead={result:(await browser.tabs.executeScript(tab.id,{code:'('+sceneSource+')({mode:"read",view:"content",maxChars:20000})',runAt:'document_end'}))[0]};}catch(error){errorPageRead={error:String(error)};}
    await report({kind:'error-page-settlement',finalFrame,errorPageRead});
    if(!errorPageRead.error)throw new Error('expected restricted error page read rejection');
    await report({kind:'case',name,effects_requested,sourceFrame,lifecycle:events,error_policy:'new_commit_then_read'});continue;
   }
   if(name==='hash'){const hashScene=(await browser.tabs.executeScript(tab.id,{code:'('+sceneSource+')({mode:"read",view:"content",maxChars:20000})',runAt:'document_end'}))[0];if(hashScene.url!==base+'/final#hash')throw new Error('hash scene URL mismatch');if(event.documentId!==sourceFrame.documentId)throw new Error('hash replaced native document');await report({kind:'case',name,effects_requested,sourceFrame,lifecycle:events});continue;}
