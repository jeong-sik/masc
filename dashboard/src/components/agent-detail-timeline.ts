...
function ToolCallEventRow({ evt, idx }: { evt: AgentTimelineEvent; idx: number }) {
  ...
  return html`
    <div class="flex flex-col py-1.5 px-2 rounded hover:bg-[var(--white-4)] transition-colors animate-[activityFadeIn_0.25s_ease-out]" key=${idx}>
      ...
      ${args ? html`
        <div class="ml-8 mt-0.5 text-3xs text-[var(--text-dim)] font-mono truncate" title=${typeof args === 'string' ? args : formatArgs(args)}>
      ...
      ${events.length === 0
        ...
        : html`
            ...
            ${filtered.length === 0
              ...
              : html`
                  <div role="log" aria-label="활동 타임라인" class="flex flex-col gap-0.5 max-h-100 overflow-y-auto custom-scrollbar">
