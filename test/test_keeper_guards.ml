...
let test_render_inline_skip_reason () =
  ...
  check
    bool
    "contains tool=keeper_fs_read"
    true
    (contains_substring s "tool=keeper_fs_read");
  check bool "contains code=streak_gate" true (contains_substring s "code=streak_gate");
  check
    bool
    "contains source=keeper_hook"
    true
...
let test_deny_guard_blocks () =
  ...
  let hook =
    KG.deny_guard
      ~meta_ref
      ~on_gate_decision:no_gate_observer
...
let test_deny_guard_notifies_gate_observer () =
  ...
  let hook = KG.deny_guard ~meta_ref ~on_gate_decision ~denied:[ "dangerous_tool" ] in
  let d =
    invoke
      hook
      (pre_tool_use_event
  ...
  | [ event ] ->
    ...
    check
      ...
      (match event.KG.input with
       ...
       | _ -> false)
  | events -> failf "expected one observer event, got %d" (List.length events)
;;

let test_deny_guard_continues () =
  let meta_ref = make_meta_ref "test_keeper" in
  let hook =
    KG.deny_guard ~meta_ref ~on_gate_decision:no_gate_observer ~denied:[ "other_tool" ]
  in
...
let test_cost_guard_blocks () =
  let meta_ref = make_meta_ref "test_keeper" in
  let hook =
    KG.cost_guard ~meta_ref ~on_gate_decision:no_gate_observer ~max_cost_usd:(Some 0.10)
  in
  let d =
    invoke hook (pre_tool_use_event ~tool_name:"expensive" ~accumulated_cost_usd:0.15 ())
  in
...
let test_cost_guard_under_limit () =
  ...
  in
  let d =
    invoke hook (pre_tool_use_event ~tool_name:"cheap" ~accumulated_cost_usd:0.05 ())
  in
...
let test_cost_guard_disabled () =
  let meta_ref = make_meta_ref "test_keeper" in
  let hook =
    KG.cost_guard ~meta_ref ~on_gate_decision:no_gate_observer ~max_cost_usd:None
  in
  let d =
    invoke hook (pre_tool_use_event ~tool_name:"any" ~accumulated_cost_usd:999.0 ())
  in
...
let test_streak_guard_under_threshold () =
  ...
  let hook =
    KG.streak_guard ~meta_ref ~on_gate_decision:no_gate_observer ~state ~threshold:5
  in
...
let test_streak_guard_resets_on_different_tool () =
  ...
  let state = KG.make_streak_state () in
  let hook =
    KG.streak_guard ~meta_ref ~on_gate_decision:no_gate_observer ~state ~threshold:3
  in
...
let test_custom_guard_blocks () =
  ...
  in
  let hook = KG.custom_guard ~meta_ref ~on_gate_decision:no_gate_observer ~guard in
    ...
    let d =
      invoke
        ...
        (pre_tool_use_event
           ...
           ())
    in
    check string "high-risk tool -> ApprovalRequired" "ApprovalRequired" (decision_kind d);
    match !observed with
    | [ event ] ->
...
let test_compose_all_short_circuits_at_first_override () =
  ...
  let d =
    invoke
      hooks
      (pre_tool_use_event ~tool_name:"blocked_tool" ~accumulated_cost_usd:999.0 ())
  in
  ...
  check
    bool
    "short-circuit preserves first reason"
    true
    (contains_substring text "code=keeper_deny")
;;
...
let test_compose_all_preserves_order () =
  ...
  in
  let _ =
    invoke hooks (pre_tool_use_event ~tool_name:"x" ~accumulated_cost_usd:0.05 ())
  in
  let _ =
    invoke hooks (pre_tool_use_event ~tool_name:"x" ~accumulated_cost_usd:0.05 ())
  in
...
let test_hooks_of_pre_tool_use_slots () =
  let hooks = KG.hooks_of_pre_tool_use (fun _ -> Agent_sdk.Hooks.Continue) in
  ...
  run
    ...
    ; ( "streak_guard"
      , [ test_case "under threshold -> continue" `Quick test_streak_guard_under_threshold
        ...
        ; test_case
            "resets on different tool"
            `Quick
            test_streak_guard_resets_on_different_tool
    ...
    ; ( "slot_helpers"
      , [ test_case
            "hooks_of_pre_tool_use only fills pre_tool_use"
            `Quick
