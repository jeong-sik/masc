...
```text
GET /mcp                     (유지, MCP transport observer/workspace session stream)
...
```ocaml
(* lib/sse.ml 발췌 — #12129 *)
type session_kind = Observer | Workspace session | Presence
type broadcast_target = All | Observers | Workspace sessions | Presence_only
