...
let test_generate_token_hex () =
  let token = Auth.generate_token () in
  let is_hex =
    String.for_all (fun c -> (c >= '0' && c <= '9') || (c >= 'a' && c <= 'f')) token
  in
...
let test_sha256_hash_hex () =
  let hash = Auth.sha256_hash "test" in
  let is_hex =
    String.for_all (fun c -> (c >= '0' && c <= '9') || (c >= 'a' && c <= 'f')) hash
  in
  check bool "all hex" true is_hex
;;
...
let test_same_origin_browser_request_rejects_missing_origin () =
  ...
  | Error e ->
    fail (Printf.sprintf "expected Unauthorized, got %s" (Types.masc_error_to_string e))
;;

let test_same_origin_browser_request_allows_matching_origin () =
  let module Server_auth = Masc_mcp.Server_auth in
  let headers =
    Httpun.Headers.of_list [ "host", "127.0.0.1:8935"; "origin", "http://127.0.0.1:8935" ]
  in
  let request = Httpun.Request.create ~headers `POST "/api/v1/operator/action" in
  match Server_auth.ensure_same_origin_browser_request request with
  | Ok () -> ()
...
let test_same_origin_browser_request_rejects_cross_origin () =
  let module Server_auth = Masc_mcp.Server_auth in
  let headers =
    Httpun.Headers.of_list [ "host", "127.0.0.1:8935"; "origin", "https://evil.example" ]
  in
  let request = Httpun.Request.create ~headers `POST "/api/v1/operator/action" in
  match Server_auth.ensure_same_origin_browser_request request with
...
let test_same_origin_https_tunnel_same_host () =
  ...
  let headers =
    Httpun.Headers.of_list
      [ "host", "masc.crying.pictures"; "origin", "https://masc.crying.pictures" ]
  in
  let request = Httpun.Request.create ~headers `POST "/api/v1/operator/action" in
  match Server_auth.ensure_same_origin_browser_request request with
  | Ok () -> ()
...
let test_same_origin_allows_loopback_alias_same_port () =
  let module Server_auth = Masc_mcp.Server_auth in
  let headers =
    Httpun.Headers.of_list [ "host", "127.0.0.1:8935"; "origin", "http://localhost:8935" ]
  in
  let request = Httpun.Request.create ~headers `POST "/api/v1/operator/action" in
  match Server_auth.ensure_same_origin_browser_request request with
  | Ok () -> ()
...
let test_same_origin_allows_allowlisted_dashboard_dev_origin () =
  let module Server_auth = Masc_mcp.Server_auth in
  let headers =
    Httpun.Headers.of_list [ "host", "localhost:8935"; "origin", "http://localhost:5173" ]
  in
  let request = Httpun.Request.create ~headers `POST "/api/v1/operator/action" in
  match Server_auth.ensure_same_origin_browser_request request with
  | Ok () -> ()
...
let test_same_origin_rejects_non_allowlisted_loopback_cross_port () =
  let module Server_auth = Masc_mcp.Server_auth in
  let headers =
    Httpun.Headers.of_list [ "host", "localhost:9000"; "origin", "http://localhost:8935" ]
  in
  let request = Httpun.Request.create ~headers `POST "/api/v1/operator/action" in
  match Server_auth.ensure_same_origin_browser_request request with
...
let test_same_origin_rejects_different_explicit_port_on_public_host () =
  ...
  let headers =
    Httpun.Headers.of_list
      [ "host", "example.com:9000"; "origin", "http://example.com:8935" ]
  in
  let request = Httpun.Request.create ~headers `POST "/api/v1/operator/action" in
  match Server_auth.ensure_same_origin_browser_request request with
...
let test_same_origin_allows_explicit_default_port_https () =
  let module Server_auth = Masc_mcp.Server_auth in
  let headers =
    Httpun.Headers.of_list [ "host", "example.com:443"; "origin", "https://example.com" ]
  in
  let request = Httpun.Request.create ~headers `POST "/api/v1/operator/action" in
  match Server_auth.ensure_same_origin_browser_request request with
  | Ok () -> ()
...
let test_same_origin_allows_explicit_default_port_http () =
  let module Server_auth = Masc_mcp.Server_auth in
  let headers =
    Httpun.Headers.of_list [ "host", "localhost:80"; "origin", "http://localhost" ]
  in
  let request = Httpun.Request.create ~headers `POST "/api/v1/operator/action" in
  match Server_auth.ensure_same_origin_browser_request request with
  | Ok () -> ()
...
let test_http_auth_token_from_header_only () =
  let module Server_auth = Masc_mcp.Server_auth in
  let headers = Httpun.Headers.of_list [ "authorization", "Bearer header-token" ] in
  let request =
    Httpun.Request.create ~headers `GET "/api/v1/tools/masc_board_vote?token=query-token"
  in
  check
    (option string)
    "header bearer token extracted"
    (Some "header-token")
...
let test_http_auth_accepts_internal_keeper_header () =
  ...
  check
    (option string)
    "internal keeper token extracted"
...
let test_observer_sse_auth_rejects_query_token_on_non_observer_path () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let request =
         Httpun.Request.create `GET ("/mcp?session_id=dash_test&token=" ^ raw_token)
       in
       match Server_auth.verify_mcp_observer_stream_auth ~base_path:dir request with
       ...
       | Error msg ->
         check
           bool
           "non-observer path still requires header"
           true
  ...
  with_env "MASC_HTTP_BASE_URL" "https://masc.crying.pictures" (fun () ->
    with_env "MASC_HOST" "127.0.0.1" (fun () ->
      clear_env "MASC_HTTP_AUTH_STRICT" (fun () ->
        check
          bool
          "tunnel base URL forces strict auth"
          true
  ...
  with_env "MASC_HTTP_BASE_URL" "http://127.0.0.1:8935" (fun () ->
    with_env "MASC_HOST" "127.0.0.1" (fun () ->
      clear_env "MASC_HTTP_AUTH_STRICT" (fun () ->
        check
          bool
          "localhost base URL no forced strict"
          false
          (SA.http_auth_strict_enabled ()))))
;;
    ...
    with_env "MASC_HOST" "127.0.0.1" (fun () ->
      with_env "MASC_HTTP_PORT" "8935" (fun () ->
        clear_env "MASC_HTTP_AUTH_STRICT" (fun () ->
          check
            bool
            "no base URL + loopback = no strict"
            false
            (SA.http_auth_strict_enabled ())))))
;;
  ...
  with_env "MASC_HTTP_AUTH_STRICT" "true" (fun () ->
    with_env "MASC_HOST" "127.0.0.1" (fun () ->
      clear_env "MASC_HTTP_BASE_URL" (fun () ->
        check
          bool
          "env flag forces strict regardless"
          true
          (SA.http_auth_strict_enabled ()))))
;;
...
let test_custom_dev_origin_allows_loopback_cross_port () =
  ...
  with_env "MASC_HTTP_DEV_MUTATION_ORIGINS" "http://localhost:4317" (fun () ->
    let headers =
      Httpun.Headers.of_list
        [ "host", "localhost:9000"; "origin", "http://localhost:4317" ]
    in
    let request = Httpun.Request.create ~headers `POST "/api/v1/operator/action" in
...
let test_public_read_cors_allows_matching_origin () =
  let module SA = Masc_mcp.Server_auth in
  let headers =
    Httpun.Headers.of_list [ "host", "localhost:9000"; "origin", "http://localhost:9000" ]
  in
...
let test_public_read_cors_rejects_cross_origin () =
  let module SA = Masc_mcp.Server_auth in
  let headers =
    Httpun.Headers.of_list [ "host", "localhost:9000"; "origin", "https://evil.example" ]
  in
  let request = Httpun.Request.create ~headers `GET "/api/v1/gate/events" in
  match SA.public_read_cors_origin_opt request with
  | None -> ()
  ...
  with_env "MASC_HTTP_DEV_MUTATION_ORIGINS" "http://localhost:4317" (fun () ->
    ...
    | Some origin ->
      Alcotest.(check string)
        "allowlisted loopback origin reflected"
        "http://localhost:4317"
        origin
...
let test_public_read_path_allows_generic_connector_status () =
  ...
  check
    bool
    "generic connector status is public read"
    true
...
let test_public_read_path_rejects_generic_connector_bind () =
  ...
  check
    bool
    "generic connector bind is not public read"
    false
...
let test_public_read_path_rejects_generic_connector_unbind () =
  ...
  check
    bool
    "generic connector unbind is not public read"
    false
...
let test_resolve_agent_name_prefers_token_for_generated_actor () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let request = Httpun.Request.create ~headers `POST "/mcp" in
       match
         SA.resolve_agent_name_for_auth ~base_path:dir request ~token:(Some raw_token)
       with
       | Ok (Some agent_name) ->
...
let test_resolve_agent_name_preserves_explicit_stable_actor () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let headers =
         Httpun.Headers.of_list
           [ "authorization", "Bearer " ^ raw_token; "x-masc-agent", "dashboard-admin" ]
       in
       let request = Httpun.Request.create ~headers `POST "/mcp" in
       match
         SA.resolve_agent_name_for_auth ~base_path:dir request ~token:(Some raw_token)
...
let test_sanitized_dashboard_actor_for_request_uses_token_owner () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let headers =
         Httpun.Headers.of_list
           [ "authorization", "Bearer " ^ raw_token; "x-masc-agent", "dashboard-admin-ㅊ" ]
       in
       let request = Httpun.Request.create ~headers `POST "/api/v1/gate/connector/bind" in
       check
         (option string)
         "token owner wins and remains cache-safe"
         (Some "stable-admin")
...
let test_resolve_agent_name_rejects_invalid_token () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let request = Httpun.Request.create ~headers `POST "/mcp" in
       match
         SA.resolve_agent_name_for_auth ~base_path:dir request ~token:(Some invalid_token)
       with
    ...
    (fun () ->
       ...
       let headers =
         Httpun.Headers.of_list
           [ "authorization", "Bearer " ^ raw_token; "x-masc-agent", "dashboard-admin" ]
       in
       let request = Httpun.Request.create ~headers `GET "/api/v1/dashboard/shell" in
...
let test_resolve_agent_name_uses_internal_keeper_header () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let headers =
         Httpun.Headers.of_list
           [ "x-masc-internal-token", raw_token; "x-masc-keeper-name", "sangsu" ]
       in
       let request = Httpun.Request.create ~headers `POST "/mcp" in
       match
         SA.resolve_agent_name_for_auth ~base_path:dir request ~token:(Some raw_token)
       ...
       | Ok (Some agent_name) ->
         check
           string
           "internal keeper resolves canonical agent"
           "keeper-sangsu-agent"
           agent_name
...
let test_resolve_agent_name_rejects_internal_keeper_without_name () =
  ...
  Fun.protect
    ...
    (fun () ->
       ignore (Auth.enable_auth dir ~require_token:true ~agent_name:"bootstrap-admin");
       let raw_token = Auth.ensure_internal_keeper_token dir in
       let headers = Httpun.Headers.of_list [ "x-masc-internal-token", raw_token ] in
       let request = Httpun.Request.create ~headers `POST "/mcp" in
       match
         SA.resolve_agent_name_for_auth ~base_path:dir request ~token:(Some raw_token)
       with
  ...
  run
    ...
    ; ( "permission_for_tool"
      , [ test_case "init" `Quick test_permission_for_tool_init
        ...
        ; test_case "autoresearch_stop" `Quick test_permission_for_tool_autoresearch_stop
        ; test_case
            "keeper_create_from_persona"
            `Quick
