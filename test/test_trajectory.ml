...
let with_tmpdir f =
  let dir =
    Filename.concat
      ...
      (Printf.sprintf "test_trajectory_%d" (Random.int 100000))
  in
...
let test_entropy_triggered () =
  with_tmpdir (fun dir ->
    ...
    let entropy = Trajectory.detect_entropy ~threshold:3 acc "keeper_bash" in
    match entropy with
    | Some (_name, count) -> Alcotest.(check int) "entropy count" 4 count
...
let test_task_id_in_trajectory_json () =
  with_tmpdir (fun dir ->
    ...
    | `String s -> Alcotest.(check string) "task_id in json" "task-json-test" s
...
let test_hourly_single_bucket () =
  let entries =
    [ { (mk_entry "keeper_bash" 100 0.0 "2026-04-06T10:05:00Z") with
        Trajectory.ts = 1743937500.0
      }
    ; { (mk_entry "keeper_bash" 100 0.0 "2026-04-06T10:30:00Z") with
        Trajectory.ts = 1743939000.0
      }
    ]
  in
...
let test_hourly_with_errors () =
  let entries =
    ...
    ; { (mk_entry ~error:(Some "fail") "keeper_bash" 100 0.0 "2026-04-06T10:30:00Z") with
        Trajectory.ts = 1743939000.0
      }
    ]
  in
...
let test_hourly_bucket_json () =
  let b : Trajectory.hourly_bucket =
    { hour = "2026-04-06T10:00:00Z"; call_count = 5; error_count = 1 }
  in
...
let test_entry_to_json_includes_contract_and_radius () =
  ...
  let action_radius =
    Keeper_runtime_contract.action_radius_json
      ...
      ()
  in
  let json = Trajectory.entry_to_json ~runtime_contract ~action_radius entry in
  let open Yojson.Safe.Util in
  Alcotest.(check string)
    "runtime keeper"
    "alpha"
  ...
  Alcotest.(check string)
    ...
    "/tmp/work"
    (json
     |> member "action_radius"
     |> member "observed_paths"
     |> to_list
     |> List.hd
     |> to_string)
;;
...
let test_read_entries_since () =
  with_tmpdir (fun dir ->
    ...
    let entries =
      Trajectory.read_entries_since ~masc_root ~keeper_name:keeper ~since:1500.0
    in
...
let test_read_entries_since_result_parses_gate_summary () =
  with_tmpdir (fun dir ->
    ...
    close_out oc;
    let result =
      Trajectory.read_entries_since_result ~masc_root ~keeper_name:keeper ~since:0.0
    in
...
let test_read_entries_since_no_dir () =
  with_tmpdir (fun dir ->
    let entries =
      Trajectory.read_entries_since ~masc_root:dir ~keeper_name:"nonexistent" ~since:0.0
    in
...
let () =
  Alcotest.run
    ...
    ; "finalize", [ Alcotest.test_case "finalize completed" `Quick test_finalize ]
    ; "outcome", [ Alcotest.test_case "outcome_to_string" `Quick test_outcome_to_string ]
    ; ( "task_id"
    ...
    ; ( "aggregate_tool_stats"
      , [ Alcotest.test_case "basic aggregation" `Quick test_aggregate_basic
        ; Alcotest.test_case
            "with errors and rejected gates"
            `Quick
    ...
    ; ( "json_serialization"
      , [ Alcotest.test_case "tool_stat to json" `Quick test_tool_stat_json_roundtrip
        ...
        ; Alcotest.test_case
            "entry carries runtime/action telemetry"
            `Quick
