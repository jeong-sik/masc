...
```mermaid
flowchart LR
  I["models=[A:w10, B:w20, C:w5]"] --> W[compute effective weights]
  W --> G{전부 cooldown?}
  G -->|no| O["ordered=weighted shuffle (B 확률 높음)"]
  G -->|yes| G2[starvation guard: 전부 w=1 복원]
```
...
`~/.masc/config/cascade.json`의 **2026-04-17 스냅샷**. `*_strategy` 키가 없으면 `Failover` 기본값.

> **Drift 경고**: 이 표는 live cascade.json 변경 시 실효성을 잃는다. 표와 실제가 어긋나면 아래 one-liner로 재생성 후 표 전체를 덮어쓰는 것을 원칙으로 한다. 표는 검색/grep용 캐시일 뿐 SSOT 아님.
...
공통 키(`{profile}_models`, `_max_cycles`, `_temperature` 등)는
[`docs/spec/14-configuration.md`](../spec/14-configuration.md)가 SSOT. 여기서는
**전략별 추가 키와 enum만** 나열 (drift 위험 최소화).
...
| `weighted_random` | `{profile}_weighted_entries` | `object[]` | `{label, weight}` 리스트 (`_models` 대신 쓸 수 있음) |

### 전략 값 enum

`failover` | `capacity_aware` | `weighted_random` | `circuit_breaker_cycling` | `priority_tier` | `sticky` | `round_robin`

알 수 없는 값이면 warning 로그 남기고 `failover`로 폴백
(`cascade_config.ml` `parse_kind_or_default`).
...
**대응**: `curl -s http://localhost:8935/api/v1/cascade/strategy_trace?cascade=<name> | jq` 로 최근 decision 확인.
원인 1이면 strategy 변경, 2면 설계 준수라 provider/정책 조정, 3이면 로컬 FD/port 리밋 조사 (`ulimit -n`, `lsof -p <pid>`).

### "전부 cooldown이라 빈 리스트"

`Circuit_breaker_cycling`만 발생. `StrategyFilteredEmptyStorm` 알림이 울린다.

**대응**: provider 추가가 근본 해결. 즉시 완화는 `cascade_health_tracker`의 cooldown 상수 조정 (코드 수정 + 재빌드 필요. cascade.json으로 노출 안 됨 — 확장 필요 시 별도 PR).

### "Sticky가 고정 안 된다"

`record_choice`는 Sticky일 때만 state를 쓴다(`cascade_strategy.ml` `record_choice`). 다른 전략은 기록 안 함. Sticky로 전환 후 최초 1회 성공이 있어야 state가 생긴다. 초기 상태는 failover로 동작.

**대응**: 최초 call이 실패하면 영원히 sticky state 부재 → 매번 failover. 설정 바꾼 뒤 1회 dry-run을 보내 state를 심을 것.

### "Round_robin인데 provider가 안 바뀐다"

cursor는 process 내부 상태. MASC 재시작 시 reset. cursor 증가는 `rotate_round_robin`이 호출될 때만 — 한 cycle 내 여러 candidate를 순회해도 cursor는 그대로.

**대응**: `strategy_trace`에서 `cascade_name`별 chosen provider 분포 확인. 특정 provider로 쏠리면 재시작 타이밍 문제. 재현 재검증이 필요하면 기대 cursor 기록 후 조작.
