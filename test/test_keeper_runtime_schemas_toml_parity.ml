...
let expected =
    [ {|keeper_artifact_read|}, {|Read an explicit byte range from a [masc:blob ...] ToolResult. Use the marker's sha256, then continue with the returned next_offset. Text pages use UTF-8; arbitrary bytes use base64. The full artifact is never restored into model history.|}, {|{"additionalProperties":false,"properties":{"max_bytes":{"default":65536,"description":"Maximum source bytes to expose in this model-visible page.","maximum":65536,"minimum":1,"type":"integer"},"offset":{"default":0,"description":"Byte offset to read from.","minimum":0,"type":"integer"},"sha256":{"description":"Exact sha256 from a [masc:blob ...] ToolResult.","type":"string"}},"required":["sha256"],"type":"object"}|}
