...
let make_review_required_verdict ?(run_id = "review-run-001") () : CT.contract_verdict =
...
let claim_and_start config ~agent_name ~task_id =
  (match Coord.transition_task_r config ~agent_name ~task_id ~action:Types.Claim () with
   | Ok _ -> ()
   | Error err -> Alcotest.fail (Types.show_masc_error err));
  match Coord.transition_task_r config ~agent_name ~task_id ~action:Types.Start () with
  | Ok _ -> ()
  | Error err -> Alcotest.fail (Types.show_masc_error err)
;;
