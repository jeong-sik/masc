...
- An access refusal (HTTP 401) on a lane's head no longer demotes it. The refusal was held as candidate evidence that has no expiry and that only the candidate's own answer clears, so while a sibling answered the head was never dispatched again, and a key restored through the environment did not bring it back before a restart. The refusal now rotates to the next candidate within the turn and leaves no evidence, and every new turn tries the head first (RFC-0458 §3.4, §6) (#38096).
