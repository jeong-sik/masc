...
let test_delta_json_all_replacement_ops () =
  ...
  | Error err ->
    Alcotest.failf "expected delta to apply: %s" (Masc_agent_core.Error.to_string err)
;;
...
let test_delta_json_null_and_rejected_legacy_limit_paths () =
  ...
  (match Checkpoint.apply_delta base decoded with
   ...
   | Error err ->
     Alcotest.failf "expected null delta to apply: %s" (Masc_agent_core.Error.to_string err));
...
let test_delta_roundtrip_preserves_message_metadata () =
  ...
  | Error err ->
    Alcotest.failf "expected delta to apply, got %s" (Masc_agent_core.Error.to_string err)
  | Ok rebuilt ->
