...
- Dashboard operator verdicts run as `operator:<actor>` (`server_routes_http_routes_verification.ml:41-43`) — a namespace that never equals an assignee identity key, sidestepping the self-approval check for a human who also drives an assignee keeper.
