...
let check_reject name (result : Masc_mcp.Oas.Hooks.approval_decision) =
  ...
  | Approve ->
    Alcotest.fail
      (Printf.sprintf
         "%s: expected Reject, got Approve (fail-open bug #7883 regressed)"
         name)
  | Edit _ -> Alcotest.fail (Printf.sprintf "%s: expected Reject, got Edit" name)
;;
...
let check_approve name (result : Masc_mcp.Oas.Hooks.approval_decision) =
  ...
  | Approve -> ()
  | Reject reason ->
    Alcotest.fail (Printf.sprintf "%s: expected Approve, got Reject %S" name reason)
  | Edit _ -> Alcotest.fail (Printf.sprintf "%s: expected Approve, got Edit" name)
;;
...
let test_reject_by_default_rejects_any_tool () =
  ...
  List.iter
    (fun tool_name ->
       let decision = AC.reject_by_default ~tool_name ~input:(`Assoc []) in
...
let test_reject_by_default_reason_mentions_tool () =
  let decision : Masc_mcp.Oas.Hooks.approval_decision =
    AC.reject_by_default ~tool_name:"masc_worktree_create" ~input:(`Assoc [])
  in
  match decision with
  | Reject reason ->
    Alcotest.(check bool)
      ...
      true
      (try
         ignore (Str.search_forward (Str.regexp_string "masc_worktree_create") reason 0);
         true
       with
       | Not_found -> false)
  | Approve -> Alcotest.fail "reject_by_default regressed to Approve"
  | Edit _ -> Alcotest.fail "reject_by_default regressed to Edit"
;;

(* ── 2. auto_approve contract unchanged for trusted system runs ─── *)

let test_auto_approve_approves () =
  let decision = AC.auto_approve ~tool_name:"anything" ~input:(`Assoc []) in
...
let builder_sites =
  [ (* Worker OAS Builder — issue #8232 path. *)
    "lib/worker_oas.ml", "Oas.Builder.build_safe", "Oas.Builder.with_approval"
  ; (* Oas_worker_exec Builder — run_named pipeline. This file wires
     config.approval into the builder directly. *)
    "lib/oas_worker_exec.ml", "Oas.Builder.build_safe", "Oas.Builder.with_approval"
  ]
;;
...
let test_builder_sites_wire_approval () =
  ...
  List.iter
    (fun (rel_path, marker, approval_marker) ->
       ...
       Alcotest.(check bool)
         (Printf.sprintf
            "%s: Builder site must install approval via %S (fail-open guard, #7883)"
            rel_path
            approval_marker)
         true
         (file_contains path approval_marker))
    builder_sites
;;
...
let test_run_named_sites_pass_approval () =
  ...
  List.iter
    (fun rel_path ->
       ...
       Alcotest.(check bool)
         (Printf.sprintf
            "%s: run_named call site must pass ~approval:... (fail-open guard, #7883)"
            rel_path)
         true
         (file_matches path "~approval:"))
    run_named_sites
;;
...
let () =
  Alcotest.run
    ...
    ; "auto_approve", [ Alcotest.test_case "approves" `Quick test_auto_approve_approves ]
    ; ( "structural"
      , [ Alcotest.test_case
            "Builder.build_safe sites install with_approval"
            `Quick
            test_builder_sites_wire_approval
        ; Alcotest.test_case
            "run_named call sites pass ~approval"
            `Quick
            test_run_named_sites_pass_approval
        ] )
    ]
;;
