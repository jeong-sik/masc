...
Fusion topology(`Simple|Refine|Conditional|Judge_of_judges|Staged_judge_of_judges`)는 keeper의 도구 입력 인자로 존재한다. 이 RFC 작성 시점의 4개 위상은 backend에서 서로 다른 judge 호출 구조로 실행되지만, sink에 도달하는 시점엔 `fusion_orchestrator.ml`의 `judge = Result.map fst judge_full`로 **단일 `judge_synthesis`로 collapse**됐다. 결과적으로:
...
- 대시보드는 `judges` 배열의 shape만으로 구조를 렌더한다: **1 node = simple, 2 = refine, N개 first + meta = judge-of-judges, first groups + stage_meta nodes + final_meta = staged judge-of-judges**. 위상 이름 vocabulary를 frontend에 하드코딩하지 않는다.
...
type judge_error_node = { failed_role : judge_role; error : string; usage : usage }
type judge_outcome = Synthesized of judge_node | Judge_failed of judge_error_node
```

`First of string`은 panelist_id를 정체성으로 보존(panel `model`과 대칭). `Stage_meta n`은 `stage-n`, `Final_meta`는 `final` 정체성으로 emit한다. 전부 `[@@deriving yojson, show, eq]`.
...
- `Staged_judge_of_judges` → stage별 `firsts` 노드 + `Stage_meta n` 노드를 반복한 뒤 `Final_meta` 노드. canonical은 final meta 성공 시 final_s, 실패 시 첫 성공 stage_s.

**콤비네이터/plan-tree를 도입하지 않는다**(§4). 닫힌 enum dispatch는 그대로 — 컴파일러 exhaustiveness가 새 위상 추가 시 모든 site를 강제한다.

### 3.3 sink — additive emit

`judge_synthesis_fields`를 `judge_meta`(canonical `judge` 키)와 `judge_node_meta`(`judges` 배열)가 공유한다(5섹션 직렬화 키 매핑 중복 회피). `judge_node_meta`는 `role`(single/refine/first/meta/stage_meta/final_meta) + `identity`(First는 panelist_id, stage는 `stage-N`) + 5섹션 + 노드별 usage를 emit한다.

### 3.4 byte-identity

기존 4 위상의 **결정 동작**(canonical synthesis decision/answer, chat 결론, wake payload, board headline, `observed_usage`)은 불변이다. canonical `judge_full`은 기존 로직과 동일하고, usage = 실행된 judge wave usage 합산이 기존 계산과 일치한다(refine 성공 u1+u2 / 실패 u1 / JOJ firsts+meta / meta 실패 firsts). emit-shape 변경(`judge` 단일 + `judges` 배열 추가)은 ADDITIVE — 의도된 변화. staged JOJ는 새 topology라 additive surface다.
...
**추출 트리거**: 진짜 5번째 위상이 기존 primitive 조합으로 표현 가능하고 hand-written 중복이 *3번째* 재발할 때. `Staged_judge_of_judges` 추가 시점에는 named topology arm + 작은 helper(`staged_judge_groups`, first-node helpers)로 충분하며, graph/plan DSL은 여전히 도입하지 않는다. 3-Try/추상화 거부 기준 적용.
