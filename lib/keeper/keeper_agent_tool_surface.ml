...
let generic_required_tool_gate_guidance ~(has_current_task : bool)
  ...
  then
    Printf.sprintf
      "[TOOL SIGNAL] This turn has an actionable runtime signal, but no \
       ...
       satisfy a tool hint.%s Emit a concise [STATE] blocker instead."
      claim_context_note
  else
    Printf.sprintf
      "[TOOL SIGNAL] This turn has an actionable runtime signal. Prefer a \
       currently visible keeper runtime tool when it directly advances the \
       work. Preferred tools for this signal: %s%s. Passive reads/status alone \
       do not advance the task.%s"
      preview
      suffix
      claim_context_note
