...
let test_default_latency () =
  ...
  let sum = Prometheus.metric_value_or_zero Prometheus.metric_sse_broadcast_duration () in
  let count =
  ...
  run
    ...
    [ ( "broadcast_latency"
      , [ test_case "default latency with no data" `Quick test_default_latency
        ; test_case "high latency detected via metrics" `Quick test_high_latency_detected
        ] )
    ]
;;
