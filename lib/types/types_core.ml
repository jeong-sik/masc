...
let parse_iso8601_opt s =
  try
    Scanf.sscanf s "%04d-%02d-%02dT%02d:%02d:%02dZ" (fun year mon day hour min sec ->
      let tm =
        { Unix.tm_sec = sec
        ; tm_min = min
        ; tm_hour = hour
...
let valid_agent_status_strings = List.map agent_status_to_string all_agent_statuses
...
type agent_meta =
  ...
  ; keeper_name : string option
        [@default None] (* stable keeper owner, when this runtime is keeper-owned *)
...
let normalize_agent_last_seen ~joined_at = function
  ...
  | `Int seconds -> Some (`String (iso8601_of_unix_seconds (float_of_int seconds)))
  | `Float seconds -> Some (`String (iso8601_of_unix_seconds seconds))
  ...
  | Error original_error ->
    (match json with
     | `Assoc fields ->
       ...
       in
       let now_iso () = `String (iso8601_of_unix_seconds (Unix.gettimeofday ())) in
       ...
       (match normalized_last_seen with
        | Some normalized_last_seen ->
          let fields_without_last_seen =
            ("last_seen", normalized_last_seen) :: List.remove_assoc "last_seen" fields
          in
...
let task_status_to_yojson = function
  ...
  | AwaitingVerification { assignee; submitted_at; verification_id; deadline; _ } ->
    `Assoc
...
let task_execution_links_to_yojson (links : task_execution_links) =
  `Assoc
    ...
    ; "autoresearch_loop_id", Json_util.string_opt_to_json links.autoresearch_loop_id
    ]
;;
...
let task_execution_links_of_yojson json =
  ...
  try
    Ok
      ...
      ; autoresearch_loop_id = json |> member "autoresearch_loop_id" |> to_string_option
      }
  with
  | e -> Error (Printexc.to_string e)
;;
...
let task_contract_to_yojson (contract : task_contract) =
  `Assoc
    ...
    ; "completion_contract", string_list_to_yojson contract.completion_contract
    ...
    ; "inspect_gate_evidence", string_list_to_yojson contract.inspect_gate_evidence
    ; "verify_gate_evidence", string_list_to_yojson contract.verify_gate_evidence
...
let task_contract_of_yojson json =
  ...
  try
    ...
    let links =
      ...
      | `Null -> { operation_id = None; session_id = None; autoresearch_loop_id = None }
      | links_json ->
    ...
    Ok
      ...
      ; inspect_gate_evidence = task_contract_string_list json "inspect_gate_evidence"
      ; verify_gate_evidence = task_contract_string_list json "verify_gate_evidence"
      ; links
      }
  with
  | e -> Error (Printexc.to_string e)
...
let task_to_yojson t =
  ...
  let with_contract =
    ...
    | None -> with_stage
    | Some contract -> with_stage @ [ "contract", task_contract_to_yojson contract ]
  in
  let with_handoff_context =
    match t.handoff_context with
    | None -> with_contract
    | Some handoff_context ->
      with_contract
      @ [ "handoff_context", task_handoff_context_to_yojson handoff_context ]
  in
  (* cycle_count omitted when 0 for backward-compat on existing backlogs. *)
  let with_cycle_count =
    if t.cycle_count = 0
    then with_handoff_context
    else with_handoff_context @ [ "cycle_count", `Int t.cycle_count ]
  in
  let with_do_not_reclaim =
...
let task_of_yojson json =
  ...
  try
    ...
    let worktree =
      match json |> member "worktree" with
      | `Null -> None
      | wt_json ->
    ...
    let stage =
      match json |> member "stage" |> to_string_option with
      | Some s ->
    ...
    in
    let contract =
      match json |> member "contract" with
      | `Null -> None
      | contract_json ->
    ...
    in
    let handoff_context =
      match json |> member "handoff_context" with
      | `Null -> None
      | handoff_json ->
...
type room_state =
  ...
  ; paused : bool [@default false]
    (** Global pause flag - when true, orchestrator won't spawn *)
...
let default_tempo_config =
  { mode = Normal; delay_ms = 0; reason = None; set_by = None; set_at = None }
;;
...
let tempo_config_of_yojson json =
  ...
  try
    ...
    let delay_ms =
      json |> member "delay_ms" |> to_int_option |> Option.value ~default:0
    in
    let reason = json |> member "reason" |> to_string_option in
...
let backlog_of_yojson json =
  ...
  try
    ...
    let last_updated =
      json |> member "last_updated" |> to_string_option |> Option.value ~default:""
    in
...
let portal_of_yojson json =
  ...
  try
    ...
    | Ok portal_status ->
      Ok { portal_from; portal_target; portal_opened_at; portal_status; task_count }
    | Error e -> Error e
  with
  | e -> Error (Printexc.to_string e)
;;
...
let tool_result_to_yojson r =
  let base = [ "success", `Bool r.success; "message", `String r.message ] in
  match r.data with
