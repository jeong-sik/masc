...
let normalize_ollama_server_url raw =
  ...
  let rec strip_trailing_slashes value =
    ...
    then strip_trailing_slashes (String.sub value 0 (len - 1))
    else value
  in
...
let ollama_ps_url server_url =
  normalize_ollama_server_url server_url ^ Masc_network_defaults.ollama_api_ps_path
;;

let ollama_generate_url server_url =
  normalize_ollama_server_url server_url ^ Masc_network_defaults.ollama_api_generate_path
;;
...
let ns_to_ms value = value |> Option.map (fun ns -> float_of_int ns /. 1_000_000.0)
...
let loaded_model_name (model : ollama_loaded_model) =
  string_or_fallback [ (fun () -> model.name); (fun () -> model.model) ]
;;
...
let ollama_loaded_models_of_ps_json json =
  ...
  |> List.map (fun item ->
    { name =
        string_or_fallback
          [ (fun () -> string_member item "name"); (fun () -> string_member item "id") ]
    ; model =
        string_or_fallback
...
let kv_cache_assessment_json run_jsons =
  ...
  | [] | [ _ ] ->
    `Assoc
      ...
      ; ( "note"
        , `String
            "Need at least two successful runs with prompt_eval_duration_ms to infer \
  ...
  | (baseline_run_index, baseline_ms) :: rest ->
      ...
      | first_repeat :: remaining_repeats ->
        List.fold_left
          (fun ((_best_idx, best_ms) as best) candidate ->
             ...
             if candidate_ms < best_ms then candidate else best)
          first_repeat
          remaining_repeats
      | [] -> None, baseline_ms
    in
    let reduction_ratio =
      if baseline_ms > 0.0
      then Some ((baseline_ms -. best_repeat_ms) /. baseline_ms)
      else None
    in
    let signal, note =
      ...
      | Some ratio when ratio >= 0.35 ->
        ( "likely_reused"
        , "Prompt evaluation time dropped materially on a repeated prompt. Timing \
           suggests warm repeated-prefix reuse." )
      | Some ratio when ratio >= 0.15 ->
        ( "possible_reuse"
        , "Prompt evaluation time improved on a repeated prompt, but not enough to treat \
           as a strong signal." )
      | Some _ ->
        ( "no_visible_reuse"
        , "Repeated prompt evaluation time did not improve enough to show an obvious \
           reuse signal." )
      | None ->
        ( "insufficient_data"
        , "Prompt evaluation duration was missing or zero, so reuse could not be \
           inferred." )
    in
    `Assoc
      ...
      ; ( "limitation"
        , `String
            "Inference only: stable Ollama APIs expose warm state and timings, not \
             direct KV occupancy or hit-rate." )
      ]
;;

let default_probe_prompt () =
  let line =
    "This is a repeated-prefix timing probe for Ollama native generate. Respond with \
     exactly READY."
  in
...
let fetch_ollama_ps ?(timeout_sec = 8) ~server_url () =
  ...
  | Ok (http_status, json) ->
    ...
    then http_status, [], Some (ollama_http_error "ps" http_status)
    else (
      let models =
...
let decide_generate_probe
  ...
  | Some _ ->
    (match before_status with
     | Some 200 ->
       if generate_when_unloaded || effective_model_loaded_before
       then Run_generate_probe
       else Skip_generate_probe Model_unloaded
     | _ ->
       if generate_when_unloaded || effective_model_loaded_before
       then Run_generate_probe
       else Skip_generate_probe Policy_skip)
;;

let should_attempt_generate_probe
      ~before_status
      ~before_error
      ~run_generate
      ~generate_when_unloaded
  ...
  =
  match
    decide_generate_probe
      ~effective_model:(Some "__probe_decision_compat__")
      ~before_status
      ~before_error
      ~run_generate
      ~generate_when_unloaded
...
let request_body_json ~think_enabled ~keep_alive ~model_id ~prompt ~max_tokens =
  let fields =
    ...
    ; (match Option.bind keep_alive trim_to_option with
       ...
       | None -> None)
    ; Some
        ("options", `Assoc [ "temperature", `Float 0.0; "num_predict", `Int max_tokens ])
    ]
...
let run_single_probe
  ...
  match
    http_post_json_text_with_status
      ~timeout_sec
      ~url
      ~body_json:
        (request_body_json ~think_enabled ~keep_alive ~model_id ~prompt ~max_tokens)
  with
  | Error err ->
    failed_probe_run
      ~run_index
  ...
  | Ok (http_status, payload) ->
    let wall_clock_ms = int_of_float ((Time_compat.now () -. started) *. 1000.0) in
    if http_status <> Some 200
    then
      failed_probe_run
        ~run_index
        ...
        ~error:
          (Printf.sprintf
             "ollama generate returned http %s"
             (match http_status with
      ...
      | json ->
        ollama_probe_run_of_generate_json ~run_index ~http_status ~wall_clock_ms json)
;;

let runtime_ollama_probe_json
      ?server_url
      ?model
      ?prompt
      ?(probe_runs = 2)
      ...
      let completed_runs =
        ...
        |> List.map (fun run_index ->
          run_single_probe
            ...
            ~run_index)
      in
      let run_errors = completed_runs |> List.filter_map (fun run -> run.error) in
      completed_runs, run_errors
  in
  let generate_skip_reason =
    match generate_decision with
    | Run_generate_probe -> None
    | Skip_generate_probe reason -> Some (generate_probe_skip_reason_to_string reason)
  in
  ...
  let observations =
    []
    |> (fun items ->
    if effective_model_loaded_before
    then
      "Effective model was already resident before the probe according to /api/ps."
      :: items
    else items)
    |> (fun items ->
    if (not effective_model_loaded_before) && effective_model_loaded_after
    then "Probe appears to have loaded the model into Ollama residency." :: items
    else items)
    |> (fun items ->
    if generate_skipped_unloaded_model
    then
      "Skipped /api/generate because the effective model was not resident; dashboard \
       probes avoid cold-loading Ollama models."
      :: items
    else items)
    |> (fun items ->
    if not run_generate
    then
      "Skipped /api/generate because run_generate=false; this is a status-only Ollama \
       residency probe."
      :: items
    else items)
    |> (fun items ->
    ...
    | `String "possible_reuse" ->
      "Repeated prompt_eval_duration_ms improved, but the signal is moderate rather than \
       decisive."
      :: items
    | `String "no_visible_reuse" ->
      "Repeated prompt_eval_duration_ms did not show a strong reuse improvement." :: items
    | _ -> items)
    |> List.rev
  in
  let errors =
  ...
  `Assoc
    ...
    ; ( "configured_default_model"
      , string_opt_to_json (trim_to_option Env_config_runtime.Ollama.default_model) )
    ...
    ; ( "limitations"
      , `List
          [ `String
              "This probe can observe warm-state and timing deltas, but not direct KV \
               cache occupancy or hit-rate."
          ; `String
              "A repeated-prefix signal is inference from prompt_eval_duration_ms, not a \
               stable Ollama-native cache metric."
          ] )
