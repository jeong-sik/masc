...
let arg_get_int ctx key default = Safe_ops.json_int ~default key ctx.arguments
...
let handle_broadcast (ctx : context) : tool_result option =
  ...
  if trimmed = ""
  then Some (false, "Broadcast message cannot be empty")
  else (
    let allowed, wait_secs = Session.check_rate_limit registry ~agent_name in
    if not allowed
    then Some (false, Printf.sprintf "Rate limited. %d sec remaining." wait_secs)
    else (
      let trace_context = Otel_trace_context.from_ambient () in
      let broadcast_result =
        Coord.broadcast ?trace_context config ~from_agent:agent_name ~content:message
      in
