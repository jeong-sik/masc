...
let test_extract_hints_common_words () =
  let hints =
    Auto_recall.extract_query_hints "the a an is are was were be to of and in for"
  in
...
let test_content_matches_positive () =
  let r =
    Auto_recall.content_matches_query
      "This is about OCaml programming"
      "OCaml programming"
  in
...
let test_format_with_items () =
  ...
  let item2 : Auto_recall.recall_item =
    { source = Masc_cache; content = "cached data"; relevance = 0.5; metadata = `Null }
  in
  let result : Auto_recall.recall_result =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp_string "broadcast") s 0 in
       true
     with
     | Not_found -> false);
  check
...
let test_format_truncated () =
  let item : Auto_recall.recall_item =
    { source = File_context; content = "file content"; relevance = 0.6; metadata = `Null }
  in
  let result : Auto_recall.recall_result =
...
let test_to_json_all_sources () =
  ...
  List.iter
    (fun source ->
       let item : Auto_recall.recall_item =
         { source; content = "c"; relevance = 0.5; metadata = `Null }
       in
       let result : Auto_recall.recall_result =
         { items = [ item ]; total_tokens = 1; truncated = false }
       in
       let j = Auto_recall.to_json result in
...
let write_file path content = Fs_compat.save_file path content
...
let test_recent_activity_skips_malformed_jsonl_lines () =
  with_temp_dir "activity-feed-jsonl"
  @@ fun base_path ->
...
let test_recent_activity_accepts_iso_string_created_at_for_board_posts () =
  with_temp_dir "activity-feed-jsonl-iso"
  @@ fun base_path ->
  let config = Coord.default_config base_path in
  let masc_dir = Coord.masc_dir config in
  Fs_compat.mkdir_p masc_dir;
  ...
  write_file
    ...
    (Yojson.Safe.to_string
     ...
     ^ "\n");
  let stderr_output =
    capture_stderr (fun () -> ignore (Activity_feed.recent_activity config ~limit:10 ()))
  in
  check
    bool
    "does not warn for ISO created_at"
    false
...
let test_recent_activity_skips_bad_task_file () =
  with_temp_dir "activity-feed-task"
  @@ fun base_path ->
  let config = Coord.default_config base_path in
  let masc_dir = Coord.masc_dir config in
...
let test_recent_activity_falls_back_from_bad_task_timestamp () =
  with_temp_dir "activity-feed-ts"
  @@ fun base_path ->
  let config = Coord.default_config base_path in
  let masc_dir = Coord.masc_dir config in
  let tasks_dir = Filename.concat masc_dir "tasks" in
  ...
  match items with
  | [ item ] -> check (float 0.01) "timestamp falls back to epoch 0.0" 0.0 item.created_at
  | _ -> fail "expected one task activity item"
;;

let test_recent_activity_ignores_backlog_json_without_timestamp_warning () =
  with_temp_dir "activity-feed-backlog"
  @@ fun base_path ->
  let config = Coord.default_config base_path in
  let masc_dir = Coord.masc_dir config in
  let tasks_dir = Filename.concat masc_dir "tasks" in
  ...
  check
    bool
    "backlog timestamp fallback warning suppressed"
    false
...
let () =
  ...
  run
    ...
    ; "default_config", [ test_case "defaults" `Quick test_default_config ]
    ; ( "make_config"
    ...
    ; ( "activity_feed_fs"
      , [ test_case
        ...
        ; test_case
            "accepts ISO string created_at for board posts"
            `Quick
        ...
        ; test_case
            "ignores backlog json without timestamp warning"
            `Quick
