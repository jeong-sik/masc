...
masc의 LLM 도구 실행 경로는 현재 문자열 중심 검증이다. 실측:
...
**Location 확정: `masc/lib/exec/` 내부 서브트리**. 별도 repo 없음. tree-sister / masc-exec-gate 등 분리 옵션 전부 reject. 본질은 masc 실행 경계 일원화 + typed dispatch이므로 분리는 소비자 2+ 발생 & 유지 비용 < 통합 혼잡도 증명 후에만 추출. tree-sitter 비교 공격 표면을 naming 레벨에서 제거.
...
| 별도 repo (tree-sister 등) | 분리 가치 증명 없음 | **masc 내부 `lib/exec/`** |
...
**추출 조건 (Phase B 이후)**: `lib/exec/` 소비자가 masc 외 1개 이상 생기고, 분리 유지 비용 < 통합 혼잡도 증명 시. 그 전까지는 내부 library. 현재 dune scope는 `masc` 프로젝트 내부 sub-library로, 외부 공개 이름 부여하지 않음.
...
(이전 v4의 "tree-sister 3800-6100 + masc 1500-2300" 동일 총량. repo 분리 비용 제거. 다른 작업 병행 시 ×1.5-2 = 12-22주.)
...
**Scope**: `masc/lib/exec/` 서브트리 초기화. 타입 모듈 + 빈 Menhir grammar 빌드 통과.
...
| R12 | `lib/exec/`가 masc 다른 모듈과 엉킴 | 중 | 중 | sub-tree 경계 + dune internal library. public interface 최소화 (Exec.run / Exec.classify만 export) |
...
| masc Δ LOC | baseline | +5,300-8,400 (모두 내부) | `git diff --stat` |
...
1. **typed capability substrate** — 구현 위치 `masc/lib/exec/` 내부 (별도 repo 없음)
...
| Item | What | PR | Status |
|---|---|---|---|
| S1 | `Exec_program.kind` typed dispatch (replaces `String.equal "git" …` etc.) | [#14216](https://github.com/jeong-sik/masc/pull/14216) | ✅ MERGED 2026-05-08 |
| S2 | `Agent_id.t` poly variant for approval-config keys | [#14227](https://github.com/jeong-sik/masc/pull/14227) | ✅ MERGED 2026-05-08 |
| S3 | `exec_gate` rollout keys typed (per-agent overlay table) | [#14227](https://github.com/jeong-sik/masc/pull/14227) | ✅ MERGED 2026-05-08 |
| S4 | `Capability_check` `Exec_program.kind` pattern matching (replaces string compare) | [#14216](https://github.com/jeong-sik/masc/pull/14216) | ✅ MERGED 2026-05-08 |
| §3.1 | `Tool_id.t` typed key for `Mode_enforcer.default_tool_entries` (46 known + `Other_tool of string` fallback) | [#14282](https://github.com/jeong-sik/masc/pull/14282) | ✅ MERGED 2026-05-09 |
...
| Item | What | PR | Status |
|---|---|---|---|
| §A0/§A1 | `Shell_ir_typed` GADT (`('i,'o,'r,'s) command`, 9 ctors + `Generic` fail-closed catch-all) | [#14240](https://github.com/jeong-sik/masc/pull/14240) | ✅ MERGED 2026-05-08 |
| §A2 | `Capability_check_typed` GADT walker | [#14240](https://github.com/jeong-sik/masc/pull/14240) | ✅ MERGED 2026-05-08 |
| §A2 | `Risk_classifier_typed` GADT classification | [#14240](https://github.com/jeong-sik/masc/pull/14240) | ✅ MERGED 2026-05-08 |
| §A2 | `Approval_policy_typed` GADT → existing `Approval_policy` bridge | [#14240](https://github.com/jeong-sik/masc/pull/14240) | ✅ MERGED 2026-05-08 |
| §A3 | `exec_gate.verdict_for_argv` consumes `Capability_check_typed.of_command` (parallel to legacy untyped path) | [#14258](https://github.com/jeong-sik/masc/pull/14258) | ✅ MERGED 2026-05-09 |
