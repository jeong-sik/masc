...
(* Titles and phases for the Goals that are still open. Read from the store,
   the same source and the same phase predicate the world observation uses, so
   a keeper is never handed a Completed goal under a heading that calls it
   available. The phase rides along so the Active Goals block can annotate a
   [Verifying] goal
