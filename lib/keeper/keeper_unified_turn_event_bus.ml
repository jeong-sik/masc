...
type t =
  ...
  ; turn_event_bus : Keeper_turn_budget.turn_event_bus_summary ref
  }
...
let create ~keeper_name () =
  ...
  ; turn_event_bus = ref Keeper_turn_budget.empty_turn_event_bus_summary
  }
;;
...
let drain ?(site = "unspecified") t =
  with_turn_event_bus_lock t (fun () ->
    ...
    let summary = Keeper_turn_budget.summarize_turn_event_bus events in
    t.turn_event_bus
    := Keeper_turn_budget.merge_turn_event_bus_summary
         !(t.turn_event_bus)
         summary;
    !(t.turn_event_bus))
;;
