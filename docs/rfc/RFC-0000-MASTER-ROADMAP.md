...
- **candidate work:** **[REPO-SSOT, 우선]** `clone_sandbox_repo`가 catalog record 전체 읽기(하드코딩 `"main"`/`[]` 제거; 동일 하드코딩이 `repo_store.ml:439` `discover_repositories`에도 있음 — 함께 수정); clone location을 catalog **id** keyed 순수 함수로; 역파싱 조인(`parse_playground_repo_path`+`find_url_by_id`, **프로덕션 caller 단 1곳** `keeper_tool_filesystem_runtime.ml:314`) 제거→`git remote get-url`(`Repo_git.get_origin_url`, **이미 `repo_store.ml:439`에서 동일 용도로 사용**) + `worktree_root`로 귀속; 등록 `local_path`를 `operator_checkout_path`로 개명(2역할 2이름, REGISTERED 소비처 ~11곳). **[identity, SEVERE]** typed `Keeper_id` 단일 canonical + explicit projection으로 5+ 핸들 통합 → `identity_matches` OR-매칭·#10440 alias 워크어라운드 제거. **[config/runtime 정리, 라이브-안전]** json config 키 legacy read-fallback 제거(always_allow/mention_targets/oas_env) → json=런타임 상태 전용; derived snapshot을 render-only projection으로 stamp; `mention_targets` persona↔toml overlay union화 또는 replace 명시. **(keeper.json 삭제는 non-goal — 런타임 상태 CAS store.)** **[나머지 축]** single read path(`read_meta` private화), single `Local`/`Docker` enum+1 parser, rejected-key 단일 closed set+`allowed_providers` 강제, docker layout을 `playground_paths` SSOT가 **구성+파싱** 둘 다. **[D8 정합]** 역파싱 귀속을 git-remote로 옮기면 playground 번들 전체가 bare per-keeper 작업디렉토리+주입 env로 붕괴 — §11 D8·D9와 한 수술.
