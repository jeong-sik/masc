...
let success_entry
  ...
  let extra_telemetry_fields =
    ...
    | [] -> []
    | reasons ->
      [ "usage_anomaly_reasons", `List (List.map (fun reason -> `String reason) reasons) ]
  in
  `Assoc
    [ "ts_unix", `Float ts
    ...
    ; ( "telemetry"
      , `Assoc
          ([ "model_used", `String model
           ; ( "tokens_per_second"
             , `Float (Float.of_int output_tokens /. (Float.of_int latency_ms /. 1000.0))
             )
...
let success_entry_without_usage
      ~model
      ~ts
      ?provider
...
let test_single_model_success () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check bool "cost > 0" true (Option.value ~default:0.0 s.total_cost_usd > 0.0);
       check bool "avg_tool_calls > 0" true (s.avg_tool_calls_per_turn > 0.0);
       check bool "latency > 0" true (Option.value ~default:0.0 s.avg_latency_ms > 0.0);
       check bool "tok/s > 0" true (Option.value ~default:0.0 s.avg_tok_per_sec > 0.0))
;;
...
let test_provider_kind_classifies_bare_model_provider () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       write_decisions
         path
         [ success_entry ~model:"kimi-k2.5" ~ts:(ts -. 5.0) ~provider_kind () ];
       let agg = M.compute ~base_path:base ~window_minutes:60 in
       check int "total_entries" 1 agg.total_entries;
       let s = List.hd agg.models in
       check string "model stays bare" "kimi-k2.5" s.model_id;
       check (option string) "provider from provider_kind" (Some "kimi_cli") s.provider;
       let recent = List.hd s.recent_entries in
       check
         (option string)
         "recent provider from provider_kind"
         (Some "kimi_cli")
         recent.re_provider;
       let rollup = M.provider_rollup agg in
       check int "provider rollup keeps entry" 1 (List.length rollup);
       check string "rollup provider" "kimi_cli" (List.hd rollup).M.ps_provider)
;;
...
let test_untrusted_usage_excluded_from_aggregates () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check int "models" 1 (List.length agg.models);
       let s = List.hd agg.models in
       check (option int) "trusted input total only" (Some 100) s.total_input_tokens;
       check (option int) "trusted output total only" (Some 20) s.total_output_tokens;
       ...
       check
         (option (float 0.001))
         "tok/s excludes untrusted outlier"
         (Some 20.0)
         s.avg_tok_per_sec;
       check
         (option string)
         "coverage flags untrusted usage"
         (Some "untrusted_usage")
         s.primary_coverage_reason;
       let recent = List.hd s.recent_entries in
       check (option string) "recent usage trust" (Some "untrusted") recent.re_usage_trust;
       check (option int) "recent untrusted input hidden" None recent.re_input_tokens;
       check
         (list string)
         "recent anomaly reasons"
         [ "input_tokens_gt_1m" ]
         recent.re_usage_anomaly_reasons)
;;
...
let test_error_turns_counted () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let error_model =
         List.find_opt (fun (s : M.model_stats) -> s.model_id = "model-a") agg.models
       in
    ...
    (fun () ->
       ...
       write_decisions
         ...
         [ success_entry
             ...
             ()
         ; success_entry ~model:"gpt-4o" ~ts:(ts -. 20.0) ~tools_used:[ "search" ] ()
         ; success_entry ~model:"claude-sonnet" ~ts:(ts -. 10.0) ~tools_used:[ "read" ] ()
         ];
       let agg = M.compute ~base_path:base ~window_minutes:60 in
    ...
    (fun () ->
       ...
       write_decisions
         ...
         [ success_entry
             ...
             ()
         ; success_entry ~model:"m1" ~ts:(ts -. 20.0) ~tools_used:[ "shell"; "write" ] ()
         ];
       let agg = M.compute ~base_path:base ~window_minutes:60 in
       let s = List.hd agg.models in
...
let test_json_roundtrip () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         bool
         "provider unresolved -> null"
         true
         (match m |> member "provider" with
          | `Null -> true
          | _ -> false);
       check int "success_count" 1 (m |> member "success_count" |> to_int);
       check int "usage_sample_count" 1 (m |> member "usage_sample_count" |> to_int);
       check
         bool
         "total_cost_usd = 0.05"
         true
       ...
       check
         ...
         "has top_tools list"
         true
         (match m |> member "top_tools" with
          | `List _ -> true
          | _ -> false);
       check
         bool
         "has recent_entries list"
         true
         (match m |> member "recent_entries" with
          | `List _ -> true
          | _ -> false);
       check int "total_error_entries" 0 (json |> member "total_error_entries" |> to_int))
;;
    ...
    (fun () ->
       ...
       write_decisions
         ...
         ; success_entry
             ~model:"mlx-vlm"
             ~ts:(ts -. 5.0)
             ~prompt_per_second:1500.0
             ~peak_memory_gb:20.25
             ()
         ];
       let agg = M.compute ~base_path:base ~window_minutes:60 in
       let s = List.hd agg.models in
       ...
       check (option string) "recent provider derived" None first_recent.re_provider;
       check
         (float 0.001)
         "recent prompt tok/s"
         1500.0
       ...
       check
         bool
         "recent provider null"
         true
         (match recent |> member "provider" with
          | `Null -> true
          | _ -> false);
       check
         (float 0.001)
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         ...
         "json input null"
         true
         (match m |> member "total_input_tokens" with
          | `Null -> true
          | _ -> false);
       check
         bool
         "json latency null"
         true
         (match m |> member "avg_latency_ms" with
          | `Null -> true
          | _ -> false);
       let recent_json = m |> member "recent_entries" |> to_list |> List.hd in
       check
         bool
         "recent json input null"
         true
         (match recent_json |> member "input_tokens" with
          | `Null -> true
          | _ -> false))
;;
...
let test_coverage_diagnostics_survive_aggregation () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         (option string)
         "primary coverage reason"
         (Some "missing_usage_and_inference")
         s.primary_coverage_reason;
       check
         (option string)
         "primary coverage stage"
         (Some "oas")
       ...
       check (option string) "recent turn lane" (Some "text_only") recent.re_turn_lane;
       check (option bool) "recent usage_reported" (Some false) recent.re_usage_reported;
       check
         (option bool)
         "recent telemetry_reported"
         (Some false)
         recent.re_telemetry_reported;
       check
         (option string)
         "recent coverage reason"
         (Some "missing_usage_and_inference")
         recent.re_coverage_reason;
       check (option string) "recent coverage stage" (Some "oas") recent.re_coverage_stage;
       let json = M.to_json agg in
       let open Yojson.Safe.Util in
       let m = json |> member "models" |> to_list |> List.hd in
       check
       ...
       check int "json usage missing count" 1 (m |> member "usage_missing_count" |> to_int);
       check
         int
         "json telemetry missing count"
         1
         (m |> member "telemetry_missing_count" |> to_int);
       check
         string
         "json primary coverage reason"
         "missing_usage_and_inference"
         (m |> member "primary_coverage_reason" |> to_string);
       check
         string
         "json primary coverage stage"
         "oas"
...
let test_costs_jsonl_backfills_wall_tok_per_sec () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         (option (float 0.001))
         "wall tok/sec from cost latency"
         (Some 200.0)
         s.avg_tok_per_sec;
...
let test_thinking_fraction_mixed () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check bool "thinking_fraction present" true (Option.is_some s.thinking_fraction);
...
let test_thinking_fraction_all_missing () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let agg = M.compute ~base_path:base ~window_minutes:60 in
       let s = List.hd agg.models in
       check bool "thinking_fraction None" true (Option.is_none s.thinking_fraction))
;;
...
let test_thinking_fraction_json_serialization () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         (float 0.001)
         "thinking_fraction in JSON"
         0.5
         (m |> member "thinking_fraction" |> to_float))
;;
...
let test_buckets_sparse () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         bool
         "sparse → 2 distinct buckets (10min apart, 5min width)"
         true
         (List.length m.mb_buckets >= 2))
;;
...
let test_buckets_cache_hit_ratio_zero_denom () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check
         bool
         "cache_hit_ratio not NaN"
         true
         (not (Float.is_nan (Option.value ~default:0.0 b.b_cache_hit_ratio)));
       check
         (option (float 0.001))
         "cache_hit_ratio = 0.0 when both tokens=0"
         (Some 0.0)
         b.b_cache_hit_ratio)
;;
...
let test_buckets_with_compute () =
  ...
  let now = now_unix () in
  write_decisions path [ success_entry ~model:"model-x" ~ts:now () ];
  Fun.protect
    ~finally:(fun () -> cleanup_dir dir)
    (fun () ->
       let agg =
         M.compute_with_buckets ~base_path:dir ~window_minutes:60 ~bucket_minutes:5
       in
...
let zero_model_stats (model_id : string) ~provider ~entry_count : M.model_stats =
  { model_id
  ; provider
  ; entry_count
...
let test_provider_rollup_empty_aggregate () =
  ...
  in
  check int "empty models gives empty rollup" 0 (List.length (M.provider_rollup agg))
;;

let test_provider_rollup_skips_unknown_provider () =
  let m1 =
    zero_model_stats "glm-coding:auto" ~provider:(Some "glm-coding") ~entry_count:5
  in
...
let test_provider_rollup_weighted_mean () =
  ...
  let m1 =
    { (zero_model_stats "ollama:qwen3.6" ~provider:(Some "ollama") ~entry_count:20) with
      avg_tok_per_sec = Some 100.0
    }
  in
  let m2 =
    { (zero_model_stats "ollama:qwen3.5" ~provider:(Some "ollama") ~entry_count:80) with
      avg_tok_per_sec = Some 50.0
    }
  in
  let agg : M.aggregate =
    { window_minutes = 30
  ...
  | Some v ->
    ...
    if Float.abs (v -. 60.0) > 0.01 then failf "weighted mean expected ~60.0 but got %f" v
...
let test_provider_rollup_all_none_yields_none () =
  ...
  let rollup = M.provider_rollup agg in
  let stats = List.hd rollup in
  check (option (float 0.0001)) "avg_tok_per_sec stays None" None stats.ps_avg_tok_per_sec;
  check
    (option (float 0.0001))
    "avg_prompt_tok_per_sec stays None"
    None
    stats.ps_avg_prompt_tok_per_sec;
  check (option (float 0.0001)) "p95_latency_ms stays None" None stats.ps_p95_latency_ms
;;
...
let test_provider_rollup_json_shape () =
  let m =
    { (zero_model_stats "kimi_cli:kimi" ~provider:(Some "kimi_cli") ~entry_count:42) with
      avg_tok_per_sec = Some 25.0
  ...
  | `Assoc fields ->
    check
      string
      "provider"
      (match List.assoc "provider" fields with
       | `String s -> s
       | _ -> "!")
      "kimi_cli";
    check
      int
      "request_count surfaces entry_count"
      42
      (match List.assoc "entry_count" fields with
       | `Int n -> n
       | _ -> -1);
  ...
  run
    ...
    [ ( "basics"
      , [ test_case "empty dir" `Quick test_empty_dir
        ...
        ; test_case
            "provider_kind classifies bare model provider"
            `Quick
            test_provider_kind_classifies_bare_model_provider
        ; test_case
            "untrusted usage excluded from aggregates"
            `Quick
    ...
    ; ( "enrichment"
      , [ test_case "top tools per model" `Quick test_top_tools_per_model
        ...
        ; test_case
            "prompt tps and peak memory aggregates"
            `Quick
            test_prompt_tps_and_peak_memory_aggregates
        ; test_case
            "missing usage serializes unknowns"
            `Quick
            test_missing_usage_serializes_unknowns
        ; test_case
            "coverage diagnostics survive aggregation"
            `Quick
            test_coverage_diagnostics_survive_aggregation
        ; test_case
            "costs.jsonl backfills wall tok/sec"
            `Quick
            test_costs_jsonl_backfills_wall_tok_per_sec
        ; test_case
            "costs.jsonl dedupes matching decision sample"
            `Quick
            test_costs_jsonl_dedupes_matching_decision_sample
    ...
    ; ( "buckets"
      , [ test_case "empty dir → no buckets" `Quick test_buckets_empty_dir
        ...
        ; test_case
            "cache_hit_ratio zero denom"
            `Quick
            test_buckets_cache_hit_ratio_zero_denom
        ; test_case "compute_with_buckets integration" `Quick test_buckets_with_compute
        ] )
    ; ( "provider_rollup"
      , [ test_case
            "empty aggregate gives empty rollup"
            `Quick
            test_provider_rollup_empty_aggregate
        ; test_case
            "skips models with provider=None"
            `Quick
            test_provider_rollup_skips_unknown_provider
        ; test_case
            "entry-weighted mean across models"
            `Quick
            test_provider_rollup_weighted_mean
        ; test_case
            "all-None perf fields preserve None in rollup"
            `Quick
        ...
        ; test_case "provider_stats_to_json shape" `Quick test_provider_rollup_json_shape
        ] )
    ]
;;
