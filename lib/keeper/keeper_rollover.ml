  ...
  let contains needle =
    ...
    then false
      ...
      let rec scan i =
        if i + nlen > blen
        then false
        else if String.sub b i nlen = needle
        then true
        else scan (i + 1)
      in
      scan 0)
...
let classify_rollover_gate
  ...
  =
  if not auto_handoff
  then Skip "auto_handoff_disabled"
  else if not cooldown_elapsed
  then Skip "cooldown"
  else (
    ...
    let signal_gate =
      current_turn_signal
      || (last_outcome = Proactive_error && blocker_indicates_overflow last_blocker)
    in
...
let maybe_rollover_oas_handoff
  ...
  | Some cp ->
    ...
    in
    let current_generation = checkpoint_generation cp ~fallback:meta.runtime.generation in
    let base_meta =
      if current_generation = meta.runtime.generation
      then meta
    ...
    (match gate_decision with
     ...
     | Go trigger_reason ->
       ...
       on_started ();
       (try
          let new_session = create_session ~session_id:new_trace_id ~base_dir in
          let save_ctx =
            { ctx with
              checkpoint =
                { (checkpoint_of_context ctx) with
                  messages = repair_orphan_tool_result_messages (messages_of_context ctx)
                }
            }
          in
          match
          ...
          | Ok _checkpoint ->
            (match Keeper_id.Trace_id.of_string new_trace_id with
             | Error err ->
               Log.Keeper.error
                 "keeper:%s OAS handoff rollover ABORTED — generated invalid trace_id \
                  %s: %s"
                 base_meta.name
                 new_trace_id
                 err;
               { rollover_base with attempted = true; failure_reason = Some err }
             | Ok parsed_trace_id ->
               let updated_meta =
                 { base_meta with
                   updated_at = now_iso ()
                 ; runtime =
                     ...
                     ; trace_history =
                         dedupe_keep_order
                           (Keeper_id.Trace_id.to_string prev_trace_id
                            :: base_meta.runtime.trace_history)
                     ; generation = next_generation
