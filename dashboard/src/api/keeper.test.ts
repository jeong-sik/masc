...
describe("keeper API module split compatibility", () => {
  it("keeps runtime-trace helpers re-exported from the keeper barrel", () => {
    expect(parseKeeperRuntimeTrace).toBe(
      parseKeeperRuntimeTraceFromRuntimeTrace,
    );
    expect(fetchKeeperRuntimeTrace).toBe(
      fetchKeeperRuntimeTraceFromRuntimeTrace,
    );
  });
  ...
  it("keeps lifecycle helpers re-exported from the keeper barrel", () => {
    ...
    expect(deleteKeeperHistorySnapshots).toBe(
      deleteKeeperHistorySnapshotsFromLifecycle,
    );
...
describe("Keeper chat durable receipt API", () => {
      ...
      expect(
        parseKeeperChatReceipt({
          ...
          state: {
            ...
            detail:
              "startup recovery could not prove terminal connector delivery",
            completed_at: 42,
            outcome_ref: null,
          },
        }).state,
      ).toMatchObject({ kind: "failed", failureKind });
    },
  );
  ...
  it("bounds chat history response-body consumption after headers arrive", async () => {
    ...
    const rejection = expect(historyPromise).rejects.toMatchObject({
      timeout: true,
    });
...
describe("fetchQueuedKeeperMessageResult", () => {
  ...
  it("normalizes cancelled queued results as cancellation text", async () => {
    ...
    expect(queuedKeeperMessageToReply(result).text).toBe(
      "요청이 취소되었습니다.",
    );
  });
  ...
  it("rejects an unknown request lifecycle instead of coercing it to error", async () => {
    ...
    await expect(
      fetchQueuedKeeperMessageResult("kmsg_sangsu_future"),
...
describe("streamKeeperMessage", () => {
  it("posts direct reply mode to the keeper chat stream endpoint", async () => {
    window.history.replaceState(
      {},
      "",
      "/?agent=dashboard-eager-manta%E3%85%8A",
    );
      ...
      if (url === "/api/v1/keepers/chat/stream") {
        ...
        if (chatAttempts === 1) {
          return Promise.resolve(
            new Response(JSON.stringify(first401Body), {
              status: 401,
              headers: { "Content-Type": "application/json" },
            }),
          );
      ...
      if (url === "/api/v1/keepers/chat/stream") {
        return Promise.resolve(
          new Response(
            JSON.stringify({
              error:
                "[AuthError] Forbidden: browser cannot cross-origin HTTP mutation",
...
describe("keeper runtime trace", () => {
  it("parses runtime trace evidence with resilient defaults", () => {
    ...
    expect(result.provider_attempts.attempts[0]?.exception_kind).toBe(
      "outer_oas_timeout",
    );
    ...
    expect(result.linked_artifacts.receipts[0]?.path).toBe(
      "/tmp/receipt.jsonl",
    );
  ...
  it("parses runtime lens evidence with safe defaults and gap codes", () => {
    ...
    expect(result.runtime_lens.axes.provider_attempt.terminal_status).toBe(
      "timeout",
    );
    expect(result.runtime_lens.swimlanes.provider.terminal_status).toBe(
      "timeout",
    );
    expect(result.runtime_lens.swimlanes.memory_context.terminal_status).toBe(
      "unknown",
    );
    expect(result.runtime_lens.clock_edges[0]?.edge_id).toBe(
      "edge-provider-start",
    );
    expect(result.runtime_lens.swimlanes.tool_runtime.completeness).toBe(
      "complete",
    );
    expect(result.runtime_lens.clock_edges[0]?.provider_attempt_id).toBe(
      "trace-lens:keeper-9:provider-attempt-1",
    );
    expect(result.runtime_lens.clock_edges[0]?.event_bus_event_count).toBe(2);
    expect(result.runtime_lens.clock_edges[0]?.event_bus_payload_kinds).toEqual(
      ["tool_called", "tool_completed"],
    );
    expect(result.runtime_lens.clock_edges[0]?.links.tool_call_log_path).toBe(
      "/tmp/tool-calls.jsonl",
    );
    expect(result.runtime_lens.clock_groups[0]?.group_type).toBe(
      "provider_attempt",
    );
    expect(result.runtime_lens.clock_groups[0]?.closed).toBe(true);
    expect(result.runtime_lens.clock_groups[0]?.terminal_events).toEqual([
  ...
  it("fetches runtime trace evidence with query params", async () => {
    ...
    const [url, init] = fetchMock.mock.calls[0]! as [string, RequestInit];
    expect(url).toBe(
      "/api/v1/keepers/keeper%20sangsu/runtime-trace?trace_id=trace+1&turn_id=7&limit=50",
    );
...
describe("keeper lifecycle", () => {
  ...
  it("falls back to the HTTP status when boot failure payload is not json", async () => {
    ...
    expect(result.error).toBe(
      "Failed to boot keeper-test (HTTP 502): auth gateway failed",
    );
  });
  ...
  it("posts selected OAS history snapshot ids for deletion", async () => {
    ...
    const result = await deleteKeeperHistorySnapshots("keeper-test", [
      "oas-snapshot-1.json",
    ]);
  ...
  it("sends POST with action=pause via directive endpoint", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({ ok: true, action: "pause", name: "janitor" }),
        {
          status: 200,
          headers: { "Content-Type": "application/json" },
        },
  ...
  it("sends POST with action=resume via directive endpoint", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({ ok: true, action: "resume", name: "janitor" }),
        {
          status: 200,
          headers: { "Content-Type": "application/json" },
        },
  ...
  it("reuses the same resume operation ID after an ambiguous failed response", async () => {
    ...
    expect(first.operator_operation_id).toBe(
      "dashboard-resume-stable-resume-uuid",
    );
  ...
  it("sends POST with action=wakeup via directive endpoint", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({ ok: true, action: "wakeup", name: "sangsu" }),
        {
          status: 200,
          headers: { "Content-Type": "application/json" },
        },
