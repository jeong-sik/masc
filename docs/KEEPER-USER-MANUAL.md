...
OAS는 `dune-project`에 `agent_sdk >= 0.80.0`으로 명시된 의존성이며, CI에서는 `scripts/opam-pin-external-deps.sh`를 통해 `jeong-sik/oas#main`을 pin해 최신 1st-party runtime과 함께 검증한다.
