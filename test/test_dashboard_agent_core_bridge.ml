(** Tests for [Dashboard_agent_core_bridge].
    ...
    filters, and the nearest-rank percentile in {!Dashboard_agent_core_bridge.summary},
    plus provider-error count aggregation for I2 (#11925). *)

module DOB = Dashboard_agent_core_bridge
...
let make_telemetry ?timings ?(request_latency_ms = 0) ?ttfrc_ms ?prefill_ms ()
  : Agent_core.Types.inference_telemetry
  =
...
let test_recent_json_provider_filter_is_runtime_alias () =
  ...
  Alcotest.(check string)
    "dashboard surface"
    "/api/v1/dashboard/agent_core/telemetry/recent"
    (json |> Json.member "dashboard_surface" |> Json.to_string);
  Alcotest.(check string)
    "source"
    "agent_core_runtime_bridge"
    (json |> Json.member "source" |> Json.to_string);
  Alcotest.(check string)
    "durable replay surface"
    "/api/v1/dashboard/telemetry?source=agent_core_event"
    (json
...
let test_summary_json_contains_aggregate () =
  ...
  Alcotest.(check string)
    "dashboard surface"
    "/api/v1/dashboard/agent_core/telemetry/summary"
    (json |> Json.member "dashboard_surface" |> Json.to_string);
  Alcotest.(check int)
    "retention cap"
    200
...
(* --- AGENT_CORE response projection --- *)
