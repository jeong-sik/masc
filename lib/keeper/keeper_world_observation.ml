...
let read_backlog_counts ~allowed_tool_names ~(config : Coord.config) ~(meta : keeper_meta)
  ...
  try
    ...
    let unclaimed_tasks =
      List.filter (fun (t : Types.task) -> t.task_status = Types.Todo) backlog.tasks
    in
    let unclaimed = List.length unclaimed_tasks in
    let required_tools_allowed required_tools =
      match allowed_tool_names with
      | Some names ->
        Coord_task_schedule.required_tools_allowed ~agent_tool_names:names required_tools
      | None -> Coord_task_schedule.required_tools_allowed required_tools
    in
    let claimable =
      List.length
        (List.filter
    ...
    (match last_cursor with
     | Some (ts, post_id)
       when compare_board_cursor_token
            ...
            > 0 -> Keeper_registry.set_board_cursor ~base_path meta.name ts (Some post_id)
     | Some (ts, post_id) ->
       Log.Keeper.debug
...
let provider_cooldown_remaining_sec_for_cascade ~(cascade_name : string) : int option =
  let model_ids =
    ...
    | [] ->
      ...
      then
        ...
        let should_run =
          ...
          && (backlog_fresh || backlog_elapsed || (idle_gate_elapsed && cooldown_elapsed))
        in
