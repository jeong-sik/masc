...
let severity_of_z z =
  ...
  if a < 1.0 then Low else if a < 2.0 then Medium else if a < 3.0 then High else Critical
;;
...
let tool_diversity_of_batch entries =
  let tool_calls =
    List.filter_map (fun e -> tool_name_of_action e.Audit_log.action) entries
  in
...
let token_volume_of_batch entries =
  ...
  if n = 0
  then None
  else Some (float_of_int (List.fold_left ( + ) 0 counts) /. float_of_int n)
;;
...
let batch_entries ~batch_hours entries =
  ...
  then []
    ...
    let sorted =
      List.sort
        (fun a b -> Float.compare a.Audit_log.timestamp b.Audit_log.timestamp)
        entries
    in
    ...
    let rec split batch_start batch_acc acc = function
      ...
      | e :: rest ->
        if e.Audit_log.timestamp -. batch_start <= span_sec
        then split batch_start (e :: batch_acc) acc rest
        else (
  ...
  then None
    ...
    then None
      ...
      then None
        ...
        let activity_stat =
          { mean = mean activity_volumes; stddev = stddev activity_volumes }
        in
        let diversity_stat =
          { mean = mean tool_diversities; stddev = stddev tool_diversities }
        in
        let token_stat =
          if token_volumes = []
          then None
    ...
    | Ok content ->
      (try
         ...
         | `Assoc fields ->
           ...
           in
           (match
              get_string "agent_id", get_int "window_days", get_int "sample_count"
            with
...
let detect_deviations ~profile ~entries ~threshold =
  ...
  then []
    ...
    add
      "activity_volume"
      observed_activity
      profile.activity_volume.mean
      profile.activity_volume;
    add
      "tool_diversity"
      observed_diversity
      profile.tool_diversity.mean
      profile.tool_diversity;
...
let check_agent ~config ~agent_id ~window_days ~threshold =
  ...
  then None
    ...
    | Some profile ->
      ...
      let overall_risk =
        List.fold_left (fun acc d -> max_risk acc d.severity) Low deviations
      in
