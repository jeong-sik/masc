---
title: Distribute legacy metrics backend metric ownership to domain modules
rfc: 0043
status: Active
created: 2026-05-08
implementation_prs: []
...
- **Created**: 2026-05-08
- **Number**: jeong-sik/masc 의 RFC 번호는 PR Draft 시점 점유. 본 RFC `0043`
  잠정. PR #13918 (RFC-0039), PR #14157 (RFC-0041), PR #14181 (RFC-0042) 동시
  Draft 점유 중. maintainer 가 머지 시점 재배정 가능.
- **Related**:
  - PR #14166 (CLOSED, **user-rejected**, "refactor(legacy metrics backend): extract 365
  ...
  - `instructions/MANIFEST.md`: "당장의 진행을 위해 잠시 회피하는 식으로 작업하기보다
    근본적으로 해결이 필요"
- **Drives**: Eliminate `legacy metrics backend module` 의 godfile size cap 위반을 *file split*
...
`legacy metrics backend module` = **3059 LOC** (limit 3000). `Godfile size` CI gate 가
...
lib/keeper/keeper_unified_turn.ml:
  Otel_metric_store.inc_counter ~name:Otel_metric_store.metric_keeper_turn_total ...

lib/keeper/keeper_execution_receipt.ml:
  Otel_metric_store.inc_counter ~name:Otel_metric_store.metric_keeper_receipt_unmapped_disposition ...
```

도메인 모듈 (`lib/keeper/*`) 이 자기 metric 의 *consumer* 이지만 *owner* 가 아니다.
새 metric 추가 = 도메인 코드 수정 + `legacy metrics backend module` 수정 (cross-file).
Owner 분산이 안 되어 있어 다음 두 비용이 영구 발생:

1. **godfile ratchet** — 새 telemetry 마다 `legacy metrics backend module` 비대화
2. **변경 marshal 비용** — 도메인 PR 이 legacy metrics backend module 까지 건드려야 → conflict
...
| NG2 | legacy metrics backend client library 교체 (현재 구현 유지) |
...
#### A. Definition site (`legacy metrics backend module` 의 `let metric_<domain>_*` 상수)
...
#### B. Use site (`Otel_metric_store.metric_<domain>_*` reference, 다른 모듈에서)

`rg -o 'legacy metrics backend\.metric_[a-z_]+' lib/ -t ml | sed 's/legacy metrics backend\.metric_//' | awk -F_ '{print $1}' | sort | uniq -c | sort -rn`:
...
현재 `legacy metrics backend module` 의 metric 상수는 plain `string` (예:
...
**API 매개변수 정합성** (실제 [`legacy metrics backend interface`](../../legacy metrics backend interface) 시그니처 기준):
...
### 3.3 legacy metrics backend module 잔여 contents

이전 후 `legacy metrics backend module` 에 남는 것:
...
`metric_store_init.register_all_metrics ()` 같은 단일 entry 가 main_eio.ml 에서 1회
호출되도록 정리. 각 도메인 모듈의 `register_all` 을 collect.

```ocaml
(* metric-store init adapter *)
...
**LOC 추정 근거**:
- legacy metrics backend module 내 metric 상수 영역 line 211-1429 (~1218 LOC, RFC body §1.2 의
...
**자연 해소 시점**: PR-2 시점에 legacy metrics backend module 약 −370 LOC → 3059 → ~2690.
godfile cap (3000) 은 PR-2 단독으로 통과. PR-3/4 가 추가로 legacy metrics backend module 을
...
init 비용 분산. 현재 legacy metrics backend module 의 register 는 *file load 시점 side effect*
인지 (`let _ = register_counter ...`) 또는 *명시 호출* 인지 확인 필요 — PR-1
시작 전 `rg 'let _ = register' legacy metrics backend module` 로 측정.
...
```
<Domain>_metrics.ml  →  legacy metrics backend  (leaf)
<Domain> 도메인 코드 (예: keeper_unified_turn)
   ─→ <Domain>_metrics  (use site, 새 경로)
   ─→ legacy metrics backend        (inc_counter, 변경 X)
```
...
`lib/dune` 의 `include_subdirs unqualified` 와 단일 `masc` library 구조 덕에
...
`Metric_store_init.register_all_metrics` 에 한 줄 추가. 새 도메인 첫 telemetry 가
...
1. `legacy metrics backend module` 의 register 가 init time side effect 인지 명시 호출인지
   (§5.1)
...
- `legacy metrics backend module` (3059 LOC, godfile cap 3000 위반)
