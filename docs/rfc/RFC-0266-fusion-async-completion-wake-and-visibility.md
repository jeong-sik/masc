...
- registry는 append-only JSONL로 완료 이력을 복원한다. 시작 event append가 실패하면 Fusion worker를 시작하지 않는다. completion event append 실패는 실제 성공/실패 결과와 분리된 typed `Persistence_failed` receipt로 남아 tool/waiting inventory/dashboard에 나타난다. restart replay의 미완료 `Running`은 `Recovery_required Worker_process_restarted`로 보존된다. 실제 재실행에는 durable canonical request/channel이 필요하며 잔여 P0다.
