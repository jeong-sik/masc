...
let cleanup_temp_base dir =
  ...
  if
    Filename.dirname dir = temp_dir
    && has_prefix ~prefix:"autoresearch-knowledge-" basename
  then (
...
let test_record_and_search () =
  with_temp_base "autoresearch-knowledge-roundtrip"
  @@ fun base_path ->
  let f = make_finding () in
  let result = Autoresearch_knowledge.record_finding ~base_path ~finding:f in
  let ok =
    Yojson.Safe.Util.(member "ok" result |> to_bool_option) |> Option.value ~default:false
  in
  check bool "record ok" true ok;
  let found = Autoresearch_knowledge.search_findings ~base_path ~query:"attention" () in
  check bool "found at least 1" true (List.length found >= 1);
  let has_ours =
    List.exists (fun (r : Autoresearch_knowledge.finding) -> r.id = f.id) found
  in
  check bool "found our finding" true has_ours
;;

let test_search_no_match () =
  with_temp_base "autoresearch-knowledge-empty"
  @@ fun base_path ->
  let found =
    Autoresearch_knowledge.search_findings ~base_path ~query:"xyzzy_nonexistent_42" ()
  in
  check int "no results" 0 (List.length found)
;;

let test_base_path_isolation () =
  with_temp_base "autoresearch-knowledge-a"
  @@ fun base_a ->
  with_temp_base "autoresearch-knowledge-b"
  @@ fun base_b ->
  let f =
    make_finding
  ...
  check
    ...
    "base A finds record"
    true
    (List.exists (fun (r : Autoresearch_knowledge.finding) -> r.id = f.id) in_a);
...
let test_record_and_search_dispatch () =
  with_temp_base "autoresearch-knowledge-dispatch"
  @@ fun base_path ->
  ...
  in
  check bool "record dispatch ok" true Yojson.Safe.Util.(member "ok" record |> to_bool);
  let search =
    dispatch_json
      ctx
  ...
  in
  check int "dispatch search count" 1 Yojson.Safe.Util.(member "count" search |> to_int)
;;

let test_record_dispatch_rejects_invalid_input () =
  with_temp_base "autoresearch-knowledge-invalid-record"
  @@ fun base_path ->
  let ctx = make_ctx base_path in
  ...
  let cases =
    ...
    ; ( "invalid confidence"
      , `Assoc (("confidence", `String "hihg") :: valid_record_fields unique) )
    ; ( "non-string tag"
      , `Assoc (("tags", `List [ `String "ok"; `Int 1 ]) :: valid_record_fields unique) )
    ; ( "negative cycle_start"
      , `Assoc (("cycle_start", `Int (-1)) :: valid_record_fields unique) )
    ; ( "reversed cycle range"
      , `Assoc
          (("cycle_start", `Int 3) :: ("cycle_end", `Int 1) :: valid_record_fields unique)
      )
    ]
  in
  List.iter
    (fun (label, args) ->
       ignore (dispatch_error label ctx ~name:"masc_autoresearch_record_finding" ~args))
    cases;
  let search =
    dispatch_json
      ctx
...
let test_search_dispatch_rejects_invalid_input () =
  ...
  List.iter
    (fun (label, args) ->
       ignore (dispatch_error label ctx ~name:"masc_autoresearch_search_findings" ~args))
    cases
;;

let test_search_dispatch_limit_returns_recent_matches () =
  with_temp_base "autoresearch-knowledge-limit"
  @@ fun base_path ->
  let ctx = make_ctx base_path in
  let unique = "limit-token-autoresearch-findings" in
  let record id =
    ignore
      (Autoresearch_knowledge.record_finding
         ~base_path
  ...
  let ids =
    Yojson.Safe.Util.(
      member "findings" search
      |> to_list
      |> List.map (fun json -> member "id" json |> to_string))
  in
  check (list string) "most recent limited matches" [ "fn-limit-3"; "fn-limit-2" ] ids
;;
...
let test_lineage_contract_normalizes_finding_tags () =
  ...
  check
    ...
    (Autoresearch_lineage.finding_tags
  ...
  run
    ...
    [ ( "record_search"
      , [ test_case "record and search roundtrip" `Quick test_record_and_search
        ...
        ; test_case
            "record dispatch rejects invalid input"
            `Quick
            test_record_dispatch_rejects_invalid_input
        ; test_case
            "search dispatch rejects invalid input"
            `Quick
            test_search_dispatch_rejects_invalid_input
        ; test_case
            "search dispatch limit returns recent matches"
            `Quick
            test_search_dispatch_limit_returns_recent_matches
        ] )
    ; ( "lineage"
      , [ test_case
            "lineage contract normalizes finding tags"
            `Quick
