...
let dashboard_planning_http_json ~(config : Coord.config) : Yojson.Safe.t =
  ...
  `Assoc
    ...
    ; "coordination_fsm", Coordination_product_snapshot.safe_build_yojson config
    ]
;;
    ...
    `Assoc
      (fields
       @ [ "keeper", `String keeper_name
         ; "execution", composite_execution_receipt_json ~config ~keeper_name
         ])
  | other -> other
;;
