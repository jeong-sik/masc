...
let row_mentions_post_id post_id rows =
  let wanted = "\"" ^ post_id ^ "\"" in
  List.exists
    (fun json ->
       Yojson.Safe.to_string json |> String_util.contains_substring wanted)
    rows
...
let test_delete_rewrite_failure_keeps_resurrection_scheduled () =
  ...
  Alcotest.(check bool)
    ...
    (row_mentions_post_id doomed_id rows);
  Alcotest.(check bool)
    "surviving post is still on disk" true (row_mentions_post_id survivor_id rows);
  Alcotest.(check bool)
