...
let test_default_backend () =
  Alcotest.(check string) "default is jsonl"
    "jsonl" (Masc_board_handlers.Board_dispatch.backend_name ())

let test_backend_returns_jsonl () =
  match Masc_board_handlers.Board_dispatch.backend () with
  | Masc_board_handlers.Board_dispatch.Jsonl _ -> ()

(** {1 Post CRUD via Dispatch} *)

let test_create_and_get_post () =
  match
    Masc_board_handlers.Board_dispatch.create_post ~author:"test-agent" ~content:"dispatch test post"
...
let test_keeper_signal_hook_failure_does_not_abort_create_post () =
  Masc_board_handlers.Board_dispatch.set_board_signal_hook (fun _ ->
      failwith "keeper signal hook failed");
  match
    Masc_board_handlers.Board_dispatch.create_post ~author:"test-agent"
...
let test_dedup_hit_does_not_emit_post_created_fanout () =
  ...
  Masc_board_handlers.Board_dispatch.set_board_signal_hook (fun _ -> incr keeper_signals);
  Masc_board_handlers.Board_dispatch.set_board_sse_hook (function
    | Masc_board_handlers.Board_dispatch.Post_created _ -> incr sse_post_created
    | _ -> ());
  let create () =
    Masc_board_handlers.Board_dispatch.create_post ~author:"dedup-agent"
...
let test_structured_post_roundtrip () =
  ...
  match Masc_board_handlers.Board_dispatch.create_post ~author:"sangsu"
...
let test_board_sse_post_created_includes_post_kind () =
  ...
  Masc_board_handlers.Board_dispatch.set_board_sse_hook (fun event -> seen := Some event);
  (match
     Masc_board_handlers.Board_dispatch.create_post ~author:"sse-agent"
   ...
   | Ok _ -> ());
  match !seen with
  | Some (Masc_board_handlers.Board_dispatch.Post_created { post_kind; _ }) ->
...
let test_list_posts () =
  ignore (Masc_board_handlers.Board_dispatch.create_post ~author:"lister" ~content:"list test 1"
            ~post_kind:Board.Human_post ());
  ignore (Masc_board_handlers.Board_dispatch.create_post ~author:"lister" ~content:"list test 2"
            ~post_kind:Board.Human_post ());
  let posts = Masc_board_handlers.Board_dispatch.list_posts ~limit:10 () in
  Alcotest.(check bool) "at least 2 posts" true (List.length posts >= 2)

let test_list_posts_with_sort () =
  let posts_hot = Masc_board_handlers.Board_dispatch.list_posts ~sort_by:Masc_board_handlers.Board_dispatch.Hot ~limit:5 () in
  let posts_recent = Masc_board_handlers.Board_dispatch.list_posts ~sort_by:Masc_board_handlers.Board_dispatch.Recent ~limit:5 () in
  let posts_trending = Masc_board_handlers.Board_dispatch.list_posts ~sort_by:Masc_board_handlers.Board_dispatch.Trending ~limit:5 () in
  let posts_updated = Masc_board_handlers.Board_dispatch.list_posts ~sort_by:Masc_board_handlers.Board_dispatch.Updated ~limit:5 () in
  let posts_discussed = Masc_board_handlers.Board_dispatch.list_posts ~sort_by:Masc_board_handlers.Board_dispatch.Discussed ~limit:5 () in
...
let test_recent_sort_bypasses_hot_cutoff () =
  ...
  let vote_up_exn ~post_id ~voter =
    match Masc_board_handlers.Board_dispatch.vote ~voter ~post_id ~direction:Board.Up with
    | Ok _ -> ()
    | Error e -> Alcotest.fail (Board.show_board_error e)
  in
  for i = 1 to 101 do
  ...
  let recent_posts =
    Masc_board_handlers.Board_dispatch.list_posts ~sort_by:Masc_board_handlers.Board_dispatch.Recent ~limit:1 ()
  in
  let hot_posts =
    Masc_board_handlers.Board_dispatch.list_posts ~sort_by:Masc_board_handlers.Board_dispatch.Hot ~limit:1 ()
  in
  let recent_post_id =
...
let test_list_posts_with_filters () =
  ...
  in
  ignore (Masc_board_handlers.Board_dispatch.create_post ~author:"filter-human" ~content:"human-filter-test"
            ~post_kind:Board.Human_post ());
  ignore (Masc_board_handlers.Board_dispatch.create_post ~author:"filter-harness-bot"
            ~content:"automation" ~visibility:Board.Internal ~ttl_hours:1
            ~hearth:"dashboard-harness" ~post_kind:Board.Automation_post ());
  ignore (Masc_board_handlers.Board_dispatch.create_post ~author:"filter-keeper" ~content:"keeper"
            ~post_kind:Board.Automation_post ~meta_json:keeper_meta ());
  let all_posts =
    Masc_board_handlers.Board_dispatch.list_posts ~sort_by:Masc_board_handlers.Board_dispatch.Recent ~limit:50 ()
    |> List.filter is_scoped_author
  in
  let no_system =
    Masc_board_handlers.Board_dispatch.list_posts ~sort_by:Masc_board_handlers.Board_dispatch.Recent ~exclude_system:true
      ~limit:50 ()
    |> List.filter is_scoped_author
  in
  let no_automation =
    Masc_board_handlers.Board_dispatch.list_posts ~sort_by:Masc_board_handlers.Board_dispatch.Recent
...
let test_list_posts_matches_comment_author () =
  let matching_post =
    match
      Masc_board_handlers.Board_dispatch.create_post ~author:"post-owner-a"
  ...
  let other_post =
    match
      Masc_board_handlers.Board_dispatch.create_post ~author:"post-owner-b"
  ...
  (match
     Masc_board_handlers.Board_dispatch.add_comment ~post_id:matching_post_id
       ~author:"comment-match-agent" ~content:"I touched this thread" ()
   with
   | Ok _ -> ()
   | Error e -> Alcotest.fail (Board.show_board_error e));
  (match
     Masc_board_handlers.Board_dispatch.add_comment ~post_id:other_post_id
...
let test_author_filter_treats_wildcards_literally () =
  ignore
    (Masc_board_handlers.Board_dispatch.create_post ~author:"wildcard-alpha"
...
let test_reclassify_posts_dry_run_and_apply () =
  ...
  let dry_run = Masc_board_handlers.Board_dispatch.reclassify_posts ~dry_run:true () in
  ...
  (match Masc_board_handlers.Board_dispatch.get_post ~post_id with
   ...
   | Ok fetched ->
       Alcotest.(check string) "legacy row resolves as automation" "automation"
         (Board.post_kind_to_string fetched.post_kind));
  let applied = Masc_board_handlers.Board_dispatch.reclassify_posts ~dry_run:false () in
...
let test_add_and_get_comments () =
  match
    Masc_board_handlers.Board_dispatch.create_post ~author:"commenter" ~content:"post for comments"
      ~post_kind:Board.Human_post ()
  with
  | Error e -> Alcotest.fail (Board.show_board_error e)
  | Ok post ->
      let pid = Board.Post_id.to_string post.id in
      (match Masc_board_handlers.Board_dispatch.add_comment ~post_id:pid ~author:"responder" ~content:"nice post" () with
       | Error e -> Alcotest.fail (Board.show_board_error e)
       | Ok _ -> ());
      match Masc_board_handlers.Board_dispatch.get_comments ~post_id:pid with
      | Error e -> Alcotest.fail (Board.show_board_error e)
      | Ok comments ->
...
let test_comment_persists_post_reply_count () =
  let post =
    match
      Masc_board_handlers.Board_dispatch.create_post ~author:"automation-author"
  ...
  (match
     Masc_board_handlers.Board_dispatch.add_comment ~post_id ~author:"automation-commenter"
...
let test_get_post_and_comments_atomic () =
  match
    Masc_board_handlers.Board_dispatch.create_post ~author:"atomic-author"
  ...
  | Ok post ->
      let pid = Board.Post_id.to_string post.id in
      (match
         Masc_board_handlers.Board_dispatch.add_comment ~post_id:pid ~author:"first"
           ~content:"first comment" ()
       with
       | Error e -> Alcotest.fail (Board.show_board_error e)
       | Ok _ -> ());
      (match
         Masc_board_handlers.Board_dispatch.add_comment ~post_id:pid ~author:"second"
           ~content:"second comment" ()
       with
       | Error e -> Alcotest.fail (Board.show_board_error e)
       | Ok _ -> ());
      match Masc_board_handlers.Board_dispatch.get_post_and_comments ~post_id:pid with
      | Error e -> Alcotest.fail (Board.show_board_error e)
      | Ok (fetched, comments) ->
...
let test_get_post_and_comments_missing_post () =
  match Masc_board_handlers.Board_dispatch.get_post_and_comments ~post_id:"never-existed" with
...
let test_vote_post () =
  match
    Masc_board_handlers.Board_dispatch.create_post ~author:"voter-test" ~content:"vote me"
      ~post_kind:Board.Human_post ()
  with
  | Error e -> Alcotest.fail (Board.show_board_error e)
  | Ok post ->
      let pid = Board.Post_id.to_string post.id in
      match Masc_board_handlers.Board_dispatch.vote ~voter:"judge" ~post_id:pid ~direction:Board.Up with
      | Error e -> Alcotest.fail (Board.show_board_error e)
      | Ok score ->
...
let test_vote_dedup () =
  match
    Masc_board_handlers.Board_dispatch.create_post ~author:"dedup-test" ~content:"dedup vote"
      ~post_kind:Board.Human_post ()
  with
  | Error e -> Alcotest.fail (Board.show_board_error e)
  | Ok post ->
      let pid = Board.Post_id.to_string post.id in
      ignore (Masc_board_handlers.Board_dispatch.vote ~voter:"same-voter" ~post_id:pid ~direction:Board.Up);
      match Masc_board_handlers.Board_dispatch.vote ~voter:"same-voter" ~post_id:pid ~direction:Board.Up with
...
let test_vote_flip () =
  match
    Masc_board_handlers.Board_dispatch.create_post ~author:"flip-test" ~content:"flip vote"
      ~post_kind:Board.Human_post ()
  with
  | Error e -> Alcotest.fail (Board.show_board_error e)
  | Ok post ->
      let pid = Board.Post_id.to_string post.id in
      ignore (Masc_board_handlers.Board_dispatch.vote ~voter:"flipper" ~post_id:pid ~direction:Board.Up);
      match Masc_board_handlers.Board_dispatch.vote ~voter:"flipper" ~post_id:pid ~direction:Board.Down with
      | Error e -> Alcotest.fail (Board.show_board_error e)
      | Ok score ->
...
let test_current_vote_lookup () =
  ...
  match
    Masc_board_handlers.Board_dispatch.create_post ~author:"state-test" ~content:"state vote"
      ~post_kind:Board.Human_post ()
  with
  | Error e -> Alcotest.fail (Board.show_board_error e)
  | Ok post ->
      let pid = Board.Post_id.to_string post.id in
      (match Masc_board_handlers.Board_dispatch.current_vote_for_post ~voter:"reader" ~post_id:pid with
       | Error e -> Alcotest.fail (Board.show_board_error e)
       | Ok vote ->
           Alcotest.(check (option string)) "post vote starts empty" None
             (vote_label vote));
      ignore (Masc_board_handlers.Board_dispatch.vote ~voter:"reader" ~post_id:pid ~direction:Board.Up);
      (match Masc_board_handlers.Board_dispatch.current_vote_for_post ~voter:"reader" ~post_id:pid with
       | Error e -> Alcotest.fail (Board.show_board_error e)
       | Ok vote ->
           Alcotest.(check (option string)) "post vote state" (Some "up")
             (vote_label vote));
      (match
         Masc_board_handlers.Board_dispatch.add_comment ~post_id:pid ~author:"commenter"
       ...
       | Ok comment ->
           ...
           (match
              Masc_board_handlers.Board_dispatch.current_vote_for_comment ~voter:"reader"
                ~comment_id:cid
            with
            | Error e -> Alcotest.fail (Board.show_board_error e)
            | Ok vote ->
                Alcotest.(check (option string)) "comment vote starts empty" None
                  (vote_label vote));
           ignore
             (Masc_board_handlers.Board_dispatch.vote_comment ~voter:"reader" ~comment_id:cid
  ...
  try
    ...
    Eio_context.with_test_env
      ...
      (fun () ->
        ...
        let post_id =
          match
            Masc_board_handlers.Board_dispatch.create_post ~author:"persist-test" ~content:"persist my vote"
              ~post_kind:Board.Human_post ()
          with
          | Error e -> Alcotest.fail (Board.show_board_error e)
          | Ok post -> Board.Post_id.to_string post.id
        in
        (match Masc_board_handlers.Board_dispatch.vote ~voter:"judge" ~post_id ~direction:Board.Up with
         | Error e -> Alcotest.fail (Board.show_board_error e)
         | Ok _ -> ());
        let store =
          match Masc_board_handlers.Board_dispatch.backend () with
          | Masc_board_handlers.Board_dispatch.Jsonl store -> store
        in
  ...
  try
    ...
    Eio_context.with_test_env
      ...
      (fun () ->
        ...
        Board.reset_global_for_test ();
        Masc_board_handlers.Board_dispatch.reset_for_test ();
        Masc_board_handlers.Board_dispatch.force_flusher_start_cas_conflicts_for_test 2;
        Masc_board_handlers.Board_dispatch.init_jsonl ();
        Alcotest.(check bool)
          "flusher starts after forced CAS contention" true
          (Masc_board_handlers.Board_dispatch.flusher_started_for_test ());
        Eio.Switch.fail sw Exit)
  with Exit -> ()
...
let test_flusher_start_backoff_delay_doubles_and_caps () =
  Alcotest.(check (float 0.0001))
    ...
    (Masc_board_handlers.Board_dispatch.flusher_start_backoff_delay_for_test ~attempt:0);
  Alcotest.(check (float 0.0001))
    "attempt 1 delay doubles" 0.002
    (Masc_board_handlers.Board_dispatch.flusher_start_backoff_delay_for_test ~attempt:1);
  Alcotest.(check (float 0.0001))
    "large attempt caps" 0.02
    (Masc_board_handlers.Board_dispatch.flusher_start_backoff_delay_for_test ~attempt:10)
  ...
  | Ok post ->
      ...
      List.iter
        (fun (target_type, target_id, user_id, emoji) ->
           match
             Masc_board_handlers.Board_dispatch.toggle_reaction ~target_type ~target_id ~user_id
               ~emoji
           with
           | Ok _ -> ()
           | Error e -> Alcotest.fail (Board.show_board_error e))
      ...
      let rows =
        Masc_board_handlers.Board_dispatch.list_reactions_batch
          ~targets:
            [
...
let test_search () =
  ignore (Masc_board_handlers.Board_dispatch.create_post ~author:"searcher"
            ~content:"unique_dispatch_search_term" ~post_kind:Board.Human_post ());
  let results = Masc_board_handlers.Board_dispatch.search ~query:"unique_dispatch_search_term" ~limit:10 in
  Alcotest.(check bool) "found search result" true (List.length results >= 1)

let test_hearths () =
  ignore (Masc_board_handlers.Board_dispatch.create_post ~author:"hearth-test" ~content:"fire topic"
...
let test_set_thread_id () =
  match
    Masc_board_handlers.Board_dispatch.create_post ~author:"thread-test" ~content:"link me"
      ~post_kind:Board.Human_post ()
  with
  | Error e -> Alcotest.fail (Board.show_board_error e)
  | Ok post ->
      let pid = Board.Post_id.to_string post.id in
      match Masc_board_handlers.Board_dispatch.set_thread_id ~post_id:pid ~thread_id:"thread-abc" with
      | Error e -> Alcotest.fail (Board.show_board_error e)
      | Ok () ->
          match Masc_board_handlers.Board_dispatch.get_post ~post_id:pid with
          | Error e -> Alcotest.fail (Board.show_board_error e)
...
let test_flush () =
  Masc_board_handlers.Board_dispatch.flush ()

(** {1 Validation} *)

let test_empty_content () =
  match
    Masc_board_handlers.Board_dispatch.create_post ~author:"validator" ~content:""
      ~post_kind:Board.Human_post ()
  with
...
let test_invalid_author () =
  match
    Masc_board_handlers.Board_dispatch.create_post ~author:"" ~content:"valid content"
      ~post_kind:Board.Human_post ()
  with
...
let test_jsonl_forced_default () =
  ...
  Alcotest.(check bool) "empty env not forced"
    false (Masc_board_handlers.Board_dispatch.jsonl_forced ())

let test_jsonl_forced_explicit () =
  Unix.putenv "MASC_BOARD_BACKEND" "jsonl";
  Alcotest.(check bool) "jsonl is forced"
    true (Masc_board_handlers.Board_dispatch.jsonl_forced ());
...
let test_sub_board_create_and_get () =
  (match Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"alpha-board" ~name:"Alpha"
  ...
  | Ok sb ->
      ...
      (match Masc_board_handlers.Board_dispatch.get_sub_board ~sub_board_id:id with
       | Error e -> Alcotest.fail (Board.show_board_error e)
       | Ok fetched ->
           Alcotest.(check string) "fetched id" id (Board.Sub_board_id.to_string fetched.id)));
  (* lookup by slug *)
  (match Masc_board_handlers.Board_dispatch.get_sub_board ~sub_board_id:"alpha-board" with
   | Error e -> Alcotest.fail (Board.show_board_error e)
   | Ok fetched ->
       Alcotest.(check string) "slug lookup" "alpha-board" fetched.Board.slug)

let test_sub_board_list () =
  ignore (Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"list-a" ~name:"A" ~description:""
            ~owner:"agent-1" ());
  ignore (Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"list-b" ~name:"B" ~description:""
            ~owner:"agent-2" ());
  let all = Masc_board_handlers.Board_dispatch.list_sub_boards () in
  Alcotest.(check bool) "has sub-boards" true (List.length all >= 2)

let test_sub_board_slug_conflict () =
  ignore (Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"conflict-slug" ~name:"First"
            ~description:"" ~owner:"agent-1" ());
  (match Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"conflict-slug" ~name:"Second"
...
let test_sub_board_delete () =
  (match Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"delete-me" ~name:"Temp"
           ~description:"" ~owner:"agent-1" () with
  | Error e -> Alcotest.fail (Board.show_board_error e)
  | Ok sb ->
      let id = Board.Sub_board_id.to_string sb.Board.id in
      (match Masc_board_handlers.Board_dispatch.delete_sub_board ~sub_board_id:id with
       | Error e -> Alcotest.fail (Board.show_board_error e)
       | Ok () ->
           (match Masc_board_handlers.Board_dispatch.get_sub_board ~sub_board_id:id with
...
let test_sub_board_create_delete_persisted_snapshot () =
  let created =
    Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"persisted-team" ~name:"Persisted"
      ~description:"" ~owner:"agent-1" ()
  in
  let id =
    match created with
...
let test_sub_board_access_default_open () =
  (match Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"open-board" ~name:"Open"
           ~description:"" ~owner:"agent-1" () with
  | Error e -> Alcotest.fail (Board.show_board_error e)
  | Ok sb ->
...
let test_sub_board_members_include_owner () =
  (match Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"member-board" ~name:"Members"
...
let test_sub_board_members_only_post_policy () =
  ignore
    (Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"policy-team" ~name:"Policy"
       ~description:"" ~owner:"agent-owner" ~members:["agent-member"]
       ~access:Board.Members_only ());
  (match
     Masc_board_handlers.Board_dispatch.create_post ~author:"agent-outsider" ~content:"nope"
  ...
  (match
     Masc_board_handlers.Board_dispatch.create_post ~author:"agent-member" ~content:"allowed"
       ~post_kind:Board.Human_post ~hearth:" POLICY-TEAM " ()
   with
   | Error e -> Alcotest.fail (Board.show_board_error e)
   | Ok _ -> ());
  (match
     Masc_board_handlers.Board_dispatch.create_post ~author:"agent-owner" ~content:"owner allowed"
       ~post_kind:Board.Human_post ~hearth:"policy-team" ()
   with
   | Error e -> Alcotest.fail (Board.show_board_error e)
   | Ok _ -> ())
...
let test_sub_board_owner_only_post_policy () =
  ignore
    (Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"owner-space" ~name:"Owner"
       ~description:"" ~owner:"agent-owner" ~members:["agent-member"]
       ~access:Board.Owner_only ());
  (match
     Masc_board_handlers.Board_dispatch.create_post ~author:"agent-member" ~content:"member denied"
...
let test_sub_board_members_only_comment_policy () =
  ignore
    (Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"comment-policy-team"
  ...
  in
  (match
     Masc_board_handlers.Board_dispatch.add_comment ~post_id ~author:"agent-outsider"
  ...
  (match
     Masc_board_handlers.Board_dispatch.add_comment ~post_id ~author:"agent-member"
       ~content:"member reply" ()
   with
   | Error e -> Alcotest.fail (Board.show_board_error e)
   | Ok _ -> ());
  (match
     Masc_board_handlers.Board_dispatch.add_comment ~post_id ~author:"agent-owner"
       ~content:"owner reply" ()
   with
   | Error e -> Alcotest.fail (Board.show_board_error e)
   | Ok _ -> ())
...
let test_sub_board_owner_only_comment_policy () =
  ignore
    (Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"comment-owner-space"
...
let test_sub_board_open_comment_policy_allows_anyone () =
  ignore
    (Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"comment-open"
  ...
  in
  (match
     Masc_board_handlers.Board_dispatch.add_comment ~post_id ~author:"agent-random"
...
let test_sub_board_update () =
  let id =
    match Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"update-target" ~name:"Before"
             ~description:"old desc" ~owner:"agent-owner" ~access:Board.Open () with
    | Error e -> Alcotest.fail (Board.show_board_error e)
    | Ok sb -> Board.Sub_board_id.to_string sb.Board.id
  in
  (match Masc_board_handlers.Board_dispatch.update_sub_board ~sub_board_id:id ~name:"After"
...
let test_sub_board_delete_clears_orphan_hearth () =
  let sb_id =
    match Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"orphan-hearth" ~name:"Orphan"
             ~description:"" ~owner:"agent-owner" () with
    | Error e -> Alcotest.fail (Board.show_board_error e)
    | Ok sb -> Board.Sub_board_id.to_string sb.Board.id
  in
  let post_result =
    Masc_board_handlers.Board_dispatch.create_post ~author:"agent-owner" ~content:"post in orphan"
  ...
  (match Masc_board_handlers.Board_dispatch.delete_sub_board ~sub_board_id:sb_id with
   | Error e -> Alcotest.fail (Board.show_board_error e)
   | Ok () -> ());
  (match Masc_board_handlers.Board_dispatch.get_post ~post_id with
   | Error e -> Alcotest.fail (Board.show_board_error e)
...
let test_sub_board_post_count_projection () =
  ignore
    (Masc_board_handlers.Board_dispatch.create_sub_board ~slug:"counted" ~name:"Counted"
       ~description:"" ~owner:"agent-owner" ());
  ignore
    (Masc_board_handlers.Board_dispatch.create_post ~author:"agent-a" ~content:"one"
       ~post_kind:Board.Human_post ~hearth:"counted" ());
  ignore
    (Masc_board_handlers.Board_dispatch.create_post ~author:"agent-b" ~content:"two"
       ~post_kind:Board.Human_post ~hearth:"COUNTED" ());
  ignore
    (Masc_board_handlers.Board_dispatch.create_post ~author:"agent-c" ~content:"other"
       ~post_kind:Board.Human_post ~hearth:"other" ());
  (match Masc_board_handlers.Board_dispatch.get_sub_board ~sub_board_id:"counted" with
   | Error e -> Alcotest.fail (Board.show_board_error e)
   | Ok sb -> Alcotest.(check int) "derived post_count" 2 sb.Board.post_count);
  let listed =
    Masc_board_handlers.Board_dispatch.list_sub_boards ()
