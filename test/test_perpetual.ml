...
let test_history_offload () = group "History Offload" (fun () ->
  ...
  (* 6. Summary message contains offload path annotation.
     ...
     When use_oas_reducer=false, SummarizeOld produces [MASC_MEMORY_SUMMARY v1]
     prefix which gets the annotation injected. Both behaviors are correct. *)
  ...
  let oas_reducer_on =
    try Sys.getenv "MASC_USE_OAS_REDUCER" = "false" |> not
    with Not_found -> true in
  if oas_reducer_on then
    (* OAS SummarizeOld is truncation-based, no summary prefix = no annotation target *)
