...
let test_alias_creates_redirect () =
  ...
  check bool "canonical is a redirect stub" true (Option.is_some canonical_target);
  (* Now write the short-form alias. *)
  (match
     Auth.ensure_credential_alias tmp ~canonical_name:"keeper-foo-agent" ~alias_name:"foo"
   with
   | Ok () -> ()
  ...
  check
    (option string)
    "alias points at the same UUID file"
    canonical_target
    (alias_redirect_basename alias_path);
  ...
  check string "alias resolves to same token" (token_of direct) (token_of via_alias)
;;
...
let test_alias_idempotent () =
  ...
  Fun.protect ~finally:(fun () -> cleanup_test_room tmp)
  @@ fun () ->
  let _ = Auth.ensure_keeper_credential tmp ~agent_name:"keeper-bar-agent" in
  let _ =
    Auth.ensure_credential_alias tmp ~canonical_name:"keeper-bar-agent" ~alias_name:"bar"
  in
  ...
  (match
   ...
   with
   | Ok () -> ()
   | Error e -> failf "second call failed: %s" (Types.masc_error_to_string e));
  let mtime_after = (Unix.stat alias_path).Unix.st_mtime in
  check
    (float 0.001)
    "second call did not rewrite the file (idempotent)"
    mtime_before
    mtime_after
;;
...
let test_self_alias_noop () =
  ...
  Fun.protect ~finally:(fun () -> cleanup_test_room tmp)
  @@ fun () ->
  let _ = Auth.ensure_keeper_credential tmp ~agent_name:"keeper-baz-agent" in
  match
    Auth.ensure_credential_alias
      tmp
      ~canonical_name:"keeper-baz-agent"
      ~alias_name:"keeper-baz-agent"
  with
  | Ok () -> ()
  | Error e -> failf "self-alias should be no-op: %s" (Types.masc_error_to_string e)
;;
...
let () =
  run
    "credential_alias_10440"
    [ ( "ensure_credential_alias"
      , [ test_case
            "creates redirect stub at short-form path"
            `Quick
            test_alias_creates_redirect
        ; test_case "idempotent on second call" `Quick test_alias_idempotent
        ; test_case "self-alias is a no-op" `Quick test_self_alias_noop
        ; test_case
            "errors on missing canonical credential"
            `Quick
