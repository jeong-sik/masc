...
let warm_meta_cognition_summary (config : Lib.Coord.config) =
  let key =
    Lib.Server_dashboard_http.dashboard_cache_key
      config
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Eio.Switch.run (fun sw ->
         ...
         check
           int
           "pending confirms zero"
           0
           (json
            |> member "operator"
            |> member "pending_confirm_summary"
            |> member "total_count"
            |> to_int);
         check
           int
           "configured keepers default to zero"
           0
           (json |> member "root" |> member "configured_keepers" |> to_int);
         check
           int
           "namespace counts expose total runtimes"
           0
           (json |> member "root" |> member "counts" |> member "total_runtimes" |> to_int);
         check
           bool
           "readiness status exposed"
           true
           (String.length (json |> member "readiness" |> member "status" |> to_string) > 0);
         check
           int
           "readiness exposes four pillars"
           4
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Eio.Switch.run (fun sw ->
         ...
         check
           int
           "fixture blocked operations"
           0
           (json
            |> member "execution"
            |> member "summary"
            |> member "blocked_operations"
            |> to_int);
         (* top_queue is null when cache has no execution_queue entries *)
         check
           bool
           "fixture top queue absent when no blockers"
           true
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Eio.Switch.run (fun sw ->
         ...
         check
           bool
           "empty room focus mentions no agents"
           true
           (String.length focus_label > 0 && focus_label <> "지금은 namespace 전체가 비교적 안정적입니다")))
;;
...
let test_dashboard_namespace_truth_keeper_only_room_not_reported_empty () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Eio.Switch.run (fun sw ->
         Fun.protect
           ~finally:(fun () -> Lib.Keeper_keepalive.stop_keepalive "sangsu")
           (fun () ->
              ...
              check
                int
                "keeper-only room counts general agents as zero"
                0
                (json |> member "root" |> member "counts" |> member "agents" |> to_int);
              check
                int
                "keeper-only room still counts keeper meta"
                1
                (json |> member "root" |> member "counts" |> member "keepers" |> to_int);
              check
                bool
                "keeper-only room does not report empty room focus"
                false
                (String.equal focus_label "등록된 런타임이 없습니다. 활동이 시작되면 여기에 포커스가 나타납니다."))))
;;
       ...
       Eio.Switch.run (fun sw ->
         Fun.protect
           ...
           (fun () ->
              ...
              check
                int
                "mixed room counts one general agent"
                1
                (json |> member "root" |> member "counts" |> member "agents" |> to_int);
              check
                int
                "mixed room counts one keeper"
                1
                (json |> member "root" |> member "counts" |> member "keepers" |> to_int);
              check
                bool
                "mixed room avoids empty runtime fallback"
                false
                (String.equal focus_label "등록된 런타임이 없습니다. 활동이 시작되면 여기에 포커스가 나타납니다."))))
;;
       ...
       Eio.Switch.run (fun sw ->
         ...
         List.iter
           (fun key ->
              ...
              check bool (Printf.sprintf "operator.%s present" key) true (value <> `Null))
           expected_keys))
;;
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Eio.Switch.run (fun sw ->
         ...
         check
           int
           "meta contested belief count surfaced"
           1
           (json
            |> member "meta_cognition"
            |> member "summary"
            |> member "contested_belief_count"
            |> to_int);
         check
           string
           "meta interpretation primary surfaced"
           "contested_belief"
           (json
            |> member "meta_cognition"
            |> member "interpretation"
            |> member "primary_salience"
            |> to_string);
         check
           string
           "operator attention points at meta cognition"
           "namespace_meta_cognition"
           (json
            |> member "operator"
            |> member "attention_summary"
            |> member "top_item"
            |> member "target_type"
            |> to_string);
         check
           string
           "focus source becomes meta cognition"
           "meta_cognition"
           (json |> member "focus" |> member "source" |> to_string);
         check
           string
           "focus jumps to overview"
           "overview"
           (json |> member "focus" |> member "suggested_tab" |> to_string);
         check
           bool
           "focus params stay empty for overview"
           true
       ...
       Eio.Switch.run (fun sw ->
         ignore
           (Lib.Server_dashboard_http.dashboard_namespace_truth_http_json
              ...
              (request "/api/v1/dashboard/namespace-truth"));
         let posts =
           Lib.Board_dispatch.list_posts
             ~hearth:"meta-cognition"
             ~post_kind_filter:Lib.Board.Automation_post
             ~sort_by:Lib.Board_dispatch.Recent
             ~limit:10
             ()
         in
         check
           int
           "namespace-truth leaves meta-cognition board empty"
           0
           (List.length posts)))
;;
    ...
    (fun () ->
       ...
       Eio.Switch.run (fun sw ->
         ...
         let compare_block key =
           check
             string
             (Printf.sprintf "%s block matches cached snapshot" key)
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Eio.Switch.run (fun sw ->
         ...
         check
           bool
           "expired warmup skips top-level initializing payload"
           true
           (json |> member "status" = `Null);
         check bool "namespace block present" true (json |> member "root" <> `Null);
         check
           int
           "execution summary falls back to zero operations"
           0
           (json
            |> member "execution"
            |> member "summary"
            |> member "active_operations"
            |> to_int);
         check
           string
           "namespace truth diagnostics keep execution cache state"
           "initializing"
           (json
            |> member "projection_diagnostics"
            |> member "execution_cache_state"
            |> to_string)))
;;
...
let test_last_good_shell_fallback_preserves_counts () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       check bool "last good shell is non-empty after warm" true (last_good <> `Assoc []);
       (* Now verify that _last_good_shell has namespace counts. *)
       let open Yojson.Safe.Util in
       let counts = last_good |> member "counts" in
       check bool "last good shell contains counts block" true (counts <> `Null);
       ...
       check
         bool
         "namespace counts block present in fallback snapshot"
         true
         (ns_counts <> `Null);
    ...
    (fun () ->
       ...
       let snapshot ~generated_at ~active_sessions =
         `Assoc
           ...
           ; ( "execution"
             , `Assoc [ "summary", `Assoc [ "active_sessions", `Int active_sessions ] ] )
           ]
       in
       check
         bool
         "first snapshot broadcasts"
         true
       ...
       check
         bool
         "generated_at-only changes stay deduped"
         false
         (Lib.Server_dashboard_http.should_broadcast_namespace_truth_snapshot
            (snapshot ~generated_at:"2026-04-09T00:00:05Z" ~active_sessions:1));
       check
         bool
         "semantic changes still broadcast"
         true
         (Lib.Server_dashboard_http.should_broadcast_namespace_truth_snapshot
...
let () =
  Alcotest.run
    "Dashboard Namespace Truth"
    [ ( "read_model"
      , [ test_case "empty room shape" `Quick test_dashboard_namespace_truth_empty_room
        ; test_case
            "execution fixture surfaces top queue"
            `Quick
            test_dashboard_namespace_truth_execution_fixture
        ; test_case
            "empty room focus label reflects no agents"
            `Quick
            test_dashboard_namespace_truth_empty_room_focus_label
        ; test_case
            "keeper-only room does not look empty"
            `Quick
            test_dashboard_namespace_truth_keeper_only_room_not_reported_empty
        ; test_case
            "mixed runtimes keep counts aligned"
            `Quick
            test_dashboard_namespace_truth_mixed_runtime_counts
        ; test_case
            "operator digest shape matches namespace-truth"
            `Quick
            test_operator_digest_shape_matches_namespace_truth
        ; test_case
            "meta cognition can drive namespace-truth focus"
            `Quick
            test_dashboard_namespace_truth_promotes_meta_cognition_focus
        ; test_case
            "namespace-truth does not auto-post meta digest"
            `Quick
            test_dashboard_namespace_truth_does_not_auto_post_meta_digest
        ; test_case
            "cached snapshot matches HTTP projection blocks"
            `Quick
            test_namespace_truth_cached_snapshot_matches_http_projection_blocks
        ; test_case
            "expired execution warmup falls back to partial truth"
            `Quick
            test_dashboard_namespace_truth_cold_cache_falls_back_to_partial_truth
        ; test_case
            "last-good shell fallback preserves namespace counts"
            `Quick
            test_last_good_shell_fallback_preserves_counts
        ; test_case
            "snapshot hash ignores generated_at churn"
            `Quick
