  ...
  run
    ...
    [ ( "mcp_session_record"
      , [ test_case "roundtrip with agent_name" `Quick (fun () ->
            ...
            in
            let json = Mcp_server_eio_governance.mcp_session_to_json s in
            let s' = Mcp_server_eio_governance.mcp_session_of_json json in
            check
              (option pass)
              "roundtrip succeeds"
              (Some ())
    ...
    ; ( "governance_config"
      , [ test_case "save roundtrip via to_yojson" `Quick (fun () ->
            let g : Mcp_server_eio_governance.governance_config =
              { level = "production"; audit_enabled = true; anomaly_detection = false }
            in
            let json = Mcp_server_eio_governance.governance_config_to_yojson g in
            match Mcp_server_eio_governance.governance_config_of_yojson json with
            | Ok g' ->
        ...
        ; test_case "extra keys in governance JSON" `Quick (fun () ->
            ...
            | Error msg -> Alcotest.failf "should ignore extra keys: %s" msg)
        ] )
    ]
;;
