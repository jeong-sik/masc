...
OAS는 `dune-project`에 `agent_sdk >= 0.81.0`으로 명시된 의존성이며, CI에서는 `scripts/opam-pin-external-deps.sh`를 통해 `jeong-sik/oas#main`을 pin해 현재 upstream runtime floor와 같은 버전대로 검증한다.
