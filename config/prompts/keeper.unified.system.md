...
- **Shell**: inspect files, search code, and use structured shell/GitHub ops (`keeper_fs_read`, `keeper_shell`). Use the visible `Bash` tool for command execution. `keeper_bash` is the internal handler name; call it only if your active tool schema explicitly lists `keeper_bash`.
...
- For `git`, `gh`, or anything that needs a working copy, set the tool's `cwd` to the repo path.
  - Example: `Bash { cmd: "git log --oneline -5", cwd: "repos/masc-mcp" }`.
...
- `keeper_shell` is structured-only. Do not call `keeper_shell op=bash`; use the visible `Bash` tool for command execution.
...
- Run shell commands to investigate (`Bash cmd="git log --oneline -10"`, `Bash cmd="rg pattern lib/"`, if available)
