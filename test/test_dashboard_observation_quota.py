    ...
    with tempfile.TemporaryDirectory(prefix='masc-observation-quota-') as temporary:
        ...
        with (base / 'server.log').open('wb') as log:
            ...
            try:
                ...
                assert status == 429 and json.loads(raw)['message'] == 'Per-agent rate limit exceeded', (status, raw)
                # Permission-guarded GETs are observations on the two other
                # wrappers: with_permission_auth and with_token_permission_auth.
                # Classified in with_read_auth alone, a dashboard poll of these
                # kept spending the operation bucket and answered 429 here.
                permission_reads = ('/api/v1/dashboard/workspace-memory-context',
                                    '/api/v1/dashboard/standalone-lanes')
                for path in permission_reads:
                ...
                print('DASHBOARD_OBSERVATION_QUOTA ' + json.dumps({'protocol': 'h1', 'asset_reads_before_operations': 60,
                    ...
                    'permission_reads_after_exhaustion': list(permission_reads),
