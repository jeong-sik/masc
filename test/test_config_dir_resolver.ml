...
let make_inputs
      ...
      ?env_home
      ?(cwd = "/tmp/cwd")
      ?(executable_name = "/tmp/bin/masc-mcp")
      ()
  =
  Lib.Config_dir_resolver.
    { cwd; executable_name; env_base_path; env_config_dir; env_personas_dir; env_home }
;;
...
let test_sanitize_inherited_test_env_opt_drops_captured_parent_shell_value () =
  let actual =
    Lib.Config_dir_resolver.sanitize_inherited_test_env_opt
      ~running_under_test_executable:true
...
let test_sanitize_inherited_test_env_opt_keeps_runtime_override () =
  ...
  in
  check (option string) "runtime override preserved" (Some "/tmp/test-config-root") actual
;;
...
let test_sanitize_inherited_test_env_opt_keeps_value_with_opt_in () =
  ...
  check
    (option string)
    "opt-in preserves config-path override"
    (Some "/Users/dancer/me/workspace/yousleepwhen/masc-mcp/config")
    actual
;;

let test_normalize_masc_base_path_input_canonicalizes_explicit_path () =
  let actual =
    Env_config_core.normalize_masc_base_path_input "/Users/dancer/me/././/.masc//"
  in
  check string "canonical explicit base path" "/Users/dancer/me" actual
;;

let test_env_override_valid () =
  with_temp_dir "config-dir-env"
  @@ fun root ->
  let config = make_config_root root in
  let resolution =
    Lib.Config_dir_resolver.resolve_with (make_inputs ~env_config_dir:config ())
  in
  check
    string
    "status"
...
let test_env_override_valid_with_toml_only_root () =
  with_temp_dir "config-dir-env-toml"
  @@ fun root ->
...
let test_cwd_fallback_opt_in () =
  with_temp_dir "config-dir-cwd-opt-in"
  @@ fun cwd ->
  let _config = make_config_root cwd in
...
let test_local_masc_fallback_precedes_home_masc () =
  with_temp_dir "config-dir-local"
  @@ fun root ->
...
let test_local_masc_fallback_collapses_explicit_masc_dir () =
  with_temp_dir "config-dir-local-masc"
  @@ fun root ->
  let target = Filename.concat root "target" in
  let local_config = make_config_root (Filename.concat target Common.masc_dirname) in
  let resolution =
...
let test_no_legacy_me_root_fallback () =
  with_temp_dir "config-dir-no-legacy"
  @@ fun me_root ->
  let _repo_root = Filename.concat me_root "workspace/yousleepwhen/masc-mcp" in
  let resolution =
    Lib.Config_dir_resolver.resolve_with
      (make_inputs ~cwd:"/tmp/missing-cwd" ~executable_name:"/tmp/nonexistent-masc" ())
  in
  check
    string
    "status"
...
let test_personas_dirs_default_repo_only () =
  ...
  check (list string) "repo personas only" [ Filename.concat config "personas" ] dirs
;;

let test_personas_dirs_ignores_home_fallback () =
  with_temp_dir "pd-home"
  @@ fun root ->
  let config = make_config_root root in
  let home = Filename.concat root "home" in
  ...
  check
    (list string)
    "repo personas only despite HOME fallback"
    [ Filename.concat config "personas" ]
    dirs
;;

let test_personas_dirs_env_override_is_sole_source () =
  with_temp_dir "pd-env"
  @@ fun root ->
...
let test_personas_dirs_ignores_base_path_fallback () =
  ...
  let base_personas =
    Filename.concat (Filename.concat base Common.masc_dirname) "personas"
  in
  mkdir_p base_personas;
  let config_personas = Filename.concat config_root "personas" in
  let inputs = make_inputs ~env_config_dir:config_root ~env_base_path:base ~cwd:root () in
  let resolution = Lib.Config_dir_resolver.resolve_with inputs in
  let dirs = Lib.Config_dir_resolver.personas_dirs_with inputs resolution in
  check (list string) "base path fallback ignored" [ config_personas ] dirs
;;
  ...
  run
    ...
    [ ( "resolution"
      , [ test_case "env override valid" `Quick test_env_override_valid
        ; test_case
            "env override valid with toml-only root"
            `Quick
            test_env_override_valid_with_toml_only_root
        ; test_case
            "env override invalid does not fallback"
            `Quick
            test_env_override_invalid_no_fallback
        ; test_case
            "cwd fallback disabled by default"
            `Quick
            test_cwd_fallback_disabled_by_default
        ; test_case "cwd fallback opt-in" `Quick test_cwd_fallback_opt_in
        ; test_case
            "exe-relative fallback opt-in"
            `Quick
            test_executable_relative_fallback_opt_in
        ; test_case
            "local masc fallback precedes home masc"
            `Quick
            test_local_masc_fallback_precedes_home_masc
        ; test_case
            "local masc fallback collapses explicit .masc dir"
            `Quick
            test_local_masc_fallback_collapses_explicit_masc_dir
        ; test_case "home masc fallback" `Quick test_home_masc_fallback
        ; test_case
            "does not fallback to legacy me_root repo path"
            `Quick
            test_no_legacy_me_root_fallback
        ] )
    ; ( "personas_dirs"
      , [ test_case
            "default returns repo personas only"
            `Quick
            test_personas_dirs_default_repo_only
        ; test_case
            "ignores HOME .masc/personas fallback"
            `Quick
            test_personas_dirs_ignores_home_fallback
        ; test_case
            "MASC_PERSONAS_DIR overrides as sole source"
            `Quick
            test_personas_dirs_env_override_is_sole_source
        ; test_case
            "ignores base_path .masc/personas fallback"
            `Quick
            test_personas_dirs_ignores_base_path_fallback
        ] )
    ; ( "test_env_sanitization"
      , [ test_case
            "drops captured parent-shell config env by default"
            `Quick
            test_sanitize_inherited_test_env_opt_drops_captured_parent_shell_value
        ; test_case
            "keeps runtime override"
            `Quick
        ...
        ; test_case
            "canonicalizes explicit base path"
            `Quick
