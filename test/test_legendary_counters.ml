...
let test_initial_zero () =
  ...
  Alcotest.(check int)
    ...
    0
    s.gate_diff_legacy_allow_shadow_deny;
  Alcotest.(check int)
    "gate_diff_legacy_deny_shadow_allow"
    0
    s.gate_diff_legacy_deny_shadow_allow;
  Alcotest.(check int) "gate_diff_shadow_cannot_parse" 0 s.gate_diff_shadow_cannot_parse;
  Alcotest.(check int) "auto_bg_observed" 0 s.auto_bg_observed;
  Alcotest.(check int) "auto_bg_would_have_promoted" 0 s.auto_bg_would_have_promoted
;;
...
let test_gate_diff_buckets () =
  ...
  Alcotest.(check int)
    "legacy_allow_shadow_deny = 1"
    1
    s.gate_diff_legacy_allow_shadow_deny;
  Alcotest.(check int)
    "legacy_deny_shadow_allow = 1"
    1
    s.gate_diff_legacy_deny_shadow_allow;
  Alcotest.(check int) "shadow_cannot_parse = 1" 1 s.gate_diff_shadow_cannot_parse
;;
...
let test_auto_bg_observed () =
  ...
  Alcotest.(check int) "auto_bg_would_have_promoted = 1" 1 s.auto_bg_would_have_promoted
;;
...
let test_snapshot_json_shape () =
  ...
  Legendary_counters.incr_gate_diff Gate_diff_types.Legacy_allow_shadow_deny;
  Legendary_counters.incr_auto_bg_observed ~promoted_candidate:true;
  let json = Legendary_counters.snapshot_to_json (Legendary_counters.snapshot ()) in
  ...
  Alcotest.(check bool)
    "has legacy_allow_shadow_deny"
    true
    (Astring.String.is_infix ~affix:"\"gate_diff_legacy_allow_shadow_deny\":1" s);
  Alcotest.(check bool)
    "has auto_bg_would_have_promoted"
    true
    (Astring.String.is_infix ~affix:"\"auto_bg_would_have_promoted\":1" s)
;;
...
let test_reset () =
  ...
  Alcotest.(check int) "post-reset would_have_promoted" 0 s.auto_bg_would_have_promoted;
...
let test_ratios_zero_denominator () =
  ...
  Legendary_counters.reset ();
  let s = Legendary_counters.snapshot () in
  check_ratio "disagree_ratio zero" ~expected:0.0 (Legendary_counters.disagree_ratio s);
  check_ratio
    "shadow_parse_coverage zero"
    ~expected:0.0
    (Legendary_counters.shadow_parse_coverage s);
  check_ratio
    "auto_bg_promotion_rate zero"
    ~expected:0.0
    (Legendary_counters.auto_bg_promotion_rate s)
;;
...
let test_disagree_ratio_math () =
  ...
  check_ratio "disagree_ratio = 3/10" ~expected:0.3 (Legendary_counters.disagree_ratio s)
;;
...
let test_shadow_parse_coverage_full () =
  ...
  let s = Legendary_counters.snapshot () in
  check_ratio "coverage = 1.0" ~expected:1.0 (Legendary_counters.shadow_parse_coverage s)
;;
...
let test_snapshot_to_json_with_ratios_shape () =
  ...
  Legendary_counters.reset ();
  let json =
    Legendary_counters.snapshot_to_json_with_ratios (Legendary_counters.snapshot ())
  in
  let s = Yojson.Safe.to_string json in
  Alcotest.(check bool)
  ...
  Alcotest.run
    ...
    ; ( "too_complex_histogram"
      , [ Alcotest.test_case "prefixed tag" `Quick test_too_complex_prefixed_tag
        ...
        ; Alcotest.test_case
            "parse_error dedicated bucket"
            `Quick
            test_too_complex_parse_error
        ; Alcotest.test_case
            "parse_aborted dedicated bucket"
            `Quick
    ...
    ; ( "derived_ratios"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case "disagree_ratio math" `Quick test_disagree_ratio_math
        ; Alcotest.test_case
            "shadow_parse_coverage math"
            `Quick
            test_shadow_parse_coverage_math
        ; Alcotest.test_case
            "shadow_parse_coverage full"
            `Quick
            test_shadow_parse_coverage_full
        ; Alcotest.test_case
            "auto_bg_promotion_rate math"
            `Quick
            test_auto_bg_promotion_rate_math
        ; Alcotest.test_case
            "JSON with ratios sibling (zero)"
            `Quick
            test_snapshot_to_json_with_ratios_shape
        ; Alcotest.test_case
            "JSON with ratios sibling (populated)"
            `Quick
