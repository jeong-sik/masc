...
export function EquipmentList({ items }: { items: string[] }) {
  if (items.length === 0) return html`<div class="py-2 px-3 text-xs text-[var(--color-fg-muted)] italic v2-monitoring-row">장비 없음</div>`

  return html`
    <div class="flex flex-col gap-1.5 v2-monitoring-row">
      ${items.map((item, i) => html`
        <div class="flex items-center justify-between py-2 px-3 rounded-[var(--r-1)] bg-[var(--color-bg-surface)] v2-monitoring-row">
...
export function RelationshipList({ rels }: { rels: Record<string, string> }) {
  ...
  if (entries.length === 0) return html`<div class="py-2 px-3 text-xs text-[var(--color-fg-muted)] italic v2-monitoring-row">관계 없음</div>`

  return html`
    <div class="max-h-55 overflow-y-auto flex flex-col gap-1.5 v2-monitoring-row">
      ${entries.map(([name, relation]) => html`
        <div class="flex items-center gap-2 py-2 px-3 bg-[var(--color-bg-surface)] rounded-[var(--r-1)] v2-monitoring-row">
