(** One operation's keeper chat turn as a seq-ordered event log (RFC-0412
    §3.3, stage 3a). The live SSE decoder and the v2 journal endpoint feed the
    same log; {!Masc_tui_keeper_chat_transcript} projects it. *)
...
(** Every entry of a v2 page, in journal order, through {!delta_of_journaled};
    lines already held by seq are skipped. *)
...
(** Total over the journal event type: one arm per constructor, no wildcard.
    An event the live view draws nothing for — adapter-only blocks, stream
    bookkeeping, message start and end — maps to [[]], exactly where the
    server's AG-UI projection maps to [None] or to a frame the live decoder
    ignores. The stage-3a golden test pins that a journal page and the wire
    decode to the same deltas. *)
