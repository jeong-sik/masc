...
agent_core sends the caller's typed `tool_choice` to the selected provider but does
not re-interpret a text response as a tool call, validate the response against
an agent_core-owned forced-tool completion contract, or retry to coerce a tool call.
...
embedding application. This keeps provider request construction in agent_core while
leaving orchestration policy outside the SDK.

## 1. Historical problem

The pre-implementation design mapped `tool_choice` to an agent_core completion
...
- Text remains text; agent_core has no text-to-tool recovery fallback.
- agent_core performs no automatic retry or corrective prompt injection to enforce
...
Consumers that previously relied on agent_core to coerce a non-compliant backend must:
