...
let test_registered_keeper_and_persistent_agent_lists_split () =
  Eio_main.run @@ fun env ->
  Eio.Switch.run @@ fun sw ->
  let base_dir = temp_dir () in
  Fun.protect
    ...
    (fun () ->
      ...
      in
      let ok, registered_up_body =
        dispatch "masc_keeper_up"
          (`Assoc
            [
              ("name", `String "keeper-demo");
              ("goal", `String "Stay active");
              ("presence_keepalive", `Bool false);
              ("proactive_enabled", `Bool false);
            ])
      in
      if not ok then fail registered_up_body;
      ...
      let ok, registered_body =
        dispatch "masc_keeper_list" (`Assoc [ ("detailed", `Bool false) ])
      in
      check bool "registered keeper list ok" true ok;
      let registration_json = Yojson.Safe.from_string registered_body in
      check bool "registered keeper listed" true
        Yojson.Safe.Util.(
          registration_json |> member "keepers" |> to_list
...
let test_registered_and_persistent_detailed_lists_annotate_runtime_class () =
  Eio_main.run @@ fun env ->
  Eio.Switch.run @@ fun sw ->
  let base_dir = temp_dir () in
  Fun.protect
    ...
    (fun () ->
      ...
      in
      check bool "registered keeper up ok" true ok;
      let ok, _ =
        dispatch "masc_persistent_agent_up"
          (`Assoc
            [
      ...
      in
      check bool "registered detailed list ok" true ok;
      let registration_json = parse_json_exn registered_body in
      let registered_row =
        Yojson.Safe.Util.(
          registration_json |> member "keepers" |> to_list
      ...
      check string "keeper runtime_class" "keeper"
        Yojson.Safe.Util.(registered_row |> member "runtime_class" |> to_string);
      check bool "registered desired mirrors registration" true
        Yojson.Safe.Util.(registered_row |> member "desired" = `Bool true);
      check bool "registered keeper" true
        Yojson.Safe.Util.(registered_row |> member "registered" |> to_bool);
      let ok, persistent_body =
...
let test_registered_list_items_expose_runtime_config_summary () =
  Eio_main.run @@ fun env ->
  Eio.Switch.run @@ fun sw ->
  let base_dir = temp_dir () in
  Fun.protect
...
let test_registered_keeper_msg_bootstraps_then_requires_message () =
  Eio_main.run @@ fun env ->
  Eio.Switch.run @@ fun sw ->
  let base_dir = temp_dir () in
  Fun.protect
    ...
    (fun () ->
      ...
      check bool "keeper bootstrap created keeper" true
        (Masc_mcp.Keeper_types.is_registered_keeper config "bootstrap-demo"))
    ...
    (fun () ->
      ...
      let ok, persistent_status_body =
        dispatch "masc_persistent_agent_status"
          (`Assoc
            [
              (* The persistent-agent alias should still surface keeper registration
      ...
      check bool "persistent alias marks registered keeper" true
        Yojson.Safe.Util.(persistent_status_json |> member "registered" |> to_bool);
      let ok, _ =
        dispatch "masc_keeper_down" (`Assoc [ ("name", `String "keeper-demo") ])
      in
      check bool "registered keeper down ok" true ok;
      check bool "keeper registration removed" false
        (Masc_mcp.Keeper_types.is_registered_keeper config "keeper-demo"))
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      let registration_path =
        Filename.concat base_dir ".masc/keeper-registrations/sangsu.json"
      in
      check bool "registration exists" true (Sys.file_exists registration_path);
      let registration_json = Yojson.Safe.from_file registration_path in
      check string "registration name" "sangsu"
        Yojson.Safe.Util.(registration_json |> member "name" |> to_string))
...
let test_write_meta_syncs_registered_registered_seed () =
  Eio_main.run @@ fun env ->
  Eio.Switch.run @@ fun sw ->
  let base_dir = temp_dir () in
  Fun.protect
    ...
    (fun () ->
      ...
      let registration_path =
        Filename.concat base_dir ".masc/keeper-registrations/buddy.json"
      in
      check bool "registration exists" true (Sys.file_exists registration_path);
      let registration_json = Yojson.Safe.from_file registration_path in
      check string "registration name" "buddy"
        Yojson.Safe.Util.(registration_json |> member "name" |> to_string);
      check bool "voice_enabled synced in boot entry" false
        Yojson.Safe.Util.(registration_json |> member "voice_enabled" |> to_bool);
      check string "voice_channel synced in boot entry" "text_only"
        Yojson.Safe.Util.(registration_json |> member "voice_channel" |> to_string);
      check string "voice_agent_id synced in boot entry" ""
        Yojson.Safe.Util.(registration_json |> member "voice_agent_id" |> to_string);
      check bool "seed_meta absent in thin format" true
        (Yojson.Safe.Util.(registration_json |> member "seed_meta") = `Null);
            ...
            let agent : Types.agent =
              {
                ...
                agent_type = "keeper";
                status = Types.Active;
                capabilities = [ "keeper"; "registered" ];
...
let test_registered_bootstrap_marks_stale_explicit_keeper () =
  Eio_main.run @@ fun env ->
  Eio.Switch.run @@ fun sw ->
  let base_dir = temp_dir () in
  Fun.protect
...
let test_registered_supervisor_recovers_missing_desired_keeper () =
  Eio_main.run @@ fun env ->
  Eio.Switch.run @@ fun sw ->
  let base_dir = temp_dir () in
  Fun.protect
...
let () =
  run "Tool_keeper" [
    ("read_file_tail_lines", [
         ...
         test_case "registered and persistent lists split" `Quick
           test_registered_keeper_and_persistent_agent_lists_split;
         test_case "registered and persistent detailed lists annotate runtime class" `Quick
           test_registered_and_persistent_detailed_lists_annotate_runtime_class;
         test_case "registered keeper list items expose runtime config summary" `Quick
           test_registered_list_items_expose_runtime_config_summary;
         ...
         test_case "keeper msg bootstraps then requires message" `Quick
           test_registered_keeper_msg_bootstraps_then_requires_message;
         ...
         test_case "keeper bootstrap marks stale explicit keeper" `Quick
           test_registered_bootstrap_marks_stale_explicit_keeper;
         test_case "keeper supervisor recovers missing desired keeper" `Quick
           test_registered_supervisor_recovers_missing_desired_keeper;
