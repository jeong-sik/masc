...
(* The remaining turn budget is not a keeper pause/stop/admission reason. It is
   still the upper bound for the nested provider attempt's OAS timeout, so a
   late attempt cannot run longer than the turn budget it is nested inside. *)
