  ...
  Eio_guard.with_mutex cache_mu (fun () ->
    Hashtbl.filter_map_inplace
      (fun key entry ->
         ...
         | Some idx ->
           let cached_name = String.sub key (idx + 1) (String.length key - idx - 1) in
...
let tail_order_of_args args =
  ...
  | "newest_first" | "newest" | "latest_first" | "desc" -> Newest_first
  | _ -> Oldest_first
;;
...
let resolve_status_target (ctx : _ context) args =
  ...
  then Error "❌ invalid keeper name"
  else (
...
let latest_metrics_json ~metrics_store ~metrics_path ~tail_bytes =
  ...
  in
  let parsed, _ = Fs_compat.parse_jsonl_lines ~source:"keeper_metrics_latest" lines in
  match
    List.rev parsed
...
let provider_scope_of_model_label model_label =
  match
    Option.bind (Option.bind model_label nonempty_trimmed) (fun label ->
      ...
      | Some idx when idx > 0 ->
        Some (String.sub label 0 idx |> String.trim |> String.lowercase_ascii)
      | _ -> None)
  with
...
let lightweight_runtime_contract_json ~selected_model ~runtime_blocker_class =
  ...
  let proof_note = "Lightweight status only. Use masc_runtime_verify for proof." in
  ...
  then
    `Assoc
      ...
      ; ( "note"
        , `String
            (if provider_scope = "non_local"
             then "Selected model is not a local llama/ollama runtime. " ^ proof_note
             else "Selected model is unknown. " ^ proof_note) )
      ]
  else (
    let endpoints_opt =
    ...
    let provider_reachable =
      ...
      | Some endpoints when endpoints <> [] ->
        Some
          (List.exists (fun (ep : Discovery_cache.endpoint_info) -> ep.healthy) endpoints)
      | _ -> None
    in
    ...
    `Assoc
      ...
      ; ( "provider_reachable"
        , match provider_reachable with
          ...
          | None -> `Null )
      ; "healthy_runtime_count", Json_util.int_opt_to_json healthy_runtime_count
...
let attempt_summary_json
  ...
  | None ->
    `Assoc
      [ ( "summary"
        , `String
            "No recent cascade observation for current keeper config. Showing configured \
             labels only." )
  ...
  | Some cascade ->
    ...
    let summary =
      ...
      | true, Some hops, Some pos, total when total > 0 ->
        Printf.sprintf
          "%d attempt(s); fallback after %d hop(s); selected candidate %d/%d."
          attempts_observed
          hops
          pos
          total
      ...
      | false, _, Some pos, total when total > 0 ->
        Printf.sprintf
          "%d attempt(s); selected candidate %d/%d without fallback."
          attempts_observed
          pos
          total
      | _, _, _, _ when Option.is_some selected_model ->
        Printf.sprintf
          "%d attempt(s) observed; selected model reported without candidate index."
          attempts_observed
...
let latest_cascade_for_current_config
      ~current_cascade_name
      ~configured_labels
      latest_metrics
  =
  let latest_cascade =
...
let model_observability_json
  ...
  =
  let latest_cascade =
    latest_cascade_for_current_config
      ~current_cascade_name
      ~configured_labels
      latest_metrics
  in
  ...
  in
  let cascade_name = Option.value ~default:"" (nonempty_trimmed current_cascade_name) in
  ...
  in
  `Assoc
    [ ("cascade_name", if cascade_name = "" then `Null else `String cascade_name)
    ; "recent_turn_observation", `Bool (Option.is_some latest_cascade)
    ; "configured_labels", string_list_to_json configured_labels_surface
    ...
    ; ( "attempt_summary"
      , attempt_summary_json
          ~configured_labels:configured_labels_surface
          ~resolved_candidates
          ~selected_model
          latest_cascade )
    ; ( "runtime_contract"
      , lightweight_runtime_contract_json ~selected_model ~runtime_blocker_class )
    ]
;;
...
let handle_keeper_status ctx args : tool_result =
  ...
  | Ok (name, m) ->
    ...
    (match
       Eio_guard.with_mutex_ro cache_mu (fun () -> Hashtbl.find_opt _cache cache_key)
     with
     | Some entry when entry.updated_at = m.updated_at && entry.args_hash = args_hash ->
       true, entry.response
     | _ ->
       ...
       let runtime_cascade_metrics =
         ...
         | `List entries ->
           ...
           |> List.find_opt (function
             | `Assoc fields ->
               List.assoc_opt "cascade_name" fields = Some (`String m.cascade_name)
             | _ -> false)
                ...
                Keeper must not reference OAS provider_kind directly. *)
                   ...
                   Some
                     (`Assoc
                         ...
                         ; ( "max_context"
                           , `Int (Cascade_runtime.max_context_of_label label) )
                         ; ( "max_output_tokens"
                           , Option.fold
                               ~none:`Null
                               ~some:(fun tokens -> `Int tokens)
                               cfg.max_tokens )
                         ; ( "max_output_tokens"
                           , match cfg.max_tokens with
                             ...
                             | None -> `Null )
                         ; ( "api_key_env"
                           , if cfg.api_key <> "" then `String "(set)" else `Null )
       ...
       let generation_index_path = keeper_generation_index_path ctx.config m.name in
       let session_dir =
         keeper_session_dir ctx.config (Keeper_id.Trace_id.to_string m.runtime.trace_id)
       in
       let generation_manifest_path =
         keeper_generation_manifest_path
           ctx.config
           (Keeper_id.Trace_id.to_string m.runtime.trace_id)
       in
       let history_path =
         keeper_history_path ctx.config (Keeper_id.Trace_id.to_string m.runtime.trace_id)
       in
       let internal_history_path =
         keeper_internal_history_path
           ctx.config
           (Keeper_id.Trace_id.to_string m.runtime.trace_id)
       in
       ...
       let metrics_tail =
         let lines =
           ...
           then dated
             ...
             read_file_tail_lines metrics_path ~max_bytes:tail_bytes ~max_lines:tail_turns
         in
         let parsed, _ = Fs_compat.parse_jsonl_lines ~source:"keeper_metrics" lines in
         `List (apply_tail_order tail_order parsed)
       in
       let metrics_window_lines =
         if include_metrics_overview
         then (
           let n = max tail_turns 200 in
           let dated = Dated_jsonl.read_recent_lines metrics_store n in
           if dated <> []
           then dated
           else read_file_tail_lines metrics_path ~max_bytes:tail_bytes ~max_lines:n)
         else []
       in
       let metrics_overview =
         ...
         else empty_metrics_summary
       in
       let last_skill_route =
         if not include_metrics_overview
         then None
         else
           ...
           let rec find_latest = function
             ...
             | line :: tl ->
               (try
                  ...
                  | _ -> find_latest tl
                with
                | Yojson.Json_error _ -> find_latest tl)
           in
               ...
               (fun (acc, raw_count, fragment_count, filtered_count) line ->
                  try
                    ...
                    let entry_kind =
                      ...
                      | "direct_user", _ | "direct_assistant", _ -> "direct_conversation"
                    ...
                    in
                    let is_fragment =
                      role_lc = "assistant" && looks_fragmentary_history_text content
                    in
                    let should_filter =
                    ...
                    let preview =
                      ...
                      then utf8_safe_prefix_bytes content ~max_bytes:200 ^ "..."
                      else content
                    in
                    let item =
                      `Assoc
                    ...
                    , filtered_count )
                  with
                  | Yojson.Json_error _ -> acc, raw_count, fragment_count, filtered_count)
               ([], 0, 0, 0)
               lines
           in
       ...
       let compaction_history_tail =
         if not include_compaction_history
         then `List [], 0
         else (
             ...
             List.fold_left
               (fun acc line ->
                  try
                    ...
                    in
                    if (not compacted) && not memory_compaction_performed
                    then acc
                    else (
                      ...
                      in
                      let event_kind =
                        if compacted && memory_compaction_performed
                        then "context+memory"
                        else if compacted
                        then "context"
                        else "memory"
                      ...
                      let item =
                        `Assoc
                          ...
                          ; ( "channel"
                            , `String (Safe_ops.json_string ~default:"turn" "channel" j) )
                          ; "ts_unix", `Float ts_unix
                          ; "age_s", Json_util.float_opt_to_json age_s
                          ; ( "trace_id"
                            , `String (Safe_ops.json_string ~default:"" "trace_id" j) )
                          ; ( "generation"
                            , `Int
                                (Safe_ops.json_int
                                   ~default:m.runtime.generation
                                   "generation"
                                   j) )
                          ; ( "context_ratio"
                            , `Float (Safe_ops.json_float ~default:0.0 "context_ratio" j)
                            )
                          ...
                          ; ( "memory_compaction_performed"
                            , `Bool memory_compaction_performed )
                          ...
                          ; ( "memory_reason"
                            , match
                                Safe_ops.json_string_opt "memory_compaction_reason" j
                              with
                              | Some reason when String.trim reason <> "" ->
                                `String reason
                              | _ -> `Null )
       ...
       let allowed_tool_preview = allowed_tools |> List.filteri (fun idx _ -> idx < 10) in
         ...
         | None ->
           ...
           ; tool_audit_at =
               (if last_autonomous <> ""
                ...
                then Some m.updated_at
                else None)
           }
       in
       let blocked_internal_tools =
         all_internal_tools |> List.filter (fun name -> not (List.mem name allowed_tools))
       in
       ...
       let effective_sandbox_image =
         if
           m.sandbox_profile = Docker
           || (m.sandbox_profile = Local && Env_config_keeper.DockerPlayground.enabled)
       ...
       let sandbox_live =
         Keeper_sandbox_control.live_status_json
           ...
           ()
       in
       let runtime_blocker_fields = runtime_blocker_fields_json ctx.config m in
       let attention_fields = attention_fields_json ctx.config m in
       let latest_metrics =
         latest_metrics_json ~metrics_store ~metrics_path ~tail_bytes
       in
       let model_observability =
         model_observability_json
           ~current_cascade_name:m.cascade_name
       ...
       in
       let runtime_trust =
         Keeper_runtime_trust_snapshot.snapshot_json ~config:ctx.config ~meta:m
       in
       let disposition = json_string_opt_member runtime_trust "disposition" in
       ...
       let json =
         `Assoc
           ([ "name", `String name
            ...
            ; ( "policy"
              , `Assoc
                  [ ( "voice_tools_available"
                    , `Bool (List.mem "keeper_voice_speak" allowed_tools) )
                  ; ( "sandbox_profile"
            ...
            ; ( "social"
              , `Assoc
                  ...
                  ; ( "recognized_model"
                    , `Bool (Keeper_social_model.is_known_social_model m.social_model) )
                  ; ( "fallback_model"
            ...
            @ [ ( "compaction_policy"
              ...
              ; ( "storage_paths"
                , `Assoc
                    ...
                    ; ( "dataset_export"
                      , `String (keeper_dataset_export_path ctx.config m.name) )
              ...
              Docker, host_root_abs for Local. *)
                 let keeper_visible_abs =
                   Keeper_sandbox.keeper_visible_root_abs sandbox
                 in
                 ( "execution_context"
                 , `Assoc
                     [ "sandbox_id", `String sandbox.sandbox_id
                     ; ( "sandbox_backend"
                       , `String (Keeper_sandbox.backend_to_string sandbox.backend) )
                     ...
                     ; ( "sandbox_container_root"
                       , Json_util.string_opt_to_json sandbox.container_root )
                     ...
                     ; ( "playground_repos"
                       , let cache_path =
                         ...
                         try
                           ...
                           | _ -> `List []
                         with
                         | Sys_error _ | Yojson.Json_error _ -> `List [] )
                     ; ( "pr_history"
                       , let pr_path =
                           Filename.concat playground_abs ".playground_pr_history.jsonl"
                         in
                         try
                     ...
                     ; ( "active_worktrees"
                       , let worktrees_dir =
                           Filename.concat ctx.config.base_path ".worktrees"
                         in
                         try
                           let entries = Sys.readdir worktrees_dir |> Array.to_list in
                           let keeper_prefix =
                             Keeper_alerting_path.sanitize_keeper_name m.name
                           in
                           let matching =
                             List.filter
                               (fun name ->
