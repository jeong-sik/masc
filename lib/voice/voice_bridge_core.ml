...
let tuning_for_agent agent_id =
  ...
  | Error _ -> { Voice_config.stability = 0.5; similarity_boost = 0.75; style = 0.0 }
;;
...
let client_for_uri_result ~sw ~net uri =
  ...
  | exn -> Error (Printf.sprintf "HTTPS client init error: %s" (Printexc.to_string exn))
;;
...
let log_info msg = Log.info "%s %s" log_prefix msg
let log_error msg = Log.error "%s %s" log_prefix msg
let log_debug msg = Log.debug "%s %s" log_prefix msg

let split_path_env value =
  String.split_on_char ':' value |> List.filter (fun entry -> String.trim entry <> "")
;;
...
let find_executable_in_path ?path_value executable =
  ...
  in
  let candidates =
    split_path_env path_value |> List.map (fun dir -> Filename.concat dir executable)
  in
  List.find_opt
    (fun path -> Sys.file_exists path && not (Sys.is_directory path))
    candidates
;;
...
let run_local_playback ~sw:_ ~agent_id ?message ~audio_file () =
  ...
  | Error e ->
    ...
    `Played None
  | Ok config ->
    if not (Voice_config.local_playback_enabled_for_agent config agent_id)
    then `Played None
    else (
      ...
      | Some argv ->
        Eio.Mutex.use_rw ~protect:true playback_mu (fun () ->
          ...
          then (
            log_info
              (Printf.sprintf
                 "voice dedup skip inside mutex: agent=%s (same message within %.0fs \
                  window)"
                 agent_id
            ...
            | exn ->
              log_error
                (Printf.sprintf "voice playback exception: %s" (Printexc.to_string exn));
              `Played None)))
;;
...
let ensure_audio_dir () =
  ...
  then log_error "voice audio path exists but is not a directory"
;;
...
let strip_provider_metadata = function
  | `Assoc fields ->
    `Assoc
      (fields |> List.filter (fun (key, _) -> not (List.mem key provider_metadata_keys)))
  | other -> other
;;
