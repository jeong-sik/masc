...
let process_timeout_observer_fn : (program:string -> timeout_sec:float -> unit) Atomic.t =
...
let observe_process_timeout argv ~timeout_sec =
  try
    (Atomic.get process_timeout_observer_fn) ~program:(argv_program argv) ~timeout_sec
  with
  | exn ->
    Log.Misc.warn "[Process_eio] timeout observer failed: %s" (Printexc.to_string exn)
;;
...
let reset_for_testing () = Atomic.set runtime_state None
...
let process_error_output ?(stderr = "") ~label:_ ~reason () =
  ...
  if stderr = ""
  then Printf.sprintf "process_eio_error: %s" reason
  else Printf.sprintf "process_eio_error: %s\nstderr:\n%s" reason stderr
;;
...
let create_stderr_tempfile () =
  ...
  let fd = Unix.openfile path [ Unix.O_WRONLY; Unix.O_TRUNC; Unix.O_CLOEXEC ] 0o600 in
  path, fd
;;
...
let read_stderr_capture path =
  ...
  | Sys_error msg ->
    Printf.sprintf "(stderr capture error) %s: %s" (Filename.basename path) msg
;;
...
let with_unix_capture
      ...
      ~(on_success : Unix.process_status -> string -> string -> 'a)
  : 'a
  =
  match argv with
  | [] -> on_error "empty argv" ""
  | prog :: _ ->
    ...
    let cleanup () =
      ...
      stderr_fd_ref := None;
      Option.iter remove_temp_file_quietly !stderr_path_ref;
    ...
    (try
       ...
       let stdin_fd =
         ...
         | None -> Unix.stdin
       in
       let pid = create_process_env ?cwd prog argv env stdin_fd stdout_w stderr_fd in
       Option.iter close_quietly !stdin_r_ref;
       stdin_r_ref := None;
       Option.iter close_quietly !stdout_w_ref;
       stdout_w_ref := None;
          ...
          Fun.protect
            ...
            (fun () ->
               let rec write_all off =
                 if off < String.length content
                 then (
                   let n =
                     Unix.write_substring stdin_w content off (String.length content - off)
                   in
                   write_all (off + n))
               in
               write_all 0)
       ...
       | Some stdout_r ->
           ...
           let rec loop () =
             try
               ...
               | n ->
                 ...
                 loop ()
             with
             | Unix.Unix_error ((Unix.EAGAIN | Unix.EWOULDBLOCK), _, _) -> `Would_block
         ...
         while (not !stdout_eof) && not !timed_out do
           ...
           then (
             ...
             let readable =
               try
                 let ready, _, _ = Unix.select [ stdout_r ] [] [] (min 0.05 remaining) in
                 ready <> []
               with
         ...
         let stderr =
           if !timed_out && String.trim stdout = "" && String.trim stderr = ""
           then
         ...
         in
         if !timed_out then observe_process_timeout argv ~timeout_sec;
         cleanup ();
...
let run_unix_argv_fallback ?(timeout_sec = 60.0) ?env (argv : string list) : string =
  ...
  with_unix_capture
    ...
    ~on_success:(fun status stdout stderr -> output_for_status ~status ~stdout ~stderr)
;;

let run_unix_argv_with_status_split_fallback
      ?(timeout_sec = 60.0)
      ?env
      ?cwd
      (argv : string list)
  ...
  with_unix_capture
    ...
    ~on_success:(fun status stdout stderr -> status, stdout, stderr)
;;

let run_unix_argv_with_stdin_fallback
      ?(timeout_sec = 60.0)
      ?env
...
let spawn_and_drain_stdout ~sw pm ~cwd ?env ?stdin_source argv stdout_buf =
  ...
  let proc =
    Eio.Process.spawn ~sw pm ~cwd ?env ?stdin:stdin_source ~stdout:stdout_w argv
  in
  ...
  (try
   ...
   | Eio.Cancel.Cancelled _ as e ->
     (try Eio.Flow.close stdout_r with
      ...
      | exn ->
        Log.Misc.warn
          "spawn_and_drain_stdout: flow close failed: %s"
...
let spawn_and_drain_both ~sw pm ~cwd ?env ?stdin_source argv stdout_buf stderr_buf =
  let stdout_r, stdout_w = Eio.Process.pipe ~sw pm in
  ...
  (try
   ...
   | Eio.Cancel.Cancelled _ as e ->
     (try Eio.Flow.close stdout_r with
      ...
      | exn ->
        Log.Misc.warn
          "spawn_and_drain_both: stdout flow close failed: %s"
          (Printexc.to_string exn));
     (try Eio.Flow.close stderr_r with
      | Eio.Cancel.Cancelled _ as ce -> raise ce
      | exn ->
        Log.Misc.warn
          "spawn_and_drain_both: stderr flow close failed: %s"
          (Printexc.to_string exn));
     raise e);
  let status = Eio.Process.await proc in
  match status with
...
let run_argv ?(timeout_sec = 60.0) ?env (argv : string list) : string =
  ...
  then run_unix_argv_fallback ~timeout_sec ?env argv
  else (
    ...
    | Ok pm, Ok clk, Ok cwd ->
      ...
      (try
       ...
       with
       | Eio.Time.Timeout ->
         Log.Misc.warn "[Process_eio] Timeout after %.0fs: %s" timeout_sec label;
...
let run_argv_with_stdin
  ...
  if not (is_initialized ())
  then run_unix_argv_with_stdin_fallback ~timeout_sec ?env ~stdin_content argv
  else (
    match get_proc_mgr (), get_clock (), get_cwd_default () with
    | Error _, _, _ | _, Error _, _ | _, _, Error _ ->
...
let run_argv_with_stdin_and_status
  ...
  let status, stdout, stderr =
    run_argv_with_stdin_and_status_split ~timeout_sec ?env ?cwd ~stdin_content argv
  in
  status, output_for_status ~status ~stdout ~stderr
;;

let run_argv_with_status_split ?(timeout_sec = 60.0) ?env ?cwd (argv : string list)
  : Unix.process_status * string * string
  =
...
let run_argv_with_status ?(timeout_sec = 60.0) ?env ?cwd (argv : string list)
  : Unix.process_status * string
  =
  let status, stdout, stderr = run_argv_with_status_split ~timeout_sec ?env ?cwd argv in
  status, output_for_status ~status ~stdout ~stderr
;;
  ...
  | bin :: _ ->
    (try
       ...
       then (
         ...
         (try if cwd <> "" then Unix.chdir cwd with
         ...
         Unix.close err_w;
         Unix.close devnull;
         try Unix.execvpe bin (Array.of_list argv) env with
         | Eio.Cancel.Cancelled _ as e -> raise e
         | _ -> Unix._exit 127)
       else (
         (* --- PARENT --- *)
     ...
     | Unix.Unix_error (err, fn, arg) ->
       Error
         (Printf.sprintf
            "spawn_detached %s: %s (%s %s)"
            bin
            (Unix.error_message err)
            fn
            arg)
     | exn -> Error (Printf.sprintf "spawn_detached %s: %s" bin (Printexc.to_string exn)))
;;
