...
첫째 줄은 그 뒤 생긴 이름·링크·인용 경로 규칙과 아래 "넓힌 선택" 이 다룬다.
...
지금 규칙(고친 테스트, 스위트 이름, 스탠자 링크, 인용 경로, 설정 폴더 트리거)을 다 거치고도, 빨간 스위트를 안고 main 에 들어간 PR 이 셋 있다.

| PR | 바꾼 것 | 빨갛게 된 스위트 | 그 스위트와 바뀐 것의 관계 |
|---|---|---|---|
| #36681 | `config/tools/*.toml` 12개 | `test_tool_loading_declarations` | 도구 파일의 `defer_loading` 선언을 읽는다. `config/tools` 트리거 목록에 없었다 |
| #29365 | `Env_config_keeper` 등 모듈 네 개 | `test_runtime_toml_overrides` | 코드가 그 모듈을 부른다. 이름·링크·인용 경로 어디에도 안 걸린다 |
| #36885 | `Runtime_setup_spec`, `scripts/install-runtime-setup.py` | `test_runtime_setup_batch`, `test_server_runtime_setup_actions`, `test_install_runtime_setup.py` | 앞의 둘은 모듈을 부르고, 뒤의 하나는 스크립트를 실행한다 |

### 더한 규칙

1. **모듈을 부르는 스위트.** 바뀐 `bin/`·`lib/`·`packages/*/lib/` 의 `.ml`·`.mli` 마다, 주석과 문자열을 걷어낸 코드에서 그 모듈을 부르는 `.ml` 스위트를 고른다. `Foo.x`, `open`, `include`, `module X = Foo`, `(module Foo : S)` 를 센다.
2. **이름이 하나뿐인 파일을 부르는 스위트.** OCaml 소스가 아니고 `test/` 밖인 파일 중, 추적 파일 가운데 같은 이름이 없는 것만 본다. 스위트 본문에 그 이름이 있으면 고른다. 이름이 겹치면 어느 파일을 말하는지 알 수 없다. `dune` 은 스위트 79개에 나온다. 정확한 경로를 인용한 스위트는 인용 경로 규칙이 이미 고른다.
3. **`config/tools` 트리거에 `test_tool_loading_declarations` 를 더한다.**

구현은 `scripts/ci/referencing_suites.py` 이고 `run-edited-tests.sh` 가 부른다 — **제안이고 아직 트리에 없다.** PR #36928 이 열려 있고, 이 절의 측정치는 그 브랜치에서 잰 값이다. 머지되면 이 문단에서 이 문장을 지운다.

세 PR 의 부모 커밋 트리에서 돌리면 빨간 스위트 다섯 개를 모두 고른다. #29365 는 1번, #36885 는 1번(두 개)과 2번(한 개), #36681 은 3번이 고른다.

기존 이름·링크 규칙은 그대로 둔다. 최근 PR 80개 중 18개에서, 1번이 못 고르는 스위트 30개를 이 두 규칙이 골랐다.

2번의 조건은 이렇게 정했다. 최근 PR 80개에서 파일 이름 규칙 하나만으로 고른 수의 합이다.

| 조건 | 합 |
|---|---|
| 처음 안: `.md`·`docs/`·`.github/`·6자 이하 이름을 뺀다 | 434 |
| 위에서 `docs/` 를 빼지 않으면 | 435 |
| 위에서 `.github/` 를 빼지 않으면 | 434 |
| 위에서 6자 이하 이름을 빼지 않으면 | 908 |
| 채택: 이름이 하나뿐인 파일만 | 75 |

`docs/`·`.github/` 제외는 거의 아무것도 안 바꿨고, 6자라는 숫자는 `dune` 같은 겹치는 이름을 막으려던 것이었다. 겹치는 이름을 직접 빼는 쪽이 이유가 분명하다.

### 얼마나 고르나

origin/main 의 최근 PR 80개(첫 부모 기준)를 현재 트리에 대고 실제 선택 함수로 셌다.

| | p50 | p75 | p90 | 최대 | 0개 | 합 |
|---|---|---|---|---|---|---|
| 넓히기 전 | 3 | 10 | 21 | 131 | 12 | 706 |
| 넓힌 뒤 | 8 | 30 | 78 | 440 | 10 | 2404 |

가장 많이 고른 PR 은 파일 137개를 바꾼 #36828 이다.

### 고른 것은 전부 돌리고, 못 돈 것은 실패로 남긴다

규칙마다 상한을 두면 잘라 낸 만큼이 다시 샌다. 대신 단계에 시간 예산을 둔다. 예산 안에 못 돈 스위트는 이름을 남기고 단계를 실패시킨다. 빠진 스위트가 있는데 초록으로 끝나는 일은 없다.

지금 러너는 스위트를 하나씩 빌드하고 실행한다. run 35216373548 에서 네 개를 돌린 시간이다.

| 스위트 | 빌드와 실행 |
|---|---|
| 첫 번째 | 71초 (공용 라이브러리 컴파일 포함) |
| 나머지 셋 | 9초, 7초, 4초 |

이대로면 p90 인 78개는 대략 71 + 77 × 7 ≈ 610초다. 지금 단계 제한 12분에 붙는다. 그래서 러너를 두 가지 바꾼다.

1. **한 번에 빌드한다.** 직접 실행할 수 있는 `.ml` 스위트의 실행 파일과 deps 를 `dune build` 한 번에 넘기고, `DUNE_JOBS=4` 로 병렬 링크한다. `_build` 는 실행마다 새로 만든다(캐시는 opam switch 뿐이다). 그래서 빌드 뒤 실행 파일이 있으면 이 체크아웃에서 링크된 것이고, 없으면 그 스위트의 빌드 실패다.
2. **예산은 스크립트가 쥔다.** `--budget-seconds` 로 받는다. 빌드와 각 스위트에 `timeout` 을 "스위트 제한과 남은 예산 중 작은 값"으로 건다. 예산이 끝나 시작하지 못했거나 도중에 끊긴 스위트는 이름을 남기고 실패로 센다.

job 제한은 30분이고, 같은 run 에서 job 시작부터 이 단계 시작까지 7분 55초가 걸렸다. 단계 제한을 20분으로 올리고 예산은 18분으로 준다. 남는 2분은 예산이 끝난 뒤 못 돈 스위트를 적고 나갈 몫이다.

한 번에 빌드한 뒤를 측정용 PR #36929 로 쟀다(run 35222082987). `keeper_meta_contract.ml` 에 주석 한 줄만 더해, 이 모듈을 부르는 스위트 70개가 골라졌다.

| 구간 | 시간 |
|---|---|
| job 시작부터 이 단계 시작까지 | 7분 58초 |
| 선택과 스탠자 판정 (스위트 72개) | 17초 |
| 70개 한 번에 빌드 | 130초 |
| 70개 실행 | 68초 |
| 단계 전체 | 3분 35초 |

하나씩 빌드하던 러너로 70개면 대략 71 + 69 × 7 ≈ 550초였다. 결과는 `ran 70, skipped 2`, 실패 없음이다. p90 인 78개도 18분 예산의 4분의 1 안에 든다.

### 이번에 하지 않는 것

- main 필수 체크로 올리지 않는다. 선택 규칙이 자리 잡고 main 빨강이 0 인지 본 뒤 다시 정한다.
- 선택 수에 상한을 두지 않는다.

### 넓힌 선택의 판정 기준

1. 세 PR 의 바뀐 파일 목록이 `run-edited-tests.sh --self-test` 픽스처에 있고, 빨갛던 스위트를 고른다.
2. 러너를 바꾼 뒤, 스위트를 여러 개 고른 PR 의 단계 로그로 스위트당 시간을 잰다. 78개가 예산 안에 드는지 본다. (09-17 #36929: 70개가 3분 35초)
3. 예산을 넘긴 PR 은 못 돈 스위트 이름을 로그에 남기고 빨갛게 끝난다.
