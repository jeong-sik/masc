...
let test_make_error_with_data () =
  let error =
    Mcp_server.make_error
      ~data:(`String "extra info")
      ~id:(`Int 1)
      (-32602)
      "Invalid params"
  in
...
let test_is_notification_true () =
  let req : Mcp_server_eio.jsonrpc_request =
    { jsonrpc = "2.0"; id = None; method_ = "notification"; params = None }
  in
...
let test_is_notification_false () =
  let req : Mcp_server_eio.jsonrpc_request =
    { jsonrpc = "2.0"; id = Some (`Int 1); method_ = "request"; params = None }
  in
...
let test_get_id_some () =
  let req : Mcp_server_eio.jsonrpc_request =
    { jsonrpc = "2.0"; id = Some (`Int 42); method_ = "test"; params = None }
  in
...
let test_get_id_none () =
  let req : Mcp_server_eio.jsonrpc_request =
    { jsonrpc = "2.0"; id = None; method_ = "test"; params = None }
  in
  match Mcp_server.get_id req with
  | `Null -> ()
...
let test_validate_initialize_params_valid () =
  let params =
    `Assoc
      ...
      ; "clientInfo", `Assoc [ "name", `String "test-client"; "version", `String "1.0.0" ]
      ; "capabilities", `Assoc []
      ]
  in
...
let test_jsonrpc_request_of_yojson_minimal () =
  let json = `Assoc [ "jsonrpc", `String "2.0"; "method", `String "notify" ] in
  match Masc_mcp.Mcp_server.jsonrpc_request_of_yojson json with
  | Ok req ->
    ...
    ; ( "validate_initialize_params"
      , [ test_case "valid" `Quick test_validate_initialize_params_valid
        ...
        ; test_case
            "missing version"
            `Quick
            test_validate_initialize_params_missing_version
        ] )
