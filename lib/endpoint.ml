...
let glm_general_base_url =
  env_url_or ~env:"ZAI_BASE_URL"
    ~default:Llm_provider.Zai_catalog.general_base_url

let glm_coding_base_url =
  env_url_or ~env:"ZAI_CODING_BASE_URL"
    ~default:Llm_provider.Zai_catalog.coding_base_url

let kimi_compat_base_url =
  env_url_or ~env:"KIMI_BASE_URL" ~default:"https://api.moonshot.ai/v1"

let kimi_coding_base_url =
  env_url_or ~env:"KIMI_CODING_BASE_URL"
...
let gemini_cli : t = {
  ...
  capabilities = {
    supports_runtime_mcp_http_headers = false;
    supports_per_call_mcp_config = false;  (* gemini-cli #4674 unimplemented *)
    emits_usage_telemetry = false;
  };
  discovery = No_discovery;
}
...
let claude_api : t = {
  ...
  auth = X_api_key {
    ...
    version_header = Some anthropic_version_header;
  };
