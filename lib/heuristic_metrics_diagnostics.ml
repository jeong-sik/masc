...
let make_tuple_key ~raw_value ~threshold ~triggered = { raw_value; threshold; triggered }
...
let merge_accum acc p =
  ...
  if p.key.triggered
  then acc.t_count <- acc.t_count + 1
...
let analyze (records : Yojson.Safe.t list) : report =
  ...
  in
  let stats =
    List.sort (fun (a : site_stat) (b : site_stat) -> String.compare a.site b.site) stats
  in
  let degenerate_sites =
    List.filter_map
      (fun (s : site_stat) ->
         if s.count >= degenerate_min_records && s.unique_tuples <= 1
         then Some s.site
         else None)
      stats
  in
  let one_sided_sites =
    List.filter_map
      (fun (s : site_stat) ->
         if s.count < degenerate_min_records
         then None
         else if s.triggered_true_count = s.count || s.triggered_false_count = s.count
         then Some s.site
         else None)
      stats
  in
  { total_records = !total; sites = stats; degenerate_sites; one_sided_sites }
;;
