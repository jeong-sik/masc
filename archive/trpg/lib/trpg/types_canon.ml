(** Types_canon — canon checks, stagnation detection, combat semantics,
  ...
  | Some (contract_id, strict) -> (
      ...
      | Some contract ->
          ...
          let event_types_seen =
            events
            |> List.map (fun (event : Engine_event.t) ->
...
let is_meaningful_event_type = function
  | Engine_event.Flag_set
  | Engine_event.Combat_attack
  | Engine_event.Combat_defense
  | Engine_event.Scene_transition
  | Engine_event.Quest_update
  | Engine_event.Hp_changed
  | Engine_event.Inventory_changed ->
      true
  | _ -> false

let detect_stagnation ~(events : Engine_event.t list) ~threshold =
  let turn_events =
    events
    |> List.filter (fun (ev : Engine_event.t) ->
           ev.event_type = Engine_event.Turn_started)
    |> List.length
  in
  if turn_events < threshold then false
    ...
    let recent_events =
      ...
      let rec take_until_n_turns n acc = function
        | [] -> acc
        | (ev : Engine_event.t) :: rest ->
            if n <= 0 then acc
            else
              let n' =
                if ev.event_type = Engine_event.Turn_started then n - 1
                else n
              in
    ...
    in
    not (List.exists (fun (ev : Engine_event.t) ->
...
let latest_stagnation_pressure_level ~(events : Engine_event.t list) : int =
  ...
  let latest_pressure_seq, latest_pressure_level =
    events
    |> List.fold_left
         (fun (best_seq, best_level) (ev : Engine_event.t) ->
           if ev.event_type <> Engine_event.World_event then
             (best_seq, best_level)
           else
             let event_type =
...
let has_event_type (events : Engine_event.t list) event_type =
  List.exists
    (fun (ev : Engine_event.t) -> ev.event_type = event_type)
    events
...
let events_since_last_session_marker (events : Engine_event.t list) :
    Engine_event.t list =
...
let latest_session_outcome_payload (events : Engine_event.t list) :
    Yojson.Safe.t option =
  events
  |> List.fold_left
       (fun acc (ev : Engine_event.t) ->
         if ev.event_type = Engine_event.Session_outcome then
