...
- **Created**: 2026-05-06
- **Synthesizes**: RFC 0021 (Anchored Thread Rail), RFC 0023 (Runtime Overlay Layer), RFC 0024 (BDI Inspector Slot), RFC 0026 (Audit Replay Timeline)
...
`replay-overlay.ts` (RFC-0026) 가 timeline scrub 시 4 source 를 동일 ReplayWindow 로 필터한다. 그러나 4 source 는 **3개 별도 surface** (anchored-thread-rail, runtime-overlay-layer, inspector-keeper-bdi) 에 독립 표시되어 user 가 4 surface 를 동시 scan 해야 cross-source 상관 (causal chain) 을 발견한다.
...
4 surface scan = cognitive load 높음. **`keeper-trace` LAYERS toggle** = single chip click 으로 4 source stitched (line gutter chip click → tooltip 안 runtime hop chain + 그 시점 thread + BDI summary 한 번에 표시).
...
export type TraceSource = 'runtime' | 'thread' | 'bdi' | 'decision'

export type TraceEventRaw =
  | { readonly kind: 'runtime'; readonly hit: RuntimeHit }                // RFC 0023
...
**없음**. 4 기존 endpoint (`/api/dashboard/runtime-overlay`, `/api/dashboard/audit-replay`, `/api/dashboard/keeper-bdi/<id>`, anchored thread store) 를 client-side join. RFC 0026 §3.2 의 `/api/dashboard/audit-replay` 가 이미 `threads` + `decisions` + `runtime_hits` 한 response 로 chunked 반환 — `bdi` 만 per-keeper fetch 추가.
...
```ts
{
  kind: 'keeper-trace',
  label: 'Keeper trace',
  description: 'stitched runtime + thread + BDI per line, replay-window aware',
...
```
   42  | ◆◆· bar()      (3 events: 1 runtime + 1 thread + 1 bdi)
   43  | ◆ ·  baz()      (1 event: 1 runtime)
   44  | ◆◆◆◆ qux()     (4+ events: stack cap 3 + overflow indicator)
```

- **chip color**: 각 source 별 token. runtime `--color-keeper-N-glow`, thread `--color-status-{kind-color-map}` (RFC 0021 §5), bdi `--color-fg-secondary`, decision `--color-status-info`.
...
cost band (RFC 0023 §5) 는 runtime chip 에 한정; thread/bdi/decision chip 에는 적용 안 함.
...
1. **Composite event 의 author**: 같은 line 에 runtime hit (selected provider) + thread (author keeper) + bdi (keeper state) 가 동시 있으면 chip 의 author hue 가 누구? **v1 = 가장 빈도 높은 keeper_id** (event count 기준). 동률 시 thread > bdi > runtime priority. v1.1 측정 후 조정.
