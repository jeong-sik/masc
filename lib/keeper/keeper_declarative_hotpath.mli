...
(** Reconstruct a "prefix:model_id" string from a resolved provider config.
    ...
    matches the format stored in {!Keeper_config_loader.weighted_entry.model}. *)

(** {1 Profile conversion} *)

val adapted_profile_to_profile :
  Keeper_declarative_adapter.adapted_profile ->
  profile option
...
val adapted_catalog_to_snapshot :
  source_path:string ->
  Keeper_declarative_adapter.adapted_catalog ->
...
val try_load_declarative :
  string ->
  (decl_snapshot,
    Keeper_declarative_adapter.adapter_error list)
  result option
