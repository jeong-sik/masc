(* Tool-use / tool-result block text renderers for the keeper context.
   ...
   ToolResult message must be flattened into plain text for
   verifier-facing serializations.

   Cap-aware: ToolResult JSON over [max_chars] collapses to a stub so
   one verifier-facing projection cannot inflate one Text block to multi-MB
