...
### Fixed
- OAS recovery no longer holds one global Keeper mutex across slot I/O, fsync, execution-store construction, or settled-scope cleanup. The process-wide registry now uses a short cross-domain-safe `Stdlib.Mutex` section only for exact recovery-key claim/release, while each prepared execution serializes only its own settlement with `Eio.Mutex`; unrelated Keeper lanes remain independent.
