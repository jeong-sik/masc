...
let test_parse_chain_design_accepts_explicit_mermaid_nodes () =
  let response =
    {|```mermaid
graph LR
  llm_step_1["LLM:gemini \"Generate a positive phrase about chain orchestration\""]
  llm_step_2["LLM:gemini \"Build a confirmation sentence using {{llm_step_1}}\""]
  llm_step_1 --> llm_step_2
```|}
  in
  match Chain_orchestrator_eio.parse_chain_design response with
...
let test_parse_chain_design_preserves_escaped_mermaid_prompts () =
  let response =
    {|```mermaid
graph LR
  draft_step["LLM:gemini \"Draft a one-sentence confirmation that the preview-run smoke test was successful.\""]
  finalize_step["LLM:gemini \"Review {{draft_step}} and finalize it into a single, clear confirmation sentence.\""]
...
```|}
  ...
  | Ok chain ->
      let find_llm_prompt node_id =
        match List.find_opt (fun (node : Chain_types.node) -> node.id = node_id) chain.Chain_types.nodes with
        | Some { node_type = Llm { prompt; _ }; _ } -> prompt
        | Some _ -> failf "node %s was not parsed as llm" node_id
...
let test_build_design_context_uses_parser_compatible_mermaid_examples () =
  let prompt =
    Chain_composer.build_design_context
      ...
      ~tasks:[]
  in
  check bool "llm example includes model-qualified syntax" true
    (contains ~needle:"LLM:gemini \\\"Analyze the goal\\\"" prompt);
