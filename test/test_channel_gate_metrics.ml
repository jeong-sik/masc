...
let test_record_attempt_tracks_connector_diagnostics () =
  with_eio (fun () ->
    ...
    let stats =
      Metrics.snapshot ()
      |> List.find (fun (row : Metrics.channel_stats) -> String.equal row.channel channel)
    in
...
let test_record_validation_error_metric_falls_back_for_invalid_json () =
  with_eio (fun () ->
    let before_messages, before_validation =
      ...
      | None -> 0, 0
    in
    Gate_routes.record_validation_error_metric ~duration_ms:0 "{invalid" "invalid json";
    let stats =
      Metrics.snapshot ()
      |> List.find (fun (row : Metrics.channel_stats) ->
        String.equal row.channel "unknown")
    in
    check int "message_count incremented" (before_messages + 1) stats.message_count;
    check
      int
...
let test_snapshot_json_reports_health_and_latency () =
  with_eio (fun () ->
    ...
    check
      int
      "avg_duration_ms uses timed attempts"
      11_000
      (row |> U.member "avg_duration_ms" |> U.to_int);
    check int "max_duration_ms" 11_500 (row |> U.member "max_duration_ms" |> U.to_int);
    check int "slow_count" 2 (row |> U.member "slow_count" |> U.to_int);
    check int "slow_rate_pct" 100 (row |> U.member "slow_rate_pct" |> U.to_int);
    check
      int
      "success_rate_pct ignores duplicates"
      50
      (row |> U.member "success_rate_pct" |> U.to_int);
    check string "health" "failing" (row |> U.member "health" |> U.to_string);
    check
      string
      "last_error"
      "upstream timeout"
...
let test_snapshot_json_includes_room_bindings () =
  with_eio (fun () ->
    ...
    in
    check string "alpha keeper" "luna" (alpha |> U.member "keeper" |> U.to_string);
    check string "alpha health" "healthy" (alpha |> U.member "health" |> U.to_string);
    check
      string
      "beta last error"
      "keeper offline"
      (beta |> U.member "last_error" |> U.to_string);
    check string "beta health" "failing" (beta |> U.member "health" |> U.to_string))
;;
...
let test_events_json_filters_newest_first () =
  with_eio (fun () ->
    ...
    check string "newest room" "room-a" (newest |> U.member "room_id" |> U.to_string);
    check
      string
      "newest outcome"
      "keeper_error"
      (newest |> U.member "outcome" |> U.to_string);
    check string "older outcome" "success" (older |> U.member "outcome" |> U.to_string);
    check
      bool
      "latest seq present"
      true
      (json |> U.member "latest_seq" |> U.to_int >= (newest |> U.member "seq" |> U.to_int)))
;;
