...
function sparklineSvg(values: number[], color: string, w = 80, h = 20): string {
  ...
  return `<svg width="${w}" height="${h}" role="img" aria-label="추이 그래프" class="inline-block align-middle" viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg"><polyline fill="none" stroke="${color}" stroke-width="1.5" stroke-linejoin="round" points="${points}"/></svg>`
}
