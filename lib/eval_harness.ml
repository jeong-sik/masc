...
let apply_deterministic_grader (g : deterministic_grader) (value : string) : grader_result
  ...
  let matched =
    ...
    | Contains ->
      ...
      let rec find_at i =
        if i + String.length e_lower > String.length v_lower
        then false
        else if String.sub v_lower i (String.length e_lower) = e_lower
        then true
        else find_at (i + 1)
      in
      find_at 0
...
let check_tool_expectations
  ...
  List.map
    (fun (exp : tool_expectation) ->
       ...
       let detail =
         if not required_ok
         then Printf.sprintf "required tool '%s' was not called" exp.tool_name
         else if not max_ok
         then
           Printf.sprintf
             "tool '%s' called %d times (max: %d)"
             exp.tool_name
             call_count
             (Option.value ~default:0 exp.max_calls)
         else Printf.sprintf "tool '%s' called %d times — OK" exp.tool_name call_count
       in
       { grader_desc = Printf.sprintf "tool_expectation: %s" exp.tool_name
       ; score = (if passed then 1.0 else 0.0)
       ; weight = 0.5
       ; (* tool expectations are secondary to content graders *)
         passed
       ; detail
       })
    expectations
...
let weighted_score (results : grader_result list) : float =
  ...
  then 0.0
    ...
    let weighted_sum =
      List.fold_left (fun acc r -> acc +. (r.score *. r.weight)) 0.0 results
    in
...
let score_std_dev (scores : float list) : float =
  ...
  | _ ->
    ...
    let variance =
      List.fold_left (fun acc s -> acc +. ((s -. mean) ** 2.0)) 0.0 scores /. n
    in
    sqrt variance
;;
...
let summarize_runs ~(scenario : scenario) ~(k : int) (runs : eval_run list) : eval_result =
  ...
  let mean = if n = 0 then 0.0 else List.fold_left ( +. ) 0.0 scores /. float_of_int n in
  let total_cost =
    List.fold_left (fun a (r : eval_run) -> a +. r.total_cost_usd) 0.0 runs
  in
  { scenario
  ; runs
...
let scenario_of_json (json : Yojson.Safe.t) : (scenario, string) result =
      ...
      | `List items ->
        List.filter_map
          (fun g ->
             ...
             | "regex" ->
               Some (parse_deterministic g (Regex (g |> member "pattern" |> to_string)))
             | "model" ->
               Some
                 (ModelBased
          ...
          (fun te ->
             try
               Some
                 ...
                 ; required =
                     (match te |> member "required" with
                      | `Bool b -> b
                      | _ -> false)
                 ; max_calls =
                     (match te |> member "max_calls" with
                      | `Int i -> Some i
                      | _ -> None)
                 ; args_contain =
                     (match te |> member "args_contain" with
                      | `String s -> Some s
                      | _ -> None)
                 }
             with
  ...
  | exn -> Error (Printf.sprintf "Failed to parse scenario: %s" (Printexc.to_string exn))
;;
...
let load_scenarios_from_file (path : string) : (scenario list, string) result =
  ...
  then Error (Printf.sprintf "Scenario file not found: %s" path)
  else (
    try
      ...
      | `List items ->
        ...
        in
        if errors <> []
        then Error (Printf.sprintf "Parse errors: %s" (String.concat "; " errors))
        else
          Ok
            (List.filter_map
               (function
...
let report_to_string (r : eval_suite_result) : string =
  ...
  List.iter
    (fun (er : eval_result) ->
       ...
       add
         (Printf.sprintf
            "  pass@k=%.2f  mean=%.2f  consistency=%.3f  cost=$%.4f\n"
            er.pass_at_k
            er.mean_score
            er.consistency
            er.total_cost_usd);
       List.iter
         (fun (run : eval_run) ->
            ...
            add
              (Printf.sprintf
                 "  run[%d] %s score=%.2f turns=%d cost=$%.4f %s\n"
                 run.run_index
                 run_status
                 run.weighted_score
                 run.total_turns
