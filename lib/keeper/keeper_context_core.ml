...
let ensure_dir path = ignore (Keeper_fs.ensure_dir path)
...
let estimate_char_tokens (s : string) : int = Oas.Context_reducer.estimate_char_tokens s
...
let max_tokens_of_context (ctx : working_context) = ctx.max_tokens

let with_max_tokens (ctx : working_context) max_tokens =
  let checkpoint = { ctx.checkpoint with max_total_tokens = Some max_tokens } in
...
let messages_of_context (ctx : working_context) = ctx.checkpoint.messages

let empty_runtime_checkpoint
      ~system_prompt
      ~messages
      ~max_tokens
...
let message_count (ctx : working_context) = List.length (messages_of_context ctx)
  ...
  let messages =
    List.map
      (fun (m : Oas.Types.message) ->
         if m.role = Oas.Types.System then { m with role = Oas.Types.Assistant } else m)
      (messages_of_context ctx)
  in
  let checkpoint = { ctx.checkpoint with system_prompt = Some system_prompt; messages } in
  { ctx with checkpoint }
;;

let append ctx (msg : Oas.Types.message) =
  let checkpoint = { ctx.checkpoint with messages = messages_of_context ctx @ [ msg ] } in
  { ctx with checkpoint }
;;

let append_many ctx msgs = List.fold_left append ctx msgs
...
let sync_oas_context (ctx : working_context) : working_context =
  ...
  let context_ratio =
    let max_tokens = max_tokens_of_context ctx in
    if max_tokens = 0 then 0.0 else float_of_int token_count /. float_of_int max_tokens
  in
  Oas.Context.set_scoped context Oas.Context.Session "message_count" (`Int message_count);
  Oas.Context.set_scoped context Oas.Context.Session "token_count" (`Int token_count);
  Oas.Context.set_scoped
    context
    Oas.Context.Session
    "context_ratio"
    (`Float context_ratio);
...
let role_to_string (r : Oas.Types.role) =
  match r with
  | System -> "system"
  | User -> "user"
  | Assistant -> "assistant"
...
let role_of_string s =
  ...
  | Some role -> role
  | None ->
    Log.Misc.warn "keeper_context_core: unknown role %S, defaulting to Tool (#8623)" s;
    Oas.Types.Tool
;;

let content_blocks_to_json (blocks : Oas.Types.content_block list) : Yojson.Safe.t =
  `List (List.map Oas.Api.content_block_to_json blocks)
;;

let content_blocks_of_json (json : Yojson.Safe.t) : Oas.Types.content_block list option =
  let open Yojson.Safe.Util in
  match json |> member "content_blocks" with
  | `List blocks ->
    let parsed = List.filter_map Oas.Api.content_block_of_json blocks in
...
let trim_messages_preserving_pairs (messages : Oas.Types.message list) ~(max_count : int)
...
let tool_result_text_of_block
  ...
  then content
    ...
    | Some value ->
      ...
      if len <= default_max_checkpoint_tool_result_chars
      then serialized
...
let repair_dangling_tool_use_messages (messages : Oas.Types.message list)
  : Oas.Types.message list
  =
  let repair_with_next (current : Oas.Types.message) (next_opt : Oas.Types.message option)
    =
    ...
    let has_dangling =
      List.exists
        (function
          | Oas.Types.ToolUse { id; _ } -> not (List.mem id next_tool_result_ids)
          | _ -> false)
        current.content
    in
    if not has_dangling
        ...
        List.map
          (function
            | Oas.Types.ToolUse { id; name; input }
              ...
              Oas.Types.Text
                (tool_use_text_of_block ~tool_use_id:id ~tool_name:name ~input)
            | other -> other)
          current.content
      in
  ...
  let rec loop acc = function
    | [] -> List.rev acc
    | [ current ] -> List.rev (repair_with_next current None :: acc)
...
let repair_orphan_tool_result_messages (messages : Oas.Types.message list)
  : Oas.Types.message list
  =
  let rec loop prev acc = function
    ...
    | msg :: rest ->
      let repaired =
        if not (has_tool_result_block msg)
        then msg
        else (
              ...
              List.map
                (function
                  | Oas.Types.ToolResult { tool_use_id; content; json; _ } ->
                    Oas.Types.Text (tool_result_text_of_block ~tool_use_id ~content ~json)
                  | other -> other)
                msg.content
            in
            { msg with content }))
...
let repair_broken_tool_call_pairs (messages : Oas.Types.message list)
  : Oas.Types.message list
  =
  messages |> repair_dangling_tool_use_messages |> repair_orphan_tool_result_messages
;;
...
let serialize_context (ctx : working_context) : string =
  let json =
    `Assoc
      [ ( "system_prompt"
        , `String (Inference_utils.sanitize_text_utf8 (system_prompt_of_context ctx)) )
...
let deserialize_context (s : string) ~max_tokens : working_context =
  ...
  let checkpoint =
    empty_runtime_checkpoint ~system_prompt ~messages ~max_tokens ~context
  in
  sync_oas_context { checkpoint; max_tokens }
;;
...
let restore_checkpoint ckpt ~max_tokens = deserialize_context ckpt.serialized ~max_tokens
...
let classify_history_entry ~(source : string) ~(content : string) : history_line_action =
  if Keeper_types.is_prompt_history_source source || has_world_state_signature content
  then Drop_line
...
let migrate_session_history_logs ~(session_dir : string) : history_migration_stats =
  ...
  if (not (Fs_compat.file_exists main_path)) && not (Fs_compat.file_exists internal_path)
  then empty_history_migration_stats
    ...
    let kept_rev, moved_rev, dropped_main, malformed_main =
      List.fold_left
        (fun (kept_rev, moved_rev, dropped_lines, malformed_lines) line ->
           ...
           | Some Keep_main -> line :: kept_rev, moved_rev, dropped_lines, malformed_lines
           | Some Move_internal ->
             kept_rev, line :: moved_rev, dropped_lines, malformed_lines
           | Some Drop_line -> kept_rev, moved_rev, dropped_lines + 1, malformed_lines
           | None -> line :: kept_rev, moved_rev, dropped_lines, malformed_lines + 1)
        ([], [], 0, 0)
        main_lines
    in
    ...
    then
      ...
      let merged_internal = dedupe_preserve_order (sanitized_internal @ moved_lines) in
      (match Fs_compat.save_file_atomic main_path (render_jsonl_lines kept_lines) with
       ...
       | Error detail ->
         Log.Keeper.error
           ...
           detail);
      (match
         Fs_compat.save_file_atomic internal_path (render_jsonl_lines merged_internal)
       with
       | Ok () -> ()
       | Error detail ->
         Log.Keeper.error
...
let history_path_for_source ~(session_dir : string) ~(source : string option) : string =
  match source with
...
let add_checkpoint_sanitize_stats
      ...
      (b : checkpoint_sanitize_stats)
  : checkpoint_sanitize_stats
  =
...
let truncate_checkpoint_text ~max_chars (text : string) : string * int =
  ...
  then "", len
    ...
    then String.sub checkpoint_text_cap_marker 0 max_chars, len
    else (
      let kept = max_chars - marker_len in
      String.sub text 0 kept ^ checkpoint_text_cap_marker, len - kept))
;;

let find_substring_from ~(haystack : string) ~(needle : string) ~(start : int)
  : int option
  =
...
let sanitize_checkpoint_text_block (text : string)
  ...
  =
  if is_ephemeral_system_context_text text
  then
    ( None
  ...
  else if has_world_state_signature text
  then (
    ...
    else if String.equal stripped text
    then Some text, empty_checkpoint_sanitize_stats
    else
      ( Some stripped
      , { empty_checkpoint_sanitize_stats with
          truncated_blocks = 1
...
let sanitize_checkpoint_message (msg : Oas.Types.message)
           ...
           (match sanitized_text with
            ...
            | Some text ->
              ...
              then
                ...
                let remaining =
                  default_max_checkpoint_text_chars_per_message - kept_text_chars
                in
                if remaining <= 0
                then
                  ( kept_rev
         ...
         | Oas.Types.ToolResult { tool_use_id; content; is_error; _ } ->
           ...
           then (
             ...
             , add_checkpoint_sanitize_stats
                 ...
                 { empty_checkpoint_sanitize_stats with
                   truncated_blocks = 1
                 ; truncated_chars = tool_chars - default_max_checkpoint_tool_result_chars
                 } ))
           else
...
let sanitize_checkpoint_messages (messages : Oas.Types.message list)
...
let sanitize_oas_checkpoint ?(repair_orphans = true) (cp : Oas.Checkpoint.t)
  : Oas.Checkpoint.t * checkpoint_sanitize_stats
  =
  let messages, stats = sanitize_checkpoint_messages cp.messages in
  let messages =
    if repair_orphans then repair_broken_tool_call_pairs messages else messages
  in
...
let context_of_oas_checkpoint
  ...
  let max_tokens = checkpoint_max_tokens cp ~fallback:primary_model_max_tokens in
  let messages =
    let messages =
      trim_messages_preserving_pairs cp.messages ~max_count:max_checkpoint_messages
    in
    if repair_orphans then repair_broken_tool_call_pairs messages else messages
  in
  let context = Oas.Context.copy cp.context in
  let checkpoint = { cp with system_prompt = Some system_prompt; messages; context } in
  sync_oas_context { checkpoint; max_tokens }
;;

let context_of_legacy_checkpoint (ckpt : checkpoint) ~(primary_model_max_tokens : int)
  : working_context
  =
...
let save_oas_checkpoint
  ...
  let capped_messages =
    trim_messages_preserving_pairs original_messages ~max_count:max_checkpoint_messages
  in
...
let checkpoint_generation (cp : Oas.Checkpoint.t) ~(fallback : int) : int =
  let open Yojson.Safe.Util in
  match
    Oas.Context.get_scoped cp.context Oas.Context.Session checkpoint_generation_key
  with
  ...
  | _ ->
    (match cp.working_context with
     | Some (`Assoc _ as sidecar) ->
       sidecar |> member "generation" |> to_int_option |> Option.value ~default:fallback
     | _ -> fallback)
;;
...
let load_context_from_checkpoint
  ...
  let oas_result =
    Keeper_checkpoint_store.load_oas ~session_dir:session.session_dir ~session_id:trace_id
  in
  ...
  let legacy_checkpoint =
    try load_latest_checkpoint session with
    | ex ->
      Log.Keeper.error
  ...
  let oas_checkpoint =
    ...
    |> Option.map (fun checkpoint ->
      ...
      if checkpoint_sanitize_changed stats
      then (
        let has_data_loss =
          stats.dropped_blocks > 0
          || stats.dropped_messages > 0
          || stats.dropped_chars > 0
        in
        (if has_data_loss then Log.Keeper.warn else Log.Keeper.debug)
          "keeper:%s checkpoint migration sanitized messages: dropped_blocks=%d \
           dropped_messages=%d dropped_chars=%d truncated_blocks=%d truncated_chars=%d"
          trace_id
        ...
        match
          Keeper_checkpoint_store.save_oas ~session_dir:session.session_dir sanitized
        with
        | Ok () -> ()
        | Error detail ->
          Log.Keeper.error
  ...
  | _, _, Some ckpt ->
    (try
       let ctx = context_of_legacy_checkpoint ckpt ~primary_model_max_tokens in
       session, Some ctx
     with
     | ex ->
       Log.Keeper.error
