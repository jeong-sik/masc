...
let test_align_keeper_runtime_status_promotes_fresh_runtime_signal () =
  ...
  in
  Alcotest.(check string) "fresh runtime signal promotes keeper status" "busy" status
;;
...
let test_align_keeper_runtime_status_ignores_zombie_runtime_signal () =
  let status =
    Operator_control_snapshot.align_keeper_runtime_status
      ...
      ~keepalive_running:true
  in
  Alcotest.(check string) "zombie runtime does not override inactive" "inactive" status
;;
...
let test_align_keeper_runtime_status_tolerates_null_status_json () =
  let status =
    Operator_control_snapshot.align_keeper_runtime_status
      ~surface_status:"inactive"
      ~diagnostic:`Null
      ~agent_status_json:`Null
      ~keepalive_running:true
  in
  Alcotest.(check string) "null runtime status keeps surface status" "inactive" status
;;

let test_max_turns_override_source_accepts_raised_ceiling () =
  Alcotest.(check string)
    "100 is a valid profile override"
    "override"
  ...
  Alcotest.(check string)
    "missing override comes from env"
    "env"
...
let test_compute_context_ratio_uses_resolved_cli_context_budget () =
  ...
  Alcotest.(check (float 0.0001))
    "codex bare provider uses 1.05M context"
    (2106223.0 /. 1050000.0)
    ratio
;;
...
let test_snapshot_prefers_metrics_context_truth_over_usage_counters () =
  ...
  Fun.protect
    ...
    (fun () ->
         ...
         |> List.find_map (fun line ->
           try
             ...
             | Some "keeper_context_status" ->
               Option.bind (Safe_ops.json_float_opt "context_ratio" json) (fun ratio ->
                 Option.bind (Safe_ops.json_int_opt "context_tokens" json) (fun tokens ->
                   Option.map
       ...
       in
       let snapshot_max = Yojson.Safe.Util.(keeper |> member "context_max" |> to_int) in
       Alcotest.(check (float 0.000001))
         "latest metrics ratio retained"
         metrics_ratio
         snapshot_ratio;
       Alcotest.(check int)
         "latest metrics tokens retained"
         metrics_tokens
         snapshot_tokens;
       Alcotest.(check int) "latest metrics max retained" metrics_max snapshot_max;
       Alcotest.(check string)
         "metrics source retained"
         "keeper_context_status"
         Yojson.Safe.Util.(keeper |> member "context_source" |> to_string);
       Alcotest.(check bool)
         "metrics ratio differs from usage fallback"
         true
         (Float.abs (snapshot_ratio -. usage_ratio) > 0.000001);
       Alcotest.(check bool)
         "metrics tokens differ from usage fallback"
         true
         (snapshot_tokens <> updated_meta.runtime.usage.last_input_tokens);
       Alcotest.(check bool)
         "nested context payload omitted"
         true
...
let test_snapshot_has_expected_sections () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       ignore (Coord.join config ~agent_name:"owner" ~capabilities:[] ());
       ignore
         (Coord.add_task config ~title:"operator backlog" ~priority:2 ~description:"");
       ignore
         (Coord.broadcast config ~from_agent:"owner" ~content:"operator snapshot seed");
       let json = Operator_control.snapshot_json (operator_ctx env sw config "owner") in
       let root = Yojson.Safe.Util.member "root" json in
       Alcotest.(check bool) "root block present" true (root <> `Null);
       Alcotest.(check bool)
         "root initialized"
         true
       ...
       Alcotest.(check string)
         ...
         "operator_remote_v1"
         (json
          |> Yojson.Safe.Util.member "server_profile"
          |> Yojson.Safe.Util.member "name"
          |> Yojson.Safe.Util.to_string);
       Alcotest.(check bool)
         "attention summary present"
         true
       ...
       Alcotest.(check bool)
         "operator judge runtime present"
         true
         (Yojson.Safe.Util.member "operator_judge_runtime" json <> `Null);
       Alcotest.(check bool)
         "operator judge enabled by default"
         true
         Yojson.Safe.Util.(
...
let test_snapshot_pending_confirm_summary_tracks_actor_scope () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Alcotest.(check bool)
         "keeper github identity login prepare listed"
         true
         (List.exists
            (fun row ->
               Yojson.Safe.Util.(row |> member "action_type" |> to_string)
            ...
            confirm_required_actions);
       Alcotest.(check bool)
         "team stop removed from confirm surface"
         false
         (List.exists
            (fun row ->
...
let test_snapshot_summary_view_excludes_retired_command_plane () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       Alcotest.(check bool)
         "command_plane field absent"
         true
         (not (List.mem_assoc "command_plane" Yojson.Safe.Util.(to_assoc json)));
       Alcotest.(check bool)
         "swarm_status omitted"
         true
         (Yojson.Safe.Util.member "swarm_status" json = `Null);
       Alcotest.(check bool)
         "attention summary still present"
         true
         (Yojson.Safe.Util.member "attention_summary" json <> `Null);
       Alcotest.(check bool)
         "recommendation summary still present"
         true
         (Yojson.Safe.Util.member "recommendation_summary" json <> `Null))
;;
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       List.iter
         (fun keeper ->
            Alcotest.(check int)
              "lightweight recent_activity omitted"
              0
              Yojson.Safe.Util.(
                keeper |> member "recent_activity" |> to_list |> List.length))
         keepers;
       Alcotest.(check int)
         "lightweight recent_messages omitted"
         0
         Yojson.Safe.Util.(json |> member "recent_messages" |> to_list |> List.length);
       Alcotest.(check int)
         "lightweight recent_actions omitted"
         0
...
let test_snapshot_lightweight_summary_keeps_tool_audit () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let first_audit =
         Operator_control_snapshot.cached_tool_audit_json ~lightweight:true config meta
       in
       Alcotest.(check bool)
         "lightweight audit returns fallback immediately"
         true
    ...
    (fun () ->
       ...
       let rec wait_for_recent_tools attempts =
         let audit =
           Operator_control_snapshot.cached_tool_audit_json ~lightweight:true config meta
         in
         let recent = Yojson.Safe.Util.(audit |> member "recent_tool_names" |> to_list) in
         if recent <> []
         then audit
         else if attempts > 0
         then (
       ...
       in
       Alcotest.(check (list string))
         "recent tool names retain recent window"
         [ "masc_status"; "masc_tasks" ]
         Yojson.Safe.Util.(
           keeper |> member "recent_tool_names" |> to_list |> List.map to_string);
       Alcotest.(check (list string))
         "latest tool names stay latest-only"
         []
         Yojson.Safe.Util.(
           keeper |> member "latest_tool_names" |> to_list |> List.map to_string))
;;
...
let test_snapshot_waiters_share_inflight_result () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let cache_key =
         Eio.Mutex.use_rw ~protect:true Operator_control_snapshot._snapshot_mu (fun () ->
           match
             Hashtbl.to_seq_keys Operator_control_snapshot._snapshot_table |> List.of_seq
           with
           | key :: _ -> key
       ...
       Eio.Mutex.use_rw ~protect:true Operator_control_snapshot._snapshot_mu (fun () ->
         Hashtbl.replace
           Operator_control_snapshot._snapshot_table
           cache_key
       ...
       Eio.Time.sleep (Eio.Stdenv.clock env) 0.05;
       let shared =
         `Assoc [ "trace_id", `String "shared-trace"; "status", `String "ok" ]
       in
       Eio.Mutex.use_rw ~protect:true Operator_control_snapshot._snapshot_mu (fun () ->
         Hashtbl.replace
           Operator_control_snapshot._snapshot_table
       ...
       Alcotest.(check string)
         "waiter a shared trace"
         "shared-trace"
         Yojson.Safe.Util.(first |> member "trace_id" |> to_string);
       Alcotest.(check string)
         "waiter b shared trace"
         "shared-trace"
         Yojson.Safe.Util.(second |> member "trace_id" |> to_string);
       let cached_retained =
         Eio.Mutex.use_rw ~protect:true Operator_control_snapshot._snapshot_mu (fun () ->
           match Hashtbl.find_opt Operator_control_snapshot._snapshot_table cache_key with
...
let test_digest_room_exposes_pending_confirm_attention () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       Alcotest.(check bool)
         "pending confirm attention present"
         true
         (List.exists
            (fun item ->
       ...
       Alcotest.(check bool)
         "no command attention in fresh room"
         true
         (not
            (List.exists
               (fun item ->
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let tool_host_attention =
         List.find_opt
           (fun item ->
              Yojson.Safe.Util.(item |> member "kind" |> to_string) = "tool_host_timeout"
              && Yojson.Safe.Util.(
                   item
                   |> member "evidence"
                   |> member "failure_envelope"
                   |> member "evidence_ref"
                   |> member "request_id"
                   |> to_string)
                 = "opsd-toolhost-1")
       ...
       Alcotest.(check string)
         "tool host operator action"
         "masc_operator_digest"
         Yojson.Safe.Util.(
           item
           |> member "evidence"
           |> member "failure_envelope"
           |> member "operator_action"
           |> to_string))
;;
...
let test_operator_digest_severity_rank_supports_critical () =
  ...
  Alcotest.(check bool)
    "critical outranks bad"
    true
    (Operator_digest.severity_rank Sev_critical > Operator_digest.severity_rank Sev_bad)
;;
