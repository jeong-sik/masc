     ...
     | Some _ | None ->
       ...
       (match Workspace_goal_index.build_goal_task_index_for_config_result config tasks with
        ...
        | Ok index ->
          ...
          then
            Error
              "goal completion requires at least one linked task; provide \
               override_note to force"
          else
            let task_progress =
            ...
            match
              Convergence.check_convergence
                ~goal_id:goal.id
                ~tasks:task_progress
                (* Completion readiness is a point-in-time gate; it does not own
                   the cross-turn stagnation counter, so it supplies the neutral
            ...
            | Some (Convergence.StagnationDetected _) | None ->
              ...
              then
                Error
                  (Printf.sprintf
                     "goal completion blocked: %d linked task(s) are still open; \
                      provide override_note to force"
                     open_count)
              else if done_count = 0
              then
                Error
                  "goal completion blocked: linked tasks are terminal but none are \
                   done; provide override_note to force"
              else
                Error
                  "goal completion blocked: linked tasks have not converged; \
                   provide override_note to force"))
;;
