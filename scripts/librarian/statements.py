...
Markup: nothing is dropped. The calibration scorer dropped backticks and
"**"; a memory about code can hold either as syntax (Codex review on
#37369), and re-measuring the question's ends on the same 12 groups (47
sources x 3 states) with nothing dropped gave the same numbers -- floor
0.94, self 0.95, ceiling 0.09 -- on 2026-09-21, so the calibration holds
...
def statements(text):
    """Line breaks, then sentence ends; nothing dropped; a piece under
...
GOLDEN_INPUTS = [
    ...
    # Backticks and ** stay: syntax in a code memory is part of what it says.
