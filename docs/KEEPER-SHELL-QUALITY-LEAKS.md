...
| Multi-repo cwd required | 286 | `lib/keeper/keeper_sandbox_docker.ml`, `lib/keeper/keeper_tool_alias.ml` | Return a typed public `Execute { executable, argv, cwd }` retry shape when sandbox-root repo scope cannot be resolved. Do not infer repository scope from `cd ... &&` command text. |
