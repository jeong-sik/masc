...
let sticky_table : (string * string, sticky_entry) Hashtbl.t = Hashtbl.create 32
...
let record_sticky_choice ~keeper ~cascade ~provider ~ttl_ms ~now =
  ...
  then ()
    ...
    Eio.Mutex.use_rw ~protect:false sticky_mutex (fun () ->
      Hashtbl.replace sticky_table (keeper, cascade) { provider; expires_at }))
;;
...
let clear_sticky () =
  Eio.Mutex.use_rw ~protect:false sticky_mutex (fun () -> Hashtbl.clear sticky_table)
;;
...
let clear_round_robin () =
  Eio.Mutex.use_rw ~protect:false rr_mutex (fun () -> Hashtbl.clear rr_table)
;;
