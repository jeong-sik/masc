...
(* Every colour something draws a meaning through, state and role alike. The
   role ones matter as much: the tool trail's bright black is the row an
   operator scans to see what a keeper just did. *)
let named_colours =
  [ "status Ok", Masc_tui_theme.Green
  ; "status Warn", Masc_tui_theme.Yellow
  ; "status Bad", Masc_tui_theme.Red
  ; "status Info", Masc_tui_theme.Cyan
  ; "chat User origin", Masc_tui_theme.Cyan
  ; "chat Keeper origin", Masc_tui_theme.Blue
  ; "chat Tool origin", Masc_tui_theme.Bright_black
  ]
;;
