...
- `local64`는 target runtime profile 이름이다. 실제 병렬 용량은 `masc_local_runtime_status`의 `configured_capacity`, `healthy_runtime_count`를 기준으로 읽는다.
...
- `masc_transition(action="claim")`만으로는 planning `current_task`가 안 잡힌다. 이 경로를 쓰면 각 worker는 `masc_set_current_task`를 호출해야 한다.
...
- worker prompt가 `masc_claim_next -> masc_complete_task` 루프를 지키는지
- 수동 claim path를 쓰면 `masc_set_current_task`를 추가로 호출하는지
...
`best_first_v1`는 기존 `CPv2` 표면 위에 붙는 opt-in strategy다.
