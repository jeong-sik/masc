      ...
      if ! call_mcp_tool 1500 "masc_keeper_up" "$(jq -cn --arg name "$KEEPER_NAME" '{name:$name,presence_keepalive:true}')" 30; then
        ...
        if ! wait_for_restarted_heartbeat "$agent_name"; then
          ...
          if [[ "$(printf '%s' "$status_after" | jq -r '.keepalive_running')" == "true" ]] \
            ...
            recovery_trace_id="$(printf '%s' "$status_after" | jq -r '.meta.trace_id')"
            ckpt_dir="${BASE_PATH}/.masc/keepers/${KEEPER_NAME}/${recovery_trace_id}"
            ckpt_file="${ckpt_dir}/${recovery_trace_id}.json"
            if [[ -f "$ckpt_file" ]]; then
              ckpt_msg_count="$(jq '.messages | length' "$ckpt_file" 2>/dev/null || echo 0)"
              ...
              append_phase "checkpoint_truth" "skip" "checkpoint file not found at ${ckpt_file} — cannot verify messages" "$snapshot_file" "$heartbeat_file"
            fi
            RECOVERY_PASS=1
