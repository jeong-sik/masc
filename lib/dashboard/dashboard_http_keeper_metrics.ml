...
let normalize_model_name s =
  ...
  let s =
    ...
    | Some i ->
      ...
      if Provider_adapter.resolve_direct_canonical_name prefix <> None
      then String.sub s (i + 1) (String.length s - i - 1)
      else s
  in
  if String.ends_with ~suffix:":latest" s
  then String.sub s 0 (String.length s - String.length ":latest")
  else s
;;
      ...
      let rec loop i last_good =
        ...
        then last_good
          ...
          then last_good
            ...
            let next = i + dlen in
            if next > max_bytes then last_good else loop next next))
      in
      let cut = loop 0 0 in
...
let truncate_text ~(max_len : int) (s : string) : string =
  ...
  if n <= max_len then s else utf8_safe_prefix_bytes s ~max_bytes:max_len ^ "..."
;;
  ...
  then false
    ...
    let rec match_at i j =
      ...
      then true
        ...
        Char.lowercase_ascii (String.unsafe_get haystack (i + j))
...
let normalize_non_word_re = Re.Pcre.re {|[^0-9a-z가-힣]+|} |> Re.compile
let normalize_collapse_spaces_re = Re.Pcre.re {| +|} |> Re.compile
...
let jaccard_similarity_text (a : string) (b : string) : float =
  ...
  then 0.0
    ...
    let inter =
      Hashtbl.fold (fun tok () acc -> if Hashtbl.mem sb tok then acc + 1 else acc) sa 0
    in
...
let proactive_preview_similarity_stats
      ?(window = 8)
      ?(warn_threshold = 0.90)
      (previews : string list)
  : int * int * float * float * bool
  =
  let previews =
    previews |> List.map String.trim |> List.filter (fun s -> s <> "") |> take_last window
  in
  ...
  in
  let max_sim = if pair_count = 0 then 0.0 else List.fold_left max 0.0 sims in
...
let keeper_metrics_24h_json ~(metrics_lines : string list) ~(now_ts : float)
  ...
  List.iter
    (fun line ->
       try
         ...
         then (
           incr sample_points;
           let bucket_ts = int_of_float (floor (ts_unix /. 3600.0) *. 3600.0) in
           let b =
           ...
           if channel = "scheduled_autonomous" || channel = "proactive"
           then (
       ...
       | exn -> Log.Server.info "keeper log parse: %s" (Printexc.to_string exn))
    lines;
  let rows =
    buckets
    |> Hashtbl.to_seq
    ...
    |> List.map (fun (bucket_ts, b) ->
      let context_ratio_avg =
        if b.sample_points = 0
        then 0.0
        else b.context_ratio_sum /. float_of_int b.sample_points
      in
      let proactive_fallback_rate =
        if b.proactive_points = 0
        then 0.0
        else float_of_int b.proactive_fallback_count /. float_of_int b.proactive_points
      in
      `Assoc
  ...
  let proactive_fallback_rate =
    if !proactive_points = 0
    then 0.0
    else float_of_int !proactive_fallback_count /. float_of_int !proactive_points
  in
  let summary =
    `Assoc
...
let keeper_history_summary_json
  ...
  =
  let history_lines =
    Keeper_memory.read_file_tail_lines history_path ~max_bytes:120000 ~max_lines:80
  in
  ...
  let conversation_rev, k2k_rev, raw_count, fragment_count, filtered_count =
    List.fold_left
      (fun (conv_acc, k2k_acc, raw_count, fragment_count, filtered_count) line ->
         try
           let j = Yojson.Safe.from_string line in
           ...
           then conv_acc, k2k_acc, raw_count, fragment_count, filtered_count
           else (
             let is_fragment =
               role_lc = "assistant"
  ...
  let k2k_mentions =
    ...
    |> Keeper_types.take 5
    |> List.map (fun (k, v) -> `Assoc [ "keeper", `String k; "count", `Int v ])
    |> fun xs -> `List xs
  in
  conversation, k2k_recent, k2k_mentions, raw_count, fragment_count, filtered_count
;;

let top_counts_json ?(limit = 5) ~(name_key : string) (tbl : (string, int) Hashtbl.t)
  : Yojson.Safe.t list
  =
  tbl
  |> Hashtbl.to_seq
  ...
  |> List.map (fun (k, v) -> `Assoc [ name_key, `String k; "count", `Int v ])
;;

let top_count_name_and_count (tbl : (string, int) Hashtbl.t) : (string * int) option =
  tbl
  |> Hashtbl.to_seq
  |> List.of_seq
  |> List.sort (fun (ka, va) (kb, vb) ->
