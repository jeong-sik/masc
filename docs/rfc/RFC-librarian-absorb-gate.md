...
| 관문별 켜기/끄기 | 선언 | `MASC_TYPESAFEAI_ABSORB_GATE_ENABLED`(이 관문, **기본 꺼짐** — Librarian 기억을 벤더로 보내는 새 반출이라 운영자가 켠다), `MASC_TYPESAFEAI_BOARD_ATTENTION_ENABLED`(Board 관문, 기본 켜짐 = 기존 동작). 키는 레인을 켜고, 관문은 이름으로 켜고 끈다 |
