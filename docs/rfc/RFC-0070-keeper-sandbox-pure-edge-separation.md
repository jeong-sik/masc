...
- **v2.6 (2026-05-12, Phase 4.2/4.3 cutover gap analysis — measurement, iter 53)**: measured the two one-shot caller sites against `Sandbox_executor` + `Keeper_sandbox_oneshot_plan` + `Docker_client_real.run` as they stand today. Phase 4.2 is **bigger than the v2 table implied** and Phase 4.3 is **mis-scoped**:
  ...
  §4 table: `4.1-g` row marked PR #14989; `4.2` row split into `4.2-i` (enrich one-shot plan + `run` + thread `~actor` — prereq) / `4.2` (cutover `keeper_shell_docker` plain path); `4.3` row dropped (re-scoped — see this entry); Phase 5 dep updated (`4.1`+`4.2` merged, not `4.3`). Source: iter 53 measurement of `keeper_shell_docker.ml:1-1086` (esp. `keeper_sandbox_container_name:61` [wall-clock F1], the hardened argv block `:817-857`, `~actor:Keeper_shell`, the git-creds variants `:894,969`) + `keeper_sandbox_oneshot_plan.mli` (4-field plan) + `docker_client_real.ml run` (unhardened argv) + `keeper_sandbox_runtime.ml:838` (`--entrypoint sh --network none` preflight).
...
- Phase 4.1 (session caller) needs Phase 3e-e + 3e-a + 3e-f (#14970/#14973/#14980 — like the 3e-b/c/d edge primitives, each shipped one PR; safe to fragment since none exposed a new layer) **and** `4.1-g` + `4.1-h` (`exec ?stdin`). `4.1-h` is an independent 3e-style extension. Order: `4.1-g` → `4.1-h` → 4.1 cutover.
- Phase 4.2 (one-shot caller, `keeper_shell_docker` plain path) needs `4.1-g` **and** `4.2-i` (enrich `Keeper_sandbox_oneshot_plan` + `Docker_client_real.run` to the hardened shape + thread `~actor`). `4.2-i` is the one-shot analogue of 3e-e + 3e-a and is sizeable; the git-creds variants and the image-preflight sites (ex-4.3) are out of scope (see v2.6). Order: `4.1-g` → `4.2-i` → 4.2 cutover. Phases 4.1-* and 4.2-* are independent of each other after `4.1-g`.
