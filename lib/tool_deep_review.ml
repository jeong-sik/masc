...
let validate_target_files target_files =
  let inputs =
    List.map
      (fun rel_path ->
         if Filename.check_suffix rel_path ".mli"
         then Adversarial_eval.Interface_contract { path = rel_path; content = "" }
         else Adversarial_eval.Changed_file { path = rel_path; content = "" })
      target_files
  in
...
let read_file_truncated path ~max_chars =
  try
    ...
    then Some (String.sub content 0 max_chars ^ "\n... (truncated)")
    else Some content
  with
  | Sys_error _ -> None
;;
...
let build_prompt ~target_files ~question ~base_path =
  ...
  | Ok () ->
    ...
    in
    if file_sections = []
    then Error "No readable target files found"
    else (
  ...
  then
    ...
    , Yojson.Safe.to_string
        (`Assoc
            [ ( "error"
              , `String
                  "target_files (non-empty array) and question (string) are required" )
            ]) )
  else (
    match build_prompt ~target_files ~question ~base_path:config.base_path with
    | Error msg -> false, Yojson.Safe.to_string (`Assoc [ "error", `String msg ])
    | Ok prompt ->
      (match
       ...
       | Error err ->
         ...
         ( true
         , Yojson.Safe.to_string
             (`Assoc [ "verdict", `String "unavailable"; "error", `String msg ]) )))
;;
...
let tool_definitions =
  [ ( "masc_deep_review"
    ...
    , [ ( "target_files"
        , "array"
        , true
        , "List of file paths relative to room base (e.g., [\"lib/foo.ml\"])" )
      ; ( "question"
        , "string"
        , true
        , "Specific question for the reviewer (e.g., 'Are there off-by-one errors?')" )
      ] )
  ]
;;
