...
const EDITOR_LINES = [
  ...
  { n: 15, blame: { k: 'masc', text: 'masc · 4h' }, note: true, code: <>&nbsp;&nbsp;&nbsp;&nbsp;<span className="tok-key">await</span> <span className="tok-key">this</span>.<span className="tok-fn">runRuntime</span>(<span className="tok-str">'provider-b'</span>);</> },
...
const THREADS = [
  ...
  { kind: 'suggest', anchor: 'keeper.ts:15', author: 'nick0cave', ts: '14m', body: 'Consider gating `runRuntime(\'provider-b\')` behind the goal priority. P1 goals only? We runtime too eagerly.', replies: 2 },
  ...
  return (
    <div style={{ display: 'grid', gridTemplateRows: '32px 26px 52px 180px minmax(0,1fr) 28px', width: w, height: h, background: 'var(--color-bg-page)', border: '1px solid var(--color-border-default)', borderRadius: 'var(--r-1)', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '0 12px', gap: 14, background: 'var(--color-bg-surface)', borderTop: '1px solid var(--color-border-strong)', fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--color-fg-muted)' }}>
        ...
        <span style={{ width: 1, height: 10, background: 'var(--color-border-strong)' }} />
        <span>runtime: <span style={{ color: 'var(--color-accent-fg)' }}>provider-b@2 · 1.24s</span></span>
  ...
  const layers = [
    { id: 'base', label: 'SOURCE · keeper.ts', z: 0, bg: 'var(--color-bg-page)', render: () => (
      <div style={{ padding: 12, fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--color-fg-secondary)', lineHeight: 1.6 }}>
        ...
        <div style={{ paddingLeft: 36 }}><span style={{ color: '#c195e8' }}>await</span> this.<span style={{ color: '#e8c976' }}>runRuntime</span>(<span style={{ color: '#a8c97a' }}>'provider-b'</span>);</div>
