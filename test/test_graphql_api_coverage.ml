...
let test_max_first_positive () = check bool "positive" true (Graphql_api.max_first > 0)
...
let test_default_first_less_than_max () =
  check bool "less than max" true (Graphql_api.default_first <= Graphql_api.max_first)
;;
