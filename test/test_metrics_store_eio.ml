...
let test_create_metric () =
  let metric =
    Metrics_store_eio.create_metric
      ~agent_id:"claude"
      ~task_id:"task-001"
...
let test_complete_metric () =
  let metric =
    Metrics_store_eio.create_metric ~agent_id:"gemini" ~task_id:"task-002" ()
  in
  let completed =
    Metrics_store_eio.complete_metric metric ~success:true ~handoff_to:"codex" ()
  in
...
let test_record_and_get () =
  with_temp_masc_dir (fun config ->
    ...
    let metric =
      Metrics_store_eio.create_metric ~agent_id:"claude" ~task_id:"task-record" ()
    in
    ...
    (* Find our metric *)
    let found =
      List.exists (fun m -> m.Metrics_store_eio.task_id = "task-record") recent
    in
    assert found);
...
let test_handoff_tracking () =
  with_temp_masc_dir (fun config ->
    ...
    let m2 = Metrics_store_eio.create_metric ~agent_id:"claude" ~task_id:"handoff-2" () in
    ...
    | Some metrics ->
      ...
      (* Both handoffs successful *)
      print_endline
        (Printf.sprintf
           "  Handoff success rate: %.0f%%"
           (metrics.handoff_success_rate *. 100.0))
    | None -> failwith "Expected metrics");
...
let test_filter_recent_month_filenames () =
  ...
  in
  let filtered =
    Metrics_store_eio.filter_recent_month_filenames ~now:1_775_091_200.0 ~days:7 filenames
  in
