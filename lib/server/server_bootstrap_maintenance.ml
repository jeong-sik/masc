...
let start_background_maintenance ~sw ~clock ~env (state : Mcp_server.server_state) =
  ...
  (* RFC-0259 §3.3/§3.4: memory-os grounding reconciler. Off the keeper hot path —
     ...
     state (a PR/issue id, P1's [external_ref]) against the source of truth via [gh]:
     a still-open ref has its [last_verified_at] advanced; a merged/closed ref backing
     an in-progress claim is retracted (the live-store false-fact class that had
     keepers acting on a closed PR for ~30 turns); [Unverifiable] (gh failure / non-PR
     kind) is never acted on — uncertainty is not contradiction. The write happens
     under the per-keeper facts lock ({!Keeper_memory_os_reconcile.run_reconcile}).
     ...
     reviews the dry-run log before enabling APPLY. Skipped entirely when proc_mgr is
     absent (no gh) or no alert repo is configured. *)
