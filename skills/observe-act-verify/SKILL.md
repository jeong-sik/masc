...
If reading an artifact fails, compare the exact handle sent to the reader with the handle in the successful capture receipt before diagnosing storage. A missing object under a different handle does not show that the captured image was lost. Correct a mismatched reference from the receipt, without guessing a replacement. If the exact issued handle fails, preserve that failure and use the tool's documented recovery. A content-addressed store can return the same handle for an unchanged image; a new capture need not produce a new identifier. Keep capture success, reference lookup failure and image interpretation failure distinct when recording lessons.
