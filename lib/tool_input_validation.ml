...
let canonical_transition_action_string (raw : string) : string option =
  ...
  | "start" | "started" | "in_progress" | "inprogress" | "working" -> Some "start"
  | "done" | "complete" | "completed" | "finish" | "finished" -> Some "done"
  | "cancel" | "cancelled" | "canceled" -> Some "cancel"
  | "release" | "released" | "todo" | "pending" | "backlog" | "open" | "unclaimed" ->
    Some "release"
  | "submit_for_verification"
  | "awaiting_verification"
  | "needs_verification"
  | "verification_requested" -> Some "submit_for_verification"
  | "approve" | "approved" | "verified" -> Some "approve"
  | "reject" | "rejected" | "changes_requested" | "changesrequested" -> Some "reject"
  | _ -> None
;;

let upsert_assoc key value fields = (key, value) :: List.remove_assoc key fields

let strip_internal_marker_args (args : Yojson.Safe.t) : Yojson.Safe.t =
  match args with
  | `Assoc fields ->
    `Assoc (List.filter (fun (key, _) -> String.length key = 0 || key.[0] <> '_') fields)
  | _ -> args
;;
...
let normalize_transition_args (args : Yojson.Safe.t) : Yojson.Safe.t =
  ...
  | `Assoc fields ->
    ...
    let notes_override, consume_note =
      if List.mem_assoc "notes" fields
      then None, false
      else (
        ...
        | _ -> None, false)
    in
    if action_override = None && notes_override = None
    then args
    else (
      let filtered =
        List.filter
...
let missing_field_re = Re.Pcre.re {|"([^"]+)":\s*MISSING|} |> Re.compile
...
let param_name_similarity a b =
  let la = String.lowercase_ascii a
  and lb = String.lowercase_ascii b in
  if la = lb
  then 1.0
  else if string_contains la lb || string_contains lb la
  then
...
let register_pre_hook () =
  ...
  Tool_dispatch.register_pre_hook (fun ~name ~args ->
    ...
    | Oas.Tool_middleware.Pass when not (Yojson.Safe.equal compat_args args) ->
    ...
    | Oas.Tool_middleware.Reject { message; _ } ->
      ...
      Reject
        ...
        ; data =
            `Assoc
              [ "error", `String augmented; "validation", `String "oas_tool_middleware" ]
        ; tool_name = name
        ; duration_ms = 0.0
        })
;;
