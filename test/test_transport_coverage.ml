...
let test_protocol_of_string_jsonrpc () =
  check
    ...
    (Some true)
    (Option.map
       (fun p -> p = Transport.JsonRpc)
       (Transport.protocol_of_string "json-rpc"));
  check
    (option bool)
    "jsonrpc"
    (Some true)
...
let test_error_codes_all_negative () =
  ...
  in
  List.iter (fun c -> check bool (Printf.sprintf "code %d negative" c) true (c < 0)) codes
;;
...
let test_make_error_with_data () =
  ...
  let resp =
    Transport.make_error ~code:(-32000) ~message:"Server error" ~data:(Some data) ()
  in
  match resp.error with
  | Some e ->
    (match e.data with
...
let test_make_error_no_data () =
  ...
  match resp.error with
  | Some e ->
    check (option string) "no data" None (Option.map Yojson.Safe.to_string e.data)
  | None -> fail "expected error"
;;
...
let test_make_success_basic () =
  ...
  check
    ...
    "no error"
    None
    (Option.map (fun e -> e.Transport.message) resp.error);
...
let test_make_success_complex_result () =
  let result =
    `Assoc [ "items", `List [ `String "a"; `String "b"; `String "c" ]; "count", `Int 3 ]
  in
  let resp = Transport.make_success ~result () in
  match resp.result with
  | Some r ->
...
let test_request_no_id () =
  let req : Transport.request =
    { id = None; method_name = "notification"; params = `Null; headers = [] }
  in
...
let test_response_success () =
  let resp : Transport.response =
    { id = Some "req-003"; success = true; result = Some (`String "done"); error = None }
  in
  check bool "success" true resp.success;
  check
    (option string)
...
let test_error_creation () =
  let e : Transport.error = { code = -32700; message = "Parse error"; data = None } in
...
let test_binding_rest () =
  let b : Transport.binding =
    { protocol = Transport.Rest; url = "https://api.example.com/v1"; options = [] }
  in
...
let test_jsonrpc_version () = check string "version" "2.0" Transport.JsonRpc.version
...
let test_jsonrpc_parse_request_no_id () =
  let json =
    `Assoc [ "jsonrpc", `String "2.0"; "method", `String "notification"; "params", `Null ]
  in
  match Transport.JsonRpc.parse_request json with
  | Ok req ->
    check (option string) "no id" None req.id;
...
let test_jsonrpc_parse_request_wrong_version () =
  let json =
    `Assoc [ "jsonrpc", `String "1.0"; "method", `String "test"; "params", `Null ]
  in
  match Transport.JsonRpc.parse_request json with
  ...
  | Error e ->
    check
      ...
      true
      (try
         ignore (Str.search_forward (Str.regexp_string "version") e 0);
         true
       with
       | Not_found ->
         (try
            ignore (Str.search_forward (Str.regexp_string "jsonrpc") e 0);
            true
          with
          | Not_found -> false))
;;
...
let test_jsonrpc_parse_request_missing_method () =
  let json = `Assoc [ "jsonrpc", `String "2.0"; "params", `Null ] in
  match Transport.JsonRpc.parse_request json with
...
let test_jsonrpc_serialize_response_success () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "2.0") json_str 0 in
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "result") json_str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_jsonrpc_serialize_response_error () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "error") json_str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_jsonrpc_serialize_response_no_id () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "null") json_str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_jsonrpc_make_request_basic () =
  let json =
    Transport.JsonRpc.make_request ~method_name:"masc_join" ~params:(`Assoc []) ()
  in
  let json_str = Yojson.Safe.to_string json in
  check
    bool
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "masc_join") json_str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_jsonrpc_make_request_with_id () =
  let json =
    Transport.JsonRpc.make_request ~id:"req-99" ~method_name:"test" ~params:`Null ()
  in
  let json_str = Yojson.Safe.to_string json in
  check
    bool
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "req-99") json_str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_rest_parse_request_operator_digest () =
  let req =
    Transport.Rest.parse_request
      ...
      ~query_params:[ "target_type", `String "namespace" ]
      ~body:""
  in
...
let test_rest_parse_request_websocket_discovery () =
  let req =
    Transport.Rest.parse_request ~http_method:"GET" ~path:"/ws" ~query_params:[] ~body:""
  in
...
let test_rest_parse_request_webrtc_answer () =
  let req =
    Transport.Rest.parse_request
      ...
      ~path:"/webrtc/answer"
      ~query_params:[]
      ~body:"{\"offer_id\":\"offer-1\",\"agent_name\":\"b\"}"
  in
...
let test_rest_parse_request_broadcast () =
  ...
  let req =
    Transport.Rest.parse_request
      ...
      ~path:"/broadcast"
      ~query_params:[]
      ~body:"{\"agent_name\":\"test\",\"message\":\"hello\"}"
  in
...
let test_rest_parse_request_root () =
  let req =
    Transport.Rest.parse_request ~http_method:"GET" ~path:"/" ~query_params:[] ~body:""
  in
  check string "method_name" "masc_status" req.method_name
;;
...
let test_rest_generate_openapi_paths () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "masc_status") json_str 0 in
       true
     with
     | Not_found -> false);
  check
...
let test_rest_generate_openapi_document () =
  ...
  check bool "mcp security present" true (mcp_post |> member "security" <> `Null);
  let operations = mcp_post |> member "x-mcp-operations" |> to_list in
  let operation_entry operation_id =
    operations
    |> List.find (fun row -> row |> member "operationId" |> to_string = operation_id)
  in
  let check_operation_tags operation_id expected =
    let tags =
      operation_entry operation_id |> member "tags" |> to_list |> List.map to_string
    in
  ...
  let sdk_aliases = status_entry |> member "x-agent-sdk" |> member "aliases" |> to_list in
  check
    bool
    "has sdk alias masc_room_status"
    true
    (List.exists
  ...
  check
    int
    "add_task has no fake rest binding"
    0
    (add_task_entry |> member "x-rest-bindings" |> to_list |> List.length);
  let broadcast_entry = operation_entry "masc_broadcast" in
  check
    bool
    "broadcast rest binding is real"
    true
    (List.exists
       (fun row -> row |> member "path" |> to_string = "/api/v1/broadcast")
...
let test_rest_generate_openapi_document_relative_server_fallback () =
  ...
  check
    string
    "relative server url when host unknown"
    "/"
...
let test_get_bindings_url_contains_host () =
  ...
  check
    ...
    (List.exists
       (fun b ->
          try
            let _ = Str.search_forward (Str.regexp "127.0.0.1") b.Transport.url 0 in
            true
          with
          | Not_found -> false)
       bindings)
...
let test_get_bindings_url_contains_port () =
  ...
  check
    ...
    (List.exists
       (fun b ->
          try
            let _ = Str.search_forward (Str.regexp "9999") b.Transport.url 0 in
            true
          with
          | Not_found -> false)
       bindings)
...
let test_bindings_to_json_has_protocol () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "protocol") json_str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_bindings_to_json_has_url () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "url") json_str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_stats_record_request_success () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "total_requests") json_str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_stats_record_request_failure () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "failed_requests") json_str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_stats_reset () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "\"total_requests\":0") json_str 0 in
       true
     with
     | Not_found -> false)
;;
...
let test_stats_avg_latency () =
  ...
  check
    ...
    true
    (try
       let _ = Str.search_forward (Str.regexp "avg_latency_ms") json_str 0 in
       true
     with
     | Not_found -> false)
;;
  ...
  run
    ...
    ; ( "rest.tool_to_endpoint"
      , [ test_case "status" `Quick test_rest_tool_to_endpoint_status
        ...
        ; test_case "operator digest" `Quick test_rest_tool_to_endpoint_operator_digest
        ; test_case "operator action" `Quick test_rest_tool_to_endpoint_operator_action
        ; test_case "operator confirm" `Quick test_rest_tool_to_endpoint_operator_confirm
        ; test_case "agent_card" `Quick test_rest_tool_to_endpoint_agent_card
        ; test_case
            "websocket_discovery"
            `Quick
            test_rest_tool_to_endpoint_websocket_discovery
    ...
    ; ( "rest.parse_request"
      , [ test_case "status" `Quick test_rest_parse_request_status
        ...
        ; test_case "operator snapshot" `Quick test_rest_parse_request_operator_snapshot
        ; test_case "operator digest" `Quick test_rest_parse_request_operator_digest
        ; test_case "operator action" `Quick test_rest_parse_request_operator_action
        ; test_case "operator confirm" `Quick test_rest_parse_request_operator_confirm
