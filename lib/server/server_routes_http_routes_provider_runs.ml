    ...
    with_public_read
      (fun state req reqd ->
         ...
         let agg =
           ...
           else Model_inference_metrics.compute ~base_path ~window_minutes:window
         in
         Http.Response.json
           ~compress:true
           ~request:req
           (Yojson.Safe.to_string (Model_inference_metrics.to_json agg))
           reqd)
      request
      reqd)
      ...
      (fun state _agent_name _req reqd ->
         Http.Request.read_body_async reqd (fun body_str ->
           try
             ...
             match
               Dashboard_provider_runs.start_run
                 ~sw
           ...
           | Yojson.Json_error error ->
             respond_json_with_cors
               ...
               (Yojson.Safe.to_string
                  (`Assoc [ "error", `String ("invalid json: " ^ error) ]))
           | Yojson.Safe.Util.Type_error (error, _) ->
             respond_json_with_cors
               ...
               reqd
               (Yojson.Safe.to_string
                  (`Assoc [ "error", `String ("invalid request shape: " ^ error) ]))))
      request
      reqd)
      ...
      (fun _state req reqd ->
         ...
         | None ->
           respond_json_with_cors
             ...
             request
             reqd
             (Yojson.Safe.to_string (`Assoc [ "error", `String "run_id is required" ]))
         | Some run_id ->
           (match Dashboard_provider_runs.run_status_json run_id with
            | Ok payload ->
              respond_json_with_cors request reqd (Yojson.Safe.to_string payload)
            | Error message ->
              respond_json_with_cors
                ~status:`Not_found
                request
