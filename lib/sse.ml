(** SSE (Server-Sent Events) module for MCP Streamable HTTP Transport
    ...
    workspace traffic.  [Workspace_client] sessions receive heartbeats
...
type session_kind =
  ...
  | Workspace_client [@tla.symbol "workspace_client"] (** MCP agent connections *)
...
type broadcast_target =
  ...
  | Workspace_clients (** Only [Workspace_client] sessions *)
...
let sync_transport_snapshot () =
  ...
  SMap.iter (fun session_id client ->
    ...
    (match client.kind with
     ...
     | Workspace_client -> incr workspace_client
  ...
  Transport_metrics.set_sse_sessions ~kind:"workspace_client" !workspace_client;
...
let event_matches_session_kind kind event =
  match kind with
  | Observer -> true
  | Workspace_client -> Sse_jsonrpc_filter.event_string_jsonrpc_message_for_workspace_client event
  | Presence -> false
...
let register ?(kind = Workspace_client) ?on_disconnect session_id ~last_event_id =
...
let client_matches_target target ~jsonrpc_payload (client : client) =
  ...
  | All -> (
      match client.kind with
      | Observer -> true
      | Workspace_client -> jsonrpc_payload
      | Presence -> false)
  | Observers -> client.kind = Observer
  | Workspace_clients -> client.kind = Workspace_client && jsonrpc_payload
...
let broadcast_impl ?(buffer = true) ?(notify_external = true)
  ...
  let jsonrpc_payload =
    Sse_jsonrpc_filter.jsonrpc_message_for_workspace_client json
  in
...
(** Broadcast event to sessions matching [target].
    ...
    - [Workspace_clients]: MCP agent sessions only *)
...
let send_to session_id json =
  if not (Sse_jsonrpc_filter.jsonrpc_message_for_workspace_client json) then
    Log.Server.warn
