...
- Keeper: an official client's start seed begins where the Librarian read, like the Agent Core lane's does. Claude Code and Antigravity carry the saved working state in place of the atoms it summarises, and their `model input carried range` line reports `origin=librarian_snapshot`. The seed still starts at the latest of the three positions — the Librarian's, the lane's own cut, and the range the last completed turn carried — so this never widens a request. A saved position that does not describe the messages the lane is about to send is not used, and the lane keeps the front it had.
