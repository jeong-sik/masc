    ...
    then
      Log.warn
        ...
        "codex_cli omitting keeper-bound runtime MCP tool(s) that require request-scoped \
         auth headers: %s (structural provider limitation for %s; subsequent omissions \
         of this same set are counted in masc_codex_cli_mcp_tool_omission_total and not \
         re-logged)"
...
let invalid_runtime_config field detail =
  Oas.Error.Config (Oas.Error.InvalidConfig { field; detail })
;;
...
let trim_nonempty value = Option.bind value trim_nonempty_string
...
let resolve_tool_lane_for_oas_tools
      ...
      ()
  : ( Oas.Tool.t list * Llm_provider.Llm_transport.runtime_mcp_policy option
      , Oas.Error.sdk_error )
      result
  =
  ...
  | _ ->
    let detail =
      let runtime_mcp_requires_http_headers =
        match runtime_mcp_policy with
        | Some policy ->
          Provider_tool_support.runtime_mcp_policy_requires_http_headers policy
        | None -> false
      in
      if
        public_tool_names <> []
      ...
      then
        Printf.sprintf
          ...
          (provider_label provider_cfg)
      else Printf.sprintf "%s does not support inline tools" (provider_label provider_cfg)
    in
...
module Kimi_cli_transport_local = struct
  ...
  let events_of_block ~index = function
    ...
    | Oas.Types.Thinking { content; _ } ->
      ...
      ; Oas.Types.ContentBlockDelta { index; delta = Oas.Types.ThinkingDelta content }
      ; Oas.Types.ContentBlockStop { index }
      ]
    | Oas.Types.ToolUse { id; name; input } ->
      ...
      ; Oas.Types.ContentBlockDelta
          { index; delta = Oas.Types.InputJsonDelta (Yojson.Safe.to_string input) }
      ; Oas.Types.ContentBlockStop { index }
      ]
    | Oas.Types.ToolResult { tool_use_id; content; _ } ->
      ...
      ; Oas.Types.ContentBlockDelta { index; delta = Oas.Types.TextDelta content }
      ; Oas.Types.ContentBlockStop { index }
      ]
  ...
  let warn_external_tools_once warned tools =
    ...
    then ()
      ...
      warned := true;
      Eio.traceln
        "[warn] kimi_cli print mode ignores OAS req.tools. Provider-native built-in \
         tools and configured MCP servers remain available; external OAS tool callbacks \
...
let non_http_transport_of_provider
  ...
  | Anthropic | OpenAI_compat | Ollama | Gemini | Glm | Kimi -> Ok None
;;
