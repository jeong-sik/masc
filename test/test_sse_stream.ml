...
let test_broadcast_popable () =
  reset ();
  ignore (Sse.register ~kind:Observer "s-pop-bc" ~last_event_id:0);
...
let test_broadcast_multiple_clients_streams () =
  reset ();
  ignore (Sse.register ~kind:Observer "s-mc-1" ~last_event_id:0);
  ignore (Sse.register ~kind:Observer "s-mc-2" ~last_event_id:0);
  ignore (Sse.register ~kind:Observer "s-mc-3" ~last_event_id:0);
...
let test_pop_blocks_then_receives () =
  reset ();
  ignore (Sse.register ~kind:Observer "s-block" ~last_event_id:0);
...
let test_broadcast_skips_already_seen () =
  ...
  ignore (Sse.register ~kind:Observer "s-skip" ~last_event_id:999_999_999);
...
let test_broadcast_event_contains_data () =
  reset ();
  ignore (Sse.register ~kind:Observer "s-data" ~last_event_id:0);
...
let test_broadcast_to_observers_only () =
  ...
  ignore (Sse.register ~kind:Workspace_session "s-workspace" ~last_event_id:0);
  ...
  Alcotest.(check bool) "workspace_session did not" true (got_workspace = None);
  Sse.unregister "s-obs";
  Sse.unregister "s-workspace"

let test_broadcast_to_workspace_sessions_only () =
  reset ();
  ignore (Sse.register ~kind:Observer "s-obs2" ~last_event_id:0);
  ignore (Sse.register ~kind:Workspace_session "s-workspace2" ~last_event_id:0);
  Sse.broadcast_to Workspace_sessions (jsonrpc_notification "notifications/test");
  ...
  Alcotest.(check bool) "workspace_session got event" true (got_workspace <> None);
...
let test_broadcast_to_all () =
  ...
  ignore (Sse.register ~kind:Workspace_session "s-all-workspace" ~last_event_id:0);
...
let test_broadcast_equals_broadcast_to_all () =
  ...
  ignore (Sse.register ~kind:Workspace_session "s-eq-workspace" ~last_event_id:0);
  ...
  Alcotest.(check bool) "workspace_session got broadcast" true (got_workspace <> None);
...
let test_broadcast_all_excludes_presence_sessions () =
  ...
  ignore (Sse.register ~kind:Observer "s-all-observer-only" ~last_event_id:0);
...
let test_non_jsonrpc_broadcast_does_not_reach_workspace_sessions () =
  reset ();
  let before_id = Sse.current_id () in
  ignore (Sse.register ~kind:Observer "s-nonjson-obs" ~last_event_id:0);
  ignore (Sse.register ~kind:Workspace_session "s-nonjson-workspace" ~last_event_id:0);
  ...
  Alcotest.(check bool) "workspace_session skipped non-JSON-RPC" true
    (got_workspace = None);
  ...
  Alcotest.(check (list string))
    "workspace_session replay skips non-JSON-RPC"
    []
    (Sse.get_events_after_for_kind Workspace_session before_id);
  Sse.unregister "s-nonjson-obs";
  Sse.unregister "s-nonjson-workspace"

let test_register_defaults_to_workspace_session () =
  reset ();
  (* Register without explicit kind *)
  ignore (Sse.register "s-default" ~last_event_id:0);
  (* Should be Workspace_session: receives Workspace_sessions-targeted broadcast *)
  Sse.broadcast_to Workspace_sessions (jsonrpc_notification "notifications/test");
  let got = Sse.try_pop "s-default" in
  Alcotest.(check bool) "default kind is Workspace_session" true (got <> None);
    ...
    [
      ...
      ( "broadcast_to_targeting",
        [
          ...
          Alcotest.test_case "workspace_sessions only" `Quick test_broadcast_to_workspace_sessions_only;
          ...
          Alcotest.test_case "non-JSON-RPC skips workspace_sessions" `Quick
            test_non_jsonrpc_broadcast_does_not_reach_workspace_sessions;
          Alcotest.test_case "default kind is Workspace_session" `Quick test_register_defaults_to_workspace_session;
        ] );
    ]
