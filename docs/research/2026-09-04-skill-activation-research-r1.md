...
`keeper_skill` **도구 서술에** 실려서 닿고, 매 턴 3,685바이트를 쓴다.

## 요약

- 스킬 10개의 이름·설명·**정확 참조(64hex 포함)**가 `keeper_skill` 도구
  서술의 `Available:` 블록으로 매 턴 실린다. 3,685B, 스킬당 368B.
...
라이브 카탈로그로 계산하면 이 `Available:` 블록이 **3,685바이트**, 스킬당
**368바이트**다. 그중 절반쯤이 정확 참조 JSON 이다(64hex content_revision 포함).
...
190바이트이고 서술 전체로는 368바이트다. 이 문서의 첫 판이 프롬프트만 보고
"0바이트"라고 반박했는데, 잘못 본 것이다 — 비용은 프롬프트가 아니라 도구
서술에 있다.

`#31324` 가 제안한 방향(참조를 enum 이나 이름으로 줄이기)이 지금 측정과
맞물린다. 매 턴 3,685바이트를 내고 0.01회를 얻는 교환에서, 그중 절반이
...
아니라 368바이트(약 94토큰... 한글 설명이라 토큰은 더 든다)가 되고, 호출부가
...
3,685바이트로 사고 있고, 회수가 0.01회/턴이며, 제일 많이 불리는 하나가
고장나 있다는 것이다.

판정 기준: 조치 전후의 **스킬 계열 호출/턴**과 **`Available:` 블록 바이트**.
지금 기준선은 0.0072회/턴(13/1,796), 3,685B.
...
- 아끼는 것: 스킬당 185바이트 × 10 = 약 1.9KB/턴. 블록이 절반이 된다.
