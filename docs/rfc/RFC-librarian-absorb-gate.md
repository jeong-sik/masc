...
Jev 를 못 부르면(키 없음, `[typesafeai] enabled = false`, HTTP 4xx/5xx, 시간 초과, 응답 모양 오류) 관문은 **열린다**:
Librarian 답을 적용하고 로그에 이유를 남긴다. 지금과 같은 동작이다. 관문이 닫히는 방향의 실패는 없다 — 판정
모델이 죽어도 기억 정리는 멈추지 않는다. 한 가지 예외: 크기 때문에 애초에 물어볼 수 없던 기억(§3 의 요청 한도)은
모델이 답했든 못 했든 현재에 남는다. 그 판정은 모델이 아니라 관문 자신이 요청 전에 내린 것이라, 다른 기억의 요청이
실패했다고 뒤집히지 않는다. 이미 답을 받아 "전하지 못한다"로 판정된 기억도 같다 — 뒤의 요청이 실패해도 그 판정은 남고,
답을 못 받은 기억만 열림으로 적용된다(#37369 Codex 리뷰).
