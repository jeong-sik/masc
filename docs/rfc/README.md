...
| 0088 | Prometheus metric ownership distribution | Draft | (this PR) 2026-05-15 | RFC-0086 Phase 2.B cycle blocker 해결 — `lib/prometheus.ml` 3090 LoC godfile (1067 caller, 221 unique masc_* metric) 의 ~2900 LoC metric name 상수를 도메인 module 로 분산 + core mechanism (~160 LoC) 를 `lib/prometheus_core/` sub-library 로 추출. PR #14166 (split-by-shard anti-pattern, user-closed "이거 해결책이 너무 바보같음") supersedes. Related RFC-0056, RFC-0086 |
