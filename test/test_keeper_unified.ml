...
let test_decide_local_only_liveness_requests_deduped_ollama_probe () =
  ...
  | UT.Probe_local_only_urls { effective_cascade; fallback_cascade; ollama_base_urls } ->
    check string "effective cascade" "local_only" effective_cascade;
    check string "fallback cascade" "keeper_unified" fallback_cascade;
    check (list string) "deduped probe URLs" [ cfg.base_url ] ollama_base_urls
;;
...
let test_resolve_ollama_only_base_url_different_hosts () =
  let resolve_label = function
    ...
    | _ -> None
  in
  match UT.resolve_ollama_only_base_url ~resolve_label [ "ollama:a"; "ollama:b" ] with
  | None -> ()
...
let test_is_ollama_saturated_returns_false_when_idle () =
  ...
  check
    ...
    false
    (UT.is_ollama_saturated ~capacity_lookup:(fun _ -> Some info) url)
;;
    ...
    EC.degraded_retry_after_recoverable_error
      ...
      (Masc_mcp.Oas_worker_named.sdk_error_of_masc_internal_error
         (Masc_mcp.Oas_worker_named.Admission_queue_timeout
            { keeper_name = "nick0cave"; cascade_name = "big_three"; wait_sec = 90.0 }))
  in
  expect_degraded_retry
...
let test_next_fail_open_cascade_for_turn_continues_to_local_recovery () =
  ...
  in
  expect_degraded_retry
    "next degraded retry after default"
    KC.local_recovery_cascade_name
    "hard_quota"
    degraded_retry
;;
...
let test_next_fail_open_cascade_for_required_tool_rejects_local_recovery_only_catalog () =
  ...
  check
    bool
    "required catalog local_recovery-only group is exhausted"
    true
    (Option.is_none degraded_retry)
;;
...
let test_fail_open_rotation_cascades_from_catalog_merges_reserved_and_assignable () =
  ...
  check
    ...
    (Some [ KC.default_cascade_name; KC.local_recovery_cascade_name; "ollama_only" ])
    rotation
;;
...
let test_fail_open_rotation_cascades_from_catalog_preserves_catalog_order () =
  ...
  check
    ...
    (Some [ "ollama_only"; KC.local_recovery_cascade_name; KC.default_cascade_name ])
    rotation
;;
...
let test_metrics_failure_response_redacts_resumable_cli_session_detail () =
  ...
  check
    ...
    canonical_detail
    updated.runtime.proactive_rt.last_preview;
  check string "last blocker is redacted" canonical_detail updated.runtime.last_blocker;
  check
    bool
...
let test_bounded_oas_timeout_uses_adaptive_when_budget_is_large () =
  ...
  | Some timeout_s ->
    ...
    check
      bool
      "bounded leaves positive finalization guard"
      true
      (timeout_s < 1200.0 && timeout_s > 1100.0)
...
let test_bounded_oas_timeout_uses_channel_turn_budget_override () =
  ...
  | Some timeout_s ->
    ...
    check
      bool
      "finalization guard reserves positive delta"
      true
      (timeout_s <= 1200.0 && timeout_s > 1100.0)
  | None -> fail "expected bounded timeout"
;;
...
let test_oas_timeout_reclassifies_only_current_attempt_budget () =
  ...
  | Some timeout_budget ->
    ...
    in
    (match Masc_mcp.Oas_worker_named.classify_masc_internal_error classified with
...
let test_pre_retry_budget_exhaustion_not_reclassified_with_stale_budget () =
  ...
  check
    bool
    "pre-dispatch exhaustion remains raw timeout"
    true
    (Option.is_none (Masc_mcp.Oas_worker_named.classify_masc_internal_error classified))
;;
...
let test_degraded_retry_budget_gate_allows_remaining_budget () =
  ...
  | UT.Degraded_retry_allowed retry ->
    ...
    check string "fallback reason" "oas_timeout_budget" retry.fallback_reason
...
let test_should_require_tools_for_initial_turn_matches_first_turn_gate () =
  ...
  check
    bool
    "work discovery requires an action tool"
    true
    (KAR.should_require_tools_for_initial_turn
       ~max_turns:3
...
let test_preferred_tool_choice_for_required_turn_claims_first () =
  ...
  (match
   ...
   | other ->
     fail
       (Printf.sprintf
          "expected Tool keeper_tasks_audit, got %s"
          (Agent_sdk.Types.show_tool_choice other)));
  (match
     choose
       ~turn_affordances:[ "task_audit" ]
  ...
  (match
   ...
   | other ->
     fail
       (Printf.sprintf
          "expected Tool keeper_tasks_list, got %s"
          (Agent_sdk.Types.show_tool_choice other)));
  ...
  run
    ...
    ; ( "transient_network_error"
      , [ test_case "NetworkError detected" `Quick (fun () ->
        ...
        ; test_case
            "pre-retry budget exhaustion does not reuse stale budget"
            `Quick
    ...
    ; ( "phase_gate"
      , [ test_case
        ...
        ; test_case
            "PR-B: empty labels are not ollama-only"
            `Quick
            test_resolve_ollama_only_base_url_empty_returns_none
        ; test_case
            "PR-B: single ollama label is ollama-only"
            `Quick
            test_resolve_ollama_only_base_url_single_ollama
        ; test_case
            "PR-B: mixed providers are not ollama-only"
            `Quick
            test_resolve_ollama_only_base_url_mixed_provider
        ; test_case
            "PR-B: different ollama hosts are not ollama-only"
            `Quick
      ...
      , [ test_case "affordance: keeper sees task_verify when pending>0" `Quick (fun () ->
        ...
        ; test_case "affordance: no meta keeps legacy surface-to-all" `Quick (fun () ->
            ...
            check
              ...
              true
              (List.mem "task_verify" affordances))
        ; test_case "affordance: work discovery requires action" `Quick (fun () ->
