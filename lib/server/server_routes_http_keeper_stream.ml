...
let keeper_chat_stream_error_json message =
  `Assoc [ "error", `Assoc [ "message", `String message ] ]
;;
...
let execute_keeper_stream_tool ~sw ~clock ?auth_token:_ state ~agent_name ~arguments =
  ...
  let error_msg =
    if success then None else Some (Printf.sprintf "duration_ms=%d" duration_ms)
  in
  ...
  then (
    ...
    | Some fs ->
      (try
         let telemetry_error_kind = if success then None else Some "tool_failure" in
       ...
       | exn -> Log.Misc.error "telemetry tracking failed: %s" (Printexc.to_string exn))
    | None -> ());
    ...
    if
      not
        (match json with
         | `Assoc _ -> true
         | _ -> false)
    then Error "request body must be a JSON object"
    else (
      let name =
        json
        |> member "name"
      ...
      then Error "message is required"
      else if
        has_connector_context
        && (channel = "" || channel_user_id = "" || channel_room_id = "")
      then
        Error
          "channel, channel_user_id, and channel_room_id are required when connector \
           context is supplied"
      else if legacy_models_present
      then
        Error
          "legacy keeper model args removed for masc_keeper_msg: models. Keepers now use \
           cascade_name and last_model_used only."
      else (
...
let split_keeper_reply_chunks (text : string) : string list =
  ...
  then []
    ...
    let push stop =
      if stop > !start then chunks := String.sub text !start (stop - !start) :: !chunks;
      start := stop;
      last_space := None
    in
    for i = 0 to len - 1 do
      ...
      let next_is_boundary = i + 1 >= len || whitespace text.[i + 1] in
      let hard_wrap =
        i - !start >= 180
        &&
    ...
    if !start < len then chunks := String.sub text !start (len - !start) :: !chunks;
    List.rev !chunks |> List.filter (fun chunk -> String.trim chunk <> ""))
;;

let keeper_stream_send_raw writer mutex closed data =
  if !closed || Httpun.Body.Writer.is_closed writer
  then (
    closed := true;
    false)
  else (
...
let send_keeper_stream_finish writer mutex closed ~thread_id ~run_id ~message_id =
  ignore
    (keeper_stream_send_event
       writer
       mutex
...
let extract_visible_reply body =
  ...
  let visible_reply =
    ...
    | Some payload_json ->
      ...
      in
      let visible =
        if String.trim reply_raw = ""
        then String.trim body
...
let handle_keeper_chat_stream ~sw ~clock state request reqd payload =
  ...
  let close_stream () =
    ...
    then (
      ...
      | Eio.Cancel.Cancelled _ as e -> raise e
      | exn -> Log.Misc.warn "keeper_stream writer close: %s" (Printexc.to_string exn))
  in
  ...
  Eio.Fiber.fork ~sw (fun () ->
    ignore
      (Eio.Switch.run
       ...
       | Error err -> send_keeper_error writer mutex closed ~thread_id ~run_id err
       | Ok (false, err) -> send_keeper_error writer mutex closed ~thread_id ~run_id err
       | Ok (true, body) ->
         (try
            ...
            send_keeper_stream_finish writer mutex closed ~thread_id ~run_id ~message_id
          with
          | Eio.Cancel.Cancelled _ as e -> raise e
          | exn ->
            send_keeper_error
