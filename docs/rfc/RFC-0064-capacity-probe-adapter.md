...
`Cascade_ollama_probe` is the only provider-specific capacity probe in the cascade
system. Keeper modules (`keeper_turn_driver`, `keeper_turn_liveness`,
`keeper_unified_turn`) import it directly and call `is_ollama_url` to branch on
provider identity before invoking Ollama-specific functions (`try_probe`,
`cached_capacity`, `refresh_many`).
...
Cascade_throttle.capacity url
  |> fallback Cascade_ollama_probe.cached_capacity url
...
2. An Ollama implementation reusing the existing `Cascade_ollama_probe` internals
...
module type Probe = sig
  ...
  val cached : ?now:float -> url:string -> Cascade_throttle.capacity_info option
  val refresh_many :
    sw:Eio.Switch.t ->
    net:[> [> `Generic ] Eio.Net.ty ] Eio.Resource.t ->
    ?timeout_s:float ->
...
| `lib/cascade/cascade_ollama_probe.ml` | Modify | Wrap internals as `Ollama_probe : Probe` |
| `lib/cascade/cascade_ollama_probe.mli` | Modify | Export `Ollama_probe` module for registration |
...
| `lib/keeper/keeper_unified_turn.ml` | Modify | Replace `Cascade_ollama_probe.cached_capacity` direct call |
| `lib/cascade/dune` | Modify | Add `cascade_capacity_probe` to libraries |

## 4. Migration Path

1. Create `Cascade_capacity_probe` with Ollama as the sole registered probe.
2. Update keeper callers one at a time (3 files).
3. `Cascade_ollama_probe` internals remain unchanged — only wrapped with a
   `Probe` module type and re-exported.
...
- **Test coverage**: Existing `test_cascade_ollama_probe.ml` covers the probe
