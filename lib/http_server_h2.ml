...
let request_of_reqd reqd =
  ...
  { meth = req.meth; target = req.target; headers = req.headers }
;;
...
module Response = struct
  ...
  let html ?(status = `OK) ?(headers = []) body reqd =
    ...
    in
    let response =
      H2.Response.create ~headers:(H2.Headers.of_list (base_headers @ headers)) status
    in
    send_body reqd response body
  ;;
  ...
  let bad_request msg reqd = text ~status:`Bad_request msg reqd
...
module Request = struct
  ...
  let method_string req = H2.Method.to_string req.meth
  let header name req = H2.Headers.get req.headers name
...
module Router = struct
  ...
  let empty : t = []
  let add meth path handler routes = { meth; path; handler } :: routes
  ...
  let find_route routes req =
    ...
    List.find_opt
      (fun route -> route.meth = req.meth && String.equal route.path path)
      routes
  ;;
...
let make_request_handler routes =
  ...
  | exn -> Response.internal_error (Printexc.to_string exn) reqd
;;
...
module Compat = struct
  ...
  let to_httpun_request (h2_req : request) : httpun_request =
    let meth =
      ...
      | `Other s -> `Other s
    in
    { meth; target = h2_req.target; headers = H2.Headers.to_list h2_req.headers }
  ;;
  ...
  let header_get headers name =
    List.find_map
      (fun (k, v) -> if String_util.equals_ci k name then Some v else None)
      headers
  ;;
...
let run ~sw ~net ~clock config request_handler =
  ...
  in
  Printf.printf
    "🚀 MASC MCP Server (HTTP/2) listening on http://%s:%d\n"
    config.host
    config.port;
  ...
  let rec accept_loop () =
      ...
      (try
         ...
         Eio.Fiber.fork ~sw (fun () ->
           Eio.Switch.run (fun conn_sw ->
             ...
             with
             | Eio.Cancel.Cancelled _ as e -> raise e
             | exn -> Log.Http.error "Connection error: %s" (Printexc.to_string exn)))
       with
       | Eio.Cancel.Cancelled _ as e -> raise e
       | exn ->
         if is_cancelled exn then raise exn;
         let delay = !backoff_s in
         Log.Http.error "Accept error: %s (backoff %.2fs)" (Printexc.to_string exn) delay;
    ...
    | exn ->
      ...
      then ()
        ...
        let delay = !backoff_s in
        Log.Http.error
          "Accept loop error: %s (backoff %.2fs)"
