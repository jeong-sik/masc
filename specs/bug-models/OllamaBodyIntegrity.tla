...
\*
\* Models the AGENT_CORE-pinned LLM provider request flow:
\*   complete.ml -> http_client.ml -> Ollama.
\*
\* Both files live in the pinned `agent_core.llm_provider` library
\* (see lib/dune for the dependency), not in masc:
\*   <agent_core>/lib/llm_provider/complete.ml
\*   <agent_core>/lib/llm_provider/http_client.ml
\*
