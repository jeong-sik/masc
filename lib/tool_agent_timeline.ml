...
let event_to_json (e : timeline_event) : Yojson.Safe.t =
  `Assoc [ "ts", `String e.ts_iso; "type", `String e.event_type; "detail", e.detail ]
;;

(* Collect agent join/status events *)
let agent_events (config : Coord.config) ~agent_name : timeline_event list =
...
let task_events (config : Coord.config) ~agent_name : timeline_event list =
  let tasks = Coord.get_tasks_safe config in
  tasks
  |> List.filter_map (fun (task : Types.task) ->
    match task.task_status with
    | Types.Claimed { assignee; claimed_at } when String.equal assignee agent_name ->
      (match parse_iso_timestamp claimed_at with
       ...
       | None -> None)
    | Types.InProgress { assignee; started_at } when String.equal assignee agent_name ->
      (match parse_iso_timestamp started_at with
       ...
       | None -> None)
    | Types.Done { assignee; completed_at; notes } when String.equal assignee agent_name
      ->
...
let message_events (config : Coord.config) ~agent_name ~limit : timeline_event list =
  let messages = Coord.get_messages_raw config ~since_seq:0 ~limit in
  messages
  |> List.filter (fun (m : Types.message) -> String.equal m.from_agent agent_name)
...
let tool_call_events (config : Coord.config) ~agent_name ~limit : timeline_event list =
  let rec take n xs =
    match n, xs with
    | n, _ when n <= 0 -> []
    | _, [] -> []
  ...
  |> List.filter_map (fun (e : Activity_graph.event) ->
    ...
    let tool_name = Safe_ops.json_string ~default:"unknown" "tool_name" e.payload in
    let success = Safe_ops.json_bool ~default:true "success" e.payload in
    let duration_ms = Safe_ops.json_int ~default:0 "duration_ms" e.payload in
...
let keeper_cdal_events (config : Coord.config) ~agent_name ~limit : timeline_event list =
  let rec take n xs =
    match n, xs with
    | n, _ when n <= 0 -> []
    | _, [] -> []
...
let turn_completed_events (config : Coord.config) ~agent_name ~limit : timeline_event list
  =
  let rec take n xs =
    match n, xs with
    | n, _ when n <= 0 -> []
  ...
  |> List.filter_map (fun (e : Activity_graph.event) ->
    ...
    let keeper_name =
      try e.payload |> member "keeper_name" |> to_string with
      | Eio.Cancel.Cancelled _ as ex -> raise ex
      | _ -> "unknown"
    in
    let input_tokens =
      try Some (e.payload |> member "input_tokens" |> to_int) with
      | Eio.Cancel.Cancelled _ as ex -> raise ex
      | _ -> None
    in
    let output_tokens =
      try Some (e.payload |> member "output_tokens" |> to_int) with
      | Eio.Cancel.Cancelled _ as ex -> raise ex
      | _ -> None
    in
    let cache_creation_tokens =
      try Some (e.payload |> member "cache_creation_tokens" |> to_int) with
      | Eio.Cancel.Cancelled _ as ex -> raise ex
      | _ -> None
    in
    let cache_read_tokens =
      try Some (e.payload |> member "cache_read_tokens" |> to_int) with
      | Eio.Cancel.Cancelled _ as ex -> raise ex
      | _ -> None
    in
    let cost_usd =
      try Some (e.payload |> member "cost_usd" |> to_float) with
      | Eio.Cancel.Cancelled _ as ex -> raise ex
      | _ -> None
    in
    let latency_ms =
      try e.payload |> member "latency_ms" |> to_int with
      | Eio.Cancel.Cancelled _ as ex -> raise ex
      | _ -> 0
    in
    let model_used =
      try e.payload |> member "model_used" |> to_string with
      | Eio.Cancel.Cancelled _ as ex -> raise ex
      | _ -> "unknown"
    in
    let work_kind =
    ...
    in
    let context_ratio =
      try e.payload |> member "context_ratio" |> to_float with
      | Eio.Cancel.Cancelled _ as ex -> raise ex
      | _ -> 0.0
    in
    let tools_used =
    ...
    let optional_fields =
      let reasoning =
        try
          match e.payload |> member "reasoning_tokens" with
      ...
      let tps =
        try
          match e.payload |> member "tokens_per_second" with
      ...
      let prompt_tps =
        try
          match e.payload |> member "prompt_per_second" with
      ...
      let hw_decode_tps =
        try
          match e.payload |> member "hw_decode_tokens_per_second" with
...
let build_timeline
  ...
  let all_events =
    ...
    let task_evts = if include_tasks then task_events config ~agent_name else [] in
    let msg_evts = message_events config ~agent_name ~limit:200 in
    let tool_evts =
      if include_tool_calls then tool_call_events config ~agent_name ~limit:200 else []
    in
    let cdal_evts = keeper_cdal_events config ~agent_name ~limit:200 in
    let turn_evts = turn_completed_events config ~agent_name ~limit:200 in
  ...
  (* Compute summary *)
  let tasks_completed =
    List.length (List.filter (fun e -> String.equal e.event_type "task_completed") events)
  in
  let tasks_claimed =
    List.length (List.filter (fun e -> String.equal e.event_type "task_claimed") events)
  in
  let messages_sent =
    List.length (List.filter (fun e -> String.equal e.event_type "broadcast") events)
  in
  let tool_calls =
    List.length (List.filter (fun e -> String.equal e.event_type "tool_call") events)
  in
  let turn_events =
  ...
  let total_input_tokens =
    List.fold_left
      (fun acc e ->
         ...
         acc
         +
         try e.detail |> member "input_tokens" |> to_int with
         | Eio.Cancel.Cancelled _ as ex -> raise ex
         | _ -> 0)
      0
      turn_events
  ...
  let total_output_tokens =
    List.fold_left
      (fun acc e ->
         ...
         acc
         +
         try e.detail |> member "output_tokens" |> to_int with
         | Eio.Cancel.Cancelled _ as ex -> raise ex
         | _ -> 0)
      0
      turn_events
  ...
  let total_cost_usd =
    List.fold_left
      (fun acc e ->
         ...
         acc
         +.
         try e.detail |> member "cost_usd" |> to_float with
         | Eio.Cancel.Cancelled _ as ex -> raise ex
         | _ -> 0.0)
      0.0
      turn_events
  ...
  `Assoc
    ...
    ; "period", `Assoc [ "from", `String since_iso; "to", `String now_iso ]
...
let schemas : Types.tool_schema list =
  [ { name = "masc_agent_timeline"
    ; description =
        "Unified timeline of an agent's activity in the currently selected room across \
         tasks, messages, and joins."
    ; input_schema =
        `Assoc
          [ "type", `String "object"
          ; ( "properties"
            , `Assoc
                ...
                ; ( "since_hours"
                  , `Assoc
                      [ "type", `String "number"
                      ; "description", `String "Look back N hours (default: 24)"
                      ] )
                ; ( "limit"
                  , `Assoc
                      [ "type", `String "integer"
                ...
                ; ( "include_tasks"
                  , `Assoc
                      [ "type", `String "boolean"
                      ; ( "description"
                        , `String "Include task state changes (default: true)" )
                      ] )
                ; ( "include_board"
                  , `Assoc
                      [ "type", `String "boolean"
                      ; ( "description"
                        , `String "Include board activity (default: false, reserved)" )
                      ] )
                ; ( "include_tool_calls"
                  , `Assoc
                      [ "type", `String "boolean"
                      ; ( "description"
                        , `String
                            "Include tool call events from Activity Graph (default: true)"
                        )
                      ] )
                ] )
...
let handle_agent_timeline (ctx : context) args : tool_result =
  ...
  if String.length agent_name = 0
  then false, "agent_name is required"
  else (
