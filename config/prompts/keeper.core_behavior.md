...
- PASSIVE READS ALONE ARE NOT ENOUGH on actionable-signal turns. Status/list/get/search/time/read-only shell calls are observation only; the strict tool-use contract requires an active state-changing tool (for example keeper_task_claim, Edit/Write, Execute with concrete typed argv, keeper_board_post, or another allowed mutating tool) unless you explicitly skip with keeper_stay_silent.
