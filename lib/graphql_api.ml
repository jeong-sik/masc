...
type ctx = { room_config : Coord_utils.config }
...
let encode_cursor ~kind value = Base64.encode_string (kind ^ ":" ^ value)
...
let decode_cursor ~kind cursor =
  ...
  | Ok decoded ->
    ...
    then Some (String.sub decoded prefix_len (String.length decoded - prefix_len))
    else None
  | Error _ -> None
;;
  ...
  Schema.obj
    ...
    ~fields:
      ...
      ; Schema.field
          ...
          ~typ:(Schema.non_null Schema.string)
          ~args:Arg.[]
          ~resolve:(fun _ (agent : Types.agent) ->
  ...
  Schema.obj
    ...
    ~fields:
      [ Schema.field
          ...
          ~typ:(Schema.non_null Schema.string)
          ~args:Arg.[]
          ~resolve:(fun _ (message : Types.message) ->
            encode_cursor ~kind:"message" (string_of_int message.seq))
      ; Schema.field
          "seq"
          ~typ:(Schema.non_null Schema.int)
          ~args:Arg.[]
...
let rec const_value_of_yojson = function
  ...
  | `Floatlit s ->
    (match float_of_string_opt s with
     ...
     | None -> `String s)
  | `String s -> `String s
  | `Assoc fields -> `Assoc (List.map (fun (k, v) -> k, const_value_of_yojson v) fields)
  | `List items -> `List (List.map const_value_of_yojson items)
  | `Tuple items -> `List (List.map const_value_of_yojson items)
  | `Variant (name, None) -> `Enum name
...
let variables_of_yojson = function
  | None -> []
  | Some `Null -> []
  | Some (`Assoc fields) -> List.map (fun (k, v) -> k, const_value_of_yojson v) fields
  | Some _ -> []
;;
...
let tasks_connection config first after =
  let tasks : Types.task list =
    if Coord_utils.is_initialized config then Coord.get_tasks_raw config else []
  in
...
let agents_connection config first after =
  let agents : Types.agent list =
    if Coord_utils.is_initialized config then get_agents config else []
  in
...
let messages_connection config first after =
  let messages : Types.message list =
    if Coord_utils.is_initialized config then get_messages config else []
  in
  let after_seq =
...
let schema =
  Schema.schema
    ...
    ; Schema.field
        ...
        ~args:Arg.[ Arg.arg "first" ~typ:Arg.int; Arg.arg "after" ~typ:Arg.string ]
...
let handle_request ~config body_str =
  ...
  match json with
  | Error msg -> { status = `Bad_request; body = graphql_error msg }
  | Ok payload ->
    ...
    in
    (match query with
     | None -> { status = `Bad_request; body = graphql_error "Missing query field" }
     | Some query_str ->
       (match Graphql_parser.parse query_str with
        | Error err -> { status = `Bad_request; body = graphql_error err }
        | Ok doc ->
          let variables = variables_of_yojson variables_json in
          let ctx = { room_config = config } in
          let result =
            if variables = []
            then Schema.execute schema ctx ?operation_name doc
            else Schema.execute schema ctx ~variables ?operation_name doc
          in
          (match result with
           | Ok (`Response json) -> { status = `OK; body = Yojson.Basic.to_string json }
           | Ok (`Stream _) ->
             { status = `Bad_request
             ; body = graphql_error "Subscriptions are not supported"
             }
           | Error err_json -> { status = `OK; body = Yojson.Basic.to_string err_json })))
;;
