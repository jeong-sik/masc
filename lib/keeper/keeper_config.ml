...
let phase_routing_cascade_names = [ local_only_cascade_name; local_recovery_cascade_name ]
...
let () =
  ...
  if alert_error_detail_max_chars <= 0
  then invalid_arg "Keeper_config alert_error_detail_max_chars must be positive";
  if tool_policy_count_warn_threshold <= 0
  then invalid_arg "Keeper_config tool_policy_count_warn_threshold must be positive";
  if tool_first_sentence_max_chars <= 0
  then invalid_arg "Keeper_config tool_first_sentence_max_chars must be positive"
;;
...
let bool_of_env_default name ~(default : bool) =
  ...
  | Some raw ->
    ...
    then true
    else if v = "0" || v = "false" || v = "no" || v = "n" || v = "off"
    then false
    else default
;;
...
let non_public_keeper_input_key_names = [ "social_model" ]
...
let present_json_keys (keys : string list) (json : Yojson.Safe.t) : string list =
  match json with
  | `Assoc fields -> keys |> List.filter (fun key -> List.mem_assoc key fields)
  | _ -> []
;;
...
let reject_removed_keeper_input_keys ~tool_name (args : Yojson.Safe.t) =
  ...
  (match non_public with
   | _ :: _ as fields ->
     Log.Keeper.warn
       "%s: ignoring non-public keeper args %s (see #7447, #9752 — accepted for \
        external-client compatibility, no runtime effect)"
       tool_name
...
let reject_removed_keeper_msg_input_keys ~tool_name (args : Yojson.Safe.t) =
  ...
  | fields ->
    Error
      (Printf.sprintf
         "removed keeper message args for %s: %s. Use masc_keeper_up for keeper creation \
          or persisted updates."
         tool_name
         (String.concat ", " fields))
;;
...
let normalize_goal_horizon_text ?(max_len = default_goal_horizon_max_chars) (raw : string)
  : string
  =
  let s = String.trim raw in
  if s = "" then "" else utf8_safe_prefix_bytes s ~max_bytes:max_len
;;
...
let resolve_goal_horizons
  ...
  =
  let short_goal =
    Option.value ~default:goal short_goal_opt |> normalize_goal_horizon_text
  in
  let mid_goal = Option.value ~default:goal mid_goal_opt |> normalize_goal_horizon_text in
  let long_goal =
    Option.value ~default:goal long_goal_opt |> normalize_goal_horizon_text
  in
...
let parse_self_model_opt args key : string option =
  ...
  | None -> None
  | Some raw -> Some (normalize_self_model_text ~max_bytes:prompt_render_max_bytes raw)
;;

let clamp_int v ~min_v ~max_v = max min_v (min max_v v)
...
let float_of_env_default name ~default ~min_v ~max_v =
  ...
  | None -> default
  | Some raw ->
    let v = Option.value ~default (float_of_string_opt (String.trim raw)) in
...
let _rp_deser_int json =
  ...
  | `Float f ->
    ...
    if Float.equal (Float.of_int i) f
    then Ok i
...
let _rp_int ~key ~default ~min_v ~max_v ~description () =
  Runtime_params.register
    ...
    ~meta:
      { Runtime_params.description
      ; value_type = "int"
      ; min_value = Some (`Int min_v)
      ; max_value = Some (`Int max_v)
      }
...
let _rp_float ~key ~default ~min_v ~max_v ~description () =
  Runtime_params.register
    ...
    ~meta:
      { Runtime_params.description
      ; value_type = "float"
      ; min_value = Some (`Float min_v)
      ; max_value = Some (`Float max_v)
      }
...
let keeper_compact_ratio_rp =
  _rp_float
    ~key:"keeper.compaction.ratio"
    ~default:(fun () ->
      float_of_env_default "MASC_KEEPER_COMPACT_RATIO" ~default:0.5 ~min_v:0.1 ~max_v:0.98)
    ~min_v:0.1
    ~max_v:0.98
...
let keeper_compact_ratio () : float = Runtime_params.get keeper_compact_ratio_rp

let keeper_compact_max_messages_rp =
  _rp_int
    ~key:"keeper.compaction.max_messages"
    ~default:(fun () ->
      int_of_env_default
...
let keeper_compact_max_tokens_rp =
  _rp_int
    ~key:"keeper.compaction.max_tokens"
    ~default:(fun () ->
      int_of_env_default
...
let keeper_compact_max_tokens () : int = Runtime_params.get keeper_compact_max_tokens_rp
...
let keeper_continuity_compaction_cooldown_sec_rp =
  _rp_int
    ~key:"keeper.compaction.cooldown_sec"
    ~default:(fun () ->
      int_of_env_default
...
let keeper_bootstrap_proactive_warmup_sec_rp =
  _rp_int
    ~key:"keeper.proactive.warmup_sec"
    ~default:(fun () ->
      int_of_env_default
...
let keeper_bootstrap_stagger_step_sec_rp =
  _rp_int
    ~key:"keeper.proactive.stagger_step_sec"
    ~default:(fun () ->
      int_of_env_default
...
let keeper_bootstrap_retry_max_rp =
  _rp_int
    ~key:"keeper.bootstrap.retry_max"
    ~default:(fun () ->
      int_of_env_default "MASC_KEEPER_BOOTSTRAP_RETRY_MAX" ~default:5 ~min_v:0 ~max_v:20)
    ~min_v:0
    ~max_v:20
...
let keeper_bootstrap_retry_max () : int = Runtime_params.get keeper_bootstrap_retry_max_rp

let keeper_bootstrap_retry_interval_sec_rp =
  _rp_int
    ~key:"keeper.bootstrap.retry_interval_sec"
    ~default:(fun () ->
      int_of_env_default
        "MASC_KEEPER_BOOTSTRAP_RETRY_INTERVAL_SEC"
        ~default:30
        ~min_v:5
        ~max_v:300)
    ~min_v:5
...
let keeper_proactive_min_cooldown_sec_rp =
  _rp_int
    ~key:"keeper.proactive.min_cooldown_sec"
    ~default:(fun () ->
      int_of_env_default
...
let keeper_proactive_task_cooldown_divisor_rp =
  _rp_int
    ~key:"keeper.proactive.task_cooldown_divisor"
    ~default:(fun () ->
      int_of_env_default
...
let keeper_proactive_task_min_cooldown_sec_rp =
  _rp_int
    ~key:"keeper.proactive.task_min_cooldown_sec"
    ~default:(fun () ->
      int_of_env_default
...
let keeper_compaction_policy_from_env () : float * int * int =
  keeper_compact_ratio (), keeper_compact_max_messages (), keeper_compact_max_tokens ()
;;

let normalize_compaction_ratio_gate (v : float) : float = max 0.1 (min 0.98 v)
let normalize_compaction_message_gate (v : int) : int = clamp_int v ~min_v:0 ~max_v:5000
let normalize_compaction_token_gate (v : int) : int = clamp_int v ~min_v:0 ~max_v:5000000
  ...
  | Some raw ->
    (match canonical_compaction_profile raw with
     ...
     | None ->
       Error
         (Printf.sprintf
            "invalid compaction_profile '%s' (allowed: aggressive, balanced, \
             conservative, custom)"
            raw))
;;
...
let compaction_policy_of_profile (profile : string) : float * int * int =
  match
    canonical_compaction_profile profile
...
let resolve_compaction_policy
  ...
  let base_profile =
    ...
    | Some p -> p
    | None -> if has_explicit_gate then "custom" else fallback_profile
  in
  let base_ratio, base_message, base_token =
    ...
    | None ->
      ...
      then fallback_ratio, fallback_message, fallback_token
      else fallback_ratio, fallback_message, fallback_token
  in
  let ratio =
    Option.value ~default:base_ratio ratio_opt |> normalize_compaction_ratio_gate
  in
  let message_gate =
    Option.value ~default:base_message message_opt |> normalize_compaction_message_gate
  in
  let token_gate =
    Option.value ~default:base_token token_opt |> normalize_compaction_token_gate
  in
  base_profile, ratio, message_gate, token_gate
;;

let normalize_proactive_idle_sec (v : int) : int = clamp_int v ~min_v:0 ~max_v:172800
let normalize_proactive_cooldown_sec (v : int) : int = clamp_int v ~min_v:0 ~max_v:172800

let keeper_batch_limit_rp =
  _rp_int
    ~key:"keeper.turn.batch_limit"
    ~default:(fun () ->
      int_of_env_default "MASC_KEEPER_BATCH_LIMIT" ~default:200 ~min_v:10 ~max_v:2000)
    ~min_v:10
    ~max_v:2000
...
let keeper_batch_limit () : int = Runtime_params.get keeper_batch_limit_rp

let keeper_tool_cost_max_usd_rp =
  _rp_float
    ~key:"keeper.turn.tool_cost_max_usd"
    ~default:(fun () ->
...
let keeper_max_tools_per_turn_rp =
  _rp_int
    ~key:"keeper.turn.max_tools_per_turn"
    ~default:(fun () ->
      int_of_env_default "MASC_KEEPER_MAX_TOOLS_PER_TURN" ~default:40 ~min_v:5 ~max_v:200)
    ~min_v:5
    ~max_v:200
...
let keeper_max_tools_per_turn () : int = Runtime_params.get keeper_max_tools_per_turn_rp
let keeper_retry_max_tools_per_turn () : int = min 15 (keeper_max_tools_per_turn ())

let keeper_board_event_limit_rp =
  _rp_int
    ~key:"keeper.turn.board_event_limit"
    ~default:(fun () ->
      int_of_env_default "MASC_KEEPER_BOARD_EVENT_LIMIT" ~default:10 ~min_v:1 ~max_v:50)
    ~min_v:1
    ~max_v:50
...
let keeper_board_event_limit () : int = Runtime_params.get keeper_board_event_limit_rp

let keeper_llm_rerank_enabled_rp =
  _rp_bool
    ~key:"keeper.turn.llm_rerank"
...
let keeper_llm_rerank_enabled () : bool = Runtime_params.get keeper_llm_rerank_enabled_rp
...
let keeper_rule_reflect_repetition_rp =
  _rp_float
    ~key:"keeper.rule.reflect_repetition"
    ~default:(fun () ->
      float_of_env_default
...
let keeper_rule_plan_goal_alignment_rp =
  _rp_float
    ~key:"keeper.rule.plan_goal_alignment_max"
    ~default:(fun () ->
      float_of_env_default
...
let keeper_rule_plan_response_alignment_rp =
  _rp_float
    ~key:"keeper.rule.plan_response_alignment_max"
    ~default:(fun () ->
      float_of_env_default
...
let keeper_rule_guardrail_repetition_rp =
  _rp_float
    ~key:"keeper.rule.guardrail_repetition"
    ~default:(fun () ->
      float_of_env_default
...
let keeper_rule_guardrail_goal_alignment_rp =
  _rp_float
    ~key:"keeper.rule.guardrail_goal_alignment_max"
    ~default:(fun () ->
      float_of_env_default
...
let keeper_rule_guardrail_response_alignment_rp =
  _rp_float
    ~key:"keeper.rule.guardrail_response_alignment_max"
    ~default:(fun () ->
      float_of_env_default
...
let keeper_rule_guardrail_context_rp =
  _rp_float
    ~key:"keeper.rule.guardrail_context_min"
    ~default:(fun () ->
      float_of_env_default
...
let keeper_unified_temperature_rp =
  _rp_float
    ~key:"keeper.turn.temperature"
    ~default:(fun () ->
      float_of_env_default "MASC_KEEPER_UNIFIED_TEMP" ~default:0.4 ~min_v:0.0 ~max_v:2.0)
    ~min_v:0.0
    ~max_v:2.0
...
let keeper_unified_max_tokens_rp =
  _rp_int
    ~key:"keeper.turn.max_output_tokens"
    ~default:(fun () ->
      int_of_env_default
    ...
    ~max_v:262144
    ~description:
      "Keeper turn max output tokens fallback (cascade.json overrides to 16384 in \
       production)"
    ()
;;

let keeper_unified_max_tokens () : int = Runtime_params.get keeper_unified_max_tokens_rp

let keeper_tool_search_top_k_rp =
  _rp_int
    ~key:"keeper.turn.tool_search_top_k"
    ~default:(fun () -> 20)
    ~min_v:3
    ~max_v:50
...
let keeper_tool_search_top_k () : int = Runtime_params.get keeper_tool_search_top_k_rp
...
let keeper_llama_slots_rp =
  _rp_int
    ~key:"keeper.turn.llama_slots"
    ~default:(fun () ->
      int_of_env_default "MASC_KEEPER_LLAMA_SLOTS" ~default:4 ~min_v:0 ~max_v:32)
    ~min_v:0
    ~max_v:32
...
let keeper_llama_slots () : int = Runtime_params.get keeper_llama_slots_rp
...
let keeper_enable_thinking_rp =
  _rp_bool
    ~key:"keeper.turn.enable_thinking"
    ~default:(fun () -> bool_of_env_default "MASC_KEEPER_ENABLE_THINKING" ~default:false)
    ~description:
      "Pass enable_thinking to OAS (default: false; Ollama+Qwen3.5 consumes all tokens \
       in thinking mode)"
    ()
;;

let keeper_enable_thinking () : bool = Runtime_params.get keeper_enable_thinking_rp

let keeper_adaptive_thinking_enabled_rp =
  _rp_bool
    ~key:"keeper.turn.adaptive_thinking"
    ~default:(fun () ->
      bool_of_env_default "MASC_KEEPER_ADAPTIVE_THINKING" ~default:false)
...
let keeper_adaptive_thinking_mode_rp =
  _rp_bool
    ~key:"keeper.turn.adaptive_thinking_mode"
    ~default:(fun () ->
      bool_of_env_default "MASC_KEEPER_ADAPTIVE_THINKING_MODE" ~default:true)
    ~description:
      "Enable per-turn boolean enable_thinking override driven by Keeper_turn_intent \
       classification (Mechanical → false, Cognitive → true). Independent of \
       adaptive_thinking budget flag. Default: on (set \
       MASC_KEEPER_ADAPTIVE_THINKING_MODE=false to opt out)."
    ()
;;
