...
let cached_text_by_key cache ~key ~ttl_s compute =
  ...
  | Some cached_key, Some value when String.equal cached_key key && now < cache.expires_at
    -> value
  | _ ->
...
let credential_state (ctx : context) ~actual_name =
  ...
  let internal_keeper_credential_available name =
    ...
    | Some _, Some raw ->
      ...
      token <> "" && Auth.verify_internal_keeper_token ctx.config.base_path ~token
    | _ -> false
  in
...
let safe_is_zombie_agent ~agent_name last_seen =
  try Coord.is_zombie_agent ~agent_name last_seen with
  | Sys_error _ | Yojson.Json_error _ -> false
  | exn ->
    Log.Coord.warn "is_zombie_agent failed for %s: %s" agent_name (Printexc.to_string exn);
    false
;;

let todo_task_has_completed_deliverable_conflict (ctx : context) (task : Types.task) =
       ...
       let deliverable_conflict_task =
         match
           List.find_opt
             ...
             active_tasks
         with
         | Some { task_status = Types.Claimed _ | Types.InProgress _; _ }
...
let coordination_fsm_attention_items ctx =
  ...
  with
  | exn ->
    Log.Coord.warn "coordination FSM status advisory failed: %s" (Printexc.to_string exn);
    []
;;
  ...
  let agents_with_state =
    List.map
      (fun (agent : Types.agent) ->
         Coord_query.safe_yield ();
         let is_zombie = safe_is_zombie_agent ~agent_name:agent.name agent.last_seen in
         agent, is_zombie)
      agents
  in
  let zombie_count =
      ...
      then
         ...
         | Some task_id ->
           ...
           @ [ Printf.sprintf
                 "Owned task %s has no planning context. Do not retry generic \
                  masc_plan_init from a drifted surface; use handoff/worktree/test logs \
                  as the temporary SSOT until a credentialed owner repair receipt \
                  exists."
                 task_id
             ]
         | None -> items)
        ...
        (match planning_state.deliverable_conflict_task with
         | Some task_id ->
           items
           @ [ Printf.sprintf
                 "Owned task %s already has a completed-looking deliverable while the \
                  task is still active. Treat this as conflict triage until board, \
                  planning, and control-plane state converge."
                 task_id
             ]
         | None -> items)
        |> fun items ->
        ...
        then
          ...
          (match binding.drift_reason with
           | Some "secondary_assignment" ->
             ...
             @ [ "Multiple assigned tasks detected. Current focus is also assigned; \
                  choose or reconcile the active lane before claiming new work."
               ]
           | Some "stale_focus" ->
             items
             @ [ "Owned/current drift detected. Planning current_task is not assigned to \
                  you; treat primary_owned as the safe task lane."
               ]
           | Some "no_owned" ->
             items
             @ [ "Planning current_task is set but no active task is assigned to you; \
                  clear or rebind current_task before following it."
               ]
           | Some _ | None -> items)
          |> fun items ->
          ...
          then
            items
            @ [ Printf.sprintf
                  "%d todo task(s) have completed-looking planning deliverables; treat \
                   them as control-plane conflicts, not fresh claimable work."
                  todo_conflict_count
              ]
          else
            items
            |> fun items ->
            items @ coordination_fsm_attention_items ctx
            |> fun items ->
            if zombie_count > 0
            then
              items
              ...
              items
              |> fun items ->
              if fresh_todo_count > 0 && binding.assigned_task_ids = []
              then
                items
                @ [ Printf.sprintf
                      "%d unclaimed task(s) are available right now."
                      fresh_todo_count
                  ]
              else items
  in
...
let handle_reset ctx args =
  ...
  then
    ( false
    , "⚠️ This will DELETE the entire .masc/ folder!\nCall with confirm=true to proceed." )
  else (
...
let handle_coordination_fsm_snapshot ctx _args =
  true, Yojson.Safe.to_string (Coordination_product_snapshot.safe_build_yojson ctx.config)
;;
...
let dispatch ctx ~name ~args : tool_result option =
  ...
  | "masc_coordination_fsm_snapshot" -> Some (handle_coordination_fsm_snapshot ctx args)
...
let tool_required_permission = function
  ...
  | "masc_goal_list" -> Some Types.CanReadState
  | "masc_goal_upsert" | "masc_goal_review" | "masc_goal_transition" | "masc_goal_verify"
    -> Some Types.CanBroadcast
  | "masc_heartbeat" -> Some Types.CanBroadcast
  | "masc_reset" -> Some Types.CanReset
  | _ -> None
;;
