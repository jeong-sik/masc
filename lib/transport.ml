...
module JsonRpc = struct
  ...
  let parse_request (json : Yojson.Safe.t) : (request, string) result =
    ...
    try
      ...
      if jsonrpc <> version
      then Error (Printf.sprintf "Invalid JSON-RPC version: %s" jsonrpc)
      else (
  ...
  let serialize_response (resp : response) : Yojson.Safe.t =
    ...
    then
      ...
      let error_obj =
        ...
        | None ->
          `Assoc
            [ "code", `Int ErrorCodes.internal_error; "message", `String "Unknown error" ]
      in
  ...
  let make_request ?id ~method_name ~params () : Yojson.Safe.t =
    let base =
      [ "jsonrpc", `String version; "method", `String method_name; "params", params ]
    in
    let with_id =
      match id with
...
module Rest = struct
  ...
  let method_json_key method_ = String.lowercase_ascii (method_to_string method_)
  ...
  let auth_mode_description = function
    | Public -> "No bearer token is required for this route."
    | Conditional_bearer ->
      "Bearer token auth is required when room auth/token enforcement is active; \
       loopback-local development may allow access without a bearer."
    | Same_origin_or_bearer ->
      "Loopback browser requests may use same-origin checks; non-browser clients should \
       use Authorization: Bearer <token>."
    | Bearer_required -> "Authorization: Bearer <token> is required for this route."
  ;;

  let openapi_bearer_security = `List [ `Assoc [ "bearerAuth", `List [] ] ]
  ...
  let auth_mode_of_operation_opt = function
    ...
    | "masc_broadcast" | "masc_operator_action" | "masc_operator_confirm" ->
      Some Bearer_required
    | "masc_status"
    | "masc_tasks"
    | "masc_who"
    | "masc_messages"
  ...
  let auth_mode_of_operation name =
    ...
    | None ->
      Log.Transport.warn
        "auth_mode_of_operation: %S not in catalog → Conditional_bearer fallback (#8687)"
        name;
      Conditional_bearer
  ;;
  ...
  let same_rest_route ~http_method ~path (method_, candidate_path) =
    String.equal
      (String.uppercase_ascii (String.trim http_method))
  ...
  let operation_of_actual_rest_route ~http_method ~path =
    operation_rest_bindings
    |> List.find_map (fun (operation, bindings) ->
      if List.exists (same_rest_route ~http_method ~path) bindings
      then Some operation
      else None)
  ;;
  ...
  let auth_response_entries mode =
    ...
    | Same_origin_or_bearer | Conditional_bearer | Bearer_required ->
      ...
      ; "403", `Assoc [ "description", `String "Authenticated caller lacks permission" ]
      ]
  ;;
  ...
  let operation_tag_groups =
    ...
    ; ( "messaging"
      , [ "masc_broadcast"; "masc_messages"; "masc_a2a_delegate"; "masc_a2a_subscribe" ] )
    ; ( "decision"
      , [ "decision_create"
        ; "decision_finalize"
        ; "decision_status"
  ...
  let tags_for_operation name =
    ...
    |> List.find_map (fun (tag, operations) ->
      if List.exists (String.equal name) operations then Some [ tag ] else None)
  ...
  let operation_catalog_entry name (schema : Types.tool_schema) =
    ...
    let rest_bindings =
      ...
      |> List.map (fun (method_, path) ->
        `Assoc [ "method", `String (method_to_string method_); "path", `String path ])
    in
    `Assoc
      [ "name", `String name
  ...
  let mcp_request_schema () =
    `Assoc
      ...
      ; ( "properties"
        , `Assoc
            [ "jsonrpc", `Assoc [ "type", `String "string"; "const", `String "2.0" ]
            ; "method", `Assoc [ "type", `String "string"; "const", `String "tools/call" ]
            ; ( "params"
              , `Assoc
                  [ "type", `String "object"
                  ; ( "properties"
      ...
      ; ( "required"
        , `List [ `String "jsonrpc"; `String "method"; `String "params"; `String "id" ] )
      ]
  ;;
  ...
  let rest_operation_json name method_ (schema : Types.tool_schema) =
    ...
    let request_fields =
      ...
      | GET | DELETE ->
        ...
        if params = [] then base_fields else ("parameters", `List params) :: base_fields
      | POST | PUT | PATCH ->
        ( "requestBody"
        , `Assoc
            [ "required", `Bool true
            ; ( "content"
              , `Assoc [ "application/json", `Assoc [ "schema", schema.input_schema ] ] )
            ] )
        :: base_fields
    in
    `Assoc
              ...
              ([ ( "200"
                 , `Assoc
                     ...
                     ; ( "content"
                       , `Assoc
                           [ ( "application/json"
                             , `Assoc [ "schema", success_response_schema ] )
                           ] )
                     ] )
               ]
  ...
  let generate_openapi_document
    ...
    in
    let sdk_tools =
      List.map Sdk_tool_contract.sdk_alias_json Sdk_tool_contract.sdk_bindings
    in
    ...
    let add_path_method path method_key operation_json =
      let existing = Hashtbl.find_opt path_table path |> Option.value ~default:[] in
    ...
    add_path_method
      ...
      (`Assoc
          ...
          ; ( "description"
            , `String
                "Primary control transport for internal agents. The canonical operation \
                 catalog is exposed through x-mcp-operations." )
          ; ( "requestBody"
            , `Assoc
                [ "required", `Bool true
                ; ( "content"
                  , `Assoc
                      [ "application/json", `Assoc [ "schema", mcp_request_schema () ] ] )
                ] )
          ; ( "responses"
            , `Assoc
                ([ ( "200"
    ...
    `Assoc
      ...
      ; ( "info"
        , `Assoc
            ...
            ; ( "description"
              , `String
                  "Internal OAS export for MASC MCP agent control. Use x-mcp-operations \
                   for canonical MCP operation metadata and x-agent-sdk-tools for the \
                   current SDK-facing aliases." )
            ] )
      ; "servers", `List [ `Assoc [ "url", `String server_url ] ]
              ...
              , `Assoc
                  [ ( "bearerAuth"
                    , `Assoc
                        ...
                        ; ( "description"
                          , `String
                              "MASC room bearer token. Some loopback-local routes may \
                               additionally permit same-origin browser access." )
                        ] )
                  ] )
            ] )
      ]
  ...
  let parse_request ~http_method ~path ~query_params ~body : request =
    let method_name =
      ...
      | None ->
        (match http_method, path with
         | _, p when String.starts_with ~prefix:"/api/v1/tools/" p && String.length p > 14
    ...
    let params =
      ...
      | "" -> `Assoc query_params
      | s ->
        (match Safe_ops.parse_json_safe ~context:"http_transport" s with
         | Ok json -> json
         | Error _ -> `Assoc query_params)
    in
...
let get_bindings ~host ~port : binding list =
  ...
  let bindings =
    ...
    then
      bindings
      @ [ { protocol = Grpc
          ; url = Printf.sprintf "grpc://%s:%d" host (Masc_grpc_server.configured_port ())
  ...
  let bindings =
    ...
    then
      ...
      @ [ { protocol = Ws
          ; url =
              Printf.sprintf "ws://%s:%d/" host (Server_ws_standalone.configured_port ())
  ...
  then
    ...
    @ [ { protocol = Webrtc
        ...
        ; options = [ "offer_path", "/webrtc/offer"; "answer_path", "/webrtc/answer" ]
        }
      ]
  else bindings
;;
...
let bindings_to_json (bindings : binding list) : Yojson.Safe.t =
  `List
    (List.map
       (fun b ->
          `Assoc
            [ "protocol", `String (protocol_to_string b.protocol); "url", `String b.url ])
       bindings)
;;
...
module Stats = struct
  ...
  let record_request ~success ~latency_ms =
    ...
    if success then Atomic.incr successful_requests else Atomic.incr failed_requests;
