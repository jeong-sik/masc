...
(async()=>{
 ...
 for(const [name,path] of [['document-change','/first'],['same-url-reload','/first'],['redirect','/redirect'],['hash','/final#hash'],['destination-error','/broken']]) {
  ...
  const onUpdated=(id,change,updated)=>{if(id===tab.id){events.push({kind:'tab-updated',change,url:updated.url,status:updated.status});if(destinationError && change.status==='complete')resolveReady(destinationError);}};
  ...
  try {
   ...
   if(name==='destination-error'){if(event.url!==base+'/broken' || commit!==null || !events.some(e=>e.kind==='before-navigate' && e.details.url===base+'/broken'))throw new Error('destination error not established');const finalFrame=await browser.webNavigation.getFrame({tabId:tab.id,frameId:0});const finalTab=await browser.tabs.get(tab.id);let errorPageRead;try{errorPageRead={result:(await browser.tabs.executeScript(tab.id,{code:'('+sceneSource+')({mode:"read",view:"content",maxChars:20000})',runAt:'document_end'}))[0]};}catch(error){errorPageRead={error:String(error)};}await report({kind:'error-page-settlement',finalFrame,finalTab,errorPageRead});await report({kind:'case',name,effects_requested,sourceFrame,lifecycle:events,error_policy:'diagnostic_error_only'});continue;}
