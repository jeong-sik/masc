...
| --- | --- | --- | --- | --- | --- |
| DeepSeek | OpenAI chat | `thinking.type`, on by default | `none`, `low`, `high`, `max` (default `high`); the reference and the guide disagree on where the field sits | `reasoning_content`, same in stream deltas | required for every earlier turn **when the request carries `tools`**, ignored when it does not |
