...
let configured_read_path env_name ~default ~legacy =
  ...
  | None ->
    ...
    if Sys.file_exists default_abs
    then default_abs
    else (
...
let binding_store_path () =
  configured_write_path
    "MASC_IMESSAGE_BINDING_STORE_PATH"
...
let binding_audit_path () =
  configured_write_path
    "MASC_IMESSAGE_BINDING_AUDIT_PATH"
...
let status_json ?(audit_limit = 10) () =
  ...
  `Assoc
    ...
    ; ( "self_chat_guid"
      , `String (redact_chat_guid (status_field "self_chat_guid" string_member "")) )
    ...
    ; ( "poll_interval_sec"
      , `Float
          (status_field
             "poll_interval_sec"
             (fun j k ->
                j |> U.member k |> U.to_float_option |> Option.value ~default:2.0)
             2.0) )
