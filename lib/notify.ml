...
let run_argv_ignore argv =
  try
    ...
    | _ ->
      Log.Misc.warn
        "notify command exited with status %s: %s"
...
let is_macos () =
  try
    ...
    os = "Darwin"
  with
  | End_of_file | Unix.Unix_error _ -> false
;;
...
let build_focus_command payload =
  ...
  | None ->
    ...
    if focus_app = None && tmux_session = None
    then None
    else (
      let parts = ref [] in
      ...
      (match tmux_session with
       | Some session ->
         ...
         then (
           let select_window =
             Printf.sprintf
               "command -v tmux >/dev/null 2>&1 && tmux select-window -t '%s' >/dev/null \
                2>&1 || true"
           ...
           then (
             let select_pane =
               Printf.sprintf
                 "command -v tmux >/dev/null 2>&1 && tmux select-pane -t '%s.0' \
                  >/dev/null 2>&1 || true"
                 (escape_shell window_target)
             in
...
let agent_emoji_table : (string, string) Hashtbl.t =
  ...
  List.iter
    (fun (k, v) -> Hashtbl.replace t k v)
    [ "claude", "🟣"; "gemini", "🔵"; "codex", "🟢"; "llama", "🦙"; "system", "⚙️" ];
  t
;;
...
let register_agent_emoji name emoji = Hashtbl.replace agent_emoji_table name emoji
...
let send_notification ?(sound = false) ?focus_cmd ~title ~subtitle ~message () =
  ...
  then
    ...
    ()
  else if Eio.Lazy.force has_terminal_notifier
  then send_via_terminal_notifier ~title ~subtitle ~message ~sound ~focus_cmd
  else (
    send_via_osascript ~title ~subtitle ~message;
    try
      let _ = focus_on_osascript () in
      ()
    with
    | Eio.Cancel.Cancelled _ as e -> raise e
    | exn -> Log.Misc.error "focus_on_osascript failed: %s" (Printexc.to_string exn)
...
let notify event =
  ...
  | Interrupt { agent; action } ->
    let focus_cmd =
      build_focus_command
        { target_agent = Some agent; from_agent = Some agent; task_id = None }
    in
    send_notification
  ...
  | TaskCompleted { agent; task_id } ->
    ...
    let focus_cmd =
      build_focus_command
        { target_agent = Some agent; from_agent = Some agent; task_id = Some task_id }
    in
    send_notification
      ~title:(Printf.sprintf "%s MASC" emoji)
  ...
  | Custom { title; subtitle; message } ->
    let focus_cmd =
      build_focus_command { target_agent = None; from_agent = None; task_id = None }
    in
...
let notify_task_done ~agent ~task_id = notify (TaskCompleted { agent; task_id })
