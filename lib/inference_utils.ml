...
let sanitize_text_utf8 (s : string) : string =
  ...
  let rec has_invalid_or_control i =
    ...
    then false
      ...
      then
        if dlen = 1 && is_disallowed_control_char s.[i]
        then true
        else has_invalid_or_control (i + dlen)
      else true)
  in
  if not (has_invalid_or_control 0)
  then s
  else (
    ...
    let rec loop i =
      ...
      then ()
        ...
        then (
          ...
          else Buffer.add_substring buf s i dlen;
          loop (i + dlen))
        else (
...
let rec sanitize_content_blocks_utf8 (blocks : Oas.Types.content_block list)
  ...
  | block :: rest ->
    let sanitized_block =
      ...
      | Oas.Types.ToolResult { tool_use_id; content; is_error; json } ->
        ...
        in
        if
          sanitized_tool_use_id == tool_use_id
    ...
    if sanitized_block == block && sanitized_rest == rest
    then blocks
...
let sanitize_message_utf8 (m : Oas.Types.message) : Oas.Types.message =
  ...
  if sanitized_content == m.content then m else { m with content = sanitized_content }
;;
...
let sanitize_messages_utf8 (msgs : Oas.Types.message list) : Oas.Types.message list =
  let rec loop messages =
    ...
    | msg :: rest ->
      ...
      if sanitized_msg == msg && sanitized_rest == rest
      then messages
