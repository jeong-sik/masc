...
let create_state_from_checkpoint config ~session_id ~base_dir =
  let checkpoint_path = Filename.concat base_dir (session_id ^ ".json") in
  if not (Fs_compat.file_exists checkpoint_path) then begin
    eprintf "[perpetual] No checkpoint found for %s, starting fresh\n%!" session_id;
    create_state config
  end else
    match Agent_sdk.Checkpoint.of_string (Fs_compat.load_file checkpoint_path) with
    | Error _ ->
      eprintf "[perpetual] Checkpoint load failed for %s, starting fresh\n%!" session_id;
      create_state config
    | Ok ckpt ->
