...
title: "도구 스키마를 매 턴 전량 싣는 것을 그만둔다"
status: Accepted
created: 2026-08-29
updated: 2026-08-31
author: vincent
...
*2026-08-31 갱신:* 부착 축을 닫고 나니 남은 90개의 89개가 내장이었고, 그중 33개는
하루 종일 한 번도 안 불렸다. 그래서 이 문서는 부착 서비스만의 축이 아니게 됐다 —
**지연 여부는 출처가 아니라 도구의 성질이다**(§4.6). 실측은 §5.2.
...
1. **B 로 간다. A 는 하지 않는다.** A 의 선언 축은 B 가 오면 지워야 하는 죽은 축이고,
   ...
   *2026-08-31: 이 결정은 유지된다.* §4.6 이 도입한 `defer_loading` 은 A 가 아니다.
   A 는 **keeper 가 provider 를 고르는** 명부고, 이쪽은 **도구가 자기 파일에서
   자기를 말하는** 선언이다. 사람이 keeper 마다 유지할 목록이 없고, 읽는 쪽도
   목록을 갖지 않는다. 명부가 되는 순간 그건 A 이자 RFC-0389 다.
...
4. ~~**내장 도구는 B 로 가지 않는다.**~~ **2026-08-31 에 뒤집혔다.** 아래 §4.6 참조.

   원문: 같은 날 로그에 남은 호출 29종이 전부 내장이다. 실사용률이 높은 축을 색인
   뒤로 감추면 왕복만 늘고 아끼는 바이트는 없다. 내장 57KB 는 상시 적재로 둔다. 이
   결정은 §5 의 중앙값이 목표에 못 미치면 다시 연다.

   뒤집은 근거: "호출 29종이 전부 내장" 은 내장을 상시로 둘 이유로 읽혔지만, 뒤집어
   읽으면 **내장 89개 중 60개는 한 번도 안 불렸다** 는 뜻이다. 어느 내장이 그 29종인지
   묻지 않고 축 전체를 상시로 둔 것이 이 결정의 실수다.
...
## 4.6 결정 4 를 다시 연다 — 2026-08-31

§4.5 의 세 단계가 다 들어간 뒤 남은 표면은 keeper 당 **90개**였다. 내장 89 + 목록 1.
결정 4 가 그 89를 통째로 상시 적재로 두기로 한 결과다.

`tool_calls` 로 그 89를 다시 쟀다.

| | |
|---|---|
| 상위 10종이 차지하는 호출 비중 | **78.5%** |
| 상위 20종 | 93.6% |
| **하루 종일 한 번도 안 불린 내장** | **33개 / 21,601B** |
| **2026-08-29·30 이틀 연속 미호출** | **13개 / 8,299B** |

한 턴이 provider 요청 83건이므로 21,601B 는 턴당 1.8MB 다.

### 무엇이 문제였나

결정 4 는 "부착이면 지연, 내장이면 상시" 를 만들었다. **도구 하나를 보는 자리가 없다.**
이건 RFC-0389 툴그룹과 같은 모양이고 묶는 축만 다르다 — 저쪽은 keeper 로 묶었고
PR #31728 이 지웠다(살아있는 keeper 9개 중 선언 0개). 출처로 묶어도 **도구 자신이
자기 비용에 대해 말하지 않는다** 는 문제는 그대로다.

바깥은 전부 도구 단위다: Anthropic API 의 `defer_loading`, Claude Code 의
`shouldDefer`/`alwaysLoad`, Hermes 의 도구별 적격 판정. Claude Code 는 MCP 서버가
도구마다 `_meta['anthropic/alwaysLoad']` 로 클라이언트 기본값을 뒤집게까지 한다.

### 대신 무엇을 하나

지연 여부는 **도구의 성질**이다. 각 도구가 자기 `config/tools/<name>.toml` 에
`defer_loading = true` 를 선언하고, 없으면 상시 적재다(#31911). 번들은 출처가 아니라
그 선언으로 가른다(#31985). 목록 하나가 부착·내장을 다 담고 **어느 출처인지 기록하지
않는다.**

읽는 쪽(`Tool_loading_declarations`)은 **자기 목록을 갖지 않는다.** 목록을 두면 무언가
그것을 입력으로 쓰는 순간 명부가 되고, 그게 툴그룹이다. 같은 이유로
`deferrable_tool_names ()` 는 소비자 0 인 채로 지웠다.

파싱 실패는 삼키지 않는다. TOML 은 `[[params]]` 뒤에 놓인 최상위 키를 그 테이블의
키로 만들어서, 위치를 잘못 쓰면 "선언 안 함" 과 구분되지 않는다. 파일 이름과 함께
raise 한다.

### 여러 도구를 한 번에 부르는 일

그룹이 필요해 보이는 자리는 이미 합성 스킬이다. `[[compositions.nodes]]` 가 도구를
이름으로 지명하고 `after` 로 순서를, JSON 포인터로 노드 간 데이터 흐름을 담는다.
그리고 스키마 0바이트다 — 모델이 안쪽 도구가 아니라 `keeper_compose_<name>` 하나를
부르기 때문이다.

## 4.7 한 번 부른 도구는 다음 턴으로 이어진다 — 2026-08-31

§4.5 2단계의 로드는 그 턴의 agent 까지만 간다. 그래서 한 가지 일을 여러 턴에 걸쳐
하는 Keeper 는 매 턴 같은 도구를 다시 부른다. 실측: rondo 가 `github_issue_read` 를
turn 532~536 **연속**으로 다시 로드했고, 하루에 `github_get_label` 34번. 목록을 쓴
84턴 중 **39턴이 로드만 하고 아무것도 못 썼다** — 로드와 사용 사이에 턴이 끝났다.

대화가 이미 기록하고 있다. 부착 도구를 부르면 그 도구 이름을 담은 `ToolUse` 블록이
히스토리에 남는다. 그것을 읽어 다음 턴 표면에 미리 싣는다(#31968 → #31975).
**저장하지 않는다** — Claude API·Claude Code·Hermes 셋 다 히스토리에서 재도출하고,
사본이 없으면 크래시·재개·실패한 턴에 맞출 상태도 없다.

경계는 **요청이 아니라 실행**이다. #31968 은 요청한 것을 이어줬는데, 한 시간 뒤
polisher 가 111/133 까지 부풀어 절감이 33.9% → 14~25% 로 떨어지는 중이었다. 요청은
필요의 증거가 아니고, 사용은 자의적 숫자 없이 생기는 경계다.
...
### 5.1 첫 번째 지표는 한동안 아무것도 재지 않았다

`wire-capture` 의 `tool_schema_bytes` 는 번들이 **만든** 목록을 세고 있었지 레인이
**보낸** 것을 세지 않았다. 같은 함정이 넷이었다 — wire capture, wake payload, 턴
레코드 두 곳이 전부 지연 전 flat 리스트를 읽었다(#31894 → #31896·#31905).

그 결과 이 지표는 기능이 켜지든 꺼지든 안 움직였고, 재배포 전후의 변화를 근처에
머지된 커밋 탓으로 돌리게 만들었다. 실제로 2026-08-30 에 그렇게 잘못 판정했다.

**지연이 실제로 도는지는 `tool_calls` 로만 증명된다** — `keeper_tool_search` 성공 행의
`output` 이 `now callable: ...` 로 나온다.

### 5.2 실측 — 2026-08-31

계기를 고치고 세 변경(#31911·#31968→#31975·#31985)을 배포한 뒤, 같은 keeper 를 재기동
전후로 대조했다.

| keeper | 전 | 후 |
|---|---|---|
| code-reviewer (부착 없음) | 89개 76,464B | **77개 70,106B** |
| analyst | 133개 127,314B | **86개 85,967B** |
| polisher | 133개 127,314B | **98개 98,628B** |
| edgar.a.poe | 145개 141,702B | **94개 94,325B** |
| rondo · taskmaster | 133개 | 133개 — CLI 레인, masc 쪽 지연 없음(설계대로). 클라이언트가 자기 쪽에서 미룬다는 것은 RFC-0451 §2 에서 2026-09-16 에 확인됐다 |

- 부착 도구 44개 스키마 48,057B → 목록 하나 **4,920B (10.2%)**. §1.6 예측 9.2%
- `keeper_tool_search` 호출: turn 당 **1.21회 → 0회**. 히스토리 도출이 미리 실어서
  모델이 물어볼 필요가 없어졌다
- `keeper_attached_tool_loaded_unused`: **39/84 turn → 0**
- `stop=error:Context overflow`: 412건 → **0건**. 다만 이 축만의 공로라고 단정할 수
  없다. `identity/disabled.json` 을 보면 운영자가 2026-08-29 10:00:09Z 에
  edgar↔amplitude 를 스위치로 껐고 지금도 꺼져 있다 — §1.5 가 "우연히 멈췄다" 고 쓴
  그 상태가 유지되는 중이다. amplitude 를 다시 붙여봐야 이 축이 그것을 구조적으로
  막는지 증명된다.
