...
let normalize_score ~baseline ~target ~lower_is_better score =
  ...
  in
  let signed_progress =
    if lower_is_better then baseline -. score else score -. baseline
  in
...
let target_met ~target ~lower_is_better score =
  match target with
  | None -> false
  | Some t -> if lower_is_better then score <= t else score >= t
;;

let rationale_of_cycle
...
let verdict_of_cycle
  ...
  | Autoresearch.Discard ->
    Verification.Fail (Printf.sprintf "autoresearch Discard — %s" rationale)
  | Autoresearch.Keep ->
    if
      target_met
...
let attribution_of_cycle
  ...
  | Autoresearch.Discard ->
    Attribution.policy_failed
      ...
      ~reason:
        (Printf.sprintf
           "autoresearch Discard (loop %s cycle %d)"
           state.loop_id
           record.cycle)
  | Autoresearch.Keep ->
    if
