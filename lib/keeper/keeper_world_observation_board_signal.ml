...
(* Exact source/store boundary for Board read failures. *)
...
let read_failure_kind_of_error : Board.board_error -> read_failure_kind = function
  | Board.Post_not_found _ ->
    (* The post was deleted or swept from the store. Post ids are
       ...
       re-reading — the dominant real-world cause of the crash-loop this type
  ...
  | Board.Comment_not_found _ ->
    ...
    Source_rejected
  | Board.Invalid_id _ ->
    (* The id string embedded in the stimulus is malformed. *)
    Source_rejected
  | Board.Io_error _ ->
  ...
  | Board.Validation_error _ ->
    (* Not reachable from [get_post]/[get_comments] today (only write paths
       produce it). Classified [Source_rejected] for exhaustiveness: it signals
       the input itself fails a business rule. *)
    Source_rejected
  | Board.Already_voted _ ->
    (* Not reachable from a read path. Classified [Source_rejected]: it names an
       already-settled action conflict. *)
    Source_rejected
...
let read_failure_kind_of_unavailable (unavailable : board_unavailable) =
  read_failure_kind_of_error unavailable.error
;;
