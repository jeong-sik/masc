...
let partial_response_of_stop ~(session_id : string) ~(model_id : string) ~(text : string)
...
let enrich_idle_detail (detail : string) (messages : Oas.Types.message list) : string =
  if String.starts_with ~prefix:"Idle detected" detail
  then (
    let last_tool =
      let rec find = function
        ...
        | (m : Oas.Types.message) :: rest ->
          ...
          if Option.is_some later
          then later
          else if m.role = Oas.Types.Assistant
          then
            List.find_map
              (function
