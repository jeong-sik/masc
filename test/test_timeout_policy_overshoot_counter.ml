...
let test_within_slack_no_increment () =
  let deadline =
    make_deadline ~layer:TP.Layer.Oas_bridge ~origin:"slack-test-9662" ~wall_cap_s:573.0
  in
...
let test_past_slack_increments () =
  let deadline =
    make_deadline
      ~layer:TP.Layer.Oas_bridge
      ~origin:"keeper_llm_bridge_9662_test"
      ~wall_cap_s:573.0
  in
  let before = counter_for ~layer:"oas_bridge" ~origin:"keeper_llm_bridge_9662_test" in
  ...
  Alcotest.(check (float 0.0001))
    "counter +1"
    (before +. 1.0)
    (counter_for ~layer:"oas_bridge" ~origin:"keeper_llm_bridge_9662_test")
;;
...
let test_layer_isolation () =
  ...
  let oas = make_deadline ~layer:TP.Layer.Oas_bridge ~origin ~wall_cap_s:60.0 in
  let tool = make_deadline ~layer:TP.Layer.Tool ~origin ~wall_cap_s:60.0 in
...
let test_origin_isolation () =
  ...
  let alpha = make_deadline ~layer ~origin:"alpha-9662" ~wall_cap_s:60.0 in
  let _beta = make_deadline ~layer ~origin:"beta-9662" ~wall_cap_s:60.0 in
...
let () =
  Alcotest.run
    "timeout_policy_overshoot_counter_9662"
    [ ( "metric_name"
      , [ Alcotest.test_case "canonical name stable" `Quick test_metric_name_stable ] )
    ; ( "increment"
      , [ Alcotest.test_case
            "within slack → no increment"
            `Quick
            test_within_slack_no_increment
        ; Alcotest.test_case
            "past slack → +1 (exact #9662 case)"
            `Quick
            test_past_slack_increments
        ] )
    ; ( "isolation"
      , [ Alcotest.test_case "layer labels isolated" `Quick test_layer_isolation
        ; Alcotest.test_case "origin labels isolated" `Quick test_origin_isolation
        ] )
    ]
;;
