...
let update_keeper (ctx : _ context) (p : parsed_args) (old : keeper_meta) : tool_result =
  ...
  let tool_access =
    ...
    | None ->
        ...
        | Custom names -> (
            match Common.first_some p.tool_preset_opt profile_tool_preset with
            | Some preset ->
                let also_allow =
                  resolve_tool_name_list
                    ~preferred:p.tool_also_allow_opt
  ...
  let updated = { old with
    ...
    work_discovery_enabled =
      Common.first_some p.profile_defaults.work_discovery_enabled old.work_discovery_enabled;
    work_discovery_sources =
      Common.first_some p.profile_defaults.work_discovery_sources old.work_discovery_sources;
    work_discovery_interval_sec =
      Common.first_some p.profile_defaults.work_discovery_interval_sec
        old.work_discovery_interval_sec;
    work_discovery_guidance =
      Common.first_some p.profile_defaults.work_discovery_guidance old.work_discovery_guidance;
    telemetry_feedback_enabled =
      Common.first_some p.profile_defaults.telemetry_feedback_enabled
        old.telemetry_feedback_enabled;
    telemetry_feedback_window_hours =
      Common.first_some p.profile_defaults.telemetry_feedback_window_hours
        old.telemetry_feedback_window_hours;
    per_provider_timeout_s =
      Common.first_some p.profile_defaults.per_provider_timeout old.per_provider_timeout_s;
    always_approve =
      Common.first_some p.profile_defaults.always_approve old.always_approve;
    proactive = {
      enabled =
