      ...
      ${hasItems ? html`
        <div class="mb-3 flex items-center gap-2">
          <input
            ...
            class="min-w-40 max-w-70 flex-1 rounded border border-[var(--white-10)] bg-[var(--white-4)] px-2 py-1 text-2xs text-[var(--text-body)] placeholder:text-[var(--text-dim)] focus:outline-none focus:border-[var(--accent)] focus-visible:ring-2 focus-visible:ring-[var(--accent)]/50"
          />
        </div>
      ` : null}
      ${items.length === 0
        ...
        : isFiltering && visibleItems.length === 0
          ? html`
              <div class="py-4 text-center text-2xs text-[var(--text-dim)]" data-testid="keeper-hitl-approval-empty-filter" role="status">
