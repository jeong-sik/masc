...
let temp_dir prefix =
  ...
  let rec loop n =
    let path =
      Filename.concat base (Printf.sprintf "%s-%d-%06d" prefix (Unix.getpid ()) n)
    in
...
let json_bool key json = Yojson.Safe.Util.(json |> member key |> to_bool)
let json_string key json = Yojson.Safe.Util.(json |> member key |> to_string)
...
let test_missing_clone () =
  ...
  let json =
    Masc_mcp.Keeper_repo_readiness.inspect ~config ~meta ~repo:"jeong-sik/masc-mcp" ()
  in
...
let test_non_git_clone () =
  ...
  let meta = make_meta "keeper-one" in
  let clone_path =
    Masc_mcp.Keeper_repo_readiness.clone_path ~config ~meta ~repo_name:"masc-mcp"
  in
  mkdir_p clone_path;
  let json =
    Masc_mcp.Keeper_repo_readiness.inspect ~config ~meta ~repo:"jeong-sik/masc-mcp" ()
...
let test_invalid_repo_name () =
  ...
  let meta = make_meta "keeper-one" in
  let json =
    Masc_mcp.Keeper_repo_readiness.inspect ~config ~meta ~repo_name:"../escape" ()
  in
  check bool "not ok" false (json_bool "ok" json);
...
let set_workspace_origin_to_github ~repo =
  run_ok ~cwd:repo "git remote set-url origin https://github.com/jeong-sik/masc-mcp.git"
;;
...
let test_auto_provisionable_workspace_repo () =
  ...
  run_ok
    ~cwd:base_path
    (Printf.sprintf "git init --bare -q --initial-branch=main %s" (Filename.quote remote));
  run_ok
    ~cwd:base_path
    (Printf.sprintf "git clone -q %s %s" (Filename.quote remote) (Filename.quote repo));
  ...
  let meta = make_meta "keeper-one" in
  let json =
    Masc_mcp.Keeper_repo_readiness.inspect ~config ~meta ~repo_name:"masc-mcp" ()
  in
  ...
  check
    ...
    Yojson.Safe.Util.(
      json
      |> member "workspace_repo_origin"
      |> to_string
      |> fun url ->
      Masc_mcp.Tool_code_write.extract_github_org_repo url |> Option.value ~default:"")
;;
  ...
  run
    ...
    [ ( "inspect"
      , [ test_case "missing clone" `Quick test_missing_clone
        ...
        ; test_case
            "auto provisionable workspace repo"
            `Quick
            test_auto_provisionable_workspace_repo
        ; test_case
            "auto provisionable workspace repo after file storm"
            `Quick
            test_auto_provisionable_workspace_repo_after_file_storm
        ; test_case
            "auto provisionable workspace repo before hidden dir storm"
            `Quick
            test_auto_provisionable_workspace_repo_before_hidden_dir_storm
        ; test_case
            "auto provisionable workspace repo before wide workspace storm"
            `Quick
            test_auto_provisionable_workspace_repo_before_wide_workspace_storm
        ] )
    ]
;;
