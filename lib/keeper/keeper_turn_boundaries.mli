(** Where each finished keeper turn left the durable history (RFC
    ...
    - A reader orders the [Atom_history] lines of one trace by [end_atom], not
      ...
      ({!Keeper_owner}) -- and a reader may rely on that. [masc_keeper_clear]
      is not a turn and can append its line while a turn runs. [turn_ref] is
      still not a key: the turn number is read from the keeper's meta when the
      turn starts, and a trace rotation changes the trace while that count
      keeps running.
