...
let get_or_create_store ~base_path : Dated_jsonl.t =
  ...
  | _ ->
    let dir = Filename.concat (Common.masc_dir_from_base_path ~base_path) "discovery" in
...
let record_to_json (r : probe_record) : Yojson.Safe.t =
  ...
  let models_field =
    if r.models = []
    then []
    else [ "models", `List (List.map (fun s -> `String s) r.models) ]
  in
  let extras =
...
let record_probe ~base_path (endpoints : Llm_provider.Discovery.endpoint_status list) =
  ...
  | exn ->
    Log.Discovery.error "discovery_history: append failed: %s" (Printexc.to_string exn)
;;
...
let read_recent ~base_path ~count : Yojson.Safe.t list =
  ...
  | Eio.Cancel.Cancelled _ as e -> raise e
  | exn ->
    Log.Discovery.error "discovery_history: read failed: %s" (Printexc.to_string exn);
    []
;;
...
let read_range ~base_path ~since ~until : Yojson.Safe.t list =
  ...
  | exn ->
    Log.Discovery.error
      "discovery_history: read_range failed: %s"
...
let prune ~base_path ~days =
  try
    ...
    if deleted > 0
    then Log.Discovery.info "discovery_history: pruned %d old day-files" deleted
  with
  | Eio.Cancel.Cancelled _ as e -> raise e
  | exn ->
    Log.Discovery.error "discovery_history: prune failed: %s" (Printexc.to_string exn)
;;
