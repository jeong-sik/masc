(* test/test_provider_mcp_tool_omission_dedup_10097.ml

   #10097: repeated WARN lines per session for provider-specific
   keeper-bound runtime MCP omissions masked unrelated warnings
   in [grep WARN] output.  [record_provider_mcp_tool_omission] splits
   the signal:
...
let test_counter_increments_on_every_call () =
  T.reset_runtime_mcp_omission_dedup_for_tests ();
  let tool = "keeper_library_search_t1_10097" in
  let before = counter_for ~tool in
  T.record_provider_mcp_tool_omission ~provider ~tools:[ tool ];
  T.record_provider_mcp_tool_omission ~provider ~tools:[ tool ];
  T.record_provider_mcp_tool_omission ~provider ~tools:[ tool ];
...
let test_fingerprint_is_order_insensitive () =
  ...
  Alcotest.(check string)
    ...
    (T.runtime_mcp_omission_fingerprint [ a; b ])
    (T.runtime_mcp_omission_fingerprint [ b; a ])
...
let test_new_fingerprint_reprints_warn () =
  T.reset_runtime_mcp_omission_dedup_for_tests ();
  let fp1 =
    T.runtime_mcp_omission_fingerprint [ "keeper_shell_t3_10097" ]
  in
  let fp2 =
    T.runtime_mcp_omission_fingerprint
      [ "keeper_shell_t3_10097"; "keeper_fs_edit_t3_10097" ]
  in
  Alcotest.(check bool)
    "fp1 is fresh on first encounter"
    false
    (T.runtime_mcp_omission_fingerprint_seen fp1);
  Alcotest.(check bool)
  ...
  Alcotest.(check bool)
    ...
    false
    (T.runtime_mcp_omission_fingerprint_seen fp2);
  Alcotest.(check bool)
...
let test_per_tool_label_isolation () =
  ...
  T.record_provider_mcp_tool_omission ~provider ~tools:[ x ];
  Alcotest.(check (float 0.0001))
...
(* Empty list must be a pure no-op: no counter, no WARN.  The
   call site checks for non-empty omitted tools before
   [record_provider_mcp_tool_omission] was introduced, and the
...
let test_empty_tools_is_noop () =
  ...
  T.record_provider_mcp_tool_omission ~provider ~tools:[];
  Alcotest.(check (float 0.0001))
    "probe bucket unchanged"
    before
    (counter_for ~tool:probe);
  let empty_fp = T.runtime_mcp_omission_fingerprint [] in
  ...
  Alcotest.(check bool)
    ...
    false
    (T.runtime_mcp_omission_fingerprint_seen empty_fp);
  (* Clean up the slot we just opened so other tests are not
     affected by the side-effect check above. *)
  T.reset_runtime_mcp_omission_dedup_for_tests ()

let () =
  Alcotest.run "provider_mcp_tool_omission_dedup_10097"
    [
      ( "counter",
        [
