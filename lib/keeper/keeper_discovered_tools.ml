...
let create ~decay_turns =
  { entries = Hashtbl.create 16; decay_turns = max 1 decay_turns }
...
let is_active ~decay_turns ~turn (e : entry) =
  turn - e.last_active_turn <= decay_turns

let active_names t ~turn =
  Hashtbl.fold (fun _key (e : entry) acc ->
...
let decay t ~turn =
  let expired = ref [] in
  Hashtbl.filter_map_inplace (fun _key (e : entry) ->
    if is_active ~decay_turns:t.decay_turns ~turn e then
      Some e
    else begin
