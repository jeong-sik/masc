...
| 2 | 측선 도입 | journal 과 남이 보낸 줄이 `╌╌╌┤`·`───┤` 로 합류 | journal 이 턴 사이에 끼어도 턴이 끊기지 않음 | #33641 |
| 3 | 행 단위 접힘 | 한 행을 접고 펴는 상태 | 접힌 행이 몇 줄을 쥐고 있는지 밝힘 | |
| 4 | 채팅 패널 마우스 | 클릭 접기/펴기 | 마우스 해제 시 키보드 경로 동일 | |
...
**긴 원문을 접으려면 접을 자리가 먼저 있어야 한다.** `Masc_tui_gate_text`
의 `lifecycle_line` 은 `tool · summary` 를 조립하는데 `summary` 에 상한이
없다. base64 인자 하나가 화면 여덟 줄을 먹는다. 여기에 상한을 두면 그것은
잘라내기이고, 잘라낸 것은 펼칠 수 없다. 접으려면 행 하나를 접었다 펴는
상태가 있어야 하는데 지금은 전역 뷰 상태 4축(`memory`·`reasoning`·`tools`
·`origin`, `chat_visibility_summary` 가 함께 요약한다)뿐이고 행 단위가 없다.
그래서 3 이 원문 문제의 선행 조건이다.

"레인 필터"는 증분에서 뺐다. 전역 축은 이미 한 함수를 공유하고 있어,
새로 만들 것이 아니라 이미 있는 것이다.
...
4. 접힘 상태를 무엇으로 키잉하는가. `(keeper, request_id, entry_index)` 는
   히스토리가 재정렬되면 다른 행을 가리킨다.
