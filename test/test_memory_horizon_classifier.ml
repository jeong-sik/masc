...
let test_json_wrapper_falls_back_to_kind () =
  ...
  check
    string
    "JSON unknown horizon falls through to kind classification"
    Policy.short_term_horizon
    (Policy.memory_horizon_of_json ~kind:"next" json_unknown);
  let json_known = `Assoc [ "horizon", `String "long_term"; "kind", `String "next" ] in
  check
    string
    "JSON known horizon takes precedence over kind"
...
let () =
  Alcotest.run
    ...
    [ ( "strict classifier"
      , [ test_case "known kinds return Some" `Quick test_known_kinds_return_some
        ; test_case "unknown kinds return None" `Quick test_unknown_kinds_return_none
        ; test_case
    ...
    ; ( "back-compat wrappers"
      , [ test_case
            "wrapper preserves legacy default"
            `Quick
            test_wrapper_preserves_legacy_default
        ; test_case
            "JSON wrapper falls back to kind"
            `Quick
