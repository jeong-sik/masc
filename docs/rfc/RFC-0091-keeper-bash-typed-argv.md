...
- **interactive shell** 또는 **REPL**. typed argv는 single-shot 실행만 지원.
  ...
  로 분리되며 본 RFC 범위 밖.
- **command-string schema 호환**. RFC 머지 PR이 `lib/worker_dev_tools.ml`
  lexer를 *같은 PR에서* 삭제한다. transitional dual-accept(`cmd: string`
  계속 받으면서 typed 추가)는 hardcoding/string-boundary zero-tolerance 원칙
  위반.
...
4. 신규 test `test/test_keeper_bash_typed_input.ml` 가 동일 입력에 대해
   command-string lexer 와 typed schema 가 동일한 외부 효과(stdout/stderr/exit)
...
### PR-3 — tool descriptor JSON schema 변경

Implementation update (2026-05-21): the descriptor variant gate has been
removed. `keeper_bash` now emits a single typed-only JSON schema.

1. `lib/tool_shard_types_schemas_bash.ml` no longer contains the descriptor
   selector gate or its two historical variants.
2. The active schema:
   - removes `cmd` from `input_schema.properties`
   - removes the `cmd` branch from `oneOf`
   - exposes only `executable | pipeline | stages`
   - keeps no top-level `required` list, because `oneOf` is the discriminator
3. Reader-side reject remains explicit:
   `lib/keeper/keeper_tool_bash_input.ml` rejects `{"cmd": ...}` at the typed
   boundary with a typed `Result.Error`.
4. Rollback is a code revert, not an environment flip. There is no remaining
   runtime setting that can silently resurrect the command-string descriptor.
5. Remaining out-of-scope work: public `Bash { command }` still maps through
   the internal raw-command execution adapter. That is a separate public
   front-door migration, not the removed model-facing `keeper_bash` descriptor.
...
1. **keeper LLM 적응 transition**. The model-facing `keeper_bash` descriptor is
   typed-only. Public `Bash { command }` remains the compatibility front door
   for shell-command strings, so the descriptor change should not strand
   keepers that use the public alias. Monitor `Boundary_invalid` after merge;
   rollback requires reverting the descriptor removal commit.
2. **외부 MCP 서버**. masc-mcp 외부 클라이언트 (Claude Code, Codex, Gemini)
   가 string cmd 로 호출하는 케이스. RFC-0084 PR-11 (old dispatch mli surface
...
- hardcoding/string-boundary zero-tolerance feedback — root-fix PR 가
  같은 머지에서 old compatibility path 동시 삭제 의무.
...
- [ ] PR-2 caller 전수 변환 + lexer 삭제
- [x] PR-3 tool descriptor JSON schema typed-only; descriptor variant gate removed
- [ ] PR-4 godfile 분해
