...
let make_args (messages : (string * string) list) : Yojson.Safe.t =
  let msg_json =
    List.map
      (fun (role, content) -> `Assoc [ "role", `String role; "content", `String content ])
      messages
  in
  `Assoc
...
let test_backend_create_remote () =
  ...
  check
    string
    "kind"
    "remote(https://s3.example.com/bucket)"
    (Fs_compat.backend_kind_to_string b.kind)
;;

let test_backend_create_local_explicit () =
  let b = Fs_compat.create_backend ~kind:Fs_compat.Local ~base_path:"/var/data" () in
...
let () =
  ...
  Eio_guard.enable ();
  run
    "tool_compact + fs_compat_backend"
    [ ( "compact_dispatch"
      , [ test_case
            "compact_context dispatch gutted"
            `Quick
            test_dispatch_returns_none_for_compact_context
        ; test_case "unknown tool name" `Quick test_dispatch_returns_none_for_unknown_tool
        ] )
    ; ( "fs_compat_backend"
