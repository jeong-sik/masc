...
let base_path_voice_config_path_opt () =
  Env_config_core.base_path_opt () |> Option.map voice_config_file_in
;;
...
let parse_endpoint ~ctx json =
  ...
  let enabled = Yojson.Safe.Util.member "enabled" json |> bool_or_default true in
  let timeout_seconds = Yojson.Safe.Util.member "timeout_seconds" json |> float_opt in
...
let rec parse_endpoints ~ctx acc = function
  | [] ->
    ...
    if endpoints = []
    then Error (Printf.sprintf "%s must not be empty" ctx)
    else Ok endpoints
  | item :: rest ->
    ...
    let rec loop acc = function
      ...
      | (agent_id, value) :: rest ->
        (match trim_nonempty value with
         | Some voice -> loop ((String.trim agent_id, voice) :: acc) rest
         | None ->
           Error
             (Printf.sprintf "tts.agent_voices.%s must be a non-empty string" agent_id))
    in
    loop [] pairs
  | `Null -> Ok []
...
let parse_voice_tuning ~ctx json =
  ...
  | `Assoc _ ->
    Ok
      { stability = Yojson.Safe.Util.member "stability" json |> float_or_default 0.5
      ; similarity_boost =
          Yojson.Safe.Util.member "similarity_boost" json |> float_or_default 0.75
      ; style = Yojson.Safe.Util.member "style" json |> float_or_default 0.0
      }
  | `Null -> Ok { stability = 0.5; similarity_boost = 0.75; style = 0.0 }
  ...
  | `Assoc pairs ->
    let rec loop acc = function
      ...
      | (agent_id, tuning_json) :: rest ->
        (match
           parse_voice_tuning ~ctx:("tts.agent_voice_settings." ^ agent_id) tuning_json
         with
...
let parse_session json =
  ...
  let* endpoints_json = require_list ~ctx:"session" ~field:"endpoints" session_json in
...
let parse_local_playback json =
  ...
  | `Assoc local_json ->
    ...
    let enabled = Yojson.Safe.Util.member "enabled" local_json |> bool_or_default false in
    let agents =
...
let load () =
  ...
  then Error (Printf.sprintf "voice config missing at %s" path)
  else (
    try
    ...
    | Sys_error error -> Error (Printf.sprintf "voice config read failed: %s" error)
    | Eio.Io _ as exn ->
...
let available_voices config =
  config.tts.default_voice :: List.map snd config.tts.agent_voices |> unique_strings
;;
...
let active_endpoint_json endpoints = endpoint_public_json endpoints
  ...
  `Assoc
    ...
    ; ( "tts"
      , `Assoc
          ...
          ; ( "available_voices"
            , `List (List.map (fun voice -> `String voice) (available_voices config)) )
