...
let parse_args () =
  ...
  Arg.parse
    specs
    (fun s -> role := s)
    "test_a2a_distributed [producer|consumer] --dir <path>"
;;
...
let producer () =
  ...
  for i = 1 to 3 do
    let content =
      Printf.sprintf "Message %d from producer at %f" i (Unix.gettimeofday ())
    in
    ...
    | Error e ->
      Printf.printf "[Producer] Broadcast failed: %s\n%!" e;
...
let consumer () =
  ...
  let rec read_messages seq found =
    ...
    then found
      ...
      | Ok msg ->
        Printf.printf
          "[Consumer] Found message seq=%d from=%s: %s\n%!"
          msg.seq
          msg.from_agent
          (String.sub msg.content 0 (min 50 (String.length msg.content)));
        read_messages (seq + 1) (found + 1)
      | Error _ -> if seq < 100 then read_messages (seq + 1) found else found)
  in
