...
let with_tmpdir f =
  let dir =
    Filename.concat
      ...
      (Printf.sprintf "test_eval_gate_%d" (Random.int 100000))
  in
...
let test_detect_force_push () =
  ...
  | Some (_pat, desc) -> Alcotest.(check string) "force push desc" "force push" desc
...
let test_detect_drop_table () =
  ...
  | Some (_pat, desc) -> Alcotest.(check string) "drop table desc" "SQL table drop" desc
  ...
  | Trajectory.Reject reason ->
    Alcotest.(check bool)
      ...
      (String.length reason > 0
       ...
       try
         ignore (Str.search_forward (Str.regexp_string "deny") r 0);
         true
       with
       | Not_found -> false)
...
let test_pre_cost_exceeded () =
  ...
  | Trajectory.Reject reason ->
    Alcotest.(check bool)
      ...
      (let r = String.lowercase_ascii reason in
       try
         ignore (Str.search_forward (Str.regexp_string "cost") r 0);
         true
       with
       | Not_found -> false)
...
let test_pre_destructive_bash () =
  let decision =
    Eval_gate.pre_check
      ...
      ~trajectory_acc:None
      ~tool_name:"keeper_bash"
      ~args_json:"{\"command\": \"rm -rf /tmp/dangerous\"}"
  in
  match decision with
  | Trajectory.Reject reason ->
    Alcotest.(check bool)
      ...
      (let r = String.lowercase_ascii reason in
       try
         ignore (Str.search_forward (Str.regexp_string "destructive") r 0);
         true
       with
       | Not_found -> false)
  ...
  with_tmpdir (fun dir ->
    ...
    | Trajectory.Reject reason ->
      Alcotest.(check bool)
        ...
        (let r = String.lowercase_ascii reason in
         try
           ignore (Str.search_forward (Str.regexp_string "entropy") r 0);
           true
         with
         | Not_found -> false)
  ...
  with_tmpdir (fun dir ->
    ...
    let decision =
      Eval_gate.pre_check
        ...
        ~trajectory_acc:(Some acc)
        ~tool_name:"keeper_bash"
        ~args_json:"{\"command\": \"echo d\"}"
    in
    match decision with
    | Trajectory.Reject _ ->
      Alcotest.fail "Should NOT reject: different args do not form an entropy streak"
    | Trajectory.Pass -> ())
;;
  ...
  with_tmpdir (fun dir ->
    ...
    | Trajectory.Reject reason ->
      Alcotest.(check bool)
        ...
        (let r = String.lowercase_ascii reason in
         try
           ignore (Str.search_forward (Str.regexp_string "turn") r 0);
           true
         with
         | Not_found -> false)
  ...
  | Some w ->
    Alcotest.(check bool)
      ...
      (let r = String.lowercase_ascii w in
       try
         ignore (Str.search_forward (Str.regexp_string "slow") r 0);
         true
       with
       | Not_found -> false)
  ...
  | Some result ->
    Alcotest.(check bool)
      ...
      (let r = String.lowercase_ascii result in
       try
         ignore (Str.search_forward (Str.regexp_string "tool execution failed") r 0);
         true
       with
       | Not_found -> false);
    Alcotest.(check bool) "has eval" true (eval_opt <> None)
  ...
  Alcotest.run
    ...
    ; ( "pre_check"
      , [ Alcotest.test_case "deny list" `Quick test_pre_deny_list
        ...
        ; Alcotest.test_case
            "entropy different args"
            `Quick
            test_pre_entropy_different_args
