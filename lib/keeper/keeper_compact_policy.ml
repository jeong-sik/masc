...
let compact_if_needed ~(meta : keeper_meta) ~(now_ts : float) (ctx : working_context)
    ...
    then
      Some
        (Printf.sprintf
           ...
           hold_s
           meta.compaction.cooldown_sec)
    else if ratio >= ratio_gate
    then Some (Printf.sprintf "ratio(%.4f>=%.4f)" ratio ratio_gate)
    else if message_gate > 0 && msg_count >= message_gate
    then Some (Printf.sprintf "messages(%d>=%d)" msg_count message_gate)
    else if token_gate > 0 && tok_count >= token_gate
    then Some (Printf.sprintf "tokens(%d>=%d)" tok_count token_gate)
    else if tool_heavy
    then Some (Printf.sprintf "tool_heavy(msgs=%d,ratio=%.4f)" msg_count ratio)
    else None
  in
  match trigger_reason with
  | None -> ctx, None, "blocked:below_thresholds"
  | Some reason ->
    if String.starts_with ~prefix:"skipped:" reason
    then ctx, None, reason
    else (* PreCompact observability: log strategy and context state (#3165) *)
      (
      let strategies =
      ...
      Prometheus.inc_counter
        Prometheus.metric_keeper_compactions
        ~labels:[ "keeper", meta.name ]
        ();
      Prometheus.set_gauge
        Prometheus.metric_keeper_compaction_ratio_change
        ~labels:[ "keeper", meta.name ]
        (ratio -. new_ratio);
      Prometheus.inc_counter
        Prometheus.metric_keeper_compaction_saved_tokens
        ~labels:[ "keeper", meta.name ]
