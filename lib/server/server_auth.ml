...
let configured_bind_host () = Env_config_core.masc_host ()
...
let http_auth_bind_host () = configured_bind_host ()
let http_auth_bind_is_loopback () = is_loopback_host (configured_bind_host ())

let strict_http_auth_error endpoint =
  Printf.sprintf
    "%s requires room auth enabled with require_token=true when server is bound to a \
     non-loopback host or MASC_HTTP_BASE_URL points to a public address."
    endpoint
;;

let ensure_strict_http_token_auth ~endpoint auth_config =
  if not (http_auth_strict_enabled ())
  then Ok auth_config
  else if not auth_config.Types.enabled
  then Error (strict_http_auth_error endpoint)
  else if not auth_config.require_token
  then Error (strict_http_auth_error endpoint)
  else Ok auth_config
;;
...
let bearer_token_from_header value =
  let prefix_len = 7 in
  (* String.length "Bearer " *)
  if
    String.length value > prefix_len && String_util.starts_with_ci ~prefix:"Bearer " value
...
let auth_token_from_request request =
  ...
  | None ->
    trim_opt (Httpun.Headers.get request.Httpun.Request.headers "x-masc-internal-token")
;;
...
let observer_sse_query_token_from_request request =
  ...
  let observer_stream_requested =
    ...
    | Some raw -> String.equal "observer" (String.lowercase_ascii (String.trim raw))
    | None -> false
  in
...
let strip_prefix ~prefix value =
  ...
  if String.length value > prefix_len && String.starts_with ~prefix value
  then String.sub value prefix_len (String.length value - prefix_len)
  else value
;;
...
let strip_suffix ~suffix value =
  ...
  if value_len > suffix_len && String.ends_with ~suffix value
...
let internal_keeper_agent_from_request request =
  ...
  | Some raw ->
    ...
    in
    if normalized = "" then None else Some (Printf.sprintf "keeper-%s-agent" normalized)
;;
...
let resolve_agent_name_for_auth_raw ~base_path request ~token
  ...
  | Some t when Auth.verify_internal_keeper_token base_path ~token:t ->
    (match internal_keeper_agent_from_request request with
     ...
     | None ->
       Error
         (Types.Unauthorized "Internal keeper auth requires x-masc-keeper-name header."))
  | Some t ->
...
let verify_mcp_auth ~base_path request =
  ...
  | Error msg -> Error msg
  | Ok auth_config ->
    if not auth_config.Types.enabled
    then Ok None (* Auth disabled - allow all *)
    else (
      match auth_token_from_request request with
      | None when not auth_config.require_token -> Ok None (* Token not required *)
      | None ->
...
let verify_mcp_observer_stream_auth ~base_path request =
  ...
  | Ok auth_config ->
    ...
    then Ok None
      ...
      | None when not auth_config.require_token -> Ok None
      | None ->
        Error
          "Authentication required. Use 'Authorization: Bearer <token>' header or \
           'token' query param for the observer SSE stream."
      | Some token ->
        (match resolve_agent_name_for_auth_raw ~base_path request ~token:(Some token) with
         | Error err -> Error (Types.masc_error_to_string err)
         | Ok agent_name_opt ->
...
let verify_operator_mcp_auth ~base_path request =
  let auth_config = Auth.load_auth_config base_path in
  if not auth_config.Types.enabled
  then Error "/mcp/operator requires room auth enabled with require_token=true."
  else if not auth_config.require_token
  then Error "/mcp/operator requires bearer token auth (require_token=true)."
  else (
    match auth_token_from_request request with
    | None -> Error "Authentication required. Use 'Authorization: Bearer <token>' header."
    | Some token ->
      (match resolve_agent_name_for_auth_raw ~base_path request ~token:(Some token) with
       | Error err -> Error (Types.masc_error_to_string err)
       | Ok agent_name_opt ->
  ...
  | Some token ->
    (match resolve_agent_name_for_auth_raw ~base_path request ~token:(Some token) with
     ...
     | Ok None ->
       ...
       Log.Auth.warn
         "[silent:dashboard_actor_fallback] outcome=none - bearer token resolved to no \
     ...
     | Error _ ->
       Log.Auth.warn
         "[silent:dashboard_actor_fallback] outcome=error - bearer token resolution \
          errored, falling back to request actor hint";
       Prometheus.inc_counter
         Prometheus.metric_silent_dashboard_actor_fallback
...
let host_port_scheme_of_origin origin =
  try
    ...
    | None -> None
    | Some host ->
      Some (String.trim host |> String.lowercase_ascii, Uri.port uri, Uri.scheme uri)
  with
  | Eio.Cancel.Cancelled _ as e -> raise e
  | exn ->
    Log.Auth.debug
      "host_port_scheme_of_origin: parse failed for %S: %s"
      origin
...
let host_port_of_request request =
  ...
  | Some host_header ->
    (try
       ...
       match Uri.host uri with
       | None -> None
       | Some host -> Some (String.trim host |> String.lowercase_ascii, Uri.port uri)
     with
     | Eio.Cancel.Cancelled _ as e -> raise e
     | exn ->
       Log.Auth.debug
         "host_port_of_request: parse failed for %S: %s"
         host_header
         (Printexc.to_string exn);
       None)
;;
...
let default_loopback_dev_mutation_origins = Masc_network_defaults.vite_dev_default_origins
...
let ensure_same_origin_browser_request request : (unit, Types.masc_error) result =
  match Httpun.Headers.get request.Httpun.Request.headers "origin" with
  | None ->
    if allow_anonymous_mutations
    then Ok ()
    else
      Error
        (Types.Unauthorized
           "Authentication required: provide a bearer token or Origin header. Set \
            MASC_ALLOW_ANONYMOUS_MUTATIONS=true for local development.")
  | Some origin ->
    (match host_port_scheme_of_origin origin, host_port_of_request request with
     | Some (origin_host, origin_port, scheme), Some (request_host, request_port)
       when String.equal
       ...
       if norm origin_port = norm request_port
       then Ok ()
       else if
...
let get_origin (request : Httpun.Request.t) =
  Httpun.Headers.get request.headers "origin" |> Option.value ~default:"*"
;;
...
let cors_headers origin =
  ...
  if origin <> "*" then ("access-control-allow-credentials", "true") :: base else base
;;
...
let auth_error_json err =
  Yojson.Safe.to_string (`Assoc [ "error", `String (Types.masc_error_to_string err) ])
;;
...
let respond_auth_error request reqd err =
  ...
  let headers =
    Httpun.Headers.of_list
      (("content-length", string_of_int (String.length body)) :: cors_headers origin)
  in
  ...
  | Some state ->
    ...
    (match admin_token, provided with
     | None, _ ->
       Http_server_eio.Response.json
         ~status:`Forbidden
         {|{"error":"MASC_ADMIN_TOKEN not configured"}|}
         reqd
     | Some _, None ->
       Http_server_eio.Response.json
         ~status:`Unauthorized
     ...
     | Some expected, Some given ->
       ...
       if !diff = 0
       then handler state request reqd
       else
         Http_server_eio.Response.json
           ~status:`Forbidden
         ...
         Error
           (Types.Unauthorized
              "Agent name required (X-Gate-Agent / X-MASC-Agent or token-bound \
               credential)")
       else Auth.check_permission base_path ~agent_name ~token ~permission)
;;
...
let authorize_tool_request ~base_path ~tool_name request : (unit, Types.masc_error) result
  =
  let auth_cfg = Auth.load_auth_config base_path in
  let token = auth_token_from_request request in
  match
    if Option.is_some token then Ok () else ensure_same_origin_browser_request request
  with
  | Error err -> Error err
  | Ok () ->
    (match
       ensure_strict_http_token_auth
         ~endpoint:("HTTP tool access for " ^ tool_name)
         auth_cfg
     with
     | Error msg -> Error (Types.Unauthorized msg)
     | Ok auth_cfg ->
          ...
          then
            Error
              (Types.Unauthorized
                 "Agent name required (X-Gate-Agent / X-MASC-Agent or token-bound \
                  credential)")
          else Auth.authorize_tool_v2 base_path ~agent_name ~token ~tool_name))
;;
...
let authorize_token_bound_permission_request ~base_path ~permission request
  ...
  else if not auth_cfg.require_token
  then
    Error
      (Types.Unauthorized "HTTP mutation requires bearer token auth (require_token=true).")
  else (
    match auth_token_from_request request with
    | None ->
      Error
    ...
    | Some token ->
      (match Auth.find_credential_by_token base_path ~token with
       | Error err -> Error err
       | Ok cred ->
         if Types.has_permission cred.role permission
         then Ok cred.agent_name
         else
           Error
             (Types.Forbidden
                { agent = cred.agent_name; action = Types.show_permission permission })))
;;
...
let not_initialized_response path =
  if is_dashboard_bootstrap_path path
  then {|{"status":"initializing","message":"Server is warming up"}|}
...
let rec with_public_read handler request reqd =
  ...
  let path = Http_server_eio.Request.path request in
  if strict && not (is_public_read_path path)
  then with_read_auth handler request reqd
  else (
    match !server_state with
...
let serve_agent_card ~host ~port request reqd =
  with_read_auth
    (fun _state req reqd ->
       ...
       in
       let _card, json =
         Agent_card.get_cached
           ~host:resolved_host
           ~port:resolved_port
           ~schemas:Config.raw_all_tool_schemas
           ()
       in
