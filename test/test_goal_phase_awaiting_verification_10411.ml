...
let assert_ok ~msg ~phase ~action expected =
  ...
  | Error e -> failf "%s — expected %s, got Error %S" msg (outcome_to_string expected) e
  | Ok actual ->
    check
      bool
      (Printf.sprintf "%s (got %s)" msg (outcome_to_string actual))
      true
      (outcome_eq expected actual)
;;

let assert_error ~msg ~phase ~action =
  match decide ~phase ~action with
  | Ok actual -> failf "%s — expected Error but got Ok %s" msg (outcome_to_string actual)
  | Error _ -> ()
;;
...
let test_reject_blocks_goal () =
  assert_ok
    ...
    ~phase:GP.Awaiting_verification
    ~action:GP.Reject_completion
    (GP.Move_to GP.Blocked)
;;
...
let all_phases =
  [ GP.Executing
  ; GP.Awaiting_verification
  ; GP.Awaiting_approval
  ; GP.Blocked
  ; GP.Paused
  ; GP.Completed
  ; GP.Dropped
...
let all_actions =
  [ GP.Request_complete
  ; GP.Approve_completion
  ; GP.Reject_completion
  ; GP.Pause
  ; GP.Resume
  ; GP.Operator_block
  ; GP.Operator_unblock
...
let test_every_phase_has_at_least_one_exit () =
  List.iter
    (fun phase ->
       let any_ok =
         List.exists (fun action -> Result.is_ok (decide ~phase ~action)) all_actions
       in
       check
         bool
         (Printf.sprintf
            "phase %s has at least one outgoing transition"
            (GP.to_string phase))
         true
         any_ok)
    all_phases
...
let () =
  run
    "goal_phase_awaiting_verification_10411"
    [ ( "verification-verdicts"
      , [ test_case "Approve_completion → Complete" `Quick test_approve_completes_goal
        ; test_case "Reject_completion → Blocked" `Quick test_reject_blocks_goal
        ] )
    ; ( "manual-escapes"
      , [ test_case "Pause → Paused" `Quick test_pause_moves_to_paused
        ; test_case "Operator_block → Blocked" `Quick test_operator_block_moves_to_blocked
        ] )
    ; ( "regressions"
      , [ test_case "Drop → Dropped still works" `Quick test_drop_still_works
        ; test_case "invalid actions still rejected" `Quick test_invalid_actions_rejected
        ] )
    ; ( "escapability"
      , [ test_case
            "every phase has ≥1 outgoing transition"
            `Quick
