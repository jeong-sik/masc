...
그 셋 가운데 첫째는 §1.2.2 가 이미 쟀다. 저장소가 담고 있고 지금 쓰이는 컴포지션 7개 가운데 §3.1 이 기록만 보고 되살리는 것은 하나도 없다. 구현에 들어가기 전에 둘 중 하나를 정해야 한다. 조건 2 를 그 일곱이 통과하도록 고칠지, 아니면 이미 쓰이는 것을 재현하지 못한다는 사실을 받아들이고 새 시퀀스만 찾을지.
