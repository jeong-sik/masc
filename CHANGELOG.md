...
- Board attention: set `MASC_TYPESAFEAI_ENABLED=false` if the server's environment carries `TYPESAFEAI_API_KEY` and Board posts should not leave the machine. A non-blank key alone turns on a first pass that, when the `board_attention_exact` lane has an HTTP slot, sends the Board post and the keeper's context to TypeSafe AI's Jev (`https://api.typesafe.ai/v1/systemone`, overridable with `MASC_TYPESAFEAI_ENDPOINT`). Without the key nothing changes (#36970).
