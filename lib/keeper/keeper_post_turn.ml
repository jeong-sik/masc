...
let apply_post_turn_lifecycle
  ...
  let apply_continuity_summary
        ~(meta : keeper_meta)
        ~(ctx : working_context)
        ~(oas_checkpoint : Oas.Checkpoint.t option)
    : keeper_meta
    =
    let progress_path =
      Filename.concat
        (Filename.concat
           (Filename.concat (Filename.dirname base_dir) "keepers")
           meta.name)
        "progress.md"
    in
    let structured_snapshot =
      ...
      | Some cp ->
        (match cp.Oas.Checkpoint.working_context with
         | Some json -> Keeper_memory_policy.snapshot_of_structured_working_context json
         | None -> None)
      | None -> None
    in
    let snapshot =
    ...
    | Some snapshot ->
      ...
      (match
       ...
       | Ok () -> ()
       | Error err ->
         Log.Keeper.warn "keeper:%s progress snapshot write failed: %s" meta.name err);
      { meta with
  ...
  | Some cp ->
    ...
    in
    let compaction_decided = String.starts_with ~prefix:"applied:" decision in
    ...
    let effective_compaction_applied, compaction_failure_reason, effective_ctx, checkpoint
      =
      if not compaction_decided
      then false, None, ctx, Some cp
      else (
        let () = on_compaction_started () in
        let session =
          create_session
            ~session_id:(Keeper_id.Trace_id.to_string base_meta.runtime.trace_id)
            ~base_dir
        in
        let compacted_ctx =
          { compacted_ctx with
            checkpoint =
              { (checkpoint_of_context compacted_ctx) with
                messages =
                  repair_orphan_tool_result_messages (messages_of_context compacted_ctx)
              }
          }
        in
        match
      ...
      map_runtime
        (fun rt ->
           { rt with
             compaction_rt =
               { count =
                   (rt.compaction_rt.count + if effective_compaction_applied then 1 else 0)
               ; last_ts =
                   (if effective_compaction_applied
                    then now_ts
...
let forced_overflow_retry_meta
  ...
  =
  let base_meta =
    if turn_generation = meta.runtime.generation
    then meta
...
let recover_latest_checkpoint_for_overflow_retry
  ...
  let oas_checkpoint =
    ...
    |> Option.map (fun checkpoint ->
      let sanitized, stats = sanitize_oas_checkpoint ~repair_orphans:false checkpoint in
      if checkpoint_sanitize_changed stats
      then (
        Log.Keeper.warn
          "keeper:%s overflow-retry migration sanitized messages: dropped_blocks=%d \
  ...
  let legacy_checkpoint =
    ...
    | exn ->
      Log.Keeper.error
        "keeper:%s overflow retry checkpoint load failed: %s"
        (Keeper_id.Trace_id.to_string meta.runtime.trace_id)
  ...
  | Some (ctx, turn_generation) ->
    ...
    let ctx =
      ...
      then ctx
        ...
        sync_oas_context
          (with_max_tokens ctx (min (max_tokens_of_context ctx) primary_model_max_tokens))
    in
    let before_tokens = token_count ctx in
    let retry_meta = forced_overflow_retry_meta meta ~turn_generation ~now_ts in
    ...
    let compaction_applied = String.starts_with ~prefix:"applied:" base_decision in
    let meaningful_reduction = after_tokens < before_tokens in
    if not (compaction_applied && meaningful_reduction)
    then None
    else (
      let compaction =
        { attempted = true
      ...
      try
        ...
        with
        | Ok checkpoint -> Some { checkpoint; compaction; turn_generation }
        | Error e ->
          Log.Keeper.error "overflow retry checkpoint save failed: %s" e;
          None
      with
      | Eio.Cancel.Cancelled _ as exn -> raise exn
      | exn ->
        log_keeper_exn ~label:"overflow retry checkpoint save exception" exn;
        None)
;;
